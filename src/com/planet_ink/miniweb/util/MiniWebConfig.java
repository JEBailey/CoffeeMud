...
public class MiniWebConfig implements Cloneable {
	...
	private static final String DEFAULT_PAGE = "index.html"; // this would
																// normally be
																// configurable
																// as a list
	...
	private static final int DEFAULT_THREAD_KEEP_ALIVE_MS = 60 * 1000; // max
																		// age
																		// of
																		// idle
																		// threads
	private static final int DEFAULT_THREAD_TIMEOUT_SECS = 30; // Timeout for
																// active
																// request
																// threads
	private static final int DEFAULT_THREAD_QUEUE_SIZE = 500;// Above this and
																// they start
																// getting
																// rejected

	private static final long DEFAULT_FILECACHE_EXPIRE_MS = 5 * 60 * 1000; // 5
																			// minutes
																			// --
																			// how
																			// long
																			...
																			// entry
																			// lived
	private static final long DEFAULT_FILECACHE_MAX_BYTES = 10 * 1024 * 1024; // the
																				// maximum
																				// number
																				// of
																				// bytes
																				...
																				// hold
																				// total
	private static final long DEFAULT_FILECACHE_MAX_FBYTES = 2 * 1024 * 1024; // the
																				// maximum
																				// size
																				// of
																				// file
	...
	private static final long DEFAULT_FILECOMP_MAX_FBYTES = 16 * 1024 * 1024; // the
																				// maximum
																				// size
																				// of
																				// file
	...
	private static final long DEFAULT_MAX_BODY_BYTES = 1024 * 1024 * 2; // maximum
																		// size
																		// of a
																		// request
																		// body
	private static final long DEFAULT_MAX_IDLE_MILLIS = 30 * 1000; // maximum
																	// time a
																	// request
																	// can be
																	// idle
																	// (between
																	// reads)
	private static final int DEFAULT_LINE_BUFFER_SIZE = 4096; // maximum length
																// of a single
																// line in the
																// main request
	private static final int DEFAULT_MAX_ALIVE_SECS = 15; // maximum age, in
															// seconds, of a
															// request
															// connection
	private static final int DEFAULT_MAX_PIPELINED_REQUESTS = 10; // maximum
																	// number of
																	// requests
																	// per
																	// connection
	...
	private static final long DEFAULT_SESSION_IDLE_MILLIS = 30 * 60 * 1000; // maximum
																			// time
																			// a
																			// session
																			// can
																			...
																			// (between
																			// requests)
	private static final long DEFAULT_SESSION_AGE_MILLIS = 24 * 60 * 60 * 1000; // maximum
																				// time
																				// a
																				// session
																				// can
	...
	/**
	 * @param fileCacheExpireMs
	 *            the fileCacheExpireMs to set
	 */
	...
	/**
	 * @param sessionMaxIdleMs
	 *            the sessionMaxIdleMs to set
	 */
	...
	/**
	 * @param sessionMaxAgeMs
	 *            the sessionMaxAgeMs to set
	 */
	 ...
	 * 
	 * @param host
	 *            the host name to search for, or "" for all hosts
	 * @param port
	 *            the port to search for, or -1 for all ports
	 * @param context
	 *            the context to search for -- NOT OPTIONAL!
	 * @return the proper mount for the given host and context and port
	 */
	public final Pair<String, String> getMount(final String host,
			final int port, final String context) {
		Map<Integer, KeyPairSearchTree<String>> portMap = mounts.get(host);
		if (portMap != null) {
			KeyPairSearchTree<String> contexts = portMap.get(Integer
					.valueOf(port));
			if (contexts != null) {
				final Pair<String, String> pair = contexts
						.findLongestValue(context);
				if (pair != null)
					return pair;
			}
	 ...
	 * @return the proper forward for the given host and context and port
	 */
	public final Pair<String, WebAddress> getPortForward(final String host,
			final int port, final String context) {
		Map<Integer, KeyPairSearchTree<WebAddress>> portMap = fwds.get(host);
		if (portMap != null) {
			KeyPairSearchTree<WebAddress> contexts = portMap.get(Integer
					.valueOf(port));
			if (contexts != null) {
				final Pair<String, WebAddress> pair = contexts
						.findLongestValue(context);
				if (pair != null)
					return pair;
			}
		...
		if (portMap != null) {
			KeyPairSearchTree<WebAddress> contexts = portMap.get(Integer
					.valueOf(port));
			if (contexts != null) {
				Pair<String, WebAddress> pair = contexts
						.findLongestValue(context);
				if (pair != null)
					return pair;
			}
	...
	/**
	 * @param sslKeyManagerEncoding
	 *            the sslKeyManagerEncoding to set
	 */
	...
	/**
	 * @param httpListenPorts
	 *            the httpListenPorts to set
	 */
	...
	/**
	 * @param httpsListenPorts
	 *            the httpsListenPorts to set
	 */
	...
	/**
	 * @param maxThreadIdleMs
	 *            the maxThreadIdleMs to set
	 */
	...
	/**
	 * @param maxThreadTimeoutSecs
	 *            the maxThreadTimeoutSecs to set
	 */
	...
	/**
	 * @param maxThreadQueueSize
	 *            the maxThreadQueueSize to set
	 */
	...
	/**
	 * @param fileCacheMaxBytes
	 *            the fileCacheMaxBytes to set
	 */
	...
	/**
	 * @param fileCacheMaxFileBytes
	 *            the fileCacheMaxFileBytes to set
	 */
	...
	/**
	 * @param fileCompMaxFileBytes
	 *            the fileCompMaxFileBytes to set
	 */
	...
	/**
	 * @param requestMaxBodyBytes
	 *            the requestMaxBodyBytes to set
	 */
	...
	/**
	 * @param requestMaxIdleMs
	 *            the requestMaxIdleMs to set
	 */
	...
	/**
	 * @param requestLineBufBytes
	 *            the requestLineBufBytes to set
	 */
	...
	/**
	 * @param requestMaxAliveSecs
	 *            the requestMaxAliveSecs to set
	 */
	...
	/**
	 * @param requestMaxPerConn
	 *            the requestMaxPerConn to set
	 */
	 ...
	 * 
	 * @param props
	 *            the props to look in
	 * @param propName
	 *            the name of the prop
	 * @param defaultVal
	 *            what to return if its not there, or isn't an int
	 * @return
	 */
	private int getInt(final Properties props, final String propName,
	 ...
	 *            the name of the prop
	 * @param defaultVal
	 *            what to return if its not there, or isn't an long
	 * @return
	 */
	private long getLong(final Properties props, final String propName,
	 ...
	 *            the name of the prop
	 * @param defaultVal
	 *            what to return if its not there
	 * @return
	 */
	private String getString(final Properties props, final String propName,
	 ...
	 * @param props
	 *            the properties
	 * @param portListPropName
	 *            the port list property name
	 * @param defaultPorts
	 *            the default to use when none found
	 * @return the new ports
	 */
	private int[] getPorts(Properties props, String portListPropName,
			int[] defaultPorts) {
		if (props.containsKey(portListPropName)) {
			...
			String[] prop = getString(props, portListPropName, str.toString())
					.split(",");
			int numPorts = 0;
	...
	/**
	 * Returns property keypairs, assuming any exist. They are formatted with a
	 * prefix and a separator followed by the first being mounted, which is
	 * equal to the value.
	 * 
	 * @param props
	 *            the properties
	 * @param prefix
	 *            the prefix that is shared amongst all entries
	 * @return a map of all keypairs found
	 */
	private Map<String, String> getPrefixedPairs(Properties props,
			String prefix, char separatorChar) {
		Map<String, String> newMounts = null;
		for (Object p : props.keySet()) {
			if ((p instanceof String)
					&& ((String) p).toUpperCase().startsWith(
							prefix + separatorChar)) {
				if (newMounts == null)
					newMounts = new HashMap<String, String>();
				String key = (String) p;
	...
	/**
	 * Parses a url host:port/context into its constituent parts and returns
	 * them
	 * 
	 * @param value
	 *            the url
	...
	private Triad<String, Integer, String> findHostPortContext(String value) {
		...
		if (portDex < 0)
			return new Triad<String, Integer, String>("", ALL_PORTS, "/"
					+ value);
		...
		if (portDex > 0) {
			...
			if (possPort.equals("*"))
				...
				try {
					...
					if (logger != null)
						logger.severe("Illegal port in forward address: "
								+ value);
					return null;
				}
			}
	...
	/**
	 * Returns one of the named properties, even ones that don't mean anything
	 * to miniwebserve per se.
	 * 
	 * @param propName
	...
	public void load(Properties props) {
		...
		sslKeyManagerEncoding = getString(props, "SSLKEYMANAGERENCODING",
		...
		coreThreadPoolSize = getInt(props, "CORETHREADPOOLSIZE",
				coreThreadPoolSize);
		...
		maxThreadQueueSize = getInt(props, "MAXTHREADQUEUESIZE",
				maxThreadIdleMs);
		if (!props.containsKey("MAXTHREADTIMEOUTSECS")
				&& props.containsKey("REQUESTTIMEOUTMINS"))
			maxThreadTimeoutSecs = getInt(props, "REQUESTTIMEOUTMINS", 1) * 60;
		else
			maxThreadTimeoutSecs = getInt(props, "MAXTHREADTIMEOUTSECS",
					maxThreadTimeoutSecs);
		fileCacheExpireMs = getLong(props, "FILECACHEEXPIREMS",
				fileCacheExpireMs);
		fileCacheMaxBytes = getLong(props, "FILECACHEMAXBYTES",
				fileCacheMaxBytes);
		fileCacheMaxFileBytes = getLong(props, "FILECACHEMAXFILEBYTES",
				fileCacheMaxFileBytes);
		fileCompMaxFileBytes = getLong(props, "FILECOMPMAXBYTES",
				fileCompMaxFileBytes);

		requestMaxBodyBytes = getLong(props, "REQUESTMAXBODYBYTES",
				requestMaxBodyBytes);
		requestMaxIdleMs = getLong(props, "REQUESTMAXIDLEMS", requestMaxIdleMs);
		requestLineBufBytes = getLong(props, "REQUESTLINEBUFBYTES",
				requestLineBufBytes);
		if (!props.containsKey("REQUESTMAXALIVESECS")
				&& props.containsKey("REQUESTTIMEOUTMINS"))
			requestMaxAliveSecs = getInt(props, "REQUESTTIMEOUTMINS", 1) * 60;
		else
			requestMaxAliveSecs = getInt(props, "REQUESTMAXALIVESECS",
					requestMaxAliveSecs);
		requestMaxPerConn = getInt(props, "REQUESTMAXPERCONN",
				requestMaxPerConn);
		...
		Map<String, String> newConverts = getPrefixedPairs(props,
				"MIMECONVERT", '.');
		if (newConverts != null)
		...
		if (newMounts != null) {
			...
			for (Entry<String, String> p : newMounts.entrySet()) {
				...
				if (from == null)
					continue;
				Map<Integer, KeyPairSearchTree<String>> portMap = mounts
						.get(from.first);
				if (portMap == null) {
		...
		if (newForwards != null) {
			...
			for (Entry<String, String> p : newForwards.entrySet()) {
				...
				Map<Integer, KeyPairSearchTree<WebAddress>> portMap = fwds
						.get(from.first);
				if (portMap == null) {
				...
				try {
					tree.addEntry(from.third, new WebAddress(to.first,
							to.second.intValue(), to.third));
				} catch (UnknownHostException ue) {
					getLogger().severe(
							"Unresolved host in forward address: " + value);
					continue;
				}
			}
		}
