...
/**
 * Thread executor with a waiting queue plus one extra feature -- a passive
 * system for timing out threads that have been active longer than we want. This
 * system kicks in whenever a thread execute attempt is rejected by the
 * underlying executor due to the queue being full. In that case, it will look
 * through the active threads to find one that can be timed out, and will
 * interrupt it -- as if that will work. :/
 * 
 * @author Bo Zimmerman
 * 
 */
public class MWThreadExecutor extends ThreadPoolExecutor {
	...
	protected static class CMLinkedBlockingQueue<E> extends
		...
		public boolean offer(E o) {
			final int allWorkingThreads = executor.getActiveCount()
					+ super.size();
			return (allWorkingThreads < executor.getPoolSize())
					&& super.offer(o);
		}
	}

	public MWThreadExecutor(String poolName, MiniWebConfig config,
			int corePoolSize, int maximumPoolSize, long keepAliveTime,
			TimeUnit unit, long timeoutSecs, int queueSize) {
		super(corePoolSize, maximumPoolSize, keepAliveTime, unit,
				new CMLinkedBlockingQueue<Runnable>(queueSize));
	...
	public void execute(Runnable r) {
		try {
			...
			// an optomization for logging purposes. When my smtp server gets
			// hit
			// by a spam-bot, the log fills up my hard drive. this helps prevent
			// that.
			if ((rejectCount > 0)
					&& (System.currentTimeMillis() - lastRejectTime) > 5000) {
				logger.warning(rejectCount + " Pool_" + poolName
						+ ": Threads rejected.");
				rejectCount = 0;
			}
		} catch (RejectedExecutionException e) {
			// a thread is rejected only when the queue is filled. Look for the
			// blockages!
			Collection<MWRunWrap> runsKilled = getTimeoutOutRuns(1);
			for (MWRunWrap runnable : runsKilled)
				logger.severe("Pool_" + poolName
						+ ": Old(er) Runnable killed: " + runnable.toString());
	 ...
	 * been active longer than permitted. It will attempt to kill those,
	 * returning those threads which were timed out. There's also the funny
	 * feature that it will kill only the number you ask for, starting with the
	 * oldest offenders and working towards the youngest.
	 * 
	 * @param maxToKill
	 *            the maximum number of threads to kill
	...
	public Collection<MWRunWrap> getTimeoutOutRuns(int maxToKill) {
		...
		synchronized (active) {
			try {
				for (Iterator<MWRunWrap> i = active.values().iterator(); i
						.hasNext();) {
						...
						if (timedOut.size() >= maxToKill) {
							...
							for (MWRunWrap r : timedOut) {
								if ((leastWorstOffender != null)
										&& (r.activeTimeMillis() < leastWorstOffender
												.activeTimeMillis()))
									leastWorstOffender = r;
							}
							if (leastWorstOffender != null) {
								if (runnable.activeTimeMillis() < leastWorstOffender
										.activeTimeMillis())
									continue;
								else
