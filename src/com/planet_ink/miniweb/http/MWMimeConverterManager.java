...
/**
 * Manages a relatively static set of servlet classes and the root contexts
...
public class MWMimeConverterManager implements MimeConverterManager {
	private final Map<MIMEType, Class<? extends HTTPOutputConverter>> converters; // map
																					// of
																					// registered
																					// servlets
																					// by
																					// context
	private final Map<Class<? extends HTTPOutputConverter>, MWRequestStats> requestStats; // stats
																							// about
																							// each
																							// servlet

	/**
	 * Construct a mime config manager, loading the converters from the config
	 * given
	 * 
	 * @param config
	 */
	public MWMimeConverterManager(MiniWebConfig config) {
		...
		for (String mimeTypeName : config.getFileConverts().keySet()) {
			...
			try {
				...
				Class<? extends HTTPOutputConverter> converterClass = (Class<? extends HTTPOutputConverter>) Class
						.forName(className);
				registerConverter(mimeType, converterClass);
			} catch (ClassNotFoundException e) {
				config.getLogger().severe(
						"Converter Manager can't load " + className);
			} catch (Exception e) {
				config.getLogger().severe(
						"Converter Manager can't converter " + mimeTypeName);
			}
		}
	}

	/**
	 * Internal method to register a servlets existence, and its context. This
	 * will go away when a config file is permitted
	 * 
	 * @param context
	 *            the uri context the servlet responds to
	 * @param converterClass
	 *            the class of the converter
	 */
	public void registerConverter(MIMEType mime,
			Class<? extends HTTPOutputConverter> converterClass) {
		converters.put(mime, converterClass);
		requestStats.put(converterClass, new MWRequestStats());
	}

	/**
	 * For anyone externally interested, will return the list of converter
	...
	/**
	 * Returns a converter (if any) that handles the given mime type. if none is
	...
	/**
	 * Returns a statistics object for the given converter class or null if none
	 * exists
	 * 
	 * @param converterClass
	 *            the converter class managed by this web server
	 * @return the converter stats object
	 */
	public MWRequestStats getConverterStats(
			Class<? extends HTTPOutputConverter> converterClass) {
