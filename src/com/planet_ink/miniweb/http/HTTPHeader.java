...
/**
 * Class to help formally manage http headers, both those from the client and
 * those sent to it. Headers defined herein also have methods for easily
 * constructing the "header line" used to output them.
 * 
 * @author Bo Zimmerman
 */
public enum HTTPHeader {
	ACCEPT("Accept"), ACCEPT_ENCODING("Accept-Encoding"), ACCEPT_RANGES(
			"Accept-Ranges", "bytes"), ALLOW("Allow", HTTPMethod
			.getAllowedList()), CACHE_CONTROL("Cache-Control"), CONNECTION(
			"Connection"), CONTENT_DISPOSITION("Content-Disposition"), CONTENT_ENCODING(
			"Content-Encoding"), CONTENT_LENGTH("Content-Length"), CONTENT_RANGE(
			"Content-Range"), CONTENT_TYPE("Content-Type"), COOKIE("Cookie"), DATE(
			"Date"), ETAG("ETag"), EXPECT("Expect"), EXPIRES("Expires"), HOST(
			"Host"), IF_MODIFIED_SINCE("If-Modified-Since"), IF_NONE_MATCH(
			"If-None-Match"), KEEP_ALIVE("Keep-Alive"), LAST_MODIFIED(
			"Last-Modified"), LOCATION("Location"), RANGE("Range"), SERVER(
	...
	/**
	 * Return a lowercase form of this headers name as used in normalized map
	 * lookups
	 * 
	...
	/**
	 * Return a header line with the given value and an end-of-line character
	 * attached
	 * 
	 * @param value
	 *            the value to assign to the header
