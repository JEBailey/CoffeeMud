 ...
 * It puts together certain http defaults, but allows those defaults to be
 * mangled at will by the servlet. As an Output Generator, it must have its
 * generateOutput method called later to get the results of all the servlet did
 * to it.
 * 
...
public class MWServletResponse implements SimpleServletResponse {
	...
	public DataBuffers generateOutput(HTTPRequest request) throws HTTPException {
		...
		str.append("HTTP/").append(request.getHttpVer()).append(" ")
				.append(statusCode).append(" ").append(statusString)
				.append(EOLN);
		HashSet<String> normalizedHeaders = new HashSet<String>();
		for (String header : headers.keySet()) {
			normalizedHeaders.add(header.toLowerCase());
			str.append(header).append(": ").append(headers.get(header))
					.append(EOLN);
		}
		// since the servlet could have overwridden ANY of our default headers,
		// we make
		// sure to check a normalizsed set of servlet generated headers BEFORE
		// writing
		// our own.
		if (bout.size() > 0) {
			if (!normalizedHeaders.contains(HTTPHeader.CONTENT_TYPE
					.lowerCaseName()))
				str.append(HTTPHeader.CONTENT_TYPE.makeLine(MIMEType.html
						.getType()));
			if (!normalizedHeaders.contains(HTTPHeader.CONTENT_LENGTH
					.lowerCaseName()))
				str.append(HTTPHeader.CONTENT_LENGTH.makeLine(bout.size()));
		}
		if ((Thread.currentThread() instanceof MWThread)
				&& ((MWThread) Thread.currentThread()).getConfig()
						.isDebugging())
			((MWThread) Thread.currentThread())
					.getConfig()
					.getLogger()
					.finer("Response Servlet: "
		...
		for (String key : cookies.keySet())
			str.append(HTTPHeader.SET_COOKIE.makeLine(key + "="
					+ cookies.get(key)));
		str.append(EOLN);
		MWDataBuffers bufs = new MWDataBuffers(str.toString().getBytes(),
