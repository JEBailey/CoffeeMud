...
public class MWSessionManager implements ServletSessionManager {
	private final Map<String, SimpleServletSession> sessions; // map of ids to
																// sessions
	...
	/**
	 * Internal method to find an existing session based on the request data. If
	 * the session does not exist, it will be created and returned
	 * 
	 * @param sessionID
	 *            the id of the session
	 */
	...
	/**
	 * A maintence method forcing the manager to examine all sessions for any
	 * that have timed out and remove them, if so.
	 * 
	...
	public void cleanUpSessions() {
		synchronized (sessions) {
			...
			final long idleExpireTime = currentTime
					- config.getSessionMaxIdleMs();
			final Date ageExpireTime = new Date(currentTime
					- config.getSessionMaxAgeMs());
	 ...
	 * 
	 * @param request
	 *            the current request to base the new session on
	 * @return the new servlet session obj
	 */
	@Override
	public SimpleServletSession createSession(HTTPRequest request) {
		String sessionID = request.getClientAddress().hashCode() + ""
				+ System.currentTimeMillis() + "" + System.nanoTime();
		try {
			while (sessions.containsKey(sessionID)) {
				Thread.sleep(1);
				sessionID = request.getClientAddress().hashCode() + ""
						+ System.currentTimeMillis() + "" + System.nanoTime();
			}
		} catch (Exception e) {
