...
/**
 * Handles the connection to a remote web server as an async socket reader. This
 * class is instantiated by an HTTPReader (no ssl supported)
 * 
 * @author Bo Zimmerman
 * 
 */
public class HTTPForwarder implements HTTPIOHandler, Runnable {
	...
	private final boolean isDebugging; // true if the log debug channel is on --
										// an optomization
	private ByteBuffer responseBuffer;
	private final String name; // the name of this handler -- to denote a
								// request ID
	...
	public HTTPForwarder(HTTPReader clientReader, MiniWebServer server,
			SocketChannel webServerChannel) {
		this.server = server;
		...
		this.responseBuffer = ByteBuffer.allocate((int) config
	...
	public void run() {
							...
							// ever.
		{
			...
			try // begin of the IO error handling and record integrity block
			{
				...
				while (!closeMe
						&& ((bytesRead = webServerChannel.read(responseBuffer)) > 0)) {
					if (!announcedAlready) {
						config.getLogger().finer(
								"Processing handler '" + name + "'");
						announcedAlready = true;
					}
					responseBuffer.flip(); // turn the writeable buffer into a
											// "readable" one
					ByteBuffer writeableBuf = ByteBuffer
							.allocate(responseBuffer.remaining());
					writeableBuf.put(responseBuffer);
					writeableBuf.flip();
					clientReader.writeBytesToChannel(new MWDataBuffers(
							writeableBuf, 0));
					responseBuffer.clear();
				}
				handleWrites();
				if ((!closeMe) // if eof is reached, close this channel and mark
								// it for deletion by the web server
						&& ((bytesRead < 0)
								|| (!webServerChannel.isConnected()) || (!webServerChannel
									.isOpen()))) {
					closeChannels();
				}
			} catch (IOException e) {
	...
	protected void closeChannels() {
		if (!closeMe) {
			...
			if ((isDebugging) && (webServerChannel.isOpen()))
				config.getLogger().finer(
						"Closed request forward handler '" + name + "'");
			try {
	...
	/**
	 * Reads bytes from the given buffer into the forwarding channel. This code
	 * is parsed out here so that it can be overridden by HTTPSReader
	 * 
	 * @param buffers
			...
			while (writeables.size() > 0) {
				...
				while (bufs.hasNext()) {
					...
					if (buf.remaining() > 0) {
						...
						if (buf.remaining() > 0) {
							server.registerChannelInterest(webServerChannel,
