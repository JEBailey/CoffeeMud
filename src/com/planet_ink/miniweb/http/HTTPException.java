 ...
 * that the client didn't expect or prefer, such as a 404, 403, etc.. They are
 * constructed from HTTPStatus and then tossed up the chain where, as an
 * OutputGenerator, they can be used to generate their own full buffer output
...
public class HTTPException extends Exception {
	...
	private final String body; // optional body to send back with the error,
								// usually ""
	private final Map<HTTPHeader, String> errorHeaders = new Hashtable<HTTPHeader, String>(); // any
																								// optional/extraneous
																								// headers
																								// to
																								// send
	...
	/**
	 * Get the extraneous headers for this exception For reading or adding-to
	 * 
	 * @return headers map
	 */
	...
	/**
	 * Like all HTTPIOHandlers, this class generates its own output buffer. The
	 * buffer generated here is typically sent straight to the socket.
	 * 
	 * @param request
	...
	public DataBuffers generateOutput(HTTPRequest request) throws HTTPException {
		...
		str.append("HTTP/").append(request.getHttpVer()).append(" ")
				.append(getStatus().getStatusCode()).append(" ")
				.append(getMessage());
		if (isDebugging)
		...
		if ((body.length() == 0) && (status.isAnError()) && (config != null)
				&& (config.getErrorPage().length() > 0)) {
			File errorFile = config.getFileManager().createFileFromPath(
					config.getErrorPage());
			...
			try {
				...
				if (mimeType != null) {
					...
					if (converterClass != null) {
						HTTPOutputConverter converter = converterClass
								.newInstance();
						finalBody = new MWDataBuffers(converter.convertOutput(
								config, request, status,
								fileBytes.flushToBuffer()), 0);
					} else
						finalBody = fileBytes;
				} else
					finalBody = fileBytes;
		...
		if (finalBody.getLength() > 0) {
			if (!headers.containsKey(HTTPHeader.CONTENT_TYPE.lowerCaseName()))
				str.append(HTTPHeader.CONTENT_TYPE.makeLine(MIMEType.html
						.getType()));
	 ...
	 * 
	 * @param status
	 *            the status of the exception to grab
