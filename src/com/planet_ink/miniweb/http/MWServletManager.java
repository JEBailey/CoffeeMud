...
public class MWServletManager implements SimpleServletManager {
	private final Map<String, Class<? extends SimpleServlet>> servlets; // map
																		// of
																		// registered
																		// servlets
																		// by
																		// context
	private final Map<Class<? extends SimpleServlet>, MWRequestStats> servletStats; // stats
																					// about
																					// each
																					// servlet
	private final Map<Class<? extends SimpleServlet>, Boolean> servletInit; // whether
																			// a
																			// servlets
																			// been
																			// initialized
	...
	public MWServletManager(MiniWebConfig config) {
		...
		for (String context : config.getServlets().keySet()) {
			...
			try {
				...
				Class<? extends SimpleServlet> servletClass = (Class<? extends SimpleServlet>) Class
						.forName(className);
				registerServlet(context, servletClass);
			} catch (ClassNotFoundException e) {
				config.getLogger().severe(
						"Servlet Manager can't load " + className);
			}
		}
	}
	 ...
	 */
	public void registerServlet(String context,
			Class<? extends SimpleServlet> servletClass) {
	...
	/**
	 * Returns a servlet (if any) that handles the given uri context. if none is
	...
	/**
	 * Returns a servlet statistics object for the given servlet class or null
	 * if none exists
	 * 
	 * @param servletClass
	 *            the servlet class managed by this web server
	 * @return the servlet stats object
	 */
	public MWRequestStats getServletStats(
			Class<? extends SimpleServlet> servletClass) {
