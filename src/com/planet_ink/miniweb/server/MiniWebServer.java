...
/**
 * Both the main() kickoff for the miniwebserver AND the main thread class for
 * the same. This class handles all the socket listening and request
 * management/timeout. When requests are received, they are passed off to an
 * HTTPReader for processing, though a separate thread may timeout those readers
 * at its discretion and shut them down.
 * 
 * @author Bo Zimmerman
 * 
 */
public class MiniWebServer extends Thread {
	...
	private volatile boolean shutdownRequested = false; // notice of external
														// shutdown request
	private Selector servSelector = null; // server io selector
	private MWThreadExecutor executor; // request handler thread pool
	private Thread timeoutThread = null; // thread to timeout connected but idle
											// channels
	private MiniWebConfig config; // list of all standard http ports to listen
									// on
	private SSLContext sslContext;
	private String serverName; // a friendly name for this server instance
	private final LinkedList<HTTPIOHandler> handlers; // list of connected
														// channels.. managed by
														// timeoutthread
	private final Map<ServerSocketChannel, Boolean> servChannels; // map of
																	// socket
																	// channels
																	// to an SSL
																	// boolean
	...
	public MiniWebServer(String serverName, MiniWebConfig config) {
		...
		executor = new MWThreadExecutor(serverName, config,
				config.getCoreThreadPoolSize(), config.getMaxThreadPoolSize(),
				config.getMaxThreadIdleMs(), TimeUnit.MILLISECONDS,
				config.getMaxThreadTimeoutSecs(),
				config.getMaxThreadQueueSize());
		...
		// if we are going to be listening on ssl, generate a global ssl context
		// to use
	...
	public List<Runnable> getOverdueThreads() {
		Collection<MWRunWrap> wraps = executor
				.getTimeoutOutRuns(Integer.MAX_VALUE);
	...
	/**
	 * Open a single web server listening sockets and register a selector for
	...
	private void openChannel(int listenPort, Boolean isSSL) throws IOException {
		...
		config.getLogger().info(
				"Started " + (isSSL.booleanValue() ? "ssl " : "http ")
						+ serverName + " on port " + listenPort);
	}

	/**
	 * Open the main web server listening sockets and register a selector for
	 * accepting connections.
	 * 
	 * @throws IOException
	 */
	...
	/**
	 * Initialize the timeoutthread. It wakes up every second to look for timed
	 * out connections It does this by calling a local final method to scan the
	 * open connections. TODO: an executor thread pool that allows both
	 * scheduled and timeoutable entries would have allowed us to run this
	 * process on the pool. Since that's not supported atm...
	 */
	private void startTimeoutThread() {
		timeoutThread = new Thread(Thread.currentThread().getThreadGroup(),
				getName() + "Timeout") {
			public void run() {
				try {
	...
	/**
	 * Handles a particular channel event from its given selectionkey. So far,
	 * only accepted connections and readable keys are managed here.
	 * 
	 * @param key
	...
	private void handleSelectionKey(final SelectionKey key) throws IOException {
		if (key.isAcceptable()) // a connection was made, so add the handler
		{
			...
			if (channel != null) {
				...
				synchronized (handlers) // synched because you can't iterate and
										// modify, and because its a linkedlist
				{
					handlers.add(handler);
				}
			}
		}
		if (key.isReadable() // bytes were received on one of the channel .. so
								// read!
				|| (((key.interestOps() & SelectionKey.OP_WRITE) == SelectionKey.OP_WRITE) && key
						.isWritable())) {
			...
			config.getLogger().finer(
					"Rejected handler key for " + handler.getName());
		}
	}
	...
	/**
	 * The main web server loop It blocks on its selector waiting for either
	 * accepted connections, or data to be read, which it then farms out to
	 * another thread. This is repeats until something external requests it to
	 * shut down.
	 */
	public void run() {
		try {
		...
		while (!shutdownRequested) {
			try {
				...
				Iterator<SelectionKey> it = servSelector.selectedKeys()
						.iterator();
				while (it.hasNext()) {
	 ...
	 * @param handler
	 *            the handler to handle it.
	 */
	public void registerNewHandler(final SocketChannel channel,
			final HTTPIOHandler handler) {
				...
				public void run() {
					try {
						...
						channel.register(servSelector, SelectionKey.OP_READ,
								handler);
						synchronized (handlers) // synched because you can't
												// iterate and modify, and
												// because its a linkedlist
						{
							handlers.add(handler);
						}
					} catch (Exception e) {
	 ...
	 *            the socket channel to register
	 * @param newOp
	 *            the new operations for this channel
	 */
	public void registerChannelInterest(final SocketChannel channel,
			final int newOp) {
		synchronized (this.registerOps) {
			final Selector servSelector = this.servSelector;
			this.registerOps.add(new Runnable() {
	...
	/**
	 * Create, Initialize, load, and create a web server configuration based
	 * around the given ini filename and the given java logger.
	 * 
	 * @param log
	 *            the java logger to use
	 * @param iniInputStream
	 *            the ini data to load further settings from
	 * @return a populated configuration object to create a miniwebserver from
	 */
	public static MiniWebConfig createConfig(java.util.logging.Logger log,
			InputStream iniInputStream) throws IOException {
		final MiniWebConfig config = new MiniWebConfig();
		return initConfig(config, log, iniInputStream);
	}

	/**
	 * Initialize, load, and create a web server configuration based around the
	 * given ini filename and the given java logger.
	 * 
	 * @param log
	 *            the java logger to use
	 * @param iniInputStream
	 ...
	 * @return a populated configuration object to create a miniwebserver from
	 */
	public static MiniWebConfig initConfig(final MiniWebConfig config,
			java.util.logging.Logger log, InputStream iniInputStream)
			throws IOException {
		...
		final MWFileCache fileCacheManager = new MWFileCache(config,
				config.getFileManager());
		final MWMimeConverterManager mimeConverterManager = new MWMimeConverterManager(
				config);
		...
		HTTPHeader.setKeepAliveHeader(HTTPHeader.KEEP_ALIVE.makeLine(String
				.format(HTTPHeader.KEEP_ALIVE_FMT,
	...
	/**
	 * Good olde main. It does nothing but initialize logging, spawn a new web
	 * server and then join its thread until it is gone. I suppose I could just
	 * create the web server and call run() on it, but somehow this feels
	 * better.
	 * 
	 * @param args
	 *            As no external configuration is permitted, no args are
	 *            accepted
	 */
	public static void main(String[] args) {
		...
		try {
			config = MiniWebServer.createConfig(Log.instance(),
					new FileInputStream(iniFilename));
		} catch (Exception e) {
			e.printStackTrace();
			System.exit(-1);
			return; // an unhit operation, but my ide is argueing with me over
					// it.
		}
