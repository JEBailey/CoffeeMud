...
/**
 * Relies on multi-part form data to write all received files to the current web
 * server directory.
 * 
 * @author Bo Zimmerman
 * 
...
public class MultipartFileWriter implements SimpleServlet {
	...
	private void writeFilesFromParts(List<MultiPartData> parts,
			StringBuilder filesList) throws IOException {
		if (parts != null) {
	...
	public void doPost(SimpleServletRequest request,
		...
		try {
			...
			response.getOutputStream()
					.write(("<html><body><h1>Done</h1><br>"
							+ filesList.toString() + "</body></html>")
							.getBytes());
		} catch (IOException e) {
