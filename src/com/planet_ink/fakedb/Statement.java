...
public class Statement implements java.sql.Statement {
	...
	public String parseWhereClause(String tableName, String sql,
		...
		while (s < sql.length()) {
			...
			if (sql.charAt(s) == '(') {
				condition = connection.getBackend().buildFakeCondition(
						tableName, null, null, null, false);
				...
				if (parenStack.size() == 0)
					throw new java.sql.SQLException(
							"Unexpected end parenthesis " + sql);
				...
				if (e >= sql.length())
					throw new java.sql.SQLException(
							"Unexpected end of where clause in " + sql);
				s = e;
				while ((s < sql.length())
						&& (sql.charAt(s) == ' ' || sql.charAt(s) == '\t'))
					s++;
				...
				if ((e < sql.length() - 5)
					...
					comparitor = "like";
					e += 5;
				} else if ((e < sql.length())
						&& (eow1.indexOf(sql.charAt(e)) > 0)) {
					while ((e < sql.length())
							&& (eow1.indexOf(sql.charAt(e)) > 0))
						e++;
				...
				if (e >= sql.length() || comparitor.length() == 0)
					throw new java.sql.SQLException(
							"Unexpected end of where clause in " + sql);
				s = e;
				while ((sql.charAt(s) == ' ' || sql.charAt(s) == '\t')
						&& (s < sql.length()))
					s++;
				if (s >= sql.length())
					throw new java.sql.SQLException(
				...
				s = e;
				condition = connection.getBackend().buildFakeCondition(
						tableName, columnName, comparitor, value, unPrepared);
				conditions.add(condition);
			}
			while ((s < sql.length())
					&& (sql.charAt(s) == ' ' || sql.charAt(s) == '\t'))
	...
	public Backend.ImplSelectStatement parseSelect(String sql, String[] token)
			throws java.sql.SQLException {
		...
		if (sql.length() > 0) {
			...
			if ((token[0] != null) && (token[0].equalsIgnoreCase("order"))) {
				...
				if (sql.length() > 0) {
					split(sql, token);
					if (token[0].equalsIgnoreCase("ASC")
							|| token[0].equalsIgnoreCase("DESC")) {
						orderConditions = new String[] { token[0].toUpperCase()
								.trim() };
						sql = split(sql, token);
					}
				}
		...
		return new Backend.ImplSelectStatement(this, tableName, cols,
				conditions, orderVars, orderConditions);
	}
	...
	protected Backend.ImplInsertStatement parseInsert(String sql, String[] token)
			throws java.sql.SQLException {
		sql = split(sql, token);
		...
		return new Backend.ImplInsertStatement(tableName,
				columnList.toArray(new String[0]),
				valuesList.toArray(new String[0]),
				unPreparedValueList.toArray(new Boolean[0]));
	}

	protected Backend.ImplUpdateStatement parseUpdate(String sql, String[] token)
			throws java.sql.SQLException {
		sql = split(sql, token);
		String tableName = token[0];
		sql = split(sql, token);
		...
		if (conditions.size() == 0)
			throw new java.sql.SQLException("no more where clause!");
		return new Backend.ImplUpdateStatement(tableName, conditions,
				columnList.toArray(new String[0]),
	...
	protected Backend.ImplDeleteStatement parseDelete(String sql, String[] token)
			throws java.sql.SQLException {
		sql = split(sql, token);
		if (!token[0].equalsIgnoreCase("from"))
			throw new java.sql.SQLException("no from clause");
	...
	public int executeUpdate(String sql) throws java.sql.SQLException {
		...
		try {
			...
			if (token[0].equalsIgnoreCase("insert")) {
				...
				connection.getBackend().dupKeyCheck(stmt.tableName,
						stmt.columns, stmt.sqlValues);
				...
				throw new java.sql.SQLException("unimplemented command: "
						+ token[0]);
			return 1;
		} catch (java.sql.SQLException e) {
			e.printStackTrace();
	...
	public int executeUpdate(String sql, String[] a)
			throws java.sql.SQLException {
		return executeUpdate(sql);
	}
	...
	public boolean execute(String sql) throws java.sql.SQLException {
		...
		try {
			...
			Backend.ImplSelectStatement stmt = parseSelect(sql, token);
			myResultSet = (ResultSet) connection.getBackend().constructScan(
					stmt);
			return true;
		} catch (java.sql.SQLException e) {
			log("unsupported SQL in executeQuery: " + sql);
