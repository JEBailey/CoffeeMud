...
public class PreparedStatement extends Statement implements
	...
	public boolean execute() throws SQLException {
		...
		case SELECT:
			myResultSet = (ResultSet) connection.getBackend().constructScan(
					(Backend.ImplSelectStatement) stmt);
			return true;
		case UPDATE:
			connection.getBackend().updateRecord(
					(Backend.ImplUpdateStatement) stmt);
			return true;
		case DELETE:
			connection.getBackend().deleteRecord(
					(Backend.ImplDeleteStatement) stmt);
			return true;
		case INSERT: {
			final Backend.ImplInsertStatement istmt = (Backend.ImplInsertStatement) stmt;
			connection.getBackend().dupKeyCheck(istmt.tableName, istmt.columns,
					istmt.sqlValues);
	...
	public java.sql.ResultSet executeQuery() throws SQLException {
		...
		return connection.getBackend().constructScan(
				(Backend.ImplSelectStatement) stmt);
	}
	...
	private boolean setRecursiveObject(final int parameterIndex,
			final Object x, final List<Backend.FakeCondition> conds,
			final int[] atIndex) {
			...
			for (Backend.FakeCondition cond : conds) {
				if (cond.unPrepared) {
					if (atIndex[0] == parameterIndex) {
						if (x == null)
							cond.conditionValue = new Backend.ComparableValue(
									null);
				...
				if (setRecursiveObject(parameterIndex, x, cond.contains,
						atIndex))
					return true;
			}
		}
