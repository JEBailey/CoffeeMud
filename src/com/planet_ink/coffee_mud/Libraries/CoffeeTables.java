...
public class CoffeeTables extends StdLibrary implements StatisticsLibrary {
	...
	public void bump(CMObject E, int type) {
		...
		if (todays == null) {
			...
			todays = (CoffeeTableRow) CMLib.database().DBReadStat(
					S.getTimeInMillis());
			if (todays == null) {
				synchronized (this) {
					if (todays == null) {
						...
						todays = (CoffeeTableRow) CMClass
								.getCommon("DefaultCoffeeTableRow");
						todays.setStartTime(S.getTimeInMillis());
						todays.setEndTime(C.getTimeInMillis());
						CMLib.database().DBCreateStat(todays.startTime(),
								todays.endTime(), todays.data());
					}
				}
			}
			return;
		...
		if ((now > todays.endTime())
				&& (!CMLib
						.time()
						.date2MonthDateString(now, true)
						.equals(CMLib.time().date2MonthDateString(
								todays.endTime(), true)))) {
			synchronized (this) {
				if ((now > todays.endTime())
						&& (!CMLib
								...
								.equals(CMLib.time().date2MonthDateString(
										todays.endTime(), true)))) {
					CMLib.database().DBUpdateStat(todays.startTime(),
							todays.data());
					Calendar S = Calendar.getInstance();
					S.set(Calendar.HOUR_OF_DAY, 0);
					S.set(Calendar.MINUTE, 0);
					...
					todays.setStartTime(S.getTimeInMillis());
					todays.setEndTime(C.getTimeInMillis());
					CoffeeTableRow testRow = (CoffeeTableRow) CMLib.database()
							.DBReadStat(todays.startTime());
					if (testRow != null)
						todays = testRow;
					else if (!CMLib.database().DBCreateStat(todays.startTime(),
							todays.endTime(), todays.data())) {
						Log.errOut("CoffeeTables",
								"Unable to manage daily-stat transition");
					}
				}
			}
		}
		...
		try {
			if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.SAVETHREAD))
				...
				if ((ok.length() != 0) && (!ok.startsWith("OK"))) {
					...
					CMLib.coffeeTables().bump(null,
							CoffeeTableRow.STAT_SPECIAL_NUMONLINE);
