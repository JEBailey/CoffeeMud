...
public class ColumbiaUniv extends StdLibrary implements ExpertiseLibrary {
	...
	public ExpertiseLibrary.ExpertiseDefinition addDefinition(String ID,
			String name, String baseName, String listMask, String finalMask,
			String[] costs, String[] data) {
		...
		// if(timeCost>0) def.addCost(CostType.PRACTICE,
		// Double.valueOf(practices));
	...
	public ExpertiseDefinition getDefinition(String ID) {
		return (ID == null) ? null : (ExpertiseDefinition) completeEduMap
				.get(ID.trim().toUpperCase());
	}
	...
	public ExpertiseDefinition findDefinition(String ID, boolean exactOnly) {
		...
		if (D != null)
			return D;
		for (Enumeration<ExpertiseDefinition> e = definitions(); e
				.hasMoreElements();) {
			D = e.nextElement();
	...
	public List<ExpertiseDefinition> myQualifiedExpertises(MOB mob) {
		...
		for (Enumeration<ExpertiseDefinition> e = definitions(); e
				.hasMoreElements();) {
			D = e.nextElement();
			if (((D.compiledFinalMask() == null) || (CMLib.masking().maskCheck(
					D.compiledFinalMask(), mob, true)))
					&& ((D.compiledListMask() == null) || (CMLib.masking()
							.maskCheck(D.compiledListMask(), mob, true))))
				V.add(D);
		}
		return V;
	}
	...
	public List<ExpertiseDefinition> myListableExpertises(MOB mob) {
		...
		for (Enumeration<ExpertiseDefinition> e = definitions(); e
			...
			if ((D.compiledListMask() == null)
					|| (CMLib.masking().maskCheck(D.compiledListMask(), mob,
							true)))
				V.add(D);
		}
		return V;
	...
	private String expertMath(String s, int l) {
		...
		while (x >= 0) {
			...
			if (y < 0)
				break;
			s = s.substring(0, x)
					+ CMath.parseIntExpression(s.substring(x + 1, y))
					+ s.substring(y + 1);
			x = s.indexOf('{');
		}
		return s;
		...
		if (!baseEduSetLists.containsKey(baseExpertiseCode)) {
			synchronized (("ListedEduBuild:" + baseExpertiseCode).intern()) {
				if (!baseEduSetLists.containsKey(baseExpertiseCode)) {
					...
					for (Enumeration<String> e = completeEduMap.keys(); e
							.hasMoreElements();) {
						key = e.nextElement();
						if (key.startsWith(baseExpertiseCode)
								&& (CMath.isInteger(key
										.substring(baseExpertiseCode.length())) || CMath
										.isRomanNumeral(key
												.substring(baseExpertiseCode
														.length()))))
							codes.add(key);
					}
					baseEduSetLists.put(baseExpertiseCode,
							new ReadOnlyVector<String>(codes));
				}
			}
		}
	...
	public String getGuessedBaseExpertiseName(final String expertiseCode) {
		...
		while ((lastBadChar >= 0)
				&& (CMath.isInteger(expertiseCode.substring(lastBadChar)) || CMath
						.isRomanNumeral(expertiseCode.substring(lastBadChar))))
			lastBadChar--;
	...
	public int getApplicableExpertiseLevel(String ID, int code, MOB mob) {
		Entry<String, Integer> e = mob.fetchExpertise(completeUsageMap[code]
				.get(ID));
	...
	public String confirmExpertiseLine(String row, String ID,
		...
		if (row.trim().toUpperCase().startsWith("DATA_")) {
			...
			if ((lastID == null) || (lastID.length() == 0))
				return "Error: No last expertise found for data: " + lastID
						+ "=" + row;
			else if (this.getDefinition(ID) != null) {
				def = getDefinition(ID);
				WKID = def.name.toUpperCase().replace(' ', '_');
				if (addIfPossible) {
					def.data = CMParms.parseCommas(row, true).toArray(
							new String[0]);
				}
			} else {
		...
		if (row.trim().toUpperCase().startsWith("HELP_")) {
			...
			if (ID.length() == 0)
				ID = lastID;
			if ((lastID == null) || (lastID.length() == 0))
				return "Error: No last expertise found for help: " + lastID
						+ "=" + row;
				...
				if ((stages == null) || (stages.size() == 0))
					return "Error: Expertise not yet defined: " + ID + "="
							+ row;
				...
				if (def != null) {
					...
					if ((x >= 0)
							&& (CMath.isInteger(WKID.substring(x + 1)) || CMath
									.isRomanNumeral(WKID.substring(x + 1)))) {
		...
		if (parts.size() != 11)
			return "Error: Expertise row malformed (Requires 11 entries/10 commas): "
					+ ID + "=" + row;
		name = parts.get(0);
		if (name.length() == 0)
			return "Error: Expertise name (" + name + ") malformed: " + ID
					+ "=" + row;
		if (!CMath.isInteger(parts.get(1)))
			return "Error: Expertise num (" + (parts.get(1)) + ") malformed: "
					+ ID + "=" + row;
		...
		if (skillMask.length() == 0)
			return "Error: Expertise skill mask (" + skillMask
					+ ") malformed: " + ID + "=" + row;
		skillsToRegister = CMLib.masking().getAbilityEduReqs(skillMask);
		if (skillsToRegister.size() == 0)
			return "Error: Expertise no skills (" + skillMask + ") found: "
					+ ID + "=" + row;
		...
		if (!didOne)
			return "Error: No flags (" + parts.get(2).toUpperCase()
					+ ") were set: " + ID + "=" + row;
		if (addIfPossible) {
			String baseName = CMStrings.replaceAll(
					CMStrings.replaceAll(ID, "@X2", ""), "@X1", "")
					.toUpperCase();
			for (int l = 1; l <= levels; l++) {
				WKID = CMStrings.replaceAll(ID, "@X1", "" + l);
				WKID = CMStrings.replaceAll(WKID, "@X2",
						"" + CMath.convertToRoman(l));
				WKname = CMStrings.replaceAll(name, "@x1", "" + l);
				WKname = CMStrings.replaceAll(WKname, "@x2",
						"" + CMath.convertToRoman(l));
				WKlistMask = CMStrings.replaceAll(listMask, "@x1", "" + l);
				WKlistMask = CMStrings.replaceAll(WKlistMask, "@x2",
						"" + CMath.convertToRoman(l));
				WKfinalMask = CMStrings.replaceAll(finalMask, "@x1", "" + l);
				WKfinalMask = CMStrings.replaceAll(WKfinalMask, "@x2", ""
						+ CMath.convertToRoman(l));
				if ((l > 1) && (listMask.toUpperCase().indexOf("-EXPERT") < 0)) {
					s = CMStrings.replaceAll(ID, "@X1", "" + (l - 1));
					s = CMStrings.replaceAll(s, "@X2",
							"" + CMath.convertToRoman(l - 1));
					WKlistMask = "-EXPERTISE \"+" + s + "\" " + WKlistMask;
				}
				WKlistMask = expertMath(WKlistMask, l);
				WKfinalMask = expertMath(WKfinalMask, l);
				def = addDefinition(WKID, WKname, baseName, WKlistMask,
						WKfinalMask, costs, data);
				if (def != null) {
	...
	public boolean canBeTaught(MOB teacher, MOB student, Environmental item,
			String msg) {
		...
		if ((CMath.bset(student.getBitmap(), MOB.ATT_NOTEACH))
			...
			if (teacher.isMonster())
				CMLib.commands().postSay(teacher, student,
						"You are refusing training at this time.", true, false);
			else
				teacher.tell(student.name()
						+ " is refusing training at this time.");
			return false;
		}
		...
		if (learnThis instanceof Ability) {
			...
			if (student.fetchAbility(theA.ID()) != null) {
				if (teacher.isMonster())
					CMLib.commands().postSay(teacher, student,
							"You already know '" + teachWhat + "'.", true,
							false);
				else
					teacher.tell(student.name()
			...
			if (student.fetchExpertise(theExpertise.ID) != null) {
				if (teacher.isMonster())
					CMLib.commands().postSay(teacher, student,
							"You already know " + theExpertise.name, true,
							false);
				else
					teacher.tell(student.name() + " already knows "
							+ theExpertise.name);
				return false;
			}
			...
			if (!myQualifiedExpertises(student).contains(theExpertise)) {
				if (teacher.isMonster())
					CMLib.commands().postSay(
							teacher,
							student,
							"I'm sorry, you do not yet fully qualify for the expertise '"
									+ theExpertise.name
									+ "'.\n\rRequirements: "
					...
					teacher.tell(student.name()
							+ " does not yet fully qualify for the expertise '"
							+ theExpertise.name
							+ "'.\n\rRequirements: "
							+ CMLib.masking().maskDesc(
									theExpertise.allRequirements()));
			...
			if (!theExpertise.meetsCostRequirements(student)) {
				if (teacher.isMonster())
					CMLib.commands().postSay(
							teacher,
							student,
							"I'm sorry, but to learn the expertise '"
									+ theExpertise.name + "' requires: "
									+ theExpertise.costDescription(), true,
							false);
				else
					teacher.tell("Training for that expertise requires "
							+ theExpertise.costDescription() + ".");
				return false;
			}
			teachWhat = theExpertise.name;
		}
		try {
			if ((!teacher.isMonster())
					...
					&& (!student.session().confirm(
							"\n\r" + teacher.Name() + " wants to teach you "
									+ teachWhat + ".  Is this Ok (y/N)?", "N",
							5000))) {
				if (student.session() != null)
					student.session().println("\n\r");
				teacher.tell(student.charStats().HeShe()
	...
	public void handleBeingTaught(MOB teacher, MOB student, Environmental item,
			String msg) {
		Object learnThis = item;
		if (learnThis == null)
			learnThis = parseLearnID(msg);
		...
		if (learnThis instanceof Ability) {
			Ability theA = (Ability) learnThis;
			teacher.charStats().setStat(CharStats.STAT_WISDOM,
					teacher.charStats().getStat(CharStats.STAT_WISDOM) + 5);
			teacher.charStats()
					.setStat(
							CharStats.STAT_INTELLIGENCE,
							teacher.charStats().getStat(
									CharStats.STAT_INTELLIGENCE) + 5);
	...
	public boolean postTeach(MOB teacher, MOB student, CMObject teachObj) {
		CMMsg msg = CMClass.getMsg(teacher, student, null, CMMsg.MSG_SPEAK,
				null);
		if (!teacher.location().okMessage(teacher, msg))
			return false;
		final Environmental tool = (teachObj instanceof Environmental) ? (Environmental) teachObj
				: null;
		final String teachWhat = teachObj.name();
		final String ID = teachObj.ID();
		msg = CMClass.getMsg(teacher, student, tool, CMMsg.MSG_TEACH,
				"<S-NAME> teach(es) <T-NAMESELF> '" + teachWhat
						+ "'^<LEARN NAME=\"" + ID + "\" /^>.");
		if (!teacher.location().okMessage(teacher, msg))
			return false;
