...
public class CoffeeMaker extends StdLibrary implements GenericBuilder {
	...
	public String getGenMOBTextUnpacked(MOB mob, String newText) {
		if ((newText != null)
				&& ((newText.length() > 10) || newText.startsWith("%DBID>"))) {
			if (newText.startsWith("%DBID>")) {
				String dbstr = CMLib.database().DBReadRoomMOBData(
						newText.substring(6, newText.indexOf('@')),
						((Object) mob).getClass().getName()
								+ newText.substring(newText.indexOf('@'))
										.trim());
				if (dbstr != null)
					return dbstr;
	...
	public int envFlags(Environmental E) {
		...
		if (E instanceof Item) {
			Item item = (Item) E;
			if (!CMath.bset(item.basePhyStats().sensesMask(),
	...
	public void setEnvFlags(Environmental E, int f) {
		...
		if (E instanceof Container) {
			Container container = (Container) E;
			container.setLidsNLocks(get(f, 32), !get(f, 32), get(f, 64),
					get(f, 64));
		} else if (E instanceof Exit) {
			Exit exit = (Exit) E;
			exit.setReadable(get(f, 4));
			if (get(f, 16))
				Log.errOut("CoffeeMaker", "Exit " + identifier(E, null)
						+ " has deprecated trap flag set!");
			...
			if (get(f, 512))
				Log.errOut("CoffeeMaker", "Exit " + identifier(E, null)
						+ " has deprecated level restriction flag set!");
			if (get(f, 1024))
				Log.errOut("CoffeeMaker", "Exit " + identifier(E, null)
						+ " has deprecated class restriction flag set!");
			if (get(f, 2048))
				Log.errOut("CoffeeMaker", "Exit " + identifier(E, null)
						+ " has deprecated alignment restriction flag set!");
			exit.setDoorsNLocks(HasDoor, (!HasDoor) || (!DefaultsClosed),
					DefaultsClosed, HasLock, HasLock && DefaultsLocked,
					DefaultsLocked);
		}
	}

	public String getGenAbilityXML(Ability A) {
		return new StringBuilder("<ABILITY ID=\"").append(A.ID())
				.append("\" TYPE=\"").append(CMClass.getSimpleClassName(A))
				.append("\">").append(A.getStat("ALLXML")).append("</ABILITY>")
				.toString();
	}
	...
	public String getPropertiesStr(Environmental E, boolean fromTop) {
		...
		return (E.isGeneric() ? getGenPropertiesStr(E) : "")
				+ (fromTop ? getOrdPropertiesStr(E) : "");
	}

	public String getOrdPropertiesStr(Environmental E) {
		if (E instanceof Room) {
			StringBuilder str = new StringBuilder("");
			str.append(CMLib.xml().convertXMLtoTag("RCLIM",
					((Room) E).getClimateTypeCode()));
			str.append(CMLib.xml().convertXMLtoTag("RATMO",
					((Room) E).getAtmosphereCode()));
			if (E instanceof GridLocale) {
				str.append(CMLib.xml().convertXMLtoTag("XGRID",
						((GridLocale) E).xGridSize()));
				str.append(CMLib.xml().convertXMLtoTag("YGRID",
						((GridLocale) E).yGridSize()));
			}
			if (E instanceof LocationRoom)
				str.append(CMLib.xml().convertXMLtoTag(
						"COREDIR",
						CMParms.toStringList(((LocationRoom) E)
			...
			str.append(CMLib.xml().convertXMLtoTag("ARCHP",
					myArea.getArchivePath()));
			...
			for (Enumeration<Area> e = myArea.getParents(); e.hasMoreElements();) {
				...
				if ((A != defaultParentArea)
						|| (A.getTimeObj() != CMLib.time().globalClock())) {
					parentstr.append("<PARENT>");
					parentstr.append(CMLib.xml().convertXMLtoTag("PARENTNAMED",
							A.name()));
					parentstr.append("</PARENT>");
				}
			}
			str.append(CMLib.xml().convertXMLtoTag("PARENTS",
					parentstr.toString()));
			for (Enumeration<Area> e = myArea.getChildren(); e
					.hasMoreElements();) {
				Area A = e.nextElement();
				childrenstr.append("<CHILD>");
				childrenstr.append(CMLib.xml().convertXMLtoTag("CHILDNAMED",
						A.name()));
				childrenstr.append("</CHILD>");
			}
			str.append(CMLib.xml().convertXMLtoTag("CHILDREN",
					childrenstr.toString()));
			str.append(getExtraEnvPropertiesStr(E));
			str.append(getGenScripts((Area) E, false));
			str.append(CMLib.xml().convertXMLtoTag("AUTHOR",
					myArea.getAuthorID()));
			str.append(CMLib.xml().convertXMLtoTag("CURRENCY",
					myArea.getCurrency()));
			if (myArea instanceof SpaceShip)
				str.append(CMLib.xml()
						.convertXMLtoTag(
								"DISP",
								CMLib.xml().parseOutAngleBrackets(
										myArea.displayText())));
			Vector<String> V = new Vector<String>();
			String flag = null;
			for (Enumeration<String> f = myArea.areaBlurbFlags(); f
					.hasMoreElements();) {
				flag = f.nextElement();
				V.addElement((flag + " " + myArea.getBlurbFlag(flag)).trim());
			}
			str.append(CMLib.xml().convertXMLtoTag("BLURBS",
					CMLib.xml().getXMLList(V)));
			str.append(CMLib.xml().convertXMLtoTag("AATMO",
					((Area) E).getAtmosphereCode()));
			if (E instanceof SpaceObject)
				str.append(CMLib.xml().convertXMLtoTag("SSRADIUS",
						((SpaceObject) E).radius()));
			if (E instanceof GridZones)
				str.append(CMLib.xml().convertXMLtoTag("XGRID",
						((GridZones) E).xGridSize())
						+ CMLib.xml().convertXMLtoTag("YGRID",
								((GridZones) E).yGridSize()));
			if (E instanceof AutoGenArea) {
				str.append(CMLib.xml().convertXMLtoTag(
						"AGXMLPATH",
						CMLib.xml().parseOutAngleBrackets(
								((AutoGenArea) E).getGeneratorXmlPath())));
				str.append(CMLib.xml().convertXMLtoTag(
						"AGAUTOVAR",
						CMLib.xml().parseOutAngleBrackets(
								CMParms.toStringEqList(((AutoGenArea) E)
										.getAutoGenVariables()))));
			}
			return str.toString();
			...
			Item I = (Item) E;
			String xml = (((I instanceof Container) && (((Container) I)
					.capacity() > 0)) ? CMLib.xml().convertXMLtoTag("IID",
					"" + I) : "")
					+ CMLib.xml()
							.convertXMLtoTag("IWORN", "" + I.rawWornCode())
					+ CMLib.xml().convertXMLtoTag(
							"ILOC",
							""
									+ ((I.container() != null) ? ("" + I
											.container()) : ""))
					+ CMLib.xml().convertXMLtoTag("IUSES",
							"" + I.usesRemaining())
					+ CMLib.xml().convertXMLtoTag("ILEVL",
							"" + I.basePhyStats().level())
					+ CMLib.xml().convertXMLtoTag("IABLE",
							"" + I.basePhyStats().ability())
					+ ((E.isGeneric() ? "" : CMLib.xml().convertXMLtoTag(
							"ITEXT", "" + I.text())));
			return xml;
		} else if (E instanceof MOB) {
			MOB M = (MOB) E;
			String xml = CMLib.xml().convertXMLtoTag("MLEVL",
					"" + M.basePhyStats().level())
					+ CMLib.xml().convertXMLtoTag("MABLE",
							"" + M.basePhyStats().ability())
					+ CMLib.xml().convertXMLtoTag("MREJV",
							"" + M.basePhyStats().rejuv())
					+ ((E.isGeneric() ? "" : CMLib.xml().convertXMLtoTag(
							"MTEXT", "" + M.text())));
			return xml;
		}
	...
	public String getGenMobAbilities(MOB M) {
		...
		for (int b = 0; b < M.numAbilities(); b++) {
			...
			if ((A != null) && (A.isSavable())) {
				...
				abilitystr.append(CMLib.xml().convertXMLtoTag("ACLASS",
						CMClass.classID(A)));
				abilitystr.append(CMLib.xml().convertXMLtoTag("APROF",
						"" + A.proficiency()));
				abilitystr.append(CMLib.xml().convertXMLtoTag("ADATA",
						getPropertiesStr(A, true)));
				abilitystr.append("</ABLTY>");
			}
		}
	...
	public String getGenScripts(PhysicalAgent E, boolean includeVars) {
		...
		for (Enumeration<ScriptingEngine> e = E.scripts(); e.hasMoreElements();) {
			...
			if ((SE != null) && (SE.isSavable())) {
				...
				scriptstr.append(CMLib.xml().convertXMLtoTag("SCRIPT",
						CMLib.xml().parseOutAngleBrackets(SE.getScript())));
				scriptstr.append(CMLib.xml().convertXMLtoTag("SQN",
						"" + SE.defaultQuestName()));
				scriptstr.append(CMLib.xml().convertXMLtoTag("SSCOP",
						SE.getVarScope()));
				if ((includeVars) && (SE.getVarScope().equals("*")))
					scriptstr.append(CMLib.xml().convertXMLtoTag("SSVAR",
							SE.getLocalVarXML()));
				scriptstr.append("</SCRPT>");
			}
		}
	...
	protected void possibleAddElectronicsManufacturers(Item I,
		...
		if ((I instanceof Electronics)
				&& (custom != null)
				&& (!((Electronics) I).getManufacturerName().equalsIgnoreCase(
						"RANDOM"))
	...
	public String getGenMobInventory(MOB M) {
		...
		for (int b = 0; b < M.numItems(); b++) {
			...
			if ((I != null) && (I.isSavable())) {
				itemstr.append("<ITEM>");
				itemstr.append(CMLib.xml().convertXMLtoTag("ICLASS",
						CMClass.classID(I)));
				itemstr.append(CMLib.xml().convertXMLtoTag("IDATA",
						getPropertiesStr(I, true)));
				itemstr.append("</ITEM>");
			}
		}
	...
	public String getGenPropertiesStr(Environmental E) {
		...
		if (E instanceof Exit) {
			Exit exit = (Exit) E;
			text.append(CMLib.xml()
					.convertXMLtoTag("CLOSTX", exit.closedText())
					...
					+ CMLib.xml().convertXMLtoTag("OPENTK",
							exit.openDelayTicks()));
		}

		if (E instanceof ClanItem) {
			text.append(CMLib.xml().convertXMLtoTag("CLANID",
					"" + ((ClanItem) E).clanID()));
			text.append(CMLib.xml().convertXMLtoTag("CITYPE",
					"" + ((ClanItem) E).ciType()));
		}

		if (E instanceof Item) {
			Item item = (Item) E;
			text.append(CMLib.xml().convertXMLtoTag("IDENT",
					item.rawSecretIdentity())
					+ CMLib.xml()
							.convertXMLtoTag("VALUE", item.baseGoldValue())
					// +CMLib.xml().convertXMLtoTag("USES",item.usesRemaining())
					// // handled 'from top' & in db
					+ CMLib.xml().convertXMLtoTag("MTRAL", item.material())
					+ CMLib.xml().convertXMLtoTag("READ", item.readableText())
					+ CMLib.xml()
							.convertXMLtoTag("WORNL", item.rawLogicalAnd())
					+ CMLib.xml().convertXMLtoTag("WORNB",
							item.rawProperLocationBitmap()));
			if (E instanceof Container) {
				text.append(CMLib.xml().convertXMLtoTag("CAPA",
						((Container) item).capacity()));
				text.append(CMLib.xml().convertXMLtoTag("CONT",
						((Container) item).containTypes()));
			}
			if (E instanceof AmmunitionWeapon)
				text.append(CMLib.xml().convertXMLtoTag("CAPA",
						((AmmunitionWeapon) item).ammunitionCapacity()));

			if (E instanceof SpaceShip) {
				text.append(CMLib.xml().convertXMLtoTag(
						"SSAREA",
						CMLib.xml().parseOutAngleBrackets(
								getAreaObjectXML(
										((SpaceShip) item).getShipArea(), null,
										null, null, true).toString())));
				text.append(CMLib.xml().convertXMLtoTag("SSOML",
						((SpaceShip) item).getOMLCoeff() + ""));
				text.append(CMLib.xml().convertXMLtoTag("SSRADIUS",
						((SpaceShip) item).radius()));
			}
		}

		if (E instanceof Coins) {
			text.append(CMLib.xml().convertXMLtoTag("CRNC",
					((Coins) E).getCurrency()));
			text.append(CMLib.xml().convertXMLtoTag("DENOM",
					"" + ((Coins) E).getDenomination()));
		}
		if (E instanceof Electronics) {
			text.append(CMLib.xml().convertXMLtoTag("POWC",
					"" + ((Electronics) E).powerCapacity()));
			text.append(CMLib.xml().convertXMLtoTag("POWR",
					"" + ((Electronics) E).powerRemaining()));
			text.append(CMLib.xml().convertXMLtoTag("EACT",
					"" + ((Electronics) E).activated()));
			text.append(CMLib.xml().convertXMLtoTag("MANUFACT",
					((Electronics) E).getManufacturerName()));

		}
		if (E instanceof Electronics.ElecPanel) {
			text.append(CMLib.xml().convertXMLtoTag("SSPANELT",
					"" + ((Electronics.ElecPanel) E).panelType().name()));
		}
		if (E instanceof ShipComponent) {
			text.append(CMLib.xml().convertXMLtoTag("INSTF",
					"" + ((ShipComponent) E).getInstalledFactor()));
		}
		if (E instanceof ShipComponent.ShipEngine) {
			text.append(CMLib.xml().convertXMLtoTag("SSTHRUST",
					"" + ((ShipComponent.ShipEngine) E).getMaxThrust()));
			text.append(CMLib.xml().convertXMLtoTag("SSIMPL",
					"" + ((ShipComponent.ShipEngine) E).getSpecificImpulse()));
			text.append(CMLib.xml().convertXMLtoTag("SSFEFF",
					"" + ((ShipComponent.ShipEngine) E).getFuelEfficiency()));
		}
		if (E instanceof Electronics.PowerGenerator) {
			text.append(CMLib.xml().convertXMLtoTag(
					"EGENAMT",
					""
							+ ((Electronics.PowerGenerator) E)
									.getGeneratedAmountPerTick()));
		}
		if (E instanceof Electronics.FuelConsumer) {
			text.append(CMLib.xml().convertXMLtoTag(
					"ECONSTYP",
					CMParms.toStringList(((Electronics.PowerGenerator) E)
							.getConsumedFuelTypes())));
		}
		if (E instanceof Recipe) {
			text.append(CMLib.xml().convertXMLtoTag("SKILLID",
					((Recipe) E).getCommonSkillID()));
		...
		if (E instanceof Light)
			text.append(CMLib.xml().convertXMLtoTag("BURNOUT",
					((Light) E).destroyedWhenBurnedOut()));

		if (E instanceof Wand)
			text.append(CMLib.xml().convertXMLtoTag("MAXUSE",
					((Wand) E).maxUses()));

		if (E instanceof Rideable) {
			text.append(CMLib.xml().convertXMLtoTag("RIDET",
					((Rideable) E).rideBasis()));
			text.append(CMLib.xml().convertXMLtoTag("RIDEC",
					((Rideable) E).riderCapacity()));
		}

		if (E instanceof RawMaterial)
			text.append(CMLib.xml().convertXMLtoTag("DOMN",
					((RawMaterial) E).domainSource() + ""));

		if (E instanceof Food) {
			text.append(CMLib.xml().convertXMLtoTag("CAPA2",
					((Food) E).nourishment()));
		...
		if (E instanceof Drink) {
			text.append(CMLib.xml().convertXMLtoTag("CAPA2",
					((Drink) E).liquidHeld()));
			text.append(CMLib.xml().convertXMLtoTag("REMAN",
					((Drink) E).liquidRemaining()));
			text.append(CMLib.xml().convertXMLtoTag("LTYPE",
					((Drink) E).liquidType()));
			text.append(CMLib.xml().convertXMLtoTag("DRINK",
					((Drink) E).thirstQuenched()));
		}

		if (E instanceof Weapon) {
			text.append(CMLib.xml().convertXMLtoTag("TYPE",
					((Weapon) E).weaponType()));
			text.append(CMLib.xml().convertXMLtoTag("CLASS",
					((Weapon) E).weaponClassification()));
			text.append(CMLib.xml().convertXMLtoTag("MINR",
					((Weapon) E).minRange()));
			text.append(CMLib.xml().convertXMLtoTag("MAXR",
					((Weapon) E).maxRange()));
		}

		if (E instanceof Armor) {
			text.append(CMLib.xml().convertXMLtoTag("LAYR",
					((Armor) E).getClothingLayer()));
			text.append(CMLib.xml().convertXMLtoTag("LAYA",
					((Armor) E).getLayerAttributes()));
		}

		if (E instanceof LandTitle)
			text.append(CMLib.xml().convertXMLtoTag("LANDID",
					((LandTitle) E).landPropertyID()));
		else if (E instanceof PrivateProperty) {
			text.append(CMLib.xml().convertXMLtoTag("OWNERID",
					((PrivateProperty) E).getOwnerName()));
			text.append(CMLib.xml().convertXMLtoTag("PRICE",
					((PrivateProperty) E).getPrice()));
		}

		if (E instanceof Perfume)
			text.append(CMLib.xml().convertXMLtoTag("SMELLLST",
					((Perfume) E).getSmellList()));
		...
		if (E instanceof DeadBody) {
			if (((DeadBody) E).charStats() != null) {
				text.append(CMLib.xml().convertXMLtoTag(
						"GENDER",
						""
								+ (char) ((DeadBody) E).charStats().getStat(
										CharStats.STAT_GENDER)));
				text.append(CMLib.xml().convertXMLtoTag("MRACE",
						"" + ((DeadBody) E).charStats().getMyRace().ID()));
				text.append(CMLib.xml().convertXMLtoTag("MDNAME",
						"" + ((DeadBody) E).mobName()));
				text.append(CMLib.xml().convertXMLtoTag("MDDESC",
						"" + ((DeadBody) E).mobDescription()));
				text.append(CMLib.xml().convertXMLtoTag("MKNAME",
						"" + ((DeadBody) E).killerName()));
				text.append(CMLib.xml().convertXMLtoTag("MTOD",
						"" + ((DeadBody) E).timeOfDeath()));
				text.append(CMLib.xml().convertXMLtoTag("MKPLAY",
						"" + ((DeadBody) E).killerPlayer()));
				text.append(CMLib.xml().convertXMLtoTag("MDLMSG",
						"" + ((DeadBody) E).lastMessage()));
				text.append(CMLib.xml().convertXMLtoTag("MBREAL",
						"" + ((DeadBody) E).destroyAfterLooting()));
				text.append(CMLib.xml().convertXMLtoTag("MPLAYR",
						"" + ((DeadBody) E).playerCorpse()));
				text.append(CMLib.xml().convertXMLtoTag("MPKILL",
						"" + ((DeadBody) E).mobPKFlag()));
				if (((DeadBody) E).savedMOB() != null)
					text.append("<MOBS>" + getMobXML(((DeadBody) E).savedMOB())
							+ "</MOBS>");
				if (((DeadBody) E).killingTool() == null)
					text.append("<KLTOOL />");
				else {
					text.append("<KLTOOL>");
					text.append(CMLib.xml().convertXMLtoTag("KLCLASS",
							CMClass.classID(((DeadBody) E).killingTool())));
					text.append(CMLib
							.xml()
							.convertXMLtoTag(
									"KLDATA",
									getPropertiesStr(
											((DeadBody) E).killingTool(), true)));
					text.append("</KLTOOL>");
				}
			} else {
		...
		if (E instanceof MOB) {
			...
			text.append(CMLib.xml().convertXMLtoTag("VARMONEY",
					"" + ((MOB) E).getMoneyVariation()));
			CMLib.beanCounter().clearInventoryMoney((MOB) E, null);
			((MOB) E).setMoney(money);
			for (Pair<Clan, Integer> p : ((MOB) E).clans())
				text.append("<CLAN ROLE=").append(p.second.toString())
						.append(">").append(p.first.clanID()).append("</CLAN>");
			text.append(CMLib.xml().convertXMLtoTag(
					"GENDER",
					""
							+ (char) ((MOB) E).baseCharStats().getStat(
									CharStats.STAT_GENDER)));
			text.append(CMLib.xml().convertXMLtoTag("MRACE",
					"" + ((MOB) E).baseCharStats().getMyRace().ID()));
			...
			if (E instanceof Banker) {
				text.append(CMLib.xml().convertXMLtoTag("BANK",
						"" + ((Banker) E).bankChain()));
				text.append(CMLib.xml().convertXMLtoTag("COININT",
						"" + ((Banker) E).getCoinInterest()));
				text.append(CMLib.xml().convertXMLtoTag("ITEMINT",
						"" + ((Banker) E).getItemInterest()));
				text.append(CMLib.xml().convertXMLtoTag("LOANINT",
						"" + ((Banker) E).getLoanInterest()));
			}
			if (E instanceof PostOffice) {
				text.append(CMLib.xml().convertXMLtoTag("POSTCHAIN",
						"" + ((PostOffice) E).postalChain()));
				text.append(CMLib.xml().convertXMLtoTag("POSTMIN",
						"" + ((PostOffice) E).minimumPostage()));
				text.append(CMLib.xml().convertXMLtoTag("POSTLBS",
						"" + ((PostOffice) E).postagePerPound()));
				text.append(CMLib.xml().convertXMLtoTag("POSTHOLD",
						"" + ((PostOffice) E).holdFeePerPound()));
				text.append(CMLib.xml().convertXMLtoTag("POSTNEW",
						"" + ((PostOffice) E).feeForNewBox()));
				text.append(CMLib.xml().convertXMLtoTag("POSTHELD",
						"" + ((PostOffice) E).maxMudMonthsHeld()));
			}
			if (E instanceof Auctioneer) {
				text.append(CMLib.xml().convertXMLtoTag("AUCHOUSE",
						"" + ((Auctioneer) E).auctionHouse()));
				// text.append(CMLib.xml().convertXMLtoTag("LIVEPRICE",""+((Auctioneer)E).liveListingPrice()));
				text.append(CMLib.xml().convertXMLtoTag("TIMEPRICE",
						"" + ((Auctioneer) E).timedListingPrice()));
				text.append(CMLib.xml().convertXMLtoTag("TIMEPCT",
						"" + ((Auctioneer) E).timedListingPct()));
				// text.append(CMLib.xml().convertXMLtoTag("LIVECUT",""+((Auctioneer)E).liveFinalCutPct()));
				text.append(CMLib.xml().convertXMLtoTag("TIMECUT",
						"" + ((Auctioneer) E).timedFinalCutPct()));
				text.append(CMLib.xml().convertXMLtoTag("MAXADAYS",
						"" + ((Auctioneer) E).maxTimedAuctionDays()));
				text.append(CMLib.xml().convertXMLtoTag("MINADAYS",
						"" + ((Auctioneer) E).minTimedAuctionDays()));
			}
			if (E instanceof Deity) {
				text.append(CMLib.xml().convertXMLtoTag("CLEREQ",
						((Deity) E).getClericRequirements()));
				text.append(CMLib.xml().convertXMLtoTag("WORREQ",
						((Deity) E).getWorshipRequirements()));
				text.append(CMLib.xml().convertXMLtoTag("CLERIT",
						((Deity) E).getClericRitual()));
				text.append(CMLib.xml().convertXMLtoTag("WORRIT",
						((Deity) E).getWorshipRitual()));
				text.append(CMLib.xml().convertXMLtoTag("CLERSIT",
						((Deity) E).getClericSin()));
				text.append(CMLib.xml().convertXMLtoTag("WORRSIT",
						((Deity) E).getWorshipSin()));
				text.append(CMLib.xml().convertXMLtoTag("CLERPOW",
						((Deity) E).getClericPowerup()));
				text.append(CMLib.xml().convertXMLtoTag("SVCRIT",
						((Deity) E).getServiceRitual()));
				...
				for (int b = 0; b < ((Deity) E).numBlessings(); b++) {
					...
					itemstr.append(CMLib.xml().convertXMLtoTag("BLCLASS",
							CMClass.classID(A)));
					itemstr.append(CMLib.xml().convertXMLtoTag("BLONLY",
							"" + ((Deity) E).fetchBlessingCleric(b)));
					itemstr.append(CMLib.xml().convertXMLtoTag("BLDATA",
							getPropertiesStr(A, true)));
					itemstr.append("</BLESS>");
				}
				text.append(CMLib.xml().convertXMLtoTag("BLESSINGS",
						itemstr.toString()));
				...
				for (int b = 0; b < ((Deity) E).numCurses(); b++) {
					...
					itemstr.append(CMLib.xml().convertXMLtoTag("CUCLASS",
							CMClass.classID(A)));
					itemstr.append(CMLib.xml().convertXMLtoTag("CUONLY",
							"" + ((Deity) E).fetchCurseCleric(b)));
					itemstr.append(CMLib.xml().convertXMLtoTag("CUDATA",
							getPropertiesStr(A, true)));
					itemstr.append("</CURSE>");
				}
				text.append(CMLib.xml().convertXMLtoTag("CURSES",
						itemstr.toString()));
				...
				for (int b = 0; b < ((Deity) E).numPowers(); b++) {
					...
					itemstr.append(CMLib.xml().convertXMLtoTag("POCLASS",
							CMClass.classID(A)));
					itemstr.append(CMLib.xml().convertXMLtoTag("PODATA",
							getPropertiesStr(A, true)));
					itemstr.append("</POWER>");
				}
				text.append(CMLib.xml().convertXMLtoTag("POWERS",
						itemstr.toString()));
			}
			if (E instanceof ShopKeeper) {
				text.append(CMLib.xml().convertXMLtoTag("SELLCD",
						((ShopKeeper) E).getWhatIsSoldMask()));
				StringBuilder itemstr = new StringBuilder("");
				for (Iterator<Environmental> i = ((ShopKeeper) E).getShop()
						.getStoreInventory(); i.hasNext();) {
					Environmental E2 = i.next();
					itemstr.append("<SHITEM>");
					itemstr.append(CMLib.xml().convertXMLtoTag("SICLASS",
							CMClass.classID(E2)));
					itemstr.append(CMLib.xml().convertXMLtoTag("SITYPE",
							CMClass.getType(E2).toString()));
					itemstr.append(CMLib.xml().convertXMLtoTag("SISTOCK",
							((ShopKeeper) E).getShop().numberInStock(E2)));
					itemstr.append(CMLib.xml().convertXMLtoTag("SIPRICE",
							((ShopKeeper) E).getShop().stockPrice(E2)));
					itemstr.append(CMLib.xml().convertXMLtoTag("SIDATA",
							getPropertiesStr(E2, true)));
					itemstr.append("</SHITEM>");
				}
				text.append(CMLib.xml().convertXMLtoTag("STORE",
						itemstr.toString()));
			}
			if (((MOB) E).tattoos().hasMoreElements()) {
				text.append("<TATTS>");
				for (Enumeration<MOB.Tattoo> e = ((MOB) E).tattoos(); e
						.hasMoreElements();)
			...
			if (((MOB) E).expertises().hasMoreElements()) {
				text.append("<EDUS>");
				for (Enumeration<String> x = ((MOB) E).expertises(); x
						.hasMoreElements();)
	...
	public String unpackRoomFromXML(String buf, boolean andContent) {
		...
		List<XMLLibrary.XMLpiece> roomData = CMLib.xml().getContentsFromPieces(
				xml, "AROOM");
		if (roomData == null)
	...
	protected String unpackRoomFromXML(Area forceArea, List<XMLpiece> xml,
			boolean andContent, boolean andSave) {
		Area myArea;
		if (forceArea != null)
			myArea = forceArea;
		else
			myArea = CMLib.map().getArea(
					CMLib.xml().getValFromPieces(xml, "RAREA"));
		if (myArea == null)
		...
		if ((forceArea == null)
				&& CMLib.map().getRoom(newRoom.roomID()) != null)
		...
		newRoom.setMiscText(CMLib.xml().restoreAngleBrackets(
				CMLib.xml().getValFromPieces(xml, "RTEXT")));

		// now EXITS!
		List<XMLLibrary.XMLpiece> xV = CMLib.xml().getContentsFromPieces(xml,
				"ROOMEXITS");
		if (xV == null)
			return unpackErr("Room", "null 'xV' in room " + newRoom.roomID());
		for (int x = 0; x < xV.size(); x++) {
			XMLLibrary.XMLpiece xblk = xV.get(x);
			if ((!xblk.tag.equalsIgnoreCase("REXIT"))
					|| (xblk.contents == null))
				return unpackErr("Room", "??" + xblk.tag + " in room "
						+ newRoom.roomID());
			int dir = CMLib.xml().getIntFromPieces(xblk.contents, "XDIRE");
			String doorID = CMLib.xml()
					.getValFromPieces(xblk.contents, "XDOOR");
			if ((dir < 0) || (dir >= Directions.NUM_DIRECTIONS())) {
				if ((dir > 255) && (!(newRoom instanceof GridLocale)))
					return unpackErr("Room", "Not GridLocale, tried " + dir
							+ " exit for room '" + newRoom.roomID() + "'");
				else if (dir > 255) {
					String xdata = CMLib.xml().getValFromPieces(xblk.contents,
							"XDATA");
					List<String> CEs = CMParms.parseSemicolons(xdata.trim(),
							true);
					...
					return unpackErr("Room", "Unknown direction: " + dir
							+ " in room " + newRoom.roomID());
			} else {
				List<XMLLibrary.XMLpiece> xxV = CMLib.xml()
						.getContentsFromPieces(xblk.contents, "XEXIT");
				if (xxV == null)
					return unpackErr("Room",
							"null 'xxV' in room " + newRoom.roomID());
				Exit exit = null;
				if (xxV.size() > 0) {
					exit = CMClass.getExit(CMLib.xml().getValFromPieces(xxV,
							"EXID"));
					if (exit == null)
						return unpackErr("Room", "null 'exit' in room "
								+ newRoom.roomID());
					exit.setTemporaryDoorLink("{{#" + newRoom.roomID() + "#}}");
					exit.setMiscText(CMLib.xml().restoreAngleBrackets(
							CMLib.xml().getValFromPieces(xxV, "EXDAT")));
				...
				if (doorID.length() > 0) {
					...
					if (linkRoom != null)
						...
						newRoom.setRawExit(dir, exit); // get will get the fake
														// one too!
		...
		try {
			if (forceArea == null) {
				...
				for (Enumeration<Room> r = forceArea.getProperMap(); r
						.hasMoreElements();) {
					Room R = r.nextElement();
					synchronized (("SYNC" + R.roomID()).intern()) {
						R = CMLib.map().getRoom(R);
		...
		if (andContent) {
			...
			List<XMLLibrary.XMLpiece> cV = CMLib.xml().getContentsFromPieces(
					xml, "ROOMCONTENT");
			if (cV == null)
				return unpackErr("Room",
						"null 'cV' in room " + newRoom.roomID());
			if (cV.size() > 0) {
				Map<MOB, String> mobRideTable = new Hashtable<MOB, String>();
				List<XMLLibrary.XMLpiece> mV = CMLib.xml()
						.getContentsFromPieces(cV, "ROOMMOBS");
				if (mV != null) // return
								// unpackErr("Room","null 'mV' in room "+newRoom.roomID());
					for (int m = 0; m < mV.size(); m++) {
						XMLLibrary.XMLpiece mblk = mV.get(m);
						if ((!mblk.tag.equalsIgnoreCase("RMOB"))
								|| (mblk.contents == null))
							return unpackErr("Room", "bad 'mblk' in room "
									+ newRoom.roomID());
						String mClass = CMLib.xml().getValFromPieces(
								mblk.contents, "MCLAS");
						MOB newMOB = CMClass.getMOB(mClass);
						if (newMOB == null)
							return unpackErr("Room", "null 'mClass': " + mClass
									+ " in room " + newRoom.roomID());

						// for rideables AND leaders now!
						String iden = CMLib.xml().getValFromPieces(
								mblk.contents, "MIDEN");
						if ((iden != null) && (iden.length() > 0))
							identTable.put(iden, newMOB);

						newMOB.setMiscText(CMLib.xml().restoreAngleBrackets(
								CMLib.xml().getValFromPieces(mblk.contents,
										"MTEXT")));
						newMOB.basePhyStats().setLevel(
								CMLib.xml().getIntFromPieces(mblk.contents,
										"MLEVL"));
						newMOB.basePhyStats().setAbility(
								CMLib.xml().getIntFromPieces(mblk.contents,
										"MABLE"));
						newMOB.basePhyStats().setRejuv(
								CMLib.xml().getIntFromPieces(mblk.contents,
										"MREJV"));
						String ride = CMLib.xml().getValFromPieces(
								mblk.contents, "MRIDE");
				...
				List<XMLLibrary.XMLpiece> iV = CMLib.xml()
						.getContentsFromPieces(cV, "ROOMITEMS");
				if (iV != null) // return
								// unpackErr("Room","null 'iV' in room "+newRoom.roomID());
					for (int i = 0; i < iV.size(); i++) {
						XMLLibrary.XMLpiece iblk = iV.get(i);
						if ((!iblk.tag.equalsIgnoreCase("RITEM"))
								|| (iblk.contents == null))
							return unpackErr("Room", "bad 'iblk' in room "
									+ newRoom.roomID());
						String iClass = CMLib.xml().getValFromPieces(
								iblk.contents, "ICLAS");
						Item newItem = CMClass.getItem(iClass);
						if (newItem instanceof ArchonOnly)
							continue;
						if (newItem == null)
							return unpackErr("Room", "null 'iClass': " + iClass
									+ " in room " + newRoom.roomID());
						if ((newItem instanceof Container)
								|| (newItem instanceof Rideable)) {
							String iden = CMLib.xml().getValFromPieces(
									iblk.contents, "IIDEN");
							if ((iden != null) && (iden.length() > 0))
								identTable.put(iden, newItem);
						}
						String iloc = CMLib.xml().getValFromPieces(
								iblk.contents, "ILOCA");
						if (iloc.length() > 0)
							itemLocTable.put(newItem, iloc);
						newItem.basePhyStats().setLevel(
								CMLib.xml().getIntFromPieces(iblk.contents,
										"ILEVL"));
						newItem.basePhyStats().setAbility(
								CMLib.xml().getIntFromPieces(iblk.contents,
										"IABLE"));
						newItem.basePhyStats().setRejuv(
								CMLib.xml().getIntFromPieces(iblk.contents,
										"IREJV"));
						newItem.setUsesRemaining(CMLib.xml().getIntFromPieces(
								iblk.contents, "IUSES"));
						newItem.setOwner(newRoom); // temporary measure to take
													// care of behaviors
						newItem.setMiscText(CMLib.xml().restoreAngleBrackets(
								CMLib.xml().getValFromPieces(iblk.contents,
										"ITEXT")));
	...
	protected void fixFillingRoomUnlinkedExits(Room newRoom, Room R,
			boolean andSave) {
		boolean changed = false;
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			Exit exit = R.getRawExit(d);
			if ((exit != null)
					&& (exit.temporaryDoorLink().equalsIgnoreCase(newRoom
							.roomID()))) {
				exit.setTemporaryDoorLink("");
				R.rawDoors()[d] = newRoom;
				changed = true;
			} else if ((R.rawDoors()[d] != null)
					&& (R.rawDoors()[d].roomID().equals(newRoom.roomID()))) {
				R.rawDoors()[d] = newRoom;
				changed = true;
			}
		}
	...
	public String fillAreaAndCustomVectorFromXML(String buf,
			List<XMLpiece> area, List<CMObject> custom,
		...
		if (error.length() > 0)
			return error;
		List<XMLLibrary.XMLpiece> areaData = CMLib.xml().getContentsFromPieces(
				xml, "AREA");
		if (areaData == null)
	...
	public String fillCustomVectorFromXML(String xml, List<CMObject> custom,
			Map<String, String> externalFiles) {
	...
	public String fillCustomVectorFromXML(List<XMLpiece> xml,
			List<CMObject> custom, Map<String, String> externalFiles) {
		List<XMLLibrary.XMLpiece> aV = CMLib.xml().getContentsFromPieces(xml,
				"CUSTOM");
			...
			for (int r = 0; r < aV.size(); r++) {
				...
				if (ablk.tag.equalsIgnoreCase("RACE")) {
					...
					if (type != null) {
						...
						if (A != null) {
							A = (Ability) A.copyOf();
							String ID = CMLib.xml().getParmValue(ablk.parms,
									"ID");
							boolean exists = CMClass.getAbility(ID) != null;
							A.setStat("ALLXML", ablk.value);
							if (!exists)
								CMClass.delClass(CMObjectType.ABILITY,
										CMClass.getAbility(ID));
							custom.add(A);
						} else
					...
					Manufacturer M = (Manufacturer) CMClass
							.getCommon("DefaultManufacturer");
					if (M != null) {
						M.setXml(ablk.value);
		...
		if ((aV != null) && (externalFiles != null)) {
			for (int r = 0; r < aV.size(); r++) {
				...
				if (!ablk.tag.equalsIgnoreCase("FILE"))
					return unpackErr("Custom", "Wrong tag in custome file! "
							+ ablk.value);
				String filename = CMLib.xml().getParmValue(ablk.parms, "NAME");
				if ((filename == null) || (filename.length() == 0))
					return unpackErr("Custom", "No custom file filename! "
							+ ablk.value);
	...
	public String fillAreasVectorFromXML(String buf,
			List<List<XMLpiece>> areas, List<CMObject> custom,
			Map<String, String> externalFiles) {
		List<XMLLibrary.XMLpiece> xml = CMLib.xml().parseAllXML(buf);
		if (xml == null)
	...
	public void addAutoPropsToAreaIfNecessary(Area newArea) {
		if ((newArea != null) && (newArea.ID().equals("StdArea"))) {
			if (!CMProps.getVar(CMProps.Str.AUTOWEATHERPARMS).equalsIgnoreCase(
					"no")) {
		...
		if ((newArea != null) && (!newArea.getParents().hasMoreElements())) {
			...
			String defaultParentAreaName = CMProps
					.getVar(CMProps.Str.DEFAULTPARENTAREA);
			if ((defaultParentAreaName != null)
					&& (defaultParentAreaName.trim().length() > 0)) {
				defaultParentArea = CMLib.map().getArea(
						defaultParentAreaName.trim());
				if (defaultParentArea == null)
					Log.errOut("RoomLoader",
							"Default parent area from coffeemud.ini '"
									+ defaultParentAreaName.trim()
									+ "' is unknown.");
			}
			if (defaultParentArea != null) {
				if ((newArea != defaultParentArea)
					...
					if (defaultParentArea.canChild(newArea)
							&& (newArea.canParent(defaultParentArea))) {
	...
	public String unpackAreaFromXML(List<XMLpiece> aV, Session S,
			String overrideAreaType, boolean andRooms) {
		...
		newArea.setDescription(CMLib.coffeeFilter().safetyFilter(
				CMLib.xml().getValFromPieces(aV, "ADESC")));
		...
		newArea.setMiscText(CMLib.xml().restoreAngleBrackets(
				CMLib.xml().getValFromPieces(aV, "ADATA")));
		if (CMLib.flags().isSavable(newArea))
			CMLib.database().DBUpdateArea(newArea.Name(), newArea);
		if (andRooms) {
			List<XMLLibrary.XMLpiece> rV = CMLib.xml().getContentsFromPieces(
					aV, "AROOMS");
			if (rV == null)
				return unpackErr("Area", "null 'rV'");
			for (int r = 0; r < rV.size(); r++) {
				XMLLibrary.XMLpiece ablk = rV.get(r);
				if ((!ablk.tag.equalsIgnoreCase("AROOM"))
						|| (ablk.contents == null))
	...
	public String unpackAreaFromXML(String buf, Session S,
			String overrideAreaType, boolean andRooms) {
		List<XMLLibrary.XMLpiece> xml = CMLib.xml().parseAllXML(buf);
		if (xml == null)
	...
	public Area unpackAreaObjectFromXML(String xml) throws CMException {
		...
		newArea.setMiscText(CMLib.xml().restoreAngleBrackets(
				CMLib.xml().getValFromPieces(aV, "ADATA")));
		List<XMLLibrary.XMLpiece> rV = CMLib.xml().getContentsFromPieces(aV,
				"AROOMS");
		if (rV == null)
	...
	public StringBuffer getAreaXML(Area area, Session S, Set<CMObject> custom,
			Set<String> files, boolean andRooms) {
		return getAreaXML(area, S, custom, files, andRooms, true);
	}

	public StringBuffer getAreaObjectXML(Area area, Session S,
			Set<CMObject> custom, Set<String> files, boolean andRooms) {
		return getAreaXML(area, S, custom, files, andRooms, false);
	}

	protected StringBuffer getAreaXML(Area area, Session S,
			Set<CMObject> custom, Set<String> files, boolean andRooms,
			boolean isInDB) {
		...
		buf.append(CMLib.xml().convertXMLtoTag("ACLIM",
				area.getClimateTypeCode()));
	...
	public StringBuffer logTextDiff(String e1, String e2) {
		...
		if (end < start)
			...
			str.append(e1.substring(start, end));
		str.append("\n\r*2>");
		if (end2 < start)
			str.append("");
		else
			str.append(e2.substring(start, end2));
		return str;
	}
	...
	public void logDiff(Environmental E1, Environmental E2) {
		...
		if (E1 instanceof MOB) {
			...
			if (mob.basePhyStats().level() != dup.basePhyStats().level())
				str.append("Level- " + mob.basePhyStats().level() + "!="
						+ dup.basePhyStats().level() + "\n\r");
			if (mob.basePhyStats().ability() != dup.basePhyStats().ability())
				str.append("Ability- " + mob.basePhyStats().ability() + "!="
						+ dup.basePhyStats().ability() + "\n\r");
			...
			if (item.basePhyStats().level() != dup.basePhyStats().level())
				str.append("Level- " + item.basePhyStats().level() + "!="
						+ dup.basePhyStats().level() + "\n\r");
			if (item.basePhyStats().ability() != dup.basePhyStats().ability())
				str.append("Ability- " + item.basePhyStats().ability() + "!="
						+ dup.basePhyStats().ability() + "\n\r");
			if (item.usesRemaining() != dup.usesRemaining())
				str.append("Uses- " + item.usesRemaining() + "!="
						+ dup.usesRemaining() + "\n\r");
	...
	public StringBuffer getMobXML(MOB mob) {
		...
		buf.append(CMLib.xml().convertXMLtoTag("MLEVL",
				mob.basePhyStats().level()));
		buf.append(CMLib.xml().convertXMLtoTag("MABLE",
				mob.basePhyStats().ability()));
		buf.append(CMLib.xml().convertXMLtoTag("MREJV",
				mob.basePhyStats().rejuv()));
		buf.append(CMLib.xml().convertXMLtoTag("MTEXT",
				CMLib.xml().parseOutAngleBrackets(mob.text())));
		buf.append("</MOB>\n\r");
		return buf;
	}

	public StringBuffer getMobsXML(List<MOB> mobs, Set<CMObject> custom,
			Set<String> files, Map<String, List<MOB>> found) {
		StringBuffer buf = new StringBuffer("");
		for (MOB mob : mobs) {
			if (mob.isSavable()) {
				...
				if (dups == null) {
					...
					for (int v = 0; v < dups.size(); v++) {
						...
						int oldGender = mob.baseCharStats().getStat(
								CharStats.STAT_GENDER);
						dup.basePhyStats().setHeight(
								mob.basePhyStats().height());
						dup.basePhyStats().setWeight(
								mob.basePhyStats().weight());
						dup.baseCharStats().setStat(
								CharStats.STAT_GENDER,
								mob.baseCharStats().getStat(
										CharStats.STAT_GENDER));
						if (CMClass.classID(mob).equals(CMClass.classID(dup))
								&& (mob.basePhyStats().level() == dup
										.basePhyStats().level())
								&& (mob.basePhyStats().ability() == dup
										.basePhyStats().ability())
								&& (mob.text().equals(dup.text())))
							matched = true;
						dup.basePhyStats().setHeight(oldHeight);
						dup.basePhyStats().setWeight(oldWeight);
						dup.baseCharStats().setStat(CharStats.STAT_GENDER,
								oldGender);
						if (matched)
							break;
					}
					if (!matched) {
						for (int v = 0; v < dups.size(); v++) {
							...
							if (Log.debugChannelOn()
									&& CMSecurity
											.isDebugging(CMSecurity.DbgFlag.EXPORT))
								logDiff(mob, dup);
							dup.basePhyStats().setHeight(oldHeight);
							dup.basePhyStats().setWeight(oldWeight);
							dup.baseCharStats().setStat(CharStats.STAT_GENDER,
	...
	public StringBuffer getRoomMobs(Room room, Set<CMObject> custom,
			Set<String> files, Map<String, List<MOB>> found) {
		StringBuffer buf = new StringBuffer("");
	...
	public StringBuffer getUniqueItemXML(Item item, int type,
			Map<String, List<Item>> found, Set<String> files) {
		StringBuffer buf = new StringBuffer("");
		switch (type) {
		case 1:
		...
		if (item.displayText().length() > 0) {
			...
			if (dups == null) {
				...
				for (int v = 0; v < dups.size(); v++) {
					...
					if (CMClass.classID(item).equals(CMClass.classID(dup))
							&& (item.basePhyStats().level() == dup
									.basePhyStats().level())
							&& (item.usesRemaining() == dup.usesRemaining())
							&& (item.basePhyStats().ability() == dup
									.basePhyStats().ability())
	...
	public StringBuffer getItemXML(Item item) {
		...
		buf.append(CMLib.xml().convertXMLtoTag("ILEVL",
				item.basePhyStats().level()));
		buf.append(CMLib.xml().convertXMLtoTag("IABLE",
				item.basePhyStats().ability()));
		buf.append(CMLib.xml().convertXMLtoTag("IREJV",
				item.basePhyStats().rejuv()));
		buf.append(CMLib.xml().convertXMLtoTag("ITEXT",
				CMLib.xml().parseOutAngleBrackets(item.text())));
		buf.append("</ITEM>\n\r");
		return buf;
	}
	...
	public Item getItemFromXML(String xmlBuffer) {
		...
		if (newItem == null)
			return null;
		newItem.basePhyStats().setLevel(
				CMLib.xml().getIntFromPieces(iblk.contents, "ILEVL"));
		newItem.basePhyStats().setAbility(
				CMLib.xml().getIntFromPieces(iblk.contents, "IABLE"));
		newItem.basePhyStats().setRejuv(
				CMLib.xml().getIntFromPieces(iblk.contents, "IREJV"));
		newItem.setUsesRemaining(CMLib.xml().getIntFromPieces(iblk.contents,
				"IUSES"));
		newItem.setMiscText(CMLib.xml().restoreAngleBrackets(
				CMLib.xml().getValFromPieces(iblk.contents, "ITEXT")));
		newItem.setContainer(null);
		newItem.recoverPhyStats();
		return newItem;
	}

	public String addItemsFromXML(String xmlBuffer, List<Item> addHere,
			Session S) {
		List<XMLLibrary.XMLpiece> xml = CMLib.xml().parseAllXML(xmlBuffer);
		if (xml == null)
			return unpackErr("Items", "null 'xml'");
		List<XMLLibrary.XMLpiece> iV = CMLib.xml().getContentsFromPieces(xml,
				"ITEMS");
		if (iV == null)
		...
		for (int i = 0; i < iV.size(); i++) {
			...
			// if(S!=null) S.rawPrint(".");
			String itemClass = CMLib.xml().getValFromPieces(iblk.contents,
					"ICLAS");
			Item newItem = CMClass.getItem(itemClass);
			if ((newItem instanceof ArchonOnly)
					&& ((S == null) || (S.mob() == null) || (!CMSecurity
							.isASysOp(S.mob()))))
				continue;
			if (newItem == null)
	...
	public MOB getMobFromXML(String xmlBuffer) {
		...
		if (newMOB == null)
			return null;
		String text = CMLib.xml().restoreAngleBrackets(
				CMLib.xml().getValFromPieces(mblk.contents, "MTEXT"));
		newMOB.setMiscText(text);
		newMOB.basePhyStats().setLevel(
				CMLib.xml().getIntFromPieces(mblk.contents, "MLEVL"));
		newMOB.basePhyStats().setAbility(
				CMLib.xml().getIntFromPieces(mblk.contents, "MABLE"));
		newMOB.basePhyStats().setRejuv(
				CMLib.xml().getIntFromPieces(mblk.contents, "MREJV"));
		newMOB.recoverCharStats();
		newMOB.recoverPhyStats();
		newMOB.recoverMaxState();
		newMOB.resetToMaxState();
	...
	public String addMOBsFromXML(String xmlBuffer, List<MOB> addHere, Session S) {
		...
		List<XMLLibrary.XMLpiece> mV = CMLib.xml().getContentsFromPieces(xml,
				"MOBS");
		if (mV == null)
		...
		for (int m = 0; m < mV.size(); m++) {
			...
			String mClass = CMLib.xml()
					.getValFromPieces(mblk.contents, "MCLAS");
			MOB newMOB = CMClass.getMOB(mClass);
			if (newMOB == null)
	...
	public StringBuffer getItemsXML(List<Item> items,
			Map<String, List<Item>> found, Set<String> files, int type) {
		StringBuffer buf = new StringBuffer("");
		for (Item I : items)
	...
	public StringBuffer getRoomItems(Room room, Map<String, List<Item>> found,
			Set<String> files, int type) // 0=item, 1=weapon, 2=armor
	{
		StringBuffer buf = new StringBuffer("");
		room = makeNewRoomContent(room, false);
		if (room == null)
		...
		for (int m = 0; m < mobs.size(); m++) {
			...
			if ((M != null) && (M.isSavable())) {
				...
				if (CMLib.coffeeShops().getShopKeeper(M) != null) {
					for (Iterator<Environmental> i = CMLib.coffeeShops()
							.getShopKeeper(M).getShop().getStoreInventory(); i
							.hasNext();) {
	...
	public StringBuffer getRoomXML(Room room, Set<CMObject> custom,
			Set<String> files, boolean andContent) {
		return getRoomXML(room, custom, files, andContent, true);
	}

	protected StringBuffer getRoomXML(Room room, Set<CMObject> custom,
			Set<String> files, boolean andContent, boolean andIsInDB) {
		StringBuffer buf = new StringBuffer("");
		if (room == null)
			return buf;
		...
		buf.append(CMLib.xml().convertXMLtoTag("RTEXT",
				CMLib.xml().parseOutAngleBrackets(room.text())));
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if (((door != null) && (door.roomID().length() > 0))
				...
				if (door == null)
					...
					buf.append(CMLib.xml().convertXMLtoTag("XDOOR",
							door.roomID()));
				if (exit == null)
					buf.append("<XEXIT />");
				else {
					...
					buf.append(CMLib.xml().convertXMLtoTag("EXDAT",
							CMLib.xml().parseOutAngleBrackets(exit.text())));
					buf.append("</XEXIT>");
				}
		...
		if (room instanceof GridLocale) {
			...
			int ordinal = 0;
			for (Iterator<WorldMap.CrossExit> i = ((GridLocale) room)
					.outerExits(); i.hasNext();) {
				...
				if ((R.roomID().length() > 0) && (!done.contains(R.roomID()))) {
					...
					for (Iterator<WorldMap.CrossExit> i2 = ((GridLocale) room)
							.outerExits(); i.hasNext();) {
						WorldMap.CrossExit CE2 = i2.next();
						if ((CE2.destRoomID.equals(R.roomID()) || (CE2.destRoomID
								.startsWith(R.roomID() + "#(")))) {
							String str = CE2.x
									+ " "
									+ CE2.y
									+ " "
									+ ((CE2.out ? 256 : 512) | CE2.dir)
									+ " "
									+ CE2.destRoomID.substring(R.roomID()
											.length()) + ";";
					...
					buf.append("<REXIT>");
					buf.append(CMLib.xml().convertXMLtoTag("XDIRE",
							(256 + (++ordinal))));
					buf.append(CMLib.xml().convertXMLtoTag("XDOOR", R.roomID()));
					buf.append(CMLib.xml().convertXMLtoTag("XDATA",
							exitStr.toString()));
					buf.append("</REXIT>");
				}
			}
		...
		if (andContent) {
			...
			if (inhabs.size() == 0)
				...
				for (int i = 0; i < inhabs.size(); i++) {
					...
					if ((mob.isMonster())
							&& ((mob.amFollowing() == null) || (mob
									.amFollowing().isMonster()))) {
						possiblyAddCustomRace(mob, custom);
						possiblyAddCustomClass(mob, custom);
						possibleAddElectronicsManufacturers(mob, custom);

						buf.append("<RMOB>");
						buf.append(CMLib.xml().convertXMLtoTag("MCLAS",
								CMClass.classID(mob)));
						if ((((mob instanceof Rideable) && (((Rideable) mob)
								.numRiders() > 0))) || (mob.numFollowers() > 0))
							buf.append(CMLib.xml().convertXMLtoTag("MIDEN",
									"" + mob));
						buf.append(CMLib.xml().convertXMLtoTag("MLEVL",
								mob.basePhyStats().level()));
						buf.append(CMLib.xml().convertXMLtoTag("MABLE",
						...
						buf.append(CMLib.xml().convertXMLtoTag("MTEXT",
								CMLib.xml().parseOutAngleBrackets(mob.text())));
						if (mob.riding() != null)
							buf.append(CMLib.xml().convertXMLtoTag("MRIDE",
									"" + mob.riding()));
			...
			if (items.size() == 0)
				...
				for (int i = 0; i < items.size(); i++) {
					...
					if (item.isSavable() || (!andIsInDB)) {
						buf.append(CMLib.xml().convertXMLtoTag("ICLAS",
								CMClass.classID(item)));
						if (((item instanceof Container) && (((Container) item)
								.capacity() > 0))
								|| ((item instanceof Rideable) && (((Rideable) item)
										.numRiders() > 0)))
							buf.append(CMLib.xml().convertXMLtoTag("IIDEN",
									"" + item));
						if (item.container() == null)
							buf.append("<ILOCA />");
						else
							buf.append(CMLib.xml().convertXMLtoTag("ILOCA",
									"" + item.container()));
						buf.append(CMLib.xml().convertXMLtoTag("IREJV",
								item.basePhyStats().rejuv()));
						buf.append(CMLib.xml().convertXMLtoTag("IUSES",
								item.usesRemaining()));
						buf.append(CMLib.xml().convertXMLtoTag("ILEVL",
								item.basePhyStats().level()));
						buf.append(CMLib.xml().convertXMLtoTag("IABLE",
	...
	public void setPropertiesStr(Environmental E, String buf, boolean fromTop) {
		...
		if (V == null)
			Log.errOut("CoffeeMaker", "setPropertiesStr: null 'V': "
					+ ((E == null) ? "" : E.Name()));
		else
	...
	public void setPropertiesStr(Environmental E, List<XMLpiece> V,
			boolean fromTop) {
		if (E == null) {
	...
	public void setOrdPropertiesStr(Environmental E, List<XMLpiece> V) {
		if (V == null) {
			Log.errOut("CoffeeMaker",
					"null XML returned on " + identifier(E, null)
							+ " parse. Load aborted.");
			return;
		}
		...
		if (E instanceof Room) {
			...
			if (E instanceof GridLocale) {
				((GridLocale) E).setXGridSize(CMLib.xml().getIntFromPieces(V,
						"XGRID"));
				((GridLocale) E).setYGridSize(CMLib.xml().getIntFromPieces(V,
						"YGRID"));
			}
			if (E instanceof LocationRoom)
				((LocationRoom) E).setDirectionFromCore(CMParms
						.toDoubleArray(CMParms.parseCommas(CMLib.xml()
								.getValFromPieces(V, "COREDIR"), true)));
			((Room) E).setClimateType(CMLib.xml().getIntFromPieces(V, "RCLIM",
					((Room) E).getClimateTypeCode()));
			((Room) E).setAtmosphere(CMLib.xml().getIntFromPieces(V, "RATMO",
					((Room) E).getAtmosphereCode()));
		} else if (E instanceof Area) {
			((Area) E).setArchivePath(CMLib.xml().getValFromPieces(V, "ARCHP"));
			if (E instanceof SpaceObject)
				((SpaceObject) E).setRadius(CMLib.xml().getLongFromPieces(V,
						"SSRADIUS"));
			if (E instanceof SpaceShip)
				((Area) E).setDisplayText(CMLib.xml().getValFromPieces(V,
						"DISP"));
			((Area) E).setAuthorID(CMLib.xml().getValFromPieces(V, "AUTHOR"));
			((Area) E).setCurrency(CMLib.xml().getValFromPieces(V, "CURRENCY"));
			((Area) E).setAtmosphere(CMLib.xml().getIntFromPieces(V, "AATMO",
					((Area) E).getAtmosphereCode()));
			List<XMLLibrary.XMLpiece> VP = CMLib.xml().getContentsFromPieces(V,
					"PARENTS");
			if (VP != null) {
				for (int i = 0; i < VP.size(); i++) {
					XMLLibrary.XMLpiece ablk = VP.get(i);
					if ((!ablk.tag.equalsIgnoreCase("PARENT"))
							|| (ablk.contents == null)) {
						Log.errOut("CoffeeMaker", "Error parsing 'PARENT' of "
								+ identifier(E, null) + ".  Load aborted");
						return;
					}
					String aName = CMLib.xml().getValFromPieces(ablk.contents,
							"PARENTNAMED");
					Area A = CMLib.map().getArea(aName);
					if (A == null)
						Log.errOut("CoffeeMaker", "Unknown 'PARENT' area "
								+ identifier(E, null) + ": " + aName);
					else {
						((Area) E).addParent(A);
						A.addChild((Area) E);
					}
			...
			List<XMLLibrary.XMLpiece> VC = CMLib.xml().getContentsFromPieces(V,
					"CHILDREN");
			if (VC != null) {
				for (int i = 0; i < VC.size(); i++) {
					XMLLibrary.XMLpiece ablk = VC.get(i);
					if ((!ablk.tag.equalsIgnoreCase("CHILD"))
							|| (ablk.contents == null)) {
						Log.errOut("CoffeeMaker", "Error parsing 'CHILD' of "
								+ identifier(E, null) + ".  Load aborted");
						return;
					}
					String aName = CMLib.xml().getValFromPieces(ablk.contents,
					...
					Area A = CMLib.map().getArea(aName);
					if (A == null)
						Log.errOut("CoffeeMaker", "Unknown 'CHILD' area "
								+ identifier(E, null) + ": " + aName);
					else {
						((Area) E).addChild(A);
						A.addParent((Area) E);
			...
			for (Enumeration<String> f = ((Area) E).areaBlurbFlags(); f
					.hasMoreElements();)
				((Area) E).delBlurbFlag(f.nextElement());
			List<String> VB = CMLib.xml().parseXMLList(
					CMLib.xml().getValFromPieces(V, "BLURBS"));
			for (String s : VB)
				((Area) E).addBlurbFlag(s);
			if (E instanceof GridZones) {
				((GridZones) E).setXGridSize(CMLib.xml().getIntFromPieces(V,
						"XGRID"));
				((GridZones) E).setYGridSize(CMLib.xml().getIntFromPieces(V,
						"YGRID"));
			}
			if (E instanceof AutoGenArea) {
				((AutoGenArea) E).setGeneratorXmlPath(CMLib.xml()
						.restoreAngleBrackets(
								CMLib.xml().getValFromPieces(V, "AGXMLPATH")));
				((AutoGenArea) E).setAutoGenVariables(CMLib.xml()
						.restoreAngleBrackets(
								CMLib.xml().getValFromPieces(V, "AGAUTOVAR")));
			}
			setExtraEnvProperties(E, V);
			...
			I.basePhyStats().setAbility(
					CMLib.xml().getIntFromPieces(V, "IABLE"));
			if (!E.isGeneric())
			...
			M.basePhyStats().setAbility(
					CMLib.xml().getIntFromPieces(V, "MABLE"));
	...
	public void setGenMobAbilities(MOB M, List<XMLLibrary.XMLpiece> buf) {
		List<XMLLibrary.XMLpiece> V = CMLib.xml().getContentsFromPieces(buf,
				"ABLTYS");
		if (V == null) {
			Log.errOut("CoffeeMaker", "Error parsing 'ABLTYS' of "
					+ identifier(M, null) + ".  Load aborted");
			return;
		}
		for (int i = 0; i < V.size(); i++) {
			XMLLibrary.XMLpiece ablk = V.get(i);
			if ((!ablk.tag.equalsIgnoreCase("ABLTY"))
					|| (ablk.contents == null)) {
				Log.errOut("CoffeeMaker", "Error parsing 'ABLTY' of "
						+ identifier(M, null) + ".  Load aborted");
				return;
			}
			Ability newOne = CMClass.getAbility(CMLib.xml().getValFromPieces(
					ablk.contents, "ACLASS"));
			if (newOne == null) {
				Log.errOut("CoffeeMaker", "Unknown ability "
						+ CMLib.xml().getValFromPieces(ablk.contents, "ACLASS")
						+ " on " + identifier(M, null) + ", skipping.");
				continue;
			}
			List<XMLLibrary.XMLpiece> adat = CMLib.xml().getContentsFromPieces(
					ablk.contents, "ADATA");
			if (adat == null) {
				Log.errOut("CoffeeMaker", "Error parsing 'ABLTY DATA' of "
						+ identifier(M, null) + ".  Load aborted");
				return;
			}
	...
	public void setGenScripts(PhysicalAgent E, List<XMLpiece> buf,
			boolean restoreVars) {
		List<XMLLibrary.XMLpiece> V = CMLib.xml().getContentsFromPieces(buf,
				"SCRPTS");
		if (V == null)
		...
		for (int i = 0; i < V.size(); i++) {
			...
			if ((!sblk.tag.equalsIgnoreCase("SCRPT"))
					|| (sblk.contents == null)) {
				Log.errOut("CoffeeMaker", "Error parsing 'SCRPT' of "
						+ identifier(E, null) + ".  Load aborted");
				return;
			}
			...
			S.setSavable(true);
			String script = CMLib.xml().getValFromPieces(sblk.contents,
					"SCRIPT");
			if (script == null) {
				Log.errOut("CoffeeMaker", "Error parsing 'SCRIPT' of "
						+ identifier(E, null) + ".  Load aborted");
				continue;
			}
			...
			if (restoreVars) {
				String svars = CMLib.xml().getValFromPieces(sblk.contents,
						"SSVAR");
	...
	public void setGenMobInventory(MOB M, List<XMLpiece> buf) {
		...
		if (V == null) {
			Log.errOut("CoffeeMaker",
					"Error parsing 'INVEN' of " + identifier(M, null)
							+ ".  Load aborted");
			return;
		}
		...
		for (int i = 0; i < V.size(); i++) {
			...
			if ((!iblk.tag.equalsIgnoreCase("ITEM")) || (iblk.contents == null)) {
				Log.errOut("CoffeeMaker", "Error parsing 'ITEM' of "
						+ identifier(M, null) + ".  Load aborted");
				return;
			}
			Item newOne = CMClass.getItem(CMLib.xml().getValFromPieces(
					iblk.contents, "ICLASS"));
			if (newOne instanceof ArchonOnly)
				continue;
			if (newOne == null) {
				Log.errOut("CoffeeMaker", "Unknown item "
						+ CMLib.xml().getValFromPieces(iblk.contents, "ICLASS")
						+ " on " + identifier(M, null) + ", skipping.");
				continue;
			}
			List<XMLLibrary.XMLpiece> idat = CMLib.xml().getContentsFromPieces(
					iblk.contents, "IDATA");
			if (idat == null) {
				Log.errOut("CoffeeMaker", "Error parsing 'ITEM DATA' of "
						+ identifier(M, null) + ".  Load aborted");
				return;
			}
			...
			if ((newOne instanceof Container)
				...
				IIDmap.put(CMLib.xml().getValFromPieces(idat, "IID"),
						(Container) newOne);
			...
			if (newOne.basePhyStats().rejuv() > 0
					&& newOne.basePhyStats().rejuv() != PhyStats.NO_REJUV)
				variableEq = true;
			...
			if (item != null) {
				...
				if (ILOC != null)
					item.setContainer(IIDmap.get(ILOC));
						...
						&& (M.fetchWornItems(Wearable.WORN_WIELD, (short) 0,
								(short) 0).size() == 0))
	...
	public void populateShops(Environmental E, List<XMLpiece> buf) {
		...
		if (V == null) {
			Log.errOut("CoffeeMaker",
					"Error parsing 'STORE' of " + identifier(E, null)
							+ ".  Load aborted");
			return;
		}
		Hashtable<String, Container> IIDmap = new Hashtable<String, Container>();
		...
		for (int i = 0; i < V.size(); i++) {
			XMLLibrary.XMLpiece iblk = V.get(i);
			if ((!iblk.tag.equalsIgnoreCase("SHITEM"))
					|| (iblk.contents == null)) {
				Log.errOut("CoffeeMaker", "Error parsing 'SHITEM' of "
						+ identifier(E, null) + ".  Load aborted");
				continue;
			}
			String itemi = CMLib.xml().getValFromPieces(iblk.contents,
					"SICLASS");
			XMLpiece x = CMLib.xml()
					.getPieceFromPieces(iblk.contents, "SITYPE");
			CMClass.CMObjectType type = (x == null) ? null : CMClass
					.getTypeByNameOrOrdinal(x.value);
			int numStock = CMLib.xml().getIntFromPieces(iblk.contents,
					"SISTOCK");
			...
			List<XMLLibrary.XMLpiece> idat = CMLib.xml().getContentsFromPieces(
					iblk.contents, "SIDATA");
			if (type != null)
				newOne = (Environmental) CMClass.getByType(itemi, type);
			if ((newOne == null)
					&& ((iblk.value.indexOf("<ABLTY>") >= 0) || (iblk.value
							.indexOf("&lt;ABLTY&gt;") >= 0)))
				newOne = CMClass.getMOB(itemi);
			if (newOne == null)
				newOne = CMClass.getUnknown(itemi);
			if (newOne == null) {
				Log.errOut("CoffeeMaker", "Unknown item " + itemi + " on "
						+ identifier(E, null) + ", skipping.");
				continue;
			}
			if (idat == null) {
				Log.errOut("CoffeeMaker", "Error parsing 'SHOP DATA' of "
						+ identifier(E, null) + ".  Load aborted");
				continue;
			}
		...
		for (Iterator<Environmental> i = shopmob.getShop().getStoreInventory(); i
				.hasNext();) {
	...
	public boolean handleCatalogItem(Physical P, List<XMLpiece> buf,
			boolean fromTop) {
		setPhyStats(P.basePhyStats(), CMLib.xml().getValFromPieces(buf, "PROP"));
		if ((CMLib.flags().isCataloged(P)) && (P.isGeneric())) {
			P.setName(CMLib.xml().getValFromPieces(buf, "NAME"));
			Physical cataP = CMLib.catalog().getCatalogObj(P);
			if (cataP != null) {
				if (CMath.bset(cataP.basePhyStats().disposition(),
						PhyStats.IS_CATALOGED))
					Log.errOut("CoffeeMaker",
							"Error with catalog object " + P.Name() + ".");
				else if (cataP != P) {
					if (fromTop)
	...
	public List<String> getAllGenStats(Physical P) {
		...
		if (P instanceof MOB) {
			...
			if (((MOB) P).playerStats() != null)
				set.addAll(Arrays
						.asList(((MOB) P).playerStats().getStatCodes()));
	...
	public void setGenPropertiesStr(Environmental E, List<XMLpiece> buf) {
		...
		if ((E instanceof MOB)
				&& (CMLib.xml().getValFromPieces(buf, "GENDER").length() == 0)) {
			Log.errOut("CoffeeMaker", "MOB " + identifier(E, null)
					+ " has malformed XML. Load aborted.");
			return;
		}
		...
		if (E instanceof MOB) {
			MOB mob = (MOB) E;
			mob.baseCharStats().setStat(CharStats.STAT_GENDER,
					CMLib.xml().getValFromPieces(buf, "GENDER").charAt(0));
			List<XMLpiece> clanPieces = CMLib.xml().getPiecesFromPieces(buf,
					"CLAN");
		...
		if (E instanceof ClanItem) {
			((ClanItem) E).setClanID(CMLib.xml()
					.getValFromPieces(buf, "CLANID"));
			((ClanItem) E).setCIType(CMLib.xml()
					.getIntFromPieces(buf, "CITYPE"));
		}
		...
		if (E instanceof Item) {
			...
			if (item instanceof Container) {
				((Container) item).setCapacity(CMLib.xml().getIntFromPieces(
						buf, "CAPA"));
				((Container) item).setContainTypes(CMLib.xml()
						.getLongFromPieces(buf, "CONT"));

			}
			if (item instanceof AmmunitionWeapon)
				((AmmunitionWeapon) item).setAmmoCapacity(CMLib.xml()
						.getIntFromPieces(buf, "CAPA"));
			item.setRawLogicalAnd(CMLib.xml().getBoolFromPieces(buf, "WORNL"));
			item.setRawProperLocationBitmap(CMLib.xml().getLongFromPieces(buf,
					"WORNB"));
			item.setReadableText(CMLib.xml().getValFromPieces(buf, "READ"));
			if (item instanceof SpaceShip) {
				((SpaceShip) item).setShipArea(CMLib.xml()
						.restoreAngleBrackets(
								CMLib.xml().getValFromPieces(buf, "SSAREA")));
				((SpaceShip) item).setOMLCoeff(CMLib.xml().getDoubleFromPieces(
						buf, "SSOML"));
				((SpaceShip) item).setRadius(CMLib.xml().getLongFromPieces(buf,
						"SSRADIUS"));
			}
		}

		if (E instanceof Rideable) {
			((Rideable) E).setRideBasis(CMLib.xml().getIntFromPieces(buf,
					"RIDET"));
			((Rideable) E).setRiderCapacity(CMLib.xml().getIntFromPieces(buf,
					"RIDEC"));
		}
		if (E instanceof Electronics) {
			((Electronics) E).setPowerCapacity(CMLib.xml().getIntFromPieces(
					buf, "POWC"));
			((Electronics) E).setPowerRemaining(CMLib.xml().getIntFromPieces(
					buf, "POWR"));
			((Electronics) E).activate(CMLib.xml().getBoolFromPieces(buf,
					"EACT"));
			((Electronics) E).setManufacturerName(CMLib.xml().getValFromPieces(
					buf, "MANUFACT"));
		}
		if (E instanceof Electronics.ElecPanel) {
			final String panelType = CMLib.xml().getValFromPieces(buf,
					"SSPANELT");
			Technical.TechType type = (Technical.TechType) CMath.s_valueOf(
					Technical.TechType.class, panelType);
			if (type != null)
				((Electronics.ElecPanel) E).setPanelType(type);
		}
		if (E instanceof ShipComponent) {
			((ShipComponent) E).setInstalledFactor((float) CMLib.xml()
					.getDoubleFromPieces(buf, "INSTF"));
		}
		if (E instanceof ShipComponent.ShipEngine) {
			((ShipComponent.ShipEngine) E).setMaxThrust(CMLib.xml()
					.getIntFromPieces(buf, "SSTHRUST"));
			((ShipComponent.ShipEngine) E).setSpecificImpulse(CMLib.xml()
					.getIntFromPieces(buf, "SSIMPL"));
			((ShipComponent.ShipEngine) E).setFuelEfficiency(CMLib.xml()
					.getDoubleFromPieces(buf, "SSFEFF"));
		}
		if (E instanceof Electronics.PowerGenerator) {
			((Electronics.PowerGenerator) E).setGenerationAmountPerTick(CMLib
					.xml().getIntFromPieces(buf, "EGENAMT"));
		}
		if (E instanceof Electronics.FuelConsumer) {
			List<String> mats = CMParms.parseCommas(CMLib.xml()
					.getValFromPieces(buf, "ECONSTYP"), true);
		...
		if (E instanceof Coins) {
			...
			((Coins) E).setDenomination(CMLib.xml().getDoubleFromPieces(buf,
					"DENOM"));
		}
		if (E instanceof Recipe) {
			((Recipe) E).setCommonSkillID(CMLib.xml().getValFromPieces(buf,
					"SKILLID"));
			int numSupported = CMLib.xml().getIntFromPieces(buf, "NUMRECIPES");
			if (numSupported <= 0)
				numSupported = 1;
			((Recipe) E).setTotalRecipePages(numSupported);
			List<XMLpiece> allRecipes = CMLib.xml().getPiecesFromPieces(buf,
					"RECIPE");
			List<String> allRecipeStrings = new ArrayList<String>(
					allRecipes.size());
			for (XMLpiece piece : allRecipes)
				allRecipeStrings.add(piece.value);
			((Recipe) E).setRecipeCodeLines(allRecipeStrings
		...
		if (E instanceof LandTitle)
			((LandTitle) E).setLandPropertyID(CMLib.xml().getValFromPieces(buf,
					"LANDID"));
		else if (E instanceof PrivateProperty) {
			((PrivateProperty) E).setOwnerName(CMLib.xml().getValFromPieces(
					buf, "OWNERID"));
			((PrivateProperty) E).setPrice(CMLib.xml().getIntFromPieces(buf,
					"PRICE"));
		}

		if (E instanceof Perfume)
			((Perfume) E).setSmellList(CMLib.xml().getValFromPieces(buf,
					"SMELLLST"));

		if (E instanceof Food) {
			((Food) E).setNourishment(CMLib.xml()
					.getIntFromPieces(buf, "CAPA2"));
			((Food) E).setBite(CMLib.xml().getIntFromPieces(buf, "BITE"));
		}

		if (E instanceof RawMaterial)
			((RawMaterial) E).setDomainSource(CMLib.xml().getValFromPieces(buf,
					"DOMN"));
		...
		if (E instanceof Drink) {
			...
			if (remaining.length() > 0) {
				...
				((Drink) E).setLiquidType(CMLib.xml().getIntFromPieces(buf,
						"LTYPE"));
			} else
				((Drink) E).setLiquidRemaining(capacity);
			((Drink) E).setThirstQuenched(CMLib.xml().getIntFromPieces(buf,
					"DRINK"));
		}
		if (E instanceof Weapon) {
			((Weapon) E).setWeaponType(CMLib.xml()
					.getIntFromPieces(buf, "TYPE"));
			((Weapon) E).setWeaponClassification(CMLib.xml().getIntFromPieces(
					buf, "CLASS"));
			((Weapon) E).setRanges(CMLib.xml().getIntFromPieces(buf, "MINR"),
					CMLib.xml().getIntFromPieces(buf, "MAXR"));
		}
		if (E instanceof Armor) {
			((Armor) E).setClothingLayer(CMLib.xml().getShortFromPieces(buf,
					"LAYR"));
			((Armor) E).setLayerAttributes(CMLib.xml().getShortFromPieces(buf,
					"LAYA"));
		}
		if (E instanceof DeadBody) {
			if (((DeadBody) E).charStats() == null)
				((DeadBody) E).setCharStats((CharStats) CMClass
						.getCommon("DefaultCharStats"));
			try {
				((DeadBody) E).charStats().setStat(CharStats.STAT_GENDER,
						CMLib.xml().getValFromPieces(buf, "GENDER").charAt(0));
				((DeadBody) E).setPlayerCorpse(CMLib.xml().getBoolFromPieces(
						buf, "MPLAYR"));
				String mobName = CMLib.xml().getValFromPieces(buf, "MDNAME");
				if (mobName.length() > 0) {
					((DeadBody) E).setMobName(mobName);
					((DeadBody) E).setMobDescription(CMLib.xml()
							.getValFromPieces(buf, "MDDESC"));
					((DeadBody) E).setTimeOfDeath(CMLib.xml()
							.getLongFromPieces(buf, "MTOD"));
					((DeadBody) E).setKillerName(CMLib.xml().getValFromPieces(
							buf, "MKNAME"));
					((DeadBody) E).setKillerPlayer(CMLib.xml()
							.getBoolFromPieces(buf, "MKPLAY"));
					((DeadBody) E).setMobPKFlag(CMLib.xml().getBoolFromPieces(
							buf, "MPKILL"));
					((DeadBody) E).setDestroyAfterLooting(CMLib.xml()
							.getBoolFromPieces(buf, "MBREAL"));
					((DeadBody) E).setLastMessage(CMLib.xml().getValFromPieces(
							buf, "MDLMSG"));
					String mobsXML = CMLib.xml().getValFromPieces(buf, "MOBS");
					if ((mobsXML != null) && (mobsXML.length() > 0)) {
						List<MOB> V = new Vector<MOB>();
						String err = addMOBsFromXML("<MOBS>" + mobsXML
								+ "</MOBS>", V, null);
						if ((err.length() == 0) && (V.size() > 0))
							((DeadBody) E).setSavedMOB(V.get(0));

					}
					List<XMLLibrary.XMLpiece> dblk = CMLib.xml()
							.getContentsFromPieces(buf, "KLTOOL");
					if ((dblk != null) && (dblk.size() > 0)) {
						String itemi = CMLib.xml().getValFromPieces(dblk,
								"KLCLASS");
						List<XMLLibrary.XMLpiece> idat = CMLib.xml()
								.getContentsFromPieces(dblk, "KLDATA");
						Environmental newOne = CMClass.getUnknown(itemi);
						if (newOne == null)
							Log.errOut("CoffeeMaker", "Unknown tool " + itemi
									+ " of " + identifier(E, null)
									+ ".  Skipping.");
						else {
							setPropertiesStr(newOne, idat, true);
		...
		if (E instanceof MOB) {
			...
			if ((alignStr.length() > 0)
				...
				CMLib.factions().setAlignmentOldRange(mob,
						CMath.s_int(alignStr));
			CMLib.beanCounter().setMoney(mob,
					CMLib.xml().getIntFromPieces(buf, "MONEY"));
			mob.setMoneyVariation(CMLib.xml().getDoubleFromPieces(buf,
					"VARMONEY"));
			...
			if (E instanceof Banker) {
				((Banker) E).setBankChain(CMLib.xml().getValFromPieces(buf,
						"BANK"));
				((Banker) E).setCoinInterest(CMLib.xml().getDoubleFromPieces(
						buf, "COININT"));
				((Banker) E).setItemInterest(CMLib.xml().getDoubleFromPieces(
						buf, "ITEMINT"));
			...
			if (E instanceof PostOffice) {
				((PostOffice) E).setPostalChain(CMLib.xml().getValFromPieces(
						buf, "POSTCHAIN"));
				((PostOffice) E).setMinimumPostage(CMLib.xml()
						.getDoubleFromPieces(buf, "POSTMIN"));
				((PostOffice) E).setPostagePerPound(CMLib.xml()
						.getDoubleFromPieces(buf, "POSTLBS"));
				((PostOffice) E).setHoldFeePerPound(CMLib.xml()
						.getDoubleFromPieces(buf, "POSTHOLD"));
				((PostOffice) E).setFeeForNewBox(CMLib.xml()
						.getDoubleFromPieces(buf, "POSTNEW"));
				((PostOffice) E).setMaxMudMonthsHeld(CMLib.xml()
						.getIntFromPieces(buf, "POSTHELD"));
			}

			if (E instanceof Auctioneer) {
				((Auctioneer) E).setAuctionHouse(CMLib.xml().getValFromPieces(
						buf, "AUCHOUSE"));
				// ((Auctioneer)E).setLiveListingPrice(CMLib.xml().getDoubleFromPieces(buf,"LIVEPRICE"));
				((Auctioneer) E).setTimedListingPrice(CMLib.xml()
						.getDoubleFromPieces(buf, "TIMEPRICE"));
				((Auctioneer) E).setTimedListingPct(CMLib.xml()
						.getDoubleFromPieces(buf, "TIMEPCT"));
				// ((Auctioneer)E).setLiveFinalCutPct(CMLib.xml().getDoubleFromPieces(buf,"LIVECUT"));
				((Auctioneer) E).setTimedFinalCutPct(CMLib.xml()
						.getDoubleFromPieces(buf, "TIMECUT"));
				((Auctioneer) E).setMaxTimedAuctionDays(CMLib.xml()
						.getIntFromPieces(buf, "MAXADAYS"));
				((Auctioneer) E).setMinTimedAuctionDays(CMLib.xml()
						.getIntFromPieces(buf, "MINADAYS"));
			}

			if (E instanceof Deity) {
				Deity godmob = (Deity) E;
				godmob.setClericRequirements(CMLib.xml().getValFromPieces(buf,
						"CLEREQ"));
				godmob.setWorshipRequirements(CMLib.xml().getValFromPieces(buf,
						"WORREQ"));
				godmob.setClericRitual(CMLib.xml().getValFromPieces(buf,
						"CLERIT"));
				godmob.setWorshipRitual(CMLib.xml().getValFromPieces(buf,
						"WORRIT"));
				godmob.setClericSin(CMLib.xml()
						.getValFromPieces(buf, "CLERSIT"));
				godmob.setWorshipSin(CMLib.xml().getValFromPieces(buf,
						"WORRSIT"));
				godmob.setClericPowerup(CMLib.xml().getValFromPieces(buf,
						"CLERPOW"));
				godmob.setServiceRitual(CMLib.xml().getValFromPieces(buf,
						"SVCRIT"));

				List<XMLLibrary.XMLpiece> V = CMLib.xml()
						.getContentsFromPieces(buf, "BLESSINGS");
				if (V == null) {
					Log.errOut("CoffeeMaker", "Error parsing 'BLESSINGS' of "
							+ identifier(E, null) + ".  Load aborted");
					return;
				}
				for (int i = 0; i < V.size(); i++) {
					XMLLibrary.XMLpiece ablk = V.get(i);
					if ((!ablk.tag.equalsIgnoreCase("BLESS"))
							|| (ablk.contents == null)) {
						Log.errOut("CoffeeMaker", "Error parsing 'BLESS' of "
								+ identifier(E, null) + ".  Load aborted");
						return;
					}
					Ability newOne = CMClass.getAbility(CMLib.xml()
							.getValFromPieces(ablk.contents, "BLCLASS"));
					if (newOne == null) {
						Log.errOut(
								"CoffeeMaker",
								"Unknown bless "
										+ CMLib.xml().getValFromPieces(
												ablk.contents, "BLCLASS")
										+ " on " + identifier(E, null)
										+ ", skipping.");
						continue;
					}
					boolean clericsOnly = CMLib.xml().getBoolFromPieces(
							ablk.contents, "BLONLY");
					List<XMLLibrary.XMLpiece> adat = CMLib.xml()
							.getContentsFromPieces(ablk.contents, "BLDATA");
					if (adat == null) {
						Log.errOut(
								"CoffeeMaker",
								"Error parsing 'BLESS DATA' of "
										+ identifier(E, null)
										+ ".  Load aborted");
						return;
					}
				...
				if (V != null) {
					for (int i = 0; i < V.size(); i++) {
						XMLLibrary.XMLpiece ablk = V.get(i);
						if ((!ablk.tag.equalsIgnoreCase("CURSE"))
								|| (ablk.contents == null)) {
							Log.errOut(
									"CoffeeMaker",
									"Error parsing 'CURSE' of "
											+ identifier(E, null)
											+ ".  Load aborted");
							return;
						}
						Ability newOne = CMClass.getAbility(CMLib.xml()
								.getValFromPieces(ablk.contents, "CUCLASS"));
						if (newOne == null) {
							Log.errOut(
									"CoffeeMaker",
									"Unknown curse "
											+ CMLib.xml().getValFromPieces(
													ablk.contents, "CUCLASS")
											+ " on " + identifier(E, null)
											+ ", skipping.");
							continue;
						}
						boolean clericsOnly = CMLib.xml().getBoolFromPieces(
								ablk.contents, "CUONLY");
						List<XMLLibrary.XMLpiece> adat = CMLib.xml()
								.getContentsFromPieces(ablk.contents, "CUDATA");
						if (adat == null) {
							Log.errOut("CoffeeMaker",
									"Error parsing 'CURSE DATA' of "
											+ identifier(E, null)
											+ ".  Load aborted");
							return;
						}
				...
				if (V != null) {
					for (int i = 0; i < V.size(); i++) {
						XMLLibrary.XMLpiece ablk = V.get(i);
						if ((!ablk.tag.equalsIgnoreCase("POWER"))
								|| (ablk.contents == null)) {
							Log.errOut(
									"CoffeeMaker",
									"Error parsing 'POWER' of "
											+ identifier(E, null)
											+ ".  Load aborted");
							return;
						}
						Ability newOne = CMClass.getAbility(CMLib.xml()
								.getValFromPieces(ablk.contents, "POCLASS"));
						if (newOne == null) {
							Log.errOut(
									"CoffeeMaker",
									"Unknown power "
											+ CMLib.xml().getValFromPieces(
													ablk.contents, "POCLASS")
											+ " on " + identifier(E, null)
											+ ", skipping.");
							continue;
						}
						List<XMLLibrary.XMLpiece> adat = CMLib.xml()
								.getContentsFromPieces(ablk.contents, "PODATA");
						if (adat == null) {
							Log.errOut("CoffeeMaker",
									"Error parsing 'POWER DATA' of "
											+ identifier(E, null)
											+ ".  Load aborted");
							return;
						}
			...
			List<String> V9 = CMParms.parseSemicolons(CMLib.xml()
					.getValFromPieces(buf, "TATTS"), true);
			for (Enumeration<MOB.Tattoo> e = ((MOB) E).tattoos(); e
					.hasMoreElements();)
			...
			V9 = CMParms.parseSemicolons(
					CMLib.xml().getValFromPieces(buf, "EDUS"), true);
	...
	public String getAccountXML(PlayerAccount account, Set<CMObject> custom,
			Set<String> files) {
		if (account == null)
			return "";
		...
		for (Enumeration<MOB> m = account.getLoadPlayers(); m.hasMoreElements();) {
			MOB M = m.nextElement();
			xml.append("<PLAYER>").append(getPlayerXML(M, custom, files))
					.append("</PLAYER>");
		}
		xml.append("</PLAYERS>");
		return xml.toString();
	...
	public String getPlayerXML(MOB mob, Set<CMObject> custom, Set<String> files) {
		...
		str.append(CMLib.xml().convertXMLtoTag("CLASS",
				mob.baseCharStats().getMyClassesStr()));
		str.append(CMLib.xml().convertXMLtoTag("RACE",
		...
		for (int i : CharStats.CODES.BASE())
			str.append(CMLib.xml().convertXMLtoTag(
					CMStrings.limit(CharStats.CODES.NAME(i), 3),
					mob.baseCharStats().getStat(i)));
		str.append(CMLib.xml().convertXMLtoTag("HIT",
				mob.baseState().getHitPoints()));
		str.append(CMLib.xml().convertXMLtoTag("LVL",
				mob.baseCharStats().getMyLevelsStr()));
		str.append(CMLib.xml().convertXMLtoTag("MANA",
				mob.baseState().getMana()));
		str.append(CMLib.xml().convertXMLtoTag("MOVE",
				mob.baseState().getMovement()));
		...
		str.append(CMLib.xml().convertXMLtoTag("ROID",
				strStartRoomID + "||" + strOtherRoomID));
		...
		str.append(CMLib.xml().convertXMLtoTag("ATTA",
				mob.basePhyStats().attackAdjustment()));
		str.append(CMLib.xml().convertXMLtoTag("AMOR",
				mob.basePhyStats().armor()));
		str.append(CMLib.xml().convertXMLtoTag("DAMG",
		...
		str.append(CMLib.xml().convertXMLtoTag("HEIT",
				mob.basePhyStats().height()));
		str.append(CMLib.xml().convertXMLtoTag("WEIT",
				mob.basePhyStats().weight()));
		str.append(CMLib.xml().convertXMLtoTag("PRPT",
				CMLib.xml().parseOutAngleBrackets(pstats.getPrompt())));
		str.append(CMLib.xml().convertXMLtoTag("COLR", pstats.getColorStr()));
		for (Pair<Clan, Integer> p : mob.clans())
			str.append("<CLAN ROLE=").append(p.second.toString()).append(">")
					.append(p.first.clanID()).append("</CLAN>");
		...
		str.append(CMLib.xml().convertXMLtoTag("SAVE",
		...
		for (int f = 0; f < mob.numFollowers(); f++) {
			...
			if ((thisMOB != null) && (thisMOB.isMonster())
				...
				fols.append(CMLib.xml().convertXMLtoTag("FCLAS",
						CMClass.classID(thisMOB)));
				fols.append(CMLib.xml()
						.convertXMLtoTag("FTEXT", thisMOB.text()));
				fols.append(CMLib.xml().convertXMLtoTag("FLEVL",
						thisMOB.basePhyStats().level()));
				fols.append(CMLib.xml().convertXMLtoTag("FABLE",
	...
	protected String addPlayersOnlyFromXML(List<XMLLibrary.XMLpiece> mV,
			List<MOB> addMobs, Session S) {
		for (int m = 0; m < mV.size(); m++) {
			XMLLibrary.XMLpiece mblk = mV.get(m);
			if ((!mblk.tag.equalsIgnoreCase("PLAYER"))
					|| (mblk.contents == null))
			...
			mob.playerStats().setPassword(
					CMLib.xml().getValFromPieces(mblk.contents, "PASS"));
			mob.baseCharStats().setMyClasses(
					CMLib.xml().getValFromPieces(mblk.contents, "CLASS"));
			mob.baseCharStats().setMyLevels(
					CMLib.xml().getValFromPieces(mblk.contents, "LVL"));
			int level = 0;
			for (int i = 0; i < mob.baseCharStats().numClasses(); i++)
				level += mob.baseCharStats().getClassLevel(
						mob.baseCharStats().getMyClass(i));
			mob.basePhyStats().setLevel(level);
			mob.baseCharStats().setMyRace(
					CMClass.getRace(CMLib.xml().getValFromPieces(mblk.contents,
							"RACE")));
			mob.baseCharStats().setStat(
					CharStats.STAT_GENDER,
					CMLib.xml().getValFromPieces(mblk.contents, "GEND")
							.charAt(0));
			for (int i : CharStats.CODES.BASE())
				mob.baseCharStats().setStat(
						i,
						CMLib.xml().getIntFromPieces(mblk.contents,
								CMStrings.limit(CharStats.CODES.NAME(i), 3)));
			mob.baseState().setHitPoints(
					CMLib.xml().getIntFromPieces(mblk.contents, "HIT"));
			mob.baseState().setMana(
					CMLib.xml().getIntFromPieces(mblk.contents, "MANA"));
			mob.baseState().setMovement(
					CMLib.xml().getIntFromPieces(mblk.contents, "MOVE"));
			String alignStr = CMLib.xml().getValFromPieces(mblk.contents,
					"ALIG");
			if ((alignStr.length() > 0)
					&& (CMLib.factions().getFaction(CMLib.factions().AlignID()) != null))
				CMLib.factions().setAlignmentOldRange(mob,
						CMath.s_int(alignStr));
			mob.setExperience(CMLib.xml()
					.getIntFromPieces(mblk.contents, "EXP"));
			mob.setExpNextLevel(CMLib.xml().getIntFromPieces(mblk.contents,
					"EXLV"));
			mob.setWorshipCharID(CMLib.xml().getValFromPieces(mblk.contents,
					"WORS"));
			mob.setPractices(CMLib.xml()
					.getIntFromPieces(mblk.contents, "PRAC"));
			mob.setTrains(CMLib.xml().getIntFromPieces(mblk.contents, "TRAI"));
			mob.setAgeMinutes(CMLib.xml().getIntFromPieces(mblk.contents,
					"AGEH"));
			mob.setWimpHitPoint(CMLib.xml().getIntFromPieces(mblk.contents,
					"WIMP"));
			mob.setQuestPoint(CMLib.xml().getIntFromPieces(mblk.contents,
					"QUES"));
			...
			mob.playerStats().setLastDateTime(
					CMLib.xml().getLongFromPieces(mblk.contents, "DATE"));
			mob.playerStats().setChannelMask(
					CMLib.xml().getIntFromPieces(mblk.contents, "CHAN"));
			mob.basePhyStats().setAttackAdjustment(
					CMLib.xml().getIntFromPieces(mblk.contents, "ATTA"));
			mob.basePhyStats().setArmor(
					CMLib.xml().getIntFromPieces(mblk.contents, "AMOR"));
			mob.basePhyStats().setDamage(
					CMLib.xml().getIntFromPieces(mblk.contents, "DAMG"));
			mob.setBitmap(CMLib.xml().getIntFromPieces(mblk.contents, "BTMP"));
			mob.setLiegeID(CMLib.xml().getValFromPieces(mblk.contents, "LEIG"));
			mob.basePhyStats().setHeight(
					CMLib.xml().getIntFromPieces(mblk.contents, "HEIT"));
			mob.basePhyStats().setWeight(
					CMLib.xml().getIntFromPieces(mblk.contents, "WEIT"));
			mob.playerStats().setPrompt(
					CMLib.xml()
							.restoreAngleBrackets(
									CMLib.xml().getValFromPieces(mblk.contents,
											"PRPT")));
			String colorStr = CMLib.xml().getValFromPieces(mblk.contents,
					"COLR");
			if ((colorStr != null) && (colorStr.length() > 0)
					&& (!colorStr.equalsIgnoreCase("NULL")))
				mob.playerStats().setColorStr(colorStr);
			List<XMLpiece> clanPieces = CMLib.xml().getPiecesFromPieces(
					mblk.contents, "CLAN");
			String oldRole = CMLib.xml()
					.getValFromPieces(mblk.contents, "CLRO");
			if ((clanPieces.size() == 1) && (oldRole != null)
					&& (oldRole.length() > 0))
				mob.setClan(clanPieces.get(0).value, CMath.s_int(oldRole));
			else
				for (XMLpiece p : clanPieces)
					mob.setClan(p.value, CMath.s_int(p.parms.get("ROLE")));
			mob.playerStats().setLastIP(
					CMLib.xml().getValFromPieces(mblk.contents, "LSIP"));
			mob.playerStats().setEmail(
					CMLib.xml().getValFromPieces(mblk.contents, "EMAL"));
			String buf = CMLib.xml().getValFromPieces(mblk.contents, "CMPFIL");
			mob.playerStats().setXML(buf);
			List<String> V9 = CMParms.parseSemicolons(CMLib.xml()
					.returnXMLValue(buf, "TATTS"), true);
			for (Enumeration<MOB.Tattoo> e = mob.tattoos(); e.hasMoreElements();)
				mob.delTattoo(e.nextElement());
			for (int v = 0; v < V9.size(); v++)
				mob.addTattoo(CMLib.database().parseTattoo(V9.get(v)));
			V9 = CMParms.parseSemicolons(CMLib.xml()
					.returnXMLValue(buf, "EDUS"), true);
			mob.delAllExpertises();
			for (int v = 0; v < V9.size(); v++)
				mob.addExpertise(V9.get(v));
			mob.baseCharStats().setNonBaseStatsFromString(
					CMLib.xml().getValFromPieces(mblk.contents, "SAVE"));
			mob.setDescription(CMLib.xml().getValFromPieces(mblk.contents,
					"DESC"));
			...
			List<XMLLibrary.XMLpiece> iV = CMLib.xml().getContentsFromPieces(
					mblk.contents, "FOLLOWERS");
			if (iV == null)
				return unpackErr("PFols", "null 'iV'");
			for (int i = 0; i < iV.size(); i++) {
				XMLLibrary.XMLpiece fblk = iV.get(i);
				if ((!fblk.tag.equalsIgnoreCase("FOLLOWER"))
						|| (fblk.contents == null))
					return unpackErr("PFols", "??" + fblk.tag);
				String mobClass = CMLib.xml().getValFromPieces(fblk.contents,
						"FCLAS");
				MOB newFollower = CMClass.getMOB(mobClass);
				if (newFollower == null)
					return unpackErr("PFols", "null 'iClass': " + mobClass);
				newFollower.basePhyStats().setLevel(
						CMLib.xml().getIntFromPieces(fblk.contents, "FLEVL"));
				newFollower.basePhyStats().setAbility(
						CMLib.xml().getIntFromPieces(fblk.contents, "FABLE"));
				newFollower.setMiscText(CMLib.xml().getValFromPieces(
						fblk.contents, "FTEXT"));
	...
	public String addPlayersAndAccountsFromXML(String xmlBuffer,
			List<PlayerAccount> addAccounts, List<MOB> addMobs, Session S) {
		List<XMLLibrary.XMLpiece> xml = CMLib.xml().parseAllXML(xmlBuffer);
		if (xml == null)
		...
		if (mV != null)
			...
			for (int m = 0; m < mV.size(); m++) {
				XMLLibrary.XMLpiece mblk = mV.get(m);
				if ((!mblk.tag.equalsIgnoreCase("ACCOUNT"))
						|| (mblk.contents == null))
					return unpackErr("ACCOUNTs", "bad 'mblk'");
				PlayerAccount account = null;
				account = (PlayerAccount) CMClass
						.getCommon("DefaultPlayerAccount");
				String name = CMLib.xml().getValFromPieces(mblk.contents,
						"NAME");
				String password = CMLib.xml().getValFromPieces(mblk.contents,
						"PASS");
				XMLLibrary.XMLpiece xmlPiece = CMLib.xml().getPieceFromPieces(
						mblk.contents, "AXML");
				String accountXML = xmlPiece.value;
				XMLLibrary.XMLpiece playersPiece = CMLib.xml()
						.getPieceFromPieces(mblk.contents, "PLAYERS");
				Vector<String> names = new Vector<String>();
				List<MOB> accountMobs = new Vector<MOB>();
				String err = addPlayersOnlyFromXML(playersPiece.contents,
						accountMobs, S);
				if (err.length() > 0)
					return err;
	...
	public String getExtraEnvPropertiesStr(Environmental E) {
		...
		if (E instanceof Economics) {
			text.append(CMLib.xml().convertXMLtoTag("PREJFC",
					((Economics) E).prejudiceFactors()));
			text.append(CMLib.xml().convertXMLtoTag("IGNMSK",
					((Economics) E).ignoreMask()));
			text.append(CMLib.xml().convertXMLtoTag("BUDGET",
					((Economics) E).budget()));
			text.append(CMLib.xml().convertXMLtoTag("DEVALR",
					((Economics) E).devalueRate()));
			text.append(CMLib.xml().convertXMLtoTag("INVRER",
					((Economics) E).invResetRate()));
			...
			if (prics.length == 0)
				...
				for (int p = 0; p < prics.length; p++)
					text.append(CMLib.xml().convertXMLtoTag("IPRIC",
							CMLib.xml().parseOutAngleBrackets(prics[p])));
				text.append("</IPRICS>");
			}
		...
		if (E instanceof PhysicalAgent) {
			...
			for (Enumeration<Behavior> e = P.behaviors(); e.hasMoreElements();) {
				...
				if (B != null) {
					...
					behaviorstr.append(CMLib.xml().convertXMLtoTag("BCLASS",
							CMClass.classID(B)));
					behaviorstr.append(CMLib.xml().convertXMLtoTag("BPARMS",
							CMLib.xml().parseOutAngleBrackets(B.getParms())));
					behaviorstr.append("</BHAVE>");
				}
			}
			text.append(CMLib.xml().convertXMLtoTag("BEHAVES",
					behaviorstr.toString()));
		}
		...
		if (E instanceof Physical) {
			...
			for (int a = 0; a < P.numEffects(); a++) // definitely personal
			{
				...
				if ((A != null) && (A.isSavable())) {
					...
					affectstr.append(CMLib.xml().convertXMLtoTag("ACLASS",
							CMClass.classID(A)));
					affectstr.append(CMLib.xml().convertXMLtoTag("ATEXT",
							CMLib.xml().parseOutAngleBrackets(A.text())));
					affectstr.append("</AFF>");
				}
			}
			text.append(CMLib.xml().convertXMLtoTag("AFFECS",
					affectstr.toString()));
		}

		String[] codes = E.getStatCodes();
		for (int i = E.getSaveStatIndex(); i < codes.length; i++)
			text.append(CMLib.xml().convertXMLtoTag(codes[i].toUpperCase(),
					E.getStat(codes[i].toUpperCase())));
		return text.toString();
	}
	...
	public void fillFileSet(Environmental E, Set<String> H) {
		...
		if (E instanceof PhysicalAgent) {
			...
			for (Enumeration<ScriptingEngine> e = P.scripts(); e
					.hasMoreElements();) {
				ScriptingEngine SE = e.nextElement();
				if (SE != null)
	...
	public String getPhyStatsStr(PhyStats E) {
		return E.ability() + "|" + E.armor() + "|" + E.attackAdjustment() + "|"
				+ E.damage() + "|" + E.disposition() + "|" + E.level() + "|"
				+ E.rejuv() + "|" + E.speed() + "|" + E.weight() + "|"
				+ E.height() + "|" + E.sensesMask() + "|";
	}

	public String getCharStateStr(CharState E) {
		return E.getFatigue() + "|" + E.getHitPoints() + "|" + E.getHunger()
				+ "|" + E.getMana() + "|" + E.getMovement() + "|"
				+ E.getThirst() + "|";
	}
	...
	public String getEnvPropertiesStr(Environmental E) {
		...
		if (E instanceof Physical)
			text.append(CMLib.xml().convertXMLtoTag("PROP",
					getPhyStatsStr(((Physical) E).basePhyStats())));
	...
	public void setPhyStats(PhyStats E, String props) {
		...
		for (int y = 0; y < props.length(); y++)
			if (props.charAt(y) == '|') {
				try {
					nums[x] = Double.valueOf(props.substring(lastBar, y))
							.doubleValue();
				} catch (Exception e) {
		...
		if (lastBar < props.length()) {
			try {
				nums[x] = Double.valueOf(props.substring(lastBar))
						.doubleValue();
			} catch (Exception e) {
	...
	public void setEnvProperties(Environmental E, List<XMLpiece> buf) {
		...
		if (E instanceof Physical)
			setPhyStats(((Physical) E).basePhyStats(), CMLib.xml()
					.getValFromPieces(buf, "PROP"));
	...
	public void setExtraEnvProperties(Environmental E, List<XMLpiece> buf) {
		...
		if (E instanceof Economics) {
			((Economics) E).setPrejudiceFactors(CMLib.xml().getValFromPieces(
					buf, "PREJFC"));
			((Economics) E).setIgnoreMask(CMLib.xml().getValFromPieces(buf,
					"IGNMSK"));
			((Economics) E).setBudget(CMLib.xml().getValFromPieces(buf,
					"BUDGET"));
			((Economics) E).setDevalueRate(CMLib.xml().getValFromPieces(buf,
					"DEVALR"));
			((Economics) E).setInvResetRate(CMLib.xml().getIntFromPieces(buf,
					"INVRER"));
			List<XMLLibrary.XMLpiece> iV = CMLib.xml().getContentsFromPieces(
					buf, "IPRICS");
			if (iV != null) {
				...
				for (int i = 0; i < iV.size(); i++) {
					XMLLibrary.XMLpiece iblk = iV.get(i);
					if ((!iblk.tag.equalsIgnoreCase("IPRIC"))
							|| (iblk.contents == null)) {
						Log.errOut("CoffeeMaker", "Error parsing 'IPRICS' of "
								+ identifier(E, null) + ".  Load aborted");
						continue;
					}
		...
		if (E instanceof PhysicalAgent) {
			List<XMLLibrary.XMLpiece> V = CMLib.xml().getContentsFromPieces(
					buf, "BEHAVES");
			if (V == null) {
				Log.errOut("CoffeeMaker", "Error parsing 'BEHAVES' of "
						+ identifier(E, null) + ".  Load aborted");
				return;
			}
			for (int i = 0; i < V.size(); i++) {
				XMLLibrary.XMLpiece ablk = V.get(i);
				if ((!ablk.tag.equalsIgnoreCase("BHAVE"))
						|| (ablk.contents == null)) {
					Log.errOut("CoffeeMaker", "Error parsing 'BHAVE' of "
							+ identifier(E, null) + ".  Load aborted");
					return;
				}
				Behavior newOne = CMClass.getBehavior(CMLib.xml()
						.getValFromPieces(ablk.contents, "BCLASS"));
				String bparms = CMLib.xml().getValFromPieces(ablk.contents,
						"BPARMS");
				if (newOne == null) {
					Log.errOut(
							"CoffeeMaker",
		...
		if (E instanceof Physical) {
			List<XMLLibrary.XMLpiece> V = CMLib.xml().getContentsFromPieces(
					buf, "AFFECS");
			if (V == null) {
				Log.errOut("CoffeeMaker", "Error parsing 'AFFECS' of "
						+ identifier(E, null) + ".  Load aborted");
				return;
			}
			for (int i = 0; i < V.size(); i++) {
				XMLLibrary.XMLpiece ablk = V.get(i);
				if ((!ablk.tag.equalsIgnoreCase("AFF"))
						|| (ablk.contents == null)) {
					Log.errOut("CoffeeMaker", "Error parsing 'AFF' of "
							+ identifier(E, null) + ".  Load aborted");
					return;
				}
				Ability newOne = CMClass.getAbility(CMLib.xml()
						.getValFromPieces(ablk.contents, "ACLASS"));
				String aparms = CMLib.xml().getValFromPieces(ablk.contents,
						"ATEXT");
				if (newOne == null) {
					Log.errOut(
							"CoffeeMaker",
		...
		String[] codes = E.getStatCodes();
		for (int i = E.getSaveStatIndex(); i < codes.length; i++) {
			String val = CMLib.xml().getValFromPieces(buf,
					codes[i].toUpperCase());
			if (val == null)
				val = "";
		...
		case 11:
			return ""
					+ (CMath.bset(I.basePhyStats().sensesMask(),
							PhyStats.SENSE_ITEMREADABLE)); // isreadable
		case 12:
			return ""
					+ (!CMath.bset(I.basePhyStats().sensesMask(),
							PhyStats.SENSE_ITEMNODROP)); // isdroppable
		case 13:
			return ""
					+ (!CMath.bset(I.basePhyStats().sensesMask(),
							PhyStats.SENSE_ITEMNOREMOVE)); // isremovable
		case 14:
	...
	public void setGenItemStat(Item I, String code, String val) {
		...
		case 8: {
			if (CMath.isLong(val) || (val.trim().length() == 0)) // properworn
				...
				for (Iterator<String> e = V.iterator(); e.hasNext();) {
					...
					if (wornIndex >= 0)
						I.setRawProperLocationBitmap(I
		...
		case 16: {
			if (CMath.isInteger(val) || (val.trim().length() == 0))
				I.basePhyStats()
						.setDisposition(CMath.s_parseIntExpression(val)); // disposition
			else {
				I.basePhyStats().setDisposition(0);
				List<String> V = CMParms.parseCommas(val, true);
				for (Iterator<String> e = V.iterator(); e.hasNext();) {
					val = e.next();
					int dispIndex = CMParms.indexOfIgnoreCase(
							PhyStats.IS_CODES, val);
					if (dispIndex >= 0)
						I.basePhyStats().setDisposition(
								I.basePhyStats().disposition()
										| (int) CMath.pow(2, dispIndex));
				}
			}
			break;
		}
		...
		case 20:
			I.basePhyStats().setAttackAdjustment(
					CMath.s_parseIntExpression(val));
			break; // attack
		case 21:
	...
	public void setGenMobStat(MOB M, String code, String val) {
		...
		case 8:
			if (CMath.s_int(val) == Integer.MAX_VALUE) // alignment
				...
				M.addFaction(CMLib.factions().AlignID(),
						CMath.s_parseIntExpression(val));
			break;
		case 9: {
			if (CMath.isInteger(val) || (val.trim().length() == 0)) // disposition
				...
				for (Iterator<String> e = V.iterator(); e.hasNext();) {
					...
					int dispIndex = CMParms.indexOfIgnoreCase(
							PhyStats.IS_CODES, val);
					if (dispIndex >= 0)
						M.basePhyStats().setDisposition(
								M.basePhyStats().disposition()
										| (int) CMath.pow(2, dispIndex));
				}
			}
			break;
		...
		case 10: {
			if (CMath.isInteger(val) || (val.trim().length() == 0)) // senses
				...
				for (Iterator<String> e = V.iterator(); e.hasNext();) {
					val = e.next();
					int dispIndex = CMParms.indexOfIgnoreCase(
							PhyStats.CAN_SEE_CODES, val);
					if (dispIndex >= 0)
						M.basePhyStats().setSensesMask(
								M.basePhyStats().sensesMask()
										| (int) CMath.pow(2, dispIndex));
				}
			}
			break;
		...
		case 12:
			...
			break; // damage
		case 13:
			M.basePhyStats().setAttackAdjustment(
					CMath.s_parseIntExpression(val));
			break; // attack
		case 14:
		...
		case 21: {
			...
			for (int v = 0; v < V10.size(); v++) {
				...
				int y = s.lastIndexOf(")");
				if ((x > 0) && (y > x))
					M.addFaction(s.substring(0, x),
							CMath.s_int(s.substring(x + 1, y)));
			}
			break;
		}
		case 22:
	...
	public void setFactionFromXML(MOB mob, List<XMLpiece> xml) {
		if (xml != null) {
			List<XMLLibrary.XMLpiece> mV = CMLib.xml().getContentsFromPieces(
					xml, "FACTIONS");
			if (mV != null) {
				for (int m = 0; m < mV.size(); m++) {
					XMLLibrary.XMLpiece mblk = mV.get(m);
					mob.addFaction(CMLib.xml().getParmValue(mblk.parms, "ID"),
							Integer.valueOf(mblk.value).intValue());
				}
			}
		}
	}
