...
public class TimsLibrary extends StdLibrary implements ItemBalanceLibrary {
	...
	public int timsLevelCalculator(Item I, Ability ADJ, Ability RES,
			Ability CAST, int castMul) {
		int level = 0;
		...
		if (I instanceof Weapon) {
			...
			level = (int) Math.round(Math.floor((2.0 * curDamage
					/ (2.0 * (I.rawLogicalAnd() ? 2.0 : 1.0) + 1.0)
					+ (curAttack - weight) / 5.0 + range)
					* (range / weight + 2.0))) + 1;
		} else {
			...
			int[] leatherPoints = { 0, 0, 1, 5, 10, 16, 23, 31, 40, 49, 58, 67,
					76, 85, 94 };
			int[] clothPoints = { 0, 3, 7, 12, 18, 25, 33, 42, 52, 62, 72, 82,
					92, 102 };
			int[] metalPoints = { 0, 0, 0, 0, 1, 3, 5, 8, 12, 17, 23, 30, 38,
		...
		if (CAST != null) {
			...
			for (int v = 0; v < theSpells.size(); v++) {
				...
				if (ID.indexOf("HAVE") >= 0)
					level += (mul * CMLib.ableMapper().lowestQualifyingLevel(
							A.ID()));
				else
					level += (mul
		...
		if (ADJ != null) {
			...
			if (savedI instanceof Weapon)
				level += (arm * 2);
			else if (savedI instanceof Armor) {
				level += (att / 2);
				level += (dam * 3);
			}
			level += ab * 5;
			...
			if (sen != 0)
				level += 5;
			level += (int) Math.round(5.0 * CMParms.getParmDoublePlus(newText,
					"spe"));
			for (int i : CharStats.CODES.BASE()) {
				int stat = CMParms.getParmPlus(newText,
						CMStrings.limit(CharStats.CODES.NAME(i), 3)
								.toLowerCase());
				int max = CMParms.getParmPlus(newText, ("max" + (CMStrings
						.limit(CharStats.CODES.NAME(i), 3).toLowerCase())));
				level += (stat * 10);
				level += (max * 15);
			}
		...
		for (int i = 0; i < I.numEffects(); i++) {
			...
			if (A instanceof TriggeredAffect) {
				...
				if (CMath.bset(flags, Ability.FLAG_ADJUSTER)
						&& ((triggers & (TriggeredAffect.TRIGGER_WEAR_WIELD
								| TriggeredAffect.TRIGGER_GET | TriggeredAffect.TRIGGER_MOUNT)) > 0))
					RET[0] = A;
	...
	private void reportChangesDestroyOldI(Item oldI, Item newI,
			StringBuffer changes, int OTLVL, int TLVL) {
		...
		changes.append(newI.name() + ":" + newI.basePhyStats().level() + "("
				+ OTLVL + ")=>" + TLVL2 + "(" + TLVL + "), ");
		for (int i = 0; i < oldI.getStatCodes().length; i++)
			if ((!oldI.getStat(oldI.getStatCodes()[i]).equals(
					newI.getStat(newI.getStatCodes()[i]))))
				changes.append(oldI.getStatCodes()[i] + "("
						+ oldI.getStat(newI.getStatCodes()[i]) + "->"
						+ newI.getStat(newI.getStatCodes()[i]) + "), ");
		changes.append("\n\r");
	...
	public boolean itemFix(Item I, int lvlOr0, StringBuffer changes) {
		...
		if ((I instanceof SpellHolder)
			...
			for (Enumeration<Ability> e = spells.elements(); e
					.hasMoreElements();)
				levels += CMLib.ableMapper().lowestQualifyingLevel(
						e.nextElement().ID());
	...
	public boolean toneDownValue(Item I) {
		...
		if (I instanceof Weapon) {
			...
			return false;
		Map<String, String> H = timsItemAdjustments(I, I.phyStats().level(),
				I.material(), hands, weaponClass, I.maxRange(),
	...
	public void balanceItemByLevel(Item I) {
		...
		Map<String, String> H = timsItemAdjustments(I,
				I.basePhyStats().level(), I.material(), hands, weaponClass,
				I.maxRange(), I.rawProperLocationBitmap());
		if (I instanceof Weapon) {
	...
	public Map<String, String> timsItemAdjustments(Item I, int level,
			int material, int hands, int wclass, int reach, long worndata) {
		...
		if (I instanceof Weapon) {
			...
			if (wclass == Weapon.CLASS_FLAILED)
				baseattack = -5;
			if (wclass == Weapon.CLASS_POLEARM) {
				basereach = 1;
				basematerial = RawMaterial.MATERIAL_METAL;
			}
			if (wclass == Weapon.CLASS_RANGED) {
				basereach = 1;
				maxreach = 5;
			}
			if (wclass == Weapon.CLASS_THROWN) {
				basereach = 1;
				maxreach = 5;
			}
			if (wclass == Weapon.CLASS_EDGED) {
				baseattack = 10;
				basematerial = RawMaterial.MATERIAL_METAL;
			}
			if (wclass == Weapon.CLASS_DAGGER) {
				baseattack = 10;
				basematerial = RawMaterial.MATERIAL_METAL;
			}
			if (wclass == Weapon.CLASS_SWORD) {
				basematerial = RawMaterial.MATERIAL_METAL;
			}
			...
			int damage = (int) Math.round(((level - 1.0)
					/ (((double) reach / (double) weight) + 2.0)
					+ ((double) weight - (double) baseattack) / 5.0 - reach)
					* (((hands * 2.0) + 1.0) / 2.0));
			int cost = (int) Math
					.round(2.0
							* (((double) weight * (double) materialvalue) + ((2.0 * damage)
									+ baseattack + (reach * 10.0))
									* damage) / (hands + 1.0));
			...
			vals.put("VALUE", "" + cost);
			vals.put("WEIGHT",
					"" + (int) Math.round((armor) / wornweights * weightpts));
		}
		return vals;
	}
	...
	public void toneDownWeapon(Weapon W, Ability ADJ) {
		...
		if ((ADJ != null) && (ADJ.text().toUpperCase().indexOf("DAMAGE+") >= 0)) {
			...
			if (newNum != 0) {
				fixdam = false;
				ADJ.setMiscText(ADJ.text().substring(0, a + 7) + newNum
						+ ADJ.text().substring(a2));
			}
		}
		...
		if (fixdam && (W.basePhyStats().damage() >= 10))
			W.basePhyStats()
					.setDamage(
							(int) Math.round(CMath.mul(W.basePhyStats()
									.damage(), 0.9)));
		else if (fixatt && (W.basePhyStats().damage() > 1))
			W.basePhyStats().setDamage(W.basePhyStats().damage() - 1);
		if (fixatt && (W.basePhyStats().attackAdjustment() >= 10))
			W.basePhyStats().setAttackAdjustment(
					(int) Math.round(CMath.mul(W.basePhyStats()
							.attackAdjustment(), 0.9)));
		else if (fixatt && (W.basePhyStats().attackAdjustment() > 1))
			W.basePhyStats().setAttackAdjustment(
					W.basePhyStats().attackAdjustment() - 1);
		W.recoverPhyStats();
	}
	...
	public void toneDownArmor(Armor A, Ability ADJ) {
		...
		if ((ADJ != null) && (ADJ.text().toUpperCase().indexOf("ARMOR-") >= 0)) {
			...
			if (newNum != 0) {
				fixit = false;
				ADJ.setMiscText(ADJ.text().substring(0, a + 6) + newNum
						+ ADJ.text().substring(a2));
			}
		}
		if (fixit && (A.basePhyStats().armor() >= 10))
			A.basePhyStats().setArmor(
					(int) Math.round(CMath.mul(A.basePhyStats().armor(), 0.9)));
			...
			if (spaceafter > plusminus) {
				...
				if (CMath.isNumber(number)) {
					...
					if (spacebefore < plusminus) {
						...
						String wd = s.substring(spacebefore, plusminus).trim()
								.toUpperCase();
						...
						if (proceed) {
							if ((num != 1) && (num != -1)) {
								int newNum = (int) Math.round(CMath.mul(num,
										0.9));
								if ((newNum == num) && (newNum > 1))
									newNum--;
								if (newNum != 0)
									s = s.substring(0, plusminus + 1) + newNum
											+ s.substring(spaceafter);
							}
						}
					}
				}
	...
	public synchronized List<Ability> getCombatSpellSet() {
		List<Ability> spellSet = (List<Ability>) Resources
				.getResource("COMPLETE_SPELL_SET");
		if (spellSet == null) {
			...
			for (Enumeration<Ability> e = CMClass.abilities(); e
				...
				if (((A.classificationCode() & (Ability.ALL_ACODES)) == Ability.ACODE_SPELL)) {
					int lowLevel = CMLib.ableMapper().lowestQualifyingLevel(
							A.ID());
	...
	public Ability getCombatSpell(boolean malicious) {
		...
		while (((++tries) < 1000)) {
			...
			if (((malicious) && (A.canTarget(Ability.CAN_MOBS)) && (A
					.enchantQuality() == Ability.QUALITY_MALICIOUS)))
				return A;
			if ((!malicious) && (A.canAffect(Ability.CAN_MOBS))
	...
	public Item enchant(Item I, int pct) {
		...
		if (I instanceof Ammunition) {
			...
			case 1: {
				...
				if (A == null)
					return I;
				A.setMiscText("att" + ((bump < 0) ? "" : "+") + (bump * 5)
						+ " dam=" + ((bump < 0) ? "" : "+") + bump);
			...
			case 2: {
				...
				if (bump < 0)
					...
					I.basePhyStats().setLevel(
							I.basePhyStats().level() + levelsFromCaster(I, A));
				}
				if (A == null)
					return I;
				I.addNonUninvokableEffect(A);
