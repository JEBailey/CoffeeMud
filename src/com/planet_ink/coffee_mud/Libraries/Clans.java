...
public class Clans extends StdLibrary implements ClanManager {
	...
	public boolean isCommonClanRelations(Clan C1, Clan C2, int relation) {
		...
		if ((i1 == i2) && ((i1 == Clan.REL_WAR) || (i1 == Clan.REL_ALLY)))
			return i1 == relation;
	...
	public boolean isCommonClanRelations(String clanID1, String clanID2,
			int relation) {
		if ((clanID1 == null) || (clanID2 == null) || (clanID1.length() == 0)
				|| (clanID2.length() == 0))
			return Clan.REL_NEUTRAL == relation;
		return isCommonClanRelations(getClan(clanID1), getClan(clanID1),
				relation);
	}
	...
	public List<Pair<Clan, Integer>> findPrivilegedClans(MOB mob,
			Clan.Function func) {
	...
	public List<Triad<Clan, Integer, Integer>> findCommonRivalrousClans(
			MOB mob1, MOB mob2) {
		List<Triad<Clan, Integer, Integer>> list = new XVector<Triad<Clan, Integer, Integer>>(
				1, true);
		...
		for (Pair<Clan, Integer> c : mob1.clans()) {
			if (c.first.isRivalrous()) {
				...
				if (c2 != null)
					list.add(new Triad<Clan, Integer, Integer>(c.first,
							c.second, c2.second));
			}
		}
		return list;
	...
	public List<Pair<Clan, Integer>> findRivalrousClans(MOB mob) {
		List<Pair<Clan, Integer>> list = new XVector<Pair<Clan, Integer>>(1,
				true);
		for (Pair<Clan, Integer> c : mob.clans())
			if (c.first.isRivalrous())
				list.add(c);
	...
	public List<Pair<Clan, Integer>> findRivalrousClans(MOB clanSourceMob,
			MOB filterMob) {
		List<Pair<Clan, Integer>> list = new XVector<Pair<Clan, Integer>>(1,
				true);
		...
		for (Pair<Clan, Integer> c : clanSourceMob.clans()) {
			if (c.first.isRivalrous()) {
				Pair<Clan, Integer> c2 = filterMob
						.getClanRole(c.first.clanID());
				if (c2 == null)
					list.add(c);
			}
		}
	...
	public List<Pair<Clan, Clan>> findUncommonRivalrousClans(MOB M1, MOB M2) {
		...
		if ((M1 == null) || (M2 == null))
			return list;
		// i need the disunion here (what's the word for that?), as a
		// order-irrelevant set of pairs
		...
		List<Pair<Clan, Clan>> finalList = new XVector<Pair<Clan, Clan>>(1,
				true);
	...
	public int getClanRelations(Clan C1, Clan C2) {
		...
		for (Enumeration<Clan> e = clans(); e.hasMoreElements();) {
			...
			if ((C != C1)
					&& (C != C2)
					&& (((C1.getClanRelations(C.clanID()) == Clan.REL_ALLY) && (C
							.getClanRelations(C2.clanID()) == Clan.REL_WAR)))
					|| ((C2.getClanRelations(C.clanID()) == Clan.REL_ALLY) && (C
							.getClanRelations(C1.clanID()) == Clan.REL_WAR)))
				return Clan.REL_WAR;
		}
		return rel;
	}
	...
	public Clan getClan(String id) {
		...
		for (Enumeration<Clan> e = all.elements(); e.hasMoreElements();) {
			...
			if (CMLib.english().containsString(
					CMStrings.removeColors(C.name()), id))
				return C;
		}
		return null;
	}
	...
	public boolean isFamilyOfMembership(MOB M, List<MemberRecord> members) {
		...
		if (members.contains(M.Name()))
			return true;
		if ((M.getLiegeID().length() > 0) && (M.isMarriedToLiege())
	...
	public void addClan(Clan C) {
		synchronized (all) {
			if (!CMSecurity.isDisabled(CMSecurity.DisFlag.CLANTICKS))
				CMLib.threads().startTickDown(C, Tickable.TICKID_CLAN,
						(int) CMProps.getTicksPerDay());
			all.put(C.clanID().toUpperCase(), C);
			all2.add(new Pair<Clan, Integer>(C, Integer.valueOf(C
					.getGovernment().getAcceptPos())));
			CMLib.map().sendGlobalMessage(
					CMLib.map().deity(),
					CMMsg.TYP_CLANEVENT,
					CMClass.getMsg(CMLib.map().deity(), CMMsg.MSG_CLANEVENT,
							"+" + C.name()));
		}
	}
	...
	public void clanAnnounceAll(String msg) {
		...
		for (int i = 0; i < channels.size(); i++)
			CMLib.commands().postChannel(channels.get(i), clanRoles(), msg,
					true);
	}
	...
	public boolean goForward(MOB mob, Clan C,
			Vector<? extends Object> commands, Clan.Function function,
		...
		try {
			...
			if (mob.session()
					.confirm(
							"This matter must be voted upon.  Would you like to start the vote now (y/N)?",
							"N")) {
				...
				C.addVote(CV);
				C.updateVotes();
				final Clan.Function voteFunctionType = (function == Clan.Function.ASSIGN) ? Clan.Function.VOTE_ASSIGN
						: Clan.Function.VOTE_OTHER;
				...
				if (votingRoles.size() > 0) {
					final String firstRoleName = C.getRoleName(votingRoles
							.iterator().next().intValue(), true, true);
					final String rest = " " + firstRoleName
							+ " should use CLANVOTE to participate.";
					if (votingRoles.size() >= (C.getRolesList().length - 2)) {
						if (function == Clan.Function.ASSIGN)
							clanAnnounce(mob, "The " + C.getGovernmentName()
									+ " " + C.clanID()
									+ " has a new election to vote upon. "
									+ rest);
						else
							clanAnnounce(mob, "The " + C.getGovernmentName()
									+ " " + C.clanID()
									+ " has a new matter to vote upon. " + rest);
					} else if (votingRoles.size() == 1)
						clanAnnounce(mob, "The " + C.getGovernmentName() + " "
								+ C.clanID()
								+ " has a new matter to vote upon. " + rest);
					else {
						final String[] roleNames = new String[votingRoles
								.size()];
						for (int i = 0; i < votingRoles.size(); i++) {
							Integer roleID = votingRoles.get(i);
							roleNames[i] = C.getRoleName(roleID.intValue(),
									true, true);
						}
						String list = CMLib.english().toEnglishStringList(
								roleNames);
						clanAnnounce(mob, "The " + C.getGovernmentName() + " "
								+ C.clanID()
								+ " has a new matter to vote upon. " + list
								+ " should use CLANVOTE to participate.");
					}
				}
	...
	public String getGovernmentHelp(MOB mob, String named, boolean exact) {
		...
		if (helpG == null) {
			...
			for (ClanGovernment G : getStockGovernments())
				for (ClanPosition P : G.getPositions())
					if (P.getName().equalsIgnoreCase(named)
							|| P.getPluralName().equalsIgnoreCase(named)) {
						gtypes.add(G);
						name = P.getName();
					}
			if (gtypes.size() == 0) {
				...
				for (ClanGovernment G : getStockGovernments())
					for (ClanPosition P : G.getPositions())
						if (P.getName().toUpperCase()
								.startsWith(named.toUpperCase())
								|| P.getPluralName().toUpperCase()
										.startsWith(named.toUpperCase())) {
							gtypes.add(G);
							name = P.getName();
						}
			...
			for (int g = 0; g < gtypes.size(); g++)
				typeNames[g] = CMStrings.capitalizeAndLower(gtypes.get(g)
						.getName());
			return "The "
					+ name
					+ " is a rank or position within the following clan types: "
	...
	public ClanGovernment createSampleGovernment() {
		...
		for (int i = 0; i < pows1.length; i++)
			pows1[i] = Authority.CAN_NOT_DO;
		Authority[] pows2 = new Authority[Function.values().length];
		for (int i = 0; i < pows2.length; i++)
			pows2[i] = Authority.CAN_DO;
		ClanPosition P1 = (ClanPosition) CMClass
				.getCommon("DefaultClanPosition");
		P1.setID("APPLICANT");
		P1.setRoleID(0);
		P1.setRank(0);
		...
		P1.setPublic(false);
		ClanPosition P2 = (ClanPosition) CMClass
				.getCommon("DefaultClanPosition");
		P2.setID("MEMBER");
		P2.setRoleID(1);
		P2.setRank(1);
		...
		ClanGovernment[] gvts = (ClanGovernment[]) Resources
				.getResource("parsed_clangovernments");
		int id = 0;
		if (gvts != null) {
		...
		ClanGovernment G = (ClanGovernment) CMClass
				.getCommon("DefaultClanGovernment");
		G.setID(id);
	...
	public ClanGovernment createGovernment(String name) {
		...
		newG.setName(name);
		govts.add(newG);
		Resources.submitResource("parsed_clangovernments",
				govts.toArray(new ClanGovernment[0]));
		return newG;
	}
	...
	public ClanGovernment[] getStockGovernments() {
		...
		if (gvts == null) {
			synchronized (this) {
				gvts = (ClanGovernment[]) Resources
						.getResource("parsed_clangovernments");
				if (gvts == null) {
					StringBuffer str = Resources.getFileResource(
							"clangovernments.xml", true);
					if (str == null)
	...
	public ClanGovernment getStockGovernment(int typeid) {
		...
		if (gvts.length <= typeid) {
			Log.errOut("Clans",
					"Someone mistakenly requested stock government typeid "
							+ typeid);
			return gvts[0];
		}
		return gvts[typeid];
	...
	public String makeGovernmentXML(ClanGovernment gvt) {
		...
		str.append("<CLANTYPE ").append("TYPEID=" + gvt.getID() + " ")
				.append("NAME=\"" + gvt.getName() + "\" ")
				.append("CATEGORY=\"" + gvt.getCategory() + "\"").append(">\n");
		if (gvt.isDefault())
			str.append(indt(1)).append("<ISDEFAULT>true</ISDEFAULT>\n");
		str.append(indt(1)).append("<SHORTDESC>")
				.append(CMLib.xml().parseOutAngleBrackets(gvt.getShortDesc()))
				.append("</SHORTDESC>\n");
		str.append(indt(1)).append("<LONGDESC>")
				.append(CMLib.xml().parseOutAngleBrackets(gvt.getLongDesc()))
				.append("</LONGDESC>\n");
		...
		for (int p = gvt.getPositions().length - 1; p >= 0; p--) {
			...
			str.append(indt(2))
					.append("<POSITION ")
					.append("ID=\"" + pos.getID() + "\" ")
					.append("ROLEID=" + pos.getRoleID() + " ")
					.append("RANK=" + pos.getRank() + " ")
					.append("NAME=\"" + pos.getName() + "\" ")
					.append("PLURAL=\"" + pos.getPluralName() + "\" ")
					.append("MAX=" + pos.getMax() + " ")
					.append("INNERMASK=\""
			...
			for (Clan.Function func : Clan.Function.values())
				if (pos.getFunctionChart()[func.ordinal()] == Clan.Authority.CAN_DO)
					str.append(indt(3)).append("<POWER>")
							.append(func.toString()).append("</POWER>\n");
		...
		if (voteSet.size() == 0)
			...
			str.append(indt(1))
					.append("<VOTING ")
					.append("MAXDAYS=" + gvt.getMaxVoteDays() + " QUORUMPCT="
							+ gvt.getVoteQuorumPct() + ">\n");
			for (Clan.Function func : voteSet)
				str.append(indt(2)).append("<POWER>").append(func.toString())
						.append("</POWER>\n");
			str.append(indt(1)).append("</VOTING>\n");
		}
		str.append(indt(1)).append("<AUTOPOSITION>")
				.append(gvt.getPositions()[gvt.getAutoRole()].getID())
				.append("</AUTOPOSITION>\n");
		str.append(indt(1)).append("<ACCEPTPOSITION>")
				.append(gvt.getPositions()[gvt.getAcceptPos()].getID())
				.append("</ACCEPTPOSITION>\n");
		str.append(indt(1))
				.append("<REQUIREDMASK>")
				.append(CMLib.xml().parseOutAngleBrackets(
						gvt.getRequiredMaskStr())).append("</REQUIREDMASK>\n");
		str.append(indt(1))
				.append("<ENTRYSCRIPT>")
				.append(CMLib.xml().parseOutAngleBrackets(gvt.getEntryScript()))
				.append("</ENTRYSCRIPT>\n");
		str.append(indt(1)).append("<EXITSCRIPT>")
				.append(CMLib.xml().parseOutAngleBrackets(gvt.getExitScript()))
				.append("</EXITSCRIPT>\n");
		str.append(indt(1)).append("<AUTOPROMOTEBY>")
				.append(gvt.getAutoPromoteBy().toString())
				.append("</AUTOPROMOTEBY>\n");
		str.append(indt(1)).append("<PUBLIC>").append(gvt.isPublic())
				.append("</PUBLIC>\n");
		str.append(indt(1)).append("<FAMILYONLY>").append(gvt.isFamilyOnly())
				.append("</FAMILYONLY>\n");
		str.append(indt(1)).append("<RIVALROUS>").append(gvt.isRivalrous())
				.append("</RIVALROUS>\n");
		str.append(indt(1)).append("<XPPERLEVELFORMULA>")
				.append(gvt.getXpCalculationFormulaStr())
				.append("</XPPERLEVELFORMULA>\n");
		if (gvt.getOverrideMinMembers() == null)
			str.append(indt(1)).append("<OVERRIDEMINMEMBERS />\n");
		else
			str.append(indt(1)).append("<OVERRIDEMINMEMBERS>")
					.append(gvt.getOverrideMinMembers().toString())
					.append("</OVERRIDEMINMEMBERS>\n");
		str.append(indt(1)).append("<CONQUEST>\n");
		{
			str.append(indt(2)).append("<ENABLED>")
					.append(gvt.isConquestEnabled()).append("</ENABLED>\n");
			str.append(indt(2)).append("<ITEMLOYALTY>")
					.append(gvt.isConquestItemLoyalty())
					.append("</ITEMLOYALTY>\n");
			str.append(indt(2)).append("<DEITYBASIS>")
					.append(gvt.isConquestByWorship())
					.append("</DEITYBASIS>\n");
		}
		str.append(indt(1)).append("</CONQUEST>\n");
		gvt.getClanLevelAbilities(Integer.valueOf(Integer.MAX_VALUE));
		final Enumeration<AbilityMapping> m = CMLib.ableMapper().getClassAbles(
				gvt.getName(), false);
		if (!m.hasMoreElements())
			...
			for (; m.hasMoreElements();) {
				...
				str.append(indt(2)).append(
						"<ABILITY ID=\"" + map.abilityID + "\" PROFF="
								+ map.defaultProficiency + " LEVEL="
								+ map.qualLevel + " QUALIFYONLY="
								+ (!map.autoGain) + " />\n");
			}
			str.append(indt(1)).append("</ABILITIES>\n");
		}
		final List<Ability> effectList = gvt.getClanLevelEffects(null, null,
	...
	public ClanGovernment[] parseGovernmentXML(StringBuffer xml) {
		...
		XMLLibrary.XMLpiece clanTypesTag = CMLib.xml().getPieceFromPieces(xmlV,
				"CLANTYPES");
		List<XMLLibrary.XMLpiece> clanTypes = null;
		if (clanTypesTag != null)
			clanTypes = clanTypesTag.contents;
		else {
			XMLLibrary.XMLpiece clanType = CMLib.xml().getPieceFromPieces(xmlV,
					"CLANTYPE");
			if (clanType != null) {
		...
		for (XMLLibrary.XMLpiece clanTypePieceTag : clanTypes) {
			...
			final int typeID = CMath
					.s_int(clanTypePieceTag.parms.get("TYPEID"));
			final boolean isDefault = CMath.s_bool(clanTypePieceTag.parms
					.get("ISDEFAULT"));
			...
			XMLLibrary.XMLpiece votingTag = CMLib.xml().getPieceFromPieces(
					clanTypePieceTag.contents, "VOTING");
			...
			for (XMLLibrary.XMLpiece piece : votingTag.contents) {
				if (piece.tag.equalsIgnoreCase("POWER")) {
					Function power = (Function) CMath.s_valueOf(
							Clan.Function.values(), piece.value);
					if (power == null)
						Log.errOut("Clans", "Illegal power found in xml: "
								+ piece.value);
					else
			...
			XMLLibrary.XMLpiece positionsTag = CMLib.xml().getPieceFromPieces(
					clanTypePieceTag.contents, "POSITIONS");
			for (XMLLibrary.XMLpiece posPiece : positionsTag.contents) {
				if (posPiece.tag.equalsIgnoreCase("POSITION")) {
					...
					final int roleID = CMath
							.s_int(posPiece.parms.get("ROLEID"));
					...
					final boolean isPublic = CMath.s_bool(posPiece.parms
							.get("PUBLIC"));
					final String innerMaskStr = CMLib.xml()
							.restoreAngleBrackets(
									posPiece.parms.get("INNERMASK"));
					for (XMLLibrary.XMLpiece powerPiece : posPiece.contents) {
						if (powerPiece.tag.equalsIgnoreCase("POWER")) {
							Function power = (Function) CMath.s_valueOf(
									Clan.Function.values(), powerPiece.value);
							if (power == null)
								Log.errOut("Clans",
										"Illegal power found in xml: "
												+ powerPiece.value);
							else
								functionChart[power.ordinal()] = Authority.CAN_DO;
						}
					}
					ClanPosition P = (ClanPosition) CMClass
							.getCommon("DefaultClanPosition");
					P.setID(ID);
					P.setRoleID(roleID);
					P.setRank(rank);
			...
			for (ClanPosition pos : positions)
				if ((pos.getRoleID() >= 0)
						&& (pos.getRoleID() < positions.size()))
					if (posArray[pos.getRoleID()] != null) {
						Log.errOut("Clans", "Bad ROLEID " + pos.getRoleID()
								+ " in positions list in " + typeName);
			...
			String autoRoleStr = CMLib.xml().getValFromPieces(
					clanTypePieceTag.contents, "AUTOPOSITION");
			...
			String acceptRoleStr = CMLib.xml().getValFromPieces(
					clanTypePieceTag.contents, "ACCEPTPOSITION");
			ClanPosition acceptRole = null;
			for (ClanPosition pos : positions)
				if (pos.getID().equalsIgnoreCase(acceptRoleStr))
					acceptRole = pos;
			if (acceptRole == null) {
				Log.errOut("Clans", "Illegal acceptRole found in xml: "
						+ acceptRoleStr);
				continue;
			}
			String requiredMaskStr = CMLib.xml().restoreAngleBrackets(
					CMLib.xml().getValFromPieces(clanTypePieceTag.contents,
							"REQUIREDMASK"));
			String entryScript = CMLib.xml().restoreAngleBrackets(
					CMLib.xml().getValFromPieces(clanTypePieceTag.contents,
							"ENTRYSCRIPT"));
			String exitScript = CMLib.xml().restoreAngleBrackets(
					CMLib.xml().getValFromPieces(clanTypePieceTag.contents,
							"EXITSCRIPT"));
			String shortDesc = CMLib.xml().restoreAngleBrackets(
					CMLib.xml().getValFromPieces(clanTypePieceTag.contents,
							"SHORTDESC"));
			String longDesc = CMLib.xml().restoreAngleBrackets(
					CMLib.xml().getValFromPieces(clanTypePieceTag.contents,
							"LONGDESC"));
			String autoPromoteStr = CMLib.xml().getValFromPieces(
					clanTypePieceTag.contents, "AUTOPROMOTEBY");
			Clan.AutoPromoteFlag autoPromote = AutoPromoteFlag
					.valueOf(autoPromoteStr);
			if (autoPromote == null) {
				Log.errOut("Clans", "Illegal AUTOPROMOTEBY found in xml: "
						+ autoPromoteStr);
				continue;
			}
			boolean isPublic = CMath.s_bool(CMLib.xml().getValFromPieces(
					clanTypePieceTag.contents, "PUBLIC"));
			boolean isFamilyOnly = CMath.s_bool(CMLib.xml().getValFromPieces(
					clanTypePieceTag.contents, "FAMILYONLY"));
			String overrideMinMembersStr = CMLib.xml().getValFromPieces(
					clanTypePieceTag.contents, "OVERRIDEMINMEMBERS");
			Integer overrideMinMembers = null;
			if ((overrideMinMembersStr != null)
					&& CMath.isInteger(overrideMinMembersStr))
				overrideMinMembers = Integer.valueOf(CMath
						.s_int(overrideMinMembersStr));
			boolean isRivalrous = true;
			String rivalrousStr = CMLib.xml().getValFromPieces(
					clanTypePieceTag.contents, "RIVALROUS");
			if (CMath.isBool(rivalrousStr))
				isRivalrous = CMath.s_bool(rivalrousStr);
			String xpPerLevelFormulaStr = CMLib.xml().getValFromPieces(
					clanTypePieceTag.contents, "XPPERLEVELFORMULA");
			XMLLibrary.XMLpiece conquestTag = CMLib.xml().getPieceFromPieces(
					clanTypePieceTag.contents, "CONQUEST");
			boolean conquestEnabled = true;
			boolean conquestItemLoyalty = true;
			boolean conquestDeityBasis = false;
			if (conquestTag != null) {
				conquestEnabled = CMath.s_bool(CMLib.xml().getValFromPieces(
						conquestTag.contents, "ENABLED"));
				conquestItemLoyalty = CMath.s_bool(CMLib.xml()
						.getValFromPieces(conquestTag.contents, "ITEMLOYALTY"));
				conquestDeityBasis = CMath.s_bool(CMLib.xml().getValFromPieces(
						conquestTag.contents, "DEITYBASIS"));
			}
			ClanGovernment G = (ClanGovernment) CMClass
					.getCommon("DefaultClanGovernment");
			G.setID(typeID);
			...
			XMLLibrary.XMLpiece abilitiesTag = CMLib.xml().getPieceFromPieces(
					clanTypePieceTag.contents, "ABILITIES");
			if ((abilitiesTag != null) && (abilitiesTag.contents != null)
					&& (abilitiesTag.contents.size() > 0)) {
				G.setStat("NUMRABLE",
						Integer.toString(abilitiesTag.contents.size()));
			...
			XMLLibrary.XMLpiece effectsTag = CMLib.xml().getPieceFromPieces(
					clanTypePieceTag.contents, "EFFECTS");
			if ((effectsTag != null) && (effectsTag.contents != null)
					&& (effectsTag.contents.size() > 0)) {
				G.setStat("NUMREFF",
						Integer.toString(effectsTag.contents.size()));
				for (int x = 0; x < effectsTag.contents.size(); x++) {
					...
					G.setStat("GETREFFPARM" + x, CMLib.xml()
							.restoreAngleBrackets(able.parms.get("PARMS")));
		...
		for (ClanGovernment govt : governments)
			if ((govt.getID() < 0) || (govt.getID() >= governments.size())
					|| (govts[govt.getID()] != null)) {
		...
		if (trophySystemActive()) {
			...
			if (CMProps.getVar(CMProps.Str.CLANTROPMB).length() > 0) {
				...
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CLANS))
					Log.debugOut("Clans", "MBTrophy: "
							+ ((winnerC == null) ? "Noone" : winnerC.clanID())
							+ " won with " + winnerMembers);
				if ((winnerC != null)
						&& (!CMath.bset(winnerC.getTrophies(),
								Trophy.Members.flagNum()))
						&& (winnerC.getExp() > 0)) {
					winnerC.setTrophies(winnerC.getTrophies()
							| Trophy.Members.flagNum());
					clanAnnounceAll("The " + winnerC.getGovernmentName() + " "
							+ winnerC.name()
							+ " has been awarded the trophy for "
							+ Trophy.Members.description + ".");
				}
				for (Enumeration<Clan> e = clans(); e.hasMoreElements();) {
					Clan C = e.nextElement();
					if ((winnerC != C)
							&& (CMath.bset(C.getTrophies(),
									Trophy.Members.flagNum()))) {
						C.setTrophies(C.getTrophies()
								- Trophy.Members.flagNum());
						C.clanAnnounce("The " + C.getGovernmentName() + " "
								+ C.name()
								+ " has lost control of the trophy for "
								+ Trophy.Members.description + ".");
					}
				}
			}
			...
			if (CMProps.getVar(CMProps.Str.CLANTROPLVL).length() > 0) {
				...
				int winnerLevel = (winnerC == null) ? 0
						: filterMedianLevel(winnerC.getFullMemberList());
				for (Enumeration<Clan> e = clans(); e.hasMoreElements();) {
					Clan C = e.nextElement();
					if (C == winnerC)
						continue;
				...
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CLANS))
					Log.debugOut("DefaultClan", "LVLTrophy: "
							+ ((winnerC == null) ? "Noone" : winnerC.clanID())
							+ " won with " + winnerLevel);
				if ((winnerC != null)
						&& (!CMath.bset(winnerC.getTrophies(),
								Trophy.MemberLevel.flagNum()))
						&& (winnerC.getExp() > 0)) {
					winnerC.setTrophies(winnerC.getTrophies()
							| Trophy.MemberLevel.flagNum());
					clanAnnounceAll("The " + winnerC.getGovernmentName() + " "
							+ winnerC.name()
							+ " has been awarded the trophy for "
							+ Trophy.MemberLevel.description + ".");
				}
				for (Enumeration<Clan> e = clans(); e.hasMoreElements();) {
					Clan C = e.nextElement();
					if ((winnerC != C)
			...
			if (CMProps.getVar(CMProps.Str.CLANTROPEXP).length() > 0) {
				...
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CLANS))
					Log.debugOut("DefaultClan", "EXPTrophy: "
							+ ((winnerC == null) ? "Noone" : winnerC.clanID())
							+ " won with "
							+ ((winnerC == null) ? "0" : "" + winnerC.getExp()));
				if ((winnerC != null)
						&& (!CMath.bset(winnerC.getTrophies(),
								Trophy.Experience.flagNum()))
						&& (winnerC.getExp() > 0)) {
					winnerC.setTrophies(winnerC.getTrophies()
							| Trophy.Experience.flagNum());
					clanAnnounceAll("The " + winnerC.getGovernmentName() + " "
			...
			if (CMProps.getVar(CMProps.Str.CLANTROPPK).length() > 0) {
				...
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CLANS))
					Log.debugOut(
							"DefaultClan",
							"PKTrophy: "
									+ ((winnerC == null) ? "Noone" : winnerC
											.clanID())
									+ " won with "
									+ ((winnerC == null) ? "0" : ""
											+ winnerC.getCurrentClanKills()));
				if ((winnerC != null)
						&& (!CMath.bset(winnerC.getTrophies(),
								Trophy.PlayerKills.flagNum()))
					...
					clanAnnounceAll("The " + winnerC.getGovernmentName() + " "
							+ winnerC.name()
							+ " has been awarded the trophy for "
							+ Trophy.PlayerKills.description + ".");
				}
				for (Enumeration<Clan> e = clans(); e.hasMoreElements();) {
					Clan C = e.nextElement();
					if ((winnerC != C)
			...
			if ((CMProps.getVar(CMProps.Str.CLANTROPAREA).length() > 0)
				...
				long mostClansControlled = (winnerMostClansControlledC == null) ? -1
						: winnerMostClansControlledC.getControlledAreas()
								.size();
				Clan winnerMostControlPointsC = getTrophyWinner(Trophy.Points);
				long mostControlPoints = (winnerMostControlPointsC == null) ? -1
						: winnerMostControlPointsC.calculateMapPoints();
				for (Enumeration<Clan> e = clans(); e.hasMoreElements();) {
					Clan C = e.nextElement();
					if ((C != winnerMostClansControlledC)
							&& (CMProps.getVar(CMProps.Str.CLANTROPAREA)
									.length() > 0)) {
				...
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CLANS))
					Log.debugOut("DefaultClan", "AREATrophy: "
							+ ((winnerMostClansControlledC == null) ? "Noone"
									: winnerMostClansControlledC.clanID())
							+ " won with " + mostClansControlled);
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CLANS))
					Log.debugOut("DefaultClan", "CPTrophy: "
							+ ((winnerMostControlPointsC == null) ? "Noone"
									: winnerMostControlPointsC.clanID())
							+ " won with " + mostControlPoints);
				if ((winnerMostClansControlledC != null)
						&& (CMProps.getVar(CMProps.Str.CLANTROPAREA).length() > 0)
						&& (mostClansControlled > 0)) {
					if (!CMath.bset(winnerMostClansControlledC.getTrophies(),
							Trophy.Areas.flagNum())) {
						winnerMostClansControlledC
								.setTrophies(winnerMostClansControlledC
										.getTrophies() | Trophy.Areas.flagNum());
						clanAnnounceAll("The "
								+ winnerMostClansControlledC
										.getGovernmentName() + " "
								+ winnerMostClansControlledC.name()
								+ " has been awarded the trophy for "
								+ Trophy.Areas.description + ".");
					}
				}
				for (Enumeration<Clan> e = clans(); e.hasMoreElements();) {
					Clan C = e.nextElement();
				...
				if ((winnerMostControlPointsC != null)
						&& (CMProps.getVar(CMProps.Str.CLANTROPCP).length() > 0)
						&& (mostControlPoints > 0)) {
					if (!CMath.bset(winnerMostControlPointsC.getTrophies(),
							Trophy.Points.flagNum())) {
						winnerMostControlPointsC
								.setTrophies(winnerMostControlPointsC
										.getTrophies()
										| Trophy.Points.flagNum());
						clanAnnounceAll("The "
								+ winnerMostControlPointsC.getGovernmentName()
								+ " " + winnerMostControlPointsC.name()
								+ " has been awarded the trophy for "
								+ Trophy.Points.description + ".");
					}
				}
				for (Enumeration<Clan> e = clans(); e.hasMoreElements();) {
					Clan C = e.nextElement();
	...
	public boolean activate() {
		if (serviceClient == null) {
			...
			serviceClient = CMLib.threads().startTickDown(this,
					...
					CMProps.getTickMillis() * CMProps.getTicksPerDay(), 1);
		}
		return true;
	}
