...
public class CoffeeUtensils extends StdLibrary implements CMMiscUtils {
	...
	public String getFormattedDate(Environmental E) {
		...
		if (E != null) {
			TimeClock C = (E instanceof Area) ? ((Area) E).getTimeObj()
					: ((CMLib.map().roomLocation(E) != null) ? CMLib.map()
							.roomLocation(E).getArea().getTimeObj() : null);
			if (C != null)
				date = CMStrings.padRight(
						C.getDayOfMonth() + "-" + C.getMonth() + "-"
								+ C.getYear(), 11);
		}
		return date;
	}
	...
	public Trap fetchMyTrap(Physical myThang) {
		if (myThang == null)
			return null;
		for (final Enumeration<Ability> a = myThang.effects(); a
				.hasMoreElements();) {
	...
	public boolean reachableItem(MOB mob, Environmental E) {
		...
		if ((mob.isMine(I))
				|| ((mob.riding() != null) && ((I == mob.riding())
						|| (I.owner() == R) || (I.ultimateContainer(R) == R)))
				|| (I.owner() == null)
				|| ((I.owner() instanceof Room) && (!((Room) I.owner())
						.isContent(I))))
			return true;
		return false;
	}
	...
	public void extinguish(MOB source, Physical target, boolean mundane) {
		...
		for (int a = target.numEffects() - 1; a >= 0; a--) // personal effects
		{
			...
			if ((A != null)
					&& ((!mundane) || ((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_PROPERTY))) {
				if ((CMath.bset(A.flags(), Ability.FLAG_HEATING) && (!mundane))
						|| (CMath.bset(A.flags(), Ability.FLAG_FIREBASED))
						|| ((A.ID().equalsIgnoreCase("Spell_SummonElemental") && A
								.text().toUpperCase().indexOf("FIRE") >= 0)))
					A.unInvoke();
			}
		}
	...
	public void roomAffectFully(CMMsg msg, Room room, int dirCode) {
		...
		if (pair != null) {
			...
			if ((msg.targetMinor() == CMMsg.TYP_OPEN) && (pair.isLocked())) {
				altMsg = CMClass.getMsg(msg.source(), pair, msg.tool(),
						CMMsg.MSG_UNLOCK, null, CMMsg.MSG_UNLOCK, null,
						CMMsg.MSG_UNLOCK, null);
	...
	public int disenchantItem(Item target) {
		...
		if (target instanceof Wand) {
			...
			if (A != null)
				level = level
						- CMLib.ableMapper().lowestQualifyingLevel(A.ID()) + 2;
		...
		for (final Enumeration<Ability> a = target.effects(); a
				.hasMoreElements();) {
			final Ability A = a.nextElement();
			if (A != null)
				affects.add(A);
			...
			if (R != null) {
				...
				if (newLastTickedDateTime == 0) {
					...
					for (int i = 0; i < rivals.size(); i++) {
						...
						if ((V.size() == 1) || (V.get(0).rawWornCode() == 0)) {
							for (int r = 0; r < V.size(); r++) {
								Item I = V.get(r);
								if (CMLib.dice().rollPercentage() < I
										.basePhyStats().rejuv())
									mob.delItem(I);
								else {
									I.basePhyStats()
							...
							int chosenChance = CMLib.dice().roll(1,
									totalChance, 0);
							totalChance = 0;
							Item chosenI = null;
							for (int r = 0; r < V.size(); r++) {
								Item I = V.get(r);
								if (chosenChance <= (totalChance + I
										.basePhyStats().rejuv())) {
									chosenI = I;
									break;
								}
					...
					if (mob instanceof ShopKeeper) {
						...
						for (Iterator<Environmental> i = shop
	...
	public void recursiveDropMOB(MOB mob, Room room, Item thisContainer,
			boolean bodyFlag) {
		// caller is responsible for recovering any env
		// stat changes!

		if (CMLib.flags().isHidden(thisContainer))
			thisContainer.basePhyStats().setDisposition(
					thisContainer.basePhyStats().disposition()
		...
		do {
			...
			for (int i = 0; i < mob.numItems(); i++) {
				...
				if ((thisItem != null)
						&& (thisItem.container() == thisContainer)) {
	...
	public boolean armorCheck(MOB mob, Item I, int allowedArmorLevel) {
		if ((((I instanceof Armor) || (I instanceof Shield)))
			...
			case RawMaterial.MATERIAL_CLOTH:
				if ((allowedArmorLevel == CharClass.ARMOR_METALONLY)
						|| (allowedArmorLevel == CharClass.ARMOR_OREONLY)
						|| ((allowedArmorLevel == CharClass.ARMOR_VEGAN) && ((I
								.material() == RawMaterial.RESOURCE_HIDE)
								|| (I.material() == RawMaterial.RESOURCE_FUR)
								|| (I.material() == RawMaterial.RESOURCE_FEATHERS) || (I
								.material() == RawMaterial.RESOURCE_WOOL))))
					ok = false;
				break;
	...
	protected TriadVector<Integer, Integer, MaskingLibrary.CompiledZapperMask> parseLootPolicyFor(
			MOB mob) {
		...
		if ((lootPolicy == null)
			...
			List<String> lootPolicies = (!mob.isMonster()) ? new Vector<String>()
					: CMParms.parseCommas(
							CMProps.getVar(CMProps.Str.ITEMLOOTPOLICY), true);
			...
			for (int p = 0; p < lootPolicies.size(); p++) {
				...
				if (maskDex >= 0) {
					...
					compiledMask = CMLib.masking().maskCompile(
							lootPolicies.get(p).substring(maskDex + 5).trim());
				} else
				...
				for (int x = 0; x < parsed.size(); x++)
					if (CMath.isInteger(parsed.elementAt(x)))
						...
						pct = (int) Math
								.round(CMath.s_pct(parsed.elementAt(x)) * 100.0);
				int flags = 0;
				...
				policies.addElement(Integer.valueOf(pct),
						Integer.valueOf(flags), compiledMask);
			}
			lootPolicy = policies;
			lastLootPolicyHash = lootPolicyStr.hashCode();
		}
	...
	public void confirmWearability(MOB mob) {
		...
		for (int i = 0; i < mob.numItems(); i++) {
			...
			if ((item != null) && (!item.amWearingAt(Wearable.IN_INVENTORY))) {
				...
				if (reWearSet.size() == 0)
					...
					short layer = (item instanceof Armor) ? ((Armor) item)
							.getClothingLayer() : 0;
					int d = 0;
					for (; d < reWearSet.size(); d++)
						if (reWearSet.elementAt(d, 1) instanceof Armor) {
							if (((Armor) reWearSet.elementAt(d, 1))
									.getClothingLayer() > layer)
								break;
						} else if (0 > layer)
							break;
					if (d >= reWearSet.size())
		...
		for (int r = 0; r < reWearSet.size(); r++) {
			...
			CMMsg msg = CMClass.getMsg(mob, item, null, CMMsg.NO_EFFECT, null,
					msgCode, null, CMMsg.NO_EFFECT, null);
	...
	public Item isRuinedLoot(MOB mob, Item I) {
		...
		if ((CMath.bset(I.phyStats().disposition(), PhyStats.IS_UNSAVABLE))
				|| (CMath.bset(I.phyStats().sensesMask(),
						PhyStats.SENSE_ITEMNORUIN)) || (I instanceof Coins))
			return I;
		...
		for (int d = 0; d < policies.size(); d++) {
			if ((policies.get(d).third.entries.length > 0)
					&& (!CMLib.masking().maskCheck(policies.get(d).third, I,
							true)))
				continue;
			if (CMLib.dice().rollPercentage() > policies.get(d).first
					.intValue())
				continue;
			int flags = policies.get(d).second.intValue();
			if (CMath.bset(flags, CMMiscUtils.LOOTFLAG_WORN)
					&& I.amWearingAt(Wearable.IN_INVENTORY))
				continue;
			...
			case RawMaterial.MATERIAL_PAPER:
				ruinDescAdder = CMStrings.capitalizeFirstLetter(I2.name())
						+ " is torn and ruined beyond repair.";
				break;
			case RawMaterial.MATERIAL_METAL:
			case RawMaterial.MATERIAL_MITHRIL:
			case RawMaterial.MATERIAL_WOODEN:
				ruinDescAdder = CMStrings.capitalizeFirstLetter(I2.name())
						+ " is battered and ruined beyond repair.";
				break;
			case RawMaterial.MATERIAL_GLASS:
				ruinDescAdder = CMStrings.capitalizeFirstLetter(I2.name())
			...
			case RawMaterial.MATERIAL_SYNTHETIC:
				ruinDescAdder = CMStrings.capitalizeFirstLetter(I2.name())
						+ " is cracked and ruined beyond repair.";
				break;
			...
			I2.setDescription(CMStrings.endWithAPeriod(I2.description()) + " "
					+ ruinDescAdder);
			String oldName = I2.Name();
			I2.setName(CMLib.english().insertUnColoredAdjective(I2.Name(),
					"ruined"));
			int x = I2.displayText().toUpperCase()
					.indexOf(oldName.toUpperCase());
			I2.setBaseValue(0);
			if (x >= 0)
				I2.setDisplayText(I2.displayText().substring(0, x) + I2.Name()
				...
				for (int c = 0; c < M.baseCharStats().numClasses(); c++)
					if (M.baseCharStats().getMyClass(c) == oldC) {
						M.baseCharStats().setMyClasses(
								M.baseCharStats().getMyClassesStr());
						break;
					}
				for (int c = 0; c < M.charStats().numClasses(); c++)
					if (M.charStats().getMyClass(c) == oldC) {
						M.charStats().setMyClasses(
								M.charStats().getMyClassesStr());
						break;
					}
			}
			for (Enumeration<MOB> e2 = CMLib.players().players(); e2
					.hasMoreElements();) {
	...
	public boolean resurrect(MOB tellMob, Room corpseRoom, DeadBody body,
			int XPLevel) {
		MOB rejuvedMOB = CMLib.players().getPlayer(body.mobName());

		if (rejuvedMOB != null) // doing this here is helpful -- it can trigger
								// a socket error.
			rejuvedMOB.tell("You are being resurrected.");

		if ((rejuvedMOB != null) && (rejuvedMOB.session() != null)
				&& (!rejuvedMOB.session().isStopped())) {
			if (rejuvedMOB.location() != corpseRoom) {
				rejuvedMOB.location().showOthers(rejuvedMOB, null,
						CMMsg.MSG_OK_VISUAL, "<S-NAME> disappears!");
			...
			while (it < rejuvedMOB.location().numItems()) {
				...
				if ((item != null) && (item.container() == body)) {
					CMMsg msg2 = CMClass.getMsg(rejuvedMOB, body, item,
							CMMsg.MSG_GET, null);
					rejuvedMOB.location().send(rejuvedMOB, msg2);
					CMMsg msg3 = CMClass.getMsg(rejuvedMOB, item, null,
							CMMsg.MSG_GET, null);
			...
			body.delEffect(body.fetchEffect("Age")); // so misskids doesn't
														// record it
			body.destroy();
			rejuvedMOB.basePhyStats().setDisposition(
					CMath.unsetb(rejuvedMOB.basePhyStats().disposition(),
							PhyStats.IS_SITTING));
			rejuvedMOB.phyStats().setDisposition(
					CMath.unsetb(rejuvedMOB.basePhyStats().disposition(),
							PhyStats.IS_SITTING));
			rejuvedMOB.location().show(rejuvedMOB, null,
					CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> get(s) up!");
			corpseRoom.recoverRoomStats();
			Vector<String> whatsToDo = CMParms.parse(CMProps
					.getVar(CMProps.Str.PLAYERDEATH));
			for (int w = 0; w < whatsToDo.size(); w++) {
				...
				if (whatToDo.startsWith("UNL"))
				...
				} else if ((whatToDo.trim().equals("0"))
						|| (CMath.s_int(whatToDo) > 0)) {
					if (XPLevel >= 0) {
						int expLost = (CMath.s_int(whatToDo) + (2 * XPLevel)) / 2;
						rejuvedMOB.tell("^*You regain " + expLost
								+ " experience points.^?^.");
						CMLib.leveler().postExperience(rejuvedMOB, null, null,
								expLost, false);
					}
					...
					for (int l = body.phyStats().level(); l < rejuvedMOB
							.phyStats().level(); l++)
						lvl = lvl / 2.0;
					int expRestored = (int) Math
							.round(((100.0 + (2.0 * (XPLevel))) * lvl) / 2.0);
					rejuvedMOB.tell("^*You regain " + expRestored
							+ " experience points.^?^.");
					CMLib.leveler().postExperience(rejuvedMOB, null, null,
							expRestored, false);
				}
			...
			corpseRoom
					.show(tellMob, body, CMMsg.MSG_OK_VISUAL,
							"<T-NAME> twitch(es) for a moment, but the spirit is too far gone.");
		return false;
	}

	public long[][] compileConditionalRange(List<String> condV, int numDigits,
			final int startOfRange, final int endOfRange) {
		...
		for (String cond : condV) {
			...
			if ((start >= startOfRange) && (finish <= endOfRange)
					&& (start <= finish)) {
	...
	public String builtPrompt(MOB mob) {
		...
		while (c < prompt.length())
			if ((prompt.charAt(c) == '%') && (c < (prompt.length() - 1))) {
				...
				case '-':
					if (c < (prompt.length() - 2)) {
						...
						if (!isFound) {
							...
							if (!isFound)
								for (int code : ccodes.all())
									if (promptSub.startsWith("BASE "
											+ ccodes.name(code))) {
										buf.append(mob.baseCharStats().getStat(
												code));
						...
						if (!isFound) {
							...
							if (!isFound)
								for (String s : mob.basePhyStats()
										.getStatCodes())
									if (promptSub.startsWith("BASE " + s)) {
										c += 6 + s.length();
										buf.append(mob.basePhyStats()
												.getStat(s));
										isFound = true;
										break;
									}
						...
						if (!isFound) {
							...
							if (!isFound)
								for (final String s : mob.maxState()
										.getStatCodes())
							...
							if (!isFound)
								for (final String s : mob.baseState()
										.getStatCodes())
									if (promptSub.startsWith("BASE " + s)) {
										c += 6 + s.length();
					...
					break;
				case 'a': {
					buf.append(CMLib.factions().getRangePercent(
				...
				case 'A': {
					final Faction.FRange FR = CMLib.factions().getRange(
							CMLib.factions().AlignID(),
							mob.fetchFaction(CMLib.factions().AlignID()));
				...
				case 'D': {
					...
					if ((I instanceof AmmunitionWeapon)
						...
						buf.append(""
								+ ((AmmunitionWeapon) I).ammunitionRemaining());
					break;
				}
				case 'e': {
				...
				case 'g': {
					buf.append((int) Math.round(Math.floor(CMLib.beanCounter()
							.getTotalAbsoluteNativeValue(mob)
							/ CMLib.beanCounter().getLowestDenomination(
									CMLib.beanCounter().getCurrency(mob)))));
					c++;
					break;
				}
				case 'G': {
					buf.append(CMLib.beanCounter().nameCurrencyShort(
							mob,
							CMLib.beanCounter()
									.getTotalAbsoluteNativeValue(mob)));
					c++;
				...
				case 'h': {
					buf.append("^<Hp^>" + mob.curState().getHitPoints()
							+ "^</Hp^>");
					c++;
					break;
				}
				case 'H': {
					buf.append("^<MaxHp^>" + mob.maxState().getHitPoints()
							+ "^</MaxHp^>");
					c++;
					break;
				}
				...
				case 'k': {
					...
					if (((c + 1) < prompt.length()) && (tank != null))
						...
						case 'E': {
							if ((mob.isInCombat())
									&& (CMLib.flags().canBeSeenBy(tank, mob)))
				...
				case 'm': {
					buf.append("^<Mana^>" + mob.curState().getMana()
							+ "^</Mana^>");
					c++;
					break;
				}
				case 'M': {
					buf.append("^<MaxMana^>" + mob.maxState().getMana()
							+ "^</MaxMana^>");
					c++;
					break;
				}
				...
				case 'v': {
					buf.append("^<Move^>" + mob.curState().getMovement()
							+ "^</Move^>");
					c++;
					break;
				}
				case 'V': {
					buf.append("^<MaxMove^>" + mob.maxState().getMovement()
							+ "^</MaxMove^>");
					c++;
					break;
				}
				...
				case 't': {
					if (mob.location() != null)
						buf.append(CMStrings
								.capitalizeAndLower(TimeClock.TOD_DESC[mob
										.location().getArea().getTimeObj()
										.getTODCode()].toLowerCase()));
					c++;
					break;
				}
				case 'T': {
					if (mob.location() != null)
						buf.append(mob.location().getArea().getTimeObj()
								.getTimeOfDay());
					c++;
					break;
				}
				case '@': {
					if (mob.location() != null)
						buf.append(mob.location().getArea().getClimateObj()
								.weatherDescription(mob.location()));
					c++;
					break;
				}
				default: {
