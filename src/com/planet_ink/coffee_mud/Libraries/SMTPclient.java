 ...
 */
public class SMTPclient extends StdLibrary implements SMTPLibrary,
	...
	public SMTPClient getClient(String emailAddress) throws IOException,
	 ...
	 */
	public SMTPclient(String SMTPServerInfo, int port)
			throws UnknownHostException, IOException {
		...
		if ((rstr == null) || (!rstr.startsWith("220")))
	...
	public SMTPclient(String emailAddress) throws IOException,
		...
		for (Enumeration<String> e = addys.elements(); e.hasMoreElements();) {
			...
			try {
				...
				sock.setSoTimeout(DEFAULT_TIMEOUT);
				send = new PrintWriter(sock.getOutputStream());
				boolean debug = CMSecurity
						.isDebugging(CMSecurity.DbgFlag.SMTPCLIENT);
				String rstr = reply.readLine();
				if (debug)
					Log.debugOut("SMTPclient", rstr);
				if ((rstr == null) || (!rstr.startsWith("220")))
	...
	public boolean emailIfPossible(String fromName, String toName, String subj,
			String msg) {
		try {
			...
			SC.sendMessage(fromName + "@" + domain, fromName + "@" + domain,
					toEmail, toEmail, subj, CMLib.coffeeFilter()
							.simpleOutFilter(msg));
			return true;
		} catch (Exception e) {
	...
	public boolean emailIfPossible(String SMTPServerInfo, String from,
			String replyTo, String to, String subject, String message) {
		try {
			SMTPclient SC = null;
			...
			SC.sendMessage(from, replyTo, to, to, subject, message);
			return true;
		} catch (Exception ioe) {
		}
		return false;
	...
	public void emailOrJournal(String SMTPServerInfo, String from,
			String replyTo, String to, String subject, String message) {
		...
		if (!this.emailIfPossible(SMTPServerInfo, fromEmail, replyToEmail,
				toEmail, subject, message))
			CMLib.database().DBWriteJournal(
					CMProps.getVar(CMProps.Str.MAILBOX), from, to, subject,
					message);
	}
	...
	/*
	 * Checks to see if the given string is a valid email address. If it is not,
	 * it returns the location from 0-length+1 where the problem occurs.
	 * 
	 * @param addy the email address
	 * 
	...
	private int getEmailAddressError(String addy) {
		...
		if ((localPart.startsWith("\"") && localPart.endsWith("\""))) {
			int z = localPart.substring(1, localPart.length() - 1)
					.indexOf("\"");
			if (z >= 0)
				return 1 + z;
		} else
	...
	public String makeValidEmailAddress(String name) {
		if (!isValidEmailAddress(name)) {
			if (CMLib.players().playerExists(name))
				name = CMLib.players().getLoadPlayer(name).playerStats()
						.getEmail();
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> Mailer.sendmsg(S, From, To, Subject, Message);
	 * 
	 * @param froaddress
	...
	public synchronized void sendMessage(String froaddress,
			String reply_address, String to_address, String mockto_address,
		...
		if (CMSecurity.isDisabled(CMSecurity.DisFlag.SMTPCLIENT)) {
			Log.debugOut("SMTPclient", "Message not sent: " + froaddress + "/"
					+ reply_address + "/" + to_address + "/" + mockto_address
					+ "/" + subject + "/" + message);
			return;
		}
		...
		for (int f = 0; f < fixMsg.length(); f++) {
			...
			} else if ((fixMsg.charAt(f) == '\r') && (f < fixMsg.length() - 1)
					&& (fixMsg.charAt(f + 1) != '\n')) {
		...
		try {
			...
			System.err
					.println("No local IP address found - is your network up?");
			throw ioe;
		}
		...
		if ((auth != null) && (auth.getAuthType().length() > 0)) {
			if (auth.getAuthType().equalsIgnoreCase("login"))
				sendLine(debug,
						"AUTH " + auth.getAuthType() + " " + auth.getLogin());
			else
	...
	/**
	 * return members of a list on an email server. 250-First Last
	 * <emailaddress>\r
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> List=Mailer.getListMembers(List);
	 * 
	 * @param list
	 *            member list
	 * @return String List of members
	 */
	public synchronized String getListMembers(String list) throws IOException,
			ProtocolException {
		...
		} catch (java.io.InterruptedIOException x) { // not really an error,
														// just a control break
		}
		sock.setSoTimeout(DEFAULT_TIMEOUT);
		if (!rstr.startsWith("250"))
			throw new ProtocolException(rstr);
