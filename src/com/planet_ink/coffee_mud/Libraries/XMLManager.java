...
public class XMLManager extends StdLibrary implements XMLLibrary {
	...
	protected final String[][] IGNORE_TAG_BOUNDS = { { "!--", "-->" },
			{ "?", "?>" }, { "![CDATA[", "]]>" } };

	protected enum State {
		START, BEFORETAG, INTAG, BEFOREATTRIB, BEGINTAGSELFEND, BEFORECLOSETAG, INCLOSETAG, AFTERCLOSETAG, INATTRIB, INPOSTATTRIB, BEFOREATTRIBVALUE, INATTRIBVALUE, INQUOTEDATTRIBVALUE
	}
		...
		while (loop < buf.length()) {
			...
			case '&':
				if (loop < buf.length() - 3) {
					if (buf.substring(loop + 1, loop + 4).equalsIgnoreCase(
							"lt;"))
						buf.replace(loop, loop + 4, "<");
					else if (buf.substring(loop + 1, loop + 4)
							.equalsIgnoreCase("gt;"))
						buf.replace(loop, loop + 4, ">");
					else if (buf.substring(loop + 1, loop + 6)
							.equalsIgnoreCase("quot;"))
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> int num=s_long(CMD.substring(14));
	 * 
	 * @param LONG
	...
	protected void abandonTagState(State newState) {
		if ((piece != null) && (piece.outerEnd < 0)) {
			...
			piece = piece.parent;
			Log.warnOut("XMLManager", "Abandoned tag " + childPiece.tag
					+ ((piece != null) ? " of parent " + piece.tag : ""));
		}
					...
					while (comDex <= (buf.length() - boundEnd.length())) {
						if (buf.charAt(comDex) == boundEnd.charAt(0)) {
							for (x = 1; x < boundEnd.length(); x++)
								if (buf.charAt(comDex + x) != boundEnd
										.charAt(x))
									break;
							if (x >= boundEnd.length()) {
								buf.delete(bufDex - 1,
										comDex + boundEnd.length());
								bufDex -= 2;
	...
	protected boolean canStartPiece(int endOfTagName) {
		final String tagName = buf
				.substring(beginDex[State.INTAG.ordinal()], endOfTagName)
				.toUpperCase().trim();
	...
	protected void closePiece(int outerEnd) {
		String closeTag = buf
				.substring(beginDex[State.INCLOSETAG.ordinal()],
						endDex[State.INCLOSETAG.ordinal()]).toUpperCase()
				.trim();
		XMLpiece closePiece = piece;
		while ((closePiece != null)
				&& (!closePiece.tag.equalsIgnoreCase(closeTag)))
			closePiece = closePiece.parent;
		if (closePiece != null) {
			if (closePiece.innerStart >= 0)
				closePiece.value = buf.substring(closePiece.innerStart,
						beginDex[State.BEFORETAG.ordinal()] - 1);
			piece = closePiece;
	...
	protected void endEmptyAttrib(int endOfAttrib) {
		if (piece != null) {
			String value = "";
			String parmName = buf.substring(beginDex[State.INATTRIB.ordinal()],
					endOfAttrib);
			if ((parmName.length() > 15) || (parmName.length() == 0))
				Log.warnOut("XMLManager", "Suspicious attribute '" + parmName
						+ "' for tag " + piece.tag);
	...
	protected void assignAttrib(int endOfValue) {
		if (piece != null) {
			String parmName = buf
					.substring(beginDex[State.INATTRIB.ordinal()],
							endDex[State.INATTRIB.ordinal()]).toUpperCase()
					.trim();
			String value = buf.substring(beginDex[state.ordinal()], endOfValue)
					.trim();
			if ((parmName.length() > 15) || (parmName.length() == 0))
				Log.warnOut("XMLManager", "Suspicious attribute '" + parmName
						+ "' for tag " + piece.tag);
	...
	public String returnXMLValue(String Blob) {
		...
		try {
			while ((start < Blob.length()) && (Blob.charAt(start) != '>'))
				start++;
			if ((start >= Blob.length()) || (Blob.charAt(start - 1) != '>')
					|| (Blob.charAt(start - 1) == '/'))
				return "";
		} catch (Throwable t) {
			return "";
	...
	public String returnXMLValue(String Blob, String Tag) {
		...
		try {
			while ((start < Blob.length()) && (Blob.charAt(start) != '>'))
				start++;
			if ((start >= Blob.length()) || (Blob.charAt(start) != '>')
					|| (Blob.charAt(start - 1) == '/'))
				return "";
		} catch (Throwable t) {
			return "";
