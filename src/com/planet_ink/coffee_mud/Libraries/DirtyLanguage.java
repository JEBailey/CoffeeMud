...
public class DirtyLanguage extends StdLibrary implements LanguageLibrary {
	...
	protected Hashtable<Object, Integer> HASHED_CMDS = CMStrings
			.makeNumericHash(new String[] { "REPLACE", "REPLACEWHOLE",
					"IGNORE", "IGNOREWHOLE", "AUTOIGNORE", "DEFINE",
					"REPLACEALL" });

	public void setLocale(String lang, String state) {
		if ((lang != null) && (state != null) && (lang.length() > 0)
				&& (state.length() > 0)) {
			country = state;
			language = lang;
		}
	...
	public String replaceWithDefinitions(DVector global, DVector local,
			String str) {
		for (int v = 0; v < local.size(); v++)
			str = CMStrings.replaceAll(str, (String) local.elementAt(v, 1),
					(String) local.elementAt(v, 2));
		for (int v = 0; v < global.size(); v++)
			str = CMStrings.replaceAll(str, (String) global.elementAt(v, 1),
					(String) global.elementAt(v, 2));
		return str;
	}
	...
	protected Hashtable<String, DVector> loadFileSections(String filename) {
		...
		if (!F.exists()) {
			Log.errOut("Language file " + filename
					+ " not found! This mud is in deep doo-doo!");
			return null;
		}
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (s.startsWith("[")) {
				...
				if ((currentSectionReplaceStrs.size() > 0)
						&& (currentSection != null))
					currentSection.addElement("REPLACEWHOLE",
							currentSectionReplaceStrs,
							currentSectionReplaceStrs);
				if ((currentSectionIgnoreStrs.size() > 0)
						&& (currentSection != null))
					currentSection.addElement("IGNOREWHOLE",
							currentSectionIgnoreStrs, currentSectionIgnoreStrs);
				...
				parserSections.put(s.substring(1, x).toUpperCase(),
						currentSection);
				sectionIndexes.addElement(s.substring(1, x).toUpperCase(),
						Integer.valueOf(v));
				localDefinitions.clear();
			} else if (s.toUpperCase().startsWith("AUTOIGNORE")) {
				int x = s.indexOf(' ');
				if (x < 0) {
					Log.errOut("Scripts", "Syntax error in '" + filename
							+ "', line " + (v + 1));
					continue;
				}
				Integer I = Integer.valueOf(CMath.s_int(s.substring(x + 1)
						.trim()));
				if (currentSection != null)
					currentSection.addElement("AUTOIGNORE", I,
							s.substring(x + 1).trim());
				...
				String variable = s.substring(regstart + 1, regend)
						.toUpperCase();
				...
				replacement = replaceWithDefinitions(globalDefinitions,
						localDefinitions, replacement);
				if (currentSection != null) {
				...
				String expression = unFilterString(s.substring(regstart + 1,
						regend));
				currentSectionIgnoreStrs.add(expression.toLowerCase());
			} else if (s.toUpperCase().startsWith("REPLACEWHOLE")) {
				int regstart = s.indexOf('"');
				if (regstart < 0) {
					Log.errOut("Scripts", "Syntax error in '" + filename
							+ "', line " + (v + 1));
				...
				String replacement = unFilterString(s.substring(regstart + 1,
						regend));
				currentSectionReplaceStrs.put(expression.toLowerCase(),
						replacement);
				...
				String replacement = unFilterString(s.substring(regstart + 1,
						regend));
				if (currentSection != null)
					currentSection.addElement(cmd, expression.toLowerCase(),
							replacement);
				currentSectionReplaceStrs.put(expression.toLowerCase(),
						replacement);
			} else if (s.toUpperCase().startsWith("REPLACE")
					|| s.toUpperCase().startsWith("IGNORE")) {
				String cmd = s.toUpperCase().startsWith("REPLACE") ? "REPLACE"
						: "IGNORE";
				int regstart = s.indexOf('"');
				if (regstart < 0) {
					Log.errOut("Scripts", "Syntax error in '" + filename
				...
				expression = replaceWithDefinitions(globalDefinitions,
						localDefinitions, expression);
				s = s.substring(regend + 1).trim();
				...
				try {
					Pattern expPattern = Pattern.compile(expression,
		...
		if ((currentSectionReplaceStrs.size() > 0) && (currentSection != null))
			currentSection.addElement("REPLACEWHOLE",
					currentSectionReplaceStrs, currentSectionReplaceStrs);
		if ((currentSectionIgnoreStrs.size() > 0) && (currentSection != null))
			currentSection.addElement("IGNOREWHOLE", currentSectionIgnoreStrs,
					currentSectionIgnoreStrs);
	...
	protected final String getLanguageTranslatorKey() {
		return "TRANSLATION_" + language.toUpperCase() + "_"
				+ country.toUpperCase();
	}
	...
	public DVector getLanguageParser(String parser) {
		...
		Hashtable<String, DVector> parserSections = (Hashtable<String, DVector>) Resources
				.getResource(parserKey);
		if (parserSections == null) {
			parserSections = loadFileSections("resources/parser_"
					+ language.toUpperCase() + "_" + country.toUpperCase()
					+ ".properties");
			if (parserSections == null)
	...
	public DVector getLanguageTranslator(String parser) {
		...
		Hashtable<String, DVector> translationSections = (Hashtable<String, DVector>) Resources
				.getResource(translatorKey);
		if (translationSections == null) {
			translationSections = loadFileSections("resources/translation_"
					+ language.toUpperCase() + "_" + country.toUpperCase()
					+ ".properties");
			if (translationSections == null)
	...
	public boolean insertExpansion(List<String> MORE_CMDS, String str, int m,
			int strLen, boolean nothingDone) {
		List<String> expansion = CMParms.parseAny(
	...
	public List<List<String>> preCommandParser(List<String> CMDS) {
		...
		DVector parser = CMLib.lang()
				.getLanguageParser("COMMAND-PRE-PROCESSOR");
		...
		for (int p = 0; p < parser.size(); p++) {
			...
			if (I != null)
						...
						for (int m = 0; m < MORE_CMDS.size(); m++) {
							...
							if (matcher.find()) {
								...
								for (int i = 0; i <= matcher.groupCount(); i++)
									str = CMStrings.replaceAll(str, "\\" + i,
											matcher.group(i));
								if (!MORE_CMDS.get(m).equals(str))
									nothingDone = insertExpansion(MORE_CMDS,
											str, m, strLen, nothingDone);
							}
						}
					}
					break;
				...
				case CMD_REPLACEWHOLE: {
					rep = ((Hashtable<String, String>) parser.elementAt(p, 2))
							.get(combinedWithTabs.toLowerCase());
					if (rep != null) {
						insertExpansion(MORE_CMDS, rep, 0,
								combinedWithTabs.length(), true);
						p = parser.size();
					}
					break;
				}
					...
					for (int m = 0; m < MORE_CMDS.size(); m++) {
						...
						if (x >= 0) {
							while (x >= 0) {
								...
								str = str.substring(0, x) + wit
										+ str.substring(x + rep.length());
								x = str.toLowerCase().indexOf(rep,
										x + wit.length());
							}
				...
				case CMD_AUTOIGNORE:
					autoIgnoreLen = ((Integer) parser.elementAt(p, 2))
							.intValue();
					if (autoIgnoreLen == 0)
						autoIgnoreLen = 100;
					break;
		...
		if ((MORE_CMDS.size() == 1)
			...
			if ((autoIgnoreLen > 0) && (str != null)
				...
				addAutoIgnoredString(combinedWithTabs, fileData, fileIndexes,
	...
	@SuppressWarnings("unchecked")
	protected String basicParser(String str, String section,
			boolean nullIfLonger, boolean isParser) {
		if (str == null)
			return null;
		DVector parser = isParser ? getLanguageParser(section)
				: getLanguageTranslator(section);
		if (parser == null)
			return null;
		Pattern pattern = null;
		Matcher matcher = null;
	...
	public void addAutoIgnoredString(String str, DVector fileData,
			DVector fileIndexes, String sectionName) {
		...
		for (int f = 0; f < fileIndexes.size(); f++)
			if (((Integer) fileIndexes.elementAt(f, 2)).intValue() > index)
				fileIndexes.setElementAt(f, 2, Integer
						.valueOf(((Integer) fileIndexes.elementAt(f, 2))
								.intValue() + 1));
