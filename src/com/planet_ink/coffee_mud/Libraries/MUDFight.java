...
public class MUDFight extends StdLibrary implements CombatLibrary {
	...
	public boolean activate() {
		attackAdjustmentFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_ATTACKADJUSTMENT));
		armorAdjustmentFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_ARMORADJUSTMENT));
		attackerFudgeBonusFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_ATTACKFUDGEBONUS));
		pvpAttackerFudgeBonusFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPATTACKFUDGEBONUS));
		spellCritChanceFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_CHANCESPELLCRIT));
		pvpSpellCritChanceFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPCHANCESPELLCRIT));
		spellCritDmgFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_DAMAGESPELLCRIT));
		pvpSpellCritDmgFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPDAMAGESPELLCRIT));
		targetedRangedDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_DAMAGERANGEDTARGETED));
		pvpTargetedRangedDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPDAMAGERANGEDTARGETED));
		targetedMeleeDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_DAMAGEMELEETARGETED));
		pvpTargetedMeleeDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPDAMAGEMELEETARGETED));
		staticRangedDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_DAMAGERANGEDSTATIC));
		staticMeleeDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_DAMAGEMELEESTATIC));
		weaponCritChanceFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_CHANCEWEAPONCRIT));
		pvpWeaponCritChanceFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPCHANCEWEAPONCRIT));
		weaponCritDmgFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_DAMAGEWEAPONCRIT));
		pvpWeaponCritDmgFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPDAMAGEWEAPONCRIT));
		spellFudgeDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_DAMAGESPELLFUDGE));
		pvpSpellFudgeDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPDAMAGESPELLFUDGE));
		meleeFudgeDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_DAMAGEMELEEFUDGE));
		pvpMeleeFudgeDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPDAMAGEMELEEFUDGE));
		rangedFudgeDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_DAMAGERANGEDFUDGE));
		pvpRangedFudgeDamageFormula = CMath.compileMathExpression(CMProps
				.getVar(CMProps.Str.FORMULA_PVPDAMAGERANGEDFUDGE));
		return true;
	}
	...
	public Set<MOB> allPossibleCombatants(MOB mob, boolean beRuthless) {
		...
		for (int m = 0; m < thisRoom.numInhabitants(); m++) {
			...
			if ((inhab != null)
					&& (inhab != mob)
					&& (!h1.contains(inhab))
					&& (CMLib.flags().isSeen(inhab) || CMLib.flags().canMove(
							inhab))
					&& (CMLib.flags().isSeen(inhab) || (!CMLib.flags()
							.isCloaked(inhab)))
					&& ((beRuthless) || (!mob.isMonster()) || (!inhab
							.isMonster())))
				h.addUnsafe(inhab);
		}
		return h;
	...
	public int adjustedAttackBonus(MOB mob, MOB target) {
		...
		return (int) Math.round(CMath.parseMathExpression(
				attackAdjustmentFormula, vars, 0.0));
	}

	public void postItemDamage(MOB mob, Item I, Environmental tool,
			int damageAmount, int messageType, String message) {
		if (mob == null)
			return;
		Room R = mob.location();
		if (R == null)
			return;
		CMMsg msg = CMClass.getMsg(mob, I, tool, CMMsg.MASK_MALICIOUS
				| CMMsg.MASK_ALWAYS | messageType, message,
				CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS | messageType,
				message, CMMsg.NO_EFFECT, null);
		if (R.okMessage(mob, msg)) {
			R.send(mob, msg);
			if (msg.value() <= 0) {
				I.setUsesRemaining(I.usesRemaining() - damageAmount);
				I.recoverPhyStats(); // important relation to setuses -- for
										// brittle
				if (I.usesRemaining() <= 0) {
					I.setUsesRemaining(100);
					I.unWear();
					msg = CMClass.getMsg(mob, null, I, CMMsg.MSG_OK_VISUAL,
							"<O-NAME> is destroyed!", null,
							"<O-NAME> carried by <S-NAME> is destroyed!");
					if (R.okMessage(mob, msg))
						R.send(mob, msg);
					I.destroy();
					mob.recoverPhyStats();
	...
	public int adjustedArmor(MOB mob) {
		...
		double[] vars = {
				...
				(mob.curState().getThirst() < 1) ? 1.0 : 0.0,
				(mob.curState().getFatigue() > CharState.FATIGUED_MILLIS) ? 1.0
						: 0.0, CMLib.flags().isSitting(mob) ? 0.0 : 1.0,
				CMLib.flags().isSleeping(mob) ? 0.0 : 1.0 };
		return (int) Math.round(CMath.parseMathExpression(
				armorAdjustmentFormula, vars, 0.0));
	}
	...
	public boolean rollToHit(MOB attacker, MOB defender) {
		...
		double vars[] = {
				...
				attacker.phyStats().level() > defender.phyStats().level() ? 1
						: -1 };
		final boolean isPVP = (attacker.isPlayer() && defender.isPlayer());
		int attackerFudgeBonusAmt = (int) Math.round(CMath.parseMathExpression(
				isPVP ? pvpAttackerFudgeBonusFormula
						: attackerFudgeBonusFormula, vars, 0.0));
		return rollToHit(adjustedAttackBonus(attacker, defender),
				adjustedArmor(defender), attackerFudgeBonusAmt);
	}
	...
	public boolean rollToHit(int attack, int defence, int adjustment) {
		...
		return CMLib.dice().normalizeAndRollLess(
				(int) Math.round(50.0 * (hisAttack / myArmor)) + adjustment);
	}
	...
	public Set<MOB> allCombatants(MOB mob) {
		...
		for (int m = 0; m < thisRoom.numInhabitants(); m++) {
			...
			if ((inhab != null)
					&& (inhab != mob)
					&& ((inhab == mob.getVictim()) || ((inhab != mob)
							&& (inhab.getVictim() != mob.getVictim())
							&& (CMLib.flags().isSeen(inhab) || (!CMLib.flags()
									.isCloaked(inhab)))
							&& (CMLib.flags().canMove(inhab) || CMLib.flags()
									.isSeen(inhab)) && (!h1.contains(inhab)))))
				h.addUnsafe(inhab);
		}
		return h;
	...
	public void postDeath(MOB killerM, MOB deadM, CMMsg addHere) {
		...
		String msp = CMLib.protocol().msp(
				"death" + CMLib.dice().roll(1, 7, 0) + ".wav", 50);
		CMMsg msg = null;
		if (isKnockedOutUponDeath(deadM, killerM))
			msg = CMClass.getMsg(deadM, null, killerM, CMMsg.MSG_OK_VISUAL,
					"^f^*^<FIGHT^>!!!!!!!!!YOU ARE DEFEATED!!!!!!!!!!^</FIGHT^>^?^.\n\r"
							+ msp, CMMsg.MSG_OK_VISUAL, null, CMMsg.MSG_DEATH,
					"^F^<FIGHT^><S-NAME> is DEFEATED!!!^</FIGHT^>^?\n\r" + msp);
		else
			msg = CMClass.getMsg(deadM, null, killerM, CMMsg.MSG_OK_VISUAL,
					"^f^*^<FIGHT^>!!!!!!!!!!!!!!YOU ARE DEAD!!!!!!!!!!!!!!^</FIGHT^>^?^.\n\r"
							+ msp, CMMsg.MSG_OK_VISUAL, null, CMMsg.MSG_DEATH,
					"^F^<FIGHT^><S-NAME> is DEAD!!!^</FIGHT^>^?\n\r" + msp);
		CMLib.map().sendGlobalMessage(
				deadM,
				CMMsg.TYP_DEATH,
				CMClass.getMsg(deadM, null, killerM, CMMsg.TYP_DEATH, null,
						CMMsg.TYP_DEATH, null, CMMsg.TYP_DEATH, null));
		CMMsg msg2 = CMClass.getMsg(deadM, null, killerM, CMMsg.MSG_DEATH,
				null, CMMsg.MSG_DEATH, null, CMMsg.MSG_DEATH, null);
		if (addHere != null) {
	...
	public boolean postAttack(MOB attacker, MOB target, Item weapon) {
		...
		CMMsg msg = CMClass.getMsg(attacker, target, weapon,
				CMMsg.MSG_WEAPONATTACK, null);
	...
	public boolean postHealing(MOB healer, MOB target, Environmental tool,
			int messageCode, int healing, String allDisplayMessage) {
		if (healer == null)
			healer = target;
		if ((healer == null) || (target == null) || (target.location() == null))
			return false;
		CMMsg msg = CMClass.getMsg(healer, target, tool, messageCode,
				CMMsg.MSG_HEALING, messageCode, allDisplayMessage);
	...
	public String replaceDamageTag(String str, int damage, int damageType,
		...
		if (str.length() < replace + 9)
			return str;
		final boolean damages = (str.charAt(replace + 7) == 'S')
				&& (str.charAt(replace + 8) == '>');
		final String showDamage = CMProps.getVar(CMProps.Str.SHOWDAMAGE);
		final boolean showNumbers = showDamage.equalsIgnoreCase("YES")
				|| ((sourceTargetSTO == 'S') && showDamage
						.equalsIgnoreCase("SOURCE"))
				|| ((sourceTargetSTO == 'T') && showDamage
						.equalsIgnoreCase("TARGET"));
		if (damages) {
			final String hitWord = CMStrings.deleteAllofAny(
					standardHitWord(damageType, damage), PARENS);
			if (!showNumbers)
				return str.substring(0, replace) + hitWord
						+ str.substring(replace + 9);
			return str.substring(0, replace) + hitWord + " (" + damage + ")"
					+ str.substring(replace + 9);
		} else if (str.charAt(replace + 7) == '>') {
			if (!showNumbers)
				return str.substring(0, replace)
						+ standardHitWord(damageType, damage)
						+ str.substring(replace + 8);
			return str.substring(0, replace)
					+ standardHitWord(damageType, damage) + " (" + damage + ")"
					+ str.substring(replace + 8);
		}
		return str;
	}

	public void postDamage(MOB attacker, MOB target, Environmental weapon,
			int damage, int messageCode, int damageType,
			String allDisplayMessage) {
		if ((attacker == null) || (target == null)
				|| (target.location() == null))
			return;
		if (allDisplayMessage != null)
			allDisplayMessage = "^F^<FIGHT^>" + allDisplayMessage
					+ "^</FIGHT^>^?";
		...
		if (attacker != target) {
			if ((weapon instanceof Ability)
					&& (damage > 0)
					&& (attacker.isMine(weapon) || (attacker.phyStats().level() > 1))) // why
																						// >1?
																						// because
																						// quickly
																						// made
			...
			damageTypeMsg = CMMsg.TYP_DAMAGE
					| (CMMsg.MASK_MALICIOUS & messageCode);
		} else
			damageTypeMsg = CMMsg.TYP_DAMAGE;

		CMMsg msg = CMClass.getMsg(attacker, target, weapon, messageCode,
				damageTypeMsg, messageCode, allDisplayMessage);
		msg.setValue(damage);
			...
			if (R.okMessage(target, msg)) {
				if (damageType >= 0)
					msg.modify(
							...
							msg.tool(),
							msg.sourceCode(),
							replaceDamageTag(msg.sourceMessage(), msg.value(),
									damageType, 'S'),
							msg.targetCode(),
							replaceDamageTag(msg.targetMessage(), msg.value(),
									damageType, 'T'),
							msg.othersCode(),
							replaceDamageTag(msg.othersMessage(), msg.value(),
									damageType, 'O'));
				R.send(target, msg);
			}
	}
	...
	public int modifySpellDamage(MOB attacker, MOB target, int baseDamage) {
		int maxInt = attacker.charStats().getMaxStat(
				CharStats.STAT_INTELLIGENCE);
		...
		int baseInt = attacker.baseCharStats().getStat(
				CharStats.STAT_INTELLIGENCE);
		if (baseInt > maxInt)
			baseInt = maxInt;
		double[] vars = {
				...
				(attacker.curState().getFatigue() > CharState.FATIGUED_MILLIS) ? 1.0
						: 0.0, attacker.phyStats().level(),
				target.phyStats().level() };
		final boolean isPVP = (attacker.isPlayer() && target.isPlayer());
		baseDamage = (int) Math.round(CMath.parseMathExpression(
				isPVP ? pvpSpellFudgeDamageFormula : spellFudgeDamageFormula,
				vars, 0.0));
		vars[0] = baseDamage;
		int spellCritChancePct = (int) Math.round(CMath.parseMathExpression(
				isPVP ? pvpSpellCritChanceFormula : spellCritChanceFormula,
				vars, 0.0));
		if (CMLib.dice().rollPercentage() < spellCritChancePct) {
			int spellCritDamageAmt = (int) Math.round(CMath
					.parseMathExpression(isPVP ? pvpSpellCritDmgFormula
							: spellCritDmgFormula, vars, 0.0));
	...
	public int adjustedDamage(MOB mob, Weapon weapon, MOB target,
			int bonusDamage, boolean allowCrits) {
		double damageAmount = 0.0;
		Physical useDmg = null;
		boolean rangedAttack = false;
		if ((weapon != null)
				&& ((weapon.weaponClassification() == Weapon.CLASS_RANGED) || (weapon
						.weaponClassification() == Weapon.CLASS_THROWN))) {
			useDmg = weapon;
			rangedAttack = true;
		} else
			useDmg = mob;
		final boolean isPVP = ((target != null) && mob.isPlayer() && target
				.isPlayer());
		if (target != null) {
			double[] vars = {
			...
			if (rangedAttack)
				damageAmount = CMath.parseMathExpression(
						isPVP ? pvpTargetedRangedDamageFormula
								: targetedRangedDamageFormula, vars, 0.0);
			else
				damageAmount = CMath.parseMathExpression(
						isPVP ? pvpTargetedMeleeDamageFormula
								: targetedMeleeDamageFormula, vars, 0.0);
		} else {
			double[] vars = {
					useDmg.phyStats().damage() + bonusDamage,
					mob.charStats().getStat(CharStats.STAT_STRENGTH),
			...
			if ((weapon != null)
					&& ((weapon.weaponClassification() == Weapon.CLASS_RANGED) || (weapon
							.weaponClassification() == Weapon.CLASS_THROWN)))
				damageAmount = CMath.parseMathExpression(
						staticRangedDamageFormula, vars, 0.0);
			else
				damageAmount = CMath.parseMathExpression(
						staticMeleeDamageFormula, vars, 0.0);
		}
		...
		double[] vars = {
				...
				(mob.curState().getThirst() < 1) ? 1.0 : 0.0,
				(mob.curState().getFatigue() > CharState.FATIGUED_MILLIS) ? 1.0
						: 0.0, mob.phyStats().level(),
				(target == null) ? 0 : target.phyStats().level() };
		if (allowCrits) {
			int weaponCritChancePct = (int) Math.round(CMath
					.parseMathExpression(isPVP ? pvpWeaponCritChanceFormula
							: weaponCritChanceFormula, vars, 0.0));
			if (CMLib.dice().rollPercentage() < weaponCritChancePct) {
				int weaponCritDmgAmt = (int) Math.round(CMath
						.parseMathExpression(isPVP ? pvpWeaponCritDmgFormula
								: weaponCritDmgFormula, vars, 0.0));
		...
		if (target != null) {
			...
			if (rangedAttack)
				damageAmount = CMath.parseMathExpression(
						isPVP ? pvpRangedFudgeDamageFormula
								: rangedFudgeDamageFormula, vars, 0.0);
			else
				damageAmount = CMath.parseMathExpression(
						isPVP ? pvpMeleeFudgeDamageFormula
								: meleeFudgeDamageFormula, vars, 0.0);
		}
		return (int) Math.round(damageAmount);
	}

	public void postWeaponDamage(MOB source, MOB target, Item item,
			boolean success) {
		if (source == null)
			return;
		...
		if (success) {
			...
			String oldHitString = "^F^<FIGHT^>"
					+ ((weapon != null) ? weapon.hitString(damageInt)
							: standardHitString(Weapon.TYPE_NATURAL,
									Weapon.CLASS_BLUNT, damageInt, item.name()))
					+ "^</FIGHT^>^?";
			CMMsg msg = CMClass.getMsg(source, target, item,
					CMMsg.MSG_OK_VISUAL, CMMsg.MSG_DAMAGE, CMMsg.MSG_OK_VISUAL,
					oldHitString);
			CMLib.color().fixSourceFightColor(msg);
			...
			if ((room != null) && (room.okMessage(source, msg))) {
				...
				if ((source.mayIFight(target)) && (source.location() == room)
						&& (target.location() == room))
					room.send(source, msg);
			}
		} else {
			String missString = "^F^<FIGHT^>"
					+ ((weapon != null) ? weapon.missString()
							: standardMissString(Weapon.TYPE_BASHING,
									Weapon.CLASS_BLUNT, item.name(), false))
					+ "^</FIGHT^>^?";
			CMMsg msg = CMClass.getMsg(source, target, weapon,
					CMMsg.MSG_NOISYMOVEMENT, missString);
			CMLib.color().fixSourceFightColor(msg);
			// why was there no okaffect here?
			Room R = source.location();
			if (R != null)
				if (R.okMessage(source, msg) && (!source.amDead())
						&& (!source.amDestroyed()))
					R.send(source, msg);
		}
	}
	...
	public MOB getCombatDominentKiller(MOB killer, MOB killed) {
		...
		checked.add(M);
		while (M.isMonster() && (M.amFollowing() != null)
	...
	protected Set<MOB> getCombatBeneficiaries(MOB killer, MOB killed,
			Room deathRoom, Set<MOB> beneficiaries, CharClass combatCharClass) {
		Set<MOB> followers = (killer != null) ? killer
				.getGroupMembers(new HashSet<MOB>()) : (new SHashSet<MOB>());
		...
		if (deathRoom != null) {
			for (int m = 0; m < deathRoom.numInhabitants(); m++) {
				...
				if ((combatCharClass.isValidClassBeneficiary(killer, killed,
						mob, followers)) && (!beneficiaries.contains(mob)))
					beneficiaries.add(mob);
			}
		}
		if ((killer != null) && (!beneficiaries.contains(killer))
				&& (killer != killed)
				&& (CMLib.flags().isInTheGame(killer, true)))
			beneficiaries.add(killer);
		return beneficiaries;
	}

	public Set<MOB> getCombatBeneficiaries(MOB killer, MOB killed,
		...
		if (R != null)
			getCombatBeneficiaries(killer, killed, R, beneficiaries,
					combatCharClass);
		R = killed.location();
	...
	protected Set<MOB> getCombatDividers(MOB killer, MOB killed,
			Room deathRoom, Set<MOB> dividers, CharClass combatCharClass) {
		Set<MOB> followers = (killer != null) ? killer
				.getGroupMembers(new HashSet<MOB>()) : (new HashSet<MOB>());
		if (combatCharClass == null)
			combatCharClass = CMClass.getCharClass("StdCharClass");
		if (deathRoom != null) {
			for (int m = 0; m < deathRoom.numInhabitants(); m++) {
				MOB mob = deathRoom.fetchInhabitant(m);
				if ((combatCharClass.isValidClassDivider(killer, killed, mob,
						followers)) && (!dividers.contains(mob)))
					dividers.add(mob);
			}
		}
		if ((killer != null) && (!dividers.contains(killer))
				&& (killer != killed)
				&& (CMLib.flags().isInTheGame(killer, true)))
			dividers.add(killer);
		return dividers;
	...
	public Set<MOB> getCombatDividers(MOB killer, MOB killed,
			CharClass combatCharClass) {
		if ((killer == null) || (killed == null))
			return new SHashSet<MOB>();
	...
	public DeadBody justDie(MOB source, MOB target) {
		...
		Set<MOB> beneficiaries = getCombatBeneficiaries(source, target,
				combatCharClass);
		...
		if ((target.isMonster()) || (target.soulMate() != null))
			cmds = CMParms.toStringArray(CMParms.parseCommas(
					CMProps.getVar(CMProps.Str.MOBDEATH), true));
		else
			cmds = CMParms.toStringArray(CMParms.parseCommas(
					CMProps.getVar(CMProps.Str.PLAYERDEATH), true));

		DeadBody body = null; // must be done before consequences because
								// consequences could be purging
		if ((!CMParms.containsIgnoreCase(cmds, "RECALL"))
				&& (!isKnockedOutUponDeath(target, source)))
			body = target.killMeDead(true);

		handleConsequences(target, source, cmds, expLost,
				"^*You lose @x1 experience points.^?^.");

		if (!isKnockedOutUponDeath(target, source)) {
			Room bodyRoom = deathRoom;
			if ((body != null) && (body.owner() instanceof Room)
			...
			if ((deadMoney > 0) && (myAmountOfDeadMoney > 0) && (body != null)
				...
				for (int g = 0; g < goldLooters.size(); g++) {
					C = CMLib.beanCounter().makeBestCurrency(currency,
							myAmountOfDeadMoney, null, body);
					if (C != null) {
						C.recoverPhyStats();
						...
						if (mob.location() == bodyRoom) {
							if ((mob.riding() != null)
								...
								mob.tell("You'll need to dismount to get "
										+ C.name() + " off the body.");
							else if ((mob.riding() != null)
									&& (mob.riding() instanceof Item))
								mob.tell("You'll need to disembark to get "
										+ C.name() + " off the body.");
			...
			if ((body != null) && (bodyRoom != null)
					&& (body.destroyAfterLooting())) {
				for (int i = bodyRoom.numItems() - 1; i >= 0; i--) {
					Item item = bodyRoom.getItem(i);
	...
	public String standardHitWord(int type, int damage) {
		...
		final int[] thresholds = CMProps
				.getListFileIntList(CMProps.ListFile.DAMAGE_WORDS_THRESHOLDS);
		...
		damnCode++; // always add 1 because index into hitwords is type=0,
					// annoy=1;
		final Object[][][] hitWords = CMProps
				.getListFileGrid(CMProps.ListFile.DAMAGE_WORDS);
			...
			for (int w = 0; w < Weapon.TYPE_DESCS.length; w++) {
				...
				for (int i = 0; i < hitWords.length; i++) {
					if (((String) hitWords[i][0][0]).equalsIgnoreCase("ALL"))
						ALL = hitWords[i];
					else if (((String) hitWords[i][0][0])
							.equalsIgnoreCase(Weapon.TYPE_DESCS[w])) {
						MINE = hitWords[i];
						break;
					}
				}
	...
	public String armorStr(MOB mob) {
		...
		int ARMOR_CEILING = CMProps
				.getListFileFirstInt(CMProps.ListFile.ARMOR_DESCS_CEILING);
		final int numArmorDescs = CMProps
				.getListFileSize(CMProps.ListFile.ARMOR_DESCS);
		return (armor < 0) ? CMProps.getListFileValue(
				CMProps.ListFile.ARMOR_DESCS, 0)
				: ((armor >= ARMOR_CEILING) ? CMProps.getListFileValue(
						CMProps.ListFile.ARMOR_DESCS, numArmorDescs - 1)
						+ (CMStrings.repeat("!", (armor - ARMOR_CEILING) / 100))
						+ "^. (" + armor + ")"
						: (CMProps.getListFileValue(
								CMProps.ListFile.ARMOR_DESCS,
								(int) Math.round(Math.floor(CMath.mul(
										CMath.div(armor, ARMOR_CEILING),
										numArmorDescs))))
								+ "^. (" + armor + ")"));
	}
	...
	public String fightingProwessStr(MOB mob) {
		...
		final int PROWESS_CEILING = CMProps
				.getListFileFirstInt(CMProps.ListFile.PROWESS_DESCS_CEILING);
		final int numProwessDescs = CMProps
				.getListFileSize(CMProps.ListFile.PROWESS_DESCS);
		return (prowess < 0) ? CMProps.getListFileValue(
				CMProps.ListFile.PROWESS_DESCS, 0)
				: ((prowess >= PROWESS_CEILING) ? CMProps.getListFileValue(
						CMProps.ListFile.PROWESS_DESCS, numProwessDescs - 1)
						+ (CMStrings.repeat("!",
								(prowess - PROWESS_CEILING) / 100))
						+ "^. ("
						+ prowess + ")" : (CMProps.getListFileValue(
						CMProps.ListFile.PROWESS_DESCS, (int) Math.round(Math
								.floor(CMath.mul(
										CMath.div(prowess, PROWESS_CEILING),
										numProwessDescs))))
						+ "^. (" + prowess + ")"));
	}

	protected int getWeaponAttackIndex(final int weaponType,
			final int weaponClassification) {
		switch (weaponClassification) {
		case Weapon.CLASS_RANGED:
			return (weaponType == Weapon.TYPE_LASERING) ? 5 : 0;
		case Weapon.CLASS_THROWN:
			return (weaponType == Weapon.TYPE_LASERING) ? 5 : 1;
		default:
			switch (weaponType) {
	...
	public String standardMissString(final int weaponType,
			final int weaponClassification, final String weaponName,
			final boolean useExtendedMissString) {
		final int listIndex = getWeaponAttackIndex(weaponType,
				weaponClassification);
		if (!useExtendedMissString)
			return CMProps.getListFileValue(CMProps.ListFile.MISS_DESCS,
					listIndex);
		return CMStrings.replaceAll(CMProps.getListFileValue(
				CMProps.ListFile.WEAPON_MISS_DESCS, listIndex), "<TOOLNAME>",
				weaponName)
				+ CMLib.protocol().msp("missed.wav", 20);
	}

	public String standardHitString(final int weaponType,
			final int weaponClass, final int damageAmount,
		...
		final StringBuilder str = new StringBuilder(CMStrings.replaceAll(
				CMProps.getListFileValue(CMProps.ListFile.WEAPON_HIT_DESCS,
						listIndex), "<TOOLNAME>", weaponName));
		switch (weaponClass) {
		case Weapon.CLASS_RANGED:
		...
		default:
			return str.append(
					CMLib.protocol().msp(
							"punch" + CMLib.dice().roll(1, 7, 0) + ".wav", 20))
					.toString();
		}
	}

	public String standardMobCondition(MOB viewer, MOB mob) {
		int pct = (int) Math.round(Math.floor((CMath.div(mob.curState()
				.getHitPoints(), mob.maxState().getHitPoints())) * 10));
		if (pct < 0)
			pct = 0;
		final int numHealthDescs = CMProps
				.getListFileSize(CMProps.ListFile.HEALTH_CHART);
		if (pct >= numHealthDescs)
			pct = numHealthDescs - 1;
		return CMStrings.replaceAll(
				CMProps.getListFileValue(CMProps.ListFile.HEALTH_CHART, pct),
				"<MOB>", mob.name(viewer));
	}
	...
	public void resistanceMsgs(CMMsg msg, MOB source, MOB target) {
		...
		case CMMsg.TYP_MIND:
			tackOn = "<S-NAME> shake(s) off the "
					+ ((tool == null) ? "mental attack" : tool) + endPart;
			break;
		case CMMsg.TYP_GAS:
			tackOn = "<S-NAME> resist(s) the "
					+ ((tool == null) ? "noxious fumes" : tool) + endPart;
			break;
		case CMMsg.TYP_COLD:
			tackOn = "<S-NAME> shake(s) off the "
					+ ((tool == null) ? "cold blast" : tool) + endPart;
			break;
		case CMMsg.TYP_ELECTRIC:
			tackOn = "<S-NAME> shake(s) off the "
					+ ((tool == null) ? "electrical attack" : tool) + endPart;
			break;
		case CMMsg.TYP_FIRE:
			tackOn = "<S-NAME> resist(s) the "
					+ ((tool == null) ? "blast of heat" : tool) + endPart;
			break;
		case CMMsg.TYP_WATER:
			tackOn = "<S-NAME> dodge(s) the "
					+ ((tool == null) ? "wet blast" : tool) + endPart;
			break;
		case CMMsg.TYP_UNDEAD:
			tackOn = "<S-NAME> shake(s) off the "
					+ ((tool == null) ? "evil attack" : tool) + endPart;
			break;
		case CMMsg.TYP_POISON:
			tackOn = "<S-NAME> shake(s) off the "
					+ ((tool == null) ? "poison" : tool) + endPart;
			break;
		case CMMsg.TYP_DISEASE:
			tackOn = "<S-NAME> resist(s) the "
					+ ((tool == null) ? "disease" : tool);
			break;
		case CMMsg.TYP_JUSTICE:
			break;
		case CMMsg.TYP_CAST_SPELL:
			tackOn = "<S-NAME> resist(s) the "
					+ ((tool == null) ? "magical attack" : tool) + endPart;
			break;
		case CMMsg.TYP_PARALYZE:
			tackOn = "<S-NAME> resist(s) the "
					+ ((tool == null) ? "paralysis" : tool) + endPart;
			break;
		case CMMsg.TYP_SONIC:
			tackOn = "<S-NAME> shake(s) off the "
					+ ((tool == null) ? "sonic" : tool) + endPart;
			break;
		case CMMsg.TYP_LASER:
			tackOn = "<S-NAME> dodge(s) the "
					+ ((tool == null) ? "laser" : tool) + endPart;
			break;
		}
		if (tackOn != null) {
			String newStr = target + "/" + source + "/" + tool;
			if (!newStr.equals(lastStr)
					|| ((System.currentTimeMillis() - lastRes) > 250))
				msg.addTrailerMsg(CMClass.getMsg(target, source,
						CMMsg.MSG_OK_ACTION, tackOn));
			lastStr = newStr;
	...
	public void handleBeingDamaged(CMMsg msg) {
		...
		if (Log.combatChannelOn()) {
			...
			String type = (msg.sourceMinor() == CMMsg.NO_EFFECT) ? "??"
					: CMMsg.TYPE_DESCS[msg.sourceMinor()];
			Log.combatOut("DAMG", attacker.Name() + ":"
					+ attacker.phyStats().getCombatStats() + ":"
					+ attacker.curState().getCombatStats() + ":"
					+ ((KI == null) ? "null" : KI.name()) + ":" + target.Name()
					+ ":" + target.phyStats().getCombatStats() + ":"
					+ target.curState().getCombatStats() + ":"
					+ ((DI == null) ? "null" : DI.name()) + ":" + tool + ":"
					+ type + ":" + dmg);
		}
		synchronized (("DMG" + target.Name().toUpperCase()).intern()) {
			if ((dmg > 0) && (target.curState().getHitPoints() > 0)) {
				if ((!target.curState().adjHitPoints(-dmg, target.maxState()))
					...
					if ((Math.round(CMath.div(dmg, target.maxState()
							.getHitPoints()) * 100.0) >= CMProps
							.getIntVar(CMProps.Int.INJBLEEDPCTHP))
							&& bleedableWeapon(msg.tool())) {
						Ability A2 = CMClass.getAbility("Bleeding");
						if (A2 != null)
							A2.invoke((target), (target), true, 0);
					}
					if ((target.curState().getHitPoints() < target
							.getWimpHitPoint())
							&& (target.getWimpHitPoint() > 0)
							&& (target.isInCombat()))
						postPanic(target, msg);
					else if ((CMProps.getIntVar(CMProps.Int.INJPCTHP) >= (int) Math
							.round(CMath.div(target.curState().getHitPoints(),
									target.maxState().getHitPoints()) * 100.0))
							&& (!CMLib.flags().isGolem(target))
							&& (target.fetchEffect("Injury") == null)) {
						Ability A = CMClass.getAbility("Injury");
						if (A != null)
							A.invoke(target, new XVector<Object>(msg), target,
									true, 0);
					}
				}
			}
			...
			if ((!deadmob.isMonster()) && (deadmob.soulMate() == null)) {
				...
				List<String> channels = CMLib.channels()
						.getFlaggedChannelNames(
								ChannelsLibrary.ChannelFlag.DETAILEDDEATHS);
				List<String> channels2 = CMLib.channels()
						.getFlaggedChannelNames(
								ChannelsLibrary.ChannelFlag.DEATHS);
						...
						if ((msg.tool() != null) && (msg.tool() instanceof MOB))
							...
							CMLib.commands().postChannel(
									...
									deadmob.Name()
											+ " has just died at "
											+ CMLib.map().getExtendedRoomID(
													deadmob.location()), true);
					}
				if (!CMLib.flags().isCloaked(deadmob)) {
					for (int i = 0; i < channels2.size(); i++)
						if ((msg.tool() != null) && (msg.tool() instanceof MOB))
							CMLib.commands().postChannel(channels2.get(i),
									deadmob.clans(),
									deadmob.Name() + " was just killed.", true);
				}
			}
		...
		if (msg.tool() instanceof MOB) {
			...
			if (Log.killsChannelOn()) {
				...
				String room = CMLib.map().getExtendedRoomID(
						(killer.location() != null) ? killer.location()
								: deadmob.location());
				Log.killsOut("KILL",
						room + ":" + killer.Name() + ":"
								+ killer.phyStats().getCombatStats() + ":"
								+ killer.curState().getCombatStats() + ":"
								+ ((KI == null) ? "null" : KI.name()) + ":"
								+ deadmob.Name() + ":"
								+ deadmob.phyStats().getCombatStats() + ":"
								+ deadmob.curState().getCombatStats() + ":"
								+ ((DI == null) ? "null" : DI.name()));
			}
			if (Log.combatChannelOn()) {
				Item DI = deadmob.fetchWieldedItem();
				Item KI = killer.fetchWieldedItem();
			...
			if ((deadmob != null) && (killer != null) && (!deadmob.isMonster())
					&& (deadmob.soulMate() == null) && (killer != deadmob)
					&& (!killer.isMonster())) {
				CMLib.coffeeTables()
						.bump(deadmob, CoffeeTableRow.STAT_PKDEATHS);
				if ((deadmob.session() != null)
						&& (killer.session() != null)
						&& (!deadmob
								.session()
								.getAddress()
								.equalsIgnoreCase(killer.session().getAddress()))) {
					List<Pair<Clan, Integer>> list = CMLib.clans()
							.findRivalrousClans(killer, deadmob);
	...
	protected void pickNextVictim(MOB observer, MOB deadmob, Set<MOB> deadGroupH) {
		...
		if (R != null) {
			...
			for (int r = 0; r < R.numInhabitants(); r++) {
				...
				if ((M != observer) && (M != deadmob) && (M != null)
						&& (M.getVictim() == observer) && (!M.amDead())
			...
			if (newTargetM == null)
				for (int r = 0; r < R.numInhabitants(); r++) {
					...
					if ((M != observer)
							&& (M != deadmob)
							&& (deadGroupH.contains(M) || ((vic != null) && (myGroupH
									.contains(vic)))) && (!M.amDead())
							&& (CMLib.flags().isInTheGame(M, true))
							&& (observer.mayIFight(M))) {
						newTargetM = M;
						break;
	...
	public void handleBeingAssaulted(CMMsg msg) {
		...
		if ((!target.isInCombat())
				...
				&& ((!CMath.bset(msg.sourceMajor(), CMMsg.MASK_ALWAYS)) || (!(msg
						.tool() instanceof DiseaseAffect)))) {
		...
		if (target.isInCombat()) {
			...
			} else if ((msg.tool() != null) && (msg.tool() instanceof Item))
	...
	public void makeFollowersFight(MOB observer, MOB target, MOB source) {
		...
		if ((target == observer) || (source == observer))
			return;
		if ((target.location() != observer.location())
				|| (target.location() != source.location()))
			return;
		if ((CMath.bset(observer.getBitmap(), MOB.ATT_AUTOASSIST)))
			return;
		...
		} else if ((observerFollows == source)
	...
	public void establishRange(MOB source, MOB target, Environmental tool) {
		...
		if ((source.rangeToTarget() < 0)) {
			if (source.riding() != null) {
				if ((target == source.riding())
						|| (source.riding().amRiding(target)))
					source.setAtRange(0);
						...
						&& (((MOB) source.riding()).rangeToTarget() < source
								.rangeToTarget())) {
			...
			if ((target.getVictim() == source) && (target.rangeToTarget() >= 0))
				source.setAtRange(target.rangeToTarget());
					...
					&& (follow.location() == source.location())) {
				...
				if ((source.location() != null)
						&& (source.location().maxRange() < newRange))
	...
	protected void subtickAttack(MOB fighter, Item weapon, int folrange) {
		...
		if ((!CMath.bset(fighter.getBitmap(), MOB.ATT_AUTOMELEE)))
			...
			boolean inminrange = (fighter.rangeToTarget() >= fighter
					.minRange(weapon));
			boolean inmaxrange = (fighter.rangeToTarget() <= fighter
					.maxRange(weapon));
			if ((folrange >= 0) && (fighter.rangeToTarget() >= 0)
				...
				if (fighter.rangeToTarget() < folrange)
					...
					inmaxrange = false;
					// we advance
					CMMsg msg = CMClass.getMsg(fighter, fighter.getVictim(),
							CMMsg.MSG_ADVANCE,
							"<S-NAME> advances(s) at <T-NAMESELF>.");
					if (fighter.location().okMessage(fighter, msg)) {
						fighter.location().send(fighter, msg);
						fighter.setAtRange(fighter.rangeToTarget() - 1);
						if ((fighter.getVictim() != null)
								&& (fighter.getVictim().getVictim() == fighter)) {
							fighter.getVictim().setAtRange(
									fighter.rangeToTarget());
			...
			if ((!inminrange) && (fighter.curState().getMovement() >= 25)) {
				CMMsg msg = CMClass.getMsg(fighter, fighter.getVictim(),
						CMMsg.MSG_RETREAT,
						"<S-NAME> retreat(s) before <T-NAME>.");
				if (fighter.location().okMessage(fighter, msg))
					fighter.location().send(fighter, msg);
	...
	protected void subtickBeforeAttack(MOB fighter) {
		...
		while ((CMProps.getIntVar(CMProps.Int.COMBATSYSTEM) == CombatLibrary.COMBAT_QUEUE)
				&& (!fighter.amDead()) && (fighter.dequeCommand())) {
		}
	}
	...
	protected void subtickAfterAttack(MOB fighter) {
		...
		if ((target != null)
				...
				&& ((fighter.amFollowing() == null) || (fighter.amFollowing()
						.isMonster())) && (!target.amDead())
			...
			for (int m = 0; m < R.numInhabitants(); m++) {
				...
				if ((M != null)
						&& (!M.isMonster())
						&& (M.getVictim() == fighter)
						&& ((nextVictimM == null) || (M.rangeToTarget() < nextVictimM
								.rangeToTarget())))
					nextVictimM = M;
			}
	...
	public void dispenseExperience(Set<MOB> killers, Set<MOB> dividers,
			MOB killed) {
		int totalLevels = 0;
		int expAmount = 100;
		int expAddition = 25;
		...
		for (Iterator<MOB> i = killers.iterator(); i.hasNext();) {
			MOB mob = i.next();
			int myAmount = (int) Math
					.round(CMath.mul(
							expAmount,
							CMath.div(mob.phyStats().level()
									* mob.phyStats().level(), totalLevels)));
			if (myAmount > 100)
				myAmount = 100;
	...
	public void tickCombat(MOB fighter) {
		...
		if ((CMath.bset(fighter.getBitmap(), MOB.ATT_AUTODRAW))
				&& (weapon == null)) {
		...
		int folrange = (CMath.bset(fighter.getBitmap(), MOB.ATT_AUTOMELEE)
				...
				&& (fighter.amFollowing().rangeToTarget() >= 0) && (fighter
				.amFollowing().fetchFollowerOrder(fighter) >= 0)) ? fighter
				.amFollowing().fetchFollowerOrder(fighter)
				+ fighter.amFollowing().rangeToTarget() : -1;
		if (CMLib.flags().aliveAwakeMobile(fighter, true)) {
			if ((combatSystem != CombatLibrary.COMBAT_MANUAL)
					|| (fighter.isMonster())) {
				int numAttacks = (int) Math
						.round(Math.floor(fighter.actions())) - saveAction;
				if ((combatSystem == CombatLibrary.COMBAT_DEFAULT)
						&& (numAttacks > (int) Math.round(Math.floor(fighter
								.phyStats().speed() + 0.9))))
					numAttacks = (int) Math.round(Math.floor(fighter.phyStats()
							.speed() + 0.9));
			...
			if (CMLib.dice().rollPercentage() > (fighter.charStats().getStat(
					CharStats.STAT_CONSTITUTION) * 4))
	...
	public boolean isKnockedOutUponDeath(MOB mob, MOB fighting) {
		...
		for (int w = 0; w < whatsToDo.size(); w++) {
			...
			if (whatToDo.startsWith("OUT ")
					&& (CMath.isMathExpression(whatToDo.substring(4).trim(),
							fakeVarVals)))
				return true;
		}
		return false;
	...
	public boolean handleConsequences(MOB mob, MOB fighting, String[] commands,
			int[] lostExperience, String message) {
		...
		if (((!mob.isMonster()) && (mob.soulMate() == null)))
			rejuv = 1;
		double[] varVals = {
				mob.basePhyStats().level() > mob.phyStats().level() ? mob
						.basePhyStats().level() : mob.phyStats().level(),
				(fighting != null) ? fighting.phyStats().level() : 0, rejuv };
		for (int w = 0; w < commands.length; w++) {
			...
			if (whatToDo.startsWith("UNL")) {
			...
			} else if (whatToDo.startsWith("OUT ")
					&& (CMath.isMathExpression(whatToDo.substring(4).trim(),
							varVals))) {
				Ability A = CMClass.getAbility("Skill_ArrestingSap");
				int tickDown = CMath.s_parseIntExpression(whatToDo.substring(4)
						.trim(), varVals);
				if ((A != null) && (tickDown > 0)) {
					A.invoke(mob, new XVector<Object>("" + tickDown, "SAFELY"),
							mob, true, 0);
					mob.resetToMaxState();
				}
				...
				if (mob.numAbilities() > 0) {
					Ability A = mob.fetchAbility(CMLib.dice().roll(1,
							mob.numAbilities(), -1));
					if (A != null) {
				...
				if (lostExperience[0] > 0) {
					message = CMStrings.replaceAll(message, "@x1", ""
							+ lostExperience[0]);
					mob.tell(message);
					CMLib.leveler().postExperience(mob, null, null,
							-lostExperience[0], false);
				}
