...
public class WebMacroCreamer extends StdLibrary implements WebMacroLibrary,
	...
	@Override
	public ByteBuffer convertOutput(MiniWebConfig config, HTTPRequest request,
			HTTPStatus status, ByteBuffer buffer) throws HTTPException {
		if (request.getRequestObjects().get("SYSTEM_HTTP_STATUS") == null) {
			request.getRequestObjects().put("SYSTEM_HTTP_STATUS",
					Integer.toString(status.getStatusCode()));
			request.getRequestObjects().put("SYSTEM_HTTP_STATUS_INFO",
					status.description());
		}
		long[] systemStartTime = new long[] { System.currentTimeMillis() };
		try {
			return ByteBuffer.wrap(virtualPageFilter(request,
					request.getRequestObjects(), systemStartTime,
					new String[] { "" },
					new StringBuffer(new String(buffer.array()))).toString()
					.getBytes());
		} catch (HTTPRedirectException he) {
			throw new HTTPException(HTTPStatus.S307_TEMPORARY_REDIRECT,
					he.getMessage());
		}
	}
	...
	public byte[] virtualPageFilter(byte[] data) throws HTTPRedirectException {
		return virtualPageFilter(new StringBuffer(new String(data))).toString()
				.getBytes();
	}
	...
	@Override
	public StringBuffer virtualPageFilter(StringBuffer s)
			throws HTTPRedirectException {
		return virtualPageFilter(
				new HTTPRequest() {
				...
				new long[] { System.currentTimeMillis() }, new String[] { "" },
				s);
	}
	...
	public String parseFoundMacro(StringBuffer s, int i, boolean lookOnly) {
		...
		for (int x = i + 1; x < s.length(); x++) {
			if ((s.charAt(x) == '@') && (extend) && (x < (s.length() - 1))
					&& (s.charAt(x + 1) == '@')) {
				if (!lookOnly)
					s.deleteCharAt(x);
			...
			if (s.charAt(i) == '@') {
				...
				if ((foundMacro != null) && (foundMacro.length() > 0)) {
					if (foundMacro.equalsIgnoreCase("break"))
						...
						WebMacro W = CMClass.getWebMacro(foundMacro
								.toUpperCase());
						if (W != null)
	...
	public StringBuffer virtualPageFilter(HTTPRequest request,
			Map<String, Object> objects, long[] processStartTime,
			String[] lastFoundMacro, StringBuffer s)
			throws HTTPRedirectException {
		String redirectTo = null;
		boolean debugMacros = CMSecurity
				.isDebugging(CMSecurity.DbgFlag.HTTPMACROS);
		boolean isAdminServer = false;
		if (Thread.currentThread() instanceof MWThread)
			isAdminServer = CMath.s_bool(((MWThread) Thread.currentThread())
					.getConfig().getMiscProp("ADMIN"));
		if ((!isAdminServer)
				&& (processStartTime[0] > 0)
				&& (System.currentTimeMillis() - processStartTime[0]) > (120 * 1000)) {
			if (debugMacros)
				Log.infoOut(Thread.currentThread().getName(),
						"Encountered TIMEOUT!");
		...
		try {
			for (int i = 0; i < s.length(); i++) {
				if (s.charAt(i) == '@') {
					String foundMacro = parseFoundMacro(s, i, lastFoundMacro,
							false);
					if ((foundMacro != null) && (foundMacro.length() > 0)) {
						if (debugMacros)
							Log.debugOut("ProcessHTTPRequest", "Found macro: "
									+ foundMacro);
						if (foundMacro.startsWith("if?")
								|| foundMacro.startsWith("IF?")) {
							int l = foundMacro.length() + 2;
							int v = myEndif(s, i + l, lastFoundMacro);
							if (v < 0) {
								if (debugMacros)
									Log.debugOut("ProcessHTTPRequest",
											"if without endif");
								...
								try {
									...
									if (debugMacros)
										Log.debugOut("ProcessHTTPRequest",
												"Found IF macro: " + foundMacro);
									String q = runMacro(request, foundMacro,
											lastFoundMacro, isAdminServer);
									if (debugMacros)
										Log.debugOut("ProcessHTTPRequest",
												"Ran IF macro: " + foundMacro
														+ "=" + q);
									if ((q != null)
											&& (q.equalsIgnoreCase(compare))) {
										if (debugMacros)
											Log.debugOut("ProcessHTTPRequest",
													"Result IF macro: TRUE");
										if (v2 >= 0)
											s.replace(v2, v + 7, "");
										else
											s.replace(v, v + 7, "");
							...
							if (v < 0)
								s.replace(i, i + l,
										"[jscript without /jscript]");
							else {
								Context cx = Context.enter();
								try {
									String script = s.substring(i + l, v);
									JScriptablePage scope = new JScriptablePage(
											request);
									cx.initStandardObjects(scope);
									scope.defineFunctionProperties(
											JScriptablePage.functions,
											JScriptablePage.class,
											ScriptableObject.DONTENUM);
									cx.evaluateString(scope, script, "<cmd>",
											1, null);
									s.replace(i, v + l + 1, scope.getBuffer());
									i = i + scope.getBuffer().length();
								} catch (Exception e) {
									s.replace(i, v + l + 1, "[jscript error: "
											+ e.getMessage() + "]");
								}
								Context.exit();
							}
							continue;
						} else if (foundMacro.startsWith("block?")
							...
							if (v < 0)
								...
								String name = foundMacro.substring(6).trim()
										.toUpperCase();
								objects.put(name, s.substring(i + l, v));
								s.delete(i, v + 8);
							}
							continue;
						} else if (foundMacro.startsWith("insert?")
								|| foundMacro.startsWith("INSERT?")) {
							int l = foundMacro.length() + 2;
							String name = foundMacro.substring(7).trim()
									.toUpperCase();
							...
							if (v < 0)
								...
								while (s3.length() > 0) {
									try {
										s3 = new String(virtualPageFilter(
												request, objects,
												processStartTime,
												lastFoundMacro,
												new StringBuffer(s2)));
								...
								if ((!isAdminServer)
										...
										&& (System.currentTimeMillis() - processStartTime[0]) > (120 * 1000)) {
									if (debugMacros)
										Log.infoOut(Thread.currentThread()
												.getName(),
												"Encountered TIMEOUT!");
									return new StringBuffer("");
								}
							...
							continue;
						} else if (foundMacro.startsWith("for?")
								|| foundMacro.startsWith("FOR?")) {
							String forCond = foundMacro.substring(4);
							int fc = forCond.indexOf('=');
							int v = myNext(s, i + foundMacro.length() + 2,
									lastFoundMacro);
							if (fc < 0)
								s.replace(i, i + foundMacro.length() + 2,
										"[for without variabledef=]");
							else if (v < 0)
								s.replace(i, i + foundMacro.length() + 2,
										"[for without next]");
							else {
								String s2 = s.substring(i + foundMacro.length()
										+ 2, v);
								s.replace(i, v + 6, "");
								if (debugMacros)
									Log.debugOut("ProcessHTTPRequest",
											"Found FOR macro: " + foundMacro);
								String varName = forCond.substring(0, fc)
										.trim().toUpperCase();
								String q = runMacro(request,
										forCond.substring(fc + 1).trim(),
										lastFoundMacro, isAdminServer);
								if (q == null)
									q = forCond.substring(fc + 1).trim();
								if (debugMacros)
									Log.debugOut("ProcessHTTPRequest",
											"Ran FOR macro: " + foundMacro
													+ "=" + q);
								int ldex = i;
								String s3 = " ";
								List<String> set;
						...
						if (foundMacro.length() > 0) {
							try {
								...
								if (debugMacros)
									Log.debugOut("ProcessHTTPRequest",
											"Ran Macro: " + foundMacro + "="
													+ q);
								if (q != null) {
									if ((debugMacros)
											&& (q.toUpperCase().indexOf(
													"@BREAK@") >= 0))
										Log.infoOut(Thread.currentThread()
												.getName(), "WebMacro:"
												+ foundMacro
												+ " generated a BREAK! at " + i);
									s.replace(i, i + l + 2, q);
								} else
			...
			String errMsg = e.getMessage() == null ? e.toString() : e
					.getMessage();
			if ((errMsg != null)
					&& ((!Log.isMaskedErrMsg(errMsg)) || (CMSecurity
							.isDebugging(CMSecurity.DbgFlag.HTTPMACROS)))) {
				Log.errOut(Thread.currentThread().getName(),
						"Exception in virtualPageFilter() - " + errMsg);
	...
	protected String runMacro(HTTPRequest request, String foundMacro,
			final String[] lastFoundMacro, final boolean isAdminServer)
		...
		if (x >= 0) {
			...
			int y = parms.indexOf("@");
			while (y >= 0) {
				String newFoundMacro = parseFoundMacro(parms, y,
						lastFoundMacro, false);
				if ((newFoundMacro != null) && (newFoundMacro.length() > 0)) {
					int l = newFoundMacro.length();
					String qq = runMacro(request, newFoundMacro,
							lastFoundMacro, isAdminServer);
					if (qq != null)
		...
		if (W != null) {
			...
			if (!isAdminServer && W.isAdminMacro()) {
				Log.errOut(Thread.currentThread().getName(),
						"Non-admin cannot access admin macro '" + W.name()
								+ "'; client IP: " + request.getClientAddress());
				q = "[error]";
			} else if (W.preferBinary()) {
				byte[] bin = W.runBinaryMacro(request, (parms == null) ? null
						: parms.toString());
				if (bin == null)
					q = " @break@";
				else
				...
				q = W.runMacro(request,
						(parms == null) ? null : parms.toString());
			if (q != null)
				return q;
			return "[error]";
		}
	...
	protected String parseFoundMacro(StringBuffer s, int i,
			String[] lastFoundMacro, boolean lookOnly) {
		String foundMacro = null;
		boolean extend = false;
		if ((i < s.length() - 2)
	...
	@Override
	public void doGet(SimpleServletRequest request,
			SimpleServletResponse response) {
		...
		if (url.length > 0) {
			...
			if (W == null) {
				...
				Log.errOut("WebMacroCreamer", "Macro " + macroName
						+ " is not a web path.");
			} else {
					...
					try {
						...
						try {
							response.getOutputStream()
									.write(HTTPException
											.standardException(
													HTTPStatus.S500_INTERNAL_ERROR)
	...
	@Override
	public void doPost(SimpleServletRequest request,
			SimpleServletResponse response) {
		doGet(request, response);
	}

	@Override
	public void service(HTTPMethod method, SimpleServletRequest request,
			SimpleServletResponse response) {
	}

	public boolean activate() {
		String flag = Resources.getPropResource("WEBMACROCREAMER",
				"RUNYAHOOMSGGRABBER");
		if (flag.equalsIgnoreCase("TRUE")) {
			int tickCount = CMath.s_int(Resources.getPropResource(
					"WEBMACROCREAMER", "YAHOOTICKSPERRUN"));
			if (tickCount < 2)
				tickCount = 2;
			name = "THCreamer"
	...
	public boolean tick(Tickable ticking, int tickID) {
		try {
			...
			String timesPerRunStr = Resources.getPropResource(
					"WEBMACROCREAMER", "TIMESPERRUN");
			if (!CMath.isInteger(timesPerRunStr)) {
				Log.errOut("WebMacroCreamer var TIMESPERRUN not properly set.");
				return true;
			}
			if (yahooSession == null) {
				String yahooUsername = Resources.getPropResource(
						"WEBMACROCREAMER", "YAHOOUSER");
				if (yahooUsername.length() == 0) {
					Log.errOut("WebMacroCreamer var YAHOOUSER not properly set.");
					return true;
				}
				String yahooPassword = Resources.getPropResource(
						"WEBMACROCREAMER", "YAHOOPASSWORD");
				if (yahooUsername.length() == 0) {
					Log.errOut("WebMacroCreamer var YAHOOPASSWORD not properly set.");
					return true;
				}
				String yahooUrl = Resources.getPropResource("WEBMACROCREAMER",
						"YAHOOURL");
				if (yahooUrl.length() == 0) {
					Log.errOut("WebMacroCreamer var YAHOOURL not properly set.");
					return true;
				}
				String forumName = Resources.getPropResource("WEBMACROCREAMER",
						"FORUM");
				if (forumName.length() == 0) {
					Log.errOut("WebMacroCreamer var FORUM not properly set.");
					return true;
				}
				String skipList = Resources.getPropResource("WEBMACROCREAMER",
						"SKIPLIST");
				int[] skipInts = CMParms.toIntArray(CMParms.parseCommas(
						skipList, true));
			...
			if (yahooSession.numTotalTimes <= 0) {
				...
				yahooSession.H = (HttpClient) CMClass
						.getCommon("DefaultHttpClient");
				try {
					String resp = this.loginToYahooSession(yahooSession);
					if (resp.length() > 0) {
						Log.warnOut("Yahoo Groups Copier failure reported: "
								+ resp);
						yahooSession.H.finished();
						yahooSession.H = null;
						return true;
			...
			if ((resp != null) && (resp.toLowerCase().startsWith("fail"))) {
				...
				Log.warnOut("Yahoo Groups Copier recovered on message "
						+ yahooSession.lastMsgNum);
			Resources.setPropResource("WEBMACROCREAMER", "LASTYAHOOMSGNUMBER",
					Integer.toString(yahooSession.lastMsgNum));
		} finally {
	...
	public String copyYahooGroupMsg(YahooGroupSession sess) {
		while ((--sess.numTimes) >= 0) {
			...
			if (sess.lastMsgNum > sess.numTotal) {
				...
				return sess.lastMsgNum + "of " + sess.numTotal
						+ " messages already processed";
			}
			if (Arrays.binarySearch(sess.skipList, sess.lastMsgNum) >= 0)
				continue;
			byte[] b = sess.H.getRawUrl(sess.url + "/message/"
					+ sess.lastMsgNum, sess.cookieSet);
			if (b == null)
				return "Failed: to read message from url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			...
			if (startOfSubject < 0) {
				...
				if ((x > 0)
						&& (msgPage.substring(0, x).trim()
								.endsWith("<div class=\"ygrp-contentblock\">"))) {
					Resources.setPropResource("WEBMACROCREAMER",
							"LASTYAHOOMSGNUMBER",
							Integer.toString(sess.lastMsgNum));
					continue;
				}
				return "Failed: to find subject start in url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			}
			startOfSubject = msgPage.indexOf(">", startOfSubject);
			int endOfSubject = msgPage.indexOf("</em>", startOfSubject);
			if (endOfSubject < 0)
				return "Failed: to find subject end in url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			String subject = msgPage
					.substring(startOfSubject + 1, endOfSubject).trim();
			if ((subject.length() == 0) || (subject.length() > 200)
					|| (subject.indexOf('<') >= 0)
					|| (subject.indexOf('>') >= 0))
				return "Failed: to find VALID subject '" + subject
						+ "' in url:" + sess.url + "/message/"
						+ sess.lastMsgNum;
			int startOfDate = msgPage
					.indexOf("<span class=\"msg-newfont\" title=\"");
			int endOfDate;
			if (startOfDate > 0)
				...
				if (startOfDate < 0)
					return "Failed: to find date start in url:" + sess.url
							+ "/message/" + sess.lastMsgNum;
				startOfDate += 33;// MAGIC NUMBER
			}
			endOfDate = msgPage.indexOf("\"", startOfDate + 1);
			if (endOfDate < 0)
				return "Failed: to find date end in url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			...
			try {
				...
				return "Failed: to parse date '" + dateStr + "' in url:"
						+ sess.url + "/message/" + sess.lastMsgNum;
			}
			int startOfAuthor = msgPage.indexOf("<span class=\"name\">");
			if (startOfAuthor < 0)
				return "Failed: to find author start in url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			startOfAuthor = msgPage.indexOf(">", startOfAuthor + 4);
			int endOfAuthor = msgPage.indexOf("</span>", startOfAuthor);
			if (endOfAuthor < 0)
				return "Failed: to find author end in url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			String author = msgPage.substring(startOfAuthor + 1, endOfAuthor)
					.trim();
			...
			if (author.indexOf("profiles.yahoo.com") > 0)
				author = author.substring(author.indexOf("\">") + 2,
						author.lastIndexOf("</a>"));
			if ((author.length() == 0) || (author.length() > 100))
				return "Failed: to find VALID author '" + author + "' in url:"
						+ sess.url + "/message/" + sess.lastMsgNum;
			int startOfMsg = msgPage.indexOf("entry-content");
			if (startOfMsg < 0)
				return "Failed: to find message in url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			startOfMsg = msgPage.indexOf(">", startOfMsg);
			int endOfMsg = msgPage.indexOf("<tr style=\"height:35px\">",
					startOfMsg);
			if (endOfMsg < 0)
				return "Failed: to find end of msg in url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			endOfMsg = msgPage.lastIndexOf("</div>", endOfMsg);
			if (endOfMsg < 0)
				return "Failed: to find end2 of msg in url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			String theMessage = msgPage.substring(startOfMsg + 1, endOfMsg)
					.trim();
			while (theMessage.startsWith("<br>"))
				theMessage = theMessage.substring(4).trim();
			while (theMessage.endsWith("<br>"))
				theMessage = theMessage.substring(0, theMessage.length() - 4)
						.trim();
			theMessage = CMStrings.replaceAll(theMessage, "\n", "");
			theMessage = CMStrings.replaceAll(theMessage, "\r", "");
			if (theMessage.trim().length() == 0)
				return "Failed: to find lengthy msg in url:" + sess.url
						+ "/message/" + sess.lastMsgNum;
			JournalsLibrary.ForumJournal forum = CMLib.journals()
					.getForumJournal(sess.journal);
			if (forum == null)
			...
			if (author.indexOf('@') >= 0) {
				...
				if (aM != null)
					author = aM.Name();
				else if (CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1) {
					PlayerAccount A = CMLib.players().getLoadAccountByEmail(
							author);
					if (A == null)
			...
			if (subject.toUpperCase().startsWith("RE:")) {
				String subj = subject;
				while (subj.toUpperCase().startsWith("RE:")
						|| subj.toLowerCase().startsWith("[coffeemud]")) {
				...
				Vector<JournalEntry> journalEntries = CMLib.database()
						.DBSearchAllJournalEntries(forum.NAME(), subj);
				if ((journalEntries != null) && (journalEntries.size() > 0)) {
					...
					if (WIN == null)
						for (JournalEntry J : journalEntries) {
							if (J.subj.toLowerCase().trim()
									.indexOf(subj.toLowerCase()) >= 0)
								WIN = J;
						}
			...
			msg.to = "ALL";
			// check for dups
			Vector<JournalsLibrary.JournalEntry> chckEntries = CMLib.database()
					.DBReadJournalMsgsNewerThan(forum.NAME(), "ALL",
							msg.date - 1);
			for (JournalsLibrary.JournalEntry entry : chckEntries)
				if ((entry.date == msg.date) && (entry.from.equals(msg.from))
	...
	protected String loginToYahooSession(YahooGroupSession sess)
			throws UnsupportedEncodingException {
		final String loginUrl = "http://login.yahoo.com?login="
				+ URLEncoder.encode(sess.user, "UTF8") + "&passwd="
				+ URLEncoder.encode(sess.password, "UTF8");
		...
		for (String val : cookies) {
			...
			int x = val.indexOf(';');
			cookieSet
					.append((x >= 0) ? val.substring(0, x).trim() : val.trim());
		}
		byte[] b = sess.H.getRawUrl(sess.url + "/messages",
				cookieSet.toString());
		if (b == null)
		...
		if (cookies != null)
			for (String val : cookies) {
				if (cookieSet.length() > 0)
					cookieSet.append(" ; ");
				int x = val.indexOf(';');
				cookieSet.append((x >= 0) ? val.substring(0, x).trim() : val
						.trim());
			}
		...
		if (Resources.isPropResource("WEBMACROCREAMER", "LASTYAHOOMSGNUMBER"))
			sess.lastMsgNum = CMath.s_int(Resources.getPropResource(
					"WEBMACROCREAMER", "LASTYAHOOMSGNUMBER"));
		return "";
	}

	@Override
	public String copyYahooGroupMsgs(String user, String password, String url,
			int numTimes, int[] skipList, String journal) {
