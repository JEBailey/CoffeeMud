...
public interface ExpertiseLibrary extends CMLibrary {
	...
	public static final String[] XFLAG_CODES = { "X1", "X2", "X3", "X4", "X5",
			"LEVEL", "TIME", "MAXRANGE", "LOWCOST", "XPCOST" };
	...
	public static class ExpertiseDefinition implements CMObject {
		...
		public MaskingLibrary.CompiledZapperMask compiledListMask() {
			if ((this.compiledListMask == null)
					&& (uncompiledListMask.length() > 0)) {
				compiledListMask = CMLib.masking().maskCompile(
						uncompiledListMask);
				CMLib.ableMapper().addPreRequisites(ID, new Vector<String>(),
						uncompiledListMask.trim());
			}
			return this.compiledListMask;
		}

		public MaskingLibrary.CompiledZapperMask compiledFinalMask() {
			if ((this.compiledFinalMask == null)
					&& (uncompiledFinalMask.length() > 0)) {
				this.compiledFinalMask = CMLib.masking().maskCompile(
						uncompiledFinalMask);
				CMLib.ableMapper().addPreRequisites(ID, new Vector<String>(),
						uncompiledFinalMask.trim());
			}
		...
		public void addFinalMask(String mask) {
			...
			if (uncompiledFinalMask == null)
				...
				uncompiledFinalMask += mask;
			compiledFinalMask = CMLib.masking()
					.maskCompile(uncompiledFinalMask);
			CMLib.ableMapper().addPreRequisites(ID, new Vector<String>(),
					uncompiledFinalMask.trim());
		}
	...
	public class SkillCostDefinition {
		...
		public SkillCostDefinition(final CostType type,
				final String costDefinition) {
			this.type = type;
	...
	public class SkillCost {
		...
		/**
		 * Returns a simple description of the Type of this cost. A MOB and
		 * sample value is required for money currencies.
		 * 
		 * @param mob
		...
		public String costType(final MOB mob) {
			...
			case XP:
				...
				break;
			case GOLD:
				ofWhat = CMLib.beanCounter().getDenominationName(mob,
						value.doubleValue());
				break;
			case PRACTICE:
			...
			case QP:
				...
				break;
			default:
				ofWhat = CMLib.english().makePlural(
		...
		public String requirements(final MOB mob) {
			...
			case GOLD:
				if (mob == null)
					return CMLib.beanCounter().abbreviatedPrice("",
							value.doubleValue());
				else
					return CMLib.beanCounter().abbreviatedPrice(mob,
							value.doubleValue());
			default:
				return value.intValue()
						+ " "
						+ ((value.intValue() == 1) ? costType.name()
								.toLowerCase() : CMLib.english().makePlural(
								costType.name().toLowerCase()));
			}
		}
		...
		public void spendSkillCost(final MOB student) {
			switch (costType) {
			case XP:
				CMLib.leveler().postExperience(student, null, "",
						value.intValue(), true);
				break;
			case GOLD:
			...
			case PRACTICE:
				...
				break;
			case QP:
				student.setQuestPoint(student.getQuestPoint()
						- value.intValue());
				break;
			}
		}
	...
	public ExpertiseDefinition addDefinition(String ID, String name,
			String baseName, String listMask, String finalMask, String[] costs,
			String[] data);
