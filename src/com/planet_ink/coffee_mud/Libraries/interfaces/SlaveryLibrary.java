...
public interface SlaveryLibrary extends CMLibrary {
	...
	public static class geasSteps extends Vector<geasStep> {
		...
		public void move(int moveCode) {
			...
			if (moveCode == 0) {
				if (!CMLib.tracking().beMobile(me, true, true, false, true,
						null, botheredPlaces))
					CMLib.tracking().beMobile(me, true, true, false, false,
							null, null);
			} else {
				if (!CMLib.tracking().beMobile(me, true, true, true, false,
						null, botheredPlaces))
					CMLib.tracking().beMobile(me, true, true, false, false,
							null, null);
			}
	...
	public static class geasStep {
		...
		public boolean sayResponse(MOB speaker, MOB target, String response) {
			MOB me = mySteps.me;
			if ((speaker != null) && (speaker != me) && (bothering != null)
					&& (speaker == bothering) && (step != STEP_EVAL)
					&& ((target == null) || (target == me))) {
				for (int s = 0; s < universalRejections.length; s++) {
					if (CMLib.english().containsString(response,
							universalRejections[s])) {
						CMLib.commands().postSay(me, speaker,
								"Ok, thanks anyway.", false, false);
						return true;
					}
				}
				...
				for (int s = 0; s < responseStarters.length; s++) {
					if (response.startsWith(responseStarters[s])) {
						...
						response = response.substring(
								responseStarters[s].length()).trim();
					}
				}
				if ((!starterFound) && (speaker.isMonster())
		...
		public String step() {
			...
			if (s.equalsIgnoreCase("itemfind")) {
				...
				if ((CMath.isNumber(item) && (CMath.s_int(item) > 0))) {
					if (CMLib.beanCounter().getTotalAbsoluteNativeValue(me) >= (CMath
							.s_int(item))) {
						step = STEP_EVAL;
						que.remove(0);
						CMLib.commands().postSay(me, null, "I got the money!",
								false, false);
						return "HOLD";
					}
					item = "coins";
				...
				if ((I != null) && (CMLib.flags().canBeSeenBy(I, me))) {
					...
					que.remove(0);
					CMLib.commands().postSay(me, null,
							"I got " + I.name(me) + "!", false, false);
					return "HOLD";
				}
				...
				for (int m = 0; m < R.numInhabitants(); m++) {
					...
					if ((M != null) && (M != me)
						...
						if ((I != null)
							...
							if (step == STEP_EVAL) {
								CMLib.commands().postSay(
										me,
										M,
										"I must have '" + I.name(me)
												+ ".  Give it to me now.",
										false, false);
								step = STEP_INT1;
								return "HOLD";
							} else if (step == STEP_INT1) {
								step = STEP_INT2;
								return "HOLD";
							} else if (step == STEP_INT2) {
								CMLib.commands().postSay(
										me,
										M,
										"I MUST HAVE '"
												+ I.name(me).toUpperCase()
												+ ".  GIVE IT TO ME NOW!!!!",
										false, false);
								step = STEP_INT3;
								return "HOLD";
							} else if (step == STEP_INT3) {
								step = STEP_INT4;
								return "HOLD";
							} else if (step == STEP_INT4) {
								CMLib.combat().postAttack(me, M,
										me.fetchWieldedItem());
								step = STEP_EVAL;
								return "HOLD";
							}
						...
						if ((!item.equals("coins")) && (sk != null)
								&& (sk.getShop().getStock(item, me) != null)) {
							Environmental E = sk.getShop().getStock(item, me);
							if ((E != null) && (E instanceof Item)) {
								double price = CMLib.coffeeShops()
										.sellingPrice(M, me, E, sk, true).absoluteGoldPrice;
								if (price <= CMLib
										.beanCounter()
										.getTotalAbsoluteShopKeepersValue(me, M)) {
									me.enqueCommand(
											CMParms.parse("BUY \"" + E.name()
													+ "\""),
								...
								price = price
										- CMLib.beanCounter()
												.getTotalAbsoluteShopKeepersValue(
														me, M);
								que.add(0, CMParms.parse("itemfind "
										+ CMLib.beanCounter()
												.nameCurrencyShort(M, price)));
								CMLib.commands().postSay(
										me,
										null,
										"Damn, I need "
				...
				if ((bothering != null) && (step > STEP_EVAL)
						&& (step <= STEP_INT4) && (!bothering.isMonster())) {
					step++;
					return "HOLD";
				}
				step = STEP_EVAL;
				return "0Can you tell me where to find "
						+ CMParms.combine(cur, 1) + "?";
				...
				if ((bothering != null) && (step > STEP_EVAL)
						&& (step <= STEP_INT4) && (!bothering.isMonster())) {
					if (CMSecurity.isDebugging(CMSecurity.DbgFlag.GEAS))
						Log.debugOut("GEAS", "MOBFIND-RESPONSEWAIT: "
								+ bothering.name());
					step++;
					return "HOLD";
				}
				...
				int dirCode = Directions.getGoodDirectionCode(CMParms.parse(
						name).firstElement());
				...
				if (CMLib.english().containsString(R.name(), name)
						|| CMLib.english()
								.containsString(R.displayText(), name)
						|| CMLib.english()
								.containsString(R.description(), name)) {
					step = STEP_EVAL;
					que.remove(0);
					return "HOLD";
				}
				...
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.GEAS))
					Log.debugOut("GEAS",
							"WANDERQUERY: " + CMParms.combine(cur, 1));
				// if asked someone something, give them time to respond.
				if ((bothering != null) && (step > STEP_EVAL)
						&& (step <= STEP_INT4) && (!bothering.isMonster())) {
					step++;
				...
				step = STEP_EVAL;
				que.remove(0);
				me.enqueCommand(cur, Command.METAFLAG_FORCED
						| Command.METAFLAG_ORDER, 0);
				return "HOLD";
			}
		}
	...
	public static final String[] universalStarters = { "go ", "go and ",
			"i want you to ", "i command you to ", "i order you to ",
			"you are commanded to ", "please ", "to ", "you are ordered to " };

	public static final String[] responseStarters = { "try at the",
			"its at the", "it`s at the", "over at the", "at the", "go to",
			"go to the", "find the", "go ", "try to the", "over to the",
			"you`ll have to go", "you`ll have to find the",
			"you`ll have to find", "look at the", "look at", "look to",
			"look to the", "look for", "look for the", "search at the",
			"search at", "search to the", "look", "i saw one at", "he`s",
			"it`s", "she`s", "hes", "shes", "its", };
	public static String[] universalRejections = { "dunno", "nope", "not",
			"never", "nowhere", "noone", "can`t", "cant", "don`t", "dont", "no" };
	...
	public static final String[][] pmap = {
			...
			// DROWN, DROWN YOURSELF, DROWN IN A LAKE, SWIM, SWIM AN OCEAN,
			// CLIMB A MOUNTAIN, CLIMB A TREE, CLIMB <X>, SWIM <x>, HANG
			// YOURSELF, CRAWL <x>
			// BLOW YOUR NOSE, VOMIT, PUKE, THROW UP, KISS MY ASS, KISS <CHAR>
			// <Body part>
			...
			// below are gos, and find someone (and report back where), take me
			// to, show me
			{ "go %d", "%d;" },
			{ "walk %d", "%d;" },
			{ "move %d", "%d;" },
			...
			{ "where %*",
					"say You want me to answer where? I don't know where!" },
