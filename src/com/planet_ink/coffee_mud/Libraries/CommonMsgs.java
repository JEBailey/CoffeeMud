...
public class CommonMsgs extends StdLibrary implements CommonCommands {
	...
	protected final List<WeakReference<MsgMonitor>> globalMonitors = new SLinkedList<WeakReference<MsgMonitor>>();

	protected String unknownCommand() {
		return "Huh?";
	}

	protected String unknownInvoke() {
		return "You don't know how to @x1 that.";
	}
	...
	public boolean handleUnknownCommand(MOB mob, List<String> command) {
		...
		if (command.size() > 0) {
			...
			if (word != null)
				msgStr = CMStrings.replaceAll(unknownInvoke(), "@x1",
						word.toLowerCase());
		}
		CMMsg msg = CMClass.getMsg(mob, null, null, CMMsg.MSG_HUH, msgStr,
				CMParms.combine(command, 0), null);
	...
	public Object unforcedInternalCommand(MOB mob, String command,
			Object... parms) {
		try {
	...
	public boolean forceStandardCommand(MOB mob, String command,
			Vector<Object> parms) {
		try {
			Command C = CMClass.getCommand(command);
			if (C != null)
	...
	public void monitorGlobalMessage(Room room, CMMsg msg) {
		MsgMonitor M;
		for (final Iterator<WeakReference<MsgMonitor>> w = globalMonitors
				.iterator(); w.hasNext();) {
	...
	public void postChannel(MOB mob, String channelName, String message,
			boolean systemMsg) {
		forceInternalCommand(mob, "Channel", Boolean.valueOf(systemMsg),
				channelName, message);
	}
	...
	public void postChannel(String channelName,
			Iterable<Pair<Clan, Integer>> clanList, String message,
			boolean systemMsg) {
		MOB talker;
		if (clanList != null) {
	...
	public boolean postDrop(MOB mob, Environmental dropThis, boolean quiet,
			boolean optimized, boolean intermediate) {
		return ((Boolean) forceInternalCommand(mob, "Drop", dropThis,
				Boolean.valueOf(quiet), Boolean.valueOf(optimized),
				Boolean.valueOf(intermediate))).booleanValue();
	}

	public boolean postOpen(MOB mob, Environmental openThis, boolean quiet) {
		return ((Boolean) forceInternalCommand(mob, "Open", openThis,
				Boolean.valueOf(quiet))).booleanValue();
	}

	public boolean postGet(MOB mob, Item container, Item getThis, boolean quiet) {
		if (container == null)
			return forceStandardCommand(mob, "Get", new XVector<Object>(
					getThis, Boolean.valueOf(quiet)));
		return forceStandardCommand(mob, "Get", new XVector<Object>(getThis,
				container, Boolean.valueOf(quiet)));
	}

	public boolean postRemove(MOB mob, Item item, boolean quiet) {
		if (quiet)
			return forceStandardCommand(mob, "Remove", new XVector<Object>(
					"REMOVE", item, "QUIETLY"));
		return forceStandardCommand(mob, "Remove", new XVector<Object>(
				"REMOVE", item));
	}
	...
	public void postRead(MOB mob, Physical target, String readOff, boolean quiet) {
		forceInternalCommand(mob, "Read", target, readOff,
				Boolean.valueOf(quiet));
	}
	...
	public void postFollow(MOB follower, MOB leader, boolean quiet) {
		if (leader != null) {
			if (quiet)
				forceStandardCommand(follower, "Follow", new XVector<Object>(
						"FOLLOW", leader, "UNOBTRUSIVELY"));
			else
				forceStandardCommand(follower, "Follow", new XVector<Object>(
						"FOLLOW", leader));
		} else {
		...
		if ((isPrivate) && (target != null)) {
			if (tellFlag) {
				...
				if (targetName.indexOf('@') >= 0) {
					String mudName = targetName.substring(targetName
							.indexOf('@') + 1);
					...
					boolean ignore = ((target.playerStats() != null) && (target
							.playerStats().getIgnored().contains(mob.Name())));
					CMMsg msg = null;
					if ((!CMLib.flags().isSeen(mob))
							|| (!CMLib.flags().isSeen(target)))
						msg = CMClass.getMsg(
								mob,
								target,
								null,
								CMMsg.MSG_TELL,
								"^t^<TELL \""
										+ CMStrings.removeColors(target
												.name(mob))
										+ "\"^>You tell <T-NAME> '" + text
										+ "'^</TELL^>^?^.",
								CMMsg.MSG_TELL,
								"^t^<TELL \""
										+ CMStrings.removeColors(mob
												.name(target))
										+ "\"^><S-NAME> tell(s) you '" + text
										+ "'^</TELL^>^?^.", CMMsg.NO_EFFECT,
								null);
					else
						msg = CMClass.getMsg(
								mob,
								...
								"^t^<TELL \""
										+ CMStrings.removeColors(target
												.name(mob)) + "\"^>You tell "
										+ target.name(mob) + " '" + text
										+ "'^</TELL^>^?^.",
								CMMsg.MSG_TELL,
								"^t^<TELL \""
										+ CMStrings.removeColors(mob
												.name(target)) + "\"^>"
										+ mob.Name() + " tell(s) you '" + text
										+ "'^</TELL^>^?^.", CMMsg.NO_EFFECT,
								null);
					if ((mob.location().okMessage(mob, msg))
						...
						if ((mob.session() != null)
								&& (mob.session()
										.getClientTelnetMode(Session.TELNET_GMCP))) {
							mob.session()
									.sendGMCPEvent(
											"comm.channel",
											"{\"chan\":\"tell\",\"msg\":\""
							...
							if ((!mob.isMonster()) && (!target.isMonster())) {
								...
								if (target.playerStats() != null) {
									...
									if ((msg.tool() instanceof Ability)
											&& ((((Ability) msg.tool())
													.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_LANGUAGE)
											&& (target.fetchEffect(msg.tool()
													.ID()) != null))
										str = CMStrings.substituteSayInMessage(
												str,
												CMStrings.getSayFromMessage(msg
														.sourceMessage()));
									target.playerStats()
											.addTellStack(
													CMLib.coffeeFilter()
				...
				CMMsg msg = CMClass.getMsg(mob, target, null, CMMsg.MSG_SPEAK,
						"^T^<SAY \"" + CMStrings.removeColors(target.name(mob))
								+ "\"^><S-NAME> say(s) '" + text
								+ "' to <T-NAMESELF>.^</SAY^>^?",
						CMMsg.MSG_SPEAK,
						"^T^<SAY \"" + CMStrings.removeColors(mob.name(target))
								+ "\"^><S-NAME> say(s) '" + text
								+ "' to <T-NAMESELF>.^</SAY^>^?",
			...
			CMMsg msg = CMClass.getMsg(
					...
					CMMsg.MSG_SPEAK,
					"^T^<SAY \""
							+ CMStrings.removeColors((target == null) ? mob
									.name(target) : target.name(mob)) + "\"^>"
							+ str,
					"^T^<SAY \"" + CMStrings.removeColors(mob.name(target))
							+ "\"^>" + str,
					"^T^<SAY \"" + CMStrings.removeColors(mob.name(target))
	...
	protected void gmcpSaySend(String sayName, MOB mob, MOB target, CMMsg msg) {
		if ((mob.session() != null)
			...
			mob.session().sendGMCPEvent(
					"comm.channel",
					"{\"chan\":\""
							+ sayName
							+ "\",\"msg\":\""
							+ MiniJSON.toJSONString(CMLib.coffeeFilter()
									.fullOutFilter(
											null,
											mob,
											mob,
		...
		if (R != null)
			for (int i = 0; i < R.numInhabitants(); i++) {
				...
				if ((M != null)
						...
						&& (M.session()
								.getClientTelnetMode(Session.TELNET_GMCP))) {
					M.session().sendGMCPEvent(
							"comm.channel",
							"{\"chan\":\""
									+ sayName
									+ "\",\"msg\":\""
									+ MiniJSON.toJSONString(CMLib
											.coffeeFilter().fullOutFilter(
													null,
													M,
													mob,
													target,
	...
	public void handleBeingMobSniffed(CMMsg msg) {
		...
		if ((sniffedmob.playerStats() != null)
			...
			if (x <= 1)
				sniffingmob.tell(sniffedmob.name(sniffingmob)
						+ " has a slight aroma about "
						+ sniffedmob.charStats().himher() + ".");
			else if (x <= 3)
				sniffingmob.tell(sniffedmob.name(sniffingmob)
						+ " smells pretty sweaty.");
			else if (x <= 7)
				sniffingmob.tell(sniffedmob.name(sniffingmob)
						+ " stinks pretty bad.");
			else if (x < 15)
	...
	public void handleSit(CMMsg msg) {
		...
		oldDisposition = oldDisposition
				& (~(PhyStats.IS_SLEEPING | PhyStats.IS_SNEAKING | PhyStats.IS_SITTING));
		sittingmob.basePhyStats().setDisposition(
				oldDisposition | PhyStats.IS_SITTING);
		sittingmob.recoverPhyStats();
		sittingmob.recoverCharStats();
		sittingmob.recoverMaxState();
		sittingmob.tell(sittingmob, msg.target(), msg.tool(),
				msg.sourceMessage());
	}
	...
	public void handleSleep(CMMsg msg) {
		...
		oldDisposition = oldDisposition
				& (~(PhyStats.IS_SLEEPING | PhyStats.IS_SNEAKING | PhyStats.IS_SITTING));
		sleepingmob.basePhyStats().setDisposition(
				oldDisposition | PhyStats.IS_SLEEPING);
		sleepingmob.recoverPhyStats();
		sleepingmob.recoverCharStats();
		sleepingmob.recoverMaxState();
		sleepingmob.tell(sleepingmob, msg.target(), msg.tool(),
				msg.sourceMessage());
	}
	...
	public void handleStand(CMMsg msg) {
		...
		standingmob.tell(standingmob, msg.target(), msg.tool(),
				msg.sourceMessage());
	}
	...
	public int tickManaConsumption(MOB mob, int manaConsumeCounter) {
		if ((CMProps.getIntVar(CMProps.Int.MANACONSUMETIME) > 0)
			...
			for (final Enumeration<Ability> a = mob.effects(); a
					.hasMoreElements();) {
				...
				if (A != null) {
					if ((!A.isAutoInvoked())
							...
							&& (((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_SPELL)
									...
									|| ((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_SONG) || ((A
									.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_PRAYER))) {
						final Ability myA = mob.fetchAbility(A.ID());
						if (((myA != null) && (myA.usageCost(mob, false)[0] > 0))
			...
			if (expenseAffects.size() > 0) {
				...
				case -200: {
					...
					for (int a1 = 0; a1 < expenseAffects.size(); a1++) {
						int lql = CMLib.ableMapper().lowestQualifyingLevel(
								expenseAffects.elementAt(a1).ID());
						if (lql > 0)
							total += lql;
						else
							total += 1;
				...
				default:
					basePrice = basePrice
							* CMProps.getIntVar(CMProps.Int.MANACONSUMEAMT);
					break;
				}

				// 1 per tick per level per msg. +1 to the affects so that way
				// it's about
				// 3 cost = 1 regen... :)
				...
				while (mob.curState().getMana() < reallyEat) {
					mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
							"<S-YOUPOSS> strength of will begins to crumble.");
					// pick one and kill it
					Ability A = expenseAffects.elementAt(CMLib.dice().roll(1,
							expenseAffects.size(), -1));
					A.unInvoke();
	...
	public void tickAging(MOB mob, long millisSinceLast) {
		if (mob == null)
			return;
		mob.setAgeMinutes(mob.getAgeMinutes() + (millisSinceLast / 60000)); // this
																			// is
																			// really
																			// minutes
				...
				if ((mob.baseCharStats().ageCategory() >= Race.AGE_VENERABLE)
					...
					for (int i : CharStats.CODES.MAX())
						if ((max + mob.charStats().getStat(i)) <= 0) {
							mob.tell("Your max "
									+ CharStats.CODES.DESC(
											CharStats.CODES.toMAXBASE(i))
											.toLowerCase()
									+ " has fallen below 1!");
	...
	protected String relativeCharStatTest(CharStats C, MOB mob,
			String weakword, String strongword, int stat) {
		double d = CMath.div(C.getStat(stat), mob.charStats().getStat(stat));
		String prepend = "";
		if ((d <= 0.5) || (d >= 3.0))
			prepend = "much ";
		if (d >= 1.6)
			return mob.charStats().HeShe() + " appears " + prepend + weakword
					+ " than the average " + mob.charStats().raceName()
					+ ".\n\r";
		if (d <= 0.67)
			return mob.charStats().HeShe() + " appears " + prepend + strongword
					+ " than the average " + mob.charStats().raceName()
					+ ".\n\r";
		return "";
	}
	...
	public String examineItemString(MOB mob, Item item) {
		...
		String level = null;
		if ((mob != null)
				&& (mob.charStats().getStat(CharStats.STAT_INTELLIGENCE) < 10)) {
			int l = (int) Math.round(Math.floor(CMath.div(item.phyStats()
					.level(), 10.0)));
			level = (l * 10) + "-" + ((l * 10) + 9);
		} else if ((mob != null)
				&& (mob.charStats().getStat(CharStats.STAT_INTELLIGENCE) < 18)) {
			int l = (int) Math.round(Math.floor(CMath.div(item.phyStats()
					.level(), 5.0)));
			level = (l * 5) + "-" + ((l * 5) + 4);
		} else
		...
		if ((mob != null)
			...
			weight = (int) Math.round(CMath.mul(l, divider)) + "-"
					+ (int) Math.round(CMath.mul(l, divider) + (divider - 1.0));
		} else if ((mob != null)
				&& (mob.charStats().getStat(CharStats.STAT_INTELLIGENCE) < 18)) {
			divider = divider / 2.0;
			double l = Math.floor(CMath.div(item.phyStats().level(), divider));
		...
		if (item instanceof CagedAnimal) {
			...
			if (M == null)
				...
				if (M.phyStats().height() > 0)
					response.append("\n\r"
							+ CMStrings.capitalizeFirstLetter(item.name())
							+ " is " + M.phyStats().height()
							+ " inches tall and weighs " + weight
							+ " pounds.\n\r");
				if ((mob == null) || (!mob.isMonster()))
					response.append(CMLib.protocol().mxpImage(M,
							" ALIGN=RIGHT H=70 W=70"));
			...
			response.append("\n\r"
					+ CMStrings.capitalizeFirstLetter(item.name(mob))
					+ " is a level " + level + " item, and weighs " + weight
					+ " pounds.  ");
			if ((item instanceof RawMaterial)
					&& (!CMLib.flags().isABonusItems(item))
					&& (item.rawSecretIdentity().length() > 0)
					&& (item.basePhyStats().weight() > 1)
					&& ((mob == null) || (mob.charStats().getStat(
							CharStats.STAT_INTELLIGENCE) > 3)))
				response.append("It appears to be a bundle of `"
						+ item.rawSecretIdentity() + "`.  ");

			if ((mob != null)
					&& (mob.charStats().getStat(CharStats.STAT_INTELLIGENCE) < 10))
				response.append("It is mostly made of a kind of "
						+ RawMaterial.Material.findByMask(
								item.material() & RawMaterial.MATERIAL_MASK)
								.noun() + ".  ");
			else
				response.append("It is mostly made of "
						+ RawMaterial.CODES.NAME(item.material()).toLowerCase()
						+ ".  ");
			if ((item instanceof Recipe)
					&& ((Recipe) item).getTotalRecipePages() > 1)
				response.append("There are "
						+ ((Recipe) item).getTotalRecipePages()
						+ " blank pages/entries remaining.  ");
			if (item instanceof Ammunition)
				response.append("It is " + ((Ammunition) item).usesRemaining()
						+ " ammunition of type '"
						+ ((Ammunition) item).ammunitionType() + "'.  ");
			else if (item instanceof Weapon) {
				if ((mob == null)
						|| mob.charStats().getStat(CharStats.STAT_INTELLIGENCE) > 10) {
					response.append("It is a ");
					if ((item.rawLogicalAnd())
							&& CMath.bset(item.rawProperLocationBitmap(),
									Wearable.WORN_WIELD | Wearable.WORN_HELD))
						response.append("two handed ");
					else
						response.append("one handed ");
					response.append(CMStrings
							.capitalizeAndLower(Weapon.CLASS_DESCS[((Weapon) item)
									.weaponClassification()])
							+ " class weapon that does "
							+ CMStrings
									.capitalizeAndLower(Weapon.TYPE_DESCS[((Weapon) item)
											.weaponType()]) + " damage.  ");
				}
				if ((item instanceof AmmunitionWeapon)
						&& ((AmmunitionWeapon) item).requiresAmmunition())
					response.append("It requires ammunition of type '"
							+ ((AmmunitionWeapon) item).ammunitionType()
							+ "'.  ");
			} else if ((item instanceof Armor)
					&& ((mob == null) || mob.charStats().getStat(
							CharStats.STAT_INTELLIGENCE) > 10)) {
				if (item.phyStats().height() > 0)
					response.append(" It is a size " + item.phyStats().height()
							+ ", and is ");
				else
					response.append(" It is your size, and is ");
				response.append(((item.rawProperLocationBitmap() == Wearable.WORN_HELD) || (item
						.rawProperLocationBitmap() == (Wearable.WORN_HELD | Wearable.WORN_WIELD))) ? new StringBuilder(
	...
	protected String dispossessionTimeLeftString(Item item) {
		...
		if (item.expirationDate() > System.currentTimeMillis())
			return "*IMMINENT*";
		return ""
				+ CMLib.time().date2EllapsedTime(
						(item.expirationDate() - System.currentTimeMillis()),
	...
	protected void handleBeingItemLookedAt(CMMsg msg) {
		...
		if (item instanceof Container) {
			...
			if ((contitem.isOpen())
					&& ((contitem.capacity() > 0)
							|| (contitem.getContents().size() > 0) || ((contitem instanceof Drink) && (((Drink) contitem)
							.liquidRemaining() > 0)))) {
				buf.append(item.name()
						+ " contains:^<!ENTITY container \""
						+ CMStrings.removeColors(item.name())
						+ "\"^>"
						+ (CMath.bset(mob.getBitmap(), MOB.ATT_COMPRESS) ? " "
								: "\n\r"));
				Vector<Item> newItems = new Vector<Item>();
				if ((item instanceof Drink)
						&& (((Drink) item).liquidRemaining() > 0)) {
					RawMaterial l = (RawMaterial) CMClass
							.getItem("GenLiquidResource");
					...
					String name = RawMaterial.CODES.NAME(myResource)
							.toLowerCase();
					l.setName("some " + name);
				...
				if (item.owner() instanceof MOB) {
					...
					buf.append(CMLib.lister().lister(mob, newItems, true,
							"CMItem", "", false,
							CMath.bset(mob.getBitmap(), MOB.ATT_COMPRESS)));
			...
			} else if ((contitem.hasALid())
					&& ((contitem.capacity() > 0) || (contitem.getContents()
							.size() > 0)))
				buf.append(item.name() + " is closed.");
		}
		if (!msg.source().isMonster())
			buf.append(CMLib.protocol()
					.mxpImage(item, " ALIGN=RIGHT H=70 W=70"));
	...
	protected void handleBeingItemSniffed(CMMsg msg) {
		...
		if ((s != null) && (s.length() > 0))
			msg.source().tell(msg.source(), item, null,
					"<T-NAME> has a " + s + " smell.");
	}
	...
	public void handleIntroductions(MOB speaker, MOB me, String msg) {
		if ((me.playerStats() != null)
				...
				&& (CMLib.english().containsString(
						CMStrings.getSayFromMessage(msg), speaker.Name())))
	...
	protected void handleBeingRoomLookedAt(CMMsg msg) {
		...
		if (msg.targetMinor() != CMMsg.TYP_EXAMINE)
			lookCode = (msg.sourceMessage() == null) ? LOOK_BRIEFOK
					: LOOK_NORMAL;
		...
		if (sysmsgs
				&& (!CMSecurity
						.isAllowed(mob, room, CMSecurity.SecFlag.SYSMSGS))) {
		...
		if (sysmsgs) {
			...
			final String rscName = room.myResource() >= 0 ? RawMaterial.CODES
					.NAME(room.myResource()) : "";
			final String domType;
			final StringBuilder domCond = new StringBuilder("");
			if ((room.domainType() & Room.INDOORS) == 0)
				domType = Room.outdoorDomainDescs[room.domainType()];
			else
				domType = Room.indoorDomainDescs[CMath.unsetb(
						room.domainType(), Room.INDOORS)];
			...
			Say.append("^!RoomID:^N " + CMLib.map().getExtendedRoomID(room)
					+ "\n\r^!" + room.ID() + "^N: " + domType + " "
					+ domCond.toString() + " <" + rscName + "> "
					+ room.basePhyStats().weight() + "mv \n\r");
		}
		if (CMLib.flags().canBeSeenBy(room, mob)) {
			Say.append("^O^<RName^>" + room.displayText(mob) + "^</RName^>"
					+ CMLib.flags().colorCodes(room, mob) + "^L\n\r");
			if ((lookCode != LOOK_BRIEFOK)
					|| (!CMath.bset(mob.getBitmap(), MOB.ATT_BRIEF))) {
					...
					for (int c = 0; c < room.numItems(); c++) {
						...
						if ((item.container() == null)
							...
							for (int k = 0; k < keyWords.size(); k++) {
								...
								while (x >= 0) {
									if (((x <= 0) || ((!Character
											.isLetterOrDigit(roomDesc
													.charAt(x - 1))) && (roomDesc
											.charAt(x - 1) != '>')))
											&& (((x + word.length()) >= (roomDesc
													.length() - 1)) || ((!Character
													.isLetterOrDigit(roomDesc
															.charAt((x + word
																	.length())))) && (roomDesc
													.charAt(x + word.length()) != '^')))) {
										int brackCheck = roomDesc.substring(x)
												.indexOf("^>");
										int brackCheck2 = roomDesc.substring(x)
												.indexOf("^<");
										if ((brackCheck < 0)
												|| (brackCheck2 < brackCheck)) {
											int start = x;
											while ((start >= 0)
													&& (!Character
															.isWhitespace(roomDesc
																	.charAt(start))))
												start--;
											start++;
											int end = (x + word.length());
											while ((end < roomDesc.length())
													&& (!Character
															.isWhitespace(roomDesc
																	.charAt(end))))
												end++;
											int l = roomDesc.length();
											roomDesc = roomDesc.substring(0,
													start)
													+ "^H^<WItem \""
													+ CMStrings
															.removeColors(item
																	.name())
													+ "\"^>"
													+ roomDesc.substring(start,
															end)
													+ "^</WItem^>^?"
													+ roomDesc.substring(end);
				...
				if ((CMProps.getIntVar(CMProps.Int.EXVIEW) == 1)
						|| (CMProps.getIntVar(CMProps.Int.EXVIEW) == 2))
					roomDesc += getRoomExitsParagraph(mob, room);
				Say.append("^L^<RDesc^>" + roomDesc + "^</RDesc^>");

				if ((!mob.isMonster())
						&& (sess.getClientTelnetMode(Session.TELNET_MXP)))
					Say.append(CMLib.protocol().mxpImage(room,
							" ALIGN=RIGHT H=70 W=70"));
				if (compress)
					Say.append("^N  ");
				else
		...
		StringBuilder itemStr = CMLib.lister().lister(mob, viewItems, false,
				"RItem", " \"*\"", lookCode == LOOK_LONG, compress);
		if (itemStr.length() > 0)
			Say.append(itemStr);
		...
		for (int i = 0; i < room.numInhabitants(); i++) {
			...
			if ((mob2 != null) && (mob2 != mob)) {
				...
				if ((displayText.length() > 0) || (sysmsgs)) {
					if (CMLib.flags().canBeSeenBy(mob2, mob)) {
						if ((!compress)
								&& (!mob.isMonster())
								&& (sess.getClientTelnetMode(Session.TELNET_MXP)))
							Say.append(CMLib.protocol().mxpImage(mob2,
									" H=10 W=10", "", " "));
						Say.append("^M^<RMob \""
								+ CMStrings.removeColors(mob2.name()) + "\"^>");
						if (compress)
							Say.append(CMLib.flags().colorCodes(mob2, mob)
									+ "^M ");
						if (displayText.length() > 0)
							Say.append(CMStrings.endWithAPeriod(CMStrings
									.capitalizeFirstLetter(displayText)));
						else
							Say.append(CMStrings.endWithAPeriod(CMStrings
									.capitalizeFirstLetter(mob2.name())));
						Say.append("^</RMob^>");
						if (sysmsgs)
		...
		if (Say.length() == 0)
			...
			if ((CMProps.getIntVar(CMProps.Int.AWARERANGE) > 0)
				...
				if (awarenessA != null) {
					...
					awarenessA.invoke(mob, list, mob.location(), true,
							CMProps.getIntVar(CMProps.Int.AWARERANGE));
					for (Object o : list)
	...
	private static String getRoomExitsParagraph(MOB mob, Room room) {
		...
		Room R;
		Exit E;
		final boolean useShipNames = ((room instanceof SpaceShip) || (room
				.getArea() instanceof SpaceShip));
		...
		if (continues.size() > 0) {
			str.append("  ^L"
					+ CMStrings.capitalizeFirstLetter(room.displayText(mob))
							.trim() + " continues ");
			if (continues.size() == 1) {
				if (useShipNames)
					str.append(Directions.getShipInDirectionName(continues
							.firstElement().intValue()) + ".");
				else
					str.append(Directions.getInDirectionName(continues
							.firstElement().intValue()) + ".");
			} else {
					...
					if (useShipNames)
						str.append(Directions
								.getShipDirectionName(
										continues.elementAt(i).intValue())
								.toLowerCase().trim()
								+ ", ");
					else
						str.append(Directions
				...
				if (useShipNames)
					str.append("and "
							+ Directions.getShipInDirectionName(
									continues.lastElement().intValue()).trim()
							+ ".");
				else
					str.append("and "
							+ Directions.getInDirectionName(
									continues.lastElement().intValue()).trim()
							+ ".");
			}
		}
		...
		if (exitDirs.size() > 0) {
			str.append("  ^L"
					+ CMStrings.capitalizeFirstLetter(
							getExitFragment(mob, room, exitDirs.firstElement()
									.intValue(), style)).trim());
			if (exitDirs.size() > 1) {
				str.append("^L, ");
				for (int i = 1; i < exitDirs.size() - 1; i++)
					str.append(getExitFragment(mob, room,
							exitDirs.elementAt(i).intValue(), style).trim()
							+ "^L, ");
				str.append("and "
						+ getExitFragment(mob, room,
								exitDirs.lastElement().intValue(), style)
								.trim());
			}
			str.append("^L.");
		}
	...
	private static String getExitFragment(MOB mob, Room room, int dir,
			boolean style) {
		if (room == null)
			return "";
		...
		if (exit == null)
			return "";
		final boolean useShipDirs = (room instanceof SpaceShip)
				|| (room.getArea() instanceof SpaceShip);
		final String inDirName = useShipDirs ? Directions
				.getShipInDirectionName(dir) : Directions
				.getInDirectionName(dir);
		if (style)
			return inDirName + " is "
					+ exit.viewableText(mob, room.getRoomInDir(dir));
		else
			return exit.viewableText(mob, room.getRoomInDir(dir)).toString()
					.trim()
					+ " is " + inDirName;
	}

	public boolean isHygienicMessage(final CMMsg msg, final int minHygiene,
			final long adjHygiene) {
		if ((msg.sourceMajor(CMMsg.MASK_MOVE) || ((msg.tool() instanceof Social) && ((msg
				.tool().Name().toUpperCase().startsWith("BATHE")) || (msg
				.tool().Name().toUpperCase().startsWith("WASH"))))))
			return (msg.source().playerStats() != null)
					&& (msg.source().soulMate() == null);
		return false;
	}

	public void handleHygienicMessage(final CMMsg msg, final int minHygiene,
			final long adjHygiene) {
		...
		if (CMLib.flags().canBeSeenBy(exit, mob)) {
			if (exit.description(mob).trim().length() > 0)
				...
				if (isAClearExitView(mob, room, exit) && (direction >= 0)
					...
					StringBuilder seenThatWay = CMLib.lister().lister(
							msg.source(), items, true, "", "", false, true);
					if (seenThatWay.length() > 0)
						mob.tell("Yonder, you can also see: "
								+ seenThatWay.toString());
				}
			} else
			...
			String image = CMLib.protocol().mxpImage(exit,
					" ALIGN=RIGHT H=70 W=70");
		...
		if (CMLib.flags().canBeSeenBy(viewedmob, viewermob)) {
			if (CMath.bset(viewermob.getBitmap(), MOB.ATT_SYSOPMSGS))
				myDescription.append("\n\rType :"
						+ viewedmob.ID()
						+ "\n\rRejuv:"
						+ viewedmob.basePhyStats().rejuv()
						+ "\n\rAbile:"
						+ viewedmob.basePhyStats().ability()
						+ "\n\rLevel:"
						+ viewedmob.basePhyStats().level()
						+ "\n\rDesc : "
						+ viewedmob.description()
						+ "\n\rRoom :'"
						+ ((viewedmob.getStartRoom() == null) ? "null"
								: viewedmob.getStartRoom().roomID())
			...
			if (!viewedmob.isMonster()) {
				...
				if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES))
					...
					levelStr = CMLib.english().startWithAorAn(
							viewedmob.charStats().displayClassLevel(viewedmob,
									false));
				else if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.LEVELS))
						&& (!viewedmob.charStats().getCurrentClass().leveless())
						&& (!viewedmob.charStats().getMyRace().leveless()))
					levelStr = "level "
							+ viewedmob.charStats().displayClassLevelOnly(
									viewedmob);
				else if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES))
						&& (!viewedmob.charStats().getMyRace().classless()))
					levelStr = CMLib.english().startWithAorAn(
							viewedmob.charStats().displayClassName());
				if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.RACES))
					...
					if (viewedmob.charStats().getStat(CharStats.STAT_AGE) > 0)
						myDescription.append(viewedmob.charStats().ageName()
								.toLowerCase()
								+ " ");
			...
			if (viewedmob.phyStats().height() > 0)
				myDescription.append(viewedmob.charStats().HeShe() + " is "
						+ viewedmob.phyStats().height()
						+ " inches tall and weighs "
						+ viewedmob.basePhyStats().weight() + " pounds.\n\r");
			if ((longlook)
					&& (viewermob.charStats().getStat(
							CharStats.STAT_INTELLIGENCE) > 12)) {
				...
				myDescription.append(relativeCharStatTest(C, viewedmob,
						"weaker", "stronger", CharStats.STAT_STRENGTH));
				myDescription.append(relativeCharStatTest(C, viewedmob,
						"clumsier", "more nimble", CharStats.STAT_DEXTERITY));
				myDescription
						.append(relativeCharStatTest(C, viewedmob,
								"more sickly", "healthier",
								CharStats.STAT_CONSTITUTION));
				myDescription.append(relativeCharStatTest(C, viewedmob,
						"more repulsive", "more attractive",
						CharStats.STAT_CHARISMA));
				myDescription.append(relativeCharStatTest(C, viewedmob,
						"more naive", "wiser", CharStats.STAT_WISDOM));
				myDescription.append(relativeCharStatTest(C, viewedmob,
						"dumber", "smarter", CharStats.STAT_INTELLIGENCE));
				testMOB.destroy();
			}
			if (!viewermob.isMonster())
				myDescription.append(CMLib.protocol().mxpImage(viewedmob,
						" ALIGN=RIGHT H=70 W=70"));
			myDescription.append(viewedmob.healthText(viewermob) + "\n\r\n\r");
			myDescription.append(viewedmob.description(viewermob) + "\n\r\n\r");

			StringBuilder eq = CMLib.commands().getEquipment(viewermob,
					viewedmob);
			if (eq.length() > 0) {
				if ((CMProps.getIntVar(CMProps.Int.EQVIEW) > 1)
						|| ((viewermob != viewedmob) && (CMProps
								.getIntVar(CMProps.Int.EQVIEW) > 0)))
					myDescription.append(viewedmob.charStats().HeShe()
							+ " is wearing " + eq.toString());
				else
					myDescription.append(viewedmob.charStats().HeShe()
							+ " is wearing:\n\r" + eq.toString());
			}
	...
	public void handleBeingGivenTo(CMMsg msg) {
		...
		if (giveemob.location() != null) {
			CMMsg msg2 = CMClass.getMsg(givermob, msg.tool(), null,
					CMMsg.MSG_DROP | CMMsg.MASK_INTERMSG, null, CMMsg.MSG_DROP
							| CMMsg.MASK_INTERMSG, null, CMMsg.MSG_DROP
							| CMMsg.MASK_INTERMSG, null);
			giveemob.location().send(givermob, msg2);
			msg2 = CMClass.getMsg((MOB) msg.target(), msg.tool(), null,
					CMMsg.MSG_GET | CMMsg.MASK_INTERMSG, null, CMMsg.MSG_GET
							| CMMsg.MASK_INTERMSG, null, CMMsg.MSG_GET
							| CMMsg.MASK_INTERMSG, null);
			...
			if (CMLib.flags().canBeSeenBy(msg.target(), mob)) {
				...
				if ((msg.target() instanceof Exit)
					...
					if (((text == null) || (text.length() == 0))
							...
							&& ((targetI.displayText(mob).length() == 0) || (!CMLib
									.flags().isGettable(targetI))))
						text = targetI.description(mob);
				}
				if ((text != null) && (text.length() > 0)) {
					if (text.toUpperCase().startsWith("FILE=")) {
						StringBuffer buf = Resources.getFileResource(
								text.substring(5), true);
						if ((buf != null) && (buf.length() > 0))
							mob.tell("It says '" + buf.toString() + "'.");
						else if (msg.target() instanceof Electronics)
							mob.tell("There is nothing on "
									+ ((Electronics) msg.target()).name(mob)
									+ ".");
						else if (msg.target() instanceof Physical)
							mob.tell("There is nothing written on "
									+ ((Physical) msg.target()).name(mob) + ".");
						else
							mob.tell("There is nothing written on "
									+ msg.target().name() + ".");
					} else
	...
	public void handleBeingGetted(CMMsg msg) {
		...
		if (item instanceof Container) {
			...
			} else if (!mob.isMine(item)) {
				item.setContainer(null);
				mob.moveItemTo(item);
				if (!CMath.bset(msg.targetMajor(), CMMsg.MASK_OPTIMIZE))
					mob.location().recoverRoomStats();
				else
					mob.phyStats().setWeight(
							mob.phyStats().weight() + item.recursiveWeight());
			} else {
				item.setContainer(null);
				item.unWear();
				if (!CMath.bset(msg.targetMajor(), CMMsg.MASK_OPTIMIZE))
				...
				item.basePhyStats()
						.setDisposition(
								item.basePhyStats().disposition()
										& ((int) PhyStats.ALLMASK - PhyStats.IS_HIDDEN));
			...
			if (!mob.isMine(item)) {
				...
				if (CMath.bset(msg.targetMajor(), CMMsg.MASK_OPTIMIZE))
					mob.phyStats().setWeight(
							mob.phyStats().weight() + item.phyStats().weight());
			}
			item.unWear();
			if (!CMath.bset(msg.targetMajor(), CMMsg.MASK_OPTIMIZE))
				mob.location().recoverRoomStats();
	...
	public void handleBeingDropped(CMMsg msg) {
		...
		if (mob.isMine(item) && (item instanceof Container)) {
			item.setContainer(null);
			CMLib.utensils().recursiveDropMOB(mob, mob.location(), item,
					item instanceof DeadBody);
			if (!CMath.bset(msg.targetMajor(), CMMsg.MASK_OPTIMIZE))
				mob.location().recoverRoomStats();
		}
		if (mob.isMine(item)) {
	...
	public void handleBeingWorn(CMMsg msg) {
		if (!(msg.target() instanceof Item))
			return;
		Item item = (Item) msg.target();
		long wearLocation = (msg.value() <= 0) ? 0
				: ((long) (1 << msg.value()) / 2);
		MOB mob = msg.source();
	...
	public void lookAtExits(Room room, MOB mob) {
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if (Say.length() > 0) {
				Dir = CMStrings.padRightPreserve(
						useShipNames ? Directions.getShipDirectionName(d)
								: Directions.getDirectionName(d), 5);
				if ((mob.playerStats() != null) && (room2 != null)
						&& (!mob.playerStats().hasVisited(room2)))
					buf.append("^U^<EX^>" + Dir + "^</EX^>:^.^N ^u" + Say
							+ "^.^N\n\r");
				else
					buf.append("^D^<EX^>" + Dir + "^</EX^>:^.^N ^d" + Say
							+ "^.^N\n\r");
			}
		}
		Item I = null;
		for (int i = 0; i < room.numItems(); i++) {
			...
			if ((I instanceof Exit) && (((Exit) I).doorName().length() > 0)
				...
				if (Say.length() > 0) {
					...
					if (Say.length() > 5)
						buf.append("^D^<MEX^>" + ((Exit) I).doorName()
								+ "^</MEX^>:^.^N ^d" + Say + "^.^N\n\r");
					else
						buf.append("^D^<MEX^>"
								+ CMStrings.padRight(((Exit) I).doorName(), 5)
								+ "^</MEX^>:^.^N ^d" + Say + "^.^N\n\r");
				}
			}
		}
	...
	public void lookAtExitsShort(Room room, MOB mob) {
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if ((exit != null)
				...
				buf.append("^<EX^>"
						+ (useShipNames ? Directions.getShipDirectionName(d)
								: Directions.getDirectionName(d)) + "^</EX^> ");
		}
		Item I = null;
		for (int i = 0; i < room.numItems(); i++) {
			I = room.getItem(i);
			if ((I instanceof Exit) && (I.container() == null)
