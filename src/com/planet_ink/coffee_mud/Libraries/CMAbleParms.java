...
public class CMAbleParms extends StdLibrary implements AbilityParameters {
	...
	protected final static int[] ALL_BUCKET_MATERIAL_CHOICES = new int[] {
			RawMaterial.MATERIAL_CLOTH, RawMaterial.MATERIAL_METAL,
			RawMaterial.MATERIAL_LEATHER, RawMaterial.MATERIAL_LIQUID,
			RawMaterial.MATERIAL_WOODEN, RawMaterial.MATERIAL_PRECIOUS,
			RawMaterial.MATERIAL_VEGETATION, RawMaterial.MATERIAL_ROCK };
	protected final static int[] ALLOWED_BUCKET_ACODES = new int[] {
			Ability.ACODE_CHANT, Ability.ACODE_SPELL, Ability.ACODE_PRAYER,
			Ability.ACODE_SONG, Ability.ACODE_SKILL, Ability.ACODE_THIEF_SKILL };
	protected final static int[] ALLOWED_BUCKET_QUALITIES = new int[] {
			Ability.QUALITY_BENEFICIAL_OTHERS, Ability.QUALITY_BENEFICIAL_SELF,
			Ability.QUALITY_INDIFFERENT, Ability.QUALITY_OK_OTHERS,
	...
	protected String parseLayers(short[] layerAtt, short[] clothingLayers,
			String misctype) {
	...
	public void parseWearLocation(short[] layerAtt, short[] layers,
			long[] wornLoc, boolean[] logicalAnd, double[] hardBonus,
			String wearLocation) {
		if (layers != null) {
			layerAtt[0] = 0;
			layers[0] = 0;
	...
	public Vector<Object> parseRecipeFormatColumns(String recipeFormat) {
		...
		for (int c = 0; c <= recipeFmtC.length; c++) {
			...
			if ((C == '|') && (c < (recipeFmtC.length - 1))
				...
				currentColumns = null;
				if ((C == '.') && (c < (recipeFmtC.length - 2))
						&& (recipeFmtC[c + 1] == '.')
						&& (recipeFmtC[c + 2] == '.')) {
					c += 2;
					columnsV.addElement("...");
				} else if (columnsV.lastElement() instanceof String)
					columnsV.setElementAt(
							((String) columnsV.lastElement()) + C,
							columnsV.size() - 1);
				else
	...
	public String makeRecipeFromItem(final ItemCraftor C, final Item I)
			throws CMException {
		...
		for (int d = 0; d < columns.size(); d++) {
			if (columns.get(d) instanceof String) {
				...
				List<AbilityParmEditor> colV = (List<AbilityParmEditor>) columns
						.get(d);
				for (int c = 0; c < colV.size(); c++) {
					AbilityParmEditor A = editors.get(colV.get(c));
					if (A == null)
						throw new CMException("Column name " + (colV.get(c))
								+ " is not found.");
					if ((applicableA == null)
							|| (A.appliesToClass(I) > applicableA
									.appliesToClass(I)))
						applicableA = A;
				}
				if ((applicableA == null)
						|| (applicableA.appliesToClass(I) < 0))
					applicableA = editors.get("N_A");
				columns.set(d, applicableA.ID());
			} else
				throw new CMException("Col name " + (columns.get(d))
						+ " is not a String or List.");
			AbilityParmEditor A = editors.get(columns.get(d));
			if (A == null)
				throw new CMException("Editor name " + (columns.get(d))
						+ " is not defined.");
	...
	protected Item getSampleItem(DVector dataRow) {
		...
		for (int d = 0; d < dataRow.size(); d++)
			if ((dataRow.elementAt(d, 1) instanceof List) && (!classIDRequired)
	...
	@SuppressWarnings("unchecked")
	protected Vector<DVector> parseDataRows(StringBuffer recipeData,
			Vector<? extends Object> columnsV, int numberOfDataColumns)
			throws CMException {
		...
		while (str.length() > 0) {
			...
			if (dataRow.size() != numberOfDataColumns)
				throw new CMException("Row " + (rowsV.size() + 1) + " has "
						+ dataRow.size() + "/" + numberOfDataColumns);
			rowsV.addElement(dataRow);
			dataRow = new DVector(2);
			if (str.length() == lastLen)
				throw new CMException("UNCHANGED: Row " + (rowsV.size() + 1)
						+ " has " + dataRow.size() + "/" + numberOfDataColumns);
		}
		if (str.length() < 2)
			str.setLength(0);
		return rowsV;
	...
	@SuppressWarnings("unchecked")
	protected boolean fixDataColumn(DVector dataRow, int rowShow,
			final Item classModelI) throws CMException {
		Map<String, AbilityParmEditor> editors = getEditors();
		if (classModelI == null) {
			Log.errOut("CMAbleParms", "Data row " + rowShow
					+ " discarded due to null/empty classID");
			return false;
		}
		for (int d = 0; d < dataRow.size(); d++) {
			...
			if (colV.size() == 1) {
				...
				for (int c = 0; c < colV.size(); c++) {
					AbilityParmEditor A = editors.get(colV.get(c));
					if (A == null)
						throw new CMException("Col name " + (colV.get(c))
								+ " is not defined.");
					if ((applicableA == null)
							|| (A.appliesToClass(classModelI) > applicableA
									.appliesToClass(classModelI)))
						applicableA = A;
				}
				if ((applicableA == null)
						|| (applicableA.appliesToClass(classModelI) < 0))
					applicableA = editors.get("N_A");
				dataRow.setElementAt(d, 1, applicableA.ID());
			}
			AbilityParmEditor A = editors.get(dataRow.elementAt(d, 1));
			if (A == null) {
				Log.errOut("CMAbleParms",
						"Item id " + classModelI.ID() + " has no editor for "
								+ ((String) dataRow.elementAt(d, 1)));
				return false;
			} else if ((rowShow >= 0)
					&& (!A.confirmValue((String) dataRow.elementAt(d, 2))))
				Log.errOut("CMAbleParms", "Item id " + classModelI.ID()
						+ " has bad data '"
						+ ((String) dataRow.elementAt(d, 2)) + "' for column "
						+ ((String) dataRow.elementAt(d, 1)) + " at row "
						+ rowShow);
		}
		return true;
	}
	...
	public void testRecipeParsing(StringBuffer recipesString,
			String recipeFormat) throws CMException {
	...
	public void testRecipeParsing(String recipeFilename, String recipeFormat,
		...
		StringBuffer str = new CMFile(Resources.buildResourcePath("skills")
				+ recipeFilename, null, CMFile.FLAG_LOGERRORS).text();
		try {
			testRecipeParsing(str, recipeFormat, save ? recipeFilename : null);
		} catch (CMException e) {

			Log.errOut("CMAbleParms",
					"File: " + recipeFilename + ": " + e.getMessage());
			return;
		}
	}

	@SuppressWarnings("unchecked")
	public void testRecipeParsing(StringBuffer str, String recipeFormat,
			String saveRecipeFilename) throws CMException {
		...
		for (int c = 0; c < columnsV.size(); c++)
			if (columnsV.elementAt(c) instanceof List)
				numberOfDataColumns++;
		Vector<DVector> rowsV = parseDataRows(str, columnsV,
				numberOfDataColumns);
		...
		for (int r = 0; r < rowsV.size(); r++) {
			...
			for (int a = 0; a < editRow.size(); a++) {
				...
				try {
					...
					fakeSession.getPreviousCMD().addAll(
							new XVector<String>(A.fakeUserInput(oldVal)));
					String newVal = A.commandLinePrompt(mob, oldVal,
							showNumber, showFlag);
		...
		if (saveRecipeFilename != null)
			resaveRecipeFile(mob, saveRecipeFilename, rowsV, convertedColumnsV,
					false);
	}

	protected void calculateRecipeCols(int[] lengths, String[] headers,
			Vector<DVector> rowsV) {
		Map<String, AbilityParmEditor> editors = getEditors();
		...
		for (int r = 0; r < rowsV.size(); r++) {
			...
			for (int c = 0; c < dataRow.size(); c++) {
				...
				if (A == null)
					Log.errOut("CMAbleParms", "Inexplicable lack of a column: "
							+ ((String) dataRow.elementAt(c, 1)));
				else if (headers[c] == null) {
					headers[c] = A.colHeader();
					lengths[c] = headers[c].length();
				} else if ((!headers[c].startsWith("#"))
	...
	public AbilityRecipeData parseRecipe(String recipeFilename,
			String recipeFormat) {
		AbilityRecipeDataImpl recipe = new AbilityRecipeDataImpl(
				recipeFilename, recipeFormat);
		return recipe;
	}

	public StringBuffer getRecipeList(ItemCraftor iA) {
		AbilityRecipeData recipe = parseRecipe(iA.parametersFile(),
				iA.parametersFormat());
		if (recipe.parseError() != null)
			return new StringBuffer("File: " + iA.parametersFile() + ": "
					+ recipe.parseError());
	...
	private StringBuffer getRecipeList(AbilityRecipeData recipe) {
		...
		for (int l = 0; l < recipe.columnLengths().length; l++)
			list.append(CMStrings.padRight(recipe.columnHeaders()[l],
					recipe.columnLengths()[l])
					+ " ");
		list.append("\n\r");
		for (int r = 0; r < recipe.dataRows().size(); r++) {
			...
			for (int c = 0; c < dataRow.size(); c++)
				list.append(CMStrings.padRight(CMStrings.limit(
						(String) dataRow.elementAt(c, 2),
						recipe.columnLengths()[c]), recipe.columnLengths()[c])
						+ " ");
			list.append("\n\r");
		}
		return list;
	...
	@SuppressWarnings("unchecked")
	public void modifyRecipesList(MOB mob, String recipeFilename,
			String recipeFormat) throws java.io.IOException {
		Map<String, AbilityParmEditor> editors = getEditors();
		AbilityRecipeData recipe = parseRecipe(recipeFilename, recipeFormat);
		if (recipe.parseError() != null) {
			Log.errOut("CMAbleParms",
					"File: " + recipeFilename + ": " + recipe.parseError());
			return;
		}
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			...
			if (lineNum.equalsIgnoreCase("A")) {
				...
				if (keyIndex >= 0) {
					AbilityParmEditor A = editors.get(((List<String>) editRow
							.elementAt(keyIndex, 1)).get(0));
					if (A != null) {
						classFieldData = A.commandLinePrompt(mob,
								(String) editRow.elementAt(keyIndex, 2),
								new int[] { 0 }, -999);
			...
			if (editRow != null) {
				...
				while (!ok) {
					...
					for (int a = 0; a < editRow.size(); a++)
						if (a != keyIndex) {
							AbilityParmEditor A = editors.get(editRow
									.elementAt(a, 1));
							String newVal = A.commandLinePrompt(mob,
									(String) editRow.elementAt(a, 2),
									showNumber, showFlag);
							editRow.setElementAt(a, 2, newVal);
						}
					if (showFlag < -900) {
		...
		if ((mob.session() != null) && (!mob.session().isStopped())) {
			String prompt = "Save to V)FS, F)ilesystem, or C)ancel ("
					+ (recipe.wasVFS() ? "V/f/c" : "v/F/c") + "): ";
			String choice = mob.session().choose(prompt, "VFC",
					recipe.wasVFS() ? "V" : "F");
			if (choice.equalsIgnoreCase("C"))
				mob.tell("Cancelled.");
			else {
				boolean saveToVFS = choice.equalsIgnoreCase("V");
				resaveRecipeFile(mob, recipeFilename, recipe.dataRows(),
						recipe.columns(), saveToVFS);
			}
		}
	}

	public void resaveRecipeFile(MOB mob, String recipeFilename,
			Vector<DVector> rowsV, Vector<? extends Object> columnsV,
			boolean saveToVFS) {
		...
		CMFile file = new CMFile((saveToVFS ? "::" : "//")
				+ Resources.buildResourcePath("skills") + recipeFilename, null,
				CMFile.FLAG_LOGERRORS);
		if (!file.canWrite())
			Log.errOut("CMAbleParms", "File: " + recipeFilename
					+ " can not be written");
		else if ((!file.exists()) || (!file.text().equals(saveBuf))) {
			file.saveText(saveBuf);
			if (!saveToVFS) {
				file = new CMFile("::" + Resources.buildResourcePath("skills")
						+ recipeFilename, null, CMFile.FLAG_LOGERRORS);
				if ((file.exists()) && (file.canWrite())) {
					file.saveText(saveBuf);
				}
			}
			Log.sysOut("CMAbleParms", "User: " + mob.Name() + " modified "
					+ (saveToVFS ? "VFS" : "Local") + " file " + recipeFilename);
			Resources.removeResource("PARSED_RECIPE: " + recipeFilename);
			Resources.removeMultiLists(recipeFilename);
		}
	}

	protected static int getAppropriateResourceBucket(final Item I,
			final Object A) {
		final int myMaterial = ((I.material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_MITHRIL) ? RawMaterial.MATERIAL_METAL
				: (I.material() & RawMaterial.MATERIAL_MASK);
		if (A instanceof Behavior)
			return CMLib.dice().pick(
					new int[] { RawMaterial.MATERIAL_LEATHER,
							RawMaterial.MATERIAL_VEGETATION }, myMaterial);
		if (A instanceof Ability) {
			switch (((Ability) A).abilityCode() & Ability.ALL_ACODES) {
			case Ability.ACODE_CHANT:
				return CMLib.dice().pick(
						new int[] { RawMaterial.MATERIAL_VEGETATION,
								RawMaterial.MATERIAL_ROCK }, myMaterial);
			case Ability.ACODE_SPELL:
				return CMLib.dice().pick(
						new int[] { RawMaterial.MATERIAL_WOODEN,
								RawMaterial.MATERIAL_PRECIOUS }, myMaterial);
			case Ability.ACODE_PRAYER:
				return CMLib.dice().pick(
						new int[] { RawMaterial.MATERIAL_METAL,
								RawMaterial.MATERIAL_ROCK }, myMaterial);
			case Ability.ACODE_SONG:
				return CMLib.dice().pick(
						new int[] { RawMaterial.MATERIAL_LIQUID,
								RawMaterial.MATERIAL_WOODEN }, myMaterial);
			case Ability.ACODE_THIEF_SKILL:
			case Ability.ACODE_SKILL:
				return CMLib.dice().pick(
						new int[] { RawMaterial.MATERIAL_CLOTH,
								RawMaterial.MATERIAL_METAL }, myMaterial);
			case Ability.ACODE_PROPERTY:
				if (A instanceof TriggeredAffect)
					return CMLib.dice().pick(
							new int[] { RawMaterial.MATERIAL_PRECIOUS,
									RawMaterial.MATERIAL_METAL }, myMaterial);
				break;
			default:
				break;
		...
		for (Enumeration<Room> e = CMClass.locales(); e.hasMoreElements();) {
			...
			if (!(R instanceof GridLocale))
				if ((R.resourceChoices() != null)
						&& (R.resourceChoices().contains(I)))
					return true;
		}
		return false;
	}

	protected static void addExtraMaterial(
			final Map<Integer, int[]> extraMatsM, final Item I, final Object A,
			double weight) {
		int times = 1;
		if (weight >= 1.0) {
		...
		if (myBucket != RawMaterial.RESOURCE_NOTHING) {
			final SPairList<Integer, Double> bucket = RawMaterial.CODES
					.instance().getValueSortedBucket(myBucket);
			Integer resourceCode = (bucket.size() == 0) ? Integer.valueOf(CMLib
					.dice().pick(RawMaterial.CODES.ALL(), I.material()))
					: bucket.get((weight >= .99) ? bucket.size() - 1 : 0).first;
			for (Iterator<Pair<Integer, Double>> b = bucket.iterator(); b
					.hasNext();) {
				final Pair<Integer, Double> p = b.next();
				if ((weight <= p.second.doubleValue())
						&& (isResourceCodeRoomMapped(p.first.intValue()))) {
			...
			int tries = 100;
			while ((--tries > 0)
					&& (!isResourceCodeRoomMapped(resourceCode.intValue())))
				resourceCode = bucket.get(CMLib.dice().roll(1, bucket.size(),
						-1)).first;
	...
	protected static void addExtraAbilityMaterial(
			final Map<Integer, int[]> extraMatsM, final Item I, final Ability A) {
		...
		if (level <= 0.0) {
			...
			double levelCap = CMLib.ableMapper()
					.getCalculatedMedianLowestQualifyingLevel();
			addExtraMaterial(extraMatsM, I, A,
					CMath.div(level, (levelCap * 2.0)));
		}
	}
	...
	public static Map<Integer, int[]> extraMaterial(final Item I) {
			...
			if (A.isSavable()) {
				if ((A.abilityCode() & Ability.ALL_ACODES) == Ability.ACODE_PROPERTY) {
					if (A instanceof AbilityContainer) {
						for (Enumeration<Ability> a1 = ((AbilityContainer) A)
								.abilities(); a1.hasMoreElements();) {
							addExtraAbilityMaterial(extraMatsM, I,
									a1.nextElement());
						}
					}
					if (A instanceof TriggeredAffect) {
						if ((A.flags() & Ability.FLAG_ADJUSTER) != 0) {
							int count = CMStrings.countSubstrings(
									new String[] { A.text() }, ADJUSTER_TOKENS);
							if (count == 0)
								count = 1;
				...
				} else if ((CMParms.indexOf(ALLOWED_BUCKET_ACODES,
						A.abilityCode() & Ability.ALL_ACODES) >= 0)
						&& (CMParms.indexOf(ALLOWED_BUCKET_QUALITIES,
								A.abstractQuality()) >= 0)) {
					addExtraAbilityMaterial(extraMatsM, I, A);
				}
			}
		...
		for (Enumeration<Behavior> b = I.behaviors(); b.hasMoreElements();) {
			...
			if (B.isSavable()) {
				addExtraMaterial(extraMatsM, I, B, CMath.div(CMProps
						.getIntVar(CMProps.Int.LASTPLAYERLEVEL), I
						.basePhyStats().level()));
			}
		}
		return extraMatsM;
	...
	public synchronized Map<String, AbilityParmEditor> getEditors() {
		...
		Vector<AbilityParmEditorImpl> V = new XVector<AbilityParmEditorImpl>(
				new AbilityParmEditorImpl[] {
						new AbilityParmEditorImpl("SPELL_ID", "The Spell ID",
								PARMTYPE_CHOICES) {
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
						...
						new AbilityParmEditorImpl("RESOURCE_NAME", "Resource",
								PARMTYPE_CHOICES) {
							public void createChoices() {
						...
						new AbilityParmEditorImpl("ITEM_NAME",
								"Item Final Name", PARMTYPE_STRING) {
							public void createChoices() {
							}
							...
							public String convertFromItem(final ItemCraftor A,
								...
								if (I.material() == RawMaterial.RESOURCE_GLASS)
									return CMLib.english().cleanArticles(
											oldName);

								String newName = oldName;
								List<String> V = CMParms.parseSpaces(oldName,
										true);
								for (int i = 0; i < V.size(); i++) {
									String s = V.get(i);
									int code = RawMaterial.CODES
											.FIND_IgnoreCase(s);
						...
						new AbilityParmEditorImpl("ITEM_LEVEL", "Lvl",
								PARMTYPE_NUMBER) {
							public void createChoices() {
							}
							...
							public String convertFromItem(final ItemCraftor A,
								...
								if ((I instanceof Weapon)
										|| (I instanceof Armor)) {
									int timsLevel = CMLib.itemBuilder()
											.timsLevelCalculator(I);
									if ((timsLevel > I.basePhyStats().level())
											&& (timsLevel < CMProps
						...
						new AbilityParmEditorImpl("BUILD_TIME_TICKS", "Time",
								PARMTYPE_NUMBER) {
							public void createChoices() {
							}
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
								return ""
										+ (10 + (I.basePhyStats().level() / 2));
							}
						},
						new AbilityParmEditorImpl("AMOUNT_MATERIAL_REQUIRED",
								"Amt", PARMTYPE_NUMBER) {
							public void createChoices() {
							}
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
								return ""
										+ Math.round(CMath.mul(
												I.basePhyStats().weight(),
												(A != null) ? A
														.getItemWeightMultiplier(false)
														: 1.0));
							}
						},
						new AbilityParmEditorImpl("MATERIALS_REQUIRED",
								"Amount/Cmp", PARMTYPE_SPECIAL) {
							public void createChoices() {
							}
							...
							public String webValue(HTTPRequest httpReq,
									java.util.Map<String, String> parms,
									String oldVal, String fieldName) {
								if (httpReq
										.isUrlParameter(fieldName + "_WHICH")) {
									String which = httpReq
											.getUrlParameter(fieldName
													+ "_WHICH");
									if ((which.trim().length() == 0)
										...
										return httpReq
												.getUrlParameter(fieldName
														+ "_AMOUNT");
									if (which.trim().equalsIgnoreCase(
											"COMPONENT"))
										return httpReq
												.getUrlParameter(fieldName
														+ "_COMPONENT");
									...
									while (httpReq.isUrlParameter(fieldName
											+ "_CUST_TYPE_" + x)) {
										String connector = httpReq
												.getUrlParameter(fieldName
														+ "_CUST_CONN_" + x);
										String amt = httpReq
												.getUrlParameter(fieldName
														+ "_CUST_AMT_" + x);
										String strVal = httpReq
												.getUrlParameter(fieldName
														+ "_CUST_STR_" + x);
										String loc = httpReq
												.getUrlParameter(fieldName
										...
										if (connector == null)
											connector = "AND";
										if (connector.equalsIgnoreCase("DEL")
												|| (connector.length() == 0)) {
											x++;
											continue;
										}
										try {
											AbilityComponent able = CMLib
													.ableMapper()
											...
											able.setMask("");
											able.setConsumed((con != null)
													&& con.equalsIgnoreCase("on"));
											able.setLocation(AbilityComponent.CompLocation
													.valueOf(loc));
											able.setType(
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
								int amt = (int) Math.round(CMath.mul(
										I.basePhyStats().weight() - 1,
										(A != null) ? A
												.getItemWeightMultiplier(false)
												: 1.0));
								if (amt < 1)
									amt = 1;
								Map<Integer, int[]> extraMatsM = CMAbleParms
										.extraMaterial(I);
								if ((extraMatsM == null)
										|| (extraMatsM.size() == 0)) {
									return "" + amt;
								...
								List<AbilityComponent> comps = new Vector<AbilityComponent>();
								AbilityComponent able = CMLib.ableMapper()
										.createBlankAbilityComponent();
								...
								for (Integer resourceCode : extraMatsM.keySet()) {
									...
									able.setLocation(AbilityComponent.CompLocation.ONGROUND);
									able.setType(
											AbilityComponent.CompType.RESOURCE,
											resourceCode);
									comps.add(able);
								}
								return CMLib.ableMapper()
										.getAbilityComponentCodedString(comps);
							}

							public String webField(HTTPRequest httpReq,
									java.util.Map<String, String> parms,
									String oldVal, String fieldName) {
								String value = webValue(httpReq, parms, oldVal,
										fieldName);
								if (value.endsWith("$"))
									value = value.substring(0,
											oldVal.length() - 1);
								value = value.trim();
								String curWhich = httpReq
										.getUrlParameter(fieldName + "_WHICH");
								int type = 0;
								if ("COMPONENT".equalsIgnoreCase(curWhich))
									type = 1;
								else if ("EMBEDDED".equalsIgnoreCase(curWhich))
									type = 2;
								else if ("AMOUNT".equalsIgnoreCase(curWhich))
									type = 0;
								else if (CMLib
										.ableMapper()
										.getAbilityComponentMap()
										.containsKey(value.toUpperCase().trim()))
									type = 1;
								...
								StringBuffer str = new StringBuffer(
										"<FONT SIZE=-1>");
								str.append("<INPUT TYPE=RADIO NAME="
										+ fieldName + "_WHICH "
										+ (type == 0 ? "CHECKED " : "")
										+ "VALUE=\"AMOUNT\">");
								str.append("\n\rAmount: <INPUT TYPE=TEXT SIZE=3 NAME="
										+ fieldName
										+ "_AMOUNT VALUE=\""
										+ (type != 0 ? "" : value)
										+ "\"  ONKEYDOWN=\"document.RESOURCES."
										+ fieldName
										+ "_WHICH[0].checked=true;\">");
								str.append("\n\r<BR>");
								str.append("<INPUT TYPE=RADIO NAME="
										+ fieldName + "_WHICH "
										+ (type == 1 ? "CHECKED " : "")
								...
								str.append("\n\r<SELECT NAME="
										+ fieldName
										+ "_COMPONENT ONCHANGE=\"document.RESOURCES."
										+ fieldName
										+ "_WHICH[1].checked=true;\">");
								str.append("<OPTION VALUE=\"0\"");
								if ((type != 1) || (value.length() == 0)
										|| (value.equalsIgnoreCase("0")))
									str.append(" SELECTED");
								str.append(">&nbsp;");
								for (String S : CMLib.ableMapper()
										.getAbilityComponentMap().keySet()) {
								...
								for (int i = 0; i <= comps.size(); i++) {
									...
									if (i > 0) {
										str.append("\n\r<SELECT NAME="
												...
												+ "_CUST_CONN_"
												+ (i + 1)
												+ " ONCHANGE=\"document.RESOURCES."
												+ fieldName
												+ "_WHICH[2].checked=true;\">");
										if (comp != null)
											str.append("<OPTION VALUE=\"DEL\">DEL");
										else if (type == 2)
											str.append("<OPTION VALUE=\"\" SELECTED>");
										for (AbilityComponent.CompConnector conector : AbilityComponent.CompConnector
												.values()) {
											...
											str.append(">"
													+ CMStrings
															.capitalizeAndLower(conector
																	.toString()));
										}
										str.append("</SELECT>");
									}
									str.append("\n\rAmt: <INPUT TYPE=TEXT SIZE=2 NAME="
											+ fieldName
											+ "_CUST_AMT_"
											+ (i + 1)
											+ " VALUE=\""
									...
									str.append("\n\r<SELECT NAME="
											...
											+ " ONCHANGE=\"document.RESOURCES."
											+ fieldName
											+ "_WHICH[2].checked=true; ReShow();\">");
									AbilityComponent.CompType compType = (comp != null) ? comp
											.getType()
											: AbilityComponent.CompType.STRING;
									for (AbilityComponent.CompType conn : AbilityComponent.CompType
										...
										str.append(">"
												+ CMStrings
														.capitalizeAndLower(conn
																.toString()));
									}
									str.append("</SELECT>");
									if (compType == AbilityComponent.CompType.STRING)
										str.append("\n\r<INPUT TYPE=TEXT SIZE=10 NAME="
												+ fieldName
												+ "_CUST_STR_"
												+ (i + 1)
												+ " VALUE=\""
										...
										if (compType == AbilityComponent.CompType.MATERIAL) {
											...
											Arrays.sort(
													M,
													new Comparator<RawMaterial.Material>() {
														@Override
														public int compare(
																Material o1,
																Material o2) {
											...
											for (int x = 0; x < RawMaterial.CODES
													.TOTAL(); x++)
												L.add(new Pair<String, Integer>(
														RawMaterial.CODES
																.NAME(x),
														Integer.valueOf(RawMaterial.CODES
																.GET(x))));
											Collections
													.sort(L,
									...
									str.append("</SELECT>");
									str.append("\n\rConsumed:<INPUT TYPE=CHECKBOX NAME="
											+ fieldName
											+ "_CUST_CON_"
											+ (i + 1)
											+ " "
							...
							public String commandLinePrompt(MOB mob,
									String oldVal, int[] showNumber,
								...
								while (!mob.session().isStopped()) {
									...
									str = CMLib
											.genEd()
											.prompt(mob, oldVal, showNumber[0],
													showFlag, prompt(), true,
													help).trim();
									if (str.equals(oldVal))
										return oldVal;
									if (CMath.isInteger(str))
										return Integer.toString(CMath
												.s_int(str));
									if (CMLib
											.ableMapper()
											.getAbilityComponentMap()
									...
									if (str.trim().startsWith("(")) {
										error = CMLib
												...
												.addAbilityComponent(
														"ID=" + str,
														new Hashtable<String, List<AbilityComponent>>());
										if (error == null)
											return str;
									}
									mob.session()
											.println(
													"'"
															+ str
															+ "' is not an amount of material, a component key, or custom component list"
															+ (error == null ? ""
																	: "("
																			+ error
																			+ ")")
						...
						new AbilityParmEditorImpl("OPTIONAL_AMOUNT_REQUIRED",
								"Amt", PARMTYPE_NUMBER) {
							public void createChoices() {
							}
						...
						new AbilityParmEditorImpl("ITEM_BASE_VALUE", "Value",
								PARMTYPE_NUMBER) {
							public void createChoices() {
							}
						...
						new AbilityParmEditorImpl("ITEM_CLASS_ID", "Class ID",
								PARMTYPE_CHOICES) {
							public void createChoices() {
								Vector<Item> V = new Vector<Item>();
								V.addAll(new XVector<ClanItem>(CMClass
										.clanItems()));
								V.addAll(new XVector<Armor>(CMClass.armor()));
								V.addAll(new XVector<Item>(CMClass.basicItems()));
								V.addAll(new XVector<MiscMagic>(CMClass
										.miscMagic()));
								V.addAll(new XVector<Electronics>(CMClass
										.tech()));
								V.addAll(new XVector<Weapon>(CMClass.weapons()));
								Vector<Item> V2 = new Vector<Item>();
								Item I;
								for (Enumeration<Item> e = V.elements(); e
										.hasMoreElements();) {
									I = e.nextElement();
									if (I.isGeneric())
										V2.addElement(I);
						...
						new AbilityParmEditorImpl("CODED_WEAR_LOCATION",
								"Wear Locs", PARMTYPE_SPECIAL) {
							public int appliesToClass(Object o) {
								return ((o instanceof Armor) || (o instanceof MusicalInstrument)) ? 2
										: -1;
							}
							...
							public String webValue(HTTPRequest httpReq,
								...
								CMLib.ableParms().parseWearLocation(layerAtt,
										layers, wornLoc, logicalAnd, hardBonus,
										oldVal);
								if (httpReq.isUrlParameter(fieldName
										+ "_WORNDATA")) {
									wornLoc[0] = CMath.s_long(httpReq
											.getUrlParameter(fieldName
													+ "_WORNDATA"));
									for (int i = 1;; i++)
										if (httpReq.isUrlParameter(fieldName
												...
												+ (Integer.toString(i))))
											wornLoc[0] = wornLoc[0]
													| CMath.s_long(httpReq.getUrlParameter(fieldName
															+ "_WORNDATA"
															+ (Integer
																	.toString(i))));
										else
											break;
									logicalAnd[0] = httpReq.getUrlParameter(
											fieldName + "_ISTWOHANDED")
											.equalsIgnoreCase("on");
									layers[0] = CMath.s_short(httpReq
											.getUrlParameter(fieldName
													+ "_LAYER"));
									layerAtt[0] = 0;
									if ((httpReq.isUrlParameter(fieldName
								...
								return reconvert(layerAtt, layers, wornLoc,
										logicalAnd, hardBonus);
							}
							...
							public String webField(HTTPRequest httpReq,
								...
								str.append("\n\r<SELECT NAME=" + fieldName
										+ "_WORNDATA MULTIPLE>");
								Wearable.CODES codes = Wearable.CODES
										.instance();
								...
								str.append("<BR>\n\r<INPUT TYPE=RADIO NAME="
										+ fieldName
										+ "_ISTWOHANDED value=\"on\" "
										+ (logicalAnd[0] ? "CHECKED" : "")
										+ ">Is worn on All above Locations.");
								str.append("<BR>\n\r<INPUT TYPE=RADIO NAME="
										+ fieldName
										+ "_ISTWOHANDED value=\"\" "
										+ (logicalAnd[0] ? "" : "CHECKED")
										+ ">Is worn on ANY of the above Locations.");
								str.append("<BR>\n\rLayer: <INPUT TYPE=TEXT NAME="
										+ fieldName
										+ "_LAYER SIZE=5 VALUE=\""
										+ layers[0] + "\">");
								boolean seeThru = CMath.bset(layerAtt[0],
										Armor.LAYERMASK_SEETHROUGH);
								boolean multiWear = CMath.bset(layerAtt[0],
							...
							public String reconvert(short[] layerAtt,
									short[] layers, long[] wornLoc,
									boolean[] logicalAnd, double[] hardBonus) {
								StringBuffer newVal = new StringBuffer("");
								if ((layerAtt[0] != 0) || (layers[0] != 0)) {
									if (CMath.bset(layerAtt[0],
											Armor.LAYERMASK_MULTIWEAR))
										newVal.append('M');
									if (CMath.bset(layerAtt[0],
											Armor.LAYERMASK_SEETHROUGH))
								...
								Wearable.CODES codes = Wearable.CODES
										.instance();
								for (int wo = 1; wo < codes.total(); wo++) {
									if (CMath.bset(wornLoc[0],
											CMath.pow(2, wo - 1))) {
										if (needLink)
											newVal.append(logicalAnd[0] ? "&&"
													: "||");
										needLink = true;
										newVal.append(codes.name(wo)
												.toUpperCase());
									}
								}
							...
							public String convertFromItem(final ItemCraftor C,
									final Item I) {
								if (!(I instanceof Armor))
									return "HELD";
								Armor A = (Armor) I;
								final boolean[] logicalAnd = new boolean[] { I
										.rawLogicalAnd() };
								final long[] wornLoc = new long[] { I
										.rawProperLocationBitmap() };
								final double[] hardBonus = new double[] { 0.0 };
								final short[] layerAtt = new short[] { A
										.getLayerAttributes() };
								final short[] layers = new short[] { A
										.getClothingLayer() };
								return reconvert(layerAtt, layers, wornLoc,
										logicalAnd, hardBonus);
							}
							...
							public String commandLinePrompt(MOB mob,
								...
								CMLib.ableParms().parseWearLocation(layerAtt,
										layers, wornLoc, logicalAnd, hardBonus,
										oldVal);
								CMLib.genEd().wornLayer(mob, layerAtt, layers,
										++showNumber[0], showFlag);
								CMLib.genEd().wornLocation(mob, wornLoc,
										logicalAnd, ++showNumber[0], showFlag);
								return reconvert(layerAtt, layers, wornLoc,
										logicalAnd, hardBonus);
							}
						},
						new AbilityParmEditorImpl("CONTAINER_CAPACITY", "Cap.",
								PARMTYPE_NUMBER) {
							public int appliesToClass(Object o) {
						...
						new AbilityParmEditorImpl("BASE_ARMOR_AMOUNT", "Arm.",
								PARMTYPE_NUMBER) {
							public int appliesToClass(Object o) {
						...
						new AbilityParmEditorImpl("CONTAINER_TYPE", "Con.",
						...
						new AbilityParmEditorImpl("CONTAINER_TYPE_OR_LIDLOCK",
								"Con.", PARMTYPE_MULTICHOICES) {
							public void createChoices() {
								createBinaryChoices(Container.CONTAIN_DESCS);
						...
						new AbilityParmEditorImpl("CODED_SPELL_LIST",
								"Spell Affects", PARMTYPE_SPECIAL) {
							public void createChoices() {
							}
							...
							public boolean confirmValue(String oldVal) {
								...
								return CMClass.getAbility(oldVal
										.substring(0, y)) != null;
							}
							...
							public String rebuild(List<Ability> spells)
									throws CMException {
								StringBuffer newVal = new StringBuffer("");
								if (spells.size() == 1)
									...
									for (int s = 0; s < spells.size(); s++) {
										String txt = spells.get(s).text()
												.trim();
										if ((txt.indexOf(';') >= 0)
												|| (CMClass.getAbility(txt) != null))
											throw new CMException(
													"You may not have more than one spell when one of the spells parameters is a spell id or a ; character.");
							...
							public String[] fakeUserInput(String oldVal) {
								...
								List<Ability> spells = CMLib.ableParms()
										.getCodedSpells(oldVal);
								for (int s = 0; s < spells.size(); s++) {
							...
							public String webValue(HTTPRequest httpReq,
								...
								if (httpReq.isUrlParameter(fieldName
									...
									spells = CMLib.ableParms().getCodedSpells(
											oldVal);
								try {
							...
							public String webField(HTTPRequest httpReq,
								...
								for (int i = 0; i < spells.size(); i++) {
									...
									str.append("\n\r<SELECT ONCHANGE=\"EditAffect(this);\" NAME="
											+ fieldName
											+ "_AFFECT"
											+ (i + 1)
											+ ">");
									...
									String theparm = CMStrings.replaceAll(
											A.text(), "\"", "&quot;");
									str.append("\n\r<INPUT TYPE=TEXT SIZE=30 NAME="
											+ fieldName
											+ "_ADATA"
											+ (i + 1)
											+ " VALUE=\"" + theparm + "\">");
								...
								str.append("<TR><TD WIDTH=50%>");
								str.append("\n\r<SELECT ONCHANGE=\"AddAffect(this);\" NAME="
										+ fieldName
										+ "_AFFECT"
										+ (spells.size() + 1) + ">");
								str.append("<OPTION SELECTED VALUE=\"\">Select an Effect");
								for (Enumeration<Ability> a = CMClass
										.abilities(); a.hasMoreElements();) {
									Ability A = a.nextElement();
							...
							public String commandLinePrompt(MOB mob,
								...
								for (int s = 0; s < spells.size(); s++)
									rawCheck.append(spells.get(s).ID())
											.append(";")
											.append(spells.get(s).text())
											.append(";");
								...
								while (okToProceed) {
									okToProceed = false;
									CMLib.genEd().spells(mob, spells,
											showNumber[0], showFlag, true);
									StringBuffer sameCheck = new StringBuffer(
											"");
									for (int s = 0; s < spells.size(); s++)
										sameCheck.append(spells.get(s).ID())
												.append(';')
												.append(spells.get(s).text())
												.append(';');
									if (sameCheck.toString().equals(
											rawCheck.toString()))
										return oldVal;
									try {
						...
						new AbilityParmEditorImpl("BASE_DAMAGE", "Dmg.",
								PARMTYPE_NUMBER) {
							public int appliesToClass(Object o) {
							...
							public String convertFromItem(final ItemCraftor A,
								...
								if (I instanceof Weapon)
									return ""
											+ ((Weapon) I).basePhyStats()
													.damage();
								return "0";
							}
						},
						new AbilityParmEditorImpl("LID_LOCK", "Lid.",
								PARMTYPE_CHOICES) {
							public int appliesToClass(Object o) {
								return (o instanceof Container) ? 1 : -1;
							}
						...
						new AbilityParmEditorImpl("STATUE", "Statue",
								PARMTYPE_CHOICES) {
							public int appliesToClass(Object o) {
								return ((!(o instanceof Armor))
										&& (!(o instanceof Container)) && (!(o instanceof Drink))) ? 1
										: -1;
							}
						...
						new AbilityParmEditorImpl("RIDE_BASIS", "Ride",
								PARMTYPE_CHOICES) {
							public int appliesToClass(Object o) {
							...
							public void createChoices() {
								createChoices(new String[] { "", "CHAIR",
										"TABLE", "LADDER", "ENTER", "BED" });
							}
						...
						new AbilityParmEditorImpl("LIQUID_CAPACITY", "Liq.",
								PARMTYPE_NUMBER) {
							public int appliesToClass(Object o) {
						...
						new AbilityParmEditorImpl("WEAPON_CLASS", "WClas",
								PARMTYPE_CHOICES) {
							public int appliesToClass(Object o) {
								return (o instanceof Weapon) ? 2 : -1;
							}
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
								if (I instanceof Weapon)
									return Weapon.CLASS_DESCS[((Weapon) I)
											.weaponClassification()];
								return "0";
							}
						},
						new AbilityParmEditorImpl("SMOKE_FLAG", "Smoke",
								PARMTYPE_CHOICES) {
							public int appliesToClass(Object o) {
						...
						new AbilityParmEditorImpl("WEAPON_HANDS_REQUIRED",
								"Hand", PARMTYPE_NUMBER) {
							public int appliesToClass(Object o) {
								return (o instanceof Weapon) ? 2 : -1;
							}
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
								if (I instanceof Weapon)
									return ((Weapon) I).rawLogicalAnd() ? "2"
											: "1";
								return "";
							}
						},
						new AbilityParmEditorImpl("LIGHT_DURATION", "Dur.",
								PARMTYPE_NUMBER) {
							public int appliesToClass(Object o) {
								return (o instanceof Light) ? 5 : -1;
							}
						...
						new AbilityParmEditorImpl("CLAN_ITEM_CODENUMBER",
								"Typ.", PARMTYPE_CHOICES) {
							public int appliesToClass(Object o) {
								return (o instanceof ClanItem) ? 10 : -1;
							}

							public void createChoices() {
								createNumberedChoices(ClanItem.CI_DESC);
							}
						...
						new AbilityParmEditorImpl(
								"CLAN_EXPERIENCE_COST_AMOUNT", "Exp",
								PARMTYPE_NUMBER) {
							public void createChoices() {
							}
							...
							public String convertFromItem(final ItemCraftor A,
								...
								if (!(I instanceof ClanItem))
									return "100";
								if (I.getClass().getName().toString()
										.indexOf("Flag") > 0)
									return "2500";
								if (I.getClass().getName().toString()
										.indexOf("ClanItem") > 0)
									return "1000";
								if (I.getClass().getName().toString()
										.indexOf("GenClanSpecialItem") > 0)
									return "500";
								return "100";
							}
						},
						new AbilityParmEditorImpl("CLAN_AREA_FLAG", "Area",
								PARMTYPE_CHOICES) {
							public int appliesToClass(Object o) {
								return o.getClass().getName().toString()
										.indexOf("LawBook") > 0 ? 5 : -1;
							}
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
								return (I.getClass().getName().toString()
										.indexOf("LawBook") > 0) ? "AREA" : "";
							}
						},
						new AbilityParmEditorImpl("READABLE_TEXT", "Read",
						...
						new AbilityParmEditorImpl("REQUIRED_COMMON_SKILL_ID",
								"Common Skill", PARMTYPE_CHOICES) {
							public int appliesToClass(Object o) {
							...
							public void createChoices() {
								...
								Ability A = null;
								for (Enumeration<Ability> e = CMClass
										.abilities(); e.hasMoreElements();) {
									A = e.nextElement();
						...
						new AbilityParmEditorImpl("FOOD_DRINK", "ETyp",
								PARMTYPE_CHOICES) {
							public void createChoices() {
								createChoices(new String[] { "", "FOOD",
										"DRINK", "SOAP", "GenPerfume" });
							}
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
								final String str = (I.name() + " "
										+ I.displayText() + " " + I
										.description()).toUpperCase();
								if (str.startsWith("SOAP ")
										|| str.endsWith(" SOAP")
										|| (str.indexOf("SOAP") > 0))
									return "SOAP";
						...
						new AbilityParmEditorImpl("SMELL_LIST", "Smells",
								PARMTYPE_STRING) {
							public void createChoices() {
							}
						...
						new AbilityParmEditorImpl("RESOURCE_OR_KEYWORD",
								"Resc/Itm", PARMTYPE_SPECIAL) {
							public void createChoices() {
							}
							...
							public String webField(HTTPRequest httpReq,
								...
								str.append("\n\r<INPUT TYPE=RADIO NAME="
										+ fieldName + "_WHICH ");
								boolean rsc = (value.trim().length() == 0)
										|| (RawMaterial.CODES
												.FIND_IgnoreCase(value) >= 0);
								if (rsc)
									str.append("CHECKED ");
							...
							public String commandLinePrompt(MOB mob,
								...
								while (proceed && (!mob.session().isStopped())) {
									...
									if (str.length() == 0)
										return "";
									boolean isResource = CMParms.contains(
											RawMaterial.CODES.NAMES(), str);
									if ((!isResource)
											&& (mob.session() != null)
											&& (!mob.session().isStopped()))
										if (!mob.session().confirm(
												"You`ve entered a non-resource item keyword '"
														+ str + "', ok (Y/n)?",
												"Y"))
											proceed = true;
								}
						...
						new AbilityParmEditorImpl("RESOURCE_NAME_OR_HERB_NAME",
								"Resrc/Herb", PARMTYPE_SPECIAL) {
							public void createChoices() {
							}
							...
							public boolean confirmValue(String oldVal) {
								if (oldVal.trim().length() == 0)
									return true;
								if (!oldVal.endsWith("$")) {
									return CMParms.contains(
											RawMaterial.CODES.NAMES(), oldVal);
								}
								return true;
							}
							...
							public String[] fakeUserInput(String oldVal) {
								if (oldVal.endsWith("$"))
									return new String[] { oldVal.substring(0,
											oldVal.length() - 1) };
								return new String[] { oldVal };
							}
							...
							public String webValue(HTTPRequest httpReq,
									java.util.Map<String, String> parms,
									String oldVal, String fieldName) {
								AbilityParmEditor A = CMLib.ableParms()
										.getEditors()
										.get("RESOURCE_OR_KEYWORD");
								if (oldVal.endsWith("$"))
									oldVal = oldVal.substring(0,
											oldVal.length() - 1);
								String value = A.webValue(httpReq, parms,
										oldVal, fieldName);
								int r = RawMaterial.CODES
										.FIND_IgnoreCase(value);
								if (r >= 0)
							...
							public String webField(HTTPRequest httpReq,
								...
								AbilityParmEditor A = CMLib.ableParms()
										.getEditors()
										.get("RESOURCE_OR_KEYWORD");
								return A.webField(httpReq, parms, oldVal,
										fieldName);
							}

							public String webTableField(HTTPRequest httpReq,
									java.util.Map<String, String> parms,
									String oldVal) {
								if (oldVal.endsWith("$"))
									return oldVal.substring(0,
											oldVal.length() - 1);
								return oldVal;
							}
							...
							public String commandLinePrompt(MOB mob,
								...
								while (proceed && (!mob.session().isStopped())) {
									...
									if (oldVal.trim().endsWith("$"))
										oldVal = oldVal.trim().substring(0,
												oldVal.trim().length() - 1);
									str = CMLib
											.genEd()
											.prompt(mob,
													oldVal,
						...
						new AbilityParmEditorImpl("AMMO_TYPE", "Ammo",
								PARMTYPE_STRING) {
							public void createChoices() {
							}

							public int appliesToClass(Object o) {
								return ((o instanceof Weapon) || (o instanceof Ammunition)) ? 2
										: -1;
							}
							...
							public String convertFromItem(final ItemCraftor A,
								...
								if (I instanceof Ammunition)
									return ""
											+ ((Ammunition) I).ammunitionType();
								return "";
							}
						},
						new AbilityParmEditorImpl("AMMO_CAPACITY", "Ammo#",
								PARMTYPE_NUMBER) {
							public void createChoices() {
							}
						...
						new AbilityParmEditorImpl("MAXIMUM_RANGE", "Max",
								PARMTYPE_NUMBER) {
							public int appliesToClass(Object o) {
								return ((o instanceof Weapon) && (!(o instanceof Ammunition))) ? 2
										: -1;
						...
						new AbilityParmEditorImpl("RESOURCE_OR_MATERIAL",
								"Rsc/Mat", PARMTYPE_CHOICES) {
							public void createChoices() {
								XVector<String> V = new XVector<String>(
										RawMaterial.CODES.NAMES());
								Collections.sort(V);
								XVector<String> V2 = new XVector<String>(
										RawMaterial.Material.names());
								Collections.sort(V2);
								V.addAll(V2);
								createChoices(V);
							}
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
								if (CMStrings.containsWordIgnoreCase(I.Name(),
										"rice"))
									return "RICE";
						...
						new AbilityParmEditorImpl(
								"OPTIONAL_RESOURCE_OR_MATERIAL", "Rsc/Mat",
								PARMTYPE_CHOICES) {
							public void createChoices() {
								XVector<String> V = new XVector<String>(
										RawMaterial.CODES.NAMES());
						...
						new AbilityParmEditorImpl("HERB_NAME",
								"Herb Final Name", PARMTYPE_STRING) {
							public void createChoices() {
							}
						...
						new AbilityParmEditorImpl("RIDE_CAPACITY", "Ridrs",
								PARMTYPE_NUMBER) {
							public void createChoices() {
							}
						...
						new AbilityParmEditorImpl("METAL_OR_WOOD", "Metal",
								PARMTYPE_CHOICES) {
							public void createChoices() {
						...
						new AbilityParmEditorImpl("OPTIONAL_RACE_ID", "Race",
								PARMTYPE_SPECIAL) {
							public void createChoices() {
							...
							public boolean confirmValue(String oldVal) {
								if (oldVal.trim().length() == 0)
									return true;
								Vector<String> parsedVals = CMParms
										.parse(oldVal.toUpperCase());
							...
							public String webValue(HTTPRequest httpReq,
								...
								if (httpReq.isUrlParameter(fieldName + "_RACE")) {
									...
									for (int i = 0; httpReq
											.isUrlParameter(fieldName + "_RACE"
													+ id); id = "" + (++i))
										raceIDs.addElement(httpReq
												.getUrlParameter(
														fieldName + "_RACE"
																+ id)
												.toUpperCase().trim());
							...
							public String webField(HTTPRequest httpReq,
									java.util.Map<String, String> parms,
									String oldVal, String fieldName) {
								Vector<String> raceIDs = CMParms
										.parse(webValue(httpReq, parms, oldVal,
												fieldName).toUpperCase());
								StringBuffer str = new StringBuffer("");
								str.append("\n\r<SELECT NAME=" + fieldName
										+ "_RACE MULTIPLE>");
								str.append("<OPTION VALUE=\"\" "
										+ ((raceIDs.size() == 0) ? "SELECTED"
												: "") + ">");
								for (Enumeration<Race> e = CMClass.races(); e
										.hasMoreElements();) {
									Race R = e.nextElement();
									str.append("<OPTION VALUE=\""
											+ R.ID()
											+ "\" "
											+ ((raceIDs.contains(R.ID()
							...
							public String[] fakeUserInput(String oldVal) {
								...
								for (int p = 0; p < parsedVals.size(); p++) {
									Race R = CMClass.getRace(parsedVals
											.elementAt(p));
							...
							public String commandLinePrompt(MOB mob,
								...
								while ((mob.session() != null)
									...
									mob.tell(showNumber + ". " + prompt()
											+ ": '" + newVal + "'.");
									if ((showFlag != showNumber[0])
											&& (showFlag > -999))
										return newVal;
									Vector<String> parsedVals = CMParms
											.parse(newVal.toUpperCase());
									behave = mob
											.session()
											.prompt("Enter a race to add/remove (?)\n\r:",
													"");
									if (behave.length() > 0) {
										if (behave.equalsIgnoreCase("?"))
											...
											if (R != null) {
												if (parsedVals.contains(R.ID()
														.toUpperCase())) {
													mob.tell("'" + behave
															+ "' removed.");
													parsedVals.remove(R.ID()
															.toUpperCase()
															.trim());
													newVal = CMParms.combine(
															parsedVals, 0);
												} else {
													mob.tell(R.ID() + " added.");
													parsedVals
						...
						new AbilityParmEditorImpl("INSTRUMENT_TYPE",
								"Instrmnt", PARMTYPE_CHOICES) {
							public void createChoices() {
								createChoices(MusicalInstrument.TYPE_DESC);
							}

							public int appliesToClass(Object o) {
								return (o instanceof MusicalInstrument) ? 5
										: -1;
							}
							...
							public String convertFromItem(final ItemCraftor A,
								...
								if (I instanceof MusicalInstrument)
									return MusicalInstrument.TYPE_DESC[((MusicalInstrument) I)
											.instrumentType()];
								return "0";
							}
						},
						new AbilityParmEditorImpl("STONE_FLAG", "Stone",
								PARMTYPE_CHOICES) {
							public void createChoices() {
						...
						new AbilityParmEditorImpl("POSE_NAME", "Pose Word",
								PARMTYPE_ONEWORD) {
							public void createChoices() {
							}
						...
						new AbilityParmEditorImpl("POSE_DESCRIPTION",
								"Pose Description", PARMTYPE_STRING) {
							public void createChoices() {
							}
							...
							public String convertFromItem(final ItemCraftor A,
								...
								pose = CMStrings.replaceAll(pose, I.name(),
										"<S-NAME>");
								pose = CMStrings.replaceWord(pose, "himself",
										" <S-HIM-HERSELF>");
								pose = CMStrings.replaceWord(pose, "herself",
										" <S-HIM-HERSELF>");
								pose = CMStrings.replaceWord(pose, "his",
										" <S-HIS-HER>");
								pose = CMStrings.replaceWord(pose, "her",
										" <S-HIS-HER>");
								pose = CMStrings.replaceWord(pose, "him",
										" <S-HIM-HER>");
								pose = CMStrings.replaceWord(pose, "her",
										" <S-HIM-HER>");
								return pose;
						...
						new AbilityParmEditorImpl("WOOD_METAL_CLOTH", "",
								PARMTYPE_CHOICES) {
							public void createChoices() {
								createChoices(new String[] { "WOOD", "METAL",
										"CLOTH" });
							}
						...
						new AbilityParmEditorImpl("WEAPON_TYPE", "W.Type",
								PARMTYPE_CHOICES) {
							public int appliesToClass(Object o) {
								return (o instanceof Weapon) ? 2 : -1;
							}
							...
							public String convertFromItem(final ItemCraftor A,
									final Item I) {
								return (I instanceof Weapon) ? Weapon.TYPE_DESCS[((Weapon) I)
										.weaponType()] : "";
							}
						...
						new AbilityParmEditorImpl("ATTACK_MODIFICATION",
								"Att.", PARMTYPE_NUMBER) {
							public void createChoices() {
							}
						...
						new AbilityParmEditorImpl(
								"RESOURCE_NAME_AMOUNT_MATERIAL_REQUIRED",
								"Resrc/Amt", PARMTYPE_SPECIAL) {
							public void createChoices() {
								createChoices(RawMaterial.CODES.NAMES());
								choices().addElement("", "");
							}
							...
							public String webValue(HTTPRequest httpReq,
								...
								if (httpReq.isUrlParameter(fieldName
									...
									String amt = httpReq
											.getUrlParameter(fieldName
													+ "_AMOUNT");
									if ((rsc.trim().length() == 0)
											|| (rsc.equalsIgnoreCase("NOTHING"))
											|| (CMath.s_int(amt) <= 0))
										return "";
									return rsc + "/" + amt;
								}
							...
							public String webField(HTTPRequest httpReq,
								...
								str.append("</SELECT>");
								str.append("&nbsp;&nbsp;Amount: ");
								str.append("<INPUT TYPE=TEXT NAME=" + fieldName
										+ "_AMOUNT VALUE=" + amt + ">");
								return str.toString();
							}
							...
							public boolean confirmValue(String oldVal) {
								...
								if (x < 0)
									return false;
								if (!choices().getDimensionVector(1).contains(
										oldVal.substring(0, x)))
									return false;
							...
							public String[] fakeUserInput(String oldVal) {
								...
								if (x <= 0)
									return new String[] { "" };
								return new String[] { oldVal.substring(0, x),
										oldVal.substring(x + 1) };
							}
							...
							public String commandLinePrompt(MOB mob,
								...
								oldRsc = CMLib.genEd().prompt(mob, oldRsc,
										++showNumber[0], showFlag, prompt(),
										choices());
								if (oldRsc.length() > 0)
									return oldRsc
											+ "/"
											+ CMLib.genEd().prompt(mob, oldAmt,
													++showNumber[0], showFlag,
													prompt());
								return "";
							}
	...
	protected class AbilityRecipeDataImpl implements AbilityRecipeData {
		...
		public AbilityRecipeDataImpl(String recipeFilename, String recipeFormat) {
			...
			CMFile F = new CMFile(Resources.buildResourcePath("skills")
					+ recipeFilename, null, CMFile.FLAG_LOGERRORS);
			wasVFS = F.isVFSFile();
			...
			try {
				...
				if (editRow.size() == 0) {
					// classFieldIndex =
					// CMAbleParms.getClassFieldIndex(dataRow);
				} else
		...
		public DVector newRow(String classFieldData) {
			...
			for (int i = 0; i < editRow.size(); i++)
				if (i != keyIndex) {
					AbilityParmEditor A = getEditors().get(
							editRow.elementAt(i, 1));
	...
	protected abstract class AbilityParmEditorImpl implements AbilityParmEditor {
		...
		public AbilityParmEditorImpl(String fieldName, String shortHeader,
				int type) {
			ID = fieldName;
			fieldType = type;
			header = shortHeader;
		...
		public boolean confirmValue(String oldVal) {
			...
			case PARMTYPE_CHOICES:
				if (!choices.getDimensionVector(1).contains(oldVal))
					return choices.getDimensionVector(1).contains(
							oldVal.toUpperCase().trim());
				return true;
		...
		public String commandLinePrompt(MOB mob, String oldVal,
				int[] showNumber, int showFlag) throws java.io.IOException {
			String str = null;
			boolean emptyOK = false;
			boolean spaceOK = fieldType != PARMTYPE_ONEWORD;
			switch (fieldType) {
			...
			case PARMTYPE_STRING: {
				...
				while (proceed && (!mob.session().isStopped())) {
					str = CMLib
							.genEd()
							.prompt(mob, oldVal, showNumber[0], showFlag,
									prompt(), emptyOK).trim();
			...
			case PARMTYPE_NUMBER: {
				String newStr = CMLib.genEd().prompt(mob, oldVal,
						++showNumber[0], showFlag, prompt(), true);
			...
			case PARMTYPE_CHOICES:
				str = CMLib.genEd().promptMultiOrExtra(mob, oldVal,
						++showNumber[0], showFlag, prompt(), choices);
				break;
			case PARMTYPE_MULTICHOICES:
				str = CMLib.genEd().promptMultiOrExtra(mob, oldVal,
						++showNumber[0], showFlag, prompt(), choices);
		...
		public String webValue(HTTPRequest httpReq,
				java.util.Map<String, String> parms, String oldVal,
				String fieldName) {
			...
			case PARMTYPE_MULTICHOICES: {
				...
				long num = 0;
				int index = 0;
				for (; httpReq.isUrlParameter(fieldName + id); id = ""
						+ (++index)) {
		...
		public String webField(HTTPRequest httpReq,
			...
			case PARMTYPE_STRINGORNULL:
			case PARMTYPE_STRING:
				return "\n\r<INPUT TYPE=TEXT NAME=" + fieldName + " SIZE="
						+ textSize + " VALUE=\"" + webValue + "\">";
			case PARMTYPE_NUMBER:
				return "\n\r<INPUT TYPE=TEXT NAME=" + fieldName
						+ " SIZE=10 VALUE=\"" + webValue + "\">";
			case PARMTYPE_MULTICHOICES: {
				onChange = " MULTIPLE ";
				...
				if (CMath.isInteger(webValue)) {
					...
					for (int i = 0; i < choices.size(); i++) {
						int bitVal = CMath.s_int((String) choices.elementAt(i,
								1));
						if ((bitVal > 0) && (CMath.bset(bits, bitVal)))
							choiceValues.addElement((String) choices.elementAt(
									i, 1));
					}
				}
			}
			...
			for (; e.hasMoreElements();) {
				...
				if (o instanceof String)
					choices.addElement(o,
							CMStrings.capitalizeAndLower((String) o));
					...
					choices.addElement(((Environmental) o).ID(),
							((Environmental) o).ID());
			}
			return choices;
		}
