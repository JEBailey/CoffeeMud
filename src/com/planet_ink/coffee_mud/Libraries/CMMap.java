...
public class CMMap extends StdLibrary implements WorldMap {
	...
	public Map<Integer, List<WeakReference<MsgListener>>> globalHandlers = new SHashtable<Integer, List<WeakReference<MsgListener>>>();
	public Map<String, SLinkedList<LocatedPair>> scriptHostMap = new STreeMap<String, SLinkedList<LocatedPair>>();
	...
	public void addArea(Area newOne) {
		...
		if ((newOne instanceof SpaceObject)
				&& (!space.contains((SpaceObject) newOne)))
			space.insert((SpaceObject) newOne);
	}

	public void delArea(Area oneToDel) {
		areasList.remove(oneToDel);
		if ((oneToDel instanceof SpaceObject)
				&& (space.contains((SpaceObject) oneToDel)))
	...
	public Area findAreaStartsWith(String calledThis) {
		final boolean disableCaching = CMProps
				.getBoolVar(CMProps.Bool.MAPFINDSNOCACHE);
	...
	public void addGlobalHandler(MsgListener E, int category) {
		if (E == null)
			return;
		List<WeakReference<MsgListener>> V = globalHandlers.get(Integer
				.valueOf(category));
		if (V == null) {
		...
		synchronized (V) {
			for (Iterator<WeakReference<MsgListener>> i = V.iterator(); i
					.hasNext();) {
	...
	public MOB deity() {
		...
		if ((deityStandIn == null) || (deityStandIn.amDestroyed())
				|| (deityStandIn.amDead()) || (deityStandIn.location() == null)
	...
	public long getDistanceFrom(SpaceObject O1, SpaceObject O2) {
		return Math.round(Math.sqrt(CMath.mul(
				(O1.coordinates()[0] - O2.coordinates()[0]),
				(O1.coordinates()[0] - O2.coordinates()[0]))
				+ CMath.mul((O1.coordinates()[1] - O2.coordinates()[1]),
						(O1.coordinates()[1] - O2.coordinates()[1]))
				+ CMath.mul((O1.coordinates()[2] - O2.coordinates()[2]),
						(O1.coordinates()[2] - O2.coordinates()[2]))));
	}

	public static void moveSpaceObject(SpaceObject O, double[] newDirection,
			long newAccelleration) {
		...
		final double anglesDelta = (directionCombinedAngle > facingCombinedAngle) ? (directionCombinedAngle - facingCombinedAngle)
				: (facingCombinedAngle - directionCombinedAngle);
		final double yawDelta = (directionYaw > facingYaw) ? (directionYaw - facingYaw)
				: (facingYaw - directionYaw);
		final double pitchDelta = (directionPitch > facingPitch) ? (directionPitch - facingPitch)
				: (facingPitch - directionPitch);
		...
		if (yawDelta < 0.1)
			...
			newDirectionYaw = directionYaw
					+ ((directionYaw > facingYaw) ? -(accelerationMultiplier * Math
							.sin(yawDelta)) : (accelerationMultiplier * Math
							.sin(yawDelta)));
		...
		if (pitchDelta < 0.1)
			...
			newDirectionPitch = directionPitch
					+ ((directionPitch > facingPitch) ? -(accelerationMultiplier * Math
							.sin(pitchDelta)) : (accelerationMultiplier * Math
							.sin(pitchDelta)));
		if (newDirectionPitch < 0.0)
			newDirectionPitch = (Math.PI * 2.0) + newDirectionPitch;

		double newSpeed = currentSpeed
				+ Math.round(acceleration * Math.cos(anglesDelta));
		if (newSpeed < 0) {
			newSpeed = -newSpeed;
	...
	public void moveSpaceObject(SpaceObject O) {
		if (O.speed() > 0) {
			...
			moveSpaceObject(O,
					O.coordinates()[0] + Math.round(CMath.mul(O.speed(), x1)),
					O.coordinates()[1] + Math.round(CMath.mul(O.speed(), y1)),
					O.coordinates()[2] + Math.round(CMath.mul(O.speed(), z1)));
		}
	}

	public long[] getLocation(long[] oldLocation, double[] direction,
			long distance) {
	...
	public long getRelativeSpeed(SpaceObject O1, SpaceObject O2) {
		// TODO: fix this -- multiplying things by coordinates is a Bad Idea
		// (TM)
		return Math.round(Math.sqrt(((O1.speed() * O1.coordinates()[0])
				- (O2.speed() * O2.coordinates()[0])
				* (O1.speed() * O1.coordinates()[0]) - (O2.speed() * O2
				.coordinates()[0]))
				+ ((O1.speed() * O1.coordinates()[1])
						- (O2.speed() * O2.coordinates()[1])
						* (O1.speed() * O1.coordinates()[1]) - (O2.speed() * O2
						.coordinates()[1]))
				+ ((O1.speed() * O1.coordinates()[2])
						- (O2.speed() * O2.coordinates()[2])
						* (O1.speed() * O1.coordinates()[2]) - (O2.speed() * O2
						.coordinates()[2]))));
	}
	...
	public SpaceObject getSpaceObject(CMObject o, boolean ignoreMobs) {
		...
		if (o instanceof MOB)
			return ignoreMobs ? null : getSpaceObject(((MOB) o).location(),
					false);
		if (o instanceof Room)
			return getSpaceObject(((Room) o).getArea(), ignoreMobs);
		if (o instanceof Area)
			for (Enumeration<Area> a = ((Area) o).getParents(); a
					.hasMoreElements();) {
	...
	public List<SpaceObject> getSpaceObjectsWithin(final SpaceObject ofObj,
		...
		synchronized (space) {
			space.query(within,
					new BoundedObject.BoundedCube(ofObj.coordinates(),
							maxDistance));
		}
		if (within.size() <= 1)
			return within;
	...
	public String createNewExit(Room from, Room room, int direction) {
		...
		if (opRoom != null)
			reverseRoom = opRoom.rawDoors()[Directions
		...
		synchronized (("SYNC" + room.roomID()).intern()) {
			...
			if (room.rawDoors()[Directions.getOpDirectionCode(direction)] == null) {
				...
				room.setRawExit(Directions.getOpDirectionCode(direction),
						thisExit);
	...
	public List<Room> findRooms(Enumeration<Room> rooms, MOB mob,
			String srchStr, boolean displayOnly, int timePct) {
		final Vector<Room> roomsV = new Vector<Room>();
		if ((srchStr.charAt(0) == '#') && (mob != null)
				&& (mob.location() != null))
			addWorldRoomsLiberally(roomsV, getRoom(mob.location().getArea()
					.Name()
					+ srchStr));
		else
			addWorldRoomsLiberally(roomsV, getRoom(srchStr));
		addWorldRoomsLiberally(roomsV,
				findRooms(rooms, mob, srchStr, displayOnly, false, timePct));
		return roomsV;
	}

	public Room findFirstRoom(Enumeration<Room> rooms, MOB mob, String srchStr,
			boolean displayOnly, int timePct) {
		final Vector<Room> roomsV = new Vector<Room>();
		if ((srchStr.charAt(0) == '#') && (mob != null)
				&& (mob.location() != null))
			addWorldRoomsLiberally(roomsV, getRoom(mob.location().getArea()
		...
		addWorldRoomsLiberally(roomsV,
				findRooms(rooms, mob, srchStr, displayOnly, true, timePct));
		if (roomsV.size() > 0)
			return roomsV.firstElement();
		return null;
	}
	...
	public List<Room> findRooms(Enumeration<Room> rooms, MOB mob,
			String srchStr, boolean displayOnly, boolean returnFirst,
			int timePct) {
		...
		while (enumSet.hasMoreElements()) {
			findRoomsByDisplay(mob, enumSet, foundRooms, srchStr, returnFirst,
					delay);
			if ((returnFirst) && (foundRooms.size() > 0))
				return foundRooms;
			if (enumSet.hasMoreElements())
				try {
		...
		if (!displayOnly) {
			enumSet = completeRooms.elements();
			while (enumSet.hasMoreElements()) {
				findRoomsByDesc(mob, enumSet, foundRooms, srchStr, returnFirst,
						delay);
				if ((returnFirst) && (foundRooms.size() > 0))
					return foundRooms;
				if (enumSet.hasMoreElements())
	...
	protected void findRoomsByDisplay(MOB mob, Enumeration<Room> rooms,
			List<Room> foundRooms, String srchStr, boolean returnFirst,
			long maxTime) {
		...
		try {
			...
			for (; rooms.hasMoreElements();) {
				...
				if ((CMLib.english().containsString(
						CMStrings.removeColors(room.displayText(mob)), srchStr))
	...
	protected void findRoomsByDesc(MOB mob, Enumeration<Room> rooms,
			List<Room> foundRooms, String srchStr, boolean returnFirst,
			long maxTime) {
		long startTime = System.currentTimeMillis();
		try {
			...
			for (; rooms.hasMoreElements();) {
				...
				if ((CMLib.english().containsString(
						CMStrings.removeColors(room.description()), srchStr))
						&& ((mob == null) || CMLib.flags().canAccess(mob, room)))
					foundRooms.add(room);
				if ((useTimer)
						&& ((System.currentTimeMillis() - startTime) > maxTime))
	...
	public List<MOB> findInhabitants(Enumeration<Room> rooms, MOB mob,
			String srchStr, int timePct) {
		return findInhabitants(rooms, mob, srchStr, false, timePct);
	}

	public MOB findFirstInhabitant(Enumeration<Room> rooms, MOB mob,
			String srchStr, int timePct) {
		final List<MOB> found = findInhabitants(rooms, mob, srchStr, true,
				timePct);
		if (found.size() > 0)
			return found.get(0);
		return null;
	...
	public List<MOB> findInhabitants(Enumeration<Room> rooms, MOB mob,
			String srchStr, boolean returnFirst, int timePct) {
		...
		for (; rooms.hasMoreElements();) {
			room = rooms.nextElement();
			if ((room != null)
					&& (allRoomsAllowed || CMLib.flags().canAccess(mob, room))) {
	...
	public List<Item> findInventory(Enumeration<Room> rooms, MOB mob,
			String srchStr, int timePct) {
		return findInventory(rooms, mob, srchStr, false, timePct);
	}

	public Item findFirstInventory(Enumeration<Room> rooms, MOB mob,
			String srchStr, int timePct) {
		final List<Item> found = findInventory(rooms, mob, srchStr, true,
				timePct);
		if (found.size() > 0)
			return found.get(0);
		return null;
	...
	public List<Environmental> findShopStock(Enumeration<Room> rooms, MOB mob,
			String srchStr, int timePct) {
		return findShopStock(rooms, mob, srchStr, false, false, timePct);
	}

	public Environmental findFirstShopStock(Enumeration<Room> rooms, MOB mob,
			String srchStr, int timePct) {
		final List<Environmental> found = findShopStock(rooms, mob, srchStr,
				true, false, timePct);
		if (found.size() > 0)
			return found.get(0);
		return null;
	...
	public List<Environmental> findShopStockers(Enumeration<Room> rooms,
			MOB mob, String srchStr, int timePct) {
		return findShopStock(rooms, mob, srchStr, false, true, timePct);
	}

	public Environmental findFirstShopStocker(Enumeration<Room> rooms, MOB mob,
			String srchStr, int timePct) {
		final List<Environmental> found = findShopStock(rooms, mob, srchStr,
				true, true, timePct);
		if (found.size() > 0)
	...
	public List<Environmental> findShopStock(Enumeration<Room> rooms, MOB mob,
			String srchStr, boolean returnFirst, boolean returnStockers,
			int timePct) {
		...
		if (rooms == null) {
			for (Enumeration<MOB> e = CMLib.players().players(); e
				...
				if (M != null) {
					...
					if ((SK != null) && (!stocks.contains(SK))) {
						stocks.add(SK);
						Iterator<Environmental> ei = SK.getShop()
								.getStoreInventory(srchStr);
						if (ei.hasNext()) {
							if (returnFirst)
								return (returnStockers) ? new XVector<Environmental>(
										M) : new XVector<Environmental>(ei);
							if (returnStockers)
								found.add(M);
							else
								found.addAll(ei);
			...
			for (; rooms.hasMoreElements();) {
				Room room = rooms.nextElement();
				if ((room != null)
						&& (allRoomsAllowed || CMLib.flags().canAccess(mob,
								room))) {
					...
					if ((SK != null) && (!stocks.contains(SK))) {
						...
						if (ei.hasNext()) {
							if (returnFirst)
								return (returnStockers) ? new XVector<Environmental>(
										room) : new XVector<Environmental>(ei);
							if (returnStockers)
								found.add(room);
							else
								found.addAll(ei);
		...
		for (Iterator<Area> i = areas.iterator(); i.hasNext();) {
			...
			if ((SK != null) && (!stocks.contains(SK))) {
				stocks.add(SK);
				Iterator<Environmental> ei = SK.getShop().getStoreInventory(
						srchStr);
				if (ei.hasNext()) {
					if (returnFirst)
						return (returnStockers) ? new XVector<Environmental>(A)
								: new XVector<Environmental>(ei);
					if (returnStockers)
						found.add(A);
					else
	...
	public List<Item> findRoomItems(Enumeration<Room> rooms, MOB mob,
			String srchStr, boolean anyItems, int timePct) {
		return findRoomItems(rooms, mob, srchStr, anyItems, false, timePct);
	}

	public Item findFirstRoomItem(Enumeration<Room> rooms, MOB mob,
			String srchStr, boolean anyItems, int timePct) {
		List<Item> found = findRoomItems(rooms, mob, srchStr, anyItems, true,
				timePct);
		if (found.size() > 0)
			return found.get(0);
		return null;
	...
	public List<Item> findRoomItems(Enumeration<Room> rooms, MOB mob,
			String srchStr, boolean anyItems, boolean returnFirst, int timePct) {
		...
		for (; rooms.hasMoreElements();) {
			...
			if ((room != null)
					&& (allRoomsAllowed || CMLib.flags().canAccess(mob, room))) {
				found.addAll(anyItems ? room.findItems(srchStr) : room
						.findItems(null, srchStr));
				if ((returnFirst) && (found.size() > 0))
					return found;
			}
			if ((useTimer)
	...
	public Iterator<String> bankChains(Area AreaOrNull) {
		...
		for (Banker B : bankList)
			if ((!H.contains(B.bankChain()))
					&& ((AreaOrNull == null) || (getStartArea(B) == AreaOrNull) || (AreaOrNull
							.isChild(getStartArea(B)))))
				H.add(B.bankChain());
		return H.iterator();
	}
	...
	public void renameRooms(Area A, String oldName, List<Room> allMyDamnRooms) {
		...
		for (Room R : allMyDamnRooms) {
			synchronized (("SYNC" + R.roomID()).intern()) {
				...
				R.setArea(A);
				if (oldName != null) {
					if (R.roomID().toUpperCase()
							.startsWith(oldName.toUpperCase() + "#")) {
						Room R2 = getRoom(A.Name() + "#"
								+ R.roomID().substring(oldName.length() + 1));
						if ((R2 == null)
								|| (!R2.roomID().startsWith(A.Name() + "#"))) {
							String oldID = R.roomID();
							R.setRoomID(A.Name()
									+ "#"
									+ R.roomID()
											.substring(oldName.length() + 1));
	...
	public boolean isClearableRoom(Room R) {
		...
		for (int i = 0; i < R.numInhabitants(); i++) {
			...
			if (M == null)
				continue;
			sR = M.getStartRoom();
			if ((sR != null) && (!sR.roomID().equals(R.roomID())))
				return false;
			if (M.session() != null)
				return false;
		}
		...
		for (int i = 0; i < R.numItems(); i++) {
			I = R.getItem(i);
			if ((I != null)
					&& ((I.expirationDate() != 0) || ((I instanceof DeadBody) && (((DeadBody) I)
							.playerCorpse()))))
				return false;
		}
	...
	public boolean explored(Room R) {
		if ((R == null)
				|| (CMath.bset(R.phyStats().sensesMask(),
						PhyStats.SENSE_ROOMUNEXPLORABLE))
				|| (R.getArea() == null))
			return false;
		return false;
	}
	...
	public static class AreaEnumerator implements Enumeration<Room> {
		...
		public boolean hasMoreElements() {
			...
			while ((curRoomEnumeration == null)
					|| (!curRoomEnumeration.hasMoreElements())) {
				if (!curAreaEnumeration.hasMoreElements())
					return false;
				if (addSkys)
					curRoomEnumeration = curAreaEnumeration.nextElement()
							.getFilledProperMap();
				else
					curRoomEnumeration = curAreaEnumeration.nextElement()
							.getProperMap();
	...
	public void obliterateRoom(Room deadRoom) {
		for (final Enumeration<Ability> a = deadRoom.effects(); a
				.hasMoreElements();) {
			final Ability A = a.nextElement();
			if (A != null) {
				A.unInvoke();
	...
	public ThreadGroup getOwnedThreadGroup(CMObject E) {
		...
		if (area != null) {
			...
			if ((theme > 0) && (theme < Area.THEME_NAMES.length))
				return CMProps.getPrivateOwner(Area.THEME_NAMES[theme]
						+ "AREAS");
		}
		return null;
	}
	...
	public void emptyRoom(Room room, Room bringBackHere) {
		...
		for (int m = 0; m < inhabs.size(); m++) {
			...
			if (bringBackHere != null)
				...
				continue;
			else if ((M.getStartRoom() == null) || (M.getStartRoom() == room)
		...
		if (room instanceof GridLocale)
			for (Iterator<Room> r = ((GridLocale) room).getExistingRooms(); r
					.hasNext();)
	...
	public void resetRoom(Room room, boolean rebuildGrids) {
		...
		synchronized (("SYNC" + room.roomID()).intern()) {
			...
			if (resetMsg == null)
				resetMsg = CMClass.getMsg(CMClass.sampleMOB(), room,
						CMMsg.MSG_ROOMRESET, null);
			resetMsg.setTarget(room);
			room.executeMsg(room, resetMsg);
			emptyRoom(room, null);
			for (final Enumeration<Ability> a = room.effects(); a
					.hasMoreElements();) {
				final Ability A = a.nextElement();
	...
	public Room findWorldRoomLiberally(MOB mob, String cmd,
			String srchWhatAERIPMVK, int timePct, long maxMillis) {
		List<Room> rooms = findWorldRoomsLiberally(mob, cmd, srchWhatAERIPMVK,
				null, true, timePct, maxMillis);
		if ((rooms != null) && (rooms.size() != 0))
			return rooms.get(0);
		return null;
	}

	public List<Room> findWorldRoomsLiberally(MOB mob, String cmd,
			String srchWhatAERIPMVK, int timePct, long maxMillis) {
		return findWorldRoomsLiberally(mob, cmd, srchWhatAERIPMVK, null, false,
				timePct, maxMillis);
	}

	public Room findAreaRoomLiberally(MOB mob, Area A, String cmd,
			String srchWhatAERIPMVK, int timePct) {
		List<Room> rooms = findWorldRoomsLiberally(mob, cmd, srchWhatAERIPMVK,
				A, true, timePct, 120);
		if ((rooms != null) && (rooms.size() != 0))
			return rooms.get(0);
	...
	public List<Room> findAreaRoomsLiberally(MOB mob, Area A, String cmd,
			String srchWhatAERIPMVK, int timePct) {
		return findWorldRoomsLiberally(mob, cmd, srchWhatAERIPMVK, A, false,
				timePct, 120);
	}

	protected Room addWorldRoomsLiberally(List<Room> rooms,
			List<? extends Environmental> choicesV) {
		if (choicesV == null)
			return null;
		if (rooms != null) {
			...
			while (((room == null) || (room.roomID().length() == 0))
					&& ((++tries) < 200))
				room = roomLocation(choicesV.get(CMLib.dice().roll(1,
						choicesV.size(), -1)));
			return room;
		}
	}
	...
	protected boolean enforceTimeLimit(final long startTime,
	...
	public Map<String, List<MOB>> getMOBFinder() {
		Map<String, List<MOB>> finder = (Map<String, List<MOB>>) Resources
				.getResource("SYSTEM_MOB_FINDER_CACHE");
		if (finder == null) {
			finder = new PrioritizingLimitedMap<String, List<MOB>>(10,
					EXPIRE_5MINS, EXPIRE_10MINS, 100);
	...
	public Map<String, Area> getAreaFinder() {
		Map<String, Area> finder = (Map<String, Area>) Resources
				.getResource("SYSTEM_AREA_FINDER_CACHE");
		if (finder == null) {
			finder = new PrioritizingLimitedMap<String, Area>(50,
					EXPIRE_30MINS, EXPIRE_1HOUR, 100);
	...
	public Map<String, List<Item>> getRoomItemFinder() {
		Map<String, List<Item>> finder = (Map<String, List<Item>>) Resources
				.getResource("SYSTEM_RITEM_FINDER_CACHE");
		if (finder == null) {
			finder = new PrioritizingLimitedMap<String, List<Item>>(10,
					EXPIRE_5MINS, EXPIRE_10MINS, 100);
	...
	public Map<String, List<Item>> getInvItemFinder() {
		Map<String, List<Item>> finder = (Map<String, List<Item>>) Resources
				.getResource("SYSTEM_IITEM_FINDER_CACHE");
		if (finder == null) {
			finder = new PrioritizingLimitedMap<String, List<Item>>(10,
					EXPIRE_1MIN, EXPIRE_10MINS, 100);
	...
	public Map<String, List<Environmental>> getStockFinder() {
		Map<String, List<Environmental>> finder = (Map<String, List<Environmental>>) Resources
				.getResource("SYSTEM_STOCK_FINDER_CACHE");
		if (finder == null) {
			finder = new PrioritizingLimitedMap<String, List<Environmental>>(
					10, EXPIRE_10MINS, EXPIRE_1HOUR, 100);
	...
	public Map<String, List<Room>> getRoomFinder() {
		Map<String, List<Room>> finder = (Map<String, List<Room>>) Resources
				.getResource("SYSTEM_ROOM_FINDER_CACHE");
		if (finder == null) {
			finder = new PrioritizingLimitedMap<String, List<Room>>(20,
					EXPIRE_20MINS, EXPIRE_1HOUR, 100);
			Resources.submitResource("SYSTEM_ROOM_FINDER_CACHE", finder);
		}
		return finder;
	}

	protected List<Room> findWorldRoomsLiberally(MOB mob, String cmd,
			String srchWhatAERIPMVK, Area A, boolean returnFirst, int timePct,
			long maxMillis) {
		Room room = null;
		...
		// as soon as they are put away -- that's why the limited caches time
		// them out!
		final boolean disableCaching = CMProps
				.getBoolVar(CMProps.Bool.MAPFINDSNOCACHE);
		...
		if (searchRooms) {
			...
			if ((dirCode >= 0) && (curRoom != null))
				room = addWorldRoomsLiberally(rooms,
						curRoom.rawDoors()[dirCode]);
			if (room == null)
		...
		if (room == null) {
			...
			if ((cmd.charAt(0) == '#') && (curRoom != null) && (searchRooms))
				room = addWorldRoomsLiberally(rooms, getRoom(curRoom.getArea()
						.Name() + cmd));
			else {
				String srchStr = cmd;
				...
				if (searchInhabs && room == null) {
					...
					if ((mob == null) || (mob.isMonster())) {
						candidates = checkMOBCachedList(finder.get(srchStr
								.toLowerCase()));
						if (returnFirst && (candidates != null)
								&& (candidates.size() > 1))
							candidates = new XVector<MOB>(candidates.get(0));
					}
					if (candidates == null) {
						candidates = findInhabitants(rightLiberalMap(A), mob,
								srchStr, returnFirst, timePct);
						if ((!disableCaching) && (!returnFirst)
				...
				if (searchRooms && room == null) {
					...
					if (candidates == null) {
						candidates = findRooms(rightLiberalMap(A), mob,
								srchStr, false, returnFirst, timePct);
						if ((!disableCaching) && (!returnFirst)
								&& ((mob == null) || (mob.isMonster())))
							finder.put(srchStr.toLowerCase(), candidates);
					}
				...
				if (searchItems && room == null) {
					...
					if ((mob == null) || (mob.isMonster())) {
						candidates = checkRoomItemCachedList(finder.get(srchStr
								.toLowerCase()));
						if (returnFirst && (candidates != null)
								&& (candidates.size() > 1))
					...
					if (candidates == null) {
						candidates = findRoomItems(rightLiberalMap(A), mob,
								srchStr, false, returnFirst, timePct);
						if ((!disableCaching) && (!returnFirst)
								&& ((mob == null) || (mob.isMonster())))
							finder.put(srchStr.toLowerCase(), candidates);
				...
				if (searchInventories && room == null) {
					...
					List<Item> candidates = null;
					if ((mob == null) || (mob.isMonster())) {
						candidates = checkInvCachedList(finder.get(srchStr
								.toLowerCase()));
						if (returnFirst && (candidates != null)
								&& (candidates.size() > 1))
							candidates = new XVector<Item>(candidates.get(0));
					...
					if (candidates == null) {
						candidates = findInventory(rightLiberalMap(A), mob,
								srchStr, returnFirst, timePct);
						if ((!disableCaching) && (!returnFirst)
								&& ((mob == null) || (mob.isMonster())))
							finder.put(srchStr.toLowerCase(), candidates);
				...
				if (searchStocks && room == null) {
					...
					if ((mob == null) || (mob.isMonster())) {
						...
						if (returnFirst && (candidates != null)
								&& (candidates.size() > 1))
							candidates = new XVector<Environmental>(
									candidates.get(0));
					}
					if (candidates == null) {
						candidates = findShopStock(rightLiberalMap(A), mob,
								srchStr, returnFirst, false, timePct);
						if ((!disableCaching) && (!returnFirst)
								&& ((mob == null) || (mob.isMonster())))
							finder.put(srchStr.toLowerCase(), candidates);
					}
	...
	public boolean isHere(CMObject E2, Room here) {
		if (E2 == null)
			...
			return true;
		else if ((E2 instanceof MOB) && (((MOB) E2).location() == here))
			return true;
		else if ((E2 instanceof Item) && (((Item) E2).owner() == here))
			return true;
		else if ((E2 instanceof Item) && (((Item) E2).owner() != null)
	...
	protected PairVector<MOB, String> getAllPlayersHere(Area area,
	...
	public void registerWorldObjectLoaded(Area area, Room room, final CMObject o) {
		...
		if (o instanceof PhysicalAgent) {
			...
			if (o instanceof MOB)
				for (final Enumeration<Item> i = ((MOB) o).items(); i
						.hasMoreElements();)
					addScriptHost(area, room, i.nextElement());
		}
	}

	protected void cleanScriptHosts(final SLinkedList<LocatedPair> hosts,
			final PhysicalAgent oneToDel, final boolean fullCleaning) {
		PhysicalAgent PA;
		for (final Iterator<LocatedPair> w = hosts.iterator(); w.hasNext();) {
			...
			if (W == null)
				...
				PA = W.obj();
				if ((PA == null) || (PA == oneToDel) || (PA.amDestroyed())
	...
	protected boolean isAQualifyingScriptHost(final PhysicalAgent host) {
		if (host == null)
			return false;
		for (final Enumeration<Behavior> e = host.behaviors(); e
				.hasMoreElements();) {
			final Behavior B = e.nextElement();
			if ((B != null) && B.isSavable() && (B instanceof ScriptingEngine))
				return true;
		}
		for (final Enumeration<ScriptingEngine> e = host.scripts(); e
				.hasMoreElements();) {
	...
	protected boolean isAScriptHost(final SLinkedList<LocatedPair> hosts,
	...
	protected void addScriptHost(final Area area, final Room room,
			final PhysicalAgent host) {
	...
	protected void delScriptHost(Area area, final PhysicalAgent oneToDel) {
		...
		if (area == null)
			return;
		synchronized (getScriptHostSemaphore(area)) {
			final SLinkedList<LocatedPair> hosts = scriptHostMap.get(area
					.Name());
			if (hosts == null)
				return;
	...
	public Enumeration<LocatedPair> scriptHosts(final Area area) {
		...
		if (V.size() == 0)
			return EmptyEnumeration.INSTANCE;
		final MultiListEnumeration<LocatedPair> me = new MultiListEnumeration<LocatedPair>(
				V, true);
	...
	public boolean activate() {
		if (serviceClient == null) {
			...
			serviceClient = CMLib.threads().startTickDown(this,
					...
					MudHost.TIME_SAVETHREAD_SLEEP, 1);
		}
		return true;
	}
	...
	public boolean shutdown() {
		...
		if (CMLib.threads().isTicking(this,
				TICKID_SUPPORT | Tickable.TICKID_SOLITARYMASK)) {
			CMLib.threads().deleteTick(this,
					TICKID_SUPPORT | Tickable.TICKID_SOLITARYMASK);
			serviceClient = null;
		}
		return true;
	...
	public void roomMaintSweep() {
		...
		try {
			...
			CMMsg expireMsg = CMClass.getMsg(expireM, R, null,
				...
				if (stuffToGo.size() > 0) {
					...
					for (int s = 0; s < stuffToGo.size(); s++) {
						...
						if (R.okMessage(expireM, expireMsg))
							...
							success = false;
						if (debug)
							Log.sysOut("UTILITHREAD", "Expired " + E.Name()
									+ " in " + getExtendedRoomID(R) + ": "
									+ success);
					}
					stuffToGo.clear();
				}
			...
			for (int r = 0; r < roomsToGo.size(); r++) {
				...
				expireM.setLocation(R);
				expireMsg.setTarget(R);
				setThreadStatus(serviceClient, "expirating room "
						+ getExtendedRoomID(R));
				if (debug) {
		...
		try {
			for (Enumeration<Room> r = rooms(); r.hasMoreElements();) {
				...
				if (T != null) {
					setThreadStatus(serviceClient,
							"checking title in " + R.roomID() + ": "
		...
		try {
			for (Enumeration<Room> r = rooms(); r.hasMoreElements();) {
				...
				for (int m = 0; m < R.numInhabitants(); m++) {
					...
					if ((mob != null) && (mob.lastTickedDateTime() > 0)
							&& (mob.lastTickedDateTime() < lastDateTime)) {
						final boolean ticked = CMLib.threads().isTicking(mob,
								Tickable.TICKID_MOB);
						final boolean isDead = mob.amDead();
						final Room startR = mob.getStartRoom();
						final String wasFrom = (startR != null) ? startR
								.roomID() : "NULL";
						if (!ticked) {
							if (CMLib.players().getPlayer(mob.Name()) == null) {
								if (ticked) {
									// we have a dead group.. let the group
									...
									Log.errOut(
											...
											mob.name()
													+ " in room "
													+ R.roomID()
													+ " unticked in dead group (Home="
													+ wasFrom
													+ ") since: "
													+ CMLib.time()
															.date2String(
																	mob.lastTickedDateTime())
													+ ".");
									continue;
								} else
									Log.errOut(
											...
											mob.name()
													...
													+ "."
													+ (ticked ? ""
															: "  This mob has been destroyed. May he rest in peace."));
							} else
								Log.errOut(
										serviceClient.getName(),
										"Player "
												...
												+ "."
												+ (ticked ? ""
														: "  This mob has been put aside."));
							setThreadStatus(serviceClient,
									"destroying unticked mob " + mob.name());
							if (CMLib.players().getPlayer(mob.Name()) == null)
								mob.destroy();
							R.delInhabitant(mob);
							setThreadStatus(serviceClient, "checking");
	...
	protected String cmfsFilenameify(String str) {
		return CMStrings.replaceAllofAny(str, cmfsFilenameifyChars, '_')
				.toLowerCase().trim();
	}

	// this is a beautiful idea, but im scared of the memory of all the final
	// refs
	protected void addMapStatFiles(final List<CMFile.CMVFSFile> rootFiles,
			final Room R, final Environmental E, final CMFile.CMVFSDir root) {
		rootFiles.add(new CMFile.CMVFSDir(root, root.getPath() + "stats/") {
			...
			protected CMFile.CMVFSFile[] getFiles() {
				...
				for (int i = 0; i < stats.length; i++) {
					...
					myFiles.add(new CMFile.CMVFSFile(this.getPath() + statName,
							256, System.currentTimeMillis(), "SYS") {
						@Override
						public int getMaskBits(MOB accessor) {
							if (accessor == null)
								return this.mask;
							if ((E instanceof Area)
									&& (CMSecurity.isAllowed(accessor,
											((Area) E).getRandomProperRoom(),
											CMSecurity.SecFlag.CMDAREAS)))
								return this.mask;
							else if (CMSecurity.isAllowed(accessor, R,
									CMSecurity.SecFlag.CMDROOMS))
								return this.mask;
							else if ((E instanceof MOB)
									&& CMSecurity.isAllowed(accessor, R,
						...
						@Override
						public void saveData(String filename, int vfsBits,
								String author, Object O) {
							E.setStat(statName, O.toString());
							if (E instanceof Area)
								CMLib.database()
										.DBUpdateArea(oldName, (Area) E);
							else if (E instanceof Room)
								CMLib.database().DBUpdateRoom((Room) E);
							else if (E instanceof MOB)
								CMLib.database().DBUpdateMOB(R.roomID(),
										(MOB) E);
							else if (E instanceof Item)
								CMLib.database().DBUpdateItem(R.roomID(),
										(Item) E);
						}
					});
				}
	...
	public CMFile.CMVFSDir getMapRoot(final CMFile.CMVFSDir root) {
		return new CMFile.CMVFSDir(root, root.getPath() + "map/") {
			...
			protected CMFile.CMVFSFile[] getFiles() {
				List<CMFile.CMVFSFile> myFiles = new Vector<CMFile.CMVFSFile>(
						numAreas());
				for (Enumeration<Area> a = CMLib.map().areas(); a
						.hasMoreElements();) {
					final Area A = a.nextElement();
					myFiles.add(new CMFile.CMVFSFile(this.getPath()
							+ cmfsFilenameify(A.Name()) + ".cmare", 48, System
							.currentTimeMillis(), "SYS") {
						@Override
						public Object readData() {
							return CMLib.coffeeMaker().getAreaXML(A, null,
									null, null, true);
						}
					});
					myFiles.add(new CMFile.CMVFSDir(this, this.getPath()
							+ cmfsFilenameify(A.Name()) + "/") {
						@Override
						protected CMFile.CMVFSFile[] getFiles() {
							List<CMFile.CMVFSFile> myFiles = new Vector<CMFile.CMVFSFile>();
							for (Enumeration<Room> r = A.getFilledProperMap(); r
									.hasMoreElements();) {
								final Room R = r.nextElement();
								if (R.roomID().length() > 0) {
									String roomID = R.roomID();
									if (roomID.startsWith(A.Name() + "#"))
										roomID = roomID.substring(A.Name()
												.length() + 1);
									myFiles.add(new CMFile.CMVFSFile(this
											.getPath()
											+ cmfsFilenameify(R.roomID())
											+ ".cmare", 48, System
											.currentTimeMillis(), "SYS") {
										@Override
										public Object readData() {
											return CMLib.coffeeMaker()
													.getRoomXML(R, null, null,
															true);
										}
									});
									myFiles.add(new CMFile.CMVFSDir(this, this
											.getPath()
											+ cmfsFilenameify(roomID)
													.toLowerCase() + "/") {
										@Override
										protected CMFile.CMVFSFile[] getFiles() {
											List<CMFile.CMVFSFile> myFiles = new Vector<CMFile.CMVFSFile>();
											myFiles.add(new CMFile.CMVFSFile(
													this.getPath()
															+ "items.cmare",
													48,
												...
												public Object readData() {
													return CMLib
															...
															.getRoomMobs(
																	...
																	null,
																	null,
																	new TreeMap<String, List<MOB>>());
												}
											});
											myFiles.add(new CMFile.CMVFSDir(
													this, this.path + "mobs/") {
												@Override
												protected CMFile.CMVFSFile[] getFiles() {
													List<CMFile.CMVFSFile> myFiles = new Vector<CMFile.CMVFSFile>();
													...
													for (int i = 0; i < R2
														...
														myFiles.add(new CMFile.CMVFSDir(
															...
															protected CMFile.CMVFSFile[] getFiles() {
																...
																Collections
																		.sort(myFiles,
																				CMFile.CMVFSDir.fcomparator);
																return myFiles
																		.toArray(new CMFile.CMVFSFile[0]);
															}
														});
													}
													Collections
