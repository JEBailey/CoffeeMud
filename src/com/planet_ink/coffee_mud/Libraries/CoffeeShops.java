...
public class CoffeeShops extends StdLibrary implements ShoppingLibrary {
	...
	public ShopKeeper getShopKeeper(Environmental E) {
		...
		if (E instanceof Physical) {
			...
			for (final Enumeration<Ability> a = P.effects(); a
					.hasMoreElements();) {
		...
		if (here != null) {
			...
			for (int i = 0; i < here.numInhabitants(); i++) {
				...
				if ((thisMOB != null)
						...
						&& ((notMOB == null) || (CMLib.flags().canBeSeenBy(
								thisMOB, notMOB))))
	...
	public String getViewDescription(Environmental E) {
		...
		if (E instanceof Item) {
			Item I = (Item) E;
			str.append("\n\rMaterial   : "
					+ CMStrings.capitalizeAndLower(RawMaterial.CODES.NAME(
							I.material()).toLowerCase()));
			str.append("\n\rWeight     : " + I.phyStats().weight() + " pounds");
			if (I instanceof Weapon) {
				str.append("\n\rWeap. Type : "
						+ CMStrings
								.capitalizeAndLower(Weapon.TYPE_DESCS[((Weapon) I)
										.weaponType()]));
				str.append("\n\rWeap. Class: "
						+ CMStrings
								.capitalizeAndLower(Weapon.CLASS_DESCS[((Weapon) I)
					...
					if (wornCode != Wearable.IN_INVENTORY) {
						if (codes.name(wornCode).length() > 0) {
							if (((I.rawProperLocationBitmap() & wornCode) == wornCode)) {
								str.append(CMStrings.capitalizeAndLower(codes
										.name(wornCode)) + " ");
								if (I.rawLogicalAnd())
									str.append("and ");
								else
				...
				else if (str.toString().endsWith(" or "))
	...
	protected boolean shownInInventory(MOB seller, MOB buyer,
			Environmental product, ShopKeeper shopKeeper) {
		if (CMSecurity.isAllowed(buyer, buyer.location(),
		...
		if (product instanceof Ability) {
			if (shopKeeper.isSold(ShopKeeper.DEAL_TRAINER)) {
				if (!CMLib.ableMapper().qualifiesByLevel(buyer,
						(Ability) product))
					return false;
			}
		}
	...
	public double prejudiceValueFromPart(MOB customer, boolean pawnTo,
			String part) {
		int x = part.indexOf('=');
		if (x < 0)
			return 0.0;
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (bit.equalsIgnoreCase(customer.charStats().getCurrentClass()
					.name())) {
				yes = true;
				break;
			}
			if (bit.equalsIgnoreCase(customer.charStats().getCurrentClass()
					.name(customer.charStats().getCurrentClassLevel()))) {
				yes = true;
				break;
			}
			...
			if (bit.equalsIgnoreCase(customer.charStats().getMyRace()
					.racialCategory())) {
				yes = true;
				break;
			}
	...
	public double itemPriceFactor(Environmental E, Room R,
			String[] priceFactors, boolean pawnTo) {
		...
		for (int p = 0; p < priceFactors.length; p++) {
			...
			if (x < 0)
				continue;
			if (CMLib.masking().maskCheck(factorMask.substring(x + 1).trim(),
					E, false))
	...
	public ShopKeeper.ShopPrice sellingPrice(MOB seller, MOB buyer,
			Environmental product, ShopKeeper shop, boolean includeSalesTax) {
		double number = 1.0;
		...
		if (stockPrice >= 0)
			...
			val.absoluteGoldPrice = rawSpecificGoldPrice(product,
					shop.getShop(), number);

		if (buyer == null) {
			if (val.absoluteGoldPrice > 0.0)
				val.absoluteGoldPrice = CMLib.beanCounter().abbreviatedRePrice(
						seller, val.absoluteGoldPrice);
			return val;
		}

		double prejudiceFactor = prejudiceFactor(buyer,
				shop.finalPrejudiceFactors(), false);
		Room loc = CMLib.map().roomLocation(shop);
		prejudiceFactor *= itemPriceFactor(product, loc,
				shop.finalItemPricingAdjustments(), false);
		val.absoluteGoldPrice = CMath.mul(prejudiceFactor,
		...
		if (seller.isMonster())
			val.absoluteGoldPrice = val.absoluteGoldPrice
					...
					- CMath.mul(
							val.absoluteGoldPrice,
							CMath.div(
									buyer.charStats().getStat(
											CharStats.STAT_CHARISMA), 35.0));
		if (includeSalesTax) {
			double salesTax = getSalesTax(seller.getStartRoom(), seller);
			val.absoluteGoldPrice += ((salesTax > 0.0) ? (CMath.mul(
					val.absoluteGoldPrice, CMath.div(salesTax, 100.0))) : 0.0);
		}
		if (val.absoluteGoldPrice <= 0.0)
		...
		// the magical aura discount for miscmagic (potions, anything else..
		// MUST be basePhyStats tho!
	...
	public double devalue(ShopKeeper shop, Environmental product) {
		...
		double rate = (product instanceof RawMaterial) ? resourceRate
				: itemRate;
		rate = rate * num;
		if (rate > 1.0)
			rate = 1.0;
	...
	public ShopKeeper.ShopPrice pawningPrice(MOB seller, MOB buyer,
			Environmental product, ShopKeeper shop) {
		double number = 1.0;
		...
		double prejudiceFactor = prejudiceFactor(buyer,
				shop.finalPrejudiceFactors(), true);
		Room loc = CMLib.map().roomLocation(shop);
		prejudiceFactor *= itemPriceFactor(product, loc,
				shop.finalItemPricingAdjustments(), true);
		val.absoluteGoldPrice = CMath.mul(prejudiceFactor,
				val.absoluteGoldPrice);

		// gets the shopkeeper a deal on junk. Pays 5% at 3 charisma, and 50% at
		// 35
		double buyPrice = CMath.div(
				CMath.mul(val.absoluteGoldPrice,
						buyer.charStats().getStat(CharStats.STAT_CHARISMA)),
				70.0);
		if (!(product instanceof Ability))
	...
	public boolean standardSellEvaluation(MOB seller, MOB buyer,
			Environmental product, ShopKeeper shop, double maxToPay,
			double maxEverPaid, boolean sellNotValue) {
		if ((product != null) && (shop.doISellThis(product))
				&& (!(product instanceof Coins))) {
			Room sellerR = seller.location();
			if (sellerR != null) {
				int medianLevel = sellerR.getArea().getAreaIStats()[Area.Stats.MED_LEVEL
						.ordinal()];
				if (medianLevel > 0) {
					String range = CMParms.getParmStr(
							shop.finalPrejudiceFactors(), "RANGE", "0");
					int rangeI = 0;
					if ((range.endsWith("%"))
							&& (CMath.isInteger(range.substring(0,
									range.length() - 1)))) {
						rangeI = CMath.s_int(range.substring(0,
								range.length() - 1));
						rangeI = (int) Math.round(CMath.mul(medianLevel,
								CMath.div(rangeI, 100.0)));
					} else if (CMath.isInteger(range))
						rangeI = CMath.s_int(range);
					if ((rangeI > 0)
							&& (product instanceof Physical)
							&& ((((Physical) product).phyStats().level() > (medianLevel + rangeI)) || (((Physical) product)
									.phyStats().level() < (medianLevel - rangeI)))) {
						CMLib.commands().postSay(seller, buyer,
								"I'm sorry, that's out of my level range.",
								true, false);
						return false;
					}
				}
			...
			if (yourValue < 2) {
				CMLib.commands().postSay(seller, buyer, "I'm not interested.",
						true, false);
				return false;
			}
			if ((product instanceof Physical)
					&& CMLib.flags().isEnspelled((Physical) product)
					|| CMLib.flags().isOnFire((Physical) product)) {
				CMLib.commands().postSay(seller, buyer,
						"I won't buy that in it's present state.", true, false);
				return false;
			}
				...
				if (yourValue > maxEverPaid)
					CMLib.commands()
							.postSay(
									seller,
									buyer,
									"That's way out of my price range! Try AUCTIONing it.",
									true, false);
				else
					CMLib.commands()
							.postSay(
									seller,
									buyer,
									"Sorry, I can't afford that right now.  Try back later.",
									true, false);
				return false;
			}
			if (product instanceof Ability) {
			...
			if ((product instanceof Container)
				...
				for (int i = 0; i < V.size(); i++) {
					...
					if ((I instanceof DoorKey)
							&& (((DoorKey) I).getKey()
									.equals(((Container) product).keyName())))
						found = true;
					else if (CMLib.flags().isEnspelled(I)
							|| CMLib.flags().isOnFire(I)) {
						CMLib.commands()
								.postSay(
										seller,
										buyer,
										"I won't buy the contents of that in it's present state.",
										true, false);
						return false;
					}
				}
				if (!found) {
					CMLib.commands()
							.postSay(
									seller,
									buyer,
									"I won't buy that back unless you put the key in it.",
									true, false);
					return false;
				}
			}
			if ((product instanceof Item) && (buyer.isMine(product))) {
				CMMsg msg2 = CMClass.getMsg(buyer, product, CMMsg.MSG_DROP,
						null);
		...
		CMLib.commands().postSay(seller, buyer,
				"I'm sorry, I'm not buying those.", true, false);
		return false;
	}

	public boolean standardBuyEvaluation(MOB seller, MOB buyer,
			Environmental product, ShopKeeper shop, boolean buyNotView) {
		if ((product != null)
				&& (shop.getShop().doIHaveThisInStock("$" + product.Name()
						+ "$", buyer))) {
			if (buyNotView) {
				ShopKeeper.ShopPrice price = sellingPrice(seller, buyer,
						product, shop, true);
				if ((price.experiencePrice > 0)
						&& (price.experiencePrice > buyer.getExperience())) {
					CMLib.commands().postSay(
							seller,
							buyer,
							"You aren't experienced enough to buy "
									+ product.name() + ".", false, false);
					return false;
				}
				if ((price.questPointPrice > 0)
						&& (price.questPointPrice > buyer.getQuestPoint())) {
					CMLib.commands().postSay(
							seller,
							buyer,
							"You don't have enough quest points to buy "
									+ product.name() + ".", false, false);
					return false;
				}
				if ((price.absoluteGoldPrice > 0.0)
						&& (price.absoluteGoldPrice > CMLib
								.beanCounter()
								.getTotalAbsoluteShopKeepersValue(buyer, seller))) {
					CMLib.commands().postSay(seller, buyer,
							"You can't afford to buy " + product.name() + ".",
							false, false);
					return false;
				}
			}
			if (product instanceof Item) {
				if (((Item) product).phyStats().level() > buyer.phyStats()
						.level()) {
					CMLib.commands().postSay(seller, buyer,
							"That's too advanced for you, I'm afraid.", true,
							false);
					return false;
				}
			}
			if ((product instanceof LandTitle)
					&& ((shop.isSold(ShopKeeper.DEAL_CLANDSELLER)) || (shop
							.isSold(ShopKeeper.DEAL_CSHIPSELLER)))) {
				Pair<Clan, Integer> clanPair = CMLib
						.clans()
						.findPrivilegedClan(buyer, Clan.Function.PROPERTY_OWNER);
				if (clanPair == null) {
					if (!buyer.clans().iterator().hasNext())
						CMLib.commands().postSay(seller, buyer,
								"I only sell to clans.", true, false);
					else if (!buyer.isMonster())
						CMLib.commands()
								.postSay(
										seller,
										buyer,
										"You are not authorized by your clan to handle property.",
										true, false);
					return false;
				}
			}
			if (product instanceof MOB) {
				if (buyer.totalFollowers() >= buyer.maxFollowers()) {
					CMLib.commands()
							.postSay(seller, buyer,
									"You can't accept any more followers.",
									true, false);
					return false;
				}
				if ((CMProps.getIntVar(CMProps.Int.FOLLOWLEVELDIFF) > 0)
						&& (!CMSecurity.isAllowed(seller, seller.location(),
								CMSecurity.SecFlag.ORDER))
						&& (!CMSecurity.isAllowed(buyer, buyer.location(),
								CMSecurity.SecFlag.ORDER))) {
					if (seller.phyStats().level() > (buyer.phyStats().level() + CMProps
							.getIntVar(CMProps.Int.FOLLOWLEVELDIFF))) {
						buyer.tell(product.name() + " is too advanced for you.");
						return false;
					}
					if (seller.phyStats().level() < (buyer.phyStats().level() - CMProps
							.getIntVar(CMProps.Int.FOLLOWLEVELDIFF))) {
						buyer.tell(product.name()
								+ " is too inexperienced for you.");
						return false;
					}
				}
			}
			if (product instanceof Ability) {
				if (shop.isSold(ShopKeeper.DEAL_TRAINER)) {
					...
					Ability teachableA = getTrainableAbility(teacher,
							(Ability) product);
					if ((teachableA == null)
							|| (!teachableA.canBeLearnedBy(teacher, buyer))) {
						teacher.destroy();
						return false;
					}
					teacher.destroy();
						...
						if (I == null) {
							CMLib.commands()
									.postSay(
											seller,
											buyer,
											"You need to be wielding or holding the item you want this cast on.",
											true, false);
							return false;
						}
					} else {
						CMLib.commands().postSay(seller, buyer,
								"I don't know how to sell that spell.", true,
								false);
						return false;
					}
				}
		...
		CMLib.commands().postSay(seller, buyer,
				"I don't have that in stock.  Ask for my LIST.", true, false);
		return false;
	}

	public String getListInventory(MOB seller, MOB buyer,
			List<? extends Environmental> rawInventory, int limit,
			ShopKeeper shop, String mask) {
		StringBuffer str = new StringBuffer("");
		int csize = 0;
		...
		for (int i = 0; i < rawInventory.size(); i++) {
			...
			if (shownInInventory(seller, buyer, E, shop)
					&& ((mask == null) || (mask.length() == 0) || (CMLib
							.english().containsString(E.name(), mask))))
		...
		if (inventory.size() > 0) {
			int totalCols = ((shop.isSold(ShopKeeper.DEAL_LANDSELLER))
					...
					|| (shop.isSold(ShopKeeper.DEAL_SHIPSELLER)) || (shop
					.isSold(ShopKeeper.DEAL_CSHIPSELLER))) ? 1 : 2;
			int totalWidth = ListingLibrary.ColFixer.fixColWidth(
					60.0 / totalCols, buyer);
			...
			for (int i = 0; i < inventory.size(); i++) {
				...
				if ((price.experiencePrice > 0)
						&& ((("" + price.experiencePrice).length() + 2) > (4 + csize)))
					csize = ("" + price.experiencePrice).length() - 2;
				else if ((price.questPointPrice > 0)
						&& ((("" + price.questPointPrice).length() + 2) > (4 + csize)))
					csize = ("" + price.questPointPrice).length() - 2;
				else {
					showPrice = CMLib.beanCounter().abbreviatedPrice(seller,
							price.absoluteGoldPrice);
			...
			String c = "^x["
					+ CMStrings.padRight("Cost", 4 + csize)
					+ "] "
					+ CMStrings.padRight("Product",
							Math.max(totalWidth - csize, 5));
			str.append(c + ((totalCols > 1) ? c : "")
					+ "^.^N^<!ENTITY shopkeeper \""
					+ CMStrings.removeColors(seller.name()) + "\"^>^.^N\n\r");
			int colNum = 0;
			int rowNum = 0;
			String col = null;
			for (int i = 0; i < inventory.size(); i++) {
				...
				col = null;
				if (price.questPointPrice > 0)
					col = CMStrings.padRight(
							"[" + price.questPointPrice + "qp", 5 + csize)
							+ "] ^<SHOP^>"
							+ CMStrings.padRight(E.name(), "^</SHOP^>",
									Math.max(totalWidth - csize, 5));
				else if (price.experiencePrice > 0)
					col = CMStrings.padRight(
							"[" + price.experiencePrice + "xp", 5 + csize)
							+ "] ^<SHOP^>"
							+ CMStrings.padRight(E.name(), "^</SHOP^>",
									Math.max(totalWidth - csize, 5));
				else
					col = CMStrings.padRight(
							"["
									+ CMLib.beanCounter().abbreviatedPrice(
											seller, price.absoluteGoldPrice),
							5 + csize)
							+ "] ^<SHOP^>"
							+ CMStrings.padRight(E.name(), "^</SHOP^>",
									Math.max(totalWidth - csize, 5));
		...
		return "\n\r"
				+ str
				+ ((salesTax <= 0.0) ? "" : "\n\r\n\rPrices above include a "
						+ salesTax + "% sales tax.") + "^T";
	}
	...
	public String findInnRoom(InnKey key, String addThis, Room R) {
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			if ((R.getExitInDir(d) != null)
					&& (R.getExitInDir(d).keyName().equals(keyNum))) {
				final String dirName = ((R instanceof SpaceShip) || (R
						.getArea() instanceof SpaceShip)) ? Directions
						.getShipDirectionName(d) : Directions
						.getDirectionName(d);
				if (addThis.length() > 0)
	...
	public double transactPawn(MOB shopkeeper, MOB pawner, ShopKeeper shop,
		...
		if ((coreSoldItem != null) && (shop.doISellThis(coreSoldItem))) {
			...
			CMLib.beanCounter().giveSomeoneMoney(shopkeeper, pawner, currency,
					val);
			pawner.recoverPhyStats();
			pawner.tell(shopkeeper.name() + " pays you "
					+ CMLib.beanCounter().nameCurrencyShort(shopkeeper, val)
					+ " for " + rawSoldItem.name() + ".");
			if (rawSoldItem instanceof Item) {
				...
				if (V != null)
					for (int v = 0; v < V.size(); v++) {
						Item item2 = V.get(v);
						if (!shop.doISellThis(item2)
								|| (item2 instanceof DoorKey))
							item2.destroy();
						else
	...
	public void transactMoneyOnly(MOB seller, MOB buyer, ShopKeeper shop,
			Environmental product, boolean sellerGetsPaid) {
		if ((seller == null) || (seller.location() == null) || (buyer == null)
				|| (shop == null) || (product == null))
			return;
		Room room = seller.location();
		ShopKeeper.ShopPrice price = sellingPrice(seller, buyer, product, shop,
				true);
		if (price.absoluteGoldPrice > 0.0) {
			CMLib.beanCounter().subtractMoney(buyer,
			...
			if (getSalesTax(seller.getStartRoom(), seller) != 0.0) {
				...
				if ((theLaw != null) && (A2 != null)) {
					...
					if (treasuryR != null) {
						double taxAmount = totalFunds
								- sellingPrice(seller, buyer, product, shop,
										false).absoluteGoldPrice;
						totalFunds -= taxAmount;
						Coins COIN = CMLib.beanCounter().makeBestCurrency(
								CMLib.beanCounter().getCurrency(seller),
								taxAmount, treasuryR, treasuryContainer);
						if (COIN != null)
							COIN.putCoinsBack();
					}
				}
			...
			if (seller.isMonster()) {
				...
				if ((T != null) && (T.getOwnerName().length() > 0)) {
					CMLib.beanCounter()
							.modifyLocalBankGold(
									seller.getStartRoom().getArea(),
									T.getOwnerName(),
									CMLib.utensils().getFormattedDate(buyer)
											+ ": Deposit of "
											+ CMLib.beanCounter()
													.nameCurrencyShort(seller,
															totalFunds)
			...
			if (sellerGetsPaid)
				CMLib.beanCounter().giveSomeoneMoney(seller, seller,
						CMLib.beanCounter().getCurrency(seller), totalFunds);
		}
		if (price.questPointPrice > 0)
			buyer.setQuestPoint(buyer.getQuestPoint() - price.questPointPrice);
		if (price.experiencePrice > 0)
			CMLib.leveler().postExperience(buyer, null, null,
					-price.experiencePrice, false);
		buyer.recoverPhyStats();
	}

	public boolean purchaseItems(Item baseProduct,
			List<Environmental> products, MOB seller, MOB mobFor) {
		if ((seller == null) || (seller.location() == null) || (mobFor == null))
			return false;
		Room room = seller.location();
		for (int p = 0; p < products.size(); p++)
			if (products.get(p) instanceof Item)
				room.addItem((Item) products.get(p),
						ItemPossessor.Expire.Player_Drop);
		CMMsg msg2 = CMClass.getMsg(mobFor, baseProduct, seller, CMMsg.MSG_GET,
				null);
		if ((baseProduct instanceof LandTitle)
			...
			if (baseProduct instanceof InnKey) {
				...
				if (buf == null)
					buf = findInnRoom(item, "upstairs",
							room.getRoomInDir(Directions.UP));
				if (buf == null)
					buf = findInnRoom(item, "downstairs",
							room.getRoomInDir(Directions.DOWN));
				if (buf != null)
					CMLib.commands().postSay(seller, mobFor,
							"Your room is " + buf + ".", true, false);
			}
			return true;
		}
		return false;
	...
	public boolean purchaseMOB(MOB product, MOB seller, ShopKeeper shop,
			MOB mobFor) {
		if ((seller == null) || (seller.location() == null)
				|| (product == null) || (shop == null) || (mobFor == null))
			return false;
	...
	public void purchaseAbility(Ability A, MOB seller, ShopKeeper shop,
			MOB mobFor) {
		if ((seller == null) || (seller.location() == null) || (A == null)
				|| (shop == null) || (mobFor == null))
			return;
		Room room = seller.location();
		if (shop.isSold(ShopKeeper.DEAL_TRAINER)) {
			MOB teacher = CMClass.getMOB("Teacher");
			...
			if (A.canTarget(Ability.CAN_MOBS)) {
				...
				if (I == null)
					I = mobFor.fetchItem(null, Wearable.FILTER_UNWORNONLY,
							"all");
				if (I != null) {
					V.addElement("$" + I.name() + "$");
					seller.addItem(I);
	...
	public List<Environmental> addRealEstateTitles(List<Environmental> V,
			MOB buyer, CoffeeShop shop, Room myRoom) {
		...
		if (shop.isSold(ShopKeeper.DEAL_CLANDSELLER)
				&& ((buyerClanPair = CMLib.clans().findPrivilegedClan(buyer,
						Clan.Function.PROPERTY_OWNER)) != null))
		...
		if ((shop.isSold(ShopKeeper.DEAL_LANDSELLER) || (buyerClanPair != null))
			...
			for (Enumeration<Room> r = myArea.getProperMap(); r
					.hasMoreElements();) {
			...
			for (Environmental R : titles.keySet()) {
				...
				if (!roomsHandling.contains(R)) {
					...
					if ((A.getOwnerName().length() > 0)
							...
							&& ((!A.getOwnerName().equals(buyer.getLiegeID())) || (!buyer
									.isMarriedToLiege())))
						continue;
					...
					if (R instanceof Room)
						for (int d = 0; d < 4; d++) {
							...
							if ((L2.getOwnerName().equals(name))
									|| (L2.getOwnerName().equals(
											buyer.getLiegeID()) && (buyer
											.isMarriedToLiege()))) {
								skipThisOne = false;
								break;
							}
					...
					if (R instanceof Room)
						((LandTitle) I).setLandPropertyID(CMLib.map()
		...
		while (V.size() > 0) {
			if (((!(V.get(0) instanceof LandTitle)))
					|| ((x = (L = (LandTitle) V.get(0)).landPropertyID()
							.lastIndexOf('#')) < 0)) {
				if (V2.size() == 0)
				...
				int lowest = CMath.s_int(L.landPropertyID().substring(x + 1)
						.trim());
				for (int v = 1; v < V.size(); v++)
					if (V.get(v) instanceof LandTitle) {
						...
						if ((x2 > 0)
								&& (CMath.s_int(L2.landPropertyID()
										.substring(x + 1).trim()) < lowest)) {
							lowest = CMath.s_int(L2.landPropertyID()
									.substring(x + 1).trim());
							L = L2;
						}
					}
	...
	public boolean ignoreIfNecessary(MOB mob, String ignoreMask, MOB whoIgnores) {
		if ((ignoreMask.length() > 0)
			...
			mob.tell(whoIgnores, null, null,
					"<S-NAME> appear(s) to be ignoring you.");
			return false;
		}
		return true;
	}
		...
		for (int d = 1; d < ShopKeeper.DEAL_DESCS.length; d++)
			if (shop.isSold(d))
				...
				case ShopKeeper.DEAL_GENERAL:
					V.addElement("General items");
					break;
				case ShopKeeper.DEAL_ARMOR:
					V.addElement("Armor");
					break;
				case ShopKeeper.DEAL_MAGIC:
					V.addElement("Miscellaneous Magic Items");
					break;
				case ShopKeeper.DEAL_WEAPONS:
					V.addElement("Weapons");
					break;
				case ShopKeeper.DEAL_PETS:
					V.addElement("Pets and Animals");
					break;
				case ShopKeeper.DEAL_LEATHER:
					V.addElement("Leather");
					break;
				case ShopKeeper.DEAL_INVENTORYONLY:
					V.addElement("Only my Inventory");
					break;
				case ShopKeeper.DEAL_TRAINER:
					V.addElement("Training in skills/spells/prayers/songs");
					break;
				case ShopKeeper.DEAL_CASTER:
					V.addElement("Caster of spells/prayers");
					break;
				case ShopKeeper.DEAL_ALCHEMIST:
					V.addElement("Potions");
					break;
				case ShopKeeper.DEAL_INNKEEPER:
					V.addElement("My services as an Inn Keeper");
					break;
				case ShopKeeper.DEAL_JEWELLER:
					V.addElement("Precious stones and jewellery");
					break;
				case ShopKeeper.DEAL_BANKER:
					V.addElement("My services as a Banker");
					break;
				case ShopKeeper.DEAL_CLANBANKER:
					V.addElement("My services as a Banker to Clans");
					break;
				case ShopKeeper.DEAL_LANDSELLER:
					V.addElement("Real estate");
					break;
				case ShopKeeper.DEAL_CLANDSELLER:
					V.addElement("Clan estates");
					break;
				case ShopKeeper.DEAL_ANYTECHNOLOGY:
					V.addElement("Any technology");
					break;
				case ShopKeeper.DEAL_BUTCHER:
					V.addElement("Meats");
					break;
				case ShopKeeper.DEAL_FOODSELLER:
					V.addElement("Foodstuff");
					break;
				case ShopKeeper.DEAL_GROWER:
					V.addElement("Vegetables");
					break;
				case ShopKeeper.DEAL_HIDESELLER:
					V.addElement("Hides and Furs");
					break;
				case ShopKeeper.DEAL_LUMBERER:
					V.addElement("Lumber");
					break;
				case ShopKeeper.DEAL_METALSMITH:
					V.addElement("Metal ores");
					break;
				case ShopKeeper.DEAL_STONEYARDER:
					V.addElement("Stone and rock");
					break;
				case ShopKeeper.DEAL_SHIPSELLER:
					V.addElement("Ships");
					break;
				case ShopKeeper.DEAL_CSHIPSELLER:
					V.addElement("Clan Ships");
					break;
				case ShopKeeper.DEAL_SLAVES:
					V.addElement("Slaves");
					break;
				case ShopKeeper.DEAL_POSTMAN:
					V.addElement("My services as a Postman");
					break;
				case ShopKeeper.DEAL_CLANPOSTMAN:
					V.addElement("My services as a Postman for Clans");
					break;
				case ShopKeeper.DEAL_AUCTIONEER:
					V.addElement("My services as an Auctioneer");
					break;
				default:
					V.addElement("... I have no idea WHAT I sell");
					break;
				}
		return CMParms.toStringList(V);
	}

	protected boolean shopKeeperItemTypeCheck(Environmental E, int dealCode,
		...
		case ShopKeeper.DEAL_GENERAL:
			return ((E instanceof Item) && (!(E instanceof Armor))
					&& (!(E instanceof MiscMagic))
					&& (!(E instanceof ClanItem)) && (!(E instanceof Weapon))
					&& (!(E instanceof Ammunition)) && (!(E instanceof MOB))
					&& (!(E instanceof LandTitle))
					&& (!(E instanceof SpaceShip))
					&& (!(E instanceof RawMaterial)) && (!(E instanceof Ability)));
		case ShopKeeper.DEAL_LEATHER:
			return ((E instanceof Item)
					&& ((((Item) E).material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_LEATHER) && (!(E instanceof RawMaterial)));
		case ShopKeeper.DEAL_PETS:
			return ((E instanceof MOB) && (CMLib.flags()
					.isAnimalIntelligence((MOB) E)));
		case ShopKeeper.DEAL_SLAVES:
			return ((E instanceof MOB) && (!CMLib.flags().isAnimalIntelligence(
					(MOB) E)));
		case ShopKeeper.DEAL_INVENTORYONLY:
		...
		case ShopKeeper.DEAL_JEWELLER:
			return ((E instanceof Item) && (!(E instanceof Weapon))
					&& (!(E instanceof MiscMagic))
					&& (!(E instanceof ClanItem)) && (((((Item) E).material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_GLASS)
					...
					|| ((Item) E).fitsOn(Wearable.WORN_RIGHT_FINGER) || ((Item) E)
						.fitsOn(Wearable.WORN_LEFT_FINGER)));
		case ShopKeeper.DEAL_ALCHEMIST:
		...
		case ShopKeeper.DEAL_BUTCHER:
			return ((E instanceof RawMaterial) && (((RawMaterial) E).material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_FLESH);
		case ShopKeeper.DEAL_FOODSELLER:
			return (((E instanceof Food) || (E instanceof Drink)) && (!(E instanceof RawMaterial)));
		case ShopKeeper.DEAL_GROWER:
			return ((E instanceof RawMaterial) && (((RawMaterial) E).material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_VEGETATION);
		case ShopKeeper.DEAL_HIDESELLER:
			return ((E instanceof RawMaterial) && ((((RawMaterial) E)
					.material() == RawMaterial.RESOURCE_HIDE)
					...
					|| (((RawMaterial) E).material() == RawMaterial.RESOURCE_WOOL) || (((RawMaterial) E)
						.material() == RawMaterial.RESOURCE_FUR)));
		case ShopKeeper.DEAL_LUMBERER:
			return ((E instanceof RawMaterial) && ((((RawMaterial) E)
					.material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_WOODEN));
		case ShopKeeper.DEAL_METALSMITH:
			return ((E instanceof RawMaterial) && (((((RawMaterial) E)
					.material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_METAL) || (((RawMaterial) E)
					.material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_MITHRIL));
		case ShopKeeper.DEAL_STONEYARDER:
			return ((E instanceof RawMaterial) && ((((RawMaterial) E)
					.material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_ROCK));
		}
		return false;
	}
	...
	public boolean doISellThis(Environmental thisThang, ShopKeeper shop) {
		...
		if (thisThang == null)
			return false;
		if ((thisThang instanceof Coins) || (thisThang instanceof DeadBody)
	...
	public String[] bid(MOB mob, double bid, String bidCurrency,
			Auctioneer.AuctionData auctionData, Item I,
			List<String> auctionAnnounces) {
		String bidWords = CMLib.beanCounter().nameCurrencyShort(
				auctionData.currency, auctionData.bid);
		String currencyName = CMLib.beanCounter().getDenominationName(
				auctionData.currency);
		if (bid == 0.0)
			return new String[] {
					"Up for auction: " + I.name() + ".  The current bid is "
							+ bidWords + ".", null };

		if (!bidCurrency.equals(auctionData.currency))
			return new String[] {
					"This auction is being bid in " + currencyName + " only.",
					null };

		if (bid > CMLib.beanCounter().getTotalAbsoluteValue(mob,
				auctionData.currency))
			return new String[] {
					"You don't have enough " + currencyName
							+ " on hand to cover that bid.", null };

		if ((bid < auctionData.bid) || (bid == 0)) {
			String bwords = CMLib.beanCounter().nameCurrencyShort(bidCurrency,
					bid);
			return new String[] {
					"Your bid of "
							+ bwords
							+ " is insufficient."
							+ ((auctionData.bid > 0) ? " The current high bid is "
									+ bidWords + "."
									: ""), null };
		} else if ((bid > auctionData.highBid)
				|| ((bid > auctionData.bid) && (auctionData.highBid == 0))) {
			MOB oldHighBider = auctionData.highBidderM;
			if (auctionData.highBidderM != null)
				returnMoney(auctionData.highBidderM, auctionData.currency,
						auctionData.highBid);
			...
			bidWords = CMLib.beanCounter().nameCurrencyShort(
					auctionData.currency, auctionData.bid);
			String yourBidWords = CMLib.beanCounter().abbreviatedPrice(
					currencyName, auctionData.highBid);
			auctionAnnounces.add("A new bid has been entered for " + I.name()
					+ ". The current high bid is " + bidWords + ".");
			if ((oldHighBider != null) && (oldHighBider == mob))
				return new String[] {
						"You have submitted a new high bid of " + yourBidWords
								+ " for " + I.name() + ".", null };
			else if ((oldHighBider != null) && (oldHighBider != mob))
				return new String[] {
						"You have the new high reserve bid of " + yourBidWords
								+ " for " + I.name()
								+ ". The current nominal high bid is "
								+ bidWords + ".",
						"You have been outbid for " + I.name() + "." };
			else
				return new String[] {
						"You have submitted a bid of " + yourBidWords + " for "
								+ I.name() + ".", null };
		} else if ((bid == auctionData.bid)
				&& (auctionData.highBidderM != null)) {
			return new String[] {
					"You must bid higher than " + bidWords
							+ " to have your bid accepted.", null };
		} else if ((bid == auctionData.highBid)
				&& (auctionData.highBidderM != null)) {
			if ((auctionData.highBidderM != null)
					&& (auctionData.highBidderM != mob)) {
				...
				bidWords = CMLib.beanCounter().nameCurrencyShort(
						auctionData.currency, auctionData.bid);
				auctionAnnounces.add("A new bid has been entered for "
						+ I.name() + ". The current bid is " + bidWords + ".");
				return new String[] {
						"You have been outbid by proxy for " + I.name() + ".",
						"Your high bid for " + I.name() + " has been reached." };
			}
		} else {
			auctionData.bid = bid;
			bidWords = CMLib.beanCounter().nameCurrencyShort(
					auctionData.currency, auctionData.bid);
			auctionAnnounces.add("A new bid has been entered for " + I.name()
					+ ". The current bid is " + bidWords + ".");
			return new String[] {
					"You have been outbid by proxy for " + I.name() + ".", null };
		}
		return null;
	}

	public Auctioneer.AuctionData getEnumeratedAuction(String named,
			String auctionHouse) {
	...
	public void saveAuction(Auctioneer.AuctionData data, String auctionHouse,
			boolean updateOnly) {
		...
		if (!updateOnly)
			CMLib.database().DBWriteJournal(
					"SYSTEM_AUCTIONS_" + auctionHouse.toUpperCase().trim(),
					data.auctioningM.Name(), "" + data.tickDown,
					CMStrings.limit(data.auctioningI.name(), 38),
					xml.toString());
		else
			CMLib.database().DBUpdateJournal(data.auctionDBKey,
					data.auctioningI.Name(), xml.toString(), 0);
	}
	...
	public List<AuctionData> getAuctions(Object ofLike, String auctionHouse) {
		...
		List<JournalsLibrary.JournalEntry> otherAuctions = CMLib.database()
				.DBReadJournalMsgs(house);
		for (int o = 0; o < otherAuctions.size(); o++) {
			...
			for (int v = 0; v < xmlV.size(); v++) {
				...
				if (X.tag.equalsIgnoreCase("AUCTIONITEM")) {
					data.auctioningI = CMLib.coffeeMaker().getItemFromXML(
							X.value);
					break;
				}
			}
			if ((ofLike instanceof Item)
					&& (!((Item) ofLike).sameAs(data.auctioningI)))
				continue;
	...
	public String getAuctionInventory(MOB seller, MOB buyer,
			Auctioneer auction, String mask) {
		StringBuffer str = new StringBuffer("");
		str.append("^x" + CMStrings.padRight("Lvl", 3) + " "
				+ CMStrings.padRight("Item", 50) + " "
				+ CMStrings.padRight("Days", 4) + " ["
				+ CMStrings.padRight("Bid", 6) + "] Buy^.^N\n\r");
		...
		for (int v = 0; v < auctions.size(); v++) {
			...
			if (shownInInventory(seller, buyer, data.auctioningI, auction)) {
				if (((mask == null) || (mask.length() == 0) || (CMLib.english()
						.containsString(data.auctioningI.name(), mask)))
						&& ((data.tickDown > System.currentTimeMillis())
								|| (data.auctioningM == buyer) || (data.highBidderM == buyer))) {
					Area area = CMLib.map().getStartArea(seller);
					if (area == null)
						area = CMLib.map().getStartArea(buyer);
					str.append(CMStrings.padRight(""
							+ data.auctioningI.phyStats().level(), 3)
							+ " ");
					...
					} else if (data.auctioningM == buyer)
						str.append("DONE ");
					else
						str.append("WON! ");
					str.append("["
							+ CMStrings.padRight(CMLib.beanCounter()
									.abbreviatedPrice(seller, data.bid), 6)
							+ "] ");
					if (data.buyOutPrice <= 0.0)
						str.append(CMStrings.padRight("-", 6));
					else
						str.append(CMStrings.padRight(CMLib.beanCounter()
								.abbreviatedPrice(seller, data.buyOutPrice), 6));
					str.append("\n\r");
				}
			}
		}
		...
		try {
			if (CMLib.flags().isInTheGame(M, true))
				...
				CMLib.smtp().emailIfPossible(
						...
						"Auction Update for item: " + regardingItem, resp);
			}
		} catch (Exception e) {
		}
	}
			...
			if (data.highBidderM != null) {
				...
				auctionNotify(
						...
						"The auction for "
								...
								+ " was closed early.  You have been refunded your max bid.",
						data.auctioningI.Name());
