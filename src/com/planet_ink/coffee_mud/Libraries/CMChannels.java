...
public class CMChannels extends StdLibrary implements ChannelsLibrary {
	...
	public final static List<ChannelMsg> emptyQueue = new ReadOnlyList<ChannelMsg>(
			new Vector<ChannelMsg>(1));
	public final static Set<ChannelFlag> emptyFlags = new ReadOnlySet<ChannelFlag>(
			new HashSet<ChannelFlag>(1));
	...
	public boolean mayReadThisChannel(MOB sender, boolean areaReq, MOB M,
			int i, boolean offlineOK) {
		...
		if (pstats == null)
			return false;
		Room R = M.location();
		if (((!offlineOK)) && ((M.amDead()) || (R == null)))
			return false;
		CMChannel chan = getChannel(i);
		if (chan == null)
			return false;
		if (chan.flags.contains(ChannelFlag.CLANONLY)
			...
			if ((!CMLib.clans().isAnyCommonClan(sender, M))
					&& ((!chan.flags.contains(ChannelFlag.CLANALLYONLY)) || (!CMLib
							.clans().findAnyClanRelations(M, sender,
									Clan.REL_ALLY))))
				return false;
		}
		...
		if ((!pstats.getIgnored().contains(sender.Name()))
				...
				&& ((!areaReq) || (sender.location() == null) || (R == null) || (R
						.getArea() == sender.location().getArea()))
				&& (!CMath.isSet(pstats.getChannelMask(), i)))
			return true;
		return false;
	}

	public boolean mayReadThisChannel(MOB sender, boolean areaReq, Session ses,
			int i) {
		if (ses == null)
			return false;
		MOB M = ses.mob();

		if ((sender == null) || (M == null) || (M.amDead())
				|| (M.location() == null))
			return false;
		PlayerStats pstats = M.playerStats();
		if (pstats == null)
		...
		if ((!ses.isStopped())
				...
				&& (CMLib.masking().maskCheck(chan.mask, M, true))
				&& ((!areaReq) || (sender.location() == null) || (R.getArea() == sender
						.location().getArea()))
				&& (!CMath.isSet(pstats.getChannelMask(), i)))
			return true;
		return false;
	...
	public boolean mayReadThisChannel(MOB M, int i, boolean zapCheckOnly) {
		...
		if (chan == null)
			return false;
		if ((chan.flags.contains(ChannelFlag.CLANONLY) || chan.flags
				.contains(ChannelFlag.CLANALLYONLY))
	...
	public String getExtraChannelDesc(String channelName) {
		...
		if (dex >= 0) {
			...
			if ((mask != null) && (mask.trim().length() > 0))
				str.append(" The following may read this channel : "
						+ CMLib.masking().maskDesc(mask));
		}
		return str.toString();
	}
	...
	public String parseOutFlags(String mask, Set<ChannelFlag> flags,
			String[] colorOverride) {
		List<String> V = CMParms.parseSpaces(mask, true);
		for (int v = V.size() - 1; v >= 0; v--) {
			String s = V.get(v).toUpperCase();
			if (CMParms
					.contains(CMParms.toStringArray(ChannelFlag.values()), s)) {
				V.remove(v);
				flags.add(ChannelFlag.valueOf(s));
			} else {
				int colorNum = CMParms.indexOf(
						ColorLibrary.COLOR_ALLCOLORNAMES, s);
				if (colorNum >= 0) {
					V.remove(v);
					if (s.startsWith("BG"))
						colorOverride[0] = colorOverride[0]
								+ ColorLibrary.COLOR_ALLCOLORS[colorNum];
					else
						colorOverride[0] = ColorLibrary.COLOR_ALLCOLORS[colorNum]
								+ ColorLibrary.COLOR_ALLCOLORS[colorNum]
								+ colorOverride[0];
					colorOverride[1] += " "
							+ ColorLibrary.COLOR_ALLCOLORNAMES[colorNum];
				}
			}
		}
	...
	public int loadChannels(String list, String ilist, String imc2list) {
		...
		while (list.length() > 0) {
			...
			if (x > 0) {
				...
				chan.mask = parseOutFlags(item.substring(x + 1).trim(),
						chan.flags, colorOverride);
	...
	public boolean channelTo(Session ses, boolean areareq, int channelInt,
			CMMsg msg, MOB sender) {
		final MOB M = ses.mob();
		final Room R = M.location();
		boolean didIt = false;
		if (mayReadThisChannel(sender, areareq, ses, channelInt) && (R != null)
				&& ((sender.location() == R) || (R.okMessage(ses.mob(), msg)))) {
			if (ses.getClientTelnetMode(Session.TELNET_GMCP)) {
				ses.sendGMCPEvent(
						"comm.channel",
						"{\"chan\":\""
								+ getChannel(channelInt).name
								+ "\",\"msg\":\""
	...
	public void reallyChannel(MOB mob, String channelName, String message,
			boolean systemMsg) {
		...
		if (systemMsg) {
			...
			if ((!mob.name().startsWith("^")) || (mob.name().length() > 2))
				str = "<S-NAME> " + str;
			msg = CMClass.getMsg(mob, null, null, CMMsg.MASK_CHANNEL
					| CMMsg.MASK_ALWAYS | CMMsg.MSG_SPEAK, channelColor
					+ "^<CHANNEL \"" + channelName + "\"^>" + str,
					CMMsg.NO_EFFECT, null, CMMsg.MASK_CHANNEL
							| (CMMsg.TYP_CHANNEL + channelInt), channelColor
							+ "^<CHANNEL \"" + channelName + "\"^>" + str);
		} else if (message.startsWith(",")
				|| (message.startsWith(":") && (message.length() > 1) && (Character
						.isLetter(message.charAt(1)) || message.charAt(1) == ' '))) {
			...
			if (S != null)
				...
				msgstr = CMProps
						.applyINIFilter(msgstr, CMProps.Str.EMOTEFILTER);
				if (msgstr.trim().startsWith("'")
						|| msgstr.trim().startsWith("`"))
					msgstr = msgstr.trim();
				else
					msgstr = " " + msgstr.trim();
				String srcstr = "^<CHANNEL \"" + channelName + "\"^>["
						+ channelName + "] " + mob.name() + msgstr
						+ "^</CHANNEL^>^N^.";
				String reststr = "^<CHANNEL \"" + channelName + "\"^>["
						+ channelName + "] <S-NAME>" + msgstr
						+ "^</CHANNEL^>^N^.";
				msg = CMClass.getMsg(mob, null, null, CMMsg.MASK_CHANNEL
						| CMMsg.MASK_ALWAYS | CMMsg.MSG_SPEAK, channelColor
						+ "" + srcstr, CMMsg.NO_EFFECT, null,
						CMMsg.MASK_CHANNEL | (CMMsg.TYP_CHANNEL + channelInt),
						channelColor + reststr);
			}
		} else
			msg = CMClass.getMsg(mob, null, null, CMMsg.MASK_CHANNEL
					| CMMsg.MASK_ALWAYS | CMMsg.MSG_SPEAK, channelColor
					+ "^<CHANNEL \"" + channelName + "\"^>You " + channelName
					+ " '" + message + "'^</CHANNEL^>^N^.", CMMsg.NO_EFFECT,
					null,
					CMMsg.MASK_CHANNEL | (CMMsg.TYP_CHANNEL + channelInt),
					channelColor + "^<CHANNEL \"" + channelName
							+ "\"^><S-NAME> " + channelName + "S '" + message
							+ "'^</CHANNEL^>^N^.");
		if ((chan.flags.contains(ChannelsLibrary.ChannelFlag.ACCOUNTOOC))
				&& (pStats != null) && (pStats.getAccount() != null)
				&& (msg.source() == mob)) {
			String accountName = pStats.getAccount().accountName();
			if (msg.sourceMessage() != null)
				msg.setSourceMessage(CMStrings.replaceAll(msg.sourceMessage(),
						"<S-NAME>", accountName));
			if (msg.targetMessage() != null)
				msg.setTargetMessage(CMStrings.replaceAll(msg.targetMessage(),
						"<S-NAME>", accountName));
		...
		if ((R != null) && ((!R.isInhabitant(mob)) || (R.okMessage(mob, msg)))) {
		...
		if ((CMLib.intermud().i3online() && (CMLib.intermud()
				.isI3channel(channelName)))
				|| (CMLib.intermud().imc2online() && (CMLib.intermud()
						.isIMC2channel(channelName))))
