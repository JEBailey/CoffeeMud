...
public class Quests extends StdLibrary implements QuestManager {
	...
	public Object getHolidayFile() {
		...
		if ((Q == null)
				|| (!Q.script().toUpperCase().trim()
						.equalsIgnoreCase(holidayDefinition))) {
			Q = null;
			CMFile lF = new CMFile("//"
					+ Resources.makeFileResourceName(holidayFilename), null);
			CMFile vF = new CMFile("::"
					+ Resources.makeFileResourceName(holidayFilename), null);
			if ((lF.exists()) && (!vF.exists()) && (lF.canRead())
					&& (vF.canWrite())) {
				byte[] O = lF.raw();
				vF.saveRaw(O);
			}
			...
			if (Q == null)
				return "A quest named 'holidays', with the script definition '"
						+ holidayDefinition
						+ "' has not been created.  Enter the following to create this quest:\n\r"
						+ "CREATE QUEST " + holidayDefinition + "\n\r"
						+ "SAVE QUESTS";
		}
		CMFile F = new CMFile(Resources.makeFileResourceName(holidayFilename),
				null);
		if ((!F.exists()) || (!F.canRead()) || (!F.canWrite())) {
			return "The file '"
					+ Resources.makeFileResourceName(holidayFilename)
					+ "' does not exist, and is required for this feature.";
		}
	...
	public String listHolidays(Area A, String otherParms) {
		...
		String var = null;
		List<String> V = null;
		str.append("^H#  " + CMStrings.padRight("Holiday Name", 20)
				+ CMStrings.padRight("Area Name(s)", 50) + "^?\n\r");
			...
			if (nameLine != null) {
				...
				if (contains) {
					...
					str.append(CMStrings.padRight("" + s, 3)
							...
							+ CMStrings.padRight(
									CMParms.combineWithQuotes(areaLine, 2), 30)
							+ "\n\r");
				}
			}
		}
	...
	protected void promptText(MOB mob, DVector sets, String var,
			int showNumber, int showFlag, String prompt, String help,
			boolean emptyOK) throws java.io.IOException {
		...
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			String newVAL = CMLib.genEd().prompt(mob, oldVal, showNumber,
					showFlag, prompt, emptyOK);
			if (newVAL.equals("?")) {
				mob.tell(help);
				continue;
			} else if (index >= 0)
	...
	public String createHoliday(String named, String areaName, boolean save) {
		...
		if (CMLib.quests().fetchQuest(named) != null)
			return "A quest called '" + named
					+ "' already exists.  Better to pick a new name.";
		...
		for (int v = 0; v < steps.size(); v++) {
			step = steps.get(v);
			List<String> stepV = Resources.getFileLineVector(new StringBuffer(
					step));
				...
				if (lineV.size() > 1) {
					...
					if (cmd.equals("SET") && (var.equalsIgnoreCase("NAME"))) {
						...
						if (str.equalsIgnoreCase(named))
							return "A quest called '"
									+ named
									+ "' already exists.  Better to pick a new name or modify the existing one.";
					}
				}
			}
		}
	...
	public StringBuffer getDefaultHoliData(String named, String area) {
		...
		newHoliday.append("GIVE BEHAVIOR MUDCHAT +(|" + named + ")\\;9Happy "
				+ named + " $n!\\;\n\r");
		newHoliday
				.append("SET MOBGROUP RESELECT MASK=-JAVACLASS +GenShopkeeper +StdShopKeeper\n\r");
	...
	public String deleteHoliday(int holidayNumber) {
		...
		if ((holidayNumber <= 0) || (holidayNumber >= steps.size()))
			return holidayNumber
					+ " does not exist as a holiday -- enter LIST HOLIDAYS.";
	...
	public String getHolidayName(int index) {
		...
		List<String> stepV = Resources
				.getFileLineVector(new StringBuffer(step));
		for (int v1 = 0; v1 < stepV.size(); v1++) {
			line = stepV.get(v1);
			lineV = CMParms.parse(line);
			if (lineV.size() > 1) {
	...
	public RawHolidayData getEncodedHolidayData(String dataFromStepsFile) {
		...
		String[] SETTINGS = { "NAME", "WAIT", "DATE", "DURATION", "MUDDAY",
				"AREAGROUP", "MOBGROUP" };
		for (int v = 0; v < stepV.size(); v++) {
			line = stepV.get(v);
			lineV = CMParms.parse(line);
				...
				if (cmd.equals("SET") && (CMParms.indexOf(SETTINGS, var) >= 0)) {
					if (!settings.contains(var)) {
						...
						settings.addElement(var, str, Integer.valueOf(v));
							...
							&& (pricingMobIndex < 0)
							&& (CMParms.combine(lineV, 2).toUpperCase()
									.indexOf("SHOPKEEPER") > 0))
						pricingMobIndex = v;
				}
				if (cmd.equals("GIVE") && (var.equalsIgnoreCase("BEHAVIOR"))
						&& (lineV.size() > 2) && (pricingMobIndex < 0)) {
					var = lineV.elementAt(2).toUpperCase();
					behaviors.addElement(var,
							CMParms.combineWithQuotes(lineV, 3),
							Integer.valueOf(v));
				}
				if (cmd.equals("GIVE") && (var.equalsIgnoreCase("AFFECT"))
						&& (lineV.size() > 2) && (pricingMobIndex < 0)) {
					var = lineV.elementAt(2).toUpperCase();
					properties.addElement(var,
							CMParms.combineWithQuotes(lineV, 3),
							Integer.valueOf(v));
				}
				if (cmd.equals("GIVE") && (var.equalsIgnoreCase("STAT"))
						&& (lineV.size() > 2)) {
					var = lineV.elementAt(2).toUpperCase();
					if ((pricingMobIndex < 0) || (var.equals("PRICEMASKS")))
						stats.addElement(var,
								CMParms.combineWithQuotes(lineV, 3),
								Integer.valueOf(v));
				}
			}
	...
	public void modifyHoliday(MOB mob, int holidayNumber) {
		...
		if (resp instanceof List)
			...
			mob.tell("Unknown error.");
			return;
		}
		if ((holidayNumber <= 0) || (holidayNumber >= steps.size())) {
			mob.tell(holidayNumber
					+ " does not exist as a holiday -- enter LIST HOLIDAYS.");
			return;
		}
		...
		try {
			while ((mob.session() != null) && (!mob.session().isStopped())
					&& (!ok)) {
				int showNumber = 0;
				promptText(mob, settings, "NAME", ++showNumber, showFlag,
						"Holiday Name", "It's, well, a name.", false);
				...
				if (settings.indexOf("AREAGROUP") >= 0)
					promptText(
							...
							showFlag,
							"Areas List (?)",
							"Area names are space separated, and words grouped using double-quotes",
							false);
				if (settings.indexOf("MOBGROUP") >= 0)
					promptText(mob, settings, "MOBGROUP", ++showNumber,
							showFlag, "Mask for mobs that apply (?)", CMLib
									.masking().maskHelp("\n\r", "disallow"),
							false);
				promptText(
						mob,
						...
						showFlag,
						"Mood setting (?)",
						"NULL/Empty (to not use a Mood), or one of: FORMAL, POLITE, HAPPY, SAD, ANGRY, RUDE, MEAN, PROUD, GRUMPY, EXCITED, SCARED, LONELY",
						true);
				promptText(
						mob,
						behaviors,
						...
						showFlag,
						"Aggressive setting (?)",
						CMLib.help().getHelpText("Aggressive", mob, true)
								+ "\n\r\n\r** NULL/Empty (to not use Aggressive **",
						true);
				showNumber = genPricing(mob, stats, ++showNumber, showFlag);
				showNumber = genMudChat(mob, "MUDCHAT", behaviors,
						++showNumber, showFlag);
				showNumber = genBehaviors(mob, behaviors, ++showNumber,
						showFlag);
				showNumber = genProperties(mob, properties, ++showNumber,
						showFlag);
				if (showFlag < -900) {
					ok = true;
					break;
		...
		for (int v = 0; v < steps.size(); v++) {
			...
			if (v == holidayNumber) {
				...
				stepV.set(
						index,
						"SET NAME "
								+ (String) settings.elementAt(
										settings.indexOf("NAME"), 2));
				index = startLineIndex(stepV, "SET DURATION");
				stepV.set(
						index,
						"SET DURATION "
								+ (String) settings.elementAt(
										settings.indexOf("DURATION"), 2));
				...
				if (mudDayIndex >= 0)
					stepV.set(
							intervalLine,
							"SET MUDDAY "
									+ ((String) settings.elementAt(mudDayIndex,
											2)));
				else if (dateIndex >= 0)
					stepV.set(
							intervalLine,
							"SET DATE "
									+ ((String) settings
											.elementAt(dateIndex, 2)));
				else
					stepV.set(
							intervalLine,
							"SET WAIT "
									+ ((String) settings
											.elementAt(waitIndex, 2)));
				...
				if (index >= 0) {
					...
					stepV.set(
							index,
							"SET MOBGROUP RESELECT MASK="
									+ settings.elementAt(
											settings.indexOf("MOBGROUP"), 2));
				}
				if ((pricingMobIndex > 0) && (stats.indexOf("PRICEMASKS") >= 0)) {
					index = startLineIndex(stepV, "GIVE STAT PRICEMASKS");
					String s = (String) stats.elementAt(
							stats.indexOf("PRICEMASKS"), 2);
					if (s.trim().length() == 0) {
						if (index >= 0)
							stepV.remove(index);
					} else {
						if (index >= 0)
							...
							stepV.add(pricingMobIndex + 1,
									"GIVE STAT PRICEMASKS " + s);
					}
				}
				...
				if (index >= 0) {
					...
					String s = (String) behaviors.elementAt(
							behaviors.indexOf("AGGRESSIVE"), 2);
					if (s.trim().length() == 0) {
						if (index >= 0)
							stepV.remove(index);
					} else {
						if (index >= 0)
							...
							stepV.add(mobGroupIndex + 1,
									"GIVE BEHAVIOR AGGRESSIVE " + s);
					}
				}
				...
				if (index >= 0) {
					...
					String s = (String) properties.elementAt(
							properties.indexOf("MOOD"), 2);
					if (s.trim().length() == 0) {
						if (index >= 0)
							stepV.remove(index);
					} else {
						if (index >= 0)
							...
							stepV.add(mobGroupIndex + 1, "GIVE AFFECT MOOD "
									+ s);
					}
				}
				...
				for (int p = 0; p < properties.size(); p++) {
					...
					if (index >= 0)
						stepV.set(index,
								"GIVE AFFECT " + prop.toUpperCase().trim()
										+ " " + (properties.elementAt(p, 2)));
					else
						stepV.add(mobGroupIndex + 1,
								"GIVE AFFECT " + prop.toUpperCase().trim()
										+ " " + (properties.elementAt(p, 2)));
				...
				for (int p = 0; p < behaviors.size(); p++) {
					...
					if (behav.equalsIgnoreCase("AGGRESSIVE")
							|| behav.equalsIgnoreCase("MUDCHAT"))
						continue;
					mobGroupIndex = startLineIndex(stepV, "SET MOBGROUP");
					index = startLineIndex(stepV, "GIVE BEHAVIOR " + behav);
					if (index >= 0)
						stepV.set(index,
								"GIVE BEHAVIOR " + behav.toUpperCase().trim()
										+ " " + (behaviors.elementAt(p, 2)));
					else
						stepV.add(mobGroupIndex + 1,
								"GIVE BEHAVIOR " + behav.toUpperCase().trim()
										+ " " + (behaviors.elementAt(p, 2)));
				...
				for (int v1 = 0; v1 < stepV.size(); v1++) {
					if (stepV.get(v1).trim().length() > 0)
						buf.append(CMStrings.replaceAll(
								(stepV.get(v1) + "\n\r"), ";", "\\;"));
				}
				buf.append("\n\r");
			} else
	...
	protected int promptDuration(MOB mob, DVector settings, int showNumber,
		...
		if ((showFlag < 0) || (showFlag == showNumber)) {
			...
			if ((showFlag <= -999) || (showFlag == showNumber)) {
				String newVal = "?";
				while (newVal.equals("?")
						&& ((mob.session() != null) && (!mob.session()
								.isStopped()))) {
					newVal = CMLib.genEd().prompt(mob, TYPES[typeIndex],
							showNumber, showFlag, "Schedule type",
							CMParms.toStringList(TYPES));
					if (CMParms.indexOf(TYPES, newVal.toUpperCase().trim()) < 0) {
						newVal = "?";
						mob.tell("Not a valid entry.  Try ?");
						continue;
					}
					typeIndex = CMParms.indexOf(TYPES, newVal.toUpperCase()
							.trim());
					...
					if ((typeIndex == 1) && (mudDayIndex < 0))
						settings.addElement("MUDDAY", "1-1",
								Integer.valueOf(-1));
					if ((typeIndex == 2) && (dateIndex < 0))
		...
		if (mudDayIndex >= 0)
			promptText(mob, settings, "MUDDAY", ++showNumber, showFlag,
					"Mud-Day (MONTH-DAY)",
					"It's, well, a date in form month-day.", false);
		else if (dateIndex >= 0)
			promptText(mob, settings, "DATE", ++showNumber, showFlag,
					"Real Life Date (MONTH-DAY)",
					"It's, well, a date in form month-day.", false);
		else
			promptText(
					mob,
					...
					showFlag,
					"Interval Ticks (?): ",
					"It's in ticks; where 150 ticks=1 mud hour.  You can also make random interval by using ? operator where 1?5 means random number from 1-5.  Make expressions like 100+((1?5)*10)",
					false);
		promptText(
				mob,
				settings,
				...
				showFlag,
				"Duration Ticks (?): ",
				"It's in ticks; where 150 ticks=1 mud hour.  You can also make random durations by using ? operator where 1?5 means random number from 1-5.  Make expressions like 100+((1?5)*10)",
				false);
		return showNumber;
	}

	protected int genBehaviors(MOB mob, DVector behaviors, int showNumber,
			int showFlag) throws IOException {
		for (int b = 0; b <= behaviors.size(); b++) {
			if ((b < behaviors.size())
					&& (((String) behaviors.elementAt(b, 1))
							.equalsIgnoreCase("MUDCHAT") || ((String) behaviors
							.elementAt(b, 1)).equalsIgnoreCase("AGGRESSIVE")))
				continue;
			if ((showFlag > 0) && (showFlag != showNumber)) {
				if (b < behaviors.size())
					showNumber++;
			...
			if (b == behaviors.size()) {
				if ((mob.session() != null) && (!mob.session().isStopped())) {
					if (((showFlag <= -999) && CMLib.genEd().prompt(mob, false,
							showNumber, showFlag, "Add new mob behavior"))
							|| (showNumber == showFlag)) {
						behaviors.addElement("BehaviorID", "",
								Integer.valueOf(behaviors.size()));
						b -= 1;
					} else if (showFlag == -1)
			...
			if ((showFlag == showNumber) || (showFlag <= -999)) {
				behavior = CMLib.genEd().prompt(mob, behavior, showNumber,
						showFlag, "Behavior ID (NULL to delete)", true,
						toStringList(CMClass.behaviors()));
				...
				if (CMClass.getBehavior(behavior) == null) {
					mob.tell("Behavior '" + behavior
							+ "' does not exist.  Use ? for a list.");
					b--;
					showNumber--;
					continue;
				}
				StringBuilder help = CMLib.help().getHelpText(behavior, mob,
						true);
				if (help == null)
					help = new StringBuilder("No help on '" + behavior + "'");
				parms = CMLib.genEd().prompt(mob, parms, showNumber, showFlag,
						"Behavior Parameters", help.toString());
	...
	protected int genProperties(MOB mob, DVector properties, int showNumber,
			int showFlag) throws IOException {
		for (int p = 0; p <= properties.size(); p++) {
			if ((p < properties.size())
					&& (((String) properties.elementAt(p, 1))
							.equalsIgnoreCase("MOOD")))
				continue;
			if ((showFlag > 0) && (showFlag != showNumber)) {
				if (p < properties.size())
					showNumber++;
			...
			if (p == properties.size()) {
				if ((mob.session() != null) && (!mob.session().isStopped())) {
					if (((showFlag <= -999) && CMLib.genEd().prompt(mob, false,
							showNumber, showFlag, "Add new mob property"))
							|| (showNumber == showFlag)) {
						properties.addElement("AbilityID", "",
			...
			if ((showFlag == showNumber) || (showFlag <= -999)) {
				propertyID = CMLib.genEd().prompt(mob, propertyID, showNumber,
						showFlag, "Ability ID (NULL to delete)", true,
						toStringList(CMClass.abilities()));
				...
				if (CMClass.getAbility(propertyID) == null) {
					mob.tell("Ability '" + propertyID
							+ "' does not exist.  Use ? for a list.");
					p--;
					showNumber--;
					continue;
				}
				StringBuilder help = CMLib.help().getHelpText(propertyID, mob,
						true);
				if (help == null)
					help = new StringBuilder("No help on '" + propertyID + "'");
				parms = CMLib.genEd().prompt(mob, parms, showNumber, showFlag,
						"Ability Parameters", help.toString());
	...
	protected int genPricing(MOB mob, DVector stats, int showNumber,
			int showFlag) throws IOException {
		...
		for (int v = 0; v <= priceV.size(); v++) {
			...
			if (v == priceV.size()) {
				if ((mob.session() != null) && (!mob.session().isStopped())) {
					if (((showFlag <= -999) && CMLib.genEd().prompt(mob, false,
							showNumber, showFlag, "Add new price factor"))
							|| (showNumber == showFlag)) {
						priceV.add("1.0");
						v -= 1;
					} else if (showFlag == -1)
			...
			mob.tell(showNumber + ". Price Factor: "
					+ Math.round(priceFactor * 100.0) + "%: " + mask);
			if ((showFlag == showNumber) || (showFlag <= -999)) {
				priceFactor = CMLib.genEd().prompt(mob, priceFactor,
						showNumber, showFlag,
						"Price Factor (enter 0 to delete)");
				if (priceFactor == 0.0) {
					priceV.remove(v);
					v--;
					if ((showFlag == showNumber))
				...
				mob.tell(showNumber + ". Price Factor: "
						+ Math.round(priceFactor * 100.0) + "%: " + mask);
				mask = CMLib.genEd().prompt(mob, mask, showNumber, showFlag,
						"Item mask for this price",
		...
		if (pndex >= 0)
			...
			stats.addElement("PRICEMASKS", newVal,
					Integer.valueOf(stats.size()));
		return showNumber;
	}
	...
	public String breakOutMaskString(String s, List<String> p) {
		...
		if (x >= 0) {
			...
			int i = 0;
			while ((i < p.size())
					&& (p.get(i).toUpperCase().indexOf("MASK=") < 0))
				i++;
			if (i <= p.size()) {
				String pp = p.get(i);
	...
	public List<List<String>> breakOutMudChatVs(String MUDCHAT,
			DVector behaviors) {
		int mndex = behaviors.indexOf(MUDCHAT);
		String mudChatStr = (mndex < 0) ? "" : (String) behaviors.elementAt(
				mndex, 2);
	...
	protected int genMudChat(MOB mob, String var, DVector behaviors,
		...
		for (int v = 0; v <= mudChatV.size(); v++) {
			...
			if (v == mudChatV.size()) {
				if ((mob.session() != null) && (!mob.session().isStopped())) {
					if (((showFlag <= -999) && CMLib.genEd().prompt(mob, false,
							showNumber, showFlag, "Add new mud chat"))
							|| (showNumber == showFlag)) {
						V = new Vector<String>();
			...
			if ((showFlag == showNumber) || (showFlag <= -999)) {
				words = CMLib
						...
						.prompt(mob,
								...
								showNumber,
								showFlag,
								"Enter matching words (| delimited, NULL to delete)\n\r",
								true);
				...
				for (int v1 = 1; v1 <= V.size(); v1++) {
					if (v1 == V.size()) {
						if ((mob.session() != null)
								...
								&& (mob.session().confirm(
										"Add another thing to say (y/N)", "NO"))) {
							V.add("9say this");
							v1 -= 1;
						}
						continue;
					...
					s = V.get(v1);
					String newStr = "?";
					while ((newStr.equals("?")) && (mob.session() != null)
							&& (!mob.session().isStopped())) {
						newStr = mob.session().prompt(
								"Enter  # Weight + thing to say (?) '" + s
										+ "'\n\r: ", s);
						if (newStr.equals("?"))
		...
		if (mndex >= 0)
			behaviors.setElementAt(mndex, 2, (finalVal.toString().trim()
					.length() == 0) ? "" : ("+" + finalVal.toString()));
		else
			behaviors.addElement(var,
					(finalVal.toString().trim().length() == 0) ? ""
							: ("+" + finalVal.toString()), Integer
							.valueOf(behaviors.size()));
		return showNumber;
	}

	public List<List<String>> parseQuestCommandLines(List<?> script,
			String cmdOnly, int startLine) {
	...
	public List<String> parseQuestSteps(List<String> script, int startLine,
	...
	public DVector getQuestTemplate(MOB mob, String fileToGet) {
		// user security doesn't matter, because this is read-only & system
		// files.
		final int fileOpenFlag = CMFile.FLAG_LOGERRORS
				| (CMSecurity.isAllowedAnywhere(mob,
						CMSecurity.SecFlag.CMDQUESTS) ? CMFile.FLAG_FORCEALLOW
						: 0);
		CMFile tempF = new CMFile(
				Resources.makeFileResourceName("quests/templates"), null,
				fileOpenFlag);
		...
		for (int f = 0; f < files.length; f++) {
			if ((files[f].getName().toUpperCase().endsWith(".QUEST"))
					&& ((fileToGet == null) || (files[f].getName()
							.toUpperCase().startsWith(fileToGet.toUpperCase()
							.trim())))) {
					...
					if ((foundQuestScript) && (script != null))
						...
						if (s.startsWith("!")) {
							...
							if (s.startsWith("QUESTMAKER_START_SCRIPT")) {
								...
								templatesDV.addElement(name, "",
										files[f].getName(),
										new Vector<Object>(), script);
							} else if (s.startsWith("QUESTMAKER_END_SCRIPT")
									&& (foundStart)) {
								foundStart = false;
								foundQuestScript = true;
								continue;
							} else if (s.startsWith("QUESTMAKER_PAGE")
									&& (foundStart)) {
								if (!parsePages)
									break;
								...
								pageDV.addElement(
										Integer.valueOf(QuestManager.QM_COMMAND_$TITLE),
										name, "", "");
								((Vector<Object>) templatesDV.elementAt(
										templatesDV.size() - 1, 4))
										.addElement(pageDV);
							} else if (s.trim().length() > 0)
								Log.errOut("Quests",
										"Unrecognized meta-questmaker command: "
												+ s);
							...
							if (parsePages) {
								...
								if (x < 0)
									Log.errOut("Quests",
											"Illegal QuestMaker variable syntax: "
													+ s);
								else if (pageDV == null)
									Log.errOut("Quests",
											"QuestMaker syntax error, QUESTMAKER_PAGE not yet designated: "
													+ s);
								else {
									int y = s.indexOf('=', x + 1);
									if (y >= 0)
										pageDV.addElement(s.substring(x + 1, y)
												.trim(), s.substring(0, x), s
												.substring(y + 1), "");
									else
										pageDV.addElement(s.substring(x + 1)
												.trim(), s.substring(0, x), "",
												"");
									String cmd = (String) pageDV.elementAt(
											pageDV.size() - 1, 1);
									int mask = 0;
									if (cmd.startsWith("(")
											&& cmd.endsWith(")")) {
										mask = mask
												| QuestManager.QM_COMMAND_OPTIONAL;
										cmd = cmd
												.substring(1, cmd.length() - 1);
									}
									int code = CMParms.indexOf(
											QuestManager.QM_COMMAND_TYPES, cmd);
									if (code < 0) {
										Log.errOut(
												"Quests",
												"QuestMaker syntax error, '"
														+ cmd
														+ "' is an unknown command");
										pageDV.removeElementsAt(pageDV.size() - 1);
									} else
										pageDV.setElementAt(pageDV.size() - 1,
												1, Integer.valueOf(code | mask));
								}
							}
						} else if (foundStart) {
							if (pageDV == null) {
								if (s.length() == 0)
									templatesDV.setElementAt(
											templatesDV.size() - 1,
											2,
											((String) templatesDV.elementAt(
													templatesDV.size() - 1, 2))
													+ "\n\r\n\r");
								else
									templatesDV.setElementAt(
											templatesDV.size() - 1,
								...
								if ((pageDV.size() < 2)
										|| (((Integer) pageDV.elementAt(
												pageDV.size() - 1, 1))
												.intValue() == QuestManager.QM_COMMAND_$TITLE)
										|| (((Integer) pageDV.elementAt(
												pageDV.size() - 1, 1))
												.intValue() == QuestManager.QM_COMMAND_$LABEL)) {
									if (s.length() == 0) {
										if (((Integer) pageDV.elementAt(
												pageDV.size() - 1, 1))
												.intValue() == QuestManager.QM_COMMAND_$TITLE)
											pageDV.addElement(
													Integer.valueOf(QuestManager.QM_COMMAND_$LABEL),
													"", s, "");
										else
											pageDV.setElementAt(
													pageDV.size() - 1,
													3,
													((String) pageDV.elementAt(
															pageDV.size() - 1,
															3))
															+ "\n\r\n\r");
									} else
		...
		while (templatesDV.size() > 0) {
			...
			for (int t = 1; t < templatesDV.size(); t++)
				if (((String) templatesDV.elementAt(t, 1))
						.compareTo((String) templatesDV.elementAt(maxRow, 1)) < 0)
					maxRow = t;
	...
	protected String addXMLQuestMob(MOB mob, int showFlag, DVector pageDV,
			String showValue, String parm1Fixed, String lastLabel,
			boolean optionalEntry, int step, int showNumber) throws IOException {
		MOB M = null;
		Vector<MOB> choices = new Vector<MOB>();
		MOB baseM = ((showValue != null) ? baseM = CMLib.coffeeMaker()
				.getMobFromXML(showValue) : null);
		...
		lastLabel = ((lastLabel == null) ? "" : lastLabel) + "\n\rChoices: "
				+ choiceDescs.toString();
		String s = CMLib.genEd().prompt(mob, showName, showNumber, showFlag,
				parm1Fixed, optionalEntry, false, lastLabel,
		...
		String newValue = (M != null) ? CMLib.coffeeMaker().getMobXML(M)
				.toString() : showValue;
		for (int n = 0; n < newMobs.size(); n++)
			newMobs.elementAt(n).destroy();
		return newValue == null ? "" : newValue.trim();
	}

	protected String addXMLQuestItem(MOB mob, int showFlag, DVector pageDV,
			String showValue, String parm1Fixed, String lastLabel,
			boolean optionalEntry, int step, int showNumber) throws IOException {
		Item I = null;
		List<Item> choices = new Vector<Item>();
		Item baseI = ((showValue != null) ? baseI = CMLib.coffeeMaker()
				.getItemFromXML(showValue) : null);
		StringBuffer choiceDescs = new StringBuffer("");
		if (baseI != null) {
			choices.add(baseI);
		...
		String newValue = (I != null) ? CMLib.coffeeMaker().getItemXML(I)
				.toString() : showValue;
	...
	public List<Quest> getPlayerPersistantQuests(MOB player) {
		...
		for (int q = 0; q < CMLib.quests().numQuests(); q++) {
			...
			if (Q == null)
				continue;
			for (Enumeration<ScriptingEngine> e = player.scripts(); e
					.hasMoreElements();) {
	...
	public Quest questMaker(MOB mob) {
		...
		try {
			...
			int questIndex = -1;
			while ((questIndex < 0) && (mob.session() != null)
					&& (!mob.session().isStopped())) {
				String choice = mob.session().prompt(
						"Select a quest template (?): ", "");
				if (choice.equals("?")) {
					StringBuffer fullList = new StringBuffer(
							"\n\r^HCANCEL^N -- to cancel.\n\r");
					for (int t = 0; t < questTemplates.size(); t++)
						fullList.append("^H"
								+ (String) questTemplates.elementAt(t, 1)
								+ "^N\n\r"
								+ (String) questTemplates.elementAt(t, 2)
								+ "\n\r");
					...
					for (int t = 0; t < questTemplates.size(); t++)
						if (choice.equalsIgnoreCase((String) questTemplates
								.elementAt(t, 1)))
							questIndex = t;
						else
							list.append(((String) questTemplates
									.elementAt(t, 1)) + ", ");
					if (questIndex < 0)
						mob.tell("'"
								+ choice
								+ "' is not a valid quest name, use ? for a list, or select from the following: "
								+ list.toString().substring(0,
										list.length() - 2));
				}
			}
			if ((mob.session() == null) || (mob.session().isStopped())
					|| (questIndex < 0))
				return null;
			DVector qFDV = getQuestTemplate(mob,
					(String) questTemplates.elementAt(questIndex, 3));
			...
			for (int page = 0; page < qPages.size(); page++) {
				...
				while ((mob.session() != null) && (!mob.session().isStopped())
					...
					for (int step = 1; step < pageDV.size(); step++) {
						...
						String parm1Fixed = CMStrings
								.capitalizeAndLower(keyName.replace('_', ' '));
						if (parm1Fixed.startsWith("$"))
							parm1Fixed = parm1Fixed.substring(1);

						boolean optionalEntry = CMath.bset(stepType.intValue(),
								QuestManager.QM_COMMAND_OPTIONAL);
						int inputCode = stepType.intValue()
								& QuestManager.QM_COMMAND_MASK;
						switch (inputCode) {
						case QM_COMMAND_$TITLE:
							break;
						case QM_COMMAND_$HIDDEN:
							pageDV.setElementAt(step, 4, defValue == null ? ""
									: defValue);
							break;
						case QM_COMMAND_$LABEL:
							lastLabel = defValue;
							break;
						...
						case QM_COMMAND_$ROOMID: {
							String showValue = (showFlag < -900) ? defValue
									: (String) pageDV.elementAt(step, 4);
							if (inputCode == QM_COMMAND_$ZAPPERMASK)
								lastLabel = (lastLabel == null ? "" : lastLabel)
										+ "\n\r"
										+ CMLib.masking().maskHelp("\n\r",
												"disallows");
							String s = CMLib.genEd().prompt(mob, showValue,
									++showNumber, showFlag, parm1Fixed,
									optionalEntry, false, lastLabel,
						...
						case QM_COMMAND_$AREA: {
							String showValue = (showFlag < -900) ? ""
									: (String) pageDV.elementAt(step, 4);
							StringBuffer label = new StringBuffer(
									((lastLabel == null) ? "" : lastLabel)
											+ "\n\rChoices: ");
							for (Enumeration<Area> e = CMLib.map().areas(); e
									.hasMoreElements();)
								label.append("\"" + e.nextElement().name()
										+ "\" ");
							String s = CMLib.genEd().prompt(mob, showValue,
									++showNumber, showFlag, parm1Fixed,
									optionalEntry, false, label.toString(),
									QuestManager.QM_COMMAND_TESTS[inputCode],
									null);
							pageDV.setElementAt(step, 4, s);
							break;
						...
						case QM_COMMAND_$EXISTING_QUEST_NAME: {
							...
							for (int q = 0; q < CMLib.quests().numQuests(); q++)
								label.append("\""
										+ CMLib.quests().fetchQuest(q).name()
										+ "\" ");
							String s = CMLib.genEd().prompt(mob, showValue,
									++showNumber, showFlag, parm1Fixed,
									optionalEntry, false, label.toString(),
						...
						case QM_COMMAND_$CHOOSE: {
							String showValue = (showFlag < -900) ? ""
									: (String) pageDV.elementAt(step, 4);
							String label = ((lastLabel == null) ? ""
									: lastLabel) + "\n\rChoices: " + defValue;
							String s = CMLib.genEd().prompt(
									mob,
									showValue,
									++showNumber,
									showFlag,
									...
									label,
									QuestManager.QM_COMMAND_TESTS[inputCode],
									CMParms.toStringArray(CMParms.parseCommas(
											defValue.toUpperCase(), true)));
							pageDV.setElementAt(step, 4, s);
							break;
						}
						case QM_COMMAND_$ITEMXML: {
							String showValue = (showFlag < -900) ? ""
									: (String) pageDV.elementAt(step, 4);
							String newValue = addXMLQuestItem(mob, showFlag,
									pageDV, showValue, parm1Fixed, lastLabel,
									optionalEntry, step, ++showNumber);
							if (newValue != null)
						...
						case QM_COMMAND_$ITEMXML_ONEORMORE: {
							...
							while (x >= 0) {
								String xml = showValue.substring(0, x + 7)
										.trim();
								if (xml.length() > 0)
							...
							for (int i = 0; i <= itemXMLs.size(); i++) {
								showValue = (i < itemXMLs.size()) ? (String) itemXMLs
										.elementAt(i) : "";
								boolean optional = (i == 0) ? optionalEntry
										: true;
								String thisValue = addXMLQuestItem(mob,
										showFlag, pageDV, showValue,
										parm1Fixed, lastLabel, optional, step,
										++showNumber);
								if (thisValue != null) {
									if (newValue == null)
										newValue = "";
						...
						case QM_COMMAND_$MOBXML: {
							String showValue = (showFlag < -900) ? ""
									: (String) pageDV.elementAt(step, 4);
							String newValue = addXMLQuestMob(mob, showFlag,
									pageDV, showValue, parm1Fixed, lastLabel,
									optionalEntry, step, ++showNumber);
							if (newValue != null)
								pageDV.setElementAt(step, 4, newValue);
						...
						case QM_COMMAND_$MOBXML_ONEORMORE: {
							...
							while (x >= 0) {
								String xml = showValue.substring(0, x + 6)
										.trim();
								if (xml.length() > 0)
							...
							for (int i = 0; i <= mobXMLs.size(); i++) {
								showValue = (i < mobXMLs.size()) ? (String) mobXMLs
										.elementAt(i) : "";
								boolean optional = (i == 0) ? optionalEntry
										: true;
								String thisValue = addXMLQuestMob(mob,
										showFlag, pageDV, showValue,
										parm1Fixed, lastLabel, optional, step,
										++showNumber);
								if (thisValue != null) {
						...
						case QM_COMMAND_$FACTION: {
							...
							StringBuffer label = new StringBuffer(
									((lastLabel == null) ? "" : lastLabel)
											+ "\n\rChoices: ");
							for (Enumeration<Faction> f = CMLib.factions()
									.factions(); f.hasMoreElements();)
					...
					String what = mob.session().prompt(
							"Edit which (enter 0 to cancel)? ", "");
					if (what.trim().equals("0")) {
						if (mob.session().confirm(
								"Are you sure you want to abort (y/N)? ", "N")) {
							mob.tell("Aborted.");
							return null;
						}
					}
			...
			if ((mob.session() != null)
					&& (!mob.session().isStopped())
					&& (mob.session().confirm("Create the new quest: " + name
							+ " (y/N)? ", "N"))) {
				Quest Q = (Quest) CMClass.getCommon("DefaultQuest");
				CMFile newQF = new CMFile(
						Resources.makeFileResourceName("quests/" + name
								+ ".quest"), mob, CMFile.FLAG_LOGERRORS);
