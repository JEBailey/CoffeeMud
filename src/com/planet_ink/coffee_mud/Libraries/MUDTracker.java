...
public class MUDTracker extends StdLibrary implements TrackingLibrary {
	...
	protected Vector<String> getDirectionCommandSet(int direction) {
		...
		if (!directionCommandSets.containsKey(dir)) {
			Vector<String> V = new ReadOnlyVector<String>(
	...
	protected Vector<String> getOpenCommandSet(int direction) {
		Integer dir = Integer.valueOf(direction);
		if (!directionCommandSets.containsKey(dir)) {
			Vector<String> V = new ReadOnlyVector<String>(CMParms.parse("OPEN "
					+ Directions.getDirectionName(direction)));
			directionCommandSets.put(dir, V);
		}
		return directionCommandSets.get(dir);
	...
	public List<Room> findBastardTheBestWay(Room location, Room destRoom,
			TrackingFlags flags, int maxRadius) {
		return findBastardTheBestWay(location, destRoom, flags, maxRadius, null);
	}

	public List<Room> findBastardTheBestWay(Room location, Room destRoom,
			TrackingFlags flags, int maxRadius, List<Room> radiant) {
		...
		if ((radiant.size() > 0)
				&& (destRoom == radiant.get(radiant.size() - 1))) {
				...
				while (index >= 0) {
					...
					for (int i = index; i >= 0; i--) {
						...
						for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
							if ((R.getRoomInDir(d) == thisTrail.get(thisTrail
									.size() - 1))
	...
	public List<Room> findBastardTheBestWay(Room location,
			List<Room> destRooms, TrackingFlags flags, int maxRadius) {
		...
		for (int i = 0; (i < 5) && (destRooms.size() > 0); i++) {
			...
			List<Room> thisTrail = findBastardTheBestWay(location, destRoom,
					flags, maxRadius, radiant);
			if ((thisTrail != null)
					&& ((finalTrail == null) || (thisTrail.size() < finalTrail
							.size())))
		...
		if (finalTrail == null)
			for (int r = 0; r < destRooms.size(); r++) {
				...
				List<Room> thisTrail = findBastardTheBestWay(location,
						destRoom, flags, maxRadius);
				if ((thisTrail != null)
						&& ((finalTrail == null) || (thisTrail.size() < finalTrail
								.size())))
					finalTrail = thisTrail;
	...
	public int trackNextDirectionFromHere(List<Room> theTrail, Room location,
			boolean openOnly) {
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if ((R != null) && (E != null) && ((!openOnly) || (E.isOpen()))) {
	...
	public List<Room> getRadiantRooms(final Room room,
			final TrackingFlags flags, final int maxDepth) {
		final List<Room> V = new Vector<Room>();
		getRadiantRooms(room, V, flags, null, maxDepth, null);
		return V;
	}

	public List<Room> getRadiantRooms(final Room room, final RFilters filters,
			final int maxDepth) {
		final List<Room> V = new Vector<Room>();
	...
	public void getRadiantRooms(final Room room, List<Room> rooms,
			TrackingFlags flags, final Room radiateTo, final int maxDepth,
			final Set<Room> ignoreRooms) {
		if (flags == null)
			flags = EMPTY_FLAGS;
	...
	public void getRadiantRooms(final Room room, List<Room> rooms,
			final RFilters filters, final Room radiateTo, final int maxDepth,
			final Set<Room> ignoreRooms) {
		int depth = 0;
		if (room == null)
			return;
		...
		while (depth < maxDepth) {
			for (r = min; r < size; r++) {
				...
				for (d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
					...
					if ((R == null) || (E == null) || (H.contains(R))
							|| (filters.isFilteredOut(R, E, d)))
						continue;
					rooms.add(R);
					H.add(R);
					if (R == radiateTo) // R can't be null here, so if they are
										// equal, time to go!
						return;
				}
			}
		...
		if (mobsThere.size() > 0) {
			for (MOB inhab : mobsThere) {
				if ((inhab.session() != null)
						&& (CMSecurity.isAllowed(inhab, R,
								CMSecurity.SecFlag.CMDMOBS) || CMSecurity
								.isAllowed(inhab, R,
	...
	public boolean beMobile(final MOB mob, final boolean dooropen,
			final boolean wander, final boolean roomprefer,
			final boolean roomobject, final long[] status,
			final List<Room> rooms) {
		return beMobile(mob, dooropen, wander, roomprefer, roomobject, true,
				status, rooms);

	}

	private boolean beMobile(final MOB mob, boolean dooropen,
			final boolean wander, final boolean roomprefer,
			final boolean roomobject, final boolean sneakIfAble,
			final long[] status, final List<Room> rooms) {
		if (status != null)
		...
		if (((mob instanceof Rideable) && (((Rideable) mob).numRiders() > 0))
				|| ((mob.amFollowing() != null) && (mob.location() == mob
						.amFollowing().location()))) {
			if (status != null)
		...
		while (((tries++) < 10) && (direction < 0)) {
			...
			if ((nextRoom != null) && (nextExit != null)) {
				...
				Exit opExit = nextRoom.getExitInDir(Directions
						.getOpDirectionCode(direction));
				if (CMLib.flags().isTrapped(nextExit)
						|| (CMLib.flags().isHidden(nextExit) && (!CMLib.flags()
								.canSeeHidden(mob)))
						|| (CMLib.flags().isInvisible(nextExit) && (!CMLib
								.flags().canSeeInvisible(mob))))
					direction = -1;
				else if ((opExit != null) && (CMLib.flags().isTrapped(opExit)))
					direction = -1;
				else if ((oldRoom.domainType() != nextRoom.domainType())
						&& (!CMLib.flags().isInFlight(mob))
						&& ((nextRoom.domainType() == Room.DOMAIN_INDOORS_AIR) || (nextRoom
								.domainType() == Room.DOMAIN_OUTDOORS_AIR)))
					direction = -1;
				else if ((oldRoom.domainType() != nextRoom.domainType())
						&& (!CMLib.flags().isSwimming(mob))
						&& ((nextRoom.domainType() == Room.DOMAIN_INDOORS_UNDERWATER) || (nextRoom
								.domainType() == Room.DOMAIN_OUTDOORS_UNDERWATER)))
					direction = -1;
				else if ((!wander)
						&& (!oldRoom.getArea().Name()
								.equals(nextRoom.getArea().Name())))
					direction = -1;
				else if ((roomobject) && (rooms != null)
						&& (rooms.contains(nextRoom)))
					direction = -1;
				else if ((roomprefer) && (rooms != null)
						&& (!rooms.contains(nextRoom)))
					direction = -1;
				else
					break;
		...
		if (nextExit.hasADoor() && (!nextExit.isOpen()) && (dooropen)) {
			if ((nextExit.hasALock()) && (nextExit.isLocked())) {
				CMMsg msg = CMClass.getMsg(mob, nextExit, null,
						CMMsg.MSG_OK_VISUAL, CMMsg.MSG_OK_VISUAL,
						CMMsg.MSG_OK_VISUAL, null);
				if (oldRoom.okMessage(mob, msg)) {
					relock = true;
					msg = CMClass.getMsg(mob, nextExit, null,
							CMMsg.MSG_OK_VISUAL, CMMsg.MSG_UNLOCK,
							CMMsg.MSG_OK_VISUAL,
							"<S-NAME> unlock(s) <T-NAMESELF>.");
					if (oldRoom.okMessage(mob, msg))
						CMLib.utensils().roomAffectFully(msg, oldRoom,
								direction);
				}
			}
			if (!nextExit.isOpen()) {
				mob.doCommand(getOpenCommandSet(direction),
		...
		if (((nextRoom.domainType() == Room.DOMAIN_OUTDOORS_WATERSURFACE) || (nextRoom
		...
		} else if (((nextRoom.ID().indexOf("Surface") > 0)
				|| (CMLib.flags().isClimbing(nextExit)) || (CMLib.flags()
				.isClimbing(nextRoom)))
				&& (!CMLib.flags().isClimbing(mob))
				&& (!CMLib.flags().isInFlight(mob))
				&& ((mob.fetchAbility("Skill_Climb") != null) || (mob
						.fetchAbility("Power_SuperClimb") != null))) {
			...
			if (A.proficiency() < 50) {
				...
				if (A2 != null)
					A2.setProficiency(CMLib.dice().roll(1, 50,
							A.adjustedLevel(mob, 0) * 15));
			}
		...
		if ((reclose) && (mob.location() == nextRoom) && (dooropen)) {
			...
			if ((opExit != null) && (opExit.hasADoor()) && (opExit.isOpen())) {
				mob.doCommand(getCloseCommandSet(opDirection),
						Command.METAFLAG_FORCED);
				if ((opExit.hasALock()) && (relock)) {
					CMMsg msg = CMClass.getMsg(mob, opExit, null,
							CMMsg.MSG_OK_VISUAL, CMMsg.MSG_OK_VISUAL,
							CMMsg.MSG_OK_VISUAL, null);
					if (nextRoom.okMessage(mob, msg)) {
						msg = CMClass.getMsg(mob, opExit, null,
								CMMsg.MSG_OK_VISUAL, CMMsg.MSG_LOCK,
								CMMsg.MSG_OK_VISUAL,
								"<S-NAME> lock(s) <T-NAMESELF>.");
						if (nextRoom.okMessage(mob, msg))
							CMLib.utensils().roomAffectFully(msg, nextRoom,
									opDirection);
					}
				}
			}
	...
	public void wanderIn(MOB M, Room toHere) {
		...
		while ((dir < 0) && ((++tries) < 100)) {
			...
			if (R != null) {
				if (((R.domainType() == Room.DOMAIN_INDOORS_AIR) && (!CMLib
						.flags().isFlying(M)))
						|| ((R.domainType() == Room.DOMAIN_OUTDOORS_AIR) && (!CMLib
								.flags().isFlying(M)))
						|| ((R.domainType() == Room.DOMAIN_INDOORS_UNDERWATER) && (!CMLib
								.flags().isSwimming(M)))
						|| ((R.domainType() == Room.DOMAIN_OUTDOORS_UNDERWATER) && (!CMLib
								.flags().isSwimming(M)))
						|| ((R.domainType() == Room.DOMAIN_OUTDOORS_WATERSURFACE) && (!CMLib
								.flags().isWaterWorthy(M)))
						|| ((R.domainType() == Room.DOMAIN_OUTDOORS_WATERSURFACE) && (!CMLib
								.flags().isWaterWorthy(M))))
					dir = -1;
			} else
		...
		if (dir < 0)
			toHere.show(M, null, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> wanders in.");
		else {
			final String inDir = ((toHere instanceof SpaceShip) || (toHere
					.getArea() instanceof SpaceShip)) ? Directions
					.getShipDirectionName(dir) : Directions
					.getDirectionName(dir);
			toHere.show(M, null, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> wanders in from " + inDir + ".");
		}
		toHere.bringMobHere(M, true);
	}

	public void ridersBehind(List<Rider> riders, Room sourceRoom,
			Room destRoom, int directionCode, boolean flee, boolean running) {
		if (riders != null)
			for (Rider rider : riders) {
				if (rider instanceof MOB) {
					...
					if ((rMOB.location() == sourceRoom)
						...
						if (rMOB.location() == sourceRoom) {
							if (rMOB.riding() != null) {
								final String inDir = ((sourceRoom instanceof SpaceShip) || (sourceRoom
										.getArea() instanceof SpaceShip)) ? Directions
										.getShipDirectionName(directionCode)
										: Directions
												.getDirectionName(directionCode);
								rMOB.tell("You ride " + rMOB.riding().name()
										+ " " + inDir + ".");
							}
	...
	public List<Rider> ridersAhead(Rider theRider, Room sourceRoom,
			Room destRoom, int directionCode, boolean flee, boolean running) {
		...
		while ((riding != null) && (riding.mobileRideBasis())) {
			...
			if ((riding instanceof Rider)
					&& ((Rider) riding).riding() != theRider.riding())
		...
		for (ListIterator<Rideable> r = rideables.listIterator(); r.hasNext();) {
			...
			if ((riding instanceof Item)
				...
				final String inDir = ((sourceRoom instanceof SpaceShip) || (sourceRoom
						.getArea() instanceof SpaceShip)) ? Directions
						.getShipDirectionName(directionCode) : Directions
						.getDirectionName(directionCode);
				((MOB) riding).tell("You are ridden " + inDir + ".");
				if (!move(((MOB) riding), directionCode, false, false, true,
						false, running)) {
					if (theRider instanceof MOB)
						((MOB) theRider).tell(((MOB) riding).name()
								+ " won't seem to let you go that way.");
					for (; r.hasPrevious();) {
						riding = r.previous();
						if ((riding instanceof Item)
	...
	public boolean walk(MOB mob, int directionCode, boolean flee,
			boolean nolook, boolean noriders) {
		return walk(mob, directionCode, flee, nolook, noriders, false);
	}

	public boolean run(MOB mob, int directionCode, boolean flee,
			boolean nolook, boolean noriders) {
		return run(mob, directionCode, flee, nolook, noriders, false);
	}

	public boolean walk(MOB mob, int directionCode, boolean flee,
			boolean nolook, boolean noriders, boolean always) {
	...
	public boolean move(final MOB mob, final int directionCode,
			final boolean flee, final boolean nolook, final boolean noriders,
			final boolean always, final boolean running) {
		if (directionCode < 0)
			return false;
		if (mob == null)
			return false;
		...
		final boolean useShipDirs = ((thisRoom instanceof SpaceShip) || (thisRoom
		...
		final String dirName = useShipDirs ? Directions
				.getShipDirectionName(directionCode) : Directions
				.getDirectionName(directionCode);
		final String fromDir = useShipDirs ? Directions
				.getShipFromDirectionName(opDir) : Directions
				.getFromDirectionName(opDir);
		final String directionName = (directionCode == Directions.GATE)
				&& (exit != null) ? "through " + exit.name() : dirName;
		final String otherDirectionName = (Directions
				.getOpDirectionCode(directionCode) == Directions.GATE)
				&& (exit != null) ? exit.name() : fromDir;
		...
		if ((mob.riding() != null) && (mob.riding().mobileRideBasis())) {
			enterMsg = CMClass.getMsg(mob, destRoom, exit, generalMask
					| CMMsg.MSG_ENTER, null, CMMsg.MSG_ENTER, null,
					CMMsg.MSG_ENTER, "<S-NAME> ride(s) " + mob.riding().name()
							+ " in from " + otherDirectionName + ".");
			leaveMsg = CMClass.getMsg(mob, thisRoom, opExit, leaveCode,
					((flee) ? "You flee " + directionName + "." : null),
					leaveCode, null, leaveCode,
					((flee) ? "<S-NAME> flee(s) with " + mob.riding().name()
							+ " " + directionName + "." : "<S-NAME> ride(s) "
							+ mob.riding().name() + " " + directionName + "."));
		} else {
			enterMsg = CMClass.getMsg(
					mob,
					destRoom,
					exit,
					generalMask | CMMsg.MSG_ENTER,
					null,
					CMMsg.MSG_ENTER,
					null,
					CMMsg.MSG_ENTER,
					"<S-NAME> "
							+ CMLib.flags().dispositionString(mob,
									CMFlagLibrary.flag_arrives) + " from "
							+ otherDirectionName + ".");
			leaveMsg = CMClass.getMsg(
					mob,
					thisRoom,
					opExit,
					leaveCode,
					...
					null,
					leaveCode,
					((flee) ? "<S-NAME> flee(s) " + directionName + "."
							: "<S-NAME> "
									+ CMLib.flags().dispositionString(mob,
											CMFlagLibrary.flag_leaves) + " "
									+ directionName + "."));
		}
		final boolean gotoAllowed = (!mob.isMonster())
				&& CMSecurity.isAllowed(mob, destRoom, CMSecurity.SecFlag.GOTO);
		if ((exit == null) && (!gotoAllowed)) {
			mob.tell("You can't go that way.");
			return false;
		} else if (exit == null)
			thisRoom.showHappens(CMMsg.MSG_OK_VISUAL, "The area to the "
					+ directionName + " shimmers and becomes transparent.");
		...
		if (mob.riding() != null) {
			...
			if (!mob.isMonster()) {
				final int expense = running ? CMProps
						.getIntVar(CMProps.Int.RUNCOST) : CMProps
						.getIntVar(CMProps.Int.WALKCOST);
				for (int i = 0; i < expense; i++)
					mob.curState().expendEnergy(mob, mob.maxState(), true);
			}
			if ((!flee) && (mob.curState().getMovement() <= 0)
					&& (!gotoAllowed)) {
				mob.tell("You are too tired.");
				return false;
			}
			if ((mob.soulMate() == null) && (mob.playerStats() != null)
					&& (mob.riding() == null) && (mob.location() != null))
				mob.playerStats().adjHygiene(mob.location().pointsPerMove(mob));
		}

		List<Rider> riders = null;
		if (!noriders) {
			riders = ridersAhead(mob, (Room) leaveMsg.target(),
					(Room) enterMsg.target(), directionCode, flee, running);
			if (riders == null)
				return false;
		}
		List<CMMsg> enterTrailersSoFar = null;
		List<CMMsg> leaveTrailersSoFar = null;
		if ((leaveMsg.trailerMsgs() != null)
				&& (leaveMsg.trailerMsgs().size() > 0)) {
			leaveTrailersSoFar = new LinkedList<CMMsg>();
			leaveTrailersSoFar.addAll(leaveMsg.trailerMsgs());
			leaveMsg.trailerMsgs().clear();
		}
		if ((enterMsg.trailerMsgs() != null)
				&& (enterMsg.trailerMsgs().size() > 0)) {
		...
		if (!nolook) {
			...
			if ((!mob.isMonster())
					...
					&& (((Room) enterMsg.target()).getArea().getClimateObj()
							.weatherType(((Room) enterMsg.target())) != Climate.WEATHER_CLEAR)
					&& (((Room) enterMsg.target()).isInhabitant(mob)))
				mob.tell("\n\r"
						+ ((Room) enterMsg.target()).getArea().getClimateObj()
								.weatherDescription(((Room) enterMsg.target())));
		}

		if (!noriders)
			ridersBehind(riders, (Room) leaveMsg.target(),
					(Room) enterMsg.target(), directionCode, flee, running);
			...
			for (int f = 0; f < mob.numFollowers(); f++) {
				...
				if (follower != null) {
					if ((follower.amFollowing() == mob)
							&& ((follower.location() == thisRoom) || (follower
									.location() == destRoom))) {
						if ((follower.location() == thisRoom)
								&& (CMLib.flags().aliveAwakeMobile(follower,
										true))) {
							if (CMath.bset(follower.getBitmap(),
									MOB.ATT_AUTOGUARD))
								thisRoom.show(follower, null, null,
										CMMsg.MSG_OK_ACTION,
										"<S-NAME> remain(s) on guard here.");
							else {
								final String inDir = ((thisRoom instanceof SpaceShip) || (thisRoom
										.getArea() instanceof SpaceShip)) ? Directions
										.getShipDirectionName(directionCode)
										: Directions
												.getDirectionName(directionCode);
								follower.tell("You follow "
										+ mob.name(follower) + " " + inDir
										+ ".");
				...
				if ((e != null) && (r != null)) {
					if ((CMLib.flags().canBeSeenBy(e, mob))
							&& ((e.name().equalsIgnoreCase(desc))
									...
									|| (r.displayText(mob)
											.equalsIgnoreCase(desc)) || (e
										.description().equalsIgnoreCase(desc)))) {
						dir = d;
						break;
					}
				}
		...
		if (dir < 0)
			for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
				...
				if ((e != null) && (r != null)) {
					if ((CMLib.flags().canBeSeenBy(e, mob))
							&& (((CMLib.english()
									.containsString(e.name(), desc))
									|| (CMLib.english().containsString(
											e.displayText(), desc))
									|| (CMLib.english().containsString(
											r.displayText(), desc)) || (CMLib
	...
	public List<List<Integer>> findAllTrails(final Room from, final Room to,
			final List<Room> radiantTrail) {
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if (R != null) {
				if ((R == from)
						&& (from.getRoomInDir(Directions.getOpDirectionCode(d)) == to)) {
					finalSets.add(new XVector<Integer>(Integer
							.valueOf(Directions.getOpDirectionCode(d))));
					return finalSets;
				}
				int dex = radiantTrail.indexOf(R);
				if ((dex >= 0)
						&& (dex < index)
						&& (R.getRoomInDir(Directions.getOpDirectionCode(d)) == to)) {
					List<List<Integer>> allTrailsBack = findAllTrails(from, R,
							radiantTrail);
					for (int a = 0; a < allTrailsBack.size(); a++) {
						List<Integer> thisTrail = allTrailsBack.get(a);
						thisTrail.add(Integer.valueOf(Directions
								.getOpDirectionCode(d)));
	...
	public List<List<Integer>> findAllTrails(final Room from,
			final List<Room> tos, final List<Room> radiantTrail) {
		List<List<Integer>> finalSets = new Vector<List<Integer>>();
		if (from == null)
			return finalSets;
		Room to = null;
	...
	protected int getRoomDirection(Room R, Room toRoom, List<Room> ignore) {
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--)
			if ((R.getRoomInDir(d) == toRoom) && (R != toRoom)
					&& (!ignore.contains(R)))
				return d;
		return -1;
	}

	public String getTrailToDescription(Room R1, List<Room> set, String where,
			boolean areaNames, boolean confirm, int radius,
			Set<Room> ignoreRooms, int maxMins) {
			...
			for (Enumeration<Area> a = CMLib.map().areas(); a.hasMoreElements();) {
				...
				if (A.name().equalsIgnoreCase(where)) {
					if (set.size() == 0) {
						...
						for (Enumeration<Room> r = A.getCompleteMap(); r
								.hasMoreElements();) {
							Room R = r.nextElement();
							int x = R.roomID().indexOf('#');
							if ((x >= 0)
									&& (CMath
											.s_int(R.roomID().substring(x + 1)) < lowest))
								lowest = CMath.s_int(R.roomID()
										.substring(x + 1));
						}
		...
		for (int s = trailV.size() - 1; s >= 1; s--) {
			...
			theDirTrail.add(Directions.getDirectionChar(getRoomDirection(R, RA,
					empty)) + " ");
		}
		...
		if ((confirm) && (trailV.size() > 1)) {
			...
			Room R = trailV.get(1);
			theTrail.append("\n\r"
					+ CMStrings.padRight("From", 30)
					+ ": "
					+ Directions
							.getDirectionName(getRoomDirection(R, R2, empty))
					+ " <- " + R.roomID());
			theTrail.append("\n\r" + CMStrings.padRight("Room", 30) + ": "
					+ R.displayText() + "/" + R.description());
			theTrail.append("\n\r\n\r");
		}
