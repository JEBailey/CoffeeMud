...
public class BeanCounter extends StdLibrary implements MoneyLibrary {
	...
	protected MoneyDenomination[] createCurrencySet(
			Hashtable<String, MoneyDenomination[]> currencies, String currency) {
		...
		for (int v = 0; v < CV.size(); v++) {
			...
			if ((s.length() > 0) && (d > 0.0)) {
				...
				if ((insertAt < 0) || (insertAt >= DV.size()))
					...
					DV.insertElementAt(new MoneyDenomination(d, s, shortName),
							insertAt);
	...
	public double abbreviatedRePrice(String currency, double absoluteAmount) {
		double lowDenom = lowestAbbreviatedDenomination(currency,
				absoluteAmount);
	...
	public String abbreviatedPrice(String currency, double absoluteAmount) {
		double lowDenom = lowestAbbreviatedDenomination(currency,
				absoluteAmount);
		long lowAmt = Math.round(absoluteAmount / lowDenom);
		String denominationShortCode = getDenominationShortCode(currency,
				lowDenom);
		...
		if (DV != null) {
			...
			for (int d = DV.length - 1; d >= 0; d--) {
				...
				if ((denom <= absoluteValue)
						&& (absoluteValue
								- (Math.floor(absoluteValue / denom) * denom) < low))
					return denom;
			}
		}
		return denom;
	...
	public double getBestDenomination(String currency, int numberOfCoins,
	...
	public String getConvertableDescription(String currency, double denomination) {
		...
		if (low == denomination)
			return "";
		return "Equal to "
				+ getDenominationName(currency, low,
						Math.round(Math.floor(denomination / low))) + ".";
	}
	...
	public String nameCurrencyShort(String currency, double absoluteValue) {
		...
		if (denom > 0.0)
			return getDenominationName(currency, denom,
					Math.round(Math.floor(absoluteValue / denom)));
		return getDenominationName(currency, denom,
				Math.round(Math.floor(absoluteValue)));
	}
	...
	public Coins makeBestCurrency(MOB mob, double absoluteValue,
			Environmental owner, Container container) {
		return makeBestCurrency(getCurrency(mob), absoluteValue, owner,
				container);
	}
	...
	protected void parseDebt(Vector<DebtItem> debt, String debtor, String xml) {
		...
		List<XMLLibrary.XMLpiece> debtData = CMLib.xml().getContentsFromPieces(
				V, "DEBT");
		if (debtData == null) {
		...
		for (int p = 0; p < debtData.size(); p++) {
			...
			if ((!ablk.tag.equalsIgnoreCase("OWE")) || (ablk.contents == null)
					|| (ablk.contents.size() == 0))
				continue;
			...
			double interest = CMLib.xml().getDoubleFromPieces(ablk.contents,
					"INT");
			debt.addElement(new DebtItem(debtor, owed, amt, reason, due,
					interest));
		}
	}

	protected String unparseDebt(Vector<DebtItem> debt, String name,
			String owedTo) {
		...
		for (int d = 0; d < debt.size(); d++) {
			if ((debt.elementAt(d).debtor.equalsIgnoreCase(name))
				...
				xml.append("<OWE>");
				xml.append(CMLib.xml().convertXMLtoTag("TO",
						debt.elementAt(d).owedTo));
				xml.append(CMLib.xml().convertXMLtoTag("AMT",
						"" + debt.elementAt(d).amt));
				xml.append(CMLib.xml().convertXMLtoTag("FOR",
						debt.elementAt(d).reason));
				xml.append(CMLib.xml().convertXMLtoTag("DUE",
						"" + debt.elementAt(d).due));
				xml.append(CMLib.xml().convertXMLtoTag("INT",
						"" + debt.elementAt(d).interest));
				xml.append("</OWE>");
			}
		}
		xml.append("</DEBT>");
	...
	public double getDebtOwed(String name, String owedTo) {
		String key = name.toUpperCase() + "-DEBT-"
				+ owedTo.toUpperCase().trim();
	...
	public Vector<DebtItem> getDebtOwed(String owedTo) {
		List<PlayerData> rows = CMLib.database().DBReadDataKey("DEBT",
				".*-DEBT-" + owedTo.toUpperCase().trim());
	...
	public void adjustDebt(String name, String owedTo, double adjustAmt,
			String reason, double interest, long due) {
		String key = name.toUpperCase() + "-DEBT-"
				+ owedTo.toUpperCase().trim();
		synchronized (key.intern()) {
			...
			for (int d = 0; d < debts.size(); d++) {
				...
				if ((debt.debtor.equalsIgnoreCase(name))
						...
						&& (debt.interest == interest) && (debt.due == due)
			...
			if ((!done) && (adjustAmt >= 0.0))
				debts.addElement(new DebtItem(name, owedTo, adjustAmt, reason,
						due, interest));

			String xml = unparseDebt(debts, name, owedTo);
			if (update) {
				if (debts.size() == 0)
					CMLib.database().DBDeleteData(name.toUpperCase(), "DEBT",
							key);
				else
					CMLib.database().DBUpdateData(key, xml);
			} else
				CMLib.database().DBCreateData(name.toUpperCase(), "DEBT", key,
						xml);
		}
	}

	public Vector<DebtItem> getDebt(String name, String owedTo) {
		List<PlayerData> rows = CMLib.database().DBReadData(name.toUpperCase(),
				"DEBT",
				name.toUpperCase() + "-DEBT-" + owedTo.toUpperCase().trim());
		Vector<DebtItem> debt = new Vector<DebtItem>(rows.size());
		for (int r = 0; r < rows.size(); r++) {
			PlayerData row = rows.get(r);
			String debtor = row.who;
	...
	public Coins makeCurrency(String currency, double denomination,
			long numberOfCoins) {
		if (numberOfCoins > 0) {
	...
	public void addMoney(MOB customer, Container container, String currency,
			int absoluteValue) {
	...
	public void addMoney(MOB mob, Container container, String currency,
			double absoluteValue) {
		if (mob == null)
			return;
	...
	public void giveSomeoneMoney(MOB recipient, double absoluteValue) {
		giveSomeoneMoney(recipient, recipient, getCurrency(recipient),
				absoluteValue);
	}

	public void giveSomeoneMoney(MOB recipient, String currency,
			double absoluteValue) {
	...
	public void giveSomeoneMoney(MOB banker, MOB customer, String currency,
			double absoluteValue) {
		if (banker == null)
			banker = customer;
		if (banker == customer) {
		...
		for (int i = 0; i < V.size(); i++) {
			Coins C = V.get(i);
			banker.addItem(C);
			CMMsg newMsg = CMClass.getMsg(banker, customer, C, CMMsg.MSG_GIVE,
					"<S-NAME> give(s) " + C.Name() + " to <T-NAMESELF>.");
	...
	public void dropMoney(Room R, Container container, String currency,
			double absoluteValue) {
		List<Coins> V = makeAllCurrency(currency, absoluteValue);
		for (Coins I : V) {
	...
	public void removeMoney(Room R, Container container, String currency,
			double absoluteValue) {
	...
	public void bankLedger(String bankName, String owner, String explanation) {
		synchronized ((this + "LEDGER" + bankName).intern()) {
			List<PlayerData> V = CMLib.database().DBReadData(owner,
					"LEDGER-" + bankName, "LEDGER-" + bankName + "/" + owner);
			if ((V != null) && (V.size() > 0)) {
				...
				CMLib.database().DBReCreateData(owner, D.section, D.key,
						last + explanation + ";|;");
			} else
				CMLib.database()
						.DBCreateData(owner, "LEDGER-" + bankName,
								"LEDGER-" + bankName + "/" + owner,
								explanation + ";|;");
		}
	}

	public boolean modifyBankGold(String bankName, String owner,
			String explanation, String currency, double absoluteAmount) {
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (last.startsWith("COINS;")) {
				if ((bankName == null) || (bankName.length() == 0)
						|| (bankName.equals(D.section))) {
					Coins C = (Coins) CMClass.getItem("StdCoins");
					CMLib.coffeeMaker().setPropertiesStr(C, last.substring(6),
							true);
					if ((C.getDenomination() == 0.0)
							&& (C.getNumberOfCoins() > 0))
					...
					if ((absoluteAmount > 0.0) || (value >= (-absoluteAmount))) {
						...
						if (C != null)
							CMLib.database().DBReCreateData(
									...
									"COINS;"
											+ CMLib.coffeeMaker()
													.getPropertiesStr(C, true));
						else
							CMLib.database().DBDeleteData(owner, D.section,
									D.key);
	...
	public boolean modifyThisAreaBankGold(Area A, Set<String> triedBanks,
			String owner, String explanation, String currency,
			double absoluteAmount) {
		Banker B = null;
		Room R = null;
		for (Enumeration<Banker> e = CMLib.map().banks(); e.hasMoreElements();) {
			...
			if ((R != null) && (R.getArea() == A)
				...
				if (modifyBankGold(B.bankChain(), owner, explanation, currency,
						absoluteAmount))
					return true;
			}
		}
	...
	public boolean modifyLocalBankGold(Area A, String owner,
			String explanation, String currency, double absoluteAmount) {
		HashSet<String> triedBanks = new HashSet<String>();
		if (modifyThisAreaBankGold(A, triedBanks, owner, explanation, currency,
				absoluteAmount))
			return true;
		for (Enumeration<Area> e = A.getParents(); e.hasMoreElements();) {
			Area A2 = e.nextElement();
			if (modifyThisAreaBankGold(A2, triedBanks, owner, explanation,
					currency, absoluteAmount))
				return true;
		}
		return modifyBankGold(null, owner, explanation, currency,
				absoluteAmount);
	}
	...
	public void subtractMoneyGiveChange(MOB banker, MOB mob,
			double absoluteAmount) {
		subtractMoneyGiveChange(banker, mob,
				(banker != null) ? getCurrency(banker) : getCurrency(mob),
				absoluteAmount);
	}

	public void subtractMoneyGiveChange(MOB banker, MOB mob, String currency,
			double absoluteAmount) {
		if (mob == null)
			return;
	...
	public void subtractMoney(MOB mob, Container container, String currency,
			double absoluteAmount) {
		if (mob == null)
			return;
	...
	public void subtractMoney(MOB mob, double denomination,
			double absoluteAmount) {
		subtractMoney(mob, getCurrency(mob), denomination, absoluteAmount);
	}

	public void subtractMoney(MOB mob, String currency, double denomination,
			double absoluteAmount) {
		if (mob == null)
			return;
		List<Coins> V = getStandardCurrency(mob, currency);
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (C.getDenomination() == denomination) {
				if (C.getTotalValue() > absoluteAmount) {
					C.setNumberOfCoins(Math.round(Math.floor((C.getTotalValue() - absoluteAmount)
							/ denomination)));
					C.text();
					break;
				}
	...
	public List<Coins> getStandardCurrency(Room R, Item container,
			String currency) {
		Vector<Coins> V = new Vector<Coins>();
		if (R == null)
			return V;
		for (int i = 0; i < R.numItems(); i++) {
			...
			if ((I != null)
					&& (I instanceof Coins)
					&& ((currency == null) || ((Coins) I).getCurrency()
	...
	public List<Coins> getStandardCurrency(MOB mob, Item container,
			String currency) {
		Vector<Coins> V = new Vector<Coins>();
		if (mob == null)
			return V;
		if (((currency == null) || (currency.equals(getCurrency(mob))))
				&& (mob.getMoney() > 0) && (container == null)) {
	...
	public String getCurrency(Environmental E) {
		if (E instanceof MOB) {
			...
			if (s.length() == 0)
				for (Enumeration<Area> p = ((Area) E).getParents(); p
						.hasMoreElements();) {
