...
public class CoffeeTime extends StdLibrary implements TimeManager {
	...
	public Calendar string2Date(String dateTimeStr) {
		...
		if ((dateTimeStr.indexOf('.') == 19)
				|| ((dateTimeStr.indexOf('-') == 4) && (dateTimeStr
						.indexOf(':') == 13))) {
			...
			if (dateTimeStr.trim().endsWith("PM"))
				...
				AP = Calendar.AM;
			else if (HH == 12) // as 12 always means 12 noon in international
								// date/time -- 0 = 12am
				AP = Calendar.PM;
	...
	public String date2APTimeString(long time) {
		Calendar C = makeCalendar(time);
		return date2HRString(C) + ":" + date2MINString(C) + " "
				+ date2AMPMString(C);
	}
	...
	public String date2BriefString(long time) {
		...
		if ((nowC.get(Calendar.YEAR) != C.get(Calendar.YEAR))
			...
			str.append(C.get(Calendar.YEAR)).append("/")
					.append(C.get(Calendar.MONTH) + 1).append("/")
					.append(C.get(Calendar.DATE)).append(" ");
		str.append(date2HRString(C)).append(":").append(date2MINString(C))
				.append(date2AMPMString(C).toLowerCase());
		return str.toString();
	}
	...
	public String date2EllapsedTime(long time, TimeUnit minUnit,
		...
		if (time > (TimeManager.MILI_MINUTE)) {
			if (str.length() > 0)
				str.append(shortest ? " " : ", ");
			int num = (int) Math
					.round(CMath.div(time, TimeManager.MILI_MINUTE));
		...
		if (time > (TimeManager.MILI_SECOND)) {
			if (str.length() > 0)
				str.append(shortest ? " " : ", ");
			int num = (int) Math
					.round(CMath.div(time, TimeManager.MILI_SECOND));
		...
		if (str.length() > 0)
			str.append(shortest ? " " : ", ");
		return str.append(time + (shortest ? "ms" : " milliseconds"))
				.toString().trim();
	}
	...
	public String date2SecondsString(long time) {
		...
		return (StrDate.substring(0, StrDate.length() - 3) + ":"
				+ C.get(Calendar.SECOND) + " " + StrDate.substring(StrDate
				.length() - 2));
	}
	...
	private double getTickExpressionMultiPlier(String lastWord) {
		...
		if (lastWord.startsWith("MINUTE") || lastWord.equals("MINS")
				|| lastWord.equals("MIN"))
			return CMath.div(TimeManager.MILI_MINUTE, CMProps.getTickMillisD());
		else if (lastWord.startsWith("SECOND") || lastWord.equals("SECS")
				|| lastWord.equals("SEC"))
			...
			return CMath.div(CMProps.getMillisPerMudHour()
					* globalClock().getHoursInDay(), CMProps.getTickMillisD());
		else if (lastWord.startsWith("MUDWEEK"))
			return CMath.div(CMProps.getMillisPerMudHour()
					* globalClock().getHoursInDay()
					* globalClock().getDaysInWeek(), CMProps.getTickMillisD());
		else if (lastWord.startsWith("MUDMONTH"))
			return CMath.div(CMProps.getMillisPerMudHour()
					* globalClock().getHoursInDay()
					* globalClock().getDaysInMonth(), CMProps.getTickMillisD());
		else if (lastWord.startsWith("MUDYEAR"))
			return CMath
					.div(CMProps.getMillisPerMudHour()
							* globalClock().getHoursInDay()
	...
	public int parseTickExpression(String val) {
		...
		if (multiPlier == 0.0)
			return CMath.s_parseIntExpression(val);
		return (int) Math.round(CMath.mul(multiPlier,
				CMath.s_parseIntExpression(val.substring(0, x).trim())));
	}
