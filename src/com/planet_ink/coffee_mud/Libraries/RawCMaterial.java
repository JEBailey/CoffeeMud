...
public class RawCMaterial extends StdLibrary implements MaterialLibrary {
	...
	public int getRandomResourceOfMaterial(int material) {
		...
		if ((material < 0)
				|| (material >= RawMaterial.Material.values().length))
			return -1;
		...
		if (owner instanceof Room) {
			...
			for (int i = 0; i < R.numItems(); i++) {
				...
				if ((I instanceof RawMaterial)
						...
						&& (I.rawSecretIdentity().equals(item
								.rawSecretIdentity())))
					found.addElement(I);
			}
	...
	protected List<Environmental> disBundle(Item I, int number, int bundleSize,
			Container C) {
		if ((I == null) || (I.amDestroyed()) || (bundleSize < 1))
			return null;
		if ((I instanceof PackagedItems) && (I.container() == C)
				&& (!CMLib.flags().isOnFire(I))) {
			...
			return bundle;
		} else if ((I instanceof RawMaterial) && (I.container() == C)
				&& (!CMLib.flags().isOnFire(I))
				&& (!CMLib.flags().enchanted(I))) {
			...
			if (I.basePhyStats().weight() > 1) {
				...
				for (int x = 0; x < number; x += bundleSize) {
					E = makeResource(I.material(), null, true,
							I.rawSecretIdentity());
					if (E instanceof Item) {
						...
						if ((E instanceof Decayable)
							...
							((Decayable) E).setDecayTime(((Decayable) I)
									.decayTime());
				...
				if ((I.basePhyStats().weight() - number) > 1) {
					I.basePhyStats().setWeight(
							I.basePhyStats().weight() - number);
					I.setBaseValue(I.baseGoldValue() - loseValue);
					this.adjustResourceName(I);
					if (I instanceof Food) {
						((Food) I).setNourishment(((Food) I).nourishment()
								- loseNourishment);
						if (((Food) I).nourishment() <= 0)
							((Food) I).setNourishment(0);
					}
					if (I instanceof Drink) {
						((Drink) I).setLiquidHeld(((Drink) I).liquidHeld()
								- loseThirstHeld);
						if (((Drink) I).liquidHeld() <= 0)
							((Drink) I).setLiquidHeld(0);
						((Drink) I).setLiquidRemaining(((Drink) I)
								.liquidRemaining() - loseThirstRemain);
	...
	public PhysicalAgent makeResource(int myResource, String localeCode,
			boolean noAnimals, String fullName) {
		if (myResource < 0)
			return null;
	...
	public void adjustResourceName(Item I) {
		...
		if (I.basePhyStats().weight() == 1) {
			...
			if (I instanceof Drink)
				I.setName("a " + I.basePhyStats().weight() + "# pool of "
						+ name);
			else
				I.setName("a " + I.basePhyStats().weight() + "# " + name
						+ " bundle");
	...
	public int destroyResourcesValue(Room E, int howMuch, int finalMaterial,
			int otherMaterial, Item never) {
		return destroyResourcesValue(getAllItems(E), howMuch, finalMaterial,
				otherMaterial, never, null);
	}

	public int destroyResourcesValue(MOB E, int howMuch, int finalMaterial,
			int otherMaterial, Item never) {
		return destroyResourcesValue(getAllItems(E), howMuch, finalMaterial,
				otherMaterial, never, null);
	}

	public int destroyResourcesValue(List<Item> V, int howMuch,
			int finalMaterial, int otherMaterial, Item never, Container C) {
		return destroyResourcesAll(V, howMuch, finalMaterial, otherMaterial,
				never, C)[0];
	}

	protected int[] destroyResourcesAll(List<Item> V, int howMuch,
			int finalMaterial, int otherMaterial, Item never, Container C) {
		int lostValue = 0;
		int lostAmt = 0;
		if ((V == null) || (V.size() == 0))
		...
		if ((howMuch > 0) || (otherMaterial > 0))
			for (int i = V.size() - 1; i >= 0; i--) {
				...
				if ((otherMaterial > 0) && (I instanceof RawMaterial)
						&& (I.material() == otherMaterial)
						&& (I.container() == C) && (!CMLib.flags().isOnFire(I))
						&& (!CMLib.flags().enchanted(I))) {
					if (I.basePhyStats().weight() > 1) {
						I.basePhyStats().setWeight(
								I.basePhyStats().weight() - 1);
						Environmental E = makeResource(otherMaterial, null,
								true, I.rawSecretIdentity());
						if (E instanceof Item)
							lostValue += ((Item) E).value();
						if (I.baseGoldValue() >= (I.basePhyStats().weight() * ((Item) E)
								.value()))
							I.setBaseValue(I.baseGoldValue()
									- ((Item) E).value());
						adjustResourceName(I);
					} else {
						lostValue += I.value();
					...
					if ((finalMaterial < 0) || (howMuch <= 0))
						...
						&& (I.container() == C) && (!CMLib.flags().isOnFire(I))
						&& (!CMLib.flags().enchanted(I)) && (howMuch > 0)) {
					if (I.basePhyStats().weight() > howMuch) {
						I.basePhyStats().setWeight(
								I.basePhyStats().weight() - howMuch);
						lostAmt += howMuch;
						Environmental E = makeResource(finalMaterial, null,
								true, I.rawSecretIdentity());
						if (E instanceof Item)
	...
	protected Item findFirstResource(List<Item> V, int resource) {
		for (int i = 0; i < V.size(); i++) {
			Item I = V.get(i);
			if ((I instanceof RawMaterial) && (I.material() == resource)
					&& (!CMLib.flags().isOnFire(I))
