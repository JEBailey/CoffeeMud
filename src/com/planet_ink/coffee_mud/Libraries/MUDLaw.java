...
public class MUDLaw extends StdLibrary implements LegalLibrary {
	...
	public LegalBehavior getLegalBehavior(Area A) {
		if (A == null)
			return null;
		List<Behavior> V = CMLib.flags().flaggedBehaviors(A,
	...
	public LegalBehavior getLegalBehavior(Room R) {
		if (R == null)
			return null;
		List<Behavior> V = CMLib.flags().flaggedBehaviors(R,
				Behavior.FLAG_LEGALBEHAVIOR);
		if (V.size() > 0)
			return (LegalBehavior) V.get(0);
	...
	public List<LandTitle> getAllUniqueTitles(Enumeration<Room> e,
		...
		for (; e.hasMoreElements();) {
			...
			if ((T != null)
					...
					&& ((owner == null)
							|| (owner.length() == 0)
							|| (owner.equals("*") && (T.getOwnerName().length() > 0)) || (T
								.getOwnerName().equals(owner)))) {
		...
		for (int d = 0; d < Directions.NUM_DIRECTIONS(); d++) {
			if ((d != Directions.UP) && (d != Directions.DOWN)
				...
				if (isHomePeerRoom(sideRoom) && (!doneRooms.contains(sideRoom))) {
					...
					doneRooms.addAll(getHomePeersOnThisFloor(sideRoom,
							doneRooms));
				}
			}
		}
	...
	public boolean doesHavePriviledgesHere(MOB mob, Room room) {
		...
		if (title.getOwnerName().equals(mob.Name()))
			return true;
		if ((title.getOwnerName().equals(mob.getLiegeID()) && (mob
				.isMarriedToLiege())))
			return true;
		Pair<Clan, Integer> clanRole = mob.getClanRole(title.getOwnerName());
		if ((clanRole != null)
				&& (clanRole.first.getAuthority(clanRole.second.intValue(),
						Clan.Function.HOME_PRIVS) != Clan.Authority.CAN_NOT_DO))
			return true;
	...
	public boolean doesAnyoneHavePrivilegesHere(MOB mob, String overrideID,
		...
		if ((CMLib.law().doesHavePriviledgesHere(mob, R))
				|| ((overrideID.length() > 0) && (mob.Name().equals(overrideID))))
			return true;
		if (overrideID.length() > 0) {
			Pair<Clan, Integer> clanPair = mob.getClanRole(overrideID);
			if ((clanPair != null)
					&& (clanPair.first.getAuthority(clanPair.second.intValue(),
							Clan.Function.HOME_PRIVS) != Clan.Authority.CAN_NOT_DO))
				return true;
		}
		for (int i = 0; i < R.numInhabitants(); i++) {
			...
			if (overrideID.length() > 0) {
				...
				if ((clanPair != null)
						&& (clanPair.first.getAuthority(
								clanPair.second.intValue(),
								Clan.Function.HOME_PRIVS) != Clan.Authority.CAN_NOT_DO))
					return true;
			}
		}
	...
	public boolean doesOwnThisProperty(MOB mob, Room room) {
		...
		if ((clanRole != null)
				&& (clanRole.first.getAuthority(clanRole.second.intValue(),
						Clan.Function.PROPERTY_OWNER) != Clan.Authority.CAN_NOT_DO))
			return true;
		if (mob.amFollowing() != null)
