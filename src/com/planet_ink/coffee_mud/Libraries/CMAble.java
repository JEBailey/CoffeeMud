...
public class CMAble extends StdLibrary implements AbilityMapper {
	...
	protected Map<String, Map<String, AbilityMapping>> completeAbleMap = new SHashtable<String, Map<String, AbilityMapping>>();
	...
	protected Map<String, Map<String, AbilityMapping>> reverseAbilityMap = new TreeMap<String, Map<String, AbilityMapping>>();
	protected List<AbilityMapping> eachClassSet = null;
	protected Map<String, int[]> hardOverrideCache = new Hashtable<String, int[]>();

	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, boolean autoGain) {
		addCharAbilityMapping(ID, qualLevel, abilityID, 0, 100, "", autoGain,
				false, new Vector(), "");
	}

	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, boolean autoGain, String extraMasks) {
		addCharAbilityMapping(ID, qualLevel, abilityID, 0, 100, "", autoGain,
				false, new Vector(), extraMasks);
	}

	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, boolean autoGain, List<String> skillPreReqs) {
		addCharAbilityMapping(ID, qualLevel, abilityID, 0, 100, "", autoGain,
				false, skillPreReqs, "");
	}
	...
	public void addCharAbilityMapping(String ID, int qualLevel,
			...
			String extraMasks) {
		addCharAbilityMapping(ID, qualLevel, abilityID, 0, 100, "", autoGain,
				false, skillPreReqs, extraMasks);
	}

	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, int defaultProficiency, String defParm,
			boolean autoGain) {
		addCharAbilityMapping(ID, qualLevel, abilityID, defaultProficiency,
				100, defParm, autoGain, false, new Vector(), "");
	}

	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, int defaultProficiency, String defParm,
			boolean autoGain, String extraMasks) {
		addCharAbilityMapping(ID, qualLevel, abilityID, defaultProficiency,
				100, defParm, autoGain, false, new Vector(), extraMasks);
	}

	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, int defaultProficiency, boolean autoGain) {
		addCharAbilityMapping(ID, qualLevel, abilityID, defaultProficiency,
				100, "", autoGain, false, new Vector(), "");
	}
	...
	public void addCharAbilityMapping(String ID, int qualLevel,
			...
			String extraMasks) {
		addCharAbilityMapping(ID, qualLevel, abilityID, defaultProficiency,
				100, "", autoGain, false, new Vector(), extraMasks);
	}
	...
	public Enumeration<AbilityMapping> getClassAbles(String ID, boolean addAll) {
		...
		final Map<String, AbilityMapping> allAbleMap = completeAbleMap
				.get("All");
		if ((!addAll) || (allAbleMap == null))
			return new IteratorEnumeration<AbilityMapping>(ableMap.values()
					.iterator());
						...
						new Filterer<Object>() {
							public boolean passesFilter(Object obj) {
								if ((obj instanceof AbilityMapping)
										&& (ableMap
												.containsKey(((AbilityMapping) obj).abilityID)))
									return false;
								return true;
							}
						}) };
	...
	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, int defaultProficiency, String defaultParam,
			boolean autoGain, boolean secret) {
		addCharAbilityMapping(ID, qualLevel, abilityID, defaultProficiency,
				100, defaultParam, autoGain, secret, new Vector(), "");
	}

	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, int defaultProficiency, String defaultParam,
			boolean autoGain, boolean secret, String extraMasks) {
		addCharAbilityMapping(ID, qualLevel, abilityID, defaultProficiency,
				100, defaultParam, autoGain, secret, new Vector(), extraMasks);
	}

	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, int defaultProficiency, String defaultParam,
			boolean autoGain, boolean secret, List<String> preReqSkillsList,
			String extraMask) {
		addCharAbilityMapping(ID, qualLevel, abilityID, defaultProficiency,
				100, defaultParam, autoGain, secret, preReqSkillsList,
				extraMask, null);
	}

	public void addCharAbilityMapping(String ID, int qualLevel,
			String abilityID, int defaultProficiency, int maxProficiency,
			String defaultParam, boolean autoGain, boolean secret) {
		addCharAbilityMapping(ID, qualLevel, abilityID, defaultProficiency,
				maxProficiency, defaultParam, autoGain, secret, new Vector(),
				"");
	}
	...
	public void addDynaAbilityMapping(String ID, int qualLevel,
			String abilityID, int defaultProficiency, String defaultParam,
			boolean autoGain, boolean secret) {
		...
		AbilityMapping able = makeAbilityMapping(ID, qualLevel, abilityID,
				defaultProficiency, 100, defaultParam, autoGain, secret, false,
				new Vector(), "", null);
	...
	public void addCharAbilityMapping(String ID, int qualLevel,
		...
		addCharAbilityMapping(ID, qualLevel, abilityID, defaultProficiency,
				maxProficiency, defaultParam, autoGain, secret,
				preReqSkillsList, extraMask, null);
	}

	public void addPreRequisites(String ID, List<String> preReqSkillsList,
			String extraMask) {
		...
		if ((extraMask != null) && (extraMask.trim().length() > 0)) {
			List<String> preReqsOf = CMLib.masking().getAbilityEduReqs(
					extraMask);
	...
	public boolean isDomainIncludedInAnyAbility(int domain, int acode) {
		STreeSet<Integer> V = (STreeSet<Integer>) completeDomainMap.get(Integer
				.valueOf(domain));
		if (V == null) {
			Ability A = null;
	...
	public List<QualifyingID> getClassAllowsList(String classID) {
		List<AbilityMapping> ABLES = getUpToLevelListings(classID,
				CMProps.getIntVar(CMProps.Int.LASTPLAYERLEVEL), false, false);
		SHashtable alreadyDone = new SHashtable();
		List<QualifyingID> DV = new Vector<QualifyingID>(2);
		Integer Ix = null;
		for (AbilityMapping able : ABLES) {
			for (Iterator<String> i = getAbilityAllowsList(able.abilityID); i
					.hasNext();) {
				String s = i.next();
	...
	public Iterator<String> getAbilityAllowsList(String ableID) {
		...
		return (set == null) ? new SVector<String>(1).iterator() : set
				.iterator();
	}
	...
	public void addCharAbilityMapping(String ID, int qualLevel,
		...
		AbilityMapping able = makeAbilityMapping(ID, qualLevel, abilityID,
				defaultProficiency, maxProficiency, defaultParam, autoGain,
				secret, false, preReqSkillsList, extraMask, costOverrides);
		addClassAbility(abilityID, ableMap, able);
	}

	public AbilityMapping makeAbilityMapping(String ID, int qualLevel,
			String abilityID, int defaultProficiency, int maxProficiency,
			String defaultParam, boolean autoGain, boolean secret,
			boolean isAllQualified, List<String> preReqSkillsList,
			String extraMask, Integer[] costOverrides) {
	...
	public int getCalculatedMedianLowestQualifyingLevel() {
		Integer[] allLevelsArray = lowestQualifyingLevelMap.values().toArray(
				new Integer[0]);
		...
		return allLevelsArray[(int) Math.round(CMath.div(allLevelsArray.length,
				2.0))].intValue();
	}

	public void addClassAbility(String abilityID,
			Map<String, AbilityMapping> ableMap, AbilityMapping able) {
		...
		if (qualLevel > 0) {
			if ((lowLevel == null) || (qualLevel < lowLevel.intValue()))
				lowestQualifyingLevelMap.put(abilityID,
						Integer.valueOf(qualLevel));
		}

		// and max proficiency maps
		Integer maxProf = maxProficiencyMap.get(abilityID);
		if ((maxProf == null) || (maxProficiency > maxProf.intValue()))
	...
	public synchronized void handleEachAndClassAbility(
			Map<String, AbilityMapping> ableMap,
			Map<String, Map<String, AbilityMapping>> allQualMap, String ID) {
		if (eachClassSet == null) {
			...
			for (AbilityMapping mapped : eachMap.values()) {
				if (CMSecurity
						.isAbilityDisabled(mapped.abilityID.toUpperCase()))
					continue;
		...
		for (final Iterator<AbilityMapping> a = eachClassSet.iterator(); a
				.hasNext();) {
	...
	public boolean qualifiesByAnyCharClass(String abilityID) {
		...
		for (Enumeration e = CMClass.charClasses(); e.hasMoreElements();) {
			...
			if (completeAbleMap.containsKey(C.ID())) {
				Map<String, AbilityMapping> ableMap = completeAbleMap.get(C
						.ID());
				if (ableMap.containsKey(abilityID))
					return true;
			}
	...
	public boolean availableToTheme(String abilityID, int theme,
			boolean publicly) {
		for (String key : completeAbleMap.keySet()) {
	...
	public List<String> getLevelListings(String ID, boolean checkAll, int level) {
		...
		if ((checkAll) && (completeAbleMap.containsKey("All"))) {
			...
			for (String key : ableMap.keySet()) {
				AbilityMapping able = ableMap.get(key);
				if ((able.qualLevel == level) && (!V.contains(key)))
					V.add(key);
			}
		}
		return V;
	...
	public List<AbilityMapping> getUpToLevelListings(String ID, int level,
			boolean ignoreAll, boolean gainedOnly) {
	...
	protected List<String> getOrSet(String errStr, String abilityID) {
		...
		for (int p = 0; p < preorset.size(); p++) {
			...
			if (abilityID.startsWith("*")) {
				...
				String a = abilityID.substring(0, abilityID.length() - 1)
						.toUpperCase();
				for (Enumeration<Ability> e = CMClass.abilities(); e
						.hasMoreElements();) {
					preA = e.nextElement();
		...
		for (int o = orset.size() - 1; o >= 0; o--) {
			...
			if (preA == null) {
				...
				if (preA != null)
					...
					Log.errOut("CMAble", "Skill " + errStr
							+ " requires nonexistant skill " + abilityID + ".");
					orset.clear();
					break;
				}
			}
	...
	public DVector getRawPreRequisites(String ID, boolean checkAll,
			String abilityID) {
		if (completeAbleMap.containsKey(ID)) {
			Map<String, AbilityMapping> ableMap = completeAbleMap.get(ID);
			if (ableMap.containsKey(abilityID))
		...
		if ((preReqs != null) && (preReqs.size() > 0)) {
			...
			for (int p = 0; p < preReqs.size(); p++) {
				...
				if (preReqs.elementAt(p, 1) instanceof List) {
					...
					Ability A = CMClass.getAbility((String) preReqs.elementAt(
							p, 1));
					if (A != null) {
						names.append("'" + A.name() + "'");
						if (prof.intValue() > 0)
	...
	public final List<String> getCurrentlyQualifyingIDs(final MOB studentM,
			final String AID) {
		...
		int qualRacelevel = getQualifyingLevel(cStats.getMyRace().ID(), false,
				AID);
		...
		for (Pair<Clan, Integer> c : studentM.clans()) {
			int qualClanlevel = getQualifyingLevel(c.first.getGovernmentName(),
					false, AID);
			if ((qualClanlevel >= 0)
	...
	public DVector getRawPreRequisites(MOB studentM, Ability A) {
		...
		final List<String> qualifyingIDs = getCurrentlyQualifyingIDs(studentM,
				AID);
		DVector reqs = null;
		...
		reqs = getRawPreRequisites(studentM.charStats().getCurrentClass().ID(),
				true, A.ID());
	...
	public String getApplicableMask(MOB studentM, Ability A) {
		...
		mask = getExtraMask(studentM.charStats().getCurrentClass().ID(), true,
				A.ID());
	...
	public int qualifyingLevel(MOB studentM, Ability A) {
		...
		int raceLevel = getQualifyingLevel(studentM.charStats().getMyRace()
				.ID(), false, A.ID());
		int charLevel = studentM.basePhyStats().level();
		if ((raceLevel >= 0) && (charLevel >= raceLevel)
				&& ((charLevel - raceLevel) > greatestDiff)) {
			greatestDiff = charLevel - raceLevel;
			theLevel = raceLevel;
		}
		for (Pair<Clan, Integer> c : studentM.clans()) {
			int clanLevel = getQualifyingLevel(c.first.getGovernmentName(),
					false, A.ID());
			if ((clanLevel >= 0) && (c.first.getClanLevel() >= clanLevel)
		...
		if (theLevel < 0)
			return getQualifyingLevel(studentM.charStats().getCurrentClass()
					.ID(), true, A.ID());
		return theLevel;
	}
	...
	public int qualifyingClassLevel(MOB studentM, Ability A) {
		...
		if (theClass == null)
			return studentM.charStats().getClassLevel(
					studentM.charStats().getCurrentClass());
	...
	public boolean qualifiesByCurrentClassAndLevel(MOB studentM, Ability A) {
		...
		int level = getQualifyingLevel(C.ID(), true, A.ID());
		if ((level >= 0) && (studentM.charStats().getClassLevel(C) >= level))
			return true;
		level = getQualifyingLevel(studentM.charStats().getMyRace().ID(),
				false, A.ID());
		if ((level >= 0) && (studentM.phyStats().level() >= level))
			return true;
		for (Pair<Clan, Integer> c : studentM.clans()) {
			level = getQualifyingLevel(c.first.getGovernmentName(), false,
					A.ID());
	...
	public boolean qualifiesOnlyByRace(MOB studentM, Ability A) {
		int level = getQualifyingLevel(studentM.charStats().getMyRace().ID(),
				false, A.ID());
		if ((level >= 0) && (studentM.phyStats().level() >= level))
			return true;
		return false;
	...
	public boolean qualifiesOnlyByClan(MOB studentM, Ability A) {
		for (Pair<Clan, Integer> c : studentM.clans()) {
			int level = getQualifyingLevel(c.first.getGovernmentName(), false,
					A.ID());
			if ((level >= 0) && (c.first.getClanLevel() >= level))
				return true;
		}
	...
	public AbilityLimits getCommonSkillLimit(MOB studentM, Ability A) {
		...
		if (A instanceof CommonSkill) {
			...
			aL.specificSkillLimit = crafting ? aL.craftingSkills
	...
	public AbilityLimits getCommonSkillRemainders(MOB student) {
		...
		PairVector<String, Integer> culturalAbilitiesDV = student
				.baseCharStats().getMyRace().culturalAbilities();
		for (int i = 0; i < culturalAbilitiesDV.size(); i++)
			culturalAbilities
					.add(culturalAbilitiesDV.getFirst(i).toLowerCase());
		for (int a = 0; a < student.numAbilities(); a++) {
			...
			if (A2 instanceof CommonSkill) {
				...
				for (int c = 0; c < CS.numClasses(); c++)
					if (CMLib.ableMapper().getQualifyingLevel(
							CS.getMyClass(c).ID(), false, A2.ID()) >= 0) {
						foundInAClass = true;
						break;
					}
				if (foundInAClass)
	...
	public final List<AbilityMapping> getAllAbilityMappings(final MOB mob,
		...
		for (int c = 0; c < mob.charStats().numClasses(); c++) {
			...
			if (completeAbleMap.containsKey(charClass)) {
				final Map<String, AbilityMapping> ableMap = completeAbleMap
						.get(charClass);
				if (ableMap.containsKey(abilityID))
		...
		if (completeAbleMap.containsKey(mob.charStats().getMyRace().ID())) {
			final Map<String, AbilityMapping> ableMap = completeAbleMap.get(mob
					.charStats().getMyRace().ID());
			if (ableMap.containsKey(abilityID))
				list.add(ableMap.get(abilityID));
		}
	...
	public boolean getSecretSkill(MOB mob, String abilityID) {
		boolean secretFound = false;
		final List<AbilityMapping> mappings = getAllAbilityMappings(mob,
				abilityID);
		for (final Iterator<AbilityMapping> a = mappings.iterator(); a
				.hasNext();) {
	...
	public boolean getSecretSkill(String abilityID) {
		...
		for (Enumeration e = CMClass.charClasses(); e.hasMoreElements();) {
			...
			if (completeAbleMap.containsKey(charClass)
					&& (!charClass.equals("Archon"))) {
				Map<String, AbilityMapping> ableMap = completeAbleMap
						.get(charClass);
				if (ableMap.containsKey(abilityID)) {
					if (!ableMap.get(abilityID).isSecret)
						return false;
	...
	public Integer[] getCostOverrides(String ID, boolean checkAll,
			String abilityID) {
		Integer[] found = null;
		if (completeAbleMap.containsKey(ID)) {
			Map<String, AbilityMapping> ableMap = completeAbleMap.get(ID);
	...
	public int getDefaultProficiency(String ID, boolean checkAll,
			String abilityID) {
		if (completeAbleMap.containsKey(ID)) {
			Map<String, AbilityMapping> ableMap = completeAbleMap.get(ID);
			if (ableMap.containsKey(abilityID))
	...
	public boolean IsItemComponent(MOB mob, AbilityComponent comp, int[] amt,
			Item I, List<Object> thisSet) {
		if (I == null)
			return false;
		Item container = null;
		if ((comp.getType() == AbilityComponent.CompType.STRING)
			...
			return false;
				...
				&& ((!(I instanceof RawMaterial)) || (I.material() != comp
						.getLongType())))
			return false;
		else if ((comp.getType() == AbilityComponent.CompType.MATERIAL)
				&& ((!(I instanceof RawMaterial)) || ((I.material() & RawMaterial.MATERIAL_MASK) != comp
						.getLongType())))
			return false;
		...
		case INVENTORY:
			if ((container.owner() instanceof Room)
					|| (!container.amWearingAt(Wearable.IN_INVENTORY)))
				return false;
			break;
		case HELD:
			if ((container.owner() instanceof Room)
					|| (!container.amWearingAt(Wearable.WORN_HELD)))
				return false;
			break;
		case WORN:
			if ((container.owner() instanceof Room)
		...
		case ONGROUND:
			if ((!(container.owner() instanceof Room))
					|| (!CMLib.flags().canBeSeenBy(container, mob)))
				return false;
			break;
		}
		if ((comp.getType() != AbilityComponent.CompType.STRING)
	...
	// returns Vector of components found if all good, returns Integer of bad
	// row if not.
	public List<Object> componentCheck(MOB mob, List<AbilityComponent> req) {
		...
		for (int i = 0; i < req.size(); i++) {
			...
			// if they fail the zappermask, its like the req is NOT even
			// there...
			if ((comp.getCompiledMask() != null)
					&& (!CMLib.masking().maskCheck(comp.getCompiledMask(), mob,
							true)))
				continue;
			...
			if (comp.getLocation() != CompLocation.ONGROUND)
				for (int ii = 0; ii < mob.numItems(); ii++) {
					found = IsItemComponent(mob, comp, amt, mob.getItem(ii),
							thisSet);
					if (found)
						break;
				}
			if ((!found)
					&& (room != null)
					&& ((comp.getLocation() == CompLocation.ONGROUND) || (comp
							.getLocation() == CompLocation.NEARBY))) {
				for (int ii = 0; ii < room.numItems(); ii++) {
					found = IsItemComponent(mob, comp, amt, room.getItem(ii),
							thisSet);
					if (found)
						break;
				}
	...
	public DVector getAbilityComponentDecodedDVector(AbilityComponent comp) {
		...
		curr.addElement(
				"ANDOR",
				comp.getConnector() == AbilityComponent.CompConnector.AND ? "&&"
						: "||");
		...
		if (comp.getType() == AbilityComponent.CompType.STRING)
			...
			itemDesc = RawMaterial.Material
					.findByMask((int) comp.getLongType()).desc().toUpperCase();
		else if (comp.getType() == AbilityComponent.CompType.RESOURCE)
			itemDesc = RawMaterial.CODES.NAME((int) comp.getLongType())
					.toUpperCase();
	...
	public void setAbilityComponentCodedFromDecodedDVector(DVector decodedDV,
		...
		if (depth >= 0)
			comp.setType(AbilityComponent.CompType.RESOURCE,
					Integer.valueOf(depth));
		else {
			depth = CMLib.materials().getMaterialCode(s[4], false);
			if (depth >= 0)
				comp.setType(AbilityComponent.CompType.MATERIAL,
						Integer.valueOf(depth));
			else
				comp.setType(AbilityComponent.CompType.STRING, s[4]
						.toUpperCase().trim());
		}
		comp.setMask(s[5]);
	}

	public List<DVector> getAbilityComponentDecodedDVectors(
			List<AbilityComponent> req) {
		if (req == null)
			return null;
	...
	public AbilityComponent createBlankAbilityComponent() {
		AbilityComponent comp = (AbilityComponent) CMClass
	...
	protected String getAbilityComponentCodedStringFromDVectors(
			List<DVector> comps) {
	...
	public String getAbilityComponentDesc(MOB mob, List<AbilityComponent> req,
		...
		if (r > 0)
			buf.append(comp.getConnector() == AbilityComponent.CompConnector.AND ? ", and "
					: ", or ");
		if ((mob != null)
		...
		amt = comp.getAmount();
		if (comp.getType() == AbilityComponent.CompType.STRING)
			itemDesc = ((amt > 1) ? (amt + " " + comp.getStringType() + "s")
					: CMLib.english().startWithAorAn(comp.getStringType()));
		else if (comp.getType() == AbilityComponent.CompType.MATERIAL)
			itemDesc = amt
					+ ((amt > 1) ? " pounds" : " pound")
					+ " of "
					+ RawMaterial.Material.findByMask((int) comp.getLongType())
							.noun().toLowerCase();
		else if (comp.getType() == AbilityComponent.CompType.RESOURCE)
			itemDesc = amt
					+ ((amt > 1) ? " pounds" : " pound")
					+ " of "
					+ RawMaterial.CODES.NAME((int) comp.getLongType())
							.toLowerCase();
	...
	public String addAbilityComponent(String s,
			Map<String, List<AbilityComponent>> H) {
		int x = s.indexOf('=');
		if (x < 0)
		...
		while (parms.length() > 0) {
			build = (AbilityComponent) CMClass
					.getCommon("DefaultAbilityComponent");
			...
			if (parmS.substring(0, x).equalsIgnoreCase("held"))
				...
				build.setLocation(AbilityComponent.CompLocation.ONGROUND);
					...
					&& (!parmS.substring(0, x).equalsIgnoreCase("inventory"))) {
			...
			if (parmS.substring(0, x).equalsIgnoreCase("kept"))
				build.setConsumed(false);
					...
					&& (!parmS.substring(0, x).equalsIgnoreCase("consumed"))) {
			...
			if (depth >= 0)
				build.setType(AbilityComponent.CompType.RESOURCE,
						Long.valueOf(depth));
			else {
				depth = CMLib.materials().getMaterialCode(rsc, false);
				if (depth >= 0)
					build.setType(AbilityComponent.CompType.MATERIAL,
							Long.valueOf(depth));
				else
					build.setType(AbilityComponent.CompType.STRING, rsc
							.toUpperCase().trim());
			}
			parmS = parmS.substring(x + 1);
	...
	// format of each data entry is 1=ANDOR(B), 2=DISPO(I), 3=CONSUMED(B),
	// 4=AMT(I), 5=MATERIAL(L)RESOURCE(I)NAME(S), 6=MASK(S)
	public Map<String, List<AbilityComponent>> getAbilityComponentMap() {
		Map<String, List<AbilityComponent>> H = (Map) Resources
				.getResource("COMPONENT_MAP");
		if (H == null) {
			H = new Hashtable();
			StringBuffer buf = new CMFile(
					Resources.makeFileResourceName("skills/components.txt"),
	...
	public void alterAbilityComponentFile(String compID, boolean delete) {
		CMFile F = new CMFile(
				Resources.makeFileResourceName("skills/components.txt"), null,
				CMFile.FLAG_LOGERRORS);
		if (delete) {
			Resources.findRemoveProperty(F, compID);
			return;
		}
		String parms = CMLib.ableMapper()
				.getAbilityComponentCodedString(compID);
		...
		for (int t = 0; t < text.length(); t++) {
			if (text.charAt(t) == '\n')
				...
				break;
					...
					&& (Character.toUpperCase(text.charAt(t)) == upID.charAt(0))) {
				if ((text.substring(t).toUpperCase().startsWith(upID))
						&& (text.substring(t + upID.length()).trim()
								.startsWith("="))) {
					addIt = false;
					delFromHere = t;
				}
	...
	public AbilityMapping makeAllQualifyMapping(String s) {
			...
			for (int i = 0; i < s.length(); i++) {
				if ((lastC == ' ') && (Character.isLetter(us.charAt(i)))) {
					...
					if (ss.startsWith("MASK=")) {
						...
						autogain = true;
						i += 8;
					} else if (ss.startsWith("AUTOGAIN") && (ss.length() == 8)) {
						cur = null;
						autogain = true;
						break;
					} else if (cur != null)
		...
		return makeAbilityMapping(abilityID, qualLevel, abilityID,
				CMath.s_int(prof.toString().trim()), 100, "", autogain, false,
				true, CMParms.parseSpaces(preReqs.toString().trim(), true),
				mask.toString().trim(), null);
	}

	public Map<String, Map<String, AbilityMapping>> getAllQualifiesMap(
		...
		CMFile f = new CMFile(
				Resources.makeFileResourceName("skills/allqualifylist.txt"),
				null);
	...
	public String buildAllQualifysSection(Map<String, AbilityMapping> map) {
		...
		for (AbilityMapping mapped : map.values()) {
			List<AbilityMapping> subMap = sortedMap.get(Integer
					.valueOf(mapped.qualLevel));
			if (subMap == null) {
		...
		for (Integer LEVEL : sortedMap.keySet()) {
			...
			for (AbilityMapping mapped : subMap) {
				...
				if (mapped.autoGain)
					str.append("AUTOGAIN ");
				if ((mapped.extraMask != null)
						&& (mapped.extraMask.length() > 0))
					str.append("MASK=").append(mapped.extraMask).append(" ");
				if ((mapped.originalSkillPreReqList != null)
						&& (mapped.originalSkillPreReqList.trim().length() > 0))
					str.append("REQUIRES=")
							.append(CMParms.combineWith(CMParms.parseCommas(
	...
	public synchronized void saveAllQualifysFile(
			Map<String, Map<String, AbilityMapping>> newMap) {
		...
		StringBuilder str = new StringBuilder("");
		for (String line : set) {
			if (line.toUpperCase().startsWith("[ALL]")
					|| line.toUpperCase().startsWith("[EACH]")) {
				str.append("\n\r\n\r");
				break;
