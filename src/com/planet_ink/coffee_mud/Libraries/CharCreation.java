...
public class CharCreation extends StdLibrary implements CharCreationLibrary {
	...
	public void reRollStats(MOB mob, CharStats C, int pointsLeft) {
		...
		while (pointsLeft > 0) {
			int whichStat = CharStats.CODES.BASE()[CMLib.dice().roll(1,
					CharStats.CODES.BASE().length, -1)];
	...
	public boolean canChangeToThisClass(MOB mob, CharClass thisClass, int theme) {
		if ((CMProps.isTheme(thisClass.availabilityCode()))
				&& ((theme < 0) || (CMath.bset(thisClass.availabilityCode(),
						theme)))
				&& (!CMath.bset(thisClass.availabilityCode(),
						Area.THEME_SKILLONLYMASK))
				&& ((mob == null) || (thisClass
						.qualifiesForThisClass(mob, true))))
			return true;
		return false;
	}
	...
	public List<Race> raceQualifies(MOB mob, int theme) {
		...
		for (Enumeration<Race> r = CMClass.races(); r.hasMoreElements();) {
			...
			if ((CMProps.isTheme(R.availabilityCode()))
					&& (!CMath.bset(R.availabilityCode(),
							Area.THEME_SKILLONLYMASK))
					&& ((!CMSecurity.isDisabled(CMSecurity.DisFlag.STDRACES)) || (R
							.isGeneric()))
	...
	public boolean isBadName(String login) {
		...
		for (int i = 0; i < DEFAULT_BADNAMES.length; i++)
			if (CMLib.english().containsString(login, DEFAULT_BADNAMES[i]))
				return true;
		List<String> V2 = CMParms.parseCommas(
				CMProps.getVar(CMProps.Str.BADNAMES), true);
	...
	public boolean isOkName(String login, boolean spacesOk) {
		...
		for (int c = 0; c < login.length(); c++)
			if ((login.charAt(c) != ' ')
					&& (!Character.isLetter(login.charAt(c))))
				return false;
	...
	public void showTheNews(MOB mob) {
		...
		if ((mob.session() == null) || (mob.isMonster())
	...
	public boolean isExpired(PlayerAccount acct, Session session,
			long expiration) {
		...
		if ((acct != null) && (acct.isSet(PlayerAccount.FLAG_NOEXPIRE)))
			return false;
		if ((session.mob() != null)
				&& ((CMSecurity.isASysOp(session.mob())) || (CMSecurity
						.isAllowedEverywhere(session.mob(),
								CMSecurity.SecFlag.NOEXPIRE))))
			return false;
	...
	private void executeScript(MOB mob, List<String> scripts) {
		...
		for (int s = 0; s < scripts.size(); s++) {
			...
			CMMsg msg2 = CMClass.getMsg(mob, mob, null, CMMsg.MSG_OK_VISUAL,
					null, null, "CHARCREATION");
	...
	private Map<String, List<String>> getLoginScripts() {
		...
		final String[] VALID_SCRIPT_CODES = { "PASSWORD", "EMAIL", "ANSI",
				"THEME", "RACE", "GENDER", "STATS", "CLASS", "FACTIONS", "END" };
		List<String> extras = CMParms.parseCommas(
				CMProps.getVar(CMProps.Str.CHARCREATIONSCRIPTS), true);
			...
			if (x > 0) {
				...
				for (int v = 0; v < VALID_SCRIPT_CODES.length; v++)
					if (VALID_SCRIPT_CODES[v].equals(code)) {
						code = VALID_SCRIPT_CODES[v];
						found = true;
						break;
					} else if (VALID_SCRIPT_CODES[v].startsWith(code)) {
						code = VALID_SCRIPT_CODES[v];
						found = true;
						break;
					}
				if (!found) {
					Log.errOut("CharCreation",
							"Error in CHARCREATIONSCRIPTS, invalid code: "
									+ code);
					continue;
				}
				s = s.substring(x + 1);
	...
	protected void finishCreateAccount(final LoginSession loginObj,
			final PlayerAccount acct, final String login, final String pw,
			final String emailAddy, final Session session) {
		...
		if (((pw == null) || (pw.length() == 0))
				&& (!CMProps.getVar(CMProps.Str.EMAILREQ).startsWith("DISABLE"))) {
			...
			CMLib.database().DBCreateAccount(acct);
			CMLib.players().addAccount(acct);
			StringBuffer doneText = new CMFile(
					Resources.buildResourcePath("text") + "doneacct.txt", null,
	...
	protected String buildQualifyingClassList(MOB mob, List<CharClass> classes,
			String finalConnector) {
		StringBuilder list = new StringBuilder("");
		int highestAttribute = -1;
		for (int attrib : CharStats.CODES.BASE())
			if ((highestAttribute < 0)
					|| (mob.baseCharStats().getStat(attrib) > mob
							.baseCharStats().getStat(highestAttribute)))
				highestAttribute = attrib;
		for (Iterator<CharClass> i = classes.iterator(); i.hasNext();) {
			CharClass C = i.next();
			String color = (C.getAttackAttribute() == highestAttribute) ? "^H"
					: "^w";
			if (!i.hasNext()) {
				if (list.length() > 0) {
	...
	protected void getUniversalStartingItems(int theme, MOB mob) {
		List<String> newItemPartsV = CMParms.parseCommas(
				CMProps.getVar(CMProps.Str.STARTINGITEMS), true);
		for (String item : newItemPartsV) {
			...
			for (int i = 0; i < num; i++) {
				...
				if (I == null) {
					Log.errOut("CharCreation",
							"Unable to give new STARTINGITEM '" + item + "'");
				} else {
					mob.addItem(I);
				}
			}
	...
	private boolean loginsDisabled(MOB mob) {
		if ((CMSecurity.isDisabled(CMSecurity.DisFlag.LOGINS))
				...
				&& (!((mob.playerStats() != null)
						&& (mob.playerStats().getAccount() != null) && (CMProps
							.isOnWhiteList(CMProps.SYSTEMWL_LOGINS, mob
									.playerStats().getAccount().accountName()))))
				&& (!((mob.session() != null) && (CMProps.isOnWhiteList(
						CMProps.SYSTEMWL_LOGINS, mob.session().getAddress()))))) {
			StringBuffer rejectText = Resources.getFileResource(
					"text/nologins.txt", true);
			try {
				rejectText = CMLib.webMacroFilter().virtualPageFilter(
						rejectText);
			} catch (Exception ex) {
			}
	...
	public LoginResult prelimChecks(Session session, String login,
		...
		for (Session S : CMLib.sessions().allIterable()) {
			MOB M = S.mob();
			if ((M != null) && (S != session) && (M == player.loadedMOB)) {
	...
	public void notifyFriends(MOB mob, String message) {
		try {
			for (Session S : CMLib.sessions().localOnlineIterable()) {
				...
				if ((listenerM != null)
						&& (listenerM != mob)
						&& ((!CMLib.flags().isCloaked(mob)) || (CMSecurity
								.isASysOp(listenerM)))
						&& (CMath.bset(listenerM.getBitmap(),
								MOB.ATT_AUTONOTIFY))) {
					PlayerStats listenerPStats = listenerM.playerStats();
					if ((listenerPStats != null)
							&& ((listenerPStats.getFriends().contains(
									mob.Name()) || listenerPStats.getFriends()
									.contains("All"))))
	...
	private String getMSSPPacket() {
		...
		rpt.append("PLAYERS");
		rpt.append("\t");
		rpt.append(Integer.toString(CMLib.sessions().getCountLocalOnline()));
		rpt.append("\r\n");
		rpt.append("STATUS");
		rpt.append("\t");
		...
		if (host != null) {
			...
			if (Thread.currentThread() instanceof MWThread) {
				String webServerPort = Integer.toString(((MWThread) Thread
						.currentThread()).getConfig().getHttpListenPorts()[0]);
				rpt.append("\r\n");
				rpt.append("WEBSITE");
				rpt.append("\t");
				rpt.append(("http://" + host.getHost() + ":" + webServerPort));
			}
			rpt.append("\r\n");
			rpt.append("LANGUAGE");
			rpt.append("\t");
		...
		rpt.append("EMAIL");
		rpt.append("\t");
		rpt.append(CMProps.getVar(CMProps.Str.ADMINEMAIL));
		rpt.append("\r\n");
		rpt.append("CODEBASE");
		rpt.append("\t");
		rpt.append(("CoffeeMud v" + CMProps.getVar(CMProps.Str.MUDVER)));
		rpt.append("\r\n");
		rpt.append("AREAS");
		rpt.append("\t");
		...
		rpt.append("MOBILES");
		rpt.append("\t");
		rpt.append(Long.toString(CMClass
				.numPrototypes(CMClass.CMObjectType.MOB)));
		rpt.append("\r\n");
		rpt.append("OBJECTS");
		rpt.append("\t");
		rpt.append(Long.toString(CMClass
				.numPrototypes(CMClass.OBJECTS_ITEMTYPES)));
		rpt.append("\r\n");
		rpt.append("ROOMS");
		rpt.append("\t");
		...
		if (!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES))
			numClasses = CMLib
					.login()
					.classQualifies(null,
							CMProps.getIntVar(CMProps.Int.MUDTHEME) & 0x07)
					.size();
		rpt.append("\t");
		...
		rpt.append("MCCP");
		rpt.append("\t");
		rpt.append((!CMSecurity.isDisabled(CMSecurity.DisFlag.MCCP) ? "1" : "0"));
		rpt.append("\r\n");
		rpt.append("MSP");
		rpt.append("\t");
		rpt.append((!CMSecurity.isDisabled(CMSecurity.DisFlag.MSP) ? "1" : "0"));
		rpt.append("\r\n");
		rpt.append("MXP");
		rpt.append("\t");
		rpt.append((!CMSecurity.isDisabled(CMSecurity.DisFlag.MXP) ? "1" : "0"));
		rpt.append("\r\nMSSP-REPLY-END\r\n");
		return rpt.toString();
	}

	public LoginResult loginSystem(Session session, LoginSession loginObj)
			throws IOException {
		if (session == null)
			return LoginResult.NO_LOGIN;
		if (loginObj == null)
	...
	protected LoginResult loginSubsystem(final LoginSession loginObj,
	...
	protected LoginResult loginStart(final LoginSession loginObj,
	...
	protected LoginResult loginName(final LoginSession loginObj,
			final Session session) {
		...
		if (loginObj.login.length() == 0) {
			...
			session.println("^w* Enter a new name to create "
					+ ((CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1) ? "an account"
							: " a character") + ".^N");
			loginObj.state = LoginState.LOGIN_START;
		...
		if (loginObj.login.equalsIgnoreCase("MSSP-REQUEST")
		...
		if (loginObj.login.endsWith(" !")) {
			loginObj.login = loginObj.login.substring(0,
					loginObj.login.length() - 2);
			loginObj.login = loginObj.login.trim();
			loginObj.wizi = true;
		}
		...
		if (CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1) {
			...
			if (loginObj.acct != null) {
				...
				loginObj.player.expiration = loginObj.acct
				...
				if (loginObj.player != null) {
					...
					session.println("\n\rAccount '"
							+ CMStrings.capitalizeAndLower(loginObj.login)
							+ "' does not exist.");
			...
			if ((mob != null) && (mob.playerStats() != null)) {
				...
				loginObj.player.expiration = mob.playerStats()
						.getAccountExpiration();
	...
	protected LoginResult loginAcctcharPword(final LoginSession loginObj,
			final Session session) {
		...
		if (loginObj.player.matchesPassword(loginObj.password)) {
			if ((loginObj.player.accountName == null)
				...
				session.promptPrint("Would you like to create your new master account and call it '"
						+ loginObj.player.name + "' (y/N)? ");
				loginObj.state = LoginState.LOGIN_ACCTCONV_CONFIRM;
				return LoginResult.INPUT_REQUIRED;
			} else {
				session.println("\n\rThis mud uses an account system.  Your account name is `^H"
						+ loginObj.player.accountName
						+ "^N`.\n\r"
	...
	protected LoginResult loginAcctconvConfirm(final LoginSession loginObj,
			final Session session) {
		final String input = loginObj.lastInput.toUpperCase().trim();
		if (input.startsWith("Y")) {
			loginObj.acct = (PlayerAccount) CMClass
	...
	protected LoginResult acctcreateStart(final LoginSession loginObj,
			final Session session) {
	...
	protected LoginResult acctcreateANSIConfirm(final LoginSession loginObj,
			final Session session) {
		...
		StringBuffer introText = new CMFile(Resources.buildResourcePath("text")
				+ "newacct.txt", null, CMFile.FLAG_LOGERRORS).text();
		try {
			introText = CMLib.webMacroFilter().virtualPageFilter(introText);
		} catch (Exception ex) {
		}
		session.println(null, null, null, "\n\r\n\r" + introText.toString());
		final boolean emailPassword = ((CMProps.getVar(CMProps.Str.EMAILREQ)
	...
	protected LoginResult acctcreateEmailStart(final LoginSession loginObj,
			final Session session) {
		if (CMProps.getVar(CMProps.Str.EMAILREQ).toUpperCase()
				.startsWith("DISABLE")) {
			finishCreateAccount(loginObj, loginObj.acct, loginObj.login,
					loginObj.password, "", session);
			return null;
		}
		StringBuffer emailIntro = new CMFile(
				Resources.buildResourcePath("text") + "email.txt", null,
				CMFile.FLAG_LOGERRORS).text();
		try {
	...
	protected LoginResult acctcreateEmailPrompt(final LoginSession loginObj,
			final Session session) {
	...
	protected LoginResult acctcreateEmailEntered(final LoginSession loginObj,
			final Session session) {
		boolean emailPassword = ((CMProps.getVar(CMProps.Str.EMAILREQ)
				.toUpperCase().startsWith("PASS")) && (CMProps.getVar(
				CMProps.Str.MAILBOX).length() > 0));
		boolean emailReq = (!CMProps.getVar(CMProps.Str.EMAILREQ).toUpperCase()
				.startsWith("OPTION"));
		String newEmail = loginObj.lastInput;
		if ((emailReq || emailPassword)
				&& ((newEmail == null) || (newEmail.trim().length() == 0) || (!CMLib
						.smtp().isValidEmailAddress(newEmail)))) {
		...
		if (emailPassword)
			session.println("This email address will be used to send you a password.");
		if (emailReq || emailPassword) {
			session.promptPrint("Confirm that '" + newEmail
	...
	protected LoginResult acctcreateEmailConfirmed(final LoginSession loginObj,
			final Session session) {
		boolean emailReq = (!CMProps.getVar(CMProps.Str.EMAILREQ).toUpperCase()
				.startsWith("OPTION"));
		...
		if ((newEmail.length() > 0)
				&& (newEmail.equalsIgnoreCase(loginObj.lastInput)))
		...
		if (emailConfirmed || ((!emailReq) && (newEmail.trim().length() == 0))) {
			finishCreateAccount(loginObj, loginObj.acct, loginObj.login,
					loginObj.password, newEmail, session);
			return null;
		}
	...
	protected LoginResult acctcreatePassworded(final LoginSession loginObj,
			final Session session) {
	...
	protected LoginResult loginPassStart(final LoginSession loginObj,
			final Session session) {
		if (loginObj.player == null) {
			if (CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1) {
			...
			} else if (newCharactersAllowed(loginObj.login, session,
					loginObj.acct, false)) {
				session.promptPrint("\n\r'"
						+ CMStrings.capitalizeAndLower(loginObj.login)
			...
			for (Session otherSess : CMLib.sessions().allIterable())
				if ((otherSess != session)
						&& (otherSess.isPendingLogin(loginObj))) {
	...
	protected LoginResult loginNewcharConfirm(final LoginSession loginObj,
			final Session session) {
	...
	protected LoginResult loginNewaccountConfirm(final LoginSession loginObj,
			final Session session) {
	...
	protected LoginResult loginPassReceived(final LoginSession loginObj,
			final Session session) throws IOException {
		loginObj.password = loginObj.lastInput;
		if (loginObj.player.matchesPassword(loginObj.password)) {
			LoginResult prelimResults = prelimChecks(session, loginObj.login,
					loginObj.player);
			...
			if (loginObj.acct != null) {
				if (isExpired(loginObj.acct, session,
						loginObj.player.expiration))
					return LoginResult.NO_LOGIN;
				session.setAccount(loginObj.acct);
				StringBuilder loginMsg = new StringBuilder("");
				loginMsg.append(session.getAddress())
						.append(" " + session.getTerminalType())
						.append(session.getClientTelnetMode(Session.TELNET_MXP) ? " MXP"
								: "")
						.append(session
								.getClientTelnetMode(Session.TELNET_MSDP) ? " MSDP"
								: "")
						.append(session
								.getClientTelnetMode(Session.TELNET_ATCP) ? " ATCP"
								: "")
						.append(session
								.getClientTelnetMode(Session.TELNET_GMCP) ? " GMCP"
								: "")
						.append((session
								.getClientTelnetMode(Session.TELNET_COMPRESS) || session
								.getClientTelnetMode(Session.TELNET_COMPRESS2)) ? " CMP"
								: "")
						.append(session
								.getClientTelnetMode(Session.TELNET_ANSI) ? " ANSI"
								: "")
						.append(", account login: "
								+ loginObj.acct.accountName());
				Log.sysOut(loginMsg.toString());
				// session.setStatus(SessionStatus.ACCOUNTMENU);
				loginObj.state = LoginState.ACCTMENU_START;
				return null;
			} else {
				LoginResult completeResult = completeCharacterLogin(session,
						loginObj.login, loginObj.wizi);
			...
			session.println("\n\rInvalid password.\n\r");
			if ((!session.isStopped()) && (loginObj.player.email.length() > 0)
	...
	protected LoginResult loginEmailPassword(final LoginSession loginObj,
			final Session session) throws IOException {
		final String input = loginObj.lastInput.toUpperCase().trim();
		if (input.startsWith("Y")) {
			...
			if (CMProps.getBoolVar(CMProps.Bool.HASHPASSWORDS)) {
				if (loginObj.acct != null) {
					...
					MOB playerM = CMLib.players().getLoadPlayer(
							loginObj.player.name);
					if ((playerM != null) && (playerM.playerStats() != null)) {
						password = CMLib.encoder().generateRandomPassword();
						playerM.playerStats().setPassword(password);
						loginObj.player.password = playerM.playerStats()
								.getPasswordStr();
						CMLib.database().DBUpdatePassword(loginObj.player.name,
								loginObj.player.password);
					}
				}
			}
			CMLib.smtp().emailOrJournal(
					CMProps.getVar(CMProps.Str.SMTPSERVERNAME),
					loginObj.player.name,
					"noreply@"
							+ CMProps.getVar(CMProps.Str.MUDDOMAIN)
					...
					"Your password for " + loginObj.player.name + " at "
							+ CMProps.getVar(CMProps.Str.MUDDOMAIN) + " is: '"
							+ password + "'.");
			session.stopSession(false, false, false);
			loginObj.reset = true;
			loginObj.state = LoginState.LOGIN_START;
	...
	protected LoginResult acctmenuStart(final LoginSession loginObj,
			final Session session) {
		final PlayerAccount acct = loginObj.acct;
		session.setServerTelnetMode(Session.TELNET_ANSI,
				acct.isSet(PlayerAccount.FLAG_ANSI));
		session.setClientTelnetMode(Session.TELNET_ANSI,
				acct.isSet(PlayerAccount.FLAG_ANSI));
		// if its not a new account, do this?
		StringBuffer introText = new CMFile(Resources.buildResourcePath("text")
				+ "selchar.txt", null, CMFile.FLAG_LOGERRORS).text();
		try {
			introText = CMLib.webMacroFilter().virtualPageFilter(introText);
		} catch (Exception ex) {
		}
	...
	protected LoginResult acctmenuShowChars(final LoginSession loginObj,
			final Session session) {
		final PlayerAccount acct = loginObj.acct;
		...
		buf.append("^.^N\n\r");
		for (Enumeration<PlayerLibrary.ThinPlayer> p = acct.getThinPlayers(); p
				.hasMoreElements();) {
	...
	protected LoginResult acctmenuShowMenu(final LoginSession loginObj,
			final Session session) {
		final PlayerAccount acct = loginObj.acct;
		StringBuffer buf = new StringBuffer("");
		if (!acct.isSet(PlayerAccount.FLAG_ACCOUNTMENUSOFF)) {
	...
	protected LoginResult acctmenuPrompt(final LoginSession loginObj,
			final Session session) {
	...
	protected LoginResult acctmenuConfirmCommand(final LoginSession loginObj,
			final Session session) {
		final String input = loginObj.lastInput.trim().toUpperCase();
		if (input.startsWith("Y")) {
			loginObj.lastInput = String.valueOf(loginObj.savedInput)
					+ " <CONFIRMED>";
			loginObj.state = LoginState.ACCTMENU_COMMAND;
		} else if ((input.length() > 0) && (!input.startsWith("N"))) {
			session.promptPrint(RECONFIRMSTR);
	...
	protected LoginResult acctmenuAddToCommand(final LoginSession loginObj,
			final Session session) {
	...
	protected LoginResult acctmenuCommand(final LoginSession loginObj,
			final Session session) throws IOException {
		final PlayerAccount acct = loginObj.acct;
		...
		if (cmd.equalsIgnoreCase("?") || (("HELP").startsWith(cmd))) {
			StringBuffer accountHelp = new CMFile(
					Resources.buildResourcePath("help") + "accts.txt", null,
					CMFile.FLAG_LOGERRORS).text();
			try {
				accountHelp = CMLib.webMacroFilter().virtualPageFilter(
						accountHelp);
			} catch (Exception ex) {
			}
			session.println(null, null, null,
		...
		if (("QUIT").startsWith(cmd)) {
			if ((parms.length > 1)
					&& (parms[parms.length - 1].equalsIgnoreCase("<CONFIRMED>"))) {
		...
		if (("NEW ").startsWith(cmd)) {
			...
			if (newCharactersAllowed(parms[1], session, acct,
					parms[1].equalsIgnoreCase(acct.accountName()))) {
				final String login = CMStrings.capitalizeAndLower(parms[1]);
				if ((parms.length > 2)
						&& (parms[parms.length - 1]
								.equalsIgnoreCase("<CONFIRMED>"))) {
					loginObj.login = login;
					loginObj.state = LoginState.CHARCR_START;
					return null;
				}
				session.promptPrint("Create a new character called '" + login
						+ "' (y/N)?");
				loginObj.state = LoginState.ACCTMENU_CONFIRMCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
		...
		if (("MENU").startsWith(cmd)) {
			if ((parms.length > 1)
				...
				} else if (!acct.isSet(PlayerAccount.FLAG_ACCOUNTMENUSOFF)) {
				...
				final String promptStr = acct
						.isSet(PlayerAccount.FLAG_ACCOUNTMENUSOFF) ? "Turn menus back on (y/N)?"
						: "Turn menus off (y/N)?";
		...
		if ("EMAIL".startsWith(cmd)
				&& (!CMProps.getVar(CMProps.Str.EMAILREQ).toUpperCase()
						.startsWith("DISABLE"))) {
			if (parms.length < 2) {
				if (CMProps.getVar(CMProps.Str.EMAILREQ).equalsIgnoreCase(
						"PASSWORD"))
			...
			if ((parms[1].length() == 0) || (parms[1].indexOf('@') < 0)
					|| (parms[1].length() < 6)) {
				session.println("Aborted.");
				loginObj.state = LoginState.ACCTMENU_SHOWMENU;
				return null;
		...
		if (("RETIRE").startsWith(cmd) || ("DELETE ").startsWith(cmd)) {
			...
			for (Enumeration<PlayerLibrary.ThinPlayer> p = acct
					.getThinPlayers(); p.hasMoreElements();) {
				PlayerLibrary.ThinPlayer player = p.nextElement();
			...
			if (delMeChk == null) {
				...
				session.println("The character '"
						+ CMStrings.capitalizeAndLower(parms[1])
						+ "' is unknown.");
				loginObj.state = LoginState.ACCTMENU_SHOWMENU;
				return null;
			}
			...
			if ((parms.length > 2)
				...
				session.promptPrint("Are you sure you want to retire and delete '"
						+ delMe.name + "' (y/N)?");
				loginObj.state = LoginState.ACCTMENU_CONFIRMCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
		...
		if (("EXPORT ").startsWith(cmd)
				&& (acct.isSet(PlayerAccount.FLAG_CANEXPORT))) {
			if (parms.length < 2) {
				session.promptPrint("\n\rPlease the name of the character: ");
				loginObj.state = LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			...
			if ((parms.length > 3)
				...
				if (M != null) {
					...
					session.println(delMe.name
							+ " has been exported from your account.");
				}
			} else {
				session.promptPrint("Are you sure you want to remove character  '"
						+ delMe.name + "' from your account (y/N)?");
				loginObj.state = LoginState.ACCTMENU_CONFIRMCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			loginObj.state = LoginState.ACCTMENU_SHOWMENU;
		...
		if (("IMPORT ").startsWith(cmd)) {
			...
			if ((CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) <= acct
					.numPlayers())
					&& (!acct.isSet(PlayerAccount.FLAG_NUMCHARSOVERRIDE))) {
				session.println("You may only have "
						+ CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM)
						+ " characters.  Please delete one to create another.");
				loginObj.state = LoginState.ACCTMENU_SHOWMENU;
				return null;
			}
			String name = CMStrings.capitalizeAndLower(parms[1]);
			final PlayerLibrary.ThinnerPlayer newCharT = CMLib.database()
					.DBUserSearch(name);
			if (parms.length < 3) {
				session.promptPrint("\n\rEnter the existing password for your character '"
						+ name + "': ");
				loginObj.state = LoginState.ACCTMENU_ADDTOCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			...
			if ((newCharT == null)
					...
					|| ((newCharT.accountName != null)
							&& (newCharT.accountName.length() > 0) && (!newCharT.accountName
								.equalsIgnoreCase(acct.accountName())))) {
			...
			if ((parms.length > 3)
				...
				if (M != null) {
					...
					CMLib.database().DBUpdateAccount(acct);
					CMLib.database().DBUpdatePlayer(M);
					session.println(M.name()
							+ " has been imported into your account.");
				}
				loginObj.state = LoginState.ACCTMENU_SHOWMENU;
			} else {
				session.promptPrint("Are you sure you want to import character  '"
						+ newCharT.name + "' into your account (y/N)?");
				loginObj.state = LoginState.ACCTMENU_CONFIRMCOMMAND;
				return LoginResult.INPUT_REQUIRED;
			}
			return null;
		}
		boolean wizi = (parms.length > 1)
				&& (parms[parms.length - 1]).equalsIgnoreCase("!");
		...
		if (playMe == null) {
			session.println("'" + name
					+ "' is an unknown character or command.  Use ? for help.");
			loginObj.state = LoginState.ACCTMENU_SHOWMENU;
			return null;
		}
		MOB realMOB = CMLib.players().getLoadPlayer(playMe.name);
		if (realMOB == null) {
			session.println("Error loading character '" + name
		...
		for (Session S : CMLib.sessions().allIterable())
			if ((S.mob() != null) && (S.mob().playerStats() != null)
					&& (S.mob().playerStats().getAccount() == acct))
				numAccountOnline++;
		if ((CMProps.getIntVar(CMProps.Int.MAXCONNSPERACCOUNT) > 0)
				&& (numAccountOnline >= CMProps
						.getIntVar(CMProps.Int.MAXCONNSPERACCOUNT))
				&& (!CMSecurity
						.isDisabled(CMSecurity.DisFlag.MAXCONNSPERACCOUNT))
				&& (!CMProps.isOnWhiteList(CMProps.SYSTEMWL_CONNS,
						session.getAddress()))
				&& (!CMProps.isOnWhiteList(CMProps.SYSTEMWL_LOGINS,
						playMe.accountName))
				&& (!CMProps
						.isOnWhiteList(CMProps.SYSTEMWL_LOGINS, playMe.name))
				&& (!acct.isSet(PlayerAccount.FLAG_MAXCONNSOVERRIDE))) {
			session.println("You may only have "
					+ CMProps.getIntVar(CMProps.Int.MAXCONNSPERACCOUNT)
					+ " of your characters on at one time.");
			loginObj.state = LoginState.ACCTMENU_SHOWMENU;
			return null;
		}
	...
	protected LoginResult charcrStart(final LoginSession loginObj,
			final Session session) {
		...
		StringBuffer introText = new CMFile(Resources.buildResourcePath("text")
				+ "newchar.txt", null, CMFile.FLAG_LOGERRORS).text();
		try {
			introText = CMLib.webMacroFilter().virtualPageFilter(introText);
		} catch (Exception ex) {
		}
		session.println(null, null, null, "\n\r\n\r" + introText.toString());
		String password = (loginObj.acct != null) ? loginObj.acct
				.getPasswordStr() : "";
	...
	protected LoginResult charcrPasswordDone(final LoginSession loginObj,
			final Session session) {
		String password = (loginObj.acct != null) ? loginObj.acct
				.getPasswordStr() : "";
		boolean emailPassword = ((CMProps.getVar(CMProps.Str.EMAILREQ)
		...
		} else if (!CMProps.getVar(CMProps.Str.EMAILREQ).toUpperCase()
				.startsWith("DISABLE")) {
	...
	protected LoginResult charcrEmailStart(final LoginSession loginObj,
			final Session session) {
		StringBuffer emailIntro = new CMFile(
				Resources.buildResourcePath("text") + "email.txt", null,
				CMFile.FLAG_LOGERRORS).text();
	...
	protected LoginResult charcrEmailPrompt(final LoginSession loginObj,
			final Session session) {
		session.promptPrint("\n\rEnter your e-mail address: ");
	...
	protected LoginResult charcrEmailEntered(final LoginSession loginObj,
			final Session session) {
		boolean emailPassword = ((CMProps.getVar(CMProps.Str.EMAILREQ)
				.toUpperCase().startsWith("PASS")) && (CMProps.getVar(
				CMProps.Str.MAILBOX).length() > 0));
	...
	protected LoginResult charcrEmailConfirmed(final LoginSession loginObj,
			final Session session) {
		...
		if (emailConfirmed || ((!emailReq) && (newEmail.trim().length() == 0))) {
			...
			loginObj.mob.setBitmap(CMath.setb(mob.getBitmap(),
					MOB.ATT_AUTOFORWARD));
			loginObj.state = LoginState.CHARCR_EMAILDONE;
			return null;
		}
	...
	protected LoginResult charcrEmailDone(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		session.setMob(loginObj.mob);
		final PlayerAccount acct = loginObj.acct;
		if ((mob.playerStats().getEmail() != null)
				&& CMSecurity.isBanned(mob.playerStats().getEmail())) {
			session.println("\n\rYou are unwelcome.  No one likes you here. Go away.\n\r\n\r");
			if (mob == session.mob())
				session.stopSession(false, false, false);
			return LoginResult.NO_LOGIN;
	...
	protected LoginResult charcrANSIConfirmed(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		session.setMob(loginObj.mob);
		final String input = loginObj.lastInput.trim().toUpperCase();
	...
	protected LoginResult charcrANSIDone(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		session.setMob(loginObj.mob);
	...
	protected LoginResult charcrThemeStart(final LoginSession loginObj,
			final Session session) {
		session.setMob(loginObj.mob);
		int themeCode = CMProps.getIntVar(CMProps.Int.MUDTHEME);
		loginObj.theme = -1;
	...
	public void moveSessionToCorrectThreadGroup(final Session session, int theme) {
		...
		if ((themeDex >= 0) && (themeDex < Area.THEME_BIT_NAMES.length)) {
			ThreadGroup privateGroup = CMProps
					.getPrivateOwner(Area.THEME_BIT_NAMES[themeDex] + "PLAYERS");
			if ((privateGroup != null) && (privateGroup.getName().length() > 0)
					&& (!privateGroup.getName().equals(session.getGroupName()))) {
				if (session.getGroupName().length() > 0) {
					if (CMLib.library(session.getGroupName().charAt(0),
							CMLib.Library.SESSIONS) != CMLib.library(
							privateGroup.getName().charAt(0),
							CMLib.Library.SESSIONS)) {
						((Sessions) CMLib.library(session.getGroupName()
								.charAt(0), CMLib.Library.SESSIONS))
								.remove(session);
						((Sessions) CMLib.library(privateGroup.getName()
								.charAt(0), CMLib.Library.SESSIONS))
								.add(session);
					}
				}
	...
	protected LoginResult charcrThemeDone(final LoginSession loginObj,
			final Session session) {
	...
	protected LoginResult charcrRaceStart(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		session.setMob(loginObj.mob);
		if (CMSecurity.isDisabled(CMSecurity.DisFlag.RACES)) {
			...
			if (newRace != null) {
				...
				Log.errOut("CharCreation",
						"Races are disabled, but neither PlayerRace nor StdRace exists?!");
			}
		}
		if (!CMSecurity.isDisabled(CMSecurity.DisFlag.RACES)) {
			StringBuffer introText = new CMFile(
					Resources.buildResourcePath("text") + "races.txt", null,
					CMFile.FLAG_LOGERRORS).text();
			try {
				introText = CMLib.webMacroFilter().virtualPageFilter(introText);
			} catch (Exception ex) {
	...
	protected LoginResult charcrRaceReEntered(final LoginSession loginObj,
			final Session session) {
		...
		if (raceStr.trim().equalsIgnoreCase("?") || (raceStr.length() == 0))
			session.println(null, null, null,
					"\n\r"
							+ new CMFile(Resources.buildResourcePath("text")
									+ "races.txt", null, CMFile.FLAG_LOGERRORS)
									.text().toString());
		else {
			Race newRace = CMClass.getRace(raceStr);
			if ((newRace != null)
					&& ((!CMProps.isTheme(newRace.availabilityCode()))
							|| (!CMath.bset(newRace.availabilityCode(),
									loginObj.theme)) || (CMath.bset(
							newRace.availabilityCode(),
							Area.THEME_SKILLONLYMASK))))
				newRace = null;
			if (newRace == null)
				for (Enumeration<Race> r = CMClass.races(); r.hasMoreElements();) {
					...
					if ((R.name().equalsIgnoreCase(raceStr))
							...
							&& (CMath
									.bset(R.availabilityCode(), loginObj.theme))
							&& (!CMath.bset(R.availabilityCode(),
									Area.THEME_SKILLONLYMASK))) {
						newRace = R;
						break;
			...
			if (newRace == null)
				for (Enumeration<Race> r = CMClass.races(); r.hasMoreElements();) {
					Race R = r.nextElement();
					if ((R.name().toUpperCase().startsWith(raceStr
							.toUpperCase()))
							&& (CMProps.isTheme(R.availabilityCode()))
							&& (CMath
									.bset(R.availabilityCode(), loginObj.theme))
			...
			if (newRace != null) {
				StringBuilder str = CMLib.help().getHelpText(
						newRace.ID().toUpperCase(), mob, false);
				if (str != null)
					session.println("\n\r^N" + str.toString() + "\n\r");
				session.promptPrint("^!Is ^H" + newRace.name()
						+ "^N^! correct (Y/n)?^N");
				mob.baseCharStats().setMyRace(newRace);
	...
	protected LoginResult charcrRaceConfirmed(final LoginSession loginObj,
			final Session session) {
		final String input = loginObj.lastInput.trim().toUpperCase();
		if (input.startsWith("N"))
			loginObj.state = LoginState.CHARCR_RACESTART;
	...
	protected LoginResult charcrRaceDone(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
	...
	protected LoginResult charcrGenderStart(final LoginSession loginObj,
			final Session session) {
	...
	protected LoginResult charcrGenderDone(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		final PlayerAccount acct = loginObj.acct;
		...
		if ((CMProps.getBoolVar(CMProps.Bool.ACCOUNTEXPIRATION))
				&& (mob.playerStats() != null) && (acct == null))
	...
	protected LoginResult charcrStatInit(final LoginSession loginObj,
			final Session session, final int bonusPoints) {
		final MOB mob = loginObj.mob;
		int startStat = CMProps.getIntVar(CMProps.Int.STARTSTAT);
		if ((CMSecurity.isDisabled(CMSecurity.DisFlag.ATTRIBS) && (startStat <= 0)))
			startStat = 10;
		if (startStat > 0) {
			mob.baseCharStats().setAllBaseValues(
					CMProps.getIntVar(CMProps.Int.STARTSTAT));
			for (int i = 0; i < bonusPoints; i++) {
				int randStat = CMLib.dice().roll(1,
						CharStats.CODES.BASE().length, -1);
				mob.baseCharStats().setStat(randStat,
						mob.baseCharStats().getStat(randStat) + 1);
			}
			mob.recoverCharStats();
			loginObj.state = LoginState.CHARCR_STATDONE;
		} else {
			StringBuffer introText = new CMFile(
					Resources.buildResourcePath("text") + "stats.txt", null,
					CMFile.FLAG_LOGERRORS).text();
			try {
				introText = CMLib.webMacroFilter().virtualPageFilter(introText);
			} catch (Exception ex) {
			...
			for (int i = 0; i < CharStats.CODES.BASE().length; i++)
				mob.baseCharStats().setStat(i,
						CMProps.getIntVar(CMProps.Int.BASEMINSTAT));
			mob.recoverCharStats();
	...
	protected LoginResult charcrStatStart(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		if (loginObj.baseStats == null)
			loginObj.baseStats = (CharStats) mob.baseCharStats().copyOf();
		List<String> validStats = new ArrayList<String>(
				CharStats.CODES.BASE().length);
		for (int i : CharStats.CODES.BASE())
			validStats
					.add(CMStrings.capitalizeAndLower(CharStats.CODES.NAME(i)));
		...
		if (!randomRoll || (qualifyingClassListV.size() > 0)
			...
			for (int i : CharStats.CODES.BASE()) {
				total += CT.getStat(i);
				statstr.append("^H"
						+ CMStrings.padRight(CMStrings
								.capitalizeAndLower(CharStats.CODES.DESC(i)),
								15)
						+ "^N: ^w"
			...
			statstr.append("^w" + CMStrings.padRight("STATS TOTAL", 15)
					+ "^N: ^w" + total + "^N/^w"
					+ (CMProps.getIntVar(CMProps.Int.BASEMAXSTAT) * 6) + "^.^N");
			session.println(statstr.toString());
			if (!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES)
					&& (!mob.baseCharStats().getMyRace().classless())
					&& (randomRoll || qualifyingClassListV.size() > 0)
					&& ((qualifyingClassListV.size() != 1) || (!CMProps.getVar(
							CMProps.Str.MULTICLASS).startsWith("APP-"))))
				session.println("\n\rThis would qualify you for ^H"
						+ buildQualifyingClassList(mob, qualifyingClassListV,
								"and") + "^N.");
			if (randomRoll) {
				...
				if (loginObj.statPoints == 0) {
					...
					session.println("\n\r^NYou have ^w" + loginObj.statPoints
							+ "^N points remaining.^N");
	...
	protected LoginResult charcrStatConfirm(final LoginSession loginObj,
			final Session session) {
		final String input = loginObj.lastInput.toUpperCase().trim();
		if (input.startsWith("Y"))
			loginObj.state = LoginState.CHARCR_STATSTART;
	...
	protected LoginResult charcrStatPickAdd(final LoginSession loginObj,
			final Session session) {
		loginObj.lastInput = loginObj.savedInput + " " + loginObj.lastInput;
		loginObj.state = LoginState.CHARCR_STATPICK;
		return null;
	...
	protected LoginResult charcrStatPick(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		if (loginObj.baseStats == null)
			loginObj.baseStats = (CharStats) mob.baseCharStats().copyOf();
		...
		List<CharClass> qualifyingClassListV = classQualifies(mob,
				loginObj.theme);
		...
		if (prompt.trim().length() > 0) {
			...
			if ((space > 0)
					&& (CMath.isInteger(prompt.substring(space + 1).trim()) || (prompt
							.substring(space + 1).trim().startsWith("+")))) {
				...
				if ((num > -1000) && (num < 1000) && (num != 0))
					...
					session.println("^r'" + numStr
							+ "' is not a positive or negative number.^N");
					loginObj.state = LoginState.CHARCR_STATSTART;
					return null;
				}
				...
				session.println("^r'" + prompt.substring(space + 1)
						+ "' is not a positive or negative number.^N");
				loginObj.state = LoginState.CHARCR_STATSTART;
				return null;
			} else if (remove)
			...
			List<String> validStats = new ArrayList<String>(
					CharStats.CODES.BASE().length);
			for (int i : CharStats.CODES.BASE())
				validStats.add(CMStrings.capitalizeAndLower(CharStats.CODES
						.NAME(i)));
			int statCode = CharStats.CODES.findWhole(prompt, false);
			if ((statCode < 0)
					|| (!validStats
							.contains(CMStrings
									.capitalizeAndLower(CharStats.CODES
											.NAME(statCode))))) {
				session.println("^r'" + prompt
						+ "' is an unknown code.  Try one of these: "
						+ CMParms.toStringList(validStats) + "^N");
				loginObj.state = LoginState.CHARCR_STATSTART;
				return null;
			}
			if (statPointsChange == 0) {
			...
			long[][] costs = CMLib.utensils().compileConditionalRange(
					CMParms.parseCommas(list.trim(), true), 1, 0, 101);
			int pointsCost = 0;
			int curStatValue = CT.getStat(statCode);
			for (int i = 0; i < statPointsChange; i++) {
				int statPoint = remove ? curStatValue - 1 : curStatValue;
				int statCost = 1;
				if ((statPoint > 0) && (statPoint < costs.length)
			...
			if (loginObj.statPoints - pointsCost < 0) {
				if (loginObj.statPoints > 0)
					session.println("^rYou need " + pointsCost
							+ " points to do that, but only have "
							+ loginObj.statPoints + " remaining.^N");
				else
					session.println("^rYou don't have enough remaining points to do that.^N");
				loginObj.state = LoginState.CHARCR_STATSTART;
				return null;
			} else {
				String friendlyName = CMStrings
						.capitalizeAndLower(CharStats.CODES.NAME(statCode));
				if (remove) {
					if (CT.getStat(statCode) <= loginObj.baseStats
							.getStat(statCode)) {
						session.println("^rYou can not lower '" + friendlyName
								+ " any further.^N");
						loginObj.state = LoginState.CHARCR_STATSTART;
						return null;
					} else if (CT.getStat(statCode) - statPointsChange < loginObj.baseStats
							.getStat(statCode)) {
						session.println("^rYou can not lower '" + friendlyName
								+ " any further.^N");
						loginObj.state = LoginState.CHARCR_STATSTART;
					...
					if (CT.getStat(statCode) >= max) {
						session.println("^rYou can not raise '" + friendlyName
								+ " any further.^N");
						loginObj.state = LoginState.CHARCR_STATSTART;
						return null;
					} else if (CT.getStat(statCode) + statPointsChange > max) {
						session.println("^rYou can not raise '" + friendlyName
								+ " any by that amount.^N");
						loginObj.state = LoginState.CHARCR_STATSTART;
						return null;
					}
				}
	...
	protected LoginResult charcrStatDone(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
	...
	protected LoginResult charcrClassInit(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		...
		if (CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES)
			...
			if ((newClass == null) && (qualClassesV.size() > 0))
				newClass = qualClassesV.get(CMLib.dice().roll(1,
						qualClassesV.size(), -1));
			if (newClass == null)
				newClass = CMClass.getCharClass("PlayerClass");
			if (newClass == null)
			...
			if (newClass == null) {
				Log.errOut("CharCreation",
						"Char Classes are disabled, but no PlayerClass or StdCharClass is defined?!");
		...
		if (qualClassesV.size() == 0) {
			...
			if (newClass == null) {
				Log.errOut("CharCreation",
						"No classes qualified for, but no Apprentice or StdCharClass is defined?!");
				loginObj.state = LoginState.CHARCR_CLASSSTART;
			} else {
				mob.baseCharStats().setCurrentClass(newClass);
				loginObj.state = LoginState.CHARCR_CLASSDONE;
			...
			return null;
		} else if ((qualClassesV.size() == 1) || (session == null)
				|| (session.isStopped())) {
		...
		if (!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES)
			...
			session.println(null, null, null,
					new CMFile(Resources.buildResourcePath("text")
							+ "classes.txt", null, CMFile.FLAG_LOGERRORS)
							.text().toString());
		loginObj.state = LoginState.CHARCR_CLASSSTART;
		return null;
	}

	protected LoginResult charcrClassStart(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
	...
	protected LoginResult charcrClassPicked(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		...
		if (ClassStr.trim().equalsIgnoreCase("?")) {
			session.println(null, null, null,
					"\n\r"
							+ new CMFile(Resources.buildResourcePath("text")
									+ "classes.txt", null,
									CMFile.FLAG_LOGERRORS).text().toString());
			loginObj.state = LoginState.CHARCR_CLASSSTART;
			return null;
		}
		List<CharClass> qualClassesV = classQualifies(mob, loginObj.theme);
		...
		if ((newClass != null)
			...
			StringBuilder str = CMLib.help().getHelpText(
					newClass.ID().toUpperCase(), mob, false, false);
			if (str != null) {
				session.println("\n\r^N" + str.toString() + "\n\r");
			}
			session.promptPrint("^NIs ^H" + newClass.name()
					+ "^N correct (Y/n)?");
			mob.baseCharStats().setCurrentClass(newClass);
	...
	protected LoginResult charcrClassConfirm(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		if (loginObj.lastInput.toUpperCase().trim().startsWith("N"))
			...
			session.promptPrint("^NIs ^H"
					+ mob.baseCharStats().getCurrentClass().name()
					+ "^N correct (Y/n)?");
			return LoginResult.INPUT_REQUIRED;
		}
		return null;
	...
	protected LoginResult charcrClassDone(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		mob.basePhyStats().setLevel(1);
		mob.baseCharStats().setClassLevel(
				mob.baseCharStats().getCurrentClass(), 1);
	...
	protected LoginResult charcrFactionNext(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		loginObj.index++;
		...
		if ((F.choiceIntro() != null) && (F.choiceIntro().length() > 0)) {
			StringBuffer intro = new CMFile(Resources.makeFileResourceName(F
					.choiceIntro()), null, CMFile.FLAG_LOGERRORS).text();
			try {
	...
	protected LoginResult charcrFactionPick(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		Faction F = CMLib.factions().getFactionByNumber(loginObj.index);
		if (F == null) {
	...
	protected LoginResult charcrFactionDone(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		...
		mob.baseCharStats().setStat(
				CharStats.STAT_AGE,
				mob.playerStats().initializeBirthday(0,
						mob.baseCharStats().getMyRace()));

		StringBuffer introText = new CMFile(Resources.buildResourcePath("text")
				+ "newchardone.txt", null, CMFile.FLAG_LOGERRORS).text();
		try {
			introText = CMLib.webMacroFilter().virtualPageFilter(introText);
		} catch (Exception ex) {
		}
	...
	protected LoginResult charcrFinish(final LoginSession loginObj,
			final Session session) {
		final MOB mob = loginObj.mob;
		boolean emailPassword = ((CMProps.getVar(CMProps.Str.EMAILREQ)
				.toUpperCase().startsWith("PASS")) && (CMProps.getVar(
		...
		if (emailPassword && (loginObj.acct == null)) {
			...
			CMLib.database().DBUpdatePassword(mob.Name(),
					mob.playerStats().getPasswordStr());
			CMLib.smtp()
					.emailOrJournal(
							CMProps.getVar(CMProps.Str.SMTPSERVERNAME),
							mob.Name(),
			...
			mob.location().showOthers(mob, mob.location(),
					CMMsg.MASK_ALWAYS | CMMsg.MSG_ENTER, "<S-NAME> appears!");
		}
		...
		for (int i = 0; i < channels.size(); i++)
			CMLib.commands().postChannel(channels.get(i), mob.clans(),
					mob.Name() + " has just been created.", true);
	...
	protected List<String> getNamedFactionChoices(Faction F, List<Integer> mine) {
		...
		for (int m = 0; m < mine.size(); m++) {
			Faction.FRange FR = CMLib.factions().getRange(F.factionID(),
					mine.get(m).intValue());
			if (FR != null)
	...
	public NewCharNameCheckResult finishNameCheck(String login, String ipAddress) {
		if ((CMProps.getIntVar(CMProps.Int.MUDTHEME) == 0)
				|| ((CMSecurity.isDisabled(CMSecurity.DisFlag.LOGINS))
						&& (!CMProps.isOnWhiteList(CMProps.SYSTEMWL_LOGINS,
								login)) && (!CMProps.isOnWhiteList(
						CMProps.SYSTEMWL_LOGINS, ipAddress))))
			return NewCharNameCheckResult.NO_NEW_LOGINS;
		else if ((CMProps.getIntVar(CMProps.Int.MAXNEWPERIP) > 0)
				&& (CMProps.getCountNewUserByIP(ipAddress) >= CMProps
						.getIntVar(CMProps.Int.MAXNEWPERIP))
				&& (!CMSecurity.isDisabled(CMSecurity.DisFlag.MAXNEWPERIP))
				&& (!CMProps.isOnWhiteList(CMProps.SYSTEMWL_NEWPLAYERS, login))
				&& (!CMProps.isOnWhiteList(CMProps.SYSTEMWL_NEWPLAYERS,
						ipAddress)))
			return NewCharNameCheckResult.CREATE_LIMIT_REACHED;
		return NewCharNameCheckResult.OK;
	}

	public NewCharNameCheckResult newCharNameCheck(String login,
			String ipAddress, boolean skipAccountNameCheck) {
		final boolean accountSystemEnabled = CMProps
				.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1;

		if (((CMSecurity.isDisabled(CMSecurity.DisFlag.NEWPLAYERS) && (!accountSystemEnabled)) || (CMSecurity
				.isDisabled(CMSecurity.DisFlag.NEWCHARACTERS)))
				&& (!CMProps.isOnWhiteList(CMProps.SYSTEMWL_NEWPLAYERS, login))
				&& (!CMProps.isOnWhiteList(CMProps.SYSTEMWL_NEWPLAYERS,
						ipAddress)))
			return NewCharNameCheckResult.NO_NEW_PLAYERS;
				...
				|| (!skipAccountNameCheck && CMLib.players().accountExists(
						login)))
	...
	public NewCharNameCheckResult newAccountNameCheck(String login,
			String ipAddress) {
	...
	public boolean newCharactersAllowed(String login, Session session,
			PlayerAccount acct, boolean skipAccountNameCheck) {
		switch (newCharNameCheck(login, session.getAddress(),
		...
		case BAD_USED_NAME:
			session.println("\n\r'"
					+ CMStrings.capitalizeAndLower(login)
		...
		case OK:
			if ((acct != null)
					&& (CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) <= acct
							.numPlayers())
					&& (!acct.isSet(PlayerAccount.FLAG_NUMCHARSOVERRIDE))) {
				session.println("You may only have "
						+ CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM)
						+ " characters.  Please retire one to create another.");
				return false;
			}
			return true;
		}
	...
	public boolean newAccountsAllowed(String login, Session session,
			PlayerAccount acct) {
	...
	public void setGlobalBitmaps(MOB mob) {
		if (mob == null)
			return;
		List<String> defaultFlagsV = CMParms.parseCommas(
				CMProps.getVar(CMProps.Str.DEFAULTPLAYERFLAGS).toUpperCase(),
				true);
	...
	public LoginResult completeCharacterLogin(Session session, String login,
			boolean wiziFlag) throws IOException {
		// count number of multiplays
		int numAtAddress = 0;
		try {
			for (Session S : CMLib.sessions().allIterable())
				if ((S != session)
						&& (session.getAddress().equalsIgnoreCase(S
								.getAddress())))
					numAtAddress++;
		} catch (Exception e) {
		}
		...
		if ((CMProps.getIntVar(CMProps.Int.MAXCONNSPERIP) > 0)
				&& (numAtAddress >= CMProps
						.getIntVar(CMProps.Int.MAXCONNSPERIP))
		...
		if ((mob != null) && (mob.session() != null)) {
			...
			if (isExpired(mob.playerStats().getAccount(), session, mob
					.playerStats().getAccountExpiration()))
				return LoginResult.NO_LOGIN;
			...
			if (wiziFlag) {
				...
				if ((C != null)
						&& (C.securityCheck(mob) || C.securityCheck(mob)))
					C.execute(mob, new XVector<Object>("WIZINV"), 0);
			}
			showTheNews(mob);
			Room startRoom = mob.location();
			if (startRoom == null) {
				Log.errOut("CharCreation", mob.name()
						+ " has no location.. sending to start room");
			...
			CMLib.coffeeTables().bump(mob, CoffeeTableRow.STAT_LOGINS);
			startRoom.showOthers(mob, startRoom, CMMsg.MASK_ALWAYS
					| CMMsg.MSG_ENTER, "<S-NAME> appears!");
		} else {
		...
		for (int f = 0; f < mob.numFollowers(); f++) {
			...
			if ((follower.isMonster()) && (!follower.isPossessing())
				...
				R.showOthers(follower, R, CMMsg.MASK_ALWAYS | CMMsg.MSG_ENTER,
						"<S-NAME> appears!");
			}
		}
		...
		if ((pstats.getEmail() != null)
				&& CMSecurity.isBanned(pstats.getEmail())) {
			session.println("\n\rYou are unwelcome.  No one likes you here. Go away.\n\r\n\r");
			session.stopSession(false, false, false);
			return LoginResult.NO_LOGIN;
		}
		...
		if ((CMProps.getVar(CMProps.Str.PKILL).startsWith("NEVER"))
			...
			mob.setBitmap(mob.getBitmap() - MOB.ATT_PLAYERKILL);
		List<String> channels = CMLib.channels().getFlaggedChannelNames(
				ChannelsLibrary.ChannelFlag.LOGINS);
		if (!CMLib.flags().isCloaked(mob))
			for (int i = 0; i < channels.size(); i++)
				CMLib.commands().postChannel(channels.get(i), mob.clans(),
						mob.Name() + " has logged on.", true);
		setGlobalBitmaps(mob);
		return LoginResult.NORMAL_LOGIN;
	}

	public Room getDefaultStartRoom(MOB mob) {
		String race = mob.baseCharStats().getMyRace().racialCategory()
				.toUpperCase();
		race = race.replace(' ', '_');
		String charClass = mob.baseCharStats().getCurrentClass().ID()
				.toUpperCase();
	...
	public Room getDefaultBodyRoom(MOB mob) {
		Pair<Clan, Integer> clanMorgue = CMLib.clans().findPrivilegedClan(mob,
				Clan.Function.MORGUE);
		if ((clanMorgue != null)
			...
			if (C.getMorgue().length() > 0) {
				...
				if ((room != null)
						&& (CMLib.law().doesHavePriviledgesHere(mob, room)))
					return room;
			}
		}
		String charClass = mob.baseCharStats().getCurrentClass().ID()
	...
	public int getTrainingCost(MOB mob, int abilityCode, boolean quiet) {
		...
		long[][] costs = CMLib.utensils().compileConditionalRange(
				CMParms.parseCommas(list.trim(), true), 1, 0, maxStat + 10);
		int curStatIndex = curStat;
		while ((curStatIndex > 0)
				&& ((curStatIndex >= costs.length)
						|| (costs[curStatIndex] == null) || (costs[curStatIndex].length == 0)))
			curStatIndex--;
		int val = 1;
		if (curStatIndex > 0)
	...
	@Override
	public void promptPlayerStats(int theme, MOB mob, Session session,
			int bonusPoints) throws IOException {
	...
	@Override
	public CharClass promptCharClass(int theme, MOB mob, Session session)
			throws IOException {
		final LoginSession loginObj = new LoginSession();
		if (mob.playerStats() != null)
			loginObj.acct = mob.playerStats().getAccount();
	...
	@Override
	public LoginResult createCharacter(PlayerAccount acct, String login,
		...
		try {
			while (!session.isStopped()) {
				if (res == LoginResult.INPUT_REQUIRED)
					loginObj.lastInput = session.blockingIn(90000);
				if ((res == LoginResult.NORMAL_LOGIN)
						|| (res == LoginResult.NO_LOGIN))
					return res;
