...
public class MUDZapper extends StdLibrary implements MaskingLibrary {
	...
	public String rawMaskHelp() {
		return DEFAULT_MASK_HELP;
	}

	protected volatile List<SavedClass> savedCharClasses = new Vector<SavedClass>(
			1);
	...
	public synchronized void buildSavedClasses() {
		...
		for (final Enumeration<CharClass> c = CMClass.charClasses(); c
				.hasMoreElements();) {
	...
	public CompiledZapperMask getPreCompiledMask(final String str) {
		Map<String, CompiledZapperMask> H = (Map<String, CompiledZapperMask>) Resources
				.getResource("SYSTEM_HASHED_MASKS");
		if (H == null) {
			H = new PrioritizingLimitedMap<String, CompiledZapperMask>(200,
					10 * 60 * 1000, Long.MAX_VALUE, 50);
	...
	public Map<String, Integer> getMaskCodes() {
		if (zapCodes.size() == 0) {
			...
			zapCodes.put("-ITEM", Integer.valueOf(33));
			zapCodes.put("+CLASS", Integer.valueOf(34)); // for compiled use
															// ONLY
			zapCodes.put("+ALIGNMENT", Integer.valueOf(35)); // for compiled use
																// ONLY
			zapCodes.put("+GENDER", Integer.valueOf(36)); // for compiled use
															// ONLY
			zapCodes.put("+LVLGR", Integer.valueOf(37)); // for compiled use
															// ONLY
			zapCodes.put("+LVLGE", Integer.valueOf(38)); // for compiled use
															// ONLY
			zapCodes.put("+LVLLT", Integer.valueOf(39)); // for compiled use
															// ONLY
			zapCodes.put("+LVLLE", Integer.valueOf(40)); // for compiled use
															// ONLY
			zapCodes.put("+LVLEQ", Integer.valueOf(41)); // for compiled use
															// ONLY
			...
			zapCodes.put("+RACE", Integer.valueOf(112)); // for compiled use
															// ONLY
	...
	protected Object makeSkillFlagObject(final String str) {
		...
		if (o == null)
			for (int d = 0; d < Ability.DOMAIN_DESCS.length; d++)
				if (Ability.DOMAIN_DESCS[d].startsWith(str)
						|| Ability.DOMAIN_DESCS[d].endsWith(str)) {
		...
		if (o == null)
			for (short d = 0; d < Ability.QUALITY_DESCS.length; d++)
				if (Ability.QUALITY_DESCS[d].startsWith(str)
						|| Ability.QUALITY_DESCS[d].endsWith(str)) {
					o = new Short(d);
					break;
				}
		return o;
	...
	protected boolean skillFlagCheck(final List<String> V,
			final char plusMinus, final int fromHere, final MOB mob) {
		for (int v = fromHere; v < V.size(); v++) {
			...
			if (o == null)
				continue;
			for (final Enumeration<Ability> a = mob.allAbilities(); a
					.hasMoreElements();) {
	...
	protected CompiledZapperMaskEntry levelCompiledHelper(final String str,
			final char c) {
		final Vector<Object> parms = new Vector<Object>();
		Integer entryType = null;
		if (str.startsWith(c + ">=")
				&& (CMath.isNumber(str.substring(3).trim()))) {
			entryType = getMaskCodes().get("+LVLGE");
			parms.addElement(Integer.valueOf(CMath.s_int(str.substring(3)
					.trim())));
			...
			parms.addElement(Integer.valueOf(CMath.s_int(str.substring(3)
				...
				&& (CMath.isNumber(str.substring(2).trim()))) {
			entryType = getMaskCodes().get("+LVLGR");
			parms.addElement(Integer.valueOf(CMath.s_int(str.substring(2)
					.trim())));
		} else if (str.startsWith(c + "<")
				&& (CMath.isNumber(str.substring(2).trim()))) {
		...
		if ((entryType == null) || (parms.size() == 0))
			return null;
		return new CompiledZapperMaskEntry(entryType.intValue(),
				parms.toArray(new Object[0]));
	}

	protected StringBuilder levelHelp(final String lvl, final char c,
			final String append) {
		if (lvl.startsWith(c + ">=")
				&& (CMath.isNumber(lvl.substring(3).trim())))
			return new StringBuilder(append
					+ "levels greater than or equal to "
					+ lvl.substring(3).trim() + ".  ");
		else if (lvl.startsWith(c + "<=")
				&& (CMath.isNumber(lvl.substring(3).trim())))
			return new StringBuilder(append + "levels less than or equal to "
					+ lvl.substring(3).trim() + ".  ");
		else if (lvl.startsWith(c + ">")
				&& (CMath.isNumber(lvl.substring(2).trim())))
			return new StringBuilder(append + "levels greater than "
					+ lvl.substring(2).trim() + ".  ");
		else if (lvl.startsWith(c + "<")
				&& (CMath.isNumber(lvl.substring(2).trim())))
			return new StringBuilder(append + "levels less than "
					+ lvl.substring(2).trim() + ".  ");
		else if (lvl.startsWith(c + "=")
				&& (CMath.isNumber(lvl.substring(2).trim())))
	...
	protected int levelMinHelp(final String lvl, final char c,
			final int minMinLevel, final boolean reversed) {
		if (lvl.startsWith(c + ">=")
				&& (CMath.isNumber(lvl.substring(3).trim())))
			return reversed ? minMinLevel : CMath
					.s_int(lvl.substring(3).trim());
		else if (lvl.startsWith(c + "<=")
				&& (CMath.isNumber(lvl.substring(3).trim())))
			return reversed ? CMath.s_int(lvl.substring(3).trim()) + 1
					: minMinLevel;
		else if (lvl.startsWith(c + ">")
				&& (CMath.isNumber(lvl.substring(2).trim())))
			return reversed ? minMinLevel : CMath
					.s_int(lvl.substring(2).trim()) + 1;
		else if (lvl.startsWith(c + "<")
				&& (CMath.isNumber(lvl.substring(2).trim())))
			return reversed ? CMath.s_int(lvl.substring(2).trim())
					: minMinLevel;
		else if (lvl.startsWith(c + "=")
				&& (CMath.isNumber(lvl.substring(2).trim())))
			return reversed ? minMinLevel : CMath
					.s_int(lvl.substring(2).trim());
		return Integer.MIN_VALUE;
	}

	protected boolean fromHereEqual(final List<String> V, final char plusMinus,
			final int fromHere, final String find) {
		for (int v = fromHere; v < V.size(); v++) {
			final String str = V.get(v);
			if (str.length() == 0)
				continue;
	...
	protected boolean fromHereStartsWith(final List<String> V,
			final char plusMinus, final int fromHere, final String find) {
		for (int v = fromHere; v < V.size(); v++) {
			final String str = V.get(v);
			if (str.length() == 0)
				continue;
	...
	protected boolean fromHereEndsWith(final List<String> V,
			final char plusMinus, final int fromHere, final String find) {
		for (int v = fromHere; v < V.size(); v++) {
			final String str = V.get(v);
			if (str.length() == 0)
	...
	public int countQuals(final List<String> V, final int v,
	...
	public boolean multipleQuals(final List<String> V, final int v,
			final String startsWith) {
			...
			if (zapCodes.containsKey(str))
				...
				case 0: // -class
				{
					...
					if (buf.toString().endsWith(", "))
						buf = new StringBuilder(buf.substring(0,
								buf.length() - 2));
					buf.append(".  ");
				}
					break;
				...
				case 2: // -Race
				{
					...
					for (final SavedRace R : races()) {
						if ((!cats.contains(R.name) && (fromHereStartsWith(V,
								'+', v + 1, R.nameStart))))
							cats.add(R.name);
					}
				...
				case 12: // -Racecats
				{
					buf.append(skipFirstWord ? "Only these racial categor"
							+ (multipleQuals(V, v, "+") ? "ies" : "y") + " "
							: "Allows only these racial categor"
									+ (multipleQuals(V, v, "+") ? "ies" : "y")
									+ " ");
					final LinkedList<String> cats = new LinkedList<String>();
					for (final SavedRace R : races()) {
						if ((!cats.contains(R.racialCategory) && (fromHereStartsWith(
								V, '+', v + 1, R.upperCatName))))
				...
				{
					buf.append(skipFirstWord ? "Only " : "Allows only ");
					if (fromHereStartsWith(V, '+', v + 1, Faction.Align.EVIL
							.toString().substring(0, 3)))
						buf.append(Faction.Align.EVIL.toString().toLowerCase()
								+ ", ");
					if (fromHereStartsWith(V, '+', v + 1, Faction.Align.GOOD
							.toString().substring(0, 3)))
						buf.append(Faction.Align.GOOD.toString().toLowerCase()
								+ ", ");
					if (fromHereStartsWith(V, '+', v + 1, Faction.Align.NEUTRAL
							.toString().substring(0, 3)))
						buf.append(Faction.Align.NEUTRAL.toString()
								.toLowerCase() + ", ");
					if (buf.toString().endsWith(", "))
						buf = new StringBuilder(buf.substring(0,
								buf.length() - 2));
				...
				{
					for (int v2 = v + 1; v2 < V.size(); v2++)
						buf.append(levelHelp(V.elementAt(v2), '+',
								skipFirstWord ? "Only " : "Allows only "));
				}
					break;
				case 6: // -ClassLevels
				{
				...
				case 103: // -MaxclassLevels
				{
					for (int v2 = v + 1; v2 < V.size(); v2++)
						buf.append(levelHelp(V.elementAt(v2), '+',
								skipFirstWord ? "Only highest class "
										: "Allows only highest class "));
				}
					break;
				case 7: // -Tattoos
				{
					buf.append((skipFirstWord ? "The" : "Requires")
							+ " the following tattoo"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 8: // +Tattoos
				{
					buf.append("Disallows the following tattoo"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 121: // +Mood
				{
					buf.append("Disallows the following mood"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 80: // +security
				{
					buf.append("Disallows the following security flag"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 81: // -expertises
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2))
							break;
						if (str2.startsWith("+")) {
							ExpertiseLibrary.ExpertiseDefinition E = CMLib
									.expertises().getDefinition(
											str2.substring(1).toUpperCase()
													.trim());
							if (E != null)
								buf.append(E.name + ", ");
						}
				...
				case 82: // +expertises
				{
					buf.append("Disallows the following expertise"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 101: // -skillflags
				{
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " skill of type: ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
							break;
						if (str2.startsWith("+")) {
							List<String> V3 = CMParms.parseAny(
									str2.substring(1), '&', true);
							String str3 = null;
							for (int v3 = 0; v3 < V3.size(); v3++) {
								str3 = CMStrings.replaceAll(CMStrings
										.capitalizeAndLower(V3.get(v3)), "_",
										" ");
								if (str3.startsWith("!"))
				...
				case 83: // -skills
				{
					buf.append((skipFirstWord ? "O" : "Requires o")
							+ "ne of the following skills: ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (str2.startsWith("+")) {
							...
							if (x > 0) {
								if (str2.endsWith(")"))
									prof = CMath.s_int(str2.substring(x + 1,
											str2.length() - 1));
				...
				case 84: // +skills
				{
					buf.append("Disallows the following skill"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
						...
						if (str2.startsWith("-")) {
							...
							if (A != null) {
								if (prof <= 0)
									...
									buf.append(A.name() + " at more than "
											+ prof + "% proficiency, ");
							}
						}
					}
				...
				case 14: // -Clan
				{
					buf.append((skipFirstWord ? "M" : "Requires m")
							+ "embership in the following clan"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 15: // +Clan
				{
					buf.append("Disallows the following clan"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 50: // -Material
				{
					buf.append((skipFirstWord ? "C" : "Requires c")
							+ "onstruction from the following material"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
							break;
						if (str2.startsWith("+")) {
							final int code = CMLib.materials().getMaterialCode(
									str2.substring(1), false);
							if (code >= 0)
								buf.append(RawMaterial.Material
										.findByMask(code).noun() + ", ");
						}
					}
					if (buf.toString().endsWith(", "))
						buf = new StringBuilder(buf.substring(0,
				...
				case 49: // +Material
				{
					buf.append("Disallows items of the following material"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 58: // -wornon
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2))
							break;
						if (str2.startsWith("+")) {
							long code = Wearable.CODES.FIND_endsWith(str2
									.substring(1));
							if (code >= 0)
				...
				case 72: // -senses
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2))
							break;
						if (str2.startsWith("+")) {
							final int code = CMLib.flags().getSensesCode(
									str2.substring(1));
							if (code >= 0)
				...
				case 71: // +senses
				{
					buf.append("Disallows the following sense"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 73: // +HOUR
				{
					buf.append("Disallowed during the following time"
							+ (multipleQuals(V, v, "-") ? "s" : "")
							+ " of the day: ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
				...
				case 74: // -HOUR
				{
					buf.append((skipFirstWord ? "Only " : "Allowed only ")
				...
				case 75: // +season
				{
					buf.append("Disallowed during the following season"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
						...
						if (str2.startsWith("-"))
							if (CMath.isInteger(str2.substring(1).trim())) {
								int season = CMath.s_int(str2.substring(1)
										.trim());
								if ((season >= 0)
										&& (season < TimeClock.SEASON_DESCS.length))
									buf.append(TimeClock.SEASON_DESCS[season]
											+ ", ");
							} else {
								int season = determineSeason(str2.substring(1)
										.trim());
								if ((season >= 0)
										&& (season < TimeClock.SEASON_DESCS.length))
									buf.append(TimeClock.SEASON_DESCS[season]
				...
				case 104: // +weather
				{
					buf.append("Disallowed during the following weather condition"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
						...
						if (str2.startsWith("-"))
							if (CMath.isInteger(str2.substring(1).trim())) {
								int weather = CMath.s_int(str2.substring(1)
										.trim());
								if ((weather >= 0)
										&& (weather < Climate.WEATHER_DESCS.length))
									buf.append(CMStrings
											.capitalizeAndLower(Climate.WEATHER_DESCS[weather])
											+ ", ");
							} else {
								int weather = CMParms.indexOf(
										Climate.WEATHER_DESCS, str2
				...
				{
					buf.append((skipFirstWord ? "Only " : "Allowed only ")
							+ "during the following weather condition"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 77: // +month
				{
					buf.append("Disallowed during the following month"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 106: // +day
				{
					buf.append("Disallowed during the following day"
							+ (multipleQuals(V, v, "-") ? "s" : "")
							+ " of the month: ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
				...
				case 85: // +quallvl
					if ((v + 1) < V.size()) {
						final Ability A = CMClass
								.getAbility(V.elementAt(v + 1));
						if (A != null) {
							int adjustment = 0;
							if (((v + 2) < V.size())
									&& (CMath.isInteger(V.elementAt(v + 2))))
								adjustment = CMath.s_int(V.elementAt(v + 2));
							buf.append(A.Name());
							if (adjustment != 0)
								buf.append("Qualifies for " + A.Name());
							else if (adjustment < 0)
								buf.append((-adjustment)
										+ " levels before qualifying for "
										+ A.Name());
							else
								buf.append(adjustment
										+ " levels after qualifying for "
										+ A.Name());
							buf.append(".  ");
						}
					}
					...
					if ((v + 1) < V.size()) {
						...
						if (A != null) {
							...
							if (adjustment != 0)
								...
								buf.append("Still prior to " + (-adjustment)
										+ " levels before qualifying for "
										+ A.Name());
							else
								buf.append("Still prior to " + adjustment
				...
				case 70: // -disposition
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2))
							break;
						if (str2.startsWith("+")) {
							final int code = CMLib.flags().getDispositionCode(
									str2.substring(1));
							if (code >= 0)
				...
				case 69: // +disposition
				{
					buf.append("Disallows the following disposition"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 52: // -Resource
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2))
							break;
						if (str2.startsWith("+")) {
							final int code = CMLib.materials().getResourceCode(
									str2.substring(1), false);
							if (code >= 0)
								buf.append(CMStrings
										.capitalizeAndLower(RawMaterial.CODES
												.NAME(code))
										+ ", ");
						}
					}
				...
				case 53: // -JavaClass
				{
					buf.append((skipFirstWord ? "B" : "Requires b")
							+ "eing of the following type"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 44: // -Deity
				{
					buf.append((skipFirstWord ? "W" : "Requires w")
							+ "orshipping the following deity"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 16: // +Names
				{
					buf.append("Disallows the following mob/player name"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 113: // -Questwin
				{
					buf.append((skipFirstWord ? "Completing"
							: "Requires completing")
							+ " the following quest"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (str2.startsWith("+")) {
							...
							if (Q == null)
								buf.append(str2.substring(1) + ", ");
							else if ((Q.displayName() != null)
									&& (Q.displayName().trim().length() > 0))
				...
				case 114: // +Questwin
				{
					buf.append("Disallows those who`ve won the following quest"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 112: // +races
				{
					buf.append("Disallows the following race"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 13: // +racecats
				{
					buf.append("Disallows the following racial category"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 17: // -anyclass
				{
					buf.append((skipFirstWord ? "L" : "Requires l")
							+ "evels in one of the following:  ");
					for (final SavedClass C : charClasses()) {
				...
				case 19: // +adjstr
					...
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " strength of at least " + val + ".  ");
					break;
				case 20: // +adjint
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "An" : "Requires an")
							+ " intelligence of at least " + val + ".  ");
					break;
				case 21: // +adjwis
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " wisdom of at least " + val + ".  ");
					break;
				case 22: // +adjdex
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " dexterity of at least " + val + ".  ");
					break;
				case 23: // -adjcha
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " constitution of at least " + val + ".  ");
					break;
				case 24: // +adjcha
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " charisma of at least " + val + ".  ");
					break;
				case 25: // -adjstr
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " strength of at most " + val + ".  ");
					break;
				case 26: // -adjint
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "An" : "Requires an")
							+ " intelligence of at most " + val + ".  ");
					break;
				case 27: // -adjwis
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " wisdom of at most " + val + ".  ");
					break;
				case 28: // -adjdex
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " dexterity of at most " + val + ".  ");
					break;
				case 29: // -adjcon
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " constitution of at most " + val + ".  ");
					break;
				case 30: // -adjcha
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " charisma of at most " + val + ".  ");
					break;
				case 87: // +str
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base strength of at least " + val + ".  ");
					break;
				case 88: // +int
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base intelligence of at least " + val + ".  ");
					break;
				case 89: // +wis
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base wisdom of at least " + val + ".  ");
					break;
				case 90: // +dex
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base dexterity of at least " + val + ".  ");
					break;
				case 91: // +con
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base constitution of at least " + val + ".  ");
					break;
				case 92: // +cha
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base charisma of at least " + val + ".  ");
					break;
				case 93: // -str
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base strength of at most " + val + ".  ");
					break;
				case 94: // -int
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base intelligence of at most " + val + ".  ");
					break;
				case 95: // -wis
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base wisdom of at most " + val + ".  ");
					break;
				case 96: // -dex
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base dexterity of at most " + val + ".  ");
					break;
				case 97: // -con
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base constitution of at most " + val + ".  ");
					break;
				case 98: // -cha
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " base charisma of at most " + val + ".  ");
					break;
				case 122: // -chance
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "" : "Allowed ") + " "
							+ (100 - val) + "% of the time.  ");
					break;
				case 55: // +able
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " magic/ability of at most " + val + ".  ");
					break;
				case 56: // -able
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " magic/ability of at least " + val + ".  ");
					break;
				case 59: // +value
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " value of at most " + val + ".  ");
					break;
				case 60: // -value
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " value of at least " + val + ".  ");
					break;
				case 61: // +weight
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " weight/encumbrance of at most " + val + ".  ");
					break;
				case 62: // -weight
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " weight/encumbrance of at least " + val + ".  ");
					break;
				case 63: // +armor
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " armor rating of at most " + val + ".  ");
					break;
				case 64: // -armor
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " armor rating of at least " + val + ".  ");
					break;
				case 65: // +damage
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " damage ability of at most " + val + ".  ");
					break;
				case 66: // -damage
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " damage ability of at least " + val + ".  ");
					break;
				case 67: // +attack
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "An" : "Requires an")
							+ " attack bonus of at most " + val + ".  ");
					break;
				case 68: // -attack
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "An" : "Requires an")
							+ " attack bonus of at least " + val + ".  ");
					break;
				case 32: // +Area
				{
					buf.append("Disallows the following area"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 31: // -Area
				{
					buf.append((skipFirstWord ? "The" : "Requires the")
							+ " following area"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
				...
				case 99: // +Home
				{
					buf.append("Disallows those whose home is the following area"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 100: // -Home
				{
					buf.append((skipFirstWord ? "From the"
							: "Requires being from the")
							+ " following area"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
				...
				case 42: // +Effects
				{
					buf.append("Disallows the following activities/effect"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 43: // -Effects
				{
					buf.append((skipFirstWord ? "P" : "Requires p")
							+ "articipation in the following activities/effect"
							+ (multipleQuals(V, v, "+") ? "s" : "") + ": ");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
				...
				case 46: // -faction
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2))
							break;
						if (str2.startsWith("+")) {
							Faction.FRange FR = getRange(str2.substring(1)
									.toUpperCase().trim());
							if (FR != null) {
								String desc = CMLib.factions()
										.rangeDescription(FR, "or ");
								if (desc.length() > 0)
									buf.append(desc + "; ");
							}
						}
				...
				case 115: // -groupsize
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " group size of at most " + val + ".  ");
					break;
				case 116: // +groupsize
					val = ((++v) < V.size()) ? CMath.s_int(V.elementAt(v)) : 0;
					buf.append((skipFirstWord ? "A" : "Requires a")
							+ " group size of at least " + val + ".  ");
					break;
				case 118: // -if
					buf.append((skipFirstWord ? "n" : "Requires n")
							+ "ot meeting the following condition(s):");
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						final String str2 = V.elementAt(v2);
						if (zapCodes.containsKey(str2))
							break;
				...
				case 117: // +baseclass
				{
					buf.append("Disallows the following types"
							+ (multipleQuals(V, v, "-") ? "s" : "") + ": ");
					HashSet<String> seenBase = new HashSet<String>();
					for (final SavedClass C : charClasses()) {
						if (!seenBase.contains(C.baseClass)) {
							seenBase.add(C.baseClass);
							if (fromHereStartsWith(V, '-', v + 1,
									C.baseClassStart))
								buf.append(C.baseClass + " types, ");
						}
					}
				...
				for (final SavedRace R : races()) {
					if ((str.startsWith(R.minusCatNameStart))
							&& (!cats.contains(R.racialCategory))) {
						cats.add(R.racialCategory);
						buf.append("Disallows " + R.racialCategory + ".  ");
					}
				}
				if (str.startsWith("-"
						+ Faction.Align.EVIL.toString().substring(0, 3)))
					buf.append("Disallows "
							+ Faction.Align.EVIL.toString().toLowerCase()
							+ ".  ");
				if (str.startsWith("-"
						+ Faction.Align.GOOD.toString().substring(0, 3)))
					buf.append("Disallows "
							+ Faction.Align.GOOD.toString().toLowerCase()
							+ ".  ");
				if (str.startsWith("-"
						+ Faction.Align.NEUTRAL.toString().substring(0, 3)))
					buf.append("Disallows "
							+ Faction.Align.NEUTRAL.toString().toLowerCase()
							+ ".  ");
			...
			if (zapCodes.containsKey(str)) {
				...
				case 101: // -skillflag
				{
					...
					for (final Object O : objs)
						for (final Enumeration<Ability> e = CMClass.abilities(); e
								.hasMoreElements();) {
	...
	public CompiledZapperMask maskCompile(final String text) {
		...
		if (text.trim().length() == 0)
			return new CompiledZapperMask(new boolean[] { false, false },
					buf.toArray(new CompiledZapperMaskEntry[0]));
		final Map<String, Integer> zapCodes = getMaskCodes();
		final Vector<String> V = CMParms.parse(text.toUpperCase());
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (entryType != null)
				...
				case 0: // -class
				{
					...
					buf.add(new CompiledZapperMaskEntry(entryType.intValue(),
							parms.toArray(new Object[0])));
				}
					break;
				case 1: // -baseclass
				...
				case 103: // -MaxclassLevels
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						final CompiledZapperMaskEntry e = levelCompiledHelper(
								str2, '+');
						if (e != null) {
				...
				case 79: // -security
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2)) {
							...
							CMSecurity.SecFlag flag = (CMSecurity.SecFlag) CMath
									.s_valueOf(CMSecurity.SecFlag.class, str2
											.substring(1).toUpperCase().trim()
											.replace(' ', '_'));
							if (flag == null)
								Log.errOut("MUDZapper",
										"Illegal security flag '" + str2);
							else
					...
					buf.add(new CompiledZapperMaskEntry(entryType.intValue(),
							parms.toArray(new CMSecurity.SecFlag[0])));
					break;
				}
				case 7: // -Tattoos
				case 31: // -Area
				...
				case 46: // -Faction
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2)) {
							...
							final String str3 = str2.substring(1).toUpperCase()
									.trim();
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2)) {
							...
							final int code = CMLib.materials().getMaterialCode(
									str2.substring(1), false);
							if (code >= 0)
								parms.addElement(RawMaterial.Material
										.findByMask(
												code
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2)) {
							...
							break;
								...
								|| (str2.startsWith("+"))) {
							final long code = Wearable.CODES.FIND_endsWith(str2
									.substring(1));
							if (code >= 0)
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2)) {
							...
							final int code = CMLib.flags().getDispositionCode(
									str2.substring(1));
							if (code >= 0)
								parms.addElement(Integer.valueOf((int) CMath
										.pow(2, code)));
						}
						v = V.size();
					}
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2)) {
							...
							break;
								...
								|| (str2.startsWith("+"))) {
							if (CMath.isInteger(str2.substring(1).trim()))
								parms.addElement(Integer.valueOf(CMath
										.s_int(str2.substring(1).trim())));
							else if (determineSeason(str2.substring(1).trim()) >= 0)
								parms.addElement(Integer
										.valueOf(determineSeason(str2
												.substring(1).trim())));
						}
						v = V.size();
					}
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2)) {
							...
							if (CMath.isInteger(str2.substring(1).trim()))
								parms.addElement(Integer.valueOf(CMath
										.s_int(str2.substring(1).trim())));
							else if (CMParms.indexOf(Climate.WEATHER_DESCS,
									str2.substring(1).trim()) >= 0)
								parms.addElement(Integer.valueOf(CMParms
										.indexOf(Climate.WEATHER_DESCS, str2
												.substring(1).trim())));
						}
						v = V.size();
					}
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2)) {
							...
							break;
								...
								|| (str2.startsWith("+")))
							parms.addElement(Integer.valueOf(CMath.s_int(str2
									.substring(1).trim())));
						v = V.size();
					}
					buf.add(new CompiledZapperMaskEntry(entryType.intValue(),
				...
				case 86: // -quallvl
					if ((v + 1) < V.size()) {
						...
						if (A != null) {
							...
							buf.add(new CompiledZapperMaskEntry(entryType
									.intValue(), parms.toArray(new Object[0])));
						}
					}
					break;
				case 110: // +subop
				...
				case 119: // +if
				{
					...
					for (int v2 = v + 1; v2 < V.size(); v2++) {
						...
						if (zapCodes.containsKey(str2)
							...
							ScriptingEngine SE = (ScriptingEngine) CMClass
							...
							try {
								...
								Log.errOut("MUDZapper",
										"Script parse Exception for " + str2);
				...
				if ((!found)
						&& (str.startsWith("-"
								+ Faction.Align.NEUTRAL.toString().substring(0,
										3)))) {
					final Vector<Object> parms = new Vector<Object>();
					entryType = zapCodes.get("+ALIGNMENT");
					parms.addElement(Faction.Align.NEUTRAL.toString());
				...
				if ((!found) && (str.startsWith("-FEMALE"))) {
					final Vector<Object> parms = new Vector<Object>();
					entryType = zapCodes.get("+GENDER");
					parms.addElement("F");
					found = true;
				...
				if ((!found) && (str.startsWith("-NEUTER"))) {
					final Vector<Object> parms = new Vector<Object>();
					entryType = zapCodes.get("+GENDER");
					parms.addElement("N");
					found = true;
		...
		return new CompiledZapperMask(new boolean[] { buildItemFlag,
				buildRoomFlag }, buf.toArray(new CompiledZapperMaskEntry[0]));
	}
	...
	public boolean maskCheck(final String text, final Environmental E,
			final boolean actual) {
		return maskCheck(getPreCompiledMask(text), E, actual);
	}

	public boolean maskCheck(final CompiledZapperMask cset,
			final Environmental E, final boolean actual) {
		if (E == null)
			return true;
		...
		final Item item = flags[0] ? ((E instanceof Item) ? (Item) E
				: nonCrashingItem(mob)) : null;
		final Room room = flags[1] ? ((E instanceof Area) ? outdoorRoom((Area) E)
		...
		for (CompiledZapperMaskEntry entry : cset.entries) {
			try {
				...
				case 110: // +subop
					if (CMSecurity.isASysOp(mob)
							|| ((room != null) && (room.getArea().amISubOp(mob
									.Name()))))
						return true;
					break;
				case 111: // -subop
				...
				case 0: // -class
				{
					if (!CMParms.contains(entry.parms, actual ? mob
							.baseCharStats().getCurrentClass().name() : mob
				...
				case 1: // -baseclass
				{
					String baseClass = mob.baseCharStats().getCurrentClass()
							.baseClass();
					if ((!actual)
							&& (!baseClass.equals(mob.charStats()
									.displayClassName()))) {
						final CharClass C = CMClass.getCharClass(mob
								.charStats().displayClassName());
						if (C != null)
				...
				case 2: // -race
					if (!CMParms.contains(entry.parms, actual ? mob
							.baseCharStats().getMyRace().name() : mob
							.charStats().raceName()))
						return false;
					break;
				case 3: // -alignment
					if (!CMParms.contains(entry.parms, CMLib.flags()
							.getAlignmentName(mob)))
						return false;
					break;
				case 4: // -gender
				{
				...
				case 5: // -level
					if (P != null) {
						final int level = actual ? P.basePhyStats().level() : P
								.phyStats().level();
						boolean found = false;
						for (int v = 0; v < entry.parms.length - 1; v += 2)
							switch (((Integer) entry.parms[v]).intValue()) {
							case 37: // +lvlgr
								if (level > ((Integer) entry.parms[v + 1])
										.intValue())
									found = true;
								break;
							case 38: // +lvlge
				...
				{
					boolean found = false;
					int cl = actual ? mob.baseCharStats().getClassLevel(
							mob.baseCharStats().getCurrentClass()) : mob
							.charStats().getClassLevel(
				...
				case 103: // -maxclasslevel
				{
					...
					int c2 = 0;
					if (actual) {
						cl = mob.baseCharStats().getClassLevel(
								mob.baseCharStats().getMyClass(0));
						for (int v = 1; v < mob.baseCharStats().numClasses(); v++) {
							c2 = mob.baseCharStats().getClassLevel(
									mob.baseCharStats().getMyClass(v));
							if (c2 > cl)
								cl = c2;
						}
					} else {
						cl = mob.charStats().getClassLevel(
								mob.charStats().getMyClass(0));
						for (int v = 1; v < mob.charStats().numClasses(); v++) {
							c2 = mob.charStats().getClassLevel(
									mob.charStats().getMyClass(v));
							if (c2 > cl)
								cl = c2;
						}
					}
				...
				case 7: // -tattoo
				{
					...
					for (final Object o : entry.parms)
						if ((mob.findTattoo((String) o) != null)
								|| ((room != null) && (room.getArea()
										.getBlurbFlag((String) o) != null))) {
							found = true;
							break;
						}
				...
				case 83: // -skill
				{
					...
					for (int v = 0; v < entry.parms.length - 1; v += 2) {
						final Ability A = mob
								.fetchAbility((String) entry.parms[v]);
						if ((A != null)
								&& (A.proficiency() >= ((Integer) entry.parms[v + 1])
										.intValue())) {
							found = true;
							break;
						}
				...
				case 79: // -security
				{
					boolean found = false;
					for (final Object o : entry.parms)
						if (CMSecurity.isAllowed(mob, room,
								(CMSecurity.SecFlag) o)) {
							found = true;
							break;
						}
					if (!found)
				...
				case 12: // -racecat
				{
					String raceCat = mob.baseCharStats().getMyRace()
							.racialCategory();
					if ((!actual)
							&& (!mob.baseCharStats().getMyRace().name()
									.equals(mob.charStats().raceName()))) {
						final Race R2 = CMClass.getRace(mob.charStats()
								.raceName());
						if (R2 != null)
				...
				case 14: // -clan
				{
					...
					if (E instanceof ClanItem) {
						...
						for (Pair<Clan, Integer> c : ((MOB) E).clans()) {
							for (final Object o : entry.parms)
								if (c.first.clanID().equalsIgnoreCase(
										(String) o)) {
									found = true;
									break;
								}
				...
				case 57: // +wornOn
					if (item != null)
						for (final Object o : entry.parms)
							if ((item.rawProperLocationBitmap() & ((Long) o)
									.longValue()) > 0)
								return false;
					break;
				case 58: // -wornOn
				...
				case 69: // +disposition
					if (P != null)
						for (final Object o : entry.parms)
							if ((P.phyStats().disposition() & ((Integer) o)
									.intValue()) > 0)
								return false;
					break;
				case 70: // -disposition
				...
				case 71: // +senses
					if (P != null)
						for (final Object o : entry.parms)
							if ((P.phyStats().sensesMask() & ((Integer) o)
									.intValue()) > 0)
								return false;
					break;
				case 72: // -senses
				...
				case 73: // +HOUR
				{
					if (room != null)
						for (final Object o : entry.parms)
							if (room.getArea().getTimeObj().getTimeOfDay() == ((Integer) o)
									.intValue())
								return false;
				}
					break;
				...
				case 75: // +season
				{
					if (room != null)
						for (final Object o : entry.parms)
							if (room.getArea().getTimeObj().getSeasonCode() == ((Integer) o)
									.intValue())
								return false;
				}
					break;
				...
				case 104: // +weather
				{
					if (room != null)
						for (final Object o : entry.parms)
							if (room.getArea().getClimateObj()
									.weatherType(room) == ((Integer) o)
									.intValue())
								return false;
				}
					break;
				...
				case 77: // +month
				{
					if (room != null)
						for (final Object o : entry.parms)
							if (room.getArea().getTimeObj().getMonth() == ((Integer) o)
									.intValue())
								return false;
				}
					break;
				...
				case 106: // +day
				{
					if (room != null)
						for (final Object o : entry.parms)
							if (room.getArea().getTimeObj().getDayOfMonth() == ((Integer) o)
									.intValue())
								return false;
				}
					break;
				...
				case 85: // +quallvl
					if (entry.parms.length > 1) {
						final Ability A = CMClass
								.getAbility((String) entry.parms[0]);
						final int adjustment = ((Integer) entry.parms[1])
								.intValue();
						final int lvl = CMLib.ableMapper()
								.qualifyingClassLevel(mob, A);
						final int clvl = CMLib.ableMapper().qualifyingLevel(
								mob, A)
								+ adjustment;
						if (lvl < clvl)
							return false;
				...
				case 51: // +resource
					if ((item != null)
							&& CMParms.contains(entry.parms,
									RawMaterial.CODES.NAME(item.material())))
						return false;
					break;
				case 52: // -resource
					if ((item != null)
				...
				case 45: // +deity
				{
					if (mob.getWorshipCharID().length() > 0)
						for (final Object o : entry.parms)
							if (mob.getWorshipCharID().equalsIgnoreCase(
									(String) o)) {
								return false;
							}
				}
					...
					if (actual) {
						...
						for (final Object o : entry.parms)
							if ((mob.charStats().getClassLevel((String) o) >= 0)
									|| (mob.charStats().displayClassName()
				...
				case 19: // +adjstr
					if ((entry.parms.length > 0)
							&& (mob.charStats()
									.getStat(CharStats.STAT_STRENGTH) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 20: // +adjint
					if ((entry.parms.length > 0)
							&& (mob.charStats().getStat(
									CharStats.STAT_INTELLIGENCE) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 21: // +adjwis
					if ((entry.parms.length > 0)
							&& (mob.charStats().getStat(CharStats.STAT_WISDOM) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 22: // +adjdex
					if ((entry.parms.length > 0)
							&& (mob.charStats().getStat(
									CharStats.STAT_DEXTERITY) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 23: // -adjcha
					if ((entry.parms.length > 0)
							&& (mob.charStats().getStat(
									CharStats.STAT_CONSTITUTION) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 24: // +adjcha
					if ((entry.parms.length > 0)
							&& (mob.charStats()
									.getStat(CharStats.STAT_CHARISMA) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 25: // -adjstr
				...
				case 87: // +str
					base = getBaseCharStats(base, mob);
					if ((entry.parms.length > 0)
							&& (base.getStat(CharStats.STAT_STRENGTH) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 88: // +int
					base = getBaseCharStats(base, mob);
					if ((entry.parms.length > 0)
							&& (base.getStat(CharStats.STAT_INTELLIGENCE) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 89: // +wis
					base = getBaseCharStats(base, mob);
					if ((entry.parms.length > 0)
							&& (base.getStat(CharStats.STAT_WISDOM) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 90: // +dex
					base = getBaseCharStats(base, mob);
					if ((entry.parms.length > 0)
							&& (base.getStat(CharStats.STAT_DEXTERITY) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 91: // +con
					base = getBaseCharStats(base, mob);
					if ((entry.parms.length > 0)
							&& (base.getStat(CharStats.STAT_CONSTITUTION) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 92: // +cha
					base = getBaseCharStats(base, mob);
					if ((entry.parms.length > 0)
							&& (base.getStat(CharStats.STAT_CHARISMA) < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 93: // -str
				...
				case 122: // -chance
					if ((entry.parms.length > 0)
							&& (CMLib.dice().rollPercentage() < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 55: // +able
					if ((entry.parms.length > 0)
							&& (P != null)
							&& (P.phyStats().ability() > (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 56: // -able
				...
				case 61: // +weight
					if ((entry.parms.length > 0)
							&& (P != null)
							&& (P.phyStats().weight() > (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 62: // -weight
				...
				case 63: // +armor
					if ((entry.parms.length > 0)
							&& (P != null)
							&& (P.phyStats().armor() > (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 64: // -armor
				...
				case 65: // +damage
					if ((entry.parms.length > 0)
							&& (P != null)
							&& (P.phyStats().damage() > (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 66: // -damage
				...
				case 67: // +attack
					if ((entry.parms.length > 0)
							&& (P != null)
							&& (P.phyStats().attackAdjustment() > (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 68: // -attack
				...
				case 59: // +value
					if (E instanceof MOB) {
						if ((entry.parms.length > 0)
								&& (CMLib
										.beanCounter()
										.getTotalAbsoluteValueAllCurrencies(mob) > (((Integer) entry.parms[0])
										.intValue())))
							return false;
					} else {
						if ((entry.parms.length > 0)
								&& (item != null)
								&& (item.baseGoldValue() > (((Integer) entry.parms[0])
										.intValue())))
							return false;
					}
					break;
				...
				case 37: // +lvlgr
					if ((entry.parms.length > 0)
							&& (P != null)
							&& ((actual ? P.basePhyStats().level() : P
									.phyStats().level()) > ((Integer) entry.parms[0])
									.intValue()))
						return false;
					break;
				case 38: // +lvlge
				...
				case 116: // +groupsize
					if ((entry.parms.length > 0)
							&& (mob.getGroupMembers(new HashSet<MOB>(1)).size() < (((Integer) entry.parms[0])
									.intValue())))
						return false;
					break;
				case 115: // -groupsize
				...
				case 118: // -if
				{
					...
					if (E instanceof PhysicalAgent)
						for (int v = 0; v < entry.parms.length - 2; v += 3) {
							...
							final Item defaultItem = (E instanceof Item) ? (Item) E
									: null;
							if (SE.eval((PhysicalAgent) E, M, null, M,
									defaultItem, null, "", tmp, EVAL, 0)) {
								oneIsOK = true;
								break;
							}
						}
	...
	public boolean maskCheck(final CompiledZapperMask cset,
			final PlayerLibrary.ThinPlayer E) {
		if (E == null)
			return true;
		...
		for (CompiledZapperMaskEntry entry : cset.entries) {
			try {
				...
				case 110: // +subop
					if (CMSecurity.isASysOp(E))
						return true;
					for (final Enumeration<Area> e = CMLib.map().areas(); e
							.hasMoreElements();)
				...
				case 0: // -class
				{
					...
					if ((C == null)
							|| (!CMParms.contains(entry.parms, C.name())))
						return false;
					break;
				}
				case 1: // -baseclass
				{
					final CharClass C = CMClass.getCharClass(E.charClass);
					if ((C == null)
							|| (!CMParms.contains(entry.parms, C.baseClass())))
						return false;
					break;
				}
				case 117: // +baseclass
				...
				case 2: // -race
				{
					...
					if ((R == null)
							|| (!CMParms.contains(entry.parms, R.name())))
						return false;
					break;
				}
				case 5: // -level
