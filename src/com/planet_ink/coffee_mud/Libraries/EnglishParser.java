...
public class EnglishParser extends StdLibrary implements EnglishParsing {
	...
	private final static String[] articles = { "a", "an", "all of", "some one",
			"a pair of", "one of", "all", "the", "some" };
	public static boolean[] PUNCTUATION_TABLE = null;
	public final static char[] ALL_CHRS = "ALL".toCharArray();
	public final static String[] fwords = { "calf", "half", "knife", "life",
			"wife", "elf", "self", "shelf", "leaf", "sheaf", "thief", "loaf",
			"wolf" };
	public final static List<Environmental> empty = new ReadOnlyVector<Environmental>(
			1);
	...
	public String makePlural(String str) {
		if ((str == null) || (str.length() == 0))
			return str;
		final boolean uppercase = Character
				.isUpperCase(str.charAt(str.length() - 1));
		final String lowerStr = str.toLowerCase();
		if (lowerStr.endsWith("is"))
			return str.substring(0, str.length() - 2)
					+ (uppercase ? "ES" : "es");
		if (lowerStr.endsWith("s") || lowerStr.endsWith("z")
				|| lowerStr.endsWith("x") || lowerStr.endsWith("ch")
				|| lowerStr.endsWith("sh"))
			return str + (uppercase ? "ES" : "es");
		if (lowerStr.endsWith("ay") || lowerStr.endsWith("ey")
				|| lowerStr.endsWith("iy") || lowerStr.endsWith("oy")
				|| lowerStr.endsWith("uy"))
	...
	public String insertUnColoredAdjective(String str, String adjective) {
		...
		if ((uppStr.startsWith("A ")) || (uppStr.startsWith("AN ")))
			return properIndefiniteArticle(adjective) + " " + adjective + " "
	...
	public CMObject findCommand(MOB mob, List<String> commands) {
		if ((mob == null) || (commands == null) || (mob.location() == null)
				|| (commands.size() == 0))
			return null;
		...
		if ((firstWord.length() > 1)
				&& (!Character.isLetterOrDigit(firstWord.charAt(0)))) {
		...
		if ((C != null)
				&& (C.securityCheck(mob))
				&& (!CMSecurity.isCommandDisabled(CMClass.classID(C)
						.toUpperCase())))
			return C;
		...
		for (Enumeration<Ability> a = mob.allAbilities(); a.hasMoreElements();) {
			...
			if ((A != null) && (A.triggerStrings() != null))
				for (int t = 0; t < A.triggerStrings().length; t++)
					if ((A.triggerStrings()[t].toUpperCase()
						...
						if ((A2 != null)
								&& (!CMSecurity.isAbilityDisabled(A2.ID()
										.toUpperCase()))) {
		...
		for (int i = 0; i < thisAbility.triggerStrings().length; i++) {
			if (thisAbility.triggerStrings()[i].equalsIgnoreCase(thisWord)) {
				if (((thisAbility.name().toUpperCase().startsWith(secondWord)))
						|| (collapsedName(thisAbility).toUpperCase()
	...
	public Ability getToEvoke(MOB mob, List<String> commands) {
		...
		for (Enumeration<Ability> a = mob.allAbilities(); a.hasMoreElements();) {
			...
			if ((A != null) && (evokedBy(A, evokeWord))) {
		...
		if ((evokableAbility != null) && (commands.size() > 1)) {
			int classCode = evokableAbility.classificationCode()
					& Ability.ALL_ACODES;
			switch (classCode) {
		...
		if (evokableAbility != null)
			...
			if ((evokableAbility != null) && (!foundMoreThanOne))
				...
				String secondAndThirdWord = secondWord + " "
						+ commands.get(1).toUpperCase();
				...
				for (Enumeration<Ability> a = mob.allAbilities(); a
					...
					if ((A != null)
							&& (evokedBy(A, evokeWord,
									secondAndThirdWord.toUpperCase()))) {
						evokableAbility = A;
						break;
					}
				}
				...
				for (Enumeration<Ability> a = mob.allAbilities(); a
					...
					if ((A != null)
							&& (evokedBy(A, evokeWord))
							&& (A.name().toUpperCase()
									.indexOf(" " + secondWord.toUpperCase()) > 0)) {
						evokableAbility = A;
						commands.remove(0);
						break;
					}
	...
	public boolean preEvoke(MOB mob, List<String> commands, int secondsElapsed,
	...
	public boolean equalsPunctuationless(char[] strC, char[] str2C) {
		...
		while ((s1len > 0)
				&& (Character.isWhitespace(strC[s1len - 1]) || isPunctuation((byte) strC[s1len - 1])))
			s1len--;
		int s2len = str2C.length;
		while ((s2len > 0)
				&& (Character.isWhitespace(str2C[s2len - 1]) || isPunctuation((byte) str2C[s2len - 1])))
			s2len--;
		while (s1 < s1len) {
	...
	public boolean containsString(final String toSrchStr, final String srchStr) {
		...
		while ((!found) && (tos < toSrchC.length)) {
			...
			if ((topOnly) && (!found))
				break;
			while ((!found) && (tos < toSrchC.length)
					&& (Character.isLetter(toSrchC[tos])))
				tos++;
			tos++;
		}
		return found;
	...
	public int getContextNumber(Collection<? extends Environmental> list,
			Environmental E) {
		if (list == null)
			return 0;
		int context = 1;
		for (Object O : list)
			if ((((Environmental) O).Name().equalsIgnoreCase(E.Name()))
				...
				if ((!(O instanceof Item)) || (!(E instanceof Item))
	...
	public int getContextSameNumber(Collection<? extends Environmental> list,
			Environmental E) {
		if (list == null)
			return 0;
		int context = 1;
	...
	public String getContextName(Collection<? extends Environmental> list,
			Environmental E) {
		if (list == null)
			return E.name();
	...
	public String getContextSameName(Collection<? extends Environmental> list,
			Environmental E) {
		if (list == null)
			return E.name();
	...
	public Environmental parseShopkeeper(MOB mob, List<String> commands,
			String error) {
		if (commands.size() == 0) {
			if (error.length() > 0)
				mob.tell(error);
		...
		List<Environmental> V = CMLib.coffeeShops().getAllShopkeepers(
				mob.location(), mob);
		if (V.size() == 0) {
			if (error.length() > 0)
				mob.tell(error);
		...
		if (V.size() > 1) {
			...
			if ((shopkeeper != null)
				...
				mob.tell("You don't see anyone called '"
						+ commands.get(commands.size() - 1)
						+ "' here buying or selling.");
				return null;
			}
			return shopkeeper;
		}
		...
		if (commands.size() > 1) {
			MOB M = mob.location().fetchInhabitant(
					commands.get(commands.size() - 1));
	...
	public List<Item> fetchItemList(Environmental from, MOB mob,
			Item container, List<String> commands,
			Filterer<Environmental> filter, boolean visionMatters) {
		int addendum = 1;
		String addendumStr = "";
		List<Item> V = new Vector<Item>();

		int maxToItem = Integer.MAX_VALUE;
		if ((commands.size() > 1) && (CMath.s_int(commands.get(0)) > 0)) {
			maxToItem = CMath.s_int(commands.get(0));
			commands.set(0, "all");
		}

		String name = CMParms.combine(commands, 0);
		boolean allFlag = (commands.size() > 0) ? commands.get(0)
				.equalsIgnoreCase("all") : false;
		...
		while (doBugFix || ((allFlag) && (addendum <= maxToItem))) {
			...
			if (from instanceof MOB)
				item = ((MOB) from).fetchItem(container, filter, name
						+ addendumStr);
			else if (from instanceof Room)
				item = ((Room) from).fetchFromMOBRoomFavorsItems(mob,
						container, name + addendumStr, filter);
			if ((item != null)
					&& (item instanceof Item)
					&& ((!visionMatters)
							|| (CMLib.flags().canBeSeenBy(item, mob)) || (item instanceof Light))
					&& (!V.contains(item))) {
				V.add((Item) item);
		...
		if (wornOnly && (V.size() > 0)) {
			...
			while (V.size() > 0) {
				Item I = V.get(0);
				topLayer = (I instanceof Armor) ? ((Armor) I)
						.getClothingLayer() : 0;
				which = 0;
				for (int v = 1; v < V.size(); v++) {
					I = V.get(v);
					curLayer = (I instanceof Armor) ? ((Armor) I)
							.getClothingLayer() : 0;
	...
	public long numPossibleGold(Environmental mine, String itemID) {
		...
		if (CMath.isInteger(itemID)) {
			...
			if (mine instanceof MOB) {
				List<Coins> V = CMLib.beanCounter().getStandardCurrency(
						(MOB) mine, CMLib.beanCounter().getCurrency(mine));
				for (int v = 0; v < V.size(); v++)
		...
		if ((V.size() > 1)
				&& ((CMath.isInteger(V.firstElement())) && (matchAnyCurrencySet(CMParms
						.combine(V, 1)) != null)))
			...
			if (currency != null) {
				if (mine instanceof MOB) {
					List<Coins> V2 = CMLib.beanCounter().getStandardCurrency(
							(MOB) mine, currency);
					double denomination = matchAnyDenomination(currency,
							CMParms.combine(V, 1));
					Coins C = null;
				...
				&& (matchAnyCurrencySet(CMParms.combine(V, 0)) != null))
			return 1;
		return 0;
	}
	...
	public double matchAnyDenomination(String currency, String itemID) {
		...
		if (DV != null)
			for (int v2 = 0; v2 < DV.length; v2++) {
				...
				if (containsString(s, itemID))
					return DV[v2].value;
				else if ((s.length() > 0) && (containsString(s, itemID)))
					return DV[v2].value;
			}
		return 0.0;
	}

	public Item possibleRoomGold(MOB seer, Room room, Container container,
			String itemID) {
		if (itemID.toUpperCase().trim().startsWith("A PILE OF "))
			itemID = itemID.substring(10);
		long gold = 0;
		...
		if (gold > 0) {
			for (int i = 0; i < room.numItems(); i++) {
				...
				if ((I.container() == container)
						...
						&& ((itemID.length() == 0) || (containsString(I.name(),
								itemID)))) {
					if (((Coins) I).getNumberOfCoins() <= gold)
						return I;
					((Coins) I).setNumberOfCoins(((Coins) I).getNumberOfCoins()
							- gold);
	...
	public Item bestPossibleGold(MOB mob, Container container, String itemID) {
		...
		if (CMath.isInteger(itemID)) {
			gold = CMath.s_long(itemID);
			double totalAmount = CMLib.beanCounter().getTotalAbsoluteValue(mob,
					currency);
			double bestDenomination = CMLib.beanCounter().getBestDenomination(
					currency, (int) gold, totalAmount);
			if (bestDenomination == 0.0) {
				bestDenomination = CMLib.beanCounter().getBestDenomination(
						null, (int) gold, totalAmount);
				if (bestDenomination > 0.0)
					currency = null;
			}
			if (bestDenomination == 0.0)
		...
		if (gold > 0) {
			double amt = CMLib.beanCounter().getTotalAbsoluteValue(mob,
					currency);
			if (amt >= CMath.mul(denomination, gold)) {
				double expectedAmt = amt - CMath.mul(denomination, gold);
				CMLib.beanCounter().subtractMoney(mob, currency, denomination,
						CMath.mul(denomination, gold));
				double newAmt = CMLib.beanCounter().getTotalAbsoluteValue(mob,
						currency);
				if (newAmt > expectedAmt)
					CMLib.beanCounter().subtractMoney(mob, currency,
							(newAmt - expectedAmt));
				Coins C = (Coins) CMClass.getItem("StdCoins");
			...
			mob.tell("You don't have that much "
					+ CMLib.beanCounter().getDenominationName(currency,
							denomination) + ".");
			List<Coins> V = CMLib.beanCounter().getStandardCurrency(mob,
					currency);
			for (int v = 0; v < V.size(); v++)
	...
	public List<Container> possibleContainers(MOB mob, List<String> commands,
			Filterer<Environmental> filter, boolean withContentOnly) {
		...
		for (int i = commands.size() - 2; i > 0; i--)
			if (commands.get(i).equalsIgnoreCase("from")) {
				...
				if (((containerDex + 1) < commands.size())
						&& ((commands.get(containerDex).equalsIgnoreCase("all")) || (CMath
								.s_int(commands.get(containerDex)) > 0)))
					containerDex++;
				break;
			}
		...
		if (preWord.equalsIgnoreCase("all")) {
			...
			allFlag = true;
			possibleContainerID = "ALL "
					+ possibleContainerID.substring(0,
							possibleContainerID.length() - 4);
		}
		...
		while (doBugFix || ((allFlag) && (addendum <= maxContained))) {
			doBugFix = false;
			Environmental E = mob.location().fetchFromMOBRoomFavorsItems(mob,
					null, possibleContainerID + addendumStr, filter);
			if ((E != null)
					&& (E instanceof Item)
					&& (((Item) E) instanceof Container)
					&& ((!withContentOnly) || (((Container) E).getContents()
							.size() > 0))
	...
	public Item possibleContainer(MOB mob, List<String> commands,
		...
		Environmental E = R.fetchFromMOBRoomFavorsItems(mob, null,
				possibleContainerID, filter);
		if (E == null)
			for (int i = 0; i < R.numItems(); i++) {
				Item I = R.getItem(i);
				if ((I instanceof Electronics.ElecPanel)
						&& (((Electronics.ElecPanel) I).isOpen())) {
					E = R.fetchFromMOBRoomFavorsItems(mob, I,
							possibleContainerID, filter);
	...
	public String returnTime(long millis, long ticks) {
		...
		minutes -= (hours * 60);
		if (hours < 24)
			return hours + "h " + minutes + "m " + seconds + "s " + millis
					+ "ms" + avg;
		long days = hours / 24;
		hours -= (days * 24);
	...
	public Object[] parseMoneyStringSDL(MOB mob, String amount,
		...
		if (amount.length() > 0) {
			...
			if (myCurrency != null) {
				denomination = CMLib.english().numPossibleGoldDenomination(
						null, correctCurrency, amount);
		...
		return new Object[] { myCurrency, Double.valueOf(denomination),
				Long.valueOf(Math.round(b / denomination)) };
	}

	public int calculateMaxToGive(MOB mob, List<String> commands,
			boolean breakPackages, Environmental checkWhat, boolean getOnly) {
		int maxToGive = Integer.MAX_VALUE;
		if ((commands.size() > 1)
				&& (CMLib.english().numPossibleGold(mob,
						CMParms.combine(commands, 0)) == 0)
			...
			if (breakPackages) {
				...
				if ((commands.size() > 2)
						&& ("FROM".startsWith(commands.get(1).toUpperCase()))) {
					throwError = true;
					commands.remove(1);
				}
				...
				if (checkWhat instanceof MOB)
					...
					fromWhat = ((Room) checkWhat).fetchFromMOBRoomFavorsItems(
							mob, null, packCheckName,
				...
				if (fromWhat instanceof Item) {
					...
					Environmental toWhat = CMLib.materials().unbundle(
							(Item) fromWhat, maxToGive, null);
					if (toWhat == null) {
						if (throwError) {
							mob.tell("You can't get anything from "
									+ fromWhat.name() + ".");
							return -1;
						}
					} else if (getOnly && mob.isMine(fromWhat)
							&& mob.isMine(toWhat)) {
						mob.tell("Ok");
						return -1;
	...
	public Environmental fetchEnvironmental(
			Iterable<? extends Environmental> list, String srchStr,
			boolean exactOnly) {
		FetchFlags flags = fetchFlags(srchStr);
		if (flags == null)
			return null;
		...
		try {
			if (exactOnly) {
				...
				for (final Iterator<? extends Environmental> e = list
						.iterator(); e.hasNext();) {
				...
				for (final Iterator<? extends Environmental> e = list
					...
					final Environmental E = e.next();
					if ((E != null)
							&& (containsString(E.name(), srchStr) || containsString(
									E.Name(), srchStr))
							&& ((!allFlag) || ((E.displayText() != null) && (E
									.displayText().length() > 0))))
						if ((--myOccurrance) <= 0)
							return E;
				}
				...
				for (final Iterator<? extends Environmental> e = list
					...
					if ((E != null)
							...
							&& (containsString(E.displayText(), srchStr) || ((E instanceof MOB) && containsString(
									((MOB) E).genericName(), srchStr))))
						if ((--myOccurrance) <= 0)
							return E;
				}
			}
	...
	public List<Environmental> fetchEnvironmentals(
			List<? extends Environmental> list, String srchStr,
			boolean exactOnly) {
	...
	public Environmental fetchEnvironmental(
			Map<String, ? extends Environmental> list, String srchStr,
			boolean exactOnly) {
		if (list.size() == 0)
			return null;
		FetchFlags flags = fetchFlags(srchStr);
			...
			for (Iterator<String> k = list.keySet().iterator(); k.hasNext();) {
				...
				if (E != null)
					if ((containsString(E.displayText(), srchStr))
							|| ((E instanceof MOB) && containsString(
									((MOB) E).genericName(), srchStr)))
						if ((--myOccurrance) <= 0)
							return E;
			}
	...
	public Item fetchAvailableItem(List<Item> list, String srchStr,
			Item goodLocation, Filterer<Environmental> filter, boolean exactOnly) {
		if (list.size() == 0)
			return null;
		FetchFlags flags = fetchFlags(srchStr);
		if (flags == null)
		...
		if (exactOnly) {
				...
				for (final Iterator<Item> i = list.iterator(); i.hasNext();) {
					...
					if ((I.container() == goodLocation)
							...
							&& (I.ID().equalsIgnoreCase(srchStr)
									|| (I.Name().equalsIgnoreCase(srchStr)) || (I
										.name().equalsIgnoreCase(srchStr))))
						if ((!allFlag)
								|| ((I.displayText() != null) && (I
										.displayText().length() > 0)))
							if ((--myOccurrance) <= 0)
								return I;
				}
			...
			try {
				for (final Iterator<Item> i = list.iterator(); i.hasNext();) {
					...
					if ((I != null)
							...
							&& (filter.passesFilter(I))
							&& ((containsString(I.name(), srchStr) || containsString(
									I.Name(), srchStr)) && ((!allFlag) || ((I
									.displayText() != null) && (I.displayText()
									.length() > 0)))))
						if ((--myOccurrance) <= 0)
							return I;
				}
			...
			try {
				for (final Iterator<Item> i = list.iterator(); i.hasNext();) {
					final Item I = i.next();
					if ((I != null) && (I.container() == goodLocation)
							&& (filter.passesFilter(I))
	...
	public List<Item> fetchAvailableItems(List<Item> list, String srchStr,
			Item goodLocation, Filterer<Environmental> filter, boolean exactOnly) {
	...
	public Environmental fetchAvailable(
			Collection<? extends Environmental> list, String srchStr,
			Item goodLocation, Filterer<Environmental> filter, boolean exactOnly) {
		if (list.size() == 0)
			return null;
		FetchFlags flags = fetchFlags(srchStr);
