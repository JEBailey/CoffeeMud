...
public class CoffeeFilter extends StdLibrary implements TelnetFilter {
	...
	public Hashtable<Object, Integer> getTagTable() {
		if (tagTable == null)
			tagTable = CMStrings.makeNumericHash(TelnetFilter.FILTER_DESCS);
		return tagTable;
	}
	...
	public String[] wrapOnlyFilter(String msg, int wrap) {
		...
		while (buf.length() > loop) {
			...
			while ((loop < len) && (buf.length() > loop)) {
				...
				case '!':
					if ((loop < buf.length() - 10)
							&& (buf.charAt(loop + 1) == '!')
							&& ((buf.substring(loop + 2, loop + 7)
									.equalsIgnoreCase("sound")) || (buf
									.substring(loop + 2, loop + 7)
				...
				case '&':
					if (loop < amperStop)
						...
						if (buf.substring(loop, loop + 3).equalsIgnoreCase(
								"lt;"))
							buf.replace(loop, loop + 3, "<");
						else if (buf.substring(loop, loop + 3)
								.equalsIgnoreCase("gt;"))
							buf.replace(loop, loop + 3, ">");
					}
					break;
				...
				default:
					if ((firstAlpha < 0)
							&& (Character.isLetter(buf.charAt(loop))))
						firstAlpha = loop;
					break;
				}
				loop++;
			...
			if ((len < buf.length()) && (loop != lastSp) && (lastSp >= 0)
					&& (loop >= 0) && (loop < buf.length())
					&& (buf.charAt(loop) != 13) && (buf.charAt(loop) != 10)) {
				amperStop = loop;
		...
		if (firstAlpha < buf.length())
			buf.setCharAt(firstAlpha,
					Character.toUpperCase(buf.charAt(firstAlpha)));
		return buf.toString().split("\n\r");
	}

	protected int convertEscape(final Session S, final StringBuffer str,
			final int index) {
		int enDex = index + 1;
		...
		case '?': {
			if ((S != null) && (S.getClientTelnetMode(Session.TELNET_ANSI))) {
				...
				if (lastColor.foregroundCode < 256)
					...
					escapeSequence = "\033[38;5;"
							+ (lastColor.foregroundCode & 0xff) + "m";
				if (lastColor.backgroundCode == '.')
					...
					if (lastColor.backgroundCode < 256) {
						...
						else if (ColorLibrary.MAP_COLOR_TO_BGCOLOR
								.containsKey(bgEscapeSequence))
							bgEscapeSequence = ColorLibrary.MAP_COLOR_TO_BGCOLOR
									.get(bgEscapeSequence);
					} else
						bgEscapeSequence = "\033[48;5;"
								+ (lastColor.backgroundCode & 0xff) + "m";
			...
			if (finalNum >= 0) {
				...
				if (isFg) {
					...
					if ((S != null)
						...
						escapeSequence = ColorLibrary.COLOR_NONE
								+ escapeSequence;
				} else
				...
				if (S != null) {
					if (isFg) {
						S.setLastColor(S.getCurrentColor());
						S.setCurrentColor(ColorState.valueOf(
								(char) (256 | finalNum), '.'));
					} else {
						S.setCurrentColor(ColorState.valueOf(
								S.getCurrentColor().foregroundCode,
								(char) (256 | finalNum)));
					}
				}
				return index + escapeSequence.length() - 1;
		...
		case ColorLibrary.COLORCODE_BACKGROUND: {
			...
			final String[] clookup = (S == null) ? CMLib.color()
					.standardColorLookups() : S.getColorCodes();
			final String escapeSequence = clookup[bc];
			final String bgEscapeSequence = ColorLibrary.MAP_COLOR_TO_BGCOLOR
					.get(escapeSequence);
			if (bgEscapeSequence != null) {
		...
		case '>':
			/*
			 * why was this here? if (currentColor > 0) { if (clookup()[c] ==
			 * null) escapeCodes = clookup()[currentColor]; else if
			 * (clookup()[currentColor] == null) escapeCodes = clookup[c]; else
			 * escapeCodes = clookup()[c] + clookup()[currentColor]; } else
			 */
		...
		default:
			if ((c >= 0)
					&& (c < 256)
					&& ((S == null) || (S
							.getClientTelnetMode(Session.TELNET_ANSI)))) {
				final String[] clookup = (S == null) ? CMLib.color()
						.standardColorLookups() : S.getColorCodes();
				String escapeSequence = clookup[c];
				if (escapeSequence == null)
					escapeSequence = "";
				if ((S != null) && (escapeSequence.length() > 0)
	...
	public String colorOnlyFilter(String msg, Session S) {
		...
		while (buf.length() > loop) {
			...
			case '&':
				if ((S != null)
						&& (S.isAllowedMxp(buf.substring(loop, loop + 3)))) {
					...
					if (buf.substring(loop, loop + 3).equalsIgnoreCase("lt;"))
						buf.replace(loop, loop + 3, "<");
					else if (buf.substring(loop, loop + 3).equalsIgnoreCase(
							"gt;"))
						buf.replace(loop, loop + 3, ">");
				}
				break;
		...
		if (CMSecurity.isDebugging(CMSecurity.DbgFlag.OUTPUT))
			Log.debugOut("CoffeeFilter",
					"OUTPUT: "
							+ (((S != null) && (S.mob() != null)) ? S.mob()
									.Name() : "") + ": " + buf.toString());
		return buf.toString();
	}
	...
	public String getLastWord(StringBuffer buf, int lastSp, int lastSpace) {
		String lastWord = "";
		if (lastSp > lastSpace) {
			lastWord = CMStrings.removeColors(buf.substring(lastSpace, lastSp))
					.trim().toUpperCase();
			while ((lastWord.length() > 0)
					&& (!Character.isLetterOrDigit(lastWord.charAt(0))))
				lastWord = lastWord.substring(1);
			while ((lastWord.length() > 0)
					&& (!Character.isLetterOrDigit(lastWord.charAt(lastWord
							.length() - 1))))
			...
			for (int i = (lastSpace - 1); ((i >= 0) && (!Character
					.isLetterOrDigit(buf.charAt(i)))); i--)
	...
	public String fullOutFilter(Session S, MOB mob, Physical source,
		...
		if (msg.length() == 0)
			return msg;
		String newMsg = (S == null) ? null : CMLib.lang().sessionTranslation(
				msg);
		if (newMsg != null)
			msg = newMsg;
			...
			if (loop == lastLoop) {
				if (++loopDebugCtr > 5) // BZ: delete when this is fixed.
				{
					Log.debugOut(
							...
							"LOOP: " + loop + "/" + wrap + "/!"
									+ (buf.charAt(loop) == '\033') + "!/"
									+ lastSpace + "/" + firstAlpha + "/"
									+ amperStop + "/" + doSagain + "/"
									+ firstSdone + "/" + buf.length() + "/"
									+ loopDebugCtr);
			...
			while ((loop < len) && (buf.length() > loop)) {
				...
				case '!':
					if ((loop < buf.length() - 10)
						...
						if ((x >= 0) && (y >= x)) {
							if ((S != null)
									&& (S.getClientTelnetMode(Session.TELNET_MSP) || S
											.getClientTelnetMode(Session.TELNET_MXP))
									&& ((source == null) || (source == mob) || (CMLib
											.flags().canBeHeardSpeakingBy(
											source, mob)))) {
								if (S.getClientTelnetMode(Session.TELNET_MXP)) {
									...
									loop = y - 1;
										...
										.getClientTelnetMode(Session.TELNET_MSP)) {
									if (wrap > 0)
										len = len + (y - loop) + 1;
									loop = y;
								}
				...
				case '(':
					if ((!wrapOnly) && (loop < (buf.length() - 1))) {
						...
						if (((loop < buf.length() - 2)
								&& (buf.charAt(loop + 2) == ')') && (c2 == 'S'))
								|| ((loop < buf.length() - 3)
										&& (buf.charAt(loop + 3) == ')')
										&& (Character.toUpperCase(buf
												.charAt(loop + 2)) == 'S') && ((c2 == 'Y') || (c2 == 'E')))) {
							String lastWord = getLastWord(buf, lastSp,
									lastSpace);
							int lastParen = (c2 == 'S') ? loop + 2 : loop + 3;
							if (lastWord.equals("A") || lastWord.equals("YOU")
									|| lastWord.equals("1") || doSagain) {
								if (c2 == 'Y')
									buf.replace(
											loop,
											lastParen + 1,
											CMStrings.sameCase("y",
													buf.charAt(loop + 1)));
								else
				...
				case '<':
					if ((!wrapOnly) && ((loop + 1) < buf.length())) {
						...
						if ((cmd != null) && (ldex < buf.length())
								&& (buf.charAt(ldex) == '>')
								&& (cmd.length() > 1) && (cmd.length() < 14)) {
							...
							if (I == null) {
								if ((S != null)
										&& (S.isAllowedMxp(buf.substring(loop,
												loop + 1)))) {
									buf.delete(loop, loop + 1);
									buf.insert(loop, "&lt;".toCharArray());
								}
								...
								case NAME: {
									if (regarding == null)
										...
										if (!firstSdone)
											...
											&& ((!CMLib.flags().canSee(mob)) || (!CMLib
													.flags().canBeSeenBy(
															regarding, mob)))
											&& (regarding.Name().trim()
													.length() > 0))
										replacement = ((regarding instanceof MOB) ? "someone"
												: "something");
									else if (regarding instanceof PhysicalAgent)
										replacement = ((PhysicalAgent) regarding)
												.name(mob);
									else
									...
									if (regarding == null)
										...
										if ((((MOB) regarding).playerStats() != null)
												&& (((MOB) regarding)
														.playerStats()
														.getAccount() != null))
											replacement = ((MOB) regarding)
													.playerStats().getAccount()
													.accountName();
										else
											replacement = ((MOB) regarding)
													.name(mob);
								...
								case NAMENOART: {
									if (regarding == null)
										...
										replacement = CMLib
												.english()
												.cleanArticles(
														((PhysicalAgent) regarding)
																.name(mob));
									else
										replacement = CMLib
												.english()
												.cleanArticles(regarding.name());
								}
									break;
								case NAMESELF: {
									if (regarding == null)
										...
										replacement = ((regarding instanceof MOB) ? (((MOB) regarding)
												.charStats().himher() + "self")
												: "itself");
									else if (regarding instanceof PhysicalAgent)
										replacement = ((PhysicalAgent) regarding)
												.name(mob);
								...
								case YOUPOSS: {
									if (regarding == null)
										...
										replacement = "your";
											...
											&& (regarding.Name().trim()
													.length() > 0))
										replacement = ((regarding instanceof MOB) ? "someone's"
												: "something's");
									else if (regarding instanceof PhysicalAgent)
										replacement = ((PhysicalAgent) regarding)
												.name(mob) + "'s";
							...
							if (replacement != null) {
								String newReplacement = CMLib.lang()
										.filterTranslation(replacement);
				...
				case '\033': // skip escapes
				{
					if ((loop < buf.length() - 1)
						...
						// if(buf.charAt(loop)=='\033')
						// loop--; // force a retry of this char. 2013: why do
						// this? only possible if you didn't move, and this
						// promises less moving!
					}
					break;
				}
				case '^': {
		...
		/*
		 * fabulous debug code for(int i=0;i<buf.length();i+=25) { for(int
		 * x=0;x<25;x++) { if((i+x)<buf.length()) { char c=buf.charAt(i+x);
		 * if((c!='\r')&&(c!='\n')) System.out.print(c); else
		 * System.out.print("?"); } } System.out.print(" "); for(int
		 * x=0;x<25;x++) { if((i+x)<buf.length()) { int c=(int)buf.charAt(i+x);
		 * int a=c/16; int b=c%16;
		...
		while (x < input.length()) {
			...
			if (c == '\'')
				...
				if (x == 0)
					...
					input = new StringBuilder(newStr.substring(0, x - 1)
							+ newStr.substring(x + 1));
					x--;
				}
				x--;
			}
