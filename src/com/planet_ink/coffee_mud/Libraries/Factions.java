...
public class Factions extends StdLibrary implements FactionManager {
	...
	public void reloadFactions(String factionList) {
		List<String> preLoadFactions = CMParms.parseSemicolons(factionList,
				true);
		clearFactions();
	...
	private Faction buildFactionFromXML(StringBuffer buf, String factionID) {
		...
		for (Enumeration<FRange> e = F.ranges(); e.hasMoreElements();) {
			...
			String CodeName = (FR.codeName().length() > 0) ? FR.codeName()
					.toUpperCase() : FR.name().toUpperCase();
			if (!hashedFactionRanges.containsKey(CodeName))
				hashedFactionRanges.put(CodeName, FR);
			String SimpleUniqueCodeName = F.name().toUpperCase() + "."
					+ CodeName;
	...
	public String makeFactionFilename(String factionID) {
		...
		if (new CMFile(Resources.makeFileResourceName(filename), null).exists())
			return filename;
		if (new CMFile(Resources.makeFileResourceName(factionID), null)
				.exists())
			return factionID;
		filename = factionID;
	...
	public Faction getFaction(String factionID) {
		...
		CMFile f = new CMFile(
				Resources.makeFileResourceName(makeFactionFilename(factionID)),
	...
	public Faction getFactionByRangeCodeName(String rangeCodeName) {
		if (hashedFactionRanges.containsKey(rangeCodeName.toUpperCase())) {
			Faction.FRange FR = hashedFactionRanges.get(rangeCodeName
					.toUpperCase());
			if (FR != null) {
				Faction F = FR.getFaction();
				if (!F.amDestroyed())
	...
	public boolean removeFaction(String factionID) {
		Faction F;
		if (factionID == null) {
			for (Enumeration<Faction> e = factionSet.elements(); e
					.hasMoreElements();) {
				F = e.nextElement();
				if (F != null)
	...
	public double getRangePercent(String factionID, int faction) {
		...
		if (F == null)
			return 0.0;
		return CMath.div(
				(int) Math.round(CMath.div((faction - F.minimum()),
						(F.maximum() - F.minimum())) * 10000.0), 100.0);
	}
	...
	public boolean postChangeAllFactions(MOB mob, MOB victim, int amount,
			boolean quiet) {
		if ((mob == null))
			return false;
		CMMsg msg = CMClass.getMsg(mob, victim, null, CMMsg.MASK_ALWAYS
				| CMMsg.TYP_FACTIONCHANGE, null, CMMsg.NO_EFFECT, null,
	...
	public boolean postFactionChange(MOB mob, Environmental tool,
			String factionID, int amount) {
		if ((mob == null))
			return false;
		CMMsg msg = CMClass.getMsg(mob, null, tool, CMMsg.MASK_ALWAYS
				| CMMsg.TYP_FACTIONCHANGE, null, CMMsg.NO_EFFECT, null,
				CMMsg.NO_EFFECT, factionID);
		msg.setValue(amount);
		if (mob.location() != null) {
			if (mob.location().okMessage(mob, msg))
				mob.location().send(mob, msg);
	...
	protected Faction makeReactionFaction(String prefix, String classID,
			String Name, String code, String baseTemplateFilename) {
		String codedName = Name.toUpperCase().trim().replace(' ', '_');
		String factionID = prefix + codedName;
		Faction templateF = getFaction(this.makeFactionFilename(codedName
				.toLowerCase()));
		if (templateF == null)
			templateF = getFaction(baseTemplateFilename);
		if (templateF == null) {
			Log.errOut("Factions", "Could not find base template '"
	...
	public Faction[] getSpecialFactions(MOB mob, Room R) {
		if ((mob == null) || (R == null))
			return null;
		Faction F = null;
		String SPECIALTYPE = CMProps.getVar(CMProps.Str.AUTOREACTION)
				.toUpperCase().trim();
		...
		if (SPECIALTYPE.equals("AREA")) {
			Area A = R.getArea();
			if (A != null) {
				String areaCode = A.Name().toUpperCase().trim()
						.replace(' ', '_');
				F = getFaction("AREA_" + areaCode);
				if (F == null)
					F = makeReactionFaction("AREA_", A.ID(), A.Name(),
							areaCode, "examples/areareaction.ini");
				if (F == null)
					return null;
				return new Faction[] { F };
			}
		} else if (SPECIALTYPE.equals("NAME")) {
			Vector<Faction> Fs = new Vector<Faction>();
			for (int i = 0; i < R.numInhabitants(); i++) {
				MOB M = R.fetchInhabitant(i);
				if ((M != null) && (M != mob) && (M.isMonster())) {
					String nameCode = M.Name().toUpperCase().trim()
							.replace(' ', '_');
					F = getFaction("NAME_" + nameCode);
					if (F == null)
						F = makeReactionFaction("NAME_", M.ID(), M.Name(),
								nameCode, "examples/namereaction.ini");
					if (F != null)
						Fs.add(F);
				}
			}
		...
		} else if (SPECIALTYPE.equals("RACE")) {
			Vector<Faction> Fs = new Vector<Faction>();
			HashSet<Race> done = new HashSet<Race>(2);
			for (int i = 0; i < R.numInhabitants(); i++) {
				MOB M = R.fetchInhabitant(i);
				if ((M != null) && (M != mob) && (M.isMonster())
						&& (!done.contains(M.charStats().getMyRace()))) {
					Race rR = M.charStats().getMyRace();
					done.add(rR);
					String nameCode = rR.name().toUpperCase().trim()
							.replace(' ', '_');
					F = getFaction("RACE_" + nameCode);
					if (F == null)
						F = makeReactionFaction("RACE_", rR.ID(), rR.name(),
								nameCode, "examples/racereaction.ini");
					if (F != null)
						Fs.add(F);
				}
			}
	...
	public void updatePlayerFactions(MOB mob, Room R) {
		...
		if (Fs != null)
			for (Faction F : Fs)
				if ((F != null) && (!F.hasFaction(mob))
						&& (F.findAutoDefault(mob) != Integer.MAX_VALUE))
					mob.addFaction(F.factionID(), F.findAutoDefault(mob));
	}

	protected void addOutsidersAndTimers(Faction F,
			Vector<Faction.FactionChangeEvent> outSiders,
			Vector<Faction.FactionChangeEvent> timers) {
		...
		if (CEs != null)
			for (int i = 0; i < CEs.length; i++) {
				if (CEs[i].triggerParameters().length() == 0)
					...
					if (ctr == null) {
						ctr = new int[] { CMath.s_int(CEs[i]
								.getTriggerParm("ROUNDS")) };
	...
	public int getAlignPurity(int faction, Faction.Align eq) {
		...
		case NEUTRAL:
			return Math.abs(getPercent(AlignID(),
					(int) Math.round(CMath.div((top + bottom), 2)))
					- pct);
		default:
			return 0;
		}
	...
	public int isFactionTag(String tag) {
		for (int i = 0; i < Faction.TAG_NAMES.length; i++)
			if (tag.equalsIgnoreCase(Faction.TAG_NAMES[i]))
				return i;
			else if (Faction.TAG_NAMES[i].endsWith("*")
					&& tag.startsWith(Faction.TAG_NAMES[i].substring(0,
							Faction.TAG_NAMES[i].length() - 1)))
				return i;
		return -1;
	}

	public Faction.Align getAlignEnum(String str) {
		Faction.Align A = (Faction.Align) CMath.s_valueOf(Faction.Align.class,
	...
	public void modifyFaction(MOB mob, Faction me) throws IOException {
		...
		while ((mob.session() != null) && (!mob.session().isStopped()) && (!ok)) {
			int showNumber = 0;
			// name
			me.setName(CMLib.genEd().prompt(mob, me.name(), ++showNumber,
					showFlag, "Name"));
			...
			while ((mob.session() != null) && (!mob.session().isStopped())
					&& (!((showFlag > 0) && (showFlag != showNumber)))) {
				StringBuffer list = new StringBuffer(showNumber
						+ ". Faction Division/Ranges List:\n\r");
				list.append(CMStrings.padRight("   Code", 16)
						+ CMStrings.padRight("Name", 21)
						+ CMStrings.padRight("Min", 11)
						+ CMStrings.padRight("Max", 11)
						+ CMStrings.padRight("Align", 6) + "\n\r");
				for (Enumeration<FRange> e = me.ranges(); e.hasMoreElements();) {
					...
					list.append(CMStrings.padRight(FR.alignEquiv().toString(),
							5) + "\n\r");
				}
				mob.tell(list.toString());
				if ((showFlag != showNumber) && (showFlag > -999))
					break;
				String which = mob.session().prompt(
						"Enter a CODE to add, remove, or modify:", "");
				if (which.length() == 0)
					break;
				...
				if (FR == null) {
					if (mob.session().confirm(
							"Create a new range code named '" + which
									+ "' (y/N): ", "N")) {
						FR = me.addRange("0;100;Change My Name;" + which + ";");
					}
				} else if (mob
						.session()
						.choose("Would you like to M)odify or D)elete this range (M/d): ",
								"MD", "M").toUpperCase().startsWith("D")) {
					me.delRange(FR);
				...
				if (FR != null) {
					String newName = mob.session().prompt(
							"Enter a new name (" + FR.name() + ")\n\r: ",
							FR.name());
					boolean error99 = false;
					if (newName.length() == 0)
						error99 = true;
					else
						for (Enumeration<FRange> e = me.ranges(); e
								.hasMoreElements();) {
							Faction.FRange FR3 = e.nextElement();
							if (FR3.name().equalsIgnoreCase(FR.name())
									&& (FR3 != FR)) {
								mob.tell("A range already exists with that name!");
								error99 = true;
								break;
							}
						}
					...
					newName = mob.session().prompt(
							"Enter the low end of the range (" + FR.low()
									+ ")\n\r: ", "" + FR.low());
					...
					newName = mob.session().prompt(
							"Enter the high end of the range (" + FR.high()
									+ ")\n\r: ", "" + FR.high());
					if ((!CMath.isInteger(newName))
							|| (CMath.s_int(newName) < FR.low()))
						mob.tell("(no change)");
					else
						FR.setHigh(CMath.s_int(newName));
					StringBuffer prompt = new StringBuffer(
							"Select the 'virtue' (if any) of this range:\n\r");
					...
					for (Faction.Align i : Faction.Align.values()) {
						...
						if (i == Faction.Align.INDIFF)
							...
							prompt.append(i.ordinal() + ") "
									+ i.toString().toLowerCase() + "\n\r");
					}
					FR.setAlignEquiv(Faction.Align.values()[CMath.s_int(mob
							.session()
							.choose(prompt.toString()
									+ "Enter alignment equivalency or 0: ",
									choices.toString(),
									"" + FR.alignEquiv().ordinal()))]);
				}
			}

			// show in score
			me.setShowInScore(CMLib.genEd().prompt(mob, me.showInScore(),
					++showNumber, showFlag, "Show in 'Score'"));

			// show in factions
			me.setShowInFactionsCommand(CMLib.genEd().prompt(mob,
					me.showInFactionsCommand(), ++showNumber, showFlag,
			...
			if (!alreadyReporter)
				me.setShowInSpecialReported(CMLib.genEd().prompt(mob,
						me.showInSpecialReported(), ++showNumber, showFlag,
						"Show in Reports"));

			// show in editor
			me.setShowInEditor(CMLib.genEd().prompt(mob, me.showInEditor(),
					++showNumber, showFlag, "Show in MOB Editor"));

			// auto defaults
			boolean error = true;
			me.setAutoDefaults(CMParms.parseSemicolons(
					CMLib.genEd()
							.prompt(mob,
									CMParms.toSemicolonList(me.autoDefaults()),
									++showNumber,
									showFlag,
									"Optional automatic assigned values with zapper masks (semicolon delimited).\n\r    "),
					true));
			...
			++showNumber;
			while (error && (mob.session() != null)
					&& (!mob.session().isStopped())) {
				error = false;
				String newDefaults = CMLib
						.genEd()
						.prompt(mob, CMParms.toSemicolonList(me.defaults()),
								showNumber, showFlag,
								"Other default values with zapper masks (semicolon delimited).\n\r    ");
				if ((showFlag != showNumber) && (showFlag > -999))
					break;
			...
			me.setChoices(CMParms.parseSemicolons(
					CMLib.genEd()
							.prompt(mob, CMParms.toSemicolonList(me.choices()),
									++showNumber, showFlag,
									"Optional new player value choices (semicolon-delimited).\n\r    "),
					true));
			if (me.choices().hasMoreElements())
				me.setChoiceIntro(CMLib.genEd().prompt(mob, me.choiceIntro(),
						++showNumber, showFlag,
						"Optional choices introduction text. Filename"));

			// rate modifier
			String newModifier = CMLib.genEd().prompt(mob,
					CMath.toPct(me.rateModifier()), ++showNumber, showFlag,
					"Rate modifier");
			...
			while (error2 && (mob.session() != null)
				...
				for (int i = 0; i < Faction.EXPAFFECT_NAMES.length; i++) {
					if (me.experienceFlag().equalsIgnoreCase(
							Faction.EXPAFFECT_NAMES[i]))
						myval = i;
					nextPrompt
							.append("  "
									+ (i + 1)
									+ ") "
									+ CMStrings
											.capitalizeAndLower(Faction.EXPAFFECT_NAMES[i]
													.toLowerCase()) + "\n\r");
				}
				if (myval < 0) {
				...
				if ((showFlag != showNumber) && (showFlag > -999)) {
					mob.tell(showNumber + ". Affect on experience: "
							+ Faction.EXPAFFECT_NAMES[myval]);
					break;
				}
				String prompt = "Affect on experience:  "
						+ Faction.EXPAFFECT_NAMES[myval]
						+ nextPrompt.toString() + "\n\rSelect a value: ";
				int mynewval = CMLib.genEd().prompt(mob, myval + 1, showNumber,
						showFlag, prompt);
				if ((showFlag != showNumber) && (showFlag > -999))
					break;
				if ((mynewval <= 0)
						|| (mynewval > Faction.EXPAFFECT_NAMES.length)) {
			...
			while ((mob.session() != null) && (!mob.session().isStopped())
				...
				StringBuffer list = new StringBuffer(
						showNumber
								+ ". Faction change adjustment Factors with Zapper Masks:\n\r");
				list.append("    #) " + CMStrings.padRight("Zapper Mask", 31)
						+ CMStrings.padRight("Gain", 6)
						+ CMStrings.padRight("Loss", 6) + "\n\r");
				StringBuffer choices = new StringBuffer("");
				int numFactors = 0;
				for (Enumeration<Faction.FZapFactor> e = me.factors(); e
						.hasMoreElements();) {
					...
					list.append(CMStrings.padRight(
							"" + CMath.toPct(factor.gainFactor()), 5)
							+ " ");
					list.append(CMStrings.padRight(
							"" + CMath.toPct(factor.lossFactor()), 5)
							+ "\n\r");
					numFactors++;
				}
				mob.tell(list.toString());
				...
				String which = mob
						.session()
						.choose("Enter a # to remove, or modify, or enter 0 to Add:",
								"0" + choices.toString(), "").trim()
						.toUpperCase();
				int factorNum = choices.toString().indexOf(which);
				if ((which.length() != 1)
						|| ((!which.equalsIgnoreCase("0")) && ((factorNum < 0) || (factorNum >= numFactors))))
					break;
				...
				if (factor != null) {
					String mask = mob.session().prompt(
							"Enter a new zapper mask (" + factor.MOBMask()
									+ ")\n\r: ", factor.MOBMask());
					double newHigh = factor.gainFactor();
					String newName = mob.session().prompt(
							"Enter gain adjustment (" + CMath.toPct(newHigh)
									+ "): ", CMath.toPct(newHigh));
					...
					newName = mob.session().prompt(
							"Enter loss adjustment (" + CMath.toPct(newLow)
									+ "): ", CMath.toPct(newLow));
					if ((!CMath.isNumber(newName)) && (!CMath.isPct(newName)))
						mob.tell("(no change)");
					else
			...
			while ((mob.session() != null) && (!mob.session().isStopped())
				...
				list.append("    Faction" + CMStrings.padRight("", 25)
						+ "Percentage change\n\r");
				for (Enumeration<String> e = me.relationFactions(); e
						.hasMoreElements();) {
				...
				if ((showFlag != showNumber) && (showFlag > -999))
					break;
				String which = mob.session().prompt(
						"Enter a faction to add, remove, or modify relations:",
						"");
				if (which.length() == 0)
					break;
				Faction theF = null;
				...
				if (theF == null) {
					...
					else if (mob.session().confirm(
							"Create a new relation for faction  '"
									+ possibleF.name() + "' (y/N):", "N")) {
						theF = possibleF;
						me.addRelation(theF.factionID(), 1.0);
					}
				} else if (mob
						.session()
						.choose("Would you like to M)odify or D)elete this relation (M/d): ",
								"MD", "M").toUpperCase().startsWith("D")) {
				...
				if (theF != null) {
					String amount = CMath
							.toPct(me.getRelation(theF.factionID()));
					String newName = mob.session().prompt(
							"Enter a relation amount (" + amount + "): ",
							"" + amount);
					if ((!CMath.isNumber(newName)) && (!CMath.isPct(newName)))
						mob.tell("(no change)");
					me.delRelation(theF.factionID());
			...
			while ((mob.session() != null) && (!mob.session().isStopped())
				...
				list.append("    " + CMStrings.padRight("Type", 15) + " "
						...
						+ CMStrings.padRight("Flags", 20) + " Mask\n\r");
				int numChanges = 0;
				StringBuffer choices = new StringBuffer("");
				Hashtable<Character, Faction.FactionChangeEvent> choicesHashed = new Hashtable<Character, Faction.FactionChangeEvent>();
				for (Enumeration<String> e = me.changeEventKeys(); e
						.hasMoreElements();) {
					Faction.FactionChangeEvent[] CEs = me.getChangeEvents(e
							.nextElement());
					if (CEs != null) {
						for (int e1 = 0; e1 < CEs.length; e1++) {
							...
							list.append(" "
									+ (((char) ('A' + numChanges)) + ") "));
							choicesHashed.put(Character
									.valueOf((char) ('A' + numChanges)), CE);
							if (CE.triggerParameters().trim().length() == 0)
								list.append(CMStrings.padRight(CE.eventID(), 15)
										+ " ");
							else
								list.append(CMStrings.padRight(CE.eventID()
										+ ":" + CE.triggerParameters(), 15)
										+ " ");
							list.append(CMStrings
									.padRight(
											Faction.FactionChangeEvent.CHANGE_DIRECTION_DESCS[CE
													.direction()], 10)
									+ " ");
							list.append(CMStrings.padRight(
									CMath.toPct(CE.factor()), 10)
									+ " ");
				...
				if ((showFlag != showNumber) && (showFlag > -999))
					break;
				String which = mob.session().prompt(
						"Select a ID to add, remove, or modify:", "");
				which = which.toUpperCase().trim();
				if (which.length() == 0)
					continue;
				Faction.FactionChangeEvent CE = (which.length() > 0) ? choicesHashed
						.get(Character.valueOf(which.charAt(0))) : null;
				if (CE == null) {
					String newID = mob.session()
							.prompt("Enter a new change ID (?): ")
							.toUpperCase().trim();
					if (newID.length() == 0)
						break;
					if (newID.equalsIgnoreCase("?")) {
						mob.tell("Valid triggers: \n\r"
								+ me.ALL_CHANGE_EVENT_TYPES());
						continue;
					}
					...
					if (CE == null) {
						...
						continue;
							...
							"Create a new trigger using ID '" + newID
									+ "' (y/N): ", "N")) {
						...
						.session()
						.choose("Would you like to M)odify or D)elete this trigger (M/d): ",
								"MD", "M").toUpperCase().startsWith("D")) {
					me.delChangeEvent(CE);
				...
				if (CE != null) {
					String newFlags = mob.session().prompt(
							"Trigger parms (" + CE.triggerParameters() + "): ",
							CE.triggerParameters());
					if ((newFlags.length() == 0)
							|| (newFlags.equals(CE.triggerParameters())))
						mob.tell("(no change)");
					else
				...
				if (CE != null) {
					...
					for (int i = 0; i < Faction.FactionChangeEvent.CHANGE_DIRECTION_DESCS.length; i++) {
						directions
								.append(((char) ('A' + i))
										+ ") "
										+ Faction.FactionChangeEvent.CHANGE_DIRECTION_DESCS[i]
										+ "\n\r");
					...
					String str = mob
							...
							.choose(directions
									...
									+ Faction.FactionChangeEvent.CHANGE_DIRECTION_DESCS[CE.direction()]
									+ "): ", cmds.toString() + "\n\r", "");
					if ((str.length() == 0) || str.equals("\n")
							|| str.equals("\r")
							|| (cmds.toString().indexOf(str.charAt(0)) < 0))
						mob.tell("(no change)");
					else
				...
				if (CE != null) {
					...
					String newName = mob.session().prompt(
							"Enter the amount factor (" + amount + "): ",
							"" + amount);
					if ((!CMath.isNumber(newName)) && (!CMath.isPct(newName)))
						mob.tell("(no change)");
					else
				...
				if (CE != null) {
					mob.tell("Valid flags include: "
							+ CMParms
									.toStringList(Faction.FactionChangeEvent.FLAG_DESCS)
							+ "\n\r");
					String newFlags = mob.session().prompt(
							"Enter new flag(s) (" + CE.flagCache() + "): "
									+ CE.flagCache(), CE.flagCache());
					if ((newFlags.length() == 0)
							|| (newFlags.equals(CE.flagCache())))
						mob.tell("(no change)");
					else
				...
				if (CE != null) {
					String newFlags = mob.session().prompt(
							"Zapper mask (" + CE.targetZapper() + "): "
									+ CE.targetZapper(), CE.targetZapper());
					if ((newFlags.length() == 0)
							|| (newFlags.equals(CE.targetZapper())))
						mob.tell("(no change)");
					else
			...
			while ((mob.session() != null) && (!mob.session().isStopped())
				...
				StringBuffer list = new StringBuffer(showNumber
						+ ". Ability allowances:\n\r");
				list.append("    #) " + CMStrings.padRight("Ability masks", 40)
						+ " " + CMStrings.padRight("Low value", 10) + " "
						+ CMStrings.padRight("High value", 10) + "\n\r");
				int numUsages = 0;
				StringBuffer choices = new StringBuffer("0\n\r");
				for (Enumeration<Faction.FAbilityUsage> e = me.abilityUsages(); e
						.hasMoreElements();) {
				...
				String which = mob
						.session()
						.choose("Select an allowance to remove or modify, or enter 0 to Add:",
								choices.toString(), "");
				if (which.length() != 1)
					break;
				which = which.toUpperCase().trim();
				...
				if (!which.equalsIgnoreCase("0")) {
					...
					if (mob.session()
							.choose("Would you like to M)odify or D)elete this allowance (M/d): ",
									"MD", "M").toUpperCase().startsWith("D")) {
				...
				if (CA != null) {
					...
					while ((!cont) && (!mob.session().isStopped())) {
						String newFlags = mob.session().prompt(
								"Ability determinate masks or ? ("
										+ CA.abilityFlags() + "): "
										+ CA.abilityFlags(), CA.abilityFlags());
						if (newFlags.equalsIgnoreCase("?")) {
							...
							cont = true;
							if ((newFlags.length() == 0)
									|| (newFlags.equals(CA.abilityFlags())))
								mob.tell("(no change)");
							else {
								List<String> unknowns = CA
										.setAbilityFlag(newFlags);
								if (unknowns.size() > 0)
									for (int i = unknowns.size() - 1; i >= 0; i--)
										if (CMClass.getAbility(unknowns.get(i)) != null)
											unknowns.remove(i);
								if (unknowns.size() > 0) {
									mob.tell("The following are unknown masks: '"
											+ CMParms.toStringList(unknowns)
											+ "'.  Please correct them.");
									cont = false;
								}
							}
					...
					String newName = mob.session().prompt(
							"Enter the minimum value to use the ability ("
									+ CA.low() + "): ", "" + CA.low());
					if ((!CMath.isInteger(newName))
							|| (CA.low() == CMath.s_int(newName)))
						mob.tell("(no change)");
					else
						CA.setLow(CMath.s_int(newName));
					newName = mob.session().prompt(
							"Enter the maximum value to use the ability ("
									+ CA.high() + "): ", "" + CA.high());
					if ((!CMath.isInteger(newName))
							|| (CA.high() == CMath.s_int(newName)))
						mob.tell("(no change)");
					else
			...
			while ((mob.session() != null) && (!mob.session().isStopped())
				...
				String ID = null;
				for (Enumeration<String> e = me.affectsBehavs(); e
						.hasMoreElements();) {
					ID = e.nextElement();
				...
				String which = mob
						.session()
						.choose("Select an ability/behavior ID to remove or modify, or enter 0 to Add:",
								choices.toString(), "");
				if (which.length() != 1)
					break;
				which = which.toUpperCase().trim();
				if (!which.equalsIgnoreCase("0")) {
					...
					if (mob.session()
							.choose("Would you like to M)odify or D)elete this "
									+ type + " (M/d): ", "MD", "M")
							.toUpperCase().startsWith("D")) {
						me.delAffectBehav(ID);
						mob.tell(CMStrings.capitalizeAndLower(type)
								+ " deleted.");
						ID = null;
					}
				} else {
					...
					while ((cont) && (!mob.session().isStopped())) {
						cont = false;
						ID = mob.session().prompt(
								"Enter a new Ability or Behavior ID or ?: ");
						if (ID.equalsIgnoreCase("?")) {
							StringBuffer vals = new StringBuffer(
									"Valid IDs: \n\r");
							vals.append(CMParms.toCMObjectStringList(CMClass
									.abilities()));
							vals.append(CMParms.toCMObjectStringList(CMClass
									.behaviors()));
							mob.tell(vals.toString());
							...
							if (type == null) {
								mob.tell("'"
										+ ID
										+ " is neither a valid behavior ID or ability ID.  Use ? for a list.");
								cont = true;
							} else if (!mob.session().confirm(
									"Create a new " + type + " (y/N): ", "N")) {
								ID = null;
								break;
							} else
				...
				if (ID != null) {
					...
					while ((cont) && (!mob.session().isStopped())) {
						cont = false;
						String mask = mob.session().prompt(
								"Enter a new Zapper Mask or ? (" + oldData[1]
										+ ")\n\r: ", oldData[1]);
					...
					while ((cont) && (!mob.session().isStopped())) {
						cont = false;
						String parms = mob.session().prompt(
								"Enter new " + type + " parameters for " + ID
										+ " or ? (" + oldData[0] + ")\n\r: ",
								oldData[0]);
						if (parms.equalsIgnoreCase("?")) {
							mob.tell(CMLib.help().getHelpText(ID, mob, true)
									.toString());
							cont = true;
			...
			while ((mob.session() != null) && (!mob.session().isStopped())
				...
				if ((showFlag > 0) && (showFlag != showNumber))
					break;
				StringBuffer list = new StringBuffer(showNumber
						+ ". Reaction Commands/Effects/Behaviors:\n\r");
				list.append("    #) " + CMStrings.padRight("Range", 15) + " "
				...
				for (Enumeration<Faction.FReactionItem> e = me.reactions(); e
						.hasMoreElements();) {
					item = e.nextElement();
					list.append("    " + ((char) ('A' + numReactions) + ") "));
					list.append(CMStrings.padRight(item.rangeCodeName(), 15)
							+ " ");
					list.append(CMStrings.padRight(item.presentMOBMask() + "",
							18) + " ");
					list.append(CMStrings.padRight(
							item.reactionObjectID() + "", 15) + " ");
				...
				if ((showFlag != showNumber) && (showFlag > -999))
					break;
				String which = mob.session().choose(
						"Select one to remove or modify, or enter 0 to Add:",
						choices.toString(), "");
				if (which.length() != 1)
					break;
				which = which.toUpperCase().trim();
				...
				if (item != null) {
					...
					oldData = new String[] { item.rangeCodeName(),
							item.presentMOBMask(), item.reactionObjectID(),
							item.parameters() };
				}
				...
				while ((cont) && (!mob.session().isStopped())) {
					...
					String rangeCode = mob
							.session()
							.prompt("Enter a new range code or ? ("
									+ oldData[0] + ")\n\r: ", oldData[0])
							.toUpperCase().trim();
					if (rangeCode.equalsIgnoreCase("?")) {
						StringBuffer str = new StringBuffer("");
						for (Enumeration<Faction.FRange> e = me.ranges(); e
								.hasMoreElements();) {
							Faction.FRange FR = e.nextElement();
						...
						for (Enumeration<Faction.FRange> e = me.ranges(); e
								.hasMoreElements();)
							if (e.nextElement().codeName()
									.equalsIgnoreCase(rangeCode)) {
								newData[0] = rangeCode;
								cont = false;
							}
						if (cont)
							mob.tell("'" + rangeCode
									+ "' is not a valid range code.  Use ?");
					}
				}
				...
				while ((cont) && (!mob.session().isStopped())) {
					cont = false;
					String ID = mob.session().prompt(
							"Enter a new Ability, Behavior, or Command ID ("
									+ oldData[2] + ")\n\r: ", oldData[2]);
					if (ID.equalsIgnoreCase("?")) {
						...
						if (type == null) {
							mob.tell("'"
									+ ID
									+ " is neither a valid behavior, command, ability ID. Use ? for a list.");
							cont = true;
						} else
							newData[2] = ID;
					}
				...
				while ((cont) && (!mob.session().isStopped())) {
					cont = false;
					String parms = mob.session().prompt(
							"Enter new " + type + " parameters for "
									+ newData[2] + " or ? (" + oldData[3]
									+ ")\n\r: ", oldData[3]);
					if (parms.equalsIgnoreCase("?")) {
						mob.tell(CMLib.help()
								.getHelpText(newData[3], mob, true).toString());
						cont = true;
				...
				for (int n = 0; n < oldData.length; n++)
					if (newData[n] == null)
						newData[n] = oldData[n];
				if (item == null)
					me.addReaction(newData[0], newData[1], newData[2],
							newData[3]);
				else {
			...
			if (me.reactions().hasMoreElements())
				me.setLightReactions(CMLib.genEd().prompt(mob,
						me.useLightReactions(), ++showNumber, showFlag,
						"Use 'Light' Reactions"));
			else
	...
	private StringBuffer rebuildFactionProperties(Faction F) {
		List<String> oldV = Resources.getFileLineVector(Resources
				.getFileResource(makeFactionFilename(F.factionID()), true));
		if (oldV.size() < 10) {
		...
		for (int v = 0; v < oldV.size(); v++) {
			String s = oldV.get(v);
			if (!(s.trim().startsWith("#") || s.trim().length() == 0 || (s
					.indexOf('=') < 0))) {
				String tag = s.substring(0, s.indexOf('=')).trim()
						.toUpperCase();
		...
		for (int v = 0; v < oldV.size(); v++) {
			...
			if (s.trim().length() == 0) {
				if ((lastCommented >= 0) && (!done[lastCommented])) {
					...
					buf.append(F
							.getINIDef(Faction.TAG_NAMES[lastCommented], CR)
							+ CR);
					lastCommented = -1;
				}
				...
				if (x >= 0) {
					...
					for (; first >= 0; first--)
						if (!Character.isLetterOrDigit(s.charAt(first)))
							break;
					first = CMLib.factions().isFactionTag(
							s.substring(first).trim().toUpperCase());
					if (first >= 0)
						lastCommented = first;
				}
			} else {
	...
	public String resaveFaction(Faction F) {
		if ((F.factionID().length() > 0)
			...
			if (!CMath.bset(F.getInternalFlags(), Faction.IFLAG_NEVERSAVE)) {
				...
				if (!Resources.updateFileResource(
						makeFactionFilename(F.factionID()), buf))
					return "Faction File '"
							+ F.factionID()
							+ "' could not be modified.  Make sure it is not READ-ONLY.";
			}
		} else
