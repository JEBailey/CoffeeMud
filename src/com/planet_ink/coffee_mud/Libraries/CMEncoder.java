...
public class CMEncoder extends StdLibrary implements TextEncoders {
	...
	public synchronized String decompressString(byte[] b) {
		try {
			...
			synchronized (encodeBuffer) {
				...
				return new String(encodeBuffer, 0, len,
						CMProps.getVar(CMProps.Str.CHARSETINPUT));
			}
		} catch (Exception ex) {
			Log.errOut(Thread.currentThread().getName(),
					"Error occurred during decompression: " + ex.getMessage());
	...
	public synchronized byte[] compressString(String s) {
		...
		try {
			compresser.reset();
			compresser.setInput(s.getBytes(CMProps
			...
			Log.errOut("MUD",
					"Error occurred during compression: " + ex.getMessage());
			encodeBuffer = new byte[65536];
		}
	...
	public String makeRandomHashString(final String password) {
		...
		return "|"
				+ B64Encoder.B64encodeBytes(ByteBuffer.allocate(4).putInt(salt)
						.array())
				+ "|"
				+ B64Encoder.B64encodeBytes(ByteBuffer.allocate(4)
						.putInt(passHash).array());
	}

	public boolean isARandomHashString(final String password) {
		return ((password.length() > 2) && (password.startsWith("|")) && (password
				.indexOf('|', 1) > 1));
	}

	public boolean checkAgainstRandomHashString(final String checkString,
			final String hashString) {
		int hashDex = hashString.indexOf('|', 1);
		int salt = ByteBuffer.wrap(
				B64Encoder.B64decode(hashString.substring(1, hashDex)))
				.getInt();
		int hash = ByteBuffer.wrap(
				B64Encoder.B64decode(hashString.substring(hashDex + 1)))
				.getInt();
