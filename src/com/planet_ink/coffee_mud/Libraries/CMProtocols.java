...
public class CMProtocols extends StdLibrary implements ProtocolLibrary {
	...
	public String msp(final String soundName, final int volume,
			final int priority) {
	...
	public String mxpImage(final Environmental E, final String parms) {
		...
		if (fixedFilenames[0].length() == 0)
			return "";
		return "^<IMAGE '" + fixedFilenames[1] + "' URL=\"" + fixedFilenames[0]
				+ "\" " + parms + "^>^N";
	}
	...
	public String mxpImage(final Environmental E, final String parms,
		...
		if (fixedFilenames[0].length() == 0)
			return "";
		return pre + "^<IMAGE '" + fixedFilenames[1] + "' URL=\""
				+ fixedFilenames[0] + "\" " + parms + "^>^N" + post;
	}
	...
	public String getHashedMXPImage(final Map<String, String> H,
			final String key) {
		if (H == null)
			return "";
	...
	public String getDefaultMXPImage(final Object O) {
		if ((CMProps.getVar(Str.MXPIMAGEPATH).length() == 0)
				|| (CMSecurity.isDisabled(CMSecurity.DisFlag.MXP)))
			return "";
		Map<String, String> H = (Map) Resources
				.getResource("PARSED: mxp_images.ini");
		if (H == null) {
			H = new Hashtable<String, String>();
			List<String> V = Resources.getFileLineVector(new CMFile(
					"resources/mxp_images.ini", null).text());
			if ((V != null) && (V.size() > 0)) {
				String s = null;
				int x = 0;
		...
		if (O instanceof Race) {
			image = getHashedMXPImage(H, "RACE_"
					+ ((Race) O).ID().toUpperCase());
			if (image == null)
				image = getHashedMXPImage(H,
						"RACECAT_"
								+ ((Race) O).racialCategory().toUpperCase()
										.replace(' ', '_'));
			if (image == null)
				image = getHashedMXPImage(H, "RACE_*");
			if (image == null)
			...
			image = getHashedMXPImage(H, "ROOM_"
					+ ((Room) O).ID().toUpperCase());
			if (image == null)
				if (CMath.bset(((Room) O).domainType(), Room.INDOORS))
					image = getHashedMXPImage(H, "LOCALE_INDOOR_"
							+ Room.indoorDomainDescs[((Room) O).domainType()
									- Room.INDOORS]);
				else
					image = getHashedMXPImage(H, "LOCALE_"
							+ Room.outdoorDomainDescs[((Room) O).domainType()]);
			if (image == null)
				image = getHashedMXPImage(H, "ROOM_*");
			if (image == null)
				image = getHashedMXPImage(H, "LOCALE_*");
			...
			image = getHashedMXPImage(H, "EXIT_"
					+ ((Exit) O).ID().toUpperCase());
			if (image == null)
				image = getHashedMXPImage(H, "EXIT_"
						+ ((Exit) O).doorName().toUpperCase());
			if (image == null)
				if (((Exit) O).hasADoor())
			...
			image = getHashedMXPImage(H, "RIDEABLE_"
					+ Rideable.RIDEABLE_DESCS[((Rideable) O).rideBasis()]);
			if (image == null)
				image = getHashedMXPImage(H, "RIDEABLE_*");
		} else if (O instanceof Shield) {
			image = getHashedMXPImage(
					H,
					"SHIELD_"
							+ RawMaterial.Material.findByMask(
									((Shield) O).material()
											& RawMaterial.MATERIAL_MASK).desc());
			if (image == null)
				image = getHashedMXPImage(H, "SHIELD_*");
		} else if (O instanceof Coins) {
			image = getHashedMXPImage(H,
					"COINS_" + RawMaterial.CODES.NAME(((Coins) O).material()));
			if (image == null)
				image = getHashedMXPImage(H, "COINS_*");
		} else if (O instanceof Ammunition) {
			image = getHashedMXPImage(H,
					"AMMO_"
							+ ((Ammunition) O).ammunitionType().toUpperCase()
									.replace(' ', '_'));
			if (image == null)
			...
			image = getHashedMXPImage(H, "CLAN_"
					+ ((ClanItem) O).ID().toUpperCase());
			if (image == null)
				image = getHashedMXPImage(
						H,
						"CLAN_"
								+ ClanItem.CI_DESC[((ClanItem) O).ciType()]
										.toUpperCase());
			if (image == null)
				image = getHashedMXPImage(H, "CLAN_*");
			...
			image = getHashedMXPImage(
					H,
					"RESOURCE_"
							+ RawMaterial.CODES.NAME(((RawMaterial) O)
									.material()));
			if (image == null)
				image = getHashedMXPImage(
						H,
			...
			image = getHashedMXPImage(H,
					"KEY_" + RawMaterial.CODES.NAME(((DoorKey) O).material()));
			image = getHashedMXPImage(
					H,
					"KEY_"
							+ RawMaterial.Material.findByMask(
			...
			image = getHashedMXPImage(
					H,
					"MUSINSTR_"
							+ MusicalInstrument.TYPE_DESC[((MusicalInstrument) O)
									.instrumentType()]);
			if (image == null)
			...
			if (image == null)
				image = getHashedMXPImage(H, "SCROLL_*");
		} else if (O instanceof Electronics.PowerGenerator) {
			final String key = "POWERGENERATOR_"
					+ ((Electronics) O).ID().toUpperCase();
			if (H.containsKey(key))
				image = getHashedMXPImage(H, "POWERGENERATOR_" + key);
			else
				image = getHashedMXPImage(H, "POWERGENERATOR");
		} else if (O instanceof Electronics.PowerSource) {
			final String key = "POWERSOURCE_"
					+ ((Electronics) O).ID().toUpperCase();
			if (H.containsKey(key))
				image = getHashedMXPImage(H, key);
			else
				image = getHashedMXPImage(H, "POWERSOURCE");
		} else if (O instanceof Electronics.ElecPanel) {
			final String key = "ELECPANEL_"
					+ ((Electronics) O).ID().toUpperCase();
			if (H.containsKey(key))
				image = getHashedMXPImage(H, key);
			else if (H.containsKey(((Electronics.ElecPanel) O).panelType()
					.toString()))
				image = getHashedMXPImage(H, ((Electronics.ElecPanel) O)
						.panelType().toString());
			else if (H.containsKey(((Electronics.ElecPanel) O).getTechType()
					.toString()))
				image = getHashedMXPImage(H, ((Electronics.ElecPanel) O)
						.getTechType().toString());
			else
				image = getHashedMXPImage(H, "ELECPANEL");
		} else if (O instanceof ShipComponent) {
			final String key = "SHIPCOMP_"
					+ ((ShipComponent) O).ID().toUpperCase();
			if (H.containsKey(key))
				image = getHashedMXPImage(H, key);
			else
				image = getHashedMXPImage(H, ((ShipComponent) O).getTechType()
						.toString());
			if (image == null)
			...
			Armor A = (Armor) O;
			final long[] bits = { Wearable.WORN_TORSO, Wearable.WORN_FEET,
					Wearable.WORN_LEGS, Wearable.WORN_HANDS,
					Wearable.WORN_ARMS, Wearable.WORN_HEAD, Wearable.WORN_EARS,
					Wearable.WORN_EYES, Wearable.WORN_MOUTH,
					Wearable.WORN_NECK, Wearable.WORN_LEFT_FINGER,
					Wearable.WORN_LEFT_WRIST, Wearable.WORN_BACK,
					Wearable.WORN_WAIST, Wearable.WORN_ABOUT_BODY,
					Wearable.WORN_FLOATING_NEARBY, Wearable.WORN_HELD,
					Wearable.WORN_WIELD };
			final String[] bitdesc = { "TORSO", "FEET", "LEGS", "HANDS",
					"ARMS", "HEAD", "EARS", "EYES", "MOUTH", "NECK", "FINGERS",
					"WRIST", "BACK", "WAIST", "BODY", "FLOATER", "HELD",
					"WIELDED" };
			...
			image = getHashedMXPImage(H, "WEAPON_"
					+ Weapon.CLASS_DESCS[((Weapon) O).weaponClassification()]);
			if (image == null)
				image = getHashedMXPImage(H, "WEAPON_"
						+ Weapon.TYPE_DESCS[((Weapon) O).weaponType()]);
			if (O instanceof AmmunitionWeapon)
				if (image == null)
					image = getHashedMXPImage(H, "WEAPON_"
							+ ((AmmunitionWeapon) O).ammunitionType()
									.toUpperCase().replace(' ', '_'));
			if (image == null)
				image = getHashedMXPImage(H, "WEAPON_*");
		} else if (O instanceof Wand) {
			image = getHashedMXPImage(H, "WAND_"
					+ ((Wand) O).ID().toUpperCase());
			if (image == null) {
			...
			image = getHashedMXPImage(H, "FOOD_"
					+ ((Food) O).ID().toUpperCase());
			if (image == null)
				image = getHashedMXPImage(H,
						"FOOD_" + RawMaterial.CODES.NAME(((Food) O).material()));
			if (image == null)
				image = getHashedMXPImage(
						H,
						"FOOD_"
			...
			image = getHashedMXPImage(H, "DRINK_"
					+ ((Drink) O).ID().toUpperCase());
			if (image == null)
				image = getHashedMXPImage(
						H,
						"DRINK_"
								+ RawMaterial.CODES.NAME(((Item) O).material()));
			if (image == null)
				image = getHashedMXPImage(
						H,
						"DRINK_"
			...
			image = getHashedMXPImage(H, "CONTAINER_"
					+ ((Container) O).ID().toUpperCase());
		...
		if ((image == null) && (O instanceof Item)) {
			image = getHashedMXPImage(H, "ITEM_"
					+ ((Item) O).ID().toUpperCase());
			if (image == null)
				image = getHashedMXPImage(H,
						"ITEM_" + RawMaterial.CODES.NAME(((Item) O).material()));
			if (image == null)
				image = getHashedMXPImage(
						H,
						"ITEM_"
		...
		if (o instanceof StringBuilder)
			...
			for (Object key : ((Map) o).keySet())
				if (key instanceof StringBuilder)
					newO.put(((StringBuilder) key).toString().toUpperCase(),
							msdpStringify(((Map) o).get(key)));
			return newO;
	...
	protected enum MSDPVariable {
		ACCOUNT_NAME, CHARACTER_NAME, SERVER_ID, SERVER_TIME, SPECIFICATION, AFFECTS, ALIGNMENT, EXPERIENCE, EXPERIENCE_MAX, EXPERIENCE_TNL, EXPERIENCE_TNL_MAX, HEALTH, HEALTH_MAX, LEVEL, MANA, MANA_MAX, MONEY, MOVEMENT, MOVEMENT_MAX, OPPONENT_LEVEL, OPPONENT_HEALTH, OPPONENT_HEALTH_MAX, OPPONENT_NAME, OPPONENT_STRENGTH, WORLD_TIME, ROOM, LOCATION, ROOM_NAME, ROOM_VNUM, ROOM_AREA, ROOM_TERRAIN, ROOM_EXITS
	}

	protected enum MSDPConfigurableVar {
	}

	protected Object getMsdpComparable(final Session session,
			final MSDPVariable var) {
		...
		case ALIGNMENT:
			if (M != null) {
				Faction.FRange FR = CMLib.factions().getRange(
						...
						M.fetchFaction(CMLib.factions().AlignID()));
				if (FR != null)
					return FR.name();
			}
			break;
		...
		case MONEY:
			if (M != null)
				return Double.valueOf(CMLib.beanCounter()
						.getTotalAbsoluteNativeValue(M));
			break;
		case MOVEMENT:
			if (M != null)
	...
	protected byte[] processMsdpSend(final Session session, final String var)
			throws UnsupportedEncodingException, IOException {
		final MSDPVariable type = (MSDPVariable) CMath.s_valueOf(
				MSDPVariable.class, var.toUpperCase().trim());
		...
		case ACCOUNT_NAME:
			if ((M != null) && (M.playerStats() != null)) {
				if (M.playerStats().getAccount() != null)
					buf.write(M.playerStats().getAccount().accountName()
							.getBytes(Session.MSDP_CHARSET));
				else
		...
		case EXPERIENCE:
			if (M != null)
				buf.write(Integer.toString(M.getExperience()).getBytes(
						Session.MSDP_CHARSET));
			break;
		case EXPERIENCE_MAX:
			if (M != null)
				buf.write(Integer.toString(M.getExpNextLevel()).getBytes(
						Session.MSDP_CHARSET));
			break;
		case EXPERIENCE_TNL:
			if (M != null)
				buf.write(Integer.toString(M.getExpNeededLevel()).getBytes(
						Session.MSDP_CHARSET));
			break;
		case EXPERIENCE_TNL_MAX:
			if (M != null)
		...
		case HEALTH:
			if (M != null)
				buf.write(Integer.toString(M.curState().getHitPoints())
						.getBytes(Session.MSDP_CHARSET));
			break;
		case HEALTH_MAX:
			if (M != null)
				buf.write(Integer.toString(M.maxState().getHitPoints())
						.getBytes(Session.MSDP_CHARSET));
			break;
		case LEVEL:
			if (M != null)
				buf.write(Integer.toString(M.phyStats().level()).getBytes(
						Session.MSDP_CHARSET));
			break;
		case MANA:
			if (M != null)
				buf.write(Integer.toString(M.curState().getMana()).getBytes(
						Session.MSDP_CHARSET));
			break;
		case MANA_MAX:
			if (M != null)
				buf.write(Integer.toString(M.maxState().getMana()).getBytes(
						Session.MSDP_CHARSET));
			break;
		case MONEY:
			if (M != null)
				buf.write(Double.toString(
						CMLib.beanCounter().getTotalAbsoluteNativeValue(M))
						.getBytes(Session.MSDP_CHARSET));
			break;
		case MOVEMENT:
			if (M != null)
				buf.write(Integer.toString(M.curState().getMovement())
						.getBytes(Session.MSDP_CHARSET));
			break;
		case MOVEMENT_MAX:
			if (M != null)
				buf.write(Integer.toString(M.maxState().getMovement())
						.getBytes(Session.MSDP_CHARSET));
			break;
		case OPPONENT_HEALTH:
			if ((M != null) && (M.getVictim() != null))
				buf.write(Integer.toString(
						M.getVictim().curState().getHitPoints()).getBytes(
						Session.MSDP_CHARSET));
			break;
		case OPPONENT_HEALTH_MAX:
			if ((M != null) && (M.getVictim() != null))
				buf.write(Integer.toString(
						M.getVictim().maxState().getHitPoints()).getBytes(
						Session.MSDP_CHARSET));
			break;
		case OPPONENT_LEVEL:
			if ((M != null) && (M.getVictim() != null))
		...
		case OPPONENT_STRENGTH:
			if ((M != null) && (M.getVictim() != null)) {
				...
				try {
					buf.write(C.executeInternal(M, 0, M.getVictim()).toString()
							.getBytes(Session.MSDP_CHARSET));
				} catch (IOException e) {
					buf.write(Integer
							.toString(M.getVictim().phyStats().level())
							.getBytes(Session.MSDP_CHARSET));
				}
			}
			break;
		...
		case ROOM:
			if ((M != null) && (M.location() != null)) {
				...
				buf.write(Session.MSDP_VAL);
				buf.write(Integer.toString(
						CMLib.map().getExtendedRoomID(R).hashCode()).getBytes(
						Session.MSDP_CHARSET));
				buf.write(Session.MSDP_VAR);
				...
				for (int d = 0; d < Directions.NUM_DIRECTIONS(); d++) {
					...
					if ((R2 != null) && (R.getExitInDir(d) != null)) {
						...
						if (roomID.length() > 0) {
							buf.write(Session.MSDP_VAR);
							buf.write(Directions.getDirectionChar(d).getBytes(
									Session.MSDP_CHARSET));
							buf.write(Session.MSDP_VAL);
							buf.write(Integer.toString(roomID.hashCode())
									.getBytes(Session.MSDP_CHARSET));
						}
					}
				}
		...
		case ROOM_NAME:
			if ((M != null) && (M.location() != null))
				buf.write(M.location().displayText()
						.getBytes(Session.MSDP_CHARSET));
			break;
		case ROOM_VNUM:
			if ((M != null) && (M.location() != null))
				buf.write(Integer.toString(
						CMLib.map().getExtendedRoomID(M.location()).hashCode())
						.getBytes(Session.MSDP_CHARSET));
			break;
		case ROOM_AREA:
			if ((M != null) && (M.location() != null))
				buf.write(M.location().getArea().Name()
						.getBytes(Session.MSDP_CHARSET));
			break;
		case ROOM_TERRAIN:
			if ((M != null) && (M.location() != null)) {
		...
		case ROOM_EXITS:
			...
			break;
		case SERVER_ID:
			buf.write(CMProps.getVar(CMProps.Str.MUDNAME).getBytes(
					Session.MSDP_CHARSET));
			break;
		case SERVER_TIME:
			buf.write(CMLib.time()
					.date2APTimeString(System.currentTimeMillis())
					.getBytes(Session.MSDP_CHARSET));
			break;
		case SPECIFICATION:
			buf.write("http://tintin.sourceforge.net/msdp/"
					.getBytes(Session.MSDP_CHARSET));
			break;
		case WORLD_TIME:
			buf.write(CMLib.time().globalClock().getShortestTimeDescription()
					.getBytes(Session.MSDP_CHARSET));
			break;
		default:
			break;
	...
	protected byte[] msdpListToMsdpArray(final Object[] stuff)
			throws UnsupportedEncodingException, IOException {
		ByteArrayOutputStream buf = new ByteArrayOutputStream();
	...
	protected byte[] processMsdpList(final Session session, final String var,
			final Map<Object, Object> reportables)
			throws UnsupportedEncodingException, IOException {
		ByteArrayOutputStream buf = new ByteArrayOutputStream();
		final MSDPListable type = (MSDPListable) CMath.s_valueOf(
				MSDPListable.class, var.toUpperCase().trim());
		if (type == null)
			return buf.toByteArray();
		switch (type) {
		case COMMANDS:
	...
	protected void resetMsdpConfigurable(final Session session, final String var) {
		final MSDPConfigurableVar type = (MSDPConfigurableVar) CMath.s_valueOf(
				MSDPConfigurableVar.class, var.toUpperCase().trim());
		if (type == null)
			return;
		// TODO:
	}

	public byte[] pingMsdp(final Session session,
			final Map<Object, Object> reportables) {
		try {
			...
			synchronized (reportables) {
				...
				for (Entry<Object, Object> e : reportables.entrySet()) {
					newValue = getMsdpComparable(session,
							(MSDPVariable) e.getKey());
	...
	@SuppressWarnings("rawtypes")
	public byte[] processMsdp(final Session session, final char[] data,
			final int dataSize, final Map<Object, Object> reportables) {
		try {
			...
			if (cmds.containsKey(MSDPCommand.REPORT.toString())) {
				synchronized (reportables) {
					...
					if (o instanceof String) {
						final MSDPVariable type = (MSDPVariable) CMath
								.s_valueOf(MSDPVariable.class, ((String) o)
										.toUpperCase().trim());
						if (type != null)
							reportables.put(type,
									getMsdpComparable(session, type));
						buf.write(processMsdpSend(session, (String) o));
					} else if (o instanceof List)
						for (Object o2 : ((List) o))
							if (o2 instanceof String) {
								final MSDPVariable type = (MSDPVariable) CMath
										.s_valueOf(MSDPVariable.class,
					...
					for (Object o2 : ((List) o))
						if (o2 instanceof String) {
							...
							buf.write(((String) o2)
									.getBytes(Session.MSDP_CHARSET));
							buf.write(Session.MSDP_VAL);
							buf.write(processMsdpList(session, (String) o2,
									reportables));
						}
			}
			if (cmds.containsKey(MSDPCommand.UNREPORT.toString())) {
				...
				if (o instanceof String) {
					final MSDPVariable type = (MSDPVariable) CMath.s_valueOf(
							MSDPVariable.class, ((String) o).toUpperCase()
									.trim());
					if (type != null)
	...
	public enum gmcpCommand {
		core_hello, core_supports_set, core_supports_add, core_supports_remove, core_keepalive, core_ping, core_goodbye, char_vitals, char_statusvars, char_status, char_base, char_maxstats, char_worth, group, room_info, comm_channel, ire_composer_setbuffer
	}

	protected String processGmcpStr(final Session session, final char[] data,
			final int dataSize, final Map<String, Double> supportables) {
			...
			try {
						...
						for (Object o : list) {
							...
							if (x > 0) {
								if (CMath.isNumber(s.substring(x + 1).trim()))
									ver = CMath.s_double(s.substring(x + 1)
											.trim());
				...
				case char_vitals: {
					if (mob != null) {
						...
						doc.append("\"hp\":")
								.append(mob.curState().getHitPoints())
								.append(",");
						doc.append("\"mana\":")
								.append(mob.curState().getMana()).append(",");
						doc.append("\"moves\":").append(
				...
				case char_statusvars:
					if (mob != null) {
						StringBuilder doc = new StringBuilder(
								"char.statusvars {");
						doc.append("\"level\":\"")
								.append(mob.phyStats().level()).append("\",");
						doc.append("\"race\":\"")
								.append(MiniJSON.toJSONString(mob.charStats()
										.raceName())).append("\"");
						...
						if (clans.size() == 1)
							doc.append(",\"guild\":\"").append(clans.get(0))
									.append("\"");
				...
				case char_base:
					if (mob != null) {
						...
						doc.append("\"name\":\"").append(mob.name())
								.append("\"").append(",");
						doc.append("\"class\":\"")
								.append(mob.charStats().getCurrentClass()
										.baseClass()).append("\"").append(",");
						doc.append("\"subclass\":\"")
								.append(MiniJSON.toJSONString(mob.charStats()
										.displayClassName())).append("\"")
								.append(",");
						doc.append("\"race\":\"")
								.append(MiniJSON.toJSONString(mob.charStats()
										.raceName())).append("\"").append(",");
						doc.append("\"perlevel\":").append(
								mob.getExpNextLevel());
						String title = (mob.playerStats() != null) ? mob
								.playerStats().getActiveTitle() : null;
						if (title != null)
							doc.append(",\"pretitle\":\"")
									.append(MiniJSON.toJSONString(title))
									.append("\"");
						final List<String> clans = new LinkedList<String>();
						for (Pair<Clan, Integer> p : mob.clans())
							clans.add(MiniJSON.toJSONString(p.first.name()));
						if (clans.size() == 1)
							doc.append(",\"clan\":\"").append(clans.get(0))
									.append("\"");
						else if (clans.size() > 1) {
							doc.append(",\"clan\":[");
							for (int i = 0; i < clans.size(); i++) {
				...
				case char_maxstats:
					if (mob != null) {
						...
						doc.append("\"maxhp\":")
								.append(mob.maxState().getHitPoints())
								.append(",");
						doc.append("\"maxmana\":")
								.append(mob.maxState().getMana()).append(",");
						doc.append("\"maxmoves\":").append(
				...
				case char_status:
					if (mob != null) {
						...
						doc.append("\"level\":").append(mob.phyStats().level())
								.append(",");
						doc.append("\"tnl\":").append(mob.getExpNeededLevel())
								.append(",");
						doc.append("\"hunger\":")
								.append(mob.curState().getHunger()).append(",");
						doc.append("\"thirst\":")
								.append(mob.curState().getThirst()).append(",");
						doc.append("\"fatigue\":")
								.append(mob.curState().getFatigue())
								.append(",");
						int align = mob
								.fetchFaction(CMLib.factions().AlignID());
						...
						if (session.isAfk())
							...
							state = 8;
						else if (CMath.bset(mob.getBitmap(), MOB.ATT_SYSOPMSGS))
							state = 6;
						else if (CMath.bset(mob.getBitmap(), MOB.ATT_AUTORUN))
							state = 12;
						doc.append("\"state\":").append(state).append(",");
						doc.append("\"pos\":\"")
								.append(CMLib.flags().isSleeping(mob) ? "Sleeping"
										: CMLib.flags().isSitting(mob) ? "Sitting"
												: "Standing").append("\"");
						MOB vicM = mob.getVictim();
						if (vicM != null) {
							doc.append(",");
							doc.append("\"enemy\":").append(vicM.name())
									.append(",");
							doc.append("\"enemypct\":")
									.append(Math.round((double) vicM.maxState()
											.getHitPoints()
											/ (double) vicM.curState()
													.getHitPoints() * 100.0))
									.append(",");
						}
						doc.append("}");
				...
				case char_worth:
					if (mob != null) {
						...
						doc.append("\"gold\":")
								.append(CMLib.beanCounter()
										.getTotalAbsoluteNativeValue(mob))
								.append(",");
						doc.append("\"qp\":").append(mob.getQuestPoint())
								.append(",");
						// doc.append("\"tp\":").append(mob.getTrains()).append(",");
						doc.append("\"trains\":").append(mob.getTrains())
								.append(",");
				...
				case room_info:
					if (mob != null) {
						...
						if (room != null) {
							...
							final String roomID = CMLib.map()
									.getExtendedRoomID(room);
							final String domType;
							if ((room.domainType() & Room.INDOORS) == 0)
								domType = Room.outdoorDomainDescs[room
										.domainType()];
							else
								domType = Room.indoorDomainDescs[CMath.unsetb(
										room.domainType(), Room.INDOORS)];
							doc.append("\"num\":")
									.append(roomID.hashCode())
									.append(",")
									.append("\"id\":\"")
									.append(roomID)
									.append("\",")
									.append("\"name\":\"")
									.append(MiniJSON.toJSONString(room
											.displayText(mob)))
									.append("\",")
									.append("\"zone\":\"")
									.append(MiniJSON.toJSONString(room
							...
							for (int d = 0; d < Directions.NUM_DIRECTIONS(); d++) {
								...
								if ((R2 != null)
									...
									final String room2ID = CMLib.map()
				...
				case group:
					if (mob != null) {
						...
						final Set<MOB> group = mob
								.getGroupMembers(new HashSet<MOB>());
						final MOB leaderM = (mob.amFollowing() == null) ? mob
								: mob.amUltimatelyFollowing();
						doc.append("\"groupname\":\"")
								.append(leaderM.name(mob)).append("s group")
								.append("\",").append("\"leader\":\"")
								.append(leaderM.name(mob)).append("\",")
								.append("\"status\":\"").append("Private")
								.append("\",").append("\"count\":")
						...
						for (MOB M : group) {
							if (comma)
								doc.append(",");
							comma = true;
							doc.append("{\"name\":\"").append(M.name(mob))
									.append("\",").append("{\"info\":{")
									.append("\"hp\":")
									.append(M.curState().getHitPoints())
									.append(",").append("\"mhp\":")
									.append(M.maxState().getHitPoints())
									.append(",").append("\"mn\":")
									.append(M.curState().getMana()).append(",")
									.append("\"mmn\":")
									.append(M.maxState().getMana()).append(",")
									.append("\"mv\":")
									.append(M.curState().getMovement())
									.append(",").append("\"mmv\":")
									.append(M.maxState().getMovement())
									.append(",").append("\"lvl\":")
									.append(M.phyStats().level()).append(",");
							int align = mob.fetchFaction(CMLib.factions()
									.AlignID());
							if (align != Integer.MAX_VALUE)
								doc.append("\"align\":").append(align)
										.append(",");
							doc.append("\"tnl\":")
									.append(M.getExpNeededLevel());
							doc.append("}");
						}
						doc.append("]");
						doc.append("}");
	...
	public byte[] processGmcp(final Session session, final char[] data,
			final int dataSize, final Map<String, Double> supportables) {
	...
	protected byte[] possiblePingGmcp(final Session session,
			final Map<String, Long> reporteds,
			final Map<String, Double> supportables, final String command) {
		final char[] cmd = command.toCharArray();
		final String chunkStr = processGmcpStr(session, cmd, cmd.length,
				supportables);
		if (chunkStr != null) {
	...
	public byte[] pingGmcp(final Session session,
			final Map<String, Long> reporteds,
		...
		try {
			...
			final Long nextTruePingReport = reporteds
					.get("system.nextTruePing");
			...
			if (charSupported || supportables.containsKey("char.vitals")) {
				buf = possiblePingGmcp(session, reporteds, supportables,
						"char.vitals");
				if (buf != null)
					bout.write(buf);
			}
			if ((nextTruePingReport == null)
					|| (now > nextTruePingReport.longValue())) {
				final long tickMillis = CharState.ADJUST_FACTOR
						* CMProps.getTickMillis();
				reporteds
						.put("system.nextTruePing",
								new Long(
										(nextTruePingReport == null) ? (now + tickMillis)
												: (nextTruePingReport
														.longValue() + tickMillis)));
				if (supportables.containsKey("comm.tick")
						|| supportables.containsKey("comm")) {
					if (CMSecurity.isDebugging(DbgFlag.TELNET))
			...
			if ((nextMedReport == null) || (now > nextMedReport.longValue())) {
				...
				if ((mob != null)
						&& ((mob.amFollowing() != null) || (mob.numFollowers() > 0))) {
					if (supportables.containsKey("group")) {
						buf = possiblePingGmcp(session, reporteds,
								supportables, "group");
						if (buf != null)
							bout.write(buf);
					}
			...
			if ((nextLongReport == null) || (now > nextLongReport.longValue())) {
				...
				if (charSupported
						|| supportables.containsKey("char.statusvars")) {
					buf = possiblePingGmcp(session, reporteds, supportables,
							"char.statusvars");
					if (buf != null)
						bout.write(buf);
			...
			if (supportables.containsKey("room.info")
				...
				if (mob != null) {
					final Room room = mob.location();
					if (room != null) {
						final Long oldRoomHash = reporteds
								.get("system.currentRoom");
						if ((oldRoomHash == null)
								|| (room.hashCode() != oldRoomHash.longValue())) {
							reporteds.put("system.currentRoom",
									Long.valueOf(room.hashCode()));
							final String command = "room.info";
							final char[] cmd = command.toCharArray();
							buf = processGmcp(session, cmd, cmd.length,
									supportables);
							if (buf != null)
								bout.write(buf);
						}
