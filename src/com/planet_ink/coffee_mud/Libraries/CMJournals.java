...
public class CMJournals extends StdLibrary implements JournalsLibrary {
	...
	protected Hashtable<String, JournalSummaryStats> getSummaryStats() {
		...
		journalSummaryStats = (Hashtable<String, JournalSummaryStats>) Resources
				.getResource("FORUM_JOURNAL_STATS");
		if (journalSummaryStats == null) {
			synchronized ("FORUM_JOURNAL_STATS".intern()) {
				...
				if (journalSummaryStats == null) {
					...
					Resources.submitResource("FORUM_JOURNAL_STATS",
							journalSummaryStats);
				}
			}
		}
	...
	public JournalSummaryStats getJournalStats(String journalName) {
		...
		JournalSummaryStats stats = journalSummaryStats.get(journalName
				.toUpperCase().trim());
		if (stats == null) {
			synchronized (journal.NAME().intern()) {
				stats = journalSummaryStats.get(journalName.toUpperCase()
						.trim());
				if (stats == null) {
					stats = new JournalSummaryStats();
					stats.name = journal.NAME();
					CMLib.database().DBReadJournalSummaryStats(stats);
					journalSummaryStats.put(journalName.toUpperCase().trim(),
							stats);
				}
			}
		}
	...
	public int loadCommandJournals(String list) {
		...
		while (list.length() > 0) {
					...
					if (keyx >= 0) {
						...
						if ((keyx == 0)
								|| (Character.isWhitespace(mask
										.charAt(keyx - 1)))) {
							String parm = mask.substring(keyx + flag.length(),
									keyy).trim();
							if ((parm.length() == 0) || (parm.startsWith("="))) {
								if (parm.startsWith("="))
									parm = parm.substring(1);
								flags.put(CommandJournalFlags.values()[pf],
										parm);
								mask = mask.substring(0, keyx).trim() + " "
										+ mask.substring(keyy).trim();
							}
						}
					}
				}
			...
			commandJournals.put(item.toUpperCase().trim(), new CommandJournal(
					item.toUpperCase().trim(), mask, flags));
		}
		return commandJournals.size();
	}

	public boolean canReadMessage(JournalEntry entry, String srchMatch,
			MOB readerM, boolean ignorePrivileges) {
		if (entry == null)
			return false;
		String to = entry.to;
		if ((srchMatch != null)
				...
				&& ((to.toLowerCase().indexOf(srchMatch) < 0)
						...
						&& (entry.subj.toLowerCase().indexOf(srchMatch) < 0) && (entry.msg
						.toLowerCase().indexOf(srchMatch) < 0)))
			return false;
		boolean priviledged = false;
		if (readerM != null)
			priviledged = CMSecurity.isAllowedAnywhere(readerM,
					CMSecurity.SecFlag.JOURNALS) && (!ignorePrivileges);
		if (to.equalsIgnoreCase("all")
				|| ((readerM != null) && (priviledged
						|| to.equalsIgnoreCase(readerM.Name()) || (to
						.toUpperCase().trim().startsWith("MASK=") && (CMLib
						.masking().maskCheck(to.trim().substring(5), readerM,
						true))))))
			return true;
		return false;
	}
		...
		while (list.length() > 0) {
			...
			if (x > 0) {
				...
				while (x > 0) {
					...
					if (y > 0) {
						try {
							ForumJournalFlags.valueOf(rest.substring(y, x)
									.toUpperCase().trim());
				...
				for (Integer flagDex : flagDexes) {
					String piece = rest
							.substring(lastStart, flagDex.intValue());
					lastStart = flagDex.intValue();
					x = piece.indexOf('=');
					try {
						ForumJournalFlags flagVar = ForumJournalFlags
								.valueOf(piece.substring(0, x).toUpperCase()
										.trim());
			...
			CMSecurity.registerJournal(item.toUpperCase().trim());
			forumJournals.put(item.toUpperCase().trim(),
					new ForumJournal(item.trim(), flags));
		}
	...
	public HashSet<String> getArchonJournalNames() {
		HashSet<String> H = (HashSet<String>) Resources
				.getResource("ARCHON_ONLY_JOURNALS");
		if (H == null) {
			Item I = null;
			H = new HashSet<String>();
			for (Enumeration<Item> e = CMClass.basicItems(); e
					.hasMoreElements();) {
				I = e.nextElement();
				if ((I instanceof ArchonOnly) && (!I.isGeneric()))
	...
	public void expirationJournalSweep() {
						...
						for (int i = items.size() - 1; i >= 0; i--) {
							...
							if (System.currentTimeMillis() > compdate) {
								...
								Log.sysOut(Thread.currentThread().getName(),
										"Expired " + CMJ.NAME() + " from "
												+ from + ": " + message);
								CMLib.database().DBDeleteJournal(
										CMJ.JOURNAL_NAME(), entry.key);
							}
						}
		...
		try {
			for (Enumeration<ForumJournal> e = forumJournals(); e
					.hasMoreElements();) {
				...
				if ((num != null) && (CMath.isNumber(num))
						&& (CMath.s_double(num) > 0.0)) {
					setThreadStatus(serviceClient,
							"updating journal " + FMJ.NAME());
					List<JournalsLibrary.JournalEntry> items = CMLib.database()
							.DBReadJournalMsgs(FMJ.NAME());
					if (items != null)
						for (int i = items.size() - 1; i >= 0; i--) {
							JournalEntry entry = items.get(i);
							if (!CMath.bset(entry.attributes,
									JournalEntry.ATTRIBUTE_PROTECTED)) {
								long compdate = entry.update;
								compdate = compdate
										+ Math.round(CMath.mul(
												TimeManager.MILI_DAY,
								...
								if (System.currentTimeMillis() > compdate) {
									String from = entry.from;
									String message = entry.msg;
									Log.debugOut(Thread.currentThread()
											.getName(), "Expired " + FMJ.NAME()
											+ " from " + from + ": " + message);
									CMLib.database().DBDeleteJournal(
											FMJ.NAME(), entry.key);
								}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		try {
			if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.SAVETHREAD))
					&& (!CMSecurity
							.isDisabled(CMSecurity.DisFlag.JOURNALTHREAD))) {
	...
	public MsgMkrResolution makeMessage(final MOB mob,
			final String messageTitle, final List<String> vbuf, boolean autoAdd)
			throws IOException {
		final Session sess = mob.session();
		if ((sess == null) || (sess.isStopped()))
			return MsgMkrResolution.CANCELFILE;
		final boolean canExtEdit = ((mob.session() != null) && (mob.session()
				.getClientTelnetMode(Session.TELNET_GMCP)));
		final String help = "^HCoffeeMud Message Maker Options:^N\n\r"
		...
		while ((mob.session() != null) && (!sess.isStopped())) {
			...
			if (!menuMode) {
				String line = sess.prompt(
						"^X" + CMStrings.padRight("" + vbuf.size(), 3)
								+ ")^.^N ", "");
				if (line.trim().equals("."))
					menuMode = true;
				else
				...
				String option = sess.choose("^HMenu ^N(?/A/D/L/I/E/R/S/Q"
						+ (canExtEdit ? "/W" : "") + ")^H: ^N", "ADLIERSQ?"
						+ (canExtEdit ? "W" : ""), "?", -1, paramsOut);
				String paramAll = (paramsOut.size() > 0) ? CMParms.combine(
						paramsOut, 0) : null;
				String param1 = (paramsOut.size() > 0) ? paramsOut.getFirst()
						: null;
				String param2 = (paramsOut.size() > 1) ? CMParms.combine(
						paramsOut, 1) : null;
				switch (option.charAt(0)) {
				case 'S':
					if (((paramAll != null) && (paramAll.equalsIgnoreCase("Y")))
							|| (sess.confirm(
									"Save and exit, are you sure (N/y)? ", "N"))) {
				...
				case 'R': {
					if (vbuf.size() == 0)
						...
						if (line == null)
							line = sess
									.prompt("Text to search for (case sensitive): ",
											"");
						if (line.length() > 0) {
							String str = param2;
							if (str == null)
							...
							for (int i = 0; i < vbuf.size(); i++)
								vbuf.set(i, CMStrings.replaceAll(vbuf.get(i),
										line, str));
						} else
							mob.tell("(aborted)");
					}
				...
				case 'E': {
					if (vbuf.size() == 0)
						...
						if (line == null)
							line = sess.prompt(
									"Line to edit (0-" + (vbuf.size() - 1)
											+ "): ", "");
						if ((CMath.isInteger(line)) && (CMath.s_int(line) >= 0)
								&& (CMath.s_int(line) < (vbuf.size()))) {
							int ln = CMath.s_int(line);
							mob.tell("Current: \n\r"
									+ CMStrings.padRight("" + ln, 3) + ") "
									+ vbuf.get(ln));
							String str = param2;
							if (str == null)
				...
				case 'W': {
					if (mob.session() != null) {
						...
						mob.session().sendGMCPEvent(
								"IRE.Composer.Edit",
								"{\"title\":\""
										+ MiniJSON.toJSONString(messageTitle)
										+ "\",\"text\":\""
										+ MiniJSON.toJSONString(oldDoc
												.toString()) + "\"}");
						oldDoc = null;
						String newText = mob
								.session()
								.prompt("Re-Enter the whole doc using your GMCP editor.\n\r"
				...
				case 'L': {
					...
					for (int v = 0; v < vbuf.size(); v++)
						list.append(CMLib.coffeeFilter()
								.colorOnlyFilter(
										"^X" + CMStrings.padRight("" + v, 3)
												+ ")^.^N ", sess)
								+ vbuf.get(v) + "\n\r");
				...
				case 'I': {
					if (vbuf.size() == 0)
						...
						String line = param1;
						if (line == null)
							line = sess.prompt("Line to insert before (0-"
									+ (vbuf.size() - 1) + "): ", "");
						if ((CMath.isInteger(line)) && (CMath.s_int(line) >= 0)
								&& (CMath.s_int(line) < (vbuf.size()))) {
							int ln = CMath.s_int(line);
							...
							if (str == null)
								str = sess
										.prompt("Enter text to insert here.\n\r: ");
							vbuf.add(ln, str);
						} else
							mob.tell("'" + line
									+ "' is not a valid line number.");
	...
	public boolean subscribeToJournal(String journalName, String userName,
			boolean saveMailingList) {
		boolean updateMailingLists = false;
		if ((CMProps.getVar(CMProps.Str.MAILBOX).length() > 0)
				&& (CMLib.players().playerExists(userName) || CMLib.players()
						.accountExists(userName))) {
			Map<String, List<String>> lists = Resources.getCachedMultiLists(
					"mailinglists.txt", true);
			...
			if (!found) {
				...
				if (CMProps.getBoolVar(CMProps.Bool.EMAILFORWARDING)) {
					...
					String subscribedMsg = "You are now subscribed to "
							+ journalName
							+ ". To unsubscribe, send an email with a subject of unsubscribe.";
					String[] msgs = CMProps
							.getListVar(CMProps.StrList.SUBSCRIPTION_STRS);
					if ((msgs != null) && (msgs.length > 0)) {
						if (msgs[0].length() > 0)
							subscribeTitle = CMLib.coffeeFilter().fullInFilter(
									CMStrings.replaceAll(msgs[0], "<NAME>",
											journalName));
						if ((msgs.length > 0) && (msgs[1].length() > 0))
							subscribedMsg = CMLib.coffeeFilter().fullInFilter(
									CMStrings.replaceAll(msgs[1], "<NAME>",
											journalName));
					}
					CMLib.database().DBWriteJournalEmail(
							CMProps.getVar(CMProps.Str.MAILBOX), journalName,
							journalName, userName, subscribeTitle,
							subscribedMsg);
				}
			}
		}
	...
	public boolean unsubscribeFromJournal(String journalName, String userName,
			boolean saveMailingList) {
		boolean updateMailingLists = false;
		if (CMProps.getVar(CMProps.Str.MAILBOX).length() == 0)
			return false;
		...
		for (int l = mylist.size() - 1; l >= 0; l--)
			if (mylist.get(l).equalsIgnoreCase(userName)) {
				...
				if (CMProps.getBoolVar(CMProps.Bool.EMAILFORWARDING)) {
					...
					String unsubscribedMsg = "You are no longer subscribed to "
							+ journalName
							+ ". To subscribe again, send an email with a subject of subscribe.";
					String[] msgs = CMProps
							.getListVar(CMProps.StrList.SUBSCRIPTION_STRS);
					if ((msgs != null) && (msgs.length > 2)) {
						if (msgs[2].length() > 0)
							unsubscribeTitle = CMLib.coffeeFilter()
									.fullInFilter(
											CMStrings.replaceAll(msgs[2],
													"<NAME>", journalName));
						if ((msgs.length > 3) && (msgs[1].length() > 0))
							unsubscribedMsg = CMLib.coffeeFilter()
									.fullInFilter(
											CMStrings.replaceAll(msgs[3],
													"<NAME>", journalName));
					}
					CMLib.database().DBWriteJournalEmail(
							CMProps.getVar(CMProps.Str.MAILBOX), journalName,
							journalName, userName, unsubscribeTitle,
							unsubscribedMsg);
				}
			}
		if (updateMailingLists && saveMailingList) {
