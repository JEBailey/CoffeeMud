...
public class ApartmentLayout extends AbstractLayout {
	...
	public java.util.List<LayoutNode> generate(int num, int dir) {
		...
		case Directions.EAST:
			...
			break;
		case Directions.WEST:
			n = new DefaultLayoutNode(new long[] { fullNumHallway,
					hallwayLength });
		...
		if (n != null) {
			...
			for (int h = 1; h < numHallways; h++) {
				...
				LayoutNode nextNode = lSet.makeNextNode(prevNode,
						sidewayDirection);
			...
			for (LayoutNode hallwayNode : hallways) {
				...
				for (int h = 0; h < hallwayLength; h++) {
					...
					nextNode = lSet.makeNextNode(prevNode,
							Directions.getOpDirectionCode(sidewayDirection));
					lSet.use(nextNode, LayoutTypes.leaf);
					nextNode.crossLink(prevNode);
				}
			}
