...
public class MazeLayout extends AbstractLayout {
	...
	public void fillMaze(LayoutSet lSet, LayoutNode p, int width, int height) {
		...
		for (int r = 0; r < rdirs.size(); r++) {
			...
			if ((!lSet.isUsed(p2.coord())) && (p2.coord()[0] >= 0)
					&& (p2.coord()[1] <= 0) && (p2.coord()[0] < width)
					&& (p2.coord()[1] > -height)) {
	...
	public List<LayoutNode> generate(int num, int dir) {
		...
		case Directions.SOUTH:
			n = new DefaultLayoutNode(new long[] { (diameter + plusX) / 2,
					-diameter + 1 });
			break;
		...
		case Directions.WEST:
			n = new DefaultLayoutNode(new long[] { diameter + plusX - 1,
					(-diameter + 1) / 2 });
			break;
