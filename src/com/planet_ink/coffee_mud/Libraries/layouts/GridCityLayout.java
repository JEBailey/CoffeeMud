...
public class GridCityLayout extends AbstractLayout {
	...
	public List<LayoutNode> generate(int num, int dir) {
		...
		for (int x = 0; x < diameter + plusX; x += 2) {
			...
			if (n != null) {
				if ((dir == Directions.NORTH) && (x >= middle - 1)
						&& (x <= middle + 1))
					firstNode = n;
				...
				lSet.use(n, LayoutTypes.street);
				n.flagRun(LayoutRuns.ns);
				if ((dir == Directions.SOUTH) && (x >= middle - 1)
						&& (x <= middle + 1))
					firstNode = n;
			}
		}
		...
		for (int y = -2; y >= -diameter + 1; y -= 2) {
			...
			if (n != null) {
				if ((dir == Directions.EAST) && (y >= middle - 1)
						&& (y <= middle + 1))
					firstNode = n;
				...
				lSet.use(n, LayoutTypes.street);
				n.flagRun(LayoutRuns.ew);
				if ((dir == Directions.WEST) && (y >= middle - 1)
						&& (y <= middle + 1))
					firstNode = n;
			}
		}
