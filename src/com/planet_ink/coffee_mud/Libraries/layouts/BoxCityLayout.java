...
public class BoxCityLayout extends AbstractLayout {
	...
	public void halfLineN(LayoutSet lSet, int startX, int endX, int height,
			TreeSet<Integer> xposUsed) {
	...
	public void halfLineE(LayoutSet lSet, int startY, int endY, int width,
			TreeSet<Integer> yposUsed) {
	...
	public List<LayoutNode> generate(int num, int dir) {
		...
		int plusX = (diff(diameter, diameter, num) > diff(diameter + 1,
				diameter, num)) ? 1 : 0;
		...
		for (Integer y : yposUsed) {
			Integer lastX = null;
			for (Iterator<Integer> thisXE = xposUsed.iterator(); thisXE
					.hasNext();) {
				Integer thisX = thisXE.next();
				if (lastX != null) {
					x = lastX.intValue()
							+ ((thisX.intValue() - lastX.intValue()) / 2);
					if (y.intValue() > (-diameter + 1))
						if (!fillMaze(lSet, lSet.getNode(x, y.intValue()),
								Directions.NORTH))
							fillMaze(lSet, lSet.getNode(x + 1, y.intValue()),
									Directions.NORTH);
					if (thisXE.hasNext()) {
						lastX = thisX;
						thisX = thisXE.next();
		...
		while ((n == null) && ((++tryDiff) < 10)) {
			...
			case Directions.NORTH:
				n = lSet.getNode(new long[] {
						((diameter + plusX) / 2) + tryDiff, 0 });
				break;
			case Directions.SOUTH:
				n = lSet.getNode(new long[] {
						((diameter + plusX) / 2) + tryDiff, -diameter + 1 });
				break;
			case Directions.EAST:
				n = lSet.getNode(new long[] { 0,
						((-diameter + 1) / 2) + tryDiff });
				break;
			case Directions.WEST:
				n = lSet.getNode(new long[] { diameter + plusX - 1,
						((-diameter + 1) / 2) + tryDiff });
				break;
			}
		...
		if (n != null) {
			...
			Log.errOut("BoxCityLayout",
					"Not able to find start room for direction: " + dir);
		return set;
	}
