...
public class CMPlayers extends StdLibrary implements PlayerLibrary {
	...
	protected CrossRefTreeMap<MOB, Room> playerLocations = new CrossRefTreeMap<MOB, Room>(
			Integer.MAX_VALUE, 1);
	...
	public MOB getLoadPlayerByEmail(String email) {
		...
		for (Enumeration<ThinPlayer> e = thinPlayers("", null); e
				.hasMoreElements();) {
	...
	public PlayerAccount getLoadAccountByEmail(String email) {
		...
		for (Enumeration<PlayerAccount> e = accounts("", null); e
				.hasMoreElements();) {
			PlayerAccount P = e.nextElement();
			if (P.getEmail().equalsIgnoreCase(email))
				return P;
	...
	public PlayerAccount getAccount(String calledThis) {
		...
		for (MOB M : playersList)
			if ((M.playerStats() != null)
					...
					&& (M.playerStats().getAccount().accountName()
							.equals(calledThis))) {
	...
	public void renamePlayer(MOB mob, String oldName) {
		...
		for (Enumeration<MOB> p = CMLib.players().players(); p
				.hasMoreElements();) {
		...
		if (pstats != null) {
			...
			for (Enumeration<Item> e = pstats.getExtItems().items(); e
					.hasMoreElements();) {
				Item I = e.nextElement();
				if (I instanceof PrivateProperty)
					if (((PrivateProperty) I).getOwnerName().equalsIgnoreCase(
							oldName))
						((PrivateProperty) I).setOwnerName(newName);
			}
		}

		for (Enumeration<Room> r = CMLib.map().roomsFilled(); r
				.hasMoreElements();) {
			...
			if ((R != null) && (R.roomID().length() > 0)) {
				synchronized (("SYNC" + R.roomID()).intern()) {
					...
					for (Enumeration<Ability> a = R.effects(); a
							.hasMoreElements();) {
	...
	public void obliteratePlayer(MOB deadMOB, boolean deleteAssets,
		...
		for (Session S : CMLib.sessions().allIterable())
			if ((!S.isStopped()) && (S.mob() != null)
					&& (S.mob().Name().equals(deadMOB.Name())))
				deadMOB = S.mob();
		...
		if (deleteAssets) {
			CMMsg msg = CMClass
					.getMsg(deadMOB,
							...
							CMMsg.MSG_RETIRE,
							(quiet) ? null
									: "A horrible death cry is heard throughout the land.");
			if (deadLoc != null)
	...
	public int savePlayers() {
		int processed = 0;
		final boolean noCachePlayers = CMProps
				.getBoolVar(CMProps.Bool.PLAYERSNOCACHE);
		for (MOB mob : playersList) {
			...
			if (!mob.isMonster()) {
				...
				if ((pStats.lastUpdated() == 0)
						...
						|| (noCachePlayers && (!CMLib.flags().isInTheGame(mob,
								true)))) {
					setThreadStatus(serviceClient, "just saving " + mob.Name());
					CMLib.database().DBUpdatePlayerMOBOnly(mob);
	...
	public Enumeration<ThinPlayer> thinPlayers(String sort,
		...
		Vector<PlayerLibrary.ThinPlayer> V = (cache == null) ? null
				: (Vector<PlayerLibrary.ThinPlayer>) cache
						.get("PLAYERLISTVECTOR" + sort);
		if (V == null) {
	...
	public Enumeration<PlayerAccount> accounts(String sort,
			Map<String, Object> cache) {
		Vector<PlayerAccount> V = (cache == null) ? null
				: (Vector<PlayerAccount>) cache.get("ACCOUNTLISTVECTOR" + sort);
		if (V == null) {
			...
			if (!allAccountsLoaded) {
				if (CMProps.getBoolVar(CMProps.Bool.ACCOUNTSNOCACHE))
					...
					List<PlayerAccount> rV = CMLib.database().DBListAccounts(
							null);
	...
	private boolean isProtected(List<String> protectedOnes, String name) {
		...
		if (protectedOne) {
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.AUTOPURGE))
				Log.debugOut(serviceClient.getName(), name
						+ " is protected from purging.");
			return true;
		}
		return false;
	}
	...
	private boolean autoPurge() {
		...
		if (mask.hashCode() != this.autoPurgeHash) {
			...
			long[][] presorted = CMLib.utensils().compileConditionalRange(
					CMParms.parseCommas(mask.trim(), true), 2, 0, lastLevel);
			...
			for (int i = 0; i < presorted.length; i++) {
				...
				if ((set == null) || (set.length < 2)) {
					Log.errOut("CMPlayers", "Error in AUTOPURGE definition #"
							+ (i + 1)
							+ " in coffeemud.ini file! Fix immediately!!");
					continue;
				}
				long val = set[0];
				if (set[0] <= 0)
		...
		List<PlayerLibrary.ThinPlayer> allUsers = CMLib.database()
				.getExtendedUserList();
		List<String> protectedOnes = Resources.getFileLineVector(Resources
				.getFileResource("protectedplayers.ini", false));
		...
		for (ThinPlayer user : allUsers) {
			...
			if (level >= autoPurgeDaysLevels.length) {
				...
				purgeDateTime = userLastLoginDateTime
						+ autoPurgeDaysLevels[autoPurgeDaysLevels.length - 1];
				warnDateTime = userLastLoginDateTime
						+ prePurgeLevels[prePurgeLevels.length - 1];
			} else if (level >= 0) {
				...
				Log.debugOut(
						...
						name
								...
								+ CMLib.time().date2String(
										userLastLoginDateTime)
								+ " will be warned on "
								+ CMLib.time().date2String(warnDateTime)
								+ " and purged on "
								+ CMLib.time().date2String(purgeDateTime));
			if ((System.currentTimeMillis() > purgeDateTime)
					|| (System.currentTimeMillis() > warnDateTime)) {
				if (isProtected(protectedOnes, name))
					continue;

				List<String> warnedOnes = Resources.getFileLineVector(Resources
						.getFileResource("warnedplayers.ini", false));
				...
				if ((warnedOnes != null) && (warnedOnes.size() > 0))
					for (int b = 0; b < warnedOnes.size(); b++) {
						...
						if (B.trim().length() > 0) {
							...
							warningDateTime = CMath.s_long(B.substring(
									lastSpace + 1).trim());
							if ((warningDateTime > 0)
									&& (System.currentTimeMillis() < warningDateTime
											+ (10 * TimeManager.MILI_DAY)))
								warnStr.append(B + "\n");
							if (B.toUpperCase().startsWith(
									name.toUpperCase() + " "))
				...
				if ((foundWarningDateTime < 0)
					...
					if ((M != null) && (M.playerStats() != null)) {
						warnStr.append(M.name() + " "
								+ M.playerStats().getEmail() + " "
								+ System.currentTimeMillis() + "\n");
						Resources.updateFileResource("::warnedplayers.ini",
								warnStr);
						if (CMSecurity
								.isDebugging(CMSecurity.DbgFlag.AUTOPURGE))
							Log.debugOut(serviceClient.getName(), name
									+ " is now warned.");
						warnPrePurge(M,
								purgeDateTime - System.currentTimeMillis());
					}
					...
					if ((M != null)
						...
						Log.sysOut(
								...
								"AutoPurged user "
										...
										+ ". Last logged in "
										+ (CMLib.time()
												.date2String(userLastLoginDateTime))
										+ ".");
					}
				}
			}
		...
		// accounts!
		if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.PURGEACCOUNTS))
				&& (CMProps.getIntVar(CMProps.Int.ACCOUNTPURGEDAYS) > 0))
			for (final Enumeration<PlayerAccount> pe = CMLib.players()
					.accounts("", null); pe.hasMoreElements();) {
				PlayerAccount PA = pe.nextElement();
				if ((PA.numPlayers() > 0)
						|| (isProtected(protectedOnes, PA.accountName())))
					continue;
				final long lastDateTimePurge = PA.lastDateTime()
						+ (TimeManager.MILI_DAY * CMProps
								.getIntVar(CMProps.Int.ACCOUNTPURGEDAYS));
				final long lastUpdatedPurge = PA.lastUpdated()
						+ (TimeManager.MILI_DAY * CMProps
								.getIntVar(CMProps.Int.ACCOUNTPURGEDAYS));
				...
				if (System.currentTimeMillis() > lastTime) {
					Log.sysOut(serviceClient.getName(), "AutoPurged account "
							+ PA.accountName() + ".");
	...
	private void warnPrePurge(MOB mob, long timeLeft) {
		...
		if ((mob.playerStats() == null)
				|| (mob.playerStats().getEmail().length() == 0)) // no email
																	// addy to
																	// forward
																	// TO
			return;
		...
		String msg = "Your character, "
				+ to
				+ ", is going to be autopurged by the system in "
				+ textTimeLeft
				+ ".  If you would like to keep this character active, please re-login.  This is an automated message, please do not reply.";
		...
		try {
			...
			Log.errOut(serviceClient.getName(), "Unable to notify " + to
					+ " of impending autopurge.  Invalid email address.");
			return;
		...
		try {
			SC.sendMessage(from + "@" + domain, replyTo + "@" + domain, mob
					.playerStats().getEmail(), mob.playerStats().getEmail(),
					subj, CMLib.coffeeFilter().simpleOutFilter(msg));
		} catch (java.io.IOException ioe) {
			Log.errOut(serviceClient.getName(), "Unable to notify " + to
					+ " of impending autopurge.");
		}
	}
			...
			if (!CMSecurity.isDisabled(CMSecurity.DisFlag.PLAYERTHREAD)) {
				...
				if (checkDatabase()) {
					...
					if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.SAVETHREAD))
							&& (!CMSecurity
									.isDisabled(CMSecurity.DisFlag.PLAYERTHREAD))) {
						setThreadStatus(serviceClient,
								"checking player titles.");
						for (MOB M : playersList)
							if (M.playerStats() != null) {
								if ((CMLib.titles().evaluateAutoTitles(M))
