...
public class StdRoom implements Room {
	...
	public StdRoom() {
		super();
		// CMClass.bumpCounter(this,CMClass.CMObjectType.LOCALE);//removed for
		// mem & perf
	...
	// CMClass.unbumpCounter(this,CMClass.CMObjectType.LOCALE); }//removed for
	// mem & perf
	...
	public int getAtmosphere() {
		if (getGridParent() != null)
			...
			return (myArea == null) ? RawMaterial.RESOURCE_AIR : myArea
					.getAtmosphere();
	}
	...
	protected void cloneFix(Room R) {
		...
		for (final Enumeration<ScriptingEngine> e = R.scripts(); e
				.hasMoreElements();) {
	...
	public CMObject copyOf() {
		try {
			...
			// CMClass.bumpCounter(R,CMClass.CMObjectType.LOCALE);//removed for
			// mem & perf
			R.xtraValues = (xtraValues == null) ? null : (String[]) xtraValues
					.clone();
			R.cloneFix(this);
			return R;
	...
	public void setRawExit(int direction, Environmental to) {
		 ...
		 * cant be done
		 * 
		 * if((E!=null)&&(E.exitUsage((short)0)==0)) E.destroy();
		 */
	}
	...
	public void giveASky(int depth) {
		...
		skyedYet = true;
		if ((roomID().length() == 0) && (getGridParent() != null)
			...
			for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--)
				if ((d != Directions.UP) && (d != Directions.DOWN)) {
					...
					if ((thatSky != null)
						...
						if (thatRoom != null) {
							xo = thatRoom.getRawExit(Directions
									.getOpDirectionCode(d));
							if ((xo == null) || (xo.hasADoor()))
								xo = dnE;
							thatSky.setRawExit(
									Directions.getOpDirectionCode(d), xo);
						}
	...
	public void setResource(int resourceCode) {
		...
		lastResourceTime = (resourceCode >= 0) ? lastResourceTime = System
				.currentTimeMillis() : 0;
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_EXPIRE: {
				if ((gridParent != null)
						&& (!gridParent.okMessage(myHost, msg)))
					return false;
			...
			case CMMsg.TYP_DIG:
				...
				break;
			default:
				if (((msg.targetMajor(CMMsg.MASK_HANDS)) || (msg
						.targetMajor(CMMsg.MASK_MOUTH)))
		...
		for (final Enumeration<MOB> m = inhabitants(); m.hasMoreElements();) {
			...
			if ((M != msg.source()) && (!M.okMessage(this, msg)))
				return false;
		}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.sourceMinor() == CMMsg.TYP_SHUTDOWN) {
			try {
				...
				} else if (CMSecurity.isSaveFlag("ROOMSHOPS")) {
					eachInhabitant(new EachApplicable<MOB>() {
						public final void apply(final MOB M) {
							if ((M instanceof ShopKeeper) && (M.isSavable())
									&& (M.getStartRoom() != me)
									&& (M.getStartRoom() != null))
								M.getStartRoom().bringMobHere(M, false);
						}
			...
			synchronized (("SYNC" + roomID()).intern()) {
				...
				for (DeadBody D : deadBodies) {
					...
					if ((M != null) && (M.getStartRoom() != null)) {
						...
						M.tell("Your corpse has been moved to "
								+ startRoom.displayText());
				...
				if (gridParent != null)
					gridParent.executeMsg(myHost, msg);
						...
						&& ((getArea() == null) || (!CMath.bset(getArea()
								.flags(), Area.FLAG_INSTANCE_CHILD)))
						&& (CMSecurity.isSaveFlag("ROOMMOBS")
								|| CMSecurity.isSaveFlag("ROOMITEMS") || CMSecurity
									.isSaveFlag("ROOMSHOPS"))) {
					...
					if (CMSecurity.isSaveFlag("ROOMMOBS")) {
						eachInhabitant(new EachApplicable<MOB>() {
							public final void apply(final MOB M) {
								if (M.isSavable()) {
									M.setStartRoom(me);
									M.text(); // this permanizes his current
												// state
								}
							}
						});
		...
		eachItem(new EachApplicable<Item>() {
			public final void apply(final Item item) {
				if (item.container() == null) {
					final ItemTicker I = (ItemTicker) CMClass
							.getAbility("ItemRejuv");
	...
	public void affectPhyStats(final Physical affected,
		...
		final int disposition = phyStats().disposition()
				& ((~(PhyStats.IS_DARK | PhyStats.IS_LIGHTSOURCE
	...
	public void affectCharStats(final MOB affectedMob,
			final CharStats affectableStats) {
	...
	public void affectCharState(final MOB affectedMob,
			final CharState affectableMaxState) {
	...
	protected String parseVariesCodes(final MOB mob, final Area A,
			final String text) {
		...
		while (aligatorDex >= 0) {
			for (final VariationCode code : VariationCode.values())
				if (text.startsWith(code.openTag, aligatorDex)) {
					...
					int y = text.indexOf(code.closeTag, aligatorDex
							+ code.openTag.length());
					if (y < 0) {
						curDex = text.length();
						y = text.length();
					...
					case 'W':
						...
						break;
					case 'C':
						addMe = A.getTimeObj().getTODCode() == code.num;
						break;
					case 'S':
						addMe = A.getTimeObj().getSeasonCode() == code.num;
						break;
					case 'M':
						addMe = ((mob != null) && (CMath.bset(mob.phyStats()
								.disposition(), code.num)));
						break;
					case 'V':
						addMe = ((mob != null) && (mob.playerStats() != null) && (mob
								.playerStats().hasVisited(this)));
						break;
					}
					if (addMe)
						buf.append(parseVariesCodes(
								mob,
								A,
								text.substring(
										aligatorDex + code.openTag.length(), y)));
	...
	public void bringMobHere(MOB mob, boolean andFollowers) {
		...
		if ((andFollowers) && (oldRoom != null)) {
			for (final Enumeration<MOB.Follower> f = mob.followers(); f
					.hasMoreElements();) {
		...
		if ((RI != null) && (CMLib.map().roomLocation(RI) == oldRoom)) {
			if ((RI.isMobileRideBasis())
				...
				// refuse is good for above, since mostly moving player stuff
				// around
			} else
				mob.setRiding(null);
		}
		if ((oldRoom != null) && (mob instanceof Rideable) && (oldRoom != this)) {
			for (final Iterator<Rider> r = ((Rideable) mob).riders(); r
					.hasNext();) {
					...
					if (((Rideable) mob).isMobileRideBasis()) {
						if ((RR instanceof MOB) && (andFollowers))
							...
							if (andFollowers)
								...
								moveItemTo((Item) RR,
										ItemPossessor.Expire.Player_Drop);
							// refuse is good for above, since mostly moving
							// player stuff around
						}
					} else
						RR.setRiding(null);
	...
	public void moveItemTo(Item item, Expire expire, Move... moveFlags) {
		...
		if ((RI != null) && (o instanceof Room)
				&& (CMLib.map().roomLocation(RI) == o)) {
			if ((RI.isMobileRideBasis())
					&& ((!(RI instanceof Item)) || (!CMLib.flags().isMobile(RI)))) {
				if (RI instanceof MOB)
		...
		if (CMParms.contains(moveFlags, Move.Followers) && (o instanceof Room)
				&& (item instanceof Rideable) && (o != this)) {
			Rider RR = null;
	...
	public Exit getReverseExit(int direction) {
		...
		if (opRoom != null)
			return opRoom
					.getExitInDir(Directions.getOpDirectionCode(direction));
		return null;
	}
	...
	public Room prepareRoomInDir(Room R, int direction) {
		...
		if (expirationDate() != 0)
			setExpirationDate(System.currentTimeMillis()
					+ WorldMap.ROOM_EXPIRATION_MILLIS);
		return this;
	}
	...
	protected void reallyReallySend(MOB source, CMMsg msg) {
		if ((Log.debugChannelOn())
				&& (CMSecurity.isDebugging(CMSecurity.DbgFlag.MESSAGES)))
			Log.debugOut(
					"StdRoom",
					((msg.source() != null) ? msg.source().ID() : "null")
							+ ":"
							+ msg.sourceCode()
							+ ":"
							+ msg.sourceMessage()
							+ "/"
							+ ((msg.target() != null) ? msg.target().ID()
									: "null") + ":" + msg.targetCode() + ":"
							+ msg.targetMessage() + "/"
							+ ((msg.tool() != null) ? msg.tool().ID() : "null")
							+ "/" + msg.othersCode() + ":"
							+ msg.othersMessage());
		...
		if (depth < 3) {
			if ((msg.trailerMsgs() != null) && (msg.trailerMsgs().size() > 0)) {
				for (CMMsg msg2 : msg.trailerMsgs())
					if ((msg != msg2)
							&& ((msg2.target() == null)
									|| (!(msg2.target() instanceof MOB)) || ((!((MOB) msg2
									.target()).amDead()) || (msg2.sourceMinor() == CMMsg.TYP_DEATH)))
	...
	public void showHappens(int allCode, String allMessage) {
		...
		CMMsg msg = CMClass.getMsg(everywhereMOB, null, null, allCode, allCode,
				allCode, allMessage);
	...
	public boolean show(MOB source, Environmental target, int allCode,
			String allMessage) {
		CMMsg msg = CMClass.getMsg(source, target, null, allCode, allCode,
				allCode, allMessage);
		if ((!CMath.bset(allCode, CMMsg.MASK_ALWAYS))
				&& (!okMessage(source, msg)))
			return false;
		send(source, msg);
		return true;
	}

	public boolean show(MOB source, Environmental target, Environmental tool,
			int allCode, String allMessage) {
		CMMsg msg = CMClass.getMsg(source, target, tool, allCode, allCode,
				allCode, allMessage);
		if ((!CMath.bset(allCode, CMMsg.MASK_ALWAYS))
				&& (!okMessage(source, msg)))
			return false;
	...
	public boolean show(MOB source, Environmental target, Environmental tool,
			int srcCode, int tarCode, int othCode, String allMessage) {
		CMMsg msg = CMClass.getMsg(source, target, tool, srcCode, tarCode,
				othCode, allMessage);
		if ((!CMath.bset(srcCode, CMMsg.MASK_ALWAYS))
				&& (!okMessage(source, msg)))
			return false;
		send(source, msg);
		return true;
	...
	public boolean show(MOB source, Environmental target, Environmental tool,
			int allCode, String srcMessage, String tarMessage, String othMessage) {
		CMMsg msg = CMClass.getMsg(source, target, tool, allCode, srcMessage,
				allCode, tarMessage, allCode, othMessage);
		if ((!CMath.bset(allCode, CMMsg.MASK_ALWAYS))
				&& (!okMessage(source, msg)))
			return false;
		send(source, msg);
	...
	public boolean show(MOB source, Environmental target, Environmental tool,
			int srcCode, String srcMessage, int tarCode, String tarMessage,
			int othCode, String othMessage) {
		CMMsg msg = CMClass.getMsg(source, target, tool, srcCode, srcMessage,
				tarCode, tarMessage, othCode, othMessage);
		if ((!CMath.bset(srcCode, CMMsg.MASK_ALWAYS))
				&& (!okMessage(source, msg)))
			return false;
		send(source, msg);
	...
	public boolean showOthers(MOB source, Environmental target, int allCode,
			String allMessage) {
		CMMsg msg = CMClass.getMsg(source, target, null, allCode, allCode,
				allCode, allMessage);
		if ((!CMath.bset(allCode, CMMsg.MASK_ALWAYS))
	...
	public boolean showOthers(MOB source, Environmental target,
			Environmental tool, int allCode, String allMessage) {
		CMMsg msg = CMClass.getMsg(source, target, tool, allCode, allCode,
				allCode, allMessage);
		if ((!CMath.bset(allCode, CMMsg.MASK_ALWAYS))
				&& (!okMessage(source, msg)))
	...
	public boolean showSource(MOB source, Environmental target, int allCode,
			String allMessage) {
		CMMsg msg = CMClass.getMsg(source, target, null, allCode, allCode,
				allCode, allMessage);
		if ((!CMath.bset(allCode, CMMsg.MASK_ALWAYS))
	...
	public boolean showSource(MOB source, Environmental target,
			Environmental tool, int allCode, String allMessage) {
		CMMsg msg = CMClass.getMsg(source, target, tool, allCode, allCode,
				allCode, allMessage);
		if ((!CMath.bset(allCode, CMMsg.MASK_ALWAYS))
	...
	public boolean isSavable() {
		return ((roomID().length() > 0)
				&& ((getArea() == null) || (!CMath.bset(getArea().flags(),
						Area.FLAG_INSTANCE_CHILD))) && (CMLib.flags()
				.isSavable(this)));
	}
	...
	public MOB fetchInhabitant(String inhabitantID) {
		if (inhabitants.size() == 0)
			return null;
		MOB mob = (MOB) CMLib.english().fetchEnvironmental(inhabitants,
				inhabitantID, true);
		if (mob == null)
			mob = (MOB) CMLib.english().fetchEnvironmental(inhabitants,
					inhabitantID, false);
		return mob;
	}

	private static final ReadOnlyVector<MOB> emptyMOBV = new ReadOnlyVector<MOB>(
			1);

	public List<MOB> fetchInhabitants(String inhabitantID) {
		if (inhabitants.size() == 0)
			return emptyMOBV;
		List inhabs = CMLib.english().fetchEnvironmentals(inhabitants,
				inhabitantID, true);
		if (inhabs.size() == 0)
			inhabs = CMLib.english().fetchEnvironmentals(inhabitants,
					inhabitantID, false);
		return inhabs;
	}
	...
	public Item findItem(String itemID) {
		if (contents.size() == 0)
			return null;
		Item item = (Item) CMLib.english().fetchEnvironmental(contents, itemID,
				true);
		if (item == null)
			item = (Item) CMLib.english().fetchEnvironmental(contents, itemID,
					false);
		return item;
	}
	...
	public Item findItem(Item goodLocation, String itemID) {
		if (contents.size() == 0)
			return null;
		Item item = CMLib.english().fetchAvailableItem(contents, itemID,
				goodLocation, Wearable.FILTER_ANY, true);
		if (item == null)
			item = CMLib.english().fetchAvailableItem(contents, itemID,
	...
	public List<Item> findItems(Item goodLocation, String itemID) {
		...
		List<Item> items = CMLib.english().fetchAvailableItems(contents,
				itemID, goodLocation, Wearable.FILTER_ANY, true);
		if (items.size() == 0)
			items = CMLib.english().fetchAvailableItems(contents, itemID,
					goodLocation, Wearable.FILTER_ANY, false);
		return items;
	}
	...
	public List<Item> findItems(String itemID) {
		if (contents.size() == 0)
			return new Vector<Item>(1);
		List items = CMLib.english()
				.fetchEnvironmentals(contents, itemID, true);
		if (items.size() == 0)
			items = CMLib.english()
					.fetchEnvironmentals(contents, itemID, false);
		return items;
	}
	...
	public void addItem(Item item, Expire expire) {
		...
		if (numMins == 0)
			...
			item.setExpirationDate(System.currentTimeMillis()
					+ (numMins * TimeManager.MILI_MINUTE));
	}
			...
			for (int e = 0; e < exits.length; e++)
				if (exits[e] == E)
					if ((this instanceof SpaceShip)
							|| (this.getArea() instanceof SpaceShip))
	...
	public PhysicalAgent fetchFromMOBRoomItemExit(MOB mob, Item goodLocation,
			String thingName, final Filterer<Environmental> filter) {
		PhysicalAgent found = null;
		String newThingName = CMLib.lang().preItemParser(thingName);
		if (newThingName != null)
			thingName = newThingName;
		boolean mineOnly = (mob != null)
				&& (thingName.toUpperCase().trim().startsWith("MY "));
		if (mineOnly)
			...
			found = mob.fetchItem(goodLocation, new Filterer<Environmental>() {
				...
				public boolean passesFilter(Environmental obj) {
					return filter.passesFilter(obj)
							&& Wearable.FILTER_UNWORNONLY.passesFilter(obj);
				}
			}, thingName);
		if ((found == null) && (!mineOnly)) {
			found = (Exit) CMLib.english().fetchEnvironmental(
					Arrays.asList(exits), thingName, true);
			if (found == null)
				found = CMLib.english().fetchAvailableItem(contents, thingName,
						goodLocation, filter, true);
			if (found == null)
				found = (Exit) CMLib.english().fetchEnvironmental(
						Arrays.asList(exits), thingName, false);
			if (found == null)
				found = CMLib.english().fetchAvailableItem(contents, thingName,
			...
			if ((found instanceof Item) // the smurfy well/gate exception
				...
				visibleItem = (Exit) CMLib.english().fetchEnvironmental(
						Arrays.asList(exits), thingName, false);
				if (visibleItem == null)
					visibleItem = fetchFromMOBRoomItemExit(null, null,
							thingName + ".2", filter);
				if (visibleItem != null)
					found = visibleItem;
			}
		...
		if ((mob != null) && (found == null)
			...
			found = mob.fetchItem(null, new Filterer<Environmental>() {
				...
				public boolean passesFilter(Environmental obj) {
					return filter.passesFilter(obj)
							&& Wearable.FILTER_WORNONLY.passesFilter(obj);
				}
			}, thingName);
		if (found == null) {
			newThingName = CMLib.lang().failedItemParser(thingName);
			if (newThingName != null)
				return fetchFromMOBRoomItemExit(mob, goodLocation,
						newThingName, filter);
		}
		return found;
	}
	...
	public PhysicalAgent fetchFromRoomFavorItems(Item goodLocation,
		...
		PhysicalAgent found = null;
		if (contents.size() > 0)
			found = (PhysicalAgent) CMLib.english().fetchAvailable(contents,
					thingName, goodLocation, Wearable.FILTER_ANY, true);
		if (found == null)
			found = (PhysicalAgent) CMLib.english().fetchAvailable(
					Arrays.asList(exits), thingName, goodLocation,
					Wearable.FILTER_ANY, true);
		if ((found == null) && (inhabitants.size() > 0))
			found = (PhysicalAgent) CMLib.english().fetchAvailable(inhabitants,
					thingName, goodLocation, Wearable.FILTER_ANY, true);
		if ((found == null) && (contents.size() > 0))
			found = (PhysicalAgent) CMLib.english().fetchAvailable(contents,
					thingName, goodLocation, Wearable.FILTER_ANY, false);
		if (found == null)
			found = (PhysicalAgent) CMLib.english().fetchAvailable(
					Arrays.asList(exits), thingName, goodLocation,
		...
		if ((found != null) // the smurfy well exception
				...
				&& (found.displayText().length() == 0)
				&& (thingName.indexOf('.') < 0)) {
			final PhysicalAgent visibleItem = fetchFromRoomFavorItems(null,
					thingName + ".2");
			if (visibleItem != null)
				found = visibleItem;
		}
	...
	public PhysicalAgent fetchFromMOBRoomFavorsItems(MOB mob,
			Item goodLocation, String thingName, Filterer<Environmental> filter) {
		return fetchFromMOBRoom(mob, goodLocation, thingName, filter, true);
	}

	public PhysicalAgent fetchFromMOBRoomFavorsMOBs(MOB mob, Item goodLocation,
			String thingName, Filterer<Environmental> filter) {
		return fetchFromMOBRoom(mob, goodLocation, thingName, filter, false);
	}

	private PhysicalAgent fetchFromMOBRoom(MOB mob, Item goodLocation,
			String thingName, final Filterer<Environmental> filter,
			boolean favorItems) {
		PhysicalAgent found = null;
		String newThingName = CMLib.lang().preItemParser(thingName);
		...
		if (found == null) {
			boolean inShip = (this instanceof SpaceShip)
					|| (this.getArea() instanceof SpaceShip);
			for (int d = 0; d < exits.length; d++)
				if ((exits[d] != null)
						&& (thingName.equalsIgnoreCase(inShip ? Directions
		...
		if (found == null) {
			newThingName = CMLib.lang().failedItemParser(thingName);
			if (newThingName != null)
				return fetchFromMOBRoom(mob, goodLocation, newThingName,
						filter, favorItems);
		}
		return found;
	}
	...
	public int pointsPerMove(MOB mob) {
		return getArea().getClimateObj().adjustMovement(phyStats().weight(),
				mob, this);
	}
	...
	public int thirstPerRound(MOB mob) {
		...
		if (CMath.bset(derivedClimate, Places.CLIMASK_WINDY))
			adjustment += 1;
		return getArea().getClimateObj().adjustWaterConsumption(
				baseThirst() + adjustment, mob, this);
	}
	...
	public Enumeration<Ability> effects() {
		return (affects == null) ? EmptyEnumeration.INSTANCE : affects
				.elements();
	}
	...
	public void addBehavior(Behavior to) {
		...
		for (final Behavior B : behaviors)
			if (B.ID().equals(to.ID()))
				return;
		if (behaviors.size() == 0)
			CMLib.threads().startTickDown(this, Tickable.TICKID_ROOM_BEHAVIOR,
					1);
	...
	public Enumeration<Behavior> behaviors() {
		return (behaviors == null) ? EmptyEnumeration.INSTANCE : behaviors
				.elements();
	}
	...
	public void addScript(ScriptingEngine S) {
		...
		if (!scripts.contains(S)) {
			for (final Enumeration<ScriptingEngine> s2 = scripts(); s2
					.hasMoreElements();) {
			...
			if (scripts.size() == 0)
				CMLib.threads().startTickDown(this,
						Tickable.TICKID_ROOM_BEHAVIOR, 1);
	...
	protected static final String[] STDCODES = { "CLASS", "DISPLAY",
			"DESCRIPTION", "TEXT", "AFFBEHAV", "IMAGE", "CLIMATE", "ATMOSPHERE" };
	...
	public void setStat(String code, String val) {
		...
		case 4: {
			delAllEffects(true);
			delAllBehaviors();
			CMLib.coffeeMaker().setExtraEnvProperties(this,
					CMLib.xml().parseAllXML(val));
			break;
		}
		case 5:
			setImage(val);
			break;
		case 6:
			setClimateType((CMath.s_int(val) < 0) ? -1 : CMath
					.s_parseBitIntExpression(Area.CLIMATE_DESCS, val));
			break;
		case 7: {
	...
	public boolean isSameRoom(Object O) {
		if (O == this)
			return true;
		if (O instanceof Room) {
			if (CMLib.map().getExtendedRoomID(this)
