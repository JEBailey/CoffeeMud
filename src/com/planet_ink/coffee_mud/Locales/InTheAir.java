...
public class InTheAir extends StdRoom {
	...
	public static void makeFall(Physical P, Room room, int avg) {
		...
		if (((P instanceof MOB) && (!CMLib.flags().isInFlight(P)))
				|| ((P instanceof Item) && (((Item) P).container() == null) && (!CMLib
						.flags().isFlying(((Item) P).ultimateContainer(null))))) {
	...
	public static boolean isOkAirAffect(Room room, CMMsg msg) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_SLEEP)
				&& (!msg.sourceMajor(CMMsg.MASK_ALWAYS))) {
			msg.source().tell("You can't sleep here.");
			return false;
		}

		if ((msg.targetMinor() == CMMsg.TYP_ENTER) && (msg.amITarget(room))) {
			MOB mob = msg.source();
			...
			if (CMLib.dice().rollPercentage() > 50)
				...
				case Climate.WEATHER_BLIZZARD:
					room.show(mob, null, CMMsg.MSG_OK_VISUAL,
							"The swirling blizzard inhibits <S-YOUPOSS> progress.");
					return false;
				case Climate.WEATHER_HAIL:
					room.show(mob, null, CMMsg.MSG_OK_VISUAL,
							"The hail storm inhibits <S-YOUPOSS> progress.");
					return false;
				case Climate.WEATHER_RAIN:
					room.show(mob, null, CMMsg.MSG_OK_VISUAL,
							"The rain storm inhibits <S-YOUPOSS> progress.");
					return false;
				case Climate.WEATHER_SLEET:
					room.show(mob, null, CMMsg.MSG_OK_VISUAL,
							"The biting sleet inhibits <S-YOUPOSS> progress.");
					return false;
				case Climate.WEATHER_THUNDERSTORM:
					room.show(mob, null, CMMsg.MSG_OK_VISUAL,
							"The thunderstorm inhibits <S-YOUPOSS> progress.");
					return false;
				case Climate.WEATHER_WINDY:
					room.show(mob, null, CMMsg.MSG_OK_VISUAL,
							"The hard winds inhibit <S-YOUPOSS> progress.");
					return false;
				}
		}
