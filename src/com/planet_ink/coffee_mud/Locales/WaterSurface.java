...
public class WaterSurface extends StdRoom implements Drink {
	...
	public void giveASky(int depth) {
		...
		if ((rawDoors()[Directions.DOWN] == null)
			...
			GridLocale sea = (GridLocale) CMClass
					.getLocale(UnderWaterLocaleID());
			sea.setRoomID("");
			sea.setArea(getArea());
			...
			for (int d = 0; d < 4; d++) {
				...
				if ((thatSea != null) && (thatSea.roomID().length() == 0)
					...
					if (thatRoom != null) {
						Exit xo = thatRoom.getRawExit(Directions
	...
	public void clearSky() {
		...
		if (room == null)
			return;
		if ((room.roomID().length() == 0) && (IsUnderWaterFatClass(room))) {
	...
	public static int isOkWaterSurfaceAffect(Room room, CMMsg msg) {
		...
		if (((msg.targetMinor() == CMMsg.TYP_LEAVE)
				|| (msg.targetMinor() == CMMsg.TYP_ENTER) || (msg.targetMinor() == CMMsg.TYP_FLEE))
				&& (msg.amITarget(room))
				&& (msg.sourceMinor() != CMMsg.TYP_RECALL)
				&& ((msg.targetMinor() == CMMsg.TYP_ENTER)
						|| (!(msg.tool() instanceof Ability)) || (!CMath.bset(
			...
			if ((!CMLib.flags().isWaterWorthy(mob)) && (!hasBoat)
				...
				&& ((msg.source().riding() == null) || (!CMLib.flags()
						.isSwimming(msg.source().riding())))) {
