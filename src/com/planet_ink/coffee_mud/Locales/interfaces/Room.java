...
public interface Room extends PhysicalAgent, ItemPossessor, Places {
	...
	public final static String[] outdoorDomainDescs = { "CITY", "WOODS",
			"ROCKY", "PLAINS", "UNDERWATER", "AIR", "WATERSURFACE", "JUNGLE",
			"SWAMP", "DESERT", "HILLS", "MOUNTAINS", "SPACEPORT" };
	...
	public final static String[] indoorDomainDescs = { "STONE", "WOODEN",
			"CAVE", "MAGIC", "UNDERWATER", "AIR", "WATERSURFACE", "METAL" };
	...
	public enum VariationCode {
		SUMMER('S', TimeClock.SEASON_SUMMER), SPRING('S',
				TimeClock.SEASON_SPRING), WINTER('S', TimeClock.SEASON_WINTER), FALL(
				'S', TimeClock.SEASON_FALL), DAY('C', TimeClock.TIME_DAY), DAYTIME(
				'C', TimeClock.TIME_DAY), NIGHT('C', TimeClock.TIME_NIGHT), NIGHTTIME(
				'C', TimeClock.TIME_NIGHT), DAWN('C', TimeClock.TIME_DAWN), DUSK(
				'C', TimeClock.TIME_DUSK), RAIN('W', Climate.WEATHER_RAIN), SLEET(
				'W', Climate.WEATHER_SLEET), SNOW('W', Climate.WEATHER_SNOW), CLEAR(
				'W', Climate.WEATHER_CLEAR), HEATWAVE('W',
				Climate.WEATHER_HEAT_WAVE), THUNDERSTORM('W',
				Climate.WEATHER_THUNDERSTORM), BLIZZARD('W',
				Climate.WEATHER_BLIZZARD), WINDY('W', Climate.WEATHER_WINDY), DROUGHT(
				'W', Climate.WEATHER_DROUGHT), DUSTSTORM('W',
				Climate.WEATHER_DUSTSTORM), COLD('W',
				Climate.WEATHER_WINTER_COLD), HAIL('W', Climate.WEATHER_HAIL), CLOUDY(
				'W', Climate.WEATHER_CLOUDY), SWIMMING('M',
				PhyStats.IS_SWIMMING), FLYING('M', PhyStats.IS_FLYING), CRAWLING(
				'M', PhyStats.IS_SITTING), SITTING('M', PhyStats.IS_SITTING), FALLING(
				'M', PhyStats.IS_FALLING), CLIMBING('M', PhyStats.IS_CLIMBING), INVISIBLE(
				'M', PhyStats.IS_INVISIBLE), HIDDEN('M', PhyStats.IS_HIDDEN), VISITED(
				'V', -1), ELSE('\n', -1), VARIES('\r', -1);
		public final char c;
