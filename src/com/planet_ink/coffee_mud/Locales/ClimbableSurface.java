...
public class ClimbableSurface extends StdRoom {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(this)
			...
			if ((!CMLib.flags().isClimbing(msg.source()))
					&& (!CMLib.flags().isFalling(msg.source()))) {
				msg.source().tell(
						"You need to climb that way, if you know how.");
				return false;
			}
		}
		return true;
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_THROW)
				...
				&& ((!(msg.tool() instanceof Rideable)) || (((Rideable) msg
						.tool()).rideBasis() != Rideable.RIDEABLE_LADDER))
				&& (!CMLib.flags().isFlying((Item) msg.tool())))
			InTheAir.makeFall((Item) msg.tool(), this, 0);
		else if ((msg.targetMinor() == CMMsg.TYP_DROP)
				&& (msg.target() instanceof Item)
				&& ((!(msg.target() instanceof Rideable)) || (((Rideable) msg
						.target()).rideBasis() != Rideable.RIDEABLE_LADDER))
			...
			if (isInhabitant(mob)) {
				if ((!CMLib.flags().isInFlight(mob))
					...
					if (!CMLib.flags().isClimbing(mob)) {
						...
						if (CMLib.flags().isClimbing(mob))
							CMLib.tracking().walk(mob, Directions.DOWN, false,
									true);
						else
