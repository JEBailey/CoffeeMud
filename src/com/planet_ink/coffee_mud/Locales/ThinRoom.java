...
public class ThinRoom implements Room {
	...
	public int getAtmosphere() {
		return (myArea != null) ? myArea.getAtmosphere()
	...
	public Room prepareRoomInDir(Room R, int direction) {
		...
		synchronized (("SYNC" + roomID).intern()) {
			...
			if (myR == null) {
				Map<String, Room> V = CMLib.database().DBReadRoomData(roomID,
						false);
				if (V.size() > 0) {
					...
					if (CMath.bset(myR.getArea().flags(), Area.FLAG_THIN))
						myR.setExpirationDate(System.currentTimeMillis()
								+ WorldMap.ROOM_EXPIRATION_MILLIS);
				}
			}
		}
		if ((myR != null) && (direction >= 0)
	...
	public CMObject copyOf() {
		try {
			...
			// CMClass.bumpCounter(E,CMClass.CMObjectType.LOCALE);//removed for
			// mem & perf
			return E;
	...
	protected static final PhyStats phyStats = (PhyStats) CMClass
