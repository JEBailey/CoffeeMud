...
public class StdThinGrid extends StdRoom implements GridLocale {
	...
	protected final SVector<ThinGridEntry> rooms = new SVector<ThinGridEntry>();
	protected static boolean tickStarted = false;

	public StdThinGrid() {
		super();
		myID = getClass().getName().substring(
	...
	private int properRoomIndex(int x, int y) {
		...
		synchronized (rooms) {
			...
			while (start <= end) {
				mid = (end + start) / 2;
				comptotal = ((long) rooms.elementAt(mid).xy.x << 31)
						+ rooms.elementAt(mid).xy.y;
	...
	private void addSortedRoom(Room R, int x, int y) {
		...
		synchronized (rooms) {
			...
			long comptotal = ((long) rooms.elementAt(pos).xy.x << 31)
					+ rooms.elementAt(pos).xy.y;
	...
	private Room getSortedRoom(int x, int y) {
		...
		synchronized (rooms) {
			int pos = properRoomIndex(x, y);
			if ((rooms.elementAt(pos).xy.x == x)
					&& (rooms.elementAt(pos).xy.y == y))
	...
	public Room prepareGridLocale(Room fromRoom, Room toRoom, int direction) {
		...
		if ((xy.x >= 0) && (xy.x < xGridSize()) && (xy.y >= 0)
				&& (xy.y < yGridSize()))
				...
				if (descriptions != null)
					if (descriptions.length > 0) {
						if ((c < 0) || (c > descriptions.length)
								|| (descriptions.length != displayTexts.length))
	...
	public XYVector getRoomXY(Room room) {
		if ((room.getGridParent() != this) && (room.getGridParent() != null))
			return null;
		try {
	...
	protected void fillExitsOfGridRoom(Room R, int x, int y) {
		...
		} else if ((rawDoors()[Directions.NORTH] != null)
				&& (exits[Directions.NORTH] != null))
			linkRoom(R, rawDoors()[Directions.NORTH], Directions.NORTH,
					exits[Directions.NORTH], exits[Directions.NORTH]);

		if (x > 0) {
			R2 = getMakeSingleGridRoom(x - 1, y);
			if (R2 != null)
				linkRoom(R, R2, Directions.WEST, ox, ox);
		} else if ((rawDoors()[Directions.WEST] != null)
				&& (exits[Directions.WEST] != null))
			linkRoom(R, rawDoors()[Directions.WEST], Directions.WEST,
					exits[Directions.WEST], exits[Directions.WEST]);
		if (y < (yGridSize() - 1)) {
			R2 = getMakeSingleGridRoom(x, y + 1);
			if (R2 != null)
				linkRoom(R, R2, Directions.SOUTH, ox, ox);
		} else if ((rawDoors()[Directions.SOUTH] != null)
				&& (exits[Directions.SOUTH] != null))
			linkRoom(R, rawDoors()[Directions.SOUTH], Directions.SOUTH,
					exits[Directions.SOUTH], exits[Directions.SOUTH]);

		if (x < (xGridSize() - 1)) {
			R2 = getMakeSingleGridRoom(x + 1, y);
			if (R2 != null)
				linkRoom(R, R2, Directions.EAST, ox, ox);
		} else if ((rawDoors()[Directions.EAST] != null)
				&& (exits[Directions.EAST] != null))
			linkRoom(R, rawDoors()[Directions.EAST], Directions.EAST,
					exits[Directions.EAST], exits[Directions.EAST]);
		...
		if (Directions.NORTHEAST < Directions.NUM_DIRECTIONS()) {
			if ((y > 0) && (x > 0)) {
				...
				linkRoom(R, rawDoors()[Directions.NORTHWEST],
						Directions.NORTHWEST, exits[Directions.NORTHWEST],
			...
			if ((x > 0) && (y < (yGridSize() - 1))) {
				...
				linkRoom(R, rawDoors()[Directions.SOUTHWEST],
						Directions.SOUTHWEST, exits[Directions.SOUTHWEST],
			...
			if ((x < (xGridSize() - 1)) && (y > 0)) {
				...
				linkRoom(R, rawDoors()[Directions.NORTHEAST],
						Directions.NORTHEAST, exits[Directions.NORTHEAST],
			...
			if ((x < (xGridSize() - 1)) && (y < (yGridSize() - 1))) {
				...
				linkRoom(R, rawDoors()[Directions.SOUTHEAST],
						Directions.SOUTHEAST, exits[Directions.SOUTHEAST],
		...
		synchronized (R.basePhyStats()) {
			R.basePhyStats().setSensesMask(
					CMath.unsetb(R.basePhyStats().sensesMask(),
							PhyStats.SENSE_ROOMGRIDSYNC));
		}
	}
	...
	public Room getAltRoomFrom(Room loc, int direction) {
		...
		for (int d = 0; d < gridexits.size(); d++) {
			WorldMap.CrossExit EX = gridexits.elementAt(d);
			if ((!EX.out) && (EX.destRoomID.equalsIgnoreCase(roomID))
					&& (EX.dir == direction) && (EX.x >= 0) && (EX.y >= 0)
					&& (EX.x < xGridSize()) && (EX.y < yGridSize()))
	...
	public List<Room> getAllRooms() {
		getRandomGridChild();
		return new ConvertingList<ThinGridEntry, Room>(rooms,
				ThinGridEntryConverter.INSTANCE);
	}

	public Iterator<Room> getExistingRooms() {
		return new ConvertingIterator<ThinGridEntry, Room>(rooms.iterator(),
				ThinGridEntryConverter.INSTANCE);
	}
	...
	protected void halfLink(Room room, Room loc, int dirCode, Exit o) {
		...
		if (room.rawDoors()[dirCode] != null) {
			if (room.rawDoors()[dirCode].getGridParent() == null)
				return;
			if (room.rawDoors()[dirCode].getGridParent().isMyGridChild(
					room.rawDoors()[dirCode]))
				return;
	...
	public Room getRandomGridChild() {
		...
		if (R == null)
			Log.errOut("StdThinGrid", roomID()
					+ " failed to get a random child!");
		return R;
	}
	...
	public Room getGridChild(String childCode) {
		...
		int y = CMath.s_int(childCode.substring(comma + 1,
				childCode.length() - 1));
		return getMakeGridRoom(x, y);
	}
			...
			if (msg.target() == this) {
				...
				if ((mob.location() != null)
					...
					msg.modify(msg.source(),
							...
							msg.tool(), msg.sourceCode(), msg.sourceMessage(),
	...
	public String[] getStatCodes() {
		return (codes != null) ? codes : (codes = CMProps.getStatCodesList(
				CMParms.appendToArray(STDCODES, MYCODES), this));
	}
}
