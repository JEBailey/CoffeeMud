...
public class StdGrid extends StdRoom implements GridLocale {
			...
			if (grid != null)
				for (int d = 0; d < gridexits.size(); d++) {
					...
					if ((!EX.out) && (EX.destRoomID.equalsIgnoreCase(roomID))
							&& (EX.dir == direction) && (EX.x >= 0)
							&& (EX.y >= 0) && (EX.x < xGridSize())
							&& (EX.y < yGridSize())
							&& (grid[EX.x][EX.y] != null))
	...
	protected void linkRoom(Room room, Room loc, int dirCode, Exit o, Exit ao) {
		...
		if (loc.rawDoors()[opCode] != null) {
			if (loc.rawDoors()[opCode].getGridParent() == null)
				return;
			if (loc.rawDoors()[opCode].getGridParent().isMyGridChild(
					loc.rawDoors()[opCode]))
				return;
	...
	protected int[] initCenterRoomAdjustsXY(int dirCode) {
		...
		if ((XY_ADJUSTMENT_CHART != null)
				&& (dirCode < XY_ADJUSTMENT_CHART.length))
	...
	protected Room findCenterRoom(int dirCode) {
		...
		while ((subMap != null) && (moveAndCheckAgain)) {
			...
			if (((xy[0] + xadjust) < xGridSize())
					&& ((xy[1] + yadjust) < yGridSize())) {
				moveAndCheckAgain = true;
				try {
					if (subMap[xy[0] + xadjust][xy[1] + yadjust] != null)
						return subMap[xy[0] + xadjust][xy[1] + yadjust];
	...
	protected void buildFinalLinks() {
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if ((dirExit == null) || (dirExit.hasADoor()))
				dirExit = ox;
			if (dirRoom != null) {
				Exit altExit = dirRoom.getRawExit(Directions
				...
				case Directions.SOUTH:
					for (int x = 0; x < subMap.length; x++)
						linkRoom(subMap[x][subMap[x].length - 1], dirRoom, d,
								dirExit, altExit);
					break;
				case Directions.NORTHEAST:
					linkRoom(subMap[subMap.length - 1][0], dirRoom, d, dirExit,
							altExit);
					break;
				case Directions.NORTHWEST:
					...
					break;
				case Directions.SOUTHEAST:
					linkRoom(subMap[subMap.length - 1][subMap[0].length - 1],
							dirRoom, d, dirExit, altExit);
					break;
				case Directions.SOUTHWEST:
					linkRoom(subMap[0][subMap[0].length - 1], dirRoom, d,
							dirExit, altExit);
					break;
				case Directions.EAST:
	...
	public Room getGridChild(int x, int y) {
		if ((subMap == null) || (x < 0) || (y < 0) || (x >= subMap.length)
				|| (y >= subMap[x].length))
			return null;
		return subMap[x][y];
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.targetMinor() == CMMsg.TYP_ENTER) && (msg.target() == this)) {
			MOB mob = msg.source();
			if ((mob.location() != null)
				...
				msg.modify(msg.source(),
						getAltRoomFrom(mob.location(), direction), msg.tool(),
