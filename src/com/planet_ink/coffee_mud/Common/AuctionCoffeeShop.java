...
public class AuctionCoffeeShop implements CoffeeShop {
	...
	public Environmental getStock(String name, MOB mob) {
		List<AuctionData> auctions = CMLib.coffeeShops().getAuctions(null,
				auctionShop);
		...
		for (int a = 0; a < auctionItems.size(); a++) {
			...
			I.setExpirationDate(CMLib.english().getContextNumber(auctionItems,
					I));
		}
		Environmental item = CMLib.english().fetchEnvironmental(auctionItems,
				name, true);
		if (item == null)
			item = CMLib.english()
					.fetchEnvironmental(auctionItems, name, false);
		return item;
	}
