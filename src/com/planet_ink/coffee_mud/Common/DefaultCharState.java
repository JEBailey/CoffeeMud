...
public class DefaultCharState implements CharState {
	...
	public void copyInto(CharState intoState) {
		if (intoState instanceof DefaultCharState) {
			...
			for (int i = 0; i < getStatCodes().length; i++)
				intoState
						.setStat(getStatCodes()[i], getStat(getStatCodes()[i]));
	}
	...
	public String getCombatStats() {
		return "H" + states[STAT_HITPOINTS] + ":M" + states[STAT_MANA] + ":V"
				+ states[STAT_MOVE] + ":F" + Fatigue;
	}
	...
	public void recoverTick(MOB mob, CharState maxState) {
		...
		double man = ((charStats.getStat(CharStats.STAT_INTELLIGENCE) + charStats
				.getStat(CharStats.STAT_WISDOM)));
		...
		double hpGain = (con > 1.0) ? ((con / 50.0) * lvlby1p5) + (con / 4.5)
				+ 2.0 : 1.0;
		double manaGain = (man > 2.0) ? ((man / 90.0) * lvlby1p5) + (man / 4.5)
				+ 2.0 : 1.0;
		double moveGain = (str > 1.0) ? ((str / 50.0) * lvl) + (str / 3.0)
				+ 5.0 : 1.0;
	...
	public void expendEnergy(final MOB mob, final CharState maxState,
			final boolean expendMovement) {
		final Room room = mob.location();
		if (room != null) {
			if (expendMovement) {
				int move = -room.pointsPerMove(mob);
				if (mob.phyStats().weight() > mob.maxCarry())
					move += (int) Math.round(CMath.mul(move, 10.0 * CMath.div(
			...
			if ((!CMLib.flags().isSleeping(mob))
				...
				if (!CMSecurity.isDisabled(CMSecurity.DisFlag.THIRST))
					adjThirst(-(room.thirstPerRound(mob) * factor),
							maxState.maxThirst(mob.baseWeight()));
			...
			if ((hungry || thirsty) && (!expendMovement)) {
				...
				if ((ticksThirsty > DEATH_THIRST_TICKS)
					...
					if (thirsty) {
						...
						else if (ticksThirsty > (DEATH_THIRST_TICKS / 2))
					...
					if (hungry) {
						...
						else if (ticksHungry > (DEATH_HUNGER_TICKS / 2))
	...
	private final static String[] CODES = { "HITS", "MANA", "MOVE", "HUNGER",
			"THIRST", "FATIGUE" };
