...
public class DefaultCMIntegerGrouper implements CMIntegerGrouper {
	...
	public String text() {
		return "{" + CMParms.toStringList(xs) + "},{"
				+ CMParms.toStringList(ys) + "}";
	}
	...
	public long random() {
		...
		Log.errOut("CMINTS", "Unable to select a random room int. Picked "
				+ which + "/" + roomCount);
		return -1;
	}
