 ...
 */
/**
 * A quest object manages the details and text for a single descriptive script
 * that is scheduled and, when directed, spawns, creates, watches, shuts down,
 * and cleans up the various objects, subsidiary quests, and existing objects
 * modifications related to this Quest.
 * 
 * To the user, a quest is a task the user must complete for reward. To the
 * Archon, a quest is something that adds content to an area at particular
 * times, or under particular circumstances.
 * 
...
public interface Quest extends Tickable, CMCommon, Modifiable {
	...
	/**
	 * Returns the unique start date of the quest. The format is either
	 * MONTH-DAY for real life dates, or MUDDAY MONTH-DAY for mudday based
	 * dates.
	 * 
	 ...
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#setStartMudDate(String)
	 * @param newName
	 *            the real-life start date of this quest
	 */
	 ...
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#setStartDate(String)
	 * @param newName
	 *            the in-game mud start date of this quest
	 */
	public void setStartMudDate(String newName);

	/**
	 * Returns the duration, in ticks of this quest. A value of 0 means the
	 * quest runs indefinitely.
	 * 
	...
	/**
	 * Sets the duration, in ticks of this quest. A value of 0 means the quest
	 * runs indefinitely.
	 * 
	...
	/**
	 * Returns whether this quest object is suspended. A suspended quest is
	...
	/**
	 * Sets whether this quest object is suspended. A suspended quest should
	 * always in a stopped state.
	 * 
	...
	/**
	 * Sets the quest script. This may be semicolon-separated instructions, or a
	 * LOAD command followed by the quest script path.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#script()
	 * @param parm
	 *            the actual quest script
	 */
	public void setScript(String parm);

	/**
	 * Accepts a pre-parsed quest script and extracts certain non-iterative
	 * variables, such as the quest name and similar variables.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest
	 * @param script
	 *            the parsed quest script
	 * @param startAtLine
	 *            which line of the script to start at
	 */
	...
	/**
	 * This will execute the quest script. If the quest is running, it will call
	 * stopQuest first to shut it down. It will spawn its subquests and
	...
	/**
	 * this will stop executing of the quest script. It will clean up any
	 * objects or mobs which may have been loaded, restoring map mobs to their
	 * previous state. If the quest is autorandom, it will restart the waiting
	 * process
	 * 
	 ...
	 * this will stop executing of the quest script. It will clean up any
	 * objects or mobs which may have been loaded, restoring map mobs to their
	 * previous state. It will then enter a stopped-paused state for the given
	 * ticks. Any start failures after that will cause the pause time to be
	 * doubled before the next try.
	 * 
	 * @param firstPauseTicks
	 *            ticks to remain in stopped state before restarting
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#startQuest()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#stepQuest()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#stopQuest()
	 */
	...
	/**
	 * If any files are embedded and cached inside this quest script, this
	 * method will clear them from resources and memory.
	 */
	public void internalQuestDelete();

	/**
	 * This method is called when a quest is done with a particular step in a
	 * multi-step quest. This method will clean up any objects from the current
	 * step or previous steps and attempt to start up the next step in the
	 * quest. If there are no more steps, or the quest is only 1 step,
	 * stopQuest() will be called.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#startQuest()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#stopQuest()
	...
	/**
	 * A dormant state is the state where a quest is no longer running, but is
	 * not, or has not yet, been scheduled to wait for another run time. This
	 * may result in a quest being deleted if it was a spawned temporary quest.
	 * 
	 ...
	 * @param truefalse
	 *            true if this quest object is a spawned copy
	 */
	...
	/**
	 * Sets the flag denoting whether this quest spawns new ones from its
	 * several steps and if so, by what method.
	 * 
	...
	/**
	 * Returns the flag denoting whether this quest spawns new ones from its
	 * several steps and if so, by what method.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#SPAWN_ANY
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#SPAWN_FIRST
	...
	/**
	 * Quest scripts can have files of various sorts embedded in them. This
	 * method will return the text of any such files of the given name, if they
	 * were embedded, or if not, it will attempt to open the file in the
	 * filesystem and return that one instead.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest
	 * @param named
	 *            the name of the resource path file to return
	 * @return the text of the file, if found.
	 */
	public StringBuffer getResourceFileData(String named);

	/**
	 * Returns the index of a room, mob, or item of the given name in use by
	 * this quest.
	 * 
	...
	/**
	 * From the given official quest variable name, it derives either an object
	 * or a vector of objects that reflect it.
	 * 
	...
	/**
	 * they are called when you want the quest engine to be aware of a a
	 * quest-specific object thats being added to the map, so that it can be
	 * cleaned up later. Ditto for abilities, affects, and behaviors. this
	 * method should only be used WHILE a quest script is being interpreted
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterBehavior(PhysicalAgent,
	 *      String, String, boolean)
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterEffect(PhysicalAgent,
	 *      String, String, boolean)
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterObject(PhysicalAgent)
	 * @param mob
	 *            the mob receiving the ability
	 * @param abilityID
	 *            the id of the ability
	 * @param parms
	 *            any ability parameters
	 * @param give
	 *            false to remove this ability, true to replace an existing one
	 */
	public void runtimeRegisterAbility(MOB mob, String abilityID, String parms,
			boolean give);

	/**
	 * Called when you want the quest engine to be aware of a quest specific
	 * object that is being added to the map, so that it can be cleaned up
	 * later. this method should only be used WHILE a quest script is being
	 * interpreted
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterAbility(MOB,
	 *      String, String, boolean)
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterBehavior(PhysicalAgent,
	 *      String, String, boolean)
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterEffect(PhysicalAgent,
	 ...
	 * Called when you want the quest engine to be aware of a quest specific
	 * object that is being added to the map, so that it can be cleaned up
	 * later. This is called to add an effect to the given object. this method
	 * should only be used WHILE a quest script is being interpreted
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterAbility(MOB,
	 *      String, String, boolean)
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterBehavior(PhysicalAgent,
	 ...
	 * @param give
	 *            false to remove this effect, true to replace an existing one
	 */
	 ...
	 * Called when you want the quest engine to be aware of a quest specific
	 * object that is being added to the map, so that it can be cleaned up
	 * later. This is called to add a behavior to the given object. this method
	 * should only be used WHILE a quest script is being interpreted
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterAbility(MOB,
	 *      String, String, boolean)
	 ...
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#runtimeRegisterObject(PhysicalAgent)
	 * @param behaving
	 *            the object receiving the behavior
	 * @param behaviorID
	 *            the id of the behavior
	 * @param parms
	 *            any behavior parameters
	 * @param give
	 *            false to remove this behavior, true to replace an existing one
	 */
	public void runtimeRegisterBehavior(PhysicalAgent behaving,
			String behaviorID, String parms, boolean give);

	/**
	 * Registers the given player name as having won this quest. The name may be
	 * prefixed with a "-" to undeclare the winner (for player deletes).
	 * 
	 ...
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#wasWinner(String)
	 * @param list
	 *            a semicolon delimtied list of player names
	 */
	...
	/**
	 * Returns the run level. -1 means runs always, otherwise, this quest will
	 * always defer to running quests of equal or lower run level. Higher,
	 * therefore, is weaker.
	 * 
	...
	/**
	 * Sets the run level. -1 means runs always, otherwise, this quest will
	 * always defer to running quests of equal or lower run level. Higher,
	 * therefore, is weaker.
	 * 
	...
	/**
	 * Returns the zappermask that determines who counts as an elligible player
	 * for the purposes of the minPlayer setting.
	 * 
	...
	/**
	 * Sets the zappermask that determines who counts as an elligible player for
	 * the purposes of the minPlayer setting.
	 * 
	 * @see com.planet_ink.coffee_mud.Libraries.interfaces.MaskingLibrary
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#setMinPlayers(int)
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#minPlayers()
	 ...
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#setWaitInterval(int)
	 * @param wait
	 *            the minimum number of ticks between attempts to run this
	 *            quest.
	 */
	public void setMinWait(int wait);

	/**
	 * Returns the maximum ticks, above the minimum wait, that must go by before
	 * an attempt to run a quest. This is therefore, the random part.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#setMinWait(int)
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#minWait()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#setWaitInterval(int)
	...
	/**
	 * Sets the maximum ticks, above the minimum wait, that must go by before an
	 * attempt to run a quest. This is therefore, the random part.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#setMinWait(int)
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#minWait()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Quest#waitInterval()
	...
	/**
	 * After a quest is added to the list of quests, this method is called to
	 * put the quest into its initial wait state, and get it thread time.
	 */
	...
	/**
	 * Returns the number of ticks before this quest will go from a running
	...
	/**
	 * Returns the number of minutes before this quest will go from a running
	 * state to a stopped state.
	 * 
	...
	/**
	 * Returns the number of ticks before this quest will attempt to start. A
	 * number >=0 means the quest is currently in a stopped state.
	 * 
	...
	/**
	 * Sets the number of ticks before this quest will attempt to start. A
	 * number >=0 means the quest is currently in a stopped state.
	 * 
	 ...
	 * @return true if the quest is successfully put into a non-running wait
	 *         state
	 */
	...
	/**
	 * A quest spawn flag denoting that this quest attempts to spawn every step
	 * at once
	 */
	...
	/**
	 * The list of BASIC non-iterative variable codes that pertain to a quest
	 * object
	 */
	public final static String[] QCODES = { "CLASS", "NAME", "DURATION",
			"WAIT", "MINPLAYERS", "PLAYERMASK", "RUNLEVEL", "DATE", "MUDDAY",
			"INTERVAL", "SPAWNABLE", "DISPLAY", "INSTRUCTIONS", "PERSISTANCE",
			"AUTHOR" };
	/**
	 * The list of basic quest objects defined in an iterative fashion during
	 * quest script execution
	 */
	public final static String[] QOBJS = { "LOADEDMOBS", "LOADEDITEMS", "AREA",
			"ROOM", "MOBGROUP", "ITEMGROUP", "ROOMGROUP", "ITEM", "ENVOBJ",
			"STUFF", "MOB" };
	/**
	 * The list of basic mystery quest objects defined in an iterative fashion
	 * during quest script execution
	 */
	public static final String[] MYSTERY_QCODES = { "FACTION", "FACTIONGROUP",
			"AGENT", "AGENTGROUP", "ACTION", "ACTIONGROUP", "TARGET",
			"TARGETGROUP", "MOTIVE", "MOTIVEGROUP", "WHEREHAPPENED",
			"WHEREHAPPENEDGROUP", "WHEREAT", "WHEREATGROUP", "WHENHAPPENED",
			"WHENHAPPENEDGROUP", "WHENAT", "WHENATGROUP", "TOOL", "TOOLGROUP" };
	/**
	 * the list of room-related mystery quest objects defined in an iterative
	 * fashion during quest script execution
	 */
	public static final String[] ROOM_REFERENCE_QCODES = { "WHEREHAPPENED",
			"WHEREHAPPENEDGROUP", "WHEREAT", "WHEREATGROUP", "ROOM",
			"ROOMGROUP" };

	/**
	 * A Quest script contains strings, and options, which are mini-quest
	 * scripts of strings.
	 * 
	 * @author bzimmerman
	 */
