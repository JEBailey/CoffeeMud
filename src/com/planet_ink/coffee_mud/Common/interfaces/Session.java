 ...
 */
/**
 * A Session object is the key interface between the internet user and their
 * player MOB. In fact, the presence of an attached session object to a MOB is
 * the only difference between an NPC MOB and a player MOB. This object handles
 * input, output, and related processes.
 */
public interface Session extends CMCommon, Modifiable, CMRunnable {

	/**
	 * Negotiates various telnet options (or attempts to), and prints the
	 * introTextStr to the user.
	 * 
	 * @param s
	 *            the socket the user connected from
	 * @param groupName
	 *            the name of the thread group the session goes to
	 * @param introTextStr
	 *            introductory text string (Hello!)
	 */
	public void initializeSession(Socket s, String groupName,
			String introTextStr);
	 ...
	 * coffeemud color code. May be from the standard list, or read from player
	 * records for a customized list.
	 * 
	...
	/**
	 * Low level text output method. Implements such features as snoops,
	 * spam-stacking, page breaks, and line caching
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#rawCharsOut(char[])
	 * @param msg
	 *            the string to send to the user
	 * @param noCache
	 *            true to disable line caching, false otherwise
	 */
	public void onlyPrint(String msg, boolean noCache);

	/**
	 * Low level text output method. Implements such features as snoops,
	 * spam-stacking. No page breaking, and Always line caching
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#onlyPrint(String,
	 *      boolean)
	 * @param msg
	 *            the string to send to the user
	 */
	...
	/**
	 * Lowest level user-output method. Does nothing but send the string to the
	 * user, period.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#rawCharsOut(char[])
	 * @param msg
	...
	/**
	 * Low level line-output method. Sets the prompt flag after write, and
	 * inserts additional pre-linefeed if currently at the prompt. Adds post
	 * linefeed of course. Does not do a page break.
	 * 
	...
	/**
	 * Low level line-output method. Sets the prompt flag after write, and
	 * inserts additional pre-linefeed if currently at the prompt.
	 * 
	...
	/**
	 * Higher-level line output method. Does full filtering of special
	...
	/**
	 * Higher-level line output method. Does full filtering of special
	 * characters and codes using given variable values.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#rawPrint(String)
	 * @param Source
	 *            variable for special code parsing: Source
	 * @param Target
	 *            variable for special code parsing: Target
	 * @param Tool
	 *            variable for special code parsing: Tool
	 * @param msg
	 *            the string to send to the user
	 */
	public void stdPrint(Physical Source, Environmental Target,
			Environmental Tool, String msg);
	 ...
	 *            the string to send to the user
	 */
	public void stdPrintln(Physical Source, Environmental Target,
			Environmental Tool, String msg);
	 ...
	 * 
	 * @param c
	 *            string (as char array) to send out to the user
	 */
	public void rawCharsOut(char[] c);

	/**
	 * Checks whether this session is currently over its time limit trying to
	 * write data to its socket. For some reason this happens, and this method
	 * detects it.
	 * 
	...
	/**
	 * Returns true if this session is merely a placeholder, and does not
	 * represent an actual user connection. Will ensure that the system
	 * continues to treat the user as a mob.
	 * 
	...
	/**
	 * Returns whether this session is engaged in a login/account menu
	 * transaction. If a loginsession is passed it, it will return true only if
	 * the two sessions are engaging the same login name.
	 * 
	 * @param loginObj
	 *            another login session to potentially match
	 * @return true if it is pending, false otherwise
	 */
	public boolean isPendingLogin(
			final CharCreationLibrary.LoginSession loginObj);

	/**
	 * Medium-level text output method. Does full filtering of special
	 * characters and codes. Does not manage the prompt, and should NOT be used
	 * for prompts.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#onlyPrint(String,
	 *      boolean)
	...
	/**
	 * Medium-level text output method. Does full filtering of special
	 * characters and codes. Does not manage the prompt. Should be used for
	 * prompts!
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#onlyPrint(String,
	 *      boolean)
	...
	/**
	 * Medium-level text output method. Does full filtering of special
	 * characters and codes using given variable values. Does not manage the
	 * prompt.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#onlyPrint(String,
	 *      boolean)
	 ...
	 *            the string to send to the user
	 */
	public void print(Physical Source, Environmental Target,
			Environmental Tool, String msg);

	/**
	 * Medium-level text output method. Does full filtering of special
	 * characters and codes. Does not manage the prompt. Adds a linefeed at the
	 * end though.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#onlyPrint(String,
	 *      boolean)
	 ...
	 * Medium-level text output method. Does full filtering of special
	 * characters and codes using given variable values. Does not manage the
	 * prompt. Adds a linefeed at the end though.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#onlyPrint(String,
	 *      boolean)
	 * @param Source
	 ...
	 *            the string to send to the user
	 */
	public void println(Physical Source, Environmental Target,
			Environmental Tool, String msg);

	/**
	 * Notifies this session to output the users prompt again once it has
	 * reached a suitable lapse in text output.
	 * 
	 * @param truefalse
	 *            true to send another prompt, false otherwise
	 */
	public void setPromptFlag(boolean truefalse);

	/**
	 * Medium-level text output method. Does full filtering of special
	 * characters and codes. DOES manage the prompt, but turns OFF word wrap!
	...
	/**
	 * Lower-Medium-level text output method. Does only the parsing of color
	 * codes, no word wrapping, no codes. Adds a linefeed at the end.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#onlyPrint(String,
	 *      boolean)
	 * @param msg
	...
	/**
	 * Lower-Medium-level text output method. Does only the parsing of color
	 * codes, no word wrapping, no codes.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#onlyPrint(String,
	 *      boolean)
	 * @param msg
	...
	/**
	 * Waits the given milliseconds for a key to be pressed, after which it
	 * returns that key, or \0 if nothing pressed.
	 * 
	 * @param maxWait
	...
	/**
	 * Prompts the user to enter a string, and then returns what the enter. Does
	 * not time out, but may throw an exception on disconnnect.
	 * 
	 ...
	 *             a disconnect
	 */
	public String prompt(String Message, String Default) throws IOException;

	/**
	 * Puts the session into an input state, returning immediately. The given
	 * callback will be made when the ENTER key has been hit, and the session
	 * will go back into a prompt state. This is basically for non-blocking
	 * input.
	 * 
	 * @see InputCallback
	 * @param callBack
	 *            the callback to modify and make when done
	 */
	public void prompt(InputCallback callBack);

	/**
	 * Prompts the user to enter a string, and then returns what the enter.
	 * Possibly times out, and may throw an exception on disconnnect or time
	 * out.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#prompt(String)
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#prompt(String,
	 ...
	 *            the default response if the user just hits enter
	 * @param maxTime
	 *            max number of milliseconds to wait before timing out
	 * @return the string entered by the user, or the Default
	 * @throws IOException
	 *             a disconnect or time out
	 */
	 ...
	 *             a disconnect or time out
	 */
	public String prompt(final String Message, long maxTime) throws IOException;

	/**
	 * Prompts the user to enter Y or N, and returns what they enter. Will not
	 * time out, but may throw an exception on disconnect.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#confirm(String,
	 *      String, long)
	 * @param Message
	 *            the prompt message to display to the user
	 * @param Default
	...
	/**
	 * Prompts the user to enter Y or N, and returns what they enter. Possibly
	 * times out, and may throw an exception on disconnnect or time out.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#confirm(String,
	 *      String)
	 * @param Message
	...
	/**
	 * Prompts the user to enter one character responses from a set of valid
	 * choices. Repeats the prompt if the user does not enter a valid choice.
	 * ENTER is a valid choice for Default. Does not time out, but may throw an
	 * exception on disconnnect.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#choose(String,
	 *      String, String, long)
	 * @param Message
	 *            the prompt message to display to the user
	 * @param Choices
	 *            a list of uppercase characters that may be entered
	 * @param Default
	 *            the default response if the user just hits enter
	 ...
	 *             a disconnect
	 */
	public String choose(final String Message, final String Choices,
			final String Default) throws IOException;
	 ...
	 *            max number of milliseconds to wait before timing out
	 * @param paramsOut
	 *            an empty list to put any extra crap added to the end of the
	 *            choice.
	 * @return the character entered from the choices
	 * @throws IOException
	 *             a disconnect
	 */
	public String choose(final String Message, final String Choices,
			final String Default, long maxTime, List<String> paramsOut)
			throws IOException;

	/**
	 * Prompts the user to enter one character responses from a set of valid
	 * choices. Repeats the prompt if the user does not enter a valid choice.
	 * ENTER is a valid choice for Default. May time out, and may throw an
	 * exception on disconnnect.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#choose(String,
	 *      String, String)
	 ...
	 */
	public String choose(final String Message, final String Choices,
			final String Default, long maxTime) throws IOException;

	/**
	 * Notifies this session that the given session is snooping it. This session
	 * will manage said snooping.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#setBeingSnoopedBy(Session,
	 *      boolean)
	 ...
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#snoopSuspension(int)
	 * @param S
	 *            the session to check for a snoop on me.
	 * @return true if the given session is snooping on me, false otherwise
	 */
	public boolean isBeingSnoopedBy(Session S);

	/**
	 * Increments or decrements the snoop suspension counter by the given
	 * amount. Only when the counter is 0 does snooping of this session actually
	 * occur.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#setBeingSnoopedBy(Session,
	 *      boolean)
	 ...
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#snoopSuspension(int)
	 * @param change
	 *            the amount to change the snoop suspension counter by
	 * @return the current value of the snoop suspension counter after the
	 *         change
	 */
	public int snoopSuspension(int change);

	/**
	 * Force the current player to logoff, end the session, and/or kill the
	 * thread.
	 * 
	 * @param removeMOB
	 *            true to remove the mob from the game
	 * @param dropSession
	 *            true to force closed sockets, and removed session
	 * @param killThread
	 *            true to force a thread death, and false to be more lenient
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#isStopped()
	 */
	public void stopSession(boolean removeMOB, boolean dropSession,
			boolean killThread);
	...
	/**
	 * Allows the user to select a different character, taking them back to the
	 * login prompt, or to the account character listing screen, whichever is
	 * appropriate.
	 * 
	 * @param removeMOB
	 *            true to remove the mob from the game
	...
	/**
	 * Blocks the current thread until the user attached to this session hits
	 * ENTER, returning the characters they enter. Completely filtered input.
	 * 
	 * @param timeoutMillis
	 *            milliseconds before InterruptedIOException thrown
	 * @return the string entered by the user
	 * @throws IOException
	 *             any exception generated during input
	 */
	public String blockingIn(long timeoutMillis) throws IOException;

	/**
	 * Blocks for a short amount of time, returning an input string only if the
	 * user happens to have hit enter.
	 * 
	 * @return a string entered by the user
	 * @throws IOException
	 *             exceptions thrown, typically a timeout
	 */
	public String readlineContinue() throws IOException, SocketException;

	/**
	 * Returns a pre-parsed, pre-filtered Vector of strings representing the
	 * last command entered by the user through this session.
	 * 
	 ...
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#mob()
	 * @param account
	 *            the player account attached to this session object.
	 */
	...
	/**
	 * Gets the number of milliseconds since a user entry was registered by this
	 * session
	 * 
	...
	/**
	 * Gets the milliseconds timestamp since this user last registered a pk
	 * fight.
	 * 
	...
	/**
	 * Sets now as the milliseconds timestamp since this user last registered a
	 * pk fight.
	 * 
	...
	/**
	 * Gets the milliseconds timestamp since this user last registered a npc
	 * fight.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#setLastPKFight()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#setLastNPCFight()
	...
	/**
	 * Returns the last time in milliseconds that this session began its input
	 * loop. Is typically only held up by executing a user command, so it is an
	 * accurate gauge of a locked up user command.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#setInputLoopTime()
	 * 
	 * @return the last time in milliseconds that this session began its input
	 *         loop
	 */
	public long getInputLoopTime();

	/**
	 * Sets the last time in milliseconds that this session began its input
	 * loop. Is typically only held up by executing a user command, so it is an
	 * accurate gauge of a locked up user command.
	 * 
	...
	/**
	 * Returns a Vector of the last several message strings received by this
	 * user. All are already previously filtered and parsed and ready to
	 * display.
	 * 
	 ...
	 *            the telnet code
	 * @param onOff
	 *            true to turn on, false to turn off the flag
	 */
	 ...
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#TELNET_ANSI
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#setServerTelnetMode(int,
	 *      boolean)
	 * @param telnetCode
	 *            the telnet code
	 * 
	 ...
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#TELNET_ANSI
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#setClientTelnetMode(int,
	 *      boolean)
	 * 
	 * @param telnetCode
	 *            the telnet code
	...
	/**
	 * Change telnet mode by sending the appropriate command to the clients
	 * client. A response received later will trigger mode changed.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#TELNET_ANSI
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#setClientTelnetMode(int,
	 *      boolean)
	...
	/**
	 * Initializes a telnet mode between this session and the connected client
	 * by negotiating certain fundamental flags, like ANSI, MXP, and MSP. It
	 * will use a bitmap of MOB flags passed in as a guide.
	 * 
	 * @see com.planet_ink.coffee_mud.MOBS.interfaces.MOB#getBitmap()
	 * @see com.planet_ink.coffee_mud.MOBS.interfaces.MOB#ATT_MXP
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Session#changeTelnetMode(int,
	 *      boolean)
	 * @param mobbitmap
	 *            the mobbitmap the bitmap of mob flags to use as a guide in
	 *            negotiation
	 */
	 ...
	 * 
	 * @param eventName
	 *            the event name, like comm.channel
	 * @param json
	 *            the json doc, like {"blah":"BLAH"}
	 */
	...
	/**
	 * TELNET CODE: Indicates that what follows is subnegotiation of the
	 * indicated option
	 */
	public static final int TELNET_SB = 250;
	/**
	 * TELNET CODE: Indicates the desire to begin performing, or confirmation
	 * that you are now performing, the indicated option
	 */
	public static final int TELNET_WILL = 251;
	/**
	 * TELNET CODE: Indicates the refusal to perform, or continue performing,
	 * the indicated option
	 */
	public static final int TELNET_WONT = 252;
	/**
	 * TELNET CODE: Indicates the request that the other party perform, or
	...
	/**
	 * TELNET CODE: Indicates the demand that the other party stop performing,
	 * or confirmation that you are no longer expecting the other party to
	 * perform, the indicated option.
	 */
	public static final int TELNET_DONT = 254;
	/**
	 * TELNET CODE: Indicates that the other party can go ahead and transmit --
	 * I'm done.
	 */
	...
	/**
	 * Array String-friendly descriptions of the various telnet codes. Indexed
	 * by code id 0-255
	 */
	public static final String[] TELNET_DESCS = { "BINARY", "ECHO", "2",
			"SUPRESS GO AHEAD", "4", "5", "6", "7", "8",
			"9", // 0-9
			"10", "11", "12", "13", "14", "15", "16", "17", "LOGOUT",
			"19", // 10-19
			"20", "21", "22", "23", "TERMTYPE", "25", "26", "27", "28",
			"29", // 20-29
			"30", "NAWS", "32", "FLOWCONTROL", "LINEMODE", "35", "36", "37",
			"38", "39", // 30-39
	...
	public final static byte[] TELNETBYTES_GMCP_HEAD = new byte[] {
			(byte) Session.TELNET_IAC, (byte) Session.TELNET_SB,
			(byte) Session.TELNET_GMCP };
	/** How to end a telnet subnegotiation */
	public final static byte[] TELNETBYTES_END_SB = new byte[] {
			(byte) Session.TELNET_IAC, (byte) Session.TELNET_SE };

	/**
	 * The internal class to managing asynchronous user input. This class
	 * supports three types of input: open text (PROMPT), one-letter options
	 * (CHOOSE), and Y/N (CONFIRM).
	 * 
	 * @author Bo Zimmerman
	 * 
	 */
	public static abstract class InputCallback {
		/**
		 * The threa different types of user input processing supported by the
		 * abstract InputCallback class
		 * 
		 * @author Bo Zimmerman
		 * 
		 */
		...
		/**
		 * Full constructor. Receives the Type of processing, a default input
		 * for when the user just hits ENTER, a list of one-character choices
		 * (if CHOOSE Type is used) and an amount of time, in ms, for the user
		 * to be given before timeout and the timedOut() method is called.
		 * 
		 * @param type
		 ...
		 *            default input value
		 * @param choicesStr
		 *            list of one-character options (if CHOOSE Type)
		 * @param timeoutMs
		 *            time, in ms, before the user is kicked
		 */
		public InputCallback(Type type, String defaultInput, String choicesStr,
				long timeoutMs) {
			this.type = type;
		 ...
		 * when the user just hits ENTER, and an amount of time, in ms, for the
		 * user to be given before timeout and the timedOut() method is called.
		 * 
		 * @param type
		 *            the type of processing
		 * @param defaultInput
		...
		/**
		 * Constructor. Receives the Type of processing, and an amount of time,
		 * in ms, for the user to be given before timeout and the timedOut()
		 * method is called.
		 * 
		 * @param type
		 *            the type of processing
		...
		/**
		 * Returns true if a timeout was given, and this class has been active
		 * longer than that amount of time.
		 * 
		 * @return true if timed out, false otherwise.
		 */
		public boolean isTimedOut() {
			boolean isTimedOut = (timeout > 0)
		...
		/**
		 * This method allows reuse of a given InputCallback. It will re-start
		 * the timeout period, and flag the callback for requiring more input.
		 * 
		 * @return this
		 */
		...
		/**
		 * This method is called by InputCallback before user input is
		 * requested. If a CHOOSE or CONFIRM type is used, and the user enters
		 * something unrecognized, then this method is called again before more
		 * input.
		 */
		public abstract void showPrompt();

		/**
		 * This method is call by InputCallback if a timeout value > 0 is given
		 * and that amount of time has been exceeded.
		 */
		public abstract void timedOut();

		/**
		 * This method is called if the user hits ENTER, and their input data is
		 * valid (one of the choices for CHOOSE or CONFIRM), or anything else
		 * for PROMPT. If the user has entered nothing, and a default value has
		 * been given, the default value is returned. The user entry is
		 * available to this method in this.input variable.
		 */
	...
	public abstract class TickingCallback extends InputCallback {
		 ...
		 * 
		 * @param tickerMs
		 *            the time is ms between timeouts
		 */
		 ...
		 * This is also where you control the process.
		 * 
		 * @param counter
	...
	/**
	 * Some sort of TELNET protocol constant related to LINEMODE -- I've no idea
	 * what it does
	 */
	...
	public static enum SessionStatus {
		HANDSHAKE_OPEN, HANDSHAKE_MCCP, HANDSHAKE_MXP, HANDSHAKE_MXPPAUSE, HANDSHAKE_DONE, LOGIN, LOGIN2, ACCOUNT_MENU, CHARCREATE, IDLE, MAINLOOP, LOGOUT, LOGOUT1, LOGOUT2, LOGOUT3, LOGOUT4, LOGOUT5, LOGOUT6, LOGOUT7, LOGOUT8, LOGOUT9, LOGOUT10, LOGOUT11, LOGOUT12, LOGOUTFINAL
	}
}
