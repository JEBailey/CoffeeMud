 ...
 * 
 *      Source The source of any message must always be a valid reference to an
 *      instance of the MOB interface. In short, all events that occur in the
 *      system are a direct result of the activity of a MOB. This is on the
 *      theory that the universe is controlled and governed by sentience. In the
 *      extremely rare instances where a mob is not readily available to provide
 *      a message source, one should be instantiated -- even if it is just a
 *      blank, new StdMOB.
 * 
 *      Target The target of a message may be null, or any valid reference to an
 *      instance of the Environmental interface, which includes Items, MOBs,
 *      Rooms, Exits, etc. The type and context of message you wish to generate
 *      will typically tell you intuitively whether the source is doing
 *      something to someone or something else, or is acting independently. This
 *      is usually another mob or an item, but you will find examples of all
 *      kinds of targets in the code.
 * 
 *      Tool The tool of a message may be null, or any valid reference to an
 *      instance of the Environmental interface, which includes Items,
 ...
 *      used), or an Item object (like a weapon in an attack event).
 * 
 *      Source Code This is an encoded integer which represents what the source
 *      MOB is actually doing. Codes are separable into major and minor codes,
 *      with the major code being a bitmask and the minor code being a constant
 *      integer representing the event type. See below.
 * 
 *      Source Message This is the string which the source MOB will see should
 *      the event occur successfully.
 * 
 *      Target Code This is an encoded integer which represents what is
 *      happening to the target. If there is no target, this number will
 *      typically have the value of 0 (CMMsg.NOEFFECT). Codes are separable into
 *      major and minor codes, with the major code being a bitmask and the minor
 *      code being a constant integer representing the event type. See below.
 * 
 *      Target Message This is the string which the target MOB (if it is a MOB)
 *      will see should the event occur successfully. If there is no target,
 *      this string is null.
 * 
 *      Others Code This is an encoded integer which represents how any other
 *      objects (such as MOBs, Items, Rooms, Exits) other than the source and
 *      target, in the same room, perceive the event. If the event is completely
 *      imperceptible by anything other than the source, it may be 0
 *      (CMMsg.NOEFFECT) Codes are separable into major and minor codes, with
 *      the major code being a bitmask and the minor code being a constant
 *      integer representing the event type. See below.
 * 
 *      Others Message This is the string which other MOBs in the same room as
 *      the source and target MOBs will see should the event occur successfully.
 *      If the event is completely imperceptible by other MOBs, it may be null.
 * 
 * 
 *      ** * ** * ** * ** * ** * ** * ** * ** * ** * ** * ** * ** * ** * ** * **
 *      * ** * ** * ** * ** You should also familiarize yourself with the
 *      interface that handles these events
 * @see com.planet_ink.coffee_mud.core.interfaces.MsgListener#okMessage(Environmental,
 *      CMMsg)
 * @see com.planet_ink.coffee_mud.core.interfaces.MsgListener#executeMsg(Environmental,
 *      CMMsg)
...
public interface CMMsg extends CMCommon {
	 ...
	 * about how this event affects or is perceived by the target of the action.
	 * 
	...
	/**
	 * Returns the string seen by the target of the event, and only by the
	 * target of the event.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#target()
	...
	/**
	 * Sets the string seen by the target of the event, and only by the target
	 * of the event.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#target()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#targetMessage()
	...
	/**
	 * Returns whether the given code or mask is either the minor code or a part
	 * of the major code of the targetCode
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#targetCode()
	 * @param codeOrMask
	...
	/**
	 * Returns whether the given code or mask string matches the minor code or a
	 * part of the major code of the targetCode.
	 * 
	...
	/**
	 * Returns whether the given Environmental object is, in fact, the target of
	 * this message.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#target()
	...
	/**
	 * A combination bitmask, action type integer that describes information
	 * about how this event affects or is perceived by the source of the action.
	 * 
	...
	/**
	 * Returns whether the given code or mask is either the minor code or a part
	 * of the major code of the sourceCode
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#sourceCode()
	 * @param codeOrMask
	...
	/**
	 * Returns whether the given code or mask string matches the minor code or a
	 * part of the major code of the sourceCode.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#TYPE_DESCS
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#MASK_DESCS
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#sourceCode()
	...
	/**
	 * Returns whether the given Environmental object is, in fact, the source of
	 * this message.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#source()
	 * @see com.planet_ink.coffee_mud.core.interfaces.Environmental
	...
	/**
	 * Returns the string seen by the source of the event, and only by the
	 * source of the event.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#source()
	...
	/**
	 * Sets the string seen by the source of the event, and only by the source
	 * of the event.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#source()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#sourceMessage()
	...
	/**
	 * Returns high order bitmask for the others code (Others are everyone not
	 * source, not target)
	 * 
	...
	/**
	 * Returns whether high order bitmask for the others code is set (Others are
	 * everyone not source, not target)
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#othersCode()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#othersMinor()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#MASK_MAGIC
	...
	/**
	 * Returns low order action type integer for the others code (Others are
	 * everyone not source, not target)
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#othersCode()
	...
	/**
	 * A combination bitmask, action type integer that describes information
	 * about how this event affects or is perceived by the others of the action.
	...
	/**
	 * Returns whether the given code or mask is either the minor code or a part
	 * of the major code of the othersCode
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#othersCode()
	 * @param codeOrMask
	...
	/**
	 * Returns whether the given code or mask string matches the minor code or a
	 * part of the major code of the othersCode. (Others are everyone not
	 * source, not target)
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#TYPE_DESCS
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#MASK_DESCS
	...
	/**
	 * Returns the string seen by the others of the event, and only by the
	 * others of the event. (Others are everyone not source, not target)
	 * 
	...
	/**
	 * Sets the string seen by the others of the event, and only by the others
	 * of the event. (Others are everyone not source, not target)
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#othersMessage()
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg#othersCode()
	 * @param str
	...
	/**
	 * Returns whether the given Environmental object is neither the source nor
	 ...
	 * @param E
	 *            the Environmental to inspect, may not be null
	 * @return whether the given E is neither the source nor target of this
	 *         message
	 */
	...
	/**
	 * Returns the arbitrary value integer associated with this event. Values
	 * tend to be defined in a event-action code specific way, so that it will
	 * mean nothing most of the time, and something different for each type of
	 * message. For custom messages therefore, it can be used for whatever one
	 * pleases, whereas for established codebase message types, you should
	 * understand its meaning before doing anything with it.
	 * 
	...
	/**
	 * Sets an arbitrary integer value associated with this event. Values tend
	 * to be defined in a event-action code specific way, so that it will mean
	 * nothing most of the time, and something different for each type of
	 * message. For custom messages therefore, it can be used for whatever one
	 * pleases, whereas for established codebase message types, you should
	 * understand its meaning before doing anything with it.
	 * 
	 ...
	 * @param newAllCode
	 *            the source, target, and others code
	 * @param allMessage
	 *            the source, target, and others message
	 */
	public void modify(final MOB source, final Environmental target,
	 ...
	 *            the new source of this event
	 * @param newAllCode
	 *            the new source, target, and others code of this event
	 * @param allMessage
	 *            the new source, target, and others message of this event
	 */
	public void modify(final MOB source, final int newAllCode,
	 ...
	 */
	public void modify(final MOB source, final Environmental target,
			final Environmental tool, final int newAllCode,
			final String allMessage);
	 ...
	 *            the new source, target, and others code of this event
	 * @param sourceMessage
	 *            the new source message for this event
	 * @param targetMessage
	 *            the new target message for this event
	 * @param othersMessage
	 *            the new others message for this event
	 */
	public void modify(final MOB source, final Environmental target,
			final Environmental tool, final int newAllCode,
			final String sourceMessage, final String targetMessage,
	 ...
	 *            the new tool for this event
	 * @param newSourceCode
	 *            the new source code for this event
	 * @param sourceMessage
	 *            the new source message for this event
	 * @param newTargetCode
	 *            the new target code for this event
	 * @param targetMessage
	 *            the new target message for this event
	 * @param newOthersCode
	 *            the new others code for this event
	 * @param othersMessage
	 *            the new others message for this event
	 */
	public void modify(final MOB source, final Environmental target,
			final Environmental tool, final int newSourceCode,
			final String sourceMessage, final int newTargetCode,
			final String targetMessage, final int newOthersCode,
			final String othersMessage);
	...
	public void modify(final MOB source, final Environmental target,
			final Environmental tool, final int newSourceCode,
			final int newTargetCode, final int newOthersCode,
			final String allMessage);

	/**
	 * Returns a List of other CMMsg events which are slated to be confirmed and
	 * executed AFTER this current message is handled. This is implemented by
	 * the Room object
	 * 
	 * @see com.planet_ink.coffee_mud.Locales.interfaces.Room#send(MOB, CMMsg)
	...
	/**
	 * Appends to the Vector of other CMMsg events which are slated to be
	 * confirmed and executed AFTER this current message is handled. This is
	 * implemented by the Room object
	 * 
	 * @see com.planet_ink.coffee_mud.Locales.interfaces.Room#send(MOB, CMMsg)
	 * @param msg
	...
	/**
	 * Mask to remove the MAJOR_MASK from the source, target, or others code,
	 * leaving only TYPE
	 */
	public static final int MINOR_MASK = 2047;
	/**
	 * Mask to remove the TYPE CODE from the source, target, or others code,
	 * leaving only MASK bits
	 */
	public static final int MAJOR_MASK = 2147482624;

	// masks for all messages
	/**
	 * MAJOR_MASK bit denoting a source, target, or others code does small hand
	 * movements
	 */
	public static final int MASK_HANDS = 2048; // small hand movements
	/**
	 * MAJOR_MASK bit denoting a source, target, or others code does large body
	 * movements (travel)
	 */
	public static final int MASK_MOVE = 4096; // large body movements (travel)
	/**
	 * MAJOR_MASK bit denoting a source, target, or others code does looking and
	 * seeing
	 */
	public static final int MASK_EYES = 8192; // looking and seeing
	/**
	 * MAJOR_MASK bit denoting a source, target, or others code does speaking
	 * and eating
	 */
	public static final int MASK_MOUTH = 16384; // speaking and eating
	/**
	 * MAJOR_MASK bit denoting a source, target, or others code does general
	 * body noises
	 */
	public static final int MASK_SOUND = 32768; // general body noises
	/**
	 * MAJOR_MASK bit denoting a source, target, or others code is always
	 * confirmed (involuntary usually)
	 */
	...
	/**
	 * MAJOR_MASK bit denoting a source, target, or others code is thiefly,
	 * delicate
	 */
	public static final int MASK_DELICATE = 262144; // for thief skills!
	/**
	 * MAJOR_MASK bit denoting a source, target, or others code is malicious,
	 * harmful to the target
	 */
	public static final int MASK_MALICIOUS = 524288; // for attacking
	/**
	 * MAJOR_MASK bit denoting a source, target, or others code is a channel
	 * message
	 */
	public static final int MASK_CHANNEL = 1048576; // for channel messages
	/**
	 * MAJOR_MASK bit denoting an event that will be repeated, in order to
	 * optimize execution
	 */
	public static final int MASK_OPTIMIZE = 2097152; // to optimize a repeated
														// msg
	/**
	 * MAJOR_MASK bit denoting an event that is for system processing, and not
	 * observable
	 */
	public static final int MASK_CNTRLMSG = 4194304; // to denote an internal
														// only control message
	/**
	 * MAJOR_MASK bit denoting an event that is for system processing, and not
	 * observable
	 */
	public static final int MASK_INTERMSG = 8388608; // to denote an
														// intermediate (not
														// final goal) message
	...
	/**
	 * MINOR_MASK minor action code type, denoting a noisy, large movement
	 * action
	 */
	public static final int TYP_NOISYMOVEMENT = 21;
	/**
	 * MINOR_MASK minor action code type, denoting a quiet, large movement
	 * action
	 */
	...
	/**
	 * MINOR_MASK minor action code type, denoting a general noisy movement
	 * action
	 */
	public static final int TYP_OK_ACTION = 53;
	/**
	 * MINOR_MASK minor action code type, denoting a general non-noisy movement
	 * action
	 */
	...
	/**
	 * MINOR_MASK minor action code type, denoting a small hands, non-movement
	 * action
	 */
	...
	/**
	 * MINOR_MASK minor action code type, denoting a missile weapon reload
	 * action
	 */
	...
	/**
	 * MINOR_MASK minor action code type, denoting an item/room expiration
	 * action
	 */
	...
	/**
	 * MINOR_MASK minor action code type, denoting a missile weapon unload
	 * action
	 */
	...
	/**
	 * MINOR_MASK minor action code type, denoting possession of one body by
	 * another
	 */
	public static final int TYP_POSSESS = 105;
	/**
	 * MINOR_MASK minor action code type, denoting dispossession of a possessed
	 * body
	 */
	...
	/**
	 * MINOR_MASK minor action code type, denoting a specific glance at the
	 * exits
	 */
	...
	/**
	 * MINOR_MASK minor action code type, denoting a channel action -- 2000-2047
	 * are channels
	 */
	public static final int TYP_CHANNEL = 2000; // (2000-2047 are channels)

	/**
	 * Addendum to TYPE_DESCS for codes above the last index (channels only at
	 * this point)
	 */
	public static final Object[][] MISC_DESCS = { { "CHANNEL",
			Integer.valueOf(2000), Integer.valueOf(2047) }, };

	/** Index string descriptions of all the MINOR_MASK action code TYP_s */
	public static final String[] TYPE_DESCS = { "NOTHING", "AREAAFFECT",
			"PUSH", "PULL", "RECALL", "OPEN", "CLOSE", "PUT", "GET", "UNLOCK",
			"LOCK", "WIELD", "GIVE", "BUY", "SELL", "DROP", "WEAR", "FILL",
			"DELICATE_HANDS_ACT", "VALUE", "HOLD", "NOISYMOVEMENT",
			"QUIETMOVEMENT", "WEAPONATTACK", "LOOK", "READ", "NOISE", "SPEAK",
			"CAST_SPELL", "LIST", "EAT", "ENTER", "FOLLOW", "LEAVE", "SLEEP",
			"SIT", "STAND", "FLEE", "NOFOLLOW", "WRITE", "FIRE", "COLD",
			"WATER", "GAS", "MIND", "GENERAL", "JUSTICE", "ACID", "ELECTRIC",
			"POISON", "UNDEAD", "MOUNT", "DISMOUNT", "OK_ACTION", "OK_VISUAL",
			"DRINK", "HANDS", "PARALYZE", "WAND_USE", "SERVE", "REBUKE",
			"ADVANCE", "DISEASE", "DEATH", "DEPOSIT", "WITHDRAW", "EMOTE",
			"QUIT", "SHUTDOWN", "VIEW", "RETIRE", "RETREAT", "PANIC", "THROW",
			"EXTINGUISH", "TELL", "SITMOVE", "KNOCK", "PRACTICE", "TEACH",
			"REMOVE", "EXPCHANGE", "DAMAGE", "HEALING", "ROOMRESET", "RELOAD",
			"SNIFF", "ACTIVATE", "DEACTIVATE", "FACTIONCHANGE", "LOGIN",
			"LEVEL", "EXAMINE", "ORDER", "EXPIRE", "BORROW", "HUH", "LIFE",
			"BID", "CLANEVENT", "UNLOAD", "DUELCHALLENGE", "LEGALWARRANT",
			"DIG", "PREINVOKE", "POSSESS", "DISPOSSESS", "POWERCURRENT",
			"CONTEMPLATE", "POUR", "LOOKEXITS", "LASER", "SONIC" };

	/** Index string descriptions of all the MAJOR_MASK code MAKS_s */
	public static final String[] MASK_DESCS = { "TOUCH", "MOVE", "EYES",
			"MOUTH", "SOUND", "GENERAL", "MAGIC", "DELICATE", "MALICIOUS",
		...
		public static Map<String, Integer> getMSGTYPE_DESCS() {
			...
			synchronized (MSGTYPE_DESCS) {
				...
				for (int i = 0; i < MASK_DESCS.length; i++)
					MSGTYPE_DESCS.put(MASK_DESCS[i],
							Integer.valueOf((int) CMath.pow(2, 11 + i)));
				for (int i = 0; i < CMMsg.MISC_DESCS.length; i++)
					for (int i2 = ((Integer) MISC_DESCS[i][1]).intValue(); i2 < ((Integer) MISC_DESCS[i][2])
							.intValue(); i2++)
						MSGTYPE_DESCS.put((String) MISC_DESCS[i][0],
								Integer.valueOf(i2));
			}
			return MSGTYPE_DESCS;
		}
		...
		public static Map<Integer, String> getMSGDESC_TYPES() {
			...
			synchronized (MSGDESC_TYPES) {
				...
				for (int i = 0; i < CMMsg.MISC_DESCS.length; i++)
					for (int i2 = ((Integer) MISC_DESCS[i][1]).intValue(); i2 < ((Integer) MISC_DESCS[i][2])
							.intValue(); i2++)
						MSGDESC_TYPES.put(Integer.valueOf(i2),
								(String) MISC_DESCS[i][0]);
			}
			return MSGDESC_TYPES;
		}
	}
	...
	/**
	 * Useful MAJOR_MASK shortcut combining other MASK_ constants related to
	 * casting verbal magic
	 */
	public static final int MSK_CAST_VERBAL = MASK_SOUND | MASK_MOUTH
			| MASK_MAGIC;
	/**
	 * Useful MAJOR_MASK shortcut combining other MASK_ constants related to
	 * casting verbal malicious magic
	 */
	public static final int MSK_CAST_MALICIOUS_VERBAL = MASK_SOUND | MASK_MOUTH
			| MASK_MAGIC | MASK_MALICIOUS;
	/**
	 * Useful MAJOR_MASK shortcut combining other MASK_ constants related to
	 * casting somantic magic
	 */
	public static final int MSK_CAST_SOMANTIC = MASK_HANDS | MASK_MAGIC;
	/**
	 * Useful MAJOR_MASK shortcut combining other MASK_ constants related to
	 * casting somantic malicious magic
	 */
	public static final int MSK_CAST_MALICIOUS_SOMANTIC = MASK_HANDS
			| MASK_MAGIC | MASK_MALICIOUS;
	/**
	 * Useful MAJOR_MASK shortcut combining other MASK_ constants related to
	 * haggling over price
	...
	/**
	 * Useful MAJOR_MASK shortcut combining other MASK_ constants related to
	 * casting both verbal and somantic
	 */
	public static final int MSK_CAST = MSK_CAST_VERBAL | MSK_CAST_SOMANTIC;
	/**
	 * Useful MAJOR_MASK shortcut combining other MASK_ constants related to
	 * casting both verbal and somantic maliciously
	 */
	public static final int MSK_CAST_MALICIOUS = MSK_CAST_MALICIOUS_VERBAL
			| MSK_CAST_MALICIOUS_SOMANTIC;
	/**
	 * Useful MAJOR_MASK shortcut combining other MASK_ constants related to
	 * malicious noisy movements
	 */
	public static final int MSK_MALICIOUS_MOVE = MASK_MALICIOUS | MASK_MOVE
			| MASK_SOUND;

	// all major messages
	/**
	 * combined MAJOR and MINOR codes for useful event message type that does
	 * absolutely nothing
	 */
	public static final int NO_EFFECT = 0;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for general
	 * area effects
	 */
	public static final int MSG_AREAAFFECT = MASK_ALWAYS | TYP_AREAAFFECT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a push
	 * event
	 */
	public static final int MSG_PUSH = MASK_HANDS | TYP_PUSH;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a pull
	 * event
	 */
	public static final int MSG_PULL = MASK_HANDS | TYP_PULL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a recall
	 * event
	 */
	public static final int MSG_RECALL = MASK_SOUND | TYP_RECALL; // speak
																	// precludes
																	// animals
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a open
	 * event
	 */
	public static final int MSG_OPEN = MASK_HANDS | TYP_OPEN;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a close
	 * event
	 */
	public static final int MSG_CLOSE = MASK_HANDS | TYP_CLOSE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a put
	 * event
	 */
	public static final int MSG_PUT = MASK_HANDS | TYP_PUT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a get
	 * event
	 */
	public static final int MSG_GET = MASK_HANDS | TYP_GET;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a unlock
	 * event
	 */
	public static final int MSG_UNLOCK = MASK_HANDS | TYP_UNLOCK;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a lock
	 * event
	 */
	public static final int MSG_LOCK = MASK_HANDS | TYP_LOCK;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a wield
	 * event
	 */
	public static final int MSG_WIELD = MASK_HANDS | TYP_WIELD;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a give
	 * event
	 */
	public static final int MSG_GIVE = MASK_HANDS | TYP_GIVE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a buy
	 * event
	 */
	public static final int MSG_BUY = MSK_HAGGLE | TYP_BUY;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a sell
	 * event
	 */
	public static final int MSG_SELL = MSK_HAGGLE | TYP_SELL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a drop
	 * event
	 */
	public static final int MSG_DROP = MASK_HANDS | TYP_DROP;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a wear
	 * event
	 */
	public static final int MSG_WEAR = MASK_HANDS | TYP_WEAR;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a fill
	 * event
	 */
	public static final int MSG_FILL = MASK_HANDS | MASK_MOVE | MASK_SOUND
			| TYP_FILL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * thiefly delicate quiet event
	 */
	public static final int MSG_DELICATE_SMALL_HANDS_ACT = MASK_HANDS
			| MASK_DELICATE | TYP_DELICATE_HANDS_ACT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * thiefly large quiet event
	 */
	public static final int MSG_DELICATE_HANDS_ACT = MASK_HANDS | MASK_MOVE
			| MASK_DELICATE | TYP_DELICATE_HANDS_ACT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * general thiefly embarrassing event
	 */
	public static final int MSG_THIEF_ACT = MASK_HANDS | MASK_MOVE
			| MASK_DELICATE | TYP_JUSTICE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * shopkeeper value event
	 */
	public static final int MSG_VALUE = MSK_HAGGLE | TYP_VALUE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a hold
	 * event
	 */
	public static final int MSG_HOLD = MASK_HANDS | TYP_HOLD;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a large
	 * noisy movement event
	 */
	public static final int MSG_NOISYMOVEMENT = MASK_HANDS | MASK_SOUND
			| MASK_MOVE | TYP_NOISYMOVEMENT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a large
	 * quiet movement event
	 */
	public static final int MSG_QUIETMOVEMENT = MASK_HANDS | MASK_MOVE
			| TYP_QUIETMOVEMENT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * missile weapon reload event
	 */
	public static final int MSG_RELOAD = MASK_HANDS | TYP_RELOAD;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * physical attack event
	 */
	public static final int MSG_WEAPONATTACK = MASK_HANDS | MASK_MOVE
			| MASK_SOUND | MASK_MALICIOUS | TYP_WEAPONATTACK;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a look
	 * event
	 */
	public static final int MSG_LOOK = MASK_EYES | TYP_LOOK;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a read
	 * event
	 */
	public static final int MSG_READ = MASK_EYES | TYP_READ;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a noisy
	 * event
	 */
	public static final int MSG_NOISE = MASK_SOUND | TYP_NOISE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a speak
	 * event
	 */
	public static final int MSG_SPEAK = MASK_SOUND | MASK_MOUTH | TYP_SPEAK;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a verbal
	 * spellcasting event
	 */
	public static final int MSG_CAST_VERBAL_SPELL = MSK_CAST_VERBAL
			| TYP_CAST_SPELL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * shopkeeper list event
	 */
	public static final int MSG_LIST = MASK_SOUND | MASK_MOUTH | TYP_LIST;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a eat
	 * event
	 */
	public static final int MSG_EAT = MASK_HANDS | MASK_MOUTH | TYP_EAT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a enter
	 * event
	 */
	public static final int MSG_ENTER = MASK_MOVE | MASK_SOUND | TYP_ENTER;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * malicious verbal spellcasting event
	 */
	public static final int MSG_CAST_ATTACK_VERBAL_SPELL = MSK_CAST_MALICIOUS_VERBAL
			| TYP_CAST_SPELL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a leave
	 * event
	 */
	public static final int MSG_LEAVE = MASK_MOVE | MASK_SOUND | TYP_LEAVE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a sleep
	 * event
	 */
	public static final int MSG_SLEEP = MASK_MOVE | TYP_SLEEP;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a sit
	 * event
	 */
	public static final int MSG_SIT = MASK_MOVE | TYP_SIT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a stand
	 * event
	 */
	public static final int MSG_STAND = MASK_MOVE | TYP_STAND;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a flee
	 * event
	 */
	public static final int MSG_FLEE = MASK_MOVE | MASK_SOUND | TYP_FLEE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * somantic spellcasting event
	 */
	public static final int MSG_CAST_SOMANTIC_SPELL = MSK_CAST_SOMANTIC
			| TYP_CAST_SPELL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * malicious somantic spellcasting event
	 */
	public static final int MSG_CAST_ATTACK_SOMANTIC_SPELL = MSK_CAST_MALICIOUS_SOMANTIC
			| TYP_CAST_SPELL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * spellcasting event
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * malicious spellcasting event
	 */
	public static final int MSG_CAST_MALICIOUS = MSK_CAST_MALICIOUS
			| TYP_CAST_SPELL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a always
	 * confirmed noisy event
	 */
	public static final int MSG_OK_ACTION = MASK_SOUND | MASK_ALWAYS
			| TYP_OK_ACTION;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a always
	 * confirmed quiet event
	 */
	public static final int MSG_OK_VISUAL = MASK_ALWAYS | TYP_OK_VISUAL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a drink
	 * event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a follow
	 * event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a write
	 * event
	 */
	public static final int MSG_WRITE = MASK_HANDS | TYP_WRITE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a mount
	 * event
	 */
	 ...
	 * dismount event
	 */
	public static final int MSG_DISMOUNT = MASK_MOVE | MASK_SOUND
			| TYP_DISMOUNT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a serve
	 * event
	 */
	public static final int MSG_SERVE = MASK_MOUTH | MASK_SOUND | TYP_SERVE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a rebuke
	 * event
	 */
	public static final int MSG_REBUKE = MASK_MOUTH | MASK_SOUND | TYP_REBUKE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a combat
	 * advance event
	 */
	public static final int MSG_ADVANCE = MASK_MOVE | MASK_SOUND
			| MASK_MALICIOUS | TYP_ADVANCE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a death
	 * event
	 */
	public static final int MSG_DEATH = MASK_SOUND | MASK_ALWAYS | TYP_DEATH;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a bank
	 * withdraw event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a game
	 * quit event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a shop
	 * view event
	 */
	public static final int MSG_VIEW = MASK_SOUND | MASK_MOUTH | TYP_VIEW;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a retire
	 * event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a panic
	 * event
	 */
	public static final int MSG_PANIC = MASK_MOVE | MASK_SOUND | TYP_PANIC;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a throw
	 * event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a tell
	 * event
	 */
	public static final int MSG_TELL = MASK_ALWAYS | TYP_TELL;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a crawl
	 * event
	 */
	public static final int MSG_SITMOVE = MASK_MOVE | TYP_SITMOVE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a knock
	 * event
	 */
	 ...
	 * practice event
	 */
	public static final int MSG_PRACTICE = MASK_HANDS | MASK_SOUND | MASK_MOVE
			| TYP_PRACTICE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a teach
	 * event
	 */
	public static final int MSG_TEACH = MASK_HANDS | MASK_SOUND | MASK_MOUTH
			| MASK_MOVE | TYP_TEACH;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a remove
	 * event
	 */
	public static final int MSG_REMOVE = MASK_HANDS | TYP_REMOVE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a damage
	 * event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a room
	 * reset event
	 */
	public static final int MSG_ROOMRESET = MASK_ALWAYS | TYP_ROOMRESET;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a sniff
	 * event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a login
	 * event
	 */
	public static final int MSG_LOGIN = MASK_ALWAYS | TYP_LOGIN;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a level
	 * event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a target
	 * order event
	 */
	public static final int MSG_ORDER = MASK_SOUND | MASK_MOUTH | TYP_ORDER;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a item
	 * expire event
	 */
	public static final int MSG_EXPIRE = MASK_ALWAYS | TYP_EXPIRE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a banker
	 * borrow event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for an
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a clan
	 * event
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a duel
	 * challenge
	 */
	public static final int MSG_DUELCHALLENGE = MASK_SOUND | MASK_MOUTH
			| TYP_DUELCHALLENGE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a duel
	 * challenge
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * physical digging action
	 */
	public static final int MSG_DIG = MASK_HANDS | MASK_MOVE | MASK_SOUND
			| TYP_DIG;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a
	 * possibly invoked skill
	 */
	...
	/**
	 * combined MAJOR and MINOR codes for useful event message type for power
	 * current flow
	 */
	public static final int MSG_POWERCURRENT = CMMsg.MASK_ALWAYS
			| TYP_POWERCURRENT;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for power
	 * current flow
	 */
	public static final int MSG_CONTEMPLATE = CMMsg.MASK_ALWAYS
			| TYP_CONTEMPLATE;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for a pour
	 * onto event
	 */
	public static final int MSG_POUR = MASK_HANDS | MASK_MOVE | MASK_SOUND
			| TYP_POUR;
	/**
	 * combined MAJOR and MINOR codes for useful event message type for looking
	 * around at exits
	 */
