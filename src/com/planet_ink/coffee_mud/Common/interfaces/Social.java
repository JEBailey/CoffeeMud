...
/**
 * Although at one time a simple public variable object, the Social became an
 * environmental when it became import to be able to pass it as a tool of a
 * message for various purposes. It still retains much of its original
 * awkwardness as can be seen in the method names. Today the social is still
 * mostly a variable storage unit, with a few methods to make use of those
 * variables.
 * 
 * A social object exists not simply as one for each social command (smile,
 * wiggy) but one for each social variation (smile, smile self, smile bob). The
 * Name() field holds the full unique name of each social object, with ID only
 * having "DefaultSocial".
 * 
...
public interface Social extends Environmental, CMCommon {
	...
	/**
	 * Base name is the name of the social's command word. It is NOT unique to a
	 * social object however, since a socials name usually includes its target
	 * extension.
	 * 
	...
	/**
	 * Third_party_sees: what everyone but the player and their target sees when
	 * a player uses this social.
	 * 
	...
	/**
	 * Target_sees: what the target of this social sees when social is used on
	 * them.
	 * 
	 ...
	 * @param str
	 *            what the target of this social sees when social is used on
	 *            them
	 */
	public void setTarget_sees(String str);

	/**
	 * See_when_no_target: what the player sees when this social is used
	 * targeted, but the target is not available.
	 * 
	...
	/**
	 * The official CoffeeMud message code used as the source code for this
	 * social, which reflects what the player is doing. Only certain codes
	 * allowed.
	 * 
	...
	/**
	 * Sets the official CoffeeMud message code used as the source code for this
	 * social, which reflects what the player is doing. Only certain codes
	 * allowed.
	 * 
	...
	/**
	 * The official CoffeeMud message code used as the others code for this
	 * social, which reflects what the player appears to be doing. Only certain
	 * codes allowed.
	 * 
	...
	/**
	 * Sets the official CoffeeMud message code used as the others code for this
	 * social, which reflects what the player appears to be doing. Only certain
	 * codes allowed.
	 * 
	...
	/**
	 * The official CoffeeMud message code used as the target code for this
	 * social, which reflects what is being done to the target. Only certain
	 * codes allowed.
	 * 
	...
	/**
	 * Sets the official CoffeeMud message code used as the target code for this
	 * social, which reflects what is being done to the target. Only certain
	 * codes allowed.
	 * 
	 ...
	 * help from the provided command line strings in a vector, and with
	 * override message code flag. It will generate a proper message and send it
	 * to the same room as the mob.
	 * 
	 * @param mob
	 *            the source of the social action
	 * @param commands
	 *            the vector of strings
	 * @param target
	 *            the target of the social
	 * @param auto
	 ...
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Social#makeMessage(MOB,
	 *      String, String, int, int, Vector, String, boolean)
	 * 
	 * @param mob
	 *            the mob doing the social
	 * @param channelInt
	 *            the channel int representing which chat channel
	 * @param channelName
	 *            the channel name of the chat channel
	 * @param commands
	 *            the commands as a string vector entered by the user
	 * @param makeTarget
	 *            true to create a fake target, or false to find a real one.
	 * 
	 * @return the CMMsg that can now be sent to the world as a chat channel
	 *         message
	 */
	public CMMsg makeChannelMsg(MOB mob, int channelInt, String channelName,
			Vector commands, boolean makeTarget);

	/**
	 * Manufactures a proper message that represents executing this social as an
	 * action
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CMMsg
	 * @see com.planet_ink.coffee_mud.Common.interfaces.Social#makeChannelMsg(MOB,
	 *      int, String, Vector, boolean)
	 * 
	 * @param mob
	 *            the mob doing the social
	 * @param str
	 *            the str an optional "header" string for the msg, usually mxp
	 *            related
	 * @param end
	 *            the end an optional "trailer" string for the msg, usually mxp
	 *            related
	 * @param srcMask
	 *            the src mask to logically OR with the generated message source
	 *            code
	 * @param fullCode
	 *            the override target and others code from the one in this
	 *            social
	 * @param commands
	 *            the commands as a string vector entered by the user
	 * @param I3channelName
	 *            the i3channel name or null if n/a
	 * @param makeTarget
	 ...
	 *         message
	 */
	public CMMsg makeMessage(MOB mob, String str, String end, int srcMask,
			int fullCode, Vector commands, String I3channelName,
			boolean makeTarget);
}
