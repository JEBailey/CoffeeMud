...
/**
 * Portions Copyright (c) 2003 Jeremy Vyska Portions Copyright (c) 2004-2014 Bo
 * Zimmerman Licensed under the Apache License, Version 2.0 (the "License"); you
...
public class DefaultFaction implements Faction, MsgListener {
	...
	protected PrioritizingLimitedMap<Integer, FRange> rangeRangeMap = new PrioritizingLimitedMap<Integer, FRange>(
			10, 60000, 600000, 100);
	...
	public Enumeration<Faction.FReactionItem> reactions(String rangeName) {
		SVector<Faction.FReactionItem> V = reactionHash.get(rangeName
				.toUpperCase().trim());
		if (V != null)
			return V.elements();
	...
	public boolean isPreLoaded() {
		return CMParms.parseSemicolons(
				CMProps.getVar(CMProps.Str.PREFACTIONS).toUpperCase(), true)
				.contains(factionID().toUpperCase());
	}

	public Faction.FAbilityUsage getAbilityUsage(int x) {
		return ((x >= 0) && (x < abilityUsages.size())) ? (Faction.FAbilityUsage) abilityUsages
				.elementAt(x) : null;
	}
	...
	public void initializeFaction(StringBuffer file, String fID) {
		...
		ID = fID;
		CMProps alignProp = new CMProps(new ByteArrayInputStream(
				CMStrings.strToBytes(file.toString())));
		...
		defaults = new SVector<String>(CMParms.parseSemicolons(
				alignProp.getStr("DEFAULT"), true));
		autoDefaults = new SVector<String>(CMParms.parseSemicolons(
				alignProp.getStr("AUTODEFAULTS"), true));
		choices = new SVector<String>(CMParms.parseSemicolons(
				alignProp.getStr("AUTOCHOICES"), true));
		...
		for (Enumeration<Object> e = alignProp.keys(); e.hasMoreElements();) {
			...
			if (key.startsWith("AFFBEHAV")) {
				Object[] O = CMParms.parseSafeSemicolonList(words, false)
						.toArray();
				if (O.length == 3)
			...
			if (key.startsWith("FACTOR")) {
				...
				if (factor.size() > 2)
					factors.add(new DefaultFactionZapFactor(CMath
							.s_double(factor.get(0)), CMath.s_double(factor
							.get(1)), factor.get(2)));
			}
			...
			if (key.startsWith("REACTION")) {
				DefaultFactionReactionItem item = new DefaultFactionReactionItem(
						words);
				addReaction(item.rangeCodeName(), item.presentMOBMask(),
						item.reactionObjectID(), item.parameters());
			}
		}
	}
	...
	private void recalc() {
		...
		rangeRangeMap = new PrioritizingLimitedMap<Integer, FRange>(num * 5,
				60000, 600000, 100);
	}
	...
	public String getTagValue(String tag) {
		...
		if ((tagRef < TAG_NAMES.length) && (TAG_NAMES[tagRef].endsWith("*")))
			if (CMath.isInteger(tag.substring(TAG_NAMES[tagRef].length() - 1)))
				numCall = CMath
						.s_int(tag.substring(TAG_NAMES[tagRef].length() - 1));
		switch (tagRef) {
		case TAG_NAME:
			return name;
		case TAG_MINIMUM:
		...
		case TAG_DEFAULT:
			return CMParms.toSemicolonList(defaults);
		case TAG_AUTODEFAULTS:
			return CMParms.toSemicolonList(autoDefaults);
		case TAG_CHOICEINTRO:
			return choiceIntro;
		case TAG_AUTOCHOICES:
			return CMParms.toSemicolonList(choices);
		...
		case TAG_CHANGE_: {
			int sz = 0;
			for (Enumeration<Faction.FactionChangeEvent[]> es = changes
					.elements(); es.hasMoreElements();)
				sz += es.nextElement().length;
			if ((numCall < 0) || (numCall >= sz))
				return "" + sz;
			int i = 0;
			for (Enumeration<Faction.FactionChangeEvent[]> e = changes
	...
	public String getINIDef(String tag, String delimeter) {
		...
		if (TAG_NAMES[tagRef].endsWith("*")) {
			...
			for (int i = 0; i < number; i++) {
				String value = getTagValue(rawTagName.substring(0,
						rawTagName.length() - 1)
						+ i);
				str.append(rawTagName.substring(0, rawTagName.length() - 1)
						+ (i + 1) + "=" + value + delimeter);
			}
			return str.toString();
		}
	...
	public boolean addAffectBehav(String ID, String parms, String gainMask) {
		...
		if ((CMClass.getBehavior(ID) == null)
				&& (CMClass.getAbility(ID) == null))
			return false;
		affBehavs
				.put(ID.toUpperCase().trim(), new String[] { parms, gainMask });
		lastFactionDataChange[0] = System.currentTimeMillis();
		return true;
	}
	...
	public String[] getAffectBehav(String ID) {
		if (affBehavs.containsKey(ID.toUpperCase().trim()))
			return CMParms.toStringArray(new XVector<String>(affBehavs.get(ID
					.toUpperCase().trim())));
		return null;
	}

	public boolean delReaction(Faction.FReactionItem item) {
		SVector<Faction.FReactionItem> V = reactionHash.get(item
				.rangeCodeName().toUpperCase().trim());
		if (V != null)
			V.remove(item);
	...
	public boolean addReaction(String range, String mask, String abilityID,
			String parms) {
		SVector<Faction.FReactionItem> V = reactionHash.get(range.toUpperCase()
				.trim());
		DefaultFactionReactionItem item = new DefaultFactionReactionItem();
			...
			if (CMath.isInteger(s))
				...
				for (int j = 0; j < V.size(); j++) {
					if (CMath.isInteger(V.elementAt(j)))
						mine.addElement(Integer.valueOf(CMath.s_int(V
								.elementAt(j))));
				}
			}
		}
	...
	public FactionChangeEvent[] findAbilityChangeEvents(Ability key) {
		...
		if (changes.containsKey(key.ID().toUpperCase())) {
			abilityChangesCache.put(key.ID().toUpperCase(),
					abilityChangesCache.get(key.ID().toUpperCase()));
			return abilityChangesCache.get(key.ID().toUpperCase());
		}
		// By TYPE or FLAGS
		FactionChangeEvent[] Cs = null;
		Vector<FactionChangeEvent> events = new Vector<FactionChangeEvent>();
		for (Enumeration<FactionChangeEvent[]> e = changes.elements(); e
				.hasMoreElements();) {
			Cs = e.nextElement();
			for (FactionChangeEvent C : Cs) {
				if ((key.classificationCode() & Ability.ALL_ACODES) == C
						.IDclassFilter())
					events.addElement(C);
				else if ((key.classificationCode() & Ability.ALL_DOMAINS) == C
						.IDdomainFilter())
					events.addElement(C);
				else if ((C.IDflagFilter() > 0)
						&& (CMath.bset(key.flags(), C.IDflagFilter())))
					events.addElement(C);
			}
		}
	...
	public int asPercent(int faction) {
		return (int) Math.round(CMath.mul(
				CMath.div(faction - minimum, (maximum - minimum)), 100));
	}
	...
	public int asPercentFromAvg(int faction) {
		...
		return (int) Math.round(CMath.mul(
				CMath.div(((maximum + minimum) / 2) - faction, maximum
						- minimum), 100));
	}
	...
	public int findAutoDefault(MOB mob) {
		String s;
		for (Enumeration<String> e = autoDefaults.elements(); e
				.hasMoreElements();) {
			s = e.nextElement();
			if (CMath.isNumber(s))
				return CMath.s_int(s);
	...
	public boolean hasUsage(Ability A) {
		for (FAbilityUsage usage : abilityUsages) {
			if ((usage.possibleAbilityID() && usage.abilityFlags()
					.equalsIgnoreCase(A.ID()))
					|| (((usage.type() < 0) || ((A.classificationCode() & Ability.ALL_ACODES) == usage
							.type()))
							&& ((usage.flag() < 0) || (CMath.bset(A.flags(),
									usage.flag())))
							&& ((usage.notflag() < 0) || (!CMath.bset(
									A.flags(), usage.notflag()))) && ((usage
							.domain() < 0) || ((A.classificationCode() & Ability.ALL_DOMAINS) == usage
							.domain()))))
				return true;
		}
		return false;
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_DEATH) // A death occured
			...
			events = getChangeEvents((msg.source() == myHost) ? "MURDER"
					: "KILL");
				...
				for (int e = 0; e < events.length; e++) {
					...
					if (eventC.applies(killingBlowM, killedM)) {
						CharClass combatCharClass = CMLib.combat()
								.getCombatDominantClass(killingBlowM, killedM);
						Set<MOB> combatBeneficiaries = CMLib.combat()
								.getCombatBeneficiaries(killingBlowM, killedM,
										combatCharClass);
						for (Iterator<MOB> i = combatBeneficiaries.iterator(); i
								.hasNext();)
		...
		if ((msg.tool() instanceof Ability)
			...
			if ((R != null) && (R.getArea() != null)) {
				...
				if (events != null) {
					...
					if ((B != null)
							&& (B.isAnyOfficer(R.getArea(), msg.source()))) {
						for (int e = 0; e < events.length; e++) {
							eventC = events[e];
							if (eventC
									.applies(msg.source(), (MOB) msg.target()))
								executeChange(msg.source(), (MOB) msg.target(),
										eventC);
						}
					}
				}
			...
			if (events != null)
				for (int e = 0; e < events.length; e++) {
					...
					if (eventC.applies(msg.source(), (MOB) msg.target())) {
						double amount = CMath.s_double(eventC
								.getTriggerParm("AMOUNT"));
						double pctAmount = CMath.s_pct(eventC
								.getTriggerParm("PCT"))
								* CMLib.beanCounter()
		...
		if ((msg.sourceMinor() == CMMsg.TYP_SPEAK) // Talk watching
				&& (msg.othersMessage() != null) && (msg.source() == myHost)) {
			FactionChangeEvent eventC;
			...
			if ((events != null) && (events.length > 0)) {
				...
				String sayMsg = CMStrings.getSayFromMessage(msg.othersMessage()
						.toLowerCase());
				Matcher M = null;
				if ((sayMsg != null) && (sayMsg.length() > 0) && (R != null))
					for (int e = 0; e < events.length; e++) {
						...
						if (M.matches()) {
							...
							if (addTime == null) {
								addTime = Long.valueOf(CMath.s_long(eventC
										.getTriggerParm("WAIT"))
										* CMProps.getTickMillis());
								eventC.setStateVariable(2, addTime);
							}
							eventC.setStateVariable(
									1,
			...
			if ((events != null) && (events.length > 0)) {
				...
				for (MOB target : targets) {
					...
					Behavior B = null;
					for (Enumeration<Behavior> e = target.behaviors(); e
							.hasMoreElements();) {
						B = e.nextElement();
					...
					if (mudChatB != null) {
						String sayMsg = CMStrings.getSayFromMessage(msg
								.othersMessage().toLowerCase());
						if ((sayMsg != null) && (sayMsg.length() > 0))
							for (int e = 0; e < events.length; e++) {
								eventC = events[e];
								Long time = (Long) eventC.stateVariable(0);
								if (time == null)
									time = Long.valueOf(System
											.currentTimeMillis());
								...
								if (addTime == null) {
									addTime = Long.valueOf(CMath.s_long(eventC
											.getTriggerParm("WAIT"))
											* CMProps.getTickMillis());
									if (addTime.longValue() < CMProps
											.getTickMillis())
										addTime = Long.valueOf(CMProps
												.getTickMillis());
								...
								eventC.setStateVariable(
										...
										Long.valueOf(System.currentTimeMillis()
												+ addTime.longValue()));
								if (eventC.applies(msg.source(), target)) {
									if ((mudChatB.getLastRespondedTo() == msg
											.source())
											&& (mudChatB.getLastThingSaid() != null)
											&& (!mudChatB.getLastThingSaid()
													.equalsIgnoreCase(sayMsg))) {
										executeChange(msg.source(), target,
												eventC);
										foundOne = true;
									}
								}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((msg.sourceMinor() == CMMsg.TYP_EXPCHANGE) // Experience is being
														// altered
				&& (msg.target() instanceof MOB) // because a mob died
				&& (myHost == msg.source()) // this Faction is on the mob that
											// killed them
				&& (!experienceFlag.equals("NONE")) && (msg.value() > 0)) {
			MOB killer = msg.source();
			MOB vic = (MOB) msg.target();

			if (experienceFlag.equals("HIGHER"))
				msg.setValue((int) Math.round(((msg.value()) * .75)
						+ (((msg.value()) * .25) * CMath.div(
								Math.abs(killer.fetchFaction(ID) - minimum),
								(maximum - minimum)))));
			else if (experienceFlag.equals("LOWER"))
				msg.setValue((int) Math.round(((msg.value()) * .75)
						+ (((msg.value()) * .25) * CMath.div(
								Math.abs(maximum - killer.fetchFaction(ID)),
								(maximum - minimum)))));
			else if (vic.fetchFaction(ID) != Integer.MAX_VALUE) {
				if (experienceFlag.equals("EXTREME"))
					msg.setValue((int) Math.round(((msg.value()) * .75)
							+ (((msg.value()) * .25) * CMath.div(
									Math.abs(vic.fetchFaction(ID)
											- killer.fetchFaction(ID)),
									(maximum - minimum)))));
				else if (experienceFlag.equals("FOLLOWHIGHER"))
					msg.setValue((int) Math.round(((msg.value()) * .75)
							+ (((msg.value()) * .25) * CMath.div(
									Math.abs(vic.fetchFaction(ID) - minimum),
									(maximum - minimum)))));
				else if (experienceFlag.equals("FOLLOWLOWER"))
					msg.setValue((int) Math.round(((msg.value()) * .75)
							+ (((msg.value()) * .25) * CMath.div(
									Math.abs(maximum - vic.fetchFaction(ID)),
									(maximum - minimum)))));
				if (msg.value() <= 0)
					msg.setValue(0);
			}
	...
	public void executeChange(MOB source, MOB target, FactionChangeEvent event) {
		...
		if (changeDir != 0) {
			...
			// They're the result of making graphs of scenarios and massaging
			// the formula, nothing more or less.
			if ((hasFaction(target)) || (event.outsiderTargetOK()))
				factionAdj = changeDir
						* (int) Math.round(rateModifier * baseChangeAmount);
			else
				factionAdj = 0;
			factionAdj *= event.factor();
			factionAdj = (int) Math.round(CMath.mul(factionAdj,
					findFactor(source, (factionAdj >= 0))));
		}

		if (factionAdj == 0)
			return;

		CMMsg FacMsg = CMClass.getMsg(source, target, null, CMMsg.MASK_ALWAYS
		...
		if (R != null) {
			if (R.okMessage(source, FacMsg)) {
				...
				if ((factionAdj != Integer.MAX_VALUE)
						&& (factionAdj != Integer.MIN_VALUE)) {
					// Now execute the changes on the relation. We do this AFTER
					// the execution of the first so
					// that any changes from okMessage are incorporated
					for (Enumeration<String> e = relations.keys(); e
							.hasMoreElements();) {
						String relID = (e.nextElement());
						FacMsg = CMClass.getMsg(source, target, null,
								CMMsg.MASK_ALWAYS | CMMsg.TYP_FACTIONCHANGE,
								null, CMMsg.NO_EFFECT, null, CMMsg.NO_EFFECT,
								relID);
						FacMsg.setValue((int) Math.round(CMath.mul(factionAdj,
								relations.get(relID).doubleValue())));
						if (R.okMessage(source, FacMsg))
							R.send(source, FacMsg);
					}
				}
			}
		} else if ((factionAdj == Integer.MAX_VALUE)
				|| (factionAdj == Integer.MIN_VALUE))
		...
		for (FAbilityUsage usage : abilityUsages) {
			if ((usage.possibleAbilityID() && usage.abilityFlags()
				...
				for (Enumeration<FRange> e = ranges(); e.hasMoreElements();) {
					...
					if ((((R.high() <= usage.high()) && (R.high() >= usage
							.low())) || ((R.low() >= usage.low()))
							&& (R.low() <= usage.high()))
	...
	public String ALL_CHANGE_EVENT_TYPES() {
		...
		for (int i = 0; i < Faction.FactionChangeEvent.MISC_TRIGGERS.length; i++)
			ALL_TYPES
					.append(Faction.FactionChangeEvent.MISC_TRIGGERS[i] + ", ");
		...
		_ALL_TYPES = ALL_TYPES.toString()
				+ " a valid Skill, Spell, Chant, etc. ID.";
		return _ALL_TYPES;
	}
	...
	public Faction.FactionChangeEvent createChangeEvent(String key) {
		...
		Faction.FactionChangeEvent[] events = changes.get(event.eventID()
				.toUpperCase().trim());
		if (events == null)
	...
	private boolean replaceEvents(String key, Faction.FactionChangeEvent event,
			boolean strict) {
		...
		for (int x = 0; x < events.length; x++) {
			if ((strict && (events[x] == event))
					|| ((!strict) && (events[x].toString().equals(event
							.toString())))) {
				if (nevents.length == 0)
					changes.remove(key);
				else
	...
	public class DefaultFactionChangeEvent implements
		...
		public MaskingLibrary.CompiledZapperMask compiledTargetZapper() {
			if (compiledTargetZapper == null) {
				if (targetZapperStr.trim().length() > 0)
					compiledTargetZapper = CMLib.masking().maskCompile(
							targetZapperStr);
			}
		...
		public MaskingLibrary.CompiledZapperMask compiledSourceZapper() {
			if (compiledSourceZapper == null) {
				...
				if ((sourceZapperStr != null) && (sourceZapperStr.length() > 0))
					compiledSourceZapper = CMLib.masking().maskCompile(
							sourceZapperStr);
			}
		...
		public String toString() {
			if (triggerParms.trim().length() > 0)
				return ID + "(" + triggerParms.replace(';', ',') + ");"
						+ CHANGE_DIRECTION_DESCS[direction] + ";"
						+ ((int) Math.round(factor * 100.0)) + "%;" + flagCache
						+ ";" + targetZapperStr;
			else
				return ID + ";" + CHANGE_DIRECTION_DESCS[direction] + ";"
						+ ((int) Math.round(factor * 100.0)) + "%;" + flagCache
						+ ";" + targetZapperStr;
		}
		...
		public DefaultFactionChangeEvent(Faction F, String key) {
			...
			if ((x > 0) && (trigger.endsWith(")"))) {
				setTriggerParameters(trigger.substring(x + 1,
						trigger.length() - 1));
		...
		public boolean applies(MOB source, MOB target) {
			if (!CMLib.masking().maskCheck(compiledTargetZapper(), target,
					false))
				return false;
			if (!CMLib.masking().maskCheck(compiledSourceZapper(), target,
					false))
				return false;
			return true;
		}
		...
		public Object stateVariable(int x) {
			return ((x >= 0) && (x < stateVariables.length)) ? stateVariables[x]
					: null;
		}
	...
	public Faction.FAbilityUsage addAbilityUsage(String key) {
		Faction.FAbilityUsage usage = (key == null) ? new DefaultFaction.DefaultFactionAbilityUsage()
	...
	public class DefaultFactionData implements FData {
		...
		public void setValue(int newValue) {
			this.value = newValue;
			if ((currentRange == null) || (this.value < currentRange.low())
					|| (this.value > currentRange.high())) {
				synchronized (this) {
					if ((currentRange != null)
							&& (this.value >= currentRange.low())
							&& (this.value <= currentRange.high()))
						return;
					currentRange = fetchRange(value);
					if (currentRange == null) {
						if (!erroredOut)
							Log.errOut("DefaultFactionData", "Faction "
									+ factionID()
									+ " does not define a range for "
									+ this.value);
						erroredOut = true;
					} else {
						erroredOut = false;
						currentReactionSets = new DVector(2);
						for (Enumeration<Faction.FReactionItem> e = reactions(); e
								.hasMoreElements();) {
							Faction.FReactionItem react = e.nextElement();
							if (!react.rangeCodeName().equalsIgnoreCase(
									currentRange.codeName()))
								continue;
							Faction.FReactionItem sampleReact = null;
							Vector<Faction.FReactionItem> reactSet = null;
							for (int r = 0; r < currentReactionSets.size(); r++) {
								reactSet = (Vector<Faction.FReactionItem>) currentReactionSets
										.elementAt(r, 2);
							...
							if (react != null)
								currentReactionSets.addElement(react
										.compiledPresentMOBMask(),
										new XVector<Faction.FReactionItem>(
												react));
						}
						// noReactions=currentReactionSets.size()==0;
					}
					noListeners = (myEffects.length == 0)
							&& (myBehaviors.length == 0)
							&& (currentReactionSets.size() == 0);
					noTickers = (myBehaviors.length == 0)
							&& (myEffects.length == 0)
							&& ((currentReactionSets.size() == 0) || (!useLightReactions()));
				}
			}
		}
		...
		public void addHandlers(Vector<Ability> listeners,
				Vector<Behavior> tickers) {
			this.myEffects = listeners.toArray(new Ability[0]);
			this.myBehaviors = tickers.toArray(new Behavior[0]);
			noListeners = (listeners.size() == 0) && (tickers.size() == 0)
					&& (currentReactionSets.size() == 0);
			noTickers = (listeners.size() == 0)
					&& (tickers.size() == 0)
					&& ((currentReactionSets.size() == 0) || (!useLightReactions()));
		...
		private Ability setPresenceReaction(MOB M, Physical myHost) {
			...
			for (int d = 0; d < currentReactionSets.size(); d++)
				if (CMLib
						.masking()
						.maskCheck(
								(MaskingLibrary.CompiledZapperMask) currentReactionSets
										.elementAt(d, 1), M, true)) {
					if (myReactions == null)
					...
					for (Faction.FReactionItem reactionItem : tempReactSet)
						myReactions.add(reactionItem.reactionObjectID() + "="
								+ reactionItem.parameters());
				}
			if (myReactions != null)
				if (useLightReactions()) {
					presenceReactionPrototype.invoke(M, myReactions, myHost,
							false, 0);
			...
			synchronized (lightPresenceAbilities) {
				if ((currentReactionSets.size() > 0)
					...
					if (presenceReactionPrototype == null)
						if ((presenceReactionPrototype = CMClass
								.getAbility("PresenceReaction")) == null)
							return;
					if ((msg.source() == myHost) && (!msg.source().isMonster())) {
						MOB M = null;
						...
						for (int m = 0; m < R.numInhabitants(); m++) {
							...
							if ((M != null) && (M != myHost) && (M.isMonster())) {
								...
								if (A != null) // means yes, we are using light,
												// and yes, heres a reaction to
												// add
									lightPresenceReactions.add(A);
							}
						}
						lightPresenceAbilities = lightPresenceReactions
								.toArray(new Ability[0]);
					} else if ((msg.source().isMonster())
							&& (msg.target() == CMLib.map()
									.roomLocation(myHost))
							&& (myHost instanceof Physical)) {
						Ability A = setPresenceReaction(msg.source(),
								(Physical) myHost);
						if (A != null) { // means yes, we are using light, and
											// yes, heres a reaction to add
							lightPresenceAbilities = Arrays.copyOf(
									lightPresenceAbilities,
									lightPresenceAbilities.length + 1);
					...
					if ((msg.source() == myHost) && (!msg.source().isMonster())) {
						...
						for (int m = 0; m < R.numInhabitants(); m++) {
							M = R.fetchInhabitant(m);
							if ((M != null) && (M != myHost) && (M.isMonster()))
								presenceReactionPrototype.invoke(M,
										new Vector(), null, true, 0); // this
																		// shuts
																		// it
																		// down
						}
						lightPresenceAbilities = new Ability[0];
							...
							&& (myHost instanceof Physical)) {
						presenceReactionPrototype.invoke(msg.source(),
								new Vector(), null, true, 0);
						...
						for (int a = 0; a < lightPresenceAbilities.length; a++)
							...
							} else if (lightPresenceAbilities[a].affecting() == msg
									.source())
								lightPresenceAbilities[a].invoke(msg.source(),
										new Vector(), null, true, 0);
							else
	...
	public class DefaultFactionReactionItem implements Faction.FReactionItem {
		...
		public String toString() {
			return rangeName + ";" + mobMask + ";" + reactionObjectID + ";"
					+ parms;
		}
	...
	public class DefaultFactionAbilityUsage implements Faction.FAbilityUsage {
		...
		public List<String> setAbilityFlag(String str) {
			...
			for (int f = 0; f < flags.size(); f++) {
				...
				case 1:
					type = CMParms.indexOfIgnoreCase(Ability.ACODE_DESCS,
							strflag);
					break;
				case 2:
					domain = CMParms.indexOfIgnoreCase(Ability.DOMAIN_DESCS,
							strflag);
					break;
				case 3:
					int val = CMParms.indexOfIgnoreCase(Ability.FLAG_DESCS,
							strflag);
					if (not) {
						if (notflag < 0)
							notflag = 0;
