...
public class DefaultSession implements Session {
	...
	protected static final byte[] TELNETGABYTES = { (byte) TELNET_IAC,
			(byte) TELNET_GA };
	...
	public DefaultSession() {
		threadGroupChar = Thread.currentThread().getThreadGroup().getName()
				.charAt(0);
	}
	...
	public void initializeSession(final Socket s, final String groupName,
			final String introTextStr) {
		...
		try {
			...
			Charset charSet = Charset.forName(CMProps
					.getVar(CMProps.Str.CHARSETINPUT));
			inMaxBytesPerChar = (int) Math.round(Math.ceil(charSet.newEncoder()
					.maxBytesPerChar()));
			...
			out = new PrintWriter(new OutputStreamWriter(rawout,
					CMProps.getVar(CMProps.Str.CHARSETOUTPUT)));
					...
					try {
						...
						case HANDSHAKE_OPEN: {
							if ((!terminalType.equalsIgnoreCase("ANSI"))
									&& (getClientTelnetMode(TELNET_ECHO)))
								changeTelnetModeBackwards(rawout, TELNET_ECHO,
										false);
							rawout.flush();
						...
						case HANDSHAKE_MCCP: {
							if (((lastIACIn > firstIACIn) && ((System
									.currentTimeMillis() - lastIACIn) > 500))
									|| ((System.currentTimeMillis() - lastIACIn) > 5000)) {
								if (getClientTelnetMode(TELNET_COMPRESS2)) {
									negotiateTelnetMode(rawout,
											TELNET_COMPRESS2);
									rawout.flush();
									if (getClientTelnetMode(TELNET_COMPRESS2)) {
										ZOutputStream zOut = new ZOutputStream(
												rawout,
						...
						case HANDSHAKE_MXPPAUSE: {
							if (((System.currentTimeMillis() - lastStateChangeMs) > 2000)
									|| (mxpSupportSet.contains("+IMAGE.URL")
											|| mxpSupportSet.contains("+IMAGE") || mxpSupportSet
												.contains("-IMAGE.URL")))
								setStatus(SessionStatus.HANDSHAKE_DONE);
							break;
						}
						case HANDSHAKE_DONE: {
							...
							if (out != null) {
								...
								if ((getClientTelnetMode(Session.TELNET_MXP))
										&& ((mxpSupportSet
												.contains("+IMAGE.URL")) || ((mxpSupportSet
												.contains("+IMAGE")) && (!mxpSupportSet
												.contains("-IMAGE.URL"))))) {
									// also the intro page
									String[] paths = CMLib.protocol()
											.mxpImagePath("intro.jpg");
									if (paths[0].length() > 0) {
										CMFile introDir = new CMFile(
												"/web/pub/images/mxp", null,
												CMFile.FLAG_FORCEALLOW);
										String introFilename = paths[1];
										if (introDir.isDirectory()) {
											CMFile[] files = introDir
													.listFiles();
											...
											for (int f = 0; f < files.length; f++)
												if (files[f].getName()
														...
														&& files[f]
																...
																.endsWith(
																		".jpg"))
													choices.addElement(files[f]
															.getName());
											if (choices.size() > 0)
												introFilename = (String) choices
														.elementAt(CMLib
																.dice()
																.roll(1,
																		choices.size(),
						...
						default:
							...
							if (collectedInput.length() > 0)
								fakeInput = new StringBuffer(
										collectedInput.toString());
							return false;
						}
						return (out != null);
					} catch (Exception e) {
	...
	private void negotiateTelnetMode(OutputStream out, int optionCode)
		...
		if (optionCode == TELNET_TERMTYPE) {
			byte[] stream = { (byte) TELNET_IAC, (byte) TELNET_SB,
					(byte) optionCode, (byte) 1, (byte) TELNET_IAC,
					(byte) TELNET_SE };
			rawBytesOut(out, stream);
		} else {
			byte[] stream = { (byte) TELNET_IAC, (byte) TELNET_SB,
					(byte) optionCode, (byte) TELNET_IAC, (byte) TELNET_SE };
			rawBytesOut(out, stream);
		}
		out.flush();
	}
	...
	private void changeTelnetMode(OutputStream out, int telnetCode,
			boolean onOff) throws IOException {
		byte[] command = { (byte) TELNET_IAC,
				onOff ? (byte) TELNET_WILL : (byte) TELNET_WONT,
				(byte) telnetCode };
		rawBytesOut(out, command);
		out.flush();
		if (CMSecurity.isDebugging(CMSecurity.DbgFlag.TELNET))
			Log.debugOut("Sent: " + (onOff ? "Will" : "Won't") + " "
					+ Session.TELNET_DESCS[telnetCode]);
	...
	// this is stupid, but a printwriter can not be cast as an outputstream, so
	// this dup was necessary
	...
	public void changeTelnetModeBackwards(int telnetCode, boolean onOff)
			throws IOException {
		byte[] command = { (byte) TELNET_IAC,
				onOff ? (byte) TELNET_DO : (byte) TELNET_DONT,
				(byte) telnetCode };
		out.flush();
		rawBytesOut(rawout, command);
		rawout.flush();
		if (CMSecurity.isDebugging(CMSecurity.DbgFlag.TELNET))
			Log.debugOut("Back-Sent: " + (onOff ? "Do" : "Don't") + " "
					+ Session.TELNET_DESCS[telnetCode]);
		setServerTelnetMode(telnetCode, onOff);
	}

	public void changeTelnetModeBackwards(OutputStream out, int telnetCode,
			boolean onOff) throws IOException {
		byte[] command = { (byte) TELNET_IAC,
				onOff ? (byte) TELNET_DO : (byte) TELNET_DONT,
				(byte) telnetCode };
	...
	public void negotiateTelnetMode(int telnetCode) {
		try {
			...
			if (telnetCode == TELNET_TERMTYPE) {
				byte[] command = { (byte) TELNET_IAC, (byte) TELNET_SB,
						(byte) telnetCode, (byte) 1, (byte) TELNET_IAC,
						(byte) TELNET_SE };
				rawBytesOut(rawout, command);
			} else {
				byte[] command = { (byte) TELNET_IAC, (byte) TELNET_SB,
						(byte) telnetCode, (byte) TELNET_IAC, (byte) TELNET_SE };
				rawBytesOut(rawout, command);
			}
			rawout.flush();
		} catch (Exception e) {
	...
	public void initTelnetMode(int mobbitmap) {
		...
		boolean mxpSet = (!CMSecurity.isDisabled(CMSecurity.DisFlag.MXP))
				&& CMath.bset(mobbitmap, MOB.ATT_MXP);
		if (mxpSet != getClientTelnetMode(TELNET_MXP)) {
			changeTelnetMode(TELNET_MXP, !getClientTelnetMode(TELNET_MXP));
			changedSomething = true;
		}
		boolean mspSet = (!CMSecurity.isDisabled(CMSecurity.DisFlag.MSP))
				&& CMath.bset(mobbitmap, MOB.ATT_SOUND);
	...
	public int getWrap() {
		...
		return ((mob != null) && (mob.playerStats() != null)) ? mob
				.playerStats().getWrap() : 78;
	}
	...
	private int metaFlags() {
		return ((snoops.size() > 0) ? Command.METAFLAG_SNOOPED : 0)
				| (((mob != null) && (mob.soulMate() != null)) ? Command.METAFLAG_POSSESSED
						: 0);
	}
	...
	public void setPreviousCmd(List cmds) {
		...
		if (cmds.size() == 0)
			return;
		if ((cmds.size() > 0)
				&& (((String) cmds.get(0)).trim().startsWith("!")))
			return;
	...
	public String getAfkMessage() {
		if (mob == null)
			return "";
		if ((afkMessage == null)
				|| (CMStrings.removeColors(afkMessage).trim().length() == 0))
	...
	public final void rawBytesOut(final OutputStream out, final byte[] bytes)
			throws IOException {
		try {
	...
	public void rawCharsOut(final PrintWriter out, char[] chars) {
		...
		try {
			if (writeLock.tryLock(10000, TimeUnit.MILLISECONDS)) {
				...
				Log.errOut("DefaultSession", "Kicked out " + name
						+ " due to write-lock (" + out.getClass().getName()
						+ ".");
				stopSession(true, true, true);
	...
	public void snoopSupportPrint(final String msg, final boolean noCache) {
		try {
			if ((snoops.size() > 0) && (snoopSuspensionStack <= 0)) {
				String msgColored;
				String preFix = CMLib.coffeeFilter().colorOnlyFilter(
						"^Z" + ((mob == null) ? "?" : mob.Name()) + ":^N ",
						this);
				...
				for (int s = 0; s < snoops.size(); s++)
					((Session) snoops.get(s)).onlyPrint(preFix + msgColored,
							noCache);
			}
	...
	public void onlyPrint(String msg, boolean noCache) {
		...
		try {
			...
			if (msg.endsWith("\n\r") && (msg.equals(lastStr))
				...
				if (spamStack > 1)
					lastStr = lastStr.substring(0, lastStr.length() - 2) + "("
							+ spamStack + ")"
							+ lastStr.substring(lastStr.length() - 2);
				...
				if (pageBreak > 0)
					for (int i = 0; i < msg.length(); i++) {
						if (msg.charAt(i) == '\n') {
							...
							if (lines >= pageBreak) {
								lines = 0;
								if ((i < (msg.length() - 1) && (msg
										.charAt(i + 1) == '\r')))
									i++;
								rawCharsOut(msg.substring(last, i + 1)
										.toCharArray());
								last = i + 1;
	...
	public void print(String msg) {
		onlyPrint(
				CMLib.coffeeFilter().fullOutFilter(this, mob, mob, mob, null,
						msg, false), false);
	}
	...
	public void stdPrint(String msg) {
		rawPrint(CMLib.coffeeFilter().fullOutFilter(this, mob, mob, mob, null,
				msg, false));
	}

	public void print(Physical src, Environmental trg, Environmental tol,
			String msg) {
		onlyPrint((CMLib.coffeeFilter().fullOutFilter(this, mob, src, trg, tol,
				msg, false)), false);
	}

	public void stdPrint(Physical src, Environmental trg, Environmental tol,
			String msg) {
		rawPrint(CMLib.coffeeFilter().fullOutFilter(this, mob, src, trg, trg,
				msg, false));
	}
	...
	public void wraplessPrintln(String msg) {
		if (msg != null)
			onlyPrint(
					CMLib.coffeeFilter().fullOutFilter(this, mob, mob, mob,
							null, msg, true)
							+ "\n\r", false);
		needPrompt = true;
	}
	...
	public void stdPrintln(String msg) {
		if (msg != null)
			rawPrint(CMLib.coffeeFilter().fullOutFilter(this, mob, mob, mob,
					null, msg, false)
					+ "\n\r");
	}

	public void println(Physical src, Environmental trg, Environmental tol,
			String msg) {
		if (msg != null)
			onlyPrint(
					CMLib.coffeeFilter().fullOutFilter(this, mob, src, trg,
							tol, msg, false)
							+ "\n\r", false);
	}

	public void stdPrintln(Physical src, Environmental trg, Environmental tol,
			String msg) {
		if (msg != null)
			rawPrint(CMLib.coffeeFilter().fullOutFilter(this, mob, src, trg,
					tol, msg, false)
					+ "\n\r");
	}
		...
		if (!pstats.getColorStr().equals(lastColorStr)) {
			if (pstats.getColorStr().length() == 0)
				...
				while (x > 0) {
					...
					clookup[sub.charAt(0)] = CMLib.color()
							.translateCMCodeToANSI(sub.substring(1));
					x = changes.indexOf('#');
				}
	...
	public void handleSubOption(int optionCode, char[] suboptionData,
			int dataSize) throws IOException {
		switch (optionCode) {
		case TELNET_TERMTYPE:
			if (dataSize >= 1) {
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.TELNET))
					Log.debugOut("For suboption "
							+ Session.TELNET_DESCS[optionCode] + ", got code "
							+ ((int) suboptionData[0]) + ": "
							+ new String(suboptionData, 1, dataSize - 1));
				if (suboptionData[0] == 0) {
					...
					if (terminalType.equalsIgnoreCase("ZMUD")
						...
						if (mightSupportTelnetMode(TELNET_ECHO))
							telnetSupportSet.remove(Integer
									.valueOf(TELNET_ECHO));
					...
					else if (terminalType.startsWith("WINTIN.NET")) {
						rawOut("\n\r\n\r**** Your MUD Client is Broken! Please use another!!****\n\r\n\r");
						rawout.flush();
						CMLib.s_sleep(1000);
						rawout.close();
					} else if (terminalType.toLowerCase().startsWith(
							"mushclient")
		...
		case TELNET_NAWS:
			if (dataSize == 4) // It should always be 4.
			{
				...
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.TELNET))
					Log.debugOut("For suboption "
							+ Session.TELNET_DESCS[optionCode] + ", got: "
							+ terminalWidth + "x" + terminalHeight);
			}
			break;
		case TELNET_MSDP: {
			byte[] resp = CMLib.protocol().processMsdp(this, suboptionData,
					dataSize, this.msdpReportables);
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.TELNET))
				Log.debugOut("For suboption "
						+ Session.TELNET_DESCS[optionCode] + ", got "
						+ dataSize + " bytes, sent "
			...
			break;
		case TELNET_GMCP: {
			byte[] resp = CMLib.protocol().processGmcp(this, suboptionData,
					dataSize, this.gmcpSupports);
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.TELNET)) {
				Log.debugOut("For suboption "
						+ Session.TELNET_DESCS[optionCode] + ", got "
						+ dataSize + " bytes, sent "
			...
			if (parts.size() > 1) {
				...
				if (tag.equals("VERSION")) {
					for (int p = 1; p < parts.size(); p++) {
						...
						mxpVersionInfo.put(pp.substring(0, x).trim(), pp
								.substring(x + 1).trim());
					}
					...
					if ((M != null)
						...
						boolean keepDown = parts.size() > 3 ? CMath
								.s_bool(parts.get(3)) : true;
						String externalCmd = (parts.size() > 4) ? CMParms
								.combine(parts, 4) : null;
						Vector cmd = new XVector("SHUTDOWN", "NOPROMPT");
						if (!keepDown) {
							cmd.add("RESTART");
							if ((externalCmd != null)
									&& (externalCmd.length() > 0))
								cmd.add(externalCmd);
						}
	...
	public void handleIAC() throws IOException, InterruptedIOException {
		if ((in == null) || (out == null))
			return;
		...
		case TELNET_SB: {
			...
			while (((last = readByte()) != -1) && (!killFlag)) {
		...
		case TELNET_DO: {
			...
			if ((terminalType.equalsIgnoreCase("zmud") || terminalType
					.equalsIgnoreCase("cmud")) && (last == Session.TELNET_ECHO))
	...
	public int readChar() throws IOException {
		...
		if (1 == inMaxBytesPerChar)
			return b;
		if ((b == TELNET_IAC) || ((b & 0xff) == TELNET_IAC) || (b == '\033')
				|| (b == 27) || (in == null))
			return b;
		charWriter.write(b);
		int maxBytes = inMaxBytesPerChar;
		while ((in != null) && !in.ready() && !killFlag && (rawin != null)
				&& (rawin.available() > 0) && (--maxBytes >= 0)) {
			try {
				return in.read();
	...
	public int nonBlockingIn(boolean appendInputFlag) throws IOException {
		try {
			int c = readChar();
			if (c < 0)
	...
	public String blockingIn(long maxTime) throws IOException {
		if ((in == null) || (out == null))
			return "";
		this.input.setLength(0);
		final long start = System.currentTimeMillis();
		final long timeoutTime = (maxTime <= 0) ? Long.MAX_VALUE
				: (start + maxTime);
		...
		try {
			...
			while ((!killFlag)
				...
				if ((lastC = nonBlockingIn(true)) == 0)
					...
					CMLib.s_sleep(100); // if they entered nothing, make sure we
										// dont do a busy poll
				}
			}
			suspendCommandLine = false;
			...
			this.input.setLength(0);
			String str = CMLib.coffeeFilter().simpleInFilter(
					inStr,
					CMSecurity.isAllowed(mob, (mob != null) ? mob.location()
							: null, CMSecurity.SecFlag.MXPTAGS));
			if (str == null)
				return null;
	...
	public String readlineContinue() throws IOException, SocketException {
		...
		String str = CMLib.coffeeFilter().simpleInFilter(
				inStr,
				CMSecurity.isAllowed(mob,
						(mob != null) ? mob.location() : null,
						CMSecurity.SecFlag.MXPTAGS));
		if (str == null)
			return null;
		snoopSupportPrint(str + "\n\r", true);
		return str;
	...
	public String choose(final String Message, final String Choices,
			String Default) throws IOException {
	...
	protected void closeSocks(String finalMsg) {
		if (sock[0] != null) {
			synchronized (sock) {
				if (sock[0] != null) {
					try {
						Log.sysOut("Disconnect: "
								+ finalMsg
								+ getAddress()
								+ " ("
								+ CMLib.time().date2SmartEllapsedTime(
										getMillisOnline(), true) + ")");
	...
	private void preLogout(MOB M) {
		...
		if (!CMLib.flags().isCloaked(M)) {
			List<String> channels = CMLib.channels().getFlaggedChannelNames(
					ChannelsLibrary.ChannelFlag.LOGOFFS);
			for (int i = 0; i < channels.size(); i++)
				CMLib.commands().postChannel(channels.get(i), M.clans(),
						name + " has logged out", true);
		}
		CMLib.login()
				.notifyFriends(M, "^X" + M.Name() + " has logged off.^.^?");

		// the player quit message!
		CMLib.threads().executeRunnable(groupName,
				new LoginLogoutThread(M, CMMsg.MSG_QUIT));
		...
		Log.sysOut("Logout: "
				...
				+ " ("
				+ CMLib.time().date2SmartEllapsedTime(
						System.currentTimeMillis() - userLoginTime, true) + ")");
		if (inTheGame)
	...
	private void removeMOBFromGame(boolean killSession) {
		...
		if (M != null) {
			...
			if (!CMSecurity.isDisabled(CMSecurity.DisFlag.LOGOUTS)) {
				if (pstats != null) // cant do this when logouts are suspended
									// -- folks might get killed!
	...
	public void run() {
		...
		if ((activeMillis >= nextMsdpPing) && (connectionComplete)) {
			...
			if (getClientTelnetMode(TELNET_MSDP)) {
				byte[] msdpPingBuf = CMLib.protocol().pingMsdp(this,
						msdpReportables);
				if (msdpPingBuf != null) {
					try {
					...
					if (CMSecurity.isDebugging(CMSecurity.DbgFlag.TELNET))
						Log.debugOut("MSDP Reported: " + msdpPingBuf.length
								+ " bytes");
				}
			}
			if (getClientTelnetMode(TELNET_GMCP)) {
				byte[] gmcpPingBuf = CMLib.protocol().pingGmcp(this, gmcpPings,
						gmcpSupports);
				if (gmcpPingBuf != null) {
					try {
	...
	public void loginSystem() {
		try {
			...
			} else if (loginSession.skipInput)
			...
			if (!killFlag) {
				CharCreationLibrary.LoginResult loginResult = CMLib.login()
						.loginSystem(this, loginSession);
				switch (loginResult) {
				case INPUT_REQUIRED:
					return;
				case NO_LOGIN: {
				...
				case NORMAL_LOGIN: {
					...
					if ((!killFlag) && ((mob != null))) {
						if (mob.playerStats() != null)
							CMLib.threads().suspendResumeRecurse(mob, false,
									false);
						userLoginTime = System.currentTimeMillis();
						StringBuilder loginMsg = new StringBuilder("");
						loginMsg.append(getAddress())
								.append(" " + terminalType)
								.append(((CMath.bset(mob.getBitmap(),
										MOB.ATT_MXP) && getClientTelnetMode(Session.TELNET_MXP))) ? " MXP"
										: "")
								...
								.append((getClientTelnetMode(Session.TELNET_COMPRESS) || getClientTelnetMode(Session.TELNET_COMPRESS2)) ? " CMP"
										: "")
								.append(((CMath.bset(mob.getBitmap(),
										MOB.ATT_ANSI) && getClientTelnetMode(Session.TELNET_ANSI))) ? " ANSI"
										: "")
						...
						if (loginResult != CharCreationLibrary.LoginResult.NO_LOGIN) {
							CMMsg msg = CMClass.getMsg(mob, null,
									CMMsg.MSG_LOGIN, null);
							if (!CMLib.map().sendGlobalMessage(mob,
									CMMsg.TYP_LOGIN, msg))
								setKillFlag(true);
							else
								CMLib.commands().monitorGlobalMessage(
										mob.location(), msg);
						}
					}
	...
	public void logoutFinal() {
		try {
			...
			if (M != null)
				...
				finalMsg = "";
			previousCmd.clear(); // will let system know you are back in login
									// menu
			if (M != null) {
				try {
					if (CMSecurity.isDisabled(CMSecurity.DisFlag.LOGOUTS)) {
		...
		try {
			...
			if (input == null) {
				...
				if ((CMDS.size() > 0) && (mob != null)) {
					...
					PlayerStats pstats = mob.playerStats();
					String alias = (pstats != null) ? pstats
							.getAlias(firstWord) : "";
					Vector ALL_CMDS = new Vector();
					boolean echoOn = false;
					if (alias.length() > 0) {
						CMDS.remove(0);
						List<String> all_stuff = CMParms
								.parseSquiggleDelimited(alias, true);
					...
					for (int v = 0; v < ALL_CMDS.size(); v++) {
						...
						List<List<String>> MORE_CMDS = CMLib.lang()
								.preCommandParser(CMDS);
			...
			if (((System.currentTimeMillis() - lastBlahCheck) >= 60000)
				...
				Vector<String> V = CMParms.parse(CMProps
						.getVar(CMProps.Str.IDLETIMERS));
				if ((V.size() > 0)
						&& (!CMSecurity.isAllowed(mob(), mob().location(),
								CMSecurity.SecFlag.IDLEOK))
					...
					if (minsIdle >= CMath.s_int(V.firstElement())) {
						...
						println(mob(), null, null,
								"\n\r^ZIf you don't do something, you will be logged out in "
										+ remain + " minute(s)!^?");
					}
				}
				...
				if (!isAfk()) {
					if (getIdleMillis() >= 600000)
						setAfkFlag(true);
				} else if ((getIdleMillis() >= 10800000) && (!isStopped())) {
			...
			if ((needPrompt) && (waiting)) {
				if ((mob != null) && (mob.isInCombat())) {
					if (((System.currentTimeMillis() - promptLastShown) >= CMProps
							.getTickMillis()) || (input != null)) {
						showPrompt();
						needPrompt = false;
					}
				} else {
	...
	public static class LoginLogoutThread implements CMRunnable, Tickable {
		public String name() {
			return (theMOB == null) ? "Dead LLThread" : "LLThread for "
					+ theMOB.Name();
		}
		...
		public int compareTo(CMObject o) {
			return CMClass.classID(this)
					.compareToIgnoreCase(CMClass.classID(o));
		}
			...
			if ((!CMProps.getBoolVar(CMProps.Bool.MUDSHUTTINGDOWN))
				...
				try {
					for (Enumeration e = CMLib.map().rooms(); e
						...
						if ((!skipRooms.contains(R))
								&& (theMOB.location() != null))
							R.sendOthers(theMOB, msg);
					}
				} catch (Exception e) {
				}
		...
		public long activeTimeMillis() {
			return (activeMillis > 0) ? System.currentTimeMillis()
					- activeMillis : 0;
		}
	}
	...
	private static enum SESS_STAT_CODES {
		PREVCMD, ISAFK, AFKMESSAGE, ADDRESS, IDLETIME, LASTMSG, LASTNPCFIGHT, LASTPKFIGHT, TERMTYPE, TOTALMILLIS, TOTALTICKS, WRAP, LASTLOOPTIME
	}
	...
	private SESS_STAT_CODES getStatIndex(String code) {
		return (SESS_STAT_CODES) CMath
				.s_valueOf(SESS_STAT_CODES.values(), code);
	}
	...
	public String getStat(String code) {
		...
		switch (stat) {
		case PREVCMD:
			return CMParms.combineWithQuotes(getPreviousCMD(), 0);
		case ISAFK:
			return "" + isAfk();
		case AFKMESSAGE:
		...
		case ADDRESS:
			return getAddress();
		case IDLETIME:
			return CMLib.time().date2String(
					System.currentTimeMillis() - getIdleMillis());
		case LASTMSG:
			return CMParms.combineWithQuotes(getLastMsgs(), 0);
		case LASTNPCFIGHT:
		...
		case TOTALMILLIS:
			return CMLib.time().date2String(
					System.currentTimeMillis() - getTotalMillis());
		case TOTALTICKS:
	...
	public void setStat(String code, String val) {
		...
		case TERMTYPE:
			...
			break;
		case TOTALMILLIS:
			milliTotal = System.currentTimeMillis()
					- CMLib.time().string2Millis(val);
			break;
		case TOTALTICKS:
