...
public class DefaultClimate implements Climate {
	...
	protected final static int[] seasonalWeather = {
	...
	protected final static int[] cold = {
	/* - CL WD RA TH SN HA HE SL BL DU DR WC */
	...
	protected final static int[] hot = {
	/* - CL WD RA TH SN HA HE SL BL DU DR WC */
	/* SPRING */5, 5, -5, 10, 0, -5, 0, 5, -5, 0, 0, 0, -10,
	/* SUMMER */-5, -1, -5, 0, 0, -1, -1, 20, -1, -1, 0, 0, -5,
	/* FALL */0, 0, -1, 5, 0, -1, -1, 5, -1, -1, 0, 0, -5,
	/* WINTER */15, 0, 0, 4, 2, -5, -2, 0, -2, -2, 0, 0, -10, };
	protected final static int[] dry = {
	/* - CL WD RA TH SN HA HE SL BL DU DR WC */
	...
	protected final static int[] changeMap = {
	/* - CL WD RA TH SN HA HE SL BL DU DR WC */
	...
	public boolean canSeeTheMoon(Room room, Ability butNotA) {
		if (canSeeTheStars(room))
			return true;
		List<Ability> V = CMLib.flags().domainAffects(room,
				Ability.DOMAIN_MOONSUMMONING);
	...
	public boolean canSeeTheStars(Room room) {
		if (((room.getArea().getTimeObj().getTODCode() != TimeClock.TIME_NIGHT) && (room
				.getArea().getTimeObj().getTODCode() != TimeClock.TIME_DUSK))
	...
	public boolean canSeeTheSun(Room room) {
		if (((room.getArea().getTimeObj().getTODCode() != TimeClock.TIME_DAY) && (room
				.getArea().getTimeObj().getTODCode() != TimeClock.TIME_DAWN))
				|| (!CMLib.map().hasASky(room))
	...
	protected String getWeatherStop(int weatherCode) {
		if ((weatherCode >= 0) && (weatherCode < Climate.NUM_WEATHER))
			return CMProps.getListFileValue(CMProps.ListFile.WEATHER_ENDS,
					weatherCode);
		return "";
	}
	...
	public void weatherTick(Area A) {
		...
		if ((--weatherTicker) <= 0) {
			...
			for (int g = 0; g < Climate.NUM_WEATHER; g++) {
				// take the base chance for a seasonal weather occurrence (rain
				// in winter, etc)
				int seasonalNum = seasonal[(A.getTimeObj().getSeasonCode() * Climate.NUM_WEATHER)
						+ g];
				// find the chance of changing from what it will be, to some new
				// condition.
				int changeNum = changeMap[(nextWeather * Climate.NUM_WEATHER)
						+ g];
				// add them together to find the chance of a particular change
				// in a particular season
				// to a particular condition.
				int chance = seasonalNum + changeNum;
				// total all the change chances, negative means NO chance of
				// this change
				if (chance > 0)
			 ...
			 * StringBuffer(name()+"/"+(TimeClock.SEASON_DESCS
			 * [A.getTimeObj().getSeasonCode
			 * ()])+"/"+Climate.WEATHER_DESCS[nextWeather]+"->"); for(int
			 * g=0;g<Climate.NUM_WEATHER;g++) { int
			 * seasonalNum=seasonal[(A.getTimeObj
			 * ().getSeasonCode()*Climate.NUM_WEATHER)+g]; int
			 * changeNum=changeMap[(nextWeather*Climate.NUM_WEATHER)+g]; int
			 * chance=seasonalNum+changeNum; //if(chance>0)
			 * buf.append(Climate.WEATHER_DESCS
			 * [g]+"="+chance+"("+seasonalNum+"+"+changeNum+"), "); }
			 */

			// roll a number from this to that. Like the lottery, whosever
			// number gets rolled wins!
			...
			if (oldWeather != currentWeather) {
				...
				switch (sayMap[(oldWeather * Climate.NUM_WEATHER)
						+ currentWeather]) {
				case 0:
			...
			if ((say != null)
					&& !CMSecurity
							.isDisabled(CMSecurity.DisFlag.WEATHERNOTIFIES)) {
	...
	protected String theWeatherDescription(Area A, int weather) {
		...
		if ((weather < 0) || (weather >= Climate.NUM_WEATHER))
			return "";
		final int listFileOrd = CMProps.ListFile.WEATHER_CLEAR.ordinal()
				+ weather;
		...
		else if (((derivedClimate & Area.CLIMASK_HOT) > 0)
				|| (A.getTimeObj().getSeasonCode() == TimeClock.SEASON_SUMMER))
			prefix = CMProps.getListFileValue(listFileEnum, 3);
		else if ((derivedClimate & Area.CLIMASK_WET) > 0)
			prefix = CMProps.getListFileValue(listFileEnum, 1);
		else if ((derivedClimate & Area.CLIMASK_DRY) > 0)
		...
		desc.append((suffix.trim().length() > 0) ? prefix + " " + suffix
				: prefix);
		switch (weather) {
		case Climate.WEATHER_HAIL:
