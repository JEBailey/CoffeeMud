...
public class DefaultCoffeeShop implements CoffeeShop {
	...
	public SVector<Environmental> enumerableInventory = new SVector<Environmental>(); // for
																						// Only
																						// Inventory
																						// situations
	...
	public Iterator<Environmental> getStoreInventory() {
		return new ConvertingIterator<ShelfProduct, Environmental>(
				storeInventory.iterator(), converter);
	}

	public Iterator<Environmental> getStoreInventory(String srchStr) {
		List<Environmental> storeInv = new ConvertingList<ShelfProduct, Environmental>(
				storeInventory, converter);
		List<Environmental> V = CMLib.english().fetchEnvironmentals(storeInv,
				srchStr, true);
		if ((V != null) && (V.size() > 0))
			return V.iterator();
	...
	public Environmental addStoreInventory(Environmental thisThang, int number,
		...
		if (number < 0)
			number = 1;
		if ((isSold(ShopKeeper.DEAL_INVENTORYONLY))
				&& (!inEnumerableInventory(thisThang))) {
	...
	public boolean doIHaveThisInStock(String name, MOB mob) {
		List<Environmental> storeInv = new ConvertingList<ShelfProduct, Environmental>(
				storeInventory, converter);
		Environmental item = CMLib.english().fetchEnvironmental(storeInv, name,
				true);
		if (item == null)
			item = CMLib.english().fetchEnvironmental(storeInv, name, false);
		if ((item == null)
				&& (mob != null)
				&& ((isSold(ShopKeeper.DEAL_LANDSELLER))
						|| (isSold(ShopKeeper.DEAL_CLANDSELLER))
						|| (isSold(ShopKeeper.DEAL_SHIPSELLER)) || (isSold(ShopKeeper.DEAL_CSHIPSELLER)))) {
			List<Environmental> titles = CMLib.coffeeShops()
					.addRealEstateTitles(new Vector<Environmental>(), mob,
							this, startRoom());
	...
	public void resubmitInventory(List<Environmental> shopItems) {
		...
		for (Environmental shopItem : shopItems) {
			...
			addBacks.addElement(shopItem, Integer.valueOf(num),
					Integer.valueOf(price));
		}
		emptyAllShelves();
		for (int a = 0; a < addBacks.size(); a++)
			addStoreInventory((Environmental) addBacks.elementAt(a, 1),
			...
			for (ShelfProduct SP : storeInventory) {
				...
				if ((I instanceof Item) && (((Item) I).container() == product)) {
					if ((I instanceof DoorKey)
							&& (((DoorKey) I).getKey().equals(C.keyName())))
						foundKey = (DoorKey) I;
					((Item) I).unWear();
					V.addElement(I);
					storeInventory.remove(SP);
	...
	public String makeXML() {
		...
		itemstr.append(CMLib.xml().convertXMLtoTag("ISELL",
				shopKeeper().getWhatIsSoldMask()));
		itemstr.append(CMLib.xml().convertXMLtoTag("IPREJ",
				shopKeeper().prejudiceFactors()));
		itemstr.append(CMLib.xml().convertXMLtoTag("IBUDJ",
				shopKeeper().budget()));
		itemstr.append(CMLib.xml().convertXMLtoTag("IDVAL",
				shopKeeper().devalueRate()));
		itemstr.append(CMLib.xml().convertXMLtoTag("IGNOR",
				shopKeeper().ignoreMask()));
		itemstr.append(CMLib.xml().convertXMLtoTag("PRICM",
				CMParms.toStringList(shopKeeper().itemPricingAdjustments())));
		itemstr.append("<INVS>");
		for (Iterator<Environmental> i = getStoreInventory(); i.hasNext();) {
			...
			itemstr.append(CMLib.xml().convertXMLtoTag("ICLASS",
					CMClass.classID(E)));
			itemstr.append(CMLib.xml().convertXMLtoTag("INUM",
					"" + numberInStock(E)));
			itemstr.append(CMLib.xml().convertXMLtoTag("IVAL",
					"" + stockPrice(E)));
			itemstr.append(CMLib.xml().convertXMLtoTag("IDATA",
					CMLib.coffeeMaker().getPropertiesStr(E, true)));
			itemstr.append("</INV>");
		}
	...
	public void buildShopFromXML(String text) {
		...
		if (shop == null) {
			Log.errOut("DefaultCoffeeShop",
					"Error getting base shopKeeper obj host from " + text);
			return;
		}
		if (!text.trim().startsWith("<")) {
			String parm = CMParms.getParmStr(text, "ISELL", ""
					+ ShopKeeper.DEAL_ANYTHING);
			...
			if (parm != null)
				shop.setItemPricingAdjustments((parm.trim().length() == 0) ? new String[0]
						: CMParms.toStringArray(CMParms.parseCommas(parm, true)));
			return;
		}
		...
		List<XMLLibrary.XMLpiece> iV = CMLib.xml().getContentsFromPieces(xmlV,
				"INVS");
		if (iV == null) {
		...
		for (int i = 0; i < iV.size(); i++) {
			...
			String itemi = CMLib.xml()
					.getValFromPieces(iblk.contents, "ICLASS");
