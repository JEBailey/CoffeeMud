...
public class DefaultQuest implements Quest, Tickable, CMObject {
	...
	public void internalQuestDelete() {
		...
		if ((internalFiles != null) && (internalFiles.size() > 0)) {
			for (int i = 0; i < internalFiles.size(); i++) {
				String filename = ((String) internalFiles.elementAt(i, 1))
						.toUpperCase();
				Vector<String> delThese = new Vector<String>();
				boolean foundKey = false;
				for (Iterator<String> k = Resources.findResourceKeys(filename); k
						.hasNext();) {
					String key = k.next();
					if (key.startsWith("PARSEDPRG: ")
							&& (key.toUpperCase().endsWith(filename))) {
						foundKey = true;
	...
	public void setStartDate(String newDate) {
		...
		if ((x > 0) && (CMath.isMathExpression(newDate.substring(0, x)))
	...
	public void setVars(List<?> script, int startAtLine) {
		...
		String var = null;
		String val = null;
		List<List<String>> setScripts = CMLib.quests().parseQuestCommandLines(
				script, "SET", startAtLine);
	...
	public StringBuffer getResourceFileData(String named) {
		...
		StringBuffer buf = new CMFile(Resources.makeFileResourceName(named),
				null, CMFile.FLAG_LOGERRORS).text();
		return buf;
	}
	...
	private final Iterable<Room> buildAppropriateRoomIterable(
			final QuestState q, final Iterable useThese) {
	...
	private List sortSelect(Environmental E, String str, List choices,
			List choices0, List choices1, List choices2, List choices3) {
		...
		if (str.equalsIgnoreCase("any")) {
			...
			if ((choices == null) || (choices == choices2)
					|| (choices == choices3))
				choices = choices1;
			choices1.add(E);
	...
	private TimeClock getMysteryTimeNowFromState() {
		...
		if ((questState.mysteryData.whereAt != null)
			...
			NOW = (TimeClock) questState.mysteryData.whereAt.getArea()
					.getTimeObj().copyOf();
		else if ((questState.mysteryData.whereHappened != null)
				&& (questState.mysteryData.whereHappened.getArea() != null))
			NOW = (TimeClock) questState.mysteryData.whereHappened.getArea()
					.getTimeObj().copyOf();
		else if ((questState.room != null)
	...
	public void parseQuestScript(Vector script, List args, int startLine) {
		...
		for (int v = 0; v < script.size(); v++) {
			if (script.elementAt(v) instanceof String)
				...
				int vs = v;
				while ((v < script.size())
						&& (script.elementAt(v) instanceof List))
					v++;
		...
		for (int v = vStart; v < script.size(); v++) {
			...
			if (p.size() > 0) {
				...
				if (cmd.equals("<SCRIPT>")) {
					...
					while (((++v) < script.size())
							&& (!((String) script.elementAt(v)).trim()
									.toUpperCase().startsWith("</SCRIPT>")))
					...
					if (v >= script.size()) {
						errorOccurred(
								...
								"Quest '"
										+ name()
										+ "', <SCRIPT> command without </SCRIPT> found.");
						break;
					}
					if (!CMSecurity.isApprovedJScript(jscript)) {
						errorOccurred(
								...
								"Quest '"
										+ name()
										+ "', <SCRIPT> not approved.  Use MODIFY JSCRIPT to approve.");
						break;
					}
					...
					try {
						...
						scope.defineFunctionProperties(JScriptQuest.functions,
								JScriptQuest.class, ScriptableObject.DONTENUM);
						cx.evaluateString(scope, jscript.toString(), "<cmd>",
								1, null);
					} catch (Exception e) {
						errorOccurred(q, false, "Quest '" + name()
								+ "', JScript q.error: " + e.getMessage() + ".");
						Context.exit();
						break;
					}
					Context.exit();
				...
				if (cmd.equals("STEP")) {
					...
					if ((p.size() > 1)
							&& (p.elementAt(1).equalsIgnoreCase("BREAK"))) {
						q.lastLine = script.size();
						q.done = true;
					} else if ((p.size() > 1)
							&& (p.elementAt(1).equalsIgnoreCase("BACK"))) {
						if (startLine >= 0)
							q.lastLine = q.startLine;
					} else if ((p.size() > 1)
							&& (p.elementAt(1).equalsIgnoreCase("AUTO"))) {
						if (startLine >= 0)
							q.lastLine = v + 1;
				...
				if (cmd.equals("RESET")) {
					if (q.room != null)
						...
						errorOccurred(
								...
								"Quest '"
										+ name()
										+ "', no resettable room, roomgroup, area, or areagroup set.");
						break;
					}
				} else if (cmd.equals("SET")) {
					if (p.size() < 2) {
						errorOccurred(q, isQuiet, "Quest '" + name()
								+ "', unfound variable on set.");
						break;
					}
					...
					if (cmd.equals("AREA")) {
						...
						if ((p.size() > 3)
								&& (p.elementAt(2).equalsIgnoreCase("any")))
						...
						for (int n = 0; n < names.size(); n++) {
							...
							if (oldSize == areas.size())
								for (Enumeration e = CMLib.map().areas(); e
										.hasMoreElements();) {
						...
						if (areas.size() > 0)
							q.area = (Area) areas.elementAt(CMLib.dice().roll(
									1, areas.size(), -1));
						if (q.area == null) {
							errorOccurred(q, isQuiet,
									"Quest '" + name() + "', unknown area '"
											+ CMParms.combine(p, 2) + "'.");
							break;
						}
						...
						for (int n = 0; n < names.size(); n++) {
							...
							if (oldSize == areas.size()) {
								if (addAll) {
									for (Enumeration<Area> e = CMLib.map()
											.areas(); e.hasMoreElements();) {
						...
						if (areas.size() > 0) {
							...
							Area A = null;
							Room R = null;
							for (Enumeration<Area> e = areas.elements(); e
									.hasMoreElements();) {
								A = e.nextElement();
								for (Enumeration<Room> e2 = A.getMetroMap(); e2
										.hasMoreElements();) {
									R = e2.nextElement();
						...
						if ((p.size() > 2)
								&& (p.elementAt(2).equalsIgnoreCase("reselect"))) {
							p.removeElementAt(2);
							reselect = true;
						}
						if (p.size() < 3) {
						...
						try {
							...
							Vector<String> mobTypes = CMParms.parse(CMParms
									.combine(p, 2).toUpperCase());
							for (int t = 0; t < mobTypes.size(); t++) {
								...
								if (q.mobGroup == null) {
									try {
										for (Enumeration e = getAppropriateRoomSet(q); e
												.hasMoreElements();) {
											...
											for (int i = 0; i < R2
												...
												if ((M2 != null)
														&& (M2.isMonster())
														&& ((M2.amUltimatelyFollowing() == null) || (M2
								...
								for (int i = choices.size() - 1; i >= 0; i--) {
									...
									if ((M2 != null)
											...
											&& ((M2.amUltimatelyFollowing() == null) || (M2
													.amUltimatelyFollowing()
													.isMonster()))) {
										if ((CMClass.classID(M2).toUpperCase()
												.indexOf(mobType) >= 0)
												|| (M2.charStats().getMyRace()
														.racialCategory()
														.toUpperCase()
														.indexOf(mobType) >= 0)
												|| (M2.charStats().getMyRace()
							...
							if (choices.size() > 0) {
								for (int c = choices.size() - 1; c >= 0; c--)
									if (((!reselect) || (!q.reselectable
											.contains(choices.get(c))))
											&& (CMLib.quests().objectInUse(
													choices.get(c)) != null))
										choices.remove(c);
								if ((choices.size() == 0) && (!isQuiet))
									errorOccurred(q, isQuiet, "Quest '"
											+ name()
											+ "', all choices were taken: '"
											+ p + "'.");
							}
							if (choices.size() > 0)
								q.mob = choices.get(CMLib.dice().roll(1,
										choices.size(), -1));
						}
						if (q.mob == null) {
							errorOccurred(q, isQuiet, "Quest '" + name()
						...
						// why is this being done -- atm, this is a simple map
						// mob, minding his own business.
						...
						String maskStr = CMLib.quests()
								.breakOutMaskString(s, p);
						MaskingLibrary.CompiledZapperMask mask = (maskStr
								.trim().length() == 0) ? null : CMLib.masking()
								.maskCompile(maskStr);
						if (mask != null)
									...
									for (int i = 0; i < R2.numInhabitants(); i++) {
										...
										if ((M2 != null)
												...
												&& ((M2.amUltimatelyFollowing() == null) || (M2
														.amUltimatelyFollowing()
														.isMonster()))) {
											if (CMLib.masking().maskCheck(mask,
													M2, true)) {
												if (addAll) {
													choices = choices0;
													choices.add(M2);
												} else
													choices = sortSelect(M2,
															mobName, choices,
															choices0, choices1,
															choices2, choices3);
											}
						...
						try {
							...
							for (int t = 0; t < itemTypes.size(); t++) {
								String itemType = ((String) itemTypes
										.elementAt(t)).toUpperCase();
							...
							for (int t = 0; t < itemTypes.size(); t++) {
								...
								for (int i = choices.size() - 1; i >= 0; i--) {
									...
									if ((CMClass.classID(I2).toUpperCase()
											.indexOf(itemType) >= 0)
											|| (I2.name().toUpperCase()
													.indexOf(itemType) >= 0)
											|| (I2.displayText().toUpperCase()
													.indexOf(itemType) >= 0))
										choices.remove(I2);
								}
							}
							...
							if (choices.size() > 0)
								q.item = choices.get(CMLib.dice().roll(1,
										choices.size(), -1));
						}
						if (q.item == null) {
							errorOccurred(q, isQuiet, "Quest '" + name()
						...
						questifyScriptableBehavs(q.item); // this really makes
															// little sense,
															// though is
															// harmless
						if (reselect)
						...
						if (q.room != null)
							q.room.moveItemTo(q.item,
									ItemPossessor.Expire.Never,
						...
						q.preserveState = CMath.parseIntExpression(p
								.elementAt(2));
					else if (cmd.equals("LOCALE") || cmd.equals("LOCALEGROUP")
							|| cmd.equals("LOCALEGROUPAROUND")) {
						int range = 0;
						if (cmd.equals("LOCALE")) {
							try {
								q.room = (Room) getObjectIfSpecified(p, args,
										2, 0);
								if (q.room != null) {
									q.area = q.room.getArea();
									q.envObject = q.room;
							...
							if (q.room == null) {
								errorOccurred(q, isQuiet, "Quest '" + name()
										+ "', localegrouparound !room.");
								break;
							}
						} else {
							try {
								q.roomGroup = (List) getObjectIfSpecified(p,
										args, 2, 1);
								q.envObject = q.roomGroup;
								continue;
							} catch (CMException ex) {
						...
						if (range > 0) {
							...
							useThese = CMLib.tracking().getRadiantRooms(q.room,
									flags, range);
						}
						final Iterable<Room> list = buildAppropriateRoomIterable(
								q, useThese);
						for (int n = 0; n < names.size(); n++) {
							final String localeName = ((String) names
									.elementAt(n)).toUpperCase();
							try {
								final Iterator<Room> e = list.iterator();
								boolean addAll = (localeName
										.equalsIgnoreCase("any") || localeName
								...
								for (; e.hasNext();) {
									...
									if (addAll
											|| CMClass.classID(R2)
													.toUpperCase()
													.indexOf(localeName) >= 0)
										choices.add(R2);
									else {
										int dom = R2.domainType();
										if ((dom & Room.INDOORS) > 0) {
											if (Room.indoorDomainDescs[dom
													- Room.INDOORS]
														.indexOf(localeName) >= 0)
												choices.add(R2);
										} else if (Room.outdoorDomainDescs[dom]
												.indexOf(localeName) >= 0)
											choices.add(R2);
									}
								}
						...
						if (cmd.equalsIgnoreCase("LOCALEGROUP")
								|| cmd.equalsIgnoreCase("LOCALEGROUPAROUND")) {
							if (choices.size() > 0)
								q.roomGroup = choices;
							else {
								errorOccurred(
							...
							if (choices.size() > 0)
								q.room = choices.get(CMLib.dice().roll(1,
										choices.size(), -1));
							if (q.room == null) {
								errorOccurred(q, isQuiet,
										"Quest '" + name() + "', !locale '"
					...
					} else if (cmd.equals("ROOM") || cmd.equals("ROOMGROUP")
						...
						for (int n = 0; n < names.size(); n++) {
							...
							try {
								...
								for (; e.hasNext();) {
									final Room R2 = e.next();
									final String display = R2.displayText()
											.toUpperCase();
									final String desc = R2.description()
											.toUpperCase();
									if ((mask != null)
									...
									if (addAll) {
										...
										choices0.add(R2);
											...
											.getExtendedRoomID(R2)
											.equalsIgnoreCase(localeName)) {
										choices = choices0;
										choices0.add(R2);
						...
						if (cmd.equalsIgnoreCase("ROOMGROUP")
								|| cmd.equalsIgnoreCase("ROOMGROUPAROUND")) {
							if ((choices != null) && (choices.size() > 0))
								q.roomGroup = choices;
							else {
								errorOccurred(q, isQuiet,
										"Quest '" + name() + "', !roomgroup '"
												+ CMParms.combine(p, 2) + "'.");
								break;
							}
							q.envObject = q.roomGroup;
								...
								for (MOB M2 : q.mobGroup) {
									if ((M2 != null)
										...
										if (!CMLib.masking().maskCheck(mask,
												M2, true))
											continue;
										choices = sortSelect(M2, mobName,
												choices, choices0, choices1,
												choices2, choices3);
									}
								}
							} else {
									...
									for (Item I2 : q.itemGroup) {
										if (I2 != null) {
											if (!CMLib.masking().maskCheck(
													mask, I2, true))
												continue;
											choices = sortSelect(I2, itemName,
													choices, choices0,
													choices1, choices2,
													choices3);
										}
						...
						questifyScriptableBehavs(q.mob); // should be done to
															// loaded or
															// q-scripted mobs
															// only
						...
						try {
							...
							if (numStr.equalsIgnoreCase("ANY")) {
								int numFactions = CMLib.factions()
										.numFactions();
								int whichFaction = CMLib.dice().roll(1,
										numFactions, -1);
								int curFaction = 0;
								for (Enumeration<Faction> e = CMLib.factions()
										.factions(); e.hasMoreElements(); curFaction++) {
									F = e.nextElement();
						...
						try {
							q.mysteryData.factionGroup = (List) getObjectIfSpecified(
									p, args, 2, 1);
						} catch (CMException ex) {
							q.mysteryData.factionGroup = null;
							String numStr = CMParms.combine(p, 2);
							...
							if (q.mysteryData.faction != null)
								q.mysteryData.factionGroup
										.add(q.mysteryData.faction);
							if (CMath.isMathExpression(numStr)
									|| numStr.equalsIgnoreCase("ALL")) {
								int numFactions = CMLib.factions()
										.numFactions();
								...
								if (num >= numFactions)
									num = numFactions;
								int tries = 500;
								while ((q.mysteryData.factionGroup.size() < num)
										&& (--tries > 0)) {
									int whichFaction = CMLib.dice().roll(1,
											numFactions, -1);
									int curFaction = 0;
									for (Enumeration<Faction> e = CMLib
											.factions().factions(); e
											.hasMoreElements(); curFaction++) {
										F = e.nextElement();
										if (curFaction == whichFaction)
											break;
									}
						...
						if ((q.mysteryData.factionGroup != null)
							...
							q.mysteryData.faction = q.mysteryData.factionGroup
									.get(CMLib.dice().roll(1,
											q.mysteryData.factionGroup.size(),
											-1));
						...
						try {
							q.mysteryData.agentGroup = (List) getObjectIfSpecified(
									p, args, 2, 1);
							if ((q.mysteryData.agentGroup != null)
									&& (q.mysteryData.agentGroup.size() > 0)
									&& (q.mysteryData.agent == null))
								q.mysteryData.agent = q.mysteryData.agentGroup
										.get(CMLib
												.dice()
												.roll(1,
														q.mysteryData.agentGroup
																.size(), -1));
						} catch (CMException ex) {
							q.mysteryData.agentGroup = null;
							...
							if (q.mysteryData.agent != null)
								q.mysteryData.agentGroup
										.add(q.mysteryData.agent);
							int num = CMath.parseIntExpression(numStr);
							if (num >= V.size())
								num = V.size();
							while ((q.mysteryData.agentGroup.size() < num)
									&& (V.size() > 0)) {
					...
					} else if (cmd.equals("WHEREHAPPENEDGROUP")
							|| cmd.equals("WHEREATGROUP")) {
						if (q.mysteryData == null)
							q.mysteryData = new MysteryData();
						if (cmd.equals("WHEREHAPPENEDGROUP")) {
							try {
								q.mysteryData.whereHappenedGroup = (List) getObjectIfSpecified(
										p, args, 2, 1);
								q.roomGroup = q.mysteryData.whereHappenedGroup;
								q.mysteryData.whereHappened = ((q.roomGroup == null) || (q.roomGroup
										.size() == 0)) ? null
										: (Room) q.roomGroup
												.get(CMLib.dice().roll(1,
														q.roomGroup.size(), -1));
							...
							try {
								q.mysteryData.whereAtGroup = (List) getObjectIfSpecified(
										p, args, 2, 1);
								q.roomGroup = q.mysteryData.whereAtGroup;
								q.mysteryData.whereAt = ((q.roomGroup == null) || (q.roomGroup
										.size() == 0)) ? null
										: (Room) q.roomGroup
												.get(CMLib.dice().roll(1,
														q.roomGroup.size(), -1));
					...
					} else if (cmd.equals("WHENHAPPENEDGROUP")
							|| cmd.equals("WHENATGROUP")) {
						if (q.mysteryData == null)
							q.mysteryData = new MysteryData();
						List V2;
						TimeClock TC = null;
						if (cmd.equals("WHENHAPPENEDGROUP")) {
							try {
								q.mysteryData.whenHappenedGroup = (List) getObjectIfSpecified(
										p, args, 2, 1);
								V2 = q.mysteryData.whenHappenedGroup;
								if ((V2 != null) && (V2.size() > 0))
									q.mysteryData.whenHappened = (TimeClock) V2
											.get(CMLib.dice().roll(1,
													V2.size(), -1));
								continue;
							} catch (CMException ex) {
							...
							try {
								q.mysteryData.whenAtGroup = (List) getObjectIfSpecified(
										p, args, 2, 1);
								V2 = q.mysteryData.whenAtGroup;
								if ((V2 != null) && (V2.size() > 0))
									q.mysteryData.whenAt = (TimeClock) V2
											.get(CMLib.dice().roll(1,
													V2.size(), -1));
								continue;
							} catch (CMException ex) {
						...
						if ((V2.size() > 0) && (TC == null))
							TC = (TimeClock) V2.get(CMLib.dice().roll(1,
									V2.size(), -1));
						if (cmd.equals("WHENHAPPENEDGROUP"))
						...
						if (cmd.equals("WHENHAPPENED")) {
							try {
								q.mysteryData.whenHappened = (TimeClock) getObjectIfSpecified(
										p, args, 2, 0);
								continue;
							} catch (CMException ex) {
							...
							try {
								q.mysteryData.whenAt = (TimeClock) getObjectIfSpecified(
										p, args, 2, 0);
								continue;
							} catch (CMException ex) {
						...
						if (cmd.equals("WHENHAPPENED"))
							...
							q.mysteryData.whenAt = TC;
					} else if (cmd.equals("MOTIVEGROUP")
							|| cmd.equals("ACTIONGROUP")) {
						if (q.mysteryData == null)
							q.mysteryData = new MysteryData();
						List<String> V2 = null;
						int num = -1;
						if ((p.size() > 2)
								&& (CMath.isMathExpression(p.elementAt(2)))) {
							num = CMath.s_parseIntExpression(p.elementAt(2));
							p.removeElementAt(2);
						}
						if (cmd.equals("MOTIVEGROUP")) {
							try {
								q.mysteryData.motiveGroup = (List) getObjectIfSpecified(
										p, args, 2, 1);
								sizeDownTo(q.mysteryData.motiveGroup, num);
								V2 = q.mysteryData.motiveGroup;
								if ((V2 != null) && (V2.size() > 0))
									q.mysteryData.motive = V2.get(CMLib.dice()
											.roll(1, V2.size(), -1));
								continue;
							} catch (CMException ex) {
							...
							try {
								q.mysteryData.actionGroup = (List) getObjectIfSpecified(
										p, args, 2, 1);
								sizeDownTo(q.mysteryData.actionGroup, num);
								V2 = q.mysteryData.actionGroup;
								if ((V2 != null) && (V2.size() > 0))
									q.mysteryData.action = V2.get(CMLib.dice()
											.roll(1, V2.size(), -1));
								continue;
							} catch (CMException ex) {
						...
						if (p.size() < 3)
							continue;
						try {
							q.mysteryData.motive = (String) getObjectIfSpecified(
									p, args, 2, 0);
							continue;
						} catch (CMException ex) {
						...
						if (p.size() < 3)
							continue;
						try {
							q.mysteryData.action = (String) getObjectIfSpecified(
									p, args, 2, 0);
							continue;
						} catch (CMException ex) {
						...
						if (cmd.equals("WHEREHAPPENED")) {
							try {
								q.mysteryData.whereHappened = (Room) getObjectIfSpecified(
										p, args, 2, 0);
							...
							try {
								q.mysteryData.whereAt = (Room) getObjectIfSpecified(
										p, args, 2, 0);
						...
						if (p.size() > 2) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', " + cmd.toLowerCase() + " syntax '"
									+ CMParms.combine(p, 2) + "'.");
							break;
						}
						if (q.room == null) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', " + cmd.toLowerCase() + " !room.");
							break;
						}
						if (cmd.equals("WHEREHAPPENED"))
						...
						if (cmd.equals("TARGETGROUP")) {
							try {
								q.mysteryData.targetGroup = (List) getObjectIfSpecified(
										p, args, 2, 1);
								V2 = q.mysteryData.targetGroup;
								if ((V2 != null) && (V2.size() > 0)) {
									if (V2.get(0) instanceof MOB) {
										q.mobGroup = V2;
										q.mob = (MOB) V2.get(CMLib.dice().roll(
												1, V2.size(), -1));
										q.envObject = q.mobGroup;
							...
							try {
								q.mysteryData.toolGroup = (List) getObjectIfSpecified(
										p, args, 2, 1);
						...
						if (finalE instanceof MOB) {
							...
							q.mob = (MOB) finalE;
							questifyScriptableBehavs(q.mob); // i just dont get
																// it
					...
					} else if (cmd.equals("TARGET") || cmd.equals("TOOL")) {
						if (q.mysteryData == null)
							q.mysteryData = new MysteryData();
						if (cmd.equals("TARGET")) {
							try {
								q.mysteryData.target = (Environmental) getObjectIfSpecified(
										p, args, 2, 0);
							...
							try {
								q.mysteryData.tool = (Environmental) getObjectIfSpecified(
										p, args, 2, 0);
						...
						if ((q.envObject instanceof List)
							...
							q.envObject = ((List) q.envObject).get(CMLib.dice()
									.roll(1, ((List) q.envObject).size(), -1));
						if (!(q.envObject instanceof Environmental)) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', " + cmd.toLowerCase() + " !object.");
							break;
						}
						if (cmd.equals("TARGET"))
						...
						if (q.envObject instanceof MOB) {
							...
							questifyScriptableBehavs(q.mob); // useless, but
																// harmless
					...
					if (cmd.equals("MOBS")) {
						...
						StringBuffer buf = getResourceFileData(CMParms.combine(
								p, 2));
						...
						String errorStr = CMLib.coffeeMaker().addMOBsFromXML(
								buf.toString(), q.loadedMobs, null);
						...
						String errorStr = CMLib.coffeeMaker().addItemsFromXML(
								buf.toString(), q.loadedItems, null);
						if (errorStr.length() > 0) {
							errorOccurred(q, isQuiet,
									"Quest '" + name()
											+ "',Error on import of: '"
						...
						errorOccurred(q, isQuiet, "Quest '" + name()
								+ "', unknown import type '" + cmd + "'.");
						break;
					}
				} else if (cmd.startsWith("LOAD=")) {
					boolean error = q.error;
					List args2 = new Vector();
					parseQuestScriptWArgs(parseLoadScripts(s, args, args2),
							args2);
					if ((!error) && (q.error))
						break;
					...
					if (cmd.equals("MOB") || cmd.equals("MOBGROUP")) {
						if (q.loadedMobs.size() == 0) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', cannot load mob, no mobs imported.");
							break;
						}
						...
						if ((p.size() > 2)
								&& (CMath.isMathExpression(p.elementAt(2)))) {
							maxToLoad = CMath
									.parseIntExpression(p.elementAt(2));
							p.removeElementAt(2);
						}
						if (p.size() < 3) {
							errorOccurred(q, isQuiet, "Quest '" + name()
							...
							if ((CMLib.masking().maskCheck(mask, M2, true))
									&& (addAll
											|| (CMLib.english().containsString(
													M2.name(), mobName))
											|| (CMLib.english().containsString(
													M2.displayText(), mobName)) || (CMLib
												.english().containsString(
											M2.description(), mobName))))
								choices.add((MOB) M2.copyOf());
						}
						if (choices.size() == 0) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', no mob found to load '" + mobName
									+ "'!");
							break;
						}
						List<MOB> mobsToDo = new Vector<MOB>();
						if (cmd.equalsIgnoreCase("MOB"))
							mobsToDo.add(choices.get(CMLib.dice().roll(1,
									choices.size(), -1)));
						else {
						...
						while ((mobsToDo.size() > maxToLoad) && (maxToLoad > 0))
							mobsToDo.remove(CMLib.dice().roll(1,
									mobsToDo.size(), -1));
						while ((mobsToDo.size() < maxToLoad) && (maxToLoad > 0)
								&& (maxToLoad < Integer.MAX_VALUE))
							mobsToDo.add((MOB) mobsToDo.get(
									CMLib.dice().roll(1, mobsToDo.size(), -1))
									.copyOf());
						...
						for (int m = 0; m < mobsToDo.size(); m++) {
							...
							if (q.room == null) {
								if (q.roomGroup != null)
									q.room = q.roomGroup.get(CMLib.dice().roll(
											1, q.roomGroup.size(), -1));
								else if (q.area != null)
							...
							if (q.room != null) {
								...
								q.mob.basePhyStats()
										.setRejuv(PhyStats.NO_REJUV);
								q.mob.basePhyStats().setDisposition(
										q.mob.basePhyStats().disposition()
							...
							if (q.mob != null)
								q.mob.setStartRoom(null); // necessary to tell
															// qm to clean him
															// UP!
						}
						...
						if (q.loadedItems.size() == 0) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', cannot load item, no items imported.");
							break;
						}
						int maxToLoad = Integer.MAX_VALUE;
						if ((p.size() > 2)
						...
						for (int i = 0; i < q.loadedItems.size(); i++) {
							...
							if ((itemName.equalsIgnoreCase("any"))
									|| (CMLib.english().containsString(
											I2.name(), itemName))
									|| (CMLib.english().containsString(
											I2.displayText(), itemName))
									|| (CMLib.english().containsString(
											I2.description(), itemName)))
								choices.add((Item) I2.copyOf());
						}
						if (choices.size() == 0) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', no item found to load '" + itemName
									+ "'!");
							break;
						}
						List<Item> itemsToDo = new Vector<Item>();
						if (cmd.equalsIgnoreCase("ITEM"))
							itemsToDo.add(choices.get(CMLib.dice().roll(1,
									choices.size(), -1)));
						else {
							itemsToDo.addAll(choices);
							q.itemGroup = choices;
						}
						while ((itemsToDo.size() > maxToLoad)
								&& (maxToLoad > 0))
							itemsToDo.remove(CMLib.dice().roll(1,
									itemsToDo.size(), -1));
						while ((itemsToDo.size() < maxToLoad)
								&& (maxToLoad > 0)
								&& (maxToLoad < Integer.MAX_VALUE))
							itemsToDo.add((Item) (itemsToDo.get(CMLib.dice()
									.roll(1, itemsToDo.size(), -1))).copyOf());
						Room choiceRoom = q.room;
						for (int m = 0; m < itemsToDo.size(); m++) {
							...
							if (q.room != null) {
								q.item.basePhyStats().setRejuv(
										PhyStats.NO_REJUV);
								q.item.basePhyStats().setDisposition(
										q.item.basePhyStats().disposition()
												| PhyStats.IS_UNSAVABLE);
					...
					if (cmd.equals("FOLLOWER")) {
						if (q.mob == null) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', cannot give follower, no mob set.");
							break;
						}
						if (p.size() < 3) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give follower, follower name not given.");
							break;
						}
						String mobName = CMParms.combine(p, 2);
						List<MOB> choices = new Vector();
						ReverseEnumeration<PreservedQuestObject> e;
						for (e = new ReverseEnumeration<PreservedQuestObject>(
								q.worldObjects); e.hasMoreElements();) {
						...
						if (choices.size() == 0) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give follower, no mobs called '"
											+ mobName
											+ "' previously set in script.");
							break;
						}
						MOB M2 = choices.get(CMLib.dice().roll(1,
								choices.size(), -1));
						M2.setFollowing(q.mob);
					} else if (cmd.equals("ITEM")
							|| (cmd.equalsIgnoreCase("ITEMS"))) {
						if ((q.item == null) && (q.itemGroup == null)
							...
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give item(s), no item(s) set or loaded.");
							break;
						}
						if ((q.mob == null) && (q.mobGroup == null)) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', cannot give item(s), no mob set.");
							break;
						}
						if (p.size() > 2) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give item(s), parameter unnecessarily given: '"
											+ CMParms.combine(p, 2) + "'.");
							break;
						}
						...
						for (int i = 0; i < toSet.size(); i++) {
							...
							if (cmd.equals("ITEMS")) {
								...
								Item I3 = itemSet.get(CMLib.dice().roll(1,
										itemSet.size(), -1));
								questifyScriptableBehavs(I3);
								if (q.item == I3) {
									M2.moveItemTo(I3);
						...
						if ((q.mob == null) && (q.mobGroup == null)) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', cannot give ability, no mob set.");
							break;
						}
						if (p.size() < 3) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give ability, ability name not given.");
							break;
						}
						...
						if (A3 == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give ability, ability name unknown '"
											+ (p.elementAt(2)) + ".");
							break;
						}
						List<MOB> toSet = new Vector<MOB>();
						...
						for (int i = 0; i < toSet.size(); i++) {
							MOB M2 = toSet.get(i);
							runtimeRegisterAbility(M2, A3.ID(),
									CMParms.combineWithQuotes(p, 3), true);
						}
						...
						if (q.envObject == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give behavior, no mob or item set.");
							break;
						}
						if (p.size() < 3) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give behavior, behavior name not given.");
							break;
						}
						...
						if (B == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give behavior, behavior name unknown '"
											+ (p.elementAt(2)) + ".");
							break;
						}
						...
						for (int i = 0; i < toSet.size(); i++) {
							...
							if (E2 instanceof PhysicalAgent)
								runtimeRegisterBehavior((PhysicalAgent) E2,
										B.ID(),
										CMParms.combineWithQuotes(p, 3), true);
						}
						...
						if (q.envObject == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give stat, no mob or item set.");
							break;
						}
						if (p.size() < 3) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give stat, stat name not given.");
							break;
						}
						...
						for (int i = 0; i < toSet.size(); i++) {
							...
							if (stat.equalsIgnoreCase("KEYPLAYER")
									&& (E2 instanceof Physical)) {
								Ability A = ((Physical) E2)
										.fetchEffect("QuestBound");
								if (A != null)
									A.setStat("KEY", val);
							} else
						...
						if (q.envObject == null) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', cannot give script, no object set.");
							break;
						}
						boolean proceed = true;
						boolean savable = false;
						String word = null;
						String scope = CMStrings.replaceAll(name(), " ", "_")
								.toUpperCase().trim();
						while (proceed && (p.size() > 2)) {
							...
							if (word.equalsIgnoreCase("SAVABLE")) {
								...
								scope = "";
								proceed = true;
							} else if (word.equalsIgnoreCase("INDIVIDUAL")
									|| word.equals("*")) {
								scope = "*";
								proceed = true;
							}
						...
						if (p.size() < 3) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give script, script not given.");
							break;
						}
						...
						for (int i = 0; i < toSet.size(); i++) {
							...
							if (E2 instanceof PhysicalAgent) {
								...
								synchronized (questState) {
									questState.addons.addElement(new XVector(
											E2, S), Integer
											.valueOf(questState.preserveState));
								}
							}
						}
						...
						if (q.envObject == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give Effect, no mob, room or item set.");
							break;
						}
						if (p.size() < 3) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give Effect, ability name not given.");
							break;
						}
						Ability A3 = CMClass.findAbility(p.elementAt(2));
						if (A3 == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot give Effect, ability name unknown '"
											+ (p.elementAt(2)) + ".");
							break;
						}
						List toSet = new Vector();
						...
						for (Object o : toSet)
							if (o instanceof PhysicalAgent)
								runtimeRegisterEffect((PhysicalAgent) o,
										A3.ID(),
										CMParms.combineWithQuotes(p, 3), true);
					} else {
						errorOccurred(q, isQuiet, "Quest '" + name()
								+ "', unknown give type '" + cmd + "'.");
						break;
					}
				} else if (cmd.equals("TAKE")) {
					if (p.size() < 2) {
						errorOccurred(q, isQuiet, "Quest '" + name()
								+ "', unfound type on take.");
						break;
					}
					cmd = p.elementAt(1).toUpperCase();
					if (cmd.equals("ABILITY")) {
						if ((q.mob == null) && (q.mobGroup == null)) {
							errorOccurred(q, isQuiet, "Quest '" + name()
									+ "', cannot take ability, no mob set.");
							break;
						}
						if (p.size() < 3) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot take ability, ability name not given.");
							break;
						}
						Ability A3 = CMClass.findAbility(p.elementAt(2));
						if (A3 == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot take ability, ability name unknown '"
											+ (p.elementAt(2)) + ".");
							break;
						}
						List<MOB> toSet = new Vector<MOB>();
						...
						for (int i = 0; i < toSet.size(); i++) {
							MOB M2 = toSet.get(i);
							runtimeRegisterAbility(M2, A3.ID(),
									CMParms.combineWithQuotes(p, 3), false);
						}
					} else if (cmd.equals("BEHAVIOR")) {
						if (q.envObject == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot take behavior, no mob or item set.");
							break;
						}
						if (p.size() < 3) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot take behavior, behavior name not given.");
							break;
						}
						Behavior B = CMClass.getBehavior(p.elementAt(2));
						if (B == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot take behavior, behavior name unknown '"
											+ (p.elementAt(2)) + ".");
							break;
						}
						List toSet = new Vector();
						...
						if (q.envObject == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot take Effect, no mob, room or item set.");
							break;
						}
						if (p.size() < 3) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot take Effect, ability name not given.");
							break;
						}
						Ability A3 = CMClass.findAbility(p.elementAt(2));
						if (A3 == null) {
							errorOccurred(
									...
									"Quest '"
											+ name()
											+ "', cannot take Effect, ability name unknown '"
											+ (p.elementAt(2)) + ".");
							break;
						}
						List toSet = new Vector();
	...
	public boolean spawnQuest(String script, Vector baseVars, boolean reTime) {
		...
		if (reTime) {
			...
			ellapsed = Long.valueOf(ellapsed.longValue()
					+ (System.currentTimeMillis() - lastStartDateTime));
	...
	public boolean startQuest() {
		if ((!running()) && (!isCopy()))
			CMLib.coffeeTables().bump(this,
					CoffeeTableRow.STAT_QUESTSTARTATTEMPT);
	...
	public boolean startQuestInternal() {
		...
		if ((!isCopy()) && (getSpawn() != SPAWN_NO)) {
			if (getSpawn() == SPAWN_FIRST)
				...
				List<String> parsed = CMLib.quests().parseQuestSteps(
						baseScript, 0, false);
			...
			enterDormantState();
			return false; // always return false, since, per se, this quest is
							// NOT started.
		}
		try {
		...
		if (questState.error) {
			if (!questState.beQuiet) {
				...
				if ((durable) && (resetData == null))
					...
					retry = resetData[0];
				Log.errOut("Quest", "Errors starting '" + name()
						+ "', quest not started"
			...
			CMLib.coffeeTables().bump(this,
					CoffeeTableRow.STAT_QUESTFAILEDSTART);
		} else if (!questState.done)
			Log.errOut("Quest", "Nothing parsed in '" + name()
					+ "', quest not started.");
		else if (duration() < 0) {
			Log.errOut("Quest", "No duration, quest '" + name()
					+ "' not started.");
			questState.error = true;
		}
	...
	public void cleanQuestStep() {
		...
		if (questState.worldObjects.size() > 0) {
			synchronized (questState) {
				...
				for (e = new ReverseEnumeration<PreservedQuestObject>(
						questState.worldObjects); e.hasMoreElements();) {
					PO = e.nextElement();
					...
					if (P instanceof Item) {
						if ((CMath.bset(P.basePhyStats().disposition(),
								PhyStats.IS_UNSAVABLE))
								&& (!((Item) P).amDestroyed()))
							((Item) P).destroy();
					} else if (P instanceof MOB) {
						MOB M = (MOB) P;
						ScriptingEngine B = (ScriptingEngine) M
								.fetchBehavior("Scriptable");
						if (B != null)
							B.endQuest(M, M, name());
						Room R = M.getStartRoom();
						if ((R == null)
								|| (CMath.bset(M.basePhyStats().disposition(),
										PhyStats.IS_UNSAVABLE))) {
							M.setFollowing(null);
				...
				synchronized (questState) {
					if (stoppingQuest)
						for (int i = questState.addons.size() - 1; i >= 0; i--) {
							Integer I = (Integer) questState.addons.elementAt(
									i, 2);
							if (I.intValue() > 0) {
								questState.addons.setElementAt(i, 2,
							...
							if (O instanceof String) {
								...
								if (CMStrings.contains(E.getStatCodes(), stat
									...
									E.setStat(stat, parms);
										...
										&& CMStrings.contains(((Physical) E)
												.basePhyStats().getStatCodes(),
												stat.toUpperCase().trim())) {
									((Physical) E).basePhyStats().setStat(
											stat.toUpperCase().trim(), parms);
									((Physical) E).recoverPhyStats();
								} else if ((E instanceof MOB)
										&& (CMStrings.contains(CharStats.CODES
												.NAMES(), stat.toUpperCase()
												.trim()))) {
									((MOB) E).baseCharStats().setStat(
											CMParms.indexOf(
													CharStats.CODES.NAMES(),
													stat.toUpperCase().trim()),
											CMath.s_int(parms));
									((MOB) E).recoverCharStats();
								} else if ((E instanceof MOB)
										&& CMStrings.contains(((MOB) E)
												.baseState().getStatCodes(),
												stat)) {
								...
								if (E instanceof PhysicalAgent) {
									...
									Behavior B = BB
											.fetchBehavior(((Behavior) O).ID());
									if ((E instanceof MOB)
											&& (B instanceof ScriptingEngine))
										((ScriptingEngine) B).endQuest(
												(PhysicalAgent) E, (MOB) E,
												name());
									if ((V.size() > 2)
								...
								if ((V.size() > 2)
										...
										&& (E instanceof MOB)) {
									Ability A = ((MOB) E)
											.fetchAbility(((Ability) O).ID());
									if ((V.size() > 3)
											&& (V.get(3) instanceof String)) {
										if (A == null) {
											A = (Ability) O;
											((MOB) E).addAbility(A);
										}
									...
									Ability A = ((Physical) E)
											.fetchEffect(((Ability) O).ID());
									if ((V.size() > 2)
											&& (V.get(2) instanceof String)) {
										if (A == null) {
											A = (Ability) O;
	...
	public boolean stepQuest() {
		...
		if (questState.error) {
			if (!questState.beQuiet)
				Log.errOut("Quest", "One or more errors in '" + name()
						+ "', quest not started");
		} else if (!questState.done) {
		...
		if (startDate().length() > 0) {
			if (startDate().toUpperCase().startsWith("MUDDAY")) {
				...
				if ((mudmonth < NOW.getMonth())
						|| ((mudmonth == NOW.getMonth()) && (mudday < NOW
								.getDayOfMonth())))
				...
				if (x < 0)
					return false;
				int month = CMath.s_parseIntExpression(startDate
						.substring(0, x));
				int day = CMath
						.s_parseIntExpression(startDate.substring(x + 1));
				int year = Calendar.getInstance().get(Calendar.YEAR);
				long distance = CMLib.time().string2Millis(
						month + "/" + day + "/" + year + " 12:00 AM");
				while (distance < System.currentTimeMillis())
					distance = CMLib.time().string2Millis(
							month + "/" + day + "/" + (++year) + " 12:00 AM");
				waitRemaining = (int) ((distance - System.currentTimeMillis()) / CMProps
						.getTickMillis());
			}
		} else
			waitRemaining = (minWait + (CMLib.dice().roll(1, maxWait, 0)))
					- (int) (ellapsedTime / CMProps.getTickMillis());
		return true;
	}
		...
		if (running()) {
			...
			if (ticksRemaining < 0) {
				if ((questState != null) && (questState.autoStepAfterDuration)) {
					...
					CMLib.coffeeTables().bump(this,
							CoffeeTableRow.STAT_QUESTTIMESTOP);
					stopQuest();
				}
			}
			...
			if (startQuestOnTime()) {
				CMLib.coffeeTables().bump(this,
						CoffeeTableRow.STAT_QUESTTIMESTART);
			}
		}
	...
	protected boolean startQuestOnTime() {
		...
		if (runLevel() >= 0)
			for (int q = 0; q < CMLib.quests().numQuests(); q++) {
				...
				if ((!Q.name().equals(name)) && (Q.running())
						&& (Q.duration() != 0) && (Q.runLevel() <= runLevel())) {
					allowedToRun = false;
					break;
				}
			}
		if (allowedToRun) {
			...
			for (Session S : CMLib.sessions().localOnlineIterable())
				if ((S.mob() != null)
						&& ((!isMask) || CMLib.masking().maskCheck(playerMask,
								S.mob(), true)))
					numElligiblePlayers++;
			ticksRemaining = -1;
	...
	public void runtimeRegisterAbility(MOB mob, String abilityID, String parms,
		...
		synchronized (questState) {
			questState.addons.addElement(V,
					Integer.valueOf(questState.preserveState));
		}
	}
	...
	public void runtimeRegisterObject(PhysicalAgent P) {
		synchronized (questState) {
			...
			questState.worldObjects.add(new PreservedQuestObject(P,
					questState.preserveState));
	...
	public void runtimeRegisterEffect(PhysicalAgent affected, String abilityID,
			String parms, boolean give) {
		if (affected == null)
			return;
	...
	public void runtimeRegisterBehavior(PhysicalAgent behaving,
			String behaviorID, String parms, boolean give) {
		if (behaving == null)
			return;
	...
	public void runtimeRegisterStat(Environmental E, String stat, String parms,
			boolean give) {
		if (E == null)
			return;
		if (E instanceof PhysicalAgent)
		...
		if (CMStrings.contains(E.getStatCodes(), stat))
			oldVal = E.getStat(stat);
				...
				&& CMStrings.contains(((Physical) E).basePhyStats()
						.getStatCodes(), stat))
			oldVal = ((Physical) E).basePhyStats().getStat(stat);
		else if ((E instanceof MOB)
				&& (CMStrings.contains(CharStats.CODES.NAMES(), stat)))
			oldVal = ""
					+ ((MOB) E).baseCharStats().getStat(
							CMParms.indexOf(CharStats.CODES.NAMES(), stat));
		else if ((E instanceof MOB)
				&& CMStrings.contains(((MOB) E).baseState().getStatCodes(),
						stat))
	...
	public int getQuestThingIndex(Iterator<? extends Environmental> i,
			String name, CMClass.CMObjectType type, int[] num) {
		if (i == null)
			return -1;
		Environmental E;
		for (; i.hasNext();) {
			...
			if (CMClass.isType(E, type)) {
				switch (type) {
				case LOCALE:
					if (CMLib.map().getExtendedRoomID((Room) E)
	...
	public int getQuestMobIndex(String name) {
		...
		synchronized (questState) {
			if (questState.worldObjects != null)
				x = getQuestThingIndex(
						PreservedQuestObject.getPOIter(questState.worldObjects),
						name, CMClass.CMObjectType.MOB, num);
			if ((x < 0) && (questState.mobGroup != null))
				x = getQuestThingIndex(questState.mobGroup.iterator(), name,
						CMClass.CMObjectType.MOB, num);
			if ((x < 0) && (questState.mysteryData != null)
					&& (questState.mysteryData.agentGroup != null))
				x = getQuestThingIndex(
						questState.mysteryData.agentGroup.iterator(), name,
						CMClass.CMObjectType.MOB, num);
		}
		return x;
	}
	...
	public int getQuestRoomIndex(String roomID) {
		...
		synchronized (questState) {
			if (questState.worldObjects != null)
				x = getQuestThingIndex(
						PreservedQuestObject.getPOIter(questState.worldObjects),
						roomID, CMClass.CMObjectType.LOCALE, num);
			if ((x < 0) && (questState.roomGroup != null))
				x = getQuestThingIndex(questState.roomGroup.iterator(), roomID,
						CMClass.CMObjectType.LOCALE, num);
			if (questState.mysteryData != null) {
				if ((x < 0) && (questState.mysteryData.whereAtGroup != null))
					x = getQuestThingIndex(
							questState.mysteryData.whereAtGroup.iterator(),
							roomID, CMClass.CMObjectType.LOCALE, num);
				if ((x < 0)
						&& (questState.mysteryData.whereHappenedGroup != null))
					x = getQuestThingIndex(
							questState.mysteryData.whereHappenedGroup
									.iterator(),
							roomID, CMClass.CMObjectType.LOCALE, num);
			}
		}
	...
	public int getQuestItemIndex(String name) {
		...
		synchronized (questState) {
			if (questState.worldObjects != null)
				x = getQuestThingIndex(
						PreservedQuestObject.getPOIter(questState.worldObjects),
						name, CMClass.CMObjectType.ITEM, num);
			if ((x < 0) && (questState.itemGroup != null))
				x = getQuestThingIndex(questState.itemGroup.iterator(), name,
						CMClass.CMObjectType.ITEM, num);
			if (questState.mysteryData != null) {
				if ((x < 0) && (questState.mysteryData.toolGroup != null))
					x = getQuestThingIndex(
							questState.mysteryData.toolGroup.iterator(), name,
							CMClass.CMObjectType.ITEM, num);
			}
		}
		return x;
	...
	public Environmental getQuestThing(Iterator<? extends Environmental> e,
			int dex, CMClass.CMObjectType type, int[] num) {
		if (e == null)
			return null;
		Environmental E;
		for (; e.hasNext();) {
	...
	public MOB getQuestMob(int i) {
		...
		synchronized (questState) {
			if (questState.worldObjects != null)
				E = getQuestThing(
						PreservedQuestObject.getPOIter(questState.worldObjects),
						i, CMClass.CMObjectType.MOB, num);
			if (E instanceof MOB)
				return (MOB) E;
			if (questState.mobGroup != null)
				E = getQuestThing(questState.mobGroup.iterator(), i,
						CMClass.CMObjectType.MOB, num);
			if (E instanceof MOB)
				return (MOB) E;
			if ((questState.mysteryData != null)
					&& (questState.mysteryData.agentGroup != null))
				E = getQuestThing(questState.mysteryData.agentGroup.iterator(),
						i, CMClass.CMObjectType.MOB, num);
			if (E instanceof MOB)
				return (MOB) E;
		}
	...
	public Item getQuestItem(int i) {
		...
		synchronized (questState) {
			if (questState.worldObjects != null)
				E = getQuestThing(
						PreservedQuestObject.getPOIter(questState.worldObjects),
						i, CMClass.CMObjectType.ITEM, num);
			if (E instanceof Item)
				return (Item) E;
			if (questState.itemGroup != null)
				E = getQuestThing(questState.itemGroup.iterator(), i,
						CMClass.CMObjectType.ITEM, num);
			if (E instanceof Item)
				return (Item) E;
			if ((questState.mysteryData != null)
					&& (questState.mysteryData.toolGroup != null))
				E = getQuestThing(questState.mysteryData.toolGroup.iterator(),
						i, CMClass.CMObjectType.ITEM, num);
			if (E instanceof Item)
				return (Item) E;
		}
	...
	public Room getQuestRoom(int i) {
		...
		synchronized (questState) {
			if (questState.worldObjects != null)
				E = getQuestThing(
						PreservedQuestObject.getPOIter(questState.worldObjects),
						i, CMClass.CMObjectType.LOCALE, num);
			if (E instanceof Room)
				return (Room) E;
			E = getQuestThing(questState.roomGroup.iterator(), i,
					CMClass.CMObjectType.LOCALE, num);
			if (E instanceof Room)
				return (Room) E;
			if ((questState.mysteryData != null)
					&& (questState.mysteryData.whereAtGroup != null))
				E = getQuestThing(
						questState.mysteryData.whereAtGroup.iterator(), i,
						CMClass.CMObjectType.LOCALE, num);
			if (E instanceof Room)
				return (Room) E;
			if ((questState.mysteryData != null)
					&& (questState.mysteryData.whereHappenedGroup != null))
				E = getQuestThing(
						questState.mysteryData.whereHappenedGroup.iterator(),
						i, CMClass.CMObjectType.LOCALE, num);
			if (E instanceof Room)
				return (Room) E;
		}
			...
			if (V.size() > 0) {
				...
				try {
					...
					Log.errOut(
							...
							"'"
									+ text
									+ "' either has a space in the filename, or unknown parms.");
				}
			}
		}
		int x = text.toLowerCase().indexOf(
				XMLLibrary.FILE_XML_BOUNDARY.toLowerCase());
		if (x >= 0) {
			String xml = text.substring(
					x + XMLLibrary.FILE_XML_BOUNDARY.length()).trim();
			text = text.substring(0, x);
			if ((xml.length() > 0) && (internalFiles == null)) {
				List<XMLLibrary.XMLpiece> topXMLV = CMLib.xml()
						.parseAllXML(xml);
				for (int t = 0; t < topXMLV.size(); t++) {
					XMLLibrary.XMLpiece filePiece = topXMLV.get(t);
					String name = null;
					String data = null;
					if (filePiece.tag.equalsIgnoreCase("FILE")
							&& (filePiece.contents != null))
						for (int p = 0; p < filePiece.contents.size(); p++) {
							XMLLibrary.XMLpiece piece = filePiece.contents
									.get(p);
					...
					if ((name != null) && (data != null)
						...
						internalFiles.addElement(name.toUpperCase().trim(),
	...
	private String modifyStringFromArgs(String s, List args) {
		...
		while ((x >= 0) && (x < s.length() - 1)) {
			int y = x + 1;
			if ((y < s.length())
					&& (VALID_ASTR_CODES.indexOf(s.charAt(y)) >= 0))
				y++;
			while ((y < s.length()) && (Character.isLetterOrDigit(s.charAt(y))))
				y++;
			try {
				String possObjName = (x + 1 >= y) ? null : s
						.substring(x + 1, y);
				if ((possObjName != null) && (possObjName.length() > 0)) {
					...
					Object O = getObjectIfSpecified(
							CMParms.paramParse(possObjName), args, 0, 0);
					...
					case '&':
						...
						break;
					case '|':
						replace = CMLib.english().cleanArticles(replace).trim()
								.replace(' ', '|');
						break;
					}
	...
	private Object getObjectIfSpecified(List<String> parms, List args,
			int startp, int object0vector1) throws CMException {
		...
		for (int i = 0; i <= allParms.length(); i++) {
			...
			if ((i == allParms.length())
					|| ((i < allParms.length()) && ((allParms.charAt(i) == '+')
							|| (allParms.charAt(i) == '-') || (allParms
							.charAt(i) == '#')))) {
				eval = allParms.substring(lastI, i).trim().toUpperCase();
				lastI = i + 1;
			}
			if (eval != null) {
				if (eval.startsWith("ARG")
						&& CMath.isMathExpression(eval.substring(3))) {
	...
	public void setStat(String code, String val) {
		...
		default:
			if ((code.toUpperCase().trim().equalsIgnoreCase("REMAINING"))
					&& (running()))
	...
	public boolean isStat(String code) {
		if ((getCodeNum(code) >= 0) || (questState.vars.containsKey(code)))
			return true;
		return false;
	}
	...
	public String getStat(String code) {
		...
		default: {
			...
			if (questState.isStat(code)) {
				...
				if (O != null)
					return O.toString();
			} else if (code.endsWith("_ROOMID")
					&& (questState.isStat(code.substring(0, code.length() - 7)))) {
				code = code.substring(0, code.length() - 7);
				Object O = questState.getStat(code);
				if (O instanceof List) {
					if (((List) O).size() > 0)
						O = ((List) O).get(CMLib.dice().roll(1,
								((List) O).size(), -1));
				}
				if (O instanceof Environmental) {
			...
			} else if (code.endsWith("_CLASS")
					&& (questState.isStat(code.substring(0, code.length() - 6)))) {
	 ...
	 * quest-state to quest-state. This object maps a world object in use to its
	...
	public static class PreservedQuestObject {
		...
		public static Iterator<PhysicalAgent> getPOIter(
				List<PreservedQuestObject> o) {
			ConvertingIterator<PreservedQuestObject, PhysicalAgent> iter = new ConvertingIterator<PreservedQuestObject, PhysicalAgent>(
					o.iterator(), converter);
			return iter;
		}
	}
	 ...
	 * is highly state dependent, but the state is not explicit in the script
	 * itself.
	 * 
	 * @author bzimmerman
	 * 
	...
	public static class QuestState implements Cloneable {
		...
		public Object getStat(String statName) {
			...
			case 8:
				...
				break;
			case 9:
				O = new ConvertingList<PreservedQuestObject, PhysicalAgent>(
						worldObjects, PreservedQuestObject.converter);
				break;
			case 10:
				O = mob;
				break;
	...
	/**
	 * MysteryData is a helper class for QuestState that stores quest state
	 * variables for the parsing of quests that make use of the "mystery"
	 * variables (mystery as in Sherlock Holmes, not as in unknown).
	 * 
	 * @author bzimmerman
	 */
	...
	protected static class JScriptQuest extends ScriptableObject {
		...
		public static String[] functions = { "quest", "setupState",
				"toJavaString" };
