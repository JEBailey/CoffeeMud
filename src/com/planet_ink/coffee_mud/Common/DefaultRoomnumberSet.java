...
public class DefaultRoomnumberSet implements RoomnumberSet {
		...
		if (x <= 0) {
			...
			if ((x >= 0) && (theRest.endsWith(")"))
				...
				if (comma > 0) {
					...
					roomNum += Long.parseLong(theRest.substring(comma + 1,
							theRest.length() - 1));
	...
	public void parseXML(String xml) {
		...
		if ((V == null) || (V.size() == 0))
			return;
		List<XMLLibrary.XMLpiece> xV = CMLib.xml().getContentsFromPieces(V,
				"AREAS");
		root.clear();
		String ID = null;
		String NUMS = null;
