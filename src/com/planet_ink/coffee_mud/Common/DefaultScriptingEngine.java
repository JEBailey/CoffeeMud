...
public class DefaultScriptingEngine implements ScriptingEngine {
	...
	protected boolean noDelay = CMSecurity
			.isDisabled(CMSecurity.DisFlag.SCRIPTABLEDELAY);
	...
	public DefaultScriptingEngine() {
		super();
		// CMClass.bumpCounter(this,CMClass.CMObjectType.COMMON);//removed for
		// mem & perf
	...
	public void setVarScope(String newScope) {
		if ((newScope == null) || (newScope.trim().length() == 0)
				|| newScope.equalsIgnoreCase("GLOBAL")) {
			scope = "";
	...
	public String getLocalVarXML() {
		...
		for (Iterator<String> k = resources._findResourceKeys("SCRIPTVAR-"); k
				.hasNext();) {
			String key = k.next();
			if (key.startsWith("SCRIPTVAR-")) {
				...
				for (Enumeration e = H.keys(); e.hasMoreElements();) {
					...
					str.append("<" + vn + ">"
							+ CMLib.xml().parseOutAngleBrackets(val) + "</"
							+ vn + ">");
				}
				str.append("</" + key.substring(10) + ">");
			}
	...
	public void setLocalVarXML(String xml) {
		for (Iterator<String> k = Resources.findResourceKeys("SCRIPTVAR-"); k
				.hasNext();) {
			String key = k.next();
			if (key.startsWith("SCRIPTVAR-"))
	...
	private Quest getQuest(String named) {
		if ((defaultQuestName.length() > 0)
				&& (named.equals("*") || named
						.equalsIgnoreCase(defaultQuestName)))
			return defaultQuest();
	...
	public CMObject copyOf() {
		try {
			...
			// CMClass.bumpCounter(S,CMClass.CMObjectType.COMMON);//removed for
			// mem & perf
			S.reset();
			return S;
	...
	// protected void
	// finalize(){CMClass.unbumpCounter(this,CMClass.CMObjectType.COMMON);}//removed
	// for mem & perf
	...
	protected String[] parseBits(String line, String instructions) {
		...
		for (int i = 0; i < instructions.length(); i++)
			...
			case 'r':
				...
				break;
			case 'R':
				newLine[i] = CMParms.getPastBitClean(line, i - 1).toUpperCase()
						.trim();
				break;
			case 'p':
				...
				break;
			case 'P':
				newLine[i] = CMParms.getPastBit(line, i - 1).toUpperCase()
						.trim();
				break;
			case 'S':
			...
			case 't': {
				...
				if (CMParms.getCleanBit(s, 0).equalsIgnoreCase("P")) {
					...
					int numNewBits = (s.trim().length() == 0) ? 1 : CMParms
							.numBits(s);
	 ...
	 * c=clean bit, r=pastbitclean, p=pastbit, s=remaining clean bits, t=trigger
	 */
	protected String[] parseBits(String[][] oldBits, int start,
			String instructions) {
		String[] tt = oldBits[0];
	...
	public boolean endQuest(PhysicalAgent hostObj, MOB mob, String quest) {
		if (mob != null) {
			...
			for (int v = 0; v < scripts.size(); v++) {
				...
				if (script.size() > 0) {
					trigger = ((String) script.elementAt(0, 1)).toUpperCase()
							.trim();
					...
					if ((getTriggerCode(trigger, tt) == 13) // questtimeprog
						...
						if ((tt[1].equals(quest) || (tt[1].equals("*")))
							...
							execute(hostObj, mob, mob, mob, null, null, script,
									null, newObjs());
							return true;
						}
					}
	...
	protected String getVarHost(Environmental E, String rawHost, MOB source,
			Environmental target, PhysicalAgent scripted, MOB monster,
			Item primaryItem, Item secondaryItem, String msg, Object[] tmp) {
		if (!rawHost.equals("*")) {
			if (E == null)
				rawHost = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, rawHost);
	...
	public String getVar(Environmental E, String rawHost, String var,
			MOB source, Environmental target, PhysicalAgent scripted,
			MOB monster, Item primaryItem, Item secondaryItem, String msg,
			Object[] tmp) {
		return getVar(
				getVarHost(E, rawHost, source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp), var);
	}
	...
	public String getVar(String host, String var) {
		...
		if (H != null)
			...
			if (M != null)
				for (Enumeration<ScriptingEngine> e = M.scripts(); e
						.hasMoreElements();) {
					ScriptingEngine SE = e.nextElement();
					if ((SE != null)
							&& (SE != this)
							&& (defaultQuestName.equalsIgnoreCase(SE
									.defaultQuestName()))
							&& (SE.isVar(host, var)))
	...
	private StringBuffer getResourceFileData(String named) {
		if (getQuest("*") != null)
			return getQuest("*").getResourceFileData(named);
		return new CMFile(Resources.makeFileResourceName(named), null,
	...
	public void setScript(String newParms) {
		...
		if (myScript.length() > 100)
			scriptKey = "PARSEDPRG: " + myScript.substring(0, 100)
					+ myScript.length() + myScript.hashCode();
		else
	...
	protected String parseLoads(String text, int depth, Vector filenames,
			StringBuffer nonFilenameScript) {
		StringBuffer results = new StringBuffer("");
		String parse = text;
		if (depth > 10)
			return ""; // no including off to infinity
		String p = null;
		while (parse.length() > 0) {
			...
			if (y >= 0) {
				...
				if (z > 0) {
					...
					results.append(parseLoads(getResourceFileData(filename)
							.toString(), depth + 1, filenames, null));
				} else {
	...
	protected Room getRoom(String thisName, Room imHere) {
		...
		if ((imHere != null) && (imHere.roomID().equalsIgnoreCase(thisName)))
			return imHere;
		if ((imHere != null) && (thisName.startsWith("#"))
				&& (CMath.isLong(thisName.substring(1))))
		...
		if ((room != null) && (room.roomID().equalsIgnoreCase(thisName))) {
			if (CMath.bset(room.getArea().flags(), Area.FLAG_INSTANCE_PARENT)
					&& (imHere != null)
					&& (CMath.bset(imHere.getArea().flags(),
							Area.FLAG_INSTANCE_CHILD))
					&& (imHere.getArea().Name().endsWith("_"
							+ room.getArea().Name()))
					&& (thisName.indexOf('#') >= 0)) {
				Room otherRoom = CMLib.map().getRoom(
						imHere.getArea().Name()
								+ thisName.substring(thisName.indexOf('#')));
				if ((otherRoom != null)
						&& (otherRoom.roomID().endsWith(thisName)))
					return otherRoom;
			}
			return room;
		}
		...
		if ((imHere != null) && (imHere.getArea() != null))
			rooms = CMLib.map().findAreaRoomsLiberally(null, imHere.getArea(),
					thisName, "RIEPM", 100);
		if (rooms.size() == 0) {
			if (debugBadScripts)
				Log.debugOut("ScriptingEngine",
						"World room search called for: " + thisName);
			rooms = CMLib.map().findWorldRoomsLiberally(null, thisName,
					"RIEPM", 100, 2000);
		}
		if (rooms.size() > 0)
	...
	protected void logError(Environmental scripted, String cmdName,
			String errType, String errMsg) {
		if (scripted != null) {
			Room R = CMLib.map().roomLocation(scripted);
			Log.errOut("Scripting", scripted.name() + "/"
					+ CMLib.map().getExtendedRoomID(R) + "/" + cmdName + "/"
					+ errType + "/" + errMsg);
			if (R != null)
				R.showHappens(CMMsg.MSG_OK_VISUAL,
						"Scripting Error: " + scripted.name() + "/"
								+ CMLib.map().getExtendedRoomID(R) + "/"
								+ CMParms.toStringList(externalFiles()) + "/"
								+ cmdName + "/" + errType + "/" + errMsg);
		} else
			Log.errOut("Scripting",
					"*/*/" + CMParms.toStringList(externalFiles()) + "/"
							+ cmdName + "/" + errType + "/" + errMsg);

	}

	protected boolean simpleEvalStr(Environmental scripted, String arg1,
			String arg2, String cmp, String cmdName) {
	...
	protected boolean simpleEval(Environmental scripted, String arg1,
			String arg2, String cmp, String cmdName) {
	...
	protected boolean simpleExpressionEval(Environmental scripted, String arg1,
			String arg2, String cmp, String cmdName) {
	...
	protected List<PhysicalAgent> loadMobsFromFile(Environmental scripted,
			String filename) {
		filename = filename.trim();
		List monsters = (List) Resources.getResource("RANDOMMONSTERS-"
				+ filename);
		if (monsters != null)
			return monsters;
		...
		Room R = CMLib.map().roomLocation(scripted);
		if (R != null)
			thangName = scripted.name() + " at "
					+ CMLib.map().getExtendedRoomID((Room) scripted);
		else if (scripted != null)
			thangName = scripted.name();
		if ((buf == null) || (buf.length() < 20)) {
			logError(scripted, "XMLLOAD", "?", "Unknown XML file: '" + filename
					+ "' in " + thangName);
			return null;
		}
		if (buf.substring(0, 20).indexOf("<MOBS>") < 0) {
			logError(scripted, "XMLLOAD", "?", "Invalid XML file: '" + filename
					+ "' in " + thangName);
			return null;
		}
		monsters = new Vector();
		String error = CMLib.coffeeMaker().addMOBsFromXML(buf.toString(),
				monsters, null);
		if (error.length() > 0) {
			logError(scripted, "XMLLOAD", "?", "Error in XML file: '"
					+ filename + "'");
			return null;
		}
		if (monsters.size() <= 0) {
	...
	protected List<PhysicalAgent> loadItemsFromFile(Environmental scripted,
			String filename) {
		filename = filename.trim();
		...
		items = new Vector();
		String error = CMLib.coffeeMaker().addItemsFromXML(buf.toString(),
				items, null);
		if (error.length() > 0) {
			logError(scripted, "XMLLOAD", "?", "Error in XML file: '"
					+ filename + "'");
	...
	protected Environmental findSomethingCalledThis(String thisName, MOB meMOB,
			Room imHere, List<Environmental> OBJS, boolean mob) {
		if (thisName.length() == 0)
			return null;
		...
		if (thisName.toUpperCase().trim().startsWith("FROMFILE ")) {
			try {
				...
				if (mob)
					...
					V = loadItemsFromFile(null,
							CMParms.getCleanBit(thisName, 1));
				if (V != null) {
					...
					if (name.equalsIgnoreCase("ALL"))
						...
						if (V.size() > 0)
							areaThing = (Environmental) V.get(CMLib.dice()
									.roll(1, V.size(), -1));
					} else {
						areaThing = CMLib.english().fetchEnvironmental(V, name,
								true);
						if (areaThing == null)
							areaThing = CMLib.english().fetchEnvironmental(V,
									name, false);
					}
				}
			} catch (Exception e) {
			...
			try {
				if (areaThing == null) {
					...
					if (mob) {
						all.addAll(CMLib.map().findInhabitants(
								A.getProperMap(), null, thisName, 100));
						if (all.size() == 0)
							all.addAll(CMLib.map().findShopStock(
									A.getProperMap(), null, thisName, 100));
						for (int a = all.size() - 1; a >= 0; a--)
							if (!(all.elementAt(a) instanceof MOB))
								all.removeElementAt(a);
						if (all.size() > 0)
							areaThing = (Environmental) all.elementAt(CMLib
									.dice().roll(1, all.size(), -1));
						else {
							all.addAll(CMLib.map().findInhabitants(
									CMLib.map().rooms(), null, thisName, 100));
							if (all.size() == 0)
								all.addAll(CMLib.map().findShopStock(
										CMLib.map().rooms(), null, thisName,
										100));
							for (int a = all.size() - 1; a >= 0; a--)
								if (!(all.elementAt(a) instanceof MOB))
									all.removeElementAt(a);
							if (all.size() > 0)
								thing = (Environmental) all.elementAt(CMLib
										.dice().roll(1, all.size(), -1));
						}
					}
					if (all.size() == 0) {
						all.addAll(CMLib.map().findRoomItems(A.getProperMap(),
								null, thisName, true, 100));
						if (all.size() == 0)
							all.addAll(CMLib.map().findInventory(
									A.getProperMap(), null, thisName, 100));
						if (all.size() == 0)
							all.addAll(CMLib.map().findShopStock(
									A.getProperMap(), null, thisName, 100));
						if (all.size() > 0)
							...
							all.addAll(CMLib.map().findRoomItems(
									CMLib.map().rooms(), null, thisName, true,
									100));
							if (all.size() == 0)
								all.addAll(CMLib.map().findInventory(
										CMLib.map().rooms(), null, thisName,
	...
	protected PhysicalAgent getArgumentMOB(String str, MOB source, MOB monster,
			Environmental target, Item primaryItem, Item secondaryItem,
			String msg, Object[] tmp) {
		return getArgumentItem(str, source, monster, monster, target,
				primaryItem, secondaryItem, msg, tmp);
	}

	protected PhysicalAgent getArgumentItem(String str, MOB source,
			MOB monster, PhysicalAgent scripted, Environmental target,
			Item primaryItem, Item secondaryItem, String msg, Object[] tmp) {
		if (str.length() < 2)
			return null;
		if (str.charAt(0) == '$') {
			if (Character.isDigit(str.charAt(1))) {
				...
				else if ((O instanceof List) && (str.length() > 3)
						&& (str.charAt(2) == '.')) {
					List V = (List) O;
					String back = str.substring(2);
					if (back.charAt(1) == '$')
						back = varify(source, target, scripted, monster,
								primaryItem, secondaryItem, msg, tmp, back);
					if ((back.length() > 1)
				...
				switch (str.charAt(1)) {
				case 'a':
					return (lastKnownLocation != null) ? lastKnownLocation
							.getArea() : null;
				case 'B':
				case 'b':
					return (lastLoaded instanceof PhysicalAgent) ? (PhysicalAgent) lastLoaded
							: null;
				case 'N':
				case 'n':
					return ((source == backupMOB) && (backupMOB != null) && (monster != scripted)) ? scripted
							: source;
				case 'I':
				case 'i':
					return scripted;
				case 'T':
				case 't':
					return ((target == backupMOB) && (backupMOB != null) && (monster != scripted)) ? scripted
							: (target instanceof PhysicalAgent) ? (PhysicalAgent) target
									: null;
				case 'O':
				case 'o':
					return primaryItem;
				...
				case 'X':
					if (lastKnownLocation != null) {
						if ((str.length() > 2)
								&& (Directions.getGoodDirectionCode(""
										+ str.charAt(2)) >= 0))
							return lastKnownLocation.getExitInDir(Directions
									.getGoodDirectionCode("" + str.charAt(2)));
						int i = 0;
						Exit E = null;
						while (((++i) < 100) || (E != null))
							E = lastKnownLocation.getExitInDir(CMLib.dice()
									.roll(1, Directions.NUM_DIRECTIONS(), -1));
						return E;
					}
					return null;
				case '[': {
		...
		if (lastKnownLocation != null) {
			str = varify(source, target, scripted, monster, primaryItem,
					secondaryItem, msg, tmp, str);
			Environmental E = lastKnownLocation.fetchFromRoomFavorMOBs(null,
					str);
			if (E == null)
				E = lastKnownLocation.fetchFromMOBRoomFavorsItems(monster,
						null, str, Wearable.FILTER_ANY);
			if (E == null)
	...
	public String varify(MOB source, Environmental target,
		...
		while ((t >= 0) && (t < varifyable.length() - 1)) {
			...
			if (Character.isDigit(c))
				...
				case '@':
					if ((t < varifyable.length() - 2)
							&& Character.isLetter(varifyable.charAt(t + 2))) {
						Environmental E = getArgumentItem(
								"$" + varifyable.charAt(t + 2), source,
								monster, scripted, target, primaryItem,
								secondaryItem, msg, tmp);
				...
				case 'C':
					...
					break;
				case 'd':
					middle = (lastKnownLocation != null) ? lastKnownLocation
							.displayText(monster) : "";
					break;
				case 'D':
					middle = (lastKnownLocation != null) ? lastKnownLocation
				...
				case 'l':
					if (lastKnownLocation != null) {
						...
						for (int i = 0; i < lastKnownLocation.numInhabitants(); i++) {
							...
							if ((M != null) && (M != monster)
									&& (CMLib.flags().canBeSeenBy(M, monster)))
								str.append("\"" + M.name() + "\" ");
						}
						middle = str.toString();
					}
					...
					if (lastKnownLocation != null) {
						...
						for (int i = 0; i < lastKnownLocation.numItems(); i++) {
							...
							if ((I != null) && (I.container() == null)
									&& (CMLib.flags().canBeSeenBy(I, monster)))
								str.append("\"" + I.name() + "\" ");
						}
						middle = str.toString();
					}
				...
				case 'T':
					...
					break;
				case 'w':
					middle = primaryItem != null ? primaryItem.owner().Name()
							: middle;
					break;
				case 'W':
					middle = secondaryItem != null ? secondaryItem.owner()
							.Name() : middle;
					break;
				case 'x':
				case 'X':
					if (lastKnownLocation != null) {
						...
						if ((t < varifyable.length() - 2)
								&& (Directions.getGoodDirectionCode(""
										+ varifyable.charAt(t + 2)) >= 0)) {
							dir = Directions.getGoodDirectionCode(""
									+ varifyable.charAt(t + 2));
							E = lastKnownLocation.getExitInDir(dir);
						} else {
							int i = 0;
							while (((++i) < 100) || (E != null)) {
								dir = CMLib.dice().roll(1,
				...
				case '<': {
					...
					if (x >= 0) {
						...
						if (y >= 0) {
							...
							E = getArgumentItem(arg1, source, monster, monster,
									target, primaryItem, secondaryItem, msg,
									tmp);
							mid = mid.substring(y + 1).trim();
						}
						if (arg1.length() > 0)
							middle = getVar(E, arg1, mid, source, target,
									scripted, monster, primaryItem,
									secondaryItem, msg, tmp);
				...
				case '%': {
					...
					int x = back.indexOf('%');
					if (x >= 0) {
						middle = functify(monster, source, target, monster,
								primaryItem, secondaryItem, msg, tmp, back
										.substring(0, x).trim());
						back = back.substring(x + 1);
					}
			...
			if ((back.startsWith(".")) && (back.length() > 1)) {
				if (back.charAt(1) == '$')
					back = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, back);
	...
	protected DVector getScriptVarSet(String mobname, String varname) {
		...
		if (mobname.equals("*")) {
			...
			Hashtable H = (Hashtable) resources._getResource("SCRIPTVAR-"
					+ mobname);
			if (varname.equals("*")) {
				for (Enumeration e = H.keys(); e.hasMoreElements();) {
					String vn = (String) e.nextElement();
	...
	protected String getStatValue(Environmental E, String arg2) {
		...
		if ((!found) && (E instanceof MOB)) {
			...
			for (int i : CharStats.CODES.ALL())
				if (CharStats.CODES.NAME(i).equalsIgnoreCase(arg2)
						|| CharStats.CODES.DESC(i).equalsIgnoreCase(arg2)) {
					val = "" + M.charStats().getStat(CharStats.CODES.NAME(i)); // yes,
																				// this
																				// is
																				// right
					found = true;
			...
			if (!found)
				for (int i = 0; i < M.curState().getStatCodes().length; i++)
					if (M.curState().getStatCodes()[i].equalsIgnoreCase(arg2)) {
						val = M.curState().getStat(
								M.curState().getStatCodes()[i]);
						found = true;
						break;
					}
			if (!found)
				for (int i = 0; i < M.phyStats().getStatCodes().length; i++)
					if (M.phyStats().getStatCodes()[i].equalsIgnoreCase(arg2)) {
						val = M.phyStats().getStat(
								M.phyStats().getStatCodes()[i]);
						found = true;
						break;
					}
			if ((!found) && (M.playerStats() != null))
				for (int i = 0; i < M.playerStats().getStatCodes().length; i++)
					if (M.playerStats().getStatCodes()[i]
							.equalsIgnoreCase(arg2)) {
						val = M.playerStats().getStat(
								M.playerStats().getStatCodes()[i]);
						found = true;
						break;
					}
			if ((!found) && (arg2.toUpperCase().startsWith("BASE")))
				for (int i = 0; i < M.baseState().getStatCodes().length; i++)
					if (M.baseState().getStatCodes()[i].equalsIgnoreCase(arg2
							.substring(4))) {
						val = M.baseState().getStat(
								M.baseState().getStatCodes()[i]);
						found = true;
						break;
					}
			if ((!found) && (gstatH.containsKey(arg2.toUpperCase()))) {
				...
				case GSTATADD_CLAN: {
					...
					if (C == null)
						C = M.clans().iterator().hasNext() ? M.clans()
								.iterator().next().first : null;
					val = (C != null) ? C.clanID() : "";
					break;
				}
			...
			if (E instanceof MOB) {
				for (int i = 0; i < GenericBuilder.GENMOBCODES.length; i++) {
					if (GenericBuilder.GENMOBCODES[i].equalsIgnoreCase(arg2)) {
						val = CMLib.coffeeMaker().getGenMobStat((MOB) E,
								GenericBuilder.GENMOBCODES[i]);
						found = true;
						break;
					}
				}
				if (!found) {
					...
					for (int i : CharStats.CODES.ALL())
						if (CharStats.CODES.NAME(i).equalsIgnoreCase(arg2)
								|| CharStats.CODES.DESC(i).equalsIgnoreCase(
										arg2)) {
							val = ""
									+ M.charStats().getStat(
											CharStats.CODES.NAME(i));
							found = true;
							break;
						}
					if (!found)
						for (int i = 0; i < M.curState().getStatCodes().length; i++)
							if (M.curState().getStatCodes()[i]
									.equalsIgnoreCase(arg2)) {
								val = M.curState().getStat(
										M.curState().getStatCodes()[i]);
								found = true;
					...
					if ((!found) && (arg2.toUpperCase().startsWith("BASE")))
						for (int i = 0; i < M.baseState().getStatCodes().length; i++)
							if (M.baseState().getStatCodes()[i]
									.equalsIgnoreCase(arg2.substring(4))) {
								val = M.baseState().getStat(
										M.baseState().getStatCodes()[i]);
								found = true;
								break;
						...
						case GSTATADD_CLANROLE: {
							...
							if (C == null)
								C = M.clans().iterator().hasNext() ? M.clans()
										.iterator().next().first : null;
							if (C != null) {
								Pair<Clan, Integer> p = M.getClanRole(C
										.clanID());
								val = (p != null) ? p.second.toString() : "";
							}
							break;
				...
				for (int i = 0; i < GenericBuilder.GENITEMCODES.length; i++) {
					if (GenericBuilder.GENITEMCODES[i].equalsIgnoreCase(arg2)) {
						val = CMLib.coffeeMaker().getGenItemStat((Item) E,
								GenericBuilder.GENITEMCODES[i]);
						found = true;
						break;
					}
				}
	...
	public void setVar(String baseName, String key, String val) {
		...
		for (int v = 0; v < V.size(); v++) {
			...
			Hashtable H = (Hashtable) resources._getResource("SCRIPTVAR-"
					+ name);
			if ((H == null) && (defaultQuestName != null)
			...
			if (val.equals("++")) {
				...
				if (amount == 0)
					Log.errOut("Scripting",
							"Scripting SetVar error: Division by 0: " + name
									+ "/" + key + "=" + val);
				else
					val = Integer.toString(CMath.s_int(num.trim()) / amount);
			}
	...
	public String[] parseEval(String evaluable) throws ScriptParseException {
		...
		for (int c = 0; c < evalC.length; c++)
				...
				if (Character.isWhitespace(evalC[c])) {
					...
					if (s.length() > 0) {
						...
						if (funcH.containsKey(s))
							...
							throw new ScriptParseException("Unknown keyword: "
									+ s);
					}
				} else if (Character.isLetter(evalC[c])) { /* move along */
				} else
					switch (evalC[c]) {
					case '!': {
						if (c == evalC.length - 1)
					...
					case ')':
						...
						break;
					default:
						throw new ScriptParseException("Unknown character at: "
								+ new String(evalC, dex, c - dex + 1).trim()
								+ ": " + evaluable);
					}
				break;
			}
			case STATE_MAYFUNCTION: {
				if (evalC[c] == '(') {
					...
					throw new ScriptParseException("Expected ( at " + evalC[c]
							+ ": " + evaluable);
				break;
			}
		...
		if ((state == STATE_POSTFUNCQUOTE) || (state == STATE_INFUNCQUOTE))
			throw new ScriptParseException("Unclosed " + lastQuote
					+ " somewhere");
		if (depth > 0)
		...
		if (stack.size() > 0) {
			...
			if (O instanceof Integer) {
				...
				if ((stack.size() > 0)
					...
					boolean preTrueFalse = ((Boolean) stack.elementAt(stack
							.size() - 1)).booleanValue();
					stack.removeElementAt(stack.size() - 1);
					switch (connector) {
					case CONNECTOR_AND:
				...
				boolean preTrueFalse = ((Boolean) stack
						.elementAt(stack.size() - 1)).booleanValue();
				stack.removeElementAt(stack.size() - 1);
				trueFalse = preTrueFalse && trueFalse;
			}
		}
		...
		for (int t = startEval; t < tt.length; t++)
			if (tt[t].equals("("))
				...
				if ((!(stack.lastElement() instanceof Boolean))
					...
					logError(scripted, "EVAL", "SYNTAX", ") Format error: "
							+ CMParms.toStringList(tt));
					return false;
				}
				...
				if ((stack.size() > 0)
						&& (stack.lastElement() instanceof Integer)) {
				...
				if ((t == tt.length - 1) || (!tt[t + 1].equals("("))) {
					logError(scripted, "EVAL", "SYNTAX", "No ( for fuction "
							+ tt[t] + ": " + CMParms.toStringList(tt));
					return false;
				}
				t += 2;
				int tlen = 0;
				...
				if ((t + tlen) == tt.length) {
					logError(scripted, "EVAL", "SYNTAX", "No ) for fuction "
							+ tt[t - 1] + ": " + CMParms.toStringList(tt));
					return false;
				}
				...
				case 2: // has
				{
					...
					String arg1 = tt[t + 0];
					String arg2 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 1]);
					Environmental E = getArgumentItem(arg1, source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
					if (arg2.length() == 0) {
						logError(scripted, "HAS", "Syntax", funcParms);
						return returnable;
					}
					Environmental E2 = getArgumentItem(arg2, source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
					if (E == null)
						returnable = false;
						...
						returnable = CMLib.english().containsString(E.name(),
								arg2);
					else if (E instanceof Room) {
				...
				case 74: // hasnum
				{
					...
					String arg1 = tt[t + 0];
					String item = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 1]);
					String cmp = tt[t + 2];
					String value = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 3]);
					Environmental E = getArgumentItem(arg1, source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
					...
					if (E == null)
						...
						returnable = simpleEval(scripted, "" + num, value, cmp,
								"HASNUM");
					} else if (E instanceof Item) {
						num = CMLib.english().containsString(E.name(), item) ? 1
								: 0;
						returnable = simpleEval(scripted, "" + num, value, cmp,
								"HASNUM");
					} else if (E instanceof Room) {
				...
				case 67: // hastitle
				{
					...
					String arg2 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 1]);
					Environmental E = getArgumentMOB(arg1, source, monster,
							target, primaryItem, secondaryItem, msg, tmp);
					if (arg2.length() == 0) {
					...
					if (E instanceof MOB) {
						MOB M = (MOB) E;
						returnable = (M.playerStats() != null)
								&& (M.playerStats().getTitles().contains(arg2));
					} else
						returnable = false;
					break;
				}
				case 3: // worn
				{
					...
					if (E == null)
						...
						returnable = (((MOB) E).fetchItem(null,
								Wearable.FILTER_WORNONLY, arg2) != null);
					else if (E instanceof Item)
						returnable = (CMLib.english().containsString(E.name(),
								arg2) && (!((Item) E)
				...
				case 87: // isbirthday
				{
					...
					if ((E == null) || (!(E instanceof MOB)))
						...
						if (mob.playerStats() == null)
							...
							int day = CMLib.time().globalClock()
									.getDayOfMonth();
							int bday = mob.playerStats().getBirthday()[0];
							int bmonth = mob.playerStats().getBirthday()[1];
							if ((CMLib.time().globalClock().getYear() > mob
									.playerStats().getBirthday()[3])
				...
				{
					String arg1 = CMParms.cleanBit(funcParms);
					Physical P = getArgumentItem(arg1, source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
					if (P == null)
						returnable = false;
				...
				case 58: // isable
				{
					...
					if ((E != null) && ((E instanceof MOB))
							&& (!((MOB) E).amDead())) {
						ExpertiseLibrary X = (ExpertiseLibrary) CMLib
								.expertises().findDefinition(arg2, true);
						if (X != null)
				...
				case 59: // isopen
				{
					...
					returnable = false;
					if (dir < 0) {
						Environmental E = getArgumentItem(arg1, source,
								monster, scripted, target, primaryItem,
								secondaryItem, msg, tmp);
				...
				case 11: // isimmort
				{
					...
					if ((E == null) || (!(E instanceof MOB)))
						...
						returnable = CMSecurity.isAllowed(((MOB) E),
								lastKnownLocation, CMSecurity.SecFlag.IMMORT);
					break;
				}
				case 12: // ischarmed
				{
					String arg1 = CMParms.cleanBit(funcParms);
					Physical E = getArgumentMOB(arg1, source, monster, target,
							primaryItem, secondaryItem, msg, tmp);
					if ((E == null) || (!(E instanceof MOB)))
						returnable = false;
					else
						returnable = CMLib.flags()
								.flaggedAffects(E, Ability.FLAG_CHARMING)
								.size() > 0;
					break;
				}
				case 15: // isfollow
				...
				case 73: // isservant
				{
					...
					if ((E == null) || (!(E instanceof MOB))
							|| (lastKnownLocation == null))
						returnable = false;
					else if ((((MOB) E).getLiegeID() == null)
							|| (((MOB) E).getLiegeID().length() == 0))
						returnable = false;
					else if (lastKnownLocation.fetchInhabitant("$"
							+ ((MOB) E).getLiegeID() + "$") == null)
						returnable = false;
					else
						returnable = true;
					break;
				...
				case 95: // isspeaking
				{
					...
					if ((E == null) || (!(E instanceof MOB))
						...
						if ((L != null)
								...
								&& (L.ID().equalsIgnoreCase(arg2)
										|| L.Name().equalsIgnoreCase(arg2) || arg2
											.equalsIgnoreCase("any")))
							returnable = true;
						else if (arg2.equalsIgnoreCase("common")
								|| arg2.equalsIgnoreCase("none"))
							returnable = true;
						else
							returnable = false;
					}
				...
				case 55: // ispkill
				{
					...
					if ((E == null) || (!(E instanceof MOB)))
						returnable = false;
					else if (CMath.bset(((MOB) E).getBitmap(),
							MOB.ATT_PLAYERKILL))
						returnable = true;
					else
						returnable = false;
				...
				case 56: // name
				{
					...
					String arg1 = tt[t + 0];
					String arg2 = tt[t + 1];
					String arg3 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 2]);
					Environmental E = getArgumentItem(arg1, source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
					if (E == null)
						...
						returnable = simpleEvalStr(scripted, E.Name(), arg3,
								arg2, "NAME");
					break;
				}
				case 75: // currency
				{
					...
					if (E == null)
						...
						returnable = simpleEvalStr(scripted, CMLib
								.beanCounter().getCurrency(E), arg3, arg2,
								"CURRENCY");
					break;
				}
				case 61: // strin
				{
					if (tlen == 1)
						tt = parseBits(eval, t, "cr"); /* tt[t+0] */
					String arg1 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 0]);
					String arg2 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 1]);
				...
				case 62: // callfunc
				{
					...
					for (int v = 0; v < scripts.size(); v++) {
						...
						if (script2.size() < 1)
							continue;
						trigger = ((String) script2.elementAt(0, 1))
								.toUpperCase().trim();
						ttrigger = (String[]) script2.elementAt(0, 2);
						if (getTriggerCode(trigger, ttrigger) == 17) {
							String fnamed = (ttrigger != null) ? ttrigger[1]
							...
							if (fnamed.equalsIgnoreCase(arg1)) {
								...
								found = execute(
										...
										secondaryItem,
										script2,
										varify(source, target, scripted,
												monster, primaryItem,
												secondaryItem, msg, tmp, arg2),
										tmp);
								if (found == null)
									found = "";
								break;
					...
					if (!validFunc)
						logError(scripted, "CALLFUNC", "Unknown", "Function: "
								+ arg1);
					else if (found != null)
				...
				case 69: // isbehave
				{
					...
					String arg2 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 1]);
					PhysicalAgent P = getArgumentItem(arg1, source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
					if (P == null)
						returnable = false;
				...
				case 70: // ipaddress
				{
					...
					if ((E == null) || (!(E instanceof MOB))
						...
						returnable = simpleEvalStr(scripted, ((MOB) E)
								.session().getAddress(), arg3, arg2, "ADDRESS");
					break;
				}
				case 28: // questwinner
				{
					...
					String arg1 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 0]);
					String arg2 = tt[t + 1];
					Environmental E = getArgumentMOB(tt[t + 0], source,
							monster, target, primaryItem, secondaryItem, msg,
							tmp);
				...
				case 93: // questscripted
				{
					if (tlen == 1)
						tt = parseBits(eval, t, "cr"); /* tt[t+0] */
					PhysicalAgent E = getArgumentMOB(tt[t + 0], source,
							monster, target, primaryItem, secondaryItem, msg,
							tmp);
					String arg2 = tt[t + 1];
					Quest Q = getQuest(arg2);
				...
				case 32: // nummobsinarea
				{
					...
					if ((arg3.toUpperCase().startsWith("MASK") && (arg3
							.substring(4).trim().startsWith("=")))) {
						arg3 = arg3.substring(4).trim();
						arg3 = arg3.substring(1).trim();
						MASK = CMLib.masking().maskCompile(arg3);
					}
					for (Enumeration<Room> e = lastKnownLocation.getArea()
							.getProperMap(); e.hasMoreElements();) {
						...
						for (int m = 0; m < R.numInhabitants(); m++) {
							...
							if (MASK != null) {
								if (CMLib.masking().maskCheck(MASK, M, true))
									num++;
							} else if (CMLib.english().containsString(M.name(),
									arg1))
								num++;
						}
					}
					returnable = simpleEval(scripted, "" + num, arg3, arg2,
							"NUMMOBSINAREA");
					break;
				}
				case 33: // nummobs
				{
					...
					try {
						for (Enumeration e = CMLib.map().rooms(); e
								.hasMoreElements();) {
							...
							for (int m = 0; m < R.numInhabitants(); m++) {
								...
								if (MASK != null) {
									if (CMLib.masking()
											.maskCheck(MASK, M, true))
										num++;
								} else if (CMLib.english().containsString(
										M.name(), arg1))
									num++;
							}
						}
				...
				case 34: // numracesinarea
				{
					...
					Room R = null;
					MOB M = null;
					for (Enumeration e = lastKnownLocation.getArea()
							.getProperMap(); e.hasMoreElements();) {
						R = (Room) e.nextElement();
						for (int m = 0; m < R.numInhabitants(); m++) {
							M = R.fetchInhabitant(m);
							if ((M != null)
									&& (M.charStats().raceName()
				...
				case 92: // isodd
				{
					String val = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp,
							CMParms.cleanBit(funcParms)).trim();
					boolean isodd = false;
					if (CMath.isLong(val)) {
				...
				case 16: // hitprcnt
				{
					...
					if ((E == null) || (!(E instanceof MOB)))
						...
						double hitPctD = CMath.div(((MOB) E).curState()
								.getHitPoints(), ((MOB) E).maxState()
								.getHitPoints());
						int val1 = (int) Math.round(hitPctD * 100.0);
						returnable = simpleEval(scripted, "" + val1, arg3,
								arg2, "HITPRCNT");
					}
					break;
				}
				case 50: // isseason
				{
					String arg1 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp,
							CMParms.cleanBit(funcParms));
					returnable = false;
					if (monster.location() != null)
						for (int a = 0; a < TimeClock.SEASON_DESCS.length; a++)
							if ((TimeClock.SEASON_DESCS[a]).startsWith(arg1
									.toUpperCase())
									&& (monster.location().getArea()
											.getTimeObj().getSeasonCode() == a)) {
								returnable = true;
								break;
							}
					break;
				...
				case 51: // isweather
				{
					...
					if (monster.location() != null)
						for (int a = 0; a < Climate.WEATHER_DESCS.length; a++)
							if ((Climate.WEATHER_DESCS[a]).startsWith(arg1
									.toUpperCase())
									&& (monster.location().getArea()
											.getClimateObj()
											.weatherType(monster.location()) == a)) {
								returnable = true;
								break;
							}
					break;
				...
				case 57: // ismoon
				{
					...
					if (monster.location() != null) {
						if (arg1.length() == 0)
							returnable = monster.location().getArea()
									.getClimateObj()
									.canSeeTheStars(monster.location());
						else
							for (int a = 0; a < TimeClock.PHASE_DESC.length; a++)
								if ((TimeClock.PHASE_DESC[a]).startsWith(arg1
										.toUpperCase())
										&& (monster.location().getArea()
												.getTimeObj().getMoonPhase() == a)) {
									returnable = true;
									break;
								}
					}
				...
				case 38: // istime
				{
					String arg1 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp,
							CMParms.cleanBit(funcParms)).toLowerCase().trim();
					if (monster.location() == null)
						returnable = false;
					else if (("daytime").startsWith(arg1)
							&& (monster.location().getArea().getTimeObj()
									.getTODCode() == TimeClock.TIME_DAY))
						returnable = true;
					else if (("dawn").startsWith(arg1)
							&& (monster.location().getArea().getTimeObj()
									.getTODCode() == TimeClock.TIME_DAWN))
						returnable = true;
					else if (("dusk").startsWith(arg1)
							&& (monster.location().getArea().getTimeObj()
									.getTODCode() == TimeClock.TIME_DUSK))
						returnable = true;
					else if (("nighttime").startsWith(arg1)
							&& (monster.location().getArea().getTimeObj()
									.getTODCode() == TimeClock.TIME_NIGHT))
						returnable = true;
					else if ((monster.location().getArea().getTimeObj()
							.getTimeOfDay() == CMath.s_int(arg1)))
						returnable = true;
					else
						returnable = false;
					break;
				...
				case 45: // nummobsroom
				{
					...
					if (lastKnownLocation != null) {
						...
						if (signH.containsKey(tt[t + 1])) {
							String name = varify(source, target, scripted,
									monster, primaryItem, secondaryItem, msg,
									tmp, tt[t + 0]);
							startbit++;
							if (!name.equalsIgnoreCase("*")) {
								num = 0;
								MaskingLibrary.CompiledZapperMask MASK = null;
								if ((name.toUpperCase().startsWith("MASK") && (name
										.substring(4).trim().startsWith("=")))) {
									boolean usePreCompiled = (name
											.equals(tt[t + 0]));
									name = name.substring(4).trim();
									name = name.substring(1).trim();
									MASK = usePreCompiled ? CMLib.masking()
											.getPreCompiledMask(name) : CMLib
											.masking().maskCompile(name);
								}
								for (int i = 0; i < lastKnownLocation
										.numInhabitants(); i++) {
									MOB M = lastKnownLocation
											.fetchInhabitant(i);
									if (M == null)
										continue;
									if (MASK != null) {
						...
						logError(scripted, "NUMMOBSROOM", "Syntax",
								"No SIGN found: " + funcParms);
						return returnable;
					}

					String comp = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t
									+ startbit]);
					String arg2 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t
									+ startbit + 1]);
					if (lastKnownLocation != null)
						returnable = simpleEval(scripted, "" + num, arg2, comp,
								"NUMMOBSROOM");
					break;
				}
				case 63: // numpcsroom
				{
					...
					String arg2 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 1]);
					if (lastKnownLocation != null)
						returnable = simpleEval(scripted, ""
								+ lastKnownLocation.numPCInhabitants(), arg2,
								arg1, "NUMPCSROOM");
					break;
				}
				case 79: // numpcsarea
					...
					if (lastKnownLocation != null) {
						...
						for (Session S : CMLib.sessions().localOnlineIterable())
							if ((S.mob().location() != null)
									&& (S.mob().location().getArea() == lastKnownLocation
											.getArea()))
								num++;
						returnable = simpleEval(scripted, "" + num, arg2, arg1,
								"NUMPCSAREA");
					}
					break;
				}
				case 77: // explored
				{
					if (tlen == 1)
						tt = parseBits(eval, t, "cccr"); /* tt[t+0] */
					String whom = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 0]);
					String where = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 1]);
					String cmp = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 2]);
					String arg2 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 3]);
					Environmental E = getArgumentMOB(whom, source, monster,
							target, primaryItem, secondaryItem, msg, tmp);
					if ((E == null) || (!(E instanceof MOB))) {
					...
					if (!where.equalsIgnoreCase("world")) {
						...
						if (A == null) {
							Environmental E2 = getArgumentItem(where, source,
									monster, scripted, target, primaryItem,
									secondaryItem, msg, tmp);
							if (E2 != null)
						...
						if (A == null) {
							logError(scripted, "EXPLORED", "Unknown Area",
									where);
							return returnable;
						}
					}
					if (lastKnownLocation != null) {
						...
						returnable = simpleEval(scripted, "" + pct, arg2, cmp,
								"EXPLORED");
					}
					break;
				}
				case 72: // faction
				{
					...
					if (!M.hasFaction(F.factionID()))
						...
						if (CMath.isNumber(arg2.trim()))
							...
							Faction.FRange FR = CMLib.factions().getRange(
									F.factionID(), myfac);
							if (FR == null)
								value = "";
							else
					...
					if (lastKnownLocation != null)
						returnable = simpleEval(scripted, value, arg2, cmp,
								"FACTION");
					break;
				}
				case 46: // numitemsroom
				{
					...
					returnable = simpleEval(scripted, "" + ct, arg2, arg1,
							"NUMITEMSROOM");
					break;
				}
				case 47: // mobitem
				{
					...
					if (which == null)
						...
						returnable = (CMLib.english().containsString(
								which.name(), arg3)
								|| CMLib.english().containsString(which.Name(),
										arg3) || CMLib.english()
								.containsString(which.displayText(), arg3));
					break;
				}
				case 49: // hastattoo
				{
				...
				case 48: // numitemsmob
				{
					...
					MOB which = null;
					if (lastKnownLocation != null)
						which = lastKnownLocation.fetchInhabitant(CMath
								.s_int(arg1.trim()) - 1);
					int ct = 1;
					if (which != null)
					...
					returnable = simpleEval(scripted, "" + ct, arg3, arg2,
							"NUMITEMSMOB");
					break;
				}
				case 43: // roommob
				{
					...
					if (which == null)
						...
						returnable = (CMLib.english().containsString(
								...
								|| CMLib.english().containsString(which.Name(),
										arg2) || CMLib.english()
								.containsString(which.displayText(), arg2));
					break;
				}
				case 44: // roomitem
				{
				...
				case 36: // ishere
				{
					String arg1 = varify(source, target, scripted, monster,
							...
							CMParms.cleanBit(funcParms));
					if (lastKnownLocation != null)
						returnable = ((lastKnownLocation.findItem(arg1) != null) || (lastKnownLocation
								.fetchInhabitant(arg1) != null));
					else
						returnable = false;
					break;
				}
				case 17: // inroom
				{
					...
					String arg2;
					if (signH.containsKey(tt[t + 1])) {
						E = getArgumentItem(tt[t + 0], source, monster,
								scripted, target, primaryItem, secondaryItem,
								msg, tmp);
						comp = tt[t + 1];
						arg2 = varify(source, target, scripted, monster,
								primaryItem, secondaryItem, msg, tmp, tt[t + 2]);
					} else
						arg2 = varify(source, target, scripted, monster,
								primaryItem, secondaryItem, msg, tmp, tt[t + 0]);
					Room R = null;
					if (arg2.startsWith("$"))
						R = CMLib.map().roomLocation(
								this.getArgumentItem(arg2, source, monster,
										scripted, target, primaryItem,
										secondaryItem, msg, tmp));
					if (R == null)
					...
					if (E == null)
						...
						if ((R == null)
								&& ((arg2.length() == 0) || (R2 == null)))
							returnable = true;
						else if ((R == null) || (R2 == null))
							returnable = false;
						else
							returnable = simpleEvalStr(scripted, CMLib.map()
									.getExtendedRoomID(R2), CMLib.map()
									.getExtendedRoomID(R), comp, "INROOM");
					}
					break;
				}
				case 90: // inarea
				{
					...
					if (R == null)
						try {
							if ((lastKnownLocation != null)
									&& (lastKnownLocation.getArea().Name()
											.equalsIgnoreCase(arg2)))
								R = lastKnownLocation;
							if (R == null)
								for (Enumeration a = CMLib.map().areas(); a
										.hasMoreElements();) {
									Area A = (Area) a.nextElement();
									if ((A != null)
											&& (A.Name().equalsIgnoreCase(arg2))) {
										if ((lastKnownLocation != null)
												&& (lastKnownLocation.getArea()
														.Name()
														.equals(A.Name())))
											R = lastKnownLocation;
					...
					if (E == null)
						...
						if ((R == null)
							...
							returnable = simpleEvalStr(scripted, R2.getArea()
									.Name(), R.getArea().Name(), comp, "INAREA");
					}
					break;
				}
				case 89: // isrecall
				{
					...
					if (E == null)
						...
						if ((R == null)
							...
							returnable = simpleEvalStr(scripted, CMLib.map()
									.getExtendedRoomID(R2), CMLib.map()
									.getExtendedRoomID(R), comp, "ISRECALL");
					}
					break;
				}
				case 37: // inlocale
				{
					if (tlen == 1) {
						if (CMParms.numBits(funcParms) > 1)
							...
							String[] parsed = null;
							if (CMParms.cleanBit(funcParms).equals(funcParms))
								parsed = parseBits("'" + funcParms + "'"
										+ CMStrings.repeat(" .", numBits - 1),
										"cr");
							else
								parsed = parseBits(
										funcParms
					...
					if (E == null)
						...
						if (R == null)
							returnable = false;
						else if (CMClass.classID(R).toUpperCase()
								.indexOf(arg2.toUpperCase()) >= 0)
							returnable = true;
						else
							returnable = false;
					}
				...
				case 18: // sex
				{
					...
					if ((E == null) || (!(E instanceof MOB)))
						...
						String sex = ("" + ((char) ((MOB) E).charStats()
								.getStat(CharStats.STAT_GENDER))).toUpperCase();
						if (arg2.equals("=="))
				...
				case 91: // datetime
				{
					...
					String arg2 = tt[t + 1];
					String arg3 = tt[t + 2];
					int index = CMParms.indexOf(ScriptingEngine.DATETIME_ARGS,
							arg1.trim());
					if (index < 0)
						logError(
								scripted,
						...
						case 2:
							val = ""
									+ CMLib.map().areaLocation(scripted)
											.getTimeObj().getDayOfMonth();
							break;
						case 3:
							val = ""
									+ CMLib.map().areaLocation(scripted)
						...
						default:
							val = ""
									+ CMLib.map().areaLocation(scripted)
											.getTimeObj().getTimeOfDay();
							break;
						}
						returnable = simpleEval(scripted, val, arg3, arg2,
								"DATETIME");
					}
					break;
				}
				case 13: // stat
				{
					...
					String arg2 = tt[t + 1];
					String arg3 = tt[t + 2];
					String arg4 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 3]);
					Environmental E = getArgumentItem(arg1, source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
					...
					if (E == null)
						...
						if (arg3.equals("=="))
							...
							returnable = simpleEval(scripted, val, arg4, arg3,
									"STAT");
					}
					break;
				}
				...
				case 83: // qvar
				{
					...
					if (Q == null)
						...
						returnable = simpleEvalStr(scripted, Q.getStat(arg2),
								arg4, arg3, "QVAR");
					break;
				}
				case 84: // math
				{
					...
					returnable = simpleExpressionEval(scripted, arg1, arg3,
							arg2, "MATH");
					break;
				}
				case 81: // trains
				...
				case 66: // clanrank
				{
					...
					if (!(E instanceof MOB))
						...
						if (C == null)
							C = ((MOB) E).clans().iterator().hasNext() ? ((MOB) E)
									.clans().iterator().next().first
									: null;
						if (C != null)
							val1 = ((MOB) E).getClanRole(C.clanID()).second
									.intValue();
						returnable = simpleEval(scripted, "" + val1, arg3,
								arg2, "CLANRANK");
					}
				...
				case 68: // clandata
				{
					...
					if ((E != null) && (E instanceof MOB)) {
						...
						clanID = varify(source, target, scripted, monster,
								primaryItem, secondaryItem, msg, tmp, arg1);
						if ((scripted instanceof MOB)
								&& (CMLib.clans().getClan(clanID) == null)) {
							List<Pair<Clan, Integer>> Cs = CMLib.clans()
									.getClansByCategory((MOB) scripted, clanID);
					...
					if (C != null) {
						if (!C.isStat(arg2))
							logError(scripted, "CLANDATA", "RunTime", arg2
									+ " is not a valid clan variable.");
						else {
							String whichVal = C.getStat(arg2).trim();
							if (CMath.isNumber(whichVal)
									&& CMath.isNumber(arg4.trim()))
								returnable = simpleEval(scripted, whichVal,
										arg4, arg3, "CLANDATA");
							else
								returnable = simpleEvalStr(scripted, whichVal,
										arg4, arg3, "CLANDATA");
						}
					}
					break;
				...
				case 98: // clanqualifies
				{
					...
					if ((C != null) && (E instanceof MOB)) {
						...
						if (C.isOnlyFamilyApplicants()
								&& (!CMLib.clans().isFamilyOfMembership(mob,
										C.getMemberList())))
							returnable = false;
						else if (CMLib.clans()
								.getClansByCategory(mob, C.getCategory())
								.size() > CMProps.getMaxClansThisCategory(C
								.getCategory()))
							returnable = false;
						if (returnable
								&& (!CMLib.masking().maskCheck(
										C.getBasicRequirementMask(), mob, true)))
							returnable = false;
						else if (returnable
								&& (CMLib.masking().maskCheck(
										C.getAcceptanceSettings(), mob, true)))
							returnable = false;
					} else {
						logError(scripted, "CLANQUALIFIES", "Unknown clan "
								+ arg2 + " or " + arg1 + " is not a mob",
								funcParms);
						return returnable;
					}
					break;
				...
				case 88: // mood
				{
					...
					String arg3 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 2]);
					Physical P = getArgumentMOB(arg1, source, monster, target,
							primaryItem, secondaryItem, msg, tmp);
					if (arg2.length() == 0) {
					...
					if ((P == null) || (!(P instanceof MOB)))
						returnable = false;
					else if (P.fetchEffect("Mood") != null) {
				...
				case 21: // class
				{
					...
					if ((E == null) || (!(E instanceof MOB)))
						...
						String sex = ((MOB) E).charStats().displayClassName()
								.toUpperCase();
						if (arg2.equals("=="))
							returnable = sex.startsWith(arg3);
						else if (arg2.equals("!="))
				...
				case 22: // baseclass
				{
					...
					if ((E == null) || (!(E instanceof MOB)))
						...
						String sex = ((MOB) E).charStats().getCurrentClass()
				...
				case 23: // race
				{
					...
					if ((E == null) || (!(E instanceof MOB)))
						...
						String sex = ((MOB) E).charStats().raceName()
								.toUpperCase();
						if (arg2.equals("=="))
							returnable = sex.startsWith(arg3);
						else if (arg2.equals("!="))
				...
				case 24: // racecat
				{
					...
					if ((E == null) || (!(E instanceof MOB)))
						...
						String sex = ((MOB) E).charStats().getMyRace()
								.racialCategory().toUpperCase();
						if (arg2.equals("=="))
							returnable = sex.startsWith(arg3);
						else if (arg2.equals("!="))
							returnable = !sex.startsWith(arg3);
				...
				case 25: // goldamt
				{
					...
					if (E == null)
						...
						int val1 = 0;
						if (E instanceof MOB)
							val1 = (int) Math.round(CMLib.beanCounter()
									.getTotalAbsoluteValue(
											(MOB) E,
											CMLib.beanCounter().getCurrency(
													scripted)));
						else if (E instanceof Coins)
							val1 = (int) Math
									.round(((Coins) E).getTotalValue());
						else if (E instanceof Item)
							val1 = ((Item) E).value();
						else {
							logError(scripted, "GOLDAMT", "Syntax", funcParms);
				...
				case 76: // value
				{
					...
					if (!CMLib.beanCounter().getAllCurrencies()
							.contains(arg2.toUpperCase())) {
						logError(scripted, "VALUE", "Syntax", arg2
								+ " is not a valid designated currency.");
						break;
					}
					if (E == null)
						returnable = false;
						...
						if (E instanceof MOB)
							val1 = (int) Math.round(CMLib.beanCounter()
									.getTotalAbsoluteValue((MOB) E,
											arg2.toUpperCase()));
						else if (E instanceof Coins) {
							if (((Coins) E).getCurrency()
									.equalsIgnoreCase(arg2))
								val1 = (int) Math.round(((Coins) E)
										.getTotalValue());
						} else if (E instanceof Item)
							val1 = ((Item) E).value();
						else {
						...
						returnable = simpleEval(scripted, "" + val1, arg4,
								arg3, "GOLDAMT");
					}
					break;
				}
				...
				case 27: // var
				{
					...
					String val = getVar(E, arg1, arg2, source, target,
							scripted, monster, primaryItem, secondaryItem, msg,
							tmp);
					if (arg3.equals("=="))
						...
						returnable = CMath.s_int(val.trim()) > CMath.s_int(arg4
								.trim());
					else if (arg3.equals("<"))
						returnable = CMath.s_int(val.trim()) < CMath.s_int(arg4
								.trim());
					else if (arg3.equals(">="))
						returnable = CMath.s_int(val.trim()) >= CMath
								.s_int(arg4.trim());
					else if (arg3.equals("<="))
						returnable = CMath.s_int(val.trim()) <= CMath
								.s_int(arg4.trim());
				...
				case 42: // randnum
				{
					if (tlen == 1)
						tt = parseBits(eval, t, "ccr"); /* tt[t+0] */
					String arg1s = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 0])
							.toUpperCase().trim();
					int arg1 = 0;
					...
					String arg2 = tt[t + 1];
					String arg3s = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 2])
							.trim();
					int arg3 = 0;
					...
					returnable = simpleEval(scripted, "" + arg1, "" + arg3,
							arg2, "RANDNUM");
					break;
				}
				case 71: // rand0num
				...
				case 96: // iscontents
				{
					...
					if (E == null)
						...
						if (arg2.length() == 0)
							...
							returnable = CMLib.english().fetchEnvironmental(
									new XVector(((Rideable) E).riders()), arg2,
									false) != null;
					}
					if (E instanceof Container) {
						if (arg2.length() == 0)
							...
							returnable = CMLib.english().fetchEnvironmental(
									((Container) E).getContents(), arg2, false) != null;
					} else
						returnable = false;
					break;
				}
				case 97: // wornon
				{
					if (tlen == 1)
						tt = parseBits(eval, t, "ccr"); /* tt[t+0] */
					String arg1 = tt[t + 0];
					PhysicalAgent E = getArgumentMOB(arg1, source, monster,
							target, primaryItem, secondaryItem, msg, tmp);
					String arg2 = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[t + 1]);
					String arg3 = varify(source, target, scripted, monster,
					...
					int wornLoc = CMParms.indexOf(Wearable.CODES.NAMESUP(),
							arg2.toUpperCase().trim());
					returnable = false;
					if (wornLoc < 0)
						logError(scripted, "EVAL", "BAD WORNON LOCATION", arg2);
					else if (E instanceof MOB) {
						final List<Item> items = ((MOB) E).fetchWornItems(
								Wearable.CODES.GET(wornLoc), (short) -2048,
								(short) 0);
						if ((items.size() == 0) && (arg3.length() == 0))
							returnable = true;
						else
							returnable = CMLib.english().fetchEnvironmental(
									items, arg3, false) != null;
					}
					break;
				}
				default:
					logError(scripted, "EVAL", "UNKNOWN",
							CMParms.toStringList(tt));
					return false;
				}
				...
				logError(scripted, "EVAL", "SYNTAX", "BAD CONJUCTOR " + tt[t]
						+ ": " + CMParms.toStringList(tt));
				return false;
			}
		if ((stack.size() != 1) || (!(stack.firstElement() instanceof Boolean))) {
			logError(scripted, "EVAL", "SYNTAX",
					"Unmatched (: " + CMParms.toStringList(tt));
			return false;
		}
		return ((Boolean) stack.firstElement()).booleanValue();
	}

	public String functify(PhysicalAgent scripted, MOB source,
			Environmental target, MOB monster, Item primaryItem,
		...
		String preFab = (y >= 0) ? evaluable.substring(0, y).toUpperCase()
				.trim() : "";
		...
		if ((y < 0) || (z < y)) {
			...
			case 2: // has
			{
				String arg1 = varify(source, target, scripted, monster,
						...
						CMParms.cleanBit(funcParms));
				Environmental E = getArgumentItem(arg1, source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				...
				if (choices.size() > 0)
					results.append(((Item) choices.elementAt(CMLib.dice().roll(
							1, choices.size(), -1))).name());
				break;
			}
			case 74: // hasnum
			{
				...
				String item = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp,
						CMParms.getCleanBit(funcParms, 1));
				Environmental E = getArgumentItem(arg1, source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
			...
			case 3: // worn
			{
				...
				if (E == null)
					...
					for (int i = 0; i < ((MOB) E).numItems(); i++) {
						...
						&& (!(((Item) E).amWearingAt(Wearable.IN_INVENTORY)))) {
					if (E instanceof Container)
						choices.addAll(((Container) E).getContents());
					else
						choices.addElement(E);
			...
			{
				String arg1 = CMParms.cleanBit(funcParms);
				Environmental E = getArgumentMOB(arg1, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if ((E != null) && (E instanceof MOB)
					...
					if (C.getDaysInWeek() > 0) {
						long x = ((long) year) * ((long) C.getMonthsInYear())
								* C.getDaysInMonth();
						x = x + ((long) (bmonth - 1))
								* ((long) C.getDaysInMonth());
						x = x + bmonth;
						timeDesc.append(C.getWeekNames()[(int) (x % C
								.getDaysInWeek())] + ", ");
					}
					...
					if (C.getYearNames().length > 0)
						timeDesc.append(", "
								+ CMStrings.replaceAll(C.getYearNames()[year
										% C.getYearNames().length], "#", ""
										+ year));
			...
			case 6: // isgood
			{
				...
				if ((E != null) && ((E instanceof MOB))) {
					Faction.FRange FR = CMLib.factions().getRange(
							...
							((MOB) E).fetchFaction(CMLib.factions().AlignID()));
					if (FR != null)
						results.append(FR.name());
					else
						results.append(((MOB) E).fetchFaction(CMLib.factions()
								.AlignID()));
				}
				break;
			}
			case 8: // isevil
			{
				...
				Environmental E = getArgumentMOB(arg1, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if ((E != null) && ((E instanceof MOB)))
					results.append(CMLib.flags().getAlignmentName(E)
							.toLowerCase());
				break;
			}
			case 9: // isneutral
			...
			case 58: // isable
			{
				...
				if ((E != null) && ((E instanceof MOB))
						&& (!((MOB) E).amDead())) {
					ExpertiseLibrary X = (ExpertiseLibrary) CMLib.expertises()
							.findDefinition(arg2, true);
					if (X != null) {
						Entry<String, Integer> s = ((MOB) E).fetchExpertise(X
								.ID());
						if (s != null)
							results.append(s.getKey()
			...
			case 62: // callfunc
			{
				String arg1 = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp,
						CMParms.getCleanBit(funcParms, 0));
				String arg2 = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp,
						CMParms.getPastBitClean(funcParms, 0));
				String found = null;
				boolean validFunc = false;
				List<DVector> scripts = getScripts();
				String trigger = null;
				...
				for (int v = 0; v < scripts.size(); v++) {
					...
					if (script2.size() < 1)
						continue;
					trigger = ((String) script2.elementAt(0, 1)).toUpperCase()
							.trim();
					ttrigger = (String[]) script2.elementAt(0, 2);
					if (getTriggerCode(trigger, ttrigger) == 17) {
						...
						if (fnamed.equalsIgnoreCase(arg1)) {
							...
							found = execute(
									...
									secondaryItem,
									script2,
									varify(source, target, scripted, monster,
											primaryItem, secondaryItem, msg,
											tmp, arg2), tmp);
							if (found == null)
								found = "";
							break;
			...
			case 55: // ispkill
			{
				...
				else if (CMath.bset(((MOB) E).getBitmap(), MOB.ATT_PLAYERKILL))
					results.append("true");
				else
					results.append("false");
				break;
			...
			case 12: // ischarmed
			{
				...
				Physical E = getArgumentMOB(arg1, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if (E != null) {
					List<Ability> V = CMLib.flags().flaggedAffects(E,
							Ability.FLAG_CHARMING);
					for (int v = 0; v < V.size(); v++)
			...
			case 14: // affected
			{
				...
				if ((E instanceof Physical)
						&& (((Physical) E).numEffects() > 0))
					results.append(((Physical) E).effects().nextElement()
							.name());
				break;
			}
			case 69: // isbehave
			{
				String arg1 = CMParms.cleanBit(funcParms);
				PhysicalAgent E = getArgumentItem(arg1, source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				if (E != null)
			...
			{
				String arg1 = CMParms.cleanBit(funcParms);
				PhysicalAgent E = getArgumentMOB(arg1, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if ((E != null) && (E instanceof MOB)
						&& (!((MOB) E).isMonster())) {
					for (Enumeration<ScriptingEngine> e = E.scripts(); e
							.hasMoreElements();) {
						ScriptingEngine SE = e.nextElement();
						if ((SE != null) && (SE.defaultQuestName() != null)
								&& (SE.defaultQuestName().length() > 0)) {
							Quest Q = CMLib.quests().fetchQuest(
									SE.defaultQuestName());
							if (Q != null)
								results.append(Q.name() + " ");
							else
			...
			case 30: // questobj
			{
				...
				questName = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, questName);
				Quest Q = getQuest(questName);
				if (Q == null) {
					logError(scripted, "QUESTOBJ", "Unknown", "Quest: "
							+ questName);
					break;
				}
			...
			case 32: // nummobsinarea
			{
				...
				int num = 0;
				MaskingLibrary.CompiledZapperMask MASK = null;
				if ((arg1.toUpperCase().startsWith("MASK") && (arg1
						.substring(4).trim().startsWith("=")))) {
			...
			case 34: // numracesinarea
			{
				...
				Room R = null;
				MOB M = null;
				for (Enumeration e = lastKnownLocation.getArea().getProperMap(); e
						.hasMoreElements();) {
					R = (Room) e.nextElement();
					for (int m = 0; m < R.numInhabitants(); m++) {
						M = R.fetchInhabitant(m);
			...
			{
				if (monster.location() != null)
					results.append(TimeClock.SEASON_DESCS[monster.location()
							.getArea().getTimeObj().getSeasonCode()]);
				break;
			}
			case 51: // isweather
			{
				if (monster.location() != null)
					results.append(Climate.WEATHER_DESCS[monster.location()
							.getArea().getClimateObj()
							.weatherType(monster.location())]);
				break;
			}
			case 57: // ismoon
			{
				if (monster.location() != null)
					results.append(TimeClock.PHASE_DESC[monster.location()
							.getArea().getTimeObj().getMoonPhase()]);
				break;
			}
			case 38: // istime
			{
				if (lastKnownLocation != null)
					results.append(TimeClock.TOD_DESC[lastKnownLocation
							.getArea().getTimeObj().getTODCode()].toLowerCase());
				break;
			}
			case 39: // isday
			{
				if (lastKnownLocation != null)
					results.append(""
							+ lastKnownLocation.getArea().getTimeObj()
									.getDayOfMonth());
				break;
			}
			case 43: // roommob
			{
				...
				if (lastKnownLocation != null) {
					...
					if (which != null) {
						...
						results.append(CMLib.english().getContextName(list,
								which));
					}
				}
				break;
			...
			case 45: // nummobsroom
			{
				...
				if (lastKnownLocation != null) {
					num = lastKnownLocation.numInhabitants();
					String name = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp,
							CMParms.cleanBit(funcParms));
			...
			case 77: // explored
			{
				...
				String where = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp,
						CMParms.getCleanBit(funcParms, 1));
				Environmental E = getArgumentMOB(whom, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if (E instanceof MOB) {
					Area A = null;
					if (!where.equalsIgnoreCase("world")) {
			...
			case 17: // inroom
			{
				...
				if ((E == null) || arg1.length() == 0)
					results.append(CMLib.map().getExtendedRoomID(
							lastKnownLocation));
				else
					results.append(CMLib.map().getExtendedRoomID(
							CMLib.map().roomLocation(E)));
				break;
			}
			case 90: // inarea
			{
				...
				Environmental E = getArgumentItem(arg1, source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				if ((E == null) || arg1.length() == 0)
					results.append(lastKnownLocation == null ? "Nowhere"
							: lastKnownLocation.getArea().Name());
				else {
					Room R = CMLib.map().roomLocation(E);
			...
			case 89: // isrecall
			{
				...
				if (E != null)
					results.append(CMLib.map().getExtendedRoomID(
							CMLib.map().getStartRoom(E)));
				break;
			}
			case 37: // inlocale
			{
				String parms = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp,
						CMParms.cleanBit(funcParms));
				if (parms.trim().length() == 0) {
					...
					Environmental E = getArgumentItem(parms, source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
					if (E != null) {
						Room R = CMLib.map().roomLocation(E);
			...
			{
				String arg1 = CMParms.getCleanBit(funcParms, 0);
				int index = CMParms.indexOf(ScriptingEngine.DATETIME_ARGS, arg1
						.toUpperCase().trim());
				if (index < 0)
					logError(scripted, "DATETIME", "Syntax", "Unknown arg: "
							+ arg1 + " for " + scripted.name());
				else if (CMLib.map().areaLocation(scripted) != null)
					switch (index) {
					case 2:
						results.append(CMLib.map().areaLocation(scripted)
								.getTimeObj().getDayOfMonth());
						break;
					case 3:
						results.append(CMLib.map().areaLocation(scripted)
			...
			case 13: // stat
			{
				...
				if (E != null) {
					String val = getStatValue(E, arg2);
					if (val == null) {
						logError(scripted, "STAT", "Syntax", "Unknown stat: "
								+ arg2 + " for " + E.name());
						break;
					}
			...
			case 52: // gstat
			{
				...
				if (E != null) {
					String val = getGStatValue(E, arg2);
					if (val == null) {
						logError(scripted, "GSTAT", "Syntax", "Unknown stat: "
								+ arg2 + " for " + E.name());
						break;
					}
			...
			{
				String arg1 = CMParms.cleanBit(funcParms);
				Physical P = getArgumentItem(arg1, source, monster, scripted,
						target, primaryItem, secondaryItem, msg, tmp);
				if (P != null) {
					String sex = "STANDING";
					if (CMLib.flags().isSleeping(P))
			...
			case 84: // math
			{
				String arg1 = varify(source, target, scripted, monster,
						...
						CMParms.cleanBit(funcParms));
				results.append(""
						+ Math.round(CMath.s_parseMathExpression(arg1)));
				break;
			}
			case 85: // islike
			{
			...
			case 68: // clandata
			{
				...
				if ((E != null) && (E instanceof MOB)) {
					...
					if (C == null)
						C = ((MOB) E).clans().iterator().hasNext() ? ((MOB) E)
								.clans().iterator().next().first : null;
					if (C != null)
						clanID = C.clanID();
				} else
					clanID = varify(source, target, scripted, monster,
			...
			case 98: // clanqualifies
			{
				...
				if (C != null) {
					if (C.getAcceptanceSettings().length() > 0)
						results.append(CMLib.masking().maskDesc(
					...
					int total = CMProps
							.getMaxClansThisCategory(C.getCategory());
					if (C.getCategory().length() > 0)
						results.append("May belong to only " + total + " "
								+ C.getCategory() + " clan. ");
					else
						results.append("May belong to only " + total
								+ " standard clan. ");
			...
			case 67: // hastitle
			{
				...
				Environmental E = getArgumentMOB(arg1, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if ((arg2.length() > 0) && (E instanceof MOB)
						&& (((MOB) E).playerStats() != null)) {
					MOB M = (MOB) E;
			...
			case 88: // mood
			{
				...
				Environmental E = getArgumentMOB(arg1, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if ((E instanceof MOB)
						&& (((MOB) E).fetchEffect("Mood") != null))
					results.append(CMStrings.capitalizeAndLower(((MOB) E)
							.fetchEffect("Mood").text()));
				break;
			}
			case 22: // baseclass
			{
				...
				Environmental E = getArgumentMOB(arg1, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if ((E != null) && (E instanceof MOB))
					results.append(((MOB) E).charStats().getCurrentClass()
							.baseClass());
				break;
			}
			case 23: // race
			...
			case 24: // racecat
			{
				...
				Environmental E = getArgumentMOB(arg1, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if ((E != null) && (E instanceof MOB))
					results.append(((MOB) E).charStats().getMyRace()
							.racialCategory());
				break;
			}
			case 25: // goldamt
			...
			case 76: // value
			{
				...
				if (E == null)
					...
					if (E instanceof MOB)
						val1 = (int) Math.round(CMLib.beanCounter()
								.getTotalAbsoluteValue((MOB) E, arg2));
					else if (E instanceof Coins) {
			...
			case 27: // var
			{
				...
				Environmental E = getArgumentItem(arg1, source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				String val = getVar(E, arg1, arg2, source, target, scripted,
						monster, primaryItem, secondaryItem, msg, tmp);
				results.append(val);
				break;
			}
			case 41: // eval
			...
			case 42: // randnum
			{
				String arg1String = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp,
						CMParms.cleanBit(funcParms)).toUpperCase();
				int arg1 = 0;
			...
			case 96: // iscontents
			{
				...
				if (E instanceof Rideable) {
					for (final Iterator<Rider> r = ((Rideable) E).riders(); r
							.hasNext();)
						results.append(
								CMParms.quoteIfNecessary(r.next().name()))
								.append(" ");
				}
				if (E instanceof Container) {
					for (final Iterator<Item> i = ((Container) E).getContents()
							.iterator(); i.hasNext();)
						results.append(
								CMParms.quoteIfNecessary(i.next().name()))
								.append(" ");
				}
				break;
			}
			case 97: // wornon
			{
				...
				int wornLoc = -1;
				if (arg2.length() > 0)
					wornLoc = CMParms.indexOf(Wearable.CODES.NAMESUP(), arg2
							.toUpperCase().trim());
				if (wornLoc < 0)
					logError(scripted, "EVAL", "BAD WORNON LOCATION", arg2);
				else if (E instanceof MOB) {
					final List<Item> items = ((MOB) E).fetchWornItems(
							...
							(short) 0);
					for (Item item : items)
						results.append(CMParms.quoteIfNecessary(item.name()))
								.append(" ");
				}
				break;
			}
	...
	protected MOB getRandPC(MOB monster, Object[] tmp, Room room) {
		if ((tmp[SPECIAL_RANDPC] == null) || (tmp[SPECIAL_RANDPC] == monster)) {
			...
			if (room != null) {
				...
				if (choices.size() > 0)
					tmp[SPECIAL_RANDPC] = choices.elementAt(CMLib.dice().roll(
							1, choices.size(), -1));
			}
		}
	...
	protected MOB getRandAnyone(MOB monster, Object[] tmp, Room room) {
		if ((tmp[SPECIAL_RANDANYONE] == null)
				|| (tmp[SPECIAL_RANDANYONE] == monster)) {
			MOB M = null;
			if (room != null) {
				Vector choices = new Vector();
				for (int p = 0; p < room.numInhabitants(); p++) {
				...
				if (choices.size() > 0)
					tmp[SPECIAL_RANDANYONE] = choices.elementAt(CMLib.dice()
		...
		for (int si = 1; si < script.size(); si++) {
			...
			case 57: // <SCRIPT>
			{
				...
				if (CMSecurity.isApprovedJScript(jscript)) {
					...
					try {
						JScriptEvent scope = new JScriptEvent(this, scripted,
								source, target, monster, primaryItem,
								secondaryItem, msg, tmp);
						cx.initStandardObjects(scope);
						String[] names = { "host", "source", "target",
								"monster", "item", "item2", "message",
								"getVar", "setVar", "toJavaString" };
						scope.defineFunctionProperties(names,
								JScriptEvent.class, ScriptableObject.DONTENUM);
						cx.evaluateString(scope, jscript.toString(), "<cmd>",
								1, null);
					} catch (Exception e) {
						Log.errOut(
								"Scripting",
								scripted.name()
										...
										+ CMLib.map().getExtendedRoomID(
												lastKnownLocation)
										+ "/JSCRIPT Error: " + e.getMessage());
					}
					Context.exit();
				} else if (CMProps.getIntVar(CMProps.Int.JSCRIPTS) == 1) {
					if (lastKnownLocation != null)
						lastKnownLocation
								.showHappens(
										CMMsg.MSG_OK_ACTION,
										"A Javascript was not authorized.  Contact an Admin to use MODIFY JSCRIPT to authorize this script.");
				}
				break;
			}
			case 19: // if
			{
				...
				String[][] EVAL = { tt };
				boolean condition = eval(scripted, source, target, monster,
						primaryItem, secondaryItem, msg, tmp, EVAL, 1);
				if (EVAL[0] != tt) {
					tt = EVAL[0];
					script.setElementAt(si, 2, tt);
				}
				...
				while (si < script.size()) {
					...
					if (cmd.equals("<SCRIPT>")) {
						...
						if (s.substring(4).trim().length() > 0) {
							...
							script.insertElementAt(si + 1, s.substring(4)
									.trim(), null);
						} else if (tt == null)
							tt = parseBits(script, si, "C");
					} else {
				...
				if (V.size() > 1) {
					...
					String response = execute(scripted, source, target,
							monster, primaryItem, secondaryItem, V, msg, tmp);
					if (response != null) {
						tickStatus = Tickable.STATUS_END;
						return response;
					}
			...
			case 70: // switch
			{
				...
				String var = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]).trim();
				DVector V = new DVector(2);
				V.addElement("", null);
				int depth = 0;
				boolean foundendif = false;
				...
				while (si < script.size()) {
					...
					if (cmd.equals("<SCRIPT>")) {
						...
						s2 = varify(source, target, scripted, monster,
								primaryItem, secondaryItem, msg, tmp, tt[1])
								.trim();
				...
				if (!foundendif) {
					logError(scripted, "SWITCH", "Syntax",
							" Without ENDSWITCH!");
					tickStatus = Tickable.STATUS_END;
					return null;
				}
			...
			case 62: // for x = 1 to 100
			{
				...
				String varStr = tt[1];
				if ((varStr.length() != 2) || (varStr.charAt(0) != '$')
						|| (!Character.isDigit(varStr.charAt(1)))) {
					logError(scripted, "FOR", "Syntax", "'" + varStr
							+ "' is not a tmp var $1, $2..");
					tickStatus = Tickable.STATUS_END;
					return null;
				}
				int whichVar = CMath
						.s_int(Character.toString(varStr.charAt(1)));
				if ((tmp[whichVar] instanceof String)
						&& (((String) tmp[whichVar]).length() > 0)
						&& (CMath.isInteger(((String) tmp[whichVar]).trim()))) {
					logError(scripted, "FOR", "Syntax", "'" + whichVar
							+ "' is already in use! Use a different one!");
					tickStatus = Tickable.STATUS_END;
					return null;
				}
				if (!tt[2].equals("=")) {
					logError(scripted, "FOR", "Syntax", "'" + s
							+ "' is illegal for syntax!");
					tickStatus = Tickable.STATUS_END;
					return null;
				}
				...
				String from = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]).trim();
				String to = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[5]).trim();
				if ((!CMath.isInteger(from)) || (!CMath.isInteger(to))) {
					logError(scripted, "FOR", "Syntax", "'" + from + "-" + to
							+ "' is illegal range!");
					tickStatus = Tickable.STATUS_END;
					return null;
				}
				...
				if (V.size() > 1) {
					...
					if (((increment > 0) && (fromInt <= (toInt + toAdd)))
						...
						for (int forLoop = fromInt; forLoop != toInt; forLoop += increment) {
							...
							response = execute(scripted, source, target,
									monster, primaryItem, secondaryItem, V,
									msg, tmp);
							if (response != null)
								break;
							if (System.currentTimeMillis() > tm) {
								logError(scripted, "FOR", "Runtime",
										"For loop violates 10 second rule: "
												+ s);
								break;
							}
						}
						tmp[whichVar] = "" + toInt;
						response = execute(scripted, source, target, monster,
								primaryItem, secondaryItem, V, msg, tmp);
					}
					if (response != null) {
						tickStatus = Tickable.STATUS_END;
						return response;
			...
			case 1: // mpasound
			{
				...
				String echo = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
				...
				for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
					...
					if ((R2 != null) && (E2 != null) && (E2.isOpen()))
						R2.showOthers(monster, null, null, CMMsg.MSG_OK_ACTION,
								echo);
				}
				break;
			}
			case 4: // mpjunk
			{
				...
				if (tt[1].equals("ALL") && (monster != null)) {
					...
					Environmental E = getArgumentItem(tt[1], source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
					Item I = null;
					...
					if ((I == null) && (monster != null))
						I = monster.findItem(varify(source, target, scripted,
								monster, primaryItem, secondaryItem, msg, tmp,
								tt[1]));
					if ((I == null) && (scripted instanceof Room))
						I = ((Room) scripted).findItem(varify(source, target,
								scripted, monster, primaryItem, secondaryItem,
								msg, tmp, tt[1]));
					if (I != null)
						I.destroy();
				}
			...
			case 13: // mpunaffect
			{
				...
				Physical newTarget = getArgumentItem(tt[1], source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				String which = tt[2];
				if (newTarget != null)
					if (which.equalsIgnoreCase("all") || (which.length() == 0)) {
						...
						if (A != null) {
							if ((newTarget instanceof MOB)
								...
								Log.sysOut("Scripting", newTarget.Name()
										+ " was MPUNAFFECTED by " + A.Name());
							A.unInvoke();
			...
			case 3: // mpslay
			{
				...
				Environmental newTarget = getArgumentItem(tt[1], source,
						monster, scripted, target, primaryItem, secondaryItem,
						msg, tmp);
			...
			case 73: // mpsetinternal
			{
				...
				String arg2 = tt[1];
				String arg3 = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				if (arg2.equals("SCOPE"))
					setVarScope(arg3);
				else if (arg2.equals("NODELAY"))
					noDelay = CMath.s_bool(arg3);
				else if (arg2.equals("ACTIVETRIGGER")
						|| arg2.equals("ACTIVETRIGGERS"))
					...
					logError(scripted, "MPSETINTERNAL", "Syntax",
							"Unknown stat: " + arg2);
				break;
			}
			case 74: // mpprompt
			{
				...
				String var = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				String promptStr = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
				if ((newTarget != null) && (newTarget instanceof MOB)
						&& ((((MOB) newTarget).session() != null))) {
					try {
						String value = ((MOB) newTarget).session().prompt(
								promptStr, 60000);
			...
			case 75: // mpconfirm
			{
				...
				String var = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				String defaultVal = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
				String promptStr = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[4]);
				if ((newTarget != null) && (newTarget instanceof MOB)
						&& ((((MOB) newTarget).session() != null))) {
					try {
						String value = ((MOB) newTarget).session().confirm(
								promptStr, defaultVal, 60000) ? "Y" : "N";
						setVar(newTarget.Name(), var, value);
					} catch (Exception e) {
						return "";
					}
			...
			case 76: // mpchoose
			{
				...
				String var = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				String choices = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
				String defaultVal = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[4]);
				String promptStr = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[5]);
				if ((newTarget != null) && (newTarget instanceof MOB)
						&& ((((MOB) newTarget).session() != null))) {
					try {
						String value = ((MOB) newTarget).session().choose(
								promptStr, choices, defaultVal, 60000);
						setVar(newTarget.Name(), var, value);
					} catch (Exception e) {
						return "";
					}
			...
			case 16: // mpset
			{
				...
				if (newTarget != null) {
					...
					if ((!found) && (newTarget instanceof MOB)) {
						...
						for (int i : CharStats.CODES.ALL())
							if (CharStats.CODES.NAME(i).equalsIgnoreCase(arg2)
									|| CharStats.CODES.DESC(i)
											.equalsIgnoreCase(arg2)) {
								if (arg3.equals("++"))
									arg3 = ""
											+ (M.baseCharStats().getStat(i) + 1);
								if (arg3.equals("--"))
									arg3 = ""
											+ (M.baseCharStats().getStat(i) - 1);
								M.baseCharStats().setStat(i,
										CMath.s_int(arg3.trim()));
								M.recoverCharStats();
								if (arg2.equalsIgnoreCase("RACE"))
									M.charStats().getMyRace()
											.startRacing(M, false);
								found = true;
								break;
							}
						...
						if (!found)
							for (int i = 0; i < M.basePhyStats().getStatCodes().length; i++)
								if (M.basePhyStats().getStatCodes()[i]
										.equalsIgnoreCase(arg2)) {
									if (arg3.equals("++"))
										arg3 = ""
												+ (CMath.s_int(M
										...
										arg3 = ""
												+ (CMath.s_int(M
														...
														.getStat(
																M.baseState()
																		.getStatCodes()[i])) - 1);
									M.baseState().setStat(arg2.substring(4),
											arg3);
									found = true;
									break;
								}
						if ((!found)
								&& (gstatH.containsKey(arg2.toUpperCase()))) {
							found = true;
							switch (gstatH.get(arg2.toUpperCase()).intValue()) {
							case GSTATADD_DEITY:
							...
							case GSTATADD_CLAN: {
								...
								if (p == null) {
									...
									if (C != null)
										p = new Pair<Clan, Integer>(C,
												Integer.valueOf(C
														.getGovernment()
														.getAcceptPos()));
								}
								if (p != null)
									M.setClan(p.first.clanID(),
											p.second.intValue());
								break;
							}
							case GSTATADD_CLANROLE: {
								Clan C = CMLib.clans().findRivalrousClan(
										(MOB) newTarget);
								if (C == null)
									C = ((MOB) newTarget).clans().iterator()
											.hasNext() ? ((MOB) newTarget)
											.clans().iterator().next().first
											: null;
								if (C != null)
					...
					if (!found) {
						logError(scripted, "MPSET", "Syntax", "Unknown stat: "
								+ arg2 + " for " + newTarget.Name());
						break;
					}
					if (newTarget instanceof MOB)
					...
					if (newTarget instanceof MOB) {
						...
						if (arg2.equalsIgnoreCase("LEVEL")) {
							CMLib.leveler().fillOutMOB(((MOB) newTarget),
									((MOB) newTarget).basePhyStats().level());
							((MOB) newTarget).recoverMaxState();
							((MOB) newTarget).recoverCharStats();
			...
			case 63: // mpargset
			{
				...
				if ((arg1.length() != 2) || (!arg1.startsWith("$"))) {
					logError(scripted, "MPARGSET", "Syntax",
							"Mangled argument var: " + arg1 + " for "
									+ scripted.Name());
					break;
				}
				Object O = getArgumentMOB(arg2, source, monster, target,
						primaryItem, secondaryItem, msg, tmp);
				if (O == null)
					O = getArgumentItem(arg2, source, monster, scripted,
							target, primaryItem, secondaryItem, msg, tmp);
				if ((O == null)
						&& ((!arg2.trim().startsWith("$"))
								|| (arg2.length() < 2) || ((!Character
								.isDigit(arg2.charAt(1))) && (!Character
								.isLetter(arg2.charAt(1))))))
					O = varify(source, target, scripted, monster, primaryItem,
							secondaryItem, msg, tmp, arg2);
				char c = arg1.charAt(1);
				if (Character.isDigit(c)) {
					if ((O instanceof String)
							&& (((String) O).equalsIgnoreCase("null")))
						O = null;
			...
			case 35: // mpgset
			{
				...
				if (newTarget != null) {
								...
								arg3 = ""
										+ (CMath.s_int(newTarget
												.getStat(newTarget
														.getStatCodes()[i])) - 1);
							newTarget
									.setStat(newTarget.getStatCodes()[i], arg3);
							found = true;
							break;
						}
					}
					if (!found)
						if (newTarget instanceof MOB) {
							for (int i = 0; i < GenericBuilder.GENMOBCODES.length; i++) {
								if (GenericBuilder.GENMOBCODES[i]
										.equalsIgnoreCase(arg2)) {
									if (arg3.equals("++"))
										arg3 = ""
												+ (CMath.s_int(CMLib
														...
														.getGenMobStat(
																(MOB) newTarget,
																GenericBuilder.GENMOBCODES[i])) + 1);
									if (arg3.equals("--"))
										arg3 = ""
												+ (CMath.s_int(CMLib
														.coffeeMaker()
							...
							if (!found) {
								...
								if (!found)
									for (int i = 0; i < M.curState()
											.getStatCodes().length; i++) {
										if (M.curState().getStatCodes()[i]
												.equalsIgnoreCase(arg2)) {
											if (arg3.equals("++"))
												arg3 = ""
														...
														+ (CMath.s_int(M
																...
																.getStat(
																		M.baseState()
																				.getStatCodes()[i])) - 1);
											M.baseState().setStat(
													arg2.substring(4), arg3);
											found = true;
											break;
										}
								if ((!found)
										&& (gstatH.containsKey(arg2
												.toUpperCase()))) {
									found = true;
									switch (gstatH.get(arg2.toUpperCase())
											.intValue()) {
									case GSTATADD_DEITY:
										M.setWorshipCharID(arg3);
										break;
									case GSTATADD_CLAN: {
										Pair<Clan, Integer> p = M
												.getClanRole(arg3);
										if (p == null) {
											Clan C = CMLib.clans()
													.getClan(arg3);
											if (C != null)
												p = new Pair<Clan, Integer>(
														C,
														Integer.valueOf(C
																.getGovernment()
																.getAcceptPos()));
									...
									case GSTATADD_CLANROLE: {
										Clan C = CMLib.clans()
												.findRivalrousClan(
														(MOB) newTarget);
										if (C == null)
											C = ((MOB) newTarget).clans()
													.iterator().hasNext() ? ((MOB) newTarget)
													.clans().iterator().next().first
													: null;
										if (C != null)
											M.setClan(C.clanID(),
							...
							for (int i = 0; i < GenericBuilder.GENITEMCODES.length; i++) {
								if (GenericBuilder.GENITEMCODES[i]
										.equalsIgnoreCase(arg2)) {
									if (arg3.equals("++"))
										arg3 = ""
												+ (CMath.s_int(CMLib
														...
														.getGenItemStat(
																(Item) newTarget,
																GenericBuilder.GENITEMCODES[i])) + 1);
									if (arg3.equals("--"))
										arg3 = ""
												+ (CMath.s_int(CMLib
														.coffeeMaker()
														.getGenItemStat(
																(Item) newTarget,
																GenericBuilder.GENITEMCODES[i])) - 1);
									CMLib.coffeeMaker().setGenItemStat(
											(Item) newTarget,
											GenericBuilder.GENITEMCODES[i],
											arg3);
									found = true;
					...
					if (!found) {
						logError(scripted, "MPGSET", "Syntax", "Unknown stat: "
								+ arg2 + " for " + newTarget.Name());
						break;
					}
					if (newTarget instanceof MOB)
			...
			case 11: // mpexp
			{
				...
				Physical newTarget = getArgumentItem(tt[1], source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				String amtStr = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]).trim();
				int t = CMath.s_int(amtStr);
				if ((newTarget != null) && (newTarget instanceof MOB)) {
					if ((amtStr.endsWith("%"))
							&& (((MOB) newTarget).getExpNeededLevel() < Integer.MAX_VALUE)) {
						int baseLevel = newTarget.basePhyStats().level();
						int lastLevelExpNeeded = (baseLevel <= 1) ? 0 : CMLib
								.leveler().getLevelExperience(baseLevel - 1);
						int thisLevelExpNeeded = CMLib.leveler()
								.getLevelExperience(baseLevel);
						t = (int) Math.round(CMath.mul(
								thisLevelExpNeeded - lastLevelExpNeeded,
								CMath.div(
										CMath.s_int(amtStr.substring(0,
												amtStr.length() - 1)), 100.0)));
					}
					if (t != 0)
						CMLib.leveler().postExperience((MOB) newTarget, null,
								null, t, false);
				}
				break;
			}
			case 77: // mpmoney
			{
				...
				Environmental newTarget = getArgumentMOB(tt[1], source,
						monster, scripted, primaryItem, secondaryItem, msg, tmp);
				if (newTarget == null)
					newTarget = getArgumentItem(tt[1], source, monster,
							scripted, target, primaryItem, secondaryItem, msg,
							tmp);
				String amtStr = tt[2];
				amtStr = varify(source, target, scripted, monster, primaryItem,
						secondaryItem, msg, tmp, amtStr).trim();
				boolean plus = !amtStr.startsWith("-");
				if (amtStr.startsWith("+") || amtStr.startsWith("-"))
					amtStr = amtStr.substring(1).trim();
				String currency = CMLib.english().numPossibleGoldCurrency(
						source, amtStr);
				long amt = CMLib.english().numPossibleGold(source, amtStr);
				double denomination = CMLib.english()
						.numPossibleGoldDenomination(source, currency, amtStr);
				Container container = null;
				if (newTarget instanceof Item) {
					container = (newTarget instanceof Container) ? (Container) newTarget
							: null;
					newTarget = ((Item) newTarget).owner();
				}
				if (newTarget instanceof MOB) {
					if (plus)
						CMLib.beanCounter().giveSomeoneMoney((MOB) newTarget,
								currency, amt * denomination);
					else
						CMLib.beanCounter().subtractMoney((MOB) newTarget,
								currency, amt * denomination);
				} else {
					if (!(newTarget instanceof Room))
						newTarget = lastKnownLocation;
					if (plus)
						CMLib.beanCounter().dropMoney((Room) newTarget,
								container, currency, amt * denomination);
					else
						CMLib.beanCounter().removeMoney((Room) newTarget,
								container, currency, amt * denomination);
				}
				break;
			}
				...
				if (newTarget instanceof MOB) {
					if (CMath.isNumber(val))
						((MOB) newTarget).setQuestPoint(CMath.s_int(val));
							...
							&& (CMath.isNumber(val.substring(2).trim())))
						((MOB) newTarget).setQuestPoint(((MOB) newTarget)
								.getQuestPoint()
								+ CMath.s_int(val.substring(2).trim()));
					else if (val.startsWith("--")
							&& (CMath.isNumber(val.substring(2).trim())))
						((MOB) newTarget).setQuestPoint(((MOB) newTarget)
								.getQuestPoint()
						...
						logError(scripted, "QUESTPOINTS", "Syntax",
								"Bad syntax " + val + " for " + scripted.Name());
				}
				break;
			}
			case 65: // MPQSET
			{
				...
				String qstr = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
				String var = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				PhysicalAgent obj = getArgumentItem(tt[3], source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				String val = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
				Quest Q = getQuest(qstr);
				if (Q == null)
					logError(scripted, "MPQSET", "Syntax", "Unknown quest "
							+ qstr + " for " + scripted.Name());
					...
					if (obj == null)
						logError(
								...
								"MPQSET",
								"Syntax",
								"Unknown object " + tt[3] + " for "
										+ scripted.Name());
					else {
						obj.basePhyStats().setDisposition(
								obj.basePhyStats().disposition()
										| PhyStats.IS_UNSAVABLE);
						obj.recoverPhyStats();
						Q.runtimeRegisterObject(obj);
					}
				} else if (var.equalsIgnoreCase("STATISTICS")
						&& (val.equalsIgnoreCase("ACCEPTED")))
					CMLib.coffeeTables().bump(Q,
							CoffeeTableRow.STAT_QUESTACCEPTED);
				else if (var.equalsIgnoreCase("STATISTICS")
						&& (val.equalsIgnoreCase("SUCCESS") || val
								.equalsIgnoreCase("WON")))
					CMLib.coffeeTables().bump(Q,
							CoffeeTableRow.STAT_QUESTSUCCESS);
				else if (var.equalsIgnoreCase("STATISTICS")
						&& (val.equalsIgnoreCase("FAILED")))
					CMLib.coffeeTables().bump(Q,
							CoffeeTableRow.STAT_QUESTFAILED);
				else {
					if (val.equals("++"))
			...
			case 66: // MPLOG
			{
				...
				String type = tt[1];
				String head = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				String val = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
				if (type.startsWith("E"))
					...
					logError(scripted, "MPLOG", "Syntax", "Unknown log type "
							+ type + " for " + scripted.Name());
				break;
			}
			case 67: // MPCHANNEL
			{
				...
				String channel = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
			...
			case 68: // unload
			{
				...
				String scriptname = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
				if (!new CMFile(Resources.makeFileResourceName(scriptname),
						null, CMFile.FLAG_FORCEALLOW).exists())
					logError(
							scripted,
							"MPUNLOADSCRIPT",
							"Runtime",
							"File does not exist: "
									+ Resources
											.makeFileResourceName(scriptname));
				else {
					Vector delThese = new Vector();
					boolean foundKey = false;
					scriptname = scriptname.toUpperCase().trim();
					String parmname = scriptname;
					for (Iterator<String> k = Resources
							.findResourceKeys(parmname); k.hasNext();) {
						String key = k.next();
						if (key.startsWith("PARSEDPRG: ")
								&& (key.toUpperCase().endsWith(parmname))) {
							foundKey = true;
					...
					if (foundKey)
						for (int i = 0; i < delThese.size(); i++)
							Resources.removeResource((String) delThese
									.elementAt(i));
				}
				...
				if (newTarget instanceof MOB) {
					if (CMath.isNumber(val))
						((MOB) newTarget).setTrains(CMath.s_int(val));
							...
							&& (CMath.isNumber(val.substring(2).trim())))
						((MOB) newTarget).setTrains(((MOB) newTarget)
								.getTrains()
								+ CMath.s_int(val.substring(2).trim()));
					else if (val.startsWith("--")
							&& (CMath.isNumber(val.substring(2).trim())))
						...
						logError(scripted, "TRAINS", "Syntax", "Bad syntax "
								+ val + " for " + scripted.Name());
				}
				break;
			}
			case 61: // pracs
				...
				if (newTarget instanceof MOB) {
					if (CMath.isNumber(val))
						((MOB) newTarget).setPractices(CMath.s_int(val));
							...
							&& (CMath.isNumber(val.substring(2).trim())))
						((MOB) newTarget).setPractices(((MOB) newTarget)
								.getPractices()
								+ CMath.s_int(val.substring(2).trim()));
					else if (val.startsWith("--")
							&& (CMath.isNumber(val.substring(2).trim())))
						...
						logError(scripted, "PRACS", "Syntax", "Bad syntax "
								+ val + " for " + scripted.Name());
				}
				break;
			}
			case 5: // mpmload
			{
				...
				if (lastKnownLocation != null) {
					if (Ms.size() == 0)
						findSomethingCalledThis(name, monster,
								lastKnownLocation, Ms, true);
				...
				if ((scripted instanceof MOB) && (monster != null)) {
					...
					if (containerIndex >= 0) {
						...
						findSomethingCalledThis(
								name.substring(containerIndex + 6).trim(),
								monster, lastKnownLocation, containers, false);
						for (int c = 0; c < containers.size(); c++)
							if ((containers.elementAt(c) instanceof Container)
									&& (((Container) containers.elementAt(c))
											.capacity() > 0)) {
					...
					if (coins > 0) {
						String currency = CMLib.english()
								.numPossibleGoldCurrency(scripted, name);
						double denom = CMLib.english()
								.numPossibleGoldDenomination(scripted,
										currency, name);
						Coins C = CMLib.beanCounter().makeCurrency(currency,
								denom, coins);
						monster.addItem(C);
						C.putCoinsBack();
					} else if (lastKnownLocation != null) {
						...
						if (m != null)
							...
							findSomethingCalledThis(name, (MOB) scripted,
									lastKnownLocation, Is, false);
			...
			case 41: // mpoloadroom
			{
				...
				if (lastKnownLocation != null) {
					...
					if (containerIndex >= 0) {
						...
						findSomethingCalledThis(
								name.substring(containerIndex + 6).trim(),
								null, lastKnownLocation, containers, false);
						for (int c = 0; c < containers.size(); c++)
							if ((containers.elementAt(c) instanceof Container)
									&& (((Container) containers.elementAt(c))
											.capacity() > 0)) {
					...
					if (coins > 0) {
						String currency = CMLib.english()
								.numPossibleGoldCurrency(monster, name);
						double denom = CMLib.english()
								.numPossibleGoldDenomination(monster, currency,
										name);
						Coins C = CMLib.beanCounter().makeCurrency(currency,
								denom, coins);
						Is.addElement(C);
			...
			case 42: // mphide
			{
				...
				Physical newTarget = getArgumentItem(tt[1], source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				if (newTarget != null) {
					newTarget.basePhyStats().setDisposition(
							newTarget.basePhyStats().disposition()
			...
			case 58: // mpreset
			{
				...
				String arg = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
				if (arg.equalsIgnoreCase("area")) {
					...
					if (R != null)
						...
						if (A != null)
							...
							logError(scripted, "MPRESET", "Syntax",
									"Unknown location: " + arg + " for "
											+ scripted.Name());
					}
				}
				break;
			...
			case 71: // mprejuv
			{
				...
				String next = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
				String rest = "";
				if (tt.length > 2)
					rest = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[2]);
				int tickID = -1;
				if (rest.equalsIgnoreCase("item")
						|| rest.equalsIgnoreCase("items"))
					tickID = Tickable.TICKID_ROOM_ITEM_REJUV;
				else if (rest.equalsIgnoreCase("mob")
						|| rest.equalsIgnoreCase("mobs"))
				...
				if (next.equalsIgnoreCase("area")) {
					...
					if (R != null)
						...
						if (A != null)
							for (Enumeration e = A.getProperMap(); e
									.hasMoreElements();)
								CMLib.threads().rejuv((Room) e.nextElement(),
										tickID);
						else
							logError(scripted, "MPREJUV", "Syntax",
									"Unknown location: " + next + " for "
											+ scripted.Name());
					}
				}
				break;
			...
			case 56: // mpstop
			{
				...
				if (who.equalsIgnoreCase("all")) {
					...
					Environmental newTarget = getArgumentItem(who, source,
							monster, scripted, target, primaryItem,
							secondaryItem, msg, tmp);
					if (newTarget instanceof MOB)
			...
			case 43: // mpunhide
			{
				...
				Physical newTarget = getArgumentItem(tt[1], source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				if ((newTarget != null)
						&& (CMath.bset(newTarget.basePhyStats().disposition(),
								PhyStats.IS_NOT_SEEN))) {
					newTarget.basePhyStats().setDisposition(
							newTarget.basePhyStats().disposition()
									- PhyStats.IS_NOT_SEEN);
			...
			case 44: // mpopen
			{
				...
				if ((newTarget instanceof Exit)
					...
					E.setDoorsNLocks(E.hasADoor(), true, E.defaultsClosed(),
							E.hasALock(), false, E.defaultsLocked());
					if (lastKnownLocation != null)
						lastKnownLocation.recoverRoomStats();
				} else if ((newTarget instanceof Container)
						&& (((Container) newTarget).hasALid())) {
			...
			case 46: // mplock
			{
				...
				if ((newTarget instanceof Exit)
					...
					Exit E = (Exit) newTarget;
					E.setDoorsNLocks(E.hasADoor(), false, E.defaultsClosed(),
							E.hasALock(), true, E.defaultsLocked());
					if (lastKnownLocation != null)
						lastKnownLocation.recoverRoomStats();
				} else if ((newTarget instanceof Container)
			...
			case 48: // return
				...
				tickStatus = Tickable.STATUS_END;
				return varify(source, target, scripted, monster, primaryItem,
						secondaryItem, msg, tmp, tt[1]);
			case 7: // mpechoat
			{
				if (tt == null) {
				...
				String parm = tt[1];
				Environmental newTarget = getArgumentMOB(parm, source, monster,
						target, primaryItem, secondaryItem, msg, tmp);
				if ((newTarget != null) && (newTarget instanceof MOB)
						&& (lastKnownLocation != null)) {
					if (newTarget == monster)
						lastKnownLocation.showSource(
								monster,
								null,
								null,
								CMMsg.MSG_OK_ACTION,
				...
				} else if (parm.equalsIgnoreCase("area")
						&& (lastKnownLocation != null)) {
					lastKnownLocation
							.showSource(
									monster,
					...
					lastKnownLocation
							.showSource(
									...
									varify(source, target, scripted, monster,
											primaryItem, secondaryItem, msg,
											tmp, tt[2]));
					for (Enumeration e = CMLib.map().findArea(parm)
							.getMetroMap(); e.hasMoreElements();) {
						Room R = (Room) e.nextElement();
						if (R.numInhabitants() > 0)
							R.showOthers(
									monster,
			...
			case 8: // mpechoaround
			{
				...
				Environmental newTarget = getArgumentMOB(tt[1], source,
						monster, target, primaryItem, secondaryItem, msg, tmp);
				if ((newTarget != null) && (newTarget instanceof MOB)
						&& (lastKnownLocation != null)) {
					lastKnownLocation
							.showOthers(
			...
			case 9: // mpcast
			{
				...
				String cast = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
				Physical newTarget = getArgumentItem(tt[2], source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				Ability A = null;
				if (cast != null)
			...
			case 30: // mpaffect
			{
				...
				Physical newTarget = getArgumentItem(tt[2], source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				String m2 = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
				Ability A = null;
				if (cast != null)
					A = CMClass.findAbility(cast);
			...
			case 80: // mpspeak
			{
				...
				String language = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				Environmental newTarget = getArgumentMOB(tt[1], source,
						monster, target, primaryItem, secondaryItem, msg, tmp);
				...
				if ((A instanceof Language) && (newTarget instanceof MOB)) {
					...
					if (langA != null) {
						...
						langA.invoke((MOB) newTarget, CMParms.parse(""),
								(MOB) newTarget, false, 0);
					} else
						A.invoke((MOB) newTarget, CMParms.parse(""),
								(MOB) newTarget, false, 0);
				}
				break;
			}
			case 81: // mpsetclan
			{
				...
				PhysicalAgent newTarget = getArgumentMOB(tt[1], source,
						monster, target, primaryItem, secondaryItem, msg, tmp);
				String clan = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				String roleStr = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
			...
			case 31: // mpbehave
			{
				...
				String cast = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
				PhysicalAgent newTarget = getArgumentItem(tt[2], source,
						monster, scripted, target, primaryItem, secondaryItem,
						msg, tmp);
				String m2 = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
				Behavior B = null;
				Behavior B2 = (cast == null) ? null : CMClass
						.findBehavior(cast);
				if (B2 != null)
					cast = B2.ID();
			...
			case 72: // mpscript
			{
				...
				PhysicalAgent newTarget = getArgumentItem(tt[1], source,
						monster, scripted, target, primaryItem, secondaryItem,
						msg, tmp);
				String m2 = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				...
				while (proceed) {
					...
					} else if (m2.toUpperCase().startsWith("INDIVIDUAL")
							|| m2.equals("*")) {
						scope = "*";
						proceed = true;
				...
				if ((newTarget != null) && (m2.length() > 0)) {
					if ((newTarget instanceof MOB)
							&& (!((MOB) newTarget).isMonster()))
						Log.sysOut("Scripting", newTarget.Name()
								+ " was MPSCRIPTED: " + defaultQuestName);
			...
			case 32: // mpunbehave
			{
				...
				if ((newTarget != null) && (cast != null)) {
					...
					if ((newTarget instanceof MOB)
							&& (!((MOB) newTarget).isMonster()) && (B != null))
						Log.sysOut("Scripting", newTarget.Name()
								+ " was MPUNBEHAVED with " + B.name());
				}
				break;
			}
			case 33: // mptattoo
			{
				...
				Environmental newTarget = getArgumentItem(tt[1], source,
						monster, scripted, target, primaryItem, secondaryItem,
						msg, tmp);
				String tattooName = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				if ((newTarget != null) && (tattooName.length() > 0)
			...
			case 55: // mpnotrigger
			{
				...
				String trigger = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
				String time = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				int triggerCode = -1;
				for (int i = 0; i < progs.length; i++)
					if (trigger.equalsIgnoreCase(progs[i]))
						triggerCode = i;
				if (triggerCode < 0)
					logError(scripted, "MPNOTRIGGER", "RunTime", trigger
							+ " is not a valid trigger name.");
				else if (!CMath.isInteger(time.trim()))
					logError(scripted, "MPNOTRIGGER", "RunTime", time
							+ " is not a valid milisecond time.");
				else {
					noTrigger.remove(Integer.valueOf(triggerCode));
					noTrigger.put(
							Integer.valueOf(triggerCode),
							Long.valueOf(System.currentTimeMillis()
									+ CMath.s_long(time.trim())));
				}
				break;
			}
			case 54: // mpfaction
			{
				...
				Environmental newTarget = getArgumentMOB(tt[1], source,
						monster, target, primaryItem, secondaryItem, msg, tmp);
				String faction = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				String range = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]).trim();
				Faction F = CMLib.factions().getFaction(faction);
				if ((newTarget != null) && (F != null)
						&& (newTarget instanceof MOB)) {
					MOB themob = (MOB) newTarget;
					if ((range.startsWith("--"))
							&& (CMath.isInteger(range.substring(2).trim()))) {
						int amt = CMath.s_int(range.substring(2).trim());
						themob.tell("You lose " + amt + " faction with "
								+ F.name() + ".");
						range = "" + (themob.fetchFaction(faction) - amt);
					} else if ((range.startsWith("+"))
							&& (CMath.isInteger(range.substring(1).trim()))) {
						int amt = CMath.s_int(range.substring(1).trim());
						themob.tell("You gain " + amt + " faction with "
								+ F.name() + ".");
						range = "" + (themob.fetchFaction(faction) + amt);
					} else if (CMath.isInteger(range))
						themob.tell("Your faction with " + F.name()
								+ " is now " + CMath.s_int(range.trim()) + ".");
					if (CMath.isInteger(range))
						themob.addFaction(F.factionID(),
								CMath.s_int(range.trim()));
					else {
						Faction.FRange FR = null;
						Enumeration<Faction.FRange> e = CMLib.factions()
								.getRanges(CMLib.factions().AlignID());
						if (e != null)
						...
						if (FR == null)
							logError(scripted, "MPFACTION", "RunTime", range
									+ " is not a valid range for " + F.name()
									+ ".");
						else {
							themob.tell("Your faction with " + F.name()
									+ " is now " + FR.name() + ".");
			...
			case 49: // mptitle
			{
				...
				Environmental newTarget = getArgumentItem(tt[1], source,
						monster, scripted, target, primaryItem, secondaryItem,
						msg, tmp);
				String titleStr = varify(monster, newTarget, scripted, monster,
						secondaryItem, secondaryItem, msg, tmp, tt[2]);
				if ((newTarget != null) && (titleStr.length() > 0)
						&& (newTarget instanceof MOB)) {
					MOB themob = (MOB) newTarget;
			...
			case 51: // mpsetclandata
			{
				...
				if ((newTarget != null) && (newTarget instanceof MOB)) {
					...
					if (C == null)
						C = ((MOB) newTarget).clans().iterator().hasNext() ? ((MOB) newTarget)
								.clans().iterator().next().first
								: null;
					if (C != null)
						clanID = C.clanID();
					...
					clanID = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[1]);
				String clanvar = tt[2];
				String clanval = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
				Clan C = CMLib.clans().getClan(clanID);
				if (C != null) {
					if (!C.isStat(clanvar))
						logError(scripted, "MPSETCLANDATA", "RunTime", clanvar
								+ " is not a valid clan variable.");
					else {
				...
				if ((newTarget != null) && (newTarget instanceof MOB)) {
					...
					for (int i = 0; i < V.size(); i++) {
						if (CMath.isInteger(V.elementAt(i).trim()))
							((MOB) newTarget).baseCharStats().setClassLevel(
									((MOB) newTarget).baseCharStats()
											.getCurrentClass(),
									CMath.s_int(V.elementAt(i).trim()));
						else {
			...
			case 12: // mppurge
			{
				if (lastKnownLocation != null) {
					...
					if (s2.equalsIgnoreCase("self")
							|| s2.equalsIgnoreCase("me"))
						E = scripted;
					else if (flag == 1) {
						s2 = varify(source, target, scripted, monster,
								primaryItem, secondaryItem, msg, tmp, s2);
						...
						E = getArgumentItem(s2, source, monster, scripted,
								target, primaryItem, secondaryItem, msg, tmp);
					if (E != null) {
						if (E instanceof MOB) {
							if (!((MOB) E).isMonster()) {
								if (((MOB) E).getStartRoom() != null)
									((MOB) E).getStartRoom().bringMobHere(
											(MOB) E, false);
								((MOB) E).session().stopSession(false, false,
										false);
			...
			case 14: // mpgoto
			{
				...
				if ((roomID.length() > 0) && (lastKnownLocation != null)) {
					...
					if (roomID.startsWith("$"))
						goHere = CMLib.map().roomLocation(
								this.getArgumentItem(roomID, source, monster,
										scripted, target, primaryItem,
										secondaryItem, msg, tmp));
					if (goHere == null)
						goHere = getRoom(
				...
				if (lastKnownLocation != null) {
					...
					if (roomName.length() > 0) {
						...
						if (roomName.startsWith("$"))
							goHere = CMLib.map().roomLocation(
									this.getArgumentItem(roomName, source,
											monster, scripted, target,
											primaryItem, secondaryItem, msg,
											tmp));
						if (goHere == null)
							goHere = getRoom(
									varify(source, target, scripted, monster,
											primaryItem, secondaryItem, msg,
											tmp, roomName), lastKnownLocation);
						if (goHere != null) {
							goHere.bringMobHere(monster, true);
							DVector DV = new DVector(2);
							DV.addElement("", null);
							...
							lastKnownLocation = goHere;
							execute(scripted, source, target, monster,
									primaryItem, secondaryItem, DV, msg, tmp);
			...
			case 17: // mptransfer
			{
				...
				Room newRoom = null;
				if (roomName.startsWith("$"))
					newRoom = CMLib.map().roomLocation(
							this.getArgumentItem(roomName, source, monster,
									scripted, target, primaryItem,
									secondaryItem, msg, tmp));
				...
				if (roomName.length() > 0) {
					if (newRoom == null)
						newRoom = getRoom(
								varify(source, target, scripted, monster,
										primaryItem, secondaryItem, msg, tmp,
										roomName), lastKnownLocation);
					if (newRoom != null) {
						Vector V = new Vector();
						if (mobName.startsWith("$")) {
							Environmental E = getArgumentItem(mobName, source,
									monster, scripted, target, primaryItem,
									secondaryItem, msg, tmp);
							if (E != null)
								V.addElement(E);
						...
						if (V.size() == 0) {
							mobName = varify(source, target, scripted, monster,
									primaryItem, secondaryItem, msg, tmp,
									mobName);
							if (mobName.equalsIgnoreCase("all")) {
								if (lastKnownLocation != null) {
									for (int x = 0; x < lastKnownLocation
											.numInhabitants(); x++) {
										MOB m = lastKnownLocation
												.fetchInhabitant(x);
										if ((m != null) && (m != monster)
								...
								Area A = null;
								if (lastKnownLocation != null) {
									findOne = lastKnownLocation
											.fetchInhabitant(mobName);
								...
								if ((findOne == null) && (A != null))
									for (Enumeration r = A.getProperMap(); r
											.hasMoreElements();) {
						...
						for (int v = 0; v < V.size(); v++) {
							if (V.elementAt(v) instanceof MOB) {
								...
								// scripting guide calls for NO text -- empty is
								// probably req tho
								CMMsg enterMsg = CMClass.getMsg(
										follower,
										newRoom,
										null,
										CMMsg.MSG_ENTER,
										...
										" "
												+ CMLib.protocol().msp(
														"appear.wav", 10));
								CMMsg leaveMsg = CMClass.getMsg(follower,
										thisRoom, null, CMMsg.MSG_LEAVE, " ");
								if (thisRoom.okMessage(follower, leaveMsg)
										&& newRoom
												.okMessage(follower, enterMsg)) {
									if (follower.isInCombat()) {
										CMLib.commands().postFlee(follower,
												("NOWHERE"));
										follower.makePeace();
									}
									...
									&& (newRoom != CMLib.map().roomLocation(
											(Environmental) V.elementAt(v))))
								newRoom.moveItemTo((Item) V.elementAt(v),
										ItemPossessor.Expire.Player_Drop,
										ItemPossessor.Move.Followers);
			...
			case 25: // mpbeacon
			{
				...
				Room newRoom = null;
				if ((roomName.length() > 0) && (lastKnownLocation != null)) {
					String beacon = varify(source, target, scripted, monster,
							primaryItem, secondaryItem, msg, tmp, tt[2]);
					if (roomName.startsWith("$"))
						newRoom = CMLib.map().roomLocation(
								this.getArgumentItem(roomName, source, monster,
					...
					if ((newRoom != null) && (lastKnownLocation != null)) {
						...
						if (beacon.equalsIgnoreCase("all")) {
							for (int x = 0; x < lastKnownLocation
								...
								if ((m != null) && (m != monster)
										&& (!m.isMonster()) && (!V.contains(m)))
									V.addElement(m);
							}
						} else {
							MOB findOne = lastKnownLocation
									.fetchInhabitant(beacon);
			...
			case 18: // mpforce
			{
				...
				PhysicalAgent newTarget = getArgumentMOB(tt[1], source,
						monster, target, primaryItem, secondaryItem, msg, tmp);
				String force = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]).trim();
				if (newTarget != null) {
					DVector vscript = new DVector(2);
					vscript.addElement(
							"FUNCTION_PROG MPFORCE_"
									+ System.currentTimeMillis()
									+ Math.random(), null);
					vscript.addElement(force, null);
					// this can not be permanently parsed because it is variable
					execute(newTarget, source, target, getMakeMOB(newTarget),
							primaryItem, secondaryItem, vscript, msg, tmp);
				}
				break;
			}
			case 79: // mppossess
			{
				...
				PhysicalAgent newSource = getArgumentMOB(tt[1], source,
						monster, target, primaryItem, secondaryItem, msg, tmp);
				PhysicalAgent newTarget = getArgumentMOB(tt[2], source,
						monster, target, primaryItem, secondaryItem, msg, tmp);
				if ((!(newSource instanceof MOB))
						|| (((MOB) newSource).isMonster()))
					logError(scripted, "MPPOSSESS", "RunTime", tt[1]
							+ " is not a player.");
				else if ((!(newTarget instanceof MOB))
						|| (!((MOB) newTarget).isMonster())
						|| CMSecurity.isASysOp((MOB) newTarget))
					logError(scripted, "MPPOSSESS", "RunTime", tt[2]
							+ " is not a mob.");
				else {
			...
			case 20: // mpsetvar
			{
				...
				String which = tt[1];
				Environmental E = getArgumentItem(which, source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				String arg2 = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				String arg3 = varify(source, target, scripted, monster,
				...
				if (!which.equals("*")) {
					if (E == null)
						which = varify(source, target, scripted, monster,
								primaryItem, secondaryItem, msg, tmp, which);
					else if (E instanceof Room)
			...
			case 36: // mpsavevar
			{
				...
				Environmental E = getArgumentItem(which, source, monster,
						scripted, target, primaryItem, secondaryItem, msg, tmp);
				which = getVarHost(E, which, source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp);
				if ((which.length() > 0) && (arg2.length() > 0)) {
					...
					for (int v = 0; v < V.size(); v++) {
						...
						Hashtable H = (Hashtable) resources
								._getResource("SCRIPTVAR-" + which);
						String val = "";
						if (H != null) {
						...
						if (val.length() > 0)
							CMLib.database().DBReCreateData(
									which,
									"SCRIPTABLEVARS",
									which.toUpperCase() + "_SCRIPTABLEVARS_"
											+ arg2, val);
						else
			...
			case 39: // mploadvar
			{
				...
				if (arg2.length() > 0) {
					...
					which = getVarHost(E, which, source, target, scripted,
							monster, primaryItem, secondaryItem, msg, tmp);
					if (arg2.equals("*"))
						V = CMLib.database()
								.DBReadData(which, "SCRIPTABLEVARS");
					else
						V = CMLib.database()
								.DBReadData(
										which,
										"SCRIPTABLEVARS",
										which.toUpperCase()
												+ "_SCRIPTABLEVARS_" + arg2);
					if ((V != null) && (V.size() > 0))
						for (int v = 0; v < V.size(); v++) {
							DatabaseEngine.PlayerData VAR = V.get(v);
							String varName = VAR.key;
							if (varName.startsWith(which.toUpperCase()
									+ "_SCRIPTABLEVARS_"))
								varName = varName
										.substring((which + "_SCRIPTABLEVARS_")
												.length());
			...
			case 40: // MPM2I2M
			{
				...
				if (E instanceof MOB) {
					...
					logError(scripted, "MPM2I2M", "RunTime", arg1
							+ " is not a mob or a caged item.");
				break;
			}
			case 28: // mpdamage
			{
				...
				if ((newTarget != null) && (arg2.length() > 0)) {
					if (newTarget instanceof MOB) {
						...
						if (arg4.equalsIgnoreCase("MEKILL")
								|| arg4.equalsIgnoreCase("ME"))
							killerM = monster;
						...
						if (max < min)
							max = min;
						if (min > 0) {
							int dmg = (max == min) ? min : CMLib.dice().roll(1,
									max - min, min);
						...
						if (min > 0) {
							...
							if (destroy) {
								if (lastKnownLocation != null)
									lastKnownLocation.showHappens(
											CMMsg.MSG_OK_VISUAL, E.name()
													+ " is destroyed!");
								E.destroy();
							}
						}
				...
				if ((newTarget != null) && (arg2.length() > 0)) {
					if (newTarget instanceof MOB) {
						...
						if (max < min)
							max = min;
						if (min > 0) {
							int amt = (max == min) ? min : CMLib.dice().roll(1,
									max - min, min);
							if (amt > 0)
								CMLib.combat().postHealing(
										healerM,
										healedM,
										null,
										CMMsg.MASK_ALWAYS
												| CMMsg.TYP_CAST_SPELL, amt,
										null);
						}
					} else if (newTarget instanceof Item) {
						Item E = (Item) newTarget;
			...
			case 53: // mpwalkto
			{
				...
				if (A != null) {
					altStatusTickable = A;
					A.invoke(monster, CMParms.parse(arg1 + " LANDONLY"), null,
							true, 0);
					altStatusTickable = null;
				}
				break;
			...
			case 21: // MPENDQUEST
			{
				...
				String q = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1].trim());
				Quest Q = getQuest(q);
				if (Q != null)
					Q.stopQuest();
				else if ((tt[1].length() > 0) && (defaultQuestName != null)
						&& (defaultQuestName.length() > 0)) {
					PhysicalAgent newTarget = getArgumentMOB(tt[1].trim(),
							source, monster, target, primaryItem,
							secondaryItem, msg, tmp);
					if (newTarget == null)
						logError(scripted, "MPENDQUEST", "Unknown",
								"Quest or MOB: " + s);
					else {
			...
			case 69: // MPSTEPQUEST
			{
				...
				String qName = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1].trim());
			...
			case 64: // MPLOADQUESTOBJ
			{
				...
				String questName = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1].trim());
				Quest Q = getQuest(questName);
				if (Q == null) {
					logError(scripted, "MPLOADQUESTOBJ", "Unknown", "Quest: "
							+ questName);
					break;
				}
				Object O = Q.getDesignatedObject(varify(source, target,
						scripted, monster, primaryItem, secondaryItem, msg,
						tmp, tt[2]));
				if (O == null) {
					logError(scripted, "MPLOADQUESTOBJ", "Unknown",
							"Unknown var " + tt[2] + " for Quest: " + questName);
					break;
				}
				String varArg = tt[3];
				if ((varArg.length() != 2) || (!varArg.startsWith("$"))) {
					logError(scripted, "MPLOADQUESTOBJ", "Syntax",
							"Invalid argument var: " + varArg + " for "
									+ scripted.Name());
					break;
				}
			...
			case 22: // MPQUESTWIN
			{
				...
				String whoName = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[1]);
				MOB M = null;
				if (lastKnownLocation != null)
				...
				if (whoName.length() > 0) {
					...
					if (Q != null)
						...
						logError(scripted, "MPQUESTWIN", "Unknown", "Quest: "
								+ s);
				}
				break;
			}
			case 24: // MPCALLFUNC
			{
				...
				for (int v = 0; v < scripts.size(); v++) {
					...
					if (script2.size() < 1)
						continue;
					String trigger = ((String) script2.elementAt(0, 1))
							.toUpperCase().trim();
				...
				if (!found)
					logError(scripted, "MPCALLFUNC", "Unknown", "Function: "
							+ named);
				break;
			}
			case 27: // MPWHILE
			{
				...
				int evalEnd = 2;
				int depth = 0;
				while ((evalEnd < tt.length)
						&& ((!tt[evalEnd].equals(")")) || (depth > 0))) {
				...
				if (evalEnd == tt.length) {
					logError(scripted, "MPWHILE", "Syntax", " no closing ')': "
							+ s);
					break;
				}
				...
				while ((eval(scripted, source, target, monster, primaryItem,
						secondaryItem, msg, tmp, EVALO, 0))
						&& ((System.currentTimeMillis() - time) < 4000))
					execute(scripted, source, target, monster, primaryItem,
							secondaryItem, vscript, msg, tmp);
				if (vscript.elementAt(1, 2) != DOT) {
					int oldDotLen = (DOT == null) ? 1 : DOT.length;
					String[] newDOT = (String[]) vscript.elementAt(1, 2);
					String[] newTT = new String[tt.length - oldDotLen
							+ newDOT.length];
					int end = 0;
				...
				if ((System.currentTimeMillis() - time) >= 4000) {
					logError(scripted, "MPWHILE", "RunTime",
							"4 second limit exceeded: " + s);
					break;
				}
				break;
			}
			case 26: // MPALARM
			{
				...
				DVector vscript = new DVector(2);
				vscript.addElement(
						"FUNCTION_PROG ALARM_" + time + Math.random(), null);
				vscript.addElement(parms, null);
				prequeResponse(scripted, source, target, monster, primaryItem,
						secondaryItem, vscript, CMath.s_int(time.trim()), msg);
				break;
			}
			case 37: // mpenable
			{
				...
				String cast = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[2]);
				String p2 = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[3]);
				String m2 = varify(source, target, scripted, monster,
						primaryItem, secondaryItem, msg, tmp, tt[4]);
				Ability A = null;
				if (cast != null) {
					...
					if (A == null) {
						ExpertiseLibrary.ExpertiseDefinition D = CMLib
								.expertises().findDefinition(cast, false);
						if (D == null)
							logError(scripted, "MPENABLE", "Syntax",
									"Unknown skill/expertise: " + cast);
				...
				if ((newTarget != null) && (A != null)
					...
					if (!((MOB) newTarget).isMonster())
						Log.sysOut("Scripting", newTarget.Name()
								+ " was MPENABLED with " + A.Name());
					if (p2.trim().startsWith("++"))
						p2 = ""
								+ (CMath.s_int(p2.trim().substring(2)) + A
										.proficiency());
					else if (p2.trim().startsWith("--"))
						p2 = ""
								+ (A.proficiency() - CMath.s_int(p2.trim()
										.substring(2)));
			...
			case 38: // mpdisable
			{
				...
				if ((newTarget != null) && (newTarget instanceof MOB)) {
					...
					if ((!((MOB) newTarget).isMonster()) && (A != null))
						Log.sysOut("Scripting", newTarget.Name()
								+ " was MPDISABLED with " + A.Name());
					ExpertiseLibrary.ExpertiseDefinition D = CMLib.expertises()
							.findDefinition(cast, false);
			...
			default:
				if (cmd.length() > 0) {
					Vector<String> V = CMParms.parse(varify(source, target,
							scripted, monster, primaryItem, secondaryItem, msg,
							tmp, s));
	...
	protected List<DVector> getScripts() {
		if (CMSecurity.isDisabled(CMSecurity.DisFlag.SCRIPTABLE)
				|| CMSecurity.isDisabled(CMSecurity.DisFlag.SCRIPTING))
			return empty;
	...
	private Item makeCheapItem(Environmental E) {
		...
		if (E instanceof Item)
			...
			if (E instanceof Physical)
				product.setBasePhyStats((PhyStats) ((Physical) E)
						.basePhyStats().copyOf());
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		...
		for (int v = 0; v < scripts.size(); v++) {
				...
				if (canTrigger(42)) {
					...
					if (x > 0) {
						...
						for (int i = 0; i < x; i++)
							...
							case 'S':
								chk = chk
										&& msg.isSource(command
												.substring(x + 1));
								break;
							case 'T':
								chk = chk
										&& msg.isTarget(command
												.substring(x + 1));
								break;
							case 'O':
								chk = chk
										&& msg.isOthers(command
												.substring(x + 1));
								break;
							default:
								chk = false;
						...
						chk = msg.isSource(command) || msg.isTarget(command)
								|| msg.isOthers(command);
					if (chk) {
						str = "";
						if ((msg.source().session() != null)
								&& (msg.source().session().getPreviousCMD() != null))
							str = " "
									+ CMParms.combine(
											msg.source().session()
													.getPreviousCMD(), 0)
			...
			if (tryIt) {
				...
				if ((monster == null) || (monster.amDead())
						|| (lastKnownLocation == null))
					return true;
				Item defaultItem = (affecting instanceof Item) ? (Item) affecting
						: null;
				Item Tool = null;
				...
				if (msg.target() instanceof MOB)
					resp = execute(affecting, msg.source(), msg.target(),
							monster, Tool, defaultItem, script, str, newObjs());
				else if (msg.target() instanceof Item)
					resp = execute(affecting, msg.source(), msg.target(),
							monster, Tool, (Item) msg.target(), script, str,
							newObjs());
				else
					resp = execute(affecting, msg.source(), msg.target(),
							monster, Tool, defaultItem, script, str, newObjs());
	...
	protected String standardTriggerCheck(DVector script, String[] t,
			Environmental E) {
		if (E == null)
			return null;
		if (t == null)
		...
		if ((t[1].length() == 0)
				|| (t[1].equals("ALL"))
				|| (t[1].equals("P") && (t.length > 2) && ((t[2].indexOf(NAME) >= 0)
						|| (E.ID().equalsIgnoreCase(t[2])) || (t[2]
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		...
		if (msg.amITarget(eventMob) && (!msg.amISource(monster))
		...
		for (int v = 0; v < scripts.size(); v++) {
			...
			case 1: // greet_prog
				if ((msg.targetMinor() == CMMsg.TYP_ENTER)
						...
						&& canTrigger(1)
						&& ((!(affecting instanceof MOB)) || CMLib.flags()
								.canSenseEnteringLeaving(msg.source(),
										(MOB) affecting))) {
					if (t == null)
						t = parseBits(script, 0, "CR");
					int prcnt = CMath.s_int(t[1]);
					if (CMLib.dice().rollPercentage() < prcnt) {
						enqueResponse(affecting, msg.source(), monster,
								monster, defaultItem, null, script, 1, null);
						return;
					}
				}
				break;
			case 45: // arrive_prog
				if (((msg.targetMinor() == CMMsg.TYP_ENTER) || (msg
						.sourceMinor() == CMMsg.TYP_LIFE))
						&& (msg.amISource(eventMob))
						&& (isFreeToBeTriggered(monster) || (!(affecting instanceof MOB)))
						&& canTrigger(45)) {
					if (t == null)
			...
			case 2: // all_greet_prog
				...
				break;
			case 3: // speech_prog
				if (((msg.sourceMinor() == CMMsg.TYP_SPEAK) || (msg
						.targetMinor() == CMMsg.TYP_SPEAK))
						&& canTrigger(3)
						&& (!msg.amISource(monster))
						&& (!msg.othersMajor(CMMsg.MASK_CHANNEL))
						&& (((msg.othersMessage() != null) && ((msg.tool() == null)
								|| (!(msg.tool() instanceof Ability)) || ((((Ability) msg
								.tool()).classificationCode() & Ability.ALL_ACODES) != Ability.ACODE_LANGUAGE))) || ((msg
								.target() == monster)
								&& (msg.targetMessage() != null) && (msg.tool() == null)))
						&& (isFreeToBeTriggered(monster) || (!(affecting instanceof MOB)))) {
					if (t == null)
						t = parseBits(script, 0, "CT");
					String str = null;
					if (msg.othersMessage() != null)
						str = CMStrings.replaceAll(CMStrings
								.getSayFromMessage(msg.othersMessage()
										.toUpperCase()), "`", "'");
					else
						str = CMStrings.replaceAll(CMStrings
								.getSayFromMessage(msg.targetMessage()
										.toUpperCase()), "`", "'");
					str = (" " + str + " ").toUpperCase();
					str = CMStrings.removeColors(str);
					str = CMStrings.replaceAll(str, "\n\r", " ");
					if ((t[1].length() == 0) || (t[1].equals("ALL"))) {
						enqueResponse(affecting, msg.source(), msg.target(),
								monster, defaultItem, null, script, 1, str);
						return;
					} else if ((t[1].equals("P")) && (t.length > 2)) {
						if (match(str.trim(), t[2])) {
							enqueResponse(affecting, msg.source(),
									msg.target(), monster, defaultItem, null,
									script, 1, str);
							return;
						}
					} else
						for (int i = 1; i < t.length; i++) {
							...
							if (x >= 0) {
								enqueResponse(affecting, msg.source(),
										msg.target(), monster, defaultItem,
										null, script, 1, str.substring(x)
												.trim());
								return;
							}
						}
			...
			case 4: // give_prog
				if ((msg.targetMinor() == CMMsg.TYP_GIVE)
						...
						&& ((msg.amITarget(monster))
								|| (msg.tool() == affecting)
								|| (affecting instanceof Room) || (affecting instanceof Area))
						&& (!msg.amISource(monster))
					...
					if (check != null) {
						...
						lastMsg = msg;
						enqueResponse(affecting, msg.source(), monster,
								monster, (Item) msg.tool(), defaultItem,
								script, 1, check);
						return;
					}
				}
			...
			case 40: // llook_prog
				if ((msg.targetMinor() == CMMsg.TYP_EXAMINE)
					...
					if (check != null) {
						enqueResponse(affecting, msg.source(), msg.target(),
								monster, (Item) msg.target(), defaultItem,
								script, 1, check);
						return;
					}
				}
				...
				if (canTrigger(41)) {
					...
					if (chk) {
						...
						if (doIt) {
							...
							if (msg.target() instanceof MOB)
								enqueResponse(affecting, msg.source(),
										msg.target(), monster, Tool,
										defaultItem, script, 1, str);
							else if (msg.target() instanceof Item)
								enqueResponse(affecting, msg.source(),
										msg.target(), monster, Tool,
										(Item) msg.target(), script, 1, str);
							else
								enqueResponse(affecting, msg.source(),
										msg.target(), monster, Tool,
										defaultItem, script, 1, str);
			...
			case 39: // look_prog
				if ((msg.targetMinor() == CMMsg.TYP_LOOK)
					...
					if (check != null) {
						enqueResponse(affecting, msg.source(), msg.target(),
								monster, defaultItem, defaultItem, script, 1,
								check);
						return;
					}
				}
			...
			case 20: // get_prog
				if ((msg.targetMinor() == CMMsg.TYP_GET)
						...
						&& (msg.amITarget(affecting)
								...
								|| (affecting instanceof Area) || (affecting instanceof MOB))
						&& (!msg.amISource(monster))
				...
				if ((msg.targetMinor() == CMMsg.TYP_DROP)
					...
					if (check != null) {
						...
						if (msg.target() instanceof Coins)
							execute(affecting, msg.source(), msg.target(),
									monster, (Item) msg.target(),
									(Item) ((Item) msg.target()).copyOf(),
									script, check, newObjs());
						else
							enqueResponse(affecting, msg.source(),
									msg.target(), monster, (Item) msg.target(),
									defaultItem, script, 1, check);
						return;
					}
				}
				break;
			...
			case 37: // unlock_prog
			{
				...
				if ((msg.targetMinor() == targetMinorTrigger)
						&& canTrigger(triggerCode)
						&& ((msg.amITarget(affecting))
								|| (affecting instanceof Room)
								|| (affecting instanceof Area) || (affecting instanceof MOB))
						&& (!msg.amISource(monster))
						&& (isFreeToBeTriggered(monster) || (!(affecting instanceof MOB)))) {
					Item I = (msg.target() instanceof Item) ? (Item) msg
							.target() : defaultItem;
					String check = standardTriggerCheck(script, t, msg.target());
					if (check != null) {
						enqueResponse(affecting, msg.source(), msg.target(),
								monster, I, defaultItem, script, 1, check);
						return;
					}
				}
				break;
			...
			case 27: // buy_prog
				if ((msg.targetMinor() == CMMsg.TYP_BUY)
						&& canTrigger(27)
						&& ((!(affecting instanceof ShopKeeper)) || msg
								.amITarget(affecting))
						&& (!msg.amISource(monster))
						&& (isFreeToBeTriggered(monster) || (!(affecting instanceof MOB)))) {
					String check = standardTriggerCheck(script, t, msg.tool());
					if (check != null) {
						...
						if ((product instanceof Coins)
							...
							execute(affecting, msg.source(), monster, monster,
									product, (Item) product.copyOf(), script,
									check, newObjs());
						else
							enqueResponse(affecting, msg.source(), monster,
									monster, product, product, script, 1, check);
						return;
					}
				}
				break;
			...
			case 23: // wear_prog
				if (((msg.targetMinor() == CMMsg.TYP_WEAR)
						|| (msg.targetMinor() == CMMsg.TYP_HOLD) || (msg
						.targetMinor() == CMMsg.TYP_WIELD))
						&& ((msg.amITarget(affecting))
								|| (affecting instanceof Room)
								|| (affecting instanceof Area) || (affecting instanceof MOB))
						&& (!msg.amISource(monster))
			...
			case 19: // bribe_prog
				if ((msg.targetMinor() == CMMsg.TYP_GIVE)
					...
					if (t[1].startsWith("ANY") || t[1].startsWith("ALL"))
						t[1] = t[1].trim();
					else if (!((Coins) msg.tool()).getCurrency().equals(
							CMLib.beanCounter().getCurrency(monster)))
						break;
					double d = 0.0;
			...
			case 8: // entry_prog
				if ((msg.targetMinor() == CMMsg.TYP_ENTER)
						...
						&& (msg.amISource(eventMob)
								...
								|| (msg.tool() == affecting) || (affecting instanceof Item))
						&& (isFreeToBeTriggered(monster) || (!(affecting instanceof MOB)))) {
					if (t == null)
						t = parseBits(script, 0, "CR");
					int prcnt = CMath.s_int(t[1]);
					if (CMLib.dice().rollPercentage() < prcnt) {
						...
						String roomID = null;
						if (msg.target() != null)
							roomID = CMLib.map().getExtendedRoomID(
									CMLib.map().roomLocation(msg.target()));
						for (int q = 0; q < V.size(); q++) {
							SB = V.get(q);
							if ((SB.scr == script) && (SB.s == msg.source())) {
								if (que.remove(SB))
									execute(SB.h, SB.s, SB.t, SB.m, SB.pi,
											SB.si, SB.scr, SB.message,
											newObjs());
								break;
							}
						}
						enqueResponse(affecting, msg.source(), monster,
								monster, defaultItem, null, script, 1, roomID);
						return;
					}
				}
				break;
			...
			case 10: // death prog
				if ((msg.sourceMinor() == CMMsg.TYP_DEATH)
					...
					if ((src == null) || (src.location() != monster.location()))
						src = ded;
					execute(affecting, src, ded, ded, defaultItem, null,
							script, null, newObjs());
					return;
				}
				break;
			...
			case 26: // damage prog
				if ((msg.targetMinor() == CMMsg.TYP_DAMAGE)
					...
					if (msg.tool() instanceof Item)
						I = (Item) msg.tool();
					execute(affecting, msg.source(), msg.target(), eventMob,
							defaultItem, I, script, "" + msg.value(), newObjs());
					return;
				}
				break;
			case 29: // login_prog
				if (!registeredEvents
						.contains(Integer.valueOf(CMMsg.TYP_LOGIN))) {
			...
			case 32: // level_prog
				if (!registeredEvents
						.contains(Integer.valueOf(CMMsg.TYP_LEVEL))) {
			...
			case 43: // imask_prog
				if ((triggerCode != 43)
						|| (msg.amISource(monster) && canTrigger(43))) {
					if (t == null) {
						t = parseBits(script, 0, "CT");
						for (int i = 1; i < t.length; i++)
							t[i] = CMLib.english().stripPunctuation(
									CMStrings.removeColors(t[i]));
					}
					boolean doIt = false;
					...
					if (str == null)
						break;
					str = CMLib.coffeeFilter().fullOutFilter(null, monster,
							msg.source(), msg.target(), msg.tool(), str, false);
					str = CMLib.english().stripPunctuation(
							CMStrings.removeColors(str));
					str = " "
							+ CMStrings.replaceAll(str, "\n\r", " ")
									.toUpperCase().trim() + " ";
					if ((t[1].length() == 0) || (t[1].equals("ALL")))
						doIt = true;
					else if ((t[1].equals("P")) && (t.length > 2)) {
					...
					if (doIt) {
						...
						if (msg.target() instanceof MOB)
							enqueResponse(affecting, msg.source(),
									msg.target(), monster, Tool, defaultItem,
									script, 1, str);
						else if (msg.target() instanceof Item)
							enqueResponse(affecting, msg.source(),
									msg.target(), monster, Tool,
				...
				break;
			case 38: // social prog
				if (!msg.amISource(monster) && canTrigger(38)
						&& (msg.tool() instanceof Social)) {
					if (t == null)
						t = parseBits(script, 0, "CR");
					if (((Social) msg.tool()).Name().toUpperCase()
							.startsWith(t[1])) {
						Item Tool = defaultItem;
						if (msg.target() instanceof MOB)
							enqueResponse(affecting, msg.source(),
									msg.target(), monster, Tool, defaultItem,
									script, 1, msg.tool().Name());
						else if (msg.target() instanceof Item)
							enqueResponse(affecting, msg.source(),
									msg.target(), monster, Tool,
									(Item) msg.target(), script, 1, msg.tool()
											.Name());
						else
							enqueResponse(affecting, msg.source(),
									msg.target(), monster, Tool, defaultItem,
			...
			case 33: // channel prog
				if (!registeredEvents.contains(Integer
						.valueOf(CMMsg.TYP_CHANNEL))) {
				...
				if (!msg.amISource(monster)
					...
					String str = null;
					if (channel.equalsIgnoreCase(CMLib.channels().getChannel(
							channelInt).name)) {
						str = msg.sourceMessage();
						if (str == null)
						...
						str = CMLib
								.coffeeFilter()
								.fullOutFilter(null, monster, msg.source(),
										msg.target(), msg.tool(), str, false)
								.toUpperCase().trim();
						int dex = str.indexOf("[" + channel + "]");
						if (dex > 0)
							str = str.substring(dex + 2 + channel.length())
									.trim();
						else {
							dex = str.indexOf('\'');
			...
			case 31: // regmask prog
				if (!msg.amISource(monster) && canTrigger(31)) {
					...
					if (CMParms.getCleanBit(t[1], 0).equalsIgnoreCase("p"))
						...
						if (P == null) {
							P = Pattern.compile(t[1], Pattern.CASE_INSENSITIVE
									| Pattern.DOTALL);
							patterns.put(t[1], P);
						}
	...
	public MOB getMakeMOB(Tickable ticking) {
		...
		if (ticking instanceof MOB) {
			...
			if ((backupMOB == null) || (backupMOB.amDestroyed())
					|| (backupMOB.amDead())) {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		PhysicalAgent affecting = (ticking instanceof PhysicalAgent) ? ((PhysicalAgent) ticking)
				: null;
		...
		for (int thisScriptIndex = 0; thisScriptIndex < scripts.size(); thisScriptIndex++) {
			...
			case 5: // rand_Prog
				if ((!mob.amDead()) && canTrigger(5)) {
					...
					int prcnt = CMath.s_int(t[1]);
					if (CMLib.dice().rollPercentage() < prcnt)
						execute(affecting, mob, mob, mob, defaultItem, null,
								script, null, newObjs());
				}
				break;
			case 16: // delay_prog
				if ((!mob.amDead()) && canTrigger(16)) {
					int targetTick = -1;
					if (delayTargetTimes.containsKey(Integer
							.valueOf(thisScriptIndex)))
						targetTick = delayTargetTimes.get(
								Integer.valueOf(thisScriptIndex)).intValue();
					else {
						if (t == null)
						...
						if (high < low)
							high = low;
						targetTick = CMLib.dice().roll(1, high - low + 1,
								low - 1);
						delayTargetTimes.put(Integer.valueOf(thisScriptIndex),
								Integer.valueOf(targetTick));
					}
					int delayProgCounter = 0;
					if (delayProgCounters.containsKey(Integer
							.valueOf(thisScriptIndex)))
						delayProgCounter = delayProgCounters.get(
								Integer.valueOf(thisScriptIndex)).intValue();
					else
						delayProgCounters.put(Integer.valueOf(thisScriptIndex),
								Integer.valueOf(0));
					...
					delayProgCounters.put(Integer.valueOf(thisScriptIndex),
							Integer.valueOf(delayProgCounter + 1));
				}
				break;
			case 7: // fightProg
				if ((mob.isInCombat()) && (!mob.amDead()) && canTrigger(7)) {
					...
					int prcnt = CMath.s_int(t[1]);
					if (CMLib.dice().rollPercentage() < prcnt)
						execute(affecting, mob.getVictim(), mob, mob,
								defaultItem, null, script, null, newObjs());
				} else if ((ticking instanceof Item) && canTrigger(7)
					...
					if (CMLib.dice().rollPercentage() < prcnt) {
						...
						if (!M.amDead())
							execute(affecting, M, mob.getVictim(), mob,
									defaultItem, null, script, null, newObjs());
					}
				}
				break;
			case 11: // hitprcnt
				if ((mob.isInCombat()) && (!mob.amDead()) && canTrigger(11)) {
					if (t == null)
						t = parseBits(script, 0, "CR");
					int prcnt = CMath.s_int(t[1]);
					int floor = (int) Math.round(CMath.mul(CMath.div(prcnt,
							100.0), mob.maxState().getHitPoints()));
					if (mob.curState().getHitPoints() <= floor)
						execute(affecting, mob.getVictim(), mob, mob,
								defaultItem, null, script, null, newObjs());
				} else if ((ticking instanceof Item) && canTrigger(11)
						&& (((Item) ticking).owner() instanceof MOB)
						&& (((MOB) ((Item) ticking).owner()).isInCombat())) {
					MOB M = (MOB) ((Item) ticking).owner();
					if (!M.amDead()) {
			...
			case 6: // once_prog
				...
				break;
			case 14: // time_prog
				if ((mob.location() != null) && canTrigger(14)
						&& (!mob.amDead())) {
					if (t == null)
						t = parseBits(script, 0, "CT");
					int lastTimeProgDone = -1;
					if (lastTimeProgsDone.containsKey(Integer
							.valueOf(thisScriptIndex)))
						lastTimeProgDone = lastTimeProgsDone.get(
								Integer.valueOf(thisScriptIndex)).intValue();
					int time = mob.location().getArea().getTimeObj()
							.getTimeOfDay();
					if (lastTimeProgDone != time) {
						...
						for (int i = 1; i < t.length; i++) {
							if (time == CMath.s_int(t[i])) {
								...
								execute(affecting, mob, mob, mob, defaultItem,
										null, script, "" + time, newObjs());
								lastTimeProgsDone.remove(Integer
										.valueOf(thisScriptIndex));
								lastTimeProgsDone.put(
										Integer.valueOf(thisScriptIndex),
			...
			case 15: // day_prog
				if ((mob.location() != null) && canTrigger(15)
					...
					if (lastDayProgsDone.containsKey(Integer
							.valueOf(thisScriptIndex)))
						lastDayProgDone = lastDayProgsDone.get(
								Integer.valueOf(thisScriptIndex)).intValue();
					int day = mob.location().getArea().getTimeObj()
							.getDayOfMonth();
					if (lastDayProgDone != day) {
						...
						for (int i = 1; i < t.length; i++) {
							if (day == CMath.s_int(t[i])) {
								...
								execute(affecting, mob, mob, mob, defaultItem,
										null, script, null, newObjs());
								lastDayProgsDone.remove(Integer
										.valueOf(thisScriptIndex));
								lastDayProgsDone.put(
										Integer.valueOf(thisScriptIndex),
										Integer.valueOf(day));
								break;
							}
	...
	public void enqueResponse(PhysicalAgent host, MOB source,
			Environmental target, MOB monster, Item primaryItem,
			Item secondaryItem, DVector script, int ticks, String msg) {
		if (noDelay)
			execute(host, source, target, monster, primaryItem, secondaryItem,
					script, msg, newObjs());
		else
			que.add(new ScriptableResponse(host, source, target, monster,
					primaryItem, secondaryItem, script, ticks, msg));
	}

	public void prequeResponse(PhysicalAgent host, MOB source,
			Environmental target, MOB monster, Item primaryItem,
			Item secondaryItem, DVector script, int ticks, String msg) {
		que.add(0, new ScriptableResponse(host, source, target, monster,
				primaryItem, secondaryItem, script, ticks, msg));
	}
		...
		try {
			...
			for (int q = que.size() - 1; q >= 0; q--) {
				...
				if (SB.checkTimeToExecute()) {
					execute(SB.h, SB.s, SB.t, SB.m, SB.pi, SB.si, SB.scr,
							SB.message, newObjs());
					que.remove(SB);
				}
			}
	...
	protected static class JScriptEvent extends ScriptableObject {
		...
		public Object get(final String name, Scriptable start) {
			...
			if (methH.containsKey(name)
					|| funcH.containsKey(name)
					|| (name.endsWith("$") && (funcH.containsKey(name
				...
				return new Function() {
					...
					public Object call(Context cx, Scriptable scope,
						...
						if (methH.containsKey(name)) {
							...
							return c.execute(h, s, t, m, pi, si, DV, message,
									objs);
						}
						if (name.endsWith("$")) {
									...
									if (i == args.length - 1)
										...
										strb.append(" ").append(
												"'" + String.valueOf(args[i])
														+ "'");
							strb.append(" ) ");
							return c.functify(h, s, t, m, pi, si, message,
									objs, strb.toString());
						}
						...
						return Boolean.valueOf(c.eval(h, s, t, m, pi, si,
								message, objs, EVAL, 0));
					}
		...
		public JScriptEvent(DefaultScriptingEngine scrpt, PhysicalAgent host,
				MOB source, Environmental target, MOB monster,
				Item primaryItem, Item secondaryItem, String msg, Object[] tmp) {
			c = scrpt;
			h = host;
			s = source;
