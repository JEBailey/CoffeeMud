...
public class DefaultClanGovernment implements ClanGovernment {
	...
	public int ID;
	/**
	 * The name of this government type, which is its identifier when ID above
	 * is -1
	 */
	public String name;
	/**
	 * The category of this government type. Players can only belong to one of
	 * each category
	 */
	...
	/**
	 * The number of minimum members for the clan to survive -- overrides
	 * coffeemud.ini
	 */
	...
	/**
	 * minimum % of voters who must have voted for a vote to be valid if time
	 * expires
	 */
	...
	/**
	 * The list of ClanPosition objects for each holdable position in this
	 * government
	 */
	public ClanPosition[] positions;
	/**
	 * Whether an unfilled topRole is automatically filled by those who meet its
	 * innermask
	 */
	public Clan.AutoPromoteFlag autoPromoteBy;

	// derived variable
	public static final SearchIDList<Ability> emptyIDs = new CMUniqSortSVec<Ability>(
			1);
	public static final List<Ability> empty = new ReadOnlyList<Ability>(
			new Vector<Ability>());
	public static final String DEFAULT_XP_FORMULA = "(500 * @x1) + (1000 * @x1 * @x1 * @x1)";
	public LinkedList<CMath.CompiledOperation> xpCalculationFormula = CMath
			.compileMathExpression(DEFAULT_XP_FORMULA);

	// derived and internal vars
	protected Map<Integer, SearchIDList<Ability>> clanAbilityMap = null;
	protected Map<Integer, SearchIDList<Ability>> clanEffectMap = null;
	...
	public void setXpCalculationFormulaStr(String newXpCalculationFormula) {
		...
		if (xpCalculationFormulaStr.trim().length() == 0)
			this.xpCalculationFormula = CMath
					.compileMathExpression(DEFAULT_XP_FORMULA);
		else
			try {
				this.xpCalculationFormula = CMath
						.compileMathExpression(xpCalculationFormulaStr);
			} catch (Exception e) {
	...
	private static enum GOVT_STAT_CODES {
		NAME, AUTOROLE, ACCEPTPOS, SHORTDESC, REQUIREDMASK, ISPUBLIC, ISFAMILYONLY, OVERRIDEMINMEMBERS, CONQUESTENABLED, CONQUESTITEMLOYALTY, CONQUESTDEITYBASIS, MAXVOTEDAYS, VOTEQUORUMPCT, AUTOPROMOTEBY, VOTEFUNCS, LONGDESC, XPLEVELFORMULA, NUMRABLE, GETRABLE, GETRABLEPROF, GETRABLEQUAL, GETRABLELVL, NUMREFF, GETREFF, GETREFFPARM, GETREFFLVL, CATEGORY, ISRIVALROUS, ENTRYSCRIPT, EXITSCRIPT
	}
	...
	private GOVT_STAT_CODES getStatIndex(String code) {
		return (GOVT_STAT_CODES) CMath
				.s_valueOf(GOVT_STAT_CODES.values(), code);
	}
	...
	public String getStat(String code) {
		...
		case NAME:
			return name;
		case AUTOROLE:
			return (autoRole < 0 || autoRole > positions.length) ? ""
					: positions[autoRole].getID();
		case ACCEPTPOS:
			return (acceptPos < 0 || acceptPos > positions.length) ? ""
					: positions[acceptPos].getID();
		case SHORTDESC:
			return shortDesc;
		case LONGDESC:
			return longDesc;
		case XPLEVELFORMULA:
			return xpCalculationFormulaStr == null ? ""
		...
		case OVERRIDEMINMEMBERS:
			return overrideMinMembers == null ? "" : overrideMinMembers
					.toString();
		...
		case NUMRABLE:
			return (clanAbilityNames == null) ? "0"
					: ("" + clanAbilityNames.length);
		case GETRABLE:
			return (clanAbilityNames == null) ? ""
					: ("" + clanAbilityNames[num]);
		case GETRABLEPROF:
			return (clanAbilityProficiencies == null) ? "0"
					: ("" + clanAbilityProficiencies[num]);
		case GETRABLEQUAL:
			return (clanAbilityQuals == null) ? "false"
					: ("" + clanAbilityQuals[num]);
		case GETRABLELVL:
			return (clanAbilityLevels == null) ? "0"
					: ("" + clanAbilityLevels[num]);
		case NUMREFF:
			return (clanEffectNames == null) ? "0"
					: ("" + clanEffectNames.length);
		case GETREFF:
			return (clanEffectNames == null) ? "" : ("" + clanEffectNames[num]);
		case GETREFFPARM:
			return (clanEffectParms == null) ? "0"
					: ("" + clanEffectParms[num]);
		case GETREFFLVL:
			return (clanEffectLevels == null) ? "0"
					: ("" + clanEffectLevels[num]);
		case CATEGORY:
			return category;
		default:
	...
	public void setStat(String code, String val) {
		...
		case EXITSCRIPT:
			...
			break;
		case AUTOPROMOTEBY: {
			Clan.AutoPromoteFlag flag = (Clan.AutoPromoteFlag) CMath.s_valueOf(
					Clan.AutoPromoteFlag.values(), val);
			if (flag != null)
				autoPromoteBy = flag;
			break;
		}
		case VOTEFUNCS: {
			final List<String> funcs = CMParms.parseCommas(val.toUpperCase()
					.trim(), true);
			for (ClanPosition pos : positions) {
				for (int a = 0; a < Function.values().length; a++)
					if (pos.getFunctionChart()[a] == Authority.MUST_VOTE_ON)
				...
				for (final String funcName : funcs) {
					Clan.Function func = (Clan.Function) CMath.s_valueOf(
							Function.values(), funcName);
					if (func != null)
	...
	public String getHelpStr() {
		...
		if (helpStr == null) {
			...
			while (sortedPositions.size() < showablePositions.size()) {
				...
				for (ClanPosition P : showablePositions)
					if ((!sortedPositions.contains(P))
							&& ((highPos == null) || (highPos.getRank() < P
									.getRank())))
						highPos = P;
			...
			while ((funcMaxLen > funcMaxAvg)
					&& ((funcMaxAvg + posTotalLen) > 78))
				funcMaxLen--;
			if (posses.length > 0)
			...
			str.append(CMStrings.padRight("Command", commandColLen - 1))
					.append("!");
			for (int p = 0; p < posses.length; p++) {
				ClanPosition pos = sortedPositions.get(p);
				String name = CMStrings.capitalizeAndLower(pos.getName()
						.replace('_', ' '));
			...
			for (Clan.Function func : Clan.Function.values()) {
				String fname = CMStrings.capitalizeAndLower(func.toString()
						.replace('_', ' '));
				str.append(CMStrings.padRight(fname, commandColLen - 1))
						.append("!");
				for (int p = 0; p < sortedPositions.size(); p++) {
					ClanPosition pos = sortedPositions.get(p);
			...
			if ((clanAbilityLevels != null) && (clanEffectLevels != null)
				...
				int maxLevel = -1;
				for (int x : clanEffectLevels)
					if (x > maxLevel)
						maxLevel = x;
				for (int x : clanAbilityLevels)
					if (x > maxLevel)
						maxLevel = x;
				for (int l = 1; l <= maxLevel; l++) {
					...
					for (int x = 0; x < clanAbilityLevels.length; x++)
						if (clanAbilityLevels[x] == l) {
							final Ability A = CMClass
									.getAbility(clanAbilityNames[x]);
							if (A != null) {
								if (clanAbilityQuals[x])
									levelBenefits.add("Members qualify for: "
											+ A.name());
								else
									levelBenefits
											.add("Members automatically gain: "
													+ A.name());
							}
						}
	...
	public SearchIDList<Ability> getClanLevelAbilities(Integer level) {
		...
		if ((clanAbilityMap == null) && (clanAbilityNames != null)
				&& (clanAbilityLevels != null)
				&& (clanAbilityProficiencies != null)
				&& (clanAbilityQuals != null)) {
			...
			for (int i = 0; i < clanAbilityNames.length; i++) {
				...
				if (A != null) {
					CMLib.ableMapper().addDynaAbilityMapping(clanGvtID,
							clanAbilityLevels[i], A.ID(),
							clanAbilityProficiencies[i], "",
							!clanAbilityQuals[i], false);
				}
			}
		}
		...
		List<AbilityMapper.AbilityMapping> V = CMLib.ableMapper()
				.getUpToLevelListings(clanGvtID, level.intValue(), true, true);
		...
		for (AbilityMapper.AbilityMapping able : V) {
			...
			if (A != null) {
				A.setProficiency(CMLib.ableMapper().getDefaultProficiency(
						clanGvtID, false, A.ID()));
				A.setSavable(false);
				A.setMiscText(CMLib.ableMapper().getDefaultParm(clanGvtID,
						false, A.ID()));
				finalV.add(A);
			}
		}
	...
	public List<Ability> getClanLevelEffectsList(final MOB mob,
			final Integer level) {
		if (clanEffectNames == null)
			return empty;

		if ((clanEffectMap == null) && (clanEffectNames != null)
				&& (clanEffectLevels != null) && (clanEffectParms != null))
				...
				new ChameleonList.Signaler<Ability>(empty) {
					...
					public void rebuild(final ChameleonList<Ability> me) {
						...
						if (mob.getClanRole(clan.clanID()) != null)
							me.changeMeInto(clan
									.getGovernment()
									.getClanLevelEffects(
											mob,
											clan,
											Integer.valueOf(clan.getClanLevel())));
					}
				});
	}

	public ChameleonList<Ability> getClanLevelEffects(final MOB mob,
			final Clan clan, final Integer level) {
		if (level == null)
					...
					new ChameleonList.Signaler<Ability>(myList) {
						public boolean isDeprecated() {
							if ((mob.amDestroyed()) || (clan == null)
									|| (mob.getClanRole(clan.clanID()) == null))
								return true;
							if ((clan.getGovernment() != myGovt)
									|| (getClanLevelEffectsList(mob, Integer
											.valueOf(clan.getClanLevel())) != oldReferenceListRef
											.get()))
								return true;
							return false;
						}
						...
						public void rebuild(final ChameleonList<Ability> me) {
							...
							if ((mob.amDestroyed()) || (C == null))
								...
								me.changeMeInto(C.getGovernment()
										.getClanLevelEffects(
												mob,
												clan,
												Integer.valueOf(C
