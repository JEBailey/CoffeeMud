...
public class DefaultLawSet implements Law {
	...
	public void propertyTaxTick(Area A, boolean debugging) {
		if (lastMonthChecked != A.getTimeObj().getMonth()) {
			...
			List<LandTitle> titles = CMLib.law().getAllUniqueTitles(
					A.getMetroMap(), "*", false);
			...
			for (String owner : owners.keySet()) {
				...
				if (owed > 0)
					for (int p = 0; p < particulars.size(); p++) {
						...
						if (T.backTaxes() < 0) {
							if ((-T.backTaxes()) >= owed) {
								paid += owed;
								T.setBackTaxes((int) Math.round((T.backTaxes())
										+ owed));
								T.updateTitle();
								break;
							}
				...
				if (owed > 0) {
					owed -= paid;
					if ((owed > 0)
							&& (!CMLib.beanCounter().modifyLocalBankGold(
									A,
									owner,
									CMLib.utensils().getFormattedDate(A)
											...
											+ ": Taxes on property: "
											+ properties.toString(),
									CMLib.beanCounter().getCurrency(A), -owed))) {
						boolean owesButNotConfiscated = false;
						for (int p = 0; p < particulars.size(); p++) {
							T = particulars.elementAt(p);
							double owedOnThisLand = CMath
									.mul(T.getPrice(), tax);
							...
							if (owedOnThisLand > 0) {
								...
								if ((T.getPrice() / T.backTaxes()) < 4) {
									Clan clanC = CMLib.clans().getClan(
											T.getOwnerName());
									if (clanC != null) {
										List<Pair<Clan, Integer>> clanSet = new Vector<Pair<Clan, Integer>>();
										clanSet.add(new Pair<Clan, Integer>(C,
												Integer.valueOf(0)));
										...
										for (int i = 0; i < channels.size(); i++)
											CMLib.commands()
													.postChannel(
															...
															clanSet,
															T.getOwnerName()
																	+ " has lost the title to "
																	+ T.landPropertyID()
																	+ " due to failure to pay property taxes.",
															false);
									} else if (CMLib.players().getPlayer(
											T.getOwnerName()) != null)
										CMLib.players()
												.getPlayer(T.getOwnerName())
												.tell("You have lost the title to "
														+ T.landPropertyID()
														+ " due to failure to pay property taxes.");
									T.setOwnerName("");
									T.updateTitle();
						...
						if ((owesButNotConfiscated)
								...
								&& (responsibleMob != null))
							legalDetails.fillOutWarrant(responsibleMob, this,
									A, null, evasionBits[Law.BIT_CRIMELOCS],
						...
						if ((treasuryR != null) && ((owed + paid) > 0)) {
							List<Coins> V = CMLib.beanCounter()
									.makeAllCurrency(
						...
						if ((evasionBits != null)
								&& (evasionBits[Law.BIT_CRIMENAME].length() > 0)
								&& (responsibleMob != null))
							while (getWarrant(responsibleMob,
									evasionBits[Law.BIT_CRIMENAME], true,
									debugging) != null) {
							}
					}
				}
	...
	public String getMessage(int which) {
		if ((which >= 0) && (which < messages.length)
				&& (messages[which] != null))
	...
	public String paroleMessages(int which) {
		if ((which >= 0) && (which < paroleMessages.length)
	...
	public int paroleTimes(int which) {
		if ((which >= 0) && (which < paroleTimes.length)
	...
	public String jailMessages(int which) {
		if ((which >= 0) && (which < jailMessages.length)
	...
	public int jailTimes(int which) {
		if ((which >= 0) && (which < jailTimes.length)
	...
	private void resetLaw(Properties laws) {
		...
		messages[Law.MSG_COPKILLER] = isInternalStr("COPKILLERMSG") ? getInternalStr("COPKILLERMSG")
				: "COPKILLER!!!! ARGH!!!!!!";
		...
		paroleTimes[0] = Integer.valueOf(CMath
				.s_int(getInternalStr("PAROLE1TIME")));
		paroleTimes[1] = Integer.valueOf(CMath
				.s_int(getInternalStr("PAROLE2TIME")));
		paroleTimes[2] = Integer.valueOf(CMath
				.s_int(getInternalStr("PAROLE3TIME")));
		paroleTimes[3] = Integer.valueOf(CMath
				.s_int(getInternalStr("PAROLE4TIME")));
		...
		jailTimes[0] = Integer
				.valueOf(CMath.s_int(getInternalStr("JAIL1TIME")));
		jailTimes[1] = Integer
				.valueOf(CMath.s_int(getInternalStr("JAIL2TIME")));
		jailTimes[2] = Integer
				.valueOf(CMath.s_int(getInternalStr("JAIL3TIME")));
		jailTimes[3] = Integer
				.valueOf(CMath.s_int(getInternalStr("JAIL4TIME")));

		jailRooms = CMParms.parseSemicolons(getInternalStr("JAIL"), true);
		releaseRooms = CMParms.parseSemicolons(getInternalStr("RELEASEROOM"),
				true);
		...
		for (Enumeration<Object> e = laws.keys(); e.hasMoreElements();) {
			...
			if (x >= 0) {
				if (key.startsWith("CRIME")) {
					...
					List<String> parsed = CMParms.parseSemicolons(
							words.substring(x + 1), false);
					...
					for (int i = 0; i < Law.BIT_NUMBITS; i++)
						if (i < parsed.size())
							...
							bits[i] = "";
					bannedBits.add(bits);
				} else if ((key.startsWith("$") && (CMClass.getAbility(key
						.substring(1)) != null))
						|| (CMClass.getAbility(key) != null))
					abilityCrimes
							.put(key.toUpperCase(), getInternalBits(words));
			}
		}
	}
