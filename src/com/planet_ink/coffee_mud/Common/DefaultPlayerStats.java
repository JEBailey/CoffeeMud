...
public class DefaultPlayerStats implements PlayerStats {
	...
	protected static String[] CODES = { "CLASS", "FRIENDS", "IGNORE", "TITLES",
			"ALIAS", "LASTIP", "LASTDATETIME", "CHANNELMASK", "COLORSTR",
			"PROMPT", "POOFIN", "POOFOUT", "TRANPOOFIN", "TRAINPOOFOUT",
			"ANNOUNCEMSG", "NOTES", "WRAP", "BIRTHDAY", "ACCTEXPIRATION",
			"INTRODUCTIONS", "PAGEBREAK", "SAVEDPOSE", "THEME", "LEGLEVELS" };
	...
	public String getStat(String code) {
		...
		case 16:
			return "" + wrap;
		case 17:
			return CMParms.toStringList(birthday);
		case 18:
	...
	public CMObject copyOf() {
		try {
			...
			O.xtraValues = (xtraValues == null) ? null : (String[]) xtraValues
					.clone();
	...
	public boolean matchesPassword(String checkPass) {
		...
		if (CMLib.encoder().isARandomHashString(password))
			return CMLib.encoder().checkAgainstRandomHashString(checkPass,
					password);
	...
	private RoomnumberSet roomSet() {
		if (visitedRoomSet == null)
			visitedRoomSet = ((RoomnumberSet) CMClass
	...
	public String getAliasXML() {
		...
		for (String key : alias.keySet())
			str.append("<ALIAS CMD=\"")
					.append(key)
					.append("\" VAL=\"")
					.append(CMLib.xml().parseOutAngleBracketsAndQuotes(
							alias.get(key))).append("\" />");
		return str.toString();
	}
	...
	public String getLegacyXML() {
		...
		for (String key : legacy.keySet())
			str.append("<LEGACY CAT=\"").append(key).append("\" LVL=\"")
					.append(legacy.get(key)).append("\" />");
		return str.toString();
	}
		...
		for (int t = 0; t < titles.size(); t++) {
			...
			str.append("<TITLE>"
					+ CMLib.xml().parseOutAngleBrackets(
							CMLib.coffeeFilter().safetyFilter(s)) + "</TITLE>");
		}
		return str.toString();
	}
	...
	public int initializeBirthday(int ageHours, Race R) {
		...
		if ((month < birthday[1])
				|| ((month == birthday[1]) && (birthday[0] < day)))
	...
	public String getXML() {
		...
		for (int x = getSaveStatIndex(); x < codes.length; x++) {
			...
			rest.append("<" + code + ">"
					+ CMLib.xml().parseOutAngleBrackets(getStat(code)) + "</"
					+ code + ">");
		}
		...
		return ((f.length() > 0) ? "<FRIENDS>" + f + "</FRIENDS>" : "")
				...
				+ ((birthday != null) ? "<BIRTHDAY>"
						+ CMParms.toStringList(birthday) + "</BIRTHDAY>" : "")
				+ ((poofin.length() > 0) ? "<POOFIN>"
						+ CMLib.xml().parseOutAngleBrackets(poofin)
						+ "</POOFIN>" : "")
				+ ((notes.length() > 0) ? "<NOTES>"
						+ CMLib.xml().parseOutAngleBrackets(notes) + "</NOTES>"
						: "")
				+ ((poofout.length() > 0) ? "<POOFOUT>"
						+ CMLib.xml().parseOutAngleBrackets(poofout)
						+ "</POOFOUT>" : "")
				+ ((announceMsg.length() > 0) ? "<ANNOUNCE>"
						+ CMLib.xml().parseOutAngleBrackets(announceMsg)
						+ "</ANNOUNCE>" : "")
				+ ((savedPose.length() > 0) ? "<POSE>"
						+ CMLib.xml().parseOutAngleBrackets(savedPose)
						+ "</POSE>" : "")
				+ ((tranpoofin.length() > 0) ? "<TRANPOOFIN>"
						+ CMLib.xml().parseOutAngleBrackets(tranpoofin)
						+ "</TRANPOOFIN>" : "")
				+ ((tranpoofout.length() > 0) ? "<TRANPOOFOUT>"
						+ CMLib.xml().parseOutAngleBrackets(tranpoofout)
						+ "</TRANPOOFOUT>" : "") + "<DATES>"
				+ this.getLevelDateTimesStr() + "</DATES>" + "<SECGRPS>"
				+ CMLib.xml().parseOutAngleBrackets(getSetSecurityFlags(null))
				+ "</SECGRPS>" + roomSet().xml() + rest.toString();
	}
	...
	private void setAliasXML(List<XMLpiece> xml) {
		...
		while ((++a) >= 0) {
			...
			if ((name.length() == 0) || (value.length() == 0))
				break;
			alias.put(name.toUpperCase().trim(), CMLib.xml()
					.restoreAngleBrackets(value));
		}
	}
	...
	private void setTitleXML(List<XMLpiece> xml) {
		...
		int t = -1;
		while ((++t) >= 0) {
			String title = CMLib.xml().restoreAngleBrackets(
					CMLib.xml().getValFromPieces(xml, "TITLE" + t));
			if (title.length() == 0)
				break;
			titles.add(title);
		}
	...
	public void setXML(String xmlStr) {
		...
		if (xmlStr == null)
			return;
		final boolean debug = CMSecurity
				.isDebugging(CMSecurity.DbgFlag.PLAYERSTATS);
		if (debug)
		...
		str = CMLib.xml().restoreAngleBrackets(
				CMLib.xml().getValFromPieces(xml, "SECGRPS"));
		if (debug)
		...
		if (str.length() > 0) {
			...
			for (int ss = 0; ss < sets.size(); ss++) {
				...
				if (CMath.s_int(twin.get(0)) >= lastNum) {
					levelInfo.addElement(
							Integer.valueOf(CMath.s_int(twin.get(0))),
		...
		if (levelInfo.size() == 0)
			levelInfo.addElement(Integer.valueOf(0),
		...
		for (int i = getSaveStatIndex(); i < codes.length; i++) {
			...
			if (str == null)
				str = "";
			setStat(codes[i].toUpperCase(),
					CMLib.xml().restoreAngleBrackets(str));
		}
	...
	public String getSetSecurityFlags(String newFlags) {
		if (newFlags != null) {
			securityFlags = CMSecurity.instance().createGroup("",
					CMParms.parseSemicolons(newFlags, true));
		}
	...
	public long getAccountExpiration() {
		return (account != null) ? account.getAccountExpiration()
				: accountExpires;
	}
			...
			for (Enumeration<Area> e = CMLib.map().areas(); e.hasMoreElements();) {
				...
				if ((CMLib.flags().canAccess(mob, A))
					...
					if (stats[Area.Stats.VISITABLE_ROOMS.ordinal()] > 0) {
						totalRooms += stats[Area.Stats.VISITABLE_ROOMS
								.ordinal()];
	...
	public void setLeveledDateTime(int level, Room R) {
		...
		for (int l = 0; l < levelInfo.size(); l++) {
			if (level == ((Integer) levelInfo.elementAt(l, 1)).intValue()) {
				levelInfo.setElementAt(l, 2,
						Long.valueOf(System.currentTimeMillis()));
				levelInfo.setElementAt(l, 3, CMLib.map().getExtendedRoomID(R));
				return;
			} else if ((System.currentTimeMillis() - lastTime) < TimeManager.MILI_HOUR)
				return;
			else if (level < ((Integer) levelInfo.elementAt(l, 1)).intValue()) {
				levelInfo.insertElementAt(l, Integer.valueOf(level), Long
						.valueOf(System.currentTimeMillis()), CMLib.map()
						.getExtendedRoomID(R));
				return;
			}
		...
		if ((System.currentTimeMillis() - lastTime) < TimeManager.MILI_HOUR)
			return;
		levelInfo.addElement(Integer.valueOf(level), Long.valueOf(System
				.currentTimeMillis()), CMLib.map().getExtendedRoomID(R));
	}
