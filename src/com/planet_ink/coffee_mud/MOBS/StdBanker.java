...
public class StdBanker extends StdShopKeeper implements Banker {
	...
	public void addDepositInventory(String depositorName, Item thisThang) {
		...
		CMLib.database()
				.DBCreateData(
						depositorName,
						bankChain(),
						"" + thisThang + Math.random(),
						name
								+ ";"
								+ CMLib.coffeeMaker().getPropertiesStr(
										thisThang, true));
	}
	...
	protected Item makeItem(String data) {
		...
		if (I != null) {
			CMLib.coffeeMaker()
					.setPropertiesStr(I, data.substring(x + 1), true);
			if ((I instanceof Coins) && (((Coins) I).getDenomination() == 0.0)
	...
	protected void bankLedger(String depositorName, String msg) {
		...
		CMLib.beanCounter().bankLedger(bankChain(), depositorName,
				date + ": " + msg);
	}
	...
	public MoneyLibrary.DebtItem getDebtInfo(String depositorName) {
		Vector<MoneyLibrary.DebtItem> debt = CMLib.beanCounter().getDebtOwed(
				bankChain());
		...
		try {
			if (tickID == Tickable.TICKID_MOB) {
				...
				synchronized (bankChain().intern()) {
					...
					if (proceed) {
						List<PlayerData> bankDataV = CMLib.database()
								.DBReadData(bankChain());
						...
						Vector<MoneyLibrary.DebtItem> debts = CMLib
								.beanCounter().getDebtOwed(bankChain());
						for (int u = 0; u < userNames.size(); u++) {
							...
							if (totalValue > 0)
								newBalance += CMath.mul(totalValue,
										itemInterest);
							for (int d = debts.size() - 1; d >= 0; d--) {
								MoneyLibrary.DebtItem debtItem = debts
										.elementAt(d);
								...
								if (debtor.equalsIgnoreCase(name)) {
									...
									double intDue = CMath.mul(intRate,
											dueAmount);
									long timeRemaining = debtDueAt
											- System.currentTimeMillis();
									if ((timeRemaining < 0)
											&& (newBalance < ((dueAmount) + intDue)))
										newBalance = -1.0;
									else {
										double amtDueNow = (timeRemaining < 0) ? (dueAmount + intDue)
												: CMath.div(
														(dueAmount + intDue),
															...
															CMLib.utensils()
																	...
																	+ CMLib.beanCounter()
																			.nameCurrencyShort(
																					...
																					amtDueNow)
																	+ ": Loan payment made.");
											CMLib.beanCounter().adjustDebt(
													debtor, bankChain(),
													intDue - amtDueNow, reason,
													intRate, debtDueAt);
											newBalance -= amtDueNow;
										} else
											CMLib.beanCounter().adjustDebt(
													debtor, bankChain(),
													intDue, reason, intRate,
													debtDueAt);
									}
							...
							if (newBalance < 0) {
								...
								delAllDeposits(name);
								CMLib.beanCounter().delAllDebt(name,
										bankChain());
												...
												.bankLedger(
														...
														CMLib.utensils()
																...
																+ CMLib.beanCounter()
																		.nameCurrencyShort(
																				this,
																				coinItem.getTotalValue()
																						- newBalance)
																+ ": Interest charged.");
									delDepositInventory(name, coinItem);
								}
								String currency = CMLib.beanCounter()
										.getCurrency(this);
								coinItem = CMLib.beanCounter()
										.makeBestCurrency(currency, newBalance);
								if (coinItem != null)
									addDepositInventory(name, coinItem);
							}
						}
						for (int d = debts.size() - 1; d >= 0; d--)
							CMLib.beanCounter().delAllDebt(
									debts.elementAt(d).debtor, bankChain());
					}
				}
			}
		} catch (Exception e) {
	...
	public String getBankClientName(MOB mob, Clan.Function func, boolean checked) {
		if (isSold(ShopKeeper.DEAL_CLANBANKER)) {
			Pair<Clan, Integer> clanPair = CMLib.clans().findPrivilegedClan(
					mob, func);
			if (clanPair != null)
				...
				if (mob.clans().iterator().hasNext()) {
					CMLib.commands()
							.postSay(
									this,
									mob,
									"I'm sorry, you aren't authorized by your clan to do that.",
									true, false);
					return null;
				} else {
					CMLib.commands().postSay(this, mob,
							"I'm sorry, I only deal with clans.", true, false);
					return null;
				}
			}
		}
		...
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_DEPOSIT:
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					String depositorName = getBankClientName(msg.source(),
							Clan.Function.DEPOSIT, false);
					if (msg.tool() instanceof Container)
						((Container) msg.tool()).emptyPlease(true);
					CMMsg msg2 = CMClass.getMsg(msg.source(), msg.tool(), null,
					...
					if (msg.tool() instanceof Coins) {
						...
						Item old = findDepositInventory(depositorName, ""
								+ Integer.MAX_VALUE);
						if ((old == null)
								&& (!isSold(ShopKeeper.DEAL_CLANBANKER))
								&& (msg.source().isMarriedToLiege())) {
							old = findDepositInventory(msg.source()
									.getLiegeID(), "" + Integer.MAX_VALUE);
							if (old != null) {
								MOB owner = CMLib.players().getPlayer(
										msg.source().getLiegeID());
								if (owner != null)
						...
						Coins item = CMLib.beanCounter()
								.makeBestCurrency(
										CMLib.beanCounter().getCurrency(this),
										newValue);
						bankLedger(
								depositorName,
								"Deposit of "
										+ CMLib.beanCounter()
												.nameCurrencyShort(this,
														newValue) + ": "
										+ msg.source().Name());
						if (old != null)
							delDepositInventory(depositorName, old);
						if (item != null)
						...
						if (isSold(ShopKeeper.DEAL_CLANBANKER))
							CMLib.commands()
									.postSay(
											...
											"Ok, "
													+ CMStrings
															.capitalizeFirstLetter(depositorName)
													+ " now has a balance of "
													+ CMLib.beanCounter()
															.nameCurrencyLong(
																	this,
																	getBalance(depositorName))
													+ ".", true, false);
						else
							CMLib.commands()
									.postSay(
						...
						if (msg.sourceMessage() != null)
							msg.setSourceMessage(CMStrings.replaceAll(msg
									.sourceMessage(), "<O-NAME>", msg.tool()
									.name()));
						if (msg.targetMessage() != null)
							msg.setTargetMessage(CMStrings.replaceAll(msg
									.targetMessage(), "<O-NAME>", msg.tool()
									.name()));
						if (msg.othersMessage() != null)
							msg.setOthersMessage(CMStrings.replaceAll(msg
									.othersMessage(), "<O-NAME>", msg.tool()
									.name()));
						((Coins) msg.tool()).setNumberOfCoins(0); // prevents
																	// banker
																	// from
																	// accumulating
																	// wealth
						double riches = CMLib.beanCounter()
								.getTotalAbsoluteNativeValue(this);
						if (riches > 0.0)
						...
						if (!item.amDestroyed()) {
							...
							CMLib.commands().postSay(
									...
									mob,
									"Thank you, " + item.name()
											+ " is safe with us.", true, false);
							((Item) msg.tool()).destroy();
						} else
							CMLib.commands().postSay(this, mob,
									"Whoops! Where'd it go?", true, false);
					}
				}
				return;
			case CMMsg.TYP_BORROW:
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					...
					if (old instanceof Coins) {
						String borrowerName = getBankClientName(msg.source(),
								Clan.Function.WITHDRAW, false);
						bankLedger(borrowerName, "Loan of " + old.Name() + ": "
								+ msg.source().Name());
						addItem(old);
						double amt = ((Coins) old).getTotalValue();
						CMMsg newMsg = CMClass.getMsg(this, msg.source(), old,
								CMMsg.MSG_GIVE,
						...
						int months = 2;
						while ((months < (location().getArea().getTimeObj()
								.getMonthsInYear() * 10))
								&& (CMath.div(amt, months) > 250.0))
							months++;
						long dueAt = System.currentTimeMillis()
								+ (timeInterval() * months);
						CMLib.beanCounter().adjustDebt(borrowerName,
								bankChain(), amt, "Bank Loan", interestRate,
								dueAt);
					}
				}
				break;
			case CMMsg.TYP_WITHDRAW:
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					String withdrawerName = getBankClientName(msg.source(),
							Clan.Function.WITHDRAW, false);
					Item old = (Item) msg.tool();
					if (old instanceof Coins) {
						Item depositInventoryItem = findDepositInventory(
								withdrawerName, "" + Integer.MAX_VALUE);
						if ((!isSold(ShopKeeper.DEAL_CLANBANKER))
								&& (msg.source().isMarriedToLiege())
								&& ((depositInventoryItem == null) || ((depositInventoryItem instanceof Coins) && (((Coins) depositInventoryItem)
										.getTotalValue() < ((Coins) old)
										.getTotalValue())))) {
							Item item2 = findDepositInventory(msg.source()
									.getLiegeID(), "" + Integer.MAX_VALUE);
							if ((item2 != null)
									&& (item2 instanceof Coins)
									&& (((Coins) item2).getTotalValue() >= ((Coins) old)
											.getTotalValue())) {
						...
						if ((depositInventoryItem != null)
								...
								&& (((Coins) depositInventoryItem)
										.getTotalValue() >= ((Coins) old)
										.getTotalValue())) {
							Coins coins = CMLib.beanCounter().makeBestCurrency(
									this,
									((Coins) depositInventoryItem)
											.getTotalValue()
											- ((Coins) old).getTotalValue());
							bankLedger(
									withdrawerName,
									"Withdrawl of "
											+ CMLib.beanCounter()
													.nameCurrencyShort(
															...
															((Coins) old)
																	.getTotalValue())
											+ ": " + msg.source().Name());
							delDepositInventory(withdrawerName,
									depositInventoryItem);

							addItem(old);
							CMMsg newMsg = CMClass
									.getMsg(this, msg.source(), old,
											CMMsg.MSG_GIVE,
											"<S-NAME> give(s) <O-NAME> to <T-NAMESELF>.");
							if (location().okMessage(this, newMsg)) {
								location().send(this, newMsg);
								...
								CMLib.commands().postDrop(this, old, true,
										false, false);
							if ((coins == null)
									|| (coins.getNumberOfCoins() <= 0)) {
								if (isSold(ShopKeeper.DEAL_CLANBANKER))
									CMLib.commands()
											.postSay(
													this,
													mob,
													"I have closed the account for "
															+ CMStrings
																	.capitalizeFirstLetter(withdrawerName)
									...
									CMLib.commands()
											.postSay(
													this,
													mob,
													"I have closed that account. Thanks for your business.",
													true, false);
								return;
							}
						...
						CMLib.commands().postSay(this, mob,
								"Thank you for your trust.", true, false);
						if (location() != null)
							location().addItem(old,
									ItemPossessor.Expire.Player_Drop);
						CMMsg msg2 = CMClass.getMsg(mob, old, this,
								CMMsg.MSG_GET, null);
			...
			case CMMsg.TYP_LIST: {
				super.executeMsg(myHost, msg);
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					String listerName = getBankClientName(msg.source(),
							Clan.Function.DEPOSIT_LIST, false);
					List<Item> V = null;
					if (isSold(ShopKeeper.DEAL_CLANBANKER))
					...
					final int COL_LEN = ListingLibrary.ColFixer.fixColWidth(
							34.0, mob);
					...
					for (int i = 0; i < V.size(); i++) {
						...
						if (!(I instanceof Coins)) {
							...
							String col = null;
							col = "["
									+ CMStrings.padRight(I.name(mob), COL_LEN)
									+ "] ";
							if ((++colNum) > 2) {
								str.append("\n\r");
								colNum = 1;
					...
					if (balance > 0) {
						if (isSold(ShopKeeper.DEAL_CLANBANKER))
							str.append(CMStrings
									.capitalizeFirstLetter(listerName)
									+ " has a balance of ^H"
									+ CMLib.beanCounter().nameCurrencyLong(
											this, balance) + "^?.");
						else
							str.append("Your balance is ^H"
									+ CMLib.beanCounter().nameCurrencyLong(
											this, balance) + "^?.");
					}
					Vector<MoneyLibrary.DebtItem> debts = CMLib.beanCounter()
							.getDebt(listerName, bankChain());
					if (debts != null)
						for (int d = 0; d < debts.size(); d++) {
							...
							if (timeRemaining > 0)
								str.append("\n\r"
										+ ((isSold(ShopKeeper.DEAL_CLANBANKER)) ? CMStrings
												.capitalizeFirstLetter(listerName)
												: "You")
										+ " owe ^H"
										+ CMLib.beanCounter().nameCurrencyLong(
					...
					if (coinInterest != 0.0) {
						...
						String ci = ((coinInterest > 0.0) ? "pay " : "charge ")
								+ cci + "% interest ";
						str.append("\n\rThey " + ci
								+ "monthly on money deposited here.");
					}
					if (itemInterest != 0.0) {
						double cci = CMath.mul(Math.abs(itemInterest), 100.0);
						String ci = ((itemInterest > 0.0) ? "pay " : "charge ")
								+ cci + "% interest ";
						str.append("\n\rThey " + ci
								+ "monthly on items deposited here.");
					}
		...
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_DEPOSIT: {
				...
				if (msg.tool() == null)
					return false;
				String listerName = getBankClientName(msg.source(),
						Clan.Function.DEPOSIT, true);
				if (listerName == null)
					return false;
				double balance = getBalance(listerName);
				if (msg.tool() instanceof Coins) {
					if ((Double.MAX_VALUE - balance) <= ((Coins) msg.tool())
							.getTotalValue()) {
						CMLib.commands()
								.postSay(
										this,
										mob,
										"I'm sorry, the law prevents us from holding that much money in one account.",
										true, false);
						return false;
					}
					if (!((Coins) msg.tool()).getCurrency().equalsIgnoreCase(
							CMLib.beanCounter().getCurrency(this))) {
						CMLib.commands().postSay(
								this,
								mob,
								"I'm sorry, this bank only deals in "
										+ CMLib.beanCounter()
												.getDenominationName(
														CMLib.beanCounter()
																.getCurrency(
				...
				if (!(msg.tool() instanceof Item)) {
					mob.tell(mob.charStats().HeShe()
							+ " doesn't look interested.");
					return false;
				}
				if (CMLib.flags().isEnspelled((Item) msg.tool())
						|| CMLib.flags().isOnFire((Item) msg.tool())) {
					mob.tell(this, msg.tool(), null,
							"<S-HE-SHE> refuses to accept <T-NAME> for deposit.");
					return false;
				}
				double minbalance = (totalItemsWorth(listerName) / MIN_ITEM_BALANCE_DIVIDEND)
						+ CMath.div(((Item) msg.tool()).value(),
				...
				if (balance < minbalance) {
					if (isSold(ShopKeeper.DEAL_CLANBANKER))
						CMLib.commands().postSay(
								this,
								mob,
								CMStrings.capitalizeFirstLetter(listerName)
										+ " will need a total balance of "
										+ CMLib.beanCounter()
												.nameCurrencyShort(this,
														minbalance)
										+ " for me to hold that.", true, false);
					else
						CMLib.commands().postSay(
								this,
								mob,
								"You'll need a total balance of "
										+ CMLib.beanCounter()
												.nameCurrencyShort(this,
														minbalance)
										+ " for me to hold that.", true, false);
			...
			case CMMsg.TYP_WITHDRAW: {
				if (!CMLib.coffeeShops().ignoreIfNecessary(msg.source(),
					...
					return false;
				String withdrawerName = getBankClientName(msg.source(),
						Clan.Function.WITHDRAW, true);
				if (withdrawerName == null)
					return false;
				if ((msg.tool() == null) || (!(msg.tool() instanceof Item))) {
					CMLib.commands().postSay(this, mob,
							"What do you want? I'm busy!", true, false);
					return false;
				}
				if ((msg.tool() != null)
				...
				if (msg.tool() instanceof Coins) {
					if (!((Coins) msg.tool()).getCurrency().equals(
							CMLib.beanCounter().getCurrency(this))) {
						CMLib.commands().postSay(
								this,
								mob,
					...
					if ((!isSold(ShopKeeper.DEAL_CLANBANKER))
						...
						if (M != null)
							b = getBalance(M.Name());
						if ((M != null)
								&& (b >= ((Coins) msg.tool()).getTotalValue())) {
							owner = M;
							balance = b;
							withdrawerName = owner.Name();
						}
					}
				} else if (findDepositInventory(withdrawerName, msg.tool()
						.Name()) == null) {
					if ((!isSold(ShopKeeper.DEAL_CLANBANKER))
							&& (msg.source().isMarriedToLiege())
							&& (findDepositInventory(msg.source().getLiegeID(),
									msg.tool().Name()) != null))
						owner = CMLib.players().getPlayer(
								msg.source().getLiegeID());
					else {
						CMLib.commands().postSay(this, mob, "You want WHAT?",
								true, false);
						return false;
					}
				} else if (msg.tool() instanceof Item) {
					double debt = CMLib.beanCounter().getDebtOwed(
							withdrawerName, bankChain());
					if ((debt > 0.0)
						...
						CMLib.commands()
								.postSay(
										this,
										mob,
										"I'm sorry, but that item is being held as collateral against your debt at this time.",
										true, false);
						return false;
					}
				}
					...
					if (((Coins) msg.tool()).getTotalValue() > balance) {
						if (isSold(ShopKeeper.DEAL_CLANBANKER))
							...
							CMLib.commands().postSay(
									this,
									mob,
									"I'm sorry, you have only "
											+ CMLib.beanCounter()
													.nameCurrencyShort(this,
															balance)
											+ " in that account.", true, false);
						return false;
					}
					if (minbalance == 0)
						return true;
						...
						if ((balance - minbalance) > 0)
							CMLib.commands()
									.postSay(
											this,
											mob,
											"I'm sorry, you may only withdraw "
													+ CMLib.beanCounter()
															.nameCurrencyShort(
																	this,
																	balance
							...
							CMLib.commands()
									.postSay(
											this,
											mob,
											"I am holding other items in trust, so you may not withdraw funds at this time.",
											true, false);
						return false;
					}
				}
			...
			case CMMsg.TYP_BORROW: {
				...
				if (!(msg.tool() instanceof Coins)) {
					CMLib.commands().postSay(this, mob,
							"I'm sorry, only MONEY can be borrowed.", true,
							false);
					return false;
				}
				String withdrawerName = getBankClientName(msg.source(),
				...
				if ((numberDeposited(withdrawerName) == 0)
						&& ((isSold(ShopKeeper.DEAL_CLANBANKER))
								|| (!msg.source().isMarriedToLiege()) || (numberDeposited(msg
								.source().getLiegeID()) == 0))) {
					StringBuffer str = new StringBuffer("");
					if (isSold(ShopKeeper.DEAL_CLANBANKER))
						str.append(CMStrings
								.capitalizeFirstLetter(withdrawerName)
								+ " does not have an account with us, I'm afraid.");
					else
						str.append("You don't have an account with us, I'm afraid.");
					CMLib.commands().postSay(this, mob, str.toString() + "^T",
							true, false);
					return false;
				}
				double debt = CMLib.beanCounter().getDebtOwed(withdrawerName,
						bankChain());
				if (debt > 0.0) {
					StringBuffer str = new StringBuffer("");
					if (isSold(ShopKeeper.DEAL_CLANBANKER))
						str.append(CMStrings
								.capitalizeFirstLetter(withdrawerName)
								+ " already has a "
								+ CMLib.beanCounter().nameCurrencyShort(this,
										debt) + " loan out with us.");
					else
						str.append("You already have a "
								+ CMLib.beanCounter().nameCurrencyShort(this,
										debt) + " loan out with us.");
					CMLib.commands().postSay(this, mob, str.toString() + "^T",
							true, false);
				...
				double collateralRemaining = ((Coins) msg.tool())
						.getTotalValue() - totalItemsWorth(withdrawerName);
				if (collateralRemaining > 0) {
					...
					str.append("will need to deposit enough items with us as collateral.  You'll need items worth "
							+ CMLib.beanCounter().nameCurrencyShort(this,
									collateralRemaining) + " more to qualify.");
					CMLib.commands().postSay(this, mob, str.toString() + "^T",
							true, false);
					return false;
				}
			...
			case CMMsg.TYP_LIST: {
				if (!CMLib.coffeeShops().ignoreIfNecessary(msg.source(),
					...
					return false;
				String listerName = getBankClientName(msg.source(),
						Clan.Function.DEPOSIT_LIST, true);
				if (listerName == null)
					return false;
				if ((numberDeposited(listerName) == 0)
					...
					StringBuffer str = new StringBuffer("");
					if (isSold(ShopKeeper.DEAL_CLANBANKER))
						str.append(CMStrings.capitalizeFirstLetter(listerName)
								+ " does not have an account with us, I'm afraid.");
					else
						str.append("You don't have an account with us, I'm afraid.");
					if (coinInterest != 0.0) {
