...
public class GenAuctioneer extends StdAuctioneer {
	...
	private final static String[] MYCODES = { "WHATISELL", "PREJUDICE",
			"AUCHOUSE", "LIVEPRICE", "TIMEPRICE", "TIMEPCT", "LIVECUT",
			"TIMECUT", "MAXADAYS", "MINADAYS", "IGNOREMASK", "PRICEMASKS" };
		...
		if (CMLib.coffeeMaker().getGenMobCodeNum(code) >= 0)
			...
			case 0: {
				if ((val.length() == 0) || (CMath.isLong(val)))
					...
					setWhatIsSoldMask(CMParms.indexOfIgnoreCase(
							ShopKeeper.DEAL_DESCS, val));
				break;
			}
			case 1:
	...
	public String[] getStatCodes() {
		...
		String[] MYCODES = CMProps
				.getStatCodesList(GenAuctioneer.MYCODES, this);
