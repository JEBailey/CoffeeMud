...
public class Dragon extends StdMOB {
	...
	public void setupDragonIfNecessary() {
		...
		// ===== set the basics
		setName(getAgeDescription(DragonAge()).toString() + " "
				+ getColorDescription(DragonColor()) + " Dragon");
		setDescription("A majestic " + getColorDescription(DragonColor())
				+ " Dragon, simply being in its presence makes you uneasy.");
		setDisplayText(getAgeDescription(DragonAge()).toString() + " "
				+ getColorDescription(DragonColor())
				+ " Dragon watches you intently.");
		...
		if (room != null)
			for (int x = 0; x < room.numInhabitants(); x++) {
				...
				if ((target != null) && (!target.ID().equals(ID()))) {
					CMMsg Message = CMClass.getMsg(this, target, null,
							CMMsg.MSK_MALICIOUS_MOVE | AffectCode,
							CMMsg.MSK_MALICIOUS_MOVE | AffectCode,
							CMMsg.MSG_NOISYMOVEMENT, msgText);
					if (room.okMessage(this, Message)) {
						room.send(this, Message);
						int damage = ((short) Math
								.round(CMath.div(CMath.mul(Math.random(),
										7 * DragonAge()), 2.0)));
						if (Message.value() <= 0)
							damage = ((short) Math.round(Math.random() * 7) * DragonAge());
						if (dragonbreath == null)
							dragonbreath = CMClass.getAbility("Dragonbreath");
						CMLib.combat().postDamage(this, target, dragonbreath,
								damage, CMMsg.MASK_ALWAYS | AffectCode,
								WeaponType, "The blast <DAMAGE> <T-NAME>.");
					}
				}
			}
		return true;
	...
	protected boolean trySwallowWhole() {
		...
		if (CMLib.flags().aliveAwakeMobileUnbound(this, true)
				&& (rangeToTarget() == 0)
				&& (CMLib.flags().canHear(this) || CMLib.flags().canSee(this) || CMLib
						.flags().canSmell(this))) {
			...
			if (TastyMorsel.phyStats().weight() < 1500) {
				...
				if (roll < 2) {
					...
					CMMsg EatMsg = CMClass.getMsg(this, TastyMorsel, null,
							CMMsg.MSG_EAT, CMMsg.MASK_ALWAYS
									| CMMsg.TYP_JUSTICE,
	...
	public void recoverCharStats() {
		...
		if ((dragonAge < 0) || (dragonAge > 20))
			dragonAge = 20;
		charStats().setStat(CharStats.STAT_SAVE_MAGIC,
				charStats().getStat(CharStats.STAT_SAVE_MAGIC) + dragonAge * 5);
		switch (DragonColor()) {
		case GOLD:
			charStats().setStat(CharStats.STAT_SAVE_FIRE,
					charStats().getStat(CharStats.STAT_SAVE_FIRE) + 100);
			charStats().setStat(CharStats.STAT_SAVE_GAS,
					charStats().getStat(CharStats.STAT_SAVE_GAS) + 100);
			break;
		case RED:
		case BRASS:
			charStats().setStat(CharStats.STAT_SAVE_FIRE,
		...
		case BLUE:
		case BRONZE:
			charStats().setStat(CharStats.STAT_SAVE_ELECTRIC,
					charStats().getStat(CharStats.STAT_SAVE_ELECTRIC) + 100);
			break;
		case WHITE:
		case SILVER:
			charStats().setStat(CharStats.STAT_SAVE_COLD,
					charStats().getStat(CharStats.STAT_SAVE_COLD) + 100);
			break;
		case BLACK:
		case COPPER:
			charStats().setStat(CharStats.STAT_SAVE_ACID,
					charStats().getStat(CharStats.STAT_SAVE_ACID) + 100);
			break;
		}
	}
	...
	protected boolean digestTastyMorsels() {
		...
		for (int x = 0; x < morselCount; x++) {
			...
			if (TastyMorsel != null) {
				CMMsg DigestMsg = CMClass.getMsg(this, TastyMorsel, null,
						CMMsg.MSG_OK_ACTION,
						"<S-NAME> digest(s) <T-NAMESELF>!!");
				Stomach.send(this, DigestMsg);
				int damage = ((int) Math.round(CMath.div(TastyMorsel.curState()
						.getHitPoints(), 2)));
				if (damage < (TastyMorsel.phyStats().level() + 6))
					damage = TastyMorsel.curState().getHitPoints() + 1;
				if (DigestMsg.value() != 0)
					damage = damage / 2;
				CMLib.combat().postDamage(this, TastyMorsel, null, damage,
		...
		if ((Stomach != null) && (room != null)) {
			...
			for (int y = itemCount - 1; y >= 0; y--) {
				...
				if (PartiallyDigestedItem != null) {
					room.addItem(PartiallyDigestedItem,
