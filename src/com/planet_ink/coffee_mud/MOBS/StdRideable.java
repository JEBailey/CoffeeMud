...
public class StdRideable extends StdMOB implements Rideable {
	...
	public String stateStringSubject(Rider R) {
		if ((R instanceof Rideable)
				&& ((Rideable) R).rideBasis() == Rideable.RIDEABLE_WAGON)
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		case CMMsg.TYP_SIT:
			if (amRiding(msg.source())) {
				...
				msg.source().tell(
						"You cannot simply sit on " + name(msg.source())
								+ ", try 'mount'.");
				return false;
			} else if (msg.source() instanceof Rideable) {
				msg.source().tell(
						"You are not allowed on " + name(msg.source()) + ".");
				return false;
			}
			break;
		...
		case CMMsg.TYP_MOUNT:
			...
			if (msg.amITarget(this)) {
				...
				if (amRiding(whoWantsToRide)) {
					msg.source().tell(
							whoWantsToRide.name(msg.source()) + " is "
									+ stateString(whoWantsToRide) + " "
									+ name(msg.source()) + "!");
				...
				if ((msg.tool() instanceof Rideable)
					...
					msg.source().tell(
							((Item) msg.tool()).name(msg.source())
									+ " can not be mounted on "
									+ name(msg.source()) + ".");
					return false;
				}
				if ((basePhyStats().weight() * 5 < whoWantsToRide
						.basePhyStats().weight())) {
					msg.source().tell(
							name(msg.source()) + " is too small for "
									+ whoWantsToRide.name(msg.source()) + ".");
					return false;
				}
				if ((numRiders() >= riderCapacity())
					...
					// for items
					msg.source().tell(
							"No more can fit on " + name(msg.source()) + ".");
					// for mobs
		...
		case CMMsg.TYP_SELL:
			if (amRiding(msg.source())) {
				msg.source().tell(
						"You cannot do that while " + stateString(msg.source())
								+ " " + name(msg.source()) + ".");
				return false;
			}
			break;
		...
		if (CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS)) {
			if ((msg.amITarget(this))
					&& ((msg.source().riding() == this) || (this.amRiding(msg
							.source())))) {
				msg.source().tell(
						"You can't attack " + name(msg.source())
								+ " right now.");
				...
				return false;
					...
					&& (msg.target() != null)
					&& (msg.target() instanceof MOB)
					&& ((amRiding((MOB) msg.target())) || (((MOB) msg.target())
							.riding() == this)))
		...
		case CMMsg.TYP_EXAMINE:
			if ((msg.target() == this) && (numRiders() > 0)
				...
				msg.addTrailerMsg(CMClass.getMsg(msg.source(), null, null,
						CMMsg.MSG_OK_VISUAL, displayText(msg.source()),
