...
public class StdDeity extends StdMOB implements Deity {
	...
	protected void cloneFix(MOB E) {
		...
		if (E instanceof StdDeity) {
			worshipTriggers = new XVector<DeityTrigger>(
					((StdDeity) E).worshipTriggers);
			worshipCurseTriggers = new XVector<DeityTrigger>(
					((StdDeity) E).worshipCurseTriggers);
			clericTriggers = new XVector<DeityTrigger>(
					((StdDeity) E).clericTriggers);
			clericPowerTriggers = new XVector<DeityTrigger>(
					((StdDeity) E).clericPowerTriggers);
			clericCurseTriggers = new XVector<DeityTrigger>(
					((StdDeity) E).clericCurseTriggers);
			...
			trigBlessingParts = new XHashtable<String, boolean[]>(
					((StdDeity) E).trigBlessingParts);
			trigBlessingTimes = new XHashtable<String, Long>(
					((StdDeity) E).trigBlessingTimes);
			trigPowerParts = new XHashtable<String, boolean[]>(
					((StdDeity) E).trigPowerParts);
			trigPowerTimes = new XHashtable<String, Long>(
					((StdDeity) E).trigPowerTimes);
			trigCurseParts = new XHashtable<String, boolean[]>(
					((StdDeity) E).trigCurseParts);
			trigCurseTimes = new XHashtable<String, Long>(
					((StdDeity) E).trigCurseTimes);
			trigServiceParts = new XHashtable<String, boolean[]>(
					((StdDeity) E).trigServiceParts);
			trigServiceTimes = new XHashtable<String, Long>(
					((StdDeity) E).trigServiceTimes);
		}
	}
	...
	public void setServiceRitual(String ritual) {
		if ((ritual == null) || (ritual.length() == 0))
			ritual = "SAY Bless us " + name() + "&wait 10&wait 10&SAY May "
					+ name() + " bless you all&ALLSAY Amen.&SAY Go in peace";
	...
	public String getTriggerDesc(List<DeityTrigger> V) {
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (v > 0)
				buf.append(", "
						+ ((DT.previousConnect == CONNECT_AND) ? "and " : "or "));
			...
			case TRIGGER_READING:
				if (DT.parm1.equals("0"))
					...
					buf.append("the player should read '"
							+ DT.parm1.toLowerCase() + "'");
				break;
			case TRIGGER_TIME:
			...
			case TRIGGER_PUTTHING:
				buf.append("the player should put " + DT.parm1.toLowerCase()
						+ " in " + DT.parm2.toLowerCase());
				break;
			...
			case TRIGGER_INROOM: {
				if (DT.parm1.equalsIgnoreCase("holy")
					...
					buf.append("the player should be in the deities room of infused "
							+ DT.parm1.toLowerCase() + "-ness.");
				else {
					Room R = CMLib.map().getRoom(DT.parm1);
					if (R == null)
						buf.append("the player should be in some unknown place");
					else
						buf.append("the player should be in '"
								+ R.displayText(null) + "'");
				}
			}
				break;
			...
			case TRIGGER_WAIT:
				buf.append("wait "
						+ ((CMath.s_int(DT.parm1) * CMProps.getTickMillis()) / 1000)
						+ " seconds");
				break;
			case TRIGGER_YOUSAY:
				buf.append("then you will automatically say '"
						+ DT.parm1.toLowerCase() + "'");
				break;
			case TRIGGER_OTHERSAY:
				buf.append("then all others will say '"
						+ DT.parm1.toLowerCase() + "'");
				break;
			case TRIGGER_ALLSAY:
			...
			case TRIGGER_PUTVALUE:
				buf.append("the player should put an item worth at least "
						+ DT.parm1.toLowerCase() + " in "
						+ DT.parm2.toLowerCase());
				break;
			case TRIGGER_PUTMATERIAL: {
				...
				buf.append("the player puts an item made of " + material
						+ " in " + DT.parm2.toLowerCase());
			}
				break;
			...
			case TRIGGER_BURNVALUE:
				buf.append("the player should burn an item worth at least "
						+ DT.parm1.toLowerCase());
				break;
	...
	public String getClericRequirementsDesc() {
		return "The following may be clerics of " + name() + ": "
				+ CMLib.masking().maskDesc(getClericRequirements());
	}

	public String getClericTriggerDesc() {
		if (numBlessings() > 0)
			return "The blessings of " + name() + " are bestowed to "
					+ charStats().hisher()
					+ " clerics whenever the cleric does the following: "
					+ getTriggerDesc(clericTriggers) + ".";
		return "";
	}

	public String getWorshipRequirementsDesc() {
		return "The following are acceptable worshipers of " + name() + ": "
				+ CMLib.masking().maskDesc(getWorshipRequirements());
	}
	...
	public String getWorshipTriggerDesc() {
		if (numBlessings() > 0)
			return "The blessings of " + name() + " are bestowed to "
					+ charStats().hisher()
					+ " worshippers whenever they do the following: "
					+ getTriggerDesc(worshipTriggers) + ".";
		return "";
	}

	public String getServiceTriggerDesc() {
		return "The services of " + name() + " are the following: "
				+ getTriggerDesc(serviceTriggers) + ".";
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.targetMinor() == CMMsg.TYP_EXPIRE)
			...
			case CMMsg.TYP_SERVE:
				...
				if (msg.source().charStats().getCurrentClass().baseClass()
						.equalsIgnoreCase("Cleric")) {
					if (!CMLib.masking().maskCheck(getClericRequirements(),
							msg.source(), true)) {
						msg.source().tell(
								"You are unworthy of serving " + name() + ".");
						if (msg.source().isMonster())
							CMLib.commands().postSay(msg.source(), null,
									"I am unworthy of serving " + name() + ".");
						return false;
					}
				} else if (!CMLib.masking().maskCheck(getWorshipRequirements(),
						msg.source(), true)) {
					msg.source().tell("You are unworthy of " + name() + ".");
					if (msg.source().isMonster())
						CMLib.commands().postSay(msg.source(), null,
								"I am unworthy of " + name() + ".");
					return false;
				}
				break;
	...
	public synchronized void bestowPower(MOB mob, Ability Power) {
		if ((mob.fetchAbility(Power.ID()) == null)
			...
			Power.setProficiency(CMLib.ableMapper().getMaxProficiency(mob,
					true, Power.ID()));
	...
	public synchronized void bestowBlessings(MOB mob) {
		...
		try {
			if ((!alreadyBlessed(mob)) && (numBlessings() > 0)) {
				mob.location().show(this, mob, CMMsg.MSG_OK_VISUAL,
						"You feel the presence of <S-NAME> in <T-NAME>.");
				if ((mob.charStats().getCurrentClass().baseClass()
						.equals("Cleric")) || (CMSecurity.isASysOp(mob))) {
	...
	public synchronized void bestowPowers(MOB mob) {
		...
		try {
			if ((!alreadyPowered(mob)) && (numPowers() > 0)) {
				mob.location().show(this, mob, CMMsg.MSG_OK_VISUAL,
						"You feel the power of <S-NAME> in <T-NAME>.");
				Ability Power = fetchPower(CMLib.dice()
						.roll(1, numPowers(), -1));
				if (Power != null)
	...
	public synchronized void bestowCurses(MOB mob) {
		...
		try {
			if (numCurses() > 0) {
				mob.location().show(this, mob, CMMsg.MSG_OK_VISUAL,
						"You feel the wrath of <S-NAME> in <T-NAME>.");
				if (mob.charStats().getCurrentClass().baseClass()
						.equals("Cleric")
	...
	public void removeBlessings(MOB mob) {
		if ((alreadyBlessed(mob)) && (mob.location() != null)) {
			mob.location().show(this, mob, CMMsg.MSG_OK_VISUAL,
					"<S-NAME> remove(s) <S-HIS-HER> blessings from <T-NAME>.");
			for (int a = mob.numEffects() - 1; a >= 0; a--) // reverse, and
															// personal
			{
	...
	public void removePowers(MOB mob) {
		if ((alreadyPowered(mob)) && (mob.location() != null)) {
			mob.location().show(this, mob, CMMsg.MSG_OK_VISUAL,
					"<S-NAME> remove(s) <S-HIS-HER> powers from <T-NAME>.");
	...
	public boolean triggerCheck(CMMsg msg, List<DeityTrigger> V,
			Map<String, boolean[]> trigParts, Map<String, Long> trigTimes) {
			...
			if ((msg.sourceMinor() == TRIG_WATCH[DT.triggerCode])
				...
				case TRIGGER_SAY:
					if ((msg.sourceMessage() != null)
							&& (msg.sourceMessage().toUpperCase()
									.indexOf(DT.parm1) > 0))
						yup = true;
					break;
				case TRIGGER_TIME:
					if ((msg.source().location() != null)
							&& (msg.source().location().getArea().getTimeObj()
									.getTimeOfDay() == CMath.s_int(DT.parm1)))
						yup = true;
					break;
				case TRIGGER_RANDOM:
				...
				case TRIGGER_YOUSAY:
					if (v <= 0)
						...
						if ((checks != null) && (checks[v - 1]) && (!checks[v])) {
							...
							norecurse = true;
							CMLib.commands().postSay(msg.source(), null,
									CMStrings.capitalizeAndLower(DT.parm1));
							norecurse = false;
						...
						if ((checks != null) && (checks[v - 1]) && (!checks[v])
							...
							for (int t = v + 1; t < checks.length; t++)
								if (checks[t])
									proceed = false;
							if (proceed) {
								if (System.currentTimeMillis() > (trigTimes
										.get(msg.source().Name()).longValue() + (CMath
										.s_int(DT.parm1) * CMProps
										.getTickMillis()))) {
									yup = true;
				...
				case TRIGGER_PUTTHING:
					if ((msg.target() != null)
							...
							&& (CMLib.english().containsString(msg.tool()
									.name(), DT.parm1))
							&& (CMLib.english().containsString(msg.target()
									.name(), DT.parm2)))
						yup = true;
					break;
				case TRIGGER_BURNTHING:
				...
				case TRIGGER_EAT:
					if ((msg.target() != null)
							&& (DT.parm1.equals("0") || CMLib.english()
									.containsString(msg.target().name(),
											DT.parm1)))
						yup = true;
					break;
				case TRIGGER_INROOM:
					if (msg.source().location() != null) {
						if (DT.parm1.equalsIgnoreCase("holy")
								|| DT.parm1.equalsIgnoreCase("unholy")
								|| DT.parm1.equalsIgnoreCase("balance"))
							yup = CMLib.law().getClericInfused(
									msg.source().location()) == this;
						else if (msg.source().location().roomID()
								.equalsIgnoreCase(DT.parm1))
							yup = true;
					}
					break;
				case TRIGGER_RIDING:
					if ((msg.source().riding() != null)
							&& (CMLib.english().containsString(msg.source()
									.riding().name(), DT.parm1)))
						yup = true;
					break;
				case TRIGGER_CAST:
					if ((msg.tool() != null)
							&& ((msg.tool().ID().equalsIgnoreCase(DT.parm1)) || (CMLib
									.english().containsString(
									msg.tool().name(), DT.parm1))))
						yup = true;
					break;
				case TRIGGER_EMOTE:
					if ((msg.sourceMessage() != null)
				...
				case TRIGGER_PUTVALUE:
					if ((msg.tool() != null)
							&& (msg.tool() instanceof Item)
							&& (((Item) msg.tool()).baseGoldValue() >= CMath
									.s_int(DT.parm1))
							&& (msg.target() != null)
							&& (msg.target() instanceof Container)
							&& (CMLib.english().containsString(msg.target()
				...
				case TRIGGER_PUTMATERIAL:
					if ((msg.tool() != null)
							&& (msg.tool() instanceof Item)
							&& (((((Item) msg.tool()).material() & RawMaterial.RESOURCE_MASK) == CMath
									.s_int(DT.parm1)) || ((((Item) msg.tool())
									.material() & RawMaterial.MATERIAL_MASK) == CMath
									.s_int(DT.parm1)))
							&& (msg.target() != null)
							&& (msg.target() instanceof Container)
							&& (CMLib.english().containsString(msg.target()
				...
				case TRIGGER_BURNMATERIAL:
					if ((msg.target() != null)
							...
							&& (((((Item) msg.target()).material() & RawMaterial.RESOURCE_MASK) == CMath
									.s_int(DT.parm1)) || ((((Item) msg.target())
									.material() & RawMaterial.MATERIAL_MASK) == CMath
									.s_int(DT.parm1))))
						yup = true;
					break;
				case TRIGGER_BURNVALUE:
					if ((msg.target() != null)
							&& (msg.target() instanceof Item)
							&& (((Item) msg.target()).baseGoldValue() >= CMath
									.s_int(DT.parm1)))
						yup = true;
					break;
				case TRIGGER_SITTING:
		...
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_REBUKE:
				if (msg.source().getWorshipCharID().equals(Name())) {
					...
					if (msg.source().charStats().getCurrentClass().baseClass()
						...
						msg.source().tell(
								"You feel the wrath of " + name() + "!");
						if (!CMSecurity.isDisabled(CMSecurity.DisFlag.LEVELS))
							msg.source().charStats().getCurrentClass()
									.unLevel(msg.source());
					} else {
						msg.source().tell(
								name() + " takes " + xpwrath
										+ " of experience from you.");
						CMLib.leveler().postExperience(msg.source(), null,
								null, -xpwrath, false);
					}
				}
				break;
			...
			if (numBlessings() > 0) {
				...
				if (msg.source().charStats().getCurrentClass().baseClass()
					...
					V = clericTriggers;
				if ((V != null) && (V.size() > 0)) {
					boolean recheck = triggerCheck(msg, V, trigBlessingParts,
							trigBlessingTimes);

					if ((recheck) && (!norecurse)
							&& (!alreadyBlessed(msg.source()))) {
						boolean[] checks = trigBlessingParts.get(msg.source()
								.Name());
						if ((checks != null) && (checks.length == V.size())
								&& (checks.length > 0)) {
			...
			if (numCurses() > 0) {
				...
				if ((V != null) && (V.size() > 0)) {
					boolean recheck = triggerCheck(msg, V, trigCurseParts,
							trigCurseTimes);
					if ((recheck) && (!norecurse)) {
						boolean[] checks = trigCurseParts.get(msg.source()
								.Name());
						if ((checks != null) && (checks.length == V.size())
								&& (checks.length > 0)) {
							boolean rollingTruth = checks[0];
			...
			if ((numPowers() > 0)
					&& (msg.source().charStats().getCurrentClass().baseClass()
							.equals("Cleric") || (CMSecurity.isASysOp(msg
							.source())))) {
				List<DeityTrigger> V = clericPowerTriggers;
				if ((V != null) && (V.size() > 0)) {
					boolean recheck = triggerCheck(msg, V, trigPowerParts,
							trigPowerTimes);

					if ((recheck) && (!norecurse)
							&& (!alreadyPowered(msg.source()))) {
						boolean[] checks = trigPowerParts.get(msg.source()
								.Name());
						if ((checks != null) && (checks.length == V.size())
								&& (checks.length > 0)) {
							boolean rollingTruth = checks[0];
			...
			if ((msg.source().charStats().getCurrentClass().baseClass()
					.equals("Cleric") || (CMSecurity.isASysOp(msg.source())))
					&& (CMLib.law().getClericInfused(msg.source().location()) == this)) {
				List<DeityTrigger> V = serviceTriggers;
				if ((V != null) && (V.size() > 0)) {
					boolean recheck = triggerCheck(msg, V, trigServiceParts,
							trigServiceTimes);

					if ((recheck)
							&& (!norecurse)
							&& (!alreadyServiced(msg.source(), msg.source()
									.location()))) {
						boolean[] checks = trigServiceParts.get(msg.source()
								.Name());
						if ((checks != null) && (checks.length == V.size())
								&& (checks.length > 0)) {
							boolean rollingTruth = checks[0];
							for (int v = 1; v < V.size(); v++) {
								DeityTrigger DT = V.get(v);
								if (rollingTruth)
									startServiceIfNecessary(msg.source(), msg
											.source().location());
								if (DT.previousConnect == CONNECT_AND)
									rollingTruth = rollingTruth && checks[v];
								else
							...
							if (rollingTruth)
								finishService(msg.source(), msg.source()
										.location());
						}
					}
				}
	...
	protected void startServiceIfNecessary(MOB mob, Room room) {
		...
		List<Room> V = CMLib.tracking().getRadiantRooms(room, flags,
				5 + (mob.phyStats().level() / 5));
		for (int v = 0; v < V.size(); v++) {
			R = V.get(v);
			if (CMLib.law().getClericInfused(R) != this)
				for (int m = 0; m < R.numInhabitants(); m++) {
					...
					if (M.getWorshipCharID().equals(Name())) {
						if (!M.isMonster())
							M.tell("Services for " + Name()
									+ " are now starting at "
									+ room.displayText(null) + ".");
						else if (!CMLib.flags().isATrackingMonster(M)) {
							Ability TRACKA = CMClass.getAbility("Skill_Track");
							if (TRACKA != null) {
								TRACKA.invoke(M, CMParms.parse("\""
										+ CMLib.map().getExtendedRoomID(room)
										+ "\""), room, true, 0);
	...
	public boolean finishService(MOB mob, Room room) {
		...
		undoService(service.parishaners);
		int exp = (int) Math.round(CMath.div(totalLevels, mob.phyStats()
				.level()) * 10.0);
	...
	public boolean cancelService(WorshipService service) {
		...
		room.showHappens(CMMsg.MASK_ALWAYS,
				"The service conducted by " + mob.Name()
						+ " has been cancelled.");
	...
	public boolean tick(Tickable ticking, int tickID) {
		if (!super.tick(ticking, tickID))
			return false;
		if ((tickID == Tickable.TICKID_MOB) && ((--rebukeCheckDown) < 0)) {
			rebukeCheckDown = 10;
			for (Enumeration p = CMLib.players().players(); p.hasMoreElements();) {
				...
				if ((!M.isMonster()) && (M.getWorshipCharID().equals(name()))
					...
					if (M.charStats().getCurrentClass().baseClass()
							.equalsIgnoreCase("Cleric")) {
						if (!CMLib.masking().maskCheck(getClericRequirements(),
								M, true)) {
							if ((blacklist == M) && ((++blackmarks) > 30)) {
								CMMsg msg = CMClass
										.getMsg(M, this, null,
												CMMsg.MSG_REBUKE,
												"<S-NAME> <S-HAS-HAVE> been rebuked by <T-NAME>!!");
								if ((M.location() != null)
										&& (M.okMessage(M, msg)))
								...
								if ((blackmarks % 5) == 0)
									M.tell("You feel dirtied by the disappointment of "
											+ name() + ".");
							}
						} else if (blacklist == M) {
							blackmarks = 0;
					...
					} else if (!CMLib.masking().maskCheck(
							getWorshipRequirements(), M, true)) {
						if ((blacklist == M) && ((++blackmarks) > 30)) {
							CMMsg msg = CMClass
									.getMsg(M, this, null, CMMsg.MSG_REBUKE,
											"<S-NAME> <S-HAS-HAVE> been rebuked by <T-NAME>!!");
							...
							lastBlackmark = System.currentTimeMillis();
							if (blackmarks == 1)
								M.tell("Woshipper, you have disappointed "
										+ name()
										+ ". Make amends or face my wrath!");
						}
					} else if (blacklist == M) {
						blackmarks = 0;
						blacklist = null;
		...
		for (Iterator<MOB> m = waitingFor.iterator(); m.hasNext();) {
			try {
				final MOB M = m.next();
				waitingFor.remove(M);
				executeMsg(this, CMClass.getMsg(M, null, null,
	...
	public boolean fetchBlessingCleric(String ID) {
		for (int a = 0; a < numBlessings(); a++) {
			Ability A = fetchBlessing(a);
			if ((A != null)
					&& ((A.ID().equalsIgnoreCase(ID)) || (A.Name()
							.equalsIgnoreCase(ID))))
	...
	public Ability fetchBlessing(String ID) {
		...
		return (Ability) CMLib.english().fetchEnvironmental(
				new ConvertingList<DeityPower, Ability>(blessings,
						new Converter<DeityPower, Ability>() {
				...
				if (trig.length() > 0) {
					...
					if (V.size() > 1) {
						...
						if (cmd.equals("SAY")) {
							...
							DT.parm1 = "" + CMath.s_int(CMParms.combine(V, 1));
						} else if ((cmd.equals("BURNMATERIAL"))
								|| (cmd.equals("BURN"))) {
							DT.triggerCode = TRIGGER_BURNMATERIAL;
							DT.parm1 = CMParms.combine(V, 1);
							int cd = RawMaterial.CODES
									.FIND_StartsWith(DT.parm1);
							boolean found = cd >= 0;
							if (found)
								DT.parm1 = "" + cd;
							else {
							...
							if (!found) {
								Log.errOut("StdDeity", Name()
										+ "- Unknown material: " + trig);
								break;
							}
						} else if (cmd.equals("PUTMATERIAL")) {
							...
							DT.parm1 = CMParms.combine(V, 1);
						} else if (cmd.equals("INROOM")) {
							DT.triggerCode = TRIGGER_INROOM;
							DT.parm1 = CMParms.combine(V, 1);
						} else if (cmd.equals("RIDING")) {
						...
						Log.errOut("StdDeity", Name()
								+ "- Illegal trigger (need more parameters): "
								+ trig);
						break;
					}
				}
	...
	protected static final int[] TRIG_WATCH = { CMMsg.TYP_SPEAK, // 0
			-999, // 1
			CMMsg.TYP_PUT, // 2
			CMMsg.TYP_FIRE, // 3
			CMMsg.TYP_EAT, // 4
	...
	/**
	 * Manipulation of curse objects, which includes spells, traits, skills,
	 * etc.
	 */
	...
	public Ability fetchCurse(String ID) {
		...
		return (Ability) CMLib.english().fetchEnvironmental(
				new ConvertingList<DeityPower, Ability>(curses,
						new Converter<DeityPower, Ability>() {
							public Ability convert(DeityPower obj) {
								return obj.power;
							}
	...
	public String getClericSinDesc() {
		if (numCurses() > 0)
			return "The curses of " + name() + " are placed upon "
					+ charStats().hisher()
					+ " clerics whenever the cleric does the following: "
					+ getTriggerDesc(clericCurseTriggers) + ".";
		return "";
	}
	...
	public String getWorshipSinDesc() {
		if (numCurses() > 0)
			return "The curses of "
					+ name()
					+ " are placed upon "
					+ charStats().hisher()
					+ " worshippers whenever the worshipper does the following: "
					+ getTriggerDesc(clericCurseTriggers) + ".";
		return "";
	}

	/**
	 * Manipulation of granted clerical powers, which includes spells, traits,
	 * skills, etc.
	 */
	...
	public String getClericPowerupDesc() {
		if (numPowers() > 0)
			return "Special powers of " + name() + " are bestowed to "
					+ charStats().hisher()
					+ " clerics whenever the cleric does the following: "
					+ getTriggerDesc(clericPowerTriggers) + ".";
		return "";
	}
}
