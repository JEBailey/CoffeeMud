				...
				if (V.size() > 0) {
					...
					if (s.startsWith("DAY"))
						...
						if (s.startsWith("HOUR"))
							...
							budgetTickDown = location().getArea().getTimeObj()
									.getDaysInWeek()
									* CMProps
											.getIntVar(CMProps.Int.TICKSPERMUDDAY);
		...
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_SELL: {
				if (!CMLib.coffeeShops().ignoreIfNecessary(msg.source(),
					...
					return false;
				if (CMLib.coffeeShops().standardSellEvaluation(this,
			...
			case CMMsg.TYP_BID: {
				CMLib.commands().postSay(this, msg.source(),
						"I'm afraid my prices are firm.", false, false);
				return false;
			}
			case CMMsg.TYP_BUY:
			case CMMsg.TYP_VIEW: {
				...
				if ((msg.targetMinor() == CMMsg.TYP_BUY)
					...
					return false;
				if (CMLib.coffeeShops().standardBuyEvaluation(this,
		...
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_GIVE:
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					if ((msg.tool() != null)
							&& ((CMSecurity.isAllowed(msg.source(), location(),
									CMSecurity.SecFlag.ORDER)
									|| (CMLib.law().doesHavePriviledgesHere(
											msg.source(), getStartRoom()))
									|| (CMSecurity.isAllowed(msg.source(),
											location(),
											CMSecurity.SecFlag.CMDMOBS) && (isMonster())) || (CMSecurity
									.isAllowed(msg.source(), location(),
											CMSecurity.SecFlag.CMDROOMS) && (isMonster()))))
							&& ((doISellThis(msg.tool())) || (isSold(DEAL_INVENTORYONLY)))) {
						CMLib.commands().postSay(
								this,
								msg.source(),
								"Yes, I will now sell " + msg.tool().name()
										+ ".", false, false);
									...
									// from a player
			{
				super.executeMsg(myHost, msg);
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					double paid = CMLib.coffeeShops().transactPawn(this,
							msg.source(), this, msg.tool());
					if (paid > Double.MIN_VALUE) {
						budgetRemaining = budgetRemaining - Math.round(paid);
						if (mySession != null)
							mySession.stdPrintln(msg.source(), msg.target(),
									msg.tool(), msg.targetMessage());
								...
								// shopkeeper
			{
				super.executeMsg(myHost, msg);
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					MOB mobFor = CMLib.coffeeShops().parseBuyingFor(
							msg.source(), msg.targetMessage());
					if ((msg.tool() != null)
							&& (getShop().doIHaveThisInStock("$"
						...
						Environmental item = getShop().getStock(
								"$" + msg.tool().Name() + "$", mobFor);
						if (item != null)
							CMLib.coffeeShops().transactMoneyOnly(this,
									msg.source(), this, item, !isMonster());
						...
						if (product instanceof Item) {
							if (!CMLib.coffeeShops().purchaseItems(
									(Item) product, products, this, mobFor))
								return;
							...
							CMLib.coffeeShops().purchaseAbility(
									(Ability) product, this, this, mobFor);
			...
			case CMMsg.TYP_LIST: {
				...
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					String forMask = CMLib.coffeeShops().getListForMask(
							msg.targetMessage());
					List<Environmental> inventory = new XVector(getShop()
							.getStoreInventory());
					inventory = CMLib.coffeeShops().addRealEstateTitles(
							inventory, mob, getShop(), getStartRoom());
					int limit = CMParms.getParmInt(finalPrejudiceFactors(),
							"LIMIT", 0);
					String s = CMLib.coffeeShops().getListInventory(this, mob,
							inventory, limit, this, forMask);
					if (s.length() > 0)
						mob.tell(s);
				}
				break;
