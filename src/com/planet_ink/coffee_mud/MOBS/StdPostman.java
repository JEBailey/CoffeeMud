...
public class StdPostman extends StdShopKeeper implements PostOffice {
	...
	public void addToBox(String mob, Item thisThang, String from, String to,
			long holdTime, double COD) {
	...
	public boolean delFromBox(String boxName, Item thisThang) {
		...
		for (int v = V.size() - 1; v >= 0; v--) {
			...
			if ((PD != null) && (PD.key.startsWith(postalBranch() + ";"))) {
	...
	public Map<String, String> getOurOpenBoxes(String boxName) {
		...
		List<PlayerData> V = CMLib.database()
				.DBReadData(boxName, postalChain());
		if (V == null)
			return branches;
	...
	public void createBoxHere(String boxName, String forward) {
		if (!getOurOpenBoxes(boxName).containsKey(postalBranch())) {
			CMLib.database().DBCreateData(boxName, postalChain(),
					postalBranch() + "/" + forward, "50");
		}
	}
	...
	protected double getSimplePostage(int chargeableWeight) {
		if (getStartRoom() == null)
			return 0.0;
		return minimumPostage()
				+ CMath.mul(postagePerPound(), chargeableWeight);
	}
	...
	protected double getHoldingCost(MailPiece data, int chargeableWeight) {
		...
		if (getStartRoom() == null)
			return 0.0;
		double amt = 0.0;
		TimeClock TC = (getStartRoom() == null) ? CMLib.time().globalClock()
				: getStartRoom().getArea().getTimeObj();
		long time = System.currentTimeMillis() - CMath.s_long(data.time);
		long millisPerMudMonth = TC.getDaysInMonth()
				* CMProps.getMillisPerMudHour() * TC.getHoursInDay();
		if (time <= 0)
			return amt;
		amt += CMath.mul(CMath.mul(
				Math.floor(CMath.div(time, millisPerMudMonth)),
				holdFeePerPound()), chargeableWeight);
		return amt;
	}
	...
	protected String getBranchPostableTo(String toWhom, String branch,
			Map<String, String> allBranchBoxes) {
	...
	public String findProperBranch(String toWhom) {
		if (CMLib.players().getLoadPlayer(toWhom) != null) {
			...
			if (M.getStartRoom() != null) {
				...
				PostOffice P = CMLib.map().getPostOffice(postalChain(),
						M.getStartRoom().getArea().Name());
				String branch = null;
				if (P != null) {
					branch = getBranchPostableTo(toWhom, P.postalBranch(),
							allBranchBoxes);
					if (branch != null)
						return branch;
				...
				branch = getBranchPostableTo(toWhom, postalBranch(),
						allBranchBoxes);
				if (branch != null)
					return branch;
				for (String tryBranch : allBranchBoxes.keySet()) {
					branch = getBranchPostableTo(toWhom, tryBranch,
							allBranchBoxes);
					if (branch != null)
						return branch;
				}
			...
			Map<String, String> allBranchBoxes = getOurOpenBoxes(toWhom);
			String branch = getBranchPostableTo(toWhom, postalBranch(),
					allBranchBoxes);
			if (branch != null)
				return branch;
			for (String tryBranch : allBranchBoxes.keySet()) {
	...
	public long postalWaitTime() {
		if ((postalWaitTime < 0) && (getStartRoom() != null))
			...
			postalWaitTime = (getStartRoom().getArea().getTimeObj()
					.getHoursInDay()) * CMProps.getMillisPerMudHour();
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((tickID == Tickable.TICKID_MOB) && (getStartRoom() != null)) {
			...
			Long L = (Long) postalTimes.get(postalChain() + "/"
					+ postalBranch());
			...
			if (proceed) {
				...
				for (int v = 0; v < parsed.size(); v++) {
					...
					if (deliveryBranch != null) {
						P = CMLib.map().getPostOffice(postalChain(),
								deliveryBranch);
						Item I = makeItem(V2);
						if ((P != null) && (I != null)) {
							P.addToBox(toWhom, I, V2.from, V2.to,
									CMath.s_long(V2.time),
									CMath.s_double(V2.cod));
							continue;
						}
					}
					...
					if (deliveryBranch != null) {
						...
						Item I = makeItem(V2);
						if ((P != null) && (I != null))
							P.addToBox(fromWhom, I, V2.to, "POSTMASTER",
						...
						if (V2.key.startsWith(postalBranch() + ";")) {
							...
							if ((data != null) && (getStartRoom() != null)) {
								long time = System.currentTimeMillis()
										- CMath.s_long(data.time);
								long millisPerMudMonth = TC.getDaysInMonth()
										* CMProps.getMillisPerMudHour()
										* TC.getHoursInDay();
								if (time > 0) {
									int months = (int) Math.round(Math
											.floor(CMath.div(time,
													millisPerMudMonth)));
									if (months > maxMudMonthsHeld()) {
										Item I = makeItem(data);
										CMLib.database().DBDeleteData(V2.who,
												V2.section, V2.key);
										if (I != null)
		...
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_DEPOSIT:
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					...
					if ((!msg.source().isMonster()) && (S != null)
							&& (msg.tool() instanceof Item)) {
						autoGive(msg.source(), this, (Item) msg.tool());
						if (isMine(msg.tool())) {
							final StdPostman me = this;
							S.prompt(new InputCallback(
									InputCallback.Type.PROMPT, "", 0) {
								@Override
								public void showPrompt() {
									S.promptPrint("Address this to whom? ");
								}

								@Override
								public void timedOut() {
									autoGive(me, msg.source(),
											(Item) msg.tool());
								}
								...
								public void callBack() {
									if ((this.input != null)
											...
											&& ((CMLib.players().getLoadPlayer(
													this.input) != null) || (CMLib
													.clans().findClan(
															this.input) != null))) {
										final String fromWhom = getSenderName(
												msg.source(),
												Clan.Function.DEPOSIT, false);
										final String toWhom;
										if (CMLib.players().getLoadPlayer(
												this.input) != null)
											toWhom = CMLib.players()
													.getLoadPlayer(this.input)
													.Name();
										else
											toWhom = CMLib.clans()
													.findClan(this.input)
													.name();
										final double amt = getSimplePostage(getChargeableWeight((Item) msg
												.tool()));
										S.prompt(new InputCallback(
												InputCallback.Type.CHOOSE, "P",
												"CP\n", 0) {
											@Override
											public void showPrompt() {
												S.promptPrint("Postage on this will be "
														+ CMLib.beanCounter()
																.nameCurrencyShort(
																		me, amt)
											...
											public void callBack() {
												...
												if (choice.startsWith("C")) {
													S.prompt(new InputCallback(
														...
														@Override
														public void showPrompt() {
															S.promptPrint("Enter COD amount ("
																	+ CMLib.beanCounter()
																			.getDenominationName(
																					CMLib.beanCounter()
																							.getCurrency(
															...
															if ((CODstr
																...
																CMLib.commands()
																		.postSay(
																				me,
																				mob,
																				"I'll deliver that for ya right away!",
																				true,
																				false);
																((Item) msg
																		.tool())
										...
										CMLib.commands()
												.postSay(
														me,
														mob,
														"That is not a valid player or clan name.",
														true, false);
										autoGive(me, msg.source(),
												(Item) msg.tool());
									}
			...
			case CMMsg.TYP_WITHDRAW:
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					String fromWhom = getSenderName(msg.source(),
							Clan.Function.WITHDRAW, false);
					Item old = (Item) msg.tool();
					MailPiece data = findExactBoxData(fromWhom,
							(Item) msg.tool());
					if ((data == null)
							&& (!isSold(ShopKeeper.DEAL_CLANPOSTMAN))
							&& (msg.source().isMarriedToLiege()))
						data = findExactBoxData(msg.source().getLiegeID(),
								(Item) msg.tool());
					if (data == null)
						CMLib.commands().postSay(this, mob,
								"You want WHAT? Try LIST.", true, false);
					else {
						...
						if ((totalCharge > 0.0)
								&& (CMLib.beanCounter()
										.getTotalAbsoluteShopKeepersValue(
												msg.source(), this) >= totalCharge)) {

							CMLib.beanCounter().subtractMoney(msg.source(),
									totalCharge);
							double COD = CMath.s_double(data.cod);
							Coins returnMoney = null;
							if (COD > 0.0)
								returnMoney = CMLib.beanCounter()
										.makeBestCurrency(this, COD);
							if (returnMoney != null) {
								CMLib.commands().postSay(
										this,
										mob,
										"The COD amount of "
												...
												+ data.from + ".", true, false);
								addToBox(postalChain(), returnMoney, data.to,
										data.from, System.currentTimeMillis(),
										0.0);
								CMLib.commands().postSay(
										this,
										mob,
										"The total charge on that was a COD charge of "
												+ returnMoney.Name()
												+ " plus "
												+ CMLib.beanCounter()
														.nameCurrencyShort(
																...
																totalCharge
																		- COD)
												+ " postage and holding fees.",
										true, false);
							} else
								CMLib.commands().postSay(
										this,
										mob,
										"The total charge on that was "
												+ CMLib.beanCounter()
														.nameCurrencyShort(
																this,
																totalCharge)
												+ " in holding/storage fees.",
										true, false);
						}
						CMLib.commands().postSay(this, mob, "There ya go!",
								true, false);
						if (location() != null)
			...
			case CMMsg.TYP_SPEAK: {
				...
				if ((str != null) && (str.trim().equalsIgnoreCase("open"))) {
					String theName = getSenderName(msg.source(),
							Clan.Function.DEPOSIT, false);
					if (getOurOpenBoxes(theName).containsKey(postalBranch())) {
						if (isSold(ShopKeeper.DEAL_CLANPOSTMAN))
							CMLib.commands().postSay(
									this,
									mob,
									CMStrings.capitalizeFirstLetter(theName)
											+ " already has a box open here!",
									true, false);
						else
							CMLib.commands().postSay(this, mob,
									"You already have a box open here!", true,
									false);
					} else if (feeForNewBox() > 0.0) {
						if (CMLib.beanCounter()
								.getTotalAbsoluteShopKeepersValue(msg.source(),
										this) < feeForNewBox()) {
							CMLib.commands()
									.postSay(this, mob,
						...
						if (isSold(ShopKeeper.DEAL_CLANPOSTMAN))
							CMLib.commands()
									.postSay(
											this,
											mob,
											"A box has been opened for "
													+ CMStrings
															.capitalizeFirstLetter(theName)
													+ ".", true, false);
						else
							CMLib.commands().postSay(this, mob,
									"A box has been opened for you.", true,
									false);
					} else {
						createBoxHere(theName, theName);
						if (isSold(ShopKeeper.DEAL_CLANPOSTMAN))
						...
						&& (str.trim().equalsIgnoreCase("close"))) {
					String theName = getSenderName(msg.source(),
							Clan.Function.WITHDRAW, false);
					if (!getOurOpenBoxes(theName).containsKey(postalBranch())) {
						if (isSold(ShopKeeper.DEAL_CLANPOSTMAN))
							CMLib.commands().postSay(
									...
									mob,
									CMStrings.capitalizeFirstLetter(theName)
											+ " does not have a box here!",
									true, false);
						else
							CMLib.commands().postSay(this, mob,
									"You don't have a box open here!", true,
									false);
						...
						CMLib.commands()
								.postSay(
										this,
										mob,
										"That box has pending items which must be removed first.",
										true, false);
					else {
						deleteBoxHere(theName);
						CMLib.commands().postSay(this, mob,
								"That box is now closed.", true, false);
					}
				} else if ((str != null)
						&& (str.toUpperCase().trim().startsWith("FORWARD"))) {
					String theName = getSenderName(msg.source(),
							Clan.Function.WITHDRAW, false);
					...
					if (!getOurOpenBoxes(theName).containsKey(postalBranch())) {
						...
						if (A == null)
							CMLib.commands().postSay(
									this,
									mob,
									"I don't know of an area called '" + str
											+ "'.", true, false);
						else {
							PostOffice P = CMLib.map().getPostOffice(
									postalChain(), A.Name());
							if (P == null)
								CMLib.commands().postSay(
										this,
										mob,
										"I'm sorry, we don't have a branch in "
												+ A.name() + ".", true, false);
							else if (!P.getOurOpenBoxes(theName).containsKey(
									P.postalBranch())) {
								if (isSold(ShopKeeper.DEAL_CLANPOSTMAN))
									CMLib.commands()
											.postSay(
													...
													"I'm sorry, "
															+ CMStrings
																	.capitalizeFirstLetter(theName)
															+ " does not have a box at our branch in "
															+ A.name() + ".",
													true, false);
								else
									CMLib.commands().postSay(
											this,
											mob,
											"I'm sorry, you don't have a box at our branch in "
													+ A.name() + ".", true,
											false);
							} else {
								deleteBoxHere(theName);
								...
								CMLib.commands().postSay(
										this,
										mob,
										"Ok, mail will now be forwarded to our branch in "
												+ A.name() + ".", true, false);
							}
						}
					}
			...
			case CMMsg.TYP_LIST: {
				...
				if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
					...
					if (getStartRoom() != null)
						C = getStartRoom().getArea().getTimeObj();
					...
					if (V.size() == 0)
						...
						for (int i = 0; i < V.size(); i++) {
							DatabaseEngine.PlayerData PD = (DatabaseEngine.PlayerData) V
									.elementAt(i);
												...
												.padRight(
														""
																+ CMLib.beanCounter()
																		.abbreviatedPrice(
																				this,
																				getCODChargeForPiece(pieces)),
														8) + "]");
							} else
								str.append("[        ]");
							str.append("["
									+ CMStrings.padRight(pieces.from, 15) + "]");
							TimeClock C2 = C.deriveClock(CMath
									.s_long(pieces.time));
							str.append("["
									+ CMStrings.padRight(
											C2.getShortestTimeDescription(), 15)
									+ "]");
					...
					str.append("* This branch charges minimum "
							+ CMLib.beanCounter().nameCurrencyShort(this,
									minimumPostage())
							+ " postage for first pound.\n\r");
					str.append("* An additional "
							+ CMLib.beanCounter().nameCurrencyShort(this,
									postagePerPound())
							+ " per pound is charged for packages.\n\r");
					str.append("* A charge of "
							+ CMLib.beanCounter().nameCurrencyShort(this,
									holdFeePerPound())
							+ " per pound per month is charged for holding.\n\r");
					str.append("* To forward your mail, 'say \"" + name()
							+ "\" \"forward <areaname>\"'.\n\r");
					str.append("* To close your box, 'say \"" + name()
							+ "\" close'.\n\r");
					mob.tell(str.toString());
				}
				return;
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.targetMinor() == CMMsg.TYP_EXPIRE)
			...
			case CMMsg.TYP_DEPOSIT: {
				...
				if (msg.tool() == null)
					return false;
				String senderName = getSenderName(msg.source(),
				...
				if (CMLib.flags().isEnspelled((Item) msg.tool())
					...
					mob.tell(this, msg.tool(), null,
							"<S-HE-SHE> refuses to accept <T-NAME> for delivery.");
					return false;
				}
			}
				return true;
			case CMMsg.TYP_WITHDRAW: {
				...
				if ((data == null) && (!isSold(ShopKeeper.DEAL_CLANPOSTMAN))
						&& (msg.source().isMarriedToLiege()))
					data = findExactBoxData(msg.source().getLiegeID(),
							(Item) msg.tool());
				if (data == null) {
				...
				double totalCharge = getCODChargeForPiece(data);
				if (CMLib.beanCounter().getTotalAbsoluteShopKeepersValue(
						msg.source(), this) < totalCharge) {
					CMLib.commands().postSay(
							this,
							mob,
							"The total charge to receive that item is "
									+ CMLib.beanCounter().nameCurrencyShort(
											this, totalCharge)
									+ ". You don't have enough.", true, false);
					return false;
				}
			}
				return true;
			...
			case CMMsg.TYP_LIST: {
				...
				if ((!getOurOpenBoxes(senderName).containsKey(postalBranch()))
						&& ((isSold(ShopKeeper.DEAL_CLANPOSTMAN))
								|| (!msg.source().isMarriedToLiege()) || (!getOurOpenBoxes(
									msg.source().getLiegeID()).containsKey(
								postalBranch())))) {
					if ((!isSold(ShopKeeper.DEAL_CLANPOSTMAN))
							&& (msg.source().getStartRoom().getArea() == getStartRoom()
									.getArea())) {
					...
					StringBuffer str = new StringBuffer("");
					if (isSold(ShopKeeper.DEAL_CLANPOSTMAN))
						str.append(CMStrings.capitalizeFirstLetter(senderName)
								+ " does not have a postal box at this branch, I'm afraid.");
					else
						str.append("You don't have a postal box at this branch, I'm afraid.");
					if (postalChain().length() > 0)
						str.append("\n\rThis branch is part of the "
								+ postalChain() + " postal chain.");
					CMLib.commands().postSay(this, mob, str.toString() + "^T",
							true, false);
					mob.tell("Use 'say \""
							+ name()
							+ "\" open' to open a box here"
							+ ((feeForNewBox() <= 0.0) ? "." : (" for "
									+ CMLib.beanCounter().nameCurrencyShort(
											this, feeForNewBox()) + ".")));
					return false;
				}
