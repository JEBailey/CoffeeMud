...
public class StdMOB implements MOB {
	...
	protected CMUniqSortSVec<Behavior> behaviors = new CMUniqSortSVec<Behavior>(
			1);
	...
	protected SHashtable<String, FData> factions = new SHashtable<String, FData>(
			1);
	...
	public int getExpPrevLevel() {
		if (basePhyStats().level() <= 1)
			return 0;
		int neededLowest = CMLib.leveler().getLevelExperience(
	...
	public int getExpNeededLevel() {
		if ((CMProps.getIntVar(CMProps.Int.LASTPLAYERLEVEL) > 0)
				&& (CMProps.getIntVar(CMProps.Int.LASTPLAYERLEVEL) <= basePhyStats()
						.level()))
	...
	public String getFactionListing() {
		...
		for (final Enumeration e = fetchFactions(); e.hasMoreElements();) {
			final Faction F = CMLib.factions().getFaction(
	...
	public String genericName() {
		if (charStats().getStat(CharStats.STAT_AGE) > 0)
			return charStats().ageName().toLowerCase() + " "
					+ charStats().raceName().toLowerCase();
	...
	public String image() {
		...
		if (!baseCharStats().getMyRace().name()
				.equalsIgnoreCase(charStats().raceName()))
	...
	protected void cloneFix(MOB M) {
		...
		for (final Enumeration<Ability> a = M.personalEffects(); a
				.hasMoreElements();) {
			A = a.nextElement();
			if (A != null) {
				A = (Ability) A.copyOf();
		...
		for (final Enumeration<ScriptingEngine> e = M.scripts(); e
				.hasMoreElements();) {
			SE = e.nextElement();
			if (SE != null)
	...
	public CMObject copyOf() {
		try {
			...
			// & perf
			E.xtraValues = (xtraValues == null) ? null : (String[]) xtraValues
					.clone();
			E.cloneFix(this);
	...
	public void recoverPhyStats() {
		...
		if (getMoney() > 0)
			phyStats().setWeight(
					phyStats().weight()
							+ (int) Math.round(CMath.div(getMoney(), 100.0)));
		...
		if (!CMLib.flags().isLightSource(this)) {
			for (final Enumeration<Follower> f = followers(); f
					.hasMoreElements();)
				if (CMLib.flags().isLightSource(f.nextElement().follower))
					phyStats.setDisposition(phyStats().disposition()
	...
	public int maxCarry() {
		...
		return (int) Math.round(bodyWeight
				+ ((str + 10.0) * str * bodyWeight / 150.0) + (str * 5.0));
	}
	...
	public int maxFollowers() {
		return ((int) Math.round(CMath.div(
				charStats().getStat(CharStats.STAT_CHARISMA) - 8, 4.0)) + 1);
	}
	...
	public void recoverCharStats() {
		...
		if ((playerStats != null) && (soulMate == null)
				&& (playerStats.getHygiene() >= PlayerStats.HYGIENE_DELIMIT)) {
			...
			if ((charStats.getStat(CharStats.STAT_CHARISMA) / 2) > chaAdjust)
				charStats.setStat(CharStats.STAT_CHARISMA,
						charStats.getStat(CharStats.STAT_CHARISMA) - chaAdjust);
			else
				charStats.setStat(CharStats.STAT_CHARISMA,
						charStats.getStat(CharStats.STAT_CHARISMA) / 2);
		}
	}
		...
		if ((location != null) && (location.isInhabitant(this))) {
			...
			if ((session() != null)
				...
				location().show(this, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> vanish(es) in a puff of smoke.");
		}
		if (playerStats != null)
			CMLib.players().changePlayersLocation(this, null);
		setFollowing(null);
			...
			if (follower != null) {
				if ((follower.isMonster()) && (!follower.isPossessing()))
					oldFollowers.addElement(follower,
							Integer.valueOf(fetchFollowerOrder(follower)));
	...
	public void bringToLife(Room newLocation, boolean resetStats) {
		...
		if ((miscText != null) && (resetStats) && (isGeneric())) {
			if (CMProps.getBoolVar(CMProps.Bool.MOBCOMPRESS)
				...
				CMLib.coffeeMaker().resetGenMOB(
						this,
						CMLib.coffeeMaker().getGenMOBTextUnpacked(
								this,
								CMLib.encoder().decompressString(
										(byte[]) miscText)));
			else
				CMLib.coffeeMaker().resetGenMOB(
						this,
						CMLib.coffeeMaker().getGenMOBTextUnpacked(this,
								CMStrings.bytesToStr(miscText)));
		}
		if (CMLib.map().getStartRoom(this) == null)
			setStartRoom(isMonster() ? newLocation : CMLib.login()
					.getDefaultStartRoom(this));
		...
		if (session() != null) {
			...
			if (area != null)
				CMLib.login().moveSessionToCorrectThreadGroup(session(),
						area.getTheme());
		}
		...
		if (location() == null) {
			Log.errOut("StdMOB", name() + " of "
					+ CMLib.map().getExtendedRoomID(newLocation)
					+ " was auto-destroyed!");
			destroy();
			return;
		}
	...
	public boolean isInCombat() {
		...
		try {
			...
			if ((vicR == null) || (location() == null) || (vicR != location())
					|| (victim.amDead())) {
	...
	protected boolean isPermissableToFight(final MOB mob) {
		...
		if (CMath.bset(getBitmap(), MOB.ATT_PLAYERKILL)) {
			if (CMSecurity
					.isAllowed(this, location(), CMSecurity.SecFlag.PKILL)
	...
	public boolean mayPhysicallyAttack(MOB mob) {
		if ((!mayIFight(mob)) || (location() != mob.location())
	...
	public void setVictim(MOB mob) {
		...
		recoverMaxState();
		if (mob != null) {
			if ((mob.location() == null) || (location() == null)
					|| (mob.amDead()) || (amDead())
				...
				if (Log.combatChannelOn()) {
					...
					Log.combatOut("STRT",
							Name() + ":" + phyStats().getCombatStats() + ":"
									+ curState().getCombatStats() + ":"
									+ ((I == null) ? "null" : I.name()) + ":"
									+ mob.Name() + ":"
									+ mob.phyStats().getCombatStats() + ":"
									+ mob.curState().getCombatStats() + ":"
	...
	public void setRiding(Rideable ride) {
		...
		if ((ride != null) && (amRiding != null) && (amRiding == ride)
	...
	public String displayText(MOB viewerMob) {
		if ((displayText.length() == 0)
				...
				|| (riding() != null)
				|| ((amFollowing() != null) && (amFollowing()
						.fetchFollowerOrder(this) > 0))
				|| ((this instanceof Rideable)
						&& (((Rideable) this).numRiders() > 0) && CMLib.flags()
						.hasSeenContents(this)) || (isInCombat())) {
			...
			sendBack.append(" ");
			sendBack.append(CMLib.flags().dispositionString(this,
					CMFlagLibrary.flag_is));
			...
			if (riding() != null) {
				...
				if (riding() == viewerMob)
					...
					&& (((Rideable) this).stateStringSubject(
							((Rideable) this).fetchRider(0)).length() > 0)) {
				...
				for (int r = 0; r < me.numRiders(); r++) {
					...
					if ((rider != null)
							&& (me.stateStringSubject(rider).equals(first))) {
						if (r > 0) {
							sendBack.append(", ");
							if (r == me.numRiders() - 1)
								sendBack.append("and ");
			...
			if ((isInCombat()) && (CMLib.flags().canMove(this))
					&& (!CMLib.flags().isSleeping(this))) {
			...
			if ((amFollowing() != null)
				...
				final List<MOB> whoseAhead = CMLib.combat()
						.getFormationFollowed(this);
	...
	public String healthText(MOB viewer) {
		final String mxp = "^<!ENTITY vicmaxhp \"" + maxState().getHitPoints()
				...
				+ "\"^>^<Health^>^<HealthText \""
				+ CMStrings.removeColors(name(viewer)) + "\"^>";
		if ((charStats() != null) && (charStats().getMyRace() != null))
			return mxp + charStats().getMyRace().healthText(viewer, this)
					+ "^</HealthText^>";
		return mxp + CMLib.combat().standardMobCondition(viewer, this)
				+ "^</HealthText^>";
	}
	...
	public boolean dequeCommand() {
		while ((!removeFromGame) && (!amDestroyed)
			...
			synchronized (commandQue) {
				...
				if (diff >= 0.0) {
					final long nextTime = lastCommandTime
							+ Math.round(cmd.actionDelay / phyStats().speed()
			...
			if (doCommand != null) {
				...
				doCommand(doCommand.commandObj, doCommand.commandVector,
						doCommand.metaFlags);
				synchronized (commandQue) {
					if (commandQue.size() > 0) {
						final QMCommand cmd = commandQue.getFirst();
						final Object O = cmd.commandObj;
						cmd.actionDelay = calculateActionDelay(O,
								cmd.commandVector, 0.0);
					} else
						return false;
					return true;
				}
			...
			synchronized (commandQue) {
				...
				if ((cmd == null)
						|| (System.currentTimeMillis() < cmd.nextCheck))
					return false;
			}
			...
			try {
				if (O instanceof Command) {
					if (!((Command) O).preExecute(this, commands, metaFlags,
							secondsElapsed, -diff)) {
	...
	protected double calculateActionDelay(Object command,
			List<String> commands, double tickDelay) {
		if (tickDelay <= 0.0) {
			if (command == null) {
				tell("Huh?!");
				return -1.0;
			...
			if (command instanceof Command)
				tickDelay = ((Command) command).checkedActionsCost(this,
						commands);
			else if (command instanceof Ability)
				tickDelay = ((Ability) command).checkedCastingCost(this,
						commands);
			else
				tickDelay = 1.0;
		}
	...
	public void enqueCommand(List<String> commands, int metaFlags,
			double tickDelay) {
		if (commands == null)
			return;
		final CMObject O = CMLib.english().findCommand(this, commands);
		if ((O == null)
				|| ((O instanceof Ability)
						&& CMath.bset(metaFlags, Command.METAFLAG_ORDER) && CMath
							.bset(((Ability) O).flags(),
									Ability.FLAG_NOORDERING))) {
			CMLib.commands().handleUnknownCommand(this, commands);
			return;
		}
		tickDelay = calculateActionDelay(O, commands, tickDelay);
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		for (final Enumeration<ScriptingEngine> s = scripts(); s
				.hasMoreElements();) {
		...
		if ((msg.sourceCode() != CMMsg.NO_EFFECT) && (msg.amISource(this))) {
			if ((msg.sourceMinor() == CMMsg.TYP_DEATH)
					&& (CMSecurity.isAllowed(this, location(),
							CMSecurity.SecFlag.IMMORT))) {
				curState().setHitPoints(1);
				if ((msg.tool() != null) && (msg.tool() != this)
						&& (msg.tool() instanceof MOB))
					((MOB) msg.tool()).tell(name((MOB) msg.tool())
							+ " is immortal, and can not die.");
			...
			if (!msg.sourceMajor(CMMsg.MASK_ALWAYS)) {
				...
				if (CMath.bset(srcCode, CMMsg.MASK_MALICIOUS)) {
					if ((msg.target() != this) && (msg.target() != null)
							&& (msg.target() instanceof MOB)) {
						final MOB target = (MOB) msg.target();
						if ((amFollowing() != null)
								&& (target == amFollowing())) {
							tell("You like "
									+ amFollowing().charStats().himher()
									+ " too much.");
							return false;
						}
						if ((getLiegeID().length() > 0)
								&& (target.Name().equals(getLiegeID()))) {
							if (isMarriedToLiege())
						...
						CMLib.combat().establishRange(this, (MOB) msg.target(),
								msg.tool());
					}
				}
				...
				if (CMath.bset(srcCode, CMMsg.MASK_MOUTH)) {
					if (((srcMinor != CMMsg.TYP_LIST) || srcM.amDead() || CMLib
					...
					if (CMath.bset(srcCode, CMMsg.MASK_SOUND)) {
						...
						if ((!CMLib.flags().canBeSeenBy(msg.target(), this))
							...
							srcM.tell("You don't see '"
									+ ((Item) msg.target()).name(this)
									+ "' here.");
							return false;
						}
						if (!CMLib.flags().canTaste(this)) {
							if ((msg.sourceMinor() == CMMsg.TYP_EAT)
									|| (msg.sourceMinor() == CMMsg.TYP_DRINK))
				...
				if (CMath.bset(srcCode, CMMsg.MASK_HANDS)) {
					...
					if ((CMLib.flags().isSitting(this))
							...
							&& ((msg.sourceMessage() != null) || (msg
									.othersMessage() != null))
							&& (((!CMLib.utensils().reachableItem(this,
									msg.target())) || (!CMLib.utensils()
									.reachableItem(this, msg.tool())))
									&& (location() != null) && (!CMath.bset(
									location().phyStats().sensesMask(),
				...
				case CMMsg.TYP_CLOSE:
					if (isInCombat()) {
						if ((msg.target() != null)
								&& ((msg.target() instanceof Exit) || (srcM
										.isMine(msg.target()))))
							break;
						tell("Not while you are fighting!");
						return false;
				...
				case CMMsg.TYP_LEAVE:
					if ((isInCombat()) && (location() != null)
						...
						for (final Enumeration<MOB> m = location()
								.inhabitants(); m.hasMoreElements();) {
							MOB M = m.nextElement();
							if ((M != this)
									&& (M.getVictim() == this)
									&& (CMLib.flags().aliveAwakeMobile(M, true))
									&& (CMLib.flags().canSenseEnteringLeaving(
											srcM, M))) {
								tell("Not while you are fighting!");
								return false;
							}
					...
					if ((msg.target() == null)
						...
						if (msg.target() != null) {
							if ((msg.target() instanceof MOB)
									&& (!CMLib.flags().canBeHeardSpeakingBy(
											this, (MOB) msg.target()))) {
								tell(((Physical) msg.target()).name(this)
										+ " can't hear you!");
								return false;
								...
								// should work.
							} else if ((!((msg.target() instanceof MOB) && (((MOB) msg
									.target()).getLiegeID().equals(Name()))))
									&& (!msg.target().Name()
											.equals(getLiegeID()))) {
								tell(((Physical) msg.target()).name(this)
										+ " does not serve you, and you do not serve "
										+ ((Physical) msg.target()).name(this)
										+ ".");
								return false;
							} else if ((msg.target() instanceof MOB)
									&& (((MOB) msg.target()).getLiegeID()
											.equals(Name()))
									&& (getLiegeID()
											.equals(msg.target().Name()))
									&& (((MOB) msg.target()).isMarriedToLiege())) {
								tell("You cannot rebuke "
										+ ((Physical) msg.target()).name(this)
				...
				case CMMsg.TYP_SERVE:
					...
					if (msg.target() instanceof Deity)
						break;
					if ((msg.target() instanceof MOB)
							&& (!CMLib.flags().canBeHeardSpeakingBy(this,
									(MOB) msg.target()))) {
						tell(((Physical) msg.target()).name(this)
								+ " can't hear you!");
						return false;
		...
		if ((msg.sourceCode() != CMMsg.NO_EFFECT) && (msg.amISource(this))
				&& (msg.target() != null) && (msg.target() != this)
			...
			if (((msg.targetMinor() == CMMsg.TYP_WEAPONATTACK) && (rangeToTarget() > maxRange(msg
					.tool())))
					|| ((msg.sourceMinor() == CMMsg.TYP_THROW)
							&& (rangeToTarget() > 2) && (maxRange(msg.tool()) <= 0))) {
				String newstr = "<S-NAME> advance(s) at ";
				msg.modify(this, trgM, null, CMMsg.MSG_ADVANCE,
						newstr + trgM.name(this), CMMsg.MSG_ADVANCE, newstr
								+ "you", CMMsg.MSG_ADVANCE,
						newstr + trgM.name());
				...
				if ((useRange >= 0) && (maxRange(tool) < useRange)) {
					srcM.tell("You are too far away from " + trgM.name(srcM)
							+ " to use " + tool.name() + ".");
					return false;
				} else if ((useRange >= 0) && (minRange(tool) > useRange)) {
					srcM.tell("You are too close to " + trgM.name(srcM)
							+ " to use " + tool.name() + ".");
					if ((msg.targetMinor() == CMMsg.TYP_WEAPONATTACK)
							&& (tool instanceof Weapon)
							&& (!((Weapon) tool)
									.amWearingAt(Wearable.IN_INVENTORY)))
						CMLib.commands().postRemove(this, (Weapon) msg.tool(),
								false);
					return false;
				}
			}
		...
		if ((msg.targetMinor() != CMMsg.NO_EFFECT) && (msg.amITarget(this))) {
			...
			if (CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS)) {
				if (Log.combatChannelOn())
					Log.combatOut(srcM.Name()
							+ ":"
							+ Name()
							+ ":"
							+ CMMsg.TYPE_DESCS[msg.targetMinor()]
							+ ":"
							+ ((msg.tool() != null) ? msg.tool().Name()
									: "null"));
				...
				if ((!mayIFight(srcM))
						&& ((!(msg.tool() instanceof Ability))
								|| (((((Ability) msg.tool())
										.classificationCode() & Ability.ALL_ACODES) != Ability.ACODE_POISON) && ((((Ability) msg
				...
				if ((srcM != this)
						...
						&& (soulMate() == null)
						&& (srcM.soulMate() == null)
						&& (CMath.abs(srcM.phyStats().level()
								- phyStats().level()) > CMProps
									.getPKillLevelDiff())
						&& (!CMSecurity.isAllowed(this, location(),
								CMSecurity.SecFlag.PKILL))
						&& (!CMSecurity.isAllowed(srcM, srcM.location(),
								CMSecurity.SecFlag.PKILL))
						&& ((!(msg.tool() instanceof Ability)) || (((Ability) msg
								.tool()).classificationCode() & Ability.ALL_ACODES) != Ability.ACODE_DISEASE)) {
				...
				if (isInCombat()) {
					if ((rangeToTarget() > 0) && (getVictim() != srcM)
							&& (srcM.getVictim() == this)
				...
				if ((msg.targetMinor() != CMMsg.TYP_WEAPONATTACK)
					...
					if (chanceToFail > Integer.MIN_VALUE) {
						int diff = (phyStats().level() - srcM.phyStats()
								.level());
			...
			case CMMsg.TYP_WEAR:
			case CMMsg.TYP_WIELD:
				srcM.tell(srcM, this, null,
						"You can't do that to <T-NAMESELF>.");
				return false;
			case CMMsg.TYP_TEACH:
				if ((msg.target() instanceof MOB)
						&& (!CMLib.expertises().canBeTaught(msg.source(),
								(MOB) msg.target(), msg.tool(),
								msg.targetMessage())))
					return false;
				break;
			case CMMsg.TYP_PULL:
				...
				if (phyStats().weight() > (srcM.maxCarry() / 2)) {
					srcM.tell(srcM, this, null,
							"<T-NAME> is too big for you to pull.");
					return false;
				}
				break;
			case CMMsg.TYP_PUSH:
				...
				if (phyStats().weight() > srcM.maxCarry()) {
					srcM.tell(srcM, this, null,
							"<T-NAME> is too heavy for you to push.");
					return false;
				}
				break;
			...
			case CMMsg.TYP_GIVE:
				...
				if (!(msg.tool() instanceof Item))
					return false;
				if (CMSecurity.isAllowed(this, location(),
						CMSecurity.SecFlag.ORDER)
						|| (CMSecurity.isAllowed(this, location(),
								CMSecurity.SecFlag.CMDMOBS) && (isMonster()))
						|| (CMSecurity.isAllowed(this, location(),
								CMSecurity.SecFlag.CMDROOMS) && (isMonster())))
					return true;
				if ((getWearPositions(Wearable.WORN_ARMS) == 0)
						&& (!CMath.bset(msg.targetMajor(), CMMsg.MASK_ALWAYS))) {
					srcM.tell(name(srcM)
							+ " is unable to accept that from you.");
					return false;
				}
				...
				if ((msg.target() != null) && (msg.target() instanceof MOB)) {
					msg2 = CMClass.getMsg((MOB) msg.target(), msg.tool(), null,
							GC | CMMsg.MSG_GET | CMMsg.MASK_INTERMSG, null, GC
									| CMMsg.MSG_GET | CMMsg.MASK_INTERMSG,
							null, GC | CMMsg.MSG_GET | CMMsg.MASK_INTERMSG,
							null);
					if (!location().okMessage(msg.target(), msg2)) {
						srcM.tell(((Physical) msg.target()).name(srcM)
	...
	public void tell(final MOB source, final Environmental target,
			final Environmental tool, final String msg) {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		final boolean canhearsrc = (msg.targetMinor() == CMMsg.TYP_SPEAK) ? CMLib
				.flags().canBeHeardSpeakingBy(srcM, this) : CMLib.flags()
				.canBeHeardMovingBy(srcM, this);
		...
		if ((msg.sourceCode() != CMMsg.NO_EFFECT) && (msg.amISource(this))) {
			...
			case CMMsg.TYP_SPEAK:
				if ((CMProps.getBoolVar(CMProps.Bool.INTRODUCTIONSYSTEM))
						&& (!asleep) && (canhearsrc))
					CMLib.commands().handleIntroductions(srcM, this,
							msg.targetMessage());
				break;
			default:
				if ((CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS))
					...
					if ((playerStats() != null)
							...
							&& (!CMath.isSet(playerStats().getChannelMask(),
									channelCode)))
						tell(srcM, msg.target(), msg.tool(),
								msg.targetMessage());
				}
				break;
			}
			...
			// now do the says
			if ((CMath.bset(targetMajor, CMMsg.MASK_SOUND)) && (canhearsrc)
					&& (!asleep)) {
				if ((msg.targetMinor() == CMMsg.TYP_SPEAK) && (srcM != null)
						&& (playerStats() != null) && (!srcM.isMonster())
				...
				tell(srcM, msg.target(), msg.tool(), msg.targetMessage());
			else if ((CMath.bset(targetMajor, CMMsg.MASK_EYES))
					&& ((!asleep) && (canseesrc)))
				tell(srcM, msg.target(), msg.tool(), msg.targetMessage());
			else if (CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS))
				tell(srcM, msg.target(), msg.tool(), msg.targetMessage());
			else if (((CMath.bset(targetMajor, CMMsg.MASK_HANDS))
					|| (CMath.bset(targetMajor, CMMsg.MASK_MOVE)) || ((CMath
					.bset(targetMajor, CMMsg.MASK_MOUTH) && (!CMath.bset(
					targetMajor, CMMsg.MASK_SOUND)))))
					&& (!asleep) && ((canhearsrc) || (canseesrc)))
				tell(srcM, msg.target(), msg.tool(), msg.targetMessage());
		} else if ((msg.othersCode() != CMMsg.NO_EFFECT)
				&& (!msg.amISource(this)) && (!msg.amITarget(this))) {
			...
			if (CMath.bset(msg.othersMajor(), CMMsg.MASK_MALICIOUS)
					...
					&& ((!CMath.bset(msg.sourceMajor(), CMMsg.MASK_ALWAYS)) || (!(msg
							.tool() instanceof DiseaseAffect))))
				CMLib.combat().makeFollowersFight(this, (MOB) msg.target(),
						srcM);
			...
			if ((othersMinor == CMMsg.TYP_ENTER) // exceptions to movement
				...
				if ((playerStats() != null)
						...
						&& (!CMath.isSet(playerStats().getChannelMask(),
								channelCode)))
					tell(srcM, msg.target(), msg.tool(), msg.othersMessage());
			} else if ((CMath.bset(othersMajor, CMMsg.MASK_SOUND)) && (!asleep)
					&& (canhearsrc)) {
				if ((CMProps.getBoolVar(CMProps.Bool.INTRODUCTIONSYSTEM))
				...
				tell(srcM, msg.target(), msg.tool(), msg.othersMessage());
					...
					|| (CMath.bset(othersMajor, CMMsg.MASK_HANDS)) || (CMath
						.bset(othersMajor, CMMsg.MASK_ALWAYS)))
					&& (!CMath.bset(msg.othersMajor(), CMMsg.MASK_CNTRLMSG))
					&& ((!asleep) && (canseesrc))) {
				tell(srcM, msg.target(), msg.tool(), msg.othersMessage());
			} else if (((CMath.bset(othersMajor, CMMsg.MASK_MOVE)) || ((CMath
					.bset(othersMajor, CMMsg.MASK_MOUTH)) && (!CMath.bset(
					othersMajor, CMMsg.MASK_SOUND))))
	...
	public boolean tick(final Tickable ticking, final int tickID) {
		...
		if (tickID == Tickable.TICKID_MOB) {
			...
			if (amDead) {
				...
				if (isMonster) {
					if ((phyStats().rejuv() != PhyStats.NO_REJUV)
							&& (basePhyStats().rejuv() > 0)) {
						phyStats().setRejuv(phyStats().rejuv() - 1);
						if ((phyStats().rejuv() < 0)
								|| (CMProps
										.getBoolVar(CMProps.Bool.MUDSHUTTINGDOWN))) {
							...
							if (room != null) {
								...
								if ((lastTickedTime < 0)
									...
									lastTickedTime = CMLib.utensils()
											.processVariableEquipment(this);
								room.showOthers(this, null,
										CMMsg.MSG_OK_ACTION,
										"<S-NAME> appears!");
							}
						}
				...
				if ((--recoverTickCter) <= 0) {
					...
					recoverTickCter = CMProps
							.getIntVar(CMProps.Int.RECOVERRATE);
				}
				if (!isMonster)
				...
				if (!CMLib.flags().isGolem(this)) {
					if (!CMLib.flags().canBreathe(this)) {
						...
						CMLib.combat().postDamage(
								...
								this,
								null,
								(int) Math.round(CMath.mul(Math.random(),
										basePhyStats().level() + 2)),
								CMMsg.MASK_ALWAYS | CMMsg.TYP_WATER, -1, null);
					} else if (!CMLib.flags().canBreatheHere(this, location())) {
						...
						if ((atmo & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_LIQUID) {
							location().show(
									...
									this,
									CMMsg.MSG_OK_VISUAL,
									("^Z<S-NAME> <S-IS-ARE> drowning in "
											+ RawMaterial.CODES.NAME(atmo)
													.toLowerCase() + "!^.^?")
											+ CMLib.protocol().msp("choke.wav",
													10));
							CMLib.combat().postDamage(
									this,
									this,
							...
							CMLib.combat()
									.postDamage(
											...
											this,
											null,
											(int) Math.round(CMath.mul(
													Math.random(),
													basePhyStats().level() + 2)),
											CMMsg.MASK_ALWAYS | CMMsg.TYP_GAS,
											-1, null);
						}
					}
				}
				...
				setActions(actions()
						+ (CMLib.flags().isSitting(this) ? phyStats().speed() / 2.0
								: phyStats().speed()));
				...
				if (isInCombat()) {
					...
					if (CMath.bset(getBitmap(), MOB.ATT_AUTODRAW)
							...
							&& (CMLib.flags().aliveAwakeMobileUnbound(this,
									true)))
				...
				if (!isMonster) {
					if (CMLib.flags().isSleeping(this))
						curState().adjFatigue(-CharState.REST_PER_SLEEP,
								maxState());
					...
					if (!CMSecurity.isAllowed(this, location(),
						...
						curState()
								.adjFatigue(
										Math.round(CMProps.getTickMillis()),
										maxState());
						if (curState().getFatigue() > CharState.FATIGUED_MILLIS) {
							boolean smallChance = (CMLib.dice()
									.rollPercentage() == 1);
							if (smallChance
									&& (!CMSecurity
											.isDisabled(CMSecurity.DisFlag.AUTODISEASE))) {
								Ability theYawns = CMClass
										.getAbility("Disease_Yawning");
								if (theYawns != null)
									theYawns.invoke(this, this, true, 0);
							}
							if (smallChance
									&& curState().getFatigue() > (CharState.FATIGUED_EXHAUSTED_MILLIS)) {
								location()
										.show(this, null, CMMsg.MSG_OK_ACTION,
												"<S-NAME> fall(s) asleep from exhaustion!!");
								basePhyStats().setDisposition(
										basePhyStats().disposition()
				...
				final Rideable riding = riding();
				if ((riding != null)
						&& (CMLib.map().roomLocation(riding) != location()))
					setRiding(null);

				if ((!isMonster) && (soulMate() == null)) {
					CMLib.coffeeTables().bump(this,
							CoffeeTableRow.STAT_TICKSONLINE);
			...
			manaConsumeCter = CMLib.commands().tickManaConsumption(this,
					manaConsumeCter);
	...
	public List<Item> findItems(Item goodLocation, String itemName) {
		...
		List<Item> items = CMLib.english().fetchAvailableItems(inventory,
				itemName, goodLocation, Wearable.FILTER_ANY, true);
		if (items.size() == 0)
			items = CMLib.english().fetchAvailableItems(inventory, itemName,
					goodLocation, Wearable.FILTER_ANY, false);
		return items;
	}
	...
	public Item fetchFromInventory(Item goodLocation, String itemName,
			Filterer<Environmental> filter, boolean respectLocationAndWornCode) {
		if (inventory.size() == 0)
			return null;
		final SVector inv = inventory;
		Item item = null;
		if (respectLocationAndWornCode) {
			item = CMLib.english().fetchAvailableItem(inv, itemName,
					goodLocation, filter, true);
			if (item == null)
				item = CMLib.english().fetchAvailableItem(inv, itemName,
						goodLocation, filter, false);
		} else {
			item = (Item) CMLib.english().fetchEnvironmental(inv, itemName,
					true);
			if (item == null)
				item = (Item) CMLib.english().fetchEnvironmental(inv, itemName,
						false);
	...
	public Item findItem(Item goodLocation, String itemName) {
		return fetchFromInventory(goodLocation, itemName, Wearable.FILTER_ANY,
				true);
	}

	public Item fetchItem(Item goodLocation, Filterer<Environmental> filter,
			String itemName) {
	...
	public List<Item> findItems(final String itemName) {
		if (inventory.size() > 0) {
			List V = CMLib.english().fetchEnvironmentals(inventory, itemName,
					true);
			if ((V != null) && (V.size() > 0))
				return V;
	...
	public Enumeration<Follower> followers() {
		return (followers == null) ? EmptyEnumeration.INSTANCE : followers
				.elements();
	}
	...
	public MOB fetchFollower(final String named) {
		if (followers == null)
			return null;
		final List<MOB> list = new ConvertingList<Follower, MOB>(followers,
				Follower.converter);
	...
	public boolean willFollowOrdersOf(MOB mob) {
		if ((amFollowing() == mob)
				|| ((isMonster() && CMSecurity.isAllowed(mob, location(),
						CMSecurity.SecFlag.ORDER)))
				|| (getLiegeID().equals(mob.Name()))
				|| (CMLib.law().doesOwnThisProperty(mob, CMLib.map()
						.getStartRoom(this))))
			return true;
		if ((!isMonster())
			...
			return true;
		for (Triad<Clan, Integer, Integer> t : CMLib.clans()
				.findCommonRivalrousClans(this, mob)) {
	...
	public MOB amUltimatelyFollowing() {
		...
		while ((following != null) && (following.amFollowing() != null)
	...
	public Enumeration<Ability> allAbilities() {
		final MultiListEnumeration multi = new MultiListEnumeration(new List[] {
	...
	public Ability fetchAbility(int index) {
		try {
			...
			final List<Ability> racialAbilities = charStats().getMyRace()
					.racialAbilities(this);
	...
	public Ability findAbility(String ID) {
		final Race R = charStats().getMyRace();
		Ability A = (Ability) CMLib.english().fetchEnvironmental(abilitys, ID,
				true);
		if (A == null)
			A = (Ability) CMLib.english().fetchEnvironmental(
					R.racialAbilities(this), ID, true);
		if (A == null)
			for (Pair<Clan, Integer> p : clans()) {
				A = (Ability) CMLib.english().fetchEnvironmental(
						p.first.clanAbilities(this), ID, true);
				if (A != null)
					return A;
			}
		if (A == null)
			A = (Ability) CMLib.english().fetchEnvironmental(abilitys, ID,
					false);
		if (A == null)
			A = (Ability) CMLib.english().fetchEnvironmental(
					R.racialAbilities(this), ID, false);
		if (A == null)
			for (Pair<Clan, Integer> p : clans()) {
				A = (Ability) CMLib.english().fetchEnvironmental(
						p.first.clanAbilities(this), ID, false);
				if (A != null)
					return A;
			}
		if (A == null)
	...
	protected final List<Ability> clanEffects() {
		if (clanAffects == null) {
			...
			if (!c.hasNext())
				clanAffects = CMLib.clans().getDefaultGovernment()
						.getClanLevelEffects(this, null, null);
			else {
		...
		if (role < 0) {
			...
			if (p == null) {
				...
				if (C == null)
					Log.errOut(
							"StdMOB",
							"Unknown clan: " + clanID + " on " + Name()
									+ " in "
									+ CMLib.map().getExtendedRoomID(location()));
				else {
	...
	public Ability fetchEffect(int index) {
		try {
			...
			return clanEffects().get(
					index - affects.size() - racialEffects().size());
		} catch (java.lang.ArrayIndexOutOfBoundsException x) {
		}
		return null;
	}
	...
	public Enumeration<Ability> effects() {
		return new MultiListEnumeration(new List[] { affects, racialEffects(),
				clanEffects() });
	}
	...
	public void addExpertise(String code) {
		final Entry<String, Integer> p = CMath.getStringFollowedByNumber(code,
				true);
		final String key = p.getKey().toUpperCase();
		final Integer oldNum = expertises.get(key);
		if ((oldNum == null)
				|| ((p.getValue() != null) && (oldNum.intValue() < p.getValue()
						.intValue()))) {
	...
	public void delExpertise(String baseCode) {
		...
		if (expertises.remove(baseCode.toUpperCase()) == null) {
			final Entry<String, Integer> p = CMath.getStringFollowedByNumber(
					baseCode, true);
	...
	@Override
	public Entry<String, Integer> fetchExpertise(final String baseCode) {
		if (baseCode == null)
			return null;
		final Entry<String, Integer> p = CMath.getStringFollowedByNumber(
				baseCode, true);
		...
		if ((expertises.containsKey(key))
				&& ((num == null) || (p.getValue() == null) || (p.getValue()
	...
	public Enumeration<String> expertises() {
		return new Enumeration<String>() {
			final Iterator<Entry<String, Integer>> i = expertises.entrySet()
					.iterator();
	...
	public Enumeration<ScriptingEngine> scripts() {
		return (scripts == null) ? EmptyEnumeration.INSTANCE : scripts
				.elements();
	}
	...
	public List<String> fetchFactionRanges() {
		...
		for (final Enumeration e = fetchFactions(); e.hasMoreElements();) {
			...
			if (F == null)
				continue;
			final Faction.FRange FR = CMLib.factions().getRange(F.factionID(),
					fetchFaction(F.factionID()));
			if (FR != null)
	...
	public List<Item> fetchWornItems(long wornCode, short aboveOrAroundLayer,
			short layerAttributes) {
				...
				if (thisItem instanceof Armor) {
					...
					if (lay >= (aboveOrAroundLayer - 1)) {
						if (((lay > aboveOrAroundLayer - 2)
								&& (lay < aboveOrAroundLayer + 2) && ((!equalOk) || ((((Armor) thisItem)
	...
	public boolean hasOnlyGoldInInventory() {
		for (int i = 0; i < numItems(); i++) {
			...
			if (I.amWearingAt(Wearable.IN_INVENTORY)
					&& ((I.container() == null) || (I.ultimateContainer(null)
							.amWearingAt(Wearable.IN_INVENTORY)))
					&& (!(I instanceof Coins)))
				return false;
		}
	...
	public Item fetchWieldedItem() {
		...
		if (wieldRef != null) {
			...
			if (I == null)
				return null;
			if ((I.owner() == this) && (I.amWearingAt(Wearable.WORN_WIELD))
	...
	public Item fetchHeldItem() {
		...
		if (heldRef != null) {
			...
			if (I == null)
				return null;
			if ((I.owner() == this) && (I.amWearingAt(Wearable.WORN_HELD))
					&& (!I.amDestroyed()) && (I.container() == null))
				return I;
			possHeldItem = null;
		}
	...
	public void moveItemTo(Item container, ItemPossessor.Expire expire,
			Move... moveFlags) {
	...
	public void moveItemTo(Item container) {
		...
		if (CMLib.flags().isHidden(container))
			container.basePhyStats().setDisposition(
					container.basePhyStats().disposition()
	...
	public int getSaveStatIndex() {
		return (xtraValues == null) ? getStatCodes().length
				: getStatCodes().length - xtraValues.length;
	}
