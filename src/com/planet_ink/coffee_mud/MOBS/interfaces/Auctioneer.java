...
public interface Auctioneer extends ShopKeeper {
	public static class AuctionData {
		...
		public int daysRemaining(MOB mob, MOB mob2) {
			...
			daysRemain = Math.round(Math.floor(CMath.div(CMath.div(daysRemain,
					CMProps.getMillisPerMudHour()), A.getTimeObj()
					.getHoursInDay())));
	...
	/*
	 * public double liveListingPrice(); public void setLiveListingPrice(double
	 * d);
	 * 
	 * public double liveFinalCutPct(); public void setLiveFinalCutPct(double
	 * d);
	 */
	...
	public static class AuctionRates {
		...
		public AuctionRates() {
			List<String> ratesV = CMParms.parseCommas(
					CMProps.getVar(CMProps.Str.AUCTIONRATES), true);
			while (ratesV.size() < 7)
				ratesV.add("0");
		...
		public AuctionRates(Auctioneer A) {
			...
			timeListPrice = A.timedListingPrice() < 0.0 ? base.timeListPrice
					: A.timedListingPrice();
			timeListPct = A.timedListingPct() < 0.0 ? base.timeListPct : A
					.timedListingPct();
			liveCutPct = base.liveCutPct;
			timeCutPct = A.timedFinalCutPct() < 0.0 ? base.timeCutPct : A
					.timedFinalCutPct();
			maxDays = A.maxTimedAuctionDays() < 0 ? base.maxDays : A
					.maxTimedAuctionDays();
			minDays = A.minTimedAuctionDays() < 0 ? base.minDays : A
					.minTimedAuctionDays();
			if (minDays > maxDays)
				minDays = maxDays;
		}
