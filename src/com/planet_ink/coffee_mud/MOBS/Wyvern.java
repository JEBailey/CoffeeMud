...
public class Wyvern extends StdMOB {
	...
	public Wyvern() {
		...
		baseCharStats().setStat(CharStats.STAT_INTELLIGENCE,
				5 + Math.abs(randomizer.nextInt() % 3));
	...
	public void recoverCharStats() {
		...
		charStats().setStat(CharStats.STAT_SAVE_POISON,
				charStats().getStat(CharStats.STAT_SAVE_POISON) + 100);
	}
	...
	protected boolean sting() {
		if (CMLib.flags().aliveAwakeMobileUnbound(this, true)
			...
			if (roll < 20) {
				...
				CMMsg msg = CMClass
						.getMsg(this, target, null, CMMsg.MSK_MALICIOUS_MOVE
								| CMMsg.TYP_POISON,
								"^F^<FIGHT^><S-NAME> sting(s) <T-NAMESELF>!^</FIGHT^>^?");
