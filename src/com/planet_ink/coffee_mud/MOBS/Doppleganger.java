...
public class Doppleganger extends StdMOB {
	...
	protected void revert() {
		...
		baseCharStats().setStat(CharStats.STAT_INTELLIGENCE,
				10 + Math.abs(randomizer.nextInt() % 6));
		...
		baseCharStats().setStat(CharStats.STAT_DEXTERITY,
				9 + Math.abs(randomizer.nextInt() % 6));
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(this))
			...
			if (mimicing != null) {
				if ((mimicing.getVictim() != null)
						&& (mimicing.getVictim() != this))
					mimicing = null;
				else if ((mimicing.location() != null)
						&& (mimicing.location() != location()))
					mimicing = null;
			}
			if ((mimicing == null) && (location() != null)
					&& (msg.source() != null)) {
				location().show(this, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> take(s) on a new form!");
				mimicing = msg.source();
