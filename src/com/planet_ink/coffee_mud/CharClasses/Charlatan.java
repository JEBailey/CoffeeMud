...
public class Charlatan extends StdCharClass {
	...
	protected volatile WeakReference<Ability> invokable = new WeakReference(
			null);
	...
	public void initializeClass() {
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 3, "Skill_Disguise",
				true);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 6, "Skill_Songcraft",
				true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 6, "Spell_ReadMagic",
				false);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 6, "Song_Detection",
				false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 7, "Skill_Imitation",
				true);
		...
		CMLib.ableMapper()
				.addCharAbilityMapping(ID(), 10, "Skill_Dodge", false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 11, "Skill_FalseArrest",
				true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 11,
				"Song_Comprehension", true);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 12, "Skill_Spellcraft",
				true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 12, "Ranger_Track",
				false);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 12,
				"Spell_MagicMissile", false);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 14, "Thief_AnalyzeMark",
				true);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 15, "Skill_Chantcraft",
				true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 15, "Song_Protection",
				true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 15,
				"Chant_SummonPlants", false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 16, "Skill_Shuffle",
				false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 17, "Skill_Attack2",
				false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 18, "Skill_Prayercraft",
				true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 18, "Prayer_CureLight",
				false);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 20, "Thief_Detection",
				true);
		...
		CMLib.ableMapper()
				.addCharAbilityMapping(ID(), 22, "Skill_Parry", false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 23, "Thief_Con", true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 23, "Song_Strength",
				false);

		CMLib.ableMapper()
				.addCharAbilityMapping(ID(), 24, "Song_Disgust", true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 24, "Thief_FrameMark",
				false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 25,
				"Skill_MarkDisguise", true);
			...
			if (msg.amISource(myChar) && (!myChar.isMonster())
				...
				if ((curRef != null) && (msg.tool() == curRef.get())) {
					...
					if (CMath.bset(A.usageType(), Ability.USAGE_MANA))
						myChar.curState().adjMana(
								usageCost[Ability.USAGEINDEX_MANA] / 4,
								myChar.maxState());
					if (CMath.bset(A.usageType(), Ability.USAGE_MOVEMENT))
						myChar.curState().adjMovement(
								usageCost[Ability.USAGEINDEX_MOVEMENT] / 4,
								myChar.maxState());
					if (CMath.bset(A.usageType(), Ability.USAGE_HITPOINTS))
						myChar.curState().adjHitPoints(
								usageCost[Ability.USAGEINDEX_HITPOINTS] / 4,
								myChar.maxState());
				}
				...
				if (((((Ability) msg.tool()).classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_SPELL)
						...
						&& (CMLib.dice().roll(1, 100, 0) < (myChar.charStats()
								.getClassLevel(this) * 4))) {
					myChar.location().show(
							msg.source(),
							myChar,
							CMMsg.MSG_OK_ACTION,
							"<T-NAME> fool(s) <S-NAMESELF>, causing <S-HIM-HER> to fizzle "
									+ msg.tool().name() + ".");
					return false;
				}
			}
		...
		if (mob.playerStats() != null) {
			...
			for (Enumeration<Ability> a = CMClass.abilities(); a
				...
				if ((CMLib.ableMapper().qualifyingLevel(mob, A) <= 0)
						...
						&& (CMLib.ableMapper().availableToTheme(A.ID(),
								Area.THEME_FANTASY, true))
						&& (!CMLib.ableMapper().qualifiesOnlyByClan(mob, A))
						&& (!CMLib.ableMapper().qualifiesOnlyByRace(mob, A))
						&& (A.isAutoInvoked() || ((A.triggerStrings() != null) && (A
