...
public class StdCharClass implements CharClass {
	...
	private static final CMSecurity.SecGroup empty = new CMSecurity.SecGroup(
			new CMSecurity.SecFlag[] {});
	...
	protected final static String[][] hitPointDescReplacePairs = {
			{ "@x1", "Lvl" }, { "(@x2<@x3)", "Str" }, { "@x2<@x3", "Str" },
			{ "(@x4<@x5)", "Dex" }, { "@x4<@x5", "Dex" },
			{ "(@x6<@x7)", "Con" }, { "@x6<@x7", "Con" }, { "@x2", "Str" },
			{ "@x3", "Str" }, { "@x4", "Dex" }, { "@x5", "Dex" },
			{ "@x6", "Con" }, { "@x7", "Con" }, { "@x8", "Int" },
			{ "@x9", "Wis" }, { "1?", "d" }, { "*", "X" } };
	protected final String[][] manaDescReplacePairs = { { "@x1", "Lvl" },
			{ "(@x2<@x3)", "Wis" }, { "@x2<@x3", "Wis" },
			{ "(@x4<@x5)", "Int" }, { "@x4<@x5", "Int" },
			{ "(@x6<@x7)", getShortAttackAttribute() },
			{ "@x6<@x7", getShortAttackAttribute() }, { "@x2", "Wis" },
			{ "@x3", "Wis" }, { "@x4", "Int" }, { "@x5", "Int" },
			{ "@x6", "Con" }, { "@x7", "Con" }, { "@x8", "Cha" },
			{ "@x9", "Dex" }, { "1?", "d" }, { "*", "X" } };
	...
	public boolean qualifiesForThisClass(MOB mob, boolean quiet) {
		if (CMSecurity.isDisabled(CMSecurity.DisFlag.STDCLASSES)
				&& (!isGeneric()))
			return false;
		...
		if (multiClassSecondRule.length() > 0) {
			if (multiClassSecondRule.startsWith("GRP-")) {
				CharClass possibleClass = CMClass
						.findCharClass(multiClassSecondRule.substring(4));
				if (possibleClass != null)
					canOnlyBeBaseClassID = possibleClass.ID();
				multiClassSecondRule = "NO";
			} else {
				CharClass possibleClass = CMClass
						.findCharClass(multiClassSecondRule);
				if (possibleClass != null)
		...
		if (mob == null) {
			if (multiClassFirstRule.equals("SUB")
				...
				if ((changeToBaseClassID.equals(changeToClassID))
						|| (changeToSubClassRule == SubClassRule.ANY))
					return true;
				return false;
			} else
				return true;
		...
		for (Pair<String, Integer> minReq : getMinimumStatRequirements()) {
			...
			if (statCode >= 0) {
				if (mob.baseCharStats().getStat(statCode) < minReq.second
						.intValue()) {
					if (!quiet)
						mob.tell("You need at least a "
								+ minReq.second.toString()
								+ " "
								+ CMStrings.capitalizeAndLower(CharStats.CODES
										.NAME(statCode)) + " to become a "
								+ name() + ".");
					return false;
				}
			}
		...
		if (!foundOne) {
			if (!quiet) {
				final StringBuilder str = new StringBuilder("You need to be a ")
						.append(getRaceList(raceList)).append(
								"to be a " + name() + ".");
		...
		if ((!mob.isMonster()) && (mob.basePhyStats().level() > 0)) {
			...
			if (currentClassID.equalsIgnoreCase("StdCharClass")) // this is the
																	// starting
																	// character
																	// rule
			{
				if ((canOnlyBeClassID.length() > 0)
						&& (!changeToClassID.equals(canOnlyBeClassID)))
					return false;
				if ((canOnlyBeBaseClassID.length() > 0)
						&& (!changeToBaseClassID.equals(canOnlyBeBaseClassID)))
					return false;
				if ((multiClassRule.equals("NO"))
						|| (multiClassRule.equals("MULTI")))
					return true;
				if ((multiClassRule.equals("SUB") || multiClassSecondRule
						.equals("BASE"))
				...
				if (!quiet)
					mob.tell("You can't train to be a " + name() + "!");
				return false;
			} else if (curClass.getSubClassRule() == SubClassRule.ANY) // if you
																		// are
																		// an
																		// apprentice
			{
				...
				if (multiClassFirstRule.equals("SUB")
						|| multiClassSecondRule.equals("SUB")) {
					if ((changeToBaseClassID.equals(changeToClassID))
							|| (changeToBaseClassID.equals(currentBaseClassID)))
						return true;
					if (!quiet)
						mob.tell("You must be a " + changeToBaseClassID
								+ " type to become a " + name() + ".");
				}
				return false;
			} else {
				if (multiClassFirstRule.equals("MULTI")
						|| multiClassSecondRule.equals("MULTI"))
					return true;
				else if (multiClassFirstRule.equals("NO")
						|| (multiClassSecondRule.equals("NO")))
					mob.tell("You should be happy to be a " + curClass.name()
							+ ".");
				else if (multiClassFirstRule.equals("SUB")
						|| multiClassSecondRule.equals("SUB")) {
					if (currentBaseClassID.equals(changeToBaseClassID)
							|| (curClass.getSubClassRule() == SubClassRule.ANY))
						return true;
	...
	private StringBuilder getRaceList(String[] raceList) {
		...
		if (raceList.length == 1)
			...
			str.append(CMStrings.capitalizeAndLower(raceList[0]))
					.append(" or ")
					.append(CMStrings.capitalizeAndLower(raceList[1]));
		else
	...
	public String getStatQualDesc() {
		...
		for (int x = 0; x < reqs.length; x++) {
			...
			if (x > 0)
				str.append(", ");
			str.append(CMStrings.capitalizeAndLower(req.first)).append(" ")
					.append(req.second.toString()).append("+");
		}
		return str.toString();
	}
	...
	public String getMaxStatDesc() {
		...
		for (int i : CharStats.CODES.BASE())
			if (maxStatAdjustments()[i] != 0)
				str.append(CMStrings.capitalizeAndLower(CharStats.CODES.DESC(i))
						+ " ("
						+ (CMProps.getIntVar(CMProps.Int.BASEMAXSTAT) + maxStatAdjustments()[i])
						+ "), ");
	...
	public String getHitPointDesc() {
		if (hitPointsDesc == null) {
			...
			if (x > 0) {
				...
				if (y > x)
					formula = formula.substring(0, x) + "d"
							+ formula.substring(x + 4, y)
							+ formula.substring(y + 1);
			}
			formula = CMStrings.replaceAlls(formula, hitPointDescReplacePairs);
			hitPointsDesc = CMProps.getIntVar(CMProps.Int.STARTHP) + " +"
					+ formula + " per lvl";
		}
	...
	public String getManaDesc() {
		if (manaDesc == null) {
			...
			manaDesc = CMProps.getIntVar(CMProps.Int.STARTMANA) + " +"
					+ formula + " /lvl";
		}
		return manaDesc;
	}
	...
	public String getMovementDesc() {
		if (movementDesc == null) {
			...
			movementDesc = CMProps.getIntVar(CMProps.Int.STARTMOVE) + " +"
					+ formula + " per lvl";
		}
		return movementDesc;
	}
	...
	public String getPrimeStatDesc() {
		return CMStrings.capitalizeAndLower(CharStats.CODES
				.DESC(getAttackAttribute()));
	}
	...
	protected HashSet buildDisallowedWeaponClasses() {
	...
	protected boolean isQualifyingAuthority(MOB mob, Ability A) {
		CharClass C = null;
		int ql = 0;
		for (int i = (mob.charStats().numClasses() - 1); i >= 0; i--) // last
																		// one
																		// is
																		// current
		{
			...
			if (C != null) {
				ql = CMLib.ableMapper()
						.getQualifyingLevel(C.ID(), true, A.ID());
				if ((ql > 0) && (ql <= mob.charStats().getClassLevel(C)))
	...
	protected boolean armorCheck(MOB mob, int sourceCode, Environmental E) {
		...
		if ((allowedArmorLevel() != CharClass.ARMOR_ANY)
				...
				&& (CMLib.dice().rollPercentage() > (mob.charStats().getStat(
						getAttackAttribute()) * 2)))
			return false;
		return true;
	}
	...
	protected boolean weaponCheck(MOB mob, int sourceCode, Environmental E) {
		if ((((sourceCode & CMMsg.MINOR_MASK) == CMMsg.TYP_WEAPONATTACK) || ((sourceCode & CMMsg.MINOR_MASK) == CMMsg.TYP_THROW))
				...
				&& (((requiredWeaponMaterials() != null) && (!requiredWeaponMaterials()
						.contains(
								Integer.valueOf(((Weapon) E).material()
										& RawMaterial.MATERIAL_MASK)))) || ((disallowedWeaponClasses(mob) != null) && (disallowedWeaponClasses(mob)
						.contains(Integer.valueOf(((Weapon) E)
								.weaponClassification())))))
				&& (CMLib.dice().rollPercentage() > (mob.charStats().getStat(
						getAttackAttribute()) * 2))
				&& (mob.fetchWieldedItem() != null)) {
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> fumble(s) horribly with " + E.name() + ".");
			return false;
		}
		return true;
	}

	protected boolean giveMobAbility(MOB mob, Ability A, int proficiency,
			String defaultParm, boolean isBorrowedClass) {
		return giveMobAbility(mob, A, proficiency, defaultParm,
				isBorrowedClass, true);
	}

	protected boolean giveMobAbility(MOB mob, Ability A, int proficiency,
			String defaultParm, boolean isBorrowedClass, boolean autoInvoke) {
		...
		if (CMSecurity.isAllowedEverywhere(mob, CMSecurity.SecFlag.ALLSKILLS)) {
			...
			for (int a = 0; a < mob.numAbilities(); a++) {
				...
				if (A != null) {
					...
					if (A2 != null)
						A2.setProficiency(CMLib.ableMapper().getMaxProficiency(
								mob, true, A.ID()));
					else
						A.autoInvocation(mob);
			...
			for (Enumeration e = CMLib.expertises().definitions(); e
					.hasMoreElements();)
				mob.addExpertise(((ExpertiseLibrary.ExpertiseDefinition) e
						.nextElement()).ID);
			alreadyAble.clear();
			alreadyAff.clear();
		} else {
			...
			for (Enumeration<Ability> a = CMClass.abilities(); a
				...
				if ((CMLib.ableMapper().getQualifyingLevel(ID(), true, A.ID()) > 0)
						&& (CMLib.ableMapper().getQualifyingLevel(ID(), true,
								A.ID()) <= mob.baseCharStats().getClassLevel(
								this))
	...
	public CharClass makeGenCharClass() {
		if (isGeneric())
			return this;
		CharClass CR = (CharClass) CMClass.getCharClass("GenCharClass")
				.copyOf();
		CR.setClassParms("<CCLASS><ID>" + ID() + "</ID><NAME>" + name()
				+ "</NAME></CCLASS>");
		...
		for (int i = 0; i < data1.size(); i++) {
			...
			CR.setStat("GETCABLEPROF" + i,
					Integer.toString(able.defaultProficiency));
			...
			CR.setStat("GETCABLEMASK" + i, able.extraMask == null ? ""
					: able.extraMask);
			CR.setStat("GETCABLEMAXP" + i,
					Integer.toString(able.maxProficiency));
		...
		CR.setStat(
				"DISFLAGS",
				""
						+ ((raceless() ? CharClass.GENFLAG_NORACE : 0)
								| (leveless() ? CharClass.GENFLAG_NOLEVELS : 0)
								| (expless() ? CharClass.GENFLAG_NOEXP : 0) | (showThinQualifyList() ? CharClass.GENFLAG_THINQUALLIST
									: 0)));
		...
		int nameDex = 0;
		List<String> firstSet = CMParms.parseSemicolons(getSecurityFlags(0)
				.toString(';'), true);
		...
		for (int x = 1; x < 20000; x++) {
			...
			if (getSecurityFlags(x).size() != cumulativeSet.size()) {
				...
				V.addAll(CMParms.parseSemicolons(
						getSecurityFlags(x).toString(';'), true));
		...
		if ((securitySets.size() == 1) && (securitySets.get(0).size() == 0)) {
			securitySets.clear();
			securityLvls.clear();
		}
		...
		if (affectableStats.getCurrentClass().ID().equals(ID()))
			for (int i : CharStats.CODES.MAX())
				affectableStats.setStat(i, affectableStats.getStat(i)
						+ maxStatAdjustments()[i]
						+ maxStatAdjustments()[CharStats.CODES.toMAXBASE(i)]);
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((msg.source() == myHost) && (!msg.source().isMonster())
				&& (msg.source().charStats().getCurrentClass() == this)) // this
																			// is
																			// important
																			// because
																			// of
				...
				if (msg.tool() == null)
					msg.source()
							...
							.show(msg.source(),
									null,
									CMMsg.MSG_OK_VISUAL,
									CMStrings.replaceAll(armorFailMessage(),
											"<SKILL>", "maneuver"));
				else
					msg.source()
							.location()
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((msg.source() == myHost) && (msg.target() instanceof Item)
				&& (msg.source().charStats().getCurrentClass() == this) // this
																		// is
																		// important
																		// because
			...
			case CMMsg.TYP_WIELD: {
				if ((msg.target() instanceof Weapon)
						&& (((requiredWeaponMaterials() != null) && (!requiredWeaponMaterials()
								.contains(
										Integer.valueOf(((Weapon) msg.target())
												.material()
												& RawMaterial.MATERIAL_MASK)))) || ((disallowedWeaponClasses(msg
								.source()) != null) && (disallowedWeaponClasses(msg
								.source()).contains(Integer
								.valueOf(((Weapon) msg.target())
										.weaponClassification()))))))
					msg.addTrailerMsg(CMClass.getMsg(msg.source(),
							msg.target(), null, CMMsg.TYP_OK_VISUAL,
							"<T-NAME> feel(s) a bit strange in your hands.",
							CMMsg.NO_EFFECT, null, CMMsg.NO_EFFECT, null));
				break;
			}
			case CMMsg.TYP_WEAR:
			case CMMsg.TYP_HOLD: {
				if (!CMLib.utensils().armorCheck(msg.source(),
						(Item) msg.target(), allowedArmorLevel())) {
					final String choice = CMProps
							.getAnyListFileValue(CMProps.ListFile.ARMOR_MISFITS);
					msg.addTrailerMsg(CMClass.getMsg(msg.source(),
							msg.target(), null, CMMsg.TYP_OK_VISUAL, choice,
							CMMsg.NO_EFFECT, null, CMMsg.NO_EFFECT, null));
				}
				break;
			}
