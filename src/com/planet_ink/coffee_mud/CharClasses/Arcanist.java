...
public class Arcanist extends Thief {
	...
	public void initializeClass() {
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Apothecary", false,
				"+WIS 12");
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "ThievesCant", 25,
				true);
		...
		// clan magic
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqAcid", 0,
				"", false, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqCold", 0,
				"", false, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqElectric",
				0, "", false, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqFire", 0,
				"", false, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqGas", 0,
				"", false, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqMind", 0,
				"", false, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqParalysis",
				0, "", false, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqPoison", 0,
				"", false, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqWater", 0,
				"", false, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_CEqDisease",
				0, "", false, true);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 3, "Skill_WandUse", 25,
				true);
		...
		CMLib.ableMapper()
				.addCharAbilityMapping(ID(), 5, "Skill_Revoke", false);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 7, "Skill_Spellcraft",
				true);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 9, "Thief_RemoveTraps",
				false);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 12, "Spell_Scribe",
				false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 13,
				"Spell_DisenchantWand", true);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 15, "Spell_EnchantWand",
				false);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 15, "Thief_PowerGrab",
				true);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 16, "Spell_WardArea",
				false);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 17, "Skill_Attack2",
				false);
		CMLib.ableMapper()
				.addCharAbilityMapping(ID(), 17, "Thief_Shadow", true);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 18, "Thief_Detection",
				false);
		CMLib.ableMapper()
				.addCharAbilityMapping(ID(), 18, "Spell_Knock", false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 19, "Spell_Refit", true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 19, "Spell_LightenItem",
				false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 20,
				"Spell_EnchantArmor", false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 21, "Thief_Observation",
				true);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 22,
				"Spell_EnchantWeapon", false);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 22, "Thief_DampenAuras",
				false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 23, "Spell_Mend", true);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 24, "Spell_Disenchant",
				false);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 24,
				"Spell_ComprehendLangs", false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 25, "Spell_StoreSpell",
				false);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 25,
				"Thief_SlipperyMind", false);

		CMLib.ableMapper().addCharAbilityMapping(ID(), 30, "Spell_MagicItem",
				true);
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amISource(mob)) && (mob.charStats().getClassLevel(this) > 4)) {
			if (((msg.sourceMinor() == CMMsg.TYP_BUY) || (msg.sourceMinor() == CMMsg.TYP_LIST))
					&& (msg.tool() != null) && (msg.tool() instanceof Potion)) {
				mob.basePhyStats().setDisposition(
						mob.basePhyStats().disposition() | PhyStats.IS_BONUS);
				mob.recoverPhyStats();
	...
	private void addAbilityToSpellcraftList(MOB mob, Ability A) {
		...
		if (enabledA != null) {
			...
			if (!ables.contains(A.ID())) {
				...
				mob.addAbility(A);
			} else if (mob.isMine(A) && (A.proficiency() < 75)
					&& (!A.isSavable()))
				A.setProficiency(A.proficiency()
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amISource(mob)) {
			if (((msg.sourceMinor() == CMMsg.TYP_LOOK) || (msg.sourceMinor() == CMMsg.TYP_EXAMINE))
				...
				String message = "<O-NAME> has "
						+ ((Wand) msg.target()).usesRemaining()
						+ " charges remaining.";
				msg.addTrailerMsg(CMClass.getMsg(mob, null, msg.target(),
						CMMsg.MSG_OK_VISUAL, CMMsg.NO_EFFECT, CMMsg.NO_EFFECT,
						message));
				...
				if (msg.tool().ID().equals("Skill_Spellcraft")) {
					if ((msg.tool().text().length() > 0)
						...
						Ability A = ((MOB) msg.target()).fetchAbility(msg
								.tool().text());
						if (A == null)
							return;
						...
						if (myA != null) {
							if ((!A.isSavable())
									...
									&& (CMLib.ableMapper()
											.lowestQualifyingLevel(A.ID()) < 30))
								addAbilityToSpellcraftList(mob, A);
						} else if (CMLib.ableMapper().lowestQualifyingLevel(
								A.ID()) < 30) {
							Vector otherChoices = new Vector();
							for (int a = 0; a < mob.numAbilities(); a++) {
								Ability A2 = mob.fetchAbility(a);
							...
							A.setProficiency(0);
							A.setSavable(false);
							if (otherChoices.size() > (mob.charStats()
									.getClassLevel(this) / 3)) {
								Ability A2 = (Ability) otherChoices
										.elementAt(CMLib.dice().roll(1,
												otherChoices.size(), -1));
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((ticking instanceof MOB)
			...
			((MOB) ticking).basePhyStats().setDisposition(
					((MOB) ticking).basePhyStats().disposition()
							- PhyStats.IS_BONUS);
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_SAVE_MAGIC,
				affectableStats.getStat(CharStats.STAT_SAVE_MAGIC)
						+ (affectableStats.getClassLevel(this)));
		if (CMath
				.bset(affected.basePhyStats().disposition(), PhyStats.IS_BONUS))
			affectableStats.setStat(CharStats.STAT_CHARISMA,
					affectableStats.getStat(CharStats.STAT_CHARISMA) + 30);
	}
}
