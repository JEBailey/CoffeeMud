...
public class Wizard extends Mage {
	...
	public void initializeClass() {
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Skill_Spellcraft",
				50, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Skill_ScrollCopy",
				100, true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Spell_Scribe", 75,
				true);
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1, "Papermaking", 75,
				true);
		...
		CMLib.ableMapper().addCharAbilityMapping(ID(), 1,
				"Spell_ResistMagicMissiles", false);
		CMLib.ableMapper()
				.addCharAbilityMapping(ID(), 1, "Spell_Shield", false);
		...
		for (Enumeration<Ability> a = CMClass.abilities(); a.hasMoreElements();) {
			...
			if ((A != null)
				...
				int level = CMLib.ableMapper().getQualifyingLevel(ID(), true,
						A.ID());
				if (level > 0) {
					AbilityMapper.AbilityMapping able = CMLib.ableMapper()
							.getAbleMap(ID(), A.ID());
					if ((able != null)
							&& (!CMLib.ableMapper().getDefaultGain(ID(), true,
									A.ID()))) {
						able.costOverrides = new Integer[] {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amISource(mob) && (msg.tool() != null)) {
			if (msg.tool() instanceof Ability) {
				...
				if ((A != null)
						&& (!CMLib.ableMapper().getDefaultGain(ID(), false,
								A.ID()))
						&& ((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_SPELL)) {
					mob.delAbility(A);
					mob.recoverMaxState();
