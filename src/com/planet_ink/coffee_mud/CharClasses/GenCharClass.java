...
public class GenCharClass extends StdCharClass {
	...
	protected int maxNonCraftingSkills = CMProps
			.getIntVar(CMProps.Int.MAXNONCRAFTINGSKILLS);
	protected int maxCraftingSkills = CMProps
			.getIntVar(CMProps.Int.MAXCRAFTINGSKILLS);
	protected int maxCommonSkills = CMProps
			.getIntVar(CMProps.Int.MAXCOMMONSKILLS);
	...
	private HashSet requiredWeaponMaterials = null; // set of Integer material
													// masks
	...
	protected HashSet disallowedWeaponSet = null; // set of Integers for weapon
													// classes
	...
	public String name(int classLevel) {
		for (int i = nameLevels.length - 1; i >= 0; i--)
			if ((classLevel >= nameLevels[i].intValue()) && (i < names.length))
				return names[i];
		return names[0];
	}
	...
	public String getWeaponLimitDesc() {
		...
		if ((disallowedWeaponClasses(null) != null)
				&& (disallowedWeaponClasses(null).size() > 0)) {
			str.append("The following weapon types may not be used: ");
			for (Iterator i = disallowedWeaponClasses(null).iterator(); i
					.hasNext();) {
				Integer I = (Integer) i.next();
				str.append(CMStrings.capitalizeAndLower(Weapon.CLASS_DESCS[I
						.intValue()]) + " ");
			}
			str.append(".  ");
		}
		if ((requiredWeaponMaterials() != null)
				&& (requiredWeaponMaterials().size() > 0)) {
			...
			for (Iterator i = requiredWeaponMaterials().iterator(); i.hasNext();) {
				Integer I = (Integer) i.next();
				str.append(CMStrings.capitalizeAndLower(CMLib.materials()
						.getMaterialDesc(I.intValue())) + " ");
			}
			str.append(".  ");
		}

		if (str.length() == 0)
			str.append("No limitations.");
		...
		if (mob != null) {
			if ((!mob.isMonster()) && (mob.basePhyStats().level() > 0)) {
				if (!CMLib.masking().maskCheck(qualifications, mob, true)) {
					if (!quiet)
						mob.tell("You must meet the following qualifications to be a "
								+ name() + ":\n" + getStatQualDesc());
					return false;
				}
			}
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		if (adjPStats != null) {
			...
			affectableStats.setAttackAdjustment(affectableStats
					.attackAdjustment() + adjPStats.attackAdjustment());
			affectableStats.setDamage(affectableStats.damage()
					+ adjPStats.damage());
			affectableStats.setDisposition(affectableStats.disposition()
					| adjPStats.disposition());
			affectableStats.setHeight(affectableStats.height()
					+ adjPStats.height());
			affectableStats.setLevel(affectableStats.level()
					+ adjPStats.level());
			affectableStats.setSensesMask(affectableStats.sensesMask()
					| adjPStats.sensesMask());
			affectableStats.setSpeed(affectableStats.speed()
					+ adjPStats.speed());
			affectableStats.setWeight(affectableStats.weight()
					+ adjPStats.weight());
		}
		if (statBuddy != null)
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((eventBuddy != null) && (!eventBuddy.okMessage(myHost, msg)))
			return false;
	...
	public void affectCharState(MOB affectedMob, CharState affectableMaxState) {
		if (adjState != null) {
			affectableMaxState.setFatigue(affectableMaxState.getFatigue()
					+ adjState.getFatigue());
			affectableMaxState.setHitPoints(affectableMaxState.getHitPoints()
					+ adjState.getHitPoints());
			affectableMaxState.setHunger(affectableMaxState.getHunger()
					+ adjState.getHunger());
			affectableMaxState.setMana(affectableMaxState.getMana()
					+ adjState.getMana());
			affectableMaxState.setMovement(affectableMaxState.getMovement()
					+ adjState.getMovement());
			affectableMaxState.setThirst(affectableMaxState.getThirst()
					+ adjState.getThirst());
		}
		if (statBuddy != null)
	...
	public String classParms() {
		...
		for (int i = 0; i < names.length; i++) {
			...
			str.append(CMLib.xml().convertXMLtoTag("NAMELEVEL" + i,
					nameLevels[i].intValue()));
		}
		...
		str.append(CMLib.xml()
				.convertXMLtoTag("FSTTRAN", "" + trainsFirstLevel));
		str.append(CMLib.xml().convertXMLtoTag("LVLDAM",
		...
		str.append(CMLib.xml().convertXMLtoTag("MAXNCS",
				"" + maxNonCraftingSkills));
		str.append(CMLib.xml()
				.convertXMLtoTag("MAXCRS", "" + maxCraftingSkills));
		str.append(CMLib.xml().convertXMLtoTag("MAXCMS", "" + maxCommonSkills));
		str.append(CMLib.xml().convertXMLtoTag("MAXLGS", "" + maxLanguages));
		str.append(CMLib.xml().convertXMLtoTag("SUBRUL",
				subClassRule.toString()));
		str.append(CMLib.xml().convertXMLtoTag("RACQUAL",
				CMParms.toStringList(getRequiredRaceList())));
		if (getMinimumStatRequirements().length == 0)
			str.append("<MINSTATS />");
		else {
			str.append("<MINSTATS>");
			for (Pair<String, Integer> stat : getMinimumStatRequirements())
				str.append("<STAT NAME=\"" + stat.first + "\" MIN="
						+ stat.second.toString() + " />");
			str.append("</MINSTATS>");
		}

		str.append(CMLib.xml().convertXMLtoTag("HELP",
				CMLib.xml().parseOutAngleBrackets(helpEntry)));
		if (adjPStats == null)
			str.append("<ESTATS/>");
		else
			str.append(CMLib.xml().convertXMLtoTag("ESTATS",
					CMLib.coffeeMaker().getPhyStatsStr(adjPStats)));
		if (adjStats == null)
			str.append("<ASTATS/>");
		else
			str.append(CMLib.xml().convertXMLtoTag("ASTATS",
					CMLib.coffeeMaker().getCharStatsStr(adjStats)));
		if (setStats == null)
			str.append("<CSTATS/>");
		else
			str.append(CMLib.xml().convertXMLtoTag("CSTATS",
					CMLib.coffeeMaker().getCharStatsStr(setStats)));
		if (adjState == null)
			str.append("<ASTATE/>");
		else
			str.append(CMLib.xml().convertXMLtoTag("ASTATE",
					CMLib.coffeeMaker().getCharStateStr(adjState)));
		if (startAdjState == null)
			str.append("<STARTASTATE/>");
		else
			str.append(CMLib.xml().convertXMLtoTag("STARTASTATE",
					CMLib.coffeeMaker().getCharStateStr(startAdjState)));
		...
		if ((ables == null) || (ables.size() == 0))
			...
			for (int r = 0; r < ables.size(); r++) {
				str.append("<CABILITY>");
				str.append("<CACLASS>"
						+ ((AbilityMapper.AbilityMapping) ables.elementAt(r)).abilityID
						+ "</CACLASS>");
				str.append("<CALEVEL>"
						+ ((AbilityMapper.AbilityMapping) ables.elementAt(r)).qualLevel
						+ "</CALEVEL>");
				str.append("<CAPROFF>"
						+ ((AbilityMapper.AbilityMapping) ables.elementAt(r)).defaultProficiency
						+ "</CAPROFF>");
				str.append("<CAAGAIN>"
						+ ((AbilityMapper.AbilityMapping) ables.elementAt(r)).autoGain
						+ "</CAAGAIN>");
				str.append("<CASECR>"
						+ ((AbilityMapper.AbilityMapping) ables.elementAt(r)).isSecret
						+ "</CASECR>");
				str.append("<CAPARM>"
						+ ((AbilityMapper.AbilityMapping) ables.elementAt(r)).defaultParm
						+ "</CAPARM>");
				str.append("<CAPREQ>"
						+ ((AbilityMapper.AbilityMapping) ables.elementAt(r)).originalSkillPreReqList
						+ "</CAPREQ>");
				str.append("<CAMASK>"
						+ ((AbilityMapper.AbilityMapping) ables.elementAt(r)).extraMask
						+ "</CAMASK>");
				str.append("<CAMAXP>"
						+ ((AbilityMapper.AbilityMapping) ables.elementAt(r)).maxProficiency
						+ "</CAMAXP>");
				str.append("</CABILITY>");
			}
			str.append("</CABILITIES>");
		...
		if ((disallowedWeaponSet == null) || (disallowedWeaponSet.size() == 0))
			...
			for (Iterator i = disallowedWeaponSet.iterator(); i.hasNext();) {
				Integer I = (Integer) i.next();
				str.append(CMLib.xml().convertXMLtoTag("WCLASS",
						"" + I.intValue()));
			}
			str.append("</NOWEAPS>");
		}
		if ((requiredWeaponMaterials == null)
			...
			for (Iterator i = requiredWeaponMaterials.iterator(); i.hasNext();) {
				Integer I = (Integer) i.next();
				str.append(CMLib.xml().convertXMLtoTag("WMAT",
						"" + I.intValue()));
			}
			str.append("</NOWMATS>");
		}
		if ((outfit(null) == null) || (outfit(null).size() == 0))
			...
			for (Item I : outfit(null)) {
				str.append("<OFTITEM>");
				str.append(CMLib.xml().convertXMLtoTag("OFCLASS",
						CMClass.classID(I)));
				str.append(CMLib.xml().convertXMLtoTag("OFDATA",
						CMLib.xml().parseOutAngleBrackets(I.text())));
				str.append("</OFTITEM>");
			}
			str.append("</OUTFIT>");
		}
		for (int i = 0; i < securityGroups.length; i++)
			if (i < securityGroupLevels.length) {
				str.append(CMLib.xml().convertXMLtoTag("SSET" + i,
						CMParms.combineWithQuotes(securityGroups[i], 0)));
				str.append(CMLib.xml().convertXMLtoTag("SSETLEVEL" + i,
						securityGroupLevels[i].intValue()));
			}
		str.append(CMLib.xml().convertXMLtoTag("ARMORMINOR",
				"" + requiredArmorSourceMinor));
		str.append(CMLib.xml().convertXMLtoTag("STATCLASS",
				getCharClassLocatorID(statBuddy)));
		str.append(CMLib.xml().convertXMLtoTag("EVENTCLASS",
				getCharClassLocatorID(eventBuddy)));
		if (xtraValues == null)
			xtraValues = CMProps.getExtraStatCodesHolder(this);
		for (int i = this.getSaveStatIndex(); i < getStatCodes().length; i++)
			str.append(CMLib.xml().convertXMLtoTag(getStatCodes()[i],
					getStat(getStatCodes()[i])));
		str.append("</CCLASS>");
		return str.toString();
	}
	...
	public void setClassParms(String parms) {
		...
		List<XMLLibrary.XMLpiece> classData = CMLib.xml()
				.getContentsFromPieces(xml, "CCLASS");
		if (classData == null) {
		...
		if ((singleName != null) && (singleName.length() > 0)) {
			...
			int lastLevel = -1;
			while (true) {
				String name = CMLib.xml().getValFromPieces(classData,
						"NAME" + index);
				int level = CMLib.xml().getIntFromPieces(classData,
						"NAMELEVEL" + index);
		...
		if ((hitPointsFormula == null) || (hitPointsFormula.length() == 0)) {
			...
			hitPointsFormula = "((@x6<@x7)/" + hpDivisor + ")+(" + hpDice
					+ "*(1?" + hpDie + "))";
		}
		...
		levelsPerBonusDamage = CMLib.xml()
				.getIntFromPieces(classData, "LVLDAM");
		movementFormula = CMLib.xml().getValFromPieces(classData, "MVMTFRM");
		if ((movementFormula == null) || (movementFormula.length() == 0)) {
			int movementMultiplier = CMLib.xml().getIntFromPieces(classData,
					"LVLMOVE");
		...
		maxNonCraftingSkills = CMLib.xml()
				.getIntFromPieces(classData, "MAXNCS");
		...
		subClassRule = (SubClassRule) CMath.s_valueOf(SubClassRule.class, CMLib
				.xml().getValFromPieces(classData, "SUBRUL"),
				SubClassRule.BASEONLY);
		helpEntry = CMLib.xml().restoreAngleBrackets(
				CMLib.xml().getValFromPieces(classData, "HELP"));
		raceRequiredList = CMParms.parseCommas(
				CMLib.xml().getValFromPieces(classData, "RACQUAL"), true)
		...
		if ((mV != null) && (mV.size() > 0))
			for (XMLLibrary.XMLpiece p : mV)
				if (p.tag.equalsIgnoreCase("STAT"))
					statQuals.add(new Pair<String, Integer>(
							p.parms.get("NAME"), Integer.valueOf(CMath
									.s_int(p.parms.get("MIN")))));
		...
		List<XMLLibrary.XMLpiece> xV = CMLib.xml().getContentsFromPieces(
				classData, "CABILITIES");
		CMLib.ableMapper().delCharMappings(ID());
		if ((xV != null) && (xV.size() > 0))
			for (int x = 0; x < xV.size(); x++) {
				XMLLibrary.XMLpiece iblk = xV.get(x);
				if ((!iblk.tag.equalsIgnoreCase("CABILITY"))
						|| (iblk.contents == null))
					continue;
				// I hate backwards compatibility.
				String maxProff = CMLib.xml().getValFromPieces(iblk.contents,
						"CAMAXP");
				...
				CMLib.ableMapper()
						.addCharAbilityMapping(
								...
								CMLib.xml().getValFromPieces(iblk.contents,
										"CAPARM"),
								CMLib.xml().getBoolFromPieces(iblk.contents,
										"CAAGAIN"),
								CMLib.xml().getBoolFromPieces(iblk.contents,
										"CASECR"),
								CMParms.parseCommas(
										CMLib.xml().getValFromPieces(
												iblk.contents, "CAPREQ"), true),
								CMLib.xml().getValFromPieces(iblk.contents,
										"CAMASK"), null);
			}
		...
		if ((xV != null) && (xV.size() > 0)) {
			...
			for (int x = 0; x < xV.size(); x++) {
				XMLLibrary.XMLpiece iblk = xV.get(x);
				if ((!iblk.tag.equalsIgnoreCase("WCLASS"))
						|| (iblk.contents == null))
					continue;
				disallowedWeaponSet
						.add(Integer.valueOf(CMath.s_int(iblk.value)));
			}
		}
		...
		if ((xV != null) && (xV.size() > 0)) {
			...
			for (int x = 0; x < xV.size(); x++) {
				XMLLibrary.XMLpiece iblk = xV.get(x);
				if ((!iblk.tag.equalsIgnoreCase("WMAT"))
						|| (iblk.contents == null))
					continue;
				requiredWeaponMaterials.add(Integer.valueOf(CMath
						.s_int(iblk.value)));
			}
		}

		// now OUTFIT!
		List<XMLLibrary.XMLpiece> oV = CMLib.xml().getContentsFromPieces(
				classData, "OUTFIT");
		outfitChoices = null;
		if ((oV != null) && (oV.size() > 0)) {
			...
			for (int x = 0; x < oV.size(); x++) {
				...
				if ((!iblk.tag.equalsIgnoreCase("OFTITEM"))
						|| (iblk.contents == null))
					continue;
				Item newOne = CMClass.getItem(CMLib.xml().getValFromPieces(
						iblk.contents, "OFCLASS"));
				String idat = CMLib.xml().getValFromPieces(iblk.contents,
						"OFDATA");
		...
		int lastLevel = -1;
		while (true) {
			String groups = CMLib.xml().getValFromPieces(classData,
					"SSET" + index);
			int groupLevel = CMLib.xml().getIntFromPieces(classData,
					"SSETLEVEL" + index);
		...
		requiredArmorSourceMinor = CMLib.xml().getIntFromPieces(classData,
				"ARMORMINOR");
		setStat("STATCLASS",
				CMLib.xml().getValFromPieces(classData, "STATCLASS"));
		setStat("EVENTCLASS",
				CMLib.xml().getValFromPieces(classData, "EVENTCLASS"));
		xtraValues = CMProps.getExtraStatCodesHolder(this);
		for (int i = this.getSaveStatIndex(); i < getStatCodes().length; i++)
			setStat(getStatCodes()[i],
					CMLib.xml().getValFromPieces(classData, getStatCodes()[i]));
	}

	protected Vector getAbleSet() {
		Vector VA = new Vector(9);
		List<AbilityMapper.AbilityMapping> V = CMLib.ableMapper()
				.getUpToLevelListings(ID(), Integer.MAX_VALUE, true, false);
		for (AbilityMapper.AbilityMapping able : V) {
			...
			newMAP.qualLevel = CMLib.ableMapper().getQualifyingLevel(ID(),
					false, AID);
			newMAP.defaultProficiency = CMLib.ableMapper()
					.getDefaultProficiency(ID(), false, AID);
			newMAP.autoGain = CMLib.ableMapper().getDefaultGain(ID(), false,
					AID);
			newMAP.isSecret = CMLib.ableMapper().getSecretSkill(ID(), false,
					AID);
			newMAP.defaultParm = CMLib.ableMapper().getDefaultParm(ID(), false,
					AID);
			newMAP.originalSkillPreReqList = CMLib.ableMapper()
					.getPreReqStrings(ID(), false, AID);
			newMAP.extraMask = CMLib.ableMapper()
					.getExtraMask(ID(), false, AID);
			newMAP.maxProficiency = CMLib.ableMapper().getMaxProficiency(ID(),
					false, AID);
	...
	protected static String[] CODES = { "ID", "NAME", "BASE",
			"HITPOINTSFORMULA", "SUBRUL", "LVLPRAC", "RACQUAL", "LVLATT",
			"ATTATT", "FSTTRAN", "FSTPRAC", "LVLDAM", "MOVEMENTFORMULA",
			"ARMOR", "STRWEAP", "STRARM", "STRLMT", "STRBON", "QUAL", "PLAYER",
			"ESTATS", "ASTATS", "CSTATS", "ASTATE", "NUMCABLE", "GETCABLE",
			"GETCABLELVL", "GETCABLEPROF", "GETCABLEGAIN", "GETCABLESECR",
			"GETCABLEPARM", "NUMWEP", "GETWEP", "NUMOFT", "GETOFTID",
			"GETOFTPARM", "NUMMINSTATS", "MANAFORMULA", "GETMINSTAT",
			"DISFLAGS", "STARTASTATE", "NUMNAME", "NAMELEVEL", "NUMSSET",
			"SSET", "SSETLEVEL", "NUMWMAT", "GETWMAT", "ARMORMINOR",
			"STATCLASS", "EVENTCLASS", "GETCABLEPREQ", "GETCABLEMASK", "HELP",
			"LEVELCAP", "GETCABLEMAXP", "MAXNCS", "MAXCRS", "MAXCMS", "MAXLGS",
			"GETSTATMIN" };
	...
	public String getStat(String code) {
		...
		case 5:
			return "" + bonusPracLevel;
		case 6:
			return CMParms.toStringList(getRequiredRaceList());
		case 7:
			return "" + bonusAttackLevel;
		case 8:
			return "" + attackAttribute;
		...
		case 19:
			return "" + selectability;
		case 20:
			return (adjPStats == null) ? "" : CMLib.coffeeMaker()
					.getPhyStatsStr(adjPStats);
		case 21:
			return (adjStats == null) ? "" : CMLib.coffeeMaker()
					.getCharStatsStr(adjStats);
		case 22:
			return (setStats == null) ? "" : CMLib.coffeeMaker()
					.getCharStatsStr(setStats);
		case 23:
			return (adjState == null) ? "" : CMLib.coffeeMaker()
					.getCharStateStr(adjState);
		case 24:
			return Integer.toString(getAbleSet().size());
		case 25:
			return ((AbilityMapper.AbilityMapping) getAbleSet().elementAt(num)).abilityID;
		case 26:
			return Integer
					.toString(((AbilityMapper.AbilityMapping) getAbleSet()
							.elementAt(num)).qualLevel);
		case 27:
			return Integer
					.toString(((AbilityMapper.AbilityMapping) getAbleSet()
							.elementAt(num)).defaultProficiency);
		case 28:
			return Boolean
					.toString(((AbilityMapper.AbilityMapping) getAbleSet()
							.elementAt(num)).autoGain);
		case 29:
			return Boolean
					.toString(((AbilityMapper.AbilityMapping) getAbleSet()
							.elementAt(num)).isSecret);
		case 30:
			return ((AbilityMapper.AbilityMapping) getAbleSet().elementAt(num)).defaultParm;
		case 31:
			return ""
					+ ((disallowedWeaponSet != null) ? disallowedWeaponSet
							.size() : 0);
		case 32:
			return CMParms.toStringList(disallowedWeaponSet);
		case 33:
			return "" + ((outfit(null) != null) ? outfit(null).size() : 0);
		case 34:
			return ""
					+ ((outfit(null) != null) ? outfit(null).get(num).ID() : "");
		case 35:
			return ""
					+ ((outfit(null) != null) ? outfit(null).get(num).text()
							: "");
		case 36:
		...
		case 39:
			return "" + disableFlags;
		case 40:
			return (startAdjState == null) ? "" : CMLib.coffeeMaker()
					.getCharStateStr(startAdjState);
		case 41:
			return "" + names.length;
		case 42:
		...
		case 46:
			return ""
					+ ((requiredWeaponMaterials != null) ? requiredWeaponMaterials
							.size() : 0);
		case 47:
			return CMParms.toStringList(requiredWeaponMaterials);
		case 48:
			return "" + requiredArmorSourceMinor();
		case 49:
		...
		case 51:
			return ((AbilityMapper.AbilityMapping) getAbleSet().elementAt(num)).originalSkillPreReqList;
		case 52:
			return ((AbilityMapper.AbilityMapping) getAbleSet().elementAt(num)).extraMask;
		case 53:
			return helpEntry;
		case 54:
			return "" + levelCap;
	...
	public void setStat(String code, String val) {
		...
		case 3:
			...
			break;
		case 4:
			subClassRule = (SubClassRule) CMath.s_valueOf(SubClassRule.class,
					val.toUpperCase().trim(), SubClassRule.BASEONLY);
			break;
		case 5:
			bonusPracLevel = CMath.s_parseIntExpression(val);
			break;
		case 6:
			raceRequiredList = CMParms.parseCommas(val, true).toArray(
					new String[0]);
			break;
		case 7:
			...
			break;
		case 8:
			attackAttribute = CMath.s_parseListIntExpression(
					CharStats.CODES.NAMES(), val);
			break;
		case 9:
		...
		case 12:
			...
			break;
		case 13:
			allowedArmorLevel = CMath.s_parseListIntExpression(
					CharClass.ARMOR_DESCS, val);
			break;
		case 14:
		...
		case 18:
			...
			break;
		case 19:
			selectability = CMath.s_parseBitIntExpression(Area.THEME_BIT_NAMES,
					val);
			break;
		case 20:
			adjPStats = null;
		...
		case 25:
			CMLib.ableMapper()
					.addCharAbilityMapping(ID(), CMath.s_int(tempables[1]),
							val, CMath.s_int(tempables[2]),
							CMath.s_int(tempables[8]), tempables[5],
		...
		case 32: {
			...
			if (V.size() > 0) {
				...
				for (int v = 0; v < V.size(); v++)
					disallowedWeaponSet.add(Integer.valueOf(CMath.s_int(V
							.get(v))));
			} else
				disallowedWeaponSet = null;
			break;
		...
		case 36: {
			...
			for (int i = 0; i < CMath.s_int(val); i++)
				minimumStatRequirements[i] = new Pair<String, Integer>("",
						Integer.valueOf(0));
			break;
		}
		case 37:
		...
		case 40:
			startAdjState = null;
			if (val.length() > 0) {
				startAdjState = (CharState) CMClass
			...
			if (num > 0) {
				...
				if (newNames.length > names.length)
					for (int i = names.length; i < newNames.length; i++) {
						...
						newLevels[i] = Integer.valueOf(newLevels[i - 1]
								.intValue() + 1);
					}
				names = newNames;
		...
		case 49: {
			...
			try {
				if (statBuddy == null)
					statBuddy = (CharClass) CMClass.getLoadNewClassInstance(
							CMObjectType.CHARCLASS, val, true);
			} catch (Exception e) {
			}
			break;
		}
		case 50: {
			...
			try {
				if (eventBuddy == null)
					eventBuddy = (CharClass) CMClass.getLoadNewClassInstance(
							CMObjectType.CHARCLASS, val, true);
			} catch (Exception e) {
			}
			break;
		...
		case 59:
			...
			break;
		case 60:
			minimumStatRequirements[num].second = Integer.valueOf(CMath
					.s_int(val));
			break;
		default:
			CMProps.setStatCodeExtensionValue(getStatCodes(), xtraValues, code,
					val);
			break;
		}
	}
	...
	public void startCharacter(MOB mob, boolean isBorrowedClass,
		...
		if ((!verifyOnly) && (startAdjState != null)) {
			mob.baseState().setFatigue(
					mob.baseState().getFatigue() + startAdjState.getFatigue());
			mob.baseState().setHitPoints(
					mob.baseState().getHitPoints()
							+ startAdjState.getHitPoints());
			mob.baseState().setHunger(
					mob.baseState().getHunger() + startAdjState.getHunger());
			mob.baseState().setMana(
					mob.baseState().getMana() + startAdjState.getMana());
			mob.baseState()
					.setMovement(
							mob.baseState().getMovement()
									+ startAdjState.getMovement());
			mob.baseState().setThirst(
					mob.baseState().getThirst() + startAdjState.getThirst());
		}
	}
