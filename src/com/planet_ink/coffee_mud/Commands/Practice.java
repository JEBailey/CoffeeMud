...
public class Practice extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() < 2) {
			mob.tell("You have "
					+ mob.getPractices()
					+ " practice points.  Enter HELP PRACTICE for more information.");
			return false;
		}
		...
		MOB teacher = null;
		if (commands.size() > 1) {
			teacher = mob.location().fetchInhabitant(
		...
		if (teacher == null)
			for (int i = 0; i < mob.location().numInhabitants(); i++) {
				...
				if ((possTeach != null)
						&& (possTeach.findAbility(abilityName) != null)
						&& (possTeach != mob)) {
					teacher = possTeach;
					break;
				}
		...
		if (!myAbility.isSavable()) {
			mob.tell(abilityName
					+ " cannot be practiced, as it is a native skill.");
			return false;
		}
		...
		if (!mob.location().okMessage(mob, msg))
			return false;
		msg = CMClass.getMsg(teacher, mob, null, CMMsg.MSG_NOISYMOVEMENT,
				"<S-NAME> practice(s) '" + myAbility.name()
						+ "' with <T-NAMESELF>.");
		if (!mob.location().okMessage(mob, msg))
			return false;
