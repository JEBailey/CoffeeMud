...
public class ListCmd extends StdCommand {
	...
	public StringBuilder listAllQualifies(Session viewerS, Vector cmds) {
		...
		str.append(CMStrings.padRight("Skill ID",
				ListingLibrary.ColFixer.fixColWidth(20.0, viewerS)));
		str.append(CMStrings.padRight("Lvl",
				ListingLibrary.ColFixer.fixColWidth(4.0, viewerS)));
		str.append(CMStrings.padRight("Gain",
				ListingLibrary.ColFixer.fixColWidth(5.0, viewerS)));
		str.append(CMStrings.padRight("Prof",
				ListingLibrary.ColFixer.fixColWidth(5.0, viewerS)));
		str.append(CMStrings.padRight("Mask",
				ListingLibrary.ColFixer.fixColWidth(40.0, viewerS)));
		str.append("\n\r");
		for (AbilityMapper.AbilityMapping mapped : subMap.values()) {
			str.append(CMStrings.padRight(mapped.abilityID,
					ListingLibrary.ColFixer.fixColWidth(20.0, viewerS)));
			str.append(CMStrings.padRight("" + mapped.qualLevel,
					ListingLibrary.ColFixer.fixColWidth(4.0, viewerS)));
			str.append(CMStrings.padRight(mapped.autoGain ? "yes" : "no",
					ListingLibrary.ColFixer.fixColWidth(5.0, viewerS)));
			str.append(CMStrings.padRight("" + mapped.defaultProficiency,
					ListingLibrary.ColFixer.fixColWidth(5.0, viewerS)));
			str.append(CMStrings.padRight(mapped.extraMask,
					ListingLibrary.ColFixer.fixColWidth(40.0, viewerS)));
			str.append("\n\r");
		}
		str.append("\n\r");
	...
	public StringBuilder roomDetails(Session viewerS, Vector these,
			Room likeRoom) {
		return roomDetails(viewerS, these.elements(), likeRoom);
	}

	public StringBuilder roomDetails(Session viewerS, Enumeration these,
			Room likeRoom) {
		...
		for (Enumeration r = these; r.hasMoreElements();) {
			...
			if ((thisOne.length() > 0)
					&& (thisThang.getArea().Name().equals(likeRoom.getArea()
							.Name())))
				lines.append(CMStrings.padRightPreserve("^<LSTROOMID^>"
						+ thisOne + "^</LSTROOMID^>", COL_LEN1)
						+ ": "
						+ CMStrings.limit(thisThang.displayText(), COL_LEN2)
						+ "\n\r");
		}
		lines.append("\n\r");
		return lines;
	...
	public StringBuilder roomExpires(Session viewerS, Enumeration these,
			Room likeRoom) {
		StringBuilder lines = new StringBuilder("The time is: "
				+ CMLib.time().date2String(System.currentTimeMillis())
				+ "\n\r\n\r");
		if (!these.hasMoreElements())
			return lines;
		if (likeRoom == null)
		...
		for (Enumeration r = these; r.hasMoreElements();) {
			...
			if ((thisOne.length() > 0)
				...
				if (thisThang.expirationDate() == 0)
					...
					expires = CMLib.time().date2String(
							thisThang.expirationDate());
				lines.append(CMStrings.padRightPreserve("^<LSTROOMID^>"
						+ thisOne + "^</LSTROOMID^>", 30)
						+ ": " + expires + "\n\r");
			}
		}
		lines.append("\n\r");
	...
	public StringBuilder roomPropertyDetails(Session viewerS, Area A,
			String rest) {
		if (rest.trim().length() == 0)
			...
			return roomPropertyDetails(viewerS, A.getMetroMap(), rest.trim()
					.substring(5).trim());
		else if (rest.trim().toLowerCase().startsWith("world "))
			return roomPropertyDetails(viewerS, CMLib.map().rooms(), rest
					.trim().substring(6).trim());
		else
			return new StringBuilder(
					"Illegal parameters... try LIST REALESTATE AREA/WORLD (USERNAME/CLANNAME)");
	}

	public StringBuilder roomPropertyDetails(Session viewerS,
			Enumeration these, String owner) {
		StringBuilder lines = new StringBuilder("");
		if (!these.hasMoreElements())
			return lines;
		LandTitle t = null;
		...
		for (Enumeration r = these; r.hasMoreElements();) {
			...
			if (t != null) {
				thisOne = thisThang.roomID();
				if ((thisOne.length() > 0)
						&& ((owner == null) || (t.getOwnerName()
								.equalsIgnoreCase(owner))))
					lines.append(CMStrings.padRightPreserve("^<LSTROOMID^>"
							+ thisOne + "^</LSTROOMID^>", 30)
							+ ": "
							+ CMStrings.limit(thisThang.displayText(), 23)
							+ CMStrings.limit(" (" + t.getOwnerName() + ", $"
									+ t.getPrice() + ")", 20) + "\n\r");
			}
		}
		lines.append("\n\r");
	...
	public boolean canShowTo(MOB showTo, MOB show) {
		if ((show != null)
				&& (show.session() != null)
				&& (showTo != null)
				&& (((show.phyStats().disposition() & PhyStats.IS_CLOAKED) == 0) || ((CMSecurity
						.isAllowedAnywhere(showTo, CMSecurity.SecFlag.CLOAK) || CMSecurity
						.isAllowedAnywhere(showTo, CMSecurity.SecFlag.WIZINV)) && (showTo
						.phyStats().level() >= show.phyStats().level()))))
			return true;
		return false;
	}

	public StringBuffer getStuff(MOB mob, Vector commands, int start,
			Enumeration<Room> r) {
		...
		if ((who.equals("ROOM")) || (who.equals("ROOMS"))) {
			roomOnly = true;
			...
			rest = CMParms.combine(commands, start);
				...
				|| (who.equalsIgnoreCase("TYPE") || who
						.equalsIgnoreCase("TYPES"))) {
			return new StringBuffer(roomResources(mob.session(), r,
					mob.location()).toString());
		} else if ((who.equals("EXIT ")) || (who.equals("EXITS"))) {
			exitOnly = true;
			commands.remove(start);
			rest = CMParms.combine(commands, start);
		} else if ((who.equals("ITEM")) || (who.equals("ITEMS"))) {
			itemOnly = true;
			commands.remove(start);
			rest = CMParms.combine(commands, start);
		} else if ((who.equals("MOB")) || (who.equals("MOBS"))) {
			mobOnly = true;
			commands.remove(start);
			rest = CMParms.combine(commands, start);
		} else if (who.equals("MOBMASK") || who.equals("MOBMASK=")) {
			mobOnly = true;
			zapperMask = true;
			commands.remove(start);
			lines.append("^xMask used:^?^.^N " + CMLib.masking().maskDesc(who)
					+ "\n\r");
			compiledZapperMask = CMLib.masking().maskCompile(
					CMParms.combine(commands, start));
			rest = "";
		} else if (who.equals("ITEMMASK") || who.equals("ITEMMASK=")) {
			itemOnly = true;
			zapperMask = true;
			commands.remove(start);
			lines.append("^xMask used:^?^.^N " + CMLib.masking().maskDesc(who)
			...
			compiledZapperMask = CMLib.masking().maskCompile(
					CMParms.combine(commands, start));
			rest = "";
		} else if (who.equals("MOBMASK2") || who.equals("MOBMASK2=")) {
			mobOnly = true;
			zapperMask2 = true;
			commands.remove(start);
			rest = CMParms.combine(commands, start);
			lines.append("^xMask used:^?^.^N " + CMLib.masking().maskDesc(rest)
					+ "\n\r");
		} else if (who.equals("ITEMMASK2") || who.equals("ITEMMASK2=")) {
			itemOnly = true;
			zapperMask2 = true;
			commands.remove(start);
			rest = CMParms.combine(commands, start);
				...
				if ((R != null)
					...
					if (exitOnly) {
						for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
							...
							if ((E != null)
									&& ((rest.length() == 0)
											|| ((E.Name().length() > 0) && (CMLib
													.english().containsString(
													E.Name(), rest)))
											|| ((E.doorName().length() > 0) && CMLib
													.english().containsString(
															E.doorName(), rest)) || (CMLib
												.english().containsString(E
											.viewableText(mob, R).toString(),
											rest)))) {
								lines.append("^!"
										+ CMStrings.padRight(
												Directions.getDirectionName(d),
												17) + "^N| ");
								lines.append(" (^<LSTROOMID^>"
										+ CMLib.map().getExtendedRoomID(R)
										+ "^</LSTROOMID^>)");
					...
					if ((!mobOnly) && (!roomOnly) && (!exitOnly))
						for (int i = 0; i < R.numItems(); i++) {
							...
							if ((zapperMask) && (itemOnly)) {
								if (CMLib.masking().maskCheck(
										compiledZapperMask, I, true)) {
									lines.append("^!"
											+ CMStrings.padRight(cataMark(I)
													+ I.name(mob), 17) + "^N| ");
									lines.append(R.displayText(mob));
									lines.append(" (^<LSTROOMID^>"
											+ CMLib.map().getExtendedRoomID(R)
							...
							if ((!mobOnly) && (!roomOnly) && (!exitOnly)) {
								for (int i = 0; i < M.numItems(); i++) {
									...
									if ((zapperMask) && (itemOnly)) {
										...
										if (CMLib.masking().maskCheck(rest, I,
												true)) {
											lines.append("^!"
													+ CMStrings.padRight(
															cataMark(I)
								...
								ShopKeeper SK = CMLib.coffeeShops()
										.getShopKeeper(M);
								if (SK != null)
									for (Iterator<Environmental> i = SK
											.getShop().getStoreInventory(); i
											.hasNext();) {
										...
										if ((zapperMask) && (E instanceof Item)
											...
											if (CMLib.masking()
												...
												&& (E instanceof Item)
												&& (itemOnly)) {
											if (CMLib.masking().maskCheck(rest,
													E, true)) {
												lines.append("^!"
														+ CMStrings.padRight(
																cataMark(E)
	...
	public StringBuilder roomTypes(MOB mob, Enumeration<Room> these,
			Room likeRoom, Vector commands) {
		StringBuilder lines = new StringBuilder("");
		if (!these.hasMoreElements())
			return lines;
		if (likeRoom == null)
		...
		if (commands.size() == 1) {
			...
			for (Enumeration r = these; r.hasMoreElements();) {
				...
				thisOne = thisThang.roomID();
				if ((thisOne.length() > 0)
						&& (thisThang.getArea().Name().equals(likeRoom
								.getArea().Name())))
					lines.append(CMStrings.padRightPreserve(thisOne, 30) + ": "
							+ thisThang.ID() + "\n\r");
			}
			lines.append("\n\r");
		} else {
	...
	public StringBuilder roomResources(Session viewerS, Vector these,
			Room likeRoom) {
		return roomResources(viewerS, these.elements(), likeRoom);
	}

	public StringBuilder roomResources(Session viewerS, Enumeration these,
			Room likeRoom) {
		...
		StringBuilder lines = new StringBuilder(CMStrings.padRight("Room ID#",
				COL_LEN1)
				+ "| "
	...
	public StringBuilder areaConquests(Session viewerS, Enumeration these) {
		...
		StringBuilder lines = new StringBuilder(CMStrings.padRight("Area",
				COL_LEN1)
				+ "| "
	...
	public void dumpThreadGroup(Session viewerS, StringBuilder lines,
			ThreadGroup tGroup, boolean ignoreZeroTickThreads) {
		...
		for (int i = 0; i < ac; ++i) {
			if (tArray[i] != null) {
				...
				if (tArray[i] instanceof MudHost)
					summary = CMClass.classID(tArray[i]) + ": "
							+ ((MudHost) tArray[i]).getStatus();
				else {
					final Runnable R = CMLib.threads().findRunnableByThread(
							tArray[i]);
					if (R instanceof TickableGroup)
						summary = ((TickableGroup) R).getName() + ": "
								+ ((TickableGroup) R).getStatus();
					else if (R instanceof Session) {
						final Session S = (Session) R;
						final MOB mob = S.mob();
						final String mobName = (mob == null) ? "null" : mob
								.Name();
						summary = "session "
								+ mobName
								+ ": "
								...
								+ ": "
								+ CMParms.combineWithQuotes(S.getPreviousCMD(),
										0);
					} else if (R instanceof CMRunnable)
						summary = CMClass.classID(R) + ": active for "
								+ ((CMRunnable) R).activeTimeMillis() + "ms";
		...
		if (agc > 0) {
			...
			for (int i = 0; i < agc; ++i) {
				if (tgArray[i] != null)
					dumpThreadGroup(viewerS, lines, tgArray[i],
	...
	public StringBuilder listThreads(Session viewerS, MOB mob,
		...
		try {
			...
			lines.append("\n\rBastards! Exception while listing threads: "
					+ e.getMessage() + "\n\r");
		}
		return lines;
	...
	public void addScripts(DVector DV, Room R, ShopKeeper SK, MOB M, Item I,
			PhysicalAgent E) {
		if (E == null)
			return;
		for (Enumeration<Behavior> e = E.behaviors(); e.hasMoreElements();) {
			...
			if (B instanceof ScriptingEngine) {
				...
				String nonFiles = ((ScriptingEngine) B).getVar("*",
						"COFFEEMUD_SYSTEM_INTERNAL_NONFILENAME_SCRIPT");
	...
	public void addShopScripts(DVector DV, Room R, MOB M, Item I,
			Environmental E) {
		if (E == null)
			return;
		ShopKeeper SK = CMLib.coffeeShops().getShopKeeper(E);
		if (SK != null) {
			for (Iterator<Environmental> i = SK.getShop().getStoreInventory(); i
					.hasNext();) {
	...
	public StringBuilder listScripts(Session viewerS, MOB mob, Vector cmds) {
		...
		if (cmds.size() == 0)
			return new StringBuilder(
					"List what script details? Try LIST SCRIPTS (COUNT/DETAILS/CUSTOM)");
		...
		for (Enumeration e = CMLib.map().areas(); e.hasMoreElements();) {
			...
			if (A == null)
				continue;
			for (Enumeration<WorldMap.LocatedPair> ae = CMLib.map()
					.scriptHosts(A); ae.hasMoreElements();) {
				LP = ae.nextElement();
				if (LP == null)
					continue;
				AE = LP.obj();
		...
		if (rest.equalsIgnoreCase("COUNT")) {
			final int COL_LEN1 = ListingLibrary.ColFixer.fixColWidth(50.0,
					viewerS);
			final int COL_LEN2 = ListingLibrary.ColFixer.fixColWidth(5.0,
					viewerS);
			...
			final int COL_LEN1 = ListingLibrary.ColFixer.fixColWidth(30.0,
					viewerS);
			final int COL_LEN2 = ListingLibrary.ColFixer.fixColWidth(20.0,
					viewerS);
			final int COL_LEN3 = ListingLibrary.ColFixer.fixColWidth(25.0,
					viewerS);
			lines = new StringBuilder("^x")
					.append(CMStrings.padRight("Script File", COL_LEN1))
			...
			if (scriptTree.size() > 0) {
				for (int d = 0; d < scriptTree.size(); d++) {
					String scriptFilename = (String) scriptTree.elementAt(d, 1);
					Environmental host = (Environmental) scriptTree.elementAt(
							d, 2);
					...
					lines.append(CMStrings.padRight(CMLib.map()
							.getExtendedRoomID(room), COL_LEN3));
					lines.append("^.^N\n\r");
				}
			}
		} else if (rest.equalsIgnoreCase("CUSTOM")) {
			lines = new StringBuilder("^xCustom Scripts").append("^.^N\n\r");
			scriptTree.sortBy(1);
			if (scriptTree.size() > 0) {
				for (int d = 0; d < scriptTree.size(); d++) {
					String scriptFilename = (String) scriptTree.elementAt(d, 1);
					if (scriptFilename.startsWith("*Custom*")) {
						Environmental host = (Environmental) scriptTree
								.elementAt(d, 2);
						Room room = (Room) scriptTree.elementAt(d, 3);
						lines.append("^xHost: ^.^N").append(host.Name())
								.append(", ^xLocation: ^.^N")
								.append(CMLib.map().getExtendedRoomID(room));
						lines.append("^.^N\n\r");
			...
			return new StringBuilder(
					"Invalid parameter for LIST SCRIPTS.  Enter LIST SCRIPTS alone for help.");
		return lines;
	}
	...
	public StringBuilder listLinkages(Session viewerS, MOB mob, String rest) {
		...
		for (; a.hasMoreElements();) {
			...
			buf.append(A.name() + "\t" + A.numberOfProperIDedRooms()
					+ " rooms\t");
			...
			for (Enumeration r = A.getCompleteMap(); r.hasMoreElements();) {
				...
				for (int m = 0; m < R.numInhabitants(); m++) {
					...
					if ((M != null) && (M.isMonster())
						...
						numMobs++;
						if ((useFaction != null)
								&& (CMLib.factions().getFaction(
										useFaction.factionID()) != null)
								&& (M.fetchFaction(useFaction.factionID()) != Integer.MAX_VALUE))
							totalAlignment += M.fetchFaction(useFaction
									.factionID());
				...
				if (R.roomID().length() > 0)
					for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
						...
						if ((R2 != null) && (R2.getArea() != R.getArea())) {
							ext.append(Directions.getDirectionName(d) + " to "
									+ R2.getArea().name() + " (" + R.roomID()
									+ "/" + R2.roomID() + ") ");
			...
			if (numMobs > 0)
				buf.append(numMobs + " mobs\t" + (totalLevels / numMobs)
						+ " avg levels\t");
			if ((numMobs > 0)
					&& (useFaction != null)
					&& (CMLib.factions().getFaction(useFaction.factionID()) != null))
				buf.append((totalAlignment / numMobs) + " avg "
						+ useFaction.name());
	...
	public StringBuilder journalList(Session viewerS, String partialjournal) {
		...
		for (Enumeration<JournalsLibrary.CommandJournal> e = CMLib.journals()
			...
			if ((CMJ.NAME() + "S").startsWith(partialjournal.toUpperCase()
					.trim()))
				journal = CMJ.NAME().trim();
		}
		if (journal == null)
			return buf;
		List<JournalsLibrary.JournalEntry> V = CMLib.database()
				.DBReadJournalMsgs("SYSTEM_" + journal + "S");
		...
		if (V != null) {
			buf.append("\n\r^x" + CMStrings.padRight("#", COL_LEN1 + 2)
					+ CMStrings.padRight("From", COL_LEN2) + " Entry^.^N\n\r");
			buf.append("---------------------------------------------\n\r");
			for (int j = 0; j < V.size(); j++) {
				JournalsLibrary.JournalEntry entry = V.get(j);
				String from = entry.from;
				String message = entry.msg;
				buf.append(CMStrings.padRight((j + 1) + "", COL_LEN1) + ") "
						+ CMStrings.padRight(from, COL_LEN2) + " " + message
						+ "\n\r");
			}
		}
		return buf;
	...
	public StringBuilder listReports(Session viewerS, MOB mob) {
		...
		buf.append("The system has been running for ^H"
				+ CMLib.english().returnTime(totalTime, 0) + "^?.\n\r");
		long free = Runtime.getRuntime().freeMemory() / 1024;
		long total = Runtime.getRuntime().totalMemory() / 1024;
		buf.append("The system is utilizing ^H" + (total - free)
				+ "^?kb out of ^H" + total + "^?kb.\n\r");
		...
		long totalMillis = CMath.s_long(CMLib.threads().systemReport(
				"totalMillis"));
		long totalTicks = CMath.s_long(CMLib.threads().systemReport(
				"totalTicks"));
		buf.append("There are ^H" + totalTickers + "^? ticking objects in ^H"
				+ tickGroupSize + "^? groups.\n\r");
		buf.append("The ticking objects have consumed: ^H"
				+ CMLib.english().returnTime(totalMillis, totalTicks)
				+ "^?.\n\r");
		/*
		 * String topGroupNumber=CMLib.threads().systemReport("topGroupNumber");
		 * long
		 * topGroupMillis=CMath.s_long(CMLib.threads().systemReport("topGroupMillis"
		 * )); long
		 * topGroupTicks=CMath.s_long(CMLib.threads().systemReport("topGroupTicks"
		 * )); long topObjectMillis=CMath.s_long(CMLib.threads().systemReport(
		 * "topObjectMillis")); long
		 * topObjectTicks=CMath.s_long(CMLib.threads().
		 * systemReport("topObjectTicks"));
		 * buf.append("The most active group, #^H"
		 * +topGroupNumber+"^?, has consumed: ^H"
		 * +CMLib.english().returnTime(topGroupMillis,topGroupTicks)+"^?.\n\r");
		 * String
		 * topObjectClient=CMLib.threads().systemReport("topObjectClient");
		 * String topObjectGroup=CMLib.threads().systemReport("topObjectGroup");
		 * if(topObjectClient.length()>0) {
		 * buf.append("The most active object has been '^H"
		 * +topObjectClient+"^?', from group #^H"+topObjectGroup+"^?.\n\r");
		 * buf.append("That object has consumed: ^H"+CMLib.english().returnTime(
		 * topObjectMillis,topObjectTicks)+"^?.\n\r"); }
		 */
		buf.append("\n\r");
		buf.append("^xServices report:^.^N\n\r");
		buf.append("There are ^H"
				+ CMLib.threads().systemReport("numactivethreads")
				+ "^? active out of ^H"
				+ CMLib.threads().systemReport("numthreads")
				+ "^? live worker threads.\n\r");
		int threadNum = 0;
		String threadName = CMLib.threads().systemReport(
				"Thread" + threadNum + "name");
		while (threadName.trim().length() > 0) {
			long saveThreadMilliTotal = CMath.s_long(CMLib.threads()
					.systemReport("Thread" + threadNum + "MilliTotal"));
			long saveThreadTickTotal = CMath.s_long(CMLib.threads()
					.systemReport("Thread" + threadNum + "TickTotal"));
			buf.append("Service '"
					+ threadName
					+ "' has consumed: ^H"
					+ CMLib.english().returnTime(saveThreadMilliTotal,
							saveThreadTickTotal)
					+ " ("
					+ CMLib.threads().systemReport(
							"Thread" + threadNum + "Status") + ")^?.");
			buf.append("\n\r");
			threadNum++;
			threadName = CMLib.threads().systemReport(
					"Thread" + threadNum + "name");
		}
		buf.append("\n\r");
		buf.append("^xSession report:^.^N\n\r");
		long totalMOBMillis = CMath.s_long(CMLib.threads().systemReport(
				"totalMOBMillis"));
		long totalMOBTicks = CMath.s_long(CMLib.threads().systemReport(
				"totalMOBTicks"));
		buf.append("There are ^H" + CMLib.sessions().getCountLocalOnline()
				+ "^? ticking players logged on.\n\r");
		buf.append("The ticking players have consumed: ^H"
				+ CMLib.english().returnTime(totalMOBMillis, totalMOBTicks)
				+ "^?.\n\r");
		/*
		 * long
		 * topMOBMillis=CMath.s_long(CMLib.threads().systemReport("topMOBMillis"
		 * )); long
		 * topMOBTicks=CMath.s_long(CMLib.threads().systemReport("topMOBTicks"
		 * )); String topMOBClient=CMLib.threads().systemReport("topMOBClient");
		 * if(topMOBClient.length()>0) {
		 * buf.append("The most active mob has been '^H"
		 * +topMOBClient+"^?'\n\r");
		 * buf.append("That mob has consumed: ^H"+CMLib
		 * .english().returnTime(topMOBMillis,topMOBTicks)+"^?.\n\r"); }
		 */
		return buf;
	}
	...
	public void listUsers(Session viewerS, MOB mob, Vector commands) {
		...
		switch (sortBy) {
		case 6:
			head.append(CMStrings.padRight("E-Mail", COL_LEN5) + " ");
			break;
		case 7:
			head.append(CMStrings.padRight("IP Address", COL_LEN5) + " ");
			break;
		default:
			head.append(CMStrings.padRight("Last", COL_LEN6) + " ");
			break;
		}
		...
		while ((oldSet.size() > 0) && (sortBy >= 0) && (sortBy <= 7)) {
			...
			if ((sortBy < 3) || (sortBy > 4)) {
				...
				for (int u = 1; u < oldSet.size(); u++) {
					...
					if (lib.getThinSortValue(selected, sortBy).compareTo(
							lib.getThinSortValue(U, sortBy)) > 0)
						selected = U;
				}
				if (selected != null) {
				...
				for (int u = 1; u < oldSet.size(); u++) {
					PlayerLibrary.ThinPlayer U = oldSet.get(u);
					if (CMath.s_long(lib.getThinSortValue(selected, sortBy)) > CMath
							.s_long(lib.getThinSortValue(U, sortBy)))
						selected = U;
				}
				if (selected != null) {
					oldSet.remove(selected);
		...
		for (int u = 0; u < allUsers.size(); u++) {
			...
			default:
				head.append(CMStrings.padRight(
						CMLib.time().date2String(U.last), COL_LEN6) + " ");
				break;
			}
			head.append("] "
					+ CMStrings.padRight("^<LSTUSER^>" + U.name
							+ "^</LSTUSER^>", COL_LEN7));
			head.append("\n\r");
		}
	...
	public void listAccounts(Session viewerS, MOB mob, Vector commands) {
		...
		switch (sortBy) {
		default:
			head.append(CMStrings.padRight("E-Mail", COL_LEN3) + " ");
			break;
		case 7:
			head.append(CMStrings.padRight("IP Address", COL_LEN3) + " ");
			break;
		}
		...
		while ((oldSet.size() > 0) && (sortBy >= 0) && (sortBy <= 7)) {
			...
			if ((sortBy < 3) || (sortBy > 4)) {
				...
				if (selected != null) {
					PlayerLibrary.ThinPlayer selectedU = thinAcctHash
							.get(selected.accountName());
					for (int u = 1; u < oldSet.size(); u++) {
						PlayerAccount acct = oldSet.get(u);
						PlayerLibrary.ThinPlayer U = thinAcctHash.get(acct
								.accountName());
						if (lib.getThinSortValue(selectedU, sortBy).compareTo(
								lib.getThinSortValue(U, sortBy)) > 0) {
							selected = acct;
							selectedU = U;
						}
				...
				if (selected != null) {
					...
					for (int u = 1; u < oldSet.size(); u++) {
						...
						PlayerLibrary.ThinPlayer U = thinAcctHash.get(acct
								.accountName());
						if (CMath.s_long(lib
								.getThinSortValue(selectedU, sortBy)) > CMath
								.s_long(lib.getThinSortValue(U, sortBy))) {
							selected = acct;
							selectedU = U;
						}
		...
		for (int u = 0; u < allAccounts.size(); u++) {
			...
			line.append(CMStrings.padRight(
					CMLib.time().date2String(U.lastDateTime()), COL_LEN2)
					+ " ");
			...
			default:
				...
				break;
			case 7:
				line.append(CMStrings.padRight(U.lastIP(), COL_LEN3) + " ");
				break;
			}
			line.append("] ");
			int len = line.length();
	...
	public StringBuilder listRaces(Session viewerS, Enumeration these,
			String rest) {
		...
		if (shortList) {
			...
			for (Enumeration e = these; e.hasMoreElements();) {
				...
				if ((restRest.length() == 0)
						...
						|| (CMLib.english().containsString(R.racialCategory(),
								restRest)))
			...
			for (Enumeration e = these; e.hasMoreElements();) {
				...
				if ((restRest.length() == 0)
					...
					lines.append(CMStrings.padRight(
							R.ID() + (R.isGeneric() ? "*" : "") + " ("
	...
	public StringBuilder listCharClasses(Session viewerS, Enumeration these,
			boolean shortList) {
		StringBuilder lines = new StringBuilder("");
		if (!these.hasMoreElements())
			return lines;
		...
		if (shortList) {
			...
			for (Enumeration e = these; e.hasMoreElements();) {
				...
				lines.append(CMStrings.padRight(
						thisThang.ID() + (thisThang.isGeneric() ? "*" : "")
	...
	public StringBuilder listRaceCats(Session viewerS, Enumeration these,
			boolean shortList) {
		StringBuilder lines = new StringBuilder("");
		if (!these.hasMoreElements())
			return lines;
	...
	public StringBuilder listQuests(Session viewerS) {
		...
		if (CMLib.quests().numQuests() == 0)
			...
			buf.append("\n\r^xQuest Report:^.^N\n\r");
			final int COL_LEN1 = ListingLibrary.ColFixer.fixColWidth(5.0,
					viewerS);
			final int COL_LEN2 = ListingLibrary.ColFixer.fixColWidth(30.0,
					viewerS);
			buf.append("\n\r^x" + CMStrings.padRight("#", COL_LEN1)
					+ CMStrings.padRight("Name", COL_LEN2) + " Status^.^N\n\r");
			for (int i = 0; i < CMLib.quests().numQuests(); i++) {
				Quest Q = CMLib.quests().fetchQuest(i);
				if (Q != null) {
					buf.append(CMStrings.padRight("" + (i + 1), COL_LEN1)
							+ CMStrings.padRight("^<LSTQUEST^>" + Q.name()
									+ "^</LSTQUEST^>", COL_LEN2) + " ");
					if (Q.running()) {
						String minsLeft = "(" + Q.minsRemaining()
								+ " mins left)";
						if (Q.duration() == 0)
							minsLeft = "(Eternal)";
						...
						if (min > 0) {
							...
							if (min > 60000)
								buf.append("waiting (" + (min / 60000)
										+ " minutes left)");
							else
								buf.append("waiting (" + (min / 1000)
										+ " seconds left)");
						} else
	...
	public StringBuilder listJournals(Session viewerS) {
		...
		if (journals.size() == 0)
			...
			for (int i = 0; i < journals.size(); i++) {
				...
				int messages = CMLib.database().DBCountJournal(journal, null,
						null);
				buf.append(CMStrings.padRight("" + (i + 1), COL_LEN1)
						+ CMStrings.padRight(journal, COL_LEN2) + " "
						+ messages);
				buf.append("^N\n\r");
			}
		}
	...
	public StringBuilder listTicks(Session viewerS, String whichGroupStr) {
		...
		if (x > 0) {
			String lastWord = whichGroupStr.substring(x + 1).trim()
					.toLowerCase();
			final String[] validCols = { "tickername", "tickerid",
					"tickerstatus", "tickerstatusstr", "tickercodeword",
					"tickertickdown", "tickerretickdown", "tickermillitotal",
					"tickermilliavg", "tickerlaststartmillis",
					"tickerlaststopmillis", "tickerlaststartdate",
					"tickerlaststopdate", "tickerlastduration",
					"tickersuspended" };
			...
			if (!finalCol.equals(lastWord))
				return new StringBuilder("Invalid column: '" + lastWord
						+ "'.  Valid cols are: "
						+ CMParms.toStringList(validCols));
			else {
				...
				finalColName = CMStrings.limit(
						CMStrings.capitalizeAndLower(finalColName), 5);
			}
		}

		if ("ACTIVE".startsWith(whichGroupStr.toUpperCase())
				&& (whichGroupStr.length() > 0))
			activeOnly = true;
		else if ("PROBLEMS".startsWith(whichGroupStr.toUpperCase())
				&& (whichGroupStr.length() > 0)) {
			...
			for (String set : sets) {
				...
				if (pair.size() == 2)
					whichTicks.add(new Pair<Integer, Integer>(Integer
							.valueOf(CMath.s_int(pair.get(0))), Integer
							.valueOf(CMath.s_int(pair.get(1)))));
			}
		} else if (CMath.isInteger(whichGroupStr)
				&& (whichGroupStr.length() > 0)) {
		...
		if (!activeOnly)
			msg.append(CMStrings.padRight("Grp", COL_LEN1)
					+ CMStrings.padRight("Client", COL_LEN2) + " "
					+ CMStrings.padRight("ID", COL_LEN3)
					+ CMStrings.padRight(finalColName, COL_LEN4));
		msg.append(CMStrings.padRight("Grp", COL_LEN1)
				+ CMStrings.padRight("Client", COL_LEN2) + " "
				+ CMStrings.padRight("ID", COL_LEN3)
				+ CMStrings.padRight(finalColName, COL_LEN4) + "\n\r");
		...
		for (int group = 0; group < numGroups; group++) {
			if ((whichGroups == null)
					|| (whichGroups.contains(Integer.valueOf(group)))) {
				int tickersSize = CMath.s_int(CMLib.threads().tickInfo(
						"tickersSize" + group));
				for (int tick = 0; tick < tickersSize; tick++) {
					if ((whichTicks == null)
							|| (whichTicks.contains(new Pair<Integer, Integer>(
									Integer.valueOf(group), Integer
											.valueOf(tick))))) {
						long tickerlaststartdate = CMath.s_long(CMLib.threads()
								.tickInfo(
										"tickerlaststartmillis" + group + "-"
												+ tick));
						long tickerlaststopdate = CMath.s_long(CMLib.threads()
								.tickInfo(
										"tickerlaststopmillis" + group + "-"
												+ tick));
						boolean isActive = (tickerlaststopdate < tickerlaststartdate);
						if ((!activeOnly) || (isActive)) {
							String name = CMLib.threads().tickInfo(
									"tickerName" + group + "-" + tick);
							if ((mask == null)
									|| (name.toUpperCase().indexOf(mask) >= 0)) {
								String id = CMLib.threads().tickInfo(
										"tickerID" + group + "-" + tick);
								String status = CMLib.threads().tickInfo(
										finalCol + group + "-" + tick);
								boolean suspended = CMath.s_bool(CMLib
										.threads().tickInfo(
												"tickerSuspended" + group + "-"
														+ tick));
								if (((col++) >= 2) || (activeOnly)) {
									msg.append("\n\r");
									col = 1;
								...
								chunk = CMStrings
										.padRight("" + group, COL_LEN1)
										+ CMStrings.padRight(name, COL_LEN2)
										+ " "
										+ CMStrings.padRight(id + "", COL_LEN3)
										+ CMStrings
												.padRight(
														(activeOnly ? (status + (suspended ? "*"
																: ""))
																: status
																		+ (suspended ? "*"
																				: "")),
			...
			for (Enumeration r = CMLib.map().rooms(); r.hasMoreElements();) {
				...
				if (R.roomID().length() > 0)
					for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
						Room R2 = R.rawDoors()[d];
						if ((R2 != null)
								&& (R2.rawDoors()[Directions
										.getOpDirectionCode(d)] != R))
							str.append(CMStrings.padRight(R.roomID(), 30)
									+ ": " + Directions.getDirectionName(d)
									+ " to " + R2.roomID() + "\n\r");
					}
			}
		...
		try {
			for (Enumeration r = CMLib.map().rooms(); r.hasMoreElements();) {
				...
				for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
					...
					if ((R2 == null) && (E2 != null))
						str.append(CMStrings.padRight(R.roomID(), 30) + ": "
								+ Directions.getDirectionName(d) + " to "
								+ E2.temporaryDoorLink() + " ("
								+ E2.displayText() + ")\n\r");
				}
			}
		} catch (NoSuchElementException e) {
	...
	public String listResources(MOB mob, String parm) {
		...
		if (!keyIter.hasNext()) {
			...
			if (o instanceof List)
				...
				str.append(CMParms.toStringList((Map) o));
		...
		Enumeration<String> keys = new IteratorEnumeration<String>(
				Resources.findResourceKeys(parm));
	...
	public String listHelpFileRequests(MOB mob, String rest) {
		...
		StringBuilder str = new StringBuilder(
				"^HHelp entries, sorted by popularity: ^N\n\r");
		for (int d = 0; d < sightingsDV.size(); d++)
			str.append(
					"^w"
							+ CMStrings.padRight(sightingsDV.elementAt(d, 2)
									.toString(), 4)).append(" ")
	...
	public String listRecipes(MOB mob, String rest) {
		...
		if (rest.trim().length() == 0) {
			...
			Ability A = CMClass.findAbility(rest, Ability.ACODE_COMMON_SKILL,
					-1, false);
			if (A == null)
				str.append("Ability '" + rest
						+ "' does not exist -- try list recipes");
			else if (!(A instanceof ItemCraftor))
				str.append("Ability '" + A.ID()
						+ "' is not a proper ability -- try list recipes");
			else {
				ItemCraftor iA = (ItemCraftor) A;
				if ((iA.parametersFormat() == null)
						|| (iA.parametersFormat().length() == 0)
						...
						|| (iA.parametersFile().length() == 0))
					str.append("Ability '" + A.ID()
							+ "' is not editable -- try list recipes");
				else
	...
	public String listEnvResources(Session viewerS, String rest) {
		...
		final int COL_LEN7 = COL_LEN1 + 1 + COL_LEN2 + 1 + COL_LEN3 + 1
				+ COL_LEN4 + 1 + COL_LEN5 + 1;
		...
		for (int i : RawMaterial.CODES.ALL()) {
			String resourceName = CMStrings
					.capitalizeAndLower(RawMaterial.CODES.NAME(i).toLowerCase());
			String materialName = RawMaterial.Material
					.findByMask(i & RawMaterial.MATERIAL_MASK).noun()
					.toLowerCase();
			if ((rest.length() == 0) || (resourceName.indexOf(rest) >= 0)
				...
				str.append(CMStrings.padRight("" + RawMaterial.CODES.VALUE(i),
						COL_LEN3 + 1));
				str.append(CMStrings.padRight(
						"" + RawMaterial.CODES.FREQUENCY(i), COL_LEN4 + 1));
				str.append(CMStrings.padRight(
						"" + RawMaterial.CODES.HARDNESS(i), COL_LEN5 + 1));
				...
				for (Enumeration e = CMClass.locales(); e.hasMoreElements();) {
					...
					if (!(R instanceof GridLocale))
						if ((R.resourceChoices() != null)
								&& (R.resourceChoices().contains(Integer
										.valueOf(i))))
							locales.append(R.ID() + " ");
				}
				while (locales.length() > COL_LEN6) {
					...
					locales = new StringBuilder(locales.toString().substring(
							COL_LEN6));
				}
	...
	public int getMyCmdCode(MOB mob, String s) {
		...
		for (Enumeration<JournalsLibrary.CommandJournal> e = CMLib.journals()
				.commandJournals(); e.hasMoreElements();) {
			JournalsLibrary.CommandJournal CMJ = e.nextElement();
			if (((CMJ.NAME() + "S").startsWith(s) || CMJ.NAME().equals(s) || CMJ
					.NAME().replace('_', ' ').equals(s))
					&& ((CMSecurity.isJournalAccessAllowed(mob, CMJ.NAME())) || CMSecurity
							.isAllowed(mob, mob.location(),
									CMSecurity.SecFlag.LISTADMIN)))
				return 29;
		}
		return -1;
	...
	public String listComponents(Session viewerS) {
		StringBuilder buf = new StringBuilder(
				"^xAll Defined Spells and required components: ^N\n\r");
		for (String ID : CMLib.ableMapper().getAbilityComponentMap().keySet()) {
			List<AbilityComponent> DV = CMLib.ableMapper()
					.getAbilityComponentMap().get(ID);
			if (DV != null)
				buf.append(CMStrings.padRight(ID, 20) + ": "
						+ CMLib.ableMapper().getAbilityComponentDesc(null, ID)
						+ "\n\r");
		}
		if (buf.length() == 0)
			return "None defined.";
	...
	public String listExpertises(Session viewerS) {
		StringBuilder buf = new StringBuilder(
				"^xAll Defined Expertise Codes: ^N\n\r");
	...
	public String listTitles(Session viewerS) {
		StringBuilder buf = new StringBuilder(
				"^xAll Defined Auto-Titles: ^N\n\r");
		for (Enumeration e = CMLib.titles().autoTitles(); e.hasMoreElements();) {
			String title = (String) e.nextElement();
			String maskDesc = CMLib.masking().maskDesc(
					CMLib.titles().getAutoTitleMask(title));
	...
	public String listClanGovernments(Session viewerS, List commands) {
		...
		final int SCREEN_LEN = ListingLibrary.ColFixer.fixColWidth(78.0,
				viewerS);
		for (ClanGovernment G : CMLib.clans().getStockGovernments())
			buf.append(CMStrings.padRight(G.getName(), glen) + ": "
					+ CMStrings.limit(G.getShortDesc(), SCREEN_LEN - glen - 2)
					+ "\n\r");
		return buf.toString();
	}
	...
	public String listClans(Session viewerS, List commands) {
		...
		for (Enumeration<Clan> c = CMLib.clans().clans(); c.hasMoreElements();) {
			...
			buf.append(CMStrings.padRight(C.clanID(), clen)
					+ ": "
					+ CMStrings.limit(C.getMemberList().size() + " members",
							SCREEN_LEN - clen - 2) + "\n\r");
		}
		return buf.toString();
	}
	...
	public StringBuilder listContent(MOB mob, Vector commands) {
		...
		if (rest.equalsIgnoreCase("area"))
			...
			if (A != null)
				...
				if (R != null)
					...
					return new StringBuilder("There's no such place as '"
							+ rest + "'");
			}
		}
		StringBuilder buf = new StringBuilder("");
		...
		for (; roomsToDo.hasMoreElements();) {
			...
			if (R.roomID().length() == 0)
				continue;
			set = CMLib.database().DBReadRoomData(
					CMLib.map().getExtendedRoomID(R), false);
			if ((set == null) || (set.size() == 0))
				buf.append("'" + CMLib.map().getExtendedRoomID(R)
				...
				for (int m = 0; m < TR.numInhabitants(); m++) {
					...
					if (M == null)
						continue;
					buf.append("^M"
							+ CMStrings.padRight(M.ID(), SCREEN_LEN1)
							+ ": "
							+ CMStrings.padRight(M.displayText(), SCREEN_LEN2)
							+ ": "
							+ CMStrings.padRight(M.phyStats().level() + "",
									SCREEN_LEN3) + ": "
							+ CMLib.flags().getAlignmentName(M) + "^N\n\r");
					for (int i = 0; i < M.numItems(); i++) {
						Item I = M.getItem(i);
						if (I != null)
							buf.append("    ^I"
									+ CMStrings.padRight(I.ID(), SCREEN_LEN1)
									+ ": "
									+ CMStrings.padRight(
											(I.displayText().length() > 0 ? I
													.displayText() : I.Name()),
											SCREEN_LEN2)
									+ ": "
									+ CMStrings.padRight(I.phyStats().level()
											+ "", SCREEN_LEN3) + ": " + "^N"
									+ ((I.container() != null) ? I.Name() : "")
									+ "\n\r");
					}
				}
		...
		if (!i.hasNext())
			...
			for (; i.hasNext(); v++) {
				...
				if (!CMath.bset(P.getFlags(), Poll.FLAG_ACTIVE))
					...
					str.append(" (expires: "
							+ CMLib.time().date2String(P.getExpiration()) + ")");
				str.append("\n\r");
			}
	...
	public void listLog(MOB mob, Vector commands) {
		int pageBreak = ((mob.playerStats() != null) ? mob.playerStats()
				.getPageBreak() : 0);
		int lineNum = 0;
		if (commands.size() < 2) {
			Log.LogReader log = Log.instance().getLogReader();
			String line = log.nextLine();
			while ((line != null) && (mob.session() != null)
		...
		for (int i = 1; i < commands.size(); i++) {
			...
			if ((s.equalsIgnoreCase("front") || (s.equalsIgnoreCase("first")) || (s
					.equalsIgnoreCase("head"))) && (i < (commands.size() - 1))) {
				...
				if (CMath.isInteger(s)) {
					...
					|| (s.equalsIgnoreCase("last")) || (s
						.equalsIgnoreCase("tail")))
					&& (i < (commands.size() - 1))) {
				s = (String) commands.elementAt(i + 1);
				if (CMath.isInteger(s)) {
					i++;
	...
	public final static ListCmdEntry[] SECURITY_LISTMAP = {
			/* 00 */new ListCmdEntry("UNLINKEDEXITS", new SecFlag[] {
					SecFlag.CMDEXITS, SecFlag.CMDROOMS, SecFlag.CMDAREAS }),
			/* 01 */new ListCmdEntry("ITEMS",
					new SecFlag[] { SecFlag.CMDITEMS }),
			/* 02 */new ListCmdEntry("ARMOR",
					new SecFlag[] { SecFlag.CMDITEMS }),
			/* 03 */new ListCmdEntry("ENVRESOURCES", new SecFlag[] {
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS }),
			/* 04 */new ListCmdEntry("WEAPONS",
					new SecFlag[] { SecFlag.CMDITEMS }),
			/* 05 */new ListCmdEntry("MOBS", new SecFlag[] { SecFlag.CMDMOBS }),
			/* 06 */new ListCmdEntry("ROOMS", new SecFlag[] { SecFlag.CMDMOBS,
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS,
					SecFlag.CMDEXITS, SecFlag.CMDRACES, SecFlag.CMDCLASSES }),
			/* 07 */new ListCmdEntry("AREA", new SecFlag[] { SecFlag.CMDMOBS,
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS,
					SecFlag.CMDEXITS, SecFlag.CMDRACES, SecFlag.CMDCLASSES }),
			/* 08 */new ListCmdEntry("LOCALES",
					new SecFlag[] { SecFlag.CMDROOMS }),
			/* 09 */new ListCmdEntry("BEHAVIORS", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES }),
			/* 10 */new ListCmdEntry("EXITS",
					new SecFlag[] { SecFlag.CMDEXITS }),
			/* 11 */new ListCmdEntry("RACES", new SecFlag[] { SecFlag.CMDRACES,
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS }),
			/* 12 */new ListCmdEntry("CLASSES", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDCLASSES }),
			/* 13 */new ListCmdEntry("STAFF",
					new SecFlag[] { SecFlag.CMDAREAS }),
			/* 14 */new ListCmdEntry("SPELLS", new SecFlag[] { SecFlag.CMDMOBS,
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS,
					SecFlag.CMDEXITS, SecFlag.CMDRACES, SecFlag.CMDCLASSES,
					SecFlag.CMDABILITIES }),
			/* 15 */new ListCmdEntry("SONGS", new SecFlag[] { SecFlag.CMDMOBS,
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS,
					SecFlag.CMDEXITS, SecFlag.CMDRACES, SecFlag.CMDCLASSES,
					SecFlag.CMDABILITIES }),
			/* 16 */new ListCmdEntry("PRAYERS", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES, SecFlag.CMDABILITIES }),
			/* 17 */new ListCmdEntry("PROPERTIES", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES, SecFlag.CMDABILITIES }),
			/* 18 */new ListCmdEntry("THIEFSKILLS", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES, SecFlag.CMDABILITIES }),
			/* 19 */new ListCmdEntry("COMMON", new SecFlag[] { SecFlag.CMDMOBS,
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS,
					SecFlag.CMDEXITS, SecFlag.CMDRACES, SecFlag.CMDCLASSES,
					SecFlag.CMDABILITIES }),
			/* 20 */new ListCmdEntry("JOURNALS",
					new SecFlag[] { SecFlag.JOURNALS }),
			/* 21 */new ListCmdEntry("SKILLS", new SecFlag[] { SecFlag.CMDMOBS,
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS,
					SecFlag.CMDEXITS, SecFlag.CMDRACES, SecFlag.CMDCLASSES,
					SecFlag.CMDABILITIES }),
			/* 22 */new ListCmdEntry("QUESTS",
					new SecFlag[] { SecFlag.CMDQUESTS }),
			/* 23 */new ListCmdEntry("DISEASES", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES, SecFlag.CMDABILITIES }),
			/* 24 */new ListCmdEntry("POISONS", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES, SecFlag.CMDABILITIES }),
			/* 25 */new ListCmdEntry("TICKS",
					new SecFlag[] { SecFlag.LISTADMIN }),
			/* 26 */new ListCmdEntry("MAGIC",
					new SecFlag[] { SecFlag.CMDITEMS }),
			/* 27 */new ListCmdEntry("TECH", new SecFlag[] { SecFlag.CMDITEMS }),
			/* 28 */new ListCmdEntry("CLANITEMS", new SecFlag[] {
					SecFlag.CMDITEMS, SecFlag.CMDCLANS }),
			/* 29 */new ListCmdEntry("COMMANDJOURNAL", new SecFlag[] {}), // blank,
																			// but
																			// used!
			/* 30 */new ListCmdEntry("REALESTATE", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES }),
			/* 31 */new ListCmdEntry("NOPURGE",
					new SecFlag[] { SecFlag.NOPURGE }),
			/* 32 */new ListCmdEntry("BANNED", new SecFlag[] { SecFlag.BAN }),
			/* 33 */new ListCmdEntry("RACECATS", new SecFlag[] {
					SecFlag.CMDRACES, SecFlag.CMDMOBS, SecFlag.CMDITEMS,
					SecFlag.CMDROOMS, SecFlag.CMDAREAS, SecFlag.CMDEXITS }),
			/* 34 */new ListCmdEntry("LOG", new SecFlag[] { SecFlag.LISTADMIN }),
			/* 35 */new ListCmdEntry("USERS", new SecFlag[] {
					SecFlag.CMDPLAYERS, SecFlag.STAT }),
			/* 36 */new ListCmdEntry("LINKAGES",
					new SecFlag[] { SecFlag.CMDAREAS }),
			/* 37 */new ListCmdEntry("REPORTS",
					new SecFlag[] { SecFlag.LISTADMIN }),
			/* 38 */new ListCmdEntry("THREADS",
					new SecFlag[] { SecFlag.LISTADMIN }),
			/* 39 */new ListCmdEntry("RESOURCES",
					new SecFlag[] { SecFlag.LOADUNLOAD }),
			/* 40 */new ListCmdEntry("ONEWAYDOORS", new SecFlag[] {
					SecFlag.CMDEXITS, SecFlag.CMDROOMS, SecFlag.CMDAREAS }),
			/* 41 */new ListCmdEntry("CHANTS", new SecFlag[] { SecFlag.CMDMOBS,
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS,
					SecFlag.CMDEXITS, SecFlag.CMDRACES, SecFlag.CMDCLASSES,
					SecFlag.CMDABILITIES }),
			/* 42 */new ListCmdEntry("POWERS", new SecFlag[] { SecFlag.CMDMOBS,
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS,
					SecFlag.CMDEXITS, SecFlag.CMDRACES, SecFlag.CMDCLASSES,
					SecFlag.CMDABILITIES }),
			/* 43 */new ListCmdEntry("SUPERPOWERS", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES, SecFlag.CMDABILITIES }),
			/* 44 */new ListCmdEntry("COMPONENTS", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.COMPONENTS }),
			/* 45 */new ListCmdEntry("EXPERTISES", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.EXPERTISES }),
			/* 46 */new ListCmdEntry("FACTIONS", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.CMDFACTIONS }),
			/* 47 */new ListCmdEntry("MATERIALS", new SecFlag[] {
					SecFlag.CMDITEMS, SecFlag.CMDROOMS, SecFlag.CMDAREAS }),
			/* 48 */new ListCmdEntry("OBJCOUNTERS",
					new SecFlag[] { SecFlag.LISTADMIN }),
			/* 49 */new ListCmdEntry("POLLS", new SecFlag[] { SecFlag.POLLS,
					SecFlag.LISTADMIN }),
			/* 50 */new ListCmdEntry("CONTENTS", new SecFlag[] {
					SecFlag.CMDROOMS, SecFlag.CMDITEMS, SecFlag.CMDMOBS,
					SecFlag.CMDAREAS }),
			/* 51 */new ListCmdEntry("EXPIRES", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES }),
			/* 52 */new ListCmdEntry("TITLES", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.TITLES }),
			/* 53 */new ListCmdEntry("AREARESOURCES", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES }),
			/* 54 */new ListCmdEntry("CONQUERED", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES }),
			/* 55 */new ListCmdEntry("HOLIDAYS", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.CMDMOBS, SecFlag.CMDITEMS,
					SecFlag.CMDROOMS, SecFlag.CMDAREAS, SecFlag.CMDEXITS,
					SecFlag.CMDRACES, SecFlag.CMDCLASSES }),
			/* 56 */new ListCmdEntry("RECIPES", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.CMDRECIPES }),
			/* 57 */new ListCmdEntry("HELPFILEREQUESTS",
					new SecFlag[] { SecFlag.LISTADMIN }),
			/* 58 */new ListCmdEntry("SCRIPTS", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES }),
			/* 59 */new ListCmdEntry("ACCOUNTS", new SecFlag[] {
					SecFlag.CMDPLAYERS, SecFlag.STAT }),
			/* 60 */new ListCmdEntry("GOVERNMENTS",
					new SecFlag[] { SecFlag.CMDCLANS }),
			/* 61 */new ListCmdEntry("CLANS",
					new SecFlag[] { SecFlag.CMDCLANS }),
			/* 62 */new ListCmdEntry("DEBUGFLAG",
					new SecFlag[] { SecFlag.LISTADMIN }),
			/* 63 */new ListCmdEntry("DISABLEFLAG",
					new SecFlag[] { SecFlag.LISTADMIN }),
			/* 64 */new ListCmdEntry("ALLQUALIFYS", new SecFlag[] {
					SecFlag.CMDABILITIES, SecFlag.LISTADMIN }),
			/* 65 */new ListCmdEntry("NEWS", new SecFlag[] { SecFlag.LISTADMIN,
					SecFlag.JOURNALS, SecFlag.NEWS }),
			/* 66 */new ListCmdEntry("AREAS", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.CMDAREAS, SecFlag.CMDROOMS }),
			/* 67 */new ListCmdEntry("SESSIONS",
					new SecFlag[] { SecFlag.SESSIONS }),
			/* 68 */new ListCmdEntry("WORLD", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.CMDAREAS, SecFlag.CMDROOMS }),
			/* 69 */new ListCmdEntry("PLANETS", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.CMDAREAS, SecFlag.CMDROOMS }),
			/* 70 */new ListCmdEntry("CURRENTS", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.CMDAREAS, SecFlag.CMDROOMS,
					SecFlag.CMDMOBS }),
			/* 71 */new ListCmdEntry("MANUFACTURERS", new SecFlag[] {
					SecFlag.LISTADMIN, SecFlag.CMDITEMS }),
			/* 72 */new ListCmdEntry("TECHSKILLS", new SecFlag[] {
					SecFlag.CMDMOBS, SecFlag.CMDITEMS, SecFlag.CMDROOMS,
					SecFlag.CMDAREAS, SecFlag.CMDEXITS, SecFlag.CMDRACES,
					SecFlag.CMDCLASSES, SecFlag.CMDABILITIES }),
			/* 73 */new ListCmdEntry("SOFTWARE",
					new SecFlag[] { SecFlag.CMDITEMS }), };
	...
	public void listNews(MOB mob, Vector commands) {
		...
		CMMsg newMsg = CMClass.getMsg(mob, I, null, CMMsg.MSG_READ
				| CMMsg.MASK_ALWAYS, null, CMMsg.MSG_READ | CMMsg.MASK_ALWAYS,
				theRest, CMMsg.MSG_READ | CMMsg.MASK_ALWAYS, null);
	...
	public void listSql(MOB mob, String rest) {
		...
		try {
			List<String[]> rows = CMLib.database().DBRawQuery(
					rest.replace('`', '\''));
	...
	private enum ListAreaStats {
		NAME("Name", 30), AUTHOR("Auth", 15), DESCRIPTION("Desc", 50), ROOMS(
				"Rooms", 6), STATE("State", 10), HIDDEN("Hiddn", 6);
		...
		public Object getFromArea(Area A) {
			...
			case ROOMS:
				return Integer.valueOf(A.getProperRoomnumbers()
						.roomCountAllAreas());
			case STATE:
	...
	public Object getAreaStatFromSomewhere(Area A, String stat) {
		...
		ListAreaStats ls = (ListAreaStats) CMath.s_valueOf(ListAreaStats.class,
				stat);
	...
	public void listManufacturers(MOB mob, Vector commands) {
		...
		List<Manufacturer> l = new XVector<Manufacturer>(CMLib.tech()
				.manufacterers());
		...
		for (Iterator<Manufacturer> i = l.iterator(); i.hasNext();) {
			...
			str.append(
					CMStrings.padRight(
							M.getMinTechLevelDiff() + "-"
									+ M.getMaxTechLevelDiff(), 5)).append(" ");
			str.append(
					CMStrings.padRight(Math.round(M.getEfficiencyPct() * 100.0)
							+ "%", 4)).append(" ");
			str.append(
					CMStrings.padRight(
							Math.round(M.getReliabilityPct() * 100.0) + "%", 4))
					.append(" ");
			if (i.hasNext()) {
				M = i.next();
				str.append("!");
				...
				str.append(
						CMStrings.padRight(
								Math.round(M.getEfficiencyPct() * 100.0) + "%",
								4)).append(" ");
				str.append(CMStrings.padRight(
						Math.round(M.getReliabilityPct() * 100.0) + "%", 4));
			}
	...
	public void listCurrents(MOB mob, Vector commands) {
		...
		for (String key : CMLib.tech().getMakeRegisteredKeys()) {
			str.append("Registered key: "
					+ key
					+ " : "
					+ (CMLib.tech().isCurrentActive(key) ? "Activated"
							: "Suspended") + "\n\r");
	...
	public void listAreas(MOB mob, Vector commands, Filterer<Area> filter) {
		...
		if (commands.size() > 0) {
			...
			while (commands.size() > 0) {
				if (commands.get(0).toString().equalsIgnoreCase("sortby")) {
					...
					addTos = sortBys;
				} else if (commands.get(0).toString().equalsIgnoreCase("cols")
					...
					String stat = commands.get(0).toString().toUpperCase()
							.trim();
					ListAreaStats ls = (ListAreaStats) CMath.s_valueOf(
							ListAreaStats.class, stat);
					Area.Stats as = (Area.Stats) CMath.s_valueOf(
							Area.Stats.class, stat);
					if ((ls == null) && (as == null)) {
						mob.tell("'" + stat
								+ "' is not recognized.  Try one of these: "
								+ CMParms.toStringList(ListAreaStats.values())
								+ ", "
								+ CMParms.toStringList(Area.Stats.values()));
						return;
					}
					addTos.add(stat);
					commands.remove(0);
					...
					mob.tell("'"
							+ commands.get(0).toString()
							+ "' is not recognized.  Try 'columns' or 'sortby' followed by one or more of these: "
							+ CMParms.toStringList(ListAreaStats.values())
							+ ", " + CMParms.toStringList(Area.Stats.values()));
					return;
				}
		...
		if ((colNames != null) && (colNames.size() > 0)) {
			for (String newCol : colNames) {
				ListAreaStats ls = (ListAreaStats) CMath.s_valueOf(
						ListAreaStats.class, newCol);
				Area.Stats as = (Area.Stats) CMath.s_valueOf(Area.Stats.class,
						newCol);
				if (ls != null)
					columns.add(new Triad<String, String, Integer>(
							ls.shortName, ls.name(), ls.len));
				else if (as != null)
					columns.add(new Triad<String, String, Integer>(CMStrings
							.scrunchWord(CMStrings.capitalizeAndLower(newCol),
									6), as.name(), Integer.valueOf(6)));
			}
		} else {
			// AREASTAT_DESCS
			columns.add(new Triad<String, String, Integer>(
					ListAreaStats.NAME.shortName, ListAreaStats.NAME.name(),
					ListAreaStats.NAME.len));
			columns.add(new Triad<String, String, Integer>(
					ListAreaStats.HIDDEN.shortName,
					ListAreaStats.HIDDEN.name(), ListAreaStats.HIDDEN.len));
			columns.add(new Triad<String, String, Integer>(
					ListAreaStats.ROOMS.shortName, ListAreaStats.ROOMS.name(),
					ListAreaStats.ROOMS.len));
			columns.add(new Triad<String, String, Integer>(
					ListAreaStats.STATE.shortName, ListAreaStats.STATE.name(),
					ListAreaStats.STATE.len));
			columns.add(new Triad<String, String, Integer>("Pop",
					Area.Stats.POPULATION.name(), Integer.valueOf(6)));
			columns.add(new Triad<String, String, Integer>("MedLv",
					Area.Stats.MED_LEVEL.name(), Integer.valueOf(6)));
		}
		...
		str.append("\n\r");
		Triad<String, String, Integer> lastColomn = columns
				.get(columns.size() - 1);
		Enumeration<Area> a;
		if (sortBys != null) {
			TreeMap<Comparable, Area> sorted = new TreeMap<Comparable, Area>();
			for (Enumeration<Area> as = CMLib.map().areas(); as
					.hasMoreElements();) {
			...
			for (int si = sortBys.size() - 1; si >= 0; si--) {
				...
				for (Iterator<Area> a2 = sorted.values().iterator(); a2
						.hasNext();) {
					Area A = a2.next();
		...
		for (; a.hasMoreElements();) {
			...
			for (Triad<String, String, Integer> head : columns) {
				...
				if (val == null)
					val = "?";
				if (head == lastColomn)
					str.append(CMStrings.scrunchWord(val.toString(),
							head.third.intValue() - 1));
				else
					str.append(CMStrings.padRight(
							CMStrings.scrunchWord(val.toString(),
									head.third.intValue() - 1),
							head.third.intValue()));
			}
			str.append("\n\r");
	...
	public void listSessions(MOB mob, Vector commands) {
		...
		for (Session S : CMLib.sessions().allIterable()) {
			...
			set[1] = (S.isStopped() ? "^H" : "")
					+ CMStrings.padRight(S.getStatus().toString(), 9)
					+ (S.isStopped() ? "^?" : "") + "| ";
			if (S.mob() != null) {
				set[2] = CMStrings.padRight(((S.mob().session() == S) ? "Yes"
						: "^HNO!^?"), 5)
						+ "| ";
				set[3] = "^!"
						+ CMStrings.padRight("^<LSTUSER^>" + S.mob().Name()
								+ "^</LSTUSER^>", 17) + "^?| ";
			} else {
			...
			set[5] = CMStrings.padRight(
					CMLib.english().returnTime(S.getIdleMillis(), 0) + "", 17);
		...
		if (sort.length() > 0) {
			if ("STATUS".startsWith(sort))
				...
				sortNum = 4;
			else if (("IDLE".startsWith(sort))
					|| ("MILLISECONDS".startsWith(sort)))
				sortNum = 5;
		}
		if (sortNum < 0)
			sorted = broken;
			...
			while (broken.size() > 0) {
				...
				for (int s = 1; s < broken.size(); s++) {
					...
					if (S[sortNum].compareToIgnoreCase(((String[]) broken
							.elementAt(selected))[sortNum]) < 0)
						selected = s;
				}
	...
	public void archonlist(MOB mob, Vector commands) {
		...
		String rest = (commands.size() > 1) ? rest = CMParms.combine(commands,
				1) : "";
		int code;
		...
		if ((code < 0) || (listWord.length() == 0)) {
			...
			if (V.size() == 0)
				...
				mob.tell("You cannot list '" + listWord + "'.  Try "
						+ str.toString() + ".");
			}
			return;
		}
		...
		case 0:
			...
			break;
		case 1:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.basicItems()).toString());
			break;
		case 2:
			s.wraplessPrintln(CMLib.lister().reallyList(mob, CMClass.armor())
					.toString());
			break;
		case 3:
			...
			break;
		case 4:
			s.wraplessPrintln(CMLib.lister().reallyList(mob, CMClass.weapons())
					.toString());
			break;
		case 5:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.mobTypes()).toString());
			break;
		case 6:
			s.wraplessPrintln(roomDetails(mob.session(),
					mob.location().getArea().getMetroMap(), mob.location())
					.toString());
			break;
		case 7:
			s.wraplessPrintln(roomTypes(mob,
					mob.location().getArea().getMetroMap(), mob.location(),
					commands).toString());
			break;
		case 8:
			s.wraplessPrintln(CMLib.lister().reallyList(mob, CMClass.locales())
					.toString());
			break;
		case 9:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.behaviors()).toString());
			break;
		case 10:
			s.wraplessPrintln(CMLib.lister().reallyList(mob, CMClass.exits())
					.toString());
			break;
		case 11:
			...
			break;
		case 12:
			s.wraplessPrintln(listCharClasses(s, CMClass.charClasses(),
					rest.equalsIgnoreCase("SHORT")).toString());
			break;
		case 13:
		...
		case 14:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.abilities(), Ability.ACODE_SPELL)
					.toString());
			break;
		case 15:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.abilities(), Ability.ACODE_SONG)
					.toString());
			break;
		case 16:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.abilities(), Ability.ACODE_PRAYER)
					.toString());
			break;
		case 17:
			s.wraplessPrintln(CMLib
					.lister()
					.reallyList(mob, CMClass.abilities(),
							Ability.ACODE_PROPERTY).toString());
			break;
		case 18:
			s.wraplessPrintln(CMLib
					.lister()
					.reallyList(mob, CMClass.abilities(),
							Ability.ACODE_THIEF_SKILL).toString());
			break;
		case 19:
			s.wraplessPrintln(CMLib
					.lister()
					.reallyList(mob, CMClass.abilities(),
							Ability.ACODE_COMMON_SKILL).toString());
			break;
		case 20:
		...
		case 21:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.abilities(), Ability.ACODE_SKILL)
					.toString());
			break;
		case 22:
		...
		case 23:
			s.wraplessPrintln(CMLib
					.lister()
					.reallyList(mob, CMClass.abilities(), Ability.ACODE_DISEASE)
					.toString());
			break;
		case 24:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.abilities(), Ability.ACODE_POISON)
					.toString());
			break;
		case 25:
			s.println(listTicks(mob.session(), CMParms.combine(commands, 1))
					.toString());
			break;
		case 26:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.miscMagic()).toString());
			break;
		case 27:
			s.wraplessPrintln(CMLib.lister().reallyList(mob, CMClass.tech())
					.toString());
			break;
		case 28:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.clanItems()).toString());
			break;
		case 29:
			s.println(journalList(mob.session(), listWord).toString());
			break;
		case 30:
			s.wraplessPrintln(roomPropertyDetails(mob.session(),
					mob.location().getArea(), rest).toString());
			break;
		case 31: {
			StringBuilder str = new StringBuilder("\n\rProtected players:\n\r");
			List<String> protectedOnes = Resources.getFileLineVector(Resources
					.getFileResource("protectedplayers.ini", false));
		...
		case 33:
			s.wraplessPrintln(listRaceCats(s, CMClass.races(),
					rest.equalsIgnoreCase("SHORT")).toString());
			break;
		case 34:
		...
		case 38:
			s.println(listThreads(mob.session(), mob,
					CMParms.combine(commands, 1).equalsIgnoreCase("SHORT"))
					.toString());
			break;
		case 39:
		...
		case 41:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.abilities(), Ability.ACODE_CHANT)
					.toString());
			break;
		case 42:
		case 43:
			s.wraplessPrintln(CMLib
					.lister()
					.reallyList(mob, CMClass.abilities(),
							Ability.ACODE_SUPERPOWER).toString());
			break;
		case 44:
		...
		case 50:
			...
			break;
		case 51:
			s.wraplessPrintln(roomExpires(mob.session(),
					mob.location().getArea().getProperMap(), mob.location())
					.toString());
			break;
		case 52:
			...
			break;
		case 53:
			s.wraplessPrintln(roomResources(mob.session(),
					mob.location().getArea().getMetroMap(), mob.location())
					.toString());
			break;
		case 54:
			s.wraplessPrintln(areaConquests(mob.session(), CMLib.map().areas())
					.toString());
			break;
		case 55:
			s.wraplessPrintln(CMLib.quests().listHolidays(
					mob.location().getArea(), CMParms.combine(commands, 1)));
			break;
		case 56:
			s.wraplessPrintln(listRecipes(mob, CMParms.combine(commands, 1)));
			break;
		case 57:
			s.wraplessPrint(listHelpFileRequests(mob,
					CMParms.combine(commands, 1)));
			break;
		case 58:
			s.wraplessPrintln(listScripts(mob.session(), mob, commands)
					.toString());
			break;
		case 59:
		...
		case 62:
			s.println("\n\r^xDebug Settings: ^?^.^N\n\r"
					+ CMParms.toStringList(new XVector<CMSecurity.DbgFlag>(
							CMSecurity.getDebugEnum())) + "\n\r");
			break;
		case 63:
			s.println("\n\r^xDisable Settings: ^?^.^N\n\r"
					+ CMParms.toStringList(new XVector<CMSecurity.DisFlag>(
							CMSecurity.getDisablesEnum())) + "\n\r");
			break;
		case 64:
			s.wraplessPrintln(listAllQualifies(mob.session(), commands)
					.toString());
			break;
		case 65:
			...
			break;
		case 66:
			listAreas(mob, commands, WorldMap.mundaneAreaFilter);
			break;
		case 67: {
		...
		case 68:
			...
			break;
		case 69:
			listAreas(mob, commands, WorldMap.planetsAreaFilter);
			break;
		case 70:
		...
		case 72:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.abilities(), Ability.ACODE_TECH)
					.toString());
			break;
		case 73:
			s.wraplessPrintln(CMLib.lister()
					.reallyList(mob, CMClass.tech(new Filterer<Electronics>() {
						@Override
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() == 0) {
			...
			List<Environmental> V2 = CMLib.coffeeShops().getAllShopkeepers(
					mob.location(), mob);
			Environmental shopkeeper = CMLib.english().fetchEnvironmental(V2,
					what, false);
			if ((shopkeeper == null)
					&& (what.equals("shop") || what.equals("the shop")))
		...
		for (int i = 0; i < V.size(); i++) {
			...
			if (forWhat != null)
				str += " for '" + forWhat + "'";
			str += ".";
			CMMsg newMsg = CMClass.getMsg(mob, shopkeeper, null,
					CMMsg.MSG_LIST, str);
