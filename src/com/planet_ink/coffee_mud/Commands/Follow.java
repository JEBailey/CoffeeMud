...
public class Follow extends StdCommand {
	...
	public boolean nofollow(MOB mob, boolean errorsOk, boolean quiet) {
		...
		if (mob.amFollowing() != null) {
			CMMsg msg = CMClass.getMsg(mob, mob.amFollowing(), null,
					CMMsg.MSG_NOFOLLOW, quiet ? null
							: "<S-NAME> stop(s) following <T-NAMESELF>.");
	...
	public boolean processFollow(MOB mob, MOB tofollow, boolean quiet) {
		...
		if (tofollow != null) {
			...
			if (mob.getGroupMembers(new HashSet<MOB>()).contains(tofollow)) {
				if (!quiet)
					mob.tell("You are already a member of " + tofollow.name()
							+ "'s group!");
				return false;
			}
			if (nofollow(mob, false, false)) {
				CMMsg msg = CMClass.getMsg(mob, tofollow, null,
						CMMsg.MSG_FOLLOW, quiet ? null
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((whomToFollow.equalsIgnoreCase("self") || whomToFollow
				.equalsIgnoreCase("me"))
		...
		if ((ultiTarget != null)
				&& (CMath.bset(ultiTarget.getBitmap(), MOB.ATT_NOFOLLOW))) {
