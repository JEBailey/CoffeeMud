...
public class Affect extends StdCommand {
	...
	// private final static Class[][] internalParameters=new
	...
	public String getMOBState(final MOB mob) {
		...
		if ((mob.playerStats() != null)
			...
			if (CMSecurity.isASysOp(mob))
				...
				if (x <= 1)
					msg.append("^!You could use a bath.^?\n\r");
				else if (x <= 3)
					msg.append("^!You could really use a bath.^?\n\r");
				else if (x <= 7)
					msg.append("^!You need to bathe, soon.^?\n\r");
				else if (x < 15)
					msg.append("^!You desperately need to bathe.^?\n\r");
				else
					msg.append("^!Your stench is horrendous! Bathe dammit!^?\n\r");
			}
		}
		...
		// dont do falling -- the flag doubles for drowning/treading water
		// anyway.
		...
		if (mob.riding() != null)
			msg.append("^!You are " + mob.riding().stateString(mob) + " "
					+ mob.riding().name() + ".^?\n\r");
		...
		for (final Enumeration<Ability> a = P.effects(); a.hasMoreElements();) {
			...
			if (disp.length() > 0) {
				if (disp.startsWith("(") && disp.endsWith(")")) {
					...
					if (A.invoker() != null)
						tr = tr
								- (System.currentTimeMillis() - A.invoker()
										.lastTickedDateTime());
					if (tr < Ability.TICKS_ALMOST_FOREVER)
						disp += " ^.^N"
								+ CMLib.time().date2EllapsedTime(tr,
										TimeUnit.SECONDS, true);
				}
				if (xtra)
					disp += ", BY="
							+ ((A.invoker() == null) ? "N/A" : A.invoker()
									.Name());
				String[] disps = { disp };
				if (CMStrings.lengthMinusColors(disp) > (COL_LEN * NUM_COLS)) {
					String s = CMLib.coffeeFilter().fullOutFilter(S, mob, null,
							null, null, disp, true);
				...
				for (int d = 0; d < disps.length; d++) {
					disp = disps[d];
					if (((++colnum) >= NUM_COLS)
							|| (CMStrings.lengthMinusColors(disp) > COL_LEN)) {
						msg.append("\n\r");
						colnum = 0;
					}
					msg.append("^S"
							+ CMStrings.padRightPreserve("^<HELPNAME NAME='"
									+ CMStrings.removeColors(A.name()) + "'^>"
									+ disp + "^</HELPNAME^>", COL_LEN));
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (S != null) {
			if (CMSecurity.isAllowed(mob, mob.location(),
				...
				if (name.length() > 0) {
					Physical P = null;
					if ((name.equalsIgnoreCase("here") || (name
							.equalsIgnoreCase("room"))))
						P = CMLib.map().roomLocation(mob);
					else if ((name.equalsIgnoreCase("area") || (name
							.equalsIgnoreCase("zone"))))
						P = CMLib.map().areaLocation(mob);
					else
						P = mob.location().fetchFromMOBRoomFavorsItems(mob,
								null, name, Wearable.FILTER_ANY);
					if (P == null)
						S.colorOnlyPrint("You don't see " + name
								+ " here.\n\r^N");
					else {
						if (S == mob.session())
							S.colorOnlyPrint(" \n\r^!" + P.name()
	...
	public Object executeInternal(MOB mob, int metaFlags, Object... args)
			throws java.io.IOException {
		// if(!super.checkArguments(internalParameters, args)) return
