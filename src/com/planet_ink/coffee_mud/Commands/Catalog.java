...
public class Catalog extends StdCommand {
	...
	public boolean catalog(Room R, MOB mob, Physical P)
		...
		if (newCat == null)
			newCat = "";
		String msg = "<S-NAME> catalog(s) <T-NAMESELF> into category '"
				+ newCat + "'.";
		...
		if (cataP != null) {
			...
			if ((diffs == null) || (diffs.length() == 0)) {
				if ((data != null) && (!data.category().equals(newCat))) {
							...
							|| (!mob.session()
									.confirm(
											"The object '"
													...
													+ catagory
													+ " , exactly as it is.  Would you like to change it to category '"
													+ newCat + "'(y/N)?", "N"))) {
						return false;
					} else {
					...
					mob.tell("The object '" + cataP.Name()
							+ "' already exists in the catalog" + catagory
							+ " , exactly as it is.");
					return true;
				}
			}
			...
			if ((mob.session() == null)
					|| (!mob.session().confirm(
							"Cataloging that object will change the existing cataloged '"
									+ P.Name() + "'" + catagory
	...
	public Physical findCatalog(int whatKind, String ID, boolean exactOnly) {
		...
		if ((data[0] == null) && ((whatKind == 0) || (whatKind == 1))) {
			...
			for (int x = 0; x < names.length; x++)
				if (CMLib.english().containsString(names[x], ID)) {
					data[0] = CMLib.catalog().getCatalogMob(names[x]);
					data[1] = Integer.valueOf(1);
					break;
				}
			if (data[0] == null) {
		...
		if ((data[0] == null) && ((whatKind == 0) || (whatKind == 2))) {
			...
			for (int x = 0; x < names.length; x++)
				if (CMLib.english().containsString(names[x], ID)) {
					data[0] = CMLib.catalog().getCatalogItem(names[x]);
					data[1] = Integer.valueOf(2);
					break;
				}
			if (data[0] == null) {
	...
	public Enumeration getRoomSet(MOB mob, String which) {
		...
		if (which.equalsIgnoreCase("ROOM"))
			rooms = new XVector(CMLib.map().getExtendedRoomID(mob.location()))
					.elements();
		else if (which.equalsIgnoreCase("AREA"))
			rooms = mob.location().getArea().getProperRoomnumbers()
					.getRoomIDs();
	...
	public int getObjectType(Vector commands) {
		int whatKind = 0;
		if ((commands.size() > 0)
				&& ("MOBS".startsWith(((String) commands.firstElement())
						.toUpperCase().trim()))) {
			commands.removeElementAt(0);
			whatKind = 1;
		}
		if ((commands.size() > 0)
				&& ("ITEMS".startsWith(((String) commands.firstElement())
						.toUpperCase().trim()))) {
			commands.removeElementAt(0);
			whatKind = 2;
		}
		...
		if ((commands != null) && (commands.size() > 1)) {
			...
			if (checkUserRoomSetEntry(commands)) {
				...
				if ((mob.session() != null)
						&& (mob.session()
								.confirm(
										"You are about to auto-catalog (room-reset and save) all "
												+ which
												+ " "
												+ type
												+ ".\n\r"
												+ "This command, if used improperly, may alter "
												+ type
												+ " in this "
												+ which
												+ ".\n\rAre you absolutely sure (y/N)?",
										"N"))
						&& ((which.equalsIgnoreCase("ROOM")) || mob
								.session()
								.confirm(
										"I'm serious now.  You can't abort this, and it WILL modify stuff.\n\r"
												+ "Have you tested this command on small areas and know what you're doing?.\n\rAre you absolutely POSITIVELY sure (y/N)?",
										"N"))) {
					Physical P = null;
					String roomID = null;
					for (; rooms.hasMoreElements();) {
						roomID = (String) rooms.nextElement();
						R = CMLib.coffeeMaker().makeNewRoomContent(
								CMLib.map().getRoom(roomID), false);
						Vector<CatalogLibrary.RoomContent> contents = CMLib
								.catalog().roomContent(R);
						boolean dirty = false;
						for (CatalogLibrary.RoomContent content : contents) {
							...
							} else if ((P instanceof Item) && (whatKind != 1)) {
								if (catalog(R, mob, P)) {
									content.flagDirty();
									dirty = true;
								}
				...
				if (commands.size() > 0) {
					...
					if (ID.length() > 0) {
						if ((!CMParms.contains(CMLib.catalog()
								.getItemCatalogCatagories(), ID))
								&& (!CMParms.contains(CMLib.catalog()
										.getMobCatalogCatagories(), ID))) {
							final Session session = mob.session();
							final String newCat = ID;
							if (newCat.equalsIgnoreCase("GLOBAL")
									|| newCat.equalsIgnoreCase("NONE")
									|| newCat.equalsIgnoreCase("UNCATEGORIZED"))
								mob.tell("That is not a valid new catagory to create.");
							else if (session != null)
								session.prompt(new InputCallback(
										InputCallback.Type.CONFIRM, "N") {
									@Override
									public void showPrompt() {
										session.promptPrint("Create new category '"
												+ newCat + "' (y/N)?");
									}
									...
									public void callBack() {
										if (this.confirmed) {
											...
											mob.tell("Your category is now '"
													+ newCat
													+ "' for new mob/item catalog additions.");
							...
							mob.tell("Your category is now '" + ID
									+ "' for new mob/item catalog additions.");
							mob.tell("To change back to the global category, enter CATALOG CATAGORY NONE");
						}
					} else {
				...
				if ((whatKind == 0) || (whatKind == 1)) {
					...
					for (int i = 0; i < names.length; i++) {
						...
						if ((ID == null)
								|| (ID.length() == 0)
								|| (CMLib.english()
										.containsString(M.Name(), ID))) {
							list.append(CMStrings.padRight(M.Name(), 34))
									.append(" ");
							list.append(CMStrings.padRight(
									Integer.toString(data.numReferences()), 3));
							if (col == 0) {
								col++;
							} else {
								col++;
				...
				if (ID.equalsIgnoreCase("everydamnmob"))
					...
					if (P == null) {
						mob.tell("'" + ID
								+ "' not found in catalog! Try CATALOG LIST");
						return false;
					}
					del = new Physical[] { P };
				}
				for (int d = 0; d < del.length; d++) {
					...
					if (P instanceof MOB) {
						...
						if ((data != null) && (data.numReferences() > 0))
							prefix = "Catalog MOB '" + ((MOB) P).Name()
									+ "' is currently listed as being in use '"
									+ data.numReferences() + " times.  ";
						if ((mob.session() != null)
								&& ((del.length > 10) || (mob.session()
										.confirm(
												prefix
														+ "This will permanently delete mob '"
														+ ((MOB) P).Name()
														+ "' from the catalog.  Are you sure (y/N)?",
												"N")))) {
							CMLib.catalog().delCatalog(P);
							mob.tell("MOB '"
									+ ((MOB) P).Name()
									+ " has been permanently removed from the catalog.");
						}
					} else if (P instanceof Item) {
						String prefix = "";
						if ((data != null) && (data.numReferences() > 0))
							prefix = "Catalog Item '" + ((Item) P).Name()
									+ "' is currently listed as being in use '"
									+ data.numReferences() + " times.  ";
						if ((mob.session() != null)
								&& ((del.length > 10) || (mob.session()
				...
				if (P instanceof MOB) {
					...
					if (mob.session() != null) {
						Double newPct = null;
						while (newPct == null) {
							String newRate = mob.session().prompt(
									"Enter a new Drop Rate or 0% to disable ("
											+ CMath.toPct(data.getRate())
											+ "): ",
									CMath.toPct(data.getRate()));
							if (newRate.trim().length() == 0)
								...
								mob.tell("'"
										+ newRate
										+ "' is not a valid percentage value.  Try something like 10%");
						}
						...
						if (data.getRate() <= 0.0) {
							...
							CMLib.database().DBUpdateItem("CATALOG_ITEMS",
									(Item) P);
							Log.sysOut("Catalog", mob.Name()
									+ " modified catalog item " + P.Name());
							mob.tell("No drop item.");
							return false;
						}
						String choice = mob.session().choose(
								"Is this for L)ive mobs or D)ead ones ("
										+ (data.getWhenLive() ? "L" : "D")
										+ "): ", "LD",
								(data.getWhenLive() ? "L" : "D"));
						data.setWhenLive(choice.equalsIgnoreCase("L"));
						String newMask = "?";
						while (newMask.equalsIgnoreCase("?")) {
							newMask = mob.session().prompt(
									"Enter new MOB selection mask, or NULL ("
											+ data.getMaskStr() + ")\n\r: ",
									data.getMaskStr());
						...
						CMLib.database()
								.DBUpdateItem("CATALOG_ITEMS", (Item) P);
						Log.sysOut("Catalog", mob.Name()
								+ " modified catalog item " + P.Name());
					...
					|| (((String) commands.firstElement())
							.equalsIgnoreCase("DBSCAN"))) {
				boolean db = ((String) commands.firstElement()).toUpperCase()
						.startsWith("DB");
				commands.removeElementAt(0);
				if (checkUserRoomSetEntry(commands)) {
					String which = ((String) commands.firstElement())
							.toLowerCase();
					Enumeration rooms = getRoomSet(mob, which);
					commands.removeElementAt(0);
						...
						for (CatalogLibrary.RoomContent content : contents) {
							...
							if ((P instanceof MOB) && (whatKind != 2)
								...
								if (CMLib.catalog().getCatalogObj(P) != null)
									mob.tell("Check: MOB " + P.Name() + " in "
											+ roomID + " is cataloged.");
								else
									mob.tell("Error: MOB " + P.Name() + " in "
											+ roomID + " is falsely cataloged.");

							if ((P instanceof Item) && (whatKind != 1)
									&& (CMLib.flags().isCataloged(P)))
								if (CMLib.catalog().getCatalogObj(P) != null)
									mob.tell("Check: Item " + P.Name() + " in "
											+ roomID + " is cataloged.");
								else
									mob.tell("Error: Item " + P.Name() + " in "
											+ roomID + " is falsely cataloged.");
						}
						if (db)
							R.destroy();
					...
					.equalsIgnoreCase("OVERLOOK"))
					|| (((String) commands.firstElement())
							.equalsIgnoreCase("DBOVERLOOK"))) {
				boolean db = ((String) commands.firstElement()).toUpperCase()
						.startsWith("DB");
				commands.removeElementAt(0);
				if (checkUserRoomSetEntry(commands)) {
						...
						for (CatalogLibrary.RoomContent content : contents) {
							...
							if ((E instanceof MOB) && (whatKind != 2)
								...
								if (CMLib.catalog().getCatalogMob(E.Name()) != null)
									mob.tell("MOB " + E.Name() + " in "
											+ roomID
											+ " should be tied to the catalog.");
								else
									mob.tell("MOB " + E.Name() + " in "
											+ roomID + " is not cataloged.");

							if ((E instanceof Item) && (whatKind != 1)
									&& (!CMLib.flags().isCataloged(E)))
								if (CMLib.catalog().getCatalogItem(E.Name()) != null)
									mob.tell("Item " + E.Name() + " in "
											+ roomID
											+ " should be tied to the catalog.");
								else
									mob.tell("Item " + E.Name() + " in "
				...
				if (checkUserRoomSetEntry(commands)) {
					...
					if ((mob.session() != null)
							&& (mob.session()
									.confirm(
											"You are about to auto-clean (and auto-save) all "
													+ which
													+ " "
													+ type
													+ ".\n\r"
						...
						for (; rooms.hasMoreElements();) {
							...
							for (CatalogLibrary.RoomContent content : contents) {
								...
								if ((P instanceof MOB) && (whatKind != 2)) {
									if ((CMLib.flags().isCataloged(P))
											&& (CMLib.catalog()
													.getCatalogObj(P) == null)) {
										CMLib.catalog().changeCatalogUsage(P,
												false);
										content.flagDirty();
										dirty = true;
										mob.tell("MOB " + P.Name() + " in "
												+ roomID + " was cleaned.");
									}
								} else if ((P instanceof Item)
										&& (whatKind != 1)) {
									if ((CMLib.flags().isCataloged(P))
										...
										content.flagDirty();
										dirty = true;
										mob.tell("Item " + P.Name() + " in "
												+ roomID + " was cleaned.");
									}
								}
							}
							if (dirty) {
								CMLib.catalog().updateRoomContent(roomID,
										contents);
								CMLib.map().resetRoom(
										CMLib.map().getRoom(roomID), true);
							}
							R.destroy();
						}
					}
				...
				if (P != null) {
					...
					if ((P instanceof DBIdentifiable)
						...
						if (startRoom != null) {
							if (P instanceof MOB)
								CMLib.database().DBUpdateMOB(
										startRoom.roomID(), (MOB) P);
							else
								CMLib.database().DBUpdateItem(
										startRoom.roomID(), (Item) P);
						}
					}
				} else
