...
public class Copy extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (dirCode >= 0)
			...
			if (dirCode >= 0) {
				...
				if (E == null) {
					if (commands.size() > 1) {
						int subDirCode = Directions
								.getGoodDirectionCode((String) commands
										.lastElement());
						if (subDirCode >= 0) {
							commands.removeElementAt(commands.size() - 1);
							name = CMParms.combine(commands, 0);
					...
					if (E == null) {
						mob.tell("Room ID '"
								+ name
								+ "' does not exist.  You can also try exit <dir> and room <dir>.");
						mob.location().showOthers(mob, null,
								CMMsg.MSG_OK_ACTION,
								"<S-NAME> flub(s) a spell..");
						return false;
		...
		if ((E == null) && (srchContainer == null)) {
			try {
				E = CMLib.map().findFirstInhabitant(
						mob.location().getArea().getMetroMap(), mob, name, 50);
				if (E == null)
					E = CMLib.map().findFirstRoomItem(
							mob.location().getArea().getMetroMap(), mob, name,
							true, 50);
				if (E == null)
				...
				if (E == null)
					E = CMLib.map().findFirstInventory(CMLib.map().rooms(),
							mob, name, 50);
				if (E == null)
					E = CMLib.map().findFirstShopStock(CMLib.map().rooms(),
							mob, name, 50);
		...
		for (int i = 0; i < number; i++) {
			if (E instanceof MOB) {
				...
				if (i == 0) {
					if (number > 1)
						room.show(newMOB, null, CMMsg.MSG_OK_ACTION,
								"Suddenly, " + number + " " + newMOB.name()
										+ "s instantiate from the Java Plane.");
					else
						room.show(newMOB, null, CMMsg.MSG_OK_ACTION,
								"Suddenly, " + newMOB.name()
										+ " instantiates from the Java Plane.");
					Log.sysOut("SysopUtils", mob.Name() + " copied " + number
							+ " mob " + newMOB.Name() + ".");
				}
			} else if ((E instanceof Item) && (!(E instanceof ArchonOnly))) {
				if (!CMSecurity.isAllowed(mob, mob.location(),
				...
				if (i == 0) {
					if (number > 1) {
						if (newItem.name().toLowerCase().endsWith("s"))
							room.showHappens(CMMsg.MSG_OK_ACTION, "Suddenly, "
									+ number + " " + newItem.name() + " falls "
									+ end + ".");
						else
							room.showHappens(CMMsg.MSG_OK_ACTION, "Suddenly, "
									+ number + " " + newItem.name()
						...
						room.showHappens(CMMsg.MSG_OK_ACTION, "Suddenly, "
								+ newItem.name() + " fall " + end + ".");
					Log.sysOut("SysopUtils", mob.Name() + " " + number
							+ " copied " + newItem.ID() + " item.");
				}
			} else if ((E instanceof Room) && (dirCode >= 0)) {
				if (!CMSecurity.isAllowed(mob, mob.location(),
						CMSecurity.SecFlag.COPYROOMS)) {
					mob.tell("You are not allowed to copy " + E.name());
					return false;
				}
				if (room.getRoomInDir(dirCode) != null) {
					...
					mob.tell("A room already exists "
							+ (useShipDirs ? Directions
									.getShipInDirectionName(dirCode)
				...
				synchronized (("SYNC" + room.roomID()).intern()) {
					...
					newRoom.setRawExit(Directions.getOpDirectionCode(dirCode),
							(Exit) (room.getRawExit(dirCode).copyOf()));
					newRoom.setRoomID(room.getArea()
							.getNewRoomID(room, dirCode));
					...
					final String inDirName = useShipDirs ? Directions
							.getShipInDirectionName(dirCode) : Directions
							.getInDirectionName(dirCode);
					if (i == 0) {
						if (number > 1)
							room.showHappens(CMMsg.MSG_OK_ACTION, "Suddenly, "
									+ number + " " + newRoom.displayText(mob)
									+ "s fall " + inDirName + ".");
						else
							room.showHappens(CMMsg.MSG_OK_ACTION, "Suddenly, "
									+ newRoom.displayText(mob) + " falls "
									+ inDirName + ".");
						Log.sysOut("SysopUtils", mob.Name() + " copied "
								+ number + " rooms " + newRoom.roomID() + ".");
					} else
						room.showHappens(CMMsg.MSG_OK_ACTION, "Suddenly, "
								+ newRoom.displayText(mob) + " falls "
								+ inDirName + ".");
				...
				if (editRoom.getRawExit(dirCode) == E) {
					...
					if ((editRoom == null)
							|| (editRoom.getRoomInDir(dirCode) == null)) {
				...
				synchronized (("SYNC" + editRoom.roomID()).intern()) {
					...
					final boolean useShipDirs = (editRoom instanceof SpaceShip)
							|| (editRoom.getArea() instanceof SpaceShip);
					final String inDirName = useShipDirs ? Directions
							.getShipInDirectionName(dirCode) : Directions
							.getInDirectionName(dirCode);
					room.showHappens(CMMsg.MSG_OK_ACTION,
							"Suddenly, " + E.name() + " falls " + inDirName
									+ ".");
				}
				...
				for (Enumeration<Room> r = ((Area) E).getCompleteMap(); r
						.hasMoreElements();) {
					...
					if (hashDex > 0)
						R.setRoomID(newArea.Name()
								+ R.roomID().substring(hashDex));
					else
						R.setRoomID(newArea.Name() + R.roomID());
					translationMap.put(oldR, R);
				}
				for (Enumeration<Room> ir = newArea.getCompleteMap(); ir
						.hasMoreElements();) {
				...
				room.show(mob, null, CMMsg.MSG_OK_ACTION,
						"Suddenly, a massive new landscape instantiates from the Java Plane.");
				Log.sysOut("SysopUtils",
						mob.Name() + " copied area " + E.Name());
			} else {
				mob.tell("I can't just make a copy of a '" + E.name()
						+ "'.\n\r");
				room.showOthers(mob, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> flub(s) a spell..");
				break;
			}
		}
	...
	public boolean securityCheck(MOB mob) {
		return CMSecurity.isAllowedContainsAny(mob, mob.location(),
				CMSecurity.SECURITY_COPY_GROUP);
	}
