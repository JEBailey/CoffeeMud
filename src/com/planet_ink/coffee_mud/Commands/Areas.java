	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		for (int i = 1; i < commands.size(); i++) {
			...
			if (s.toUpperCase().startsWith("SORT=NA")) {
				...
				TreeSet<Area> levelSorted = new TreeSet<Area>(
						new Comparator<Area>() {
				...
				TreeSet<Area> levelSorted = new TreeSet<Area>(
						new Comparator<Area>() {
							public int compare(Area arg0, Area arg1) {
								int lvl1 = arg0.getAreaIStats()[Stats.MED_LEVEL
										.ordinal()];
								int lvl2 = arg1.getAreaIStats()[Stats.MED_LEVEL
										.ordinal()];
								if (lvl1 == lvl2)
									return 1;
								return Integer.valueOf(lvl1).compareTo(
										Integer.valueOf(lvl2));
							}
						});
				for (; a.hasMoreElements();)
					levelSorted.add(a.nextElement());
				...
				for (int x = 0; x < Area.Stats.values().length; x++)
					if (s.toUpperCase().endsWith(
							"=" + Area.Stats.values()[x].name()))
						statVal = x;
				if (statVal < 0) {
					mob.tell("There was an error in your SORT= qualifier: '"
							+ s.substring(5) + "' is unknown.");
					return false;
				}
				...
				a = new IteratorEnumeration<Area>(levelSorted.iterator());
				append = " (sorted by "
						+ Area.Stats.values()[statVal].name().toLowerCase()
						+ ")";
				addStat = sortStat;
				commands.remove(i);
				i--;
		...
		StringBuffer msg = new StringBuffer("^HComplete areas list" + append
				+ ":^?^N\n\r");
		if (commands.size() > 1) {
			expression = CMParms.combineWithQuotes(commands, 1);
			msg = new StringBuffer("^HFiltered areas list" + append
					+ ":^?^N\n\r");
		}
		...
		for (; a.hasMoreElements();) {
			...
			if (CMLib.flags().canAccess(mob, A)
				...
				final String levelStr = (addStat >= 0 ? (Integer.toString(A
						.getAreaIStats()[addStat]) + ":") : "");
				String name = levelStr
						+ ((!CMLib.flags().isHidden(A)) ? " " + A.name() : "("
								+ A.name() + ")");
				if (sysop)
				...
				if (expression != null) {
					...
					if (stats != null) {
						...
						for (int i = 0; i < stats.length; i++)
							H.put(Area.Stats.values()[i].name(),
									Integer.toString(stats[i]));
						try {
							if (!CMStrings.parseStringExpression(expression, H,
									false))
								continue;
						} catch (Exception e) {
							if (mob != null)
								mob.tell("There was an error in your AREA qualifier parameters. See help on AREA for more information. The error was: "
										+ e.getMessage());
							return false;
						}
					}
		...
		for (int i = 0; i < areasVec.size(); i++) {
			...
			msg.append(CMStrings.padRight((String) areasVec.elementAt(i),
					colWidth) + "^N");
		}
