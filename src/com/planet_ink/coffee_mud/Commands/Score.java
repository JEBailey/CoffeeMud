...
public class Score extends Affect {
	...
	public StringBuilder getScore(MOB mob, String parm) {
		...
		int classLevel = mob.charStats().getClassLevel(
		...
		if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES))
			...
			if (classLevel >= mob.phyStats().level())
				...
				levelStr = mob.charStats().getCurrentClass()
						.name(mob.charStats().getCurrentClassLevel())
		...
		if (CMProps.getBoolVar(CMProps.Bool.ACCOUNTEXPIRATION)
			...
			msg.append("Your account is Registered and Active until: "
					+ CMLib.time().date2String(
							mob.playerStats().getAccountExpiration()) + "!\n\r");
		...
		if (mob.baseCharStats().getStat(CharStats.STAT_AGE) > 0)
			msg.append("^!" + mob.baseCharStats().getStat(CharStats.STAT_AGE)
					+ "^? year old ");
		...
		if (mob.clans().iterator().hasNext()) {
			msg.append("You are ");
			for (Iterator<Pair<Clan, Integer>> c = mob.clans().iterator(); c
					.hasNext();) {
		...
		msg.append("You have ^H" + mob.curState().getHitPoints() + "/"
				+ mob.maxState().getHitPoints()
				+ "^? ^<HELP^>hit points^</HELP^>, ^H");
		msg.append(mob.curState().getMana() + "/" + mob.maxState().getMana()
				+ "^? ^<HELP^>mana^</HELP^>, and ^H");
		msg.append(mob.curState().getMovement() + "/"
				+ mob.maxState().getMovement()
				+ "^? ^<HELP^>movement^</HELP^>.\n\r");
		if (mob.phyStats().height() < 0)
			msg.append("You are incorporeal, but still weigh ^!"
					+ mob.baseWeight() + "^? pounds.\n\r");
		else
			msg.append("You are ^!" + mob.phyStats().height()
					+ "^? inches tall and weigh ^!" + mob.baseWeight()
					+ "^? pounds.\n\r");
		if (CMSecurity.isAllowed(mob, mob.location(),
				CMSecurity.SecFlag.CARRYALL))
			msg.append("You are carrying ^!" + mob.numItems()
					+ "^? items weighing ^!" + mob.phyStats().weight()
					+ "^? pounds.\n\r");
		else
			msg.append("You are carrying ^!" + mob.numItems() + "^?/^!"
					+ mob.maxItems() + "^? items weighing ^!"
					+ mob.phyStats().weight() + "^?/^!" + mob.maxCarry()
					+ "^? pounds.\n\r");
		msg.append("You have ^!" + mob.getPractices()
				+ "^? ^<HELP^>practices^</HELP^>, ^!" + mob.getTrains()
				+ "^? ^<HELP^>training sessions^</HELP^>, and ^!"
				+ mob.getQuestPoint() + "^? ^<HELP^>quest points^</HELP^>.\n\r");
		if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.EXPERIENCE))
			...
			if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.LEVELS))
					&& (!mob.charStats().getCurrentClass().leveless())
					&& (!mob.charStats().getMyRace().leveless())) {
				if (((CMProps.getIntVar(CMProps.Int.LASTPLAYERLEVEL) > 0) && (mob
						.basePhyStats().level() > CMProps
						.getIntVar(CMProps.Int.LASTPLAYERLEVEL)))
						|| (mob.getExpNeededLevel() == Integer.MAX_VALUE)
						|| (mob.charStats().isLevelCapped(mob.charStats()
								.getCurrentClass())))
					msg.append("You have scored ^!" + mob.getExperience()
							+ "^? ^<HELP^>experience points^</HELP^>, ^!"
							+ (mob.getExpNeededDelevel())
							+ "^? over your last level.\n\r");
				else
					msg.append("You have scored ^!"
							+ mob.getExperience()
							+ "^? ^<HELP^>experience points^</HELP^>, and need ^!"
							+ (mob.getExpNeededLevel()) + "^? to advance.\n\r");
			} else
				msg.append("You have scored ^!" + mob.getExperience()
						+ "^? ^<HELP^>experience points^</HELP^>.\n\r");
		}
		msg.append("You have been online for ^!"
				+ Math.round(CMath.div(mob.getAgeMinutes(), 60.0))
				+ "^? hours.\n\r");
		for (Enumeration e = mob.fetchFactions(); e.hasMoreElements();) {
			...
			if (F != null) {
				...
				Faction.FRange FR = CMLib.factions().getRange(factionID,
						factionAmt);
				if ((FR != null) && (F.showInScore()))
					msg.append("Your "
							+ CMStrings.padRight("^<HELP^>" + F.name()
									+ "^</HELP^> is", 18) + ": ^H" + FR.name()
							+ " ^.(" + factionAmt + ").\n\r");
			}
		}
		msg.append("Your ^<HELP^>armored defence^</HELP^> is: ^H"
				+ CMLib.combat().armorStr(mob) + "^..\n\r");
		msg.append("Your ^<HELP^>combat prowess^</HELP^> is : ^H"
				+ CMLib.combat().fightingProwessStr(mob) + "^..\n\r");
		// if(CMLib.flags().canSeeHidden(mob))
		// msg.append("Your ^<HELP^>observation score^</HELP^> : ^H"+CMLib.flags().getDetectScore(mob)+"^?.\n\r");
		msg.append("Wimpy is set to ^!" + mob.getWimpHitPoint()
				+ "^? hit points.\n\r");
