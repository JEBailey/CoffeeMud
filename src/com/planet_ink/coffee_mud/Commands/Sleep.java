...
public class Sleep extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (R == null)
			return false;
		if (commands.size() <= 1) {
			CMMsg msg = CMClass.getMsg(mob, null, null, CMMsg.MSG_SLEEP,
		...
		if (E instanceof Rideable)
			mountStr = "<S-NAME> "
					+ ((Rideable) E).mountString(CMMsg.TYP_SLEEP, mob)
					+ " <T-NAME>.";
		else
		...
		if (!CMLib.flags().canBeSeenBy(E, mob))
			...
			sourceMountStr = CMStrings.replaceAll(mountStr, "<T-NAME>",
					E.name());
			sourceMountStr = CMStrings.replaceAll(sourceMountStr,
					"<T-NAMESELF>", E.name());
		}
		CMMsg msg = CMClass.getMsg(mob, E, null, CMMsg.MSG_SLEEP,
				sourceMountStr, mountStr, mountStr);
		if (R.okMessage(mob, msg))
			R.send(mob, msg);
		return false;
	}
