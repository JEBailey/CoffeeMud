...
public class Qualify extends Skills {
	...
	public StringBuffer getQualifiedAbilities(MOB viewerM, MOB ableM,
			int ofType, int ofDomain, String prefix, boolean shortOnly) {
	...
	public StringBuffer getQualifiedAbilities(MOB viewerM, MOB ableM,
			HashSet<Integer> ofTypes, int mask, String prefix, boolean shortOnly) {
		int highestLevel = 0;
		StringBuffer msg = new StringBuffer("");
		boolean checkUnMet = ableM.charStats().getCurrentClass()
				.showThinQualifyList();
		for (Enumeration<Ability> a = CMClass.abilities(); a.hasMoreElements();) {
			...
			if ((CMLib.ableMapper().qualifiesByLevel(ableM, A))
					...
					&& (level > highestLevel)
					&& (level < (CMLib.ableMapper().qualifyingClassLevel(ableM,
							A) + 1))
					&& (ofTypes.contains(Integer.valueOf(A.classificationCode()
							& mask)))
					&& (CMLib.ableMapper().getCommonSkillRemainder(ableM, A).specificSkillLimit > 0)
					&& (ableM.fetchAbility(A.ID()) == null)
					&& (!checkUnMet || CMLib.ableMapper()
							.getUnmetPreRequisites(ableM, A).size() == 0))
				highestLevel = level;
		}
		int col = 0;
		...
		for (int l = 0; l <= highestLevel; l++) {
			...
			for (Enumeration<Ability> a = CMClass.abilities(); a
				...
				if ((CMLib.ableMapper().qualifiesByLevel(ableM, A))
						...
						&& (!CMLib.ableMapper().getSecretSkill(ableM, A.ID()))
						&& (ableM.fetchAbility(A.ID()) == null)
						&& (ofTypes.contains(Integer.valueOf(A
								.classificationCode() & mask)))
						&& (CMLib.ableMapper()
								.getCommonSkillRemainder(ableM, A).specificSkillLimit > 0)
						&& (!checkUnMet || CMLib.ableMapper()
								.getUnmetPreRequisites(ableM, A).size() == 0)) {
					if ((++col) > 2) {
						thisLine.append("\n\r");
					...
					thisLine.append("^N[^H"
							+ CMStrings.padRight("" + l, COL_LEN1)
							+ "^?] "
							+ CMStrings.padRight("^<HELP^>" + A.name()
									+ "^</HELP^>", COL_LEN2)
							+ " "
							+ CMStrings.padRight(A.requirements(viewerM),
									(col == 2) ? COL_LEN3 : COL_LEN4));
				}
			}
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (showAll || ("COMMON SKILLS").startsWith(qual))
			msg.append(getQualifiedAbilities(mob, mob,
					Ability.ACODE_COMMON_SKILL, -1, "\n\r^HCommon Skills:^? ",
					shortOnly));
		if (showAll || ("THIEVES SKILLS".startsWith(qual))
				|| "THIEF SKILLS".startsWith(qual))
			msg.append(getQualifiedAbilities(mob, mob,
					Ability.ACODE_THIEF_SKILL, -1, "\n\r^HThief Skills:^? ",
					shortOnly));
		if (showAll || "SPELLS".startsWith(qual)
				|| "MAGE SPELLS".startsWith(qual))
			msg.append(getQualifiedAbilities(mob, mob, Ability.ACODE_SPELL, -1,
					"\n\r^HSpells:^? ", shortOnly));
		if (showAll || "PRAYERS".startsWith(qual)
				|| "CLERICAL PRAYERS".startsWith(qual))
			msg.append(getQualifiedAbilities(mob, mob, Ability.ACODE_PRAYER,
					-1, "\n\r^HPrayers:^? ", shortOnly));
		if (showAll || "POWERS".startsWith(qual)
				|| "SUPER POWERS".startsWith(qual))
			msg.append(getQualifiedAbilities(mob, mob,
					Ability.ACODE_SUPERPOWER, -1, "\n\r^HSuper Powers:^? ",
					shortOnly));
		if (showAll || "TECHS".startsWith(qual)
				|| "TECH SKILLS".startsWith(qual))
			msg.append(getQualifiedAbilities(mob, mob, Ability.ACODE_TECH, -1,
					"\n\r^HTech Skills:^? ", shortOnly));
		if (showAll || "CHANTS".startsWith(qual)
				|| "DRUID CHANTS".startsWith(qual))
			msg.append(getQualifiedAbilities(mob, mob, Ability.ACODE_CHANT, -1,
					"\n\r^HDruidic Chants:^? ", shortOnly));
		if (showAll || "SONGS".startsWith(qual)
				|| "BARD SONGS".startsWith(qual))
			msg.append(getQualifiedAbilities(mob, mob, Ability.ACODE_SONG, -1,
					"\n\r^HSongs:^? ", shortOnly));
		if (showAll || "LANGUAGES".startsWith(qual) || "LANGS".startsWith(qual))
			msg.append(getQualifiedAbilities(mob, mob, Ability.ACODE_LANGUAGE,
		...
		if (!showAll) {
			for (int i = 1; i < Ability.DOMAIN_DESCS.length; i++)
				if (Ability.DOMAIN_DESCS[i].startsWith(qual.toUpperCase())) {
					domain = i << 5;
					break;
				} else if ((Ability.DOMAIN_DESCS[i].indexOf('/') >= 0)
						&& (Ability.DOMAIN_DESCS[i]
								.substring(Ability.DOMAIN_DESCS[i].indexOf('/') + 1)
								.startsWith(qual.toUpperCase()))) {
					domain = i << 5;
					break;
				}
			if (domain > 0) {
				domainName = CMStrings
						.capitalizeAndLower(Ability.DOMAIN_DESCS[domain >> 5]);
				msg.append(getQualifiedAbilities(mob, mob, Ability.ACODE_SPELL,
		...
		if ((mob != null)
				...
				&& (!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES) && (!mob
						.baseCharStats().getMyRace().classless()))) {
			int col = 0;
			...
			for (Enumeration c = CMClass.charClasses(); c.hasMoreElements();) {
				...
				if ((mob.charStats().getCurrentClass() != C)
					...
					thisLine.append("^N[^H"
							+ CMStrings.padRight("" + 1, COL_LEN1)
							+ "^?] "
							+ CMStrings.padRight("^<HELP^>" + C.name()
									+ "^</HELP^>", COL_LEN2)
							+ " "
							+ CMStrings.padRight("1 train",
									(col == 2) ? COL_LEN3 : COL_LEN4));
				}
				if (thisLine.length() > 0) {
					if (msg2.length() == 0)
		...
		if ((mob != null)
				&& (showAll || (qual.equalsIgnoreCase("EXPS")) || ("EXPERTISES"
						.startsWith(qual)))) {
				...
				if (showAll) {
					...
					for (int e = 0; e < V.size(); e++) {
						...
						if (def.name.length() >= COL_LEN) {
							...
							msg.append(CMStrings.padRightPreserve("^<HELP^>"
									+ def.name + "^</HELP^>", COL_LEN));
							int spaces = (COL_LEN * 2) - def.name.length();
							for (int i = 0; i < spaces; i++)
								msg.append(" ");
							col++;
						} else
							msg.append(CMStrings.padRight("^<HELP^>" + def.name
									+ "^</HELP^>", COL_LEN));
						if ((++col) >= 3) {
							msg.append("\n\r");
							col = 0;
					...
					for (int v = 0; v < V.size(); v++) {
						def = V.get(v);
						req = CMLib.masking().maskDesc(def.finalRequirements(),
								true);
						prefix = "^<HELP^>" + def.name + "^</HELP^>";
						if (req.length() <= 46)
							msg2.append(CMStrings.padRight(prefix, COL_LEN)
									+ req + "\n\r");
						else
							while (req.length() > 0) {
								int x = req.indexOf(".  ");
								if (x < 0) {
									msg2.append(CMStrings.padRight(prefix,
											COL_LEN) + req + "\n\r");
									req = "";
									break;
								}
		...
		if (mob != null) {
			if (msg.length() == 0) {
				if (qual.length() > 0)
					mob.tell("You don't appear to qualify for any '"
							+ qual
							+ "'. Parameters to the QUALIFY command include SKILLS, THIEF, COMMON, SPELLS, PRAYERS, CHANTS, SONGS, EXPERTISES, or LANGUAGES.");
				else
				...
				AbilityMapper.AbilityLimits limits = CMLib.ableMapper()
						.getCommonSkillRemainders(mob);
				if (limits.commonSkills < Integer.MAX_VALUE / 2)
					msg.append("\n\r^HYou may learn ^w" + limits.commonSkills
							+ "^H more common skills.^N");
				if (limits.craftingSkills < Integer.MAX_VALUE / 2)
					msg.append("\n\r^HYou may learn ^w" + limits.craftingSkills
							+ "^H more crafting skills.^N");
				if (limits.nonCraftingSkills < Integer.MAX_VALUE / 2)
					msg.append("\n\r^HYou may learn ^w"
							+ limits.nonCraftingSkills
							+ "^H more non-crafting common skills.^N");
				mob.session().wraplessPrintln(
						"^!You now qualify for the following unknown abilities:^?"
								+ msg.toString());
