...
public class Go extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		boolean inAShip = (R instanceof SpaceShip)
				|| (R.getArea() instanceof SpaceShip);
		final String validDirs = inAShip ? Directions.SHIP_NAMES_LIST()
				: Directions.NAMES_LIST();
		...
		if (whereStr.equalsIgnoreCase("OUT")) {
			...
			for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
				if ((R.getExitInDir(d) != null)
						...
						&& (!CMath.bset(R.getRoomInDir(d).domainType(),
								Room.INDOORS))) {
					if (direction >= 0) {
		...
		if (direction < 0) {
			if (mob.isMonster())
				...
				direction = (inAShip) ? Directions
						.getGoodShipDirectionCode(whereStr) : Directions
						.getGoodCompassDirectionCode(whereStr);
		}
		if (direction < 0) {
		...
		if (direction >= 0)
			...
			for (int v = 1; v < commands.size(); v++) {
				...
				if (mob.isMonster())
					...
					direction = (inAShip) ? Directions
							.getGoodShipDirectionCode(s) : Directions
							.getGoodCompassDirectionCode(s);
				if (direction >= 0) {
					doneAnything = true;
					for (int i = 0; i < num; i++) {
						if (mob.isMonster()) {
							if (!CMLib.tracking().walk(mob, direction, false,
									false, false, false))
								return false;
						} else {
							Vector V = new Vector();
							V.addElement(doing);
							V.addElement(inAShip ? Directions
									.getShipDirectionName(direction)
			...
			if (!doneAnything)
				mob.tell(CMStrings.capitalizeAndLower(doing)
	...
	public double actionsCost(final MOB mob, final List<String> cmds) {
		double cost = CMath.div(CMProps.getIntVar(CMProps.Int.DEFCMDTIME),
				100.0);
