...
public class Modify extends StdCommand {
	...
	public void items(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
			...
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
			...
			if (rest.equalsIgnoreCase("room"))
				...
				if (M == null) {
					...
					if (I instanceof Container)
						...
						mob.location().showOthers(mob, null,
								CMMsg.MSG_OK_ACTION,
								"<S-NAME> flub(s) a spell..");
						return;
					}
		...
		if ((srchMob != null) && (srchRoom != null))
			modItem = (Item) srchRoom.fetchFromMOBRoomFavorsItems(srchMob,
					srchContainer, itemID, Wearable.FILTER_ANY);
		...
		mob.location().showOthers(mob, modItem, CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around <T-NAMESELF>.");
		...
		if (command.equals("LEVEL")) {
			...
			if (newLevel >= 0) {
				...
				mob.location().show(
						...
						CMMsg.MSG_OK_ACTION,
						modItem.name()
								+ " shake(s) under the transforming power.");
			}
			...
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					modItem.name() + " shake(s) under the transforming power.");
			...
			set.addAll(CMParms.parseCommas(
					"LEVEL,ABILITY,HEIGHT,REJUV,USES,MISC", true));
			set.addAll(CMLib.coffeeMaker().getAllGenStats(modItem));
			mob.tell("...but failed to specify an aspect.  Try one of: "
					+ CMParms.toStringList(set));
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
		}
	...
	protected void flunkRoomCmd(MOB mob) {
		...
		mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
				"<S-NAME> flub(s) a powerful spell.");
	}
	...
	public void rooms(MOB mob, Vector commands) throws IOException {
		...
		mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around the room.");
		if (commands.size() == 2) {
			...
			if ((!oldRoom.sameAs(newRoom)) && (!newRoom.amDestroyed())) {
				...
				newRoom.showHappens(CMMsg.MSG_OK_ACTION,
						"There is something different about this place...\n\r");
				Log.sysOut("Rooms",
						mob.Name() + " modified room " + newRoom.roomID() + ".");
			}
			oldRoom.destroy();
		...
		if (command.equalsIgnoreCase("AREA")) {
				...
				if (!mob.isMonster()) {
					if (mob.session().confirm(
							"\n\rThis command will create a BRAND NEW AREA\n\r with Area code '"
									+ restStr + "'.  Are you SURE (y/N)?", "N")) {
						String areaType = "";
						int tries = 0;
						while ((areaType.length() == 0) && ((++tries) < 10)) {
							areaType = mob
									.session()
									.prompt("Enter an area type to create (default=StdArea): ",
											"StdArea");
							if (CMClass.getAreaType(areaType) == null) {
								mob.session().println(
										"Invalid area type! Valid ones are:");
								mob.session().println(
										CMLib.lister()
												.reallyList(mob,
														CMClass.areaTypes())
			...
			if (reid) {
				...
				synchronized (("SYNC" + R.roomID()).intern()) {
					...
					if (A != null) {
						if (reference != null)
							checkID = A.getNewRoomID(reference, CMLib.map()
									.getRoomDir(reference, R));
						else
			...
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"There is something different about this place...\n\r");
		} else if ((command.equalsIgnoreCase("XGRID"))
			...
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"There is something different about this place...\n\r");
		} else if ((command.equalsIgnoreCase("YGRID"))
				&& (mob.location() instanceof GridLocale)) {
			if (commands.size() < 4) {
				flunkRoomCmd(mob);
				return;
			...
			CMLib.database().DBUpdateRoom(mob.location());
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"The very nature of reality changes.\n\r");
			...
			set.addAll(CMParms
					.parseCommas(
							"NAME,AREA,DESCRIPTION,AFFECTS,BEHAVIORS,CLASS,XGRID,YGRID",
							true));
		...
		Log.sysOut("Rooms", mob.Name() + " modified room "
				+ mob.location().roomID() + ".");
	}

	public void accounts(MOB mob, Vector commands) throws IOException {
		mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around the heavens.");
		...
		if (commands.size() == 2) {
			...
			String accountName = CMStrings.capitalizeAndLower(CMParms.combine(
					commands, 2));
		...
		Log.sysOut("Modify",
				mob.Name() + " modified account " + theAccount.accountName()
						+ ".");
		if (!oldName.equals(theAccount.accountName())) {
			Vector<MOB> V = new Vector<MOB>();
			for (Enumeration<String> es = theAccount.getPlayers(); es
					.hasMoreElements();) {
			...
			PlayerAccount acc = (PlayerAccount) CMClass
	...
	public void areas(MOB mob, Vector commands) throws IOException {
		...
		mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around wildly.");
		Resources.removeResource("HELP_" + myArea.Name().toUpperCase());
		if (commands.size() == 2)
			CMLib.genEd().modifyGenArea(mob, myArea);
		else if ((commands.size() == 3)
				&& (CMLib.map().getArea((String) commands.elementAt(2)) != null)) {
			...
			helpSet.addAll(CMParms
					.parseCommas(
							"NAME,DESCRIPTION,CLIMATE,FILE,AFFECTS,BEHAVIORS,ADDSUB,DELSUB,XGRID,YGRID,PASSIVE,ACTIVE,FROZEN,STOPPED",
							true));
			...
			if (command.equalsIgnoreCase("NAME")) {
				...
				for (int i = 0; i < restStr.length(); i++)
					...
					case 'N':
						...
						break;
					default:
						mob.tell("Invalid CLIMATE code: '"
								+ restStr.charAt(i)
								+ "'.  Valid codes include: R)AINY, H)OT, C)OLD, D)RY, W)INDY, N)ORMAL.\n\r");
						mob.location().showOthers(mob, null,
								CMMsg.MSG_OK_ACTION,
								"<S-NAME> flub(s) a powerful spell.");
						return;
				...
				if ((commands.size() < 4)
						|| (!CMLib.players().playerExists(restStr))) {
				...
				if ((commands.size() < 4) || (!myArea.amISubOp(restStr))) {
					mob.tell("Unknown or invalid staff name given.  Valid names are: "
							+ myArea.getSubOpList() + ".\n\r");
					mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> flub(s) a powerful spell.");
				}
		...
		if ((!myArea.Name().equals(oldName)) && (!mob.isMonster())) {
			if (mob.session()
					.confirm(
							"Is changing the name of this area really necessary (y/N)?",
							"N")) {
				for (Enumeration r = myArea.getCompleteMap(); r
						.hasMoreElements();) {
						...
						if ((R.roomID().startsWith(oldName + "#"))
								&& (CMLib.map().getRoom(
										myArea.Name()
												+ "#"
												+ R.roomID().substring(
														oldName.length() + 1)) == null)) {
							R = CMLib.map().getRoom(R);
							String oldID = R.roomID();
							R.setRoomID(myArea.Name()
									+ "#"
									+ R.roomID()
	...
	public void quests(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3)
			...
			if (commands.size() > 3) {
				cmdDex = CMParms.indexOf(CMDS,
						((String) commands.lastElement()).toUpperCase());
				if (cmdDex >= 0)
			...
			if (Q == null) {
				...
				mob.location().showOthers(
						...
						null,
						CMMsg.MSG_OK_ACTION,
						"<S-NAME> wave(s) <S-HIS-HER> hands around " + Q.name()
								+ ".");
				int showFlag = -1;
				...
				while (!ok) {
					...
					if ((doCmd < 0)
							&& (CMLib.genEd().promptToggle(mob, ++showNumber,
									showFlag, "Started: " + Q.running())))
						doCmd = Q.running() ? 1 : 0;
					if ((doCmd < 0)
							&& (CMLib.genEd().promptToggle(mob, ++showNumber,
									showFlag, "Enabled: " + (!Q.suspended()))))
						doCmd = Q.suspended() ? 2 : 3;
					if (doCmd < 0) {
						String oldScript = Q.script();
						newScript = CMLib.genEd().prompt(
								mob,
								oldScript,
								++showNumber,
								showFlag,
								...
								false,
								false,
								CMLib.help().getHelpText("QUESTS", mob, true)
										.toString(), null, null);
						if (!newScript.equals(oldScript)) {
							...
							if (Q.name().length() == 0) {
								...
								for (int q = 0; q < CMLib.quests().numQuests(); q++) {
									...
									if (Q1.name().equalsIgnoreCase(Q.name())
											&& (Q1 != Q)) {
					...
					case 0: {
						if (Q.running())
							...
							Q.startQuest();
							if ((!Q.running())
									&& (Q.getSpawn() != Quest.SPAWN_ANY))
								mob.tell("Quest '"
										+ Q.name()
										+ "' NOT started -- check your mud.log for errors.");
							else
					...
					case 1: {
						if (!Q.running())
							...
							if (!Q.running())
								...
								mob.tell("Quest '"
										+ Q.name()
										+ "' NOT stopped -- check your mud.log for errors.");
						}
						break;
					}
					case 2: {
	...
	public void updateChangedExit(MOB mob, Room baseRoom, Exit thisExit,
			Exit prevExit) {
		...
		if (!prevExit.sameAs(thisExit))
			Log.sysOut("CreateEdit",
					mob.Name() + " modified exit " + thisExit.ID() + ".");
		prevExit.destroy();
		mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
				thisExit.name() + " shake(s) under the transforming power.");
		baseRoom.getArea().fillInAreaRoom(baseRoom);
	}

	public void exits(MOB mob, Vector commands) throws IOException {
		if (mob.location().roomID().equals("")) {
			mob.tell("This command is invalid from within a GridLocaleChild room.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a powerful spell.");
		...
		int direction = Directions.getGoodDirectionCode(((String) commands
				.elementAt(2)));
		if (direction < 0) {
			mob.tell("You have failed to specify a direction.  Try "
					+ Directions.LETTERS() + ".\n\r");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		...
		if (thisExit == null) {
			mob.tell("You have failed to specify a valid exit '"
					+ ((String) commands.elementAt(2)) + "'.\n\r");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
		...
		final String inDirName = useShipDirs ? Directions
				.getShipInDirectionName(direction) : Directions
				.getInDirectionName(direction);
		mob.location().showOthers(
				mob,
				null,
				CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around to the " + inDirName
						+ ".");
	...
	public boolean races(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		if (!(R.isGeneric())) {
			mob.tell("'"
					+ R.ID()
					+ "' is not generic, and may not be modified as it is.  Use CREATE RACE "
					+ R.ID() + " to convert it to a generic race.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return false;
		}
		mob.location().showOthers(
				...
				null,
				CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around all " + R.name()
						+ "s.");
		CMLib.genEd().modifyGenRace(mob, R);
		CMLib.database().DBDeleteRace(R.ID());
		CMLib.database().DBCreateRace(R.ID(), R.racialParms());
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				R.name() + "'s everywhere shake under the transforming power!");
		return true;
	}

	public void allQualify(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 4) {
		...
		if ((!eachOrAll.equalsIgnoreCase("each"))
				&& (!eachOrAll.equalsIgnoreCase("all"))) {
			mob.tell("You have failed to specify the proper fields.\n\rThe format is MODIFY ALLQUALIFY EACH/ALL [SKILL ID]\n\r");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		...
		if (A == null) {
			mob.tell("Ability with the ID '" + classD
					+ "' does not exist! Try LIST ABILITIES.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
		...
		if (!subMap.containsKey(classD.toUpperCase().trim())) {
			mob.tell("All-Qualify entry (" + eachOrAll + ") ID '" + A.ID()
					+ "' does not exist!  Try CREATE, or LIST ALLQUALIFYS.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
		AbilityMapper.AbilityMapping mapped = CMLib.genEd()
				.modifyAllQualifyEntry(mob, eachOrAll.toUpperCase().trim(), A);
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The skill of the world just changed!");
	}

	public boolean classes(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		if (!(C.isGeneric())) {
			mob.tell("'"
					+ C.ID()
					+ "' is not generic, and may not be modified as it is.  Use CREATE CLASS "
					+ C.ID() + " to convert it to a generic character class.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return false;
		}
		mob.location().showOthers(
				...
				null,
				CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around all " + C.name()
						+ "s.");
		CMLib.genEd().modifyGenClass(mob, C);
		CMLib.database().DBDeleteClass(C.ID());
		CMLib.database().DBCreateClass(C.ID(), C.classParms());
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				C.name() + "'s everywhere shake under the transforming power!");
		return true;
	}

	public boolean abilities(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		if (!(A.isGeneric())) {
			mob.tell("'" + A.ID()
					+ "' is not generic, and may not be modified.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return false;
		}
		...
		if (A instanceof ItemCraftor) {
			mob.tell("'" + A.ID()
					+ "' is a crafting skill.  Try MODIFY CRAFTSKILL.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return false;
		}
		mob.location().showOthers(
				...
				null,
				CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around all " + A.name()
						+ "s.");
		CMLib.genEd().modifyGenAbility(mob, A);
		CMLib.database().DBDeleteAbility(A.ID());
		CMLib.database().DBCreateAbility(A.ID(), "GenAbility",
				A.getStat("ALLXML"));
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				A.name() + "'s everywhere shake under the transforming power!");
		return true;
	}

	public boolean languages(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		CMLib.database().DBDeleteAbility(A.ID());
		CMLib.database().DBCreateAbility(A.ID(), "GenLanguage",
				A.getStat("ALLXML"));
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				A.name() + "'s everywhere shake under the transforming power!");
		return true;
	...
	public boolean craftSkills(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		if (A instanceof Language) {
			mob.tell("'" + A.ID()
					+ "' is a crafting skill.  Try MODIFY LANGUAGE.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return false;
		}
		if (!(A instanceof ItemCraftor)) {
			mob.tell("'" + A.ID()
					+ "' is not a crafting skill.  Try MODIFY ABILITY.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return false;
		}
		...
		CMLib.database().DBDeleteAbility(A.ID());
		CMLib.database().DBCreateAbility(A.ID(), "GenCraftSkill",
				A.getStat("ALLXML"));
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				A.name() + "'s everywhere shake under the transforming power!");
		return true;
	...
	public void components(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		if (A == null) {
			mob.tell("'" + skillID
					+ "' is not a proper skill/spell ID.  Try LIST ABILITIES.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
		...
		if (CMLib.ableMapper().getAbilityComponentMap()
				.get(A.ID().toUpperCase()) == null) {
			mob.tell("A component definition for '" + A.ID()
					+ "' doesn't exists, you'll need to create it first.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
		CMLib.genEd().modifyComponents(mob, skillID);
		String parms = CMLib.ableMapper().getAbilityComponentCodedString(
				skillID);
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The complication of skill usage just increased!");
	}

	public void socials(MOB mob, Vector commands) throws IOException {
		if (mob.isMonster())
			return;
		...
		if (stuff.equals("NONE"))
			stuff = "";
		Social S = CMLib.socials().fetchSocial((name + " " + stuff).trim(),
				false);
		if (S == null) {
		...
		mob.location().showOthers(
				...
				null,
				CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around the idea of  "
						+ S.name() + "s.");
		CMLib.socials().modifySocialInterface(mob,
				(name + " " + oldStuff).trim());
		...
		if (changed)
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"The happiness of all mankind has just fluxuated!");
	}
	...
	public void players(MOB mob, Vector commands) throws IOException {
		...
		mob.location().showOthers(mob, M, CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around <T-NAMESELF>.");
		...
		if (commands.size() < 5) {
			...
			String command = ((String) commands.elementAt(3)).toUpperCase();
			String restStr = CMParms.combine(commands, 4);
			if (command.equalsIgnoreCase("PROFICIENCIES")
					|| command.equalsIgnoreCase("PROFICIENCY")) {
				...
				mob.tell("All of " + M.Name()
						+ "'s skill proficiencies set to " + prof);
				Log.sysOut("Mobs", mob.Name() + " modified player " + M.Name()
						+ " skill proficiencies.");
			} else if (command.toUpperCase().startsWith("PROFICIENCY(")
					&& command.endsWith(")")) {
				int prof = CMath.s_int(restStr);
				String ableName = command.substring(12, command.length() - 1)
						.trim();
				Ability A = M.findAbility(ableName);
				if (A == null) {
					mob.tell("...but failed to specify an valid ability name.  Try one of: "
							+ CMParms.toCMObjectStringList(M.abilities()));
					mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> flub(s) a spell..");
					return;
				}
				...
				mob.tell(M.Name() + "'s skill proficiency in " + A.ID()
						+ " set to " + prof);
				Log.sysOut("Mobs", mob.Name() + " modified player " + M.Name()
						+ " skill proficiency in " + A.ID() + ".");
				...
				if (!copyMOB.sameAs(M))
					Log.sysOut("Mobs",
							mob.Name() + " modified player " + M.Name() + ".");
			} else {
				STreeSet<String> set = new STreeSet<String>();
	...
	public void manufacturer(MOB mob, Vector commands) throws IOException {
		...
		Manufacturer manufacturer = CMLib.tech()
				.getManufacturer(manufacturerID);
		if ((manufacturer == null)
				|| (manufacturer == CMLib.tech().getDefaultManufacturer())) {
			mob.tell("There's no manufacturer called '" + manufacturerID
					+ "' Try LIST MANUFACTURERS.\n\r");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
	...
	public void mobs(MOB mob, Vector commands) throws IOException {
		...
		mob.location().showOthers(mob, modMOB, CMMsg.MSG_OK_ACTION,
				"<S-NAME> wave(s) <S-HIS-HER> hands around <T-NAMESELF>.");
		if (command.equals("LEVEL")) {
			int newLevel = CMath.s_int(restStr);
			if (newLevel >= 0) {
				...
				mob.location()
						.show(mob,
								...
								CMMsg.MSG_OK_ACTION,
								modMOB.name()
										+ " shakes under the transforming power.");
			}
		} else if (command.equals("ABILITY")) {
			int newAbility = CMath.s_int(restStr);
			...
			modMOB.recoverPhyStats();
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					modMOB.name() + " shakes under the transforming power.");
		} else if (command.equals("REJUV")) {
			int newRejuv = CMath.s_int(restStr);
			if (newRejuv > 0) {
			...
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					modMOB.name() + " shake(s) under the transforming power.");
		} else {
			STreeSet<String> set = new STreeSet<String>();
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commandType.equals("ITEM")) {
			...
			String name = CMParms.combine(commands, 2);
			Ability A = CMClass.findAbility(name, Ability.ACODE_COMMON_SKILL,
					-1, false);
			if (A == null) {
			...
			CMLib.ableParms().modifyRecipesList(mob, iA.parametersFile(),
					iA.parametersFormat());
			...
			if (!CMSecurity.isAllowed(mob, mob.location(),
				...
				return errorOut(mob);
			rooms(mob, commands);
		} else if ((commandType.equals("ACCOUNT"))
			...
			if (!CMSecurity.isAllowed(mob, mob.location(),
					CMSecurity.SecFlag.CMDRACES))
				return errorOut(mob);
			races(mob, commands);
		} else if (commandType.equals("CLASS")) {
			if (!CMSecurity.isAllowed(mob, mob.location(),
					CMSecurity.SecFlag.CMDCLASSES))
				return errorOut(mob);
			classes(mob, commands);
		} else if (commandType.equals("ABILITY")) {
			if (!CMSecurity.isAllowed(mob, mob.location(),
					CMSecurity.SecFlag.CMDABILITIES))
				return errorOut(mob);
			...
			mobs(mob, commands);
			mob.location()
					.show(mob, null, CMMsg.MSG_OK_ACTION,
							"The space-time continuum shake(s) under the transforming power.");
			...
			mob.location().getArea().getTimeObj()
					.setDayOfMonth(CMath.s_int((String) commands.get(2)));
			...
			mob.location().getArea().getTimeObj()
					.setMonth(CMath.s_int((String) commands.get(2)));
			mob.location().getArea().getTimeObj().save();
			mob.location()
					.show(mob, null, CMMsg.MSG_OK_ACTION,
							"The space-time continuum shake(s) under the transforming power.");
			...
			mob.location().getArea().getTimeObj()
					.setYear(CMath.s_int((String) commands.get(2)));
			mob.location().getArea().getTimeObj().save();
			mob.location()
					.show(mob, null, CMMsg.MSG_OK_ACTION,
							"The space-time continuum shake(s) under the transforming power.");
			...
			mob.location().show(
					...
					null,
					CMMsg.MSG_OK_VISUAL,
					"^S<S-NAME> wave(s) <S-HIS-HER> arms around the idea of "
							+ P.getSubject() + ".^?");
			CMLib.polls().modifyVote(P, mob);
			mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
					"^SThe world's uncertainty has changed.^?");
			Log.sysOut("CreateEdit",
					mob.Name() + " modified Poll " + P.getName() + ".");
		} else if (commandType.equals("HOLIDAY")) {
			if (!CMSecurity.isAllowed(mob, mob.location(),
					CMSecurity.SecFlag.CMDQUESTS))
				return errorOut(mob);
			String name = CMParms.combine(commands, 2);
			int num = -1;
			if (CMath.isInteger(name))
			...
			CMMsg newMsg = CMClass.getMsg(mob, I, null, CMMsg.MSG_WRITE
					| CMMsg.MASK_ALWAYS, null, CMMsg.MSG_WRITE
					| CMMsg.MASK_ALWAYS, CMParms.combine(commands, 2),
					CMMsg.MSG_WRITE | CMMsg.MASK_ALWAYS, null);
			if (mob.location().okMessage(mob, newMsg)
					&& I.okMessage(mob, newMsg)) {
				mob.location().send(mob, newMsg);
				I.executeMsg(mob, newMsg);
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"^SThe world is now more informed!^?");
			}
			...
			try {
				...
				if (sql.equals("ping")) {
					...
					List<String[]> results = CMLib.database().DBRawQuery(
							sql.replace('`', '\''));
						...
						for (int r = 1; r < results.size(); r++) {
							...
							for (int c = 0; c < row.length; c++) {
								if (c < headerRow.length)
									buf.append(CMStrings.padRight(row[c],
											headerRow[c].length()));
								else
									buf.append(row[c]);
							}
					...
					int resp = CMLib.database().DBRawExecute(
							sql.replace('`', '\''));
				...
				if (G == null)
					for (ClanGovernment g : CMLib.clans().getStockGovernments())
						if (g.getName().toLowerCase()
							...
							G = g;
				if (G == null)
					mob.tell("Government '" + name
							+ "' is unknown.  Try list governments.");
				else if (!mob.isMonster()) {
					mob.location().showOthers(
							mob,
							null,
							CMMsg.MSG_OK_ACTION,
							"<S-NAME> wave(s) <S-HIS-HER> hands around "
									+ G.getName() + ".");
					CMLib.genEd().modifyGovernment(mob, G);
					CMLib.clans().reSaveGovernmentsXML();
					Log.sysOut("CreateEdit", mob.Name()
							+ " modified Clan Government " + G.getName() + ".");
				}
			}
		} else if (commandType.equals("FACTION")) {
			if (!CMSecurity.isAllowed(mob, mob.location(),
					CMSecurity.SecFlag.CMDFACTIONS))
				return errorOut(mob);
			if (commands.size() < 3)
				...
				if (F == null)
					mob.tell("Faction '" + name
							+ "' is unknown.  Try list factions.");
				else if (!mob.isMonster()) {
					mob.location().showOthers(
							mob,
							null,
					...
					Log.sysOut("CreateEdit", mob.Name() + " modified Faction "
							+ F.name() + " (" + F.factionID() + ").");
				}
			}
				...
				if (rest.equalsIgnoreCase("room"))
					...
					if (M == null) {
						...
						if (I instanceof Container)
							...
							mob.tell("MOB or Container '" + rest
									+ "' not found.");
							mob.location().showOthers(mob, null,
									CMMsg.MSG_OK_ACTION,
									"<S-NAME> flub(s) a spell..");
			...
			if ((srchMob != null) && (srchRoom != null))
				thang = srchRoom.fetchFromMOBRoomFavorsItems(srchMob,
						srchContainer, allWord, Wearable.FILTER_ANY);
			else if (srchMob != null)
				thang = srchMob.findItem(allWord);
			else if (srchRoom != null)
				thang = srchRoom
						.fetchFromRoomFavorItems(srchContainer, allWord);
			if ((thang != null) && (thang instanceof Item)) {
				...
				mob.location()
						.showOthers(mob, thang, CMMsg.MSG_OK_ACTION,
								"<S-NAME> wave(s) <S-HIS-HER> hands around <T-NAMESELF>.");
				if (!thang.isGeneric()) {
				...
				if (!copyItem.sameAs(thang))
					Log.sysOut("CreateEdit", mob.Name() + " modified item "
							+ thang.Name() + " (" + thang.ID() + ") in "
				...
				if ((!thang.isGeneric()) && (((MOB) thang).isMonster())) {
					...
					if (!CMSecurity.isAllowed(mob, mob.location(),
						...
						return errorOut(mob);
					players(mob,
							CMParms.parse("MODIFY USER \"" + thang.Name()
									+ "\""));
				} else {
					MOB copyMOB = (MOB) thang.copyOf();
					mob.location()
				...
				mob.location().show(
						...
						thang.name()
								+ " shake(s) under the transforming power.");
			} else if ((Directions.getGoodDirectionCode(allWord) >= 0)
					|| (thang instanceof Exit)) {
				if (Directions.getGoodDirectionCode(allWord) >= 0)
					thang = mob.location().getRawExit(
							Directions.getGoodDirectionCode(allWord));
				...
				if (thang != null) {
					...
					updateChangedExit(mob, mob.location(), (Exit) thang,
							copyExit);
				} else {
				...
				mob.tell("\n\rYou cannot modify a '"
						+ commandType
						+ "'. However, you might try an ITEM, RACE, CLASS, ABILITY, LANGUAGE, CRAFTSKILL, ALLQUALIFY, AREA, EXIT, COMPONENT, RECIPE, EXPERTISE, TITLE, QUEST, MOB, USER, HOLIDAY, MANUFACTURER, GOVERNMENT, JSCRIPT, FACTION, SOCIAL, CLAN, POLL, NEWS, DAY, MONTH, YEAR, TIME, HOUR, or ROOM.");
		}
		return false;
	}
