...
public class GModify extends StdCommand {
	...
	private static final Hashtable<Object, Integer> EQUATORS = CMStrings
			.makeNumericHash(new String[] { "$", "=", "!=", ">", "<", "<=",
					">=" });
	...
	private static boolean tryModfy(MOB mob, Room room, Environmental E,
			DVector changes, DVector onfields, boolean noisy) {
		...
		for (int i = 0; i < onfields.size(); i++) {
			...
			if (noisy)
				gmodifydebugtell(mob, field + "/" + getStat(E, field) + "/"
						+ value + "/" + getStat(E, field).equals(value));
			int matchStart = -1;
			int matchEnd = -1;
			...
			if (EQ != null)
				...
				case EQUATOR_GT: {
					...
					if (CMath.isNumber(stat) && CMath.isNumber(value))
						matchStart = (CMath.s_long(stat) > CMath.s_long(value)) ? 0
								: -1;
					else
			...
			if (matchStart >= 0)
				matches.addElement(field, Integer.valueOf(matchStart),
						Integer.valueOf(matchEnd));
		...
		if (checkedOut) {
			if (changes.size() == 0)
				mob.tell("Matched on " + E.name() + " from "
				...
				for (int i = 0; i < changes.size(); i++) {
					...
					if (noisy)
						gmodifydebugtell(mob, E.name() + " wants to change "
								+ field + " value " + getStat(E, field)
								+ " to " + value + "/"
								+ (!getStat(E, field).equals(value)));
					if (CMath.bset(codes, FLAG_SUBSTRING)) {
						int matchStart = -1;
						int matchEnd = -1;
						for (int m = 0; m < matches.size(); m++)
							if (((String) matches.elementAt(m, 1))
									.equals(field)) {
								matchStart = ((Integer) matches.elementAt(m, 2))
										.intValue();
								matchEnd = ((Integer) matches.elementAt(m, 3))
										.intValue();
							}
						if (matchStart >= 0) {
							stat = getStat(E, field);
							value = stat.substring(0, matchStart) + value
									+ stat.substring(matchEnd);
						}
					}
					if (!getStat(E, field).equals(value)) {
						Log.sysOut("GMODIFY",
								"The " + CMStrings.capitalizeAndLower(field)
										+ " field on " + E.Name() + " in "
										+ room.roomID() + " was changed from "
										+ getStat(E, field) + " to " + value
										+ ".");
	...
	public void sortEnumeratedList(Enumeration<? extends Environmental> e,
			Vector<String> allKnownFields, StringBuffer allFieldsMsg) {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((commands.size() > 0)
			...
			if (!CMSecurity
					.isAllowedEverywhere(mob, CMSecurity.SecFlag.GMODIFY)) {
		...
		for (int i = 0; i < newSet.size(); i++) {
			...
			while (str.trim().length() > 0) {
				...
				if (divLen == 0) {
					mob.tell("String '"
							+ str
							+ "' does not contain an equation divider.  Even CHANGE needs at least an = sign!");
					return false;
				}
				...
				Pattern P = null;
				if (use == null) {
					mob.tell("'" + ((String) commands.elementAt(i))
							+ "' goes to an unknown parameter!");
					return false;
				}
				while (val.trim().startsWith("[")) {
					...
					if (cd.equalsIgnoreCase("ss"))
						code = Integer
								.valueOf(code.intValue() | FLAG_SUBSTRING);
		...
		for (int i = placesToDo.size() - 1; i >= 0; i--) {
			if (placesToDo.elementAt(i) instanceof Area) {
				...
				for (Enumeration r = A.getCompleteMap(); r.hasMoreElements();) {
					...
					if (CMSecurity
							.isAllowed(mob, R, CMSecurity.SecFlag.GMODIFY))
		...
		if (noisy)
			gmodifydebugtell(
					mob,
					"When fields="
							+ CMParms.toStringList(onfields
									.getDimensionVector(1)));
		if (noisy)
			gmodifydebugtell(
					mob,
					"Change fields="
							+ CMParms.toStringList(changes
									.getDimensionVector(1)));
		...
		for (int r = 0; r < placesToDo.size(); r++) {
			...
			synchronized (("SYNC" + R.roomID()).intern()) {
				...
				for (int i = 0; i < R.numItems(); i++) {
					Item I = R.getItem(i);
					if ((I != null)
							&& (tryModfy(mob, R, I, changes, onfields, noisy)))
						saveitems = true;
				}
				for (int m = 0; m < R.numInhabitants(); m++) {
					...
					if ((M != null) && (M.isSavable())) {
						...
						for (int i = 0; i < M.numItems(); i++) {
							Item I = M.getItem(i);
							if ((I != null)
									&& (tryModfy(mob, R, I, changes, onfields,
											noisy)))
								savemobs = true;
						}
						...
						if (SK != null) {
							for (Iterator<Environmental> i = SK.getShop()
								...
								if (E2 instanceof Item) {
									Item I = (Item) E2;
									if (tryModfy(mob, R, I, changes, onfields,
											noisy))
										savemobs = true;
								}
							}
