...
public class Shutdown extends StdCommand implements Tickable {
	...
	protected void showDisplayableShutdownTimeRemaining() {
		...
		String tm = CMLib.time().date2EllapsedTime(until, TimeUnit.SECONDS,
				false);
		...
		for (Session S : CMLib.sessions().allIterable())
			S.colorOnlyPrintln("\n\r\n\r^Z"
					+ CMProps.getVar(CMProps.Str.MUDNAME) + " will be "
					+ (keepItDown ? "shutting down" : "restarting") + tm
					+ "^.^?\n\r");
	}
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		for (int i = commands.size() - 1; i >= 1; i--) {
			...
			if (s.equalsIgnoreCase("RESTART")) {
				...
				commands.removeElementAt(i);
				long timeMultiplier = CMLib.english()
						.getMillisMultiplierByName(multiplier);
				if ((timeMultiplier < 0) || (wait <= 0)) {
					mob.tell("I don't know how to shutdown within the next "
							+ wait + " " + multiplier
							+ "; try `5 minutes` or something similar.");
					return false;
				}
				if ((!mob.session().confirm(
						"Shutdown " + CMProps.getVar(CMProps.Str.MUDNAME)
								+ " in " + wait + " "
								+ multiplier.toLowerCase() + " (y/N)?", "N")))
					return false;
				shuttingDownCompletes = System.currentTimeMillis()
						+ (wait * timeMultiplier) - 1;
				shuttingDownNextAnnounce = System.currentTimeMillis()
						+ ((wait * timeMultiplier) / 2) - 100;
				shuttingDownMob = mob;
	...
	public void startShutdown(final MOB mob) {
		new Thread(Thread.currentThread().getThreadGroup(), "Shutdown"
				+ Thread.currentThread().getThreadGroup().getName().charAt(0)) {
			public void run() {
				for (Session S : CMLib.sessions().allIterable())
					S.colorOnlyPrintln("\n\r\n\r^Z"
							+ CMProps.getVar(CMProps.Str.MUDNAME) + " is now "
							+ (keepItDown ? "shutting down" : "restarting")
							+ "!^.^?\n\r");
				if (keepItDown)
					Log.errOut("CommandProcessor", mob.Name()
							+ " starts system shutdown...");
				else if (externalCommand != null)
					Log.errOut("CommandProcessor", mob.Name()
							+ " starts system restarting '" + externalCommand
							+ "'...");
				else
					Log.errOut("CommandProcessor", mob.Name()
							+ " starts system restart...");
				mob.tell("Starting " + (keepItDown ? "shutdown" : "restart")
						+ "...");
				com.planet_ink.coffee_mud.application.MUD.globalShutdown(
						mob.session(), keepItDown, externalCommand);
			}
		}.start();
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (System.currentTimeMillis() > shuttingDownCompletes) {
			...
			shuttingDownNextAnnounce = System.currentTimeMillis()
					+ ((shuttingDownCompletes - System.currentTimeMillis()) / 2)
					- 100;
			showDisplayableShutdownTimeRemaining();
		}
		return true;
