...
public class ClanReject extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
			...
			for (Pair<Clan, Integer> c : mob.clans())
				if ((clanName.length() == 0)
						...
						&& (c.first.getAuthority(c.second.intValue(),
								Clan.Function.REJECT) != Authority.CAN_NOT_DO)) {
					C = c.first;
					break;
				}
		...
		if (memberStr.length() > 0) {
			if (C == null) {
				mob.tell("You aren't allowed to reject anyone from "
			...
			if (skipChecks
				...
				if (found) {
					...
					if (M == null) {
						mob.tell(memberStr
								+ " was not found.  Could not reject from "
								+ C.getGovernmentName() + ".");
						return false;
					}
					if (skipChecks
							|| CMLib.clans().goForward(mob, C, commands,
									Clan.Function.REJECT, true)) {
						C.delMember(M);
						mob.tell(M.Name() + " has been denied acceptance to "
								+ C.getGovernmentName() + " '" + C.clanID()
								+ "'.");
						if ((M.session() != null) && (M.session().mob() == M))
							M.tell("You have been rejected as a member of "
									+ C.getGovernmentName() + " '" + C.clanID()
									+ "'.");
						return false;
					}
				...
				msg.append("You aren't in the right position to reject applicants to your "
						+ C.getGovernmentName() + ".");
			}
		} else {
