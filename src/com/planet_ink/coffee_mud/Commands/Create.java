...
public class Create extends StdCommand {
	...
	public void exits(MOB mob, Vector commands) throws IOException {
		...
		if (opRoom != null)
			reverseExit = opRoom.getRawExit(Directions
		...
		} else if ((reverseExit == null) && (opExit == null)
				&& (opRoom != null)) {
			int revDirCode = Directions.getOpDirectionCode(direction);
			if ((opRoom.getRawExit(revDirCode) == null)
					&& (opRoom.rawDoors()[revDirCode] == mob.location())) {
				opRoom.setRawExit(revDirCode, (Exit) thisExit.copyOf());
				CMLib.database().DBUpdateExits(opRoom);
			}
		}
		...
		Log.sysOut("Exits", mob.location().roomID() + " exits changed by "
				+ mob.Name() + ".");
	}
	...
	public void items(MOB mob, Vector commands) throws IOException {
		...
		if (x > 0) {
			...
			if ((!rest.equalsIgnoreCase("room")) && (rest.length() > 0)) {
		...
		if ((newItem == null)
				&& (CMLib.english().numPossibleGold(null, itemID) > 0)) {
			long numCoins = CMLib.english().numPossibleGold(null, itemID);
			String currency = CMLib.english().numPossibleGoldCurrency(mob,
					itemID);
			double denom = CMLib.english().numPossibleGoldDenomination(mob,
					currency, itemID);
			if ((numCoins > 0) && (denom > 0.0))
				newItem = CMLib.beanCounter().makeCurrency(currency, denom,
						numCoins);
		}
		...
		if ((newItem instanceof ArchonOnly) && (!CMSecurity.isASysOp(mob))) {
			mob.tell("NO!");
			return;
		}
		...
		if (dest instanceof Room) {
			...
			mob.location().showHappens(
					CMMsg.MSG_OK_ACTION,
					"Suddenly, " + newItem.name() + " drops into "
							+ dest.name() + "'s arms.");
		}
	...
	public void manufacturer(MOB mob, Vector commands) throws IOException {
		...
		Manufacturer manufacturer = (Manufacturer) CMClass
		...
		if (((manuCheck != null) && (manuCheck != CMLib.tech()
	...
	public void rooms(MOB mob, Vector commands) {
		...
		if (thisRoom == null) {
			mob.tell("You have failed to specify a valid room type '"
					+ Locale
		...
		thisRoom.setDisplayText(CMClass.classID(thisRoom) + "-"
				+ thisRoom.roomID());
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"Suddenly a block of earth falls from the sky.\n\r");
	...
	public void accounts(MOB mob, Vector commands) {
		...
		String AcctName = CMStrings.capitalizeAndLower((String) commands
				.elementAt(2));
		String password = CMStrings.capitalizeAndLower(CMParms.combine(
				commands, 3));
		...
		if (CMProps.getBoolVar(CMProps.Bool.ACCOUNTEXPIRATION))
			thisAcct.setAccountExpiration(System.currentTimeMillis()
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"A new soul descends from the heavens and dissipates.\n\r");
		Log.sysOut("Create",
				mob.Name() + " created account " + thisAcct.accountName() + ".");
	}
	...
	public void mobs(MOB mob, Vector commands) throws IOException {
		...
		newMOB.baseCharStats()
				.getMyRace()
				.setHeightWeight(
						newMOB.basePhyStats(),
						(char) newMOB.baseCharStats().getStat(
	...
	public void races(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
			...
			if ((mob.session() == null)
					|| (!mob.session()
							.confirm(
									"Currently, "
											+ R.ID()
											+ " is a standard race.  This will convert the "
		...
		if (raceID.indexOf(' ') >= 0) {
			mob.tell("'" + raceID
					+ "' is an invalid race id, because it contains a space.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
		if (GR == null) {
			...
			GR.setRacialParms("<RACE><ID>"
					+ CMStrings.capitalizeAndLower(raceID) + "</ID><NAME>"
					+ CMStrings.capitalizeAndLower(raceID) + "</NAME></RACE>");
		}
		CMClass.addRace(GR);
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The diversity of the world just increased!");
	}
	...
	public void areas(MOB mob, Vector commands) throws IOException {
		...
		while ((areaType.length() == 0) && ((++tries) < 10)) {
			areaType = mob.session().prompt(
					"Enter an area type to create (default=StdArea): ",
					"StdArea");
			if (CMClass.getAreaType(areaType) == null) {
				mob.session().println("Invalid area type! Valid ones are:");
				mob.session().println(
						CMLib.lister().reallyList(mob, CMClass.areaTypes())
								.toString());
				areaType = "";
			}
		}
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The size of the world just increased!");
	...
	public void components(MOB mob, Vector commands) throws IOException {
		...
		if (CMLib.ableMapper().getAbilityComponentMap()
			...
			mob.tell("'" + A.ID()
					+ "' already exists, you'll need to destroy it first.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
		...
		CMLib.ableMapper().getAbilityComponentMap()
				.put(skillID.toUpperCase().trim(), DV);
	...
	public void expertises(MOB mob, Vector commands) {
		if ((commands.size() < 3)
				|| (CMParms.combine(commands, 1).indexOf('=') < 0)) {
			mob.tell("You have failed to specify the proper fields.\n\rFormat: CREATE EXPERTISE [EXPERTISE ID]=[PARAMETERS] as follows: \n\r");
			StringBuffer buf = new CMFile(
					Resources.makeFileResourceName("skills/expertises.txt"),
		...
		String error = CMLib.expertises().confirmExpertiseLine(parms, null,
				false);
		if (error != null) {
			mob.tell(error);
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The power of skill usage just increased!");
	}
	...
	public void titles(MOB mob, Vector commands) {
		if ((commands.size() < 3)
			...
			StringBuffer buf = new CMFile(
					Resources.makeFileResourceName("titles.txt"), null,
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The prestige of the players just increased!");
	}

	public void abilities(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		if ((A != null) && (A.isGeneric())) {
			mob.tell("A generic ability with the ID '" + A.ID()
					+ "' already exists!");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		...
		if (classD.indexOf(' ') >= 0) {
			mob.tell("'" + classD
					+ "' is an invalid  id, because it contains a space.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
		...
		CMLib.database().DBCreateAbility(CR.ID(), "GenAbility",
				CR.getStat("ALLXML"));
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The skill of the world just increased!");
	}

	public void languages(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		CMLib.database().DBCreateAbility(CR.ID(), "GenLanguage",
				CR.getStat("ALLXML"));
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The skill of the world just increased!");
	}

	public void craftSkills(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		CMLib.database().DBCreateAbility(CR.ID(), "GenCraftSkill",
				CR.getStat("ALLXML"));
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The skill of the world just increased!");
	}
	...
	public void allQualify(MOB mob, Vector commands) throws IOException {
		...
		if (subMap.containsKey(classD.toUpperCase().trim())) {
			mob.tell("All-Qualify entry (" + eachOrAll + ") ID '" + A.ID()
					+ "' already exists.  Try DESTROY or MODIFY.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
	...
	public void classes(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		if ((C != null) && (C.isGeneric())) {
			mob.tell("A generic class with the ID '" + C.ID()
					+ "' already exists!");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
			...
			if ((mob.session() == null)
					|| (!mob.session()
							.confirm(
									"Currently, "
											+ C.ID()
											+ " is a standard character class.  This will convert the "
		...
		if (classD.indexOf(' ') >= 0) {
			mob.tell("'" + classD
					+ "' is an invalid class id, because it contains a space.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
		if (CR == null) {
			...
			CR.setClassParms("<CCLASS><ID>"
					+ CMStrings.capitalizeAndLower(classD) + "</ID><NAME>"
					+ CMStrings.capitalizeAndLower(classD) + "</NAME></CCLASS>");
		}
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The employment of the world just increased!");
	}
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commandType.equals("EXIT")) {
			...
			String err = CMLib.quests().createHoliday(named,
					mob.location().getArea().name(), true);
			if (err.length() > 0) {
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"^S<S-NAME> wave(s) <S-HIS-HER> arms...^?");
				mob.tell(err);
				...
				mob.doCommand(new XVector("MODIFY", "HOLIDAY", named),
						metaFlags);
			...
			mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
					"^S<S-NAME> wave(s) <S-HIS-HER> arms...^?");
			if ((commands.size() < 3)
					|| (CMParms.combine(commands, 2).indexOf('.') < 0)) {
				...
				if (F != null) {
					...
					return false;
						...
						&& (mob.session().confirm(
								"Create a new faction with ID/filename: 'resources/"
										+ name + "' (N/y)? ", "N"))) {
					// name=Resources.buildResourcePath("")+name;
					StringBuffer template = new CMFile(
							Resources.buildResourcePath("examples")
									+ "factiontemplate.ini", null,
							CMFile.FLAG_LOGERRORS).text();
					...
					Resources.saveFileResource("::"
							+ CMLib.factions().makeFactionFilename(name), null,
							template);
					F = (Faction) CMClass.getCommon("DefaultFaction");
					F.initializeFaction(template, name);
					CMLib.factions().modifyFaction(mob, F);
					Log.sysOut("CreateEdit", mob.Name() + " created Faction "
							+ F.name() + " (" + F.factionID() + ").");
				} else
					return false;
			}
			...
			mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
					"^SThe world has grown more uncertain.^?");
			Log.sysOut("CreateEdit",
					mob.Name() + " created Poll " + P.getName() + ".");
			...
			if (commands.size() < 3) {
				if ((mob.session() != null)
						&& (mob.session()
								.confirm(
										"Create a new Quest using the Quest Maker Wizard (y/N)? ",
										"N")))
				...
				if ((Q.name().trim().length() == 0) || (Q.duration() < 0)) {
					...
					return false;
						...
						&& ((mob.isMonster()) || (!mob
								.session()
								.confirm(
										"That quest is already loaded.  Load a duplicate (N/y)? ",
										"N"))))
					return false;
				else {
			...
			if (commands.size() < 3) {
				...
				if (G == null) {
					...
					Log.sysOut("CreateEdit", mob.Name()
							+ " created Clan Government " + G.getName() + ".");
				} else
					return false;
			}
		} else {
			...
			if (((E != null) && (E instanceof Item))
				...
				if (((E != null) && (E instanceof MOB))
					...
					execute(mob, commands, metaFlags);
						...
						&& (Directions.getGoodDirectionCode(lastWord) >= 0)) {
					...
					if ((E != null) && (E instanceof Room)) {
						...
						mob.tell("\n\rYou cannot create a '"
								+ commandType
								+ "'. However, you might try an EXIT, ITEM, QUEST, FACTION, COMPONENT, GOVERNMENT, HOLIDAY, CLAN, MOB, RACE, ABILITY, LANGUAGE, CRAFTSKILL, MANUFACTURER, ALLQUALIFY, CLASS, POLL, DEBUGFLAG, DISABLEFLAG, NEWS, USER, or ROOM.");
						return false;
					}
				} else if (CMath.isInteger(firstWord) && (lastWord != null)) {
					...
					if ((theRest.length() > 0) && (matCode >= 0)) {
						for (int i = 0; i < num; i++)
							mob.location()
									.addItem(
											CMLib.materials().makeItemResource(
													matCode),
											ItemPossessor.Expire.Player_Drop);
						mob.location().showHappens(
								CMMsg.MSG_OK_VISUAL,
								"Suddenly " + num + " "
										+ RawMaterial.CODES.NAME(matCode)
										+ " fall from the sky.");
					} else {
					...
					mob.tell("\n\rYou cannot create a '"
							+ commandType
							+ "'. However, you might try an EXIT, ITEM, QUEST, FACTION, MOB, COMPONENT, GOVERNMENT, MANUFACTURER, HOLIDAY, CLAN, RACE, ABILITY, LANGUAGE, CRAFTSKILL, ALLQUALIFY, CLASS, POLL, USER, DEBUGFLAG, NEWS, DISABLEFLAG, ROOM.");
					return false;
				}
			}
		}
