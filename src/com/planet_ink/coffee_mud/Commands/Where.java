...
public class Where extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((CMSecurity
				.isAllowed(mob, mob.location(), CMSecurity.SecFlag.WHERE))
				&& (!overrideSet)) {
			...
			if (who.length() == 0) {
				for (Session S : CMLib.sessions().localOnlineIterable()) {
					...
					if (canShowTo(mob, mob2)) {
						...
						if (CMSecurity.isAllowed(mob, R,
								CMSecurity.SecFlag.WHERE)) {
							lines.append("^!"
									+ CMStrings.padRight(mob2.Name(), 17)
									+ "^N| ");
							if (R != null) {
				...
				if ((who.toUpperCase().startsWith("EXIT "))
				...
				} else if (who.toUpperCase().startsWith("MOBMASK ")
						|| who.toUpperCase().startsWith("MOBMASK=")) {
					mobOnly = true;
					zapperMask = true;
					who = who.substring(8).trim();
					mob.tell("^xMask used:^?^.^N "
							+ CMLib.masking().maskDesc(who) + "\n\r");
					compiledZapperMask = CMLib.masking().maskCompile(who);
				} else if (who.toUpperCase().startsWith("ITEMMASK ")
						|| who.toUpperCase().startsWith("ITEMMASK=")) {
					itemOnly = true;
					zapperMask = true;
					...
					mob.tell("^xMask used:^?^.^N "
							+ CMLib.masking().maskDesc(who) + "\n\r");
					compiledZapperMask = CMLib.masking().maskCompile(who);
				} else if (who.toUpperCase().startsWith("MOBMASK2 ")
						|| who.toUpperCase().startsWith("MOBMASK2=")) {
					mobOnly = true;
					zapperMask2 = true;
					mob.tell("^xMask used:^?^.^N "
							+ CMLib.masking().maskDesc(who) + "\n\r");
					who = who.substring(9).trim();
				} else if (who.toUpperCase().startsWith("ITEMMASK2 ")
						|| who.toUpperCase().startsWith("ITEMMASK2=")) {
					itemOnly = true;
					zapperMask2 = true;
					mob.tell("^xMask used:^?^.^N "
							+ CMLib.masking().maskDesc(who) + "\n\r");
				...
				Enumeration<Room> r = (roomOnly || exitOnly) ? CMLib.map()
						.rooms() : CMLib.map().roomsFilled();
				if (who.toUpperCase().startsWith("AREA ") || areaFlag)
					r = (roomOnly || exitOnly) ? mob.location().getArea()
							.getProperMap() : mob.location().getArea()
							.getFilledProperMap();
				if (who.toUpperCase().startsWith("AREA "))
					who = who.substring(5).trim();
				Room R = null;
								...
								if ((M != null)
									...
									if ((!itemOnly) && (!roomOnly)
										...
										if ((zapperMask) && (mobOnly)) {
											...
											if (CMLib.masking().maskCheck(who,
													M, true)) {
												lines.append("^!"
														+ CMStrings.padRight(
																cataMark(M)
			...
			for (int i = commands.size() - 1; i >= 0; i--) {
				...
				if (s.equalsIgnoreCase("good")) {
					alignment = CMLib.factions().getAlignMedianFacValue(
							Faction.Align.GOOD);
			...
			for (Enumeration a = CMLib.map().areas(); a.hasMoreElements();) {
				...
				if ((CMLib.flags().canAccess(mob, A))
					...
					int median = A.getAreaIStats()[Area.Stats.MED_LEVEL
							.ordinal()];
					int medianDiff = 0;
					int diffLimit = 6;
					if ((median < (moblevel + diffLimit))
						...
						if (mob.phyStats().level() >= median)
							medianDiff = (int) Math.round(9.0 * CMath.div(
									median, moblevel));
						else
							medianDiff = (int) Math.round(10.0 * CMath.div(
									moblevel, median));
					}
					whereAdd(levelsVec, A, medianDiff);

					whereAdd(mobsVec, A,
							A.getAreaIStats()[Area.Stats.POPULATION.ordinal()]);

					int align = A.getAreaIStats()[Area.Stats.MED_ALIGNMENT
							.ordinal()];
					int alignDiff = ((int) Math
							.abs((double) (alignment - align)));
					whereAdd(alignVec, A, alignDiff);
				}
			}
			StringBuffer msg = new StringBuffer("You are currently in: ^H"
					+ mob.location().getArea().name() + "^?\n\r");
			if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.ROOMVISITS))
					&& (mob.playerStats() != null))
				msg.append("You have explored "
						+ mob.playerStats().percentVisited(mob,
			...
			msg.append("^x" + CMStrings.padRight("Area Name", 35)
					+ CMStrings.padRight("Level", 6)
					+ CMStrings.padRight("Alignment", 20)
					+ CMStrings.padRight("Pop", 10) + "^.^?\n\r");
			for (int i = scores.size() - 1; ((i >= 0) && (i >= (scores.size() - 15))); i--) {
				...
				msg.append(CMStrings.padRight(A.name(), 35))
						...
						.append(CMStrings.padRight(
								CMLib.factions()
										.getRange(CMLib.factions().AlignID(),
												align).name(), 20))
						.append(CMStrings.padRight(Integer.toString(A
								.getAreaIStats()[Area.Stats.POPULATION
								.ordinal()]), 10)).append("\n\r");
			}
