...
public class Throw extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((commands.size() == 2) && (mob.isInCombat()))
			commands.addElement(mob.getVictim().location()
					.getContextName(mob.getVictim()));
		if (commands.size() < 3) {
		...
		if (dir < 0)
			...
			if ((target == null) || (mob.location().getExitInDir(dir) == null)
		...
		if ((dir < 0)
				&& ((target == null) || ((target != mob.getVictim()) && (!CMLib
						.flags().canBeSeenBy(target, mob))))) {
		...
		if (!(target instanceof Room)) {
			...
			if (mob.location().okMessage(mob, newMsg)) {
				...
				CMMsg msg = CMClass.getMsg(mob, target, item, CMMsg.MSG_THROW,
						targetMsg, CMMsg.MSG_THROW,
						"<S-NAME> throw(s) <O-NAME> at <T-NAMESELF>.");
				if (mob.location().okMessage(mob, msg))
					mob.location().send(mob, msg);
			}
		} else {
			final boolean useShipDirs = ((mob.location() instanceof SpaceShip) || (mob
					.location().getArea() instanceof SpaceShip));
			final int opDir = Directions.getOpDirectionCode(dir);
			final String inDir = useShipDirs ? Directions
			...
			CMMsg msg = CMClass.getMsg(mob, target, item, CMMsg.MSG_THROW,
					"<S-NAME> throw(s) <O-NAME> " + inDir.toLowerCase() + ".");
			CMMsg msg2 = CMClass.getMsg(mob, target, item, CMMsg.MSG_THROW,
					"<O-NAME> fl(ys) in from " + fromDir.toLowerCase() + ".");
			if (mob.location().okMessage(mob, msg)
					&& ((Room) target).okMessage(mob, msg2)) {
				mob.location().send(mob, msg);
