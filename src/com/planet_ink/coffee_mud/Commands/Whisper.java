...
public class Whisper extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() > 2) {
			...
			if ((target != null)
					&& (!target.name().equalsIgnoreCase(possibleTarget))
					&& (possibleTarget.length() < 4))
				target = null;
			if ((target != null)
					&& (CMLib.flags().canBeSeenBy(target, mob))
					&& ((!(target instanceof Rider)) || (((Rider) target)
							.riding() == mob.riding())))
		...
		if (target == null) {
			...
			if (riddenR == null) {
				msg = CMClass.getMsg(mob, null, null, CMMsg.MSG_SPEAK,
						"^T<S-NAME> whisper(s) to <S-HIM-HERSELF> '"
								+ combinedCommands + "'.^?"
								+ CMLib.protocol().msp("whisper.wav", 40),
						CMMsg.NO_EFFECT, null, CMMsg.MSG_QUIETMOVEMENT,
						"^T<S-NAME> whisper(s) to <S-HIM-HERSELF>.^?"
								+ CMLib.protocol().msp("whisper.wav", 40));
				if (R.okMessage(mob, msg))
					R.send(mob, msg);
			} else {
				msg = CMClass.getMsg(mob, riddenR, null, CMMsg.MSG_SPEAK,
						"^T<S-NAME> whisper(s) around <T-NAMESELF> '"
								+ combinedCommands + "'.^?"
								+ CMLib.protocol().msp("whisper.wav", 40),
						CMMsg.MSG_SPEAK,
						"^T<S-NAME> whisper(s) around <T-NAMESELF> '"
				...
				if (R.okMessage(mob, msg)) {
					...
					for (Enumeration<Environmental> e = targets.elements(); e
							.hasMoreElements();) {
								...
								msg = CMClass.getMsg(
										...
										"^T<S-NAME> whisper(s) around "
												...
												+ CMLib.protocol().msp(
														"whisper.wav", 40),
										CMMsg.MSG_SPEAK,
										"^T<S-NAME> whisper(s) something around "
												+ riddenR.name()
												+ ".^?"
												+ CMLib.protocol().msp(
														"whisper.wav", 40),
			...
			msg = CMClass.getMsg(mob, target, null, CMMsg.MSG_SPEAK,
					"^T^<WHISPER \"" + CMStrings.removeColors(target.name())
							+ "\"^><S-NAME> whisper(s) to <T-NAMESELF> '"
							+ combinedCommands + "'.^</WHISPER^>^?"
							+ CMLib.protocol().msp("whisper.wav", 40),
					CMMsg.MSG_SPEAK,
					"^T^<WHISPER \"" + CMStrings.removeColors(target.name())
							+ "\"^><S-NAME> whisper(s) to <T-NAMESELF> '"
					...
					"^T<S-NAME> whisper(s) something to <T-NAMESELF>.^?"
							+ CMLib.protocol().msp("whisper.wav", 40));
			if (R.okMessage(mob, msg))
				R.send(mob, msg);
		}
