...
public class Wear extends StdCommand {
	...
	public boolean wear(MOB mob, Item item, int locationIndex, boolean quiet) {
		...
		if (item.rawProperLocationBitmap() == Wearable.WORN_HELD) {
			...
			str = "<S-NAME> put(s) <T-NAME> on <S-HIS-HER> "
					+ Wearable.CODES.NAME(locationIndex).toLowerCase() + ".";
		CMMsg newMsg = CMClass.getMsg(mob, item, null, msgType, quiet ? null
				: str);
		...
		if (commands.firstElement() instanceof Item) {
			...
			if (commands.size() > 0) {
				if (commands.firstElement() instanceof Integer) {
					wearLocationIndex = ((Integer) commands.firstElement())
							.intValue();
					commands.removeElementAt(0);
				} else if (commands.firstElement() instanceof String) {
					int newDex = codes.findDex_ignoreCase((String) commands
							.firstElement());
					if (newDex > 0) {
		...
		for (int i = commands.size() - 2; i > 0; i--)
			if (((String) commands.elementAt(i)).equalsIgnoreCase("on")) {
				if ((i < commands.size() - 2)
						&& ((String) commands.elementAt(i + 1))
								.equalsIgnoreCase("my"))
					commands.removeElementAt(i + 1);
				String possibleWearLocation = CMParms.combine(commands, i + 1)
						.toLowerCase().trim();
				int possIndex = CMParms.indexOfIgnoreCase(
						Wearable.CODES.NAMES(), possibleWearLocation);
				if (possIndex < 0)
					possIndex = Wearable.CODES.FINDDEX_endsWith(" "
							+ possibleWearLocation);
				if (possIndex > 0) {
					...
					mob.tell("You can't wear anything on your '"
							+ possibleWearLocation + "'");
					return false;
				}
