...
public class CharGen extends StdCommand {
	...
	protected void equipPlayer(MOB M) {
		...
		for (long wornCode : Wearable.CODES.ALL()) {
			...
			if (wornCode == Wearable.WORN_WIELD) {
				...
				W.setRawProperLocationBitmap(Wearable.WORN_WIELD
						| Wearable.WORN_HELD);
	...
	protected MOB levelMOBup(int level, CharClass C, boolean player) {
		...
		for (int lvl = 1; lvl < level; lvl++) {
			if ((lvl % 3) == 0) {
				...
				if (stat >= 0)
					if (mob.baseCharStats().getStat(stat) < (max + mob
							.charStats().getStat(
									CharStats.CODES.toMAXBASE(stat))))
						mob.baseCharStats().setStat(stat,
								mob.baseCharStats().getStat(stat) + 1);
			}
			if (mob.getExpNeededLevel() == Integer.MAX_VALUE)
				CMLib.leveler().level(mob);
			else
				CMLib.leveler().postExperience(mob, null, null,
						mob.getExpNeededLevel() + 1, false);
			mob.recoverPhyStats();
			mob.recoverCharStats();
			mob.recoverMaxState();
		}
	...
	public void averageout(MOB avgMob, int tries) {
		for (int i : CharStats.CODES.BASE())
			avgMob.baseCharStats().setStat(
					i,
					(int) Math.round(CMath.div(avgMob.baseCharStats()
							.getStat(i), tries)));
		avgMob.basePhyStats().setArmor(
				(int) Math.round(CMath
						.div(avgMob.basePhyStats().armor(), tries)));
		avgMob.baseState().setHitPoints(
				(int) Math.round(CMath.div(avgMob.baseState().getHitPoints(),
						tries)));
		avgMob.baseState().setMovement(
				(int) Math.round(CMath.div(avgMob.baseState().getMovement(),
						tries)));
		avgMob.baseState()
				.setMana(
						(int) Math.round(CMath.div(
								avgMob.baseState().getMana(), tries)));
	...
	public void addHimIn(MOB avgMob, MOB mob2) {
		for (int i : CharStats.CODES.BASE())
			avgMob.baseCharStats().setStat(
					i,
					avgMob.baseCharStats().getStat(i)
							+ mob2.baseCharStats().getStat(i));
		avgMob.basePhyStats().setArmor(
				avgMob.basePhyStats().armor() + mob2.basePhyStats().armor());
		avgMob.baseState().setHitPoints(
				avgMob.baseState().getHitPoints()
						+ mob2.baseState().getHitPoints());
		avgMob.baseState().setMovement(
				avgMob.baseState().getMovement()
						+ mob2.baseState().getMovement());
		avgMob.baseState().setMana(
				avgMob.baseState().getMana() + mob2.baseState().getMana());
		avgMob.recoverCharStats();
		avgMob.recoverPhyStats();
		avgMob.recoverMaxState();
		avgMob.resetToMaxState();
	...
	public MOB AverageAllClassMOB(MOB mob, int level, int numTriesClass,
			int numTriesMOB, boolean player) {
		MOB avgMob = null;
		int tries = 0;
		int numClasses = 0;
	...
	private class CombatStats {
		...
		int skipLevels = 0;
		int[][][] allData = null;// new
									// int[classSet.size()][levelEnd-levelStart+1][17];
		String[][][] allSkills = null;// new
										// String[classSet.size()][levelEnd-levelStart+1][4];
		Area A = null;
		MOB mob = null;
	...
	public void combatRun(MOB mob, Vector commands) {
		...
		String[][] CAMATCH = { { "Commoner", "CombatAbilities" },
				{ "Bard", "Bardness" }, { "Cleric", "Clericness" },
				{ "Druid", "Druidness" }, { "Mage", "Mageness" },
				{ "Thief", "Thiefness" }, { "Fighter", "Fighterness" }, };
		...
		for (Enumeration e = CMClass.charClasses(); e.hasMoreElements();) {
			...
			if ((CMath.bset(C.availabilityCode(), Area.THEME_FANTASY)
					|| CMath.bset(C.availabilityCode(), Area.THEME_HEROIC) || CMath
						.bset(C.availabilityCode(), Area.THEME_TECHNOLOGY))
					&& (!CMath.bset(C.availabilityCode(),
		...
		for (int i = 0; i < commands.size(); i++) {
			...
			if (CMath.isInteger(s)) {
				...
				for (Enumeration e = CMClass.charClasses(); e.hasMoreElements();) {
					...
					if ((CMath.bset(C.availabilityCode(), Area.THEME_FANTASY)
							|| CMath.bset(C.availabilityCode(),
									Area.THEME_HEROIC) || CMath.bset(
							C.availabilityCode(), Area.THEME_TECHNOLOGY))
							&& (!CMath.bset(C.availabilityCode(),
									Area.THEME_SKILLONLYMASK))
							&& (C.baseClass().equalsIgnoreCase(s) || (s
									.equalsIgnoreCase("charclasse")))) {
						if (!classCleared) {
							classCleared = true;
							c.classSet = new DVector(2);
						}
		...
		c.allSkills = new String[c.classSet.size()][c.levelEnd - c.levelStart
				+ 1][4];

		final String[] allDataHeader = { "BestIterScore",// 0
				"BestHitScore",// 1
		...
		final java.util.concurrent.CountDownLatch latch = new java.util.concurrent.CountDownLatch(
				c.classSet.size());
		final java.util.concurrent.atomic.AtomicInteger IDGEN = new java.util.concurrent.atomic.AtomicInteger(
				1);
		final Race humanR = CMClass.getRace("Human");
		for (int charClassDex = 0; charClassDex < c.classSet.size(); charClassDex++) {
			new Thread(Thread.currentThread().getThreadGroup(), "CharGen"
				...
				public void start(CombatStats c, int charClassDex,
						String fileExp,
				...
				public void run() {
					...
					for (int level = c.levelStart; level <= c.levelEnd; level += c.skipLevels) {
						CharClass C = (CharClass) c.classSet.elementAt(
								charClassDex, 1);
						mob.tell(C.ID() + ": " + level);
						int roomRobin = 0;
						Room R = null;
						...
						for (int tries = 0; tries < c.TOTAL_ITERATIONS; tries++) {
							...
							Behavior B1 = CMClass.getBehavior((String) classSet
									.elementAt(charClassDex, 2));
							B1.setParms(C.ID());
							switch (roomRobin) {
							case 0:
							...
							if (R != null) {
								R.addNonUninvokableEffect(CMClass
										.getAbility("Spell_Light"));
								R.setRoomID(c.A.name() + "#"
										+ IDGEN.addAndGet(1));
								R.setArea(c.A);
								R.recoverPhyStats();
							}
							c.A.getTimeObj().setTimeOfDay(
									CMLib.dice().roll(1,
											c.A.getTimeObj().getHoursInDay(),
											-1));

							// Session
							// S=(Session)CMClass.getCommon("FakeSession");
							// S.initializeSession(null,"MEMORY");
							MOB M1 = null;
							if (C.ID().equalsIgnoreCase("StdCharClass")) {
								M1 = CMClass.getFactoryMOB(); // MOB stat
								Behavior B2 = CMClass
										.getBehavior("CombatAbilities");
								...
								CMLib.threads().deleteTick(M1,
										Tickable.TICKID_MOB);
								CMLib.leveler().fillOutMOB(M1, level);
								// int
								// hp=M1CMLib.leveler().getLevelMOBPlayerHP(M1);
							...
							if (!playerExampleShown) {
								...
								/*
								 * StringBuffer
								 * msg=CMLib.commands().getScore(M1);
								 * if(!mob.isMonster())
								 * mob.session().wraplessPrintln
								 * (msg.toString()); if(mob.session()!=null) {
								 * mob
								 * .session().println(M1.name()+" has "+B1.ID(
								 * )+" behavior for "
								 * +M1.numAbilities()+" abilities.");
							...
							// chargen combat charclasses export=test.tab
							// iterations=100 skiplevels=20 1 91
							while ((M1.getVictim() == M2)
									&& (M2.getVictim() == M1) && (!M1.amDead())
									&& (!M2.amDead()) && (!M1.amDestroyed())
									&& (!M2.amDestroyed())
								...
								int h = h2
										- (M2.amDead() ? 0 : M2.curState()
												.getHitPoints());
								h = h
										- (h1 - (M1.amDead() ? 0 : M1
												.curState().getHitPoints()));
								if ((h == 0)
										&& ((!M1.amDead()) && (!M2.amDead()))) {
									zeroCheck++;
									if (zeroCheck == 1) {
										// ZEROMOBS=new
										// MOB[]{(MOB)M1.copyOf(),(MOB)M2.copyOf()};
										ZEROSKILL1 = ALMOSTZEROSKILL;
										ZEROSKILL2 = B1.getStat("LASTSPELL");
									} else if (zeroCheck == 20)
										Log.errOut("CharGen",
												"Stale combat warning: "
														+ ZEROSKILL1 + "/"
														+ ZEROSKILL2);
									else if (zeroCheck == 100) {

										Log.errOut("CharGen",
												"Stale Combat Abort: " + level
														+ "/" + tries + "/"
														+ iterations + "/"
														+ ZEROSKILL1 + "/"
														+ ZEROSKILL2);
								...
								if (h > bestSingleHitScore[0]) {
									...
									bestSingleHitPhys[0] = CMath.s_int(B2
											.getStat("PHYSDAMTAKEN")) - l1;
									bestSingleHitSkill[0] = B1
											.getStat("LASTSPELL");
								}
								cumScore += h;
							}
							if (iterations >= 1000)
								Log.errOut("CharGen", "Stale Combat Abort: "
										+ level + "/" + tries + "/"
										+ ZEROSKILL1 + "/" + ZEROSKILL2);
							else if (M1.amDead() || M2.amDead()
									|| M1.amDestroyed() || M2.amDestroyed()) {
								if (M1.amDead() || M1.amDestroyed())
									losses[0]++;
								medScore.addElement(Integer.valueOf(cumScore));
								medPhysDone.addElement(Integer.valueOf(CMath
										.s_int(B2.getStat("PHYSDAMTAKEN"))));
								medPhysTaken.addElement(Integer.valueOf(CMath
										.s_int(B1.getStat("PHYSDAMTAKEN"))));
								medHitPct.addElement(Double.valueOf((CMath.div(
										hits, iterations) * 100)));
								medIsHitPct.addElement(Double.valueOf((CMath
										.div(ishits, iterations) * 100)));
								medPlayerDamPct.addElement(Double
										.valueOf(100 - (CMath.div(M1.curState()
												.getHitPoints(), H1) * 100.0)));
								medPlayerManaPct.addElement(Double
										.valueOf(100 - (CMath.div(M1.curState()
												.getMana(), M1.maxState()
												.getMana()) * 100.0)));
								if (M1.amDead())
									medLossIters.addElement(Integer
											.valueOf(iterations));
								else
									medWinIters.addElement(Integer
											.valueOf(iterations));
								...
								if (c.failSkillCheck != null) {
									...
									for (int v = 0; v < V.size(); v++) {
										...
										int[] times = (int[]) c.failSkillCheck
												.get(s);
										if (times == null) {
											times = new int[2];
						...
						if (medScore.size() > 0)
							allData[charClassDex][level - levelStart][5] = medScore
									.elementAt(
											(int) Math.round(Math.floor(CMath
													.mul(0.5, medScore.size()))))
									.intValue();
						if (medHitPct.size() > 0)
							allData[charClassDex][level - levelStart][6] = medHitPct
									.elementAt(
											(int) Math.round(Math.floor(CMath
													.mul(0.5, medHitPct.size()))))
									.intValue();
						if (medLossIters.size() > 0)
							allData[charClassDex][level - levelStart][7] = medLossIters
									.elementAt(
											(int) Math.round(Math.floor(CMath
													.mul(0.75,
															medLossIters.size()))))
						...
						if (medWinIters.size() > 0)
							allData[charClassDex][level - levelStart][8] = medWinIters
									.elementAt(
											(int) Math.round(Math.floor(CMath
													.mul(0.25,
															medWinIters.size()))))
						...
						if (medPhysDone.size() > 0)
							allData[charClassDex][level - levelStart][9] = medPhysDone
									.elementAt(
											(int) Math.round(Math.floor(CMath
													.mul(0.5,
															medPhysDone.size()))))
						...
						if (medPhysTaken.size() > 0)
							allData[charClassDex][level - levelStart][10] = medPhysTaken
									.elementAt(
											(int) Math.round(Math.floor(CMath
													.mul(0.5,
															medPhysTaken.size()))))
						...
						if (medIsHitPct.size() > 0)
							allData[charClassDex][level - levelStart][11] = medIsHitPct
									.elementAt(
											(int) Math.round(Math.floor(CMath
													.mul(0.5,
															medIsHitPct.size()))))
						...
						if (medPlayerDamPct.size() > 0)
							allData[charClassDex][level - levelStart][15] = medPlayerDamPct
									.elementAt(
											(int) Math.round(Math.floor(CMath
													.mul(0.5, medPlayerDamPct
															.size()))))
									.intValue();
						if (medPlayerManaPct.size() > 0)
							allData[charClassDex][level - levelStart][16] = medPlayerManaPct
									.elementAt(
											(int) Math.round(Math.floor(CMath
													.mul(0.5, medPlayerManaPct
															.size()))))
									.intValue();
						...
						if (fileExp == null) {
							...
							mob.tell("QUICKEST : " + bestIterScore[0] + ": "
									+ bestIterSkill[0]);
							mob.tell("MOST DAM : " + bestHitScore[0] + ": "
									+ bestHitSkill[0]);
							mob.tell("BEST HIT : " + bestSingleHitScore[0]
									+ ", Phys: " + bestSingleHitPhys[0]
									+ ", Skill: " + bestSingleHitSkill[0]);
							mob.tell("MEDIANS  : HITS: "
									+ allData[charClassDex][level - levelStart][5]
									+ " ("
									+ allData[charClassDex][level - levelStart][6]
									+ "%), LOSS ITERS: "
									+ allData[charClassDex][level - levelStart][7]
									+ ", WIN ITERS: "
									+ allData[charClassDex][level - levelStart][8]);
							mob.tell("MEDIANS  : PHYS DONE: "
									+ allData[charClassDex][level - levelStart][9]
									+ ", PHYS TAKEN: "
									+ allData[charClassDex][level - levelStart][10]
									+ " ("
									+ allData[charClassDex][level - levelStart][11]
									+ "%)");
							mob.tell("LOSSES   : " + losses[0]);
							if ((c.failSkillCheck != null)
									&& (c.failSkillCheck.size() > 0)) {
								StringBuffer fails = new StringBuffer(
										"SKILLFAILS: ");
								for (Enumeration i = c.failSkillCheck.keys(); i
										.hasMoreElements();) {
			...
			if (file.canWrite()) {
				...
				for (int d = 0; d < c.allData[0][0].length; d++) {
					...
					for (int charClassDex = 0; charClassDex < c.classSet.size(); charClassDex++) {
						CharClass C = (CharClass) c.classSet.elementAt(
								charClassDex, 1);
						buf.append(C.ID()).append("\t").append(C.baseClass())
								.append("\t");
						for (int level = c.levelStart; level <= c.levelEnd; level += c.skipLevels) {
							buf.append(
									c.allData[charClassDex][level
											- c.levelStart][d]).append('\t');
							// for(int
							// i=0;i<allSkills[charClassDex][level-levelStart].length;i++)
					...
					for (int b = 0; b < baseClasses.size(); b++) {
						...
						double ct = 0;
						for (int charClassDex = 0; charClassDex < c.classSet
								.size(); charClassDex++)
							if (((CharClass) c.classSet.elementAt(charClassDex,
									1)).baseClass().equalsIgnoreCase(baseClass)) {
								ct += 1.0;
								for (int level = c.levelStart; level <= c.levelEnd; level += c.skipLevels)
									levels[level] += c.allData[charClassDex][level
											- c.levelStart][d];
							}
						if (ct > 0) {
							buf.append(baseClass).append("\t")
									.append(baseClass).append("\t");
							for (int level = c.levelStart; level <= c.levelEnd; level += c.skipLevels)
								buf.append(
										Math.round(Integer.valueOf(
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((C == null)
				&& (createNewOnly || (ClassName.toUpperCase().indexOf("ALL") < 0))) {
			...
			C = CMClass.findCharClass(ClassName);
			if ((C == null)
					&& (createNewOnly || (ClassName.toUpperCase()
							.indexOf("ALL") < 0)))
				return false;
		}
		...
		if (level <= 0) {
			try {
				level = CMath.s_int(mob.session().prompt(
						"Enter a level (1-"
								+ CMProps
