...
public class Merge extends StdCommand {
	...
	public static String getStat(Environmental E, String stat) {
		if ((stat != null) && (stat.length() > 0)
				&& (stat.equalsIgnoreCase("REJUV")) && (E instanceof Physical)) {
	...
	protected static boolean tryMerge(MOB mob, Room room, Environmental E,
			List things, List<String> changes, List<String> onfields,
			List<String> ignore, boolean noisy) {
		...
		for (int t = 0; t < things.size(); t++) {
			...
			if (noisy)
				mergedebugtell(mob,
						E.name() + "/" + E2.name() + "/" + CMClass.classID(E)
								+ "/" + CMClass.classID(E2));
			if (CMClass.classID(E).equals(CMClass.classID(E2))) {
				...
				if (noisy)
					mergedebugtell(
							mob,
							"fieldsToCheck-"
									+ CMParms.toStringList(fieldsToCheck));
				if (checkedOut)
				...
				if (checkedOut) {
					...
					if (noisy)
						mergedebugtell(
								mob,
								"fieldsToChange-"
										+ CMParms.toStringList(fieldsToChange));
	...
	public void sortEnumeratedList(Enumeration e, List<String> allKnownFields,
			StringBuffer allFieldsMsg) {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((commands.size() > 0)
			...
			sortEnumeratedList(CMClass.basicItems(), allKnownFields,
					allFieldsMsg);
			...
			sortEnumeratedList(CMClass.clanItems(), allKnownFields,
					allFieldsMsg);
			sortEnumeratedList(CMClass.miscMagic(), allKnownFields,
					allFieldsMsg);
		...
		StringBuffer buf = new CMFile(filename, mob, CMFile.FLAG_LOGERRORS)
				.text();
		...
		if ((buf.length() > 20)
			...
			if (mob.session() != null)
				mob.session().rawPrint(
						"Unpacking mobs from file: '" + filename + "'...");
			...
			if (mob.session() != null)
				mob.session().rawPrint(
						"Unpacking items from file: '" + filename + "'...");
			...
			while (x >= 0) {
				...
				if (s.length() > 0) {
					...
					if (allKnownFields.contains(s))
						...
						mob.tell("'"
								+ s
								+ "' is an unknown field name.  Valid fields include: "
		...
		if ((onfields.size() == 0) && (ignore.size() == 0)
				&& (changes.size() == 0)) {
		...
		for (int i = placesToDo.size() - 1; i >= 0; i--) {
			...
			} else if (placesToDo.elementAt(i) instanceof Room)
				if (mob.session() != null)
					mob.session().rawPrint(".");
				else
					return false;
		...
		if (noisy)
			mergedebugtell(mob,
					"Change fields=" + CMParms.toStringList(changes));
				...
				if (aremobs) {
					for (int m = 0; m < R.numInhabitants(); m++) {
						...
						if ((M != null) && (M.isSavable()))
							if (tryMerge(mob, R, M, things, changes, onfields,
									ignore, noisy))
								savemobs = true;
					}
				} else {
					for (int i = 0; i < R.numItems(); i++) {
						Item I = R.getItem(i);
						if ((I != null)
								&& (tryMerge(mob, R, I, things, changes,
										onfields, ignore, noisy)))
							saveitems = true;
					}
					for (int m = 0; m < R.numInhabitants(); m++) {
						MOB M = R.fetchInhabitant(m);
						if ((M != null) && (M.isSavable())) {
							for (int i = 0; i < M.numItems(); i++) {
								Item I = M.getItem(i);
								if ((I != null)
										&& (tryMerge(mob, R, I, things,
												changes, onfields, ignore,
												noisy)))
									savemobs = true;
							}
	...
	private static final SHashtable<String, CMClass.CMObjectType> OBJECT_TYPES = new SHashtable<String, CMClass.CMObjectType>(
			new Object[][] { { "MOBS", CMClass.CMObjectType.MOB },
	...
	private boolean amMerging(CMClass.CMObjectType doType,
			MaskingLibrary.CompiledZapperMask mask, Environmental E) {
	...
	public boolean dbMerge(MOB mob, String name, Modifiable dbM, Modifiable M,
			Set<String> ignores) throws java.io.IOException, CMException {
		if ((M instanceof Physical) && (dbM instanceof Physical)) {
			...
			if (CMLib.flags().isCataloged(PM)) {
				...
				PM.basePhyStats().setDisposition(
						CMath.unsetb(PM.basePhyStats().disposition(),
								PhyStats.IS_CATALOGED));
			}
			if (CMLib.flags().isCataloged(dbPM))
				dbPM.basePhyStats().setDisposition(
						CMath.unsetb(dbPM.basePhyStats().disposition(),
								PhyStats.IS_CATALOGED));
			PM.image();
			dbPM.image();
		}
		...
		while (!ok) {
			...
			for (int i = 0; i < statCodes.length; i++) {
				...
				if (ignores.contains(statCode)
						|| ((M instanceof MOB) && statCode
								.equalsIgnoreCase("INVENTORY")))
					continue;
				String promptStr = CMStrings.capitalizeAndLower(M
						.getStatCodes()[i]);
				...
				String res = mob
						.session()
						.choose("D)atabase Value, E)dit Value, or N)o Change, or Q)uit All: ",
								"DENQ", "N");
				...
				M.setStat(
						statCode,
						CMLib.genEd().prompt(mob, M.getStat(statCode),
								++showNumber, showFlag, promptStr));
			}
			if (showNumber == 0)
				return didSomething;
			if (showFlag < -900) {
	...
	public boolean doArchonDBCompare(MOB mob, String scope, String firstWord,
			Vector commands) throws java.io.IOException {
		...
		int dbPingIntMins = CMParms.getParmInt(theRest, "DBPINGINTERVALMINS",
				30);
		...
		Set<String> ignores = new SHashSet(CMParms.parseCommas(
				ignore.toUpperCase(), true));
		MaskingLibrary.CompiledZapperMask mask = CMLib.masking().maskCompile(
				maskStr);
		if (dbClass.length() == 0) {
		...
		dbConnector = new DBConnector(dbClass, dbService, dbUser, dbPass,
				dbConns, dbPingIntMins, dbReuse, false, false);
		...
		if (scope.equalsIgnoreCase("AREA"))
			rooms.addAll(Arrays.asList(dbInterface.DBReadRoomObjects(mob
					.location().getArea().Name(), false)));
		else if (scope.equalsIgnoreCase("ROOM")) {
			Room R = dbInterface.DBReadRoomObject(mob.location().roomID(),
					false);
			if (R != null)
				rooms.add(R);
		} else
			for (Enumeration<Area> e = CMLib.map().areas(); e.hasMoreElements();)
				rooms.addAll(Arrays.asList(dbInterface.DBReadRoomObjects(e
						.nextElement().Name(), false)));
		if (rooms.size() == 0) {
		...
		try {
			for (Room dbR : rooms) {
				...
				synchronized (("SYNC" + dbR.roomID()).intern()) {
					...
					for (Enumeration<MOB> e = dbR.inhabitants(); e
							.hasMoreElements();)
					...
					for (MOB dbM : mobSet) {
						...
						MOB M = null;
						int ctr = ct;
						for (Enumeration<MOB> m = R.inhabitants(); m
								.hasMoreElements();) {
							MOB M1 = m.nextElement();
							if (M1.Name().equalsIgnoreCase(dbM.Name())
									&& ((--ctr) <= 0)) {
								M = M1;
								break;
							}
						...
						if (M == null) {
							if (amMerging(doType, mask, dbM)
								...
								if (mob.session()
									...
									doneM.add(M);
									updateMobs = true;
									Log.sysOut("Merge", mob.Name()
											+ " added mob " + dbR.roomID()
											+ "." + rName);
								}
							}
						} else {
							...
							if (amMerging(doType, mask, dbM)) {
								if (!dbM.sameAs(M)) {
									...
									if ((dbMerge(mob, "^MMOB " + dbR.roomID()
											+ "." + rName + "^N", dbM, M,
											ignores))
											&& (!oldM.sameAs(M))) {
										Log.sysOut("Merge",
												mob.Name() + " modified mob "
														+ dbR.roomID() + "."
														+ rName);
										updateMobs = true;
									}
							...
							for (Enumeration<Item> e = dbM.items(); e
									.hasMoreElements();)
							...
							for (Item dbI : itemSet) {
								...
								Item I = null;
								ctr = ict;
								for (Enumeration<Item> i = M.items(); i
										.hasMoreElements();) {
									Item I1 = i.nextElement();
									if (I1.Name().equalsIgnoreCase(dbI.Name())
											&& ((--ctr) <= 0)) {
										I = I1;
										break;
									}
								...
								if (I == null) {
									if (amMerging(doType, mask, dbI)
										...
										if (mob.session()
											...
											M.addItem(I);
											doneI.add(I);
											Item cI = (dbI.container() == null) ? null
													: M.findItem(dbI
															.container().Name());
					...
					for (Enumeration<MOB> r = R.inhabitants(); r
							.hasMoreElements();) {
						MOB M = r.nextElement();
						if (amMerging(doType, mask, M) && (!doneM.contains(M))
								&& (M.isMonster())
								&& (!ignore.contains("EXTRA"))) {
							if (mob.session()
									.confirm(
					...
					for (Item dbI : itemSet) {
						...
						Item I = null;
						int ctr = ct;
						for (Enumeration<Item> i = R.items(); i
								.hasMoreElements();) {
							Item I1 = i.nextElement();
							if (I1.Name().equalsIgnoreCase(dbI.Name())
									&& ((--ctr) <= 0)) {
						...
						if (I == null) {
							if (amMerging(doType, mask, dbI)
								...
								if (mob.session()
									...
									doneI.add(I);
									Item cI = (dbI.container() == null) ? null
											: R.findItem(dbI.container().Name());
									if (cI instanceof Container)
										I.setContainer((Container) cI);
									updateItems = true;
									Log.sysOut("Merge", mob.Name()
											+ " added item " + dbR.roomID()
											+ "." + rName);
								}
							}
						} else if (amMerging(doType, mask, dbI)) {
							...
							if (!dbI.sameAs(I)) {
								Item oldI = (Item) I.copyOf();
								if ((dbMerge(mob, "^IITEM " + dbR.roomID()
										+ "." + rName + "^N", dbI, I, ignores))
										&& (!oldI.sameAs(I))) {
									Log.sysOut("Merge", mob.Name()
											+ " modified item " + dbR.roomID()
											+ "." + rName);
									updateItems = true;
								}
							}
					...
					for (Enumeration<Item> i = R.items(); i.hasMoreElements();) {
						Item I = i.nextElement();
						if (amMerging(doType, mask, I) && (!doneI.contains(I))
								&& (!ignore.contains("EXTRA"))) {
							if (mob.session()
									.confirm(
											"Item: "
