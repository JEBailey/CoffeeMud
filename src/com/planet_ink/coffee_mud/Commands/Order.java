...
public class Order extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (whomToOrder.toUpperCase().endsWith(".ALL")) {
			allFlag = true;
			whomToOrder = "ALL "
					+ whomToOrder.substring(0, whomToOrder.length() - 4);
		}
		int addendum = 1;
		String addendumStr = "";
		...
		if (V.size() == 0) {
			if (whomToOrder.equalsIgnoreCase("ALL"))
				mob.tell("You don't see anyone called '" + whomToOrder
						+ "' here.");
			else
		...
		if (!CMSecurity
				.isAllowed(mob, mob.location(), CMSecurity.SecFlag.ORDER)) {
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (!CMSecurity.isAllowed(mob, mob.location(),
				...
				if ((O instanceof Command)
						&& ((!((Command) O).canBeOrdered()) || (!((Command) O)
								.securityCheck(mob)))) {
					mob.tell("You can't order " + target.name(mob) + " to '"
							+ order + "'.");
					continue;
				}
				...
				if (O instanceof Ability) {
					if (CMath.bset(((Ability) O).flags(),
							Ability.FLAG_NOORDERING)) {
						mob.tell("You can't order " + target.name(mob)
								+ " to '" + order + "'.");
						continue;
					}
				}
			...
			if ((!CMLib.flags().canBeSeenBy(target, mob))
				...
				CMMsg msg = CMClass.getMsg(mob, target, null, CMMsg.MSG_SPEAK,
						CMMsg.MSG_ORDER, CMMsg.MSG_SPEAK,
						"^T<S-NAME> order(s) <T-NAMESELF> to '" + order
								+ "'^?.");
		...
		for (int v = 0; v < doV.size(); v++) {
			...
			target.enqueCommand((List) commands.clone(), metaFlags
