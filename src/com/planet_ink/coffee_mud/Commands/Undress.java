		...
		if (target.willFollowOrdersOf(mob)
			...
			if ((item == null) || (!CMLib.flags().canBeSeenBy(item, mob))
				...
				mob.tell(target.name(mob)
						+ " doesn't seem to be equipped with '" + what + "'.");
				return false;
			}
			...
			if (mob.location().okMessage(mob, msg)) {
				msg = CMClass.getMsg(target, item, null, CMMsg.MASK_ALWAYS
						| CMMsg.MSG_REMOVE, CMMsg.MSG_REMOVE, CMMsg.MSG_REMOVE,
						null);
				if (mob.location().okMessage(mob, msg)) {
					mob.location().send(mob, msg);
					msg = CMClass.getMsg(target, item, null, CMMsg.MASK_ALWAYS
							| CMMsg.MSG_DROP, CMMsg.MSG_DROP, CMMsg.MSG_DROP,
							null);
					if (mob.location().okMessage(mob, msg)) {
						mob.location().send(mob, msg);
						if (CMLib.commands().postGet(mob, null, item, true))
							mob.location()
									.show(mob,
											...
											"<S-NAME> take(s) <O-NAME> off <T-NAMESELF>.");
					} else
						mob.tell("You cannot seem to get " + item.name()
								+ " off " + target.name(mob) + ".");
				} else
					mob.tell("You cannot seem to get " + item.name()
							+ " off of " + target.name(mob) + ".");
			}
		} else
