...
public class LineWrap extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() < 2) {
			String wrap = (mob.playerStats().getWrap() != 0) ? ("" + mob
					.playerStats().getWrap()) : "Disabled";
			mob.tell("Change your line wrap to what? Your current line wrap setting is: "
					+ wrap + ". Enter a number larger than 10 or 'disable'.");
			return false;
		}
		...
		if ((CMath.isInteger(newWrap)) && (CMath.s_int(newWrap) > 10))
			...
			mob.tell("'"
					+ newWrap
					+ "' is not a valid setting. Enter a number larger than 10 or 'disable'.");
			return false;
		}
