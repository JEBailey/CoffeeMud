...
public class Auction extends Channel implements Tickable {
	...
	protected final static String MESSAGE_NOAUCTION() {
		return "There is not currently a live auction.  Use AUCTION UP syntax to add one, or visit an auctioneer for a long auction.";
	}

	public String liveAuctionStatus() {
		if (liveData.auctioningI != null) {
			String bidWords = CMLib.beanCounter().nameCurrencyShort(
					liveData.currency, liveData.bid);
			if (bidWords.length() == 0)
				bidWords = "0";
			return "Up for live auction: " + liveData.auctioningI.name()
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((tickID == Tickable.TICKID_LIVEAUCTION)
			...
			if ((liveData.state == STATE_START)
					&& ((System.currentTimeMillis() - liveData.start) < (5 * 15000))) {
			...
			case STATE_RUNOUT:
				V.addElement("The live auction for "
						+ liveData.auctioningI.name()
						+ " is almost done. The current bid is "
						+ CMLib.beanCounter().nameCurrencyShort(
								liveData.currency, liveData.bid) + ".");
				break;
			case STATE_ONCE:
				V.addElement(CMLib.beanCounter().nameCurrencyShort(
						liveData.currency, liveData.bid)
						+ " for "
						+ liveData.auctioningI.name()
						+ " going ONCE!");
			...
			case STATE_TWICE:
				...
				break;
			case STATE_THREE:
				V.addElement(liveData.auctioningI.name()
						+ " going for "
						+ CMLib.beanCounter().nameCurrencyShort(
								liveData.currency, liveData.bid)
						+ "! Last chance!");
			...
			case STATE_CLOSED: {
				if ((winnerM != null) && (winnerM != liveData.auctioningM)) {
					...
					if (liveData.auctioningI != null) {
						...
						winnerM.location().moveItemTo(liveData.auctioningI,
								ItemPossessor.Expire.Player_Drop);
						double houseCut = Math.floor(liveData.bid
								* aRates.liveCutPct);
						double finalAmount = liveData.bid - houseCut;
						CMLib.coffeeShops().returnMoney(winnerM,
								liveData.currency,
								liveData.highBid - liveData.bid);
						CMLib.coffeeShops().returnMoney(auctioneerM,
								liveData.currency, finalAmount);
						auctioneerM
								.tell(CMLib.beanCounter().nameCurrencyShort(
										liveData.currency, finalAmount)
										+ " has been transferred to you as payment from "
										...
										+ CMLib.beanCounter()
												.nameCurrencyShort(
														liveData.currency,
														houseCut)
										+ ".  The goods have also been transferred "
										+ "in exchange.");
						if (CMLib.commands().postGet(winnerM, null,
								liveData.auctioningI, false)
								|| (winnerM.isMine(liveData.auctioningI))) {
							winnerM.tell(CMLib.beanCounter().nameCurrencyShort(
									liveData.currency, liveData.bid)
							...
							if (liveData.auctioningI instanceof LandTitle) {
								CMMsg msg = CMClass.getMsg(auctioneerM,
										winnerM, liveData.auctioningI,
										CMMsg.MASK_ALWAYS | CMMsg.TYP_GIVE,
										null);
							...
							auctioneerM
									.tell("Your transaction could not be completed because "
											+ winnerM.name(auctioneerM)
											+ " was unable to collect the item.  Please contact "
											+ winnerM.name(auctioneerM)
											+ " about receipt of "
											+ liveData.auctioningI
													.name(winnerM)
											+ " for "
											+ CMLib.beanCounter()
													.nameCurrencyShort(
															liveData.currency,
															liveData.bid) + ".");
							winnerM.tell("Your transaction could not be completed because you were unable to collect the item.  Please contact "
									+ auctioneerM.name(winnerM)
									+ " about receipt of "
									+ liveData.auctioningI.name(winnerM)
									+ " for "
									+ CMLib.beanCounter().nameCurrencyShort(
											liveData.currency, liveData.bid)
									+ ".");
				...
				} else if (!auctioneerM.isMine(liveData.auctioningI))
					auctioneerM.moveItemTo(liveData.auctioningI);
	...
	public boolean doLiveAuction(MOB mob, Vector commands, Environmental target) {
		...
		if (target != null) {
			...
			liveData.currency = CMLib.english()
					.numPossibleGoldCurrency(mob, sb);
			...
			V.addElement("New live auction: " + liveData.auctioningI.name()
					+ ".  The opening bid is " + bidWords + ".");
			...
			Object[] bidObjs = CMLib.english().parseMoneyStringSDL(mob, sb,
					liveData.currency);
			String currency = (String) bidObjs[0];
			double amt = CMath.mul(((Double) bidObjs[1]).doubleValue(),
					((Long) bidObjs[2]).doubleValue());
			String[] resp = CMLib.coffeeShops().bid(mob, amt, currency,
					liveData, liveData.auctioningI, V);
			if (resp != null) {
				if (resp[0] != null)
					mob.tell(resp[0]);
			...
			if ((V.size() > 2) && (liveData.auctioningM != null))
				liveData.auctioningM.doCommand(V, Command.METAFLAG_FORCED);
		}
		return true;
	}
	...
	public void auctionNotify(MOB M, String resp, String regardingItem)
		...
		if (CMLib.flags().isInTheGame(M, true))
			...
			CMLib.smtp().emailIfPossible(
					...
					"noreply@"
							+ CMProps.getVar(CMProps.Str.MUDDOMAIN)
									.toLowerCase(), M.playerStats().getEmail(),
		...
		if (cmd.equals("LIST")) {
			...
			if ((commands.size() >= 2)
					&& ((CMLib.english().numPossibleGold(mob,
							(String) commands.lastElement()) > 0) || (((String) commands
							.lastElement()).equals("0")))) {
			...
			if ((E instanceof Container)
					&& (((Container) E).getContents().size() > 0)) {
			...
			String depositAmt = CMLib.beanCounter().nameCurrencyLong(mob,
					deposit);
			...
			if (liveData.highBid > 0.0)
				CMLib.coffeeShops().returnMoney(liveData.highBidderM,
						liveData.currency, liveData.highBid);
			liveData.auctioningM = null;
			liveData.auctioningI = null;
			...
			CMLib.commands().handleBeingLookedAt(
					CMClass.getMsg(mob, CMMsg.MASK_ALWAYS | CMMsg.MSG_EXAMINE,
							null));
