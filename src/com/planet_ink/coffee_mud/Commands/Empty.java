	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() > 1) {
			...
			if (s.equalsIgnoreCase("here"))
				...
				target = CMLib.english().possibleContainer(mob, commands,
						false, Wearable.FILTER_UNWORNONLY);
				if (target == null)
		...
		else if ((V.size() == 1) && (V.firstElement() instanceof Drink)
				&& (!((Drink) V.firstElement()).containsDrink()))
			mob.tell(mob, (Drink) V.firstElement(), null,
					"<T-NAME> is already empty.");
		else
			for (int v = 0; v < V.size(); v++) {
				...
				if ((C instanceof Drink) && (((Drink) C).containsDrink())) {
					if (target instanceof Drink) {
						...
						C2.execute(mob, new XVector("POUR", "$" + C.Name()
								+ "$", "$" + target.Name() + "$"), metaFlags);
						skipMessage = true;
					} else {
				...
				CMMsg msg = CMClass
						.getMsg(mob, C, CMMsg.MSG_QUIETMOVEMENT, str);
				Room R = mob.location();
				if (skipMessage || (R.okMessage(mob, msg))) {
					...
					for (int v2 = 0; v2 < V2.size(); v2++) {
						...
						if (((I.container() == null) || (Get.get(mob, C, I,
								true, null, true))) && (I.container() == null)) {
							if (target instanceof Room)
								drop(mob, I, true, true, false);
							else if (target instanceof Container) {
								CMMsg putMsg = CMClass.getMsg(mob, target, I,
