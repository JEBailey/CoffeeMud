...
public class Equipment extends StdCommand {
	...
	public static StringBuilder getEquipment(MOB seer, MOB mob,
			boolean allPlaces) {
		...
		for (int l = 0; l < codes.all_ordered().length; l++) {
			...
			if (paragraphView)
				...
				header = "^N(^H" + wornName + "^?)";
				header += CMStrings.SPACES.substring(0,
						headWrap - header.length())
						+ ": ^!";
			}
			List<Item> wornHere = mob.fetchWornItems(wornCode,
					(short) (Short.MIN_VALUE + 1), (short) 0);
			int shownThisLoc = 0;
					...
					if ((thisItem.container() == null)
						...
						if (CMLib.flags().canBeSeenBy(thisItem, seer)) {
							...
							msg.append(header
									...
									+ CMLib.flags().colorCodes(thisItem, seer)
											.toString().trim() + "^?\n\r");
							shownThisLoc++;
						}
					}
			...
			if (emptySlots > 0) {
				...
				int numTattoosToShow = (int) Math.round(Math.ceil(CMath.mul(
						numTattoosTotal, CMath.div(emptySlots, numLocations))));
				for (Enumeration<MOB.Tattoo> e = mob.tattoos(); e
						.hasMoreElements();) {
					MOB.Tattoo T = e.nextElement();
					if ((T.tattooName.startsWith(wornName + ":"))
						...
						if (paragraphView) {
							...
							tat = CMStrings.capitalizeAndLower(tat.substring(
									wornName.length() + 1).toLowerCase());
			...
			if (paragraphView) {
				if (longView)
					session.wraplessPrintln("You are wearing "
							+ getEquipment(mob, mob, true));
				else
					session.wraplessPrintln("You are wearing "
							+ getEquipment(mob, mob, false));
			} else if (longView)
				session.wraplessPrintln("You are wearing:\n\r"
						+ getEquipment(mob, mob, true));
			else
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((commands.size() == 1) && (commands.firstElement() instanceof MOB)) {
			commands.addElement(getEquipment((MOB) commands.firstElement(),
					mob, false));
			return true;
		}
		viewEquipment(
