...
public class Take extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (CMSecurity.isAllowed(mob, mob.location(), CMSecurity.SecFlag.ORDER)
			...
			if (commands.size() < 2) {
				mob.tell("From whom should I take the "
						+ (String) commands.elementAt(0));
				return false;
			}

			MOB victim = mob.location().fetchInhabitant(
					(String) commands.lastElement());
			if ((victim == null) || (!CMLib.flags().canBeSeenBy(victim, mob))) {
				mob.tell("I don't see anyone called "
						+ (String) commands.lastElement() + " here.");
				return false;
			}
			if ((!victim.isMonster())
			...
			int maxToGive = CMLib.english().calculateMaxToGive(mob, commands,
					true, victim, false);
			if (maxToGive < 0)
				return false;
			...
			if (thingToGive.toUpperCase().endsWith(".ALL")) {
				allFlag = true;
				thingToGive = "ALL "
						+ thingToGive.substring(0, thingToGive.length() - 4);
			}
			...
			if ((thingToGive.equalsIgnoreCase("qp"))
				...
				if ((victim.getQuestPoint() <= 0)
						|| (victim.getQuestPoint() < numToTake)) {
					if (victim.getQuestPoint() <= 0)
						mob.tell(victim.name() + " has no quest points!");
					else
						mob.tell(victim.name() + " has only "
								+ victim.getQuestPoint() + " quest points!");
					return false;
				}
				mob.tell("You silently take " + numToTake
						+ " quest points from " + victim.name() + ".");
			...
			while (doBugFix || ((allFlag) && (addendum <= maxToGive))) {
				doBugFix = false;
				Environmental giveThis = CMLib.english().bestPossibleGold(
						victim, null, thingToGive);

				if (giveThis != null) {
					if (((Coins) giveThis).getNumberOfCoins() < CMLib.english()
							.numPossibleGold(victim, thingToGive))
						return false;
					allFlag = false;
				} else
					giveThis = victim.fetchItem(null,
							Wearable.FILTER_UNWORNONLY, thingToGive
									+ addendumStr);
				if ((giveThis == null) && (V.size() == 0)
			...
			if (V.size() == 0)
				...
				for (int i = 0; i < V.size(); i++) {
					...
					CMMsg newMsg = CMClass.getMsg(victim, mob, giveThis,
							CMMsg.MASK_ALWAYS | CMMsg.MSG_GIVE,
							"<T-NAME> take(s) <O-NAME> from <S-NAMESELF>.");
