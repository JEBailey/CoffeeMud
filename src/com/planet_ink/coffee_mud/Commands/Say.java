...
public class Say extends StdCommand {
	...
	private final String[] access = { "SAY", "ASK", "`", "SA", "SAYTO" };

	public String[] getAccessWords() {
		return access;
	}

	protected static final String[] impossibleTargets = { "HERE", "THERE",
			"IS", "JUST", "A", "AN", "TO", "THE", "SOME", "SITS", "RESTS",
			"LEFT", "HAS", "BEEN" };

	protected void gmcpSaySend(String sayName, MOB mob, Environmental target,
			CMMsg msg) {
		if ((mob.session() != null)
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (((String) commands.elementAt(0)).equalsIgnoreCase("ASK"))
			theWord = "Ask";
		else if (((String) commands.elementAt(0)).equalsIgnoreCase("YELL"))
			theWord = "Yell";
		else if (((String) commands.elementAt(0)).equalsIgnoreCase("SAYTO")
			...
			if (whom.equalsIgnoreCase("self"))
				...
				if ((!toFlag) && (target != null)) {
					if (!(target instanceof MOB))
						target = null;
					else if (target.name().toUpperCase()
							.indexOf(whom.toUpperCase()) < 0)
						target = null;
					else if ((!target.name().equalsIgnoreCase(whom))
							&& (whom.length() < 4))
						target = null;
				}
		...
		if (!CMSecurity.isDisabled(CMSecurity.DisFlag.AUTOLANGUAGE)) {
			...
			// you are addressing.
			if ((langTarget == null) && (target == null)
					&& (R.numInhabitants() == 2)) {
		...
		if (toFlag
				&& ((target == null) || (!CMLib.flags()
						.canBeSeenBy(target, mob)))) {
		...
		if ((!theWord.equalsIgnoreCase("ASK")) && (target != null))
			...
			theWord += "(s)";
		String fromSelf = "^T^<SAY \""
				+ CMStrings.removeColors((target != null) ? target.name() : mob
						.name()) + "\"^><S-NAME> " + theWord.toLowerCase()
				+ theWordSuffix + " <T-NAMESELF> '" + combinedCommands
				+ "'^</SAY^>^?";
		String toTarget = "^T^<SAY \"" + CMStrings.removeColors(mob.name())
				+ "\"^><S-NAME> " + theWord.toLowerCase() + theWordSuffix
				+ " <T-NAMESELF> '" + combinedCommands + "'^</SAY^>^?";
		if (target == null)
			msg = CMClass.getMsg(mob, null, null, CMMsg.MSG_SPEAK, "^T^<SAY \""
					+ CMStrings.removeColors(mob.name()) + "\"^><S-NAME> "
					+ theWord.toLowerCase() + theWordSuffix + " '"
					+ combinedCommands + "'^</SAY^>^?");
		else
			msg = CMClass.getMsg(mob, target, null, CMMsg.MSG_SPEAK, fromSelf,
					toTarget, fromSelf);
		...
		if (R.okMessage(mob, msg)) {
			...
			if (theWord.toUpperCase().startsWith("YELL")) {
				...
				for (Room R2 : yellRooms) {
					...
					final String inDirName = (dirCode < 0) ? ""
							: (useShipDirs ? Directions
									.getShipInDirectionName(opDirCode)
									: Directions.getInDirectionName(opDirCode));
					msg = CMClass.getMsg(mob, null, null, CMMsg.MSG_SPEAK,
							"^TYou hear someone yell '" + combinedCommands
									+ "' " + inDirName + "^?");
