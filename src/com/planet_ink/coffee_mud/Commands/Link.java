...
public class Link extends At {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (thisRoom == null) {
			thisRoom = CMLib.map().findWorldRoomLiberally(mob, RoomID, "R",
					100, 120000);
			if (thisRoom == null) {
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"Suddenly a portal opens up in the landscape.\n\r");
		Log.sysOut("Link", mob.Name() + " linked "
				+ CMLib.map().getExtendedRoomID(mob.location()) + " to room "
				+ CMLib.map().getExtendedRoomID(thisRoom) + ".");
		return false;
	}
	...
	protected void exitifyNewPortal(MOB mob, Room room, int direction) {
		...
		if ((reverseRoom != null)
				&& ((reverseRoom == mob.location()) || (reverseRoom == mob
						.location().getGridParent())))
		...
		GridLocale hereGL = (mob.location().getGridParent() != null) ? mob
				.location().getGridParent() : null;
		int hereX = (hereGL != null) ? hereGL.getGridChildX(mob.location())
				: -1;
		int hereY = (hereGL != null) ? hereGL.getGridChildY(mob.location())
				: -1;
		GridLocale thereGL = (room.getGridParent() != null) ? room
				.getGridParent() : null;
		int thereX = (thereGL != null) ? thereGL.getGridChildX(room) : -1;
		int thereY = (thereGL != null) ? thereGL.getGridChildY(room) : -1;
		if (hereGL != null) {
			for (Iterator<WorldMap.CrossExit> hereIter = hereGL.outerExits(); hereIter
					.hasNext();) {
				CE = hereIter.next();
				if ((CE.out) && (CE.dir == direction) && (CE.x == hereX)
						&& (CE.y == hereY))
					hereGL.delOuterExit(CE);
			}
			CE = WorldMap.CrossExit.make(hereX, hereY, direction, CMLib.map()
					.getExtendedRoomID(room), true);
		...
		if (thereGL != null) {
			for (Iterator<WorldMap.CrossExit> thereIter = thereGL.outerExits(); thereIter
					.hasNext();) {
				CE = thereIter.next();
				if ((!CE.out)
						&& (CE.dir == direction)
						&& (CE.destRoomID.equals(CMLib.map().getExtendedRoomID(
								mob.location()))))
					thereGL.delOuterExit(CE);
			}
			CE = WorldMap.CrossExit.make(thereX, thereY, direction, CMLib.map()
					.getExtendedRoomID(mob.location()), false);
			thereGL.addOuterExit(CE);

			if ((room.rawDoors()[opDir] == null)
					|| (thereGL == room.rawDoors()[opDir])
					|| (thereGL.isMyGridChild(room.rawDoors()[opDir]))) {
				for (Iterator<WorldMap.CrossExit> thereIter = thereGL
						.outerExits(); thereIter.hasNext();) {
					CE = thereIter.next();
					if ((CE.out) && (CE.dir == opDir) && (CE.x == thereX)
							&& (CE.y == thereY))
						thereGL.delOuterExit(CE);
				}
				CE = WorldMap.CrossExit.make(thereX, thereY, opDir, CMLib.map()
						.getExtendedRoomID(mob.location()), true);
				thereGL.addOuterExit(CE);
				if (hereGL != null) {
					room.rawDoors()[opDir] = hereGL;
					for (Iterator<WorldMap.CrossExit> hereIter = hereGL
							.outerExits(); hereIter.hasNext();) {
						CE = hereIter.next();
						if ((!CE.out)
								&& (CE.dir == opDir)
								&& (CE.destRoomID.equals(CMLib.map()
										.getExtendedRoomID(room))))
							hereGL.delOuterExit(CE);
					}
					CE = WorldMap.CrossExit.make(hereX, hereY, opDir, CMLib
							.map().getExtendedRoomID(room), false);
					hereGL.addOuterExit(CE);
				} else
			...
			if (hereGL != null) {
				...
				for (Iterator<WorldMap.CrossExit> hereIter = hereGL
						.outerExits(); hereIter.hasNext();) {
					CE = hereIter.next();
					if ((!CE.out) && (CE.dir == opDir)
							&& (CE.destRoomID.equals(room.roomID())))
						hereGL.delOuterExit(CE);
				}
				CE = WorldMap.CrossExit.make(hereX, hereY, opDir, CMLib.map()
						.getExtendedRoomID(room), false);
				hereGL.addOuterExit(CE);
			} else
				room.rawDoors()[opDir] = mob.location();
			room.setRawExit(opDir, thisExit);
