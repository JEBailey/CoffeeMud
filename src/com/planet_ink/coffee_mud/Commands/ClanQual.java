...
public class ClanQual extends StdCommand {
	...
	public boolean execute(final MOB mob, Vector commands, int metaFlags)
			...
			for (Pair<Clan, Integer> c : mob.clans())
				if ((clanName.length() == 0)
						...
						&& (c.first.getAuthority(c.second.intValue(),
								Clan.Function.PREMISE) != Authority.CAN_NOT_DO)) {
					chkC = c.first;
					break;
				}
		...
		final Clan C = chkC;
		if (C == null) {
			mob.tell("You aren't allowed to set qualifications for "
		...
		if ((!skipChecks)
				&& (!CMLib.clans().goForward(mob, C, commands,
						Clan.Function.PREMISE, false))) {
			mob.tell("You aren't in the right position to set the qualifications to your "
					+ C.getGovernmentName() + ".");
			return false;
		}
		...
		IC[0] = new InputCallback(InputCallback.Type.PROMPT, "", 0) {
			...
			public void showPrompt() {
				session.promptPrint("Describe your " + C.getGovernmentName()
						+ "'s Qualification Code (?)\n\r: ");
			}
			...
			public void callBack() {
				...
				session.prompt(new InputCallback(InputCallback.Type.CHOOSE,
						"Y", "YN\n", 0) {
					@Override
					public void showPrompt() {
						session.println("Your qualifications will be as follows: "
								+ CMLib.masking().maskDesc(qualMask) + "\n\r");
					...
					public void callBack() {
						...
						if (skipChecks
								|| CMLib.clans().goForward(mob, C, cmds,
										Clan.Function.PREMISE, true)) {
	...
	public void setClanQualMask(MOB mob, Clan C, String qualMask) {
		...
		CMLib.clans().clanAnnounce(
				mob,
				"The qualifications of " + C.getGovernmentName() + " "
						+ C.clanID() + " have been changed.");
	}
