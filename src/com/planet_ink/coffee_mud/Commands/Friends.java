...
public class Friends extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((commands.size() < 2)
				|| (((String) commands.elementAt(1)).equalsIgnoreCase("list"))) {
			if (h.size() == 0)
		...
		} else if (((String) commands.elementAt(1)).equalsIgnoreCase("ADD")) {
			...
			h.add(name);
			mob.tell("The Player '" + name
					+ "' has been added to your friends list.");
		} else if (((String) commands.elementAt(1)).equalsIgnoreCase("REMOVE")) {
			String name = CMParms.combine(commands, 2);
			if (name.length() == 0) {
			...
			if (!h.contains(name)) {
				mob.tell("That name '" + name
						+ "' does not appear on your list.  Watch your casing!");
				return false;
			}
			h.remove(name);
			mob.tell("The Player '" + name
			...
			mob.tell("Parameter '" + ((String) commands.elementAt(1))
					+ "' is not recognized.  Try LIST, ADD, or REMOVE.");
			return false;
		}
		return false;
	}
