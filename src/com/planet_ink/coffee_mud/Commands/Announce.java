...
public class Announce extends StdCommand {
	...
	public void sendAnnounce(MOB from, String announcement, Session S) {
		...
		if ((from != null) && (from.playerStats() != null)
				&& (from.playerStats().announceMessage().length() > 0))
			Message.append(from.playerStats().announceMessage() + " '"
					+ announcement + "'.^.^N");
		else {
			int alignType = 2;
			...
			case 1:
				...
				break;
			case 2:
				Message.append("^pA powerful voice rings out '" + announcement
						+ "'.^N");
				break;
			}
		}
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (cmd.equalsIgnoreCase("ANNOUNCEMSG")) {
			...
			if (s.length() == 0)
				mob.tell("Your announce message is currently: "
						+ mob.playerStats().announceMessage());
			else {
			...
			if ((!cmd.equalsIgnoreCase("ANNOUNCETO"))
					|| (((String) commands.elementAt(1)).toUpperCase()
							.equals("ALL"))) {
				String text = null;
				if (cmd.equalsIgnoreCase("ANNOUNCETO"))
				...
				for (Session S : CMLib.sessions().localOnlineIterable()) {
					if ((S.mob() != null)
							...
							&& (CMSecurity.isAllowed(mob, S.mob().location(),
									CMSecurity.SecFlag.ANNOUNCE))
							&& (((name.equalsIgnoreCase("here")) && (S.mob()
									.location() == mob.location())) || (CMLib
									.english().containsString(S.mob().name(),
									name)))) {
