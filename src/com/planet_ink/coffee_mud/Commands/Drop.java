...
public class Drop extends StdCommand {
	...
	public boolean drop(MOB mob, Environmental dropThis, boolean quiet,
			boolean optimize, boolean intermediate) {
		final Room R = mob.location();
		if (R == null)
			return false;
		final int msgCode = (optimize ? CMMsg.MASK_OPTIMIZE : 0)
				| (intermediate ? CMMsg.MASK_INTERMSG : 0) | CMMsg.MSG_DROP;
		CMMsg msg = CMClass.getMsg(mob, dropThis, null, msgCode, quiet ? null
	...
	/**
	 * This method actually performs the drop, when the given parsed set of
	 * command-line words.
	 * 
	 * The commands list is almost always the set of strings, starting with the
	 * access word that triggered the command. This command does have a custom
	 * API however, that allows an Item, and two Boolean objects to be
	 * substitued for the normal command strings.
	 * 
	 ...
	 * @return whether the command was successfully executed. true if it was
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((commands.size() >= 3) && (commands.firstElement() instanceof Item)
			...
			return drop(mob, (Item) commands.firstElement(),
					...
					((Boolean) commands.elementAt(2)).booleanValue(), false);
		}
		...
		int maxToDrop = CMLib.english().calculateMaxToGive(mob, commands, true,
				mob, false);
		if (maxToDrop < 0)
			return false;
		...
		if (dropThis != null) {
			if (((Coins) dropThis).getNumberOfCoins() < CMLib.english()
					.numPossibleGold(mob, whatToDrop + addendumStr))
				return false;
		...
		if (V.size() == 0)
			while (doBugFix || ((allFlag) && (addendum <= maxToDrop))) {
				doBugFix = false;
				dropThis = mob.fetchItem(container, Wearable.FILTER_UNWORNONLY,
				...
				if ((allFlag) && (!onlyGoldFlag) && (dropThis instanceof Coins)
						&& (whatToDrop.equalsIgnoreCase("all")))
					dropThis = null;
				else {
					if (dropThis == null)
						break;
	...
	public Object executeInternal(MOB mob, int metaFlags, Object... args)
		...
		return Boolean.valueOf(drop(mob, (Environmental) args[0],
