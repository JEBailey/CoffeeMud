...
public class Test extends StdCommand {
	...
	public static final String[] spells = { "Spell_Blur",
	...
	public static final String[] maliciousspells = { "Spell_Blindness",
			"Spell_Mute" };
	...
	public boolean testResistance(MOB mob) {
		...
		CMLib.combat().postDamage(mob, mob, I, 5, CMMsg.MSG_WEAPONATTACK,
				Weapon.TYPE_PIERCING, "<S-NAME> <DAMAGE> <T-NAME>.");
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() > 1) {
			...
			if (what.equalsIgnoreCase("levelxptest")) {
				for (int i = 0; i < 100; i++)
					CMLib.leveler().getLevelExperience(
							CMLib.dice().roll(1, 100, 0));
				MOB M = CMClass.getMOB("StdMOB");
				M.setExperience(0);
				for (int i = 1; i < 100; i++) {
					M.basePhyStats().setLevel(i);
					M.phyStats().setLevel(i);
					M.baseCharStats().setClassLevel(
							M.baseCharStats().getCurrentClass(), i);
					M.charStats().setClassLevel(
							M.baseCharStats().getCurrentClass(), i);
					int level = M.basePhyStats().level();
					int xp = 0;
					String s = i + ") " + M.getExperience() + "/"
							+ M.getExpNextLevel() + "/" + M.getExpNeededLevel()
							+ ": ";
				...
				StringBuffer str = new StringBuffer("");
				for (int i = 0; i < 110; i++)
					str.append(i + "=" + CMLib.leveler().getLevelExperience(i)
							+ "\r");
				...
				for (Enumeration<Ability> a = CMClass.abilities(); a
					...
					if (A instanceof ItemCraftor) {
						...
						if ((theRest.length() == 0)
								|| (I.ID().toUpperCase().indexOf(theRest) >= 0)) {
							List<ItemCraftor.ItemKeyPair> set = I
									.craftAllItemSets(false);
							for (ItemCraftor.ItemKeyPair KP : set) {
								if ((KP.item instanceof Armor)
										|| (KP.item instanceof Weapon)) {
									int newLevel = CMLib.itemBuilder()
											.timsLevelCalculator(KP.item);
									if ((newLevel < Math.round(KP.item
											.basePhyStats().level() * .7))
											|| (newLevel > Math
													.round(KP.item
															.basePhyStats()
															.level() * 1.3)))
										mob.tell(KP.item.name()
												+ ": "
												+ KP.item.basePhyStats()
														.level() + "!="
												+ newLevel);
								}
							}
			...
			if (what.equalsIgnoreCase("deconstruction")) {
									...
									if (!oI.mayICraft(P.item)) {
										if (oI == I) {
											Log.sysOut(
													"INFO",
													P.item.name()
															+ " can't even be built by "
															+ oI.ID());
										}
									} else {
										if (oI != I)
											Log.sysOut("INFO", P.item.name()
													+ ", owned by " + I.ID()
				...
				for (int i = 0; i < num; i++)
					if (mob.location().getArea().getRandomProperRoom() == null)
						numNull++;
				mob.tell("Picked " + (num - numNull) + "/" + num
						+ " rooms in this area.");
			} else if (what.equalsIgnoreCase("edrecipe")) {
				boolean save = CMParms.combine(commands, 2).equalsIgnoreCase(
						"save");
					...
					if (A instanceof ItemCraftor) {
						...
						if (iA.parametersFormat().length() > 0)
							CMLib.ableParms().testRecipeParsing(
									iA.parametersFile(), iA.parametersFormat(),
									save);
					}
				}
				...
				M.setStartRoom(null);
				ScriptingEngine S = (ScriptingEngine) M
				...
				A.destroy();
				Resources
						.removeResource("PARSEDPRG: LOAD=progs/scriptableTest.script");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getWeapon("Sword"), HaveEnabler, mobs[0],
						null, 0);
				...
				IS[0].unWear();
				R.moveItemTo(IS[0], ItemPossessor.Expire.Never,
				...
				R.moveItemTo(IS[0], ItemPossessor.Expire.Never,
						ItemPossessor.Move.Followers);
				R.moveItemTo(IS[1], ItemPossessor.Expire.Never,
						ItemPossessor.Move.Followers);
				R.recoverRoomStats();
				if (spellCheck(spells, mobs[0])) {
					mob.tell("Error1-5");
			...
			if ((what.equalsIgnoreCase("all_properties"))
					...
					|| what.equalsIgnoreCase("all")) {
				reset(mobs, backups, R, IS, R2);
				Ability HaveSpellCast = CMClass
						.getAbility("Prop_HaveSpellCast");
				HaveSpellCast.setMiscText(semiSpellList());
				mob.tell("Test#2-1: " + HaveSpellCast.accountForYourself());
				IS = giveTo(CMClass.getWeapon("Sword"), HaveSpellCast, mobs[0],
						null, 0);
				...
				reset(mobs, backups, R, IS, R2);
				HaveSpellCast
						.setMiscText(semiSpellList() + "MASK=-RACE +Dwarf");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getWeapon("Sword"), WearEnabler, mobs[0],
						null, 1);
				if (!spellCheck(spells, mobs[0])) {
					mob.tell("Error3-1");
					return false;
			...
			if ((what.equalsIgnoreCase("all_properties"))
					...
					|| what.equalsIgnoreCase("all")) {
				reset(mobs, backups, R, IS, R2);
				Ability WearSpellCast = CMClass
						.getAbility("Prop_WearSpellCast");
				WearSpellCast.setMiscText(semiSpellList());
				mob.tell("Test#4-1: " + WearSpellCast.accountForYourself());
				IS = giveTo(CMClass.getWeapon("Sword"), WearSpellCast, mobs[0],
						null, 1);
				if (!effectCheck(spells, mobs[0])) {
					mob.tell("Error4-1");
					return false;
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getItem("Boat"), RideEnabler, mobs[0],
						null, 2);
				if (!spellCheck(spells, mobs[0])) {
					mob.tell("Error5-1");
					return false;
			...
			if ((what.equalsIgnoreCase("all_properties"))
					...
					|| what.equalsIgnoreCase("all")) {
				reset(mobs, backups, R, IS, R2);
				Ability RideSpellCast = CMClass
						.getAbility("Prop_RideSpellCast");
				RideSpellCast.setMiscText(semiSpellList());
				// mob.tell("Test#6-1: "+RideSpellCast.accountForYourself());
				IS = giveTo(CMClass.getItem("Boat"), RideSpellCast, mobs[0],
						null, 2);
				if (!effectCheck(spells, mobs[0])) {
					mob.tell("Error6-1");
					return false;
			...
			if ((what.equalsIgnoreCase("all_properties"))
					...
					|| what.equalsIgnoreCase("all")) {
				reset(mobs, backups, R, IS, R2);
				Ability HereSpellCast = CMClass
						.getAbility("Prop_HereSpellCast");
				HereSpellCast
						.setMiscText(semiSpellList() + "MASK=-RACE +Dwarf");
			...
			if ((what.equalsIgnoreCase("all_properties"))
					...
					|| what.equalsIgnoreCase("all")) {
				reset(mobs, backups, R, IS, R2);
				Ability UseSpellCast = CMClass.getAbility("Prop_UseSpellCast"); // put
																				// IN
				UseSpellCast.setMiscText(semiSpellList());
				mob.tell("Test#9-1: " + UseSpellCast.accountForYourself());
				IS = giveTo(CMClass.getItem("SmallSack"), UseSpellCast,
						mobs[0], null, 0);
				I = CMClass.getItem("StdFood");
				mobs[0].addItem(I);
				C = CMClass.getCommand("Put");
				C.execute(mobs[0], new XVector("Put", "Food", "Sack"),
						metaFlags);
				if (!effectCheck(spells, mobs[0])) {
					mob.tell("Error9-1");
					return false;
				...
				mobs[1].addItem(I);
				C = CMClass.getCommand("Put");
				C.execute(mobs[1], new XVector("Put", "Food", "Sack"),
						metaFlags);
				if (effectCheck(spells, mobs[1])) {
					mob.tell("Error9-3");
					return false;
			...
			if ((what.equalsIgnoreCase("all_properties"))
					...
					|| what.equalsIgnoreCase("all")) {
				reset(mobs, backups, R, IS, R2);
				Ability UseSpellCast2 = CMClass
						.getAbility("Prop_UseSpellCast2"); // EAT
				UseSpellCast2.setMiscText(semiSpellList());
				mob.tell("Test#10-1: " + UseSpellCast2.accountForYourself());
				IS = giveTo(CMClass.getItem("StdFood"), UseSpellCast2, mobs[0],
						null, 0);
			...
			if (what.equalsIgnoreCase("cmparms")
				...
				if (V.size() != 3) {
					mob.tell("Error cmparms-1");
					return false;
				}
				if (!CMParms.combineWithX(V, "~", 0).equals("blah~BLAH~BLAH!~")) {
					mob.tell("Error cmparms-2");
					return false;
				}

				V = CMParms.parseAny("blah~~", '~', true);
				if (V.size() != 1) {
					mob.tell("Error cmparms-3");
					return false;
				}
				if (!V.get(0).equals("blah")) {
					mob.tell("Error cmparms-4");
					return false;
				}

				V = CMParms.parseAny("blah~~", '~', false);
				if (V.size() != 3) {
					mob.tell("Error cmparms-5");
					return false;
				}
				if (!CMParms.combineWithX(V, "~", 0).equals("blah~~~")) {
					mob.tell("Error cmparms-6");
					return false;
				}

				V = CMParms.parseAny("blah~~BLAH~~BLAH!", "~~", true);
				if (V.size() != 3) {
					mob.tell("Error cmparms-7");
					return false;
				}
				if (!CMParms.combineWithX(V, "~~", 0).equals(
						"blah~~BLAH~~BLAH!~~")) {
					mob.tell("Error cmparms-8");
					return false;
				}

				V = CMParms.parseAny("blah~~~~", "~~", true);
				if (V.size() != 1) {
					mob.tell("Error cmparms-9");
					return false;
				}
				if (!V.get(0).equals("blah")) {
					mob.tell("Error cmparms-10");
					return false;
				}

				V = CMParms.parseAny("blah~~~~", "~~", false);
				if (V.size() != 3) {
					mob.tell("Error cmparms-11");
					return false;
				}
				if (!CMParms.combineWithX(V, "~~", 0).equals("blah~~~~~~")) {
					mob.tell("Error cmparms-12");
					return false;
				}

				V = CMParms.parseSentences("blah. blahblah. poo");
				if (V.size() != 3) {
					mob.tell("Error cmparms-13");
					return false;
				}
				if (!V.get(0).equals("blah.")) {
					mob.tell("Error cmparms-14:" + V.get(0));
					return false;
				}
				if (!V.get(1).equals("blahblah.")) {
					mob.tell("Error cmparms-15:" + V.get(1));
					return false;
				}
				if (!V.get(2).equals("poo")) {
					mob.tell("Error cmparms-16:" + V.get(2));
					return false;
				}

				V = CMParms.parseAny("blah~BLAH~BLAH!~", '~', true);
				if (V.size() != 3) {
					mob.tell("Error cmparms-17");
					return false;
				}
				if (!CMParms.combineWithX(V, "~", 0).equals("blah~BLAH~BLAH!~")) {
					mob.tell("Error cmparms-18");
					return false;
				}

				V = CMParms.parseAny("blah~~BLAH~~BLAH!~~", "~~", true);
				if (V.size() != 3) {
					mob.tell("Error cmparms-19");
					return false;
				}
				if (!CMParms.combineWithX(V, "~~", 0).equals(
						"blah~~BLAH~~BLAH!~~")) {
					mob.tell("Error cmparms-20");
					return false;
				}

				V = CMParms.parseAny("blah~BLAH~BLAH!~", '~', false);
				if (V.size() != 4) {
					mob.tell("Error cmparms-21");
					return false;
				}
				if (!CMParms.combineWithX(V, "~", 0)
						.equals("blah~BLAH~BLAH!~~")) {
					mob.tell("Error cmparms-22");
					return false;
				}

				V = CMParms.parseAny("blah~~BLAH~~BLAH!~~", "~~", false);
				if (V.size() != 4) {
					mob.tell("Error cmparms-23");
					return false;
				}
				if (!CMParms.combineWithX(V, "~~", 0).equals(
						"blah~~BLAH~~BLAH!~~~~")) {
					mob.tell("Error cmparms-24");
					return false;
				}
			}
			if ((what.equalsIgnoreCase("all_properties"))
					...
					|| what.equalsIgnoreCase("all")) {
				reset(mobs, backups, R, IS, R2);
				Ability FightSpellCast = CMClass
						.getAbility("Prop_FightSpellCast");
				FightSpellCast.setMiscText(maliciousSemiSpellList());
				mob.tell("Test#11-1: " + FightSpellCast.accountForYourself());
				IS = giveTo(CMClass.getWeapon("Sword"), FightSpellCast,
						mobs[0], null, 1);
				...
				for (int i = 0; i < 100; i++) {
					...
					CMLib.combat().postAttack(mobs[0], mobs[1],
							mobs[0].fetchWieldedItem());
					if (effectCheck(maliciousspells, mobs[1]))
						break;
				}
				if (!effectCheck(maliciousspells, mobs[1])) {
				...
				for (int i = 0; i < 100; i++) {
					mobs[1].curState().setHitPoints(1000);
					mobs[0].curState().setHitPoints(1000);
					CMLib.combat().postAttack(mobs[1], mobs[0],
							mobs[1].fetchWieldedItem());
					if (effectCheck(maliciousspells, mobs[1]))
						break;
				}
				if (effectCheck(maliciousspells, mobs[1])) {
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getWeapon("Sword"), HaveZapper, mobs[0],
						mobs[1], 2);
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getItem("Boat"), RideZapper, mobs[0],
						mobs[1], 3);
				msg = CMClass.getMsg(mobs[0], IS[0], null, CMMsg.MSG_MOUNT,
						"<S-NAME> mount(s) <T-NAMESELF>.");
				if (R.okMessage(mobs[0], msg))
					R.send(mobs[0], msg);
				msg = CMClass.getMsg(mobs[1], IS[1], null, CMMsg.MSG_MOUNT,
						"<S-NAME> mount(s) <T-NAMESELF>.");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getWeapon("Sword"), WearZapper, mobs[0],
						mobs[1], 0);
				msg = CMClass.getMsg(mobs[0], IS[0], null, CMMsg.MSG_WIELD,
						"<S-NAME> wield(s) <T-NAMESELF>.");
				if (R.okMessage(mobs[0], msg))
					R.send(mobs[0], msg);
				msg = CMClass.getMsg(mobs[1], IS[1], null, CMMsg.MSG_WIELD,
						"<S-NAME> wield(s) <T-NAMESELF>.");
				if (R.okMessage(mobs[1], msg))
					R.send(mobs[1], msg);
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				reset(mobs, backups, R, IS, R2);
				Resistance
						.setMiscText("pierce 100% holy 100% acid 30% MASK=-RACE +DWARF");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getItem("SmallSack"), HaveResister,
						mobs[0], null, 0);
				R.recoverRoomStats();
				if (!testResistance(mobs[0])) {
					mob.tell("Error16-2");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getWeapon("Sword"), WearResister, mobs[0],
						null, 1);
				R.recoverRoomStats();
				if (!testResistance(mobs[0])) {
					mob.tell("Error17-2");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getItem("Boat"), RideResister, mobs[0],
						null, 2);
				R.recoverRoomStats();
				if (!testResistance(mobs[0])) {
					mob.tell("Error18-2");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				HaveAdjuster
						.setMiscText("abi+10 gen=F class=Fighter cha+10 man+1000");
				mob.tell("Test#19-1: " + HaveAdjuster.accountForYourself());
				if (isAnyAdjusted(mobs[0])) {
					mob.tell("Error19-1");
					return false;
				}
				IS = giveTo(CMClass.getItem("SmallSack"), HaveAdjuster,
						mobs[0], null, 0);
				R.recoverRoomStats();
				...
				HaveAdjuster
						.setMiscText("abi+10 gen=F class=Fighter cha+10 man+1000 MASK=-RACE +Dwarf");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getItem("SmallSack"), WearAdjuster,
						mobs[0], null, 1);
				R.recoverRoomStats();
				if (!isAllAdjusted(mobs[0])) {
					mob.tell("Error20-2");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				IS = giveTo(CMClass.getItem("Boat"), RideAdjuster, mobs[0],
						null, 2);
				R.recoverRoomStats();
				if (!isAllAdjusted(mobs[0])) {
					mob.tell("Error21-2");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				reset(mobs, backups, R, IS, R2);
				HereAdjuster
						.setMiscText("abi+10 gen=F class=Fighter cha+10 man+1000 MASK=-Human");
			...
			if ((what.equalsIgnoreCase("all_properties"))
				...
				R2.addNonUninvokableEffect(reqA);
				IS = giveTo(CMClass.getWeapon("Sword"), null, mobs[0], mobs[1],
						0);
				...
				msg = CMClass
						.getMsg(mobs[0], IS[0], null, CMMsg.MSG_DROP, null);
				...
				msg = CMClass
						.getMsg(mobs[0], IS[1], null, CMMsg.MSG_DROP, null);
				if (R2.okMessage(mobs[0], msg)) {
					mob.tell("Error24-4");
					return false;
				}
			...
			if ((what.equalsIgnoreCase("all") || what.equalsIgnoreCase("rtree"))
					&& (mob.session() != null)) {
				...
				for (int g = 0; g < 15; g++) {
					...
					for (int i = 0; i < 100; i++) {
						...
						final BoundedObject.BoundedCube cube = new BoundedObject.BoundedCube(
								gcenterX - dist, gcenterX + dist, gcenterY
										- dist, gcenterY + dist, gcenterZ
										- dist, gcenterZ + dist);
						final int num = i;
				...
				List<BoundedObject> setToAdd = new Vector<BoundedObject>(
						origSet.size());
				List<BoundedObject> randomSet = new Vector<BoundedObject>(
						origSet.size());
				setToAdd.addAll(origSet);
				while (setToAdd.size() > 0) {
					BoundedObject O = setToAdd
							.remove(r.nextInt(setToAdd.size()));
				...
				mob.tell("Average set size=" + (totalSize / samples.size())
						+ ", time=" + ((System.currentTimeMillis() - t1))
						+ ", count=" + tree.count());
				for (BoundedObject O : origSet) {
					if ((!tree.contains(O)) || (!tree.leafSearch(O))) {
						mob.tell("Error25-0.2");
						return false;
					}
				}
				for (int gnum = 0; gnum < samples.size(); gnum++) {
					...
					for (int i = 0; i < setToAdd.size(); i++)
						if (!setToAdd.get(i).toString()
								.startsWith("g" + gnum + "#")) {
							mob.tell("Error25-1.1");
							return false;
						}
				}
				for (int gnum = 0; gnum < samples.size(); gnum++) {
					...
					{
						BoundedObject O2 = setToAdd.get(i2);
						for (int i3 = setToAdd.size() - 1; i3 > i2; i3--)
							// remove dups
					...
					for (int i = 0; i < setToAdd.size(); i++) {
						...
						if ((!tree.contains(O)) && (!tree.leafSearch(O))) {
							mob.tell("Error25-2#" + gnum + "/" + i);
							return false;
						}
						if (!tree.remove(O)) {
							mob.tell("Error25-3#" + gnum + "/" + i);
							return false;
						}
						for (int i2 = i + 1; i2 < setToAdd.size(); i2++) {
							BoundedObject O2 = setToAdd.get(i2);
							if ((!tree.contains(O2)) && (!tree.leafSearch(O2))) {
								mob.tell("Error25-3.2#" + gnum + "/" + i + "/"
										+ i2 + "/" + setToAdd.size());
								return false;
							}
						}
						if (tree.contains(O)) {
							mob.tell("Error25-4#" + gnum + "/" + i);
							return false;
						}
						List<BoundedObject> dblCheck = new Vector<BoundedObject>(
								setToAdd.size() - i);
						tree.query(dblCheck, pt[0], pt[1], pt[2]);
						if (dblCheck.contains(O)) {
							mob.tell("Error25-5#" + gnum + "/" + i);
							return false;
						}
						if (tree.leafSearch(O)) {
							mob.tell("Error25-6#" + gnum + "/" + i);
							return false;
						}
						if (tree.count() != ct - 1) {
							mob.tell("Error25-7#" + gnum + "/" + i + ":"
									+ tree.count() + "/" + (ct - 1));
							return false;
						}
					}
			...
			if ((what.equalsIgnoreCase("all"))
				...
				String str = ColorLibrary.COLOR_GREY
						+ ColorLibrary.COLOR_BGGREEN
						+ "^<CHANNEL \"TEST\"^>You TEST 'message'^</CHANNEL^>^N^.";
				str = CMLib.coffeeFilter().fullOutFilter(mob.session(), mob,
						mob, null, null, str, false);
				str = CMLib.coffeeFilter().fullOutFilter(mob.session(), mob,
						mob, null, null, str, false);
				str = CMLib.coffeeFilter().fullOutFilter(mob.session(), mob,
						mob, null, null, str, false);
			...
			if ((what.equalsIgnoreCase("all"))
				...
				reset(mobs, backups, R, IS, R2);
				mobs[0].setPlayerStats((PlayerStats) CMClass
						.getCommon("DefaultPlayerStats"));
				mobs[1].setPlayerStats((PlayerStats) CMClass
						.getCommon("DefaultPlayerStats"));
				Session S1 = (Session) CMClass.getCommon("FakeSession");
				Session S2 = (Session) CMClass.getCommon("FakeSession");
				S1.initializeSession(null, Thread.currentThread()
						.getThreadGroup().getName(), "MEMORY");
				S2.initializeSession(null, Thread.currentThread()
						.getThreadGroup().getName(), "MEMORY");
