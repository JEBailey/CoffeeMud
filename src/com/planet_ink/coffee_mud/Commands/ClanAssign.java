...
public class ClanAssign extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		String memberStr = (commands.size() > 2) ? (String) commands
				.get(commands.size() - 2) : "";
		String pos = (commands.size() > 1) ? (String) commands.get(commands
				.size() - 1) : "";
		String clanName = (commands.size() > 3) ? CMParms.combine(commands, 1,
				commands.size() - 2) : "";
			...
			for (Pair<Clan, Integer> c : mob.clans())
				if ((clanName.length() == 0)
						...
						&& (c.first.getAuthority(c.second.intValue(),
								Clan.Function.ASSIGN) != Authority.CAN_NOT_DO)) {
					C = c.first;
					break;
				}
		...
		if (memberStr.length() > 0) {
			if (C == null) {
				mob.tell("You aren't allowed to assign anyone from "
			...
			if (skipChecks
					|| CMLib.clans().goForward(mob, C, commands,
							Clan.Function.ASSIGN, false)) {
				List<MemberRecord> members = C.getMemberList();
				if (members.size() < 1) {
					mob.tell("There are no members in your "
							+ C.getGovernmentName() + "");
					return false;
				}
				...
				if (found) {
					...
					Pair<Clan, Integer> oldRole = (M != null) ? M.getClanRole(C
							.clanID()) : null;
					if ((M == null) || (oldRole == null)) {
						mob.tell(memberStr
								+ " was not found.  Could not change "
								+ C.getGovernmentName() + " role.");
						return false;
					}
					if (!C.canBeAssigned(M, newPos)) {
						mob.tell(M.name(mob) + " may not be assigned to "
								+ C.getRoleName(newPos, true, false) + ".");
						return false;
					}
					if (skipChecks
							|| CMLib.clans().goForward(mob, C, commands,
									Clan.Function.ASSIGN, true)) {
						...
						List<Integer> topRoleIDs = C
								.getTopRankedRoles(Function.ASSIGN);
						if (topRoleIDs.contains(Integer.valueOf(oldPos))) { // If
																			// you
																			// WERE
																			// already
																			// the
							...
							for (MemberRecord member : members)
								if (!M.Name().equalsIgnoreCase(member.name))
									if (topRoleIDs.contains(Integer
											.valueOf(member.role)))
										numMembers++;
							if (numMembers == 0) {
								mob.tell(M.Name()
										+ " is the last "
										+ C.getRoleName(oldPos, true, false)
										+ " and must be replaced before being reassigned.");
								return false;
							}
						}
						if ((currentMembersInNewPosV.size() > 0)
								&& (maxInNewPos < Integer.MAX_VALUE)) {
							// if there are too many in the new position, demote
							// some of them.
							while (currentMembersInNewPosV.size() >= maxInNewPos) {
								String s = (String) currentMembersInNewPosV
										.elementAt(0);
								...
								if (M2 != null)
									M2.setClan(C.clanID(), C.getGovernment()
											.getAcceptPos());
								CMLib.database().DBUpdateClanMembership(s,
										C.clanID(),
						...
						mob.tell(M.Name()
								...
								+ " "
								+ C.clanID()
								+ " has been assigned to be "
								+ CMLib.english().startWithAorAn(
										C.getRoleName(newPos, false, false))
								+ ". ");
						if ((M.session() != null) && (M.session().mob() == M))
							M.tell("You have been assigned to be "
									+ CMLib.english()
											.startWithAorAn(
													C.getRoleName(newPos,
															false, false))
									+ " of " + C.getGovernmentName() + " "
									+ C.clanID() + ".");
						return false;
					}
				} else {
					msg.append(memberStr + " isn't a member of your "
							+ C.getGovernmentName() + ".");
				}
			} else {
				msg.append("You aren't in the right position to assign anyone in your "
						+ C.getGovernmentName() + ".");
			}
		} else {
