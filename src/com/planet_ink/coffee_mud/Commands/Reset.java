...
public class Reset extends StdCommand {
	...
	public boolean securityCheck(MOB mob) {
		return CMSecurity.isAllowed(mob, mob.location(),
				CMSecurity.SecFlag.RESET);
	}

	public int resetAreaOramaManaI(MOB mob, Item I, Hashtable rememberI,
			String lead) throws java.io.IOException {
		int nochange = 0;
		if (I instanceof AmmunitionWeapon) {
			AmmunitionWeapon W = (AmmunitionWeapon) I;
			if ((W.requiresAmmunition()) && (W.ammunitionCapacity() > 0)) {
				String str = mob.session().prompt(
						lead + I.Name() + " requires (" + W.ammunitionType()
								+ "): ");
				if (str.length() > 0) {
					if ((str.trim().length() == 0)
						...
						str = mob.session().prompt(
								lead + I.Name() + " new weapon type: ");
		...
		if (IT != null) {
			if (IT.intValue() == I.material()) {
				mob.tell(lead + I.Name() + " still "
						+ RawMaterial.CODES.NAME(I.material()));
				return nochange;
			}
			I.setMaterial(IT.intValue());
			mob.tell(lead + I.Name() + " Changed to "
					+ RawMaterial.CODES.NAME(I.material()));
			return 1;
		}
		while (true) {
			...
			if (str.equals("?"))
				mob.tell(I.Name() + "/" + I.displayText() + "/"
						+ I.description());
			else {
				int material = RawMaterial.CODES.FIND_CaseSensitive(str
						.toUpperCase());
				if (material >= 0) {
					I.setMaterial(RawMaterial.CODES.GET(material));
					mob.tell(lead + "Changed to "
							+ RawMaterial.CODES.NAME(material));
					rememberI.put(I.Name(), Integer.valueOf(I.material()));
					return 1;
				}
		...
		if (names.length % 3 != 0)
			...
			for (int i = 0; i < 6; i++) {
				...
				if (i % 3 == 0) {
					...
					M.setMaxTechLevelDiff((byte) (M.getMinTechLevelDiff() + CMLib
							.dice().roll(1, 3, 2)));
				} else if (i % 3 == 1) {
					...
					M.setMinTechLevelDiff((byte) (8 - CMLib.dice()
							.roll(1, 3, 0)));
					M.setMaxTechLevelDiff((byte) 10);
				}
				M.setEfficiencyPct(0.75 + CMath.div(CMLib.dice()
						.rollNormalDistribution(1, 50, 0), 100.0));
				M.setReliabilityPct(0.75 + CMath.div(CMLib.dice()
						.rollNormalDistribution(1, 50, 0), 100.0));
	...
	protected int rightImportMat(MOB mob, Item I, boolean openOnly)
		...
		if ((I != null) && (I.description().trim().length() > 0)) {
			...
			if ((x < 0) || ((x > 0) && (y == x))) {
				...
				if (rightMat < 0) {
					Log.sysOut("Reset",
							"Unconventional material: " + I.description());
				...
				if (rightMat < 0)
					...
					if (mob != null) {
						if (mob.session().confirm(
								"Change " + I.name() + "/" + I.displayText()
										+ " material to "
										+ RawMaterial.CODES.NAME(rightMat)
										+ " (y/N)?", "N")) {
	...
	public String resetWarning(MOB mob, Room R) {
		...
		for (Session S : CMLib.sessions().localOnlineIterable())
			if ((S != null) && (S.mob() != null) && (S.mob() != mob)
					&& (S.mob().location() == R))
				warning.append("A player, '" + S.mob().Name() + "' is in "
						+ CMLib.map().getExtendedRoomID(R) + "\n\r");
		Item I = null;
		for (int i = 0; i < R.numItems(); i++) {
			I = R.getItem(i);
			if ((I instanceof DeadBody) && (((DeadBody) I).playerCorpse()))
				warning.append("A player corpse, '" + I.Name() + "' is in "
						+ CMLib.map().getExtendedRoomID(R) + "\n\r");
		}
	...
	private void reportChangesDestroyNewM(MOB oldM, MOB newM,
			StringBuffer changes) {
		if ((changes == null) || (oldM == null))
			return;
		changes.append(newM.name() + ":" + newM.basePhyStats().level() + ", ");
		for (int i = 0; i < oldM.getStatCodes().length; i++)
			if ((!oldM.getStat(oldM.getStatCodes()[i]).equals(
					newM.getStat(newM.getStatCodes()[i]))))
				changes.append(oldM.getStatCodes()[i] + "("
						+ oldM.getStat(oldM.getStatCodes()[i]) + "->"
						+ newM.getStat(newM.getStatCodes()[i]) + "), ");
		changes.append("\n\r");
	...
	public boolean fixMob(MOB M, StringBuffer recordedChanges) {
		MOB M2 = CMLib.leveler().fillOutMOB(
				M.baseCharStats().getCurrentClass(), M.basePhyStats().level());
		if ((M.basePhyStats().attackAdjustment() != M2.basePhyStats()
				.attackAdjustment())
			...
			M.basePhyStats().setAttackAdjustment(
					M2.basePhyStats().attackAdjustment());
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (s.equalsIgnoreCase("rejuv")) {
			...
			if (s.equalsIgnoreCase("room")) {
				...
				for (Enumeration<Room> e = A.getProperMap(); e
						.hasMoreElements();)
			...
			if ((mob.session() == null)
					|| (warning == null)
					|| (mob.session().confirm(warning
							+ "\n\rReset the contents of the room '"
							+ mob.location().displayText(mob) + "', OK (Y/n)?",
							"Y"))) {
				for (Session S : CMLib.sessions().localOnlineIterable())
					if ((S != null) && (S.mob() != null)
							&& (S.mob().location() != null)
							&& (S.mob().location() == mob.location()))
						S.mob().tell(mob, null, null,
								"<S-NAME> order(s) this room to normalcy.");
				CMLib.map().resetRoom(mob.location(), true);
				mob.tell("Done.");
			} else
				mob.tell("Cancelled.");
		} else if (s.equalsIgnoreCase("INIFILE")
				|| s.equalsIgnoreCase("coffeemud.ini")) {
			...
			if (A != null) {
				...
				if ((mob.session() == null)
						|| (mob.session().confirm(
								"Reset the contents of the area '" + A.name()
										+ "', OK (Y/n)?", "Y"))) {
					for (Session S : CMLib.sessions().localOnlineIterable())
						if ((S != null)
								&& (S.mob() != null)
								&& (S.mob().location() != null)
								&& (A.inMyMetroArea(S.mob().location()
										.getArea())))
							S.mob().tell(mob, null, null,
									"<S-NAME> order(s) this area to normalcy.");
		...
		} else if (s.equalsIgnoreCase("arearoomids")
		...
		} else if (!CMSecurity.isAllowed(mob, mob.location(),
				CMSecurity.SecFlag.RESETUTILS)) {
			mob.tell("'"
					+ s
					+ "' is an unknown reset.  Try ROOM, AREA, AREAROOMIDS *.\n\r * = Reset functions which may take a long time to complete.");
			return false;
			...
			for (Enumeration e = CMClass.races(); e.hasMoreElements();) {
				...
				if (R.isGeneric()) {
					...
					if (save) {
						R.setStat("ASTATS", CMLib.coffeeMaker()
								.getCharStatsStr(ADJSTAT1));
		...
		} else if (s.equalsIgnoreCase("genraceagingcharts")
				&& (CMSecurity.isAllowed(mob, mob.location(),
						CMSecurity.SecFlag.CMDRACES))) {
			for (Enumeration e = CMClass.races(); e.hasMoreElements();) {
				Race R = (Race) e.nextElement();
				...
				if ((R.isGeneric()) && (R.ID().length() > 1)
						&& (!R.ID().endsWith("Race"))
						&& (Character.isUpperCase(R.ID().charAt(0)))) {
					int lastStart = 0;
					int c = 1;
					while (c <= R.ID().length()) {
						if ((c == R.ID().length())
								|| (Character.isUpperCase(R.ID().charAt(c)))) {
							if ((lastStart == 0) && (c == R.ID().length())
									&& (!R.ID().endsWith("ling"))
									&& (!R.ID().startsWith("Half")))
								break;
							...
							if (partial.equals("Half")
								...
								if ((R2 != null) && (R2 != R)) {
									...
									lastStart = c;
									R2 = CMClass.getRace(partial.substring(0,
											partial.length() - 4));
									if (R2 != null)
										racesToBaseFrom.add(R2);
								}
					...
					for (int i = 0; i < racesToBaseFrom.size(); i++)
						answer.append(((Race) racesToBaseFrom.elementAt(i))
								.ID() + " ");
			...
			for (int v = 0; v < V.size(); v++) {
				...
				CMLib.database().DBCreateData(name, bank, "" + I,
						classID + ";" + data);
			}
			CMLib.database().DBDeleteJournal(bank, null); // banks are no longer
															// journaled
			...
			if (s.toUpperCase().startsWith("ROOM"))
				...
				try {
					for (Enumeration e = mob.location().getArea()
							.getCompleteMap(); e.hasMoreElements();)
				...
				try {
					for (Enumeration e = CMLib.map().areas(); e
						...
						boolean skip = false;
						for (int i = 1; i < commands.size(); i++)
							if (((String) commands.elementAt(i))
									.equalsIgnoreCase(A.Name())
									|| rest.equalsIgnoreCase(A.Name())) {
								skip = true;
								break;
							}
						if (skip)
							continue;
						for (Enumeration r = A.getCompleteMap(); r
								.hasMoreElements();)
					...
					synchronized (("SYNC" + R.roomID()).intern()) {
						...
						for (int m = 0; m < R.numInhabitants(); m++) {
							...
							if ((M.isSavable())
									...
									&& (M.getStartRoom() == R))
								somethingDone = fixMob(M, recordedChanges)
										|| somethingDone;
						}
						if (somethingDone) {
		...
		} else if (s.equalsIgnoreCase("groundlydoors")
					...
					if (R.roomID().length() > 0)
						for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
							...
							if ((E != null) && E.hasADoor()
								...
								E.setName("a door");
								E.setExitParams("door", "close", "open",
										"a door, closed.");
								changed = true;
							}
						}
					if (changed) {
						Log.sysOut("Reset", "Groundly doors in " + R.roomID()
								+ " fixed.");
			...
			mob.session().println("done!");
		} else if (s.equalsIgnoreCase("allmobarmorfix")
				&& (CMSecurity.isASysOp(mob))) {
			if (mob.session() == null)
				return false;
			mob.session().print("working...");
				...
				for (Enumeration r = A.getCompleteMap(); r.hasMoreElements();) {
					...
					synchronized (("SYNC" + R.roomID()).intern()) {
						...
						for (int i = 0; i < R.numInhabitants(); i++) {
							...
							if ((M.isMonster())
									&& (M.getStartRoom() == R)
									&& (M.basePhyStats().armor() == ((100 - (M
											.basePhyStats().level() * 7))))) {
								int oldArmor = M.basePhyStats().armor();
								M.basePhyStats().setArmor(
										CMLib.leveler().getLevelMOBArmor(M));
								M.recoverPhyStats();
								Log.sysOut("Reset", "Updated " + M.name()
										+ " in room " + R.roomID() + " from "
										+ oldArmor + " to "
										+ M.basePhyStats().armor() + ".");
								didSomething = true;
							} else
								Log.sysOut("Reset", "Skipped " + M.name()
										+ " in room " + R.roomID());
						}
						mob.session().print(".");
						if (didSomething)
			...
			mob.session().println("done!");
		} else if (s.equalsIgnoreCase("goldceilingfixer")
				&& (CMSecurity.isASysOp(mob))) {
			if (mob.session() == null)
				return false;
			mob.session().print("working...");
				...
				for (Enumeration r = A.getCompleteMap(); r.hasMoreElements();) {
					...
					synchronized (("SYNC" + R.roomID()).intern()) {
						...
						for (int i = 0; i < R.numInhabitants(); i++) {
							...
							if ((M.isMonster())
									&& (M.getStartRoom() == R)
									&& (CMLib.beanCounter().getMoney(M) > (M
											.basePhyStats().level() + 1))) {
								CMLib.beanCounter().setMoney(
										M,
										CMLib.dice().roll(1,
												M.basePhyStats().level(), 0)
			...
			mob.session().println("done!");
		} else if (s.equalsIgnoreCase("areainstall")
				&& (CMSecurity.isASysOp(mob))) {
			if (mob.session() == null)
				return false;
			if (commands.size() < 2) {
			...
			if (O == null) {
				mob.tell("'" + ID
						+ "' is not a known property or behavior.  Try LIST.");
				return false;
			}
			...
			for (Enumeration a = CMLib.map().areas(); a.hasMoreElements();) {
				...
				if ((O instanceof Behavior)) {
					...
					if (B == null) {
						...
						A.addBehavior(B);
						changed = true;
					} else if (!B.getParms().equals(
							CMParms.combine(commands, 2))) {
						B.setParms(CMParms.combine(commands, 2));
						changed = true;
					}
			...
			mob.session().println("done!");
		} else if (s.equalsIgnoreCase("worldmatconfirm")
				&& (CMSecurity.isASysOp(mob))) {
			if (mob.session() == null)
				return false;
			mob.session().print("working...");
						...
						synchronized (("SYNC" + R.roomID()).intern()) {
							...
							for (int i = 0; i < R.numItems(); i++)
								changedItems = changedItems
										|| (rightImportMat(null, R.getItem(i),
												false) >= 0);
							for (int m = 0; m < R.numInhabitants(); m++) {
								MOB M = R.fetchInhabitant(m);
								if (M == mob)
								...
								if (SK != null) {
									for (Iterator<Environmental> i = SK
										...
										if (E instanceof Item) {
											Item I = (Item) E;
											boolean didSomething = false;
											didSomething = rightImportMat(null,
													I, false) >= 0;
											changedMOBS = changedMOBS
													|| didSomething;
											if (didSomething) {
												int numInStock = SK.getShop()
														.numberInStock(I);
												int stockPrice = SK.getShop()
														.stockPrice(I);
												SK.getShop()
			...
			mob.session().println("done!");
		} else if (s.equalsIgnoreCase("itemstats")
				&& (CMSecurity.isASysOp(mob))) {
			s = "room";
			if (commands.size() > 1)
				s = (String) commands.elementAt(1);
				...
				synchronized (("SYNC" + R.roomID()).intern()) {
					...
					for (int m = 0; m < R.numInhabitants(); m++) {
						...
						for (int i = 0; i < M.numItems(); i++) {
							Item I = M.getItem(i);
							int lvl = -1;
							if ((I.basePhyStats().level() > M.basePhyStats()
									.level())
									|| ((I.basePhyStats().level() > 91) && ((I
											.basePhyStats().level() + (I
											.basePhyStats().level() / 10)) < M
											.basePhyStats().level())))
						...
						if (SK != null) {
							for (Iterator<Environmental> i = SK.getShop()
									.getStoreInventory(); i.hasNext();) {
								Environmental E = i.next();
								if (E instanceof Item) {
									Item I = (Item) E;
									boolean didSomething = false;
									didSomething = CMLib.itemBuilder().itemFix(
											I, -1, recordedChanges);
									...
									if (didSomething) {
										...
										SK.getShop().addStoreInventory(I,
												numInStock, stockPrice);
									}
								}
							}
						}
		...
		} else if (s.equalsIgnoreCase("arearacemat")
				&& (CMSecurity.isASysOp(mob))) {
							...
							for (int m = 0; m < R.numInhabitants(); m++) {
								...
								if (R2 != null) {
									if (M.charStats().getMyRace() == R2)
										mob.tell(" " + M.Name() + " still "
												+ R2.name());
									else {
										M.baseCharStats().setMyRace(R2);
										R2.setHeightWeight(
												M.basePhyStats(),
												(char) M.baseCharStats()
														.getStat(
								...
								if (SK != null) {
									for (Iterator<Environmental> i = SK
										...
										if (E instanceof Item) {
											Item I = (Item) E;
											int returned = resetAreaOramaManaI(
													mob, I, rememberI, " - ");
											if (returned < 0) {
												SK.getShop()
														.delAllStoreInventory(I);
			...
			TechType[] types;
			String[] names;
			List<Manufacturer> m = new XVector<Manufacturer>(CMLib.tech()
					.manufacterers());
			for (Manufacturer M : m)
				CMLib.tech().delManufacturer(M);
			types = new TechType[] { TechType.SHIP_SOFTWARE };
			names = new String[] { "Nanoapp", "ExaSoft", "Cryptosoft",
					"M.S. Corp", "AmLogic", "InnovaSoft", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_COMPUTER };
			names = new String[] { "IniProc", "picoSystems", "cybProc",
					"GuthrieTronics", "SolidCorp", "DelCorp", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_SPACESHIP };
			names = new String[] { "AtomiCorp", "FrontierCorp", "Vargas",
					"JarviSys", "Solar Corp", "DynamiCorp", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_ENGINE };
			names = new String[] { "Aczev Ltd", "ItsukCorp", "E.B.H. Ltd",
					"Globomotors", "Bowers Eng.", "DynamiDrive", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_SENSOR };
			names = new String[] { "BunovaCorp", "Censys", "Dering-Hao",
					"ProgressTek", "NetCorp", "McNeil Ltd", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_DAMPENER };
			names = new String[] { "S.H. Ltd", "Malik-Ni", "HsuiCorp",
					"RiddleCorp", "TotCorp", "Apex Ltd", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_ENVIRO_CONTROL };
			names = new String[] { "YehCorp", "A.O.P. Ltd", "Callahan",
					"Innovatak", "SharpeyTek", "Holtz-Derkova", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_POWER };
			names = new String[] { "Optcel", "Shinacells", "Choe Corp",
					"hyDat Ltd", "LumoDigital", "Li-Lai Ltd", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_WEAPON };
			names = new String[] { "A.S.U. Ltd", "Mi-Gievora", "A.T.F. Corp",
					"Paragon Systems", "Physionetics", "DestructoCorp", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_SHIELD };
			names = new String[] { "Lee-Lavanchy", "Hsaio GmbH",
					"Kikko-Thoran", "LarsonTek", "ElectriCorp",
					"Grant DefCorp", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.SHIP_SHIELD, TechType.SHIP_WEAPON };
			names = new String[] { "Hsaio", "Toy-Miyazu", "Minova",
					"DiversiCorp", "Unidynamics", "Nucleomatic", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.ANY };
			names = new String[] { "enTek", "triNet", "ReadyTek", "GenTech",
					"GeneralCorp", "A.C.M.E.", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.PERSONAL_WEAPON };
			names = new String[] { "Chu", "Asan", "Noda", "Ocano", "PhaseCorp",
					"Chao-Itsuk", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.PERSONAL_SHIELD };
			names = new String[] { "Xiu-Nova", "I.B.B. Ltd", "M.O.B. Corp",
					"BoltonTek", "Maddox", "Digital Defense Corp", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.PERSONAL_SENSOR };
			names = new String[] { "cenApp", "virGenCorp", "Umin-Kahi Ltd",
					"Tyrrell", "ArcrSys", "Tsaka Corp", };
			makeManufacturer(names, types);
			types = new TechType[] { TechType.PERSONAL_WEAPON,
					TechType.PERSONAL_SENSOR, TechType.PERSONAL_SHIELD };
			names = new String[] { "MilSec", "A.O.H. Corp", "GenScience",
					"PersoTek", "StrateTek", "Maynard", };
		} else
			mob.tell("'"
					+ s
					+ "' is an unknown reset.  Try ROOM, AREA, MOBSTATS ROOM, MOBSTATS AREA *, MOBSTATS WORLD *, MOBSTATS CATALOG *, ITEMSTATS ROOM, ITEMSTATS AREA *, ITEMSTATS WORLD *, ITEMSTATS CATALOG *, AREARACEMAT *, AREAROOMIDS *, AREAINSTALL.\n\r * = Reset functions which may take a long time to complete.");
		return false;
	}
