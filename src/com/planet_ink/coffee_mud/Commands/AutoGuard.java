...
public class AutoGuard extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((!CMath.bset(mob.getBitmap(), MOB.ATT_AUTOGUARD))
				|| ((commands.size() > 0) && (((String) commands.firstElement())
						.toUpperCase().startsWith("G")))) {
			...
			if (mob.isMonster())
				CMLib.commands().postSay(mob, null, "I am now on guard.",
						false, false);
		} else {
			...
			if (mob.isMonster())
				CMLib.commands().postSay(mob, null,
						"I will now follow my group leader.", false, false);
		}
		return false;
	}
