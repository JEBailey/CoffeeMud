	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (numGold > 0) {
			...
			for (Iterator<MOB> i = H.iterator(); i.hasNext();) {
				...
				if ((!recipientM.isMonster()) && (recipientM != mob)
			...
			if ((totalAbsoluteValue * num) > CMLib.beanCounter()
					.getTotalAbsoluteValue(mob, currency)) {
				mob.tell("You don't have that much "
						+ CMLib.beanCounter().getDenominationName(currency,
								denom) + ".");
				return false;
			}
			List<Coins> V = CMLib.beanCounter().makeAllCurrency(currency,
					totalAbsoluteValue);
			...
			for (Iterator e = H.iterator(); e.hasNext();) {
				...
				for (int v = 0; v < V.size(); v++) {
					...
					C = (Coins) C.copyOf();
					mob.addItem(C);
					CMMsg newMsg = CMClass.getMsg(mob, recipient, C,
							CMMsg.MSG_GIVE,
			...
			Vector<String> v = CMParms.parse("GET " + howMuch + " FROM \""
