	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() < 2) {
			if (!mob.isInCombat()) {
				mob.tell("Kill whom?");
				return false;
			} else if (CMProps.getIntVar(CMProps.Int.COMBATSYSTEM) == CombatLibrary.COMBAT_DEFAULT)
				return false;
			else
		...
		if (CMSecurity.isAllowed(mob, mob.location(),
				CMSecurity.SecFlag.KILLDEAD) && (!mob.isMonster())) {
		...
		if (reallyKill) {
			CMMsg msg = CMClass.getMsg(mob, target, null, CMMsg.MSG_OK_ACTION,
					"^F^<FIGHT^><S-NAME> touch(es) <T-NAMESELF>.^</FIGHT^>^?");
		...
		if (mob.isInCombat()) {
			...
			if (((oldVictim != null) && (oldVictim == target) && (CMProps
					.getIntVar(CMProps.Int.COMBATSYSTEM) == CombatLibrary.COMBAT_DEFAULT))) {
				mob.tell("^f^<FIGHT^>You are already fighting "
						+ mob.getVictim().name() + ".^</FIGHT^>^?");
				return false;
			}

			if ((mob.location().okMessage(mob,
					CMClass.getMsg(mob, target, CMMsg.MSG_WEAPONATTACK, null)))
					&& (oldVictim != target)) {
				if ((oldVictim != null)
						&& (target.getVictim() == oldVictim.getVictim())
						&& (target.rangeToTarget() >= 0)
						&& (oldVictim.rangeToTarget() >= 0)) {
					int range = target.rangeToTarget()
							- oldVictim.rangeToTarget();
					if (mob.rangeToTarget() >= 0)
						range += mob.rangeToTarget();
					if (range >= 0)
						mob.setAtRange(range);
				}
				mob.tell("^f^<FIGHT^>You are now targeting " + target.name(mob)
						+ ".^</FIGHT^>^?");
		...
		if ((!mob.mayPhysicallyAttack(target)))
			...
			if (weapon == null) {
				...
				if ((possibleOtherWeapon != null)
					...
					CMLib.commands()
							.postRemove(mob, possibleOtherWeapon, false);
