...
public class Rebuke extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (target == null) {
			mob.tell("You don't see anybody called '"
					+ CMParms.combine(commands, 1)
					+ "' or you aren't serving '"
					+ CMParms.combine(commands, 1) + "'.");
			return false;
		}

		CMMsg msg = null;
		msg = CMClass.getMsg(mob, target, null, CMMsg.MSG_REBUKE,
				"<S-NAME> rebuke(s) " + target.Name() + ".");
		if (mob.location().okMessage(mob, msg))
			mob.location().send(mob, msg);
		if ((target.amFollowing() == mob) && (target.location() != null)) {
			Room R = target.location();
			msg = CMClass.getMsg(target, target.amFollowing(), null,
					CMMsg.MSG_NOFOLLOW,
