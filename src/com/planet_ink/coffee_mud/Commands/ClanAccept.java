...
public class ClanAccept extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		String memberStr = (commands.size() > 1) ? (String) commands
				.get(commands.size() - 1) : "";
		String clanName = (commands.size() > 2) ? CMParms.combine(commands, 1,
				commands.size() - 1) : "";
			...
			for (Pair<Clan, Integer> c : mob.clans())
				if ((clanName.length() == 0)
						...
						&& (c.first.getAuthority(c.second.intValue(),
								Clan.Function.ACCEPT) != Authority.CAN_NOT_DO)) {
					C = c.first;
					break;
				}
		...
		if (memberStr.length() > 0) {
			if (C == null) {
				mob.tell("You aren't allowed to accept anyone into "
						+ ((clanName.length() == 0) ? "anything" : clanName)
						+ ".");
				return false;
			}
			if (C.getGovernment().getAutoRole() == C.getGovernment()
					.getAcceptPos()) {
			...
			if (skipChecks
					|| CMLib.clans().goForward(mob, C, commands,
							Clan.Function.ACCEPT, false)) {
				List<MemberRecord> apps = C.getMemberList(C.getGovernment()
						.getAutoRole());
				if (apps.size() < 1) {
					mob.tell("There are no applicants to your "
							+ C.getGovernmentName() + ".");
					return false;
				}
				...
				if (found) {
					...
					if (M == null) {
						mob.tell(memberStr
								+ " was not found.  Could not add to "
								+ C.getGovernmentName() + ".");
						return false;
					}
					if (skipChecks
							|| CMLib.clans().goForward(mob, C, commands,
									Clan.Function.ACCEPT, true)) {
						...
						if (C.getGovernment().getEntryScript().trim().length() > 0) {
							...
							CMMsg msg2 = CMClass.getMsg(M, M, null,
						...
						CMLib.clans().clanAnnounce(
								mob,
								M.Name() + " is now a new member of "
										+ C.getGovernmentName() + " "
										+ C.name() + ".");
						mob.tell(M.Name() + " has been accepted into "
								+ C.getGovernmentName() + " '" + C.clanID()
								+ "'.");
						if ((M.session() != null) && (M.session().mob() == M))
							M.tell(mob.Name()
									+ " has accepted you as a member of "
									+ C.getGovernmentName() + " '" + C.clanID()
									+ "'.");
						return false;
					}
				} else {
					msg.append(memberStr + " isn't an applicant of your "
							+ C.getGovernmentName() + ".");
				}
			} else {
				msg.append("You aren't in the right position to accept members into your "
						+ C.getGovernmentName() + ".");
			}
		} else {
