...
public class Consider extends StdCommand {
	...
	public int relativeLevelDiff(MOB mob1, MOB mob2) {
		...
		double mob2HitRound = (((CMath
				.div(CMLib.dice().normalizeBy5(
						(int) Math.round(50.0 * mob2Attack / mob1Armor)), 100.0)) * CMath
				.div(mob2Dmg, 2.0)) + 1.0)
				* CMath.mul(mob2.phyStats().speed(), 1.0);
		double mob1HitRound = (((CMath
				.div(CMLib.dice().normalizeBy5(
						(int) Math.round(50.0 * mob1Attack / mob2Armor)), 100.0)) * CMath
				.div(mob1Dmg, 2.0)) + 1.0)
				* CMath.mul(mob1.phyStats().speed(), 1.0);
		double mob2SurvivalRounds = CMath.div(mob2Hp, mob1HitRound);
		double mob1SurvivalRounds = CMath.div(mob1Hp, mob2HitRound);

		// int
		// levelDiff=(int)Math.round(CMath.div((mob1SurvivalRounds-mob2SurvivalRounds),1));
	...
	public int doConsider(MOB mob, Physical target) {
		...
		if (R == null)
			return 0;
		CMMsg msg = CMClass.getMsg(mob, target, null, CMMsg.MASK_EYES
				| CMMsg.TYP_OK_VISUAL, null,
				"<S-NAME> consider(s) <T-NAMESELF>.",
				"<S-NAME> consider(s) <T-NAMESELF>.");
		...
		if (target instanceof MOB) {
			...
			if (lvlDiff == 0)
				...
				levelMsg = targetMOB.charStats().HeShe()
						+ " is vastly inferior to you";
			else if (lvlDiff > CMProps.getIntVar(CMProps.Int.EXPRATE))
				levelMsg = targetMOB.charStats().HeShe()
						+ " is far superior to you";
			else if (CMProps.getIntVar(CMProps.Int.EXPRATE) != 0) {
				int relLvlDiff = (lvlDiff < 0) ? -lvlDiff : lvlDiff;
				double pct = CMath.div(relLvlDiff,
						CMProps.getIntVar(CMProps.Int.EXPRATE));
				if ((lvlDiff < 0) && (pct < 0.5))
					levelMsg = targetMOB.charStats().HeShe()
							+ " is almost your equal";
				else if ((lvlDiff < 0) && (pct <= 1.0))
					levelMsg = targetMOB.charStats().HeShe()
							+ " is somewhat inferior to you";
				else if ((lvlDiff < 0))
					levelMsg = targetMOB.charStats().HeShe()
							+ " is inferior to you";
				else if ((lvlDiff > 0) && (pct < 0.5))
					levelMsg = "You are almost "
							+ targetMOB.charStats().hisher() + " equal";
				else if ((lvlDiff > 0) && (pct < 0.8))
					levelMsg = targetMOB.charStats().HeShe()
							+ " is somewhat superior to you";
				else
					levelMsg = targetMOB.charStats().HeShe()
							+ " is superior to you";
			}
		...
		for (Enumeration<Ability> a = mob.allAbilities(); a.hasMoreElements();) {
			...
			if ((A instanceof MendingSkill)
					&& (((MendingSkill) A).supportsMending((target))))
				mendors.addElement(A);
		}
		for (int m = 0; m < mendors.size(); m++) {
			Ability A = (Ability) mendors.elementAt(m);
			if (m == 0)
				withWhat.append("You could probably help " + target.name(mob)
						+ " out with your " + A.name() + " skill");
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (ID.equalsIgnoreCase("SELF") || ID.equalsIgnoreCase("ME"))
			target = mob;
		if (target == null)
			target = mob.location().fetchFromMOBRoomFavorsItems(mob, null,
					targetName, Wearable.FILTER_ANY);
