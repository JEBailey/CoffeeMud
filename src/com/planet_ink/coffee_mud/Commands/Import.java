...
public class Import extends StdCommand {
	...
	protected static String getAreaName(List<String> V) {
		...
		if ((firstLine.indexOf('~') >= 0) && (firstLine.indexOf('}') >= 0)) {
			...
			if ((lineAfter.indexOf(".are") >= 0) && (V.size() > 2)
					&& (lineAfter.indexOf("@@") < 0)) {
				lineAfter = V.get(2);
				if (lineAfter.indexOf('~') < 0)
					return "";
				...
				if (lineAfter.length() > 0)
					...
					if (lineAfter.endsWith("~"))
						lineAfter = lineAfter.substring(0,
								lineAfter.length() - 1);
		...
		if (areaName.toUpperCase().startsWith("NAME ")) {
			if ((areaName.length() >= 41) && (areaName.charAt(12) == '{')
					&& (areaName.charAt(21) != ' ')
					&& (areaName.charAt(40) == ' '))
				areaName = areaName.substring(21, 41).trim();
			else if ((areaName.length() > 13)
					&& ((areaName.charAt(12) == '{') || (areaName.charAt(13) == '{'))) {
		...
		if ((areaName.indexOf("" + ((char) 27)) >= 0)
				|| (areaName.indexOf('&') >= 0) || (areaName.indexOf('{') >= 0)
				|| (areaName.indexOf("@@") >= 0)) {
			for (int s1 = 0; s1 < colors.length; s1++)
				if (areaName.indexOf(colors[s1][0]) >= 0)
					areaName = CMStrings.replaceAll(areaName, colors[s1][0],
							colors[s1][1]);
		}
		return CMStrings.removeColors(CMLib.coffeeFilter().safetyFilter(
				areaName));
	}

	protected static Room getRoom(java.util.Map<String, Room> hashedRoomSet,
			String areaName, String calledThis) {
		if (calledThis.startsWith("#")) {
			...
			while (calledThis.startsWith(areaName + "#0")
					&& (calledThis.length() > (areaName.length() + 2)))
				calledThis = areaName + "#"
						+ calledThis.substring(areaName.length() + 2);
	...
	protected static Room getRoom(
			java.util.Map<String, Room> areaHashedRoomSet,
			java.util.Map<String, Room> hashedRoomSet, String areaName,
			String calledThis) {
		Room R = null;
	...
	protected static void processRoomRelinks(Vector reLinkTable,
			String areaName, Hashtable areaHashedRoomSet,
			Hashtable hashedRoomSet) {
		...
		if (reLinkTable != null)
			for (int r = 0; r < reLinkTable.size(); r++) {
				...
				synchronized (("SYNC" + sourceRoomID).intern()) {
					...
					Room sourceRoom = getRoom(areaHashedRoomSet, hashedRoomSet,
							areaName, sourceRoomID);
					Room destRoom = getRoom(areaHashedRoomSet, hashedRoomSet,
							areaName, destRoomID);
					if ((sourceRoom == null) || (destRoom == null))
						Log.errOut("Import", "Relink error: " + sourceRoomID
								+ "=" + sourceRoom + "/" + destRoomID + "="
								+ destRoom);
					else {
						sourceRoom.rawDoors()[direction] = destRoom;
						if (((!hashedRoomSet.containsValue(sourceRoom)))
								&& ((nextLink.length() == 0) || (!nextLink
										.startsWith(sourceRoomID + "/"))))
	...
	protected static boolean temporarilyDeleteArea(MOB mob, Vector reLinkTable,
			String areaName) {
		if ((mob != null)
				&& (CMLib.flags().isInTheGame(mob, true) && (mob.location()
						.getArea().Name().equalsIgnoreCase(areaName)))) {
				...
				if (!R.getArea().Name().equalsIgnoreCase(areaName))
					for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
						...
						if ((dirR != null)
							...
							reLinkTable.addElement(R.roomID() + "/" + d + "/"
									+ dirR.roomID());
					}
			}
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (s.toUpperCase().startsWith("#AUTHOR ")) {
				...
				if (s.endsWith("~"))
					s = s.substring(0, s.length() - 1).trim();
				if ((s.length() == 0)
						|| (s.toUpperCase().equalsIgnoreCase("NONE")))
					continue;
				return s;
				...
				if (s.trim().startsWith("{")) {
					...
					x = s.indexOf(' ');
					if ((x > 1)
							&& (s.substring(0, x).trim().toUpperCase()
									.equals("THE"))
	...
	protected static final String[][] colors = {
			...
			{ ((char) 27) + "monster" + ((char) 27), "^M" }, { "_whi ", "^w" },
			{ "_yel ", "^y" }, { "_blu ", "^b" }, { "_dbl ", "^B" },
			{ "_lbl ", "^C" }, { "_cya ", "^c" }, { "_gre ", "^g" },
			{ "_dgr ", "^G" }, { "_cha ", "^W" }, { "_dch ", "^W" },
			{ "_pur ", "^P" }, { "_red ", "^r" }, { "_ora ", "^Y" },
			{ "_bro ", "^Y" }, { "_pin ", "^p" }, { "_bla ", "" },
			{ "&x", "" }, { "&r", "^R" }, { "&g", "^G" }, { "&O", "^Y" },
			{ "&b", "^B" }, { "&p", "^P" }, { "&c", "^C" }, { "&w", "^W" },
			{ "&z", "^W" }, { "&R", "^r" }, { "&G", "^g" }, { "&Y", "^y" },
	...
	protected static String eatLineSquiggle(List<String> V) {
		...
		if ((s.indexOf("" + ((char) 27)) >= 0) || (s.indexOf('&') >= 0)
	...
	protected static boolean returnAnError(Session sess, String str,
			boolean compileErrors, Vector commands) {
	...
	protected static void importCustomFiles(MOB mob, Hashtable files,
			Set<String> customBother, boolean noPrompt, boolean noDelete)
			throws IOException {
		if (files.size() == 0)
			return;
	...
	protected static void importCustomObjects(MOB mob, List<CMObject> custom,
			Set<String> customBother, boolean noPrompt, boolean noDelete)
			throws IOException {
		if (custom.size() == 0)
			return;
		...
		for (int c = 0; c < custom.size(); c++) {
			if (custom.get(c) instanceof Race) {
					...
					if (noDelete)
						...
						if (!mob.session()
								.confirm(
										"Custom Race '"
												+ R.ID()
												+ "' found which would override your standard race.  Import anyway (Y/n)?",
										"Y"))
							continue;
					CMClass.addRace(R);
					CMLib.database().DBCreateRace(R.ID(), R.racialParms());
					...
					if (noDelete)
						...
						if (!mob.session()
								.confirm(
										"Custom Char Class '"
												+ C.ID()
												+ "' found which would override your standard class.  Import anyway (Y/n)?",
										"Y"))
							continue;
					CMClass.addCharClass(C);
					CMLib.database().DBCreateClass(C.ID(), C.classParms());
				...
				if (A2 == null) {
					...
					CMLib.database().DBCreateAbility(A.ID(),
							CMClass.getSimpleClassName(A), A.getStat("ALLXML"));
					...
					if (noDelete)
						...
						if (!mob.session()
								.confirm(
										"Custom Ability '"
												+ A.ID()
												+ "' found which would override your standard Ability.  Import anyway (Y/n)?",
										"Y"))
							continue;
					...
					if (noDelete)
						...
						if (!mob.session()
								.confirm(
										"Custom Manufacturer '"
												+ M.name()
												+ "' found which would override your existing one.  Import anyway (Y/n)?",
										"Y"))
							continue;
					eM.setXml(M.getXml());
				}
	...
	protected static String getSpell(String word, int i) {
		if ((word.trim().length() > 0)
				&& ((Character.isLetter(word.trim().charAt(0)))
						|| (word.trim().startsWith("'")) || (word.trim()
						.startsWith("`")))) {
	...
	protected static void readBlocks(List<String> buf, List<String> areaData,
			List<List<String>> roomData, List<String> mobData,
			List<String> resetData, List<String> objectData,
			List<String> mobProgData, List<String> objProgData,
			List<String> shopData, List<String> specialData,
			List<String> socialData, List<String> roomProgData) {
		...
		while (buf.size() > 0) {
			...
			if (s.startsWith("#")) {
				s = s.substring(1).trim();
				if ((useThisOne == areaData)
						&& (s.length() > 0)
						&& (CMath.isInteger(s) || Character.isDigit(s.charAt(s
								.length() - 1)))) {
					wasUsingThisOne = mobData;
					useThisOne = mobData;
				}

				if ((s.startsWith("AREA")) || (s.startsWith("AUTHOR"))) {
					...
					if ((s.indexOf('~') >= 0)
						...
						okString = true;
					useThisOne = areaData;
				} else if ((s.startsWith("MOBILE")) || (s.startsWith("MOBOLD"))) {
					wasUsingThisOne = mobData;
					useThisOne = mobData;
				} else if ((s.startsWith("OBJECT")) || (s.startsWith("OBJOLD"))
					...
					useThisOne = objectData;
						...
						|| (s.startsWith("VNUMS")) || (s.startsWith("FLAGS"))
						|| (s.startsWith("REPAIRS"))
						|| (s.startsWith("OBJFUNS")) || (s.startsWith("HELPS"))
				...
				} else if (((importNumber(s) > 0) || (s.equals("0")
						&& (buf.size() > 1) && (buf.get(1).indexOf('~') > 0)))
						&& (wasUsingThisOne != null)) {
					...
					okString = false;
						...
						&& (useThisOne == socialData))
					okString = true;
				else if ((useThisOne == mobProgData)
						|| (useThisOne == roomProgData))
					okString = true;
				else {
					// useThisOne=null;
					Log.sysOut("Import", "Suspect line (probably ok="
							+ (useThisOne != null) + "): " + s);
				}
			}
			if (useThisOne != null) {
				if (okString)
					useThisOne.add(CMLib.coffeeFilter()
							.safetyFilter(buf.get(0)));
				buf.remove(0);
			} else {
		...
		if (helpsToEat.size() > 0) {
			Log.sysOut("Import", "Ate " + helpsToEat.size()
					+ " unsupported lines.");
			helpsToEat.clear();
		}
	}

	protected static void doWeapon(Weapon I, String name, int val1,
			String str1, int val2, int val3, int val4, String str4) {
		final String[][] weaponTypes = { { "exotic", "0" }, { "sword", "1" },
				{ "dagger", "9" }, { "spear", "3" }, { "staff", "10" },
				{ "mace", "4" }, { "axe", "5" }, { "flail", "6" },
				{ "whip", "7" }, { "polearm", "8" }, { "bow", "0" },
				{ "arrow", "3" }, { "lance", "3" } };
		...
		for (int wt = 0; wt < weaponTypes.length; wt++)
			if (str1.startsWith(weaponTypes[wt][0])) {
				val1 = CMath.s_int(weaponTypes[wt][1]);
				break;
			}
		if (val1 == 0)
			for (int wt = 0; wt < weaponTypes.length; wt++)
		...
		if ((str4.trim().length() > 0)
				&& ((Character.isLetter(str4.trim().charAt(0))) || (str4.trim()
						.startsWith("'")))) {
		...
		case 8:
		case 27:
			I.setWeaponType(Weapon.TYPE_BASHING);
			break;
		case 29:
		case 55:
		case 56:
		case 57:
			I.setWeaponType(Weapon.TYPE_BURNING);
			break;
		case 22:
		case 5:
		case 10:
		...
		case 17:
		case 24:
			I.setWeaponType(Weapon.TYPE_NATURAL);
			break;
		case 2:
		case 11:
			I.setWeaponType(Weapon.TYPE_PIERCING);
			break;
		case 25:
		case 21:
		case 4:
		case 3:
		case 1:
			I.setWeaponType(Weapon.TYPE_SLASHING);
			break;

		default:
			I.setWeaponType(Weapon.TYPE_BURSTING);
			break;
		}
	}
	...
	protected static MOB getMOB(String OfThisID, Room putInRoom,
			Session session, List mobData, List mobProgData, List specialData,
			List shopData, Hashtable areaMOBS, Hashtable doneMOBS,
			String areaFileName, boolean compileErrors, Vector commands) {
		if (OfThisID.startsWith("#")) {
			if (areaMOBS.containsKey(OfThisID.substring(1))) {
				MOB M = (MOB) ((MOB) areaMOBS.get(OfThisID.substring(1)))
						.copyOf();
			...
			if (doneMOBS.containsKey(OfThisID.substring(1))) {
				MOB M = (MOB) ((MOB) doneMOBS.get(OfThisID.substring(1)))
						.copyOf();
				M.setStartRoom(putInRoom);
				M.setLocation(putInRoom);
				return M;
		...
		for (int m = 0; m < mobData.size(); m++) {
			...
			if (mobData.get(m) instanceof List)
				...
				if ((!s.toUpperCase().trim().startsWith("#MOB"))
						&& (s.length() > 0))
					returnAnError(session, "Eating mob immaterial line: "
							+ mobData.get(m), compileErrors, commands);
				continue;
			} else
				continue;
			...
			String simpleName = CMLib.coffeeFilter().safetyFilter(
					eatLineSquiggle(objV));
			String mobName = CMLib.coffeeFilter().safetyFilter(
					eatLineSquiggle(objV));
			String mobDisplay = CMLib.coffeeFilter().safetyFilter(
					eatLineSquiggle(objV));
			String mobDescription = CMLib.coffeeFilter().safetyFilter(
					eatLineSquiggle(objV));
			Race R = null;
			...
			if ((!mobID.startsWith("#"))
					|| ((mobName.length() == 0) && ((CMParms.numBits(codeStr1) < 3)
							|| (CMParms.numBits(codeStr1) > 4)
							|| (CMParms.numBits(codeStr2) < 2) || (CMParms
							.numBits(codeStr3) < 2)))) {
				returnAnError(session, "Malformed mob! Aborting this mob "
						+ mobID + ", display=" + mobDisplay + ", simple="
						+ simpleName + ", name=" + mobName + ", codeStr1="
						+ codeStr1 + ", codeStr2=" + codeStr2 + ", codeStr3="
						+ codeStr3 + "!", compileErrors, commands);
				continue;
			}
			if (mobName.length() == 0)
				mobName = "Unknown";
			...
			if (CMath.isSet(actFlag, 14) || (checkName.indexOf("GHOUL") >= 0)
			...
			for (int i = 0; i < shopData.size(); i++) {
				...
				if (("#" + s).startsWith(OfThisID + " ")
						|| ("#" + s).startsWith(OfThisID + "\t")
						|| ("#" + s).startsWith(OfThisID + "	")) {
					...
					else if ((mobName.toUpperCase().indexOf("PET ") >= 0)
							|| (mobName.toUpperCase().indexOf("PETS ") >= 0))
			...
			if (CMath.isSet(actFlag, 4))
				M.addNonUninvokableEffect(CMClass
						.getAbility("Prop_Invisibility"));
			if (CMath.isSet(actFlag, 5) && CMath.isSet(actFlag, 1))
				M.addBehavior(CMClass.getBehavior("Aggressive"));
			M.setWimpHitPoint(0);
			if (CMath.isSet(actFlag, 7)) // this needs to be adjusted further
											// down!
				M.setWimpHitPoint(2);
			...
			if (CMath.isSet(affFlag, 0))
				M.basePhyStats().setSensesMask(
						M.basePhyStats().sensesMask() | PhyStats.CAN_NOT_SEE);
			if (CMath.isSet(affFlag, 1))
				M.basePhyStats().setDisposition(
						M.basePhyStats().disposition() | PhyStats.IS_INVISIBLE);
			if (CMath.isSet(affFlag, 2))
				M.basePhyStats().setSensesMask(
						M.basePhyStats().sensesMask() | PhyStats.CAN_SEE_EVIL);
			...
			if (CMath.isSet(affFlag, 4))
				M.basePhyStats().setSensesMask(
						M.basePhyStats().sensesMask() | PhyStats.CAN_SEE_BONUS);
			...
			if (CMath.isSet(affFlag, 6)) {
				if (CMLib.flags().isEvil(M))
					M.addNonUninvokableEffect(CMClass
							.getAbility("Prayer_UnholyWord"));
				else
					M.addNonUninvokableEffect(CMClass
							.getAbility("Prayer_HolyWord"));
				M.addNonUninvokableEffect(CMClass
						.getAbility("Prayer_Sanctuary"));
			...
			if (CMath.isSet(affFlag, 15)) {
				Ability A = (Ability) CMClass.getAbility("Thief_Sneak")
						.copyOf();
			...
			if (CMath.isSet(affFlag, 25))
				M.basePhyStats().setSensesMask(
						M.basePhyStats().sensesMask() | PhyStats.CAN_SEE_DARK);
			...
			if (CMParms.numBits(codeStr2) >= 4) {
				M.basePhyStats().setLevel(
						CMath.s_int(CMParms.getCleanBit(codeStr2, 0).trim()));
				...
				if (circleFormat) {
					if (CMath.isNumber(CMParms.getCleanBit(codeStr4, 2).trim()))
						sexCode = CMath.s_int(CMParms.getCleanBit(codeStr4, 2)
								.trim());
					else if (CMParms.getCleanBit(codeStr4, 2).trim()
							.toUpperCase().equals("MALE"))
						sexCode = 1;
					else if (CMParms.getCleanBit(codeStr4, 2).trim()
							.toUpperCase().equals("FEMALE"))
						sexCode = 2;
					else if (CMParms.getCleanBit(codeStr4, 2).trim()
							.toUpperCase().equals("EITHER"))
						sexCode = (CMLib.dice().rollPercentage() > 50) ? 1 : 2;
					else
						sexCode = 3;

					if (CMath.isNumber(CMParms.getCleanBit(codeStr4, 0).trim()))
						positionCode = CMath.s_int(CMParms.getCleanBit(
								codeStr4, 2).trim());
					else if (CMParms.getCleanBit(codeStr4, 0).trim()
							.startsWith("STAND"))
						positionCode = 8;
					else if (CMParms.getCleanBit(codeStr4, 0).trim()
							.startsWith("SIT"))
			...
			case 4:
				M.basePhyStats().setDisposition(
						M.basePhyStats().disposition() | PhyStats.IS_SLEEPING);
				break;
			case 5:
				M.basePhyStats().setDisposition(
						M.basePhyStats().disposition() | PhyStats.IS_SITTING);
				break;
			case 6:
				M.basePhyStats().setDisposition(
						M.basePhyStats().disposition() | PhyStats.IS_SITTING);
			...
			if (circleFormat) {
				...
				if (CMath.isSet(off, 19)) {
					...
					for (int b = M.numBehaviors() - 1; b >= 0; b--) {
						...
						if ((B != null)
								&& (CMath.bset(B.flags(),
										Behavior.FLAG_MOBILITY))) {
							if (guardian.ID().equals("GoodGuardian"))
								guardian = CMClass
										.getBehavior("MobileGoodGuardian");
				...
				if (CMath.isSet(res, 14))
					M.addNonUninvokableEffect(CMClass
							.getAbility("Spell_ResistMagicMissiles"));
				...
				if (resist.text().length() > 0) {
					resist.setMiscText(resist.text() + " "
							+ (10 + M.basePhyStats().level()) + "%");
			...
			while (objV.size() > 0) {
				...
				if (s.startsWith(">")) {
					...
					if (!s.substring(1).trim().toUpperCase()
							.startsWith("IN_FILE_PROG")) {
					...
					if (B != null) {
						if (B.getParms().length() == 0)
							...
							B.setParms(B.getParms() + ";"
									+ codeLine.substring(1).trim());
					}
			...
			for (int mp = 0; mp < mobProgData.size(); mp++) {
				...
				if (s.startsWith("M ")) {
					...
					if (mobprg.equals("JANITOR.PRG"))
						...
						try {
							CMFile F2 = new CMFile(areaFileName, M,
									CMFile.FLAG_LOGERRORS);
							if ((F2.exists()) && (!F2.isDirectory())) {
								int x = F2.getAbsolutePath().lastIndexOf('/');
								String path = F2.getAbsolutePath().substring(0,
										x)
										+ "/" + mobprg;
								StringBuffer buf = new CMFile(path, M,
										CMFile.FLAG_LOGERRORS).text();
								if ((buf == null) || (buf.length() == 0))
									returnAnError(session, "Unknown MobPrg: "
											+ mobprg, compileErrors, commands);
								else {
									if (buf.indexOf("\n") > 0)
										...
										buf = new StringBuffer(
												CMStrings.replaceAll(
														buf.toString(), "\n",
														""));
									List<String> V = Resources
											.getFileLineVector(buf);
									while (V.size() > 0) {
										s = nextLine(V);
										if (s.startsWith(">")) {
											s = eatLineSquiggle(V).substring(1)
													.trim();
											scriptStuff += s + ";";
											s = nextLine(V);
											while (s.indexOf('~') < 0) {
							...
							returnAnError(session, "Unknown MobPrg: " + mobprg,
									compileErrors, commands);
						}
					}
					...
					returnAnError(session, "MobPrg line: " + s, compileErrors,
							commands);
			}
			...
			for (int mp = 0; mp < specialData.size(); mp++) {
				...
				if (s.startsWith("M ")) {
					...
					if ((special.equals("SPEC_CAST_MAGE"))
						...
						M.addAbility(CMClass
								.getAbility("Undead_WeakEnergyDrain"));
						...
						M.addBehavior(CMClass.getBehavior("CombatAbilities"));
						M.addAbility(CMClass
								.getAbility("Prayer_SummonElemental"));
						...
						returnAnError(session, "Unknown mob special: "
								+ special, compileErrors, commands);
				} else if ((s.startsWith("#SPE"))
						|| (s.startsWith("S"))
						|| (s.startsWith("*") || (s.startsWith("#$")) || (s
								.startsWith("R ")))) {
				} else if (s.trim().length() > 0)
					returnAnError(session, "Unknown mob special line: " + s,
							compileErrors, commands);
			}
			...
			long rejuv = CMProps.getTicksPerMinute()
					...
					+ (CMProps.getTicksPerMinute() * M.basePhyStats().level() / 2);
			...
			if (B != null) {
				Behavior BB = B;
				for (Enumeration<Behavior> e = M.behaviors(); e
						.hasMoreElements();) {
					B = e.nextElement();
					if ((B != null)
							&& (B.getClass().getSuperclass().getName()
									.endsWith("CombatAbilities"))) {
						M.delBehavior(BB);
						M.recoverPhyStats();
						break;
					}
	...
	private static void applyItemApplyCode(String codesLine, Item I,
			Environmental adjuster, Environmental caster, Environmental resister) {
		int num = CMath.s_int(CMParms.getCleanBit(codesLine, 0).trim());
		int val = CMath.s_int(CMParms.getCleanBit(codesLine, 1).trim());
		switch (num) {
		case 1:
			adjuster.setMiscText(adjuster.text() + " str"
					+ ((val >= 0) ? ("+" + val) : ("" + val)));
			break;
		case 2:
			adjuster.setMiscText(adjuster.text() + " dex"
					+ ((val >= 0) ? ("+" + val) : ("" + val)));
			break;
		case 3:
			adjuster.setMiscText(adjuster.text() + " int"
					+ ((val >= 0) ? ("+" + val) : ("" + val)));
			break;
		case 4:
			adjuster.setMiscText(adjuster.text() + " wis"
					+ ((val >= 0) ? ("+" + val) : ("" + val)));
			break;
		case 5:
			adjuster.setMiscText(adjuster.text() + " con"
					+ ((val >= 0) ? ("+" + val) : ("" + val)));
			break;
		case 6:
			...
			break;
		case 7:
			adjuster.setMiscText(adjuster.text() + " cha"
					+ ((val >= 0) ? ("+" + val) : ("" + val)));
			break;
		case 8:
			break;
		...
		case 11:
			break;
		case 12:
			adjuster.setMiscText(adjuster.text() + " mana"
					+ ((val >= 0) ? ("+" + val) : ("" + val)));
			break;
		case 13:
			adjuster.setMiscText(adjuster.text() + " hit"
					+ ((val >= 0) ? ("+" + val) : ("" + val)));
			break;
		case 14:
			adjuster.setMiscText(adjuster.text() + " move"
					+ ((val >= 0) ? ("+" + val) : ("" + val)));
			break;
		case 15:
			break;
		...
		case 17:
			if ((val > 0) && (I instanceof Armor))
				...
				adjuster.setMiscText(adjuster.text() + " armor"
						+ ((val >= 0) ? ("+" + (val * 5)) : ("" + (val * 5))));
			break;
		case 18:
			if ((val > 0) && (I instanceof Weapon))
				I.basePhyStats().setAttackAdjustment(
						I.basePhyStats().attackAdjustment() + (val * 5));
			else
				adjuster.setMiscText(adjuster.text() + " attack"
						+ ((val >= 0) ? ("+" + (val * 5)) : ("" + (val * 5))));
			break;
		case 19:
			if ((val > 0) && (I instanceof Weapon))
				...
				adjuster.setMiscText(adjuster.text() + " damage"
						+ ((val >= 0) ? ("+" + (val)) : ("" + (val))));
			break;
	...
	private static Item getItem(String OfThisID, Session session,
			String areaName, List objectData, List objProgData,
			Hashtable areaItems, Hashtable doneItems, Hashtable areaRooms,
			Hashtable doneRooms, boolean compileErrors, Vector commands) {
		if (OfThisID.startsWith("#")) {
			if (areaItems.containsKey(OfThisID.substring(1)))
				return (Item) ((Item) areaItems.get(OfThisID.substring(1)))
						.copyOf();
			if (doneItems.containsKey(OfThisID.substring(1)))
				return (Item) ((Item) doneItems.get(OfThisID.substring(1)))
						.copyOf();
		} else {
		...
		for (int o = 0; o < objectData.size(); o++) {
			...
			if (objectData.get(o) instanceof List)
				...
				if ((!s.toUpperCase().trim().startsWith("#OBJ"))
						&& (s.length() > 0))
					returnAnError(session, "Eating immaterial line: "
							+ objectData.get(o) + ", area=" + areaName,
							compileErrors, commands);
				continue;
			} else
				continue;
			...
			String simpleName = CMLib.coffeeFilter().safetyFilter(
					eatLineSquiggle(objV));
			String objectName = CMLib.coffeeFilter().safetyFilter(
					eatLineSquiggle(objV));
			String objectDisplay = CMLib.coffeeFilter().safetyFilter(
					eatLineSquiggle(objV));
			String objectDescription = "";
			if ((nextLine(objV).indexOf('~') >= 0)
					|| ((nextLine(objV).length() > 0) && (!Character
							.isDigit(nextLine(objV).charAt(0)))))
				objectDescription = CMLib.coffeeFilter().safetyFilter(
						eatLineSquiggle(objV));
			String codeStr1 = eatNextLine(objV);
			String codeStr2 = eatNextLine(objV);
			String codeStr3 = eatNextLine(objV);
			...
			if ((!objectID.startsWith("#"))
					|| ((objectName.length() == 0) && ((CMParms
							.numBits(codeStr1) < 3)
							|| (CMParms.numBits(codeStr1) > 4)
							|| (CMParms.numBits(codeStr2) < 4) || (codeStr3
							.length() == 0)))) {
				returnAnError(session,
						"Malformed object! Aborting this object " + objectID
								+ ", display=" + objectDisplay + ", simple="
								+ simpleName + ", name=" + objectName
								+ ", codeStr1=" + codeStr1 + ", codeStr2="
								+ codeStr2 + ", codeStr3=" + codeStr3
								+ ", area=" + areaName, compileErrors, commands);
				continue;
			}
			...
			if ((obj.trim().length() > 1)
					&& (Character.isLetter(obj.charAt(0))))
				circleForm = true;
			int objType = CMath.s_int(obj);
			final String[][] itemTypes = { { "light", "1" }, { "scroll", "2" },
					{ "wand", "3" }, { "staff", "4" }, { "weapon", "5" },
					{ "treasure", "8" }, { "armor", "9" }, { "potion", "10" },
					{ "clothing", "11" }, { "furniture", "12" },
					{ "trash", "13" }, { "container", "15" },
					{ "drink", "17" }, { "key", "18" }, { "food", "19" },
					{ "money", "20" }, { "boat", "22" },
					{ "npc_corpse", "99" }, { "pc_corpse", "99" },
					{ "fountain", "25" }, { "pill", "26" }, { "protect", "" },
					{ "map", "28" }, { "portal", "97" }, { "warp_stone", "" },
					{ "room_key", "98" }, { "gem", "" }, { "jewelry", "" },
					{ "jukebox", "" }, { "tattoo", "" }, { "pipe", "32" } };
			...
			if ((codeStr2.indexOf('~') >= 0)
					&& (codeStr2.lastIndexOf('~') > codeStr2.indexOf('~'))) {
				...
				if (V.size() == 4) {
					...
					returnAnError(session, "Invalid object codeStr2 line: "
							+ codeStr2
							+ ", item not aborted, but stuff will be wrong!",
							compileErrors, commands);
			...
			case 2:
				...
				((SpellHolder) I).setSpellList(getSpell(str2, val2) + ";"
						+ getSpell(str3, val3) + ";" + getSpell(str4, val4));
				break;
			case 3:
			...
			case 5:
				...
				doWeapon((Weapon) I, objectName, val1, str1, val2, val3, val4,
						str4);
				adjuster = CMClass.getAbility("Prop_WearAdjuster");
				break;
			case 6:
			...
			case 9:
				...
				I.basePhyStats().setArmor(
						(int) Math.round(CMath.div(
								(val1 + val2 + val3 + val4 + 1), 4.0) + 1));
				adjuster = CMClass.getAbility("Prop_WearAdjuster");
				break;
			case 10:
				I = CMClass.getMiscMagic("GenPotion");
				I.basePhyStats().setLevel(val1);
				((Potion) I).setSpellList(getSpell(str2, val2) + ";"
						+ getSpell(str3, val3) + ";" + getSpell(str4, val4));
				break;
			case 11:
				I = CMClass.getArmor("GenArmor");
			...
			case 15:
				if (CMLib.english().containsString(objectName, "belt")
						|| CMLib.english().containsString(objectName,
								"bandolier")
			...
			case 17:
				...
				if (((val3 > 0) && (val3 < 6)) || (str3.indexOf("BEER") >= 0)
						|| (str3.indexOf("ALE") >= 0)
						|| (str3.indexOf("BREW") >= 0)
						|| (str3.indexOf("WINE") >= 0)) {
			...
			case 26:
				I = CMClass.getMiscMagic("GenPill");
				I.basePhyStats().setLevel(val1);
				((Pill) I).setSpellList(getSpell(str2, val2) + ";"
						+ getSpell(str3, val3) + ";" + getSpell(str4, val4));
				break;
			case 27:
				I = CMClass.getBasicItem("GenItem");
			...
			if (CMath.isSet(wearFlag, 1))
				I.setRawProperLocationBitmap(Wearable.WORN_LEFT_FINGER
						| Wearable.WORN_RIGHT_FINGER
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 2))
				I.setRawProperLocationBitmap(Wearable.WORN_NECK
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 3))
				I.setRawProperLocationBitmap(Wearable.WORN_TORSO
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 4))
				I.setRawProperLocationBitmap(Wearable.WORN_HEAD
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 5))
				I.setRawProperLocationBitmap(Wearable.WORN_LEGS
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 6))
				I.setRawProperLocationBitmap(Wearable.WORN_FEET
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 7))
				I.setRawProperLocationBitmap(Wearable.WORN_HANDS
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 8))
				I.setRawProperLocationBitmap(Wearable.WORN_ARMS
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 9))
				I.setRawProperLocationBitmap(Wearable.WORN_HELD
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 10))
				I.setRawProperLocationBitmap(Wearable.WORN_ABOUT_BODY
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 11))
				I.setRawProperLocationBitmap(Wearable.WORN_WAIST
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 12))
				I.setRawProperLocationBitmap(Wearable.WORN_LEFT_WRIST
						| Wearable.WORN_RIGHT_WRIST
						| I.rawProperLocationBitmap());
			if (CMath.isSet(wearFlag, 13))
				I.setRawProperLocationBitmap(Wearable.WORN_WIELD
						| Wearable.WORN_HELD | I.rawProperLocationBitmap());
			...
			if (CMath.isSet(wearFlag, 17))
				I.setRawProperLocationBitmap(Wearable.WORN_EARS
						| I.rawProperLocationBitmap());
			...
			// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
			// the relation of this stuff is vital! must follow properlocation
			// setting
			...
			if (CMParms.numBits(codeStr3) > 2) {
				int level = CMath
						.s_int(CMParms.getCleanBit(codeStr3, 0).trim());
				I.basePhyStats().setLevel(level);
				I.basePhyStats()
						.setWeight(
								CMath.s_int(CMParms.getCleanBit(codeStr3, 1)
										.trim()) / 10);
				...
				if (I instanceof Rideable)
					I.basePhyStats()
							.setWeight(
									CMath.s_int(CMParms
											.getCleanBit(codeStr3, 1).trim()) * 10);
			...
			if ((I instanceof Weapon)
					&& ((objectName.toUpperCase().indexOf("TWO-HANDED") >= 0) || (objectName
							.toUpperCase().indexOf("TWO HANDED") >= 0))) {
				I.setRawLogicalAnd(true);
				I.setRawProperLocationBitmap(Wearable.WORN_HELD
						| I.rawProperLocationBitmap());
			}
			...
			if ((((I.material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_CLOTH) || ((I
			...
			if ((I instanceof Container)
				...
				if (CMLib.english().containsString(objectName, "belt")
						...
						|| CMLib.english().containsString(objectName, "sheath")) {
					((Container) I)
							.setContainTypes(Container.CONTAIN_ONEHANDWEAPONS);
					if (((Container) I).capacity() - I.basePhyStats().weight() < 30)
						((Container) I)
								.setCapacity(I.basePhyStats().weight() + 30);
					...
					if (((Container) I).capacity() - I.basePhyStats().weight() < 10)
						((Container) I)
								.setCapacity(I.basePhyStats().weight() + 10);
				}
			}

			if (CMath.isSet(extraFlag, 0))
				I.basePhyStats().setDisposition(
						I.basePhyStats().disposition() | PhyStats.IS_GLOWING);

			// if((extraFlag&2)==2) coffeemud has no hummers
			if (CMath.isSet(extraFlag, 2))
				I.basePhyStats().setDisposition(
						I.basePhyStats().disposition() | PhyStats.IS_DARK);

			if (CMath.isSet(extraFlag, 4))
				I.basePhyStats().setDisposition(
						I.basePhyStats().disposition() | PhyStats.IS_EVIL);

			if (CMath.isSet(extraFlag, 5))
				I.basePhyStats().setDisposition(
						I.basePhyStats().disposition() | PhyStats.IS_INVISIBLE);

			if (CMath.isSet(extraFlag, 6))
				I.basePhyStats().setDisposition(
						I.basePhyStats().disposition() | PhyStats.IS_BONUS);
			...
			if (CMath.isSet(extraFlag, 11))
				prop_WearZapper.setMiscText(prop_WearZapper.text()
						+ " -neutral");
			...
			// if(extraFlag&16384)==16384) coffeemud doesn't support rotting
			// cargo
			...
			if (CMath.isSet(extraFlag, 17))
				I.basePhyStats().setDisposition(
						I.basePhyStats().disposition() | PhyStats.IS_GOOD);
			...
			while (objV.size() > 0) {
				...
				if (codeLine.equals("E")) {
					...
					if ((CMClass.getItem(I.ID()) != null)
							&& (I.description().equals(CMClass.getItem(I.ID())
									.description())))
						I.setDescription("");
					...
					for (int y = 0; y < objV.size(); y++) {
						...
						if (ts.equals("A") || ts.equals("E") || ts.equals("L")
								|| ts.equals("F")) {
							objV.add(y, "~");
							squiggleFound = true;
							break;
					...
					if (!squiggleFound)
						objV.add("~");
					String desc = CMLib.coffeeFilter().safetyFilter(
							eatLineSquiggle(objV));
					...
					if (CMParms.numBits(codesLine) != 2)
						returnAnError(session, "Malformed 'A' code for item "
								+ objectID + ", " + I.Name() + ": " + codesLine
								+ ", area=" + areaName, compileErrors, commands);
					else
						applyItemApplyCode(codesLine, I, adjuster, caster,
								resister);
				} else if (codeLine.equals("F")) {
					eatNextLine(objV);
					String codesLine = eatNextLine(objV);
					if (CMParms.numBits(codesLine) != 4)
						returnAnError(session, "Malformed 'F' code for item "
								+ objectID + ", " + I.Name() + ": " + codesLine
								+ ", area=" + areaName, compileErrors, commands);
					else {
						...
						if (codeType.equals("V")) {
							...
							String[] resistances = { " teleport", " mind",
									" magic", " weapons", " blunt", " pierce",
									" slash", " fire", " cold", " elec",
									" acid", " poison", " evil", " holy", "",
									" mind", " disease", " gas" };
							for (int rei = 0; rei < resistances.length; rei++) {
								if ((CMath.isSet(res, rei))
										&& (resistances[rei].length() > 0))
									resister.setMiscText(resister.text()
											+ resistances[rei] + " -25%");
								else if ((CMath.isSet(imm, rei))
										&& (resistances[rei].length() > 0))
									resister.setMiscText(resister.text()
											+ resistances[rei] + " -100%");
							}
							...
							if (CMath.isSet(codeBits, 5))
								sense = sense | PhyStats.CAN_SEE_HIDDEN
										| PhyStats.CAN_SEE_SNEAKERS;
							if (CMath.isSet(codeBits, 6))
								caster.setMiscText(caster.text()
										+ ("Spell_IronGrip") + ";");
							...
							// if(CMath.isSet(codeBits,18)) item cannot charm
							// you
							// caster.setMiscText(caster.text()+(new
							// Poison().ID())+";");
							...
							if (CMath.isSet(codeBits, 29))
								caster.setMiscText(caster.text()
										+ ("Spell_Slow") + ";");
							if (sense > 0)
								adjuster.setMiscText(adjuster.text() + " sen+"
										+ sense);
							if (dis > 0)
								adjuster.setMiscText(adjuster.text() + " dis+"
										+ dis);
						} else {
							long res = getBitMask(codesLine, 3);
							long imm = getBitMask(codesLine, 3);
					...
					codeLine = eatLineSquiggle(objV);
					String scriptStuff = "";
					if (!codeLine.substring(1).trim().toUpperCase()
							.startsWith("IN_FILE_PROG")) {
				...
				} else if ((forgiveZeroes) && (codeLine.length() > 0)
						&& (CMath.isNumber("" + codeLine.charAt(0)))
						&& (CMParms.numBits(codeLine) > 1)) {
					eatNextLine(objV);
					// eos support
				} else if ((forgiveZeroes) && (codeLine.endsWith("~"))) {
					eatNextLine(objV);
					// eos support
				} else if ((codeLine.startsWith("#"))
						|| (codeLine.length() == 0)) {
					...
					String desc = CMLib.coffeeFilter().safetyFilter(
							eatLineSquiggle(objV)); // description
					I.setDescription(I.description() + desc);
					if (I.ID().equals("GenReadable"))
						I.setReadableText(fixReadableContent(I.description()));
					...
					eatNextLine(objV);
					returnAnError(session, "Unknown code for item " + objectID
							+ ", " + I.Name() + ": " + codeLine + ", area="
							+ areaName, compileErrors, commands);
				}
			}
	...
	public static Room applyRoomCodeBits(Room R, Room lastRoom,
			Map<Room, Room> petShops, long codeBits, boolean circleFormat) {
		...
		// if(CMath.isSet(codeBits,1)) //BANKS are forked up in the ROM files,
		// who knows WHAT this is...
		// circlemud says this is a death trap -- well, homie dont play dat
		// either
		...
		if (circleFormat) {
			...
			if (CMath.isSet(codeBits, 7)) // circle says no magic
				R.addNonUninvokableEffect(CMClass
						.getAbility("Prop_MagicFreedom"));
			...
			if (CMath.isSet(codeBits, 12))
				petShops.put(R, R);
			else if ((lastRoom != null) && (petShops.get(lastRoom) != null)
					&& (petShops.get(lastRoom) == lastRoom)) {
			...
			if (CMath.isSet(codeBits, 20))
				R.addNonUninvokableEffect(CMClass
						.getAbility("Prop_NoTeleportOut"));

			// if(CMath.isSet(codeBits,23)) No "dirt" in CoffeeMud, so this
			// doesn't matter
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((commands.size() > 0)
				&& (commands.lastElement() instanceof StringBuffer)) {
			compileErrors = true;
			session = null;
		} else {
			if (session == null)
				return false;
			if (commands.size() < 1)
				return returnAnError(session,
						"Import what?  Specify the path/filename!",
						compileErrors, commands);
		...
		while (commands.size() > 0) {
			...
			if (O instanceof StringBuffer) {
				...
				i.closeEntry();
				if (commands.size() == 0) {
					commands.add(new Pair<String, StringBuffer>("unzip:"
							+ entry.getName(), new StringBuffer(
							new String(data))));
					commands.add(i);
				} else {
					commands.add(0, new Pair<String, StringBuffer>("unzip:"
							+ entry.getName(), new StringBuffer(
							new String(data))));
					commands.add(1, i);
				}
				...
				final String ext = (x > 0) ? areaFileName.toLowerCase()
						.substring(x + 1) : "";
				// read in the .are file

				if (ext.equalsIgnoreCase("zip")) {
					java.util.zip.ZipInputStream i = new java.util.zip.ZipInputStream(
							new CMFile(areaFileName, mob, CMFile.FLAG_LOGERRORS)
									.getRawStream());
					if (commands.size() == 0)
						commands.add(i);
					else
				...
				if (ext.equals("zon") || ext.endsWith("wld")
						|| ext.endsWith("obj") || ext.endsWith("mob")
						|| ext.endsWith("shp") || ext.endsWith("doc")) {
					final String baseAreaFileName = areaFileName
							.substring(0, x);
					if (baseFilesAlreadyDone.contains(baseAreaFileName
							.toLowerCase()))
						continue;
					...
					if ((buf != null) && (buf.length() > 0)) {
						...
						if (wldDivDex < 0)
							return returnAnError(session,
									"Malformed Zone at: '" + areaFileName
											+ "'!", compileErrors, commands);
						int startDex = 0;
						if (buf.charAt(0) == '#')
							startDex = buf.indexOf(eoln) + eoln.length();
						finalBuf.append("#AREA").append(eoln)
								.append(buf.substring(startDex, wldDivDex))
								.append(eoln).append(eoln);
						resetsStr = "#RESETS"
								+ eoln
								+ buf.substring(wldDivDex + (eoln.length() * 2))
								+ eoln + eoln;
						areaFileName = baseAreaFileName + ".wld";
						CF = new CMFile(areaFileName, mob,
								CMFile.FLAG_LOGERRORS);
						buf = CF.text();
					}
					if ((buf != null) && (buf.length() > 0)) {
						finalBuf.append("#ROOMS").append(eoln)
								.append(buf.toString()).append(eoln)
								.append(eoln);
					...
					if ((buf != null) && (buf.length() > 0)) {
						finalBuf.append("#MOBILES").append(eoln)
								.append(buf.toString()).append(eoln)
								.append(eoln);
					...
					if ((buf != null) && (buf.length() > 0)) {
						finalBuf.append("#OBJECTS").append(eoln)
								.append(buf.toString()).append(eoln)
								.append(eoln);
				...
				if ((buf == null) || (buf.length() == 0))
					return returnAnError(session, "File not found at: '"
							+ areaFileName + "'!", compileErrors, commands);
			}
			try {
				if ((areaFileName != null)
						&& (areaFileName.toUpperCase().trim().endsWith(".LST"))) {
					if (session != null)
						session.println("Unpacking areas lists from file : '"
								+ areaFileName + "'...");
					String filePrefix = "";
				...
				if ((buf != null) && (buf.length() > 20)
						&& (buf.substring(0, 20).indexOf("<AREAS>") >= 0)) {
					if (!CMSecurity.isAllowedEverywhere(mob,
							CMSecurity.SecFlag.IMPORTROOMS)) {
						returnAnError(session,
								"You are not allowed to import areas in '"
										+ areaFileName + "'.", compileErrors,
								commands);
						continue;
					}
					...
					if (session != null)
						session.rawPrint("Unpacking area(s) from file: '"
								+ areaFileName + "'...");
					String error = CMLib.coffeeMaker().fillAreasVectorFromXML(
							buf.toString(), areas, custom, externalFiles);
					if (error.length() == 0)
						importCustomObjects(mob, custom, customBotherChecker,
								!prompt, nodelete);
					if (error.length() == 0)
						importCustomFiles(mob, externalFiles,
								customBotherChecker, !prompt, nodelete);
					if (error.length() > 0)
						return false;
					if (session != null)
						session.rawPrintln("!");
					if (session != null)
						session.println("Found " + areas.size() + " areas.");
					int num = areas.size();
					int a = 0;
					while (areas.size() > 0) {
						if (session != null)
							session.rawPrint("Unpacking area #" + (a + 1) + "/"
									+ num + "...");
						List<XMLLibrary.XMLpiece> area = areas.get(0);
						error = CMLib.coffeeMaker().unpackAreaFromXML(area,
								session, areaType, true);
						if (session != null)
							session.rawPrintln("!");
						if (error.startsWith("Area Exists: ")) {
							String areaName = error.substring(13).trim();
							if ((nodelete) && (!prompt))
								return returnAnError(session, "Area '"
										+ areaName + "' already exists.",
										compileErrors, commands);
							else if (((!prompt) || ((session != null) && session
									.confirm("Area: \"" + areaName
											+ "\" exists, obliterate first?",
											"N")))) {
								if (reLinkTable == null)
									reLinkTable = new Vector();
								if ((mob.location() != null)
										&& (mob.location().getArea().Name()
												.equalsIgnoreCase(areaName))) {
									try {
										for (Enumeration r = CMLib.map()
												.rooms(); r.hasMoreElements();) {
											Room R = (Room) r.nextElement();
											if ((R != null)
													&& (!R.getArea()
															.Name()
								...
								if (!temporarilyDeleteArea(mob, reLinkTable,
										areaName))
									return false;
							} else
								return false;
						} else if (error.length() > 0)
							return returnAnError(
									session,
									"An error occurred on import: "
											+ error
											+ "\n\rPlease correct the problem and try the import again.",
									compileErrors, commands);
						else {
							areas.remove(area);
							a++;
					...
					Log.sysOut("Import", mob.Name() + " imported "
							+ areaFileName);
					if (session != null)
						session.println("Area(s) successfully imported!");
					continue;
				} else if ((buf != null) && (buf.length() > 20)
						&& (buf.substring(0, 20).indexOf("<AREA>") >= 0)) {
					if (!CMSecurity.isAllowedEverywhere(mob,
							CMSecurity.SecFlag.IMPORTROOMS)) {
						returnAnError(session,
								"You are not allowed to import area in '"
										+ areaFileName + "'.", compileErrors,
								commands);
						continue;
					}
					if (CF != null)
						buf = CF.textUnformatted();
					if (session != null)
						session.rawPrint("Unpacking area from file: '"
								+ areaFileName + "'...");
					Vector areaD = new Vector();
					String error = CMLib.coffeeMaker()
							.fillAreaAndCustomVectorFromXML(buf.toString(),
									areaD, custom, externalFiles);
					if (error.length() == 0)
						importCustomObjects(mob, custom, customBotherChecker,
								!prompt, nodelete);
					if (error.length() == 0)
						importCustomFiles(mob, externalFiles,
								customBotherChecker, !prompt, nodelete);
					if (error.length() == 0)
						error = CMLib.coffeeMaker().unpackAreaFromXML(areaD,
								session, areaType, true);
					if (session != null)
						session.rawPrintln("!");
					if (error.startsWith("Area Exists: ")) {
						String areaName = error.substring(13).trim();
						if ((nodelete) && (!prompt))
							return returnAnError(session, "Area '" + areaName
									+ "' already exists.", compileErrors,
									commands);
						else if ((!prompt)
					...
					Log.sysOut("Import", mob.Name() + " imported "
							+ areaFileName);
					if (session != null)
						session.println("Area successfully imported!");
					continue;
				} else if ((buf != null) && (buf.length() > 20)
					...
					if (!CMSecurity.isAllowedEverywhere(mob,
							CMSecurity.SecFlag.IMPORTROOMS)) {
						returnAnError(session,
								"You are not allowed to import room in '"
										+ areaFileName + "'.", compileErrors,
								commands);
						continue;
					}
					if (CF != null)
						buf = CF.textUnformatted();
					if (session != null)
						session.println("Unpacking room from file: '"
								+ areaFileName + "'...");
					String error = CMLib.coffeeMaker().fillCustomVectorFromXML(
							buf.toString(), custom, externalFiles);
					if (error.length() == 0)
						importCustomObjects(mob, custom, customBotherChecker,
								!prompt, nodelete);
					if (error.length() == 0)
						importCustomFiles(mob, externalFiles,
								customBotherChecker, !prompt, nodelete);
					if (error.length() == 0)
						error = CMLib.coffeeMaker().unpackRoomFromXML(
								buf.toString(), true);
					if (error.startsWith("Room Exists: ")) {
						Room R = CMLib.map()
								.getRoom(error.substring(13).trim());
						if (R != null) {
							if ((nodelete) && (!prompt))
								return returnAnError(session, "Room '" + R.ID()
										+ "' already exists.", compileErrors,
										commands);
							reLinkTable = new Vector();
							try {
								for (Enumeration r = CMLib.map().rooms(); r
									...
									if (R2 != R)
										for (int d = Directions
												.NUM_DIRECTIONS() - 1; d >= 0; d--) {
											Room dirR = R2.rawDoors()[d];
											if ((dirR != null) && (dirR == R))
												reLinkTable.addElement(R2
														.roomID()
														+ "/"
														+ d
														+ "/" + dirR.roomID());
					...
					Log.sysOut("Import", mob.Name() + " imported "
							+ areaFileName);
					if (session != null)
						session.println("Room successfully imported!");
					continue;
				} else if ((buf != null) && (buf.length() > 20)
						&& (buf.substring(0, 20).indexOf("<MOBS>") >= 0)) {
					if (!CMSecurity.isAllowed(mob, mob.location(),
							CMSecurity.SecFlag.IMPORTMOBS)) {
						returnAnError(session,
								"You are not allowed to import mobs in '"
										+ areaFileName + "' here.",
								compileErrors, commands);
						continue;
					}
					if (CF != null)
						buf = CF.textUnformatted();
					if (session != null)
						session.rawPrint("Unpacking mobs from file: '"
								+ areaFileName + "'...");
					...
					if (error.length() == 0)
						importCustomFiles(mob, externalFiles,
								customBotherChecker, !prompt, nodelete);
					if (error.length() == 0)
						error = CMLib.coffeeMaker().addMOBsFromXML(
								buf.toString(), mobs, session);
					if (session != null)
						session.rawPrintln("!");
					if (error.length() > 0)
						return returnAnError(
					...
					if (mob.location() == null)
						return returnAnError(session,
								"You must be in a room to import mobs.",
								compileErrors, commands);
					...
					Log.sysOut("Import", mob.Name() + " imported "
							+ areaFileName);
					if (session != null)
						session.println("MOB(s) successfully imported!");
					continue;
				} else if ((buf != null)
						&& (buf.length() > 20)
						&& ((buf.substring(0, 20).indexOf("<PLAYERS>") >= 0) || (buf
								.substring(0, 20).indexOf("<ACCOUNTS>") >= 0))) {
					if (!CMSecurity.isAllowedEverywhere(mob,
							CMSecurity.SecFlag.IMPORTPLAYERS)) {
						returnAnError(session,
								"You are not allowed to import players in '"
										+ areaFileName + "' here.",
								compileErrors, commands);
						continue;
					}
					if (CF != null)
						buf = CF.textUnformatted();
					if (session != null)
						session.rawPrint("Unpacking players from file: '"
								+ areaFileName + "'...");
					...
					if (error.length() == 0)
						importCustomFiles(mob, externalFiles,
								customBotherChecker, !prompt, nodelete);
					if (error.length() == 0)
						error = CMLib.coffeeMaker()
								.addPlayersAndAccountsFromXML(buf.toString(),
										accounts, mobs, session);
					if (session != null)
						session.rawPrintln("!");
					if (error.length() > 0)
						return returnAnError(
						...
						for (int m = 0; m < accounts.size(); m++) {
							...
							for (int af = 0; af < commands.size(); af++)
								if ((commands.elementAt(af) instanceof String)
										&& (A.accountName()
												.equalsIgnoreCase((String) commands
														.elementAt(af)))) {
									if (names == null)
										names = new Vector();
					...
					for (int m = 0; m < accounts.size(); m++) {
						...
						if (CMLib.players().accountExists(A.accountName())) {
							if (!prompt) {
								...
								CMLib.players().obliterateAccountOnly(
										CMLib.players().getLoadAccount(
												A.accountName()));
						}
						A.setLastUpdated(System.currentTimeMillis());
						Log.sysOut("Import",
								"Imported account: " + A.accountName());
					...
					for (int m = 0; m < mobs.size(); m++) {
						...
						if (CMLib.players().playerExists(M.Name())) {
							if (!prompt) {
								returnAnError(session, "Player '" + M.Name()
										+ "' already exists.  Skipping.",
										compileErrors, commands);
								continue;
							} else if ((session != null)
								...
								CMLib.players()
										.obliteratePlayer(
												CMLib.players().getLoadPlayer(
														M.Name()), false, false);
						}
						if (M.playerStats() != null)
							M.playerStats().setLastUpdated(
						...
						List<String> channels = CMLib.channels()
								.getFlaggedChannelNames(
										ChannelsLibrary.ChannelFlag.NEWPLAYERS);
						for (int i = 0; i < channels.size(); i++)
							CMLib.commands().postChannel(channels.get(i),
									M.clans(),
									M.Name() + " has just been created.", true);
						...
						if (M.playerStats().getBirthday() == null) {
							M.baseCharStats().setStat(
									CharStats.STAT_AGE,
									M.playerStats().initializeBirthday(
											(int) Math.round(CMath.div(
													M.getAgeMinutes(), 60.0)),
											M.baseCharStats().getMyRace()));
							M.recoverCharStats();
						}
					...
					Log.sysOut("Import", mob.Name() + " imported "
							+ areaFileName);
					if (session != null)
						session.println("Player(s)/Account(s) successfully imported!");
					continue;
				} else if ((buf != null) && (buf.length() > 20)
						&& (buf.substring(0, 20).indexOf("<ITEMS>") >= 0)) {
					if (!CMSecurity.isAllowed(mob, mob.location(),
							CMSecurity.SecFlag.IMPORTITEMS)) {
						returnAnError(session,
								"You are not allowed to import items in '"
										+ areaFileName + "' here.",
								compileErrors, commands);
						continue;
					}
					if (CF != null)
						buf = CF.textUnformatted();
					if (session != null)
						session.rawPrint("Unpacking items from file: '"
								+ areaFileName + "'...");
					...
					if (error.length() == 0)
						importCustomFiles(mob, externalFiles,
								customBotherChecker, !prompt, nodelete);
					if (error.length() == 0)
						error = CMLib.coffeeMaker().addItemsFromXML(
								buf.toString(), items, session);
					if (session != null)
						session.rawPrintln("!");
					if (error.length() > 0)
						return returnAnError(
					...
					if (mob.location() == null)
						return returnAnError(session,
								"You must be in a room to import items.",
								compileErrors, commands);
					...
					Log.sysOut("Import", mob.Name() + " imported "
							+ areaFileName);
					if (session != null)
						session.println("Item(s) successfully imported!");
					continue;
				}
			} catch (Exception e) {
				Log.errOut("Import-", e);
				return returnAnError(session, e.getMessage(), compileErrors,
						commands);
			}
			...
			Log.sysOut("Import", "Importing data from file '" + areaFileName
					+ "'");
			readBlocks(V, areaData, roomData, mobData, resetData, objectData,
					mobProgData, objProgData, shopData, specialData,
					socialData, roomProgData);
			boolean didSocials = false;
			try {
				while (socialData.size() > 0) {
					String codeLine = eatNextLine(socialData);
					if ((!codeLine.startsWith("#"))
							&& (codeLine.trim().length() > 0)) {
						didSocials = true;
						...
						Social S2 = CMLib.socials().fetchSocial(
								word + " <T-NAME>", true);
						Social S3 = CMLib.socials().fetchSocial(word + " SELF",
								true);
						boolean changing = true;
						if ((S1 == null)
								|| (!S1.name().toUpperCase().equals(word))) {
						...
						if ((S1.You_see() == null)
								|| (!S1.You_see().equals(str))) {
							if ((changing) && prompt && (session != null))
								session.rawPrint("Change Social '" + S1.name()
										+ "' from '" + S1.You_see()
										+ "', you see, to: '" + str + "'");
							if ((!changing)
									|| ((session != null) && prompt && session
											.confirm("?", "Y")))
								S1.setYou_see(str);
						}
						...
						if ((S1.Third_party_sees() == null)
								|| (!S1.Third_party_sees().equals(str))) {
							if ((changing) && prompt && (session != null))
								session.rawPrint("Change Social '" + S1.name()
										+ "' from '" + S1.Third_party_sees()
										+ "', others see, to: '" + str + "'");
							if ((!changing)
									|| ((session != null) && prompt && (session
											.confirm("?", "Y"))))
						...
						if ((S2.You_see() == null)
								|| (!S2.You_see().equals(str))) {
							if ((changing) && prompt && (session != null))
								session.rawPrint("Change Social '" + S2.name()
										+ "' from '" + S2.You_see()
										+ "', you see, to: '" + str + "'");
							if ((!changing)
									|| ((session != null) && prompt && session
						...
						if ((S2.Third_party_sees() == null)
								|| (!S2.Third_party_sees().equals(str))) {
							if ((session != null) && prompt && changing)
								session.rawPrint("Change Social '" + S2.name()
										+ "', others see from '"
										+ S2.Third_party_sees() + "', to: '"
										+ str + "'");
							if ((!changing)
									|| ((session != null) && prompt && session
											.confirm("?", "Y")))
						...
						if ((S2.Target_sees() == null)
								|| (!S2.Target_sees().equals(str))) {
							if ((session != null) && prompt && changing)
								session.rawPrint("Change Social '" + S2.name()
										+ "', target sees from '"
										+ S2.Target_sees() + "', to: '" + str
										+ "'");
							if ((!changing)
									|| ((session != null) && prompt && session
											.confirm("?", "Y")))
						...
						if ((S2.See_when_no_target() == null)
								|| (!S2.See_when_no_target().equals(str))) {
							if ((session != null) && prompt && changing)
								session.rawPrint("Change Social '" + S2.name()
										+ "', no target sees from '"
										+ S2.See_when_no_target() + "', to: '"
										+ str + "'");
							if ((!changing)
									|| ((session != null) && prompt && session
											.confirm("?", "Y")))
						...
						if ((S3.You_see() == null)
								|| (!S3.You_see().equals(str))) {
							if ((session != null) && prompt && changing)
								session.rawPrint("Change Social '" + S3.name()
										+ "', you see from '" + S3.You_see()
										+ "', to: '" + str + "''");
							if ((!changing)
									|| ((session != null) && prompt && session
											.confirm("?", "Y")))
						...
						if ((S3.Third_party_sees() == null)
								|| (!S3.Third_party_sees().equals(str))) {
							if ((session != null) && prompt && changing)
								session.rawPrint("Change Social '" + S3.name()
										+ "', others see from '"
										+ S3.Third_party_sees() + "', to: '"
										+ str + "'");
							if ((!changing)
									|| ((session != null) && prompt && session
											.confirm("?", "Y")))
				...
				if (didSocials) {
					Log.sysOut("Import", mob.Name() + " imported socials from "
							+ areaFileName);
			...
			if ((roomData.size() == 0) || (areaData.size() == 0)) {
				if (!didSocials) {
					if (multiArea) {
						returnAnError(session, "No data in " + areaFileName,
								compileErrors, commands);
					...
					returnAnError(
							session,
							"Missing data! It is very unlikely this is an .are file.",
							compileErrors, commands);
					return false;
				}
			}
			...
			if ((areaName == null) || (areaName.length() == 0)) {
				if (!didSocials)
					returnAnError(session, "#AREA tag not found!",
							compileErrors, commands);
				if (multiArea)
					continue;
				return false;
			...
			if ((areaName.toUpperCase().startsWith(areaAuthor.toUpperCase()
					+ " "))
					&& (areaName.substring(areaAuthor.length() + 1).trim()
							.length() > 0))
			...
			try {
				...
				if (exists) {
					if ((nodelete) && (!prompt))
						return returnAnError(session, "Area '" + areaName
								+ "' already exists.", compileErrors, commands);
					else if ((!prompt)
							|| ((session != null) && (session.confirm(
									"Area: \"" + areaName
											+ "\" exists, obliterate first?",
									"N")))) {
						reLinkTable = new Vector();
				...
				if (session != null)
					session.println("Loading and Linking Rooms...");
				Log.sysOut("Import", mob.Name() + " imported " + areaName
						+ " from " + areaFileName);
				...
				for (int r = 0; r < roomData.size(); r++) {
					...
					if (roomData.elementAt(r) instanceof List) {
						...
						if ((roomV.size() == 1)
								&& (roomV.get(1).toUpperCase().trim()
										.startsWith("#ROOM")))
							continue;
					} else if (roomData.elementAt(r) instanceof String) {
						String s = (String) roomData.elementAt(r);
						if (!s.toUpperCase().trim().startsWith("#ROOM"))
							returnAnError(session, "Eating immaterial line: "
									+ roomData.elementAt(r) + ", area="
									+ areaName, compileErrors, commands);
						continue;
					} else
						continue;
					...
					if (isBadID(plainRoomID)) {
						Log.errOut("IMPORT", "Eating bad room id "
								+ plainRoomID);
						roomData.remove(r);
						r--;
						continue;
					...
					if ((roomV.size() > 2) && (roomV.get(0).trim().equals("~"))
						...
						R.setDisplayText(CMLib.coffeeFilter().safetyFilter(
								eatLineSquiggle(roomV)));
						R.setDescription(CMLib.coffeeFilter().safetyFilter(
								eatLineSquiggle(roomV)));
					}
					...
					if ((!R.roomID().startsWith("#"))
						...
						returnAnError(
								session,
								"Malformed room! Aborting this room "
										+ R.roomID() + ", display="
										+ R.displayText() + ", description="
										+ R.description() + ", numBits="
										+ CMParms.numBits(codeLine) + ", area="
										+ areaName, compileErrors, commands);
						continue;
					}
					...
					final String[][] secTypes = { { "inside", "0" },
							{ "city", "1" }, { "field", "2" },
							{ "forest", "3" }, { "hills", "4" },
							{ "mountain", "5" },
							...
							{ "desert", "10" } };
					boolean circleFormat = false;
					if ((CMParms.numBits(codeLine) >= 6)
							|| (CMParms.numBits(codeLine) == 4)) // wierder
																	// circlemud
																	// exception
					{
						codeBits = (getBitMask(codeLine, 2) << 16)
								| getBitMask(codeLine, 1); // ignoring 3 & 4
						sectorType = CMath.s_int(CMParms.getBit(codeLine, 5));
						circleFormat = true;
					} else if (CMParms.numBits(codeLine) == 5) // wierd
																// circlemud
																// exception
					{
						codeBits = sectorType;
						codeBits = (getBitMask(codeLine, 2) << 16)
								| getBitMask(codeLine, 1); // ignoring 3
					...
					R = applyRoomCodeBits(R, lastRoom, petShops, codeBits,
							circleFormat);

					roomV.add(0, R.roomID());
					newRooms.addElement(R);
					if (plainRoomID.startsWith("#")) {
						while (plainRoomID.startsWith("#0")
								&& (plainRoomID.length() > 2))
				...
				for (int r = 0; r < roomData.size(); r++) {
					...
					if (R == null) {
						Log.errOut("Import", "Unhashed room " + roomID
								+ "! Aborting!");
						return false;
					}
					...
					while (roomV.size() > 0) {
						...
						if (nextLine.toUpperCase().startsWith("S"))
							...
							String nameString = CMLib.coffeeFilter()
									.safetyFilter(eatLineSquiggle(roomV));
							String descString = CMLib.coffeeFilter()
									.safetyFilter(eatLineSquiggle(roomV));
							Item I = null;
							...
							int dirCode = CMath.s_int(CMParms
									.getCleanBit(nextLine, 0).substring(1)
									.trim());
							String descStr = CMLib.coffeeFilter().safetyFilter(
									eatLineSquiggle(roomV));
							String nameStr = CMLib.coffeeFilter().safetyFilter(
									eatLineSquiggle(roomV));
							...
							if ((dirCode < 0)
									|| (dirCode >= Directions.NUM_DIRECTIONS())) {
								returnAnError(session, "Room: " + R.roomID()
										+ ", Unknown direction code: "
										+ dirCode + ", aborting exit, area="
										+ areaName, compileErrors, commands);
								continue;
							}
							if ((R.getRawExit(dirCode) != null)
									|| (R.rawDoors()[dirCode] != null)) {
								returnAnError(session, "Room: " + R.roomID()
										+ ", Redundant exit codeStr "
										+ nextLine + "/" + codeStr
										+ ", dircode=" + dirCode
										+ ".  Aborting exit, area=" + areaName,
										compileErrors, commands);
								continue;
							}
							if (codeStr.length() == 0) {
								String nextStr = nextLine(roomV);
								if ((CMParms.numBits(nextStr) == 3)
										&& (CMath.isInteger(CMParms
												.getCleanBit(nextStr, 0))))
									codeStr = eatLine(roomV);
							}
							if ((CMParms.numBits(codeStr) == 1)
									&& (CMath.isInteger(codeStr.trim()))) {
								String nextStr = nextLine(roomV);
								if ((CMParms.numBits(nextStr) == 2)
										&& (CMath.isInteger(CMParms
												.getCleanBit(nextStr, 0))))
									codeStr += " " + eatLine(roomV);
							}
							int exitFlag = (CMath.s_int(CMParms.getCleanBit(
									codeStr, 0).trim()) & 31);
							int doorState = CMath.s_int(CMParms.getCleanBit(
									codeStr, 1).trim());
							int linkRoomID = CMath.s_int(CMParms.getCleanBit(
									codeStr, 2).trim());
							if (CMParms.numBits(codeStr) == 11) // wierd circle
																// format
							{ /* all is well */
								...
								returnAnError(session, "Room: " + R.roomID()
										+ ", Malformed exit codeStr " + codeStr
										+ ".  Aborting exit, area=" + areaName,
										compileErrors, commands);
								continue;
							}
							Exit X = CMClass.getExit("GenExit");
							Room linkRoom = getRoom(areaRooms, doneRooms,
									areaName, "" + linkRoomID);
							if (linkRoomID >= 0) {
								...
								X.setDoorsNLocks(hasDoor, !defaultsClosed,
										defaultsClosed, hasLock,
										defaultsLocked, defaultsLocked);
							}
							X.setDisplayText(descStr);
							String name = CMParms.getCleanBit(nameStr, 0)
									.trim();
							if (name.equalsIgnoreCase("SECRET")) {
								name = "secret door";
								X.basePhyStats().setDisposition(
										X.basePhyStats().disposition()
												| PhyStats.IS_HIDDEN);
								X.recoverPhyStats();
							}
							...
							X.setExitParams(name, X.closeWord(), X.openWord(),
									X.Name() + ", closed");
							X.setDescription(descStr);
							R.setRawExit(dirCode, X);
							Exit opExit = null;
							if (((linkRoom == null) || (linkRoom.getArea()
									.Name() != R.getArea().Name()))
									&& (linkRoomID >= 0)) {
								try {
									for (Enumeration r2 = CMLib.map().rooms(); r2
											.hasMoreElements();) {
										Room R2 = (Room) r2.nextElement();
										if ((R2.roomID().endsWith("#"
												+ linkRoomID))
												&& (R2 != R)) {
							...
							if ((linkRoom == null)
								...
								returnAnError(
										...
										"Room: "
												...
												+ "ward to unknown room #"
												+ linkRoomID + ", area="
												+ areaName, compileErrors,
										commands);
							...
							// R ERROR 700 100~
							// No idea what this is. Not important enough to
							// generate an error from
							...
							if (nextLine.endsWith("~"))
								nextLine = nextLine.substring(0,
										nextLine.length() - 1);
							long codeBits = getBitMask(nextLine, 0);
							R = applyRoomCodeBits(R, lastRoom, petShops,
									codeBits, true);
							...
							String scriptStuff = "";
							if (!nextLine.substring(1).trim().toUpperCase()
									.startsWith("IN_FILE_PROG")) {
							...
							// ?
							// not important enough to generate an error from
						} else if ((!nextLine.equalsIgnoreCase("#0"))
								&& (nextLine.trim().length() > 0))
							returnAnError(session, "Unknown room code: "
									+ nextLine + ", area=" + areaName,
									compileErrors, commands);
					}
				}

				if (session != null)
					session.print("Loading objects..");
				...
				while (resetData.size() > 0) {
					...
					if ((s.startsWith("#RE")) || (s.startsWith("*"))
							|| (s.startsWith("S"))) {
						...
						if (R == null) {
							if (multiArea)
								...
								returnAnError(session,
										"Reset error (no room) on line: " + s
												+ ", area=" + areaName,
										compileErrors, commands);
						} else {
							M = getMOB("#" + mobID, R, session,
									CMParms.copyFlattenVector(mobData),
									CMParms.copyFlattenVector(mobProgData),
									CMParms.copyFlattenVector(specialData),
									CMParms.copyFlattenVector(shopData),
									areaMOBS, doneMOBS, areaFileName,
									compileErrors, commands);
							if (M == null) {
								if (multiArea)
									nextResetData.addElement(s);
						...
						if (M == null) {
							if (multiArea)
								...
								returnAnError(session,
										"Reset error (no mob) on line: " + s
												+ ", area=" + areaName,
										compileErrors, commands);
						} else {
							String itemID = CMParms.getCleanBit(s, 2).trim();
							Item I = getItem("#" + itemID, session, areaName,
									CMParms.copyFlattenVector(objectData),
									CMParms.copyFlattenVector(objProgData),
									areaItems, doneItems, areaRooms, doneRooms,
									compileErrors, commands);
							if (I == null) {
								if (multiArea)
									nextResetData.addElement(s);
								...
								if (M instanceof ShopKeeper) {
									...
									int num = CMath.s_int(CMParms.getCleanBit(
											s, 3).trim());
									if (num < 0)
										num = 100;
									((ShopKeeper) M).getShop()
											.addStoreInventory(I, num, -1);
									if ((I instanceof Light)
										...
										((ShopKeeper) M)
												.getShop()
												.addStoreInventory(
														CMClass.getBasicItem("OilFlask"),
														num * 2, -1);
									else if (((I.ID().equals("GenReadable")) || (I instanceof com.planet_ink.coffee_mud.Items.interfaces.RoomMap))
											&& (!((ShopKeeper) M).getShop()
													.doIHaveThisInStock(
															"Parchment", null))) {
										((ShopKeeper) M).setWhatIsSoldMask(0);
										((ShopKeeper) M)
												.addSoldType(ShopKeeper.DEAL_INVENTORYONLY);
										((ShopKeeper) M)
												.getShop()
												.addStoreInventory(
														CMClass.getBasicItem("Parchment"),
														num, -1);
										Item journal2 = CMClass
												.getBasicItem("GenJournal");
										journal2.setName("the adventurers journal");
										journal2.setBaseValue(250);
										journal2.recoverPhyStats();
										journal2.text();
										((ShopKeeper) M).getShop()
												.addStoreInventory(journal2,
														num, -1);
									} else if (((ShopKeeper) M)
											.isSold(ShopKeeper.DEAL_WEAPONS)) {
										Item arrows = CMClass
												.getBasicItem("GenAmmunition");
										((Ammunition) arrows)
										...
										arrows.text();
										((ShopKeeper) M).getShop()
												.addStoreInventory(arrows, num,
														-1);
									}
								} else {
									if (I.basePhyStats().level() < 1) {
										I.basePhyStats().setLevel(
						...
						Room R2 = getRoom(areaRooms, doneRooms, areaName,
								roomID);
						MOB M2 = null;
						if (R2 != null)
						...
						if (R == null) {
							...
							if (I == null) {
								...
								if (CMLib.flags().isGettable(I)) {
									int rejuv = (int) Math.round(CMath.div(
											(long) 60000,
											CMProps.getTickMillis()) * 4.0);
									I.basePhyStats().setRejuv(
											rejuv * I.basePhyStats().level());
								}
								I.recoverPhyStats();
								if (I instanceof Container) {
						...
						Item I = getItem("#" + itemID, session, areaName,
								...
								areaItems, doneItems, areaRooms, doneRooms,
								compileErrors, commands);
						Container C = (Container) containerHash
								.get(containerID);
						if (I == null) {
							if (multiArea)
								nextResetData.addElement(s);
							...
							if (multiArea)
								...
								returnAnError(session,
										"Reset error (no container) on line: "
												+ s + ", area=" + areaName,
										compileErrors, commands);
							...
							if (multiArea)
								...
								returnAnError(session,
										"Reset error (no container owner) on line: "
												+ s + ", area=" + areaName,
										compileErrors, commands);
							...
							default:
								returnAnError(
										...
										"Room: "
												...
												+ ", Unknown direction code: "
												+ dirCode
												+ " (not so bad at this point, it was probably aborted earlier, area="
												+ areaName, compileErrors,
										commands);
							}
							if (dirCode < Directions.NUM_DIRECTIONS()) {
								...
								if (E == null)
									...
									E.setDoorsNLocks(HasDoor, Open,
											DefaultsClosed, HasLock, Locked,
											DefaultsLocked);
									if (E.hasADoor()
											&& E.name().equals("the ground")) {
										E.setName("a door");
										E.setExitParams("door", "close",
												"open", "a door, closed.");
									}
								}
							}
						...
						returnAnError(session, "Reset, unknown command: " + s
								+ ", area=" + areaName, compileErrors, commands);
				...
				for (Enumeration e = petShops.keys(); e.hasMoreElements();) {
					...
					if (shopRoom == null)
						returnAnError(session, "Unknown store room: "
								+ storeRoom.roomID() + ", area=" + areaName,
								compileErrors, commands);
					else
					...
					if (shopKeeper == null)
						returnAnError(
								session,
								"Unknown shopkeeper not in room: "
										+ storeRoom.roomID() + ", area="
										+ areaName, compileErrors, commands);
					else
						while (storeRoom.numInhabitants() > 0) {
							...
							if (pet != null) {
								shopKeeper.getShop().addStoreInventory(pet, 20,
										-1);
				...
				if (newRooms.size() == 0) {
					...
					if (session != null)
						session.println("\nDone!!!!!!  A good room to look at would be "
								+ ((Room) newRooms.elementAt(0)).roomID()
								+ "\n\r");
				} else {
					if (session != null)
		...
		for (Enumeration e = doneRooms.elements(); e.hasMoreElements();) {
			...
			for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
				...
				if ((X != null) && (X.isGeneric()) && (!X.hasADoor())
						&& (!X.hasALock())
						&& (X.name().equalsIgnoreCase("the ground"))
						&& (!X.isReadable()) && (X.numEffects() == 0)
						&& (X.numBehaviors() == 0)
	...
	public boolean securityCheck(MOB mob) {
		return CMSecurity.isAllowedContainsAny(mob,
				CMSecurity.SECURITY_IMPORT_GROUP);
	}
