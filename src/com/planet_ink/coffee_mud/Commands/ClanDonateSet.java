...
public class ClanDonateSet extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
			...
			for (Pair<Clan, Integer> c : mob.clans())
				if ((clanName.length() == 0)
						...
						&& (c.first.getAuthority(c.second.intValue(),
								Clan.Function.SET_DONATE) != Authority.CAN_NOT_DO)) {
					C = c.first;
					break;
				}
		...
		if (C == null) {
			mob.tell("You aren't allowed to set a donation room for "
					+ ((clanName.length() == 0) ? "anything" : clanName) + ".");
			return false;
		}

		if (C.getStatus() > Clan.CLANSTATUS_ACTIVE) {
			mob.tell("You cannot set a donation room.  Your "
					+ C.getGovernmentName()
		...
		if (skipChecks
				|| CMLib.clans().goForward(mob, C, commands,
						Clan.Function.SET_DONATE, false)) {
			...
			if (skipChecks
					|| CMLib.clans().goForward(mob, C, commands,
							Clan.Function.SET_DONATE, true)) {
				C.setDonation(CMLib.map().getExtendedRoomID(R));
				C.update();
				mob.tell("The donation room for " + C.getGovernmentName() + " "
						+ C.clanID() + " is now set to " + R.displayText(mob)
						+ ".");
				CMLib.clans().clanAnnounce(
						mob,
						"The donation room for " + C.getGovernmentName() + " "
			...
			mob.tell("You aren't in the right position to set your "
					+ C.getGovernmentName() + "'s donation room.");
			return false;
		}
		return false;
	}
