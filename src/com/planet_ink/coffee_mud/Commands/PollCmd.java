...
public class PollCmd extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((mob == null) || mob.isMonster())
			return false;
		java.util.List<Poll>[] mypolls = CMLib.polls().getMyPollTypes(mob,
				(commands == null));
		...
		if ((mypolls[0].size() == 0) && (mypolls[2].size() == 0)) {
			if ((commands != null) && (mypolls[1].size() == 0)) {
				...
				if (mypolls[1].size() > 0)
					mob.tell(mypolls[1].size()
							+ " poll(s) <-IS-ARE> awaiting your participation.");
				return false;
			}
		}
		...
		if (commands == null) {
			if (mypolls[1].size() == 1)
				mob.tell("\n\r^H"
						+ mypolls[1].size()
						+ " other poll(s) <-IS-ARE> awaiting your participation.^N\n\r");
			if (mypolls[2].size() > 0)
				mob.tell("\n\r^HResults from " + mypolls[2].size()
						+ " poll(s) <-IS-ARE> still available.^N\n\r");
			return true;
		}
