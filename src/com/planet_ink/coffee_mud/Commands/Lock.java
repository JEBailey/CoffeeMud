...
public class Lock extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (lockThis == null)
			lockThis = mob.location().fetchFromMOBRoomItemExit(mob, null,
					whatTolock, Wearable.FILTER_ANY);

		if ((lockThis == null) || (!CMLib.flags().canBeSeenBy(lockThis, mob))) {
			mob.tell("You don't see '" + whatTolock + "' here.");
			return false;
		}
		final String lockMsg = "<S-NAME> lock(s) <T-NAMESELF>."
				+ CMLib.protocol().msp("doorlock.wav", 10);
		CMMsg msg = CMClass.getMsg(mob, lockThis, null, CMMsg.MSG_LOCK,
				lockMsg, whatTolock, lockMsg);
				...
				if ((dirCode >= 0)
					...
					if ((opE != null) && (opE.isLocked())
						...
						opR.showHappens(CMMsg.MSG_OK_ACTION, opE.name() + " "
								+ inDirName + " is locked from the other side.");
					}
				}
			}
