...
public class Sheath extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() == 0) {
			...
			for (int i = 0; i < sheaths.size(); i++) {
				...
				} else if ((item2 != null) && (!items.contains(item2))
			...
			if (item2 != null)
				for (int i = 0; i < sheaths.size(); i++) {
					Container sheath = (Container) sheaths.elementAt(i);
					if ((sheath.canContain(item2)) && (!items.contains(item2))) {
						items.addElement(item2);
						containers.addElement(sheath);
					}
				}
			...
			Container container = (Container) CMLib.english()
					.possibleContainer(mob, commands, false,
		...
		if (items.size() == 0) {
			if (!noerrors)
				if (sheaths.size() == 0)
					...
					mob.tell("You aren't wearing anything you can sheath "
							+ sheathable.name() + " in.");
			...
			for (int i = 0; i < items.size(); i++) {
				...
				if (CMLib.commands().postRemove(mob, putThis, true)) {
					CMMsg putMsg = CMClass
							.getMsg(mob,
									container,
									putThis,
									CMMsg.MSG_PUT,
									((quiet ? null
											: "<S-NAME> sheath(s) <O-NAME> in <T-NAME>.")));
	...
	public double actionsCost(final MOB mob, final List<String> cmds) {
		return CMProps.getActionCost(ID(),
				CMath.div(CMProps.getIntVar(CMProps.Int.DEFCMDTIME), 200.0));
	}

	public double combatActionsCost(MOB mob, List<String> cmds) {
		return CMProps.getCombatActionCost(ID(),
				CMath.div(CMProps.getIntVar(CMProps.Int.DEFCOMCMDTIME), 200.0));
	}
