...
public class Bid extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		Environmental shopkeeper = CMLib.english().parseShopkeeper(mob,
				commands, "Bid how much, on what, with whom?");
		if (shopkeeper == null)
			return false;
		if (commands.size() < 2) {
		...
		if (CMLib.english().numPossibleGold(mob, bidStr) <= 0) {
			mob.tell("It does not look like '" + bidStr
					+ "' is enough to offer.");
			return false;
		}
		Object[] bidThang = CMLib.english().parseMoneyStringSDL(mob, bidStr,
				null);
		bidStr = CMLib.beanCounter().nameCurrencyShort(
				(String) bidThang[0],
				CMath.mul(((Double) bidThang[1]).doubleValue(),
						((Long) bidThang[2]).longValue()));
		...
		if (V.size() == 0)
			mob.tell(mob, shopkeeper, null,
					"<T-NAME> do(es)n't appear to have any '" + whatName
							+ "' available for auction.  Try LIST.");
		else
			for (int v = 0; v < V.size(); v++) {
				Environmental thisThang = (Environmental) V.elementAt(v);
				CMMsg newMsg = CMClass.getMsg(mob, shopkeeper, thisThang,
						CMMsg.MSG_BID, "<S-NAME> bid(s) " + bidStr
								+ " on <O-NAME> with <T-NAMESELF>.",
						CMMsg.MSG_BID, "<S-NAME> bid(s) '" + bidStr
								+ "' on <O-NAME> with <T-NAMESELF>.",
						CMMsg.MSG_BID,
						"<S-NAME> place(s) a bid with <T-NAMESELF>.");
