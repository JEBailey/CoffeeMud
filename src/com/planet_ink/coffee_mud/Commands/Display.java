...
public class Display extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		while (doBugFix || ((allFlag) && (addendum <= maxToGive))) {
			doBugFix = false;
			Environmental giveThis = CMLib.english().bestPossibleGold(mob,
					null, thingToGive);
			if (giveThis != null) {
		...
		if (V.size() == 0)
			...
			for (int i = 0; i < V.size(); i++) {
				...
				CMMsg newMsg = CMClass.getMsg(recipient, giveThis, mob,
						CMMsg.MSG_LOOK,
						"<O-NAME> show(s) <T-NAME> to <S-NAMESELF>.");
				if (mob.location().okMessage(recipient, newMsg)) {
					recipient.tell(recipient, giveThis, mob,
							"<O-NAME> show(s) <T-NAME> to <S-NAMESELF>.");
