...
public class Skills extends StdCommand {
	...
	protected boolean parsedOutIndividualSkill(MOB mob, String qual, int acode) {
		return parsedOutIndividualSkill(mob, qual,
				new XVector(Integer.valueOf(acode)));
	}

	protected boolean parsedOutIndividualSkill(MOB mob, String qual,
			Vector acodes) {
		if ((qual == null) || (qual.length() == 0)
				|| (qual.equalsIgnoreCase("all")))
			return false;
		if (qual.length() > 0)
			for (int i = 1; i < Ability.DOMAIN_DESCS.length; i++)
				if (Ability.DOMAIN_DESCS[i].replace('_', ' ').equalsIgnoreCase(
						qual))
					return false;
				else if ((Ability.DOMAIN_DESCS[i].replace('_', ' ')
						.indexOf('/') >= 0)
						&& (Ability.DOMAIN_DESCS[i]
								.replace('_', ' ')
								.substring(
										Ability.DOMAIN_DESCS[i].indexOf('/') + 1)
								.equalsIgnoreCase(qual)))
					return false;
		Ability A = CMClass.findAbility(qual);
		if ((A != null)
				&& (CMLib.ableMapper().qualifiesByAnyCharClass(A.ID()))
				&& (acodes.contains(Integer.valueOf(A.classificationCode()
						& Ability.ALL_ACODES)))) {
			...
			if (A2 == null)
				...
				line.append("^N[^H"
						+ CMStrings.padRight(
								Integer.toString(A2.proficiency()), 3)
						+ "%^?]^N "
						+ CMStrings.padRight("^<HELP^>" + A2.name()
								+ "^</HELP^>", 19));
				line.append("^?\n\r");
				if (mob.session() != null)
	...
	protected int parseOutLevel(Vector commands) {
		if ((commands.size() > 1) && (commands.lastElement() instanceof String)
	...
	protected void parseDomainInfo(MOB mob, Vector commands, Vector acodes,
			int[] level, int[] domain, String[] domainName) {
		...
		if (qual.length() > 0)
			for (int i = 1; i < Ability.DOMAIN_DESCS.length; i++)
				if (Ability.DOMAIN_DESCS[i].replace('_', ' ').startsWith(qual)) {
					domain[0] = i << 5;
					break;
				} else if ((Ability.DOMAIN_DESCS[i].replace('_', ' ').indexOf(
						'/') >= 0)
						&& (Ability.DOMAIN_DESCS[i]
								.replace('_', ' ')
								.substring(
		...
		if ((domain[0] < 0) && (qual.length() > 0)) {
			...
			for (int i = 1; i < Ability.DOMAIN_DESCS.length; i++) {
				...
				for (int a = 0; a < acodes.size(); a++)
					found = found
							|| CMLib.ableMapper().isDomainIncludedInAnyAbility(
									i << 5,
									((Integer) acodes.elementAt(a)).intValue());
				if (found)
					domains.append(Ability.DOMAIN_DESCS[i].toLowerCase()
							.replace('_', ' ') + ", ");
			}
			if (domains.toString().endsWith(", "))
				domains = new StringBuffer(domains.substring(0,
						domains.length() - 2));
			if (!mob.isMonster())
	...
	protected StringBuilder getAbilities(MOB viewerM, MOB ableM, int ofType,
			int ofDomain, boolean addQualLine, int maxLevel) {
		Vector V = new Vector();
	...
	protected StringBuilder getAbilities(MOB viewerM, MOB ableM,
			Vector ofTypes, int mask, boolean addQualLine, int maxLevel) {
		...
		for (int l = 0; l <= highestLevel; l++) {
			...
			int col = 0;
			for (Enumeration<Ability> a = ableM.allAbilities(); a
					.hasMoreElements();) {
				Ability A = a.nextElement();
				int level = CMLib.ableMapper().qualifyingLevel(ableM, A);
				if (level < 0)
				...
				if ((A != null)
					...
					thisLine.append("^N[^H"
							+ CMStrings.padRight(
									Integer.toString(A.proficiency()), COL_LEN1)
							+ "%^?]^N "
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (domain[0] >= 0) {
			...
			for (int v = 0; v < V.size(); v++)
				V.setElementAt(
						Integer.valueOf(((Integer) V.elementAt(v)).intValue()
								+ domain[0]), v);
		}
		if ((domain[0] >= 0) || (qual.length() == 0))
			msg.append("\n\r^HYour " + domainName[0].replace('_', ' ')
					+ "skills:^? "
					+ getAbilities(mob, mob, V, mask, true, level[0]));
		if (!mob.isMonster())
