...
public class Alias extends StdCommand {
		...
		IC[0] = new InputCallback(InputCallback.Type.PROMPT, "", 0) {
			...
			public void showPrompt() {
				...
				for (int i = 0; i < aliasNames.length; i++)
					menu.append(CMStrings.padRight((i + 1) + ". "
							+ aliasNames[i], 15)
							+ ": " + pStats.getAlias(aliasNames[i]) + "\n\r");
				...
				if (num <= (pStats.getAliasNames().length)) {
					...
					session.prompt(new InputCallback(InputCallback.Type.PROMPT,
						...
						public void callBack() {
							...
							final String commandStr = this.input.toUpperCase()
									.trim();
							...
							for (int i = 0; i < commandStr.length(); i++)
								if (!Character.isLetterOrDigit(commandStr
										.charAt(i))) {
	...
	public void modifyAlias(final MOB mob, final Session session,
			final PlayerStats pStats, final String aliasName,
			final InputCallback[] IC) {
		session.prompt(new InputCallback(InputCallback.Type.PROMPT, "", 0) {
			@Override
			public void showPrompt() {
				session.rawPrintln("\n\rEnter a value for alias '" + aliasName
			...
			public void callBack() {
				...
				value = CMStrings.replaceAll(value, "&", "");
				if ((value.length() == 0)
						&& (pStats.getAlias(aliasName).length() > 0))
					mob.tell("(No change)");
