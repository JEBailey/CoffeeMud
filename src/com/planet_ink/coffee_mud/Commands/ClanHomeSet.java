...
public class ClanHomeSet extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
			...
			for (Pair<Clan, Integer> c : mob.clans())
				if ((clanName.length() == 0)
						...
						&& (c.first.getAuthority(c.second.intValue(),
								Clan.Function.SET_HOME) != Authority.CAN_NOT_DO)) {
					C = c.first;
					break;
				}
		...
		if (C == null) {
			mob.tell("You aren't allowed to set a home room for "
					+ ((clanName.length() == 0) ? "anything" : clanName) + ".");
			return false;
		}

		if (C.getStatus() > Clan.CLANSTATUS_ACTIVE) {
			mob.tell("You cannot set a home.  Your " + C.getGovernmentName()
		...
		if (skipChecks
			...
			if (skipChecks
				...
				C.update();
				mob.tell("The " + C.getGovernmentName() + " " + C.clanID()
						+ " home is now set to " + R.displayText(mob) + ".");
				CMLib.clans().clanAnnounce(
						mob,
						"The " + C.getGovernmentName() + " " + C.clanID()
								+ " home is now set to " + R.displayText(mob)
								+ ".");
				return true;
			}
