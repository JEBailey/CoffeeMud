	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (helpStr.length() == 0) {
			...
			if ((thisTag != null) && (helpStr.equalsIgnoreCase("more"))) {
				StringBuffer theRest = (StringBuffer) Resources
						.getResource("arc_help.therest");
				if (theRest == null) {
					Vector V = new Vector();
					...
					for (Enumeration<Ability> a = CMClass.abilities(); a
						...
						Ability A = a.nextElement();
						if ((A != null)
								&& ((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_SUPERPOWER))
							V.addElement(A.ID());
					}
					if (V.size() > 0) {
						theRest.append("\n\rSuper Powers:\n\r");
					...
					for (Enumeration<Ability> a = CMClass.abilities(); a
						...
						Ability A = a.nextElement();
						if ((A != null)
								&& ((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_TECH))
							V.addElement(A.ID());
					}
					if (V.size() > 0) {
						theRest.append("\n\rTech Skills:\n\r");
					...
					V.clear();
					for (Enumeration b = CMClass.behaviors(); b
							.hasMoreElements();) {
			...
			StringBuilder text = CMLib.help().getHelpText(helpStr,
					CMLib.help().getArcHelpFile(), mob);
			if (text != null)
				thisTag = new StringBuffer(text.toString());
		}
		if (thisTag == null) {
			mob.tell("No archon help is available on "
					+ helpStr
					+ " .\n\rEnter 'COMMANDS' for a command list, or 'TOPICS' for a complete list.");
			Log.errOut("Help: " + mob.Name() + " wanted archon help on "
					+ helpStr);
