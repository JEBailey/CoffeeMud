...
public class Report extends Skills {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() < 2) {
			StringBuffer buf = new StringBuffer("say \"I have "
			...
			if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.EXPERIENCE))
				...
				buf.append(", and need " + mob.getExpNeededLevel()
						+ " to level");
			buf.append(".\"");
			...
			if ("AFFECTS".startsWith(s) || (s.equalsIgnoreCase("ALL"))) {
				...
				if (C != null)
					aff.append(C.executeInternal(mob, metaFlags, mob)
							.toString());
			...
			if ("STATS".startsWith(s) || (s.equalsIgnoreCase("ALL"))) {
				...
				for (int i : CharStats.CODES.BASE())
					stats.append("^c"
							+ CMStrings.capitalizeAndLower(CMStrings.limit(
									CharStats.CODES.NAME(i), 3))
							+ ": ^w"
							+ CMStrings.padRight(
									Integer.toString(CT.getStat(i)), 2) + "/"
			...
			if (s.equalsIgnoreCase("ALL")) {
				...
				say.append("\n\r^NMy skills:^? "
						+ getAbilities(null, mob, V, Ability.ALL_ACODES, false,
								level));
			} else if ("SPELLS".startsWith(s))
				say.append("\n\r^NMy spells:^? "
						+ getAbilities(null, mob, Ability.ACODE_SPELL, -1,
								false, level));
				...
				say.append("\n\r^NMy prayers:^? "
						+ getAbilities(null, mob, Ability.ACODE_PRAYER, -1,
								false, level));
			else if (("POWERS".startsWith(s)) || ("SUPER POWERS".startsWith(s)))
				say.append("\n\r^NMy super powers:^? "
						+ getAbilities(null, mob, Ability.ACODE_SUPERPOWER, -1,
								false, level));
			else if ("CHANTS".startsWith(s))
				say.append("\n\r^NMy chants:^? "
						+ getAbilities(null, mob, Ability.ACODE_CHANT, -1,
								false, level));
			else if ("SONGS".startsWith(s))
				say.append("\n\r^NMy songs:^? "
						+ getAbilities(null, mob, Ability.ACODE_SONG, -1,
								false, level));
			else if ("TECH SKILLS".startsWith(s))
				say.append("\n\r^NMy tech skills:^? "
						+ getAbilities(null, mob, Ability.ACODE_TECH, -1,
								false, level));

			if (say.length() == 0)
				mob.tell("'"
						+ s
						+ "' is unknown.  Try SPELLS, SKILLS, PRAYERS, CHANTS, SONGS, STATS, or ALL.");
			else
