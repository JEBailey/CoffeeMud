...
public class Shell extends StdCommand {
	...
	protected String[][] SUB_CMDS = { { "$", "DIRECTORY", "LS" },
			{ ">", "COPY", "CP" }, { ".", "CHANGEDIRECTORY", "CD", "GO" },
			{ "-", "DELETE", "RM", "RD" }, { "\\", "TYPE", "CAT", "TP" },
			{ "+", "MAKEDIRECTORY", "MKDIR", "MD" }, { "*", "FINDFILE", "FF" },
			{ "&", "SEARCHTEXT", "GREP", "ST" }, { "/", "EDIT" },
			{ "~", "MOVE", "MV" }, { "?", "COMPAREFILES", "DIFF", "CF" }, };

	protected final static String[] badTextExtensions = { ".ZIP", ".JPE",
			".JPG", ".GIF", ".CLASS", ".WAV", ".BMP", ".JPEG", ".GZ", ".TGZ",
			".JAR" };
			...
			if ((CF.isDirectory()) && (CF.exists())) {
				...
				while (x <= dirs.size()) {
					if (x == dirs.size()) {
						dirs.addElement(CF);
						break;
					} else if (dirs.elementAt(x).getVFSPathAndName().length() < CF
							.getVFSPathAndName().length())
						x++;
					else {
	...
	public static final String incorporateBaseDir(String currentPath,
			String filename) {
		String starter = "";
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		String pwd = (pwds.contains(mob)) ? (String) pwds.elementAt(
				pwds.indexOf(mob), 2) : "";
		...
		case 0: // directory
		{
			...
			CMFile[] dirs = CMFile.getFileList(
					incorporateBaseDir(pwd, CMParms.combine(commands, 1)), mob,
					opts.recurse, true);
			if (dirs == null) {
		...
		case 1: // copy
		{
			...
			CMFile[] dirs = CMFile.getFileList(incorporateBaseDir(pwd, source),
					mob, opts.recurse, true);
			if (dirs == null) {
			...
			if ((dirs.length == 1)
					&& (!target.trim().startsWith("::") && (!target.trim()
							.startsWith("//"))))
				target = (dirs[0].isLocalFile()) ? "//" + target.trim() : "::"
						+ target.trim();
			CMFile DD = new CMFile(incorporateBaseDir(pwd, target), mob);
			java.util.List<CMFile> ddirs = sortDirsUp(dirs);
			for (CMFile SF : ddirs) {
				if ((SF == null) || (!SF.exists())) {
					mob.tell("^xError: source " + desc(SF)
							+ " does not exist!^N");
					return false;
				}
				if (!SF.canRead()) {
					mob.tell("^xError: access denied to source " + desc(SF)
							+ "!^N");
					return false;
				}
				...
				if (DD.isDirectory()) {
					...
					if (DD.demandedVFS())
						...
						target = (SF.isLocalFile() && DD.canLocalEquiv()) ? "//"
								+ target
								: "::" + target;
				...
				if (DF.mustOverwrite()) {
					mob.tell("^xError: destination " + desc(DF)
							+ " already exists!^N");
					return false;
				}
				if (!DF.canWrite()) {
					mob.tell("^xError: access denied to destination "
							+ desc(DF) + "!^N");
					return false;
				}
				if ((SF.isDirectory()) && (opts.recurse)) {
					if (!DF.mkdir())
						mob.tell("^xWarning: failed to mkdir " + desc(DF)
								+ " ^N");
					else
					...
					byte[] O = SF.raw();
					if (O.length == 0) {
						mob.tell("^xWarning: " + desc(SF)
								+ " file had no data^N");
					}
					if (!DF.saveRaw(O))
						mob.tell("^xWarning: write failed to " + desc(DF)
								+ " ^N");
					else
						mob.tell(desc(SF) + " copied to " + desc(DF));
				}
		...
		case 2: // cd
		{
			CMFile newDir = new CMFile(incorporateBaseDir(pwd,
					CMParms.combine(commands, 1)), mob);
			String changeTo = newDir.getVFSPathAndName();
			if (!newDir.exists()) {
				mob.tell("^xError: Directory '" + CMParms.combine(commands, 1)
						+ "' does not exist.^N");
				return false;
			}
		...
		{
			CMFile CF = new CMFile(incorporateBaseDir(pwd,
					CMParms.combine(commands, 1)), mob);
			if (CF.exists()) {
		...
		case 6: // findfiles
		{
			...
			if (substring.length() == 0)
				substring = "*";
			CMFile[] dirs = CMFile.getFileList(
					incorporateBaseDir(pwd, substring), mob, true, true);
			StringBuffer msg = new StringBuffer("");
			if (dirs.length == 0) {
				mob.tell("^xError: no files matched^N");
				return false;
		...
		case 7: // searchtext
		{
			...
			CMFile[] dirs = CMFile.getFileList(incorporateBaseDir(pwd, "*"),
					mob, true, true);
			if (dirs.length == 0) {
		...
		case 8: // edit
		{
			CMFile file = new CMFile(incorporateBaseDir(pwd,
					CMParms.combine(commands, 1)), mob);
			...
			JournalsLibrary.MsgMkrResolution resolution = CMLib.journals()
					.makeMessage(mob, messageTitle, vbuf, false);
			if (resolution == JournalsLibrary.MsgMkrResolution.SAVEFILE) {
				StringBuffer text = new StringBuffer("");
				for (int i = 0; i < vbuf.size(); i++)
					text.append((vbuf.get(i)) + CR);
				if (file.saveText(text)) {
					for (final Iterator<String> i = Resources
							.findResourceKeys(file.getName()); i.hasNext();)
		...
		case 10: // compare files
		{
			...
			CMFile file1 = new CMFile(incorporateBaseDir(pwd, firstFilename),
					mob);
			...
			CMFile file2 = new CMFile(prefix
					+ incorporateBaseDir(pwd, secondFilename), mob);
			...
			LinkedList<CMStrings.Diff> diffs = CMStrings.diff_main(
					text1.toString(), text2.toString(), false);
			boolean flipFlop = false;
			for (CMStrings.Diff d : diffs) {
				StringBuilder str = new StringBuilder("\n\r^H"
						+ d.operation.toString() + ": ");
		...
		default:
			mob.tell("'" + first
					+ "' is an unknown command.  Valid commands are: "
					+ allcmds.toString()
					+ "and SHELL alone to check your current directory.");
			return false;
		}
		return true;
	}
	...
	public String desc(CMFile CF) {
		return (CF.isLocalFile() ? "Local file " : "VFS file ") + "'/"
				+ CF.getVFSPathAndName() + "'";
	}
