...
public class Tell extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (((String) commands.firstElement()).equalsIgnoreCase("last")
			...
			if ((V.size() == 0) || (CMath.bset(metaFlags, Command.METAFLAG_AS))
		...
		if (targetM == null) {
			if (targetName.indexOf('@') >= 0) {
				String mudName = targetName
						.substring(targetName.indexOf('@') + 1);
				targetName = targetName.substring(0, targetName.indexOf('@'));
				if (CMLib.intermud().i3online()
						|| CMLib.intermud().imc2online())
					CMLib.intermud().i3tell(mob, targetName, mudName,
							combinedCommands);
				else
		...
		try {
			...
			CMLib.commands()
					.postSay(mob, targetM, combinedCommands, true, true);
		} finally {
			if (ts != null)
				ts.snoopSuspension(-1);
		}
