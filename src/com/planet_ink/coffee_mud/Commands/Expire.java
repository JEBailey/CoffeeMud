...
public class Expire extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() < 1) {
			...
			String playerName = CMStrings.capitalizeAndLower((String) commands
					.elementAt(0));
			if (CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1)
				...
				if (M != null)
					stats = CMLib.players().getLoadPlayer(playerName)
							.playerStats();
			}
			if (stats == null) {
				mob.tell("No player/account named '" + playerName
						+ "' was found.");
				return false;
			}
			long timeLeft = stats.getAccountExpiration()
					- System.currentTimeMillis();
			mob.tell("Player '" + playerName + "' currently has "
					+ (CMLib.english().returnTime(timeLeft, 0)) + " left.");
			return false;
		} else {
			long days = CMath.s_long((String) commands.elementAt(1)) * 1000
					* 60 * 60 * 24;
			String playerName = CMStrings.capitalizeAndLower((String) commands
					.elementAt(0));
			if (CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1)
			...
			mob.tell("Player '"
					+ playerName
					+ "' now has "
					+ (CMLib.english().returnTime(stats.getAccountExpiration()
							- System.currentTimeMillis(), 0)) + " days left.");
			return false;
		}
	}
