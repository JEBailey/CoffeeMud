...
public class ClanDeclare extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
			...
			for (Pair<Clan, Integer> c : mob.clans())
				if ((clanName.length() == 0)
						...
						&& (c.first.getAuthority(c.second.intValue(),
								Clan.Function.DECLARE) != Authority.CAN_NOT_DO)) {
					C = c.first;
					break;
				}
		}
		if (C2 == null) {
			for (Pair<Clan, Integer> c : mob.clans())
				if (CMLib.english()
						.containsString(c.first.getName(), clan2Name)) {
					C2 = c.first;
					break;
				}
		}
		...
		if (C == null) {
			mob.tell("You aren't allowed to declare " + rel.toLowerCase()
					+ " on behalf of "
		...
		if ((!C2.isRivalrous()) || (!C.isRivalrous())) {
			mob.tell("Relations between " + C.getName() + " and "
					+ C2.getName() + " are impossible.");
			return false;
		}
		...
		if (rel.length() > 0) {
			if (skipChecks
				...
				if (newRole < 0) {
					mob.tell("'"
							+ rel
							+ "' is not a valid relationship. Try WAR, HOSTILE, NEUTRAL, FRIENDLY, or ALLY.");
					return false;
				}
				if (C2 == C) {
				...
				if (C.getClanRelations(C2.clanID()) == newRole) {
					mob.tell(C.getName() + " is already in that state with "
							+ C2.getName() + ".");
					return false;

				}
				long last = C.getLastRelationChange(C2.clanID());
				if (last > (CMProps.getIntVar(CMProps.Int.TICKSPERMUDMONTH) * CMProps
						.getTickMillis())) {
					last = last
							+ (CMProps.getIntVar(CMProps.Int.TICKSPERMUDMONTH) * CMProps
									.getTickMillis());
					...
					CMLib.clans()
							.clanAnnounce(
									...
									"The "
											...
											+ " has declared "
											+ CMStrings
													.capitalizeAndLower(Clan.REL_STATES[newRole]
															.toLowerCase())
											+ " " + C2.name() + ".");
					C.setClanRelations(C2.clanID(), newRole,
							System.currentTimeMillis());
					C.update();
					return false;
				}
			} else {
				msg.append("You aren't in the right position to declare relationships with your "
						+ C.getGovernmentName() + ".");
			}
		} else {
			mob.tell("You must specify the clans name, and a new relationship.");
