...
public class ClanExile extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
			...
			for (Pair<Clan, Integer> c : mob.clans())
				if ((clanName.length() == 0)
						...
						&& (c.first.getAuthority(c.second.intValue(),
								Clan.Function.EXILE) != Authority.CAN_NOT_DO)) {
					C = c.first;
					break;
				}
		...
		if (memberStr.length() > 0) {
			if (C == null) {
				mob.tell("You aren't allowed to exile anyone from "
			...
			if (skipChecks
				...
				if (found) {
					...
					if (M == null) {
						mob.tell(memberStr
								+ " was not found.  Could not exile from "
								+ C.getGovernmentName() + ".");
						return false;
					}
					if (skipChecks
							|| CMLib.clans().goForward(mob, C, commands,
									Clan.Function.EXILE, true)) {
						if (C.getGovernment().getExitScript().trim().length() > 0) {
							Pair<Clan, Integer> curClanRole = M.getClanRole(C
									.clanID());
							if (curClanRole != null)
								M.setClan(C.clanID(),
							...
							CMMsg msg2 = CMClass
									.getMsg(M, M, null, CMMsg.MSG_OK_VISUAL,
											null, null, "CLANEXIT");
							S.executeMsg(M, msg2);
							S.dequeResponses();
						...
						CMLib.clans().clanAnnounce(
								mob,
								"Member exiled from " + C.getGovernmentName()
										+ " " + C.name() + ": " + M.Name());
						mob.tell(M.Name() + " has been exiled from "
								+ C.getGovernmentName() + " '" + C.clanID()
								+ "'.");
						if ((M.session() != null) && (M.session().mob() == M))
							M.tell("You have been exiled from "
									+ C.getGovernmentName() + " '" + C.clanID()
									+ "'.");
						C.delMember(M);
						return false;
				...
				msg.append("You aren't in the right position to exile anyone from your "
						+ C.getGovernmentName() + ".");
			}
		} else {
