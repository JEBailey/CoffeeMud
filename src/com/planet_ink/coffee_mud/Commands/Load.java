...
public class Load extends StdCommand {
	...
	public final String[] combine(final String[] set1,
			final CMClass.CMObjectType[] set2) {
	...
	public final String ARCHON_LIST[] = combine(new String[] { "RESOURCE",
			"FACTION" }, CMClass.CMObjectType.values());

	public final Ammunition getNextAmmunition(String type,
			List<Ammunition> ammos) {
		for (final Ammunition ammo : ammos)
			if ((!ammo.amDestroyed()) && (ammo.usesRemaining() > 0)
					&& (ammo.ammunitionType().equalsIgnoreCase(type)))
				return ammo;
		return null;
	}
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if (commands.size() < 3) {
			if (tryArchon)
				mob.tell("LOAD what? Try " + CMParms.toStringList(ARCHON_LIST)
						+ " [CLASSNAME]");
			else
		...
		if (!tryArchon) {
			...
			final List<Item> baseAmmoItems = CMLib.english().fetchItemList(mob,
					mob, null, ammoV, Wearable.FILTER_UNWORNONLY, false);
			...
			if (baseAmmoItems.size() == 0)
				...
				if (baseItems.size() == 0)
					...
					for (final AmmunitionWeapon W : items) {
						Ammunition ammunition = getNextAmmunition(
								W.ammunitionType(), ammos);
						if (ammunition == null) {
							mob.tell("You are all out of " + W.ammunitionType()
									+ ".");
						} else
							while ((ammunition != null)
									&& ((W.ammunitionRemaining() < W
											.ammunitionCapacity()) || (!doneOne))) {
								CMMsg newMsg = CMClass
										.getMsg(mob, W, ammunition,
												CMMsg.MSG_RELOAD,
												"<S-NAME> reload(s) <T-NAME> with <O-NAME>.");
								if (mob.location().okMessage(mob, newMsg)) {
									doneOne = true;
									mob.location().send(mob, newMsg);
									ammunition = getNextAmmunition(
											W.ammunitionType(), ammos);
								} else
									break;
							}
			...
			if ((what.equalsIgnoreCase("FACTION"))
				...
				if (F == null)
					...
					mob.tell("Faction '" + F.name() + "' from file '" + name
							+ "' was loaded.");
				return false;
				...
				if ((!F.exists()) || (!F.canRead()))
					...
					StringBuffer buf = Resources.getFileResource(name, true); // enforces
																				// its
																				// own
																				// security
				...
				try {
					if (name.toUpperCase().endsWith(".JAVA")) {
						...
						Object CO = null;
						try {
							C = Class.forName("com.sun.tools.javac.Main", true,
									CMClass.instance());
							if (C != null)
								CO = C.newInstance();
						} catch (Exception e) {
						...
						if (C != null) {
							java.lang.reflect.Method M = C.getMethod("compile",
									new Class[] { args.getClass(),
											PrintWriter.class });
							Object returnVal = M.invoke(CO, new Object[] {
									args, pout });
							if ((returnVal instanceof Integer)
									&& (((Integer) returnVal).intValue() != 0)) {
					...
					if (unloadClassName.toUpperCase().endsWith(".CLASS"))
						unloadClassName = unloadClassName.substring(0,
								unloadClassName.length() - 6);
					unloadClassName = unloadClassName.replace('\\', '.');
					unloadClassName = unloadClassName.replace('/', '.');

					if (what.equalsIgnoreCase("CLASS")) {
						Object O = CMClass
								.getObjectOrPrototype(unloadClassName);
						if (O != null) {
					...
					if (whatType == null)
						...
						if (CMClass.loadClass(whatType, name, false)) {
							mob.tell(CMStrings.capitalizeAndLower(what) + " "
									+ name + " was successfully loaded.");
							return true;
						}
					}
				...
				mob.tell(CMStrings.capitalizeAndLower(what) + " " + name
						+ " was not loaded.");
			}
		}
		return false;
