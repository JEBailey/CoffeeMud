...
public class Destroy extends StdCommand {
	...
	public boolean mobs(MOB mob, Vector commands) {
		...
		boolean allFlag = ((String) commands.elementAt(2))
		...
		while (deadMOB != null) {
			...
			mob.location().showHappens(CMMsg.MSG_OK_VISUAL,
					deadMOB.name() + " vanishes in a puff of smoke.");
	...
	public void manufacturer(MOB mob, Vector commands) throws IOException {
		...
		if (mob.session()
				.confirm(
						"This will complete OBLITERATE the manufacturer '"
								+ manufacturerID
								+ "' forever.  This means all the stuff that is made by this manufacturer will get transferred to ACME. Are you SURE?! (y/N)?",
						"N")) {
			CMLib.tech().delManufacturer(manufacturer);
			mob.location().recoverRoomStats();
			Log.sysOut(mob.Name() + " destroyed manufacturer "
					+ manufacturer.name() + ".");
		}
	}
	...
	public void accounts(MOB mob, Vector commands) throws IOException {
		...
		if (mob.session()
				.confirm(
						"This will complete OBLITERATE the account '"
								+ theAccount.accountName() + "' and players '"
								+ playerList
								+ "' forever.  Are you SURE?! (y/N)?", "N")) {
			for (Enumeration<String> p = theAccount.getPlayers(); p
					.hasMoreElements();) {
				MOB deadMOB = CMLib.players().getLoadPlayer(p.nextElement());
				CMLib.players().obliteratePlayer(deadMOB, true, false);
				mob.tell("The user '" + CMParms.combine(commands, 2)
						+ "' is no more!\n\r");
				Log.sysOut("Mobs",
						mob.Name() + " destroyed user " + deadMOB.Name() + ".");
				deadMOB.destroy();
			}
			CMLib.players().obliterateAccountOnly(theAccount);
			mob.location().recoverRoomStats();
			Log.sysOut("Destroy", mob.Name() + " destroyed account "
					+ theAccount.accountName() + " and players '" + playerList
					+ "'.");
		}
	}

	public static boolean players(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
		...
		String name = CMStrings
				.capitalizeAndLower(CMParms.combine(commands, 2));
		...
		if (mob.session().confirm(
				"This will complete OBLITERATE the user '" + name
						+ "' forever.  Are you SURE?! (y/N)?", "N")) {
		...
		for (int i = 0; i < ac; ++i) {
			if (tArray[i] != null) {
				...
				if (R instanceof TickableGroup) {
					if (((TickableGroup) R).getName().equalsIgnoreCase(
							threadName))
						return tArray[i];
					final TickClient T = ((TickableGroup) R).getLastTicked();
					if ((T != null)
							&& (T.getName().equalsIgnoreCase(threadName)))
						return tArray[i];
				}
			}
		}
	...
	public void rooms(MOB mob, Vector commands) throws IOException {
		...
		if ((deadRoom == null) && (direction < 0)) {
			if (thecmd.equalsIgnoreCase("UNLINK"))
				...
				mob.tell("You have failed to specify a direction.  Try a VALID ROOM ID, or ("
						+ Directions.LETTERS() + ").\n\r");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a powerful spell.");
			return;
		...
		if (deadRoom != null) {
			if (!CMSecurity.isAllowed(mob, deadRoom,
			...
			if (!confirmed)
				if (!mob.session().confirm(
						"You are fixing to permanantly destroy Room \""
								+ deadRoom.roomID()
			...
			mob.location().showOthers(mob, null, CMMsg.MSG_NOISE,
					"The sound of massive destruction rings in your ears.");
			Log.sysOut("Rooms",
					mob.Name() + " destroyed room " + deadRoom.roomID() + ".");
		} else {
			...
			if ((mob.location().getGridParent() != null)
				...
				for (Iterator<WorldMap.CrossExit> i = GL.outerExits(); i
						.hasNext();) {
					WorldMap.CrossExit CE = i.next();
					if ((CE.out) && (CE.x == myX) && (CE.y == myY)
							&& (CE.dir == direction))
						GL.delOuterExit(CE);
				}
			...
			if (unRoom instanceof GridLocale) {
				...
				for (Iterator<WorldMap.CrossExit> i = GL.outerExits(); i
					...
					if ((!CE.out)
							&& (CE.dir == direction)
							&& (CE.destRoomID.equalsIgnoreCase(CMLib.map()
			...
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"A wall of inhibition falls " + inDirName + ".");
			final String dirName = useShipDirs ? Directions
					.getShipDirectionName(direction) : Directions
					.getDirectionName(direction);
			Log.sysOut("Rooms", mob.Name() + " unlinked direction " + dirName
					+ " from room " + mob.location().roomID() + ".");
		}
	}
	...
	public boolean items(MOB mob, Vector commands) {
		...
		boolean doneSomething = false;
		Item deadItem = null;
		deadItem = (srchRoom == null) ? null : srchRoom.findItem(srchContainer,
				itemID);
		if ((!allFlag) && (deadItem == null))
			deadItem = (srchMob == null) ? null : srchMob
					.findItem(null, itemID);
		while (deadItem != null) {
			mob.location().recoverRoomStats();
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					deadItem.name() + " disintegrates!");
			doneSomething = true;
			Log.sysOut("Items",
					mob.Name() + " destroyed item " + deadItem.name() + ".");
			if (srchMob != null)
			...
			deadItem = null;
			if (!allFlag)
				deadItem = (srchMob == null) ? null : srchMob.findItem(null,
						itemID);
			if (deadItem == null)
				deadItem = (srchRoom == null) ? null : srchRoom.findItem(null,
						itemID);
			if (!allFlag)
				break;
		}
	...
	public void areas(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
			...
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a thunderous spell.");
			return;
		}
		boolean confirmed = false;
		if ((commands.size() > 3)) {
			...
			if (((String) commands.get(2)).equalsIgnoreCase("all")) {
				...
				if (((String) commands.get(3)).equalsIgnoreCase("except")) {
					for (int i = 4; i < commands.size(); i++) {
						final Area A = CMLib.map().getArea(
								(String) commands.elementAt(i));
						if (A == null) {
							mob.tell("There is no such area as '"
									+ ((String) commands.elementAt(i)) + "'");
							mob.location().showOthers(mob, null,
									CMMsg.MSG_OK_ACTION,
									"<S-NAME> flub(s) a thunderous spell.");
		...
		for (String areaName : areaNames) {
			...
			if (!confirmed)
				if (mob.session().confirm(
						"Area: \"" + areaName + "\", OBLITERATE IT???", "N")) {
					if (mob.location().getArea().Name()
			...
			if (confirmed) {
				mob.location()
						.showHappens(CMMsg.MSG_OK_ACTION,
								"A thunderous boom of destruction is heard in the distance.");
	...
	public boolean races(MOB mob, Vector commands) {
		...
		CMClass.loadClass(CMObjectType.RACE, "com/planet_ink/coffee_mud/Races/"
				+ oldRID + ".class", true);
		...
		if (!oldR.ID().equals("StdRace"))
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"The diversity of the world just changed?!");
		else
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"The diversity of the world just decreased!");
		return true;
	}
	...
	public boolean components(MOB mob, Vector commands) {
		...
		CMLib.ableMapper().getAbilityComponentMap()
				.remove(classID.toUpperCase());
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The complication of skill usage just decreased!");
		return true;
	}
	...
	public boolean expertises(MOB mob, Vector commands) {
		...
		String classID = CMParms.combine(commands, 2);
		CMFile F = new CMFile(
				Resources.makeFileResourceName("skills/expertises.txt"), null,
		...
		if (removed) {
			...
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"The power of skill usage just decreased!");
	...
	public boolean titles(MOB mob, Vector commands) {
		...
		if (!CMLib.titles().isExistingAutoTitle(classID)) {
			mob.tell("'" + classID
					+ "' is not an existing auto-title, try LIST TITLES.");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return false;
		}
		...
		if (removed) {
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"The prestige of players just decreased!");
	...
	public boolean classes(MOB mob, Vector commands) {
		...
		String oldCID = C.ID();
		CMClass.loadClass(CMObjectType.CHARCLASS,
				"com/planet_ink/coffee_mud/CharClasses/" + oldCID + ".class",
				true);
		...
		if (!oldC.ID().equals("StdCharClass"))
			...
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"The employment of the world just decreased!");
		return true;
	}
	...
	public boolean abilities(MOB mob, Vector commands) {
		...
		if (!(O instanceof Ability)) {
			mob.tell("'" + classID
					+ "' can not be deleted, because it is also an "
					+ CMClass.getSimpleClassName(O) + ".");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return false;
		}
		...
		mob.location().showHappens(CMMsg.MSG_OK_ACTION,
				"The skill of the world just decreased!");
		return true;
	}
	...
	public void socials(MOB mob, Vector commands) throws IOException {
		if (commands.size() < 3) {
			...
			if (!((therest.equalsIgnoreCase("<T-NAME>")
					|| therest.equalsIgnoreCase("SELF") || therest
						.equalsIgnoreCase("ALL")))) {
				mob.session()
						.rawPrintln(
								"but fail to specify the proper second parameter.\n\rThe format is DESTROY SOCIAL [NAME] ([<T-NAME>], [SELF])\n\r");
				mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> flub(s) a powerful spell.");
				return;
			}
		...
		Social soc2 = CMLib.socials().fetchSocial(
				CMParms.combine(commands, 2).toUpperCase(), true);
		if (soc2 == null) {
		...
		if (mob.session().confirm(
				"Are you sure you want to delete that social (y/N)? ", "N")) {
			CMLib.socials().remove(soc2.name());
			CMLib.socials().save(mob);
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"The happiness of all mankind has just decreased!");
			Log.sysOut("SysopSocials",
					mob.Name() + " destroyed social " + soc2.name() + ".");
		} else
			mob.location().showHappens(CMMsg.MSG_OK_ACTION,
					"The happiness of all mankind has just increased!");
	}

	public static boolean destroyItem(MOB mob, Environmental dropThis,
			boolean quiet, boolean optimize) {
		String msgstr = null;
		int material = (dropThis instanceof Item) ? ((Item) dropThis)
				.material() : -1;
		if (!quiet)
		...
		CMMsg msg = CMClass.getMsg(mob, dropThis, null,
				CMMsg.MSG_NOISYMOVEMENT, (optimize ? CMMsg.MASK_OPTIMIZE : 0)
	...
	public void allQualify(MOB mob, Vector commands) throws IOException {
		...
		if (!subMap.containsKey(classD.toUpperCase().trim())) {
			mob.tell("All-Qualify entry (" + eachOrAll + ") ID '" + classD
					+ "' does not exist! Try LIST ALLQUALIFYS");
			mob.location().showOthers(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> flub(s) a spell..");
			return;
		}
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((!CMSecurity.isAllowedContainsAny(mob,
				CMSecurity.SECURITY_CMD_GROUP))
				&& (!CMSecurity.isAllowedContainsAny(mob, mob.location(),
						CMSecurity.SECURITY_KILL_GROUP))
				&& (!CMSecurity.isAllowed(mob, mob.location(),
						CMSecurity.SecFlag.BAN))
				&& (!CMSecurity.isAllowed(mob, mob.location(),
						CMSecurity.SecFlag.NOPURGE))) {
			...
			if (whatToDrop.toUpperCase().endsWith(".ALL")) {
				allFlag = true;
				whatToDrop = "ALL "
						+ whatToDrop.substring(0, whatToDrop.length() - 4);
			}
			int addendum = 1;
			String addendumStr = "";
			...
			while (doBugFix || ((allFlag) && (addendum <= maxToDrop))) {
				doBugFix = false;
				Item dropThis = mob.fetchItem(null, Wearable.FILTER_UNWORNONLY,
						whatToDrop + addendumStr);
				if ((dropThis == null) && (V.size() == 0)
						&& (addendumStr.length() == 0) && (!allFlag)) {
					dropThis = mob.fetchItem(null, Wearable.FILTER_WORNONLY,
							whatToDrop);
					if (dropThis != null) {
						int matType = dropThis.material()
						...
						if ((matType != RawMaterial.MATERIAL_GLASS)
							...
							mob.tell(dropThis.Name()
									+ " can not be easily destroyed.");
							return false;
						} else if ((!dropThis.amWearingAt(Wearable.WORN_HELD))
								&& (!dropThis.amWearingAt(Wearable.WORN_WIELD))) {
							mob.tell("You must remove that first.");
							return false;
						} else {
							CMMsg newMsg = CMClass.getMsg(mob, dropThis, null,
									CMMsg.MSG_REMOVE, null);
			...
			for (int i = 0; i < V.size(); i++) {
				if (destroyItem(mob, (Item) V.elementAt(i), false, true))
					didAnything = true;
				else if (V.elementAt(i) instanceof Coins)
					((Coins) V.elementAt(i)).putCoinsBack();
				else if (V.elementAt(i) instanceof RawMaterial)
		...
		for (Enumeration<JournalsLibrary.CommandJournal> e = CMLib.journals()
			...
			if ((CMJ.NAME().equals(commandType))
				...
				List<JournalsLibrary.JournalEntry> entries = CMLib.database()
						.DBReadJournalMsgs(CMJ.JOURNAL_NAME());

				if ((which <= 0) || (which > entries.size()))
					mob.tell("Please enter a valid " + CMJ.NAME().toLowerCase()
							+ " number to delete.  Use LIST " + CMJ.NAME()
							+ "S for more information.");
				else {
					JournalsLibrary.JournalEntry entry = entries.get(which - 1);
					CMLib.database().DBDeleteJournal(CMJ.JOURNAL_NAME(),
							entry.key);
		...
		if (commandType.equals("EXIT")) {
			...
			if (!CMSecurity.isAllowed(mob, mob.location(),
				...
				return errorOut(mob);
			String named = CMParms.combine(commands, 2);
			CMSecurity.DbgFlag flag = (CMSecurity.DbgFlag) CMath.s_valueOf(
					CMSecurity.DbgFlag.values(), named.toUpperCase().trim());
			if (flag == null) {
				mob.tell("'" + named + "' is not a valid flag.  Try: "
						+ CMParms.toStringList(CMSecurity.DbgFlag.values()));
				return false;
			}
			...
			if (which <= 0)
				...
				List<String> protectedOnes = Resources
						.getFileLineVector(Resources.getFileResource(
								"protectedplayers.ini", false));
				...
				Resources.updateFileResource("::protectedplayers.ini",
						newNoPurge);
				mob.tell("Ok.");
			}
			...
			if (whichT == null)
				...
				Log.sysOut("CreateEdit", mob.Name() + " destroyed thread "
						+ whichT.getName() + ".");
			...
			if (S == null)
				...
				if (S.getStatus() == Session.SessionStatus.LOGOUTFINAL)
					...
					mob.tell("Failed to gracefully shutdown: "
							+ S.getStatus().toString()
							+ ", but a forcable stop was issued.");
			}
			...
			if (which < 0)
				mob.tell("Please enter a valid journal name to delete.  Use List Journals for more information.");
					...
					"This will destroy all "
							+ CMLib.database().DBCountJournal(name, null, null)
							+ " messages.  Are you SURE (y/N)? ", "N")) {
					...
					mob.tell("Faction '" + name
						...
						&& (mob.session()
								.confirm(
										"Destroy faction '"
												+ F.factionID()
												+ "' -- this could have unexpected consequences in the future -- (N/y)? ",
										"N"))) {
					try {
						CMFile F2 = new CMFile(
								Resources.makeFileResourceName(CMLib.factions()
										.makeFactionFilename(F.factionID())),
								null);
						if (F2.exists())
							F2.deleteAll();
						else
						...
						Log.sysOut("CreateEdit",
								mob.Name() + " destroyed Faction " + F.name()
										+ " (" + F.factionID() + ").");
						mob.tell("Faction File '" + F.factionID()
								+ "' deleted.");
					} catch (Exception e) {
						Log.errOut("CreateEdit", e);
						mob.tell("Faction '" + F.factionID()
			...
			mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
					"^S<S-NAME> wave(s) <S-HIS-HER> arms...^?");
			if ((mob.session() != null)
					&& (mob.session().confirm("Destroy POLL " + P.getName()
							+ ", are you SURE? (Y/n)? ", "Y"))) {
				CMLib.polls().deletePoll(P);
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"^SThe world has grown a bit more certain.^?");
				Log.sysOut("CreateEdit",
			...
			if (commands.size() < 3)
				...
				if (C == null)
					...
					C.destroyClan();
					Log.sysOut("CreateEdit", "Clan '" + C.name()
							+ " destroyed by " + mob.Name() + ".");
				}
			}
			...
			if (commands.size() < 3)
				...
				if (G == null)
					...
					Log.sysOut("CreateEdit", "Government '" + G.getName()
							+ " destroyed by " + mob.Name() + ".");
				} else
			...
			Environmental thang = mob.location().fetchFromRoomFavorItems(null,
					allWord);
			if (thang == null)
				thang = mob.location().fetchFromMOBRoomFavorsItems(mob, null,
						allWord, Wearable.FILTER_ANY);
