...
public class Stat extends Skills {
	...
	public static final String[][] ABLETYPE_DESCS = {
			{ "EQUIPMENT", "EQ", "EQUIP" }, { "INVENTORY", "INVEN", "INV" },
			{ "QUESTWINS", "QUESTS", "QUEST", "QUESTWIN" },
			{ "TATTOOS", "TATTOO", "TATT" }, { "COMBAT" }, { "SCRIPTS" },
			{ "TITLES", "TITLE" }, { "ROOMSEXPLORED" }, { "AREASEXPLORED" },
			{ "WORLDEXPLORED" }, { "FACTIONS", "FACTION" }, };
	...
	public boolean showTableStats(MOB mob, int days, int scale, String rest) {
		...
		List<CoffeeTableRow> V = CMLib.database().DBReadStats(
				ENDQ.getTimeInMillis() - 1);
		if (V.size() == 0) {
			mob.tell("No Stats?!");
			return false;
		}
		...
		table.append("^xStatistics since "
				+ CMLib.time().date2String(ENDQ.getTimeInMillis())
				+ ":^.^N\n\r\n\r");
		if (skillUse)
			table.append(CMStrings.padRight("Skill", 25)
					+ CMStrings.padRight("Uses", 10)
		...
		if (skillUse) {
			...
			while ((V.size() > 0) && (curTime > (ENDQ.getTimeInMillis()))) {
				...
				for (int s = 0; s < set.size(); s++) {
					...
					for (int x = 0; x < allSkills.size(); x++)
						T.totalUp("A"
								+ ((Ability) allSkills.elementAt(x)).ID()
										.toUpperCase(), totals[x]);
				}
				if (scale == 0)
					break;
			}
			...
			while ((V.size() > 0) && (curTime > (ENDQ.getTimeInMillis()))) {
				...
				for (int s = 0; s < set.size(); s++) {
					...
					for (int x = 0; x < CMLib.quests().numQuests(); x++)
						T.totalUp(
								"U"
										+ T.tagFix(CMLib.quests().fetchQuest(x)
												.name()), totals[x]);
				}
				if (scale == 0)
					break;
			...
			for (int x = 0; x < CMLib.quests().numQuests(); x++) {
				...
				table.append(CMStrings.padRight(Q.name(), 30)
						+ CMStrings
								.centerPreserve(
										""
												+ totals[x][CoffeeTableRow.STAT_QUESTSTARTATTEMPT],
										5)
						+ CMStrings
								.centerPreserve(
										""
												+ totals[x][CoffeeTableRow.STAT_QUESTTIMESTART],
										5)
						+ CMStrings
								.centerPreserve(
										""
												+ totals[x][CoffeeTableRow.STAT_QUESTFAILEDSTART],
										5)
						+ CMStrings.centerPreserve(""
								+ totals[x][CoffeeTableRow.STAT_QUESTACCEPTED],
								5)
						+ CMStrings.centerPreserve(""
								+ totals[x][CoffeeTableRow.STAT_QUESTSUCCESS],
								5)
						+ CMStrings
								.centerPreserve(
										""
												+ totals[x][CoffeeTableRow.STAT_QUESTFAILED],
										5)
						+ CMStrings.centerPreserve(""
								+ totals[x][CoffeeTableRow.STAT_QUESTDROPPED],
								5)
						+ CMStrings.centerPreserve(""
								+ totals[x][CoffeeTableRow.STAT_QUESTTIMESTOP],
								5)
						+ CMStrings.centerPreserve(""
								+ totals[x][CoffeeTableRow.STAT_QUESTSTOP], 5));
				table.append("\n\r");
			}
			table.append("\n\r");
		} else
			while ((V.size() > 0) && (curTime > (ENDQ.getTimeInMillis()))) {
				...
				totals[CoffeeTableRow.STAT_TICKSONLINE] = (totals[CoffeeTableRow.STAT_TICKSONLINE] * CMProps
						.getTickMillis()) / scale / (1000 * 60);
				double avgOnline = (numberOnlineCounter > 0) ? CMath.div(
						numberOnlineTotal, numberOnlineCounter) : 0.0;
				avgOnline = CMath.div(Math.round(avgOnline * 10.0), 10.0);
				table.append(CMStrings
						.padRight(
								CMLib.time().date2DateString(curTime + 1)
										+ " - "
										+ CMLib.time().date2DateString(
												lastCur - 1), 25)
						+ CMStrings.centerPreserve(""
								+ totals[CoffeeTableRow.STAT_LOGINS], 5)
						+ CMStrings.centerPreserve("" + highestOnline, 5)
						+ CMStrings.centerPreserve("" + avgOnline, 5)
						+ CMStrings.centerPreserve(""
								+ totals[CoffeeTableRow.STAT_TICKSONLINE], 5)
						+ CMStrings.centerPreserve(""
								+ totals[CoffeeTableRow.STAT_NEWPLAYERS], 5)
						+ CMStrings.centerPreserve(""
								+ totals[CoffeeTableRow.STAT_DEATHS], 5)
						+ CMStrings.centerPreserve(""
								+ totals[CoffeeTableRow.STAT_PKDEATHS], 5)
						+ CMStrings.centerPreserve(""
								+ totals[CoffeeTableRow.STAT_CLASSCHANGE], 5)
						+ CMStrings.centerPreserve(""
								+ totals[CoffeeTableRow.STAT_PURGES], 5)
						+ CMStrings.centerPreserve(""
								+ totals[CoffeeTableRow.STAT_MARRIAGES], 5)
						+ "\n\r");
				if (scale == 0)
					break;
			}
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((commands.size() > 0)
			...
			StringBuilder msg = new StringBuilder(
					"STAT allows the following options: \n\r");
		...
		String s1 = (commands.size() > 0) ? ((String) commands.elementAt(0))
				.toUpperCase() : "";
		String s2 = (commands.size() > 1) ? ((String) commands.elementAt(1))
				.toUpperCase() : "";
		...
		if (commands.size() > 1) {
			...
			if (ableTypes == -1)
				for (int a = 0; a < Ability.ACODE_DESCS.length; a++) {
					if ((Ability.ACODE_DESCS[a] + "S").equals(s)
							|| (Ability.ACODE_DESCS[a]).equals(s)) {
						ableTypes = a;
						commands.removeElementAt(0);
						break;
					}
		...
		if (((target.isMonster()) && (!CMSecurity.isAllowed(mob,
				target.location(), CMSecurity.SecFlag.CMDMOBS)))
				|| ((!target.isMonster()) && (!CMSecurity.isAllowed(mob,
						target.location(), CMSecurity.SecFlag.CMDPLAYERS)))) {
		...
		if (ableTypes >= 0) {
			...
			int q = 1;
			for (Enumeration<ScriptingEngine> e = target.scripts(); e
					.hasMoreElements(); q++) {
			...
			str.append("Tattoos:");
			for (Enumeration<MOB.Tattoo> e = target.tattoos(); e
					.hasMoreElements();)
			...
			str.append("Factions:\n\r");
			for (Enumeration<String> f = target.fetchFactions(); f
					.hasMoreElements();) {
				Faction F = CMLib.factions().getFaction(f.nextElement());
				if (F != null)
					str.append("^W[^H" + F.name() + "^N(" + F.factionID()
							+ "): " + target.fetchFaction(F.factionID())
							+ "^W]^N, ");
			}
			str.append("\n\r");
		} else if (ableTypes == ABLETYPE_WORLDEXPLORED) {
			if (target.playerStats() != null)
				str.append(target.name() + " has explored "
			...
			if (target.playerStats() != null) {
				...
				str = new StringBuilder(str.toString().substring(0,
			...
			str.append("^c" + CMStrings.padRight("Base Attack", 40) + ": ^W"
					+ base + "\n\r");
			for (int i = 0; i < target.numItems(); i++) {
				...
				if ((I != null) && (!I.amWearingAt(Wearable.IN_INVENTORY))) {
					...
					testMOB(target, M, I);
					int diff = CMLib.combat().adjustedAttackBonus(M, null)
							- base;
			...
			recoverMOB(M);
			str.append("^W-------------------------\n\r");
			str.append("^C" + CMStrings.padRight("Total", 40) + ": ^W"
					+ CMLib.combat().adjustedAttackBonus(target, null) + "\n\r");
			str.append("\n\r");
			base = CMLib.combat().adjustedArmor(M);
			str.append("^C" + CMStrings.padRight("Base Armor", 40) + ": ^W"
					+ base + "\n\r");
			for (int i = 0; i < target.numItems(); i++) {
				Item I = target.getItem(i);
				if (I != null) {
					...
					base = CMLib.combat().adjustedArmor(M);
					testMOB(target, M, I);
					int diff = CMLib.combat().adjustedArmor(M) - base;
					reportOnDiffMOB(I, diff, str);
				}
			}
			recoverMOB(M);
			...
			str.append("^W-------------------------\n\r");
			str.append("^C" + CMStrings.padRight("Total", 40) + ": ^W"
					+ CMLib.combat().adjustedArmor(target) + "\n\r");
			str.append("\n\r");
			base = CMLib.combat().adjustedDamage(M, null, null, 0, false);
			str.append("^C" + CMStrings.padRight("Base Damage", 40) + ": ^W"
					+ base + "\n\r");
			for (int i = 0; i < target.numItems(); i++) {
				Item I = target.getItem(i);
				if (I != null) {
			...
			str.append("^W-------------------------\n\r");
			str.append("^C"
					+ CMStrings.padRight("Total", 40)
					+ ": ^W"
					+ CMLib.combat().adjustedDamage(target, null, null, 0,
							false) + "\n\r");
			str.append("\n\r");
			base = (int) Math.round(M.phyStats().speed() * 100);
			str.append("^C" + CMStrings.padRight("Base Attacks%", 40) + ": ^W"
					+ base + "\n\r");
			for (int i = 0; i < target.numItems(); i++) {
				Item I = target.getItem(i);
				if (I != null) {
					...
					base = (int) Math.round(M.phyStats().speed() * 100);
					testMOB(target, M, I);
					int diff = (int) Math.round(M.phyStats().speed() * 100)
							- base;
					reportOnDiffMOB(I, diff, str);
				}
			}
			...
			str.append("^W-------------------------\n\r");
			str.append("^C" + CMStrings.padRight("Total", 40) + ": ^W"
					+ (int) Math.round(target.phyStats().speed() * 100)
					+ "\n\r");
			str.append("\n\r");
			base = M.maxState().getHitPoints();
			str.append("^C" + CMStrings.padRight("Base Hit Points", 40)
					+ ": ^W" + base + "\n\r");
			for (int i = 0; i < target.numItems(); i++) {
				Item I = target.getItem(i);
				if (I != null) {
					...
					base = M.maxState().getHitPoints();
					testMOB(target, M, I);
					int diff = M.maxState().getHitPoints() - base;
					reportOnDiffMOB(I, diff, str);
				}
			}
			recoverMOB(M);
	...
	public void reportOnDiffMOB(Environmental test, int diff, StringBuilder str) {
		if (diff > 0)
			str.append("^C" + CMStrings.padRight(test.Name(), 40) + ": ^W+"
					+ diff + "\n\r");
		else if (diff < 0)
			str.append("^C" + CMStrings.padRight(test.Name(), 40) + ": ^W"
					+ diff + "\n\r");
