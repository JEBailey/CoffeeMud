...
public class Mount extends StdCommand {
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		Rider RI = null;
		if (commands.size() > 1) {
			Item I = mob.location().findItem(null,
			...
			if (RI == null) {
				...
				if (M != null) {
					if (!CMLib.flags().canBeSeenBy(M, mob)) {
						mob.tell("You don't see "
								+ ((String) commands.firstElement()) + " here.");
						return false;
					}
					if ((!CMLib.flags().isBoundOrHeld(M))
							&& (!M.willFollowOrdersOf(mob))) {
		...
		recipient = CMLib.english().fetchEnvironmental(possRecipients,
				CMParms.combine(commands, 0), true);
		if (recipient == null)
			recipient = CMLib.english().fetchEnvironmental(possRecipients,
					CMParms.combine(commands, 0), false);
		if (recipient == null)
			recipient = mob.location().fetchFromRoomFavorMOBs(null,
		...
		if (recipient instanceof Rideable) {
			if (RI != null)
				...
				mountStr = "<S-NAME> "
						+ ((Rideable) recipient).mountString(CMMsg.TYP_MOUNT,
								mob) + " <T-NAMESELF>.";
		} else {
			if (RI != null)
		...
		CMMsg msg = CMClass.getMsg(mob, recipient, RI, CMMsg.MSG_MOUNT,
				mountStr);
