...
public class Unload extends StdCommand {
	...
	final String[] ARCHON_LIST = { "CLASS", "HELP", "USER", "AREA", "FACTION",
			"ALL", "FILE", "RESOURCE", "INIFILE", "[FILENAME]" };

	public boolean execute(MOB mob, Vector commands, int metaFlags)
			throws java.io.IOException {
		if (mob == null)
			return true;
		boolean tryArchon = CMSecurity.isAllowed(mob, mob.location(),
				CMSecurity.SecFlag.LOADUNLOAD);
		if (commands.size() < 2) {
			if (tryArchon)
				mob.tell("UNLOAD what? Try "
						+ CMParms.toStringList(ARCHON_LIST));
			else
		...
		if (!tryArchon) {
			...
			final List<Item> baseItems = CMLib.english().fetchItemList(mob,
					mob, null, commands, Wearable.FILTER_ANY, false);
			...
			if (baseItems.size() == 0)
				...
				for (final AmmunitionWeapon W : items) {
					Item ammunition = CMLib.coffeeMaker().makeAmmunition(
							W.ammunitionType(), W.ammunitionRemaining());
					CMMsg newMsg = CMClass.getMsg(mob, W, ammunition,
							CMMsg.MSG_UNLOAD,
							"<S-NAME> unload(s) <O-NAME> from <T-NAME>.");
					if (mob.location().okMessage(mob, newMsg))
						mob.location().send(mob, newMsg);
				}
		} else {
			String what = (String) commands.elementAt(1);
			if ((what.equalsIgnoreCase("CLASS") || (CMClass
					.findObjectType(what) != null))
				...
				if (what.equalsIgnoreCase("CLASS")) {
					Object O = CMClass.getObjectOrPrototype((String) commands
							.elementAt(2));
					if (O != null) {
				...
				if (whatType == null)
					mob.tell("Don't know how to load a '" + what
					...
					for (int i = 0; i < commands.size(); i++) {
						...
						if (!(O instanceof CMObject))
							mob.tell("Class '" + name
									+ "' was not found in the class loader.");
						else if (!CMClass.delClass(whatType, (CMObject) O))
							mob.tell("Failed to unload class '" + name
			...
			if ((((String) commands.elementAt(1)).equalsIgnoreCase("USER"))
				...
				if (which.equalsIgnoreCase("all"))
					for (Enumeration e = CMLib.players().players(); e
							.hasMoreElements();)
				...
				boolean saveFirst = mob.session().confirm("Save first (Y/n)?",
						"Y");
			...
			if ((((String) commands.elementAt(1)).equalsIgnoreCase("FACTION"))
				...
				String which = CMParms.combine(commands, 2);
				if (which.length() == 0) {
					// No factions specified. That's fine, they must mean ALL
					// FACTIONS!!! hahahahaha
					CMLib.factions().removeFaction(null);
				} else {
					...
					mob.tell("Unknown Faction '" + which
							+ "'.  Use LIST FACTIONS.");
					return false;
				}
			} else
			...
			if ((((String) commands.elementAt(1)).equalsIgnoreCase("AREA"))
			...
			} else if (("EXPERTISE".startsWith(((String) commands.elementAt(1))
					.toUpperCase()))
					&& (CMSecurity.isAllowed(mob, mob.location(),
							CMSecurity.SecFlag.EXPERTISE))) {
				...
				if (!k.hasNext()) {
					mob.tell("Unknown resource '" + which
							+ "'.  Use LIST RESOURCES.");
					return false;
				}
				for (; k.hasNext();) {
				...
				if (!F1.exists()) {
					...
					if (x < 0)
						x = which.lastIndexOf(' ');
					if (x >= 0)
						F1 = new CMFile(which.substring(x + 1).trim(), mob,
								CMFile.FLAG_FORCEALLOW);
				}
				if (!F1.exists()) {
					F1 = new CMFile(Resources.buildResourcePath(which), mob,
							CMFile.FLAG_FORCEALLOW);
					if (!F1.exists()) {
						int x = which.indexOf(':');
						if (x < 0)
							x = which.lastIndexOf(' ');
						if (x >= 0)
							F1 = new CMFile(Resources.buildResourcePath(which
									.substring(x + 1).trim()), mob,
									CMFile.FLAG_FORCEALLOW);
					}
				}
				if (F1.exists()) {
					CMFile F2 = new CMFile(F1.getVFSPathAndName(), mob,
				...
				if (!F1.exists()) {
					...
					if (x < 0)
						x = str.lastIndexOf(' ');
					if (x >= 0)
						F1 = new CMFile(str.substring(x + 1).trim(), mob,
								CMFile.FLAG_FORCEALLOW);
				}
				if (!F1.exists()) {
					F1 = new CMFile(Resources.buildResourcePath(str), mob,
							CMFile.FLAG_FORCEALLOW);
					if (!F1.exists()) {
						int x = str.indexOf(':');
						if (x < 0)
							x = str.lastIndexOf(' ');
						if (x >= 0)
							F1 = new CMFile(Resources.buildResourcePath(str
									.substring(x + 1).trim()), mob,
									CMFile.FLAG_FORCEALLOW);
					}
				}
				if (F1.exists()) {
					...
					mob.tell("Unknown resource type '"
							+ ((String) commands.elementAt(1)) + ". Try "
							+ CMParms.toStringList(ARCHON_LIST) + ".");
			}
		}
		return false;
