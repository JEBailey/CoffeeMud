...
public class WillQualify extends Skills {
	...
	public StringBuffer getQualifiedAbilities(MOB viewerM, MOB ableM,
			String classID, int maxLevel, String prefix, HashSet<Object> types) {
		...
		List<AbilityMapper.QualifyingID> DV = CMLib.ableMapper()
				.getClassAllowsList(classID);
		for (int l = 0; l <= highestLevel; l++) {
			StringBuffer thisLine = new StringBuffer("");
			for (Enumeration a = CMLib.ableMapper()
					.getClassAbles(classID, true); a.hasMoreElements();) {
				AbilityMapper.AbilityMapping cimable = (AbilityMapper.AbilityMapping) a
						.nextElement();
				if ((cimable.qualLevel == l) && (!cimable.isSecret)) {
					Ability A = CMClass.getAbility(cimable.abilityID);
					if ((A != null)
							&& ((types.size() == 0)
									|| (types.contains(Integer.valueOf(A
											.classificationCode()
											& Ability.ALL_ACODES))) || (types
										.contains(Integer.valueOf(A
												.classificationCode()
												& Ability.ALL_DOMAINS))))
							&& (CMLib.ableMapper()
									.getCommonSkillLimit(ableM, A).specificSkillLimit > 0)) {
						if ((++col) > 2) {
							thisLine.append("\n\r");
							col = 1;
						}
				...
				if (E != null) {
					...
					if ((minLevel == l)
							&& ((types.size() == 0)
									...
									|| types.contains(E.ID.toUpperCase()) || types
						...
						thisLine.append("^N[^H"
								+ CMStrings.padRight("" + l, 3)
								+ "^?] "
								+ CMStrings.padRight("^<HELP^>" + E.name
										+ "^</HELP^>", 19)
								+ " "
								+ CMStrings.padRight(E.costDescription(),
										(col == 2) ? 12 : 13));
					}
				}
			}
	...
	public boolean execute(MOB mob, Vector commands, int metaFlags)
		...
		if ((commands.size() > 0)
				&& (CMath.isNumber((String) commands.firstElement()))) {
		...
		if (commands.size() > 0) {
			CharClass C2 = CMClass.findCharClass((String) commands
					.firstElement());
			if (C2 != null) {
				C = C2;
				commands.removeElementAt(0);
		...
		while (commands.size() > 0) {
			String str = ((String) commands.firstElement()).toUpperCase()
					.trim();
			...
			if (x >= 0)
				...
				if (x < 0)
					x = CMParms.indexOf(Ability.DOMAIN_DESCS,
							str.replace(' ', '_'));
				if (x < 0) {
					if ((CMLib.expertises().findDefinition(str, false) == null)
						...
						mob.tell("'"
								+ str
								+ "' is not a valid skill type, domain, expertise, or character class.");
						mob.tell(willQualErr);
						return false;
					}
		...
		msg.append("At level " + level + " of class '" + C.name()
