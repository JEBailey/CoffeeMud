...
public class StdSpaceShip implements Area, SpaceShip {
	...
	protected TimeClock localClock = (TimeClock) CMClass
			.getCommon("DefaultTimeClock");
	protected String currency = "";
	private long expirationDate = 0;
	protected int atmosphere = RawMaterial.RESOURCE_AIR; // at least for
															// awhile...
	...
	protected double omlCoeff = SpaceObject.ATMOSPHERIC_DRAG_STREAMLINE
	...
	protected volatile long nextStaleCheck = System.currentTimeMillis()
			+ STALE_AIR_INTERVAL;
	protected volatile long nextStaleWarn = System.currentTimeMillis()
			+ (60 * 1000);
	...
	public LocationRoom getIsDocked() {
		return (LocationRoom) CMLib.map().getRoom(savedDock);
	}
	...
	public int getAtmosphere() {
		return atmosphere == ATMOSPHERE_INHERIT ? RawMaterial.RESOURCE_AIR
				: atmosphere;
	}
	...
	public long[] coordinates() {
		return (shipItem != null) ? shipItem.coordinates() : new long[3];
	}

	@Override
	public void setCoords(long[] coords) {
		if (shipItem != null)
			shipItem.setCoords(coords);
	}

	@Override
	public double[] direction() {
		return (shipItem != null) ? shipItem.direction() : new double[2];
	}

	@Override
	public double roll() {
		return (shipItem != null) ? shipItem.roll() : 0;
	}

	@Override
	public void setRoll(double dir) {
		if (shipItem != null)
			shipItem.setRoll(dir);
	}

	@Override
	public void setDirection(double[] dir) {
		if (shipItem != null)
			shipItem.setDirection(dir);
	}

	@Override
	public double[] facing() {
		return (shipItem != null) ? shipItem.facing() : new double[2];
	}

	@Override
	public void setFacing(double[] dir) {
		if (shipItem != null)
			shipItem.setFacing(dir);
	}

	@Override
	public long speed() {
		return (shipItem != null) ? shipItem.speed() : 0;
	}
	...
	public SpaceObject knownTarget() {
		return (shipItem != null) ? shipItem.knownTarget() : null;
	}

	@Override
	public void setKnownTarget(SpaceObject O) {
		if (shipItem != null)
			shipItem.setKnownTarget(O);
	}
	...
	protected void cloneFix(StdSpaceShip ship) {
		...
		for (Enumeration<ScriptingEngine> e = ship.scripts(); e
				.hasMoreElements();) {
			SE = e.nextElement();
			if (SE != null)
	...
	public CMObject copyOf() {
		try {
			...
			// CMClass.bumpCounter(E,CMClass.CMObjectType.AREA);//removed for
			// mem & perf
				...
				if (command != null) {
					...
					if (parms != null) {
						if ((command == Technical.TechCommand.AIRREFRESH)
							...
							int numToClear = (int) Math.round(CMath.mul(
									staleAirList.size(), pct));
							while ((numToClear > 0)
									&& (staleAirList.size() > 0)) {
								String roomID = staleAirList.iterator().next();
								staleAirList.remove(roomID);
								changeRoomAir(getRoom(roomID), null,
										atmoResource);
								numToClear--;
							}
							changeRoomAir(getRandomMetroRoom(), null,
									atmoResource);
		...
		if (R.getAtmosphere() != atmoResource) {
			if (atmoResource == 0) {
				R.showHappens(CMMsg.MSG_OK_ACTION,
						RawMaterial.CODES.NAME(R.getAtmosphere()).toLowerCase()
								+ " rushes out of the room.");
				if ((notifyRoom != null) && (notifyRoom != R))
					notifyRoom
							.showHappens(CMMsg.MSG_OK_ACTION, RawMaterial.CODES
									.NAME(R.getAtmosphere()).toLowerCase()
									+ " rushes out of the room.");
			} else {
				R.showHappens(CMMsg.MSG_OK_ACTION,
						RawMaterial.CODES.NAME(atmoResource).toLowerCase()
								+ " rushes into the room.");
				if ((notifyRoom != null) && (notifyRoom != R))
					notifyRoom.showHappens(CMMsg.MSG_OK_ACTION,
							RawMaterial.CODES.NAME(atmoResource).toLowerCase()
									+ " rushes into the room.");
			}
	...
	protected void moveAtmosphereOut(Set<Room> doneRooms, Room startRoom,
			int atmo) {
				...
				if ((staleAirList.contains(R.roomID()))
					...
					if (atmo > 0)
						for (int i = 0; i < R.numInhabitants(); i++) {
							...
							if ((M != null)
									&& (!M.isMonster())
									&& (!CMLib.flags().canBreatheThis(M,
											RawMaterial.RESOURCE_NOTHING)))
								M.tell("The "
										+ RawMaterial.CODES.NAME(atmo)
												.toLowerCase()
										+ " is seaming a bit stale.");
						}
				}
			}
		}
		if (System.currentTimeMillis() >= nextStaleCheck) {
			...
			for (Enumeration<Room> r = getProperMap(); r.hasMoreElements();) {
				...
				if (!staleAirList.contains(R.roomID()))
					...
					R.setAtmosphere(RawMaterial.RESOURCE_NOTHING); // WE NOW
																	// HAVE A
																	// VACUUM
																	// HERE!!!
			}
		...
		for (Enumeration<Room> r = getProperMap(); r.hasMoreElements();) {
			...
			for (int d = 0; d < Directions.NUM_DIRECTIONS(); d++)
				if ((R.getRawExit(d) != null)
						&& ((R.rawDoors()[d] == null) || (R.rawDoors()[d]
								.getArea() != this))) {
					R.rawDoors()[d] = roomR;
					shipExitCache.add(new Pair<Room, Integer>(R, Integer
							.valueOf(d)));
				}
		}
	}
	...
	public String getNewRoomID(Room startRoom, int direction) {
		...
		try {
			...
			if (!CMLib.flags().isSavable(this)) {
				for (Enumeration<String> i = getProperRoomnumbers()
						.getRoomIDs(); i.hasMoreElements();) {
		...
		if (!CMLib.flags().isSavable(this)) {
			for (int i = lowest; i <= highest + 1000; i++) {
				if ((!set.contains(i)) && (getRoom(Name() + "#" + i) == null))
					return Name() + "#" + i;
			}
		}
		for (int i = lowest; i <= highest + 1000; i++) {
	...
	public Room getRoom(String roomID) {
		...
		synchronized (myRooms) {
			...
			while (start <= end) {
				int mid = (end + start) / 2;
				int comp = myRooms.elementAt(mid).roomID()
						.compareToIgnoreCase(roomID);
				if (comp == 0)
	...
	private static final String[] CODES = { "CLASS", "CLIMATE", "DESCRIPTION",
			"TEXT", "THEME", "BLURBS", "OMLCOEFF", "RADIUS" };
	...
	public void setStat(String code, String val) {
		...
		case 0:
			return;
		case 1:
			setClimateType(CMath.s_parseBitIntExpression(Area.CLIMATE_DESCS,
					val));
			break;
		case 2:
			setDescription(val);
