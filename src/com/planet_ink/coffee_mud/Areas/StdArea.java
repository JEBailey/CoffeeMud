...
import com.planet_ink.coffee_mud.Common.interfaces.CMIntegerGrouper;
...
public class StdArea implements Area {
	...
	protected STreeMap<String, Room> properRooms = new STreeMap<String, Room>(
			new RoomIDComparator());
	...
	public void setCurrency(String newCurrency) {
		if (currency.length() > 0) {
			...
			for (Enumeration e = CMLib.map().areas(); e.hasMoreElements();)
				CMLib.beanCounter().getCurrencySet(
						((Area) e.nextElement()).getCurrency());
		} else {
			currency = newCurrency;
	...
	protected Enumeration<String> allBlurbFlags() {
		MultiEnumeration<String> multiEnum = new MultiEnumeration<String>(
				areaBlurbFlags());
	...
	protected Climate climateObj = (Climate) CMClass
			.getCommon("DefaultClimate");
	...
	// protected void
	// finalize(){CMClass.unbumpCounter(this,CMClass.CMObjectType.AREA);}//removed
	// for mem & perf
	public void destroy() {
	...
	public boolean isSavable() {
		return ((!amDestroyed)
				&& (!CMath.bset(flags(), Area.FLAG_INSTANCE_CHILD)) && (CMLib
					.flags().isSavable(this)));
	}
	...
	public void setAreaState(State newState) {
		if ((newState == State.ACTIVE)
			...
			if (!CMLib.threads().isTicking(this, Tickable.TICKID_AREA))
				Log.errOut("StdArea", "Area " + name()
						+ " failed to start ticking.");
		}
		flag = newState;
		derivedClimate = CLIMASK_INHERIT;
		derivedAtmo = ATMOSPHERE_INHERIT;
	...
	public String getNewRoomID(Room startRoom, int direction) {
		...
		CMIntegerGrouper set = (CMIntegerGrouper) CMClass
				.getCommon("DefaultCMIntegerGrouper");
		try {
			String roomID = null;
			int newnum = 0;
	...
	protected void cloneFix(StdArea areaA) {
		...
		for (final Enumeration<Ability> a = areaA.effects(); a
				.hasMoreElements();) {
		...
		for (Enumeration<ScriptingEngine> e = areaA.scripts(); e
				.hasMoreElements();) {
			SE = e.nextElement();
			if (SE != null)
	...
	public String[] finalItemPricingAdjustments() {
		...
		if (s.length > 0)
			return s;
		return CMParms.toStringArray(CMParms.parseSemicolons(
				CMProps.getVar(CMProps.Str.PRICEFACTORS).trim(), true));
	}
		...
		if (!msg.source().isMonster()) {
			...
			if ((flag == State.PASSIVE)
					&& ((msg.sourceMinor() == CMMsg.TYP_ENTER)
							...
							.sourceMinor() == CMMsg.TYP_FLEE)))
				flag = State.ACTIVE;
		}
		if ((flag == State.FROZEN) || (flag == State.STOPPED)
		...
		if ((getThemeCode() > 0)
				...
				if (room != null) {
					if (room.getArea() == this)
						room.showHappens(CMMsg.MSG_OK_VISUAL,
								"Magic doesn't seem to work here.");
					else
						room.showHappens(CMMsg.MSG_OK_VISUAL,
								"Magic doesn't seem to work there.");
				}

				return false;
			}
		} else if ((getTheme() > 0)
				&& (!CMath.bset(getTheme(), Area.THEME_TECHNOLOGY))) {
			...
			default:
				if (msg.tool() instanceof Electronics) {
					...
					if (room != null)
						room.showHappens(CMMsg.MSG_OK_VISUAL,
								"Technology doesn't seem to work here.");
					return false;
				}
				break;
			}
	...
	public void affectPhyStats(final Physical affected,
		...
		final int senses = phyStats.sensesMask()
				& (~(PhyStats.SENSE_UNLOCATABLE | PhyStats.CAN_NOT_SEE));
		if (senses > 0)
			affectableStats
					.setSensesMask(affectableStats.sensesMask() | senses);
		...
		if ((affected instanceof Room)
			...
			if (((C == null)
					|| (((C.weatherType((Room) affected) == Climate.WEATHER_BLIZZARD) || (C
							.weatherType((Room) affected) == Climate.WEATHER_DUSTSTORM)) && (!CMSecurity
							.isDisabled(CMSecurity.DisFlag.DARKWEATHER))) || ((getTimeObj()
					.getTODCode() == TimeClock.TIME_NIGHT) && (!CMSecurity
					.isDisabled(CMSecurity.DisFlag.DARKNIGHTS))))
	...
	public void addScript(ScriptingEngine S) {
		...
		if (!scripts.contains(S)) {
			...
			for (int s = 0; s < scripts.size(); s++) {
				...
				if ((S2 != null)
						&& (S2.getScript().equalsIgnoreCase(S.getScript())))
					return;
			}
	...
	protected int[] buildAreaIStats() {
		...
		statData[Area.Stats.VISITABLE_ROOMS.ordinal()] = getProperRoomnumbers()
				.roomCountAllAreas();
		...
		for (Enumeration<Room> r = getProperMap(); r.hasMoreElements();) {
			...
			for (int i = 0; i < R.numInhabitants(); i++) {
				...
				if ((mob != null) && (mob.isMonster())) {
					...
					if ((theFaction != null)
							&& (mob.fetchFaction(theFaction.factionID()) != Integer.MAX_VALUE)) {
						alignRanges.add(Integer.valueOf(mob
								.fetchFaction(theFaction.factionID())));
						totalAlignments += mob.fetchFaction(theFaction
								.factionID());
					}
		...
		if ((statData[Area.Stats.POPULATION.ordinal()] == 0)
			...
			statData[Area.Stats.MED_LEVEL.ordinal()] = levelRanges.get(
					(int) Math.round(Math.floor(CMath.div(levelRanges.size(),
							2.0)))).intValue();
			statData[Area.Stats.MED_ALIGNMENT.ordinal()] = alignRanges.get(
					(int) Math.round(Math.floor(CMath.div(alignRanges.size(),
							2.0)))).intValue();
			statData[Area.Stats.AVG_LEVEL.ordinal()] = (int) Math.round(CMath
					.div(statData[Area.Stats.TOTAL_LEVELS.ordinal()],
							statData[Area.Stats.POPULATION.ordinal()]));
			statData[Area.Stats.AVG_ALIGNMENT.ordinal()] = (int) Math
					.round(((double) totalAlignments)
							/ ((double) statData[Area.Stats.POPULATION
									.ordinal()]));
		}
		return statData;
	}
	...
	protected StringBuffer buildAreaStats(int[] statData) {
		...
		s.append("Number of rooms: ^H"
				+ statData[Area.Stats.VISITABLE_ROOMS.ordinal()] + "^N\n\r");
		Faction theFaction = null;
		for (Enumeration<Faction> e = CMLib.factions().factions(); e
				.hasMoreElements();) {
			Faction F = e.nextElement();
		...
		if (statData[Area.Stats.POPULATION.ordinal()] == 0) {
			if (getProperRoomnumbers().roomCountAllAreas() / 2 < properRooms
					.size())
				s.append("Population     : ^H0^N\n\r");
		} else {
			s.append("Population     : ^H"
					+ statData[Area.Stats.POPULATION.ordinal()] + "^N\n\r");
			String currName = CMLib.beanCounter().getCurrency(this);
			if (currName.length() > 0)
				s.append("Currency       : ^H"
						+ CMStrings.capitalizeAndLower(currName) + "^N\n\r");
			else
			...
			if (B != null) {
				...
				if (ruler.length() > 0) {
					...
					if (C != null)
						s.append("Controlled by  : ^H" + C.getGovernmentName()
								+ " " + C.name() + "^N\n\r");
				}
			}
			s.append("Level range    : ^H"
					+ statData[Area.Stats.MIN_LEVEL.ordinal()] + "^N to ^H"
					+ statData[Area.Stats.MAX_LEVEL.ordinal()] + "^N\n\r");
			s.append("Average level  : ^H"
					+ statData[Area.Stats.AVG_LEVEL.ordinal()] + "^N\n\r");
			s.append("Median level   : ^H"
					+ statData[Area.Stats.MED_LEVEL.ordinal()] + "^N\n\r");
			if (theFaction != null)
				s.append("Avg. "
						+ CMStrings.padRight(theFaction.name(), 10)
						+ ": ^H"
						+ theFaction
								.fetchRangeName(statData[Area.Stats.AVG_ALIGNMENT
										.ordinal()]) + "^N\n\r");
			if (theFaction != null)
				s.append("Med. "
						+ CMStrings.padRight(theFaction.name(), 10)
						+ ": ^H"
						+ theFaction
								.fetchRangeName(statData[Area.Stats.MED_ALIGNMENT
										.ordinal()]) + "^N\n\r");
			try {
				boolean blurbed = false;
				String flag = null;
				for (Enumeration<String> f = allBlurbFlags(); f
						.hasMoreElements();) {
	...
	public synchronized StringBuffer getAreaStats() {
		...
		StringBuffer s = (StringBuffer) Resources.getResource("HELP_"
				+ Name().toUpperCase());
		if (s != null)
			return s;
	...
	public void addMetroRoomnumber(String roomID) {
		...
		if ((roomID != null) && (roomID.length() > 0)
			...
			if (!CMath.bset(flags(), Area.FLAG_INSTANCE_CHILD))
				for (final Iterator<Area> a = getParentsReverseIterator(); a
						.hasNext();)
	...
	public void delMetroRoomnumber(String roomID) {
		if ((metroRoomIDSet != null) && (roomID != null)
				&& (roomID.length() > 0) && (metroRoomIDSet.contains(roomID))) {
	...
	public int numberOfProperIDedRooms() {
		...
		for (Enumeration e = getProperMap(); e.hasMoreElements();) {
			...
			if (R.roomID().length() > 0)
				if (R instanceof GridLocale)
					num += ((GridLocale) R).xGridSize()
							* ((GridLocale) R).yGridSize();
				else
					num++;
		}
		return num;
	...
	public Room getRandomMetroRoom() {
		 ...
		 * R=(Room
		 * )metroRooms.elementAt(CMLib.dice().roll(1,metroRooms.size(),-1));
	...
	public Enumeration<Room> getProperMap() {
		return new CompleteRoomEnumerator(new IteratorEnumeration<Room>(
				properRooms.values().iterator()));
	}
	...
	public Enumeration<Room> getMetroMap() {
		MultiEnumeration<Room> multiEnumerator = new MultiEnumeration<Room>(
				new IteratorEnumeration<Room>(properRooms.values().iterator()));
	...
	protected static final String[] STDAREACODES = { "CLASS", "CLIMATE",
			"DESCRIPTION", "TEXT", "THEME", "BLURBS", "PREJUDICE", "BUDGET",
			"DEVALRATE", "INVRESETRATE", "IGNOREMASK", "PRICEMASKS",
			"ATMOSPHERE" };
	...
	public void setStat(String code, String val) {
		...
		case 10:
			...
			break;
		case 11:
			setItemPricingAdjustments((val.trim().length() == 0) ? new String[0]
