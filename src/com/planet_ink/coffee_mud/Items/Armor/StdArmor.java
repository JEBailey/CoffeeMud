...
public class StdArmor extends StdContainer implements Armor {
	...
	protected String armorHealth() {
		if (usesRemaining() >= 100)
			...
			case RawMaterial.MATERIAL_GLASS:
				return name() + " has a few hairline cracks ("
						+ usesRemaining() + "%)";
			...
			case RawMaterial.MATERIAL_WOODEN:
				return name() + " has a few small splinters ("
						+ usesRemaining() + "%)";
			default:
			...
			case RawMaterial.MATERIAL_PAPER:
				return name() + " has a a few tears and rips ("
						+ usesRemaining() + "%)";
			...
			case RawMaterial.MATERIAL_PAPER:
			case RawMaterial.MATERIAL_CLOTH:
				return name() + " has numerous tears and rips ("
						+ usesRemaining() + "%)";
			case RawMaterial.MATERIAL_ROCK:
			case RawMaterial.MATERIAL_PRECIOUS:
			case RawMaterial.MATERIAL_SYNTHETIC:
			case RawMaterial.MATERIAL_GLASS:
				return name() + " has numerous streaking cracks ("
						+ usesRemaining() + "%)";
			case RawMaterial.MATERIAL_FLESH:
			case RawMaterial.MATERIAL_LEATHER:
			...
			case RawMaterial.MATERIAL_METAL:
			case RawMaterial.MATERIAL_MITHRIL:
				return name() + " is badly dented and cracked ("
						+ usesRemaining() + "%)";
			case RawMaterial.MATERIAL_WOODEN:
				return name() + " is badly cracked and splintered ("
						+ usesRemaining() + "%)";
			default:
			...
			case RawMaterial.MATERIAL_FLESH:
			case RawMaterial.MATERIAL_LEATHER:
				return name() + " is badly shredded and ripped ("
						+ usesRemaining() + "%)";
			case RawMaterial.MATERIAL_METAL:
			case RawMaterial.MATERIAL_MITHRIL:
	...
	public final static long strangeDeviations = Wearable.WORN_FLOATING_NEARBY
			| Wearable.WORN_MOUTH | Wearable.WORN_EYES | Wearable.WORN_EARS
			| Wearable.WORN_NECK;
	public final static long deviation20 = Wearable.WORN_TORSO
			| Wearable.WORN_LEGS | Wearable.WORN_WAIST | Wearable.WORN_ARMS
			| Wearable.WORN_HANDS | Wearable.WORN_FEET;

	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (!super.okMessage(myHost, msg))
			return false;
		if ((msg.amITarget(this)) && (phyStats().height() > 0)
				&& (msg.source().phyStats().height() > 0)
				&& (msg.targetMinor() == CMMsg.TYP_WEAR)) {
			int devianceAllowed = 200;
			if ((rawProperLocationBitmap() & deviation20) > 0)
				devianceAllowed = 20;
			else if ((rawProperLocationBitmap() & strangeDeviations) > 0) {
			...
			if ((devianceAllowed > 0)
					&& (!CMSecurity.isDisabled(CMSecurity.DisFlag.EQUIPSIZE))) {
				if (msg.source().phyStats().height() < (phyStats().height() - devianceAllowed)) {
					msg.source().tell(
							name() + " doesn't fit you -- it's too big.");
					return false;
				}
				if (msg.source().phyStats().height() > (phyStats().height() + devianceAllowed)) {
					msg.source().tell(
							name() + " doesn't fit you -- it's too small.");
					return false;
				}
			}
		}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(this))
				...
				&& (subjectToWearAndTear()) && (usesRemaining() < 100)
			...
			msg.source().tell(armorHealth());
				...
				&& (msg.amITarget(owner()))
				&& (CMLib.dice().rollPercentage() > ((phyStats().level() / 2)
						+ (10 * phyStats().ability()) + (CMLib.flags()
						.isABonusItems(this) ? 20 : 0)))
				&& (subjectToWearAndTear())
				&& (CMLib.dice().rollPercentage() > (((MOB) owner())
						.charStats().getStat(CharStats.STAT_DEXTERITY)))) {
			int weaponType = -1;
			if ((msg.targetMinor() == CMMsg.TYP_DAMAGE) && ((msg.value()) > 0)) {
			...
			if (weaponType >= 0) {
				...
				case RawMaterial.MATERIAL_PRECIOUS:
					...
					case Weapon.TYPE_LASERING:
						if ((CMLib.dice().rollPercentage() < 50)
								&& ((material & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_PRECIOUS))
							setUsesRemaining(usesRemaining()
									- CMLib.dice().roll(1, 50, 0));
						break;
					case Weapon.TYPE_SONICING:
			...
			if ((usesRemaining() < 10) && (oldUses != usesRemaining())
					&& (owner() != null) && (owner() instanceof MOB)
					&& (usesRemaining() > 0))
				((MOB) owner()).tell(name() + " is nearly destroyed! ("
						+ usesRemaining() + "%)");
			else if ((usesRemaining() <= 0) && (owner() != null)
					&& (owner() instanceof MOB)) {
				MOB owner = (MOB) owner();
				setUsesRemaining(100);
				msg.addTrailerMsg(CMClass.getMsg(((MOB) owner()), null, null,
						CMMsg.MSG_OK_VISUAL, "^I" + name()
								+ " is destroyed!!^?", CMMsg.NO_EFFECT, null,
						CMMsg.MSG_OK_VISUAL, "^I" + name()
								+ " being worn by <S-NAME> is destroyed!^?"));
				unWear();
				destroy();
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((!amWearingAt(Wearable.IN_INVENTORY))
			...
			affectableStats.setArmor(affectableStats.armor()
					- phyStats().armor());
			if (phyStats().armor() != 0) {
				final int ability = super.wornLogicalAnd ? (phyStats()
						.ability() * CMath.numberOfSetBits(super.myWornCode))
						: phyStats().ability();
				if (amWearingAt(Wearable.WORN_TORSO))
					affectableStats.setArmor(affectableStats.armor()
							- (ability * 5));
				else if ((amWearingAt(Wearable.WORN_HEAD))
				...
				if (affectableStats.getStat(CharStats.STAT_SAVE_ELECTRIC) > (Short.MIN_VALUE / 2))
					affectableStats
							.setStat(
									...
									affectableStats
											.getStat(CharStats.STAT_SAVE_ELECTRIC) - 10);
				break;
			case RawMaterial.MATERIAL_LEATHER:
				if (affectableStats.getStat(CharStats.STAT_SAVE_ACID) < (Short.MAX_VALUE / 2))
					affectableStats
							.setStat(CharStats.STAT_SAVE_ACID, affectableStats
									.getStat(CharStats.STAT_SAVE_ACID) + 10);
				break;
			case RawMaterial.MATERIAL_MITHRIL:
				if (affectableStats.getStat(CharStats.STAT_SAVE_MAGIC) < (Short.MAX_VALUE / 2))
					affectableStats
							.setStat(CharStats.STAT_SAVE_MAGIC, affectableStats
									.getStat(CharStats.STAT_SAVE_MAGIC) + 10);
				break;
			case RawMaterial.MATERIAL_CLOTH:
			case RawMaterial.MATERIAL_PAPER:
				if (affectableStats.getStat(CharStats.STAT_SAVE_FIRE) > (Short.MIN_VALUE / 2))
					affectableStats
							.setStat(CharStats.STAT_SAVE_FIRE, affectableStats
									.getStat(CharStats.STAT_SAVE_FIRE) - 10);
				break;
			case RawMaterial.MATERIAL_GLASS:
			case RawMaterial.MATERIAL_ROCK:
			case RawMaterial.MATERIAL_PRECIOUS:
				...
				if (affectableStats.getStat(CharStats.STAT_SAVE_PARALYSIS) < (Short.MAX_VALUE / 2))
					affectableStats
							.setStat(
									...
									affectableStats
											.getStat(CharStats.STAT_SAVE_PARALYSIS) + 10);
				break;
			}
	}
	...
	public int value() {
		if (usesRemaining() < 1000)
			return (int) Math.round(CMath.mul(super.value(),
					CMath.div(usesRemaining(), 100)));
		return super.value();
	}
