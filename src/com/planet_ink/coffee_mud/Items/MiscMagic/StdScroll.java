...
public class StdScroll extends StdItem implements MiscMagic, Scroll {
	...
	public String secretIdentity() {
		return StdScroll.makeSecretIdentity("scroll", super.secretIdentity(),
				" Charges: " + usesRemaining(), getSpells(this));
	}

	public static String makeSecretIdentity(String thang, String id,
			String more, List<Ability> V) {
				...
				if (me.usesRemaining() <= 0)
					...
					if (Spells.size() == 0)
						...
						if (spellName.length() > 0) {
							...
							thisOne = (Ability) CMLib
									.english()
									.fetchEnvironmental(Spells, spellName, true);
							if (thisOne == null)
								thisOne = (Ability) CMLib.english()
										.fetchEnvironmental(Spells, spellName,
												false);
							while ((thisOne == null)
									&& (spellName.length() > 0)) {
								...
								if (t < 0)
									...
									params.insertElementAt(
											spellName.substring(t).trim(), 0);
									spellName = spellName.substring(0, t);
									thisOne = (Ability) CMLib.english()
											.fetchEnvironmental(Spells,
													spellName, true);
									if (thisOne == null)
										thisOne = (Ability) CMLib.english()
												.fetchEnvironmental(Spells,
						...
						if ((thisOne != null)
								&& (me.useTheScroll(thisOne, mob))) {
							thisOne = (Ability) thisOne.copyOf();
							int level = phyStats().level();
							int lowest = CMLib.ableMapper()
									.lowestQualifyingLevel(thisOne.ID());
							if (level < lowest)
								level = lowest;
							...
							StringBuffer theNews = new StringBuffer(
									"The scroll contains the following spells:\n\r");
							Spells = me.getSpells();
							for (int u = 0; u < Spells.size(); u++) {
								Ability A = Spells.get(u);
								theNews.append("Level "
										+ CMStrings
												.padRight(
														""
																+ CMLib.ableMapper()
	...
	public boolean isReadableScrollBy(String name) {
		return (readableScrollBy != null)
				&& (readableScrollBy.equalsIgnoreCase(name));
	}
