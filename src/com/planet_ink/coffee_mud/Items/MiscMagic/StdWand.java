...
public class StdWand extends StdItem implements Wand {
	...
	public String secretIdentity() {
		...
		if (A != null)
			id = "'A wand of " + A.name() + "' Charges: " + usesRemaining()
					+ "\n\r" + id;
		return id + "\n\rSay the magic word :`" + secretWord
				+ "` to the target.";
	}
	...
	public static void waveIfAble(MOB mob, Physical afftarget, String message,
			Wand me) {
		if ((mob.isMine(me)) && (message != null)
				&& (!me.amWearingAt(Wearable.IN_INVENTORY))) {
			...
			if (x >= 0) {
				...
				if ((wandUse == null)
						|| (!wandUse.proficiencyCheck(null, 0, false)))
					mob.tell(me.name()
