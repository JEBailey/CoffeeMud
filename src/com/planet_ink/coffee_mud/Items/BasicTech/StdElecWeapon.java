...
public class StdElecWeapon extends StdElecItem implements Weapon, Electronics {
	...
	protected enum ModeType {
		STUN, NORMAL, KILL, DISINTEGRATE, MAIM, DISRUPT, LASER, SONIC
	}
	...
	public void setMiscText(String text) {
		...
		if (CMath.isInteger(text)) {
			...
			ModeType t = (ModeType) CMath.s_valueOf(ModeType.class, text
					.toUpperCase().trim());
	...
	public String secretIdentity() {
		return super.secretIdentity() + "\n\rAttack: "
				+ phyStats().attackAdjustment() + ", Damage: "
				+ phyStats().damage();
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		} else if ((owner() instanceof MOB) && msg.amISource((MOB) owner())
			...
			case CMMsg.TYP_WEAPONATTACK:
				if ((msg.tool() == this)
					...
					msg.addTrailerMsg(CMClass.getMsg(msg.source(), this,
							CMMsg.MSG_EXPIRE | CMMsg.MASK_ALWAYS, null));
				break;
				...
				if (msg.tool() == this) {
					...
					case STUN: {
						if (msg.value() > 0) {
							...
							if ((A != null)
								...
								msg.source()
										.location()
										.show((MOB) msg.target(), null,
												CMMsg.MSG_OK_VISUAL,
												"<S-NAME> go(es) unconscious!");
								((MOB) msg.target()).basePhyStats()
										.setDisposition(
												((MOB) msg.target())
														.basePhyStats()
														.disposition()
														| PhyStats.IS_SLEEPING);
								((MOB) msg.target()).phyStats().setDisposition(
										((MOB) msg.target()).phyStats()
												.disposition()
												| PhyStats.IS_SLEEPING);
							}
					...
					case DISINTEGRATE: {
						...
						if ((msg.value() > (basePhyStats().damage() * 1.5))
								&& ((!(targ instanceof MOB)) || (msg.value() >= ((MOB) targ)
							...
							if (R != null) {
								if (targ instanceof MOB) {
									if ((!((MOB) targ).amDead())
											|| (((MOB) targ).curState()
													.getHitPoints() > 0))
										CMLib.combat().postDeath(msg.source(),
												(MOB) targ, null);
									((MOB) targ).location().show(mob, targ,
											CMMsg.MSG_OK_ACTION,
											"<T-NAME> disintegrate(s)!");
									if (((MOB) targ).amDead()) {
										...
										&& ((!(targ instanceof DeadBody) || (!((DeadBody) targ)
												.playerCorpse())))) {
									((MOB) targ)
											.location()
											.show(mob, targ,
													CMMsg.MSG_OK_ACTION,
													"^S<S-NAME> fire(s) <O-NAME> at <T-NAME> and it disintegrates!^?");
									((Item) targ).destroy();
								}
								R.recoverRoomStats();
							}
							...
							if (R != null) {
								if (targ instanceof MOB) {
									...
									((MOB) targ)
											...
											.show(mob, targ,
													CMMsg.MSG_OK_ACTION,
													"^S<S-NAME> fire(s) <O-NAME> at <T-NAME> and it explodes!^?");
									((Item) targ).destroy();
								}
								R.recoverRoomStats();
							}
	...
	public boolean okMessage(Environmental myHost, CMMsg msg) {
		if (!super.okMessage(myHost, msg))
			return false;
		if ((owner() instanceof MOB) && msg.amISource((MOB) owner())
				&& (!amWearingAt(Item.IN_INVENTORY))) {
			MOB mob = (MOB) owner();
			switch (msg.targetMinor()) {
			case CMMsg.TYP_ACTIVATE:
			...
			case CMMsg.TYP_WEAPONATTACK:
				if (msg.tool() == this) {
					if ((powerRemaining() <= 0) || (!activated)) {
						...
						CMMsg msg2 = CMClass.getMsg(mob, this, null,
								CMMsg.MSG_OK_VISUAL, msgStr);
						if ((mob.location() != null)
								&& (mob.location().okMessage(myHost, msg2)))
			...
			case CMMsg.TYP_DAMAGE: // remember 50% miss rate
				if (msg.tool() == this) {
					...
					long powerConsumed = Math.round(phyStats().damage()
							* m.getEfficiencyPct());
					if (powerRemaining() >= powerConsumed) {
						setPowerRemaining(powerRemaining() - powerConsumed);
						if (msg.value() > 0)
							msg.setValue((int) Math.round(successFactor
									* msg.value()));
					} else {
						setPowerRemaining(0);
					}
