 ...
 */
public class GenSpaceShip extends StdPortal implements Electronics, SpaceShip,
		PrivateProperty {
	public String ID() {
		return "GenSpaceShip";
	}
	...
	public void setShipArea(String xml) {
		try {
			...
			if (area instanceof SpaceShip) {
				area.setSavable(false);
				((SpaceShip) area).setKnownSource(this);
				for (Enumeration<Room> r = area.getCompleteMap(); r
						.hasMoreElements();)
	...
	public CMObject copyOf() {
		...
		s.destroyed = false;
		String xml = CMLib.coffeeMaker()
				.getAreaObjectXML(getShipArea(), null, null, null, true)
				.toString();
		s.setShipArea(xml);
		/*
		 * if(s.getShipArea().Name().startsWith("UNNAMED_")) { String
		 * num=Double.toString(Math.random()); String oldName=s.Name(); String
		 * oldDisplay=s.displayText();
		 * s.renameSpaceShip("UNNAMED_"+num.substring(num.indexOf('.')+1));
		 * s.setName(oldName); s.setDisplayText(oldDisplay); }
		 */
		return s;
	}
	...
	protected Room getDestinationRoom() {
		...
		if (V.size() > 0)
			R = getShipArea()
					.getRoom(V.get(CMLib.dice().roll(1, V.size(), -1)));
		return R;
	}
	...
	public long getMass() {
		return basePhyStats().weight()
				+ ((area instanceof SpaceShip) ? ((SpaceShip) area).getMass()
						: 1000);
	}
	...
	public Manufacturer getFinalManufacturer() {
		if (cachedManufact == null) {
			cachedManufact = CMLib.tech().getManufacturerOf(this,
					getManufacturerName().toUpperCase().trim());
			if (cachedManufact == null)
	...
	public SpaceObject knownSource() {
		return (area instanceof SpaceObject) ? ((SpaceObject) area)
				.knownSource() : null;
	}
	...
	@Override
	public long radius() {
		return (area instanceof SpaceObject) ? ((SpaceObject) area).radius()
				: 50;
	}
	...
	public double getOMLCoeff() {
		return (area instanceof SpaceShip) ? ((SpaceShip) area).getOMLCoeff()
				: SpaceObject.ATMOSPHERIC_DRAG_STREAMLINE
						+ ((SpaceObject.ATMOSPHERIC_DRAG_BRICK - SpaceObject.ATMOSPHERIC_DRAG_STREAMLINE) / 2.0);
	}
	...
	public void renameSpaceShip(String newName) {
		...
		if (area instanceof SpaceShip) {
			...
			area.addBlurbFlag("REGISTRY Registry#"
					+ registryNum.substring(registryNum.indexOf('.') + 1));
			setReadableText(oldEntry.roomID());
			setShipArea(CMLib.coffeeMaker()
					.getAreaObjectXML(area, null, null, null, true).toString());
		}
		for (String word : new String[] { "NAME", "NEWNAME", "SHIPNAME", "SHIP" }) {
			for (String rubs : new String[] { "<>", "[]", "{}", "()" }) {
				if (Name().indexOf(rubs.charAt(0) + word + rubs.charAt(1)) >= 0)
					setName(CMStrings.replaceAll(Name(), rubs.charAt(0) + word
							+ rubs.charAt(1), newName));
			}
			for (String rubs : new String[] { "<>", "[]", "{}", "()" }) {
				if (displayText().indexOf(
						rubs.charAt(0) + word + rubs.charAt(1)) >= 0)
					setDisplayText(CMStrings.replaceAll(displayText(),
							rubs.charAt(0) + word + rubs.charAt(1), newName));
			}
		}
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(this)) {
			if ((msg.targetMinor() == CMMsg.TYP_OPEN)
				...
				msg.setOthersMessage(CMStrings.replaceAll(msg.othersMessage(),
						"<T-NAME>", "a hatch on <T-NAME>"));
				msg.setOthersMessage(CMStrings.replaceAll(msg.othersMessage(),
						"<T-NAMESELF>", "a hatch on <T-NAMESELF>"));
				msg.setSourceMessage(CMStrings.replaceAll(msg.othersMessage(),
						"<T-NAME>", "a hatch on <T-NAME>"));
				msg.setSourceMessage(CMStrings.replaceAll(msg.othersMessage(),
						"<T-NAMESELF>", "a hatch on <T-NAMESELF>"));
				msg.setTargetMessage(CMStrings.replaceAll(msg.othersMessage(),
						"<T-NAME>", "a hatch on <T-NAME>"));
				msg.setTargetMessage(CMStrings.replaceAll(msg.othersMessage(),
						"<T-NAMESELF>", "a hatch on <T-NAMESELF>"));
			}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(this)) {
			...
			else if ((msg.targetMinor() == CMMsg.TYP_ACTIVATE)
				...
				if (command != null) {
					...
					if (parms != null) {
						if (command == Technical.TechCommand.ACCELLLERATION) {
							...
							// long
							// specificImpulse=((Long)parms[2]).longValue();
							switch (dir) {
							case STARBOARD:
								facing[0] -= amount;
								break;
							...
							case AFT: {
								...
								// force equation in air= A=((thrust / (m *
								// inertial dampener <= 1 ))-1)*(1- OML))
								// force equation in space= A=(thrust / (m *
								// inertial dampener <= 1 )

								// remember you are moving in a direction, but
								// facing a different one.
								/*
								 * int mass=getMass(); double
								 * ke1=0.5*(velocity*velocity); // *mass double
								 * ke2=0.5*(amount*amount); // *mass
								 * direction()[0]+=Math.sin(ke1/ke2);
								 */
								// TODO: how does the dir affect velocity and
								// direction given thrust and facing?
								break;
							}
							}
		...
		} else if ((msg.targetMinor() == CMMsg.TYP_SELL)
				&& (msg.tool() == this) && (msg.target() instanceof ShopKeeper)) {
			setOwnerName("");
			recoverPhyStats();
				...
				&& ((msg.source().Name().equals(getOwnerName()))
						|| (msg.source().getLiegeID().equals(getOwnerName()) && msg
								.source().isMarriedToLiege()) || (CMLib.clans()
						.checkClanPrivilege(msg.source(), getOwnerName(),
								Clan.Function.PROPERTY_OWNER)))
	...
	protected LocationRoom findNearestDocks(Room R) {
		...
		if (R != null) {
			...
			if (docks.size() == 0)
				for (Enumeration<Room> r = R.getArea().getMetroMap(); r
						.hasMoreElements();) {
	...
	protected void transferOwnership(final MOB buyer) {
		if (CMLib.clans().checkClanPrivilege(buyer, getOwnerName(),
				Clan.Function.PROPERTY_OWNER)) {
			Pair<Clan, Integer> targetClan = CMLib.clans().findPrivilegedClan(
					buyer, Clan.Function.PROPERTY_OWNER);
			if (targetClan != null)
		...
		// String
		// randNum=CMStrings.limit(registryNum.substring(registryNum.indexOf('.')+1),
		// 4);
		...
		if (session != null) {
			...
			namer[0] = new InputCallback(InputCallback.Type.PROMPT) {
				...
				public void callBack() {
					if ((this.input.trim().length() == 0)
							|| (!CMLib.login()
									.isOkName(this.input.trim(), true))
							|| (CMLib.tech().getMakeRegisteredKeys()
									.contains(this.input.trim()))) {
						session.println("^ZThat is not a permitted name.^N");
						session.prompt(namer[0].reset());
						return;
					}
					me.renameSpaceShip(this.input.trim());
					buyer.tell(name() + " is now signed over to "
							+ getOwnerName() + ".");
					LocationRoom finalR = findNearestDocks(R);
					if (finalR == null) {
						Log.errOut("Could not dock ship in area "
								+ R.getArea().Name()
								+ " due to lack of spaceport.");
						buyer.tell("Nowhere was found to dock your ship.  Please contact the administrators!.");
					} else {
						me.dockHere(finalR);
						buyer.tell("You'll find your ship docked at '"
								+ finalR.displayText(buyer) + "'.");
					}
					if ((buyer.playerStats() != null)
							&& (!buyer.playerStats().getExtItems()
									.isContent(me)))
			...
			buyer.tell(name() + " is now signed over to " + getOwnerName()
					+ ".");
			if ((buyer.playerStats() != null)
					&& (!buyer.playerStats().getExtItems().isContent(this)))
				buyer.playerStats().getExtItems().addItem(this);
			LocationRoom finalR = findNearestDocks(R);
			if (finalR == null)
				Log.errOut("Could not dock ship in area " + R.getArea().Name()
						+ " due to lack of spaceport.");
			else
				dockHere(finalR);
		}
	...
	private final static String[] MYCODES = { "HASLOCK", "HASLID", "CAPACITY",
			"CONTAINTYPES", "RIDEBASIS", "MOBSHELD", "FUELTYPE", "POWERCAP",
			"ACTIVATED", "POWERREM", "MANUFACTURER", "AREA", "COORDS",
			"RADIUS", "ROLL", "DIRECTION", "SPEED", "FACING", "OWNER", "PRICE" };
	...
	public String getStat(String code) {
		...
		case 10:
			return (area == null) ? "" : CMLib.coffeeMaker()
					.getAreaXML(area, null, null, null, true).toString();
		case 11:
	...
	public void setStat(String code, String val) {
		if (CMLib.coffeeMaker().getGenItemCodeNum(code) >= 0)
			...
			case 13:
				...
				break;
			case 14:
				setDirection(CMParms.toDoubleArray(CMParms.parseCommas(val,
						true)));
				break;
			case 15:
				setSpeed(CMath.s_long(val));
				break;
			case 16:
