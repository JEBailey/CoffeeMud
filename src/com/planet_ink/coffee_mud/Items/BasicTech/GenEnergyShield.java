...
public class GenEnergyShield extends GenPersonalShield {
	...
	protected String fieldOnStr(MOB viewerM) {
		return (owner() instanceof MOB) ? "An energy field surrounds <O-NAME>."
				: "An energy field surrounds <T-NAME>.";
	}

	@Override
	protected String fieldDeadStr(MOB viewerM) {
		return (owner() instanceof MOB) ? "The energy field around <O-NAME> flickers and dies out."
				...
				if (s.indexOf("<DAMAGE> <T-HIM-HER>") > 0)
					mob.location().show(
							...
							CMStrings.replaceAll(s, "<DAMAGE>",
									"it is absorbed by the shield around"));
					...
					mob.location().show(
							...
							CMStrings.replaceAll(s, "<DAMAGES>",
									"is absorbed by the shield around"));
				else
					mob.location()
							.show(mob, msg.source(), msg.tool(),
									CMMsg.MSG_OK_VISUAL,
									"The field around <S-NAME> absorbs the <O-NAMENOART> damage.");
			} else
				mob.location()
						.show(mob, msg.source(), msg.tool(),
								CMMsg.MSG_OK_VISUAL,
