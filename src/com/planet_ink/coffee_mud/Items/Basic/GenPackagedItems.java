...
public class GenPackagedItems extends GenItem implements PackagedItems {
	...
	public String name() {
		return "a package of " + numberOfItemsInPackage() + " " + Name().trim()
				+ "(s)";
	}
	...
	public boolean packageMe(Item I, int number) {
		if ((I == null) || (!CMLib.utensils().disInvokeEffects(I))
				|| (I.amDestroyed()))
			return false;
		...
		itemstr.append(CMLib.xml().convertXMLtoTag("PICLASS",
				CMClass.classID(I)));
		itemstr.append(CMLib.xml().convertXMLtoTag("PIDATA",
				CMLib.coffeeMaker().getPropertiesStr(I, true)));
		itemstr.append("</PAKITEM>");
