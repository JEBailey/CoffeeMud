...
public class StdSmokable extends StdContainer implements Light {
		...
		case CMMsg.TYP_WEAR:
			...
			if (room != null) {
				if (((LightSource.inTheRain(room) && (goesOutInTheRain())) || (LightSource
						...
						&& (durationTicks > 0) && (mob.isMine(this))) {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((tickID == Tickable.TICKID_LIGHT_FLICKERS) && (isLit())
				&& (tickStatus == Tickable.STATUS_NOT) && (owner() != null)) {
			...
			if (((--durationTicks) > 0) && (!destroyed)) {
				if (((durationTicks % puffTicks) == 0)
					...
					if ((mob.location() != null)
						...
						mob.location().show(mob, this, this, CMMsg.MSG_HANDS,
								"<S-NAME> puff(s) on <T-NAME>.");
			...
			case CMMsg.TYP_WEAR:
				if (durationTicks > 0) {
					...
					CMLib.threads().startTickDown(this,
							Tickable.TICKID_LIGHT_FLICKERS, 1);
					recoverPhyStats();
					room.recoverRoomStats();
				}
				break;
