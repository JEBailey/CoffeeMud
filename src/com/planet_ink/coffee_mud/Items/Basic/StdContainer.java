...
public class StdContainer extends StdItem implements Container {
			...
			case CMMsg.TYP_PUT:
				if (msg.tool() instanceof Item) {
					if (!CMLib.flags().isDroppable((Item) msg.tool())) {
						mob.tell("You can't seem to let go of "
								+ msg.tool().name() + ".");
						return false;
					}
					...
					if (hasALid() && (!isOpen())) {
						...
						if (!canContain(newitem)) {
							mob.tell("You can't put " + newitem.name() + " in "
									+ name() + ".");
							return false;
							...
							mob.tell(newitem.name() + " won't fit in " + name()
									+ ".");
							return false;
						} else if ((recursiveWeight() + newitem.phyStats()
								.weight()) > capacity) {
							if (getContents().size() == 0)
								mob.tell(name() + " is too small.");
							else if ((newitem instanceof Software)
									&& (this instanceof Electronics.Computer))
						...
						if ((!msg.source().isMine(this))
								&& (msg.source().isMine(newitem)))
							if (!CMLib.commands().postDrop(msg.source(),
									newitem, true, true, true))
								return false;
						return true;
					}
				}
				...
				if ((msg.tool() != null) && (msg.tool() instanceof Item)) {
					...
					if (newitem.container() == this) {
						if ((!(CMLib.flags().canBeSeenBy(newitem, mob) || (newitem instanceof Light)))
							...
							mob.tell(mob, newitem, this,
									"You can't see <T-NAME> in <O-NAME>.");
							return false;
						} else if (hasALid() && (!isOpen())) {
							mob.tell(name() + " is closed.");
							return false;
						} else if ((mob.phyStats().level() < newitem.phyStats()
								.level() - (10 + (mob.phyStats().level() / 5)))
								&& (!(mob instanceof ShopKeeper))
								&& (!mob.charStats().getMyRace().leveless())
								&& (!mob.charStats().getCurrentClass()
										.leveless())) {
							mob.tell(newitem.name()
									+ " is too powerful to endure possessing it.");
							return false;
						} else if ((newitem.recursiveWeight() > (mob.maxCarry() - mob
								.phyStats().weight())) && (!mob.isMine(this))) {
							mob.tell(newitem.name() + " is too heavy.");
							return false;
						} else if ((newitem.numberOfItems() > (mob.maxItems() - mob
								.numItems())) && (!mob.isMine(this))) {
					...
					mob.tell(mob, newitem, this,
							"You can't see <T-NAME> in <O-NAME>.");
					return false;
				} else if ((recursiveWeight() > (mob.maxCarry() - mob
						.phyStats().weight())) && (!mob.isMine(this))) {
					mob.tell(name() + " is too heavy.");
					return false;
				} else if (((numberOfItems() > mob.maxItems() - mob.numItems()))
						&& (!mob.isMine(this))) {
					mob.tell("You can't carry that many items.");
					return false;
				}
					...
					if ((!isLocked) && (msg.targetMinor() == CMMsg.TYP_UNLOCK)) {
						...
						for (int i = 0; i < mob.numItems(); i++) {
							...
							if ((item != null)
									...
									&& ((DoorKey) item).getKey().equals(
											keyName())
									&& ((item.container() == null) || ((item
											.container().container() == null) && ((item
											.container().containTypes() & Container.CONTAIN_KEYS) > 0)))
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if (msg.amITarget(this)) {
			...
			default:
				...
				&& (msg.source() != null) && (msg.source().isMine(this))) {
			setContainer(null);
			Room R = CMLib.map().roomLocation(msg.target());
			if (R != null) {
				CMLib.utensils().recursiveDropMOB(msg.source(), R, this,
						this instanceof DeadBody);
		...
		for (int i = 0; i < Container.CONTAIN_DESCS.length; i++)
			if (CMath.isSet((int) containType, i))
				...
				case CONTAIN_CLOTHES:
					if ((E instanceof Armor)
							&& (((Armor) E).fitsOn(Wearable.WORN_ABOUT_BODY)
									...
									|| ((Armor) E).fitsOn(Wearable.WORN_TORSO) || ((Armor) E)
										.fitsOn(Wearable.WORN_WAIST)))
						return true;
					break;
				...
				case CONTAIN_READABLES:
					if ((E instanceof Item) && (((Item) E).isReadable()))
						return true;
					break;
	...
	public void setLidsNLocks(boolean newHasALid, boolean newIsOpen,
			boolean newHasALock, boolean newIsLocked) {
		hasALid = newHasALid;
		isOpen = newIsOpen;
	...
	public int recursiveWeight() {
		...
		if (owner() instanceof MOB) {
			...
			for (int i = 0; i < M.numItems(); i++) {
				...
				if ((thisItem != null) && (thisItem != this)
						&& (thisItem.ultimateContainer(this) == this))
