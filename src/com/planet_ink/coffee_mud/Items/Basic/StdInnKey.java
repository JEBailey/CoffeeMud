...
public class StdInnKey extends StdKey implements InnKey {
	...
	public void hangOnRack(ShopKeeper SK) {
		if (myShopkeeper == null) {
			...
			setDescription("The key goes to room "
					+ (y + 1)
					+ ", but will expire soon, so you better use it quickly! Give the key to your innkeeper, "
					+ SK.name() + ", when you leave.");
			setMiscText("INN" + (y + 1));
		}
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (((msg.targetMinor() == CMMsg.TYP_GIVE) || (msg.targetMinor() == CMMsg.TYP_SELL))
				&& (myShopkeeper != null)
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (((msg.targetMinor() == CMMsg.TYP_GIVE) || (msg.targetMinor() == CMMsg.TYP_SELL))
			...
			return false;
				...
				&& (myShopkeeper != null) && (msg.tool() == myShopkeeper)
				&& (msg.target() == this))
			CMLib.threads().startTickDown(this,
				...
				&& (((Room) msg.target()).getArea() != msg.source().location()
						.getArea()) && (super.miscText != null)) {
			Area shopArea = CMLib.map().areaLocation(myShopkeeper);
			if ((shopArea == ((Room) msg.target()).getArea())
					|| (shopArea == null)) {
