...
public class StdMap extends StdItem implements
		com.planet_ink.coffee_mud.Items.interfaces.RoomMap {
	public String ID() {
		return "StdMap";
	}
			...
			for (int y = 0; y < grid[0].length; y++) {
				...
				if (room != null) {
					if (((x > 0) && (grid[x - 1][y] != null))
							...
							|| ((y < (grid[0].length - 1))
									&& (x < (grid.length - 1)) && (grid[x + 1][y + 1] != null)))
						somethingsLeft++;
					else {
						room.x = 0;
						room.y = 0;
	...
	public void clearTheSkys(Hashtable mapRooms) {
		for (Enumeration e = mapRooms.keys(); e.hasMoreElements();) {
			...
			if ((DOWN != null) && (DOWN.roomID().length() == 0)
	...
	public StringBuffer[][] finishMapMaking(int width) {
		...
		if (mapRooms.size() > 0) {
			...
			if ((grid.length == 0) || (grid[0].length == 0))
				return map;
			int numXSquares = (int) Math.round(Math.floor(CMath.div(width - 6,
					8)));
			int numYSquares = ((numXSquares / 2) + 1);
			int xsize = grid.length / numXSquares;
			int ysize = grid[0].length / numYSquares;
			if ((xsize < 0) || (ysize < 0)) {
				Log.errOut("StdMap", "Error finishing " + xsize + "/" + ysize
						+ "/" + width);
				return map;
			}
			...
			for (int y = 0; y < grid[0].length; y++) {
				...
				for (int x = 0; x < grid.length; x++) {
					...
					if (room == null) {
						...
						String paddedName = CMStrings.padRight(room.r
								.displayText().trim(), 30);
						line1 += "---"
								+ dirChar(Directions.NORTH, grid, x, y, '-')
								+ "----";
						line2 += "|" + paddedName.substring(0, 6) + "|";
						line3 += dirChar(Directions.WEST, grid, x, y, '|')
								+ paddedName.substring(6, 12)
								+ dirChar(Directions.EAST, grid, x, y, '|');
						line4 += "|" + paddedName.substring(12, 18) + "|";
						line5 += "|" + paddedName.substring(18, 24) + "|";
						line6 += "---"
								+ dirChar(Directions.SOUTH, grid, x, y, '-')
								+ "----";
					}
				}
				if (xcoord >= 0) {
	...
	public StringBuffer[][] getMyMappedRoom(int width) {
		...
		/*
		 * if(oldLevel!=phyStats().level()) { myMap=null;
		 * oldLevel=phyStats().level(); }
		 * 
		 * if(myMap!=null) return myMap;
		 * 
		 * Object
		 * o=Resources.getResource("map"+phyStats().level()+":"+getMapArea());
		...
		Resources.submitResource("map" + phyStats().level() + ":"
				+ getMapArea(), myMap);
		return myMap;
	}
	...
	public boolean isEmptyCluster(Hashtable processed, int x, int y) {
		for (Enumeration e = processed.elements(); e.hasMoreElements();) {
			MapRoom room = (MapRoom) e.nextElement();
			if ((((room.x > x - CLUSTERSIZE) && (room.x < x + CLUSTERSIZE)) && ((room.y > y
					- CLUSTERSIZE) && (room.y < y + CLUSTERSIZE)))
					|| ((room.x == x) && (room.y == y)))
				return false;
		}
		return true;
	...
	public boolean okToPlace(MapRoom room) {
		...
		if ((phyStats().level() < 1)
				&& ((room.r.domainType() & Room.INDOORS) == Room.INDOORS))
			return false;
		...
		for (int d = 0; d < Directions.NUM_DIRECTIONS() - 1; d++) {
			...
			if (E != null) {
				if (phyStats().level() < 2) {
					if ((!CMLib.flags().isHidden(E))
							&& (!CMLib.flags().isInvisible(E))
							&& (!E.defaultsLocked()))
						ok = true;
				} else
					ok = true;
	...
	public boolean okToPlace(MapRoom room, Exit exit) {
		...
		if (exit == null)
			return false;
		if (phyStats().level() < 2) {
			if ((CMLib.flags().isHidden(exit) || CMLib.flags()
					.isInvisible(exit)))
				return false;
		...
		if (!msg.amITarget(this))
			...
			case CMMsg.TYP_READ:
				if (CMLib.flags().canBeSeenBy(this, mob)) {
					...
					if ((isReadable()) && (map != null) && (map.length > 0)
							&& (map[0].length > 0) && (!mob.isMonster())) {
						int x = 0;
						int y = 0;
						String sec = "A0";
						if ((msg.targetMessage().length() > 0)
								&& (!msg.targetMessage().startsWith("<S-NAME>"))) {
							sec = "";
							boolean trans = false;
						...
						if ((map.length > 1) || (map[0].length > 1)) {
							...
							if (map.length > 25)
								for (int l = 0; l < map.length / 26; l++)
									letsec += 'Z';
							letsec = letsec.substring(0, letsec.length() - 1)
									+ ((char) ((('A') + map.length % 26) - 1));
							if (name().length() > 0)
								mob.session()
										.rawPrintln(
												"("
														+ sec
														+ ") Use 'READ SEC MAPNAME' to read sections A0 through "
														+ letsec
														+ (map[0].length - 1)
														+ " (A-" + letsec
														+ ", 0-"
	...
	public void placeRoom(MapRoom room, Hashtable areaMap, int favoredX,
			int favoredY, Hashtable processed, boolean doNotDefer,
			boolean passTwo, int depth) {
		if (room == null)
			return;
		if (depth > 500)
			return;
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if ((nextRoom != null)
				...
				placeRoom(nextRoom, areaMap, newFavoredX, newFavoredY,
						processed, false, passTwo, depth + 1);
			}
		}
	}
}
