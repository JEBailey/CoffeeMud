...
public class StdLawBook extends StdItem {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (msg.amITarget(this))
			...
			case CMMsg.TYP_WRITE:
				msg.source().tell(
						"You are not allowed to write on " + name()
								+ ". Try reading it.");
				return false;
			}
		...
		if (msg.amITarget(this))
			...
			case CMMsg.TYP_READ:
				if (!CMLib.flags().canBeSeenBy(this, mob))
					...
					if (B == null) {
						msg.source().tell(
								"The pages appear blank, and damaged.");
						return;
					}
					...
					boolean allowedToModify = (CMSecurity.isAllowed(mob,
							mob.location(), CMSecurity.SecFlag.ABOVELAW));
					if (A.getMetroMap().hasMoreElements())
						allowedToModify = (CMSecurity.isAllowed(mob,
								(A.getMetroMap().nextElement()),
								CMSecurity.SecFlag.ABOVELAW));
					String rulingClan = B.rulingOrganization();
					if ((!allowedToModify) && (rulingClan.length() > 0)
							&& (mob.getClanRole(rulingClan) != null)) {
						Clan C = CMLib.clans().getClan(rulingClan);
						if ((C != null)
								&& (C.getAuthority(
										mob.getClanRole(rulingClan).second
												.intValue(),
										Clan.Function.ORDER_CONQUERED) == Clan.Authority.CAN_DO))
					...
					try {
						if (which < 1) {
							...
							case 1:
								if (mob.session() != null)
									mob.tell(CMStrings.replaceAll(
											getFromTOC("P1"
													+ (theLaw
															.hasModifiableLaws() ? "MOD"
															: "")
													+ (theLaw
															.hasModifiableNames() ? "NAM"
															: "")), "<AREA>", A
													.name()));
								break;
							case 2:
			...
			case CMMsg.TYP_WRITE:
				try {
					...
					if (A2 == null) {
						msg.source()
								.tell("The pages appear blank, and too damaged to write on.");
						return;
					}
					return;
				} catch (Exception e) {
	...
	public String getFromTOC(String tag) {
		...
		try {
			if ((lawProps == null) || (lawProps.isEmpty())) {
				...
				lawProps.load(new ByteArrayInputStream(new CMFile(
						"resources/lawtoc.ini", null).raw()));
	...
	public void changeTheLaw(Environmental A, LegalBehavior B, MOB mob,
			Law theLaw, String tag, String newValue) {
	...
	public String shortLawDesc(String[] bits) {
		if ((bits == null) || (bits.length < Law.BIT_NUMBITS))
			return "Not illegal.";
		String flags = bits[Law.BIT_CRIMEFLAGS] + " "
				+ bits[Law.BIT_CRIMELOCS].trim();
	...
	public String shortLawHeader() {
		return CMStrings.padRight("Crime", 19) + " "
				+ CMStrings.padRight("Flags", 24) + " " + "Sentence";
	}
	...
	public final static String[][] locflags = {
			...
			{ "A crime ONLY if the person is outside and not inside",
					"!indoors" },
	...
	// @SupressWarnings
	public String[] modifyLaw(Area A, LegalBehavior B, Law theLaw, MOB mob,
			String[] oldLaw) throws IOException {
		if (mob.session() == null)
			return oldLaw;
		...
		if (oldLaw == null) {
			if (mob.session()
					.confirm(
							"This is not presently a crime, would you like to make it one (Y/n)?",
							"Y")) {
		...
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			StringBuffer str = new StringBuffer("Modify Law: "
					+ oldLaw[Law.BIT_CRIMENAME] + "\n\r\n\r");
			...
			mob.session().colorOnlyPrintln(str.toString());
			String s = mob.session().choose(
					"Enter a number to modify or RETURN: ", "123456\n", "\n");
			int x = CMath.s_int(s);
			if (x == 0)
				return oldLaw;
			oldLaw = oldLaw.clone();
			switch (x) {
			case 1:
				oldLaw[Law.BIT_CRIMENAME] = mob.session().prompt(
						"Enter a new name for this crime: ",
						oldLaw[Law.BIT_CRIMENAME]);
				break;
			case 5:
				oldLaw[Law.BIT_WARNMSG] = mob.session().prompt(
						"Shame/Justification Message: ",
						oldLaw[Law.BIT_WARNMSG]);
				break;
			case 6:
			...
			case 4: {
				...
				for (int v = 0; v < V.size(); v++) {
					...
					if (!sent) {
						for (int i = 0; i < Law.PUNISHMENTMASK_DESCS.length; i++) {
							if (t.toUpperCase().startsWith(
									Law.PUNISHMENTMASK_DESCS[i])) {
								int x1 = t.indexOf('=');
								if (x1 > 0)
									V2.addElement(Law.PUNISHMENTMASK_DESCS[i]
											.toLowerCase(), t.substring(x1 + 1));
								else
									V2.addElement(Law.PUNISHMENTMASK_DESCS[i]
											.toLowerCase(), "");
								break;
				...
				if (t == null) {
					while (t == null) {
						...
						msg.append("\n\rSelect a flag to toggle or RETURN ("
								+ oldFlags + "): ");
						int selectedMask = -1;
						...
						int indexIfExists = -1;
						for (int i = 0; i < Law.PUNISHMENTMASK_DESCS.length; i++) {
							if (Law.PUNISHMENTMASK_DESCS[i].startsWith(t
									.toUpperCase())) {
								selectedMask = i;
								indexIfExists = V2
										.indexOf(Law.PUNISHMENTMASK_DESCS[selectedMask]
												.toLowerCase());
								t = null;
								break;
							}
						...
						if (t == null) {
							if (indexIfExists >= 0) {
								mob.tell("'" + V2.elementAt(indexIfExists, 1)
										+ "' has been removed.");
								...
								case Law.PUNISHMENTMASK_DETAIN:
									if (!CMLib
										...
										abort = true;
											...
											.session()
											.confirm(
													"Add this room as a new detention center room (y/N)? ",
													"N")) {
										String time = mob
												.session()
												.prompt("Enter the amount of time before they are released: ",
														"");
										if ((time.length() == 0)
												|| (!CMath.isInteger(time))
												|| (CMath.s_int(time) < 0)
												|| (CMath.s_int(time) > 10000)) {
								...
								case Law.PUNISHMENTMASK_FINE: {
									String fine = mob
											.session()
											.prompt("Enter the amount of the fine in base-gold value: ",
													"");
									if ((fine.length() == 0)
											|| (!CMath.isNumber(fine))
											|| (CMath.s_double(fine) < 0)
											|| (CMath.s_double(fine) > 100000.0)) {
										mob.tell("Invalid entry.  Aborted.");
										abort = true;
									} else
										parm = fine;
								...
								if (!abort) {
									V2.addElement(
											Law.PUNISHMENTMASK_DESCS[selectedMask],
											parm);
									mob.tell("'"
											+ V2.elementAt(V2.size() - 1, 1)
											+ parm + "' has been added.");
								} else
									mob.tell("'"
											+ V2.elementAt(V2.size() - 1, 1)
											+ parm + "' has been aborted.");
							}
						} else
							mob.tell("'" + t
									+ "' is not a valid flag.  Unchanged.");
					}
			...
			case 3: {
				...
				for (int i = 0; i < locflags.length; i++) {
					...
					if (lastAnswer
							&& ((("!" + lastOle).equals(locflags[i][1])) || (lastOle
									.equals("!" + locflags[i][1])))) {
						lastAnswer = false;
						lastOle = "";
						continue;
					...
					if (oldVal.startsWith(locflags[i][1].toUpperCase())
							|| (oldVal.indexOf(" "
									+ locflags[i][1].toUpperCase()) >= 0))
						there = true;
					lastAnswer = false;
					lastOle = locflags[i][1];
					if (mob.session().confirm(
							locflags[i][0] + " " + (there ? "(Y/n)" : "(y/N)")
									+ "?", there ? "Y" : "N")) {
						lastAnswer = true;
						s2.append(" " + lastOle);
					}
				...
				restLoca = mob.session().prompt(
						"Enter any other location masks (" + restLoca + "): ",
						restLoca);
				oldLaw[Law.BIT_CRIMELOCS] = (s2.toString() + " " + restLoca)
						.trim();
				break;
			}
			case 2: {
				...
				for (int i = 0; i < lawflags.length; i++) {
					if (lastAnswer
							&& ((("!" + lastOle).equals(lawflags[i][1])) || (lastOle
									.equals("!" + lawflags[i][1])))) {
						lastAnswer = false;
						lastOle = "";
						continue;
					...
					if (oldVal.startsWith(lawflags[i][1].toUpperCase())
							|| (oldVal.indexOf(" "
									+ lawflags[i][1].toUpperCase()) >= 0))
						there = true;
					lastAnswer = false;
					lastOle = lawflags[i][1];
					if (mob.session().confirm(
							lawflags[i][0] + " " + (there ? "(Y/n)" : "(y/N)")
									+ "?", there ? "Y" : "N")) {
						lastAnswer = true;
						s2.append(" " + lastOle);
					}
	...
	public void doIllegalEmotation(Area A, LegalBehavior B, Law theLaw,
			MOB mob, boolean allowedToModify) throws IOException {
		if (mob.session() == null)
			return;
		mob.tell(getFromTOC("P10" + (theLaw.hasModifiableLaws() ? "MOD" : "")));
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			StringBuffer str = new StringBuffer("");
			str.append(CMStrings.padRight("#  Words", 20) + " "
					+ shortLawHeader() + "\n\r");
			for (int x = 0; x < theLaw.otherCrimes().size(); x++) {
				String crime = CMParms.combineWithQuotes(theLaw.otherCrimes()
						.get(x), 0);
				String[] set = theLaw.otherBits().get(x);
				str.append(CMStrings.padRight("" + (x + 1) + ". " + crime, 20)
						+ " " + shortLawDesc(set) + "\n\r");
			}
			...
			if (s.length() == 0)
				...
				if (s.length() > 0) {
					...
					if (newValue != null) {
						...
						changeTheLaw(A, B, mob, theLaw, "CRIME"
								+ (theLaw.otherBits().size() + 1),
								s2.toString());
						mob.tell("Added.");
					}
				}
				...
				if ((x > 0) && (x <= theLaw.otherCrimes().size())) {
					...
					if (newValue != oldLaw) {
						...
						for (int c = 0; c < theLaw.otherCrimes().size(); c++) {
							String crimeWords = CMParms.combineWithQuotes(
									theLaw.otherCrimes().get(c), 0);
						...
						for (int c = 0; c < newBits.length; c++)
							changeTheLaw(A, B, mob, theLaw, "CRIME" + (c + 1),
									newBits[c]);
						changeTheLaw(A, B, mob, theLaw, "CRIME"
								+ (newBits.length + 1), "");
						if (newValue != null)
	...
	public void doBannedSubstances(Area A, LegalBehavior B, Law theLaw,
			MOB mob, boolean allowedToModify) throws IOException {
		if (mob.session() == null)
			return;
		mob.tell(getFromTOC("P10" + (theLaw.hasModifiableLaws() ? "MOD" : "")));
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			StringBuffer str = new StringBuffer("");
			str.append(CMStrings.padRight("#  Items", 20) + " "
					+ shortLawHeader() + "\n\r");
			for (int x = 0; x < theLaw.bannedSubstances().size(); x++) {
				String crime = CMParms.combineWithQuotes(theLaw
						.bannedSubstances().get(x), 0);
			...
			if (s.length() == 0)
					...
					if (resource
							|| mob.session()
									.confirm(
											"'"
													+ s
													+ "' is not a known resource.  Add as a key word anyway (y/N)?",
											"N")) {
						String[] newValue = modifyLaw(A, B, theLaw, mob, null);
						if (newValue != null) {
							StringBuffer s2 = new StringBuffer(s + ";");
							...
							changeTheLaw(A, B, mob, theLaw, "BANNED"
									+ (theLaw.bannedBits().size() + 1),
									s2.toString());
							mob.tell("Added.");
						}
					}
				...
				if ((x > 0) && (x <= theLaw.bannedSubstances().size())) {
					...
					if (newValue != oldLaw) {
						...
						String[] newBits = new String[theLaw.bannedBits()
								.size()];
						for (int c = 0; c < theLaw.bannedSubstances().size(); c++) {
							String crimeWords = CMParms.combineWithQuotes(
									theLaw.bannedSubstances().get(c), 0);
						...
						for (int c = 0; c < newBits.length; c++)
							changeTheLaw(A, B, mob, theLaw, "BANNED" + (c + 1),
									newBits[c]);
						changeTheLaw(A, B, mob, theLaw, "BANNED"
								+ (newBits.length + 1), "");
						if (newValue != null)
	...
	public void doIllegalSkill(Area A, LegalBehavior B, Law theLaw, MOB mob,
			boolean allowedToModify) throws IOException {
		if (mob.session() == null)
			return;
		mob.tell(getFromTOC("P9" + (theLaw.hasModifiableLaws() ? "MOD" : "")));
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			StringBuffer str = new StringBuffer("");
			str.append(CMStrings.padRight("#  Ability", 20) + " "
					+ shortLawHeader() + "\n\r");
			...
			for (String key : theLaw.abilityCrimes().keySet()) {
				...
				if (((AB == null) && (CMLib.flags().getAbilityType(key) < 0) && (CMLib
						.flags().getAbilityDomain(key) < 0))
						|| (set == null)
			...
			for (Enumeration e = filteredTable.keys(); e.hasMoreElements();) {
				...
				str.append(CMStrings.padRight("" + (highest + 1) + ". " + name,
						20) + " " + shortLawDesc(set) + "\n\r");
				highest++;
			}
			str.append("A. ADD A NEW ONE\n\r");
			...
			if (s.length() == 0)
				...
				if (s.length() > 0) {
					...
					if (AB != null)
						s = AB.ID();
					if ((AB == null) && (CMLib.flags().getAbilityType(s) < 0)
						...
						if (newValue != null) {
							...
							changeTheLaw(A, B, mob, theLaw, s.toUpperCase(),
									s2.toString());
							mob.tell("Added.");
						}
					}
				...
				int count = 1;
				if ((x > 0) && (x <= highest))
					for (Enumeration e = filteredTable.keys(); e
							.hasMoreElements();) {
						String key = (String) e.nextElement();
						String[] set = (String[]) filteredTable.get(key);
						if (count == x) {
				...
				if (crimeName.length() > 0) {
					...
					if (newValue != oldLaw) {
						...
						changeTheLaw(A, B, mob, theLaw, crimeName,
								s2.toString());
						mob.tell("Changed.");
					}
				} else
	...
	public void doTaxLaw(Area A, LegalBehavior B, Law theLaw, MOB mob,
		...
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			StringBuffer str = new StringBuffer("");
			str.append("1. PROPERTY TAX   : "
					+ (CMath.s_double(theLaw.getInternalStr("PROPERTYTAX")))
					+ "%\n\r");
			str.append("2. SALES TAX      : "
					+ (CMath.s_double(theLaw.getInternalStr("SALESTAX")))
					+ "%\n\r");
			str.append("3. CITIZEN TAX    : "
					+ (CMath.s_double(theLaw.getInternalStr("CITTAX")))
					+ "%\n\r");
			str.append("4. TAX EVASION    : "
					+ shortLawDesc((String[]) theLaw.taxLaws()
							.get("TAXEVASION")) + "\n\r");
			...
			switch (x) {
			case 1:
				s = mob.session().prompt("Enter a new tax amount: ",
						theLaw.getInternalStr("PROPERTYTAX"));
				if (CMath.s_double(s) != CMath.s_double(theLaw
						.getInternalStr("PROPERTYTAX"))) {
					changeTheLaw(A, B, mob, theLaw, "PROPERTYTAX",
							"" + CMath.s_double(s));
					mob.tell("Changed.");
				}
				break;
			case 2:
				s = mob.session().prompt("Enter a new tax amount: ",
						theLaw.getInternalStr("SALESTAX"));
				if (CMath.s_double(s) != CMath.s_double(theLaw
						.getInternalStr("SALESTAX"))) {
					changeTheLaw(A, B, mob, theLaw, "SALESTAX",
							"" + CMath.s_double(s));
					mob.tell("Changed.");
				}
				break;
			case 3:
				s = mob.session().prompt("Enter a new tax amount: ",
						theLaw.getInternalStr("CITTAX"));
				if (CMath.s_double(s) != CMath.s_double(theLaw
						.getInternalStr("CITTAX"))) {
					changeTheLaw(A, B, mob, theLaw, "CITTAX",
							"" + CMath.s_double(s));
					mob.tell("Changed.");
				}
				break;
			...
			case 5: {
				String room2 = "/";
				while ((room2.equals("/")) || (!room2.equals("*"))
						&& (room2.length() > 0)
						&& (CMLib.map().getRoom(room2) == null))
					room2 = mob.session().prompt(
							"Enter a new room ID (RETURN=" + room
									+ ", *=any): ", room);
				String item2 = mob.session().prompt(
						"Enter an optional container name (RETURN=" + item
								+ "): ", item);
				if ((!room.equalsIgnoreCase(room2))
						|| (!item.equalsIgnoreCase(item2))) {
					changeTheLaw(A, B, mob, theLaw, "TREASURY", "" + room2
							+ ";" + item2);
					mob.tell("Changed.");
				}
				break;
	...
	public void doIllegalInfluence(Area A, LegalBehavior B, Law theLaw,
			MOB mob, boolean allowedToModify) throws IOException {
		if (mob.session() == null)
			return;
		...
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			StringBuffer str = new StringBuffer("");
			str.append(CMStrings.padRight("#  Effect", 20) + " "
					+ shortLawHeader() + "\n\r");
			Hashtable filteredTable = new Hashtable();
			for (String key : theLaw.abilityCrimes().keySet()) {
				String[] set = theLaw.abilityCrimes().get(key);
				...
				if (((AB == null)
						&& (CMLib.flags().getAbilityType(key.substring(1)) < 0) && (CMLib
						.flags().getAbilityDomain(key.substring(1)) < 0))
						|| (set == null) || (set.length < Law.BIT_NUMBITS))
					continue;
				...
				if (s.length() > 0) {
					...
					if ((AB == null) && (CMLib.flags().getAbilityType(s) < 0)
						...
						if (newValue != null) {
							...
							changeTheLaw(A, B, mob, theLaw,
									"$" + s.toUpperCase(), s2.toString());
							mob.tell("Added.");
						}
					}
				}
	...
	public void doBasicLaw(Area A, LegalBehavior B, Law theLaw, MOB mob,
		...
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			StringBuffer str = new StringBuffer("");
			str.append(CMStrings.padRight("#  Law Name", 20) + " "
					+ shortLawHeader() + "\n\r");
			str.append("1. ASSAULT           "
					+ shortLawDesc(theLaw.basicCrimes().get("ASSAULT"))
					+ "\n\r");
			str.append("2. MURDER            "
					+ shortLawDesc(theLaw.basicCrimes().get("MURDER")) + "\n\r");
			str.append("3. NUDITY            "
					+ shortLawDesc(theLaw.basicCrimes().get("NUDITY")) + "\n\r");
			str.append("4. ARMED             "
					+ shortLawDesc(theLaw.basicCrimes().get("ARMED")) + "\n\r");
			str.append("5. RESISTING ARREST  "
					+ shortLawDesc(theLaw.basicCrimes().get("RESISTINGARREST"))
					+ "\n\r");
			str.append("6. ROBBING HOMES     "
					+ shortLawDesc(theLaw.basicCrimes().get("PROPERTYROB"))
					+ "\n\r");
			str.append("\n\r");
			mob.session().colorOnlyPrintln(str.toString());
			if ((!theLaw.hasModifiableLaws()) || (!allowedToModify))
	...
	public void doParoleAndRelease(Area A, LegalBehavior B, Law theLaw,
			MOB mob, boolean allowedToModify) throws IOException {
		if (mob.session() == null)
			return;
		...
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			StringBuffer str = new StringBuffer("");
			str.append("1. LEVEL 1 PAROLE TIME: "
					+ (CMath.s_int(theLaw.getInternalStr("PAROLE1TIME"))
							* CMProps.getTickMillis() / 1000) + " seconds.\n\r");
			str.append("2. LEVEL 2 PAROLE TIME: "
					+ (CMath.s_int(theLaw.getInternalStr("PAROLE2TIME"))
							* CMProps.getTickMillis() / 1000) + " seconds.\n\r");
			str.append("3. LEVEL 3 PAROLE TIME: "
					+ (CMath.s_int(theLaw.getInternalStr("PAROLE3TIME"))
							* CMProps.getTickMillis() / 1000) + " seconds.\n\r");
			str.append("4. LEVEL 4 PAROLE TIME: "
					+ (CMath.s_int(theLaw.getInternalStr("PAROLE4TIME"))
							* CMProps.getTickMillis() / 1000) + " seconds.\n\r");
			str.append("\n\r");
			...
			if (s.equalsIgnoreCase("A")) {
				if (!CMLib.law().getLegalObject(A)
						.inMyMetroArea(mob.location().getArea()))
					mob.tell("You can not add this room as a release room, as it is not in the area.");
				else if (mob.session().confirm(
						"Add this room as a new release room (y/N)? ", "N")) {
				...
				if ((x > 0) && (x <= highest)) {
					if (x > 4) {
						if (mob.session().confirm(
								"Remove this room as a release room (y/N)? ",
								"N")) {
							V.remove(x - 5);
							changed = true;
						}
					} else {
						long oldTime = CMath.s_int(theLaw
								.getInternalStr("PAROLE" + x + "TIME"))
								* CMProps.getTickMillis() / 1000;
						s = mob.session().prompt(
								"Enter a new number of seconds (" + oldTime
										+ "): ", "" + oldTime);
						if ((CMath.s_int(s) != oldTime) && (CMath.s_int(s) > 0)) {
							long x1 = CMath.s_int(s);
							x1 = x1 * 1000 / CMProps.getTickMillis();
							changeTheLaw(A, B, mob, theLaw, "PAROLE" + x
									+ "TIME", "" + x1);
							mob.tell("Changed.");
						}
					}
	...
	public void doJailPolicy(Area A, LegalBehavior B, Law theLaw, MOB mob,
		...
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			StringBuffer str = new StringBuffer("");
			str.append("1. LEVEL 1 JAIL TIME: "
					+ (CMath.s_int(theLaw.getInternalStr("JAIL1TIME"))
							* CMProps.getTickMillis() / 1000) + " seconds.\n\r");
			str.append("2. LEVEL 2 JAIL TIME: "
					+ (CMath.s_int(theLaw.getInternalStr("JAIL2TIME"))
							* CMProps.getTickMillis() / 1000) + " seconds.\n\r");
			str.append("3. LEVEL 3 JAIL TIME: "
					+ (CMath.s_int(theLaw.getInternalStr("JAIL3TIME"))
							* CMProps.getTickMillis() / 1000) + " seconds.\n\r");
			str.append("4. LEVEL 4 JAIL TIME: "
					+ (CMath.s_int(theLaw.getInternalStr("JAIL4TIME"))
							* CMProps.getTickMillis() / 1000) + " seconds.\n\r");
			str.append("\n\r");
			...
			if (s.equalsIgnoreCase("A")) {
				if (!CMLib.law().getLegalObject(A)
					...
					mob.tell("You can not add this room as a jail, as it is not in the area.");
						...
						"Add this room as a new jail room (y/N)? ", "N")) {
					V.add(CMLib.map().getExtendedRoomID(mob.location()));
					changed = true;
				}
			} else {
				...
				if ((x > 0) && (x <= highest)) {
					if (x > 4) {
						if (mob.session().confirm(
								"Remove this room as a jail room (y/N)? ", "N")) {
							V.remove(x - 5);
							changed = true;
						}
					} else {
						long oldTime = CMath.s_int(theLaw.getInternalStr("JAIL"
								+ x + "TIME"))
								* CMProps.getTickMillis() / 1000;
						s = mob.session().prompt(
								"Enter a new number of seconds (" + oldTime
	...
	public void doTresspassingLaw(Area A, LegalBehavior B, Law theLaw, MOB mob,
		...
		while ((mob.session() != null) && (!mob.session().isStopped())) {
			mob.tell("1. Trespassers : "
					+ CMLib.masking().maskDesc(
							theLaw.getInternalStr("TRESPASSERS")));
			mob.tell("2. Law         : "
					+ shortLawDesc(theLaw.basicCrimes().get("TRESPASSING")));
			if ((!theLaw.hasModifiableLaws()) || (!allowedToModify))
				return;
			String prompt = mob.session().choose(
					"Enter one to change or RETURN: ", "12\n", "\n");
			...
			if (x == 1) {
				...
				while (s.trim().equals("?")) {
					s = mob.session().prompt(
							"Enter a new mask, ? for help, or RETURN=["
									+ theLaw.getInternalStr("TRESPASSERS")
									+ "]\n\r: ",
							theLaw.getInternalStr("TRESPASSERS"));
					if (s.trim().equals("?"))
				...
				if (newValue != oldLaw) {
					...
					changeTheLaw(A, B, mob, theLaw, "TRESPASSING",
							s2.toString());
					mob.tell("Changed.");
				}
			}
	...
	public void doVictimsOfCrime(Area A, LegalBehavior B, Law theLaw, MOB mob,
		...
		mob.tell("Protected victims: "
				+ CMLib.masking().maskDesc(theLaw.getInternalStr("PROTECTED")));
		if ((theLaw.hasModifiableLaws()) && (allowedToModify)) {
			String s = "?";
			while (s.trim().equals("?")) {
				s = mob.session()
						.prompt("Enter a new mask, ? for help, or RETURN=["
								+ theLaw.getInternalStr("PROTECTED")
	...
	public void doOfficersAndJudges(Area A, LegalBehavior B, Area legalO,
			Law theLaw, MOB mob, boolean allowedToModify) throws IOException {
		if (mob.session() == null)
			return;
		mob.tell(getFromTOC("P2" + (theLaw.hasModifiableLaws() ? "MOD" : "")
				+ (theLaw.hasModifiableNames() ? "NAM" : "")));
			...
			for (int i = 0; i < R.numInhabitants(); i++) {
				...
				if (M != null) {
					...
					if (B.isAnyOfficer(legalO, M))
						duhOfficers.append(M.name(mob) + " from room '"
								+ R2.displayText(mob) + "'\n\r");
					else if (B.isJudge(legalO, M))
						duhJudge = M.name(mob) + " from room '"
								+ R2.displayText(mob) + "'\n\r";
				}
			}
		}
		...
		if (theLaw.hasModifiableNames() && theLaw.hasModifiableLaws()
				&& allowedToModify) {
			int w = CMath.s_int(mob.session().choose(
					"Enter one to modify, or RETURN to cancel: ", "12\n", ""));
			if (w == 0)
				return;
			String modifiableTag = (w == 1) ? "JUDGE" : "OFFICERS";
			String s = mob.session().prompt(
					"Enter key words from officials name(s) ["
							+ theLaw.getInternalStr(modifiableTag) + "]\n\r: ",
					theLaw.getInternalStr(modifiableTag));
