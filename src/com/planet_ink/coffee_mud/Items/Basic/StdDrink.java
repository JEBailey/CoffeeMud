		...
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_FILL:
				...
				if ((msg.tool() instanceof Container)
						&& ((!(msg.tool() instanceof Drink)) || ((Drink) msg
								.tool()).liquidRemaining() <= 0)
						&& (msg.tool() != msg.target())) {
					...
					if (I instanceof Drink)
						...
						msg.source()
								.tell(((Container) msg.tool()).name(msg
										.source())
										+ " has nothing you can fill this with.");
						return false;
					}
				}
			...
			case CMMsg.TYP_FILL:
				if (msg.tool() instanceof Drink) {
					...
					if (amountOfLiquidRemaining <= 0) {
						...
						if ((thePuddle instanceof RawMaterial)
								&& (((RawMaterial) thePuddle).container() != null)) {
							List<Item> V = this.getContents();
							Drink addHereI = null;
							for (Item I : V)
								if ((I instanceof Drink)
										&& (I instanceof RawMaterial)
										&& (I.Name().equals(thePuddle.Name())))
									addHereI = (Drink) I;
							if (addHereI == null) {
								...
								if (((RawMaterial) thePuddle).owner() instanceof MOB)
									((MOB) (((RawMaterial) thePuddle).owner()))
											.addItem((RawMaterial) addHereI);
								else if (((RawMaterial) thePuddle).owner() instanceof Room)
									((Room) (((RawMaterial) thePuddle).owner()))
											.addItem(
													(RawMaterial) addHereI,
													ItemPossessor.Expire.Player_Drop);
							}
							addHereI.setLiquidRemaining(addHereI
									.liquidRemaining() + amountToTake);
							addHereI.setLiquidHeld(addHereI.liquidHeld()
									+ amountToTake);
							amountToTake = 0;
						}
					}
