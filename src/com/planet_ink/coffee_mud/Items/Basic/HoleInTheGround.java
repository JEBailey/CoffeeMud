...
public class HoleInTheGround extends StdContainer {
	...
	public HoleInTheGround() {
		...
		basePhyStats().setSensesMask(
				basePhyStats.sensesMask() | PhyStats.SENSE_ITEMNOTGET
						| PhyStats.SENSE_ITEMNOWISH | PhyStats.SENSE_ITEMNORUIN
						| PhyStats.SENSE_UNLOCATABLE);
		basePhyStats.setDisposition(basePhyStats.disposition()
				| PhyStats.IS_UNSAVABLE | PhyStats.IS_NOT_SEEN);
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if (msg.target() == owner()) {
			switch (msg.targetMinor()) {
			case CMMsg.TYP_DIG:
				if (CMath.bset(basePhyStats().disposition(),
						PhyStats.IS_NOT_SEEN)
						|| CMath.bset(basePhyStats().disposition(),
								PhyStats.IS_HIDDEN)) {
					basePhyStats().setDisposition(
							CMath.unsetb(basePhyStats().disposition(),
									PhyStats.IS_NOT_SEEN));
					basePhyStats().setDisposition(
							CMath.unsetb(basePhyStats().disposition(),
									PhyStats.IS_HIDDEN));
				...
				if ((msg.tool() != null) && (msg.tool() instanceof Item)) {
					...
					if (pstats != null) {
						...
						if (msg.tool() instanceof Decayable)
							((Decayable) msg.tool())
									.setDecayTime(((Decayable) msg.tool())
											.decayTime() / 2);
