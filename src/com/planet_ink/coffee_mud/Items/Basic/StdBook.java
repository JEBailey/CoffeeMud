...
public class StdBook extends StdItem {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (msg.amITarget(this))
			...
			case CMMsg.TYP_WRITE: {
				...
				boolean admin = (adminReq.length() > 0)
						&& CMLib.masking().maskCheck(adminReq, msg.source(),
								true);
		...
		if (msg.amITarget(this))
			...
			case CMMsg.TYP_READ:
				if (!CMLib.flags().canBeSeenBy(this, mob))
					...
					String adminReq = getAdminReq().trim();
					boolean admin = (adminReq.length() > 0)
							&& CMLib.masking().maskCheck(adminReq, mob, true);
					...
					Vector read = DBRead(mob, Name(), which - 1, lastTime,
							newOnly, all);
			...
			case CMMsg.TYP_WRITE:
				try {
					...
					if (!mob.isMonster()) {
						...
						String subject = mob
								.session()
								.prompt("Enter the name of the chapter (Chapter 1: Start of book),etc : ");
						...
						if (CMLib.journals().makeMessage(mob, messageTitle,
								vbuf, true) == JournalsLibrary.MsgMkrResolution.CANCELFILE) {
							mob.tell("Aborted.");
							return;
						}
	...
	public Vector DBRead(MOB readerMOB, String Journal, int which,
			long lastTimeDate, boolean newOnly, boolean all) {
			...
			for (int j = 0; j < journal.size(); j++) {
				...
				if (to.equals("ALL")
						...
						|| (to.toUpperCase().trim().startsWith("MASK=") && CMLib
								.masking().maskCheck(to.trim().substring(5),
										readerMOB, true))) {
