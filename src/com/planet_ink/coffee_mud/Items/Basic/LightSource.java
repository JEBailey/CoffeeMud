...
public class LightSource extends StdItem implements Light {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		case CMMsg.TYP_HOLD:
			...
			if (room != null) {
				if (((LightSource.inTheRain(room) && (goesOutInTheRain())) || (LightSource
						.inTheWater(msg.source(), room)))
						&& (getDuration() > 0) && (mob.isMine(this))) {
	...
	public void recoverPhyStats() {
		if ((getDuration() > 0) && (isLit()))
			basePhyStats().setDisposition(
					basePhyStats().disposition() | PhyStats.IS_LIGHTSOURCE);
	...
	public boolean tick(Tickable ticking, int tickID) {
		if (tickID == Tickable.TICKID_LIGHT_FLICKERS) {
			if ((owner() != null) && (isLit()) && (getDuration() > 0)) {
	...
	public static boolean inTheRain(Room room) {
		if (room == null)
			return false;
		return (((room.domainType() & Room.INDOORS) == 0) && ((room.getArea()
				.getClimateObj().weatherType(room) == Climate.WEATHER_RAIN) || (room
				.getArea().getClimateObj().weatherType(room) == Climate.WEATHER_THUNDERSTORM)));
	}
	...
	public static boolean inTheWater(MOB mob, Room room) {
		...
		if ((room.domainType() == Room.DOMAIN_OUTDOORS_UNDERWATER)
			...
			return true;
		if ((!CMLib.flags().isFlying(mob)) && (mob.riding() == null)
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (((LightSource.inTheRain(room) && goesOutInTheRain()) || (LightSource
				...
				&& ((!CMLib.flags().isInFlight(mob))
						|| (LightSource.inTheRain(room)) || ((room.domainType() != Room.DOMAIN_OUTDOORS_WATERSURFACE) && (room
			...
			case CMMsg.TYP_EXTINGUISH:
				if (isLit()) {
					...
					CMLib.threads().deleteTick(this,
							Tickable.TICKID_LIGHT_FLICKERS);
					recoverPhyStats();
				...
				if (getDuration() > 0) {
					if (!isLit())
						msg.addTrailerMsg(CMClass.getMsg(mob, null,
								CMMsg.MSG_OK_ACTION, "<S-NAME> light(s) up "
										+ name() + "."));
					else
						mob.tell(name() + " is already lit.");
					light(true);
					CMLib.threads().startTickDown(this,
							Tickable.TICKID_LIGHT_FLICKERS, getDuration());
					recoverPhyStats();
