...
public class GenRecipe extends GenReadable implements Recipe {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if (msg.amITarget(this) && (msg.targetMinor() == CMMsg.TYP_READ)
			...
			if (getTotalRecipePages() > 1) {
				str.append(name() + " contains " + recipeLines.length
						+ " recipe(s)/schematic(s) out of "
						+ getTotalRecipePages() + " total entries.\n\r");
				if (A != null)
					str.append("The following recipes are for the " + A.name()
							+ " skill:\n\r");
			} else if (A != null)
				str.append("The following recipe is for the " + A.name()
						+ " skill:\n\r");
			if (A instanceof ItemCraftor) {
				ItemCraftor C = (ItemCraftor) A;
				for (String line : recipeLines) {
					List<String> V = CMParms.parseTabs(line + " ", false);
					Pair<String, Integer> nameAndLevel = C
							.getDecodedItemNameAndLevel(V);
					String components = C.getDecodedComponentsDescription(
							msg.source(), V);
	...
	public void recoverPhyStats() {
		...
		if (this.getTotalRecipePages() == 1) {
			if (replaceName == null) {
				replaceName = "";
				int x = Name().indexOf('%');
				if ((recipeLines != null) && (recipeLines.length == 1)
						&& (this.getCommonSkillID().length() > 0)) {
					Ability A = CMClass.getAbility(this.getCommonSkillID());
					if (A instanceof ItemCraftor) {
						List<String> V = CMParms.parseTabs(recipeLines[0],
								false);
						Pair<String, Integer> nameAndLevel = ((ItemCraftor) A)
								.getDecodedItemNameAndLevel(V);
						String itemName = CMStrings.replaceAll(
								nameAndLevel.first, "% ", "");
	...
	private final static String[] MYCODES = { "SKILLID", "RECIPES",
			"NUMRECIPES" };
