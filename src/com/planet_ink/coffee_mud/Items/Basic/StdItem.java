...
public class StdItem implements Item {
	...
	public void recoverPhyStats() {
		...
		if (((phyStats().ability() > 0) && abilityImbuesMagic())
				|| (this instanceof MiscMagic))
			phyStats().setDisposition(
					phyStats().disposition() | PhyStats.IS_BONUS);
		if ((owner() != null) && (owner() instanceof MOB)
				&& (CMLib.flags().isHidden(this)))
			phyStats()
					.setDisposition(
							(int) (phyStats().disposition() & (PhyStats.ALLMASK - PhyStats.IS_HIDDEN)));
	}
	...
	public void setRiding(Rideable ride) {
		if ((ride != null) && (riding() != null) && (riding() == ride)
				&& (riding().amRiding(this)))
			return;
	...
	public boolean wearIfPossible(MOB mob, long wearCode) {
		...
		if ((fitsOn(wearCode)) && (canWear(mob, wearCode))) {
			wearAt(wearCode);
			return true;
		}
		return false;
	...
	public void affectPhyStats(final Physical affected,
		...
		if (affected instanceof Room) {
			...
			if (CMLib.flags().isLightSource(this)) {
				if (rawWornCode() != Wearable.IN_INVENTORY)
					affectableStats.setDisposition(affectableStats
							.disposition() | PhyStats.IS_LIGHTSOURCE);
			...
			if ((amWearingAt(Wearable.WORN_MOUTH)) && (affected instanceof MOB)) {
				...
				if ((this instanceof Armor)
						&& (!CMath.bset(((Armor) this).getClothingLayer(),
	...
	public boolean tick(final Tickable ticking, final int tickID) {
		...
		if (tickID == Tickable.TICKID_ITEM_BEHAVIOR) {
			...
			if ((numBehaviors() == 0) && (numScripts() == 0))
				return false;
		} else if ((tickID != Tickable.TICKID_CLANITEM)
				&& (tickID != Tickable.TICKID_ELECTRONICS)) {
	...
	protected boolean canWearComplete(MOB mob, long wearWhere) {
		if (!canWear(mob, wearWhere)) {
			...
			Item alreadyWearing = (cantWearAt == 0) ? null : mob
					.fetchFirstWornItem(cantWearAt);
			Wearable.CODES codes = Wearable.CODES.instance();
			if (alreadyWearing != null) {
				if ((cantWearAt != Wearable.WORN_HELD)
						&& (cantWearAt != Wearable.WORN_WIELD)) {
					if (!CMLib.commands()
							.postRemove(mob, alreadyWearing, false)) {
						mob.tell("You are already wearing "
								+ alreadyWearing.name() + " on your "
								+ codes.name(cantWearAt) + ".");
						return false;
					}
					...
					short layer = (this instanceof Armor) ? ((Armor) this)
							.getClothingLayer() : 0;
					short layer2 = (alreadyWearing instanceof Armor) ? ((Armor) alreadyWearing)
							.getClothingLayer() : 0;
					if ((rawProperLocationBitmap() == alreadyWearing
							.rawProperLocationBitmap())
							&& (rawLogicalAnd())
							&& (alreadyWearing.rawLogicalAnd())
							&& (layer == layer2)
							&& (CMLib.commands().postRemove(mob,
									alreadyWearing, false)))
						return true;
					if (cantWearAt == Wearable.WORN_HELD)
						mob.tell("You are already holding "
								+ alreadyWearing.name() + ".");
					else if (cantWearAt == Wearable.WORN_WIELD)
						mob.tell("You are already wielding "
								+ alreadyWearing.name() + ".");
					else
						mob.tell("You are already wearing "
								+ alreadyWearing.name() + " on your "
				...
				if (locs.length() == 0)
					...
					mob.tell("You can't wear that on your "
							+ locs.toString().substring(1).trim() + ".");
				return false;
			} else {
		...
		if (msg.tool() == this) {
			...
			case CMMsg.TYP_POUR:
				if (!(this instanceof Drink)) {
					mob.tell(mob, this, null,
							"You can't do that with <T-NAMESELF>.");
					return false;
				}
				break;
			default:
			...
			return true;
				...
				&& ((displayText().length() == 0) || ((msg.tool() != null)
						&& (msg.tool() instanceof Ability) && (((Ability) msg
						.tool()).abstractQuality() == Ability.QUALITY_MALICIOUS)))) {
			...
			case CMMsg.TYP_RELOAD:
				if ((this instanceof AmmunitionWeapon)
					...
					if (((AmmunitionWeapon) this).ammunitionRemaining() >= ((AmmunitionWeapon) this)
							.ammunitionCapacity()) {
			...
			case CMMsg.TYP_HOLD:
				if ((!fitsOn(Wearable.WORN_HELD)) || (properWornBitmap == 0)) {
					StringBuffer str = new StringBuffer("You can't hold "
							+ name() + ".");
				...
				if ((!rawLogicalAnd()) || (properWornBitmap == 0)) {
					if (!canWear(mob, Wearable.WORN_HELD)) {
						...
						if (alreadyWearing != null) {
							if ((!CMLib.commands().postRemove(mob,
									alreadyWearing, false))
			...
			case CMMsg.TYP_WEAR:
				...
				return canWearComplete(mob, (msg.value() <= 0) ? 0
						: ((long) (1 << msg.value()) / 2));
			case CMMsg.TYP_WIELD:
				...
				if ((!rawLogicalAnd()) || (properWornBitmap == 0)) {
					if (!canWear(mob, Wearable.WORN_WIELD)) {
						Item alreadyWearing = mob
								.fetchFirstWornItem(Wearable.WORN_WIELD);
						if (alreadyWearing != null) {
							if (!CMLib.commands().postRemove(mob,
									alreadyWearing, false)) {
								mob.tell("You are already wielding "
			...
			case CMMsg.TYP_GET:
				if ((msg.tool() == null) || (msg.tool() instanceof MOB)) {
					...
					if ((mob.phyStats().level() < phyStats().level()
							- (10 + (mob.phyStats().level() / 5)))
							&& (!(mob instanceof ShopKeeper))
							&& (!mob.charStats().getMyRace().leveless())
							&& (!mob.charStats().getCurrentClass().leveless())) {
						mob.tell(name()
								+ " is too powerful to endure possessing it.");
						return false;
					}
					if ((phyStats().weight() > (mob.maxCarry() - mob.phyStats()
							.weight()))
							&& (!mob.isMine(this))
							&& (phyStats().weight() != 0)) {
					...
					if ((numberOfItems() > (mob.maxItems() - mob.numItems()))
							&& (!mob.isMine(this))) {
				...
				if ((msg.tool() == null) || (msg.tool() instanceof MOB)) {
					...
					if (rawWornCode() > 0)
						for (int i = 0; i < mob.numItems(); i++) {
							I = mob.getItem(i);
							if ((I != null) && (I != this)
									&& ((I.rawWornCode() & rawWornCode()) > 0)) {
								thislayer = (I instanceof Armor) ? ((Armor) I)
										.getClothingLayer() : 0;
								if (thislayer > layer) {
									mob.tell(mob, I, null,
											"You must remove <T-NAME> first.");
									return false;
								}
							}
						}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		MOB mob = msg.source();
		if ((msg.tool() == this) && (msg.sourceMinor() == CMMsg.TYP_THROW)
				&& (mob != null) && (msg.target() != null)) {
			...
			case CMMsg.TYP_WRITE:
				if (isReadable() && (!(this instanceof Electronics)))
					setReadableText((readableText() + " " + msg.targetMessage())
							.trim());
				break;
			case CMMsg.TYP_EXPIRE:
			case CMMsg.TYP_DEATH:
	...
	public void removeFromOwnerContainer() {
		...
		if (owner instanceof Room) {
			...
			for (int r = thisRoom.numItems() - 1; r >= 0; r--) {
				Item thisItem = thisRoom.getItem(r);
				if ((thisItem != null) && (thisItem.container() != null)
						&& (thisItem.container() == this))
	...
	public void addBehavior(Behavior to) {
		...
		if (behaviors.size() == 0)
			CMLib.threads().startTickDown(this, Tickable.TICKID_ITEM_BEHAVIOR,
					1);
		...
		if (!scripts.contains(S)) {
			...
			if (scripts.size() == 0)
				CMLib.threads().startTickDown(this,
						Tickable.TICKID_ITEM_BEHAVIOR, 1);
	...
	protected static String[] CODES = { "CLASS", "USES", "LEVEL", "ABILITY",
			"TEXT" };
