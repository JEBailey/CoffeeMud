	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		case CMMsg.TYP_ENTER: {
			...
			if ((myStationaryPortal != null)
				...
				myStationaryPortal.setReadableText(CMLib.map()
						.getExtendedRoomID(R));
			else {
				...
				if (choices.size() > 0) {
					if (choices.size() == 1)
						...
						if (((myStationaryPortal == null) || (myStationaryPortal
								.amDestroyed()))
								&& (secretIdentity().length() > 0))
							for (int i = 0; i < choices.size(); i++)
								if (((Item) choices.elementAt(i))
										.secretIdentity().equals(
												secretIdentity())) {
									myStationaryPortal = (StdPortal) choices
											.elementAt(i);
									break;
								}
						if ((myStationaryPortal == null)
								|| (myStationaryPortal.amDestroyed()))
							for (int i = 0; i < choices.size(); i++)
								if (((Item) choices.elementAt(i)).Name()
										.equals(Name())) {
									myStationaryPortal = (StdPortal) choices
											.elementAt(i);
									break;
