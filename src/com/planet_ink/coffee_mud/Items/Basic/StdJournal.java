...
public class StdJournal extends StdItem {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (msg.amITarget(this))
			...
			case CMMsg.TYP_WRITE: {
				...
				boolean admin = ((adminReq.length() > 0) && CMLib.masking()
						.maskCheck(adminReq, msg.source(), true))
						|| CMSecurity.isJournalAccessAllowed(msg.source(),
								Name());
				if ((!CMLib.masking().maskCheck(getWriteReq(), msg.source(),
						true))
						&& (!admin)
						&& (!(CMSecurity.isAllowed(msg.source(), msg.source()
								.location(), CMSecurity.SecFlag.JOURNALS)))) {
					msg.source().tell(
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(this))
			...
			case CMMsg.TYP_READ:
				if (!CMLib.flags().canBeSeenBy(this, mob))
				...
				else if ((!mob.isMonster()) && (mob.playerStats() != null)) {
					String adminReq = getAdminReq().trim();
					boolean admin = ((adminReq.length() > 0) && CMLib.masking()
							.maskCheck(adminReq, mob, true))
							|| CMSecurity.isJournalAccessAllowed(mob, Name());
					long lastTime = mob.playerStats().lastDateTime();
					if ((!admin)
							&& (!CMLib.masking().maskCheck(getReadReq(), mob,
									true))) {
					...
					for (int v = 0; v < parse.size(); v++) {
						...
						if (CMath.s_long(s) > 0)
							...
							newOnly = true;
								...
								|| s.equalsIgnoreCase("OLD"))
							all = true;
					}
					JournalsLibrary.JournalEntry read = DBRead(mob, Name(),
							which - 1, lastTime, newOnly, all);
					...
					while ((megaRepeat) && (read != null)) {
						...
						if ((entry.toString().trim().length() > 0)
								&& (which > 0)
								&& (CMLib.masking().maskCheck(getWriteReq(),
										mob, true)
										|| (admin) || (CMSecurity.isAllowed(
										msg.source(), msg.source().location(),
										CMSecurity.SecFlag.JOURNALS)))) {
							boolean repeat = true;
							while ((repeat) && (mob.session() != null)
								...
								try {
									...
									if ((CMProps.getVar(CMProps.Str.MAILBOX)
											.length() > 0)
											&& (from.length() > 0))
									...
									prompt += "or RETURN: ";
									String s = mob.session().choose(prompt,
											cmds + "\n", "\n");
									if (s.equalsIgnoreCase("S"))
										...
										if ((M == null)
											...
											mob.tell("Player '"
													+ from
													+ "' does not exist, or has no email address.");
											repeat = true;
										} else if (!mob
												.session()
												.choose("Send email to "
														+ M.Name() + " (Y/n)?",
														"YN\n", "Y")
												.equalsIgnoreCase("N")) {
											String replyMsg = mob
													.session()
													.prompt("Enter your email response\n\r: ");
										...
										try {
											journal = mob
													.session()
													.prompt("Enter the journal to transfer this msg to: ",
															"", 30000);
										} catch (IOException e) {
											mob.tell("Timed out.");
											repeat = true;
											...
											if (realName == null)
												realName = CMLib
														...
														.DBGetRealJournalName(
																journal.toUpperCase());
											if (realName == null)
												mob.tell("The journal '"
														+ journal
														+ "' does not presently exist.  Aborted.");
											else {
												List<JournalsLibrary.JournalEntry> journal2 = CMLib
														.database()
														.DBReadJournalMsgs(
																Name());
												JournalsLibrary.JournalEntry entry2 = journal2
														.get(which - 1);
												CMLib.database()
														.DBDeleteJournal(
																Name(),
										...
										read = DBRead(mob, Name(), which - 1,
												lastTime, newOnly, all);
										if (read != null) {
			...
			case CMMsg.TYP_WRITE:
				try {
					...
					if ((msg.targetMessage().toUpperCase().startsWith("DEL"))
							&& (CMSecurity.isAllowed(mob, mob.location(),
									CMSecurity.SecFlag.JOURNALS) || admin)
							&& (!mob.isMonster())) {
						if (mob.session()
								.confirm(
										"Delete all journal entries? Are you sure (y/N)?",
										"N"))
						...
						if (CMath.s_bool(getParm("PRIVATE")))
							...
							if (((!to.toUpperCase().trim().startsWith("MASK=") || (!CMSecurity
						...
						String subject = mob.session().prompt(
								"Enter a subject: ");
						...
						if ((subject.toUpperCase().startsWith("MOTD")
								|| subject.toUpperCase().startsWith("MOTM") || subject
								.toUpperCase().startsWith("MOTY"))
								&& (!admin)
								&& (!(CMSecurity.isAllowed(mob, mob.location(),
										CMSecurity.SecFlag.JOURNALS))))
						...
						CMLib.database().DBWriteJournal(Name(), mob.Name(), to,
								subject, message);
			...
			long[] newestDate = CMLib.database().DBJournalLatestDateNewerThan(
					Name(), msg.source().Name(),
					msg.source().playerStats().lastDateTime());
			if ((newestDate[0] > 0)
				...
				msg.addTrailerMsg(CMClass.getMsg(msg.source(), null, null,
						CMMsg.MSG_OK_VISUAL, name() + " has " + newestDate[1]
								+ " new messages.", CMMsg.NO_EFFECT, null,
	...
	public JournalsLibrary.JournalEntry DBRead(MOB reader, String Journal,
			int which, long lastTimeDate, boolean newOnly, boolean all) {
		StringBuffer buf = new StringBuffer("");
		List<JournalsLibrary.JournalEntry> journal = CMLib.database()
				.DBReadJournalMsgs(Journal);
		...
		if ((which < 0) || (journal == null) || (which >= journal.size())) {
			buf.append("#\n\r "
					...
					+ ((shortFormat) ? "" : "" + CMStrings.padRight("From", 11)
							+ CMStrings.padRight("To", 11))
					+ CMStrings.padRight("Date", 20) + "Subject\n\r");
			buf.append("-------------------------------------------------------------------------\n\r");
			if (journal == null) {
		...
		if ((which < 0) || (which >= journal.size())) {
			...
			for (int j = 0; j < journal.size(); j++) {
				...
				boolean mayRead = (to.equals("ALL")
						|| to.equalsIgnoreCase(reader.Name()) || from
						.equalsIgnoreCase(reader.Name()));
				if ((to.toUpperCase().trim().startsWith("MASK="))
						&& (CMLib.masking().maskCheck(to.trim().substring(5),
								reader, true))) {
					mayRead = true;
				...
				if (mayRead) {
					if (compdate > lastTimeDate)
						...
						selection.append(" ");
					selection.append("^<JRNL \""
							+ CMStrings.removeColors(name())
							+ "\"^>"
							+ CMStrings.padRight((j + 1) + "", 4)
							+ "^</JRNL^>) "
							+ ((shortFormat) ? "" : ""
									+ CMStrings.padRight(from, 10) + " "
									+ CMStrings.padRight(to, 10) + " ")
							+ CMStrings.padRight(
									CMLib.time().date2String(date), 19)
							+ " "
							+ CMStrings.padRight(subject,
									25 + (shortFormat ? 22 : 0)) + "\n\r");
			...
			boolean allMine = (to.equalsIgnoreCase(reader.Name()) || from
					.equalsIgnoreCase(reader.Name()));
			if ((to.toUpperCase().trim().startsWith("MASK="))
					&& (CMLib.masking().maskCheck(to.trim().substring(5),
							reader, true))) {
			...
			try {
				if (message.startsWith("<cmvp>"))
					message = new String(CMLib.webMacroFilter()
							.virtualPageFilter(message.substring(6).getBytes()));
			} catch (HTTPRedirectException e) {
			}

			if ((allMine) || (to.equals("ALL")))
				buf.append("\n\r^<JRNL \"" + CMStrings.removeColors(name())
						+ "\"^>" + CMStrings.padRight((which + 1) + "", 3)
						+ "^</JRNL^>)\n\r" + "FROM: " + from + "\n\rTO  : "
	...
	private String getParm(String parmName) {
		if (readableText().length() == 0)
			return "";
		Map<String, String> h = CMParms.parseEQParms(readableText()
				.toUpperCase(), new String[] { "READ", "WRITE", "REPLY",
