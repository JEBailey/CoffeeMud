...
public class Corpse extends GenContainer implements DeadBody {
	...
	public boolean destroyAfterLooting() {
		return destroyAfterLooting;
	}

	public void setDestroyAfterLooting(boolean truefalse) {
		destroyAfterLooting = truefalse;
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((msg.targetMinor() == CMMsg.TYP_SIT)
				...
				&& (msg.source().phyStats().weight() <= 0) && (playerCorpse())
				&& (mobName().length() > 0)) {
			CMLib.utensils().resurrect(msg.source(), msg.source().location(),
					this, -1);
			return;
		}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((msg.amITarget(this) || (msg.tool() == this)) && (playerCorpse())
				&& (mobName().length() > 0)) {
			if ((msg.targetMinor() == CMMsg.TYP_SIT)
					&& (msg.source().name().equalsIgnoreCase(mobName()))
					&& (CMLib.flags().isGolem(msg.source()))
			...
			if (((msg.targetMinor() == CMMsg.TYP_GET) || ((msg.tool() instanceof Ability)
					&& (!msg.tool().ID().equalsIgnoreCase("Prayer_Resurrect"))
					&& (!msg.tool().ID()
							.equalsIgnoreCase("Prayer_PreserveBody")) && (!msg
					.tool().ID().equalsIgnoreCase("Song_Rebirth"))))
					&& (CMProps.getVar(CMProps.Str.CORPSEGUARD).length() > 0)
					&& (!msg.targetMajor(CMMsg.MASK_INTERMSG))) {
				if (CMSecurity.isAllowed(msg.source(), msg.source().location(),
						CMSecurity.SecFlag.CMDITEMS))
					return true;

				MOB ultimateFollowing = msg.source().amUltimatelyFollowing();
				if ((msg.source().isMonster())
						&& ((ultimateFollowing == null) || (ultimateFollowing
								.isMonster())))
					return true;
				if (CMProps.getVar(CMProps.Str.CORPSEGUARD).equalsIgnoreCase(
						"ANY"))
					return true;
				if (mobName().equalsIgnoreCase(msg.source().Name()))
					return true;
				else if (CMProps.getVar(CMProps.Str.CORPSEGUARD)
						.equalsIgnoreCase("SELFONLY")) {
					msg.source().tell(
							"You may not loot another players corpse.");
					return false;
				} else if (CMProps.getVar(CMProps.Str.CORPSEGUARD)
						.equalsIgnoreCase("PKONLY")) {
					if (!(CMath.bset((msg.source()).getBitmap(),
							MOB.ATT_PLAYERKILL))) {
						msg.source()
								.tell("You can not get that.  You are not a player killer.");
						return false;
					} else if (mobPKFlag()) {
						msg.source().tell(
								"You can not get that.  " + mobName()
										+ " was not a player killer.");
						return false;
					}
				}
			}
