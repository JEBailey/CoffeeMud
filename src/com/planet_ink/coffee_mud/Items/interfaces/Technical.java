...
public interface Technical {
	...
	public static enum TechType {
		ANY, GIZMO, PERSONAL_WEAPON, PERSONAL_SENSOR, PERSONAL_SHIELD, PERSONAL_SOFTWARE, PERSONAL_TRACTOR, SHIP_SPACESHIP, SHIP_PANEL, SHIP_WEAPON, SHIP_SHIELD, SHIP_ENGINE, SHIP_SENSOR, SHIP_POWER, SHIP_COMPUTER, SHIP_SOFTWARE, SHIP_ENVIRO_CONTROL, SHIP_GENERATOR, SHIP_DAMPENER, SHIP_TRACTOR
	}
	...
	public static enum TechCommand {
		THRUST(ShipEngine.ThrustPort.class, Integer.class), ACCELLLERATION(
				ShipEngine.ThrustPort.class, Integer.class, Long.class), COMPONANTFAILURE(
				Technical.TechType.class, String[].class), AIRREFRESH(
		...
		public Object[] confirmAndTranslate(String[] parts) {
			...
			for (int i = 0; i < parms.length; i++)
				if (parms[i].isEnum()) {
					resp[i] = CMath.s_valueOf((Class<? extends Enum>) parms[i],
							parts[i + 1]);
					if (resp[i] == null)
						return null;
				} else if (Integer.class.isAssignableFrom(parms[i])
						|| Long.class.isAssignableFrom(parms[i])) {
				...
				} else if (Double.class.isAssignableFrom(parms[i])
						|| Float.class.isAssignableFrom(parms[i])) {
		...
		public static TechCommand findCommand(String[] parts) {
			if (parts.length == 0)
				return null;
			return (TechCommand) CMath.s_valueOf(TechCommand.class, parts[0]
					.toUpperCase().trim());
		}
	}
}
