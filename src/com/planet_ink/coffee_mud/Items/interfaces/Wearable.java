...
public interface Wearable extends Environmental {
	/**
	 * a constant used in the Locale item search classes to filter on only items
	 * being worn
	 */
	...
	/**
	 * a constant used in the Locale item search classes to filter on only items
	 * being worn OR not being worn
	 */
	@SuppressWarnings("unchecked")
	public static final Filterer<Environmental> FILTER_ANY = Filterer.ANYTHING;

	/**
	 * Can test where, if anywhere, an item is being worn. The value may be 0 to
	 * see if the item is not being worn (since 0 means inventory) or a
	 * combination of 1 or more of the worn codes listed in the Item interface.
	 * 
	 * @see Item
	 * @param wornCode
	...
	/**
	 * Whether this item is designated as being wearable on the given worn codes
	 * defined in the Item interface. This method does not care about the
	 * practicality of wearing for a given mob (in other words, whether an item
	 * is already being worn on the location is not checked).
	 * 
	 * @see Item
	 * @param wornCode
	...
	/**
	 * Returns a bitmap of all the locations where the given mob is practically
	 * unable to wear an item. The method compares items the mob is currently
	 * wearing to all possible wear locations, including locations where the mob
	 * can't wear things, and returns a bitmap representing all of those
	 * unwearable locations. The bitmap is constructed using the bitmap
	 * constants defined in the Item interface.
	 * 
	 * @see Item
	 * @param mob
	 *            the player or mob to evaluate for unwearable locations
	 * @return a bitmap made up of unwearable location bitmaps, from constants
	 *         in Item interface
	 */
	public long whereCantWear(MOB mob);

	/**
	 * Returns whether it is practical for the given mob to wear this Item on
	 * the given worn location as given by the wornCode. The method checks for
	 * existing limbs and wear locations, and for items already being worn by
	 * the mob, and finally determines if the worn location given by the
	 * wornCode is among the available slots.
	 * 
	 * @see Item
	 * @param mob
	 *            the player or mob being evaluated
	 * @param wornCode
	 *            the worn location to check and see if this Item can be worn
	 *            there.
	 * @return whether this item can be worn by the given player at the given
	 *         wornCode
	 */
	public boolean canWear(MOB mob, long wornCode);

	/**
	 * Using the canWear method, this method will put the item into a state of
	 * being worn only if it is practical for the given mob or player to wear
	 * this Item. The mob or player must have the item in his or her inventory
	 * first.
	 * 
	 * @param mob
	...
	/**
	 * Using the canWear method, this method will put the item into a state of
	 * being worn on the given location only if it is practical for the given
	 * mob or player to wear this Item at the given location. The mob or player
	 * must have the item in his or her inventory first.
	 * 
	 * @param mob
	 *            the player or mob to put this item on.
	 * @param wearCode
	...
	/**
	 * Puts this item into a state of being worn regardless of whether it is
	 * practical for the given mob to wear it -- for instance, even if an item
	 * is already being worn where this item wants to be worn, or if the player
	 * has no such limbs to wear this item. None of that matter to this method.
	 * The item must be in the mobs inventory first.
	 * 
	 * @param mob
	 *            the player or mob
	 */
	...
	/**
	 * This method is similar to the wearEvenIfImpossible method method above,
	 * except that it does not inspect this item for allowed wearable locations,
	 * but always puts the item on the wear location represented by the given
	 * wornCode bitmap. This bitmap is made up of constants from the Item
	 * interface.
	 * 
	 * @see Item
	 * @see Wearable#wearEvenIfImpossible(MOB)
	 * @param wornCode
	 *            the bitmap from Item interface constants used
	 */
	public void wearAt(long wornCode);

	/**
	 * Removes this item from a state of being worn, and puts it back into the
	 * mob or player inventory. setContainer(null) may still need be called to
	 * make the item visible to the players inventory.
	 */
	public void unWear();

	/**
	 * Returns the bitmap representing where this item is presently being worn
	 * by the mob or player who has the item in his or inventory. The value
	 * returned by this method is set by the wear* methods defined above. The
	 * bitmap is made up of a combination of the wear location constants from
	 * the Item interface.
	 * 
	 * @see Item
	 ...
	 * the mob or player who has the item in his or inventory. The value set by
	 * this method is set by the wear* methods defined above. The bitmap is made
	 * up of a combination of the wear location constants from the Item
	 * interface.
	 * 
	 * @see Item
	 * @see Wearable#wearAt(long)
	...
	/**
	 * Returns the builder-defined bitmap made from a logical combination of the
	 * worn location constants from the Item interface. When combined with the
	 * rawLogicalAnd() method, it says whether this item is wearable on all of
	 * the locations defined by this bitmap at the same time or whether this
	 * item is wearable on any of the locations defined by this bitmap.
	 * 
	 * @see Item
	...
	/**
	 * Sets the builder-defined bitmap made from a logical combination of the
	 * worn location constants from the Item interface. When combined with the
	 * rawLogicalAnd() method, it says whether this item is wearable on all of
	 * the locations defined by this bitmap at the same time or whether this
	 * item is wearable on any of the locations defined by this bitmap.
	...
	/**
	 * Flag which determines whether the rawProperLocationBitmap represents the
	 * fact that it is worn on ALL locations (value of true) or worn on any of
	 * the locations.
	 * 
	 * @see Wearable#rawProperLocationBitmap()
	 * @return whether this item is worn on all locations or any of the
	 *         locations
	 */
	public boolean rawLogicalAnd();

	/**
	 * Sets flag which determines whether the rawProperLocationBitmap represents
	 * the fact that it is worn on ALL locations (value of true) or worn on any
	 * of the locations.
	 * 
	 * @see Wearable#rawProperLocationBitmap()
	 * @param newAnd
	 *            whether this item is worn on all locations or any of the
	 *            locations
	 */
	public void setRawLogicalAnd(boolean newAnd);

	/**
	 * compares whether this item is allowed to be worn on the same locations as
	 * the given item.
	 * 
	 * @param toThis
	 *            the item to compare this items allowed worn locations to
	 * @return whether this item is allowed to be worn on the same place as the
	 *         param
	 */
	 ...
	 * The array has worn location constants in the order in which they are
	 * presented to the user.
	 */
	public static final long[] DEFAULT_WORN_ORDER = { WORN_HEAD, WORN_EYES,
			WORN_EARS, WORN_MOUTH, WORN_NECK, WORN_BACK, WORN_ABOUT_BODY,
			WORN_TORSO, WORN_ARMS, WORN_LEFT_WRIST, WORN_RIGHT_WRIST,
			WORN_HANDS, WORN_LEFT_FINGER, WORN_RIGHT_FINGER, WORN_WAIST,
			WORN_LEGS, WORN_FEET, WORN_FLOATING_NEARBY, WORN_WIELD, WORN_HELD, };

	/**
	 * An array representing the armor protective strength of the worn location
	 * bitmaps, in the same order as their numeric value.
	 */
	public static final double[] DEFAULT_WORN_WEIGHTS = { 0.0, // inventory
			1.0, // ON_HEAD
			0.5, // ON_NECK
			3.0, // ON_TORSO
			1.0, // ON_ARMS
	...
	/**
	 * An array representing the relative weight of items made for each of the
	 * several worn locations, in the same order as their numeric value. These
	 * weights are broken, in turn, into values for cloth, leather, and metal
	 * armors respectively.
	 */
	public static final double[][] DEFAULT_WORN_WEIGHT_POINTS = { // cloth,
																	// leather,
																	// metal
	...
	/**
	 * An array naming each of the worn location constants, in the order of
	 * their numeric value.
	 */
	public static final String[] DEFAULT_WORN_DESCS = { "inventory", "head",
			"neck", "torso", "arms", "left wrist", "right wrist",
			"left finger", "right finger", "feet", "held", "wield", "hands",
			"floating nearby", "waist", "legs", "eyes", "ears", "body",
			"mouth", "back" };

	/**
	 * An array containing all of the worn codes,in the order of their numeric
	 * value.
	 */
	public static final long[] DEFAULT_WORN_CODES = { IN_INVENTORY, WORN_HEAD,
			WORN_NECK, WORN_TORSO, WORN_ARMS, WORN_LEFT_WRIST,
			WORN_RIGHT_WRIST, WORN_LEFT_FINGER, WORN_RIGHT_FINGER, WORN_FEET,
			WORN_HELD, WORN_WIELD, WORN_HANDS, WORN_FLOATING_NEARBY,
			WORN_WAIST, WORN_LEGS, WORN_EYES, WORN_EARS, WORN_ABOUT_BODY,
			WORN_MOUTH, WORN_BACK, };

	/**
	 * A Chart, indexed by WORN_CODE, showing the other parts dependent on that
	 * one.
	 */
	public final static long[] DEFAULT_WORN_DEPENDENCYGRID = {
			...
			WORN_HEAD | WORN_NECK | WORN_ARMS | WORN_LEFT_WRIST
					| WORN_RIGHT_WRIST | WORN_LEFT_FINGER | WORN_RIGHT_FINGER
					| WORN_FEET | WORN_HELD | WORN_WIELD | WORN_HANDS
					| WORN_WAIST | WORN_LEGS | WORN_EYES | WORN_EARS
					| WORN_MOUTH | WORN_BACK,// torso
			WORN_LEFT_WRIST | WORN_RIGHT_WRIST | WORN_LEFT_FINGER
					| WORN_RIGHT_FINGER | WORN_HELD | WORN_WIELD | WORN_HANDS, // arms
			WORN_LEFT_FINGER | WORN_HANDS | WORN_HELD | WORN_WIELD,
			WORN_RIGHT_FINGER | WORN_HANDS | WORN_HELD | WORN_WIELD, -1, -1,
			-1, -1, -1, WORN_HELD | WORN_WIELD, -1, WORN_WAIST, WORN_FEET, -1,
			-1, -1, -1, -1, };
	...
	public class CODES {
			...
			synchronized (this) {
				...
				for (int i = 0; i < Wearable.DEFAULT_WORN_CODES.length; i++)
					add(DEFAULT_WORN_DESCS[i], DEFAULT_WORN_DEPENDENCYGRID[i],
							DEFAULT_WORN_WEIGHTS[i], CMParms.indexOf(
									DEFAULT_WORN_ORDER, DEFAULT_WORN_CODES[i]),
				...
				allCodesInOrder = Arrays.copyOf(Wearable.DEFAULT_WORN_ORDER,
						Wearable.DEFAULT_WORN_ORDER.length);
				...
				for (int i = 0; i < addExtra.length + repExtra.length; i++) {
					...
					String stat = array[0].toLowerCase().trim()
							.replace('_', ' ');
					String p = array[1];
					List<String> V = CMParms.parseCommas(p, false);
					if (V.size() != 6) {
						Log.errOut("Wearable",
								"Bad coffeemud.ini wear loc row (requires 6 elements, separated by ,): "
										+ p);
						continue;
					}
					String type = "ADD";
					...
					if (replace) {
						...
						if (idx >= 0) {
							...
							Log.errOut("Wearable",
									"Bad replace worn loc in coffeemud.ini file: "
											+ stat);
							continue;
						}
					}
					String dependencyMaskStr = ((String) V.get(0))
							.toLowerCase();
					long dependencyMask = 0;
					List<String> subLocs = CMParms.parseAny(dependencyMaskStr,
							'|', true);
					for (int s = 0; s < subLocs.size(); s++) {
						int idx = CMParms.indexOf(DEFAULT_WORN_DESCS, subLocs
								.get(s).toLowerCase());
						if (idx >= 0)
							dependencyMask |= DEFAULT_WORN_CODES[idx];
						else
							Log.errOut("Wearable",
									"Bad dependency mask in coffeemud.ini file: "
											+ subLocs.get(s).toLowerCase());
					}
					...
					if (type.equalsIgnoreCase("ADD"))
						add(stat, dependencyMask, armorStrength, wornOrder,
								clothWeight, leatherWeight, metalWeight);
					else if (type.equalsIgnoreCase("REPLACE")
							&& (oldLocationCodeIndex >= 0))
						replace(oldLocationCodeIndex, stat, dependencyMask,
								armorStrength, wornOrder, clothWeight,
								leatherWeight, metalWeight);
				}
			}
		}
		...
		/**
		 * Returns a comma-delimited list of location names represented by the
		 * given worn code.
		 * 
		 * @param wornCode
		...
		public synchronized void add(String desc, long dependencyMask,
				double armorStrength, int wornOrder, double clothWeight,
				double leatherWeight, double metalWeight) {
			...
			dependencyMasks = Arrays.copyOf(dependencyMasks,
					dependencyMasks.length + 1);
			...
			wornWeightPoints = Arrays.copyOf(wornWeightPoints,
					wornWeightPoints.length + 1);
		...
		public synchronized void replace(int codeIndex, String desc,
				long dependencyMask, double armorStrength, int wornOrder,
