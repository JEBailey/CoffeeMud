 ...
 */
public class StdCompBattery extends StdElecCompItem implements
				...
				if (activated()
						&& ((Math.random() > super.getInstalledFactor())
								|| (Math.random() > super
										...
										.getReliabilityPct()) || ((subjectToWearAndTear())
								&& (usesRemaining() <= 100) && (Math.random() > CMath
								.div(usesRemaining(), 100))))
						&& (Math.random() > 0.9)) {
					Room R = CMLib.map().roomLocation(this);
					if (R != null) {
						// malfunction!
						CMMsg msg2 = CMClass.getMsg(msg.source(), this, null,
