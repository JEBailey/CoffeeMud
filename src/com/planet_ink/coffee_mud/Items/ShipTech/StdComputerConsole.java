 ...
 */
public class StdComputerConsole extends StdRideable implements ShipComponent,
		Electronics.Computer, Electronics.ElecPanel {
	public String ID() {
	...
	public StdComputerConsole() {
		...
		basePhyStats.setSensesMask(basePhyStats.sensesMask()
				| PhyStats.SENSE_ITEMREADABLE | PhyStats.SENSE_ITEMNOTGET);
	...
	public boolean canContain(Environmental E) {
		return (E instanceof Software)
				&& (((Software) E).getTechType() == TechType.SHIP_SOFTWARE);
	}
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_POWERCURRENT: {
				...
				if (activated()) {
					final List<Software> software = getSoftware();
					CMMsg msg2 = CMClass.getMsg(msg.source(), null, null,
							CMMsg.NO_EFFECT, null, CMMsg.MSG_POWERCURRENT,
							null, CMMsg.NO_EFFECT, null);
					synchronized (software) {
						if ((Math.random() < getFinalManufacturer()
								.getReliabilityPct() * getInstalledFactor())
								&& (subjectToWearAndTear() && (Math.random() < CMath
										.div(usesRemaining(), 100)))) {
							for (Software sw : software) {
								msg2.setTarget(sw);
								msg2.setValue(((powerToGive > 0) ? 1 : 0)
										+ (this.getActiveMenu().equals(
