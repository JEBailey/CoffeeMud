...
public class LifeScanProgram extends GenSoftware {
	...
	public int getScanMsg(Room R, Set<Room> roomsDone, String dirBuilder,
			int depthLeft, CMMsg scanMsg, StringBuilder str) {
		...
		for (int i = 0; i < R.numItems(); i++) {
			...
			if ((I instanceof SpaceShip) && (depthLeft > 0)) {
				...
				if (shipR != null)
					numFound += getScanMsg(shipR, roomsDone, dirBuilder + 'S',
							depthLeft - 1, scanMsg, str);
			}
		}
		if (depthLeft > 0) {
			...
			for (int d = 0; d < Directions.NUM_DIRECTIONS(); d++) {
				...
				numFound += getScanMsg(R2, roomsDone, dirBuilder + dirBCode
						+ ((char) ('a' + d)), depthLeft - 1, scanMsg, str);
			}
		}
		return numFound;
	}
	...
	public String getScanMsg() {
		...
		int numFound = getScanMsg(R, new HashSet<Room>(), "", phyStats()
				.weight() + 1, getScanMsg(R), str);
		if (activated)
			super.setCurrentScreenDisplay("LIFESCAN: Activated: " + numFound
					+ " found last scan.\n\r");
		else
			super.setCurrentScreenDisplay("LIFESCAN: Activate for continual scanning, type for on-demand.\n\r");
		if (str.length() == 0)
	...
	public boolean isActivationString(String word) {
		return "lifescan".startsWith(CMLib.english().getFirstWord(
				word.toLowerCase()));
	}
	...
	public void onActivate(MOB mob, String message) {
		...
		// TODO: lifescan for particular races? Is that a special version of
		// lifescan?
