...
public class StdClanFlag extends StdItem implements ClanItem {
	...
	public void setOwner(ItemPossessor E) {
		if ((E == null) && (super.owner != null) && (!amDestroyed())
				&& (CMSecurity.isDebugging(CMSecurity.DbgFlag.FLAGWATCHING))) {
			Log.debugOut("FLAGWATCH", name());
			Log.debugOut("FLAGWATCH", new Exception(name()
					+ " is being null-ownered."));
		}
		super.setOwner(E);
	}

	public void destroy() {
		if ((super.owner != null) && (!amDestroyed())
				&& (CMSecurity.isDebugging(CMSecurity.DbgFlag.FLAGWATCHING))) {
			Log.debugOut("FLAGWATCH", name());
			Log.debugOut("FLAGWATCH", new Exception(name()
					+ " is being destroyed."));
			...
			if ((msg.amITarget(this)) && (clanID().length() > 0)
				...
				if ((msg.targetMinor() == CMMsg.TYP_DROP)
					...
					if (s.length() > 0)
						...
						msg.source()
								.tell("This area is under the control of the Archons.");
					return;
				} else if ((msg.targetMinor() == CMMsg.TYP_SPEAK)
						&& (CMSecurity.isAllowed(msg.source(), R,
								CMSecurity.SecFlag.CMDROOMS))
						&& (msg.targetMessage() != null)) {
					String msgStr = CMStrings.getSayFromMessage(msg
							.targetMessage().toUpperCase());
			...
			if (msg.source().getClanRole(clanID()) == null) {
				if ((msg.targetMinor() == CMMsg.TYP_GET)
					...
					if (CMLib.clans().findRivalrousClan(msg.source()) == null) {
						msg.source()
								.tell("You must belong to an elligible clan to take a clan item.");
						return false;
					} else if (R != null) {
						for (int i = 0; i < R.numInhabitants(); i++) {
							...
							if ((M != null)
								...
								R.show(M, null, CMMsg.MSG_QUIETMOVEMENT,
										"<S-NAME> guard(s) " + name()
												+ " closely.");
								return false;
							}
						...
						if (msg.source().getClanRole(rulingClan) == null) {
							msg.source()
									.tell("You must conquer and fully control this area to take the clan flag.");
							return false;
						}
					...
					if ((T == null)
							|| ((!T.getOwnerName().equals(clanID()))
									&& ((!T.getOwnerName().equals(
											msg.source().getLiegeID())) || (!msg
											.source().isMarriedToLiege())) && (!T
						...
						if (!ok) {
							msg.source()
									.tell("You can not place a flag here, this place is controlled by the Archons.");
							return false;
						}
					}
				}
				...
				if (!foundOne) {
					msg.source().tell(
							"You are guarding " + name() + " too closely.");
					return false;
				}
			}
		}
		...
		if (StdClanItem.stdOkMessage(this, msg)) {
			if ((clanID().length() > 0) && (msg.amITarget(this))
				...
				if (rulingClan.length() == 0)
					msg.source().tell(
							"Area '" + msg.source().location().getArea().name()
									+ "' is presently neutral.");
				else {
					msg.source().tell(
							"Area '" + msg.source().location().getArea().name()
									+ "' is presently controlled by "
									+ rulingClan + ".");
					if (!rulingClan.equals(clanID())) {
						...
						if (relation != Clan.REL_WAR) {
							msg.source()
									.tell("You must be at war with this clan to put down your flag on their area.");
							return false;
						}
					}
				}
