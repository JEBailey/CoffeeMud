...
public class StdClanCommonItem extends StdClanItem {
	...
	public List enCode(MOB M, String req) {
		...
		if ((M.location() != null)
				...
				&& (resourceHere(M.location(), RawMaterial.MATERIAL_WOODEN)
						.size() == 0))
	...
	public void affectPhyStats(Physical affected, PhyStats stats) {
		...
		if ((glows)
				...
				&& (((MOB) affected).getStartRoom().getArea() == ((MOB) affected)
						.location().getArea()))
			...
			if (M.fetchEffect(readableText()) == null) {
				...
				if ((A != null)
					...
					if (((A.classificationCode() & Ability.ALL_DOMAINS) != Ability.DOMAIN_CRAFTINGSKILL)
						...
						DVector DV = (DVector) needChart.get(M.location()
								.getArea());
						if (DV != null) {
							List needs = null;
							MOB M2 = null;
							...
							if (A.ID().equalsIgnoreCase("FireBuilding")) {
								...
								for (int i = 0; i < DV.size(); i++) {
									...
									if ((needs != null)
											...
											&& (needs
													.contains(Integer
															.valueOf(RawMaterial.MATERIAL_METAL)))
											&& (!fireHere(M2.location()))
											&& (resourceHere(M2.location(),
													RawMaterial.MATERIAL_WOODEN)
													.size() > 0)) {
										if (M.location() == M2.location()) {
											getToWork = true;
											break;
										} else if ((possibleMOBToGoTo == null)
												|| (CMLib.dice().roll(1, 2, 0) == 1))
											possibleMOBToGoTo = M2;
									}
								}
								if ((!getToWork) && (possibleMOBToGoTo != null)
					...
					if ((M.location() != null)
						...
						case 5:
							CMLib.commands().postSay(M, null,
									"Is anyone there?  I can't see!");
							break;
						case 6:
							CMLib.commands()
									.postSay(M, null,
											"Someone turn on the lights to I can work!");
							break;
						case 7:
							CMLib.commands()
									.postSay(M, null,
											"I could use some light, if you expect me to work.");
							break;
						}
					...
					if ((M.numItems() > 1)
						...
						if (I != null)
							V.addElement(I.name());
						success = A.invoke(M, V, null, false, phyStats()
								.level());
					} else
						success = A.invoke(M, new Vector(), null, false,
								phyStats().level());
					if ((A.classificationCode() & Ability.ALL_DOMAINS) == Ability.DOMAIN_CRAFTINGSKILL) {
						DVector DV = (DVector) needChart.get(M.location()
								.getArea());
						if (!success) {
							...
							if (reqIndex > 0)
								DV.addElement(M,
										enCode(M, req.substring(reqIndex + 1)));
							else
