...
public class ArchonStaff extends Staff implements Wand, MiscMagic, ArchonOnly {
	...
	public boolean safetyCheck(MOB mob, String message) {
		if ((!mob.isMonster())
				...
				&& (CMParms.combine(mob.session().getPreviousCMD(), 0)
						.toUpperCase().indexOf(message.toUpperCase()) < 0)) {
	...
	public void waveIfAble(MOB mob, Physical afftarget, String message) {
		if ((mob.isMine(this)) && (!this.amWearingAt(Wearable.IN_INVENTORY))) {
			if ((mob.location() != null) && (afftarget != null)
				...
				if (message.toUpperCase().indexOf("LEVEL ALL UP") > 0) {
				...
				} else if (message.toUpperCase().startsWith("LEVEL ")
						&& message.toUpperCase().endsWith(" UP")) {
					if (!safetyCheck(mob, message))
						return;
					...
					return;
						...
						&& message.toUpperCase().endsWith(" DOWN")) {
					if (!safetyCheck(mob, message))
						return;
					message = message.substring(6).trim();
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.source().location() != null)
			...
			CMMsg msg2 = CMClass.getMsg(msg.source(), msg.target(),
					new ArchonStaff(), CMMsg.MSG_OK_ACTION,
					CMMsg.MSK_MALICIOUS_MOVE | CMMsg.TYP_FIRE,
					...
					if (!((MOB) msg.target()).amDead())
						CMLib.combat()
								.postDamage(
										...
										Weapon.TYPE_BURNING,
										name()
												+ " shoots a flame which <DAMAGE> <T-NAME>!");
				}
			}
		}
	}
