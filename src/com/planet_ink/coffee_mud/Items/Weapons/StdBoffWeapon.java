...
public class StdBoffWeapon extends StdWeapon {
	...
	public String hitString(int damageAmount) {
		...
		boolean showDamn = CMProps.getVar(CMProps.Str.SHOWDAMAGE)
				.equalsIgnoreCase("YES");
		switch (weaponClassification()) {
		case Weapon.CLASS_RANGED:
			return "<S-NAME> fire(s) " + name() + " at <T-NAMESELF> and "
					+ word + ((showDamn) ? " (" + damageAmount + ")" : "")
					+ " <T-HIM-HER>." + CMLib.protocol().msp("arrow.wav", 20);
		case Weapon.CLASS_THROWN:
			return "<S-NAME> throw(s) " + name() + " at <T-NAMESELF> and "
					+ word + ((showDamn) ? " (" + damageAmount + ")" : "")
					+ " <T-HIM-HER>." + CMLib.protocol().msp("arrow.wav", 20);
		default:
			return "<S-NAME> "
					+ word
					+ ((showDamn) ? " (" + damageAmount + ")" : "")
					+ " <T-NAMESELF> with "
					+ name()
					+ "."
					+ CMLib.protocol().msp(
