...
public class HolyAvenger extends TwoHandedSword {
	...
	public HolyAvenger() {
		...
		basePhyStats().setDisposition(
				basePhyStats().disposition() | PhyStats.IS_GOOD
						| PhyStats.IS_BONUS);
		baseGoldValue = 15500;
		recoverPhyStats();
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(this))
			...
			case CMMsg.TYP_GET:
				if ((!msg.source().charStats().getCurrentClass().ID()
						.equals("Paladin"))
					...
					mob.location()
							.show(mob,
									...
									CMMsg.MSG_OK_ACTION,
									name()
											+ " flashes and flies out of <S-HIS-HER> hands!");
					if (msg.source().isMine(this))
						CMLib.commands().postDrop(msg.source(), this, true,
								false, false);
					return false;
				}
				break;
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.source().location() != null)
			...
			CMMsg msg2 = CMClass.getMsg(msg.source(), msg.target(),
					new HolyAvenger(), CMMsg.MSG_OK_ACTION,
			...
			if (msg.source().location().okMessage(msg.source(), msg2)) {
				...
				msg.addTrailerMsg(CMClass.getMsg(
						...
						CMMsg.MSG_OK_ACTION,
						name()
								+ " dispels evil within <T-NAME> and "
								+ CMLib.combat().standardHitWord(
										Weapon.TYPE_BURSTING, damage)
								+ " <T-HIM-HER>>!"));
				CMMsg msg3 = CMClass.getMsg(msg.source(), msg.target(), null,
						CMMsg.MSG_OK_VISUAL, CMMsg.MSG_DAMAGE, CMMsg.NO_EFFECT,
						null);
