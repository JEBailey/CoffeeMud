...
public class StdWeapon extends StdItem implements Weapon, AmmunitionWeapon {
		...
		if (amWearingAt(Wearable.WORN_WIELD)) {
			if (phyStats().attackAdjustment() != 0)
				affectableStats.setAttackAdjustment(affectableStats
						.attackAdjustment() + (phyStats().attackAdjustment()));
	...
	public void recoverPhyStats() {
		...
		if (phyStats().damage() != 0) {
			final int ability = super.wornLogicalAnd ? (phyStats().ability() * CMath
					.numberOfSetBits(super.myWornCode)) : phyStats().ability();
			phyStats().setDamage(phyStats().damage() + (ability * 2));
			phyStats().setAttackAdjustment(
					phyStats().attackAdjustment() + (ability * 10));
		}
		if ((subjectToWearAndTear()) && (usesRemaining() < 100))
			phyStats().setDamage(
					((int) Math.round(CMath.mul(phyStats().damage(),
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(this)) {
			...
			case CMMsg.TYP_RELOAD:
				if (msg.tool() instanceof Ammunition) {
					...
					for (final Enumeration<Ability> a = effects(); a
							.hasMoreElements();) {
		...
		if ((msg.targetMinor() == CMMsg.TYP_DAMAGE) && (msg.tool() == this)
				&& (amWearingAt(Wearable.WORN_WIELD))
				&& (weaponClassification() != Weapon.CLASS_NATURAL)
				&& (weaponType() != Weapon.TYPE_NATURAL)
				&& (msg.target() instanceof MOB) && ((msg.value()) > 0)
				&& (owner() instanceof MOB) && (msg.amISource((MOB) owner()))) {
			...
			if ((hurt > (tmob.maxState().getHitPoints() / 10) || (hurt > 50))
				...
				if ((!tmob.isMonster())
						...
						&& (CMLib.dice().rollPercentage() > (tmob.charStats()
								.getStat(CharStats.STAT_CONSTITUTION) * 4))
						&& (!CMSecurity
			...
			if ((subjectToWearAndTear())
				...
				CMLib.combat().postItemDamage(ownerM, this, null, 1,
						CMMsg.TYP_JUSTICE, null);
			}
		}
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.targetMinor() == CMMsg.TYP_WEAPONATTACK)
				&& (msg.tool() == this) && (requiresAmmunition())
			...
			if (ammunitionRemaining() <= 0) {
				if (lastReloadTime != msg.source().lastTickedDateTime()) {
					msg.source().tell(
							name() + " is out of " + ammunitionType() + ".");
					if ((msg.source().isMine(this))
							&& (msg.source().location() != null)
							&& (CMLib.flags().aliveAwakeMobile(msg.source(),
									true))) {
						lastReloadTime = msg.source().lastTickedDateTime();
						if ((!msg.source().isMonster())
								|| inventoryAmmoCheck(msg.source()))
							msg.source().enqueCommand(
									CMParms.parse("LOAD ALL \"$" + name()
											+ "$\""), 0, 0);
						else
							msg.source()
									.enqueCommand(
											CMParms.parse("REMOVE \"$" + name()
													+ "$\""), 0, 0);
					}
				}
				return false;
	...
	protected boolean inventoryAmmoCheck(MOB M) {
		...
		for (int i = 0; i < M.numItems(); i++) {
			...
			if ((I != null)
					&& (I instanceof Ammunition)
					&& (((Ammunition) I).ammunitionType()
							.equalsIgnoreCase(ammunitionType())))
				return true;
		}
		return false;
	}
	...
	protected String weaponHealth() {
		if (usesRemaining() >= 100)
			...
			case Weapon.CLASS_POLEARM:
			case Weapon.CLASS_SWORD:
				return name() + " has some notches and chinks ("
						+ usesRemaining() + "%)";
			default:
			...
			return name() + " is so damaged, it is practically harmless ("
					+ usesRemaining() + "%)";
	}

	public String missString() {
		return CMLib.combat().standardMissString(weaponType,
				weaponClassification, name(), useExtendedMissString);
	}

	public String hitString(int damageAmount) {
		return CMLib.combat().standardHitString(weaponType,
	...
	public boolean subjectToWearAndTear() {
		return ((!requiresAmmunition()) && (!(this instanceof Wand))
				&& (usesRemaining() <= 1000) && (usesRemaining() >= 0));
	}
