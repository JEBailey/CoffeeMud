...
public class SMTPserver extends Thread implements Tickable {
	...
	public static final String ServerVersionString = "CoffeeMud SMTPserver/"
	...
	public SMTPserver() {
		...
		mud = null;
		isOK = false;
		threadPool = new CMThreadPoolExecutor("SMTP", 0, 3, 30,
				TimeUnit.SECONDS, 5, 256);
	...
	public SMTPserver(MudHost a_mud) {
		...
		if (!initServer())
			...
			isOK = true;
		threadPool = new CMThreadPoolExecutor("SMTP", 0, maxThreads, 30,
				TimeUnit.SECONDS, threadTimeoutMins, 256);
		setDaemon(true);
	}
	...
	protected boolean initServer() {
		...
		CMProps.setBoolVar(CMProps.Bool.EMAILFORWARDING,
				CMath.s_bool(page.getStr("FORWARD")));
		...
		if ((CMProps.getListVar(CMProps.StrList.SUBSCRIPTION_STRS) == null)
			...
			String[] msgs = new String[] { page.getStr("SUBSCRIBEDTITLE"),
	...
	public TreeMap<String, JournalsLibrary.SMTPJournal> parseJournalList(
			String journalStr) {
			...
			if (V.size() > 0) {
				for (int v = 0; v < V.size(); v++) {
					...
					set.put(s.toUpperCase().trim(),
							new JournalsLibrary.SMTPJournal(s, forward,
									subscribeOnly, keepAll, crit.toString()
											.trim()));
				}
			}
		}
	...
	public void run() {
		...
		if ((page == null) || (!page.isLoaded())) {
			Log.errOut(getName(),
					"ERROR: SMTPserver will not run with no properties. Shutting down.");
			isOK = false;
			return;
		}
		...
		if (page.getStr("BIND") != null && page.getStr("BIND").length() > 0) {
			try {
				...
				Log.errOut(getName(), "ERROR: Could not bind to address "
						+ page.getStr("BIND"));
			}
		}
		...
		try {
			...
			// if we've been interrupted, servsock will be null and serverOK
			// will be true
			Log.errOut(getName(), e.getMessage());
			// this prevents initHost() from running if run() has failed (eg
			// socket in use)
			if (!serverOK)
				isOK = false;
		}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((tickID == Tickable.TICKID_READYTOSTOP)
				|| (tickID == Tickable.TICKID_EMAIL)) {
			MassMailer massMailer = new MassMailer(page, domain,
					oldEmailComplaints);

			TreeMap<String, JournalsLibrary.SMTPJournal> set = getJournalSets();
			long lastAllProcessing = 0;
			if (Resources.isPropResource("SMTP", "LASTALLPROCESING"))
				lastAllProcessing = CMath.s_long(Resources.getPropResource(
						"SMTP", "LASTALLPROCESING"));
			...
			for (JournalsLibrary.SMTPJournal smtpJournal : set.values()) {
				...
				if (smtpJournal.forward) {
					// Vector mailingList=?
					final List<JournalsLibrary.JournalEntry> msgs = CMLib
							.database().DBReadJournalMsgsNewerThan(journalName,
									"ALL", lastAllProcessing - 1);
					for (final JournalsLibrary.JournalEntry msg : msgs) {
						// if(msg.to.equalsIgnoreCase("ALL")) // implied by the
						// query
						...
						if ((subj.equalsIgnoreCase("subscribe"))
							...
							CMLib.database().DBDeleteJournal(journalName,
									msg.key);
							updatedMailingLists = CMLib.journals()
									.subscribeToJournal(journalName, msg.from,
											false)
									|| updatedMailingLists;
							...
							CMLib.database().DBDeleteJournal(journalName,
									msg.key);
							updatedMailingLists = CMLib.journals()
									.unsubscribeFromJournal(journalName,
											msg.from, false)
									|| updatedMailingLists;
						} else {
							if (CMProps
									.getBoolVar(CMProps.Bool.EMAILFORWARDING)) {
								String jrnlSubj;
								if (msg.subj.indexOf("[" + journalName + "]") < 0) {
									if (msg.subj.startsWith("RE: "))
										jrnlSubj = "RE: [" + journalName + "] "
												+ msg.subj.substring(4);
									else
										jrnlSubj = "[" + journalName + "] "
												+ msg.subj;
								} else
									jrnlSubj = msg.subj;
								...
								if (jrnlMessage.startsWith("<HTML><BODY>")) {
									...
									if (jrnlMessage.indexOf("<HTML>") > 0) {
										jrnlMessage = CMStrings.replaceAll(
												jrnlMessage, "<HTML>", "");
										jrnlMessage = CMStrings.replaceAll(
												jrnlMessage, "</HTML>", "");
										jrnlMessage = CMStrings.replaceAll(
			...
			Resources.setPropResource("SMTP", "LASTALLPROCESSING",
					Long.toString(nextAllProcessing));
			...
			if ((tickID == Tickable.TICKID_EMAIL)
				...
				if ((mailboxName() != null) && (mailboxName().length() > 0)) {
					List<JournalsLibrary.JournalEntry> emails = CMLib
							.database().DBReadJournalMsgs(mailboxName());
					if (emails != null)
						for (JournalsLibrary.JournalEntry mail : emails) {
							if ((mail.data.length() > 0)
									&& (isAForwardingJournal(mail.data)))
								massMailer.addMail(mail, mailboxName(),
										mail.data, true);
							else
								massMailer.addMail(mail, mailboxName(), null,
										true);
						}
				}
			}
			...
			new Thread(Thread.currentThread().getThreadGroup(), massMailer,
					"MassMailer"
