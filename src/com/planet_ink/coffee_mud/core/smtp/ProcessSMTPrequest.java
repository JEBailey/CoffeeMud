...
public class ProcessSMTPrequest implements Runnable {
	...
	public String validLocalAccount(String s, boolean checkFROMcase) {
		...
		if (x > 0) {
			...
			if (!domain.toUpperCase().endsWith(
					server.domainName().toUpperCase())) {
				if (server.mailboxName().length() > 0) {
					...
					{
						final PlayerAccount A = CMLib.players()
								.getLoadAccountByEmail(s);
						if (A != null)
	...
	public MOB getAccountMob(String s) {
		...
		if (CMLib.players().accountExists(s)) {
			...
			if (A.numPlayers() == 0)
				...
				ThinPlayer tP = null;
				for (Enumeration<ThinPlayer> e = A.getThinPlayers(); e
						.hasMoreElements();) {
	...
	public void run() {
		...
		try {
			...
			final String initialMsg = "220 ESMTP " + server.domainName() + " "
					+ SMTPserver.ServerVersionString + "; "
			...
			while (!quitFlag) {
				...
				while (!quitFlag) {
					...
					if (c < 0) {
						if (debug)
							Log.debugOut(runnableName,
									"Internal: EOF observed.");
						throw new IOException("reset by peer");
					}
					if (c == 65535) {
						final long ellapsed = System.currentTimeMillis()
								- timeSinceLastChar;
						if (ellapsed > IDLE_TIMEOUT) {
							quitFlag = true;
							if (debug)
								Log.debugOut(runnableName,
										"Internal: generated timeout: "
												+ msgsSent + " msgs sent");
							break;
						} else
							break;
					...
					if (input.length() > server.getMaxMsgSize()) {
						if (debug) {
							...
							Log.debugOut(runnableName,
									"Internal: 552 String exceeds size limit ("
											+ server.getMaxMsgSize() + "): "
											+ str.toString());
						}
						// Log.errOut("SMTPR","Long request from "+sock.getInetAddress());
						sout.write("552 String exceeds size limit. You are very bad!"
								+ cr);
						sout.flush();
						quitFlag = true;
						input.setLength(0);
				...
				while (cmdQueue.size() > 0) {
					...
					if ((dataMode) && (s.equals("."))) {
						...
						/*
						 * When the SMTP server accepts a message either for
						 * relaying or for final delivery, it inserts a trace
						 * record (also referred to interchangeably as a
						 * "time stamp line" or "Received" line) at the top of
						 * the mail data. This trace record indicates the
						 * identity of the host that sent the message, the
						 * identity of the host that received the message (and
						 * is inserting this time stamp), and the date and time
						 * the message was received.
						 */
						if (data.length() >= server.getMaxMsgSize())
							replyData = ("552 Message exceeds size limit." + cr)
									.getBytes();
						else {
							replyData = ("250 Message accepted for delivery." + cr)
									.getBytes();
							msgsSent++;
							...
							int boundryState = -1;
							try {
								BufferedReader lineR = new BufferedReader(
								...
								while (true) {
									...
									if (debug)
										Log.debugOut(runnableName,
												"Header State=" + boundryState
														+ ", " + s2);

									if ((startBuffering) && (boundry != null)
											&& (s2.indexOf(boundry) >= 0)) {
										if (debug)
											Log.debugOut(runnableName,
													"Multipart boundary "
															+ boundry
										...
										if (debug)
											Log.debugOut(
													...
													"Boundary "
															+ boundry
															+ " spotted -- state is now 2.");
										if (s2.indexOf(boundry) >= 0)
											boundryState = 2;
										continue;
									} else if ((s2u.startsWith("SUBJECT: "))
											&& (boundryState < 2)) {
										...
										if (s2u.substring(27).trim()
												.startsWith("QUOTED-PRINTABLE")
												|| s2u.substring(27)
														.trim()
														.startsWith(
																"QUOTED PRINTABLE"))
										...
										if (debug)
											...
											.startsWith("CONTENT TYPE: ") || s2u
											.startsWith("CONTENT-TYPE: "))) {
										if ((boundryState < 0)
												&& (s2u.substring(14).trim()
														.startsWith("MULTIPART/"))) {
											String contentType = s2.substring(
													14).trim();
											int y = s2u.indexOf(';');
											if (y >= 0)
												contentType = s2.substring(14,
														y).trim();
											...
											if (x < 0) {
												replyData = ("552 Message content type '"
														+ contentType
														+ "' not accepted without boundry." + cr)
														.getBytes();
												subject = null;
												break;
											}
											if (s2 != null) {
												boundry = s2.substring(x + 9)
														.trim();
												y = s2.indexOf(';');
												if (y >= 0)
													s2 = s2.substring(0, y)
															.trim();
												if (boundry.startsWith("\"")
														&& boundry
																.endsWith("\""))
													boundry = boundry
															.substring(
							...
							if ((replyData != null)
									&& (new String(replyData).startsWith("250"))) {
								if ((finalData.length() == 0)
										&& (!startBuffering)) {
									finalData = new StringBuilder(
											data.toString());
								...
								if (dataBlocks.containsKey(Character
										.valueOf('h'))) {
									bodyType = 'h';
									finalData = dataBlocks.get(Character
											.valueOf('h'));
								} else if (finalData.toString().trim().length() == 0) {
									if (dataBlocks.size() > 0) {
										bodyType = dataBlocks.keySet()
												.iterator().next().charValue();
										finalData = dataBlocks.get(Character
												.valueOf(bodyType));
									}
								}
								...
								if (subject != null) {
									...
									for (int i = 0; i < to.size(); i++) {
										String journal = server
												.getAnEmailJournal(to
														.elementAt(i));
										if (journal != null) {
											String parentKey = "";
											String fdat = finalData.toString()
													.trim();
											if ((subject != null)
													&& (!subject
															.trim()
													...
													&& (!fdat
															...
															.equalsIgnoreCase(
																	"unsubscribe"))
													&& (server
															.isAForwardingJournal(journal))) {
												if (server
														.isASubscribeOnlyJournal(journal)) {
													if ((fromM == null)
															|| (!CMSecurity
																	.isAllowedAnywhere(
																			fromM,
																			CMSecurity.SecFlag.JOURNALS))) {
														replyData = ("552 Mailbox '"
																+ journal
																+ "' only accepts subscribe/unsubscribe." + cr)
																.getBytes();
														break;
													}
												} else {
													Map<String, List<String>> lists = Resources
															.getCachedMultiLists(
																	"mailinglists.txt",
																	true);
													...
													if ((mylist == null)
														...
														if (debug)
															Log.debugOut(
																	runnableName,
																	from
																			+ " is not in mailing list for journal "
																			+ journal);
														replyData = ("552 Mailbox '"
																+ journal
																+ "' only accepts messages from subscribers.  Send an email with 'subscribe' as the subject." + cr)
																.getBytes();
														break;
													}
													JournalsLibrary.ForumJournal forum = CMLib
															.journals()
															.getForumJournal(
																	journal);
													if ((forum != null)
														...
														if (realSubject
															...
															realSubject = realSubject
																	.substring(
																			journal.length() + 2)
																	.trim();
														List<JournalsLibrary.JournalEntry> entries = CMLib
																.database()
											...
											if (debug)
												Log.debugOut(runnableName,
														"Written: " + journal
																...
																+ "/ALL/"
																+ bodyType);
											StringBuilder finalFinalData = new StringBuilder(
													finalData);
											if (bodyType == 'h')
												cleanHtml(journal,
														finalFinalData);
													...
													.DBWriteJournalChild(
															...
															CMLib.coffeeFilter()
																	.simpleInFilter(
																			finalData)
																	.toString());
										} else if (finalData.toString().trim()
												.length() > 0) {
											if (debug)
												Log.debugOut(
														runnableName,
						...
						if (parm.length() == 0) {
							replyData = ("214-This is "
									...
									+ cr
									+ "214-For local information send email to your local Archon."
									+ cr + "214 End of HELP info" + cr)
									.getBytes();
						} else if (parm.equals("NOOP")) {
							replyData = ("214-NOOP" + cr
									+ "214-    Do nothing." + cr
									+ "214 End of HELP info" + cr).getBytes();
						} else if (parm.equals("HELO")) {
							replyData = ("214-HELO <hostname>" + cr
							...
							replyData = ("214-MAIL FROM: <sender> [ <parameters> ]"
									+ cr
							...
							replyData = ("214-VERB" + cr
									+ "214-    Not implemented in this server."
									+ cr + "214 End of HELP info" + cr)
									.getBytes();
						} else if (parm.equals("ETRN")) {
							replyData = ("214-ETRN [ <hostname> | @<domain> | #<queuename> ]"
									+ cr
									+ "214-    Not implemented in this server."
									+ cr + "214 End of HELP info" + cr)
									.getBytes();
						} else if (parm.equals("DSN")) {
							replyData = ("214-MAIL FROM: <sender> [ RET={ FULL | HDRS} ] [ ENVID=<envid> ]"
									+ cr
							...
							replyData = ("214-RCPT TO: <recipient> [ <parameters> ]"
									+ cr
						...
						if ((domain != null) && (parm.trim().length() == 0))
							replyData = ("503 "
									...
									+ " Duplicate HELO/EHLO" + cr).getBytes();
						else if (parm.trim().length() == 0)
							replyData = ("501 " + cmd
									+ " requires domain address" + cr)
									.getBytes();
						else {
							domain = parm;
							if (cmd.equals("EHLO")) {
								replyData = ("250-" + server.domainName()
										+ " Ok." + cr + "250-8BITMIME" + cr
										+ "250-SIZE " + server.getMaxMsgSize()
										+ cr + "250-HELP" + cr + "250-ONEX"
										+ cr + "250-PIPELINING" + cr
										+ "250 DSN" + cr).getBytes();
							} else
								replyData = ("250 "
										+ sock.getLocalAddress().getHostName()
										...
										+ sock.getInetAddress()
												.getHostAddress()
										+ "], pleased to meet you" + cr)
										.getBytes();
						}
					} else if (cmd.equals("MAIL")) {
						int x = parm.indexOf(':');
						if (x < 0)
							replyData = ("501 Syntax error in \"" + parm + "\"" + cr)
									.getBytes();
						else {
							String to2 = parm.substring(0, x).trim();
							if (!to2.equalsIgnoreCase("from"))
								replyData = ("500 Unrecognized command \""
										+ cmd + "\"" + cr).getBytes();
							else {
								...
								if (parm.startsWith("<")) {
									x = parm.indexOf('>');
									if (x < 0) {
										replyData = ("501 Syntax error in \""
												+ parm + "\"" + cr).getBytes();
										error = true;
									} else {
										parmparms = parm.substring(x + 1)
												.trim();
								...
								if (parmparms.trim().length() > 0) {
									if ((parmparms.trim().toUpperCase()
											.startsWith("SIZE="))
											|| (!CMath.isNumber(parmparms
													.trim().toUpperCase()
													.substring(5)))) {
										int size = CMath.s_int(parmparms.trim()
												.toUpperCase().substring(5));
										if (size > server.getMaxMsgSize()) {
											replyData = ("552 String exceeds size limit. But you were nice to tell me!" + cr)
													.getBytes();
											error = true;
										}
									} else {
										replyData = ("502 Parameters not supported... \""
												+ parmparms + "\"" + cr)
												.getBytes();
										error = true;
									}
								...
								if (!error) {
									...
									if (name == null) {
										if ((++failures) == 3) {
											replyData = ("421 Quit Fishing!" + cr)
													.getBytes();
											quitFlag = true;
										} else
											replyData = ("551 Requested action not taken: User is not local." + cr)
													.getBytes();
									} else {
										replyData = ("250 OK " + name + cr)
												.getBytes();
										from = name;
						...
						if (from == null)
							replyData = ("503 Need MAIL command" + cr)
									.getBytes();
						else if (to == null)
							replyData = ("503 Need RCPT (recipient)" + cr)
									.getBytes();
						else {
							replyData = ("354 Enter mail, end with \".\" on a line by itself" + cr)
									.getBytes();
							dataMode = true;
						}
					} else if (cmd.equals("QUIT")) {
						replyData = ("221 " + server.domainName()
								+ " closing connection" + cr).getBytes();
						quitFlag = true;
					} else if (cmd.equals("VRFY"))
						replyData = ("252 Cannot VRFY user; try RCPT to attempt delivery (or try finger)" + cr)
								.getBytes();
					else if (cmd.equals("EXPN"))
						replyData = ("502 Sorry, we don't allow mailing lists" + cr)
								.getBytes();
					else if (cmd.equals("VERB"))
						replyData = ("502 Verbose unavailable" + cr).getBytes();
					else if (cmd.equals("ETRN"))
						replyData = ("502 ETRN not implemented" + cr)
								.getBytes();
								...
								if (parmparms.trim().length() > 0) {
									...
									if (name == null) {
										if ((++failures) == 3) {
											...
											replyData = ("553 Requested action not taken: User is not local." + cr)
													.getBytes();
									} else {
										if (server.getAnEmailJournal(name) != null) {
											...
											if (server.getJournalCriteria(name) != null) {
												if (from == null) {
													replyData = ("503 Need MAIL before RCPT" + cr)
															.getBytes();
													jerror = true;
												} else {
													if ((fromM == null)
															|| (!CMLib
																	.masking()
																	.maskCheck(
																			server.getJournalCriteria(name),
																			fromM,
																			false))) {
														replyData = ("552 User '"
																+ from
																+ "' may not send emails to '"
																+ name + "'." + cr)
																.getBytes();
														jerror = true;
													}
										...
										} else if (CMLib.database()
												.DBCountJournal(
														server.mailboxName(),
														null, name) >= server
												.getMaxMsgs())
											replyData = ("552 Mailbox '" + name
													+ "' is full." + cr)
													.getBytes();
										else {
											replyData = ("250 OK " + name + cr)
					...
					if ((replyData != null)) {
						...
						if ((cmdQueue.size() == 0) && (respQueue.size() > 0)) {
							// we should be looping through these .. why does ZD
							// act so wierd?!
			...
			try {
				if (sout != null) {
					sout.write("421 You're taking too long.  I'm outa here."
							+ cr);
					sout.flush();
				}
			} catch (Exception e) {
			...
			final StringBuilder msg = new StringBuilder(
					errorMessage == null ? "EMPTY e.getMessage()"
							: errorMessage);
			final StackTraceElement[] ts = e.getStackTrace();
			if (ts != null)
				for (StackTraceElement t : ts)
					msg.append(" ").append(t.getFileName()).append("(")
							.append(t.getLineNumber()).append(")");
