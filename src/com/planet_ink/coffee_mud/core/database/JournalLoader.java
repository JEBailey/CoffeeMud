...
public class JournalLoader {
	...
	public int DBCount(String Journal, String from, String to) {
		...
		synchronized (Journal.toUpperCase().intern()) {
			...
			try {
				D = DB.DBFetch();
				ResultSet R = D
						.query("SELECT CMFROM,CMTONM FROM CMJRNL WHERE CMJRNL='"
								+ Journal + "'");
				while (R.next()) {
					if ((from != null)
							&& (!from.equalsIgnoreCase(DBConnections.getRes(R,
									"CMFROM"))))
						continue;
					if ((to != null)
							&& (!to.equalsIgnoreCase(DBConnections.getRes(R,
									"CMTONM"))))
						continue;
					ct++;
				}
	...
	public String DBGetRealName(String possibleName) {
		...
		try {
			...
			ResultSet R = D.query("SELECT CMJRNL FROM CMJRNL WHERE CMJRNL='"
					+ possibleName + "'");
			if (R.next()) {
	...
	public long[] DBJournalLatestDateNewerThan(String Journal, String to,
		...
		try {
			...
			String str = "SELECT CMUPTM FROM CMJRNL WHERE CMJRNL='" + Journal
					+ "' AND CMUPTM > " + olderTime;
			if (to != null)
				str += " AND CMTONM='" + to + "'";
	...
	public synchronized List<String> DBReadJournals() {
		...
		try {
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT CMJRNL FROM CMJRNL"); // add unique to
																// fakedb
			while (R.next()) {
	...
	protected JournalsLibrary.JournalEntry DBReadJournalEntry(ResultSet R) {
		...
		if ((subject.startsWith("MOTD")) || (subject.startsWith("MOTM"))
	...
	public Vector<JournalsLibrary.JournalEntry> DBReadJournalPageMsgs(
		...
		try {
			...
			if (searching)
				str += " AND (CMSUBJ LIKE '%" + searchStr
						+ "%' OR CMMSGT LIKE '%" + searchStr + "%')";
			str += " ORDER BY CMUPTM";
			...
			while (((cardinal < limit) || (limit == 0)) && R.next()) {
				...
				if ((parent != null)
						&& (CMath.bset(entry.attributes,
								JournalsLibrary.JournalEntry.ATTRIBUTE_STUCKY)))
					continue;
				if (searching) {
					long updated = entry.update;
					String parentKey = ((entry.parent != null) && (entry.parent
							.length() > 0)) ? entry.parent : entry.key;
			...
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CMJRNL))
				Log.debugOut("JournalLoader", "Query (" + journal.size()
						+ "): " + str);
			if ((journal.size() > 0) && (parent != null)) // set last entry --
															// make sure its not
															// stucky
			{
				...
				while (R.next()) {
					...
					if (CMath.bset(attributes,
							JournalsLibrary.JournalEntry.ATTRIBUTE_STUCKY))
						continue;
					journal.lastElement().isLastEntry = false;
					break;
				}
			} else if ((journal.size() > 0) && (!R.next()))
				journal.lastElement().isLastEntry = true;
		} catch (Exception sqle) {
			Log.errOut("Journal", sqle);
			return null;
	...
	public Vector<JournalsLibrary.JournalEntry> DBSearchAllJournalEntries(
		...
		try {
			...
			str += " AND (CMSUBJ LIKE '%" + searchStr + "%' OR CMMSGT LIKE '%"
					+ searchStr + "%')";
			str += " ORDER BY CMUPTM";
			str += " ASC";
	...
	public JournalsLibrary.JournalEntry DBReadJournalEntry(String Journal,
		...
		synchronized (Journal.toUpperCase().intern()) {
			...
			try {
				D = DB.DBFetch();
				String str = "SELECT * FROM CMJRNL WHERE CMJKEY='" + Key
						+ "' AND CMJRNL='" + Journal + "'";
				ResultSet R = D.query(str);
				if (R.next()) {
					JournalsLibrary.JournalEntry entry = DBReadJournalEntry(R);
	...
	public int getFirstMsgIndex(List<JournalEntry> journal, String from,
			String to, String subj) {
		from = DB.injectionClean(from);
		to = DB.injectionClean(to);
	...
	public void DBUpdateJournal(String key, String subject, String msg,
		...
		String sql = "UPDATE CMJRNL SET CMSUBJ=?, CMMSGT=?, CMATTR="
				+ newAttributes + " WHERE CMJKEY='" + key + "'";
		if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CMJRNL))
	...
	public void DBUpdateJournal(String Journal,
		...
		String sql = "UPDATE CMJRNL SET " + "CMFROM='" + entry.from + "' , "
				+ "CMDATE='" + entry.date + "' , " + "CMTONM='" + entry.to
				+ "' , " + "CMSUBJ=? ," + "CMPART='" + entry.parent + "' ,"
				+ "CMATTR=" + entry.attributes + " ," + "CMDATA='" + entry.data
				+ "' " + "WHERE CMJRNL='" + Journal + "' AND CMJKEY='"
				+ entry.key + "'";
		if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CMJRNL))
			Log.debugOut("JournalLoader", sql);
		DB.updateWithClobs(sql, entry.subj);

		sql = "UPDATE CMJRNL SET " + "CMUPTM=" + entry.update + ", "
				+ "CMIMGP='" + entry.msgIcon + "', " + "CMVIEW=" + entry.views
				+ ", " + "CMREPL=" + entry.replies + ", " + "CMMSGT=? "
	...
	public void DBTouchJournalMessage(String key, long newDate) {
		key = DB.injectionClean(key);
		String sql = "UPDATE CMJRNL SET CMUPTM=" + newDate + " WHERE CMJKEY='"
				+ key + "'";
		if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CMJRNL))
			Log.debugOut("JournalLoader", sql);
		DB.update(sql);
	...
	public void DBUpdateMessageReplies(String key, int numReplies) {
		key = DB.injectionClean(key);
		String sql = "UPDATE CMJRNL SET CMUPTM=" + System.currentTimeMillis()
				+ ", CMREPL=" + numReplies + " WHERE CMJKEY='" + key + "'";
		if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CMJRNL))
			Log.debugOut("JournalLoader", sql);
		DB.update(sql);
	}
	...
	public void DBViewJournalMessage(String key, int views) {
		key = DB.injectionClean(key);
		String sql = "UPDATE CMJRNL SET CMVIEW=" + views + " WHERE CMJKEY='"
				+ key + "'";
		if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CMJRNL))
			Log.debugOut("JournalLoader", sql);
		DB.update(sql);
	...
	public void DBDeletePlayerData(String name) {
		...
		try {
			...
			while (R.next()) {
				...
				if ((parent != null) && (parent.length() > 0))
					notifiableParentsV
							.add(new String[] { journalName, parent });
				deletableEntriesV.add(key);
			}
			for (String[] parentKey : notifiableParentsV) {
				if (!deletableEntriesV.contains(parentKey[0])) {
					JournalsLibrary.JournalEntry parentEntry = DBReadJournalEntry(
							parentKey[0], parentKey[1]);
					if (parentEntry != null)
						DBUpdateMessageReplies(parentEntry.key,
								parentEntry.replies - 1);
				}
			}
			for (String s : deletableEntriesV)
				D.update("DELETE FROM CMJRNL WHERE CMJKEY='" + s
						+ "' OR CMPART='" + s + "'", 0);
		} catch (Exception sqle) {
			Log.errOut("Journal", sqle);
		} finally {
	...
	public void DBUpdateJournalStats(String Journal,
		...
		try {
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMJRNL WHERE CMJRNL='"
					+ stats.name + "' AND CMTONM='JOURNALINTRO'");
	...
	public void DBReadJournalSummaryStats(
		...
		try {
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMJRNL WHERE CMJRNL='"
					+ stats.name + "' AND CMTONM='ALL' AND CMPART=''");
			long topTime = 0;
			while (R.next()) {
				String key = R.getString("CMJKEY");
	...
	public void DBDelete(String Journal, String key) {
		...
		synchronized (Journal.toUpperCase().intern()) {
			String sql;
			if (key != null) {
				sql = "DELETE FROM CMJRNL WHERE CMJKEY='" + key
						+ "' OR CMPART='" + key + "'";
			} else {
	...
	public void DBWriteJournalReply(String Journal, String key, String from,
		...
		if (Journal == null)
			return;
		synchronized (Journal.toUpperCase().intern()) {
			JournalsLibrary.JournalEntry entry = DBReadJournalEntry(Journal,
					key);
			if (entry == null)
				return;
			...
			message = oldmsg + JournalsLibrary.JOURNAL_BOUNDARY
					+ "^yReply from^N: " + from + "    ^yDate/Time ^N: "
					+ CMLib.time().date2String(now) + "%0D" + message;
			String sql = "UPDATE CMJRNL SET CMUPTM=" + now
					+ ", CMMSGT=?, CMREPL=" + replies + " WHERE CMJKEY='"
					+ oldkey + "'";
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.CMJRNL))
				Log.debugOut("JournalLoader", sql);
	...
	public void DBWrite(String Journal, String from, String to, String subject,
			String message) {
		DBWrite(Journal, "", from, to, subject, message);
	}

	public void DBWrite(String Journal, String journalSource, String from,
			String to, String subject, String message) {
		DBWrite(Journal, journalSource, from, to, "", subject, message);
	}

	public void DBWrite(String Journal, String journalSource, String from,
			String to, String parentKey, String subject, String message) {
		Journal = DB.injectionClean(Journal);
		from = DB.injectionClean(from);
		to = DB.injectionClean(to);
		subject = DB.injectionClean(subject);
	...
	public void DBWrite(String Journal, JournalsLibrary.JournalEntry entry) {
		...
		synchronized (Journal.toUpperCase().intern()) {
			...
			if (entry.update == 0)
				entry.update = now;
			String sql = "INSERT INTO CMJRNL (" + "CMJKEY, " + "CMJRNL, "
					+ "CMFROM, " + "CMDATE, " + "CMTONM, " + "CMSUBJ, "
					+ "CMPART, " + "CMATTR, " + "CMDATA, " + "CMUPTM, "
					+ "CMIMGP, " + "CMVIEW, " + "CMREPL, " + "CMMSGT "
					+ ") VALUES ('"
					+ entry.key
					+ "','"
					+ Journal
