...
public class QuestLoader {
		...
		try {
			...
			while (R.next()) {
				...
				if (Q.name().length() == 0)
					Q.setName(questName);
				if (Q.name().length() == 0)
					Log.sysOut("QuestLoader", "Unable to load Quest '"
							+ questName + "' due to blank name.");
				else if (Q.duration() < 0)
					Log.sysOut("QuestLoader", "Unable to load Quest '"
							+ questName + "' due to duration " + Q.duration()
							+ ".");
				else if (CMLib.quests().fetchQuest(Q.name()) != null)
					Log.sysOut("QuestLoader", "Unable to load Quest '"
							+ questName + "' due to it already being loaded.");
				else
	...
	public void DBUpdateQuest(Quest Q) {
		...
		DB.updateWithClobs(
				"INSERT INTO CMQUESTS (" + "CMQUESID, " + "CMQUTYPE, "
						+ "CMQFLAGS, " + "CMQSCRPT, " + "CMQWINNS "
						+ ") values (" + "'" + Q.name() + "'," + "'"
						+ CMClass.classID(Q) + "'," + Q.getFlags() + "," + "?,"
						+ "?" + ")",
	...
	public void DBUpdateQuests(List<Quest> quests) {
		...
		try {
			Thread.sleep((1000 + (quests.size() * 100)));
		} catch (Exception e) {
		}
		if (DB.queryRows("SELECT * FROM CMQUESTS WHERE CMQUTYPE='" + quType
				+ "'") > 0)
		...
		for (int m = 0; m < quests.size(); m++) {
			...
			if (Q.isCopy())
				continue;
			try {
				D.rePrepare("INSERT INTO CMQUESTS (" + "CMQUESID, "
						+ "CMQUTYPE, " + "CMQFLAGS, " + "CMQSCRPT, "
						+ "CMQWINNS " + ") values (" + "'" + Q.name() + "',"
						+ "'" + CMClass.classID(Q) + "'," + Q.getFlags() + ","
						+ "?," + "?" + ")");
				D.setPreparedClobs(new String[] { Q.script() + " ",
						Q.getWinnerStr() + " " });
				D.update("", 0);
