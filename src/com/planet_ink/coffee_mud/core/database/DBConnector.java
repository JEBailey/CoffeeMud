...
public class DBConnector {
	...
	public DBConnector(String dbClass, String dbService, String dbUser,
			String dbPass, int numConnections, int dbPingIntMins,
			boolean reuse, boolean doErrorQueueing, boolean retryErrorQueue) {
		super();
	...
	public void reconnect() {
		...
		dbConnections = new DBConnections(dbClass, dbService, dbUser, dbPass,
				numConnections, dbReuse, doErrorQueueing);
	...
	public int update(final String[] updateStrings) {
		return (dbConnections != null) ? dbConnections.update(updateStrings)
				: 0;
	}

	public int update(final String updateString) {
		return (dbConnections != null) ? dbConnections
				.update(new String[] { updateString }) : 0;
	}

	public int updateWithClobs(String[] updateStrings, String[][][] values) {
		return (dbConnections != null) ? dbConnections.updateWithClobs(
				updateStrings, values) : 0;
	}

	public int updateWithClobs(final List<DBPreparedBatchEntry> entries) {
		return (dbConnections != null) ? dbConnections.updateWithClobs(entries)
				: 0;
	}
	...
	public int updateWithClobs(final String updateString,
			final String... values) {
	...
	public int updateWithClobs(final String updateString,
			final String[][] values) {
		return (dbConnections != null) ? dbConnections.updateWithClobs(
				updateString, values) : 0;
	}

	public int queryRows(String queryString) {
		return (dbConnections != null) ? dbConnections.queryRows(queryString)
				: 0;
	}
	...
	/**
	 * Fetch a single, not in use DBConnection object. Must be rePrepared
	 * afterwards You can then call DBConnection.query and DBConnection.update
	 * on this object. The user must ALWAYS call DBDone when done with the
	 * object.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage: DB=DBFetchEmpty();</b>
	 * 
	 * @return DBConnection The DBConnection to use
	 */
	...
	public DBConnection DBFetchPrepared(String SQL) {
		return (dbConnections != null) ? dbConnections.DBFetchPrepared(SQL)
				: null;
	}
	 ...
	 * 
	 * @param querySql
	 *            the query to ping with
	 * @return the number of pings done
	 */
	public int pingAllConnections(final String querySql) {
		return (dbConnections != null) ? dbConnections.pingAllConnections(
				querySql, dbPingIntMins * (60 * 1000)) : 0;
	}
	 ...
	 * @param querySql
	 *            the query to ping with
	 * @param overridePingIntMillis
	 *            the age of a connection before a ping is necessary
	 * @return the number of pings done
	 */
	public int pingAllConnections(final String querySql,
			final long overridePingIntMillis) {
		return (dbConnections != null) ? dbConnections.pingAllConnections(
				querySql, overridePingIntMillis) : 0;
	}
	...
	public StringBuffer errorStatus() {
		...
		if (status.length() == 0)
			return new StringBuffer("OK! Connections in use="
					+ dbConnections.numInUse() + "/"
					+ dbConnections.numConnectionsMade());
		return new StringBuffer("<BR>"
				+ status.toString().replaceAll("\n", "<BR>")
				+ "Connections in use=" + dbConnections.numInUse() + "/"
				+ dbConnections.numConnectionsMade());
	}
}
