...
public class DBInterface implements DatabaseEngine {
	...
	public DBInterface(DBConnector DB, Set<String> privacyV) {
		...
		DatabaseEngine baseEngine = (DatabaseEngine) CMLib.library(
				MudHost.MAIN_HOST, CMLib.Library.DATABASE);
		if ((privacyV != null) && (baseEngine != null)
				&& (baseEngine.getConnector() != DB)
				&& (baseEngine.isConnected()))
			oldBaseDB = baseEngine.getConnector();
		this.GAbilityLoader = new GAbilityLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBABILITY.toString()) ? DB
				: oldBaseDB);
		this.GCClassLoader = new GCClassLoader(
				(privacyV == null)
						|| privacyV.contains(DatabaseTables.DBCHARCLASS
								.toString()) ? DB : oldBaseDB);
		this.GRaceLoader = new GRaceLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBRACE.toString()) ? DB
				: oldBaseDB);
		this.MOBloader = new MOBloader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBPLAYERS.toString()) ? DB
				: oldBaseDB);
		this.RoomLoader = new RoomLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBMAP.toString()) ? DB
				: oldBaseDB);
		this.DataLoader = new DataLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBPLAYERS.toString()) ? DB
				: oldBaseDB);
		this.StatLoader = new StatLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBSTATS.toString()) ? DB
				: oldBaseDB);
		this.PollLoader = new PollLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBPOLLS.toString()) ? DB
				: oldBaseDB);
		this.VFSLoader = new VFSLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBVFS.toString()) ? DB
				: oldBaseDB);
		this.JournalLoader = new JournalLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBJOURNALS.toString()) ? DB
				: oldBaseDB);
		this.QuestLoader = new QuestLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBQUEST.toString()) ? DB
				: oldBaseDB);
		this.ClanLoader = new ClanLoader((privacyV == null)
				|| privacyV.contains(DatabaseTables.DBCLANS.toString()) ? DB
				: oldBaseDB);
	}
	...
	public JournalsLibrary.JournalEntry DBReadJournalEntry(String Journal,
			String Key) {
	...
	public Vector<JournalsLibrary.JournalEntry> DBSearchAllJournalEntries(
			String Journal, String searchStr) {
		return JournalLoader.DBSearchAllJournalEntries(Journal, searchStr);
	}

	public Vector<JournalsLibrary.JournalEntry> DBReadJournalMsgsNewerThan(
			String Journal, String to, long olderDate) {
		return JournalLoader.DBReadJournalMsgsNewerThan(Journal, to, olderDate);
	}

	public Vector<JournalsLibrary.JournalEntry> DBReadJournalPageMsgs(
			String Journal, String parent, String searchStr, long newerDate,
			int limit) {
		return JournalLoader.DBReadJournalPageMsgs(Journal, parent, searchStr,
				newerDate, limit);
	}
	...
	public long[] DBJournalLatestDateNewerThan(String Journal, String to,
			long olderTime) {
		return JournalLoader.DBJournalLatestDateNewerThan(Journal, to,
				olderTime);
	}

	public void DBWriteJournal(String Journal,
			JournalsLibrary.JournalEntry entry) {
		JournalLoader.DBWrite(Journal, entry);
	}

	public void DBWriteJournal(String Journal, String from, String to,
			String subject, String message) {
		JournalLoader.DBWrite(Journal, from, to, subject, message);
	}

	public void DBWriteJournalEmail(String MailBox, String journalSource,
			String from, String to, String subject, String message) {
		JournalLoader.DBWrite(MailBox, journalSource, from, to, subject,
				message);
	}

	public void DBWriteJournalChild(String Journal, String journalSource,
			String from, String to, String parentKey, String subject,
			String message) {
		JournalLoader.DBWrite(Journal, journalSource, from, to, parentKey,
				subject, message);
	}
	...
	public void DBWriteJournalReply(String Journal, String key, String from,
			String to, String subject, String message) {
		JournalLoader.DBWriteJournalReply(Journal, key, from, to, subject,
				message);
	}
	...
	public void DBReCreateData(String name, String section, String key,
			String xml) {
	...
	public void DBCreateData(String player, String section, String key,
			String data) {
	...
	public void DBCreatePoll(String name, String player, String subject,
			String description, String optionXML, int flag, String qualZapper,
			String results, long expiration) {
		PollLoader.DBCreate(name, player, subject, description, optionXML,
				flag, qualZapper, results, expiration);
	}

	public void DBUpdatePoll(String oldName, String name, String player,
			String subject, String description, String optionXML, int flag,
			String qualZapper, String results, long expiration) {
		PollLoader.DBUpdate(oldName, name, player, subject, description,
				optionXML, flag, qualZapper, results, expiration);
	}
	...
	public void DBCreateVFSFile(String filename, int bits, String creator,
			long updateTime, Object data) {
		VFSLoader.DBCreate(filename, bits, creator, updateTime, data);
	}

	public void DBUpSertVFSFile(String filename, int bits, String creator,
			long updateTime, Object data) {
	...
	public int DBRawExecute(String sql) throws CMException {
		...
		try {
			...
			throw new CMException((e.getMessage() == null) ? "Unknown error"
					: e.getMessage());
		} finally {
			if (DBToUse != null)
				DB.DBDone(DBToUse);
		...
		try {
			...
			if (metaData != null) {
				...
				for (int c = 1; c <= metaData.getColumnCount(); c++)
					header.add(CMStrings.padRight(metaData.getColumnName(c),
							metaData.getColumnDisplaySize(c)));
