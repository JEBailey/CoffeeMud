...
public class DBConnections {
	...
	/**
	 * Initialize this class. Must be called at first, and after any
	 * killConnections() calls.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> Initialize("ODBCSERVICE","USER","PASSWORD",10);
	 * 
	 * @param NEWDBClass
	 *            the odbc service
	 * @param NEWDBService
	 ...
	 *            the odbc user login
	 * @param NEWDBPass
	 *            the odbc user password
	 * @param NEWnumConnections
	 *            Connections to maintain
	 * @param NEWreuse
	 *            Whether to reuse connections
	 * @param DoErrorQueueing
	 *            whether to save errors to a file
	 */
	public DBConnections(String NEWDBClass, String NEWDBService,
			String NEWDBUser, String NEWDBPass, int NEWnumConnections,
			boolean NEWreuse, boolean DoErrorQueueing) {
		dbClass = NEWDBClass;
	 ...
	 * <br>
	 * <br>
	 * <b>Usage: update("UPDATE...");</b>
	 * 
	 * @param updateStrings
	 *            the update SQL commands
	 ...
	 * <br>
	 * <br>
	 * <b>Usage: updateWithClobs("UPDATE...");</b>
	 * 
	 * @param entries
	 *            the update SQL commands
	 * @return int the responseCode, or -1
	 ...
	 *            the update SQL commands
	 * @param values
	 *            the update SQL values
	 * @return int the responseCode, or -1
	 */
	public int updateWithClobs(String updateString, String[][] values) {
		return updateWithClobs(Arrays.asList(new DBPreparedBatchEntry(
				updateString, values)));
	}
	 ...
	 * <br>
	 * <br>
	 * <b>Usage: n=numConnectionsMade();</b>
	 * 
	...
	/**
	 * Fetch a single, not in use DBConnection object. You can then call
	 * DBConnection.query and DBConnection.update on this object. The user must
	 * ALWAYS call DBDone when done with the object.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage: DB=DBFetch();</b>
	 * 
	 ...
	 * <br>
	 * <br>
	 * <b>Usage: DB=DBFetchPrepared();</b>
	 * 
	 * @param SQL
	 *            The prepared statement SQL
	 * @param type
	 *            the type of fetching to do
	 * @return DBConnection The DBConnection to use
	 */
	public DBConnection DBFetchAny(final String SQL,
			final DBConnection.FetchType type) {
		...
		while (ThisDB == null) {
			...
			if (connections.size() < maxConnections)
				try {
					ThisDB = new DBConnection(this, dbClass, dbService, dbUser,
							dbPass, reuse);
			...
			if ((ThisDB == null) && (reuse)) {
				try {
					for (DBConnection conn : connections) {
						...
						} else if (type == DBConnection.FetchType.STATEMENT) {
							if (conn.use("")) {
								ThisDB = conn;
								break;
							}
			...
			if ((ThisDB != null)
					&& (ThisDB.isProbablyDead() || ThisDB.isProbablyLockedUp() || (!ThisDB
							.ready()))) {
			...
			if (ThisDB == null) {
				...
				if (connections.size() >= maxConnections) {
					...
					if (consecutiveFailures == 180) {
						Log.errOut("DBConnections",
								"Serious failure obtaining DBConnection ("
										+ inuse + "/" + connections.size()
										+ " in use).");
						for (DBConnection conn : connections)
							if (conn.inUse())
								Log.errOut("DBConnections", "Last SQL was: "
										+ conn.lastSQL);
						if (inuse == 0)
							resetConnections();
					} else if (consecutiveFailures == 90)
						Log.errOut("DBConnections",
								"Moderate failure obtaining DBConnection ("
										+ inuse + "/" + connections.size()
										+ " in use).");
					else if (consecutiveFailures == 30)
						Log.errOut("DBConnections",
								"Minor failure obtaining DBConnection(" + inuse
										+ "/" + connections.size()
										+ " in use).");
					try {
	 ...
	 * 
	 * @param D
	 *            The Database connection to return to the pool
	 */
	...
	/**
	 * When reading a database table, this routine will read in the given Field
	 * NAME, returning the value. The value will be trim()ed, and will not be
	 * NULL.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> str=getRes(R,"FIELD");
	 * 
	 * @param Results
	 *            The ResultSet object to use
	 * @param Field
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> str=getLongRes(R,"FIELD");
	 * 
	 * @param Results
	 *            The ResultSet object to use
	 * @param Field
	...
	/**
	 * When reading a database table, this routine will read in the given One
	 * index number, returning the value. The value will be trim()ed, and will
	 * not be NULL.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> str=getRes(R,1);
	 * 
	 * @param Results
	 *            The ResultSet object to use
	 * @param One
	 *            Field number to return
	 * @return String The value of the field being returned
	 */
	...
	/**
	 * Return the happiness level of the connections <br>
	 * <br>
	 * <b>Usage:</b> amIOk()
	 * 
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> enQueueError("UPDATE SQL","error string");
	 * 
	 * @param SQLString
	 *            UPDATE style SQL statement
	 * @param SQLError
	 *            The error message being reported
	 * @param tries
	 *            The number of tries to redo it so far
	 */
	public void enQueueError(String SQLString, String SQLError, String tries) {
		...
		synchronized ("SQLErrors.que") {
			...
			try {
				out = new PrintWriter(new FileOutputStream("SQLErrors.que",
						true), true);
			...
			if (out != null) {
				out.println(SQLString + "\t!|!\t" + SQLError + "\t!|!\t"
						+ tries);
				out.close();
			}
		...
		if (Queue.size() == 0) {
			...
			while (Queue.size() > 0) {
				...
				if ((firstTab > 0) && (secondTab > firstTab)) {
					...
					int oldAttempts = CMath.s_int(queueLine
							.substring(secondTab + 5));
					if (oldAttempts > 20) {
						Log.errOut("DBConnections", "Giving up on :" + retrySQL);
						Log.errOut("DBConnections", "Giving up because: "
								+ queueLine.substring(firstTab + 5, secondTab));
					} else {
						try {
							DB = DBFetch();
							try {
								...
								// DO NOT DO THIS AGAIN -- the UPDATE WILL
								// GENERATE the ENQUE!!!
								// enQueueError(retrySQL,""+sqle,oldDate);
							}
						} catch (Exception e) {
							unsuccesses++;
							enQueueError(retrySQL, e.getMessage(), ""
									+ (oldAttempts + 1));
						}
						if (DB != null) {
							DB.clearFailures();
					...
					Log.errOut("DBConnections", "Could not retry line: "
							+ queueLine + "/" + firstTab + "/" + secondTab);
			}
			clearErrors();
			if (unsuccesses > successes)
				Log.errOut("DBConnections",
						"Finished running retry Que. Successes: " + successes
								+ "/Failures: " + unsuccesses);
			else
				Log.sysOut("DBConnections",
						"Finished running retry Que. Successes: " + successes
								+ "/Failures: " + unsuccesses);
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> listConnections(out);
	 * 
	 * @param out
	 *            place to send the list out to
	 */
	public void listConnections(PrintStream out) {
		...
		for (DBConnection conn : connections) {
			...
			if ((conn.isProbablyDead()) && (conn.isProbablyLockedUp()))
				OKString = "Completely dead"
						+ (conn.inSQLServerCommunication() ? " (SERVER COMM)"
								: "") + ".";
				...
				OKString = "Locked up"
						+ (conn.inSQLServerCommunication() ? " (SERVER COMM)"
								: "") + ".";
			out.println(Integer.toString(p) + ". Connected=" + conn.ready()
					+ ", In use=" + conn.inUse() + ", Status=" + OKString);
			p++;
		}
		out.println("\n");
	}
