...
public class GAbilityLoader {
	...
	public List<DatabaseEngine.AckRecord> DBReadAbilities() {
		...
		try {
			...
			while (R.next())
				rows.addElement(new DatabaseEngine.AckRecord(DBConnections
						.getRes(R, "CMGAID"),
						DBConnections.getRes(R, "CMGAAT"), DBConnections
								.getRes(R, "CMGACL")));
	...
	public void DBCreateAbility(String classID, String typeClass, String data) {
		DB.updateWithClobs("INSERT INTO CMGAAC (" + "CMGAID, " + "CMGAAT, "
				+ "CMGACL " + ") values (" + "'" + classID + "'," + "?," + "'"
				+ typeClass + "'" + ")", data + " ");
	}
