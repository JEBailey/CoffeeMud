...
public class MOBloader {
	...
	public String DBReadUserOnly(MOB mob) {
		...
		try {
			...
			if (R.next()) {
				...
				stats.setStat(CharStats.STAT_STRENGTH,
						CMath.s_int(DBConnections.getRes(R, "CMSTRE")));
				stats.setMyRace(CMClass.getRace(DBConnections.getRes(R,
						"CMRACE")));
				stats.setStat(CharStats.STAT_DEXTERITY,
						CMath.s_int(DBConnections.getRes(R, "CMDEXT")));
				stats.setStat(CharStats.STAT_CONSTITUTION,
						CMath.s_int(DBConnections.getRes(R, "CMCONS")));
				stats.setStat(CharStats.STAT_GENDER,
						DBConnections.getRes(R, "CMGEND").charAt(0));
				stats.setStat(CharStats.STAT_WISDOM,
						CMath.s_int(DBConnections.getRes(R, "CMWISD")));
				stats.setStat(CharStats.STAT_INTELLIGENCE,
						CMath.s_int(DBConnections.getRes(R, "CMINTE")));
				stats.setStat(CharStats.STAT_CHARISMA,
						CMath.s_int(DBConnections.getRes(R, "CMCHAR")));
				...
				mob.setAgeMinutes(CMath.s_long(DBConnections
						.getRes(R, "CMAGEH")));
				...
				if (pstats.getBirthday() == null)
					stats.setStat(
							CharStats.STAT_AGE,
							pstats.initializeBirthday(
									(int) Math.round(CMath.div(
											mob.getAgeMinutes(), 60.0)),
									stats.getMyRace()));
				mob.setImage(CMLib.xml().returnXMLValue(buf, "IMG"));
				List<XMLLibrary.XMLpiece> CleanXML = CMLib.xml().parseAllXML(
				...
				if ((CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1)
					...
					// yes, this can happen when you wiggle in and out of the
					// account system.
					for (Enumeration<PlayerAccount> a = CMLib.players()
							.accounts(); a.hasMoreElements();) {
	...
	public void DBRead(MOB mob) {
		...
		mob.basePhyStats().setDisposition(
				PhyStats.IS_NOT_SEEN | PhyStats.IS_SNEAKING);
							...
							if (roomID.startsWith("SPACE.")
								...
								if (itemR != null) {
									if ((newItem instanceof SpaceShip)
											&& (itemR instanceof LocationRoom))
										((SpaceShip) newItem)
												.dockHere((LocationRoom) itemR);
									else
		...
		try {
			...
			while (R.next()) {
				...
				if ((proficiency == Integer.MIN_VALUE)
						|| (proficiency == Integer.MIN_VALUE + 1)) {
					if (abilityID.equalsIgnoreCase("ScriptingEngine")) {
						if (CMClass.getCommon("DefaultScriptingEngine") == null)
							...
							if (xml.length() > 0)
								CMLib.coffeeMaker().setGenScripts(mob,
										CMLib.xml().parseAllXML(xml), true);
						}
					} else {
						...
						if (newBehavior == null)
							...
							newBehavior.setParms(DBConnections.getRes(R,
									"CMABTX"));
					...
					if (newAbility == null)
						...
						if ((proficiency < 0)
							...
							if (proficiency == Integer.MAX_VALUE) {
								newAbility.setProficiency(CMLib.ableMapper()
										.getMaxProficiency(newAbility.ID()));
	...
	protected PlayerLibrary.ThinPlayer parseThinUser(ResultSet R) {
		try {
			...
			if (R2 != null)
				...
				thisUser.race = rrace;
			List<String> lvls = CMParms.parseSemicolons(
					DBConnections.getRes(R, "CMLEVL"), true);
			thisUser.level = 0;
			for (String lvl : lvls)
	...
	public MOB.Tattoo parseTattoo(String tattoo) {
		...
		int tickDown = 0;
		if ((tattoo.length() > 0) && (Character.isDigit(tattoo.charAt(0)))) {
			int x = tattoo.indexOf(' ');
			if ((x > 0) && (CMath.isNumber(tattoo.substring(0, x).trim()))) {
			...
			for (Enumeration<MOB> e = CMLib.players().players(); e
				...
				if ((M.getLiegeID().equals(liegeID))
					...
					head.append(CMStrings
							...
							+ " ");
					head.append(CMStrings
							.padRight("" + M.phyStats().level(), 4) + " ");
					head.append(CMStrings.padRight(
							M.getExperience() + "/" + M.getExpNextLevel(), 17));
	...
	public DVector worshippers(String deityID) {
		...
		try {
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMCHAR WHERE CMWORS='"
					+ deityID + "'");
			if (R != null)
				while (R.next()) {
					String username = DBConnections.getRes(R, "CMUSERID");
					...
					int x = cclass.lastIndexOf(';');
					if ((x > 0) && (x < cclass.length() - 2))
						cclass = CMClass.getCharClass(cclass.substring(x + 1))
								.name();
					String race = (CMClass.getRace(DBConnections.getRes(R,
							"CMRACE"))).name();
					List<String> lvls = CMParms.parseSemicolons(
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if ((newMOB.getStartRoom() != null)
					&& (CMLib.law().doesHavePriviledgesHere(mob,
							newMOB.getStartRoom()))
					&& ((newMOB.location() == null) || (!CMLib.law()
							.doesHavePriviledgesHere(mob, newMOB.location()))))
				newMOB.setLocation(newMOB.getStartRoom());
			if (bringToLife) {
				newMOB.bringToLife(mob.location(), true);
				mob.location().showOthers(newMOB, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> appears!");
			}
		}
	}
		...
		try {
			...
			if (R != null)
				while (R.next()) {
					...
					Clan.MemberRecord member = new Clan.MemberRecord(username,
							clanRole);
					members.add(member);
				}
		} catch (Exception sqle) {
	...
	public void DBUpdateClan(String name, String clan, int role) {
		...
		clanStatements.add("DELETE FROM CMCHCL WHERE CMUSERID='" + name
				+ "' AND CMCLAN='" + clan + "'");
		if (role >= 0)
			clanStatements
					.add("INSERT INTO CMCHCL (CMUSERID, CMCLAN, CMCLRO) values ('"
							+ name + "','" + clan + "'," + role + ")");
	...
	public void DBUpdatePassword(String name, String password) {
		...
		DB.update("UPDATE CMCHAR SET CMPASS='" + password
				+ "' WHERE CMUSERID='" + name.replace('\'', 'n') + "'");
	}
	...
	public void DBUpdateJustPlayerStats(MOB mob) {
		...
		DB.updateWithClobs(
				"UPDATE CMCHAR SET CMPFIL=? WHERE CMUSERID='" + mob.Name()
						+ "'", pfxml.toString());
	}
	...
	public void DBUpdateJustMOB(MOB mob) {
		...
		if ((mob.location() != null)
				...
				&& (CMath.bset(mob.location().getArea().flags(),
						Area.FLAG_INSTANCE_PARENT) || CMath.bset(mob.location()
						.getArea().flags(), Area.FLAG_INSTANCE_CHILD)))
		...
		DB.updateWithClobs(
				"UPDATE CMCHAR SET  CMPASS='"
						...
						+ ", CMINTE="
						+ mob.baseCharStats().getStat(
								CharStats.STAT_INTELLIGENCE) + ", CMCHAR="
						...
						+ ", CMALIG=-1" + ", CMEXPE=" + mob.getExperience()
						+ ", CMEXLV=" + mob.getExpNextLevel() + ", CMWORS='"
						+ mob.getWorshipCharID() + "'" + ", CMPRAC="
						+ mob.getPractices() + ", CMTRAI=" + mob.getTrains()
						+ ", CMAGEH=" + mob.getAgeMinutes() + ", CMGOLD="
						+ mob.getMoney() + ", CMWIMP=" + mob.getWimpHitPoint()
						+ ", CMQUES=" + mob.getQuestPoint() + ", CMROID='"
						+ strStartRoomID + "||" + strOtherRoomID + "'"
						+ ", CMDATE='" + pstats.lastDateTime() + "'"
						+ ", CMCHAN=" + pstats.getChannelMask() + ", CMATTA="
						+ mob.basePhyStats().attackAdjustment() + ", CMAMOR="
						+ mob.basePhyStats().armor() + ", CMDAMG="
						+ mob.basePhyStats().damage() + ", CMBTMP="
						+ mob.getBitmap() + ", CMLEIG='" + mob.getLiegeID()
						+ "'" + ", CMHEIT=" + mob.basePhyStats().height()
						+ ", CMWEIT=" + mob.basePhyStats().weight()
						+ ", CMPRPT=?" + ", CMCOLR=?" + ", CMLSIP='"
						+ pstats.lastIP() + "'" + ", CMEMAL=?" + ", CMPFIL=?"
						+ ", CMSAVE=?" + ", CMMXML=?" + ", CMDESC=?"
						+ "  WHERE CMUSERID='" + mob.Name() + "'",
				new String[][] { { pstats.getPrompt(), pstats.getColorStr(),
						pstats.getEmail(), pfxml,
						mob.baseCharStats().getNonBaseStatsAsString(),
						cleanXML.toString(), mob.description() } });
		List<String> clanStatements = new LinkedList<String>();
	...
	protected String getDBItemUpdateString(final MOB mob, final Item thisItem) {
		...
		String container = ((thisItem.container() != null) ? ("" + thisItem
				.container()) : "");
		...
		if (pStats != null) {
			...
			for (final Item thisItem : finalCollection) {
				if (!done.contains("" + thisItem)) {
					...
					final Item cont = thisItem.ultimateContainer(null);
					final String str = getDBItemUpdateString(mob, thisItem);
					final String roomID = ((cont.owner() == null)
							&& (thisItem instanceof SpaceShip) && (CMLib.map()
							.isObjectInSpace((SpaceShip) thisItem))) ? ("SPACE." + CMParms
							.toStringList(((SpaceShip) thisItem).coordinates()))
							: CMLib.map()
									.getExtendedRoomID((Room) cont.owner());
					final String text = "<ROOM ID=\"" + roomID + "\" EXPIRE="
							+ thisItem.expirationDate() + " />"
							+ thisItem.text();
	...
	public void DBUpdateItems(MOB mob) {
		...
		statements.add(new DBPreparedBatchEntry(
				"DELETE FROM CMCHIT WHERE CMUSERID='" + mob.Name() + "'"));
		statements.addAll(getDBItemUpdateStrings(mob));
		DB.updateWithClobs(statements);
	}

	// this method is unused, but is a good idea of how to collect riders,
	// followers, carts, etc.
	protected void addFollowerDependent(PhysicalAgent P, DVector list,
			String parent) {
		if (P == null)
			return;
		if (list.contains(P))
	...
	public void DBUpdateFollowers(MOB mob) {
		...
		List<DBPreparedBatchEntry> statements = new LinkedList<DBPreparedBatchEntry>();
		statements.add(new DBPreparedBatchEntry(
				"DELETE FROM CMCHFO WHERE CMUSERID='" + mob.Name() + "'"));
	...
	public void DBNameChange(String oldName, String newName) {
		if ((oldName == null) || (oldName.trim().length() == 0)
				|| (oldName.indexOf('\'') >= 0) || (newName == null)
				|| (newName.trim().length() == 0)
				|| (newName.indexOf('\'') >= 0))
			return;
		DB.update("UPDATE CMCHAB SET CMUSERID='" + newName
				+ "' WHERE CMUSERID='" + oldName + "'");
		DB.update("UPDATE CMCHAR SET CMUSERID='" + newName
				+ "' WHERE CMUSERID='" + oldName + "'");
		DB.update("UPDATE CMCHAR SET CMWORS='" + newName + "' WHERE CMWORS='"
				+ oldName + "'");
		DB.update("UPDATE CMCHAR SET CMLEIG='" + newName + "' WHERE CMLEIG='"
				+ oldName + "'");
		DB.update("UPDATE CMCHCL SET CMUSERID='" + newName
				+ "' WHERE CMUSERID='" + oldName + "'");

		DB.update("UPDATE CMCHFO SET CMUSERID='" + newName
				+ "' WHERE CMUSERID='" + oldName + "'");
		DB.update("UPDATE CMCHIT SET CMUSERID='" + newName
				+ "' WHERE CMUSERID='" + oldName + "'");
		DB.update("UPDATE CMJRNL SET CMFROM='" + newName + "' WHERE CMFROM='"
				+ oldName + "'");
		DB.update("UPDATE CMJRNL SET CMTONM='" + newName + "' WHERE CMTONM='"
				+ oldName + "'");
		DB.update("UPDATE CMPDAT SET CMPLID='" + newName + "' WHERE CMPLID='"
				+ oldName + "'");
	}
	...
	public void DBDelete(MOB mob, boolean deleteAssets) {
		...
		List<String> channels = CMLib.channels().getFlaggedChannelNames(
				ChannelsLibrary.ChannelFlag.PLAYERPURGES);
		for (int i = 0; i < channels.size(); i++)
			CMLib.commands().postChannel(channels.get(i), mob.clans(),
					mob.Name() + " has just been deleted.", true);
	...
	public void DBUpdateAbilities(MOB mob) {
		...
		List<DBPreparedBatchEntry> statements = new LinkedList<DBPreparedBatchEntry>();
		statements.add(new DBPreparedBatchEntry(
				"DELETE FROM CMCHAB WHERE CMUSERID='" + mob.Name() + "'"));
		...
		for (int a = 0; a < mob.numAbilities(); a++) {
			...
			if ((thisAbility != null) && (thisAbility.isSavable())) {
				...
				if (effectA != null) {
					if ((effectA.isSavable()) && (!effectA.canBeUninvoked())
				...
				String str = "INSERT INTO CMCHAB (CMUSERID, CMABID, CMABPF,CMABTX"
						...
						+ "'," + proficiency + ",?)";
				statements
						.add(new DBPreparedBatchEntry(str, thisAbility.text()));
			}
		}
		for (final Enumeration<Ability> a = mob.personalEffects(); a
				.hasMoreElements();) {
			final Ability A = a.nextElement();
			if ((A != null) && (!H.contains(A.ID())) && (A.isSavable())
					&& (!A.canBeUninvoked())) {
				String str = "INSERT INTO CMCHAB (CMUSERID, CMABID, CMABPF,CMABTX"
						+ ") values ('"
						+ mob.Name()
		...
		if (scriptStuff.length() > 0) {
			String str = "INSERT INTO CMCHAB (CMUSERID, CMABID, CMABPF,CMABTX"
					+ ") values ('" + mob.Name() + "','ScriptingEngine',"
					+ (Integer.MIN_VALUE + 1) + ",?" + ")";
	...
	public void DBUpdateAccount(PlayerAccount account) {
		...
		DB.updateWithClobs(
				"UPDATE CMACCT SET CMPASS='" + account.getPasswordStr()
						+ "',  CMCHRS=?,  CMAXML=?  WHERE CMANAM='"
						+ account.accountName() + "'", new String[][] { {
						characters, account.getXML() } });
	}
	...
	public void DBCreateAccount(PlayerAccount account) {
		if (account == null)
			return;
		account.setAccountName(CMStrings.capitalizeAndLower(account
				.accountName()));
		String characters = CMParms.toSemicolonList(account.getPlayers());
		DB.updateWithClobs(
				"INSERT INTO CMACCT (CMANAM, CMPASS, CMCHRS, CMAXML) "
						+ "VALUES ('" + account.accountName() + "','"
						+ account.getPasswordStr() + "',?,?)",
				new String[][] { { characters, account.getXML() } });
	}

	public PlayerAccount MakeAccount(String username, ResultSet R)
			throws SQLException {
	...
	public PlayerAccount DBReadAccount(String Login) {
		...
		try {
			...
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMACCT WHERE CMANAM='"
					+ CMStrings.replaceAll(CMStrings.capitalizeAndLower(Login),
							"\'", "n") + "'");
			if (R != null)
				while (R.next()) {
			...
			if (R != null)
				while (R.next()) {
					...
					if ((mask == null) || (mask.length() == 0)
							|| (username.toLowerCase().indexOf(mask) >= 0)) {
						account = MakeAccount(username, R);
	...
	public PlayerLibrary.ThinnerPlayer DBUserSearch(String Login) {
		...
		try {
			...
			D = DB.DBFetch();
			ResultSet R = D.query("SELECT * FROM CMCHAR WHERE CMUSERID='"
					+ CMStrings.capitalizeAndLower(Login).replace('\'', 'n')
					+ "'");
			if (R != null)
				while (R.next()) {
					String username = DB.getRes(R, "CMUSERID");
		...
		if ((buf != null) && (thinPlayer != null)) {
			...
			if ((thinPlayer.accountName != null)
					&& (thinPlayer.accountName.length() > 0))
			...
			if ((acct != null)
				...
				thinPlayer.expiration = CMath.s_long(CMLib.xml()
						.returnXMLValue(buf, "ACCTEXP"));
			else {
	...
	public String DBPlayerEmailSearch(String email) {
		...
		try {
			...
			if (((R == null) || (!R.next())) && (!CMStrings.isLowerCase(email)))
				R = D.query("SELECT * FROM CMCHAR WHERE CMEMAL='"
						+ email.toLowerCase() + "'");
			if ((R == null) || (!R.next()))
