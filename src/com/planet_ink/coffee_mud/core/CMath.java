 ...
 * functions, especially dealing with explicit type conversions, and special
...
public class CMath {
	...
	private static final String[] ROMAN_HUNDREDS = { "C", "CC", "CCC", "CD",
			"D", "DC", "DCC", "DCCC", "CM", "P" };
	private static final String[] ROMAN_TENS = { "X", "XX", "XXX", "XL", "L",
			"LX", "LXX", "LXXX", "XC", "C" };
	private static final String[] ROMAN_ONES = { "I", "II", "III", "IV", "V",
			"VI", "VII", "VIII", "IX", "X" };
	private static final String ROMAN_ALL = "CDMPXLIV";
	private static final java.text.DecimalFormat twoPlaces = new java.text.DecimalFormat(
			"0.#####%");
	...
	/**
	 * Returns which object in the object array is same as the string, when cast
	 * to a string.
	 * 
	 * @param o
	 *            array of objects
	...
	public final static Enum<? extends Enum> s_valueOf(Class<? extends Enum> c,
			String s) {
		if ((c == null) || (s == null))
			return null;
		try {
	 ...
	 *            the string to look
	 * @param def
	 *            the value to use when null
	 * @return the enum
	 */
	@SuppressWarnings("rawtypes")
	public final static Enum<? extends Enum> s_valueOf(Class<? extends Enum> c,
			String s, Enum<? extends Enum> def) {
	...
	/**
	 * Divide a by b, making sure both are cast to doubles and that the return
	 * is precisely double.
	 * 
	 * @param a
	 *            the dividend
	...
	/**
	 * Multiply a and b, making sure both are cast to doubles and that the
	 * return is precisely double.
	 * 
	 * @param a
	 *            the first number
	 * @param b
	...
	/**
	 * Raises x to the y power, making sure both are cast to doubles and that
	 * the return is rounded off.
	 * 
	 * @param x
	 *            the base number
	 * @param y
	...
	/**
	 * Returns true if the given number has the bits represented by the given
	 * bitmask set.
	 * 
	 * @param num
	 *            the number
	...
	/**
	 * Returns the given number, after having set the bits represented by the
	 * given bit mask.
	 * 
	 * @param num
	 *            the number
	...
	/**
	 * Returns true if any of the bits represented by the given bitmask are set
	 * in the given number.
	 * 
	 * @param num
	 *            the given number
	 * @param bitmask
	 *            the bitmask of bits to check
	...
	/**
	 * Unsets those bits in the given number which are turned ON in the given
	 * bitmask.
	 * 
	 * @param num
	 *            the given number
	...
	public final static int firstBitSetIndex(int bits) {
		return ((bits & 0x80000000) != 0) ? 31
				: firstBitSetIndex((bits << 1) | 1) - 1;
	}
	...
	/**
	 * Returns true if the given string represents a percentage in the form X%
	 ...
	 * 
	 * @param str
	 *            the stringbuffer to assess
	 * @param values
	 *            values to replace each variable with
	 */
	public final static void replaceVariables(final StringBuffer str,
			final double values[]) {
		final int valueLen = (values.length <= 10) ? 1 : Integer.toString(
				values.length).length();
		for (int i = 0; i < str.length() - (1 + valueLen); i++)
			if ((str.charAt(i) == '@') && (str.charAt(i + 1) == 'x')
					&& (Character.isDigit(str.charAt(i + 2)))) {
				int endDex = 1;
				while ((endDex < valueLen)
						&& (Character.isDigit(str.charAt(i + 2 + endDex))))
					endDex++;
				final int valueDex = Integer.valueOf(
						str.substring(i + 2, i + 2 + endDex)).intValue();
				final double newNumValue = (valueDex > 0 && valueDex <= values.length) ? values[valueDex - 1]
						: 0.0;
				final String newValue = (Math.round(newNumValue) == newNumValue) ? Long
						.toString(Math.round(newNumValue)) : Double
						.toString(newNumValue);
	 ...
	 */
	public final static String replaceVariables(final String str,
			final double values[]) {
	...
	/**
	 * Converts the given string to a floating point number, 1>=N>=0,
	 * representing the whole percentage of the string. The string format is
	 * either X or X%, where 100>=X>=0 If the format is bad, 0.0 is returned.
	 * 
	 * @param s
	...
	/**
	 * Converts the string to a double percentage and then converts that back to
	 * a percentage.
	 * 
	 * @param s
	 *            the string number
	...
	/**
	 * Returns whether the given string is a valid math expression (5 + 7)/2,
	 * etc. Does this by evaluating the expression and returning false if an
	 * error is found. No variables are allowed.
	 * 
	 * @param st
	 *            the possible math expression
	...
	/**
	 * Returns whether the given string is a valid math expression (@x1 + 7)/2,
	 * etc. Does this by evaluating the expression and returning false if an
	 * error is found. All necessary variables MUST be included (@x1=vars[0])
	 * 
	 * @param st
	 *            the possible math expression
	 * @param vars
	 *            the 0 based variables
	 * @return true if it is a math expression
	 */
	public final static boolean isMathExpression(final String st,
			final double[] vars) {
	...
	/**
	 * Returns the result of evaluating the given math expression. An expression
	 * can be a double or int number, or a full expression using ()+-/*?<>.
	 * Variable @xx will refer to current computed value. Returns 0.0 on any
	 * parsing error
	 * 
	 * @param st
	 *            a full math expression string
	 ...
	 * Returns the result of evaluating the given math expression. An expression
	 * can be a double or int number, or a full expression using ()+-/*?<>.
	 * Variables are included as @x1, etc.. The given variable values list is 0
	 * based, so @x1 = vars[0]. Variable @xx will refer to current computed
	 * value. Returns 0.0 on any parsing error
	 * 
	 * @param st
	 *            a full math expression string
	 * @param vars
	 ...
	 * Returns the result of evaluating the given math expression. An expression
	 * can be a double or int number, or a full expression using ()+-/*?<>.
	 * Variable @xx will refer to current computed value. Rounds the result to a
	 * long. Returns 0 on any parsing error
	 * 
	 * @param st
	 *            a full math expression string
	 * @return the result of the expression
	 ...
	 * Variables are included as @x1, etc.. The given variable values list is 0
	 * based, so @x1 = vars[0]. Variable @xx will refer to current computed
	 * value. Rounds the result to a long. Returns 0 on any parsing error
	 * 
	 * @param st
	 *            a full math expression string
	 * @param vars
	 ...
	 * Returns the result of evaluating the given math expression. An expression
	 * can be a double or int number, or a full expression using ()+-/*?<>.
	 * Variable @xx will refer to current computed value. Round the result to an
	 * integer. Returns 0 on any parsing error
	 * 
	 * @param st
	 *            a full math expression string
	 * @return the result of the expression
	 ...
	 * Variables are included as @x1, etc.. The given variable values list is 0
	 * based, so @x1 = vars[0]. Variable @xx will refer to current computed
	 * value. Rounds the result to an integer. Returns 0 on any parsing error
	 * 
	 * @param st
	 *            a full math expression string
	 * @param vars
	...
	private final static double parseMathExpression(final StreamTokenizer st,
			final boolean inParen, final double[] vars, final double previous)
		...
		try {
			...
			while (c != StreamTokenizer.TT_EOF) {
				...
				case '@': {
					...
					if ((c == 'x') || (c == 'X'))
						...
						if (c != StreamTokenizer.TT_NUMBER)
							throw new ArithmeticException("'" + c
									+ "' is an unexpected token after @x.");
						if (vars == null)
							throw new ArithmeticException(
									"vars have not been defined for @x"
											+ st.nval);
						if ((st.nval > vars.length) || (st.nval < 1.0))
							throw new ArithmeticException("'" + st.nval + "/"
									+ vars.length
									+ "' is an illegal variable reference.");
				...
				case '\\':
					...
					break;
				case '?':
					finalValue = ((curValue - finalValue + 0.5) * rand
							.nextDouble()) + finalValue;
					break;
				}
				c = st.nextToken();
			}
		...
		if (inParen)
			throw new ArithmeticException(
					"')' was missing from this expression");
		return finalValue;
	}

	/**
	 * A class representing a single piece of a compiled operation. Optomized
	 * for speed of execution rather than the obvious wastefulness of storage.
	 */
	public static final class CompiledOperation {
		...
		public CompiledOperation(LinkedList<CompiledOperation> list) {
			type = OPERATION_LIST;
			this.list = list;
		}
	 ...
	 * @throws ArithmeticException
	 */
	public final static LinkedList<CompiledOperation> compileMathExpression(
			final String formula) {
		return compileMathExpression(new StreamTokenizer(new InputStreamReader(
				new ByteArrayInputStream(formula.getBytes()))), false);
	}
	 ...
	 * @param inParen
	 *            whether or not you are in parenthesis mode
	 * @return the pre-compiled expression
	 * @throws ArithmeticException
	 */
	private final static LinkedList<CompiledOperation> compileMathExpression(
			final StreamTokenizer st, final boolean inParen)
			throws ArithmeticException {
		if (!inParen) {
			st.ordinaryChar('/');
			st.ordinaryChar('x');
		...
		try {
			...
			while (c != StreamTokenizer.TT_EOF) {
				...
				case StreamTokenizer.TT_NUMBER:
					...
					break;
				case '(':
					list.add(new CompiledOperation(compileMathExpression(st,
							true)));
					break;
				case ')':
					if (!inParen)
				...
				case '@': {
					...
					if ((c == 'x') || (c == 'X'))
						...
						if ((st.nval > 11) || (st.nval < 1.0))
							throw new ArithmeticException("'" + st.nval
									+ "/11' is an illegal variable reference.");
				...
				default:
					throw new ArithmeticException("'" + (char) c + "' (" + c
							+ ") is an illegal expression.");
				}
				switch (lastOperation) {
				case '+':
	...
	/**
	 * Parse a pre-compiled expression. Requires a vars variable of at least 10
	 * entries to ensure NO exceptions (other than /0).
	 * 
	 ...
	 */
	public final static double parseMathExpression(
			final LinkedList<CompiledOperation> list, final double[] vars,
	 ...
	 * can be a double or int number, or a full expression using ()+-/*?<>.
	 * Variable @xx will refer to current computed value. Rounds the result to a
	 * long Throws an exception on any parsing error
	 * 
	 * @param formula
	 *            a full math expression string
	 * @return the result of the expression
	...
	public final static long parseLongExpression(final String formula) {
		return Math.round(parseMathExpression(
	 ...
	 * Variables are included as @x1, etc.. The given variable values list is 0
	 * based, so @x1 = vars[0]. Variable @xx will refer to current computed
	 * value. Rounds the result to a long Throws an exception on any parsing
	 * error
	 * 
	 * @param formula
	 *            a full math expression string
	 ...
	 * Returns the result of evaluating the given math expression. An expression
	 * can be a double or int number, or a full expression using ()+-/*?<>.
	 * Variable @xx will refer to current computed value. Rounds the result to
	 * an integer. Throws an exception on any parsing error
	 * 
	 * @param formula
	 *            a full math expression string
	 * @return the result of the expression
	 */
	public final static int parseIntExpression(final String formula)
			throws ArithmeticException {
		return (int) Math.round(parseMathExpression(
				new StreamTokenizer(new InputStreamReader(
						new ByteArrayInputStream(formula.getBytes()))), false,
				null, 0));
	}
	 ...
	 * Variables are included as @x1, etc.. The given variable values list is 0
	 * based, so @x1 = vars[0]. Variable @xx will refer to current computed
	 * value. Rounds the result to an integer. Throws an exception on any
	 * parsing error
	 * 
	 * @param formula
	 *            a full math expression string
	 ...
	 */
	public final static int parseIntExpression(final String formula,
			final double[] vars) throws ArithmeticException {
		return (int) Math.round(parseMathExpression(
				new StreamTokenizer(new InputStreamReader(
						new ByteArrayInputStream(formula.getBytes()))), false,
				vars, 0));
	 ...
	 * Returns the result of evaluating the given math expression. An expression
	 * can be a double or int number, or a full expression using ()+-/*?<>.
	 * Variable @xx will refer to current computed value. Throws an exception on
	 * any parsing error
	 * 
	 * @param formula
	 *            a full math expression string
	 * @return the result of the expression
	 */
	public final static double parseMathExpression(String formula)
			throws ArithmeticException {
		return parseMathExpression(new StreamTokenizer(new InputStreamReader(
				new ByteArrayInputStream(formula.getBytes()))), false, null, 0);
	}
	 ...
	 * Variables are included as @x1, etc.. The given variable values list is 0
	 * based, so @x1 = vars[0]. Variable @xx will refer to current computed
	 * value. Throws an exception on any parsing error
	 * 
	 * @param formula
	 *            a full math expression string
	 * @param vars
	 ...
	 * @return the result of the expression
	 */
	public final static double parseMathExpression(final String formula,
			final double[] vars) throws ArithmeticException {
		return parseMathExpression(new StreamTokenizer(new InputStreamReader(
				new ByteArrayInputStream(formula.getBytes()))), false, vars, 0);
	}
	 ...
	 * <br>
	 * <b>Usage:</b> lSize =
	 * WebIQBase.s_float(WebIQBase.getRes(AttStatsRes,"BlobSize"));
	 * 
	 * @param FLOAT
	 *            String to convert
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(isBool(CMD.substring(14)));
	 * 
	 * @param BOOL
	 *            Boolean value of string
	 ...
	 *            Integer value of string
	 * @param def
	 *            default value if the given string is not an int
	 * @return int Integer value of the string
	 */
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> int num=s_short(CMD.substring(14));
	 * 
	 * @param SHORT
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(isLong(CMD.substring(14)));
	 * 
	 * @param LONG
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(isInteger(CMD.substring(14)));
	 * 
	 * @param INT
	 *            Integer value of string
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(isFloat(CMD.substring(14)));
	 * 
	 * @param DBL
	...
	/**
	 * Returns a int representing either the given value, or the 2^ power of the
	 * comma separated values in the order they appear in the given string list.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(s_parseBitIntExpression(CMDS,CMD.substring(14)));
	 * 
	 * @param bits
	 *            the ordered string values from 0-whatever.
	 * @param val
	 *            the expression, or list of string values
	 * @return the int value, or 0
	 */
	public final static int s_parseBitIntExpression(final String[] bits,
			final String val) {
		return (int) s_parseBitLongExpression(bits, val);
	}

	/**
	 * Returns a long representing either the given value, or the 2^ power of
	 * the comma separated values in the order they appear in the given string
	 * list.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(s_parseBitLongExpression(CMDS,CMD.substring(14)));
	 * 
	 * @param bits
	 *            the ordered string values from 0-whatever.
	 * @param val
	 ...
	 */
	public final static long s_parseBitLongExpression(final String[] bits,
			String val) {
		if ((val == null) || (val.trim().length() == 0)
				|| (isMathExpression(val)))
	...
	/**
	 * Replaces the internal Random object with the one passed in. Intended to
	...
	/**
	 * Returns a long representing either the given value, or the index of the
	 * value in the order they appear in the given string list.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(s_parseListLongExpression(CMDS,CMD.substring(14)));
	 * 
	 * @param descs
	 *            the ordered string values from 0-whatever.
	 * @param val
	 ...
	 * @return the long value, or 0
	 */
	public final static long s_parseListLongExpression(final String[] descs,
			final String val) {
		if ((val == null) || (val.trim().length() == 0)
				|| (isMathExpression(val)))
			return s_parseLongExpression(val);
	...
	/**
	 * Returns a int representing either the given value, or the index of the
	 * value in the order they appear in the given string list.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(s_parseListIntExpression(CMDS,CMD.substring(14)));
	 * 
	 * @param descs
	 *            the ordered string values from 0-whatever.
	 * @param val
	 ...
	 * @return the int value, or 0
	 */
	public final static int s_parseListIntExpression(final String[] descs,
			final String val) {
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(isDouble(CMD.substring(14)));
	 * 
	 * @param DBL
	...
	/**
	 * Returns whether the given string is a number followed by 1 or more
	 * characters.
	 * 
	 * @param str
	 *            the string to check
	...
	/**
	 * Returns a number/string pair built from a given string, if it is a string
	 * with a number followed by one or more characters. Returns null if the
	 * given string does not match that criteria
	 * 
	 * @param str
	 *            the string to check
	 * @return a Math.Entry pair of the separated number/string.
	 */
	public final static Entry<Integer, String> getNumberFollowedByString(
			final String str) {
		if ((str == null) || (str.length() < 2))
			return null;
		if (!Character.isDigit(str.charAt(0)))
	...
	/**
	 * If the given string is 1 or more characters followed by decimal digits,
	 * this will return a Map.Entry with those parts separated. If the string is
	 * characters followed by a roman numeral digits, it will return a Map.Entry
	 * with those parts separated. Otherwise, it will return a Map.Entry with
	 * the string and a null integer.
	 * 
	 * @param str
	 *            the string to check
	 * @param romanOK
	 *            true to check for roman numerals, false to just check for
	 *            decimal
	 * @return the Map.Entry
	 */
	public final static Entry<String, Integer> getStringFollowedByNumber(
			final String str, final boolean romanOK) {
		...
		if ((str == null) || (str.length() < 1)) {
			...
			if (!Character.isDigit(str.charAt(dex))) {
				if ((!romanOK) || (!isRomanDigit(str.charAt(dex)))) {
					...
					number = Integer.valueOf(convertFromRoman(str
							.substring(dex + 1)));
				}
			} else {
				dex--;
