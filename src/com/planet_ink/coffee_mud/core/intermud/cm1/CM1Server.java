	...
	public void run() {
		while (!shutdownRequested) {
			try {
				...
				if ((page.getStr("BIND") != null)
						&& (page.getStr("BIND").trim().length() > 0))
					serverSocket.bind(new InetSocketAddress(InetAddress
							.getByName(page.getStr("BIND")), port));
				else
				...
				while (!shutdownRequested) {
					try {
						...
						while (it.hasNext()) {
							...
							if (key.isAcceptable()) {
								...
								if (channel != null) {
									RequestHandler handler = new RequestHandler(
											channel,
											page.getInt("IDLETIMEOUTMINS"));
									channel.configureBlocking(false);
									channel.register(servSelector,
											SelectionKey.OP_READ, handler);
									handlers.put(channel, handler);
									handler.sendMsg("CONNECTED TO "
											+ name.toUpperCase());
								}
								// sayHello (channel);
							}
							try {
								if (key.isReadable()) {
									RequestHandler handler = (RequestHandler) key
											.attachment();
									if ((!handler.isRunning())
											&& (!handler.needsClosing()))
										CMLib.threads()
												.executeRunnable(handler);
								}
							} finally {
								it.remove();
							}
