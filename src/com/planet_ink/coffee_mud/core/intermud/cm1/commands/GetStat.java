...
public class GetStat extends CM1Command {
	...
	protected static final String[] STATTYPES = { "SESSION", "MOB", "CHAR",
			"STATE", "PHYSICAL", "BASECHAR", "MAXSTATE", "BASESTATE",
			"BASEPHYSICAL", "PLAYERSTATS", "ITEM", "EXIT", "ROOM", "AREA" };
	protected static final String[] TYPESTYPE = { "P", "MP", "MP", "MP",
			"MPIREA", "MP", "MP", "MP", "MPIREA", "P", "I", "E", "R", "A" };

	protected static final String[] PHYSSTATS = { "EFFECT" };
	protected static final String[] PHYASTATS = { "BEHAVIOR" };
	protected static final String[] MOBASTATS = { "ABILITY", "FACTION",
			"EXPERTISE", "FOLLOWER" };
	...
	public boolean isAuthorized(MOB user, PhysicalAgent target) {
		if (target instanceof MOB) {
			if (CMLib.players().playerExists(target.Name()))
				return CMSecurity.isAllowed(user, user.location(),
	...
	public void run() {
		try {
			...
			if (x > 0) {
				...
				if ((stat.length() > 0)
						&& (!Character.isLetter(stat.charAt(0)))) {
			...
			if ((stat.length() == 0) || (!isAStat(P, mod, stat))) {
				req.sendMsg("[FAIL USAGE: GETSTAT " + type + " "
						+ CMParms.toStringList(getStatCodes(P, mod)) + "]");
				return;
			}
			if (mod instanceof Physical)
				...
				case 0: {
					if (rest.trim().length() == 0)
						...
						Ability A = ((Physical) mod).fetchEffect(CMath
								.s_int(rest));
						if (A == null)
			...
			if (mod instanceof PhysicalAgent)
				...
				case 0: {
					if (rest.trim().length() == 0)
						req.sendMsg("[OK "
								+ ((PhysicalAgent) mod).numBehaviors() + "]");
					else {
						Behavior A = ((PhysicalAgent) mod).fetchBehavior(CMath
								.s_int(rest));
						if (A == null)
			...
			if (mod instanceof MOB)
				...
				case 0: {
					if (rest.trim().length() == 0)
						...
						if (A == null)
							...
							req.sendMsg("[OK " + A.ID() + " " + A.proficiency()
									+ " " + A.text() + "]");
					}
					return;
				}
				case 1: {
					if (rest.trim().length() == 0) {
						...
						for (Enumeration<String> f = ((MOB) mod)
								.fetchFactions(); f.hasMoreElements();)
				...
				case 2: {
					if (rest.trim().length() == 0) {
						int numExpertises = 0;
						for (Enumeration<String> i = ((MOB) mod).expertises(); i
								.hasMoreElements(); numExpertises++)
							i.nextElement();
			...
			if (mod instanceof ItemPossessor)
				...
				case 0: {
					if (rest.trim().length() == 0)
						...
						Item I = ((ItemPossessor) mod).getItem(CMath
								.s_int(rest));
						if (I == null)
			...
			if (!UseGenBuilder(P, mod))
				...
				for (int i = 0; i < codes.length; i++)
					if (codes[i].equalsIgnoreCase(stat))
						if (P instanceof MOB)
							req.sendMsg("[OK "
									+ CMLib.coffeeMaker().getGenMobStat(
											(MOB) P, stat) + "]");
						else if (P instanceof Item)
							req.sendMsg("[OK "
									+ CMLib.coffeeMaker().getGenItemStat(
											(Item) P, stat) + "]");
	...
	public String getHelp(MOB user, PhysicalAgent target, String rest) {
		...
		if (mod == null)
			return "USAGE: " + getCommandWord() + " "
					+ CMParms.toStringList(getApplicableStatCodes(target));
		else if (rest != null)
			return "USAGE: " + getCommandWord() + " "
					+ rest.toUpperCase().trim() + " "
					+ CMParms.toStringList(getStatCodes(target, mod));
		else
			return "USAGE: " + getCommandWord() + " "
					+ CMParms.toStringList(getStatCodes(target, mod));
