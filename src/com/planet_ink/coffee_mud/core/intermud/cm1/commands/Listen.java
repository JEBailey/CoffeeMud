...
public class Listen extends CM1Command {
	...
	protected static enum STATTYPE {
		CHANNEL, LOGINS, MOB, ROOM, PLAYER, ABILITY, ITEM, TARGET, SOURCE, TOOL, TARGETCODE, SOURCECODE, OTHERSCODE, AREA, TARGETMASK, SOURCEMASK, OTHERSMASK
	}
	...
	protected class Listener implements MsgMonitor {
		...
		public boolean doesMonitor(final ListenCriterium crit, final Room room,
				final CMMsg msg) {
			...
			case LOGINS:
				return (msg.othersMinor() == CMMsg.TYP_LOGIN)
						|| (msg.othersMinor() == CMMsg.TYP_QUIT);
			case MOB:
		...
		private String minorDesc(int code) {
			String desc = CMMsg.Desc.getMSGDESC_TYPES().get(
		...
		public String messageToString(final CMMsg msg) {
			...
			case CHANNEL:
				return CMLib.coffeeFilter().fullOutFilter(null,
						CMLib.map().deity(), msg.source(), msg.target(),
						msg.tool(), msg.othersMessage(), false);
			case LOGINS:
			...
			default: {
				...
				str.append('\"').append(msg.source().Name()).append('\"')
						.append(' ');
				str.append(minorDesc(msg.sourceMinor())).append(' ');
				if (msg.target() != null)
					str.append('\"').append(msg.target().Name()).append('\"')
							.append(' ');
				else
					str.append("NULL ");
				str.append(minorDesc(msg.targetMinor())).append(' ');
				if (msg.tool() != null)
					str.append('\"').append(msg.tool().Name()).append('\"')
							.append(' ');
				else
					str.append("NULL ");
				str.append(minorDesc(msg.othersMinor())).append(' ');
				str.append(Integer.toString(msg.value())).append(' ');
				str.append(CMStrings.removeColors(CMLib.coffeeFilter()
						.fullOutFilter(null, CMLib.map().deity(), msg.source(),
								msg.target(), msg.tool(), msg.othersMessage(),
								false)));
				return str.toString();
			}
			}
	...
	public boolean securityCheck(MOB user, ListenCriterium crit) {
		...
		case CHANNEL: {
			if (crit.parmInt < 0)
				return false;
			ChannelsLibrary.CMChannel chan = CMLib.channels().getChannel(
					crit.parmInt);
			if (!CMLib.masking().maskCheck(chan.mask, user, true))
				return false;
			Set<ChannelsLibrary.ChannelFlag> flags = chan.flags;
			if (flags.contains(ChannelsLibrary.ChannelFlag.CLANONLY)
					|| flags.contains(ChannelsLibrary.ChannelFlag.CLANALLYONLY))
				return CMSecurity.isAllowedAnywhere(user,
		...
		case OTHERSMASK:
			...
			return true;
		case PLAYER:
			return CMSecurity.isAllowedEverywhere(user,
	...
	public boolean parameterCheck(MOB user, ListenCriterium crit) {
		...
		case OTHERSMASK:
			...
			return true;
		case MOB:
			return (crit.obj instanceof MOB)
					&& (!CMLib.players().playerExists(crit.obj.Name()));
		case ROOM:
	...
	public List<ListenCriterium> getCriterium(String rest) throws IOException {
		...
		while (rest.length() > 0) {
			...
			try {
				...
				req.sendMsg("[FAIL " + codeStr + " NOT "
						+ CMParms.toStringList(STATTYPE.values()) + "]");
				return null;
			}
			String parm = null;
			x = rest.indexOf(' ');
			...
			ListenCriterium crit = new ListenCriterium(
					STATTYPE.valueOf(codeStr), req.getTarget(), parm);
	...
	public String getHelp(MOB user, PhysicalAgent target, String rest) {
		return "USAGE: " + getCommandWord() + " <" + getCommandWord()
				+ "ER NAME> " + CMParms.toStringList(STATTYPE.values());
	}
}
