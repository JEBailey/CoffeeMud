	...
	public void run() {
		try {
			...
			if ((stat.length() == 0) || (!isAStat(P, mod, stat))) {
				req.sendMsg("[FAIL USAGE: SETSTAT " + type + " "
				...
				case 0: {
					if (adjuster == '-') {
						...
						if (A == null)
							...
							M.setLocation(CMLib.map().roomLocation(
									(Physical) mod));
							try {
								A.invoke(M, CMParms.parse(restValue),
										(Physical) mod, true, 0);
							} catch (Exception e) {
							}
							M.destroy();
			...
			if (mod instanceof PhysicalAgent)
				...
				case 0: {
					if (adjuster == '-') {
						Behavior A = ((PhysicalAgent) mod)
								.fetchBehavior(firstValue);
						if (A != null) {
					...
					if (F == null)
						...
						if (f < Integer.MAX_VALUE) {
							if (CMath.isInteger(restValue))
								((MOB) mod).addFaction(F.factionID(),
										f - CMath.s_int(restValue));
							else
								((MOB) mod).removeFaction(F.factionID());
							req.sendMsg("[OK -" + F.factionID() + " "
									+ restValue + "]");
						} else
							req.sendMsg("[FAIL " + F.factionID()
									+ " NOT FOUND]");
					} else {
						int def = 0;
						if (CMath.isInteger(restValue))
						...
						req.sendMsg("[OK " + F.factionID() + " "
								+ ((MOB) mod).fetchFaction(F.factionID()) + "]");
					}
					return;
				}
				case 2: {
					ExpertiseLibrary.ExpertiseDefinition def = CMLib
							.expertises().findDefinition(firstValue, true);
					if (def == null)
						def = CMLib.expertises().findDefinition(firstValue,
								false);
					if (def == null)
						req.sendMsg("[FAIL " + firstValue + " NOT EXIST]");
					else if (adjuster == '-') {
			...
			if (mod instanceof Room)
				...
				case 0: {
					if (adjuster == '-') {
						...
						if (M == null)
							M = CMLib.map().findFirstInhabitant(
									((Room) mod).getArea().getMetroMap(), null,
									restValue, 10);
						if (M == null)
			...
			if (!UseGenBuilder(P, mod))
				...
				for (int i = 0; i < codes.length; i++)
					if (codes[i].equalsIgnoreCase(stat))
						if (P instanceof MOB)
							CMLib.coffeeMaker().setGenMobStat((MOB) P, stat,
									value);
						else if (P instanceof Item)
							CMLib.coffeeMaker().setGenItemStat((Item) P, stat,
									value);
