...
public class CommandHandler implements Runnable {
	...
	private static final void AddCommand(Class<? extends CM1Command> c)
						...
						if (F.getName().endsWith(".class")
							...
							String name = packageName
									+ F.getName().substring(0,
											F.getName().length() - 6);
							try {
								AddCommand((Class<? extends CM1Command>) CMClass
										.instance().loadClass(name, true));
							} catch (Exception e) {
								e.printStackTrace();
							}
						}
		...
		if (cmd.length() > 0) {
			try {
				if (cmd.equalsIgnoreCase("HELP")) {
					if (rest.length() > 0) {
						Class<? extends CM1Command> commandClass = commandList
								.get(rest.toUpperCase().trim());
						CM1Command command = CM1Command.newInstance(
								commandClass, req, rest);
						if ((command == null)
								|| (!command.passesSecurityCheck(req.getUser(),
										req.getTarget())))
						...
						for (String cmdWord : commandList.keySet()) {
							...
							CM1Command command = CM1Command.newInstance(
									commandClass, req, "");
							if (command.passesSecurityCheck(req.getUser(),
									req.getTarget()))
								str.append(" " + cmdWord);
						}
						str.append("]");
				...
				Class<? extends CM1Command> commandClass = commandList.get(cmd
						.toUpperCase().trim());
