...
public class RequestHandler implements CMRunnable {
	...
	public RequestHandler(SocketChannel chan, int maxIdleMillis)
			throws IOException {
		super();
	...
	public void sendMsg(String msg) throws IOException {
		if ((msg.startsWith("[OK") || msg.startsWith("[FAIL") || msg
				.startsWith("[MESSAGE")))
	...
	public void run() {
		...
		synchronized (this) {
			try {
				...
				while (chan.isConnected() && (chan.isOpen())
						&& (chan.read(buffer) > 0)) {
					buffer.flip();
					int containIndex = -1;
					for (int i = 0; i < buffer.limit(); i++)
						if ((containIndex = CMParms.containIndex(buffer,
								markBlocks, i)) >= 0) {
							int containIndexLength = markBlocks[containIndex].length;
							workingBuffers.remove(buffer);
							if (i > 0) {
								ByteBuffer prevBuf = ByteBuffer
