...
/**
 * IMC2 version 0.10 - an inter-mud communications protocol Copyright (C) 1996 &
 * 1997 Oliver Jowett <oliver@randomly.org>
 * 
 * IMC2 Gold versions 1.00 though 2.00 are developed by MudWorld. Copyright (C)
 * 1999 - 2002 Haslage Net Electronics (Anthony R. Haslage)
 * 
 * IMC2 MUD-Net version 3.10 is developed by Alsherok and Crimson Oracles
 * Copyright (C) 2002 Roger Libiez ( Samson ) Additional code Copyright (C) 2002
 * Orion Elder Registered with the United States Copyright Office TX 5-555-584
 * 
...
public final class IMC2Driver extends Thread {
	...
	public void shutdown() {
		...
		int i = 0;
		while (((++i) < 60)
				&& ((!c_thread.isShutdown) || (!c_thread2.isShutdown) || (!isShutdown)))
			try {
				sleep(1000);
			} catch (Exception ex) {
			}
	...
	final boolean imc_connect_to() {
		...
		buf = "PW " + imc_name.trim() + " " + this_imcmud.clientpw
				+ " version=" + IMC_VERSION + " autosetup "
	...
	final public boolean imc_startup(boolean force, String loginName,
			String host, String email, String web, String hub, int port,
			String passclient, String passsrvr, List<CMChannel> imc2Channels) {
		if (imc_active != IA_NONE) {
		...
		imc_siteinfo.port = CMath.s_int(CMParms.parse(
				CMProps.getVar(CMProps.Str.MUDPORTS)).elementAt(0));
	...
	final public String do_imcinfo() {
		...
		return "IMC Statistics:\\n\\r\\n"
				...
				+ "\\n\\r"
				+ "Site Host           :"
				+ ((host.length() > 0) ? host : sa.getLocalAddress()
						.getHostName())
				+ "\\n\\r"
				+ "Site Port           :"
				+ imc_siteinfo.port
				+ "\\n\\r"
				+ ((imc_siteinfo.www.length() > 0) ? "Web Address         :"
						+ imc_siteinfo.www + "\\n\\r" : "")
				+ ((imc_siteinfo.www.length() > 0) ? "Email               :"
						+ imc_siteinfo.email + "\\n\\r" : "")
		...
		if (st.countTokens() < 5) {
			if (st.countTokens() == 4) {
				...
				if (tmp.equalsIgnoreCase("autosetup")) {
					 ...
					 * if(st.nextToken().equalsIgnoreCase("accept")) { String
					 * networkName = st.nextToken(); }
					 */
				}
			} else {
	...
	final String generate2(PACKET p) {
		...
		temp = p.i.from + " " + p.i.sequence + " " + newpath + " " + p.type
				+ " " + p.i.to + " " + printkeys(p);
	...
	public void imc_register_call_out(int hbeat, String function_name,
			Object param) {
					...
					for (int k = 0; k < funcs.length; k++) {
						...
						if (m_name.equals(fun)) {
							try {
								...
								tracef(0, "imc: call_out failed with error: "
										+ e.toString());
							}
						}
					}
	...
	final String update_wholist() {
		StringBuffer str = new StringBuffer("Players on "
	...
	final void imc_recv_whois(imc_char_data from, String sender, int level) {
		...
		imc_addkey(out, "text", "imcpfind " + from.name + "@" + imc_name
				+ " is "
				+ (CMLib.flags().isInTheGame(M, false) ? "online" : "offline")
				+ ".");
	...
	final void imc_recv_chat(imc_char_data d, String from, String channel,
		...
		String str = channelColor + "^<CHANNEL \"" + channelName + "\"^>"
				+ mob.name() + " " + channelName + "(S) '" + text
				+ "'^</CHANNEL^>^N^.";
		if (emote > 0) {
			if (emote == 1)
				str = channelColor + "^<CHANNEL \"" + channelName + "\"^>["
						+ channelName + "] " + from + " " + text
						+ "^</CHANNEL^>^N^.";
			else
				str = channelColor + "^<CHANNEL \"" + channelName + "\"^>["
						+ channelName + "] " + text + "^</CHANNEL^>^N^.";
		}

		msg = CMClass.getMsg(mob, null, null, CMMsg.NO_EFFECT, null,
				CMMsg.NO_EFFECT, null, CMMsg.MASK_CHANNEL
						| (CMMsg.TYP_CHANNEL + channelInt), str);
	...
	final void imc_recv_ping_reply(imc_char_data d, String from, String path) {
		...
		StringBuffer text = new StringBuffer("Traceroute information for "
				+ from + "\n\r");
	...
	final void imc_recv_update(String from, String chan, String owner,
			String operators, String policy, String invited, String excluded) {
		IMC_CHANNEL c;
		String mud;
	...
	final public void exec_commands(PACKET p) {
		...
		if (!to_mud.equalsIgnoreCase(imc_name) && !to_mud.equals("*")) {
			tracef(8, "Message was for " + to_mud + ", not " + imc_name
					+ " -- rejecting!");
			return;
		}

		d.name = imc_playerof(d.name);
		tracef(8, "Received message was sent to " + d.name + ", " + p.type
				+ ", " + CMParms.toStringList(p.value));
		...
		if (p.type.equals("ice-msg-b")) {
			...
			imc_recv_chat(d, p.i.from, imc_getkey(p, "channel", "ICHAT"),
					imc_getkey(p, "text", ""),
					CMath.s_int(imc_getkey(p, "emote", "0")));
		}
	...
	final public void imc_read_from_socket(BufferedReader in) {
		try {
			...
			if ((errMsg != null) && (!shutdown)
					&& (errMsg.toUpperCase().indexOf("CONNECTION") >= 0)) {
				imc_active = IA_NONE;
				tracef(1, "Waiting " + (reconnectBackoffTime / 1000)
				...
				this.imc_startup(true, imc_name, imc_siteinfo.host,
						imc_siteinfo.email, imc_siteinfo.www,
						this_imcmud.hubname, this_imcmud.port,
	...
	final public void imc_write_to_socket(DataOutputStream out) {
		try {
			...
			this.imc_startup(true, imc_name, imc_siteinfo.host,
					imc_siteinfo.email, imc_siteinfo.www, this_imcmud.hubname,
					this_imcmud.port, this_imcmud.clientpw,
					this_imcmud.serverpw, rebuildChannelMap());
		}
	}
		...
		final public void run() {
			...
			while (!shutdown) {
				...
				if ((System.currentTimeMillis() - timeSinceLastPing) > (30 * 60 * 1000)) {
					...
					// everything is taken care of from here. detect disconnect,
	...
	final public void run_imcpinfo(String name, String mudname, String who,
			int level, int invis) {
	...
	final public void imc_send_who(String name, String mudname, String type,
			int level, int invis) {
		imc_char_data test = new imc_char_data();
		test.name = name;
		test.level = level;
	...
	final public String imc_send_tell(String from, String to, String text,
			int level, int invis) {
	...
	final public String imc_send_reply(String from, String text, int level,
			int invis) {
		imc_char_data chr = new imc_char_data();
		chr.name = from;
		chr.level = level;
	...
	final public String imc_send_chat(String from, String to, String text,
			int level, int emote) {
		imc_char_data chr = new imc_char_data();
		chr.name = from;
		chr.level = level;
	...
	final String imc_send_chat(imc_char_data from, String to, String argument,
		...
		/*
		 * if (isreply > 0) imc_addkeyi(out, "isreply", isreply);
		 */
		...
		String chatText = from.name + "@" + this.imc_name + " [" + chan + "] "
				+ " " + text + "^?";
