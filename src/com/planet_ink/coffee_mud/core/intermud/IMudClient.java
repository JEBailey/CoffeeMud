...
public class IMudClient implements I3Interface {
	...
	public void i3who(MOB mob, String mudName) {
		...
		if (i3online() && Intermud.isAPossibleMUDName(mudName)) {
			...
			imc2.imc_send_who(mob.name(), imc2.getIMC2Mud(mudName).name, "who",
					mob.phyStats().level(), 0);
		else {
	...
	public void imc2mudInfo(MOB mob, String parms) {
		...
		if ((parms == null) || (parms.length() == 0)
				|| (imc2.getIMC2Mud(parms) == null)) {
			mob.tell("You must specify a mud name.");
			return;
		}
		imc2.imc_send_who(mob.name(), imc2.getIMC2Mud(parms).name, "info", mob
				.phyStats().level(), 0);
	}
	...
	public void i3chanwho(MOB mob, String channel, String mudName) {
		...
		if ((channel == null) || (channel.length() == 0)
				|| (Intermud.getRemoteChannel(channel).length() == 0)) {
	...
	public void i3tell(MOB mob, String tellName, String mudName, String message) {
		...
		if (i3online() && Intermud.isAPossibleMUDName(mudName)) {
			...
			tellName = CMStrings.capitalizeAndLower(tellName) + "@"
					+ imc2.getIMC2Mud(mudName).name;
			mob.tell("^CYou tell " + tellName + " '" + message + "'^?");
			if (mob.playerStats() != null)
				mob.playerStats().addTellStack(
						"You tell " + tellName + " '" + message + "'");
			imc2.imc_send_tell(mob.name(), tellName, message, 0, CMLib.flags()
					.isInvisible(mob) ? 1 : 0);
		} else {
			mob.tell(mudName + " is an unknown mud.");
			return;
		...
		if (i3online() && Intermud.getRemoteChannel(channelName).length() > 0) {
			...
			if ((message.startsWith(":") || message.startsWith(","))
				...
				if (S != null) {
					...
					if ((msg.target() != null) && (msg.targetMessage() != null)
							&& (msg.targetMessage().length() > 0))
						ck.message_target = socialFixOut(CMStrings
								.removeColors(msg.targetMessage()));
					if ((msg.othersMessage() != null)
							&& (msg.othersMessage().length() > 0))
						ck.message = socialFixOut(CMStrings.removeColors(msg
								.othersMessage()));
					else
						ck.message = socialFixOut(CMStrings.removeColors(msg
								.sourceMessage()));
			...
			if ((message.startsWith(":") || message.startsWith(","))
				...
				if (S != null) {
					...
					if ((msg.othersMessage() != null)
							&& (msg.othersMessage().length() > 0))
						message = CMLib.coffeeFilter().fullOutFilter(null,
								CMClass.sampleMOB(), mob2, msg.target(), null,
								CMStrings.removeColors(msg.othersMessage()),
								false);
					else
						message = CMLib.coffeeFilter().fullOutFilter(null,
								CMClass.sampleMOB(), mob2, msg.target(), null,
								CMStrings.removeColors(msg.sourceMessage()),
								false);
					if (message.toUpperCase().startsWith(
							(mob.Name() + "@" + imc2.imc_name).toUpperCase()))
						message = message.substring(
								(mob.Name() + "@" + imc2.imc_name).length())
								.trim();
					emote = 2;
				}
				emote = 1;
	...
	public String getMudInfo(I3Mud mudToShow) {
		...
		buf.append(CMStrings.padRight("Address", 10) + ": " + mudToShow.address
				+ "\n\r");
		buf.append(CMStrings.padRight("Port", 10) + ": "
				+ mudToShow.player_port + "\n\r");
		buf.append(CMStrings.padRight("Admin@", 10) + ": "
				+ mudToShow.admin_email + "\n\r");
		buf.append(CMStrings.padRight("Base", 10) + ": "
				+ mudToShow.base_mudlib + "\n\r");
	...
	public List<I3Mud> mudFinder(String parms) {
		...
		for (I3Mud m : l.values()) {
			if ((m.state < 0)
					&& (CMLib.english().containsString(m.mud_name, parms)))
				muds.add(m);
		}
		return muds;
	}
	...
	public void i3mudInfo(MOB mob, String parms) {
		...
		if (muds.size() == 0)
			...
			for (I3Mud mudToShow : muds) {
				buf.append(CMStrings.padRight("Name", 10) + ": "
						+ mudToShow.mud_name + "\n\r");
				buf.append(CMStrings.padRight("Address", 10) + ": "
						+ mudToShow.address + "\n\r");
				buf.append(CMStrings.padRight("Port", 10) + ": "
						+ mudToShow.player_port + "\n\r");
				buf.append(CMStrings.padRight("Admin@", 10) + ": "
				...
				buf.append(CMStrings.padRight("Base", 10) + ": "
						+ mudToShow.base_mudlib + "\n\r");
				buf.append(CMStrings.padRight("MudLib", 10) + ": "
						+ mudToShow.mudlib + "\n\r");
				buf.append(CMStrings.padRight("Type", 10) + ": "
						+ mudToShow.mud_type + "\n\r");
				buf.append(CMStrings.padRight("Driver", 10) + ": "
						+ mudToShow.driver + "\n\r");
				buf.append(CMStrings.padRight("Status", 10) + ": "
						+ mudToShow.status + "\n\r");
			}
	...
	public void giveIMC2MudList(MOB mob) {
		...
		for (int v = 0; v < V.size(); v++) {
			...
			buf.append("[" + CMStrings.padRight(m.name, 15) + "]["
					+ CMStrings.padRight(m.version, 30) + "] "
					+ CMStrings.padRight(m.network, 13) + " ("
					+ CMStrings.padRight(m.hub, 10) + ")\n\r");
		}
		mob.session().wraplessPrintln(buf.toString());
	}
	...
	public void giveI3MudList(MOB mob) {
		...
		if (list != null) {
			for (I3Mud m : list.getMuds().values()) {
				if (m.state < 0) {
					...
					for (int v = 0; v < V.size(); v++) {
						...
						if (m2.mud_name.toUpperCase().compareTo(
								m.mud_name.toUpperCase()) > 0) {
							V.insertElementAt(m, v);
							done = true;
							break;
						}
			...
			for (int v = 0; v < V.size(); v++) {
				...
				buf.append("[" + CMStrings.padRight(m.mud_name, 20) + "]["
						+ CMStrings.padRight(m.base_mudlib, 20) + "] "
						+ m.address + " (" + m.player_port + ")\n\r");
			}
		}
		mob.session().wraplessPrintln(buf.toString());
	}
	...
	public void giveI3ChannelsList(MOB mob) {
		...
		if (list != null) {
			...
			for (Enumeration e = l.elements(); e.hasMoreElements();) {
				...
				if (c.type == 0)
					buf.append("[" + CMStrings.padRight(c.channel, 20) + "] "
							+ c.owner + "\n\r");
			}
		}
		mob.session().wraplessPrintln(buf.toString());
	...
	public void giveIMC2ChannelsList(MOB mob) {
		...
		buf.append(CMStrings.padRight("Name", 22)
				+ CMStrings.padRight("Policy", 25)
				+ CMStrings.padRight("Owner", 20) + "\n\r");
