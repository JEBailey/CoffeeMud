...
public class IMudPeer implements PersistentPeer {
		...
		if (myobj instanceof Intermud) {
			try {
				...
				ObjectInputStream in = new ObjectInputStream(
						new ByteArrayInputStream(F.raw()));
				Object newobj;
				...
				Log.errOut("IMudPeer", "Unable to read /resources/ppeer."
						+ myID);
			}
		}
		isRestoring = false;
	...
	public void save() throws PersistenceException {
		if (myobj instanceof Intermud) {
			try {
				...
				out.flush();
				bout.flush();
				new CMFile("::resources/ppeer." + myID, null).saveRaw(bout
						.toByteArray());
				out.close();
				bout.close();
			} catch (Exception e) {
	...
	public void setPersistent(Persistent ob) {
		myobj = ob;
		myID = ob.getClass().getName()
				.substring(ob.getClass().getName().lastIndexOf('.') + 1);
	}
