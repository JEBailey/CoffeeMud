...
/**
 * The Intermud class is the central focus of incoming and outgoing Intermud 3
 * packets. It creates the link to the I3 router, handles reconnection, and
 * routing of packets to the mudlib. The mudlib is responsible for providing two
 * specific objects to interface with this object: an implementation of
 * com.planet_ink.coffee_mud.core.intermud.i3.packets.ImudServices an
 * implementation of
 * com.planet_ink.coffee_mud.core.intermud.i3.persist.PersistentPeer To start up
 * the Intermud connection, call the class method setup(). The class itself
 * creates an instance of itself and serves as a way to interface to the rest of
 * the mudlib. When the mudlib needs to send a packet, it sends it through a
 * class method which then routes it to the proper instance of Intermud.
 * 
 * @author George Reese
 * @version 1.0
...
public class Intermud implements Runnable, Persistent, Serializable {
	...
	/**
	 * Sends a packet to the router. The packet must be a valid subclass of
	 * com.planet_ink.coffee_mud.core.intermud.i3.packets.Packet. This method
	 * will then route the packet to the currently running Intermud instance.
	 * 
	 * @param p
	 *            an instance of a subclass of
	 *            com.planet_ink.coffee_mud.core.intermud.i3.packets.Packet
	...
	/**
	 * Creates the initial link to an I3 router. It will handle subsequent
	 * reconnections as needed for as long as the mud process is running.
	 * 
	 * @param imud
	 *            an instance of the mudlib implementation of
	 *            com.planet_ink.coffee_mud
	 *            .core.intermud.i3.packets.ImudServices
	 * @param peer
	 *            and instance of the mudlib implementation of
	 *            com.planet_ink.coffee_mud
	 *            .core.intermud.i3.packets.IntermudPeer
	 * @see com.planet_ink.coffee_mud.core.intermud.i3.packets.ImudServices
	 * @see com.planet_ink.coffee_mud.core.intermud.i3.persist.PersistentPeer
	 */
	...
	static public String registerFakeChannel(String c) {
		if ((!isConnected())
				|| (thread.intermud.getLocalChannel(c).length() > 0))
			return "";
	...
	static public String removeFakeChannel(String c) {
		...
		if ((mask.equalsIgnoreCase("+FAKE")) && (thread.intermud.delChannel(c)))
			return name;
		return "";
	}

	/**
	 * Returns a String representing the local channel name for the specified
	 * remote channel by calling the ImudServices implementation of
	 * getLocalChannel().
	 * 
	 * @param c
	 *            the remote channel name
	...
	/**
	 * Returns a String representing the remote channel name for the specified
	 * local channel by calling the ImudServices implementation of
	 * getRemoteChannel().
	 * 
	 * @param c
	...
	/**
	 * Determines whether or not the specified mud is up. You may pass user
	 * entered mud names, as this method will take the time to convert to a
	 * canonical name.
	 * 
	 * @param mud
	 *            the name of the mud being checked
	...
	private Intermud(final ImudServices imud, PersistentPeer p) {
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (V2.size() >= 3)
				name_servers.add(new NameServer(V2.get(0), CMath.s_int(V2
						.get(1)), V2.get(2)));
		}
		...
		muds = new MudList(-1);
		if ((save_thread == null)
				|| (!CMLib.threads().isTicking(save_thread,
						Tickable.TICKID_SUPPORT))) {
					...
					try {
						if (CMSecurity.isDisabled(CMSecurity.DisFlag.I3)) {
							...
							long ellapsedTime = System.currentTimeMillis()
									- imud.getLastPacketReceivedTime();
										...
										"No I3 Ping sent in "
												+ CMLib.time()
														.date2EllapsedTime(
																...
																false)
												+ ". Connected="
												+ Intermud.isConnected());
								CMLib.threads().executeRunnable(new Runnable() {
									public void run() {
										try {
											I3Server.shutdown();
											CMLib.hosts().get(0)
													.executeCommand("START I3");
											Log.errOut(
													"I3SaveTick",
													"Restarted your Intermud system.  To stop receiving these messages, DISABLE the I3 system.");
										} catch (Exception e) {
										}
									}
								});
	...
	private synchronized void channelList(Vector packet) {
		...
		synchronized (channels) {
			channels.setChannelListId(((Integer) packet.elementAt(6))
					.intValue());
			while (keys.hasMoreElements()) {
				...
				if (ob instanceof Integer) {
					...
					if (info.elementAt(1) instanceof Integer)
						...
						Log.errOut(
								"InterMud",
								"Received unexpected channel-reply: "
										+ CMParms.toStringList((List) info
												.elementAt(1)));
					addChannel(c);
				}
			}
				...
				for (int i = 0; i < name_servers.size(); i++) {
					...
					try {
						...
						send("({\"startup-req-3\",5,\""
								...
								+ muds.getMudListId()
								+ ","
								+ channels.getChannelListId()
								+ ","
								+ intermud.getMudPort()
								+ ",0,0,\""
								+ intermud.getMudVersion()
								+ "\",\""
								+ intermud.getMudVersion()
								+ "\",\""
								+ intermud.getMudVersion()
								+ "\",\"CoffeeMud\","
								+ "\""
								+ intermud.getMudState()
								+ "\",\""
								+ CMProps.getVar(CMProps.Str.ADMINEMAIL)
										.toLowerCase()
								+ "\",(["
								+ "\"who\":1,\"finger\":1,\"channel\":1,\"tell\":1,\"locate\":1,\"auth\":1,]),([]),})");
					} catch (java.io.IOException e) {
						connectionStatuses.addElement(currentRouter.ip + ": "
								+ currentRouter.port + ": " + e.getMessage());
						continue;
					}
					connected = true;
					input_thread = new Thread(Thread.currentThread()
							.getThreadGroup(), this);
					input_thread.setDaemon(true);
					input_thread.setName("I3Client:" + currentRouter.ip + "@"
							+ currentRouter.port);
					input_thread.start();
					...
					while (e.hasMoreElements()) {
						...
						send("({\"channel-listen\",5,\""
								+ intermud.getMudName() + "\",0,\""
								+ currentRouter.name + "\",0,\"" + chan
								+ "\",1,})");
					}
					Log.sysOut("Intermud3", "I3 client connection: "
							+ currentRouter.ip + "@" + currentRouter.port);
					break;
				}
				if (!connected)
					for (int e = 0; e < connectionStatuses.size(); e++)
						Log.errOut("Intermud",
								(String) connectionStatuses.elementAt(e));
			}
		} catch (Exception e) {
			try {
		...
		while (connected && (!shutdown)) {
			...
			if (CMSecurity.isDisabled(CMSecurity.DisFlag.I3)) {
				continue;
					...
					&& (System.currentTimeMillis() - lastPingSentTime) > (30 * 60 * 1000)) {
				lastPingSentTime = System.currentTimeMillis();
				try {
				...
				long ellapsedTime = System.currentTimeMillis()
						- intermud.getLastPacketReceivedTime();
				if (ellapsedTime > (60 * 60 * 1000)) // one hour
				{
					Log.errOut(
							"Intermud",
							"No I3 Ping received in "
									+ CMLib.time().date2EllapsedTime(
											ellapsedTime, TimeUnit.SECONDS,
											false) + ". Connected="
									+ Intermud.isConnected());
					CMLib.threads().executeRunnable(new Runnable() {
						public void run() {
			...
			try {
				...
				if (len > 65536) {
					...
					Log.errOut("Intermud", "Got illegal packet: " + skipped
							+ "/" + len + " bytes.");
					continue;
				}
				...
				while (!shutdown) {
					try { // please don't compress this again
						...
						CMLib.s_sleep(1000);
						Log.errOut("Intermud",
								"Timeout receiving packet sized " + len);
						continue;
					}
				}
				str = new String(tmp);
			...
			if (type.equals("channel-m") || type.equals("channel-e")
					|| type.equals("channel-t")) {
				try {
	...
	/**
	 * Adds a channel to the channel list. This does not subscribe the mud to
	 * that channel. In order to subscribe, the channel needs to be added to the
	 * ImudServices implementation's getChannels() method.
	 * 
	 * @param c
	 *            the channel to add to the list of known channels
