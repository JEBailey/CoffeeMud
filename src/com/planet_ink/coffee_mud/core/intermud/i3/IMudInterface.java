...
public class IMudInterface implements ImudServices, Serializable {
	...
	public List<CMChannel> channels = new XVector<CMChannel>(new CMChannel[] {
			new CMChannel("I3CHAT", "diku_chat", ""),
			new CMChannel("I3GOSSIP", "diku_immortals", ""),
			new CMChannel("GREET", "diku_code", "") });

	private static volatile long lastPacketReceivedTime = System
			.currentTimeMillis();
	...
	{ "^K", "%^BLACK%^", "\033[0;0;30m" }, // Black
			...
			{ "", "%^B_YELLOW%^", "\033[43m" }, // Yellow, which may as well be
												// orange since ANSI doesn't do
												// that
			{ "", "%^B_BLUE%^", "\033[44m" }, // Blue
			...
			{ "^k^*", "%^BLACK%^%^BOLD%^%^FLASH%^", "\033[1;5;30m" }, // Dark
																		// Grey
			...
			{ "^c^*", "%^CYAN%^%^BOLD%^%^FLASH%^", "\033[1;5;36m" }, // Light
																		// Blue
			{ "^w^*", "%^WHITE%^%^BOLD%^%^FLASH%^", "\033[1;5;37m" } // White
	};

	public IMudInterface(String Name, String Version, int Port,
			String i3status, List<CMChannel> Channels) {
		if (Name != null)
			name = Name;
		if (i3status != null)
			i3state = i3status;
	...
	/**
	 * Handles an incoming I3 packet asynchronously. An implementation should
	 * make sure that asynchronously processing the incoming packet will not
	 * have any impact, otherwise you could end up with bizarre behaviour like
	 * an intermud chat line appearing in the middle of a room description. If
	 * your mudlib is not prepared to handle multiple threads, just stack up
	 * incoming packets and pull them off the stack during your main thread of
	 * execution.
	 * 
	 * @param packet
	 *            the incoming packet
	...
	public void receive(Packet packet) {
		...
		case Packet.CHAN_TARGET: {
			...
			if ((ck.sender_mud != null)
					&& (ck.sender_mud.equalsIgnoreCase(getMudName())))
				return;
			...
			if (channelColor.length() == 0)
				channelColor = "^Q";
			ck.message = fixColors(CMProps.applyINIFilter(ck.message,
					CMProps.Str.CHANNELFILTER));
			if (ck.message_target != null)
				ck.message_target = fixColors(CMProps.applyINIFilter(
						ck.message_target, CMProps.Str.CHANNELFILTER));
			...
			if (ck.type == Packet.CHAN_TARGET) {
				if ((ck.target_mud != null)
						&& (ck.target_mud.equalsIgnoreCase(getMudName())))
					targetMOB = CMLib.players().getLoadPlayer(ck.target_name);
				if ((ck.target_visible_name != null) && (ck.target_mud != null)
						&& (targetMOB == null)) {
					killtargetmob = true;
					targetMOB = CMClass.getFactoryMOB();
					targetMOB.setName(ck.target_visible_name + "@"
							+ ck.target_mud);
				...
				targmsgs = CMProps.applyINIFilter(targmsgs,
						CMProps.Str.EMOTEFILTER);
				String str = channelColor + "^<CHANNEL \"" + channelName
						+ "\"^>[" + channelName + "] " + msgs
						+ "^</CHANNEL^>^N^.";
				String str2 = channelColor + "^<CHANNEL \"" + channelName
						+ "\"^>[" + channelName + "] " + targmsgs
						+ "^</CHANNEL^>^N^.";
				msg = CMClass.getMsg(mob, targetMOB, null, CMMsg.NO_EFFECT,
						null, CMMsg.MASK_CHANNEL
								| (CMMsg.TYP_CHANNEL + channelCode), str2,
						CMMsg.MASK_CHANNEL | (CMMsg.TYP_CHANNEL + channelCode),
						str);
				...
				String str = channelColor + "^<CHANNEL \"" + channelName
						+ "\"^>[" + channelName + "] " + msgs
						+ "^</CHANNEL^>^N^.";
				msg = CMClass.getMsg(mob, null, null, CMMsg.NO_EFFECT, null,
						CMMsg.NO_EFFECT, null, CMMsg.MASK_CHANNEL
								| (CMMsg.TYP_CHANNEL + channelCode), str);
			} else {
				String str = channelColor + "^<CHANNEL \"" + channelName
						+ "\"^>" + mob.name() + " " + channelName + "(S) '"
						+ ck.message + "'^</CHANNEL^>^N^.";
				msg = CMClass.getMsg(mob, null, null, CMMsg.NO_EFFECT, null,
						CMMsg.NO_EFFECT, null, CMMsg.MASK_CHANNEL
								| (CMMsg.TYP_CHANNEL + channelCode), str);
			}
			...
			for (Session S : CMLib.sessions().localOnlineIterable()) {
				MOB M = S.mob();
				if (((channelInt < 0) || CMLib.channels().mayReadThisChannel(
						mob, false, S, channelInt))
						&& (M.location() != null)
		...
		case Packet.LOCATE_QUERY: {
			...
			if (smob != null) {
				...
				name = null;
			if (name != null) {
				LocateReplyPacket lpk = new LocateReplyPacket(lk.sender_name,
						lk.sender_mud, name, 0, stat);
				try {
					lpk.send();
				} catch (Exception e) {
		...
		case Packet.LOCATE_REPLY: {
			...
			if (smob != null)
				smob.tell(fixColors(lk.located_visible_name) + "@"
						+ fixColors(lk.located_mud_name) + " (" + lk.idle_time
						+ "): " + fixColors(lk.status));
		}
			break;
		case Packet.FINGER_REQUEST: {
			...
			if (M == null)
				pkt = new ErrorPacket(lk.sender_name, lk.sender_mud,
						"unk-user", "User " + lk.target_name
								+ " is not known here.", "0");
			else {
				FingerReply fpkt = new FingerReply(lk.sender_name,
						lk.sender_mud);
				pkt = fpkt;
				fpkt.e_mail = "0";
				...
				fpkt.loginout_time = CMLib.time().date2String(
						M.playerStats().lastDateTime());
				fpkt.real_name = "0"; // don't even know this
				if (M.titledName().equals(M.name()))
					fpkt.title = "An ordinary "
							+ M.charStats().displayClassName();
				else
					fpkt.title = M.titledName();
				fpkt.visible_name = M.name();
				fpkt.extra = M.name() + " is a " + M.charStats().raceName()
						+ " " + M.charStats().displayClassName();
			}
			try {
				pkt.send();
		...
		case Packet.FINGER_REPLY: {
			...
			if (smob != null) {
				...
				if ((lk.visible_name.length() > 0)
						&& (!lk.visible_name.equals("0")))
					response.append("^H")
							.append(CMStrings.padRight("Name", 10))
							.append(": ^N").append(lk.visible_name)
							.append("\n\r");
				if ((lk.title.length() > 0) && (!lk.title.equals("0")))
					response.append("^H")
							.append(CMStrings.padRight("Title", 10))
							.append(": ^N").append(lk.title).append("\n\r");
				if ((lk.real_name.length() > 0) && (!lk.real_name.equals("0")))
					response.append("^H")
							.append(CMStrings.padRight("Real Name", 10))
							.append(": ^N").append(lk.real_name).append("\n\r");
				if ((lk.e_mail.length() > 0) && (!lk.e_mail.equals("0")))
					response.append("^H")
							.append(CMStrings.padRight("Email", 10))
							.append(": ^N").append(lk.e_mail).append("\n\r");
				if ((lk.loginout_time.length() > 0)
						&& (!lk.loginout_time.equals("0")))
					response.append("^H")
							.append(CMStrings.padRight("Logged", 10))
							.append(": ^N").append(lk.loginout_time)
							.append("\n\r");
				if ((lk.ip_time.length() > 0) && (!lk.ip_time.equals("0")))
					response.append("^H")
							.append(CMStrings.padRight("IP Time", 10))
							.append(": ^N").append(lk.ip_time).append("\n\r");
				if ((lk.extra.length() > 0) && (!lk.extra.equals("0")))
					response.append("^H")
							.append(CMStrings.padRight("Extra", 10))
							.append(": ^N").append(lk.extra).append("\n\r");
		...
		case Packet.MAUTH_REQUEST: {
			...
			MudAuthReply pkt = new MudAuthReply(lk.sender_mud,
		...
		case Packet.MAUTH_REPLY: {
			...
			if (lk.sender_mud.equalsIgnoreCase(I3Server.getMudName())) {
				...
				Log.sysOut("I3", "MUD " + lk.sender_mud
						+ " replied to my mud-auth with key " + lk.key + ".");
		}
			break;
		...
		case Packet.CHAN_WHO_REP: {
			...
			if (smob != null) {
				StringBuffer buf = new StringBuffer("\n\rListening on "
						+ wk.channel + "@" + fixColors(wk.sender_mud) + ":\n\r");
				Vector V = wk.who;
				if (V.size() == 0)
					buf.append("Nobody!");
				else
		...
		case Packet.CHAN_WHO_REQ: {
			...
			for (Session S : CMLib.sessions().localOnlineIterable()) {
				MOB M = S.mob();
				if ((CMLib.channels().mayReadThisChannel(M, false, S,
						channelInt))
						&& (M != null)
		...
		case Packet.WHO_REQUEST: {
			...
			for (Session S : CMLib.sessions().localOnlineIterable()) {
				...
				if ((smob != null) && (smob.soulMate() != null))
					smob = smob.soulMate();
				if ((!S.isStopped()) && (smob != null) && (!smob.amDead())
					...
					whoV2.addElement(smob.charStats().displayClassLevel(smob,
							true));
		...
		case Packet.TELL: {
			...
			if (smob != null) {
				...
				mob.setLocation(getUniversalRoom());
				tk.message = fixColors(CMProps.applyINIFilter(tk.message,
						CMProps.Str.SAYFILTER));
	...
	/**
	 * Given a I3 channel name, this method should provide the local name for
	 * that channel. Example:
	 * 
	 * <PRE>
	 * if (str.equals(&quot;imud_code&quot;))
	 * 	return &quot;intercre&quot;;
	 * </PRE>
	 * 
	 * @param str
	 *            the remote name of the desired channel
	...
	/**
	 * Given a local channel name, this method should provide the local mask for
	 * that channel. Example:
	 * 
	 * <PRE>
	 * if (str.equals(&quot;ICODE&quot;))
	...
	/**
	 * Given a remote channel name, returns the mask required. Example:
	 * 
	 * <PRE>
	 * if (str.equals(&quot;intercre&quot;))
	 * 	return &quot;&quot;;
	 * </PRE>
	 * 
	 * @param str
