...
/**
 * The Server class uses exactly one thread ServerThread object during the
 * course of program execution. This thread loops until the Server class tells
 * it to shut down. The loop is executed in the thread's run() method.
 * 
...
public class ServerThread implements Tickable {
	...
	protected ServerThread(String mname, int mport, ImudServices imud) {
		mud_name = mname;
		port = mport;
		intermuds = imud;
	}
	...
	protected synchronized ServerObject copyObject(String str)
		...
		try {
			...
			throw new ObjectLoadException("Failed to load object: "
					+ e.getMessage());
		}
		return ob;
	}

	protected synchronized ServerObject findObject(String str)
			throws ObjectLoadException {
		ServerObject ob;
	...
	/**
	 * While the mud is running, this method repeats the following steps over
	 * and over:
	 * <ul>
	 * <LI>Check for pending user input and trigger user commands
	 * <LI>Check for pending object events and execute them
	 * <LI>Check for incoming user connections and create an interactive object
	 * for each.
	 * </ul>
	 */
	public void start() {
		...
		try {
			Intermud.setup(
					intermuds,
					(PersistentPeer) Class
							.forName(
									"com.planet_ink.coffee_mud.core.intermud.i3.IMudPeer")
							.newInstance());
		} catch (Exception e) {
			Log.errOut("I3Server", e);
			return;
