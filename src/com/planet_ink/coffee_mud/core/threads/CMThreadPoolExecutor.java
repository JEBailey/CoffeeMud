...
public class CMThreadPoolExecutor extends ThreadPoolExecutor {
	...
	protected static class CMArrayBlockingQueue<E> extends
	...
	public CMThreadPoolExecutor(String poolName, int corePoolSize,
			int maximumPoolSize, long keepAliveTime, TimeUnit unit,
			long timeoutMins, int queueSize) {
		super(corePoolSize, maximumPoolSize, keepAliveTime, unit,
				new CMArrayBlockingQueue<Runnable>(queueSize));
	...
	public void execute(Runnable r) {
		try {
			...
			if ((rejectCount > 0)
				...
				Log.warnOut(rejectCount + " Pool_" + poolName,
						"Threads rejected.");
				rejectCount = 0;
			}
	...
	public Collection<CMRunnable> getTimeoutOutRuns(int maxToKill) {
		...
		synchronized (active) {
			try {
				for (Iterator<Runnable> e = active.keySet().iterator(); e
						.hasNext();) {
					...
					if (runnable instanceof CMRunnable) {
						...
						if (cmRunnable.activeTimeMillis() > timeoutMillis) {
							if (timedOut.size() >= maxToKill) {
								...
								if (leastWorstOffender != null) {
									if (cmRunnable.activeTimeMillis() < leastWorstOffender
											.activeTimeMillis())
										continue;
									else
