...
public class ServiceEngine implements ThreadEngine {
	public static final long STATUS_ALLMISCTICKS = Tickable.STATUS_MISC
			| Tickable.STATUS_MISC2 | Tickable.STATUS_MISC3
			| Tickable.STATUS_MISC4 | Tickable.STATUS_MISC5
			| Tickable.STATUS_MISC6;
	...
	protected CMThreadPoolExecutor getPoolExecutor(String threadGroupName) {
		...
		threadPools[threadGroupNum] = new CMThreadPoolExecutor(
				sessionThreadGroupName, minThreads, maxThreads,
				CMProps.getTickMillis() * 2, TimeUnit.MILLISECONDS,
				(LONG_TICK_TIMEOUT / 60000), 1024);
		threadPools[threadGroupNum].setThreadFactory(new CMThreadFactory(
				sessionThreadGroupName));
	...
	public Runnable findRunnableByThread(final Thread thread) {
		if ((thread == null) || (threadPools == null))
			return null;
		final Runnable possR = (thread instanceof CMFactoryThread) ? ((CMFactoryThread) thread)
				.getRunnable() : null;
		for (int i = 0; i < threadPools.length; i++) {
			...
			if ((executor.getThreadFactory() instanceof CMThreadFactory)
					&& (!((CMThreadFactory) executor.getThreadFactory())
	...
	public TickClient startTickDown(Tickable E, int tickID, long TICK_TIME,
			int numTicks) {
		return startTickDown(CMLib.map().getOwnedThreadGroup(E), E, tickID,
				TICK_TIME, numTicks);
	}

	public synchronized TickClient startTickDown(ThreadGroup group, Tickable E,
			int tickID, long TICK_TIME, int numTicks) {
	...
	protected boolean suspendResumeTicking(Tickable E, int tickID,
			boolean suspend) {
		for (final Iterator<TickableGroup> e = tickGroups(); e.hasNext();) {
			final TickableGroup almostTock = e.next();
			for (final Iterator<TickClient> set = almostTock.getTickSet(E,
					tickID); set.hasNext();)
	...
	public String systemReport(final String itemCode) {
		...
		if (itemCode.equalsIgnoreCase("totalMOBMillis"))
			...
			for (Iterator<TickableGroup> e = tickGroups(); e.hasNext();) {
				final TickableGroup almostTock = e.next();
				int tick = 0;
				for (Iterator<TickClient> et = almostTock.tickers(); et
						.hasNext();) {
					TickClient C = et.next();
					if (C.getTickTotal() == 0)
						continue;
					Long avg = Long.valueOf(C.getMilliTotal()
							/ C.getTickTotal());
					int i = 0;
					for (; i < list.size(); i++) {
						...
						if (avg.longValue() >= t.first.longValue()) {
							list.add(
									i,
									new Triad<Long, Integer, Integer>(avg,
					...
					if ((list.size() == 0)
							|| ((i >= list.size()) && (list.size() < total)))
						list.add(new Triad<Long, Integer, Integer>(avg, Integer
								.valueOf(group), Integer.valueOf(tick)));
		...
		if (itemCode.equalsIgnoreCase("freeMemory"))
			...
			return ""
					+ CMLib.english().returnTime(
							System.currentTimeMillis()
									- CMSecurity.getStartTime(), 0);
			...
			int xend = xstart;
			while ((xend < itemCode.length())
					&& (Character.isDigit(itemCode.charAt(xend))))
				xend++;
			int threadNum = CMath.s_int(itemCode.substring(xstart, xend));
			int curThreadNum = 0;
			for (Enumeration<CMLibrary> e = CMLib.libraries(); e
					.hasMoreElements();) {
				...
				if (serviceClient != null) {
					if (curThreadNum == threadNum) {
						...
						if (instrCode.equalsIgnoreCase("MilliTotalTime"))
							return CMLib.english().returnTime(
									serviceClient.getMilliTotal(), 0);
						if (instrCode.equalsIgnoreCase("MiliTotalTime"))
							return CMLib.english().returnTime(
									serviceClient.getMilliTotal(), 0);
						if (instrCode
								.equalsIgnoreCase("MilliTotalTimePlusAverage")
								|| instrCode
										.equalsIgnoreCase("MiliTotalTimePlusAverage"))
							return CMLib.english().returnTime(
									serviceClient.getMilliTotal(),
									serviceClient.getTickTotal());
	...
	public void suspendResumeRecurse(CMObject O, boolean skipEmbeddedAreas,
			boolean suspend) {
		if (O instanceof Item) {
			Item I = (Item) O;
			suspendResumeTicking(I, -1, suspend);
			if ((I instanceof SpaceShip) && (!skipEmbeddedAreas))
				suspendResumeRecurse(((SpaceShip) I).getShipArea(), true,
						suspend);
			...
			for (int i = 0; i < R.numInhabitants(); i++)
				suspendResumeRecurse(R.fetchInhabitant(i), skipEmbeddedAreas,
						suspend);
			...
			for (Enumeration<Room> r = A.getFilledProperMap(); r
					.hasMoreElements();)
				suspendResumeRecurse(r.nextElement(), skipEmbeddedAreas,
						suspend);
		}
	}
	...
	public void deleteAllTicks(Tickable ticker) {
		...
		if (ticker instanceof Room) {
			...
			for (Iterator<TickableGroup> e = tickGroups(); e.hasNext();) {
				almostTock = e.next();
				try {
					for (Iterator<TickClient> i = almostTock.tickers(); i
							.hasNext();) {
						C = i.next();
						E2 = C.getClientObject();
						if (map.isHere(E2, (Room) ticker))
							almostTock.delTicker(C);
								...
								&& (map.isHere(((Ability) E2).affecting(),
										(Room) ticker)))
							almostTock.delTicker(C);
						else
							continue;
	...
	public String tickInfo(String which) {
		...
		if (which.equalsIgnoreCase("tickGroupSize"))
			...
			if ((group >= 0) && (group < allTicks.size())) {
				List<TickableGroup> enumeratedTicks = new XVector<TickableGroup>(
						allTicks);
		...
		if (which.toLowerCase().startsWith("tickername")) {
			...
			return ((C.getClientObject() == null) ? "" : ("" + C
					.getClientObject().getTickStatus()));
			...
			if (C.getLastStopTime() >= C.getLastStartTime())
				return CMLib.english().returnTime(
						C.getLastStopTime() - C.getLastStartTime(), 0);
			return CMLib.english().returnTime(
					System.currentTimeMillis() - C.getLastStartTime(), 0);
	...
	public boolean shutdown() {
		...
		while (allTicks.size() > 0) {
			...
			if (tock != null) {
				CMProps.setUpAllLowVar(CMProps.Str.MUDSTATUS,
						"Shutting down...shutting down Service Engine: killing "
								+ tock.getName() + ": " + tock.getStatus());
				tock.shutdown();
				allTicks.remove(tock);
			}
			try {
		...
		CMProps.setUpAllLowVar(CMProps.Str.MUDSTATUS,
				"Shutting down...shutting down Service Engine: " + ID()
						+ ": thread shutdown");
		...
		for (Iterator<TickableGroup> e = tickGroups(); e.hasNext();) {
			...
			for (Iterator<TickClient> i = almostTock.tickers(); i.hasNext();) {
				...
				if ((C.getClientObject() != null)
						&& (C.getClientObject().name().toUpperCase()
								.indexOf(name) >= 0)
		...
		if (obj instanceof Environmental) {
			if (CMath.bset(code, Tickable.STATUS_BEHAVIOR)
				...
				if ((b >= 0) && (b < ((MOB) obj).numScripts())) {
					...
					codeWord += " (" + CMStrings.limit(S.getScript(), 20)
							+ ": " + S.getTickStatus();
				}
				return codeWord;
	...
	public void insertOrderDeathInOrder(DVector DV, long lastStart, String msg,
			TickableGroup tock) {
		if (DV.size() > 0)
	...
	public void debugDumpStack(final String ID, Thread theThread) {
		// I wish Java had compiler directives. Would be great to un-comment
		// this for 1.5 JVMs
		...
		if (theThread == null)
			...
			for (int i = 0; i < s.length; i++)
				dump.append("\n   " + s[i].getClassName() + ": "
						+ s[i].getMethodName() + "(" + s[i].getFileName()
						+ ": " + s[i].getLineNumber() + ")");
		}
	...
	public final void checkHealth() {
		...
		try {
			...
			for (Iterator<TickableGroup> e = tickGroups(); e.hasNext();) {
				...
				if ((almostTock.isAwake())
					...
					final Tickable ticker = (tickClient != null) ? tickClient
							.getClientObject() : null;
					if (tickClient == null)
						insertOrderDeathInOrder(orderedDeaths, 0,
								"LOCKED GROUP " + almostTock.getName() + ": "
										+ almostTock.getStatus()
										+ "! No further information.",
								almostTock);
					else if ((!CMath.bset(tickClient.getTickID(),
							Tickable.TICKID_LONGERMASK))
							|| (almostTock.getLastStopTime() < longerDateTime)) {
						if (ticker == null)
							...
							if (ticker instanceof Environmental)
								str = new StringBuffer("LOCKED GROUP "
										+ almostTock.getName()
										+ ": "
										+ almostTock.getStatus()
										+ ": "
										+ ticker.name()
										+ " ("
										+ ((Environmental) ticker).ID()
										+ ") @"
										+ CMLib.time().date2String(
												tickClient.getLastStartTime())
										+ ", status(" + code + " (" + codeWord
										+ "), tickID " + tickClient.getTickID());
							else
								str = new StringBuffer("LOCKED GROUP "
										+ almostTock.getName()
										+ ": "
							...
							if ((ticker instanceof MOB)
									&& (((MOB) ticker).location() != null))
								msg = str.toString() + " in "
										+ ((MOB) ticker).location().roomID();
							else if ((ticker instanceof Item)
									&& (((Item) ticker).owner() != null)
									&& (((Item) ticker).owner() instanceof Room))
								msg = str.toString()
										+ " in "
										+ ((Room) ((Item) ticker).owner())
												.roomID();
							else if ((ticker instanceof Item)
									&& (((Item) ticker).owner() != null)
									&& (((Item) ticker).owner() instanceof MOB))
								msg = str.toString()
										+ " owned by "
										+ ((MOB) ((Item) ticker).owner())
												.name();
								...
								msg = str.toString();
							insertOrderDeathInOrder(orderedDeaths,
									tickClient.getLastStartTime(), msg,
									almostTock);
						}
					}
					// no isDEBUGGING check -- just always let her rip.
					debugDumpStack(Thread.currentThread().getName(),
							almostTock.getCurrentThread());
				}
			}
		...
		for (int i = 0; i < orderedDeaths.size(); i++)
			Log.errOut(Thread.currentThread().getName(),
					(String) orderedDeaths.elementAt(i, 2));

		setSupportStatus("killing tick groups.");
		for (int x = 0; x < orderedDeaths.size(); x++) {
			TickableGroup almostTock = (TickableGroup) orderedDeaths.elementAt(
					x, 3);
			...
			try {
				...
				Log.errOut("Error " + t.getMessage() + " killing "
						+ almostTock.getName());
			}
			...
			for (int i = 0; i < tockClients.size(); i++) {
				...
				startTickDown(c.getClientObject(), c.getTickID(),
						c.getTotalTickDown());
			}
		}
			...
			try {
				...
				synchronized (allTicks) {
					for (final TickableGroup T : allTicks) {
						if (!T.isAwake()
								&& (!getPoolExecutor(T.getThreadGroupName())
										.isActiveOrQueued(T))) {
			...
			supportClient = startTickDown(
					...
					new Tickable() {
						...
						public boolean tick(Tickable ticking, int tickID) {
							if ((!CMSecurity
									.isDisabled(CMSecurity.DisFlag.UTILITHREAD))
									&& (!CMSecurity
											.isDisabled(CMSecurity.DisFlag.THREADTHREAD))) {
					...
					}, Tickable.TICKID_MISCELLANEOUS
							| Tickable.TICKID_SOLITARYMASK,
					MudHost.TIME_UTILTHREAD_SLEEP, 1);
		if (drivingThread == null) {
			drivingThread = new Thread(Thread.currentThread().getThreadGroup(),
					this, "Scheduler"
