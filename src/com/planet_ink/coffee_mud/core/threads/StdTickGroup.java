...
public class StdTickGroup implements TickableGroup, Cloneable {
	...
	public StdTickGroup(String a_name, long sleep, String threadGroupName,
			boolean isSolitary) {
	...
	public StdTickGroup(ThreadEngine theEngine, long sleep,
			String threadGroupName, boolean isSolitary) {
		name = "Tick." + (tickObjReference + 1);
		tickObjectCounter = tickObjReference++;
		myEngine = theEngine;
		tickTime = sleep;
	...
	public Iterator<TickClient> getTickSet(final Tickable T, final int tickID) {
		...
		if (tickID < 0)
			subSet.addAll(tickers.subSet(new StdTickClient(T, 0, 0), true,
					new StdTickClient(T, 0, Integer.MAX_VALUE), true));
		else
			subSet.addAll(tickers.subSet(new StdTickClient(T, 0, tickID), true,
					new StdTickClient(T, 0, tickID), true));
	...
	public boolean contains(final Tickable T, final int tickID) {
		...
		return tickers.subSet(new StdTickClient(T, 0, -1), true,
				new StdTickClient(T, 0, Integer.MAX_VALUE), true).size() > 0;
	}
	...
	public String getStatus() {
		...
		return ticker.ID()
				...
				+ ticker.name()
				+ ": "
				+ ((myEngine != null) ? myEngine.getTickStatusSummary(ticker)
						: "null");
	}
