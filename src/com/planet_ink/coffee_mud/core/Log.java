...
/**
 * One of the oldest classes in CoffeeMud -- ye olde logger. Features include
 * date formatting, the standard set of log channels, plus optional numeric
 * levels within those in case some kinds of info are more important than
 * others. Also manages multiple log files if necessary, up to a limit defined
...
public class Log extends java.util.logging.Logger {
	...
	private final PrintWriter systemOutWriter = new PrintWriter(System.out,
			true);
	/** always to systemout */
	...
	private static class Conf {
		...
		public Conf(Target target, int maxLevel, int maxLogs, int maxLines,
				int maxBytes) {
			this.target = target;
	...
	public static enum Type {
		error, help, debug, info, warning, kills, combat, access;
	...
	public static final void shareWith(final char threadCode) {
		final char tc = Thread.currentThread().getThreadGroup().getName()
				.charAt(0);
		if (logs[threadCode] != null)
	...
	/**
	 * Optional method to determine if message is a masked out throwable message
	 * type.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> if(isMaskedErrMsg(errException.getMessage()))
	 * 
	 * @param str
	 *            the message
	...
	private final PrintWriter rollLog(final PrintWriter writer)
			throws IOException {
		if (writer == this.fileOutWriter[0]) {
			WRITTEN.remove(writer);
			writer.close();
			FileOutputStream fileOut = this.openLogFile(logName, false,
					this.numberOfFWLogs);
			...
			for (final Type type : Type.values()) {
				...
				if ((config.target == Target.OWNFILE)
						&& (config.writers[0][0] == writer)) {
					WRITTEN.remove(writer);
					writer.close();
					FileOutputStream fileOut = this.openLogFile(
							getLogFilename(type), false, config.maxLogs);
					PrintWriter newPW = new PrintWriter(fileOut, true);
	...
	private final PrintWriter writeBytes(final Type type, final Conf config,
			final PrintWriter writer, final String str) {
		try {
			writer.println(str);
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> PrintWriter W=getWriter("BOTH");
	 * 
	 * @param type
	 *            log type
	 * @param config
	 ...
	 */
	private final PrintWriter getWriter(final Type type, final Conf config,
			final int priority) {
		...
		case OWNFILE: {
			try {
				FileOutputStream fileStream = this.openLogFile(logName + "_"
						+ type.toString().toLowerCase(), false, config.maxLogs);
				final PrintWriter[] writer = new PrintWriter[] { new PrintWriter(
						fileStream, true) };
				for (int i = 0; i < 10; i++)
					writers[i] = writer;
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> PrintWriter W=makeConfig("BOTH");
	 * 
	 * @param type
	 *            LogType
	 * @param code
	 *            ON, OFF, BOTH, OWNFILE, FILE
	 * @param defaultNumberOfLogs
	 *            default number of log files
	 * @return Conf the config
	 */
	private final Conf makeConfig(final Type type, String code,
			int defaultNumberOfLogs) {
		...
		if (x > 0) {
			...
			if (Character.isDigit(numStr.charAt(0))) {
				...
				while ((x < numStr.length())
						&& (Character.isDigit(numStr.charAt(x)))) {
					x++;
				}
		...
		if (t == null)
			t = Target.OFF;
		if (t == Target.OWNFILE)
			return new Conf(t, maxLevel, maxNumberOfLogs, maxNumberOfEntries,
					maxNumberOfBytes);
		else
	...
	private final Conf getConfig(final Type type) {
		...
		if (t == null) {
			t = makeConfig(
					type,
					System.getProperty("LOG." + LOGNAME + "_"
							+ type.toString().toUpperCase().trim()), 1);
			CONFS.put(type, t);
		}
		return t;
	}
	...
	/**
	 * Reset all of the log files ON, OFF, FILE, BOTH <br>
	 * <br>
	 * <b>Usage:</b> CMProps.initLog(Log.LogType.info,"ON");
	 * 
	 * @param type
	 *            the log to set the code for
	 * @param code
	 *            the code
	 */
	public final void configureLog(final Type type, final String code) {
		System.setProperty("LOG." + LOGNAME + "_"
				+ type.toString().toUpperCase(), code.toUpperCase().trim());
	 ...
	 * Reset all of the log files ON, OFF, FILE, BOTH <br>
	 * <br>
	 * <b>Usage:</b> CMProps.initLog(Log.LogType.info,"OWNFILE",20);
	 * 
	 * @param type
	 *            the log to set the code for
	 * @param code
	 *            the code
	 * @param numberOfLogs
	 *            if code = "OWNFILE", then how many back logs to keep
	 */
	public final void configureLog(final Type type, final String code,
	...
	private final FileOutputStream openLogFile(String logName,
			final boolean append, final int numberOfLogs) throws IOException {
		if (logName.toLowerCase().endsWith(".log")
				|| logName.toLowerCase().endsWith(".txt"))
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> configureLogFile("mud",5);
	 * 
	 * @param logFilePath
	 *            maximum name of files
	 * @param numberOfLogs
	 *            maximum number of files
	 */
	public final void configureLogFile(final String logFilePath,
			final int numberOfLogs) {
		...
		try {
			final FileOutputStream fileStream = openLogFile(logName, false,
					numberOfLogs);
			fileOutWriter = new PrintWriter[] { new PrintWriter(fileStream,
					true) };
			System.setErr(new PrintStream(fileStream));
		} catch (IOException e) {
	...
	/**
	 * Returns an internally managed log reader class to make reading lines from
	 * the log slightly easier
	 * 
			...
			public String nextLine() {
				if (reader == null) {
					try {
						...
						standardOut(Type.error, "Log", e.getMessage(),
								Integer.MIN_VALUE);
						return null;
					}
				}
	 ...
	 * <br>
	 * <br>
	 * <b>Usage:</b> SysOutWriter.println(getLogHeader(S,LogType.info,Module,
	 * Message));
	 * 
	 * @param type
	 *            type of information
	 ...
	 */
	public final static String makeLogEntry(final Type type,
			final String module, final String message) {
		final StringBuilder header = new StringBuilder(dateFormat.format(Long
				.valueOf(System.currentTimeMillis()))).append(" ");
		header.append(type.getSixChars());
		if (module.length() >= 14)
			header.append(module.substring(0, 14));
		else
			header.append(module).append(
					SPACES15.substring(0, 14 - module.length()));
	...
	public static final void debugOut(final Throwable e) {
		l().shortExOut(Type.debug, Thread.currentThread().getName(),
				Integer.MIN_VALUE, e);
	}

	public static final void errOut(final Throwable e) {
		l().standardExOut(Type.error, Thread.currentThread().getName(),
				Integer.MIN_VALUE, e);
	}
	...
	public static final void sysOut(final String Out, final String Message,
			final int priority) {
		infoOut(Out, Message);
	}
	...
	public static final void debugOut(final String Module,
	...
	public static final void killsOut(final String Module,
			final String Message, final int priority) {
		l().standardOut(Type.kills, Module, Message, priority);
	}

	public static final void combatOut(final String Module,
			final String Message, final int priority) {
		l().standardOut(Type.combat, Module, Message, priority);
	}

	public static final void accessOut(final String Module,
			final String Message, final int priority) {
		l().standardOut(Type.access, Module, Message, priority);
	}

	public static final void debugOut(final String Module, final int priority,
			final Exception e) {
		l().shortExOut(Type.debug, Module, priority, e);
	}

	public static final void errOut(final String Module, final int priority,
			final Throwable e) {
		l().standardExOut(Type.error, Module, priority, e);
	}

	public static final void warnOut(final String Module, final int priority,
			final Throwable e) {
		l().standardExOut(Type.error, Module, priority, e);
	}
	...
	/**
	 * Handles long exception logging entries. Sends them to System.out, the log
	 * file, or nowhere.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> standardExOut(Thread.currentThread().getName(),Out);
	 * 
	 * @param type
	 ...
	 *            The module to print
	 * @param e
	 *            The exception whose string one wishes to print
	 */
	public final void standardExOut(final Type type, final String Module,
			final int priority, final Throwable e) {
	...
	/**
	 * Handles error logging entries. Sends them to System.out, the log file, or
	 * nowhere.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b>
	 * shortExOut(LogType.info,Thread.currentThread().getName(),Out);
	 * 
	 * @param type
	 *            The type of channel
	 * @param Module
	 ...
	 *            The exception whose string one wishes to print
	 */
	public final void shortExOut(final Type type, final String Module,
			final int priority, final Throwable e) {
		final Conf conf = getConfig(type);
	...
	/**
	 * Handles raw info logging entries. Sends them to System.out, the log file,
	 * or nowhere.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> rawStandardOut(LogType.info,"REQ-OUT:"+REQ);
	 * 
	 * @param type
	 *            The type of message
	 * @param line
	 *            The message to print
	 * @param priority
	 *            The priority of the message, high is less priority, 0=always
	 */
	public final void rawStandardOut(final Type type, final String line,
			final int priority) {
		final Conf conf = getConfig(type);
		final PrintWriter outWriter = getWriter(type, conf, priority);
		if (outWriter != null) {
	...
	/**
	 * Handles debug logging entries. Sends them to System.out, the log file, or
	 * nowhere.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b>
	 * standardOut(LogType.info,Thread.currentThread().getName(),Out);
	 * 
	 * @param type
	 *            The type of writer
	 * @param module
	 ...
	 *            The priority of the message, high is less priority, 0=always
	 */
	private final void standardOut(final Type type, final String module,
			final String msg, final int priority) {
		final Conf conf = getConfig(type);
		final PrintWriter outWriter = getWriter(type, conf, priority);
		if (outWriter != null) {
			synchronized (outWriter) {
	...
	/**
	 * Handles debug timing entries. Sends them to System.out, the log file, or
	 * nowhere.
	 * 
	 * <br>
	 * <br>
	 * <b>Usage:</b> timeOut(LogType.info,Thread.currentThread().getName(),Out);
	 * 
	 * @param type
	 *            Channel name
	 * @param module
	 ...
	 *            The priority of the message, high is less priority, 0=always
	 */
	public final void timeOut(final Type type, final String module,
			final String msg, final int priority) {
		final Conf conf = getConfig(type);
		final PrintWriter outWriter = getWriter(type, conf, priority);
		if (outWriter != null) {
			synchronized (outWriter) {
				...
				final String line = makeLogEntry(type, module,
	...
	/**
	 * totally optional, this is the list of maskable error message types.
	 * Useful for internet apps
	 */
	private final static String[] maskErrMsgs = { "broken pipe",
			"reset by peer", "socket closed", "connection abort",
			"connection reset", "network is unreachable", "jvm_recv",
			"timed out", "stream closed", "no route to host",
	...
	private String toStringList(Object[] os) {
		...
		StringBuilder str = new StringBuilder((os[0] == null) ? "null"
				: os[0].toString());
	...
	public void config(final String msg) {
		l().standardOut(Type.debug, Thread.currentThread().getName(), msg,
				Integer.MIN_VALUE);
	}
	...
	public void entering(final String sourceClass, final String sourceMethod) {
		l().standardOut(Type.debug, toModuleName(sourceClass), sourceMethod,
				Integer.MIN_VALUE);
	}
	...
	public void entering(final String sourceClass, final String sourceMethod,
			final Object param1) {
		l().standardOut(Type.debug, toModuleName(sourceClass),
				sourceMethod + ": " + param1, Integer.MIN_VALUE);
	}
	...
	public void entering(final String sourceClass, final String sourceMethod,
			final Object[] params) {
		l().standardOut(Type.debug, toModuleName(sourceClass),
				sourceMethod + ": " + toStringList(params), Integer.MIN_VALUE);
	}
	...
	public void exiting(final String sourceClass, final String sourceMethod,
			final Object result) {
		l().standardOut(Type.debug, toModuleName(sourceClass),
				sourceMethod + ": " + result, Integer.MIN_VALUE);
	}

	// Log a FINE message.
	@Override
	public void fine(final String msg) {
		l().standardOut(Type.access, Thread.currentThread().getName(), msg,
				Integer.MIN_VALUE);
	}
	...
	public void info(final String msg) {
		standardOut(Type.info, Thread.currentThread().getName(), msg,
				Integer.MIN_VALUE);
	}
	...
	public void log(final Level level, final String msg) {
		standardOut(getTypeFromLevel(level), Thread.currentThread().getName(),
	...
	public void log(final Level level, final String msg, final Object param1) {
		standardOut(getTypeFromLevel(level), Thread.currentThread().getName(),
				msg + ": " + ((param1 == null) ? "null" : param1.toString()),
				Integer.MIN_VALUE);
	}
	...
	public void log(final Level level, final String msg, final Object[] params) {
		standardOut(getTypeFromLevel(level), Thread.currentThread().getName(),
				msg + ": " + (toStringList(params)), Integer.MIN_VALUE);
	}
	...
	public void log(final Level level, final String msg, final Throwable thrown) {
		if (thrown == null)
			...
			standardExOut(getTypeFromLevel(level), toModuleName(msg),
					Integer.MIN_VALUE, thrown);
	}
	...
	public void logp(final Level level, final String sourceClass,
		...
		standardOut(getTypeFromLevel(level), toModuleName(sourceClass),
				sourceMethod + ": " + msg, Integer.MIN_VALUE);
	}
	...
	public void logp(final Level level, final String sourceClass,
		...
		standardOut(getTypeFromLevel(level), toModuleName(sourceClass),
				sourceMethod + ": " + msg + ": " + (toStringList(params)),
				Integer.MIN_VALUE);
	}
	...
	public void logp(final Level level, final String sourceClass,
		...
		if (thrown == null)
			...
			standardExOut(getTypeFromLevel(level), toModuleName(sourceClass),
					Integer.MIN_VALUE, thrown);
	}
	...
	public void logrb(final Level level, final String sourceClass,
			...
			final String msg, Object param1) {
	...
	public void severe(final String msg) {
		standardOut(Type.error, Thread.currentThread().getName(), msg,
				Integer.MIN_VALUE);
	}
	...
	public void throwing(final String sourceClass, final String sourceMethod,
		...
		standardExOut(Type.error, toModuleName(sourceClass), Integer.MIN_VALUE,
				thrown);
	}
	...
	public void warning(final String msg) {
		standardOut(Type.warning, Thread.currentThread().getName(), msg,
				Integer.MIN_VALUE);
	}
}
