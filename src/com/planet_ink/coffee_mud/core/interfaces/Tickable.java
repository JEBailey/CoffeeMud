 ...
 */
/**
 * This interface is implemented by any object which wishes to get periodic
 * thread time from the threads engine. Almost all CoffeeMud objects implement
 * this interface
 * 
 * @author Bo Zimmerman
 * 
...
public interface Tickable extends CMObject {
	...
	/**
	 * A coded status for this object during the period where its tick method is
	 * being called. The statis is defined, at least in part, by constants in
	 * this interface STATUS_*. STATUS_NOT should be returned when the objects
	 * tick method is not currently in execution. It should never return
	 ...
	 * How often it is called depends on the parameters passed to the threadding
	 * engine when it is submitted for thread access. Typically the period is
	 * once per TIME_TICK period, but that is determined when the object is
	 * submitted to the thread engine.
	 * 
	 * @see Tickable
	 * @see com.planet_ink.coffee_mud.core.threads.ServiceEngine
	 * @see TickableGroup
	 * @param ticking
	 *            a reference to this Tickable object
	 * @param tickID
	 *            the TICKID_ constant describing this periodic call, as defined
	 *            in Tickable
	 * @return true always, unless this object no longer wishes to ever tick
	 *         again, in which case false
	 */
	...
	/**
	 * a tick status constant representing the state of just starting its tick
	 * access
	 */
