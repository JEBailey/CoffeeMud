...
public class Resources {
	...
	private final Map<String, Object> resources = new STreeMap<String, Object>(
	...
	public static void shareWith(char code) {
		if (Thread.currentThread().getThreadGroup().getName().charAt(0) == code)
			...
			initialize();
			rscs[code] = rscs[Thread.currentThread().getThreadGroup().getName()
					.charAt(0)];
	...
	public static final StringBuffer getFileResource(final String filename,
			final boolean reportErrors) {
		return r()._getFileResource(filename, reportErrors);
	}

	public static final boolean saveFileResource(final String filename,
			final MOB whom, final StringBuffer myRsc) {
		return r()._saveFileResource(filename, whom, myRsc);
	}

	public static final boolean updateFileResource(final String filename,
			final Object obj) {
		return r()._updateFileResource(filename, obj);
	}

	public static final boolean findRemoveProperty(final CMFile F,
			final String match) {
	...
	public static final List<String> getFileLineVector(final StringBuffer buf) {
		...
		for (int i = 0; i < buf.length(); i++) {
			if (((buf.charAt(i) == '\n') && (i < buf.length() - 1) && (buf
					.charAt(i + 1) == '\r'))
					|| ((buf.charAt(i) == '\r') && (i < buf.length() - 1) && (buf
							.charAt(i + 1) == '\n'))) {
				i++;
	...
	public static final void updateMultiList(String filename,
	...
	public static final Map<String, List<String>> getCachedMultiLists(
			final String filename, boolean createIfNot) {
		final String key = "PARSED_MULTI: " + filename.toUpperCase();
	...
	public static final Map<String, List<String>> getMultiLists(String filename) {
		...
		try {
			...
			V = getFileLineVector(new CMFile(prefix + "resources/" + filename,
					null).text());
		} catch (Exception e) {
		}
		if ((V != null) && (V.size() > 0)) {
	...
	public final Iterator<String> _findResourceKeys(final String srch) {
		...
		return new FilteredIterator<String>(resources.keySet().iterator(),
				new Filterer<String>() {
					public boolean passesFilter(String obj) {
						return (allOfThem)
								|| (obj.toLowerCase().indexOf(lowerSrch) >= 0);
					}
				});
	}
	...
	public final Object _getResource(final String ID) {
		...
		if (O != null) {
			if (compress && (O instanceof CompressedResource))
				return new StringBuffer(CMLib.encoder().decompressString(
						((CompressedResource) O).data));
			return O;
		}
		return null;
	}
	...
	public static final Object prepareObject(final Object obj) {
		...
		if (obj instanceof StringBuffer)
			return CMLib.encoder().compressString(
	...
	public final StringBuffer _getFileResource(final String filename,
			final boolean reportErrors) {
		Object rsc = _getResource(filename);
		if (rsc != null)
			return _toStringBuffer(rsc);
		StringBuffer buf = new CMFile(makeFileResourceName(filename), null,
				reportErrors ? CMFile.FLAG_LOGERRORS : 0).text();
		if (!CMProps.getBoolVar(CMProps.Bool.FILERESOURCENOCACHE))
			_submitResource(filename, buf);
		return buf;
	}

	public final boolean _updateFileResource(final String filename,
			final Object obj) {
		if (!CMProps.getBoolVar(CMProps.Bool.FILERESOURCENOCACHE))
	...
	public final boolean _saveFileResource(String filename, final MOB whoM,
			final StringBuffer myRsc) {
	...
	public final boolean _findRemoveProperty(final CMFile F, final String match) {
		...
		while (x >= 0) {
			if (((x == 0) || (!Character.isLetterOrDigit(text.charAt(x - 1))))
					&& (text.substring(x + match.length()).trim()
							.startsWith("="))) {
			...
			x = text.toString().toUpperCase()
					.indexOf(match.toUpperCase(), x + 1);
		}
		if (removed)
			F.saveRaw(text);
		return removed;
	...
	public static final Map<String, String> getAllPropResources(String section) {
		if (propResources == null) {
			synchronized (propResourceSync) {
				if (propResources == null) {
					CMFile file = new CMFile("::/coffeemud_properties.ini",
							...
							if (line.startsWith("[") && (line.endsWith("]"))) {
								...
								try {
									...
									final String value = URLDecoder.decode(
											line.substring(eqSepIndex + 1),
											"UTF-8");
									currSecMap.put(key.toUpperCase().trim(),
											value);
		...
		if (section.length() > 0)
			section = Thread.currentThread().getThreadGroup().getName()
					.charAt(0)
	...
	public static final void setPropResource(String section, String key,
			String value) {
		final Map<String, String> secMap = getAllPropResources(section);
		key = key.toUpperCase().trim();
		synchronized (secMap) {
