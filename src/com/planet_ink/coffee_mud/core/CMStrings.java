...
public class CMStrings {
	...
	public final static String endWithAPeriod(final String str) {
		if ((str == null) || (str.length() == 0))
			return str;
		int x = str.length() - 1;
		while ((x >= 0) && ((Character.isWhitespace(str.charAt(x))) // possible
																	// #~ color
																	// concerns,
																	// but
																	// normally
	...
	public final static Map.Entry<Character, String>[] splitMulti(
			final String str, char[] splitters) {
	...
	public final static boolean containsWordIgnoreCase(final String thisStr,
			final String word) {
		if ((thisStr == null) || (word == null) || (thisStr.length() == 0)
				|| (word.length() == 0))
			return false;
		return containsWord(thisStr.toLowerCase(), word.toLowerCase());
	}

	public final static boolean containsWord(final String thisStr,
			final String word) {
		if ((thisStr == null) || (word == null) || (thisStr.length() == 0)
				|| (word.length() == 0))
			return false;
		for (int i = thisStr.length() - 1; i >= 0; i--) {
			if ((thisStr.charAt(i) == word.charAt(0))
				...
				if ((thisStr.substring(i).startsWith(word.toLowerCase()))
						&& ((thisStr.length() == i + word.length()) || (!Character
								.isLetter(thisStr.charAt(i + word.length()))))) {
					return true;
				}
		}
		return false;
	...
	public final static String replaceAllofAny(final String str,
			final char[] theseChars, final char with) {
		if ((str == null) || (theseChars == null) || (str.length() == 0)
	...
	public final static String deleteAllofAny(final String str,
			final char[] theseChars) {
		if ((str == null) || (theseChars == null) || (str.length() == 0)
				|| (!containsAny(str, theseChars)))
			return str;
	...
	public final static String replaceAll(String str, final String thisStr,
			final String withThisStr) {
		if ((str == null) || (thisStr == null) || (withThisStr == null)
				|| (str.length() == 0) || (thisStr.length() == 0))
			return str;
		for (int i = str.length() - 1; i >= 0; i--)
			if (str.charAt(i) == thisStr.charAt(0))
				if (str.substring(i).startsWith(thisStr))
					str = str.substring(0, i) + withThisStr
							+ str.substring(i + thisStr.length());
		return str;
	}

	public final static String replaceAlls(String str, final String pairs[][]) {
		if ((str == null) || (pairs == null) || (str.length() == 0)
				|| (pairs.length == 0))
			return str;
	...
	public final static String replaceWord(String str, final String thisStr,
			final String withThisStr) {
		if ((str == null) || (thisStr == null) || (withThisStr == null)
				|| (str.length() == 0) || (thisStr.length() == 0))
			return str;
		...
		for (int i = str.length() - 1; i >= 0; i--) {
			if ((str.charAt(i) == thisStr.charAt(0))
				...
				if ((str.substring(i).toLowerCase().startsWith(thisStr
						.toLowerCase()))
						&& ((str.length() == i + thisStr.length()) || (!Character
								.isLetter(str.charAt(i + thisStr.length()))))) {
					String oldWord = str.substring(i, i + thisStr.length());
					if (oldWord.toUpperCase().equals(oldWord))
						str = str.substring(0, i) + uppercaseWithThisStr
								+ str.substring(i + thisStr.length());
					else if (oldWord.toLowerCase().equals(oldWord))
						str = str.substring(0, i)
								+ uppercaseWithThisStr.toLowerCase()
								+ str.substring(i + thisStr.length());
					else if ((oldWord.length() > 0)
							&& (Character.isUpperCase(oldWord.charAt(0))))
						str = str.substring(0, i)
								+ uppercaseWithThisStr.charAt(0)
								+ uppercaseWithThisStr.substring(1)
										.toLowerCase()
								+ str.substring(i + thisStr.length());
					else
						str = str.substring(0, i)
	...
	public final static String replaceFirstWord(String str,
			final String thisStr, final String withThisStr) {
		if ((str == null) || (thisStr == null) || (withThisStr == null)
				|| (str.length() == 0) || (thisStr.length() == 0))
			return str;
		...
		for (int i = str.length() - 1; i >= 0; i--) {
			if ((str.charAt(i) == thisStr.charAt(0))
				...
				if ((str.substring(i).toLowerCase().startsWith(thisStr
					...
					String oldWord = str.substring(i, i + thisStr.length());
					if (oldWord.toUpperCase().equals(oldWord))
						return str.substring(0, i) + uppercaseWithThisStr
								+ str.substring(i + thisStr.length());
					else if (oldWord.toLowerCase().equals(oldWord))
	...
	public final static String replaceFirst(String str, final String thisStr,
			final String withThisStr) {
		if ((str == null) || (thisStr == null) || (withThisStr == null)
				|| (str.length() == 0) || (thisStr.length() == 0))
			return str;
	...
	public final static String substituteSayInMessage(final String affmsg,
			final String msg) {
		if (affmsg == null)
			return null;
	...
	public final static boolean containsIgnoreCase(final String[] strs,
			final String str) {
	...
	public final static boolean compareStringArrays(final String[] A1,
			final String[] A2) {
	...
	/**
	 * Returns the length of the string as if it has neither ansi nor cm color
	 * codes.
	 * 
	 * @param thisStr
			...
			if (incomment) {
				...
				if ((c == '\n') || (c == '\r')) {
					if ((i > 0) && (i < finalData.length() - 2)) {
						...
						if ((Character.isLetterOrDigit(n) || ((".?,;\"'!@#$%^*()_-+={}[]:")
								.indexOf(n) >= 0))
								&& (Character.isLetterOrDigit(p) || ((".?,;\"'!@#$%^*()_-+={}[]:"
										.indexOf(p) >= 0))))
				...
				case 1: // eval start tag
					if (c == '>') {
						...
						if (start == i - 1)
							...
							tag = finalData.substring(start + 1, i)
									.toUpperCase();
							state = 3;
						}
					} else if ((i - start) > 20)
				...
				case 2: // eval end tag
					if (c == '>') {
						...
						if (start == i - 1)
							...
							state = 4;
							tag = finalData.substring(start + 2, i)
									.toUpperCase();
						}
					} else if ((i - start) > 20)
						start = -1;
				...
				case 5: // during & thing
					if (c == ';') // the end
					{
						final String code = finalData.substring(start + 1, i)
								.toLowerCase();
						finalData.delete(start, i + 1);
						if (code.equals("nbsp"))
	...
	public final static Hashtable<Object, Integer> makeNumericHash(
			final Object[] obj) {
	...
	public final static String padCenter(final String thisStr,
			final int thisMuch) {
	...
	public final static String padLeftWith(final String thisStr, final char c,
			final int thisMuch) {
		final int lenMinusColors = lengthMinusColors(thisStr);
		if (lenMinusColors > thisMuch)
			return removeColors(thisStr).substring(0, thisMuch);
	...
	public final static String padLeft(final String thisStr,
			final String colorPrefix, final int thisMuch) {
		final int lenMinusColors = lengthMinusColors(thisStr);
		if (lenMinusColors > thisMuch)
			return colorPrefix + removeColors(thisStr).substring(0, thisMuch);
		return SPACES.substring(0, thisMuch - lenMinusColors) + colorPrefix
				+ thisStr;
	}
	...
	public final static String padRightWith(final String thisStr, final char c,
			final int thisMuch) {
		final int lenMinusColors = lengthMinusColors(thisStr);
		if (lenMinusColors == thisMuch)
			return thisStr;
	...
	public final static String padRight(final String thisStr,
			final String colorSuffix, final int thisMuch) {
		final int lenMinusColors = lengthMinusColors(thisStr);
		if (lenMinusColors > thisMuch)
			return removeColors(thisStr).substring(0, thisMuch) + colorSuffix;
		if (thisMuch - lenMinusColors >= SPACES.length())
			return thisStr + colorSuffix + SPACES;
		return thisStr + colorSuffix
				+ SPACES.substring(0, thisMuch - lenMinusColors);
	}

	public final static String padRightPreserve(final String thisStr,
			final int thisMuch) {
		final int lenMinusColors = lengthMinusColors(thisStr);
		if (lenMinusColors > thisMuch)
	...
	public final static String centerPreserve(final String thisStr,
			final int thisMuch) {
		final int lenMinusColors = lengthMinusColors(thisStr);
		if (lenMinusColors > thisMuch)
			return removeColors(thisStr);
		final int left = (thisMuch - lenMinusColors) / 2;
		final int right = ((left + left + lenMinusColors) < thisMuch) ? left + 1
				: left;
		if (thisMuch - lenMinusColors >= SPACES.length())
			return thisStr + SPACES;
	...
	public final static String padLeftPreserve(final String thisStr,
			final int thisMuch) {
		final int lenMinusColors = lengthMinusColors(thisStr);
		if (lenMinusColors > thisMuch)
			return removeColors(thisStr);
	...
	// states: 0 = done after this one,-1 = done a char ago,-2 = eat & same
	// state,-99 = error,
	...
	private static class StringExpToken {
		...
		public final static StringExpToken token(final int type,
	...
	private static StringExpToken nextToken(final List<StringExpToken> tokens,
			final int[] index) {
	...
	private static StringExpToken makeTokenType(String token,
			final Map<String, Object> variables, final boolean emptyVars)
			throws Exception {
		if ((token == null) || (token.length() == 0))
			return null;
		if (token.startsWith("\""))
			return StringExpToken.token(STRING_EXP_TOKEN_STRCONST,
		...
		if (token.startsWith("$")) {
			...
			if ((value.toString().length() > 0)
					&& (!CMath.isNumber(value.toString())))
				return StringExpToken.token(STRING_EXP_TOKEN_STRCONST,
						value.toString());
			return StringExpToken.token(STRING_EXP_TOKEN_UKNCONST,
					value.toString());
		}
		if ((token.charAt(0) == '_')
	...
	private static StringExpToken nextStringToken(final String expression,
			final int[] index, final Map<String, Object> variables,
			final boolean emptyVars) throws Exception {
	...
	private final static StringExpToken matchSimpleValue(
			final List<StringExpToken> tokens, final int[] index,
	...
	private static StringExpToken matchCombinedValue(
			final List<StringExpToken> tokens, final int[] index,
			final Map<String, Object> variables) throws Exception {
		int[] i = index.clone();
		...
		if (token.type == STRING_EXP_TOKEN_OPENPAREN) {
			final StringExpToken testInside = matchCombinedValue(tokens, i,
					variables);
			if (testInside != null) {
		...
		i[0] = i2[0];
		final StringExpToken rightValue = matchCombinedValue(tokens, i,
				variables);
		if (rightValue == null)
			return null;
		index[0] = i[0];
		if ((leftValue.type == STRING_EXP_TOKEN_STRCONST)
				|| (rightValue.type == STRING_EXP_TOKEN_STRCONST)) {
			if (!token.value.equals("+"))
				throw new Exception("Can't combine a string using '"
						+ token.value + "'");
			index[0] = i[0];
		...
		if (token.value.equals("+")) {
			...
			index[0] = i[0];
			result.numValue = Math
					.round((Math.random() * (rightValue.numValue - leftValue.numValue))
							+ leftValue.numValue);
			return result;
		} else
	...
	private static Boolean matchValueEvaluation(
			final List<StringExpToken> tokens, final int[] index,
			final Map<String, Object> variables) throws Exception {
		int[] i = index.clone();
		StringExpToken token = nextToken(tokens, i);
		...
		} else if (token.type == STRING_EXP_TOKEN_OPENPAREN) {
			final Boolean testInside = matchValueEvaluation(tokens, i,
					variables);
			if (testInside != null) {
				token = nextToken(tokens, i);
				if (token.type != STRING_EXP_TOKEN_CLOSEPAREN)
		...
		i = index.clone();
		final StringExpToken leftValue = matchCombinedValue(tokens, i,
				variables);
		if (leftValue == null)
			return null;
		token = nextToken(tokens, i);
		...
		} else if ((leftValue.type == STRING_EXP_TOKEN_NUMCONST)
				|| (rightValue.type == STRING_EXP_TOKEN_NUMCONST)) {
			compare = Double.valueOf(leftValue.numValue).compareTo(
					Double.valueOf(rightValue.numValue));
		} else if (token.value.equalsIgnoreCase("IN")) {
			compare = leftValue.value.compareToIgnoreCase(rightValue.value);
		} else {
			// the great assumption -- when both are unknown, assume they are
			// numbers!
			compare = Double.valueOf(leftValue.value).compareTo(
					Double.valueOf(rightValue.value));
		}
		...
		if (token.value.equals(">"))
			...
			result = new Boolean(rightValue.value.toUpperCase().indexOf(
					leftValue.value.toUpperCase()) >= 0);
		else
			return null;
		index[0] = i[0];
		return result;
	...
	private static Boolean matchExpression(final List<StringExpToken> tokens,
			final int[] index, final Map<String, Object> variables)
			throws Exception {
		int[] i = index.clone();
		StringExpToken token = nextToken(tokens, i);
		...
		if (token.value.equalsIgnoreCase("AND"))
			result = new Boolean(leftExpression.booleanValue()
					&& rightExpression.booleanValue());
			...
			result = new Boolean(
					leftExpression.booleanValue() != rightExpression
							.booleanValue());
		else
			throw new Exception(
					"Parse Exception: Illegal expression evaluation combiner: "
							+ token.value);
		index[0] = i[0];
		return result;
	}

	public final static boolean parseStringExpression(final String expression,
			final Map<String, Object> variables, final boolean emptyVarsOK)
			throws Exception {
		...
		if (value == null)
			throw new Exception("Parse error on following statement: "
					+ expression);
		return value.booleanValue();
	}

	public final static int countSubstrings(final String[] set,
			final String[] things) {
		if (set == null)
			return 0;
		if (things == null)
	 ...
	 * texts to create a patch. Applies the patch onto another text, allowing
	 * for errors.
	 * 
	 * @author fraser@google.com (Neil Fraser)
	 */
	 ...
	 * @return Linked List of Diff objects.
	 */
	private static LinkedList<Diff> diff_main(String text1, String text2,
	 ...
	 * @return LinkedList of Diff objects.
	 */
	private static LinkedList<Diff> diff_bisectSplit(String text1,
	 ...
	 */
	private static LinesToCharsResult diff_linesToChars(String text1,
			String text2) {
	...
	private static String diff_linesToCharsMunge(String text,
		...
		while (lineEnd < text.length() - 1) {
			...
			if (lineHash.containsKey(line)) {
				chars.append(String.valueOf((char) lineHash.get(line)
						.intValue()));
			} else {
				lineArray.add(line);
	 ...
	 *            List of unique strings.
	 */
	private static void diff_charsToLines(LinkedList<Diff> diffs,
			List<String> lineArray) {
		StringBuilder text;
		for (Diff diff : diffs) {
	...
	/**
	 * f Does a substring of shorttext exist within longtext such that the
	...
	private static void diff_cleanupSemantic(LinkedList<Diff> diffs) {
		...
		while (thisDiff != null) {
			if ((prevDiff != null)
					&& (prevDiff.operation == DiffOperation.DELETE && thisDiff.operation == DiffOperation.INSERT)) {
				...
				if (overlap_length1 >= overlap_length2) {
					if (overlap_length1 >= deletion.length() / 2.0
						...
						// surrounding edits.
						pointer.previous();
						pointer.add(new Diff(DiffOperation.EQUAL, insertion
					...
					if (overlap_length2 >= deletion.length() / 2.0
						...
						// edits.
						pointer.previous();
						pointer.add(new Diff(DiffOperation.EQUAL, deletion
	...
	private static void diff_cleanupSemanticLossless(LinkedList<Diff> diffs) {
		...
		while (nextDiff != null) {
			if ((prevDiff != null)
					&& (thisDiff != null)
					&& (prevDiff.operation == DiffOperation.EQUAL && nextDiff.operation == DiffOperation.EQUAL)) {
	...
	private static Pattern BLANKLINEEND = Pattern.compile("\\n\\r?\\n\\Z",
			Pattern.DOTALL);
	private static Pattern BLANKLINESTART = Pattern.compile(
			"\\A\\r?\\n\\r?\\n", Pattern.DOTALL);
