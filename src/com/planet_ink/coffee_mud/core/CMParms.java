...
public class CMParms {
	...
	public final static String combine(final List<?> commands,
			final int startAt, final int endAt) {
		final StringBuilder combined = new StringBuilder("");
		if (commands != null)
	...
	public final static String combineWith(final List<?> commands,
			final char withChar, final int startAt, final int endAt) {
		final StringBuilder combined = new StringBuilder("");
		if (commands != null)
			for (int commandIndex = startAt; commandIndex < endAt; commandIndex++)
				combined.append(commands.get(commandIndex).toString()
	...
	public final static String combineWithQuotes(final List<?> commands,
			final int startAt, final int endAt) {
	...
	public final static String[] combine(final String[] strs1,
			final String[] strs2) {
	...
	public final static String combineAfterIndexWithQuotes(
			final Vector<?> commands, final String match) {
		final StringBuffer Combined = new StringBuffer("");
		if (commands != null) {
			String s;
	...
	public final static String combineWithTabs(final List<?> commands,
			final int startAt) {
		return combineWithX(commands, "\t", startAt);
	}

	public final static String combineWithX(final List<?> commands,
			final String X, final int startAt) {
		final StringBuffer Combined = new StringBuffer("");
		if (commands != null) {
			String s;
			for (int commandIndex = startAt; commandIndex < commands.size(); commandIndex++) {
	...
	public final static List<String> parseCommas(final String s,
			final boolean ignoreNulls) {
		return parseAny(s, ',', ignoreNulls);
	}

	public final static List<String> parseCommandFlags(final String s,
			final String[] flags) {
	...
	public final static List<String> parseTabs(final String s,
			final boolean ignoreNulls) {
		return parseAny(s, '\t', ignoreNulls);
	}

	public final static List<String> parseAny(final String s,
			final String delimeter, final boolean ignoreNulls) {
		...
		for (int i = 0; i < s.length() - delimeter.length() + 1; i++)
			if ((s.charAt(i) == firstChar)
					&& (s.substring(i, i + delimeter.length())
							.equals(delimeter))) {
		...
		final String sub = (last >= s.length()) ? "" : s.substring(last,
				s.length()).trim();
		if (!ignoreNulls || (sub.length() > 0))
			V.add(sub);
		return V;
	...
	public final static List<String> parseAnyWords(final String s,
			final String delimeter, final boolean ignoreNulls) {
		final Vector<String> V = new Vector<String>(1);
		if ((s == null) || (s.length() == 0))
			return V;
		...
		for (int i = 0; i < tests.length() - upDelimeter.length() + 1; i++)
			if ((tests.charAt(i) == firstChar)
					&& (tests.substring(i, i + upDelimeter.length())
							.equals(upDelimeter))) {
				String sub = s.substring(last, i).trim();
	...
	public final static List<String> parseAny(final String s,
			final char delimiter, final boolean ignoreNulls) {
		final Vector<String> V = new Vector<String>(1);
		if ((s == null) || (s.length() == 0))
			return V;
		int last = 0;
	...
	public final static List<String> parseSquiggleDelimited(final String s,
			final boolean ignoreNulls) {
		return parseAny(s, '~', ignoreNulls);
	}

	public final static List<String> parseSemicolons(final String s,
			final boolean ignoreNulls) {
		return parseAny(s, ';', ignoreNulls);
	}

	public final static List<String> parseSpaces(final String s,
			final boolean ignoreNulls) {
		return parseAny(s, ' ', ignoreNulls);
	}
	...
	public final static Object getBitWork(final String s, final int which,
			final int op) {
		int currOne = 0;
		int start = -1;
		char q = ' ';
			...
			default:
				if ((cs[c] == q) || (q == ' ' && cs[c] == '\t')) {
					...
					if (which == currOne) {
						...
						case 1:
							return new String(cs, c + 1, cs.length - c - 1)
									.trim();
						}
					}
					currOne++;
	...
	public final static String getParmStr(String text, final String key,
	...
	public final static int[] getParmCompare(String text, final String key,
			final int value) {
		int x = text.toUpperCase().indexOf(key.toUpperCase());
		while (x >= 0) {
			if ((x == 0) || (!Character.isLetter(text.charAt(x - 1)))) {
				while ((x < text.length()) && (text.charAt(x) != '>')
						&& (text.charAt(x) != '<') && (text.charAt(x) != '!')
						&& (text.charAt(x) != '='))
					x++;
				if (x < text.length() - 1) {
					...
					if (x < text.length() - 1) {
						...
						if (x < text.length()) {
							...
							if (andEqual && (found == value))
								return makeIntArray(comp, (comp == '!') ? -1
										: 1);
							switch (comp) {
							case '>':
								return makeIntArray(comp, (value > found) ? 1
										: -1);
							case '<':
								return makeIntArray(comp, (value < found) ? 1
										: -1);
	...
	private static int strIndex(final Vector<String> V, final String str,
			final int start) {
		if (str.indexOf(' ') < 0)
	...
	private static int stringContains(final Vector<String> V,
			final char combiner, final StringBuffer buf, int lastIndex) {
	...
	private static int stringContains(final Vector<String> V, final char[] str,
	...
	public final static Map<String, String> parseEQParms(final String str,
			final String[] parmList) {
			...
			if (Character.isLetter(c)) {
				for (int p = 0; p < parmList.length; p++) {
					if ((Character.toUpperCase(parmList[p].charAt(0)) == c)
							...
							&& (str.substring(x, x + parmList[p].length())
									.equalsIgnoreCase(parmList[p]))) {
						int chkX = x + parmList[p].length();
						while ((chkX < str.length())
								&& (Character.isWhitespace(str.charAt(chkX))))
							chkX++;
			...
			case 4:
				if (c == '\\')
					...
					if (value.length() == 0)
						...
						h.put(parmName, str.substring(start, lastPossibleStart)
								.trim());
						parmName = str.substring(lastPossibleStart, x)
								.toUpperCase().trim();
						state = 2;
					}
				} else if (c == '\n') {
	...
	public final static Map<String, String> parseEQParms(
			final List<String> parms, final int start, final int end) {
		final Map<String, String> h = new Hashtable<String, String>();
		for (Object O : parms)
	...
	public final static List<List<String>> parseDoubleDelimited(String text,
			final char delim1, final char delim2) {
		...
		for (String s : preparseV) {
			...
			while (s.length() > 0) {
				...
				if ((cmd.length() > 0) && (!cmd.startsWith("#"))) {
					...
					groupV.add(CMStrings.replaceAll(cmd, "\\" + delim2, ""
							+ delim2));
				}
			}
			if (groupV.size() > 0)
	...
	public final static int stringContains(final String str1, final String str2) {
		...
		for (int i = buf1.length() - 1; i >= 0; i--) {
			if ((buf1.charAt(i) != ' ') && (buf1.charAt(i) != '\'')
					&& (buf1.charAt(i) != '\"') && (buf1.charAt(i) != '`')
			...
			if ((x == 0) || (!Character.isLetter(text.charAt(x - 1)))) {
				...
				if (x < text.length()) {
					...
					if (x < text.length()) {
						...
						x = 0;
						while ((x < text.length())
								&& ((Character.isDigit(text.charAt(x))) || (text
										.charAt(x) == '.')))
							x++;
	...
	public final static double getParmDouble(String text, final String key,
	...
	public final static int getParmInt(String text, final String key,
	...
	public final static boolean getParmBool(String text, final String key,
	...
	public final static Set<String> toStringSet(final List<?> V,
			final Set<String> S) {
		if ((V == null) || (V.size() == 0)) {
			return S;
		}
	...
	public final static List<String> parseSafeSemicolonList(final String list,
			final boolean ignoreNulls) {
		if (list == null)
	...
	public final static List<String> toNameVector(
			final Set<? extends Environmental> V) {
	...
	public final static List<String> toNameVector(
			final Enumeration<? extends Environmental> V) {
		final Vector<String> s = new Vector<String>();
		if (V == null)
			return s;
	...
	public final static String toEnvironmentalStringList(
			final Enumeration<? extends Environmental> e) {
		if (!e.hasMoreElements())
			return "";
	...
	public final static String toCMObjectStringList(
			final Enumeration<? extends CMObject> e) {
		if (!e.hasMoreElements())
			return "";
	...
	public final static String toCMObjectStringList(
			final Iterator<? extends CMObject> e) {
		if (!e.hasNext())
			return "";
		CMObject o = e.next();
		final StringBuilder s = new StringBuilder(o.ID());
	...
	public final static String[] appendToArray(final String[] front,
			final String[] back) {
		if (back == null)
			return front;
		if (front == null)
	...
	public final static int indexOf(final String[] supported,
	...
	public final static int indexOfIgnoreCase(final Enumeration<?> supported,
			final String key) {
		if (supported == null)
			return -1;
		int index = -1;
	...
	public final static int indexOf(final Enumeration<?> supported,
			final Object key) {
		if (supported == null)
			return -1;
		int index = -1;
	...
	public final static int indexOfIgnoreCase(final Iterator<?> supported,
			final String key) {
		if (supported == null)
			return -1;
		int index = -1;
	...
	public final static int indexOf(final Iterator<?> supported,
			final Object key) {
		if (supported == null)
			return -1;
		int index = -1;
	...
	public final static int indexOfIgnoreCase(final String[] supported,
			final String expertise) {
		if (supported == null)
			return -1;
		if (expertise == null)
	...
	public final static int indexOfIgnoreCase(final List<?> supported,
			final String str) {
		if (supported == null)
			return -1;
		if (str == null)
	...
	public final static boolean contains(final String[] supported,
			final String expertise) {
		return indexOf(supported, expertise) >= 0;
	}

	public final static boolean contains(final Enumeration<String> supported,
			final String expertise) {
		for (; supported.hasMoreElements();)
	...
	public final static boolean containsIgnoreCase(final String[] supported,
			final String expertise) {
		return indexOfIgnoreCase(supported, expertise) >= 0;
	}

	public final static boolean containsIgnoreCase(final List<?> supported,
			final String str) {
		return indexOfIgnoreCase(supported, str) >= 0;
	}

	public final static int indexOf(final Object[] supported,
	...
	public final static boolean contains(final Object[] supported,
			final Object expertise) {
		return indexOf(supported, expertise) >= 0;
	}
	...
	public final static boolean contains(final ByteBuffer buf,
			final byte[] bytes, final int pos) {
	...
	public final static int containIndex(final ByteBuffer buf,
			final byte[][] bytes, final int pos) {
	...
	public final static int startsWith(final String[] supported,
			final String expertise) {
		if (supported == null)
			return 0;
		if (expertise == null)
	...
	public final static int startsWithIgnoreCase(final String[] supported,
			final String expertise) {
		if (supported == null)
			return 0;
		if (expertise == null)
	...
	public final static boolean startsAnyWith(final String[] supported,
			final String expertise) {
		return startsWith(supported, expertise) >= 0;
	}

	public final static boolean startsAnyWithIgnoreCase(
			final String[] supported, final String expertise) {
		return startsWithIgnoreCase(supported, expertise) >= 0;
	}

	public final static int endsWith(final String[] supported,
			final String expertise) {
		if (supported == null)
			return 0;
		if (expertise == null)
	...
	public final static int endsWithIgnoreCase(final String[] supported,
			final String expertise) {
		if (supported == null)
			return 0;
		if (expertise == null)
	...
	public final static boolean endsAnyWith(final String[] supported,
			final String expertise) {
		return endsWith(supported, expertise) >= 0;
	}

	public final static boolean endsAnyWithIgnoreCase(final String[] supported,
			final String expertise) {
		return endsWithIgnoreCase(supported, expertise) >= 0;
	}

	/**
	 * constant value representing an undefined/unimplemented miscText/parms
	 * format.
	 */
