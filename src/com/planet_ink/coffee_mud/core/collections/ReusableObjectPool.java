...
public class ReusableObjectPool<T extends CMObject> {
	...
	private class PoolFixer implements Runnable {
		...
		public void startFixing() {
			...
			synchronized (this) {
				if (!isRunning) {
					isRunning = true;
					new Thread(Thread.currentThread().getThreadGroup(), this,
							"PoolFixer"
									+ Thread.currentThread().getThreadGroup()
											.getName().charAt(0)).start();
					return;
				}
			}
			fix();
	...
	private class NotifyingCMObjectVector<K extends CMObject> extends Vector<T> {
		...
		protected void finalize() throws Throwable {
			final NotifyingCMObjectVector<T> V = new NotifyingCMObjectVector<T>(
					this);
			synchronized (sync) {
				returned++;
				masterPool.push(V);
	...
	public ReusableObjectPool(final List<T> initialEntry, final int minEntries) {
		if (initialEntry.size() == 0)
			this.masterList = new NotifyingCMObjectVector<T>(
					new ReadOnlyList<T>(initialEntry));
		else
	...
	private final NotifyingCMObjectVector<T> makeNewEntry() {
		final NotifyingCMObjectVector<T> myList = new NotifyingCMObjectVector<T>(
				masterList.size());
		for (final T o : masterList)
			myList.add((T) o.copyOf());
		created++;
		if (created == minEntries * 1000)
			Log.errOut("ReuseOP", "Reusable Object Pool pass all reason: "
					+ CMParms.toStringList(masterList));
		return myList;
	}
