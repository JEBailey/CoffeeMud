	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (invoker.location() != mob.location())
			...
			if (msg.amISource(invoker)
				...
				invoker.location().show((MOB) affected, msg.target(),
						CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> defend(s) <S-HIM-HERSELF> against <T-NAME>.");
				return false;
			}
		}
		...
		if (tickID == Tickable.TICKID_MOB) {
			if (fullRound) {
				...
				if (mob.location() != null) {
					if (mob.location()
							.show(mob, null, this, CMMsg.MSG_OK_VISUAL,
									"<S-YOUPOSS> successful defence <S-HAS-HAVE> allowed <S-HIM-HER> to disengage.")) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, null, this,
					CMMsg.MSG_CAST_SOMANTIC_SPELL,
					"^S<S-NAME> assume(s) an all-out defensive posture.^?");
			...
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to assume an all-out defensive posture, but fail(s).");
