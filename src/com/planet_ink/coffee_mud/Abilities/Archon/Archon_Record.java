...
public class Archon_Record extends ArchonSkill {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (sess == null)
			return false;
		if ((affected instanceof MOB) && (((MOB) affected).session() != null)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		MOB target = CMLib.players()
				.getLoadPlayer(CMParms.combine(commands, 0));
		if (target == null)
			target = getTargetAnywhere(mob, commands, givenTarget, false, true,
					false);
		if (target == null)
			return false;
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MASK_MOVE
					...
					"^F<S-NAME> begin(s) recording <T-NAMESELF>.^?");
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (!file.canWrite()) {
					if (!CMSecurity.isASysOp(mob)
							|| (CMSecurity.isASysOp(target)))
						Log.sysOut("Record", mob.Name()
								+ " failed to start recording " + target.name()
								+ ".");
				} else {
					if (!CMSecurity.isASysOp(mob)
							|| (CMSecurity.isASysOp(target)))
						Log.sysOut("Record", mob.Name() + " started recording "
								+ target.name() + " to /" + filename + ".");
					Archon_Record A2 = (Archon_Record) copyOf();
					Session F = (Session) CMClass.getCommon("FakeSession");
					F.initializeSession(null, Thread.currentThread()
							.getThreadGroup().getName(), filename);
					if (target.session() == null)
						target.setSession(F);
					A2.sess = F;
					target.addNonUninvokableEffect(A2);
					mob.tell("Enter RECORD " + target.Name()
