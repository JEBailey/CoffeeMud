...
public class Archon_Banish extends ArchonSkill {
	...
	protected String timeRemaining() {
		...
		return "for another "
				+ CMLib.english().returnTime(
						releaseTime - System.currentTimeMillis(), 0);
	}
	...
	public Room prison() {
		...
		if (myPrison != null) {
			...
			prisonRoom
					.setDescription("You are standing on an immense, grey stone floor that stretches as far as you can see in all directions.  Rough winds plunging from the dark, starless sky tear savagely at your fragile body.");
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (affected instanceof Room) {
			...
			if ((msg.tool() != null)
					...
					&& (msg.source() != null)
					&& (msg.source().location() != null)
					&& ((msg.source().location() == affected) || (msg.source()
							.location().getArea() == affected))
				...
				if ((msg.source() != null) && (msg.source().location() != null))
					msg.source()
							...
							.show(msg.source(), null, CMMsg.MSG_OK_ACTION,
									"<S-NAME> attempt(s) to recall, but the magic fizzles.");
				return false;
			}
		}
		return true;
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
			...
			if ((CMath.isInteger(num)) && (CMath.s_int(num) > 0)) {
				if ("DAYS".startsWith(last))
					time = System.currentTimeMillis()
							+ (TimeManager.MILI_DAY * CMath.s_int(num));
				else if ("MONTHS".startsWith(last))
					time = System.currentTimeMillis()
							+ (TimeManager.MILI_MONTH * CMath.s_int(num));
				else if ("HOURS".startsWith(last))
					time = System.currentTimeMillis()
							+ (TimeManager.MILI_HOUR * CMath.s_int(num));
				else if ("MINUTES".startsWith(last))
					time = System.currentTimeMillis()
							+ (TimeManager.MILI_MINUTE * CMath.s_int(num));
				else if ("SECONDS".startsWith(last))
					time = System.currentTimeMillis()
							+ (TimeManager.MILI_SECOND * CMath.s_int(num));
				else if ("TICKS".startsWith(last))
					time = System.currentTimeMillis()
							+ (CMProps.getTickMillis() * CMath.s_int(num));
		...
		if (myPrison != null
				&& CMLib.map().getExtendedRoomID(myPrison).length() > 0) {
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MASK_MOVE
					...
					auto ? "<T-NAME> is banished!"
							: "^F<S-NAME> banish(es) <T-NAMESELF>.^?");
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if ((A != null) && (A.prison() != null)
						&& (!A.prison().isInhabitant(target))) {
					...
					mob.location().show(
							...
							null,
							CMMsg.MSG_OK_VISUAL,
							"<S-NAME> <S-IS-ARE> banished to "
									+ A.prison().displayText() + "!");
					Log.sysOut("Banish",
							mob.Name() + " banished " + target.name() + " to "
									+ CMLib.map().getExtendedRoomID(A.prison())
									+ ".");
				}
			...
			return beneficialVisualFizzle(mob, target,
					"<S-NAME> attempt(s) to banish <T-NAMESELF>, but fail(s).");
		return success;
	}
}
