...
public class Archon_Multiwatch extends ArchonSkill {
	...
	public boolean nonIPnonMonsterWithMe(MOB me) {
		if ((me.location() != null) && (me.session() != null)) {
			...
			for (int i = 0; i < R.numInhabitants(); i++) {
				...
				if ((M.session() != null)
						&& (M.session().getAddress().equals(me.session()
								.getAddress())))
					return true;
			}
		}
					...
					if ((msg.othersMessage() != null)
						...
						if (msg.sourceMessage().indexOf("order(s)") > 0) {
							if ((msg.target() instanceof MOB)
									...
									&& (((MOB) msg.target()).session()
											.getAddress().equals(msg.source()
											.session().getAddress())))
								data[DATA_ORDER]++;
						} else {
							if (nonIPnonMonsterWithMe(msg.source()))
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((tickID == Tickable.TICKID_MOB) && (affected instanceof MOB)) {
			...
			if ((mob.session() != null)
					...
					if (V != null)
						for (int v = 0; v < V.size(); v++) {
							...
							if (!CMLib.flags().isInTheGame(M, true))
								continue;
							String hisLastCmd = CMParms.combine(mob.session()
									.getPreviousCMD(), 0);
							Archon_Multiwatch A = (Archon_Multiwatch) M
									.fetchEffect(ID());
							if (A != null) {
								if ((A.lastCommand != null)
										&& (!A.lastCommand.equals(hisLastCmd)))
				...
				lastCommand = CMParms
						.combine(mob.session().getPreviousCMD(), 0);
			}
		}
		return true;
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (CMParms.combine(commands, 0).equalsIgnoreCase("auto")) {
			...
			return true;
		} else if ((commands.size() == 0) && (DATA.size() > 0)
				&& (IPS.size() > 0)) {
			...
			for (Enumeration<String> e = IPS.keys(); e.hasMoreElements();) {
				...
				for (int v = 0; v < V.size(); v++) {
					...
					report.append(CMStrings.padRight(data[DATA_TYPEDCOMMAND]
							+ "", 10));
