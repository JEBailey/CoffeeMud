...
public class GenLanguage extends StdLanguage {
	...
	public String getStat(String code) {
		...
		case 3:
			if (num == 0) {
				...
				return CMParms.toStringList(((List<String[]>) V(ID, V_WSETS))
						.get(num - 1));
			else
				return "";
		case 4:
			return CMParms.toStringList((Map<String, String>) V(ID, V_HSETS));
		case 5:
	...
	public void setStat(String code, String val) {
		...
		case 3:
			if (num == 0) {
				...
				for (final String wordList : allSets)
					wordSets.add(CMParms.parseCommas(wordList, true).toArray(
							new String[0]));
				...
				((List<String[]>) V(ID, V_WSETS)).set(num - 1, CMParms
						.parseCommas(val, true).toArray(new String[0]));
			else if ((num == ((List<String[]>) V(ID, V_WSETS)).size() + 1)
					&& (val.length() > 0))
				((List<String[]>) V(ID, V_WSETS)).add(CMParms.parseCommas(val,
						true).toArray(new String[0]));
			break;
		case 4:
			SV(ID, V_HSETS, CMParms.parseEQStringList(val));
			break;
		case 5:
			SV(ID, V_HELP, val);
			break;
		default:
			if (code.equalsIgnoreCase("allxml")
					&& ID.equalsIgnoreCase("GenLanguage"))
				parseAllXML(val);
			break;
		}
	}
