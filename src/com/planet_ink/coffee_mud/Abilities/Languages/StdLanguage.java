...
public class StdLanguage extends StdAbility implements Language {
	...
	protected ExpertiseLibrary.SkillCostDefinition getRawTrainingCost() {
		return CMProps.getLangTrainCostFormula(ID());
	}
	...
	public String translate(String language, String word) {
		if (translationHash(language).containsKey(word.toUpperCase()))
			return fixCase(word,
					translationHash(language).get(word.toUpperCase()));
		...
		if (translationVector.size() > 0) {
			...
			return choices[CMath.abs(word.toLowerCase().hashCode())
					% choices.length];
		}
		return word;
	}
			...
			if (Character.isLetter(c)) {
				if (vowels.indexOf(c) >= 0)
					w.setCharAt(
							x,
							fixCase(c,
									vowels.charAt(CMLib.dice().roll(1,
											vowels.length(), -1))));
				else
					w.setCharAt(
							x,
							fixCase(c,
									consonants.charAt(CMLib.dice().roll(1,
											consonants.length(), -1))));
				numToMess--;
			}
		}
		...
		while (start <= str.length()) {
			...
			case 0:
				if (Character.isLetter(c)) {
					...
					newStr.append(translate(language, str.substring(start, end))
							+ c);
					end++;
					start = end;
					state = -1;
	...
	protected Language getMyTranslator(String id, Physical P, Language winner) {
		...
		for (final Enumeration<Ability> a = P.effects(); a.hasMoreElements();) {
			...
			if ((A instanceof Language)
					...
					&& ((winner == null) || ((Language) A).getProficiency(id) > winner
							.getProficiency(id))) {
				winner = (Language) A;
			}
		}
	...
	protected boolean processSourceMessage(CMMsg msg, String str, int numToMess) {
		...
		msg.modify(msg.source(), msg.target(), this, msg.sourceCode(),
				CMStrings.substituteSayInMessage(msg.sourceMessage(), smsg),
				msg.targetCode(), msg.targetMessage(), msg.othersCode(),
				msg.othersMessage());
		return true;
	}

	protected boolean processNonSourceMessages(CMMsg msg, String str,
			int numToMess) {
		...
		if (msg.target() instanceof Physical) {
			...
			for (final Enumeration<Ability> a = P.effects(); a
				...
				if ((A instanceof Language) && (!A.ID().equals(ID()))) {
					msg.source().tell(
							P.name(msg.source()) + " is already written in "
									+ A.name() + " and can not have "
									+ writtenName() + " writing added.");
					return false;
				}
			}
		}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((affected instanceof MOB) && (beingSpoken(ID()))) {
			if ((msg.source() == affected)
				...
				if (str != null) {
					int numToMess = (int) Math.round(CMath.mul(numChars(str),
							CMath.div(100 - getProficiency(ID()), 100)));
				...
				case CMMsg.TYP_DEPOSIT: {
					// yes, this means that a mob speaking Common to a marketing
					// player will get failed,
					// however, remember that the LISTer language doesn't
					// matter, only the responding (this) language.
					// also, think about muds where there is no Common (an
					// interesting mud!)
					if ((!CMSecurity.isAllowed(msg.source(), msg.source()
							.location(), CMSecurity.SecFlag.ORDER))
							&& (!CMSecurity.isAllowed(msg.source(), msg
									.source().location(),
									CMSecurity.SecFlag.CMDMOBS) || (!((MOB) msg
									.target()).isMonster()))
							&& (!CMSecurity.isAllowed(msg.source(), msg
									.source().location(),
									CMSecurity.SecFlag.CMDROOMS) || (!((MOB) msg
									.target()).isMonster()))) {
						Language L = getAnyTranslator(ID(), msg.source());
						if ((L == null)
								|| (!L.beingSpoken(ID()))
								|| ((CMLib.dice().rollPercentage() * 2) > (L
										.getProficiency(ID()) + getProficiency(ID())))) {
							...
							if ((L == null) || (!L.beingSpoken(ID())))
								reply = "<S-NAME> <S-IS-ARE> speaking "
										+ name()
										+ " and <T-NAME> would not understand <S-HIM-HER>.";
							else
								reply = "<T-NAME> <T-IS-ARE> having trouble understanding <S-YOUPOSS> pronunciation.";
							msg.addTrailerMsg(CMClass.getMsg(
									(MOB) msg.target(), msg.source(), null,
									CMMsg.MSG_OK_VISUAL, reply));
						}
					}
					break;
				}
	...
	private int numLanguagesKnown(MOB student) {
		...
		for (int a = 0; a < student.numAbilities(); a++) {
			...
			if (((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_LANGUAGE)
				...
				if ((CMLib.ableMapper().getQualifyingLevel(C.ID(), false,
						A.ID()) >= 0)
	...
	public boolean canBeLearnedBy(MOB teacher, MOB student) {
		...
		if ((C.maxLanguages() > 0) && (C.maxLanguages() <= numLanguages)) {
			...
			student.tell("You may only learn " + C.maxLanguages()
					+ " languages.");
			return false;
		}
		return true;
		...
		if ((student != null) && (student.fetchAbility(ID()) != null)) {
			...
			if (remaining <= 0)
				student.tell(student.name()
						+ " may not learn any more languages.");
			else if (remaining < Integer.MAX_VALUE / 2)
				student.tell(student.name() + " may learn " + remaining
						+ " more languages.");
		}
	}
	...
	protected boolean translateOthersMessage(CMMsg msg, String sourceWords) {
		if ((msg.othersMessage() != null)
			...
			if (msg.target() != null)
				otherMes = CMLib.coffeeFilter().fullOutFilter(null,
						(MOB) affected, msg.source(), msg.target(), msg.tool(),
						otherMes, false);
			msg.addTrailerMsg(CMClass.getMsg(msg.source(), affected, null,
					CMMsg.NO_EFFECT, null, msg.othersCode(),
					CMStrings.substituteSayInMessage(otherMes, sourceWords)
							+ " (translated from " + name() + ")",
		...
		if (msg.amITarget(affected) && (msg.targetMessage() != null)) {
			...
			if (msg.target() != null)
				otherMes = CMLib.coffeeFilter().fullOutFilter(null,
						...
						otherMes, false);
			msg.addTrailerMsg(CMClass.getMsg(msg.source(), affected, null,
					CMMsg.NO_EFFECT, null, msg.targetCode(),
					CMStrings.substituteSayInMessage(otherMes, sourceWords)
							+ " (translated from " + name() + ")",
					CMMsg.NO_EFFECT, null));
			return true;
	...
	protected boolean translateChannelMessage(CMMsg msg, String sourceWords) {
		if (CMath.bset(msg.sourceMajor(), CMMsg.MASK_CHANNEL)) {
			msg.addTrailerMsg(CMClass.getMsg(
					...
					CMMsg.NO_EFFECT,
					msg.othersCode(),
					CMStrings.substituteSayInMessage(msg.othersMessage(),
							sourceWords) + " (translated from " + name() + ")"));
			return true;
		}
		return false;
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected instanceof MOB)
		...
		} else if ((affected instanceof MOB) && (msg.source() == affected)
				&& (beingSpoken(ID())) && (msg.target() instanceof Item)
			...
			I.addNonUninvokableEffect((Ability) this.copyOf());
				...
				&& ((msg.targetMessage() == null) || (!msg.targetMessage()
						.equals("CANCEL")))
			...
			msg.modify(msg.source(), msg.target(), msg.tool(),
					...
					"CANCEL", msg.othersCode(), msg.othersMessage());
			...
			if (numToMess == 0)
				msg.source().tell(
						"There is nothing written on " + affected.name() + ".");
			else {
				if (L != null)
					numToMess = (int) Math.round(CMath.mul(numChars(str),
							CMath.div(100 - L.getProficiency(ID()), 100)));
				String original = messChars(ID(), str, numToMess);
				str = scrambleAll(ID(), str, numToMess);
				msg.source().tell("It says '" + str + "'");
				if ((L != null) && (!original.equals(str)))
					msg.source().tell(
							"It says '" + original + "' (translated from "
									+ L.writtenName() + ").");
			}
		}
	}
