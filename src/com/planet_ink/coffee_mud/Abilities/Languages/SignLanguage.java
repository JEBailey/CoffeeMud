...
public class SignLanguage extends StdLanguage {
	...
	protected boolean processSourceMessage(CMMsg msg, String str, int numToMess) {
		...
		String fullMsgStr = CMStrings.substituteSayInMessage(
				msg.sourceMessage(), wordsSaid);
		...
		startFullMsg = CMStrings.replaceFirstWord(startFullMsg, "say(s)",
				"sign(s)");
		startFullMsg = CMStrings.replaceFirstWord(startFullMsg, "ask(s)",
				"sign(s) askingly");
		startFullMsg = CMStrings.replaceFirstWord(startFullMsg, "exclaim(s)",
				"sign(s) excitedly");
		if (oldStartFullMsg.equals(startFullMsg)) {
			...
			if (x < 0)
				...
				x += 3;
			startFullMsg = startFullMsg.substring(0, x) + " in sign"
					+ startFullMsg.substring(x);
		}
		...
		msg.modify(
				...
				msg.target(),
				this,
				CMath.unsetb(msg.sourceCode(), CMMsg.MASK_SOUND
						| CMMsg.MASK_MOUTH)
						| CMMsg.MASK_MOVE,
	...
	protected boolean processNonSourceMessages(CMMsg msg, String str,
			int numToMess) {
		String fullOtherMsgStr = (msg.othersMessage() == null) ? msg
				.targetMessage() : msg.othersMessage();
		...
		case 8:
			...
			break;
		case 9:
			verb = "wave(s) <S-HIS-HER> fingers";
			break;
		case 10:
			verb = "wiggle(s) <S-HIS-HER> hands";
			break;
		case 11:
		case 12:
			verb = "wave(s) <S-HIS-HER> hands";
			break;
		case 13:
			verb = "wiggle(s) <S-HIS-HER> fingers";
			break;
		}
		String oldStartFullMsg = startFullMsg;
		startFullMsg = CMStrings
				.replaceFirstWord(startFullMsg, "tell(s)", verb);
		startFullMsg = CMStrings.replaceFirstWord(startFullMsg, "say(s)", verb);
		startFullMsg = CMStrings.replaceFirstWord(startFullMsg, "ask(s)", verb
				+ " askingly");
		startFullMsg = CMStrings.replaceFirstWord(startFullMsg, "exclaim(s)",
				verb + " excitedly");
		if (oldStartFullMsg.equals(startFullMsg)) {
			...
			if (x < 0)
				...
				x += 3;
			startFullMsg = startFullMsg.substring(0, x) + " in " + verb
					+ startFullMsg.substring(x);
		}
		msg.modify(msg.source(), msg.target(), this, msg.sourceCode(),
				msg.sourceMessage(),
				CMath.unsetb(msg.targetCode(), CMMsg.MASK_SOUND)
						| CMMsg.MASK_MOVE, startFullMsg.trim() + ".",
				CMath.unsetb(msg.othersCode(), CMMsg.MASK_SOUND)
						| CMMsg.MASK_MOVE, startFullMsg.trim() + ".");
		return true;
	}
	...
	protected boolean translateChannelMessage(CMMsg msg, String sourceWords) {
		if (CMath.bset(msg.sourceMajor(), CMMsg.MASK_CHANNEL)
			...
			msg.addTrailerMsg(CMClass.getMsg(msg.source(), null, null,
					CMMsg.NO_EFFECT, CMMsg.NO_EFFECT, msg.othersCode(),
					CMStrings.substituteSayInMessage(otherMes, sourceWords)
							+ " (translated from " + name() + ")"));
			return true;
		}
