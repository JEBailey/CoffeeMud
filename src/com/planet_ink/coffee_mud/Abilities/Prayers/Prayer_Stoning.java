...
public class Prayer_Stoning extends Prayer {
			...
			for (int i = 0; i < cits.size(); i++) {
				...
				if ((M.location() != mob.location()) || (mob.amDead())) {
					...
					if (CMLib.dice().rollPercentage() >= 50) {
						...
						CMLib.combat().postDamage(M, mob, W, dmg,
								CMMsg.MSG_WEAPONATTACK | CMMsg.MASK_ALWAYS,
								Weapon.TYPE_BASHING,
								"<S-NAME> stone(s) <T-NAMESELF>!");
					} else
						R.show(M, mob, null, CMMsg.MSG_NOISE,
								"<S-NAME> shout(s) obscenities at <T-NAMESELF>.");
				}
			}
			while (cits.size() < 10) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((warrants.size() == 0)
			...
			mob.tell("You are not allowed to stone " + target.Name()
					+ " at this time.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> call(s) for the stoning of <T-NAMESELF>.^?");
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> call(s) for the stoning of <T-NAMESELF>.");
