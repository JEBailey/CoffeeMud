...
public class Prayer_Disenchant extends Prayer {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int levelBonus = mob.phyStats().level()
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> appear(s) neutralized!" : "^S<S-NAME> "
							+ prayForWord(mob)
							+ " to neutralize <T-NAMESELF>.^?");
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayForWord(mob)
					+ " to neutralize <T-NAMESELF>, but nothing happens.");
