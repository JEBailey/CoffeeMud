...
public class Prayer_Philosophy extends Prayer {
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_WISDOM,
				affectableStats.getStat(CharStats.STAT_WISDOM) + increase);
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> give(s) <T-NAMESELF> something to think about.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				mob.location()
						.show(target, null, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> start(s) pondering the mysteries of the universe.");
			...
			return beneficialWordsFizzle(
					mob,
					target,
					"<S-NAME> give(s) <T-NAMESELF> something to think about, but it just confuses <T-HIM-HER>.");
