...
public class Prayer_SunCurse extends Prayer {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((mob.location() != null)
				&& (mob.location().getArea().getClimateObj().canSeeTheSun(mob
						.location()))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!auto)
				...
				&& (target.location().getArea().getClimateObj()
						.canSeeTheSun(target.location()))) {
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "" : "^S<S-NAME> " + prayWord(mob)
							+ " for an unholy sun curse upon <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					success = maliciousAffect(mob, target, asLevel, 0, -1);
					mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> <S-IS-ARE> under a mighty sun curse!");
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to sun curse <T-NAMESELF>, but flub(s) it.");
