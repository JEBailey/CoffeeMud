	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob)) && (msg.targetMinor() == CMMsg.TYP_DAMAGE))
			...
			msg.source().tell(msg.source(), mob, null,
					"The statis field around <T-NAME> protect(s) <T-HIM-HER>.");
			return false;
		}
			...
			CMMsg msg = CMClass
					.getMsg(mob, target, this,
							...
							auto ? "" : "^S<S-NAME> " + prayForWord(mob)
									+ " to place a stasis upon <T-NAMESELF>.^?");
				...
				if (msg.value() <= 0) {
					...
					mob.location()
							.show(target, null, CMMsg.MSG_OK_VISUAL,
									"<S-NAME> <S-IS-ARE> surrounded by a stasis field!");
				}
			}
		} else
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayForWord(mob)
					+ " to place <T-NAMESELF> into stasis, but flub(s) it.");
