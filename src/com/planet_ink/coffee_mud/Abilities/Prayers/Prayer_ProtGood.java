...
public class Prayer_ProtGood extends Prayer {
			...
			CMLib.combat()
					.postDamage(
							...
							CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS
									| CMMsg.TYP_UNDEAD, Weapon.TYPE_BURSTING,
			...
			if ((CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS))
				...
				msg.source()
						...
						.show((MOB) affected,
								null,
								CMMsg.MSG_OK_VISUAL,
								"The unholy field around <S-NAME> protect(s) <S-HIM-HER> from the goodly magic attack of "
										+ msg.source().name() + ".");
				return false;
			}
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			MOB victim = mob.getVictim();
			if ((victim != null) && (CMLib.flags().isGood(victim))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> become(s) protected from goodness."
							: "^S<S-NAME> " + prayWord(mob)
									+ " for protection from goodness.^?");
