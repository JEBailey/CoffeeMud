		...
		if (mob.charStats().getMyRace().racialCategory()
			...
			CMLib.combat().postDamage(
					...
					CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS
							| CMMsg.TYP_CAST_SPELL, Weapon.TYPE_BURSTING,
					"^S<T-HIS-HER> protective aura <DAMAGE> <T-NAME>!^?");
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> become(s) protected from the undead."
							: "^S<S-NAME> " + prayWord(mob)
									+ " for protection from the undead.^?");
