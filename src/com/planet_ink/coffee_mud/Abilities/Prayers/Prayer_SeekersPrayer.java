		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				for (Enumeration<Quest> q = CMLib.quests().enumQuests(); q
					...
					if (Q.name().equalsIgnoreCase("holidays") || !Q.running()
							|| (M == null)
				...
				if (seeks.size() == 0)
					...
					for (int p = 0; p < seeks.size(); p++) {
						...
						if ((p == seeks.size() - 1) && (p > 0))
							...
							message.append(",");
						message.append(" ").append(M.name(mob))
								.append(" in \"")
								.append(M.location().getArea().name(mob))
								.append("\"");
					}
					message.append(".");
