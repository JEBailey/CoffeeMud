...
public class Prayer_FlameWeapon extends Prayer {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.source().location() != null)
				...
				&& ((msg.value()) > 0) && (msg.tool() == affected)
				&& (!notAgain) && (msg.target() instanceof MOB)
				&& (!((MOB) msg.target()).amDead())) {
			try {
				notAgain = true;
				CMMsg msg2 = CMClass
						.getMsg(msg.source(), msg.target(), affected,
								CMMsg.MSG_OK_ACTION, CMMsg.MSK_MALICIOUS_MOVE
										| CMMsg.TYP_FIRE,
								CMMsg.MSG_NOISYMOVEMENT, null);
				if (msg.source().location().okMessage(msg.source(), msg2)) {
					msg.source().location().send(msg.source(), msg2);
					if (msg2.value() <= 0) {
						int flameDamage = (int) Math.round(Math.random() * 6);
						flameDamage *= (super.getXLEVELLevel(invoker()) + (super
								.getX1Level(invoker())));
						msg.addTrailerMsg(CMClass.getMsg(
								...
								"^RThe flame around "
										+ affected.name()
										+ " "
										+ CMLib.combat().standardHitWord(
												Weapon.TYPE_BURNING,
												flameDamage) + " <T-NAME>!^?"));
						CMMsg msg3 = CMClass.getMsg(msg.source(), msg.target(),
								null, CMMsg.MASK_ALWAYS | CMMsg.TYP_FIRE,
								CMMsg.MSG_DAMAGE, CMMsg.NO_EFFECT, null);
			...
			if ((affected != null) && (affected instanceof Item)) {
				if (((((Weapon) affected).material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_WOODEN)
					...
					if ((((Item) affected).owner() != null)
						...
						((MOB) ((Item) affected).owner())
								.tell("The flames around "
										+ ((Item) affected).name()
										+ " consume it.");
					destroyMe = (Item) affected;
				} else if ((((Item) affected).owner() != null)
						&& (((Item) affected).owner() instanceof MOB))
					((MOB) ((Item) affected).owner()).tell("The flames around "
							+ ((Item) affected).name() + " fade.");
			}
		}
		super.unInvoke();
		if (destroyMe != null)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> appear(s) surrounded by flames!"
							: "^S<S-NAME> hold(s) <T-NAMESELF> and "
									+ prayWord(mob) + ".^?");
			...
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> hold(s) <T-NAMESELF> and " + prayWord(mob)
							+ ", but nothing happens.");
