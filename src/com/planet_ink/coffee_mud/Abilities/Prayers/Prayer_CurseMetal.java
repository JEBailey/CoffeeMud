...
public class Prayer_CurseMetal extends Prayer {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		for (int i = 0; i < mob.numItems(); i++) {
			...
			if ((item != null) && (!item.amWearingAt(Wearable.IN_INVENTORY))
				...
				int damage = CMLib.dice().roll(
						1,
						6 + super.getXLEVELLevel(invoker())
								+ (2 * super.getX1Level(invoker())), 1);
				CMLib.combat().postDamage(
						...
						CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS
								| CMMsg.TYP_FIRE, Weapon.TYPE_BURSTING,
						item.name() + " <DAMAGE> <T-NAME>!");
		...
		if ((!mob.isInCombat()) && (mob.isMonster()) && (mob != invoker)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			return maliciousFizzle(mob, target, "<S-NAME> " + prayForWord(mob)
					+ " to curse <T-NAMESELF>, but the spell fizzles.");
