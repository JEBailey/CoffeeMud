...
public class Prayer_UndeniableFaith extends Prayer {
	...
	public long flags() {
		return Ability.FLAG_HOLY | Ability.FLAG_UNHOLY | Ability.FLAG_CHARMING;
	}
		...
		if (M.location() != null) {
			if ((!M.getWorshipCharID().equals(godName))
				...
				if (M.getWorshipCharID().length() > 0) {
					...
					if (D2 != null) {
						CMMsg msg2 = CMClass.getMsg(M, D2, this,
								CMMsg.MSG_REBUKE, null);
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		if ((affected instanceof MOB)
				...
				&& (msg.target() != null)
				&& ((msg.target() == invoker()) || (msg.target().Name()
						.equals(godName)))) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((target.getWorshipCharID().length() > 0)
			...
			if (!auto)
				mob.tell(target.name(mob) + " worships "
						+ target.getWorshipCharID()
						+ ", and may not be converted with this prayer.");
			return false;
		}
		if ((CMLib.flags().isAnimalIntelligence(target)
				|| CMLib.flags().isGolem(target) || (D == null))) {
			if (!auto)
				mob.tell(target.name(mob)
						+ " can not be converted with this prayer.");
			return false;
		}
		if (!auto) {
			if (convertStack.contains(target)) {
				Long L = (Long) convertStack.elementAt(
						convertStack.indexOf(target), 2);
			...
			if (convertStack.contains(target)) {
				mob.tell(target.name(mob)
						+ " must wait to be undeniably faithful again.");
				return false;
			}
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this, type, auto ? ""
					: "^S<S-NAME> " + prayWord(mob)
							+ " for <T-NAMESELF> to BELIEVE!^?");
			CMMsg msg2 = CMClass.getMsg(target, D, this, CMMsg.MSG_SERVE,
					"<S-NAME> BELIEVE(S) !!!");
			CMMsg msg3 = CMClass.getMsg(mob, target, this,
					CMMsg.MSK_CAST_VERBAL | mal | CMMsg.TYP_MIND
			...
			if ((mob.location().okMessage(mob, msg))
				...
				if ((msg.value() <= 0) && (msg3.value() <= 0)) {
					...
					if (mob != target)
						CMLib.leveler().postExperience(mob, target, null, 25,
								false);
