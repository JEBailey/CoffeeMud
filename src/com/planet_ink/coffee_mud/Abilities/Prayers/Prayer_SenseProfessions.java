			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (professionsV.size() == 0)
					...
					mob.tell(
							...
							target,
							null,
							"<T-NAME> seem(s) like <T-HE-SHE> understands the following "
									+ senseWhatStr() + ": "
									+ CMParms.toStringList(professionsV));
			}
		} else
			beneficialWordsFizzle(mob, target, auto ? "" : "<S-NAME> "
					+ prayWord(mob)
					+ " and peer(s) at <T-NAMESELF>, but then blink(s).");
