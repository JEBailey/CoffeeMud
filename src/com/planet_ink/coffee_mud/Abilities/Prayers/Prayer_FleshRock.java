		...
		if (canBeUninvoked()) {
			...
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> flesh is no longer made of rock.");
			if (prevState != null)
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					...
					statue.setDisplayText("a rocky statue of " + name
							+ " stands here.");
					statue.setDescription("It`s a hard rocky statue, which looks exactly like "
							+ name + ".");
					...
					if (success && (A != null)) {
						...
						((Prayer_FleshRock) A).prevState = (CharState) target
								.curState().copyOf();
					}
				}
			}
			...
			return maliciousFizzle(mob, target, "<S-NAME> " + prayWord(mob)
					+ " and point(s) at <T-NAMESELF>, but nothing happens.");
