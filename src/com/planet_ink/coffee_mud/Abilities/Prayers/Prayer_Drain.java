		...
		if (success) {
			...
			CMMsg msg2 = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> reach(es) at <T-NAMESELF>, "
									+ prayingWord(mob) + "!^?");
				...
				if ((msg.value() <= 0) && (msg2.value() <= 0)) {
					...
					CMLib.combat()
							.postDamage(
									...
									auto ? "<T-NAME> shudder(s) in a draining magical wake."
											: "The draining grasp <DAMAGE> <T-NAME>.");
					if (mob != target)
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> reach(es) for <T-NAMESELF>, " + prayingWord(mob)
							+ ", but the spell fades.");
