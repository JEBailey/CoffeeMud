...
public class Prayer_AuraIntolerance extends Prayer {
	...
	public String name() {
		return "Aura of Intolerance";
	}

	public String displayText() {
		return "(Intolerance Aura)";
	}
	...
	public void unInvoke() {
		...
		if ((canBeUninvoked()) && (M != null) && (!M.amDead())
			...
			M.location().show(M, null, CMMsg.MSG_OK_VISUAL,
					"The intolerant aura around <S-NAME> fades.");
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.source() == affected)
				...
				&& (!((MOB) msg.target()).getWorshipCharID().equals(
						msg.source().getWorshipCharID()))) {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		for (int i = 0; i < R.numInhabitants(); i++) {
			...
			if ((M != null)
					&& (M != ((MOB) affected))
					&& ((M.getWorshipCharID().length() == 0) || (((MOB) affected)
							.getWorshipCharID().length() > 0)
							&& (!M.getWorshipCharID().equals(
									((MOB) affected).getWorshipCharID())))) {
				if (M.getWorshipCharID().length() > 0)
					CMLib.combat()
							.postDamage(
									...
									"The intolerant aura around <S-NAME> <DAMAGES> <T-NAMESELF>!");
				else
					CMLib.combat()
							.postDamage(
									((MOB) affected),
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			if (((mob.getWorshipCharID().length() == 0) || (CMLib.map()
					.getDeity(mob.getWorshipCharID()) == null)))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"The aura of intolerance is already with <S-NAME>.");
			return false;
		}
		if ((!auto)
				&& ((mob.getWorshipCharID().length() == 0) || (CMLib.map()
						.getDeity(mob.getWorshipCharID()) == null))) {
		...
		if (success) {
			...
			return beneficialWordsFizzle(
					...
					"<S-NAME> "
							+ prayWord(mob)
							+ " for an aura of intolerance, but <S-HIS-HER> plea is not answered.");
