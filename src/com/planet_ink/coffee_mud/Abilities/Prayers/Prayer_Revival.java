	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((affected != null) && (affected instanceof MOB)
			...
			if ((D != null) && (CMLib.dice().rollPercentage() < 50))
				...
				case 1:
					CMLib.commands().postSay(
							...
							D.name()
									+ " is great! Shout "
									+ CMStrings.capitalizeAndLower(D
											.charStats().hisher())
									+ " praises!", false, false);
					break;
				case 2:
					CMLib.commands().postSay(mob, null, "Can I hear an AMEN?!",
							false, false);
					break;
				case 3:
					CMLib.commands().postSay(mob, null,
							"Praise " + D.name() + "!", false, false);
					break;
				case 4:
					CMLib.commands().postSay(mob, null,
							"Halleluyah! " + D.name() + " is great!", false,
							false);
					break;
				case 5:
					CMLib.commands()
							.postSay(mob, null,
									"Let's hear it for " + D.name() + "!",
									false, false);
					break;
				case 6:
					CMLib.commands()
							.postSay(mob, null,
									"Exalt the name of " + D.name() + "!",
									false, false);
					break;
				case 7:
					if (clerics.size() > 1) {
						MOB M = (MOB) clerics.elementAt(CMLib.dice().roll(1,
								clerics.size(), -1));
						if (M != mob)
							CMLib.commands().postSay(mob, null,
									"Preach it " + M.name(mob) + "!", false,
									false);
						else
							CMLib.commands().postSay(mob, null,
									"I LOVE " + D.name() + "!", false, false);
					} else
						CMLib.commands().postSay(mob, null,
								"I LOVE " + D.name() + "!", false, false);
					break;
				case 8:
					CMLib.commands().postSay(mob, null,
							"Holy is the name of " + D.name() + "!", false,
							false);
					break;
				case 9:
					CMLib.commands().postSay(mob, null,
							"Do you BELIEVE?!? I BELIEVE!!!", false, false);
					break;
				case 10:
					CMLib.commands().postSay(mob, null, "Halleluyah!", false,
							false);
					break;
				case 11:
					mob.enqueCommand(CMParms
							.parse("EMOTE do(es) a spirit-filled dance!"),
							Command.METAFLAG_FORCED, 0);
					break;
				case 12:
					mob.enqueCommand(
							CMParms.parse("EMOTE wave(s) <S-HIS-HER> hands in the air!"),
							Command.METAFLAG_FORCED, 0);
					break;
				case 13:
					mob.enqueCommand(
							CMParms.parse("EMOTE catch(es) the spirit of "
									+ D.name() + "!"), Command.METAFLAG_FORCED,
							0);
					break;
				}
			if ((clerics.size() > 2) && (inhabs.size() > 0)) {
				...
				MOB M = (MOB) inhabs.elementAt(CMLib.dice().roll(1,
						inhabs.size(), -1));
				if ((M != null) && (levels >= (M.phyStats().level() + bonus))) {
					...
					if (M.getWorshipCharID().length() == 0) {
						Ability A = CMClass
								.getAbility("Prayer_UndeniableFaith");
						if (A != null)
							if (A.invoke(mob, M, true, 0)) {
								if (M.getWorshipCharID().equals(
										mob.getWorshipCharID())) {
									for (int c = 0; c < clerics.size(); c++) {
										MOB M2 = (MOB) clerics.elementAt(c);
										if (M2 != mob)
											CMLib.leveler().postExperience(M2,
													M, null, 25, false);
									}
								}
							}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already participating in a revival.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> start(s) a revival!"
									: "^S<S-NAME> "
											+ prayWord(mob)
											+ " for successful revival, and then start(s) MOVING!^?");
			...
			beneficialWordsFizzle(mob, target, "<S-NAME> " + prayWord(mob)
					+ " for a successful revival, but fail(s).");
