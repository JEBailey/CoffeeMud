...
public class Prayer_CreateWater extends Prayer {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (D.containsDrink()
				&& (D.liquidType() != RawMaterial.RESOURCE_FRESHWATER)) {
			mob.tell(target.name(mob)
					+ " already contains another liquid, and must be emptied first.");
			return false;
		}
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (target.owner() instanceof Room)
					mob.location().showHappens(CMMsg.MSG_OK_VISUAL,
							target.name() + " fills up with water!");
				else
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob)
					+ " over <T-NAME> for water, but there is no answer.");
