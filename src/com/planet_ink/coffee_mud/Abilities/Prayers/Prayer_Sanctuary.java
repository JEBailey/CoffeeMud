		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"The sanctuary around <S-NAME> fades.");
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "" : "^S<S-NAME> " + prayWord(mob)
							+ " that <T-NAME> be given sanctuary from harm.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
						"A white aura surrounds <S-NAME>.");
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob)
					+ " for sanctuary, but <S-HIS-HER> plea is not answered.");
