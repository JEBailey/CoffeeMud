...
public class Prayer_Prophecy extends Prayer {
	...
	public void unInvoke() {
		...
		if (CMLib.ableMapper().qualifyingLevel(mob, this) > 1)
			numProphesies += (super.adjustedLevel(mob, 0) / CMLib.ableMapper()
					.qualifyingLevel(mob, this));
		List<Pair<Integer, Quest>> prophesies = new Vector<Pair<Integer, Quest>>();
		for (Enumeration<Quest> q = CMLib.quests().enumQuests(); q
				.hasMoreElements();) {
			Quest Q = q.nextElement();
			if (Q.isCopy() || (Q.duration() == 0)
					|| (Q.name().equalsIgnoreCase("holidays"))) {
				continue;
			}
			...
			if (ticksRemaining != Integer.MAX_VALUE) {
				if (prophesies.size() < numProphesies)
					prophesies.add(new Pair<Integer, Quest>(Integer
							.valueOf(ticksRemaining), Q));
				else {
					Pair<Integer, Quest> highP = null;
					for (Pair<Integer, Quest> P : prophesies)
						if ((highP == null)
								|| (P.first.intValue() > highP.first.intValue()))
							highP = P;
					if ((highP == null)
							|| (highP.first.intValue() > ticksRemaining)) {
		...
		if (prophesies.size() == 0)
			...
			for (int p = 0; p < prophesies.size(); p++) {
				...
				Quest Q = P.second;
				String name = Q.name();
				final long timeTil = P.first.longValue()
						* CMProps.getTickMillis();
				final String timeTilDesc = clock
						.deriveEllapsedTimeString(timeTil);
				...
				if ((p == prophesies.size() - 1) && (p > 0))
					...
					message.append(",");
				message.append(" \"").append(name).append("\"").append(ending)
						.append(timeTilDesc);
			}
			message.append(".");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, null, auto), auto ? "" : "^S<T-NAME> "
							+ prayWord(mob) + ", entering a divine trance.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				beneficialAffect(mob, target, asLevel, 3);
			}
		} else
			beneficialWordsFizzle(mob, target, "<T-NAME> " + prayWord(mob)
					+ ", but nothing happens.");
