...
public class Prayer_HolyWord extends Prayer implements MendingSkill {
	...
	public boolean supportsMending(Physical item) {
		return (item instanceof MOB)
				&& ((Prayer_Bless.getSomething((MOB) item, true) != null) || (CMLib
						.flags().domainAffects(item, Ability.DOMAIN_CURSING)
						.size() > 0));
	}
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> blinding holy aura fades.");
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		String str = (auto ? "The holy word is spoken."
				: "^S<S-NAME> speak(s) the holy word" + ofDiety(mob)
						+ " to <T-NAMESELF>.^?")
				+ CMLib.protocol().msp("bless.wav", 10);
		String missStr = "<S-NAME> speak(s) the holy word" + ofDiety(mob)
				+ ", but nothing happens.";
					...
					if (mob.location().okMessage(mob, msg)) {
						...
						if (msg.value() <= 0) {
							if (CMLib.flags().canBeHeardSpeakingBy(mob, target)) {
								str = null;
								Item I = Prayer_Bless
										.getSomething(target, true);
								HashSet<Item> alreadyDone = new HashSet<Item>();
								while ((I != null)
										&& (!alreadyDone.contains(I))) {
									alreadyDone.add(I);
									CMMsg msg2 = CMClass.getMsg(target, I,
											null, CMMsg.MASK_ALWAYS
													| CMMsg.MSG_DROP,
											"<S-NAME> release(s) <T-NAME>.");
									target.location().send(target, msg2);
									Prayer_Bless.endLowerCurses(I, CMLib
											.ableMapper()
											.lowestQualifyingLevel(ID()));
									I.recoverPhyStats();
									I = Prayer_Bless.getSomething(target, true);
								}
								Prayer_Bless.endAllOtherBlessings(mob, target,
										CMLib.ableMapper()
												.lowestQualifyingLevel(ID()));
								Prayer_Bless.endLowerCurses(target,
										CMLib.ableMapper()
												.lowestQualifyingLevel(ID()));
								beneficialAffect(mob, target, asLevel, 0);
								target.recoverPhyStats();
							} else if (CMath.bset(affectType,
									CMMsg.MASK_MALICIOUS))
								maliciousFizzle(mob, target,
										"<T-NAME> did not hear the word!");
							else
