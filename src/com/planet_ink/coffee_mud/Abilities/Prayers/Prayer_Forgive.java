...
public class Prayer_Forgive extends Prayer {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (B != null) {
			criminals = B.getCriminals(
					CMLib.law().getLegalObject(mob.location()), name);
			if (criminals.size() > 0)
		...
		if (success) {
			if (warrants.size() == 0)
				beneficialWordsFizzle(mob, null, "<S-NAME> " + prayForWord(mob)
						+ " to forgive " + name + " for no reason at all.");
			else {
			...
			beneficialWordsFizzle(mob, null, "<S-NAME> " + prayForWord(mob)
					+ " to forgive " + name + ", but nothing happens.");
