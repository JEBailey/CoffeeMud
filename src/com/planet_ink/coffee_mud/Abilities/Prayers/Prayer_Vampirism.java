			...
			if ((M.isMonster())
				...
				for (int i = 0; i < M.location().numItems(); i++) {
					...
					if ((I != null)
							...
							&& (raceWithBlood(((DeadBody) I).charStats()
									.getMyRace())))
						B = (DeadBody) I;
					else if ((I != null)
							&& (I instanceof Drink)
							&& (I.container() == null)
							&& ((I.material() == RawMaterial.RESOURCE_BLOOD) || (((Drink) I)
									.liquidType() == RawMaterial.RESOURCE_BLOOD)))
						D = (Drink) I;
				}
				if (D != null) {
					...
					if ((M2 != null) && (M2 != M)
							&& (raceWithBlood(M2.charStats().getMyRace())))
						M.setVictim(M2);
				}
			}
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> invoke(s) a vampiric hunger upon <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					mob.location()
							.show(target, null, CMMsg.MSG_OK_VISUAL,
									"<S-NAME> <S-IS-ARE> inflicted with vampiric hunger!");
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to inflict vampirism upon <T-NAMESELF>, but flub(s) it.");
