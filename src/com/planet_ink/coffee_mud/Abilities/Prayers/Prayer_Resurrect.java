...
public class Prayer_Resurrect extends Prayer implements MendingSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Physical body = null;
		body = getTarget(mob, mob.location(), givenTarget, commands,
		...
		if ((body == null) && (CMSecurity.isASysOp(mob))) {
			...
			if (name.equalsIgnoreCase("list")) {
				...
				for (int i = 0; i < allObjs.size(); i++) {
					...
					mob.tell(CMStrings.padRight(
							((DatabaseEngine.PlayerData) allDataPs.elementAt(i)).who,
							15)
							+ CMStrings.padRight(body.name(), 45)
							+ CMStrings.padRight(
									((age == null) ? "" : CMLib.time()
											.date2String(
													CMath.s_long(age.text()))),
									16)
							+ "\n\r"
							+ CMStrings.padRight("", 15)
							+ body.description());
				}
				return false;
			}
			Physical P = (Physical) CMLib.english().fetchEnvironmental(allObjs,
					name, true);
			if (P == null)
				P = (Physical) CMLib.english().fetchEnvironmental(allObjs,
						name, false);
			if (P == null)
				return false;
			for (int i = 0; i < allObjs.size(); i++)
				if (allObjs.elementAt(i) == P) {
					nonPlayerData = (DatabaseEngine.PlayerData) allDataPs
							.elementAt(i);
					body = P;
					break;
				}
		...
		if (nonPlayerData == null) {
			...
			if (!playerCorpse) {
				Ability AGE = body.fetchEffect("Age");
				if ((AGE != null) && (CMath.isLong(AGE.text()))
						&& (CMath.s_long(AGE.text()) > Short.MAX_VALUE)) {
					MOB M = null;
					...
					if (nonPlayerData == null) {
						mob.tell("You can't seem to focus on "
								+ body.Name()
								+ "'s spirit.  Perhaps if loved ones were here?");
						return false;
					}
				} else {
					mob.tell("You can't resurrect "
							+ ((DeadBody) body).charStats().himher() + ".");
					return false;
				}
			}
		}
		...
		if (success) {
			...
			beneficialWordsFizzle(
					...
					body,
					auto ? ""
							: "<S-NAME> attempt(s) to resurrect <T-NAMESELF>, but nothing happens.");
