...
public class Prayer_HolyDay extends Prayer {
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		if ((msg.tool() instanceof Ability)
				...
				&& (msg.source() != invoker())) {
			msg.source().tell(
					"You are not allowed to work on the holy day of " + godName
							+ ".");
			return false;
				...
				|| (msg.sourceMinor() == CMMsg.TYP_BORROW) || (msg
				.sourceMinor() == CMMsg.TYP_DEPOSIT))
				&& (msg.source() != invoker())) {
			msg.source().tell(
					"You are not allowed to work or do commerce on the holy day of "
							+ godName + ".");
			return false;
			...
			if ((B != null) && (B.controlPoints() > 0)) {
				msg.source().tell(
						"There can be no conquest on the holy day of "
								+ godName + ".");
				return false;
			}
		}
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				for (Enumeration e = target.getMetroMap(); e.hasMoreElements();) {
					...
					if ((godName.length() == 0)
							|| (CMLib.map().getDeity(godName) == null))
						godName = "the gods";
					R.showHappens(CMMsg.MSG_OK_VISUAL, "A holy day of "
							+ godName + " has begun!");
				}
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob)
					+ " for a holy day, but <S-HIS-HER> plea is not answered.");
