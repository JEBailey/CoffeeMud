	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					(auto ? "<T-NAME> become(s) clothed in holiness."
							: "^S<S-NAME> " + prayForWord(mob)
									+ " to clothe <T-NAMESELF> in holiness.^?")
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				while ((I != null) && (!alreadyDone.contains(I))) {
					alreadyDone.add(I);
					CMMsg msg2 = CMClass.getMsg(target, I, null,
							CMMsg.MASK_ALWAYS | CMMsg.MSG_DROP,
							"<S-NAME> release(s) <T-NAME>.");
					target.location().send(target, msg2);
					Prayer_Bless.endLowerCurses(I, CMLib.ableMapper()
							.lowestQualifyingLevel(ID()));
					I.recoverPhyStats();
					I = Prayer_Bless.getSomething(target, true);
				}
				Prayer_Bless.endAllOtherBlessings(mob, target, CMLib
						.ableMapper().lowestQualifyingLevel(ID()));
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob) + " for holiness, but nothing happens.");
