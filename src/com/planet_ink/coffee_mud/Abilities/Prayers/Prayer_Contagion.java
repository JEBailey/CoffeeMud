	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		for (final Enumeration<Ability> a = mob.effects(); a.hasMoreElements();) {
			...
			if ((A != null)
					...
					&& (((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_SPELL) || ((A
		...
		Ability thisOne = (Ability) choices.elementAt(CMLib.dice().roll(1,
				choices.size(), -1));
		if ((target == null) || (thisOne == null)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> become(s) contagious!" : "^S<S-NAME> "
							+ prayWord(mob)
							+ " for a contagion to inflict <T-NAMESELF>.^?");
			CMMsg msg2 = CMClass.getMsg(mob, target, this, CMMsg.TYP_DISEASE
