...
public class Prayer_ProtPoison extends Prayer {
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectedStats) {
		...
		affectedStats.setStat(CharStats.STAT_SAVE_POISON,
				affectedStats.getStat(CharStats.STAT_SAVE_POISON) + 100);
	}
			...
			if ((msg.tool() != null) && (CMLib.dice().rollPercentage() > 50)
				...
				msg.source()
						...
						.show((MOB) affected, null, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> magically repell(s) the poison.");
				return false;
			}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE> protection from poison.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> attain(s) a anitidotal protection."
							: "^S<S-NAME> " + prayWord(mob)
									+ " for protection from poisons.^?");
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob)
					+ " for protection from poisons, but go(es) unanswered.");
