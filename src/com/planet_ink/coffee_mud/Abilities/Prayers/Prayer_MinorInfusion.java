		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "A soft yellow glow surrounds <T-NAME>."
									: "^S<S-NAME> "
											+ prayWord(mob)
											+ ", delivering a light touch of infusion to <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				int healing = CMLib.dice().roll(2, adjustedLevel(mob, asLevel),
						10);
