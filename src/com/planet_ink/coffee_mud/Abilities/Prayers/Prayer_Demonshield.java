		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> demonic flame shield vanishes.");
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							((auto ? "" : "^S<S-NAME> " + prayWord(mob) + ".  ") + "A field of unholy flames erupt(s) around <T-NAME>!^?")
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob) + ", but only sparks emerge.");
