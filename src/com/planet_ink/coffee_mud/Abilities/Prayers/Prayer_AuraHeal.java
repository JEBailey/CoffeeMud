...
public class Prayer_AuraHeal extends Prayer {
	...
	public void unInvoke() {
		...
		if (canBeUninvoked())
			R.showHappens(CMMsg.MSG_OK_VISUAL,
					"The healing aura around you fades.");
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		for (int i = 0; i < R.numInhabitants(); i++) {
			...
			if ((M != null)
					&& (M.curState().getHitPoints() < M.maxState()
							.getHitPoints())
					&& ((H == null) || (M.getVictim() == null) || (!H
							.contains(M.getVictim())))) {
				int oldHP = M.curState().getHitPoints();
				if (invoker() != null) {
					int healing = CMLib.dice().roll(2,
							adjustedLevel(invoker(), 0), 4);
					CMLib.combat().postHealing(invoker(), M, this,
							CMMsg.MASK_ALWAYS | CMMsg.TYP_CAST_SPELL, healing,
							null);
				} else {
					int healing = CMLib.dice().roll(2,
							CMLib.ableMapper().lowestQualifyingLevel(ID()), 4);
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> " + prayWord(mob)
									+ " for all to feel better.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"A healing aura descends over the area!");
			...
			return beneficialWordsFizzle(
					...
					"<S-NAME> "
							+ prayWord(mob)
							+ " for an aura of healing, but <S-HIS-HER> plea is not answered.");
