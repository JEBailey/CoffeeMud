...
public class Prayer_Doomspout extends Prayer implements DiseaseAffect {
		...
		if ((--plagueDown) <= 0) {
			...
			case 1:
				CMLib.commands().postSay(mob, null,
						"Repent, or " + godName + " will consume your soul!",
						false, false);
				break;
			case 2:
				CMLib.commands().postSay(mob, null,
						"We are all damned! Hope is forgotten!", false, false);
				break;
			case 3:
				CMLib.commands().postSay(mob, null,
						godName + " has damned us all!", false, false);
				break;
			case 4:
				CMLib.commands().postSay(mob, null,
						"Death is the only way out for us now!", false, false);
				break;
			case 5:
				CMLib.commands().postSay(mob, null,
						"The finger of " + godName + " will destroy all!",
						false, false);
				break;
			case 6:
				CMLib.commands()
						.postSay(
								mob,
								null,
								"The waters will dry! The air will turn cold! Our bodies will fail! We are Lost!",
								false, false);
				break;
			case 7:
				CMLib.commands().postSay(
						mob,
						null,
						"Nothing can save you! Throw yourself on the mercy of "
								+ godName + "!", false, false);
				break;
			case 8:
				CMLib.commands().postSay(mob, null,
						godName + " will show us no mercy!", false, false);
				break;
			case 9:
				CMLib.commands().postSay(mob, null,
						godName + " has spoken! We will all be destroyed!",
						false, false);
				break;
			case 10:
			case 11:
			case 12:
				CMLib.commands().postSay(mob, null,
						"Our doom is upon us! The end is near!", false, false);
				break;
			}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected != null) && (msg.source() == affected)
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead())) {
				...
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> doomspout disease clear up.");
			}
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> inflict(s) an unholy disease upon <T-NAMESELF>.^?");
			CMMsg msg2 = CMClass.getMsg(mob, target, this,
					CMMsg.MSK_CAST_MALICIOUS_VERBAL | CMMsg.TYP_DISEASE
							| (auto ? CMMsg.MASK_ALWAYS : 0), null);
			CMMsg msg3 = CMClass.getMsg(mob, target, this,
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to inflict a disease upon <T-NAMESELF>, but flub(s) it.");
