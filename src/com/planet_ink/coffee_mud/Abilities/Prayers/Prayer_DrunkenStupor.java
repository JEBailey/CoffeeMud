...
public class Prayer_DrunkenStupor extends Prayer {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.source().location() == null)
			return true;
		if ((!msg.targetMajor(CMMsg.MASK_ALWAYS)) && (msg.targetMajor() > 0)) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> "
											+ prayForWord(mob)
											+ " to inflict a drunken stupor upon <T-NAMESELF>.^?");
			...
			return maliciousFizzle(
					...
					"<S-NAME> "
							+ prayForWord(mob)
							+ " to inflict a drunken stupor upon <T-NAMESELF>, but flub(s) it.");
