...
public class Prayer_AnimateGhost extends Prayer {
	...
	public void makeGhostFrom(Room R, DeadBody body, MOB mob, int level) {
		...
		newMOB.basePhyStats().setDisposition(
				PhyStats.IS_FLYING
						| ((mob == null) ? PhyStats.IS_INVISIBLE : 0));
		newMOB.basePhyStats().setSensesMask(
				PhyStats.CAN_SEE_DARK | PhyStats.CAN_SEE_INVISIBLE);
		...
		while (it < newMOB.location().numItems()) {
			...
			if ((item != null) && (item.container() == body)) {
				CMMsg msg2 = CMClass.getMsg(newMOB, body, item, CMMsg.MSG_GET,
						null);
				newMOB.location().send(newMOB, msg2);
				CMMsg msg4 = CMClass.getMsg(newMOB, item, null, CMMsg.MSG_GET,
						null);
				newMOB.location().send(newMOB, msg4);
				CMMsg msg3 = CMClass.getMsg(newMOB, item, null, CMMsg.MSG_WEAR,
						null);
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> " + prayForWord(mob)
									+ " to animate <T-NAMESELF> as a ghost.^?");
