		...
		if ((tickID == Tickable.TICKID_MOB)
			...
			for (int i = 0; i < lastRoom.numInhabitants(); i++) {
				...
				if ((mob != null)
					...
					((MOB) affected).tell(mob.name((MOB) affected)
							+ " gives off a cold dark vibe.");
			}
		}
		return true;
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already sensing undead things.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> gain(s) dark cold senses!" : "^S<S-NAME> "
							+ prayWord(mob)
							+ ", and gain(s) dark cold senses!^?");
