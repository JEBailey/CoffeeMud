...
public class Prayer_Position extends Prayer {
	...
	public String trailTo(Room R1, Room R2) {
		...
		CMLib.tracking().getRadiantRooms(R1, set, flags, R2, Integer.MAX_VALUE,
				null);
		int foundAt = -1;
		...
		for (int s = trailV.size() - 1; s >= 1; s--) {
			...
			theDirTrail.addElement(Character
					.toString(Directions.getDirectionName(
							getRoomDirection(R, RA, empty)).charAt(0))
					+ " ");
		}
		...
		while (theDirTrail.size() > 0) {
			...
			if (lastNum == 0) {
				...
				if (lastNum == 1)
					...
					theTrail.append(Integer.toString(lastNum)
							+ Character.toString(lastDir) + " ");
				lastDir = s.charAt(0);
				lastNum = 1;
			}
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, null, this,
					verbalCastCode(mob, null, auto), auto ? "" : "^S<S-NAME> "
							+ prayWord(mob) + " for a position check.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				mob.tell("The trail from " + lastPosition.name()
						+ " to here is: "
						+ trailTo(lastPosition, mob.location()));
				lastPosition = mob.location();
			}
		} else
			beneficialWordsFizzle(mob, null, "<S-NAME> " + prayWord(mob)
					+ " for a position check, but fail(s).");
