		...
		if ((success) && (B != null)) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> accuse(s) <T-NAMESELF> of heresy"
									+ againstTheGods(mob) + "!^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					...
					if (D == null) {
						...
						desc = "Angering " + D.name()
								+ " will bring doom upon us all!";
					String crimeLocs = "";
					...
					B.addWarrant(CMLib.law().getLegalObject(mob.location()),
							target, D, crimeLocs, crimeFlags, crime, sentence,
							desc);
				}
			}
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> accuse(s) <T-NAMESELF> of heresy"
							+ againstTheGods(mob) + ", but nothing happens.");
