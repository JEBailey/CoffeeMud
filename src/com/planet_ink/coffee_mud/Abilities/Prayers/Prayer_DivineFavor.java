...
public class Prayer_DivineFavor extends Prayer {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((affected instanceof MOB)
				...
				&& (!((MOB) affected).getVictim().getWorshipCharID()
						.equals(((MOB) affected).getWorshipCharID()))) {
			MOB deityM = CMLib.map().getDeity(
			...
			if (deityM != null) {
				...
				((MOB) affected)
						...
						.showOthers(
								deityM,
								((MOB) affected).getVictim(),
								null,
								CMMsg.MSG_OK_ACTION,
								deityM.name()
										+ " strike(s) down <T-NAME> with all of <S-HIS-HER> divine fury!");
				CMLib.combat().postDeath(deityM, ((MOB) affected).getVictim(),
						null);
			}
		}
		return true;
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> become(s) divinely favored."
