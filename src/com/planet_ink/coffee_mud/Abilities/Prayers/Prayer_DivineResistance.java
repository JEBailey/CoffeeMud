...
public class Prayer_DivineResistance extends Prayer {
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		...
		if ((msg.target() == affected) && (affected instanceof MOB)
				&& ((msg.tool() == null) || (!permProts.contains(msg.tool())))
				&& (prots > 0) && (msg.source().location() != null)) {
			...
			if ((msg.tool() instanceof Trap) || (proceed)) {
				...
				msg.source()
						.location()
						.show((MOB) msg.target(), msg.source(), this,
								CMMsg.MSG_OK_VISUAL,
								"<S-YOUPOSS> divine protection glows!");
				if (prots == 0)
					unInvoke();
			}
		}
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> divine resistance fades.");
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "<T-NAME> become(s) protected by divine resistance."
									: "^S<S-NAME> "
											+ prayWord(mob)
											+ " for <T-NAMESELF> to be protected by divine resistance.^?"));
			...
			return beneficialWordsFizzle(
					...
					"<S-NAME> "
							+ prayWord(mob)
							+ " for <T-NAMESELF> to have divine resistance, but nothing happens.");
