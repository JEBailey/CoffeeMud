...
public class Prayer_Behemoth extends Prayer {
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectedStats) {
		...
		affectedStats.setStat(CharStats.STAT_STRENGTH,
				affectedStats.getStat(CharStats.STAT_STRENGTH) + 5);
		affectedStats.setStat(CharStats.STAT_DEXTERITY,
				affectedStats.getStat(CharStats.STAT_DEXTERITY) - 5);
		long newWeight = affectedStats.getStat(CharStats.STAT_WEIGHTADJ)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already BEHEMOTH in size.");
			return false;
		}
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				target.location().show(target, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> grow(s) to BEHEMOTH size!");
