...
public class Prayer_Faithless extends Prayer {
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			...
			if (target instanceof MOB) {
				if (((MOB) target).charStats().getCurrentClass().baseClass()
						.equals("Cleric"))
					return Ability.QUALITY_INDIFFERENT;
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target == null)
			return false;
		if ((!auto)
				&& (target.charStats().getCurrentClass().baseClass()
						.equals("Cleric"))) {
		...
		if (CMLib.flags().isAnimalIntelligence(target)
			...
			if (!auto)
				mob.tell(target.name(mob)
						+ " can not be affected by this prayer.");
			return false;
		}
		...
		if ((success) && (D != null)) {
			...
			maliciousFizzle(mob, target, auto ? "" : "<S-NAME> "
