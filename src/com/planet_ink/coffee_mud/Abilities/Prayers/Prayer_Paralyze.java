		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> invoke(s) an unholy paralysis upon <T-NAMESELF>.^?");
			CMMsg msg2 = CMClass
			...
			if ((mob.location().okMessage(mob, msg))
				...
				if ((msg.value() <= 0) && (msg2.value() <= 0)) {
					...
					if (duration < 2)
						duration = 2;
					success = maliciousAffect(mob, target, asLevel, duration,
							-1);
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to paralyze <T-NAMESELF>, but flub(s) it.");
