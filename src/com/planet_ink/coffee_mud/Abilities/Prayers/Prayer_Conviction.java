	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((mob.getWorshipCharID().length() > 0)
			...
			affectableStats.setAttackAdjustment(affectableStats
					.attackAdjustment()
					+ (affectableStats.attackAdjustment() / (8 - xlvl)));
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			...
			if (target instanceof MOB) {
				if (!((MOB) target).getWorshipCharID().equals(
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!target.getWorshipCharID().equals(mob.getWorshipCharID())) {
			mob.tell(target.name(mob)
					+ " must worship your god for this prayer to work.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> become(s) filled with conviction!"
									: "^S<S-NAME> "
											+ prayWord(mob)
											+ " for <T-YOUPOSS> religious conviction!^?");
			...
			return beneficialWordsFizzle(mob, null, "<S-NAME> " + prayWord(mob)
					+ " for <T-YOUPOSS> conviction, but there is no answer.");
