...
public class Prayer_UnholyWord extends Prayer {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		String str = auto ? "The unholy word is spoken."
				: "^S<S-NAME> speak(s) the unholy word" + ofDiety(mob)
						+ " to <T-NAMESELF>.^?";
			...
			for (int i = 0; i < room.numInhabitants(); i++) {
				...
				if (success) {
					...
					if (room.okMessage(mob, msg)) {
						...
						if (msg.value() <= 0) {
							if (CMLib.flags().canBeHeardSpeakingBy(mob, target)) {
								...
								if (I != null) {
									Prayer_Curse.endLowerBlessings(I, CMLib
											.ableMapper()
											.lowestQualifyingLevel(ID()));
									I.recoverPhyStats();
								}
								Prayer_Curse.endLowerBlessings(target,
										CMLib.ableMapper()
												.lowestQualifyingLevel(ID()));
								...
								maliciousFizzle(mob, target,
										"<T-NAME> did not hear the unholy word!");
							else
					...
					if (CMath.bset(affectType, CMMsg.MASK_MALICIOUS))
						maliciousFizzle(mob, target,
								"<S-NAME> attempt(s) to speak the unholy word to <T-NAMESELF>, but flub(s) it.");
					else
						beneficialWordsFizzle(mob, target,
								"<S-NAME> attempt(s) to speak the unholy word to <T-NAMESELF>, but flub(s) it.");
					return false;
