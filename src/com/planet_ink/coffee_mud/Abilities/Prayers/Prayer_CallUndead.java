...
public class Prayer_CallUndead extends Prayer {
	...
	public long flags() {
		return Ability.FLAG_UNHOLY | Ability.FLAG_TRANSPORTING
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> call(s) <S-HIS-HER> undead to come to <S-HIM-HER>!^?");
			if ((mob.location().okMessage(mob, msg)) && (oldRoom != null)
				...
				if (A != null) {
					A.invoke(
							follower,
							CMParms.parse("\""
									+ CMLib.map().getExtendedRoomID(newRoom)
									+ "\""), newRoom, true, 0);
					return true;
				}
			}
			...
			beneficialWordsFizzle(mob, null,
					"<S-NAME> attempt(s) to call <S-HIS-HER> undead, but fail(s).");
