...
public class Prayer_ProtDisease extends Prayer {
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectedStats) {
		...
		affectedStats.setStat(CharStats.STAT_SAVE_DISEASE,
				affectedStats.getStat(CharStats.STAT_SAVE_DISEASE) + 100);
	}
			...
			if ((msg.tool() != null) && (CMLib.dice().rollPercentage() > 50)
				...
				msg.source()
						...
						.show((MOB) affected, null, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> magically repell(s) the disease.");
				return false;
			}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE> protection from disease.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> attain(s) disease protection."
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob)
					+ " for protection from diseases, but go(es) unanswered.");
