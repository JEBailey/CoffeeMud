...
public class Prayer_GuardianHearth extends Prayer {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (((msg.tool() instanceof Trap)
				|| (prots.contains(Integer.valueOf(msg.sourceMinor()))) || (prots
					.contains(Integer.valueOf(msg.targetMinor()))))
				&& (msg.target() instanceof MOB)
				&& ((msg.source() != msg.target()) || (msg
						.sourceMajor(CMMsg.MASK_ALWAYS)))) {
			...
			for (Iterator e = H.iterator(); e.hasNext();) {
				...
				if ((CMLib.law().doesHavePriviledgesHere(M, R))
					...
					R.show(((MOB) msg.target()), null, this,
							CMMsg.MSG_OK_VISUAL,
							"The guardian hearth protect(s) <S-NAME>!");
					break;
				}
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			beneficialWordsFizzle(mob, target, "<S-NAME> " + prayForWord(mob)
					+ " to guard this place, but <S-IS-ARE> not answered.");
