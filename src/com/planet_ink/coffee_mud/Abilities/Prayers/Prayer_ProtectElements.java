...
public class Prayer_ProtectElements extends Prayer {
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_SAVE_ACID,
				affectableStats.getStat(CharStats.STAT_SAVE_ACID) + 50);
		affectableStats.setStat(CharStats.STAT_SAVE_COLD,
				affectableStats.getStat(CharStats.STAT_SAVE_COLD) + 50);
		affectableStats.setStat(CharStats.STAT_SAVE_ELECTRIC,
				affectableStats.getStat(CharStats.STAT_SAVE_ELECTRIC) + 50);
		...
		affectableStats.setStat(CharStats.STAT_SAVE_GAS,
				affectableStats.getStat(CharStats.STAT_SAVE_GAS) + 50);
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE> protection from elements.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> attain(s) elemental protection."
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob)
					+ " for elemental protection, but nothing happens.");
