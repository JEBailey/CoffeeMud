...
public class Prayer_DivineConstitution extends Prayer {
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_CONSTITUTION,
				affectableStats.getStat(CharStats.STAT_CONSTITUTION) + conPts);
		affectableStats.setStat(CharStats.STAT_MAX_CONSTITUTION_ADJ,
				affectableStats.getStat(CharStats.STAT_MAX_CONSTITUTION_ADJ)
						+ conPts);
	}
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		...
		if ((msg.target() == affected)
				...
				&& (msg.source() == invoker())
				&& (conPts < maxPoints)
				&& (CMLib.dice().rollPercentage() < (50 + (5 * getX1Level(msg
						.source()))))) {
			MOB M = (MOB) affected;
			if (M != null) {
				Room R = M.location();
				int diff = (M.curState().getHitPoints() - M.maxState()
						.getHitPoints()) + msg.value();
				if ((diff > 0) && (msg.value() > diff)) {
					R.show(M, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> gain(s) divine health!");
					conPts++;
					xtraHPs += 1 + diff;
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> divine constitution fades.");
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "<T-NAME> become(s) covered by divine constitution."
									: "^S<S-NAME> "
											+ prayWord(mob)
											+ " for <T-NAMESELF> to be covered by divine constitution.^?"));
			...
			return beneficialWordsFizzle(
					...
					"<S-NAME> "
							+ prayWord(mob)
							+ " for <T-NAMESELF> to have divine constitution, but nothing happens.");
