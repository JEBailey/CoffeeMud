...
public class Prayer_Sermon extends Prayer {
	...
	public long flags() {
		return Ability.FLAG_CHARMING | Ability.FLAG_HOLY | Ability.FLAG_UNHOLY;
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob))
			...
			mob.tell("You admire " + mob.amFollowing().charStats().himher()
					+ " too much.");
			return false;
		} else if ((msg.amISource(mob)) && (!mob.isMonster())
			...
			mob.tell("You believe in " + mob.amFollowing().name()
					+ " too much.");
			return false;
		}
		...
		if (success) {
			if (mob.location()
					.show(mob,
							...
							auto ? ""
									: "^S<S-NAME> begin(s) sermonizing on the wonders of "
											+ hisHerDiety(mob) + ".^?"))
				for (Enumeration f = h.elements(); f.hasMoreElements();) {
					...
					if ((CMLib.flags().canBeHeardSpeakingBy(mob, target))
						...
						if ((mob.location().okMessage(mob, msg))
							...
							if ((success) && (msg.value() <= 0)) {
								...
								target.location().show(
										...
										null,
										CMMsg.MSG_OK_ACTION,
										"<S-NAME> begin(s) nodding and shouting praises to "
												+ hisHerDiety(mob) + ".");
						...
						beneficialWordsFizzle(mob, target,
								"<T-NAME> seem(s) unmoved by the <S-YOUPOSS> sermon.");
				}
		} else
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> forget(s) how <S-YOUPOSS> sermon to "
							+ hisHerDiety(mob) + " goes.");
