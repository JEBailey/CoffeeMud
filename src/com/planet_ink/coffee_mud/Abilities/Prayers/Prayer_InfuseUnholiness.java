...
public class Prayer_InfuseUnholiness extends Prayer {
	...
	public String name() {
		return "Infuse Unholiness";
	}

	public String displayText() {
		return "(Infused Unholiness)";
	}
		...
		if (target instanceof Room) {
			...
			for (Enumeration e = A.getMetroMap(); e.hasMoreElements();) {
				...
				if (CMLib.law().getClericInfused((Room) target) == D) {
					mob.tell("There is already an unholy place of " + D.Name()
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "An unholy aura appears around <T-NAME>."
									: "^S<S-NAME> "
											+ prayForWord(mob)
											+ " to infuse an unholy aura around <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(mob, target, "<S-NAME> " + prayForWord(mob)
					+ " to infuse an unholy aura in <T-NAMESELF>, but fail(s).");
