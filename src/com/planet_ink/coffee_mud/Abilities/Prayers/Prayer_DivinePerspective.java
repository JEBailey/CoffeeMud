...
public class Prayer_DivinePerspective extends Prayer {
	...
	public void unInvoke() {
		...
		if (canBeUninvoked())
			if (invoker != null)
				invoker.tell("The perspective of '" + mob.name(invoker)
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> invoke(s) the holy perspective of '"
									+ mobName + "'.^?");
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to invoke the holy perspective of "
							+ target.Name() + ", but fail(s).");
