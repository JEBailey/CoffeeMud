...
public class Prayer_InfuseBalance extends Prayer {
	...
	public String name() {
		return "Infuse Balance";
	}

	public String displayText() {
		return "(Infused Balance)";
	}
		...
		if (target instanceof Room) {
			...
			for (Enumeration e = A.getMetroMap(); e.hasMoreElements();) {
				...
				if (CMLib.law().getClericInfused((Room) target) == D) {
					mob.tell("There is already a balanced place of " + D.Name()
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "A holy balanced aura appears around <T-NAME>."
									: "^S<S-NAME> "
											+ prayForWord(mob)
											+ " to infuse a holy balanced aura around <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(
					...
					"<S-NAME> "
							+ prayForWord(mob)
							+ " to infuse a holy balanced aura in <T-NAMESELF>, but fail(s).");
