...
public class Prayer_Curse extends Prayer {
		...
		if (!(affected instanceof MOB)) {
			if (canBeUninvoked())
				if ((affected instanceof Item)
					...
					((MOB) ((Item) affected).owner()).tell("The curse on "
							+ ((Item) affected).name() + " is lifted.");
			super.unInvoke();
			return;
		}
	...
	public static boolean isBlessed(Item item) {
		return CMLib.flags().domainAffects(item, Ability.DOMAIN_BLESSING)
				.size() > 0;
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> <T-IS-ARE> cursed!"
							: "^S<S-NAME> curse(s) <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					Item I = getSomething(mob, true);
					if (I != null) {
						endLowerBlessings(I, CMLib.ableMapper()
								.lowestQualifyingLevel(ID()));
						I.recoverPhyStats();
					}
					endLowerBlessings(target, CMLib.ableMapper()
							.lowestQualifyingLevel(ID()));
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to curse <T-NAMESELF>, but nothing happens.");
