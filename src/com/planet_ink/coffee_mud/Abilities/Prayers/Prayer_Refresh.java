	...
	public boolean supportsMending(Physical item) {
		return (item instanceof MOB)
				&& (((((MOB) item).curState()).getFatigue() > 0)
						|| ((((MOB) item).curState()).getMovement() < (((MOB) item)
								.maxState()).getMovement())
						|| ((((MOB) item).curState()).getMana() < (((MOB) item)
								.maxState()).getMana()) || ((((MOB) item)
						.curState()).getHitPoints() < (((MOB) item).maxState())
						.getHitPoints()));
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "A bright yellow glow surrounds <T-NAME>."
									: "^S<S-NAME> "
											+ prayWord(mob)
											+ ", delivering a strong touch of infusion to <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				target.curState().setMana(target.maxState().getMana());
				target.curState()
						.setHitPoints(target.maxState().getHitPoints());
				target.curState().setMana(target.maxState().getMana());
