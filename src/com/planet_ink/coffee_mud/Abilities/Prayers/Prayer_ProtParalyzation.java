...
public class Prayer_ProtParalyzation extends Prayer {
	...
	public String name() {
		return "Protection Paralyzation";
	}
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectedStats) {
		...
		affectedStats.setStat(CharStats.STAT_SAVE_PARALYSIS,
				affectedStats.getStat(CharStats.STAT_SAVE_PARALYSIS) + 100);
	}
			...
			if ((msg.tool() != null) && (CMLib.dice().rollPercentage() > 50)
				...
				msg.source()
						...
						.show((MOB) affected, null, CMMsg.MSG_OK_VISUAL,
								"An paralyzing assault against <S-NAME> is magically repelled.");
				return false;
			}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE> protection from paralyzation.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> attain(s) a free mind and body."
							: "^S<S-NAME> " + prayWord(mob)
									+ " for a free mind and body.^?");
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob)
					+ " for a free body and mind, but nothing happens.");
