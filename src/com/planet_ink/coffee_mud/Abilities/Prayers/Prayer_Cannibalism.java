...
public class Prayer_Cannibalism extends Prayer {
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		...
		if ((msg.source() == affected) && (msg.targetMinor() == CMMsg.TYP_EAT)
				&& (msg.target() instanceof Food)
				&& (msg.target() instanceof Item)) {
			if (((((Item) msg.target()).material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_FLESH)
					&& (CMLib.english().containsString(msg.target().Name(), msg
							.source().charStats().getMyRace().name()))) {
				msg.source()
						.curState()
						.adjHunger(
								+((Food) msg.target()).nourishment(),
								msg.source().maxState()
										.maxHunger(msg.source().baseWeight()));
				msg.source()
						.curState()
						.adjThirst(
								+((Food) msg.target()).nourishment() * 2,
								msg.source().maxState()
										.maxThirst(msg.source().baseWeight()));
			} else
				msg.source()
						.curState()
						.adjHunger(
			...
			msg.source()
					.curState()
					.adjThirst(
							-((Drink) msg.target()).thirstQuenched(),
							msg.source().maxState()
									.maxThirst(msg.source().baseWeight()));
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((M.location() != null) && (!CMLib.flags().isSleeping(M))) {
			M.curState().adjThirst(-(M.location().thirstPerRound(M) * 2),
					M.maxState().maxThirst(M.baseWeight()));
			M.curState().adjHunger(-2, M.maxState().maxHunger(M.baseWeight()));
			if ((M.isMonster())
					&& ((M.curState().getThirst() <= 0) || (M.curState()
							.getHunger() <= 0))
				...
				for (int i = 0; i < M.location().numItems(); i++) {
					...
					if ((I != null)
							...
							&& (((DeadBody) I).charStats().getMyRace() == M
									.charStats().getMyRace()))
						B = (DeadBody) I;
					else if ((I != null)
							&& (I instanceof Food)
							...
							&& (CMLib.english().containsString(I.Name(), M
									.charStats().getMyRace().name())))
						F = (Food) I;
				}
				if (F != null) {
					...
					if ((M2 != null)
							&& (M2 != M)
							&& (M.charStats().getMyRace() == M2.charStats()
									.getMyRace()))
						M.setVictim(M2);
				}
			}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		boolean success = proficiencyCheck(mob,
				-((target.charStats().getStat(CharStats.STAT_WISDOM) * 2)),
				auto);
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> invoke(s) a cannibalistic hunger upon <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					mob.location()
							.show(target, null, CMMsg.MSG_OK_VISUAL,
									"<S-NAME> <S-IS-ARE> inflicted with cannibalistic urges!");
			...
			return maliciousFizzle(
					mob,
					target,
					"<S-NAME> attempt(s) to inflict cannibalistic urges upon <T-NAMESELF>, but flub(s) it.");
