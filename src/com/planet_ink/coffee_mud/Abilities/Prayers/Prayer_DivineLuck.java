...
public class Prayer_DivineLuck extends Prayer {
		...
		for (int i : CharStats.CODES.SAVING_THROWS())
			affectableStats.setStat(
					...
					affectableStats.getStat(i)
							+ 1
							+ ((affectedMOB.phyStats().level() + (2 * super
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		affectableStats.setAttackAdjustment(affectableStats.attackAdjustment()
				+ ((affected.phyStats().level() + (2 * xlvl)) / 5) + 1);
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> become(s) divinely lucky." : "^S<S-NAME> "
							+ prayWord(mob) + " for divine luck.^?");
			...
			return beneficialWordsFizzle(mob, null, "<S-NAME> " + prayWord(mob)
					+ ", but as luck would have it, there's no answer.");
