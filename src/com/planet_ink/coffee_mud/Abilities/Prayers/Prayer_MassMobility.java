...
public class Prayer_MassMobility extends Prayer {
			...
			try {
				...
				if ((!CMLib.flags().aliveAwakeMobileUnbound(newMOB, true))
					...
					mob.location().show(
							...
							null,
							CMMsg.MSG_OK_VISUAL,
							"The aura around <S-NAME> repels the " + A.name()
									+ " from <T-NAME>.");
					newMOB.destroy();
					return false;
				}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((success) && (room != null)) {
			CMMsg msg = CMClass.getMsg(mob, null, this, affectType, auto ? ""
					: "^S<S-NAME> " + prayWord(mob)
							+ " for an aura of mobility!^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				for (int i = 0; i < room.numInhabitants(); i++) {
					...
					// what happened.
					msg = CMClass.getMsg(mob, target, this, affectType,
							"Mobility is invoked upon <T-NAME>.");
					if (mob.location().okMessage(mob, msg)) {
						mob.location().send(mob, msg);
