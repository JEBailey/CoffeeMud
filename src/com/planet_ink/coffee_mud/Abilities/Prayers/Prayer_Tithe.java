...
public class Prayer_Tithe extends Prayer {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((msg.sourceMinor() == CMMsg.TYP_GET) && (msg.source() == affected)
			...
			if ((invoker() != msg.source()) && ((num / 10) > 0)) {
				...
				CMLib.beanCounter().addMoney(
						invoker(),
						currency,
						CMath.mul(((Coins) msg.target()).getDenomination(),
								(num / 10)));
			}
		}
		if ((msg.sourceMinor() == CMMsg.TYP_BUY) && (msg.amITarget(affected))
				&& (msg.tool() != null)) {
			ShopKeeper SK = CMLib.coffeeShops().getShopKeeper(affected);
			if (SK.getShop().doIHaveThisInStock("$" + msg.tool().Name() + "$",
					msg.source())) {
				ShopKeeper.ShopPrice price = CMLib.coffeeShops().sellingPrice(
						(MOB) affected, msg.source(), msg.tool(), SK, true);
				if ((price.absoluteGoldPrice > 0.0)
						&& (price.absoluteGoldPrice <= CMLib.beanCounter()
								.getTotalAbsoluteShopKeepersValue(msg.source(),
										invoker())))
					if (invoker() != msg.target()) {
						invoker().tell(msg.source(), null, null,
								"<S-NAME> tithes.");
						CMLib.beanCounter().addMoney(invoker(),
								CMath.div(price.absoluteGoldPrice, 10.0));
					}
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> become(s) filled with a need to tithe!"
							: "^S<S-NAME> " + prayWord(mob)
									+ " for <T-YOUPOSS> need to tithe!^?");
			...
			return maliciousFizzle(mob, null, "<S-NAME> " + prayWord(mob)
					+ " for <T-YOUPOSS> tithing need but there is no answer.");
