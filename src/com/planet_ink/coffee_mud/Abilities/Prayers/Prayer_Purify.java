...
public class Prayer_Purify extends Prayer {
	...
	public int classificationCode() {
		return ((affecting() instanceof Food) && (!canBeUninvoked())) ? Ability.ACODE_PROPERTY
				: Ability.ACODE_PRAYER | Ability.DOMAIN_RESTORATION;
	}

	public void affectPhyStats(Physical affecting, PhyStats stats) {
		if ((affecting instanceof Decayable)
				&& (((Decayable) affecting).decayTime() > 0))
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> purify <T-NAMESELF>"
									+ inTheNameOf(mob) + ".^?", auto ? ""
							: "^S<S-NAME> purifies <T-NAMESELF>"
									+ inTheNameOf(mob) + ".^?", auto ? ""
							: "^S<S-NAME> purifies <T-NAMESELF>"
									+ inTheNameOf(mob) + ".^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				if (doneSomething)
					mob.location().showHappens(CMMsg.MSG_OK_VISUAL,
							target.name() + " appears purified!");
				target.recoverPhyStats();
				mob.location().recoverRoomStats();
			}
		} else
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob) + " for purification, but nothing happens.");
