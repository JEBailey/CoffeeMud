...
public class Prayer_RemoveParalysis extends Prayer implements MendingSkill {
	...
	public boolean supportsMending(Physical item) {
		return (item instanceof MOB)
				&& (CMLib
						.flags()
						.flaggedAffects(item,
								Ability.FLAG_PARALYZING | Ability.FLAG_UNHOLY)
						.size() > 0);
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		List<Ability> offensiveAffects = CMLib.flags().flaggedAffects(target,
				Ability.FLAG_PARALYZING | Ability.FLAG_UNHOLY);
		...
		if ((success) && (offensiveAffects.size() > 0)) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "A visible glow surrounds <T-NAME>." : "^S<S-NAME> "
							+ prayWord(mob)
							+ " for <T-NAMESELF> to be able to move.^?");
