	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amISource(myChar)
				...
				&& (CMLib.ableMapper().getQualifyingLevel(ID(), true,
						msg.tool().ID()) > 0) && (myChar.isMine(msg.tool()))
				&& (msg.value() > 0))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already surrounded by a brightening aura.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<S-NAME> attain(s) a brightening aura."
									: "^S<S-NAME> invoke(s) a brightening aura upon <T-NAMESELF>.^?");
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob)
					+ " for a brightening aura, but nothing happens.");
