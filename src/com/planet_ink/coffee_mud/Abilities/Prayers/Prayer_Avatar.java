...
public class Prayer_Avatar extends Prayer {
	...
	public void affectPhyStats(Physical affected, PhyStats affectedStats) {
		...
		int xlvl = 2 + (int) Math.round(CMath.div(adjustedLevel(invoker(), 0),
				1.5));
		affectedStats.setArmor(affectedStats.armor() - (xlvl));
		affectedStats.setSpeed(affectedStats.speed() + 1.0
				+ CMath.mul(0.33, super.getXLEVELLevel(invoker())));
		affectedStats.setAttackAdjustment(affectedStats.attackAdjustment()
				+ (xlvl * 2));
		if (affected instanceof MOB) {
			MOB mob = (MOB) affected;
			if (mob.getMyDeity() != null)
				affectedStats.setName(mob.name() + ", the Avatar of "
						+ mob.getMyDeity().name());
			else
		...
		if (mob.location() != null) {
			if (mob.isInCombat()) {
				...
				if (attack == null) {
					...
					if (dir >= 0) {
						String godName = mob.getWorshipCharID().length() == 0 ? "Your god"
								: mob.getWorshipCharID();
						mob.tell(godName + " directs you "
								+ Directions.getInDirectionName(dir) + ".");
						CMLib.tracking().walk(mob, dir, false, false);
					}
				}
				if (attack != null)
					CMLib.combat().postAttack(mob, attack,
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already the AVATAR.");
			return false;
		}
