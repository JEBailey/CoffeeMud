...
public class Prayer extends StdAbility {
	...
	public boolean invoke(MOB mob, Vector commands, Physical target,
		...
		if ((!auto) && (!mob.isMonster()) && (!disregardsArmorCheck(mob))
				&& (mob.isMine(this)) && (!appropriateToMyFactions(mob))) {
			int hq = 500;
			if (CMath.bset(flags(), Ability.FLAG_HOLY)) {
				if (!CMath.bset(flags(), Ability.FLAG_UNHOLY))
					hq = 1000;
			} else if (CMath.bset(flags(), Ability.FLAG_UNHOLY))
				hq = 0;
