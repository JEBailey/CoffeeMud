...
public class Prayer_Blindsight extends Prayer {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((affected instanceof MOB)
				&& (!CMLib.flags().canSee((MOB) affected)))
			affectableStats.setSensesMask(affectableStats.sensesMask()
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE> the blindsight.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> attain(s) blindsight." : "^S<S-NAME> "
							+ prayWord(mob) + " for the blindsight.^?");
			...
			return beneficialWordsFizzle(mob, target, "<S-NAME> "
					+ prayWord(mob) + " for blindsight, but go(es) unanswered.");
