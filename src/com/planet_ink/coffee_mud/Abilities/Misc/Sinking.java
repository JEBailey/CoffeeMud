...
public class Sinking extends StdAbility {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected != null) && (affected instanceof MOB)
			...
			if ((msg.sourceMinor() == CMMsg.TYP_ENTER)
					...
					&& ((((Room) msg.target()).domainType() == Room.DOMAIN_INDOORS_AIR) || (((Room) msg
							.target()).domainType() == Room.DOMAIN_OUTDOORS_AIR))
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (affected instanceof MOB) {
			...
			if (!isWaterSurface(R) || (CMLib.flags().isWaterWorthy(mob))
			...
			if (((direction == Directions.DOWN) && (A != null))
					&& (A.proficiencyCheck(mob, 25, (A.proficiency() >= 75)) && (mob
							.curState().getMovement() > 0))) {
				if ((R.show(mob, null, CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> tread(s) water.")) && (!mob.isMonster())) {
					isTreading = true;
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((!CMLib.flags().isWaterWorthy(affected))
				&& (!CMLib.flags().isInFlight(affected)) && (!isTreading)
