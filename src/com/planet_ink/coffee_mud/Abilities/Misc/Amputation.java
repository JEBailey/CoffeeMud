...
public class Amputation extends StdAbility implements Amputator,
	...
	public String displayText() {
		if (missingLimbNameSet().size() == 0)
			return "";
		return "(Missing your"
				+ CMLib.english().toEnglishStringList(missingLimbNameSet())
				+ ")";
	}
	...
	private static final long[] LEFT_LOCS = { Wearable.WORN_LEFT_FINGER,
			Wearable.WORN_LEFT_WRIST };
	private static final long[] RIGHT_LOCS = { Wearable.WORN_RIGHT_FINGER,
	...
	public final static int[][] extraamuputees = {
			{ -1 },// antenea
			{ -1 },// eye
			{ -1 },// ear
			{ Race.BODY_EAR, Race.BODY_EYE, Race.BODY_MOUTH, Race.BODY_NOSE,
					Race.BODY_ANTENEA, Race.BODY_GILL },// head
			{ Race.BODY_EAR, Race.BODY_EYE, Race.BODY_MOUTH, Race.BODY_NOSE,
					Race.BODY_ANTENEA, Race.BODY_GILL, Race.BODY_HEAD },// nect
			{ Race.BODY_HAND },// arm
			{ -1 },// hand
			{ -1 },// torso
			{ Race.BODY_FOOT },// leg
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		if (affected instanceof MOB) {
			MOB M = (MOB) affected;
			if ((msg.target() == M)
					&& ((msg.targetMinor() == CMMsg.TYP_LOOK) || (msg
							.targetMinor() == CMMsg.TYP_EXAMINE))
					&& (CMLib.flags().canBeSeenBy(M, msg.source()))) {
				String s = CMLib.utensils().niceCommaList(missingLimbNameSet(),
						true);
				if (s.length() > 0)
					msg.addTrailerMsg(CMClass.getMsg(msg.source(), null, null,
							CMMsg.MSG_OK_VISUAL, "\n\r" + M.name(msg.source())
									+ " is missing " + M.charStats().hisher()
									+ " " + s + ".\n\r", CMMsg.NO_EFFECT, null,
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if (affected instanceof MOB) {
			...
			if ((amputations[Race.BODY_LEG] < 0)
					&& (mob.getWearPositions(Wearable.WORN_LEGS) == 0)) {
				if ((amputations[Race.BODY_ARM] < 0)
						&& (mob.getWearPositions(Wearable.WORN_ARMS) == 0))
					affectableStats.setSensesMask(affectableStats.sensesMask()
							| PhyStats.CAN_NOT_MOVE);
				affectableStats.setDisposition(affectableStats.disposition()
						| PhyStats.IS_SITTING);
			}
			if ((amputations[Race.BODY_EYE] < 0)
					&& (mob.getWearPositions(Wearable.WORN_EYES) == 0))
				affectableStats.setSensesMask(affectableStats.sensesMask()
						| PhyStats.CAN_NOT_SEE);
			if ((amputations[Race.BODY_EAR] < 0)
					&& (mob.getWearPositions(Wearable.WORN_EARS) == 0))
				affectableStats.setSensesMask(affectableStats.sensesMask()
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> limbs have been restored.");
	}
	...
	public MOB getTarget(MOB mob, Vector commands, Environmental givenTarget,
		...
		else if ((targetName.length() == 0) && (mob.isInCombat())
	...
	public List<String> missingLimbNameSet() {
		...
		for (int v = 0; v < missingLimbs.size(); v++) {
			...
			code = Race.BODYPARTHASH.get(s.substring(right ? 6 : left ? 5 : 0)
					.trim());
			if (code != null) {
		...
		for (int i = 0; i < limbs.length; i++) {
			...
			if ((limbs[i] > 0) && (validamputees[i])) {
				if (limbs[i] - amputations[i] == 1) {
					if (!missingLimbNameSet().contains(
							Race.BODYPARTSTR[i].toLowerCase()))
						V.addElement(Race.BODYPARTSTR[i].toLowerCase());
				} else if (limbs[i] - amputations[i] == 2) {
					if (!missingLimbNameSet().contains(
							"left " + Race.BODYPARTSTR[i].toLowerCase()))
						V.addElement("left "
								+ Race.BODYPARTSTR[i].toLowerCase());
					if (!missingLimbNameSet().contains(
							"right " + Race.BODYPARTSTR[i].toLowerCase()))
						V.addElement("right "
								+ Race.BODYPARTSTR[i].toLowerCase());
				} else
					for (int ii = 0; ii < limbs[i]; ii++)
		...
		if (target != null) {
			if (target instanceof MOB) {
				((MOB) target).location().show(((MOB) target), null,
						CMMsg.MSG_OK_VISUAL,
						"^G<S-YOUPOSS> " + gone + " miraculously regrows!!^?");
			} else if ((target instanceof DeadBody)
					&& (((Item) target).owner() != null)
					&& (((Item) target).owner() instanceof Room)) {
				((Room) ((Item) target).owner()).showHappens(
						CMMsg.MSG_OK_VISUAL, "^G" + target.name() + "'s "
								+ gone + " miraculously regrows!!^?");
			}
		}
	...
	public List<String> affectedLimbNameSet(Object O, String missing,
			List<String> missingLimbs) {
	...
	public Item amputate(Physical target, Amputator A, String gone) {
		...
		if (target != null) {
			if (target instanceof MOB) {
				...
				if (gone.toLowerCase().endsWith("eye"))
					success = ((MOB) target).location().show(((MOB) target), A,
							CMMsg.MSG_OK_VISUAL,
							"^G<S-YOUPOSS> " + gone + " is destroyed!^?");
				else
					success = ((MOB) target).location().show(((MOB) target), A,
							CMMsg.MSG_OK_VISUAL,
							"^G<S-YOUPOSS> " + gone + " falls off!^?");
				if (!success)
					return null;
			} else if ((target instanceof DeadBody)
					&& (((Item) target).owner() != null)
		...
		if (!isFakeLimb) {
			...
			if ((R != null) && (R.myResources() != null)
					&& (R.myResources().size() > 0))
		...
		if ((target instanceof MOB) && (((MOB) target).location() != null))
			((MOB) target).location().addItem(limb,
					ItemPossessor.Expire.Monster_EQ);
		else if ((target instanceof DeadBody)
				&& (((Item) target).owner() != null)
				&& (((Item) target).owner() instanceof Room))
			((Room) ((Item) target).owner()).addItem(limb,
					ItemPossessor.Expire.Monster_EQ);

		if (!isFakeLimb) {
			List<String> theRest = A.affectedLimbNameSet(target, gone,
					A.missingLimbNameSet());
			if (!theRest.contains(gone))
				theRest.add(gone);
			for (int i = 0; i < theRest.size(); i++)
				A.setMiscText(A.text() + (theRest.get(i)) + ";");
		...
		Injury I = (target == null) ? null : (Injury) target
				.fetchEffect("Injury");
		if (I != null) {
			Vector V = null;
			for (int i = 0; i < I.injuries.length; i++) {
				...
				if (V != null)
					for (int v = 0; v < V.size(); v++)
						if (((String) ((Object[]) V.elementAt(v))[0])
								.equalsIgnoreCase(gone)) {
							V.removeElementAt(v);
							if (V.size() == 0)
								I.injuries[i] = null;
							break;
		...
		if ((target instanceof MOB)
				&& (CMLib.dice().roll(1, 100, 0) <= CMProps
						.getIntVar(CMProps.Int.INJBLEEDPCTCHANCE))) {
		...
		if (named.length() > 0) {
			...
			for (int i = 0; i < tmob.numItems(); i++) {
				...
				if ((I != null)
						...
						&& ((I.name().toUpperCase().endsWith(named)) || (I
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (givenTarget != null) {
			if ((commands.size() > 0)
					&& (((String) commands.firstElement()).equals(givenTarget
							.name())))
		...
		if (!auto) {
			...
			if ((warrants.size() == 0)
				...
				mob.tell("You are not authorized by law to amputate from "
						+ target.Name() + " at this time.");
				return false;
			}
			...
			if (!CMSecurity.isASysOp(mob)) {
				...
				if ((!CMLib.flags().isBoundOrHeld(target))
					...
					mob.tell(target.charStats().HeShe()
							+ " must be bound, and asleep on an operating bed before you can amputate.");
					return false;
				}
			}
		}
		...
		if (success) {
			...
			String str = auto ? ""
					: "^F^<FIGHT^><S-NAME> amputate(s) <T-YOUPOSS> " + goneName
							+ "!^</FIGHT^>^?";
			CMMsg msg = CMClass.getMsg(mob, target, this,
					CMMsg.MSK_MALICIOUS_MOVE | CMMsg.TYP_DELICATE_HANDS_ACT
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to amputate <T-NAMESELF>, but fail(s).");
		return success;
	}
}
