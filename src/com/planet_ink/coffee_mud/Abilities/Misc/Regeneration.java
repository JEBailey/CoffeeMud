...
public class Regeneration extends StdAbility implements HealthCondition {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		doneAnything = doneAnything
				|| mob.curState()
						.adjHitPoints(
								(int) Math.round(CMath.div(mob.phyStats()
										.level(), 2.0)), mob.maxState());
		doneAnything = doneAnything
				|| mob.curState().adjMana(mob.phyStats().level() * 2,
						mob.maxState());
		doneAnything = doneAnything
				|| mob.curState().adjMovement(mob.phyStats().level() * 3,
						mob.maxState());
		if (doneAnything)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			String str = auto ? ""
					: "<S-NAME> lay(s) regenerative magic upon <T-NAMESELF>.";
			CMMsg msg = CMClass.getMsg(mob, target, null,
					CMMsg.MSG_QUIETMOVEMENT, str);
