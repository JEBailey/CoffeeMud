...
public class TemporaryAffects extends StdAbility {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		for (Object[] A : affects)
			if (A[0] instanceof StatsAffecting)
				((StatsAffecting) A[0]).affectPhyStats(affected,
						affectableStats);
	}
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		for (Object[] A : affects)
			if (A[0] instanceof StatsAffecting)
				((StatsAffecting) A[0]).affectCharStats(affected,
						affectableStats);
	}
	...
	public void affectCharState(MOB affected, CharState affectableStats) {
		...
		for (Object[] A : affects)
			if (A[0] instanceof StatsAffecting)
				((StatsAffecting) A[0]).affectCharState(affected,
						affectableStats);
	}
	...
	public void setMiscText(String txt) {
		...
		if (txt.startsWith("-")) {
			...
			for (Object[] A : affects)
				if ((A[0] instanceof Ability)
						&& (((Environmental) A[0]).name().toLowerCase()
								.startsWith(txt))) {
					unAffectAffected(A);
					return;
				}
			...
			if (A != null) {
				affects.addElement(new Object[] { A,
						new int[] { CMath.s_int(numTicksStr) } });
				if (A instanceof Ability)
					((Ability) A).setMiscText(parms);
				if ((A instanceof Behavior)
						&& (affected instanceof PhysicalAgent))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (T != null) {
			T.setMiscText("+" + A.ID() + " " + numTicks.trim() + " "
					+ parms.trim());
			T.makeLongLasting();
		}
		return true;
