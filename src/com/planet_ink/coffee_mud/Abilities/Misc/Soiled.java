...
public class Soiled extends StdAbility {
	...
	public void unInvoke() {
		...
		if (canBeUninvoked()) {
			if (E instanceof MOB) {
				...
				((MOB) ((Item) E).owner()).tell(E.name()
						+ " is no longer soiled.");
		}
	}

	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if (((msg.source() == affected) || ((affected instanceof Item) && (((Item) affected)
				.owner() == msg.source())))) {
			if ((msg.sourceMajor(CMMsg.MASK_MOVE))
					...
					&& ((msg.source().location().domainType() == Room.DOMAIN_INDOORS_WATERSURFACE)
							...
							|| (msg.source().location().domainType() == Room.DOMAIN_INDOORS_UNDERWATER) || (msg
							.source().location().domainType() == Room.DOMAIN_OUTDOORS_UNDERWATER)))
				unInvoke();
		...
		if ((msg.target() == affected)
			...
			case 1:
				smell = "<T-NAME> is stinky!";
				break;
			case 2:
				smell = "<T-NAME> smells like poo.";
				break;
			case 3:
				smell = "<T-NAME> has soiled a diaper.";
				break;
			case 4:
				smell = "Whew! <T-NAME> stinks!";
				break;
			case 5:
				smell = "<T-NAME> must have let one go!";
				break;
			}
				...
				if (R != null) {
					...
					if (M == null) {
						...
						killmob = true;
							...
							&& (M.playerStats().getHygiene() < 10000)) {
					...
					switch (CMLib.dice().roll(1, 5, 0)) {
					case 1:
						smell = "<S-NAME> <S-IS-ARE> stinky!";
						break;
					case 2:
						smell = "<S-NAME> smells like poo.";
						break;
					case 3:
						smell = "<S-NAME> has soiled a diaper.";
						break;
					case 4:
						smell = "Whew! <S-NAME> stinks!";
						break;
					case 5:
						smell = "<S-NAME> must have let one go!";
						break;
					}
					if ((smell != null) && (CMLib.flags().isInTheGame(M, true))) {
						CMMsg msg = CMClass.getMsg(M, null, null,
								CMMsg.TYP_EMOTE | CMMsg.MASK_ALWAYS, smell);
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		mob.location().show(mob, msgTarget, CMMsg.MSG_OK_VISUAL,
				"<T-NAME> has soiled <T-HIM-HERSELF>!");
		if (target instanceof MOB) {
			Item pants = ((MOB) target).fetchFirstWornItem(Wearable.WORN_WAIST);
			if ((pants != null) && (pants.fetchEffect(ID()) == null)) {
				A = (Ability) copyOf();
				A.startTickDown((MOB) target, pants,
