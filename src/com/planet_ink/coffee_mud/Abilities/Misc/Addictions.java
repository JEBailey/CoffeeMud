...
public class Addictions extends StdAbility {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((craving())
				&& (CMLib.dice().rollPercentage() <= ((System
						.currentTimeMillis() - lastFix) / TimeManager.MILI_HOUR))
			...
			if ((puffCredit != null)
					&& (puffCredit.amDestroyed()
							|| puffCredit.amWearingAt(Wearable.IN_INVENTORY) || puffCredit
							.owner() != (MOB) affected))
				puffCredit = null;
			switch (CMLib.dice().roll(1, 7, 0)) {
			case 1:
				((MOB) ticking).tell("Man, you could sure use some " + text()
						+ ".");
				break;
			case 2:
				((MOB) ticking).tell("Wouldn't some " + text()
						+ " be great right about now?");
				break;
			case 3:
				((MOB) ticking).tell("You are seriously craving " + text()
						+ ".");
				break;
			case 4:
				((MOB) ticking).tell("There's got to be some " + text()
						+ " around here somewhere.");
				break;
			case 5:
				((MOB) ticking).tell("You REALLY want some " + text() + ".");
				break;
			case 6:
				((MOB) ticking).tell("You NEED some " + text() + ", NOW!");
				break;
			case 7:
				((MOB) ticking).tell("Some " + text() + " would be lovely.");
				break;
			}
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		if ((affected != null) && (affected instanceof MOB)) {
			if ((msg.source() == affected)
					...
					&& (CMath.bset(
							((Item) msg.target()).rawProperLocationBitmap(),
							Wearable.WORN_MOUTH))
					&& (((Container) msg.target()).getContents().size() > 0)
					&& (CMLib.english().containsString(
							((Environmental) ((Container) msg.target())
									.getContents().get(0)).Name(), text())))
		...
		if ((affected != null) && (affected instanceof MOB)) {
			if (msg.source() == affected) {
				if (((msg.targetMinor() == CMMsg.TYP_EAT) || (msg.targetMinor() == CMMsg.TYP_DRINK))
						...
						&& (CMLib.english().containsString(msg.target().Name(),
								text())))
				...
				if ((msg.amISource((MOB) affected))
						...
						&& (msg.target() == msg.tool())
						&& (((Light) msg.target())
								.amWearingAt(Wearable.WORN_MOUTH))
						&& (((Light) msg.target()).isLit())
						&& ((puffCredit != null) || CMLib.english()
								.containsString(msg.target().Name(), text())))
					lastFix = System.currentTimeMillis();
			}
		}
