...
public class Dragonbreath extends StdAbility {
	...
	private final static String[][] DragonColors = { { "WHITE", "c" },
			{ "BLACK", "a" }, { "BLUE", "l" }, { "GREEN", "g" },
			{ "RED", "f" }, { "BRASS", "f" }, { "COPPER", "a" },
			{ "BRONZE", "l" }, { "SILVER", "c" }, { "GOLD", "g" }, };
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((text().length() == 0)
			...
			for (int i = 0; i < DragonColors.length; i++)
				if (CMLib.english().containsString(mob.Name(),
						DragonColors[i][0])) {
					color = i;
					break;
				}
			if (color < 0)
				for (int i = 0; i < DragonColors.length; i++)
					if (CMLib.english().containsString(mob.displayText(),
							DragonColors[i][0])) {
						color = i;
						break;
					}
		...
		case 'c':
			...
			stuffWord = "cold";
			castPhrase = "<S-NAME> blast(s) a frozen cone of frost from <S-HIS-HER> mouth!"
		...
		case 'a':
			...
			castPhrase = "<S-NAME> spray(s) acid from <S-HIS-HER> mouth!"
					+ CMLib.protocol().msp("water.wav", 40);
		...
		if ((success) && (R != null)) {
			...
			if (text().length() == 0)
				setMiscText("");
			if (R.show(mob, null, this, CMMsg.MSG_NOISYMOVEMENT,
					auto ? autoPhrase : castPhrase))
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					CMMsg msg = CMClass.getMsg(mob, target, this,
							CMMsg.MSK_MALICIOUS_MOVE | strikeType
