...
public class ItemRejuv extends StdAbility implements ItemTicker {
	...
	public void loadMeUp(Item item, Room room) {
		...
		CMLib.threads().startTickDown(ability, Tickable.TICKID_ROOM_ITEM_REJUV,
				item.phyStats().rejuv());
	}
	...
	public synchronized void verifyFixContents() {
		...
		for (int i = 0; i < contents.size(); i++) {
			...
			if ((!R.isContent(thisItem))
					&& ((!CMLib.flags().isMobile(thisItem)) || (!CMLib.flags()
							.isInTheGame(thisItem, true)))) {
				Item newThisItem = (Item) ((Item) ccontents.elementAt(i))
						.copyOf();
				...
				for (int c = 0; c < ccontents.size(); c++) {
					...
					if ((thatItem.container() == thisItem)
							&& (newThisItem instanceof Container))
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (tickID == Tickable.TICKID_ROOM_ITEM_REJUV) {
			verifyFixContents();
			if ((!R.isContent(item))
					&& ((!CMLib.flags().isMobile(item)) || (!CMLib.flags()
							.isInTheGame(item, true)))) {
				unloadIfNecessary(item);
