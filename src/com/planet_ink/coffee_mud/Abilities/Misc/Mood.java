...
public class Mood extends StdAbility {
	...
	public String displayText() {
		return (moodCode <= 0) ? "" : "(In "
				+ CMLib.english().startWithAorAn(
						MOODS[moodCode][0].toLowerCase()) + " mood)";
	}
	...
	public static final String[] BOAST_CHANNELS = { "BOAST", "GRATZ",
			"ANNOUNCE", "GOSSIP", "OOC", "CHAT" };
	...
	public final static String[] uglyPhrases = { "orc-brain", "jerk", "dork",
			"dim-wit", "excremental waste", "squeegy", "ding-dong",
			"female-dog", "smelly dork", "geek", "illegitimate offspring",
			"gluteous maximus cavity", "uncle copulator", "ugly yokle",
			"brainless goop", "stupid noodle", "stupid ugly-bottom", "pig-dog",
			"son of a silly person", "silly K...kanigget",
			"empty-headed animal", "food trough wiper",
			"perfidious mousedropping hoarder", "son of a window-dresser",
			"brightly-colored, mealy-templed, cranberry-smeller",
			"electric donkey-bottom biter", "bed-wetting type",
				...
				if (str != null) {
					...
					if (CMath.bset(msg.sourceMajor(), CMMsg.MASK_CHANNEL)) {
						...
						for (int i = 0; i < tags.length; i++) {
							...
							if ((msg.othersMessage() != null)
									&& (msg.othersMessage().indexOf(
											MOODS[moodCode][3]) < 0))
								msg.setOthersMessage(CMStrings.replaceFirst(
										msg.othersMessage(), tag, tag + " "
												+ MOODS[moodCode][3]));
							if ((msg.targetMessage() != null)
									&& (msg.targetMessage().indexOf(
											MOODS[moodCode][3]) < 0))
								msg.setTargetMessage(CMStrings.replaceFirst(
										msg.targetMessage(), tag, tag + " "
												+ MOODS[moodCode][3]));
							if ((msg.sourceMessage() != null)
									&& (msg.sourceMessage().indexOf(
											MOODS[moodCode][3]) < 0))
								msg.setSourceMessage(CMStrings.replaceFirst(
										msg.sourceMessage(), tag, tag + " "
												+ MOODS[moodCode][3]));
						}
					}
					String oldStr = str;
					...
					case 0: // formal
					{
						if (str.toUpperCase().startsWith("YOU "))
							str = CMStrings
									.replaceFirstWord(str, "you", "thou");
						str = CMStrings.replaceWord(str, "you`ll",
								"thou willst");
						str = CMStrings
								.replaceWord(str, "youll", "thou willst");
						str = CMStrings.replaceWord(str, "you`re", "thou art");
						str = CMStrings.replaceWord(str, "youre", "thou art");
						str = CMStrings.replaceWord(str, "you`d",
								"thou wouldst");
						str = CMStrings
								.replaceWord(str, "youd", "thou wouldst");
						...
						str = CMStrings
								.replaceWord(str, "hello", "salutations");
						str = CMStrings.replaceWord(str, "no", "negative");
						str = CMStrings.replaceWord(str, "hey", "greetings");
						str = CMStrings.replaceWord(str, "where is",
								"where might we find");
						str = CMStrings.replaceWord(str, "how do",
								"how wouldst");
						str = CMStrings.replaceWord(str, "can`t", "canst not");
						str = CMStrings.replaceWord(str, "cant", "canst not");
						str = CMStrings.replaceWord(str, "couldn`t",
								"couldst not");
						str = CMStrings.replaceWord(str, "couldnt",
								"couldst not");
						...
						str = CMStrings.replaceWord(str, "wouldnt",
								"wouldst not");
						str = CMStrings.replaceWord(str, "wouldn`t",
								"wouldst not");
						...
						str = CMStrings.replaceWord(str, "shouldn`t",
								"should not");
						str = CMStrings.replaceWord(str, "shouldnt",
								"should not");
						...
						str = CMStrings.replaceWord(str, "they`ll",
								"they willst");
						str = CMStrings.replaceWord(str, "theyll",
								"they willst");
						str = CMStrings.replaceWord(str, "they`re", "they art");
						str = CMStrings.replaceWord(str, "theyre", "they art");
						str = CMStrings.replaceWord(str, "they`d",
								"they wouldst");
						str = CMStrings.replaceWord(str, "theyd",
								"they wouldst");
						str = CMStrings
								.replaceWord(str, "they`ve", "they hast");
						str = CMStrings.replaceWord(str, "theyve", "they hast");
						str = CMStrings
								.replaceWord(str, "there`s", "there ist");
						str = CMStrings.replaceWord(str, "theres", "there ist");
						str = CMStrings.replaceWord(str, "there`d",
								"there wouldst");
						str = CMStrings.replaceWord(str, "thered",
								"there wouldst");
						str = CMStrings.replaceWord(str, "there`ll",
								"there willst");
						str = CMStrings.replaceWord(str, "therell",
								"there shall");
						str = CMStrings.replaceWord(str, "that`s", "that ist");
						str = CMStrings.replaceWord(str, "thats", "that ist");
						str = CMStrings.replaceWord(str, "that`d",
								"that wouldst");
						str = CMStrings.replaceWord(str, "thatd",
								"that wouldst");
						str = CMStrings.replaceWord(str, "that`ll",
								"that willst");
						str = CMStrings.replaceWord(str, "thatll",
								"that willst");
					...
					{
						if ((M != null) && (lastOne != M)) {
							msg.source().doCommand(
									new XVector("HANDSHAKE", M.Name()),
									Command.METAFLAG_FORCED);
							lastOne = M;
						}
						switch (CMLib.dice().roll(1, 5, 0)) {
						case 1:
							str = "If you please, " + str;
							break;
						case 2:
							str = CMStrings.endWithAPeriod(str) + " Thank you.";
							break;
						case 3:
							str = CMStrings.endWithAPeriod(str)
									+ " If you please.";
						...
						case 6:
							...
							break;
						case 7:
							str = "Humbly speaking, " + str;
							break;
						default:
							if (msg.source().charStats()
									.getStat(CharStats.STAT_GENDER) == 'F') {
								if (M != null)
									msg.source().doCommand(
											new XVector("CURTSEY", M.Name()),
											Command.METAFLAG_FORCED);
								else
									msg.source().doCommand(
											new XVector("CURTSEY"),
								...
								msg.source().doCommand(
										new XVector("BOW", M.Name()),
										Command.METAFLAG_FORCED);
							else
								msg.source().doCommand(new XVector("BOW"),
										Command.METAFLAG_FORCED);
							break;
						}
						switch (CMLib.dice().roll(1, 5, 0)) {
					...
					case 4: // angry
					{
						...
						case 8:
							if (M != null)
								msg.source().doCommand(
										new XVector("GRUMBLE", M.Name()),
										Command.METAFLAG_FORCED);
							else
								msg.source().doCommand(new XVector("GRUMBLE"),
										Command.METAFLAG_FORCED);
							break;
						default:
							break;
					...
					case 6: // mean
					{
						...
						if (rand < 5)
							str = "Hey "
									+ uglyPhrases[CMLib.dice().roll(1,
											uglyPhrases.length, -1)] + ", "
									+ str;
						else if (rand < 15)
							str = CMStrings.endWithAPeriod(str)
									+ "..you "
							...
							if (M != null)
								msg.source().doCommand(
										new XVector("WHAP", M.Name()),
										Command.METAFLAG_FORCED);
							else
								msg.source().doCommand(new XVector("WHAP"),
										Command.METAFLAG_FORCED);
						}
						if ((M != null) && (CMLib.dice().roll(1, 10, 0) == 1)
					...
					case 9: // excited
					{
						...
						case 5:
							if (M != null)
								msg.source().doCommand(
										new XVector("FIDGET", M.Name()),
										Command.METAFLAG_FORCED);
							else
								msg.source().doCommand(new XVector("FIDGET"),
										Command.METAFLAG_FORCED);
							break;
						default:
							break;
					...
					case 10: // scared
					{
						...
						case 6:
							if (M != null)
								msg.source().doCommand(
										new XVector("WINCE", M.Name()),
										Command.METAFLAG_FORCED);
							else
								msg.source().doCommand(new XVector("WINCE"),
										Command.METAFLAG_FORCED);
							break;
						default:
							break;
					...
					if (!oldStr.equals(str))
						msg.modify(
								...
								msg.tool(),
								msg.sourceCode(),
								CMStrings.substituteSayInMessage(
										msg.sourceMessage(), str),
								msg.targetCode(),
								CMStrings.substituteSayInMessage(
										msg.targetMessage(), str),
								msg.othersCode(),
								CMStrings.substituteSayInMessage(
										msg.othersMessage(), str));
				}
			}
		}
		return super.okMessage(myHost, msg);
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		case 7: // proud
		{
			if ((msg.sourceMinor() == CMMsg.TYP_DEATH)
					&& (msg.tool() == affected) && (msg.source() != null)
				...
				for (int c = 0; c < CHANNELS.length; c++)
					if (CMStrings.contains(BOAST_CHANNELS, CHANNELS[c])) {
						channelIndex = CMLib.channels().getChannelIndex(
								CHANNELS[c]);
						channelC = c;
						if (channelIndex >= 0)
							break;
				...
				if (channelIndex >= 0) {
					...
					((MOB) affected).doCommand(new XVector(CHANNELS[channelC],
							"*I* just killed " + msg.source().Name() + addOn),
							Command.METAFLAG_FORCED);
				}
			}
			break;
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (moodCode.trim().length() == 0)
			moodCode = "NORMAL";
		String moodName = CMLib.english()
				.startWithAorAn(moodCode.toLowerCase());
		...
		if ((choice == null) && (entered.length() > 0)
				&& (Character.isLetter(entered.charAt(0)))) {
		...
		if ((choice == null) || (entered.equalsIgnoreCase("list"))) {
			...
			if (entered.equalsIgnoreCase("LIST"))
				...
				mob.tell("'" + entered
						+ "' is not a known mood. Choices include: "
						+ choices.substring(2));
			return false;
		}
		if (moodCode.equalsIgnoreCase(choice)) {
			if (origEntered.equalsIgnoreCase("RANDOM"))
				return false;
			mob.tell("You are already in "
					+ CMLib.english().startWithAorAn(choice.toLowerCase())
					+ " mood.");
			return false;
		}

		if ((mask.length() > 0)
				&& (!CMLib.masking().maskCheck(mask, mob, true))) {
			if (origEntered.equalsIgnoreCase("RANDOM"))
				return false;
			mob.tell("You must meet the following criteria to be in that mood: "
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(
					...
					this,
					CMMsg.MSG_OK_VISUAL,
					"<T-NAME> appear(s) to be in "
							+ CMLib.english().startWithAorAn(
									choice.toLowerCase()) + " mood.");
