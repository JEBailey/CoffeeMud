...
public class Burning extends StdAbility {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((affected instanceof Item)
			...
			String what = null;
			switch (((Room) (((Item) affected).owner())).getArea()
					.getClimateObj()
					.weatherType(((Room) (((Item) affected).owner())))) {
			...
			if (CMLib.dice().rollPercentage() < unInvokeChance) {
				...
				if (R.numInhabitants() > 0)
					R.showHappens(CMMsg.MSG_OK_ACTION, "The " + what
							+ " puts out " + affected.name() + ".");
				unInvoke();
				return false;
			}
		}
					...
					if ((affected instanceof RawMaterial)
						...
						for (int i = 0; i < room.numItems(); i++) {
							...
							if (I.name().equals(affected.name())
									...
									&& (I.material() == ((Item) affected)
											.material())) {
								int durationOfBurn = CMLib.flags()
										.burnStatus(I);
								if (durationOfBurn <= 0)
									durationOfBurn = 5;
						...
						default: {
							if (CMLib.flags().isABonusItems(affected)) {
								if (invoker == null) {
									...
									invoker.setLocation(CMClass
											.getLocale("StdRoom"));
									invoker.basePhyStats().setLevel(
											affected.phyStats().level());
									invoker.phyStats().setLevel(
											affected.phyStats().level());
								}
								room.showHappens(CMMsg.MSG_OK_ACTION,
										affected.name() + " EXPLODES!!!");
								for (int i = 0; i < room.numInhabitants(); i++) {
									...
									CMLib.combat().postDamage(
											...
											null,
											CMLib.dice()
													.roll(affected.phyStats()
															.level(), 5, 1),
											CMMsg.MASK_ALWAYS | CMMsg.TYP_FIRE,
											Weapon.TYPE_BURNING,
											"The blast <DAMAGE> <T-NAME>!");
								}
								((Item) affected).destroy();
							} else {
								...
								if ((affected instanceof RawMaterial)
									...
									affected.basePhyStats()
											.setWeight(
													affected.basePhyStats()
															.weight() - 1);
									affected.recoverPhyStats();
									this.tickDown = CMLib.flags().burnStatus(
											affected);
									CMLib.materials().adjustResourceName(
											(Item) affected);
		...
		if (affected instanceof Item) {
			...
			if ((I.subjectToWearAndTear()) && (I.usesRemaining() > 1)) {
				if (owner instanceof MOB)
					CMLib.combat().postItemDamage((MOB) owner, I, null, 1,
							CMMsg.TYP_FIRE, null);
				else if (owner instanceof Room) {
					MOB M = CMLib.map().getFactoryMOB((Room) owner);
					CMLib.combat().postItemDamage(M, I, null, 1,
							CMMsg.TYP_FIRE, null);
					M.destroy();
				}
			}
	...
	public boolean ouch(MOB mob) {
		if (CMLib.dice().rollPercentage() > (mob.charStats().getSave(
			...
			CMLib.combat().postDamage(invoker, mob, this,
					CMLib.dice().roll(1, 5, 5),
					CMMsg.MASK_ALWAYS | CMMsg.TYP_FIRE, Weapon.TYPE_BURNING,
					null);
			return false;
		}
		return true;
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((affected != null) && (affected instanceof Item)
			...
			if ((C instanceof Drink) && (((Drink) C).containsDrink())) {
				msg.addTrailerMsg(CMClass.getMsg(invoker, null,
						CMMsg.MSG_OK_VISUAL, I.name() + " is extinguished."));
				I.delEffect(this);
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical target,
		...
		if (target.fetchEffect("Burning") == null) {
			...
			if (mob != null) {
				...
				if (room != null) {
					CMMsg msg = CMClass.getMsg(mob, target, CMMsg.MASK_ALWAYS
							| CMMsg.TYP_FIRE, null);
