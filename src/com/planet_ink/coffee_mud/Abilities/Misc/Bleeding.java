...
public class Bleeding extends StdAbility implements HealthCondition {
	...
	public void unInvoke() {
		if ((affected instanceof MOB) && (canBeUninvoked())
			...
			((MOB) affected).location().show((MOB) affected, null, null,
					CMMsg.MSG_OK_VISUAL, "<S-NAME> stop(s) bleeding.");
		super.unInvoke();
	}

	public void executeMsg(Environmental myHost, CMMsg msg) {
		super.executeMsg(myHost, msg);
		if ((affected != null) && (msg.amITarget(affected))
			...
			if (msg.targetMinor() == CMMsg.TYP_HEALING) {
				...
				msg.source().tell((MOB) msg.target(), null, null,
						"^R<S-NAME> <S-IS-ARE> still bleeding...");
		} else if ((msg.source() == affected) && (msg.target() instanceof Room)
			...
			if ((dir >= 0) && (R.findItem(null, "a trail of blood") == null)) {
				...
				if (lastDir >= 0)
					I.setDisplayText("A faint trail of blood leads from "
							+ Directions.getDirectionName(lastDir) + " to "
							+ Directions.getDirectionName(dir) + ".");
				else
					I.setDisplayText("A faint trail of blood leads "
							+ Directions.getDirectionName(dir) + ".");
				I.phyStats().setDisposition(
						I.phyStats().disposition() | PhyStats.IS_HIDDEN
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((ticking instanceof MOB) && (tickID == Tickable.TICKID_MOB)) {
			...
			if (hpToKeep <= 0) {
				...
				int maxMana = (int) Math.round(CMath.mul(mob.maxState()
						.getMana(), healthPct(mob)));
				if (mob.curState().getMana() > maxMana)
					mob.curState().setMana(maxMana);
				int maxMovement = (int) Math.round(CMath.mul(mob.maxState()
						.getMovement(), healthPct(mob)));
	...
	public boolean invoke(MOB mob, Vector commands, Physical target,
		...
		if (CMLib.flags().isGolem(target))
			return false;
		if (((MOB) target).phyStats().level() < CMProps
				.getIntVar(CMProps.Int.INJBLEEDMINLEVEL))
			return false;
		if (((MOB) target).fetchEffect(ID()) != null)
			return false;
		if (((MOB) target).location() == null)
			return false;
		if (((MOB) target).location().show((MOB) target, null, this,
				CMMsg.MSG_OK_VISUAL, "^R<S-NAME> start(s) BLEEDING!^?"))
