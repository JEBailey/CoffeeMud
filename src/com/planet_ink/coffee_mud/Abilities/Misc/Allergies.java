...
public class Allergies extends StdAbility implements HealthCondition {
	...
	public String getHealthConditionDesc() {
		...
		if (list.size() == 0)
			return "";
		return "Suffers from allergies to "
				+ CMLib.english().toEnglishStringList(list) + ".";
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		if (!super.tick(ticking, tickID))
			return false;
		if (((++allergicCheckDown) > 10) && (affected instanceof MOB)) {
			allergicCheckDown = 0;
			...
			if ((CMLib.flags().aliveAwakeMobile(mob, true))
				...
				if (raceAllergies.size() > 0) {
					...
					for (int i = 0; i < R.numInhabitants(); i++) {
						...
						if ((M != null)
								&& (M != mob)
								&& (raceAllergies.contains(M.charStats()
										.getMyRace())))
							R.show(mob, null, this, CMMsg.TYP_NOISYMOVEMENT,
									"<S-NAME> sneeze(s)! AAAAACHHHOOOO!");
					}
					...
					for (int i = 0; i < R.numItems(); i++) {
						...
						if ((I != null)
								&& (I.container() == null)
								&& (resourceAllergies.contains(Integer
										.valueOf(I.material()))))
							R.show(mob, null, this, CMMsg.TYP_NOISYMOVEMENT,
									"<S-NAME> sneeze(s)! AAAAACHHHOOOO!");
					}
						...
						if (M != null)
							for (int i = 0; i < M.numItems(); i++) {
								...
								if ((I != null)
										...
										&& (resourceAllergies.contains(Integer
												.valueOf(I.material()))))
									R.show(mob, null, this,
											CMMsg.TYP_NOISYMOVEMENT,
											"<S-NAME> sneeze(s)! AAAAACHHHOOOO!");
							}
					}
				}
		...
		if ((affected != null) && (affected instanceof MOB)) {
			if (msg.source() == affected) {
				if ((msg.targetMinor() == CMMsg.TYP_EAT)
						&& (((msg.target() instanceof Item) && (resourceAllergies
								.contains(Integer.valueOf(((Item) msg.target())
										.material())))) || ((msg.target() instanceof MOB) && (raceAllergies
								.contains(((MOB) msg.target()).charStats()
										.getMyRace()))))) {
					...
					&& (raceAllergies.contains(msg.source().charStats()
							.getMyRace()))
					&& ((msg.targetMajor(CMMsg.MASK_HANDS)) || (msg
							.targetMajor(CMMsg.MASK_MOVE)))
					&& (((MOB) affected).location() != null)
					&& (((MOB) affected).location().isInhabitant(msg.source()))
					&& ((msg.tool() == null) || ((!msg.tool().ID()
							.equals("Poison_Hives")) && (!msg.tool().ID()
							.equals("Poison_Heartstopper"))))) {
				Ability A = CMClass.getAbility("Poison_Hives");
				if (A != null)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			String allergies = "";
			if ((choice.length() > 0)
					&& (allChoices.contains(choice.toUpperCase())))
				allergies = choice.toUpperCase();
			else
				for (int i = 0; i < allChoices.size(); i++)
					if ((CMLib.dice().roll(1, allChoices.size(), 0) == 1)
							&& (!(((String) allChoices.elementAt(i))
									.equalsIgnoreCase(mob.charStats()
											.getMyRace().ID().toUpperCase()))))
