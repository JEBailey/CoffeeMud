...
public class Falling extends StdAbility {
	...
	protected boolean canFallFrom(Room fromHere, int direction) {
		if ((fromHere == null) || (direction < 0)
				|| (direction >= Directions.NUM_DIRECTIONS()))
			return false;

		Room toHere = fromHere.getRoomInDir(direction);
		if ((toHere == null) || (fromHere.getExitInDir(direction) == null)
	...
	protected boolean stopFalling(MOB mob) {
		...
		if (reversed()) {
			if (!hitTheCeiling) {
				hitTheCeiling = true;
				if (R != null)
					R.show(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> hit(s) the ceiling."
									+ CMLib.protocol().msp("splat.wav", 50));
				CMLib.combat().postDamage(mob, mob, this, damageToTake,
						CMMsg.MASK_ALWAYS | CMMsg.TYP_JUSTICE, -1, null);
			}
			return true;
		}
		hitTheCeiling = false;
		...
		if (R != null) {
			if (isAirRoom(R))
				R.show(mob,
						null,
						CMMsg.MSG_OK_ACTION,
						"<S-NAME> stop(s) falling."
								+ CMLib.protocol().msp("splat.wav", 50));
			else if (isWaterSurface(R) || isUnderWater(R))
				R.show(mob,
						null,
						CMMsg.MSG_OK_ACTION,
						"<S-NAME> hit(s) the water."
								+ CMLib.protocol().msp("splat.wav", 50));
			else
				R.show(mob,
						null,
						CMMsg.MSG_OK_ACTION,
						"<S-NAME> hit(s) the ground."
								+ CMLib.protocol().msp("splat.wav", 50));
			CMLib.combat().postDamage(mob, mob, this, damageToTake,
					CMMsg.MASK_ALWAYS | CMMsg.TYP_JUSTICE, -1, null);
		}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (affected instanceof MOB) {
			...
			if (CMLib.flags().isInFlight(mob)) {
				...
				if (mob.phyStats().weight() < 1) {
					...
					int damage = CMLib.dice().roll(
							1,
							(int) Math.round(CMath.mul(CMath.mul(mob.maxState()
									.getHitPoints(), 0.1), CMath.div(
									mob.baseWeight(), 150.0))), 0);
					damageToTake = reversed() ? damage
							: (damageToTake + damage);
				}
			...
			if (canFallFrom(room, direction)) {
				room.show(invoker, null, item, CMMsg.MSG_OK_ACTION,
						"<O-NAME> falls " + addStr + ".");
				nextRoom.moveItemTo(item, ItemPossessor.Expire.Player_Drop);
				room = nextRoom;
				nextRoom.show(invoker, null, item, CMMsg.MSG_OK_ACTION,
						"<O-NAME> falls in from "
								+ (reversed() ? "below" : "above") + ".");
				return true;
			}
			if (reversed())
				return true;
		...
		if ((affected != null) && (affected instanceof MOB))
			if (msg.amISource((MOB) affected)) {
				...
				if (msg.targetMajor(CMMsg.MASK_MOVE) && (!hitTheCeiling)) {
					msg.source().tell(
							"You are too busy falling to do that right now.");
					return false;
				}
			}
		return true;
