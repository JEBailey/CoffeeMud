...
public class Immunities extends StdAbility {
	...
	public static SHashtable<String, Integer> immunityTypes = new SHashtable<String, Integer>(
			new Object[][] { { "ACID", Integer.valueOf(CMMsg.TYP_ACID) },
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (((!(affected instanceof MOB)) || (msg.amITarget(affected) && (!((MOB) affected)
				.amDead())))
				&& (immunes.contains(Integer.valueOf(msg.targetMinor())) || immunes
						.contains(Integer.valueOf(msg.sourceMinor())))
				&& (CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS)
						|| (msg.targetMinor() == CMMsg.TYP_DAMAGE) || (msg
						.targetMinor() == CMMsg.TYP_LEGALWARRANT))) {
			...
			if (!msg.sourceMajor(CMMsg.MASK_CNTRLMSG)
				...
				if (msg.target() != msg.source())
					R.show(msg.source(), msg.target(), CMMsg.MSG_OK_VISUAL,
							"<T-NAME> seem(s) immune to " + immunityName
									+ " attacks from <S-NAME>.");
				else
					R.show(msg.source(), msg.target(), CMMsg.MSG_OK_VISUAL,
							"<T-NAME> seem(s) immune to " + immunityName + ".");
		...
		if (commands.size() > 0)
			for (final Object o : commands) {
				...
				if (s.startsWith("TICKS=")
						&& (CMath.isInteger(s.substring(6).trim())))
