...
public class Dueler extends StdAbility {
	...
	public void unInvoke() {
		if (affected instanceof MOB) {
			...
			if ((mob != null) && (oldCurState != null)) {
				...
				for (Enumeration<Ability> a = mob.effects(); a
						.hasMoreElements();) {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((ticking instanceof MOB) && (tickID == Tickable.TICKID_MOB)) {
			...
			if ((tDuel == null)
					...
					|| (!(CMLib.flags().isInTheGame((MOB) tDuel.affecting(),
							false)))
					|| (mob.getVictim() == null)
					|| ((mob.getVictim() != tDuel.affecting()) && (mob
							.getVictim().amUltimatelyFollowing() != tDuel
							.affecting())))
				unInvoke();
			else {
	...
	public void init(MOB mob) {
		...
		oldEffects.clear();
		for (Enumeration<Ability> a = mob.personalEffects(); a
				.hasMoreElements();)
	...
	public boolean invoke(MOB mob, Vector commands, Physical target,
		...
		if (((MOB) target).location() == null)
			return false;
		if (((MOB) target).location().show(mob, target, this,
				CMMsg.MSG_OK_VISUAL,
				"^R<S-NAME> and <T-NAME> start(s) dueling!^?")) {
			MOB tmob = (MOB) target;
			Dueler A;
			Dueler tA;
