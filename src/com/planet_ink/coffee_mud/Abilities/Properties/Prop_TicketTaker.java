...
public class Prop_TicketTaker extends Property {
	...
	protected boolean isMine(Environmental host, Rideable R) {
		if (host instanceof Rider) {
			...
			if ((R instanceof Rider) && (((Rider) R).riding() == mob.riding()))
				return true;
			if ((mob.riding() instanceof Rider)
					&& (((Rider) mob.riding()).riding() == R))
				return true;
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (((myHost instanceof Rider) && (((Rider) myHost).riding() != null))
			...
			MOB mob = msg.source();
			if ((msg.target() != null) && (myHost != mob) && (!mob.isMonster())
				...
				case CMMsg.TYP_SLEEP: {
					...
					if (CMLib.beanCounter()
							.getTotalAbsoluteValue(mob, currency) >= cost()) {
						...
						mob.location()
								.show(mob,
										...
										"<S-NAME> give(s) " + costStr
												+ " to <T-NAME>.");
						CMLib.beanCounter()
								.subtractMoney(mob, currency, cost());
					}
				}
					break;
				}
		...
		if (((myHost instanceof Rider) && (((Rider) myHost).riding() != null))
			...
			if ((msg.target() != null) && (myHost != mob) && (!mob.isMonster())
				...
				case CMMsg.TYP_SLEEP: {
					...
					if (CMLib.beanCounter()
							.getTotalAbsoluteValue(mob, currency) < cost()) {
						String costStr = CMLib.beanCounter().nameCurrencyLong(
								currency, cost());
						if (myHost instanceof MOB)
							CMLib.commands().postSay((MOB) myHost, mob,
									"You'll need " + costStr + " to board.",
									false, false);
						else
