...
public class Prop_SpellAdder extends Property implements AbilityContainer,
		TriggeredAffect {
	public String ID() {
	...
	protected int canAffectCode() {
		return Ability.CAN_ITEMS | Ability.CAN_ROOMS | Ability.CAN_AREAS
				| Ability.CAN_MOBS;
	}
	...
	public List<Ability> getMySpellsV() {
		...
		for (int s = 0; s < set.size(); s++) {
			...
			if ((pctDex > 0)
					&& (thisOne.substring(pctDex + 1).trim().length() > 0))
	...
	public MOB getBestInvokerMOB(Environmental target) {
		if (target instanceof MOB)
			return (MOB) target;
		if ((target instanceof Item) && (((Item) target).owner() != null)
				&& (((Item) target).owner() instanceof MOB))
	...
	public boolean addMeIfNeccessary(PhysicalAgent source, Physical target,
			boolean makeLongLasting, int asLevel, short maxTicks) {
		...
		for (int v = 0; v < VTOO.size(); v += 2) {
			...
			lastMOB = target;
			// this needs to go here because otherwise it makes non-item-invoked
			// spells long lasting,
	...
	public String accountForYourself() {
		return spellAccountingsWithMask("Casts ",
				" on the first one who enters.");
	}
