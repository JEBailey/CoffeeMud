...
public class Prop_LotsForSale extends Prop_RoomForSale {
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if ((R2 != null) && (R2.roomID().length() > 0) && (!V.contains(R2))) {
				...
				if ((R2.getArea() == R.getArea()) && (A != null))
					...
					V.remove(R); // purpose here is to put the "front" door up
									// front.
					V.add(0, R);
				}
			}
	...
	protected static boolean isCleanRoom(Room fromRoom, Room theRoom) {
		...
		if ((theRoom.roomID().length() > 0)
				&& ((CMLib.law().getLandTitle(theRoom) == null) || (CMLib.law()
						.getLandTitle(theRoom).getOwnerName().length() > 0)))
			return false;
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if ((R != null)
					&& (R != fromRoom)
					&& (R.roomID().length() > 0)
					&& ((CMLib.law().getLandTitle(R) == null) || (CMLib.law()
							.getLandTitle(R).getOwnerName().length() > 0)))
				return false;
		}
		return true;
	}
	...
	public void updateLot(List optPlayerList) {
		...
		try {
			synchronized (("SYNC" + R.roomID()).intern()) {
				...
				lastItemNums = updateLotWithThisData(R, this, true,
				...
				if (getOwnerName().length() == 0) {
					...
					for (int d = 0; d < Directions.NUM_DIRECTIONS(); d++) {
						...
						if ((R2 != null) && (isCleanRoom(R, R2))) {
							...
							updateExits = true;
							if (R2 != null) {
								E = R2.getRawExit(Directions
										.getOpDirectionCode(d));
								if ((E != null) && (E.hasALock())
										&& (E.isGeneric())) {
									E.setKeyName("");
									E.setDoorsNLocks(E.hasADoor(), E.isOpen(),
											E.defaultsClosed(), false, false,
											false);
					...
					for (int d = 0; d < Directions.NUM_DIRECTIONS(); d++) {
						...
						if (R2 == null) {
							...
							R2.setRawExit(Directions.getOpDirectionCode(d),
									CMClass.getExit("Open"));
							updateExits = true;
							if (CMSecurity
									.isDebugging(CMSecurity.DbgFlag.PROPERTY))
								Log.debugOut("Lots4Sale", R2.roomID()
										+ " created and put up for sale.");
							CMLib.database().DBCreateRoom(R2);
							if (newTitle != null)
								colorForSale(R2, newTitle.rentalProperty(),
										true);
							R2.getArea().fillInAreaRoom(R2);
							CMLib.database().DBUpdateExits(R2);
							didAnything = true;
			...
			if (didAnything)
				getConnectedPropertyRooms(); // recalculates the unique id for
												// this lot of rooms
		}
	}
}
