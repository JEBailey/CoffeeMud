...
public class Prop_SpellReflecting extends Property implements TriggeredAffect {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS))
			...
			int lvl = CMLib.ableMapper().qualifyingLevel(msg.source(),
					((Ability) msg.tool()));
			if (lvl <= 0)
				lvl = CMLib.ableMapper().lowestQualifyingLevel(
						((Ability) msg.tool()).ID());
			if (lvl <= 0)
				lvl = 1;
			...
			target.location().show(target, affected, CMMsg.MSG_OK_VISUAL,
					"The field around <T-NAMESELF> reflects the spell!");
			Ability A = (Ability) msg.tool();
			A.invoke(target, msg.source(), true, msg.source().phyStats()
					.level());
			setAbilityCode(abilityCode() - lvl);
			if (abilityCode() <= 0) {
				if (affected instanceof MOB) {
					target.location().show(target, target, CMMsg.MSG_OK_VISUAL,
							"The field around <T-NAMESELF> fades.");
					if (fade > 0)
						target.delEffect(this);
				} else if (affected instanceof Item) {
					if (fade > 0) {
						target.location().show(target, affected,
								CMMsg.MSG_OK_VISUAL, "<T-NAMESELF> vanishes!");
