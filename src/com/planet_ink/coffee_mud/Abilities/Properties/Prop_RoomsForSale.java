...
public class Prop_RoomsForSale extends Prop_RoomForSale {
	...
	public void updateLot(List optPlayerList) {
		if (affected instanceof Room) {
			lastItemNums = updateLotWithThisData((Room) affected, this, false,
					scheduleReset, optPlayerList, lastItemNums);
			if ((lastDayDone != ((Room) affected).getArea().getTimeObj()
					.getDayOfMonth())
				...
				for (int v = 0; v < V.size(); v++) {
					Room R2 = V.get(v);
					Prop_RoomForSale PRFS = (Prop_RoomForSale) R2
							.fetchEffect(ID());
					if (PRFS != null)
						PRFS.lastDayDone = R.getArea().getTimeObj()
								.getDayOfMonth();
				}
				if ((getOwnerName().length() > 0) && rentalProperty()
					...
					if (doRentalProperty(R.getArea(), R.roomID(),
						...
						setOwnerName("");
						updateTitle();
						lastItemNums = updateLotWithThisData((Room) affected,
								this, false, scheduleReset, optPlayerList,
								lastItemNums);
					}
			}
			scheduleReset = false;
