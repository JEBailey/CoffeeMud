...
public class Prop_OpenPassword extends Property {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((msg.sourceMinor() == CMMsg.TYP_SPEAK) && (affected != null)
				&& (msg.sourceMessage() != null)
				&& ((msg.sourceMajor() & CMMsg.MASK_MAGIC) == 0)) {
			int start = msg.sourceMessage().indexOf("\'");
			int end = msg.sourceMessage().lastIndexOf("\'");
			if ((start > 0) && (end > start)) {
				String str = msg.sourceMessage().substring(start + 1, end)
						.trim();
				MOB mob = msg.source();
				if (str.equalsIgnoreCase(text()) && (text().length() > 0)
						&& (mob.location() != null)) {
					Room R = mob.location();
					if (affected instanceof Exit) {
						...
						if (!E.isOpen()) {
							...
							if (dirCode >= 0) {
								CMMsg msg2 = CMClass.getMsg(mob, E, null,
										CMMsg.MSG_UNLOCK, null);
								CMLib.utensils().roomAffectFully(msg2, R,
										dirCode);
								msg2 = CMClass.getMsg(mob, E, null,
										CMMsg.MSG_OPEN, "<T-NAME> opens.");
								CMLib.utensils().roomAffectFully(msg2, R,
										dirCode);
							}
						}
					} else if (affected instanceof Container) {
						CMMsg msg2 = CMClass.getMsg(mob, affected, null,
								CMMsg.MSG_UNLOCK, null);
						affected.executeMsg(mob, msg2);
						msg2 = CMClass.getMsg(mob, affected, null,
								CMMsg.MSG_OPEN, "<T-NAME> opens.");
						affected.executeMsg(mob, msg2);
					}
				}
