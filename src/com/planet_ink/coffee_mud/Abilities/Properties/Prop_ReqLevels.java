...
public class Prop_ReqLevels extends Property implements TriggeredAffect {
	...
	public boolean passesMuster(MOB mob, Environmental R) {
		...
		if ((allFlag)
				...
				|| (!(R instanceof Room))
				|| (CMSecurity
						.isAllowed(mob, (Room) R, CMSecurity.SecFlag.GOTO)))
			return true;

		if ((sysopFlag)
				&& (R instanceof Room)
				&& (!CMSecurity.isAllowed(mob, (Room) R,
		...
		while (x >= 0) {
			...
			if (x >= 0) {
				...
				String cmpString = "";
				while ((x < text.length())
						&& (((text.charAt(x) == ' ') && (cmpString.length() == 0)) || (Character
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((affected != null)
			...
			for (Iterator e = H.iterator(); e.hasNext();) {
				...
				if ((E instanceof MOB) && (passesMuster((MOB) E, msg.target())))
