...
public class Prop_ReqStat extends Property implements TriggeredAffect {
	...
	public String accountForYourself() {
		return "Entry restricted as follows: "
	...
	public boolean passesMuster(MOB mob, String msg) {
		...
		for (int c : CharStats.CODES.ALL()) {
			comp = CMParms.getParmCompare(text(), CharStats.CODES.NAME(c), mob
					.charStats().getStat(c));
			if (comp[1] < 0) {
				switch (comp[0]) {
				case '=':
				case '!':
					mob.tell("You aren't the right "
							+ CMStrings.capitalizeAndLower(CharStats.CODES
									.NAME(c)) + " to " + msg + ".");
					break;
				case '<':
					mob.tell("You are too "
							+ CMStrings.capitalizeAndLower(CharStats.CODES
									.ATTDESC(c)) + " to " + msg + ".");
					break;
				case '>':
					mob.tell("You are not "
							+ CMStrings.capitalizeAndLower(CharStats.CODES
									.ATTDESC(c)) + " enough to " + msg + ".");
					break;
				}
				return false;
			}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (affected != null) {
			if ((msg.target() == affected) && (affected instanceof Exit)
					&& (((Exit) affected).hasADoor())) {
				switch (msg.targetMinor()) {
				case CMMsg.TYP_OPEN:
					if (passesMuster(msg.source(), ((Exit) affected).openWord()
							+ " that"))
						return super.okMessage(myHost, msg);
					return false;
				case CMMsg.TYP_CLOSE:
					if (passesMuster(msg.source(),
							((Exit) affected).closeWord() + " that"))
						return super.okMessage(myHost, msg);
					return false;
				}
				...
				case CMMsg.TYP_CLOSE:
					...
					&& (((msg.target() instanceof Room) && (msg.targetMinor() == CMMsg.TYP_ENTER)) || ((msg
							.target() instanceof Rideable) && (msg
