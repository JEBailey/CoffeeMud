...
public class Prop_HaveResister extends Property implements TriggeredAffect {
	...
	public void setMiscText(String newText) {
		...
		for (int i : CharStats.CODES.SAVING_THROWS()) {
			if (parmString.toUpperCase().indexOf(CharStats.CODES.NAME(i)) >= 0)
				...
				adjCharStats.setStat(i, getProtection(CMStrings.limit(
						CharStats.CODES.NAME(i), 4)));
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectedStats) {
		...
		if ((!ignoreCharStats)
				...
				&& ((maskString.length() == 0) || (CMLib.masking().maskCheck(
						maskString, affectedMOB, false))))
			for (int i : CharStats.CODES.SAVING_THROWS())
				affectedStats.setStat(i, affectedStats.getStat(i)
						+ adjCharStats.getStat(i));
		super.affectCharStats(affectedMOB, affectedStats);
	}

	public boolean checkProtection(String protType) {
		return getProtection(protType) != 0;
	}
	...
	protected int weaponProtection(String kind, int damage, int myLevel,
			int hisLevel) {
		int protection = remainingProtection;
		if ((System.currentTimeMillis() - lastProtection) >= CMProps
				.getTickMillis()) {
		...
		return (int) Math.round(CMath.mul(damage,
				1.0 - CMath.div(protection, 100.0)));
	}
		...
		if ((msg.targetMinor() == CMMsg.TYP_DAMAGE) && ((msg.value()) > 0)
			...
			if (checkProtection("weapons")) {
				if ((maskString.length() == 0)
						|| (CMLib.masking().maskCheck(maskString, mob, false)))
					msg.setValue(weaponProtection("weapons", msg.value(), mob
							.phyStats().level(), msg.source().phyStats()
							.level()));
			} else {
				Weapon W = (Weapon) msg.tool();
				if ((W.weaponType() == Weapon.TYPE_BASHING)
						&& (checkProtection("blunt"))
						&& ((maskString.length() == 0) || (CMLib.masking()
								.maskCheck(maskString, mob, false))))
					msg.setValue(weaponProtection("blunt", msg.value(), mob
							.phyStats().level(), msg.source().phyStats()
							.level()));
				if ((W.weaponType() == Weapon.TYPE_PIERCING)
						...
						&& ((maskString.length() == 0) || (CMLib.masking()
								.maskCheck(maskString, mob, false))))
					msg.setValue(weaponProtection("pierce", msg.value(), mob
							.phyStats().level(), msg.source().phyStats()
							.level()));
				if ((W.weaponType() == Weapon.TYPE_SLASHING)
						...
						&& ((maskString.length() == 0) || (CMLib.masking()
								.maskCheck(maskString, mob, false))))
					msg.setValue(weaponProtection("slash", msg.value(), mob
							.phyStats().level(), msg.source().phyStats()
							.level()));
			}
			return;
				...
				if (CMath.bset(A.flags(), Ability.FLAG_TRANSPORTING)) {
					if ((checkProtection("teleport"))
							&& ((maskString.length() == 0) || (CMLib.masking()
									.maskCheck(maskString, mob, false)))) {
						msg.source().tell(
								"You can't seem to fixate on '" + mob.name()
										+ "'.");
						return false;
					}
				} else if (!CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS))
					return true;
				else if (((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_PRAYER)
					...
					if ((checkProtection("holy"))
							&& ((maskString.length() == 0) || (CMLib.masking()
									.maskCheck(maskString, mob, false)))) {
						mob.location()
								.show(msg.source(), mob, CMMsg.MSG_OK_VISUAL,
										"Holy energies from <S-NAME> are repelled from <T-NAME>.");
						return false;
					}
				}
			}
	...
	public boolean canResist(Environmental E) {
		if ((affected instanceof Item) && (E instanceof MOB)
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((canResist(msg.target())) && (msg.target() instanceof MOB)
				&& (((MOB) msg.target()).location() != null)) {
			if ((msg.value() <= 0)
					&& (!isOk(msg, this, (MOB) msg.target(), maskString)))
				return false;
