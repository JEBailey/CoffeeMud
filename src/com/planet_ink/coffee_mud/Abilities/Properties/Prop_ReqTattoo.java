	...
	public boolean passesMuster(Vector mask, boolean[] flags, MOB mob) {
		...
		for (int v = 0; v < mask.size(); v++) {
			...
			if (s.equals("+ALL"))
				...
				char c = s.charAt(0);
				boolean found = ((c == '+') || (c == '-')) ? (mob.findTattoo(s
						.substring(1)) != null) : (mob.findTattoo(s) != null);
				switch (allFlag) {
		...
		if (allFlag < 0)
			return false; // if not returned, does not have any of them
		if (allFlag == 0)
			return true; // none were missing, so its all good.
		if (allFlag > 0)
			return true; // all were missing, so its all good.
		return true;
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((affected != null)
			...
			if (((msg.target() instanceof Room) && (msg.targetMinor() == CMMsg.TYP_ENTER))
					|| ((msg.target() instanceof Item) && ((msg.targetMinor() == CMMsg.TYP_GET) || (msg
				...
				for (Iterator e = H.iterator(); e.hasNext();) {
					...
					if ((E instanceof MOB) && (passesMuster(V, flags, (MOB) E)))
						return super.okMessage(myHost, msg);
				}
				if (msg.target() instanceof Room)
					msg.source()
							.tell(themsg.length() == 0 ? "You have not been granted authorization to go that way."
									: themsg);
					...
					msg.source()
							...
							.show(msg.source(),
									...
									affected,
									CMMsg.MSG_OK_ACTION,
									themsg.length() == 0 ? "<O-NAME> flashes and flies out of <S-HIS-HER> hands!"
											: themsg);
				return false;
			}
		}
