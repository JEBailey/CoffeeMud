	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((affected != null) && (affected instanceof MOB)) {
			...
			if (mob.location() != null) {
				if ((lastMoveIn > 0)
						&& ((System.currentTimeMillis() - lastMoveIn) > 4)) {
					lastMoveIn = 0;
						...
						if ((mob.amFollowing() != null)
							...
							CMLib.commands()
									.postSay(
											mob,
											mob.amFollowing(),
											"Is this my new permanent post?  If so, order me to NOFOLLOW and I'll stay here.",
											false, false);
							...
							CMLib.commands()
									.postSay(
											mob,
											mob.amFollowing(),
											"I guess this is my new permanent posting?",
											false, false);
					}
				}
				if (payPeriodLengthInMudDays > 0) {
					if (lastPayDayTimestamp == 0) {
						...
						miscText = payAmountPerPayPeriod + ";"
								+ payPeriodLengthInMudDays + ";"
								+ lastPayDayTimestamp;
					}
					...
					if ((System.currentTimeMillis() > (lastPayDayTimestamp + payPeriodLengthInMilliseconds))
						...
						if (mob.amFollowing() != null) {
							...
							if ((t != null)
									...
									&& (!t.getOwnerName().equalsIgnoreCase(
											mob.amFollowing().Name()))
									&& (mob.amFollowing().getClanRole(
											t.getOwnerName()) == null)) {
								CMLib.commands().postSay(mob, null,
										"Hey, I'm not a crook!", false, false);
						...
						if (owner.length() == 0) {
							CMLib.commands().postSay(mob, null,
									"Argh! I quit!", false, false);
							mob.setFollowing(null);
							CMLib.tracking().wanderAway(mob, true, false);
							mob.destroy();
							return false;
						}
						boolean paid = CMLib.beanCounter().modifyLocalBankGold(
								mob.location().getArea(),
								owner,
								CMLib.utensils().getFormattedDate(mob)
										+ ": Withdrawal of "
										+ CMLib.beanCounter()
												.nameCurrencyShort(mob,
														payAmountPerPayPeriod)
										+ ": Payroll: " + Name(),
								CMLib.beanCounter().getCurrency(mob),
								(-payAmountPerPayPeriod));
						if (paid)
							CMLib.commands().postSay(mob, null, "Payday!",
									false, false);
						else {
							CMLib.commands().postSay(mob, null,
									"I don't work for free!  I quit!", false,
									false);
							mob.setFollowing(null);
							CMLib.tracking().wanderAway(mob, true, false);
							mob.destroy();
	...
	public void tellSkills(MOB me, MOB toMe) {
		...
		if (me instanceof ShopKeeper)
			skills.append(", selling "
					+ CMLib.coffeeShops()
							.storeKeeperString(((ShopKeeper) me).getShop())
							.toLowerCase());
		for (Enumeration<Ability> a = me.allAbilities(); a.hasMoreElements();) {
			Ability A = a.nextElement();
			if (A != null) {
				if (A.proficiency() == 0)
					A.setProficiency(50 + me.phyStats().level()
							- CMLib.ableMapper().lowestQualifyingLevel(A.ID()));
		...
		if (skills.length() > 2)
			CMLib.commands().postSay(me, toMe,
					"My skills include: " + skills.substring(2) + ".", false,
					false);
	}
				...
				if ((msg.tool() == affected)
					...
					if (payPeriodLengthInMudDays > 0)
						CMLib.commands().postSay(
								mob,
								msg.source(),
								"I accept your terms of employment, and I understand I will be paid "
										+ CMLib.beanCounter().abbreviatedPrice(
												mob, payAmountPerPayPeriod)
										+ " every " + payPeriodLengthInMudDays
										+ " days.", false, false);
					else
						CMLib.commands().postSay(mob, msg.source(),
								"I accept your terms of employment.", false,
								false);
					CMLib.commands().postSay(mob, msg.source(),
							"Please show me the way to my permanent post.",
							false, false);
					lastPayDayTimestamp = 0;
					...
					if ((msg.sourceMinor() == CMMsg.TYP_SHUTDOWN)
							|| ((msg.targetMinor() == CMMsg.TYP_EXPIRE) && ((msg
									.target() == room) || (msg.target() == mob) || (msg
									.target() == mob.amFollowing())))
							|| ((msg.sourceMinor() == CMMsg.TYP_QUIT) && (msg
									.amISource(mob.amFollowing()))))
						mob.setFollowing(null);
				}
			}
		}
