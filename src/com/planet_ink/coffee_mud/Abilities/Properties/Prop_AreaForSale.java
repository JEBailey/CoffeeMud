...
public class Prop_AreaForSale extends Property implements LandTitle {
	...
	public void updateTitle() {
		if (affected instanceof Area)
			CMLib.database().DBUpdateArea(((Area) affected).name(),
					(Area) affected);
		else if (affected instanceof Room)
			Log.errOut("Prop_AreaForSale",
					"Prop_AreaForSale goes on an Area, NOT "
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (((msg.sourceMinor() == CMMsg.TYP_SHUTDOWN)
				|| ((msg.targetMinor() == CMMsg.TYP_EXPIRE) && (msg.target() instanceof Room)) || (msg
	...
	public void updateLot(List optPlayerList) {
		if (((System.currentTimeMillis() - lastCall) > 360000)
			...
			for (int v = 0; v < V.size(); v++) {
				...
				lastItemNums.put(R, Integer.valueOf(Prop_RoomForSale
						.updateLotWithThisData(R, this, false, false,
								optPlayerList, (lastItemNum == null) ? -1
										: lastItemNum.intValue())));
			}
			lastCall = System.currentTimeMillis();
			Area A = null;
			if (affected instanceof Area)
			...
			if ((A != null) && (lastDayDone != A.getTimeObj().getDayOfMonth())) {
				...
				if ((getOwnerName().length() > 0) && rentalProperty())
					if (Prop_RoomForSale.doRentalProperty(A, A.Name(),
