...
public class Prop_Artifact extends Property {
	...
	/**
	 * the purpose of this is to determine if the source of the call to this
	 * method includes the Destroy command. If so, we have a winner.
	 */
		...
		if ((affected != null) && (getItemID().length() > 0)) {
			try {
				...
				if (o.toString().indexOf("Commands.Destroy.execute") >= 0)
					CMLib.database().DBDeleteData(getItemID(), "ARTIFACTS",
							"ARTIFACTS/" + getItemID());
			} catch (Exception e) {
			}
		}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.target() != null)
				&& ((msg.target() == affected)
						...
						.target() == ((Item) affected).ultimateContainer(null)))) {
			if ((nomobs) && (msg.targetMinor() == CMMsg.TYP_GET)
					&& (msg.source().isMonster())) {
				msg.source().tell(
						"You are not allowed to possess " + affected.Name());
				return false;
			}
			if (nocast
				...
				if (room != null)
					room.showHappens(CMMsg.MSG_OK_VISUAL,
							"Magic energy fizzles around " + affected.Name()
									+ " and is absorbed into the air.");
				return false;
			} else if (nocast && (msg.tool() instanceof Ability)) {
				msg.source().tell(
						"That doesn't appear to work on " + affected.name());
				return false;
			}
		}
		...
		if ((msg.sourceMinor() == CMMsg.TYP_QUIT)
			...
			msg.source().tell(
					"^HYou lose your hold over " + affected.name() + "^?");
		...
		if (((msg.sourceMinor() == CMMsg.TYP_SHUTDOWN) || (msg.sourceMinor() == CMMsg.TYP_ROOMRESET))
			...
			if (msg.sourceMinor() == CMMsg.TYP_SHUTDOWN) {
				...
				if (autodrop) {
					if ((I.owner() instanceof MOB)
							&& ((!((MOB) I.owner()).isMonster()) || (CMLib
									.players().getPlayer(
											((MOB) I.owner()).Name()) != null)))
						R.moveItemTo(I);
				}
			} else {
				waitToReload = System.currentTimeMillis() + 60000;
			...
			if ((R != null) && (!I.amDestroyed())) {
				...
				if (I.owner() instanceof Room)
					data.append(CMLib.xml().convertXMLtoTag("ROOMID",
						...
						data.append(CMLib.xml()
								.convertXMLtoTag(
										...
										CMLib.map().getExtendedRoomID(
												M.getStartRoom())));
						data.append(CMLib.xml().convertXMLtoTag("MOB",
								((MOB) I.owner()).Name()));
					} else
						data.append(CMLib.xml().convertXMLtoTag("ROOMID",
								CMLib.map().getExtendedRoomID(R)));
				...
				data.append(CMLib.xml().convertXMLtoTag("ITEXT",
						CMLib.xml().parseOutAngleBrackets(I.text())));
				data.append("</ARTITEM>");
				I.destroy();
				CMLib.database().DBReCreateData(getItemID(), "ARTIFACTS",
						"ARTIFACTS/" + getItemID(), data.toString());
			}
		}
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((waitToReload > 0) && (tickID == Tickable.TICKID_ITEM_BOUNCEBACK)) {
			if (System.currentTimeMillis() > waitToReload) {
				waitToReload = 0;
				if ((affected instanceof Item) && (!affected.amDestroyed())
							...
							if ((iblk.tag.equalsIgnoreCase("ARTITEM"))
								...
								if (R != null) {
									...
									if (newItem != null) {
										...
										if (!registeredArtifacts
												.containsKey(getItemID()))
											registeredArtifacts.put(
													getItemID(), newItem);
										else
											Log.errOut("Prop_Artifact",
													"Possible duplicate artifact: "
															+ newItem.name());
										MOB foundMOB = null;
										if (MOBname.length() > 0)
											for (int i = 0; i < R
										...
										Item newItemMinusArtifact = (Item) newItem
												.copyOf();
										Ability A2 = newItemMinusArtifact
												.fetchEffect(ID());
										if (A2 != null)
											newItemMinusArtifact.delEffect(A2);
										Item I = null;
										if (foundMOB != null) {
											for (int i = 0; i < foundMOB
													.numItems(); i++) {
												I = foundMOB.getItem(i);
												if (I == null)
													break;
												if (I.Name().equals(
														newItemMinusArtifact
																.Name())) {
													I = (Item) I.copyOf();
													A2 = I.fetchEffect(ID());
													if (A2 != null)
														I.delEffect(A2);
													if (newItemMinusArtifact
															.sameAs(I))
														I.destroy();
												}
											}
													...
													"Unable to reset: "
															...
															+ CMLib.map()
																	.getExtendedRoomID(
																			R));
											waitToReload = System
													.currentTimeMillis() + 10 * 60000;
											return true;
										}
									}
								} else
									Log.errOut("Prop_Artifact",
											"Unknown artifact room artifact: "
													+ roomID + " for "
													+ getItemID());
							}
						}
			...
			Log.errOut("Prop_Artifact", "No Artifact record for: "
					+ getItemID());
			return true;
		}
