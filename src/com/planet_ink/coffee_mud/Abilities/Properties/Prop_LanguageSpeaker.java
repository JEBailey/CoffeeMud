...
public class Prop_LanguageSpeaker extends Property {
	...
	public void setMiscText(String txt) {
		...
		if ((x >= 0) && (txt.substring(x + 1).trim().length() > 0))
			mobMask = CMLib.masking().getPreCompiledMask(
					txt.substring(x + 1).trim());
		lang = null;
	...
	public String accountForYourself() {
		return "Forces speaking the language: "
				+ ((lang != null) ? lang.name() : "?");
	}

	public void startSpeaking(MOB mob) {
		Room mobHomeRoom = mob.getStartRoom();
		Area mobHomeArea = ((mobHomeRoom == null) ? null : mobHomeRoom
				.getArea());
		if (((lang != null) || (langStr.length() > 0))
				...
				&& ((mobMask == null) || CMLib.masking().maskCheck(mobMask,
						mob, true))) {
			if (lang == null)
				lang = getLanguage();
			if (lang == null) {
				lang = (Language) CMClass.getAbility("Common");
				Log.errOut("Prop_LanguageSpeaker", "Unknown language "
						+ langStr);
			}
			if (lang != null) {
				switch (affectedType) {
