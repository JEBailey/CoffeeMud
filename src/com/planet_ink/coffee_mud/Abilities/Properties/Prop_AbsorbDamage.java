...
public class Prop_AbsorbDamage extends Property implements TriggeredAffect {
	...
	public String accountForYourself() {
		String id = "Absorbs damage of the following amount and types: "
				+ text();
		return id;
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected != null) && (msg.targetMinor() == CMMsg.TYP_DAMAGE)
			...
			if ((x < 0) && (msg.tool() instanceof Weapon)) {
				...
				if (x < 0)
					x = (CMLib.flags().isABonusItems(W)) ? text
							.indexOf("MAGIC") : -1;
				if (x < 0)
				...
				if (lastNumber >= 0) {
					...
					if (text.endsWith("%"))
						msg.setValue(msg.value()
								- (int) Math.round(CMath.mul(msg.value(), CMath
										.div(CMath.s_int(text.substring(0,
												text.length() - 1)), 100.0))));
					else
