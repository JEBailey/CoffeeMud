...
public class Prop_TattooAdder extends Property {
	...
	protected int canAffectCode() {
		return Ability.CAN_ITEMS | Ability.CAN_ROOMS | Ability.CAN_AREAS
				| Ability.CAN_MOBS | Ability.CAN_EXITS;
	}
			...
			if (tattooMinus) {
				if (!silent)
					mob.location().show(
							...
							affected,
							CMMsg.MSG_OK_ACTION,
							"<T-NAME> takes away the "
									+ pT.tattooName.toLowerCase()
									+ " tattoo from <S-NAME>.");
				mob.delTattoo(T);
			}
		} else {
			if (tattooPlus) {
				if (!silent)
					mob.location().show(
							...
							affected,
							CMMsg.MSG_OK_ACTION,
							"<T-NAME> gives <S-NAME> the "
									+ pT.tattooName.toLowerCase() + " tattoo.");
				mob.addTattoo(pT);
			}
		}
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((tattooCode() == CMMsg.TYP_DEATH)
				&& (msg.sourceMinor() == tattooCode())) {
			if ((msg.tool() == affected) && (msg.source() != affected))
				applyTattooCodes(msg.source(), false, true);
			else if ((msg.source() == affected) && (msg.tool() instanceof MOB)
					&& (msg.tool() != affected))
