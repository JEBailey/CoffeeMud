				...
				&& ((msg.source().location() == affected)
						|| ((affected instanceof Area) && (((Area) affected)
								.inMyMetroArea(msg.source().location()
										.getArea()))) || (msg.target() == affected))
				&& (!CMSecurity.isAllowed(msg.source(),
						msg.source().location(), CMSecurity.SecFlag.CMDMOBS))) {
			if (affected instanceof MOB)
				msg.source()
						.tell("You don't feel very commanding around here.");
			else
				msg.source().tell(msg.source(), msg.target(), null,
						"<T-NAME> isn't paying any attention to you.");
			return false;
		}
		return true;
	}
