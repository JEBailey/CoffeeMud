	...
	public void setMiscText(String newStr) {
		if (newStr.startsWith("+")) {
			...
			if (x >= 0) {
				...
				super.setMiscText(text().substring(0, x)
						+ text().substring(x + len));
				smells = null;
			} else
				return;
		} else {
		...
		for (int i = 0; i < allsmells.size(); i++) {
			...
			if (smell.length() > 0) {
				...
				for (int ii = parsedSmell.size() - 1; ii >= 0; ii--) {
					String s = ((String) parsedSmell.elementAt(ii))
							.toUpperCase();
					...
					if (s.startsWith("CHANCE=")) {
						pct = (pct & (FLAG_BROADCAST + FLAG_EMOTE))
								+ CMath.s_int(s.substring(5).trim());
						parsedSmell.removeElementAt(ii);
					}
					if (s.equals("EMOTE")) {
						pct = pct & FLAG_EMOTE;
				...
				if (finalSmell.length() > 0)
					smells.addElement(finalSmell, Integer.valueOf(pct),
		...
		if ((affected instanceof MOB) && (CMLib.dice().rollPercentage() <= 20)) {
			...
			for (int i = sm.size() - 1; i >= 0; i--) {
				if (((Integer) sm.elementAt(i, 3)).intValue() > 0) {
					Integer I = Integer.valueOf(((Integer) smells.elementAt(i,
							3)).intValue() - 1);
					if (I.intValue() > 0) {
