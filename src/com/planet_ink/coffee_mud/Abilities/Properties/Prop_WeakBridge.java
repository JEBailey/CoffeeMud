...
public class Prop_WeakBridge extends Property implements TriggeredAffect {
					...
					synchronized (mobsToKill) {
						if (!mobsToKill.contains(mob)) {
							...
							CMLib.threads().startTickDown(this,
									Tickable.TICKID_SPELL_AFFECT, 1);
						}
					}
				}
			}
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((affected != null) && (affected instanceof Room) && (bridgeIsUp))
			...
			if (bridgeIsUp) {
				synchronized (mobsToKill) {
					...
					if (affected instanceof Room) {
						...
						for (int i = 0; i < room.numInhabitants(); i++) {
							MOB M = room.fetchInhabitant(i);
							if ((M != null) && (!CMLib.flags().isInFlight(M))
									&& (!V.contains(M)))
								V.add(M);
						}
					}
					for (int i = 0; i < V.size(); i++) {
						...
						if ((mob.location() != null)
							...
							if ((affected instanceof Room)
									&& ((((Room) affected).domainType() == Room.DOMAIN_INDOORS_AIR) || (((Room) affected)
								...
								mob.location().showSource(mob, null,
										CMMsg.MSG_OK_VISUAL,
										"The bridge breaks under your weight!");
								mob.location()
										.show(mob, null, CMMsg.MSG_OK_VISUAL,
												"<S-NAME> fall(s) to <S-HIS-HER> death!!");
								mob.location().show(mob, null, CMMsg.MSG_DEATH,
										null);
							}
						}
					}
					...
					CMLib.threads().deleteTick(this,
							Tickable.TICKID_SPELL_AFFECT);
					CMLib.threads().startTickDown(this,
							Tickable.TICKID_SPELL_AFFECT, ticksDown);
				}
			} else {
				bridgeIsUp = true;
