...
public class Prop_ClosedDayNight extends Property {
	...
	protected boolean closed(Environmental E) {
		...
		if (R == null)
			R = ((exitRoom == null) ? (Room) CMLib.map().rooms().nextElement()
					: exitRoom);
		if (R == null)
			return false;
	...
	public boolean okMessage(Environmental E, CMMsg msg) {
		...
		if ((affected != null)
				...
				&& ((msg.targetMinor() == CMMsg.TYP_BUY)
						...
						|| (msg.targetMinor() == CMMsg.TYP_BORROW) || (msg
						.targetMinor() == CMMsg.TYP_VIEW))) {
			...
			if (sk != null)
				CMLib.commands()
						.postSay(
								(MOB) affected,
								msg.source(),
								(shopMsg != null) ? shopMsg
										: "Sorry, I'm off right now.  Try me tomorrow.",
								false, false);
			return false;
		}
		return true;
	...
	protected Room getHomeRoom() {
		...
		if ((affected != null) && (affected instanceof MOB)) {
			...
			if (mob.location() != null) {
				...
				for (Iterator<Room> r = checkSet.iterator(); r.hasNext();) {
					...
					if ((R2.roomID().indexOf(Home) >= 0)
							...
							|| CMLib.english().containsString(R2.displayText(),
									Home)
							|| CMLib.english().containsString(R2.description(),
									Home)) {
						R = R2;
						break;
					}
			...
			if (R != null)
				return R;
			try {
				List<Room> rooms = CMLib.map().findRooms(CMLib.map().rooms(),
						mob, Home, false, 10);
				if (rooms.size() > 0)
					R = rooms.get(CMLib.dice().roll(1, rooms.size(), -1));
				else {
					List<MOB> inhabs = CMLib.map().findInhabitants(
							CMLib.map().rooms(), mob, Home, 10);
					if (inhabs.size() > 0)
						R = CMLib.map().roomLocation(
								inhabs.get(CMLib.dice().roll(1, inhabs.size(),
										-1)));
				}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((affected != null)
			...
			if (closed(affected)) {
				...
				if ((mob.location() == mob.getStartRoom()) && (lockupFlag))
					for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
						Exit E = mob.location().getExitInDir(d);
						Room R2 = mob.location().getRoomInDir(d);
						if ((E != null) && (R2 != null) && (E.hasADoor())
								&& (E.hasALock())) {
							CMMsg msg = null;
							if (E.isOpen()) {
