...
public class Prop_WearEnabler extends Prop_HaveEnabler {
	...
	public boolean addMeIfNeccessary(Environmental source,
			Environmental target, boolean makeLongLasting, short maxTicks) {
		if (disabled && checked)
			return false;
	...
	public void affectPhyStats(Physical host, PhyStats affectableStats) {
		...
		if (host instanceof Item) {
			...
			boolean worn = (!myItem.amWearingAt(Wearable.IN_INVENTORY))
					&& ((!myItem.amWearingAt(Wearable.WORN_FLOATING_NEARBY)) || (myItem
			...
			if ((lastMOB == null) && (worn) && (myItem.owner() != null)
