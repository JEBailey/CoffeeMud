	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((affected != null)
			...
			if ((!msg.source().isMonster())
					&& (!CMath.bset(msg.source().getBitmap(),
							MOB.ATT_PLAYERKILL))) {
				msg.source()
						.tell("You must have your playerkill flag set to enter here.");
				return false;
			}
		}
		if ((!msg.source().isMonster())
			...
			if ((R != null)
					&& ((R == affected) || (R.getArea() == affected) || ((affected instanceof Area) && (((Area) affected)
							.inMyMetroArea(R.getArea()))))) {
				msg.source().tell("Your PLAYERKILL flag is now ON!");
				msg.source()
						.setBitmap(
								CMath.setb(msg.source().getBitmap(),
										MOB.ATT_PLAYERKILL));
			}
		}
