...
public class Prop_HaveAdjuster extends Property implements TriggeredAffect {
	...
	public boolean addIfPlussed(String newText, String parm, int parmCode,
			Vector addTo) {
	...
	public void setMiscText(String newText) {
		...
		for (int i : CharStats.CODES.BASE()) {
			String name = CMStrings.limit(
					CharStats.CODES.NAME(i).toLowerCase(), 3);
			addIfPlussed(parameters[0], name, i, charStatsV);
			addIfPlussed(parameters[0], "max" + name,
					CharStats.CODES.toMAXBASE(i), charStatsV);
		}
		...
		for (int c : CharStats.CODES.SAVING_THROWS())
			if (CMMSGMAP[c] != -1)
				addIfPlussed(
						parameters[0],
						"save"
								+ CMStrings.limit(CharStats.CODES.NAME(c)
										.toLowerCase(), 3), c, charStatsV);
	...
	public void phyStuff(Object[] changes, PhyStats phyStats) {
		...
		for (int c = 0; c < changes.length; c += 2)
			...
			case PhyStats.STAT_ABILITY:
				phyStats.setAbility(phyStats.ability()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case PhyStats.STAT_ARMOR:
				phyStats.setArmor(phyStats.armor()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case PhyStats.STAT_ATTACK:
				phyStats.setAttackAdjustment(phyStats.attackAdjustment()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case PhyStats.STAT_DAMAGE:
				phyStats.setDamage(phyStats.damage()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			...
			case PhyStats.STAT_LEVEL: {
				phyStats.setLevel(phyStats.level()
						+ ((Integer) changes[c + 1]).intValue());
				if (phyStats.level() < 0)
					phyStats.setLevel(0);
				break;
			...
			case PhyStats.STAT_REJUV:
				phyStats.setRejuv(phyStats.rejuv()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case PhyStats.STAT_SENSES:
				phyStats.setSensesMask(phyStats.sensesMask()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case PhyStats.STAT_WEIGHT:
				phyStats.setWeight(phyStats.weight()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case PhyStats.STAT_HEIGHT:
				phyStats.setHeight(phyStats.height()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case PhyStats.NUM_STATS:
				phyStats.setSpeed(phyStats.speed()
						+ ((Double) changes[c + 1]).doubleValue());
				break;
			}
	}
	...
	public boolean canApply(MOB mob) {
		if ((affected != null)
				...
				&& ((mask == null) || (CMLib.masking().maskCheck(mask, mob,
						true))))
			return true;
		return false;
	}
	...
	public void adjCharStats(Object[] changes, CharStats charStats) {
		...
		for (int i = 0; i < changes.length; i += 2) {
			if (changes[i] instanceof Integer)
				...
				case 'G':
					charStats.setStat(CharStats.STAT_GENDER,
							((Character) changes[i + 1]).charValue());
					break;
				case 'C':
	...
	public void adjCharState(Object[] changes, CharState charState) {
		...
		for (int c = 0; c < changes.length; c += 2)
			...
			case CharState.STAT_HITPOINTS:
				charState.setHitPoints(charState.getHitPoints()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case CharState.STAT_HUNGER:
				charState.setHunger(charState.getHunger()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case CharState.STAT_THIRST:
				charState.setThirst(charState.getThirst()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case CharState.STAT_MANA:
				charState.setMana(charState.getMana()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			case CharState.STAT_MOVE:
				charState.setMovement(charState.getMovement()
						+ ((Integer) changes[c + 1]).intValue());
				break;
			}
	}
	...
	public static final String fixAccoutingsWithMask(String parameters,
			final String mask) {
		int x = parameters.toUpperCase().indexOf("ARM");
		for (StringBuffer ID = new StringBuffer(parameters); ((x > 0) && (x < parameters
				.length())); x++)
		...
		if (x >= 0) {
			...
			if ((val != 0) && (y > x)) {
				...
				parameters = parameters.substring(0, x)
						+ middle.toString().trim()
						+ parameters.substring(y + (("" + val).length()));
			}
		}
	...
	public String accountForYourself() {
		return fixAccoutingsWithMask("Affects the owner: " + parameters[0],
				parameters[1]);
	}
}
