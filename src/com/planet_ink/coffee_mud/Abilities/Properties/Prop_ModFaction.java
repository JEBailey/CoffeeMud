...
public class Prop_ModFaction extends Property {
	...
	public String accountForYourself() {
		final Faction F = (factionID.length() > 0) ? CMLib.factions()
				.getFaction(factionID) : null;
		String gainOrLoss = (gainonly) ? "gained " : lossonly ? "lost " : "";
		final String factionName = (factionID.length() == 0) ? "any faction"
				: reactions ? "certain factions"
						: ((F == null) ? "some faction" : F.name());
		return "Modifies " + gainOrLoss + "faction with " + factionName + ": "
				+ operationFormula;
	}
	...
	public int translateAmount(int amount, String val) {
		if (amount < 0)
			amount = -amount;
		if (val.endsWith("%"))
			return (int) Math.round(CMath.mul(amount, CMath.div(
					CMath.s_int(val.substring(0, val.length() - 1)), 100)));
	...
	public void setMiscText(String newText) {
		...
		if (x >= 0) {
			mask = CMLib.masking()
					.getPreCompiledMask(s.substring(x + 1).trim());
		...
		if (s.startsWith("="))
			operation = CMath.compileMathExpression(translateNumber(
					s.substring(1)).trim());
		else if (s.startsWith("+"))
			operation = CMath.compileMathExpression("@x1 + "
					+ translateNumber(s.substring(1)).trim());
		else if (s.startsWith("-"))
			operation = CMath.compileMathExpression("@x1 - "
					+ translateNumber(s.substring(1)).trim());
		else if (s.startsWith("*"))
		...
		operationFormula = CMStrings.replaceAll(operationFormula, "@x1",
				"Amount");
	}
		...
		if ((msg.sourceMinor() == CMMsg.TYP_FACTIONCHANGE)
				...
				&& (((msg.target() == affected) && (affected instanceof MOB))
						|| ((affected instanceof Item)
								&& (msg.source() == ((Item) affected).owner()) && (!((Item) affected)
									.amWearingAt(Wearable.IN_INVENTORY)))
						|| (affected instanceof Room) || (affected instanceof Area))
				&& (msg.value() != Integer.MAX_VALUE)
				&& (msg.value() != Integer.MIN_VALUE)
				&& ((!gainonly) || (msg.value() > 0))
				&& ((!lossonly) || (msg.value() < 0))
				&& ((factionID.length() == 0) || (msg.othersMessage()
						.equalsIgnoreCase(factionID)))) {
			if (reactions) {
			...
			if (mask != null) {
				if (affected instanceof Item) {
					if ((msg.target() == null)
							...
							|| (!CMLib.masking().maskCheck(mask, msg.target(),
									true)))
						return super.okMessage(myHost, msg);
				} else if (!CMLib.masking().maskCheck(mask, msg.source(), true))
					return super.okMessage(myHost, msg);
			}
			msg.setValue((int) Math.round(CMath.parseMathExpression(operation,
					new double[] { msg.value() }, 0.0)));
		}
		return super.okMessage(myHost, msg);
	}
}
