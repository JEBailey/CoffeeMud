...
public class Prop_ReqCapacity extends Property implements TriggeredAffect {
	...
	public String accountForYourself() {
		return "Person limit: "
				+ ((peopleCap == Integer.MAX_VALUE) ? "None" : ("" + peopleCap))
				+ "\n\rPlayer limit: "
				+ ((playerCap == Integer.MAX_VALUE) ? "None" : ("" + playerCap))
				+ "\n\rMOB limit   : "
				+ ((mobCap == Integer.MAX_VALUE) ? "None" : ("" + mobCap))
				+ "\n\rItem limit  : "
				+ ((itemCap == Integer.MAX_VALUE) ? "None" : ("" + itemCap))
				+ "\n\rWeight limit: "
				+ ((maxWeight == Integer.MAX_VALUE) ? "None" : ("" + maxWeight));
	}
	...
	public void setMiscText(String txt) {
		...
		if (txt.length() == 0)
			...
			containersOk = CMParms.getParmBool(txt, "droponly", containersOk)
					|| CMParms.getParmBool(txt, "containersok", containersOk);
		}
	}
		...
		if (affected != null)
			...
			case CMMsg.TYP_ENTER:
				if ((msg.target() instanceof Room)
						&& (peopleCap < Integer.MAX_VALUE)
						&& ((!indoorOnly) || ((((Room) msg.target())
								.domainType() & Room.INDOORS) == Room.INDOORS))
						&& ((msg.amITarget(affected))
								|| (msg.tool() == affected) || (affected instanceof Area))) {
					...
					if (msg.source().isMonster()
							&& (((Room) msg.target()).numInhabitants() - ((Room) msg
									.target()).numPCInhabitants()) >= mobCap) {
			...
			case CMMsg.TYP_DROP:
				if ((msg.target() instanceof Item)
						...
						&& ((!msg.targetMajor(CMMsg.MASK_INTERMSG)) || (!containersOk))) // intermsgs
																							// are
																							// PUTs
																							// on
																							// the
																							// ground
				{
					...
					if ((!indoorOnly)
						...
						if (itemCap < Integer.MAX_VALUE) {
							...
							if (soFar >= itemCap) {
								msg.source().tell(
										"There is no more room in here to drop "
												+ msg.target().Name() + ".");
								if ((rawResources > 0)
										&& (CMath.div(rawResources, itemCap) > 0.5))
									msg.source()
											.tell("You should consider bundling up some of those resources.");
								return false;
							}
						}
						if (maxWeight < Integer.MAX_VALUE) {
							...
							if ((soFar + targetI.phyStats().weight()) >= maxWeight) {
								msg.source().tell(
										"There is no room in here to put "
												+ targetI.Name() + ".");
								return false;
							}
						}
