...
public class Prop_AddDamage extends Property implements TriggeredAffect {
	...
	public String accountForYourself() {
		String id = "Does extra damage of the following amount and types: "
				+ text();
		return id;
	}
		...
		for (String s : parms) {
			...
			if (CMath.isPct(s))
				...
				if ((!done) && (!s.equals("ALL")))
					Log.errOut(
							"Prop_AddDamage",
							"Unknown weapon type/attack: "
									+ s
									+ " in "
									+ CMLib.map().getExtendedRoomID(
											CMLib.map().roomLocation(affected)));
			}
		}
	}

	protected final int getDamage(final CMMsg msg) {
		final int dmg = (int) CMath.round(CMath.mul(msg.value(), pctDamage))
				+ bonusDamage;
		if (dmg < 0)
			return 0;
		return dmg;
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (affected instanceof Item) {
			...
			return;
		if ((msg.source() != null) && (msg.targetMinor() == CMMsg.TYP_DAMAGE)
				&& (msg.value() > 0) && (msg.target() instanceof MOB)
				&& (!((MOB) msg.target()).amDead()) && (msg.tool() != this)
				&& (msg.source().location() != null)) {
			if (((affected instanceof Armor) || (affected instanceof Shield))
					&& (msg.amITarget(mob))
					&& (!msg.amISource(mob))
					&& (CMLib.dice().rollPercentage() > 32 + msg.source()
						...
						CMLib.combat()
								.postDamage(
										...
										CMMsg.MASK_MALICIOUS
												| CMMsg.MASK_ALWAYS
												| typeOfEffect, weaponType,
										"^F^<FIGHT^><S-YOUPOSS> <O-NAME> <DAMAGE> <T-NAME>!^</FIGHT^>^?");
					}
				}
			} else if ((msg.tool() == affected) && (!msg.amITarget(mob))
					&& (msg.amISource(mob)) && (!(msg.tool() instanceof Wand))) {
				final int damage = getDamage(msg);
						...
						try {
							CMLib.combat().postDamage(
									...
									CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS
											| typeOfEffect, weaponType, str);
						} finally {
							norecurse = false;
						}
					}
				}
			} else if ((mob == affected) && (!msg.amITarget(mob))
					&& (msg.amISource(mob)) && (msg.tool() instanceof Weapon)
