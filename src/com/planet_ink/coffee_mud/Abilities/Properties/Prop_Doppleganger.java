...
public class Prop_Doppleganger extends Property {
	...
	public void setMiscText(String text) {
		...
		if (CMath.isInteger(text))
			...
			matchPlayersFollowersOnly = CMParms.getParmBool(text,
					"PLAYERSNFOLS", false);
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((affected instanceof Item)
				&& ((((Item) affected).owner() != lastOwner) || ((lastOwner != null) && (lastOwner
						.phyStats().level() != lastLevel)))
				&& (((Item) affected).owner() instanceof MOB)) {
			lastOwner = ((Item) affected).owner();
			lastLevel = lastOwner.phyStats().level();
			int level = (int) Math.round(CMath.mul(((MOB) lastOwner).phyStats()
					.level(), levelPct))
					+ levelAdd;
			if (level < minLevel)
				level = minLevel;
			...
			((Item) affected).basePhyStats().setLevel(
					((MOB) lastOwner).phyStats().level());
			((Item) affected).phyStats().setLevel(
					((MOB) lastOwner).phyStats().level());
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((affected instanceof MOB)
				&& (((msg.target() instanceof Room) && (msg.sourceMinor() == CMMsg.TYP_ENTER)) || (msg
						.sourceMinor() == CMMsg.TYP_LIFE)))
		// &&(lastLevelChangers))
		{
			// lastLevelChangers=false;
			MOB mob = (MOB) affected;
			Room R = (msg.target() instanceof Room) ? ((Room) msg.target())
					: msg.source().location();
			if ((R != null)
					&& (CMLib.flags().aliveAwakeMobile(mob, true))
					&& (mob.curState().getHitPoints() >= mob.maxState()
							.getHitPoints())) {
				int total = 0;
				int num = 0;
				...
				if (num > 0) {
					int level = (int) Math.round(CMath.mul(
							CMath.div(total, num), levelPct))
							+ levelAdd;
					if (level < minLevel)
						level = minLevel;
					if (level > maxLevel)
					...
					if (level != mob.basePhyStats().level()) {
						...
						mob.basePhyStats().setArmor(
								CMLib.leveler().getLevelMOBArmor(mob));
						mob.basePhyStats().setAttackAdjustment(
								CMLib.leveler().getLevelAttack(mob));
						mob.basePhyStats().setDamage(
								CMLib.leveler().getLevelMOBDamage(mob));
						mob.basePhyStats().setSpeed(
								1.0 + (CMath.div(level, 100) * 4.0));
						mob.baseState().setHitPoints(
								CMLib.leveler().getPlayerHitPoints(mob));
						mob.baseState().setMana(
								CMLib.leveler().getLevelMana(mob));
						mob.baseState().setMovement(
								CMLib.leveler().getLevelMove(mob));
						mob.recoverPhyStats();
