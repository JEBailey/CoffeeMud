...
public class Prop_RoomForSale extends Property implements LandTitle {
	...
	public void setPrice(int price) {
		setMiscText(getOwnerName() + "/" + (rentalProperty() ? "RENTAL " : "")
	...
	public void setOwnerName(String owner) {
		if ((owner.length() == 0) && (getOwnerName().length() > 0))
			scheduleReset = true;
		setMiscText(owner + "/" + (rentalProperty() ? "RENTAL " : "")
				+ ((backTaxes() != 0) ? "TAX" + backTaxes() + "X " : "")
				+ getPrice());
	}
	...
	public void setRentalProperty(boolean truefalse) {
		setMiscText(getOwnerName() + "/" + (truefalse ? "RENTAL " : "")
				+ ((backTaxes() != 0) ? "TAX" + backTaxes() + "X " : "")
				+ getPrice());
	}
	...
	public String getTitleID() {
		if (affected instanceof Room)
			return "LAND_TITLE_FOR#"
					+ CMLib.map().getExtendedRoomID((Room) affected);
		else {
			Room R = CMLib.map().getRoom(landPropertyID());
			if (R != null)
	...
	public static boolean shopkeeperMobPresent(Room R) {
		...
		for (int i = 0; i < R.numInhabitants(); i++) {
			...
			if ((M.getStartRoom() == R) && (M.isMonster())
	...
	public static boolean robberyCheck(LandTitle A, CMMsg msg) {
		if ((msg.targetMinor() == CMMsg.TYP_GET)
			...
			if ((msg.target() instanceof Item)
					&& (((Item) msg.target()).owner() == msg.source()
							.location())
					&& ((!(msg.tool() instanceof Item)) || (msg.source()
							.isMine(msg.tool())))
				...
				if (B != null) {
					...
					if (D == null)
						return true;
					B.accuse(CMLib.law().getLegalObject(R), msg.source(), D,
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (((msg.sourceMinor() == CMMsg.TYP_SHUTDOWN)
				|| ((msg.targetMinor() == CMMsg.TYP_EXPIRE) && (msg.target() == affected)) || (msg
				...
				synchronized (("SYNC" + R.roomID()).intern()) {
					...
					for (int m = 0; m < R.numInhabitants(); m++) {
						...
						if ((M != null)
								&& (M.isSavable())
								&& (M.getStartRoom() == R)
								&& ((M.basePhyStats().rejuv() == 0) || (M
										.basePhyStats().rejuv() == PhyStats.NO_REJUV))) {
	...
	public static void colorForSale(Room R, boolean rental, boolean reset) {
		synchronized (("SYNC" + R.roomID()).intern()) {
			...
			if ((x < 0)
					|| (reset && (!R
							.displayText()
							.equals(CMath.bset(R.domainType(), Room.INDOORS) ? INDOORSTR
									: OUTDOORSTR)))) {
				if (reset) {
					R.setDescription("");
					R.setDisplayText(CMath.bset(R.domainType(), Room.INDOORS) ? INDOORSTR
							: OUTDOORSTR);
					x = -1;
				}
				if (x < 0)
					...
					R.setDescription(R.description().substring(0,
							x + theStr.trim().length()));
			...
			if ((I == null) || (!I.ID().equals("GenWallpaper"))) {
				...
				I.setName("id");
				I.setReadableText("This room is "
						+ CMLib.map().getExtendedRoomID(R));
				I.setDescription("This room is "
						+ CMLib.map().getExtendedRoomID(R));
				R.addItem(I);
	...
	public static int updateLotWithThisData(Room R, LandTitle T,
			boolean resetRoomName, boolean clearAllItems, List optPlayerList,
			int lastNumItems) {
		...
		synchronized (("SYNC" + R.roomID()).intern()) {
			...
			if (T.getOwnerName().length() == 0) {
				...
				for (int i = R.numItems() - 1; i >= 0; i--) {
					...
					if (clearAllItems) {
						...
						if (I.expirationDate() == 0) {
							...
							now += (TimeManager.MILI_MINUTE * CMProps
									.getIntVar(CMProps.Int.EXPIRE_PLAYER_DROP));
				...
				for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
					...
					if ((E != null) && (E.hasALock()) && (E.isGeneric())) {
						E.setKeyName("");
						E.setDoorsNLocks(E.hasADoor(), E.isOpen(),
								E.defaultsClosed(), false, false, false);
						updateExits = true;
						if (R2 != null) {
							E = R2.getRawExit(Directions.getOpDirectionCode(d));
							if ((E != null) && (E.hasALock())
									&& (E.isGeneric())) {
								E.setKeyName("");
								E.setDoorsNLocks(E.hasADoor(), E.isOpen(),
										E.defaultsClosed(), false, false, false);
			...
			if ((lastNumItems < 0)
					&& (!CMSecurity
							.isDisabled(CMSecurity.DisFlag.PROPERTYOWNERCHECKS))
					&& (optPlayerList != null)) {
				boolean playerExists = (CMLib.players().getPlayer(
						T.getOwnerName()) != null);
				if (!playerExists)
				...
				if (!playerExists)
					for (int i = 0; i < optPlayerList.size(); i++)
						if (((String) optPlayerList.get(i)).equalsIgnoreCase(T
								.getOwnerName())) {
							playerExists = true;
							break;
						}
			...
			// 1. if an item has ONLY been removed, the lastNumItems will be !=
			// current # items
			// 2. if an item has ONLY been added, the dispossessiontime will be
			// != null
			// 3. if an item has been added AND removed, the dispossession time
			// will be != null on the added
			...
			for (int i = 0; i < R.numItems(); i++) {
				...
				if ((I.expirationDate() != 0)
						...
						&& ((!(I instanceof DeadBody)) || (((DeadBody) I)
								.playerCorpse()))) {
			...
			if ((!CMSecurity.isSaveFlag("NOPROPERTYITEMS")) && (updateItems))
				CMLib.database().DBUpdateItems(R);
		}
		return lastNumItems;
	}

	public static boolean doRentalProperty(Area A, String ID, String owner,
			int rent) {
		...
		if (pDataV.size() == 0) {
			...
			CMLib.database().DBCreateData(owner, "RENTAL INFO",
					"RENTAL INFO/" + owner, pData.xml);
			pDataV.add(pData);
			...
			while (x >= 0) {
				...
				if (thisOne.startsWith(ID + "|~>|")) {
					...
					if (dateV.size() == 3) {
						int lastYear = CMath
								.s_int((String) dateV.lastElement());
						int lastMonth = CMath
								.s_int((String) dateV.elementAt(1));
						int lastDay = CMath
								.s_int((String) dateV.firstElement());
						while (!needsToPay) {
							if (lastYear < year)
								needsToPay = true;
							else if ((lastYear == year) && (lastMonth < month)
									&& (day >= lastDay))
								needsToPay = true;
							if (needsToPay) {
								if (CMLib.beanCounter().modifyLocalBankGold(
										A,
										owner,
			...
			if (changesMade) {
				CMLib.database().DBReCreateData(owner, "RENTAL INFO",
						"RENTAL INFO/" + owner, reparse.toString());
				pData = new DatabaseEngine.PlayerData();
				pData.who = owner;
				pData.section = "RENTAL INFO";
				pData.key = "RENTAL INFO/" + owner;
	...
	public void updateLot(List optPlayerList) {
		if (affected instanceof Room) {
			...
			synchronized (("SYNC" + R.roomID()).intern()) {
				R = CMLib.map().getRoom(R);
				lastItemNums = updateLotWithThisData(R, this, false,
						scheduleReset, optPlayerList, lastItemNums);
				if ((lastDayDone != R.getArea().getTimeObj().getDayOfMonth())
					...
					if ((getOwnerName().length() > 0) && rentalProperty()
							&& (R.roomID().length() > 0))
						if (doRentalProperty(R.getArea(), R.roomID(),
								getOwnerName(), getPrice())) {
							setOwnerName("");
							CMLib.database().DBUpdateRoom(R);
							lastItemNums = updateLotWithThisData(R, this,
									false, scheduleReset, optPlayerList,
									lastItemNums);
						}
				}
				scheduleReset = false;
