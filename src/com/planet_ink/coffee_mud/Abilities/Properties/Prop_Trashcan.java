...
public class Prop_Trashcan extends Property {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (tickID == Tickable.TICKID_PROPERTY_SPECIAL) {
			synchronized (trashables) {
				if ((System.currentTimeMillis() - lastAddition) < ((tickDelay - 1) * CMProps
						.getTickMillis()))
					return true;
				...
				CMLib.threads().deleteTick(this,
						Tickable.TICKID_PROPERTY_SPECIAL);
			}
			return false;
		}
		return true;
			...
			synchronized (trashables) {
				if (lastAddition == 0) {
					...
					CMLib.threads().startTickDown(this,
							Tickable.TICKID_PROPERTY_SPECIAL, tickDelay);
				}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected instanceof Item) && (msg.targetMinor() == CMMsg.TYP_PUT)
				&& (msg.amITarget(affected)) && (msg.tool() != null)
