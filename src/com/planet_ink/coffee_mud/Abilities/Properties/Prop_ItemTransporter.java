...
public class Prop_ItemTransporter extends Property {
	...
	private synchronized boolean setDestination() {
		...
		if ((possibilities == null) || (lastLook == null)
				|| (lastLook.intValue() < 0)) {
		...
		if (possibilities.size() > 0) {
			PhysicalAgent P = possibilities.get(CMLib.dice().roll(1,
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (((msg.amITarget(affected))
				&& ((msg.targetMinor() == CMMsg.TYP_PUT) || (msg.targetMinor() == CMMsg.TYP_GIVE)) && (msg
					.tool() instanceof Item))
				|| ((affected instanceof MOB)
						&& (msg.amISource((MOB) affected))
						&& ((msg.targetMinor() == CMMsg.TYP_GET)
								|| (msg.targetMinor() == CMMsg.TYP_PUSH) || (msg
								.targetMinor() == CMMsg.TYP_PULL)) && (msg
							.target() instanceof Item))
				|| ((affected instanceof Room)
						&& (msg.targetMinor() == CMMsg.TYP_DROP) && (msg
							.target() instanceof Item))
				|| ((affected instanceof Room)
						&& (msg.sourceMinor() == CMMsg.TYP_THROW)
						&& (affected == CMLib.map().roomLocation(msg.target())) && (msg
							.tool() instanceof Item))) {
			if (!setDestination()) {
				msg.source().tell(
						"The transporter has no possible ItemTransReceiver with the code '"
								+ text() + "'.");
				return false;
			}
		}
		...
		if ((mobDestination != null) || (roomDestination != null)) {
			...
			if (affected instanceof Room)
				...
				if ((container.owner() != null)
						&& (container.owner() instanceof Room))
					roomMover = (Room) container.owner();
				else if ((container.owner() != null)
						&& (container.owner() instanceof MOB))
				...
				if (mob != null)
					for (int i = 0; i < itemsToMove.size(); i++) {
						...
						if (mob instanceof ShopKeeper)
							((ShopKeeper) mob).getShop()
									.addStoreInventory(item);
						else
							mob.addItem(item);
					}
				if (room != null)
