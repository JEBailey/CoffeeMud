...
public class Specialization_Weapon extends StdAbility {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((activated) && (msg.source() == affected)
			...
			if ((w.weaponClassification() == weaponClass) || (weaponClass < 0)
					|| (w.weaponClassification() == secondWeaponClass)) {
				if ((msg.targetMinor() == CMMsg.TYP_WEAPONATTACK)
						&& (CMLib.dice().rollPercentage() < 10))
					helpProficiency((MOB) affected, 0);
				else if ((msg.targetMinor() == CMMsg.TYP_DAMAGE)
						&& ((w.weaponClassification() == Weapon.CLASS_NATURAL) || (!w
								.amWearingAt(Wearable.IN_INVENTORY)))
						&& (msg.value() > 0))
					msg.setValue(msg.value()
							+ (this.getDamageBonus(msg.source(), w.weaponType())));
			}
		}
	}
		...
		if (affected instanceof MOB) {
			...
			if ((myWeapon instanceof Weapon)
					&& ((((Weapon) myWeapon).weaponClassification() == weaponClass)
							|| (weaponClass < 0) || (((Weapon) myWeapon)
							.weaponClassification() == secondWeaponClass))) {
				activated = true;
				affectableStats.setAttackAdjustment(affectableStats
						.attackAdjustment()
						+ (int) Math.round(15.0 * (CMath.div(proficiency(),
								100.0)))
