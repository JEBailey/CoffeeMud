...
public class Trap_BearTrap extends StdTrap {
	...
	public Trap setTrap(MOB mob, Physical P, int trapBonus,
		...
		if (mob != null) {
			...
			if (I == null)
				I = findMostOfMaterial(mob.location(),
	...
	public boolean canSetTrapOn(MOB mob, Physical P) {
		...
		if (mob != null) {
			...
			if ((I == null)
					|| (super
							.findNumberOfResource(mob.location(), I.material()) < 30)) {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((sprung) && (trapped != null) && (affected != null)
				&& (msg.amISource(trapped)) && (trapped.location() != null)) {
			if ((((msg.targetMinor() == CMMsg.TYP_LEAVE) || (msg.targetMinor() == CMMsg.TYP_FLEE))
				...
				if (trapped.location().show(trapped, null,
						CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> struggle(s) to get out of the bear trap.")) {
					amountRemaining -= trapped.charStats().getStat(
							CharStats.STAT_STRENGTH);
					amountRemaining -= trapped.phyStats().level();
					if (amountRemaining <= 0) {
						trapped.location().show(trapped, null,
								CMMsg.MSG_OK_VISUAL,
								"<S-NAME> pull(s) free of the bear trap.");
						trapped = null;
					} else
						return false;
				} else
		...
		if ((target != invoker()) && (target.location() != null)) {
			if ((!invoker().mayIFight(target))
				...
				target.location().show(target, null, null,
						...
						"<S-NAME> avoid(s) a bear trap!");
			else if (target.location().show(target, target, this,
					CMMsg.MASK_ALWAYS | CMMsg.MSG_NOISE,
					"<S-NAME> step(s) on a bear trap!")) {
				super.spring(target);
				int damage = CMLib.dice().roll(trapLevel() + abilityCode(), 6,
						1);
				trapped = target;
