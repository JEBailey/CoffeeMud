...
public class Trap_RoomPit extends StdTrap {
	...
	public void unInvoke() {
		if ((pit != null) && (canBeUninvoked()) && (pit.size() > 1)) {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((tickID == Tickable.TICKID_TRAP_DESTRUCTION)
				...
				&& (pit != null)
				&& (pit.size() > 1)
				&& (((((Room) pit.firstElement()).numPCInhabitants() > 0) || (((Room) pit
						.lastElement()).numPCInhabitants() > 0))))
			return true;
	...
	protected synchronized void makePit(MOB target) {
		if ((pit == null) || (pit.size() < 2)) {
			...
			myPitUp.basePhyStats().setDisposition(
					myPitUp.basePhyStats().disposition() | PhyStats.IS_DARK);
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((!sprung)
				...
				&& ((msg.targetMinor() == CMMsg.TYP_ENTER) && (!msg.source()
						.isMine(affected))) && (msg.tool() != null)
		...
		if ((target != invoker()) && (target.location() != null)
			...
			if ((!invoker().mayIFight(target))
				...
				target.location().show(target, null, null,
						...
						"<S-NAME> avoid(s) falling into a pit!");
			else if (target.location().show(target, target, this,
					CMMsg.MASK_ALWAYS | CMMsg.MSG_NOISE,
					"<S-NAME> fall(s) into a pit!")) {
				super.spring(target);
				makePit(target);
				...
				((Room) pit.lastElement()).rawDoors()[Directions.UP] = target
						.location();
				if ((target.location().getRoomInDir(Directions.DOWN) == null)
					...
					target.location().rawDoors()[Directions.DOWN] = ((Room) pit
							.lastElement());
				}
				if ((!door.isOpen()) && (affected instanceof Room))
					door.executeMsg(target, CMClass.getMsg(target, door, null,
							CMMsg.MASK_ALWAYS | CMMsg.MSG_OPEN, null));
