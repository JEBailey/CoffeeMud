...
public class Trap_Boulders extends StdTrap {
	...
	public boolean canSetTrapOn(MOB mob, Physical P) {
		...
		if (mob != null) {
			...
			if ((I == null)
					|| (super
							.findNumberOfResource(mob.location(), I.material()) < 50)) {
		...
		if ((target != invoker()) && (target.location() != null)) {
			if ((!invoker().mayIFight(target))
				...
				target.location().show(target, null, null,
						...
						"<S-NAME> avoid(s) setting off a boulder trap!");
			else if (target.location().show(target, target, this,
					CMMsg.MASK_ALWAYS | CMMsg.MSG_NOISE,
					"<S-NAME> trigger(s) a trap!")) {
				super.spring(target);
				int damage = CMLib.dice().roll(trapLevel() + abilityCode(), 20,
						1);
				CMLib.combat().postDamage(
						...
						CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS
								| CMMsg.TYP_JUSTICE, Weapon.TYPE_BASHING,
						"Dozens of boulders <DAMAGE> <T-NAME>!");
			}
		}
	}
}
