...
public class Trap_Trap extends StdAbility implements Trap {
	...
	public Trap setTrap(MOB mob, Physical P, int trapBonus,
		...
		if (perm) {
			...
			T.setSavable(false);
			CMLib.threads().startTickDown(T, Tickable.TICKID_TRAP_DESTRUCTION,
					level * 30);
		}
		return T;
	}
	...
	public void gas(MOB mob) {
		if (CMLib.dice().rollPercentage() < mob.charStats().getSave(
			...
			mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
					"<S-NAME> avoid(s) a gas trap set in <T-NAME>.");
		else if (mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
				"<S-NAME> trigger(s) a trap set in <T-NAME>!"))
			if (mob.phyStats().level() > 15) {
				...
				for (int i = 0; i < mob.location().numInhabitants(); i++) {
					...
					int dmg = CMLib.dice().roll(target.phyStats().level(), 10,
							1);
					CMMsg msg = CMClass.getMsg(invoker(), target, this,
							CMMsg.MSG_OK_ACTION, CMMsg.MSK_MALICIOUS_MOVE
									| CMMsg.TYP_GAS, CMMsg.MSG_NOISYMOVEMENT,
							null);
					if (target.location().okMessage(target, msg)) {
						...
						CMLib.combat().postDamage(
								...
								CMMsg.MASK_ALWAYS | CMMsg.MASK_MALICIOUS
										| CMMsg.TYP_GAS, Weapon.TYPE_GASSING,
								"The gas <DAMAGE> <T-NAME>!");
					}
				}
			} else {
				MOB target = mob;
				...
				CMMsg msg = CMClass.getMsg(invoker(), target, this,
						CMMsg.MSG_OK_ACTION, CMMsg.MSK_MALICIOUS_MOVE
								| CMMsg.TYP_GAS, CMMsg.MSG_NOISYMOVEMENT, null);
				if (target.location().okMessage(target, msg)) {
					target.location().send(target, msg);
					if (msg.value() > 0)
						dmg = (int) Math.round(CMath.div(dmg, 2.0));
					CMLib.combat().postDamage(
							...
							CMMsg.MASK_ALWAYS | CMMsg.MASK_MALICIOUS
									| CMMsg.TYP_GAS, Weapon.TYPE_GASSING,
							"A sudden blast of gas <DAMAGE> <T-NAME>!");
				}
			}
	}
	...
	public void needle(MOB mob) {
		if (CMLib.dice().rollPercentage() < mob.charStats().getSave(
				CharStats.STAT_SAVE_TRAPS))
			mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
					"<S-NAME> avoid(s) a needle trap set in <T-NAME>.");
		else if (mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
				"<S-NAME> trigger(s) a needle trap set in <T-NAME>!")) {
			MOB target = mob;
			int dmg = CMLib.dice().roll(target.phyStats().level(), 5, 1);
			CMMsg msg = CMClass.getMsg(invoker(), target, this,
					CMMsg.MSG_OK_ACTION, CMMsg.MSK_MALICIOUS_MOVE
							| CMMsg.TYP_JUSTICE, CMMsg.MSG_NOISYMOVEMENT, null);
			if (target.location().okMessage(target, msg)) {
				target.location().send(target, msg);
				if (msg.value() > 0)
					dmg = (int) Math.round(CMath.div(dmg, 2.0));
				CMLib.combat().postDamage(
						...
						dmg,
						CMMsg.MASK_ALWAYS | CMMsg.MASK_MALICIOUS
								| CMMsg.TYP_JUSTICE, Weapon.TYPE_PIERCING,
						"The needle <DAMAGE> <T-NAME>!");
	...
	public void blade(MOB mob) {
		if (CMLib.dice().rollPercentage() < mob.charStats().getSave(
				CharStats.STAT_SAVE_TRAPS))
			mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
					"<S-NAME> avoid(s) a blade trap set in <T-NAME>.");
		else if (mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
				"<S-NAME> trigger(s) a blade trap set in <T-NAME>!")) {
			MOB target = mob;
	...
	public void victimOfSpell(MOB mob) {
		if (CMLib.dice().rollPercentage() < mob.charStats().getSave(
				CharStats.STAT_SAVE_TRAPS))
			mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
					"<S-NAME> avoid(s) a magic trap set in <T-NAME>.");
		else if (mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
				"<S-NAME> trigger(s) a trap set in <T-NAME>!")) {
			String spell = text();
			int x = spell.indexOf(';');
		...
		if (CMLib.flags().isInFlight(mob)) {
			mob.location()
					.show(mob,
							null,
							CMMsg.MSG_OK_ACTION,
							"<S-NAME> trigger(s) a trap door beneath <S-HIS-HER> feet! <S-NAME> pause(s) over it in flight.");
			return;
		} else if (CMLib.dice().rollPercentage() < mob.charStats().getSave(
				CharStats.STAT_SAVE_TRAPS))
			mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
					"<S-NAME> avoid(s) a trap door beneath <S-HIS-HER> feet.");
		else if (mob
				.location()
				.show(mob,
						affected,
						this,
						CMMsg.MSG_OK_ACTION,
						"<S-NAME> trigger(s) a trap door beneath <S-HIS-HER> feet! <S-NAME> fall(s) in!")) {
			if ((myPit == null) || (myPitUp == null)) {
				...
				myPitUp.basePhyStats()
						.setDisposition(
								myPitUp.basePhyStats().disposition()
										| PhyStats.IS_DARK);
				...
				myPit.basePhyStats().setDisposition(
						myPit.basePhyStats().disposition() | PhyStats.IS_DARK);
				...
				myPit.setRawExit(Directions.UP,
						CMClass.getExit("StdOpenDoorway"));
			...
			if (mob.phyStats().weight() < 5)
				...
				CMLib.combat().postDamage(invoker(), mob, this, damage,
						CMMsg.MASK_MALICIOUS | CMMsg.TYP_JUSTICE, -1, null);
			}
		...
		default:
			target.location()
					.show(target, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> trigger(s) a trap, but it appears to have misfired.");
			break;
		}

		if ((getReset() > 0) && (getReset() < Integer.MAX_VALUE))
			CMLib.threads().startTickDown(this, Tickable.TICKID_TRAP_RESET,
					getReset());
		else
			unInvoke();
	}
	...
	public void unInvoke() {
		if ((trapType() == Trap.TRAP_PIT_BLADE) && (affected instanceof Exit)
				&& (myPit != null) && (canBeUninvoked()) && (myPitUp != null)) {
