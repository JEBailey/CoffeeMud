...
public class Trap_SpikePit extends Trap_RoomPit {
	...
	public Trap setTrap(MOB mob, Physical P, int trapBonus,
		...
		while ((I != null) && ((++num) < 6)) {
			...
			daggerDamages
					.addElement(Integer.valueOf(I.basePhyStats().damage()));
			I.destroy();
			I = getDagger(mob);
		}
	...
	public List<Item> getTrapComponents() {
		...
		if ((daggerDamages == null) || (daggerDamages.size() == 0))
			...
			for (int d = 0; d < daggerDamages.size(); d++) {
				...
				I.basePhyStats().setDamage(
						((Integer) daggerDamages.elementAt(d)).intValue());
				I.recoverPhyStats();
				V.addElement(I);
			}
		return V;
	...
	public void finishSpringing(MOB target) {
		if ((!invoker().mayIFight(target)) || (target.phyStats().weight() < 5))
			...
			target.location().show(target, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> hit(s) the pit floor!");
			int damage = CMLib.dice().roll(trapLevel() + abilityCode(), 6, 1);
			if ((daggerDamages != null) && (daggerDamages.size() > 0)) {
				for (int i = 0; i < daggerDamages.size(); i++)
					damage += CMLib.dice().roll(1,
							((Integer) daggerDamages.elementAt(i)).intValue(),
							0);
			} else
