...
public class Skill_Imitation extends BardSkill {
		...
		if (msg.tool() != null) {
			if ((msg.amISource(mob))
					&& ((msg.tool().ID().equals("Skill_Spellcraft"))
							...
							|| (msg.tool().ID().equals("Skill_Chantcraft")) || (msg
							.tool().ID().equals("Skill_Prayercraft")))
			...
			} else if ((msg.tool() instanceof Ability) && (!msg.amISource(mob))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		String cmd = (commands.size() > 0) ? CMParms.combine(commands, 0)
				.toUpperCase() : "";
		...
		if ((cmd.length() == 0) || (found == null)) {
			if (found != null)
				mob.tell("'" + cmd
						+ "' is not something you know how to imitate.");
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					CMMsg.MSG_NOISYMOVEMENT | CMMsg.MASK_DELICATE
							| (auto ? CMMsg.MASK_ALWAYS : 0),
					immitations.get(found));
			if (mob.location().okMessage(mob, msg))
				mob.location().send(mob, msg);
		} else
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to imitate " + found
							+ ", but fail(s).");
