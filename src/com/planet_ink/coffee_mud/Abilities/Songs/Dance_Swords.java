...
public class Dance_Swords extends Dance {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((!super.okMessage(myHost, msg)) || (affected == null)) {
	...
	public void unInvoke() {
		...
		super.unInvoke();
		if ((E != null) && (E instanceof Item) && (((Item) E).owner() != null)
				&& (((Item) E).owner() instanceof Room)) {
			((Room) ((Item) E).owner()).showHappens(CMMsg.MSG_OK_ACTION,
					E.name() + " stops dancing!");
		}
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (affected == M) {
			...
			return true;
				...
				&& (((Weapon) affected).owner() instanceof Room) && (M != null)
			...
			if (M.isInCombat()) {
				boolean isHit = (CMLib.combat().rollToHit(CMLib.combat()
						.adjustedAttackBonus(M, victiM)
						+ ((Weapon) affected).phyStats().attackAdjustment(),
						CMLib.combat().adjustedArmor(victiM), 0));
				if ((!isHit) || (!(affected instanceof Weapon)))
					M.location().show(M, victiM, affected, CMMsg.MSG_OK_ACTION,
							"<O-NAME> attacks <T-NAME> and misses!");
				else {
					int bonusDamage = (affected.phyStats().damage() + 5 + getXLEVELLevel(M))
							- M.phyStats().damage();
					int damage = CMLib.combat().adjustedDamage(M,
							(Weapon) affected, victiM, bonusDamage, false);
					CMLib.combat()
							.postDamage(
									M,
									victiM,
				...
				case 1:
					M.location().showHappens(CMMsg.MSG_OK_VISUAL,
							affected.name() + " twiches a bit.");
					break;
				case 2:
					M.location().showHappens(CMMsg.MSG_OK_VISUAL,
							affected.name() + " is looking for trouble.");
					break;
				case 3:
					M.location().showHappens(CMMsg.MSG_OK_VISUAL,
							affected.name() + " practices its moves.");
					break;
				case 4:
					M.location().showHappens(CMMsg.MSG_OK_VISUAL,
		...
		if (success) {
			...
			for (int v = 0; v < commonRoomSet.size(); v++) {
				...
				if (R.okMessage(mob, msg)) {
					...
					if ((CMLib.flags().canBeSeenBy(invoker, follower) && (follower
						...
						if (R.okMessage(mob, msg2)) {
							...
							if ((msg2.value() <= 0)
									&& (follower.fetchEffect(newOne.ID()) == null))
