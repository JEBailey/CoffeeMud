...
public class Song extends StdAbility {
	...
	public void affectPhyStats(Physical affectedEnv, PhyStats affectableStats) {
		if (this.invoker() == affectedEnv)
			affectableStats.addAmbiance("(?)singing of "
					+ songOf().toLowerCase());
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		...
		if ((affected == invoker) && (msg.amISource(invoker)) && (!unInvoked)) {
			if (((msg.sourceMinor() == CMMsg.TYP_SPEAK) || ((msg.sourceMinor() == CMMsg.TYP_CAST_SPELL) && (CMath
					.bset(msg.sourceCode(), CMMsg.MASK_SOUND))))
				...
				if (msg.source().location() != null)
					msg.source()
							...
							.show(msg.source(), null, CMMsg.MSG_NOISE,
									"<S-NAME> stop(s) singing.");
				unInvoke();
			} else if ((msg.target() instanceof Armor)
					&& (msg.targetMinor() == CMMsg.TYP_WEAR)
					&& (CMath.bset(
							((Armor) msg.target()).rawProperLocationBitmap(),
							Wearable.WORN_MOUTH))) {
				if (msg.source().location() != null)
					msg.source()
							.location()
	...
	protected String getCorrectMsgString(Room R, String str, int v) {
		...
		if (R == originRoom)
			...
			if (dir >= 0)
				msgStr = "^SYou hear the " + songOf() + " being sung "
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!auto) && (!mob.isMonster()) && (!disregardsArmorCheck(mob))
			...
			mob.location().show(
					...
					null,
					CMMsg.MSG_OK_VISUAL,
					"<S-NAME> hit(s) a foul note on " + name()
		...
		if (success) {
			...
			String str = auto ? "^SThe " + songOf() + " begins to play!^?"
