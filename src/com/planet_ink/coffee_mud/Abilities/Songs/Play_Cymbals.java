...
public class Play_Cymbals extends Play_Instrument {
	...
	protected int requiredInstrumentType() {
		return MusicalInstrument.TYPE_CYMBALS;
	}
			...
			if (R != null) {
				...
				if (mob == invoker()) {
					for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
						...
						if ((e != null) && (e.hasADoor()) && (e.hasALock())
								&& (e.isLocked())) {
							knockables.add(e);
							dirCode = d;
						}
				...
				for (Physical P : knockables) {
					int levelDiff = P.phyStats().level()
							- (mob.phyStats().level() + (2 * super
									.getXLEVELLevel(mob)));
					if (levelDiff < 0)
						levelDiff = 0;
					if (proficiencyCheck(mob, -(levelDiff * 25), false)) {
						CMMsg msg = CMClass.getMsg(mob, P, this,
								CMMsg.MSG_CAST_VERBAL_SPELL, P.name()
										+ " begin(s) to glow!");
						if (R.okMessage(mob, msg)) {
							R.send(mob, msg);
							msg = CMClass.getMsg(mob, P, null,
									CMMsg.MSG_UNLOCK, null);
							CMLib.utensils().roomAffectFully(msg, R, dirCode);
							msg = CMClass.getMsg(mob, P, null, CMMsg.MSG_OPEN,
									"<T-NAME> opens.");
							CMLib.utensils().roomAffectFully(msg, R, dirCode);
						}
					}
