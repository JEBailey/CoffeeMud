...
public class Play_Solo extends Play {
	...
	public boolean okMessage(Environmental E, CMMsg msg) {
		...
		if ((affected != null) && (affected instanceof MOB)) {
			...
			if (!msg.amISource(myChar)
				...
				if (((otherBard.phyStats().level()
						+ CMLib.dice().roll(1, 30, 0) + getXLEVELLevel(otherBard)) > (myChar
						.phyStats().level() + CMLib.dice().roll(1, 20, 0) + getXLEVELLevel(myChar)))
						&& (otherBard.location() != null)) {
					if ((otherBard.location()
							.show(otherBard, myChar, null, CMMsg.MSG_OK_ACTION,
									"<S-NAME> upstage(s) <T-NAMESELF>, stopping <T-HIS-HER> solo!"))
							&& ((otherBard.location() == originRoom)
									|| (originRoom == null) || originRoom
										.showOthers(otherBard, myChar, null,
												CMMsg.MSG_OK_ACTION,
												"<S-NAME> upstage(s) <T-NAMESELF>, stopping <T-HIS-HER> solo!")))
						unplayMe(myChar, null);
				} else if (otherBard.location() != null) {
					otherBard.tell("You can't seem to upstage " + myChar.name()
							+ "'s solo.");
					if (!invoker().curState()
							.adjMana(-10, invoker().maxState()))
						unplayMe(myChar, null);
					return false;
				}
			}
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			if ((!mob.isInCombat())
					|| (CMLib.flags()
							.domainAffects(mob.getVictim(), Ability.ACODE_SONG)
							.size() == 0))
			...
			for (int v = 0; v < commonRoomSet.size(); v++) {
				...
				if (R.okMessage(mob, msg)) {
					...
					for (int i = 0; i < R.numInhabitants(); i++) {
						...
						if (M != null)
							for (int a = 0; a < M.numEffects(); a++) // personal
																		// affects
							{
					...
					if (mob.curState().getMana() < reqMana) {
						mob.tell("You needed " + reqMana
								+ " mana to play this solo!");
						return false;
					}
