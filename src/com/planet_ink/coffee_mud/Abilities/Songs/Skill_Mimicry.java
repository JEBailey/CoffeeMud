...
public class Skill_Mimicry extends BardSkill {
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if ((affecting instanceof MOB)
				&& (!CMLib.flags().aliveAwakeMobileUnbound((MOB) affecting,
						true)))
			return;
		if (disabled)
			return;
		...
		if (((!(affecting instanceof MOB)) || (!msg.amISource((MOB) affecting)))
				&& ((text().length() == 0) || (text().equalsIgnoreCase(msg
						.source().Name())))
				&& ((msg.sourceMinor() != CMMsg.TYP_EMOTE) || (msg.tool() instanceof Social)))
			lastMsg = msg;
	}
	...
	public void fixSNameTo(CMMsg msg, MOB sMOB, Environmental ticking) {
		...
		// if(src!=null)
		// src=CMStrings.replaceAll(src,"<S-NAME>",ticking.name());
		// if(src!=null)
		// src=CMStrings.replaceAll(src,"You ",ticking.name()+" ");
		// if(src!=null)
		// src=CMStrings.replaceAll(src,"Your ",ticking.name()+"`s ");
		if (trg != null)
			trg = CMStrings.replaceAll(trg, "<S-NAME>", ticking.name());
		if (oth != null)
			oth = CMStrings.replaceAll(oth, "<S-NAME>", ticking.name());
		// if(src!=null)
		// src=CMStrings.replaceAll(src,"<S-HIM-HERSELF>","itself");
		if (trg != null)
			trg = CMStrings.replaceAll(trg, "<S-HIM-HERSELF>", "itself");
		if (oth != null)
			oth = CMStrings.replaceAll(oth, "<S-HIM-HERSELF>", "itself");
		// if(src!=null)
		// src=CMStrings.replaceAll(src,"<S-HIS-HERSELF>","itself");
		if (trg != null)
		...
		msg.modify(sMOB, sMOB, msg.tool(), msg.sourceCode(), oth,
				msg.targetCode(), trg, msg.othersCode(), oth);
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (msg == null)
			return true;
		lastMsg = null;
		if (((affected instanceof MOB) && (!CMLib.flags()
				.aliveAwakeMobileUnbound((MOB) affected, true))))
			return true;
		...
		if (msg.sourceMinor() == CMMsg.TYP_EMOTE) {
			if (affected instanceof MOB)
				...
				msg.modify(newSMOB, msg.source(), msg.tool(), msg.sourceCode(),
			...
			if (affected instanceof MOB)
				...
				newSMOB.setName(affected.name());
				newSMOB.recoverCharStats();
				msg.modify(newSMOB, target, msg.tool(), msg.sourceCode(),
						msg.sourceMessage(), msg.targetCode(),
						msg.targetMessage(), msg.othersCode(),
						msg.othersMessage());
			}
			...
			return true;
		disabled = true;
		if ((sMOB.location() != null) && (sMOB.location().okMessage(sMOB, msg))) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					CMMsg.MSG_QUIETMOVEMENT | (auto ? CMMsg.MASK_ALWAYS : 0),
					auto ? "" : "<S-NAME> begin(s) mimicing <T-NAMESELF>.");
			...
			return beneficialVisualFizzle(mob, target,
					"<S-NAME> attempt(s) to mimic <T-NAMESELF>, but fail(s).");
		return success;
	}
}
