...
public class Dance extends StdAbility {
	...
	public void affectPhyStats(Physical affectedEnv, PhyStats affectableStats) {
		if (this.invoker() == affectedEnv)
			affectableStats.addAmbiance("performing the "
					+ danceOf().toLowerCase());
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((invoker == null) || (CMLib.flags().isSitting(invoker()))
				|| (!CMLib.flags().aliveAwakeMobile(mob, true))
		...
		if (!mob.curState()
				.adjMovement(-(invokerManaCost / 15), mob.maxState())) {
	...
	protected String getCorrectMsgString(Room R, String str, int v) {
		...
		if (R == originRoom)
			...
			if (dir >= 0)
				msgStr = "^SYou see the " + danceOf() + " being performed "
						+ Directions.getInDirectionName(dir) + "!^?";
			else
				msgStr = "^SYou see the " + danceOf()
						+ " being performed nearby!^?";
		}
		return msgStr;
	}
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		if ((invoker() != null) && (!unInvoked) && (affected == invoker())
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			String str = auto ? "^SThe " + danceOf() + " begins!^?"
					: "^S<S-NAME> begin(s) to dance the " + danceOf() + ".^?";
			...
			for (int v = 0; v < commonRoomSet.size(); v++) {
				...
				if (R.okMessage(mob, msg)) {
					...
					for (Iterator f = h.iterator(); f.hasNext();) {
						...
						if ((R2 != null)
								&& (CMLib.flags()
										.canBeSeenBy(invoker, follower) && (follower
										.fetchEffect(this.ID()) == null))) {
							...
							if ((mindAttack()) && (follower != mob))
								msg2 = CMClass
										.getMsg(mob,
												follower,
												this,
												CMMsg.MSK_CAST_MALICIOUS_SOMANTIC
														| CMMsg.TYP_MIND
