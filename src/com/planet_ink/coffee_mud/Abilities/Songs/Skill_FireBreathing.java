...
public class Skill_FireBreathing extends BardSkill {
	...
	private static final String[] triggerStrings = { "FIREBREATHING",
			"FIREBREATH" };
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							target,
							this,
							CMMsg.MASK_MALICIOUS | CMMsg.MSG_NOISYMOVEMENT
									| (auto ? CMMsg.MASK_ALWAYS : 0),
							(auto ? "Suddenly flames come up and attack <T-HIM-HER>!^?"
									: ((fireSource != null) ? "^S<S-NAME> hold(s) "
											+ fireSource.name()
											+ " up and puff(s) fire at <T-NAMESELF>!^?"
											: "<S-NAME> breath(es) fire at <T-NAMESELF>!^?"))
			...
			if ((mob.location().okMessage(mob, msg))
				...
				int numDice = (int) Math.round(CMath.div(
						adjustedLevel(mob, asLevel)
								+ (2 * super.getX1Level(mob)), 1.5)) + 1;
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to breathe fire at <T-NAMESELF>, but only puff(s) smoke.");
