		...
		if ((msg.target() != null) && (msg.target() instanceof MOB)
			...
			if ((msg.amISource(myChar) || (msg.amITarget(myChar))
				...
				if (msg.amISource(myChar))
					...
					msg.source().tell(
							myChar.name() + " doesn't look like "
									+ myChar.charStats().heshe()
									+ " feels like it.");
				return false;
			}
		}
		...
		affectableStats
				.setAttackAdjustment(affectableStats.attackAdjustment()
						- ((invoker().charStats().getStat(
								CharStats.STAT_CHARISMA) / 4) + (adjustedLevel(
								invoker(), 0))));
	}
	...
	public void affectCharStats(MOB mob, CharStats stats) {
		...
		if ((invoker() != null) && (invoker() != mob))
			stats.setStat(
					...
					stats.getStat(CharStats.STAT_SAVE_JUSTICE)
							- (invoker().charStats().getStat(
									CharStats.STAT_CHARISMA) + getXLEVELLevel(invoker())));
	}
}
