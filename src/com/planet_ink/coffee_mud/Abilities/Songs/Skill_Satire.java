...
public class Skill_Satire extends BardSkill {
	...
	public void criminalFail(LegalBehavior B, Area A2, MOB mob, MOB witness) {
		...
		B.addWarrant(A2, mob, witness, crimeLocs, crimeFlags, crime, sentence,
				desc);
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!success) {
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to mock <T-NAME>, but <S-IS-ARE> not funny.");
			if (CMLib.dice().rollPercentage() > mob.charStats().getStat(
					CharStats.STAT_CHARISMA))
				criminalFail(B, A2, mob, target);
			return false;
		}
		CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MSK_CAST_VERBAL
				| CMMsg.TYP_JUSTICE, "<S-NAME> mock(s) <T-NAME>.");
		if (mob.location().okMessage(mob, msg)) {
			mob.location().send(mob, msg);
			for (MOB M : forgivables) {
				if (B.aquit(A2, M, null)) {
					room.show(M, target, CMMsg.MSG_OK_VISUAL,
							"<T-NAME>, smiling, forget(s) <S-YOUPOSS> crime.");
					return false;
				}
			}
			if ((msg.value() > 0)
					|| (CMLib.dice().rollPercentage() < (25 - mob.charStats()
							.getStat(CharStats.STAT_CHARISMA)))) {
				criminalFail(B, A2, mob, target);
			}
		}
		return success;
