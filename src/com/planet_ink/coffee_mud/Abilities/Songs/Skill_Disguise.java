...
public class Skill_Disguise extends BardSkill {
	...
	protected final static String[] whats = {
			// 0! 1! 2! 3! 4! 5! 6! 7! 8!
			"WEIGHT", "LEVEL", "SEX", "RACE", "HEIGHT", "NAME", "CLASS",
			"ALIGNMENT", "AGE" };
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(mob)
			...
			if (msg.othersMessage() != null) {
				omsg = CMStrings.replaceAll(msg.othersMessage(), "<T-NAME>",
						mob.name());
				omsg = CMStrings.replaceAll(omsg, "<T-NAMESELF>", mob.name());
			}
			msg.modify(msg.source(), this, msg.tool(), msg.sourceCode(),
					msg.sourceMessage(), msg.targetCode(), msg.targetMessage(),
					msg.othersCode(), omsg);
		}
		return true;
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(this)
			...
			if (!mob.isMonster()) {
				...
				myDescription.append(mob.name(msg.source()) + " the "
						+ mob.charStats().raceName() + " is a " + levelStr
						+ ".\n\r");
			}
			...
			if (height > 0)
				myDescription.append(mob.charStats().HeShe() + " is " + height
						+ " inches tall and weighs " + weight + " pounds.\n\r");
			...
			myDescription.append(mob.charStats().HeShe() + " is wearing:\n\r"
					+ CMLib.commands().getEquipment(msg.source(), mob));
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((CMLib.ableMapper().qualifyingLevel(mob, this) > 0)
			...
			mob.tell("You must have " + levels[which]
					+ " levels in this skill to use that disguise.");
			return false;
		}
		commands.removeElementAt(0);
		if (commands.size() == 0) {
			mob.tell("Disguise " + whats[which].toLowerCase()
		...
		case 0: // weight
		{
			if (CMath.s_int(how) <= 0) {
				mob.tell("You cannot disguise your weight as " + how
						+ " pounds!");
				return false;
			}
			...
			if (x < 0)
				x = x * -1;
			adjustment = -((int) Math.round(CMath.div(x, mob.basePhyStats()
					.weight()) * 100.0));
			break;
		}
		case 1: // level
		...
		{
			if (CMath.s_int(how) <= 0) {
				mob.tell("You cannot disguise your height as " + how
						+ " inches!");
				return false;
			}
			...
			if (x < 0)
				x = x * -1;
			adjustment = -((int) Math.round(CMath.div(x, mob.phyStats()
					.height()) * 100.0));
			break;
		}
		case 5: // name
		...
		case 7: // alignment
		{
			if ((!how.equalsIgnoreCase("good"))
					&& (!how.equalsIgnoreCase("evil"))) {
		...
		case 8: // age
		{
			...
			int x = mob.baseCharStats().getStat(CharStats.STAT_AGE)
					- CMath.s_int(how);
			if (x < 0)
				x = x * -1;
			adjustment = -((int) Math.round(CMath.div(x, mob.baseCharStats()
					.getStat(CharStats.STAT_AGE)) * 100.0));
			break;
		}
		}
