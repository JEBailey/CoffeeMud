...
public class Skill_QuickChange extends BardSkill {
	...
	public List<PackedItem> getAllWornItems(MOB mob) {
		...
		for (Enumeration<Item> i = mob.items(); i.hasMoreElements();) {
			...
			if ((!I.amWearingAt(Wearable.IN_INVENTORY))
					|| (!I.ultimateContainer(null).amWearingAt(
							Wearable.IN_INVENTORY)))
	...
	public List<PackedItem> getAllPackedItems(Session S) {
		...
		if (super.miscText.trim().length() > 0) {
			...
			if ((locStart > 0) && (contStart > locStart)) {
				...
				CMLib.coffeeMaker().addItemsFromXML(
						super.miscText.substring(0, locStart), itemList, S);
				List<String> itemLocList = CMParms.parseAny(
						super.miscText.substring(
								locStart + locationsDelim.length(), contStart),
						';', true);
				List<String> itemConList = CMParms.parseAny(
						super.miscText.substring(contStart
								+ containerDelim.length()), ';', true);
				if ((itemLocList.size() == itemList.size())
						&& (itemLocList.size() == itemConList.size())) {
					for (int i = 0; i < itemList.size(); i++) {
						...
						items.add(new PackedItem(itemList.get(i), containerI,
								wornLoc));
					}
				}
			}
	...
	public void packThese(List<PackedItem> items) {
		...
		str.append(CMLib.coffeeMaker().getItemsXML(itemList, new Hashtable(),
				new HashSet(), 0));
		str.append("</ITEMS>");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, null, this,
					...
					"<S-NAME> perform(s) a quick costume change.");
			...
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to change clothes, but forget(s) how.");
