...
public class Song_Ode extends Song {
	...
	protected String songOf() {
		return "Ode" + ((whom == null) ? "" : " to " + whom.name()) + "";
	}
	...
	protected static final String[][] stuff = {
			...
			{ "" + CMMsg.TYP_GET, "cs", "" + CharStats.STAT_STRENGTH,
					"The strength of <O-NAME> astounds us all!" },
			{ "" + CMMsg.TYP_GAS, "ca", "" + CharStats.STAT_SAVE_GAS,
					"<O-NAME> has gas, and saves us from it!" },
			{ "" + CMMsg.TYP_FIRE, "ca", "" + CharStats.STAT_SAVE_FIRE,
					"We learn from firey <O-NAME> to keep our cool!" },
			{ "" + CMMsg.TYP_DRINK, "s", "t",
					"<O-NAME> quenches our thirst for health!" },
			{ "" + CMMsg.TYP_DISEASE, "ca", "" + CharStats.STAT_SAVE_DISEASE,
					"<O-NAME> worries so about our health!" },
			{ "" + CMMsg.TYP_UNDEAD, "ca", "" + CharStats.STAT_SAVE_UNDEAD,
					"<O-NAME>, the deadly one, fills our hearts with life!" },
			{ "" + CMMsg.TYP_COLD, "ca", "" + CharStats.STAT_SAVE_COLD,
					"Cold-hearted <O-NAME> has shown us how to keep warm!" },
			{ "" + CMMsg.TYP_CAST_SPELL, "ca", "" + CharStats.STAT_SAVE_MAGIC,
					"<O-NAME>, the enchanting, helps us avoid magic spells!" },
			{ "" + CMMsg.TYP_ACID, "ca", "" + CharStats.STAT_SAVE_ACID,
					"<O-NAME> drips with warnings about acid!" },
			{ "" + CMMsg.TYP_ELECTRIC, "ca", "" + CharStats.STAT_SAVE_ELECTRIC,
					"The electric one, <O-NAME>, has shown us how to stay grounded!" },
			{ "" + CMMsg.TYP_LOOK, "es",
					"" + (PhyStats.CAN_SEE_DARK | PhyStats.CAN_SEE_HIDDEN),
					"<O-NAME> is ever watchful, and has opened our eyes!" },
			{ "" + CMMsg.TYP_EXAMINE, "es",
					"" + (PhyStats.CAN_SEE_BONUS | PhyStats.CAN_SEE_METAL),
					"<O-NAME> is a carefuly observer, and never misses the finest detail!" },
			{ "" + CMMsg.TYP_JUSTICE, "ca", "" + CharStats.STAT_SAVE_JUSTICE,
					"The dreadful <O-NAME> shows us the way out of danger!" },
			{ "" + CMMsg.TYP_MIND, "ca", "" + CharStats.STAT_SAVE_MIND,
					"<O-NAME>, the deceiver, reminds us to keep our wits!" },
			{ "" + CMMsg.TYP_PARALYZE, "ca",
					"" + CharStats.STAT_SAVE_PARALYSIS,
					"<O-NAME> paralyzes <O-HIS-HER> enemies, that we may stay free!" },
			{ "" + CMMsg.TYP_POISON, "ca", "" + CharStats.STAT_SAVE_POISON,
					"<O-NAME> is poison to others, but protects <O-HIS-HER> friends!" },
			{ "" + CMMsg.TYP_SLEEP, "s", "i",
					"<O-NAME>, the sleepy, reminds us to stay fit and rested!" },
			{ "" + CMMsg.TYP_SIT, "s", "m",
					"<O-NAME>, the contemplative, teaches us <O-HIS-HER> power!" },
			{ "" + CMMsg.TYP_SPEAK, "cs", "" + CharStats.STAT_WISDOM,
					"<O-NAME>, the chatty one, fills us with <O-HIS-HER> wisdom!" },
			{ "" + CMMsg.TYP_WATER, "ca", "" + CharStats.STAT_SAVE_WATER,
					"<O-NAME>, the wet one, keeps us so dry!" },
			{ "" + CMMsg.TYP_WEAPONATTACK, "e", "a",
					"<O-NAME> the viscious has shown us to take arms!" },
			{ "" + CMMsg.TYP_LEAVE, "s", "v",
					"<O-NAME>, the wanderer, gives us our second wind!" } };
		...
		if (trail != null) {
			...
			for (int i = 0; i < 3; i++) {
				...
				while ((which == null) || (V2.contains(which))) {
					Integer w = (Integer) V.elementAt(CMLib.dice().roll(1,
							V.size(), -1));
			...
			for (Enumeration e = H.keys(); e.hasMoreElements();) {
				...
				if ((chk != null) && (chk[1].startsWith("c"))) {
					...
					if (CharStats.CODES.isBASE(stat))
						if (ticks > 5)
							ticks = 5;
					affectableStats.setStat(stat, affectableStats.getStat(stat)
							+ ticks + getXLEVELLevel(invoker()));
				}
			}
		}
	}
		...
		if ((whom != null) && (song != null)) {
			...
			for (Enumeration e = H.keys(); e.hasMoreElements();) {
				...
				if ((chk != null) && (chk[1].startsWith("s"))) {
					...
					switch (chk[2].charAt(0)) {
					case 'h':
						affectableStats.setHunger(affectableStats.getHunger()
								+ ticks + getXLEVELLevel(invoker()));
						break;
					case 't':
						affectableStats.setThirst(affectableStats.getThirst()
								+ ticks + getXLEVELLevel(invoker()));
						break;
					case 'v':
						affectableStats.setMovement(affectableStats
								.getMovement()
								+ ticks
								+ getXLEVELLevel(invoker()));
						break;
					case 'm':
						affectableStats.setMana(affectableStats.getMana()
								+ ticks + getXLEVELLevel(invoker()));
						break;
					case 'i':
						affectableStats.setHitPoints(affectableStats
								.getHitPoints()
								+ ticks
								+ getXLEVELLevel(invoker()));
						break;
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((whom != null) && (song == null) && (msg.amISource(whom))
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (mob == null)
			return false;
		if (song == null) {
			if ((whom == null) || (commonRoomSet == null)
		...
		if ((whom != null) && (song != null) && (affected == invoker())
			...
			Integer I = (Integer) V.elementAt(CMLib.dice()
					.roll(1, V.size(), -1));
			String[] chk = stuff[I.intValue()];
			invoker().location().show(invoker(), this, whom, CMMsg.MSG_SPEAK,
					"<S-NAME> sing(s) '" + chk[3] + "'.");
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			...
			if ((A != null) && (A.whom != null) && (A.song == null)) {
				String str = "^S<S-NAME> finish(es) composing the "
						+ A.songOf() + ".^?";
				CMMsg msg = CMClass.getMsg(mob, null, this,
						(auto ? CMMsg.MASK_ALWAYS : 0)
								| CMMsg.MSG_DELICATE_SMALL_HANDS_ACT, str);
			...
			if (str.length() > 0)
				mob.tell("You presently have odes written about: "
						+ str.toString().trim() + ".");
			return false;
		}
		...
		if (success) {
			...
			mob.location().show(mob, null, CMMsg.MSG_NOISE,
					"<S-NAME> lose(s) <S-HIS-HER> inspiration.");
		return success;
	}
}
