...
public class Skill_Juggle extends BardSkill {
	...
	public int maxJuggles() {
		if ((affected != null) && (affected instanceof MOB))
			return 5 + (CMLib.ableMapper().qualifyingClassLevel((MOB) affected,
					this) + (2 * getXLEVELLevel((MOB) affected)));
		return 5;
	}
	...
	public int maxAttacks() {
		if ((affected != null) && (affected instanceof MOB))
			return (int) Math.round(affected.phyStats().speed())
					+ ((CMLib.ableMapper().qualifyingClassLevel((MOB) affected,
							this) + (2 * getXLEVELLevel((MOB) affected))) / 5);
		return 1;
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (((msg.targetMinor() == CMMsg.TYP_GET)
			...
			msg.source().tell(msg.source(), msg.target(), null,
					"<T-NAME> is moving too fast for you to grab it.");
			return false;
		}
		return true;
	}
	...
	protected synchronized void juggle() {
		...
		for (int i = 0; i < M.numItems(); i++) {
			Item I = M.getItem(i);
			if ((I != null)
					&& ((I.amWearingAt(Wearable.WORN_WIELD) || I
							.amWearingAt(Wearable.WORN_HELD)))
					&& (!juggles.contains(I))
					&& (juggles.size() < maxJuggles())) {
				if (M.location().show(M, I, CMMsg.MSG_DELICATE_HANDS_ACT,
						"<S-NAME> start(s) juggling <T-NAMESELF>."))
					juggleItem(I);
				else {
					unJuggle(I);
		...
		for (int i = 0; i < copy.size(); i++) {
			...
			if ((i == jug) || (i == jug + 1)) {
				...
				if (!M.location().isContent(I))
					M.location()
							.moveItemTo(I, ItemPossessor.Expire.Player_Drop);
			}
		}
		M.recoverPhyStats();
		M.recoverCharStats();
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if (!pause) {
			...
			if (((msg.targetMinor() == CMMsg.TYP_GET) || (msg.targetMinor() == CMMsg.TYP_REMOVE))
				...
				if (juggles.size() == 0)
					unInvoke();
			} else if ((invoker() != null) && (!unInvoked)
					&& (affected == invoker()) && (msg.amISource(invoker()))
				...
				if (mob.location() != null) {
					if (!mob.location().show(
						...
						for (int i = 0; ((i < maxAttacks) && (copy.size() > 0)); i++) {
							Item I = copy.elementAt(CMLib.dice().roll(1,
									copy.size(), -1));
							I.unWear();
							mob.moveItemTo(I);
							if ((mob.isMine(I))
									&& (CMLib.commands().postDrop(mob, I, true,
											false, false))) {
								...
								w.basePhyStats().setDamage(
										CMLib.dice().roll(1,
												adjustedLevel(mob, 0), 0));
								w.basePhyStats().setWeight(
	...
	public void unInvoke() {
		if ((affected != null) && (affected instanceof MOB)) {
			...
			while (juggles.size() > 0) {
				...
				M.location().show(M, I, CMMsg.MSG_OK_ACTION,
						"<S-NAME> stop(s) juggling <T-NAMESELF>.");
				unJuggle(I);
				I.unWear();
				if (!M.isMine(I))
					M.moveItemTo(I);
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			if (mob.isInCombat()
					&& (mob.fetchItem(null, Wearable.FILTER_UNWORNONLY, "all") != null))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		String whatToJuggle = (mob.isMonster() && (givenTarget instanceof MOB)) ? "all"
		...
		if (whatToJuggle.toUpperCase().endsWith(".ALL")) {
			allFlag = true;
			whatToJuggle = "ALL "
					+ whatToJuggle.substring(0, whatToJuggle.length() - 4);
		}
		int addendum = 1;
		String addendumStr = "";
		...
		while (doBugFix || ((allFlag) && (addendum <= maxToJuggle))) {
			...
			if ((juggleThis != null)
					&& (!juggleThis.amWearingAt(Wearable.IN_INVENTORY))) {
				if ((!juggleThis.amWearingAt(Wearable.WORN_HELD))
						&& (!juggleThis.amWearingAt(Wearable.WORN_WIELD))) {
		...
		if (success) {
			...
			for (int i = 0; i < V.size(); i++) {
				...
				CMMsg msg = CMClass.getMsg(mob, I, this,
			...
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> attempt(s) to juggle, but messes up.");
