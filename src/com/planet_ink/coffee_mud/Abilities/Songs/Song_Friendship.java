...
public class Song_Friendship extends Song {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob))
			...
			mob.tell("You like " + mob.amFollowing().charStats().himher()
					+ " too much.");
			return false;
		} else if ((msg.amISource(mob))
				&& (!mob.isMonster())
					...
					if (mob.isMonster()) {
						if ((CMLib.dice().rollPercentage() > 50)
								|| ((mob.getStartRoom() != null)
										...
										.flags().isAggressiveTo(mob, invoker))
										|| (invoker == null) || (!mob
		...
		if (success) {
			...
			for (int v = 0; v < commonRoomSet.size(); v++) {
				...
				if (mob.location().okMessage(mob, msg)) {
					...
					Set<MOB> h = sendMsgAndGetTargets(mob, R, msg, givenTarget,
							auto);
					if (h == null)
						continue;
					...
					for (Iterator f = h.iterator(); f.hasNext();) {
						...
						if ((CMLib.flags().canBeHeardSpeakingBy(invoker,
							...
							if ((levelDiff > (3 + ((mob.phyStats().level() + (getXLEVELLevel(mob) * 2)) / 10)))
									&& (mindAttack()))
								mob.tell(mob, follower, null,
										"<T-NAME> looks too powerful.");
							else if ((R.okMessage(mob, msg2))
