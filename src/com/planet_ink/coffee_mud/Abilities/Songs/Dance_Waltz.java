...
public class Dance_Waltz extends Dance {
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectedStats) {
		...
		if (statadd == null) {
			...
			int classLevel = CMLib.ableMapper().qualifyingClassLevel(invoker(),
					this)
					+ (3 * getXLEVELLevel(invoker()));
			classLevel = (classLevel + 1) / 9;
			classLevel++;

			for (int i = 0; i < classLevel; i++)
				statadd[CharStats.CODES.BASE()[CMLib.dice().roll(1,
						CharStats.CODES.BASE().length, -1)]] += 3;
		}
