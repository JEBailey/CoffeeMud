...
public class Play extends StdAbility {
	...
	public int invokerLevel() {
		if (invoker() != null) {
			if (instrument != null)
				return invoker().phyStats().level()
						+ instrument.phyStats().ability()
						+ (getXLEVELLevel(invoker()) * 2);
			return invoker().phyStats().level()
					+ (getXLEVELLevel(invoker()) * 2);
	...
	public static boolean usingInstrument(MusicalInstrument I, MOB mob) {
		...
		if (I instanceof Rideable)
			return (((Rideable) I).amRiding(mob)
					&& (mob.fetchFirstWornItem(Wearable.WORN_WIELD) == null) && (mob
						.fetchHeldItem() == null));
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((invoker == null)
				|| ((instrument != null) && (!usingInstrument(instrument,
						invoker)))
	...
	public void affectPhyStats(Physical affectedEnv, PhyStats affectableStats) {
		if ((this.invoker() == affectedEnv) && (instrument != null))
			affectableStats.addAmbiance("(?)playing " + songOf().toLowerCase()
					+ " on " + instrument.name() + ":playing "
					+ instrument.name());
		...
		if ((affected == invoker) && (msg.amISource(invoker)) && (!unInvoked)
			...
			if ((msg.sourceMinor() == CMMsg.TYP_SPEAK)
				...
				if (msg.source().location() != null)
					msg.source()
							.location()
							.show(msg.source(), null, CMMsg.MSG_NOISE,
									"<S-NAME> stop(s) playing.");
				unInvoke();
			} else if (((msg.sourceMinor() == CMMsg.TYP_REMOVE)
					|| (msg.sourceMinor() == CMMsg.TYP_WEAR) || (msg
					.sourceMinor() == CMMsg.TYP_WIELD))
		...
		if ((A instanceof Play)
			...
			if (P.timeOut == 0)
				P.timeOut = System.currentTimeMillis()
						+ (CMProps.getTickMillis() * (((invoker() != null) && (invoker() != mob)) ? super
								.getXTIMELevel(invoker()) : 0));
	...
	public static MusicalInstrument getInstrument(MOB mob,
			int requiredInstrumentType, boolean noisy) {
		MusicalInstrument instrument = null;
		if ((mob.riding() != null)
				&& (mob.riding() instanceof MusicalInstrument)) {
			if (!usingInstrument((MusicalInstrument) mob.riding(), mob)) {
				if (noisy)
					mob.tell("You need to free your hands to play "
							+ mob.riding().name() + ".");
				return null;
			}
		...
		if (instrument == null)
			for (int i = 0; i < mob.numItems(); i++) {
				...
				if ((I != null) && (I instanceof MusicalInstrument)
						&& (I.container() == null)
		...
		if ((requiredInstrumentType >= 0)
				&& (instrument.instrumentType() != requiredInstrumentType)) {
			if (noisy)
				mob.tell("This song can only be played on "
						+ MusicalInstrument.TYPE_DESC[requiredInstrumentType]
								.toLowerCase() + ".");
			return null;
		}
		return instrument;
	...
	protected Vector getInvokerScopeRoomSet(MOB backupMob) {
		...
		CMLib.tracking().getRadiantRooms(invoker().location(), rooms, flags,
				null, depth, null);
	...
	protected String getCorrectMsgString(Room R, String str, int v) {
		...
		if (R == originRoom)
			...
			if (!songOf.equals(this.instrumentName()))
				songOf = "the " + songOf;
			if (dir >= 0)
				msgStr = "^SYou hear " + songOf + " being played "
	...
	public Set<MOB> sendMsgAndGetTargets(MOB mob, Room R, CMMsg msg,
			Environmental givenTarget, boolean auto) {
		if (originRoom == R)
			R.send(mob, msg);
		else
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			String str = auto ? "^S" + songOf() + " begins to play!^?"
					: "^S<S-NAME> begin(s) to play " + songOfStr
							+ instrumentName() + ".^?";
			if ((!auto) && (mob.fetchEffect(this.ID()) != null))
				str = "^S<S-NAME> start(s) playing " + songOfStr
			...
			for (int v = 0; v < commonRoomSet.size(); v++) {
				...
				if (R.okMessage(mob, msg)) {
					...
					Set<MOB> h = this.sendMsgAndGetTargets(mob, R, msg,
							givenTarget, auto);
					if (h == null)
						continue;
					...
					for (Iterator f = h.iterator(); f.hasNext();) {
						...
						int msgType = CMMsg.MASK_MAGIC | CMMsg.MASK_SOUND
								| CMMsg.TYP_CAST_SPELL;
						int mndMsgType = CMMsg.MASK_MAGIC | CMMsg.MASK_SOUND
								| CMMsg.MASK_MALICIOUS | CMMsg.TYP_MIND;
						if (auto) {
							msgType |= CMMsg.MASK_ALWAYS;
							mndMsgType |= CMMsg.MASK_ALWAYS;
						}
						if ((castingQuality(mob, follower) == Ability.QUALITY_MALICIOUS)
								&& (follower != mob))
						...
						if (CMLib.flags().canBeHeardSpeakingBy(invoker,
							...
							CMMsg msg2 = CMClass.getMsg(mob, follower, this,
									msgType | CMMsg.MASK_HANDS, null, msgType,
									null, msgType, null);
							CMMsg msg3 = msg2;
							if ((mindAttack()) && (follower != mob))
								msg2 = CMClass.getMsg(mob, follower, this,
										mndMsgType | CMMsg.MASK_HANDS, null,
										mndMsgType, null, mndMsgType, null);
							if ((R.okMessage(mob, msg2))
								...
								if (msg2.value() <= 0) {
									...
									if ((msg3.value() <= 0)
											&& (follower.fetchEffect(newOne
													.ID()) == null)) {
										if (persistantSong()) {
											newOne.setSavable(false);
											if (follower != mob)
												follower.addEffect((Ability) newOne
														.copyOf());
											else
