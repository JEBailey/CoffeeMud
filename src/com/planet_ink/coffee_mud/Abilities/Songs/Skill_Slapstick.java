...
public class Skill_Slapstick extends BardSkill {
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			if ((mob.isInCombat()) && (target instanceof MOB)
					&& (((MOB) target) != mob))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MASK_SOUND
					| CMMsg.MASK_HANDS | CMMsg.MASK_MOVE | CMMsg.TYP_JUSTICE
					| (auto ? CMMsg.MASK_ALWAYS : 0), str);
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				target.curState().adjMana(-mob.phyStats().level(),
						target.maxState());
			}
		} else
			return beneficialVisualFizzle(mob, target,
					"<S-NAME> attempt(s) to do something silly to <T-NAMESELF>, but fail(s).");
