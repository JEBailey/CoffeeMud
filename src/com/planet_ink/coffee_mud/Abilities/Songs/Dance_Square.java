...
public class Dance_Square extends Dance {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if (msg.amISource(invoker()) && (affected != invoker())
			...
			if (cmd != null) {
				MOB M = (MOB) affected;
				CMMsg omsg = CMClass.getMsg(invoker(), affected, null,
						CMMsg.MSG_ORDER, null);
				if (CMLib.flags().canBeHeardMovingBy(invoker(), M)
					...
					if (omsg.sourceMinor() == CMMsg.TYP_ORDER) {
						CMObject O = CMLib.english().findCommand(M,
								CMParms.parse(cmd));
						if ((O != null)
								&& ((!(O instanceof Command)) || (((Command) O)
										.canBeOrdered())))
							M.enqueCommand(CMParms.parse(cmd),
