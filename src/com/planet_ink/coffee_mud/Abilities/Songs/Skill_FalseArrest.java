...
public class Skill_FalseArrest extends BardSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (mob.location() != null) {
			...
			if ((B == null)
					|| (!B.hasWarrant(CMLib.law()
							.getLegalObject(mob.location()), target)))
				B = null;
			else
		...
		if (B == null)
			for (Enumeration e = CMLib.map().areas(); e.hasMoreElements();) {
				...
				if (CMLib.flags().canAccess(mob, A)) {
					...
					if ((B != null)
							&& (B.hasWarrant(CMLib.law().getLegalObject(A),
									target))) {
		...
		if (B == null) {
			mob.tell(target.name(mob)
					+ " is not wanted for anything, anywhere.");
			return false;
		}
		...
		if (!success) {
			beneficialWordsFizzle(mob, target,
					"<S-NAME> frown(s) at <T-NAMESELF>, but lose(s) the nerve.");
			return false;
		}
		CMMsg msg = CMClass.getMsg(mob, target, this,
				CMMsg.MSG_DELICATE_HANDS_ACT,
				"<S-NAME> frown(s) at <T-NAMESELF>.", CMMsg.NO_EFFECT, null,
				CMMsg.NO_EFFECT, null);
		if (mob.location().okMessage(mob, msg)) {
			mob.location().send(mob, msg);
			if (!B.arrest(A2, mob, target)) {
				mob.tell("You are not able to arrest " + target.name(mob)
						+ " at this time.");
				return false;
			}
		}
