...
public class Play_Symphony extends Play {
		...
		if (msg.targetMinor() == CMMsg.TYP_DAMAGE)
			...
			case CODE_UPDAMAGEPER5:
				if ((affected == invoker())
						...
						&& ((((Physical) msg.target()).fetchEffect(ID()) == null) || (((Physical) msg
								.target()).fetchEffect(ID()).invoker() != invoker()))) {
					int dmg = 0;
	...
	public void affectCharStats(MOB mob, CharStats stats) {
		...
		switch (toDoCode) {
		case CODE_UPSAVE:
			stats.setStat(toDoVal,
					stats.getStat(toDoVal) + adjustedLevel(invoker(), 0));
			break;
		case CODE_UPSTAT:
	...
	public void affectPhyStats(Physical mob, PhyStats stats) {
		...
		if (((toDoCode == CODE_UPENVPER2) || (toDoCode == CODE_UPENVPER5))
				&& (toDoString.length() > 0) && (invoker() != null)) {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		case CODE_UPMOVEMENT:
			((MOB) affected).curState().adjMovement(1,
					((MOB) affected).maxState());
			break;
		...
		case CODE_SPEEDCOMMONSKILLS: {
			MOB M = (MOB) affected;
			for (final Enumeration<Ability> a = M.effects(); a
					.hasMoreElements();) {
		...
		if ((per > 0)
				&& (CMLib.dice().rollPercentage() < ((addedLevel + adjustedLevel(
						invoker(), 0)) / per)))
			switch (toDoCode) {
			case CODE_CASTMALICIOUSSPELLPER10: {
				MOB M = (MOB) affected;
