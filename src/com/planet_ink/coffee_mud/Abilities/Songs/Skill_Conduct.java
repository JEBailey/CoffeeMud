...
public class Skill_Conduct extends BardSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			String str = auto ? "^SSymphonic Conduction Begins!^?"
					: "^S<S-NAME> begin(s) to wave <S-HIS-HER> arms in a mystical way!^?";
				...
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					if (CMLib.flags().canBeSeenBy(invoker, follower)) {
						...
						if (mob.location().okMessage(mob, msg2)) {
							...
							if (msg2.value() <= 0)
								SYMPHONY.invoke(follower, new Vector(), null,
										false, asLevel
												+ (3 * getXLEVELLevel(mob)));
						}
					}
				}
			...
			mob.location().show(mob, null, CMMsg.MSG_NOISE,
					"<S-NAME> wave(s) <S-HIS-HER> arms around, looking silly.");
