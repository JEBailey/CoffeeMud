...
public class Fighter_CalledStrike extends FighterSkill {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		affectableStats
				.setAttackAdjustment(affectableStats.attackAdjustment() - 100);
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amISource(mob) && (msg.amITarget(target))
			...
			if (hurtAmount >= (target.baseState().getHitPoints() / reqDivisor)) {
				...
				mob.tell(mob, target, null,
						"You failed to cut off <T-YOUPOSS> '" + gone + "'.");
			unInvoke();
		}
		return super.okMessage(myHost, msg);
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target.name(mob) + " already has a call against one of "
					+ target.charStats().hisher() + " limbs.");
			return false;
		}
		...
		if (mob.isMonster())
			gone = remainingLimbList.get(CMLib.dice().roll(1,
					remainingLimbList.size(), -1));
			...
			if ((off.equalsIgnoreCase("head"))
				...
				for (int i = 0; i < remainingLimbList.size(); i++)
					if (remainingLimbList.get(i).toUpperCase()
							.startsWith(off.toUpperCase())) {
		...
		if ((success) && (gone.length() > 0)) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					"^F^<FIGHT^><S-NAME> call(s) '" + gone + "'!^</FIGHT^>^?");
			...
			return maliciousFizzle(mob, null, "<S-NAME> call(s) '" + gone
					+ "', but fail(s) <S-HIS-HER> attack.");
