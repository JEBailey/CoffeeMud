...
public class Fighter_Spring extends FighterSkill {
	...
	private static final String[] triggerStrings = { "SPRINGATTACK", "SPRING",
			"SATTACK" };
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (mob.getVictim() == target) {
					msg = CMClass.getMsg(mob, target, this, CMMsg.MSG_RETREAT,
							"^F^<FIGHT^><S-NAME> spring(s) back!^</FIGHT^>^?");
					CMLib.color().fixSourceFightColor(msg);
					if (mob.location().okMessage(mob, msg)) {
						mob.location().send(mob, msg);
						if (mob.rangeToTarget() < mob.location().maxRange()) {
							msg = CMClass.getMsg(mob, target, this,
									CMMsg.MSG_RETREAT, null);
							if (mob.location().okMessage(mob, msg))
								mob.location().send(mob, msg);
						}
					}
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> fail(s) to spring attack <T-NAMESELF>.");
