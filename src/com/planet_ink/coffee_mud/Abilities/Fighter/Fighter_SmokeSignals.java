...
public class Fighter_SmokeSignals extends FighterSkill {
	...
	private static final String[] triggerStrings = { "SMOKESIGNALS",
			"SMOKESIGNAL" };
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((affected instanceof MOB) && (!msg.amISource((MOB) affected))
			...
			msg.addTrailerMsg(CMClass.getMsg((MOB) affected, null, null,
					CMMsg.MSG_OK_VISUAL, "The smoke signals seem to say '"
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> begin(s) smoking uncontrollably!"
									: "<S-NAME> puff(s) up a mighty series of smoke signals!");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				String str = CMParms.combine(commands, 0);
				CMMsg msg2 = CMClass.getMsg(mob, null, this, CMMsg.NO_EFFECT,
						null, CMMsg.NO_EFFECT, str, CMMsg.MSG_OK_VISUAL,
						"You see some smoke signals in the distance.");
				TrackingLibrary.TrackingFlags flags = new TrackingLibrary.TrackingFlags();
				List<Room> checkSet = CMLib.tracking().getRadiantRooms(
						mob.location(), flags, 50);
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to send a smoke signal, but goof(s) it up.");
		return success;
	}
}
