...
public class Fighter_Pin extends FighterSkill {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_DEATH) && (pairedWith != null)
				...
				if (mob == invoker) {
					if (mob.location() != null)
						mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
								"<S-NAME> release(s) <S-HIS-HER> pin.");
					else
						mob.tell("You release your pin.");
				} else {
					if (mob.location() != null)
						mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
								"<S-NAME> <S-IS-ARE> released from the pin");
					else
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> get(s) pinned!"
									: "^F^<FIGHT^><S-NAME> pin(s) <T-NAMESELF> to the floor!^</FIGHT^>^?");
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					...
					Fighter_Pin targetPin = (Fighter_Pin) target
							.fetchEffect(ID());
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to pin <T-NAMESELF>, but fail(s).");
