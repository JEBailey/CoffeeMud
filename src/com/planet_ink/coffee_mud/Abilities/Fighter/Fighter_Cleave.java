...
public class Fighter_Cleave extends FighterSkill {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((thisTarget != null) && (nextTarget != null)
				&& (thisTarget.amDead()) && (!nextTarget.amDead())
			...
			CMMsg msg = CMClass
					.getMsg(mob, nextTarget, this, CMMsg.MSG_NOISYMOVEMENT,
							"^F^<FIGHT^><S-NAME> CLEAVE(S) INTO <T-NAME>!!^</FIGHT^>^?");
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		MOB mob = (MOB) affected;
		if ((msg.amISource(mob)) && (mob.getVictim() != null)
				&& (msg.amITarget(mob.getVictim())) && (!msg.amITarget(mob))
			...
			if ((damAmount > victim.curState().getHitPoints())
				...
				for (int i = 0; i < mob.location().numInhabitants(); i++) {
					...
					if ((vic != null) && (vic.getVictim() == mob)
							&& (vic != mob) && (vic != victim)
							&& (!vic.amDead()) && (vic.rangeToTarget() == 0)) {
						nextTarget = vic;
						break;
					}
				}
