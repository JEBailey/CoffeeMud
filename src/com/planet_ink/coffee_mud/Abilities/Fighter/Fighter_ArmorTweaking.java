...
public class Fighter_ArmorTweaking extends FighterSkill {
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		...
		if (affected instanceof Item) {
			if (((Item) affected).amWearingAt(Wearable.IN_INVENTORY)
					|| ((invoker() != null) && (CMLib.flags().isInTheGame(
							invoker(), false) && (((Item) affected).owner() != invoker()))))
				unInvoke();
		}
	}
	...
	public void unInvoke() {
		if ((affected instanceof Item) && (!((Item) affected).amDestroyed())
				&& (((Item) affected).owner() instanceof MOB)) {
			MOB M = (MOB) ((Item) affected).owner();
			if ((!M.amDead()) && (CMLib.flags().isInTheGame(M, true))
					&& (!((Item) affected).amWearingAt(Wearable.IN_INVENTORY)))
				M.tell(M, affected, null,
						"<T-NAME> no longer feel(s) quite as snuggly tweaked.");
		}
		super.unInvoke();
	}

	public void affectPhyStats(Physical affected, PhyStats stats) {
		if ((affected instanceof Item) && (armorBonus > 0)
				&& (((Item) affected).owner() instanceof MOB)) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Item armor = super.getTarget(mob, null, givenTarget, null, commands,
		...
		if ((!armor.amWearingAt(Wearable.WORN_ABOUT_BODY))
			...
			mob.tell(armor.name()
					+ " can not be tweaked to provide any more benefit.");
			return false;
		}
		...
		int bonus = (int) Math.round(CMath.mul(
				0.10 + (0.10 * getXLEVELLevel(mob)), armor.phyStats().armor()));
		if (bonus < 1) {
			mob.tell(armor.name()
					+ " is too weak of an armor to provide any more benefit from tweaking.");
			return false;
		}
		...
		if (success) {
			String str = auto ? armor.name() + " snuggly covers <S-NAME>!"
					: "<S-NAME> tweak(s) <T-NAMESELF> until it is as snuggly protective as possible.";
			CMMsg msg = CMClass.getMsg(mob, armor, this,
			...
			return beneficialVisualFizzle(mob, armor,
					"<S-NAME> attempt(s) to tweak <T-NAME>, but just can't get it quite right.");
		return success;
	}
