...
public class Fighter_FlyingKick extends FighterSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		boolean success = proficiencyCheck(mob,
				(mob.charStats().getStat(CharStats.STAT_DEXTERITY) - target
						.charStats().getStat(CharStats.STAT_DEXTERITY)) * 2,
				auto);
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				CMLib.combat().postDamage(
						...
						Weapon.TYPE_BASHING,
						"^F^<FIGHT^><S-NAME> <DAMAGE> <T-NAME> with a flying KICK!^</FIGHT^>^?"
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> fail(s) to land a flying kick on <T-NAMESELF>.");
