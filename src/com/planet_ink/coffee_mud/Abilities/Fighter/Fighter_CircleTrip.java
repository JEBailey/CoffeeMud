	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		CMMsg msg = CMClass
				.getMsg(mob,
						...
						auto ? ""
								: "^F^<FIGHT^><S-NAME> slide(s) into a circle trip!^<FIGHT^>^?");
			...
			for (Iterator e = h.iterator(); e.hasNext();) {
				...
				if (CMLib.flags().isInFlight(target)) {
					mob.tell(target.name(mob)
							+ " is flying and can't be tripped!");
					return false;
				}
				...
				int adjustment = (-levelDiff)
						+ (-(35 + ((int) Math
								.round(((target.charStats()
										.getStat(CharStats.STAT_DEXTERITY)) - 9.0) * 3.0))));
				...
				if (success) {
					...
					return maliciousFizzle(mob, target,
							"<S-NAME> attempt(s) to circle trip <T-NAMESELF>, but fail(s).");
			}
		} else
			success = false;
		return success;
