...
public class Fighter_FieldTactics extends FighterSkill {
	...
	public boolean hiding(Environmental mob) {
		if (!(mob instanceof MOB))
			return false;
		return CMLib.flags().isSitting((MOB) mob)
				&& (((MOB) mob).riding() == null);
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((affected instanceof MOB) && (activated)
			...
			msg.setValue(msg.value()
					+ (int) Math.round(CMath.mul(msg.value(), CMath.div(
							proficiency(),
							400.0 - (20.0 * getXLEVELLevel(msg.source()))))));
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((affected instanceof MOB) && (((MOB) affected).location() != null)
			...
			if ((hidden)
					&& ((System.currentTimeMillis() - sitTime) > (60 * 2 * 1000)))
