...
public class Fighter_BodyToss extends MonkSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (mob.rangeToTarget() > 0) {
			mob.tell("You must get closer to " + target.charStats().himher()
					+ " first!");
			return false;
		}
		if (CMLib.flags().isSitting(mob)) {
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							"^F^<FIGHT^><S-NAME> pick(s) up <T-NAMESELF> and toss(es) <T-HIM-HER> into the air!^</FIGHT^>^?");
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				CMLib.combat().postDamage(mob, target, this,
						CMLib.dice().roll(1, 12, 0),
						CMMsg.MASK_ALWAYS | CMMsg.TYP_JUSTICE,
						Weapon.TYPE_BASHING,
						"The hard landing <DAMAGE> <T-NAME>!");
			...
			return beneficialVisualFizzle(mob, target,
					"<S-NAME> attempt(s) to pick up <T-NAMESELF>, but fail(s).");
