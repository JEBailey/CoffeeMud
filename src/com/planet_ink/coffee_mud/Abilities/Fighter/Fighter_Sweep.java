...
public class Fighter_Sweep extends FighterSkill {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		affectableStats.setAttackAdjustment((int) Math.round(CMath.div(
				affectableStats.attackAdjustment(), 2.0 - f)));
		affectableStats.setDamage((int) Math.round(CMath.div(
				affectableStats.damage(), 3.0 - f)));
	}
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				for (Iterator e = h.iterator(); e.hasNext();) {
					...
					msg = CMClass.getMsg(mob, target, this,
							CMMsg.MSK_MALICIOUS_MOVE | CMMsg.TYP_OK_ACTION
