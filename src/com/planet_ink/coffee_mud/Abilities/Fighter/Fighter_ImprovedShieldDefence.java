...
public class Fighter_ImprovedShieldDefence extends FighterSkill {
	...
	public String name() {
		return "Improved Shield Defence";
	}
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		if ((!(affected instanceof MOB)) || (amountOfShieldArmor <= 0))
			return;
		affectableStats
				.setArmor(affectableStats.armor()
						- ((int) Math.round(CMath.mul(amountOfShieldArmor,
								(CMath.div(proficiency()
										+ (5.0 * getXLEVELLevel(invoker())),
										100.0))))));
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob))
				...
				&& (amountOfShieldArmor > 0) && (mob.isInCombat())
				&& (CMLib.dice().rollPercentage() == 1) && (!mob.amDead()))
