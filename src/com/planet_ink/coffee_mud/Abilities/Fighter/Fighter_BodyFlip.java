...
public class Fighter_BodyFlip extends FighterSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.riding() != null) {
			mob.tell("You can't flip someone "
		...
		int levelDiff = target.phyStats().level()
				- ((2 * getXLEVELLevel(mob)) + mob.phyStats().level());
		if (levelDiff > 0)
			levelDiff = levelDiff * 5;
		else
			levelDiff = 0;
		int adjustment = (-levelDiff)
				+ (-((int) Math.round(((target.charStats()
						.getStat(CharStats.STAT_STRENGTH)) - 9.0) * 3.0)));
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> flip(s) over!"
									: "^F^<FIGHT^><S-NAME> flip(s) <T-NAMESELF> over!^</FIGHT^>^?");
			CMLib.color().fixSourceFightColor(msg);
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				maliciousAffect(mob, target, asLevel, 2, -1);
				target.location().show(target, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> hit(s) the floor!");
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to flip <T-NAMESELF> over, but fail(s).");
		return success;
	}
}
