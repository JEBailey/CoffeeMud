...
public class Fighter_PointBlank extends FighterSkill {
	...
	protected void cloneFix(Ability E) {
		super.cloneFix(E);
		qualifiedWeapons = new XVector<Weapon>(
				((Fighter_PointBlank) E).qualifiedWeapons);
	}
		...
		if ((affected instanceof Weapon)
			...
			if ((W.weaponClassification() == Weapon.CLASS_RANGED)
				...
				if (((msg.targetMinor() == CMMsg.TYP_WEAR)
						|| (msg.targetMinor() == CMMsg.TYP_WIELD) || (msg
						.targetMinor() == CMMsg.TYP_HOLD))
						&& (!qualifiedWeapons.contains(W))
						&& ((msg.source().fetchAbility(ID()) == null) || proficiencyCheck(
								null, 0, false))) {
					...
					W.addEffect(A);
					W.recoverPhyStats();
				} else if (((msg.targetMinor() == CMMsg.TYP_REMOVE) || (msg
		...
		if (--checkDown <= 0) {
			...
			if ((w != null)
					...
					&& ((mob.fetchAbility(ID()) == null) || proficiencyCheck(
							null, 0, false))) {
				if ((CMLib.dice().rollPercentage() < 5) && (mob.isInCombat())
						&& (mob.rangeToTarget() == 0))
