...
public class Fighter_Fragmentation extends FighterSkill {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((msg.target() instanceof Weapon) && (msg.tool() == this))
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((affected instanceof MOB)
			...
			CMMsg msg2 = CMClass.getMsg((MOB) msg.target(), msg.tool(), this,
					CMMsg.MSG_OK_VISUAL,
					"^F^<FIGHT^><T-NAME> fragment(s) in <S-NAME>!^</FIGHT^>^?");
			CMLib.color().fixSourceFightColor(msg2);
			msg.addTrailerMsg(msg2);
			msg.setValue(msg.value()
					+ (int) Math.round(CMath.mul(3.0 * msg.value(), CMath.div(
							proficiency(),
							100.0 - (10.0 * getXLEVELLevel(invoker()))))));
		}
