...
public class Fighter_CoupDeGrace extends FighterSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!auto) {
			...
			if (!CMLib.flags().isSleeping(mob.getVictim())) {
				if (CMLib.flags().isSitting(mob.getVictim()))
					mob.tell(mob.getVictim().charStats().HeShe()
							+ " is on the ground, but not prone!");
				else
					mob.tell(mob.getVictim().charStats().HeShe()
							+ " is not prone!");
				return false;
		...
		int chance = (-levelDiff)
				+ (-(target.charStats().getStat(CharStats.STAT_CONSTITUTION) * 2));
		...
		if ((success)
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				CMLib.combat()
						.postDamage(
								...
								auto ? ""
										: "^F^<FIGHT^><S-NAME> rear(s) back and Coup-de-Grace(s) <T-NAME>!^</FIGHT^>^?"
												+ CMLib.protocol().msp(
														"decap.wav", 30));
			...
			String str = auto ? ""
					: "<S-NAME> attempt(s) a Coup-de-Grace and fail(s)!";
			CMMsg msg = CMClass.getMsg(mob, target, null, CMMsg.MASK_MALICIOUS
					| CMMsg.MSG_OK_ACTION, str);
			if (mob.location().okMessage(mob, msg))
				mob.location().send(mob, msg);
		}
		return success;
