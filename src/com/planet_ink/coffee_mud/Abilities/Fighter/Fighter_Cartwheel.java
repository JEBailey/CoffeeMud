...
public class Fighter_Cartwheel extends FighterSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			// what happened.
			CMMsg msg = CMClass.getMsg(mob, victim, this, CMMsg.MSG_RETREAT,
					"<S-NAME> cartwheel(s) away from <T-NAMESELF>!");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				int tries = 2;
				while (((--tries) >= 0)
						&& (mob.rangeToTarget() < mob.location().maxRange())) {
					msg = CMClass.getMsg(mob, victim, this, CMMsg.MSG_RETREAT,
							null);
					if (mob.location().okMessage(mob, msg))
						mob.location().send(mob, msg);
				}
			...
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to cartwheel and fail(s).");
