		...
		if (invoker != null) {
			...
			affectableStats.setAttackAdjustment(affectableStats
					...
					- (int) Math.round(CMath.div(
							affectableStats.attackAdjustment(), 2.0 + f)));
		}
	}
			...
			if ((msg.tool() != null) && (!mob.amDead())
				...
				if (!((MOB) msg.target()).amDead())
					msg.addTrailerMsg(CMClass.getMsg((MOB) msg.target(),
							...
							"<S-NAME> tumble(s) around the attack from <T-NAME>."));
				if ((++hits) >= 2)
					unInvoke();
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> begin(s) tumbling around!"
							: "<S-NAME> tumble(s) around!");
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to tumble, but goof(s) it up.");
		return success;
	}
}
