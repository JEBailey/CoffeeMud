...
public class Fighter_TrueShot extends FighterSkill {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((((Weapon) w).weaponClassification() == Weapon.CLASS_RANGED)
			...
			gettingBonus = true;
			int bonus = (int) Math.round(CMath.mul(affectableStats
					.attackAdjustment(), (CMath.div(proficiency(),
					200.0 - (10 * getXLEVELLevel(invoker()))))));
			affectableStats.setAttackAdjustment(affectableStats
					.attackAdjustment() + bonus);
		} else
			gettingBonus = false;
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amISource(mob)) && (gettingBonus)
				&& (msg.targetMinor() == CMMsg.TYP_WEAPONATTACK)
				&& (CMLib.dice().rollPercentage() > 95) && (mob.isInCombat())
				&& (!mob.amDead()) && (msg.target() instanceof MOB))
