...
public class Fighter_Kick extends FighterSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		boolean success = proficiencyCheck(mob,
				(mob.charStats().getStat(CharStats.STAT_STRENGTH) - target
						.charStats().getStat(CharStats.STAT_STRENGTH)) * 2,
				auto);
		if (success) {
			...
			if ((R != null) && (R.okMessage(mob, msg))) {
				...
				CMLib.combat().postDamage(
						...
						this,
						damage,
						CMMsg.MASK_ALWAYS | CMMsg.MASK_SOUND | CMMsg.MASK_MOVE
								| CMMsg.TYP_JUSTICE,
						Weapon.TYPE_BASHING,
						"^F^<FIGHT^><S-NAME> <DAMAGE> <T-NAME> with a ferocious KICK!^</FIGHT^>^?"
								+ CMLib.protocol().msp("bashed1.wav", 30));
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> fail(s) to kick <T-NAMESELF>.");
