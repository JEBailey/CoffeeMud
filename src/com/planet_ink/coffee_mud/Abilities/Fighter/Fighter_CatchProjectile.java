	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(mob)
				...
				&& ((mob.fetchAbility(ID()) == null) || proficiencyCheck(null,
						-85 + mob.charStats().getStat(CharStats.STAT_DEXTERITY)
								+ (3 * getXLEVELLevel(mob)), false))
				&& (mob.freeWearPositions(Wearable.WORN_HELD, (short) 0,
						(short) 0) > 0)) {
			...
			} else if ((w instanceof AmmunitionWeapon)
			...
			if (mob.location().isContent(w)) {
				CMMsg msg2 = CMClass.getMsg(mob, w, msg.source(),
						CMMsg.MSG_GET,
						"<S-NAME> catch(es) the <T-NAME> shot by <O-NAME>!");
