	...
	public int castingQuality(MOB mob, Physical target) {
		if ((mob != null) && (target != null)) {
			...
			if ((target instanceof MOB)
					&& (mob.baseWeight() < (((MOB) target).baseWeight() / 2)))
				return Ability.QUALITY_INDIFFERENT;
			if (anyWeapons(mob))
				return Ability.QUALITY_INDIFFERENT;
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (CMLib.flags().isGolem(target)) {
			mob.tell(target, null, null,
					"You can't hurt <S-NAMESELF> with Atemi Strike.");
			return false;
		}
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> hit(s) the floor!"
									: "^F^<FIGHT^><S-NAME> deliver(s) a deadly Atemi strike to <T-NAMESELF>!^</FIGHT^>^?");
			CMLib.color().fixSourceFightColor(msg);
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> do(es) not look well.");
					success = maliciousAffect(
							mob,
							target,
							asLevel,
							((2 * getXLEVELLevel(mob)) + mob.phyStats().level()) / 3,
							-1);
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) the deadly Atemi strike on <T-NAMESELF>, but fail(s).");
