...
public class Fighter_LightningStrike extends MonkSkill {
	...
	private static final String[] triggerStrings = { "LIGHTNINGSTRIKE",
			"LSTRIKE" };
	...
	public int castingQuality(MOB mob, Physical target) {
		if ((mob != null) && (target != null)) {
			...
			if (mob.charStats().getStat(CharStats.STAT_DEXTERITY) < CMProps
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!auto)
				&& (mob.charStats().getStat(CharStats.STAT_DEXTERITY) < CMProps
						.getIntVar(CMProps.Int.BASEMAXSTAT))) {
			mob.tell("You need at least an "
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^F^<FIGHT^><S-NAME> unleash(es) a flurry of lightning strikes against <T-NAMESELF>!^</FIGHT^>^?");
			CMLib.color().fixSourceFightColor(msg);
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				int num = getXLEVELLevel(mob)
				...
				if ((!anyWeapons(mob)) && (msg.value() <= 0)) {
					mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> collapse(s) in exhaustion.");
			...
			return maliciousFizzle(
					mob,
					target,
					"<S-NAME> attempt(s) to flurry <T-NAMESELF> with lighting strikes, but fail(s).");
