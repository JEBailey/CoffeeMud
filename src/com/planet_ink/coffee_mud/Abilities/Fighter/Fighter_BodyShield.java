...
public class Fighter_BodyShield extends FighterSkill {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(mob) && (msg.targetMinor() == CMMsg.TYP_DAMAGE)
				&& (!mob.amDead()) && (!CMLib.flags().isSleeping(mob))
				&& (msg.source() != mob.getVictim()) && (msg.source() != mob)
				&& ((msg.value()) > 0) && (msg.tool() != null)
				&& (msg.tool() instanceof Weapon) && (mob.getVictim() != null)
			...
			if ((A != null) && (A.invoker() == mob)) {
				doneThisRound = true;
				int regain = (int) Math.round(CMath.mul((msg.value()),
						CMath.div(proficiency(), 100.0)));
				msg.setValue(msg.value() - regain);
				CMMsg msg2 = CMClass.getMsg(mob, mob.getVictim(), this,
						CMMsg.MSG_DAMAGE,
						"<S-NAME> use(s) <T-NAMESELF> as a body shield!");
