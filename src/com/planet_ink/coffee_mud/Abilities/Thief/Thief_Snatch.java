...
public class Thief_Snatch extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		MOB target = (auto && (givenTarget instanceof MOB)) ? (MOB) givenTarget
				: mob.getVictim();
		...
		if (weapon == null) {
			...
			return false;
		} else if (mob.freeWearPositions(Wearable.WORN_HELD, (short) 0,
				(short) 0) > 0) {
		...
		if ((success)
				&& ((hisWeapon.rawProperLocationBitmap() == Wearable.WORN_WIELD) || (hisWeapon
						...
						+ Wearable.WORN_HELD))) {
			CMMsg msg = CMClass.getMsg(target, hisWeapon, null, CMMsg.MSG_DROP,
					null);
			CMMsg msg2 = CMClass.getMsg(mob, null, this, CMMsg.MSG_THIEF_ACT,
					null);
			if ((mob.location().okMessage(mob, msg))
				...
				mob.location().show(mob, target, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> disarm(s) <T-NAMESELF>!");
				if (mob.location().isContent(hisWeapon)) {
					CMLib.commands().postGet(mob, null, hisWeapon, true);
					if (mob.isMine(hisWeapon)) {
						msg = CMClass
								.getMsg(mob, hisWeapon, null, CMMsg.MSG_HOLD,
										"<S-NAME> snatch(es) the <T-NAME> out of mid-air!");
						if (mob.location().okMessage(mob, msg))
			...
			maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to disarm <T-NAMESELF> and fail(s)!");
		return success;
	}
}
