...
public class Thief_PowerGrab extends ThiefSkill {
	...
	public double castingTime(final MOB mob, final List<String> cmds) {
		return CMProps.getActionSkillCost(ID(), 0.0);
	}

	public double combatCastingTime(final MOB mob, final List<String> cmds) {
		return CMProps.getCombatActionSkillCost(ID(), 0.0);
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!success)
			beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to power grab something and fail(s).");
		else {
			CMMsg msg = CMClass
					.getMsg(mob,
							target,
							...
							CMMsg.MSG_DELICATE_SMALL_HANDS_ACT
									| CMMsg.MASK_MAGIC,
							auto ? ""
									: "^S<S-NAME> carefully attempt(s) to acquire <T-NAME>^?");
