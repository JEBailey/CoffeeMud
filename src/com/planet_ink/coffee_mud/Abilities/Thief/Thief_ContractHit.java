...
public class Thief_ContractHit extends ThiefSkill {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected != null) && (affected instanceof MOB)
			...
			if (readyToHit
				...
				if (level > (invoker.phyStats().level() + (2 * super
						.getXLEVELLevel(invoker))))
					level = (invoker.phyStats().level() + (2 * super
							.getXLEVELLevel(invoker)));
				...
				for (int i = 0; i < num; i++) {
					...
					M.baseState().setHitPoints(
							CMLib.dice()
									.rollHP(level, M.basePhyStats().level()));
	...
	public void unInvoke() {
		...
		if ((M != null) && (M2 != null) && (((done) || (M2.amDead())))) {
			if (M.location() != null) {
				M.location().showHappens(
						CMMsg.MSG_OK_VISUAL,
						"Someone steps out of the shadows and whispers something to "
								+ M.name() + ".");
				M.tell("'It is done.'");
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		try {
			V = CMLib.map().findInhabitants(CMLib.map().rooms(), mob,
					CMParms.combine(commands, 0), 10);
		...
		if (!mob.mayIFight(target)) {
			mob.tell("You are not allowed to put out a hit on "
					+ target.name(mob) + ".");
			return false;
		}
		...
		if (CMLib.beanCounter().getTotalAbsoluteValue(mob, localCurrency) < goldRequired) {
			...
			mob.tell("You'll need at least " + costWords
					+ " to put a hit out on " + target.name(mob) + ".");
			return false;
		}
		...
		CMMsg msg = CMClass
				.getMsg(mob,
						...
						CMMsg.MSG_THIEF_ACT,
						CMMsg.MSG_THIEF_ACT,
						"<S-NAME> whisper(s) to a dark figure stepping out of the shadows.  The person nods and slips away.");
