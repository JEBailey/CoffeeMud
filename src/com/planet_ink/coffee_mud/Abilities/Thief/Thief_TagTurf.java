...
public class Thief_TagTurf extends ThiefSkill {
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		...
		if ((msg.tool() instanceof Ability)
			...
			msg.source().tell(
					"You definitely aren't allowed to do that on " + text()
							+ "'s turf.");
			return false;
		}
		return true;
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		...
		if ((msg.target() == affected)
				...
				&& ((CMLib.flags().canSeeHidden(msg.source())) || (msg.source()
						.Name().equals(text())))) {
			if ((msg.source().Name().equals(text()))
					|| ((msg.source().getClanRole(text()) != null) && CMLib
							.clans().checkClanPrivilege(msg.source(),
									Clan.Function.CLAN_BENEFITS)))
				msg.addTrailerMsg(CMClass.getMsg(msg.source(), msg.target(),
						null, CMMsg.MSG_OK_VISUAL, "This is your turf.",
						CMMsg.NO_EFFECT, null, CMMsg.NO_EFFECT, null));
			else
				msg.addTrailerMsg(CMClass.getMsg(msg.source(), msg.target(),
						null, CMMsg.MSG_OK_VISUAL,
						"This turf has been claimed by " + text() + ".",
						CMMsg.NO_EFFECT, null, CMMsg.NO_EFFECT, null));
		}
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (A != null) {
			...
			if ((A.text().equals(mob.Name()) || ((clanRole != null) && (clanRole.second
					.intValue() >= clanRole.first.getGovernment()
					.getAcceptPos())))
		...
		if ((!CMLib.law().doesOwnThisProperty(mob, mob.location()))
				...
				&& (CMLib.law().getLandTitle(mob.location()).getOwnerName()
						.length() > 0)) {
		...
		CMMsg msg = CMClass.getMsg(mob, null, this, auto ? CMMsg.MASK_ALWAYS
				...
				CMMsg.MSG_OK_VISUAL, auto ? ""
						: "<S-NAME> tag(s) this place as <S-HIS-HER> turf.");
		if (!success)
			return beneficialVisualFizzle(
					mob,
					null,
					auto ? ""
							: "<S-NAME> attempt(s) to tag this place, but can't get into it.");
