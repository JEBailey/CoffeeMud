...
public class Thief_DeathTrap extends ThiefSkill implements Trap {
	...
	public void spring(MOB M) {
		if ((!sprung)
				&& (CMLib.dice().rollPercentage()
						+ (2 * getXLEVELLevel(invoker())) > M.charStats()
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((msg.targetMinor() == CMMsg.TYP_ENTER)
				...
				&& ((canBeUninvoked()) || (!CMLib.law()
						.doesHavePriviledgesHere(msg.source(), (Room) affected)))
				&& (CMLib.dice().rollPercentage() > msg.source().charStats()
						.getSave(CharStats.STAT_SAVE_TRAPS)))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Item resource = CMLib.materials().findMostOfMaterial(mob.location(),
				RawMaterial.MATERIAL_METAL);
		if (resource == null)
			resource = CMLib.materials().findMostOfMaterial(mob.location(),
					RawMaterial.MATERIAL_MITHRIL);
		int amount = 0;
		if (resource != null)
			amount = CMLib.materials().findNumberOfResource(mob.location(),
					resource.material());
		if (amount < 100) {
		...
		if (resource != null)
			CMLib.materials().destroyResourcesValue(mob.location(), 100,
					resource.material(), -1, null);

		CMMsg msg = CMClass.getMsg(mob, trapThis, this,
				auto ? CMMsg.MSG_OK_ACTION : CMMsg.MSG_THIEF_ACT,
				CMMsg.MASK_ALWAYS | CMMsg.MSG_DELICATE_HANDS_ACT,
				CMMsg.MSG_OK_ACTION, (auto ? trapThis.name()
						+ " begins to glow!"
						: "<S-NAME> attempt(s) to lay a trap here."));
		if (mob.location().okMessage(mob, msg)) {
			...
			if (success) {
				...
				setTrap(mob,
						...
						(CMLib.ableMapper().qualifyingClassLevel(mob, this) - CMLib
								.ableMapper().lowestQualifyingLevel(ID())) + 1,
						false);
				Thief_DeathTrap T = (Thief_DeathTrap) trapThis
						.fetchEffect(ID());
				if (T != null)
