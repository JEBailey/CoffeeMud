...
public class Thief_Evesdrop extends ThiefSkill {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.targetMinor() == CMMsg.TYP_TELL) && (msg.target() == affected)
				&& (affected instanceof MOB) && (msg.source() != invoker())
				&& (invoker() != null)
			...
			if (targMsg != null) {
				...
				if (x >= 0)
					targMsg = targMsg.substring(0, x + 8) + affected.Name()
							+ targMsg.substring(x + 11);
			}
			msg2.modify(msg.source(), invoker(), msg.tool(), msg.sourceCode(),
					msg.sourceMessage(), msg.targetCode(), targMsg,
	...
	public void unInvoke() {
		if (canBeUninvoked()) {
			if ((invoker != null) && (affected != null))
				invoker.tell("You are no longer evesdropping on "
						+ affected.name() + ".");
		}
		super.unInvoke();
	}
		...
		if (A != null) {
			if (A.invoker() == mob)
				...
				mob.tell(mob, target, null,
						"It is too crowded to evesdrop on <T-NAME>.");
				return false;
			}
		}
		if (mob.isInCombat()) {
		...
		if (!success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							CMMsg.MSG_OK_VISUAL,
							auto ? ""
									: "Your attempt to evesdrop on <T-NAMESELF> fails; <T-NAME> spots you!",
							CMMsg.MSG_OK_VISUAL,
							auto ? ""
									: "You spot <S-NAME> trying to evesdrop on you.",
							CMMsg.NO_EFFECT, null);
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							"You are now evesdroping on <T-NAME>.  Enter 'evesdrop <targetname>' again to disengage.",
