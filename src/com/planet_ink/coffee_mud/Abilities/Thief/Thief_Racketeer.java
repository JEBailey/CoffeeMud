...
public class Thief_Racketeer extends ThiefSkill {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((!msg.source().Name().equals(text()))
			...
			if (invoker() == source) {
				source.tell(((Physical) msg.target()).name(source)
						+ " is currently under your protection.");
			} else {
				source.tell(((Physical) msg.target()).name(source)
						+ " is under " + invoker().name(source)
						+ "'s protection.");
				invoker().tell(
						"Word on the street is that " + source.name(invoker())
								+ " is hassling "
								+ ((Physical) msg.target()).name(invoker())
		...
		if (mob != null) {
			...
			if (target instanceof PhysicalAgent) {
				...
				if ((CMLib.coffeeShops().getShopKeeper(target) == null)
						&& (AE.fetchBehavior("MoneyChanger") == null)
						&& (AE.fetchBehavior("ItemMender") == null)
						&& (AE.fetchBehavior("ItemIdentifier") == null)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (A != null) {
			if (A.invoker() == mob)
				...
				mob.tell(target.name(mob) + " is already under "
						+ A.invoker().name(mob) + "'s protection.");
				A.invoker().tell(
						"Word on the street is that " + mob.name(A.invoker())
		...
		if (!super.invoke(mob, commands, givenTarget, auto, asLevel))
			return false;
		double amount = CMLib.dice().roll(proficiency(),
				target.phyStats().level(), 0);
