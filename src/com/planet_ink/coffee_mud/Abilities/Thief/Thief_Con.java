...
public class Thief_Con extends ThiefSkill {
	...
	public double castingTime(final MOB mob, final List<String> cmds) {
		return CMProps.getActionSkillCost(ID(),
				CMath.div(CMProps.getIntVar(CMProps.Int.DEFABLETIME), 20.0));
	}
	...
	public boolean preInvoke(MOB mob, List<String> commands,
		...
		if (secondsElapsed > 0) {
			if ((secondsElapsed % 4) == 0)
				return mob.location().show(
						mob,
						target,
						CMMsg.MSG_SPEAK,
						"^T<S-NAME> continue(s) conning <T-NAMESELF> to '"
		...
		CMMsg msg = CMClass.getMsg(
				...
				this,
				CMMsg.MSG_SPEAK,
				"^T<S-NAME> attempt(s) to con <T-NAMESELF> to '"
						+ CMParms.combine(commands, 0) + "'.^?");
	...
	public boolean conCheck(MOB mob, List<String> commands,
			Environmental givenTarget, boolean auto, int asLevel) {
		if (commands != null)
			commands = new XVector<String>(commands);
		if (commands.size() < 1) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int levelDiff = ((mob.phyStats().level() + (2 * super
				.getXLEVELLevel(mob))) - target.phyStats().level()) * 10;
		if (levelDiff > 0)
			levelDiff = 0;
		boolean success = proficiencyCheck(mob,
				(mob.charStats().getStat(CharStats.STAT_CHARISMA) * 2)
						+ levelDiff, auto);
