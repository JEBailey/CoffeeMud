...
public class Thief_Distract extends ThiefSkill {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		affectableStats.setArmor(affectableStats.armor()
				+ super.getXLEVELLevel(invoker()) + 30 + abilityCode());
		affectableStats
				.setAttackAdjustment((affectableStats.attackAdjustment() - (int) Math
						.round(CMath.div(affectableStats.attackAdjustment(),
								2.0 - f)))
						- abilityCode());
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (invoker.location() != mob.location())
			...
			if (msg.amISource(invoker)
					&& (CMLib.dice().rollPercentage() > (mob.charStats()
							.getStat(CharStats.STAT_WISDOM) * 2))
					&& (msg.targetMinor() == CMMsg.TYP_WEAPONATTACK)) {
				invoker.location().show(invoker, mob, CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> distract(s) <T-NAME>.");
				return false;
			}
		}
	...
	public void unInvoke() {
		...
		if (canBeUninvoked()) {
			if (!mob.amDead()) {
				if ((invoker != null) && (invoker.location() == mob.location())
						&& (!invoker.amDead()))
					invoker.tell(invoker, mob, null,
							"You are no longer distracting <T-NAMESELF>.");
				if ((mob.location() != null) && (!mob.amDead()))
					mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> <S-IS-ARE> no longer so distracted.");
			}
		}
		super.unInvoke();
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!CMLib.flags().aliveAwakeMobileUnbound(mob, false))
			return false;
		if (mob.isInCombat() && (mob.rangeToTarget() > 0)) {
			mob.tell("You are too far away to distract "
					+ mob.getVictim().name() + "!");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MASK_MALICIOUS
					| CMMsg.MSG_THIEF_ACT,
					auto ? "<T-NAME> seem(s) distracted!"
							: "<S-NAME> distract(s) <T-NAMESELF>!");
			...
			return beneficialVisualFizzle(mob, target,
					"<S-NAME> attempt(s) to distract <T-NAMESELF>, but flub(s) it.");
		return success;
	}
}
