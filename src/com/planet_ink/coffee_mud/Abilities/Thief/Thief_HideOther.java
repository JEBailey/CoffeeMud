...
public class Thief_HideOther extends ThiefSkill {
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_SAVE_DETECTION, prof + bonus
		...
		if (!success) {
			if (highestMOB != null)
				beneficialVisualFizzle(mob, target,
						"<S-NAME> attempt(s) to hide <T-NAMESELF> from "
								+ highestMOB.name(mob) + " and fail(s).");
			else
				beneficialVisualFizzle(mob, target,
						"<S-NAME> attempt(s) to hide <T-NAMESELF> and fail(s).");
		} else {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				super.beneficialAffect(mob, target, asLevel,
						Ability.TICKS_ALMOST_FOREVER);
				Thief_HideOther newOne = (Thief_HideOther) target
						.fetchEffect(ID());
				if (newOne != null) {
