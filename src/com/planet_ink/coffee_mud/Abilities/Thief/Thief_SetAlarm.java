...
public class Thief_SetAlarm extends ThiefSkill implements Trap {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(affected)) && (msg.targetMinor() == CMMsg.TYP_OPEN)) {
			if ((!msg.amISource(invoker()) && (CMLib.dice().rollPercentage() > msg
					.source().charStats().getSave(CharStats.STAT_SAVE_TRAPS))))
				spring(msg.source());
		}
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		if (!super.tick(ticking, tickID))
			return false;
		if ((affected == null) || (!(affected instanceof Exit))
		...
		if (sprung) {
			...
			CMLib.tracking().getRadiantRooms(room1, rooms, flags, null,
					10 + (getXLEVELLevel(invoker()) * 2), null);
			CMLib.tracking().getRadiantRooms(room2, rooms, flags, null,
					10 + (getXLEVELLevel(invoker()) * 2), null);
			Vector mobsDone = new Vector();
			room1.showHappens(CMMsg.MSG_NOISE,
					"A horrible alarm is going off here.");
			room2.showHappens(CMMsg.MSG_NOISE,
					"A horrible alarm is going off here.");
			for (int r = 0; r < rooms.size(); r++) {
				...
				if ((R != room1) && (R != room2)) {
					...
					if (dir >= 0) {
						R.showHappens(CMMsg.MSG_NOISE, "You hear a loud alarm "
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		CMMsg msg = CMClass.getMsg(
				...
				CMMsg.MSG_OK_ACTION,
				(auto ? alarmThis.name() + " begins to glow!"
						: "<S-NAME> attempt(s) to lay a trap on "
								+ alarmThis.name() + "."));
			...
			if (success) {
				...
				if (CMLib.dice().rollPercentage() > 50) {
					...
					mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> trigger(s) the alarm on accident!");
