...
public class Thief_KillLog extends ThiefSkill {
	...
	public MOB getMark(MOB mob) {
		Thief_Mark A = null;
		if ((lastMarker != null) && (lastMarker.affecting() == mob)
				&& (!lastMarker.amDestroyed()) && (lastMarker.mark != null))
			A = lastMarker;
		if (A == null) {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((mark != null) && msg.amISource(mark)
			...
			if (V != null)
				for (int i = 0; i < V.size(); i++) {
					...
					if (ablk.tag.equalsIgnoreCase("MOB")) {
						...
						one[0] = CMLib.xml().getValFromPieces(ablk.contents,
								"NAME");
						one[1] = CMLib.xml().getValFromPieces(ablk.contents,
								"LEVEL");
						one[2] = CMLib.xml().getValFromPieces(ablk.contents,
								"TOTAL");
						one[3] = CMLib.xml().getValFromPieces(ablk.contents,
								"KILLS");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (proficiencyCheck(mob, 0, auto)) {
			...
			str.append(CMStrings.padRight("Name", cols[0])
					+ CMStrings.padRight("Level", cols[1]) + "Kill Pct.\n\r");
			...
			for (int i = 0; i < order.size(); i++) {
				...
				str.append(CMStrings.padRight(one[0], cols[0])
						+ CMStrings.padRight(one[1], cols[1]) + pct + "%\n\r");
			}
			if (mob.session() != null)
