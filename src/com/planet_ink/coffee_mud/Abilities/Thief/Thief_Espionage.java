	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.othersMajor(CMMsg.MASK_CHANNEL))) {
			...
			ChannelsLibrary.CMChannel chan = CMLib.channels().getChannel(
					channelInt);
			boolean areareq = chan.flags
					.contains(ChannelsLibrary.ChannelFlag.SAMEAREA);
			if ((chan.flags.contains(ChannelsLibrary.ChannelFlag.CLANONLY) || chan.flags
					.contains(ChannelsLibrary.ChannelFlag.CLANALLYONLY))
					&& (invoker() != null)
					&& ((!CMLib.clans()
							.isAnyCommonClan(invoker(), msg.source())) && ((!chan.flags
							.contains(ChannelFlag.CLANALLYONLY)) || (!CMLib
							.clans().findAnyClanRelations(invoker(),
									msg.source(), Clan.REL_ALLY))))
					&& (!CMLib.channels().mayReadThisChannel(msg.source(),
							areareq, invoker(), channelInt)))
	...
	public void unInvoke() {
		if (canBeUninvoked()) {
			if ((invoker != null) && (affected != null))
				invoker.tell("You are no longer committing espionage with "
						+ affected.name() + ".");
		}
		super.unInvoke();
	}
		...
		if (A != null) {
			if (A.invoker() == mob) {
				...
				mob.tell(mob, target, null,
						"It is too crowded to commit espionage with <T-NAME>.");
				return false;
			}
		}
		if (mob.isInCombat()) {
		...
		if (!success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							CMMsg.MSG_OK_VISUAL,
							auto ? ""
									: "Your attempt to commit espionage using <T-NAMESELF> fails; <T-NAME> spots you!",
							CMMsg.MSG_OK_VISUAL,
							auto ? ""
									: "You spot <S-NAME> trying to commit espionage through you.",
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							"You are now committing espionage with <T-NAME>.  Enter 'espionage <targetname>' again to disengage.",
