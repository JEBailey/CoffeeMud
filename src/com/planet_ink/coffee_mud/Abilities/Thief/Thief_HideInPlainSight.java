...
public class Thief_HideInPlainSight extends ThiefSkill {
		...
		if ((msg.target() == affected)
			...
			if (msg.source() == affected) {
				if (!msg.othersMajor(CMMsg.MASK_SOUND))
					...
					return makeObscurinator(msg.source())
							.okMessage(myHost, msg);
			} else if ((msg.sourceMinor() != CMMsg.TYP_SPEAK)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int levelDiff = mob.phyStats().level() - getMOBLevel(highestMOB)
				- (this.getXLEVELLevel(mob) * 2);
		...
		if (!success) {
			if (highestMOB != null)
				beneficialVisualFizzle(
						mob,
						highestMOB,
						"<S-NAME> step(s) to the side of <T-NAMESELF>, but end(s) up looking like an idiot.");
			else
				beneficialVisualFizzle(mob, null,
						"<S-NAME> step(s) to the side and look(s) like an idiot.");
		} else {
