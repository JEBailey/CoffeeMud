...
public class Thief_Shadow extends ThiefSkill {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (((msg.targetMinor() == CMMsg.TYP_LEAVE) || (msg.targetMinor() == CMMsg.TYP_FLEE))
				...
				&& ((shadowing.riding() == null) || (msg.source().riding() != shadowing
						.riding()))) {
			int dir = -1;
			...
			if ((dir >= 0) && (msg.source().location() != lastRoom)) {
				...
				if (!mob.isMonster())
					mob.enqueCommand(CMParms.parse(directionWent),
							Command.METAFLAG_FORCED, 0);
				else
					CMLib.tracking().walk(mob, dir, false, false);
			}
		}
		if ((shadowing != null) && (invoker != null)
				&& (shadowing.location() == invoker.location()))
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		if ((shadowing != null)
				&& (shadowing.location() == affected.location()))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int levelDiff = target.phyStats().level()
				- (mob.phyStats().level() + abilityCode() + (super
		...
		if (!success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							CMMsg.MSG_OK_VISUAL,
							auto ? ""
									: "Your attempt to shadow <T-NAMESELF> fails; <T-NAME> spots you!",
							CMMsg.MSG_OK_VISUAL,
							auto ? ""
									: "You spot <S-NAME> trying to shadow you.",
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							"You are now shadowing <T-NAME>.  Enter 'shadow' again to disengage.",
							CMMsg.NO_EFFECT, null, CMMsg.NO_EFFECT, null);
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				shadowing = target;
				if (beneficialAffect(mob, target, asLevel,
						Ability.TICKS_FOREVER)) {
