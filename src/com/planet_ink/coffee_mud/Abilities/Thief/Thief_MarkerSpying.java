...
public class Thief_MarkerSpying extends ThiefSkill {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if (super.canBeUninvoked()) {
			...
			if (mark != affected) {
				MOB invoker = invoker();
				unInvoke();
				if ((mark != null) && (mark.fetchEffect(ID()) == null)
						&& (invoker != null))
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.targetMinor() == CMMsg.TYP_READ) && (msg.source() == affected)
				&& (msg.target() != null) && (invoker() != null)
				&& (CMLib.flags().isInTheGame(invoker(), true))
				&& (getMark(invoker()) == msg.source())) {
			CMMsg msg2 = (CMMsg) msg.copyOf();
			msg2.modify(invoker(), msg.target(), msg.tool(), msg.sourceCode(),
					msg.sourceMessage(), msg.targetCode(), msg.targetMessage(),
					CMMsg.NO_EFFECT, null);
			invoker().tell(
					"You remember something else from " + msg.source().Name()
							+ "'s papers:");
		...
		if (A != null) {
			if (A.invoker() == mob)
				...
				mob.tell(mob, target, null,
						"It is too crowded to spy on <T-NAME>.");
				return false;
			}
		}
		...
		if (!success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							CMMsg.MSG_OK_VISUAL,
							auto ? ""
									: "Your attempt to spy on <T-NAMESELF> fails; <T-NAME> spots you!",
							CMMsg.MSG_OK_VISUAL,
							auto ? ""
									: "You spot <S-NAME> trying to spy on you.",
							CMMsg.NO_EFFECT, null);
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							"You are now spying on <T-NAME>.  Enter 'spy <targetname>' again to disengage.",
