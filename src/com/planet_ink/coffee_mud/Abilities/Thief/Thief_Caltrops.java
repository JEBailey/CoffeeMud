...
public class Thief_Caltrops extends ThiefSkill implements Trap {
	...
	public void spring(MOB mob) {
		if ((!invoker().mayIFight(mob))
			...
			CMLib.combat().postDamage(
					...
					mob,
					null,
					CMLib.dice().roll(1, 6, adjustedLevel(invoker(), 0)),
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(CMStrings.capitalizeFirstLetter(caltropTypeName())
					+ "Caltrops have already been tossed down here.");
			return false;
		}
		...
		if (success) {
			if (mob.location()
					.show(mob,
							...
							(auto ? CMMsg.MASK_ALWAYS : 0)
									| CMMsg.MSG_THIEF_ACT,
							"<S-NAME> throw(s) down " + caltropTypeName()
									+ "caltrops!"))
			...
			maliciousFizzle(mob, target,
					"<S-NAME> fail(s) to throw down <S-HIS-HER> "
							+ caltropTypeName() + "caltrops properly.");
		return success;
	}
}
