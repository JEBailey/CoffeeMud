...
public class Thief_Autosneak extends ThiefSkill {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected instanceof MOB) && (!noRepeat)
			...
			if (dir >= 0) {
				...
				if (A != null) {
					noRepeat = true;
					if (A.invoke(mob,
							CMParms.parse(Directions.getDirectionName(dir)),
							null, false, 0)) {
						int[] usage = A.usageCost(mob, false);
						if (CMath.bset(A.usageType(), Ability.USAGE_HITPOINTS)
								&& (usage[USAGEINDEX_HITPOINTS] > 0))
							mob.curState().adjHitPoints(
									usage[USAGEINDEX_HITPOINTS] / 2,
									mob.maxState());
						if (CMath.bset(A.usageType(), Ability.USAGE_MANA)
								&& (usage[USAGEINDEX_MANA] > 0))
							mob.curState().adjMana(usage[USAGEINDEX_MANA] / 2,
									mob.maxState());
						if (CMath.bset(A.usageType(), Ability.USAGE_MOVEMENT)
								&& (usage[USAGEINDEX_MOVEMENT] > 0))
							mob.curState().adjMovement(
									usage[USAGEINDEX_MOVEMENT] / 2,
									mob.maxState());
					}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to get into <S-HIS-HER> sneaking stance, but fail(s).");
		return success;
	}
