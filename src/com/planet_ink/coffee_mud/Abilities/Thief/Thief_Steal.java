		...
		if (!success) {
			if (CMLib.dice().rollPercentage() > discoverChance) {
				...
				CMMsg msg = CMClass
						.getMsg(mob,
								...
								auto ? ""
										: "You fumble the attempt to steal; <T-NAME> spots you!",
								CMMsg.MSG_NOISYMOVEMENT,
								auto ? ""
										: "<S-NAME> tries to steal from you and fails!",
								CMMsg.MSG_NOISYMOVEMENT,
								auto ? ""
										: "<S-NAME> tries to steal from <T-NAME> and fails!");
			...
			if (!auto)
				if ((stolen != null)
					...
					str = "<S-NAME> steal(s) " + stolen.name()
							+ " from <T-NAMESELF>.";
				else {
					code = CMMsg.MSG_QUIETMOVEMENT;
					str = "<S-NAME> attempt(s) to steal from <T-HIM-HER>, but it doesn't appear "
