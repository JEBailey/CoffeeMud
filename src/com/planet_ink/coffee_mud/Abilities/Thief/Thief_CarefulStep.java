...
public class Thief_CarefulStep extends ThiefSkill {
	...
	public double castingTime(final MOB mob, final List<String> cmds) {
		return CMProps.getActionSkillCost(ID(),
				CMath.div(CMProps.getIntVar(CMProps.Int.DEFABLETIME), 50.0));
	}
	...
	public boolean preInvoke(MOB mob, List<String> commands,
		...
		if (secondsElapsed == 0) {
			...
			CMMsg msg = CMClass.getMsg(
					...
					"<S-NAME> start(s) walking carefully "
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int levelDiff = mob.phyStats().level()
				+ (2 * super.getXLEVELLevel(mob)) - getMOBLevel(highestMOB);
		...
		CMMsg msg = CMClass.getMsg(mob, null, this, auto ? CMMsg.MSG_OK_VISUAL
				: CMMsg.MSG_DELICATE_HANDS_ACT, "<S-NAME> walk(s) carefully "
				+ Directions.getDirectionName(dirCode) + ".");
		if (mob.location().okMessage(mob, msg)) {
			mob.location().send(mob, msg);
			if (levelDiff < 0)
