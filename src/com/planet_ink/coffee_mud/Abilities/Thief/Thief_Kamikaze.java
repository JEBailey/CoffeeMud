...
public class Thief_Kamikaze extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!target.mayIFight(mob))
			...
			mob.tell("You can't talk " + target.name(mob)
					+ " into a kamikaze mission.");
			return false;
		}

		if ((s.length() == 0) || (CMParms.parse(s).size() == 0)) {
			mob.tell("Send " + target.charStats().himher()
					+ " which direction?");
			return false;
		}
		...
		double goldRequired = ((Math
				.round((100.0 - ((mob.charStats().getStat(
						CharStats.STAT_CHARISMA) + (2.0 * getXLEVELLevel(mob))) * 2.0))) * target
				.phyStats().level()));
		...
		if (bombFound == null) {
			mob.tell(target.name(mob)
					+ " must have some bombs for this to work.");
			return false;
		}
		...
		if (!success) {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MSG_SPEAK,
					"^T<S-NAME> attempt(s) to convince <T-NAMESELF> to kamikaze "
							+ s + ", but no deal is reached.^?");
			if (mob.location().okMessage(mob, msg))
				mob.location().send(mob, msg);
		} else {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MSG_SPEAK,
					"^T<S-NAME> pay(s) <T-NAMESELF> to Kamikaze " + s + " for "
							+ costWords + ".^?");
			...
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				CMLib.beanCounter().addMoney(target, localCurrency,
						goldRequired);
