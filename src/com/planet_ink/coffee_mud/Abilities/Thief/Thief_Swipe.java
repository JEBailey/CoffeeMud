...
public class Thief_Swipe extends ThiefSkill {
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			...
			if (((MOB) target).amDead()
				...
				return Ability.QUALITY_INDIFFERENT;
			if ((mob.isInCombat())
					&& (CMLib.flags().aliveAwakeMobile((MOB) target, true) || (mob
							.getVictim() != target)))
				return Ability.QUALITY_INDIFFERENT;
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((mob.isInCombat())
				&& (CMLib.flags().aliveAwakeMobile(target, true) || (mob
						.getVictim() != target))) {
			mob.tell(mob, mob.getVictim(), null,
					"Not while you are fighting <T-NAME>!");
			return false;
		}
		...
		if (levelDiff > 0)
			...
			levelDiff = -(levelDiff * ((!CMLib.flags().canBeSeenBy(mob, target)) ? 1
					: 2));
		if (!CMLib.flags().aliveAwakeMobile(target, true)) {
			levelDiff = 100;
			discoverChance = 0;
		}
		...
		if (!success) {
			if (CMLib.dice().rollPercentage() > discoverChance) {
				...
				CMMsg msg = CMClass
						.getMsg(mob,
								...
								auto ? ""
										: "You fumble the swipe; <T-NAME> spots you!",
								CMMsg.MSG_NOISYMOVEMENT,
								auto ? ""
										: "<S-NAME> tries to pick your pocket and fails!",
								CMMsg.MSG_OK_VISUAL,
								auto ? ""
										: "<S-NAME> tries to pick <T-NAME>'s pocket and fails!");
			...
			if (levelDiff > 10)
				pct = 0.05;
			double goldTaken = CMLib.beanCounter().getTotalAbsoluteNativeValue(
					target)
					* pct * Math.random();
			if (goldTaken < (CMLib.ableMapper().qualifyingClassLevel(mob, this)))
				goldTaken = CMLib.ableMapper().qualifyingClassLevel(mob, this);
			if (goldTaken > CMLib.beanCounter().getTotalAbsoluteNativeValue(
					target))
				goldTaken = CMLib.beanCounter().getTotalAbsoluteNativeValue(
						target);
			String goldTakenStr = CMLib.beanCounter().nameCurrencyShort(target,
					goldTaken);
			...
			if (!auto)
				if (goldTaken > 0)
					str = "<S-NAME> pick(s) <T-HIS-HER> pocket for "
							+ goldTakenStr + ".";
				else {
