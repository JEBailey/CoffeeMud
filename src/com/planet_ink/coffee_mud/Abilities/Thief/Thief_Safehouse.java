...
public class Thief_Safehouse extends ThiefSkill {
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		...
		if ((msg.target() == affected)
				...
				&& (affected instanceof Room) && (isLaw(msg.source()))) {
			msg.source().tell(
					"You don't think there's anything going on in there.");
			return false;
		}
		return true;
	}
		...
		if (R != null) {
			...
			if (law != null) {
				...
				for (int r = 0; r < R.numInhabitants(); r++) {
					...
					if ((M != null)
							&& (law.isAnyOfficer(A, M) || law.isJudge(A, M)))
						return true;
				}
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!isGoodSafehouse(target)) {
			...
			List<Room> V = CMLib.tracking().getRadiantRooms(target, flags,
			...
			if ((isGoodSafehouse(R)) && (!isLawHere(R))) {
				V = CMLib.tracking().findBastardTheBestWay(target,
						new XVector(R), flags, 50 + (2 * getXLEVELLevel(mob)));
				StringBuffer trail = new StringBuffer("");
				int dir = CMLib.tracking().trackNextDirectionFromHere(V,
						target, true);
				while (target != R) {
					...
					dir = CMLib.tracking().trackNextDirectionFromHere(V,
							target, true);
				}
				if (target == R)
					mob.tell("You happen to know of one nearby though.  Go: "
							+ trail.toString());
			}
			return false;
		}
		...
		CMMsg msg = CMClass.getMsg(mob, null, this, auto ? CMMsg.MASK_ALWAYS
				...
				CMMsg.MSG_OK_VISUAL, auto ? ""
						: "<S-NAME> hide(s) out from the law here.");
		if (!success)
