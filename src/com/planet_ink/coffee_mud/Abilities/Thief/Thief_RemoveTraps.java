...
public class Thief_RemoveTraps extends ThiefSkill {
	...
	private static final String[] triggerStrings = { "DETRAP", "UNTRAP",
			"REMOVETRAPS" };
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((commands.size() > 0)
				&& (commands.lastElement() instanceof Boolean)) {
		...
		if ((unlockThis instanceof Room)
			...
			if ((CMLib.law().doesOwnThisProperty(mob, R))
					|| ((R2 != null) && (CMLib.law().doesOwnThisProperty(mob,
							R2))))
				permanent = true;
			if (dirCode >= 0) {
		...
		CMMsg msg = CMClass.getMsg(mob, unlockThis, this,
				...
				auto ? unlockThis.name() + " begins to glow."
						: "<S-NAME> attempt(s) to safely deactivate a trap on "
								+ unlockThis.name() + ".");
		if ((success) && (!lastDone.contains("" + unlockThis))) {
			while (lastDone.size() > 40)
				lastDone.removeElementAt(0);
			lastDone.addElement("" + unlockThis);
			msg.setValue(1); // this is to notify that the thief gets xp from
								// doing this.
		} else
			msg.setValue(0);
