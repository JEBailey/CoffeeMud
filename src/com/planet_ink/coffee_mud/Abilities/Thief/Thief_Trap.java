...
public class Thief_Trap extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int qualifyingClassLevel = CMLib.ableMapper().qualifyingClassLevel(mob,
				this)
				+ (getXLEVELLevel(mob))
				- CMLib.ableMapper().qualifyingLevel(mob, this) + 1;
		if (qualifyingClassLevel > maxLevel())
			qualifyingClassLevel = maxLevel();
		for (Enumeration<Ability> a = CMClass.abilities(); a.hasMoreElements();) {
			Ability A = a.nextElement();
			if ((A instanceof Trap) && (!((Trap) A).isABomb())
		...
		if (trapThis != null) {
			...
			while (((++cuts) < 100) && (theTrap == null)) {
				theTrap = (Trap) traps.elementAt(CMLib.dice().roll(1,
						traps.size(), -1));
				if (!theTrap.canSetTrapOn(mob, trapThis))
					theTrap = null;
			}
		} else if (CMParms.combine(commands, 0).equalsIgnoreCase("list")) {
			StringBuffer buf = new StringBuffer(CMStrings.padRight("Trap Name",
					15)
					+ " "
					+ CMStrings.padRight("Affects", 17)
					+ " Requires\n\r");
			...
			if (commands.size() < 2) {
				mob.tell("Trap what, with what kind of trap? Use " + cmdWord
						+ " list for a list.");
				return false;
			}
			String name;
			...
			if (theTrap == null) {
				mob.tell("'" + name + "' is not a valid trap name.  Try "
						+ cmdWord.toUpperCase() + " LIST.");
				return false;
			}
			...
			if (trapThis == null)
				trapThis = this.getAnyTarget(mob, commands, givenTarget,
		...
		boolean success = proficiencyCheck(
				mob,
				+((mob.phyStats().level() + (getXLEVELLevel(mob) * 2) - trapThis
		...
		CMMsg msg = CMClass
				.getMsg(mob,
						...
						CMMsg.MASK_ALWAYS | CMMsg.MSG_THIEF_ACT,
						CMMsg.MSG_OK_ACTION,
						(auto ? trapThis.name() + " begins to glow!"
								: "<S-NAME> attempt(s) to lay a trap on <T-NAMESELF>."));
			...
			if (success) {
				...
				if ((trapThis instanceof Room)
					...
					if ((CMLib.law().doesOwnThisProperty(mob, R))
							|| ((R2 != null) && (CMLib.law()
									.doesOwnThisProperty(mob, R2))))
						permanent = true;
				}
				if (theTrap != null) {
					theTrap.setTrap(mob, trapThis, getXLEVELLevel(mob),
							adjustedLevel(mob, asLevel), permanent);
					if (permanent)
						CMLib.database().DBUpdateRoom(mob.location());
				}
			} else {
				if ((CMLib.dice().rollPercentage() > 50) && (theTrap != null)) {
					Trap T = theTrap.setTrap(mob, trapThis,
							getXLEVELLevel(mob), adjustedLevel(mob, asLevel),
							false);
					mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> trigger(s) the trap on accident!");
					T.spring(mob);
				} else {
