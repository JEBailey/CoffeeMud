...
public class Thief_Robbery extends ThiefSkill {
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			...
			if ((!((MOB) target).mayIFight(mob))
					|| (CMLib.coffeeShops().getShopKeeper(target) == null))
				return Ability.QUALITY_INDIFFERENT;
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int levelDiff = target.phyStats().level()
				- (mob.phyStats().level() + (getXLEVELLevel(mob) * 2));
		...
		if (stolen != null) {
			ShopKeeper.ShopPrice price = CMLib.coffeeShops().sellingPrice(
					target, mob, stolen, shop, false);
			if ((stolen instanceof Ability)
				...
				mob.tell(mob, target, stolen,
						"You cannot rob '<O-NAME>' from <T-NAME>.");
				return false;
			}
		...
		int discoverChance = (mob.charStats().getStat(CharStats.STAT_CHARISMA) - (target
				.charStats().getStat(CharStats.STAT_WISDOM)) * 5)
				+ (getX1Level(mob) * 5);
		...
		if (!success) {
			if (CMLib.dice().rollPercentage() > discoverChance) {
				CMMsg msg = CMClass
						.getMsg(mob,
								...
								auto ? ""
										: "You fumble the attempt to rob <T-NAMESELF>; <T-NAME> spots you!",
								CMMsg.MSG_NOISYMOVEMENT,
								auto ? ""
										: "<S-NAME> tries to rob you and fails!",
								CMMsg.MSG_NOISYMOVEMENT,
								auto ? ""
										: "<S-NAME> tries to rob <T-NAME> and fails!");
				...
				mob.tell(mob, target, null, auto ? ""
						: "You fumble the attempt to rob <T-NAME>.");
		} else {
			String str = null;
			int code = CMMsg.MSG_THIEF_ACT;
			if (!auto)
				if (stolen != null)
					str = "<S-NAME> rob(s) " + stolen.name()
							+ " from <T-NAMESELF>.";
				else {
					str = "<S-NAME> attempt(s) to rob <T-HIM-HER>, but it doesn't appear "
			...
			CMMsg msg = CMClass.getMsg(mob, target, this, code, str, hisCode,
					hisStr, CMMsg.NO_EFFECT, null);
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				Thief_Robbery A = (Thief_Robbery) target.fetchEffect(ID());
				if (A == null)
				...
				if ((!target.isMonster()) && (mob.isMonster())
					...
					if (mob.getVictim() == target)
						...
						&& (!alreadyFighting)
						&& ((stolen == null) || (CMLib.dice().rollPercentage() > stolen
								.phyStats().level()))) {
					if (target.getVictim() == mob)
						target.makePeace();
				}
				if (stolen != null) {
					List<Environmental> products = shop.getShop()
							.removeSellableProduct(stolen.Name(), mob);
					if (products.get(0) instanceof Item) {
						stolen = (Item) products.get(0);
						mob.location().addItem((Item) stolen,
