...
public class Thief_SetDecoys extends ThiefSkill implements Trap {
	...
	public void spring(MOB mob) {
		...
		if ((!invoker().mayIFight(mob))
				...
				|| (CMLib.dice().rollPercentage() < mob.charStats().getSave(
						...
						- (getXLEVELLevel(invoker()) * 5)))
			R.show(mob,
					affected,
					this,
					CMMsg.MSG_OK_ACTION,
					"A decoy pops up, prompting <S-NAME> to glance toward(s) it, but <S-HE-SHE> <S-IS-ARE> not fooled.");
		else if (R.show(mob, null, this, CMMsg.MSG_OK_VISUAL,
				"A decoy pops up, confusing <S-NAME>!")) {
			int max = R.maxRange();
			...
			while ((mob.isInCombat()) && (mob.rangeToTarget() < max)) {
				...
				if (!R.show(mob, mob.getVictim(), null, CMMsg.MSG_RETREAT,
						"<S-NAME> advance(s) toward(s) the decoy."))
					break;
			...
			for (int m = 0; m < R.numInhabitants(); m++) {
				...
				if (M != null) {
					if (CMLib.flags().canBeSeenBy(R, M)) {
						numWhoCanSee++;
						combat = combat
								|| ((M != mob) && (M.getVictim() == mob));
					}
				}
			}
			MOB target = null;
			int tries = 20;
			while (combat && (R.numInhabitants() > 1) && (target == null)
					&& ((--tries) >= 0)) {
				target = R.fetchRandomInhabitant();
				if ((target == mob) || (target.getVictim() != mob)
						|| (!CMLib.flags().canBeSeenBy(R, target)))
					target = null;
			}
			if (target != null)
				spring(target);
				...
				R.showHappens(
						CMMsg.MSG_OK_VISUAL,
						"A decoy pops up, causing everyone's gaze to be momentarily distracted towards it.");
			else if (numWhoCanSee == 1)
				R.showHappens(CMMsg.MSG_OK_VISUAL,
						"A decoy pops up, causing your gaze to be momentarily distracted towards it.");
		}
		return true;
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			if (mob.location().show(mob, target,
					(auto ? CMMsg.MASK_ALWAYS : 0) | CMMsg.MSG_THIEF_ACT,
					"<S-NAME> set(s) several decoys around the room."))
				maliciousAffect(mob, target, asLevel, 0, -1);
			else
				success = false;
		} else
			maliciousFizzle(mob, target,
					"<S-NAME> fail(s) to set <S-HIS-HER> decoys properly.");
		return success;
	}
}
