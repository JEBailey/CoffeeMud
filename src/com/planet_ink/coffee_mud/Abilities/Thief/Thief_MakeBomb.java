...
public class Thief_MakeBomb extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (trapThis != null)
			theTrap = (Trap) traps.elementAt(CMLib.dice().roll(1, traps.size(),
					-1));
		else if (CMParms.combine(commands, 0).equalsIgnoreCase("list")) {
			StringBuffer buf = new StringBuffer(CMStrings.padRight("Bomb Name",
					colWidth) + " Requires\n\r");
			...
			if (theTrap == null) {
				mob.tell("'" + name
						+ "' is not a valid bomb name.  Try BOMB LIST.");
				return false;
			}
		...
		boolean success = proficiencyCheck(
				mob,
				+((mob.phyStats().level() + (getXLEVELLevel(mob) * 3) - trapThis
			...
			if (success) {
				...
				if (CMLib.dice().rollPercentage() > 50) {
					...
					mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> set(s) the bomb off on accident!");
					T.spring(mob);
				} else {
