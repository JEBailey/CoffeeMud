	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MASK_MALICIOUS
					| CMMsg.MSG_THIEF_ACT,
					"<S-NAME> tar(s) and feather(s) <T-NAMESELF>!");
			if (mob.location().okMessage(mob, msg)) {
				...
				if (I != null) {
					...
					for (int i = 0; i < codes.all_ordered().length; i++) {
						...
						if ((!CMath.bset(target.charStats()
								.getWearableRestrictionsBitmap(), code))
					...
					for (int i = 0; i < Race.BODY_WEARGRID.length; i++) {
						if ((target.charStats().getBodyPart(i) <= 0)
							...
							wearCode = CMath.unsetb(wearCode,
									Race.BODY_WEARGRID[i][0]);
					}
					...
					long thetime = (long) CMProps
							.getIntVar(CMProps.Int.TICKSPERMUDDAY) * 3;
					B.setParms("notrigger=1 answer=dissolves! min=" + thetime
							+ " max=" + thetime + " chance=100");
					I.addBehavior(B);
				}
			}
		} else
			maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to tar and feather <T-NAMESELF>, but fail(s).");
		return success;
	}
}
