	...
	public void spring(MOB mob) {
		if ((!invoker().mayIFight(mob))
			...
			mob.location().show(
					...
					this,
					CMMsg.MSG_OK_ACTION,
					"<S-NAME> avoid(s) looking at some " + caltropTypeName()
							+ "caltrops on the floor.");
		else if (mob.curState().getMana() > 6) {
			mob.curState().adjMana(
					-CMLib.dice().roll(3 + getX1Level(mob), 8, 20),
					mob.maxState());
			mob.location()
					.show(invoker(),
							mob,
							...
							"The "
									+ caltropTypeName()
									+ "caltrops on the ground sparkle and confuse <T-NAME>");
		}
