...
public class Thief_Appraise extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (mob.location().okMessage(mob, msg)) {
			...
			if (!success) {
				double deviance = CMath.div(CMLib.dice().roll(1, 100, 0) + 50,
						100);
				realValue = CMath.mul(realValue, deviance);
				materialCode = CMLib.dice().roll(1, RawMaterial.CODES.TOTAL(),
						-1);
			...
			str.append(target.name(mob) + " is made of "
					+ RawMaterial.CODES.NAME(materialCode));
			str.append(" is worth about "
					+ CMLib.beanCounter().nameCurrencyShort(mob, realValue)
					+ ".");
			if (target instanceof Armor)
				str.append("\n\r" + target.name(mob) + " is a size " + height
						+ ".");
			if (weight != allWeight)
				str.append("\n\rIt weighs " + weight + " pounds empty and "
						+ allWeight + " pounds right now.");
			else
