...
public class Thief_Comprehension extends ThiefSkill {
	...
	public String name() {
		return "Linguistic Comprehension";
	}

	public String displayText() {
		return "(Lang. Comprehension)";
	}
	...
	private static final String[] triggerStrings = { "COMPREHEND",
			"COMPREHENSION" };
	...
	public boolean tick(Tickable ticking, int tickID) {
		if (!super.tick(ticking, tickID))
			return false;
		if ((queue.size() > 0) && (affected instanceof MOB)
		...
		if ((affected instanceof MOB)
			...
			if (str != null) {
				if (CMath.bset(msg.sourceMajor(), CMMsg.MASK_CHANNEL))
					queue.addElement(CMClass.getMsg(
							msg.source(),
							null,
							null,
							CMMsg.NO_EFFECT,
					...
					if (msg.target() != null)
						otherMes = CMLib.coffeeFilter().fullOutFilter(
								((MOB) affected).session(), (MOB) affected,
					...
					queue.addElement(CMClass.getMsg(
							...
							msg.othersCode(),
							CMMsg.NO_EFFECT,
							CMStrings.substituteSayInMessage(otherMes, str)
									+ " (translated from "
									+ ((Ability) msg.tool()).name() + ")"));
				}
			}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> feel(s) more comprehending."
									: "^S<S-NAME> listen(s) with intense comprehension.^?");
			...
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to listen with comprehension, but fail(s) miserably.");
