...
public class Thief_Panhandling extends ThiefSkill {
	...
	public void executeMsg(Environmental oking, CMMsg msg) {
		...
		if (affected instanceof MOB) {
			...
			if ((msg.source() == mob) && (msg.target() == mob.location())
				...
				msg.addTrailerMsg(CMClass.getMsg(mob, msg.source(),
						CMMsg.MSG_SPEAK,
						"^T<S-NAME> say(s) 'Thank you gov'ner!' to <T-NAME> ^?"));
		}
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (affected instanceof MOB) {
			...
			for (int i = 0; i < mob.location().numInhabitants(); i++) {
				...
				if ((mob2 != null) && (CMLib.flags().canBeSeenBy(mob2, mob))
						&& (mob2 != mob) && (!mobsHitUp.contains(mob2))
						&& (proficiencyCheck(mob, 0, false))) {
					switch (CMLib.dice().roll(1, 10, 0)) {
					case 1:
						CMLib.commands().postSay(mob, mob2,
								"A little something for a vet please?", false,
								false);
						break;
					case 2:
						CMLib.commands()
								.postSay(
										...
										mob2,
										"Spare a gold piece "
												+ ((mob2.charStats().getStat(
														CharStats.STAT_GENDER) == 'M') ? "mister?"
														: "madam?"), false,
										false);
						break;
					case 3:
						CMLib.commands().postSay(mob, mob2,
								"Spare some change?", false, false);
						break;
					case 4:
						CMLib.commands()
								.postSay(
										...
										"Please "
												+ ((mob2.charStats().getStat(
														CharStats.STAT_GENDER) == 'M') ? "mister"
														: "madam")
												+ ", a little something for an poor soul down on "
					...
					case 5:
						CMLib.commands()
								.postSay(
										mob,
										mob2,
										"Hey, I lost my 'Will Work For Food' sign.  Can you spare me the money to buy one?",
										false, false);
						break;
					case 6:
						CMLib.commands().postSay(mob, mob2,
								"Spread a little joy to an poor soul?", false,
								false);
						break;
					case 7:
						CMLib.commands().postSay(mob, mob2, "Change?", false,
								false);
						break;
					case 8:
						CMLib.commands().postSay(mob, mob2,
								"Can you spare a little change?", false, false);
						break;
					case 9:
						CMLib.commands().postSay(mob, mob2,
								"Can you spare a little gold?", false, false);
						break;
					case 10:
						CMLib.commands().postSay(
								mob,
								mob2,
								"Gold piece for a poor soul down on "
										+ mob.charStats().hisher() + " luck?",
								false, false);
						break;
					}
					if (CMLib.dice().rollPercentage() > (mob2.charStats()
							.getSave(CharStats.STAT_SAVE_JUSTICE) + (CMLib
							.flags().isGood(mob) ? 10 : 0))) {
						double total = CMLib.beanCounter()
								.getTotalAbsoluteNativeValue(mob2);
						if (total > 1.0) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already panhandling.");
			return false;
		}
		...
		CMMsg msg = CMClass.getMsg(mob, null, this, auto ? CMMsg.MASK_ALWAYS
				...
				CMMsg.MSG_DELICATE_SMALL_HANDS_ACT, auto ? ""
						: "<S-NAME> start(s) panhandling.");
		if (!success)
			return beneficialVisualFizzle(
					mob,
					null,
					auto ? ""
							: "<S-NAME> can't seem to get <S-HIS-HER> panhandling act started.");
