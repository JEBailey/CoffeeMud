...
public class Thief_Snipe extends ThiefSkill {
		...
		if (mob != null) {
			...
			if (target instanceof MOB) {
				...
				if ((w == null) || (!(w instanceof Weapon)))
					return Ability.QUALITY_INDIFFERENT;
				Weapon ww = (Weapon) w;
				if (((ww.weaponClassification() != Weapon.CLASS_RANGED) && (ww
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		String str = auto ? ""
				: "<S-NAME> strike(s) <T-NAMESELF> from the shadows!";
		int otherCode = success ? code : CMMsg.NO_EFFECT;
		String otherStr = success ? str : null;
		CMMsg msg = CMClass.getMsg(mob, target, this, code, str, otherCode,
				otherStr, otherCode, otherStr);
		if (mob.location().okMessage(mob, msg)) {
			boolean alwaysInvis = CMath.bset(mob.basePhyStats().disposition(),
