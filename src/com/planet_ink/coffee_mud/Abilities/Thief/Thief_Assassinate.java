...
public class Thief_Assassinate extends ThiefSkill {
		...
		if (tickID == Tickable.TICKID_MOB) {
			...
			if (room.isInhabitant(tracking)) {
				if (CMLib.flags().isHidden(mob)) {
					...
					CMLib.combat().postAttack(mob, tracking,
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amISource(mob)) && (msg.amITarget(mob.location()))
			...
			nextDirection = CMLib.tracking().trackNextDirectionFromHere(
					theTrail, mob.location(), true);
	}

	public void unInvoke() {
		MOB mob = (affected instanceof MOB) ? (MOB) affected : null;
		super.unInvoke();
		if ((mob != null) && (!mob.amDead()) && (mob.isMonster())
				&& (!CMLib.flags().isMobile(mob))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!mob.isMonster()) && (mob.fetchEffect("Thief_Mark") != null)) {
			...
			MOB M = ((givenTarget instanceof MOB) && (((MOB) givenTarget)
					.location() == mob.location())) ? (MOB) givenTarget : mob
					.location().fetchInhabitant(mobName);
			if (M != null) {
		...
		if (tracking != null) {
			...
			try {
				...
				List<Room> checkSet = CMLib.tracking().getRadiantRooms(
						mob.location(), flags, 50 + (2 * getXLEVELLevel(mob)));
		...
		if (givenTarget != null && auto && mob.isMonster())
			flags.plus(TrackingLibrary.TrackingFlag.AREAONLY);
		if (rooms.size() > 0)
			theTrail = CMLib.tracking().findBastardTheBestWay(mob.location(),
					rooms, flags, 50 + (2 * getXLEVELLevel(mob)));
		...
		if ((success) && (theTrail != null) && (tracking != null)) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							CMMsg.MSG_THIEF_ACT,
							mob.isMonster() ? null
									: "<S-NAME> begin(s) to track <T-NAMESELF> for assassination.",
			...
			if ((mob.location().okMessage(mob, msg))
				...
				newOne.nextDirection = CMLib.tracking()
						.trackNextDirectionFromHere(theTrail, mob.location(),
								true);
			}
		} else
			return beneficialVisualFizzle(mob, tracking,
					"<S-NAME> attempt(s) to track <T-NAMESELF> for assassination, but fail(s).");
