...
public class Thief_SilentOpen extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Environmental item = super.getAnyTarget(mob, commands, givenTarget,
				Wearable.FILTER_UNWORNONLY);
		if (item == null)
			return false;
		if ((item instanceof MOB) || (item instanceof Area)
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, item, this, CMMsg.MSG_THIEF_ACT,
					"<S-NAME> open(s) <T-NAME>.", CMMsg.NO_EFFECT, null,
			...
			beneficialVisualFizzle(mob, item,
					"<S-NAME> attempt(s) to open <T-NAME> quietly, but fail(s).");
