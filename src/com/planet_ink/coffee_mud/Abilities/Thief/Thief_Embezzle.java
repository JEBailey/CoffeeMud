...
public class Thief_Embezzle extends ThiefSkill {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((msg.amITarget(affected)) && (mobs.contains(msg.source()))) {
			if ((msg.targetMinor() == CMMsg.TYP_BUY)
				...
				msg.source().tell(
						affected.name()
								+ " looks unwilling to do business with you.");
				return false;
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!(target instanceof Banker)) {
			mob.tell("You can't embezzle from " + target.name(mob)
					+ "'s accounts.");
			return false;
		}
		if (mob.isInCombat()) {
		...
		if (A != null) {
			mob.tell(target.name(mob) + " is watching "
					+ target.charStats().hisher() + " books too closely.");
			return false;
		}
		...
		if (bank.isSold(ShopKeeper.DEAL_CLANBANKER)) {
			Pair<Clan, Integer> clanPair = CMLib.clans().findPrivilegedClan(
					mob, Clan.Function.WITHDRAW);
			if (clanPair == null)
				clanPair = CMLib.clans().findPrivilegedClan(mob,
						Clan.Function.DEPOSIT_LIST);
			if (clanPair == null)
				clanPair = CMLib.clans().findPrivilegedClan(mob,
						Clan.Function.DEPOSIT);
			if (clanPair != null)
		...
		while ((hisCoins == null) && ((++tries) < 10)) {
			String possVic = accounts.get(CMLib.dice().roll(1, accounts.size(),
					-1));
			Item C = bank.findDepositInventory(possVic, "1");
			if ((C != null) && (C instanceof Coins)
		...
		int classLevel = CMLib.ableMapper().qualifyingClassLevel(mob, this)
		...
		boolean success = proficiencyCheck(mob,
				(-(levelDiff + (timesPicked(mob) * 50))), auto);
		if ((success) && (hisAmount > 0) && (hisCoins != null)) {
			String str = "<S-NAME> embezzle(s) "
					+ CMLib.beanCounter().nameCurrencyShort(target, hisAmount)
					+ " from the " + victim
					+ " account maintained by <T-NAME>.";
			CMMsg msg = CMClass.getMsg(mob, target, this,
					(auto ? CMMsg.MASK_ALWAYS : 0) | CMMsg.MSG_THIEF_ACT, str,
					null, str);
				...
				beneficialAffect(
						...
						(int) (((CMProps.getMillisPerMudHour()
								* mob.location().getArea().getTimeObj()
										.getHoursInDay() * mob.location()
								.getArea().getTimeObj().getDaysInMonth()) / CMProps
								.getTickMillis())));
				bank.delDepositInventory(victim, hisCoins);
				hisCoins = CMLib.beanCounter().makeBestCurrency(target,
						hisCoins.getTotalValue() - (hisAmount / 3.0));
				if (hisCoins.getNumberOfCoins() > 0)
					bank.addDepositInventory(victim, hisCoins);
				bank.delDepositInventory(myAcct, myCoins);
				myCoins = CMLib.beanCounter().makeBestCurrency(mob,
						((Coins) myCoins).getTotalValue() + hisAmount);
				if (((Coins) myCoins).getNumberOfCoins() > 0)
					bank.addDepositInventory(myAcct, myCoins);
			}
		} else
			maliciousFizzle(mob, target,
					"<T-NAME> catch(es) <S-NAME> trying to embezzle money!");
		return success;
	}
