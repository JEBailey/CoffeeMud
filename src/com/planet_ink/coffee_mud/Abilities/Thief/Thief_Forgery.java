...
public class Thief_Forgery extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (forgeWhat.length() == 0) {
			mob.tell("Forge what onto '" + target.name(mob)
					+ "'?  Try a spell name, a room ID, or a bank note name.");
			return false;
		}
		...
		if (room != null) {
			...
			((LandTitle) I).setLandPropertyID(CMLib.map().getExtendedRoomID(
					room));
			newName = I.name();
		...
		if (newName.length() == 0) {
			...
			if ((A != null)
				...
				if (!(target instanceof Scroll)) {
					...
					newSecretIdentity = "a scroll of " + A.name()
							+ " Charges: 10\n";
				}
			}
		}
		if (newName.length() == 0) {
			MoneyLibrary.MoneyDenomination[] DV = CMLib.beanCounter()
					.getCurrencySet(CMLib.beanCounter().getCurrency(mob));
			for (int i = 0; i < DV.length; i++) {
				Item note = CMLib.beanCounter().makeBestCurrency(
						CMLib.beanCounter().getCurrency(mob), DV[i].value);
				if ((note != null)
						&& (CMLib.english().containsString(note.name(),
								forgeWhat))) {
					newName = note.name();
		...
		if (newName.length() == 0) {
			mob.tell("You don't know how to forge a '" + forgeWhat
					+ "'.  Try a spell name, a room ID, or a bank note name.");
			return false;
		}
		forgeWhat = newName;
		...
		int levelDiff = (mob.phyStats().level() + (2 * getXLEVELLevel(mob)))
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MSG_THIEF_ACT,
					"<S-NAME> forge(s) " + forgeWhat + " on <T-NAMESELF>.");
			...
			beneficialVisualFizzle(mob, target, "<S-NAME> attempt(s) to forge "
					+ forgeWhat + ", but fail(s).");
		return success;
	}
}
