...
public class Thief_UsePoison extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Item poison = mob.fetchItem(null, Wearable.FILTER_UNWORNONLY,
		...
		CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MSG_THIEF_ACT,
				"<S-NAME> attempt(s) to poison <T-NAMESELF>.");
		if (mob.location().okMessage(mob, msg)) {
			...
			if (success) {
				...
				if (A != null) {
					...
					if (amountToTake < 1)
						amountToTake = 1;
					dPoison.setLiquidRemaining(dPoison.liquidRemaining()
							- amountToTake);
					if (dPoison.disappearsAfterDrinking()
							|| ((dPoison instanceof RawMaterial) && (dPoison
									.liquidRemaining() <= 0)))
						dPoison.destroy();
				}
