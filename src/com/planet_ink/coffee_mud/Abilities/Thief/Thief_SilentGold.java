	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected != null) && (affected instanceof MOB)) {
			if ((msg.sourceMinor() == CMMsg.TYP_DEATH)
				...
				lastMsg = msg;
				double money = CMLib.beanCounter().getTotalAbsoluteNativeValue(
						msg.source());
				...
				if (gold > 0.0) {
					...
					if ((C != null) && (C.getNumberOfCoins() > 0)) {
						...
						mob.location().addItem(C,
								ItemPossessor.Expire.Monster_EQ);
						mob.location().recoverRoomStats();
						MOB victim = mob.getVictim();
						mob.setVictim(null);
						CMMsg msg2 = CMClass.getMsg(mob, C, this,
								CMMsg.MSG_THIEF_ACT,
								"You silently loot <T-NAME> from the corpse of "
										+ msg.source().name(mob),
								CMMsg.MSG_THIEF_ACT, null, CMMsg.NO_EFFECT,
								null);
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to start silently looting gold from corpses, but fail(s).");
		return success;
	}
