	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.source() == affected) || (affected instanceof Room)) {
			if (((msg.sourceMinor() == CMMsg.TYP_SIT) || (msg.sourceMinor() == CMMsg.TYP_STAND))
				...
				return true;
			else if (((msg.sourceMinor() == CMMsg.TYP_LEAVE) || (msg
				...
				if (canBeUninvoked()) {
					if (msg.source()
							...
							.show(msg.source(),
									null,
									CMMsg.MSG_OK_ACTION,
									"<S-NAME> struggle(s) against "
											+ ropeName.toLowerCase()
											+ " binding <S-HIM-HER>.")) {
						amountRemaining -= (msg.source().charStats()
								.getStat(CharStats.STAT_STRENGTH) + msg
					...
					msg.source().tell(
							"You are constricted by " + ropeName.toLowerCase()
									+ " and can't move!");
				return false;
			}
		}
		...
		if (canBeUninvoked()) {
			if (!mob.amDead())
				mob.location().show(
						...
						"<S-NAME> manage(s) to break <S-HIS-HER> way free of "
								+ ropeName + ".");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((commands.size() > 0)
			...
			mob.tell(target.name(mob)
					+ " doesn't appear to be bound with ropes.");
			return false;
		}
		...
		if ((!CMLib.flags().isSleeping(target))
				&& (CMLib.flags().canMove(target) && (!auto))) {
		...
		if (success) {
			if (auto)
				maxRange = 10;
			String str = auto ? "<T-NAME> become(s) bound by " + ropeName + "."
					: "<S-NAME> bind(s) <T-NAME> with " + ropeName + ".";
			CMMsg msg = CMClass.getMsg(mob, target, this,
					(auto ? CMMsg.MASK_ALWAYS : 0) | CMMsg.MSG_THIEF_ACT
							| CMMsg.MASK_SOUND | CMMsg.MASK_MALICIOUS,
					auto ? "" : str, str, str);
