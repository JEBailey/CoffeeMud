...
public class Thief_Flay extends ThiefSkill {
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		...
		if ((affected instanceof MOB)
				...
				&& ((CMath.bset(
						...
						Wearable.WORN_BACK)) || (CMath.bset(
						((Item) msg.target()).rawProperLocationBitmap(),
						Wearable.WORN_TORSO)))) {
			msg.source()
					.tell("The flayed marks on your back make wearing that too painful.");
			return false;
		}
		return true;
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		for (int i = 0; i < target.numItems(); i++) {
			...
			if ((I != null)
					&& ((I.amWearingAt(Wearable.WORN_BACK)) || (I
							.amWearingAt(Wearable.WORN_TORSO)))) {
				mob.tell(target.name(mob)
						+ " must be remove items worn on the torso or back first.");
				return false;
			}
		}
		...
		if (!auto) {
			...
			if (ww.weaponClassification() != Weapon.CLASS_FLAILED) {
				mob.tell("You cannot flay with a " + ww.name()
						+ ", you need a flailing weapon!");
				return false;
			}
			if (w.material() != RawMaterial.RESOURCE_LEATHER) {
				mob.tell("You cannot flay with a " + ww.name()
						+ ", you need a weapon made of leather!");
				return false;
			}
		...
		CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MASK_MALICIOUS
				| CMMsg.MSG_THIEF_ACT,
				"<S-NAME> flay(s) the bare back of <T-NAMESELF>!");
		boolean makePeace = CMLib.flags().isBound(target)
				&& (mob.getVictim() == null) && (target.getVictim() == null);
		...
		if (success) {
			...
			maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) flay <T-NAMESELF>, but fail(s).");
		return success;
	}
}
