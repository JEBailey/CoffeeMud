...
public class Thief_Arsonry extends ThiefSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (dir >= 0) {
			...
			target = (Item) choices.elementAt(CMLib.dice().roll(1,
					choices.size(), -1));
			targetRoom = room;
		} else {
			Item item = getTarget(mob, mob.location(), givenTarget, null,
					commands, Wearable.FILTER_UNWORNONLY);
			if (item == null)
				return false;
			target = item;
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					"<S-NAME> commit(s) arsonry against <T-NAME>.");
			if ((mob.location().okMessage(mob, msg))
					&& ((targetRoom == mob.location()) || (targetRoom
							.okMessage(mob, msg)))) {
				...
				if (B != null)
					B.invoke(mob, target, true, CMLib.flags()
							.burnStatus(target));
			}
		} else
			beneficialVisualFizzle(mob, target,
					"<S-NAME> attempt(s) arsonry against <T-NAME>, but fails.");
		return success;
	}
