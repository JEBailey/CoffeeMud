...
public class Disease_Narcolepsy extends Disease {
	...
	protected String DISEASE_DONE() {
		return "Your narcolepsy is cured!";
	}

	protected String DISEASE_START() {
		return "^G<S-NAME> seem(s) sleepy.^?";
	}

	protected String DISEASE_AFFECT() {
		return "<S-NAME> <S-IS-ARE> getting sleepy...";
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((!mob.amDead()) && ((--diseaseTick) <= 0)) {
			...
			mob.curState().adjFatigue(
					mob.curState().getFatigue() + CharState.FATIGUED_MILLIS,
					mob.maxState());
			...
			if (!CMLib.flags().isSleeping(mob)) {
				...
				try {
					if (C != null)
						C.execute(mob, CMParms.parse("Sleep"),
