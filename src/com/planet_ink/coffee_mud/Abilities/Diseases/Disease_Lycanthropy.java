...
public class Disease_Lycanthropy extends Disease {
	...
	protected String DISEASE_DONE() {
		return "Your lycanthropy is cured.";
	}

	protected String DISEASE_START() {
		return "^G<S-NAME> feel(s) different.^?";
	}
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if (lycanRace() != null) {
			if (affected.name().indexOf(' ') > 0)
				affectableStats.setName("a " + lycanRace().name() + " called "
						+ affected.name());
			else
		...
		if (lycanRace() != null) {
			...
			if (affected.baseCharStats().getStat(CharStats.STAT_AGE) > 0)
				affectableStats.setStat(CharStats.STAT_AGE,
						lycanRace().getAgingChart()[affected.baseCharStats()
								.ageCategory()]);
		}
	}
	...
	public MOB victimHere(Room room, MOB mob) {
		...
		for (int i = 0; i < room.numInhabitants(); i++) {
			...
			if ((M != null) && (M != mob) && (!CMLib.flags().isEvil(M))
					&& (mob.mayIFight(M))
	...
	public void tickLycanthropically(MOB mob) {
		...
		if ((CMLib.dice().rollPercentage() < 15)
			...
			mob.location().show(mob, null, CMMsg.MSG_NOISE,
					"<S-NAME> howl(s) at the moon! ARROOOOOOOO!!!!");
		...
		if (deathTrail == null) {
			...
			if ((findVictim(mob, mob.location(), rooms, 0))
					&& (rooms.size() > 0)) {
				...
				deathTrail = CMLib.tracking().findBastardTheBestWay(
						mob.location(), rooms, flags, 50);
				if (deathTrail != null)
					deathTrail.add(mob.location());
			}
		}
		if (deathTrail != null) {
			int nextDirection = CMLib.tracking().trackNextDirectionFromHere(
					deathTrail, mob.location(), true);
			if ((nextDirection == 999) || (nextDirection == -1))
				deathTrail = null;
			else if (nextDirection >= 0) {
				Room nextRoom = mob.location().getRoomInDir(nextDirection);
				if ((nextRoom != null)
						&& ((nextRoom.getArea() == mob.location().getArea()))
						|| (!mob.isMonster())) {
					if (!CMLib.tracking()
							.walk(mob, nextDirection, false, false))
						deathTrail = null;
					else if (CMLib.dice().rollPercentage() < 15)
						mob.location().show(mob, null, CMMsg.MSG_NOISE,
								"<S-NAME> sniff(s) at the air.");
