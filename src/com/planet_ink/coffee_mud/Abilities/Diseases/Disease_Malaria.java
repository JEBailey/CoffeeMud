...
public class Disease_Malaria extends Disease {
	...
	protected String DISEASE_DONE() {
		return "Your malaria clears up.";
	}

	protected String DISEASE_START() {
		return "^G<S-NAME> come(s) down with malaria.^?";
	}

	protected String DISEASE_AFFECT() {
		return "<S-NAME> ache(s) and sneeze(s). AAAAAAAAAAAAAACHOOO!!!!";
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (diseaser == null)
			diseaser = mob;
		if ((!mob.amDead())
				&& ((++tickUp) == CMProps.getIntVar(CMProps.Int.TICKSPERMUDDAY))) {
			tickUp = 0;
			conDown++;
