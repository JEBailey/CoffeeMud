...
public class Disease_Obesity extends Disease {
	...
	protected String DISEASE_DONE() {
		return "You've become fit and trim!";
	}

	protected String DISEASE_START() {
		return "^G<S-NAME> look(s) like <S-HE-SHE> <S-HAS-HAVE> been gaining some weight.^?";
	}
	...
	public void affectCharStats(MOB affectedMob, CharStats affectableStats) {
		...
		affectableStats.setStat(
				CharStats.STAT_WEIGHTADJ,
				affectableStats.getStat(CharStats.STAT_WEIGHTADJ)
						+ (int) Math.round(CMath.mul(affectedMob.basePhyStats()
								.weight(),
								CMath.div(CMath.s_int(text()), 100.0))));
	}
	...
	public void affectCharState(MOB affected, CharState affectableState) {
		...
		affectableState.setMovement(affectableState.getMovement()
				- (int) Math.round(CMath.mul(affectableState.getMovement(),
						CMath.div(CMath.s_int(text()), 100.0))));
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((ticking == affected) && (tickID == Tickable.TICKID_MOB)
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		if ((msg.source() == affected)
				...
				&& (msg.target() != null)
				&& (msg.source().curState().getHunger() >= msg.source()
						.maxState().maxHunger(msg.source().baseWeight()))) {
