...
public class Disease_Alzheimers extends Disease {
	...
	protected String DISEASE_DONE() {
		return "Your alzheimers is cured!";
	}

	protected String DISEASE_START() {
		return "^G<S-NAME> feel(s) like <S-HE-SHE> <S-HAS-HAVE> forgotten something.^?";
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (((everyTick > 0) || (CMLib.dice().roll(1, 150, 0) == 1))
			...
			if (CMLib.flags().aliveAwakeMobileUnbound(mob, true)) {
				...
				Room R = mob.location();
				if ((CMLib.flags().isStanding(mob)) && (R != null)
					...
					if (dirs.size() == 0)
						...
						int dir = ((Integer) dirs.elementAt(CMLib.dice().roll(
								1, dirs.size(), -1))).intValue();
