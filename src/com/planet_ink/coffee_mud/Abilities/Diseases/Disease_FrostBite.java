...
public class Disease_FrostBite extends Disease {
	...
	public int[] limbsAffectable = { Race.BODY_EAR, Race.BODY_ANTENEA,
			Race.BODY_FOOT, Race.BODY_HAND, Race.BODY_NOSE };
	...
	protected String DISEASE_START() {
		return "^G<S-NAME> <S-IS-ARE> getting frost bite.^?";
	}
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		if (where == null) {
			...
			if (choices.size() <= 0) {
				...
				where = Race.BODYPARTSTR[((Integer) choices.elementAt(CMLib
						.dice().roll(1, choices.size(), -1))).intValue()];
		}
	}
