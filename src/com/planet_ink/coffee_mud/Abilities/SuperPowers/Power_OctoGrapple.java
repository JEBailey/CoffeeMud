...
public class Power_OctoGrapple extends SuperPower {
			...
			if ((msg.sourceMajor(CMMsg.MASK_EYES))
				...
				if (msg.sourceMessage() != null) {
					if (mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> struggle(s) against the grappling arms.")) {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		affectableStats.setDisposition(affectableStats.sensesMask()
				| PhyStats.IS_BOUND);
	}
				...
				if (mob == invoker) {
					if (mob.location() != null)
						mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
								"<S-NAME> release(s) <S-HIS-HER> grapple.");
					else
					...
					if (mob.location() != null)
						mob.location()
								.show(mob, null, CMMsg.MSG_OK_ACTION,
										"<S-NAME> <S-IS-ARE> released from the grapple");
					else
		...
		boolean success = proficiencyCheck(
				...
				(-levelDiff)
						+ (-(((target.charStats().getStat(
								CharStats.STAT_STRENGTH) - mob.charStats()
								.getStat(CharStats.STAT_STRENGTH)) * 5))), auto)
				&& (hit);
		success = success
				&& (target.charStats().getBodyPart(Race.BODY_ARM) > 2);
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> get(s) grappled!"
									: "^F^<FIGHT^><S-NAME> grab(s) <T-NAMESELF> with <S-HIS-HER> huge metallic arms!^</FIGHT^>^?");
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to grab <T-NAMESELF>, but fail(s).");
