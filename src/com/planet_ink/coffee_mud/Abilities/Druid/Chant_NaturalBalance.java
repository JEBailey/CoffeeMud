...
public class Chant_NaturalBalance extends Chant {
	...
	public String displayText() {
		return "(Communing with the Natural Balance)";
	}
			...
			if (!mob.amDead()) {
				if (mob.location() != null)
					mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> end(s) <S-HIS-HER> natural communion.");
				else
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amISource(mob) && (!msg.sourceMajor(CMMsg.MASK_ALWAYS))
				&& (!CMath.bset(msg.sourceMajor(), CMMsg.MASK_CHANNEL)) && (msg
				.sourceMajor() > 0))) {
		}
		return super.okMessage(myHost, msg);
	}
			...
			if (CMLib.factions().getAlignPurity(myAlignment,
				...
				if (CMLib.factions().getAlignPurity(myAlignment,
					...
					CMLib.factions().postFactionChange(mob, this,
							CMLib.factions().AlignID(), oneHalfPct);
				else
					CMLib.factions().postFactionChange(mob, this,
							CMLib.factions().AlignID(), -oneHalfPct);
				...
				case 0:
					room.show(mob, null, this, CMMsg.MSG_CONTEMPLATE,
							"<S-NAME> empathize(s) with the plants.");
					break;
				case 1:
					room.show(mob, null, this, CMMsg.MSG_CONTEMPLATE,
							"<S-NAME> learn(s) from the birds.");
					break;
				case 2:
					room.show(mob, null, this, CMMsg.MSG_CONTEMPLATE,
							"<S-NAME> watch(es) the insects.");
					break;
				case 3:
					room.show(mob, null, this, CMMsg.MSG_HANDS
							| CMMsg.MASK_ALWAYS, "<S-NAME> hug(s) the ground.");
					break;
				case 4:
					room.show(mob, null, this, CMMsg.MSG_CONTEMPLATE,
							"<S-NAME> inhale(s) the fresh air.");
					break;
				case 5:
					room.show(mob, null, this, CMMsg.MSG_CONTEMPLATE,
							"<S-NAME> watch(es) the plants grow.");
					break;
				case 6:
					room.show(mob, null, this, CMMsg.MSG_CONTEMPLATE,
							"<S-NAME> become(s) one with life.");
					break;
				case 7:
					room.show(mob, null, this, CMMsg.MSG_CONTEMPLATE,
							"<S-NAME> seek(s) the inner beauty of the natural order.");
					break;
				case 8:
					room.show(mob, null, this, CMMsg.MSG_CONTEMPLATE,
				...
				case 9:
					room.show(mob, null, this, CMMsg.MSG_CONTEMPLATE,
							"<S-NAME> find(s) clarity in the natural world.");
					break;
				}
			}
		} else {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			invoker = mob;
			CMMsg msg = CMClass
					.getMsg(mob, null, this, somanticCastCode(mob, null, auto),
							"^S<S-NAME> begin(s) to commune with the natural balance...^?");
			...
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> chant(s) to commune with nature, but lose(s) concentration.");
