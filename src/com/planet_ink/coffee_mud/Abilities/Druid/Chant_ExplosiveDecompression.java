...
public class Chant_ExplosiveDecompression extends Chant {
	...
	public String name() {
		return "Explosive Decompression";
	}
	...
	public void affectPhyStats(Physical affecting, PhyStats stats) {
		...
		if ((affected instanceof MOB)
				&& (((MOB) affected).charStats().getBreathables().length > 0))
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> chant(s) loudly.  A ball of fire forms around <S-NAME>.^?");
					...
					for (int i = 0; i < target.numInhabitants(); i++) {
						...
						if ((M != null) && (M != mob)) {
							CMMsg msg2 = CMClass.getMsg(mob, M, this,
									verbalCastMask(mob, target, auto)
											| CMMsg.TYP_FIRE, null);
							if (mob.location().okMessage(mob, msg2)) {
								mob.location().send(mob, msg2);
								invoker = mob;
								int numDice = adjustedLevel(mob, asLevel)
										+ (2 * super.getX1Level(mob));
								...
								CMLib.combat().postDamage(mob, M, this, damage,
										...
										Weapon.TYPE_BURNING,
										"The flaming blast <DAMAGE> <T-NAME>!");
							}
							if ((M.charStats().getBodyPart(Race.BODY_FOOT) > 0)
									&& (!CMLib.flags().isFlying(M))
									&& (CMLib.flags().isStanding(M)))
								mob.location()
										.show(M,
												null,
												CMMsg.MASK_ALWAYS
														| CMMsg.TYP_SIT,
												"<S-NAME> <S-IS-ARE> blown off <S-HIS-HER> feet!");
						}
					}
					maliciousAffect(mob, target, asLevel, 20, -1);
					mob.location().showHappens(CMMsg.MSG_OK_VISUAL,
							"The fire burns off all the air here!");
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> chant(s) loudly, but nothing happens.");
