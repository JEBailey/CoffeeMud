		...
		if (mobTarget != null) {
			...
			for (int i = 0; i < mobTarget.numItems(); i++) {
				...
				if (goodPossibilities.size() > 0)
					return (Item) goodPossibilities.elementAt(CMLib.dice()
							.roll(1, goodPossibilities.size(), -1));
				...
				if (msg.value() <= 0) {
					...
					if (mobTarget == null)
						mob.location().show(mob, target, CMMsg.MSG_OK_VISUAL,
								"<T-NAME> begin(s) to twist and warp!");
					else
						mob.location()
								.show(mobTarget, target, CMMsg.MSG_OK_VISUAL,
										"<T-NAME>, possessed by <S-NAME>, twists and warps!");
