...
public class Chant_AnimalSpy extends Chant {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((tickID == Tickable.TICKID_MOB) && (affected == spy)) {
			if (spy.amDead() || (spy.amFollowing() != invoker)
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		try {
			...
			if ((msg.amISource(spy))
					&& ((msg.sourceMinor() == CMMsg.TYP_LOOK) || (msg
							.sourceMinor() == CMMsg.TYP_EXAMINE))
					&& (msg.target() != null)
					&& ((invoker.location() != spy.location()) || (!(msg
				...
				msg.target().executeMsg(invoker, newAffect);
					...
					&& (msg.othersMessage() != null) && (!disable)) {
				disable = true;
				invoker.executeMsg(invoker, msg);
			} else if (msg.amISource(invoker) && (!disable)
					&& (msg.sourceMinor() == CMMsg.TYP_SPEAK)
					&& (msg.sourceMessage() != null)
					&& ((msg.sourceMajor() & CMMsg.MASK_MAGIC) == 0)) {
				String msg2 = CMStrings.getSayFromMessage(msg.sourceMessage());
				if ((msg2 != null) && (msg2.length() > 0))
					spy.enqueCommand(CMParms.parse(msg2.trim()),
							Command.METAFLAG_FORCED, 0);
			}
		} finally {
			disable = false;
			if ((spy != null)
					&& ((spy.amFollowing() != invoker) || (spy.amDead())
							|| (!CMLib.flags().isInTheGame(spy, false)) || (!CMLib
							.flags().isInTheGame(invoker, true))))
				unInvoke();
		}
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target != null) {
			...
			if ((!CMLib.flags().isAnimalIntelligence(target))
				...
				mob.tell("You have no animal follower named '" + mobName
						+ "' here.");
				return false;
			}
		} else {
