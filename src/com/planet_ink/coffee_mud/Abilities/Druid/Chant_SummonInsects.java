...
public class Chant_SummonInsects extends Chant {
	...
	public String name() {
		return "Summon Insects";
	}

	public String displayText() {
		return "(In a swarm of insects)";
	}
		...
		if ((tickID == Tickable.TICKID_MOB) && (affected != null)
			...
			if (M.location() != castingLocation)
				...
				CMLib.combat().postDamage(
						...
						M,
						this,
						CMLib.dice().roll(1, 3 + super.getXLEVELLevel(invoker),
								0), CMMsg.MASK_ALWAYS | CMMsg.TYP_CAST_SPELL,
						-1, "<T-NAME> <T-IS-ARE> stung by the swarm!");
		...
		if (canBeUninvoked())
			if ((!mob.amDead()) && (mob.location() != null))
				mob.location().show(mob, null, CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> manage(s) to escape the insect swarm!");
	}
			...
			if (h == null) {
				mob.location()
						.show(mob,
								...
								auto ? "A swarm of stinging insects appear, then flutter away!"
										: "^S<S-NAME> chant(s) into the sky.  A swarm of stinging insects appear.  Finding no one to sting, they flutter away.^?");
				return false;
			}
			if (mob.location()
					.show(mob,
							...
							verbalCastCode(mob, null, auto),
							auto ? "A swarm of stinging insects appear, then flutter away!"
									: "^S<S-NAME> chant(s) into the sky.  A swarm of stinging insects appears and attacks!^?"))
						...
						if ((msg.value() <= 0)
							...
							success = maliciousAffect(mob, target, asLevel,
									((mob.phyStats().level() + (2 * super
											.getXLEVELLevel(mob))) * 10), -1);
							target.location()
									.show(target, null, CMMsg.MSG_OK_ACTION,
											"<S-NAME> become(s) enveloped by the swarm of stinging insects!");
						}
					}
				}
		} else
			return maliciousFizzle(mob, null,
					"<S-NAME> chant(s), but the magic fizzles.");
