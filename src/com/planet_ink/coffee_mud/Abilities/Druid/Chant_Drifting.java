		...
		if (msg.amISource(mob)) {
			if ((msg.sourceMinor() == CMMsg.TYP_ADVANCE)
				...
				return false;
					...
					&& (mob.location() != null)
					&& ((mob.location().getRoomInDir(Directions.UP) == msg
							.target()) || (mob.location().getRoomInDir(
							Directions.DOWN) == msg.target()))) {
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> chant(s) <S-HIM-HERSELF> off the ground.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					success = beneficialAffect(mob, target, asLevel, 0);
					target.location().show(target, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> start(s) drifting up!");
				}
			}
		} else
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> chant(s), but fail(s) to leave the ground.");
