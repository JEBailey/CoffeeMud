...
public class Chant_TapGrapevine extends Chant {
	...
	public void unInvoke() {
		if ((affected instanceof MOB) && (myChants != null)) {
			...
			for (int i = 0; i < V.size(); i++) {
				Ability A = V.get(i);
				if ((A.affecting() != null) && (A.ID().equals(ID()))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((mob.fetchEffect(ID()) != null)
				|| (mob.fetchEffect("Chant_Grapevine") != null)) {
		...
		Vector myRooms = (tapped == null) ? null : Druid_MyPlants
				.myPlantRooms(tapped);
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				Chant_TapGrapevine C = (Chant_TapGrapevine) mob
						.fetchEffect(ID());
				if (C == null)
					return false;
			...
			beneficialVisualFizzle(mob, myPlant,
