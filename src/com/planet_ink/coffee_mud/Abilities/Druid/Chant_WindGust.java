	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			if (mob.location().show(
					...
					(auto ? "^JA horrendous wind gust blows through here.^?"
							: "^S<S-NAME> chant(s) at <S-HIS-HER> enemies.^?")
						...
						if ((msg.value() <= 0)
							...
							if ((mob.location().getArea().getClimateObj()
									...
									|| (mob.location().getArea()
											.getClimateObj()
											.weatherType(mob.location()) == Climate.WEATHER_DUSTSTORM)
									|| (mob.location().getArea()
											.getClimateObj()
							...
							if ((victim != null)
								...
								target.setAtRange(target.rangeToTarget()
										+ (howLong / 2));
							if (target.rangeToTarget() > target.location()
									.maxRange())
								target.setAtRange(target.location().maxRange());
							mob.location().send(mob, msg);
							if ((!CMLib.flags().isInFlight(target))
									&& (CMLib.dice().rollPercentage() > (((target
											.charStats().getStat(
													CharStats.STAT_DEXTERITY) * 2) + target
											.phyStats().level()))
											- (5 * howLong))
									&& (target.charStats().getBodyPart(
											Race.BODY_LEG) > 0)) {
							...
							if (target.getVictim() != null)
								target.getVictim().setAtRange(
										target.rangeToTarget());
