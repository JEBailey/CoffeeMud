...
public class Chant_Reabsorb extends Chant {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Item target = this.getTarget(mob, mob.location(), givenTarget, null,
		...
		if (!(target.owner() instanceof Room)) {
			mob.tell("You need to put " + target.name(mob)
					+ " on the ground first.");
			return false;
		}
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> starts vibrating!"
									: "^S<S-NAME> chant(s), causing <T-NAMESELF> to decay!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					mob.location().showHappens(CMMsg.MSG_OK_VISUAL,
							"The ground reabsorbs " + target.name() + ".");
					target.destroy();
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> chant(s) at <T-NAME>, but nothing happens.");
