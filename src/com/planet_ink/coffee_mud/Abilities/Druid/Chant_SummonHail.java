	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!auto)
				&& ((mob.location().getArea().getClimateObj()
						.weatherType(mob.location()) != Climate.WEATHER_WINTER_COLD) && (mob
						.location().getArea().getClimateObj()
						.weatherType(mob.location()) != Climate.WEATHER_HAIL))) {
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "^JHailstones falling from the sky whack <T-NAME>.^?"
									: "^S<S-NAME> chant(s) to <T-NAMESELF>.  Suddenly a volley of hailstones assaults <T-HIM-HER>!^?")
									+ CMLib.protocol().msp("hail.wav", 40));
			CMMsg msg2 = CMClass.getMsg(mob, target, this,
					verbalCastMask(mob, target, auto) | CMMsg.TYP_WATER, null);
			if ((mob.location().okMessage(mob, msg))
				...
				if (target.location() == mob.location()) {
					...
					if ((I != null) && (I.amWearingAt(Wearable.WORN_HEAD)))
						target.location()
								.show(target, I, null, CMMsg.MSG_OK_ACTION,
										"Hailstones bounce harmlessly off <T-NAME> being worn by <S-NAME>.");
					else
						CMLib.combat().postDamage(mob, target, this, damage,
								CMMsg.MASK_ALWAYS | CMMsg.TYP_WATER,
								Weapon.TYPE_BASHING,
								"The hailstones <DAMAGE> <T-NAME>!");
				}
				...
				if (C.weatherType(mob.location()) != Climate.WEATHER_HAIL) {
					...
					Chant_CalmWeather.xpWorthyChange(mob, mob.location()
							.getArea(), oldC, C);
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> chant(s) at <T-NAMESELF>, but the magic fades.");
