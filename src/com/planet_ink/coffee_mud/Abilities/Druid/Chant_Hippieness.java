	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		if (affected instanceof MOB) {
			for (Pair<Clan, Integer> p : ((MOB) affected).clans())
				oldClans.add(p);
			((MOB) affected).setClan("", Integer.MIN_VALUE); // deletes all
																// clans
		}
		...
		if ((msg.source() == affected)
				...
				&& (((((Ability) msg.tool()).classificationCode() & Ability.ALL_DOMAINS) == Ability.DOMAIN_CRAFTINGSKILL) || ((((Ability) msg
						.tool()).classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_COMMON_SKILL))) {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (affected instanceof MOB) {
			...
			if (mob.numItems() > 0) {
				...
				} else if ((I != null) && (I instanceof Light)
						&& (I.fitsOn(Wearable.WORN_MOUTH))) {
		...
		if (success) {
			...
			if ((mob.location().okMessage(mob, msg))
				...
				if ((msg.value() <= 0) && (msg2.value() <= 0)) {
					...
					CMLib.commands().postSay(target, null, "Far out...", false,
							false);
					maliciousAffect(mob, target, asLevel, 0,
							verbalCastMask(mob, target, auto) | CMMsg.TYP_MIND);
				}
			}
		} else
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> chant(s) to <T-NAMESELF>, but nothing more happens.");
