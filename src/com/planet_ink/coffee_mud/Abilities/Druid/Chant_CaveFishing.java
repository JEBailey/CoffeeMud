		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> chant(s) to <T-NAME>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					mob.location().showHappens(
							CMMsg.MSG_OK_VISUAL,
							"Fish start swimming around in " + target.name()
									+ "!");
					beneficialAffect(mob, target, asLevel, 0);
					Chant_CaveFishing A = (Chant_CaveFishing) target
							.fetchEffect(ID());
					if (A != null) {
						mob.location().showHappens(
								CMMsg.MSG_OK_VISUAL,
								"Fish start swimming around in "
										+ target.name() + "!");
						A.previousResource = target.myResource();
						target.setResource(RawMaterial.CODES.FISHES()[CMLib
								.dice().roll(1,
										RawMaterial.CODES.FISHES().length, -1)]);
					}
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> chant(s) to <T-NAME>, but the magic fades.");
