		...
		if (msg.amISource(mob)) {
			if ((!msg.sourceMajor(CMMsg.MASK_ALWAYS))
				...
				if (mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> struggle(s) against the snaring plants.")) {
			...
			if (!mob.amDead())
				mob.location()
						.show(mob, null, CMMsg.MSG_NOISYMOVEMENT,
								"<S-NAME> manage(s) to break <S-HIS-HER> way free of the plants.");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			if (room.show(
					...
					auto ? ""
							: "^S<S-NAME> chant(s) to the plants around <S-HIM-HER>.^?"))
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					if ((troom != null) && (troom.okMessage(mob, msg))
							&& (target.fetchEffect(this.ID()) == null)) {
						troom.send(mob, msg);
						if (msg.value() <= 0) {
							amountRemaining = 400 + (100 * super
									.getXLEVELLevel(mob));
							if (troom == room) {
								...
								troom.show(
										...
										null,
										CMMsg.MSG_OK_ACTION,
										"<S-NAME> become(s) stuck as tangling mass of plant life grows onto <S-HIM-HER>!");
							}
						}
					}
				}
