...
public class Chant_SummonSeed extends Chant {
	...
	public static final Integer[] NON_SEEDS = {
			Integer.valueOf(RawMaterial.RESOURCE_ASH),
		...
		for (Integer code : codes) {
			if (!CMParms.contains(Chant_SummonSeed.NON_SEEDS, code)) {
				...
				buf.append(CMStrings.padRight(
						CMStrings.capitalizeAndLower(str), 15));
			}
		}
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, null, this,
					verbalCastCode(mob, null, auto), auto ? ""
							: "^S<S-NAME> chant(s) to <S-HIS-HER> hands.^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				for (int i = 2; i < (2 + (adjustedLevel(mob, asLevel) / 4)); i++) {
					...
					newItem.setName(CMLib.english().startWithAorAn(
							name + " seed"));
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> chant(s) to <S-HIS-HER> hands, but nothing happens.");
