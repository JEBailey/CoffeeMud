...
public class Chant_ControlPlant extends Chant {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Item myPlant = getTarget(mob, mob.location(), givenTarget, commands,
				Wearable.FILTER_ANY);
		if (myPlant == null)
			return false;
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, myPlant, this,
					verbalCastCode(mob, myPlant, auto), auto ? ""
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (A != null)
					A.setInvoker(mob);
				mob.tell("You wrest control of " + myPlant.name() + " from "
						+ myPlant.secretIdentity() + ".");
