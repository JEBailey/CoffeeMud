...
public class Chant_SummonFlyTrap extends Chant {
			...
			if ((affected != null) && (affected instanceof MOB)
				...
				for (int r = 0; r < R.numItems(); r++) {
					...
					if ((I != null) && (I instanceof DeadBody)
						...
						String raceCat = ((DeadBody) I).charStats().getMyRace()
								.racialCategory();
						if (raceCat.equals("Insect")
								|| raceCat.equals("Arachnid")) {
							if (R.show(mob, I, CMMsg.MSG_HANDS
									| CMMsg.MASK_SOUND,
									"<S-NAME> devour(s) <T-NAMESELF>.")) {
								I.destroy();
								break;
							}
						}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, null, this,
					verbalCastCode(mob, null, auto), auto ? ""
							: "^S<S-NAME> chant(s) to the fertile ground.^?");
	...
	public MOB determineMonster(MOB caster) {
		...
		newMOB.location().show(newMOB, null, CMMsg.MSG_OK_ACTION,
				"<S-NAME> grow(s) from the ground.");
