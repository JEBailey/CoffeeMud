...
public class Chant_Feralness extends Chant {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((affected instanceof MOB)
				&& (((MOB) affected).charStats().getMyRace() != ((MOB) affected)
						.baseCharStats().getMyRace())) {
			...
			if ((((MOB) affected).fetchWieldedItem() == null)) {
				affectableStats.setAttackAdjustment(affectableStats
						.attackAdjustment() + (2 * adjLvl));
				affectableStats
						.setDamage(affectableStats.damage() * (2 + xlvl));
			}
			...
			affectableStats.setAttackAdjustment(affectableStats
					...
					+ (int) Math.round(CMath.div(
							affectableStats.attackAdjustment(), 4.0 - bonus)));
	...
	public void affectCharState(MOB affectedMOB, CharState affectedMaxState) {
		...
		if ((affected instanceof MOB)
			...
			affectedMaxState.setHitPoints(affectedMaxState.getHitPoints()
					+ hpAdjustment + (2 * getXLEVELLevel(invoker())));
	}
	...
	public void unInvoke() {
		...
		int lostpoints = mob.maxState().getHitPoints()
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> go(es) feral!"
									: "^S<S-NAME> chant(s) to <S-NAMESELF> and become(s) feral!^?");
