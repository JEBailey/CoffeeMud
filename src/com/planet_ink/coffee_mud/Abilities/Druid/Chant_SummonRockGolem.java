...
public class Chant_SummonRockGolem extends Chant {
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> chant(s) and summon(s) help from the earth.^?");
	...
	public MOB determineMonster(MOB caster, int level) {
		...
		newMOB.basePhyStats().setArmor(
				CMLib.leveler().getLevelMOBArmor(newMOB) + 100);
		newMOB.basePhyStats().setAttackAdjustment(
				CMLib.leveler().getLevelAttack(newMOB) + 100);
		newMOB.basePhyStats().setSpeed(
				CMLib.leveler().getLevelMOBSpeed(newMOB) / 2.0);
		newMOB.basePhyStats().setDamage(
				CMLib.leveler().getLevelMOBDamage(newMOB) + 20);
		...
		newMOB.location().showOthers(newMOB, null, CMMsg.MSG_OK_ACTION,
				"<S-NAME> appears out of the cave walls!");
