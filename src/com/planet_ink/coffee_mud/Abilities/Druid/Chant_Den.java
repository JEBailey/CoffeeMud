...
public class Chant_Den extends Chant {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int d = ((Integer) dirChoices.elementAt(CMLib.dice().roll(1,
				dirChoices.size(), -1))).intValue();
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, null, this,
					verbalCastCode(mob, null, auto), auto ? ""
							: "^S<S-NAME> chant(s) for a den!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				mob.location().showHappens(
						CMMsg.MSG_OK_VISUAL,
						"Your den, carefully covered, appears to the "
				...
				newRoom.rawDoors()[Directions.getOpDirectionCode(d)] = mob
						.location();
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> chant(s) for a den, but the magic fades.");
