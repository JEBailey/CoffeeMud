	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob))
			...
			mob.location().show(
					...
					null,
					CMMsg.MSG_OK_VISUAL,
					"The ward around <S-NAME> inhibits " + msg.tool().name()
							+ "!");
			return false;
		}
		return super.okMessage(myHost, msg);
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> <T-IS-ARE> protected from prayers."
									: "^S<S-NAME> chant(s) for a ward against prayers around <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> chant(s) for a ward, but nothing happens.");
