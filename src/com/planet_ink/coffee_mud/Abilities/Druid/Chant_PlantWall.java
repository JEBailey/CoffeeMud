...
public class Chant_PlantWall extends Chant {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((invoker != null) && (mob.isInCombat()) && (mob != invoker)
			...
			if ((msg.targetMinor() == CMMsg.TYP_WEAPONATTACK)
					...
					&& (msg.tool().maxRange() > 0)) {
				CMMsg msg2 = CMClass.getMsg(mob, null, CMMsg.MSG_WEAPONATTACK,
						"^F^<FIGHT^><S-NAME> fire(s) at the plant wall with "
								+ msg.tool().name() + ".^</FIGHT^>^?");
				...
				if (w == null)
					return false;
				CMMsg msg2 = CMClass.getMsg(mob, null, CMMsg.MSG_WEAPONATTACK,
						"^F<S-NAME> hack(s) at the plant wall with " + w.name()
								+ ".^?");
				CMLib.color().fixSourceFightColor(msg2);
				if (mob.location().okMessage(mob, msg2)) {
					mob.location().send(mob, msg2);
				...
				return false;
					...
					&& (msg.tool().maxRange() > 0)) {
				CMMsg msg2 = CMClass
						.getMsg(mob, null, msg.tool(), CMMsg.MSG_OK_VISUAL,
								"^F^<FIGHT^>The plant wall absorbs <O-NAME> from <S-NAME>.^</FIGHT^>^?");
				CMLib.color().fixSourceFightColor(msg2);
				if (mob.location().okMessage(mob, msg2))
					mob.location().send(mob, msg2);
				return false;
	...
	public void unInvoke() {
		super.unInvoke();
		if (canBeUninvoked()) {
			if ((theWall != null) && (theWall.owner() != null)
					&& (theWall.owner() instanceof Room)
					&& (((Room) theWall.owner()).isContent(theWall))) {
				final MOB actorM = (invoker != null) ? invoker : CMLib.map()
						.deity();
				((Room) theWall.owner()).show(actorM, null,
						CMMsg.MSG_OK_VISUAL, deathNotice);
				Item wall = theWall;
				theWall = null;
				wall.destroy();
			}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "A plant wall appears!"
							: "^S<S-NAME> chant(s) for a plant wall!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				amountRemaining = (mob.baseState().getHitPoints() / 6)
						+ (2 * (super.getX1Level(invoker()) + super
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> incant(s), but the magic fizzles.");
