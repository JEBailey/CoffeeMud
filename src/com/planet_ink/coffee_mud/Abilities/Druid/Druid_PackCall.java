	...
	public boolean tick(Tickable ticking, int tickID) {
		if (tickID == Tickable.TICKID_MOB) {
			if ((affected != null) && (affected instanceof MOB)
				...
				if (((mob.amFollowing() == null) || (mob.amDead())
						|| (!mob.isInCombat()) || (mob.location() != invoker
						.location())))
					unInvoke();
			}
		}
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			...
			if (target instanceof MOB) {
				...
				if (((MOB) target).totalFollowers() >= ((MOB) target)
						.maxFollowers())
					return Ability.QUALITY_INDIFFERENT;
			}
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							CMMsg.MSG_NOISE,
							auto ? ""
									: "^S<S-NAME> call(s) for help from <S-HIS-HER> pack!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				int levelsRemaining = 90 + (10 * getXLEVELLevel(mob));
				while ((mob.totalFollowers() < mob.maxFollowers())
						&& (levelsRemaining > 0)) {
					...
					int level = 1;
					while (!D.raceName
							.equals(D.getRaceName(level, MOBRaceCode)))
						level++;
					level--;
					...
					if (levelsRemaining < 0)
						break;
					newMOB.baseCharStats().setMyRace(
							D.getRace(level, MOBRaceCode));
					String raceName = D.getRaceName(level, MOBRaceCode)
							.toLowerCase();
					String name = CMLib.english().startWithAorAn(raceName)
							.toLowerCase();
					newMOB.setName(name);
					...
					int dir = ((Integer) choices.elementAt(CMLib.dice().roll(1,
							choices.size(), -1))).intValue();
					if (newMOB.getVictim() != victim)
						newMOB.setVictim(victim);
					newMOB.location().showOthers(
							newMOB,
							victim,
							CMMsg.MSG_OK_ACTION,
							"<S-NAME> arrive(s) "
									+ Directions.getFromDirectionName(dir)
									+ " and attack(s) <T-NAMESELF>!");
					newMOB.setStartRoom(null); // keep before postFollow for
												// Conquest
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> call(s) for help from <S-HIS-HER> pack, but nothing happens.");
