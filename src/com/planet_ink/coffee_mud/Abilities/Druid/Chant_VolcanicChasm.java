				...
				if ((M != null)
					...
					CMLib.combat()
							.postDamage(
									...
									Weapon.TYPE_MELTING,
									"The extreme heat <DAMAGES> <T-NAME>!");
					if ((!M.isInCombat()) && (M != invoker) && (!M.amDead())
							&& (!M.amDestroyed()) && (invoker != null)
				...
				if (msg.value() <= 0) {
					...
					mob.location()
							.showHappens(CMMsg.MSG_OK_VISUAL,
									"Flames and sulfurous steam leap from cracks opening around you!");
					maliciousAffect(mob, target, asLevel, 0, -1);
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> chant(s) the walls, but the magic fades.");
