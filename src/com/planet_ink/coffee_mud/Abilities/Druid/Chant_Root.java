		...
		if ((affected != null) && (affected instanceof MOB)) {
			if (msg.amISource((MOB) affected)) {
				if ((msg.targetMinor() == CMMsg.TYP_LEAVE)
					...
					if (!uprooted) {
						msg.source()
								.tell("You can't really go anywhere -- you are rooted!");
						return false;
					}
					uprooted = false;
				}
				...
				if ((msg.tool() != null)
					...
					if (!uprooted) {
						msg.source()
								.tell((MOB) affected, null, null,
										"<S-NAME> <S-IS-ARE> rooted and can't go anywhere.");
						return false;
					}
					uprooted = false;
				}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<S-NAME> become(s) rooted to the ground!"
									: "^S<S-NAME> chant(s) as <S-HIS-HER> feet become rooted in the ground!^?");
