...
public class Chant_Chlorophyll extends Chant {
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> skin returns to a normal color.");
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		Room R = mob.location();
		if ((R != null)
				&& ((R.getArea().getTimeObj().getTODCode() == TimeClock.TIME_DAY) || (R
				...
				&& ((R.getArea().getClimateObj().weatherType(R) == Climate.WEATHER_CLEAR)
						...
						|| (R.getArea().getClimateObj().weatherType(R) == Climate.WEATHER_WINTER_COLD) || (R
						.getArea().getClimateObj().weatherType(R) == Climate.WEATHER_HEAT_WAVE)))
			mob.curState().adjHunger(2,
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<S-NAME> gain(s) chlorophyll in <S-HIS-HER> skin!"
									: "^S<S-NAME> chant(s) to <T-NAMESELF>, turning <T-HIM-HER> a light shade of chlorophyll green!^?");
