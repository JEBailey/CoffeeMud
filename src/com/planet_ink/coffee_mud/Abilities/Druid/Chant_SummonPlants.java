...
public class Chant_SummonPlants extends Chant {
	...
	public void unInvoke() {
		...
		if (littlePlants == null)
			return;
		if (canBeUninvoked())
			PlantsLocation.showHappens(CMMsg.MSG_OK_VISUAL, littlePlants.name()
					+ " wither"
	...
	public String text() {
		if ((miscText.length() == 0) && (invoker() != null))
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((msg.amITarget(littlePlants))
			...
			msg.addTrailerMsg(CMClass.getMsg(msg.source(), littlePlants, null,
					CMMsg.MSG_OK_VISUAL, CMMsg.MASK_ALWAYS | CMMsg.MSG_DEATH,
	...
	public static Item buildPlant(MOB mob, Room room) {
		...
		room.showHappens(CMMsg.MSG_OK_ACTION, "Suddenly, " + newItem.name()
				+ " sprout(s) up here.");
	...
	public Item buildMyThing(MOB mob, Room room) {
		...
		if (A.getAreaIStats()[Area.Stats.VISITABLE_ROOMS.ordinal()] > 10)
			pct = (int) Math.round(100.0 * CMath.div(V.size(),
					A.getAreaIStats()[Area.Stats.VISITABLE_ROOMS.ordinal()]));
		Item I = buildMyPlant(mob, room);
		if ((I != null)
				&& ((mob.charStats().getCurrentClass().baseClass()
						.equalsIgnoreCase("Druid")) || (CMSecurity
						.isASysOp(mob)))) {
			if (!CMLib.law().isACity(A)) {
				if (pct > 0) {
					int newPct = (int) Math.round(100.0 * CMath.div(V.size(), A
							.getAreaIStats()[Area.Stats.VISITABLE_ROOMS
							.ordinal()]));
					if ((newPct >= 50)
							&& (A.fetchEffect("Chant_DruidicConnection") == null)) {
						Ability A2 = CMClass
								.getAbility("Chant_DruidicConnection");
						if (A2 != null)
							A2.invoke(mob, A, true, 0);
					}
				}
			} else if ((bonusWorthy) && (!mob.isMonster())) {
				long[] num = (long[]) plantBonuses.get(mob.Name() + "/"
						+ room.getArea().Name());
				if ((num == null)
						|| (System.currentTimeMillis() - num[1] > (room
								.getArea().getTimeObj().getDaysInMonth()
								* room.getArea().getTimeObj().getHoursInDay() * CMProps
									.getMillisPerMudHour()))) {
					num = new long[2];
					plantBonuses.remove(mob.Name() + "/"
							+ room.getArea().Name());
					plantBonuses.put(mob.Name() + "/" + room.getArea().Name(),
							num);
