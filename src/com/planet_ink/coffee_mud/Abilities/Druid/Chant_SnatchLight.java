...
public class Chant_SnatchLight extends Chant {
	...
	public Room snatchLocation() {
		if ((invoker != null) && CMLib.flags().isInTheGame(invoker, false)
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if (CMLib.map().roomLocation(affected) == snatchLocation()) {
			affectableStats.setDisposition(affectableStats.disposition()
					| PhyStats.IS_DARK);
			if (CMath.bset(affectableStats.disposition(),
					PhyStats.IS_LIGHTSOURCE))
				affectableStats
						.setDisposition(CMath.unsetb(
								affectableStats.disposition(),
								PhyStats.IS_LIGHTSOURCE));
			if (CMath.bset(affectableStats.disposition(), PhyStats.IS_GLOWING))
				affectableStats.setDisposition(CMath.unsetb(
						affectableStats.disposition(), PhyStats.IS_GLOWING));
		} else
			unInvoke();
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((affected == invoker) && (invoker != null)) {
			...
			for (int m = 0; m < R.numInhabitants(); m++) {
				...
				if ((M != null)
						&& (CMLib.flags().isGlowing(M) || CMLib.flags()
								.isLightSource(M))
				...
				if (M != null)
					for (int i = 0; i < M.numItems(); i++) {
						...
						if ((I != null)
								&& (I.container() == null)
								&& (CMLib.flags().isGlowing(I) || CMLib.flags()
										.isLightSource(I))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already snatching light.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<S-NAME> gain(s) an aura of light snatching!"
									: "^S<S-NAME> chant(s), feeling <S-HIS-HER> body become a light snatcher!^?");
