...
public class Chant_Treeform extends Chant {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(mob)) {
			...
			CMMsg msg2 = CMClass.getMsg(msg.source(), item, msg.targetCode(),
					null);
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		affectableStats
				.setName("a tree that reminds you of " + affected.name());
		...
		if (canBeUninvoked()) {
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> body is no longer treeish.");
			if (oldState != null) {
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					...
					success = beneficialAffect(mob, target, asLevel,
							(mob.phyStats().level() + (2 * super
									.getXLEVELLevel(mob))) * 50);
					if (success) {
						mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> transform(s) into a tree!!");
