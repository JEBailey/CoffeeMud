				...
				if (R != lastRoom) {
					for (int i = 0; i < R.numInhabitants(); i++) {
						...
						if ((M != null) && (M != affected)) {
							...
							for (Enumeration<Behavior> e = M.behaviors(); e
								...
								if ((B != null)
										&& (B.grantsAggressivenessTo((MOB) affected)))
									enemy = true;
							}
						}
						if (enemy || fighting)
					...
					if (enemy || fighting) {
						...
						if (dir >= 0) {
							...
							if (fighting)
								((MOB) affected)
										.tell("The winds tell of fighting "
												+ far
												+ Directions
								...
								((MOB) affected)
										.tell("The winds tell of enemies "
												+ far
												+ Directions
														.getInDirectionName(dir)
												+ ".");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already attuned to the winds.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> gain(s) a sense of the winds!"
									: "^S<S-NAME> chant(s) for a sense of the winds!^?");
