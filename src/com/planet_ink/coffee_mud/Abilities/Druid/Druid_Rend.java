	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		boolean success = proficiencyCheck(mob,
				mob.charStats().getStat(CharStats.STAT_STRENGTH)
						- target.charStats().getStat(CharStats.STAT_STRENGTH)
						- 10, auto);
		if (success) {
			...
			int damage = CMLib.dice().roll(1, topDamage,
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				CMLib.combat()
						.postDamage(
								...
								"^F^<FIGHT^><S-NAME> <DAMAGE> <T-NAME> by rending <T-HIM-HER> with <S-HIS-HER> feet!^</FIGHT^>^?");
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> fail(s) to rend <T-NAMESELF>.");
