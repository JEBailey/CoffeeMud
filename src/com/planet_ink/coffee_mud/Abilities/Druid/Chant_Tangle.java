...
public class Chant_Tangle extends Chant {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amISource(mob)) {
			if ((!msg.sourceMajor(CMMsg.MASK_ALWAYS))
				...
				mob.location().show(mob, null, thePlants, CMMsg.MSG_OK_ACTION,
						"<S-NAME> struggle(s) against <O-NAME>.");
	...
	public void unInvoke() {
		...
		if (canBeUninvoked()) {
			if (!mob.amDead())
				mob.location()
						.show(mob, null, thePlants, CMMsg.MSG_NOISYMOVEMENT,
								"<S-NAME> manage(s) to break <S-HIS-HER> way free of <O-NAME>.");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			if (mob.location().show(mob, null, this,
					verbalCastCode(mob, null, auto),
					auto ? "" : "^S<S-NAME> begin(s) to chant.^?")) {
						...
						if (target.location() == mob.location()) {
							...
							target.location()
									.show(target,
											...
											thePlants,
											CMMsg.MSG_OK_ACTION,
											"<S-NAME> become(s) stuck in <O-NAME> as they grow and twist around <S-HIM-HER>!");
						}
					}
				}
			}
