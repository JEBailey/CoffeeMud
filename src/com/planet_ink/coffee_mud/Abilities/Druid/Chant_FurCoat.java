...
public class Chant_FurCoat extends Chant {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((theArmor.amWearingAt(Wearable.IN_INVENTORY)
				|| (theArmor.owner() == null) || (theArmor.owner() instanceof Room)))
			unInvoke();
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE> a fur coat.");
			return false;
		}
		...
		if (target.freeWearPositions(Wearable.WORN_TORSO, (short) -2048,
				(short) 0) <= 0) {
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "A thick coat of fur appears on <T-NAME>."
							: "^S<S-NAME> chant(s) for a thick coat of fur!^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				theArmor.basePhyStats().setArmor(
						2 * CMLib.ableMapper().qualifyingClassLevel(mob, this));
				long wornCode = (Wearable.WORN_TORSO | Wearable.WORN_ARMS
						| Wearable.WORN_FEET | Wearable.WORN_WAIST | Wearable.WORN_LEGS);
			...
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> chant(s) for a thick coat of fur, but nothing happen(s).");
