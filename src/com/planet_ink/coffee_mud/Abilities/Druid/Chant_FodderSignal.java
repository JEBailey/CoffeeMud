			...
			if (room != null) {
				for (int i = 0; i < room.numInhabitants(); i++) {
					...
					if ((M != null)
							...
							&& (M.isMonster())
							&& (!M.isInCombat())
							&& (!dummy.getGroupMembers(new HashSet<MOB>())
									.contains(M))
							&& (CMLib.flags().canBeSeenBy(dummy, M))) {
						if (room.show(M, dummy, CMMsg.MASK_MOVE
								| CMMsg.MSG_NOISE,
								"<S-NAME> howl(s) in anger at <T-NAMESELF>!"))
							CMLib.combat().postAttack(M, dummy,
									M.fetchWieldedItem());
					}
				}
			}
				...
				if (msg.value() <= 0) {
					mob.location()
							.show(target, null, CMMsg.MSG_OK_VISUAL,
									"A horrible angering flag is emitting from <S-NAME>!");
