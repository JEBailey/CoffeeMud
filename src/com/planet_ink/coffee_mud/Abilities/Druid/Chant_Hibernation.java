...
public class Chant_Hibernation extends Chant {
			...
			if (!mob.amDead()) {
				if (mob.location() != null)
					mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> end(s) <S-HIS-HER> hibernation.");
				else
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((!mob.isInCombat()) && (CMLib.flags().isSleeping(mob))) {
			roundsHibernating++;
			double man = ((mob.charStats().getStat(CharStats.STAT_INTELLIGENCE) + mob
					.charStats().getStat(CharStats.STAT_WISDOM)));
			mob.curState().adjMana(
					(int) Math.round((man * .1)
							+ ((mob.phyStats().level() + (2.0 * super
									.getXLEVELLevel(invoker()))) / 2.0)),
					mob.maxState());
			...
			mob.curState().adjMovement(
					(int) Math.round((move * .1)
							+ ((mob.phyStats().level() + (2.0 * super
									.getXLEVELLevel(invoker()))) / 2.0)),
					mob.maxState());
			if (!CMLib.flags().isGolem(mob)) {
				double hp = mob.charStats()
						.getStat(CharStats.STAT_CONSTITUTION);
				if (!CMLib.combat().postHealing(
						mob,
						mob,
						this,
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			// what happened.
			invoker = mob;
			CMMsg msg = CMClass.getMsg(mob, null, this, CMMsg.MSG_SLEEP
					| CMMsg.MASK_MAGIC, "<S-NAME> begin(s) to hibernate...");
			...
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> chant(s) to hibernate, but lose(s) concentration.");
