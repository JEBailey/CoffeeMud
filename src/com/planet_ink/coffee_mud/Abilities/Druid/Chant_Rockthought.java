...
public class Chant_Rockthought extends Chant {
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		if ((affected instanceof MOB)
				...
				&& (!CMath.bset(msg.sourceMajor(), CMMsg.MASK_ALWAYS)
						...
						&& (msg.othersMessage() != null) && (msg
						.othersMessage().length() > 0)))
			stubb = msg;
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		if ((affected instanceof MOB)
				&& (stubb != null)
				&& (!CMath.bset(msg.sourceMajor(), CMMsg.MASK_ALWAYS) && (!stubb
						.equals(msg)))) {
			// this can cause all kinds of potential problems ..
			// the number of checks to get around them probably isn't worth the
			// cost.
		}
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected instanceof MOB) && (stubb != null)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					stubb = null;
					success = maliciousAffect(mob, target, asLevel, 20,
