...
public class Chant_SummonTornado extends Chant {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((mob.location().getArea().getClimateObj()
				.weatherType(mob.location()) != Climate.WEATHER_THUNDERSTORM)
				&& (mob.location().getArea().getClimateObj()
						.weatherType(mob.location()) != Climate.WEATHER_WINDY)
				&& (mob.location().getArea().getClimateObj()
						.weatherType(mob.location()) != Climate.WEATHER_BLIZZARD)
				&& (!auto)) {
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							((auto ? "^JA"
									: "^S<S-NAME> chant(s) to the sky and a") + " tornado touches down!^?")
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (stuff.size() == 0)
					mob.location().showHappens(CMMsg.MSG_OK_ACTION,
							"The tornado dissipates harmlessly.");
				else
					while (stuff.size() > 0) {
						Object O = stuff.elementAt(CMLib.dice().roll(1,
								stuff.size(), -1));
						stuff.removeElement(O);
						Room R = (Room) availableRooms.elementAt(CMLib.dice()
								.roll(1, availableRooms.size(), -1));
						if (O instanceof Item) {
							...
							if (R == mob.location())
								mob.location()
										.show(mob, null, I,
												CMMsg.MSG_OK_ACTION,
												"The tornado picks up <O-NAME> and whisks it around.");
							else {
								mob.location()
										.show(mob, null, I,
												CMMsg.MSG_OK_ACTION,
												"The tornado picks up <O-NAME> and whisks it away.");
								R.moveItemTo(I, ItemPossessor.Expire.Never,
							...
							if (I.subjectToWearAndTear()) {
								...
								if (damage > 0)
									CMLib.combat().postItemDamage(mob, I, this,
											damage, CMMsg.TYP_COLD, null);
							}
						} else if (O instanceof MOB) {
							MOB M = (MOB) O;
							msg = CMClass.getMsg(M, mob.location(), null,
									CMMsg.MSG_LEAVE | CMMsg.MASK_ALWAYS,
									CMMsg.MSG_LEAVE, CMMsg.NO_EFFECT, null);
							CMMsg msg2 = CMClass.getMsg(mob, M, this,
									verbalCastCode(mob, M, auto), null);
							CMMsg msg3 = CMClass.getMsg(mob, M, this,
									verbalCastMask(mob, M, auto)
							...
							if ((mob.location().okMessage(M, msg))
								...
								if (R == mob.location())
									mob.location()
											.show(M, null, null,
													CMMsg.MSG_OK_ACTION,
													"The tornado picks <S-NAME> up and whisks <S-HIM-HER> around.");
								else {
									mob.location()
											.show(M, null, null,
													CMMsg.MSG_OK_ACTION,
													"The tornado picks <S-NAME> up and whisks <S-HIM-HER> away.");
									R.bringMobHere(M, false);
								}
								int maxDie = (int) Math.round(CMath.div(
										adjustedLevel(mob, asLevel), 2.0));
								int damage = CMLib.dice().roll(maxDie, 7, 1);
								if ((msg.value() > 0) || (msg2.value() > 0))
									damage = (int) Math.round(CMath.div(damage,
											2.0));
								CMLib.combat().postDamage(mob, M, this, damage,
										CMMsg.MASK_ALWAYS | CMMsg.TYP_GAS,
										Weapon.TYPE_BASHING,
										"The tornado <DAMAGE> <T-NAME>!");
								// if(R!=mob.location())
								// M.tell("Wait a minute! Where are you?");
							}
						}
					}
			}
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> chant(s) into the sky, but nothing happens.");
