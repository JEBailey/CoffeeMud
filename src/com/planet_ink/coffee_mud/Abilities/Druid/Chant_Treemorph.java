...
public class Chant_Treemorph extends Chant {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((tickID == Tickable.TICKID_MOB) && (affected != null)
				&& (tree != null) && (affected instanceof MOB)) {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((treeForm != null) && (affected instanceof MOB)) {
			if (affected.name().indexOf(' ') > 0)
				affectableStats.setName("a " + treeForm.name() + " called "
						+ affected.name());
			else
		...
		if (canBeUninvoked()) {
			...
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> <S-IS-ARE> no longer a tree.");
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					...
					tree.setDisplayText("an oak tree that reminds you of "
							+ target.name() + " is growing here.");
					tree.setDescription("It`s a tall oak tree, which seems to remind you of "
							+ target.name() + ".");
