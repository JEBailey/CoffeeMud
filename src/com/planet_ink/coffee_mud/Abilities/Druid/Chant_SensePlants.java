...
public class Chant_SensePlants extends Chant {
		...
		if ((okMaterials() != null) && (okMaterials().length > 0))
			for (int m = 0; m < okMaterials().length; m++)
				if ((R.myResource() & RawMaterial.MATERIAL_MASK) == okMaterials()[m])
					return "You sense "
							+ RawMaterial.CODES.NAME(R.myResource())
									.toLowerCase() + " here.";
	...
	public void messageTo(MOB mob) {
		...
		if (here.length() > 0)
			...
			if ((dirs.length() == 0) && (last.length() > 0))
				...
				mob.tell("You sense " + word() + " to " + dirs.substring(2)
						+ ", and " + last + ".");
		}
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null, "<S-NAME> <S-IS-ARE> already sensing "
					+ word() + ".");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> gain(s) sensitivity to " + word() + "!"
							: "^S<S-NAME> chant(s) and gain(s) sensitivity to "
									+ word() + "!^?");
