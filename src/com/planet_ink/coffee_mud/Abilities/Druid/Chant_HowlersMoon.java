...
public class Chant_HowlersMoon extends Chant {
	...
	public void unInvoke() {
		...
		if (!(affected instanceof MOB)) {
			if (affected instanceof Room)
				((Room) affected).showHappens(CMMsg.MSG_OK_VISUAL,
						"The howlers moon sets.");
			super.unInvoke();
			return;
		}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (affected instanceof Room) {
			...
			if ((numWolfs > 5)
					|| ((invoker() != null) && (numWolfs > (invoker()
							.phyStats().level() + (2 * super
							.getXLEVELLevel(invoker()))) / 10)))
				return true;
			if (fromDir < 0) {
				Vector choices = fillChoices(room);
				if (choices.size() == 0)
					return true;
				fromDir = ((Integer) choices.elementAt(CMLib.dice().roll(1,
			...
			if (fromDir >= 0) {
				...
				if (invoker() != null)
					level = invoker().phyStats().level() + 5
				...
				if (target.location() == room) {
					...
					if ((d < 33) && (invoker() != null)
							&& (invoker().location() == room)) {
					...
					} else if ((d > 66) && (invoker() != null)
							&& (invoker().location() == room))
						target.setVictim(invoker());
					beneficialAffect(target, target, 0,
	...
	protected Vector fillChoices(Room R) {
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if ((room != null) && ((room.domainType() & Room.INDOORS) == 0)
					&& (room.domainType() != Room.DOMAIN_OUTDOORS_AIR)
					&& ((exit != null) && (exit.isOpen())) && (opExit != null)
					&& (opExit.isOpen()))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		for (final Enumeration<Ability> a = target.effects(); a
			...
			if ((A != null)
					&& ((A.classificationCode() & Ability.ALL_DOMAINS) == Ability.DOMAIN_MOONALTERING)) {
				mob.tell("The moon is already under " + A.name()
						+ ", and can not be changed until this magic is gone.");
				return false;
			}
		}
