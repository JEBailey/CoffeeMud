	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((success) && (theTrail != null) && (target != null)) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					"^S<S-NAME> chant(s) for the animals.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				invoker = mob;
				displayText = "(seeking " + target.name() + ")";
				Chant_LocateAnimals newOne = (Chant_LocateAnimals) this
						.copyOf();
			...
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> chant(s) for the animals, but nothing happens.");
