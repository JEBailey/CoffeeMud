...
public class Chant_PlantTrap extends Chant implements Trap {
	...
	public static final String[] choices = { "Chant_PlantChoke",
	...
	public void doMyThing(MOB target) {
		if ((target != invoker()) && (target.location() != null)) {
			if ((!invoker().mayIFight(target))
				...
				if (them.size() > 0) {
					String s = (String) them.elementAt(CMLib.dice().roll(1,
							them.size(), -1));
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "This area seems to writh with malicious plants."
									: "^S<S-NAME> chant(s), stirring the plant life into maliciousness.^?");
