		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "^JA lightning bolt streaks out of the sky!^?"
									: "^S<S-NAME> chant(s) to <T-NAMESELF>.  Suddenly a lightning bolt streaks from the sky!^?")
									+ CMLib.protocol().msp("lightning.wav", 40));
			CMMsg msg2 = CMClass.getMsg(mob, target, this,
					verbalCastMask(mob, target, auto) | CMMsg.TYP_ELECTRIC,
					null);
