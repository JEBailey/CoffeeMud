	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (affected instanceof MOB) {
			if (msg.amISource((MOB) affected)
					&& ((msg.sourceMinor() == CMMsg.TYP_STAND) || (msg
			...
			if ((msg.target() == affected)
				...
				if (invoker.location() == affected)
					...
					int dir = CMLib.tracking().radiatesFromDir((Room) affected,
							rooms);
					if (dir >= 0)
						invoker.tell("You feel footsteps "
								+ Directions.getInDirectionName(dir));
				}
			} else if ((msg.tool() instanceof Ability)
					&& ((msg.tool().ID().equals("Prayer_Tremor")) || (msg
							.tool().ID().endsWith("_Earthquake")))) {
				if (invoker.location() == affected)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already sensing tremors.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), (auto ? ""
							...
							+ "<T-NAME> gain(s) a sense of the earth!^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				CMLib.tracking().getRadiantRooms(mob.location(), rooms, flags,
						null, 5, null);
				for (int r = 0; r < rooms.size(); r++) {
					Room R = rooms.elementAt(r);
					if ((R != mob.location())
