...
public class Chant_CaveIn extends Chant {
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		if ((affected instanceof Exit)
				&& ((msg.targetMinor() == CMMsg.TYP_ENTER)
						...
						.targetMinor() == CMMsg.TYP_OPEN))
				&& (msg.source().phyStats().height() >= 0)
				&& ((msg.tool() == affected) || (msg.target() == affected))) {
			msg.source()
					.tell("This exit is blocked by rubble, and can not be moved through.");
			return false;
		} else if ((affected instanceof MOB) && (msg.amISource((MOB) affected))) {
			...
			if ((!msg.sourceMajor(CMMsg.MASK_ALWAYS))
				...
				mob.location()
						.show(mob, null, null, CMMsg.MSG_OK_ACTION,
								"<S-NAME> struggle(s) to get out from under the rocks.");
				amountRemaining -= (mob.charStats().getStat(
						CharStats.STAT_STRENGTH) * 4);
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (mob.isMonster() && (givenTarget instanceof MOB))
			...
			int dir = Directions.getGoodDirectionCode(CMParms.combine(commands,
					0));
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "" : "^S<S-NAME> chant(s) at <T-NAMESELF>!^?");
				...
				if (target instanceof Exit)
					mob.location()
							.show(mob, target, CMMsg.MSG_OK_VISUAL,
									"A cave-in causes rubble to fall, blocking <T-NAME>!");
				else if (target instanceof MOB) {
					mob.location().show(mob, target, CMMsg.MSG_OK_VISUAL,
							"A cave-in drops rocks on <T-NAME>!");
					...
					if (((MOB) target).location() == mob.location())
						CMLib.combat().postDamage(mob, (MOB) target, this,
								...
								Weapon.TYPE_BASHING,
								"The falling rubble <DAMAGE> <T-NAME>!");
				}
				if (msg.value() <= 0)
					success = maliciousAffect(mob, target, asLevel,
							(target instanceof Exit) ? 0 : 10, 0);
			}
		} else
