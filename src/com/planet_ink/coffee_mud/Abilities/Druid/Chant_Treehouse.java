...
public class Chant_Treehouse extends Chant {
		...
		if (canBeUninvoked()) {
			...
			if ((R != null) && (R.roomID().equalsIgnoreCase(""))) {
				R.showHappens(CMMsg.MSG_OK_VISUAL,
						"The treehouse fades away...");
				while (R.numInhabitants() > 0) {
					MOB M = R.fetchInhabitant(0);
					if (M != null)
						room.bringMobHere(M, false);
				}
				while (R.numItems() > 0) {
					Item I = R.getItem(0);
					if (I != null)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		boolean isATree = ((mob.location().domainType() == Room.DOMAIN_OUTDOORS_WOODS) || (mob
				.location().domainType() != Room.DOMAIN_OUTDOORS_JUNGLE));
		Item myPlant = Druid_MyPlants.myPlant(mob.location(), mob, 0);
		if ((myPlant != null)
				&& ((myPlant.material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_WOODEN))
			isATree = true;
		...
		if ((mob.location().getRoomInDir(Directions.UP) != null)
				&& (mob.location().getRoomInDir(Directions.UP).roomID()
						.length() > 0)) {
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, null, this,
					verbalCastCode(mob, null, auto), auto ? ""
							: "^S<S-NAME> chant(s) for a treehouse!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				mob.location().showHappens(CMMsg.MSG_OK_VISUAL,
						"A treehouse appears up in a nearby tree!");
				...
				mob.location().setRawExit(Directions.UP,
						CMClass.getExit("ClimbableExit"));
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> chant(s) for a treehouse, but the magic fades.");
