		...
		if (affected != null)
			if (msg.amITarget(affected)) {
				...
				case CMMsg.TYP_DRINK: {
					MOB M = msg.source();
					int hp = CMLib.dice().roll(
							1,
							M.charStats().getStat(CharStats.STAT_CONSTITUTION)
									+ super.getX1Level(invoker())
									+ super.getXLEVELLevel(invoker()), 0);
					CMLib.combat().postHealing(M, M, this,
							CMMsg.MASK_ALWAYS | CMMsg.TYP_CAST_SPELL, hp, null);
					int mana = CMLib.dice().roll(
							1,
							((M.charStats().getStat(CharStats.STAT_WISDOM) + M
									.charStats().getStat(
											CharStats.STAT_INTELLIGENCE)) / 2)
									+ super.getX1Level(invoker())
									+ super.getXLEVELLevel(invoker()), 0);
					M.curState().adjMana(mana, M.maxState());
					int move = CMLib.dice().roll(
							1,
							((M.charStats().getStat(CharStats.STAT_WISDOM) + M
									.charStats().getStat(
											CharStats.STAT_INTELLIGENCE)) / 2)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> chant(s) for nectar, but nothing happens.");
