...
public class Chant_Worms extends Chant implements DiseaseAffect {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		mob.curState().adjHunger(-100,
				mob.maxState().maxHunger(mob.baseWeight()));
		if ((--plagueDown) <= 0) {
			plagueDown = 5;
			if (invoker == null)
				invoker = mob;
			int dmg = ((mob.phyStats().level() + (2 * super
					.getXLEVELLevel(invoker()))) / 4) + 1;
			CMLib.combat()
					.postDamage(invoker, mob, this, dmg, CMMsg.TYP_DISEASE, -1,
							"<T-NAME> feel(s) <T-HIS-HER> innards being consumed by worms!");
		}
		return true;
	}
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead())) {
				...
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> worms disease is cured.");
			}
	}
			...
			if ((mob.location().okMessage(mob, msg))
				...
				if ((msg.value() <= 0) && (msg2.value() <= 0)) {
					...
					mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> feel(s) very sick in the stomach!");
				} else
