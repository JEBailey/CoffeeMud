			...
			if ((mob.isInCombat()) && (mob.fetchWieldedItem() == null)) {
				...
				if ((R != null)
						...
						&& ((R.domainType() == Room.DOMAIN_OUTDOORS_WOODS)
								|| ((R.myResource() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_WOODEN) || (R
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				newItem.setName(RawMaterial.CODES.NAME(material).toLowerCase()
						+ " club");
				...
				int damage = 6;
				try {
					damage = (((level + (2 * super.getXLEVELLevel(mob))) - 1) / 2) + 2;
				} catch (Exception t) {
				}
				if (damage < 6)
					damage = 6;
				newItem.basePhyStats()
						.setDamage(damage + super.getX1Level(mob));
				...
				mob.location().showHappens(CMMsg.MSG_OK_ACTION,
						"A good looking club grows out of a tree and drops.");
