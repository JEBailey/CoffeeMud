...
public class Chant_FungusFeet extends Chant implements DiseaseAffect {
			...
			if (drawups >= 3.1) {
				if ((mob.location() != null)
					...
					mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
							"<S-YOU-POSS> feet rot off!");
					Ability A = CMClass.getAbility("Amputation");
					if (A != null) {
						int x = 100;
						while (((--x) > 0)
								&& A.invoke(mob, CMParms.parse("foot"), mob,
										true, 0)) {/* do nothing */
						}
	...
	public void affectCharState(MOB affected, CharState affectableState) {
		...
		if (affected == null)
			return;
		affectableState.setMovement((int) Math.round(CMath.div(
				affectableState.getMovement(),
				drawups + (0.1 * super.getX1Level(invoker())))));
	}
	...
	public void unInvoke() {
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead())
				...
				spreadImmunity(mob);
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"The fungus on <S-YOUPOSS> feet dies and falls off.");
			}
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "" : "^S<S-NAME> chant(s) at <T-YOUPOSS> feet!^?");
			CMMsg msg2 = CMClass
					.getMsg(mob, target, this,
							verbalCastMask(mob, target, auto)
									| CMMsg.TYP_DISEASE, null);
			if ((mob.location().okMessage(mob, msg))
				...
				if ((msg.value() <= 0) && (msg2.value() <= 0)) {
					...
					mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
							"A fungus sprouts up between <S-YOUPOSS> toes!");
				} else
					spreadImmunity(target);
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> chant(s) at <T-YOUPOSS> feet, but nothing happens.");
