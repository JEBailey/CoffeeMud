...
public class Chant_SummonDustdevil extends Chant {
			...
			if ((affected != null) && (affected instanceof MOB)
				...
				if (((mob.amFollowing() == null) || (mob.amDead()) || (mob
					...
					for (int i = 0; i < V.size(); i++) {
						...
						if ((mob.maxCarry() >= mob.phyStats().weight()
								+ I.phyStats().weight())
								&& (mob.maxItems() >= (mob.numItems() + I
										.numberOfItems())))
						...
						for (int i = 0; i < V.size(); i++) {
							CMMsg msg = CMClass
									.getMsg(mob, invoker,
											(Item) V.elementAt(i),
											CMMsg.MSG_GIVE,
											"<S-NAME> whirl(s) <O-NAME> to <T-NAMESELF>.");
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> chant(s) and summon(s) help from the air.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
	...
	public MOB determineMonster(MOB caster, int material) {
		...
		newMOB.basePhyStats().setWeight(
				caster.phyStats().level()
						* (caster.phyStats().level() + (2 * super
								.getXLEVELLevel(caster))));
		newMOB.baseCharStats().setStat(CharStats.STAT_STRENGTH,
				caster.phyStats().level() + (2 * super.getXLEVELLevel(caster)));
		newMOB.basePhyStats().setSensesMask(
				newMOB.basePhyStats().sensesMask() | PhyStats.CAN_SEE_DARK);
		newMOB.basePhyStats()
				.setSensesMask(
						newMOB.basePhyStats().sensesMask()
								| PhyStats.CAN_SEE_INVISIBLE);
		newMOB.basePhyStats().setSensesMask(
				newMOB.basePhyStats().sensesMask() | PhyStats.CAN_SEE_HIDDEN);
