		...
		if ((affected != null) && (affected instanceof MOB)
			...
			case CMMsg.TYP_LEAVE:
				if ((msg.tool() instanceof Exit)
					...
					msg.source()
							.tell("Your corporeal equipment, suspended in your shapeless form, will not pass through the door.");
					return false;
				}
				break;
			case CMMsg.TYP_GET:
			...
			case CMMsg.TYP_HANDS:
				msg.source().tell(
						"You have trouble manipulating matter in this form.");
				return false;
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already shapeless.");
			return false;
		}
		if ((!auto)
				&& (!mob.location().getArea().getClimateObj()
						.canSeeTheMoon(mob.location(), null))) {
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> chant that <T-NAME> be given a shapeless form.^?");
			...
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> chant(s) for a new shape, but nothing happens.");
