...
public class Chant_FindMate extends Chant {
	...
	public boolean isSuitableMate(MOB mate, MOB forMe) {
		...
		if ((mate == null) || (forMe == null))
			return false;
		if (mate.charStats().getStat(CharStats.STAT_GENDER) == forMe
		...
		if (((mate.charStats().getMyRace().ID().equals("Human"))
				|| (mate.charStats().getMyRace().ID().equals("Human")) || (mate
				.charStats().getMyRace().canBreedWith(mate.charStats()
				.getMyRace())))
				&& (mate.fetchWornItems(
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((success) && (theTrail != null)) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? null
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (A != null) {
					target.location().show(target, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> yearn(s) for a mate!");
					A.makeLongLasting();
					A.nextDirection = CMLib.tracking()
							.trackNextDirectionFromHere(theTrail,
									mob.location(), true);
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> chant(s) to <T-NAMESELF>, but nothing happen(s).");
