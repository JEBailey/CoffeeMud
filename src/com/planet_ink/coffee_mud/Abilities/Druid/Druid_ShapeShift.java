...
public class Druid_ShapeShift extends StdAbility {
	...
	private static String[][] shapes = {
			{ "Mouse", "Kitten", "Puppy", "Robin", "Garden Snake", "Cub",
					"Grasshopper", "Spider Monkey", "Calf" },
			{ "Rat", "Cat", "Dog", "Owl", "Snake", "Young Bear", "Centipede",
					"Chimp", "Cow" },
			{ "Dire Rat", "Puma", "Wolf", "Hawk", "Python", "Brown Bear",
					"Tarantula", "Ape", "Buffalo" },
			{ "WereRat", "Lion", "Dire Wolf", "Eagle", "Cobra", "Black Bear",
					"Scarab", "Gorilla", "Bull" },
			{ "WereBat", "Manticore", "WereWolf", "Harpy", "Naga", "WereBear",
					"ManScorpion", "Sasquatch", "Minotaur" } };
	private static String[][] races = {
			{ "Mouse", "Kitten", "Puppy", "Robin", "GardenSnake", "Cub",
					"Grasshopper", "Monkey", "Calf" },
			{ "Rat", "Cat", "Dog", "Owl", "Snake", "Cub", "Centipede", "Chimp",
					"Cow" },
			{ "DireRat", "Puma", "Wolf", "Hawk", "Python", "Bear", "Tarantula",
					"Ape", "Buffalo" },
			{ "WereRat", "Lion", "DireWolf", "Eagle", "Cobra", "Bear",
					"Scarab", "Gorilla", "Bull" },
			{ "WereBat", "Manticore", "WereWolf", "Harpy", "Naga", "WereBear",
					"ManScorpion", "Sasquatch", "Minotaur" } };
	private static double[] attadj = { .7, 1.0, 1.0, .2, .3, 1.2, .7, 1.0, .7 };
	private static double[] dmgadj = { .4, .6, .8, 1.0, .4, .6, .6, .8, 1.0 };
	private static double[] armadj = { 1.0, .5, .4, .5, 1.0, .3, 1.0, .2, .2 };

	private static String[] forms = { "Rodent form", "Feline form", "K-9 form",
			"Bird form", "Snake form", "Bear form", "Insect form",
			"Monkey form", "Bovine form" };
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((newRace != null) && (affected instanceof MOB)) {
			...
			newRace.setHeightWeight(affectableStats, (char) ((MOB) affected)
	...
	public void unInvoke() {
		...
		if ((canBeUninvoked()) && (mob.location() != null))
			mob.location().show(
					...
					CMMsg.MSG_OK_VISUAL,
					"<S-NAME> revert(s) to "
							+ mob.charStats().raceName().toLowerCase()
							+ " form.");
	}
	...
	public int getRaceCode() {
		if ((myRaceCode < 0) || (myRaceCode > attadj.length))
			return 0;
		return myRaceCode;
	}
		...
		if (mob != null) {
			if (target instanceof MOB) {
				if ((((MOB) target).isInCombat())
					...
					int qualClassLevel = CMLib.ableMapper()
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (myRaceCode < 0) {
			if (mob.isMonster()) {
				...
				try {
					if (!mob.session()
							.confirm(
									"You have not yet chosen your form, would you like to now (Y/n)?",
									"Y"))
						return false;
					...
					for (int i = 0; i < forms.length; i++) {
						if (racesTaken[i] == 0) {
							str.append(CMStrings.padLeft("" + (i + 1), 2)
									+ ") " + forms[i] + "\n\r");
							choices.append("" + (i + 1));
						}
					}
					str.append("Please select: ");
					String choice = mob.session().choose(str.toString(),
							choices.toString(), "");
		...
		if (parm.length() > 0) {
			...
			for (int i1 = raceLevel; i1 >= 0; i1--)
				if (CMLib.english()
						.containsString(shapes[i1][myRaceCode], parm)) {
					parm = "";
					this.myRaceLevel = i1;
				}
		}
		...
		if ((triggerStrings().length > 0) && (parm.length() > 0)
				&& (allShapeshifts.size() > 1)) {
			...
			for (int i = 0; i < allShapeshifts.size(); i++) {
				Druid_ShapeShift A = (Druid_ShapeShift) allShapeshifts
						.elementAt(i);
				if (A.myRaceCode >= 0) {
					if ((A.raceName == null) || (A.raceName.length() == 0))
						A.setRaceName(mob);
					if ((A.raceName == null) || (A.raceName.length() == 0))
						list.append(CMStrings.padLeft("" + (i + 1), 2)
								+ ") Not yet chosen.\n\r");
					else {
						list.append(CMStrings.padLeft("" + (i + 1), 2) + ") "
								+ forms[A.myRaceCode] + ": ");
						...
						if (CMLib.english().containsString(A.raceName, parm))
							return A.invoke(mob, new Vector(), givenTarget,
									auto, asLevel);
						if (CMLib.english().containsString(forms[A.myRaceCode],
								parm))
							return A.invoke(mob, new Vector(), givenTarget,
									auto, asLevel);
						for (int i1 = raceLevel; i1 >= 0; i1--)
							if (CMLib.english().containsString(
									shapes[i1][A.myRaceCode], parm))
								return A.invoke(mob, new XVector(parm),
										givenTarget, auto, asLevel);
					}
				}
			}
		...
		if (success) {
			...
			// what happened.
			CMMsg msg = CMClass.getMsg(mob, null, this, CMMsg.MSG_OK_ACTION,
					null);
			if (mob.location().okMessage(mob, msg)) {
				...
				mob.location().show(
						...
						null,
						CMMsg.MSG_OK_VISUAL,
						"<S-NAME> take(s) on " + raceName.toLowerCase()
								+ " form.");
				beneficialAffect(mob, mob, asLevel, Ability.TICKS_FOREVER);
				raceName = CMStrings.capitalizeAndLower(CMLib.english()
						.startWithAorAn(raceName.toLowerCase()));
				CMLib.utensils().confirmWearability(mob);
			}
		} else
			beneficialWordsFizzle(mob, null,
					"<S-NAME> chant(s) to <S-HIM-HERSELF>, but nothing happens.");
