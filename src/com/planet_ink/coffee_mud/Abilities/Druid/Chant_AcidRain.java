				...
				for (int i = 0; i < R.numInhabitants(); i++) {
					...
					if (M != null) {
						if (CMLib.dice().rollPercentage() > M.charStats()
								.getSave(CharStats.STAT_SAVE_ACID))
							CMLib.combat()
									.postDamage(
											invoker(),
											M,
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					...
					mob.location().showHappens(CMMsg.MSG_OK_VISUAL,
							"Acid rain starts pouring from the sky!");
					maliciousAffect(mob, target, asLevel, 0, -1);
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> chant(s) to the rain, but the magic fades.");
