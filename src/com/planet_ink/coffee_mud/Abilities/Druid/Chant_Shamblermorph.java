...
public class Chant_Shamblermorph extends Chant {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		if ((treeForm != null) && (affected instanceof MOB)) {
			if (affected.name().indexOf(' ') > 0)
				affectableStats.setName("a shambling mound called "
						+ affected.name());
			else
		...
		if (canBeUninvoked()) {
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> <S-IS-ARE> no longer a shambling mound.");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		boolean malicious = !target.getGroupMembers(new HashSet<MOB>())
				.contains(mob);
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(
					...
					target,
					this,
					(malicious ? CMMsg.MASK_MALICIOUS : 0)
				...
				if (msg.value() <= 0) {
					target.location()
							.show(target,
									null,
									CMMsg.MSG_OK_VISUAL,
									"Leaves sprout from <S-YOUPOSS> skin as <S-HE-SHE> grow(s) into a Shambling Mound!");
					if (malicious)
