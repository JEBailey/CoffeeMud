...
public class Chant_MagneticEarth extends Chant {
				...
				if ((M != null) && (M != invoker)) {
					...
					for (int i = 0; i < M.numItems(); i++) {
						...
						if ((I != null)
								...
								&& (I.amWearingAt(Wearable.IN_INVENTORY)
										...
										|| I.amWearingAt(Wearable.WORN_EYES) || I
					...
					for (int i = 0; i < toGo.size(); i++) {
						...
						if (CMLib.commands().postDrop(M, I, true, true, false)) {
							didSomething = true;
							R.show(M, I, CMMsg.MSG_OK_VISUAL,
									"<T-NAME> is pulled away from <S-NAME> to the magnetic ground!");
						}
					}
				}
			}
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		if ((!checked) && (msg.targetMinor() == CMMsg.TYP_ENTER)
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					for (int i = 0; i < target.numInhabitants(); i++) {
						...
						if ((M != null) && (mob != M))
							mob.location().show(mob, M,
									CMMsg.MASK_MALICIOUS | CMMsg.TYP_OK_VISUAL,
									null);
					}
					mob.location().showHappens(CMMsg.MSG_OK_VISUAL,
							"The ground gains a powerful magnetic field!");
					maliciousAffect(mob, target, asLevel, 0, -1);
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> chant(s) the ground, but the magic fades.");
