...
public class Chant_WindColor extends Chant {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((tickID == Tickable.TICKID_MOB)
			...
			String prediction = getWindColor((MOB) affected,
					((MOB) affected).location());
			if (prediction.length() > 0)
				((MOB) affected).tell("The winds are " + prediction + ".");
		}
		return true;
	}
	...
	public static String getWindColor(MOB mob, Room R) {
		...
		for (int i = 0; i < R.numInhabitants(); i++) {
			...
			if ((M != null) && (M != mob) && (!group.contains(M))) {
				...
				int done = 0;
				if (M.charStats().getMyRace().racialCategory()
		...
		if (V.size() == 1)
			return str.toString()
					+ getColor(((Integer) V.firstElement()).intValue());
		for (int i = 0; i < V.size(); i++) {
			int x = ((Integer) V.elementAt(i)).intValue();
			if (i == V.size() - 1)
				str.append("and " + getColor(x) + " ");
			else if (i > 0)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already watching the winds.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> gain(s) visions of the winds!"
							: "^S<S-NAME> chant(s) for visions on the wind!^?");
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> chant(s) into the air, but the magic fizzles.");
