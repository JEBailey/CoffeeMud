		...
		if ((msg.target() != null) && (msg.target() instanceof MOB)) {
			...
			if ((msg.amISource(myChar))
					...
					&& ((myChar.charStats().getStat(CharStats.STAT_GENDER) == ('M')) || (myChar
							.charStats().getStat(CharStats.STAT_GENDER) == ('F')))
					&& (!mate.charStats().getMyRace()
							.canBreedWith(myChar.charStats().getMyRace()))
					...
					&& (mate.fetchWornItems(
							Wearable.WORN_LEGS | Wearable.WORN_WAIST,
							(short) -2048, (short) 0).size() == 0)
					&& ((mate.charStats().getStat(CharStats.STAT_AGE) == 0) || ((mate
							.charStats().ageCategory() > Race.AGE_CHILD) && (mate
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> seem(s) strangely fertile!");
