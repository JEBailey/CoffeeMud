		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> chant(s) and summon(s) a companion from the Java Plane.^?");
			...
			if (room.okMessage(mob, msg)) {
				...
				newRoom.showOthers(target, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> appears!");
				...
				return beneficialWordsFizzle(mob, null,
						"<S-NAME> chant(s) and summon(s), but nothing happens.");
		} else
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> chant(s) and summon(s), but nothing happens.");
