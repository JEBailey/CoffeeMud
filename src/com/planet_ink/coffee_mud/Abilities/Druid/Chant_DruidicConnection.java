...
public class Chant_DruidicConnection extends Chant {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((invoker == null)
				...
				|| ((affected instanceof Area) && (!((Area) affected)
						.inMyMetroArea(invoker().location().getArea())))) {
			unInvoke();
			return false;
		}
		...
		if (affected instanceof Area) {
			...
			long millisPerHoursPerDay = hoursPerDay
					* CMProps.getMillisPerMudHour();
			if (ellapsed >= millisPerHoursPerDay) {
				lastTime = System.currentTimeMillis();
				Vector V = Druid_MyPlants.myAreaPlantRooms(invoker(),
						(Area) affected);
				int pct = 0;
				if (((Area) affected).getAreaIStats()[Area.Stats.VISITABLE_ROOMS
						.ordinal()] > 10)
					pct = (int) Math
							.round(100.0 * CMath.div(
									V.size(),
									((Area) affected).getAreaIStats()[Area.Stats.VISITABLE_ROOMS
											.ordinal()]));
				if (pct < 50) {
					unInvoke();
					return false;
				...
				int xp = (int) Math
						.round(5.0 * CMath.mul(
								CMath.div(
										V.size(),
										((Area) affected).getAreaIStats()[Area.Stats.VISITABLE_ROOMS
												.ordinal()]), ((Area) affected)
										.getAreaIStats()[Area.Stats.AVG_LEVEL
										.ordinal()]));
				CMLib.leveler()
						.postExperience(invoker(), null, null, xp, false);
			}
		}
		return true;
	}
		...
		if (affected instanceof MOB) {
			...
			if ((mob == invoker())
				...
				stats.setAttackAdjustment((int) Math.round(CMath.mul(
						1.1 + (0.05 * super.getXLEVELLevel(invoker())),
						stats.attackAdjustment())));
				stats.setArmor(stats.armor() - (2 * stats.level())
	...
	public void unInvoke() {
		if ((canBeUninvoked()) && (invoker != null)
			...
			Vector V = Druid_MyPlants
					.myAreaPlantRooms(invoker, (Area) affected);
			if (V.size() > 1)
				V.removeElementAt(0);
			for (int v = 0; v < V.size(); v++) {
				Item I = Druid_MyPlants.myPlant((Room) V.elementAt(v), invoker,
						0);
				int num = 0;
				while (I != null)
					I = Druid_MyPlants.myPlant((Room) V.elementAt(v), invoker,
							++num);
			...
			invoker.tell("You have destroyed your connection with "
					+ affected.name() + "!");
			for (Enumeration e = ((Area) affected).getMetroMap(); e
					.hasMoreElements();)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target == null)
			return false;
		boolean quietly = ((commands != null) && (commands.size() > 0) && (commands
				.contains("QUIETLY")));
		...
		if (target.getAreaIStats()[Area.Stats.VISITABLE_ROOMS.ordinal()] > 10)
			pct = (int) Math
					.round(100.0 * CMath.div(V.size(),
							target.getAreaIStats()[Area.Stats.VISITABLE_ROOMS
									.ordinal()]));
		if (pct < 50) {
			if (!quietly)
		...
		if ((!auto)
				&& (!mob.charStats().getCurrentClass().baseClass()
						.equalsIgnoreCase("Druid"))) {
			if (!quietly)
				mob.tell("Only druids can make this connection.");
			return false;
		}

		if (CMLib.law().isACity(target) && (!auto)) {
			if (!quietly)
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "This area seems connected to <S-NAME>."
									: "^S<S-NAME> chant(s), establishing a natural connection with this area.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				beneficialAffect(mob, target, asLevel, 0);
				Chant_DruidicConnection A = (Chant_DruidicConnection) target
						.fetchEffect(ID());
				if (A != null) {
					A.setSavable(false);
					A.makeLongLasting();
					A.lastTime = System.currentTimeMillis();
					for (Enumeration e = target.getMetroMap(); e
							.hasMoreElements();)
						((Room) e.nextElement()).recoverRoomStats();
				}
			} else
