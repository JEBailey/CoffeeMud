		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> chant(s) softly to <S-HIM-HERSELF>!^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				for (Iterator<Room> r = checkSet.iterator(); r.hasNext();) {
					...
					if ((((R.domainType() & Room.INDOORS) == 0)
							&& (R.domainType() != Room.DOMAIN_OUTDOORS_CITY) && (R
						...
						for (int m = 0; m < R.numInhabitants(); m++) {
							...
							if ((M != null)
								...
								lines.append("^!"
										+ CMStrings.padRight(M.name(mob), 25)
										+ "^?| ");
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> chant(s) softly to <S-HIM-HERSELF>, but the magic fades.");
