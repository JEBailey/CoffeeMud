...
public class Chant_ControlWeather extends Chant {
		...
		if ((msg.tool() instanceof Ability)
			...
			if (abilityCode() == 1) {
				...
				if ((!msg.amISource(invoker()))
					...
					msg.source()
							.tell("The weather is finely balanced here, and will not heed your call.");
					return false;
				}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Chant_ControlWeather A = (Chant_ControlWeather) mob.location()
				.getArea().fetchEffect(ID());
		if ((A != null) && (A.abilityCode() == 1)) {
			long remaining = A.tickDown * CMProps.getTickMillis();
			mob.tell("This area is under an enchantment of climactic balance, which can not be controlled for "
					+ mob.location().getArea().getTimeObj()
							.deriveEllapsedTimeString(remaining) + ".");
			return false;
		}
		...
		if (A != null)
			size = size
					- ((A.invoker().phyStats().level() - (mob.phyStats()
							.level() + (super.getXLEVELLevel(mob)))) * 10);
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							this,
							verbalCastCode(mob, mob.location().getArea(), auto),
							auto ? "The sky changes color as the weather comes under control!"
									: "^S<S-NAME> chant(s) into the sky for control of the weather!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if ((A != null) && (A.invoker() != mob))
					mob.tell("You successfully wrest control of the weather from "
							+ A.invoker().name() + ".");
				if (A != null)
					A.unInvoke();
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> chant(s) into the sky for control, but the magic fizzles.");
