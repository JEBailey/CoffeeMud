...
public class Chant_Stonewalking extends Chant {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amISource(mob)
				&& ((CMath.bset(msg.sourceMajor(), CMMsg.MASK_MALICIOUS)) || ((msg
						.sourceMinor() == CMMsg.TYP_GET) && ((msg.target() == null) || (!mob
						.isMine(msg.target()))))))
			unInvoke();
		return;
	}
			...
			if ((mob.location() != null) && (!mob.amDead())) {
				...
				if ((R.domainType() == Room.DOMAIN_INDOORS_CAVE)
					...
					mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> <S-IS-ARE> drawn out of the walls.");
				else if ((R.domainType() == Room.DOMAIN_OUTDOORS_MOUNTAINS)
						|| (R.domainType() == Room.DOMAIN_OUTDOORS_ROCKS))
					mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> <S-IS-ARE> drawn out of the rocks.");
				else
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null, "<S-NAME> <S-IS-ARE> already "
					+ name() + ".");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> chant(s) quietly to <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> fade(s) into the walls!");
				beneficialAffect(mob, target, asLevel, 0);
			}
		} else
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> chant(s) quietly to <T-NAMESELF>, but nothing more happens.");
