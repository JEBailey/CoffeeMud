		...
		if (allResources == null) {
			...
			if (okResources() != null)
				for (int m = 0; m < okResources().length; m++)
					if (!allResources.contains(Integer
							.valueOf(okResources()[m])))
						allResources.addElement(Integer
								.valueOf(okResources()[m]));
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (tickID == Tickable.TICKID_MOB) {
			...
			if (nextDirection == 999) {
				...
				mob.tell("Your sense " + lookingFor + " "
						+ Directions.getInDirectionName(nextDirection) + ".");
				nextDirection = -2;
			}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already trying to " + name());
			return false;
		}
		...
		if (commands.size() == 0) {
			mob.tell("Find which " + lookingFor + "?  Use 'CHANT \"" + name()
					+ "\" LIST' for a list.");
			return false;
		}
		...
		if (s.equalsIgnoreCase("LIST")) {
			StringBuffer msg = new StringBuffer(
					"You may search for any of the following: ");
			for (int i = 0; i < allOkResources().size(); i++)
				msg.append(RawMaterial.CODES.NAME(
						((Integer) allOkResources().elementAt(i)).intValue())
						.toLowerCase()
						+ ", ");
		...
		if (whatImLookingFor < 0) {
			mob.tell("'" + s
					+ "' cannot be found with this chant.    Use 'CHANT \""
					+ name() + "\" LIST' for a list.");
			return false;
		}
		...
		if ((success) && (theTrail != null)) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> begin(s) to " + name().toLowerCase()
							+ "s!" : "^S<S-NAME> chant(s) for " + lookingFor
							+ ".^?");
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> chant(s), but gain(s) nothing from it.");
