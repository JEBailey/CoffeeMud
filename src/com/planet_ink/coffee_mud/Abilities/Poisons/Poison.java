...
public class Poison extends StdAbility implements HealthCondition {
	...
	protected int POISON_DAMAGE() {
		return (invoker != null) ? CMLib.dice().roll(1,
				invoker().phyStats().level(), 1) : 0;
	}
	...
	protected boolean catchIt(MOB mob, Physical target) {
		...
		if ((target != null) && (target instanceof MOB)
			...
			String startMsg = CMStrings
					.replaceAll(POISON_START(), "<S-", "<T-");
			CMMsg msg = CMClass
					.getMsg(poisoner, targetMOB, this, CMMsg.MASK_ALWAYS
							| CMMsg.MASK_MALICIOUS | CMMsg.TYP_POISON, startMsg);
			if ((POISON_START_TARGETONLY().length() > 0)
					|| ((targetMOB.location() != null) && (targetMOB.location()
							.okMessage(targetMOB, msg)))) {
				targetMOB.location().send(targetMOB, msg);
				if ((POISON_AFFECTTARGET() && (msg.value() <= 0))) {
					maliciousAffect(poisoner, target,
							(affected instanceof Item) ? affected.phyStats()
									.level() : 0, POISON_TICKS(), -1);
					return true;
				}
				return false;
			}
		...
		if ((--poisonTick) <= 0) {
			...
			if (POISON_AFFECT().length() > 0)
				mob.location().show(
						...
						CMMsg.MSG_OK_VISUAL,
						POISON_AFFECT()
								+ CMLib.protocol().msp("poisoned.wav", 10));
			if (invoker == null)
				invoker = mob;
			if (POISON_DAMAGE() != 0) {
				CMLib.combat().postDamage(invoker, mob, this, POISON_DAMAGE(),
						CMMsg.MASK_ALWAYS | CMMsg.TYP_POISON, -1, null);
				if ((!mob.isInCombat()) && (mob != invoker)
						&& (mob.location() != null)
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_CONSTITUTION,
				affectableStats.getStat(CharStats.STAT_CONSTITUTION) - 5);
				...
				if (msg.amITarget(myItem))
					...
					case CMMsg.TYP_DRINK:
						if (myItem instanceof Drink) {
							...
							if (((((Drink) myItem).liquidRemaining() < 0) || ((((Drink) myItem)
									.liquidRemaining() - ((Drink) myItem)
									.thirstQuenched()) <= 0))
									&& (!((Drink) myItem)
					...
					case CMMsg.TYP_FILL:
						if ((msg.target() instanceof Drink)
							...
							((Drink) msg.target()).addEffect((Ability) this
									.copyOf());
							if ((((Drink) affected).liquidRemaining() - ((Drink) msg
									.target())
									.amountTakenToFillMe((Drink) affected)) <= 0)
								affected.delEffect(this);
						}
						break;
					}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Environmental target = this.getAnyTarget(mob, commands, givenTarget,
				...
				if (msg.value() <= 0) {
					if (target instanceof MOB) {
						if (POISON_START_TARGETONLY().length() > 0)
							...
							R.show((MOB) target, null, CMMsg.MSG_OK_VISUAL,
									POISON_START());
						if (POISON_AFFECTTARGET())
							success = maliciousAffect(mob, (MOB) target,
									asLevel, POISON_TICKS(), -1);
						else
							success = true;
					} else if (target instanceof Physical)
						success = maliciousAffect(mob, (Physical) target,
								asLevel, 100, -1);
				} else
					success = false;
			} else
