...
public class Skill_Revoke extends StdSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((whatToRevoke.length() == 0) && (mob.location().numEffects() > 0))
			target = mob.location();
			...
			if (dir >= 0)
				...
				target = mob.location().fetchFromRoomFavorMOBs(null,
						whatToRevoke);
				if (target == null)
		...
		if ((target == null) || (!CMLib.flags().canBeSeenBy(target, mob))) {
			mob.tell("Revoke from what?  You don't see '" + whatToRevoke
					+ "' here.");
			return false;
		}
		...
		for (int a = 0; a < target.numEffects(); a++) {
			...
			if ((A != null)
					...
					&& (((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_SPELL)
							...
							|| ((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_PRAYER) || ((A
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, null, this, CMMsg.MSG_HANDS,
					"<S-NAME> revoke(s) " + revokeThis.name() + " from "
							+ target.name());
			...
			beneficialVisualFizzle(mob, target,
					"<S-NAME> attempt(s) to revoke " + revokeThis.name()
							+ " from " + target.name() + ", but flub(s) it.");
		return success;
	}
