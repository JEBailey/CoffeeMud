	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		if ((affected instanceof MOB) && (((MOB) affected).isInCombat()))
			affectableStats.setSpeed(affectableStats.speed()
					+ (numberOfFullAttacks() * (proficiency() / 100.0)));
		if ((freeToNerf) && (affectableStats.attackAdjustment() > 0))
			affectableStats.setAttackAdjustment((int) Math
					.round(affectableStats.attackAdjustment() * nerfAmount()));
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amISource(mob))
			...
			if (freeToNerf) {
				...
				mob.recoverPhyStats();
				// chargen combat fighter iterations=100 skiplevels=4
				// export=output6.txt 1 91
			}
		}
	}
