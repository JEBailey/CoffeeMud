...
public class Skill_HandCuff extends StdSkill {
		...
		if (msg.amISource(mob)) {
			if (msg.sourceMinor() == CMMsg.TYP_RECALL) {
				if ((msg.source() != null) && (msg.source().location() != null))
					msg.source()
							.location()
							.show(msg.source(), null, CMMsg.MSG_OK_ACTION,
									"<S-NAME> attempt(s) to recall, but the handcuffs prevent <S-HIM-HER>.");
				return false;
			} else if (((msg.sourceMinor() == CMMsg.TYP_FOLLOW) && (msg
					.target() != invoker()))
					|| ((msg.sourceMinor() == CMMsg.TYP_NOFOLLOW) && (msg
							.source().amFollowing() == invoker()))) {
				mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> struggle(s) against <S-HIS-HER> cuffs.");
				...
				return true;
					...
					&& (msg.target() != null) && (msg.target() instanceof Room) && (!((Room) msg
						.target()).isInhabitant(invoker)))) {
				mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> struggle(s) against <S-HIS-HER> cuffs.");
				amountRemaining -= (mob.charStats().getStat(
						CharStats.STAT_STRENGTH) + mob.phyStats().level());
				...
				&& (msg.amITarget(affected)) && (!mob.isInCombat())
				&& (mob.amFollowing() != null) && (msg.source().isMonster())
		...
		if (canBeUninvoked()) {
			...
			if (!mob.amDead())
				mob.location().show(mob, null, CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> <S-IS-ARE> released from the handcuffs.");
			if (!oldAssist)
	...
	public int castingQuality(MOB mob, Physical target) {
		if ((mob != null) && (target instanceof MOB)) {
			...
			if (Skill_Arrest.getWarrantsOf((MOB) target,
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((commands.size() > 0)
				&& ((String) commands.firstElement()).equalsIgnoreCase("UNTIE")) {
			commands.removeElementAt(0);
			MOB target = super.getTarget(mob, commands, givenTarget, false,
					true);
			if (target == null)
				return false;
			Ability A = target.fetchEffect(ID());
			if (A != null) {
				if (mob.location().show(mob, target, null, CMMsg.MSG_HANDS,
						"<S-NAME> attempt(s) to unbind <T-NAMESELF>.")) {
					A.unInvoke();
					return true;
				}
				return false;
		...
		if (Skill_Arrest.getWarrantsOf(target,
				CMLib.law().getLegalObject(mob.location().getArea())).size() == 0) {
		...
		if (success) {
				...
				if (msg.value() <= 0) {
					...
					if (target.location() == mob.location()) {
						success = maliciousAffect(mob, target, asLevel,
								Ability.TICKS_ALMOST_FOREVER, -1);
						if (success) {
							Skill_HandCuff A = (Skill_HandCuff) target
									.fetchEffect(ID());
							if (A != null) {
							...
							oldAssist = CMath.bset(target.getBitmap(),
									MOB.ATT_AUTOASSIST);
							if (!oldAssist)
								target.setBitmap(CMath.setb(target.getBitmap(),
										MOB.ATT_AUTOASSIST));
							oldGuard = CMath.bset(target.getBitmap(),
									MOB.ATT_AUTOASSIST);
							if (oldGuard)
								target.setBitmap(CMath.unsetb(
										target.getBitmap(), MOB.ATT_AUTOGUARD));
							boolean oldNOFOL = CMath.bset(target.getBitmap(),
									MOB.ATT_NOFOLLOW);
							if (target.numFollowers() > 0)
								CMLib.commands().forceStandardCommand(target,
										"NoFollow",
										new XVector("UNFOLLOW", "QUIETLY"));
							target.setBitmap(CMath.unsetb(target.getBitmap(),
									MOB.ATT_NOFOLLOW));
							...
							if (oldNOFOL)
								...
								target.setBitmap(CMath.unsetb(
										target.getBitmap(), MOB.ATT_NOFOLLOW));
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to bind <T-NAME> and fail(s).");
