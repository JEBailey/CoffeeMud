...
public class Skill_Map extends StdSkill {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if (!(affected instanceof MOB))
			return;
		MOB mob = (MOB) affected;
		if ((map.owner() == null) || (map.owner() != mob))
			unInvoke();
				...
				&& (!CMath.bset(((Room) msg.target()).phyStats().sensesMask(),
						PhyStats.SENSE_ROOMUNMAPPABLE))) {
			roomsMappedAlready.addElement(msg.target());
			map.setReadableText(map.readableText() + ";"
					+ CMLib.map().getExtendedRoomID((Room) msg.target()));
			if (map instanceof com.planet_ink.coffee_mud.Items.interfaces.RoomMap)
				((com.planet_ink.coffee_mud.Items.interfaces.RoomMap) map)
						.doMapArea();
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MSG_WRITE,
					"<S-NAME> start(s) mapping on <T-NAMESELF>.",
					CMMsg.MSG_WRITE, ";", CMMsg.MSG_WRITE,
					"<S-NAME> start(s) mapping on <T-NAMESELF>.");
			if (mob.location().okMessage(mob, msg)) {
				...
				if (!item.ID().equals("BardMap")) {
					...
					CMLib.flags().setRemovable(B,
							CMLib.flags().isRemovable(item));
			...
			mob.location()
					.show(mob, target, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> attempt(s) to start mapping on <T-NAMESELF>, but mess(es) up.");
		return success;
	}
