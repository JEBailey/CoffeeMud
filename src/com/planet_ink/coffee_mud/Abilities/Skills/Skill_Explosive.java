...
public class Skill_Explosive extends StdSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		String str = null;
		if (success) {
			str = auto ? "<T-NAME> is **BLASTED**!"
					: "^F^<FIGHT^><S-NAME> ** BLAST(S) ** <T-NAMESELF>!^</FIGHT^>^?";
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				int damage = CMLib.dice().roll(1, 90 + mob.phyStats().level(),
						30);
				if (msg.value() > 0)
					damage = damage / 2;
				CMLib.combat().postDamage(mob, target, this, damage,
						CMMsg.MASK_ALWAYS | CMMsg.TYP_CAST_SPELL,
						Weapon.TYPE_BURSTING, "The blast <DAMAGE> <T-NAME>!!!");
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to ** BLAST ** <T-NAMESELF>, but end(s) up looking silly.");
