...
public class Skill_RegionalAwareness extends StdSkill {
	...
	private static final String[] triggerStrings = { "REGION",
			"REGIONALAWARENESS" };
	...
	public char roomColor(Room room) {
		if (room == null)
			return ' ';
		if (CMath.bset(room.phyStats().sensesMask(),
				PhyStats.SENSE_ROOMUNMAPPABLE))
			return 'w';
	...
	public String[] getMiniMap(Room room, final int diameter, boolean openOnly) {
		...
		CMLib.tracking().getRadiantRooms(room, rooms, flags, null, diameter,
				null);
		rmap[diameter / 2][diameter / 2] = room;
		map[diameter / 2][diameter / 2] = '*';
		for (int i = 0; i < rooms.size(); i++) {
			Room R = (Room) rooms.elementAt(i);
			if ((closedPaths.contains(R)) || (R == room))
				continue;
			Room parentR = null;
			int parentDir = -1;
			int[] xy = null;
			...
			if (xy != null) {
				if ((parentDir < 0) || (xy[0] < 0) || (xy[0] >= diameter)
						|| (xy[1] < 0) || (xy[1] >= diameter)
						|| (map[xy[1]][xy[0]] != ' '))
					closedPaths.add(R);
				else {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (auto && (givenTarget instanceof Room) && (asLevel > 0)) {
			final String[] miniMap = getMiniMap((Room) givenTarget, asLevel,
					false);
			if (commands != null) {
		...
		if ((!auto)
				&& ((mob.location().domainType() & Room.INDOORS) == Room.INDOORS)) {
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							CMMsg.MSG_LOOK,
							auto ? ""
									: "<S-NAME> peer(s) at the horizon with a distant expression.");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				final String[] miniMap = getMiniMap(mob.location(),
						2 + (adjustedLevel(mob, asLevel) / 10), true);
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> peer(s) around distantly, looking frustrated.");
		return success;
	}
}
