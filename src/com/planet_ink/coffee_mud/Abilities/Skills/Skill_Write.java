...
public class Skill_Write extends StdSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (((item.material() != RawMaterial.RESOURCE_PAPER)
				...
				&& (item.material() != RawMaterial.RESOURCE_HIDE) && (item
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MSG_WRITE,
					"<S-NAME> write(s) on <T-NAMESELF>.", CMMsg.MSG_WRITE,
					CMParms.combine(commands, 1), CMMsg.MSG_WRITE,
					"<S-NAME> write(s) on <T-NAMESELF>.");
			if (mob.location().okMessage(mob, msg))
				mob.location().send(mob, msg);
		} else
			mob.location()
					.show(mob, target, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> attempt(s) to write on <T-NAMESELF>, but mess(es) up.");
		return success;
	}
