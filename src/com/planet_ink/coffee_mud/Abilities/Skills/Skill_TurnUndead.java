	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((target.baseCharStats().getMyRace() == null)
				|| (!target.baseCharStats().getMyRace().racialCategory()
						.equals("Undead"))) {
			mob.tell(auto ? "Only the undead can be turned."
					: "You can only turn the undead.");
			return false;
		}
		...
		boolean success = proficiencyCheck(mob,
				((mob.phyStats().level() + (4 * getXLEVELLevel(mob))) - target
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "<T-NAME> turn(s) away."
							: "^S<S-NAME> turn(s) <T-NAMESELF>.^?");
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to turn <T-NAMESELF>, but fail(s).");
