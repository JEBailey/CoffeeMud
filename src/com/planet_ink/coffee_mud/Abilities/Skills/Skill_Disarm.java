...
public class Skill_Disarm extends StdSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((success)
				&& ((hisWeapon.fitsOn(Wearable.WORN_WIELD)) || hisWeapon
						.fitsOn(Wearable.WORN_WIELD | Wearable.WORN_HELD))) {
			if (mob.location().show(mob, victim, this, CMMsg.MSG_NOISYMOVEMENT,
					null)) {
				CMMsg msg = CMClass.getMsg(victim, hisWeapon, null,
						CMMsg.MSG_DROP, null);
				if (mob.location().okMessage(mob, msg)) {
					...
					mob.location().show(
							...
							auto ? "<T-NAME> is disarmed!"
									: "<S-NAME> disarm(s) <T-NAMESELF>!");
				}
			}
		} else
			maliciousFizzle(mob, victim,
