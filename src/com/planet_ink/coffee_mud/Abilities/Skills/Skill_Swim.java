...
public class Skill_Swim extends StdSkill {
	...
	public double castingTime(final MOB mob, final List<String> cmds) {
		return CMProps
				.getActionSkillCost(ID(), CMath.greater(CMath.div(
						CMProps.getIntVar(CMProps.Int.DEFABLETIME), 50.0), 1.0));
	}

	public double combatCastingTime(final MOB mob, final List<String> cmds) {
		return CMProps.getCombatActionSkillCost(ID(), CMath.greater(
				CMath.div(CMProps.getIntVar(CMProps.Int.DEFCOMABLETIME), 50.0),
				1.0));
	}
	...
	public boolean placeToSwim(Room r2) {
		if ((r2 == null)
				|| ((r2.domainType() != Room.DOMAIN_OUTDOORS_WATERSURFACE)
						...
						&& (r2.domainType() != Room.DOMAIN_INDOORS_UNDERWATER) && (r2
	...
	public boolean preInvoke(MOB mob, List<String> commands,
		...
		if (secondsElapsed == 0) {
			int dirCode = Directions.getDirectionCode(CMParms.combine(commands,
					0));
			if (dirCode < 0) {
				mob.tell("Swim where?");
				return false;
			...
			if ((mob.riding() != null)
				...
				mob.tell("You need to get off " + mob.riding().name()
						+ " first!");
				return false;
			}
			CMMsg msg = CMClass.getMsg(mob, null, this,
					CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> start(s) swimming "
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((R != null) && (R.okMessage(mob, msg))) {
			...
			if (!success)
				R.show(mob, null, this, CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> struggle(s) against the water, making no progress.");
			else {
			...
			mob.recoverPhyStats();
			if (mob.location() != R)
				mob.location().show(mob, null, this, CMMsg.MSG_NOISYMOVEMENT,
						null);
		}
		return success;
	}
