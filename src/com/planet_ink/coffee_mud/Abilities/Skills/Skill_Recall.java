		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							CMMsg.MSG_LEAVE,
							CMMsg.MSG_RECALL,
							auto ? "<S-NAME> disappear(s) into the Java Plane!"
									: "<S-NAME> recall(s) body and spirit to the Java Plane!");
			CMMsg msg2 = CMClass.getMsg(mob, recallRoom, this, CMMsg.MASK_MOVE
					| CMMsg.TYP_RECALL, CMMsg.MASK_MOVE | CMMsg.MSG_ENTER,
					CMMsg.MASK_MOVE | CMMsg.TYP_RECALL, null);
			if (((recalledRoom.okMessage(mob, msg)) && (recallRoom.okMessage(
					mob, msg2)))
					|| CMSecurity.isAllowed(mob, recalledRoom,
							CMSecurity.SecFlag.GOTO)) {
				if (mob.isInCombat())
				...
				if (recalledRoom.isInhabitant(mob)) {
					if (recallRoom.isInhabitant(mob)
							&& (recallRoom == recalledRoom))
						beneficialWordsFizzle(mob, null,
								"<S-NAME> attempt(s) to recall, but go(es) nowhere.");
					else
				...
				for (int f = 0; f < mob.numFollowers(); f++) {
					...
					if ((follower != null)
						...
						if (((follower.location() == fRecalledRoom))
								...
								&& (fRecalledRoom.okMessage(follower, msg) || CMSecurity
										.isAllowed(mob, recalledRoom,
												CMSecurity.SecFlag.GOTO))) {
							msg2 = CMClass.getMsg(follower, fRecalledRoom,
									this, CMMsg.MASK_MOVE | CMMsg.TYP_RECALL,
									CMMsg.MASK_MOVE | CMMsg.MSG_ENTER,
									CMMsg.MASK_MOVE | CMMsg.TYP_RECALL, null);
			...
			beneficialWordsFizzle(mob, null,
					"<S-NAME> attempt(s) to recall, but <S-HIS-HER> plea goes unheard.");
