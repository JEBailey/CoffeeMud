...
public class Skill_Track extends StdSkill {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (tickID == Tickable.TICKID_MOB) {
			...
			if (nextDirection == 999) {
				...
				mob.tell("The trail seems to continue "
				...
				if ((mob.isMonster()) && (mob.location() != null)) {
					...
					int opDirection = Directions
							.getOpDirectionCode(nextDirection);
					if ((nextRoom != null) && (nextExit != null)) {
						...
						if (nextExit.hasADoor() && (!nextExit.isOpen())) {
							if ((nextExit.hasALock()) && (nextExit.isLocked())) {
								...
								if (oldRoom.okMessage(mob, msg)) {
									...
									CMLib.utensils().roomAffectFully(msg,
											oldRoom, nextDirection);
								}
							}
							CMMsg msg = CMClass.getMsg(mob, nextExit, null,
							...
							if (oldRoom.okMessage(mob, msg)) {
								reclose = true;
								msg = CMClass.getMsg(mob, nextExit, null,
										CMMsg.MSG_OK_VISUAL, CMMsg.MSG_OPEN,
										CMMsg.MSG_OK_VISUAL, "<S-NAME> "
												+ nextExit.openWord()
												+ "(s) <T-NAMESELF>.");
						...
						if (!nextExit.isOpen())
							...
							if (mob.location() == nextRoom) {
								// backup follower mover for handcuffed
								// followers
								final LinkedList<MOB> reMoveV = new LinkedList<MOB>();
								for (Enumeration<Follower> e = mob.followers(); e
										.hasMoreElements();) {
								...
								for (MOB M : reMoveV)
									if (CMLib.flags().isBoundOrHeld(M))
										CMLib.tracking().walk(M, dir, false,
												false);
								if (reclose) {
									Exit opExit = nextRoom
											.getExitInDir(opDirection);
									if ((opExit != null) && (opExit.hasADoor())
											&& (opExit.isOpen())) {
										CMMsg msg = CMClass.getMsg(mob, opExit,
													...
													.getMsg(mob,
															...
															"<S-NAME> "
																	+ nextExit
																			.closeWord()
																	+ "(s) <T-NAMESELF>.");
											CMLib.utensils().roomAffectFully(
													msg, nextRoom, opDirection);
										}
										if ((opExit.hasALock()) && (relock)) {
											msg = CMClass.getMsg(mob, opExit,
													null, CMMsg.MSG_OK_VISUAL,
													CMMsg.MSG_OK_VISUAL,
													CMMsg.MSG_OK_VISUAL, null);
											if (nextRoom.okMessage(mob, msg)) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (V.size() > 0) {
			...
			if ((commands.size() == 0)
					|| (CMParms.combine(commands, 0).equalsIgnoreCase("stop"))) {
				tickStatus = Tickable.STATUS_NOT;
				return true;
			}
		}
		...
		if ((commands.size() > 1)
				&& (((String) commands.lastElement()).toUpperCase()
						.startsWith("RADIUS="))
				&& (CMath.isInteger(((String) commands.lastElement())
						.substring(7)))) {
			radius = CMath
					.s_int(((String) commands.lastElement()).substring(7));
			commands.removeElementAt(commands.size() - 1);
		}
		if ((commands.size() > 1)
				&& (((String) commands.lastElement())
						.equalsIgnoreCase("LANDONLY"))) {
			allowAir = false;
			allowWater = false;
			commands.removeElementAt(commands.size() - 1);
		}
		if ((commands.size() > 1)
				&& (((String) commands.lastElement()).equalsIgnoreCase("NOAIR"))) {
			allowAir = false;
			commands.removeElementAt(commands.size() - 1);
		}
		if ((commands.size() > 1)
				&& (((String) commands.lastElement())
						.equalsIgnoreCase("NOWATER"))) {
			allowWater = false;
			commands.removeElementAt(commands.size() - 1);
		}
		...
		else if ((givenTarget instanceof MOB)
				&& (((MOB) givenTarget).location() != null))
		...
		if (rooms.size() > 0) {
			...
			if ((cacheCode == 1) && (rooms.size() == 1))
				theTrail = cachedPaths.get(CMLib.map().getExtendedRoomID(
						thisRoom)
						+ "->"
						+ CMLib.map().getExtendedRoomID(
								(Room) rooms.firstElement()));
			tickStatus = Tickable.STATUS_MISC6 + 9;
			if (theTrail == null)
				theTrail = CMLib.tracking().findBastardTheBestWay(thisRoom,
						rooms, flags, radius);
			...
			if ((cacheCode == 1) && (rooms.size() == 1) && (theTrail != null))
				cachedPaths.put(
						CMLib.map().getExtendedRoomID(thisRoom)
								+ "->"
								+ CMLib.map().getExtendedRoomID(
										(Room) rooms.firstElement()), theTrail);
		}
		...
		if ((success) && (theTrail != null)) {
			...
			CMMsg msg = CMClass.getMsg(mob, null, this,
					CMMsg.MSG_QUIETMOVEMENT, mob.isMonster() ? null
							: "<S-NAME> begin(s) to track.");
			if (thisRoom.okMessage(mob, msg)) {
				...
				newOne.nextDirection = CMLib.tracking()
						.trackNextDirectionFromHere(theTrail, thisRoom, false);
			}
			tickStatus = Tickable.STATUS_MISC6 + 14;
		} else {
			tickStatus = Tickable.STATUS_NOT;
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to track, but can't find the trail.");
		}
		tickStatus = Tickable.STATUS_NOT;
