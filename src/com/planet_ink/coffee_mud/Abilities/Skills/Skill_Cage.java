	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!auto)
				&& (!(CMSecurity.isAllowed(mob, mob.location(),
						CMSecurity.SecFlag.ORDER) || target
						.willFollowOrdersOf(mob)))) {
			boolean ok = false;
			if ((target.isMonster())
		...
		if ((success) && (caged.cageMe(target))) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if ((cage != null) && (cage.owner() != null)) {
					...
					mob.addItem(caged);
				CMMsg putMsg = CMClass.getMsg(mob, cage, caged, CMMsg.MSG_PUT,
						"<S-NAME> cage(s) <O-NAME> in <T-NAME>.");
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to cage <T-NAME> and fail(s).");
