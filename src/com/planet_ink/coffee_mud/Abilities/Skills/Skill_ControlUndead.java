...
public class Skill_ControlUndead extends StdSkill {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob))
			...
			if ((!invoker().isInCombat())
					&& (msg.source().getVictim() != invoker())) {
				msg.source().tell(
						"You're too submissive towards " + invoker().name());
			...
			if (mob.isMonster()) {
				if ((CMLib.dice().rollPercentage() > 50)
						|| ((mob.getStartRoom() != null)
								&& (mob.getStartRoom().getArea() != mob
										.location().getArea()) && (CMLib
								.flags().isAggressiveTo(mob, null)
								|| (invoker == null) || (!mob.location()
								.isInhabitant(invoker)))))
	...
	public int castingQuality(MOB mob, Physical target) {
		if ((mob != null) && (target != null)) {
			...
			if ((targetM.baseCharStats().getMyRace() == null)
					|| (!targetM.baseCharStats().getMyRace().racialCategory()
							.equals("Undead")))
				return Ability.QUALITY_INDIFFERENT;
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((target.baseCharStats().getMyRace() == null)
				|| (!target.baseCharStats().getMyRace().racialCategory()
						.equals("Undead"))) {
			mob.tell(auto ? "Only the undead can be controlled."
					: "You can only control the undead.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					CMMsg.MSG_CAST_ATTACK_SOMANTIC_SPELL
							| (auto ? CMMsg.MASK_ALWAYS : 0),
					auto ? "<T-NAME> seem(s) controlled."
							: "^S<S-NAME> control(s) <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					if (((mob.phyStats().level() - target.phyStats().level()) > 1)
							|| (target.phyStats().level() == 1)) {
						if (!target.isMonster())
							success = maliciousAffect(mob, target, asLevel, 0,
									CMMsg.MSK_CAST_VERBAL | CMMsg.TYP_MIND
											| CMMsg.MASK_ALWAYS);
						if (success) {
							mob.location().show(target, null,
									CMMsg.MSG_OK_VISUAL,
									"<S-NAME> <S-IS-ARE> now controlled.");
							target.makePeace();
							...
							if (target.amFollowing() != mob)
								mob.tell(target.name(mob)
										+ " seems unwilling to obey you.");
						}
					} else {
						mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> seem(s) submissive!");
						target.makePeace();
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to control <T-NAMESELF>, but fail(s).");
