...
public class Skill_Slip extends StdSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.riding() != null) {
			mob.tell("You can't make someone "
					+ target.riding().stateString(target) + " "
					+ target.riding().name() + " slip!");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> slip(s)!"
									: "^F^<FIGHT^><S-NAME> slip(s) <T-NAMESELF>!^</FIGHT^>^?");
			...
			return maliciousFizzle(
					mob,
					target,
					auto ? "<T-NAMESELF> almost slipped, but maintain(s) <T-HIS-HER> balance."
							: "<S-NAME> attempt(s) to slip <T-NAMESELF>, but fail(s).");
		return success;
	}
}
