...
public class Skill_Convert extends StdSkill {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((text().length() > 0) && (affected instanceof MOB)
				&& (!text().equals(((MOB) affected).getWorshipCharID())))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			...
			if (mob.isMonster())
				CMLib.commands().postSay(mob, null, "I am unable to convert.",
						false, false);
			return false;
		}
		...
		if (D == null) {
			...
			if (target == null) {
				mob.tell("You've also never heard of a deity called '"
			...
			if (D == null) {
				mob.tell("A faithless one cannot convert " + target.name(mob)
						+ ".");
				if (mob.isMonster())
					CMLib.commands().postSay(mob, target,
							"I am faithless, and can not convert you.", false,
							false);
				return false;
			}
		}
		if ((CMLib.flags().isAnimalIntelligence(target))
				|| ((target.isMonster()) && (target.phyStats().level() > mob
						.phyStats().level()))) {
			mob.tell("You can't convert " + target.name(mob) + ".");
			if (mob.isMonster())
				CMLib.commands().postSay(mob, target, "I can not convert you.",
						false, false);
			return false;
		}
		if (target.getMyDeity() == D) {
			...
			if (mob.isMonster())
				CMLib.commands().postSay(mob, target,
						"You already worship " + D.Name() + ".", false, false);
			return false;
		}
		if (!auto) {
			if (convertStack.containsFirst(target)) {
				Long L = convertStack.getSecond(convertStack
						.indexOfFirst(target));
				if ((System.currentTimeMillis() - L.longValue()) > CMProps
			...
			if (convertStack.containsFirst(target)) {
				...
				if (mob.isMonster())
					CMLib.commands().postSay(mob, target,
							"You must wait to be converted again.", false,
							false);
				return false;
			}
		}
		...
		boolean targetMadeSave = CMLib.dice().roll(1, 100, 0) > (target
				.charStats().getSave(CharStats.STAT_FAITH));
		if (CMSecurity.isASysOp(mob))
			targetMadeSave = false;
		if ((!target.isMonster()) && (success) && (targetMadeSave)
				&& (target.getMyDeity() != null)) {
			mob.tell(target.name(mob) + " is worshipping "
					+ target.getMyDeity().name() + ".  "
					+ target.charStats().HeShe() + " must REBUKE "
					+ target.getMyDeity().charStats().himher() + " first.");
			if (mob.isMonster())
				CMLib.commands().postSay(
						mob,
						target,
						"You already worship " + target.getMyDeity().Name()
								+ ".", false, false);
			return false;
		}
		if ((success) && (targetMadeSave) && (!target.isMonster())
				&& (target != mob)) {
			try {
				if (!target
						.session()
						.confirm(
								"\n\r"
										+ mob.name(target)
										+ " is trying to convert you to the worship of "
										+ D.name()
										+ ".  Is this what you want (N/y)?",
								"N")) {
					mob.location().show(
							mob,
							target,
							CMMsg.MSG_SPEAK,
							"<S-YOUPOSS> attempt to convert <T-NAME> to the worship of "
									+ D.name() + " is rejected.");
					return false;
				}
		...
		if ((success) && ((!targetMadeSave) || (target == mob))) {
			...
			if (target.getMyDeity() != null) {
				...
				CMMsg msg2 = CMClass.getMsg(target, D, this, CMMsg.MSG_REBUKE,
						null);
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							this,
							CMMsg.MSG_SPEAK,
							auto ? "<T-NAME> <T-IS-ARE> converted!"
									: "<S-NAME> convert(s) <T-NAMESELF> to the worship of "
											+ D.name() + ".");
			...
			if ((mob.location().okMessage(mob, msg))
				...
				if (dRoom != null)
					dRoom.send(target, msg2);
				convertStack.addElement(target,
						Long.valueOf(System.currentTimeMillis()));
				if (mob != target)
					if (target.isMonster())
						CMLib.leveler().postExperience(mob, null, null, 1,
								false);
					else
						CMLib.leveler().postExperience(mob, null, null, 200,
								false);
				if (target.isMonster()) {
					beneficialAffect(mob, target, asLevel,
							(int) (TimeManager.MILI_HOUR / CMProps
									.getTickMillis()));
