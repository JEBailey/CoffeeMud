...
public class Skill_Chirgury extends StdSkill {
	...
	public static final Object[][] parts = { { "FETUS" }, { "BLOOD" },
	...
	public static final String[] badRaceCats = { "Earth Elemental",
			"Fire Elemental", "Water Elemental", "Air Elemental", "Unique",
			"Slime", "Vegetation", "Metal Golem", "Wood Golem",
			"Electricity Elemental", "Stone Golem", "Stone Golem", "Unknown" };

	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
			boolean auto, int asLevel) {
		if (commands.size() == 0) {
			mob.tell("Remove what from whom? Parts include: "
					+ CMParms.toStringList(parts));
			return false;
		...
		if (partCode < 0) {
			...
			mob.tell("'" + part
					+ "' is not a valid part to remove.  Try one of these: "
		...
		if ((partSet[1] instanceof Integer)
				&& (C != null)
				&& (C.getMyRace().bodyMask()[((Integer) partSet[1]).intValue()] <= 0)) {
		...
		if ((partCode == 0)
				&& ((!(target instanceof MOB)) || (target
						.fetchEffect("Pregnancy") == null))) {
			mob.tell("A baby can not be removed from " + target.name(mob) + ".");
			return false;
		}

		if ((target instanceof MOB)
				&& ((!CMLib.flags().isBoundOrHeld(target)) || (!CMLib.flags()
						.isSleeping(target)))) {
			mob.tell(((MOB) target).charStats().HeShe()
					+ " must be bound, and asleep on an operating bed before you can perform chirurgy.");
			return false;
		}
		...
		if ((oldChirge != null)
				&& (oldChirge.text().indexOf(";" + parts[partCode]) >= 0)) {
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> carefully perform(s) chirurgy upon <T-NAME>.^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				if (partCode > 0) {
					oldChirge.setMiscText(oldChirge.text() + ";"
							+ parts[partCode]);
					Item meat = null;
					int amt = 1;
					if (partCode > 1) {
						...
						((Drink) meat)
								.setLiquidType(RawMaterial.RESOURCE_BLOOD);
						...
						if (target instanceof MOB)
							CMLib.combat().postDamage(mob, (MOB) target, this,
									amt * 3,
									CMMsg.MASK_ALWAYS | CMMsg.TYP_DISEASE, -1,
									"The bleeding <DAMAGE> <T-NAME>!");
					}
					meat.setName("the "
							+ parts[partCode][0].toString().toLowerCase()
							+ " of " + target.Name());
					if ((parts[partCode][0].toString().endsWith("S"))
							&& (!parts[partCode][0].toString()
									.equalsIgnoreCase("PANCREAS")))
						meat.setDisplayText("the "
								+ parts[partCode][0].toString().toLowerCase()
								+ " of " + target.Name() + " lie here.");
					else
						meat.setDisplayText("the "
								+ parts[partCode][0].toString().toLowerCase()
								+ " of " + target.Name() + " lies here.");
					...
					for (int i = 0; i < amt; i++) {
						...
						mob.location().show(
								...
								null,
								CMMsg.MSG_GET,
								(i == 0) ? "<S-NAME> remove(s) <T-NAME> from "
										+ target.name() + "." : null);
					}
				} else {
					...
					if (preg != null) {
						...
						baby.setDisplayText(target.Name()
								+ "'s bloody fetus is lying here.");
						...
						baby.charStats().setStat(
								CharStats.STAT_GENDER,
								(CMLib.dice().rollPercentage() > 50) ? 'F'
										: 'M');
						for (int i : CharStats.CODES.BASE())
							baby.charStats().setStat(i, 1);
						for (int i : CharStats.CODES.MAX())
							baby.charStats().setStat(i, 1);
						baby.charStats().setMyRace(
								((MOB) target).charStats().getMyRace());
						baby.recoverPhyStats();
						baby.setDescription(CMStrings.capitalizeAndLower(baby
								.charStats().hisher())
								+ " body parts can be faintly made out in the twisted and mangled flesh.");
						baby.setMobDescription(baby.description());
						baby.text();
						mob.location().addItem(baby,
								ItemPossessor.Expire.Player_Drop);
						mob.location().show(
								mob,
								baby,
			...
			beneficialWordsFizzle(
					...
					target,
					auto ? ""
							: "<S-NAME> attempt(s) to cut open <T-NAME>, but lose(s) concentration.");
