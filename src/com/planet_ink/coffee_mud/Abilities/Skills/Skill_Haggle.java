...
public class Skill_Haggle extends StdSkill {
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_CHARISMA,
				affectableStats.getStat(CharStats.STAT_CHARISMA) + 10
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((commands.size() < 2)
				|| ((!cmd.equals("BUY") && (!cmd.equals("SELL"))))) {
		...
		Environmental shopkeeper = CMLib.english().parseShopkeeper(mob,
				commands, CMStrings.capitalizeAndLower(cmd) + " what to whom?");
		if (shopkeeper == null)
			return false;
		if (commands.size() == 0) {
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, shopkeeper, this, CMMsg.MSG_SPEAK,
					auto ? "" : "<S-NAME> haggle(s) with <T-NAMESELF>.");
			...
			beneficialWordsFizzle(mob, shopkeeper,
					"<S-NAME> haggle(s) with <T-NAMESELF>, but <S-IS-ARE> unconvincing.");
