...
public class Skill_Dirt extends StdSkill {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (CMLib.flags().isSleeping(target)) {
			CMLib.commands().forceStandardCommand(target, "Wake",
					new XVector("Wake"));
			if (CMLib.flags().isSleeping(target)) {
				if (!auto)
					mob.tell(target.name(mob) + " has "
							+ target.charStats().hisher() + " eyes closed.");
				return false;
			}
		}
		...
		boolean success = proficiencyCheck(mob,
				-(target.charStats().getStat(CharStats.STAT_DEXTERITY) * 3)
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "Dirt flies at <T-NAME>!"
									: "^F^<FIGHT^><S-NAME> kick(s) dirt at <T-NAMESELF>.^</FIGHT^>^?");
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				maliciousAffect(mob, target, asLevel,
						3 + (getXLEVELLevel(mob) / 3), -1);
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to kick dirt at <T-NAMESELF>, but miss(es).");
		return success;
	}
}
