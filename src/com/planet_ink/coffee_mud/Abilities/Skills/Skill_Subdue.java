...
public class Skill_Subdue extends StdSkill {
	...
	public void affectPhyStats(Physical affected, PhyStats affectableStats) {
		...
		affectableStats.setArmor(affectableStats.attackAdjustment() - 10
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (affected instanceof MOB) {
			...
			if (canBeUninvoked()
					&& (M.amDead() || (!CMLib.flags().isInTheGame(M, false))
							|| (!M.amActive()) || M.amDestroyed() || (M
							.getVictim() != whom))) {
				unInvoke();
				return true;
			}
			if ((msg.targetMinor() == CMMsg.TYP_DAMAGE) && (affected != null)
					&& (msg.source() == affected) && (msg.target() == whom)) {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if (affected instanceof MOB) {
			if ((msg.source() == affected) && (msg.target() == whom)
					&& (msg.targetMinor() == CMMsg.TYP_EXAMINE)
					&& (CMLib.flags().canBeSeenBy(whom, msg.source()))) {
				double actualHitPct = CMath.div(whom.curState().getHitPoints()
						- whomDamage, whom.baseState().getHitPoints());
				msg.source().tell(
						msg.source(),
						whom,
						null,
						"<T-NAME> is " + CMath.toPct(actualHitPct)
								+ " health away from being overcome.");
			}
			...
			if ((msg.targetMinor() == CMMsg.TYP_DAMAGE) && (affected != null)
				...
				if (sap != null)
					sap.invoke(
							whom,
							new XVector(new Object[] {
									"SAFELY",
									Integer.toString(adjustedLevel(
											msg.source(), asLevel)) }), whom,
							true, 0);
				whom.makePeace();
	...
	public void unInvoke() {
		if ((canBeUninvoked()) && (affected instanceof MOB))
			((MOB) affected).tell("You are no longer trying to subdue "
					+ whom.name());
		super.unInvoke();
	}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^F^<FIGHT^><S-NAME> attempt(s) to subdue <T-NAMESELF>!^</FIGHT^>^?");
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to subdue <T-NAMESELF>, but fails.");
		return success;
	}
}
