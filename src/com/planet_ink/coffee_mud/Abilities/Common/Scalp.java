...
public class Scalp extends CommonSkill {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((body != null) && (affected instanceof MOB)
			...
			if ((affected != null) && (affected instanceof MOB)) {
				...
				if ((body != null) && (!aborted)) {
					if ((failed) || (!mob.location().isContent(body)))
						commonTell(mob,
								"You messed up your scalping completely.");
					else {
						mob.location().show(mob, null, body,
								getActivityMessageType(),
								"<S-NAME> manage(s) to scalp <O-NAME>.");
						...
						if (body.name().startsWith("the body"))
							scalp.setName("the" + race + " scalp"
									+ body.name().substring(8));
						else
							scalp.setName("a" + race + " scalp");
						if (body.displayText().startsWith("the body"))
							scalp.setDisplayText("the" + race + " scalp"
									+ body.displayText().substring(8));
						else
							scalp.setDisplayText("a" + race
									+ " scalp sits here");
						...
						scalp.setSecretIdentity("This scalp was cut by "
								+ mob.name() + ".");
						mob.location().addItem(scalp,
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		CMMsg msg = CMClass.getMsg(mob, I, this, getActivityMessageType(),
				...
				"<S-NAME> start(s) scalping <T-NAME>.");
