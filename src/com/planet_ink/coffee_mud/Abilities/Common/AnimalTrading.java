...
public class AnimalTrading extends CommonSkill {
	...
	public String name() {
		return "Animal Trading";
	}

	private static final String[] triggerStrings = { "ANIMALTRADING",
			"ANIMALTRADE", "ANIMALSELL", "ASELL" };
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (M != null) {
			...
			if ((CMLib.flags().canMove(M)) && (!CMLib.flags().isBoundOrHeld(M))) {
				commonTell(
						mob,
						M.name(mob)
								+ " doesn't seem willing to cooperate.  You need to bind the animal before you can sell it.");
				return false;
			}
			taming = M;
			...
			if (commands.size() > 0) {
				...
				Environmental E = mob.location().fetchFromMOBRoomFavorsItems(
						mob, null, last, Wearable.FILTER_ANY);
				if ((E != null)
						&& (E instanceof Item)
			...
			taming = mob.location().fetchFromMOBRoomFavorsItems(mob, cage,
		...
		if (proficiencyCheck(mob, 0, auto)) {
			CMMsg msg = CMClass.getMsg(mob, shopkeeper, M, CMMsg.MSG_SELL,
					"<S-NAME> sell(s) <O-NAME> to <T-NAME>.");
			CMMsg msg2 = CMClass.getMsg(mob, M, this, getActivityMessageType(),
					null);
			...
			beneficialWordsFizzle(mob, shopkeeper,
					"<S-NAME> <S-IS-ARE>n't able to strike a deal with <T-NAME>.");
		return true;
	}
}
