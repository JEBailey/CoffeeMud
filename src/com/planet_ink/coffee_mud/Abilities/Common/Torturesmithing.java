...
public class Torturesmithing extends CraftingSkill implements ItemCraftor {
	...
	private static final String[] triggerStrings = { "TORTURESMITH",
			"TORTURESMITHING" };
				...
				if ((buildingI != null) && (!aborted)) {
					if (messedUp) {
						if (activity == CraftingActivity.LEARNING)
							...
							commonTell(mob,
									"You've ruined " + buildingI.name(mob)
											+ "!");
						buildingI.destroy();
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Make what? Enter \"" + triggerStrings()[0].toLowerCase()
							+ " list\" for a list, \""
							+ triggerStrings()[0].toLowerCase()
							+ " learn <item>\" to gain recipes, or \""
							+ triggerStrings()[0].toLowerCase()
							+ " stop\" to cancel.");
			return false;
		}
		...
		if (str.equalsIgnoreCase("list")) {
			...
			StringBuffer buf = new StringBuffer(CMStrings.padRight("Item", 16)
					+ " Lvl Material required\n\r");
		...
		int[][] data = fetchFoundResourceData(mob, woodRequired,
				"wood or cloth", pm, 0, null, null, bundling,
				parsedVars.autoGenerate, null);
		if (data == null)
			return false;
		...
		if (buildingI instanceof Container) {
			...
			if (misctype.equalsIgnoreCase("LID"))
				...
				((Container) buildingI)
						.setContainTypes(getContainerType(misctype));
		}
