...
public class LeatherWorking extends EnhancedCraftingSkill implements
	...
	public String name() {
		return "Leather Working";
	}

	private static final String[] triggerStrings = { "LEATHERWORK",
			"LEATHERWORKING" };
			...
			if (recipes.size() == 0)
				...
				for (int r = 0; r < recipes.size(); r++) {
					...
					if (V.size() > 0) {
						...
						V1.set(RCP_LEVEL, ""
								+ (CMath.s_int(V.get(RCP_LEVEL)) + 9));
						V2.set(RCP_FINALNAME, "Studded " + name);
						V2.set(RCP_LEVEL, ""
								+ (CMath.s_int(V.get(RCP_LEVEL)) + 18));
						pleaseAdd1.add(V1);
						pleaseAdd2.add(V2);
					}
				}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Make what? Enter \"leatherwork list\" for a list, \"leatherwork refit <item>\" to resize, \"leatherwork learn <item>\", \"leatherwork scan\", \"leatherwork mend <item>\", or \"leatherwork stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		if (str.equalsIgnoreCase("list")) {
			...
			if (foundRecipe == null) {
				commonTell(mob, "You don't know how to make a '" + recipeName
						+ "'.  Try \"leatherwork list\" for a list.");
				return false;
			}
			...
			int[][] data = fetchFoundResourceData(mob, woodRequired, "leather",
					pm, (multiplier == 3) ? 1 : 0, (multiplier == 3) ? "metal"
							: null, (multiplier == 3) ? pm1 : null, bundling,
