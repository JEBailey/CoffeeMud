...
public class JewelMaking extends EnhancedCraftingSkill implements ItemCraftor,
				...
				if ((buildingI != null) && (!aborted)) {
					if ((beingDone != null) && (beingDone.size() >= 2)) {
						if (messedUp)
							...
							buildingI.setBaseValue(buildingI.baseGoldValue()
									+ (I.baseGoldValue() * 2));
							buildingI.setDescription(buildingI.description()
									+ " " + (String) beingDone.elementAt(0));
						}
						beingDone = null;
					} else if (messedUp) {
	...
	public boolean mayICraft(final Item I) {
		...
		if ((I.material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_PRECIOUS) {
			...
			return true;
				...
				&& (((I.material() & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_METAL) || ((I
			...
			if ((A.basePhyStats().armor() < 2)
					&& (I.fitsOn(Wearable.WORN_EARS)
							...
							|| I.fitsOn(Wearable.WORN_LEFT_FINGER)
							|| I.fitsOn(Wearable.WORN_RIGHT_FINGER)
							|| I.fitsOn(Wearable.WORN_LEFT_WRIST) || I
								.fitsOn(Wearable.WORN_RIGHT_WRIST)))
				return true;
			return (isANativeItem(I.Name()));
		}
		return (isANativeItem(I.Name()));
	...
	public boolean invoke(final MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Make what? Enter \"jewel list\" for a list.  You may also enter jewel encrust <gem name> <item name>, jewel mount <gem name> <item name>, jewel refit <item name>, jewel learn <item>, jewel scan, jewel mend <item name>, or jewel stop to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		if (str.equalsIgnoreCase("list")) {
			...
			for (int r = 0; r < toggleTop; r++)
				buf.append((r > 0 ? " " : "")
						...
						+ CMStrings.padRight("Metal", cols[2]));
			buf.append("\n\r");
		...
		} else if ((str.equalsIgnoreCase("encrust"))
				|| (str.equalsIgnoreCase("mount"))) {
			String word = str.toLowerCase();
			if (commands.size() < 3) {
				commonTell(mob, CMStrings.capitalizeAndLower(word)
						+ " what jewel onto what item?");
				return false;
			}
			...
			final Environmental jewelE = mob.location()
					.fetchFromMOBRoomFavorsItems(mob, null, jewel,
							Wearable.FILTER_UNWORNONLY);
			final Environmental thangE = mob.location()
					.fetchFromMOBRoomFavorsItems(mob, null, rest,
							Wearable.FILTER_UNWORNONLY);
			...
			if ((!(jewelE instanceof RawMaterial))
					|| (!(jewelE instanceof Item))
					|| (((((Item) jewelE).material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_PRECIOUS) && ((((Item) jewelE)
							.material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_GLASS))) {
				commonTell(mob, "A " + jewelE.name() + " is not suitable to "
						+ word + " on anything.");
				return false;
			}
			final Item jewelI = (Item) CMLib.materials().unbundle(
					(Item) jewelE, 1, null);
			if (jewelI == null) {
				commonTell(mob, jewelE.name() + " is not pure enough to be "
						+ word
						+ "ed with.  You will need to use a gathered one.");
				return false;
			}
			if ((!(thangE instanceof Item))
					...
					|| (((((Item) thangE).material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_CLOTH)
							...
							&& ((((Item) thangE).material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_WOODEN) && ((((Item) thangE)
							.material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_LEATHER))) {
				commonTell(mob, "A " + thangE.name()
						+ " is not suitable to be " + word + "ed on.");
				return false;
			}
			if (!super.invoke(mob, commands, givenTarget, auto, asLevel))
				return false;
			...
			String materialName = RawMaterial.CODES.NAME(jewelI.material())
					.toLowerCase();
			if (word.equals("encrust")) {
				beingDone.addElement(CMStrings.capitalizeAndLower(buildingI
						.name())
						+ " is encrusted with bits of "
						+ materialName
						+ ".");
				startStr = "<S-NAME> start(s) encrusting " + buildingI.name()
						+ " with " + materialName + ".";
				displayText = "You are encrusting " + buildingI.name()
						+ " with " + materialName;
				verb = "encrusting " + buildingI.name() + " with bits of "
						+ materialName;
			} else {
				materialName = CMLib.english().startWithAorAn(materialName)
						.toLowerCase();
				beingDone.addElement(CMStrings.capitalizeAndLower(buildingI
						.name()) + " has " + materialName + " mounted on it.");
				startStr = "<S-NAME> start(s) mounting " + materialName
						+ " onto " + buildingI.name() + ".";
				displayText = "You are mounting " + materialName + " onto "
						+ buildingI.name();
		...
		if (str.equalsIgnoreCase("scan"))
			...
			int[] pm = { RawMaterial.MATERIAL_MITHRIL,
			...
			if ((misctype.equalsIgnoreCase("statue")) && (session != null)
				...
				session.prompt(new InputCallback(InputCallback.Type.PROMPT, "",
					...
					public void showPrompt() {
						session.promptPrint("What is this item a representation of?\n\r: ");
					}
			...
			if ((data[1][FOUND_CODE] > 0)
					&& (((data[0][FOUND_CODE] & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_METAL) || ((data[0][FOUND_CODE] & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_MITHRIL))
			...
			if (data[1][FOUND_CODE] == 0)
				...
				buildingI.setBaseValue(buildingI.baseGoldValue()
						+ RawMaterial.CODES.VALUE(data[1][FOUND_CODE]));
			}
		...
		CMMsg msg = CMClass.getMsg(mob, buildingI, getActivityMessageType(),
				startStr);
		if (mob.location().okMessage(mob, msg)) {
			mob.location().send(mob, msg);
