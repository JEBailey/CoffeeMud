...
public class Weaving extends EnhancedCraftingSkill implements ItemCraftor,
	...
	public String supportedResourceString() {
		return "WHEAT|VINE|SEAWEED|HEMP|SILK|COTTON";
	}
				...
				if ((buildingI != null) && (!aborted)) {
					if (messedUp) {
						if (activity == CraftingActivity.MENDING)
							...
							commonEmote(mob, "<S-NAME> mess(es) up weaving "
									+ buildingI.name() + ".");
					} else {
						if (activity == CraftingActivity.MENDING)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Weave what? Enter \"weave list\" for a list, \"weave refit <item>\" to resize, \"weave learn <item>\", \"weave scan\", \"weave mend <item>\", or \"weave stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		if (str.equalsIgnoreCase("list")) {
			...
			for (int r = 0; r < toggleTop; r++)
				buf.append((r > 0 ? " " : "")
						...
						+ CMStrings.padRight("Material", cols[2]));
			buf.append("\n\r");
			...
			if ((buildingI.material() != RawMaterial.RESOURCE_COTTON)
				...
				commonTell(mob,
						"That's not made of any sort of weavable material.  It can't be refitted.");
				return false;
			}
			...
			if (foundRecipe == null) {
				commonTell(mob, "You don't know how to weave a '" + recipeName
						+ "'.  Try \"weave list\" for a list.");
				return false;
			}
			...
			int[] pm = { RawMaterial.RESOURCE_COTTON,
					RawMaterial.RESOURCE_SILK, RawMaterial.RESOURCE_HEMP,
					RawMaterial.RESOURCE_VINE, RawMaterial.RESOURCE_WHEAT,
			...
			int[][] data = fetchFoundResourceData(mob, woodRequired,
					"weavable material", pm, 0, null, null, false,
