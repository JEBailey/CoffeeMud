...
public class GlassBlowing extends CraftingSkill implements ItemCraftor {
	...
	private static final String[] triggerStrings = { "GLASSBLOW",
			"GLASSBLOWING" };
				...
				if ((buildingI != null) && (!aborted)) {
					if (messedUp) {
						if (activity == CraftingActivity.LEARNING)
							...
							commonTell(
									mob,
									CMStrings.capitalizeAndLower(buildingI
											.name(mob)) + " explodes!");
						buildingI.destroy();
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Make what? Enter \"glassblow list\" for a list, \"glassblow learn <item>\" to gain recipes, or \"glassblow stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		int[] pm = { RawMaterial.RESOURCE_SAND, RawMaterial.RESOURCE_CRYSTAL,
				RawMaterial.RESOURCE_GLASS };
		int[][] data = fetchFoundResourceData(mob, woodRequired, "sand", pm, 0,
