...
public class EnhancedCraftingSkill extends CraftingSkill implements ItemCraftor {
	...
	public String name() {
		return "Enhanced Crafting Skill";
	}
	...
	protected final static String[] STAGE_KEY = { "LITE", "DURA", "QUAL",
			"LTHL", "CNTR" };
	...
	protected int[][] fetchFoundResourceData(MOB mob, int req1Required,
		...
		return super.fetchFoundResourceData(mob, req1Required, req1Desc, req1,
				req2Required, req2Desc, req2, bundle, autoGeneration, expMods);
	}

	public void fixDataForComponents(int[][] data,
			List<Object> componentsFoundList) {
	...
	protected String applyName(String name, String word) {
		...
		int insertHere = 0;
		if ((V.size() > 0) && (CMLib.english().isAnArticle(V.firstElement())))
			insertHere++;
		V.insertElementAt(word.toLowerCase(), insertHere);
		if ((insertHere > 0)
				&& ((V.firstElement().equalsIgnoreCase("A")) || (V
						.firstElement().equalsIgnoreCase("AN")))) {
			V.removeElementAt(0);
			return CMLib.english().startWithAorAn(
					CMParms.combineWithQuotes(V, 0));
		}
	...
	protected List<List<String>> loadList(StringBuffer str) {
		...
		final List<String> parmNames = CMParms.parseTabs(parametersFormat(),
				true);
		int levelParmPos = -1;
		...
		while (lists.size() > 0) {
			...
			for (int index = 0; index < lists.size(); index++)
				if ((lists.get(index).size() > levelParmPos)
						&& (CMath.s_int(lists.get(index).get(levelParmPos)) < lowestLevel)) {
					lowestLevelRecipeIndex = index;
					lowestLevel = CMath.s_int(lists.get(index)
							.get(levelParmPos));
				}
	...
	public void enhanceList(MOB mob) {
				...
				for (int s = stages - 1; s >= 0; s--) {
					if ((X != null) && (X.getValue().intValue() >= (s + 1))) {
						...
						ExpertiseLibrary.ExpertiseDefinition def = CMLib
								.expertises().getDefinition(key + stage);
						if (def == null)
							def = CMLib.expertises().getDefinition(
									key + (s + 1));
						if (def == null)
		...
		if (extras.length() > 0)
			commonTell(
					mob,
					"You can use your expertises to enhance this skill by "
							+ "prepending one or more of the following words to the name "
							+ "of the item you wish to craft: "
							+ extras.substring(0, extras.length() - 2) + ".");
	}
				...
				while (foundSomething) {
					...
					for (int t = 0; t < experTypes.size(); t++) {
						...
						if (code >= 0) {
							...
							for (int s = stages - 1; s >= 0; s--) {
								if ((X == null)
										|| (X.getValue().intValue() < (s + 1)))
									continue;
								stage = CMath.convertToRoman(s + 1);
								ExpertiseLibrary.ExpertiseDefinition def = CMLib
										.expertises()
										.getDefinition(key + stage);
								if (def == null)
									def = CMLib.expertises().getDefinition(
											key + (s + 1));
								if (def == null)
									...
									if (cmd.equalsIgnoreCase(def.data[s])) {
										...
										if (!types.contains(Integer
											...
											if (commands.size() > 0)
												...
												cmd = "";
											foundSomething = true;
											break; // you can do any from a
													// stage, but only 1 per
													// stage!
										}
									}
								}
	...
	public void addStatAdjustment(Item item, String stat, String adjustment) {
		...
		} else if ((CMLib.english().containsString(WA.text().toUpperCase(),
				stat + "+"))
				|| (CMLib.english().containsString(WA.text().toUpperCase(),
						stat + "-")))
			return;
		if (WA != null)
	...
	public void enhanceItem(MOB mob, Item item,
			PairVector<Integer, Integer> types) {
		if (types == null)
			return;
		EnhancedCraftingSkill affect = (EnhancedCraftingSkill) mob
				.fetchEffect(ID());
		if ((affect != null) && (!affect.aborted)
				&& (activity == CraftingActivity.CRAFTING) && (item != null)) {
			for (int t = 0; t < types.size(); t++) {
				int type = types.elementAt(t).first.intValue();
				int stage = types.elementAt(t).second.intValue();
				final String expertiseID = CMLib.expertises()
						.getApplicableExpertise(ID(), type);
				ExpertiseLibrary.ExpertiseDefinition def = CMLib.expertises()
						.getDefinition(expertiseID + CMath.convertToRoman(1));
				if (def == null)
				...
				case TYPE_DURACRAFT: {
					if (!(item instanceof Armor))
						commonTell(mob, def.data[stage]
								+ " only applies to armor.");
					else
						switch (stage) {
						case 0:
							applyName(item, def.data[stage]);
							item.basePhyStats().setArmor(
									item.basePhyStats().armor() + 1);
							item.setBaseValue(atLeast1(item.baseGoldValue(),
									0.1));
							break;
						case 1:
							applyName(item, def.data[stage]);
							item.basePhyStats()
									.setArmor(
											atLeast1(item.basePhyStats()
													.armor(), 0.1) + 1);
							item.setBaseValue(atLeast1(item.baseGoldValue(),
									0.2));
							affect.bumpTickDown(Math
									.round(0.25 * affect.tickDown));
							break;
						case 2:
							applyName(item, def.data[stage]);
							item.basePhyStats()
				...
				case TYPE_CNTRCRAFT: {
					if (!(item instanceof Weapon))
						...
						case 0:
							applyName(item, def.data[stage]);
							item.basePhyStats().setAttackAdjustment(
									item.basePhyStats().attackAdjustment() + 3);
							item.setBaseValue(atLeast1(item.baseGoldValue(),
									0.1));
							affect.bumpTickDown(Math
									.round(0.25 * affect.tickDown));
						...
						case 1:
							applyName(item, def.data[stage]);
							item.basePhyStats().setAttackAdjustment(
									item.basePhyStats().attackAdjustment() + 6);
							item.setBaseValue(atLeast1(item.baseGoldValue(),
									0.2));
							item.basePhyStats()
									.setWeight(
						...
						case 2:
							applyName(item, def.data[stage]);
							item.basePhyStats().setAttackAdjustment(
									item.basePhyStats().attackAdjustment() + 9);
							item.setBaseValue(atLeast1(item.baseGoldValue(),
									0.3));
							item.basePhyStats()
									.setWeight(
