...
public class Dyeing extends CommonSkill {
		...
		if (canBeUninvoked()) {
			if ((affected != null) && (affected instanceof MOB) && (!aborted)
				...
				if (writing.length() == 0)
					...
					found.setName(fixColor(found.Name(), writing.charAt(0),
							prefix + writing));
					found.setDisplayText(fixColor(found.displayText(),
							writing.charAt(0), prefix + writing));
					found.text();
				}
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() < 2) {
			commonTell(mob,
					"You must specify what you want to dye, and color to dye it.");
			return false;
		}
		...
		if ((((target.material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_CLOTH)
				...
				&& ((target.material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_VEGETATION) && ((target
		...
		if ((" white green blue red yellow cyan purple ".indexOf(" "
			...
			commonTell(
					mob,
					"You can't dye anything '"
							+ writing
							+ "'.  Try white, green, blue, red, yellow, cyan, or purple. You can also prefix the colors with the word 'dark', 'light', or 'bright'.");
			return false;
		}
		...
		verb = "dyeing "
				+ target.name()
				+ " "
				+ (darkFlag ? "dark " : brightFlag ? "bright "
						: lightFlag ? "light " : "") + writing;
		...
		CMMsg msg = CMClass.getMsg(mob, null, this, getActivityMessageType(),
				"<S-NAME> start(s) dyeing " + target.name() + ".");
