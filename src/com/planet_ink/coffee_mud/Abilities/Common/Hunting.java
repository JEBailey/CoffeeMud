...
public class Hunting extends CommonSkill {
	...
	public Room nearByRoom() {
		...
		if (possibilities.size() > 0) {
			int dir = ((Integer) possibilities.elementAt(CMLib.dice().roll(1,
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected != null) && (affected instanceof MOB)
			...
			if ((found != null) && (found.amDead())) {
				...
				found.destroy();
				unInvoke();
			} else if ((found != null) && (found.location() != null)
				...
				if (found.location() == mob.location()) {
					if ((mob.isMonster())
							...
							&& (!mob.isInCombat()))
						CMLib.combat().postAttack(mob, found,
			...
			if (tickUp == 0) {
				if (found != null) {
					commonTell(mob, "You have found some " + foundShortName
							+ " tracks!");
					commonTell(mob, "You need to find the " + foundShortName
							+ " nearby before the trail goes cold!");
					...
					StringBuffer str = new StringBuffer(
							"You can't seem to find any game around here.\n\r");
					int d = lookingFor(RawMaterial.MATERIAL_FLESH,
							mob.location());
					if (d < 0)
			...
			} else if (mob.isMonster() && (CMLib.dice().rollPercentage() > 50)
	...
	public void unInvoke() {
		if (canBeUninvoked()) {
			if ((affected != null) && (affected instanceof MOB)) {
				MOB mob = (MOB) affected;
				if ((found != null) && (!found.amDead())
						&& (found.location() != null) && (!found.isInCombat())) {
					if (found.location() == mob.location())
						moveFound();
					...
					mob.location().show(mob, null, getActivityMessageType(),
							"<S-NAME> <S-HAS-HAVE> lost the trail.");
				}
			}
		}
		super.unInvoke();
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((proficiencyCheck(mob, 0, auto))
				...
				&& (((resourceType & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_FLESH)
						...
						|| (resourceType == RawMaterial.RESOURCE_WOOL) || ((resourceType & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_LEATHER))) {
			found = (MOB) CMLib.materials().makeResource(resourceType,
			...
			if (found != null) {
				...
				if ((x = foundShortName.lastIndexOf(' ')) >= 0)
					foundShortName = foundShortName.substring(x).trim()
							.toLowerCase();
				found.setLocation(null);
			}
		}
		int duration = 10 + mob.phyStats().level()
				+ (super.getXTIMELevel(mob) * 2);
		CMMsg msg = CMClass.getMsg(mob, found, this, getActivityMessageType(),
				"<S-NAME> start(s) hunting.");
