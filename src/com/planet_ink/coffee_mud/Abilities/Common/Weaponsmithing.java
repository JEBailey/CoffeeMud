...
public class Weaponsmithing extends EnhancedCraftingSkill implements
	...
	private static final String[] triggerStrings = { "WEAPONSMITH",
			"WEAPONSMITHING" };
	...
	protected boolean canDo(String weaponClass, MOB mob) {
		...
		if (mob.baseCharStats().getCurrentClass().baseClass()
				.equalsIgnoreCase("Commoner"))
			return true;
	...
	protected boolean masterCraftCheck(final Item I) {
		if (I.basePhyStats().level() > 30)
			return false;
		if (I.name().toUpperCase().startsWith("MASTER")
				|| (I.name().toUpperCase().indexOf(" MASTER ") > 0))
			return false;
		return true;
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Make what? Enter \"weaponsmith list\" for a list, \"weaponsmith scan\", \"weaponsmith learn <item>\", \"weaponsmith mend <item>\", or \"weaponsmith stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		if (str.equalsIgnoreCase("list")) {
			...
			StringBuffer buf = new StringBuffer(
					"Weapons <S-NAME> <S-IS-ARE> skilled at making:\n\r");
			int toggler = 1;
			int toggleTop = displayColumns();
			int itemWidth = ListingLibrary.ColFixer.fixColWidth(
					(78 / toggleTop) - 9, mob.session());
			for (int r = 0; r < toggleTop; r++)
				buf.append(CMStrings.padRight("Item", itemWidth) + " Lvl "
			...
			for (int r = 0; r < recipes.size(); r++) {
				...
				if (V.size() > 0) {
					...
					if (((parsedVars.autoGenerate > 0) || (((level <= xlevel(mob)) || (allFlag)) && ((canDo(
							V.get(RCP_WEAPONCLASS), mob)))))
							&& ((mask.length() == 0)
									|| mask.equalsIgnoreCase("all") || CMLib
									.english().containsString(item, mask))) {
						buf.append(CMStrings.padRight(item, itemWidth) + " "
			...
			for (int r = 0; r < matches.size(); r++) {
				...
				if (V.size() > 0) {
					...
					if ((parsedVars.autoGenerate > 0)
							|| ((level <= mob.phyStats().level()) && (canDo(
									V.get(RCP_WEAPONCLASS), mob)))) {
						foundRecipe = V;
						break;
					}
			...
			buildingI
					.setBaseValue((CMath.s_int(foundRecipe.get(RCP_VALUE)) / 4)
			...
			if (buildingI instanceof Weapon) {
				...
				w.setWeaponClassification(specClass(foundRecipe
						.get(RCP_WEAPONCLASS)));
				w.setWeaponType(specType(foundRecipe.get(RCP_WEAPONTYPE)));
				w.setRanges(w.minRange(),
						CMath.s_int(foundRecipe.get(RCP_MAXRANGE)));
			}
			if (CMath.s_int(foundRecipe.get(RCP_HANDS)) == 2)
				buildingI.setRawLogicalAnd(true);
			buildingI.basePhyStats().setAttackAdjustment(
					CMath.s_int(foundRecipe.get(RCP_ATTACK)) + (hardness * 5)
							+ (abilityCode() - 1));
			buildingI.basePhyStats().setDamage(
					CMath.s_int(foundRecipe.get(RCP_ARMORDMG)) + hardness);
