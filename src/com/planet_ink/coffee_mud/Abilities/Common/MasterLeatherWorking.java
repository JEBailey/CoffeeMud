...
public class MasterLeatherWorking extends EnhancedCraftingSkill implements
	...
	private static final String[] triggerStrings = { "MASTERLEATHERWORKING",
			"MLEATHERWORK", "MLEATHERWORKING" };
	...
	public enum Stage {
		Designer(30, 4), Cuirbouli(37, 5), Reinforced(45, 6), Masterwork(54, 7), Laminar(
				63, 8), Battlemoulded(72, 9);
	...
	public boolean mayICraft(final Item I) {
		...
		if (I instanceof Armor) {
			final long noWearLocations = Wearable.WORN_LEFT_FINGER
					| Wearable.WORN_RIGHT_FINGER | Wearable.WORN_EARS;
		...
		if (I instanceof Weapon) {
			Weapon W = (Weapon) I;
			if (((W instanceof AmmunitionWeapon) && ((AmmunitionWeapon) W)
					.requiresAmmunition())
	...
	protected List<List<String>> loadRecipes() {
		...
		List<List<String>> recipes = (List<List<String>>) Resources
						...
						for (Stage s : Stage.values()) {
							...
							for (int i = 0; i <= newRecipes.size(); i++)
								if (newRecipes.size() == i) {
									newRecipes.add(V1);
									break;
								} else if (CMath.s_int(newRecipes.get(i).get(
										RCP_LEVEL)) > level) {
									newRecipes.add(i, V1);
									break;
								}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Make what? Enter \"mleatherwork list\" for a list, \"mleatherwork refit <item>\" to resize, \"mleatherwork learn <item>\", \"mleatherwork scan\", \"mleatherwork mend <item>\", or \"mleatherwork stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		if (str.equalsIgnoreCase("list")) {
			...
			if ((buildingI.material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_LEATHER) {
				commonTell(mob,
						"That's not made of leather.  That can't be refitted.");
				return false;
			}
			...
			int[] pm1 = { RawMaterial.MATERIAL_METAL,
					RawMaterial.MATERIAL_MITHRIL };
			String misctype = foundRecipe.get(RCP_MISCTYPE);
			bundling = misctype.equalsIgnoreCase("BUNDLE");
			int[][] data = fetchFoundResourceData(mob, woodRequired, "leather",
					pm, (multiplier == 6) ? 1 : 0, (multiplier == 6) ? "metal"
							: null, (multiplier == 6) ? pm1 : null, bundling,
					parsedVars.autoGenerate, enhancedTypes);
			if (data == null)
				return false;
			...
			duration = getDuration(
					multiplier * CMath.s_int(foundRecipe.get(RCP_TICKS)), mob,
					30, 4);
			...
			buildingI.basePhyStats().setLevel(
					CMath.s_int(foundRecipe.get(RCP_LEVEL)) + (2 * hardness));
			...
			if (buildingI instanceof Weapon) {
				((Weapon) buildingI).basePhyStats().setAttackAdjustment(
						abilityCode() + (hardness * 5) + (abilityCode() - 1)
								- 1);
				((Weapon) buildingI)
						.setWeaponClassification(Weapon.CLASS_FLAILED);
