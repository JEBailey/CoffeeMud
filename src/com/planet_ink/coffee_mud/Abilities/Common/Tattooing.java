...
public class Tattooing extends CommonSkill {
	...
	public void unInvoke() {
		if (canBeUninvoked()) {
			if ((affected != null) && (affected instanceof MOB) && (!aborted)
				...
				if (writing.length() == 0)
					commonEmote(mob, "<S-NAME> mess(es) up the tattoo on "
							+ target.name() + ".");
				else {
					commonEmote(mob, "<S-NAME> complete(s) the tattoo on "
							+ target.name() + ".");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() < 3) {
			commonTell(
					mob,
					"You must specify whom you want to tattoo, what body part to tattoo, and what the tattoo looks like. Use 'REMOVE' as the description to remove a tattoo.");
			return false;
		}
		...
		long[] tattoable = { Wearable.WORN_ARMS, Wearable.WORN_LEGS,
				Wearable.WORN_HANDS, Wearable.WORN_HEAD, Wearable.WORN_FEET,
				Wearable.WORN_LEFT_WRIST, Wearable.WORN_RIGHT_WRIST,
				Wearable.WORN_NECK, Wearable.WORN_BACK, Wearable.WORN_TORSO };
			...
			for (int ii = 0; ii < tattoable.length; ii++)
				if (codes.get(i) == tattoable[ii]) {
					if (codes.name(i).equalsIgnoreCase(part))
						partNum = i;
					allParts.append(", "
							+ CMStrings.capitalizeAndLower(codes.name(i)
									.toLowerCase()));
					break;
				}
		}
		...
		if (target.freeWearPositions(wornCode, (short) (Short.MIN_VALUE + 1),
				(short) 0) <= 0) {
		...
		for (Enumeration<MOB.Tattoo> e = target.tattoos(); e.hasMoreElements();) {
			...
			if (T.tattooName.startsWith(wornName.toUpperCase() + ":")) {
				numTattsDone++;
				if (T.tattooName.substring(wornName.length() + 1).toUpperCase()
						.startsWith("A TATTOO OF"))
					tatToRemove = T;
			}
		}
		...
		String str = "<S-NAME> start(s) tattooing " + message
				+ " on <T-YOUPOSS> " + wornName.toLowerCase() + ".";
		if ("REMOVE".startsWith(message.toUpperCase()))
			str = "<S-NAME> remove(s) the tattoo on <T-YOUPOSS> "
