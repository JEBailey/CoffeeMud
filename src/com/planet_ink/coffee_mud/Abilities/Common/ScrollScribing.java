...
public class ScrollScribing extends SpellCraftingSkill implements ItemCraftor {
	...
	public String name() {
		return "Scroll Scribing";
	}

	private static final String[] triggerStrings = { "ENSCRIBE",
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected != null) && (affected instanceof MOB)
			...
			if ((buildingI == null) || (theSpell == null)) {
				...
				commonEmote(mob,
						"<S-NAME> start(s) scribing " + buildingI.name() + ".");
	...
	protected Item buildItem(Ability theSpell, int level) {
		...
		buildingI.setDisplayText("a scroll of " + theSpell.name().toLowerCase()
				+ " sits here.");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() < 1) {
			commonTell(
					mob,
					"Enscribe what? Enter \"enscribe list\" for a list, or \"enscribe stop\" to cancel.");
			return false;
		}
		...
		if ((commands.firstElement() instanceof String)
			...
			StringBuffer buf = new StringBuffer(
					"Scrolls you know how to enscribe:\n\r");
			int colWidth = ListingLibrary.ColFixer.fixColWidth(25,
					mob.session());
			buf.append(CMStrings.padRight("Spell", colWidth) + " "
					+ CMStrings.padRight("Spell", colWidth) + " "
					+ CMStrings.padRight("Spell", colWidth));
			int toggler = 1;
			int toggleTop = 3;
			for (int r = 0; r < recipes.size(); r++) {
				...
				if (V.size() > 0) {
					...
					if ((A != null)
						...
						buf.append(CMStrings.padRight(A.name(), colWidth)
								+ ((toggler != toggleTop) ? " " : "\n\r"));
			...
			commonEmote(
					mob,
					"You must specify what magic you wish to enscribe, and the paper to enscribe it in.");
			return false;
		} else {
			...
			if (!CMath.bset(buildingI.material(), RawMaterial.MATERIAL_PAPER)) {
				commonTell(mob, buildingI.name(mob)
						+ " isn't even made of paper!");
				return false;
			}
			if (!(buildingI instanceof Scroll)) {
				commonTell(
						mob,
						"There's can't enscribe magic on "
								+ buildingI.name(mob) + "!");
				return false;
			}
			...
			if (theSpell == null) {
				commonTell(mob, "You don't know how to enscribe '" + recipeName
						+ "'.  Try \"enscribe list\" for a list.");
				return false;
			}
			int experienceToLose = 10;
			experienceToLose += CMLib.ableMapper().qualifyingLevel(mob,
					theSpell) * 10;
			experienceToLose -= CMLib.ableMapper().qualifyingClassLevel(mob,
					theSpell) * 5;
			...
			if (resourceType > 0) {
				int[] pm = { resourceType };
				data = fetchFoundResourceData(mob, 1, ingredient, pm, 0, null,
						null, bundling, -1, null);
				if (data == null)
					return false;
			}
			...
			if ((resourceType > 0) && (data != null))
				CMLib.materials().destroyResourcesValue(mob.location(),
						data[0][FOUND_AMT], data[0][FOUND_CODE], 0, null);
