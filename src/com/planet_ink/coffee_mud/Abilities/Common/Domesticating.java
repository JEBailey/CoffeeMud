...
public class Domesticating extends CommonSkill {
	...
	private static final String[] triggerStrings = { "DOMESTICATE",
			"DOMESTICATING" };
		...
		if (canBeUninvoked()) {
			if ((affected != null) && (affected instanceof MOB)) {
				...
				if ((taming != null) && (!aborted)) {
					if (messedUp)
						commonTell(mob, "You've failed to domesticate "
								+ taming.name() + "!");
					else {
						if (taming.amFollowing() == mob)
							commonTell(mob, taming.name()
									+ " is already domesticated.");
						else {
							...
							if (taming.amFollowing() == mob)
								mob.location().show(
										...
										"<S-NAME> manage(s) to domesticate "
												+ taming.name() + ".");
						}
					}
				}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		String newName = null;
		if ((commands.size() > 2)
				&& (((String) commands.firstElement()).equalsIgnoreCase("name"))) {
		...
		if (!CMLib.flags().isAnimalIntelligence(M)) {
			if (newName != null)
				...
				commonTell(mob, M, null,
						"You don't know how to domesticate <T-NAME>.");
			return false;
		}
			...
			if (oldV.size() > 1) {
				if (oldName.endsWith(", " + ((String) oldV.lastElement())))
					oldName = oldName.substring(0, oldName.length()
							- (2 + ((String) oldV.lastElement()).length()));
				else if (oldName.endsWith("named "
						+ ((String) oldV.lastElement())))
					oldName = oldName.substring(0, oldName.length()
							- (6 + ((String) oldV.lastElement()).length()));
			}
		...
		CMMsg msg = CMClass.getMsg(mob, null, this, getActivityMessageType(),
				(newName != null) ? "<S-NAME> name(s) " + M.name() + " '"
						+ theName + "'." : "<S-NAME> start(s) domesticating "
						+ M.name() + ".");
