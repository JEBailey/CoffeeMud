...
public class Pottery extends CraftingSkill implements ItemCraftor {
	...
	public boolean mayICraft(final Item I) {
		...
		if (!super.mayBeCrafted(I))
			return false;
		if ((I.material() != RawMaterial.RESOURCE_CLAY)
				&& ((I.material() != RawMaterial.RESOURCE_CHINA)))
			return false;
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Make what? Enter \"pot list\" for a list, \"pot learn <item>\" to gain recipes, or \"pot stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		int[][] data = fetchFoundResourceData(mob, woodRequired, "clay", pm, 0,
				null, null, bundling, parsedVars.autoGenerate, null);
		if (data == null)
			return false;
