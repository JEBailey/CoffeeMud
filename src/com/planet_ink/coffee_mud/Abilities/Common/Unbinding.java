...
public class Unbinding extends CommonSkill {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected != null) && (affected instanceof MOB)
			...
			if (tickUp == 3) {
				...
				if (found != null)
					affects = CMLib.flags().flaggedAffects(found,
							Ability.FLAG_BINDING);
				if ((affects != null) && (affects.size() > 0)) {
					removing = affects.get(0);
					displayText = "You are removing " + removing.name()
							+ " from " + found.name();
					verb = "removing " + removing.name() + " from "
							+ found.name();
				} else {
					StringBuffer str = new StringBuffer(
							"You can't seem to remove any of the bindings.\n\r");
				...
				if ((removing != null)
						&& (found.fetchEffect(removing.ID()) != removing)) {
					aborted = true;
					unInvoke();
				}
			}
				...
				if ((found != null) && (removing != null) && (!aborted)) {
					...
					if (found.fetchEffect(removing.ID()) == null)
						...
						mob.location().show(
								...
								null,
								getActivityMessageType(),
								"<S-NAME> fail(s) to remove " + removing.name()
										+ " from " + found.name() + ".");
				}
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		List<Ability> affects = CMLib.flags().flaggedAffects(target,
				Ability.FLAG_BINDING);
		if (affects.size() == 0) {
			mob.tell(target.name(mob)
					+ " does not have any bindings you can remove.");
			return false;
		}
		...
		int duration = CMLib.ableMapper().lowestQualifyingLevel(A.ID())
				- (CMLib.ableMapper().qualifyingLevel(mob, A) + (2 * getXLEVELLevel(mob)));
		if (duration < 5)
			duration = 4;
		CMMsg msg = CMClass.getMsg(mob, target, this,
				CMMsg.MSG_DELICATE_HANDS_ACT,
				"<S-NAME> begin(s) to unbind <T-NAMESELF>.");
