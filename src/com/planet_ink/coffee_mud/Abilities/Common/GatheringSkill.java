...
public class GatheringSkill extends CommonSkill {
		...
		while (mask.length() > 0) {
			...
			if (str.length() > 0) {
				...
				if (!found) {
					...
					int y = str.indexOf('-');
					if (y > 0) {
						List<String> restV = CMParms.parseAny(
								str.substring(y + 1), "-", true);
					...
					RawMaterial.Material m = RawMaterial.Material
							.findIgnoreCase(str);
					if (m != null) {
						List<Integer> rscs = new XVector<Integer>(
								RawMaterial.CODES.COMPOSE_RESOURCES(m.mask()));
						maskV.addAll(rscs);
	...
	public boolean bundle(MOB mob, Vector what) {
		if ((what.size() < 3)
				|| ((!CMath.isNumber((String) what.elementAt(1))) && (!((String) what
						.elementAt(1)).equalsIgnoreCase("ALL")))) {
			commonTell(mob,
					"You must specify an amount to bundle, followed by what resource to bundle.");
			return false;
		}
		...
		for (int i = 0; i < R.numItems(); i++) {
			...
			if (CMLib.english().containsString(I.Name(), name)) {
				...
				if ((I instanceof RawMaterial)
						...
						&& (I.container() == null)
						&& ((I.material() == foundResource) || ((foundResource < 0) && maskV
								.contains(Integer.valueOf(I.material()))))) {
		...
		if ((numHere == 0) || (foundResource < 0)) {
			if (foundAnyway != null)
				commonTell(mob, "You can't bundle " + foundAnyway.name()
						+ " with this skill.");
			else
				commonTell(mob, "You don't see any " + name
						+ " on the ground here.");
			return false;
		}
		if (amount == Integer.MAX_VALUE)
			amount = numHere;
		if (numHere < amount) {
			commonTell(mob, "You only see " + numHere + " pounds of " + name
					+ " on the ground here.");
			return false;
		}
		if (lowestNonZeroFoodNumber == Long.MAX_VALUE)
			lowestNonZeroFoodNumber = 0;
		Item I = (Item) CMLib.materials().makeResource(foundResource,
				Integer.toString(mob.location().domainType()), true, null);
		if (I == null) {
			commonTell(mob, "You could not bundle " + name + " due to "
					+ foundResource
					+ " being an invalid resource code.  Bug it!");
			return false;
		}
		I.setName("a " + amount + "# "
				+ RawMaterial.CODES.NAME(foundResource).toLowerCase()
				+ " bundle");
		I.setDisplayText(I.name() + " is here.");
		I.basePhyStats().setWeight(amount);
		if (R.show(mob, null, I, getActivityMessageType(),
				"<S-NAME> create(s) <O-NAME>.")) {
			int lostValue = CMLib.materials().destroyResourcesValue(R, amount,
					foundResource, -1, I);
		...
		for (Enumeration e = foundAblesH.keys(); e.hasMoreElements();)
			I.addNonUninvokableEffect((Ability) ((Environmental) foundAblesH
					.get(e.nextElement())).copyOf());
		R.recoverRoomStats();
		return true;
	}
