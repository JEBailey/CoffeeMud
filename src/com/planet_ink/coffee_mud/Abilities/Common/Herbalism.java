...
public class Herbalism extends SpellCraftingSkill implements ItemCraftor {
	...
	private static final String[] triggerStrings = { "HERBALISM", "HERBREW",
			"HBREW" };
	...
	public String parametersFormat() {
		return "SPELL_ID\tITEM_LEVEL\t" + "RESOURCE_NAME_OR_HERB_NAME\t"
				+ "RESOURCE_NAME_OR_HERB_NAME\t"
				+ "RESOURCE_NAME_OR_HERB_NAME\t"
				+ "RESOURCE_NAME_OR_HERB_NAME\t"
				+ "RESOURCE_NAME_OR_HERB_NAME\t" + "RESOURCE_NAME_OR_HERB_NAME";
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected != null) && (affected instanceof MOB)
			...
			if ((buildingI == null) || (theSpell == null)) {
				aborted = true;
				unInvoke();
			} else if (tickUp == 0) {
				commonEmote(mob,
						"<S-NAME> start(s) brewing " + buildingI.name() + ".");
		...
		if (I instanceof Potion) {
			...
			for (int i = 0; i < I.numEffects(); i++) {
				...
				if (A instanceof AbilityContainer) {
					boolean chantCheck = false;
					for (Enumeration<Ability> a = ((AbilityContainer) A)
							.abilities(); a.hasMoreElements();)
						switch (a.nextElement().classificationCode()
								& Ability.ALL_ACODES) {
						case Ability.ACODE_CHANT:
							chantCheck = true;
							break;
	...
	protected Item buildItem(Ability theSpell, int level) {
		...
		buildingI.setDisplayText("a potion of " + theSpell.name().toLowerCase()
				+ " sits here.");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() < 1) {
			commonTell(
					mob,
					"Brew what? Enter \"hbrew list\" for a list, \"hbrew learn <item>\" to learn recipes, or \"hbrew stop\" to cancel.");
			return false;
		}
		...
		if ((commands.firstElement() instanceof String)
			...
			buf.append(CMStrings.padRight("Chant", cols[0]) + " "
					+ CMStrings.padRight("Level", cols[1]) + " Ingredients\n\r");
			...
			for (int r = 0; r < recipes.size(); r++) {
				...
				if (V.size() > 0) {
					...
					if ((A != null) && (level >= 0)
						...
						buf.append(CMStrings.padRight(A.name(), cols[0]) + " "
						...
						for (int i = 2; i < V.size(); i++) {
							...
							if (fillUsage) {
								Integer I = (Integer) usage.get(s.toUpperCase()
										.trim());
								if (I == null)
									I = Integer.valueOf(0);
								else
									usage.remove(s.toUpperCase().trim());
								usage.put(s.toUpperCase().trim(),
			...
			/*
			 * for(Enumeration e=usage.keys();e.hasMoreElements();) { String
			 * key=(String)e.nextElement(); Integer I=(Integer)usage.get(key);
			...
			commonEmote(
					mob,
					"You must specify what chant you wish to brew, and the container to brew it in.");
			return false;
		} else {
			buildingI = getTarget(mob, null, givenTarget, CMParms.parse(pos),
			...
			if (!(buildingI instanceof Container)) {
				commonTell(mob, "There's nothing in " + buildingI.name(mob)
						+ " to brew!");
				return false;
			}
			if (!(buildingI instanceof Drink)) {
				commonTell(mob,
						"You can't drink out of a " + buildingI.name(mob) + ".");
				return false;
			}
			if (((Drink) buildingI).liquidRemaining() == 0) {
				commonTell(mob, "The " + buildingI.name(mob)
						+ " contains no liquid base.  Water is probably fine.");
				return false;
			}
			...
			if ((theSpell == null) || (recipe == null)) {
				commonTell(mob, "You don't know how to brew '" + recipeName
						+ "'.  Try \"hbrew list\" for a list.");
				return false;
			}
			int experienceToLose = 10;
			if ((theSpell.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_CHANT) {
				experienceToLose += CMLib.ableMapper().qualifyingLevel(mob,
						theSpell) * 10;
				experienceToLose -= CMLib.ableMapper().qualifyingClassLevel(
						mob, theSpell) * 5;
			}
			...
			for (int i = 2; i < recipe.size(); i++) {
				...
				if (ingredient.length() > 0) {
					...
					for (int v = 0; v < V.size(); v++) {
						Item I = V.get(v);
						if (CMLib.english()
								.containsString(I.Name(), ingredient)
			...
			for (int v = 0; v < V.size(); v++) {
				...
				if (!ok) {
					commonTell(mob,
							"The " + I.name(mob) + " must be removed from the "
									+ buildingI.name(mob) + " before starting.");
					return false;
				}
			}
			...
			CMLib.leveler().postExperience(mob, null, null, -experienceToLose,
					false);
			commonTell(mob, "You lose " + experienceToLose
					+ " experience points for the effort.");
