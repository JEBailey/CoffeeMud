...
public class Blacksmithing extends EnhancedCraftingSkill implements ItemCraftor {
	...
	private static final String[] triggerStrings = { "BLACKSMITH",
			"BLACKSMITHING" };
	...
	public boolean invoke(final MOB mob, Vector commands, Physical givenTarget,
		...
		bundling = false;
		int duration = 4;
		int[] cols = { ListingLibrary.ColFixer.fixColWidth(16, mob.session()),
		...
		int[][] data = fetchFoundResourceData(mob, woodRequired, "metal", pm,
				0, null, null, bundling, parsedVars.autoGenerate, enhancedTypes);
		if (data == null)
			return false;
		...
		if ((misctype.equalsIgnoreCase("statue")) && (session != null)
			...
			session.prompt(new InputCallback(InputCallback.Type.PROMPT, "", 0) {
				...
				public void showPrompt() {
					session.promptPrint("What is a statue this of?\n\r: ");
				}
