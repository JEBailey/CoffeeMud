...
public class Masonry extends CraftingSkill {
	...
	// name, wood, ok=0/roof=1/out=2, req direction=1, ok=0, ok=0, nonull=1,
	// nullonly=2
	private final static String[][] data = { { "Wall", "250", "1", "1", "0" },
			{ "Roof", "500", "2", "0", "0" },
			{ "Archway", "200", "0", "1", "0" },
			{ "Demolish", "0", "0", "1", "0" },
			{ "Title", "0", "0", "0", "0" },
	...
	public Exit generify(Exit X) {
		...
		E2.setDoorsNLocks(X.hasADoor(), X.isOpen(), X.defaultsClosed(),
				X.hasALock(), X.isLocked(), X.defaultsLocked());
		E2.setBasePhyStats((PhyStats) X.basePhyStats().copyOf());
		E2.setExitParams(X.doorName(), X.closeWord(), X.openWord(),
				X.closedText());
	...
	protected void demolishRoom(MOB mob, Room room) {
		...
		for (int d = 0; d < Directions.NUM_DIRECTIONS(); d++) {
			...
			if ((adjacentTitle == null)
					|| (adjacentTitle.getOwnerName().length() > 0))
				backupToRoom1 = R;
		...
		title.setOwnerName("");
		title.updateLot(null); // this is neat -- this will obliterate leaf
								// rooms around this one.
			...
			if ((affected != null) && (affected instanceof MOB) && (!helping)) {
				...
				if (!aborted) {
					if ((messedUp) && (room != null))
						...
						case BUILD_MONUMENT:
							commonTell(mob,
									"You've ruined the druidic monument!");
							break;
						case BUILD_WINDOW:
						...
						case BUILD_POOL: {
							synchronized (("SYNC" + room.roomID()).intern()) {
								...
								if (R.image().equalsIgnoreCase(
										CMLib.protocol().getDefaultMXPImage(
												room)))
									R.setImage(null);
								...
								try {
									...
									for (Enumeration r = CMLib.map().rooms(); r
										...
										if ((rebuild)
												&& (R2 instanceof GridLocale))
											((GridLocale) R2).buildGrid();
									}
								} catch (NoSuchElementException e) {
								}
								try {
									for (Enumeration e = CMLib.players()
											.players(); e.hasMoreElements();) {
								...
								if (doingCode == BUILD_POOL) {
									...
									R2.setRoomID(R.getArea().getNewRoomID(R,
											Directions.DOWN));
									...
									R2.setRawExit(Directions.UP,
											CMClass.getExit("Open"));
									R.clearSky();
									R.rawDoors()[Directions.DOWN] = R2;
									R.setRawExit(Directions.DOWN,
											CMClass.getExit("Open"));
									LandTitle title = CMLib.law().getLandTitle(
											R);
									if ((title != null)
											&& (CMLib.law().getLandTitle(R2) == null)) {
										LandTitle A2 = (LandTitle) title
												.newInstance();
										A2.setPrice(title.getPrice());
										R2.addNonUninvokableEffect((Ability) A2);
									}
									if (CMSecurity
											.isDebugging(CMSecurity.DbgFlag.PROPERTY))
										Log.debugOut("Masonry", R2.roomID()
												+ " created for water.");
						...
						case BUILD_STAIRS: {
							synchronized (("SYNC" + room.roomID()).intern()) {
								...
								while ((upRoom != null)
										&& (upRoom.ID().length() > 0)
										&& (CMLib.law().getLandTitle(upRoom) != null)) {
									upRoom = upRoom.getRoomInDir(Directions.UP);
									floor++;
								}
								upRoom = CMClass.getLocale(CMClass
										.classID(room));
								upRoom.setRoomID(room.getArea().getNewRoomID(
										room, Directions.UP));
								if (upRoom.roomID().length() == 0) {
									commonTell(mob,
											"You've failed to build the stairs!");
									break;
								}
								upRoom.setArea(room.getArea());
								LandTitle newTitle = CMLib.law().getLandTitle(
										room);
								if ((newTitle != null)
										&& (CMLib.law().getLandTitle(upRoom) == null)) {
									newTitle = (LandTitle) ((Ability) newTitle)
											.copyOf();
								...
								Exit upExit = CMClass
										.getExit("OpenDescriptable");
								upExit.setMiscText("Upstairs to the "
										+ (floor + 1)
										+ CMath.numAppendage(floor + 1)
										+ " floor.");
								room.setRawExit(Directions.UP, upExit);

								Exit downExit = CMClass
										.getExit("OpenDescriptable");
								downExit.setMiscText("Downstairs to the "
										+ (floor) + CMath.numAppendage(floor)
										+ " floor.");
								upRoom.rawDoors()[Directions.DOWN] = room;
								upRoom.setRawExit(Directions.DOWN, downExit);
								if (CMSecurity
										.isDebugging(CMSecurity.DbgFlag.PROPERTY))
									Log.debugOut("Lots4Sale", upRoom.roomID()
						...
						case BUILD_PORTCULIS: {
							synchronized (("SYNC" + room.roomID()).intern()) {
								...
								x.setExitParams("portcullis", "lower", "raise",
										"A portcullis blocks your way.");
								x.setDoorsNLocks(true, false, true, false,
										false, false);
								x2.setName("a portcullis");
								x2.setDescription("A portcullis lies this way.");
								x2.setExitParams("portcullis", "lower",
										"raise",
										"A portcullis blocks your way.");
								x2.setDoorsNLocks(true, false, true, false,
										false, false);
								room.setRawExit(dir, x);
								if (room.rawDoors()[dir] != null) {
									room.rawDoors()[dir].setRawExit(
											Directions.getOpDirectionCode(dir),
											x2);
									CMLib.database().DBUpdateExits(
											room.rawDoors()[dir]);
								}
								CMLib.database().DBUpdateExits(room);
							}
						...
						case BUILD_CRAWLWAY: {
							synchronized (("SYNC" + room.roomID()).intern()) {
								room = CMLib.map().getRoom(room);
								if ((workingOn >= 0)
										&& (room.getExitInDir(workingOn) != null)) {
									Exit E = room.getExitInDir(workingOn);
									if ((!E.isGeneric())
											&& (room.getRawExit(workingOn) == E)) {
										E = generify(E);
						...
						case BUILD_WINDOW: {
							synchronized (("SYNC" + room.roomID()).intern()) {
								...
								if ((workingOn >= 0)
									...
									if (R2 != null) {
										Ability A = CMClass
												.getAbility("Prop_RoomView");
										if (A != null) {
											A.setMiscText(CMLib.map()
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (("LIST").startsWith(str.toUpperCase())) {
			...
			int colWidth = ListingLibrary.ColFixer.fixColWidth(20,
					mob.session());
			StringBuffer buf = new StringBuffer(CMStrings.padRight("Item",
					colWidth) + " Stone required\n\r");
			for (int r = 0; r < data.length; r++) {
				if (((r != BUILD_MONUMENT)
						|| (mob.charStats().getCurrentClass().baseClass()
								.equals("Druid")) || CMSecurity.isASysOp(mob))
						&& ((mask == null) || (mask.length() == 0)
								|| mask.equalsIgnoreCase("all") || CMLib
								.english().containsString(data[r][DAT_NAME],
										mask))) {
					int woodRequired = adjustWoodRequired(
							CMath.s_int(data[r][DAT_WOOD]), mob);
					buf.append(CMStrings.padRight(data[r][DAT_NAME], colWidth)
							+ " " + woodRequired);
		...
		if (firstWord.equalsIgnoreCase("help")) {
			...
			helping = true;
			verb = "helping " + targetMOB.name() + " with "
					+ helpingAbility.name();
		...
		for (int r = 0; r < data.length; r++) {
			if ((r != BUILD_MONUMENT)
					|| (mob.charStats().getCurrentClass().baseClass()
							.equals("Druid")) || CMSecurity.isASysOp(mob))
				if (data[r][DAT_NAME].toUpperCase().startsWith(
						firstWord.toUpperCase()))
					doingCode = r;
		}
		if (doingCode < 0) {
			commonTell(mob, "'" + firstWord
					+ "' is not a valid masonry project.  Try LIST.");
			return false;
		}
		...
		if ((doingCode == BUILD_DEMOLISH) && (dirName.equalsIgnoreCase("roof"))
				|| (dirName.equalsIgnoreCase("ceiling"))) {

			Room upRoom = mob.location().getRoomInDir(Directions.UP);
			if (isHomePeerRoom(upRoom)) {
				commonTell(mob,
						"You need to demolish the upstairs rooms first.");
				return false;
			}
			...
			if (CMLib.law().isHomeRoomUpstairs(mob.location())) {
				commonTell(mob,
						"You can't demolish upstairs ceilings.  Try demolishing the room.");
				return false;
			}
			dir = -1;
		} else if ((doingCode == BUILD_DEMOLISH)
				&& (dirName.equalsIgnoreCase("room"))) {
			LandTitle title = CMLib.law().getLandTitle(mob.location());
			if ((!CMLib.law().doesOwnThisProperty(mob, mob.location()))
					&& (title != null) && (title.getOwnerName().length() > 0)) {
			...
			if (!CMLib.law().isHomeRoomUpstairs(mob.location())) {
				commonTell(
						mob,
						"You can only demolish upstairs rooms.  You might try just demolishing the ceiling/roof?");
				return false;
			}
			...
			commonTell(mob,
					"A valid direction in which to build must also be specified.");
			return false;
		}

		if (data[doingCode][DAT_REQNONULL].equals("1") && (dir >= 0)
				&& (mob.location().getExitInDir(dir) == null)) {
			commonTell(mob,
					"There is a wall that way that needs to be demolished first.");
			return false;
		}

		int woodRequired = adjustWoodRequired(
				CMath.s_int(data[doingCode][DAT_WOOD]), mob);
		if (((mob.location().domainType() & Room.INDOORS) == 0)
				&& (data[doingCode][DAT_ROOF].equals("1"))) {
			commonTell(mob,
					"That can only be built after a roof, which includes the frame.");
			return false;
		...
		if (doingCode == BUILD_STAIRS) {
			...
			if ((mob.location().getRoomInDir(Directions.UP) != null)
					|| (mob.location().rawDoors()[Directions.UP] != null)) {
		...
		if (doingCode == BUILD_WALL) {
			...
			if ((nextRoom != null)
					&& (CMLib.law().getLandTitle(nextRoom) == null)) {
				commonTell(mob,
						"You can not build a wall blocking off the main entrance!");
				return false;
			}
		...
		if (doingCode == BUILD_TITLE) {
			...
			for (Iterator<Room> r = checkSet.iterator(); r.hasNext();) {
				...
				if (R.displayText(mob).equalsIgnoreCase(title)) {
					commonTell(mob,
							"That title has already been taken.  Choose another.");
					return false;
				}
			}
			designTitle = title;
		} else if (doingCode == BUILD_DESC) {
			if (commands.size() < 3) {
				commonTell(
						mob,
						"You must specify an exit direction or the word room, followed by a description for it.");
				return false;
			}
			if (Directions.getGoodDirectionCode((String) commands.elementAt(1)) >= 0) {
				dir = Directions.getGoodDirectionCode((String) commands
						.elementAt(1));
				if (mob.location().getExitInDir(dir) == null) {
					commonTell(
							mob,
							"There is no exit "
									+ Directions.getInDirectionName(dir)
									+ " to describe.");
					return false;
				}
				workingOn = dir;
				commands.removeElementAt(1);
			} else if (!((String) commands.elementAt(1))
					.equalsIgnoreCase("room")) {
				commonTell(mob, "'" + ((String) commands.elementAt(1))
						+ "' is neither the word room, nor an exit direction.");
				return false;
			} else
				commands.removeElementAt(1);
		...
		} else if ((doingCode == BUILD_WINDOW) || (doingCode == BUILD_CRAWLWAY))
			workingOn = dir;
		...
		if (doingCode == BUILD_PORTCULIS) {
			...
			idata = fetchFoundResourceData(mob, woodRequired, "metal", pm, 0,
			...
			idata = fetchFoundResourceData(mob, woodRequired, "stone", pm, 0,
					null, null, false, 0, null);
			if (idata == null)
				return false;
			woodRequired = idata[0][FOUND_AMT];
		...
		if (woodRequired > 0)
			CMLib.materials().destroyResourcesValue(mob.location(),
					woodRequired, idata[0][FOUND_CODE], 0, null);
		...
		case BUILD_WALL:
			...
			break;
		case BUILD_ARCH:
			verb = "building the " + Directions.getDirectionName(dir)
					+ " archway";
			break;
		case BUILD_PORTCULIS:
			verb = "building the " + Directions.getDirectionName(dir)
		...
		default:
			if (dir < 0) {
				...
				else if ((mob.location().domainType() == Room.DOMAIN_INDOORS_UNDERWATER)
						|| (mob.location().domainType() == Room.DOMAIN_OUTDOORS_UNDERWATER)) {
					commonTell(mob, null, null,
							"You must demolish a pool from above.");
					return false;
				} else if (mob.location().domainType() != Room.DOMAIN_INDOORS_STONE) {
					commonTell(mob, null, null,
							"There are no stone constructs to demolish here!");
					return false;
				} else if (CMLib.law().isHomeRoomUpstairs(mob.location()))
					verb = "demolishing the room";
				else
					verb = "demolishing the roof";
			} else
				verb = "demolishing the " + Directions.getDirectionName(dir)
						+ " wall";
			break;
		}
		messedUp = !proficiencyCheck(mob, 0, auto);
