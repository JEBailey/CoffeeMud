...
public class SpellCraftingSkill extends CraftingSkill {
	...
	protected String getCraftableSpellName(Vector commands) {
		...
		if ((commands.size() > 0)
			...
			List<String> V = recipes.get(CMLib.dice().roll(1, recipes.size(),
					-1));
	...
	protected List<String> getCraftableSpellRow(String spellName) {
		...
		if (spellFound == null)
			for (List<String> V : recipes)
				if (CMLib.english().containsString(V.get(RCP_FINALNAME),
						spellName)) {
					spellFound = V;
					break;
				}
		if (spellFound == null)
			for (List<String> V : recipes)
				if (V.get(RCP_FINALNAME).toLowerCase()
						.indexOf(spellName.toLowerCase()) >= 0) {
					spellFound = V;
					break;
				}
		return spellFound;
	...
	protected Ability getCraftableSpellRecipeSpell(Vector commands) {
		...
		if (spellName != null) {
			...
			if (theSpell == null) {
				...
				if (spellFound != null)
					theSpell = CMClass
							.getAbility(spellFound.get(RCP_FINALNAME));
			}
		}
		return theSpell;
	}
