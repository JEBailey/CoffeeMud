...
public class Smelting extends CraftingSkill {
	...
	public String parametersFormat() {
		return "ITEM_NAME\tITEM_LEVEL\tBUILD_TIME_TICKS\t\t\t\tRESOURCE_NAME\tRESOURCE_NAME";
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected != null) && (affected instanceof MOB)
			...
			if ((buildingI == null) || (amountMaking < 1)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(mob,
					"Make what? Enter \"smelt list\" for a list, or \"smelt stop\" to cancel.");
			return false;
		}
		...
		if (str.equalsIgnoreCase("list")) {
			...
			StringBuffer buf = new StringBuffer(CMStrings.padRight("Item",
					...
					+ " "
					+ CMStrings.padRight("Metal #1", cols[2]) + " Metal #2\n\r");
				...
				if (V.size() > 0) {
					...
					if (((level <= xlevel(mob)) || allFlag)
						...
						buf.append(CMStrings.padRight(item, cols[0]) + " "
								...
								+ CMStrings.padRight(metal1, cols[2]) + " "
								+ metal2 + "\n\r");
				}
			}
		...
		if (foundRecipe == null) {
			commonTell(mob, "You don't know how to make '" + recipeName
					+ "'.  Try \"smelt list\" for a list.");
			return false;
		}
		...
		int doneResourceCode = RawMaterial.CODES
				.FIND_IgnoreCase(doneResourceDesc);
		if ((resourceCode1 < 0) || (resourceCode2 < 0)
				|| (doneResourceCode < 0)) {
			commonTell(mob,
					"CoffeeMud error in this alloy.  Please let your local Archon know.");
			return false;
		}
		int amountResource1 = CMLib.materials().findNumberOfResource(
				mob.location(), RawMaterial.CODES.GET(resourceCode1));
		int amountResource2 = CMLib.materials().findNumberOfResource(
				mob.location(), RawMaterial.CODES.GET(resourceCode2));
		if (amountResource1 == 0) {
			commonTell(mob, "There is no " + resourceDesc1 + " here to make "
					+ doneResourceDesc
					+ " from.  It might need to be put down first.");
			return false;
		}
		if (amountResource2 == 0) {
			commonTell(mob, "There is no " + resourceDesc2 + " here to make "
					+ doneResourceDesc
					+ " from.  It might need to be put down first.");
			return false;
		}
		...
		CMLib.materials().destroyResourcesValue(mob.location(), amountMaking,
				RawMaterial.CODES.GET(resourceCode1), 0, null);
		CMLib.materials().destroyResourcesValue(mob.location(), amountMaking,
				RawMaterial.CODES.GET(resourceCode2), 0, null);
		duration = getDuration(CMath.s_int(foundRecipe.get(RCP_TICKS)), mob,
				CMath.s_int(foundRecipe.get(RCP_LEVEL)), 6);
		amountMaking += amountMaking;
		buildingI = (Item) CMLib.materials().makeResource(
				RawMaterial.CODES.GET(doneResourceCode), null, false, null);
		startStr = "<S-NAME> start(s) smelting "
				+ doneResourceDesc.toLowerCase() + ".";
