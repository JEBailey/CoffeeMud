...
public class Butchering extends GatheringSkill {
	...
	private static final String[] triggerStrings = { "BUTCHER", "BUTCHERING",
			"SKIN" };
		...
		if (canBeUninvoked()) {
			if ((affected != null) && (affected instanceof MOB)) {
				...
				if ((body != null) && (!aborted)) {
					if (failed)
						commonTell(mob,
								"You messed up your butchering completely.");
					else {
						mob.location()
								.show(mob, null, body,
										getActivityMessageType(),
										"<S-NAME> manage(s) to skin and chop up <O-NAME>.");
						List<RawMaterial> resources = body.charStats()
						...
						for (int i = 0; i < body.numEffects(); i++) {
							...
							if ((A != null) && (A instanceof DiseaseAffect)) {
								if ((CMath.bset(
										((DiseaseAffect) A).spreadBitmap(),
										DiseaseAffect.SPREAD_CONSUMPTION))
										|| (CMath.bset(((DiseaseAffect) A)
												.spreadBitmap(),
						...
						for (int y = 0; y < abilityCode(); y++) {
							for (int i = 0; i < resources.size(); i++) {
								Item newFound = (Item) ((Item) resources.get(i))
										.copyOf();
								if ((newFound instanceof Food)
										|| (newFound instanceof Drink))
									for (int d = 0; d < diseases.size(); d++)
										newFound.addNonUninvokableEffect((Ability) ((Ability) diseases
												.elementAt(d)).copyOf());
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((mob.isMonster() && (!CMLib.flags().isAnimalIntelligence(mob)))
			...
			for (int i = 0; i < mob.location().numItems(); i++) {
				...
				if ((I2 != null) && (I2 instanceof DeadBody)
			...
			I = getTarget(mob, mob.location(), givenTarget, commands,
					Wearable.FILTER_UNWORNONLY);

		if (I == null)
			return false;
		if ((!(I instanceof DeadBody)) || (((DeadBody) I).charStats() == null)
				|| ((DeadBody) I).playerCorpse()
				|| (((DeadBody) I).charStats().getMyRace() == null)) {
			commonTell(mob, "You can't butcher " + I.name(mob) + ".");
			return false;
		}
		List<RawMaterial> resources = ((DeadBody) I).charStats().getMyRace()
				.myResources();
		if ((resources == null) || (resources.size() == 0)) {
			commonTell(
					mob,
					"There doesn't appear to be any good parts on "
							+ I.name(mob) + ".");
			return false;
		}
		if (!super.invoke(mob, commands, givenTarget, auto, asLevel))
		...
		CMMsg msg = CMClass.getMsg(mob, I, this, getActivityMessageType(),
				...
				"<S-NAME> start(s) butchering <T-NAME>.");
