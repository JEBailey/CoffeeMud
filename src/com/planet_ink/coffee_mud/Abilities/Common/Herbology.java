...
public class Herbology extends CommonSkill {
	...
	public void unInvoke() {
		if (canBeUninvoked()) {
			if ((affected != null) && (affected instanceof MOB) && (!aborted)
				...
				if (messedUp)
					commonTell(mob,
							"You lose your concentration on " + found.name()
									+ ".");
				else {
					List<String> herbList = Resources
							.getFileLineVector(Resources.getFileResource(
									"skills/herbology.txt", true));
					String herb = null;
					while ((herbList.size() > 2)
							&& ((herb == null) || (herb.trim().length() == 0)))
						herb = herbList
								.get(CMLib.dice().roll(1, herbList.size(), -1))
								.trim().toLowerCase();
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((target.material() != RawMaterial.RESOURCE_HERBS)
				|| ((!target.Name().toUpperCase().endsWith(" HERBS")) && (!target
						.Name().equalsIgnoreCase("herbs")))
				|| (!(target instanceof RawMaterial)) || (!target.isGeneric())) {
