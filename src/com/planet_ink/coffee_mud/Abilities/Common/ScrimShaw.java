...
public class ScrimShaw extends EnhancedCraftingSkill implements ItemCraftor,
				...
				if ((buildingI != null) && (!aborted)) {
					if (messedUp) {
						if (activity == CraftingActivity.MENDING)
							...
							commonTell(mob,
									"<S-NAME> mess(es) up scrimshawing "
											+ buildingI.name(mob) + ".");
					} else {
						if (activity == CraftingActivity.MENDING)
	...
	public boolean mayICraft(final Item I) {
		...
		if (I instanceof Weapon)
			return !((I instanceof AmmunitionWeapon) && ((AmmunitionWeapon) I)
					.requiresAmmunition());
		...
		if ((I instanceof Light)
				&& ((I.rawProperLocationBitmap() & Wearable.WORN_MOUTH) > 0))
			return true;
	...
	public boolean invoke(final MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Scrim what? Enter \"scrim list\" for a list, \"scrim scan\", \"scrim learn <item>\" to gain recipes, \"scrim mend <item>\", or \"scrim stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		if (str.equalsIgnoreCase("list")) {
			...
			if (foundRecipe == null) {
				commonTell(mob, "You don't know how to scrim a '" + recipeName
						+ "'.  Try \"scrim list\" for a list.");
				return false;
			}
			...
			int[][] data = fetchFoundResourceData(mob, woodRequired, "bone",
			...
			if (buildingI instanceof Weapon) {
				...
				setWeaponTypeClass((Weapon) buildingI, misctype,
						Weapon.TYPE_BASHING, Weapon.TYPE_PIERCING);
