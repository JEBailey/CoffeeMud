...
public class Shipwright extends CraftingSkill implements ItemCraftor,
	...
	private static final String[] triggerStrings = { "SHIPBUILD",
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Shipwright what? Enter \"shipwright list\" for a list, \"shipwright scan\", \"shipwright learn <item>\", \"shipwright mend <item>\", or \"shipwright stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		if (str.equalsIgnoreCase("list")) {
			...
			if (foundRecipe == null) {
				commonTell(mob, "You don't know how to carve a '" + recipeName
						+ "'.  Try \"shipwright list\" for a list.");
				return false;
			}
			...
			int[][] data = fetchFoundResourceData(mob, woodRequired, "wood",
					pm, 0, null, null, false, parsedVars.autoGenerate, null);
			if (data == null)
				return false;
			...
			int lostValue = parsedVars.autoGenerate > 0 ? 0 : CMLib.materials()
					.destroyResourcesValue(mob.location(), woodDestroyed,
			...
			if (buildingI instanceof Rideable) {
				...
				if (CMath.isInteger(capacity))
					((Rideable) buildingI).setRiderCapacity(CMath
							.s_int(capacity));
				((Container) buildingI).setContainTypes(canContain);
				((Container) buildingI).setCapacity(buildingI.basePhyStats()
						.weight() + 250 + (250 * CMath.s_int(capacity)));
