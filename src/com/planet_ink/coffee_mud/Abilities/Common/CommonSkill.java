...
public class CommonSkill extends StdAbility {
	...
	protected static final List<String> uninvokeEmpties = new ReadOnlyList<String>(
	...
	protected ExpertiseLibrary.SkillCostDefinition getRawTrainingCost() {
		return CMProps.getCommonTrainCostFormula(ID());
	}
	...
	protected int getActivityMessageType() {
		return canBeDoneSittingDown() ? CMMsg.MSG_HANDS | CMMsg.MASK_SOUND
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (!super.okMessage(myHost, msg))
			return false;
		if ((myHost instanceof MOB) && (myHost == this.affected)
				&& (((MOB) myHost).location() != null)) {
			if ((msg.sourceMinor() == CMMsg.TYP_SHUTDOWN)
					|| ((msg.sourceMinor() == CMMsg.TYP_QUIT) && (msg
							.amISource((MOB) myHost)))) {
				aborted = true;
				unInvoke();
			}
		}
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected instanceof MOB) && (tickID == Tickable.TICKID_MOB)) {
			...
			String sound = (playSound != null) ? CMLib.protocol().msp(
					playSound, 10) : "";
			if (tickDown == 4)
				mob.location()
						.show(mob,
								...
								"<S-NAME> <S-IS-ARE> almost done " + verb + "."
										+ sound);
			else if ((tickUp % 4) == 0) {
				...
				mob.location().show(
						...
						this,
						getActivityMessageType(),
						"<S-NAME> continue(s) " + verb + " (" + pct
								+ "% completed)." + sound, null,
						"<S-NAME> continue(s) " + verb + "." + sound);
			}
			if ((helping)
					&& (helpingAbility != null)
					&& (helpingAbility.affecting() instanceof MOB)
					&& (((MOB) helpingAbility.affecting())
							.isMine(helpingAbility)))
	...
	public void unInvoke() {
		if (canBeUninvoked()) {
			if ((affected != null) && (affected instanceof MOB)
				...
				MOB mob = (MOB) affected;
				if (aborted)
					mob.location().show(mob, null, getActivityMessageType(),
							"<S-NAME> stop(s) " + verb + ".");
				else
					mob.location().show(mob, null, getActivityMessageType(),
							"<S-NAME> <S-IS-ARE> done " + verb + ".");
				helping = false;
				helpingAbility = null;
			}
		}
	...
	protected int getDuration(int baseTicks, MOB mob, int itemLevel,
			int minDuration) {
		int ticks = baseTicks;
		int level = mob.phyStats().level() - itemLevel;
		double pct = CMath.div(level,
				CMProps.getIntVar(CMProps.Int.LASTPLAYERLEVEL)) * .5;
	...
	protected void commonTell(MOB mob, Environmental target,
			Environmental tool, String str) {
		if (mob.isMonster() && (mob.amFollowing() != null)) {
			if (str.startsWith("You"))
				str = "I" + str.substring(3);
			if (target != null)
	...
	protected void commonEmote(MOB mob, String str) {
		if (mob.isMonster() && (mob.amFollowing() != null))
			mob.location().show(mob, null,
					getActivityMessageType() | CMMsg.MASK_ALWAYS, str);
		else
	...
	protected int lookingFor(Vector materials, Room fromHere) {
		...
		if (possibilities.size() == 0)
			return -1;
		return ((Integer) (possibilities.elementAt(CMLib.dice().roll(1,
				possibilities.size(), -1)))).intValue();
	}
	...
	public Item getRequiredFire(MOB mob, int autoGenerate) {
		if ((autoGenerate > 0)
			...
			fakeFire.basePhyStats().setDisposition(
					fakeFire.basePhyStats().disposition() | PhyStats.IS_GLOWING
	...
	public int[] usageCost(MOB mob, boolean ignoreClassOverride) {
		...
		if (!rebuildCache && (myCache != null)) {
			...
			consumed = 25;
			int diff = CMLib.ableMapper().qualifyingClassLevel(mob, this)
					+ super.getXLOWCOSTLevel(mob)
	...
	public boolean confirmPossibleMaterialLocation(int resource, Room room) {
		...
		if (resources != null)
			for (int i = 0; i < resources.size(); i++)
				if (isMaterial
						&& (resource == (resources.get(i).intValue() & RawMaterial.MATERIAL_MASK)))
					return true;
	...
	public Integer[] supportedResourcesMap() {
		...
		if (resourcesMap.containsKey(rscs)) {
			...
			final List<String> set = CMParms.parseAny(
					supportedResourceString(), "|", true);
			...
			for (int i = 0; i < set.size(); i++) {
				...
				if (setMat.startsWith("_"))
					...
					if (y > 0) {
						restV = CMParms.parseAny(setMat.substring(y + 1), "-",
								true);
						setMat = setMat.substring(0, y);
					}
					RawMaterial.Material m = RawMaterial.Material
							.findIgnoreCase(setMat);
					if (m != null) {
						x = m.mask();
						if ((restV != null) && (restV.size() > 0)) {
							List<Integer> rscsV = new XVector<Integer>(
									RawMaterial.CODES.COMPOSE_RESOURCES(x));
							for (String sv : restV) {
								int code = RawMaterial.CODES
										.FIND_CaseSensitive(sv);
								if (code >= 0)
	...
	public boolean canBeLearnedBy(MOB teacherM, MOB studentM) {
		...
		AbilityMapper.AbilityLimits remainders = CMLib.ableMapper()
				.getCommonSkillRemainder(studentM, this);
		if (remainders.commonSkills <= 0) {
			teacherM.tell(studentM.name(teacherM)
					+ " can not learn any more common skills.");
			studentM.tell("You have learned the maximum " + C.maxCommonSkills()
					+ " common skills, and may not learn any more.");
			return false;
		}
		if (remainders.specificSkillLimit <= 0) {
			teacherM.tell(studentM.name(teacherM) + " can not learn any more "
					+ (crafting ? "" : "non-") + "crafting common skills.");
			final int max = crafting ? C.maxCraftingSkills() : C
					.maxNonCraftingSkills();
			studentM.tell("You have learned the maximum " + max + ""
					+ (crafting ? " " : " non-")
					+ "crafting skills, and may not learn any more.");
			return false;
		}
		return true;
	}
	...
	public void teach(MOB teacher, MOB student) {
		...
		if ((student != null) && (student.fetchAbility(ID()) != null)) {
			...
			boolean crafting = ((classificationCode() & Ability.ALL_DOMAINS) == Ability.DOMAIN_CRAFTINGSKILL);
			AbilityMapper.AbilityLimits remainders = CMLib.ableMapper()
					.getCommonSkillRemainder(student, this);
			if (remainders.commonSkills <= 0)
				student.tell(student.name()
						+ " may not learn any more common skills.");
			else if (remainders.commonSkills <= Integer.MAX_VALUE / 2)
				student.tell(student.name() + " may learn "
						+ remainders.commonSkills + " more common skills.");
			if (remainders.specificSkillLimit <= 0)
				student.tell(student.name() + " may not learn any more "
						+ (crafting ? "" : "non-") + "crafting common skills.");
			else if (remainders.specificSkillLimit <= Integer.MAX_VALUE / 2)
				student.tell(student.name() + " may learn "
						+ remainders.specificSkillLimit + " more "
						+ (crafting ? "" : "non-") + "crafting common skills.");
		}
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!allowedWhileMounted()) && (mob.riding() != null)) {
			commonEmote(mob, "You can't do that while "
					+ mob.riding().stateString(mob) + " " + mob.riding().name()
					+ ".");
			return false;
		}
		...
		for (final Enumeration<Ability> a = mob.personalEffects(); a
			...
			if ((A != null)
					&& (((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_COMMON_SKILL) || (A
							.ID().equalsIgnoreCase("AstroEngineering")))
