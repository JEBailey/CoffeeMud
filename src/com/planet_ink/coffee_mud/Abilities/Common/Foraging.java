...
public class Foraging extends GatheringSkill {
			...
			if (tickUp == 6) {
				if (found != null) {
					...
					StringBuffer str = new StringBuffer(
							"You can't seem to find anything worth foraging around here.\n\r");
					int d = lookingFor(RawMaterial.MATERIAL_VEGETATION,
							mob.location());
					if (d < 0)
			...
			if ((affected != null) && (affected instanceof MOB)) {
				...
				if ((found != null) && (!aborted)) {
					...
					mob.location().show(
							...
							"<S-NAME> manage(s) to gather " + amount + " pound"
									+ s + " of " + foundShortName + ".");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		verb = "foraging";
		found = null;
		if ((!confirmPossibleMaterialLocation(RawMaterial.MATERIAL_VEGETATION,
				mob.location()))
				&& (!confirmPossibleMaterialLocation(RawMaterial.RESOURCE_HEMP,
						mob.location()))
				&& (!confirmPossibleMaterialLocation(RawMaterial.RESOURCE_SILK,
						mob.location()))
				&& (!confirmPossibleMaterialLocation(
						RawMaterial.RESOURCE_COTTON, mob.location()))) {
		...
		if ((proficiencyCheck(mob, 0, auto))
				&& (((resourceType & RawMaterial.MATERIAL_MASK) == RawMaterial.MATERIAL_VEGETATION)
						...
						|| (resourceType == RawMaterial.RESOURCE_SILK) || (resourceType == RawMaterial.RESOURCE_COTTON))) {
		...
		CMMsg msg = CMClass.getMsg(mob, found, this, getActivityMessageType(),
				"<S-NAME> start(s) foraging.");
		if (mob.location().okMessage(mob, msg)) {
			// herb/locale customisation for jeremy
			if ((found != null)
					&& (found.material() == RawMaterial.RESOURCE_HERBS)
					&& ((found.Name().toUpperCase().endsWith(" HERBS")) || (found
							.Name().equalsIgnoreCase("herbs")))) {
				Map<String, List<String>> H = Resources.getCachedMultiLists(
						"skills/herbs.txt", false);
				if (H != null) {
					...
					if ((V != null) && (V.size() > 0)) {
						...
						for (int i = 0; i < V.size(); i++) {
							...
							if ((x >= 0)
								...
								total += CMath.s_int(s.substring(0, x).trim());
								if (choice <= total) {
									found.setSecretIdentity(s.substring(x + 1)
											.trim());
									break;
								}
							} else {
