...
public class InstrumentMaking extends CraftingSkill implements ItemCraftor {
	...
	public String name() {
		return "Instrument Making";
	}

	private static final String[] triggerStrings = { "INSTRUMENTMAKING",
			"INSTRUMENTMAKE" };
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Make what Instrument? Enter \"instrumentmake list\" for a list, \"instrumentmake learn <item>\" to gain recipes, or \"instrumentmake stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		int duration = 4;
		boolean archon = CMSecurity.isASysOp(mob)
		...
		if (str.equalsIgnoreCase("list")) {
			...
			StringBuffer buf = new StringBuffer(CMStrings.padRight("Item",
					...
					+ " "
					+ CMStrings.padRight("Type", cols[2])
					+ " Material required\n\r");
			for (int r = 0; r < recipes.size(); r++) {
				...
				if (V.size() > 0) {
					...
					String itype = CMStrings.capitalizeAndLower(
							V.get(RCP_TYPE).toLowerCase()).trim();
					if (((level <= xlevel(mob)) || allFlag)
							&& ((race.length() == 0) || archon || ((" " + race + " ")
									.toUpperCase().indexOf(
											" "
													+ mob.charStats()
															.getMyRace().ID()
						...
						buf.append(CMStrings.padRight(item, cols[0]) + " "
								...
								+ CMStrings.padRight(itype, cols[2]) + " "
								+ wood + " " + type + "\n\r");
				}
			}
			...
			if (V.size() > 0) {
				...
				if (((parsedVars.autoGenerate > 0) || (level <= xlevel(mob)))
						&& ((parsedVars.autoGenerate > 0)
								|| (race.length() == 0) || archon || ((" "
								+ race + " ").toUpperCase().indexOf(
								" "
		...
		if (foundRecipe == null) {
			commonTell(mob, "You don't know how to make a '" + recipeName
					+ "'.  Try \"instrumentmake list\" for a list.");
			return false;
		}
		...
		int[][] data = fetchFoundResourceData(mob, woodRequired, "material",
				pm, 0, null, null, bundling, parsedVars.autoGenerate, null);
		if (data == null)
			return false;
