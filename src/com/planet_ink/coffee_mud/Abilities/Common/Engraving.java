...
public class Engraving extends CommonSkill {
			...
			if ((affected != null) && (affected instanceof MOB) && (!aborted)
				...
				if (writing.length() == 0)
					...
					found.setDescription(desc
							+ " Engraved on it are the words `" + writing
							+ "`.");
				}
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() < 2) {
			commonTell(
					mob,
					"You must specify what you want to engrave onto, and what words to engrave on it.");
			return false;
		}
		...
		if ((((target.material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_GLASS)
				...
				&& ((target.material() & RawMaterial.MATERIAL_MASK) != RawMaterial.MATERIAL_WOODEN) && ((target
		...
		CMMsg msg = CMClass.getMsg(mob, target, this, getActivityMessageType(),
				"<S-NAME> start(s) engraving on <T-NAME>.");
