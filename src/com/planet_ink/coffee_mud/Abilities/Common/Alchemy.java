		...
		if ((affected != null) && (affected instanceof MOB)
			...
			if ((buildingI == null)
				...
				if ((theSpell.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_PRAYER) {
					commonEmote(mob, "<S-NAME> start(s) praying for "
							+ buildingI.name() + ".");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (parsedVars.autoGenerate > 0) {
			...
			if (parsedVars.forceLevels) {
				int minLevel = CMLib.ableMapper().lowestQualifyingLevel(
						theSpell.ID());
		...
		if (commands.size() < 1) {
			commonTell(mob,
					"Brew what? Enter \"brew list\" for a list, or \"brew stop\" to cancel.");
			return false;
		}
		int[] cols = { ListingLibrary.ColFixer.fixColWidth(25, mob.session()), };
		...
		if ((commands.firstElement() instanceof String)
			...
			buf.append(CMStrings.padRight("Spell", cols[0]) + " "
					+ CMStrings.padRight("Spell", cols[0]) + " "
					+ CMStrings.padRight("Spell", cols[0]));
			int toggler = 1;
			int toggleTop = 3;
				...
				if (V.size() > 0) {
					...
					if ((A != null)
							...
							&& ((mask.length() == 0)
									...
									.english().containsString(spell, mask))) {
			...
			commonEmote(
					mob,
					"You must specify what magic you wish to brew, and the container to brew it in.");
			return false;
		} else {
			...
			for (int r = 0; r < recipes.size(); r++) {
				...
				if (V.size() > 0) {
					String spell = V.get(0);
					Ability A = mob.fetchAbility(spell);
					if ((A != null) && (xlevel(mob) >= spellLevel(mob, A))
			...
			if (resourceType > 0) {
				if (((Drink) buildingI).liquidType() == resourceType) {
					found = true;
					if (V.size() > 0) {
						commonTell(mob, "The extraneous stuff from the "
								+ buildingI.name(mob)
				...
				if (!found) {
					commonTell(mob,
							"This potion requires " + ingredient
									+ ".  Please place some inside the "
									+ buildingI.name(mob) + " and try again.");
					return false;
				}
			}
