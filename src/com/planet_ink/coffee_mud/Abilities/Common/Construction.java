...
public class Construction extends CraftingSkill {
	...
	// nullonly=2
	private final static String[][] data = { { "Wall", "100", "1", "1", "0" },
			{ "Door", "125", "1", "1", "0" }, { "Roof", "350", "2", "0", "0" },
			{ "Gate", "50", "2", "1", "0" }, { "Fence", "50", "2", "1", "0" },
			{ "Demolish", "0", "0", "1", "0" },
			{ "Title", "0", "0", "0", "0" },
			{ "Description", "0", "0", "0", "0" },
					...
					if ((messedUp) && (room != null))
						...
						case BUILD_ROOF: {
							synchronized (("SYNC" + room.roomID()).intern()) {
								...
								if (CMSecurity
									...
									Log.debugOut("Construction", R.roomID()
											+ " updated.");
								CMLib.database().DBUpdateRoom(R);
								CMLib.database().DBUpdateExits(R);
							}
						...
						case BUILD_SECRETDOOR: {
							synchronized (("SYNC" + room.roomID()).intern()) {
								...
								X.setExitParams("door", "close", "open",
										"a closed door");
								...
								if (room.rawDoors()[dir] != null) {
									...
									if (doingCode == BUILD_SECRETDOOR)
										X2.basePhyStats().setDisposition(
												PhyStats.IS_HIDDEN);
									X2.recoverPhyStats();
									X2.text();
						...
						case BUILD_GATE: {
							synchronized (("SYNC" + room.roomID()).intern()) {
								...
								X.setDisplayText("");
								X.setOpenDelayTicks(9999);
								X.setExitParams("gate", "close", "open",
										"a closed gate");
								X.setDoorsNLocks(true, false, true, false,
										false, false);
								X.text();
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (("LIST").startsWith(str.toUpperCase())) {
			...
			StringBuffer buf = new StringBuffer(CMStrings.padRight("Item",
					colWidth) + " Wood required\n\r");
			for (int r = 0; r < data.length; r++) {
				if (((r != BUILD_SECRETDOOR) || (mob.charStats()
						.getCurrentClass().baseClass().equals("Thief")))
						&& ((mask == null) || (mask.length() == 0)
								|| mask.equalsIgnoreCase("all") || CMLib
								.english().containsString(
										CMStrings.padRight(data[r][DAT_NAME],
												colWidth), mask))) {
		...
		if (str.equalsIgnoreCase("help")) {
			...
			if (helpingAbility == null) {
				commonTell(mob, targetMOB.Name()
						+ " is not constructing anything.");
				return false;
			}
		...
		for (int r = 0; r < data.length; r++) {
			if ((data[r][0].toUpperCase().startsWith(firstWord.toUpperCase()))
					&& ((r != BUILD_SECRETDOOR) || (mob.charStats()
							.getCurrentClass().baseClass().equals("Thief"))))
				doingCode = r;
		}
		if (doingCode < 0) {
			commonTell(mob, "'" + firstWord
					+ "' is not a valid construction project.  Try LIST.");
			return false;
		}
		...
		if (doingCode == BUILD_TITLE) {
			...
			if (commands.size() < 3) {
				commonTell(mob,
						"You must specify an exit direction or room, followed by a description for it.");
				return false;
			}
		...
		int[][] idata = fetchFoundResourceData(mob, woodRequired, "wood", pm,
		...
		case BUILD_DOOR:
			...
			break;
		case BUILD_SECRETDOOR:
			verb = "building a hidden " + Directions.getDirectionName(dir)
					+ " door";
			break;
		case BUILD_WINDOW:
		...
		default:
			if (dir < 0) {
				if ((mob.location().domainType() == Room.DOMAIN_INDOORS_WATERSURFACE)
					...
					return false;
				} else if (mob.location().domainType() != Room.DOMAIN_INDOORS_WOOD) {
					commonTell(mob, null, null,
							"There are no wooden constructs to demolish here!");
					return false;
