...
public class Cooking extends CraftingSkill implements ItemCraftor {
	...
	public String parametersFormat() {
		return "ITEM_NAME\tFOOD_DRINK||RESOURCE_NAME\tSMELL_LIST||CODED_SPELL_LIST\tITEM_LEVEL\t"
	...
	public boolean isMineForCooking(MOB mob, Container cooking) {
		for (int a = 0; a < mob.numEffects(); a++) {
			...
			if ((A instanceof Cooking) && (((Cooking) A).cookingPot == cooking)
					&& (A != this))
				return false;
		}
		...
		if ((mob.location() == cooking.owner())
				&& ((CMLib.flags().isOnFire(cooking)) || (!requireFire()) || ((cooking
						.container() != null) && (CMLib.flags()
						.isOnFire(cooking.container())))))
			return true;
		return false;
	}
	...
	public boolean meetsLidRequirements(MOB mob, Container cooking) {
		...
		if ((cooking.hasALid()) && (!cooking.isOpen()))
			return true;
		if ((cooking.container() != null) && (cooking.container().hasALid())
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected != null) && (affected instanceof MOB)
			...
			if ((cookingPot == null) || (buildingI == null)
					|| (finalRecipe == null) || (finalAmount <= 0)
				...
				commonTell(mob, "You start " + cookWord() + " up some "
						+ finalDishName + ".");
	...
	public void unInvoke() {
		if (canBeUninvoked()) {
			if ((affected != null) && (affected instanceof MOB)) {
				if ((cookingPot != null) && (finalRecipe != null)
						&& (buildingI != null)) {
					...
					if (!aborted)
						for (int i = 0; i < finalAmount * (abilityCode()); i++) {
							...
							if (cookingPot.owner() instanceof Room)
								((Room) cookingPot.owner()).addItem(food,
	...
	public Hashtable<String, Integer> potContents(Container pot) {
		...
		if ((pot instanceof Drink) && (((Drink) pot).liquidRemaining() > 0)) {
			if (pot instanceof RawMaterial)
				h.put(RawMaterial.CODES.NAME(((RawMaterial) pot).material())
						+ "/",
						Integer.valueOf(((Drink) pot).liquidRemaining() / 10));
			else
				h.put(RawMaterial.CODES.NAME(((Drink) pot).liquidType()) + "/",
						Integer.valueOf(((Drink) pot).liquidRemaining() / 10));
		}
		if (pot.owner() == null)
			return h;
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (I instanceof RawMaterial) {
				...
				if (CMParms.indexOf(RawMaterial.CODES.FISHES(), I.material()) >= 0)
					ing += "/FISH";
				else if (CMParms.indexOf(RawMaterial.CODES.BERRIES(),
						I.material()) >= 0)
					ing += "/BERRIES";
			} else if ((((I.material() & RawMaterial.MATERIAL_VEGETATION) > 0)
					|| ((I.material() & RawMaterial.MATERIAL_LIQUID) > 0) || ((I
					.material() & RawMaterial.MATERIAL_FLESH) > 0))
					&& (CMParms.parse(I.name()).size() > 0))
				ing = CMParms.parse(I.name()).lastElement().toUpperCase();
			else
				ing = I.name();
			Integer INT = h.get(ing + "/" + I.secretIdentity().toUpperCase()
					+ "/" + I.Name().toUpperCase() + "/");
			if (INT == null)
				INT = Integer.valueOf(0);
			INT = Integer.valueOf(INT.intValue() + 1);
			h.put(ing + "/" + I.secretIdentity().toUpperCase() + "/"
					+ I.Name().toUpperCase() + "/", INT);
		}
		return h;
	}
	...
	public Vector countIngredients(List Vr) {
		...
		for (Enumeration e = oldPotContents.keys(); e.hasMoreElements();) {
			...
			amounts[numIngredients] = ((Integer) oldPotContents
					.get(contents[numIngredients])).intValue();
			numIngredients++;
		}
			...
			for (int vr = RCP_MAININGR; vr < Vr.size(); vr += 2) {
				...
				if (ingredient.length() > 0) {
					...
					for (int i = 0; i < contents.length; i++) {
						...
						if ((index == 0)
								|| ((index > 0) && (!Character
										.isLetter(ingredient2.charAt(index - 1))))) {
		...
		if (NotEnoughForThisRun) {
			...
			for (int i = 0; i < contents.length; i++) {
				...
				int amount2 = amounts[i];
				if ((amount2 > 0)
						&& ((!honorHerbs()) || (!ingredient2.toUpperCase()
								.startsWith("HERBS/")))
	...
	public Vector extraIngredientsInOldContents(List<String> Vr,
			boolean perfectOnly) {
		...
		for (Enumeration e = oldPotContents.keys(); e.hasMoreElements();) {
			...
			if (honorHerbs() && ingredient.toUpperCase().startsWith("HERBS/")) // herbs
																				// exception
				found = true;
			else
				for (int vr = RCP_MAININGR; vr < Vr.size(); vr += 2) {
					String ingredient2 = Vr.get(vr).toUpperCase();
					int index = ingredient.toUpperCase().indexOf(
							ingredient2 + "/");
					if ((ingredient2.length() > 0)
							&& (((!perfectOnly) && ((index == 0) || ((index > 0) && (!Character
									.isLetter(ingredient.charAt(index - 1)))))) || ((perfectOnly) && ingredient
									.toUpperCase()
									.equalsIgnoreCase(ingredient2))))
						found = true;
				}
			if (!found) {
	...
	public Vector missingIngredientsFromOldContents(List<String> Vr,
			boolean perfectOnly) {
			...
			if (ingredient.length() > 0) {
				...
				for (Enumeration e = oldPotContents.keys(); e.hasMoreElements();) {
					String ingredient2 = ((String) e.nextElement())
							.toUpperCase();
					int index = ingredient2.toUpperCase().indexOf(
							ingredient.toUpperCase() + "/");
					if ((((index == 0) || ((index > 0) && (!Character
							.isLetter(ingredient2.charAt(index - 1))))))
							|| ((!perfectOnly) && ingredient2
									.equalsIgnoreCase(ingredient.toUpperCase()))) {
						found = true;
						break;
					}
				}
	...
	public Item buildItem(MOB mob, List<String> finalRecipe, List<Item> contents) {
			...
			for (int v = 0; v < contents.size(); v++) {
				...
				if ((I instanceof RawMaterial)
						&& (RawMaterial.CODES.NAME(I.material())
								.equalsIgnoreCase(finalRecipe.get(RCP_MAININGR)))) {
					if ((((RawMaterial) I).domainSource() != null)
							&& (!CMath.isNumber(((RawMaterial) I)
									.domainSource()))
							&& (CMClass.getRace(((RawMaterial) I)
									.domainSource())) != null)
						replaceName = CMClass
								.getRace(((RawMaterial) I).domainSource())
								.name().toLowerCase();
					else {
						String name = I.Name();
		...
		if (foodType.equalsIgnoreCase("FOOD")) {
			...
			buildingI.setDisplayText("some " + ((messedUp) ? "burnt " : "")
					+ finalDishName + " is here");
			buildingI.setDescription("It looks "
					+ ((messedUp) ? "burnt!" : "good!"));
			...
			if (!messedUp) {
				...
				if (contents != null)
					for (int v = 0; v < contents.size(); v++) {
						...
						if ((I.material() == RawMaterial.RESOURCE_HERBS)
							...
							food.setNourishment(food.nourishment()
									+ (((Food) I).nourishment() + ((Food) I)
											.nourishment()));
						else
				...
				if (food.nourishment() > 300)
					food.setBite((int) Math.round(Math.ceil(CMath.div(
							food.nourishment(), 2))));
			}
			int material = -1;
			if (contents != null)
				for (int v = 0; v < contents.size(); v++) {
					...
					if ((I.material() != RawMaterial.RESOURCE_HERBS)
						...
						food.basePhyStats()
								.setWeight(
										food.basePhyStats().weight()
												+ ((I.basePhyStats().weight()) / finalAmount));
					if (I instanceof Food)
						switch (((Food) I).material()
			...
			if (material < 0) {
				String materialFoodName = replacePercent(
						finalRecipe.get(RCP_FINALFOOD), "").trim()
						.toUpperCase();
			...
			food.setMaterial(material < 0 ? RawMaterial.RESOURCE_BEEF
					: material);
			if (mob != null)
				food.setNourishment((food.nourishment() + homeCookValue(mob, 10))
						/ finalAmount);
			if (!messedUp)
				CMLib.materials().addEffectsToResource(food);
			food.basePhyStats().setWeight(
					food.basePhyStats().weight() / finalAmount);
			...
			buildingI.setDisplayText("some " + ((messedUp) ? "spoiled " : "")
					+ finalDishName + " is here.");
			buildingI.setDescription("It looks "
					+ ((messedUp) ? "spoiled!" : "good!"));
			...
			if (contents != null)
				for (int v = 0; v < contents.size(); v++) {
					...
					drink.basePhyStats()
							.setWeight(
									drink.basePhyStats().weight()
											+ ((I.basePhyStats().weight()) / finalAmount));
					if (I instanceof Food)
						drink.setLiquidRemaining(drink.liquidRemaining()
								+ ((Food) I).nourishment());
					if ((I instanceof Drink)
			...
			drink.basePhyStats().setWeight(
					drink.basePhyStats().weight() / finalAmount);
			if (messedUp)
				drink.setThirstQuenched(1);
			...
			final String ruinWord = (buildingI instanceof Drink) ? "spoiled "
					: "burnt ";
			buildingI.setName(((messedUp) ? ruinWord : "") + finalDishName);
			buildingI.setDisplayText("some " + ((messedUp) ? ruinWord : "")
					+ finalDishName + " is here");
			...
			buildingI.basePhyStats().setWeight(
					buildingI.basePhyStats().weight() / finalAmount);
			if (!messedUp)
		...
		if (buildingI != null) {
			if (mob != null)
				buildingI.setSecretIdentity("This was prepared by "
						+ mob.Name() + ".");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (parsedVars.autoGenerate > 0) {
			...
			if ((commands.size() > 0)
				...
				List<String> V = recipes.get(CMLib.dice().roll(1,
						recipes.size(), -1));
			...
			if (finalRecipe == null)
				for (int r = 0; r < allRecipes.size(); r++) {
					...
					if (Vr.size() > 0) {
						String item = Vr.get(RCP_FINALFOOD);
						if (replacePercent(item, "").toLowerCase().indexOf(
								recipeName.toLowerCase()) >= 0) {
							finalRecipe = Vr;
							break;
						}
					}
			...
			if (parsedVars.forceLevels) {
				buildingI.basePhyStats().setLevel(
						CMath.s_int(finalRecipe.get(RCP_LEVEL)));
		...
		if ((commands.size() > 0)
			...
			StringBuffer buf = new StringBuffer(CMStrings.padRight("^xRecipe",
					colWidth) + "^.^? ^B^~wIngredients required^N\n\r");
			for (int r = 0; r < allRecipes.size(); r++) {
				List<String> Vr = allRecipes.get(r);
				if (Vr.size() > 0) {
					String item = Vr.get(RCP_FINALFOOD);
					...
					if (((level <= xlevel(mob)) || allFlag)
						...
						line.append("^c"
								+ CMStrings.padRight(CMStrings
										.capitalizeAndLower(replacePercent(
												item, "")), colWidth) + "^w ");
						for (int vr = RCP_MAININGR; vr < Vr.size(); vr += 2) {
							String ingredient = Vr.get(vr);
							if (ingredient.length() > 0) {
								...
								if (ingredient.equalsIgnoreCase("water"))
									amount = amount * 10;
								String next = ingredient.toLowerCase() + "("
										+ amount + ") ";
		...
		Item possibleContainer = possibleContainer(mob, commands, true,
				Wearable.FILTER_UNWORNONLY);
		Item target = getTarget(mob, mob.location(), givenTarget,
				possibleContainer, commands, Wearable.FILTER_UNWORNONLY);
		if (target == null) {
			commonTell(mob, "The syntax for this skill is "
					+ triggerStrings()[0] + " [CONTAINER]");
			return false;
		}

		if (!(target instanceof Container)) {
			commonTell(mob, "There's nothing in " + target.name(mob) + " to "
					+ cookWordShort() + "!");
			return false;
		}
		for (int a = 0; a < mob.numEffects(); a++) {
		...
		if (!meetsLidRequirements(mob, (Container) target)) {
			commonTell(
					mob,
					"You need a closeable container to bake that in, and you need to close it to begin.");
			return false;
		}
		...
		case RawMaterial.MATERIAL_PRECIOUS:
			break;
		default:
			commonTell(mob, target.name(mob) + " is not suitable to "
					+ cookWordShort() + " in.");
			return false;
		}
		...
		for (int v = 0; v < allRecipes.size(); v++) {
			...
			for (Enumeration e = oldPotContents.keys(); e.hasMoreElements();) {
				...
				int index = ingredient2.indexOf(Vr.get(RCP_MAININGR)
						.toUpperCase() + "/");
				if ((index == 0)
						|| ((index > 0) && (!Character.isLetter(ingredient2
								.charAt(index - 1))))) {
					found = true;
					break;
				}
		...
		if (perfectRecipes.size() == 0) {
			if (closeRecipes.size() == 0) {
				commonTell(
						mob,
						"You don't know how to make anything out of those ingredients.  Have you tried LIST as a parameter?");
				return false;
			}
			for (int vr = 0; vr < closeRecipes.size(); vr++) {
				...
				String recipeName = replacePercent(
						(String) Vr.get(RCP_FINALFOOD),
						((String) Vr.get(RCP_MAININGR)).toLowerCase());
				if (extra.size() > 0) {
					StringBuffer buf = new StringBuffer(
							"If you are trying to make " + recipeName
									+ ", you need to remove ");
					for (int i = 0; i < extra.size(); i++)
						if (i == 0)
							buf.append(((String) extra.elementAt(i))
									.toLowerCase());
						else if (i == extra.size() - 1)
							buf.append(", and "
									+ ((String) extra.elementAt(i))
											.toLowerCase());
						else
							buf.append(", "
									+ ((String) extra.elementAt(i))
					...
					for (int i = 0; i < missing.size(); i++)
						if (i == 0)
							buf.append(((String) missing.elementAt(i))
									.toLowerCase());
						else if (i == missing.size() - 1)
							buf.append(", and "
									+ ((String) missing.elementAt(i))
											.toLowerCase());
						else
							buf.append(", "
									+ ((String) missing.elementAt(i))
		...
		for (int vr = 0; vr < perfectRecipes.size(); vr++) {
			...
			String recipeName = replacePercent((String) Vr.get(RCP_FINALFOOD),
					((String) Vr.get(RCP_MAININGR)).toLowerCase());
			if (counts.size() == 1) {
				if (CMath.s_int((String) Vr.get(RCP_LEVEL)) > xlevel(mob))
					complaints.addElement("If you are trying to make "
							+ recipeName
							+ ", you need to wait until you are level "
							+ CMath.s_int((String) Vr.get(RCP_LEVEL)) + ".");
				else {
					finalRecipe = Vr;
				...
				for (int i = 1; i < counts.size(); i++)
					if (i == 1)
						...
						buf.append(", and "
								+ ((String) counts.elementAt(i)).toLowerCase());
					else
						buf.append(", "
								+ ((String) counts.elementAt(i)).toLowerCase());
				...
				StringBuffer buf = new StringBuffer(
						"If you are trying to make " + recipeName
								+ ", you need to remove some of the ");
				for (int i = 1; i < counts.size(); i++)
					if (i == 1)
						buf.append(((String) counts.elementAt(i)).toLowerCase());
					else if (i == counts.size() - 1)
		...
		duration = getDuration(
				mob,
				CMath.isInteger((String) finalRecipe.get(RCP_LEVEL)) ? CMath
						.s_int((String) finalRecipe.get(RCP_LEVEL)) : 1);
		// ***********************************************
		...
		CMMsg msg = CMClass.getMsg(mob, cookingPot, this,
				getActivityMessageType(), getActivityMessageType(),
				getActivityMessageType(), "<S-NAME> start(s) " + cookWord()
						+ " something in <T-NAME>.");
