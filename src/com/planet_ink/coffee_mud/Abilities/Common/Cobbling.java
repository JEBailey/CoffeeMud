...
public class Cobbling extends EnhancedCraftingSkill implements ItemCraftor,
				...
				if ((buildingI != null) && (!aborted)) {
					if (messedUp) {
						if (activity == CraftingActivity.MENDING)
							...
							commonEmote(mob, "<S-NAME> mess(es) up cobbling "
									+ buildingI.name() + ".");
					} else {
						if (activity == CraftingActivity.MENDING)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			commonTell(
					mob,
					"Make what? Enter \"cobble list\" for a list, \"cobble refit <item>\" to resize, \"cobble learn <item>\", \"cobble scan\", or \"cobble mend <item>\", \"cobble stop\" to cancel.");
			return false;
		}
		if ((!auto)
				&& (commands.size() > 0)
		...
		if (str.equalsIgnoreCase("list")) {
			...
			int[] pm = { RawMaterial.MATERIAL_METAL,
					RawMaterial.MATERIAL_MITHRIL, RawMaterial.MATERIAL_CLOTH,
