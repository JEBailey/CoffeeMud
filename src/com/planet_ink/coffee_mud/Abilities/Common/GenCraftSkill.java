...
public class GenCraftSkill extends EnhancedCraftingSkill implements ItemCraftor {
	...
	public String getStat(String code) {
		/*
		 * int num=0; int numDex=code.length();
		 * while((numDex>0)&&(Character.isDigit(code.charAt(numDex-1))))
		 * numDex--; if(numDex<code.length()) {
	...
	public void setStat(String code, String val) {
		...
		case 3:
			...
			break;
		case 4:
			SV(ID, V_TRIG, CMParms.parseCommas(val, true)
		...
		default:
			if (code.equalsIgnoreCase("allxml")
					&& ID.equalsIgnoreCase("GenCraftSkill"))
				parseAllXML(val);
			break;
		}
	}
				...
				if ((buildingI != null) && (!aborted)) {
					if (messedUp) {
						if (activity == CraftingActivity.MENDING)
							...
							commonEmote(mob, "<S-NAME> mess(es) up "
									+ ((String) V(ID, V_VERB)) + " "
									+ buildingI.name() + ".");
					} else {
						if (activity == CraftingActivity.MENDING)
	...
	public boolean mayICraft(final MOB mob, final Item I) {
		...
		for (final List<String> recipe : recipes) {
			...
			finalName = CMStrings.replaceAll(finalName, "%", ".*")
					.toLowerCase();
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() == 0) {
			StringBuilder features = new StringBuilder(noun + " what? Enter \""
					+ noun.toLowerCase() + " list\" for a list");
			if (canMendB.booleanValue())
				features.append(", \"" + noun.toLowerCase()
						+ " mend <item>\" to mend broken items, \""
						+ noun.toLowerCase()
						+ " scan\" to scan for mendable items");
			if (canRefitB.booleanValue())
				features.append(", \"" + noun.toLowerCase()
						+ " refit <item>\" to resize wearables");
			if (canBundleB.booleanValue())
				features.append(", \"" + noun.toLowerCase()
						+ " bundle\" to make bundles");
			features.append(", or \"" + noun.toLowerCase()
					+ " stop\" to cancel.");
		...
		if (str.equalsIgnoreCase("list")) {
			...
			StringBuffer buf = new StringBuffer(
					"Item <S-NAME> <S-IS-ARE> skilled at " + verbing + ":\n\r");
			int toggler = 1;
			int toggleTop = 2;
			int[] cols = {
			...
			for (int r = 0; r < toggleTop; r++)
				buf.append((r > 0 ? " " : "")
						...
						+ CMStrings.padRight("Mats", cols[2]));
			buf.append("\n\r");
			for (int r = 0; r < recipes.size(); r++) {
				...
				if (V.size() > 0) {
					...
					String mats = getComponentDescription(mob, V,
							RCP_AMOUNTMATS);
					if (mats.length() > 5) {
						if (toggler > 1)
							buf.append("\n\r");
		...
		if (str.equalsIgnoreCase("scan") && canMendB.booleanValue())
			...
			if (buildingI == null)
				return false;
			if ((!this.mayICraft(mob, buildingI))
					&& (!super.isMadeOfSupportedResource(buildingI))) {
			...
			if (foundRecipe == null) {
				commonTell(
						mob,
						"You don't know how to " + noun.toLowerCase() + " a '"
								+ recipeName + "'.  Try \""
								+ noun.toLowerCase() + " list\" for a list.");
				return false;
			}
			...
			if (componentsFoundList == null)
				return false;
			int numRequired = CMath.isInteger(requiredMats) ? CMath
					.s_int(requiredMats) : 0;
			...
			int[][] data = fetchFoundResourceData(mob, numRequired, "material",
			...
			int lostValue = parsedVars.autoGenerate > 0 ? 0 : CMLib.materials()
					.destroyResourcesValue(mob.location(), numRequired,
			...
			buildingI.basePhyStats().setWeight(
					getStandardWeight(numRequired, bundling));
