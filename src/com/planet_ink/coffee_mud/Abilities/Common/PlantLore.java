...
public class PlantLore extends CommonSkill {
			...
			if (tickUp == 6) {
				if (success == false) {
					StringBuffer str = new StringBuffer(
							"Your growth observation attempt failed.\n\r");
			...
			if ((affected != null) && (affected instanceof MOB) && (!helping)) {
				...
				if ((success) && (!aborted) && (room != null)) {
					if ((room.domainType() & Room.INDOORS) == 0) {
						...
						CMLib.tracking().getRadiantRooms(room, V, flags, null,
								2 + (getXLEVELLevel(mob) / 2), null);
						for (int v = 0; v < V.size(); v++) {
							Room R = (Room) V.elementAt(v);
							int material = R.myResource()
									& RawMaterial.MATERIAL_MASK;
							int resource = R.myResource()
							...
							if ((material != RawMaterial.MATERIAL_VEGETATION)
								...
								continue;
							String resourceStr = RawMaterial.CODES
									.NAME(resource);
							if (R == room)
								str.append("You think this spot would be good for "
										+ resourceStr.toLowerCase() + ".\n\r");
							else {
								int isAdjacent = -1;
								...
								if (isAdjacent >= 0)
									str.append("There looks like "
											+ resourceStr.toLowerCase()
											+ " "
											+ Directions
													.getInDirectionName(isAdjacent)
											+ ".\n\r");
								else {
									int d = CMLib.tracking().radiatesFromDir(R,
											V);
									if (d >= 0) {
						...
						commonTell(mob,
								"You don't find any good plant life around here.");
				}
			}
		}
		super.unInvoke();
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		CMMsg msg = CMClass.getMsg(mob, null, this, getActivityMessageType(),
				"<S-NAME> start(s) observing the growth in this area.");
