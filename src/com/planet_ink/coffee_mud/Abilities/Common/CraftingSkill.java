...
public class CraftingSkill extends GatheringSkill {
	...
	protected enum CraftingActivity {
		CRAFTING, MENDING, LEARNING, REFITTING
	}
	...
	protected static class CraftParms {
		...
		public CraftParms(int autoGenerate, Physical givenTarget,
				boolean forceLevels) {
	...
	public int getStandardWeight(int baseWoodRequired, boolean bundling) {
		int newWeight = (int) Math.round(baseWoodRequired
				* this.getItemWeightMultiplier(bundling));
	...
	protected String replacePercent(String thisStr, String withThis) {
		if (withThis.length() == 0) {
			...
			if (x >= 0)
				return new StringBuffer(thisStr).replace(x, x + 2, withThis)
						.toString();
			x = thisStr.indexOf(" %");
			if (x >= 0)
				return new StringBuffer(thisStr).replace(x, x + 2, withThis)
			...
			x = thisStr.indexOf('%');
			if (x >= 0)
				return new StringBuffer(thisStr).replace(x, x + 1, withThis)
						.toString();
		} else {
			int x = thisStr.indexOf('%');
			if (x >= 0)
	...
	protected CraftParms parseAutoGenerate(boolean auto, Physical givenTarget,
			Vector commands) {
		if ((auto) && (commands.size() > 0)
			...
			if ((commands.size() > 0)
					&& (commands.firstElement() instanceof Boolean)) {
				preserveLevels = (Boolean) commands.firstElement();
				commands.removeElementAt(0);
			}
			return new CraftParms(autoGenerate.intValue(), null,
					preserveLevels.booleanValue());
		}
	...
	protected long getContainerType(final String s) {
		...
		for (final String splitS : allTypes) {
			int bit = CMParms.indexOf(Container.CONTAIN_DESCS, splitS
					.toUpperCase().trim());
			if (bit > 0)
				return 0;
			else
			...
			if ((I instanceof Recipe)
				...
				for (int v = 0; v < V.size(); v++) {
					...
					if (recipes.size() == 0)
						...
						if ((recipes.size() == 0)
								|| lastRecipeV.size() <= V2.size())
							recipes.add(V2);
						else {
							Log.errOut(ID(),
									"Not enough parms (" + lastRecipeV.size()
											+ "<=" + V2.size() + "): "
											+ CMParms.combine(V2));
	...
	protected int adjustWoodRequired(int woodRequired, MOB mob) {
		int newWoodRequired = woodRequired
				- (int) Math.round((0.05 * woodRequired * getXPCOSTLevel(mob)));
	...
	protected void dropAWinner(MOB mob, Item building) {
		...
		if (R == null)
			...
			if (!foundIt) {
				commonTell(mob,
						"You have won the common-skill-failure LOTTERY! Congratulations!");
	...
	protected void setWearLocation(Item I, String wearLocation,
		...
		if (I instanceof Armor) {
			...
			CMLib.ableParms().parseWearLocation(layerAtt, layers, wornLoc,
					logicalAnd, hardBonus, wearLocation);
			if (I instanceof Armor) {
				Armor armor = (Armor) I;
	...
	protected int[][] fetchFoundResourceData(MOB mob, int req1Required,
			String req1Desc, int[] req1, int req2Required, String req2Desc,
			int[] req2, boolean bundle, int autoGeneration,
		...
		if (req1 != null) {
			for (int i = 0; i < req1.length; i++) {
				if ((req1[i] & RawMaterial.RESOURCE_MASK) == 0)
					firstWood = CMLib.materials().findMostOfMaterial(
							mob.location(), req1[i]);
				else
					firstWood = CMLib.materials().findFirstResource(
							mob.location(), req1[i]);
			...
			firstWood = CMLib.materials().fetchFoundOtherEncoded(
					mob.location(), req1Desc);
		data[0][FOUND_AMT] = 0;
		if (firstWood != null) {
			data[0][FOUND_AMT] = CMLib.materials().findNumberOfResource(
					mob.location(), firstWood.material());
		...
		if (req2 != null) {
			for (int i = 0; i < req2.length; i++) {
				if ((req2[i] & RawMaterial.RESOURCE_MASK) == 0)
					firstOther = CMLib.materials().findMostOfMaterial(
							mob.location(), req2[i]);
				else
					firstOther = CMLib.materials().findFirstResource(
							mob.location(), req2[i]);
				if (firstOther != null)
					break;
			}
		} else if (req2Desc != null)
			firstOther = CMLib.materials().fetchFoundOtherEncoded(
					mob.location(), req2Desc);
		data[1][FOUND_AMT] = 0;
		if (firstOther != null) {
			data[1][FOUND_AMT] = CMLib.materials().findNumberOfResource(
					mob.location(), firstOther.material());
			...
			if (data[0][FOUND_AMT] == 0) {
				if (req1Desc != null)
					commonTell(
							mob,
							"There is no "
									+ req1Desc.toLowerCase()
									+ " here to make anything from!  It might need to be put down first.");
				return null;
			}
			if (!bundle)
				req1Required = fixResourceRequirement(data[0][FOUND_CODE],
						req1Required);
		}
		if (req2Required > 0) {
			if (req2Desc != null)
				if (((req2 != null) && (data[1][FOUND_AMT] == 0))
					...
					if (req2Desc.equalsIgnoreCase("PRECIOUS"))
						commonTell(
								mob,
								"You need some sort of precious stones to make that.  There is not enough here.  Are you sure you set it all on the ground first?");
					else
						commonTell(
								mob,
								"You need some "
										+ req2Desc.toLowerCase()
										+ " to make that.  There is not enough here.  Are you sure you set it all on the ground first?");
					return null;
				}
			if (!bundle)
				req2Required = fixResourceRequirement(data[1][FOUND_CODE],
						req2Required);
		}
		...
		if ((req2Required > 0) && (req2Required > data[1][FOUND_AMT])) {
			commonTell(
					...
					"You need "
							+ req2Required
							+ " pounds of "
							+ RawMaterial.CODES.NAME(data[1][FOUND_CODE])
									.toLowerCase()
							+ " to make that.  There is not enough here.  Are you sure you set it all on the ground first?");
			return null;
		}
	...
	protected void randomRecipeFix(MOB mob, List<List<String>> recipes,
			Vector commands, int autoGeneration) {
		if (((mob.isMonster() && (!CMLib.flags().isAnimalIntelligence(mob))) || (autoGeneration > 0))
			...
			while ((++tries) < maxtries) {
				List<String> randomRecipe = recipes.get(CMLib.dice().roll(1,
						recipes.size(), -1));
				...
				if ((randomRecipe.size() > 1)) {
					...
					if ((levelIndex > 0)
							&& (xlevel(mob) < CMath.s_int(randomRecipe
									.get(levelIndex))))
						proceed = false;
				}
	...
	public ItemKeyPair craftItem(String recipeName, int material,
		...
		mob.basePhyStats().setSensesMask(
				mob.basePhyStats().sensesMask() | PhyStats.CAN_SEE_DARK);
		mob.recoverPhyStats();
		if (material < 0) {
			List<Integer> rscs = myResources();
			if (rscs.size() == 0)
				rscs = new XVector(Integer.valueOf(RawMaterial.RESOURCE_WOOD));
			material = rscs.get(CMLib.dice().roll(1, rscs.size(), -1))
					.intValue();
		}
		while (((building == null) || (building.name().endsWith(" bundle")))
			...
			if ((V.size() > 0) && (V.lastElement() instanceof Item)) {
				if ((V.size() > 1)
						&& ((V.elementAt(V.size() - 2) instanceof DoorKey)))
	...
	public ItemKeyPair craftItem(String recipeName) {
		...
		if (rscs.size() == 0)
			rscs = new XVector(Integer.valueOf(RawMaterial.RESOURCE_WOOD));
		int material = rscs.get(CMLib.dice().roll(1, rscs.size(), -1))
				.intValue();
	...
	public List<List<String>> matchingRecipeNames(String recipeName,
			boolean beLoose) {
		return matchingRecipeNames(fetchRecipes(), recipeName, beLoose);
	}

	protected List<List<String>> matchingRecipeNames(
			List<List<String>> recipes, String recipeName, boolean beLoose) {
		...
		for (int r = 0; r < recipes.size(); r++) {
			...
			if (V.size() > 0) {
				String item = V.get(RCP_FINALNAME);
				if ((replacePercent(item, "").toUpperCase().indexOf(
						recipeName.toUpperCase()) >= 0))
					matches.add(V);
			}
		}
		if (matches.size() > 0)
		...
		if (beLoose) {
			for (int r = 0; r < recipes.size(); r++) {
				...
				if (V.size() > 0) {
					String item = V.get(RCP_FINALNAME);
					if ((recipeName.toUpperCase().indexOf(
							replacePercent(item, "").toUpperCase()) >= 0))
						matches.add(V);
				}
			}
			if (matches.size() > 0)
			...
			if (lastWord.length() > 1)
				for (int r = 0; r < recipes.size(); r++) {
					...
					if (V.size() > 0) {
						String item = V.get(RCP_FINALNAME);
						if ((replacePercent(item, "").toUpperCase().indexOf(
								lastWord.toUpperCase()) >= 0)
								|| (lastWord.toUpperCase().indexOf(
										replacePercent(item, "").toUpperCase()) >= 0))
							matches.add(V);
					}
				}
	...
	protected Vector getAllMendable(MOB mob, Environmental from, Item contained) {
		...
		if (from instanceof Room) {
			...
			for (int i = 0; i < R.numItems(); i++) {
				Item I = R.getItem(i);
				if ((I != null) && (I.container() == contained)
						&& (canMend(mob, I, true))
	...
	public boolean publicScan(MOB mob, Vector commands) {
		...
		if (rest.length() == 0)
			...
			if ((scanning == null)
					|| (!CMLib.flags().canBeSeenBy(scanning, mob))) {
				commonTell(mob, "You don't see anyone called '" + rest
						+ "' here.");
				return false;
			}
		}
		...
		if (allStuff.size() == 0) {
			if (mob == scanning)
				commonTell(mob,
						"You don't seem to have anything that needs mending with "
								+ name() + ".");
			else
				commonTell(mob, "You don't see anything on " + scanning.name()
						+ " that needs mending with " + name() + ".");
			return false;
		}
		...
		if (scanning == mob)
			...
			buf.append("The following items on " + scanning.name()
					+ " could use some " + name() + ":\n\r");
		for (int i = 0; i < allStuff.size(); i++) {
			Item I = (Item) allStuff.elementAt(i);
			buf.append(CMStrings.padRight(I.usesRemaining() + "%", 5)
					+ I.name());
	...
	protected int getPercentChanceToDeconstruct(final MOB crafterM, final Item I) {
		return (int) Math.round(((1.0 + crafterM.phyStats().level() - I
				.phyStats().level()) / crafterM.phyStats().level() / 2.0 + 0.5)
	...
	protected boolean deconstructRecipeInto(final Item I, final Recipe R) {
		...
		if (!C.mayICraft(I))
			return false;
		List<String> existingRecipes = new XVector<String>(
				R.getRecipeCodeLines());
	...
	protected boolean mayBeCrafted(final Item I) {
		...
		if ((!CMLib.flags().isDroppable(I))
				...
				|| (CMath.bset(I.phyStats().sensesMask(),
		...
		if (I.numScripts() > 0)
			return false;
		if (CMLib.flags()
				.flaggedBehaviors(I, Behavior.FLAG_POTENTIALLYAUTODEATHING)
				.size() > 0)
			return false;
	...
	public boolean isANativeItem(String name) {
		...
		if (nameV.size() == 0)
			return false;
		TreeSet<String> allExpertiseWords = (TreeSet<String>) Resources
				.getResource("CRAFTING_SKILL_EXPERTISE_WORDS");
		if (allExpertiseWords == null) {
			if (this instanceof EnhancedCraftingSkill) {
				List<ExpertiseLibrary.ExpertiseDefinition> V = ((EnhancedCraftingSkill) this)
				...
				Resources.submitResource("CRAFTING_SKILL_EXPERTISE_WORDS",
						allExpertiseWords);
			} else
				allExpertiseWords = new TreeSet<String>();
		}
		for (List<String> recipe : recipes) {
			...
			List<String> thisOneNameV = CMParms.parse(thisOnesName
					.toUpperCase());
			boolean match = false;
			for (int n = 0, o = 0;; n++) {
				...
				if (CMLib.english().isAnArticle(nw)) {
					...
					final String ow = CMLib.english().stripPunctuation(
							thisOneNameV.get(o));
	...
	public boolean mayICraft(final MOB crafterM, final Item I) {
		if (!mayICraft(I))
			return false;
		if ((!crafterM.isMine(I))
				&& (!CMLib.law().doesHavePriviledgesHere(crafterM,
						crafterM.location())))
			return false;
		return true;
	}
	...
	protected void setWeaponTypeClass(Weapon weapon, String weaponClass) {
		setWeaponTypeClass(weapon, weaponClass, Weapon.TYPE_BASHING,
				Weapon.TYPE_BASHING);
	}

	protected void setWeaponTypeClass(Weapon weapon, String weaponClass,
			int flailedType) {
		setWeaponTypeClass(weapon, weaponClass, flailedType,
				Weapon.TYPE_BASHING);
	}

	protected void setWeaponTypeClass(Weapon weapon, String weaponClass,
			int flailedType, int naturalType) {
	...
	public List<Object> getAbilityComponents(MOB mob, String componentID,
			String doingWhat, int autoGenerate) {
		if (autoGenerate > 0)
		...
		if (componentID.trim().startsWith("(")) {
			...
			String error = CMLib.ableMapper().addAbilityComponent(
					"ID=" + componentID, H);
			if (error != null)
				Log.errOut(ID(), "Error parsing custom component: "
						+ componentID);
			componentsRequirements = H.get("ID");
		} else
			componentsRequirements = CMLib.ableMapper()
					.getAbilityComponentMap().get(componentID.toUpperCase());
		if (componentsRequirements != null) {
			List<Object> components = CMLib.ableMapper().componentCheck(mob,
			...
			for (int r = 0; r < componentsRequirements.size(); r++)
				buf.append(CMLib.ableMapper().getAbilityComponentDesc(mob,
						componentsRequirements, r));
			mob.tell("You lack the necessary materials to "
					+ doingWhat.toLowerCase() + ", the requirements are: "
					+ buf.toString() + ".");
			return null;
		}
		return new LinkedList<Object>();
	...
	public Pair<String, Integer> getDecodedItemNameAndLevel(
			final List<String> recipe) {
		return new Pair<String, Integer>(recipe.get(RCP_FINALNAME),
				Integer.valueOf(CMath.s_int(recipe.get(RCP_LEVEL))));
	}

	public String getComponentDescription(final MOB mob,
			final List<String> recipe, final int RCP_WOOD) {
		...
		if (CMath.isInteger(woodStr)) {
			...
			if (componentsRequirements != null)
				return CMLib.ableMapper().getAbilityComponentDesc(mob,
						componentsRequirements);
		}
		return "?";
	}
	...
	protected boolean doLearnRecipe(MOB mob, Vector commands,
		...
		recipeHolder = null;
		if ((!(this instanceof ItemCraftor))
				|| (!((ItemCraftor) this).supportsDeconstruction())) {
			commonTell(mob,
					"You don't know how to learn new recipes with this skill.");
			return false;
		}
		...
		if (commands.size() < 1) {
			commonTell(mob,
					"You've failed to specify which item to deconstruct and learn.");
			return false;
		}
		buildingI = getTarget(mob, mob.location(), givenTarget, commands,
		...
		if (!mayICraft(mob, buildingI)) {
			commonTell(mob,
					"You can't learn anything about " + buildingI.name(mob)
							+ " with " + name() + ".");
			return false;
		}
		if (!buildingI.amWearingAt(Wearable.IN_INVENTORY)) {
			commonTell(mob, "You need to remove " + buildingI.name(mob)
					+ " first.");
			return false;
		}
		if ((buildingI instanceof Container)
				&& (((Container) buildingI).getContents().size() > 0)) {
			commonTell(mob, "You need to empty " + buildingI.name(mob)
					+ " first.");
			return false;
		}
		recipeHolder = null;
		for (int i = 0; i < mob.numItems(); i++) {
			...
			if ((I instanceof Recipe) && (I.container() == null)) {
				Recipe R = (Recipe) I;
				if (((R.getCommonSkillID().length() == 0) || (R
						.getCommonSkillID().equalsIgnoreCase(ID())))
		...
		if (recipeHolder == null) {
			commonTell(
					mob,
					"You need to have either a blank recipe page or book, or one already containing recipes for "
							+ name() + " that has blank pages.");
			return false;
		}
		...
		CMMsg msg = CMClass.getMsg(mob, buildingI, this,
				...
				"<S-NAME> start(s) deconstructing and studying <T-NAMESELF>.");
		if (mob.location().okMessage(mob, msg)) {
			mob.location().send(mob, msg);
			buildingI = (Item) msg.target();
			int duration = getDuration(10 + buildingI.phyStats().level(), mob,
					buildingI.phyStats().level(), 10);
