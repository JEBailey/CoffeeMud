...
public class GenAbility extends StdAbility {
	// data should be stored in a common instance object .. something common to
	// all genability of same id,
	...
	public ScriptingEngine getScripter() {
		if (((String) V(ID, V_SCRP)).hashCode() != scriptParmHash) {
			...
			if (parm.trim().length() == 0)
				...
				scriptObj = (ScriptingEngine) CMClass
	...
	public CMObject newInstance() {
		try {
			...
			if (scriptObj != null) {
				A.scriptObj = (ScriptingEngine) CMClass
						.getCommon("DefaultScriptingEngine");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!auto)
				...
				&& (!CMLib.masking().maskCheck((String) V(ID, V_CMSK), mob,
						true))) {
			mob.tell("You do not meet the requirements: "
					+ CMLib.masking().maskDesc((String) V(ID, V_CMSK)));
			return false;
		}
		// dont forget to allow super. calls to Spell.invoke, Chant.invoke,
		...
		if ((this.abstractQuality() == Ability.QUALITY_BENEFICIAL_SELF)
			...
			case Ability.CAN_ITEMS:
				target = super.getTarget(mob, mob.location(), givenTarget,
			...
			case Ability.CAN_EXITS: {
				...
				if (openThis == null)
					openThis = mob.location().fetchFromRoomFavorItems(null,
							whatToOpen);
			...
			case 0:
				break;
			default:
				target = super.getAnyTarget(mob, commands, givenTarget,
						Wearable.FILTER_ANY);
				if (target == null)
					return false;
				break;
		...
		if ((!auto)
				...
				&& (!CMLib.masking().maskCheck((String) V(ID, V_TMSK), target,
						true))) {
			mob.tell("The target is invalid: "
					+ CMLib.masking().maskDesc((String) V(ID, V_TMSK)));
			return false;
		}
		...
		if ((armorCheck > 0) && (!auto) && (!mob.isMonster())
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this, castCode,
					(auto ? (String) V(ID, V_ACST) : (String) V(ID, V_CAST)));
			CMMsg msg2 = null;
			Integer OTH = (Integer) V(ID, V_ATT2);
			if (OTH.intValue() > 0)
				msg2 = CMClass.getMsg(mob, target, this,
						CMMsg.MSK_CAST_MALICIOUS | OTH.intValue()
								| (auto ? CMMsg.MASK_ALWAYS : 0), null);
			if (mob.location().okMessage(mob, msg)
					&& (this.okMessage(mob, msg))
					&& ((msg2 == null) || (mob.location().okMessage(mob, msg2) && (this
							.okMessage(mob, msg2))))) {
				...
				if ((msg.value() <= 0)
					...
					if ((afterAffect.length() > 0) && (success)) {
						...
						if (P != null) {
							...
							P.invoke(mob, V, null, true, asLevel); // spell
																	// adder
																	// will
																	// return
																	// addable
							...
							if (target != null)
								for (int v = 0; v < V.size(); v += 2) {
									...
									if (target.fetchEffect(A.ID()) == null) {
										A = (Ability) A.copyOf();
										int tickDown = (abstractQuality() == Ability.QUALITY_MALICIOUS) ? getMaliciousTickdownTime(
												mob, target, 0, asLevel)
				...
				if (DMG.trim().length() > 0)
					dmg = CMath.parseIntExpression(DMG, new double[] {
							mob.phyStats().level(),
							(target == null) ? mob.phyStats().level() : target
									.phyStats().level() });
				if (((msg.value() <= 0) && ((msg2 == null) || (msg2.value() <= 0)))
					...
					if ((success) && (((String) V(ID, V_PCST)).length() > 0))
						if ((target == null)
								...
								|| (target instanceof Area)
								|| (mob.location() == CMLib.map().roomLocation(
										target))) {
							if (dmg > 0) {
					...
					if (CMLib.flags().isInTheGame(mob, true)
							&& ((target == null) || CMLib.flags().isInTheGame(
									(MOB) target, true))) {
						ScriptingEngine S = getScripter();
						if ((success) && (S != null)) {
							msg2 = CMClass.getMsg(mob, target, this,
									CMMsg.MSG_OK_VISUAL, null, null, ID);
				...
				if (((msg.value() <= 0) && ((msg2 == null) || (msg2.value() <= 0)))
						&& (CMLib.flags().isInTheGame(mob, true) && ((target == null) || CMLib
								.flags().isInTheGame(target, true)))) {
					String afterCast = (String) V(ID, V_PABL);
					if (afterCast.length() > 0) {
						Ability P = CMClass.getAbility("Prop_SpellAdder");
						if (P != null)
							P.invoke(mob, new XVector(afterCast), target, true,
									asLevel);
					}
				}
			}
	...
	public void affectPhyStats(Physical affectedEnv, PhyStats affectableStats) {
		if ((Ability) V(ID, V_HERE) != null)
			((Ability) V(ID, V_HERE)).affectPhyStats(affectedEnv,
					affectableStats);
	}

	public void affectCharStats(MOB affectedMob, CharStats affectableStats) {
		if (((Ability) V(ID, V_HERE)) != null)
			((Ability) V(ID, V_HERE)).affectCharStats(affectedMob,
					affectableStats);
	...
	public void affectCharState(MOB affectedMob, CharState affectableMaxState) {
		if (((Ability) V(ID, V_HERE)) != null)
			((Ability) V(ID, V_HERE)).affectCharState(affectedMob,
					affectableMaxState);
	}
	...
	public String getStat(String code) {
		...
		case 3:
			return (String) V(ID, V_DISP);
		case 4:
			return CMParms.toStringList((String[]) V(ID, V_TRIG));
		case 5:
			return convert(Ability.RANGE_CHOICES,
					((Integer) V(ID, V_MAXR)).intValue(), false);
		case 6:
			return convert(Ability.RANGE_CHOICES,
					((Integer) V(ID, V_MINR)).intValue(), false);
		case 7:
			return ((Boolean) V(ID, V_AUTO)).toString();
		case 8:
			return convert(Ability.FLAG_DESCS,
					((Integer) V(ID, V_FLAG)).intValue(), true);
		case 9:
			return convertClassAndDomain(((Integer) V(ID, V_CLAS)).intValue());
		case 10:
			return ((Integer) V(ID, V_OMAN)).toString();
		case 11:
			return convert(Ability.USAGE_DESCS,
					((Integer) V(ID, V_USAG)).intValue(), true);
		case 12:
			return convert(Ability.CAN_DESCS,
					((Integer) V(ID, V_CAFF)).intValue(), true);
		case 13:
			return convert(Ability.CAN_DESCS,
					((Integer) V(ID, V_CTAR)).intValue(), true);
		case 14:
			return convert(Ability.QUALITY_DESCS,
					((Integer) V(ID, V_QUAL)).intValue(), false);
		case 15:
		...
		case 23:
			return convert(CMMsg.TYPE_DESCS,
					((Integer) V(ID, V_ATT2)).intValue(), false);
		case 24:
	...
	public void setStat(String code, String val) {
		...
		case 3:
			...
			break;
		case 4:
			SV(ID, V_TRIG, CMParms.toStringArray(CMParms.parseCommas(
		...
		default:
			if (code.equalsIgnoreCase("allxml")
					&& ID.equalsIgnoreCase("GenAbility"))
				parseAllXML(val);
			break;
		}
	}
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (tacod < 0) {
				for (int i = 0; i < Ability.ACODE_DESCS.length; i++)
					if (Ability.ACODE_DESCS[i].toUpperCase().startsWith(
							val.toUpperCase()))
						tacod = i;
				if (tacod < 0) {
					int tdom = -1;
					...
					if (tdom < 0)
						for (int i = 0; i < Ability.DOMAIN_DESCS.length; i++)
							if (Ability.DOMAIN_DESCS[i].toUpperCase()
									.startsWith(val.toUpperCase())
									|| Ability.DOMAIN_DESCS[i].toUpperCase()
											.endsWith(val.toUpperCase())) {
								tdom = i << 5;
								break;
							}
					if (tdom >= 0)
	...
	private String convertClassAndDomain(int val) {
		...
		if ((acod >= 0) && (acod < Ability.ACODE_DESCS.length) && (dom >= 0)
				&& (dom < Ability.DOMAIN_DESCS.length))
	...
	private void parseAllXML(String xml) {
		...
		for (int c = 0; c < getStatCodes().length; c++)
			if (getStatCodes()[c].equals("CLASS"))
				ID = CMLib.xml().restoreAngleBrackets(
						CMLib.xml().getValFromPieces(V, getStatCodes()[c]));
			else if (!getStatCodes()[c].equals("TEXT"))
				setStat(getStatCodes()[c],
