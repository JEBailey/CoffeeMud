	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (tickID == Tickable.TICKID_MOB) {
			...
			if (nextDirection == 999) {
				...
				mob.tell("The water trail seems to continue "
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (rooms.size() > 0)
			theTrail = CMLib.tracking().findBastardTheBestWay(mob.location(),
					rooms, flags, 60 + (2 * getXLEVELLevel(mob)));

		if ((success) && (theTrail != null)) {
			CMMsg msg = CMClass.getMsg(mob, null, this,
					CMMsg.MSG_QUIETMOVEMENT,
					auto ? "<S-NAME> begin(s) sniffing around for water!"
							: "<S-NAME> begin(s) sensing water.");
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to find water, but fail(s).");
