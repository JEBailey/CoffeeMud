	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (rooms.size() > 0)
			theTrail = CMLib.tracking().findBastardTheBestWay(mob.location(),
					rooms, flags, 75 + (2 * getXLEVELLevel(mob)));
		...
		if ((success) && (theTrail != null) && (target != null)) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					CMMsg.MSG_QUIETMOVEMENT, mob.isMonster() ? null
							: "<S-NAME> begin(s) to track <T-NAMESELF>.", null,
					mob.isMonster() ? null
							: "<S-NAME> begin(s) to track <T-NAMESELF>.");
			...
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to track " + mobName
							+ ", but can't find the trail.");
