...
public class StdAbility implements Ability {
	...
	public ExpertiseLibrary.SkillCost getTrainingCost(MOB mob) {
		...
		if (mob != null) {
			...
			if (O != null) {
				...
				if (val != null)
					return new ExpertiseLibrary.SkillCost(
							ExpertiseLibrary.CostType.TRAIN, Double.valueOf(val
									.intValue()));
				val = O[AbilityMapper.AbilityMapping.COST_PRAC];
				if (val != null)
					return new ExpertiseLibrary.SkillCost(
							ExpertiseLibrary.CostType.PRACTICE,
							Double.valueOf(val.intValue()));
			}
		...
		if (rawCost == null)
			return new ExpertiseLibrary.SkillCost(
					ExpertiseLibrary.CostType.TRAIN, Double.valueOf(1.0));
		final double[] vars = new double[] { qualifyingLevel, playerLevel };
		final double value = CMath.parseMathExpression(rawCost.costDefinition,
				vars);
		return new ExpertiseLibrary.SkillCost(rawCost.type,
	...
	public int practicesToPractice(MOB mob) {
		if (mob != null) {
			...
			if ((O != null)
					&& (O[AbilityMapper.AbilityMapping.COST_PRACPRAC] != null))
	...
	protected synchronized int expertise(MOB mob, int code) {
		if ((mob != null)
				&& (this.isNowAnAutoEffect() || (this.canBeUninvoked()) || this
						.isAutoInvoked())) {
			...
			for (int x = 0; x < ExpertiseLibrary.NUM_XFLAGS; x++)
				xFlagCache[x] = CMLib.expertises().getApplicableExpertiseLevel(
						ID(), x, mob);
	...
	protected int getXPCOSTAdjustment(MOB mob, int xpLoss) {
		...
		if (xLevel <= 0)
			return xpLoss;
		return xpLoss
				- (int) Math.round(CMath.mul(xpLoss, CMath.mul(.05, xLevel)));
	}
	...
	public int adjustedMaxInvokerRange(int max) {
		...
		if (level <= 0)
			return max;
		return max
				+ (int) Math.round(Math.ceil(CMath.mul(max,
						CMath.mul(level, 0.2))));
	}
	...
	protected int canAffectCode() {
		return Ability.CAN_AREAS | Ability.CAN_ITEMS | Ability.CAN_MOBS
				| Ability.CAN_ROOMS | Ability.CAN_EXITS;
	}
	...
	public void setExpirationDate(long time) {
		if (time > System.currentTimeMillis())
			tickDown = (int) ((time - System.currentTimeMillis()) / CMProps
					.getTickMillis());
	}
	...
	protected int addedTickTime(MOB invokerMOB, int baseTickTime) {
		return (int) Math.round(CMath.mul(baseTickTime,
				CMath.mul(getXTIMELevel(invokerMOB), 0.20)));
	}
	...
	public void startTickDown(MOB invokerMOB, Physical affected, int tickTime) {
		...
		if (invoker() != null)
			for (int c = 0; c < invoker().charStats().numClasses(); c++)
				tickTime = invoker().charStats().getMyClass(c)
						.classDurationModifier(invoker(), this, tickTime);
		if (affected instanceof MOB) {
			...
			if (invoker() != affected)
				for (int c = 0; c < mob.charStats().numClasses(); c++)
					tickTime = mob.charStats().getMyClass(c)
							.classDurationModifier(mob, this, tickTime);
		} else {
	...
	protected boolean disregardsArmorCheck(MOB mob) {
		return ((mob == null) || (mob.isMonster()) || (CMLib.ableMapper()
				.qualifiesByLevel(mob, this)));
	}

	public int adjustedLevel(MOB caster, int asLevel) {
		if (caster == null)
			return 1;
		int lowestQualifyingLevel = CMLib.ableMapper().lowestQualifyingLevel(
				this.ID());
		int adjLevel = lowestQualifyingLevel;
		if (asLevel <= 0) {
			int qualifyingLevel = CMLib.ableMapper().qualifyingLevel(caster,
					this);
			if ((caster.isMonster()) || (qualifyingLevel >= 0))
				adjLevel += (CMLib.ableMapper().qualifyingClassLevel(caster,
						this) - qualifyingLevel);
			else
				adjLevel = caster.phyStats().level() - lowestQualifyingLevel
						- 25;
		} else
			adjLevel = asLevel;
		...
		for (int c = 0; c < CS.numClasses(); c++)
			level = CS.getMyClass(c).classDurationModifier(invoker(), this,
					level);
		return level;
	}
	...
	public int experienceLevels(MOB caster, int asLevel) {
		...
		int lowestQualifyingLevel = CMLib.ableMapper().lowestQualifyingLevel(
				this.ID());
		if (qualifyingLevel >= 0) {
			int qualClassLevel = CMLib.ableMapper().qualifyingClassLevel(
					caster, this);
	...
	public MOB getTarget(MOB mob, Vector commands, Environmental givenTarget,
			boolean quiet, boolean alreadyAffOk) {
		...
		if ((givenTarget != null) && (givenTarget instanceof MOB))
			...
			target = mob.getVictim();
				...
				&& (castingQuality(mob, mob) == Ability.QUALITY_BENEFICIAL_SELF))
			target = mob;
		else if ((targetName.length() == 0)
		...
		if ((target == null)
			...
			if (!quiet) {
				if (targetName.trim().length() == 0)
					...
					mob.tell("You don't see anyone called '" + targetName
							+ "' here.");
			}
			return null;
		}

		if ((!alreadyAffOk) && (!isAutoInvoked())
			...
			if ((givenTarget == null) && (!quiet)) {
				if (target == mob)
					...
					mob.tell(target, null, null,
							"<S-NAME> is already affected by " + name() + ".");
			}
			return null;
		}
		return target;
	...
	public Physical getAnyTarget(MOB mob, Vector commands,
	...
	public Physical getAnyTarget(MOB mob, Vector commands,
		...
		return getAnyTarget(mob, commands, givenTarget, filter,
				checkOthersInventory, false);
	}
	...
	public Physical getAnyTarget(MOB mob, Vector commands,
		...
		if (givenTarget != null)
			...
			if (target == null)
				target = R.fetchFromMOBRoomFavorsItems(mob, null, targetName,
						filter);
			if ((target == null)
					&& (targetName.equalsIgnoreCase("room")
							|| targetName.equalsIgnoreCase("here") || targetName
								.equalsIgnoreCase("place")))
				target = R;
			int dir = -1;
			if ((target == null)
					&& ((dir = Directions.getGoodDirectionCode(targetName)) >= 0))
		...
		if ((target == null)
				|| ((givenTarget == null)
						&& (!CMLib.flags().canBeSeenBy(target, mob)) && ((!CMLib
						.flags().canBeHeardMovingBy(target, mob)) || ((target instanceof MOB) && (!((MOB) target)
						.isInCombat()))))) {
			if (targetName.trim().length() == 0)
			...
			if (givenTarget == null) {
				if (target == mob)
					...
					mob.tell(mob, target, null,
							"<T-NAME> is already affected by " + name() + ".");
			}
			return null;
		}
		return target;
	...
	protected static Item possibleContainer(MOB mob, Vector commands,
			boolean withStuff, Filterer<Environmental> filter) {
		if ((commands == null) || (commands.size() < 2))
			return null;

		String possibleContainerID = (String) commands.lastElement();
		Environmental thisThang = mob.location().fetchFromMOBRoomFavorsItems(
				mob, null, possibleContainerID, filter);
		if ((thisThang != null)
				&& (thisThang instanceof Item)
				&& (((Item) thisThang) instanceof Container)
				&& ((!withStuff) || (((Container) thisThang).getContents()
						.size() > 0))) {
	...
	public Item getTarget(MOB mob, Room location, Environmental givenTarget,
			Vector commands, Filterer<Environmental> filter) {
		return getTarget(mob, location, givenTarget, null, commands, filter);
	}

	public Item getTarget(MOB mob, Room location, Environmental givenTarget,
			Item container, Vector commands, Filterer<Environmental> filter) {
		String targetName = CMParms.combine(commands, 0);
		...
		if ((target == null) && (targetName.length() > 0)) {
			if (location != null)
				target = location.fetchFromMOBRoomFavorsItems(mob, container,
						targetName, filter);
			else
		...
		if ((target == null)
				...
				|| ((givenTarget == null) && (!CMLib.flags().canBeSeenBy(
						target, mob)))) {
			if (targetName.length() == 0)
				...
				if (targetName.trim().length() == 0)
					...
					mob.tell("You don't see anything called '" + targetName
							+ "' here.");
			} else
				mob.tell(mob, target, null,
						"You can't do that to <T-NAMESELF>.");
	...
	protected int[] buildCostArray(MOB mob, int consumed, int minimum) {
		...
		if (useMana) {
			if (consumed == COST_ALL) {
				...
				usageCosts[0] = (int) (Math.round(CMath.mul(mob.maxState()
						.getMana(), CMath.div((COST_ALL - consumed), 100.0))) - costDown);
			else
		...
		if (useMoves) {
			if (consumed == COST_ALL) {
				...
				if (mob.baseState().getMovement() > mob.maxState()
						.getMovement())
					usageCosts[1] = (mob.baseState().getMovement() - costDown);
			} else if (consumed > COST_PCT)
				usageCosts[1] = (int) (Math
						.round(CMath.mul(mob.maxState().getMovement(),
								CMath.div((COST_ALL - consumed), 100.0))) - costDown);
			else
		...
		if (useHits) {
			if (consumed == COST_ALL) {
				...
				if (mob.baseState().getHitPoints() > mob.maxState()
						.getHitPoints())
					usageCosts[2] = (mob.baseState().getHitPoints() - costDown);
			} else if (consumed > COST_PCT)
				usageCosts[2] = (int) (Math.round(CMath.mul(mob.maxState()
						.getHitPoints(), CMath
						.div((COST_ALL - consumed), 100.0))) - costDown);
			else
	...
	public int[] usageCost(MOB mob, boolean ignoreClassOverride) {
		if (mob == null) {
			final Map<String, int[]> overrideCache = CMLib.ableMapper()
		...
		final int myCacheIndex = ignoreClassOverride ? Ability.CACHEINDEX_CLASSLESS
				: Ability.CACHEINDEX_NORMAL;
		...
		if (!rebuildCache && (myCache != null)) {
			...
			for (int c = 0; c < mob.charStats().numClasses(); c++) {
				...
				int qualifyingLevel = CMLib.ableMapper().getQualifyingLevel(
						C.ID(), true, ID());
			...
			if ((costOverrides != null)
					&& (costOverrides[AbilityMapper.AbilityMapping.COST_MANA] != null)) {
				consumed = costOverrides[AbilityMapper.AbilityMapping.COST_MANA]
						.intValue();
	...
	public void helpProficiency(MOB mob, int adjustment) {
		...
		int maxProficiency = CMLib.ableMapper().getMaxProficiency(mob, true,
				ID());
		if (A.proficiency() < maxProficiency) {
			final int currentProficiency = A.proficiency() + adjustment;
			if (((int) Math.round(Math.sqrt((mob.charStats()
					.getStat(CharStats.STAT_INTELLIGENCE)))
					* 34.0
					* Math.random())) >= currentProficiency) {
				int qualLevel = CMLib.ableMapper().qualifyingLevel(mob, A);
				if ((qualLevel < 0)
						|| (qualLevel > 30)
						|| (CMLib.dice().rollPercentage() < (int) Math
								.round(100.0 * CMath.div(31 - qualLevel,
										30 + qualLevel)))) {
					// very important, since these can be autoinvoked affects
					// (copies)!
	...
	public boolean invoke(MOB mob, Vector commands, Physical target,
		...
		if ((!auto) && (mob != null)) {
			...
			final Room room = mob.location();
			if ((getTicksBetweenCasts() > 0) && (getTimeOfNextCast() > 0)
					&& (System.currentTimeMillis() < getTimeOfNextCast())
					&& (room != null) && (room.getArea() != null)) {
				TimeClock C = room.getArea().getTimeObj();
				if (C != null)
					mob.tell("You must wait "
							+ C.deriveEllapsedTimeString(getTimeOfNextCast()
			...
			CMMsg msg = CMClass.getMsg(mob, null, this, CMMsg.MSG_PREINVOKE,
					null);
	...
	public boolean checkComponents(MOB mob) {
		if ((mob != null)
			...
			Vector<AbilityComponent> componentsRequirements = (Vector<AbilityComponent>) CMLib
					.ableMapper().getAbilityComponentMap()
					.get(ID().toUpperCase());
			if (componentsRequirements != null) {
				List<Object> components = CMLib.ableMapper().componentCheck(
						mob, componentsRequirements);
				if (components == null) {
					mob.tell("You lack the necessary materials to use this "
							+ Ability.ACODE_DESCS[classificationCode()
									& Ability.ALL_ACODES].toLowerCase()
							+ ", the requirements are: "
							+ CMLib.ableMapper().getAbilityComponentDesc(mob,
									ID()) + ".");
					return false;
				}
	...
	public Set<MOB> properTargets(MOB mob, Environmental givenTarget,
			boolean auto) {
	...
	public int getMaliciousTickdownTime(MOB mob, Physical target,
			int tickAdjustmentFromStandard, int asLevel) {
		if (tickAdjustmentFromStandard <= 0) {
			tickAdjustmentFromStandard = ((int) Math.round(CMath.mul(
					adjustedLevel(mob, asLevel), 1.3))) + 25;
			if ((target != null) && (asLevel <= 0) && (mob != null)
					&& (!(target instanceof Room)))
				tickAdjustmentFromStandard = (int) Math.round(CMath.mul(
						tickAdjustmentFromStandard, CMath.div(mob.phyStats()
								.level(), target.phyStats().level())));

			if ((tickAdjustmentFromStandard > (CMProps
					.getIntVar(CMProps.Int.TICKSPERMUDDAY)))
					|| (mob instanceof Deity))
				tickAdjustmentFromStandard = (CMProps
						.getIntVar(CMProps.Int.TICKSPERMUDDAY));
	...
	public boolean maliciousAffect(MOB mob, Physical target, int asLevel,
			int tickAdjustmentFromStandard, int additionAffectCheckCode) {
		boolean ok = true;
		final Room room = mob.location();
		if (room == null)
			return false;
		if (additionAffectCheckCode >= 0) {
			CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.NO_EFFECT,
					additionAffectCheckCode, CMMsg.NO_EFFECT, null);
		...
		if (ok) {
			...
			tickAdjustmentFromStandard = getMaliciousTickdownTime(mob, target,
					tickAdjustmentFromStandard, asLevel);
			newOne.startTickDown(invoker, target, tickAdjustmentFromStandard);
		}
		return ok;
	}

	public boolean beneficialWordsFizzle(MOB mob, Environmental target,
			String message) {
		// it didn't work, but tell everyone you tried.
		CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MSG_SPEAK, "^T"
				+ message + "^?");
		final Room room = mob.location();
		if (room == null)
			return false;
	...
	public boolean beneficialVisualFizzle(MOB mob, Environmental target,
			String message) {
		// it didn't work, but tell everyone you tried.
	...
	public int getBeneficialTickdownTime(MOB mob, Environmental target,
			int tickAdjustmentFromStandard, int asLevel) {
		if (tickAdjustmentFromStandard <= 0) {
	...
	public boolean beneficialAffect(MOB mob, Physical target, int asLevel,
		...
		if (ok) {
			...
			((StdAbility) newOne).canBeUninvoked = true;
			tickAdjustmentFromStandard = getBeneficialTickdownTime(mob, target,
					tickAdjustmentFromStandard, asLevel);
			newOne.startTickDown(invoker, target, tickAdjustmentFromStandard);
		}
		return ok;
	...
	public boolean canBeTaughtBy(MOB teacher, MOB student) {
		...
		if (CMLib.flags().isSleeping(teacher)
				|| CMLib.flags().isSitting(teacher)) {
		...
		if (yourAbility != null) {
			int prof25 = (int) Math.round(CMath.mul(CMLib.ableMapper()
					.getMaxProficiency(student, true, yourAbility.ID()), .25));
			if (yourAbility.proficiency() < prof25) {
				teacher.tell("You are not proficient enough to teach '"
						+ name() + "'");
				student.tell(teacher.name()
						+ " is not proficient enough to teach '" + name()
						+ "'.");
				return false;
			}
			return true;
	...
	public boolean canBeLearnedBy(MOB teacher, MOB student) {
		...
		if (!cost.doesMeetCostRequirements(student)) {
			...
			teacher.tell(student.name() + " does not have enough " + ofWhat
					+ " to learn '" + name() + "'.");
			student.tell("You do not have enough " + ofWhat + ".");
			return false;
		}
		if ((CMath.bset(student.getBitmap(), MOB.ATT_NOTEACH))
				&& ((!student.isMonster()) || (!student
						.willFollowOrdersOf(teacher)))) {
		...
		if (qLevel < 0) {
			teacher.tell(student.name() + " is not the right class to learn '"
					+ name() + "'.");
		...
		if ((!student.charStats().getCurrentClass().leveless())
			...
			teacher.tell(student.name()
					+ " is not high enough level to learn '" + name() + "'.");
		...
		if (qLevel > (student.charStats().getStat(CharStats.STAT_INTELLIGENCE) + 15)) {
			teacher.tell(student.name() + " is not smart enough to learn '"
					+ name() + "'.");
			student.tell("You are not of high enough intelligence to learn '"
					+ name() + "'.");
			return false;
		}
		...
		if (teacherAbility != null) {
			int prof25 = (int) Math
					.round(CMath.mul(
							CMLib.ableMapper().getMaxProficiency(student, true,
									teacherAbility.ID()), .25));
			if (teacherAbility.proficiency() < prof25) {
				teacher.tell("You aren't proficient enough to teach '" + name()
						+ "'.");
				student.tell(teacher.name()
						+ " isn't proficient enough to teach you '" + name()
						+ "'.");
				return false;
			}
		} else {
		...
		if (CMLib.flags().isSleeping(student)
				|| CMLib.flags().isSitting(student)) {
			student.tell("You need to stand up and be alert to learn.");
			teacher.tell(student.name()
					+ " needs to stand up to be taught about that.");
			return false;
		}

		String extraMask = CMLib.ableMapper().getApplicableMask(student, this);
		if ((extraMask.length() > 0)
				&& (!CMLib.masking().maskCheck(extraMask, student, true))) {
			String reason = "requirements: "
					+ CMLib.masking().maskDesc(extraMask);
			student.tell("You may not learn '" + name()
					+ "' at this time due to the " + reason + ".");
			teacher.tell(student.name() + " does not fit the '" + name() + "' "
					+ reason + ".");
			return false;
		}

		DVector prereqs = CMLib.ableMapper().getUnmetPreRequisites(student,
				this);
		if ((prereqs != null) && (prereqs.size() > 0)) {
			String names = CMLib.ableMapper().formatPreRequisites(prereqs);
			student.tell("You must learn " + names + " before you can gain "
					+ name() + ".");
			teacher.tell(student.name()
					+ " has net learned the pre-requisites to " + name()
					+ " yet.");
			return false;
		}
	...
	public boolean canBePracticedBy(MOB teacher, MOB student) {
		if ((practicesToPractice(student) > 0)
				&& (student.getPractices() < practicesToPractice(student))) {
			teacher.tell(student.name()
					+ " does not have enough practices to practice '" + name()
					+ "'.");
		...
		Ability teacherAbility = teacher.fetchAbility(ID());
		if (yourAbility == null) {
			teacher.tell(student.name() + " has not gained '" + name()
					+ "' yet.");
		...
		if (teacherAbility == null) {
			student.tell(teacher.name() + " does not know anything about '"
					+ name() + "'.");
			teacher.tell("You don't know '" + name() + "'.");
			return false;
		}

		int prof75 = (int) Math.round(CMath.mul(CMLib.ableMapper()
				.getMaxProficiency(student, true, yourAbility.ID()), .75));
		if (yourAbility.proficiency() > teacherAbility.proficiency()) {
			teacher.tell("You aren't proficient enough to teach any more about '"
					+ name() + "'.");
			student.tell(teacher.name()
					+ " isn't proficient enough to teach any more about '"
					+ name() + "'.");
			return false;
		} else if (yourAbility.proficiency() > prof75 - 1) {
			teacher.tell("You can't teach " + student.charStats().himher()
					+ " any more about '" + name() + "'.");
			student.tell("You can't learn any more about '" + name()
					+ "' except through dilligence.");
			return false;
		}

		int prof25 = (int) Math.round(CMath.mul(CMLib.ableMapper()
				.getMaxProficiency(student, true, teacherAbility.ID()), .25));
		if (teacherAbility.proficiency() < prof25) {
			teacher.tell("You aren't proficient enough to teach '" + name()
					+ "'.");
			student.tell(teacher.name()
		...
		if (CMLib.flags().isSleeping(student)
			...
			teacher.tell(student.name()
					+ " needs to stand up to practice that.");
			return false;
		}
		if (student.isInCombat()) {
			teacher.tell(student.name() + " seems busy right now.");
	...
	public void teach(MOB teacher, MOB student) {
		if (student.fetchAbility(ID()) == null) {
			...
			int prof75 = (int) Math.round(CMath.mul(CMLib.ableMapper()
					.getMaxProficiency(student, true, newAbility.ID()), .75));
			newAbility
					.setProficiency((int) Math.round(CMath.mul(
							proficiency(),
							((CMath.div(
									teacher.charStats().getStat(
			...
			int defProficiency = CMLib.ableMapper().getDefaultProficiency(
					student.charStats().getCurrentClass().ID(), true, ID());
			if ((defProficiency > 0)
					&& (defProficiency > newAbility.proficiency()))
	...
	public void practice(MOB teacher, MOB student) {
		...
		if (yourAbility != null) {
			...
			if (yourAbility.proficiency() < prof75) {
				student.setPractices(student.getPractices()
						- practicesToPractice(student));
				int newProf = yourAbility.proficiency()
						+ (int) Math.round(25.0 * (CMath.div(
								teacher.charStats().getStat(
										CharStats.STAT_WISDOM)
										+ student.charStats().getStat(
												CharStats.STAT_INTELLIGENCE),
								36.0)));
				if (newProf > prof75)
					newProf = prof75;
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((canBeUninvoked()) && (tickID == Tickable.TICKID_MOB)
				&& (tickDown != Integer.MAX_VALUE)) {
			if (tickDown < 0)
				return !unInvoked;
			if ((--tickDown) <= 0) {
