...
public class AstroEngineering extends TechSkill {
	...
	private static final String[] triggerStrings = { "ASTROENGINEER",
			"ASTROENGINEERING", "ENGINEER", "AE" };
	...
	protected static enum Operation {
		INSTALL("installing"), REPAIR("repairing"), ENHANCE("enhancing");
		public String verb = "";
			...
			if ((affected instanceof MOB)
				...
				if ((aborted) || (room == null))
					mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> stop(s) " + op.verb + ".");
				else if (failure) {
					mob.location().show(mob, targetItem, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> mess(es) up " + op.verb + " <T-NAME>.");
				} else if (op == Operation.INSTALL) {
					CMMsg msg = CMClass.getMsg(mob, targetPanel, targetItem,
							CMMsg.MSG_PUT,
							"<S-NAME> install(s) <T-NAME> into <O-NAME>.");
					// TODO: put an appropriate value
					msg.setValue(100);
					if (room.okMessage(msg.source(), msg))
						room.send(msg.source(), msg);
				} else if (op == Operation.REPAIR) {
					CMMsg msg = CMClass.getMsg(mob, targetItem, this,
							CMMsg.MSG_PUT, "<S-NAME> repair(s) <T-NAME>.");
					// TODO: put an appropriate value
					msg.setValue(1);
					if (room.okMessage(msg.source(), msg))
						room.send(msg.source(), msg);
					...
					CMMsg msg = CMClass.getMsg(mob, targetItem, this,
							CMMsg.MSG_PUT, "<S-NAME> enhance(s) <T-NAME>.");
					// TODO: put an appropriate value
					msg.setValue(100);
					if (room.okMessage(msg.source(), msg))
						room.send(msg.source(), msg);
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((affected instanceof MOB) && (tickID == Tickable.TICKID_MOB)) {
			MOB mob = (MOB) affected;
			if ((mob.isInCombat()) || (mob.location() != targetRoom)
			...
			if (tickDown == 4)
				mob.location().show(
						...
						"<S-NAME> <S-IS-ARE> almost done " + op.verb
								+ " <T-NAME>.");
				...
				mob.location().show(
						...
						this,
						CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> continue(s) " + op.verb + " <T-NAME> (" + pct
								+ "% completed).", null,
						"<S-NAME> continue(s) " + op.verb + " <T-NAME>.");
			}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.firstElement() instanceof String) {
			if (((String) commands.firstElement()).equalsIgnoreCase("INSTALL")) {
				...
				if (givenTarget instanceof Item) {
					...
					targetPanel = mob.fetchItem(null,
							Wearable.FILTER_UNWORNONLY, panelName);
					if (targetPanel == null) {
						targetPanel = targetRoom.findItem(null, panelName);
					}
					if ((targetPanel == null)
							|| (!CMLib.flags().canBeSeenBy(targetPanel, mob))) {
		...
		if (targetItem == null)
			targetItem = mob.fetchItem(null, Wearable.FILTER_UNWORNONLY,
					itemName);
		if (targetItem == null)
			targetItem = targetRoom.findItem(null, itemName);
		if ((targetItem == null)
				|| (!CMLib.flags().canBeSeenBy(targetItem, mob))) {
