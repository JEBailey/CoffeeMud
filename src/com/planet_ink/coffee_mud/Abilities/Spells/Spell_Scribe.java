...
public class Spell_Scribe extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int numSpells = (CMLib.ableMapper().qualifyingClassLevel(mob, this) - CMLib
				.ableMapper().qualifyingLevel(mob, this));
		if (numSpells < 0)
			numSpells = 1;
		if (scroll.getSpells().size() > numSpells) {
			mob.tell("You aren't powerful enough to scribe any more spells onto "
					+ scroll.name() + ".");
			return false;
		}
		...
		int experienceToLose = 10 * CMLib.ableMapper().lowestQualifyingLevel(
				scrollThis.ID());
