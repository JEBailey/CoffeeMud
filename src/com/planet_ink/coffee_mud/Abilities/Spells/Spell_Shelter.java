		...
		if (canBeUninvoked()) {
			...
			for (MOB mob : mobs) {
				...
				CMMsg enterMsg = CMClass.getMsg(mob, previousLocation, null,
						...
						CMMsg.MSG_ENTER, "<S-NAME> appears out of nowhere!");
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> wave(s) <S-HIS-HER> arms, speak(s), and suddenly vanish(es)!^?");
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> wave(s) <S-HIS-HER> arms and and speak(s), but nothing happens.");
