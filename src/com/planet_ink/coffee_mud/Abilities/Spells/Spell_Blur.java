...
public class Spell_Blur extends Spell {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob)) && (mob != msg.source()) && (!mob.amDead())
				&& (!msg.source().amDead())
				&& (CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS))
				&& (msg.targetMinor() == CMMsg.TYP_CAST_SPELL)
				&& (invoker != null) && (msg.tool() instanceof Ability)) {
			int pctDodge = invoker.charStats().getStat(
					CharStats.STAT_INTELLIGENCE);
			if (CMLib.dice().rollPercentage() < (pctDodge * 2)) {
				CMMsg msg2 = CMClass.getMsg(mob, msg.source(), null,
						CMMsg.MSG_OK_VISUAL,
						"<T-NAME> can't seem to focus on <S-NAME>.");
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> begin(s) to come back into focus.");
	}
