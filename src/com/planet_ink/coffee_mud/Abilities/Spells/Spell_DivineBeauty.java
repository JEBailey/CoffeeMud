			...
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location()
						.show(mob, null, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> begin(s) to look like <S-HIS-HER> old ugly self.");
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> speak(s) beautifully to <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (target.location() == mob.location()) {
					target.location().show(target, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> become(s) divinely beautiful!!!");
					beneficialAffect(mob, target, asLevel, 0);
				}
			}
		} else
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> speak(s) beautifully to <T-NAMESELF>, but nothing more happens.");
