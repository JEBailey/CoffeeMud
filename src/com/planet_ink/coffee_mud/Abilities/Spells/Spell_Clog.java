		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> mouth reverts to normal size.");
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> gergle(s) at <T-NAMESELF>.^?");
					...
					if (target.location() == mob.location()) {
						target.location()
								.show(target, null, CMMsg.MSG_OK_ACTION,
										"<S-NAME> start(s) gagging and spitting as <S-HIS-HER> mouth shrinks!");
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> gurgle(s) at <T-NAMESELF>, but the spell fizzles.");
