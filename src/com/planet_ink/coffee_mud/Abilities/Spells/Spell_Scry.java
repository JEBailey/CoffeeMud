...
public class Spell_Scry extends Spell {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected instanceof MOB)
				...
				&& (invoker != null)
				&& (msg.target() != null)
				&& ((invoker.location() != ((MOB) affected).location()) || (!(msg
						.target() instanceof Room))) && (!recurse)) {
			CMMsg newAffect = CMClass.getMsg(invoker, msg.target(),
					msg.sourceMinor(), null);
			recurse = true;
			msg.target().executeMsg(msg.target(), newAffect);
			recurse = false;
		} else if ((affected instanceof MOB) && (invoker != null)
				&& (msg.source() != invoker)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (commands.size() < 1) {
			...
			for (int e = 0; e < scries.size(); e++)
				if (scries.elementAt(e, 2) == mob)
					scryList.append(((e > 0) ? ", " : "")
							+ ((MOB) scries.elementAt(e, 1)).name());
			if (scryList.length() > 0)
				mob.tell("Cast on or revoke from whom?  You currently have "
						+ name() + " on the following: " + scryList.toString()
						+ ".");
			else
		...
		if (target == null) {
			try {
				List<MOB> targets = CMLib.map().findInhabitants(
						CMLib.map().rooms(), mob, mobName, 50);
				if (targets.size() > 0)
		...
		if (success) {
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to invoke scrying, but fizzle(s) the spell.");
