	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					somanticCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> concentrate(s) on <T-NAMESELF>!^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				fakeS.initializeSession(null, Thread.currentThread()
				...
				if (iwin > hewin)
					mob.tell(iwin + "% of the time, you defeat "
							+ target.charStats().himher() + " with "
							+ (ihp / iwin) + " hit points left" + addendum
							+ ".");
				else if (hewin > iwin)
					mob.tell(hewin + "% of the time you die, and "
							+ target.charStats().himher() + " still has "
							+ (hehp / hewin) + " hit points left" + addendum
							+ ".");
				else if (iwin > 0)
					mob.tell("Half of the time, you defeat "
							+ target.charStats().himher() + " with "
							+ (ihp / iwin) + " hit points left" + addendum
							+ ".");
				else
					mob.tell("You can't hurt each other .. there were "
							+ (draws * 5) + "% draws.");
				// Log.debugOut(fakeS.afkMessage());
			}
		} else
			return beneficialVisualFizzle(mob, target,
					"<S-NAME> concentrate(s) on <T-NAMESELF>, but look(s) frustrated.");
