...
public class Spell_DetectMagic extends Spell {
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> eyes cease to sparkle.");
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected != null)
			...
			for (final Enumeration<Ability> a = targetP.effects(); a
					.hasMoreElements();) {
			...
			if (msg2 != null) {
				CMMsg msg3 = CMClass.getMsg(msg.source(), targetP, this,
						CMMsg.MSG_OK_VISUAL, msg2 + ".", CMMsg.NO_EFFECT, null,
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already detecting magic.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> gain(s) sparkling eyes!"
									: "^S<S-NAME> incant(s) softly, and gain(s) sparkling eyes!^?");
			...
			beneficialVisualFizzle(
					mob,
					null,
					"<S-NAME> incant(s) and open(s) <S-HIS-HER> eyes sparkling, but the spell fizzles.");
