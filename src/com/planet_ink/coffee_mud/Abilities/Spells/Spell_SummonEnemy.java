...
public class Spell_SummonEnemy extends Spell {
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> conjur(s) the dark shadow of a living creature...^?");
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> conjur(s), but nothing happens.");
	...
	public MOB determineMonster(MOB caster, int level) {
		...
		while ((monster == null) && ((--tries) > 0)) {
			...
			if ((room != null) && CMLib.flags().canAccess(caster, room)
					&& (room.numInhabitants() > 0)) {
				MOB mob = room.fetchRandomInhabitant();
				if ((mob != null)
						&& (!(mob instanceof Deity))
						&& (mob.phyStats().level() >= level
								- (CMProps.getIntVar(CMProps.Int.EXPRATE) / 2))
						&& (mob.phyStats().level() <= (level + (CMProps
								.getIntVar(CMProps.Int.EXPRATE) / 2)))
						&& (mob.charStats() != null)
						&& (mob.charStats().getMyRace() != null)
						&& (CMProps.isTheme(mob.charStats().getMyRace()
								.availabilityCode()))
						&& ((CMLib.flags().isGood(caster) && CMLib.flags()
								.isEvil(mob)) || (CMLib.flags().isNeutral(mob)) || (CMLib
								.flags().isEvil(caster) && CMLib.flags()
								.isGood(mob))) && (caster.mayIFight(mob)))
					monster = mob;
			}
		}
		if (monster == null)
		...
		monster.location().showOthers(monster, null, CMMsg.MSG_OK_ACTION,
				"<S-NAME> appears!");
