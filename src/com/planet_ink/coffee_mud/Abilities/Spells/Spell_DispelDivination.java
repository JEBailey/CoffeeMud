...
public class Spell_DispelDivination extends Spell {
	...
	public List<Ability> returnOffensiveAffects(MOB caster, Physical fromMe) {
		...
		for (int e = 0; e < fromMe.numEffects(); e++) // personal
		{
			...
			if ((A != null)
					...
					&& ((A.invoker() == caster)
							|| (A.invoker().phyStats().level() <= caster
									.phyStats().level() + 25) || admin))
				offenders.add(A);
		}
		return offenders;
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		List<Ability> allDivinations = CMLib.flags().domainAffects(target,
				Ability.DOMAIN_DIVINATION);
		boolean foundSomethingAtLeast = ((allDivinations != null) && (allDivinations
				.size() > 0));
