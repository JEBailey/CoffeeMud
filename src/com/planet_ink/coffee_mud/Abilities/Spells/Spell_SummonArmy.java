	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				String[] choices = { "Dog", "Orc", "Tiger", "Troll", "Chimp",
						"BrownBear", "Goblin", "LargeBat", "GiantScorpion",
						"Rattlesnake", "Ogre" };
				int num = (mob.phyStats().level() + (getXLEVELLevel(mob) + (2 * getX1Level(mob)))) / 3;
				for (int i = 0; i < num; i++) {
					MOB newMOB = CMClass.getMOB(choices[CMLib.dice().roll(1,
							choices.length, -1)]);
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> call(s) for magical help, but chokes on the words.");
