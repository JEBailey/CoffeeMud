...
public class Spell_MagicMouth extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		Physical target = mob.location().fetchFromMOBRoomFavorsItems(mob, null,
				((String) commands.elementAt(0)), Wearable.FILTER_UNWORNONLY);
		...
		String triggerStr = ((String) commands.elementAt(1)).trim()
				.toUpperCase();
		...
		if (triggerStr.startsWith("HOLD"))
			...
			mob.tell("You must specify the trigger event that will cause the mouth to speak.\n\r'"
					+ triggerStr
					+ "' is not correct, but you can try TOUCH, WEAR, WIELD, HOLD, or ENTER.\n\r");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					"^S<S-NAME> invoke(s) a spell upon <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(
					mob,
					target,
					"<S-NAME> attempt(s) to invoke a spell upon <T-NAMESELF>, but the spell fizzles.");
