		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> weigh(s) the value of <T-NAMESELF> carefully.^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				if (target.value() <= 0)
					...
					str = target.name(mob)
							+ " is worth "
							+ CMLib.beanCounter().nameCurrencyShort(mob,
									(double) target.value()) + " ";
				if (mob.isMonster())
			...
			beneficialVisualFizzle(
					mob,
					target,
					"<S-NAME> weigh(s) the value of <T-NAMESELF>, looking more frustrated every second.");
