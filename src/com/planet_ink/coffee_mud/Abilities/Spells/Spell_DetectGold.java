	...
	public String metalCheck(MOB mob, Item I, Item container, StringBuffer msg) {
		...
		if (I.container() == container) {
			...
			if (msg.toString().indexOf(
					I.container().name() + " contains some sort of gold.") < 0)
				msg.append(I.container().name()
						+ " contains some sort of gold.\n\r");
		return msg.toString();
	}
	...
	public void messageTo(MOB mob) {
		...
		if ((dirs.length() != 0) || (last.length() != 0)) {
			if (dirs.length() == 0)
				...
				mob.tell("You sense golden emanations coming from "
						+ dirs.substring(2) + ", and " + last + ".");
		}
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already detecting golden things.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> gain(s) golden senses!"
									: "^S<S-NAME> incant(s) softly, and gain(s) golden senses!^?");
			...
			beneficialVisualFizzle(
					mob,
					null,
					"<S-NAME> incant(s) and open(s) <S-HIS-HER> golden eyes, but the spell fizzles.");
