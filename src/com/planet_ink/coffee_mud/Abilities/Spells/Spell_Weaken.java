...
public class Spell_Weaken extends Spell {
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats
				.setStat(CharStats.STAT_STRENGTH, (int) Math.round(CMath.div(
						affectableStats.getStat(CharStats.STAT_STRENGTH), 2.0)));
	}
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> begin(s) to look a bit stronger.");
	}
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> feel(s) weaker!");
					success = maliciousAffect(mob, target, asLevel, 0, -1);
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> incant(s) at <T-NAMESELF>, but the spell fizzles.");
