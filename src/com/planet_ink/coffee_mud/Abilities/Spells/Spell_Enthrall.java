...
public class Spell_Enthrall extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		boolean success = proficiencyCheck(
				...
				-10
						- ((target.charStats()
								.getStat(CharStats.STAT_INTELLIGENCE)) + (levelDiff * 5)),
				auto);
		if (success) {
			...
			// what happened.
			String str = auto ? ""
					: "^S<S-NAME> smile(s) powerfully at <T-NAMESELF>.^?";
			CMMsg msg = CMClass.getMsg(mob, target, this,
					CMMsg.MSG_CAST_VERBAL_SPELL, str);
		...
		if (!success)
			return maliciousFizzle(mob, target,
					"<S-NAME> smile(s) powerfully at <T-NAMESELF>, but nothing happens.");
