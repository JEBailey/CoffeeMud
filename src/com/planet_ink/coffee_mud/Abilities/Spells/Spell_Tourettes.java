	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((--plagueDown) <= 0) {
			...
			if ((target != null)
				...
				case 1:
					say = "You are a very bad "
							+ target.charStats().displayClassName() + "!";
					break;
				case 2:
					say = "I think all " + target.charStats().raceName()
							+ "s are stupid!";
					break;
				case 3:
					say = "Damn flark!";
				...
				if ((target != invoker) && (target != mob)
					...
					if (CMLib.dice().rollPercentage() > target.charStats()
						...
						mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> feel(s) different somehow...");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? ""
							: "^S<S-NAME> incant(s) rudely to <T-NAMESELF>.^?");
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> incant(s) rudely to <T-NAMESELF>, but the spell fades.");
