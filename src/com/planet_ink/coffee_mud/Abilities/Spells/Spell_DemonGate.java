...
public class Spell_DemonGate extends Spell {
			...
			if (R != null) {
				if (mob.amFollowing() != null)
					R.showOthers(
							...
							mob.amFollowing(),
							CMMsg.MSG_OK_ACTION,
							"^F^<FIGHT^><S-NAME> uses the fight to wrest itself from out of <T-YOUPOSS> control!^?%0DTo <T-YOUPOSS> great relief, it disappears back into its home plane.^</FIGHT^>^?");
				else
					R.showOthers(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> disappears back into its home plane.");
			}
			if (mob.amDead())
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> open(s) the gates of the abyss, incanting angrily.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				MOB otherMonster = mob.location().fetchInhabitant(
						"the great demonbeast$");
				MOB myMonster = determineMonster(mob, mob.phyStats().level()
						+ (getXLEVELLevel(mob) + (2 * getX1Level(mob))));
				if (otherMonster != null) {
					myMonster
							...
							.showOthers(
									...
									mob,
									CMMsg.MSG_OK_ACTION,
									"^F^<FIGHT^><S-NAME> wrests itself from out of <T-YOUPOSS> control!^</FIGHT^>^?");
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> attempt(s) to open the gates of the abyss, but fail(s).");
	...
	public MOB determineMonster(MOB caster, int level) {
		...
		newMOB.basePhyStats()
				.setLevel(level + 2 + super.getXLEVELLevel(caster));
		...
		newMOB.location()
				.showOthers(newMOB, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> tears through the fabric of reality and steps into this world!");
