...
public class Spell_DetectTraps extends Spell {
				...
				for (Iterator<Environmental> i = SK.getShop()
					...
					if (E2 instanceof Item)
						if (trapCheck((Item) E2).length() > 0)
							return P.name()
									+ " has something trapped in stock.";
				}
			}
		}
		return msg.toString();
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected != null)
			...
			} else if ((trapHere((MOB) affected, (Physical) msg.target())
					.length() > 0) && (msg.source() != msg.target())) {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already detecting traps.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> gain(s) trap sensitivities!"
									: "^S<S-NAME> incant(s) softly, and gain(s) sensitivity to traps!^?");
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> incant(s) and open(s) <S-HIS-HER> eyes, but the spell fizzles.");
