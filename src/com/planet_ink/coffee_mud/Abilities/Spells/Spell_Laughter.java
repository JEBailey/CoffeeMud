...
public class Spell_Laughter extends Spell {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (!super.tick(ticking, tickID))
			return false;
		((MOB) affected).location().show((MOB) affected, null,
				CMMsg.MSG_OK_ACTION,
				"<S-NAME> laugh(s) uncontrollably, unable to move!");
		return true;
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> tell(s) <T-NAMESELF> a magical joke.^?");
					...
					if (success)
						if (target.location() == R)
							R.show(target, null, CMMsg.MSG_OK_ACTION,
									"<S-NAME> begin(s) laughing uncontrollably, unable to move!!");
				}
			}
		} else
			return maliciousFizzle(
					mob,
					target,
					"<S-NAME> tell(s) <T-NAMESELF> a magical joke, but <T-NAME> do(es)n't think it is funny.");
