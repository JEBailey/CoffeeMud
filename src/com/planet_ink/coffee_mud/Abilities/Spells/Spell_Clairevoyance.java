	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected instanceof MOB)
			...
			if ((invoker != null)
				...
				CMMsg newAffect = CMClass.getMsg(invoker, target,
		...
		if (target == null) {
			try {
				List<MOB> targets = CMLib.map().findInhabitants(
						mob.location().getArea().getProperMap(), mob, mobName,
						10);
				if (targets.size() == 0)
					targets = CMLib.map().findInhabitants(CMLib.map().rooms(),
							mob, mobName, 10);
				if (targets.size() > 0)
		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> invoke(s) clairevoyance, calling '"
									+ mobName + "'.^?");
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to invoke clairevoyance, but fizzle(s) the spell.");
