		...
		if (success) {
			if (mob.location()
					.show(mob,
							...
							auto ? ""
									: "^S<S-NAME> wave(s) <S-HIS-HER> arms and speak(s).^?"))
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					if (mob.location().okMessage(mob, msg)) {
						...
						mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> grow(s) a pair of gills!");
						breath.beneficialAffect(mob, target, asLevel, 0);
					}
				}
		} else
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> wave(s) <S-HIS-HER> arms and speak(s), but the spell fizzles.");
