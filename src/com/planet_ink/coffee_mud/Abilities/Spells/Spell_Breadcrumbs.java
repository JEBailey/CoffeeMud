...
public class Spell_Breadcrumbs extends Spell {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if (!(affected instanceof MOB))
			return;
		MOB mob = (MOB) affected;
		if ((msg.amISource(mob)) && (trail != null)
			...
			for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
				...
				if ((adjacentRoom != null) && (newRoom.getExitInDir(d) != null)) {
					kill = false;
					t = 0;
					while (t < trail.size()) {
						if (kill)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already dropping breadcrumbs.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> attain(s) mysterious breadcrumbs."
									: "^S<S-NAME> invoke(s) the mystical breadcrumbs.^?");
			...
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke breadcrumbs, but fail(s).");
