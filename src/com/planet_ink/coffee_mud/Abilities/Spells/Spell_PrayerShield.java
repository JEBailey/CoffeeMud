	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			if (target instanceof MOB) {
				...
				if ((victim != null)
						&& (CMLib.flags()
								.domainAbilities(victim, Ability.ACODE_PRAYER)
								.size() == 0))
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob))
			...
			mob.location().show(
					...
					null,
					CMMsg.MSG_OK_VISUAL,
					"The shield around <S-NAME> blocks off "
							+ msg.tool().name() + "!");
			return false;
		}
		return super.okMessage(myHost, msg);
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> look(s) protected from prayers."
									: "^S<S-NAME> invoke(s) an anti-prayer shield around <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a shield, but fail(s).");
