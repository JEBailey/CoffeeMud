...
public class Spell_Counterspell extends Spell {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob))
				...
				&& (invoker != null)
				&& (!mob.amDead())
				&& (CMLib.dice().rollPercentage() < (70 + (2 * ((mob.phyStats()
						.level() + (2 * getXLEVELLevel(invoker()))) - msg
						.source().phyStats().level()))))) {
			mob.location().show(
					mob,
					msg.source(),
					CMMsg.MSG_OK_VISUAL,
					"The barrier around <S-NAME> dispels the "
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> feel(s) protected from spells."
									: "^S<S-NAME> invoke(s) a counterspell barrier around <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a counterspell barrier, but fail(s).");
