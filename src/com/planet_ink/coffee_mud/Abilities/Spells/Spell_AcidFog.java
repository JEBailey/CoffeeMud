	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((tickID == Tickable.TICKID_MOB) && (affected != null)
			...
			if (M.location() != castingLocation)
				...
				int damage = M.phyStats().level()
						+ super.getXLEVELLevel(invoker())
						+ (2 * super.getX1Level(invoker()));
				CMLib.combat().postDamage(invoker, M, this,
						CMLib.dice().roll(1, damage, 0), CMMsg.TYP_ACID, -1,
						"<T-NAME> sizzle(s) in the acid fog!");
		...
		if (canBeUninvoked()) {
			if ((!mob.amDead()) && (mob.location() != null))
				mob.location().show(mob, null, CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> manage(s) to escape the acid fog!");
		}
	}
		...
		if (success) {
			if (mob.location()
					.show(mob,
							...
							auto ? "A horrendous cloud of acid appears!"
									: "^S<S-NAME> incant(s) and wave(s) <S-HIS-HER> arms around.^?"))
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					CMMsg msg2 = CMClass.getMsg(mob, target, this,
							CMMsg.MSK_CAST_MALICIOUS_VERBAL | CMMsg.TYP_ACID
					...
					if ((mob.location().okMessage(mob, msg))
						...
						if ((msg.value() <= 0) && (msg2.value() <= 0)
							...
							success = maliciousAffect(
									mob,
									target,
									asLevel,
									((mob.phyStats().level()
											+ super.getXLEVELLevel(mob) + (2 * super
											.getX1Level(mob))) * 10), -1);
							target.location()
									.show(target, null, CMMsg.MSG_OK_ACTION,
											"<S-NAME> become(s) enveloped in the acid fog!");
						}
					}
				}
		} else
