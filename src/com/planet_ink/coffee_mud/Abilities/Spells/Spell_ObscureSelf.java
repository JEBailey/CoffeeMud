		...
		if ((msg.amITarget(mob)) && (msg.targetMinor() != CMMsg.TYP_DAMAGE)) {
			if ((!msg.amISource(mob))
					&& ((msg.targetMinor() == CMMsg.TYP_LOOK)
							|| (msg.targetMinor() == CMMsg.TYP_EXAMINE) || (msg
							.targetMinor() == CMMsg.TYP_READ))) {
				msg.source().tell(
						"He or she is too vague to make out any details.");
				return false;
			}
			...
			if (othersMessage != null) {
				for (int i = 0; i < stuff.length; i++) {
					...
					while (x >= 0) {
						...
						othersMessage = othersMessage.substring(0, x)
								+ stuff[i][2]
								+ othersMessage.substring(x
										+ (stuff[i][1]).length());
						x = othersMessage.indexOf(stuff[i][1]);
					}
				}
			...
			if ((!msg.amISource(mob)) && (sourceMessage != null)) {
				for (int i = 0; i < stuff.length; i++) {
					...
					while (x >= 0) {
						somethingsChanged = true;
						sourceMessage = sourceMessage.substring(0, x)
								+ stuff[i][2]
								+ sourceMessage.substring(x
										+ (stuff[i][1]).length());
						x = sourceMessage.indexOf(stuff[i][1]);
					}
				}
		...
		if (msg.amISource(mob)) {
			...
			if ((!msg.amITarget(mob)) && (targetMessage != null)) {
				for (int i = 0; i < stuff.length; i++) {
					...
					while (x >= 0) {
						somethingsChanged = true;
						targetMessage = targetMessage.substring(0, x)
								+ stuff[i][2]
								+ targetMessage.substring(x
										+ (stuff[i][0]).length());
						x = targetMessage.indexOf(stuff[i][0]);
					}
				}
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> seem(s) a bit less obscure.");
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					auto ? "^S<T-NAME> become(s) obscure!"
							: "^S<S-NAME> whisper(s) to <S-HIM-HERSELF>.^?");
			...
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> whisper(s) to <S-HIM-HERSELF>, but nothing happens.");
