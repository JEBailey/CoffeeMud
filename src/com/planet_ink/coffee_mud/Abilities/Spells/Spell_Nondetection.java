...
public class Spell_Nondetection extends Spell {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob))
				...
				&& (!mob.amDead())
				&& ((mob.fetchAbility(ID()) == null) || (proficiency() > 99) || proficiencyCheck(
						null, 0, false)))
			return false;
		return true;
	}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "An undetectable field envelopes <T-NAME>!"
									: "^S<S-NAME> invoke(s) an globe of resistance around <T-NAMESELF>.^?"));
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a resistance globe, but fail(s).");
