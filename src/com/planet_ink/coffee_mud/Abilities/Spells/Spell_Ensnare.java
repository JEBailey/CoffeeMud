		...
		if (msg.amISource(mob)) {
			...
			case CMMsg.TYP_FLEE:
				if (mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
						"<S-NAME> struggle(s) against the ensnarement.")) {
		...
		if (canBeUninvoked())
			mob.location()
					.show(mob, null, CMMsg.MSG_NOISYMOVEMENT,
							"<S-NAME> manage(s) to break <S-HIS-HER> way free of the ensnarement.");
	}
						...
						if (msg.value() <= 0) {
							...
							if (target.location() == mob.location()) {
								...
								target.location()
										.show(target, null,
												CMMsg.MSG_OK_ACTION,
												"<S-NAME> become(s) ensnared, and is unable to move <S-HIS-HER> feet!");
							}
						}
					}
				}
