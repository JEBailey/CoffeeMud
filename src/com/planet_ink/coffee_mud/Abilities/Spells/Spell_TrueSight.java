...
public class Spell_TrueSight extends Spell {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amISource((MOB) affected))
			...
			msg.addTrailerMsg(CMClass.getMsg(msg.source(), null, this,
					CMMsg.MSG_OK_VISUAL, msg.target().name() + " is truly "
							+ msg.target().Name() + ".", CMMsg.NO_EFFECT, null,
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			if (mob.isInCombat()
					&& (!CMLib.flags().canBeSeenBy(mob.getVictim(), mob)))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE>  true sight.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> gain(s) true sight!"
									: "^S<S-NAME> incant(s) softly, and gain(s) true sight!^?");
