	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (((!msg.amITarget(affected)) && (msg.tool() != affected))
				...
				|| (CMLib.law().doesHavePriviledgesHere(mob,
						msg.source().location()) && (text().toUpperCase()
						.indexOf("MALICIOUS") < 0))
				|| ((msg.target() instanceof Exit)
						&& (CMLib.law().doesHavePriviledgesInThisDirection(mob,
								msg.source().location(), (Exit) msg.target())) && (text()
						.toUpperCase().indexOf("MALICIOUS") < 0)))
			return true;
	...
	public void unInvoke() {
		if ((canBeUninvoked()) && (affected != null)) {
			if (affected instanceof Exit) {
				...
				exit.setDoorsNLocks(exit.hasADoor(), !exit.hasADoor(),
						exit.defaultsClosed(), exit.hasALock(),
						exit.hasALock(), exit.defaultsLocked());
			} else if (affected instanceof Container) {
				Container container = (Container) affected;
				container.setLidsNLocks(container.hasALid(),
						!container.hasALid(), container.hasALock(),
						container.hasALock());
			}
		}
		super.unInvoke();
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (target instanceof Exit) {
					Exit exit = (Exit) target;
					exit.setDoorsNLocks(exit.hasADoor(), false,
							exit.defaultsClosed(), exit.hasALock(), true,
							exit.defaultsLocked());
					Room R = mob.location();
					Room R2 = null;
					...
					mob.location().show(mob, target, CMMsg.MSG_OK_VISUAL,
							"<T-NAME> look(s) shut tight!");
				} else if (target instanceof Container) {
					beneficialAffect(mob, target, asLevel,
							Ability.TICKS_ALMOST_FOREVER);
					Container container = (Container) target;
					container.setLidsNLocks(container.hasALid(), false,
							container.hasALock(), true);
					mob.location().show(mob, target, CMMsg.MSG_OK_VISUAL,
							"<T-NAME> look(s) shut tight!");
				}
