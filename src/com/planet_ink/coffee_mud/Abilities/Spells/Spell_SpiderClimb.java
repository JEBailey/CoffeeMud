	...
	public void unInvoke() {
		...
		if ((canBeUninvoked()) && (!mob.amDead()) && (room != null))
			room.show(mob, null, CMMsg.MSG_OK_VISUAL,
					"<S-NAME> no longer <S-HAS-HAVE> a spidery gait.");
		super.unInvoke();
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE> spidery magic.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "^S<S-NAME> attains a climbers stance!"
									: "^S<S-NAME> invoke(s) a spidery spell upon <S-HIM-HERSELF>!^?");
