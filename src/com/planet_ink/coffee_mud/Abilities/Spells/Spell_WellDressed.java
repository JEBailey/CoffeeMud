...
public class Spell_WellDressed extends Spell {
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_CHARISMA,
				affectableStats.getStat(CharStats.STAT_CHARISMA) + dressCode);
	}
	...
	public void unInvoke() {
		 ...
		 * "<S-YOUPOSS> clothe(s) aren't quite as appealing any more.");
		 */
	}
		...
		if (mob != null) {
			if (target instanceof MOB) {
				if ((((MOB) target).isInCombat())
						&& (!((MOB) target).charStats().getCurrentClass()
								.baseClass().equalsIgnoreCase("Bard")))
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> speak(s) exquisitely to <T-NAMESELF>.^?");
			...
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> speak(s) exquisitely to <T-NAMESELF>, but nothing more happens.");
