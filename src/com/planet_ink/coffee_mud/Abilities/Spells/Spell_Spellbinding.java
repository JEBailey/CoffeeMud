...
public class Spell_Spellbinding extends Spell {
	...
	public String text() {
		...
		try {
			...
			Log.errOut("Spell_Spellbinding", "Spell bindings are corrupt for "
					+ ((affected != null) ? affected.Name() : "someone") + ".");
		}
		return super.text();
	}
		...
		if (text.length() == 0)
			...
			try {
				ByteArrayInputStream bytes = new ByteArrayInputStream(
						CMParms.fromByteList(text));
				spellbindings = (DVector) new ObjectInputStream(bytes)
						.readObject();
			} catch (Exception e) {
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		if ((msg.source() == affected)
			...
			for (int v = 0; v < spellbindings.size(); v++)
				if (((String) spellbindings.elementAt(v, 1))
						.equalsIgnoreCase(s)) {
					...
					if (!alreadyWanding)
						msg.addTrailerMsg(CMClass.getMsg(msg.source(),
								msg.target(), this, CMMsg.MASK_ALWAYS
										| CMMsg.TYP_WAND_USE, "The magic of '"
										+ s + "' swells within you!",
								CMMsg.NO_EFFECT, null, CMMsg.NO_EFFECT, null));
				}
		} else if ((msg.tool() == this) && (msg.source() == affected)
			...
			for (int v = spellbindings.size() - 1; v >= 0; v--)
				if (((String) spellbindings.elementAt(v, 1))
					...
					for (int v2 = 0; v2 < V2.size(); v2++) {
						Ability A = msg.source().fetchAbility(
								(String) V2.elementAt(v2, 1));
						int curMana = msg.source().curState().getMana();
						msg.source().curState().setMana(1000);
						if (msg.target() != null)
							A.invoke(msg.source(),
									CMParms.parse(msg.target().Name()), null,
									false, 0);
						else
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		MOB target = mob;
		Spell_Spellbinding priorBinding = (Spell_Spellbinding) target
				.fetchEffect(ID());
		if (commands.size() < 2) {
		...
		if (A != null) {
			...
			V.addElement(
					A.ID(),
					Integer.valueOf(A.usageCost(mob, true)[Ability.USAGEINDEX_MANA]));
		} else
			for (int v = 0; v < commands.size(); v++) {
				...
				if ((A == null)
						...
						|| (A.usageCost(mob, true)[Ability.USAGEINDEX_MANA] > 50)) {
					mob.tell("You can't bind '"
							+ ((String) commands.elementAt(v)) + "'.");
					return false;
				}
				V.addElement(
						A.ID(),
		...
		if (priorBinding != null)
			for (int x = 0; x < priorBinding.spellbindings.size(); x++)
				if (((String) priorBinding.spellbindings.elementAt(x, 1))
						.equalsIgnoreCase(key)) {
					thePriorKey = (DVector) priorBinding.spellbindings
							.elementAt(x, 2);
				}
		...
		for (int v = 0; v < V.size(); v++)
			for (int v2 = 0; v2 < V.size(); v2++)
				if ((v != v2)
						&& (((String) V.elementAt(v, 1)).equals(V.elementAt(v2,
								1)))) {
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? ""
									: "^S<S-NAME> shout(s) the magic of spellbinding!^?"));
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (priorBinding == null) {
					beneficialAffect(mob, target, asLevel, 0);
					priorBinding = (Spell_Spellbinding) target
							.fetchEffect(ID());
					if (priorBinding == null)
						return false;
				...
				if (thePriorKey == null)
					...
					for (int v = 0; v < V.size(); v++)
						thePriorKey.addElement(V.elementAt(v, 1),
								V.elementAt(v, 2));
				target.recoverMaxState();
			}
		} else
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> whisper(s) about spellbinding, and the magic fizzles.");
