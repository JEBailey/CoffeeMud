...
public class Spell_GhostSound extends Spell {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((tickID == Tickable.TICKID_MOB)
				&& (CMLib.dice().rollPercentage() < 10) && (affected != null)
				&& (invoker != null) && (affected instanceof Room))
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> scream(s) loudly, then fall(s) silent.^?");
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> scream(s) loudly, but then feel(s) disappointed.");
