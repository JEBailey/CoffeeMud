...
public class Spell_Disintegrate extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		for (int v = 0; v < DBs.size(); v++) {
			...
			if (DB.playerCorpse() && (!DB.mobName().equals(mob.Name()))) {
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							affectType,
							(auto ? ""
									: "^S<S-NAME> point(s) at <T-NAMESELF> and utter(s) a treacherous spell!^?")
						...
						if (((MOB) target).curState().getHitPoints() > 0)
							CMLib.combat()
									.postDamage(
											...
											"^SThe spell <DAMAGE> <T-NAME>!^?");
						if (((MOB) target).amDead())
							R.show(mob, target, CMMsg.MSG_OK_ACTION,
			...
			maliciousFizzle(
					mob,
					target,
					"<S-NAME> point(s) at <T-NAMESELF> and utter(s) a treacherous but fizzled spell!");
