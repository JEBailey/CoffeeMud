...
public class Spell_FlamingSword extends Spell {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.targetMinor() == CMMsg.TYP_DAMAGE) && (msg.tool() == affected)
			...
			CMMsg msg2 = CMClass.getMsg(msg.source(), msg.target(), affected,
					CMMsg.MSG_OK_ACTION, CMMsg.MSK_MALICIOUS_MOVE
							| CMMsg.TYP_FIRE, CMMsg.MSG_NOISYMOVEMENT, null);
			if ((room != null) && (room.okMessage(msg.source(), msg2))) {
				room.send(msg.source(), msg2);
				if (msg2.value() <= 0) {
					int flameDamage = CMLib.dice().roll(1,
							(2 + affected.basePhyStats().level()) / 2, 1);
					CMLib.combat()
							.postDamage(msg.source(), (MOB) msg.target(), null,
									flameDamage, CMMsg.TYP_FIRE,
									Weapon.TYPE_BURNING,
									name() + " <DAMAGE> <T-NAME>!");
				}
			}
	...
	public void unInvoke() {
		...
		if ((canBeUninvoked()) && (room != null))
			room.showHappens(CMMsg.MSG_OK_VISUAL, item,
					"<S-YOUPOSS> flaming sword is consumed!");
		super.unInvoke();
		if ((canBeUninvoked()) && (room != null)) {
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "^S<T-NAME> erupts into flame!"
									: "^S<S-NAME> invoke(s) a writhing flame around <T-NAMESELF>!^?");
			...
			beneficialWordsFizzle(mob, mob.location(),
					"<S-NAME> attempt(s) to invoke a flame, but cause(s) a puff of smoke.");
