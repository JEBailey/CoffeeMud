...
public class Spell_ManaBurn extends Spell {
	...
	public void adjustMana() {
		...
		if (canBeUninvoked()) {
			if (mob.curState().getMana() < curMana)
				mob.curState().adjMana(mob.curState().getMana() - curMana,
						mob.maxState());
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		boolean success = proficiencyCheck(
				mob,
				-((target.charStats().getStat(CharStats.STAT_INTELLIGENCE)) + (levelDiff * 5)),
				auto);
		if (success) {
			...
			// what happened.
			String str = auto ? ""
					: "^S<S-NAME> incant(s) hotly at <T-NAMESELF>^?";
				...
				if ((msg.value() <= 0) && (msg2.value() <= 0)) {
					...
					if (success)
						mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> seem(s) drained!");
				}
			}
		}
		if (!success)
			return maliciousFizzle(mob, target,
					"<S-NAME> incant(s) hotly at <T-NAMESELF>, but nothing happens.");
