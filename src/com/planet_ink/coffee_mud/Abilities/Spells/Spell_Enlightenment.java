...
public class Spell_Enlightenment extends Spell {
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		if (affectableStats.getCurrentClass().baseClass().equals("Fighter"))
			...
			increase = 7;
		affectableStats
				.setStat(CharStats.STAT_INTELLIGENCE,
						affectableStats.getStat(CharStats.STAT_INTELLIGENCE)
								+ increase);
	}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> educate(s) <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> look(s) at things with a new understanding!");
				beneficialAffect(mob, target, asLevel, 0);
			}
		} else
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to educate <T-NAMESELF>, but look(s) perplexed.");
