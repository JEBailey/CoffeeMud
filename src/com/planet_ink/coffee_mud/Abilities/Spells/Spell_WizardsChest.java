...
public class Spell_WizardsChest extends Spell {
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		if ((msg.target() == affected)
				&& ((msg.source() == invoker()) || (msg.source().Name()
						.equals(text()))) && (msg.sourceMessage() != null)
				&& (msg.sourceMinor() == CMMsg.TYP_SPEAK)
				&& (msg.sourceMessage().toUpperCase().indexOf("OPEN") >= 0)
				&& (affected instanceof Container)) {
			Container container = (Container) affected;
			container.setLidsNLocks(container.hasALid(), true,
					container.hasALock(), false);
			msg.addTrailerMsg(CMClass.getMsg(msg.source(), affected, null,
					CMMsg.MSG_OK_VISUAL, "<T-NAME> pop(s) open!"));
		}
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!(target instanceof Container))
				|| (!((Container) target).hasALock())
				|| (!((Container) target).hasALid())) {
		...
		if (!((Container) target).isOpen()) {
			mob.tell(target.name(mob)
					+ " must be opened before this magic will work.");
			return false;
		}
