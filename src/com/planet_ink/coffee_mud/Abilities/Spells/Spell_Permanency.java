	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (theOne == null) {
					mob.tell("There does not appear to be any of your spells on "
							+ target.name(mob)
					...
					return false;
						...
						&& (!CMLib.law().doesOwnThisProperty(mob,
								mob.location()))) {
					mob.tell("You can not make " + theOne.name()
							+ " permanent here.");
					return false;
				} else {
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> incant(s) to <T-NAMESELF>, but lose(s) patience.");
