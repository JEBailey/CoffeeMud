		...
		if (msg.amITarget(mob) && (mob.location() != null)
			...
			if (msg.targetMinor() == CMMsg.TYP_WEAPONATTACK) {
				CMMsg msg2 = CMClass.getMsg(mob, msg.source(), null,
						...
						"<S-NAME> avoid(s) the attack by <T-NAME>!");
				if ((proficiencyCheck(null,
						mob.charStats().getStat(CharStats.STAT_DEXTERITY) - 60,
						false))
						&& (!lastTime)
				...
				case CMMsg.TYP_JUSTICE:
					if ((CMath.bset(msg.targetMajor(), CMMsg.MASK_MOVE))
							&& (tool != null))
						msg2 = CMClass.getMsg(mob, msg.source(),
								CMMsg.MSG_NOISYMOVEMENT,
								"<S-NAME> avoid(s) the " + tool
										+ " from <T-NAME>.");
					break;
				case CMMsg.TYP_GAS:
					msg2 = CMClass.getMsg(mob, msg.source(),
							CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> avoid(s) the "
									+ ((tool == null) ? "noxious fumes" : tool)
									+ " from <T-NAME>.");
					break;
				case CMMsg.TYP_COLD:
					msg2 = CMClass.getMsg(mob, msg.source(),
							CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> avoid(s) the "
									+ ((tool == null) ? "cold blast" : tool)
									+ " from <T-NAME>.");
					break;
				case CMMsg.TYP_ELECTRIC:
					msg2 = CMClass.getMsg(mob, msg.source(),
							CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> avoid(s) the "
									+ ((tool == null) ? "electrical attack"
											: tool) + " from <T-NAME>.");
					break;
				case CMMsg.TYP_FIRE:
					msg2 = CMClass.getMsg(mob, msg.source(),
							CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> avoid(s) the "
									+ ((tool == null) ? "blast of heat" : tool)
									+ " from <T-NAME>.");
					break;
				case CMMsg.TYP_WATER:
					msg2 = CMClass.getMsg(mob, msg.source(),
							CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> avoid(s) the "
									+ ((tool == null) ? "weat blast" : tool)
									+ " from <T-NAME>.");
					break;
				case CMMsg.TYP_ACID:
					msg2 = CMClass.getMsg(mob, msg.source(),
							CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> avoid(s) the "
									+ ((tool == null) ? "acid attack" : tool)
									+ " from <T-NAME>.");
					break;
				case CMMsg.TYP_SONIC:
					msg2 = CMClass.getMsg(mob, msg.source(),
							CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> avoid(s) the "
									+ ((tool == null) ? "sonic attack" : tool)
									+ " from <T-NAME>.");
					break;
				case CMMsg.TYP_LASER:
					msg2 = CMClass.getMsg(mob, msg.source(),
							CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> avoid(s) the "
									+ ((tool == null) ? "laser attack" : tool)
									+ " from <T-NAME>.");
					break;
				}
				if ((msg2 != null) && (mob.location() != null)
						&& (mob.location().okMessage(mob, msg2))) {
					mob.location().send(mob, msg2);
					return false;
				}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE> the sight.");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					...
					(auto ? "<T-NAME> shout(s) combatively!"
							: "^S<S-NAME> shout(s) a combative spell!^?"));
			...
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> shout(s) combatively, but nothing more happens.");
