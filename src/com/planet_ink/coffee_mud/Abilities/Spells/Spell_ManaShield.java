...
public class Spell_ManaShield extends Spell {
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"The mana shield around <S-NAME> fades.");
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob)) && (msg.targetMinor() == CMMsg.TYP_DAMAGE)) {
			int recovery = (int) Math.round(CMath.mul((msg.value()),
					protection()));
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				mob.location()
						.show(mob,
								target,
								CMMsg.MSG_OK_VISUAL,
								CMStrings.capitalizeAndLower(adjective())
										.trim()
										+ " protective aura of mana surrounds <T-NAME>.");
				beneficialAffect(mob, target, asLevel, 0);
			}
		} else
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke" + adjective()
							+ " protective shield, but mess(es) up.");
