		...
		if ((msg.amITarget(mob))
			...
			mob.location()
					.show(mob, msg.source(), msg.tool(), CMMsg.MSG_OK_VISUAL,
							"The barrier around <S-NAME> absorbs <O-NAME> attack from <T-NAME>!");
			return false;
		}
		return super.okMessage(myHost, msg);
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> feel(s) protected."
									: "^S<S-NAME> invoke(s) an anti-bludgeoning barrier around <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke an anti-bludgeoning barrier, but fail(s).");
