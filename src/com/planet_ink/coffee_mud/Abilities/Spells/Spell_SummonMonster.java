		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> summon(s) help from the Java Plane....^?");
			if (R.okMessage(mob, msg)) {
				R.send(mob, msg);
				MOB monster = determineMonster(mob, mob.phyStats().level()
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> call(s) for magical help, but choke(s) on the words.");
	...
	public MOB determineMonster(MOB caster, int level) {
			...
			for (Enumeration e = CMClass.mobTypes(); e.hasMoreElements();) {
				...
				if ((M.basePhyStats().level() < level - range)
						...
						|| (!M.baseCharStats().getMyRace()
								.canBreedWith(M.baseCharStats().getMyRace()))
		...
		if (choices.size() > 0) {
			...
			for (int i = 1; i < choices.size(); i++) {
				...
				if (CMath.pow(level - M.basePhyStats().level(), 2) < CMath.pow(
						level - winM.basePhyStats().level(), 2))
					winM = M;
			}
			newMOB = winM;
		} else {
		...
		if (newMOB.amFollowing() != caster)
			...
			R.showOthers(newMOB, victim, CMMsg.MSG_OK_ACTION,
