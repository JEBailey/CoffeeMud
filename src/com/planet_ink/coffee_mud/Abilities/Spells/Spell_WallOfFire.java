				...
				if (!invoker.location().isContent(theWall))
					...
					for (int m = 0; m < room.numInhabitants(); m++) {
						...
						if ((mob != null) && (mob != invoker)
							...
							int damage = CMLib
									.dice()
									.roll((int) Math.round((invoker.phyStats()
											.level()
											+ super.getXLEVELLevel(invoker()) + (2.0 * super
											.getX1Level(invoker()))) / 4.0), 6,
											1);
							CMLib.combat()
									.postDamage(
											invoker,
											...
											CMMsg.MASK_MALICIOUS
													...
													| CMMsg.TYP_FIRE,
											Weapon.TYPE_BURNING,
											"The wall of fire flares and <DAMAGE> <T-NAME>!");
							if ((!mob.isInCombat())
									&& (mob.isMonster())
									&& (mob != invoker)
									&& (invoker != null)
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "A blazing wall of fire appears!"
									: "^S<S-NAME> conjur(s) up a blazing wall of fire!^?")
