...
public class Spell_FeignDeath extends Spell {
	...
	public void unInvoke() {
		...
		if ((Body != null) && (deathRoom != null)
				&& (deathRoom.isContent(Body))) {
			Body.destroy();
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amISource(mob)) && (!msg.sourceMajor(CMMsg.MASK_ALWAYS))) {
			...
			msg.source().tell(
					mob.name(msg.source()) + " doesn't seem to be here.");
			return false;
		}
		return true;
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!success) {
			return maliciousFizzle(mob, mob.location(),
					"<S-NAME> point(s) to <T-NAMESELF> and yell(s), but nothing happens.");
		}
		...
		CMMsg msg = CMClass
				.getMsg(mob,
						...
						auto ? ""
								: "^S<S-NAME> point(s) to <T-NAMESELF> and yell(s) for death!^?");
		if (mob.location().okMessage(mob, msg)) {
			...
			String msp = CMLib.protocol().msp(
					"death" + CMLib.dice().roll(1, 4, 0) + ".wav", 50);
			msg = CMClass.getMsg(target, null, null, CMMsg.MSG_OK_VISUAL,
					"^f^*^<FIGHT^>!!!!!!!!!!!!!!YOU ARE DEAD!!!!!!!!!!!!!!^</FIGHT^>^?^.\n\r"
							+ msp, CMMsg.MSG_OK_VISUAL, null,
					CMMsg.MSG_OK_VISUAL,
			...
			if (deathRoom.okMessage(target, msg)) {
				...
				Body.setDisplayText("the body of " + target.name()
						+ " lies here.");
