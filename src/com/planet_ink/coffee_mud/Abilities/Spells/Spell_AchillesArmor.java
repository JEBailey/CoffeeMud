...
public class Spell_AchillesArmor extends Spell {
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> Achilles Armor is now gone.");
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		MOB mob = (MOB) affected;
		if ((msg.amITarget(mob)) && (msg.source() != msg.target())
				&& (mob.location() != null)
				&& (mob.location().isInhabitant(msg.source()))
				&& (msg.targetMinor() == CMMsg.TYP_DAMAGE)
				&& ((msg.value()) > 0) && (!mob.amDead())) {
			int weaponType = -1;
			...
			if (weaponType != vulnerability) {
				...
				mob.location().show(
						...
						msg.source(),
						CMMsg.MSG_OK_VISUAL,
						"The armor around <S-NAME> blocks " + name
								+ " attack from <T-NAME>!");
				return false;
			}
		...
		if ((msg.tool() != null) && (msg.tool() instanceof Ability)) {
			if (msg.tool().ID().equals("Amputation")) {
				mob.location().show(mob, msg.source(), CMMsg.MSG_OK_VISUAL,
						"The armor around <S-NAME> protect(s) <T-NAME>!");
				return false;
			}
		}
		return super.okMessage(myHost, msg);
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> attain(s) Achilles Armor!"
									: "^S<S-NAME> invoke(s) Achilles Armor around <T-NAMESELF>!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				vulnerability = CMLib.dice().roll(1, Weapon.TYPE_DESCS.length,
						-1);
				beneficialAffect(mob, target, asLevel, 0);
			}
		} else
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke Achilles Armor, but fail(s).");
