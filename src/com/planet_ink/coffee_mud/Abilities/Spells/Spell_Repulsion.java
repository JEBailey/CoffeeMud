	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
						...
						if (msg.value() <= 0) {
							...
							if (target.location() == mob.location()) {
								success = maliciousAffect(
										...
										-1);
								int level = 2;
								if ((CMLib.ableMapper().qualifyingClassLevel(
										mob, this) > 0)
										&& ((adjustedLevel(mob, asLevel) - CMLib
												.ableMapper()
												.qualifyingClassLevel(mob, this)) > 10))
									level += ((adjustedLevel(mob, asLevel) - CMLib
											.ableMapper().qualifyingClassLevel(
													mob, this)) - 10) / 10;
								if (level < 2)
									level = 2;
								target.location().show(target, null,
										CMMsg.MSG_OK_ACTION,
										"<S-NAME> become(s) repelled!");
								if ((target.getVictim() != null)
										&& (target.rangeToTarget() > 0))
									target.setAtRange(target.rangeToTarget());
								else if (target.location().maxRange() < level)
									target.setAtRange(target.location()
											.maxRange());
								else
			...
			return maliciousFizzle(mob, null,
					"<S-NAME> wave(s) <S-HIS-HER> arms, but the spell fizzles.");
