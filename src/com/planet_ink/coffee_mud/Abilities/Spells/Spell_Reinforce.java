...
public class Spell_Reinforce extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!target.subjectToWearAndTear()) {
			mob.tell(target.name(mob) + " cannot be reinforced.");
			return false;
		} else if (target.usesRemaining() < 100) {
			mob.tell(target.name(mob)
					+ " must be repaired before it can be reinforced.");
			return false;
		}
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (target.usesRemaining() >= 150)
					mob.tell(target.name(mob)
							+ " cannot be reinforced further.");
				else {
					mob.location().show(mob, target, CMMsg.MSG_OK_VISUAL,
							"<T-NAME> begin(s) to glow and harden!");
