...
public class Spell_GiantStrength extends Spell {
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_STRENGTH,
				affectableStats.getStat(CharStats.STAT_STRENGTH) + increase);
	}
			...
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location()
						.show(mob, null, CMMsg.MSG_OK_VISUAL,
								"<S-YOUPOSS> muscles shrink back to their normal size.");
	}
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> grow(s) huge muscles!");
				beneficialAffect(mob, target, asLevel, 0);
			}
		} else
			return beneficialWordsFizzle(mob, target,
					"<S-NAME> speak(s) strongly to <T-NAMESELF>, but nothing more happens.");
