...
public class Spell_Immunity extends Spell {
	...
	public String displayText() {
		return "(Immunity to " + immunityName + ")";
	}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> attain(s) an immunity barrier."
									: "^S<S-NAME> invoke(s) an immunity barrier around <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke an immunity barrier, but fail(s).");
