...
public class Spell_FleshStone extends Spell {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((tickID == Tickable.TICKID_MOB) && (affected != null)
				&& (statue != null) && (affected instanceof MOB)) {
		...
		if (canBeUninvoked()) {
			...
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> flesh returns to normal.");
			if (prevState != null)
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					...
					statue.setDisplayText("a statue of " + name
							+ " stands here.");
					statue.setDescription("It`s a hard granite statue, which looks exactly like "
							+ name + ".");
					...
					mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> turn(s) into stone!!");
					success = maliciousAffect(mob, target, asLevel, (mob
					...
					if (success && (A != null)) {
						...
						((Spell_FleshStone) A).prevState = (CharState) target
								.curState().copyOf();
					}
				}
			}
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> incant(s) at <T-NAMESELF>, but the spell fades.");
