...
public class Spell_WallOfForce extends Spell {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((invoker != null) && (mob.isInCombat())
				...
				&& (mob.rangeToTarget() >= 1) && (msg.target() != null)
			...
			if (((msg.tool() != null) && (msg.tool() instanceof Ability))
					|| ((msg.targetMinor() == CMMsg.TYP_WEAPONATTACK)
							...
							&& (!((Weapon) msg.tool())
									.amWearingAt(Wearable.IN_INVENTORY)) && (((Weapon) msg
							.tool()).weaponClassification() == Weapon.CLASS_RANGED))) {
				...
				if (mob != invoker) {
					CMMsg msg2 = CMClass
							.getMsg(mob,
									...
									"^F^<FIGHT^><S-NAME> attempt(s) to penetrate the wall of force and fail(s).^</FIGHT^>^?");
	...
	public void unInvoke() {
		...
		if (canBeUninvoked()) {
			if ((theWall != null) && (invoker != null)
				...
				((Room) theWall.owner()).showHappens(CMMsg.MSG_OK_VISUAL,
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "An impenetrable wall of force appears!"
									: "^S<S-NAME> conjur(s) up a impenetrable wall of force!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				Item I = CMClass.getItem("GenItem");
				I.setName("a wall of force");
				I.setDisplayText("an impenetrable wall of force surrounds "
						+ mob.name());
