	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (!success) {
			...
			if (isBadRoom(room, mob, newRoom))
				beneficialWordsFizzle(
						mob,
						null,
						"<S-NAME> attempt(s) to invoke flagportating transportation, but fizzle(s) the spell.");
			newRoom = room;
		}

		CMMsg msg = CMClass.getMsg(mob, null, this, CMMsg.MASK_MOVE
				| verbalCastCode(mob, null, auto),
				"^S<S-NAME> invoke(s) a flagportating teleportation spell.^?");
		if (mob.location().okMessage(mob, msg)) {
			...
			for (Iterator f = h.iterator(); f.hasNext();) {
				...
				if (thisRoom.okMessage(follower, leaveMsg) && (newRoom != null)
