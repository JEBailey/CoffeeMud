...
public class Spell_BaseClanEq extends Spell {
	...
	public String name() {
		return "Enchant Clan Equipment Base Model";
	}
	...
	public boolean canBeLearnedBy(MOB teacher, MOB student) {
		if (student != null) {
			for (Enumeration<Ability> a = student.allAbilities(); a
					.hasMoreElements();) {
				Ability A = a.nextElement();
				if ((A != null) && (A instanceof Spell_BaseClanEq)) {
					teacher.tell(student.name() + " already knows '" + A.name()
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((C.getExp() < exp) || (exp < 0)) {
			mob.tell("You need " + exp + " to do that, but your "
					+ C.getGovernmentName() + " has only " + C.getExp()
					+ " experience points.");
			return false;
		}
