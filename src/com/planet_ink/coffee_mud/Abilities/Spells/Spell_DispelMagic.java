...
public class Spell_DispelMagic extends Spell {
	...
	public int castingQuality(MOB mob, Physical target) {
		if (mob != null) {
			...
			if (target == mob) {
				...
				for (final Enumeration<Ability> a = ((MOB) target)
						.personalEffects(); a.hasMoreElements();) {
					A = a.nextElement();
					if ((A != null)
							&& ((A.abstractQuality() == Ability.QUALITY_BENEFICIAL_OTHERS) || (A
									.abstractQuality() == Ability.QUALITY_BENEFICIAL_SELF))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		for (int a = 0; a < target.numEffects(); a++) {
			...
			if ((A != null)
					...
					&& (((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_SPELL)
							...
							.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_CHANT))) {
				foundSomethingAtLeast = true;
				if ((A.invoker() != null)
						&& ((A.invoker() == mob)
								|| (A.invoker().phyStats().level() <= mob
										.phyStats().level() + 5) || admin))
					revokeThis = A;
			}
		}
		...
		if (revokeThis == null) {
			if (foundSomethingAtLeast)
				mob.tell(mob, target, null,
						"The magic on <T-NAME> appears too powerful to dispel.");
			else if (auto)
				mob.tell("Nothing seems to be happening.");
			else
				mob.tell(mob, target, null,
						"<T-NAME> do(es) not appear to be affected by anything you can dispel.");
			return false;
		}
		...
		int diff = revokeThis.invoker().phyStats().level()
				- mob.phyStats().level();
		if (diff < 0)
			diff = 0;
		else
		...
		if (success) {
			...
			if (((!mob.isMonster()) && (target instanceof MOB) && (!((MOB) target)
					.isMonster()))
					|| (mob == target)
					|| (mob.getGroupMembers(new HashSet<MOB>())
							.contains(target)))
				affectType = CMMsg.MSG_CAST_VERBAL_SPELL;
			if (auto)
				affectType = affectType | CMMsg.MASK_ALWAYS;

			CMMsg msg = CMClass.getMsg(mob, target, this, affectType,
					auto ? revokeThis.name() + " is dispelled from <T-NAME>."
