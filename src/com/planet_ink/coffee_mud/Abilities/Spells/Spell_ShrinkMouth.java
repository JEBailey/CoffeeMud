...
public class Spell_ShrinkMouth extends Spell {
			...
			if (affected instanceof MOB) {
				...
				if ((mob.location() != null) && (!mob.amDead()))
					mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
							"<S-YOUPOSS> mouth returns to its normal size.");
			}
		}
		super.unInvoke();
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (!super.okMessage(myHost, msg))
			return false;
		if ((msg.targetMinor() == CMMsg.TYP_EAT) && (affected instanceof MOB)
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> cast(s) a puckering spell on <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				if ((!isJustUnInvoking) && (msg.value() <= 0)) {
					...
					if ((!auto) && (target.location() != null))
						target.location().show(mob, target,
								CMMsg.MSG_OK_VISUAL,
								"<T-YOUPOSS> mouth shrinks!");
					CMLib.utensils().confirmWearability(target);
				}
			}
		} else
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to cast a puckering spell, but fail(s).");
