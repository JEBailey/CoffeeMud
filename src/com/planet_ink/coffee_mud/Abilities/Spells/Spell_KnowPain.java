		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "A painful memory erupts!"
									: "^S<S-NAME> invoke(s) <T-YOUPOSS> most painful memories."));
			...
			if ((mob.location().okMessage(mob, msg))
				...
				int damage = 0;
				int maxDie = (target.phyStats().level() + adjustedLevel(mob,
						asLevel)) / 2;
				damage += CMLib.dice()
						.roll(maxDie,
								6,
				...
				if (target.location() == mob.location())
					CMLib.combat().postDamage(mob, target, this, damage,
							...
							"The horrible memory <DAMAGE> <T-NAME>!");
			}
		} else
