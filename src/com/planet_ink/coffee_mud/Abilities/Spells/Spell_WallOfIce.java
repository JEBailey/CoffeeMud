...
public class Spell_WallOfIce extends Spell {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((invoker != null) && (mob.isInCombat())
				&& (mob.getVictim() == invoker) && (mob.rangeToTarget() == 1)) {
			if (msg.sourceMinor() == CMMsg.TYP_ADVANCE) {
				...
				CMMsg msg2 = CMClass
						.getMsg(mob, null, w, CMMsg.MSG_WEAPONATTACK,
								"^F^<FIGHT^><S-NAME> hack(s) at the wall of ice with <O-NAME>.^</FIGHT^>^?");
				...
				if (mob.location().okMessage(mob, msg2)) {
					...
					if (amountRemaining < 0) {
						...
						for (int i = 0; i < room.numInhabitants(); i++) {
							...
							if ((M.isInCombat()) && (M.getVictim() == invoker)
									&& (M.rangeToTarget() > 0)
									&& (M.rangeToTarget() < 3) && (!M.amDead()))
								CMLib.combat()
										.postDamage(
												invoker,
												M,
												...
												CMLib.dice()
														.roll((M.phyStats()
																.level() + getXLEVELLevel(invoker())) / 2,
																6, 0),
												CMMsg.MASK_ALWAYS
														| CMMsg.TYP_COLD,
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "A mighty wall of ice appears!"
									: "^S<S-NAME> conjur(s) up a mighty wall of ice!^?");
