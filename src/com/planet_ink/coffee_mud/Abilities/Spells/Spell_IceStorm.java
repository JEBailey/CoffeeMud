	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			if (mob.location().show(
					...
					(auto ? "A ferocious ice storm appears!"
							: "^S<S-NAME> evoke(s) a ferocious ice storm!^?")
				...
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					CMMsg msg2 = CMClass.getMsg(mob, target, this,
							CMMsg.MSK_CAST_MALICIOUS_VERBAL | CMMsg.TYP_COLD
					...
					if ((mob.location().okMessage(mob, msg))
						...
						CMLib.combat().postDamage(mob, target, this, damage,
								...
								"The freezing blast <DAMAGE> <T-NAME>!");
						CMLib.combat().postDamage(
								...
								CMMsg.MASK_ALWAYS | CMMsg.TYP_COLD,
								Weapon.TYPE_FROSTING,
								"The lumps of hail <DAMAGE> <T-NAME>!"
										+ CMLib.protocol().msp("hail.wav", 40));
					}
				}
		} else
			return maliciousFizzle(mob, null,
					"<S-NAME> attempt(s) to evoke an ice storm, but the spell fizzles.");
