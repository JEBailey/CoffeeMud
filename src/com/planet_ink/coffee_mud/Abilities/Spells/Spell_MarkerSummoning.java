...
public class Spell_MarkerSummoning extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		for (int m = 0; m < oldRoom.numInhabitants(); m++) {
			...
			if (M != null) {
				inhabs.addElement(M);
				int adjustment = M.phyStats().level()
		...
		if ((success) && (inhabs.size() > 0)) {
			CMMsg msg = CMClass
					.getMsg(mob,
							null,
							this,
							CMMsg.MASK_MOVE | verbalCastCode(mob, null, auto),
							auto ? ""
									: "^S<S-NAME> summon(s) the power of <S-HIS-HER> marker energy!^?");
			if ((mob.location().okMessage(mob, msg))
				...
				for (int i = 0; i < inhabs.size(); i++) {
					...
					CMMsg leaveMsg = CMClass
							.getMsg(follower, oldRoom, this, CMMsg.MSG_LEAVE
									| CMMsg.MASK_MAGIC,
									"<S-NAME> disappear(s) in a great summoning swirl.");
				...
				for (int i = 0; i < items.size(); i++) {
					...
					oldRoom.showHappens(CMMsg.MSG_OK_VISUAL, I.name()
							+ " disappears in a summoning swirl!");
					newRoom.moveItemTo(I);
					newRoom.showHappens(CMMsg.MSG_OK_VISUAL, I.name()
