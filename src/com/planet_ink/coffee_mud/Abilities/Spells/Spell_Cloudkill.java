		...
		if (success) {
			if (mob.location()
					.show(mob,
							...
							auto ? "A horrendous green cloud appears!"
									: "^S<S-NAME> evoke(s) a horrendous green cloud.^?"))
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					if (mob.location().okMessage(mob, msg)
						...
						if (target.fetchEffect(ID()) == null) {
							...
							int midLevel = (int) Math.round(CMath.div(
									adjustedLevel(mob, asLevel), 2.0));
							if (midLevel < target.phyStats().level())
								damage = (int) Math.round(CMath.mul(damage,
										0.10));
							...
							if (target.location() == mob.location()) {
								String addOn = "";
								if ((target.charStats().getBodyPart(
										Race.BODY_LEG) > 0)
										&& (msg.value() <= 0)
										&& (msg2.value() <= 0)) {
			...
			return maliciousFizzle(mob, null,
					"<S-NAME> attempt(s) to evoke a green cloud, but the spell fizzles.");
