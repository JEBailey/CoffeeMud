...
public class Spell_Dismissal extends Spell {
	...
	public int castingQuality(MOB mob, Physical target) {
		if ((target instanceof MOB) && (((MOB) target).amFollowing() == mob)
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> point(s) at <T-NAMESELF> and utter(s) a dismissive spell!^?");
				...
				if (msg.value() <= 0) {
					if (target.getStartRoom() == null)
						...
						mob.location().show(mob, target, CMMsg.MSG_OK_ACTION,
								"<T-NAME> vanish(es) in dismissal!");
						target.getStartRoom().show(target, null,
								CMMsg.MSG_OK_VISUAL, "<S-NAME> appear(s)!");
			...
			maliciousFizzle(
					mob,
					target,
					"<S-NAME> point(s) at <T-NAMESELF> and utter(s) a dismissive but fizzled spell!");
