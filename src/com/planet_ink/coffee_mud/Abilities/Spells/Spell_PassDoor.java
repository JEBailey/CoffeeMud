...
public class Spell_PassDoor extends Spell {
	...
	public void affectPhyStats(Physical affected, PhyStats affectedStats) {
		...
		affectedStats.setDisposition(affectedStats.disposition()
		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-NAME> <S-IS-ARE> no longer translucent.");
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((!success) || (mob.fetchEffect(ID()) != null))
			beneficialVisualFizzle(mob, null,
					"<S-NAME> walk(s) " + Directions.getDirectionName(dirCode)
							+ ", but go(es) no further.");
		else if (auto) {
			CMMsg msg = CMClass.getMsg(mob, null, null,
					verbalCastCode(mob, null, auto),
					"^S<S-NAME> shimmer(s) and turn(s) translucent.^?");
			...
			CMMsg msg = CMClass.getMsg(
					...
					null,
					verbalCastCode(mob, null, auto),
					"^S<S-NAME> shimmer(s) and pass(es) "
