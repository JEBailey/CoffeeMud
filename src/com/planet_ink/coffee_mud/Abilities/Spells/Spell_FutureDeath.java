...
public class Spell_FutureDeath extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		int levelDiff = target.phyStats().level()
				- (mob.phyStats().level() + (getXLEVELLevel(mob)));
		if ((!target.mayIFight(mob))
				|| (levelDiff >= (3 + ((mob.phyStats().level() + (getXLEVELLevel(mob))) / 10)))) {
		...
		boolean success = proficiencyCheck(
				mob,
				-((target.charStats().getStat(CharStats.STAT_WISDOM) * 2) + (levelDiff * 15)),
				auto);
		if (success) {
