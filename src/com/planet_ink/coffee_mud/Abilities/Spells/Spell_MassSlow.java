		...
		if (success) {
			if (mob.location()
					.show(mob,
							...
							auto ? ""
									: "^S<S-NAME> whisper(s) and wave(s) <S-HIS-HER> arms.^?"))
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					if (CMLib.flags().canBeHeardSpeakingBy(mob, target)) {
						...
						maliciousFizzle(mob, target,
								"<T-NAME> seem(s) unaffected by the Slow spell from <S-NAME>.");
				}
		} else
			return maliciousFizzle(mob, null,
					"<S-NAME> whisper(s) a spell slowly, but the spell fizzles.");
