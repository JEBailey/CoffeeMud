...
public class Spell_RogueLimb extends Spell {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((rogueLimb != null) && (affected instanceof MOB)) {
			...
			if ((rogueLimb.amFollowing() != null)
					...
					|| (!CMLib.flags().aliveAwakeMobileUnbound((MOB) affected,
							true))
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(rogueLimb)
			...
			CMLib.combat().postDamage(rogueLimb, (MOB) affected, this,
					msg.value(), CMMsg.MASK_ALWAYS | msg.sourceCode(),
					Weapon.TYPE_NATURAL, null);
		if (msg.amISource(rogueLimb) && (msg.sourceMinor() == CMMsg.TYP_DEATH)) {
			unInvoke();
			return false;
		}
		return true;
	...
	public void unInvoke() {
		if ((affected != null) && (affected instanceof MOB))
			((MOB) affected).location().show(((MOB) affected), rogueLimb, null,
					CMMsg.MSG_OK_ACTION,
					"<S-NAME> gain(s) control of <T-NAMESELF>.");
		if (rogueLimb != null) {
			rogueLimb.destroy();
			rogueLimb = null;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> lose(s) control of <T-HIS-HER> limb!"
									: "^S<S-NAME> invoke(s) a powerful spell upon <T-NAMESELF>!^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				if (msg.value() <= 0) {
					...
					if (limbs.size() == 0)
						...
						limb = (Race.BODYPARTSTR[((Integer) limbs
								.elementAt(CMLib.dice().roll(1, limbs.size(),
										-1))).intValue()]).toLowerCase();
					rogueLimb = CMClass.getMOB("GenMob");
					rogueLimb.setName(target.name() + "'s " + limb);
					rogueLimb.setDisplayText(rogueLimb.name()
							+ " is misbehaving here.");
					rogueLimb.basePhyStats().setAttackAdjustment(
							target.phyStats().attackAdjustment());
					rogueLimb.basePhyStats()
							.setArmor(target.phyStats().armor());
					...
					rogueLimb.basePhyStats().setSensesMask(
			...
			mob.location()
					.show(mob,
							target,
							CMMsg.MSG_OK_ACTION,
							"^S<S-NAME> invoke(s) at <T-NAMESELF>, causing <T-NAME> to twitch, and nothing more.");
