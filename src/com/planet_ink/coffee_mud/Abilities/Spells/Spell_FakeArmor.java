...
public class Spell_FakeArmor extends Spell {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if ((affected != null) && (!notAgainThisRound)
				&& (msg.target() != null)
			...
			msg.addTrailerMsg(CMClass.getMsg((MOB) msg.target(), null,
					CMMsg.MSG_OK_VISUAL, affected.name()
							+ " absorbs some of the damage done to <S-NAME>."));
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				armor.basePhyStats().setDisposition(
						armor.basePhyStats().disposition() | PhyStats.IS_BONUS);
				...
				mob.addItem(armor);
				mob.location().show(mob, null, armor, CMMsg.MSG_OK_ACTION,
