	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			mob.location().show(
					...
					(auto ? "" : "^S<S-NAME> invoke(s) a thunderous spell.^?")
				...
				if (CMLib.flags().isInFlight(target))
					...
					if (msg.value() <= 0) {
						if (target.charStats().getBodyPart(Race.BODY_LEG) > 0) {
							success = maliciousAffect(mob, target, asLevel, 2,
									-1);
							if (success) {
			...
			return maliciousFizzle(mob, null,
					"<S-NAME> attempt(s) to invoke a thunderous spell, but the spell fizzles.");
