	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if (msg.amITarget(mob)) {
			if ((msg.targetMinor() == CMMsg.TYP_DAMAGE)
					&& (mob.rangeToTarget() == 0) && (msg.source() != mob)
				...
				if (source.location().okMessage(mob, msg2)) {
					...
					if ((msg2.value() <= 0) && (msg.value() > 3)) {
						int damage = CMLib.dice().roll(1,
								(getXLEVELLevel(mob) + msg.value()) / 3, 0);
						CMLib.combat()
								.postDamage(
										mob,
										source,
										...
										"The bubble around <S-NAME> <DAMAGES> <T-NAME>!");
					}
				}
			}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> <T-IS-ARE> surrounded by a Kinetic Bubble!"
									: "^S<S-NAME> invoke(s) a Kinetic Bubble around <T-NAMESELF>!^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a Kinetic Bubble, but fail(s).");
