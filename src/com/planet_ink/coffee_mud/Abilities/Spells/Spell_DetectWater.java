...
public class Spell_DetectWater extends Spell {
	...
	public String waterCheck(MOB mob, Item I, Item container, StringBuffer msg) {
		...
		} else if ((I.container() != null)
				&& (I.container().container() == container))
			if (msg.toString().indexOf(
					I.container().name() + " contains some sort of liquid.") < 0)
				msg.append(I.container().name()
	...
	public void messageTo(MOB mob) {
		...
		if ((dirs.length() != 0) || (last.length() != 0)) {
			if (dirs.length() == 0)
				...
				mob.tell("Water smells are coming from " + dirs.substring(2)
						+ ", and " + last + ".");
		}
	}
		...
		if ((affected != null)
			...
			if ((msg.tool() != null) && (msg.tool().ID().equals(ID()))) {
				...
				CMMsg msg2 = CMClass.getMsg(msg.source(), msg.target(), this,
						CMMsg.MSG_LOOK, CMMsg.NO_EFFECT, CMMsg.NO_EFFECT, null);
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> <S-IS-ARE> already detecting liquid things.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> gain(s) liquid sensitivities!"
									: "^S<S-NAME> incant(s) softly, and gain(s) liquid sensitivities!^?");
			...
			beneficialVisualFizzle(
					mob,
					null,
					"<S-NAME> incant(s) and open(s) <S-HIS-HER> liquified eyes, but the spell fizzles.");
