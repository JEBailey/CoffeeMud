...
public class Spell_Cogniportive extends Spell {
	...
	public String establishHome(MOB mob, Item me, boolean beLoose) {
		...
		try {
			mobInventory = CMLib.map().findInventory(CMLib.map().rooms(), null,
					srchStr, 10);
			for (Item I : mobInventory) {
				Environmental owner = I.owner();
				Room room = CMLib.map().roomLocation(owner);
				if ((owner instanceof MOB) && (((MOB) owner).isMonster())
		...
		try {
			List<Environmental> all = CMLib.map().findShopStockers(
					CMLib.map().rooms(), mob, srchStr, 10);
			for (Environmental O : all)
				if (O instanceof ShopKeeper) {
					...
					if ((E instanceof Item) && ((beLoose) || me.sameAs(E))
							&& (CMLib.law().getLandTitle(room) == null))
						return CMLib.map().getExtendedRoomID(room);
				}
		} catch (NoSuchElementException nse) {
		...
		try {
			...
			List<Item> targets = CMLib.map().findRoomItems(CMLib.map().rooms(),
					mob, me.Name(), false, 10);
			for (Item I : targets) {
				Room R = CMLib.map().roomLocation(I);
				if ((R != null) && ((beLoose) || me.sameAs(I))
	...
	public void waveIfAble(MOB mob, Environmental afftarget, Item me) {
		if ((mob != null) && (mob.isMine(me)) && (mob.location() != null)
				&& (me != null)) {
			if (text().length() == 0)
			...
			if ((home == null) || (!CMLib.flags().canAccess(mob, home)))
				mob.location().showHappens(CMMsg.MSG_OK_VISUAL,
						"Strange fizzled sparks fly from " + me.name() + ".");
			else {
				...
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					CMMsg enterMsg = CMClass.getMsg(follower, home, this,
							CMMsg.MSG_ENTER, null, CMMsg.MSG_ENTER, null,
							CMMsg.MSG_ENTER,
							"<S-NAME> appears in a puff of smoke.");
						...
						if ((V.size() >= 2)
							...
							if ((str.length() > 0)
									&& ((CMLib.english().containsString(
											affected.name(), str) || CMLib
											.english()
											.containsString(
													affected.displayText(), str)))) {
								...
								if (trailers != null)
									for (CMMsg msg2 : trailers)
										if (msg2.targetMinor() == CMMsg.TYP_WAND_USE)
											alreadyWanding = true;
								if (!alreadyWanding)
									msg.addTrailerMsg(CMClass.getMsg(msg
											.source(), affected, msg.target(),
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target == null) {
			...
			if (str.equals("MONEY") || str.equals("GOLD")
					|| str.equals("COINS"))
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				mob.location().show(mob, target, CMMsg.MSG_OK_ACTION,
						"<T-NAME> glow(s) softly!");
				beneficialAffect(mob, target, asLevel, 1000);
				A = target.fetchEffect(ID());
				if (A != null) {
					String home = ((Spell_Cogniportive) A).establishHome(mob,
							target, false);
					if (home.length() == 0)
						home = ((Spell_Cogniportive) A).establishHome(mob,
								target, true);
					A.setMiscText(home);
				}
