...
public class Spell_LowerResists extends Spell {
	...
	public String name() {
		return "Lower Resistance";
	}

	public String displayText() {
		return "(Lowered Resistances)";
	}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "A shimmering unresistable field appears around <T-NAMESELF>."
									: "^S<S-NAME> invoke(s) a shimmering unresistable field around <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				amount = ((mob.phyStats().level() + (2 * getXLEVELLevel(mob))) - target
						.phyStats().level()) * 5;
				if (amount < 5)
					amount = 5;
			...
			maliciousFizzle(
					mob,
					target,
					"<S-NAME> attempt(s) to invoke lowered resistances on <T-NAMESELF>, but fail(s).");
