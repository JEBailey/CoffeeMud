		...
		if (affected instanceof MOB) {
			...
			if ((msg.amITarget(mob))
					...
					&& (CMath.bset(((Ability) msg.tool()).flags(),
							Ability.FLAG_SUMMONING)) && (!mob.amDead())) {
				msg.source()
						.location()
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if ((target instanceof Room)
					...
					if (target instanceof Room) {
						Spell_SummoningWard A = (Spell_SummoningWard) target
								.fetchEffect(ID());
						if (A != null)
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a summoning ward, but fail(s).");
