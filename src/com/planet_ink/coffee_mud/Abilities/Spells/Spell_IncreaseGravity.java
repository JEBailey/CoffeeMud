...
public class Spell_IncreaseGravity extends Spell {
	...
	public boolean tick(Tickable ticking, int tickID) {
		if (!super.tick(ticking, tickID))
			return false;
		else if ((affected != null) && (affected instanceof Room)
				&& (invoker != null)) {
		...
		if (canBeUninvoked()) {
			if (affected instanceof Room) {
				...
				if ((mob.location() != null)
						&& (mob.location() != gravityRoom()))
					mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
							"Your weight returns to normal..");
			}
		}
		super.unInvoke();
	}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected != null) && (affected instanceof MOB)) {
			if ((((MOB) affected).location() != gravityRoom())
					|| ((gravityRoom() != null) && (gravityRoom().fetchEffect(
							ID()) == null))) {
				unInvoke();
				return true;
			}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(mob, null, null,
					"Gravity has already been increased here!");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), (auto ? "G"
							: "^S<S-NAME> speak(s) and wave(s) and g")
							+ "ravity begins to increase!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				theGravityRoom = mob.location();
				beneficialAffect(mob, mob.location(), asLevel,
						adjustedLevel(mob, asLevel));
			}
		} else
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> speak(s) heavily, but the spell fizzles.");
