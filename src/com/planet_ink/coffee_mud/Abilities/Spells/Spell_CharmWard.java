		...
		if (affected instanceof MOB) {
			...
			if ((msg.amITarget(mob))
					...
					&& (CMath.bset(((Ability) msg.tool()).flags(),
							Ability.FLAG_CHARMING)) && (!mob.amDead())) {
		...
		if (mob != null) {
			if (target instanceof MOB) {
				...
				if ((victim != null)
						&& (CMLib
								.flags()
								.flaggedAbilities(victim, Ability.FLAG_CHARMING)
								.size() > 0))
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> seem(s) magically protected."
									: "^S<S-NAME> invoke(s) a charm ward upon <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a charm ward, but fail(s).");
