...
public class Spell_MagicalAura extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell("There is already a magical aura around "
					+ target.name(mob) + ".");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "A magical aura appears around <T-NAME>."
									: "^S<S-NAME> invoke(s) a magical aura around <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a magical aura, but fail(s).");
