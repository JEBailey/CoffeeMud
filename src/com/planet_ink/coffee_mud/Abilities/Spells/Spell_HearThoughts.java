		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> concentrate(s) and listen(s) carefully!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				final List<Room> rooms = CMLib.tracking()
						.getRadiantRooms(mob.location(),
								new TrackingLibrary.TrackingFlags(), 50);
				...
				for (MOB target : mobs) {
					...
					else if (target.charStats().getStat(
							CharStats.STAT_INTELLIGENCE) >= 13)
					...
					if (target.charStats().getStat(CharStats.STAT_WISDOM) >= 18)
						...
						adjective += "wise, ";
					mob.tell("Regarding " + target.Name() + ", a " + adjective
							+ target.charStats().getMyRace().name() + " "
							+ target.charStats().getCurrentClass().name()
							+ " at " + room.displayText(mob) + ":");
					StringBuilder thoughts = new StringBuilder("");
					LegalBehavior LB = CMLib.law().getLegalBehavior(
							target.location());
					...
					for (Enumeration<Behavior> b = target.behaviors(); b
							.hasMoreElements();) {
			...
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> concentrate(s), but look(s) frustrated.");
