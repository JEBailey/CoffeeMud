...
public class Spell_Torture extends Spell {
		...
		if (canBeUninvoked())
			if ((!mob.amDead()) && (mob.location() != null))
				mob.location().show(mob, null, CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> manage(s) to survive the torture.");
	}
	...
	public void cryOut(MOB mob) {
			...
			case 11:
				if (mob.getLiegeID().length() > 0)
					mob.location().show(
							...
							null,
							CMMsg.MSG_SPEAK,
							"<S-NAME> admits that " + mob.getLiegeID()
									+ " is <S-HIS-HER> liege.");
				else
					roll = CMLib.dice().roll(1, 10, 0);
				break;
			case 12: {
				if (!mob.clans().iterator().hasNext())
					...
					if (C == null)
						...
						if (V.size() > 0) {
							String name = V.get(CMLib.dice().roll(1, V.size(),
									-1)).name;
			...
			case 14:
				if (!CMSecurity.isDisabled(CMSecurity.DisFlag.EXPERIENCE)
					...
					mob.location().show(
							...
							null,
							CMMsg.MSG_SPEAK,
							"<S-NAME> mutters that <S-HE-SHE> scored "
									+ mob.getExperience()
									+ " experience points.");
				break;
			case 15: {
			...
			case 16:
				if (mob.numAllAbilities() < 1)
					...
					mob.location().show(
							...
							null,
							CMMsg.MSG_SPEAK,
							"<S-NAME> admit(s) that <S-HE-SHE> knows "
									+ A.name() + " at " + A.proficiency()
									+ "%.");
				}
				break;
			}
		...
		case 1:
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> struggle(s) against the pain.");
			break;
		case 2:
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
		...
		case 7:
			mob.location()
					.show(mob, null, CMMsg.MSG_OK_ACTION,
							"<S-NAME> shake(s) in pain from <S-HIS-HER> head to <S-HIS-HER> feet.");
			break;
		case 8:
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> gasp(s) for air.");
			break;
		case 9:
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> shiver(s) in pain.");
			break;
		case 10:
			mob.location().show(mob, null, CMMsg.MSG_OK_ACTION,
					"<S-NAME> cr(ys) in anticipation of pain!");
			break;
		}
	}

	public void executeMsg(Environmental host, CMMsg msg) {
		if ((msg.amITarget(affected)) && (text().equalsIgnoreCase("HITONLY"))
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((!text().equalsIgnoreCase("HITONLY")) && (affected instanceof MOB))
			cryOut((MOB) affected);
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> whisper(s) a torturous spell to <T-NAMESELF>.^?");
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> whisper(s) a torturous spell to <T-NAMESELF>, but the spell fades.");
