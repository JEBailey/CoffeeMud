		...
		if (canBeUninvoked()) {
			if (item.amWearingAt(Wearable.WORN_FLOATING_NEARBY)) {
				mob.location().show(
						...
						item,
						CMMsg.MSG_OK_VISUAL,
						"<T-NAME> floating near <S-NAME> now floats back "
								+ ((wasntMine) ? "down to the ground"
										: "into <S-HIS-HER> hands."));
				item.unWear();
			}
			if (wasntMine)
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> begin(s) to float around."
									: "^S<S-NAME> invoke(s) a floating disc underneath <T-NAMESELF>.^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				beneficialAffect(
						...
						target,
						asLevel,
						(mob.phyStats().level() + (2 * getXLEVELLevel(mob))) * 30);
				mob.recoverPhyStats();
				mob.recoverMaxState();
				mob.recoverCharStats();
			}
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a floating disc, but fail(s).");
