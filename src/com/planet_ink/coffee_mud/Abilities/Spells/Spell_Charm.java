	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		boolean success = proficiencyCheck(
				...
				-25
						- ((target.charStats().getStat(
								CharStats.STAT_INTELLIGENCE) * 2) + (levelDiff * 15)),
				auto);
		if (success) {
			...
			// what happened.
			String str = auto ? ""
					: "^S<S-NAME> smile(s) and wink(s) at <T-NAMESELF>.^?";
			CMMsg msg = CMClass.getMsg(mob, target, this,
					CMMsg.MSG_CAST_SOMANTIC_SPELL, str);
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				if (msg.value() <= 0) {
					success = maliciousAffect(mob, target, asLevel, -levelDiff,
							CMMsg.MASK_MALICIOUS | CMMsg.MSK_CAST_SOMANTIC
									| CMMsg.TYP_MIND
		...
		if (!success)
			return maliciousFizzle(mob, target,
					"<S-NAME> smile(s) and wink(s) at <T-NAMESELF>, but nothing happens.");
