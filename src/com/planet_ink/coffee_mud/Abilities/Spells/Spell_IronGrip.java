		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.location().show(mob, null, CMMsg.MSG_OK_VISUAL,
						"<S-YOUPOSS> weapon hand becomes flesh again.");
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected != null) && (affected instanceof MOB)) {
			...
			if ((msg.amITarget(mob)) && (msg.tool() != null)
				...
				mob.location()
						.show(msg.source(), mob, CMMsg.MSG_OK_ACTION,
								"<S-NAME> attempt(s) to disarm <T-NAME>, but the grip is too strong!");
				return false;
				...
				mob.location()
						.show(mob,
								...
								msg.target(),
								CMMsg.MSG_OK_ACTION,
								"<S-NAME> attempt(s) to let go of <O-NAME>, but <S-HIS-HER> grip is too strong!");
				if ((!mob.isInCombat())
						&& (CMath.bset(mob.getBitmap(), MOB.ATT_AUTODRAW))) {
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> watch(es) <T-HIS-HER> weapon hand turn to iron!"
									: "^S<S-NAME> invoke(s) a spell on <T-NAMESELF> and <T-HIS-HER> weapon hand turns into iron!^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a spell, but fail(s).");
