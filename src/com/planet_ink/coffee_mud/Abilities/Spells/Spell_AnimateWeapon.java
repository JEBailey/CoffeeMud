		...
		if ((affected != null) && (affected instanceof Item)
			...
			if (invoker().isInCombat()) {
				boolean isHit = (CMLib.combat().rollToHit(CMLib.combat()
						.adjustedAttackBonus(invoker(), invoker().getVictim())
						+ ((Item) affected).phyStats().attackAdjustment(),
						CMLib.combat().adjustedArmor(invoker().getVictim()), 0));
				if ((!isHit) || (!(affected instanceof Weapon)))
					invoker().location().show(invoker(), invoker().getVictim(),
							affected, CMMsg.MSG_OK_ACTION,
				...
				case 1:
					invoker().location().showHappens(CMMsg.MSG_OK_VISUAL,
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				if (mob.isMine(target))
					mob.location().show(mob, target, CMMsg.MSG_DROP,
							"<T-NAME> flies out of <S-YOUPOSS> hands!");
				else
					mob.location().show(mob, target, CMMsg.MSG_OK_ACTION,
							"<T-NAME> starts flying around!");
