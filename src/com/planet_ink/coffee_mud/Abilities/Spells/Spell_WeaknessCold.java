		...
		if ((affecting() != null) && (affecting() instanceof MOB)) {
			...
			if (room != null) {
				if ((room.getArea().getClimateObj().weatherType(room) == Climate.WEATHER_WINDY)
						...
						&& (CMLib.dice().rollPercentage() > M.charStats()
								.getSave(CharStats.STAT_SAVE_COLD)))
					CMLib.combat().postDamage(
							...
							CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS
									| CMMsg.TYP_COLD, Weapon.TYPE_FROSTING,
							"The cold biting wind <DAMAGE> <T-NAME>!");
					...
					CMLib.combat().postDamage(
							...
							CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS
									| CMMsg.TYP_COLD, Weapon.TYPE_FROSTING,
							"The blistering snow <DAMAGE> <T-NAME>!");
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "A shimmering frost absorbing field appears around <T-NAMESELF>."
									: "^S<S-NAME> invoke(s) a shimmering frost absorbing field around <T-NAMESELF>.^?");
			...
			maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke weakness to cold, but fail(s).");
