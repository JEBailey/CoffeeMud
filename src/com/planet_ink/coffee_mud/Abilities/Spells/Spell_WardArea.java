...
public class Spell_WardArea extends Spell implements Trap {
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(affected)) && (!msg.amISource(invoker()))) {
	...
	public void spring(MOB mob) {
		...
		if (CMLib.dice().rollPercentage() < mob.charStats().getSave(
			...
			mob.location().show(mob, affected, this, CMMsg.MSG_OK_ACTION,
					"<S-NAME> avoid(s) a magical ward trap.");
		else {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((target.fetchEffect(this.ID()) != null) || (givenTarget != null)) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "A ward trap has already been set here!");
		...
		if (success) {
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> attempt(s) to set a magic trap, but fail(s).");
