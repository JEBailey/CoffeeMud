...
public class Spell_Blademouth extends Spell {
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		if ((msg.sourceMinor() == CMMsg.TYP_SPEAK)
			...
			try {
				CMLib.combat()
						.postDamage(invoker, msg.source(), this,
								msg.source().maxState().getHitPoints() / 10,
								CMMsg.MASK_ALWAYS | CMMsg.TYP_CAST_SPELL,
								Weapon.TYPE_SLASHING,
								"The blades in <T-YOUPOSS> mouth <DAMAGE> <T-HIM-HER>!");
			} finally {
				noRecurse = false;
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.charStats().getMyRace().bodyMask()[Race.BODY_MOUTH] <= 0) {
			if (!auto)
				mob.tell("There is no mouth on " + target.name(mob)
						+ " to fill with blades!");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "!"
									: "^S<S-NAME> invoke(s) a sharp spell upon <T-NAMESELF>"));
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> incant(s) sharply at <T-NAMESELF>, but flub(s) the spell.");
