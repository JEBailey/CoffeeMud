	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(mob))
			...
			mob.location().show(
					...
					msg.tool(),
					CMMsg.MSG_OK_VISUAL,
					"The barrier around <S-NAME> absorbs the "
							+ ((AmmunitionWeapon) msg.tool()).ammunitionType()
							+ " from <T-NAME>!");
			return false;
		}
		return super.okMessage(myHost, msg);
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> feel(s) absorbantly protected."
									: "^S<S-NAME> invoke(s) a non-porous barrier of protection around <T-NAMESELF>.^?");
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke a non-porous barrier, but fail(s).");
