		...
		if (success) {
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), auto ? ""
							: "^S<S-NAME> incant(s) softly to <T-NAMESELF>!^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				if (target.session() != null)
					for (Session S1 : CMLib.sessions().localOnlineIterable())
						if (target.session().isBeingSnoopedBy(S1))
							str.append(S1.mob().name()
									+ " is snooping on <T-NAME>.  ");
				...
				if ((A != null) && (A.invoker() != null))
					str.append(A.invoker().name()
							+ " is listening to <T-NAME>.");
				...
				CMLib.commands().postSay(mob, target, str.toString(), false,
						false);
			}
		} else
			beneficialVisualFizzle(mob, target,
					"<S-NAME> incant(s) to <T-NAMESELF>, but the spell fizzles.");
