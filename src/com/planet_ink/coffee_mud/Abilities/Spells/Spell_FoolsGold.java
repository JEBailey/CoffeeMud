...
public class Spell_FoolsGold extends Spell {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((affected != null) && (affected instanceof Item)) {
			...
			else if ((msg.tool() != null) && (msg.tool() == affected)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if ((commands.size() == 0)
				|| (CMath.s_int(CMParms.combine(commands, 0)) == 0)) {
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				mob.addItem(gold);
				mob.location().show(mob, null, gold, CMMsg.MSG_OK_ACTION,
						"Suddenly, <S-NAME> hold(s) <O-NAME>.");
				destroyOnNextTick = false;
			...
			beneficialVisualFizzle(
					mob,
					null,
					"<S-NAME> wave(s) <S-HIS-HER> arms around dramatically, but fizzle(s) the spell.");
