...
public class Spell_TeleportObject extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.amWearingAt(Wearable.IN_INVENTORY)) {
			mob.tell("You seem to be wearing or holding the item '"
					+ objectName + "'.");
			return false;
		}
		...
		if (searchWhat == null)
			searchWhat = "ERIPM";
		String destinationString = CMParms.combine(commands, 1).trim()
				.toUpperCase();
		List<Room> candidates = CMLib.map().findWorldRoomsLiberally(mob,
				destinationString, searchWhat, 10, 600000);
		if (candidates.size() == 0) {
			mob.tell("You don't know of a place called '"
					+ destinationString.toLowerCase() + "'.");
			return false;
		}
		...
		while ((tries < 20) && (newRoom == null)) {
			newRoom = candidates.get(CMLib.dice()
					.roll(1, candidates.size(), -1));
		...
		CMMsg msg = CMClass.getMsg(mob, target, this,
				...
				"^S<S-NAME> invoke(s) a teleportation spell upon <T-NAME>.^?");
		if (oldRoom.okMessage(mob, msg)) {
			oldRoom.send(mob, msg);
			newRoom.bringMobHere(mob, false);
			target.unWear();
			success = CMLib.commands()
					.postDrop(mob, target, true, false, false)
					&& (!mob.isMine(target));
			oldRoom.bringMobHere(mob, false);
			if (success) {
				oldRoom.show(mob, target, null, CMMsg.MSG_OK_VISUAL,
						"<T-NAME> vanishes!");
				newRoom.showOthers(mob, target, null, CMMsg.MSG_OK_VISUAL,
						"<T-NAME> appear(s) out of nowhere!");
			} else
