		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "A floating eye appears and begins moving around."
									: "^S<S-NAME> invoke(s) a floating eye and begin(s) chanting directions!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				Room R = mob.location();
				MOB eyeM = CMClass.getMOB("StdMOB");
				eyeM.basePhyStats().setLevel(1);
				eyeM.basePhyStats().setDisposition(
						eyeM.basePhyStats().disposition() | PhyStats.IS_FLYING);
				eyeM.basePhyStats().setSensesMask(
						eyeM.basePhyStats().sensesMask()
				...
				eyeM.addNonUninvokableEffect(CMClass
			...
			return beneficialVisualFizzle(mob, null,
					"<S-NAME> attempt(s) to invoke something, but fail(s).");
