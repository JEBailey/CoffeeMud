...
public class Spell_ClarifyScroll extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (((Scroll) target).usesRemaining() > ((Scroll) target).getSpells()
				.size()) {
		...
		if (success) {
			...
			if (mob.location().okMessage(mob, msg)) {
				...
				mob.location().show(mob, target, CMMsg.MSG_OK_VISUAL,
						"The words on <T-NAME> become more definite!");
				((Scroll) target).setUsesRemaining(((Scroll) target)
						.usesRemaining() + 1);
			}
