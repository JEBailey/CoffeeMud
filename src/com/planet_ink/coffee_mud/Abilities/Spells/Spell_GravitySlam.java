		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? ""
									: "^S<S-NAME> incant(s) and point(s) at <T-NAMESELF>!^?"));
			if (mob.location().okMessage(mob, msg)) {
				...
				int damage = 0;
				int maxDie = (int) Math
						.round((adjustedLevel(mob, asLevel) + (2.0 * super
								.getX1Level(mob))) / 2.0);
				...
				if (!CMLib.flags().isInFlight(target))
					mob.location()
							.show(target, null, CMMsg.MSG_OK_ACTION,
									"<S-NAME> <S-IS-ARE> hurled up into the air and **SLAMMED** back down!");
				else
					mob.location()
							.show(target,
									null,
									CMMsg.MSG_OK_ACTION,
									"<S-NAME> <S-IS-ARE> hurled even higher into the air and **SLAMMED** back down!");

				if (target.location() == mob.location())
					CMLib.combat().postDamage(mob, target, this, damage,
							CMMsg.MASK_ALWAYS | CMMsg.TYP_JUSTICE,
							Weapon.TYPE_BASHING, "The fall <DAMAGE> <T-NAME>!");
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> incant(s) and point(s) at <T-NAMESELF>, but flub(s) the spell.");
