...
public class Spell_Feeblemind extends Spell {
	...
	public void affectCharStats(MOB affected, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_SAVE_MIND,
				affectableStats.getStat(CharStats.STAT_SAVE_MIND) - 25);
	}
			...
			if ((R.okMessage(mob, msg)) && (R.okMessage(mob, msg2))) {
				...
				if ((msg.value() <= 0) && (msg2.value() <= 0)) {
					R.show(target, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> begin(s) to feel a bit stupid.");
					success = maliciousAffect(mob, target, asLevel, 0, -1);
				}
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> cast(s) to <T-NAMESELF>, but the spell fizzles.");
