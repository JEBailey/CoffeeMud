		...
		if ((affected != null) && (affected instanceof Room)) {
			...
			case CMMsg.TYP_FLEE:
				if (msg.source() == invoker)
					msg.source().tell(
							"You cannot travel beyond the time stopped area.");
				else
					msg.source().tell(
							"Nothing just happened.  You didn't do that.");
				return false;
			default:
				if ((msg.source() == invoker)
						&& (msg.target() != invoker)
						...
						&& ((CMath
								.bset(msg.sourceMajor(), CMMsg.MASK_MALICIOUS))
								|| (CMath.bset(msg.targetMajor(),
										CMMsg.MASK_MALICIOUS)) || (CMath.bset(
					...
					msg.source()
							.tell("Time is stopped. Nothing just happened.  You didn't do that.");
					return false;
				}
			}
		}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (success) {
			...
			CMMsg msg = CMClass.getMsg(mob, target, this,
					verbalCastCode(mob, target, auto), (auto ? "T"
							: "^S<S-NAME> speak(s) and gesture(s) and t")
							+ "ime suddenly STOPS!^?");
