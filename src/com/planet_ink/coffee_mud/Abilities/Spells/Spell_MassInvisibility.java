		...
		if (mob != null) {
			if (target instanceof MOB) {
				if ((((MOB) target).isInCombat())
						&& (((MOB) target).getGroupMembers(new HashSet<MOB>())
								.size() == 0))
		...
		if (success) {
			if (mob.location()
					.show(mob,
							...
							auto ? ""
									: "^S<S-NAME> wave(s) <S-HIS-HER> arms and speak(s) softly.^?"))
			...
			return beneficialWordsFizzle(mob, null,
					"<S-NAME> wave(s) <S-HIS-HER> arms and speak(s) softly, but the spell fizzles.");
