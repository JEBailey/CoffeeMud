		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> get(s) shoved back!"
									: "<S-NAME> incant(s) and shove(s) at <T-NAMESELF>.");
			if ((mob.location().okMessage(mob, msg))
				...
				if ((msg.value() <= 0) && (target.location() == mob.location())) {
					...
					CMMsg enterMsg = CMClass.getMsg(
							...
							null,
							CMMsg.MSG_ENTER,
							"<S-NAME> fly(s) in from "
									+ Directions
											.getFromDirectionName(Directions
													.getOpDirectionCode(dir))
									+ ".");
					CMMsg leaveMsg = CMClass.getMsg(target, thisRoom, this,
							CMMsg.MSG_LEAVE | CMMsg.MASK_MAGIC,
							"<S-NAME> <S-IS-ARE> shoved forcefully into the air and out "
									+ Directions.getInDirectionName(dir) + ".");
					if (thisRoom.okMessage(target, leaveMsg)
							&& newRoom.okMessage(target, enterMsg)) {
			...
			return maliciousFizzle(mob, null,
					"<S-NAME> incant(s), but nothing seems to happen.");
