		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> analyze(s) the nature of <T-NAMESELF> carefully.^?");
			if (mob.location().okMessage(mob, msg)) {
				...
				if (target instanceof DeadBody)
					str.append("It is a corpse of a "
							+ ((DeadBody) target).charStats().getMyRace()
									.name() + ".  ");
				...
				str.append("It is made of "
						+ RawMaterial.CODES.NAME(target.material())
								.toLowerCase() + ".  ");
				Command C = CMClass.getCommand("Affect");
				try {
					String affectStr = C.executeInternal(mob, 0, target)
							.toString();
				...
				if (mob.isMonster())
					CMLib.commands().postSay(mob, null, str.toString().trim(),
							false, false);
				else
			...
			beneficialVisualFizzle(
					mob,
					target,
					"<S-NAME> analyze(s) the nature of <T-NAMESELF>, looking more frustrated every second.");
