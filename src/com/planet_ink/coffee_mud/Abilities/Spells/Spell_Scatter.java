...
public class Spell_Scatter extends Spell {
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (mobTarget != null) {
			...
			if (target == null)
				return maliciousFizzle(mob, mobTarget,
						"<S-NAME> attempt(s) a scattering spell at <T-NAMESELF>, but nothing happens.");
		}
		...
		if (givenTarget instanceof Item)
			...
			targets = CMLib.english().fetchItemList(mob, mob, null, commands,
		...
		if (success) {
			...
			String str = null;
			if (mobTarget == null)
				str = auto ? "<S-NAME> <S-IS-ARE> enveloped in a scattering field!"
						: "^S<S-NAME> utter(s) a scattering spell!^?";
			else
				str = auto ? "<T-NAME> <T-IS-ARE> enveloped in a scattering field!"
						: "^S<S-NAME> utter(s) a scattering spell, causing <T-NAMESELF> to resonate.^?";
			CMMsg msg = CMClass.getMsg(mob, mobTarget, this,
				...
				if (msg.value() <= 0) {
					for (int i = 0; i < targets.size(); i++) {
						...
						for (int x = 0; (x < 10) && (room == null); x++)
							room = ((Area) areas.elementAt(CMLib.dice().roll(1,
									areas.size(), -1))).getRandomMetroRoom();
						if (mob.location().okMessage(mob, msg)
								&& (room != null)) {
							mob.location().send(mob, msg);
							if (msg.value() <= 0) {
								...
								if (target.owner() instanceof MOB) {
									...
									mob.location()
											.show(owner,
													...
													CMMsg.MASK_ALWAYS
															| CMMsg.MSG_THROW,
													"<O-NAME> vanishes from <S-YOUPOSS> inventory!");
									room.showOthers(
											owner,
											room,
											target,
											CMMsg.MASK_ALWAYS | CMMsg.MSG_THROW,
											"<O-NAME> appears from out of nowhere!");
								} else {
									mob.location()
											.show(mob,
													room,
			...
			return maliciousFizzle(mob, mobTarget,
					"<S-NAME> attempt(s) a scattering spell, but nothing happens.");
