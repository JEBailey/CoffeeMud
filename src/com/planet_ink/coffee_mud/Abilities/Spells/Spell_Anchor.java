		...
		if ((msg.tool() != null)
				...
				&& ((affected == null)
						|| ((affected instanceof Item)
								&& (!((Item) affected)
										.amWearingAt(Wearable.IN_INVENTORY)) && (msg
									.amITarget(((Item) affected).owner()))) || ((affected instanceof MOB) && (msg
						.amITarget(affected))))
				&& (CMath.bset(((Ability) msg.tool()).flags(),
						Ability.FLAG_MOVING) || CMath.bset(
			...
			else if ((msg.target() != null) && (msg.target() instanceof Item)) {
			...
			} else if ((msg.target() != null) && (msg.target() instanceof Room))
			...
			if (((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_CHANT)
				...
				if (roomS != null)
					roomS.showHappens(CMMsg.MSG_OK_VISUAL,
							"Magic energy fizzles and is absorbed into the air.");
				if (roomD != null)
					roomD.showHappens(CMMsg.MSG_OK_VISUAL,
							"Magic energy fizzles and is absorbed into the air.");
			}
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "An magical anchoring field envelopes <T-NAME>!"
									: "^S<S-NAME> invoke(s) an anchoring field of protection around <T-NAMESELF>.^?"));
			...
			beneficialWordsFizzle(mob, target,
					"<S-NAME> attempt(s) to invoke an anchoring field, but fail(s).");
