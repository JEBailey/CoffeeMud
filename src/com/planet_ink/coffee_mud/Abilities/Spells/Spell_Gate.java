...
public class Spell_Gate extends Spell {
	...
	public boolean isBadRoom(final Room room, final MOB mob, final Room newRoom) {
		return (room == null) || (room == newRoom) || (room == mob.location())
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		MOB target = null;
		try {
			candidates = CMLib.map().findInhabitants(CMLib.map().rooms(), mob,
					areaName, 10);
		...
		if (candidates.size() > 0) {
			target = candidates
					.get(CMLib.dice().roll(1, candidates.size(), -1));
		...
		CMMsg msg = CMClass.getMsg(mob, target, this, CMMsg.MASK_MOVE
				...
				"^S<S-NAME> invoke(s) a teleportation spell" + addOn + "^?");
		if ((mob.location().okMessage(mob, msg)) && (newRoom != null)
				&& (newRoom.okMessage(mob, msg))) {
			...
			for (Iterator f = h.iterator(); f.hasNext();) {
				...
				CMMsg leaveMsg = CMClass.getMsg(follower, thisRoom, this,
						...
						"<S-NAME> disappear(s) in a burst of light.");
