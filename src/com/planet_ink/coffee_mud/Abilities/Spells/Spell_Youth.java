	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		CMMsg msg = CMClass
				.getMsg(mob,
						...
						auto ? ""
								: "^S<S-NAME> wave(s) <S-HIS-HER> arms around <T-NAMESELF>, drawing forth <T-HIS-HER> youthful self.^?");
		if (success) {
			if (mob.location().okMessage(mob, msg)) {
				...
				if ((target.baseCharStats().getStat(CharStats.STAT_AGE) <= 0)
					...
					mob.tell(mob, target, null,
							"The magic appears to have had no effect upon <T-NAME>.");
					success = false;
				} else {
					int[] chart = target.baseCharStats().getMyRace()
							.getAgingChart();
					mob.location().show(target, null, CMMsg.MSG_OK_VISUAL,
							"<S-NAME> grow(s) younger!");
					int cat = target.baseCharStats().ageCategory();
					int age = target.baseCharStats()
							.getStat(CharStats.STAT_AGE);
					if (cat >= Race.AGE_ANCIENT) {
						int diff = chart[Race.AGE_ANCIENT]
								- chart[Race.AGE_VENERABLE];
						age = age - chart[Race.AGE_ANCIENT];
						int num = (diff > 0) ? (int) Math.abs(Math.floor(CMath
								.div(age, diff))) : 0;
						if (num <= 0)
							age = (int) Math.round(CMath.div(chart[cat]
									+ chart[cat - 1], 2.0));
						else
							age = target.baseCharStats().getStat(
									CharStats.STAT_AGE)
									- diff;
					} else
						age = (int) Math.round(CMath.div(chart[cat]
					...
					if (target.playerStats() != null) {
						...
						target.playerStats().getBirthday()[2] = C.getYear()
								- age;
			...
			beneficialVisualFizzle(mob, target,
					"<S-NAME> wave(s) <S-HIS-HER> arms around <T-NAMESELF>, but the spell fizzles.");
