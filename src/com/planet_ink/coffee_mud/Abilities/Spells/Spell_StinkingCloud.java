				...
				if ((M.curState().getHunger() <= 0))
					CMLib.combat().postDamage(
							...
							CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS
									| CMMsg.TYP_GAS, -1,
							"<T-NAME> heave(s) in the stinking cloud.");
				else {
					CMLib.combat().postDamage(
							invoker,
							M,
							...
							CMMsg.MASK_MALICIOUS | CMMsg.MASK_ALWAYS
									| CMMsg.TYP_GAS, -1,
							"<T-NAME> heave(s) all over the place!");
					M.curState().adjHunger(-500,
							M.maxState().maxHunger(M.baseWeight()));
				}
			...
			if (CMLib.flags().canSmell(mob))
				...
				case CMMsg.TYP_ADVANCE:
					if (CMLib.dice().rollPercentage() > (mob.charStats()
							.getSave(CharStats.STAT_SAVE_GAS))) {
						mob.location()
								.show(mob, null, CMMsg.MSG_OK_ACTION,
										"<S-NAME> double(s) over from the sickening gas.");
						return false;
					}
					break;
				}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		if ((affected != null) && (affected instanceof MOB)
			...
			msg.source().tell(msg.source(), affected, null,
					"<T-NAME> smell(s) nauseatingly stinky!");
		...
		if (canBeUninvoked()) {
			if ((!mob.amDead()) && (mob.location() != null))
				mob.location().show(mob, null, CMMsg.MSG_NOISYMOVEMENT,
						"<S-NAME> manage(s) to escape the stinking cloud!");
		}
	}
		...
		if (success) {
			if (mob.location()
					.show(mob,
							...
							auto ? ""
									: "^S<S-NAME> incant(s) and wave(s) <S-HIS-HER> arms around.  A horrendous cloud of green and orange gas appears!^?"))
				for (Iterator f = h.iterator(); f.hasNext();) {
					...
					CMMsg msg2 = CMClass.getMsg(mob, target, this,
							CMMsg.MSK_CAST_MALICIOUS_VERBAL | CMMsg.TYP_GAS
						...
						if ((msg.value() <= 0) && (msg2.value() <= 0)
							...
							target.location()
									.show(target, null, CMMsg.MSG_OK_ACTION,
											"<S-NAME> become(s) enveloped in the stinking cloud!");
						}
					}
				}
		} else
