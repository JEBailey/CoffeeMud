										...
										.roll(2,
												4
														...
														+ (2 * super
																.getX1Level(invoker())),
												0),
								CMMsg.MASK_ALWAYS | CMMsg.TYP_ACID, -1,
								"<T-NAME> sizzle(s) from the acid arrow residue!");
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							(auto ? "An arrow made of acid appears zooming towards <T-NAME>!"
									: "^S<S-NAME> point(s) at <T-NAMESELF>, conjuring an acid arrow!^?")
			...
			if ((R.okMessage(mob, msg)) && ((R.okMessage(mob, msg2)))) {
				...
				R.send(mob, msg2);
				invoker = mob;
				int numDice = (adjustedLevel(mob, asLevel) + (2 * super
						.getX1Level(invoker()))) / 2;
				...
				CMLib.combat().postDamage(mob, target, this, damage,
						...
						Weapon.TYPE_MELTING,
						"The acidic blast <DAMAGES> <T-NAME>!");
				maliciousAffect(mob, target, asLevel, 3, -1);
			}
		} else
			return maliciousFizzle(mob, target,
					"<S-NAME> point(s) and conjur(s) at <T-NAMESELF>, but nothing more happens.");
