		...
		if (canBeUninvoked())
			if ((mob.location() != null) && (!mob.amDead()))
				mob.tell(mob, null, null,
						"<S-YOUPOSS> death warning magic fades.");
	}
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (target.fetchEffect(this.ID()) != null) {
			mob.tell(target, null, null,
					"<S-NAME> already <S-HAS-HAVE> a death's warning.");
			return false;
		}
		...
		if (success) {
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? "<T-NAME> begin(s) listening for a death's warning!"
									: "^S<S-NAME> incant(s) coldly, and begin(s) listening for death's warning!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				beneficialAffect(mob, target, asLevel, 0);
				Spell_DeathWarning A = (Spell_DeathWarning) target
						.fetchEffect(ID());
				if (A != null)
					A.commands = commands;
			}
			...
			beneficialVisualFizzle(mob, null,
					"<S-NAME> incant(s) coldly and listen(s), but the spell fizzles.");
