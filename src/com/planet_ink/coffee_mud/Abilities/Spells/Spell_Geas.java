...
public class Spell_Geas extends Spell {
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_SPEAK) && (STEPS != null)
	...
	public boolean invoke(MOB mob, Vector commands, Physical givenTarget,
		...
		if (mob.isMonster()) {
			...
			CMLib.commands()
					.postSay(
							mob,
							null,
							"You know, if I had any ambitions, I would put the geas on myself!",
							false, false);
			return false;
		}
		...
		if (target.charStats().getStat(CharStats.STAT_INTELLIGENCE) < 5) {
			mob.tell(target.name(mob)
					+ " is too stupid to understand the instructions!");
			return false;
		}
		...
		if (success) {
			...
			CMMsg msg = CMClass
					.getMsg(mob,
							...
							auto ? ""
									: "^S<S-NAME> place(s) a powerful geas upon <T-NAMESELF>!^?");
			if (mob.location().okMessage(mob, msg)) {
				mob.location().send(mob, msg);
				STEPS = CMLib.slavery().processRequest(mob, target,
				...
				if (maliciousAffect(mob, target, asLevel, 2000,
			...
			return maliciousFizzle(mob, target,
					"<S-NAME> attempt(s) to place a geas on <T-NAMESELF>, but fails.");
