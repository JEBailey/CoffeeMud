...
public class StdExit implements Exit {
	...
	// protected void
	// finalize(){CMClass.unbumpCounter(this,CMClass.CMObjectType.EXIT);}//removed
	// for mem & perf
	...
	public CMObject copyOf() {
		try {
			...
			// CMClass.bumpCounter(this,CMClass.CMObjectType.EXIT);//removed for
			// mem & perf
			E.cloneFix(this);
			return E;
	...
	protected void mountLadder(MOB mob, Rideable ladder) {
		...
		CMMsg msg = CMClass.getMsg(mob, ladder, null, CMMsg.MSG_MOUNT,
				"<S-NAME> " + mountStr + " <T-NAMESELF>.");
	...
	protected final String closeWordPastTense() {
		if (closeWord().length() == 0)
			return "closed";
				...
				.isVowel(closeWord().charAt(closeWord().length() - 1)))
			return closeWord() + "d";
		else
			return closeWord() + "ed";
	}
		...
		if ((!msg.amITarget(this)) && (msg.tool() != this))
			...
			case CMMsg.TYP_ENTER:
				if (msg.target() instanceof Room)
					lastRoomID = CMLib.map().getExtendedRoomID(
							(Room) msg.target());
				if ((hasADoor()) && (!isOpen())
			...
			case CMMsg.TYP_CLOSE: {
				...
				mob.tell("The " + doorName() + " is already "
						+ closeWordPastTense() + ".");
				return false;
			}
			case CMMsg.TYP_OPEN: {
			...
			if (isOpen()) {
				if ((room != null) && (!CMLib.flags().canBeSeenBy(room, mob)))
					...
					Say.append(displayText()
							+ CMLib.flags().colorCodes(this, mob));
				else if (room != null)
					Say.append(room.displayText(mob)
							+ CMLib.flags().colorCodes(room, mob));
		...
		case CMMsg.TYP_OPEN:
			...
			if (defaultsClosed() || defaultsLocked()) {
				...
				CMLib.threads().startTickDown(this,
						Tickable.TICKID_EXIT_REOPEN, openDelayTicks());
			}
			isLocked = false;
			isOpen = true;
			break;
	...
	public void setDoorsNLocks(boolean newHasADoor, boolean newIsOpen,
			boolean newDefaultsClosed, boolean newHasALock,
			boolean newIsLocked, boolean newDefaultsLocked) {
		isOpen = newIsOpen;
	...
	public void addBehavior(Behavior to) {
		...
		if (behaviors.size() == 0)
			CMLib.threads().startTickDown(this, Tickable.TICKID_EXIT_BEHAVIOR,
					1);
	...
	public void addScript(ScriptingEngine S) {
		...
		if (!scripts.contains(S)) {
			...
			if (scripts.size() == 0)
				CMLib.threads().startTickDown(this,
						Tickable.TICKID_EXIT_BEHAVIOR, 1);
