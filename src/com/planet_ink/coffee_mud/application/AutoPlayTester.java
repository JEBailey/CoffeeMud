...
public class AutoPlayTester {
	...
	public LinkedList<String> bufferFill() throws IOException {
		...
		try {
			while ((c = in.read()) >= 0) {
				if (c == 13 || c == 10) {
					if ((c == 13 && lastc != 10) || (c == 10 && lastc != 13)) {
	...
	public void run() {
		...
		try {
			...
			scope.defineFunctionProperties(JScriptEvent.functions,
	...
	protected static class JScriptEvent extends ScriptableObject {
		...
		public static final String[] functions = { "tester", "toJavaString",
				"writeLine", "login", "stdout", "stderr", "waitFor",
				"waitForMultiMatch", "startsWith", "name", "rand", "sleep" };
		...
		public int rand(int x) {
			int y = (int) Math.round(Math.floor(Math.random() * ((x) - 0.001)));
			return (y > 0) ? y : -y;
		}
		...
		public Object waitForMultiMatch(Object regexO, Object numMatches) {
			try {
				return testObj.waitFor(toJavaString(regexO),
						Integer.parseInt(toJavaString(numMatches)));
			} catch (Exception e) {
				return null;
			}
		}
	...
	public static void main(String[] args) {
		if (args.length < 4) {
			...
			System.out
					.println("AutoPlayTester [host] [port] [character name] [script path]");
			System.exit(-1);
		}
		StringBuilder path = new StringBuilder(args[3]);
		for (int i = 4; i < args.length; i++)
			path.append(" ").append(args[i]);
		AutoPlayTester player = new AutoPlayTester(args[0], s_int(args[1]),
				args[2], path.toString());
		player.run();
	}
}
