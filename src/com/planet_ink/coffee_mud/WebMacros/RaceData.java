...
public class RaceData extends StdWebMacro {
	...
	private String raceDropDown(String old) {
		...
		for (Enumeration e = CMClass.races(); e.hasMoreElements();) {
			...
			if (R2.isGeneric()
					&& CMClass.checkForCMClass(CMObjectType.RACE, R2ID)) {
				str.append("<OPTION VALUE=\"" + R2.ID() + "\" "
						+ ((old.equalsIgnoreCase(R2.ID())) ? "SELECTED" : "")
						+ ">" + R2.ID() + " (Generic)");
				str.append("<OPTION VALUE=\"" + R2ID + "\" "
						+ ((old.equalsIgnoreCase(R2ID)) ? "SELECTED" : "")
						+ ">" + R2ID);
			} else if (R2.isGeneric())
				str.append("<OPTION VALUE=\""
						+ R2.ID()
						+ "\" "
						+ ((old.equalsIgnoreCase(R2.ID()) || old
								.equalsIgnoreCase(R2ID)) ? "SELECTED" : "")
						+ ">" + R2.ID() + " (Generic)");
			else
				str.append("<OPTION VALUE=\""
						+ R2ID
	...
	public static StringBuffer estats(PhyStats E, char c, HTTPRequest httpReq,
		...
		if (httpReq.isUrlParameter(c + "ESTATS1")) {
			...
			while (behav != null) {
				if (behav.length() > 0) {
					if (!behav.equalsIgnoreCase("REJUV")) {
						String prof = httpReq.getUrlParameter(c + "ESTATSV"
								+ num);
						if (prof == null)
							prof = "0";
			...
			for (int i = 0; i < E.getStatCodes().length; i++)
				if (CMath.s_int(E.getStat(E.getStatCodes()[i])) != 0)
					theclasses.addElement(E.getStatCodes()[i],
							Integer.toString(CMath.s_int(E.getStat(E
									.getStatCodes()[i]))));
		}
		...
		for (int i = 0; i < theclasses.size(); i++) {
			...
			str.append("<SELECT ONCHANGE=\"EditAffect(this);\" NAME=" + c
					+ "ESTATS" + (i + 1) + ">");
			...
			str.append("<INPUT TYPE=TEXT NAME=" + c + "ESTATSV" + (i + 1)
					+ " VALUE=\"" + theclasses.elementAt(i).second
					+ "\" SIZE=4 MAXLENGTH=4>");
			str.append("</TD>");
			str.append("</TR>");
		}
		str.append("<TR><TD WIDTH=35%>");
		str.append("<SELECT ONCHANGE=\"AddAffect(this);\" NAME=" + c + "ESTATS"
				+ (theclasses.size() + 1) + ">");
		str.append("<OPTION SELECTED VALUE=\"\">Select a stat");
		for (int i = 0; i < E.getStatCodes().length; i++)
			if ((CMath.isNumber(E.getStat(E.getStatCodes()[i])))
					&& (!theclasses.contains(E.getStatCodes()[i])))
				str.append("<OPTION VALUE=\"" + E.getStatCodes()[i] + "\">"
						+ E.getStatCodes()[i]);
		str.append("</SELECT>");
		str.append("</TD>");
		str.append("<TD WIDTH=65%>");
		str.append("<INPUT TYPE=TEXT NAME=" + c + "ESTATSV"
				+ (theclasses.size() + 1) + " VALUE=\"\" SIZE=4 MAXLENGTH=4>");
		str.append("</TD>");
		str.append("</TR>");
		str.append("</TABLE>");
		return str;
	...
	public static StringBuffer cstats(CharStats E, char c, HTTPRequest httpReq,
			java.util.Map<String, String> parms, int borderSize) {
		StringBuffer str = new StringBuffer("");
		PairVector<String, String> theclasses = new PairVector<String, String>();
		if (httpReq.isUrlParameter(c + "CSTATS1")) {
			...
			for (int i : CharStats.CODES.ALL())
				if (CMath.s_int(E.getStat(CharStats.CODES.NAME(i))) != 0)
					theclasses.addElement(CharStats.CODES.NAME(i),
							E.getStat(CharStats.CODES.NAME(i)));
		}
		str.append("<TABLE WIDTH=100% BORDER=" + borderSize
				+ " CELLSPACING=0 CELLPADDING=0>");
		for (int i = 0; i < theclasses.size(); i++) {
			...
			str.append("</TD>");
			str.append("<TD WIDTH=65%>");
			str.append("<INPUT TYPE=TEXT NAME=" + c + "CSTATSV" + (i + 1)
					+ " VALUE=\"" + theclasses.elementAt(i).second
					+ "\" SIZE=4 MAXLENGTH=4>");
			str.append("</TD>");
			str.append("</TR>");
		...
		str.append("<TR><TD WIDTH=35%>");
		str.append("<SELECT ONCHANGE=\"AddAffect(this);\" NAME=" + c + "CSTATS"
				+ (theclasses.size() + 1) + ">");
		str.append("<OPTION SELECTED VALUE=\"\">Select a stat");
		for (int i : CharStats.CODES.ALL())
			if (!theclasses.contains(CharStats.CODES.NAME(i)))
				str.append("<OPTION VALUE=\"" + CharStats.CODES.NAME(i) + "\">"
						+ CharStats.CODES.DESC(i));
		str.append("</SELECT>");
		str.append("</TD>");
		str.append("<TD WIDTH=65%>");
		str.append("<INPUT TYPE=TEXT NAME=" + c + "CSTATSV"
				+ (theclasses.size() + 1) + " VALUE=\"\" SIZE=4 MAXLENGTH=4>");
		str.append("</TD>");
		str.append("</TR>");
		str.append("</TABLE>");
	...
	public static StringBuffer cstate(CharState E, char c, HTTPRequest httpReq,
			java.util.Map<String, String> parms, int borderSize) {
		StringBuffer str = new StringBuffer("");
		PairVector<String, String> theclasses = new PairVector<String, String>();
		if (httpReq.isUrlParameter(c + "CSTATE1")) {
			...
			for (int i = 0; i < E.getStatCodes().length; i++)
				if (CMath.s_int(E.getStat(E.getStatCodes()[i])) != 0)
					theclasses.addElement(E.getStatCodes()[i],
							Integer.valueOf(E.getStat(E.getStatCodes()[i]))
									.toString());
		}
		str.append("<TABLE WIDTH=100% BORDER=" + borderSize
				+ " CELLSPACING=0 CELLPADDING=0>");
		for (int i = 0; i < theclasses.size(); i++) {
			...
			str.append("</TD>");
			str.append("<TD WIDTH=65%>");
			str.append("<INPUT TYPE=TEXT NAME=" + c + "CSTATEV" + (i + 1)
					+ " VALUE=\"" + theclasses.elementAt(i).second
					+ "\" SIZE=4 MAXLENGTH=4>");
			str.append("</TD>");
			str.append("</TR>");
		...
		str.append("<TR><TD WIDTH=35%>");
		str.append("<SELECT ONCHANGE=\"AddAffect(this);\" NAME=" + c + "CSTATE"
				+ (theclasses.size() + 1) + ">");
		str.append("<OPTION SELECTED VALUE=\"\">Select a stat");
		for (int i = 0; i < E.getStatCodes().length; i++)
			if (CMath.isNumber(E.getStat(E.getStatCodes()[i])))
		...
		str.append("</TD>");
		str.append("<TD WIDTH=65%>");
		str.append("<INPUT TYPE=TEXT NAME=" + c + "CSTATEV"
				+ (theclasses.size() + 1) + " VALUE=\"\" SIZE=4 MAXLENGTH=4>");
		str.append("</TD>");
		str.append("</TR>");
		str.append("</TABLE>");
	...
	public static StringBuffer itemList(List<? extends Item> items, char c,
			HTTPRequest httpReq, java.util.Map<String, String> parms,
			int borderSize, boolean one) {
		if (items == null)
			items = new Vector();
		StringBuffer str = new StringBuffer("");
		...
		if (!one)
			for (int i = 0; i < classes.size(); i++) {
				...
				str.append("<SELECT ONCHANGE=\"AddItem(this);\" NAME=" + c
						+ "ITEM" + (numItems) + ">");
				if (!one)
					str.append("<OPTION VALUE=\"\">Delete!");
				if (items.contains(I))
					...
					str.append("<OPTION SELECTED VALUE=\"" + I + "\">"
							+ I.Name() + " (" + I.ID() + ")");
				else
					str.append("<OPTION SELECTED VALUE=\"" + I.ID() + "\">"
							+ I.Name() + " (" + I.ID() + ")");
				str.append("</SELECT>");
				str.append("</TD>");
				str.append("<TD WIDTH=10%>");
				str.append("<INPUT TYPE=BUTTON NAME=" + c + "EDITITEM"
						+ (i + 1) + " VALUE=EDIT ONCLICK=\"EditItem('"
						+ RoomData.getItemCode(classes, I) + "','" + c + "ITEM"
						+ (numItems) + "');\">");
				str.append("</TD></TR>");
			}
		str.append("<TR><TD WIDTH=90%>");
		str.append("<SELECT ONCHANGE=\"AddItem(this);\" NAME=" + c + "ITEM"
				+ (numItems + 1) + ">");
		if (!one)
		...
		if (one) {
			...
			for (int i = 0; i < sorted.length; i++) {
				...
				str.append("<OPTION " + (selected ? "SELECTED" : "")
						+ " VALUE=\"" + (String) sorted[i] + "\">"
						+ (String) sorted[i]);
			}
		} else {
			StringBuffer mposs = (StringBuffer) Resources
					.getResource("MUDGRINDER-OTHERPOSS");
			if (mposs == null) {
		...
		str.append("</TD>");
		str.append("<TD WIDTH=10%>");
		str.append("<INPUT TYPE=BUTTON NAME=" + c
				+ "ADDITEM VALUE=\"NEW\" ONCLICK=\"AddNewItem('" + c + "ITEM"
				+ (numItems + 1) + "');\">");
		str.append("</TD></TR></TABLE>");
		return str;
	}

	public static StringBuffer dynAbilities(List<Ability> ables, String ID,
			Modifiable obj, HTTPRequest httpReq,
			java.util.Map<String, String> parms, int borderSize, String font) {
		StringBuffer str = new StringBuffer("");
		...
		if (httpReq.isUrlParameter("RABLES1")) {
			...
			for (final Iterator<Ability> a = ables.iterator(); a.hasNext();) {
				...
				if ((A != null) && (!cables.containsFirst(A.ID()))) {
					boolean defaultGain = CMLib.ableMapper().getDefaultGain(ID,
							false, A.ID());
					int qualifyingLevel = CMLib.ableMapper()
							.getQualifyingLevel(ID, false, A.ID());
					theclasses.addElement(A.ID(), A.proficiency() + "",
							defaultGain ? "" : "on", qualifyingLevel + "");
				}
			}
		}
		if (font == null)
		...
		for (int i = 0; i < theclasses.size(); i++) {
			String theclass = theclasses.elementAt(i).first;
			str.append("<TR><TD WIDTH=35%>");
			str.append("<SELECT ONCHANGE=\"EditAffect(this);\" NAME=RABLES"
					+ (i + 1) + ">");
			str.append("<OPTION VALUE=\"\">Delete!");
			str.append("<OPTION VALUE=\"" + theclass + "\" SELECTED>"
					+ theclass);
			...
			str.append(font + "Lvl:</B></FONT> <INPUT TYPE=TEXT NAME=RABLVL"
					+ (i + 1) + " VALUE=\"" + theclasses.elementAt(i).fourth
					+ "\" SIZE=3 MAXLENGTH=3>");
			str.append("</TD>");
			str.append("<TD WIDTH=15%>");
			str.append(font + "<INPUT TYPE=TEXT NAME=RABPOF" + (i + 1)
					+ " VALUE=\"" + theclasses.elementAt(i).second
					+ "\" SIZE=3 MAXLENGTH=3>%</B></I></FONT>");
			str.append("</TD>");
			str.append("<TD WIDTH=25%>");
			str.append("<INPUT TYPE=CHECKBOX NAME=RABQUA"
					+ (i + 1)
					+ " "
					+ (theclasses.elementAt(i).third.equalsIgnoreCase("on") ? "CHECKED"
							: "") + ">" + font + "Qualify Only</B></FONT></I>");
			str.append("</TD>");
			str.append("</TR>");
		}
		str.append("<TR><TD WIDTH=35%>");
		str.append("<SELECT ONCHANGE=\"AddAffect(this);\" NAME=RABLES"
				+ (theclasses.size() + 1) + ">");
		...
		str.append("</TD>");
		str.append("<TD WIDTH=25%>");
		str.append(font + "Lvl:</B></I></FONT> <INPUT TYPE=TEXT NAME=RABLVL"
				+ (theclasses.size() + 1) + " VALUE=\"\" SIZE=3 MAXLENGTH=3>");
		str.append("</TD>");
		str.append("<TD WIDTH=15%>");
		str.append(font + "<INPUT TYPE=TEXT NAME=RABPOF"
				+ (theclasses.size() + 1)
				+ " VALUE=\"\" SIZE=3 MAXLENGTH=3>%</B></I></FONT>");
		str.append("</TD>");
		str.append("<TD WIDTH=25%>");
		str.append("<INPUT TYPE=CHECKBOX NAME=RABQUA" + (theclasses.size() + 1)
	...
	public static StringBuffer dynEffects(String ID, List<Ability> ables,
			Modifiable obj, HTTPRequest httpReq,
			java.util.Map<String, String> parms, int borderSize, String font) {
		StringBuffer str = new StringBuffer("");
		...
		if (httpReq.isUrlParameter("REFFS1")) {
			...
			for (int a = 0; a < ables.size(); a++) {
				...
				if (A != null) {
					int qualifyingLevel = CMath.s_int(obj.getStat("GETREFFLVL"
							+ a));
		...
		for (int i = 0; i < theclasses.size(); i++) {
			String theclass = theclasses.elementAt(i).first;
			str.append("<TR><TD WIDTH=35%>");
			str.append("<SELECT ONCHANGE=\"EditAffect(this);\" NAME=REFFS"
					+ (i + 1) + ">");
			str.append("<OPTION VALUE=\"\">Delete!");
			str.append("<OPTION VALUE=\"" + theclass + "\" SELECTED>"
					+ theclass);
			...
			str.append("</TD>");
			str.append("<TD WIDTH=25%>");
			str.append(font + "Lvl:</B></FONT> <INPUT TYPE=TEXT NAME=REFLVL"
					+ (i + 1) + " VALUE=\"" + theclasses.elementAt(i).third
					+ "\" SIZE=3 MAXLENGTH=3>");
			str.append("</TD>");
			str.append("<TD WIDTH=40%>");
			str.append("<INPUT TYPE=TEXT NAME=REFPRM" + (i + 1) + " VALUE=\""
					+ theclasses.elementAt(i).second + "\" SIZE=25>");
			str.append("</TD>");
			str.append("</TR>");
		}
		str.append("<TR><TD WIDTH=35%>");
		str.append("<SELECT ONCHANGE=\"AddAffect(this);\" NAME=REFFS"
				+ (theclasses.size() + 1) + ">");
		str.append("<OPTION SELECTED VALUE=\"\">Select an Ability");
		for (Enumeration<Ability> a = CMClass.abilities(); a.hasMoreElements();) {
		...
		str.append("</TD>");
		str.append("<TD WIDTH=25%>");
		str.append(font + "Lvl:</B></I></FONT> <INPUT TYPE=TEXT NAME=REFLVL"
				+ (theclasses.size() + 1) + " VALUE=\"\" SIZE=3 MAXLENGTH=3>");
		str.append("</TD>");
		str.append("<TD WIDTH=40%>");
		str.append("<INPUT TYPE=TEXT NAME=REFPRM" + (theclasses.size() + 1)
				+ " VALUE=\"\" SIZE=25>");
		str.append("</TD>");
		str.append("</TR>");
		str.append("</TABLE>");
	...
	public static StringBuffer cabilities(Race E, HTTPRequest httpReq,
			java.util.Map<String, String> parms, int borderSize, String font) {
		StringBuffer str = new StringBuffer("");
		PairVector<String, String> theclasses = new PairVector<String, String>();
		if (httpReq.isUrlParameter("CABLES1")) {
			...
			for (int i = 0; i < ables.size(); i++)
				theclasses.addElement(ables.getFirst(i), ables.getSecond(i)
						.toString());
		}
		if (font == null)
			font = "<FONT COLOR=WHITE><B>";
		...
		for (int i = 0; i < theclasses.size(); i++) {
			...
			str.append("<TD WIDTH=65%>");
			str.append(font
					+ "Pct:</B></I></FONT> <INPUT TYPE=TEXT NAME=CABPOF"
					+ (i + 1) + " VALUE=\"" + theclasses.elementAt(i).second
					+ "\" SIZE=3 MAXLENGTH=3>" + font + "%</B></I></FONT>");
			str.append("</TD>");
			str.append("</TR>");
		}
		str.append("<TR><TD WIDTH=35%>");
		...
		str.append("</TD>");
		str.append("<TD WIDTH=65%>");
		str.append(font + "Pct:</B></I></FONT> <INPUT TYPE=TEXT NAME=CABPOF"
				+ (theclasses.size() + 1) + " VALUE=\"\" SIZE=3 MAXLENGTH=3>"
				+ font + "%</B></I></FONT>");
		str.append("</TD>");
		str.append("</TR>");
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (last.length() > 0) {
			...
			if ((R == null) && (newRaceID != null) && (newRaceID.length() > 0)
					&& (CMClass.getRace(newRaceID) == null)) {
				R = (Race) CMClass.getRace("GenRace").copyOf();
				R.setRacialParms("<RACE><ID>" + newRaceID + "</ID><NAME>"
						+ newRaceID + "</NAME></RACE>");
				last = newRaceID;
			...
			if (R != null) {
				...
				if (parms.containsKey("HELP")) {
					StringBuilder s = CMLib.help().getHelpText(R.ID(), null,
							false, true);
					if (s == null)
				...
				if (parms.containsKey("WEIGHT")) {
					str.append(""
							+ (R.lightestWeight() + (R.weightVariance() / 2))
							+ ", ");
				}
				...
				if (parms.containsKey("BREATHES")) {
					...
					if (httpReq.isUrlParameter("BREATHES")) {
						int breathe = CMath.s_int(httpReq
								.getUrlParameter("BREATHES"));
				...
				if (parms.containsKey("BODY")) {
					...
					for (int i = 0; i < Race.BODYPARTSTR.length; i++) {
						...
						str.append("<TD WIDTH=1%>"
								+ font
								+ Race.BODYPARTSTR[i]
								+ "</B></I></FONT></TD><TD><INPUT TYPE=TEXT NAME=BODYPART"
								+ i + " VALUE=\"" + old + "\" SIZE=3></TD>");
				...
				if (parms.containsKey("RABLE"))
					str.append(dynAbilities(R.racialAbilities(null), R.ID(), R,
							httpReq, parms, 0, parms.get("FONT")) + ", ");
				if (parms.containsKey("REFFS"))
					str.append(dynEffects(R.ID(), R.racialEffects(null), R,
							httpReq, parms, 0, parms.get("FONT")) + ", ");
				...
				if (parms.containsKey("WEARID")) {
					...
					if (old == null)
						...
						for (int i = 1;; i++)
							if (httpReq.isUrlParameter("WEARID"
									+ (Integer.toString(i))))
								mask |= CMath.s_long(httpReq
										.getUrlParameter("WEARID"
												+ (Integer.toString(i))));
							else
								break;
					}
				...
				if (parms.containsKey("OUTFIT"))
					str.append(itemList(R.outfit(null), 'O', httpReq, parms, 0,
							false) + ", ");
				...
				if (parms.containsKey("AGING")) {
					...
					if (!httpReq.isUrlParameter("AGE0"))
						for (int i = 0; i < Race.AGE_DESCS.length; i++)
							httpReq.addFakeUrlParameter("AGE" + i, ""
									+ ageChart[i]);
					int val = -1;
					for (int i = 0; i < Race.AGE_DESCS.length; i++) {
						int lastVal = val;
						...
						str.append("<INPUT TYPE=TEXT SIZE=4 NAME=AGE" + i
								+ " VALUE=" + val + ">" + Race.AGE_DESCS[i]
								+ "<BR>");
					}
					str.append(", ");
				}
					...
					for (Enumeration c = CMClass.charClasses(); c
						...
						if ((C != null)
								...
								&& (CMStrings.containsIgnoreCase(
										C.getRequiredRaceList(), "All")
										|| CMStrings
												.containsIgnoreCase(
														C.getRequiredRaceList(),
														R.ID())
										|| CMStrings.containsIgnoreCase(
												C.getRequiredRaceList(),
												R.name()) || CMStrings
