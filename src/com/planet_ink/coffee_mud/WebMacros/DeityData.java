...
public class DeityData extends StdWebMacro {
	...
	private DVector getDeityData(HTTPRequest httpReq, String deityName) {
		DVector folData = (DVector) httpReq.getRequestObjects().get(
				"DEITYDATAFOR-" + deityName.toUpperCase().trim());
		if (folData != null)
			return folData;
		folData = CMLib.database().worshippers(deityName);
		httpReq.getRequestObjects().put(
				"DEITYDATAFOR-" + deityName.toUpperCase().trim(), folData);
		return folData;
	}
			...
			if (D != null) {
				...
				if (parms.containsKey("LOCATION")) {
					if (D.getStartRoom() == null)
						...
						str.append(CMLib.map().getExtendedRoomID(
								D.getStartRoom())
				...
				if (parms.containsKey("AREA") && (D.getStartRoom() != null))
					if (parms.containsKey("ENCODED"))
						try {
							str.append(URLEncoder.encode(D.getStartRoom()
									.getArea().Name(), "UTF-8")
									+ ", ");
						} catch (Exception e) {
						}
				...
				if (parms.containsKey("MOBCODE")) {
					...
					List classes = (List) httpReq.getRequestObjects().get(
							"DEITYLIST-" + roomID);
					if (classes == null) {
						classes = new Vector();
						...
						if (R == null) {
							...
							for (Enumeration e = restoreDeities.elements(); e
									.hasMoreElements();) {
								Deity D2 = (Deity) e.nextElement();
						...
						httpReq.getRequestObjects().put("DEITYLIST-" + roomID,
								classes);
					}
					if (parms.containsKey("ENCODED"))
						try {
							str.append(URLEncoder.encode(
									RoomData.getMOBCode(classes, D), "UTF-8")
									+ ", ");
						} catch (Exception e) {
						}
					else
				...
				if (parms.containsKey("NUMPRIESTS")) {
					...
					for (int d = 0; d < data.size(); d++) {
						CharClass C = CMClass.getCharClass((String) data
								.elementAt(d, 2));
						if ((C != null)
								&& (C.baseClass().equalsIgnoreCase("CLERIC")))
							num++;
					}
					str.append(num + ", ");
				}
