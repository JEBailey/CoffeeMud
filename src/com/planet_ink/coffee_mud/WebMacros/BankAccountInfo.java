...
public class BankAccountInfo extends StdWebMacro {
	...
	public static synchronized BankAccountStuff getMakeAccountInfo(
			HTTPRequest httpReq, Banker B, MOB playerM) {
		BankAccountStuff info = (BankAccountStuff) httpReq.getRequestObjects()
				.get("BANKINFO: " + B.bankChain() + ": " + playerM.Name());
		if (info != null)
			return info;
		info = new BankAccountStuff();
		if ((B.isSold(ShopKeeper.DEAL_CLANBANKER))
				&& (playerM.getClanRole(playerM.Name()) == null)) {
		} else {
			Double bal = Double.valueOf(B.getBalance(playerM.Name())); // this
																		// works
																		// for
																		// clans
																		// because
		...
		httpReq.getRequestObjects().put(
				"BANKINFO: " + B.bankChain() + ": " + playerM.Name(), info);
		return info;
	}
		...
		try {
			...
			if ((player != null) && (player.length() > 0)) {
				...
				if (C != null) {
					...
					playerM.setClan(C.clanID(),
							C.getTopRankedRoles(Function.DEPOSIT_LIST).get(0)
									.intValue());
					destroyPlayer = true;
				} else {
					...
					if (playerM == null) {
						...
						playerM.setLocation(M.location());
						playerM.setStartRoom(M.getStartRoom());
						playerM.setClan("", Integer.MIN_VALUE); // delete all
																// sequence
						destroyPlayer = true;
					}
				}
			...
			BankAccountStuff acct = BankAccountInfo.getMakeAccountInfo(httpReq,
					B, playerM);
			...
			if ((parms.containsKey("DEBTAMT"))
				...
				String intRate = CMath.div(
						(int) Math.round(debt.interest * 10000.0), 100.0)
						+ "%";
				...
				if (timeRemaining < 0)
					...
					int mudHoursToGo = (int) (timeRemaining / CMProps
							.getMillisPerMudHour());
			...
			if (parms.containsKey("ITEMSWORTH"))
				return CMLib.beanCounter().nameCurrencyLong(playerM,
						B.totalItemsWorth(playerM.Name()));
