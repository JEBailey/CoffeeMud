...
public class SipletInterface extends StdWebMacro {
	...
	private class PipeSocket extends Socket {
		...
		public PipeSocket(InetAddress addr, PipeSocket pipeLocal)
				throws IOException {
			this.addr = addr;
			if (pipeLocal != null) {
	...
	public String runMacro(HTTPRequest httpReq, String parm)
				...
				public boolean tick(Tickable ticking, int tickID) {
					...
					synchronized (siplets) {
						for (final String key : siplets.keySet()) {
							...
							if ((p != null)
									&& ((System.currentTimeMillis() - p.lastTouched) > (2 * 60 * 1000))) {
		...
		if (httpReq.isUrlParameter("CONNECT")) {
			...
			if (url != null) {
				...
				synchronized (sipletConnectSync) {
					for (MudHost h : CMLib.hosts())
						if (h.getPort() == port) {
							try {
								PipeSocket lsock = new PipeSocket(
										httpReq.getClientAddress(), null);
								PipeSocket rsock = new PipeSocket(
										httpReq.getClientAddress(), lsock);
								success = sip.connectToURL(url, port, lsock);
								sip.setFeatures(true,
										Siplet.MSPStatus.External, false);
			...
			return Boolean.toString(success) + ';' + hex + ';' + sip.info()
					+ hex + ';';
					...
					if (data != null) {
						...
						if (p.siplet.isConnectedToURL()) {
							...
							if (p.siplet.isConnectedToURL()) {
								...
								final String jscript = p.siplet
										.getJScriptCommands();
								success = p.siplet.isConnectedToURL();
								p.response = Boolean.toString(success) + ';'
										+ data + token + ';' + jscript + token
										+ ';';
								return p.response;
							}
						}
				...
				if (p != null) {
					if (p.siplet.isConnectedToURL()) {
						if (httpReq.isUrlParameter("LAST"))
							...
							p.response = Boolean.toString(success) + ';' + data
									+ token + ';' + jscript + token + ';';
							return p.response;
						}
					}
