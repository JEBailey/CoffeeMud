...
public class ItemData extends StdWebMacro {
	...
	private static final String[] okparms = { "NAME", "CLASSES", "DISPLAYTEXT",
			...
			"MATERIALS", "ISGENERIC", "ISFOOD", "NOURISHMENT", "ISDRINK",
			...
			"USESREMAIN", "VALUE", "WEIGHT", "ISMAP", "MAPAREAS", "ISREADABLE",
			...
			"ISAMMO", "ISMOBITEM", "ISDUST", "ISPERFUME", "SMELLS", "IMAGE",
			"ISEXIT", "EXITNAME", "EXITCLOSEDTEXT", "NUMCOINS", "CURRENCY",
			"DENOM", "ISRECIPE", "RECIPESKILL", "RECIPEDATA", "LAYER",
			"SEETHRU", "MULTIWEAR", "ISCATALOGED", "CATARATE", "CATALIVE",
			"CATAMASK", "BITE", "MAXUSES", "ISELECTRONIC", "CATACAT" };
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		synchronized (sync.intern()) {
			...
			if ((playerM != null) && (R == null)) {
				I = (Item) httpReq.getRequestObjects().get(
						"PLAYER/" + player + "/" + itemCode);
				if (I == null) {
					...
					if (I != null)
						httpReq.getRequestObjects().put(
								"PLAYER/" + player + "/" + itemCode, I);
				}
				M = playerM;
			} else if ((mobNum != null) && (mobNum.length() > 0)) {
				if (R != null)
					M = (MOB) httpReq.getRequestObjects().get(
							R.roomID() + "/" + mobNum);
				else
				...
				if (M == null) {
					if (R != null)
						...
						M = RoomData.getMOBFromCode(RoomData.getMOBCache(),
								mobNum);
					if (M == null) {
				...
				if (I == null) {
					if (itemCode.startsWith("CATALOG-")
						...
						I = CMLib.catalog().getCatalogItem(
								itemCode.substring(8));
						if (I == null)
							I = CMClass.getItem("GenItem");
						else
							I = (Item) I.copyOf();
						...
						I = RoomData.getItemFromAnywhere(
								RoomData.getItemCache(), itemCode);
					if (I != null)
		...
		boolean changedClass = ((httpReq.isUrlParameter("CHANGEDCLASS") && httpReq
				.getUrlParameter("CHANGEDCLASS").equals("true")) && (itemCode
				.equals("NEW") || itemCode.startsWith("CATALOG-") || itemCode
				.startsWith("NEWCATA-")));
		...
		if ((changedLevel) && (I.isGeneric())) {
			...
			int hands = 1;
			if (httpReq.isUrlParameter("ISTWOHANDED")
					&& (httpReq.getUrlParameter("ISTWOHANDED")
							.equalsIgnoreCase("on")))
				hands = 2;
			Map<String, String> vals = null;
			if (I instanceof Weapon) {
				int wclass = CMath
						.s_int(httpReq.getUrlParameter("WEAPONCLASS"));
				int reach = CMath.s_int(httpReq.getUrlParameter("MINRANGE"));

				vals = CMLib.itemBuilder().timsItemAdjustments(I, level,
						material, hands, wclass, reach, 0);
			} else {
		...
		if (I != null) {
			...
			for (int o = 0; o < okparms.length; o++)
				if (parms.containsKey(okparms[o])) {
					...
					case 0: // name
						if (firstTime) {
							if ((itemCode.equalsIgnoreCase("NEW")
									|| itemCode.startsWith("CATALOG-") || itemCode
										.startsWith("NEWCATA-"))
					...
					case 1: // classes
					{
						...
						Object[] sorted = (Object[]) Resources
								.getResource("MUDGRINDER-ITEMS2:"
										+ parms.containsKey("GENERICONLY"));
						if (sorted == null) {
							Vector sortMe = new Vector();
							CMClass.addAllItemClassNames(sortMe, true, false,
									parms.containsKey("GENERICONLY"));
							sorted = (new TreeSet(sortMe)).toArray();
							Resources.submitResource("MUDGRINDER-ITEMS2:"
									+ parms.containsKey("GENERICONLY"), sorted);
						}
					...
					case 29: // is gettable
						if (firstTime)
							old = (!CMath.bset(I.basePhyStats().sensesMask(),
									PhyStats.SENSE_ITEMNOTGET)) ? "checked"
									: "";
					...
					case 30: // is removable
						if (firstTime)
							old = (!CMath.bset(I.basePhyStats().sensesMask(),
									PhyStats.SENSE_ITEMNOREMOVE)) ? "checked"
									: "";
						else if (old.equals("on"))
							old = "checked";
						str.append(old);
					...
					case 31: // is droppable
						if (firstTime)
							old = (!CMath.bset(I.basePhyStats().sensesMask(),
									PhyStats.SENSE_ITEMNODROP)) ? "checked"
									: "";
						else if (old.equals("on"))
							old = "checked";
						str.append(old);
					...
					case 34: // readable spells
						if (I instanceof SpellHolder)
							str.append(readableSpells((SpellHolder) I, httpReq,
									parms, 1));
						break;
					case 35: // is wand
						...
						for (Enumeration a = CMLib.map().areas(); a
							...
							if (!(A2 instanceof SpaceObject)) {
								str.append("<OPTION VALUE=\"" + A2.Name()
										+ "\"");
								if (mask.indexOf(";" + A2.Name().toUpperCase()
										+ ";") >= 0)
									str.append(" SELECTED");
								str.append(">" + A2.name());
							}
					...
					case 41: // is readable
						if (firstTime)
							old = (CMath.bset(I.basePhyStats().sensesMask(),
									PhyStats.SENSE_ITEMREADABLE)) ? "checked"
									: "";
						else if (old.equals("on"))
							old = "checked";
						str.append(old);
					...
					case 45: // liquid types
						...
						for (Integer liquid : liquids) {
							...
							if (liquid.intValue() == CMath.s_int(old))
								str.append(" SELECTED");
							str.append(">"
									+ RawMaterial.CODES.NAME(liquid.intValue()));
						}
						break;
					case 46: // ammo types
						if (firstTime) {
					...
					case 63: // being worn -- pushed back to mob/room
						if (firstTime)
							old = I.amWearingAt(Wearable.IN_INVENTORY) ? ""
									: "checked";
						else if (old.equals("on"))
							old = "checked";
						str.append(old);
					...
					case 64: // non-locatable
						if (firstTime)
							old = CMLib.flags().canBeLocated(I) ? ""
									: "checked";
						else if (old.equals("on"))
							old = "checked";
						str.append(old);
					...
					case 66: // content types
						if (I instanceof Container) {
							...
							if (httpReq.isUrlParameter("CONTENTTYPES")) {
								contains = CMath.s_long(httpReq
										.getUrlParameter("CONTENTTYPES"));
								if (contains > 0)
									for (int i = 1;; i++)
										if (httpReq
												.isUrlParameter("CONTENTTYPES"
														+ (Integer.toString(i))))
											contains = contains
					...
					case 79: // currency
					{
						...
						List<String> cs = CMLib.beanCounter()
								.getAllCurrencies();
					...
					{
						String currency = (I instanceof Coins) ? currency = ((Coins) I)
								.getCurrency() : "";
						if ((firstTime) && (I instanceof Coins))
							old = "" + ((Coins) I).getDenomination();
						MoneyLibrary.MoneyDenomination[] DV = CMLib
						...
						if (I instanceof Recipe) {
							...
							if (fieldName == null)
								...
								if (httpReq.isUrlParameter(thisFieldName)) {
									while (httpReq
											.isUrlParameter(thisFieldName)) {
										String value = httpReq
												.getUrlParameter(thisFieldName);
										if (value.length() > 0)
											allData.add(value);
										thisFieldName = CMStrings.replaceAll(
												fieldName, "###", "" + (++x));
									}
								} else {
									String[] allRecipes = ((Recipe) I)
											.getRecipeCodeLines();
									for (String recipe : allRecipes) {
										if (recipe.length() > 0)
											allData.add(CMStrings.replaceAll(
													recipe, "\t", ","));
									}
								}
								allData.add("");
								for (x = 0; x < allData.size(); x++) {
									...
									thisFieldName = CMStrings.replaceAll(
											fieldName, "###", "" + x);
									String myPrefix = CMStrings.replaceAll(
											prefix, fieldName, thisFieldName);
									String myPostfix = CMStrings.replaceAll(
											postfix, fieldName, thisFieldName);
									str.append(myPrefix)
											.append(super
													.htmlOutgoingFilter(recipeLine))
											.append(myPostfix);
								}
							}
						}
					...
					case 85: // see-thru
						if ((firstTime) && (I instanceof Armor))
							old = CMath.bset(((Armor) I).getLayerAttributes(),
									Armor.LAYERMASK_SEETHROUGH) ? "checked"
									: "";
						else if (old.equals("on"))
							old = "checked";
						str.append(old);
					...
					case 86: // multi-layer
						if ((firstTime) && (I instanceof Armor))
							old = CMath.bset(((Armor) I).getLayerAttributes(),
									Armor.LAYERMASK_MULTIWEAR) ? "checked" : "";
						else if (old.equals("on"))
							old = "checked";
						str.append(old);
						break;
	...
	public static StringBuffer readableSpells(SpellHolder P,
		...
		if (parms.containsKey("READABLESPELLS")) {
			...
			for (int i = 0; i < theclasses.size(); i++) {
				...
				str.append("<SELECT ONCHANGE=\"EditAffect(this);\" NAME=RSPELL"
						+ (i + 1) + ">");
				...
				str.append("<INPUT TYPE=TEXT SIZE=30 NAME=RSPDATA" + (i + 1)
						+ " VALUE=\"" + theparm + "\">");
				str.append("</TD></TR>");
			}
			str.append("<TR><TD WIDTH=50%>");
			str.append("<SELECT ONCHANGE=\"AddAffect(this);\" NAME=RSPELL"
					+ (theclasses.size() + 1) + ">");
			...
			str.append("</SELECT>");
			str.append("</TD><TD WIDTH=50%>");
			str.append("<INPUT TYPE=TEXT SIZE=30 NAME=RSPDATA"
