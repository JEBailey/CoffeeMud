...
public class PlayerData extends StdWebMacro {
	...
	public final static String[] BASICS = { "NAME", "DESCRIPTION",
			"LASTDATETIME", "EMAIL", "RACENAME", "CHARCLASS", "LEVEL",
			"LEVELSTR", "CLASSLEVEL", "CLASSES", "MAXCARRY", "ATTACKNAME",
			"ARMORNAME", "DAMAGENAME",
			"HOURS",
			"PRACTICES",
			"EXPERIENCE",
			"EXPERIENCELEVEL",
			...
			"CLANNAMES",
			"CLANROLE", // deprecated
			"ALIGNMENTNAME", "ALIGNMENTSTRING", "WIMP", "STARTROOM",
			"LOCATION", "STARTROOMID", "LOCATIONID", "INVENTORY", "WEIGHT",
			"ENCUMBRANCE", "GENDERNAME", "LASTDATETIMEMILLIS", "HITPOINTS",
			"MANA", "MOVEMENT", "RIDING", "HEIGHT", "LASTIP", "QUESTPOINTS",
			"BASEHITPOINTS", "BASEMANA", "BASEMOVEMENT", "IMAGE", "MAXITEMS",
			"IMGURL", "HASIMG", "NOTES", "LEVELS", "ATTACK", "DAMAGE", "ARMOR",
			"SPEEDNAME", "SPEED", "EXPERTISE", "TATTOOS", "SECURITY", "TITLES",
			"FACTIONNAMES", "ACCTEXPUSED", "ACCTEXP", "FOLLOWERNAMES",
			"ACCOUNT" };
	...
	public static String getBasic(MOB M, int i) {
		...
		case 2:
			if (M.playerStats() != null)
				str.append(CMLib.time().date2String(
		...
		case 4:
			...
			break;
		case 5:
			str.append(M.baseCharStats().getCurrentClass()
					.name(M.baseCharStats().getCurrentClassLevel())
					+ ", ");
			break;
		case 6:
		...
		case 7:
			...
			break;
		case 8:
			str.append(M.baseCharStats().getClassLevel(
		...
		case 9: {
			for (int c = M.charStats().numClasses() - 1; c >= 0; c--) {
				...
				str.append(C.name(M.baseCharStats().getCurrentClassLevel())
		...
		case 10:
			...
			break;
		case 11:
			str.append(CMStrings.capitalizeAndLower(CMStrings
					.removeColors(CMLib.combat().fightingProwessStr(M))) + ", ");
			break;
		case 12:
			str.append(CMStrings.capitalizeAndLower(CMStrings
					.removeColors(CMLib.combat().armorStr(M))) + ", ");
			break;
		case 13:
		...
		case 33:
			...
			break;
		case 34:
			str.append(CMStrings.capitalizeAndLower(M.baseCharStats()
					.genderName()) + ", ");
			break;
		case 35:
			if (M.playerStats() != null)
				...
				for (int level = 0; level <= M.phyStats().level(); level++) {
					...
					if ((dateTime > 1529122205) && (dateTime != lastDateTime)) {
						...
						if (level == 0)
							...
							str.append("<TD><FONT COLOR=WHITE>" + level
									+ "</FONT></TD>");
						str.append("<TD><FONT COLOR=WHITE>"
		...
		case 59: {
			if (M.playerStats() != null) {
				List<String> flags = CMParms.parseSemicolons(M.playerStats()
		...
		case 62:
			str.append(CMProps.getBoolVar(CMProps.Bool.ACCOUNTEXPIRATION) ? "true"
					: "false");
			break;
		case 63:
			if (M.playerStats() != null)
				str.append(CMLib.time().date2String(
						M.playerStats().getAccountExpiration()));
			break;
		case 64: {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (last.length() > 0) {
			...
			if (parms.containsKey("DEITY")) {
				...
				str.append("<OPTION " + ((old.length() == 0) ? "SELECTED" : "")
						+ " VALUE=\"\">Godless");
			...
			if (parms.containsKey("TITLELIST")) {
				if (M.playerStats() != null) {
					...
					for (b = 0; b < titles.size(); b++) {
						...
						if (B != null)
							str.append("<INPUT TYPE=TEXT NAME=TITLE" + b
									+ " SIZE=" + B.length() + " VALUE=\""
									+ CMStrings.replaceAll(B, "\"", "&quot;")
									+ "\"><BR>");
					}
					str.append("<INPUT TYPE=TEXT NAME=TITLE" + titles.size()
							+ " SIZE=60 VALUE=\"\">");
				}
			}
					...
					for (Faction.Align v : Faction.Align.values())
						if (v != Faction.Align.INDIFF) {
							...
							if (old.equalsIgnoreCase(v.toString()))
								str.append(" SELECTED");
							str.append(">"
									+ CMStrings.capitalizeAndLower(v.toString()
											.toLowerCase()));
						}
				}
			}
			if (parms.containsKey("BASEGENDER")) {
				...
				str.append("<OPTION VALUE=M "
						+ ((old.equalsIgnoreCase("M")) ? "SELECTED" : "")
						+ ">M");
				str.append("<OPTION VALUE=F "
						+ ((old.equalsIgnoreCase("F")) ? "SELECTED" : "")
						+ ">F");
				str.append("<OPTION VALUE=N "
						+ ((old.equalsIgnoreCase("N")) ? "SELECTED" : "")
						+ ">N");
			}
