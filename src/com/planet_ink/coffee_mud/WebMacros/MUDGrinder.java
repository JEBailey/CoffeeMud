...
public class MUDGrinder extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (parms != null)
			if (parms.containsKey("AREAMAP")) {
				...
				int[] xyxy = this.getAppropriateXY(A,
						httpReq.getUrlParameter("MAPSTYLE"));
				...
				return GrinderAreas.getAreaList(CMLib.map().mundaneAreas(),
						pickedA, mob, noInstances);
				...
				Area pickedA = getLoggedArea(httpReq, mob);
				boolean noInstances = parms.containsKey("NOINSTANCE");
				return GrinderAreas.getAreaList(CMLib.map().spaceAreas(),
						pickedA, mob, noInstances);
				...
				Log.sysOut("Grinder",
						mob.Name() + " obliterated area " + A.Name());
				return "The area " + A.Name()
				...
				if (err.length() > 0)
					return err;
				Log.sysOut("Grinder",
						mob.Name() + " modified account " + A.accountName());
				return "";
				...
				for (Enumeration<String> p = A.getPlayers(); p
						.hasMoreElements();) {
					MOB deadMOB = CMLib.players()
							.getLoadPlayer(p.nextElement());
					CMLib.players().obliteratePlayer(deadMOB, true, false);
					Log.sysOut("Grinder", mob.Name() + " destroyed user "
							+ deadMOB.Name() + ".");
					deadMOB.destroy();
				}
				CMLib.players().obliterateAccountOnly(A);
				Log.sysOut("Grinder",
						mob.Name() + " destroyed account " + A.accountName()
								+ " and players '" + playerList + "'.");
				return "";
				...
				if (last.startsWith("CATALOG-")) {
					...
					if (M == null)
						return "@break@";
					if (!CMSecurity.isAllowedEverywhere(mob,
							CMSecurity.SecFlag.CATALOG))
						return "@break@";
					CMLib.catalog().delCatalog(M);
					Log.sysOut("Grinder", mob.Name()
							+ " destroyed catalog mob " + last);
				...
				if (last.length() == 0)
					return "@break@";
				List<AbilityComponent> list = CMLib.ableMapper()
						.getAbilityComponentDVector(last);
				if (list == null)
					return "@break@";
				if (!CMSecurity.isAllowedEverywhere(mob,
				...
				CMLib.ableMapper().getAbilityComponentMap()
						.remove(last.toUpperCase().trim());
				CMLib.ableMapper().alterAbilityComponentFile(last, true);
				Log.sysOut("Grinder", mob.Name() + " destroyed component "
						+ last);
				return "The component " + last
						+ " has been successfully destroyed.";
			} else if (parms.containsKey("EDITCOMPONENT")
					|| parms.containsKey("ADDCOMPONENT")) {
				MOB mob = Authenticate.getAuthenticatedMob(httpReq);
				if (mob == null)
					return "@break@";
				String last = httpReq.getUrlParameter("COMPONENT");
				...
				if (err.length() > 0)
					return err;
				Log.sysOut("Grinder", mob.Name() + " modified component "
						+ last);
				return "The component " + last
				...
				Map<String, Map<String, AbilityMapper.AbilityMapping>> allQualMap = CMLib
						.ableMapper().getAllQualifiesMap(
								httpReq.getRequestObjects());
				Map<String, AbilityMapper.AbilityMapping> map = allQualMap
						.get(which.toUpperCase().trim());
				if (map == null)
					return " @break@";
				map.remove(last.toUpperCase().trim());
				CMLib.ableMapper().saveAllQualifysFile(allQualMap);
				Log.sysOut("Grinder", mob.Name()
						+ " destroyed all qualify ability " + last);
				return "The all qualify ability " + last
						+ " has been successfully destroyed.";
			} else if (parms.containsKey("EDITALLQUALIFY")
					|| parms.containsKey("ADDALLQUALIFY")) {
				MOB mob = Authenticate.getAuthenticatedMob(httpReq);
				if (mob == null)
					return "@break@";
				String last = httpReq.getUrlParameter("ALLQUALID");
				...
				if (!CMSecurity.isAllowedEverywhere(mob,
						CMSecurity.SecFlag.ABILITIES))
					return "@break@";
				String err = new GrinderAllQualifys().editAllQualify(httpReq,
						parms);
				if (err.length() > 0)
					return err;
				Log.sysOut("Grinder", mob.Name()
						+ " modified all qualify ability " + last);
				return "The all qualify ability " + last
						+ " has been successfully modified.";
				...
				C.destroyClan();
				Log.sysOut("Grinder",
						mob.Name() + " destroyed clan " + C.clanID());
				return "The clan " + C.clanID()
						+ " has been successfully destroyed.";
				...
				C.update();
				Log.sysOut("Grinder",
						mob.Name() + " modified clan " + C.clanID());
				return "The clan " + C.clanID()
						+ " has been successfully modified.";
				...
				if ((C != null) || CMLib.players().playerExists(last)
				...
				newClan.setGovernmentID(CMLib.clans().getDefaultGovernment()
						.getID());
				newClan.setStatus(Clan.CLANSTATUS_PENDING);
				newClan.create();
				Log.sysOut("Grinder",
						mob.Name() + " created clan " + newClan.clanID());
				httpReq.addFakeUrlParameter("CLAN", last);
				return "The clan " + newClan.clanID()
				...
				if (last.length() == 0)
					return "@break@";
				ClanGovernment G = CMLib.clans().getStockGovernment(
						CMath.s_int(last));
				if (G == null)
					return "@break@";
				if (!CMSecurity.isAllowedEverywhere(mob,
				...
				Log.sysOut("Grinder", mob.Name()
						+ " destroyed clan government " + G.getName());
				return "The clan government " + G.getName()
						+ " has been successfully destroyed.";
				...
				if (!CMSecurity.isAllowedEverywhere(mob,
						CMSecurity.SecFlag.CMDCLANS))
					return "@break@";
				String err = new GrinderClanGovernments().runMacro(httpReq,
						parm);
				if (err.length() > 0)
					return err;
				CMLib.clans().reSaveGovernmentsXML();
				Log.sysOut("Grinder", mob.Name() + " modified clan government "
						+ G.getName());
				return "The clan government " + G.getName()
						+ " has been successfully modified.";
				...
				if (file == null)
					file = "";
				byte[] bufBytes = (byte[]) httpReq.getRequestObjects().get(
						"FILE");
				if ((file.length() == 0) || (bufBytes == null)
						|| (bufBytes.length == 0))
					return "That file was empty.";
				boolean deleteIfExists = false;
				if (httpReq.getUrlParameter("DELFIRST") != null)
					deleteIfExists = httpReq.getUrlParameter("DELFIRST")
							.equalsIgnoreCase("ON");
				StringBuffer buf = new StringBuffer(
						CMStrings.bytesToStr(bufBytes));
				Vector<Object> V = new Vector<Object>();
				V.addAll(CMParms.parse("IMPORT "
						+ (deleteIfExists ? "" : "NODELETE ") + "NOPROMPT"));
				V.add(buf);
				...
				if ((V.size() == 0)
						|| (V.get(V.size() - 1) instanceof StringBuffer))
				...
				if (R == null)
					return "@break@";
				int dir = Directions.getGoodDirectionCode(httpReq
						.getUrlParameter("LINK"));
				if (dir < 0)
					return "@break@";
				Log.sysOut("Grinder", mob.Name() + " deleted exit " + dir
						+ " from " + R.roomID());
				...
				if (dir < 0)
					return "@break@";
				Log.sysOut("Grinder", mob.Name() + " modified exit " + dir
						+ " from " + R.roomID());
				...
				if (dir < 0)
					return "@break@";
				Room R2 = CMLib.map().getRoom(
						httpReq.getUrlParameter("OLDROOM"));
				if (R2 == null)
					return "@break@";
				int dir2 = Directions.getGoodDirectionCode(httpReq
						.getUrlParameter("OLDLINK"));
				if (dir2 < 0)
					return "@break@";
				Log.sysOut("Grinder", mob.Name() + " linked exit " + dir
						+ " from " + R.roomID());
				...
				String errMsg = "";
				if (R2 == null)
					errMsg = "No external room with ID '" + oldroom
							+ "' found.";
				else {
					errMsg = GrinderExits.linkRooms(R, R2, dir,
							Directions.getOpDirectionCode(dir));
					Log.sysOut("Grinder",
							mob.Name() + " linked area " + R.roomID() + " to "
									+ R2.roomID());
				}
				httpReq.addFakeUrlParameter("ERRMSG", errMsg);
				...
				if (R == null)
					return "@break@";
				final String multiFlagStr = httpReq
						.getUrlParameter("MULTIROOMFLAG");
				final boolean multiFlag = (multiFlagStr != null)
						&& multiFlagStr.equalsIgnoreCase("on");
				final List<String> multiRoomList = CMParms.parseSemicolons(
						httpReq.getUrlParameter("MULTIROOMLIST"), false);
				String errMsg;
				if ((multiFlag) && (multiRoomList.size() > 0)) {
					errMsg = "";
					Pair<String, String> setPairs[] = RoomData
							.makeMergableRoomFields(R, multiRoomList);
					for (final String id : multiRoomList) {
						...
						if (R == null)
							return "@break@";
						HTTPRequest mergeReq = RoomData.mergeRoomFields(
								httpReq, setPairs, R);
						errMsg += GrinderRooms
								.editRoom(mergeReq, parms, mob, R);
					}
				} else
				...
				return (err.length() > 0) ? err : "Holiday '" + last
						+ "' deleted.";
				...
				String err = GrinderHolidays.createModifyHoliday(httpReq,
						parms, last);
				if (holidayIndex < 0)
					Log.sysOut("Grinder", mob.name() + " created holiday "
							+ last);
				else
					Log.sysOut("Grinder", mob.name() + " modified holiday "
							+ last);
				...
				CMClass.loadClass(CMObjectType.RACE,
						"com/planet_ink/coffee_mud/Races/" + oldRID + ".class",
						true);
				...
				if ((R == null) || (!R.isGeneric())) {
					...
					if (R == null)
						return " @break@";
					R.setRacialParms("<RACE><ID>" + last + "</ID><NAME>" + last
							+ "</NAME></RACE>");
				}
				R = R.makeGenRace();
				String errMsg = GrinderRaces.modifyRace(httpReq, parms,
						(oldR == null) ? R : oldR, R);
				httpReq.addFakeUrlParameter("ERRMSG", errMsg);
				if (!create) {
					CMLib.database().DBDeleteRace(R.ID());
				...
				if ((oldR != null) && (!oldR.isGeneric()))
					return "Race " + R.ID() + " replaced with Generic Race "
							+ R.ID() + ".";
				...
				CMClass.loadClass(CMObjectType.CHARCLASS,
						"com/planet_ink/coffee_mud/CharClasses/" + oldCID
								+ ".class", true);
				...
				Log.sysOut("Grinder",
						mob.name() + " deleted charclass " + C.ID());
				...
				if ((C == null) || (!C.isGeneric())) {
					...
					if (C != null)
						oldC = C;
					C = (CharClass) CMClass.getCharClass("GenCharClass")
							.copyOf();
					if (C == null)
						return " @break@";
					C.setClassParms("<CCLASS><ID>" + last + "</ID><NAME>"
							+ last + "</NAME></CCLASS>");
				}
				C = C.makeGenCharClass();
				String errMsg = GrinderClasses.modifyCharClass(httpReq, parms,
						(oldC == null) ? C : oldC, C);
				httpReq.addFakeUrlParameter("ERRMSG", errMsg);
				if (!create) {
					CMLib.database().DBDeleteClass(C.ID());
				...
				if ((oldC != null) && (!oldC.isGeneric()))
					return "Char Class " + C.ID()
							+ " replaced with Generic Class " + C.ID() + ".";
				...
				if (F == null)
					return " @break@";
				CMFile F2 = new CMFile(Resources.makeFileResourceName(CMLib
						.factions().makeFactionFilename(F.factionID())), null,
				...
				if (F == null) {
					...
					if (!Resources.saveFileResource("::"
							+ CMLib.factions().makeFactionFilename(last), mob,
							template))
						return "Unable to save "
								+ Resources.buildResourcePath("")
								+ CMLib.factions().makeFactionFilename(last);
				...
				String errMsg = GrinderFactions
						.modifyFaction(httpReq, parms, F);
				if (errMsg.length() == 0)
				...
				int code = CMath.s_int(httpReq
						.getUrlParameter("CLASSIFICATION_ACODE"));
				...
				String errMsg = GrinderAbilities.modifyAbility(httpReq, parms,
						(oldA == null) ? A : oldA, A);
				httpReq.addFakeUrlParameter("ERRMSG", errMsg);
				if (!create) {
					CMLib.database().DBDeleteAbility(A.ID());
					CMLib.database().DBCreateAbility(A.ID(), type,
							A.getStat("ALLXML"));
					Log.sysOut("Grinder",
							mob.name() + " modified ability " + A.ID() + " ("
									+ type + ")");
				...
				MOB playerM = (player != null) ? CMLib.players().getLoadPlayer(
						player) : null;
				...
				MOB M = CMLib.players().getLoadPlayer(
						httpReq.getUrlParameter("PLAYER"));
				if (M == null)
					return "@break@";
				String errMsg = GrinderPlayers.editPlayer(mob, httpReq, parms,
						M);
				httpReq.addFakeUrlParameter("ERRMSG", errMsg);
				...
				if ((multiFlag) && (multiRoomList.size() > 0)) {
					...
					Room R = CMLib.map().getRoom(
							httpReq.getUrlParameter("ROOM"));
					if (R == null)
						return "@break@";
					rooms.add(R);
				}
				...
				for (final Room R : rooms) {
					...
					Log.sysOut("Grinder",
							mob.Name() + " deleted room " + R.roomID());
				...
				String errMsg = GrinderRooms.createRoom(
						R,
						dir,
						(copyThisOne != null)
								&& (copyThisOne.equalsIgnoreCase("ON")));
				httpReq.addFakeUrlParameter("ERRMSG", errMsg);
				R = R.rawDoors()[dir];
				if (R != null) {
					httpReq.addFakeUrlParameter("ROOM", R.roomID());
					Log.sysOut("Grinder",
							mob.Name() + " added room " + R.roomID());
				}
				httpReq.addFakeUrlParameter("LINK", "");
				...
				for (int l = 0; l < likeList.size(); l++) {
					...
					if (R == null)
						return "The room you entered (" + like
								+ ") could not be found.";
				...
				int brushSize = CMath.s_int(httpReq
						.getUrlParameter("BRUSHSIZE"));
				if (brushSize <= 0)
					return "Your brush size should be a number greater than 0!";
				int[] coords = getAppropriateXY(A,
						httpReq.getUrlParameter("PAINTAT"));
				if (coords == null)
					return "An error occurred trying to paint.  Relogin and try again.";
				int radius = (int) Math.round(Math.floor(CMath
						.div(brushSize, 2)));
				if (radius < 1)
					radius = 1;
				coords[4] -= radius;
						...
						for (int y = coords[5]; y <= endY; y++) {
							...
							if (A.getProperRoomnumbers().contains(roomID)) {
								...
								if (R != null) {
									...
									Log.sysOut("Grinder", mob.Name()
											+ " obliterated room " + R.roomID());
								}
							}
						}
				RoomnumberSet deferredExitSaves = (RoomnumberSet) CMClass
						.getCommon("DefaultRoomnumberSet");
				for (int x = coords[4]; x <= endX; x++)
					for (int y = coords[5]; y <= endY; y++) {
						roomID = gridRoomID(A, x, y);
						if (!A.getProperRoomnumbers().contains(roomID)) {
							Room likeRoom = (Room) RS.elementAt(CMLib.dice()
									.roll(1, RS.size(), 0) - 1);
							R = GrinderRooms.createGridRoom(A, roomID,
									likeRoom, deferredExitSaves, true);
							Log.sysOut("Grinder", mob.Name() + " added room "
									+ R.roomID());
						}
					}
				for (Enumeration e = deferredExitSaves.getRoomIDs(); e
						.hasMoreElements();) {
					R = CMLib.map().getRoom((String) e.nextElement());
					CMLib.database().DBUpdateExits(R);
				}
				Log.sysOut("Grinder", mob.name() + " updated "
						+ deferredExitSaves.roomCountAllAreas()
						+ " rooms exits.");
				// String
				// errMsg=GrinderRooms.createRoom(R,dir,(copyThisOne!=null)&&(copyThisOne.equalsIgnoreCase("ON")));
				...
				Room newRoom = GrinderRooms.createGridRoom(A, roomID, copyRoom,
						null, ((link != null) && (link.length() > 0)));
				if (newRoom == null)
					httpReq.addFakeUrlParameter("ERRMSG",
							"An error occurred trying to create your room.");
				else {
					Log.sysOut("Grinder",
							mob.Name() + " added room " + newRoom.roomID());
	...
	protected int[] getAppropriateXY(Area A, String mapStyle) {
		if ((mapStyle == null) || (mapStyle.length() == 0)
	...
	protected String quickfind(Area A, String find) {
		...
		for (Enumeration<Room> r = A.getProperMap(); r.hasMoreElements();) {
			...
			if ((R.roomID().length() > 0)
					&& (R.roomID().toUpperCase().endsWith(find.toUpperCase())))
				return R.roomID();
		}
		for (Enumeration<Room> r = A.getProperMap(); r.hasMoreElements();) {
			R = r.nextElement();
			if ((R.roomID().length() > 0)
					&& (R.displayText().toUpperCase()
							.indexOf(find.toUpperCase()) >= 0))
				return R.roomID();
		}
		for (Enumeration<Room> r = A.getProperMap(); r.hasMoreElements();) {
			R = r.nextElement();
