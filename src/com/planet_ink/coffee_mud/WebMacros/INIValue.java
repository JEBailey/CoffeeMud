...
public class INIValue extends StdWebMacro {
	...
	public String getHelpFor(String tag, String mask) {
		...
		for (int p = 0; p < page.size(); p++) {
			...
			if (s.trim().length() == 0)
				...
				if ((x >= 0)
						&& (help.size() > 0)
						&& ((s.substring(0, x).equals(mask) || (mask
								.endsWith("*") && (s.substring(0, x)
								.startsWith(mask.substring(0, mask.length() - 1))))))) {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if ((parms.size() == 0) && (last != null) && (last.length() > 0)) {
			CMProps page = CMProps.loadPropPage(CMProps
					.getVar(CMProps.Str.INIPATH));
			...
			for (int p = 0; p < page.size(); p++) {
				...
				if ((last == null)
					...
					if (parms.containsKey("VALUE")) {
						CMProps realPage = CMProps.loadPropPage(CMProps
								.getVar(CMProps.Str.INIPATH));
						if (realPage != null)
		...
		CMProps page = CMProps
				.loadPropPage(CMProps.getVar(CMProps.Str.INIPATH));
		if ((page == null) || (!page.isLoaded()))
			return "";
