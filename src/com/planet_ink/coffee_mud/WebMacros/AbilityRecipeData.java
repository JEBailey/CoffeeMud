...
public class AbilityRecipeData extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (last.length() > 0) {
			...
			if ((A != null) && (A instanceof ItemCraftor)
				...
				AbilityParameters.AbilityRecipeData recipeData = (AbilityParameters.AbilityRecipeData) httpReq
						.getRequestObjects().get("ABILITYRECIPEDATA-" + last);
				if (recipeData == null) {
					recipeData = CMLib.ableParms().parseRecipe(
							((ItemCraftor) A).parametersFile(),
							((ItemCraftor) A).parametersFormat());
					if (recipeData.parseError() != null) {
						Log.errOut(ID(), recipeData.parseError());
						return " @break@";
					}
					httpReq.getRequestObjects().put(
							"ABILITYRECIPEDATA-" + last, recipeData);
				}
				StringBuffer str = new StringBuffer("");
				String sfont = (parms.containsKey("FONT")) ? ("<FONT "
						+ (parms.get("FONT")) + ">") : "";
				String efont = (parms.containsKey("FONT")) ? "</FONT>" : "";
				String hsfont = (parms.containsKey("HFONT")) ? ("<FONT "
						+ (parms.get("HFONT")) + ">") : "";
				String hefont = (parms.containsKey("HFONT")) ? "</FONT>" : "";

				if (parms.containsKey("SAVETOVFS")) {
					if (httpReq.isUrlParameter("SAVETOVFS"))
						str.append(CMath.s_bool(httpReq
								.getUrlParameter("SAVETOVFS")) ? "CHECKED" : "");
					else
						str.append(recipeData.wasVFS() ? "CHECKED" : "");
				} else if (parms.containsKey("ROWTABLE")
					...
					if ((row > 0) && ((row - 1) < recipeData.dataRows().size()))
						...
						dataRow = recipeData.newRow(httpReq
								.getUrlParameter("CLASSFIELD"));
					str.append("\n\r<TABLE WIDTH=100% BORDER=1 CELLSPACING=0 CELLPADDING=0>");
					for (int c = 0; c < dataRow.size(); c++) {
						AbilityParameters.AbilityParmEditor editor = CMLib
								.ableParms().getEditors()
								.get(dataRow.elementAt(c, 1));
						String oldVal = (String) dataRow.elementAt(c, 2);
						if (!editor.ID().equalsIgnoreCase("N_A")) {
							str.append("\n\r<TR>");
							str.append("<TD WIDTH=20%>" + hsfont
									+ editor.prompt() + hefont + "</TD>");
							if (c == classFieldIndex)
								str.append("<TD>"
										+ sfont
										+ editor.webValue(httpReq, parms,
												oldVal, "DATA_" + row + "_" + c)
										+ efont + "</TD>");
							else
								str.append("<TD>"
										+ sfont
										+ editor.webField(httpReq, parms,
												oldVal, "DATA_" + row + "_" + c)
										+ efont + "</TD>");
							str.append("</TR>");
						}
					...
					if (classFieldIndex >= 0) {
						String oldVal = (String) dataRow.elementAt(
								classFieldIndex, 2);
						AbilityParameters.AbilityParmEditor editor = CMLib
								.ableParms().getEditors()
								.get(dataRow.elementAt(classFieldIndex, 1));
						str.append("<INPUT TYPE=HIDDEN NAME=CLASSFIELD VALUE=\""
								+ editor.webValue(httpReq, parms, oldVal,
										"CLASSFIELD") + "\">");
					}
					...
					if (recipeData.dataRows().size() == 0) {
						...
						for (int c = 0; c < recipeData.columns().size(); c++)
							if (recipeData.columns().elementAt(c) instanceof List)
								editRow.addElement(recipeData.columns()
										.elementAt(c), "");
						List<String> o = (List) editRow.elementAt(cfIndex, 1);
						classFieldEditor = CMLib.ableParms().getEditors()
								.get(o.get(0).toString());
					} else
						for (int row = 0; row < recipeData.dataRows().size(); row++) {
							if (cfIndex >= 0) {
								DVector dataRow = recipeData.dataRows()
										.elementAt(row);
								classFieldEditor = CMLib.ableParms()
										.getEditors()
										.get(dataRow.elementAt(cfIndex, 1));
							}
						}
					if (classFieldEditor != null)
						str.append(classFieldEditor.webField(httpReq, parms,
								classFieldEditor.defaultValue(),
								"NEWCLASSFIELD"));
				} else if (parms.containsKey("SAVEROW")
						&& (CMath.isInteger(rownum))) {
					DVector dataRow = null;
					int row = CMath.s_int(rownum);
					if ((row - 1 >= 0)
							&& (row - 1 < recipeData.dataRows().size()))
						dataRow = recipeData.dataRows().elementAt(row - 1);
					else {
						dataRow = recipeData.newRow(httpReq
								.getUrlParameter("CLASSFIELD"));
					...
					for (int c = 0; c < dataRow.size(); c++) {
						...
						String newVal = editor.webValue(httpReq, parms, oldVal,
								"DATA_" + row + "_" + c);
						if (newVal != null)
					...
					if (M == null)
						return " @break@";
					boolean saveToVFS = CMath.s_bool(httpReq
							.getUrlParameter("SAVETOVFS"));
					if (CMSecurity.isAllowedAnywhere(M,
							CMSecurity.SecFlag.CMDRECIPES))
						CMLib.ableParms().resaveRecipeFile(M,
								recipeData.recipeFilename(),
								recipeData.dataRows(), recipeData.columns(),
								saveToVFS);
					else
						return " @break@";
				} else if (parms.containsKey("DELROW")
						&& (CMath.isInteger(rownum))) {
					int row = CMath.s_int(rownum);
					if ((row - 1 >= 0)
							&& (row - 1 < recipeData.dataRows().size()))
					...
					str.append("\n\r<TR>");
					str.append("<TD WIDTH=1%>" + hsfont + "#" + hefont
							+ "</TD>");
					for (int c = 0; c < recipeData.columnHeaders().length; c++) {
						str.append("<TD WIDTH="
								+ Math.round(CMath.div(
										recipeData.columnLengths()[c], 72) * 100.0)
								+ "%>");
						str.append(hsfont + recipeData.columnHeaders()[c]
								+ hefont);
						str.append("</TD>");
					}
					str.append("</TR>");
					for (int r = 0; r < recipeData.dataRows().size(); r++) {
						...
						str.append("\n\r<TR>");
						str.append("<TD>");
						str.append("<A HREF=\"javascript:Select(" + (r + 1)
								+ ")\">" + sfont + "<B><FONT COLOR=YELLOW>"
								+ (r + 1) + "</FONT></B>");
						str.append(efont + "</A>");
						str.append("</TD>");
						for (int c = 0; c < dataRow.size(); c++) {
							...
							str.append(CMStrings.limit(
									val,
									(int) Math.round(CMath.div(
											recipeData.columnLengths()[c], 36) * 100.0)));
							str.append(efont + "</TD>");
						}
						str.append("</A></TR>");
					}
