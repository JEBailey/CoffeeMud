...
public class ClanData extends StdWebMacro {
	...
	public static StringBuffer members(Clan C, HTTPRequest httpReq,
		...
		if (parms.containsKey("MEMBERSLIST")) {
				...
				while (behav != null) {
					if (behav.length() > 0) {
						...
						if (M != null) {
							...
							if (role != null)
								theroles.addElement(Integer.valueOf(CMath
										.s_int(role)));
							else
								theroles.addElement(Integer.valueOf(C
			...
			for (int i = 0; i < themembers.size(); i++) {
				...
				str.append("<SELECT ONCHANGE=\"EditMember(this);\" NAME=MEMB"
						+ (i + 1) + ">");
				str.append("<OPTION VALUE=\"\">Delete!");
				str.append("<OPTION VALUE=\"" + themember + "\" SELECTED>"
						+ themember);
				str.append("</SELECT>");
				str.append("</TD>");
			...
			str.append("<TR><TD WIDTH=35%>");
			str.append("<SELECT ONCHANGE=\"AddMember(this);\" NAME=MEMB"
					+ (themembers.size() + 1) + ">");
	...
	public static StringBuffer relations(Clan C, HTTPRequest httpReq,
			java.util.Map<String, String> parms, int borderSize) {
		StringBuffer str = new StringBuffer("");
		if (parms.containsKey("RELATIONS")) {
			...
			if (httpReq.isUrlParameter("RELATION1")) {
				...
				while (behav != null) {
					if (behav.length() > 0)
						therelations.addElement(Integer.valueOf(CMath
								.s_int(behav)));
					num++;
				...
				for (Enumeration e = CMLib.clans().clans(); e.hasMoreElements();) {
					...
					if (CC == C)
						continue;
					therelations.addElement(Integer.valueOf(C
							.getClanRelations(CC.clanID())));
				}
			}
			str.append("<TABLE WIDTH=100% BORDER=" + borderSize
					+ " CELLSPACING=0 CELLPADDING=0>");
			...
			for (Enumeration e = CMLib.clans().clans(); e.hasMoreElements();) {
				...
				str.append("<TR><TD WIDTH=35%><FONT COLOR=YELLOW><B>"
						+ CC.getName() + "</B></FONT>");
				str.append("<TD WIDTH=65% COLSPAN=2>");
				...
				for (int r = 0; r < Clan.REL_DESCS.length; r++) {
					...
					if (r == relation.intValue())
						str.append(" SELECTED");
					str.append(">"
							+ CMStrings.capitalizeAndLower(Clan.REL_DESCS[r]));
				}
				str.append("</SELECT>");
				str.append("</TD>");
				str.append("</TR>");
		...
		if (last.length() > 0) {
			...
			if (C != null) {
				...
				if (parms.containsKey("CCLASSID")) {
					...
					CharClass CC = null;
					for (Enumeration e = CMClass.charClasses(); e
							.hasMoreElements();) {
						CC = (CharClass) e.nextElement();
						str.append("<OPTION VALUE=\""
								+ CC.ID()
								+ "\" "
								+ ((old.equalsIgnoreCase(CC.ID())) ? "SELECTED"
										: "") + ">" + CC.name());
					}
				}
				...
				if (parms.containsKey("STATUS"))
					str.append(CMStrings
							.capitalizeAndLower(Clan.CLANSTATUS_DESC[C
				...
				if (parms.containsKey("STATUSID")) {
					...
					for (int i = 0; i < Clan.CLANSTATUS_DESC.length; i++)
						str.append("<OPTION VALUE="
								+ i
								+ " "
								+ ((old.equals("" + i)) ? "SELECTED" : "")
								+ ">"
								+ CMStrings
										.capitalizeAndLower(Clan.CLANSTATUS_DESC[i]));
				}
				if (parms.containsKey("ACCEPTANCE"))
					str.append(CMLib.masking().maskDesc(
				...
				if (parms.containsKey("TYPEID")) {
					...
					for (ClanGovernment gvt : gvts)
						str.append("<OPTION VALUE="
								+ gvt.getID()
								+ " "
								+ ((old.equals("" + gvt.getID())) ? "SELECTED"
										: "") + ">"
								+ CMStrings.capitalizeAndLower(gvt.getName()));
				}
				if (parms.containsKey("CLANIDRELATIONS"))
					str.append(CMStrings.capitalizeAndLower(Clan.REL_DESCS[C
							.getClanRelations(httpReq.getUrlParameter("CLANID"))]
							.toLowerCase())
							+ ", ");
				...
				if (parms.containsKey("MEMBERNEXT")) {
					final String cmember = httpReq
							.getUrlParameter("CLANMEMBER");
					String lastID = "";
					String posFilter = httpReq
							.getUrlParameter("CLANFUNCFILTER");
					if (posFilter == null)
						posFilter = parms.get("CLANFUNCFILTER");
					if (posFilter == null)
						posFilter = "";
					final Clan.Function reqFunction = (Clan.Function) CMath
							.s_valueOf(Clan.Function.values(), posFilter);
					final List<MemberRecord> members = C.getMemberList();
					for (MemberRecord member : members) {
						final String name = member.name;
						if ((reqFunction != null)
								&& (C.getAuthority(member.role, reqFunction) == Clan.Authority.CAN_NOT_DO))
							continue;
						if ((cmember == null)
								|| ((cmember.length() > 0)
										&& (cmember.equals(lastID)) && (!name
											.equals(lastID)))) {
					...
					if (cmember != null) {
						...
						for (MemberRecord member : members) {
							...
							if (name.equals(cmember)) {
								int i = member.role;
								str.append(CMStrings.capitalizeAndLower(C
										.getRoleName(i, true, false)) + ", ");
								break;
							}
						}
					}
				...
				if (parms.containsKey("OTHERCLANNEXT")) {
					...
					String lastID = "";
					Clan CC = null;
					for (Enumeration e = CMLib.clans().clans(); e
							.hasMoreElements();) {
						CC = (Clan) e.nextElement();
						if (CC == C)
							continue;
				...
				if (decolor)
					str = new StringBuffer(CMStrings.removeColors(str
							.toString()));
