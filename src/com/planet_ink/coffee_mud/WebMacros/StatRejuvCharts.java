...
public class StatRejuvCharts extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		boolean hungry = (httpReq.getUrlParameter("HUNGRY") != null)
				&& (httpReq.getUrlParameter("HUNGRY").length() > 0);
		if (hungry) {
			buf.append("Hungry ");
			mob.curState().setHunger(0);
		}
		boolean thirsty = (httpReq.getUrlParameter("THIRSTY") != null)
				&& (httpReq.getUrlParameter("THIRSTY").length() > 0);
		if (thirsty) {
			buf.append("Thirsty ");
		...
		for (int level = 1; level <= 30; level++) {
			...
			for (int stats = 4; stats <= 25; stats++) {
				...
				double con = mob.charStats().getStat(
						CharStats.STAT_CONSTITUTION);
				double man = mob.charStats().getStat(
				...
				double hpGain = (con > 1.0) ? ((con / 40.0) * lvlby1p5)
						+ (con / 4.5) + 2.0 : 1.0;
				double manaGain = (man > 2.0) ? ((man / 80.0) * lvl)
						+ (man / 4.5) + 2.0 : 1.0;
				double moveGain = (str > 1.0) ? ((str / 40.0) * lvl)
						+ (str / 3.0) + 5.0 : 1.0;
				...
				if ((!mob.isInCombat()) && (!CMLib.flags().isClimbing(mob))) {
					if ((hpGain > 0) && (!CMLib.flags().isGolem(mob)))
						mob.curState().adjHitPoints((int) Math.round(hpGain),
								mob.maxState());
					if (manaGain > 0)
						mob.curState().adjMana((int) Math.round(manaGain),
								mob.maxState());
					if (moveGain > 0)
						mob.curState().adjMovement((int) Math.round(moveGain),
								mob.maxState());
				}
