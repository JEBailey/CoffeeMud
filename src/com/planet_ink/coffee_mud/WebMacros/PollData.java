...
public class PollData extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (parms.containsKey("EDIT")) {
			...
			if (!CMSecurity
					.isAllowed(M, M.location(), CMSecurity.SecFlag.POLLS))
				return "[authentication error]";
			...
			if ((last != null) && (last.length() == 0)
					&& (CMLib.polls().getPoll(newTitle) != null))
			...
			if (last.length() > 0) {
				...
				if (P != null) {
					...
					if (parms.containsKey("ISPREVIEWRESULTS")) {
						String old = httpReq
								.getUrlParameter("ISPREVIEWRESULTS");
						if (old == null)
							old = (CMath.bset(P.getFlags(),
									Poll.FLAG_PREVIEWRESULTS) ? "on" : "");
						str.append((old.equalsIgnoreCase("on") ? "CHECKED" : "")
								+ ", ");
					}
					...
					if (parms.containsKey("EXPIRATION")) // req input
					{
						...
						if (httpReq.isUrlParameter("MONTH")) {
							String AP = "AM";
							int hr = CMath.s_int(httpReq
									.getUrlParameter("HOUR"));
							if (hr > 12) {
								hr -= 12;
								AP = "PM";
							}

							String date = (CMath.s_int(httpReq
									.getUrlParameter("MONTH")) + 1)
									+ "/"
									+ httpReq.getUrlParameter("DAY")
									+ "/"
									+ httpReq.getUrlParameter("YEAR")
						...
						if (!input)
							...
							int year = Calendar.getInstance()
									.get(Calendar.YEAR);
							if ((exp.get(Calendar.YEAR) > 1900)
									&& (exp.get(Calendar.YEAR) < year))
								year = exp.get(Calendar.YEAR);
							int doneYear = Calendar.getInstance().get(
									Calendar.YEAR) + 10;
					...
					if (parms.containsKey("OPTIONS")) {
						...
						if (!httpReq.isUrlParameter("OPTION1")) {
							for (int v = 0; v < P.getOptions().size(); v++) {
								...
								httpReq.addFakeUrlParameter("OPTION" + (v + 1),
										O.text);
							}
						}
						int num = 0;
						...
						while (httpReq.isUrlParameter("OPTION" + (++num))) {
							String option = httpReq.getUrlParameter("OPTION"
									+ num);
							if ((option.length() > 0)
						...
						if (input) {
							++showNum;
							str.append(sfont + "<INPUT TYPE=TEXT NAME=OPTION"
									+ (showNum)
									+ " SIZE=60 MAXLENGTH=255 VALUE=\"\">"
									+ efont);
						}
					...
					if (parms.containsKey("RESULTS")) {
						...
						int[] votes = new int[options.size()
								+ (CMath.bset(P.getFlags(), Poll.FLAG_ABSTAIN) ? 1
										: 0)];
						PollResult R = null;
						int choice = 0;
						int total = 0;
						for (int r = 0; r < P.getResults().size(); r++) {
							...
							if (((choice <= 0) && CMath.bset(P.getFlags(),
									Poll.FLAG_ABSTAIN))
									|| ((choice >= 0) && (choice <= options
											.size()))) {
								total++;
								if (choice <= 0)
						...
						boolean pct = parms.containsKey("PCT")
								|| parms.containsKey("PERCENT");
						boolean showNum = (!pct) || parms.containsKey("VOTES")
								|| parms.containsKey("VOTE")
								|| parms.containsKey("COUNT");
						for (int o = 0; o < options.size(); o++) {
							if ((which < 0) || (which == (o + 1))) {
								...
								if (pct) {
									if (showNum)
										str.append(" ("
												+ CMath.toPct(((double) votes[o])
														/ ((double) total))
												+ ")");
									else
										str.append(CMath
												.toPct(((double) votes[o])
														/ ((double) total)));
								}
								str.append(efont + ", ");
							}
