...
public class Authenticate extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if ((parms != null) && (parms.containsKey("AUTH"))) {
			try {
				return URLEncoder.encode(Encrypt(getLogin(httpReq)) + "-"
						+ Encrypt(getPassword(httpReq)), "UTF-8");
			} catch (Exception u) {
				return "false";
			}
		}
		...
		if ((parms != null) && (parms.containsKey("SETACCOUNT"))
			...
			if ((mob != null) && (mob.playerStats() != null)
				...
				httpReq.addFakeUrlParameter("ACCOUNT", mob.playerStats()
	...
	public static boolean authenticated(HTTPRequest httpReq, String login,
			String password) {
		...
		if (CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1) {
			...
			if ((acct != null) && (acct.matchesPassword(password))
	...
	public static MOB getAuthenticatedMob(HTTPRequest httpReq) {
		...
		if ((login != null) && (login.length() > 0)) {
			...
			if ((mob == null)
					...
					|| ((CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1) && (mob
							.playerStats().getAccount() == null))) {
				if (CMProps.getIntVar(CMProps.Int.COMMONACCOUNTSYSTEM) > 1) {
					...
					|| (mob.Name().trim().length() == 0)
					|| (CMSecurity.isBanned(mob.Name()))
					|| ((mob.playerStats().getAccount() != null) && (CMSecurity
							.isBanned(mob.playerStats().getAccount()
									.accountName()))))
				mob = null;
		}
		if (mob != null)
