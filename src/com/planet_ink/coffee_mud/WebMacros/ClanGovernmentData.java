	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (last.length() > 0) {
			...
			if (!httpReq.isUrlParameter("GPOSID_" + posDexStr)) {
				...
				while (httpReq.isUrlParameter("GPOSID_" + posDexStr)
						&& httpReq.getUrlParameter("GPOSID_" + posDexStr)
								.trim().length() > 0) {
					String oldID = httpReq.getUrlParameter("GPOSID_"
							+ posDexStr);
					String oldName = httpReq.getUrlParameter("GPOSNAME_"
							+ posDexStr);
					String oldPluralName = httpReq
							.getUrlParameter("GPOSPLURALNAME_" + posDexStr);
					int oldRoleID = CMath.s_int(httpReq
							.getUrlParameter("GPOSROLEID_" + posDexStr));
					usedRoleIDs.add(Integer.valueOf(oldRoleID));
					int oldRank = CMath.s_int(httpReq
							.getUrlParameter("GPOSRANK_" + posDexStr));
					int oldMax = CMath.s_int(httpReq.getUrlParameter("GPOSMAX_"
							+ posDexStr));
					String oldMask = httpReq.getUrlParameter("GPOSINNERMASK_"
							+ posDexStr);
					String oldIsPublicStr = httpReq
							.getUrlParameter("GPOSISPUBLIC_" + posDexStr);
					boolean oldIsPublic = oldIsPublicStr == null ? false
							: oldIsPublicStr.equalsIgnoreCase("on");
					Clan.Authority powerFuncs[] = new Clan.Authority[Clan.Function
							.values().length];
					for (int f = 0; f < Clan.Function.values().length; f++)
						powerFuncs[f] = Clan.Authority.CAN_NOT_DO;
					String authDexStr = "";
					int authDex = 0;
					while (httpReq.getUrlParameter("GPOSPOWER_" + posDexStr
							+ "_" + authDexStr) != null) {
						Clan.Function auth = (Clan.Function) CMath.s_valueOf(
								Clan.Function.values(),
								httpReq.getUrlParameter("GPOSPOWER_"
										+ posDexStr + "_" + authDexStr));
			...
			if ((cmpos != null) && (cmpos.length() > 0)
					&& (CMath.s_int(cmpos) >= 0)
					&& (CMath.s_int(cmpos) < posList.size()))
			...
			if (parms.containsKey("POSITIONNEXT")) {
				...
				for (int p = 0; p < posList.size(); p++) {
					if ((cmpos == null)
							|| ((cmpos.length() > 0) && (cmpos.equals(lastPos)) && (!("" + p)
									.equals(lastPos)))) {
				...
				httpReq.addFakeUrlParameter("LASTGOVTPOSITION",
						"" + posList.size());
			...
			if (parms.containsKey("AUTOROLE")) {
				...
				for (ClanPosition pos : posList)
					str.append("<OPTION VALUE=" + pos.getRoleID() + " "
							+ ((autoPos == pos.getRoleID()) ? "SELECTED" : "")
							+ ">" + pos.getName());
			}
			...
			if (parms.containsKey("OVERRIDEMINMEMBERS")) {
				...
				if (old == null)
					old = G.getOverrideMinMembers() == null ? "" : G
							.getOverrideMinMembers().toString();
				str.append(old + ", ");
			}
			...
			if (parms.containsKey("AUTOPROMOTEBY")) {
				...
				for (Clan.AutoPromoteFlag flag : Clan.AutoPromoteFlag.values())
					str.append("<OPTION VALUE=" + flag.toString() + " "
							+ ((old.equals(flag.toString())) ? "SELECTED" : "")
							+ ">" + flag.toString());
			}
			...
			if (parms.containsKey("RABLE")) {
				...
				final Enumeration<AbilityMapping> m = CMLib.ableMapper()
						.getClassAbles(G.ID(), false);
				final List<Ability> abilities = new LinkedList<Ability>();
				for (; m.hasMoreElements();)
					abilities
							.add(CMClass.getAbility(m.nextElement().abilityID));
				str.append(RaceData.dynAbilities(
						G.getClanLevelAbilities(Integer.valueOf(1000)), G.ID(),
						G, httpReq, parms, 0, parms.get("FONT")) + ", ");
			}
			if (parms.containsKey("REFFS"))
				str.append(RaceData.dynEffects(
						G.ID(),
						G.getClanLevelEffects(null, null,
								Integer.valueOf(Integer.MAX_VALUE)), G,
