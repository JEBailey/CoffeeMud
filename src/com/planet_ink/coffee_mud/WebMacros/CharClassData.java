...
public class CharClassData extends StdWebMacro {
	...
	private String classDropDown(String old) {
		...
		for (Enumeration e = CMClass.charClasses(); e.hasMoreElements();) {
			...
			if (C2.isGeneric()
					&& CMClass.checkForCMClass(CMObjectType.CHARCLASS, C2ID)) {
				str.append("<OPTION VALUE=\"" + C2.ID() + "\" "
						+ ((old.equalsIgnoreCase(C2.ID())) ? "SELECTED" : "")
						+ ">" + C2.ID() + " (Generic)");
				str.append("<OPTION VALUE=\"" + C2ID + "\" "
						+ ((old.equalsIgnoreCase(C2ID)) ? "SELECTED" : "")
						+ ">" + C2ID);
			} else if (C2.isGeneric())
				str.append("<OPTION VALUE=\""
						+ C2.ID()
						+ "\" "
						+ ((old.equalsIgnoreCase(C2.ID()) || old
								.equalsIgnoreCase(C2ID)) ? "SELECTED" : "")
						+ ">" + C2.ID() + " (Generic)");
			else
				str.append("<OPTION VALUE=\""
						+ C2ID
	...
	public static StringBuffer cabilities(MOB mob, CharClass E,
			HTTPRequest httpReq, java.util.Map<String, String> parms,
			int borderSize, String font) {
		StringBuffer str = new StringBuffer("");
		DVector theclasses = new DVector(9);
		boolean showPreReqs = httpReq.isUrlParameter("SHOWPREREQS")
				&& httpReq.getUrlParameter("SHOWPREREQS")
						.equalsIgnoreCase("on");
		boolean showMasks = httpReq.isUrlParameter("SHOWMASKS")
				&& httpReq.getUrlParameter("SHOWMASKS").equalsIgnoreCase("on");
		boolean showParms = httpReq.isUrlParameter("SHOWPARMS")
				&& httpReq.getUrlParameter("SHOWPARMS").equalsIgnoreCase("on");
		if (httpReq.isUrlParameter("CABLES1")) {
			...
			List<AbilityMapper.AbilityMapping> data1 = CMLib.ableMapper()
					.getUpToLevelListings(E.ID(), Integer.MAX_VALUE, true,
							false);
			...
			for (AbilityMapper.AbilityMapping able : data1) {
				aID = able.abilityID;
				int qlvl = CMLib.ableMapper().getQualifyingLevel(E.ID(), false,
						aID);
				if (qlvl > maxLvl)
					maxLvl = qlvl;
				if (qlvl < minLvl)
			...
			for (int lvl = minLvl; lvl <= maxLvl; lvl++) {
				for (int i = 0; i < sortedData1.size(); i++) {
					...
					if (qLvl.intValue() == lvl) {
						...
						theclasses.addElement(
								aID,
								qLvl.toString(),
								Integer.toString(CMLib.ableMapper()
										.getDefaultProficiency(E.ID(), false,
												aID)),
								CMLib.ableMapper().getDefaultGain(E.ID(),
										false, aID) ? "" : "on",
								CMLib.ableMapper().getSecretSkill(E.ID(),
										false, aID) ? "on" : "",
								CMLib.ableMapper().getDefaultParm(E.ID(),
										false, aID),
								CMLib.ableMapper().getPreReqStrings(E.ID(),
										false, aID),
								CMLib.ableMapper().getExtraMask(E.ID(), false,
										aID), Integer.toString(CMLib
										.ableMapper().getMaxProficiency(E.ID(),
												false, aID)));
					}
				}
			}
		...
		String sfont = (parms.containsKey("FONT")) ? ("<FONT "
				+ (parms.get("FONT")) + ">") : "";
		...
		for (int i = 0; i < theclasses.size(); i++) {
			...
			used.add(theclass);
			str.append("<TR><TD WIDTH=40%>");
			str.append("<SELECT ONCHANGE=\"EditAffect(this);\" NAME=CABLES"
					+ (i + 1) + ">");
			...
			str.append("<INPUT TYPE=TEXT NAME=CABLVL" + (i + 1) + " VALUE=\""
					+ ((String) theclasses.elementAt(i, 2))
					+ "\" SIZE=2 MAXLENGTH=3>");
			str.append("</TD>");
			str.append("<TD WIDTH=10%>");
			str.append("<INPUT TYPE=TEXT NAME=CABPOF" + (i + 1) + " VALUE=\""
					+ ((String) theclasses.elementAt(i, 3))
					+ "\" SIZE=2 MAXLENGTH=3>" + font + "%</B></I></FONT>");
			str.append("</TD>");
			str.append("<TD WIDTH=10%>");
			str.append("<INPUT TYPE=TEXT NAME=CABMPOF" + (i + 1) + " VALUE=\""
					+ ((String) theclasses.elementAt(i, 9))
					+ "\" SIZE=2 MAXLENGTH=3>" + font + "%</B></I></FONT>");
			str.append("</TD>");
			str.append("<TD WIDTH=30%>");
			str.append("<INPUT TYPE=CHECKBOX NAME=CABQUA"
					+ (i + 1)
					+ " "
					+ (((String) theclasses.elementAt(i, 4))
							.equalsIgnoreCase("on") ? "CHECKED" : "") + ">"
					+ font + "Qualify Only</B></FONT></I>&nbsp;");
			str.append("<INPUT TYPE=CHECKBOX NAME=CABSCR"
					+ (i + 1)
					+ " "
					+ (((String) theclasses.elementAt(i, 5))
							.equalsIgnoreCase("on") ? "CHECKED" : "") + ">"
					+ font + "Secret</B></FONT></I>");
			if (!showParms)
				str.append("<INPUT TYPE=HIDDEN NAME=CABPRM" + (i + 1)
						+ " VALUE=\"" + ((String) theclasses.elementAt(i, 6))
						+ "\">");
			if (!showMasks)
				str.append("<INPUT TYPE=HIDDEN NAME=CABMSK" + (i + 1)
						+ " VALUE=\"" + ((String) theclasses.elementAt(i, 8))
						+ "\">");
			if (!showPreReqs)
				str.append("<INPUT TYPE=HIDDEN NAME=CABPRE" + (i + 1)
						+ " VALUE=\"" + ((String) theclasses.elementAt(i, 7))
						+ "\">");
			str.append("</TD>");
			str.append("</TR>");
			if (showParms)
				str.append("<TR><TD WIDTH=100% COLSPAN=5>" + sfont
						+ "Parameters: " + efont
						+ "<INPUT TYPE=TEXT NAME=CABPRM" + (i + 1)
						+ " VALUE=\"" + ((String) theclasses.elementAt(i, 6))
						+ "\" SIZE=50 MAXLENGTH=255></TD></TR>");
			if (showMasks)
				str.append("<TR><TD WIDTH=100% COLSPAN=5>" + sfont
						+ "Extra Mask: " + efont
						+ "<INPUT TYPE=TEXT NAME=CABMSK" + (i + 1)
						+ " VALUE=\"" + ((String) theclasses.elementAt(i, 8))
						+ "\" SIZE=50 MAXLENGTH=255></TD></TR>");
			if (showPreReqs)
				str.append("<TR><TD WIDTH=100% COLSPAN=5>" + sfont
						+ "Pre-Reqs list: " + efont
						+ "<INPUT TYPE=TEXT NAME=CABPRE" + (i + 1)
						+ " VALUE=\"" + ((String) theclasses.elementAt(i, 7))
						+ "\" SIZE=50 MAXLENGTH=255></TD></TR>");
		}
		str.append("<TR><TD WIDTH=40%>");
		str.append("<SELECT ONCHANGE=\"AddAffect(this);\" NAME=CABLES"
				+ (theclasses.size() + 1) + ">");
		...
		str.append("</TD>");
		str.append("<TD WIDTH=10%>");
		str.append("<INPUT TYPE=TEXT NAME=CABLVL" + (theclasses.size() + 1)
				+ " VALUE=\"\" SIZE=2 MAXLENGTH=3>");
		str.append("</TD>");
		str.append("<TD WIDTH=10%>");
		str.append("<INPUT TYPE=TEXT NAME=CABPOF" + (theclasses.size() + 1)
				+ " VALUE=\"\" SIZE=2 MAXLENGTH=3>" + font + "%</B></I></FONT>");
		str.append("</TD>");
		str.append("<TD WIDTH=10%>");
		str.append("<INPUT TYPE=TEXT NAME=CABMPOF" + (theclasses.size() + 1)
				+ " VALUE=\"\" SIZE=2 MAXLENGTH=3>" + font + "%</B></I></FONT>");
		str.append("</TD>");
		str.append("<TD WIDTH=30%>");
		str.append("<INPUT TYPE=CHECKBOX NAME=CABQUA" + (theclasses.size() + 1)
				+ " >" + font + "Qualify Only</B></I></FONT>&nbsp;");
		str.append("<INPUT TYPE=CHECKBOX NAME=CABSCR" + (theclasses.size() + 1)
				+ " >" + font + "Secret</B></I></FONT>");
		str.append("</TD>");
		str.append("</TR>");
		str.append("<TR><TD WIDTH=100% COLSPAN=5>" + sfont + "Parameters: "
				+ efont + "<INPUT TYPE=TEXT NAME=CABPRM"
				+ (theclasses.size() + 1)
				+ " VALUE=\"\" SIZE=50 MAXLENGTH=255></TD></TR>");
		str.append("<TR><TD WIDTH=100% COLSPAN=5>" + sfont + "Extra Mask: "
				+ efont + "<INPUT TYPE=TEXT NAME=CABMSK"
				+ (theclasses.size() + 1)
				+ " VALUE=\"\" SIZE=50 MAXLENGTH=255></TD></TR>");
		str.append("<TR><TD WIDTH=100% COLSPAN=5>" + sfont + "Pre-Reqs list: "
				+ efont + "<INPUT TYPE=TEXT NAME=CABPRE"
				+ (theclasses.size() + 1)
				+ " VALUE=\"\" SIZE=50 MAXLENGTH=255></TD></TR>");
		str.append("</TABLE>");
		return str;
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (last.length() > 0) {
			...
			CharClass C = (CharClass) httpReq.getRequestObjects().get(
					"CLASS-" + last);
			if ((C == null) && (newClassID != null)
					&& (newClassID.length() > 0)
					&& (CMClass.getCharClass(newClassID) == null)) {
				C = (CharClass) CMClass.getCharClass("GenCharClass").copyOf();
				C.setClassParms("<CCLASS><ID>" + newClassID + "</ID><NAME>"
						+ CMStrings.capitalizeAndLower(newClassID)
						+ "</NAME></CCLASS>");
				last = newClassID;
			...
			if (C != null) {
				...
				if (parms.containsKey("NAMES")) {
					...
					if (numNames <= 0)
						...
						for (int i = 0; i < numNames; i++) {
							String lvlStr = cSrc ? C.getStat("NAMELEVEL" + i)
									: httpReq.getUrlParameter("NAMELEVEL"
											+ (i + 1));
							if (CMath.isInteger(lvlStr)) {
								int minLevel = CMath.s_int(lvlStr);
								String name = cSrc ? C.getStat("NAME" + i)
										: httpReq.getUrlParameter("NAME"
												+ (i + 1));
								if ((name != null) && (name.length() > 0)) {
									if (nameSet.size() == 0)
										nameSet.addElement(
												Integer.valueOf(minLevel), name);
									else {
										boolean added = false;
										for (int n = 0; n < nameSet.size(); n++)
											if (minLevel < ((Integer) nameSet
													.elementAt(n, 1))
													.intValue()) {
												nameSet.insertElementAt(
														n,
														Integer.valueOf(minLevel),
														name);
												added = true;
												break;
											} else if (minLevel == ((Integer) nameSet
													.elementAt(n, 1))
													.intValue()) {
												added = true;
												break;
					...
					if (parms.containsKey("HEADERCOL1")
						...
						if (parms.containsKey("HEADERCOL1"))
							str.append(sfont + (parms.get("HEADERCOL1"))
									+ efont);
						str.append("</TD><TD WIDTH=80%>");
						if (parms.containsKey("HEADERCOL2"))
							str.append(sfont + (parms.get("HEADERCOL2"))
									+ efont);
						str.append("</TD></TR>");
					}
					for (int i = 0; i < nameSet.size(); i++) {
						...
						str.append("<TR><TD WIDTH=20%>");
						str.append("<INPUT TYPE=TEXT SIZE=5 NAME=NAMELEVEL"
								+ (i + 1) + " VALUE=\"" + lvl.toString()
								+ "\">");
						str.append("</TD><TD WIDTH=80%>");
						str.append("<INPUT TYPE=TEXT SIZE=30 NAME=NAME"
								+ (i + 1) + " VALUE=\"" + name + "\">");
						str.append("</TD></TR>");
					}
					str.append("<TR><TD WIDTH=25%>");
					str.append("<INPUT TYPE=TEXT SIZE=5 NAME=NAMELEVEL"
							+ (nameSet.size() + 1) + " VALUE=\"\">");
				...
				if (parms.containsKey("ATTATT")) {
					...
					for (int i : CharStats.CODES.BASE())
						str.append("<OPTION VALUE=\""
								+ i
								+ "\""
								+ ((CMath.s_int(old) == i) ? " SELECTED" : "")
								+ ">"
								+ CMStrings.capitalizeAndLower(CharStats.CODES
										.DESC(i)));
					str.append(", ");
				}
				...
				if (parms.containsKey("RACQUAL")) {
					...
					if (old == null)
						old = ""
								+ CMParms.toStringList(C.getRequiredRaceList());
					str.append(old + ", ");
				}
				...
				if (parms.containsKey("ARMOR")) {
					...
					for (int i = 0; i < CharClass.ARMOR_LONGDESC.length; i++)
						str.append("<OPTION VALUE=\""
								...
								+ ">"
								+ CMStrings
										.capitalizeAndLower(CharClass.ARMOR_LONGDESC[i]));
					str.append(", ");
				}
				if (parms.containsKey("SUBRUL")) {
					...
					for (CharClass.SubClassRule rule : CharClass.SubClassRule
							.values())
						str.append("<OPTION VALUE=\""
								+ rule.toString()
								+ "\""
								+ ((rule.toString().equalsIgnoreCase(old)) ? " SELECTED"
										: "") + ">"
								+ CMStrings.capitalizeAndLower(rule.toString()));
					str.append(", ");
				}
				...
				if (parms.containsKey("PLAYER")) {
					...
					for (int i = 0; i < Area.THEME_PHRASE_EXT.length; i++)
						str.append("<OPTION VALUE=" + i + " "
								+ ((i == mask) ? "SELECTED" : "") + ">"
								+ Area.THEME_PHRASE_EXT[i]);
					str.append(", ");
				}
				if (parms.containsKey("ESTATS") || parms.containsKey("CSTATS")
						|| parms.containsKey("ASTATS")
						|| parms.containsKey("ASTATE")
						|| parms.containsKey("STARTASTATE")) {
					C = C.makeGenCharClass();

					if (parms.containsKey("ESTATS")) {
						String eStats = C.getStat("ESTATS");
						PhyStats adjPStats = (PhyStats) CMClass
								.getCommon("DefaultPhyStats");
						adjPStats.setAllValues(0);
						if (eStats.length() > 0) {
							CMLib.coffeeMaker().setPhyStats(adjPStats, eStats);
						}
						str.append(RaceData.estats(adjPStats, 'E', httpReq,
								parms, 0) + ", ");
					}
					if (parms.containsKey("CSTATS")) {
						CharStats setStats = (CharStats) CMClass
						...
						str.append(RaceData.cstats(setStats, 'S', httpReq,
								parms, 0) + ", ");
					}
					if (parms.containsKey("ASTATS")) {
						CharStats adjStats = (CharStats) CMClass
								.getCommon("DefaultCharStats");
						...
						str.append(RaceData.cstats(adjStats, 'A', httpReq,
								parms, 0) + ", ");
					}
					if (parms.containsKey("ASTATE")) {
						CharState adjState = (CharState) CMClass
						...
						str.append(RaceData.cstate(adjState, 'A', httpReq,
								parms, 0) + ", ");
					}
					if (parms.containsKey("STARTASTATE")) {
						CharState startAdjState = (CharState) CMClass
								.getCommon("DefaultCharState");
						startAdjState.setAllValues(0);
						String saState = C.getStat("STARTASTATE");
						if (saState.length() > 0) {
							CMLib.coffeeMaker().setCharState(startAdjState,
									saState);
						}
						str.append(RaceData.cstate(startAdjState, 'S', httpReq,
								parms, 0) + ", ");
					}
				}
				if (parms.containsKey("NOWEAPS")) {
					...
					if (old == null) {
						...
						String id = "";
						set = new Vector();
						for (int i = 0; httpReq.isUrlParameter("NOWEAPS" + id); id = ""
								+ (++i))
				...
				if (parms.containsKey("MINSTAT")) {
					...
					if (old == null) {
						for (Pair<String, Integer> P : C
								.getMinimumStatRequirements())
							minStats.add(P);
					} else {
						int x = 0;
						while (httpReq.getUrlParameter("MINSTAT" + x) != null) {
							String minStat = httpReq.getUrlParameter("MINSTAT"
									+ x);
							String statMin = httpReq.getUrlParameter("STATMIN"
									+ x);
							if ((minStat != null) && (minStat.length() > 0)
									&& (CMath.isInteger(statMin)))
								minStats.add(new Pair<String, Integer>(minStat,
										Integer.valueOf(CMath.s_int(statMin))));
							x++;
						}
					}
					for (int p = 0; p < minStats.size(); p++) {
						Pair<String, Integer> P = minStats.get(p);
						str.append("<SELECT NAME=MINSTAT").append(p)
								.append(">");
						str.append("<OPTION VALUE=\"\">Delete");
						str.append("<OPTION SELECTED VALUE=\"").append(P.first)
								.append("\">" + P.first);
						str.append("</SELECT> Min Value: <INPUT NAME=STATMIN")
								.append(p).append(" VALUE=")
								.append(P.second.toString()).append(">");
						str.append("<BR>");
					}
					str.append("<SELECT NAME=MINSTAT").append(minStats.size())
							.append(">");
					for (String statName : CharStats.CODES.BASENAMES())
						str.append("<OPTION VALUE=\"")
								.append(CMStrings.capitalizeAndLower(statName))
								.append("\">")
								.append(CMStrings.capitalizeAndLower(statName));
					str.append("</SELECT> Min Value: <INPUT NAME=STATMIN")
							.append(minStats.size()).append(" VALUE=\"\">");
					str.append("<INPUT TYPE=BUTTON NAME=ADDSTATMIN VALUE=Add ONCLICK=\"ReShow();\">");
				}
				if (parms.containsKey("OUTFIT"))
					str.append(RaceData.itemList(C.outfit(null), 'O', httpReq,
							parms, 0, false) + ", ");
				if (parms.containsKey("DISFLAGS")) {
					if (!httpReq.isUrlParameter("DISFLAGS")) {
						C = C.makeGenCharClass();
						httpReq.addFakeUrlParameter("DISFLAGS",
								C.getStat("DISFLAGS"));
					}
					int flags = CMath
							.s_int(httpReq.getUrlParameter("DISFLAGS"));
				...
				if (parms.containsKey("SECURITYSETS")) {
					...
					for (int i = 0; i < numSSet; i++) {
						String lvlStr = cSrc ? C.getStat("SSETLEVEL" + i)
								: httpReq
										.getUrlParameter("SSETLEVEL" + (i + 1));
						if (CMath.isInteger(lvlStr)) {
							int minLevel = CMath.s_int(lvlStr);
							String sec = null;
							if (cSrc) {
							...
							if ((sec != null)
									&& (sec.trim().length() > 0)
									&& (CMParms.parseCommas(sec, true).size() > 0)) {
								sec = CMParms
										.combineWithX(CMParms.parseCommas(sec
												.toUpperCase().trim(), true),
												",", 0);
								if (sSet.size() == 0)
									sSet.addElement(Integer.valueOf(minLevel),
											sec);
								else {
									boolean added = false;
									for (int n = 0; n < sSet.size(); n++)
										if (minLevel < ((Integer) sSet
												.elementAt(n, 1)).intValue()) {
											sSet.insertElementAt(n,
													Integer.valueOf(minLevel),
													sec);
											added = true;
											break;
										} else if (minLevel == ((Integer) sSet
												.elementAt(n, 1)).intValue()) {
											added = true;
											break;
										}
					...
					for (int i = 0; i < sSet.size(); i++) {
						...
						str.append("<TR><TD WIDTH=20%>");
						str.append("<INPUT TYPE=TEXT SIZE=5 NAME=SSETLEVEL"
								+ (i + 1) + " VALUE=\"" + lvl.toString()
								+ "\">");
						str.append("</TD><TD WIDTH=80%>");
						str.append("<INPUT TYPE=TEXT SIZE=60 NAME=SSET"
								+ (i + 1) + " VALUE=\"" + sec + "\">");
						str.append("</TD></TR>");
					}
					str.append("<TR><TD WIDTH=25%>");
				...
				if (parms.containsKey("WEAPMATS")) {
					...
					if (old == null) {
						...
						String id = "";
						set = new Vector();
						for (int i = 0; httpReq.isUrlParameter("WEAPMATS" + id); id = ""
								+ (++i))
							if (CMath.isInteger(httpReq
									.getUrlParameter("WEAPMATS" + id)))
								set.add(httpReq
										.getUrlParameter("WEAPMATS" + id));
					}
					str.append("<OPTION VALUE=\"*\"");
				...
				if (parms.containsKey("CABILITIES"))
					str.append(cabilities(mob, C, httpReq, parms, 1,
							"<FONT SIZE=-1 COLOR=WHITE>"));
				/******************************************************/
				// Here begins the displayable only fields.
				/******************************************************/
				if (parms.containsKey("HELP")) {
					StringBuilder s = CMLib.help().getHelpText(C.ID(), null,
							false, true);
					if (s == null)
				...
				if (parms.containsKey("QUALDOMAINLIST")) {
					...
					for (Enumeration<Ability> e = CMClass.abilities(); e
							.hasMoreElements();) {
						A = e.nextElement();
						if (CMLib.ableMapper().getQualifyingLevel(C.ID(), true,
								A.ID()) > 0) {
							if ((A.classificationCode() & Ability.ALL_DOMAINS) == 0)
								domain = Ability.ACODE_DESCS[A
										.classificationCode()];
							else
								domain = Ability.DOMAIN_DESCS[(A
										.classificationCode() & Ability.ALL_DOMAINS) >> 5];
					...
					while (domains.size() > 0) {
						winner = null;
						winnerI = null;
						for (Enumeration e = domains.keys(); e
								.hasMoreElements();) {
							domain = (String) e.nextElement();
							I = (Integer) domains.get(domain);
							if ((winnerI == null)
									|| (I.intValue() > winnerI.intValue())) {
								winner = domain;
								winnerI = I;
							}
						}
				...
				if (parms.containsKey("AVGHITPOINTS")) {
					...
					str.append("(" + avgMath(10, 10, 20, f) + "/"
							+ avgMath(18, 10, 20, f) + "/"
							+ avgMath(maxCon, 10, 20, f) + ") ");
					str.append("(" + avgMath(10, 50, 20, f) + "/"
							+ avgMath(18, 50, 20, f) + "/"
							+ avgMath(maxCon, 50, 20, f) + ") ");
					str.append("(" + avgMath(10, 90, 20, f) + "/"
							+ avgMath(18, 90, 20, f) + "/"
							+ avgMath(maxCon, 90, 20, f) + ") ");
				}

				if (parms.containsKey("AVGMANA")) {
					int maxInt = 18 + C.maxStatAdjustments()[CharStats.STAT_INTELLIGENCE];
					String f = C.getManaFormula();
					str.append("(" + avgMath(10, 10, 100, f) + "/"
							+ avgMath(18, 10, 100, f) + "/"
							+ avgMath(maxInt, 10, 100, f) + ") ");
					str.append("(" + avgMath(10, 50, 100, f) + "/"
							+ avgMath(18, 50, 100, f) + "/"
							+ avgMath(maxInt, 50, 100, f) + ") ");
					str.append("(" + avgMath(10, 90, 100, f) + "/"
							+ avgMath(18, 90, 100, f) + "/"
							+ avgMath(maxInt, 90, 100, f) + ") ");
				}
				if (parms.containsKey("AVGMOVEMENT")) {
					...
					str.append("(" + avgMath(10, 10, 100, f) + "/"
							+ avgMath(18, 10, 100, f) + "/"
							+ avgMath(maxStrength, 10, 100, f) + ") ");
					str.append("(" + avgMath(10, 50, 100, f) + "/"
							+ avgMath(18, 50, 100, f) + "/"
							+ avgMath(maxStrength, 50, 100, f) + ") ");
					str.append("(" + avgMath(10, 90, 100, f) + "/"
							+ avgMath(18, 90, 100, f) + "/"
							+ avgMath(maxStrength, 90, 100, f) + ") ");
				}
				...
				if (parms.containsKey("LIMITS")) {
					...
					if (C.getLevelCap() > 0)
						limits.append("  A player may not gain more than "
								+ C.getLevelCap() + " levels in this class.");
					if (limits.length() == 0)
						str.append("None, ");
					else
	...
	public String balanceChart(CharClass C) {
		...
		for (int l = 1; l <= 30; l++) {
			List<String> set = CMLib.ableMapper().getLevelListings(C.ID(),
					true, l);
			for (int s = 0; s < set.size(); s++) {
				...
				for (Enumeration c = CMClass.charClasses(); c.hasMoreElements();) {
					...
					if (C2.baseClass().equals(C.baseClass())) {
						tlvl = CMLib.ableMapper().getQualifyingLevel(C2.ID(),
								true, able);
						if (tlvl > 0) {
							if (tlvl > l)
		...
		StringBuffer str = new StringBuffer("");
		str.append("<BR>Rule#1: Avg gained skill/level: "
				+ CMath.div(Math.round(100.0 * CMath.div(totalgained, 30)),
						(long) 100));
		str.append("<BR>Rule#2: Avg qualified skill/level: "
				+ CMath.div(Math.round(100.0 * CMath.div(totalqualified, 30)),
						(long) 100));
		str.append("<BR>Rule#3: Unique class skills gained: "
				+ uniqueClassSkillsGained + "/" + uniqueClassSkills);
		str.append("<BR><FONT COLOR=WHITE>Rule#3</FONT>: Unique class skills: "
				+ CMParms.toStringList(uniqueClassSkillsV));
		str.append("<BR>Rule#4: Uncommon class skills gained: "
				+ uncommonClassSkillsGained + "/" + uncommonClassSkills);
		str.append("<BR><FONT COLOR=WHITE>Rule#4</FONT>: Uncommon class skills: "
				+ CMParms.toStringList(uncommonClassSkillsV));
		str.append("<BR>Rule#5: Combat skills gained: "
				+ (maliciousSkillsGained + beneficialSkillsGained) + "/"
				+ (maliciousSkills + beneficialSkills));
		str.append("<BR>Rule#6: Avg Unique class skill/level: "
				+ CMath.div(
						Math.round(100.0 * CMath.div(uniqueClassSkills, 30)),
						(long) 100));
		str.append("<BR>Rule#7: CrossClass class skills diff: "
				+ totalCrossClassLevelDiffs + "/" + totalCrossClassSkills);
		str.append("<BR><FONT COLOR=WHITE>Rule#7</FONT>: CrossClass class skills: "
				+ CMParms.toStringList(totalCrossClassSkillsV));
		str.append("<BR>Rule#8: Avg Cross class skill/level: "
				+ CMath.div(Math.round(100.0 * CMath.div(totalCrossClassSkills,
						30)), (long) 100));
		M.destroy();
		return str.toString();
	}
	...
	public int avgMath(int stat, int level, int add, String formula) {
		double[] variables = { level, stat, (double) stat + 7, stat,
				(double) stat + 7, stat, (double) stat + 7, stat, stat };
		return add
				+ (level * (int) Math.round(CMath.parseMathExpression(formula,
						variables)));
	}
