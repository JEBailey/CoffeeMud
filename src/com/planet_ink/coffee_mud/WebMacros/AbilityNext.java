...
public class AbilityNext extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (!parms.containsKey("SORTEDBYNAME"))
			a = CMClass.abilities();
		else if (httpReq.getRequestObjects().containsKey(
				"ABILITIESSORTEDBYNAME"))
			a = ((Vector) httpReq.getRequestObjects().get(
					"ABILITIESSORTEDBYNAME")).elements();
		else {
			Vector<Ability> fullList = new Vector<Ability>();
			for (Enumeration<Ability> aa = CMClass.abilities(); aa
					.hasMoreElements();)
		...
		for (; a.hasMoreElements();) {
			...
			if (genericOnly)
				...
				okToShow = ((A instanceof ItemCraftor)
						...
						&& (((ItemCraftor) A).parametersFormat() != null) && (((ItemCraftor) A)
						.parametersFormat().length() > 0));

			if ((className != null) && (className.length() > 0)) {
				int level = CMLib.ableMapper().getQualifyingLevel(className,
						true, A.ID());
				if ((level < 0) && (!unqualifiedOK))
					okToShow = false;
				else if (CMLib.ableMapper().getSecretSkill(className, false,
						A.ID()))
					okToShow = false;
				...
				int level = CMLib.ableMapper().getQualifyingLevel("Archon",
						true, A.ID());
				if ((level < 0) && (!unqualifiedOK))
					okToShow = false;
			...
			if (okToShow) {
				if ((domainFlag)
						&& (!domain
								.equalsIgnoreCase(Ability.DOMAIN_DESCS[(A
