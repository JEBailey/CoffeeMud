	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (parms.containsKey("DISABLENEXT")) {
			...
			for (CMSecurity.DisFlag flag : CMSecurity.DisFlag.values()) {
				if ((lastDisable == null)
						|| ((lastDisable.length() > 0)
								&& (lastDisable.equals(lastID)) && (!flag
								.toString().equals(lastID)))) {
		...
		if (parms.containsKey("DISABLEDESC")) {
			if (lastDisable == null)
				return " @break@";
			CMSecurity.DisFlag flag = (CMSecurity.DisFlag) CMath.s_valueOf(
					CMSecurity.DisFlag.values(), lastDisable);
			if (flag == null)
				return " @break@";
		...
		if (parms.containsKey("DEBUGDESC")) {
			...
			CMSecurity.DbgFlag flag = (CMSecurity.DbgFlag) CMath.s_valueOf(
					CMSecurity.DbgFlag.values(), lastDebug);
			if (flag == null)
				return " @break@";
			return flag.description();
		}
		...
		if (query.equalsIgnoreCase("DISABLE")) {
			...
			if ((field == null) || (field.length() == 0))
				return "";
			CMSecurity.DisFlag flag = (CMSecurity.DisFlag) CMath.s_valueOf(
					CMSecurity.DisFlag.values(), field.toUpperCase().trim());
			...
			if ((field == null) || (field.length() == 0))
				return "";
			CMSecurity.DbgFlag flag = (CMSecurity.DbgFlag) CMath.s_valueOf(
					CMSecurity.DbgFlag.values(), field.toUpperCase().trim());
			...
			CMSecurity.setDisableVar(field,
					((value != null) && (value.equalsIgnoreCase("on"))));
			return "";
			...
			if ((field == null) || (field.length() == 0))
				return "";
			String value = parms.get("VALUE");
			DbgFlag flag = CMSecurity.setDebugVar(field,
					((value != null) && (value.equalsIgnoreCase("on"))));
			if ((Thread.currentThread() instanceof MWThread)
				...
				if (CMSecurity.isDebugging(DbgFlag.HTTPACCESS))
					config.setAccessLogFlag(CMProps.instance()
							.getStr("ACCMSGS"));
			}
			return "";
