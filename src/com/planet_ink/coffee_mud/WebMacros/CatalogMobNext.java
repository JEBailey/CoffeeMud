...
public class CatalogMobNext extends StdWebMacro {
	...
	final static String[] DATA = { "CATALOG_MOB_USAGE", "CATALOG_MOB_NAME",
			"CATALOG_MOB_AREA", "CATALOG_MOB_RACE", "CATALOG_MOB_GENDER",
			"CATALOG_MOB_LEVEL", "CATALOG_MOB_CLASS" };

	public static String getCataStat(MOB M, CatalogLibrary.CataData data,
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if ((sortBy != null) && (sortBy.length() > 0)) {
			final int sortIndex = CMParms.indexOf(DATA,
					"CATALOG_MOB_" + sortBy.toUpperCase());
			if ((sortIndex >= 0) || (sortBy.equalsIgnoreCase(optionalColumn))) {
				String[] sortedNames = (String[]) httpReq.getRequestObjects()
						.get("CATALOG_MOB_" + catagory + "_"
								+ sortBy.toUpperCase());
				if (sortedNames != null)
					names = sortedNames;
				else {
					...
					Arrays.sort(sortifiable, new Comparator() {
						public int compare(Object o1, Object o2) {
							Object[] O1 = (Object[]) o1;
							Object[] O2 = (Object[]) o2;
							String s1 = getCataStat((MOB) O1[1],
									(CatalogLibrary.CataData) O1[2], sortIndex,
									optionalColumn);
							String s2 = getCataStat((MOB) O2[1],
			...
			if ((last == null)
				...
				for (int d = 0; d < DATA.length; d++)
					httpReq.addFakeUrlParameter(DATA[d],
							getCataStat(M, data, d, null));
				if (optionalColumn.length() > 0)
					httpReq.addFakeUrlParameter(
							"CATALOG_MOB_" + optionalColumn,
							getCataStat(M, data, -1, optionalColumn));
				return "";
			}
			lastID = name;
		}
