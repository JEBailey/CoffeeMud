...
public class JournalInfo extends StdWebMacro {
	...
	public static JournalsLibrary.JournalEntry getEntry(
			List<JournalsLibrary.JournalEntry> msgs, String key) {
		if (msgs == null)
			return null;
		if (key == null)
			return null;
		for (Iterator<JournalsLibrary.JournalEntry> e = msgs.iterator(); e
				.hasNext();) {
	...
	public static List<JournalsLibrary.JournalEntry> getMessages(
			String journalName, String page, String mpage, String parent,
			String dbsearch, Map<String, Object> objs) {
		...
		if ((dbsearch != null) && (dbsearch.length() > 0))
			...
			parent = "";
		String httpkey = "JOURNAL: " + journalName + ": " + parent + ": "
				+ dbsearch + ": " + page;
		List<JournalsLibrary.JournalEntry> msgs = (List<JournalsLibrary.JournalEntry>) objs
				.get(httpkey);
		if (msgs == null) {
			if ((page == null) || (page.length() == 0))
				...
				if ((pageDate <= 0) && (stats.stuckyKeys != null)
						&& ((dbsearch == null) || (dbsearch.length() == 0))
						&& ((parent != null) && (parent.length() == 0))) {
					for (String stuckyKey : stats.stuckyKeys) {
						JournalsLibrary.JournalEntry entry = CMLib.database()
								.DBReadJournalEntry(journalName, stuckyKey);
						if (entry != null)
							msgs.add(entry);
					}
				}
				msgs.addAll(CMLib.database().DBReadJournalPageMsgs(journalName,
						parent, dbsearch, pageDate, limit));
				// if((dbsearch!=null)&&(dbsearch.length()>0)) // parent
				// filtering
	...
	public static void clearJournalCache(HTTPRequest httpReq, String journalName) {
		List<String> h = new XVector<String>(httpReq.getRequestObjects()
				.keySet());
		for (String o : h)
	...
	public static JournalsLibrary.JournalEntry getNextEntry(
			List<JournalsLibrary.JournalEntry> info, String key) {
		if (info == null)
			return null;
		for (Iterator<JournalsLibrary.JournalEntry> e = info.iterator(); e
				.hasNext();) {
			JournalsLibrary.JournalEntry entry = e.next();
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (msgKey.equalsIgnoreCase("FORUMLATEST")) {
			JournalsLibrary.JournalSummaryStats stats = CMLib.journals()
					.getJournalStats(journalName);
			if ((stats != null) && (stats.latestKey != null)
					&& (stats.latestKey.length() > 0)) {
				entry = CMLib.database().DBReadJournalEntry(journalName,
						stats.latestKey);
				if (entry != null)
			...
			if ((page != null) && (page.length() > 0)) {
				List<JournalsLibrary.JournalEntry> msgs = JournalInfo
						.getMessages(journalName, page, mpage, parent,
								dbsearch, httpReq.getRequestObjects());
		...
		boolean priviledged = CMSecurity.isAllowedAnywhere(M,
				CMSecurity.SecFlag.JOURNALS) && (!parms.containsKey("NOPRIV"));
		String to = entry.to;
		if (to.equalsIgnoreCase("all")
				|| ((M != null) && (priviledged
						|| to.equalsIgnoreCase(M.Name()) || (to.toUpperCase()
						.trim().startsWith("MASK=") && (CMLib.masking()
						.maskCheck(to.trim().substring(5), M, true)))))) {
			if (parms.containsKey("CANEDIT")) {
				JournalsLibrary.ForumJournal forum = CMLib.journals()
						.getForumJournal(journalName);
				if (M == null)
					return "false";
				return String.valueOf(entry.from.equals(M.Name())
						|| priviledged
						|| (forum != null && forum.authorizationCheck(M,
								ForumJournalFlags.ADMIN)));
			} else if (parms.containsKey("QUOTEDTEXT")
					&& httpReq.isUrlParameter("QUOTEDMESSAGE")) {
				String quotedMessage = httpReq.getUrlParameter("QUOTEDMESSAGE");
				if ((quotedMessage == null) || (quotedMessage.length() == 0))
					return "";
				JournalsLibrary.JournalEntry quotedEntry = CMLib.database()
						.DBReadJournalEntry(journalName, quotedMessage);
				return "<P><BLOCKQUOTE style=\"border : solid #000 1px; padding : 3px; margin-left: 1em; margin-bottom:0.2em; background: #f9f9f9 none; color: #000;\">"
						+ "<FONT SIZE=-1>Quoted from "
						+ quotedEntry.from
						+ " &nbsp;("
						+ CMLib.time().date2String(quotedEntry.date)
						+ "):</FONT><HR>"
						+ "<I>"
						+ quotedEntry.msg
						+ "</I></BLOCKQUOTE></P>";
			...
			else if (parms.containsKey("ISSTICKY")
					|| parms.containsKey("ISSTUCKY"))
				return String.valueOf(CMath.bset(entry.attributes,
				...
				if (Calendar.getInstance().get(Calendar.YEAR) != meC
						.get(Calendar.YEAR))
					return CMLib.time().date2Date2String(entry.update);
				String dateString = CMLib.time().date2MonthDateString(
						entry.date, false);
				String todayString = CMLib.time().date2MonthDateString(
						System.currentTimeMillis(), false);
				if (dateString.equals(todayString))
					return "Today";
				C.add(Calendar.DATE, -1);
				String yesterdayString = CMLib.time().date2MonthDateString(
				...
				if (parms.containsKey("PLAIN")) {
					...
					s = CMStrings.replaceAll(s,
							JournalsLibrary.JOURNAL_BOUNDARY, "<HR>");
				...
				if ((entry.parent == null) || (entry.parent.length() == 0))
					CMLib.database().DBViewJournalMessage(entry.key,
							++entry.views);
				return s;
			}
			if (parms.containsKey("EMAILALLOWED"))
				return ""
						+ ((entry.from.length() > 0) && (CMProps.getVar(
