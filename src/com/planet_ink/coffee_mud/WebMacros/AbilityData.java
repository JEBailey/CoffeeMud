...
public class AbilityData extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if ((A == null) && (newCraftSkillID != null)
			...
			if (A != null) {
				...
				if (A instanceof Language) {
					if (parms.containsKey("WORDLISTS")) {
						List<String[]> wordLists = ((Language) A)
								.translationVector(A.ID());
						if (httpReq.isUrlParameter("WORDLIST1")) {
							...
							while (httpReq.isUrlParameter("WORDLIST" + x)) {
								wordLists
										.add(CMParms
												.parseCommas(
														httpReq.getUrlParameter("WORDLIST"
																+ x), true)
							...
							for (int i = wordLists.size() - 1; i >= 0; i--)
								httpReq.addFakeUrlParameter("WORDLIST"
										+ (i + 1),
										CMParms.toStringList(wordLists.get(i)));
							httpReq.removeUrlParameter("WORDLIST"
									+ (wordLists.size() + 1));
						}
						...
						if (parms.containsKey("RESET")) {
							...
							String lastID = "";
							String lastNum = httpReq
									.getUrlParameter("WORDLISTNUM");
							String nextName = "WORDLIST1";
							for (int i = 0; i < wordLists.size(); i++) {
								String thisName = "WORDLIST"
										+ Integer.toString(i + 1);
								nextName = "WORDLIST" + Integer.toString(i + 2);
								if ((lastNum == null)
										|| ((lastNum.length() > 0)
												&& (lastNum.equals(lastID)) && (!thisName
													.equals(lastID)))) {
							...
							httpReq.addFakeUrlParameter("WORDLISTNUM", "");
							httpReq.addFakeUrlParameter("WORDLISTNEXT",
									nextName);
					...
					if (parms.containsKey("HASHWORDS")) {
						Map<String, String> hashWords = ((Language) A)
								.translationHash(A.ID());
						if (httpReq.isUrlParameter("HASHWORD1")) {
							hashWords = new Hashtable<String, String>();
							int x = 1;
							while (httpReq.isUrlParameter("HASHWORD" + x)) {
								String word = httpReq
										.getUrlParameter("HASHWORD" + x)
										.toUpperCase().trim();
								String def = httpReq
										.getUrlParameter("HASHWORDDEF" + x);
							...
							for (String key : hashWords.keySet()) {
								...
								httpReq.addFakeUrlParameter("HASHWORDDEF" + x,
										hashWords.get(key));
								x++;
							}
						...
						if (parms.containsKey("RESET")) {
							...
							httpReq.addFakeUrlParameter("HASHWORDNEXT",
									nextName);
							httpReq.addFakeUrlParameter("HASHWORDDEFNEXT",
									nextDefName);
							if (parms.containsKey("EMPTYOK"))
								return "<!--EMPTY-->";
							return " @break@";
				...
				if (parms.containsKey("CLASSIFICATION_ACODE")) {
					...
					if (old == null)
						old = ""
								+ (A.classificationCode() & Ability.ALL_ACODES);
				...
				if (parms.containsKey("CLASSIFICATION_DOMAIN")) {
					String old = httpReq
							.getUrlParameter("CLASSIFICATION_DOMAIN");
					if (old == null)
						old = ""
								+ ((A.classificationCode() & Ability.ALL_DOMAINS) >> 5);
				...
				if (parms.containsKey("MINRANGE")) {
					...
					for (int i = 0; i < Ability.RANGE_CHOICES.length; i++)
						str.append("<OPTION VALUE=\""
								...
								+ ">"
								+ CMStrings
										.capitalizeAndLower(Ability.RANGE_CHOICES[i]));
					str.append(", ");
				}
				...
				if (parms.containsKey("ABILITY_FLAGS")) {
					...
					if (httpReq.isUrlParameter("ABILITY_FLAGS")) {
						String id = "";
						int num = 0;
						for (; httpReq.isUrlParameter("ABILITY_FLAGS" + id); id = ""
								+ (++num))
							list.add(httpReq.getUrlParameter("ABILITY_FLAGS"
									+ id));
					} else
					...
					for (int i = 0; i < Ability.FLAG_DESCS.length; i++)
						str.append("<OPTION VALUE=\""
								...
								+ "\""
								+ (list.contains(Ability.FLAG_DESCS[i]) ? " SELECTED"
										: "")
								+ ">"
								+ CMStrings
				...
				if (parms.containsKey("OVERRIDEMANA")) {
					...
					str.append("<OPTION VALUE=\"-1\""
							+ ((o == -1) ? " SELECTED" : "") + ">Use Default");
					str.append("<OPTION VALUE=\"0\""
							+ ((o == 0) ? " SELECTED" : "")
							+ ">None (free skill)");
					str.append("<OPTION VALUE=\"\""
							+ (((o > 0) && (o < Ability.COST_PCT)) ? " SELECTED"
									: "") + "\">Custom Value");
					str.append("<OPTION VALUE=\"" + Ability.COST_ALL + "\""
							+ ((o == Ability.COST_ALL) ? " SELECTED" : "")
							+ ">All Mana");
				...
				if (parms.containsKey("USAGEMASK")) {
					...
					if (httpReq.isUrlParameter("USAGEMASK")) {
						String id = "";
						int num = 0;
						for (; httpReq.isUrlParameter("USAGEMASK" + id); id = ""
								+ (++num))
							list.add(httpReq.getUrlParameter("USAGEMASK" + id));
					} else
						list = CMParms
								.parseCommas(A.getStat("USAGEMASK"), true);
					for (int i = 0; i < Ability.USAGE_DESCS.length; i++)
						str.append("<OPTION VALUE=\""
								+ Ability.USAGE_DESCS[i]
								+ "\""
								+ (list.contains(Ability.USAGE_DESCS[i]) ? " SELECTED"
										: "")
								+ ">"
								+ CMStrings
										.capitalizeAndLower(Ability.USAGE_DESCS[i]));
					str.append(", ");
				}
				if (parms.containsKey("CANAFFECTMASK")) {
					...
					if (httpReq.isUrlParameter("CANAFFECTMASK")) {
						String id = "";
						int num = 0;
						for (; httpReq.isUrlParameter("CANAFFECTMASK" + id); id = ""
								+ (++num))
							list.add(httpReq.getUrlParameter("CANAFFECTMASK"
									+ id));
					} else
						list = CMParms.parseCommas(A.getStat("CANAFFECTMASK"),
								true);
					for (int i = 0; i < Ability.CAN_DESCS.length; i++)
						str.append("<OPTION VALUE=\""
								+ Ability.CAN_DESCS[i]
								+ "\""
								+ (list.contains(Ability.CAN_DESCS[i]) ? " SELECTED"
										: "")
								+ ">"
								+ CMStrings
										.capitalizeAndLower(Ability.CAN_DESCS[i]));
					str.append(", ");
				}
				if (parms.containsKey("CANTARGETMASK")) {
					...
					if (httpReq.isUrlParameter("CANTARGETMASK")) {
						String id = "";
						int num = 0;
						for (; httpReq.isUrlParameter("CANTARGETMASK" + id); id = ""
								+ (++num))
							list.add(httpReq.getUrlParameter("CANTARGETMASK"
									+ id));
					} else
						list = CMParms.parseCommas(A.getStat("CANTARGETMASK"),
								true);
					for (int i = 0; i < Ability.CAN_DESCS.length; i++)
						str.append("<OPTION VALUE=\""
								+ Ability.CAN_DESCS[i]
				...
				if (parms.containsKey("VQUALITY")) // QUALITY
				{
					...
					for (int i = 0; i < Ability.QUALITY_DESCS.length; i++)
						str.append("<OPTION VALUE=\""
								...
								+ ">"
								+ CMStrings
										.capitalizeAndLower(Ability.QUALITY_DESCS[i]));
					str.append(", ");
				}
				if (parms.containsKey("HERESTATS")) // affect adj:
													// Prop_HereAdjuster
				{
				...
				if (parms.containsKey("ATTACKCODE")) {
					...
					if (old == null)
						old = ""
								+ CMParms.indexOf(CMMsg.TYPE_DESCS,
										A.getStat("ATTACKCODE"));
					for (int i = 0; i < CMMsg.TYPE_DESCS.length; i++)
						str.append("<OPTION VALUE=\""
								...
								+ ">"
								+ CMStrings
										.capitalizeAndLower(CMMsg.TYPE_DESCS[i]));
					str.append(", ");
				}
				...
				if (parms.containsKey("MATLIST")) {
					...
					if (httpReq.isUrlParameter("MATLIST")) {
						String id = "";
						int num = 0;
						for (; httpReq.isUrlParameter("MATLIST" + id); id = ""
								+ (++num))
							list.add(httpReq.getUrlParameter("MATLIST" + id)
									.toUpperCase().trim());
					} else
						list = CMParms.parseCommas(A.getStat("MATLIST"), true);
					for (RawMaterial.Material m : RawMaterial.Material.values())
						str.append("<OPTION VALUE=\"" + m.name() + "\""
								+ (list.contains(m.name()) ? " SELECTED" : "")
								+ ">" + m.noun());
					for (int i = 0; i < RawMaterial.CODES.NAMES().length; i++)
						str.append("<OPTION VALUE=\""
								+ RawMaterial.CODES.NAMES()[i]
								+ "\""
								+ (list.contains(RawMaterial.CODES.NAMES()[i]) ? " SELECTED"
										: "")
								+ ">"
								+ CMStrings
				...
				if (parms.containsKey("POSTCASTAFFECT")) {
					...
					if (httpReq.isUrlParameter("POSTCASTAFFECT")) {
						String id = "";
						int num = 0;
						for (; httpReq.isUrlParameter("POSTCASTAFFECT" + id); id = ""
								+ (++num))
							list.add(httpReq.getUrlParameter(
									"POSTCASTAFFECT" + id).toUpperCase());
					} else
						list = CMParms
								.parseSemicolons(A.getStat("POSTCASTAFFECT")
										.toUpperCase(), true);
					for (Enumeration<Ability> e = CMClass.abilities(); e
						...
						str.append("<OPTION VALUE=\""
								+ AID
								+ "\""
								+ (list.contains(AID.toUpperCase()) ? " SELECTED"
										: "") + ">" + ANAME);
					}
					str.append(", ");
				}
				if (parms.containsKey("POSTCASTABILITY")) {
					...
					if (httpReq.isUrlParameter("POSTCASTABILITY")) {
						String id = "";
						int num = 0;
						for (; httpReq.isUrlParameter("POSTCASTABILITY" + id); id = ""
								+ (++num))
							list.add(httpReq.getUrlParameter(
									"POSTCASTABILITY" + id).toUpperCase());
					} else
						list = CMParms.parseSemicolons(
								A.getStat("POSTCASTABILITY").toUpperCase(),
								true);
					for (Enumeration<Ability> e = CMClass.abilities(); e
							.hasMoreElements();) {
						Ability A2 = e.nextElement();
				...
				if (parms.containsKey("POSTCASTDAMAGE")) {
					/*
					 * Enter a damage or healing formula. Use +/-*()?.
					 * @x1=caster level, @x2=target level. Formula evaluates >0
					 * for damage, <0 for healing. Requires Can Target!"
					 */
				...
				if (parms.containsKey("HELP")) {
					StringBuilder s = CMLib.help().getHelpText(A.ID(), null,
							false);
					if (s == null)
				...
				if (parms.containsKey("ALLOWS")) {
					...
					for (Iterator<String> i = CMLib.ableMapper()
							.getAbilityAllowsList(A.ID()); i.hasNext();) {
				...
				if (parms.containsKey("DOMAIN")) {
					...
					if ((A.classificationCode() & Ability.ALL_DOMAINS) != 0) {
						int domain = A.classificationCode()
								& Ability.ALL_DOMAINS;
						domain = domain >> 5;
						thang.append(Ability.DOMAIN_DESCS[domain].toLowerCase()
								.replace('_', ' '));
					} else
						thang.append(Ability.ACODE_DESCS[A.classificationCode()
								& Ability.ALL_ACODES].toLowerCase());
					if (thang.length() > 0) {
						thang.setCharAt(0,
								Character.toUpperCase(thang.charAt(0)));

						int x = thang.toString().indexOf('/');
						if (x > 0)
							thang.setCharAt(x + 1,
									Character.toUpperCase(thang.charAt(x + 1)));
						str.append(thang.toString() + ", ");
					}
				}
				if (parms.containsKey("TYPENDOMAIN")) {
					StringBuffer thang = new StringBuffer("");
					thang.append(CMStrings
							.capitalizeAndLower(Ability.ACODE_DESCS[A
					...
					if ((A.classificationCode() & Ability.ALL_DOMAINS) != 0) {
						...
						thang.append(": "
								+ CMStrings.capitalizeAndLower(
										Ability.DOMAIN_DESCS[domain]).replace(
										'_', ' '));
					}
				...
				if (parms.containsKey("QLEVEL")) {
					...
					if ((className != null) && (className.length() > 0))
						level = CMLib.ableMapper().getQualifyingLevel(
								className, true, A.ID());
					else
						level = CMLib.ableMapper().getQualifyingLevel("Archon",
								true, A.ID());
					str.append(level + ", ");
				}
				if (parms.containsKey("QUALIFYQ")
						&& (httpReq.isUrlParameter("CLASS"))) {
					String className = httpReq.getUrlParameter("CLASS");
					if ((className != null) && (className.length() > 0)) {
						boolean defaultGain = CMLib.ableMapper()
								.getDefaultGain(className, true, A.ID());
						if (!defaultGain)
							str.append("(Qualify), ");
					}
				}
