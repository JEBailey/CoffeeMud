...
public class AbilityGainReport extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
			...
			for (int a = 0; a < player.numAbilities(); a++) {
				...
				for (int c = 0; c < player.baseCharStats().numClasses(); c++) {
					...
					if ((className.length() == 0)
							|| (C.ID().equalsIgnoreCase(className))) {
						int qlevel = CMLib.ableMapper().getQualifyingLevel(
								C.ID(), true, A.ID());
						if (qlevel >= player.baseCharStats().getClassLevel(
								C.ID())) {
							if (CMLib.ableMapper().getDefaultGain(C.ID(), true,
									A.ID()))
								autogains = true;
							qualifiesFor = true;
							if (CMLib.ableMapper().getDefaultProficiency(
									C.ID(), true, A.ID()) > bestProf)
								bestProf = CMLib.ableMapper()
										.getDefaultProficiency(C.ID(), true,
												A.ID());
						}
					}
				}
				if ((!autogains) && (qualifiesFor)
						&& (!trainedFor.contains(A.ID())))
					trainedFor.add(A.ID());
		...
		for (int s = 0; s < sorted.size(); s++) {
			String able = (String) sorted.elementAt(s, 1);
			long[] stats = (long[]) sorted.elementAt(s, 2);
			buf.append("<TR><TD>" + able + "</TD><TD>" + stats[2] + "</TD><TD>"
					+ stats[1] + "</TD></TR>");
		}
		buf.append("</TABLE>");
