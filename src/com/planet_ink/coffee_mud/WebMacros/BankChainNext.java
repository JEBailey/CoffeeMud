...
public class BankChainNext extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		try {
			...
			} else if (!CMSecurity.isAllowedEverywhere(M,
					CMSecurity.SecFlag.CMDPLAYERS))
				return "";
			...
			for (Iterator j = CMLib.map().bankChains(null); j.hasNext();) {
				...
				if ((last == null)
						|| ((last.length() > 0) && (last.equals(lastID)) && (!bankChain
								.equals(lastID)))) {
					httpReq.addFakeUrlParameter("BANKCHAIN", bankChain);
					last = bankChain;
					if (playerM != null) {
						Banker bankerM = CMLib.map().getBank(bankChain,
								bankChain);
						if ((bankerM == null)
								|| ((bankerM.isSold(ShopKeeper.DEAL_CLANBANKER)) && (playerM
										.getClanRole(playerM.Name()) == null))
								|| (BankAccountInfo.getMakeAccountInfo(httpReq,
										bankerM, playerM).balance <= 0.0)) {
							lastID = bankChain;
							continue;
						}
					}
