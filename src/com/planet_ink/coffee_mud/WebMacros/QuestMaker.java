...
public class QuestMaker extends StdWebMacro {
	...
	public DVector getPage(MOB mob, HTTPRequest httpReq, String template,
			String page, String fileToGet) {
		DVector pageList = (DVector) httpReq.getRequestObjects().get(
				"QM_PAGE_LIST");
		DVector filePages = (DVector) httpReq.getRequestObjects().get(
				"QM_FILE_PAGES");
	...
	private String itemList(List<Item> itemList, Item oldItem, String oldValue) {
		...
		for (int i = 0; i < names.length; i++) {
			...
			if ((oldItem != null) && (CMLib.flags().isCataloged(oldItem))
	...
	private String mobList(List<MOB> mobList, MOB oldMob, String oldValue) {
		...
		for (int m = 0; m < names.length; m++) {
			...
			if ((oldMob != null) && (CMLib.flags().isCataloged(oldMob))
		...
		if (parms.containsKey("QMPAGETITLE")) {
			...
			if (qTemplate.length() == 0) {
				...
				for (int d = 0; d < pageData.size(); d++) {
					list.append("<TR><TD VALIGN=TOP><INPUT TYPE=RADIO NAME=QMTEMPLATE VALUE=\""
							+ htmlOutgoingFilter((String) pageData.elementAt(d,
									3)) + "\"");
					if (pageData.elementAt(d, 3).equals(oldTemplate))
						list.append(" CHECKED");
					list.append("> " + labelColor
							+ (String) pageData.elementAt(d, 1)
							+ "</B></FONT></I></TD>");
					list.append("<TD>" + descColor
							+ (String) pageData.elementAt(d, 2)
							+ "</B></FONT></I></TD></TR>");
			...
			for (String str : httpReq.getUrlParameters())
				if (keyPattern.matcher(
						str.toUpperCase().subSequence(0, str.length()))
						.matches())
			...
			for (int v = 0; v < V.size(); v++) {
				...
				for (int step = 1; step < pageData.size(); step++) {
					...
					if (keyName.startsWith("$")
							&& (key.substring(3).toUpperCase()
									.equals(keyName.substring(1)) || (key
									.substring(3).toUpperCase()
									.startsWith(keyName.substring(1) + "_") && CMath
									.isNumber(key.substring(3 + keyName
											.length()))))) {
						thisPage = true;
						break;
					}
				...
				if (oldVal == null)
					oldVal = "";
				list.append("<INPUT TYPE=HIDDEN NAME=" + key + " VALUE=\""
						+ htmlOutgoingFilter(oldVal) + "\">\n\r");
			}
			list.append("</TD></TR>\n\r");
			...
			for (int step = 1; step < pageData.size(); step++) {
				...
				String keyNameFixed = CMStrings.capitalizeAndLower(httpKeyName
						.replace('_', ' '));
				...
				switch (inputCode) {
				case QuestManager.QM_COMMAND_$TITLE:
					break;
				case QuestManager.QM_COMMAND_$LABEL:
					lastLabel = defValue;
					break;
				...
				case QuestManager.QM_COMMAND_$UNIQUE_QUEST_NAME: {
					...
					list.append("<TR><TD COLSPAN=2>" + descColor + lastLabel
							+ "</B></FONT></I></TD></TR>\n\r");
					list.append("<TR><TD>" + labelColor + keyNameFixed
							+ "</B></FONT></I></TD>");
					list.append("<TD><INPUT TYPE=TEXT SIZE=20 NAME="
							+ httpKeyName + " ");
					list.append(" VALUE=\"" + htmlOutgoingFilter(oldValue)
							+ "\"></TD></TR>");
					break;
				}
				case QuestManager.QM_COMMAND_$LONG_STRING: {
					...
					list.append("<TR><TD>" + labelColor + keyNameFixed
							+ "</B></FONT></I></TD>");
					list.append("<TD><TEXTAREA ROWS=3 COLS=40 NAME="
							+ httpKeyName + ">");
				...
				case QuestManager.QM_COMMAND_$ZAPPERMASK: {
					...
					list.append("<TR><TD>" + labelColor + keyNameFixed
							+ "</B></FONT></I></TD>");
					list.append("<TD><TEXTAREA COLS=40 ROWS=2 NAME="
							+ httpKeyName + ">");
					list.append(oldValue + "</TEXTAREA></TD></TR>");
					break;
				}
				...
				case QuestManager.QM_COMMAND_$AREA: {
					...
					list.append("<TR><TD>" + labelColor + keyNameFixed
							+ "</B></FONT></I></TD>");
					list.append("<TD><INPUT TYPE=TEXT SIZE=40 NAME="
							+ httpKeyName + " ");
					list.append(" VALUE=\"" + htmlOutgoingFilter(oldValue)
							+ "\"></TD></TR>");
					break;
				...
				case QuestManager.QM_COMMAND_$CHOOSE: {
					...
					list.append("<TR><TD>" + labelColor + keyNameFixed
							+ "</B></FONT></I></TD>");
					list.append("<TD><SELECT NAME=" + httpKeyName + ">");
					List<String> options = CMParms.parseCommas(
				...
				case QuestManager.QM_COMMAND_$ITEMXML: {
					...
					Item oldItem = RoomData.getItemFromAnywhere(itemList,
							oldValue);
					list.append("<TR><TD COLSPAN=2><BR></TD></TR>\n\r");
					list.append("<TR><TD COLSPAN=2>" + descColor + lastLabel
							+ "</B></FONT></I></TD></TR>\n\r");
					...
					list.append("</SELECT>");
					list.append("<INPUT TYPE=BUTTON NAME=BUTT_" + httpKeyName
							+ " VALUE=\"NEW\" ONCLICK=\"AddNewItem();\">");
					list.append("</TD></TR>");
					break;
				}
				case QuestManager.QM_COMMAND_$ITEMXML_ONEORMORE: {
					...
					int which = 1;
					oldValue = httpReq.getUrlParameter(httpKeyName + "_"
							+ which);
					while (oldValue != null) {
						if ((!oldValue.equalsIgnoreCase("DELETE"))
								&& (oldValue.length() > 0))
					...
					for (int i = 0; i < oldValues.size(); i++) {
						...
						Item oldItem = (oldValue.length() > 0) ? RoomData
								.getItemFromAnywhere(itemList, oldValue) : null;
						if (i == 0) {
							list.append("<TR><TD COLSPAN=2><BR></TD></TR>\n\r");
							list.append("<TR><TD COLSPAN=2>" + descColor
									+ lastLabel
									+ "</B></FONT></I></TD></TR>\n\r");
						}
						list.append("<TR><TD>" + labelColor + keyNameFixed
								+ "</B></FONT></I></TD>");
						list.append("<TD><SELECT NAME=" + httpKeyName + "_"
								+ (i + 1) + " ONCHANGE=\"Refresh();\">");
						...
						list.append("</SELECT>");
						if (i == oldValues.size() - 1)
							list.append("<INPUT TYPE=BUTTON NAME=BUTT_"
									+ httpKeyName
									+ " VALUE=\"NEW\" ONCLICK=\"AddNewItem();\">");
						list.append("</TD></TR>");
					}
				...
				case QuestManager.QM_COMMAND_$MOBXML_ONEORMORE: {
					if (oldValue == null)
						oldValue = defValue;
					List<MOB> mobList = RoomData
							.contributeMOBs(new Vector<MOB>());
					Vector oldValues = new Vector();
					int which = 1;
					oldValue = httpReq.getUrlParameter(httpKeyName + "_"
							+ which);
					...
					for (int i = 0; i < oldValues.size(); i++) {
						oldValue = (String) oldValues.elementAt(i);
						MOB oldMob = (oldValue.length() > 0) ? RoomData
								.getMOBFromCode(mobList, oldValue) : null;
						if (i == 0) {
							list.append("<TR><TD COLSPAN=2><BR></TD></TR>\n\r");
							list.append("<TR><TD COLSPAN=2>" + descColor
									+ lastLabel
				...
				case QuestManager.QM_COMMAND_$FACTION: {
					...
					if (optionalEntry)
						list.append("<OPTION VALUE=\"\" "
								+ ((oldValue.length() == 0) ? "SELECTED" : "")
								+ ">");
					for (Enumeration f = CMLib.factions().factions(); f
							.hasMoreElements();) {
			...
			if (filePages == null)
				return "false";
			return (((Vector) filePages.elementAt(0, 4)).lastElement() == pageData) ? "true"
					: "false";
			...
			for (int step = 1; step < pageData.size(); step++) {
				...
				try {
					...
					case QuestManager.QM_COMMAND_$HIDDEN:
						...
						break;
					case QuestManager.QM_COMMAND_$ITEMXML_ONEORMORE: {
						List<Item> rawitemlist = RoomData
								.contributeItems(new Vector<Item>());
						rawitemlist.addAll(getCatalogItemsForList(CMLib
						...
						while (oldValue != null) {
							if ((!oldValue.equalsIgnoreCase("DELETE"))
								...
								oldValues.addElement(oldValue);
							which++;
							oldValue = httpReq.getUrlParameter(httpKeyName
									+ "_" + which);
						}
						if (oldValues.size() == 0)
							oldValues.addElement("");
						...
						for (int i = 0; i < oldValues.size(); i++) {
							oldValue = (String) oldValues.elementAt(i);
							Item I2 = oldValue.length() > 0 ? RoomData
									.getItemFromAnywhere(rawitemlist, oldValue)
									: null;
							if (I2 == null)
								I2 = oldValue.length() > 0 ? RoomData
										.getItemFromCatalog(oldValue) : null;
							if (I2 != null) {
								if (CMLib.flags().isCataloged(I2))
									oldValue = CMLib
											.english()
											.getContextSameName(rawitemlist, I2);
								else
									oldValue = CMLib.english().getContextName(
											rawitemlist, I2);
							}
							Object[] choices = rawitemlist.toArray();
							String thisVal = (String) eval.eval(oldValue,
									choices, optionalEntry);
							if (thisVal.length() > 0) {
								Item I3 = (Item) CMLib.english()
										.fetchEnvironmental(rawitemlist,
												thisVal, false);
								if (I3 != null) {
									if (CMLib.flags().isCataloged(I3))
										newVal += "CATALOG-" + I3.Name() + ";";
									else
										newVal += RoomData.getItemCode(
												rawitemlist, I3) + ";";
								}
							}
						}
					...
					case QuestManager.QM_COMMAND_$ITEMXML: {
						...
						if (I2 != null) {
							if (CMLib.flags().isCataloged(I2))
								oldValue = CMLib.english().getContextSameName(
										rawitemlist, I2);
							else
								oldValue = CMLib.english().getContextName(
										rawitemlist, I2);
						...
						Object[] choices = rawitemlist.toArray();
						String newVal = (String) eval.eval(oldValue, choices,
								optionalEntry);
						if (newVal.length() > 0) {
							Item I3 = (Item) CMLib.english()
									.fetchEnvironmental(rawitemlist, newVal,
											false);
							if (I3 != null) {
								if (CMLib.flags().isCataloged(I3))
									newVal = "CATALOG-" + I3.Name() + ";";
								else
									newVal = RoomData.getItemCode(rawitemlist,
											I3) + ";";
							}
						}
						httpReq.addFakeUrlParameter(httpKeyName, newVal);
					...
					case QuestManager.QM_COMMAND_$MOBXML_ONEORMORE: {
						List<MOB> rawmoblist = RoomData
								.contributeMOBs(new Vector<MOB>());
						rawmoblist.addAll(getCatalogMobsForList(CMLib.catalog()
								.getCatalogMobs()));
						Vector oldValues = new Vector();
						int which = 1;
						oldValue = httpReq.getUrlParameter(httpKeyName + "_"
						...
						for (int i = 0; i < oldValues.size(); i++) {
							oldValue = (String) oldValues.elementAt(i);
							MOB M2 = oldValue.length() > 0 ? RoomData
									.getMOBFromCode(rawmoblist, oldValue)
									: null;
							if (M2 == null)
								M2 = oldValue.length() > 0 ? RoomData
										.getMOBFromCatalog(oldValue) : null;
							if (M2 != null) {
								if (CMLib.flags().isCataloged(M2))
									oldValue = CMLib.english()
											.getContextSameName(rawmoblist, M2);
								else
									oldValue = CMLib.english().getContextName(
											rawmoblist, M2);
							}
							...
							String thisVal = (String) eval.eval(oldValue,
									choices, optionalEntry);
							if (thisVal.length() > 0) {
								MOB M3 = (MOB) CMLib.english()
										.fetchEnvironmental(rawmoblist,
												thisVal, false);
								if (M3 != null) {
									if (CMLib.flags().isCataloged(M3))
										newVal += "CATALOG-" + M3.Name() + ";";
									else
										newVal += RoomData.getMOBCode(
												rawmoblist, M3) + ";";
								}
							}
						}
					...
					case QuestManager.QM_COMMAND_$MOBXML: {
						...
						if (oldValue == null)
							oldValue = "";
						MOB M2 = oldValue.length() > 0 ? RoomData
								.getMOBFromCode(rawmoblist, oldValue) : null;
						if (M2 == null)
							M2 = oldValue.length() > 0 ? RoomData
									.getMOBFromCatalog(oldValue) : null;
						if (M2 != null) {
						...
						String newVal = (String) eval.eval(oldValue, choices,
								optionalEntry);
						if (newVal.length() > 0) {
							MOB M3 = (MOB) CMLib.english().fetchEnvironmental(
									rawmoblist, newVal, false);
							if (M3 != null) {
								if (CMLib.flags().isCataloged(M3))
									newVal = "CATALOG-" + M3.Name() + ";";
								else
									newVal = RoomData
											.getMOBCode(rawmoblist, M3) + ";";
							}
						}
						httpReq.addFakeUrlParameter(httpKeyName, newVal);
						break;
					...
					case QuestManager.QM_COMMAND_$CHOOSE: {
						if (oldValue == null)
							oldValue = "";
						Object[] choices = CMParms.parseCommas(
								defValue.toUpperCase(), true).toArray();
						String newVal = (String) eval.eval(oldValue, choices,
								optionalEntry);
						httpReq.addFakeUrlParameter(httpKeyName, newVal);
						break;
					...
					default: {
						if (oldValue == null)
							oldValue = "";
						String newVal = (String) eval.eval(oldValue, null,
								optionalEntry);
						httpReq.addFakeUrlParameter(httpKeyName, newVal);
						break;
					}
					...
					errors.append("Error in field '" + keyNameFixed + "': "
							+ e.getMessage() + "<BR>");
				}
			}
			...
			if (parms.containsKey("FINISH")) {
				...
				DVector filePages = (DVector) httpReq.getRequestObjects().get(
						"QM_FILE_PAGES");
				String script = ((StringBuffer) filePages.elementAt(0, 5))
						.toString();
				String var = null;
				String val = null;
				List<DVector> qPages = (List<DVector>) filePages
						.elementAt(0, 4);
					...
					for (int v = 0; v < pageDV.size(); v++) {
						...
						case QuestManager.QM_COMMAND_$ITEMXML_ONEORMORE: {
							...
							for (int v1 = 0; v1 < V.size(); v1++) {
								Item I = RoomData.getItemFromCode(
										RoomData.getItemCache(), V.get(v1));
								if (I == null)
									I = RoomData.getItemFromAnywhere(
											RoomData.getItemCache(), V.get(v1));
								if (I == null)
									I = RoomData.getItemFromCatalog(V.get(v1));
								if (I != null)
									val += CMLib.coffeeMaker().getItemXML(I)
											.toString();
							}
							break;
						}
						...
						case QuestManager.QM_COMMAND_$MOBXML_ONEORMORE: {
							...
							val = "";
							for (int v1 = 0; v1 < V.size(); v1++) {
								MOB M2 = RoomData.getMOBFromCode(
										RoomData.getMOBCache(), V.get(v1));
								if (M2 == null)
									M2 = RoomData.getMOBFromCatalog(V.get(v1));
								if (M2 != null)
									val += CMLib.coffeeMaker().getMobXML(M2)
											.toString();
							}
							break;
						}
				...
				CMFile newQF = new CMFile(
						Resources.makeFileResourceName("quests/" + name
								+ ".quest"), M, CMFile.FLAG_LOGERRORS);
				if (!newQF.saveText(script)) {
					httpReq.addFakeUrlParameter("QMPAGEERRORS",
							"Unable to save your quest.  Please consult the log.");
					return "";
				}
				Q.setScript("LOAD=quests/" + name + ".quest");
				if ((Q.name().trim().length() == 0) || (Q.duration() < 0)) {
					httpReq.addFakeUrlParameter("QMPAGEERRORS",
							"Unable to create your quest.  Please consult the log.");
					return "";
				}
				Quest badQ = CMLib.quests().fetchQuest(name);
				if (badQ != null) {
					httpReq.addFakeUrlParameter("QMPAGEERRORS",
							"Unable to create your quest.  One of that name already exists!");
					return "";
				}
