		...
		for (int b = 0; b < baseClasses.size(); b++) {
			...
			for (int c = 0; c < charClasses.size(); c++) {
				...
				for (Enumeration<Ability> a = CMClass.abilities(); a
					...
					int level = CMLib.ableMapper().getQualifyingLevel(
							className, true, A.ID());
					if (level >= 0) {
						...
						if (dex < 0) {
							...
							levelssum.setElementAt(
									Integer.valueOf(I.intValue() + level), dex);
							Integer I2 = (Integer) numberare.elementAt(dex);
							numberare.setElementAt(
									Integer.valueOf(I2.intValue() + 1), dex);
						}
					}
				}
			}
