...
public class SocialData extends StdWebMacro {
	...
	static String[] BTYPES = { "NONE", "ALL", "SELF", "TARGETMOB",
			"TARGETITEM", "TARGETINV", "TARGETEQUIP" };
	static String[] BEXTNS = { "", " ALL", " SELF", " <T-NAME>", " <I-NAME>",
			" <V-NAME>", " <E-NAME>" };
	static String[] BFIELDS = { "YOM", "YONM", "YOM", "YTONM", "YONM", "YONM",
			"YONM" };

	static String[] CODESTR = { "WORDS", "MOVEMENT", "SOUND", "VISUAL", "HANDS" };
	static int[] CODES = { CMMsg.MSG_SPEAK, CMMsg.MSG_NOISYMOVEMENT,
			CMMsg.MSG_NOISE, CMMsg.MSG_OK_VISUAL, CMMsg.MSG_HANDS };
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (parms.containsKey("ISVFS"))
			return ""
					+ (new CMFile("::/resources/socials.txt", null,
							CMFile.FLAG_LOGERRORS).exists());
		if (parms.containsKey("ISLFS"))
			return ""
					+ (new CMFile("///resources/socials.txt", null,
							CMFile.FLAG_LOGERRORS).exists());
		if (parms.containsKey("NEWVFS")) {
			CMFile lf = new CMFile("///resources/socials.txt", null,
					CMFile.FLAG_LOGERRORS);
			if (!lf.exists())
				return "true";
			CMFile vf = new CMFile("::/resources/socials.txt", null,
					CMFile.FLAG_LOGERRORS);
			if (!vf.exists())
				return "false";
		...
		if (parms.containsKey("TOVFS")) {
			...
			CMFile lf = new CMFile("///resources/socials.txt", M,
					CMFile.FLAG_LOGERRORS);
			if (!lf.exists())
		...
		if (parms.containsKey("EDIT")) {
			...
			if (old != null) {
				...
				for (int n = 0; n < numXtras; n++) {
					...
					if ((old != null)
							&& (old.length() > 0)
							&& (httpReq.isUrlParameter("IS"
									+ old.toUpperCase().trim()))
							&& (httpReq.getUrlParameter("IS"
									+ old.toUpperCase().trim())
									.equalsIgnoreCase("on"))) {
			...
			if (last.length() > 0) {
				...
				List<Social> SV = (List<Social>) httpReq.getRequestObjects()
						.get("SOCIAL-" + last);
				if ((SV == null) && (newSocialID != null)
				...
				if (SV != null) {
					...
					old = httpReq.getUrlParameter("NUMXTRAS");
					if (old != null) {
						int numXtras = CMath.s_int(httpReq
								.getUrlParameter("NUMXTRAS"));
						for (int n = 0; n < numXtras; n++) {
							old = httpReq.getUrlParameter("XSOCIAL" + n);
							if ((old != null)
									&& (old.length() > 0)
						...
						for (int s = 0; s < SV.size(); s++) {
							...
							if (!found) {
								...
								if (x > 0) {
									String TYPE = S.Name().substring(x + 1)
											.trim().toUpperCase();
									TYPES.addElement(TYPE);
									EXTNS.addElement(" " + TYPE);
									httpReq.addFakeUrlParameter("IS" + TYPE,
											"on");
								}
							}
						}
					...
					if ((old != null)
							...
							&& (httpReq.getUrlParameter("ADDXSOCIAL").trim()
									.length() > 0)
							&& (!TYPES.contains(httpReq
									.getUrlParameter("ADDXSOCIAL")
									.toUpperCase().trim()))) {
						String TYPE = httpReq.getUrlParameter("ADDXSOCIAL")
								.toUpperCase().trim();
						TYPES.addElement(TYPE);
						EXTNS.addElement(" " + TYPE);
					...
					if (parms.containsKey("GETEXTRA")
						...
						str.append(TYPES.elementAt(BTYPES.length
								+ CMath.s_int(parms.get("GETEXTRA")))
								+ ", ");
					...
					for (int t = 0; t < TYPES.size(); t++) {
						...
						if (parms.containsKey("IS" + TYPE)) {
							...
							if (old == null)
								old = (((S != null) && (!httpReq
										.isUrlParameter("NUMXTRAS"))) ? "on"
										: "");
							str.append("" + old.equalsIgnoreCase("on") + ", ");
							if (!old.equalsIgnoreCase("on"))
								continue;
						}
						String field = (t < BTYPES.length) ? BFIELDS[t]
								: BFIELDS[0];
						for (int f = 0; f < field.length(); f++) {
							String fnam = "SDAT_" + TYPE + "_"
									+ field.charAt(f);
							if (parms.containsKey(fnam)) {
								old = httpReq.getUrlParameter(fnam);
								if (old == null) {
									if (S == null)
										S = CMLib.socials().makeDefaultSocial(
												last, EXTN);
									switch (field.charAt(f)) {
									case 'Y':
										old = S.You_see();
