...
public class AreaScriptNext extends StdWebMacro {
	...
	public AreaScriptInstance addScript(
			TreeMap<String, ArrayList<AreaScriptInstance>> list,
			ArrayList<String> prefix, String scriptKey, String immediateHost,
		...
		AreaScriptInstance inst = new AreaScriptInstance(scriptKey, next, key,
				file);
		subList.add(inst);
		return inst;
	}

	public void addScripts(TreeMap<String, ArrayList<AreaScriptInstance>> list,
			ArrayList<String> prefix, PhysicalAgent E) {
		if (E == null)
			return;
			...
			if (B instanceof ScriptingEngine) {
				...
				if (files != null)
					for (int f = 0; f < files.size(); f++)
						addScript(list, prefix, SE.getScriptResourceKey(),
								B.ID(), files.get(f).toLowerCase(),
								files.get(f));
				...
				if ((nonFiles != null) && (nonFiles.trim().length() > 0)) {
					AreaScriptInstance inst = addScript(list, prefix,
							SE.getScriptResourceKey(), B.ID(), "Custom",
							nonFiles);
		...
		for (Enumeration<ScriptingEngine> e = E.scripts(); e.hasMoreElements();) {
			...
			for (int f = 0; f < files.size(); f++)
				addScript(list, prefix, SE.getScriptResourceKey(), null, files
						.get(f).toLowerCase(), files.get(f));
			String nonFiles = SE.getVar("*",
					"COFFEEMUD_SYSTEM_INTERNAL_NONFILENAME_SCRIPT");
			if (nonFiles.trim().length() > 0) {
				AreaScriptInstance inst = addScript(list, prefix,
						SE.getScriptResourceKey(), null, "Custom", nonFiles);
				inst.customScript = nonFiles.trim();
			}
		}
	}

	public void addShopScripts(
			TreeMap<String, ArrayList<AreaScriptInstance>> list,
			ArrayList<String> prefix, PhysicalAgent E) {
		if (E == null)
			return;
		...
		if (SK != null) {
			for (Iterator<Environmental> i = SK.getShop().getStoreInventory(); i
				...
				ArrayList<String> newPrefix = (ArrayList<String>) prefix
						.clone();
	...
	public TreeMap<String, ArrayList<AreaScriptInstance>> getAreaScripts(
			HTTPRequest httpReq, String area) {
		TreeMap<String, ArrayList<AreaScriptInstance>> list;
		list = (TreeMap<String, ArrayList<AreaScriptInstance>>) httpReq
				.getRequestObjects().get("AREA_" + area + " SCRIPTSLIST");
		if (list == null) {
			...
			httpReq.getRequestObjects().put("AREA_" + area + " SCRIPTSLIST",
					list);
		}
		return list;
	}
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		String lastID = "";
		TreeMap<String, ArrayList<AreaScriptInstance>> list = getAreaScripts(
				httpReq, area);
		for (String scriptName : list.keySet()) {
			if ((last == null)
					|| ((last.length() > 0) && (last.equals(lastID)) && (!scriptName
							.equals(lastID)))) {
