...
public class CoffeeTableRows extends StdWebMacro {
	...
	// HEADER, FOOTER, DATERANGE, DATESTART, DATEEND, LEVELSUP, DIVORCES,
	// BIRTHS, MARRIAGES, PURGES, CLASSCHANGES, PKDEATHS, DEATHS, NEWPLAYERS,
	// TOTALHOURS, AVERAGETICKS, AVERAGEONLINE, MOSTONLINE, LOGINS,
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (parms.containsKey("SKILLUSE")) {
				...
				for (int i = 0; i < orderedParms.size(); i++) {
					...
					if (key.equals("COLSPAN"))
						...
						if (x >= allSkills.size())
							...
							table.append("<TD" + colspan + ">" + header
									+ A.ID() + footer + "</TD>");
						}
		...
		} else if (parms.containsKey("QUESTNAME")
				|| parms.containsKey("QUESTRPT")) {
			...
			for (int x = 0; x < CMLib.quests().numQuests(); x++) {
				...
				for (int i = 0; i < orderedParms.size(); i++) {
					...
					if (key.equals("COLSPAN"))
						...
						table.append("<TD" + colspan + ">" + header
								+ CMLib.time().date2DateString(lastCur)
								+ footer + "</TD>");
						...
						table.append("<TD" + colspan + ">" + header
								+ totals[x][CoffeeTableRow.STAT_QUESTSTOP]
								+ footer + "</TD>");
			...
			while ((V.size() > 0) && (curTime > (ENDQ.getTimeInMillis()))) {
				...
				long minsOnline = (totals[CoffeeTableRow.STAT_TICKSONLINE] * CMProps
						.getTickMillis()) / (1000 * 60);
				totals[CoffeeTableRow.STAT_TICKSONLINE] = (totals[CoffeeTableRow.STAT_TICKSONLINE] * CMProps
						.getTickMillis()) / (1000 * 60 * 60);
				...
				for (int i = 0; i < orderedParms.size(); i++) {
					...
					if (key.equals("COLSPAN"))
						...
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_LOGINS] + footer
								+ "</TD>");
					else if (key.equalsIgnoreCase("MOSTONLINE"))
						table.append("<TD" + colspan + ">" + header
								+ highestOnline + footer + "</TD>");
					else if (key.equalsIgnoreCase("AVERAGEONLINE"))
						table.append("<TD" + colspan + ">" + header + avgOnline
								+ footer + "</TD>");
						...
						table.append("<TD"
								...
								+ ">"
								+ header
								+ ((totals[CoffeeTableRow.STAT_LOGINS] > 0) ? (minsOnline / totals[CoffeeTableRow.STAT_LOGINS])
										: 0) + "</TD>");
					else if (key.equalsIgnoreCase("TOTALHOURS"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_TICKSONLINE]
								+ footer + "</TD>");
					else if (key.equalsIgnoreCase("NEWPLAYERS"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_NEWPLAYERS]
								+ footer + "</TD>");
					else if (key.equalsIgnoreCase("DEATHS"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_DEATHS] + footer
								+ "</TD>");
					else if (key.equalsIgnoreCase("PKDEATHS"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_PKDEATHS] + footer
								+ "</TD>");
					else if (key.equalsIgnoreCase("CLASSCHANGES"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_CLASSCHANGE]
								+ footer + "</TD>");
					else if (key.equalsIgnoreCase("PURGES"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_PURGES] + footer
								+ "</TD>");
					else if (key.equalsIgnoreCase("MARRIAGES"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_MARRIAGES]
								+ footer + "</TD>");
					else if (key.equalsIgnoreCase("BIRTHS"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_BIRTHS] + footer
								+ "</TD>");
					else if (key.equalsIgnoreCase("DIVORCES"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_DIVORCES] + footer
								+ "</TD>");
					else if (key.equalsIgnoreCase("LEVELSUP"))
						table.append("<TD" + colspan + ">" + header
								+ totals[CoffeeTableRow.STAT_LEVELSGAINED]
								+ footer + "</TD>");
				}
				table.append("</TR>");
				if (scale == 0)
