...
public class GrinderMobs {
	private static final String[] okparms = { "NAME", "CLASSES", "DISPLAYTEXT",
			...
			"CURSES", "POWERS", "CLANID", "TATTOOS", "EXPERTISES", "BUDGET",
			"DEVALRATE", "INVRESETRATE", "IMAGE", "ISPOSTMAN", "POSTCHAIN",
			"POSTMIN", "POSTLBS", "POSTHOLD", "POSTNEW", "POSTHELD",
			"IGNOREMASK", "LOANINT", "SVCRIT", "AUCCHAIN", "LIVELIST",
			"TIMELIST", "TIMELISTPCT", "LIVECUT", "TIMECUT", "MAXDAYS",
			"MINDAYS", "ISAUCTION", "DEITYID", "VARMONEY", "CATACAT" };

	public static String senses(Physical P, HTTPRequest httpReq,
		...
		for (int d = 0; d < PhyStats.CAN_SEE_CODES.length; d++) {
			...
			if ((parm != null) && (parm.equals("on")))
				P.basePhyStats().setSensesMask(
						P.basePhyStats().sensesMask() | (1 << d));
		}
		return "";
	}
	...
	public static void happilyAddItem(Item I, MOB M) {
		if (I.subjectToWearAndTear()
				&& ((I.usesRemaining() < 1) || (I.usesRemaining() > 100)))
	...
	public static String abilities(MOB M, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
	...
	public static String factions(MOB E, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
	...
	public static String blessings(Deity E, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
	...
	public static String clans(MOB E, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
	...
	public static String curses(Deity E, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
		while (E.numCurses() > 0) {
		...
		if (httpReq.isUrlParameter("CURSE1")) {
			...
			while (aff != null) {
				if (aff.length() > 0) {
					Ability B = CMClass.getAbility(aff);
					boolean clericOnly = (httpReq
							.isUrlParameter("CUONLY" + num))
							&& (httpReq.getUrlParameter("CUONLY" + num))
									.equalsIgnoreCase("on");
					if (B == null)
	...
	public static String expertiseList(MOB E, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
		E.delAllExpertises();
	...
	public static String items(MOB M, Vector allitems, HTTPRequest httpReq) {
		if (httpReq.isUrlParameter("ITEM1")) {
			...
			for (int i = 0; i < cstrings.size(); i++) {
				...
				Item C2 = (Item) CMLib.english().fetchEnvironmental(items,
						CONTAINER, true);
	...
	public static String powers(Deity E, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
		while (E.numPowers() > 0) {
	...
	public static String editMob(HTTPRequest httpReq,
			java.util.Map<String, String> parms, MOB whom, Room R) {
		...
		synchronized (("SYNC" + ((R != null) ? R.roomID() : "null")).intern()) {
			...
			for (int o = 0; o < okparms.length; o++) {
				...
				if ((M.isGeneric() || (!generic)))
					...
					case 22: // shopkeeper type
						if (M instanceof ShopKeeper) {
							...
							while (httpReq.getUrlParameter(okparms[o] + x) != null) {
								((ShopKeeper) M)
										.addSoldType(CMath.s_int(httpReq
					...
					case 39: // clanid
					{
						...
						M.setClan("", Integer.MIN_VALUE); // signal to clear the
															// list
					...
					case 48: // minimum postage
						if (M instanceof PostOffice)
							((PostOffice) M).setMinimumPostage(CMath
									.s_double(old));
						break;
					case 49: // postage per pound after first
						if (M instanceof PostOffice)
							((PostOffice) M).setPostagePerPound(CMath
									.s_double(old));
						break;
					case 50: // holding fee per pound per month
						if (M instanceof PostOffice)
							((PostOffice) M).setHoldFeePerPound(CMath
									.s_double(old));
						break;
					case 51: // new box fee
						if (M instanceof PostOffice)
							((PostOffice) M).setFeeForNewBox(CMath
									.s_double(old));
						break;
					case 52: // maximum months held
						if (M instanceof PostOffice)
							((PostOffice) M).setMaxMudMonthsHeld(CMath
									.s_int(old));
						break;
					...
					case 58: // timed list
						if (M instanceof Auctioneer)
							if (old.length() == 0)
								...
								((Auctioneer) M).setTimedListingPrice(CMath
										.s_double(old));
						break;
					case 59: // timed list pct
						if (M instanceof Auctioneer)
							if (old.length() == 0)
								...
								((Auctioneer) M).setTimedListingPct(CMath
										.s_pct(old));
						break;
					case 60: // live cut
						// if(M instanceof Auctioneer)
					...
					case 61: // timed cut
						if (M instanceof Auctioneer)
							if (old.length() == 0)
								...
								((Auctioneer) M).setTimedFinalCutPct(CMath
										.s_pct(old));
						break;
					case 62: // max days
						if (M instanceof Auctioneer)
							if (old.length() == 0)
								...
								((Auctioneer) M).setMaxTimedAuctionDays(CMath
										.s_int(old));
						break;
					case 63: // min days
						if (M instanceof Auctioneer)
							if (old.length() == 0)
								...
								((Auctioneer) M).setMinTimedAuctionDays(CMath
										.s_int(old));
						break;
					case 64: // is auction
						break;
					case 65: // deity
						/*
						 * if(old.length()==0) M.setWorshipCharID(""); else
			...
			if (M.isGeneric()) {
				...
				if ((M instanceof ShopKeeper)
					...
					XVector inventory = new XVector(SK.getShop()
							.getStoreInventory());
					...
					while ((MATCHING != null) && (theparm != null)) {
						if (CMath.isNumber(MATCHING) && (inventory.size() > 0)) {
							...
							if (O != null)
								SK.getShop().addStoreInventory(O,
										CMath.s_int(theparm),
										CMath.s_int(theprice));
						} else if (MATCHING.startsWith("CATALOG-")) {
							Environmental O = RoomData
									.getMOBFromCatalog(MATCHING);
							if (O == null)
								O = RoomData
										.getItemFromAnywhere(null, MATCHING);
							if (O != null)
								SK.getShop().addStoreInventory(
										(Environmental) O.copyOf(),
										CMath.s_int(theparm),
										CMath.s_int(theprice));
				...
				if ((M instanceof Economics)
					...
					((Economics) M).setItemPricingAdjustments(CMParms
							.toStringArray(prics));
				}
			}
			...
			if (R == null) {
				if (mobCode.startsWith("CATALOG-")
						|| mobCode.startsWith("NEWCATA-")) {
					MOB M2 = CMLib.catalog()
							.getCatalogMob(mobCode.substring(8));
					...
					newMobCode = RoomData
							.getMOBCode(RoomData.getMOBCache(), M2);
				}
			} else {
				if (mobCode.equals("NEW") || mobCode.equals("NEWDEITY")) {
			...
			if (!copyMOB.sameAs(M))
				Log.sysOut("Grinder",
						whom.Name() + " modified mob " + copyMOB.Name()
