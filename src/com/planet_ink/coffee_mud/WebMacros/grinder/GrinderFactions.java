...
public class GrinderFactions {
	...
	public static String modifyFaction(HTTPRequest httpReq,
			java.util.Map<String, String> parms, Faction F) {
		...
		F.setShowInFactionsCommand((old != null)
		...
		F.setShowInSpecialReported((old != null)
				&& (old.equalsIgnoreCase("on")));
		...
		while (httpReq.getUrlParameter("RANGENAME" + num) != null) {
			...
			if (old.length() > 0) {
				if (code.length() == 0)
					code = CMStrings.replaceAll(old.toUpperCase().trim(), " ",
							"_");
				int low = CMath
						.s_int(httpReq.getUrlParameter("RANGELOW" + num));
				int high = CMath.s_int(httpReq.getUrlParameter("RANGEHIGH"
						+ num));
				if (high < low)
					high = low;
				String flag = httpReq.getUrlParameter("RANGEFLAG" + num);
				F.addRange(low + ";" + high + ";" + old + ";" + code + ";"
						+ flag);
			}
			num++;
		}
		...
		for (int i = 0; i < prefixes.length; i++) {
			...
			while (httpReq.getUrlParameter(prefix + num) != null) {
				...
				if (value.length() > 0) {
					String mask = httpReq
							.getUrlParameter(prefix + "MASK" + num);
		...
		while (httpReq.getUrlParameter("CHANGESTRIGGER" + num) != null) {
			...
			if (old.length() > 0) {
				...
				if (ctparms.trim().length() > 0)
					old += "(" + ctparms.trim() + ")";
				old += ";";
				old += Faction.FactionChangeEvent.CHANGE_DIRECTION_DESCS[CMath
						.s_int(httpReq.getUrlParameter("CHANGESDIR" + num))];
				old += ";";
				old += CMath.toPct(httpReq.getUrlParameter("CHANGESFACTOR"
						+ num));
				old += ";";
				String id = "";
				int x = 0;
				for (; httpReq.isUrlParameter("CHANGESFLAGS" + num + "_" + id); id = ""
						+ (++x))
					old += " "
							+ httpReq.getUrlParameter(
		...
		while (httpReq.getUrlParameter("ADJFACTOR" + num) != null) {
			...
			if (old.length() > 0) {
				double gain = CMath.s_pct(httpReq
						.getUrlParameter("ADJFACTORGAIN" + num));
				double loss = CMath.s_pct(httpReq
		...
		while (httpReq.getUrlParameter("RELATIONS" + num) != null) {
			...
			if (old.length() > 0)
				F.addRelation(
						old,
						CMath.s_pct(httpReq.getUrlParameter("RELATIONSAMT"
								+ num)));
			num++;
		}
		...
		while (httpReq.getUrlParameter("AFFBEHAV" + num) != null) {
			...
			if (old.length() > 0) {
				String parm = ""
						+ httpReq.getUrlParameter("AFFBEHAVPARM" + num);
				String mask = ""
						+ httpReq.getUrlParameter("AFFBEHAVMASK" + num);
				String[] oldParms = F.getAffectBehav(old);
				if ((oldParms == null) || (!oldParms[0].equals(parm))
						|| (!oldParms[1].equals(mask)))
		...
		for (int d = 0; d < affBehav.size(); d++) {
			...
			F.addAffectBehav((String) affBehav.elementAt(d, 1),
					(String) affBehav.elementAt(d, 2),
					(String) affBehav.elementAt(d, 3));
		}
		...
		while (httpReq.getUrlParameter("ABILITYUSE" + num) != null) {
			...
			if (old.length() > 0) {
				...
				if (usedType > 0) {
					int x = -1;
					while (httpReq.isUrlParameter("ABILITYUSE" + num + "_"
							+ (++x))) {
						String s = httpReq.getUrlParameter("ABILITYUSE" + num
								+ "_" + x);
						if (s.length() > 0) {
				...
				old += ";"
						+ CMath.s_int(httpReq.getUrlParameter("ABILITYMIN"
								+ num));
				old += ";"
						+ CMath.s_int(httpReq.getUrlParameter("ABILITYMAX"
								+ num));
