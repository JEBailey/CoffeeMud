...
public class GrinderExits {
	private static final String[] okparms = { "NAME", " CLASSES",
			"DISPLAYTEXT", "DESCRIPTION", "LEVEL", "LEVELRESTRICTED",
			"ISTRAPPED", "HASADOOR", "CLOSEDTEXT", "DEFAULTSCLOSED",
			"OPENWORD", "CLOSEWORD", "HASALOCK", "DEFAULTSLOCKED", "KEYNAME",
			"ISREADABLE", "READABLETEXT", "ISCLASSRESTRICTED",
			"RESTRICTEDCLASSES", "ISALIGNMENTRESTRICTED",
			"RESTRICTEDALIGNMENTS", " MISCTEXT", "ISGENERIC", "DOORNAME",
			"IMAGE", "OPENTICKS" };

	public static String dispositions(Physical P, HTTPRequest httpReq,
		...
		for (int d = 0; d < PhyStats.IS_CODES.length; d++) {
			...
			if ((parm != null) && (parm.equals("on")))
				P.basePhyStats().setDisposition(
						P.basePhyStats().disposition() | (1 << d));
		}
		return "";
	}

	public static String editExit(Room R, int dir, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
		synchronized (("SYNC" + R.roomID()).intern()) {
			...
			if ((newClassID != null)
					&& (!CMClass.classID(X).equals(newClassID))) {
			...
			for (int o = 0; o < okparms.length; o++) {
				...
				if (X.isGeneric() || (!generic))
					...
					case 7: // hasadoor
						if (old.equals("on"))
							X.setDoorsNLocks(true, !X.defaultsClosed(),
									X.defaultsClosed(), X.hasALock(),
									X.hasALock(), X.defaultsLocked());
						else
							X.setDoorsNLocks(false, true, false, false, false,
									false);
						break;
					case 8: // closedtext
						X.setExitParams(X.doorName(), X.closeWord(),
								X.openWord(), old);
						break;
					case 9: // defaultsclosed
						X.setDoorsNLocks(X.hasADoor(), X.isOpen(),
								old.equals("on"), X.hasALock(), X.isLocked(),
								X.defaultsLocked());
						break;
					case 10: // openword
						X.setExitParams(X.doorName(), X.closeWord(), old,
								X.closedText());
						break;
					case 11: // closeword
						X.setExitParams(X.doorName(), old, X.openWord(),
								X.closedText());
						break;
					case 12: // hasalock
						if (old.equals("on"))
							X.setDoorsNLocks(true, !X.defaultsClosed(),
									X.defaultsClosed(), true,
									X.defaultsLocked(), X.defaultsLocked());
						else
							X.setDoorsNLocks(X.hasADoor(), X.isOpen(),
									X.defaultsClosed(), false, false, false);
						break;
					case 13: // defaultslocked
						X.setDoorsNLocks(X.hasADoor(), X.isOpen(),
								X.defaultsClosed(), X.hasALock(), X.isLocked(),
								old.equals("on"));
						break;
					case 14: // keyname
					...
					case 22: // is generic
						break;
					case 23: // door name
						X.setExitParams(old, X.closeWord(), X.openWord(),
								X.closedText());
						break;
					case 24: // image
						X.setImage(old);
			...
			if ((makeSame != null) && (makeSame.equalsIgnoreCase("on"))) {
				...
				Exit E2 = null;
				if ((R2 != null)
						&& (R2.rawDoors()[Directions.getOpDirectionCode(dir)] == R))
