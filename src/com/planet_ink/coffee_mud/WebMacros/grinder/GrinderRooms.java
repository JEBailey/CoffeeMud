...
public class GrinderRooms {
	...
	public static String editRoom(HTTPRequest httpReq,
		...
		synchronized (("SYNC" + R.roomID()).intern()) {
			...
			boolean singleMobMode = CMath.s_bool(httpReq
					.getUrlParameter("SINGLEMOB"));
			String delMOB = singleMobMode ? httpReq.getUrlParameter("DELMOB")
					: null;
			...
			// climate
			/*
			 * if(httpReq.isUrlParameter("CLIMATE")) { int
			 * climate=CMath.s_int(httpReq.getUrlParameter("CLIMATE"));
			 * if(climate>=0) { for(int i=1;;i++)
			 * if(httpReq.isUrlParameter("CLIMATE"+(Integer.toString(i)))) { int
			 * newVal
			 * =CMath.s_int(httpReq.getUrlParameter("CLIMATE"+(Integer.toString
			 * (i)))); if(newVal<0) { climate=-1; break; }
			 * climate=climate|newVal; } else break; }
			 * R.setClimateType(climate); } else R.setClimateType(-1);
			 */
			// atmosphere
			...
			if (!copyRoom.sameAs(R))
				Log.sysOut("Grinder",
						whom.Name() + " modified room " + R.roomID() + ".");
			copyRoom.destroy();
		}
		return "";
	}
	...
	public static Room createLonelyRoom(Area A, Room linkTo, int dir,
		...
		if (linkTo != null) {
			...
			newRoom.setRawExit(Directions.getOpDirectionCode(dir),
	...
	public static Room createGridRoom(Area A, String roomID, Room copyThisOne,
			RoomnumberSet deferredExitSaves, boolean autoLink) {
		Room R = null;
		if (copyThisOne != null) {
