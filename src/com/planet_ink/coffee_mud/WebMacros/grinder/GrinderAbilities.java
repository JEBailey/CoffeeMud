...
public class GrinderAbilities {
	...
	public static String modifyAbility(HTTPRequest httpReq,
			java.util.Map<String, String> parms, Ability oldA, Ability A) {
		...
		A.setStat("TRIGSTR", (old == null) ? "TRIGSTR" : old.toUpperCase()
				.trim());
		...
		A.setStat("AUTOINVOKE",
				(old == null) ? "" : "" + old.equalsIgnoreCase("on"));
		Vector V = new Vector();
		...
		A.setStat(
				"CANMEND",
				(old == null) ? "false" : Boolean.toString(old
						.equalsIgnoreCase("on")));
		...
		if (A instanceof Language) {
			...
			if (httpReq.isUrlParameter("WORDLIST1")) {
				...
				while (httpReq.isUrlParameter("WORDLIST" + x)) {
					((Language) A).translationVector(A.ID()).add(
							CMParms.parseCommas(
									httpReq.getUrlParameter("WORDLIST" + x),
			...
			if (httpReq.isUrlParameter("HASHWORD1")) {
				...
				while (httpReq.isUrlParameter("HASHWORD" + x)) {
					String word = httpReq.getUrlParameter("HASHWORD" + x)
							.toUpperCase().trim();
