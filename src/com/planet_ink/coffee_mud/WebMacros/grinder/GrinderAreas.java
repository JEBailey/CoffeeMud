...
public class GrinderAreas {
	public static String getAreaList(Enumeration<Area> a, Area pickedA,
			MOB mob, boolean noInstances) {
		StringBuffer AreaList = new StringBuffer("");
		boolean anywhere = (CMSecurity.isAllowedAnywhere(mob,
				CMSecurity.SecFlag.CMDROOMS) || CMSecurity.isAllowedAnywhere(
				mob, CMSecurity.SecFlag.CMDAREAS));
		boolean everywhere = (CMSecurity.isASysOp(mob)
				|| CMSecurity.isAllowedEverywhere(mob,
						CMSecurity.SecFlag.CMDROOMS) || CMSecurity
				.isAllowedEverywhere(mob, CMSecurity.SecFlag.CMDAREAS));
		for (; a.hasMoreElements();) {
			Area A = a.nextElement();
			if ((everywhere || (A.amISubOp(mob.Name()) && anywhere))
					&& ((!noInstances) || (!CMath.bset(A.flags(),
							Area.FLAG_INSTANCE_CHILD))))
				if ((pickedA != null) && (pickedA == A))
					AreaList.append("<OPTION SELECTED VALUE=\"" + A.Name()
							+ "\">" + A.name());
				else
					AreaList.append("<OPTION VALUE=\"" + A.Name() + "\">"
							+ A.name());
		}
		return AreaList.toString();
	}

	public static String doBehavs(PhysicalAgent E, HTTPRequest httpReq,
	...
	public static String doAffects(Physical P, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
	...
	public static String modifyArea(HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
		...
		for (int i = 1;; i++)
			if (httpReq.isUrlParameter("SUBOP" + (Integer.toString(i))))
				A.addSubOp(httpReq.getUrlParameter("SUBOP"
						+ (Integer.toString(i))));
			else
				break;
		...
		if (httpReq.isUrlParameter("BLURBFLAG1")) {
			...
			while ((DOUBLE != null) && (MASK != null)) {
				if (DOUBLE.trim().length() > 0)
					prics.addElement((DOUBLE.toUpperCase().trim() + " " + MASK)
							.trim());
				num++;
		...
		if (INVRESETRATE == null)
			INVRESETRATE = "0";
		A.setInvResetRate(CMath.s_int(CMLib.coffeeFilter().safetyFilter(
				INVRESETRATE)));
		...
		if (A instanceof AutoGenArea) {
			...
			if (AGXMLPATH == null)
				AGXMLPATH = "";
			((AutoGenArea) A).setGeneratorXmlPath(CMLib.coffeeFilter()
					.safetyFilter(AGXMLPATH));

			String AGAUTOVAR = httpReq.getUrlParameter("AGAUTOVAR");
			if (AGAUTOVAR == null)
				AGAUTOVAR = "";
			((AutoGenArea) A).setAutoGenVariables(CMLib.coffeeFilter()
					.safetyFilter(AGAUTOVAR));
		}
		...
		if (httpReq.isUrlParameter("IPRIC1")) {
			...
			((Economics) A).setItemPricingAdjustments(CMParms
					.toStringArray(prics));
		}
		...
		for (int i = 1;; i++)
			if (httpReq.isUrlParameter("PARENT" + (Integer.toString(i)))) {
				Area parent = CMLib.map().getArea(
						httpReq.getUrlParameter("PARENT"
								+ (Integer.toString(i))));
				if (parent != null) {
					if (A.canParent(parent)) {
						...
						return "The area, '"
								+ parent.Name()
								+ "', cannot be added as a parent, as this would create a circular reference.";
				}
			} else
				break;
		...
		for (int i = 1;; i++)
			if (httpReq.isUrlParameter("CHILDREN" + (Integer.toString(i)))) {
				Area child = CMLib.map().getArea(
						httpReq.getUrlParameter("CHILDREN"
								+ (Integer.toString(i))));
				if (child != null) {
					if (A.canChild(child)) {
						A.addChild(child);
						...
						return "The area, '"
								+ child.Name()
								+ "', cannot be added as a child, as this would create a circular reference.";
				} else
					break;
			}
		...
		for (int i = 0; i < areasNeedingUpdates.size(); i++) // will always
																// include A
		{
