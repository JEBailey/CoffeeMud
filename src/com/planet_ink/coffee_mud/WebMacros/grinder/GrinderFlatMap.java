...
public class GrinderFlatMap {
	...
	public GrinderFlatMap(Area A, int[] xyxy) {
		...
		if (thinArea) {
			...
			String roomID = null;
			// RoomnumberSet
			// loadRooms=(RoomnumberSet)CMClass.getCommon("DefaultRoomnumberSet");
			for (Enumeration e = A.getProperRoomnumbers().getRoomIDs(); e
					.hasMoreElements();) {
				...
				if ((currentSet == null) || (!currentSet.contains(roomID))) {
					if (area instanceof GridZones) {
						if (xyxy != null) {
							XYVector thisXY = ((GridZones) area)
									.getRoomXY(roomID);
							if (thisXY == null)
								continue;
							if ((thisXY.x < xyxy[0]) || (thisXY.y < xyxy[1])
									|| (thisXY.x > xyxy[2])
									|| (thisXY.y > xyxy[3]))
								continue;
						}
			...
			for (; r.hasMoreElements();) {
				...
				if (R.roomID().length() > 0) {
					...
					if (thisXY == null)
						continue;
					if ((thisXY.x < xyxy[0]) || (thisXY.y < xyxy[1])
							|| (thisXY.x >= xyxy[2]) || (thisXY.y >= xyxy[3])) {
		...
		// for thin rooms, the thin-unloaded are already in areaMap and
		// hashRooms
		...
		// ((GrinderRoom)areaMap.elementAt(a)).score=scoreRoom((GrinderRoom)areaMap.elementAt(a),
		// "X!X!X", new int[2], new HashSet(), new HashSet());
	}

	public double getDistanceFrom(int[] xy1, int[] xy2) {
		return Math
				.sqrt((((double) (xy1[0] - xy2[0])) * ((double) (xy1[0] - xy2[0])))
	...
	public void rebuildGrid() {
		...
		if (area instanceof GridZones) {
			...
			for (int i = areaMap.size() - 1; i >= 0; i--) {
				...
				if (myxy == null)
					continue;
				if ((myxy.x < boundsXYXY[0]) || (myxy.y < boundsXYXY[1])
						|| (myxy.x >= boundsXYXY[2])
						|| (myxy.y >= boundsXYXY[3]))
					areaMap.remove(room);
				else {
	...
	public void placeRooms() {
		...
		while ((roomsDone.size() < areaMap.size()) && (didSomething)) {
			...
			for (int i = 0; i < areaMap.size(); i++) {
				...
				if (roomsDone.contains(room.roomID))
					continue;
				List<GrinderRoom> V = scoreRoom(hashRooms, room, roomsDone,
						false);
				if (bestSet == null)
					bestSet = V;
		...
		if (finalCluster.size() > 0) {
			boolean[][] miniGrid = new boolean[finalCluster.size() + 1][finalCluster
					.size() + 1];
			int[] midXY = new int[2];
			midXY[0] = (int) Math
					.round(Math.floor((finalCluster.size() + 1.0) / 2.0));
			midXY[1] = (int) Math
					.round(Math.floor((finalCluster.size() + 1.0) / 2.0));
			finalCluster.get(0).xy = midXY.clone();
			miniGrid[midXY[0]][midXY[1]] = true;
			for (int f = 1; f < finalCluster.size(); f++) {
				int[] bestCoords = new int[] { miniGrid.length - 1,
						miniGrid.length - 1 };
				for (int x = 0; x < miniGrid.length; x++)
					for (int y = 0; y < miniGrid[x].length; y++)
						if ((!miniGrid[x][y])
								&& (getDistanceFrom(bestCoords, midXY) > getDistanceFrom(
										new int[] { x, y }, midXY)))
	...
	public void clusterSet(List<List<GrinderRoom>> sets) {
		...
		for (int a = 1; a < sets.size(); a++) {
			...
			if (grid[bestCoords[0]][bestCoords[1]] != null) {
				Log.errOut("GFlagMap",
						"Lost a chunk of rooms to bad spiral positioning!!!");
	...
	public List<GrinderRoom> scoreRoomII(Map<String, GrinderRoom> H,
			GrinderRoom room, Set<String> roomsDone) {
		...
		while (startHere != V.size()) {
			...
			for (; s < size; s++) {
				...
				for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--)
					if ((R.doors[d] != null) && (R.doors[d].room != null)
	...
	public Vector scoreRoom(Map<String, GrinderRoom> H, GrinderRoom room,
			HashSet roomsDone, boolean finalPosition) {
		HashSet coordsDone = new HashSet();
		coordsDone.add(0 + "/" + 0);
		...
		for (int y = Ystart; y <= Ybound; y++) {
			...
			for (int l = 0; l < 5; l++) {
				...
				for (int x = Xstart; x <= Xbound; x++) {
					...
					if (GR == null)
						buf.append("<TD COLSPAN=5"
								+ ((boundsXYXY != null) ? " ID=X"
										+ (x + boundsXYXY[0]) + "_"
										+ (y + boundsXYXY[1]) : "")
								+ "><BR></TD>");
					else {
						...
						if (GR.doors[Directions.UP] != null)
							up = findRelGridDir(GR,
									GR.doors[Directions.UP].room);
						if (GR.doors[Directions.DOWN] != null)
							down = findRelGridDir(GR,
									GR.doors[Directions.DOWN].room);
						if (up < 0) {
						...
						{
							buf.append("<TD>"
									+ getDoorLabelGif(Directions.NORTHWEST, GR,
											httpReq) + "</TD>");
							buf.append("<TD><BR></TD>");
							buf.append("<TD>"
									+ getDoorLabelGif(Directions.NORTH, GR,
											httpReq) + "</TD>");
							String alt = "<BR>";
							if (up == Directions.NORTH)
								alt = getDoorLabelGif(Directions.UP, GR,
										httpReq);
							if (down == Directions.NORTH)
								alt = getDoorLabelGif(Directions.DOWN, GR,
										httpReq);
							buf.append("<TD>" + alt + "</TD>");
							buf.append("<TD>"
									+ getDoorLabelGif(Directions.NORTHEAST, GR,
											httpReq) + "</TD>");
						}
							break;
						case 1: // west, east
						...
						case 2: // nada
							buf.append("<TD>"
									+ getDoorLabelGif(Directions.WEST, GR,
											httpReq) + "</TD>");
							buf.append("<TD>"
									+ getDoorLabelGif(Directions.EAST, GR,
											httpReq) + "</TD>");
							break;
						case 3: // alt e,w
						{
						...
						{
							buf.append("<TD>"
									+ getDoorLabelGif(Directions.SOUTHWEST, GR,
											httpReq) + "</TD>");
							buf.append("<TD><BR></TD>");
							buf.append("<TD>"
									+ getDoorLabelGif(Directions.SOUTH, GR,
											httpReq) + "</TD>");
							String alt = "<BR>";
							if (up == Directions.SOUTH)
								alt = getDoorLabelGif(Directions.UP, GR,
							...
							buf.append("<TD>" + alt + "</TD>");
							buf.append("<TD>"
									+ getDoorLabelGif(Directions.SOUTHEAST, GR,
											httpReq) + "</TD>");
						}
							break;
						}
	...
	protected GrinderRoom getRoomInDir(GrinderRoom room, int d) {
		...
		if ((xy[0] >= 0) && (xy[1] >= 0) && (xy[0] < grid.length)
				&& (xy[1] < grid[xy[0]].length)) {
			GR = grid[xy[0]][xy[1]];
			if (GR != null)
				return GR;
			xy = newXY(xy, d);
	...
	protected String getDoorLabelGif(int d, GrinderRoom room,
			HTTPRequest httpReq) {
		if ((room == null) || (room.doors == null) || (d >= room.doors.length))
			return "";
		...
		if ((dir == null) || (dir.room.length() == 0))
			return "<a href=\"javascript:EC('" + Directions.getDirectionName(d)
					+ "','" + room.roomID
					+ "');\"><IMG BORDER=0 SRC=\"images/E" + dirLetter
					+ ".gif\"></a>";
		...
		if ((dir.room.length() > 0)
				&& ((roomPointer == null) || (!roomPointer.roomID
						.equals(dir.room))))
			dirLetter += "R";
		String theRest = ".gif\" BORDER=0 ALT=\"" + dirName + " to " + dir.room
				+ "\"></a>";
		Exit exit = dir.exit;
		if (exit == null)
			return "<a href=\"javascript:CEX('" + dirName + "','" + room.roomID
					+ "','" + dir.room + "');\"><IMG SRC=\"images/U"
					+ dirLetter + theRest;
		else if (exit.hasADoor())
			return "<a href=\"javascript:CEX('" + dirName + "','" + room.roomID
					+ "','" + dir.room + "');\"><IMG SRC=\"images/D"
					+ dirLetter + theRest;
		else
			return "<a href=\"javascript:CEX('" + dirName + "','" + room.roomID
					+ "','" + dir.room + "');\"><IMG SRC=\"images/O"
					+ dirLetter + theRest;
	}
	...
	// this is much like getHTMLTable, but tiny rooms for world map viewing. No
	// exits or ID's for now.
	public StringBuffer getHTMLMap(HTTPRequest httpReq, int roomSize) {
		...
		for (int y = 0; y <= Ybound; y++) {
			...
			for (int x = 0; x <= Xbound; x++) {
				GrinderRoom GR = grid[x][y];
				String tdins = (boundsXYXY != null) ? " ID=X"
						+ (x + boundsXYXY[0]) + "_" + (y + boundsXYXY[1]) : "";
				if (GR == null)
