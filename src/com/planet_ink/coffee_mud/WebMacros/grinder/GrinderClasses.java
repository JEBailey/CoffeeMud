...
public class GrinderClasses {
	...
	public static String modifyCharClass(HTTPRequest httpReq,
			java.util.Map<String, String> parms, CharClass oldC, CharClass C) {
		...
		while (httpReq.isUrlParameter("NAME" + (++num)))
			if (CMath.isInteger(httpReq.getUrlParameter("NAMELEVEL" + (num)))) {
				int minLevel = CMath.s_int(httpReq.getUrlParameter("NAMELEVEL"
						+ (num)));
				...
				if ((name != null) && (name.length() > 0)) {
					if (DV.size() == 0)
						...
						for (int n = 0; n < DV.size(); n++)
							if (minLevel < ((Integer) DV.elementAt(n, 1))
									.intValue()) {
								DV.insertElementAt(n,
										Integer.valueOf(minLevel), name);
								added = true;
								break;
							} else if (minLevel == ((Integer) DV
		...
		for (int l = 0; l < DV.size(); l++) {
			...
			C.setStat("NAMELEVEL" + l,
					((Integer) DV.elementAt(l, 1)).toString());
		}
		...
		while (httpReq.getUrlParameter("MINSTAT" + x) != null) {
			...
			if ((minStat != null) && (minStat.length() > 0)
				...
				minStats.add(new Pair<String, Integer>(minStat, Integer
						.valueOf(CMath.s_int(statMin))));
			x++;
		}
		...
		List<Item> Ivs = GrinderRaces.itemList(oldC.outfit(null), 'O', httpReq,
				false);
		...
		while (httpReq.isUrlParameter("SSET" + (++num)))
			if (CMath.isInteger(httpReq.getUrlParameter("SSETLEVEL" + (num)))) {
				int minLevel = CMath.s_int(httpReq.getUrlParameter("SSETLEVEL"
						+ (num)));
		...
		for (int i = 0; i < DV.size(); i++) {
			...
			C.setStat("GETCABLEGAIN" + i, ((String) DV.elementAt(i, 4))
					.equalsIgnoreCase("on") ? "false" : "true");
