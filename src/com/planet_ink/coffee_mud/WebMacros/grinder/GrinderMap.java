...
public class GrinderMap extends GrinderFlatMap {
	...
	public void rebuildGrid() {
		...
		for (int x = 0; x < areaMap.size(); x++) {
			...
			if (GR.xy[0] < xoffset) {
				xoffset = GR.xy[0];
				if (debug)
					Log.sysOut("GR-REGRID", "xoffset set0: " + xoffset);
				if ((debug) && (GR.xy[0] > 0))
		...
		for (int y = 0; y < areaMap.size(); y++) {
			...
			if (room.z < minZ) {
				...
				if ((debug) && ((room.z > maxZ) || (room.z < minZ)))
					Log.sysOut("GR-HTML", "Room.z error: " + room.z
							+ " outside " + maxZ + "-" + minZ + "("
							+ room.roomID + ")");
	...
	protected boolean isEmptyCluster(Hashtable processed, int x, int y, int z) {
		for (Enumeration e = processed.elements(); e.hasMoreElements();) {
			GrinderRoom room = (GrinderRoom) e.nextElement();
			if ((((room.xy[0] > x - CLUSTERSIZE) && (room.xy[0] < x
					+ CLUSTERSIZE)) && ((room.xy[1] > y - CLUSTERSIZE) && (room.xy[1] < y
					+ CLUSTERSIZE)))
					|| ((room.xy[0] == x) && (room.xy[1] == y))
					&& (room.z == z))
				return false;
		}
		return true;
	...
	protected void findEmptyCluster(Hashtable processed, Vector XYZ) {
		...
		while (true) {
			for (int i = 0; i < 8; i++) {
				...
				if (isEmptyCluster(processed, x + (spacing * xadjust), y
						+ (spacing * yadjust), z)) {
					XYZ.setElementAt(Integer.valueOf(x + (spacing * xadjust)),
							0);
					XYZ.setElementAt(Integer.valueOf(y + (spacing * yadjust)),
							1);
	...
	public void placeRooms() {
		...
		for (int x = 0; x < areaMap.size(); x++) {
			...
			if (GR.z < minZ) {
				if (debug)
					Log.sysOut("GR-PLACERS", "minZ changed: " + minZ + " to "
							+ GR.z);
				minZ = GR.z;
			}
			if (GR.z > maxZ) {
				if (debug)
					Log.sysOut("GR-PLACERS", "maxZ changed: " + maxZ + " to "
							+ GR.z);
				maxZ = GR.z;
			}
		}
		...
		if (areaMap.size() > processed.size())
			Log.errOut("GrinderMap", areaMap.size() - processed.size()
	...
	public StringBuffer getHTMLTable(HTTPRequest httpReq) {
		...
		for (int z = minZ; z <= maxZ; z++)
			buf.append("<option value=\"MapLayer" + z + "\">Level " + z
					+ "</option>");
				...
				for (int l = 0; l < 5; l++) {
					...
					for (int x = 0; x <= Xbound; x++) {
						...
						if (GR == null)
							...
							case 0: { // north, up
								buf.append("<TD>"
										+ getDoorLabelGif(Directions.NORTHWEST,
												GR, httpReq) + "</TD>");
								buf.append("<TD><BR></TD>");
								buf.append("<TD>"
										+ getDoorLabelGif(Directions.NORTH, GR,
												httpReq) + "</TD>");
								buf.append("<TD>"
										+ getDoorLabelGif(Directions.UP, GR,
												httpReq) + "</TD>");
								buf.append("<TD>"
										+ getDoorLabelGif(Directions.NORTHEAST,
												GR, httpReq) + "</TD>");
								break;
							}
							case 1: { // west, east
								...
								buf.append((boundsXYXY != null) ? " ID=X"
										+ (x + boundsXYXY[0]) + "_"
										+ (y + boundsXYXY[1]) : "");
								buf.append(">");
								String roomID = GR.roomID;
								if (roomID.startsWith(area.Name() + "#")) {
									roomID = roomID.substring(roomID
											.indexOf('#'));
								}
								try {
									buf.append("<a name=\""
											+ URLEncoder.encode(GR.roomID,
													"UTF-8")
											+ "\" href=\"javascript:RC('"
											+ GR.roomID
											+ "');\"><FONT SIZE=-1><B>"
							...
							case 4: { // south, down
								buf.append("<TD>"
										+ getDoorLabelGif(Directions.SOUTHWEST,
												GR, httpReq) + "</TD>");
								buf.append("<TD><BR></TD>");
								buf.append("<TD>"
										+ getDoorLabelGif(Directions.SOUTH, GR,
												httpReq) + "</TD>");
								buf.append("<TD>"
										+ getDoorLabelGif(Directions.DOWN, GR,
												httpReq) + "</TD>");
								buf.append("<TD>"
										+ getDoorLabelGif(Directions.SOUTHEAST,
												GR, httpReq) + "</TD>");
								break;
							}
							default:
	...
	public void placeRoom(GrinderRoom room, int favoredX, int favoredY,
			Hashtable processed, boolean doNotDefer, boolean passTwo,
			int depth, int zLevel) {
		if (room == null)
			return;
		if (depth > 500)
			return;

		GrinderRoom anythingAt = getProcessedRoomAt(processed, favoredX,
				favoredY, zLevel);
		if (anythingAt != null) {
		...
		for (int d = 0; d < Directions.NUM_DIRECTIONS(); d++) {
			...
			if ((roomID != null)
				...
				if (nextRoom != null) {
					...
					placeRoom(nextRoom, newFavoredX, newFavoredY, processed,
							false, passTwo, depth + 1, newZLevel);
				}
			}
		}
	}
