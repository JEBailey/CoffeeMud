...
public class GrinderClanGovernments {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (last.length() > 0) {
			...
			if (CMath.isInteger(last)) {
			...
			} else if ((httpReq.getUrlParameter("NAME") == null)
					|| httpReq.getUrlParameter("NAME").length() == 0)
				return " @break@";
			else {
				...
				G = CMLib.clans().createGovernment(
			...
			if (str != null)
				G.setOverrideMinMembers((str.trim().length() == 0) ? null
						: Integer.valueOf(CMath.s_int(str)));
			str = httpReq.getUrlParameter("CONQUESTENABLED");
			G.setConquestEnabled((str == null) ? false : str
					.equalsIgnoreCase("on"));
			str = httpReq.getUrlParameter("CONQUESTITEMLOYALTY");
			G.setConquestItemLoyalty((str == null) ? false : str
					.equalsIgnoreCase("on"));
			str = httpReq.getUrlParameter("CONQUESTDEITYBASIS");
			G.setConquestByWorship((str == null) ? false : str
					.equalsIgnoreCase("on"));
			...
			if (str != null)
				G.setAutoPromoteBy((Clan.AutoPromoteFlag) CMath.s_valueOf(
						Clan.AutoPromoteFlag.values(), str));
			...
			while (httpReq.isUrlParameter("GPOSID_" + posDexStr)
					&& httpReq.getUrlParameter("GPOSID_" + posDexStr).trim()
							.length() > 0) {
				...
				int oldRank = CMath.s_int(httpReq.getUrlParameter("GPOSRANK_"
						+ posDexStr));
				int oldMax = CMath.s_int(httpReq.getUrlParameter("GPOSMAX_"
						+ posDexStr));
				String oldMask = httpReq.getUrlParameter("GPOSINNERMASK_"
						+ posDexStr);
				String oldIsPublicStr = httpReq.getUrlParameter("GPOSISPUBLIC_"
						+ posDexStr);
				...
				String authDexStr = "";
				int authDex = 0;
				while (httpReq.getUrlParameter("GPOSPOWER_" + posDexStr + "_"
						+ authDexStr) != null) {
					Clan.Function auth = (Clan.Function) CMath.s_valueOf(
							Clan.Function.values(),
							httpReq.getUrlParameter("GPOSPOWER_" + posDexStr
									+ "_" + authDexStr));
