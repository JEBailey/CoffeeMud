...
public class GrinderPlayers extends GrinderMobs {
	public final static String[] BASICS = { "NAME", "DESCRIPTION",
			...
			"FACTIONNAMES", "ACCTEXPUSED", "ACCTEXP", "ACCOUNT" };
	...
	public static String titleList(MOB E, HTTPRequest httpReq,
		...
		for (int i = 0; i < BASICS.length; i++)
			if (httpReq.isUrlParameter(BASICS[i])) {
				...
				case 25: {
					if (CMath.isInteger(old)) {
						...
						if ((a >= 0) && (a < Faction.Align.values().length))
							CMLib.factions().setAlignment(M,
									Faction.Align.values()[a]);
					} else {
						Faction.Align A = (Faction.Align) CMath.s_valueOf(
								Faction.Align.class, old.toUpperCase().trim());
						if (A != null)
				...
				case 26:
					...
					break;
				case 27:
					if (CMLib.map().getRoom(old) != null)
						M.setStartRoom(CMLib.map().getRoom(old));
					break;
				case 28:
					if (CMLib.map().getRoom(old) != null)
						M.setLocation(CMLib.map().getRoom(old));
					break;
				case 29:
					if (CMLib.map().getRoom(old) != null)
						M.setStartRoom(CMLib.map().getRoom(old));
				...
				case 56:
					...
					break;
				case 57: {
					List<String> V = CMParms.parseCommas(old.toUpperCase(),
							true);
					M.delAllExpertises();
				...
				case 58: {
					...
					for (int v = 0; v < V.size(); v++)
						M.addTattoo(CMLib.database().parseTattoo(
								(String) V.get(v)));
					break;
				}
				case 59: {
					if (M.playerStats() != null) {
						List<String> V = CMParms.parseCommas(old.toUpperCase(),
								true);
						M.playerStats().getSetSecurityFlags(
								CMParms.toSemicolonList(V));
					}
					break;
				}
				case 60:
				...
				case 63:
					if (M.playerStats() != null)
						M.playerStats().setAccountExpiration(
								CMLib.time().string2Millis(old));
					break;
				case 64: {
					if (M.playerStats() != null) {
						String oldAccountName = (M.playerStats().getAccount() != null) ? M
								.playerStats().getAccount().accountName()
								: "";
						if (!old.equals(oldAccountName)) {
							PlayerAccount newAccount = CMLib.players()
									.getLoadAccount(old);
							if (newAccount != null) {
								M.playerStats().setAccount(newAccount);
								newAccount.addNewPlayer(M);
								PlayerAccount oldAccount = (oldAccountName
										.length() > 0) ? CMLib.players()
										.getLoadAccount(oldAccountName) : null;
								if (oldAccount != null) {
									oldAccount.delPlayer(M);
									CMLib.database()
											.DBUpdateAccount(oldAccount);
								}
							}
						}
					}
	...
	public static String classList(MOB M, HTTPRequest httpReq,
			java.util.Map<String, String> parms) {
	...
	public static String editPlayer(MOB whom, HTTPRequest httpReq,
			java.util.Map<String, String> parms, MOB M) {
		...
		for (int i = 0; i < MOB.AUTODESC.length; i++) {
			if (httpReq.isUrlParameter(MOB.AUTODESC[i])) {
				...
				if (old.equalsIgnoreCase("on"))
					...
					M.setBitmap((int) CMath.unsetb(M.getBitmap(),
							CMath.pow(2, i)));
			}
		}
			...
			if ((F != null) && (old != null) && (old.length() > 0))
				for (Faction.Align v : Faction.Align.values())
					if ((v != Faction.Align.INDIFF)
							&& (v.toString().equalsIgnoreCase(old)))
