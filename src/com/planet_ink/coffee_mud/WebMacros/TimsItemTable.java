...
public class TimsItemTable extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (parms.containsKey("WORLD")) {
			...
			for (ItemCraftor skill : allCraftingSkills) {
				...
				for (int i = 0; i <= craftingSkills.size(); i++)
					if (i >= craftingSkills.size()) {
						...
						break;
					} else if (ii < CMLib.ableMapper().lowestQualifyingLevel(
							craftingSkills.get(i).ID())) {
						craftingSkills.add(i, skill);
						break;
					}
			}
			for (ItemCraftor skill : craftingSkills) {
				List<ItemCraftor.ItemKeyPair> pairs = skill
			...
			int[] materials = { RawMaterial.RESOURCE_STEEL,
					RawMaterial.RESOURCE_IRON, RawMaterial.RESOURCE_OAK,
					RawMaterial.RESOURCE_LEATHER, RawMaterial.RESOURCE_COTTON };
			str.append("<TR><TD>Type</TD><TD>Level</TD>");
			for (int m = 0; m < materials.length; m++)
				str.append("<TD>" + RawMaterial.CODES.NAME(materials[m])
						+ "</TD>");
			str.append("</TR>");
			...
			for (int level = 1; level <= 91; level++) {
				...
				for (int m = 0; m < materials.length; m++) {
					...
					str.append("<TD>" + W.basePhyStats().attackAdjustment()
							+ "/" + W.basePhyStats().damage() + "</TD>");
				}
				str.append("</TR>");
			}
		} else
			str.append("<TR><TD><FONT COLOR=RED>Unknown parms: "
					+ CMParms.toStringList(parms)
					+ ".  Try WORLD or ITEMS.</TD></TR>");
	...
	public String addRow(Item I) {
		...
		Ability RES = RET[1];
		Ability CAST = RET[2];
		int tlvl = CMLib.itemBuilder().timsLevelCalculator(I, ADJ, RES, CAST,
				castMul[0]);
		...
		if (!(I instanceof Weapon))
			row.append("<TD>" + I.basePhyStats().armor()
					+ "</TD><TD>&nbsp;</TD><TD>&nbsp;</TD>");
		else {
			row.append("<TD>&nbsp;</TD><TD>"
					+ I.basePhyStats().attackAdjustment() + "</TD>");
