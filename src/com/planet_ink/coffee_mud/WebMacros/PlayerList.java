		...
		for (Session S : CMLib.sessions().allIterable()) {
			...
			if ((m != null) && (m.name() != null) && (m.name().length() > 0)
				...
				if (CMSecurity.isASysOp(m))
					s.append("Archon");
				s.append("\">");
				s.append(CMStrings.removeColors(m.Name().equals(m.name()) ? m
						.titledName() : m.name()));
				s.append(" ");
				if (m.charStats().getMyRace() != null
					...
					if (m.charStats().displayClassName().length() > 0
							&& ((!m.charStats().displayClassName()
									.equals("MOB"))
									|| CMSecurity
											.isDisabled(CMSecurity.DisFlag.CLASSES) || m
						...
						if ((!CMSecurity.isDisabled(CMSecurity.DisFlag.CLASSES))
							...
							s.append(m.charStats().displayClassLevel(m, true));
								...
								.isDisabled(CMSecurity.DisFlag.CLASSES))
								&& (!m.charStats().getMyRace().classless()))
							s.append("" + m.charStats().displayClassName());
								...
								&& (!m.charStats().getCurrentClass().leveless()))
							s.append(""
									+ m.charStats().getClassLevel(
											m.charStats().getCurrentClass()));
