...
public class FileMgr extends StdWebMacro {
	...
	public boolean matches(String s1, String s2) {
		...
		if (s1.startsWith("*") && (s1.endsWith("*")))
			return s2.toUpperCase().indexOf(
					s1.toUpperCase().substring(1, s1.length() - 1)) >= 0;
		else if (s1.startsWith("*"))
			return s2.toUpperCase().endsWith(s1.toUpperCase().substring(1));
		else if (s1.endsWith("*"))
			return s2.toUpperCase().startsWith(
					s1.toUpperCase().substring(0, s1.length() - 1));
	...
	public void compileFilenamesList(CMFile F, String regex, Vector V) {
		...
		for (int l = 0; l < list.length; l++) {
			...
			if (F2.isDirectory()
					&& !(path + list[l]).equalsIgnoreCase("/resources/map"))
	...
	public void compileTextListFromFiles(Vector files, String regex, Vector V) {
		...
		for (int f = 0; f < files.size(); f++) {
			StringBuffer buf = new CMFile((String) files.elementAt(f), null)
					.text();
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		try {
			...
			if ((filePath.length() > 2) && (!filePath.endsWith("/")))
				filePath += "/";
			String prefix = "";
			if (parms.containsKey("VFS") || parms.containsKey("LOCAL")
			...
			if (parms.containsKey("DELETE")) {
				if (F.delete()) {
					for (final Iterator<String> i = Resources
							.findResourceKeys(file); i.hasNext();)
						Resources.removeResource(i.next());
					return "File `" + last + "` was deleted.";
				}
				return "File `" + last
						+ "` was NOT deleted. Perhaps it`s read-only?";
				...
				if (parms.containsKey("VFS") || parms.containsKey("LOCAL")
					 ...
					 * !parms.containsKey("BOTH")) { CMFile dF=new
					 * CMFile("::"+filePath+file,M); if(dF.canVFSEquiv()) {
					 * if(!dF.deleteVFS())
					 ...
					 * !parms.containsKey("BOTH")) { CMFile dF=new
					 * CMFile("//"+filePath+file,M); if(dF.canLocalEquiv()) {
					 * if(!dF.deleteLocal())
					 * returnMsg.append("File `//"+last+"` not deleted -- error!  "
					 * ); else
					 * returnMsg.append("File `//"+last+"` successfully deleted"
					...
					if ((!F.canWrite()) || (!F.saveText(s)))
						returnMsg.append("File `" + prefix + last
								+ "` not updated -- error!");
					if (parms.containsKey("BOTH")) {
						F = new CMFile("//" + filePath + file, M);
						if ((!F.canWrite()) || (!F.saveText(s)))
							returnMsg.append("File `//" + last
									+ "` not updated -- error!");
					}
				...
				if (!F.exists())
					return "Directory '" + F.getAbsolutePath()
							+ "' does not exists -- directory not deleted!";
				if (!F.delete())
				...
				for (MultiPartData d : httpReq.getMultiParts()) {
					if ((d.getVariables().containsKey("filename"))
							&& (d.getData() != null)) {
						F = new CMFile(prefix + filePath
								+ d.getVariables().get("filename"), M);
