	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if ((last != null) && (last.length() > 0)) {
			for (AreaScriptInstance inst : subList) {
				String hostName = CMParms.combineWith(inst.path, '.', 0,
						inst.path.size()) + "." + inst.fileName;
		...
		if (parms.containsKey("NEXT") || parms.containsKey("RESET")) {
			...
			for (AreaScriptInstance inst : subList) {
				String hostName = CMParms.combineWith(inst.path, '.', 0,
						inst.path.size()) + "." + inst.fileName;
				if ((last == null)
						|| ((last.length() > 0) && (last.equals(lastID)) && (!hostName
								.equals(lastID)))) {
		...
		if (parms.containsKey("ENTRYPATH") && (entry != null))
			str.append(CMParms.combineWith(entry.path, '.', 0,
					entry.path.size())
					+ ", ");
		...
		if (parms.containsKey("ENCODEDFILENAME") && (entry != null)) {
			...
			try {
				str.append(URLEncoder.encode(
						((x < 0) ? path : path.substring(x + 1)), "UTF-8")
						+ ", ");
			} catch (Exception e) {
			}
		}
