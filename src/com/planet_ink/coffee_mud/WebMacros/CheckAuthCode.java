...
public class CheckAuthCode extends StdWebMacro {
	...
	public Hashtable getAuths(HTTPRequest httpReq) {
		...
		if (mob == null)
			return null;
		Hashtable auths = (Hashtable) httpReq.getRequestObjects().get(
				"AUTHS_" + mob.Name().toUpperCase().trim());
		if (auths == null) {
			auths = new Hashtable();
			boolean subOp = false;
			...
			auths.put(
					...
					""
							+ (CMSecurity.isAllowedEverywhere(mob,
									...
									.isAllowedEverywhere(mob,
											CMSecurity.SecFlag.CMDAREAS)));
			final List<String> dirs = CMSecurity.getAccessibleDirs(mob,
					mob.location());
			...
			for (Iterator<CMSecurity.SecFlag> i = CMSecurity.getSecurityCodes(
					mob, R); i.hasNext();)
				auths.put("AUTH_" + i.next().toString(), "true");
			httpReq.getRequestObjects().put(
					"AUTHS_" + mob.Name().toUpperCase().trim(), auths);
		}
		return auths;
	}
