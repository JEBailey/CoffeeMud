...
public class CatalogCatNext extends StdWebMacro {
	...
	public String runMacro(HTTPRequest httpReq, String parm) {
		...
		if (!httpReq.getRequestObjects().containsKey(
				"CATACATS" + (mobs ? "M" : "I")))
			httpReq.getRequestObjects().put(
					"CATACATS" + (mobs ? "M" : "I"),
					mobs ? CMLib.catalog().getMobCatalogCatagories() : CMLib
							.catalog().getItemCatalogCatagories());
		String[] cats = (String[]) httpReq.getRequestObjects().get(
				"CATACATS" + (mobs ? "M" : "I"));
		...
		for (String cat : cats) {
			if (cat.length() == 0)
				cat = "UNCATEGORIZED";
			if ((last == null)
					|| ((lastID != null) && (last.equals(lastID)) && (!cat
							.equals(lastID)))) {
