...
public class Chicken extends StdRace {
	...
	private static final int[] parts = { 0, 2, 0, 1, 1, 0, 0, 1, 2, 2, 0, 0, 1,
			1, 0, 2 };
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((tickID == Tickable.TICKID_MOB) && (ticking instanceof MOB)) {
			if ((CMLib.dice().rollPercentage() > 99)
					&& (((MOB) ticking).numItems() < 9)) {
			...
			if ((((MOB) ticking).numItems() > 5)
				...
				if (I != null) {
					((MOB) ticking).location().show(((MOB) ticking), null,
							CMMsg.MSG_NOISYMOVEMENT, "<S-NAME> lay(s) an egg.");
					I.removeFromOwnerContainer();
					I.executeMsg((MOB) ticking, CMClass.getMsg((MOB) ticking,
							I, null, CMMsg.TYP_ROOMRESET, null));
					((MOB) ticking).location().addItem(I,
