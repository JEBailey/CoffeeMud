...
public class GenRace extends StdRace {
	...
	public String racialParms() {
		...
		str.append(CMLib.xml()
				.convertXMLtoTag("FHEIGHT", "" + shortestFemale()));
		str.append(CMLib.xml()
				.convertXMLtoTag("VHEIGHT", "" + heightVariance()));
		str.append(CMLib.xml()
				.convertXMLtoTag("BWEIGHT", "" + lightestWeight()));
		str.append(CMLib.xml()
				.convertXMLtoTag("VWEIGHT", "" + weightVariance()));
		str.append(CMLib.xml()
				.convertXMLtoTag("WEAR", "" + forbiddenWornBits()));
		str.append(CMLib.xml().convertXMLtoTag("AVAIL", "" + availability));
		str.append(CMLib.xml().convertXMLtoTag("DESTROYBODY",
				"" + destroyBodyAfterUse()));
		...
		str.append(CMLib.xml().convertXMLtoTag("HEALTHRACE",
				getRaceLocatorID(healthBuddy)));
		str.append(CMLib.xml().convertXMLtoTag("EVENTRACE",
				getRaceLocatorID(eventBuddy)));
		str.append(CMLib.xml().convertXMLtoTag("WEAPONRACE",
				getRaceLocatorID(weaponBuddy)));
		...
		str.append(CMLib.xml().convertXMLtoTag("AGING",
				CMParms.toStringList(getAgingChart())));
		if (adjPStats == null)
			str.append("<ESTATS/>");
		else
		...
		if (myResources().size() == 0)
			...
			for (RawMaterial I : myResources()) {
				str.append("<RSCITEM>");
				str.append(CMLib.xml().convertXMLtoTag("ICLASS",
						CMClass.classID(I)));
				str.append(CMLib.xml().convertXMLtoTag("IDATA",
		...
		if (naturalWeapon == null)
			...
			str.append("<WEAPON>");
			str.append(CMLib.xml().convertXMLtoTag("ICLASS",
					CMClass.classID(naturalWeapon)));
			str.append(CMLib.xml().convertXMLtoTag("IDATA",
					CMLib.xml().parseOutAngleBrackets(naturalWeapon.text())));
			str.append("</WEAPON>");
		}
		if ((racialAbilityNames == null) || (racialAbilityNames.length == 0))
			...
			for (int r = 0; r < racialAbilityNames.length; r++) {
				...
				str.append("<RPROFF>" + racialAbilityProficiencies[r]
						+ "</RPROFF>");
		...
		str.append("<BREATHELIST>")
				.append(CMParms.toStringList(sortedBreathables))
				.append("</BREATHELIST>");
		...
		if ((culturalAbilityNames == null)
			...
			for (int r = 0; r < culturalAbilityNames.length; r++) {
				...
				str.append("<CPROFF>" + culturalAbilityProficiencies[r]
						+ "</CPROFF>");
				str.append("</CABILITY>");
			}
			str.append("</CABILITIES>");
	...
	public void setRacialParms(String parms) {
		...
		List<XMLLibrary.XMLpiece> raceData = CMLib.xml().getContentsFromPieces(
				xml, "RACE");
		if (raceData == null) {
			Log.errOut("GenRace", "Unable to get RACE data: (" + parms.length()
					+ "): " + CMStrings.padRight(parms, 30) + ".");
			return;
		}
		...
		helpEntry = CMLib.xml().restoreAngleBrackets(
				CMLib.xml().getValFromPieces(raceData, "HELP"));
		String playerval = CMLib.xml().getValFromPieces(raceData, "PLAYER")
				.trim().toUpperCase();
		...
		String avail = CMLib.xml().getValFromPieces(raceData, "AVAIL").trim()
				.toUpperCase();
		if ((avail != null) && (avail.length() > 0) && (CMath.isNumber(avail)))
			availability = CMath.s_int(avail);
		destroyBodyAfterUse = CMLib.xml().getBoolFromPieces(raceData,
				"DESTROYBODY");
		leaveStr = CMLib.xml().getValFromPieces(raceData, "LEAVE");
		arriveStr = CMLib.xml().getValFromPieces(raceData, "ARRIVE");
		setStat("HEALTHRACE",
				CMLib.xml().getValFromPieces(raceData, "HEALTHRACE"));
		setStat("EVENTRACE", CMLib.xml()
				.getValFromPieces(raceData, "EVENTRACE"));
		setStat("WEAPONRACE",
				CMLib.xml().getValFromPieces(raceData, "WEAPONRACE"));
		...
		if ((xV != null) && (xV.size() > 0)) {
			...
			for (int x = 0; x < xV.size(); x++) {
				...
				if ((!iblk.tag.equalsIgnoreCase("RSCITEM"))
						|| (iblk.contents == null))
					continue;
				Item I = CMClass.getItem(CMLib.xml().getValFromPieces(
		...
		naturalWeapon = null;
		List<XMLLibrary.XMLpiece> wblk = CMLib.xml().getContentsFromPieces(
				raceData, "WEAPON");
		if (wblk != null) {
			naturalWeapon = CMClass.getWeapon(CMLib.xml().getValFromPieces(
					wblk, "ICLASS"));
			String idat = CMLib.xml().getValFromPieces(wblk, "IDATA");
			if ((idat != null) && (naturalWeapon != null)) {
				naturalWeapon.setMiscText(CMLib.xml()
						.restoreAngleBrackets(idat));
		...
		if ((xV != null) && (xV.size() > 0)) {
			...
			for (int x = 0; x < xV.size(); x++) {
				XMLLibrary.XMLpiece iblk = xV.get(x);
				if ((!iblk.tag.equalsIgnoreCase("RABILITY"))
						|| (iblk.contents == null))
					continue;
				racialAbilityNames[x] = CMLib.xml().getValFromPieces(
						iblk.contents, "RCLASS");
				racialAbilityProficiencies[x] = CMLib.xml().getIntFromPieces(
						iblk.contents, "RPROFF");
				racialAbilityQuals[x] = CMLib.xml().getBoolFromPieces(
						iblk.contents, "RAGAIN");
				racialAbilityLevels[x] = CMLib.xml().getIntFromPieces(
						iblk.contents, "RLEVEL");
			}
		}

		sortedBreathables = CMParms.toIntArray(CMParms.parseCommas(
				CMLib.xml().getValFromPieces(raceData, "BREATHELIST",
						"" + RawMaterial.RESOURCE_AIR), true));
		...
		if ((xV != null) && (xV.size() > 0)) {
			...
			for (int x = 0; x < xV.size(); x++) {
				XMLLibrary.XMLpiece iblk = xV.get(x);
				if ((!iblk.tag.equalsIgnoreCase("REFFECT"))
						|| (iblk.contents == null))
					continue;
				racialEffectNames[x] = CMLib.xml().getValFromPieces(
						iblk.contents, "RFCLASS");
				racialEffectParms[x] = CMLib.xml().getValFromPieces(
						iblk.contents, "RFPARM");
				racialEffectLevels[x] = CMLib.xml().getIntFromPieces(
						iblk.contents, "RFLEVEL");
			}
		}
		...
		if ((xV != null) && (xV.size() > 0)) {
			...
			for (int x = 0; x < xV.size(); x++) {
				...
				if ((!iblk.tag.equalsIgnoreCase("CABILITY"))
						|| (iblk.contents == null))
					continue;
				culturalAbilityNames[x] = CMLib.xml().getValFromPieces(
						iblk.contents, "CCLASS");
				culturalAbilityProficiencies[x] = CMLib.xml().getIntFromPieces(
						iblk.contents, "CPROFF");
			}
		}
		...
		for (int i = this.getSaveStatIndex(); i < getStatCodes().length; i++)
			setStat(getStatCodes()[i],
					CMLib.xml().getValFromPieces(raceData, getStatCodes()[i]));
	}

	protected static String[] CODES = { "ID", "NAME", "CAT", "WEAR", "VWEIGHT",
			"BWEIGHT", "VHEIGHT", "FHEIGHT", "MHEIGHT", "AVAIL", "LEAVE",
			"ARRIVE", "HEALTHRACE", "BODY", "ESTATS", "ASTATS", "CSTATS",
			"ASTATE", "NUMRSC", "GETRSCID", "GETRSCPARM", "WEAPONCLASS",
			"WEAPONXML", "NUMRABLE", "GETRABLE", "GETRABLEPROF",
			"GETRABLEQUAL", "GETRABLELVL", "NUMCABLE", "GETCABLE",
			"GETCABLEPROF", "NUMOFT", "GETOFTID", "GETOFTPARM", "BODYKILL",
	...
	public String getStat(String code) {
		...
		case 23:
			return (racialAbilityNames == null) ? "0"
					: ("" + racialAbilityNames.length);
		case 24:
			return (racialAbilityNames == null) ? ""
					: ("" + racialAbilityNames[num]);
		case 25:
			return (racialAbilityProficiencies == null) ? "0"
					: ("" + racialAbilityProficiencies[num]);
		case 26:
			return (racialAbilityQuals == null) ? "false"
					: ("" + racialAbilityQuals[num]);
		case 27:
			return (racialAbilityLevels == null) ? "0"
					: ("" + racialAbilityLevels[num]);
		case 28:
			return (culturalAbilityNames == null) ? "0"
					: ("" + culturalAbilityNames.length);
		case 29:
			return (culturalAbilityNames == null) ? ""
					: ("" + culturalAbilityNames[num]);
		case 30:
			return (culturalAbilityProficiencies == null) ? "0"
					: ("" + culturalAbilityProficiencies[num]);
		case 31:
		...
		case 34:
			return "" + destroyBodyAfterUse();
		case 35:
			return (racialEffectNames == null) ? "0"
					: ("" + racialEffectNames.length);
		case 36:
			return (racialEffectNames == null) ? ""
					: ("" + racialEffectNames[num]);
		case 37:
			return (racialEffectParms == null) ? "0"
					: ("" + racialEffectParms[num]);
		case 38:
			return (racialEffectLevels == null) ? "0"
					: ("" + racialEffectLevels[num]);
		case 39:
		...
		case 44:
			return helpEntry;
		case 45:
			return CMParms.toStringList(sortedBreathables);
		default:
	...
	public void setStat(String code, String val) {
		...
		case 8:
			...
			break;
		case 9:
			availability = CMath.s_parseBitIntExpression(Area.THEME_BIT_NAMES,
					val);
			break;
		case 10:
			leaveStr = val;
		...
		case 12: {
			...
			try {
				if (healthBuddy == null)
					healthBuddy = (Race) CMClass.getLoadNewClassInstance(
							CMObjectType.RACE, val, true);
			} catch (Exception e) {
			}
			break;
		}
		...
		case 42: {
			...
			if (eventBuddy == null)
				eventBuddy = (Race) CMClass.getLoadNewClassInstance(
						CMObjectType.RACE, val, true);
			break;
		}
		case 43: {
			...
			if (weaponBuddy == null)
				weaponBuddy = (Race) CMClass.getLoadNewClassInstance(
						CMObjectType.RACE, val, true);
			break;
		}
		case 44: {
		...
		case 45: {
			sortedBreathables = CMParms.toIntArray(CMParms.parseCommas(val,
					true));
			Arrays.sort(sortedBreathables);
			break;
		}
