...
public class Halfling extends StdRace {
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_MAX_STRENGTH_ADJ,
				affectableStats.getStat(CharStats.STAT_MAX_STRENGTH_ADJ) - 1);
		affectableStats.setStat(CharStats.STAT_SAVE_PARALYSIS,
				affectableStats.getStat(CharStats.STAT_SAVE_PARALYSIS) + 10);
		affectableStats.setStat(CharStats.STAT_SAVE_DETECTION,
				affectableStats.getStat(CharStats.STAT_SAVE_DETECTION) + 10);
	}
	...
	public String healthText(MOB viewer, MOB mob) {
		...
		if (pct < .10)
			...
			return "^r" + mob.name(viewer)
					+ "^r is covered in small streams of blood.^N";
		else if (pct < .30)
			return "^r" + mob.name(viewer)
					+ "^r is bleeding badly from lots of small wounds.^N";
		else if (pct < .40)
			return "^y" + mob.name(viewer)
					+ "^y has numerous bloody wounds and small gashes.^N";
		else if (pct < .50)
			return "^y" + mob.name(viewer)
					+ "^y has some bloody wounds and small gashes.^N";
		else if (pct < .60)
			return "^p" + mob.name(viewer)
					+ "^p has a few small bloody wounds.^N";
		else if (pct < .70)
			return "^p" + mob.name(viewer)
					+ "^p is cut and bruised in small places.^N";
		else if (pct < .80)
			return "^g" + mob.name(viewer)
					+ "^g has some small cuts and bruises.^N";
		else if (pct < .90)
			return "^g" + mob.name(viewer)
					+ "^g has a few bruises and small scratches.^N";
		else if (pct < .99)
	...
	public List<RawMaterial> myResources() {
		synchronized (resources) {
			if (resources.size() == 0) {
				resources.addElement(makeResource("a pair of hairy "
						+ name().toLowerCase() + " feet",
