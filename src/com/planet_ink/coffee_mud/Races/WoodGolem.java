...
public class WoodGolem extends StdRace {
	...
	public String healthText(MOB viewer, MOB mob) {
		...
		if (pct < .10)
			...
			return "^r" + mob.name(viewer)
					+ "^r is massively splintered and damaged.^N";
		else if (pct < .30)
			return "^r" + mob.name(viewer)
					+ "^r is extremely splintered and damaged.^N";
		else if (pct < .40)
			return "^y" + mob.name(viewer)
					+ "^y is very splintered and damaged.^N";
		else if (pct < .50)
			return "^y" + mob.name(viewer) + "^y is splintered and damaged.^N";
		else if (pct < .60)
			return "^p" + mob.name(viewer)
					+ "^p is splintered and slightly damaged.^N";
		else if (pct < .70)
	...
	public List<RawMaterial> myResources() {
		synchronized (resources) {
			if (resources.size() == 0) {
				resources.addElement(makeResource("a pound of oak",
