...
public class MetalGolem extends StdRace {
	...
	public String healthText(MOB viewer, MOB mob) {
		...
		if (pct < .10)
			...
			return "^r" + mob.name(viewer)
					+ "^r is massively dented and damaged.^N";
		else if (pct < .30)
			return "^r" + mob.name(viewer)
					+ "^r is extremely dented and damaged.^N";
		else if (pct < .40)
			...
			return "^p" + mob.name(viewer)
					+ "^p is dented and slightly damaged.^N";
		else if (pct < .70)
	...
	public List<RawMaterial> myResources() {
		synchronized (resources) {
			if (resources.size() == 0) {
				resources.addElement(makeResource("a pound of iron",
