...
public class Fish extends StdRace {
	...
	private static final int[] parts = { 0, 2, 0, 1, 0, 0, 0, 1, 0, 0, 0, 2, 1,
			0, 1, 0 };
	...
	public boolean okMessage(Environmental affected, CMMsg msg) {
		if ((msg.targetMinor() == CMMsg.TYP_ENTER)
				...
				&& (Arrays.binarySearch(getBreathables(),
						((Room) msg.target()).getAtmosphere()) < 0)) {
		...
		synchronized (resources) {
			if (resources.size() == 0) {
				...
				resources.addElement(makeResource("a " + name().toLowerCase()
						+ " scales", RawMaterial.RESOURCE_SCALES));
				resources.addElement(makeResource("some "
