...
public interface Race extends Tickable, StatsAffecting, MsgListener, CMObject,
	 ...
	 * the race is available for selection by users at char creation time,
	 * whether they can change to this race via spells, or whether the race is
	...
	/**
	 * After a mob is set or changed to a new race, this method should be called
	 * to finalize or initialize any settings from this race. The verify flag is
	 * almost always true, unless the mob is a new player being created, in
	 * which case false is sent.
	 * 
	 * @param mob
	 *            the mob or player being set to this race
	 * @param verifyOnly
	 *            true flag unless this is a new player character
	 */
	public void startRacing(MOB mob, boolean verifyOnly);

	/**
	 * Will initialize a player or mobs height and weight based on this races
	 * parameters.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.PhyStats
	 * @param stats
	 *            the PhyStats object to change
	 * @param gender
	 *            the mobs gender 'M' or 'F'
	 */
	...
	/**
	 * The amount from 0-this to add to the minimum height to achieve a random
	 * height.
	 * 
	...
	/**
	 * The amount from 0-this to add to the minumum weight to achieve a random
	 * weight.
	 * 
	...
	/**
	 * Returns an integer array equal in size and index to the Race.AGE_*
	 * constants in the Race interface. Each value in the index represents the
	 * first mudyear age of that age category.
	 * 
	 * @see Race
	...
	/**
	 * A bitmap showing which on locations a member of this race can not wear
	 * clothing, even if the members have one or more of the required limbs. The
	 * bitmap is made from Wearable.WORN_* constant values.
	 * 
	...
	/**
	 * Returns an array indexed by body part codes as defined by the BODY_*
	 * constants in the Race interface. Each value is either -1 to show that the
	 * body part does not apply, 0 to show that the body part is not found on
	 * this race, and 1 or more to show how many of that part this race normally
	 * has.
	 * 
	...
	/**
	 * Converts this race to a generic race (if it isn't already) and returns
	 * it. Returns itself if its already generic.
	 * 
	...
	/**
	 * Converts this race to a generic race (if it isn't already) and mixes its
	 * attributes with the race passed it. A new race ID and a new race name
	 ...
	 * weapons, or other objects commonly given to players of this race just
	 * starting out.
	 * 
	 * @param myChar
	...
	/**
	 * Returns a description of the given mobs description, by consulting the
	 * mobs curState().getHitPoints method.
	 * 
	...
	/**
	 * Sends back a generic mob name appropriate to a mob of this race, at the
	 * given gender and age-group.
	 * 
	 * @param gender
	 *            the gender of the mob
	 * @param age
	 *            the age category
	 * @return a good name
	 */
	...
	/**
	 * Returns a Weapon object representing what a member of this race fights
	 * with when unarmed. This method may change what it returns on every call
	...
	/**
	 * Returns a Vector of RawMaterial objects (usually GenFoodResource,
	 * GenLiquidResource, or GenResource items) representing what is left over
	 * of a member of this race after they've been butchered and cut up.
	 * 
	 * @see com.planet_ink.coffee_mud.Items.interfaces.RawMaterial
	...
	/**
	 * Returns the corpse of a member of this race, populates it with the
	 * equipment of the given mob, and places it in the given room. If the
	 * destroyBodyAfterUse returns true, it will also populate the body with the
	 * contents of the myResources method.
	 * 
	 * @see #myResources()
	 * @param mob
	 *            the mob to use as a template for the body
	 * @param room
	...
	/**
	 * Whether this race object represents a Generic Race, or one which is
	 * modifiable by builders at run-time.
	 * 
	...
	/**
	 * If this race is modifiable at run time, this method will return an xml
	 * document describing the several attributes of this race.
	 * 
	 * @see #isGeneric()
	...
	/**
	 * If this race is modifiable at run time, this method will use the given
	 * xml document describing the several attributes of this race to populate
	 * this races fields and attributes.
	 * 
	 * @see #isGeneric()
	 * @see #racialParms()
	 * @param parms
	...
	/**
	 * Returns the string describing what folks see when a member of this race
	 * enters a room. Should give an idea of the gate or walking style of this
	 * race.
	 * 
	...
	/**
	 * Returns the string describing what folks see when a member of this race
	 * leaves a room. Should give an idea of the gate or walking style of this
	 * race.
	 * 
	 ...
	 * this race. If there are any special things which need to be done to a
	 ...
	 * 
	 * @param host
	 *            the player or mob whose race object this is
	 * @param mob
	 ...
	 * Usually just ID().equals(ID()), or if either is human. Passing the race
	 * to itself in this method is a good way to check for general fertility.
	 * 
	 * @param R
	 *            the race to check
	...
	/**
	 * Return a vector of skills, spells, and other ability ids granted to the
	 * given mob when they are created as this race.
	 * 
	...
	/**
	 * Return a vector of skills, spells, and other abilities granted to the
	 * given mob of the given mobs level. This method is not functionally used
	 * because it doesn't quite work correctly yet.
	 * 
	 * @see com.planet_ink.coffee_mud.Abilities.interfaces.Ability
	...
	/**
	 * Apply any affects of the given mob at the given age to the given base
	 * and/or current char stats.
	 * 
	 * @see com.planet_ink.coffee_mud.Common.interfaces.CharState
	 * @param mob
	 *            the mob to apply changes to
	 * @param baseStats
	 *            permanent charstats changes
	 * @param charStats
	 *            temporary charstats changes
	 */
	...
	/**
	 * Returns the list of modifications to disposition done by this race or
	 * nothing.
	 * 
	...
	/**
	 * Returns the list of racial abilities granted to those of this race or
	 * nothing.
	 * 
	...
	/**
	 * Returns the list of racial languages granted to those of this race or
	 * nothing.
	 * 
	...
	/**
	 * Constant string list for the names of the age constants, in their order
	 * of value
	 */
	public final static String[] AGE_DESCS = { "Infant", "Toddler", "Child",
			"Young adult", "Adult", "Mature", "Old", "Venerable", "Ancient" };
	...
	/**
	 * constant string list naming each of the BODY_* constants in the order of
	 * their value
	 */
	public final static String[] BODYPARTSTR = { "ANTENEA", "EYE", "EAR",
			"HEAD", "NECK", "ARM", "HAND", "TORSO", "LEG", "FOOT", "NOSE",
			"GILL", "MOUTH", "WAIST", "TAIL", "WING" };
	/** constant hash of BODYPARTSTR */
	public final static Hashtable<Object, Integer> BODYPARTHASH = CMStrings
			.makeNumericHash(BODYPARTSTR);
	 ...
	 * of their value
	 */
	public final static String[] GENFLAG_DESCS = { "CLASSLESS", "LEVELLESS",
			"EXPLESS", "CHARMLESS", "CHILDLESS" };

	/** array mapping worn locations to body parts, indexed by body parts. */
	public final static long[] BODY_WEARVECTOR = { Wearable.WORN_HEAD, // ANTENEA,
																		// having
																		// any
																		// of
																		// these
			...
			Wearable.WORN_HANDS, // HANDS, gains a wear position here for every
									// 1
			Wearable.WORN_TORSO, // TORSO, gains a wear position here for every
									// 1
			...
			Wearable.WORN_HEAD, // NOSE, No applicable wear position for this
								// body part
			Wearable.WORN_HEAD, // GILLS, No applicable wear position for this
								// body part
			Wearable.WORN_MOUTH, // MOUTH, gains a wear position here for every
									// 1
			Wearable.WORN_WAIST, // WAIST, gains a wear position here for every
									// 1
	 ...
	 * having two values: the first being the Wearable.WORN_ location which is
	 * affected by having or losing this body part, and then the number of such
	 * body parts necessary to gain or lose one such wear location. A value of
	 * -1 means N/A
	 */
	public final static long[][] BODY_WEARGRID = {
			{ Wearable.WORN_HEAD, -1 }, // ANTENEA, having any of these removes
										// that pos
			{ Wearable.WORN_EYES, 2 }, // EYES, having any of these adds this
										// position
			{ Wearable.WORN_EARS, 2 }, // EARS, gains a wear position here for
										// every 2
			{ Wearable.WORN_HEAD, 1 }, // HEAD, gains a wear position here for
										// every 1
			{ Wearable.WORN_NECK, 1 }, // NECK, gains a wear position here for
										// every 1
			{ Wearable.WORN_ARMS, 2 }, // ARMS, gains a wear position here for
										// every 2
			{
					Wearable.WORN_WIELD | Wearable.WORN_HELD
							| Wearable.WORN_HANDS | Wearable.WORN_LEFT_FINGER
							| Wearable.WORN_LEFT_WRIST
							| Wearable.WORN_RIGHT_FINGER
							| Wearable.WORN_RIGHT_WRIST, 1 }, // HANDS, gains a
																// wear position
																// here for
																// every 1
			// lots of exceptions apply to the above
			{ Wearable.WORN_TORSO | Wearable.WORN_BACK, 1 }, // TORSO, gains a
																// wear position
																// here for
																// every 1
			{ Wearable.WORN_LEGS, 2 }, // LEGS, gains a wear position here for
										// every 2
			{ Wearable.WORN_FEET, 2 }, // FEET, gains a wear position here for
										// every 2
			{ -1, -1 }, // NOSE, No applicable wear position for this body part
			{ -1, -1 }, // GILLS, No applicable wear position for this body part
			{ Wearable.WORN_MOUTH, 1 }, // MOUTH, gains a wear position here for
										// every 1
			{ Wearable.WORN_WAIST, 1 }, // WAIST, gains a wear position here for
										// every 1
			{ -1, -1 }, // TAIL, having any of these removes that pos
			{ Wearable.WORN_BACK, -1 }, // WINGS, having any of these removes
										// that pos
	};
}
