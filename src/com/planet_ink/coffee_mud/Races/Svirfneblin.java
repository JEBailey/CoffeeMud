...
public class Svirfneblin extends StdRace {
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_MAX_STRENGTH_ADJ,
				affectableStats.getStat(CharStats.STAT_MAX_STRENGTH_ADJ) + 2);
		...
		affectableStats
				.setStat(CharStats.STAT_MAX_INTELLIGENCE_ADJ, affectableStats
		...
		affectableStats.setStat(CharStats.STAT_MAX_WISDOM_ADJ,
				affectableStats.getStat(CharStats.STAT_MAX_WISDOM_ADJ) - 1);
		affectableStats.setStat(CharStats.STAT_SAVE_MIND,
				affectableStats.getStat(CharStats.STAT_SAVE_MIND) + 10);
		affectableStats.setStat(CharStats.STAT_SAVE_OVERLOOKING,
				affectableStats.getStat(CharStats.STAT_SAVE_OVERLOOKING) + 10);
	}
	...
	public String healthText(MOB viewer, MOB mob) {
		...
		if (pct < .10)
			return "^r" + mob.name(viewer)
					+ "^r is curiously close to death.^N";
		else if (pct < .20)
			return "^r" + mob.name(viewer)
					+ "^r is covered in excessive bloody wounds.^N";
		else if (pct < .30)
			return "^r" + mob.name(viewer)
					+ "^r is bleeding badly from a plethora of small wounds.^N";
		else if (pct < .40)
			return "^y" + mob.name(viewer)
					+ "^y has numerous bloody wounds and unexpected gashes.^N";
		else if (pct < .50)
			return "^y" + mob.name(viewer)
					+ "^y has some alarming wounds and small gashes.^N";
		else if (pct < .60)
			return "^p" + mob.name(viewer)
					+ "^p has some small unwanted bloody wounds.^N";
		else if (pct < .70)
			return "^p" + mob.name(viewer)
					+ "^p is cut and bruised in strange places.^N";
		else if (pct < .80)
			return "^g" + mob.name(viewer)
					+ "^g has some small cuts and bruises.^N";
		else if (pct < .90)
			return "^g" + mob.name(viewer)
					+ "^g has a few bruises and interesting scratches.^N";
		else if (pct < .99)
			return "^g" + mob.name(viewer)
					+ "^g has a few small curious bruises.^N";
		else
