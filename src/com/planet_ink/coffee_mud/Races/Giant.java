	...
	public String healthText(MOB viewer, MOB mob) {
		...
		if (pct < .10)
			...
			return "^r" + mob.name(viewer)
					+ "^r is bleeding badly from lots of large wounds.^N";
		else if (pct < .40)
			return "^y" + mob.name(viewer)
					+ "^y has enormous bloody wounds and gashes.^N";
		else if (pct < .50)
			return "^y" + mob.name(viewer)
					+ "^y has some huge wounds and gashes.^N";
		else if (pct < .60)
			return "^p" + mob.name(viewer)
					+ "^p has a few huge bloody wounds.^N";
		else if (pct < .70)
			return "^p" + mob.name(viewer)
					+ "^p has huge cuts and is heavily bruised.^N";
		else if (pct < .80)
			return "^g" + mob.name(viewer)
					+ "^g has some large cuts and huge bruises.^N";
		else if (pct < .90)
			return "^g" + mob.name(viewer)
					+ "^g has large bruises and scratches.^N";
		else if (pct < .99)
