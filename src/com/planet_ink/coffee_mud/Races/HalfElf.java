...
public class HalfElf extends StdRace {
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_SAVE_MAGIC,
				affectableStats.getStat(CharStats.STAT_SAVE_MAGIC) + 5);
		affectableStats.setStat(CharStats.STAT_SAVE_JUSTICE,
				affectableStats.getStat(CharStats.STAT_SAVE_JUSTICE) + 5);
	}
	...
	public String healthText(MOB viewer, MOB mob) {
		...
		if (pct < .10)
			return "^r" + mob.name(viewer)
					+ "^r is mortally wounded and will soon die.^N";
		else if (pct < .20)
		...
		synchronized (resources) {
			if (resources.size() == 0) {
				resources.addElement(makeResource("a " + name().toLowerCase()
						+ " brain", RawMaterial.RESOURCE_MEAT));
