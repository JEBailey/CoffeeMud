...
public class StdRace implements Race {
	...
	protected static final int[] breatheWaterArray = new int[] {
			RawMaterial.RESOURCE_FRESHWATER, RawMaterial.RESOURCE_SALTWATER };
	protected static final int[] breatheAirWaterArray = new int[] {
			RawMaterial.RESOURCE_AIR, RawMaterial.RESOURCE_FRESHWATER,
	...
	public String makeMobName(char gender, int age) {
		...
		case Race.AGE_INFANT:
			...
			case 'M':
			case 'm':
				return "boy infant " + name().toLowerCase();
			case 'F':
			case 'f':
				return "girl infant " + name().toLowerCase();
			default:
		...
		case Race.AGE_TODDLER:
			...
			case 'M':
			case 'm':
				return "baby boy " + name().toLowerCase();
			case 'F':
			case 'f':
				return "baby girl " + name().toLowerCase();
			default:
				return "baby " + name().toLowerCase();
			}
		case Race.AGE_CHILD:
			...
			case 'M':
			case 'm':
				return "little boy " + name().toLowerCase();
			case 'F':
			case 'f':
				return "little girl " + name().toLowerCase();
			default:
	...
	public void agingAffects(MOB mob, CharStats baseStats, CharStats charStats) {
		if ((baseStats.getStat(CharStats.STAT_AGE) > 0)
			...
			case Race.AGE_INFANT:
			case Race.AGE_TODDLER:
				charStats.setStat(CharStats.STAT_SAVE_MIND,
						charStats.getStat(CharStats.STAT_SAVE_MIND) - 10);
				charStats.setStat(CharStats.STAT_MAX_STRENGTH_ADJ,
						charStats.getStat(CharStats.STAT_MAX_STRENGTH_ADJ) - 2);
				charStats
						.setStat(
								CharStats.STAT_MAX_INTELLIGENCE_ADJ,
								charStats
										.getStat(CharStats.STAT_MAX_INTELLIGENCE_ADJ) - 4);
				charStats.setStat(CharStats.STAT_MAX_WISDOM_ADJ,
						charStats.getStat(CharStats.STAT_MAX_WISDOM_ADJ) - 4);
				break;
			case Race.AGE_CHILD:
				charStats.setStat(CharStats.STAT_SAVE_MIND,
						charStats.getStat(CharStats.STAT_SAVE_MIND) - 5);
				charStats.setStat(CharStats.STAT_MAX_STRENGTH_ADJ,
						charStats.getStat(CharStats.STAT_MAX_STRENGTH_ADJ) - 1);
				charStats
						.setStat(
								CharStats.STAT_MAX_INTELLIGENCE_ADJ,
								charStats
										.getStat(CharStats.STAT_MAX_INTELLIGENCE_ADJ) - 2);
				charStats.setStat(CharStats.STAT_MAX_WISDOM_ADJ,
						charStats.getStat(CharStats.STAT_MAX_WISDOM_ADJ) - 2);
				break;
			case Race.AGE_YOUNGADULT:
			case Race.AGE_MATURE:
				break;
			case Race.AGE_MIDDLEAGED:
				...
				charStats
						.setStat(
								CharStats.STAT_MAX_CONSTITUTION_ADJ,
								charStats
										.getStat(CharStats.STAT_MAX_CONSTITUTION_ADJ) - 1);
				charStats
						.setStat(CharStats.STAT_MAX_DEXTERITY_ADJ, charStats
								.getStat(CharStats.STAT_MAX_DEXTERITY_ADJ) - 1);
				charStats
						.setStat(
								CharStats.STAT_MAX_INTELLIGENCE_ADJ,
								charStats
										.getStat(CharStats.STAT_MAX_INTELLIGENCE_ADJ) + 1);
				charStats.setStat(CharStats.STAT_MAX_WISDOM_ADJ,
						charStats.getStat(CharStats.STAT_MAX_WISDOM_ADJ) + 1);
				charStats.setStat(CharStats.STAT_MAX_CHARISMA_ADJ,
						charStats.getStat(CharStats.STAT_MAX_CHARISMA_ADJ) + 1);
				break;
			case Race.AGE_OLD:
				charStats.setStat(CharStats.STAT_SAVE_MIND,
						charStats.getStat(CharStats.STAT_SAVE_MIND) + 10);
				charStats.setStat(CharStats.STAT_SAVE_UNDEAD,
						charStats.getStat(CharStats.STAT_SAVE_UNDEAD) - 5);
				charStats.setStat(CharStats.STAT_MAX_STRENGTH_ADJ,
						charStats.getStat(CharStats.STAT_MAX_STRENGTH_ADJ) - 2);
				charStats
						.setStat(
								CharStats.STAT_MAX_CONSTITUTION_ADJ,
								charStats
										.getStat(CharStats.STAT_MAX_CONSTITUTION_ADJ) - 2);
				charStats
						.setStat(CharStats.STAT_MAX_DEXTERITY_ADJ, charStats
								.getStat(CharStats.STAT_MAX_DEXTERITY_ADJ) - 2);
				charStats
						.setStat(
								CharStats.STAT_MAX_INTELLIGENCE_ADJ,
								charStats
				...
				charStats.setStat(CharStats.STAT_MAX_WISDOM_ADJ,
						charStats.getStat(CharStats.STAT_MAX_WISDOM_ADJ) + 2);
				charStats.setStat(CharStats.STAT_MAX_CHARISMA_ADJ,
						charStats.getStat(CharStats.STAT_MAX_CHARISMA_ADJ) + 2);
				break;
			case Race.AGE_VENERABLE:
				charStats.setStat(CharStats.STAT_SAVE_MIND,
						charStats.getStat(CharStats.STAT_SAVE_MIND) + 15);
				charStats.setStat(CharStats.STAT_SAVE_UNDEAD,
						charStats.getStat(CharStats.STAT_SAVE_UNDEAD) - 25);
				charStats.setStat(CharStats.STAT_MAX_STRENGTH_ADJ,
						charStats.getStat(CharStats.STAT_MAX_STRENGTH_ADJ) - 3);
				charStats
						.setStat(
								CharStats.STAT_MAX_CONSTITUTION_ADJ,
								charStats
										.getStat(CharStats.STAT_MAX_CONSTITUTION_ADJ) - 3);
				charStats
						.setStat(CharStats.STAT_MAX_DEXTERITY_ADJ, charStats
								.getStat(CharStats.STAT_MAX_DEXTERITY_ADJ) - 3);
				charStats
						.setStat(
								CharStats.STAT_MAX_INTELLIGENCE_ADJ,
								charStats
										.getStat(CharStats.STAT_MAX_INTELLIGENCE_ADJ) + 3);
				charStats.setStat(CharStats.STAT_MAX_WISDOM_ADJ,
						charStats.getStat(CharStats.STAT_MAX_WISDOM_ADJ) + 3);
				charStats.setStat(CharStats.STAT_MAX_CHARISMA_ADJ,
						charStats.getStat(CharStats.STAT_MAX_CHARISMA_ADJ) + 3);
				break;
			case Race.AGE_ANCIENT: {
				int[] chart = getAgingChart();
				int diff = chart[Race.AGE_ANCIENT] - chart[Race.AGE_VENERABLE];
				int age = baseStats.getStat(CharStats.STAT_AGE)
						- chart[Race.AGE_ANCIENT];
				int num = (diff > 0) ? (int) Math.abs(Math.floor(CMath.div(age,
						diff))) - 1 : 1;
				if (num == 0)
					num = 1;
				if (num > 16)
					num = 16;
				charStats.setStat(CharStats.STAT_SAVE_MIND,
						charStats.getStat(CharStats.STAT_SAVE_MIND) + 20
								+ (5 * num));
				charStats.setStat(CharStats.STAT_SAVE_UNDEAD,
						charStats.getStat(CharStats.STAT_SAVE_UNDEAD) - 50 + 15
								+ (5 * num));
				charStats.setStat(CharStats.STAT_MAX_STRENGTH_ADJ,
						charStats.getStat(CharStats.STAT_MAX_STRENGTH_ADJ)
								- (3 + (1 * num)));
				charStats.setStat(CharStats.STAT_MAX_CONSTITUTION_ADJ,
						charStats.getStat(CharStats.STAT_MAX_CONSTITUTION_ADJ)
								- (3 + (num)));
				charStats.setStat(CharStats.STAT_MAX_DEXTERITY_ADJ,
						charStats.getStat(CharStats.STAT_MAX_DEXTERITY_ADJ)
								- (3 + (num)));
				charStats.setStat(CharStats.STAT_MAX_INTELLIGENCE_ADJ,
						charStats.getStat(CharStats.STAT_MAX_INTELLIGENCE_ADJ)
								+ (3 + (num)));
				charStats.setStat(CharStats.STAT_MAX_WISDOM_ADJ,
						charStats.getStat(CharStats.STAT_MAX_WISDOM_ADJ)
								+ (3 + (num)));
				charStats.setStat(CharStats.STAT_MAX_CHARISMA_ADJ,
						charStats.getStat(CharStats.STAT_MAX_CHARISMA_ADJ)
								+ (3 + (num)));
				break;
			}
			}
	...
	public boolean okMessage(final Environmental myHost, final CMMsg msg) {
		if (uncharmable()
			...
			msg.source()
					.location()
					.show(msg.source(), myHost, CMMsg.MSG_OK_VISUAL,
							"<T-NAME> seem(s) unaffected by the charm magic from <S-NAMESELF>.");
			return false;
		}
		return true;
	}
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.tool() instanceof Social)
				...
				&& (msg.tool().Name().startsWith("MATE ") || msg.tool().Name()
						.startsWith("SEX "))) {
			if (msg.tool().Name().endsWith("SELF")) {
				if ((msg.source() == myChar)
					...
					msg.source()
							.curState()
							.adjFatigue(CharState.FATIGUED_MILLIS,
									msg.source().maxState());
					myChar.curState().adjFatigue(CharState.FATIGUED_MILLIS,
							myChar.maxState());
					Ability A = CMClass.getAbility("Spell_Blindness");
					if (A != null)
						A.invoke(myChar, myChar, true, myChar.phyStats()
								.level());
				}
			} else if ((msg.target() == myChar)
					&& (msg.tool().Name().endsWith("<T-NAME>"))) {
				boolean srcExhausted = ((msg.source().curState().getMovement() < (msg
						.source().maxState().getMovement() / 2)) || (msg
						.source().curState().getFatigue() >= CharState.FATIGUED_MILLIS));
				boolean meExhausted = ((myChar.curState().getMovement() < (myChar
						.maxState().getMovement() / 2)) || (myChar.curState()
						.getFatigue() >= CharState.FATIGUED_MILLIS));
				if ((myChar.charStats().getStat(CharStats.STAT_GENDER) == ('F'))
						&& (msg.source().charStats()
								.getStat(CharStats.STAT_GENDER) == ('M'))
						...
						&& (msg.source()
								.fetchWornItems(
										Wearable.WORN_LEGS
												| Wearable.WORN_WAIST,
										(short) -2048, (short) 0).size() == 0)
						&& (msg.source().charStats().getMyRace()
								.canBreedWith(this))
						&& ((msg.source().charStats()
								.getStat(CharStats.STAT_AGE) == 0) || ((msg
								.source().charStats().ageCategory() > Race.AGE_CHILD) && (msg
								.source().charStats().ageCategory() < Race.AGE_OLD)))
						&& ((myChar.charStats().getStat(CharStats.STAT_AGE) == 0) || ((myChar
								.charStats().ageCategory() > Race.AGE_CHILD) && (myChar
								.charStats().ageCategory() < Race.AGE_OLD)))) {
					if (srcExhausted)
						...
						msg.source()
								.curState()
								.adjMovement(
										-msg.source().maxState().getMovement() / 2,
										msg.source().maxState());
					}
					if (meExhausted)
						...
						myChar.curState().adjFatigue(CharState.FATIGUED_MILLIS,
								myChar.maxState());
						myChar.curState().adjMovement(
								-myChar.maxState().getMovement() / 2,
								myChar.maxState());
					}
					if (!srcExhausted && !meExhausted
	...
	public void startRacing(MOB mob, boolean verifyOnly) {
		...
		if (!verifyOnly) {
			...
			setHeightWeight(mob.basePhyStats(), (char) mob.baseCharStats()
	...
	public void setHeightWeight(PhyStats stats, char gender) {
		...
		if (heightVariance() > 0) {
			if (weightModifier > 0) {
				...
				heightModifier = (int) Math.round(CMath.mul(heightVariance(),
						variance));
			} else
	...
	protected RawMaterial makeResource(String name, int type) {
		PhysicalAgent A = CMLib.materials()
				.makeResource(type, ID(), true, name);
	...
	public DeadBody getCorpseContainer(MOB mob, Room room) {
		...
		if ((mob.amFollowing() != null)
				&& (mob.isMonster())
				&& ((!mob.amFollowing().isMonster()) || (!mob
						.amUltimatelyFollowing().isMonster())))
		...
		bodyI.setDisplayText("the body of " + mob.Name().replace('\'', '`')
				+ " lies here.");
		...
		for (final Enumeration<Ability> a = mob.effects(); a.hasMoreElements();) {
			...
			if ((A != null) && (A instanceof DiseaseAffect)) {
				if ((CMath.bset(((DiseaseAffect) A).spreadBitmap(),
						DiseaseAffect.SPREAD_CONSUMPTION))
						|| (CMath.bset(((DiseaseAffect) A).spreadBitmap(),
		...
		CMLib.beanCounter().getTotalAbsoluteNativeValue(mob); // converts
																// mob.get-Money();
		if (mob.getMoneyVariation() > 0.0)
			CMLib.beanCounter().addMoney(mob,
					Math.random() * mob.getMoneyVariation());
		else if (mob.getMoneyVariation() < 0.0)
			CMLib.beanCounter().subtractMoney(mob,
					-(Math.random() * mob.getMoneyVariation()));
			...
			if ((thisItem != null) && (thisItem.isSavable())) {
				if (mob.isMonster()) {
					...
					newItem.setExpirationDate(System.currentTimeMillis()
							+ CMProps.getIntVar(CMProps.Int.EXPIRE_MONSTER_EQ)
							* TimeManager.MILI_HOUR);
					...
					new ChameleonList.Signaler<Ability>(myList) {
						public boolean isDeprecated() {
							if ((mob.amDestroyed())
									...
									|| (racialEffectsList(mob) != oldReferenceListRef
											.get()))
								return true;
							return false;
						}
						...
						public void rebuild(final ChameleonList<Ability> me) {
							if (mob.amDestroyed())
								oldReferenceListRef = new WeakReference<List<Ability>>(
										empty);
							else {
								final ChameleonList<Ability> newList = mob
										.charStats().getMyRace()
										.racialEffects(mob);
	...
	public final List<Ability> racialEffectsList(final MOB mob) {
		...
		if ((racialEffectMap == null) && (racialEffectNames() != null)
		...
		for (int v = 0; v < racialEffectLevels().length; v++) {
			if ((racialEffectLevels()[v] <= level.intValue())
				...
				if (A != null) {
					// mob was set to null here to make the cache map actually
					// relevant .. see caching below
					A.setProficiency(CMLib.ableMapper().getMaxProficiency(
							(MOB) null, true, A.ID()));
	...
	public Race makeGenRace() {
		...
		GR.setRacialParms("<RACE><ID>" + ID() + "</ID><NAME>" + name()
				+ "</NAME></RACE>");
		...
		if ((W != null) && (W != NW)) {
			if (!W.isGeneric()) {
				...
				W2.basePhyStats().setAttackAdjustment(
						W.phyStats().attackAdjustment());
				W2.recoverPhyStats();
				W2.text();
				W = W2;
			}
		...
		GR.setStat(
				"DISFLAGS",
				""
						+ ((classless() ? Race.GENFLAG_NOCLASS : 0)
								| (leveless() ? Race.GENFLAG_NOLEVELS : 0)
								| (uncharmable() ? Race.GENFLAG_NOCHARM : 0)
								| (fertile() ? 0 : Race.GENFLAG_NOFERTILE) | (expless() ? Race.GENFLAG_NOEXP
									: 0)));
		...
		if (culturalAbilityNames() != null) {
			...
			for (int i = 0; i < culturalAbilityNames().length; i++) {
				...
				GR.setStat("GETCABLEPROF" + i, ""
						+ culturalAbilityProficiencies()[i]);
			}
		}
	...
	public Race mixRace(Race race, String newRaceID, String newRaceName) {
		...
		Race race1 = this;
		Race race2 = race;
		GR.setRacialParms("<RACE><ID>" + newRaceID + "</ID><NAME>"
				+ newRaceName + "</NAME></RACE>");
		if (!race1.isGeneric())
		...
		GR.setStat("BWEIGHT",
				"" + ((race1.lightestWeight() + race2.lightestWeight()) / 2));
		GR.setStat("VWEIGHT",
				"" + ((race1.weightVariance() + race2.weightVariance()) / 2));
		GR.setStat("MHEIGHT",
				"" + ((race1.shortestMale() + race2.shortestMale()) / 2));
		GR.setStat("FHEIGHT",
				"" + ((race1.shortestFemale() + race2.shortestFemale()) / 2));
		GR.setStat("VHEIGHT",
				"" + ((race1.heightVariance() + race2.heightVariance()) / 2));
		GR.setStat("PLAYER", "" + CMProps.getIntVar(CMProps.Int.MUDTHEME));
		...
		for (int i = 0; i < aging.length; i++) {
			if ((aging[i] == Race.YEARS_AGE_LIVES_FOREVER)
					&& (race2.getAgingChart()[i] == Race.YEARS_AGE_LIVES_FOREVER)) {
			} else if (((aging[i] == Race.YEARS_AGE_LIVES_FOREVER) && (race2
					.getAgingChart()[i] == 0))
		...
		for (int i : CharStats.CODES.ALL()) {
			if (CharStats.CODES.isBASE(i)) {
				SETSTAT.setStat(i,
						(SETSTAT1.getStat(i) + SETSTAT2.getStat(i)) / 2);
				int newStat = ((ADJSTAT1.getStat(i) + ADJSTAT2.getStat(i)) / 2);
				if (newStat > 5)
					ADJSTAT.setStat(i, 5);
				else
					ADJSTAT.setStat(i, newStat);
			} else if ((i != CharStats.STAT_GENDER)
					&& (i != CharStats.STAT_AGE))
				ADJSTAT.setStat(i,
						(ADJSTAT1.getStat(i) + ADJSTAT2.getStat(i)) / 2);
		}
		GR.setStat("ASTATS", CMLib.coffeeMaker().getCharStatsStr(ADJSTAT));
		GR.setStat("CSTATS", CMLib.coffeeMaker().getCharStatsStr(SETSTAT));
		...
		CMLib.coffeeMaker()
				.setCharState(STARTCS1, race1.getStat("STARTASTATE"));

		CharState STARTCS2 = (CharState) CMClass.getCommon("DefaultCharState");
		STARTCS2.setAllValues(0);
		CMLib.coffeeMaker()
				.setCharState(STARTCS1, race2.getStat("STARTASTATE"));
		...
		GR.setStat(
				"DISFLAGS",
				""
						+ (CMath.s_int(race1.getStat("DISFLAGS")) | CMath
								.s_int(race2.getStat("DISFLAGS"))));
		...
		GR.setStat("NUMOFT", "");
		Race outfitRace = (nonHuman.outfit(null) != null) ? nonHuman
				: otherRace;
		...
		List<AbilityMapping> dvata1 = CMLib.ableMapper().getUpToLevelListings(
				race1.ID(), Integer.MAX_VALUE, true, false);
		List<AbilityMapping> dvata2 = CMLib.ableMapper().getUpToLevelListings(
				race2.ID(), Integer.MAX_VALUE, true, false);
		// kill half of them.
		...
		for (int i = 0; i < dvata1.size(); i++) {
			...
			GR.setStat(
					"GETRABLELVL" + i,
					""
							+ CMLib.ableMapper().getQualifyingLevel(race1.ID(),
									false, dvata1.get(i).abilityID));
			GR.setStat(
					"GETRABLEQUAL" + i,
					""
							+ (!CMLib.ableMapper().getDefaultGain(race1.ID(),
									false, dvata1.get(i).abilityID)));
			GR.setStat(
					"GETRABLEPROF" + i,
					""
							+ CMLib.ableMapper().getDefaultProficiency(
									race1.ID(), false, dvata1.get(i).abilityID));
		}
		for (int i = 0; i < dvata2.size(); i++) {
			GR.setStat("GETRABLE" + (i + dvata1.size()),
					dvata2.get(i).abilityID);
			GR.setStat(
					"GETRABLELVL" + (i + dvata1.size()),
					""
							+ CMLib.ableMapper().getQualifyingLevel(race2.ID(),
									false, dvata2.get(i).abilityID));
			GR.setStat(
					"GETRABLEQUAL" + (i + dvata1.size()),
					""
							+ (!CMLib.ableMapper().getDefaultGain(race2.ID(),
									false, dvata2.get(i).abilityID)));
			GR.setStat(
					"GETRABLEPROF" + (i + dvata1.size()),
					""
							+ CMLib.ableMapper().getDefaultProficiency(
									race2.ID(), false, dvata2.get(i).abilityID));
		}
	...
	public PairVector<String, Integer> culturalAbilities() {
		...
		if ((culturalAbilityNames() != null)
				&& (culturalAbilityProficiencies() != null))
			for (int i = 0; i < culturalAbilityNames().length; i++)
				ables.addElement(culturalAbilityNames()[i],
						Integer.valueOf(culturalAbilityProficiencies()[i]));
		return ables;
	}

	public SearchIDList<Ability> racialAbilities(MOB mob) {
		if ((racialAbilityMap == null) && (racialAbilityNames() != null)
				&& (racialAbilityLevels() != null)
				&& (racialAbilityProficiencies() != null)
				&& (racialAbilityQuals() != null)) {
			CMLib.ableMapper().delCharMappings(ID()); // necessary for a
														// "clean start"
			racialAbilityMap = new Hashtable<Integer, SearchIDList<Ability>>();
			for (int i = 0; i < racialAbilityNames().length; i++) {
				CMLib.ableMapper().addDynaAbilityMapping(ID(),
						racialAbilityLevels()[i], racialAbilityNames()[i],
						racialAbilityProficiencies()[i], "",
						!racialAbilityQuals()[i], false);
			}
		}
		...
		List<AbilityMapper.AbilityMapping> V = CMLib.ableMapper()
				.getUpToLevelListings(ID(), level.intValue(), true,
						(mob != null));
		...
		for (AbilityMapper.AbilityMapping able : V) {
			...
			if (A != null) {
				A.setProficiency(CMLib.ableMapper().getDefaultProficiency(ID(),
						false, A.ID()));
				A.setSavable(false);
				A.setMiscText(CMLib.ableMapper().getDefaultParm(ID(), false,
						A.ID()));
				finalV.add(A);
			}
		}
	...
	protected void makeStatChgDesc() {
		if ((baseStatChgDesc == null) || (dispChgDesc == null)
				|| (sensesChgDesc == null)) {
			...
			for (int c : CharStats.CODES.ALL()) {
				...
				if (oldStat > newStat)
					str.append(CharStats.CODES.DESC(c).toLowerCase() + "-"
							+ (oldStat - newStat) + ", ");
			...
			if (baseStatChgDesc.endsWith(", "))
				baseStatChgDesc = baseStatChgDesc.substring(0,
						baseStatChgDesc.length() - 2);
			...
			for (Iterator<Ability> e = ables.iterator(); e.hasNext();) {
				A = e.next();
				str = ((A.classificationCode() & Ability.ALL_ACODES) == Ability.ACODE_LANGUAGE) ? lstr
						: astr;
				if (A.proficiency() <= 0)
					str.append(A.name() + ", ");
				else
			...
			if (abilitiesDesc.endsWith(", "))
				abilitiesDesc = abilitiesDesc.substring(0,
						abilitiesDesc.length() - 2);
			languagesDesc = lstr.toString();
			if (languagesDesc.endsWith(", "))
				languagesDesc = languagesDesc.substring(0,
						languagesDesc.length() - 2);
		}
	}
