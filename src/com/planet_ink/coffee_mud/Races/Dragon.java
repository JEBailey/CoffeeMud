...
public class Dragon extends StdRace {
	...
	public void affectCharStats(MOB affectedMOB, CharStats affectableStats) {
		...
		affectableStats.setStat(CharStats.STAT_STRENGTH,
				affectableStats.getStat(CharStats.STAT_STRENGTH) + bump);
		affectableStats.setStat(CharStats.STAT_DEXTERITY,
				affectableStats.getStat(CharStats.STAT_DEXTERITY) + bump);
		affectableStats.setStat(CharStats.STAT_INTELLIGENCE,
				affectableStats.getStat(CharStats.STAT_INTELLIGENCE) + bump);
	}
			...
			if (resources.size() == 0) {
				...
				for (int i = 0; i < 100; i++)
					resources.addElement(makeResource("a strip of "
							...
							RawMaterial.RESOURCE_DRAGONSCALES));
				for (int i = 0; i < 50; i++)
