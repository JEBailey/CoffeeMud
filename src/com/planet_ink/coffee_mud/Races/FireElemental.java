...
public class FireElemental extends StdRace {
	...
	public String healthText(MOB viewer, MOB mob) {
		...
		if (pct < .10)
			...
			return "^r" + mob.name(viewer)
					+ "^r is flickering alot and is almost smoked out.^N";
		else if (pct < .30)
			return "^r" + mob.name(viewer)
					+ "^r is flickering alot and smoking massively.^N";
		else if (pct < .40)
			return "^y" + mob.name(viewer)
					+ "^y is flickering alot and smoking a lot.^N";
		else if (pct < .50)
			return "^y" + mob.name(viewer) + "^y is flickering and smoking.^N";
		else if (pct < .60)
			return "^p" + mob.name(viewer)
					+ "^p is flickering and smoking somewhat.^N";
		else if (pct < .70)
	...
	public List<RawMaterial> myResources() {
		synchronized (resources) {
			if (resources.size() == 0) {
				resources.addElement(makeResource("a pile of coal",
