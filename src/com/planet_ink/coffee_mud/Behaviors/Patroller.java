...
public class Patroller extends ActiveTicker {
		...
		if (V.size() > 1)
			for (int i = V.size() - 1; i >= 0; i--) {
				...
				if (s.equalsIgnoreCase("RESTART")
						|| s.equalsIgnoreCase("REPEAT"))
					break;
				int dir = Directions.getGoodDirectionCode(s);
				if (dir >= 0)
					V.addElement(Directions.getDirectionName(Directions
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		if ((rideOnly)
				...
				&& (msg.source() != host) && (msg.source().riding() == host)) {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((startRoom.get() == null) && (ticking instanceof Physical))
			startRoom = new WeakReference<Room>(CMLib.map().roomLocation(
					(Physical) ticking));
		if (canAct(ticking, tickID)) {
			...
			if (thisRoom instanceof GridLocale) {
				...
				if (R != null) {
					if (ticking instanceof Item)
						...
						R.show((MOB) ticking, R, null, CMMsg.MASK_ALWAYS
			...
			if (!rideOk) {
				if (((ticking instanceof Rideable) && (((Rideable) ticking)
						.numRiders() > 0))
						|| ((ticking instanceof MOB)
								&& (((MOB) ticking).amFollowing() != null) && (((MOB) ticking)
								.location() == ((MOB) ticking).amFollowing()
								.location()))) {
					tickStatus = Tickable.STATUS_NOT;
					return true;
				}
			...
			if ((nxt.equalsIgnoreCase("RESTART") || nxt
					.equalsIgnoreCase("REPEAT")) && (step > 0)) {
				step = 0;
			...
			if (direction < 0) {
				if (CMLib.map().getExtendedRoomID(thisRoom).toUpperCase()
						.endsWith(nxt.toUpperCase())) {
					correction = null;
					step++;
					tickStatus = Tickable.STATUS_NOT;
					return true;
				...
				for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
					...
					if ((R != null)
							&& (CMLib.map().getExtendedRoomID(R).toUpperCase()
									.endsWith(nxt.toUpperCase()))) {
						correction = null;
						thatRoom = R;
						direction = d;
				...
				if (thatRoom == null) {
					...
					if ((startRoom != null) && (startRoom != thisRoom)) {
						...
						if (ticking instanceof MOB)
							...
							myStartRoom.moveItemTo((Item) E, Expire.Never,
									Move.Followers);
					}
					return true;
				}
			...
			if ((direction < 0) || (destinationRoomForThisStep == null)) {
				...
				if (R == null)
					R = CMLib.map().getRoom(thisRoom.getArea() + "#" + nxt);
				if (R != null) {
					boolean airOk = (((ticking instanceof Physical) && CMLib
							.flags().isFlying((Physical) ticking))
							|| ((ticking instanceof Rider)
									&& (((Rider) ticking).riding() != null) && (((Rider) ticking)
									.riding().rideBasis() == Rideable.RIDEABLE_AIR)) || ((ticking instanceof Rideable) && (((Rideable) ticking)
							.rideBasis() == Rideable.RIDEABLE_AIR)));
					boolean waterOk = ((ticking instanceof Physical) && CMLib
							.flags().isWaterWorthy((Physical) ticking));
					...
					if (R instanceof GridLocale) {
						...
						if (correction != null) {
							for (int r = 0; r < correction.size(); r++)
								if (((GridLocale) R).isMyGridChild(correction
										.get(r))) {
					...
					direction = -1;
					if (correction != null) {
						direction = CMLib.tracking()
								.trackNextDirectionFromHere(correction,
										thisRoom, ticking instanceof Item);
						if (direction < 0)
							correction = null;
						else
							thatRoom = thisRoom.getRoomInDir(direction);
					...
					if ((direction < 0) || (thatRoom == null)) {
						...
						correction = CMLib.tracking().findBastardTheBestWay(
								thisRoom, new XVector(R), flags, diameter);
			...
			if (mobsHere.size() > 0) {
				for (MOB inhab : mobsHere) {
					if ((!inhab.isMonster())
							&& (CMSecurity.isAllowed(inhab, thisRoom,
									CMSecurity.SecFlag.CMDMOBS) || CMSecurity
									.isAllowed(inhab, thisRoom,
			...
			if (ticking instanceof Item) {
				Item I = (Item) ticking;
				if ((ticking instanceof Rideable) && (thatRoom != null)
						&& (riders != null)) {
					...
					for (int i = 0; i < riders.size(); i++) {
						...
						if (R instanceof MOB) {
							...
							// overboard check
							if (mob.isMonster() && mob.isSavable()
									&& (mob.location() != thisRoom)
									&& CMLib.flags().isInTheGame(mob, true))
								thisRoom.bringMobHere(mob, false);

							CMMsg enterMsg = CMClass.getMsg(mob, thatRoom, E,
									CMMsg.MSG_ENTER, null, CMMsg.MSG_ENTER,
									null, CMMsg.MSG_ENTER, null);
							CMMsg leaveMsg = CMClass.getMsg(mob, thisRoom,
									opExit, CMMsg.MSG_LEAVE, null,
									CMMsg.MSG_LEAVE, null, CMMsg.MSG_LEAVE,
									null);
							try {
								rideCheckCt++;
								if (!E.okMessage(mob, enterMsg)) {
									...
									return true;
										...
										&& (!opExit.okMessage(mob, leaveMsg))) {
									tickStatus = Tickable.STATUS_NOT;
									return true;
								} else if (!enterMsg.target().okMessage(mob,
										enterMsg)) {
									tickStatus = Tickable.STATUS_NOT;
									return true;
				...
				thisRoom.showHappens(CMMsg.MSG_OK_ACTION, I, "<S-NAME> goes "
				...
				if ((I.owner() == thatRoom) && (thatRoom != null)) {
					...
					thatRoom.showHappens(
							CMMsg.MSG_OK_ACTION,
							I,
							"<S-NAME> arrives from "
									+ Directions.getFromDirectionName(Directions
							...
							if (CMLib.map().roomLocation(R) != thatRoom)
								if ((((Rideable) ticking).rideBasis() != Rideable.RIDEABLE_SIT)
									...
									if ((R instanceof MOB)
										...
										CMLib.commands()
												.postLook((MOB) R, true);
										thatRoom.show((MOB) R, thatRoom, E,
												CMMsg.MASK_ALWAYS
														| CMMsg.MSG_ENTER, null);
				...
				if (E.hasADoor() && (!E.isOpen())) {
					...
					CMMsg msg = CMClass.getMsg(mob, E, null,
							...
							CMMsg.MSG_OK_VISUAL, null);
					if (mob.location().okMessage(mob, msg)) {
						msg = CMClass.getMsg(mob, E, null, CMMsg.MSG_OK_VISUAL,
								CMMsg.MSG_OPEN, CMMsg.MSG_OK_VISUAL,
								"<S-NAME> " + E.openWord()
										+ "(s) <T-NAMESELF>.");
						CMLib.utensils().roomAffectFully(msg, thisRoom,
								direction);
