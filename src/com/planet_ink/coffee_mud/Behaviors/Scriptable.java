...
public class Scriptable extends StdBehavior implements ScriptingEngine {
	...
	protected ScriptingEngine engine() {
		if (engine == null)
			engine = (ScriptingEngine) CMClass
	...
	public boolean eval(PhysicalAgent scripted, MOB source,
			...
			int startEval) {
		return engine().eval(scripted, source, target, monster, primaryItem,
				secondaryItem, msg, tmp, eval, startEval);
	}
	...
	public String execute(PhysicalAgent scripted, MOB source,
		...
		return engine().execute(scripted, source, target, monster, primaryItem,
				secondaryItem, script, msg, tmp);
	}
	...
	public String varify(MOB source, Environmental target,
		...
		return engine().varify(source, target, scripted, monster, primaryItem,
				secondaryItem, msg, tmp, varifyable);
	}
	...
	public String functify(PhysicalAgent scripted, MOB source,
		...
		return engine().functify(scripted, source, target, monster,
				primaryItem, secondaryItem, msg, tmp, evaluable);
	}
}
