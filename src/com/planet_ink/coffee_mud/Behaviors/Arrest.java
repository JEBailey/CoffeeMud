...
public class Arrest extends StdBehavior implements LegalBehavior {
	...
	public void DebugLogLostConvicts(String lead, LegalWarrant W, MOB officer) {
		...
		errLogMsg
				.append(!W.criminal().location().isInhabitant(officer) ? "AE1 "
						: "");
		errLogMsg.append(W.criminal().amDead() ? "AE2 " : "");
		errLogMsg
				.append(!CMLib.flags().aliveAwakeMobile(W.criminal(), true) ? "AE3 "
						: "");
		errLogMsg
				.append(!CMLib.flags().isInTheGame(W.criminal(), true) ? "AE4 "
						: "");
		errLogMsg.append(W.crime().equalsIgnoreCase("pardoned") ? "AE5 " : "");
		errLogMsg
				.append(!((W.travelAttemptTime() == 0) || ((System
						.currentTimeMillis() - W.travelAttemptTime()) < (5 * 60 * 1000))) ? "AE6 "
						: "");
		errLogMsg
				.append(!CMLib.flags().aliveAwakeMobile(officer, true) ? "AE7 "
						: "");
	...
	public boolean arrest(Area myArea, MOB officer, MOB accused) {
		...
		if ((W != null)
				&& ((W.arrestingOfficer() == null) || (W.arrestingOfficer()
						.location() != accused.location()))) {
			W.setArrestingOfficer(myArea, officer);
			CMLib.commands().postSay(
					W.arrestingOfficer(),
					W.criminal(),
					"You are under arrest " + restOfCharges(laws, W.criminal())
							+ "! Sit down on the ground immediately!", false,
					false);
	...
	public boolean isElligibleOfficer(Area myArea, MOB mob) {
		if (!theLawIsEnabled())
			return false;
		Law laws = getLaws(myArea, false);
		if ((mob.isMonster()) && (mob.location() != null) && (laws != null)
	...
	public boolean updateLaw(Area myArea) {
		...
		if (laws != null) {
			laws.resetLaw();
			if (getLawParms().equalsIgnoreCase("custom") && (myArea != null)) {
				CMLib.database().DBReCreateData(myArea.Name(), "ARREST",
						myArea.Name() + "/ARREST", laws.rawLawString());
				return true;
			}
		}
		return false;
	...
	public List<MOB> getCriminals(Area myArea, String searchStr) {
		...
		for (LegalWarrant W : laws.warrants())
			if ((isStillACrime(W, debugging))
					&& ((searchStr == null) || (CMLib.english().containsString(
							W.criminal().name(), searchStr)))
	...
	public List<LegalWarrant> getWarrantsOf(Area myArea, MOB accused) {
		...
		for (LegalWarrant W : laws.warrants())
			if ((isStillACrime(W, debugging))
					&& ((accused == null) || (W.criminal() == accused)))
				V.addElement(W);
		return V;
	}
	...
	public boolean addWarrant(Law laws, LegalWarrant W) {
		...
		if ((laws != null) && (!laws.warrants().contains(W))) {
			...
			if (R != null) {
				...
				if (accuser == null)
					accuser = W.criminal();
				CMMsg msg = CMClass.getMsg(accuser, W.criminal(), W.victim(),
						CMMsg.MASK_ALWAYS | CMMsg.MSG_LEGALWARRANT,
						CMMsg.MSG_LEGALWARRANT, CMMsg.MSG_LEGALWARRANT,
						W.crime());
			...
			if (W.criminal() != null) {
				List<String> channels = CMLib.channels()
						.getFlaggedChannelNames(
								ChannelsLibrary.ChannelFlag.WARRANTS);
				for (int i = 0; i < channels.size(); i++)
					CMLib.commands().postChannel(
							channels.get(i),
							null,
							W.criminal().name() + " has been accused of "
									+ fixCharge(W) + ".", true);
			}
			return true;
		}
	...
	public boolean addWarrant(Area myArea, MOB accused, MOB victim,
			String crimeLocs, String crimeFlags, String crime, String sentence,
			String warnMsg) {
		if (!theLawIsEnabled())
			return false;
		Law laws = getLaws(myArea, false);
		if (laws != null)
			return fillOutWarrant(accused, laws, myArea, victim, crimeLocs,
					crimeFlags, crime, sentence, warnMsg);
		return false;
	}
	...
	public boolean aquit(Area myArea, MOB accused, String[] acquittableLaws) {
		...
		Law laws = getLaws(myArea, false);
		if (laws != null) {
			boolean debugging = CMSecurity
					.isDebugging(CMSecurity.DbgFlag.ARREST);
			String[] info = null;
			if (acquittableLaws != null) {
				for (int v = 0; v < acquittableLaws.length; v++) {
					String brokenLaw = acquittableLaws[v];
					if ((laws.basicCrimes().containsKey(brokenLaw))
							&& (laws.basicCrimes().get(brokenLaw) != null)) {
						info = laws.basicCrimes().get(brokenLaw);
						break;
					} else if ((laws.taxLaws().containsKey(brokenLaw))
							&& (laws.taxLaws().get(brokenLaw) instanceof String[])) {
						info = (String[]) laws.taxLaws().get(brokenLaw);
						break;
					} else if ((laws.abilityCrimes().containsKey(brokenLaw))
							&& (laws.abilityCrimes().get(brokenLaw) != null)) {
			...
			for (LegalWarrant W : laws.warrants())
				if ((isStillACrime(W, debugging))
						&& (W.criminal() == accused)
						&& ((info == null) || (W.crime()
								.equalsIgnoreCase(info[Law.BIT_CRIMENAME])))) {
					laws.warrants().remove(W);
					return true;
				}
		}
	...
	public boolean accuse(Area myArea, MOB accused, MOB victim,
			String[] accusableLaws) {
		if (!theLawIsEnabled())
			return false;
		Law laws = getLaws(myArea, false);
		if (laws != null) {
	...
	public final String getResourceKey(String lawName) {
		return "LEGAL-" + lawName
				+ Long.toString(getExtraLawString().hashCode());
	}
	...
	protected Law getLaws(Environmental what, boolean cleanOnly) {
		...
		if (laws == null) {
			...
			try {
				if ((lawName.equalsIgnoreCase("custom")) && (what != null)) {
					List<PlayerData> data = CMLib.database().DBReadData(
							what.Name(), "ARREST", what.Name() + "/ARREST");
					if ((data != null) && (data.size() > 0)) {
						DatabaseEngine.PlayerData pdata = data.get(0);
						String s = CMStrings.replaceAll(pdata.xml, "~", "\n");
						s = CMStrings.replaceAll(s, "`", "'");
						lawprops.load(new ByteArrayInputStream(CMStrings
								.strToBytes(s)));
					} else {
						...
						s = CMStrings.replaceAll(s, "'", "`");
						CMLib.database().DBCreateData(what.Name(), "ARREST",
								what.Name() + "/ARREST", s);
					}
				}
				if (lawprops.isEmpty())
					lawprops.load(new ByteArrayInputStream(new CMFile(Resources
							.makeFileResourceName(lawName), null).raw()));
			} catch (IOException e) {
				if (!loadAttempt) {
					Log.errOut("Arrest", "Unable to load: " + lawName
							+ ", legal system inoperable.");
					loadAttempt = true;
				}
	...
	public void dismissOfficer(MOB officer) {
		if (officer == null)
			return;
		if ((officer.getStartRoom() != null) && (officer.location() != null)
		...
		if (R != null)
			for (int i = 0; i < R.numInhabitants(); i++) {
				...
				if ((M != null)
						...
						&& (CMLib.dice().rollPercentage() <= (CMLib.flags()
								.isEvil(accused) ? 25 : (CMLib.flags().isGood(
								accused) ? 95 : 50))))
					return M;
			}
		return null;
	...
	public boolean isAnyKindOfOfficer(Law laws, MOB M) {
		if ((M.isMonster()) && (M.location() != null)
			...
			for (int i = 0; i < laws.officerNames().size(); i++)
				if ((CMLib.english().containsString(M.displayText(),
						laws.officerNames().get(i)) || (CMLib.english()
						.containsString(M.Name(), laws.officerNames().get(i)))))
					return true;
		}
		return false;
	}
	...
	public boolean isElligibleOfficer(Law laws, MOB M, Area myArea) {
		if ((M != null) && (M.isMonster()) && (M.location() != null)) {
			...
			if (isAnyKindOfOfficer(laws, M) && (!isBusyWithJustice(laws, M))
	...
	public MOB getElligibleOfficerHere(Law laws, Area myArea, Room R,
			MOB criminal, MOB victim) {
		if (R == null)
			return null;
		for (int i = 0; i < R.numInhabitants(); i++) {
			MOB M = R.fetchInhabitant(i);
			if ((M != null) && (M != criminal) && (M.location() != null)
					&& (myArea.inMyMetroArea(M.location().getArea()))
	...
	public MOB getAnyElligibleOfficer(Law laws, Area myArea, MOB criminal,
			MOB victim) {
	...
	public MOB getElligibleOfficer(Law laws, Area myArea, MOB criminal,
			MOB victim) {
		Room R = criminal.location();
		if (R == null)
			return null;
	...
	public boolean canFocusOn(MOB officer, MOB criminal) {
		CMMsg msg = CMClass.getMsg(officer, criminal, CMMsg.MSG_LOOK,
				"<S-NAME> look(s) closely at <T-NAME>.");
		if ((officer != null) && (officer.location() != null)
				&& (criminal.location() == officer.location())) {
			if (!officer.location().okMessage(officer, msg))
				return false;
			if (msg.sourceMessage().indexOf("<T-NAME>") < 0)
				return false;
			if ((criminal.name().toUpperCase().equals(criminal.Name()
					.toUpperCase()))
					|| (criminal.name().toUpperCase().startsWith(criminal
							.Name().toUpperCase() + " "))
					|| (criminal.name().toUpperCase().endsWith(" "
	...
	public boolean isStillACrime(LegalWarrant W, boolean debugging) {
		...
		if ((W.witness() != null) && W.witness().amDead()) {
			if (debugging)
				Log.debugOut("ARREST", "Witness is DEAD!");
			return false;
		}
		...
		if ((W.witness() != null) && H.contains(W.witness())) {
			if (debugging)
				Log.debugOut("ARREST", "Witness is a friend of the accused!");
			return false;
		}
		if ((W.victim() != null) && (H.contains(W.victim()))) {
			if (debugging)
				Log.debugOut("ARREST", "Victim is a friend of the accused!");
			return false;
		}
		// crimes expire after three real days
		if ((W.lastOffense() > 0)
				&& ((System.currentTimeMillis() - W.lastOffense()) > EXPIRATION_MILLIS)) {
			if (debugging)
				Log.debugOut("ARREST", "Crime has expired: " + W.lastOffense());
			return false;
		}
		return true;
	}

	public Vector getRelevantWarrants(List<LegalWarrant> warrants,
			LegalWarrant W, MOB criminal) {
		Vector V = new Vector();
		if (W != null)
			V.addElement(W);
		for (LegalWarrant W2 : warrants)
			if ((W2.criminal() == criminal)
					...
					&& ((W == null)
							|| (W2.crime() == null)
							|| (!CMath.bset(W.punishment(),
									Law.PUNISHMENTMASK_SEPARATE)) || (W2
								.crime().equalsIgnoreCase(W.crime()))))
				V.addElement(W2);
		return V;
	}
		...
		for (int w2 = 0; w2 < V.size(); w2++) {
			...
			if (!CMath.bset(W2.punishment(), Law.PUNISHMENTMASK_SEPARATE)) {
				if (((W2.punishment() & Law.PUNISHMENT_MASK) + W2.offenses()) > (highest & Law.PUNISHMENT_MASK))
					highest = (W2.punishment() & Law.PUNISHMENT_MASK)
							+ ((W2.offenses() < 4) ? W2.offenses() : 3);
			}
		}
		for (int w2 = 0; w2 < V.size(); w2++) {
			LegalWarrant W2 = (LegalWarrant) V.elementAt(w2);
	...
	public boolean trackTheJudge(MOB officer, Area myArea, Law laws) {
		...
		if (A != null) {
			...
			if (R != null) {
				A.invoke(
						officer,
						CMParms.parse("\"" + CMLib.map().getExtendedRoomID(R)
								+ "\""), R, true, 0);
				return true;
			}
		}
	...
	public Room getReleaseRoom(Law laws, Area myArea, MOB criminal,
			LegalWarrant W) {
		Room room = null;
		if ((criminal.isMonster()) && (criminal.getStartRoom() != null))
			room = criminal.getStartRoom();
		else {
			if ((laws.releaseRooms().size() == 0)
					|| (laws.releaseRooms().get(0).equals("@")))
				return myArea.getMetroMap().nextElement();
			if (criminal.location() != null)
				room = getRoom(criminal.location().getArea(),
						laws.releaseRooms());
			if (room == null)
	...
	public boolean isTroubleMaker(MOB M) {
		...
		for (Enumeration<Behavior> e = M.behaviors(); e.hasMoreElements();) {
			...
			if ((B != null)
					&& (CMath.bset(B.flags(), Behavior.FLAG_TROUBLEMAKING)))
				return true;
		}
		return false;
	}
	...
	public List<Room> getRooms(Area A, List<String> V) {
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if ((jail != null) && (!finalV.contains(jail)))
	...
	public void fileAllWarrants(Law laws, LegalWarrant W1, MOB mob) {
		...
		Vector<LegalWarrant> V = new Vector<LegalWarrant>();
		{
			...
			if ((W1 != null)
					&& (CMath
							.bset(W1.punishment(), Law.PUNISHMENTMASK_SEPARATE))) {
				for (int i = 0; (W = laws.getWarrant(mob, i)) != null; i++)
					if ((W.criminal() == mob)
							&& (W1.crime().equalsIgnoreCase(W.crime())))
						V.addElement(W);
			} else
				for (int i = 0; (W = laws.getWarrant(mob, i)) != null; i++)
					if (W.criminal() == mob)
		...
		for (LegalWarrant W : V) {
			...
			if (W.crime() != null) {
				...
				for (LegalWarrant oW : laws.oldWarrants())
					if ((oW.criminal() == mob) && (oW.crime() != null)
	...
	public Room findTheJail(MOB mob, Area myArea, Law laws) {
		Room jail = null;
		if ((laws.jailRooms().size() == 0)
				|| (laws.jailRooms().get(0).equals("@")))
			return null;
	...
	public Room findTheDetentionCenter(MOB mob, Area myArea, Law laws,
			LegalWarrant W) {
	...
	public boolean judgeMe(Law laws, MOB judge, MOB officer, MOB criminal,
			LegalWarrant W, Area A, boolean debugging) {
		Vector relevantCrimes = getRelevantWarrants(laws.warrants(), W,
				criminal);
		...
		if (debugging)
			Log.debugOut(
					"Arrest",
					criminal.Name() + " judged for " + W.crime()
							+ " has base action " + W.punishment()
							+ ", and final judgement "
							+ highestCrimeAction(laws, W, W.criminal()));
		...
		case Law.PUNISHMENT_WARN: {
			...
			str.append(criminal.name() + ", you are in trouble for "
					+ restOfCharges(laws, criminal) + ".  ");
			for (int w2 = 0; w2 < relevantCrimes.size(); w2++) {
				LegalWarrant W2 = (LegalWarrant) relevantCrimes.elementAt(w2);
				if (W2.criminal() == criminal) {
					if (W2.witness() != null)
						str.append("The charge of " + fixCharge(W2)
								+ " was witnessed by " + W2.witness().name()
								+ ".  ");
					if ((W2.warnMsg() != null) && (W2.warnMsg().length() > 0))
						str.append(W2.warnMsg() + "  ");
					if ((W2.offenses() > 0)
							&& (laws.getMessage(Law.MSG_PREVOFF).length() > 0)
							&& (!CMath.bset(W.punishment(),
									Law.PUNISHMENTMASK_SEPARATE)))
			...
			CMLib.commands().postSay(judge, criminal, str.toString(), false,
					false);
		}
			totallyDone = true;
			break;
		...
		case Law.PUNISHMENT_PAROLE1:
			if (judge != null) {
				if ((W.offenses() > 0)
						...
						&& (!CMath.bset(W.punishment(),
								Law.PUNISHMENTMASK_SEPARATE)))
					CMLib.commands().postSay(judge, W.criminal(),
							laws.getMessage(Law.MSG_PREVOFF), false, false);
				if (laws.paroleMessages(0).length() > 0)
					CMLib.commands().postSay(judge, criminal,
							laws.paroleMessages(0), false, false);
		...
		case Law.PUNISHMENT_PAROLE2:
			if (judge != null) {
				...
				if (laws.paroleMessages(1).length() > 0)
					CMLib.commands().postSay(judge, criminal,
							laws.paroleMessages(1), false, false);
		...
		case Law.PUNISHMENT_PAROLE3:
			if (judge != null) {
				...
				if (laws.paroleMessages(2).length() > 0)
					CMLib.commands().postSay(judge, criminal,
							laws.paroleMessages(2), false, false);
		...
		case Law.PUNISHMENT_PAROLE4:
			if (judge != null) {
				...
				if (laws.paroleMessages(3).length() > 0)
					CMLib.commands().postSay(judge, criminal,
							laws.paroleMessages(3), false, false);
		...
		case Law.PUNISHMENT_JAIL1:
			if (judge != null) {
				...
				if (laws.jailMessages(0).length() > 0)
					CMLib.commands().postSay(judge, criminal,
							laws.jailMessages(0), false, false);
		...
		case Law.PUNISHMENT_JAIL2:
			if (judge != null) {
				...
				if (laws.jailMessages(1).length() > 0)
					CMLib.commands().postSay(judge, criminal,
							laws.jailMessages(1), false, false);
		...
		case Law.PUNISHMENT_JAIL3:
			if (judge != null) {
				...
				if (laws.jailMessages(2).length() > 0)
					CMLib.commands().postSay(judge, criminal,
							laws.jailMessages(2), false, false);
		...
		case Law.PUNISHMENT_JAIL4:
			if (judge != null) {
				...
				if (laws.jailMessages(3).length() > 0)
					CMLib.commands().postSay(judge, criminal,
							laws.jailMessages(3), false, false);
		...
		case Law.PUNISHMENT_EXECUTE:
			if (judge != null) {
				...
				if (laws.getMessage(Law.MSG_EXECUTE).length() > 0)
					CMLib.commands().postSay(judge, criminal,
							laws.getMessage(Law.MSG_EXECUTE), false, false);
		...
		if ((totallyDone)
				&& (CMath.bset(W.punishment(), Law.PUNISHMENTMASK_FINE))) {
			...
			if ((fines > 0.0) && (judge != null)) {
				CMLib.commands().postSay(
						...
						"You are hereby fined "
								+ CMLib.beanCounter().nameCurrencyShort(judge,
										fines)
								+ ", payable to the local tax assessor.",
						false, false);
				...
				finesAssessed.put(criminal,
						Double.valueOf(D.doubleValue() + fines));
			}
		}
		if ((totallyDone)
				&& (CMath.bset(W.punishment(), Law.PUNISHMENTMASK_DETAIN))) {
			...
			if (debugging)
				Log.debugOut("Arrest",
						"Putting the above crime into a detain state, officer="
	...
	public boolean fillOutWarrant(MOB mob, Law laws, Area myArea,
			Environmental target, String crimeLocs, String crimeFlags,
			String crime, String sentence, String warnMsg) {
		if (mob.amDead()) {
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
				Log.debugOut("ARREST", mob.name() + ", data: " + crimeLocs
						+ "->" + crimeFlags + "->" + crime + "->" + sentence
						+ " * IS DEAD!");
			return false;
		}
		...
		if ((myArea != null)
				&& (!myArea.inMyMetroArea(mob.location().getArea()))) {
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
				Log.debugOut("ARREST", mob.name() + ", data: " + crimeLocs
						+ "->" + crimeFlags + "->" + crime + "->" + sentence
						+ "* Accused is not in the area.");
			return false;
		}
		...
		if (mob == victim) {
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
				Log.debugOut("ARREST", mob.name() + ", data: " + crimeLocs
						+ "->" + crimeFlags + "->" + crime + "->" + sentence
						+ "* Accused and victim are the same.");
			return false;
		}
			...
			for (int v = 0; v < V.size(); v++) {
				...
				if (str.endsWith("WITNESS") && (str.length() < 9)) {
					if (str.startsWith("!"))
						...
						if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
							Log.debugOut("ARREST", mob.name() + ", data: "
									+ crimeLocs + "->" + crimeFlags + "->"
									+ crime + "->" + sentence
									+ "* Witness required, but not present.");
						return false;
					}
					...
					if ((W != null) && ((thisTime - W.lastOffense()) < 600000)) {
						...
						if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
							Log.debugOut("ARREST", mob.name() + ", data: "
									+ crimeLocs + "->" + crimeFlags + "->"
									+ crime + "->" + sentence
									+ "* Recently required, but it isn't!");
						return false;
					}
				}
			}
		...
		if ((requiresWitness) && (witness == null)) {
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
				Log.debugOut("ARREST", mob.name() + ", data: " + crimeLocs
						+ "->" + crimeFlags + "->" + crime + "->" + sentence
						+ "* Witness required, and none present: .");
			return false;
		}
					...
					if ((mob.location().domainType() & Room.INDOORS) > 0) {
						...
						if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
							Log.debugOut("ARREST", mob.name() + ", data: "
									+ crimeLocs + "->" + crimeFlags + "->"
									+ crime + "->" + sentence
									+ "* Should be indoors, but isn't!");
						return false;
					}
					aCrime = true;
				} else if (str.endsWith("HOME") && (str.length() < 6)) {
					if (CMLib.law()
							.doesHavePriviledgesHere(mob, mob.location()))
						if (str.startsWith("!")) {
							if (CMSecurity
									.isDebugging(CMSecurity.DbgFlag.ARREST))
								Log.debugOut("ARREST", mob.name() + ", data: "
					...
					if (!str.startsWith("!")) {
						if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
							Log.debugOut("ARREST", mob.name() + ", data: "
									+ crimeLocs + "->" + crimeFlags + "->"
									+ crime + "->" + sentence
									+ "* Should be home, but is not!");
						return false;
					}
					aCrime = true;
				} else if (str.startsWith("!")
						&& (CMLib.english().containsString(display,
								str.substring(1)))) {
					if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
						Log.debugOut("ARREST", mob.name() + ", data: "
								+ crimeLocs + "->" + crimeFlags + "->" + crime
								+ "->" + sentence + "* Should not be at '"
								+ str.substring(1) + "', but is!");
					return false;
		...
		if ((victim != null) && (!(victim instanceof Deity))) {
			if (!CMLib.masking().maskCheck(
					laws.getMessage(Law.MSG_PROTECTEDMASK), victim, false)) {
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
					Log.debugOut("ARREST", mob.name() + ", data: " + crimeLocs
							+ "->" + crimeFlags + "->" + crime + "->"
							+ sentence + "* Victim is not a protected race!");
				return false;
			}
		}
		...
		LegalWarrant W = null;
		for (int i = 0; (W = laws.getWarrant(mob, i)) != null; i++) {
			if ((W.criminal() == mob) && (W.victim() == victim)
					&& (W.crime().equals(crime))) {
				if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
					Log.debugOut("ARREST", mob.name() + ", data: " + crimeLocs
							+ "->" + crimeFlags + "->" + crime + "->"
							+ sentence + "* Warrant already exists.");
				return false;
			}
		}
				...
				for (int i = 0; i < Law.PUNISHMENTMASK_DESCS.length; i++)
					if (s.equalsIgnoreCase(Law.PUNISHMENTMASK_DESCS[i])) {
						...
						W.setPunishment(W.punishment()
								| Law.PUNISHMENTMASK_CODES[i]);
						if (parm != null)
							W.addPunishmentParm(Law.PUNISHMENTMASK_CODES[i],
									parm);
					}
			if (!actionCodeSet) {
				Log.errOut("Arrest", "Unknown sentence: " + s + " for crime "
						+ crime);
				return false;
			}
		}
		...
		if ((isStillACrime(W, CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST)))
				&& ((W.witness() == null) || CMLib.flags().canBeSeenBy(
						W.criminal(), W.witness()))) {
			if (CMSecurity.isDebugging(CMSecurity.DbgFlag.ARREST))
				Log.debugOut("ARREST", mob.name() + ", data: " + crimeLocs
						+ "->" + crimeFlags + "->" + crime + "->" + sentence
						+ "* Warrant filled out.");
			...
			Log.debugOut("ARREST", mob.name() + ", data: " + crimeLocs + "->"
					+ crimeFlags + "->" + crime + "->" + sentence
					+ "* Warrant fails the is a crime check.");
		return true;
	}

	protected boolean isAnUltimateAuthorityHere(MOB M, Law laws) {
		if (CMSecurity.isAllowed(M, M.location(), CMSecurity.SecFlag.ABOVELAW)
				|| (isTheJudge(laws, M)))
			return true;
		return false;
	}
	...
	protected boolean theLawIsEnabled() {
		return ((CMProps.getBoolVar(CMProps.Bool.MUDSTARTED)) && (!CMSecurity
				.isDisabled(CMSecurity.DisFlag.ARREST)));
	}
	...
	public void testEntryLaw(Law laws, Area myArea, MOB testMOB, Room R) {
		if ((laws.basicCrimes().containsKey("NUDITY"))
			...
			fillOutWarrant(testMOB, laws, myArea, null,
					info[Law.BIT_CRIMELOCS], info[Law.BIT_CRIMEFLAGS],
					info[Law.BIT_CRIMENAME], info[Law.BIT_SENTENCE],
					info[Law.BIT_WARNMSG]);
		}

		Item w = null;
		if ((laws.basicCrimes().containsKey("ARMED"))
				&& ((!testMOB.isMonster()) || (!myArea.inMyMetroArea(CMLib
						.map().getStartArea(testMOB))))
				...
				&& (CMLib.flags().isSeen(w)) && (!CMLib.flags().isHidden(w))
				&& (!CMLib.flags().isInvisible(w))) {
			String info[] = laws.basicCrimes().get("ARMED");
			if ((testMOB.session() == null)
					|| (((System.currentTimeMillis() - testMOB.session()
							.getLastNPCFight()) > 30000) && ((System
							.currentTimeMillis() - testMOB.session()
							.getLastPKFight()) > 30000))) {
				fillOutWarrant(testMOB, laws, myArea, null,
						info[Law.BIT_CRIMELOCS], info[Law.BIT_CRIMEFLAGS],
						info[Law.BIT_CRIMENAME], info[Law.BIT_SENTENCE],
						info[Law.BIT_WARNMSG]);
		...
		if ((laws.basicCrimes().containsKey("TRESPASSING"))
				&& ((CMLib.masking()
						.maskCheck(laws.getMessage(Law.MSG_TRESPASSERMASK),
								testMOB, false)) || (testMOB.isMonster()
						&& (testMOB.getStartRoom() != null)
						&& (testMOB.getStartRoom().getArea() != R.getArea())
						&& (CMLib.flags().isPossiblyAggressive(testMOB))
						&& ((testMOB.amFollowing() == null) || ((!testMOB
								.amFollowing().isMonster()) && (testMOB
								.amFollowing().location() == testMOB.location()))) && (!CMLib
						.masking().maskCheck(
								laws.getMessage(Law.MSG_PROTECTEDMASK),
								testMOB, false))))) {
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_SPEAK)
			...
			int x = msg.sourceMessage().toUpperCase()
					.indexOf("I HEREBY PARDON ");
			if (x > 0) {
				...
				if (name.length() > 0)
					for (LegalWarrant W : laws.warrants()) {
						if ((W.criminal() != null)
								&& (CMLib.english().containsString(W.criminal()
										.Name(), name))) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_DEATH)
				...
				&& ((!msg.source().isMonster()) || (!isTroubleMaker(msg
						.source())))) {
			...
			fillOutWarrant(criminal, laws, myArea, msg.source(),
					bits[Law.BIT_CRIMELOCS], bits[Law.BIT_CRIMEFLAGS],
					bits[Law.BIT_CRIMENAME], bits[Law.BIT_SENTENCE],
		...
		if ((msg.tool() != null)
				...
				&& ((laws.abilityCrimes().containsKey(msg.tool().ID()
						.toUpperCase()))
						|| (laws.abilityCrimes().containsKey(CMLib.flags()
								.getAbilityType((Ability) msg.tool()))) || (laws
							.abilityCrimes().containsKey(CMLib.flags()
						.getAbilityDomain((Ability) msg.tool()))))) {
			String[] info = laws.abilityCrimes().get(
					msg.tool().ID().toUpperCase());
			if (info == null)
				info = laws.abilityCrimes().get(
						CMLib.flags().getAbilityType((Ability) msg.tool()));
			if (info == null)
				info = laws.abilityCrimes().get(
						CMLib.flags().getAbilityDomain((Ability) msg.tool()));
			fillOutWarrant(msg.source(), laws, myArea, msg.target(),
					info[Law.BIT_CRIMELOCS], info[Law.BIT_CRIMEFLAGS],
					info[Law.BIT_CRIMENAME], info[Law.BIT_SENTENCE],
					info[Law.BIT_WARNMSG]);
		}

		for (final Enumeration<Ability> a = msg.source().effects(); a
				.hasMoreElements();) {
			final Ability A = a.nextElement();
			if ((A != null)
					&& (!A.isAutoInvoked())
					&& ((A.canBeUninvoked() || (!msg.source().isMonster())))
					&& ((laws.abilityCrimes().containsKey("$"
							+ A.ID().toUpperCase()))
							|| (laws.abilityCrimes().containsKey("$"
									+ CMLib.flags().getAbilityType(A))) || (laws
								.abilityCrimes().containsKey("$"
							+ CMLib.flags().getAbilityDomain(A))))) {
				String[] info = laws.abilityCrimes().get(
						"$" + A.ID().toUpperCase());
				if (info == null)
					info = laws.abilityCrimes().get(
							"$" + CMLib.flags().getAbilityType(A));
				if (info == null)
					info = laws.abilityCrimes().get(
							"$" + CMLib.flags().getAbilityDomain(A));
				fillOutWarrant(msg.source(), laws, myArea, null,
						info[Law.BIT_CRIMELOCS], info[Law.BIT_CRIMEFLAGS],
						info[Law.BIT_CRIMENAME], info[Law.BIT_SENTENCE],
						info[Law.BIT_WARNMSG]);
			}
		...
		if ((CMath.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS))
			...
			if (isTheJudge(laws, (MOB) msg.target())) {
				if (!msg.source().isMonster())
					for (int i = 0; i < R.numInhabitants(); i++) {
						MOB M = R.fetchInhabitant(i);
						if ((M != null) && (M != msg.target())
								&& (M != msg.source())
								&& (M.getVictim() != msg.source())
								&& (isAnyKindOfOfficer(laws, M))) {
							if (msg.source().amFollowing() == M)
								msg.source().setFollowing(null);
							CMLib.commands().postSay(M, null,
									"Ack! Treason! Die!", false, false);
				...
				boolean targetIsOfficer = isAnyKindOfOfficer(laws,
						(MOB) msg.target());
				...
				if ((assaultInfo != null) && (murderInfo != null))
					for (LegalWarrant W : laws.warrants())
						if (targetIsOfficer
								...
								&& (W.criminal().location().isInhabitant(W
										.arrestingOfficer())))
							justResisting = true;
						else if ((!targetIsOfficer)
								&& (W.criminal() == msg.target())
								&& ((W.victim() == msg.source()) || ((msg
										.source().amFollowing() != null) && (W
										.victim() == msg.source().amFollowing())))
								&& (W.crime().equals(
										assaultInfo[Law.BIT_CRIMENAME]) || W
										.crime().equals(
												murderInfo[Law.BIT_CRIMENAME]))
							...
							turnAbout = true;
								...
								&& (W.criminal() == msg.source())
								&& (W.crime()
										.equals(murderInfo[Law.BIT_CRIMENAME]))
								&& (isStillACrime(W, false)))
							turnAbout = true;
				if (justResisting) {
					if (laws.basicCrimes().containsKey("RESISTINGARREST")) {
						String[] info = laws.basicCrimes().get(
								"RESISTINGARREST");
						fillOutWarrant(msg.source(), laws, myArea, null,
								info[Law.BIT_CRIMELOCS],
								info[Law.BIT_CRIMEFLAGS],
						...
						&& (assaultInfo != null)
						&& ((msg.source().isMonster()) || (!isTroubleMaker((MOB) msg
								.target()))))
					fillOutWarrant(msg.source(), laws, myArea, msg.target(),
		...
		if ((msg.othersCode() != CMMsg.NO_EFFECT)
			...
			for (int i = 0; i < laws.otherCrimes().size(); i++) {
				...
				for (int v = 0; v < V.size(); v++) {
					if (CMLib.english().containsString(msg.othersMessage(),
							V.get(v))) {
	...
	public void haveMobReactToLaw(MOB mob, MOB officer) {
		if ((mob.isMonster()) && (!CMLib.flags().isSitting(mob))
				&& (mob.amFollowing() == null) && (!mob.isInCombat())) {
			...
			if (evil
					...
					|| (CMLib
							.flags()
							.flaggedBehaviors(mob,
									Behavior.FLAG_POTENTIALLYAGGRESSIVE).size() > 0)) {
				if (mob.phyStats().level() > (officer.phyStats().level() / 2))
					mob.setVictim(officer);
				else if (!CMLib.flags().isAnimalIntelligence(mob))
					mob.enqueCommand(CMParms.parse("FLEE"),
							Command.METAFLAG_FORCED | Command.METAFLAG_ORDER, 1);
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		for (LegalWarrant W : laws.warrants()) {
			if ((W.criminal() == null) || (W.criminal().location() == null)) {
				if (debugging)
					Log.debugOut("Arrest", "Tick: " + W.crime()
							+ ": Criminal or Location is null. Skipping.");
				continue;
			}
			...
			if (debugging)
				Log.debugOut("Arrest", "Tick: Handling " + W.crime() + " for "
						+ W.criminal().Name() + ": State " + W.state());
	...
	protected void fileArrestResister(Law laws, Area myArea, LegalWarrant W) {
		if ((W.criminal() != null) && (W.arrestingOfficer() != null)
			...
			if (laws.basicCrimes().containsKey("RESISTINGARREST")) {
				...
				fillOutWarrant(W.criminal(), laws, myArea, null,
						info[Law.BIT_CRIMELOCS], info[Law.BIT_CRIMEFLAGS],
						info[Law.BIT_CRIMENAME], info[Law.BIT_SENTENCE],
						info[Law.BIT_WARNMSG]);
			}
	...
	protected void processWarrant(Area myArea, Law laws, LegalWarrant W,
			boolean debugging) {
		...
		case Law.STATE_SEEKING: {
			if ((officer == null)
				...
				officer = null;
			if (officer == null)
				officer = getElligibleOfficer(laws, myArea, W.criminal(),
						W.victim());
			W.setTravelAttemptTime(0);
			if ((officer != null) && (W.criminal().location() != null)
					&& (W.criminal().location().isInhabitant(officer))
					&& (!W.criminal().amDead())
					&& (CMLib.flags().isInTheGame(W.criminal(), true))
					&& (CMLib.flags().canBeSeenBy(W.criminal(), officer))
					&& (canFocusOn(officer, W.criminal()))) {
				if (CMSecurity.isAllowed(W.criminal(), W.criminal().location(),
						CMSecurity.SecFlag.ABOVELAW)) {
					CMLib.commands().postSay(officer, W.criminal(),
							"Damn, I can't arrest you.", false, false);
					if (CMSecurity.isAllowedEverywhere(W.criminal(),
							CMSecurity.SecFlag.ABOVELAW)) {
					...
					unCuff(W.criminal());
					W.setArrestingOfficer(myArea, null);
				} else if (judgeMe(laws, null, officer, W.criminal(), W,
						myArea, debugging)) {
					fileAllWarrants(laws, W, W.criminal());
					unCuff(W.criminal());
					...
					if (!CMLib.flags().isAnimalIntelligence(W.criminal())) {
						W.setArrestingOfficer(myArea, officer);
						LegalWarrant copKillerW = laws.getCopkiller(myArea,
								this, W.criminal());
						if (copKillerW != null) {
							CMLib.commands().postSay(W.arrestingOfficer(),
									W.criminal(),
									laws.getMessage(Law.MSG_COPKILLER), false,
									false);
						...
						LegalWarrant lawResistW = laws.getLawResister(myArea,
								this, W.criminal());
						if (lawResistW != null) {
							CMLib.commands().postSay(W.arrestingOfficer(),
									W.criminal(),
									laws.getMessage(Law.MSG_RESISTFIGHT),
									false, false);
							W.setState(Law.STATE_SUBDUEING);
							W.arrestingOfficer().setVictim(W.criminal());
							processWarrant(myArea, laws, W, debugging);
						...
						CMLib.commands()
								.postSay(
										...
										"You are under arrest "
												+ restOfCharges(laws,
														W.criminal())
												+ "! Sit down on the ground immediately!",
										false, false);
						W.setState(Law.STATE_ARRESTING);
					} else {
						W.setArrestingOfficer(myArea, officer);
						CMLib.commands().postSay(
								W.arrestingOfficer(),
								W.criminal(),
								"You are headed to the pound for "
										+ restOfCharges(laws, W.criminal())
										+ "!", false, false);
						W.setState(Law.STATE_ARRESTING);
					}
				}
			...
			if ((officer != null) && (W.criminal().location() != null)
				...
				if (officer.isInCombat()) {
					if (officer.getVictim() == W.criminal()) {
						CMLib.commands().postSay(officer, W.criminal(),
								laws.getMessage(Law.MSG_RESISTFIGHT), false,
								false);
						W.setState(Law.STATE_SUBDUEING);
					} else {
						W.setArrestingOfficer(myArea, null);
					...
					if (CMLib.flags().isSitting(W.criminal())
						...
						if (!CMLib.flags().isAnimalIntelligence(W.criminal()))
							CMLib.commands().postSay(officer, W.criminal(),
									laws.getMessage(Law.MSG_NORESIST), false,
									false);
					} else
						CMLib.commands().postSay(officer, W.criminal(),
								laws.getMessage(Law.MSG_RESISTWARN), false,
								false);
			...
			if ((officer != null) && (W.criminal().location() != null)
				...
				if (W.crime().equalsIgnoreCase("pardoned")) {
					...
					if (!W.arrestingOfficer().isInCombat())
						CMLib.commands().postSay(officer, W.criminal(),
								laws.getMessage(Law.MSG_RESIST), false, false);

					Ability A = CMClass.getAbility("Skill_ArrestingSap");
					if (A != null) {
						int curPoints = (int) Math.round(CMath.div(W.criminal()
								.curState().getHitPoints(), W.criminal()
						...
						if (!A.invoke(officer, W.criminal(), (curPoints <= 25),
							...
							if (A != null) {
								A.setProficiency(100);
								A.setAbilityCode(30);
								if (!A.invoke(officer, W.criminal(),
										(curPoints <= 50), 0))
									CMLib.combat().postAttack(officer,
											W.criminal(),
											officer.fetchWieldedItem());
							}
						}
					}
				}
				Ability cuff = W.criminal().fetchEffect("Skill_HandCuff");
				if ((CMLib.flags().isSitting(W.criminal()) || (cuff != null) || (CMLib
						.flags().isSleeping(W.criminal())))
						&& (!W.criminal().amDead())
						&& (CMLib.flags().isInTheGame(W.criminal(), true))) {
					...
					if (trackTheJudge(officer, myArea, laws))
						...
						makePeace(officer.location());
						CMLib.commands().postSay(officer, W.criminal(),
								"Since there is no judge, you may go.", false,
								false);
						W.setTravelAttemptTime(0);
						fileAllWarrants(laws, W, W.criminal());
						unCuff(W.criminal());
					...
					CMLib.commands().postSay(officer, null, "Hmph.", false,
							false);
				...
				if (officer != null) {
					CMLib.commands().postSay(officer, null, "Darn.", false,
							false);
					fileArrestResister(laws, myArea, W);
				}
				W.setTravelAttemptTime(0);
		...
		case Law.STATE_MOVING: {
			if ((officer != null)
					...
					&& (CMLib.flags().isInTheGame(W.criminal(), true))
					&& (!W.crime().equalsIgnoreCase("pardoned"))
					&& ((W.travelAttemptTime() == 0) || ((System
							.currentTimeMillis() - W.travelAttemptTime()) < (5 * 60 * 1000)))
					&& (CMLib.flags().aliveAwakeMobile(officer, true))) {
				...
				if (getTheJudgeHere(laws, officer.location()) != null)
					...
					if (!trackTheJudge(officer, myArea, laws)) {
						CMLib.commands().postSay(officer, null,
								"Now where was that court?.", false, false);
						W.setTravelAttemptTime(0);
						unCuff(W.criminal());
						W.setArrestingOfficer(myArea, null);
						W.setState(Law.STATE_SEEKING);
						...
						&& (laws.chitChat().size() > 0))
					CMLib.commands().postSay(
							officer,
							W.criminal(),
							laws.chitChat().get(
									CMLib.dice().roll(1,
											laws.chitChat().size(), -1)),
							false, false);
			} else {
				if (officer != null) {
					DebugLogLostConvicts("Officer lost criminal: ", W, officer);
					CMLib.commands().postSay(officer, null,
							"Drat! Lost another one!", false, false);
					fileArrestResister(laws, myArea, W);
				}
				W.setTravelAttemptTime(0);
				unCuff(W.criminal());
		...
		case Law.STATE_REPORTING: {
			if ((officer != null)
				...
				if (judge == null) {
					...
					if (!trackTheJudge(officer, myArea, laws)) {
						CMLib.commands().postSay(officer, null,
								"Where was that darn court!", false, false);
						W.setTravelAttemptTime(0);
						unCuff(W.criminal());
						W.setArrestingOfficer(myArea, null);
						W.setState(Law.STATE_SEEKING);
				...
				} else if (CMLib.flags().aliveAwakeMobile(judge, true)) {
					CMLib.tracking().stopTracking(officer);
					W.setTravelAttemptTime(0);
					String sirmaam = "Sir";
					if (Character.toString(
							(char) judge.charStats().getStat(
									CharStats.STAT_GENDER)).equalsIgnoreCase(
							"F"))
						sirmaam = "Ma'am";
					CMLib.commands().postSay(
							officer,
							judge,
							sirmaam + ", " + W.criminal().name()
									+ " has been arrested "
									+ restOfCharges(laws, W.criminal()) + ".",
							false, false);
					Vector warrants = getRelevantWarrants(laws.warrants(), W,
							W.criminal());
					for (int w2 = 0; w2 < warrants.size(); w2++) {
						...
						if (W2.witness() != null)
							CMLib.commands().postSay(
									officer,
									judge,
									"The charge of " + fixCharge(W2)
											+ " was witnessed by "
											+ W2.witness().name() + ".", false,
									false);
					}
					...
					if ((highestCrimeAction(laws, W, W.criminal()) == Law.PUNISHMENT_EXECUTE)
						...
						List<String> channels = CMLib.channels()
								.getFlaggedChannelNames(
										ChannelsLibrary.ChannelFlag.EXECUTIONS);
						for (int i = 0; i < channels.size(); i++)
							CMLib.commands().postChannel(
									judge,
									channels.get(i),
									W.criminal().Name()
											...
											+ judge.location().displayText(
													judge) + " for "
											+ W.criminal().charStats().hisher()
											+ " crimes.", true);
					}
				} else {
					CMLib.commands().postSay(officer, W.criminal(),
							"I guess court is not in session today.", false,
							false);
					W.setTravelAttemptTime(0);
					unCuff(W.criminal());
					W.setArrestingOfficer(myArea, null);
				...
				if (officer != null) {
					DebugLogLostConvicts("Officer can't report criminal: ", W,
							officer);
					CMLib.commands().postSay(officer, null,
							"Wha? Where'd he go?", false, false);
					fileArrestResister(laws, myArea, W);
				}
				W.setTravelAttemptTime(0);
				unCuff(W.criminal());
		...
		case Law.STATE_WAITING: {
			if ((officer != null) && (!W.criminal().amDead())
					&& (W.criminal().location().isInhabitant(officer))
					&& (CMLib.flags().isInTheGame(W.criminal(), true))
					&& (!W.crime().equalsIgnoreCase("pardoned"))
					&& (CMLib.flags().aliveAwakeMobile(officer, true))) {
				...
				if (judge == null) {
					...
					if (judgeMe(laws, judge, officer, W.criminal(), W, myArea,
							debugging)) {
						W.setTravelAttemptTime(0);
						unCuff(W.criminal());
						dismissOfficer(officer);
					...
					CMLib.commands().postSay(officer, W.criminal(),
							"Court is not in session today.", false, false);
					W.setTravelAttemptTime(0);
					unCuff(W.criminal());
					W.setArrestingOfficer(myArea, null);
					W.setState(Law.STATE_SEEKING);
				...
				if (officer != null) {
					DebugLogLostConvicts("Officer can't await criminal: ", W,
							officer);
					CMLib.commands().postSay(officer, null, "Wha? Huh?", false,
							false);
					fileArrestResister(laws, myArea, W);
				}
				W.setTravelAttemptTime(0);
		...
		case Law.STATE_PAROLING: {
			...
			if ((officer != null) && (!W.criminal().amDead())
				...
				if ((judge != null)
					...
					judge.location().show(judge, W.criminal(),
							CMMsg.MSG_OK_VISUAL,
							"<S-NAME> put(s) <T-NAME> on parole!");
					...
					CMLib.commands().postSay(judge, W.criminal(),
							laws.getMessage(Law.MSG_PAROLEDISMISS), false,
							false);
					dismissOfficer(officer);
					W.setArrestingOfficer(myArea, null);
					W.criminal().tell("\n\r\n\r");
					...
					CMLib.commands().postSay(officer, null, "No court today.",
							false, false);
					unCuff(W.criminal());
					if (W.arrestingOfficer() != null)
						dismissOfficer(W.arrestingOfficer());
				...
				if (officer != null) {
					DebugLogLostConvicts("Officer can't parole criminal: ", W,
							officer);
					CMLib.commands().postSay(officer, null, "That was wierd.",
							false, false);
				}
				unCuff(W.criminal());
				W.setArrestingOfficer(myArea, null);
			...
			if ((officer != null) && (!W.criminal().amDead())
				...
				if (jail != null) {
					...
					if (A != null) {
						...
						A.invoke(
								officer,
								CMParms.parse(CMLib.map().getExtendedRoomID(
										jail)), jail, true, 0);
					}
					if (officer.fetchEffect("Skill_Track") == null) {
						...
						unCuff(W.criminal());
						CMLib.commands().postSay(officer, W.criminal(),
								"I can't find the jail, you are free to go.",
								false, false);
						dismissOfficer(officer);
						W.setArrestingOfficer(myArea, null);
					}
					...
					CMLib.commands().postSay(W.arrestingOfficer(),
							W.criminal(),
							"But since there IS no jail, I will let you go.",
							false, false);
					dismissOfficer(officer);
					W.setArrestingOfficer(myArea, null);
				}
				...
				if (officer != null) {
					DebugLogLostConvicts("Officer can't jail criminal: ", W,
							officer);
					CMLib.commands().postSay(officer, null, "Crazy.", false,
							false);
					fileArrestResister(laws, myArea, W);
				}
				unCuff(W.criminal());
		...
		case Law.STATE_DETAINING: {
			if ((officer != null) && (!W.criminal().amDead())
					...
					&& (!W.crime().equalsIgnoreCase("pardoned"))
					&& (CMLib.flags().canBeSeenBy(W.criminal(), officer))) {
				Room jail = findTheDetentionCenter(W.criminal(), myArea, laws,
						W);
				...
				if ((jail != null) && (time >= 0)) {
					...
					W.criminal()
							.basePhyStats()
							.setDisposition(
									W.criminal().basePhyStats().disposition()
											| PhyStats.IS_SITTING);
					W.criminal()
							.phyStats()
							.setDisposition(
									W.criminal().phyStats().disposition()
											| PhyStats.IS_SITTING);
					if ((A != null)
							&& (!CMLib.flags().isBoundOrHeld(W.criminal())))
						A.invoke(officer, W.criminal(), true, 0);
					...
					if (officer.fetchEffect("Skill_Track") == null) {
						...
						CMLib.commands()
								.postSay(
										officer,
										W.criminal(),
										"I can't find the detention center, you are free to go.",
										false, false);
						dismissOfficer(officer);
						W.setArrestingOfficer(myArea, null);
					}
					...
					CMLib.commands()
							.postSay(
									W.arrestingOfficer(),
									W.criminal(),
									"But since there IS no detention center, I will let you go.",
									false, false);
					dismissOfficer(officer);
					W.setArrestingOfficer(myArea, null);
				}
				...
				if (officer != null) {
					DebugLogLostConvicts("Officer can't detain criminal: ", W,
							officer);
					CMLib.commands().postSay(officer, null, "Sad.", false,
							false);
					fileArrestResister(laws, myArea, W);
					dismissOfficer(officer);
				}
		...
		case Law.STATE_EXECUTING: {
			if ((officer != null)
				...
				if ((judge != null)
					...
					for (Enumeration<MOB.Follower> f = W.criminal().followers(); f
							.hasMoreElements();) {
					...
					W.criminal().recoverCharStats();
					if (!served)
						CMLib.combat().postAttack(judge, W.criminal(),
								judge.fetchWieldedItem());
					W.setArrestingOfficer(myArea, null);
					W.setTravelAttemptTime(0);
					unCuff(W.criminal());
				} else {
					CMLib.commands()
							.postSay(officer, null,
									"Looks like court is not in session.",
									false, false);
					W.setTravelAttemptTime(0);
					unCuff(W.criminal());
					if (W.arrestingOfficer() != null)
				...
				if (officer != null) {
					DebugLogLostConvicts("Officer can't execute criminal: ", W,
							officer);
					CMLib.commands().postSay(officer, null,
							"Didn't see that coming.", false, false);
					fileArrestResister(laws, myArea, W);
				}
				W.setTravelAttemptTime(0);
				unCuff(W.criminal());
			...
			if ((officer != null)
				...
				if (W.jail() == W.criminal().location()) {
					...
					if (officer.fetchEffect("Skill_Track") == null) {
						...
						unCuff(W.criminal());
						CMLib.commands().postSay(officer, W.criminal(),
								"I lost the jail, so you are free to go.",
								false, false);
						dismissOfficer(officer);
						W.setArrestingOfficer(myArea, null);
					}
				} else if ((CMLib.dice().rollPercentage() > 75)
						&& (laws.chitChat2().size() > 0))
					CMLib.commands().postSay(
							officer,
							W.criminal(),
							laws.chitChat2().get(
									CMLib.dice().roll(1,
											laws.chitChat2().size(), -1)),
							false, false);
			} else {
				DebugLogLostConvicts("Officer can't move2 criminal: ", W,
						officer);
				fileArrestResister(laws, myArea, W);
				unCuff(W.criminal());
				W.setArrestingOfficer(myArea, null);
			...
			if ((officer != null)
				...
				if (W.jail() == W.criminal().location()) {
					...
					if (W.criminal().fetchEffect("Prisoner") == null) {
						...
						.flaggedAffects(officer, Ability.FLAG_TRACKING).size() == 0) {
					Ability A = CMClass.getAbility("Skill_Track");
					if (A != null) {
						CMLib.tracking().stopTracking(officer);
						A.setAbilityCode(1); // tells track to cache the path
					...
					if (officer.fetchEffect("Skill_Track") == null) {
						...
						CMLib.commands()
								.postSay(
										officer,
										W.criminal(),
										"I lost the detention center, so you are free to go.",
										false, false);
						dismissOfficer(officer);
						W.setArrestingOfficer(myArea, null);
					}
				} else if ((CMLib.dice().rollPercentage() > 75)
						&& (laws.chitChat3().size() > 0))
					CMLib.commands().postSay(
							officer,
							W.criminal(),
							laws.chitChat3().get(
									CMLib.dice().roll(1,
											laws.chitChat3().size(), -1)),
							false, false);
			} else {
				DebugLogLostConvicts("Officer can't move3 criminal: ", W,
						officer);
				fileArrestResister(laws, myArea, W);
				unCuff(W.criminal());
				W.setArrestingOfficer(myArea, null);
		...
		case Law.STATE_RELEASE: {
			if (((W.criminal().fetchEffect("Prisoner") == null) || (W.crime()
					.equalsIgnoreCase("pardoned"))) && (W.jail() != null)) {
				Ability P = W.criminal().fetchEffect("Prisoner");
				if (P != null)
					P.unInvoke();
				if (CMath.bset(highestCrimeAction(laws, W, W.criminal()),
						Law.PUNISHMENTMASK_NORELEASE)) {
					W.setTravelAttemptTime(0);
					fileAllWarrants(laws, W, W.criminal());
					unCuff(W.criminal());
					W.setArrestingOfficer(myArea, null);
					...
					if ((officer == null)
							...
							|| (!CMLib.flags().isInTheGame(W.criminal(), true))
							|| (!W.criminal().location().isInhabitant(officer))) {
						W.setArrestingOfficer(
								myArea,
								getAnyElligibleOfficer(laws,
										W.jail().getArea(), W.criminal(),
										W.victim()));
						if (W.arrestingOfficer() == null)
							W.setArrestingOfficer(
									myArea,
									getAnyElligibleOfficer(laws, myArea,
											W.criminal(), W.victim()));
						if (W.arrestingOfficer() == null)
							break;
						...
						if (!canFocusOn(officer, W.criminal())) {
							W.jail()
									.show(officer, W.criminal(),
											CMMsg.MSG_QUIETMOVEMENT,
											"<S-NAME> arrive(s) to release <T-NAME>, but can't find <T-HIM-HER>.");
							dismissOfficer(officer);
							W.setArrestingOfficer(myArea, null);
						} else
							W.jail().show(officer, W.criminal(),
									CMMsg.MSG_QUIETMOVEMENT,
									"<S-NAME> arrive(s) to release <T-NAME>.");
						Ability A = CMClass.getAbility("Skill_HandCuff");
						if ((A != null)
								&& (!CMLib.flags().isBoundOrHeld(W.criminal())))
							A.invoke(officer, W.criminal(), true, 0);
					}
					W.setReleaseRoom(getReleaseRoom(laws, myArea, W.criminal(),
							W));
					W.criminal().makePeace();
					makePeace(officer.location());
					CMLib.tracking().stopTracking(officer);
					...
					if (officer.fetchEffect("Skill_Track") == null) {
						...
						unCuff(W.criminal());
						CMLib.commands().postSay(officer, W.criminal(),
								"Well, you can always recall.", false, false);
						dismissOfficer(officer);
						W.setArrestingOfficer(myArea, null);
					}
					...
					if (W.criminal().location() == W.releaseRoom()) {
						...
						if (officer != null) {
							if ((CMLib.flags().aliveAwakeMobile(officer, true))
									&& (W.criminal().location()
											.isInhabitant(officer)))
								CMLib.commands().postSay(officer, null,
										laws.getMessage(Law.MSG_LAWFREE),
										false, false);
							dismissOfficer(officer);
						}
						W.setTravelAttemptTime(0);
						...
						if ((officer != null)
								&& (CMLib.flags().aliveAwakeMobile(officer,
										true))
								&& (W.criminal().location()
										.isInhabitant(officer))
								&& ((W.travelAttemptTime() == 0) || ((System
										.currentTimeMillis() - W
										.travelAttemptTime()) < (5 * 60 * 1000)))) {
							if (officer.isMonster())
								CMLib.commands().postLook(officer, true);
							...
							if (W.arrestingOfficer().fetchEffect("Skill_Track") == null) {
								...
								if (A != null)
									A.invoke(officer,
											CMParms.parse(CMLib.map()
													.getExtendedRoomID(
															W.releaseRoom())),
											W.releaseRoom(), true, 0);
								if (W.arrestingOfficer().fetchEffect(
										"Skill_Track") == null) {
									W.setTravelAttemptTime(0);
									fileAllWarrants(laws, W, W.criminal());
									unCuff(W.criminal());
									CMLib.commands()
											.postSay(
													W.arrestingOfficer(),
													W.criminal(),
													"Don't worry, you can always recall.",
													false, false);
									dismissOfficer(W.arrestingOfficer());
									W.setArrestingOfficer(myArea, null);
								}
							...
							if (officer != null) {
								DebugLogLostConvicts(
										"Officer can't release criminal: ", W,
										officer);
								CMLib.commands().postSay(officer, null,
										"There's always recall.", false, false);
							}
							W.setTravelAttemptTime(0);
							fileAllWarrants(laws, W, W.criminal());
							unCuff(W.criminal());
					...
					if (W.arrestingOfficer() != null) {
						DebugLogLostConvicts(
								"Officer can't release2 criminal: ", W,
								W.arrestingOfficer());
						CMLib.commands().postSay(W.arrestingOfficer(), null,
								"Well, he can always recall.", false, false);
					}
					W.setTravelAttemptTime(0);
					fileAllWarrants(laws, W, W.criminal());
					unCuff(W.criminal());
