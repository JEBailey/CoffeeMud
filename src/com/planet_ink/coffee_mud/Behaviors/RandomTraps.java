...
public class RandomTraps extends ActiveTicker {
	...
	public void setParms(String newParms) {
		...
		if (x >= 0) {
			...
			String p = CMParms.getParmStr(oldParms, "ROOMS", "NO")
					.toUpperCase().trim();
			if (p.startsWith("Y"))
				doRooms = true;
			p = CMParms.getParmStr(oldParms, "ITEMS", "NO").toUpperCase()
					.trim();
			if (p.startsWith("Y")) {
				doAnyItems = true;
			...
			p = CMParms.getParmStr(oldParms, "EXITS", "LOCKED").toUpperCase()
					.trim();
			if ((p.startsWith("Y")) || (p.startsWith("DOOR"))) {
				doAnyDoors = true;
				doAnyLockedDoors = true;
			} else if (p.startsWith("LOCK")) {
				doAnyDoors = false;
	...
	protected void makeRoomElligible(Room R, Vector elligible) {
		...
		if (R instanceof GridLocale) {
			...
			if (map.size() == 0)
				...
				for (Iterator<Room> m = ((GridLocale) R).getAllRooms()
						.iterator(); m.hasNext();)
		...
		if ((canAct(ticking, tickID)) || (maintained.size() < minTraps)) {
			...
			if (maintained.size() < avgTraps) {
				...
				if (ticking instanceof Room) {
					...
					for (Enumeration m = ((Area) ticking).getMetroMap(); m
							.hasMoreElements();)
				...
				for (int r = 0; r < oldSize; r++) {
					...
					if ((doAnyDoors) || (doAnyLockedDoors))
						for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
							...
							if (((R.rawDoors()[d] != null) // important
															// optimization
							&& (R.rawDoors()[d].roomID().length() > 0))) {
								...
								if ((R.getRoomInDir(d) != null)
										...
										&& ((R.getReverseExit(d) == null) || (!elligible
												.contains(R.getReverseExit(d))))) {
					...
					if ((doAnyItems) || (doAnyContainers)
							|| (doDooredContainers) || (doLockedContainers))
				...
				if (!doRooms)
					while ((elligible.size() > 0)
							&& (elligible.firstElement() instanceof Room))
				...
				for (int e = elligible.size() - 1; e >= 0; e--) {
					...
					if ((maintained.contains(elligible.elementAt(e)))
							|| (CMLib.utensils().fetchMyTrap(
									elligible.elementAt(e)) != null))
				...
				Physical P = elligible.elementAt(CMLib.dice().roll(1,
						elligible.size(), -1));
				...
				Trap T = (Trap) elligibleTraps.elementAt(CMLib.dice().roll(1,
						elligibleTraps.size(), -1));
				T = (Trap) T.copyOf();
