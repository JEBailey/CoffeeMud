...
public class BribeGateGuard extends StdBehavior {
	...
	protected DoorKey getMyKeyTo(MOB mob, Exit e) {
		...
		for (int i = 0; i < mob.numItems(); i++) {
			...
			if ((item instanceof DoorKey)
					&& (((DoorKey) item).getKey().equals(keyCode))) {
				key = (DoorKey) item;
				break;
			}
		}
	...
	protected void payment(Coins given, MOB gateGuard, MOB mob) {
		...
		Coins item = CMLib.beanCounter().makeBestCurrency(
				CMLib.beanCounter().getCurrency(gateGuard), newNum);
		delBalance(mob);
		if (item != null)
	...
	protected double getBalance(MOB mob) {
		...
		if (surviveReboot) {
			List<JournalsLibrary.JournalEntry> V = CMLib.database()
					.DBReadJournalMsgs("BRIBEGATE_" + gates());
			...
			for (int v = 0; v < mine.size(); v++) {
				JournalsLibrary.JournalEntry V2 = (JournalsLibrary.JournalEntry) mine
						.elementAt(v);
				...
				if (fullName.equals("COINS")) {
					...
					if (item != null) {
						CMLib.coffeeMaker()
								.setPropertiesStr(item, V2.msg, true);
	...
	protected void charge(double charge, MOB gateGuard, MOB mob) {
		...
		if (newNum > 0) {
			...
			item.setDenomination(CMLib.beanCounter().getLowestDenomination(
					CMLib.beanCounter().getCurrency(gateGuard)));
	...
	protected void delBalance(MOB mob) {
		...
		if (surviveReboot) {
			...
			for (int v = 0; v < mine.size(); v++) {
				...
				String fullName = V2.subj;
				if (fullName.equals("COINS")) {
					CMLib.database().DBDeleteJournal("BRIBEGATE_" + gates(),
							V2.key);
				}
			}
		} else {
	...
	protected void writeBalance(Coins balance, MOB mob) {
		...
		if (surviveReboot) {
			CMLib.database().DBWriteJournal("BRIBEGATE_" + gates(), mob.Name(),
					CMClass.classID(balance), "COINS",
					CMLib.coffeeMaker().getPropertiesStr(balance, true));
		} else {
			Hashtable H = (Hashtable) notTheJournal.get(gates());
			if (H == null) {
				H = new Hashtable();
	...
	public boolean okMessage(Environmental oking, CMMsg msg) {
		...
		if (msg.amITarget(monster) && (!msg.amISource(monster))
				&& (msg.targetMinor() == CMMsg.TYP_GIVE)
				&& (msg.tool() != null) && (msg.tool() instanceof Coins)
				&& (!((Coins) msg.tool()).getCurrency().equals(currency))) {
			double denomination = CMLib.beanCounter().getLowestDenomination(
					currency);
			...
			if (msg.target() instanceof Exit) {
				...
				if (!msg.source().isMonster()) {
					if ((msg.targetMinor() != CMMsg.TYP_CLOSE)) {
						...
						CMMsg msgs = CMClass.getMsg(monster, mob,
						...
						if (monster.location().okMessage(monster, msgs)) {
							...
							double denomination = CMLib.beanCounter()
									.getLowestDenomination(currency);
							String thePrice = CMLib.beanCounter()
									.getDenominationName(currency,
											denomination,
											Math.round(price() / denomination));
							CMLib.commands().postSay(
									monster,
									mob,
									"I'll let you through here if you pay the fee of "
											+ thePrice + ".", true, false);
							if (debug) // debugging
								CMLib.commands().postSay(monster, mob,
										"I'm telling you this from okAffect",
										true, false);
							return false;
						}
						if (debug) // debugging
							CMLib.commands().postSay(monster, mob,
									"I'm telling you this from okAffect (2)",
									true, false);
						return false;
					}
					if (msg.target() instanceof Room) {
						if (debug) // debugging
							CMLib.commands().postSay(monster, mob,
									"I'm telling you this from okAffect (3)",
									true, false);
					}
					if (debug) {
						CMLib.commands().postSay(
						...
						CMLib.commands().postSay((MOB) oking, msg.source(),
								"source Monster? " + msg.source().isMonster(),
								true, true);
					}
					return true;
				}
		...
		if (debug) {
			// CMLib.commands().postSay((MOB)oking,msg.source(),"okAffect triggered.  WRONG LOCATION TO FIRE.",
			// true,true);
		}
		if ((mob.location() == monster.location()) && (mob != monster)
				&& (msg.target() != null)
			...
			if ((msg.tool() != null) && (msg.target() instanceof Room)
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_ENTER)
				...
				&& (CMLib.flags().canSenseEnteringLeaving(msg.source(),
						observer)) && (!msg.source().isMonster())) {
			...
			if (checkBalance(price(), source)) {
				...
				&& (!msg.amISource(observer)) && (!msg.source().isMonster())) {
			toldAlready.remove(source.Name());
			if (paidPlayers.contains(source)) { // the player that the guard
												// acknowledged as paid has now
												// left
				paidPlayers.remove(source);
				if ((msg.tool() != null) && (msg.tool() instanceof Exit)) {
					Exit exit = (Exit) msg.tool();
					if (exit.Name().equals(e.Name())) { // the player is walking
						...
						if (debug) {
							CMLib.commands().postSay(
									observer,
									source,
									"Charging " + price() + ", balance "
											+ getBalance(source) + ".", true,
									true);
						}
					}
				}
		...
		} else if (msg.amITarget(observer) && (!msg.amISource(observer))
				&& (msg.targetMinor() == CMMsg.TYP_GIVE)
				&& (msg.tool() != null) && (msg.tool() instanceof Coins)) {
			payment((Coins) msg.tool(), observer, msg.source());
			CMLib.commands().postSay(observer, source, "Thank you very much.",
					true, false);
			if (getBalance(source) > price()) {
				String currency = CMLib.beanCounter().getCurrency(observer);
				double denomination = CMLib.beanCounter()
						.getLowestDenomination(currency);
				long diff = Math.round((getBalance(source) - price())
						/ denomination);
				String difference = CMLib.beanCounter().getDenominationName(
						currency, denomination, diff);
				CMLib.commands().postSay(
						observer,
						source,
						"I'll hang on to the additional " + difference
								+ " for you", true, false);
				paidPlayers.addElement(source);
				toldAlready.put(source.Name(), Boolean.FALSE);
				if (debug) // debugging
				...
				CMMsg msg2 = CMClass.getMsg(observer, null, msg.tool(),
						CMMsg.MSG_EMOTE,
						"^E<S-NAME> look(s) carefully at <O-NAME>.^?");
				msg.addTrailerMsg(msg2);
				msg2 = CMClass
						.getMsg(observer, null, null, CMMsg.MSG_SPEAK,
								"^T<S-NAME> say(s) 'I'm afraid that this is insufficient.'^?.");
				msg.addTrailerMsg(msg2);
				msg2 = CMClass.getMsg(observer, source, msg.tool(),
						CMMsg.MSG_GIVE,
		...
		if (!e.isOpen()) {
			for (int j = 0; j < mob.location().numInhabitants(); j++) {
				...
				if (M.playerStats() != null) {
					if ((paidPlayers.contains(M))
							&& (toldAlready.containsKey(M.Name()))) {
						...
						if (!B.booleanValue()) {
							...
							double denomination = CMLib.beanCounter()
									.getLowestDenomination(currency);
							String balanceStr = CMLib.beanCounter()
									.getDenominationName(
											currency,
											denomination,
											Math.round(getBalance(M)
													/ denomination));
							CMLib.commands()
									.postSay(
											mob,
											...
											"We still have record that you gave us "
													+ balanceStr
													+ " before if you're heading through",
											true, false);
						}
						...
						double denomination = CMLib.beanCounter()
								.getLowestDenomination(currency);
						String priceStr = CMLib.beanCounter()
								.getDenominationName(currency, denomination,
										Math.round(price() / denomination));
						CMLib.commands().postSay(
								mob,
								M,
						...
						if (debug) // debugging
							CMLib.commands().postSay(mob, M,
									"I'm telling you this from tick", true,
									false);
					}
				}
			}
		}
		boolean nightTime = (mob.location().getArea().getTimeObj().getTODCode() == TimeClock.TIME_NIGHT);
		if (nightTime) {
			if ((!e.isLocked()) && (e.hasALock())) {
				if (getMyKeyTo(mob, e) != null) {
					...
					if (mob.location().okMessage(mob, msg)) {
						CMLib.utensils().roomAffectFully(msg, mob.location(),
								dir);
					}
				}
			}
