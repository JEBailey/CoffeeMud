...
public class RandomMonsters extends ActiveTicker {
	...
	public void setParms(String newParms) {
		...
		avgMonsters = CMLib.dice().roll(1, (maxMonsters - minMonsters),
				minMonsters);
		parms = newParms;
	...
	public List<MOB> getMonsters(Tickable thang, String theseparms) {
		...
		if (thang instanceof Room)
			thangName = CMLib.map().getExtendedRoomID((Room) thang);
				...
				&& (((MOB) thang).getStartRoom()) != null)
			thangName = CMLib.map().getExtendedRoomID(
					((MOB) thang).getStartRoom());
		else if (thang != null)
		...
		if (filename.trim().length() == 0) {
			...
			Log.errOut("Blank XML/filename: '" + filename
					+ "' on Behavior RandomMonsters on object " + thangName
					+ " (" + thangID + ").");
			return null;
		}
		int start = filename.indexOf("<MOBS>");
		if ((start >= 0) && (start <= 20)) {
			...
			monsters = (List<MOB>) Resources.getResource("RANDOMMONSTERS-XML/"
					+ filename.length() + "/" + filename.hashCode());
			if (monsters != null)
				return monsters;
			monsters = new Vector();
			String error = CMLib.coffeeMaker().addMOBsFromXML(filename,
					monsters, null);
			if (error.length() > 0) {
				if (alreadyTriedLoad)
					return null;
				alreadyTriedLoad = true;
				Log.errOut("RandomMonsters: Error on import of xml for '"
						+ thangName + "' (" + thangID + "): " + error + ".");
				return null;
			}
			if (monsters.size() <= 0) {
				if (alreadyTriedLoad)
			...
			Resources.submitResource("RANDOMMONSTERS-XML/" + filename.length()
					+ "/" + filename.hashCode(), monsters);
		} else {
			int extraSemicolon = filename.indexOf(';');
			if (extraSemicolon >= 0)
				filename = filename.substring(0, extraSemicolon);
			filename = filename.trim();
			monsters = (List<MOB>) Resources.getResource("RANDOMMONSTERS-"
					+ filename);
			if ((monsters == null) && (!alreadyTriedLoad)) {
				...
				if ((buf == null) || (buf.length() < 20)) {
					Log.errOut("RandomMonsters: Unknown XML file: '" + filename
							+ "' for '" + thangName + "' (" + thangID + ").");
					return null;
				}
				if (buf.substring(0, 20).indexOf("<MOBS>") < 0) {
					Log.errOut("RandomMonsters: Invalid XML file: '" + filename
							+ "' for '" + thangName + "' (" + thangID + ").");
					return null;
				}
				monsters = new Vector();
				String error = CMLib.coffeeMaker().addMOBsFromXML(
						buf.toString(), monsters, null);
				if (error.length() > 0) {
					Log.errOut("RandomMonsters: Error on import of: '"
							+ filename + "' for '" + thangName + "' ("
							+ thangID + "): " + error + ".");
					return null;
				}
				if (monsters.size() <= 0) {
					Log.errOut("RandomMonsters: No mobs loaded: '" + filename
							+ "' for '" + thangName + "' (" + thangID + ").");
					return null;
				}

				Resources
						.submitResource("RANDOMMONSTERS-" + filename, monsters);
			}
		}
		return monsters;
	}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		for (int i = maintained.size() - 1; i >= 0; i--) {
			try {
				...
				if ((M.amDead()) || (M.amDestroyed()) || (M.location() == null)
				...
				if (M != null) {
					...
					if (ticking instanceof Room) {
						...
						Resources.removeResource("HELP_"
								+ ((Room) ticking).getArea().name()
										.toUpperCase());
					} else if ((ticking instanceof Area)
							&& (((Area) ticking).metroSize() > 0)) {
