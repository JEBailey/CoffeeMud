...
public class MobileAggressive extends Mobile {
	...
	public String accountForYourself() {
		if (getParms().trim().length() > 0)
			return "wandering aggression against "
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((--aggressiveTickDown) < 0) {
			...
			veryA.tickAggressively(ticking, tickID, mobkill, misbehave,
					getParms(), attackMsg);
			tickStatus = Tickable.STATUS_MISC + 3;
			veryA.tickVeryAggressively(ticking, tickID, wander, mobkill,
					misbehave, getParms(), attackMsg);
		}
		tickStatus = Tickable.STATUS_NOT;
		return true;
	}
