...
public class Concierge extends StdBehavior {
	...
	public void setParms(String newParm) {
		...
		for (int v = 0; v < V.size(); v++) {
			...
			else if ((R != null) && (!rates.contains(R)))
	...
	public Environmental findDestination(MOB observer, MOB mob, String where) {
		...
		if ((stringsToDo != null) && (observer != null)) {
			...
			List<Room> roomsInRange = CMLib.tracking().getRadiantRooms(
					observer.location(), flags, 50);
			Room R = null;
			String place = null;
			for (int r = 0; r < stringsToDo.size(); r++) {
				place = (String) stringsToDo.elementAt(r, 1);
				R = (Room) CMLib.english().fetchEnvironmental(roomsInRange,
						place, false);
				if (R != null)
					rates.addElement(R, stringsToDo.elementAt(r, 2));
			}
			stringsToDo.clear();
			stringsToDo = null;
		}
		Environmental E = CMLib.english().fetchEnvironmental(
				rates.getDimensionVector(1), where, true);
		if (E == null)
			E = CMLib.english().fetchEnvironmental(rates.getDimensionVector(1),
					where, false);
		return E;
	}
	...
	public boolean okMessage(Environmental affecting, CMMsg msg) {
		...
		if ((source != observer)
				...
				&& (!CMSecurity.isAllowed(source, source.location(),
						CMSecurity.SecFlag.CMDROOMS))
				&& (!CMSecurity.isAllowed(source, source.location(),
						CMSecurity.SecFlag.CMDMOBS)) && (msg.tool() != null)) {
			...
			if (destIndex < 0) {
				CMLib.commands()
						.postSay(
								observer,
								source,
								"What's this for?  Please tell me where you'd like to go first.",
								true, false);
				return false;
			} else if (!(msg.tool() instanceof Coins)) {
				CMLib.commands().postSay(observer, source,
						"I'm sorry, I can only accept money.", true, false);
				return false;
			} else if (!((Coins) msg.tool()).getCurrency().equalsIgnoreCase(
					CMLib.beanCounter().getCurrency(observer))) {
				CMLib.commands().postSay(observer, source,
						"I'm sorry, I don't accept that kind of currency.",
						true, false);
				return false;
			}
			Environmental destination = (Environmental) destinations.elementAt(
					destIndex, 2);
			Double paid = (Double) destinations.elementAt(
					destinations.indexOf(source), 3);
			double owed = getPrice(destination) - paid.doubleValue();
			if (owed <= 0.0) {
				CMLib.commands().postSay(observer, source,
						"Hey, you've already paid me!", true, false);
				return false;
			}
		}
		return true;
	...
	public boolean tick(Tickable ticking, int tickID) {
		if ((ticking instanceof MOB) && (tickID == Tickable.TICKID_MOB)
				&& (thingsToSay.size() > 0) && (canFreelyBehaveNormal(ticking))) {
			...
			synchronized (thingsToSay) {
				while (thingsToSay.size() > 0) {
					...
					CMLib.commands()
							.postSay(observer, source, msg, true, false);
				}
			}
		}
	...
	public String getDestination(MOB from, Environmental to) {
		...
		int radius = 100;
		CMLib.tracking().getRadiantRooms(from.location(), set, flags, null,
				radius, null);
		return CMLib.tracking().getTrailToDescription(from.location(), set,
				name, false, false, radius, null, 1);
	}
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		MOB source = msg.source();
		MOB observer = (MOB) affecting;
		if ((source != observer) && (msg.targetMinor() == CMMsg.TYP_GIVE)
				&& (msg.amITarget(observer)) && (msg.tool() instanceof Coins)) {
			int destIndex = destinations.indexOf(source);
			if ((CMLib.flags().canBeSeenBy(source, observer))
					&& (CMLib.flags().canBeSeenBy(observer, source))
					&& (destIndex >= 0)) {
				Environmental destination = (Environmental) destinations
						.elementAt(destIndex, 2);
				Double paid = (Double) destinations.elementAt(destIndex, 3);
				double owed = getPrice(destination) - paid.doubleValue();
				owed -= ((Coins) msg.tool()).getTotalValue();
				if (owed > 0.0) {
					destinations.setElementAt(destIndex, 3,
							Double.valueOf(owed));
					CMLib.commands().postSay(
							...
							"Ok, you still owe "
									+ CMLib.beanCounter().nameCurrencyLong(
											observer, owed) + ".", true, false);
					return;
				} else if (owed < 0.0) {
					double change = -owed;
					Coins C = CMLib.beanCounter().makeBestCurrency(observer,
							change);
					if ((change > 0.0) && (C != null)) {
						// this message will actually end up triggering the
						// hand-over.
						CMMsg newMsg = CMClass
								.getMsg(observer, source, C, CMMsg.MSG_SPEAK,
				...
				thingsToSay.addElement(
						msg.source(),
						"Thank you. The way to " + destination.name()
								+ " from here is: "
								+ this.getDestination(observer, destination));
				destinations.removeElement(msg.source());
			} else if (!CMLib.flags().canBeSeenBy(source, observer))
				CMLib.commands().postSay(observer, null,
						"Wha?  Where did this come from?  Cool!", true, false);
			...
			CMLib.beanCounter().giveSomeoneMoney(msg.source(),
				...
				&& ((msg.target() == observer) || (observer.location()
						.numPCInhabitants() == 1))
			...
			if ((say != null) && (say.length() > 0)) {
				...
				if (E == null)
					synchronized (thingsToSay) {
						thingsToSay.addElement(msg.source(),
								"I'm sorry, I don't know where '" + say
										+ "' is.");
						return;
					}
				int index = destinations.indexOf(msg.source());
				Double paid = (index >= 0) ? (Double) destinations.elementAt(
						index, 3) : Double.valueOf(0.0);
				destinations.removeElement(msg.source());
				double rate = getPrice(E);
				if (rate <= 0.0)
					thingsToSay.addElement(msg.source(),
							"Yes, the way to " + E.name() + " from here is: "
									+ this.getDestination(observer, E));
				else {
					destinations.addElement(msg.source(), E, paid);
					thingsToSay.addElement(
							msg.source(),
							"Yep, I can help you find "
									+ E.name()
									+ ", but you'll need to give me "
									+ CMLib.beanCounter().nameCurrencyLong(
											observer, rate) + " first.");
				}
			}
