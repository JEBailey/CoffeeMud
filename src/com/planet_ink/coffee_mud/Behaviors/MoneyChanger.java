...
public class MoneyChanger extends StdBehavior {
	...
	public void setParms(String newParm) {
		...
		while (x > 0) {
			...
			if (lastSp < 0)
				lastSp = 0;
			if ((lastSp >= 0) && (lastSp < x - 1)
					&& (Character.isLetter(newParm.charAt(x - 1)))) {
				...
				if (x < newParm.length()) {
					...
					if (x < newParm.length()) {
						...
						x = 0;
						while ((x < newParm.length())
								&& ((Character.isDigit(newParm.charAt(x))) || (newParm
										.charAt(x) == '.')))
							x++;
	...
	public boolean okMessage(Environmental affecting, CMMsg msg) {
		...
		if ((source != observer)
				...
				&& (!CMSecurity.isAllowed(source, source.location(),
						CMSecurity.SecFlag.CMDROOMS)) && (msg.tool() != null)) {
			if (!(msg.tool() instanceof Coins)) {
				CMLib.commands().postSay(observer, source,
						"I'm sorry, I can only accept money.", true, false);
				return false;
			} else if (((rates.size() > 0) && (!rates.containsKey(((Coins) msg
					.tool()).getCurrency().toUpperCase())))) {
				CMLib.commands().postSay(observer, source,
			...
			if ((amountToTake > 0.0)
					&& (amountToTake < CMLib.beanCounter()
							.getLowestDenomination(
									CMLib.beanCounter().getCurrency(observer))))
				amountToTake = CMLib.beanCounter().getLowestDenomination(
						CMLib.beanCounter().getCurrency(observer));
			value -= amountToTake;
			...
			if ((value <= 0) || (C == null)) {
				CMLib.commands().postSay(observer, source,
						"I'm sorry, I can not change such a small amount.",
						true, false);
				return false;
			}
		}
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if ((source != observer) && (msg.amITarget(observer))
				&& (msg.targetMinor() == CMMsg.TYP_GIVE)
			...
			if ((CMLib.flags().canBeSeenBy(source, observer))
				...
				double value = ((Coins) msg.tool()).getTotalValue();
				double takeCut = cut;
				String currency = ((Coins) msg.tool()).getCurrency()
						.toUpperCase();
				if ((rates.size() > 0) && (rates.containsKey(currency)))
					takeCut = ((Double) rates.get(currency)).doubleValue();
				double amountToTake = CMLib.beanCounter().abbreviatedRePrice(
				...
				if ((value > 0.0) && (C != null)) {
					...
					CMMsg newMsg = CMClass
							.getMsg(observer, source, C, CMMsg.MSG_SPEAK,
									"^T<S-NAME> say(s) 'Thank you for your business' to <T-NAMESELF>.^?");
					C.setOwner(observer);
		...
		} else if ((msg.source() == observer) && (msg.target() instanceof MOB)
