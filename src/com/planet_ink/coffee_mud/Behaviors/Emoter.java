...
public class Emoter extends ActiveTicker {
	...
	public void setParms(String newParms) {
		...
		inroomIDs = CMParms.parseSemicolons(
				CMParms.getParmStr(parms, "inroom", "").toUpperCase().trim(),
				true);
		emotes = null;
		smells = null;
	}
	...
	protected static enum EMOTE_TYPE {
		EMOTE_VISUAL, EMOTE_SOUND, EMOTE_SMELL, EMOTE_SOCIAL
	}
	...
	protected void setEmoteTypes(Vector V, boolean respectOnlyBeginningAndEnd) {
		if (respectOnlyBeginningAndEnd) {
			if (setEmoteType((String) V.firstElement()))
				V.removeElementAt(0);
			else if (setEmoteType((String) V.lastElement()))
	...
	public void executeMsg(final Environmental myHost, final CMMsg msg) {
		...
		if ((msg.amITarget(myHost)) && (msg.targetMinor() == CMMsg.TYP_SNIFF)
				&& (CMLib.flags().canSmell(msg.source())) && (smells != null)) {
			EmoteObj emote = smells
					.get(CMLib.dice().roll(1, smells.size(), -1));
			MOB emoter = null;
	...
	protected void emoteHere(Room room, MOB emoter, EmoteObj emote,
			MOB emoteTo, boolean Wrapper) {
		if (room == null)
			return;
		if (!inRoom(room))
			return;
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((canAct(ticking, tickID)) && (emotes.size() > 0)
			...
			EmoteObj emote = emotes
					.get(CMLib.dice().roll(1, emotes.size(), -1));
			MOB emoter = null;
			...
			if (ticking instanceof MOB) {
				...
				if ((ticking instanceof Item)
						&& (!CMLib.flags().isInTheGame((Item) ticking, false)))
					return true;
			...
			if (emote.broadcast) {
				...
				for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
					...
					if ((R != null) && (E != null) && (E.isOpen())) {
						final String inDir = ((R instanceof SpaceShip) || (R
								.getArea() instanceof SpaceShip)) ? Directions
								.getShipInDirectionName(Directions
										.getOpDirectionCode(d)) : Directions
								.getInDirectionName(Directions
