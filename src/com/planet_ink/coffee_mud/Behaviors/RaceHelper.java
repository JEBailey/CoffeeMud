...
public class RaceHelper extends StdBehavior {
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if ((source != observer)
				...
				&& (!observer.isInCombat()) && (target != observer)
				&& (source != target)
			...
			if (observer.charStats().getMyRace().ID()
					.equalsIgnoreCase(target.charStats().getMyRace().ID())) {
				String reason = "THAT`S MY FRIEND!! CHARGE!!";
				if ((observer.charStats().getMyRace().ID().equals(target
						.charStats().getMyRace().ID()))
						&& (!observer.charStats().getMyRace().ID()
								.equals(source.charStats().getMyRace().ID())))
					reason = observer.charStats().getMyRace().ID()
							.toUpperCase()
							+ "s UNITE! CHARGE!";
