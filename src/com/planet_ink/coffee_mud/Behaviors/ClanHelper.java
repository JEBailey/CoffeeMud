...
public class ClanHelper extends StdBehavior {
	...
	public void startBehavior(PhysicalAgent forMe) {
		...
		if (forMe instanceof MOB) {
			if (parms.length() > 0) {
				...
				if (C != null)
					((MOB) forMe).setClan(C.clanID(), C.getGovernment()
							.getAcceptPos());
				else
					Log.errOut(
							"ClanHelper",
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if ((source != observer)
			...
			List<Triad<Clan, Integer, Integer>> list = CMLib.clans()
					.findCommonRivalrousClans(observer, target);
			if (list.size() > 0) {
				Clan C = null;
