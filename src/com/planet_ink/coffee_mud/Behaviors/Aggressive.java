...
public class Aggressive extends StdBehavior {
	...
	public static boolean startFight(MOB monster, MOB mob, boolean fightMOBs,
			boolean misBehave, String attackMsg) {
		if ((mob != null) && (monster != null) && (mob != monster)) {
			...
			if ((R != null)
					...
					&& (!monster.getGroupMembers(new HashSet<MOB>()).contains(
							mob))) {
				...
				if (CMLib.flags().isHidden(monster)) {
					...
					if (A != null) {
						...
						monster.enqueCommand(new XVector(A.triggerStrings()[0],
								R.getContextName(mob)),
								Command.METAFLAG_FORCED, 0);
					}
				}
				if ((attackMsg != null) && (monster.getVictim() != mob))
					monster.enqueCommand(new XVector("SAY", attackMsg),
							Command.METAFLAG_FORCED, 0);
				// normal attack
				monster.enqueCommand(
						new XVector("KILL", R.getContextName(mob)),
						Command.METAFLAG_FORCED, 0);
				return true;
			}
		}
	...
	public boolean pickAFight(MOB observer, String zapStr, boolean mobKiller,
			boolean misBehave, String attackMsg) {
	...
	public void tickAggressively(Tickable ticking, int tickID,
			boolean mobKiller, boolean misBehave, String zapStr,
			String attackMsg) {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((--tickDown) < 0) {
			tickDown = tickWait;
			tickAggressively(ticking, tickID, mobkill, misbehave, getParms(),
					attackMessage);
		}
		return true;
	}
