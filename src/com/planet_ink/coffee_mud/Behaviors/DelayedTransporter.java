...
public class DelayedTransporter extends ActiveTicker {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((room != null) && (destRoomNames != null)
				&& (destRoomNames.size() > 0))
				...
				if (inhab != null) {
					...
					if (I.intValue() >= minTicks)
						if ((CMLib.dice().rollPercentage() < chance)
								|| (I.intValue() > maxTicks)) {
							String roomName = (String) destRoomNames
									.elementAt(CMLib.dice().roll(1,
											destRoomNames.size(), -1));
							Room otherRoom = CMLib.map().getRoom(roomName);
							if (otherRoom == null)
								inhab.tell("You are whisked nowhere at all, since '"
										+ roomName
