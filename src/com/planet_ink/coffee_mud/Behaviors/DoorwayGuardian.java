...
public class DoorwayGuardian extends StdBehavior {
	...
	public Exit[] getParmExits(MOB monster) {
		...
		if (dirs != null)
			for (Enumeration<Integer> dirE = dirs.elements(); dirE
					.hasMoreElements();) {
				int dir = dirE.nextElement().intValue();
				if (room.getExitInDir(dir) != null) {
					Exit[] exits = { room.getExitInDir(dir),
							room.getReverseExit(dir) };
					return exits;
				}
			}
	...
	public boolean okMessage(Environmental oking, CMMsg msg) {
		...
		if ((mob.location() == monster.location())
			...
			if (msg.target() instanceof Exit) {
				...
				if ((texit != null) && (texit[0] != exit) && (texit[1] != exit))
					return true;
				...
				if ((msg.targetMinor() != CMMsg.TYP_CLOSE)
					...
					CMMsg msgs = CMClass.getMsg(monster, mob,
							CMMsg.MSG_NOISYMOVEMENT, message);
