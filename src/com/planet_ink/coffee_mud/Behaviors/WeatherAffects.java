...
public class WeatherAffects extends PuddleMaker {
	...
	private static final long[] ALL_COVERED_SPOTS = { Wearable.WORN_FEET,
			Wearable.WORN_TORSO, Wearable.WORN_LEGS };
	private static long ALL_COVERED_CODE = 0;
	private static final long[] ALL_FROST_SPOTS = { Wearable.WORN_FEET,
			Wearable.WORN_HANDS, Wearable.WORN_HEAD };
	...
	private void resetBotherTicks() {
		botherDown = CMParms.getParmInt(parms, "botherticks",
				Climate.WEATHER_TICK_DOWN / 3);
	}

	private void resetDiseaseTicks() {
		diseaseDown = CMParms.getParmInt(parms, "diseaseticks",
				Climate.WEATHER_TICK_DOWN);
	}
	...
	private void resetLightningTicks() {
		lightningDown = CMParms.getParmInt(parms, "lightningticks",
				Climate.WEATHER_TICK_DOWN * 4);
	}

	private void resetRumbleTicks() {
		rumbleDown = CMParms.getParmInt(parms, "rumbleticks",
				Climate.WEATHER_TICK_DOWN / 4);
	}

	private void resetGustTicks() {
		gustDown = CMParms.getParmInt(parms, "gustticks",
				Climate.WEATHER_TICK_DOWN);
	}

	private void resetTornadoTicks() {
		tornadoDown = CMParms.getParmInt(parms, "tornadoticks",
				Climate.WEATHER_TICK_DOWN * 10);
	}

	private void resetHailTicks() {
		hailDown = CMParms.getParmInt(parms, "hailticks",
				Climate.WEATHER_TICK_DOWN / 2);
	}
	...
	public Area area(Environmental host) {
		Area A = (host instanceof Area) ? (Area) host : CMLib.map()
				.roomLocation(host).getArea();
		return A;
	}
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		...
		if ((host instanceof Area) && (R != null) && (R.getArea() != host))
			return true;
		...
		if ((msg.targetMinor() == CMMsg.TYP_WEAPONATTACK)
				...
				&& ((msg.sourceMinor() == CMMsg.TYP_THROW) || ((msg.tool() instanceof Weapon) && ((((Weapon) msg
						.tool()).weaponClassification() == Weapon.CLASS_RANGED) || (((Weapon) msg
		...
		if (((msg.sourceMajor(CMMsg.MASK_MOVE))) && (R != null)) {
			...
			if ((what != null)
					...
					&& (CMLib.dice().rollPercentage() > ((msg.source()
							.charStats().getStat(CharStats.STAT_DEXTERITY) * 3) + 25))) {
				...
				msg.source().basePhyStats()
						.setDisposition(oldDisposition | PhyStats.IS_SITTING);
				msg.source().recoverPhyStats();
				R.show(msg.source(), null, CMMsg.MSG_OK_ACTION,
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		// handle freeze overs
		if ((coldWeather(lastWeather)) && (coldWeather(C.weatherType(null)))
		...
		if ((botherDown--) == 1) {
			...
			case Climate.WEATHER_THUNDERSTORM:
			case Climate.WEATHER_RAIN:
				Enumeration<Room> r = (ticking instanceof Room) ? new SingleEnumeration<Room>(
						(Room) ticking) : A.getProperMap();
				for (; r.hasMoreElements();) {
					...
					if (CMLib.map().hasASky(R))
						for (int i = 0; i < R.numInhabitants(); i++) {
							...
							if ((mob != null)
									&& (!mob.isMonster())
									&& (CMLib.flags().aliveAwakeMobile(mob,
											true))
									&& (CMath.bset(mob.getBitmap(),
											MOB.ATT_AUTOWEATHER)))
			...
			for (Session S : CMLib.sessions().localOnlineIterable()) {
				...
				if ((R.getClimateType() & Places.CLIMASK_COLD) > 0) {
					if (coldChance > 0)
						coldChance += 10;
					if (coldChance > 0)
						fluChance += 5; // yes, cold is related this way to flu
					if (frostBiteChance > 0)
						frostBiteChance = frostBiteChance
								+ (int) Math.round(CMath.mul(frostBiteChance,
										0.5));
				}
				...
				int save = (M.charStats().getSave(CharStats.STAT_SAVE_COLD) + M
						.charStats().getSave(CharStats.STAT_SAVE_WATER)) / 2;
				if ((CMLib.dice().rollPercentage() < (coldChance - save))
					...
					for (int i = 0; i < M.numItems(); i++) {
						...
						if (I.amWearingAt(Wearable.WORN_ABOUT_BODY))
							coveredPlaces = coveredPlaces | Wearable.WORN_TORSO
									| Wearable.WORN_LEGS;
						for (int l = 0; l < ALL_COVERED_SPOTS.length; l++)
					...
					if ((coveredPlaces != ALL_COVERED_CODE)
						...
						if (CMLib.dice().rollPercentage() < (fluChance + (((M
								.location().getClimateType() & Places.CLIMASK_WET) > 0) ? 10
								: 0)))
							COLD = CMClass.getAbility("Disease_Flu");
						if ((COLD != null)
								&& (M.fetchEffect(COLD.ID()) == null))
				...
				if ((CMLib.dice().rollPercentage() < (frostBiteChance - save))
					...
					for (int l = 0; l < ALL_FROST_SPOTS.length; l++)
						if (M.getWearPositions(ALL_FROST_SPOTS[l]) == 0)
							unfrostedPlaces = unfrostedPlaces
									| ALL_FROST_SPOTS[l];
					Item I = null;
					for (int i = 0; i < M.numItems(); i++) {
						I = M.getItem(i);
				...
				if ((heatExhaustionChance > 0)
						&& (CMLib.dice().rollPercentage() < (heatExhaustionChance - M
								.charStats().getSave(CharStats.STAT_SAVE_FIRE)))
						&& (C.weatherType(M.location()) != Climate.WEATHER_CLEAR)
						&& (!CMSecurity
								.isDisabled(CMSecurity.DisFlag.AUTODISEASE))) {
				...
				if (R != null) {
					...
					case Climate.WEATHER_THUNDERSTORM: {
						if (C.weatherType(R) != Climate.WEATHER_THUNDERSTORM) {
							if ((R.domainType() & Room.INDOORS) > 0) {
								if ((R.getArea() != null)
										&& CMath.div(
												R.getArea().getAreaIStats()[Area.Stats.INDOOR_ROOMS
														.ordinal()], R
														.getArea().properSize()) < 0.90)
									S.mob().tell(
							...
							S.mob()
									.tell("^JA thunderous rumble and CRACK of lightning can be heard as the pounding rain soaks you.^?"
											+ CMLib.protocol().msp(
													"thunderandrain.wav", 40));
						else
							S.mob()
									.tell("^JA bolt of lightning streaks across the sky as the pounding rain soaks you!^?"
											+ CMLib.protocol().msp(
													"thunderandrain.wav", 40));
						break;
					}
					...
					case Climate.WEATHER_DUSTSTORM:
						if (C.weatherType(R) == Climate.WEATHER_DUSTSTORM)
							S.mob().tell(
									"^JSwirling clouds of dust assault you.^?"
											+ CMLib.protocol().msp("windy.wav",
													40));
						break;
					case Climate.WEATHER_HAIL:
						if (C.weatherType(R) == Climate.WEATHER_HAIL)
							S.mob().tell(
									"^JYou are being pelleted by hail! Ouch!^?"
											+ CMLib.protocol().msp("hail.wav",
													40));
						break;
					case Climate.WEATHER_RAIN:
						if (C.weatherType(R) == Climate.WEATHER_RAIN)
							S.mob().tell(
									"^JThe rain is soaking you!^?"
											+ CMLib.protocol().msp(
													"rainlong.wav", 40));
						break;
					case Climate.WEATHER_SLEET:
						if (C.weatherType(R) == Climate.WEATHER_SLEET)
							S.mob().tell(
									"^JCold and blistering sleet is soaking you numb!^?"
											+ CMLib.protocol().msp("rain.wav",
													40));
						break;
					case Climate.WEATHER_WINDY:
						if (C.weatherType(R) == Climate.WEATHER_WINDY)
							S.mob().tell(
									"^JThe wind gusts around you.^?"
											+ CMLib.protocol().msp("wind.wav",
													40));
						break;
					}
				}
		...
		if ((lightningDown--) == 1) {
			...
			if (C.weatherType(null) == Climate.WEATHER_THUNDERSTORM) {
				...
				if (playerAround) {
					...
					if ((R != null) && (M != null)) {
						Ability A2 = CMClass
								.getAbility("Chant_SummonLightning");
						if (A2 != null) {
							A2.setMiscText("RENDER MUNDANE");
							A2.invoke(M, M, true, M.phyStats().level());
						}
						Room R2 = null;
						Enumeration<Room> e = (ticking instanceof Room) ? new SingleEnumeration<Room>(
								(Room) ticking) : A.getProperMap();
		...
		if ((dustDown--) == 1) {
			...
			if (C.weatherType(null) == Climate.WEATHER_DUSTSTORM) {
				...
				if (choices.size() > 0) {
					R = (Room) choices.elementAt(CMLib.dice().roll(1,
							choices.size(), -1));
					...
					case RawMaterial.MATERIAL_WOODEN: {
						...
						R.showHappens(
								...
								I.Name()
										...
										+ CMLib.protocol().msp("fire.wav", 40));
				...
				for (Session S : CMLib.sessions().localOnlineIterable()) {
					if ((S.mob() == null)
							...
							|| ((C.weatherType(S.mob().location()) != Climate.WEATHER_WINDY)
									&& (C.weatherType(S.mob().location()) != Climate.WEATHER_BLIZZARD) && (C
									.weatherType(S.mob().location()) != Climate.WEATHER_DUSTSTORM)))
						continue;
					R = S.mob().location();
					if ((R != null) && (!choices.contains(R)))
						choices.addElement(R);
			...
			for (Session S : CMLib.sessions().localOnlineIterable()) {
				...
				if (CMLib.dice().rollPercentage() < rustChance) {
					...
					for (int i = 0; i < rustThese.size(); i++) {
						...
						CMLib.combat().postItemDamage(
								M,
								I,
								null,
								1,
								CMMsg.TYP_WATER,
								(weatherType != 0) ? "<T-NAME> rusts."
										: "<T-NAME> rusts in the water.");
					}
				}
			}
		}
