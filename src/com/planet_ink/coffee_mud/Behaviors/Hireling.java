...
public class Hireling extends StdBehavior {
	...
	protected double gamehours() {
		double d = CMath.div((minutes() * 60L * 1000L),
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (System.currentTimeMillis() > onTheJobUntil) {
			...
			int additional = 0;
			if (D != null)
				additional += (int) Math.round(CMath.mul(
						CMath.div(minutes(), price()), D.doubleValue()));
			if (additional <= 0) {
				if (talkTo != null)
					CMLib.commands().postSay(observer, talkTo,
							"Your time is up.  Goodbye!", true, false);
				allDone(observer);
			} else {
				if (talkTo != null)
					CMLib.commands().postSay(
							observer,
							talkTo,
							"Your base time is up, but you've paid for "
									+ additional
									+ " more minutes, so I'll hang around.",
							true, false);
				onTheJobUntil += (additional * TimeManager.MILI_MINUTE);
			}
		} else if ((workingFor.length() > 0) && (observer.location() != null)
	...
	public boolean okMessage(Environmental affecting, CMMsg msg) {
		...
		if (affecting instanceof MOB) {
			...
			if (msg.amITarget(observer) && (!msg.amISource(observer))
				...
				if (!CMLib.masking().maskCheck(zapper(), source, false)) {
					CMLib.commands().postSay(observer, null,
							"I wouldn't work for the likes of you.", false,
							false);
					return false;
				}
			...
			if ((observer.soulMate() == null)
					...
					&& ((msg.targetMinor() == CMMsg.TYP_GIVE) || (msg
							.targetMinor() == CMMsg.TYP_DROP))
					&& ((msg.target() instanceof Coins) || (msg.tool() instanceof Coins))) {
				if ((msg.target() instanceof MOB)
						&& (!CMSecurity.isAllowed(((MOB) msg.target()),
								source.location(), CMSecurity.SecFlag.CMDROOMS)))
					CMLib.commands().postSay(observer, null,
							"I don't think so.", false, false);
				return false;
			}
		}
		return true;
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_QUIT)
				&& (msg.amISource(observer) || msg.amISource(observer
						.amFollowing())))
			allDone(observer);
		else if ((msg.sourceMinor() == CMMsg.TYP_SPEAK)
				&& (!msg.amISource(observer)) && (!msg.source().isMonster())) {
			final String upperSrcMsg = msg.sourceMessage() == null ? "" : msg
					.sourceMessage().toUpperCase();
			if (((upperSrcMsg.indexOf(" HIRE") > 0)
					|| (upperSrcMsg.indexOf("'HIRE") > 0)
					|| (upperSrcMsg.indexOf("WORK") > 0) || (upperSrcMsg
					.indexOf("AVAILABLE") > 0)) && (onTheJobUntil == 0))
				CMLib.commands().postSay(
						observer,
						null,
						"I'm for hire.  Just give me "
								+ CMLib.beanCounter().nameCurrencyShort(
										observer, price())
								+ " and I'll work for you for " + gamehours()
								+ " \"hours\".", false, false);
			else if (((upperSrcMsg.indexOf(" FIRED") > 0))
					&& ((workingFor != null) && (msg.source().Name()
							.equals(workingFor))) && (msg.amITarget(observer))
					&& (onTheJobUntil != 0)) {
				CMLib.commands().postSay(observer, msg.source(),
						"Suit yourself.  Goodbye.", false, false);
				allDone(observer);
			} else if (((upperSrcMsg.indexOf(" SKILLS") > 0))) {
				StringBuffer skills = new StringBuffer("");
				for (final Enumeration<Ability> a = observer.allAbilities(); a
						.hasMoreElements();) {
					final Ability A = a.nextElement();
					if (A != null) {
						if (A.proficiency() == 0)
							A.setProficiency(50
									+ observer.phyStats().level()
				...
				if (skills.length() > 2)
					CMLib.commands().postSay(observer, source,
							"My skills include: " + skills.substring(2) + ".",
							false, false);
			}
			...
			if (partials.get(msg.source().Name()) != null) {
				given += ((Double) partials.get(msg.source().Name()))
						.doubleValue();
					...
					if (workingFor.equals(source.Name()))
						CMLib.commands()
								.postSay(
										observer,
										source,
										"I'm still working for you.  I'll put that towards an extension though.",
										true, false);
					else
						CMLib.commands().postSay(
								observer,
								source,
								"Sorry, I'm on the job right now.  Give me "
										+ CMLib.beanCounter()
												.nameCurrencyShort(observer,
														(price() - given))
										+ " more later on and I'll work for "
										+ gamehours() + " \"hours\".", true,
								false);
				} else
					CMLib.commands().postSay(
							observer,
							...
							"My price is "
									...
									+ ".  Give me "
									+ CMLib.beanCounter().nameCurrencyShort(
											observer, (price() - given))
									+ " more and I'll work for you for "
									+ gamehours() + " \"hours\".", true, false);
				...
				if (onTheJobUntil != 0) {
					if (workingFor.equals(source.Name()))
						...
						CMLib.commands()
								.postSay(
										observer,
										source,
										"Sorry, I'm on the job right now.  Give me 1 more coin later on and I'll work.",
										true, false);
					partials.put(msg.source().Name(), Double.valueOf(given));
				} else {
					if (given > price())
						partials.put(msg.source().Name(),
								Double.valueOf(given - price()));
					StringBuffer skills = new StringBuffer("");
					for (Enumeration<Ability> a = observer.allAbilities(); a
							.hasMoreElements();) {
					...
					CMLib.commands()
							.postSay(
									observer,
									source,
									"Ok.  You've got me for at least "
											+ gamehours()
											+ " \"hours\".  My skills include: "
											+ skills.substring(2)
											+ ".  I'll follow you.  Just ORDER me to do what you want.",
									true, false);
				}
			}
		}
