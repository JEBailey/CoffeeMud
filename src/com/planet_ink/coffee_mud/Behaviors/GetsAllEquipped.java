...
public class GetsAllEquipped extends ActiveTicker {
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		...
		if ((msg.sourceMinor() == CMMsg.TYP_DEATH) && (msg.source() != host)
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((canAct(ticking, tickID)) && (ticking instanceof MOB)) {
			...
			mob.enqueCommand(new XVector("GET", "ALL"),
				...
				if ((I != null) && (!stuffIHad.contains(I))) {
					if (I instanceof DeadBody)
						dropThisStuff.addElement(I);
							...
							&& (I.container() instanceof DeadBody))
						I.setContainer(null);
				}
			}
			for (int d = 0; d < dropThisStuff.size(); d++)
				mob.enqueCommand(new XVector("DROP", "$"
						+ ((Item) dropThisStuff.elementAt(d)).Name() + "$"),
						Command.METAFLAG_FORCED, 0);
			mob.enqueCommand(new XVector("WEAR", "ALL"),
					Command.METAFLAG_FORCED, 0);
		}
		return true;
	}
