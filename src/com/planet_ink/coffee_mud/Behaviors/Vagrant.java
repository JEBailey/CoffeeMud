...
public class Vagrant extends StdBehavior {
	...
	public boolean okMessage(Environmental oking, CMMsg msg) {
		...
		if (msg.amITarget(mob)
				&& (((msg.sourceMajor() & CMMsg.MASK_MOVE) > 0) || ((msg
						.sourceMajor() & CMMsg.MASK_HANDS) > 0))) {
		...
		if ((wakeForTicks <= 0) && (sleepForTicks <= 0)) {
			if ((CMLib.dice().rollPercentage() > 50) || (mob.isInCombat())) {
				...
				if (CMLib.flags().aliveAwakeMobile(mob, true))
					mob.location()
							.show(mob, mob.location(), CMMsg.MSG_SLEEP,
									"<S-NAME> curl(s) on the ground and go(es) to sleep.");
