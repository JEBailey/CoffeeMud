...
public class CombatAbilities extends StdBehavior {
	...
	public final static String[] names = { "RANDOM", "DEFENSIVE", "OFFENSIVE",
			"MIXEDOFFENSIVE", "MIXEDDEFENSIVE", "ONLYALWAYS" };
	...
	protected void makeClass(MOB mob, String theParms, String defaultClassName) {
		...
		if (theParms.trim().length() == 0) {
			...
			if ((mob.baseCharStats().getCurrentClass() != C) && (C != null)
		...
		for (int i = 0; i < classes.size(); i++) {
			...
			mob.baseCharStats().setCurrentClass(C);
			mob.baseCharStats().setClassLevel(C,
					mob.basePhyStats().level() / classes.size());
		}
		mob.recoverCharStats();
	}
	...
	protected void newCharacter(MOB mob) {
		...
		for (Enumeration<Ability> a = mob.allAbilities(); a.hasMoreElements();) {
			...
			if (A != null) {
				int proficiency = CMLib.ableMapper().getMaxProficiency(mob,
						true, A.ID()) / 2;
		...
		for (int a = 0; a < mob.numAllAbilities(); a++) {
			...
			if ((newOne != null) && (!oldAbilities.contains(newOne))) {
				if (!CMLib.ableMapper().qualifiesByLevel(mob, newOne)) {
					...
					final int newProf = CMLib.ableMapper().getMaxProficiency(
							newOne.ID());
	...
	public void setCombatStats(MOB mob, int attack, int armor, int damage,
			int hp, int mana, int move) {
		...
		if (A == null) {
			...
			if (A != null) {
				...
				for (int stat : CharStats.CODES.BASE())
					if (stats[stat] != 0)
						parm.append(
								CMStrings.limit(CharStats.CODES.NAME(stat), 3))
								.append("=").append(numPoints).append(" ");
				if (parm.length() > 0) {
	...
	public void adjustAggro(MOB hostM, MOB attackerM, int amt) {
		...
		synchronized (aggro) {
			...
			if ((curVictim == attackerM) || (curVictim == null)
					|| (!aggro.containsKey(curVictim)))
				return;
			int vicAmt = aggro.get(curVictim)[0];
			if ((I[0] > (vicAmt * 1.5))
					&& (I[0] > hostM.maxState().getHitPoints() / 10)
					&& (!attackerM.amDead()) && (attackerM.isInCombat())) {
				if ((hostM.getGroupMembers(new HashSet<MOB>())
						.contains(attackerM))
				...
				if (victim == null) {
					...
					if (msg.target() == host)
						...
						if ((victim == msg.source())
								|| (msg.source().getGroupMembers(
										new HashSet<MOB>()).contains(victim)))
							adjustAggro(mob, msg.source(), msg.value());
					}
				} else if ((msg.targetMinor() == CMMsg.TYP_HEALING)
						&& (msg.value() > 0) && (msg.source() != mob)
						&& (msg.target() != mob)) {
					if ((msg.target() == victim)
						...
						&& (CMath.bset(((Ability) msg.tool()).flags(),
								Ability.FLAG_AGGROFYING)))
					adjustAggro(mob, msg.source(),
							((Ability) msg.tool()).adjustedLevel(msg.source(),
									0) * 2);
					...
					if ((msg.source() == victim)
							|| (msg.source()
									.getGroupMembers(new HashSet<MOB>())
									.contains(victim))) {
						int level = CMLib.ableMapper().qualifyingLevel(
								msg.source(), (Ability) msg.tool());
						if (level <= 0)
							level = CMLib.ableMapper().lowestQualifyingLevel(
									msg.tool().ID());
						if (level > 0)
	...
	public void startBehavior(PhysicalAgent forMe) {
		...
		for (int v = 0; v < V.size(); v++) {
			...
			if (s.equalsIgnoreCase("proficient"))
				proficient = true;
			else if (s.equalsIgnoreCase("nostat")
					|| s.equalsIgnoreCase("nostats"))
				noStat = true;
	...
	protected Ability useSkill(MOB mob, MOB victim, MOB leader)
			throws CMException {
		int tries = 0;
		Ability tryA = null;
		...
		while ((tryA == null) && ((++tries) < 100)
				&& (mob.numAllAbilities() > 0)) {
			if ((combatMode == COMBAT_ONLYALWAYS)
					&& (this.skillsAlways != null)
					&& (this.skillsAlways.size() > 0))
				A = mob.fetchAbility(skillsAlways.get(CMLib.dice().roll(1,
						skillsAlways.size(), -1)));
			else
			...
			if ((A == null)
					...
					|| ((skillsAlways != null) && (!skillsAlways.contains(A
							.ID())))
					|| ((skillsNever != null) && (skillsNever.contains(A.ID()))))
				continue;

			victimQuality = (victim != null) ? A.castingQuality(mob, victim)
					: Ability.QUALITY_INDIFFERENT;
			selfQuality = A.castingQuality(mob, mob);
			leaderQuality = ((mob == leader) || (leader == null)) ? Ability.QUALITY_INDIFFERENT
					: A.castingQuality(mob, leader);
			...
			if (selfQuality == Ability.QUALITY_BENEFICIAL_SELF)
				...
				target = ((leader == null) || (mob.location() != leader
						.location())) ? mob : leader;
			if ((target != null) && (tryA != null)
					&& (target.fetchEffect(tryA.ID()) != null))
				tryA = null;
		}

		if (tryA != null) {
			if (CMath.bset(tryA.usageType(), Ability.USAGE_MANA)) {
				if ((Math.random() > CMath.div(mob.curState().getMana(), mob
						.maxState().getMana()))
						|| (mob.curState().getMana() < tryA.usageCost(mob,
								false)[0])) {
					...
					mob.curState().adjMana(tryA.usageCost(mob, false)[0],
							mob.maxState());
				}
				mob.curState().adjMana(5, mob.maxState());
			}
			if (CMath.bset(tryA.usageType(), Ability.USAGE_MOVEMENT)) {
				if ((Math.random() > CMath.div(mob.curState().getMovement(),
						mob.maxState().getMovement()))
						|| (mob.curState().getMovement() < tryA.usageCost(mob,
								false)[1]))
			...
			if (CMath.bset(tryA.usageType(), Ability.USAGE_HITPOINTS)) {
				if ((Math.random() > CMath.div(mob.curState().getHitPoints(),
						mob.maxState().getHitPoints()))
						|| (mob.curState().getHitPoints() < tryA.usageCost(mob,
								false)[2]))
			...
			if (target == null)
				return null;
			boolean skillUsed = tryA.invoke(mob, new XVector(target.name()),
					null, false, 0);
			if ((combatMode == COMBAT_ONLYALWAYS) && (!skillUsed)) {
				int retries = 0;
				while ((++retries < 10) && (!skillUsed))
					skillUsed = tryA.invoke(mob, new XVector(target.name()),
							null, false, 0);
			}
			if (skillUsed) {
				skillUsed = true;
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((weaponSet.wand != null)
				&& ((weaponSet.wand.owner() != mob)
						|| (weaponSet.wand.amDestroyed()) || (weaponSet.wand
							.amWearingAt(Wearable.IN_INVENTORY))))
			rebuildSet = true;
		if ((weaponSet.offHandWand != null)
				&& ((weaponSet.offHandWand.owner() != mob)
						|| (weaponSet.offHandWand.amDestroyed()) || (!weaponSet.offHandWand
							.amWearingAt(Wearable.IN_INVENTORY))))
			rebuildSet = true;
		if (rebuildSet) {
			...
			if ((weaponSet.weapon == null) && ((--chkDown) <= 0)) {
				if ((newWeapon == null) && (R != null)) {
					...
					for (final Enumeration<Item> i = R.items(); i
							.hasMoreElements();) {
						final Item I = i.nextElement();
						if ((!(I instanceof Weapon))
								...
								|| (I.phyStats().level() > mob.phyStats()
										.level()))
							continue;
						choices.addElement(I);
					}
					final Item I = (choices.size() == 0) ? null
							: (Item) choices.elementAt(CMLib.dice().roll(1,
									choices.size(), -1));
					if (I != null) {
						CMLib.commands().forceStandardCommand(mob, "GET",
								new XVector("GET", I.Name()));
						if (mob.isMine(I))
							newWeapon = I;
					}
				}
				if (newWeapon != null) {
					CMLib.commands().forceStandardCommand(mob, "WIELD",
							new XVector("WIELD", newWeapon.Name()));
				}
				chkDown = 5;
			}
		}
		...
		if (aggro != null) {
			synchronized (aggro) {
				...
				if ((winAmt > minAmt)
						...
						&& (!winMOB.amDead())
						&& (winMOB.isInCombat())
						&& (!mob.getGroupMembers(new HashSet<MOB>()).contains(
								winMOB))) {
		...
		if ((!skillUsed)
			...
			if ((weaponSet.wand == null) && (weaponSet.offHandWand != null)
					&& (weaponSet.offHandWand.canWear(mob, Wearable.WORN_HELD))) {
				Vector V = new Vector();
				V.addElement("hold");
				...
				if (A != null) {
					...
					if (A.castingQuality(mob, mob) == Ability.QUALITY_BENEFICIAL_SELF)
						...
						target = victim;
					else if (((mob != leader) && (leader != null))
							&& (A.castingQuality(mob, leader) == Ability.QUALITY_BENEFICIAL_OTHERS))
						target = ((mob.location() != leader.location())) ? mob
								: leader;
					else
						target = null;
					if (target != null) {
	...
	public String getStat(String code) {
		...
		case 5:
			return (skillsAlways == null) ? "" : CMParms
					.toSemicolonList(skillsAlways);
		case 6:
			return (skillsAlways == null) ? "" : CMParms
					.toSemicolonList(skillsNever);
		case 7:
