...
public class MOBEater extends ActiveTicker {
	...
	public void startBehavior(PhysicalAgent forMe) {
		...
		if ((forMe != null) && (forMe instanceof MOB)) {
			...
			Stomach.setDescription("You are in the stomach of "
					+ forMe.name()
			...
			Stomach.addNonUninvokableEffect(CMClass
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((canAct(ticking, tickID)) && (((MOB) ticking).isInCombat())
				&& (!mob.amDead()))
			trySwallowWhole(mob);
		return true;
	}
	...
	public void executeMsg(Environmental mob, CMMsg msg) {
		if ((mob instanceof MOB) && (msg.amISource((MOB) mob))
	...
	protected boolean trySwallowWhole(MOB mob) {
		...
		if (CMLib.flags().aliveAwakeMobile(mob, true)
				&& (mob.rangeToTarget() == 0)
				&& (CMLib.flags().canHear(mob) || CMLib.flags().canSee(mob) || CMLib
						.flags().canSmell(mob))) {
			...
			if (TastyMorsel.baseWeight() < (mob.phyStats().weight() / 2)) {
				...
				CMMsg EatMsg = CMClass.getMsg(mob, TastyMorsel, null,
						CMMsg.MSG_EAT, CMMsg.MASK_ALWAYS | CMMsg.TYP_JUSTICE,
					...
					if (EatMsg.value() == 0) {
						...
						CMMsg enterMsg = CMClass
								.getMsg(TastyMorsel, Stomach, null,
										CMMsg.MSG_ENTER, Stomach.description(),
	...
	protected boolean digestTastyMorsels(MOB mob) {
		...
		for (int x = 0; x < morselCount; x++) {
			...
			if (TastyMorsel != null) {
				...
				int damage = (int) Math.round(TastyMorsel.curState()
						.getHitPoints() * CMath.div(pctAcidHp, 100));
