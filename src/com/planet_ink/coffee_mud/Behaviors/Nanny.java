...
public class Nanny extends StdBehavior {
	...
	public double getAllOwedBy(MOB mob) {
		...
		for (DropOff D : dropOffs)
			if (D.mommyM == mob) {
				...
				t = Math.round(Math.ceil(CMath.div(t,
	...
	public String getOwedFor(String currency, PhysicalAgent P) {
		for (DropOff D : dropOffs)
			if (D.baby == P) {
				long t = System.currentTimeMillis() - D.dropOffTime;
				t = Math.round(Math.floor(CMath.div(t,
						CMProps.getMillisPerMudHour())));
				if (t > 0)
					return CMLib.beanCounter().abbreviatedPrice(currency,
							(t + hourlyRate))
							+ " for watching " + P.name();
			}
		return "";
		...
		for (DropOff D : dropOffs)
			if (D.mommyM == mob) {
				...
				t = Math.round(Math.ceil(CMath.div(t,
						CMProps.getMillisPerMudHour())));
				if (t > 0)
					owed.append(CMLib.beanCounter().abbreviatedPrice(currency,
							(t * hourlyRate))
							+ " for " + D.baby.name() + ", ");
			}
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		...
		if ((msg.target() == host) && (targMinor == CMMsg.TYP_GIVE)) {
			if (isDropOffable(msg.tool())) {
				...
				msg.source().tell(
						...
						host,
						msg.tool(),
						"<T-NAME> won't accept <O-NAME>.  You should probably leave your "
								+ pronoun + " here.");
				return false;
			}
					...
					if (host instanceof MOB)
						...
						msg.source()
								.tell("The "
										...
										+ ".  It only accepts "
										+ CMLib.beanCounter()
												.getDenominationName(myCurrency)
										+ ".");
					return false;
				}
			}
			...
			if ((msg.source().location() == CMLib.map().roomLocation(host))
				...
				if (amt.length() > 0)
					CMLib.commands().postSay(
							(MOB) host,
							msg.source(),
							"I'm afraid that " + place + " fees of " + amt
									+ " are still owed.");
				else
					CMLib.commands().postSay(
							(MOB) host,
							msg.source(),
							"I'm afraid that " + place
									+ " fees are still owed on " + obj.name()
									+ ".");
			} else if (amt.length() > 0)
				msg.source().tell(
						"You'll need to pay " + place + " fees of " + amt
								+ " first.");
			else
				msg.source().tell(
						"You'll need to pay your " + place + " fees  for "
								+ obj.name() + " first.");
			return false;
		} else if ((CMath.bset(msg.sourceMajor(), CMMsg.MASK_MALICIOUS) || CMath
				.bset(msg.targetMajor(), CMMsg.MASK_MALICIOUS))
				&& (msg.target() instanceof PhysicalAgent)
				&& ((getDroppedOffObjIfAny((PhysicalAgent) msg.target()) != null)
						|| (msg.target() == host) || (msg.target() == CMLib
						.map().roomLocation(host)))) {
			if (msg.source() != host) {
				if ((host instanceof MOB)
						&& (msg.source().location() == CMLib.map()
								.roomLocation(host))) {
					CMLib.commands().postSay((MOB) host, msg.source(),
							"Not in my " + place + " you dont!");
			...
			if (obj != null) {
				if ((msg.tool() instanceof Ability)
					...
					boolean teleport = CMath.bset(
							((Ability) msg.tool()).flags(),
					...
					String amt = getOwedFor(
							CMLib.beanCounter().getCurrency(host), obj);
					if ((msg.source().location() == CMLib.map().roomLocation(
							host))
							&& (host instanceof MOB)) {
						if (amt.length() > 0)
							CMLib.commands().postSay(
									(MOB) host,
									msg.source(),
									"I'm afraid that " + place + " fees of "
											+ amt + " are still owed on "
											+ obj.name() + ".");
						else
							CMLib.commands().postSay(
									(MOB) host,
									msg.source(),
									"I'm afraid that " + place
											+ " fees are still owed on "
											+ obj.name() + ".");
					} else if (amt.length() > 0)
						msg.source().tell(
								"You'll need to pay " + place + " fees of "
										+ amt + " for " + obj.name()
										+ " first.");
					else
						msg.source().tell(
								"You'll need to pay your " + place
										+ " fees  for " + obj.name()
										+ " first.");
				}
				return false;
	...
	public boolean isMount(Environmental E) {
		if ((E instanceof MOB)
				...
				&& ((((Rideable) E).rideBasis() == Rideable.RIDEABLE_LAND)
						|| (((Rideable) E).rideBasis() == Rideable.RIDEABLE_AIR) || (((Rideable) E)
	...
	public boolean isDropOffable(Environmental E) {
		...
		if ((watchesBabies) && (CMLib.flags().isBaby(E)))
			return true;
		if ((watchesChildren) && (CMLib.flags().isChild(E))
				&& (!CMLib.flags().isBaby(E)))
			return true;
		if ((watchesMounts) && (isMount(E)))
			return true;
		if ((watchesMOBFollowers) && (E instanceof MOB) && (!isMount(E))
				&& (!CMLib.flags().isChild(E)) && (!CMLib.flags().isBaby(E)))
			return true;
		if ((this.watchesWagons) && (E instanceof Rideable)
				&& (((Rideable) E).rideBasis() == Rideable.RIDEABLE_WAGON))
			return true;
		if ((this.watchesCars) && (E instanceof Item)
				&& (E instanceof Rideable)
				&& (((Rideable) E).rideBasis() == Rideable.RIDEABLE_LAND))
			return true;
		if ((this.watchesBoats) && (E instanceof Item)
				&& (E instanceof Rideable)
				&& (((Rideable) E).rideBasis() == Rideable.RIDEABLE_WATER))
			return true;
		if ((this.watchesAirCars) && (E instanceof Item)
				&& (E instanceof Rideable)
	...
	public MOB getMommyOf(Physical P) {
		if ((P instanceof Item) && (((Item) P).owner() instanceof MOB)
				&& (!((MOB) ((Item) P).owner()).isMonster()))
			return (MOB) ((Item) P).owner();
		if ((P instanceof MOB) && (((MOB) P).amFollowing() != null)
				&& (!((MOB) P).amFollowing().isMonster()))
			return ((MOB) P).amFollowing();
		if ((P instanceof MOB) && (ultimateFollowing(P) != null)
		...
		if (P instanceof Rideable) {
			...
			for (int r = 0; r < R.numRiders(); r++) {
				E2 = R.fetchRider(r);
				if ((E2 instanceof MOB) && (!((MOB) E2).isMonster()))
					return (MOB) E2;
			}
		}
		...
		for (Object o : H)
			if (o instanceof PhysicalAgent) {
				...
				if ((isDropOffable(P)) && (!isAssociated(P))) {
					...
					if (source != null)
						associations.add(new DropOff(source, P, System
								.currentTimeMillis()));
				}
				if (P instanceof MOB) {
					MOB mob = (MOB) P;
					for (int t = 0; t < mob.numItems(); t++) {
						...
						if (isDropOffable(I) && (!isAssociated(I))) {
							...
							if (source != null)
								associations.add(new DropOff(source, I, System
										.currentTimeMillis()));
						}
					}
				}
	...
	public void executeMsg(Environmental host, CMMsg msg) {
		...
		if ((msg.targetMinor() == CMMsg.TYP_ENTER)
				...
				sayLaters
						.addElement(
								...
								"Welcome to my "
										...
										+ list.toString()
										+ " here under my care and protection.  Be aware that I charge "
										+ CMLib.beanCounter().abbreviatedPrice(
												currency, hourlyRate)
										+ " per hour, each.  "
										+ "No payment is due until you return to fetch your "
										+ getPronoun(myAssocs) + ".");
			...
			if (owed > 0) {
				...
				sayLaters
						.addElement(
								...
								"Welcome back, "
										+ msg.source().name()
										+ "! If are here for your "
										+ pronoun
										+ ", the total bill is: "
										+ getAllOwedBy(currency, msg.source())
										+ " ("
										+ CMLib.beanCounter().abbreviatedPrice(
												currency, owed - paid)
										+ "). "
			...
			if ((paid > owed) && (host instanceof MOB)) {
				...
				Coins C = CMLib.beanCounter()
						.makeBestCurrency(currency, change);
				MOB source = msg.source();
				if ((change > 0.0) && (C != null)) {
					// this message will actually end up triggering the
					// hand-over.
					CMMsg newMsg = CMClass
							.getMsg((MOB) host, source, C, CMMsg.MSG_SPEAK,
									"^T<S-NAME> say(s) 'Heres your change.' to <T-NAMESELF>.^?");
					C.setOwner((MOB) host);
					...
					CMLib.commands().postSay((MOB) host, source,
							"Gee, thanks. :)", true, false);
			}
			...
			if (paid >= owed) {
				...
				for (int v = 0; v < V.size(); v++) {
					P = V.get(v);
					if (P instanceof MOB) {
						CMLib.commands().postFollow((MOB) P, msg.source(),
								false);
						if (CMath
								.bset(((MOB) P).getBitmap(), MOB.ATT_AUTOGUARD))
							((MOB) P).setBitmap(CMath.unsetb(
									((MOB) P).getBitmap(), MOB.ATT_AUTOGUARD));
						if (((MOB) P).amFollowing() != msg.source()) {
							CMLib.commands()
									.postSay(
											...
											"Hmm, '"
													+ P.name()
													+ "' doesn't seem ready to leave.  Now get along!",
											true, false);
							msg.source()
									.location()
									.send((MOB) P,
				...
				sayLaters.addElement(
						msg.source(),
						"Thanks, but you still owe "
								+ CMLib.beanCounter().abbreviatedPrice(
										currency, owed - paid) + ".");
			...
			CMLib.beanCounter().giveSomeoneMoney(msg.source(),
					(MOB) msg.target(), ((Coins) msg.tool()).getTotalValue());
	}
	...
	public String getParms() {
		...
		if (dropOffs != null) {
			...
			for (DropOff D : dropOffs) {
				...
				parms.append(CMLib.xml().convertXMLtoTag("ENAM",
						CMLib.xml().parseOutAngleBrackets(eName)));
				parms.append(CMLib.xml().convertXMLtoTag("ONAM",
						CMLib.xml().parseOutAngleBrackets(oName)));
	...
	public void setParms(String parms) {
		...
		List<String> watches = CMParms.parseCommas(
				CMParms.getParmStr(parms, "WATCHES", "Babies,Children")
						.toUpperCase(), true);
		String watch = null;
		watchesBabies = false;
	...
	public boolean tick(Tickable ticking, int tickID) {
					...
					if ((V != null) && (R != null))
						for (int v = 0; v < V.size(); v++) {
							...
							if ((P != null) && (P.contents != null)
									&& (P.contents.size() == 3)
									&& (P.tag.equalsIgnoreCase("DROP"))) {
								eName = CMLib.xml().restoreAngleBrackets(
										CMLib.xml().getValFromPieces(
												P.contents, "ENAM"));
								oName = CMLib.xml().restoreAngleBrackets(
										CMLib.xml().getValFromPieces(
												P.contents, "ONAM"));
								time = CMLib.xml().getLongFromPieces(
										P.contents, "TIME");
								...
								if (PA == null)
									PA = R.findItem(eName);
								if (PA == null)
									Log.errOut("Nanny", "Unable to find "
											+ eName + " for " + oName + "!!");
								else if (!isDroppedOff(PA))
									dropOffs.add(new DropOff(M, PA, time));
							} else if (P != null)
								Log.errOut("Nanny", "Unable to parse: " + codes
										+ ", specifically: " + P.value);
						}
				}
			}
		...
		for (int s = sayLaters.size() - 1; s >= 0; s--) {
			if (ticking instanceof MOB)
				CMLib.commands().postSay((MOB) ticking,
						(MOB) sayLaters.elementAt(s, 1),
						(String) sayLaters.elementAt(s, 2));
			else
				((MOB) sayLaters.elementAt(s, 1)).tell((String) sayLaters
						.elementAt(s, 2));
		...
		if (!changedSinceLastSave)
			for (DropOff D : dropOffs)
				if ((D.baby instanceof MOB) && (R != null)
						&& (!R.isInhabitant((MOB) D.baby))) {
					dropOffs.remove(D);
					changedSinceLastSave = true;
				}
		for (DropOff D : dropOffs)
			if ((D.baby instanceof Item) && (R != null)
		...
		if ((dropOffs.size() > 0) && (ticking instanceof MOB)
				&& (CMLib.dice().rollPercentage() < 10) && (R != null)) {
			MOB mob = (MOB) ticking;
			PhysicalAgent PA = dropOffs.get(CMLib.dice().roll(1,
					dropOffs.size(), -1)).baby;
			if (CMLib.flags().isBaby(PA)) {
				if (PA.fetchEffect("Soiled") != null) {
					R.show(mob, PA, CMMsg.MSG_NOISYMOVEMENT,
							"<S-NAME> change(s) <T-YOUPOSS> diaper.");
					PA.delEffect(PA.fetchEffect("Soiled"));
				} else if (CMLib.dice().rollPercentage() > 50)
					R.show(mob, PA, CMMsg.MSG_NOISYMOVEMENT,
							"<S-NAME> play(s) with <T-NAME>.");
				else
					R.show(mob, PA, CMMsg.MSG_NOISYMOVEMENT,
							"<S-NAME> go(es) 'coochie-coochie coo' to <T-NAME>.");
				...
				if (PA instanceof MOB) {
					if ((!CMLib.flags().isAnimalIntelligence((MOB) PA))
						...
						R.show(mob, PA, CMMsg.MSG_NOISE,
								"<S-NAME> speak(s) quietly with <T-NAME>.");
					else {
						List<RawMaterial> V = ((MOB) PA).charStats()
								.getMyRace().myResources();
						boolean comb = false;
						if (V != null)
							for (int v = 0; v < V.size(); v++)
								if (((Item) V.get(v)).material() == RawMaterial.RESOURCE_FUR)
									comb = true;
						if (comb)
							R.show(mob, PA, CMMsg.MSG_QUIETMOVEMENT,
									"<S-NAME> groom(s) <T-NAME>.");
						else
							R.show(mob, PA, CMMsg.MSG_QUIETMOVEMENT,
									"<S-NAME> pet(s) <T-NAME>.");
					...
					R.show(mob, PA, CMMsg.MSG_LOCK,
							"<S-NAME> admire(s) <T-NAME>.");
			} else if (PA instanceof MOB) {
				if (CMLib.flags().isAnimalIntelligence((MOB) PA))
					R.show(mob, PA, CMMsg.MSG_QUIETMOVEMENT,
							"<S-NAME> smile(s) and pet(s) <T-NAME>.");
