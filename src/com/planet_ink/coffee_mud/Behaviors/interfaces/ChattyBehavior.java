 ...
 */
/**
 * A ChattyBehavior is a Behavior causes a mob to have a conversation, or even
 * just simply respond to a player or even another mob.
 * 
 * @see com.planet_ink.coffee_mud.Behaviors.interfaces.Behavior
 */
public interface ChattyBehavior extends Behavior {
	/**
	 * Returns the last thing actually spoken by the wielder of this behavior,
	 * or null if nothing has been said yet.
	 * 
	...
	/**
	 * A response object representing something the chatty-one will definitely
	 * be saying soon.
	 * 
	 * @author bzimmerman
	 */
	...
	public static class ChattyResponse {
		public ChattyResponse(Vector cmd, int responseDelay) {
			parsedCommand = cmd;
			delay = responseDelay;
		}
	 ...
	 * someone speaking or acting. It is only one possible response to one
	 * possible event, and is weighed against its neighbors for whether it is
	 * chosen.
	 * 
	 * @author bzimmerman
	 */
	public static class ChattyTestResponse {
		...
		public ChattyTestResponse(String resp) {
			...
			responses = CMParms.parseSquiggleDelimited(resp.substring(1), true)
					.toArray(new String[0]);
		}
	}

	/**
	 * A chatty entry embodies a test for a particular environmental event, such
	 * as someone speaking or acting, and all possible responses to that event.
	 * 
	 * @author bzimmerman
	 */
	...
	/**
	 * A chatty group is a collection of particular environmental event tests,
	 * and their possible responses. It completely embodies a particular
	 * "chat behavior" for a particular kind of chatty mob.
	 * 
	 * @author bzimmerman
	 */
	public static class ChattyGroup implements Cloneable {
		...
		public ChattyGroup(String[] names,
