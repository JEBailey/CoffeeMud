...
public class Follower extends ActiveTicker {
	...
	public boolean okMessage(Environmental host, CMMsg msg) {
		if (!super.okMessage(host, msg))
			return false;
		if ((host instanceof Item) && (msg.tool() == host)
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((ticking instanceof Item)
				&& ((lastOwner == null) || ((!inventory) && (!CMLib.flags()
						.isInTheGame(lastOwner, false))))) {
			Item I = (Item) ticking;
			if ((I.owner() != null)
					&& (I.owner() instanceof MOB)
					&& (CMLib.masking().maskCheck(getParms(), I.owner(), false))
					&& (!CMSecurity.isAllowed((MOB) I.owner(),
							((MOB) I.owner()).location(),
							CMSecurity.SecFlag.CMDMOBS))
					&& (!CMSecurity.isAllowed((MOB) I.owner(),
							((MOB) I.owner()).location(),
							CMSecurity.SecFlag.CMDROOMS)))
		...
		if (ticking instanceof MOB) {
		...
		} else if ((ticking instanceof Item) && (lastOwner != null)
			...
			if (R != lastOwner.location())
				lastOwner.location().moveItemTo(I, ItemPossessor.Expire.Never,
