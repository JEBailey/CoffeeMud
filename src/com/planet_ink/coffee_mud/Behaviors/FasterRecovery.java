...
public class FasterRecovery extends StdBehavior {
	...
	public void doBe(Room room, int burst, int health, int hits, int mana,
			int move) {
		if (room == null)
			return;
	...
	public void doBe(Area area, int burst, int health, int hits, int mana,
			int move) {
		if (area == null)
			return;
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if (ticking instanceof Room)
			...
			if (CMLib.flags().isGettable((Item) ticking)
				...
				doBe((MOB) ((Item) ticking).owner(), burst, health, hits, mana,
						move);
			else if (!CMLib.flags().isGettable((Item) ticking)
					&& (((Item) ticking).owner() instanceof Room))
				doBe((Room) ((Item) ticking).owner(), burst, health, hits,
						mana, move);
		}
		return true;
	}
