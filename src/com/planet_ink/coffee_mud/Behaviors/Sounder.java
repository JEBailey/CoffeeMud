...
public class Sounder extends StdBehavior {
	...
	protected int tickDown = (int) Math.round(Math.random()
			* (maxTicks - minTicks))
			+ minTicks;

	protected int canImproveCode() {
		return Behavior.CAN_ITEMS | Behavior.CAN_MOBS | Behavior.CAN_ROOMS
				| Behavior.CAN_EXITS | Behavior.CAN_AREAS;
	}
	...
	protected void tickReset() {
		tickDown = (int) Math.round(Math.random() * (maxTicks - minTicks))
				+ minTicks;
	}
	...
	public void executeMsg(Environmental E, CMMsg msg) {
		...
		if ((msg != lastMsg)
			...
			case CMMsg.TYP_WEAPONATTACK:
				if ((msg.target() != null)
						&& (msg.target() != E)
						&& ((msg.source() == E) || (msg.tool() == E)
								|| (E instanceof Room) || (E instanceof Exit))
						&& (!oncePerRound1))
		...
		if ((lookFor >= 0)
				...
				&& ((!(E instanceof MOB))
						|| (lookFor == CMMsg.TYP_WEAPONATTACK)
						|| (lookFor == CMMsg.TYP_DAMAGE) || (canFreelyBehaveNormal(E))))
			for (int v = 0; v < triggers.length; v++)
				if (((triggers[v] & UNDER_MASK) == lookFor)
					...
					if (CMath.bset(triggers[v], ROOM_MASK)) {
						CMMsg msg2 = CMClass.getMsg(msg.source(), null, null,
								...
								CMMsg.MSG_EMOTE, CMStrings.replaceAll(
										strings[v], "$p", E.name()));
						msg.addTrailerMsg(msg2);
					} else {
						CMMsg msg2 = CMClass.getMsg(msg.source(), null, null,
								CMMsg.MSG_EMOTE, CMMsg.NO_EFFECT,
								CMMsg.NO_EFFECT, CMStrings.replaceAll(
										strings[v], "$p", E.name()));
						msg.addTrailerMsg(msg2);
					}
				}
