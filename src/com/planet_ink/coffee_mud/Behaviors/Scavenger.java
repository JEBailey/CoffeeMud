...
public class Scavenger extends ActiveTicker {
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((canAct(ticking, tickID)) && (ticking instanceof MOB)) {
			...
			if ((mob.phyStats().weight() >= (int) Math.round(CMath.mul(
					mob.maxCarry(), 0.9)))
					|| (mob.numItems() >= mob.maxItems())) {
				if (CMLib.flags().isATrackingMonster(mob))
					return true;
				String trashRoomID = CMParms
						.getParmStr(getParms(), "TRASH", "");
				...
				if (mob.location() == R) {
					...
					for (int i = 0; i < R.numItems(); i++) {
						Item I = R.getItem(i);
						if ((I instanceof Container) && (I.container() == null)
					...
					if (C != null)
						mob.doCommand(new XVector("PUT", "ALL", C.Name()),
								Command.METAFLAG_FORCED);
					else
						mob.doCommand(new XVector("DROP", "ALL"),
								Command.METAFLAG_FORCED);
					CMLib.tracking().wanderAway(mob, false, true);
				} else if (R != null) {
					Ability A = CMLib.flags().isTracking(mob) ? null : CMClass
							.getAbility("Skill_Track");
					if (A != null)
						A.invoke(
								mob,
								CMParms.parse("\""
			...
			Vector choices = new Vector(
					thisRoom.numItems() < 1000 ? thisRoom.numItems() : 1000);
			for (int i = 0; (i < thisRoom.numItems())
					&& (choices.size() < 1000); i++) {
			...
			if (choices.size() == 0)
				return true;
			Item I = (Item) choices.elementAt(CMLib.dice().roll(1,
					choices.size(), -1));
			if (I != null)
				mob.doCommand(new XVector("GET", I.Name()),
						Command.METAFLAG_FORCED);
			choices.clear();
			choices = null;
		}
