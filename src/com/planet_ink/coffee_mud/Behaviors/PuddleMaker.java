...
public class PuddleMaker extends StdBehavior {
	...
	public void makePuddle(Room R, int oldWeather, int newWeather) {
		for (int i = 0; i < R.numItems(); i++) {
			...
			if ((I instanceof Drink)
					...
					&& ((I.name().toLowerCase().indexOf("puddle") >= 0) || (I
							.name().toLowerCase().indexOf("snow") >= 0)))
				return;
		}
		...
		I.basePhyStats().setDisposition(
				I.basePhyStats().disposition() | PhyStats.IS_UNSAVABLE);
	...
	public boolean tick(Tickable ticking, int tickID) {
				...
				if ((!anyWetWeather(A.getClimateObj().weatherType(null)))
					...
					for (Enumeration<Room> e = A.getProperMap(); e
						...
						if (((R.domainType() & Room.INDOORS) == 0)
								...
								&& (CMLib.dice().rollPercentage() < pct()))
							makePuddle(R, lastWeather, A.getClimateObj()
									.weatherType(null));
					}
			}
		}
		...
		if (ticking instanceof Room)
			lastWeather = ((Room) ticking).getArea().getClimateObj()
					.weatherType((Room) ticking);
		else if (ticking instanceof Area)
