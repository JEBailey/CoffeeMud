...
public class ProtectedCitizens extends ActiveTicker {
	...
	protected static String[] defclaims = { "Help! I'm being attacked!",
			"Help me!!" };
	...
	public MaskingLibrary.CompiledZapperMask getProtectedZapper() {
		...
		if (s.length() == 0) {
			citizenZapper = MaskingLibrary.CompiledZapperMask.EMPTY();
			return citizenZapper;
		}
		char c = ';';
		int x = s.indexOf(c);
	...
	public MaskingLibrary.CompiledZapperMask getCityguardZapper() {
		...
		String s = getParmsNoTicks();
		if (s.length() == 0) {
			helperZapper = MaskingLibrary.CompiledZapperMask.EMPTY();
			return helperZapper;
		}
		char c = ';';
		int x = s.indexOf(c);
	...
	public boolean assistMOB(MOB mob) {
		...
		for (int i = 0; i < mob.location().numInhabitants(); i++) {
			...
			if ((M != null) && (M != mob) && (M.getVictim() == mob.getVictim()))
				assistance++;
		}
		if (assistance >= maxAssistance)
			return true;

		String claim = getClaims()[CMLib.dice().roll(1, getClaims().length, -1)]
				.trim();
		if (claim.startsWith(","))
			...
			if ((M != null)
					...
					&& (CMLib.flags().aliveAwakeMobileUnbound(M, true)
							...
							&& (!BrotherHelper.isBrother(mob.getVictim(), M,
									false)) && (canFreelyBehaveNormal(M))
							&& (!CMLib.flags().isATrackingMonster(M)) && (CMLib
								.flags().canHear(M)))) {
				if (M.location() == thisRoom)
					CMLib.combat().postAttack(M, mob.getVictim(),
							M.fetchWieldedItem());
				else {
					int dir = CMLib.tracking().radiatesFromDir(M.location(),
							rooms);
					if (dir >= 0)
		...
		CMLib.tracking().getRadiantRooms(thisRoom, rooms, flags, null, radius,
				null);
			...
			if (R.getArea().Name().equals(thisRoom.getArea().Name()))
				for (int i = 0; i < R.numInhabitants(); i++) {
					...
					if ((M != null)
							...
							&& (CMLib.flags().aliveAwakeMobileUnbound(M, true)
									&& (!M.isInCombat())
									&& ((CMLib.flags().isMobile(M)) || (M
											.location() == thisRoom))
									&& (!assMOBS.contains(M))
									&& (canFreelyBehaveNormal(M))
									&& (!BrotherHelper.isBrother(
											mob.getVictim(), M, false))
									&& (CMLib.masking().maskCheck(
											getCityguardZapper(), M, false))
									&& (!CMLib.flags().isATrackingMonster(M)) && (CMLib
										.flags().canHear(M)))) {
						...
						if (!notAllowed) {
							...
							if (M.location() == thisRoom)
								...
								int dir = CMLib.tracking().radiatesFromDir(
										M.location(), rooms);
								if (dir >= 0)
									CMLib.tracking().walk(M, dir, false, false);
							}
