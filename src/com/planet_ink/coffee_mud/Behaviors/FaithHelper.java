...
public class FaithHelper extends StdBehavior {
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if ((source != observer)
			...
			if (observer.getWorshipCharID().equalsIgnoreCase(
					target.getWorshipCharID())) {
				String reason = "THAT`S MY FRIEND!! CHARGE!!";
				if ((observer.getWorshipCharID().equals(target
						.getWorshipCharID()))
						&& (!observer.getWorshipCharID().equals(
								source.getWorshipCharID())))
					reason = "BELIEVERS OF "
							+ observer.getWorshipCharID().toUpperCase()
							+ " UNITE! CHARGE!";
