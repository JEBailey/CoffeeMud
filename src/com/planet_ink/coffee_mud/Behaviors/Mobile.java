...
public class Mobile extends ActiveTicker implements MobileBehavior {
	...
	public boolean okRoomForMe(MOB mob, Room currentRoom, Room newRoom) {
		...
		if ((mob.charStats().getBreathables().length > 0)
				&& (Arrays.binarySearch(mob.charStats().getBreathables(),
						newRoom.getAtmosphere()) < 0))
			return false;
			...
			if (s.equalsIgnoreCase("WANDER"))
				...
				if (s.equalsIgnoreCase("+ALL"))
					...
					for (int i = 0; i < Room.indoorDomainDescs.length; i++)
						restrictedLocales
								.add(Integer.valueOf(Room.INDOORS + i));
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((ticking instanceof MOB) && (!((MOB) ticking).isInCombat())
			...
			if (canAct(ticking, tickID)) {
				...
				if ((!CMLib.flags().canWorkOnSomething(mob))
				...
				CMLib.tracking().beMobile((MOB) ticking, dooropen, wander,
						false, objections != null, altStatusTaker, objections);
				if (mob.location() == room)
					tickDown = 0;
			}
		}
