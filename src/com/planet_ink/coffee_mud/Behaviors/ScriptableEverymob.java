...
public class ScriptableEverymob extends StdBehavior implements ScriptingEngine {
	...
	private void giveEveryoneTheScript(Environmental forMe) {
		if ((CMProps.getBoolVar(CMProps.Bool.MUDSTARTED)) && (!started)) {
			started = true;
	...
	public boolean endQuest(PhysicalAgent hostObj, MOB mob, String quest) {
		return (sampleB == null) ? false : sampleB
				.endQuest(hostObj, mob, quest);
	}
	...
	public boolean eval(PhysicalAgent scripted, MOB source,
			...
			int startEval) {
		return (sampleB == null) ? false : sampleB.eval(scripted, source,
				target, monster, primaryItem, secondaryItem, msg, tmp, eval,
				startEval);
	}
	...
	public String execute(PhysicalAgent scripted, MOB source,
		...
		return (sampleB == null) ? "" : sampleB.execute(scripted, source,
				target, monster, primaryItem, secondaryItem, script, msg, tmp);
	}
	...
	public String varify(MOB source, Environmental target,
		...
		return (sampleB == null) ? "" : sampleB.varify(source, target,
				scripted, monster, primaryItem, secondaryItem, msg, tmp,
				varifyable);
	}
	...
	public String functify(PhysicalAgent scripted, MOB source,
		...
		return (sampleB == null) ? "" : sampleB.functify(scripted, source,
				target, monster, primaryItem, secondaryItem, msg, tmp,
				evaluable);
	}
}
