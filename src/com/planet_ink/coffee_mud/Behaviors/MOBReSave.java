...
public class MOBReSave extends ActiveTicker {
	...
	public void setParms(String newParms) {
		...
		for (int c : CharStats.CODES.ALL())
			startStats.setStat(c,
					CMParms.getParmInt(parms, CharStats.CODES.ABBR(c), -1));
	}
	...
	public String getParms() {
		...
		for (int c : CharStats.CODES.ALL())
			rebuiltParms.append(" " + CharStats.CODES.ABBR(c) + "="
					+ M.baseCharStats().getStat(c));
		return rebuiltParms.toString();
	}

	public boolean tick(Tickable ticking, int tickID) {
		if ((ticking instanceof MOB) && (tickID == Tickable.TICKID_MOB)
				&& (!((MOB) ticking).amDead()) && (!noRecurse)
				...
				synchronized (startStats) {
					for (int c : CharStats.CODES.ALL())
						if (startStats.getStat(c) > 0)
							mob.baseCharStats().setStat(c,
									startStats.getStat(c));
					startStats = null;
				}
			}
