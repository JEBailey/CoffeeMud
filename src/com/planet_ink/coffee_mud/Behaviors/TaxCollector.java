...
public class TaxCollector extends StdBehavior {
	...
	public double[] totalMoneyOwed(MOB collector, MOB M) {
		...
		if ((peopleWhoOwe.contains(M.Name())) || (belongsToAnOweingClan(M))) {
			for (int t = 0; t < taxableProperties.size(); t++) {
				LandTitle T = taxableProperties.get(t);
				if ((T.getOwnerName().equals(M.Name()) || (M.getClanRole(T
						.getOwnerName()) != null)) && (T.backTaxes() > 0))
		...
		if (B != null) {
			...
			if ((A2 != null) && (!B.isAnyOfficer(A2, M)) && (!B.isJudge(A2, M))) {
				Law theLaw = B.legalInfo(A2);
				if (theLaw != null) {
					double cittax = CMath.s_double((String) theLaw.taxLaws()
							.get("CITTAX"));
					if (cittax > 0.0)
						owed[OWE_CITIZENTAX] = CMath.mul(
								CMLib.beanCounter()
										.getTotalAbsoluteShopKeepersValue(M,
												collector), CMath.div(cittax,
										100.0));
				}
			}
		} else
			owed[OWE_CITIZENTAX] = CMath.div(CMLib.beanCounter()
					.getTotalAbsoluteShopKeepersValue(M, collector), 10.0);
		owed[OWE_TOTAL] = owed[OWE_CITIZENTAX] + owed[OWE_BACKTAXES]
				+ owed[OWE_FINES];
	...
	public void executeMsg(Environmental oking, CMMsg msg) {
		...
		if ((oking != null) && (oking instanceof MOB)) {
			...
			if (msg.amITarget(mob) && (msg.targetMinor() == CMMsg.TYP_GIVE)
				...
				if (treasuryRoomID != null) {
					...
					if (treasuryR != null) {
						Coins COIN = CMLib.beanCounter().makeBestCurrency(
								...
								paidAmount, treasuryR, treasuryContainer);
						if (COIN != null)
							COIN.putCoinsBack();
					}
				}
				...
				paid.addElement(msg.source(),
						Long.valueOf(System.currentTimeMillis()));

				if (owed[OWE_FINES] > 0) {
					LegalBehavior B = CMLib.law().getLegalBehavior(
							msg.source().location());
					Area A2 = CMLib.law().getLegalObject(
							msg.source().location());
					if ((B != null) && (A2 != null)) {
						...
						if (T.backTaxes() > 0) {
							...
							if (paidAmount >= 0) {
								if (paidAmount >= T.backTaxes()) {
									...
									paidAmount = 0;
									T.setBackTaxes(T.backTaxes()
											- (int) Math.round(paidAmount));
									T.updateTitle();
									break;
								}
							}
				...
				if ((paidBackTaxes) && (numBackTaxesUnpaid == 0)
						&& (mob.location() != null)) {
					LegalBehavior B = CMLib.law().getLegalBehavior(
				...
				if ((paidAmount > 0) && (numProperties > 0))
					for (int i = 0; i < taxableProperties.size(); i++) {
						...
						if (((T.getOwnerName().equals(msg.source().Name())))
								&& (paidAmount > 0)) {
							T.setBackTaxes(T.backTaxes()
									- (int) Math.round(CMath.div(paidAmount,
											numProperties)));
							T.updateTitle();
							paidAmount -= CMath.div(paidAmount, numProperties);
						}
					}
				msg.addTrailerMsg(CMClass.getMsg(mob, msg.source(), null,
						CMMsg.MSG_SPEAK,
						"<S-NAME> says 'Very good.  Your taxes are paid in full.' to <T-NAMESELF>."));
			}
		}
	}
	...
	public boolean okMessage(Environmental oking, CMMsg msg) {
		...
		if (msg.amITarget(mob) && (msg.targetMinor() == CMMsg.TYP_GIVE)
			...
			String owed = CMLib.beanCounter().nameCurrencyShort(currency,
					owe[OWE_TOTAL]);
			if ((!((Coins) msg.tool()).getCurrency().equals(
					CMLib.beanCounter().getCurrency(mob)))) {
				msg.source().tell(
						mob.name(msg.source()) + " refuses your money.");
				CMLib.commands().postSay(mob, msg.source(),
						"I don't accept that kind of currency.", false, false);
				return false;
			}
			if (coins < owe[OWE_TOTAL]) {
				...
				CMLib.commands()
						.postSay(
								mob,
								msg.source(),
								"That's not enough.  You owe " + owed
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((R != null)
			...
			if (theLaw != null) {
				...
				if ((taxs != null) && (taxs.length() > 0)
					...
					taxableProperties = CMLib.law().getAllUniqueTitles(
							A2.getMetroMap(), "*", false);
		...
		if ((R != null) && (!mob.isInCombat())
			...
			if ((M != null)
					...
					&& (!CMSecurity
							.isAllowed(M, R, CMSecurity.SecFlag.CMDROOMS))
					&& (CMLib.clans().findCommonRivalrousClans(mob, M).size() == 0)
					&& (CMLib.flags().canBeSeenBy(M, mob))) {
				int demandDex = demanded.indexOf(M);
				if ((demandDex >= 0)
						&& ((System.currentTimeMillis() - ((Long) demanded
								.elementAt(demandDex, 2)).longValue()) > waitTime)) {
					...
					if (M.isMonster()
						...
						B.accuse(CMLib.law().getLegalObject(R), M, mob,
								new String[] { "TAXEVASION" });
						CMLib.commands()
								.postSay(
										mob,
										M,
										"Can't pay huh?  Well, you'll be hearing from the law -- THAT's for sure!",
										false, false);
					} else {
						CMLib.commands()
								.postSay(
										mob,
										M,
										"You know what they say about death and taxes, so if you won't pay ... DIE!!!!",
										false, false);
						CMLib.combat().postAttack(mob, M,
					...
					if (owe[OWE_FINES] > 1.0)
						say.append("You owe "
								+ CMLib.beanCounter().getDenominationName(
										currency,
										denomination,
										Math.round(CMath.div(owe[OWE_FINES],
												denomination))) + " in fines");
					if (say.length() > 0) {
						CMLib.commands()
								.postSay(
										mob,
										M,
										say.toString()
												+ ".  You must pay me immediately or face the consequences.",
										false, false);
						demanded.addElement(M,
								Long.valueOf(System.currentTimeMillis()));
						if (M.isMonster()) {
							Vector V = new Vector();
							V.addElement("GIVE");
