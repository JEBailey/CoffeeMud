...
public class MudChat extends StdBehavior implements ChattyBehavior {
	...
	public void setParms(String newParms) {
		if (newParms.startsWith("+")) {
			List<String> V = CMParms.parseSemicolons(newParms.substring(1),
					false);
			StringBuffer rsc = new StringBuffer("");
			for (int v = 0; v < V.size(); v++)
				rsc.append(V.get(v) + "\n\r");
			ChattyGroup[] addGroups = parseChatData(rsc);
			ArrayList<ChattyEntry> newList = new ArrayList<ChattyEntry>(
					addedChatEntries.length);
	...
	protected static ChattyGroup newChattyGroup(String name) {
		...
		if ((names.size() == 0) && (masks.size() == 0))
			names.add("");
		return new ChattyGroup(names.toArray(new String[0]),
				masks.toArray(new MaskingLibrary.CompiledZapperMask[0]));
	}
	...
	protected static ChattyGroup[] unprotectedChatGroupLoad(String parms) {
		...
		rsc = (ChattyGroup[]) Resources.getResource("MUDCHAT GROUPS-"
		...
		synchronized (("MUDCHAT GROUPS-" + filename.toLowerCase()).intern()) {
			...
			Resources.submitResource(
					"MUDCHAT GROUPS-" + filename.toLowerCase(), rsc);
			return rsc;
		}
	}
			...
			if (str.length() > 0)
				...
				case '{':
					if (currentChatEntry != null)
						currentChatEntry.responses = currentChatEntryResponses
								.toArray(new ChattyTestResponse[0]);
				...
				case '>':
					if (currentChatEntry != null)
						currentChatEntry.responses = currentChatEntryResponses
								.toArray(new ChattyTestResponse[0]);
					currentChatGroup.entries = currentChatEntries
							.toArray(new ChattyEntry[0]);
					...
					break;
				case '@': {
					otherChatGroup = matchChatGroup(null, str.substring(1)
							.trim(), chatGroups.toArray(new ChattyGroup[0]));
					...
					break;
				case '%': {
					StringBuffer rsc2 = new StringBuffer(Resources
							.getFileResource(str.substring(1).trim(), true)
							.toString());
					if (rsc2.length() < 1) {
						Log.sysOut("MudChat", "Error reading resource "
	...
	protected static ChattyGroup[] loadChatData(String resourceName) {
		StringBuffer rsc = new CMFile(
				Resources.makeFileResourceName(resourceName), null,
	...
	protected static ChattyGroup matchChatGroup(MOB meM, String myName,
			ChattyGroup[] chatGroups) {
	...
	protected ChattyGroup getMyBaseChatGroup(MOB forMe, ChattyGroup[] chatGroups) {
		...
		if (getParms().length() > 0) {
			...
			else if (getParms().substring(x + 1).trim().length() > 0)
				matchedCG = matchChatGroup(forMe, getParms().substring(x + 1),
						chatGroups);
		}
		if (matchedCG != null)
			return matchedCG;
		matchedCG = matchChatGroup(
				forMe,
				CMLib.english().cleanArticles(
						CMStrings.removeColors(myOldName.toUpperCase())),
				chatGroups);
		if (matchedCG != null)
			return matchedCG;
		matchedCG = matchChatGroup(forMe, forMe.charStats().raceName(),
				chatGroups);
		if (matchedCG != null)
			return matchedCG;
		matchedCG = matchChatGroup(forMe, forMe.charStats().getCurrentClass()
				.name(), chatGroups);
		if (matchedCG != null)
			return matchedCG;
		return chatGroups[0];
	...
	protected void queResponse(ArrayList<ChattyTestResponse> responses,
			MOB source, MOB target, String rest) {
		int total = 0;
		...
		if (selection != null) {
			for (String finalCommand : selection.responses) {
				if (finalCommand.trim().length() == 0)
					...
					finalCommand = "sayto \"" + target.name() + "\" "
							+ finalCommand.trim();

				if (finalCommand.indexOf("$r") >= 0)
					finalCommand = CMStrings.replaceAll(finalCommand, "$r",
							rest);
				if ((target != null) && (finalCommand.indexOf("$t") >= 0))
					finalCommand = CMStrings.replaceAll(finalCommand, "$t",
							target.name());
				if ((source != null) && (finalCommand.indexOf("$n") >= 0))
					finalCommand = CMStrings.replaceAll(finalCommand, "$n",
							source.name());
				if (finalCommand.indexOf("$$") >= 0)
					finalCommand = CMStrings
							.replaceAll(finalCommand, "$$", "$");

				Vector<String> V = CMParms.parse(finalCommand);
				for (ChattyResponse R : responseQue)
					if (CMParms.combine(R.parsedCommand, 1).equalsIgnoreCase(
							finalCommand)) {
						V = null;
						break;
					}
	...
	protected boolean match(MOB speaker, String expression, String message,
		...
		if (l == 0)
			return true;
		if ((expression.charAt(0) == '(') && (expression.charAt(l - 1) == ')'))
			expression = expression.substring(1, expression.length() - 1)
					.trim();

		int end = 0;
		for (; ((end < expression.length()) && (("(&|~").indexOf(expression
				.charAt(end)) < 0)); end++) {/* loop */
		}
		String check = null;
		...
		if (check.startsWith("=")) {
			response = check.substring(1).trim()
			...
			while ((++expEnd) < check.length())
				if (check.charAt(expEnd) == '/')
					break;
			response = CMLib.masking().maskCheck(
					check.substring(1, expEnd).trim(), speaker, false);
		...
		if (expression.length() > 0) {
			if (expression.startsWith("(")) {
				...
				if (expEnd < expression.length() && (parenCount <= 0)) {
					return response
							&& match(speaker, expression.substring(1, expEnd)
									.trim(), message, rest);
				}
				return response;
			} else if (expression.startsWith("&"))
				return response
						&& match(speaker, expression.substring(1).trim(),
								message, rest);
			else if (expression.startsWith("|"))
				return response
						|| match(speaker, expression.substring(1).trim(),
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if ((msg.source() == monster) && (msg.sourceMinor() == CMMsg.TYP_SPEAK)
			...
			if ((msg.targetMinor() == CMMsg.TYP_SPEAK)
					&& (msg.amITarget(monster) || ((msg.target() == null)
							&& (mob.location() == monster.location())
							&& (talkDown <= 0) && (mob.location()
							.numPCInhabitants() < 3)))
					&& (CMLib.flags().canBeHeardSpeakingBy(mob, monster))
					&& (myChatGroup != null)
				...
				for (ChattyEntry entry : myChatGroup.entries) {
					...
					if ((expression.charAt(0) == '(')
						...
						if (match(mob, expression.substring(1,
								expression.length() - 1), str, rest)) {
			...
			if ((CMLib.flags().canBeHeardMovingBy(mob, monster))
					&& (CMLib.flags().canBeSeenBy(mob, monster))
					&& (CMLib.flags().canBeSeenBy(monster, mob))
					&& (talkDown <= 0) && (lastReactedTo != msg.source())
					&& (myChatGroup != null)) {
				str = null;
				char c1 = '[';
				char c2 = ']';
					...
					for (ChattyEntry entry : myChatGroup.entries) {
						...
						if ((expression.charAt(0) == c1)
							...
							if (match(mob, expression.substring(1,
									expression.length() - 1), str, rest)) {
								myResponses = new ArrayList<ChattyTestResponse>();
								myResponses.addAll(Arrays
										.asList(entry.responses));
								break;
							}
						}
					}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((tickID == Tickable.TICKID_MOB) && (ticking instanceof MOB)
			...
			if (responseQue.size() == 0)
				...
				for (Iterator<ChattyResponse> riter = responseQue
						.descendingIterator(); riter.hasNext();) {
					ChattyResponse R = riter.next();
					R.delay--;
					if (R.delay <= 0) {
						responseQue.remove(R);
						((MOB) ticking).doCommand(R.parsedCommand,
