...
public class ProtectedCitizen extends ActiveTicker {
	...
	public String getCityguardName() {
		...
		if ((cityguard.indexOf('+') > 0) || (cityguard.indexOf('-') > 0)
				|| (cityguard.indexOf('>') > 0) || (cityguard.indexOf('<') > 0)
				|| (cityguard.indexOf('=') > 0)) {
			zapper = cityguard;
			cityguard = null;
			return zapper;
	...
	public boolean assistMOB(MOB mob) {
		...
		CMLib.tracking()
				.getRadiantRooms(thisRoom, V, flags, null, radius, null);
					...
					if ((M2 != null)
							...
							&& (CMLib.flags().aliveAwakeMobileUnbound(M2, true)
									...
									&& (CMLib.masking().maskCheck(
											getCityguardName(), M2, false))
									&& (!BrotherHelper.isBrother(
											mob.getVictim(), M2, false))
									&& (canFreelyBehaveNormal(M2))
									&& (!CMLib.flags().isATrackingMonster(M2)) && (CMLib
										.flags().canHear(M2)))) {
						M = M2;
						break;
					}
