...
public class Decay extends ActiveTicker {
	...
	public String getParms() {
		...
		if (y < 0)
			y = s.length();
		return ("remain=" + tickDown + " " + s.substring(0, x) + s.substring(y)
				.trim()).trim();
	}
			...
			if (ticking instanceof MOB) {
				...
				if (room != null) {
					...
					if (answer.trim().length() > 0)
						room.showHappens(CMMsg.MSG_OK_VISUAL, mob.name() + " "
								+ answer.trim());
				}
				...
				if (answer.trim().length() > 0) {
					if (E instanceof MOB) {
						...
						((Room) E).showHappens(CMMsg.MSG_OK_VISUAL, item.name()
								+ " " + answer.trim());
				}
				item.destroy();
				room.recoverRoomStats();
		...
		if (msg.amITarget(affecting)) {
			if (affecting instanceof Rideable) {
				if (((msg.targetMinor() == CMMsg.TYP_SLEEP)
						...
						|| (msg.targetMinor() == CMMsg.TYP_MOUNT) || (msg
						.targetMinor() == CMMsg.TYP_ENTER))
