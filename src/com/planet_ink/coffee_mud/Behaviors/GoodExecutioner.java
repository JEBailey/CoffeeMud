	...
	public boolean grantsAggressivenessTo(MOB M) {
		...
		try {
			...
			if (CMLib.flags().isPossiblyAggressive(M))
				return true;
			return ((CMLib.flags().isEvil(M)) || (M.baseCharStats()
					.getCurrentClass().baseClass().equalsIgnoreCase("Thief")));
		} finally {
			norecurse = false;
		}
	}
	...
	public void executeMsg(Environmental affecting, CMMsg msg) {
		...
		if (msg.sourceMinor() == CMMsg.TYP_LIFE) {
			...
			synchronized (protectedOnes) {
				...
				if (x >= 0)
					protectedOnes.setElementAt(x, 2,
							Long.valueOf(System.currentTimeMillis()));
				else
					protectedOnes.addElement(msg.source().Name(),
							Long.valueOf(System.currentTimeMillis()));
			}
		}
		if ((deepBreath == 0)
			...
			synchronized (protectedOnes) {
				for (int p = protectedOnes.size() - 1; p >= 0; p--) {
					if ((System.currentTimeMillis() - ((Long) protectedOnes
							.elementAt(p, 2)).longValue()) > (30 * 1000))
