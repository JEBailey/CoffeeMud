...
public class WaterCurrents extends ActiveTicker {
		...
		if ((R != null) && (R.numInhabitants() > 0)) {
			...
			for (int m = 0; m < R.numInhabitants(); m++) {
				...
				if ((M != null)
						...
						&& ((!(M instanceof Rideable)) || (((Rideable) M)
								.numRiders() == 0)) && (!M.isInCombat())
						&& (!CMLib.flags().isMobile(M)) && (!done.contains(M))) {
					todo.addElement(M);
					done.addElement(M);
				}
			}
			...
			for (int i = 0; i < R.numItems(); i++) {
				...
				if ((I != null)
						...
						&& ((!(I instanceof Rideable))
								|| (((Rideable) I).rideBasis() != Rideable.RIDEABLE_WATER) || (((Rideable) I)
								.numRiders() == 0))
						&& (!CMLib.flags().isInFlight(I))
						&& (!CMLib.flags().isMobile(I)) && (!done.contains(I))) {
					todo.addElement(I);
					done.addElement(I);
				}
			}
					...
					if (R2 != null) {
						if ((R.getExitInDir(dir) != null)
								...
								&& ((R2.domainType() == R.domainType())
										...
										|| (R2.domainType() == Room.DOMAIN_OUTDOORS_UNDERWATER) || (R2
			...
			if (R2 != null) {
				...
				for (int m = 0; m < todo.size(); m++) {
					if (todo.elementAt(m) instanceof MOB) {
						...
						CMMsg themsg = CMClass.getMsg(M, M,
								new AWaterCurrent(), CMMsg.MSG_OK_VISUAL,
								"<S-NAME> <S-IS-ARE> swept "
		...
		if (canAct(ticking, tickID)) {
			...
			if (ticking instanceof Room) {
				...
				if ((below != null)
						...
						&& ((below.domainType() == Room.DOMAIN_INDOORS_UNDERWATER) || (below
								.domainType() == Room.DOMAIN_OUTDOORS_UNDERWATER))) {
					for (Iterator<Room> i = ((GridLocale) below).getAllRooms()
							.iterator(); i.hasNext();) {
						Room R2 = i.next();
	...
	protected static class AWaterCurrent implements Ability, Cloneable {
		...
		public boolean invoke(MOB mob, Physical target, boolean auto,
				int asLevel) {
			return false;
		}
		...
		public void startTickDown(MOB invokerMOB, Physical affected,
				int tickTime) {
		...
		public CMObject copyOf() {
			try {
				...
				// CMClass.bumpCounter(E,CMClass.CMObjectType.ABILITY);//removed
				// for mem & perf
				E.cloneFix(this);
				return E;
		...
		public boolean canAffect(Physical P) {
			...
			if (P == null)
				return false;
			if ((P instanceof MOB)
					&& ((canAffectCode() & Ability.CAN_MOBS) > 0))
				return true;
			if ((P instanceof Item)
					&& ((canAffectCode() & Ability.CAN_ITEMS) > 0))
				return true;
			if ((P instanceof Exit)
					&& ((canAffectCode() & Ability.CAN_EXITS) > 0))
				return true;
			if ((P instanceof Room)
					&& ((canAffectCode() & Ability.CAN_ROOMS) > 0))
				return true;
			if ((P instanceof Area)
					&& ((canAffectCode() & Ability.CAN_AREAS) > 0))
				return true;
			return false;
		}
