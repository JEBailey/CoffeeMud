...
public class VeryAggressive extends Aggressive {
	...
	public String accountForYourself() {
		if (getParms().trim().length() > 0)
			return "surprising aggression against "
					+ CMLib.masking().maskDesc(getParms(), true).toLowerCase();
		else
	...
	public void tickVeryAggressively(Tickable ticking, int tickID,
			boolean wander, boolean mobKiller, boolean misBehave,
			String zapStr, String attackMsg) {
		...
		if (((mob.amFollowing() != null) && (mob.location() == mob
				.amFollowing().location())) || (!CMLib.flags().canTaste(mob)))
			return;
		...
		for (int d = Directions.NUM_DIRECTIONS() - 1; d >= 0; d--) {
			...
			if ((room != null)
					&& (exit != null)
					&& (wander || room.getArea().Name()
							.equals(thisRoom.getArea().Name()))) {
				if (exit.isOpen()) {
					for (int i = 0; i < room.numInhabitants(); i++) {
						...
						if ((inhab != null)
								...
								&& (CMLib.masking().maskCheck(zapStr, inhab,
										false))
								&& ((zapStr.length() > 0) || ((inhab.phyStats()
										.level() < (mob.phyStats().level() + 15)) && (inhab
										.phyStats().level() > (mob.phyStats()
										.level() - 15))))) {
							dirCode = d;
							break;
						}
	...
	public boolean tick(Tickable ticking, int tickID) {
		...
		if ((--tickDown) < 0) {
			tickDown = tickWait;
			tickVeryAggressively(ticking, tickID, wander, mobkill, misbehave,
					getParms(), attackMessage);
		}
		return true;
	}
}
