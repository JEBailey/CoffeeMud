	...
	public static MOB makeMOBfromCorpse(DeadBody corpse, String type) {
		...
		if (corpse != null) {
			...
			mob.baseState().setHitPoints(
					CMLib.dice().rollHP(level, mob.basePhyStats().ability()));
		...
		if ((canAct(ticking, tickID)) && (ticking instanceof MOB)) {
			...
			for (int i = 0; i < thisRoom.numItems(); i++) {
				...
				if ((I != null)
						...
						&& (CMLib.flags().canBeSeenBy(I, mob) || CMLib.flags()
								.canSmell(mob))) {
					if (getParms().length() > 0) {
						...
						mob2.destroy();
					} else if (((DeadBody) I).playerCorpse())
						continue;

					if ((I instanceof Container) && (!EatItems))
						((Container) I).emptyPlease(false);
					thisRoom.show(mob, null, I, CMMsg.MSG_NOISYMOVEMENT,
							"<S-NAME> eat(s) <O-NAME>.");
					I.destroy();
					return true;
				}
