	...
	public int process(StringBuffer buf, int i, Siplet applet,
		...
		if (((String) parts.firstElement()).equalsIgnoreCase("off")) {
			if (tag.equals("MUSIC") && (musicClip != null)) {
				jscriptBuffer.append(musicClip.stopPlaying("musicplayer",
						useExternal));
			}
			if (tag.equals("SOUND") && (soundClip != null)) {
				jscriptBuffer.append(soundClip.stopPlaying("soundplayer",
						useExternal));
			}
		...
		for (int v = 1; v < parts.size(); v++) {
			...
			if ((s.startsWith("C=")) || (s.startsWith("c=")))
				newOne.continueValue = Util.s_int(trimQuotes(s.substring(2))
						.trim());
		...
		if (currentClip != null) {
			jscriptBuffer.append(currentClip.stopPlaying(currentClip.tag
					.equals("MUSIC") ? "musicplayer" : "soundplayer",
					useExternal));
			if (tag.equals("MUSIC"))
				musicClip = null;
			else if (tag.equals("SOUND"))
		...
		jscriptBuffer.append(currentClip.startPlaying(
				tag.equals("MUSIC") ? "musicplayer" : "soundplayer",
				useExternal));
		if (tag.equals("MUSIC"))
