...
public class MiniJSON {
	...
	public static class JSONObject extends TreeMap<String, Object> {
		...
		public JSONObject getCheckedJSONObject(String key)
		...
		public void appendJSONString(final StringBuilder str, Object obj) {
			if (obj instanceof String) {
				str.append("\"").append(toJSONString((String) obj))
						.append("\"");
			} else if (obj == NULL) {
				str.append("null");
	 ...
	 * @param index
	 *            one dimensional array containing current index into the doc
		...
		while (index[0] < doc.length) {
			...
			case HAVEDIGIT:
				if (c == '.')
					...
					index[0]--;
					return Long.valueOf(new String(doc, numStart, index[0]
							- numStart + 1));
				}
				break;
			case NEEDDOT:
			...
			case HAVEDOTDIGIT:
				if (Character.isDigit(c))
					...
					index[0]--;
					return Double.valueOf(new String(doc, numStart, index[0]
							- numStart + 1));
				}
				break;
			case HAVEE:
				if (c == '0')
					throw new MJSONException("Expected non-zero digit at "
							+ index[0]);
				else if (Character.isDigit(c) || (c == '+') || (c == '-'))
					state = NumberParseState.HAVEEDIGIT;
				else
					throw new MJSONException(
							"Expected +- or non-zero digit at " + index[0]);
				break;
			case HAVEEDIGIT:
				if (!Character.isDigit(c)) {
					index[0]--;
					return Double.valueOf(new BigDecimal(new String(doc,
							numStart, index[0] - numStart + 1)).doubleValue());
				}
				break;
			}
			index[0]++;
			...
			if (c == '\"')
				...
				case 'u': {
					if (index[0] >= doc.length - 5)
						throw new MJSONException(
								"Unfinished unicode escape at " + index[0]);
					...
					try {
						...
						throw new MJSONException("Illegal character at"
								+ index[0], e);
					}
					break;
				}
				default:
					throw new MJSONException("Illegal escape character: "
							+ doc[index[0]]);
				}
			} else
				str.append(c);
			...
			if (!Character.isWhitespace(c)) {
				...
				case INITIAL:
					if (c == '[')
						...
						throw new MJSONException("Expected String at "
								+ index[0]);
					break;
				case EXPECTOBJECT:
				...
				case GOTOBJECT:
					if (c == ']')
						...
						throw new MJSONException("Expected ] or , at "
								+ index[0]);
					break;
				}
			}
	...
	private Object parseElement(char[] doc, int[] index) throws MJSONException {
		...
		case 't':
			if ((index[0] < doc.length - 5)
					&& (new String(doc, index[0], 4).equals("true"))) {
				index[0] += 3;
				return Boolean.TRUE;
			}
			throw new MJSONException("Invalid true at " + index[0]);
		case 'f':
			if ((index[0] < doc.length - 6)
					&& (new String(doc, index[0], 5).equals("false"))) {
				index[0] += 4;
		...
		case 'n':
			if ((index[0] < doc.length - 5)
					&& (new String(doc, index[0], 4).equals("null"))) {
				index[0] += 3;
				return NULL;
			}
	...
	private JSONObject parseObject(char[] doc, int[] index)
			throws MJSONException {
			...
			if (!Character.isWhitespace(c)) {
				...
				case INITIAL:
					if (c == '{')
						...
						throw new MJSONException("Expected Key/String at "
								+ index[0]);
					break;
				case NEEDKEY:
					if (c == '\"') {
				...
				case GOTKEY:
					if (c == ':')
						...
						throw new MJSONException("Expected Colon at "
								+ index[0]);
					break;
				case NEEDOBJECT:
