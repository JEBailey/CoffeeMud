...
public class TelnetFilter {
	...
	private String escapeTranslate(String escapeString) {
		if (escapeString.endsWith("m")) {
			Vector V = Util.parseSemicolons(
					escapeString.substring(0, escapeString.length() - 1), true);
			...
			for (int i = 0; i < V.size(); i++) {
				...
				case 36:
				case 37:
					foreground = boldOn ? colorCodes2[code - 30]
							: colorCodes1[code - 30];
					break;
				case 39:
				...
				if ((background != null) || (foreground != null)) {
					...
					if ((!lastBackground().equals(background))
						...
						fontOn = true;
						if (MXPsupport())
							str.append("<FONT COLOR=" + foreground + " BACK="
									+ background + ">");
						else
							str.append("<FONT STYLE=\"color: " + foreground
	...
	public static final char[] mccppattern = { IAC_, IAC_SB, MCCP_COMPRESS2,
			IAC_, IAC_SE };
	public int patDex = 0;

	public void TelnetRead(StringBuffer buf, InputStream rawin,
			BufferedReader in[]) throws InterruptedIOException, IOException {
	...
	public int TelenetFilter(StringBuffer buf, DataOutputStream response,
			InputStream rawin, BufferedReader[] in) throws IOException {
		int i = 0;
		while (i < buf.length()) {
			switch (buf.charAt(i)) {
			case 0:
			...
			case IAC_: {
				...
				case IAC_SB: {
					...
					if (subOptionCode == TELOPT_TTYPE) {
						...
						data[4] = (short) 'e';
						data[5] = (short) 't';
						response.writeBytes("" + IAC_ + IAC_SB + TELOPT_TTYPE
								+ (char) 0);
						response.write(data);
						...
						String received = this.msdpModule
								.msdpReceive(subOptionData.toByteArray());
						synchronized (msdpInforms) {
						...
						String received = this.gmcpModule
								.gmcpReceive(subOptionData.toByteArray());
						synchronized (gmcpInforms) {
				...
				case IAC_WILL:
					...
					if (buf.charAt(i) == TELOPT_NAWS) {
						if (debugTelnetCodes)
							System.out
									.println("Responding with screen size to WILL NAWS.");
						byte[] data = new byte[4];
						data[1] = 80;
						data[3] = 25;
						response.writeBytes("" + IAC_ + IAC_SB + TELOPT_NAWS);
						...
						if (neverSupportMSP == MSPStatus.Disabled) {
							if (MSPsupport()) {
								...
								response.writeBytes("" + IAC_ + IAC_DONT
										+ IAC_MSP);
								response.flush();
								setMSPSupport(false);
							}
						...
						if (neverSupportGMCP) {
							...
							response.writeBytes("core.hello {\"client\":\"siplet\",\"version\":"
									+ Siplet.VERSION_MAJOR + "}");
							response.writeBytes("" + IAC_ + IAC_SE);
						}
						...
						if (neverSupportMCCP) {
							...
							response.writeBytes("" + IAC_ + IAC_DO
									+ MCCP_COMPRESS2);
							response.flush();
							setMCCPSupport(true);
						}
					...
					if (buf.charAt(i) == IAC_MSP) {
						...
						if (neverSupportMSP == MSPStatus.Disabled) {
							if (MSPsupport()) {
								...
								response.writeBytes("" + IAC_ + IAC_WONT
										+ IAC_MSP);
								response.flush();
								setMSPSupport(false);
							}
	...
	public int HTMLFilter(StringBuffer buf) {
		...
		while (i < buf.length()) {
			...
			if (comment) {
				if (((i + 3) < buf.length())
						&& buf.substring(i, i + 3).equals("-->")) {
					comment = false;
					i += 3;
				}
			} else if ((MXPsupport() && (mxpModule.eatTextUntilNextEOLN())
					&& (buf.charAt(i) != '\n') && (buf.charAt(i) != '\r'))) {
				buf.deleteCharAt(i);
				i--;
			} else
					...
					if ((i < buf.length() - 3) && (buf.charAt(i + 1) == '!')) {
						if (MSPsupport()) {
							int endl = mspModule.process(buf, i, codeBase,
									neverSupportMSP == MSPStatus.External);
							if (endl == -1)
								i--;
							else if (endl > 0)
								return endl;
				...
				case '<':
					if (!MXPsupport()) {
						...
						buf.insert(i + 1, "lt;");
						i += 3;
					} else if (((i + 4) < buf.length())
							&& (buf.substring(i + 1, i + 4).equals("!--")))
						comment = true;
					else {
				...
				case '\033': {
					...
					if (buf.charAt(++i) != '[')
						...
						if (translate.equals(oldStr)) {
							int x = mxpModule.escapeTranslate(oldStr, buf,
									savedI);
							if (x == Integer.MAX_VALUE)
								return i;
							i = savedI + x;
		...
		try {
			if (data.startsWith("\\")) {
				...
				if (cmd.equalsIgnoreCase("MSDP")) {
					try {
						byte[] newOutput = this.msdpModule
								.convertStringToMsdp(rest);
						if (newOutput != null)
							return newOutput;
