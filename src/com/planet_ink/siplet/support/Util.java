...
public class Util {
	...
	public static String endWithAPeriod(String str) {
		...
		int x = str.length() - 1;
		while ((x >= 0)
				&& ((Character.isWhitespace(str.charAt(x))) || ((x > 0) && ((str
						.charAt(x) != '^') && (str.charAt(x - 1) == '^') && ((--x) >= 0)))))
			x--;
		if (x < 0)
			return str;
	...
	public static double getParmDouble(String text, String key,
			double defaultValue) {
		int x = text.toUpperCase().indexOf(key.toUpperCase());
		while (x >= 0) {
			if ((x == 0) || (!Character.isLetter(text.charAt(x - 1)))) {
			...
			if ((x == 0) || (!Character.isLetter(text.charAt(x - 1)))) {
				...
				if (x < text.length()) {
					...
					if (x < text.length()) {
						...
						x = 0;
						while ((x < text.length())
								&& ((Character.isLetterOrDigit(text.charAt(x))) || ((endWithQuote) && (text
										.charAt(x) != '\"'))))
							x++;
	...
	public static String replaceAll(String str, String thisStr,
			String withThisStr) {
	...
	public static String replaceAllIgnoreCase(String str, String thisStr,
			String withThisStr) {
		if ((str == null) || (thisStr == null) || (withThisStr == null)
				|| (str.length() == 0) || (thisStr.length() == 0))
			return str;
		for (int i = str.length() - 1; i >= 0; i--) {
			if (Character.toUpperCase(str.charAt(i)) == Character
					.toUpperCase(thisStr.charAt(0)))
				if (str.substring(i).toUpperCase()
						.startsWith(thisStr.toUpperCase())) {
					boolean isUpperCase = Character.isUpperCase(str.charAt(i));
					if (withThisStr.length() > 0)
						withThisStr = (isUpperCase ? Character
								.toUpperCase(withThisStr.charAt(0)) : Character
								.toLowerCase(withThisStr.charAt(0)))
								+ withThisStr.substring(1);
					str = str.substring(0, i) + withThisStr
							+ str.substring(i + thisStr.length());
				}
		}
	...
	public static String replaceFirst(String str, String thisStr,
			String withThisStr) {
		if ((str == null) || (thisStr == null) || (withThisStr == null)
				|| (str.length() == 0) || (thisStr.length() == 0))
			return str;
	...
	public static int numBits(String s) {
		...
		while (i < s.length()) {
			c = s.charAt(i);
			boolean white = (Character.isWhitespace(c) || (c == ' ')
					|| (c == '	') || (c == '\t'));
			if (white
					&& in
					&& (((fc == '\'') && (lc != '\'')) || ((fc == '`') && (lc != '`'))))
	...
	public static String combineAfterIndexWithQuotes(Vector commands,
			String match) {
		StringBuffer Combined = new StringBuffer("");
		if (commands != null)
	...
	public static Vector parse(String str, int upTo) {
		...
		while (!str.equals("")) {
			...
			String CMD = "";
			if ((strIndex >= 0)
					&& ((strIndex < spaceIndex) || (spaceIndex < 0))) {
	...
	public static String stripBadHTMLTags(String s) {
		...
		while ((++i) < buf.length()) {
			...
			case '>':
				...
				if ((quotes.size() == 0) && (start >= 0) && (i - start > 0)
						&& (lastTag != null)
			...
			case '\'':
				if (start < 0)
					break;
				if ((quotes.size() > 0)
						&& (((Character) quotes.lastElement()).charValue() == buf
								.charAt(i)))
	...
	public static String padLeft(String thisStr, String colorPrefix,
			int thisMuch) {
		int lenMinusColors = lengthMinusColors(thisStr);
		if (lenMinusColors > thisMuch)
	...
	public static String padRight(String thisStr, String colorSuffix,
			int thisMuch) {
		int lenMinusColors = lengthMinusColors(thisStr);
		if (lenMinusColors > thisMuch)
	...
	public static String centerPreserve(String thisStr, int thisMuch) {
		...
		int right = ((left + left + lenMinusColors) < thisMuch) ? left + 1
				: left;
