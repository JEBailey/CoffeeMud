...
public class MXPElement implements Cloneable {
	...
	public MXPElement(String newName, String theDefinition,
			String theAttributes, String theFlag, int theBitmap) {
		super();
		name = newName;
	...
	public MXPElement copyOf() {
		try {
			...
			if (E.alternativeAttributes != null)
				E.alternativeAttributes = (Hashtable) E.alternativeAttributes
						.clone();
			if (E.userParms != null)
	...
	public Vector getUnsupportedParms() {
		if ((unsupportedParms == null)
				|| (unsupportedParms.trim().length() == 0))
			return new Vector();
	...
	public synchronized Vector getParsedAttributes() {
		...
		while ((++i) < buf.length()) {
			...
			case '\t':
				if (quotes == '\0') {
					if ((!firstEqual) && (bit.length() > 0))
						parsedAttributes.addElement(bit.toString()
								.toUpperCase().trim());
					bit = new StringBuffer("");
					firstEqual = false;
				} else
		...
		while ((++i) < buf.length()) {
			...
			case '<':
				if (quotes != '\0')
					...
					if (MXP.tagDebug) {
						System.out.println("/TAG/CLOSER2S="
								+ Util.toStringList(tags));
						System.out.flush();
					}
					return tags;
				} else
	...
	public String getFoldedDefinition(String text) {
		...
		if ((userParms != null) && (userParms.size() > 0)) {
			...
			for (int u = 0; u < userParms.size(); u++) {
				...
				if ((xx > 0)
						&& (alternativeAttributes.containsKey(userParm
								.substring(0, xx).trim()))) {
					String newKey = (String) alternativeAttributes.get(userParm
							.substring(0, xx).trim());
				...
				for (int a = 0; a < aV.size(); a++) {
					...
					if ((userParm.startsWith(avParm + "="))
							|| (avParm.equals(userParm))) {
						found = true;
						if (a > position)
							position = a;
						attributeValues.remove(avParm);
						String val = (avParm.equals(userParm)) ? "" : userParms
								.get(u).trim().substring(avParm.length() + 1);
